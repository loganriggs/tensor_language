"""tick 34 (Logan): make use of the FACTORED structure — include the OVK factor readouts, not just Q.

tick 27: combining the two query factors q1,q2 beat either alone (they're near-orthogonal, complementary).
tick 33: the CONTRACTED read z_q=Σ_k s_qk v_k is null (context-contaminated). Logan's idea: use the
FACTOR readouts of T=W_QK1⊗W_QK2⊗W_OV as per-token features (weight-derived, NO context sum, so not
contaminated like z_q). A token's full signature across roles:
  Q (destination): q1=Wq1 x, q2=Wq2 x     K (source select): k1=Wk1 x, k2=Wk2 x     O·V (write): W_OV x
Cluster tokens by progressively more factors; judge by the SAME non-circular intervention as tick 33
(joint override of the head inputs q1,q2,v by the cluster rep). Does including the OVK factors beat Q-only?
Controls: residual x, random. Causal heads. 5 seeds; more data for tighter bars than tick 33.
"""
...
def readouts(L, H):
    ...
    k1, k2 = hq(a.c_k.weight.detach()), hq(a.c_k2.weight.detach())
    vh = (Xn @ a.c_v.weight.detach().T).view(B, Tm, nh, hd)[:, :, H, :]
    Vmix = (1 - float(a.lamb)) * vh + float(a.lamb) * v1_full[:, :, H, :]
    Wo_sl = a.c_proj.weight.detach()[:, H * hd:(H + 1) * hd]            # (d_model, hd)
    OVv = Vmix @ Wo_sl.T                                               # what the token writes into the residual
    return Xn.detach(), q1.detach(), q2.detach(), k1.detach(), k2.detach(), Vmix.detach(), OVv.detach()

def col(f): return torch.stack([f[b, q] for (b, q) in valid])
def n(f): return F.normalize(col(f), 1)
def cmeans(f, L2):
...
def swap_within(L, H, q1, q2, Vm, L2):
    # SENSITIVE intervention: override only the WHERE inputs q1,q2 (tick-27 dynamic range ~+0.007);
    # the clustering FEATURE may include OVK factors — we test if that yields more coherent query groups.
...
print(f"500M two-QK. baseline CE {ce_ref:.4f}, {len(valid)} tokens. Intervention: override q1,q2 ONLY (sensitive, WHERE).\n")
for (L, H) in [(6, 3), (8, 3), (0, 2), (11, 5)]:
    Xn, q1, q2, k1, k2, Vm, OVv = readouts(L, H)
    feats = {
        "q1 only": n(q1),
        "[q1;q2] (two-Q, WHERE)": torch.cat([n(q1), n(q2)], 1),
        "[q1;q2; OVv] (Q + write)": torch.cat([n(q1), n(q2), n(OVv)], 1),
        "[q1;q2;k1;k2;OVv] (full OVK)": torch.cat([n(q1), n(q2), n(k1), n(k2), n(OVv)], 1),
        "[k1;k2;OVv] (OVK source-side)": torch.cat([n(k1), n(k2), n(OVv)], 1),
        "residual x": n(Xn),
    }
    print(f"  --- L{L}H{H} ---")
    for name, fm in feats.items():
        vals = np.array([swap_within(L, H, q1, q2, Vm, labels(fm.cpu().numpy(), s)) for s in range(5)])
        print(f"    {name:32s} {vals.mean():+.4f}±{vals.std():.4f}", flush=True)
    rr = []
    for s in range(5):
    ...
    print(f"    {'random':32s} {np.mean(rr):+.4f}±{np.std(rr):.4f}")
print("DONE")
