...
gate() {
  ...
  while true; do
    ...
    pgrep -f -x "/bin/bash ./tf_symtax_d3fair.sh" > /dev/null && napps=$((napps + 1))
...
push() {
  shopt -s nullglob
  local f=( tf_predicate_*_slot22*_interp3.json out_interp3_*.txt RESULTS.md )
  shopt -u nullglob
  [ ${#f[@]} -eq 0 ] && return 0
...
for D in 3; do
  for CAP in "" "_noqknorm"; do
    for S in 0 1 2; do
      ST="tf_predicate_d${D}_w128_b8192_s${S}_slot22${CAP}"
