...
def main():
    ...
    if OLD:
        ...
        fr = pareto([{'bits': p['bits'], 'ce': p['ce']} for p in old])
    ...
    fa = pareto([{'bits': r['bits_total'], 'ce': r['ce']}
    ...
    if B:
        ...
        fb = pareto([{'bits': r['bits_total'], 'ce': r['ce']} for r in rows])
        ...
        ref = B.get('reference', [])
        if ref:
            fr2 = pareto([{'bits': r['bits_total'], 'ce': r['ce']}
                          for r in ref])
        ...
        if rf:
            b.scatter([r['bits_total'] / 1e6 for r in rf],
                      [r['ce'] for r in rf], marker='*', s=170,
                      color=OI['green'], zorder=6,
                      label='B + coefficients refit on est CE (same bits)')
    b.scatter([bits_model / 1e6], [ce_model], marker='*', s=260,
              color=OI['black'], zorder=7, label='the model itself')
    for y, lab in ((5.204, 'closed-form bigram table (CE 5.204)'),):
        b.axhline(y, color=OI['purple'], lw=1.2, ls='-.')
        b.annotate(lab, (b.get_xlim()[0], y), xytext=(3, 3),
                   textcoords='offset points', fontsize=9, color=OI['purple'])
    b.axhline(ce_model, color=OI['black'], lw=1.2, ls=':')
    b.set_xscale('log')
    b.set_xlabel('TOTAL description length (Mbit, log) -- every table charged')
    b.set_ylabel('held cross-entropy (nats/token)')
    b.set_title('B. The description, on Logan\'s axis\n'
                'cross-entropy against the DATA, not KL from the model',
                fontsize=11, loc='left')
