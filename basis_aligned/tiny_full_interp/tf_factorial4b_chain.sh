...
LOG=tf_factorial4b_chain.log
say() { echo "[$(date -u +%H:%M:%S)] $*" | tee -a "$LOG"; }

push() {
  shopt -s nullglob
  local f=( tff_*d3_w128*.json tfb_std7_d3_w128*.json out_*d3_w128*.txt \
            RESULTS.md GRID.md )
  shopt -u nullglob
  [ ${#f[@]} -eq 0 ] && return 0
...
say "=== depth 3 width 128, CORRECTED coverage (pid $$) ==="
say "stage A: the two cap-off arms the derived chain dropped"
for S in 0 1 2; do fac softmax bilin off "$S"; done
push "depth-3 width-128 factorial: softmax + our feed-forward, cap OFF, three seeds"
for S in 0 1 2; do fac bilin gelu off "$S"; done
push "depth-3 width-128 factorial: our attention + GELU, cap OFF, three seeds -- the cap-off 2x2 is complete"
say "stage B: the cap-on arms"
for S in 0 1 2; do fac softmax bilin on "$S"; done
for S in 0 1 2; do fac bilin gelu on "$S"; done
push "depth-3 width-128 factorial: cap-on arms, three seeds -- THE FULL 2x2x2 AT THIS CELL IS COMPLETE"
...
push "depth-3 width-128 factorial: row-L1 diagnostic, three seeds"
say "=== done ==="
