...
push() {
  shopt -s nullglob
  local f=( tf_predicate_*_slot32*.json out_tf_predicate_*slot32*.txt \
...
# run DEPTH SEED CAP
run() {
  local D=$1 S=$2 CAP=$3 SUF="_slot32" EXTRA=""
  if [ "$CAP" = off ]; then SUF="_slot32_noqknorm"; EXTRA="--no-qk-norm"; fi
  local ST="tf_predicate_d${D}_w128_b8192_s${S}${SUF}"
  if [ -f "${ST}.pt" ]; then
    say "$ST exists -- skip"
  else
    say "training $ST  (depth $D seed $S cap $CAP)"
    python tf_train.py --variant predicate --depth "$D" --width 128 \
      --seed "$S" --slot 32 --suffix "$SUF" $EXTRA --no-sweep \
...
h=subprocess.run(['python','tf_train.py','--help'],capture_output=True,text=True).stdout
sys.exit(0 if '--no-qk-norm' in h else 1)"; then
  say "tf_train.py has NO --no-qk-norm flag -- the cap-off arms cannot be"
  say "trained through it. Running the CAP-ON arms only (seed 2 at both cells)"
  say "and recording the cap-off half as blocked pending a flag."
  CAPS="on"
else
  CAPS="on off"
fi
