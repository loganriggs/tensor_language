...
say "=== three-factor factorial start (pid $$) ==="
...
for SPEC in "softmax:bilin:off" "bilinnorm:bilin:off" "bilin:gelu:off" \
  ...
  run_arm "$A" "$M" "$CAP" 0
  push "three-factor factorial: ${A} attention + ${M} feed-forward, cap ${CAP}, seed 0"
done
say "=== three-factor factorial seed 0 done ==="
