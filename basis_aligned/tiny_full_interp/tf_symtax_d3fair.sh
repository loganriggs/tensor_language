...
push() {
  shopt -s nullglob
  local f=( tf_predicate_*_slot22*.json out_tf_predicate_*slot32*.txt \
...
# run DEPTH SEED CAP
run() {
  local D=$1 S=$2 CAP=$3 SUF="_slot22" EXTRA=""
  if [ "$CAP" = off ]; then SUF="_slot22_noqknorm"; EXTRA="--no-qk-norm"; fi
  ...
  if [ -f "${ST}.pt" ]; then
    ...
    python tf_train.py cell --variant predicate --depth "$D" --width 128 \
      --seed "$S" --slot 22 --suffix "$SUF" $EXTRA --no-sweep \
