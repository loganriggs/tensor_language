...
LOG=tf_secondarch_analysis.log
...
push() {
  shopt -s nullglob
  local f=( tf_predicate_*_slot${SLOT}*_interp3.json out_interp3_*.txt RESULTS.md )
  shopt -u nullglob
  [ ${#f[@]} -eq 0 ] && return 0
...
IFS=: read -r DD W SLOT <<< "$SPEC"
for D in $DD; do
  for CAP in "" "_noqknorm"; do
    for S in 0 1 2; do
      ST="tf_bandwidth_d${D}_w${W}_b8192_s${S}_slot${SLOT}${CAP}"
