...
say "stage 2: the two resized conventional arms bracketing the 7% handicap"
for EXP in 8 9; do
  for S in 0 1 2; do
    ST="tfb_std${EXP}_d3_w64_b8192_s${S}_noqknorm"
    if [ -f "${ST}.pt" ]; then say "$ST exists -- skip"; continue; fi
    say "training $ST (conventional exp $EXP, cap off)"
    python tf_baseline_std.py cell --exp "$EXP" --depth 3 --width 64 --seed "$S" \
      --suffix _noqknorm --no-qk-norm >> "out_${ST}.txt" 2>&1 \
      && say "  trained" || say "  TRAIN FAILED $ST"
  done
done
say "=== symmetry cell done ==="
