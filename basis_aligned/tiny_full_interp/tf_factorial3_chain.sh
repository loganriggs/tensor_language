...
# Predictions: tf_factorial3_predictions.json, registered before this file.
set -u
cd /workspace/tensor_language/basis_aligned/tiny_full_interp || exit 1
source /venv/main/bin/activate
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
export PYTHONUNBUFFERED=1
LOG=tf_factorial3_chain.log
say() { echo "[$(date -u +%H:%M:%S)] $*" | tee -a "$LOG"; }

push() {
  shopt -s nullglob
  local files=( tff_*.json out_tff_*.txt tf_factorial3_predictions.json \
...
run_arm() {
  ...
  local STEM="tff_${A}_${M}_d3_w64_b8192_s${S}${SUF}"
...
say "=== three-factor factorial at DEPTH 3 WIDTH 64 start (pid $$) ==="
...
for SPEC in "softmax:bilin:off" "bilinnorm:bilin:off" "bilin:gelu:off" \
  ...
  for S in 0 1 2; do run_arm "$A" "$M" "$CAP" "$S"; done
  push "three-factor factorial: ${A} attention + ${M} feed-forward, cap ${CAP}, depth 3 width 64, 3 seeds"
done
say "=== depth 3 width 64 factorial done ==="
