...
push() {
  shopt -s nullglob
  local f=( tf_codebook_d*_slot*.json out_tf_codebook_d*_slot*.txt RESULTS.md )
  shopt -u nullglob
  [ ${#f[@]} -eq 0 ] && return 0
...
train_one() {
  ...
  local ST="tf_codebook_d${D}_w${W}_b8192_s${S}${SUF}"
  if [ -f "${ST}.pt" ]; then
    say "$ST exists -- skip"
  else
    say "training $ST  (depth $D width $W slot $SLOT cap $CAP)"
    python tf_train.py cell --variant codebook --depth "$D" --width "$W" \
...
say "=== third foldable architecture (pid $$) ==="
gate
for SPEC in "1:128:61"; do
  ...
  for CAP in on off; do
    ...
    push "third architecture: bandwidth depth $D width $W slot $SLOT, cap $CAP, three seeds"
  done
done
