...
LOG=tf_factorial3_reordered.log
say() { echo "[$(date -u +%H:%M:%S)] $*" | tee -a "$LOG"; }
push() {
  shopt -s nullglob
  local f=( tff_*d3_w64*.json tfb_std7_d3_w64*.json out_*d3_w64*.txt RESULTS.md )
  shopt -u nullglob
  [ ${#f[@]} -eq 0 ] && return 0
...
fac() {  # ATTN MLP CAP SEED
  ...
  local ST="tff_${A}_${M}_d3_w64_b8192_s${S}${SUF}"
  if [ -f "${ST}.pt" ]; then say "$ST exists -- skip"; else
    say "training $ST"
    python tf_factorial.py cell --attn "$A" --mlp "$M" --depth 3 --width 64 \
...
std() {  # SEED   (conventional, matched params, cap off)
  local S=$1 ST="tfb_std7_d3_w64_b8192_s${S}_noqknorm"
  if [ -f "${ST}.pt" ]; then say "$ST exists -- skip"; else
    say "training $ST"
...
say "=== depth-3 factorial REORDERED (pid $$) ==="
say "stage 1: the two cap-off corners -- nothing is computable without them"
for S in 0 1 2; do fac bilin bilin off "$S"; done
push "depth-3 factorial: our model with the cap off, three seeds"
for S in 0 1 2; do std "$S"; done
push "depth-3 factorial: conventional model with the cap off, three seeds -- the 2x2x2 baseline and total move now exist"
say "stage 2: the two remaining 2x2x2 arms, cap on"
for S in 0 1 2; do fac softmax bilin on "$S"; done
push "depth-3 factorial: softmax + our feed-forward, cap on, three seeds"
for S in 0 1 2; do fac bilin gelu on "$S"; done
push "depth-3 factorial: our attention + GELU, cap on, three seeds -- THE 2x2x2 AT DEPTH 3 WIDTH 64 IS COMPLETE"
...
push "depth-3 factorial: row-L1 diagnostic remainder, three seeds"
say "=== depth-3 factorial reordered done ==="
