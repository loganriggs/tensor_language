...
@torch.no_grad()
def main():
 torch.set_num_threads(2);tic=time.perf_counter();stem='CONSUMER_PULLBACK_EFFECT_CPU_V1'
 ...
 metric_control=json.loads((P/'CONSUMER_PULLBACK_V1_CONTROL.json').read_text());assert metric_control['pred_a']
 metric_artifact=torch.load(P/'CONSUMER_PULLBACK_V1_ARTIFACT.pt',weights_only=True,map_location='cpu')
 c['reads'][2]=c['reads'][1]@metric_artifact['projector'].T
