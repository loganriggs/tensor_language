...
class ScalarLoggedTR(TrustRegions):
    def _add_log_entry(self, *, iteration, point, cost, **kwargs):
        if not hasattr(self,'scalar_history'):self.scalar_history=[];self.last_value=float(cost);self.maximum_increase=0.
        self.maximum_increase=max(self.maximum_increase,float(cost)-self.last_value);self.last_value=float(cost)
        # Every five outer iterations, matching the registered diagnostic cadence.
        if iteration%5==0:
            row=dict(iteration=iteration,cost=float(cost));self.scalar_history.append(row)
            if iteration%25==0:print(json.dumps(row),flush=True)


def fit(objective,bank,packed,outputs,seconds=120,tolerance=1e-7):
    manifold=manifold_for(bank,packed);cache={};counts=dict(evaluations=0,hessian_vectors=0)
    ...
    solver=ScalarLoggedTR(min_gradient_norm=tolerance,max_time=seconds,max_iterations=1000,verbosity=0)
...
def control():
    ...
    torch.set_num_threads(2);torch.set_default_dtype(torch.float64);torch.manual_seed(1531)
    ...
    with Path(__file__).with_name('BLOCK_TRUST_REGION_V1_CONTROL.json').open('x') as f:json.dump(result,f,indent=2);f.write('\n')
