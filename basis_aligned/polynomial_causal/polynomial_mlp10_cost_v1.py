...
def main():
    ...
    prepared=prepare(context,l,r,d,bias);three=prepare_three(context,l,r,d,bias);rows=[]
    for batch in (1,3,16,64):
        ...
        functions={'direct':direct,'bank_preparation_included':cold,'bank_reused':warm,'three_preparation_included':lambda:evaluate_three(z,a,prepare_three(context,l,r,d,bias)),'three_reused':lambda:evaluate_three(z,a,three)};samples={k:[] for k in functions}
        ...
        rows.append(dict(batch=batch,relative_error=error,three_relative_error=float((evaluate_three(z,a,three)-ref).norm()/ref.norm()),median_seconds=med,
    ...
    (P/'POLYNOMIAL_MLP10_COST_V1_RESULT.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2))
