...
def channels(p,current8,token_ids):
 ...
 first=F.embedding(indices,p['first_table']).to(current8.dtype).reshape(b,t,1,128)
 ...
 def factor(name):
  x=F.rms_norm(F.linear(current8,p[name].to(current8.dtype)).reshape(b,t,1,128),(128,));a,z=x.chunk(2,-1)
 ...
 v=F.linear(current8,p['current_value'].to(current8.dtype)).reshape(b,t,1,128);v=(1-p['mixture'])*v+p['mixture']*first
