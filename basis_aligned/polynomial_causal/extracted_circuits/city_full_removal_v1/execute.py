"""Donor-free full city-value removal; both native value branches retained."""
import torch
...
def source(p,residual7,token_ids,city,destination,strength=.5):
 ...
 current_value=F.linear(current,head['current_value'])[:,city]
 value=head['mixture']*inherited+(1-head['mixture'])*current_value
 delta=-strength*F.linear(route[...,None]*value[:,None],head['output'])*destination[None,:,None]
