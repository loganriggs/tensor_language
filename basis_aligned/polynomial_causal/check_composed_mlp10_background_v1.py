"""Eight fixed-prefix screen of generating the additive output background."""
...
def main():
    ...
    result=dict(cells=cells,seconds=time.perf_counter()-tic,
                ...
                'Five suffix readouts separate generated-background drift, transported interaction and total output error. No fitting,freshOOD or CPU/GPU equivalence claim. '
                'Small real-state check; does not replace pending160-prefix GPU experiment.')
    (P/'COMPOSED_MLP10_BACKGROUND_V1_SCREEN.json').write_text(json.dumps(result,indent=2)+'\n')
