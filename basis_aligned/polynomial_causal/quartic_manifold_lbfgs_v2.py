"""Projected-transport L-BFGS with direction-angle descent safeguard."""
import time,torch
...
def fit(b,n,evaluate,divisor,max_steps=180,max_seconds=1000,callback=None):
 ...
 for iteration in range(max_steps):
  ...
  direction_norm=float(inner(direction,direction))**.5
  if slope>=-.001*(norm2**.5)*direction_norm:direction=tuple(-x for x in g);slope=-norm2;pairs=[];reset=True
