...
P=Path(__file__).parent;torch.set_num_threads(2);torch.set_grad_enabled(False);start=time.monotonic();VERSION=sys.argv[1] if len(sys.argv)>1 else 'V1';PLAN_NAME=sys.argv[2] if len(sys.argv)>2 else 'TWO_READ_CORRECTION_PLAN_V1.json';assert not (P/f'TWO_READ_CORRECTION_{VERSION}.json').exists();plan=json.loads((P/PLAN_NAME).read_text());parent_receipt=json.loads((P/plan['parent_receipt']).read_text());receipt=parent_receipt['records'][0];edits=parent_receipt['plan']['replacements'];allocation=plan.get('allocation',receipt['allocation']);offsets=np.cumsum([0]+allocation);total=sum(allocation)
...
for j,n in enumerate(allocation):
 e,U=torch.linalg.eigh(S@(metric.true[2*j+1]-H0[2*j+1])@S);ix=e.abs().argsort(descending=True)[:max(14,n)];V=inv@U[:,ix]
 for edit in edits:
...
X=torch.zeros(total,2,dtype=H0.dtype)
if 'initial_amplitudes' in plan:
 prior=json.loads((P/plan['initial_amplitudes']).read_text());old=torch.tensor(prior['amplitudes'],dtype=H0.dtype);old_offsets=np.cumsum([0]+plan['initial_allocation'])
 for j,n in enumerate(plan['initial_allocation']):X[offsets[j]:offsets[j]+n]=old[old_offsets[j]:old_offsets[j+1]]
else:X[:,1]=torch.tensor(receipt['solver']['x'],dtype=H0.dtype)
def forms(X):
 H=H0.clone()
 for j in range(3):
...
initial=float(metric.ratios_full(forms(X)).max())
if 'initial_amplitudes' in plan:assert abs(initial-prior['history'][-1]['maximum'])<1e-8
history=[];sweep_start=initial
for step in range(plan['steps']):
 ...
 if accepted:X=trial
 graph=export(X);scores=audit.assess(graph);replay=float(np.max(np.abs(score_ratios(scores)-metric.ratios_full(forms(X)).numpy())));assert replay<1e-8;assert scores['source_total_multiplications']==1047648+1155*total<=plan.get('source_ceiling',1064448)
 ...
 (P/f'TWO_READ_CORRECTION_PARTIAL_{VERSION}.json').write_text(json.dumps(dict(history=history,amplitudes=X.tolist()),indent=2)+'\n')
...
(P/f'TWO_READ_CORRECTION_{VERSION}.json').write_text(json.dumps(out,indent=2)+'\n')

if out['predictions']['fidelity']:torch.save(export(X),P/f'TWO_READ_CORRECTION_PROGRAM_{VERSION}.pt')
