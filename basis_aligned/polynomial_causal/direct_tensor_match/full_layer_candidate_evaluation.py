#!/usr/bin/env python3
# Reusable native candidate evaluation; execution is requested by managed wrappers.
"""Frozen learned-factor primary: direct source swaps and natural function.
...
Natural fullMLP effecterror<10% and CEadded<.02 per domain. Attention17 held recipient.
Selected-by-training-loss learned-factor program3686products/14,067,072coefficients, no fit,48captures.
"""
import os,sys,json,time,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal/direct_tensor_match'
def evaluate(program_file,output_file):
 ...
 out=P/output_file;assert not out.exists()
 e={k:v.cuda().double() for k,v in torch.load(P/'MIDPOINT_CONTINUATION_NATIVE_OPERATOR_V1.pt',weights_only=True).items()};p={k:v.cuda() for k,v in torch.load(P/program_file,weights_only=True).items()}
 ...
 model=Bilin18TorchBackend.load('cuda').model.float();b16,b17=model.transformer.h[16],model.transformer.h[17];records=[];checks=[];scale_rows=[];natural=[]
 ...
 for domain,documents in tokens.items():
  ...
  for doc,row in enumerate(documents):
   ...
   base=logits(cache['final'])[:,16:255];changed=logits(endpoint(h,'student'))[:,16:255];effect=base-logits(h+b17.mlp.Down_bias)[:,16:255];target=row[17:256].cuda()[None];natural.append(dict(domain=domain,document_index=doc,error_energy=float((changed-base).double().square().sum()),reference_energy=float(effect.double().square().sum()),ce_added=float(F.cross_entropy(changed.flatten(0,1),target.flatten())-F.cross_entropy(base.flatten(0,1),target.flatten()))))
 ...
 for domain in tokens:
  rr=[r for r in natural if r['domain']==domain];natural_summary.append(dict(domain=domain,effect_error=(sum(r['error_energy'] for r in rr)/sum(r['reference_energy'] for r in rr))**.5,ce_added=sum(r['ce_added'] for r in rr)/len(rr)))
 primary=[r for r in summary if r['family']=='same_cohort'];pred=dict(pred_a_replay=max(checks)<1e-5,pred_b_full=all(r['effect_error'] is not None and r['effect_error']<.1 for r in primary if r['kind']=='full'),pred_c_mode=all(r['effect_error'] is not None and r['effect_error']<.1 for r in primary if r['kind']=='mode'),pred_d_natural=all(r['effect_error']<.1 and r['ce_added']<.02 for r in natural_summary))
 result=dict(program_file=program_file,predictions=pred,summary=summary,records=records,natural_records=natural,natural_summary=natural_summary,rms_ratios=scale_rows,maximum_replay=max(checks),seconds=time.monotonic()-start,scope='Path-specific MLP16 polynomial residual swap, recipient attention17 and other carry held fixed; h17/RMS17/MLP17/final RMS/softcap recomputed. Full effect compares true direct-path endpoints. Mode effect separately isolates scalar mode delta in each original background, not a claim to explain the whole path effect. Opened token-matched panel, no new fit/fresh validation. Upstream source extraction still external.')
 out.write_text(json.dumps(result,indent=2)+'\n');print(json.dumps({k:v for k,v in result.items() if k not in ('records','rms_ratios','natural_records')}),flush=True)
