...
parser=argparse.ArgumentParser();parser.add_argument("--version",type=int,default=1,choices=[1,2]);version=parser.parse_args().version
P=Path(__file__).resolve().parent;d=json.loads((P/f'PARTIAL_GRAPH_FRESH_NATIVE_V{version}.json').read_text());rng=np.random.default_rng(729);records=[]
for cell in d['cells']:
