"""Expanded calibration source-moment readout, same fixed graph and coefficient penalty."""
...
expanded=torch.load(P/'EXPANDED_COVARIANCE_STATES_V1.pt',weights_only=True)
oldtokens=torch.load(P/'MIDPOINT_SOURCE_FOLD_CALIBRATION_V1.pt',weights_only=True)['tokens']
assert not({tuple(row[:64].tolist()) for row in expanded['tokens']}&{tuple(row[:64].tolist()) for row in oldtokens})
calibration_z=torch.cat([d['z'][:1536],expanded['z'].flatten(0,1).double()])
assert len(calibration_z)==16384
x=(calibration_z-d['mu'])@d['inverse_root']
...
selected=next(r for r in records if r['key']=='source_1');control=next(r for r in records if r['key']=='source_0');baseline=[.03058409729022641,.027558449717507608,.11941807478056159]
out=dict(records=records,primary='source_1',predictions=dict(pred_a_instrument=max(max(r['normal_equation_replay'],r['execution_replay']) for r in records)<1e-8,pred_b_component_values=all(a<=.15 and a<=1.10*b for a,b in zip(selected['opened']['per_mode_errors'],baseline)),pred_c_coefficient_fidelity=selected['original_coefficient_error']<=1.1*control['original_coefficient_error']),seconds=time.perf_counter()-start,scope='Same399product fixed directions and positive original coefficient penalty; source-only empirical metric now16384states from old24plus232additionalprefixes, alladditionalprefixes excludedfromold32. Opened448unchanged; training score refersold1536only because full h unavailable elsewhere. Compareprevioussource-family fits atsame lambdas; no fresh/OOD evidence or model forwards.')
torch.save(programs,P/'EXPANDED_EMPIRICAL_READOUT_PROGRAMS_V1.pt');(P/'EXPANDED_EMPIRICAL_READOUT_V1.json').write_text(json.dumps(out,indent=2)+'\n');print(out['predictions'])
