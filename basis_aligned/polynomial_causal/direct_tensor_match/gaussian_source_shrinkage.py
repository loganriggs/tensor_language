...
root=torch.linalg.inv(c['source_inverse_root']);cov=root@root;originalQs={}
...
for rho in [0.,.01,.1,.5,1.]:
 rank=64
 covariance=(1-rho)*cov+rho*cov.trace()/len(cov)*torch.eye(len(cov),dtype=cov.dtype)
 evc,Vc=torch.linalg.eigh(covariance);shrunkroot=(Vc*evc.sqrt())@Vc.T;shrunkinv=(Vc*evc.rsqrt())@Vc.T
 spectra={}
 for k,Q in originalQs.items():
  ev,V=torch.linalg.eigh(shrunkroot@Q@shrunkroot);ids=ev.abs().argsort(descending=True)
  spectra[k]=(ev[ids],root@shrunkinv@V[:,ids])
 factors=[];reads=[]
 ...
 row=dict(covariance_shrinkage=rho,rank_per_source=rank,source_products=2*rank,stored_scalars=2*1152*rank+2*rank+4*1152+4,gaussian_numerator_variation_error=gaussian,**errors);rows.append(row);print(row,flush=True)
primary=next(r for r in rows if r['covariance_shrinkage']==.1)
out=dict(predictions=dict(pred_a_unshrunk_replay=abs(rows[0]['opened_distinct_normalized_error']-.3516988017360602)<1e-7,pred_b_shrinkage=primary['opened_distinct_normalized_error']<=min(.15,.8*rows[0]['opened_distinct_normalized_error'])),records=rows,seconds=time.perf_counter()-start,scope='Fixedrank64 spectral source truncation with covariance shrinkage toward scaled identity. Primaryrho.1; no joint refit. Training24 mean/centeredaffinebranches fixed. Native evaluation on seven previouslyopened distinct rows. Prices include sourceprogram and fixed downstream readers/writer, exclude native input producers and common RMS computation.')
(P/'GAUSSIAN_SOURCE_SHRINKAGE_V1.json').write_text(json.dumps(out,indent=2)+'\n')
