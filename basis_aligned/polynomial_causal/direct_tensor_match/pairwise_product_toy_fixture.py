...
def fixture(case,mixed=False):
 ...
 for j,(a,b) in enumerate(GROUPS):
  selected=torch.arange(0,n,2) if mixed else torch.arange(k);other=torch.arange(1,n,2) if mixed else torch.arange(k,n);templates.append(dict(shared_indices=selected,private_indices=other,product_indices=indices.clone()))
  reader=torch.zeros(d,n,dtype=torch.float64);reader[:,selected]=torch.cat([bases[a],bases[b]],1)@maps[j];reader[:,other]=private[j];L,R=product_factors(reader,indices);W=rand(n,2);raw=torch.einsum('ir,ro,jr->oij',L,W,R);targets.append((raw+raw.transpose(-1,-2))/2)
