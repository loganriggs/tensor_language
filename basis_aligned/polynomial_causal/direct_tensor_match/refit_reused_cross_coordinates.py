...
plan=json.loads((P/'REUSED_CROSS_REFIT_PLAN_V1.json').read_text());start=time.monotonic();items,outside,meta=problem();scores=torch.stack([x['theta'].square().sum(0)/x['norm'] for x in items]);mask=torch.zeros_like(scores,dtype=torch.bool).flatten();mask[scores.flatten().topk(plan['positions']).indices]=True;mask=mask.reshape_as(scores);error=outside;rows=[]
...
(P/'REUSED_CROSS_REFIT_V1.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
