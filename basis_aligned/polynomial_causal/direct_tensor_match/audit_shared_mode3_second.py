...
data=json.loads((P/'SHARED_MODE3_SECOND_NATIVE_V1.json').read_text());records=data['records'];rng=np.random.default_rng(612)
rows=[]
for domain in ['fineweb','stdlib']:
 ...
 for family in ['natural','hybrid','change']:
  for cohort in ['all','continuation','spaced_word']:
   columns=[]
   for name in ['independent128','shared_plain256']:
    values=[]
    for doc in docs:
...
out=dict(records=rows,replicates=4000,scope='Paired bootstrap over recipient document identities for FW and source-file identities for newcode. Donors and candidate selection frozen; intervals do not include donor-map uncertainty or fullmodel/semantic uncertainty.')
(P/'SHARED_MODE3_SECOND_AUDIT_V1.json').write_text(json.dumps(out,indent=2)+'\n')
for row in rows:
