...
e=torch.load(p/'MIDPOINT_COVERAGE_256_R4_V1.pt',weights_only=True);rows=torch.load(p/'MIDPOINT_CALIBRATION_ROWS_V1.pt',weights_only=True);n=rows['n'].double();m=rows['m'].double();y=rows['y'].double();S=torch.load(p/'MIDPOINT_CENTERED_V1.pt',weights_only=True)['metric_sqrt'].double();A=e['A'].double();B=e['B'].double();phi=(n@A)*(m@B);rates=[0.,1e-4,.001,.01,.1,1.];records=[]
def fit(x,y,rate):
...
out=p/'MIDPOINT_PRODUCT_READOUT_REFIT_V1.json';assert not out.exists();out.write_text(json.dumps(dict(records=records,mean_validation_error=means,chosen_ridge=chosen,scope='Conditional output-refit validation on complementary originalcalibration16document halves. Product directions were fixed by originalweightfit with fullcalibration marginal metric; these are not independent direction-validation data. Rates chosen without native diagnostic outcomes. Wholefullcalibration refit exported; native evidence pending.'),indent=2)+'\n');torch.save({'ungrouped':program},p/'MIDPOINT_PRODUCT_READOUT_REFIT_V1.pt');print(out.read_text())
