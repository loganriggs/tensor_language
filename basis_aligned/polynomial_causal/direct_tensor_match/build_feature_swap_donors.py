...
def main():
 ...
 for domain in ['fineweb','code']:
  tokens=torch.load(P/f'BLEND_CONFIRMATION_{domain.upper()}_V1.pt',weights_only=True);n,t=tokens.shape[0],256;flat=tokens[:,:t].reshape(-1);doc=torch.arange(n).repeat_interleave(t);pos=torch.arange(t).repeat(n);eligible=pos>=16;aligned=((doc+1)%n)*t+pos;aligned[~eligible]=-1;same=torch.full_like(aligned,-1)
 ...
 torch.save(maps,P/'FEATURE_SWAP_DONORS_V1.pt');(P/'FEATURE_SWAP_DONORS_V1.json').write_text(json.dumps(dict(records=records,selection='Recipient positions16:256. Aligned:next document at same position. Same-token:closest-position occurrence in another document, ties by flattened index. No activation/output selection.',scope='Previously used confirmation panels, new intervention definition; no independent OOD claim.'),indent=2)+'\n');print(records)
