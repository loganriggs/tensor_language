"""Train-only source geometry for a 24/8 document adaptation diagnostic."""
from pathlib import Path
import torch,json
from quartic_pair_metric import inner
p=Path(__file__).resolve().parent;torch.set_num_threads(2);torch.set_grad_enabled(False);cal=torch.load(p/'MIDPOINT_SOURCE_FOLD_CALIBRATION_V1.pt',weights_only=True);rows=torch.load(p/'MIDPOINT_CALIBRATION_ROWS_V1.pt',weights_only=True);mode=torch.load(p/'MIDPOINT_NATIVE_OBSERVER_MODES_V1.pt',weights_only=True);z=cal['z'].flatten(0,1).double();train_count=24*64;mu=z[:train_count].mean(0);delta=z-mu;cov=delta[:train_count].T@delta[:train_count]/train_count;v,V=torch.linalg.eigh(cov);assert v.min()>0;root=(V*v.sqrt())@V.T;inv=(V*v.rsqrt())@V.T;d=z.shape[1];D=d+3
...
joint=torch.cat([delta,(t-t[:train_count].mean())[:,None],(scale-scale[:train_count].mean())[:,None]],1);jcov=joint[:train_count].T@joint[:train_count]/train_count;ev,U=torch.linalg.eigh(jcov);jroot=(U*ev.clamp_min(0).sqrt())@U.T;frames={};checks=[]
for metric in ['covariance']:
 H=torch.eye(D,dtype=torch.float64)
 if metric=='covariance':H[:-1,:-1]=jroot
 H[d,-1]=t[:train_count].mean();H[d+1,-1]=scale[:train_count].mean();A=H.T@teacherA@H;B=H.T@teacherB@H;entry=dict(A=A,B=B,J=H.T[:,:d]@inv,unit=H[-1].clone(),teacher_norm=inner(A,B,A,B))
...
ph=((t-.5*pred[0])/scale-alpha)*(pred[1]/scale-beta);baseline=float((ph-phi).norm()/(phi-phi.mean()).norm());assert max(checks)<1e-8
...
torch.save(dict(common=common,frames=frames,whitened_delta=delta@inv,affine=torch.stack([sources[k]['constant']+delta@sources[k]['linear'] for k in ['a','b']],1)),p/'FUNCTIONAL_QUARTIC_MODE3_INPUTS_V1.pt');result=dict(frame_replay=max(checks),initial_native_mode_variation_error=baseline,rank_per_source=16,augmented_dimension=D,parameter_count=2*d*16,negative_joint_covariance_eigenvalues=int((ev<0).sum()),train_documents=list(range(24)),validation_documents=list(range(24,32)),scope='All source geometry and initialization recomputed on24trainingdocuments. Native mode3 discovery used all32documents, so remaining8 are heldout from this adaptation only, not fresh discovery. Full empirical normalized/numerator functional objectives will be compared with coefficient geometry at fixedcapacity.');(p/'FUNCTIONAL_QUARTIC_MODE3_PREP_V1.json').write_text(json.dumps(result,indent=2)+'\n');print(result)
