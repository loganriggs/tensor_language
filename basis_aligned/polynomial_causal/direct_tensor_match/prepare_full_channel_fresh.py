...
for row in torch.load(P/'FRONTIER_FRESH_TOKENS_V2.pt',weights_only=True)['fineweb']:known.add(tuple(row[:32].tolist()))
previous+=json.loads((P/'FRONTIER_FRESH_ROWS_V2.json').read_text())['fineweb_documents']
byfirst={}
...
excluded.update(v['file'] for v in json.loads((P/'FRONTIER_FRESH_ROWS_V2.json').read_text())['code_sources'])
...
torch.save(tokens,P/'FULL_CHANNEL_FRESH_TOKENS_V1.pt')
out=dict(fineweb_documents=metadata,code_sources=code_sources,known_cached_prefixes=len(known),rejections=rejections,source='HuggingFaceFW/fineweb sample-10BT train stream',scope='New document-indexed panel for frozen full lastMLP channel baseline. Excludes eight prior FineWeb panels, cached excerpts and corresponding stdlib files. One prefix per document/file. Not a pretraining or project-level independence guarantee. No fitting on this panel.')
(P/'FULL_CHANNEL_FRESH_ROWS_V1.json').write_text(json.dumps(out,indent=2)+'\n')
