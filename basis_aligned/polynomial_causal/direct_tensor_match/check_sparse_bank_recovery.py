...
def main():
 ...
 for s,family in enumerate(['independent','shared_inputs','shared_outputs','squares','cancellation']):
  ...
  target=directional(*teacher,[eye[idx[:,i]] for i in range(4)]);energy=target.square().sum()
  def objective(u,v):return profile(gram(u,v,pairs),native_cross(teacher,u,v,pairs),ridge=1e-8)
  ...
  for optimizer in ['adam','muon']:
   for restart in [0,1]:
    torch.manual_seed(10400+restart);params=[torch.nn.Parameter(torch.randn(4,d,dtype=dtype)/2) for _ in range(2)]
    ...
    for step in range(401):
     ...
     if best is None or score<best[0]:
      with torch.no_grad():error=float((entries(u,v,pairs,idx)@c.T-target).norm()/target.norm())
 ...
 result=dict(rows=rows,summary=summary,recovery_gate=any(v['below1percent']>=8 for v in summary.values()),scope='Known representable selected-pair bank,400steps/.1rate/cosine1pct,2starts; different teachers than CP suite, not cross-family optimizer league table.')
 (P/'SPARSE_QUARTIC_BANK_RECOVERY_V1.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(summary),flush=True)
