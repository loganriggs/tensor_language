...
def fit(phi,y,w):
    scale=phi.square().mean(0).sqrt().clamp_min(1e-30)
    ws=(w/w.mean()).sqrt();a=phi/scale*ws[:,None];b=y*ws
    result=torch.linalg.lstsq(a,b,driver='gelsd',rcond=1e-12)
    c=result.solution/scale;res=a@result.solution-b
    stationarity=float((a.T@res).norm()/(a.norm()*b.norm()).clamp_min(1e-30))
    assert stationarity<1e-10
    return c,dict(rank=int(result.rank),relative_smallest_singular=float(result.singular_values[-1]/result.singular_values[0]),stationarity=stationarity)
...
def main():
    ...
    for seed in [1001,1002]:
        program=torch.load(P/f'QUARTIC_CP512_SEED{seed}_V2.pt',weights_only=True)
        phis=[]
        for x in xs:
            phi=torch.ones(len(x),512,dtype=torch.float64)
            for f in program['factors']:phi*=x@f.double().T
            phis.append(phi)
