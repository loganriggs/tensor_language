...
def main():
 ...
 for name,(Gt,bt,ct) in models.items():
  ...
  scaling=1/np.sqrt(np.maximum(np.diag(A)/C,1e-18));fit=minimize(lambda y:objective(scaling*y)/C,np.zeros(32),jac=lambda y:scaling*derivative(scaling*y)/C,method='SLSQP',constraints=[dict(type='ineq',fun=lambda y:constraints(scaling*y),jac=lambda y:jac(scaling*y)*scaling[None,:])],options=dict(maxiter=1500,ftol=1e-12));fit.x=scaling*fit.x;x=torch.tensor(fit.x,dtype=V.dtype);graph=copy.deepcopy(p['graph']);directions=graph['pairs']['2']['private_reader'][:,-32:];graph['pairs']['2']['product_weights'][-32:,1]+=x/directions.norm(dim=0).square();graph=Assessment(d).correct(graph);assessment=Assessment(d).assess(graph);H=p['H'].clone();H[5]+=torch.einsum('k,kij->ij',x,p['atoms']);replay=float((decode(expand(graph)['2'])[1]-H[5]).norm()/H[5].norm());assert replay<1e-10
  scores={k:dict(before=float(cc.sqrt()),after=float((x@gg@x+2*bb@x+cc).sqrt())) for k,(gg,bb,cc) in models.items()};row=dict(objective=name,coordinate_scaling=scaling.tolist(),success=bool(fit.success),iterations=int(fit.nit),message=fit.message,max_constraint_violation=float(max(0,-constraints(fit.x).min())),scores=scores,assessment=assessment,export_replay=replay,delta_weights=fit.x.tolist());rows.append(row)
  if row['success'] and row['max_constraint_violation']<1e-8:torch.save(graph,P/f'INTERCHANGE_READ_{name.upper()}_V2.pt')
 ...
 (P/'INTERCHANGE_READ_V2.json').write_text(json.dumps(out,indent=2)+'\n')
