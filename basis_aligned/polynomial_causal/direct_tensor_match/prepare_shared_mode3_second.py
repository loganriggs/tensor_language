...
for row in torch.load(P/'SHARED_MODE3_FRESH_TOKENS_V1.pt',weights_only=True)['fineweb']:known.add(tuple(row[:32].tolist()))
previous=json.loads((P/'SHARED_MODE3_FRESH_ROWS_V1.json').read_text())['fineweb_documents']
byfirst={}
...
for path in sorted(Path(sysconfig.get_path('stdlib')).glob('*.py')):
...
oldcode=torch.load(P/'MIDPOINT_STDLIB_CONTINUATION_TOKENS_V1.pt',weights_only=True)
...
torch.save(tokens,P/'SHARED_MODE3_SECOND_TOKENS_V1.pt')
out=dict(fineweb_documents=metadata,code_sources=code_sources,known_cached_prefixes=len(known),rejections=rejections,source='HuggingFaceFW/fineweb sample-10BT train stream',scope='New document-indexed panel for this extraction study, one prefix perdocument and no knowncachedFineWeb excerpt anywhere in eachaccepted document. Not a pretraining-overlap or whole-project novelty guarantee. Second panel: previousFWdocumentIDs/texts/prefixes excluded;16newstdlibfiles. Plain256 primary selected before these outcomes.')
(P/'SHARED_MODE3_SECOND_ROWS_V1.json').write_text(json.dumps(out,indent=2)+'\n')
