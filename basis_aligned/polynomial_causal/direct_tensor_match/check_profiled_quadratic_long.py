"""Same random starts and structures; 3000-step Adam .05 schedule extension."""
from pathlib import Path
import json,time,torch
from learned_quadratic_factors import controls,dense
from profiled_learned_quadratic_factors import fit
p=Path(__file__).resolve().parent;out=p/'PROFILED_FULL_QUADRATIC_LONG_V1.json';assert not out.exists();torch.set_num_threads(2);start=time.monotonic();check=controls();records=[]
for index,name in enumerate(['independent_products','shared_input','shared_output','squares','cancelling_terms']):
 ...
 for optimizer in ('adam',):
  for lr in (.05,):
   for seed in (0,1):
    result,_=fit((c,a,b),k,optimizer,lr,960+seed,3000,x,v);result.update(family=name,optimizer=optimizer,lr=lr,seed=seed);records.append(result);print(json.dumps({k:v for k,v in result.items() if k!='history'}),flush=True)
configs=[]
for optimizer in ('adam',):
 for lr in (.05,):
...
result=dict(predictions=pred,records=records,controls=check,configurations=configs,selected=selected,seconds=time.monotonic()-start,scope='Five structural families; random initial student at planted minimal width4, teacher may have redundant cancelling terms. Exact symmetric coefficient plus artificial directional-response objective; conditional output solves each step, tiny1e-10relative Gram ridge. Selection minimizes worst then mean coefficient error across all10family/start cases. Functional recovery does not identify unique factors or prove native optimizer superiority.')
