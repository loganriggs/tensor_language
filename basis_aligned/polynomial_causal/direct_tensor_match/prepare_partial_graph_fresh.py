...
torch.save(tokens,P/'PARTIAL_GRAPH_FRESH_TOKENS_V1.pt')
out=dict(fineweb_documents=metadata,code_sources=code_sources,known_cached_prefixes=len(known),rejections=rejections,source='HuggingFaceFW/fineweb sample-10BT train stream',scope='New document-indexed panel for this extraction study, one prefix perdocument and no knowncachedFineWeb excerpt anywhere in eachaccepted document. Not a pretraining-overlap or whole-project novelty guarantee. Third panel: bothprevious FWdocumentIDs/texts/prefixes and32previousstdlibfiles excluded. Profiledpartialgraph chosen bycoefficientobjective andfrozen before these outcomes.')
(P/'PARTIAL_GRAPH_FRESH_ROWS_V1.json').write_text(json.dumps(out,indent=2)+'\n')
