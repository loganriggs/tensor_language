...
def main():
    ...
    for family_index, family in enumerate(['independent', 'shared_inputs', 'shared_outputs', 'squares', 'cancellation']):
        ...
        for optimizer in ['adam','muon']:
            for restart in [0,1]:
                ...
                parameters = [torch.nn.Parameter(torch.randn(3,4,dtype=dtype)/2) for _ in range(4)]
                opt = torch.optim.Adam(parameters,lr=.03) if optimizer=='adam' else torch.optim.Muon(parameters,lr=.03,weight_decay=0.,adjust_lr_fn='match_rms_adamw')
                ...
                for step in range(401):
                    ...
                    for group in opt.param_groups:group['lr']=.03*(.01+.99*.5*(1+math.cos(math.pi*(step+1)/400)))
                row=dict(family=family,optimizer=optimizer,restart=restart,initial_error=initial,error=best[2],selected_step=best[1],seconds=time.monotonic()-start)
    ...
    (P/'QUARTIC_CP_PROFILE_CONTROLS_V1.json').write_text(json.dumps(result,indent=2)+'\n')
