...
def fixture(seed,mixed=False):
 ...
 for j in range(3):
  selected=torch.tensor(sorted([0,2,4,1][:k]),dtype=torch.int64) if mixed else torch.arange(k);private=torch.tensor([i for i in range(n) if i not in selected.tolist()],dtype=torch.int64)
  if seed==0 and not mixed:indices=torch.stack([torch.arange(n),torch.arange(n),torch.zeros(n,dtype=torch.int64)])
  else:
...
def main(mixed=False):
 records=[]
 for seed in range(5):
  targets,templates,truth=fixture(seed,mixed=mixed);metric=OverlapMetric(targets,templates)
 ...
 (P/('OVERLAP_CROSS_VARPRO_PREFLIGHT_V1.json' if mixed else 'OVERLAP_VARPRO_PREFLIGHT_V1.json')).write_text(json.dumps(dict(controls=records),indent=2)+'\n');print('PASS five planted shared/private targets, dense losses, envelope gradients and exact-capacity controls')
