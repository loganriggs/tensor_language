...
torch.save(tokens,P/'DUAL_FRESH_TOKENS_V1.pt')
out=dict(fineweb_documents=metadata,code_sources=code_sources,known_cached_prefixes=len(known),rejections=rejections,source='HuggingFaceFW/fineweb sample-10BT train stream',scope='New document-indexed panel for this extraction study, one prefix perdocument and no knowncachedFineWeb excerpt anywhere in eachaccepted document. Not a pretraining-overlap or whole-project novelty guarantee. Sixth panel: all five previous FW panels and80previous stdlib files excluded. Three wider graphs and two matched-storage baselines frozen before collecting rows. Code search expands to immediate stdlib packages if needed; no site-packages/test directories.')
(P/'DUAL_FRESH_ROWS_V1.json').write_text(json.dumps(out,indent=2)+'\n')
