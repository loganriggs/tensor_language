...
A CPU successor checks per-document regressions and maxima. The correction reduces effect reconstruction error on every document, but the worst FineWeb document still has21.82%relative effect error (worstcode2.584%). CE damage worsens on12/32FineWebdocuments and8/16codefiles despite improving in aggregate. The aggregate pass therefore does not establish uniform fidelity or uniformly improved predictions. No semantic feature identity, selective removal, arbitrary component reuse, or standalone token-to-output circuit has been shown. Retain this as a full-layer compression baseline for the decomposition research, not an adopted interpretable circuit.
