...
P=Path(__file__).parent;torch.set_num_threads(2);plan=json.loads((P/'APPROXIMATE_PENCIL_PLAN_V1.json').read_text());rows=[];start=time.monotonic()
for case in plan['cases']:
 ...
 for level in plan['noise_levels']:
  ...
  try:
   proposal=propose_approximate(target,r,private[0].shape[1]);initial=proposal['bases']+[torch.linalg.qr(v,mode='reduced').Q for v in proposal['private']]
   metric=JointOrthogonalPrivateMetric(target,[torch.cat([initial[a],initial[b]],1) for a,b in GROUPS]);params=[torch.nn.Parameter(v.detach().clone()) for v in initial];initial_error=float(metric.loss(params,dense=True)[0].detach().sqrt());best=initial_error**2;saved=[v.detach().clone() for v in params];evaluations=0
   def closure():
    ...
    evaluations+=1;scaled=loss/max(level,1e-6)**2;scaled.backward();return scaled
   if level>0:
...
(P/'APPROXIMATE_PENCIL_TOY_V1.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(summary),flush=True)
