...
torch.save(tokens,P/'FRONTIER_FRESH_TOKENS_V1.pt')
out=dict(fineweb_documents=metadata,code_sources=code_sources,known_cached_prefixes=len(known),rejections=rejections,source='HuggingFaceFW/fineweb sample-10BT train stream',scope='New document-indexed panel for this extraction study, one prefix perdocument and no knowncachedFineWeb excerpt anywhere in eachaccepted document. Not a pretraining-overlap or whole-project novelty guarantee. Seventh panel: all six previous FW panels and96previous stdlib files excluded. Passing64-correction graph and two resource-matched323-width baselines frozen before collecting rows. Code search expands to immediate stdlib packages if needed; no site-packages/test directories.')
(P/'FRONTIER_FRESH_ROWS_V1.json').write_text(json.dumps(out,indent=2)+'\n')
