"""Unpriced dense-core relaxation within each edited pair's actual reader span."""
...
d=torch.load(P/'SHARED_PRODUCT_NATIVE_INPUTS_V1.pt',weights_only=True);S=torch.linalg.inv(d['inverse_root']);I=torch.eye(1152,dtype=S.dtype);data=torch.load(P/'LOCAL_SHARED_READER_PROGRAMS_V1.pt',weights_only=True);meta=json.loads((P/'LOCAL_SHARED_READER_V1.json').read_text());ids=d['indices'];z=d['z'][ids];h=d['h'][ids];rms=(h.square().mean(-1)+torch.finfo(torch.float32).eps).sqrt();rows=[]
for rec in meta['records']:
 program=data[rec['key']];bundle=expand(program);shaped=rec['metric']=='calibration_shaped';transform=S if shaped else I;inverse=d['inverse_root'] if shaped else I;components=[]
 for j in range(3):
...
out=dict(records=rows,scope='Separates reader-span restriction from fixed product topology for the overlapping edit. Uses opened448only; no native model forward or fresh claim.')
(P/'LOCAL_READER_SPAN_V1.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
