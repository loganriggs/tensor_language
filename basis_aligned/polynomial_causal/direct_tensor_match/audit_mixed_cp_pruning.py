...
def main():
    ...
    for seed in [1001,1002]:
        ...
        for metric,G in [('mixed',(G1+lam*G0)/(1+lam)),('gaussian',G1)]:
    ...
    primary=[r for r in rows if r['metric']=='mixed' and r['terms']==256];baseline={(r['seed'],r['metric']):r for r in rows if r['terms']==512}
    ...
    torch.save(exports,P/'MIXED_CP_PRUNING_CANDIDATES_V1.pt');result=dict(predictions=predictions,rows=rows,controls=checks,seconds=time.monotonic()-start,scope='Backward greedy compression of frozen mixedCP512 fitted parents, not native target refitting. Exactcoefficient/shiftedGaussian parentGrams; fixedbudgets andmetrics, allreported. Nativeopenedmetrics evaluationonly. Primarymixed256=768products; no finite-removal/OOD/adoption. Unit-feature-metric ridge1e-10. CP factors are retained, onlysupport/readouts change.')
    (P/'MIXED_CP_PRUNING_V1.json').write_text(json.dumps(result,indent=2)+'\n')
