...
def main(version="V1"):
  ...
  for optimizer in plan['optimizers']:
   for rate in plan['rates']:
    for seed in plan['seeds']:
     ...
     states[f'{case}_{optimizer}_{rate}_{seed}']=saved
 ...
 (P/f'TOY_OVERLAP_OPTIMIZER_{version}.json').write_text(json.dumps(out,indent=2)+'\n');torch.save(states,P/f'TOY_OVERLAP_OPTIMIZER_STATES_{version}.pt');print(json.dumps(out['summaries']),flush=True)
