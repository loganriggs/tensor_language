...
def main(frontier=False):
 ...
 for width,optimizer,lr,seed in itertools.product([4,6] if frontier else [8,12],['adam','muon'],[.005,.03] if frontier else [.005],[0,1]):
  if frontier:
   torch.manual_seed(seed);a=torch.nn.Parameter(torch.randn(width,32)/32**.5);b=torch.nn.Parameter(torch.randn(width,32)/32**.5)
   _,(u0,v0,c0)=objective(T,m,a,b);initial=float((c0@embedding(features(u0,v0),m)-T).norm().detach()/norm.sqrt())
  else:
   old=prior['allfits'][(width,optimizer,.03,seed)];a=torch.nn.Parameter(old['a'].clone());b=torch.nn.Parameter(old['b'].clone());initial=float((old['c']@embedding(features(a,b),m)-T).norm().detach()/norm.sqrt())
  opt=torch.optim.Adam([a,b],lr=lr) if optimizer=='adam' else torch.optim.Muon([a,b],lr=lr,weight_decay=0.,adjust_lr_fn='match_rms_adamw');bestloss=float('inf')
  for step in range(501):
   ...
   for g in opt.param_groups:g['lr']=lr*(.01+.99*.5*(1+math.cos(math.pi*(step+1)/500)))
  u,v,c=saved;pred=c@embedding(features(u,v),m);err=float((pred-T).norm()/norm.sqrt());cancel=float(c.square().sum()/pred.square().sum());row=dict(width=width,optimizer=optimizer,lr=lr,seed=seed,initial_function_error=initial,function_error=err,penalized_objective=bestloss,selected_step=selected,function_component_energy_ratio=cancel);records.append(row);allfits[(width,optimizer,lr,seed) if frontier else (width,optimizer,seed)]=dict(a=u,b=v,c=c)
 ...
 if frontier:
  six=next(w for w in winners if w['width']==6);pred=dict(pred_a_composed=six['fresh_errors'][0]<=.18440681,pred_b_export=all(w['cost']['stored_coefficients']==2308*w['width']+10448 and w['cost']['products']==w['width']+10 and w['graph_replay']<1e-10 for w in winners));stem='BANK_WIDTH_FRONTIER_V1'
 else:
  eight=next(w for w in winners if w['width']==8);pred=dict(pred_a_function=eight['function_error']<=.5*eight['initial_function_error'],pred_b_composed=eight['fresh_errors'][0]<=.18440681,pred_c_export=eight['cost']['stored_coefficients']==28912 and eight['cost']['products']==18 and eight['graph_replay']<1e-10);stem='NONCENTRAL_BANK_REFIT_V1'
 torch.save(dict(programs=exports,allfits=allfits,teacher_scale=source['teacher_scale']),P/(stem+'.pt'));(P/(stem+'.json')).write_text(json.dumps(dict(records=records,winners=winners,predictions=pred,seconds=time.perf_counter()-start,scope='Exact noncentral Gaussian bank function metric with fixed local root sensitivity. Complete quartic scored only after weight-space selection; same reused diagnostic/fresh panels, no new unseen evaluation.'),indent=2)+'\n');print(pred)
