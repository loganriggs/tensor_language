...
P=Path(__file__).parent;torch.set_num_threads(2);plan=json.loads((P/'ORTHOGONAL_PRIVATE_TOY_PLAN_V1.json').read_text());rows=[];start=time.monotonic()
for case in range(5):
 T,bases,private,_,_=fixture(case);shared=[torch.cat([bases[a],bases[b]],1) for a,b in GROUPS];metric=OrthogonalPrivateMetric(T,shared);oracle=private
 for optimizer in plan['optimizers']:
  for seed in plan['seeds']:
   ...
   row=dict(case=case,optimizer=optimizer,seed=seed,error=explicit**.5,private_basis_norms=[float(v.norm()) for v in saved]);rows.append(row);print(json.dumps(row),flush=True)
summaries=[]
for optimizer in plan['optimizers']:
