...
out=p/'MIDPOINT_JOINT_PRODUCT_ALS_AUDIT_V2.json';assert not out.exists()
...
records=[];exports={};start=time.perf_counter()
for item in eligible:
 ...
 a=torch.randn(batch,8,rank,dtype=torch.float64,generator=gen)
 ...
 best=torch.full((batch,),float('inf'),dtype=torch.float64);at=torch.zeros(batch,dtype=torch.long);history=[];best_factors=[v.clone() for v in [a,b,c]]
 for step in range(2001):
  if step%10==0:
   ...
   for saved,v in zip(best_factors,[a,b,c]):saved[changed]=v[changed]
 ...
 exports[item['group']]=dict(factors=best_factors,errors=best,core=core,roots=roots,ids=ix)
...
torch.save(exports,p/'MIDPOINT_JOINT_PRODUCT_ALS_FACTORS_V2.pt')
