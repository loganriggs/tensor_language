...
def main(input_programs='LOCAL_SHARED_READER_PROGRAMS_V1.pt',reference='LOCAL_SHARED_READER_V1.json',receipt='OVERLAP_NATIVE_PREFLIGHT_V1.json'):
 d=torch.load(P/'SHARED_PRODUCT_NATIVE_INPUTS_V1.pt',weights_only=True);allprograms=torch.load(P/input_programs,weights_only=True);meta=json.loads((P/reference).read_text()) if reference else None;Q=torch.stack([q for pair in d['pairs'] for q in pair['Qs']]);S=torch.linalg.inv(d['inverse_root']);I=torch.eye(1152,dtype=Q.dtype);records=[]
 for name,transform,inverse in [('calibration_shaped',S,d['inverse_root']),('native_isotropic',I,I)]:
  ...
  with torch.no_grad():
   ...
   metric_key='covariance_error' if name=='calibration_shaped' else 'native_error';coefficient_replay=None
   if meta:
    previous=next(r for r in meta['records'] if r['key']==name+'_128_160');coefficient_replay=abs(scores[metric_key]-previous[metric_key]);assert coefficient_replay<1e-7
