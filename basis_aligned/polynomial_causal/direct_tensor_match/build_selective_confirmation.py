...
def main():
 ...
 oldcode=[torch.load(P/n,weights_only=True) for n in ['CODE_SHIFT_PANEL_V1.pt','BLEND_CONFIRMATION_CODE_V1.pt']]
 ...
 assert len(rows)==16
 panels=dict(code=torch.stack(rows),fineweb=torch.load(ROOT/'basis_aligned/bilinear_quotient/.rowcache/fineweb_n192_skip7000.pt',weights_only=True)[128:160,:257].clone())
 metadata=dict(context=256,code_source_glob='archive/*.py',selection_note='Fewer than16 unused eligible jacclust files; switched to lexicographic archive files before native evaluation.',code_sources=sources,excluded_code_sources=sorted(excluded),fineweb_documents=list(range(128,160)),panels={},program_sha256=hashlib.sha256((P/'SELECTIVE_SCALAR_READOUT_V1.pt').read_bytes()).hexdigest(),scope='Unused in this decomposition study at selection time. Related local source files, not broad code representativeness; pretrained overlap unknown.')
 for name,tokens in panels.items():
  torch.save(tokens,P/f'SELECTIVE_CONFIRMATION_{name.upper()}_V1.pt');metadata['panels'][name]=dict(shape=list(tokens.shape),token_sha256=hashlib.sha256(tokens.numpy().tobytes()).hexdigest())
 (P/'SELECTIVE_CONFIRMATION_PANELS_V1.json').write_text(json.dumps(metadata,indent=2)+'\n')
 donors.PANEL_PREFIX='SELECTIVE_CONFIRMATION';donors.OUTPUT_PREFIX='SELECTIVE_CONFIRMATION_DONORS';donors.SCOPE=metadata['scope'];donors.main();print(metadata['panels'])
