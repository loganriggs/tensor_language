...
d=torch.load(P/'SHARED_PRODUCT_NATIVE_INPUTS_V1.pt',weights_only=True);states=torch.load(P/'SHARED_SUBSPACE_NATIVE_STATES_V1.pt',weights_only=True);meta=json.loads((P/'SHARED_SUBSPACE_NATIVE_V1.json').read_text());Q=torch.stack([q for pair in d['pairs'] for q in pair['Qs']]);S=torch.linalg.inv(d['inverse_root']);I=torch.eye(1152,dtype=Q.dtype);ids=d['indices'];z=d['z'][ids];h=d['h'][ids];scale=(h.square().mean(-1)+torch.finfo(torch.float32).eps).sqrt()
...
for key,params in states.items():
 ...
 hats=[];ranks=[]
 for j in range(3):
  U,s,_=torch.linalg.svd(torch.cat([params[0],params[j+1]],1),full_matrices=False);rank=int((s>1e-10*s[0]).sum());assert rank==352;U=U[:,:rank];T=A@Q[2*j:2*j+2]@A;hats.append(inv@U@(U.T@T@U)@U.T@inv);ranks.append(rank)
...
# Literal dense-core cost in orthonormal common128 + orthogonal private224 bases.
# Count the common-common products once across all three pair consumers.
r=128;p=224;n=r+p;projection=1152*(r+3*p);common_products=r*(r+1)//2;products=common_products+3*(r*p+p*(p+1)//2);readout=3*n*(n+1)
price=dict(shared_width=r,private_width=p,source_projection_multiplications=projection,distinct_nonlinear_products=products,source_readout_multiplications=readout,source_total_multiplications=projection+products+readout,stored_floats=projection+readout+11532,scope='Dense symmetric cores, common-common products computed once; full coefficients generic. Source arithmetic excludes fixed affine and downstream component work, as in original comparisons. Not an exported/adopted graph.')
