...
def main():
    ...
    for rank in [4,8,16,32]:
        source=torch.load(P/f'QUARTIC_RANK_BLOCK_{rank}_V1.pt',weights_only=True);source64={k:v.double() for k,v in source.items()};program,diagnostics=compile_program(source64);archive=cast(program,torch.float32);errors=[];double_errors=[]
        for x in xs:
        ...
        cost=price(archive);path=P/f'PAIRED_ROOT_RANK_{rank}_V1.pt';torch.save(archive,path);row=dict(rank=rank,float32_replay=errors,float64_replay=double_errors,price=cost,pairs=diagnostics);records.append(row);print(json.dumps({k:v for k,v in row.items() if k!='pairs'}),flush=True)
    ...
    result=dict(records=records,predictions=pred,seconds=time.monotonic()-start,scope='Exact pair rewrite of fixed outputrank programs; old naturalinputs and common unembeddingGram frame for replay. No freshnativebehavior, semantics oradoption. Singular/illconditioned pencils retain spectralfallback.')
    (P/'PAIRED_ROOT_NATIVE_V1.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(dict(predictions=pred,seconds=result['seconds'])))
