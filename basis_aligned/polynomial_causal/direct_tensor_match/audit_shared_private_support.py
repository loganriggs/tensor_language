...
for key in ['calibration_shaped_0','native_isotropic_26301']:
 ...
 prog=programs[key];common=torch.linalg.qr(transform@prog['input_basis'],mode='reduced').Q
 bundle=expand(prog)
 for j in range(3):
...
out=dict(records=rows,aggregates=aggregates,seconds=time.monotonic()-t0,scope='Dense-core diagnostic only. Shared128/private224 supports; free-pair comparator has no sharing constraint. Mode-Gram heuristics are not globally optimal. No core sparsity, cost advantage, native execution, OOD or circuit identity claim.')
(P/'SHARED_PRIVATE_SUPPORT_V1.json').write_text(json.dumps(out,indent=2)+'\n')
