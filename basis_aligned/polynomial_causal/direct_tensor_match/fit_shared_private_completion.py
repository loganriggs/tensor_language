...
prefix=sys.argv[1] if len(sys.argv)>1 else 'SHARED_PRIVATE_COMPLETION'
plan=json.loads((P/f'{prefix}_PLAN_V1.json').read_text());start=time.monotonic()
...
torch.save(outprograms,P/f'{prefix}_PROGRAMS_V1.pt');(P/f'{prefix}_V1.json').write_text(json.dumps(dict(plan=plan,records=records,predictions=pred,seconds=time.monotonic()-start,scope='Actual compiled1056product graphs with pairwise sharedlinear dictionaries. Common/private cross approximation is explicit; fixed-product output coefficients refitted. Opened448only, suppliedz/h remain.'),indent=2)+'\n');print(pred,flush=True)
