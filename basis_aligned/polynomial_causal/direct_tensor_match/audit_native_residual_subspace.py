...
def main(large=False):
 ...
 for seed in ([42011,42012] if large else [42001,42002]):
  z=torch.randn(n,1152,dtype=torch.float64,generator=torch.Generator().manual_seed(seed));x=mu+z@S.T;ys=[];js=[]
 ...
 for seed in [1001,1002]:
  ...
  A=(residuals[0]*weights.sqrt()[None,:,None]).reshape(-1,1152);H=A.T@A/n;ev,V=torch.linalg.eigh(H);order=ev.argsort(descending=True);ev=ev[order];V=V[:,order];captures=[]
  ...
  row=dict(seed=seed,parent_sha256=sha,eigenvalues=ev.tolist(),captures=captures,training_samples=n,checking_samples=n);rows.append(row);print(seed,[(r['rank'],r['training']['weighted_capture'],r['checking']['weighted_capture']) for r in captures],flush=True)
 pred=all(next(c for c in r['captures'] if c['rank']==64)['checking']['weighted_capture']>=.9 for r in rows)
 (P/('NATIVE_RESIDUAL_SUBSPACE_LARGE_V1.json' if large else 'NATIVE_RESIDUAL_SUBSPACE_V1.json')).write_text(json.dumps(dict(rows=rows,prediction=pred,native_euler_errors=eulers,native_finite_difference=finite,seconds=time.monotonic()-start,scope='Monte Carlo Gaussian derivative geometry for native-minus-parent outputs4–15, not text-tangent geometry or function-error guarantee. Independent synthetic check probes; no candidate export or queue changes.'),indent=2)+'\n')
if __name__=='__main__':
 parser=argparse.ArgumentParser();parser.add_argument('--large',action='store_true');args=parser.parse_args();main(args.large)
