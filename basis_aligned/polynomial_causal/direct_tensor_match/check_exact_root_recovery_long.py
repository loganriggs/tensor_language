  ...
  for optimizer in ['adam','muon']:
   for restart in [0,1]:
    ...
    for step in range(1001):
     ...
     for group in opt.param_groups:group['lr']=.01*(.01+.99*.5*(1+math.cos(math.pi*(step+1)/1000)))
    rows.append(dict(family=family,optimizer=optimizer,restart=restart,steps=1000,rate=.01,schedule="cosine1pct",planted_error=warm,selected_step=best[1],exact_coefficient_error=best[2]))
 result=dict(rows=rows,summary={o:dict(below1percent=sum(r['exact_coefficient_error']<.01 for r in rows if r['optimizer']==o),median_error=float(torch.tensor([r['exact_coefficient_error'] for r in rows if r['optimizer']==o]).median())) for o in ['adam','muon']},scope='Bounded1000step2restart optimizer control with cosine1pct, exactobjective. Compared to V1 both duration andschedulechange; not isolatedtimeeffect. Native25stepAdam alreadyqueued, no retrospectiveoptimizerselection.')
 (P/'EXACT_ROOT_RECOVERY_TOYS_V2.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2))
