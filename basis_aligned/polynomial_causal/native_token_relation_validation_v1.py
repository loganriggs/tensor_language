...
def main():
    ...
    root=Path(__file__).parent;output=root/'NATIVE_TOKEN_RELATION_VALIDATION_V1.json'
    ...
    for label,stem in [('fineweb','SHARED_NODE_PARENT1_SUPPRESSION_V1'),
        ...
        expected=receipt.get('cache_sha256',receipt.get('endpoint_cache_sha256'))
        if expected is None:
    ...
    answer=dict(pred_a=bool(a),pred_b=bool(a and b),pred_c=bool(a and c),maximum_native_replay=max(errors),
        ...
        'and retained radial/bias terms; final residual background/RMS/tanh and physical circuit writes are not approximated or tested here. '
