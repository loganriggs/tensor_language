# Preregistration — late_tail_gate_mode_rank_probe (Claude, LANE 1 CUDA) — Registered 2026-09-04 02:19Z (box clock)
...
## Rename note (02:22Z, before write-up)
Registered 02:19Z under the name late_tail_gate_rank_probe, which collided with §2782's rung of the same name (whose script, preregistration and results were overwritten on disk and are restored bit-exact from git). Renamed late_tail_gate_mode_rank_probe; predictions, bars and nulls unchanged. The first run under the colliding name (02:20, results kept in the ledger entry) gave the same numbers as the re-run.
