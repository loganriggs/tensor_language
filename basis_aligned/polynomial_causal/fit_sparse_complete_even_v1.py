"""Ten fixed-support sparse complete-even fits, weights only.
...
Pred_c normalized-column coordinate gradient RMS<=1e-8 for all ten starts.
...
def main():
    ...
    rotitem=torch.load(p/'SPARSE_READER_ROTATION_V1_PROGRAM.pt',weights_only=True)['0.25']
    rs,_,_=unpack(rotitem);baseline=float(objective(rs));rows=[];best=None
    for frame,file in [('original','SPARSE_PARENT_READER_V1_PROGRAM.pt'),('rotated','SPARSE_READER_ROTATION_V1_PROGRAM.pt')]:
        item=torch.load(p/file,weights_only=True)['0.25'];ss,_,_=unpack(item)
    ...
    torch.save({'0.25':best[1]},p/'SPARSE_COMPLETE_EVEN_FIT_V1_PROGRAM.pt')
    result=dict(pred_a=all(r['final_loss']<=r['initial_loss']+1e-12 for r in rows),
        pred_b=best[0]<=.9*baseline,pred_c=len(rows)==10 and all(r['converged'] for r in rows),
    ...
    (p/'SPARSE_COMPLETE_EVEN_FIT_V1_RESULT.json').write_text(json.dumps(result,indent=2)+'\n')
