"""Joint-product feature interventions before position transforms, retaining native attention semantics."""
import torch
import circuit_unit_greedy as g
from joint_rotary_product_coordinates_v1 import unrotate_vector,rotate_feature
    ...
    def wrapper(layer,units,original):
        ...
        def wrapped(q,k,v,q2,k2):
            ...
            for unit in units:
                ...
                tables=backend.model.transformer.h[layer].attn.rotary
                cos=tables.cos_cached.double();sin=tables.sin_cached.double()
                qc,qs=cos[pos],sin[pos];kc,ks=cos[None,:,:],sin[None,:,:]
                for key in ['q1','q2']:c[key]=unrotate_vector(c[key],qc,qs)
                for key in ['k1','k2']:c[key]=unrotate_vector(c[key],kc,ks)
                fq=feature(c['q1'],c['q2']);fk=feature(c['k1'],c['k2']);native=y[idx,h,pos].double();live.append(native)
                score=torch.einsum('bd,btd->bt',rotate_feature(fq,qc,qs),rotate_feature(fk,kc,ks)).masked_fill(~visible,0)/128**2
