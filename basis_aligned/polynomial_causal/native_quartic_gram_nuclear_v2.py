...
C all primal-dual gaps/residuals<=1e-6. 40000ADMMsteps max/tol1e-7.
"""
import json
from pathlib import Path
import torch
from quartic_gram_map_v1 import layout,canonical,coefficients
from quartic_gram_nuclear_v2 import solve
...
def main():
    ...
    out=p/'NATIVE_QUARTIC_GRAM_NUCLEAR_V2.json';ap=p/'NATIVE_QUARTIC_GRAM_NUCLEAR_V2.pt';assert not out.exists() and not ap.exists()
    control=json.loads((p/'QUARTIC_GRAM_NUCLEAR_V2_CONTROL.json').read_text());assert control['pred_a'] and control['pred_b']
    ...
    old_result=json.loads((p/'NATIVE_QUARTIC_GRAM_NUCLEAR_V1.json').read_text());old_file=p/'NATIVE_QUARTIC_GRAM_NUCLEAR_V1.pt'
    assert old_result['pred_a'] and digest(old_file)==old_result['artifact_sha256']
    old_programs=torch.load(old_file,weights_only=True,map_location='cpu')['programs']
    for m in modes:
        ...
        for index in [0,1]:
            ...
            previous=next(z for z in old_programs if z['seed']==m['seed'] and z['mode']==index)
            matrix,r=solve(c,mapping,iterations=40000,tolerance=1e-7,initial=previous['gram'])
            curves={}
            for label,g in [('canonical',base),('optimized',matrix)]:
                vals,vecs=torch.linalg.eigh(g);order=vals.abs().argsort(descending=True);vals=vals[order];vecs=vecs[:,order]
                curves[label]={str(k):float((coefficients((vecs[:,:k]*vals[:k])@vecs[:,:k].T,mapping)-c).norm()/c.norm()) for k in [1,2,4,8,16,32,64]}
            r['rank_error_curves']=curves
