...
@torch.no_grad()
def main():
 torch.set_num_threads(2);tic=time.perf_counter();out=P/'REGIONAL_SELECTED_SUFFIX_CPU_V1_RESULT.json';assert not out.exists()
 ...
 l,r,d=[state['transformer.h.17.mlp.'+k+'.weight'].float() for k in ('Left','Right','Down')];bias=state['transformer.h.17.mlp.Down.bias'].float();pre=a['pre'];base=b['base_write'];times=[]
 def margins(delta):
