...
def main():
 ...
 out={'records':records,'orthogonality_error':ortherror,'baseline_program_scalars':size(full),'scope':'Frozen best converged shared-query position-folded product fit, not data fit. Paired scalar effects versus same rank64-transport program, existing72pristineprompts, five signed amplitudes with analytic baseline, not native strength interventions. No fresh/native-logit/semantic-selectivity claim.'}
 (P/'SHARED_KEY_QUERYFOLD_STRENGTH_V1_CONTROL.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
