...
@torch.no_grad()
def main():
 torch.set_num_threads(2);rows=json.loads((P/'CITY_REMOVAL_ENDPOINT_FRESH_V1_ROWS.json').read_text())['rows'];ids=torch.tensor(sorted({t for r in rows for t in r['ids']}))
 ...
 out=P/'CITY_REMOVAL_ENDPOINT_FRESH_V1_TABLES.pt';assert not out.exists();torch.save({'token_ids':ids,'initial_table':initial,'first_table':first,'lambdas8':sd['transformer.h.8.lambdas'].clone()},out)
 receipt={'tokens':len(ids),'floating_scalars':initial.numel()+first.numel()+2,'checkpoint_sha256':binding[ckpt],'tables_sha256':hashlib.sha256(out.read_bytes()).hexdigest(),'scope':'Exact weight-derived tables only, full first-value width retained for reference instrumentation; no native model effects inspected.'};(P/'CITY_REMOVAL_ENDPOINT_FRESH_V1_TABLES_RESULT.json').write_text(json.dumps(receipt,indent=2)+'\n');print(receipt)
