...
@torch.no_grad()
def main():
    out=P/'PRODUCER_SCORE_BOUNDARY_V1_RESULT.json'
    assert not out.exists()
    signal.alarm(120);start=time.monotonic();torch.set_num_threads(2)
    model=Contexts()
    path=P/'COMPOSED_LAST_BLOCK_STATES_V1_ARTIFACT.pt'
    ...
    rowdata=json.loads((P/'FIRST_TOKEN_PATH_FRESH_V1_ROWS.json').read_text())['rows'][:24]+json.loads((P/'CROSSFIRST_THREE_GROUP_FRESH_V1_ROWS.json').read_text())['rows']
    ...
    scores=[]
    for name,prefix in [('SPARSE_INTERACTION_EXECUTOR_V1','PRODUCER_BASELINE_CACHED_REGIONAL_V1'),
                        ('UNIFORM_PRODUCER_GRADIENT_V1','PRODUCER_FIT_CACHED_REGIONAL_V1')]:
        ...
        scores.append(dict(name=name,all12_raw_energy=float(value.square().sum()),
                           all6_raw_contrast_energy=float((value[:,::2]-value[:,1::2]).square().sum()/2),
                           selected_raw_contrast_energy=float((value[torch.arange(120),uk]-value[torch.arange(120),us]).square().sum()),
                           selected_softcap_contrast_energy=float(capped.double().square().sum())))
    ...
    result=dict(utc=datetime.now(timezone.utc).isoformat(),scores=scores,fit_over_baseline=ratios,
                ...
                scope='Frozen historical native cache; score-boundary diagnostic only, no fit/nativebody/freshOOD.')
