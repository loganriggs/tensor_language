...
P=Path(__file__).resolve().parent;STEM='TYPED_FACE_CONTEXT_GENERATED_V1'
def main():
 with tempfile.TemporaryDirectory(prefix='coupled-isolated-') as tmp:
  root=Path(tmp);shutil.copytree(P/'extracted_circuits/typed_face_context_generated_v1',root/'package',ignore=shutil.ignore_patterns('__pycache__'))
...
import execute
p={'local':torch.load(root/'package/local_program.pt',weights_only=True,map_location='cpu'),'context':torch.load(root/'package/context_program.pt',weights_only=True,map_location='cpu')};fixtures=torch.load(root/'fixtures.pt',weights_only=True,map_location='cpu')['fixtures'];torch.set_num_threads(2)
errors=[]
for f in fixtures:
