...
import tiktoken
P=Path(__file__).resolve().parent;STEM='CITY_FULL_PILE_V1'
...
def main():
 ...
 doc={'schema':'regional.natural_context.rows.v1','utc':datetime.now(timezone.utc).isoformat(),'source':source,'source_split':'train','docs_scanned':scanned,'selection':'First eligible occurrence in first20distinct eligible documents in stream order.32tokens,12beforecity, exactly one mapped city in window. No model outcomes/capability filtering. One untouched arm and one city substitution. Endpoints are fixed probes, not observed next-token labels. No pretraining-disjointness guarantee.','contexts':contexts,'rows':rows,'variants':['pile'],'endpoints':endpoints,'city_pairs':PAIRS}
