"""Shared managed runner for fixed structure/metric/seed configurations."""
...
def run(config_name,chunk,runner,binding_path,dry=False):
    ...
    out=P/f'STRUCTURED_FIT_V1_{config_name}_CHUNK_{chunk:02d}.json';assert not out.exists()
    checkpoint_path=P/f'STRUCTURED_FIT_V1_{config_name}_CHECKPOINT.pt';tic=time.perf_counter();signal.alarm(900)
    ...
    if config['metric']=='weight':
        ...
        y=(data['y'][rows].float()*data['y_scale'][rows]).reshape(-1,1152).double().cuda()-sd['transformer.h.17.mlp.Down.bias'].double().cuda()
    ...
    result=dict(schema='structured.quadratic.fit.chunk.v1',config=config,chunk=chunk,
