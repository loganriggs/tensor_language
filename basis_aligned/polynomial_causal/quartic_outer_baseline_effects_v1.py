"""Complete the missing native intervention screen for the frozen outer16 baseline.
...
def main():
 ...
 out=P/'QUARTIC_OUTER_BASELINE_NATIVE_EFFECTS_V1.json';ap=P/'QUARTIC_OUTER_BASELINE_V1_PROGRAM.pt'
 ...
 for m in range(2):
  ...
  eig,vec=torch.linalg.eigh(matrix);ids=eig.abs().topk(16).indices
 ...
 replay=abs(error-prior['outer16_relative_error'])
 ...
 result.update(dict(old_write_error_replay=replay,initial_effect_regression=regression,artifact_sha256=digest(ap),
  ...
  wall_seconds=time.perf_counter()-start,scope='Previously fixed outer16 per output, exact bias-free MLP16 producer, two-output group. Reused developmental native background. No OOD, selective circuit or new weight fit.'))
