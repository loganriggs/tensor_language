...
def analyze():
 ...
 for f,ref in zip(capture['fixtures'],reference,strict=True):
 ...
 out={'pred_d':max(native_errors)<=1e-4,'algebraic_closure_max_relative':max(closures),'native_write_max_relative':max(native_errors),'term_count':len(stats),'groups':groupstats,'terms':stats,'scope':'Opened city-side fold only. Query factors and all RMS denominators native. Presence groups overlap between MLP7 and attention7; no causal or closed-port claim.'}
 (P/'CITY_SOURCE7_V1_CPU_RESULT.json').write_text(json.dumps(out,indent=2)+'\n')
