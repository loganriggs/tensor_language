...
import terminal_copy_fit_mean_recovery_v3 as recovery


def test_v2_failure_is_exact_and_has_no_success_outputs():
    recovery.validate_v2_failure_lineage()
    failure = json.loads(recovery.V2_FAILURE.read_text())
    ...
    assert all(not path.exists() for path in recovery.V2_ABSENT_OUTPUTS)


def test_v3_configuration_is_fresh_and_protects_v2_absence():
    ...
    try:
        ...
        assert all(snapshot[str(path)] is None for path in recovery.v2.V1_ABSENT_OUTPUTS)
        assert all(snapshot[str(path)] is None for path in recovery.V2_ABSENT_OUTPUTS)
        recovery.configure()
        assert len(life.SOURCE_PATHS) == len(set(life.SOURCE_PATHS))
        assert len(life.PROTECTED_PATHS) == len(set(life.PROTECTED_PATHS))
...
def test_freeze_requires_separate_v3_audit(monkeypatch):
    monkeypatch.setattr(
...
def test_v2_lineage_rejects_late_success_artifact(monkeypatch, tmp_path):
    late_bank = tmp_path / "late_v2_bank.pt"
    absent = (late_bank, *recovery.V2_ABSENT_OUTPUTS[1:])
    monkeypatch.setattr(recovery, "V2_ABSENT_OUTPUTS", absent)
    recovery.validate_v2_failure_lineage()
