...
def main():
    ...
    pairs=[('teacher','teachers'),('lawyer','lawyers'),('farmer','farmers'),('doctor','doctors'),('singer','singers'),('writer','writers')]
    templates=[('unlike_nearby','The {s}, unlike the {a} nearby,'),('beside_subject','Beside the {a}, the {s}')]
    old=set()
    for name in ['SUBJECT_ATTRACTOR_CONTROL_V696_ROWS.json','SUBJECT_CONGRUENT_ATTRACTOR_V697_ROWS.json','SUBJECT_HYBRID_FRESH_V687_ROWS.json','SEMANTIC_PORT_FRESH_OPPOSITE_ROWS.json','SEMANTIC_PORT_FRESH_CONGRUENT_ROWS.json','SOURCE_OOD_V1_OPPOSITE_ROWS.json','SOURCE_OOD_V1_CONGRUENT_ROWS.json']:
    ...
    for panel in ['opposite','congruent']:
        ...
        for template,pattern in templates:
            for i,pair in enumerate(pairs):
                for number in [0,1]:
                    ...
                    assert (apos<spos==len(ids)-1) if template=='beside_subject' else (spos<apos<len(ids)-1)
        ...
        out=p/f'SOURCE_OOD_V2_{panel.upper()}_ROWS.json';out.write_text(json.dumps(rows,indent=2)+'\n');result[panel]=dict(file=out.name,sha256=hashlib.sha256(out.read_bytes()).hexdigest(),rows=24)
    (p/'SOURCE_OOD_V2_ROW_AUDIT.json').write_text(json.dumps(result,indent=2)+'\n');print(result)
