"""Generate a selected attention7 head sum and MLP7 inputs from residual6, using native factors."""
import torch
import torch.nn.functional as F

def execute(program,residual6,token_ids,heads=tuple(range(9))):
    ...
    def factor(name):
        x=F.rms_norm(F.linear(current,program[name][indices]).reshape(b,t,n,128),(128,));a,z=x.chunk(2,-1)
    ...
    value=(1-program['mixture'])*F.linear(current,program['value'][indices])+program['mixture']*first[:,:,indices]
    channels=torch.einsum('bhqs,bshd->bqhd',pattern,value.reshape(b,t,n,128)).reshape(b,t,n*128)
    attention=F.linear(channels,program['output'][:,indices]);post=mixed+attention;lam8=program['lambdas8']
