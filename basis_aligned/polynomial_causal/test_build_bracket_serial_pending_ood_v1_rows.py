...
import tiktoken
import build_bracket_serial_pending_ood_v1_rows as builder
def test_eighth_construction_is_balanced_aligned_and_disjoint():
 rows=builder.build_rows(tiktoken.get_encoding('gpt2').encode);root=Path(__file__).parent;old=[]
 for name in ('BRACKET_NESTED_PENDING_OOD_V1_ROWS.json','BRACKET_TRIPLE_PENDING_OOD_V1_ROWS.json','BRACKET_LAYERED_PENDING_OOD_V1_ROWS.json','BRACKET_EMBEDDED_PENDING_OOD_V1_ROWS.json','BRACKET_CASCADE_PENDING_OOD_V1_ROWS.json'):old.extend(json.loads((root/name).read_text())['rows'])
...
def test_builder_is_model_blind():
 source=Path(builder.__file__).read_text();assert 'load_bilin18' not in source and 'torch' not in source and builder.OUT.name=='BRACKET_SERIAL_PENDING_OOD_V1_ROWS.json'
