"""Native attention8 output from normalized current8 and token-only first values."""
import torch
import torch.nn.functional as F

def heads(p,current8,token_ids):
 ...
 first=F.embedding(indices,p['first_table']).to(current8.dtype).reshape(b,t,9,128)
 ...
 def factor(name):
  x=F.rms_norm(F.linear(current8,p[name].to(current8.dtype)).reshape(b,t,9,128),(128,));a,z=x.chunk(2,-1)
 ...
 v=F.linear(current8,p['value'].to(current8.dtype)).reshape(b,t,9,128);v=(1-p['mixture'])*v+p['mixture']*first
 y=torch.einsum('bhqk,bkhd->bqhd',pattern,v).contiguous().reshape(b,t,d)
 return torch.stack([F.linear(y[:,:,h*128:(h+1)*128],p['output'][:,h*128:(h+1)*128].to(current8.dtype)) for h in range(9)])
