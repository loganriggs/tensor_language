# Frontier: is the front excess concentrated? Rerun with the ordering fixed. Preregistration

Registered 2026-09-04T13:51Z (substituted by `date -u` at write time). Before the run. Immutable; the rung's frozen-hash check refuses to execute if this file changes.
...
## What changed since the void run

**§2927 asked this exact question and could not answer it.** `_apply_front_split` ran **before** `_apply_front`, which resets `A` from
its snapshot whenever `a_scale` is None, so the split was wiped on every arm and all thirteen read **exactly 0.0000**. The identity arm
read 0.0000 as well — **it was satisfied by precisely the failure it exists to exclude, because a no-op knob and a faithful identity are
the same measurement.** What caught it was the **route-agreement** half of pred_c: `f_uniform` (×0.5 through the split path) read 0.0000
while `plain_half` (×0.5 through the plain multiply) read −0.1649, a disagreement of 0.1649.

**One line moved; every prediction, bar and null below is unchanged from §2927's preregistration**, so this is a rerun and not a
redesign. §2927's receipt is preserved under its own name and its verdict stands as written: pred_c, pred_d and pred_e FAILED, and the
rung is void for its scientific question.

**Carrying §2926's standing caution:** whatever this finds is a **standalone** number. §2922's CP split won standalone by 0.0206 and then
lost by 0.035 in composition. Nothing here may be adopted without a composed, held-out measurement.
...
that a manipulated component is actually in the config. Receipt: `frontier_front_split_fixed_results.json`, read with `price` in the
