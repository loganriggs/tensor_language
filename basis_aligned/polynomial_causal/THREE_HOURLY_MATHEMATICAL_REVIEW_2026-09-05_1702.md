...
The $2^5$ MLP4--5 versus MLP6--7 experiment completed as a valid causal screen. Neither half satisfies the global dominance rule; the registered distributed outcome holds. MLP6--7 is nevertheless the larger exclusive contribution in almost every aggregate. The important exception is the full parent-$M$ interaction between the halves: it contributes about $-23\%$ in plural-to-singular transfer and $+49\%$ in singular-to-plural transfer. Therefore MLP4--5 cannot be dropped as “small.”

The next exact refinement keeps $E$, $A$, $U=\mathrm{MLP0\text{--}3}$, and $W=\mathrm{MLP4\text{--}5}$ live while splitting $X$ into $Y=\mathrm{MLP6}$ and $Z=\mathrm{MLP7}$. This is a complete $2^6$ factorial so that the large signed $W\times X$ interaction remains measurable. The valid $2^5$ result is `task14_head11_3_fresh_matched_subject_mlp8_mlp4_7_source_factorial_v1_result.json`, SHA256 `11d64cb3f3dca1b4d0d3bf50a1288c5503335e23eeb8c10754bc2907d8ee637f`.
