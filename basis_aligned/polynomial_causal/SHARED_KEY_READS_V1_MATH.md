...
The subsequent [mathematical review](THREE_HOURLY_MATHEMATICAL_REVIEW_2026-09-13_1129.md) identifies a fitting-object mismatch. Compressing the inside read also changes its cross terms with full reads; the denominator is unchanged. These error contributions strongly cancel in the executed attribution. More precisely, writing the original inside score as retained plus discarded gives complete even-score error $(o_1-p_1)d_2+d_1(o_2-p_2)$: the discarded–discarded product cancels. [The exact identity](EVEN_KEY_DISCARDED_PAIR_V1_CONTROL.json) and [paired-response attribution](EVEN_KEY_ERROR_ATTRIBUTION_V1_CONTROL.json) pass. The next fitting target should include this complete replacement error rather than only the inside product.
