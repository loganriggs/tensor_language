"""Native city-side MLP7 self/mixed product decomposition at fixed denominators."""
...
def main():
    ...
    for i,f in enumerate(fixtures):
        ...
        _,not_self,_=from_projected(projected[i:i+1],projected[i:i+1]-m[i:i+1],inherit,head,city,mask)
        self_write=full-not_self;reference=f['native_delta'].double();mixed_write=reference-self_write
        packed=torch.stack([reference,self_write,mixed_write]);writes.append(packed)
        full_errors.append(float((full-f['native_full_delta'].double()).norm()/f['native_full_delta'].double().norm()))
        group_errors.append(float((present-reference).norm()/reference.norm()));closure.append(float((self_write+mixed_write-reference).norm()/reference.norm()))
        outside.append(float(packed[:,:,~mask].abs().max()))
    result={'pred_a':max(full_errors)<=1e-4 and max(group_errors)<=1e-4,'pred_b':max(closure)<=1e-12 and max(outside)==0 and all(bool(torch.isfinite(w).all()) for w in writes),'full_write_replay_error':max(full_errors),'group_replay_error':max(group_errors),'closure_error':max(closure),'sequences':len(writes),'native_block_calls':1,'full_model_forwards':0,'seconds':time.perf_counter()-start,'scope':'Opened native city-source product decomposition; MLP7 input products unsplit; no behavioral or composition verdict.','source_shas':{n:hashlib.sha256((P/n).read_bytes()).hexdigest() for n in ['CITY_SOURCE7_SELF_CROSS_V1_PREREGISTRATION.md','build_city_source7_self_cross_v1.py','city_source7_present_generator_v1.py']}}
    torch.save({'writes':writes,'arms':['present','self','mixed']},P/'CITY_SOURCE7_SELF_CROSS_V1_WRITES.pt');(P/'CITY_SOURCE7_SELF_CROSS_V1_CPU_RESULT.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
