...
def main():
    ...
    captured=torch.load(P/'CITY_RESIDUAL6_SINGLE_INPUT_FRESH_V1_ARTIFACT.pt',weights_only=True)['fixtures']
    ...
    with tempfile.TemporaryDirectory(prefix='city-residual6-isolated-') as directory:
        temp=Path(directory)
        for name in ['execute.py','attention7.py','readers.py','attention7.pt','readers.pt','head8.pt']:shutil.copyfile(P/'CITY_RESIDUAL6_SINGLE_INPUT_FRESH_V1_ATTENTION7.pt' if name=='attention7.pt' else source/name,temp/name)
        torch.save(fixtures,temp/'fixtures.pt')
        (temp/'check.py').write_text('''import json,time
from pathlib import Path
import torch
import execute,attention7,readers
torch.set_num_threads(2)
start=time.perf_counter()
assert all(Path(m.__file__).resolve().parent==Path.cwd() for m in [execute,attention7,readers])
...
r={'passes':max(errors)<=1e-10 and max(outside)==0 and finite and unknown and len(fixtures)==40,
   ...
   'imports_outside_repository':True,'seconds':time.perf_counter()-start,'scope':'Isolated CPU declared-input replay; fresh residual6 fixtures and weight-derived fresh token table; original executable/readers/head8 unchanged; compares generator writes.'}
print(json.dumps(r))
''')
    ...
    (P/'CITY_RESIDUAL6_SINGLE_INPUT_FRESH_V1_ISOLATED_RESULT.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
