...
def main():
    ...
    l=torch.randn(m,d,dtype=dtype);r=torch.randn(m,d,dtype=dtype);down=torch.randn(o,m,dtype=dtype,requires_grad=True)
    ...
    probe=torch.randn_like(pred);g=torch.autograd.grad((pred*probe).sum(),forms,retain_graph=True)[0];gd=torch.autograd.grad((actual*probe).sum(),forms,retain_graph=True)[0]
    dg=torch.autograd.grad((pred*probe).sum(),down,retain_graph=True)[0];dd=torch.autograd.grad((actual*probe).sum(),down)[0]
    errors=dict(dense=float(((pred-actual).norm()/actual.norm()).detach()),permutation=float(((permutation-pred).norm()/pred.norm()).detach()),
                diagonal=float(((diagonal-direct).norm()/direct.norm()).detach()),gradient=float((g-gd).norm()/gd.norm()),producer_gradient=float((dg-dd).norm()/dd.norm()))
    assert max(errors.values())<1e-10
    result=dict(errors=errors,source_sha=hashlib.sha256((P/'composed_eighth_contraction_v2.py').read_bytes()).hexdigest(),scope='Exact homogeneous pure-producer eighth-degree numerator. No native fit, mixed-source or behavioral claim.')
    out=P/'COMPOSED_EIGHTH_CONTRACTION_V2_CONTROL.json';assert not out.exists();out.write_text(json.dumps(result,indent=2)+'\n');print(result)
