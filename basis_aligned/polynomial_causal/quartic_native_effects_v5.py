...
@torch.no_grad()
def main():
 parser=argparse.ArgumentParser();parser.add_argument("--stem",required=True);parser.add_argument("--initial",required=True);parser.add_argument("--output",required=True);parser.add_argument("--format",choices=["square","mixed_full","mixed_sparse","hybrid","hybrid_bank","projected_neuron","projected_output"],default="square");args=parser.parse_args()
 ...
 result=json.loads((P/(args.stem+'_RESULT.json')).read_text());ap=P/(args.stem+('_PROGRAM.pt' if args.format in ('square','hybrid','hybrid_bank','projected_neuron','projected_output') else '_PROGRAMS.pt'));assert digest(ap)==result['artifact_sha256']
 ...
 if args.format not in ('square','hybrid','hybrid_bank','projected_neuron','projected_output'):pilot=pilot['programs'][0]
 if args.format.startswith('projected_'):
  source_path=P/'QUARTIC_FULL_QUADRATIC_BANK_V1_PROGRAM.pt';assert digest(source_path)==result['source_artifact_sha256']
  source=torch.load(source_path,weights_only=True);pilot['seed']=source['seed']
 ...
 def write(p,kind):
  ...
  elif kind.startswith('projected_'):
   state=torch.load(next(k for k in bindings if k.endswith('/pytorch_model.bin')),mmap=True,weights_only=True)
   l,r,d=[state['transformer.h.16.mlp.'+name+'.weight'].double() for name in ('Left','Right','Down')]
   coeff=p['neuron_coefficients'] if kind=='projected_neuron' else d.T@p['output_readers']
   values=((x@l.T)*(x@r.T))@coeff
   value=features(source['input_readers'],source['inner_weights'],x)@source['mixing']+values.square()@source['full_quadratic_mixing']
   output=value@source['output_writers'].T/den[:,None]
   cached=p['native_writes'][0 if kind=='projected_neuron' else 1]
   assert float((output-cached).norm()/cached.norm())<=1e-8
   return output
 ...
 elif args.format.startswith('projected_'):expected=[json.loads((P/'QUARTIC_FULL_QUADRATIC_BANK_V1_RESULT.json').read_text())['baseline_native_error'],result['reports'][0 if args.format=='projected_neuron' else 1]['native_reference_error']]
