...
where every constant is learned on fit rows only.  The additive gauge is fixed by
requiring the fit-frequency-weighted mean of `b(token)` to be zero and serializing
exactly one D-vector intercept `c`; `-AB mu_h` is absorbed into `c`, so `mu_h` is not
serialized. `A B` is fit directly to the residual `Down(h)-b(token)`; it may not
read, project, or cache the live MLP0 output at evaluation time.  A cloned full-rank
native map is an integrity arm, not a candidate.
...
Ranks for Q/A are determined once from fit-only occupancy and the physical bundle,
not from evaluation outcomes.  For every arm, price includes bf16 `A`, `B`, the one
intercept, occupied centroids, an unseen-token sentinel, full-vocabulary assignments
at `ceil(log2(K_occupied+1))` bits, and a fixed 4096-byte uncompressed header for one
pinned generic decoder.  The authority chooses the largest `r` whose canonical
bundle is no larger than `C_R`, and must physically serialize both `r` and `r+1` to
assert that `r` passes and `r+1` fails.  Content-dependent compression is forbidden.
Exact checkpoint-float32 `Left`/`Right` cost is reported separately and never
described as saved.  FLOPs and peak interface width are separate Pareto coordinates.

Factor gauges are canonicalized by SVD into balanced factors with descending singular
values and a deterministic largest-loading/lowest-index sign rule, then serialized
to bf16 before evaluation.  Solver, device, and operation order are authority-bound.
...
dataset order and one to three non-overlapping 513-token chunks per document.  Each
chunk is scored as two deterministic 257-token windows `[0:257]` and `[256:513]`,
preserving Stage 0's 256 predictions and position split per window while covering all
512 next-token positions. No
...
of resample indices across all arms, rungs, consumers, and cells.  Recompute one
global maximum statistic across the entire arm x rung x consumer x cell family in
every replicate; separate armwise intervals are not a multiple-comparison correction.
Seeds and document ledgers are serialized so the result can be replayed without the
model.
...
2. the one-sided family-wise 95% lower bound for the jointly recomputed bootstrap
   contrast `max(C_R)-max(H_r)` is positive in wave A, wave B, and pooled data;
