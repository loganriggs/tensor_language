...
STEM='CITY_MLP8_NORM_CLOSED_INSTALLED_V1'
@torch.no_grad()
def main():
 ...
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('80sequence equivalents;36block calls;CPU norm-closed mediator');return
 ...
 model=load_model_fast().eval();fixtures=torch.load(P/'CITY_MLP8_NORM_CLOSED_V1_ARTIFACT.pt',weights_only=True)['fixtures'];old=torch.load(P/'CITY_VALUE_MEDIATION_FRESH_V1_ARTIFACT.pt',weights_only=True)['values'][[0,3]]
 ...
 r={'pred_a':float(diff.abs().max())<=1e-4 and float(diff.norm()/old.norm())<=1e-5,'pred_b':max(errors)<=1e-4,'pred_c':outside==0 and bool(torch.isfinite(v).all()) and calls==36 and not torch.cuda.is_initialized(),'readout_effect_errors':errors,'replay_max_abs':float(diff.abs().max()),'replay_relative':float(diff.norm()/old.norm()),'outside':outside,'body_forwards':80,'batched_block_calls':calls,'seconds':time.perf_counter()-start,'scope':'Opened equivalence: exact native cityswap plus norm-closed MLP8value correction. Native z8, upstreamdelta and tokenIDs remain inputs; editedrho9 generated; fullsuffix external.','source_shas':binding}
