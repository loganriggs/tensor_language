...
STEM='CITY_RESIDUAL6_SINGLE_INPUT_FRESH_V1'
...
def main():
 ...
 candidates=[];query_errors=[];write_errors=[];fixtures=[]
 for i,row in enumerate(groups):
 ...
 preflight={'query_gate':max(query_errors)<=1e-4,'finite':all(bool(torch.isfinite(w).all()) for w in candidates),'max_query_error':max(query_errors),'max_write_error':max(write_errors),'query_errors':query_errors,'write_errors':write_errors}
  ...
  for arm in range(19):
   ...
   for i,row in enumerate(groups):
    ...
    if arm<=2:d=writes[i][arm-1].float()
     ...
     gen=torch.Generator().manual_seed(18095000+1000*(arm-3)+row['context_id'])
 ...
 r={'pred_a':row_audit['passes'] and len(rows)==240 and preflight['query_gate'] and outside==0 and max(norm_errors)<=1e-5 and bool(torch.isfinite(v).all()) and count==800 and calls==360 and not torch.cuda.is_initialized(),
    ...
    'scope':'Fresh20document CPU single-residual6 generator confirmation; approximate mixed8 RMS; full native MLP8/suffix external. Fixed spelling probes, not actual next-token labels. No independent composition or total compression claim.','source_shas':binding}
