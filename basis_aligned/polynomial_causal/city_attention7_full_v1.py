"""Generate full attention7 and MLP7 inputs from residual6, using native factors."""
import torch
...
def execute(program,residual6,token_ids):
    ...
    def factor(name):
        x=F.rms_norm(F.linear(current,program[name]).reshape(b,t,9,128),(128,));a,z=x.chunk(2,-1)
    ...
    channels=torch.einsum('bhqs,bshd->bqhd',pattern,value.reshape(b,t,9,128)).reshape(b,t,1152)
