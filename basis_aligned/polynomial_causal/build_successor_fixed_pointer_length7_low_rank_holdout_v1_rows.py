#!/usr/bin/env python3
"""Freeze outcome-blind length-seven rows for successor typed-group confirmation."""
...
OUT = HERE / "SUCCESSOR_FIXED_POINTER_LENGTH7_LOW_RANK_HOLDOUT_V1_ROWS.json"
FAMILIES = {
    "month": ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"],
    "digit": [str(i) for i in range(10)],
}
FINAL_INDICES = {"month": range(6, 10), "digit": range(6, 8)}
...
def main():
    ...
    for family, elements in FAMILIES.items():
        for final_index in FINAL_INDICES[family]:
            coherent = elements[final_index - 6:final_index + 1]
            orders = {
                "coherent": coherent,
                "early_swap_control": [coherent[1], coherent[0], *coherent[2:]],
                "late_swap_incoherent": [*coherent[:4], coherent[5], coherent[4], coherent[6]],
            }
            for condition in CONDITIONS:
                text, ids = prompt(orders[condition])
                row = {
                    "row_id": canonical(["length7", family, final_index, condition]),
    ...
    payload = {
        "schema": "successor_fixed_pointer_length7_low_rank_holdout_v1_rows",
        "selection_rule": "all length-seven month endpoints at indices 6..9 and digit endpoints at indices 6..7; fixed first-pair and penultimate-pair transpositions",
