...
The [sparse quartic core method](SPARSE_QUARTIC_CORE_V1_MATH.md) now implements exact coefficients and conditional output elimination for learned input directions, plus a shared quadratic-pair execution graph. Small-tensor controls pass; the first two-start native fit is managed-live. Follow its result and convergence status before interpreting structure.
