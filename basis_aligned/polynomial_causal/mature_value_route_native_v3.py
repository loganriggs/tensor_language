"""Native factors plus raw RMS and MLP8 inputs/outputs; completed v2 preserved."""
import torch
...
def capture(backend,world,q,mean,readers):
    model=backend.model;attn=model.transformer.h[9].attn;record={};bases=[];inputs=[];values=[];raws=[];recurrence=[];mlp_inputs=[];mlp_outputs=[]
    ...
    try:
        with torch.inference_mode():
            for start in (0,16):
                ...
                mlp_inputs.append(c['mlp_input']);mlp_outputs.append(c['mlp_output'])
    ...
    return {'native_logits':torch.cat([b['logits'] for b in bases]).cpu().tolist(),
            ...
            'mlp_inputs':torch.cat(mlp_inputs),'mlp_outputs':torch.cat(mlp_outputs),
