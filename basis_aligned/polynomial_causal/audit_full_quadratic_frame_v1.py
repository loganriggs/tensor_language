...
import torch
from full_quadratic_frame_v1 import fit
...
result=dict(correction='Original seed544 control started at truth; preserve receipt and replace recovery evidence with these independent starts.',
...
assert result['all_recovered'] and result['all_gradient_stops']
with Path(__file__).with_name('FULL_QUADRATIC_FRAME_V1_RESTART_AUDIT.json').open('x') as f:
