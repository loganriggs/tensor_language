# Large unlabeled MLP17 data capture V2 — storage repair
...
V1 failed while converting native states to direct FP16. Preserve the failed runner log. V2 first checks native finiteness, divides each state by its maximum absolute value/1024, stores scaled FP16 plus a FP32 scale, and measures error after decoding. Same rows, splits, counts and <=5e-4 fidelity bar. No optimization or scientific result from V1. Decode x*x_scale and y*y_scale in FP32.
