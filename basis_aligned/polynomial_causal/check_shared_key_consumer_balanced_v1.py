...
from shared_key_consumer_balanced_v1 import compile_program,scalar
...
def main():
 ...
 full=full_compile(native,low,lam[0]);candidates={r:compile_program(native,low,lam[0],r) for r in [16,32,48]}
 ...
 out={'records':records,'orthogonality_error':ortherror,'baseline_program_scalars':size(full),'scope':'Exact SVD after equalizing consumer Frobenius norms, not data fit. Paired scalar effects versus same rank64-transport program, existing72prompts. No fresh/native-logit/semantic-selectivity claim.'}
 (P/'SHARED_KEY_CONSUMER_BALANCED_V1_CONTROL.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
