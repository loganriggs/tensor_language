...
import tiktoken
P=Path(__file__).resolve().parent;STEM='CITY_FULL_PILE_V2'
...
def main():
 ...
 source='monology/pile-uncopyrighted';ds=load_dataset(source,split='train',streaming=True);contexts=[];rows=[];seen=set();scanned=0;collisions=0
 for di,doc in enumerate(itertools.islice(ds,MAX_DOCS)):
  ...
  for pos,tok in enumerate(ids):
   ...
   cid=len(contexts);meta=doc.get('meta',{});contexts.append({'context_id':cid,'variant':'pile','template':cid,'pair':pi,'source_doc_index':di,'source_token_position':pos,'document_sha256':dh,'source_meta':meta,'natural_cue':cue,'natural_text':enc.decode(window),'panel_status':'unopened for full-city operator; corpus may have prior unrelated use'})
   for ei,(ui,si) in enumerate(endpoint_ids):
    for label in ['British','American']:
     seq=variants[label];rows.append({'row_id':len(rows),'context_id':cid,'variant':'pile','template':cid,'pair':pi,'cue':label,'city':PAIRS[pi][0 if label=='British' else 1],'text':enc.decode(seq),'ids':seq,'city_position':LEFT,'destination_positions':list(range(LEFT+1,len(seq)-1)),'endpoint':ei,'uk_id':ui,'us_id':si,'is_untouched_natural_arm':label==cue,'document_sha256':dh})
   seen.add(dh);break
 ...
 doc={'schema':'regional.natural_context.rows.v1','utc':datetime.now(timezone.utc).isoformat(),'source':source,'source_split':'train','docs_scanned':scanned,'selection':'First preposition-qualified eligible occurrence in first20distinct eligible documents in stream order.32tokens,12beforecity, exactly one mapped city in window. Requires immediately preceding in/from/near/outside/around/at/to; excludes previously frozen document hashes. No model outcomes/capability filtering. One untouched arm and one city substitution. Endpoints are fixed probes, not observed next-token labels. No pretraining-disjointness guarantee.','contexts':contexts,'rows':rows,'variants':['pile'],'endpoints':endpoints,'city_pairs':PAIRS}
