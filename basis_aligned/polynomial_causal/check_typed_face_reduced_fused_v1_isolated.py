...
P=Path(__file__).resolve().parent;STEM='TYPED_FACE_REDUCED_FUSED_V1'
def main():
 with tempfile.TemporaryDirectory(prefix='coupled-isolated-') as tmp:
  root=Path(tmp);shutil.copytree(P/'extracted_circuits/typed_face_reduced_fused_v1',root/'package',ignore=shutil.ignore_patterns('__pycache__'))
