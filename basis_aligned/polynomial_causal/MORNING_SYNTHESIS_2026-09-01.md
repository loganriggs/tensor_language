...
|---:|---|---|---|---|---|---|---|
| 1 | Executable error contracts and lower bounds | Family tail laws have log-space R2 `.995`; the Q/K certificate-damage ray has R2 `.99945`; held-out composition certificate MAE is `0` in the calibrated test | No parameters by itself, but it selected and certified the adopted 34.14M-scalar reduction | High inside the measured Q/K, MLP, and value families; explicitly empirical outside them | High: predicts certificate survival, not only CE | Low CPU calibration, then one GPU confirmation | **Advance.** Fit all-layer MLP tail curves, solve a certificate-constrained water-filling problem, then build exactly one selected vector per fidelity tier |
| 2 | Direct bilinear-MLP compression | Native CP-atom sharing and a low-rank layer axis failed; context-metric shared-input rank succeeded at all 18 layers; two selected layers compose cleanly | 5,308,416 scalars removed by the adopted two p768 MLP cuts; prospective pool 47.8M at p768 | High for the installed subspaces and literal maps; low for raw CP atoms | High after the installed pair passed signed transport | Moderate | **Advance cautiously.** Test one prospectively selected next layer; thereafter use an all-layer allocator rather than subset search |
| 3 | Joint untied-vocabulary factorization | Frequency-weighted shared code scores `+.193/.225` FW/Wiki versus matched independent `+.552/.778` | 30.28M vocabulary scalars at the screened point, not adopted | Medium-high: shared beats a price-matched independent factorization on two corpora, but rare-token residuals dominate | None yet | Moderate | **Park as high-upside.** A fit-selected sparse exact-row residual needs census, rare-token certificates, composition, and signed gates before any claim |
...
3. Fit per-layer/context-rank tail curves for all 18 MLPs and solve a water-filling allocation with the certificate
   ray as the constraint.  Confirm one selected vector, not a sequence of hand-chosen subsets.
...
5. Revisit vocabulary sharing only as a separately budgeted rare-row-residual branch.  Its upside is large, but it
   has not passed census, certificates, composition, or intervention transport.
