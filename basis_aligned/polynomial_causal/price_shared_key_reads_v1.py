...
def main():
 ...
 import statistics
 def oldrun():
  rd,rh=features(z,h,u,a,old);return old_scalar(rd,rh)
 def newrun():
  rd,rh=features(z,h,u,a,p);return scalar(rd,rh,p)
 oldrun();newrun();times=[[],[]]
 for trial in range(7):
  for index in ([0,1] if trial%2==0 else [1,0]):
   fun=[oldrun,newrun][index];start=time.perf_counter()
   for repeat in range(20):fun()
   times[index].append((time.perf_counter()-start)/20)
 oldtime,newtime=map(statistics.median,times)
 price={'pred_a':oldtime/newtime>=1.1,'old_seconds':oldtime,'new_seconds':newtime,'speedup':oldtime/newtime,'sequence_length':n,'batch':1,'scope':'FP64 two-thread CPU folded features plus complete scalar including expansion. Final cached prefix, seven interleaved trials of20repetitions. Excludes context generation, compile time, native suffix.'}
 (P/'SHARED_KEY_READS_V1_PRICE.json').write_text(json.dumps(price,indent=2)+'\n');print(json.dumps(price))
