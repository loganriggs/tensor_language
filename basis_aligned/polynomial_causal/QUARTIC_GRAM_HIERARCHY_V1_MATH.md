...
## Native execution and the input-subspace limitation

The [frozen executable group](QUARTIC_GROUP_PROGRAM_V1.json) combines two centered output modes, each with eight signed quadratic squares. It stores 22,928 fitted floats per seed, including shared input readers and output writers. Shared quadratic-pair deletion has explicit cross terms: removing two inputs jointly is not the sum of their separate removals. Execution and removal identities hold within 1.7e-15.

[Native validation](QUARTIC_GROUP_NATIVE_V1.json) uses 128 previously cached developmental endpoints, with newly captured actual normalized MLP16 inputs. The compact group reproduces its own exact input-projected reference: physical-write errors are 0.407% and 0.741%, and paired-swap errors are at most 2.44%, with all signs preserved. However, the two fitted replicas disagree by 59.1% on writes and 44–75% on swaps. Accurate extraction of a self-defined approximation does not identify a stable circuit.

The [boundary red-team](QUARTIC_GROUP_BOUNDARY_V1.json) independently crosses the two input projectors and two output projectors. Symmetric difference accounting attributes a term with 99.0% of the total discrepancy norm to the input boundary, versus 8.1% to the output boundary. These are overlapping norm ratios, not additive variance shares. Removing the input projection, while keeping each frozen output space, lowers write disagreement to 5.66%. This changes the component definition and retains the full native weights; the small projected program does not implement it.

The [full-input native-effect screen](QUARTIC_GROUP_LIFTED_NATIVE_V1.json) passes swap replication across all four families: relative errors are 9.38% for agreement verbs, 6.41% for count nouns, 6.69% for past tense and 9.56% for progressive forms, with all signs agreeing. Removal-CE disagreement still misses the 0.02 threshold for verbs (0.02196) and nouns (0.03497); past and progressive pass. CE added above the real model is damage, lower is better. These are component-only interventions with the native background, input denominator, final RMS and capped logits retained. There is no fresh/OOD or selective-circuit claim.
