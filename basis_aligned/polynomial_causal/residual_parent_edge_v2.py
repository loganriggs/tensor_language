"""Numerically bounded private-space replacement for frozen spectral parent 1.

Preserve all old parents and group rank. Optimize the one discarded private
direction with a scalar eigenvalue-envelope bound. Each proposed core is then
the exact conditional minimizer in the selected basis.
"""
import copy
import hashlib
import json
...
from private_direction_bound_v1 import optimize
...
def propose(graph, g, parent, bases, writers, cores, target, penalty):
    ...
    discarded = private_span.shape[1]-size
    assert discarded in (0, 1)
    if discarded:
        selected, bound = optimize(s, len(ids), (1+penalty)*99245061353.47293*1e-9)
    else:
        selected = torch.eye(size)
        bound = dict(converged=True, absolute_loss_gap=0., normalized_projection_replay=0., splits=0)
    bound['normalized_objective_gap'] = bound['absolute_loss_gap']/((1+penalty)*99245061353.47293)
    ...
    return dict(group=g, ids=ids, private=private.T, core=core, bound=bound,
...
def main():
    ...
    out = root/'RESIDUAL_PARENT_EDGE_V2.json'
    ...
    errors = dict(objective_delta=abs(float(measured_delta)-rows[selected]['conditional_delta']),
                  ...
                  numerical_global_gap=max(row['bound']['normalized_objective_gap'] for row in rows),
                  projection_identity=max(row['bound']['normalized_projection_replay'] for row in rows))
    ...
    artifact = root/'RESIDUAL_PARENT_EDGE_V2.pt'
    ...
    result = dict(pred_a=valid, pred_b=held_b,
                  ...
                  scope='Numerically bounded conditional private-space search within each old-span-plus-parent union. No global reader/subspace/topology optimization, formal interval certificate, data fitting, native behavioral or circuit-identification claim.')
