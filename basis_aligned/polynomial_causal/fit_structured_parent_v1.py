"""Ten fixed-support two-of-four sparse complete-even fits, weights only.
...
def main():
    ...
    rotitem=torch.load(p/'STRUCTURED_PARENT_READER_V1_INITIAL.pt',weights_only=True)['rotated']
    rs,_,_=unpack(rotitem);baseline=float(objective(rs));rows=[];best=None
    for frame,file in [('original','STRUCTURED_PARENT_READER_V1_INITIAL.pt'),('rotated','STRUCTURED_PARENT_READER_V1_INITIAL.pt')]:
    ...
    torch.save({'2of4':best[1]},p/'STRUCTURED_PARENT_FIT_V1_PROGRAM.pt')
    ...
    (p/'STRUCTURED_PARENT_FIT_V1_RESULT.json').write_text(json.dumps(result,indent=2)+'\n')
