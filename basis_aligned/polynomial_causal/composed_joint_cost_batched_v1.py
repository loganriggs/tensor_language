...
C batched shared >=10%faster than batched direct at12pairs.
...
def main():
    ...
    def batched_direct(count):
        amplitudes=torch.cat([v for a,b in pairs[:count] for v in (a,b,a+b)],dim=0)
        raw=raw10+scale*(h9(z9-amplitudes*writer)-h0)
        ports,rho=attn.prepare(raw,matrices)
        z=raw+attn.execute(ports,rho,first.expand_as(raw),mixture,output)
        states=joint.block_output(z,l10,r10,d10,bias10)
        return [x[None] for x in states.unbind(0)]
    def batched_shared(count):
        ctx=prepare(z9,baseline_mlp9,raw10,z10,first,program,scale,matrices,mixture,output)
        amplitudes=torch.cat([v for a,b in pairs[:count] for v in (a,b,a+b)],dim=0)
        ctx['first_values']=first.expand(amplitudes.shape[0],-1,-1)
        states=joint.block_output(joint.branch(amplitudes,ctx),l10,r10,d10,bias10)
        return [x[None] for x in states.unbind(0)]
    variants={'direct':full_direct,'shared':full_shared,'batched_direct':batched_direct,'batched_shared':batched_shared}
    cells=[]
    for count in (1,4,12):
        ...
        cells.append(dict(pairs=count,branch_replay_errors=errors,median_seconds=med,
                          ...
                          batched_shared_speedup_over_batched_direct=med['batched_direct']/med['batched_shared'],batched_median_best_speedup=med['batched_direct']/min(med['shared'],med['batched_shared'])))
    ...
    result=dict(pred_a=all(max(c['branch_replay_errors'].values())<=1e-10 for c in cells),
                ...
                pred_c=cells[-1]['batched_shared_speedup_over_batched_direct']>=1/0.9,
                ...
                scope='Actual weights, synthetic B1,T17,d1152,FP64,CPU2threads. Three changed branches per pair. Identical output list retention. Shared includes all global/context preparation each measured call. Batched variants stack all3Nknown amplitudes in batch dimension, with first-values expanded, same causal sequence length. Pristine z9/MLP9/raw10/z10/first-values common supplied; no prefix or suffix cost. Local weight inventory counts objects needed by conditional operator only; native weights remain necessary for pristine background and other edits. Not whole-model compression/speed.')
    (P/'COMPOSED_JOINT_COST_BATCHED_V1_RESULT.json').write_text(json.dumps(result,indent=2)+'\n')
    print(json.dumps({k:v for k,v in result.items() if k!='cells'},indent=2))
    for c in cells:print(c['pairs'],c['median_seconds'],c['shared_speedup_over_direct'],c['batched_shared_speedup_over_batched_direct'])
