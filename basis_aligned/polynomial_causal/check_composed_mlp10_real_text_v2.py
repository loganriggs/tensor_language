"""Integrated shared response graph on two native prefixes.
Registered: old generator/product replay<=1e-10; full native discrepancy reported
without claiming fresh OOD. Existing native background/suffix supplied; candidate
changed attention states and joint norms generated internally.120second bound.
"""
...
from composed_mlp10_inputs_v2 import execute, product as shared_product


@torch.no_grad()
def main(indices=(0,96), output_name='COMPOSED_MLP10_REAL_TEXT_V2_CONTROL.json'):
    ...
    for index in indices:
        ...
        legacy=old_execute(z9,m9.double()-b9.mlp.Down_bias.double(),raw0,z0,v1.double(),a,b,
                          program,float(b10.lambdas[0]),matrices,float(b10.attn.lamb),b10.attn.c_proj.weight.double())
        generator_replay=max(rel(generated[k],legacy[k]) for k in ['child','remainder','child_residual','remainder_residual','child_attention','remainder_attention','joint_rho'])
        changes=[];states=[]
        ...
        predicted=shared_product(generated,L,R,D)
        shared_product_replay=rel(predicted,product(legacy['child'],legacy['remainder'],legacy['joint_rho']))
        assert generator_replay<=1e-10 and shared_product_replay<=1e-10
        ...
        cells.append(dict(generator_replay=generator_replay,shared_product_replay=shared_product_replay,native_effects=effects[0].tolist(),predicted_effects=effects[1].tolist(),absolute_effect_errors=(effects[1]-effects[0]).abs().tolist(),row=index,tokens=len(rows[index]['ids']),child_error=rel(generated['child'],c),
    ...
    result=dict(indices=list(indices),cells=cells,seconds=time.perf_counter()-tic,
                scope='Integrated shared response and six-product graph; fixed historical rows listed in indices. Candidate generates changed attention and joint norm. '
                ...
                'Small real-state integration check; not full-panel native validation of this new generator.')
