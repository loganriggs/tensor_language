...
def main():
    ...
    for panel in ['opposite','congruent']:
        ...
        for template,pattern in templates:
            for i,pair in enumerate(pairs):
                for number in [0,1]:
                    ...
                    assert (apos<spos==len(ids)-1) if template=='behind_subject' else (spos<apos<len(ids)-1)
        ...
        out=p/f'SOURCE_OOD_V3_{panel.upper()}_ROWS.json';assert not out.exists();out.write_text(json.dumps(rows,indent=2)+'\n');result[panel]=dict(file=out.name,sha256=hashlib.sha256(out.read_bytes()).hexdigest(),rows=24)
    (p/'SOURCE_OOD_V3_ROW_AUDIT.json').write_text(json.dumps(result,indent=2)+'\n');print(result)
