...
def execute(program, head, normalized_mlp7_city, other_city_sources, lambda8,
    ...
    folded=readers(program,normalized_mlp7_city).double()*lambda8.double()
    ...
    inv=1/(10000**(torch.arange(0,128,2,dtype=torch.float32,device=normalized_mlp7_city.device)/128))
    ...
    support=destination & (torch.arange(t,device=normalized_mlp7_city.device)>=city)
