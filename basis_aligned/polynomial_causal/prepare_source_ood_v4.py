...
def main():
    ...
    pairs=[('nurse','nurses'),('guard','guards'),('chef','chefs'),('student','students'),('worker','workers'),('guest','guests')]
    templates=[('close_to','The {s} close to the {a}'),('next_to_subject','Next to the {a}, the {s}')]
    old=set()
    for name in ['SUBJECT_ATTRACTOR_CONTROL_V696_ROWS.json','SUBJECT_CONGRUENT_ATTRACTOR_V697_ROWS.json','SUBJECT_HYBRID_FRESH_V687_ROWS.json','SEMANTIC_PORT_FRESH_OPPOSITE_ROWS.json','SEMANTIC_PORT_FRESH_CONGRUENT_ROWS.json','SOURCE_OOD_V1_OPPOSITE_ROWS.json','SOURCE_OOD_V1_CONGRUENT_ROWS.json','SOURCE_OOD_V2_OPPOSITE_ROWS.json','SOURCE_OOD_V2_CONGRUENT_ROWS.json','SOURCE_OOD_V3_OPPOSITE_ROWS.json','SOURCE_OOD_V3_CONGRUENT_ROWS.json']:
    ...
    for panel in ['opposite','congruent']:
        ...
        for template,pattern in templates:
            for i,pair in enumerate(pairs):
                for number in [0,1]:
                    ...
                    assert (apos<spos==len(ids)-1) if template=='next_to_subject' else (spos<apos==len(ids)-1)
        ...
        out=p/f'SOURCE_OOD_V4_{panel.upper()}_ROWS.json';assert not out.exists();out.write_text(json.dumps(rows,indent=2)+'\n');result[panel]=dict(file=out.name,sha256=hashlib.sha256(out.read_bytes()).hexdigest(),rows=24)
    (p/'SOURCE_OOD_V4_ROW_AUDIT.json').write_text(json.dumps(result,indent=2)+'\n');print(result)
