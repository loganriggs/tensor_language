"""Prune transported rational response directions, not raw original writers.
A full reference state/scalar <=1e-4; B at least one omitted term keeps
routing and scalar response errors <=.1 in each cached cue family.
"""
...
def main():
 ...
 for omit in [-1,0,1,2]:
  delta=terms.sum(0)-(terms[omit] if omit>=0 else 0);gp,sp=evaluate(raw[0]+delta);cells=[]
  for family in sorted(set(r['family'] for r in rows)):
   ...
   cells.append(dict(family=family,state_error=error(delta[ix][mask],(raw[1,ix]-raw[0,ix])[mask]),routing_error=error((gp[ix]-g0[ix])[gm],(cache['gamma'][1,ix]-cache['gamma'][0,ix])[gm]),scalar_error=error(sp[ix,last]-s0[ix,last],cache['scalar'][1,ix,last]-cache['scalar'][0,ix,last])))
  records.append(dict(omitted=['full','w','p','q'][omit+1],cells=cells))
 A=all(max(c['state_error'],c['scalar_error'])<=1e-4 for c in records[0]['cells']);passing=[r['omitted'] for r in records[1:] if all(max(c['routing_error'],c['scalar_error'])<=.1 for c in r['cells'])]
 out={'pred_a':A,'pred_b':A and bool(passing),'passing_omissions':passing,'records':records,'scope':'Cached native head8 removal, full head9 routing/scalar reevaluation. Rational term grouping with native pristine MLP background and dense J. No fitted coefficients, fresh data, logit or whole-program savings claim.'}
 (P/'TRANSPORTED_RATIONAL_TERMS_V1_CONTROL.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
