# MLP0 token-target CROSS-CORPUS separability probe — preregistration (Claude CPU lane)

**Registered:** 2026-09-03 14:50 UTC. **Owner:** Claude. **Script:** `ops/mlp0_token_target_cross_corpus_separability_probe.py`
(math: `ops/mlp0_hybrid_separability_lib.py` + `ops/mlp0_hybrid_separability_corpus_lib.py`, hash-frozen). Zero forwards.

## Object

TOKEN target (observed Dg_T + Dg_I -> Dg_T; §2686). §2686/§2687 used a UNIFORM token distribution. R535 found the equality S/R coordinates corpus-unstable (natural
...
sampling noise, and does a projector fitted on one corpus lose fidelity on the other? Here the token distribution enters through the CENTERED token covariance in both S_T and S_I.
...
- **pred_a_instrument_uniform_map_reproduces_frozen_result** — uniform-weight path reproduces §2686 rho=1 residual .342 within 1e-4;
