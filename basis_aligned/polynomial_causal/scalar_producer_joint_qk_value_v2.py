...
@torch.no_grad()
def main():
 torch.set_num_threads(2);tic=time.perf_counter();out=P/'SCALAR_PRODUCER_JOINT_QK_VALUE_V2_RESULT.json';assert not out.exists()
 ...
 rows=json.loads((P/'STRUCTURED_PRODUCER_CONFIRMATION_V1_ROWS.json').read_text())['rows'];values=torch.zeros(5,48,22,dtype=torch.float64)
 ...
 for i,row in enumerate(rows):
  ...
  values[4,i,:n]=((g1-g0)@(v1-v0)[...,None])[0,:,0]
 ...
 for family in range(2):
  ix=[i for i,r in enumerate(rows) if r['family']==family];delta=values[1,ix]-values[0,ix];dv=values[2,ix]-values[0,ix];dq=values[3,ix]-values[0,ix];mix=values[4,ix]
  # Mixed term is independently contracted from routing and value differences; V1 subtraction identity was tautological.
 ...
 result={'pred_a':A,'pred_b':A and all(c['value_only_error']<=.1 for c in cells),'pred_c':A and all(c['mixed_to_full_norm']<=.1 for c in cells),'cells':cells,'arms':['native','joint_changed','value_changed','routing_changed','independent_mixed'],'seconds':time.perf_counter()-tic,'scope':'Four-corner conditional scalar9 decomposition after component8 removal; QK1*QK2 kept together. Reused native states, no factor fitting or final-effect adoption.'}
 torch.save(dict(scalars=values),P/'SCALAR_PRODUCER_JOINT_QK_VALUE_V2_ARTIFACT.pt');out.write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
