"""Warm-primal ADMM with finite residual balancing for minimum nuclear norm Gram representation of a fixed quartic.
...
def solve(target,mapping,iterations=2000,tolerance=1e-7,rho=1.,initial=None):
    scale=target.norm();assert scale>0;c=target/scale
    z=canonical(c,mapping) if initial is None else initial/scale;u=torch.zeros_like(z);history=[];changes=[];start=time.perf_counter()
    for iteration in range(iterations):
        ...
        if iteration<1000 and iteration%25==24:
            rn=float((x-z).norm());sn=float(rho*(z-previous).norm());oldrho=rho
            if rn>10*sn:rho=min(64.,rho*2)
            elif sn>10*rn:rho=max(1/64,rho/2)
            if rho!=oldrho:
                u*=oldrho/rho;changes.append(dict(iteration=iteration,old_rho=oldrho,new_rho=rho))
    ...
    return x*scale,dict(converged=gap<=tolerance and gap>=-1e-10 and residual<=tolerance and dual_residual<=tolerance,
        iterations=iteration+1,relative_gap=gap,final_rho=rho,rho_changes=changes,primal_residual=residual,dual_residual=dual_residual,
