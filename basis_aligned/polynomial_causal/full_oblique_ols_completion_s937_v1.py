"""Second frozen oblique start: same full recoding, plus function stability."""
...
def main():
    ...
    parent=json.loads((P/'OBLIQUE_READER_DICTIONARY_V1_ordinary_covariance_SEED_937.json').read_text())
    ...
    cache=Path('/dev/shm/bilin18_full_oblique_ols_completion_s937_v1.pt');assert not cache.exists()
    ...
    first=json.loads((P/'FULL_OBLIQUE_OLS_COMPLETION_V1_RESULT.json').read_text())
    first_path=Path(first['cache']['path']);assert hashlib.sha256(first_path.read_bytes()).hexdigest()==first['cache']['sha256']
    previous=torch.load(first_path,weights_only=True,map_location='cpu')
    previous_codes=torch.zeros(9216,1152).scatter_(1,previous['code_indices'].long(),previous['code_values'])
    fa,fb=(previous_codes@previous['analysis_basis']).split(4608);first_function=(fa,fb,writers)
    first_energy=inner(first_function,first_function);new_energy=inner(proposal,proposal)
    function_cosine=float(inner(first_function,proposal)/(first_energy*new_energy).sqrt())
    result=dict(predictions=dict(pred_a_instrument=valid,pred_b_tensor_gain=gain>=.01,
        ...
        pred_d_replication=capture>=first['coefficient_capture_after']-.01,pred_e_function_stability=function_cosine>=.9),
        function_cosine_to_first_start=function_cosine,first_start_source=first['cache'],
    ...
    with (P/'FULL_OBLIQUE_OLS_COMPLETION_S937_V1_RESULT.json').open('x') as f:json.dump(result,f,indent=2);f.write('\n')
