...
def routing(p, current, city_state, city, freeze_mask=0):
    b,t,d = current.shape
    ...
    factors=[]
    for index,(qn,kn) in enumerate([('q1','k1'),('q2','k2')]):
        q=rotate(F.rms_norm(F.linear(current,p[qn].to(current.dtype)),(128,)))
        numerator=F.linear(key_current,p[kn].to(current.dtype))
        if freeze_mask & (1 << index):
            reference=F.linear(current,p[kn].to(current.dtype))
            denominator=(reference.square().mean(-1,keepdim=True)+torch.finfo(current.dtype).eps).sqrt()
            k=rotate(numerator/denominator)
        else:
            k=rotate(F.rms_norm(numerator,(128,)))
...
def execute(p, current, donor_city_state, recipient_token, donor_token, city, destination, freeze_mask=0):
    a0=routing(p,current,current[:,city],city)
    a1=routing(p,current,donor_city_state,city,freeze_mask)
