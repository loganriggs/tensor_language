...
def face_masks(port_indices: Sequence[int], n_ports: int) -> tuple[int, ...]:
    """Return the complete nonempty Boolean face generated by named ports.

    A factorized producer/consumer edge should usually be selected as a closed
    face rather than as unrelated Möbius atoms.  For example, ports ``(0, 2)``
    produce masks ``(1, 4, 5)``: both main effects and their interaction.
    """
    indices = tuple(int(index) for index in port_indices)
    if n_ports < 1 or not indices or len(indices) != len(set(indices)):
        raise ValueError("port indices must be a nonempty distinct sequence")
    if min(indices) < 0 or max(indices) >= n_ports:
        raise ValueError("port index outside graph")
    active = sum(1 << index for index in indices)
    return tuple(mask for mask in submasks(active) if mask)
...
def select_graph_family(target, atoms: Mapping[int, object], discovery_indices: Sequence[int],
                        families: Sequence[Sequence[int]]):
    """Choose one complete candidate graph on discovery rows without scaling.

    Candidate families are supplied by the caller's typed structural prior.
    The selector only compares their component-relative L2 errors; it never
    invents or drops atoms within a family.  Ties are deterministic by masks.
    """
    import numpy as np

    normalized = tuple(tuple(int(mask) for mask in family) for family in families)
    if not normalized or any(not family for family in normalized):
        raise ValueError("families must be nonempty graphs")
    if len(normalized) != len(set(normalized)):
        raise ValueError("duplicate graph family")
    indices = np.asarray(tuple(discovery_indices), dtype=np.int64)
    if indices.size == 0 or np.any(indices < 0):
        raise ValueError("discovery indices must be nonempty and nonnegative")
    target_array = np.asarray(target)
    reference = target_array[indices]
    denominator = max(float(np.linalg.norm(reference)), 1e-30)
    records = []
    for family in normalized:
        if len(family) != len(set(family)):
            raise ValueError("duplicate mask within graph family")
        missing = [mask for mask in family if mask not in atoms]
        if missing:
            raise ValueError(f"missing family atoms: {missing}")
        prediction = np.asarray(compose(atoms, family))[indices]
        error = float(np.linalg.norm(reference - prediction) / denominator)
        records.append({"masks": family, "relative_l2": error})
    winner = min(records, key=lambda record: (record["relative_l2"], record["masks"]))
    return winner["masks"], tuple(records)
