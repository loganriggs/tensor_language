"""V2: positive-diagonal QR preserves the initial graph and removes sign jumps.
V1 failed its reduced/full-objective replay and remains recorded separately.
"""
import json
import time
...
class Objective:
    ...
    def physical(self,point):
        ...
        private, triangular=torch.linalg.qr(private,mode='reduced')
        signs=torch.where(torch.diagonal(triangular,dim1=-2,dim2=-1)>=0,1.,-1.)
        private=private*signs[:,None,:]
...
def main():
    ...
    for label in ('spectral','native'):
        ...
        with torch.no_grad():
            _,_,_,initial_matrices=objective.physical(torch.tensor(objective.initial))
            expected=torch.stack([union.T@g['basis']@g['core']@g['basis'].T@union for g in move['groups']])
            initial_replay=float((initial_matrices-expected).norm()/expected.norm())
            assert initial_replay<1e-10, initial_replay
        ...
        executable_floats=move['parent'].numel()+sum(g[k].numel() for g in move['groups'] for k in ('private','beta','lam','alpha','writer'))
        parent_replay=max(float((g['basis'][:,0]-move['parent']).norm()) for g in move['groups'])
        row=dict(label=label,pair=move['pair'],initial_function_replay=initial_replay,shared_parent_replay=parent_replay,executable_floats=executable_floats,capture_loss=final_change,projection_capture_loss=projection,
                 ...
                 pred_c=bool(executable_floats==38078 and parent_replay<1e-10),floats_saved=39200-executable_floats,scope='Fixed-union shared-parent graph optimization; no global or whole-model convergence guarantee.')
        rows.append(row);print(json.dumps(row),flush=True)
        torch.save(dict(parent=move['parent'],groups=move['groups'],pair=move['pair']),f'/dev/shm/bilin18_ll1_shared_parent_optimize_v2_{label}.pt')
    (root/'LL1_SHARED_PARENT_OPTIMIZE_V2_AUDIT.json').write_text(json.dumps(dict(rows=rows,seconds=time.perf_counter()-start),indent=2)+'\n')
