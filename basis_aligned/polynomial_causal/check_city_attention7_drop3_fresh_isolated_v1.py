...
def main():
    source=P/'extracted_circuits/city_attention7_drop3_v1'
    captured=torch.load(P/'CITY_ATTENTION7_DROP3_FRESH_V1_ARTIFACT.pt',weights_only=True)['fixtures']
...
r={'passes':max(errors)<=1e-10 and max(outside)==0 and finite and unknown and len(fixtures)==40,
   ...
   'imports_outside_repository':True,'seconds':time.perf_counter()-start,'scope':'Isolated CPU declared-input replay; fresh residual6 fixtures and weight-derived fresh token table; packed attention7 and drop3 executable; readers/head8 unchanged; compares generator writes.'}
print(json.dumps(r))
''')
    ...
    (P/'CITY_ATTENTION7_DROP3_FRESH_V1_ISOLATED_RESULT.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
