"""Signed raw/normalization effect accounting; no source promotion from size."""
import hashlib,json
...
def main():
    p=Path(__file__).resolve().parent;source=p/'MATURE_VALUE_NORMALIZATION_V1_RESULT.json'
    ...
    for row in data['reports']:
        ...
        for kind in ('margin','readers'):
            ...
            parts={label:{'signed_projection':float(np.sum(e[k]*full)/den),
                   ...
                   for label,k in [('raw','mixed'),('normalization','spill'),('interaction','interaction')]}
    ...
    result={'reports':reports,'source_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),
            ...
            'scope':'Opened-output signed attribution relative to the full partial-component effect. Negative/greater-than-one projections indicate cancellation. Neither magnitude nor projection overrides failed fidelity.'}
    with (p/'NORMALIZED_VALUE_BRANCH_ACCOUNTING_V1_RESULT.json').open('x') as f:json.dump(result,f,indent=2);f.write('\n')
    for layout in ('original','fronted_pp'):
        ...
        print(layout,{k:{v:[min(r['margin'][k][v] for r in selected),max(r['margin'][k][v] for r in selected)]
                         for v in ('signed_projection','relative_norm')} for k in ('raw','normalization','interaction')})
