"""Frozen complete-even-error shared reader subspace; local fitted candidate."""
import torch
...
def compile_program(native,low,gain,rank):
    ...
    assert rank==48
    rotation=torch.load(Path(__file__).resolve().parent/'COMPLETE_EVEN_KEY_FIT_V1_PROGRAM.pt',weights_only=True)['basis_rotation'];vh=rotation.T
