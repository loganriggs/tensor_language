...
## Precision and intervention-strength counterchecks, 13 September 09:46

The [same-state readout diagnostic](SELF_CONSUMER_READOUT64_V1_RESULT.json) exactly replays the original FP32 scores. Keeping the transformer states FP32 but evaluating final RMS, unembedding, saturation and log-softmax in FP64 reduces FineWeb0 composition error from 64.32% to 33.36% for the target, and from 52.80% to 39.10% for the control. Both still fail 20%. This is a precision diagnostic, not a different model adopted for prediction.

The [fourfold intervention](SELF_CONSUMER_STRENGTH4_V1_RESULT.json) keeps the same 16 prefixes and multiplies only the removed node by four. With FP64 readout, all-node effect norms grow 4.094 and 4.023 times, but composition residual norms grow only 1.413 and 1.087 times. The preregistered numerical-floor prediction holds; the strong nonlinear-growth prediction fails. Relative composition errors become 11.52% and 10.57%. The largest node removal is 0.351% of its local full-state norm. Both diagnostic GPU jobs take about three seconds each.

A smooth second-order interaction would ordinarily scale quadratically near zero, whereas a fixed absolute numerical floor would not. These are diagnostic expectations, not universal bounds. The [directional audit](SELF_CONSUMER_STRENGTH_DIRECTION_V1_AUDIT.json) also finds residual-vector cosines only 0.217 and 0.504 across the two strengths; the discrepancy is not simply one fixed vector. Predicting the larger residual by 16 times the smaller is much worse than predicting it unchanged. Together the checks support a substantial numerical contribution, without isolating all FP32 body error or excluding genuine nonlinear effects. The original unit-strength failure remains a failure; no fresh/OOD or universal composition claim follows.
