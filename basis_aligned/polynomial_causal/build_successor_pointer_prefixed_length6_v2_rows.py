#!/usr/bin/env python3
"""Freeze outcome-blind second outcome-blind prefixed length-six authority for successor interaction transfer."""
...
def main():
        ...
        for family, elements in FAMILIES.items():
            for final_index in FINAL_INDICES[family]:
                ...
                for condition, ordered in orders.items():
                    ...
                    query = len(ids) - 1
                    row = {
                        "row_id": canonical(["prefixed_length6_v2", prefix_id, family, final_index, condition]),
                        "prefix_id": prefix_id,
                        "prefix": prefix,
                        "family": family,
    ...
    payload = {
        "schema": "successor_pointer_prefixed_length6_v2_rows",
        "selection_rule": "four second-authority label prefixes crossed with every previously defined length-six month/digit endpoint and the same two transpositions; no model outcomes",
