...
## Converged continuation and bounded escape audit (08:50)

The [LBFGS continuation](GRADED_SOURCE_LBFGS_V1_RESULT.json) completes in157.27seconds. Both arms reach localstationarity (gradients8.47e-7/8.92e-7), with losses0.683954688807 and0.683954688689. A passes; original B/C remain failed. Thus the original time limit no longer explains the continuation's joint native-fidelity miss. Branch3 adverb/progressive swap errors remain about15.4/20.5%; branch8 progressive about16.9%. Signs alone do not repair these magnitude failures.

[CPU replica audit](GRADED_SOURCE_LBFGS_V1_REPLICA_AUDIT.json): mean principal overlap0.99999894, minimum cosine0.99995947, and every branch/family interarm write error<=0.000231. This establishes highly reproducible conditional approximations, not agreement with the original computation. [Native plane audit V2](GRADED_NATIVE_PLANE_AUDIT_V2_RESULT.json) performs400objective evaluations across16registered planes in21.32seconds. Interpolation errors<=4.45e-16; best improvements1.73e-12/2.39e-13. A passes,B/Cfail. No meaningful escape was found in that panel, but the rank-bound gap and unsearched planes leave global optimality unproved. V1's prepared original-frame audit was never executed; V2 uses the converged continuation with unchanged selection rules/bars.

The next object-level audit is the [fully symmetric eighth-degree contraction](COMPOSED_EIGHTH_CONTRACTION_V2_MATH.md). It removes the independent-producer approximation for the homogeneous purepath. This is an executed investigation of a remaining metric assumption, not another optimization restart or an absent-structure conclusion.
