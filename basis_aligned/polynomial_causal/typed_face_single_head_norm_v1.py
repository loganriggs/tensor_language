"""Candidate: only head8.2 generates the frozen MLP8 normalization background."""
import torch
import torch.nn.functional as F
import head8
from attention8_single_head_channels_v1 import channels

def execute(p,residual7,donor_city7,token_ids,recipient_token,donor_token,city,destination,strength=.5,mode="freeze_denominator"):
    ...
    a=F.linear(values.reshape(*current8.shape[:2],128),p['context']['output'][:,256:384])
    g=mixed+a
    ...
    a2=F.linear(values[:,:,0],context['output'][:,sl])
