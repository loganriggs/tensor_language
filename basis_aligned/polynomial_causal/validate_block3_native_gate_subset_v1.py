...
AUTHORITY = HERE / "block3_native_gate_subset_v1_validation_v0_authority.json"
RESULTS = HERE / "block3_native_gate_subset_v1_validation_v0_results.json"
RECEIPT = HERE / "block3_native_gate_subset_v1_validation_v0_receipt.json"
FAILURE = HERE / "block3_native_gate_subset_v1_validation_v0_failure.json"
LOCK = Path("/workspace/runs/.block3_native_gate_subset_v1_validation_v0.lock")
SOURCE_PATHS = (
...
def verify_inputs(source: Mapping[str, Any], fit: Mapping[str, Any], rows: Mapping[str, Any], checkpoint: facade.CheckpointReceipt) -> None:
...
def arm_writes(
    ...
    if float((native_replay - prefix.native_write).abs().max()) > 3e-4:
        raise RuntimeError("native validation polarization did not replay MLP3")
    ...
    replay = float((activation - typed_replay).abs().max())
    if replay / max(float(activation.abs().max()), torch.finfo(torch.float32).tiny) > 2e-5:
        raise RuntimeError("validation program direct/typed replay failed")
    ...
    return writes, native_terms, candidate_terms, replay
...
def _authority(
    ...
    core = {
        "schema": "block3_native_gate_subset_v1_validation_v0_authority",
...
def run() -> dict[str, Any]:
    ...
    try:
        ...
        authority = _authority(source, fit, rows_binding, checkpoint)
        claim.verify()
        ...
        verify_inputs(source, fit, rows_binding, checkpoint)
        ...
        result = {
            "schema": "block3_native_gate_subset_v1_validation_v0_results",
            "authority_sha256": authority["authority_sha256"],
            "status": "validation_v0_complete_no_final_rows_opened",
            ...
            "polarization_replay_max_abs": replay,
        ...
        receipt = {
            "schema": "block3_native_gate_subset_v1_validation_v0_receipt",
            "status": "validation_v0_complete_receipt_last",
            "authority_sha256": authority["authority_sha256"],
        ...
        if not FAILURE.exists() and not RECEIPT.exists():
            try:
                ...
                collector.create_json(FAILURE, {
                    "schema": "block3_native_gate_subset_v1_validation_v0_failure",
