...
- E1.1 is complete and failed decisively, as recorded below.  This is useful pruning:
  ...
  compressed program.
- E1.3 is currently fitting the same rank-512 map on the streams generated by the
  compressed prefix itself.  It directly tests whether E1.1 failed because the old map
  was trained on native inputs but deployed on compressed inputs.
- No second GPU experiment will overlap E1.3 or Family F.  CPU analyses and harnesses
  for E1.2 and E2–E4 remain eligible while those fits run.
