...
@torch.no_grad()
def main():
    out=P/'REGIONAL_TOKEN_SUFFIX_V2_RESULT.json';assert not out.exists();torch.set_num_threads(2);torch.manual_seed(61209)
    ...
    edits=[('zero',torch.zeros_like(donor)),('donor',donor),('negative',-donor),('half',.5*donor),('one_half',1.5*donor),('synthetic',torch.randn_like(donor)*.1)]
    ...
    result=dict(passed=all(max(c['relative_state_error'],c['relative_full_logit_error'],c['relative_logit_effect_error'])<=1e-10 for c in cells),cells=cells,vocabulary_rows=len(U),folded_LR_floats_per_context=program['left'][0].numel()+program['right'][0].numel(),scope='Two preselected contexts; exact FP64 conditional source-port algebra withnativeepsilon, notbitwisefloat32modelproof or newbehavioral/OODtest. K generation,Down,U andnativebackgroundremaincharged.')
