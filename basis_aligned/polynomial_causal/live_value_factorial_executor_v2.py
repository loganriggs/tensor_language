"""Native value executor with explicit equal-length cube geometry; v1 preserved."""
import torch
...
def run_world(backend, world, projector, reader_ids, *, identity=False):
    rows=world['rows'];length=world['length'];start_of_interaction=world['interaction_start']
    assert len(rows)==32 and 0<=start_of_interaction<length and all(len(r['ids'])==length and r['semantic_position']==length-1 for r in rows)
    ...
    def batch(start):
        ...
        return P.ModelBatch(tuple(r['row_id'] for r in part),'base',tuple(tuple(r['ids']) for r in part),
                            (reader_ids[0],)*16,(reader_ids[foil],)*16,(length-1,)*16)
    def run(b):
    ...
    try:
        with torch.inference_mode():
            ...
            early=float(mixed(u)[:,:start_of_interaction].norm()/u.norm().clamp_min(1e-30))
            replacement=(v-delta).float().view(32,length,9,128)
            edited=[];identity_ok=True;qk=True;first_ok=True
            for bi,start in enumerate((0,16)):
                b=batch(start);base=baselines[bi]
                if identity:
                    with C.replace_value(attn,values[bi].view(16,length,9,128),(length,)*16,heads=tuple(range(9))):same=run(b)
                ...
                with C.replace_value(attn,replacement[start:start+16],(length,)*16,heads=tuple(range(9))):changed=run(b)
