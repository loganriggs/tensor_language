...
def main():
    ...
    out=p/'SRO_FOUR_TERM_CONFIRMATION_V1_ROWS.json'
    ...
    for family in range(3):
        for city_pair,cities in enumerate([('Oxford','Denver'),('Manchester','Houston')]):
            for concept,(uk,us,stem) in enumerate(PAIRS):
                ...
                for side,(cue,city) in enumerate(zip(['British','American'],cities)):
                    other=cities[1-side]
                    if family==0:
                        text=f'The manuscript was mailed from {city}, where its author had lived since birth. Keep the author\'s spelling in this quotation: "{stem}'
                    elif family==1:
                        text=f'A publisher in {other} received a letter from a lifelong resident of {city}. Quote the letter without changing its spelling: "{stem}'
                    else:
                        text=f'A {city} museum is recording memories from local residents. One resident wrote: "{stem}'
