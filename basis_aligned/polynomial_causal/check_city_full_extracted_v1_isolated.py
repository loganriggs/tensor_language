...
P=Path(__file__).resolve().parent;STEM='CITY_FULL_EXTRACTED_V1'
def main():
 with tempfile.TemporaryDirectory(prefix='coupled-isolated-') as tmp:
  root=Path(tmp);shutil.copytree(P/'extracted_circuits/city_full_removal_v1',root/'package',ignore=shutil.ignore_patterns('__pycache__'))
  shutil.copyfile(P/'CITY_FULL_FRESH_V1_ARTIFACT.pt',root/'fixtures.pt')
...
x=dict(fixtures[0]['candidate_inputs']);zero=float(execute.execute(p,**x,strength=0).abs().max());x['token_ids']=x['token_ids'].clone();x['token_ids'][0,0]=-1
