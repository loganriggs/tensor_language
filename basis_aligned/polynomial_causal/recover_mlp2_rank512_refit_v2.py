...
def v1_lineage_snapshot() -> dict[str, Any]:
    ...
    ) or failure.get("authority_sha256") != file_sha256(V1_AUTHORITY):
    ...
    return {
        "v1_authority_sha256": file_sha256(V1_AUTHORITY),
...
def stable_v1_sources() -> tuple[str, dict[str, str]]:
    ...
    authority = json.loads(V1_AUTHORITY.read_bytes())
    return authority["source_commit"], sources
    ...
    try:
        ...
        def receipt_guard() -> None:
            ...
            if V2_RECEIPT.exists() or V2_FAILURE.exists():
                raise RuntimeError("recovery terminal raced receipt publication")
            for path, expected in ((V2_AUTHORITY, receipt["authority_sha256"]),
                                   (V2_BUNDLE, bundle_hash), (V2_LEDGER, ledger_hash),
                                   (V2_RESULT, result_hash)):
        ...
        if not V2_RECEIPT.exists() and not V2_FAILURE.exists():
            assay.atomic_json(V2_FAILURE, failure,
                pre_link_check=lambda: assay.row_life.require_claim(claim, V2_LOCK))
        raise
    finally:
