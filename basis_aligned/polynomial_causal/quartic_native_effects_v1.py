...
@torch.no_grad()
def main():
 parser=argparse.ArgumentParser();parser.add_argument("--stem",required=True);parser.add_argument("--initial",required=True);parser.add_argument("--output",required=True);args=parser.parse_args()
 ...
 result=json.loads((P/(args.stem+'_RESULT.json')).read_text());ap=P/(args.stem+'_PROGRAM.pt');assert digest(ap)==result['artifact_sha256']
 initial=torch.load(P/(args.initial+'_PROGRAM.pt'),weights_only=True);pilot=torch.load(ap,weights_only=True)
