"""Same injected states, explicit EPS32 FP64 suffix precision discriminator."""
...
def main():
    ...
    for index in (0,24,48,72,96,112,128,144):
        ...
        outcomes64=[]
        original_rms=F.rms_norm
        def rms32eps(input,normalized_shape,weight=None,eps=None):
            return original_rms(input,normalized_shape,weight,torch.finfo(torch.float32).eps if eps is None else eps)
        F.rms_norm=rms32eps
        try:
            for initial in (bar,bar+expected,bar+predicted,generated_bar,generated_bar+predicted):
                state=initial.float().double()
                for block in model.transformer.h[11:]:state,_=block(state,v1.double(),x0.double())
                logits=(30*torch.tanh(model.lm_head(F.rms_norm(state[:,-1],(1152,)))/30))[0]
                if index<96:
                    target=logits[rows[index]['uk_id']]-logits[rows[index]['us_id']]
                    control=logits[rows[index]['control_ids'][0]]-logits[rows[index]['control_ids'][1]]
                else:target=-logits.log_softmax(-1)[198];control=logits[198]-logits[11]
                outcomes64.append([float(target),float(control)])
        finally:F.rms_norm=original_rms
        scores64=torch.tensor(outcomes64,dtype=torch.float64)
        effects64=scores64[1]-scores64[0];transported64=scores64[4]-scores64[3]
        ...
        cells.append(dict(native_effects64=effects64.tolist(),transported_effects64=transported64.tolist(),transported_effect_errors64=(transported64-effects64).tolist(),background_drift64=(scores64[3]-scores64[0]).tolist(),total_output_errors64=(scores64[4]-scores64[1]).tolist(),background_state_error=rel(generated_bar,bar),background_drift=background_drift.tolist(),transported_effects=transported.tolist(),transported_effect_errors=(transported-effects[0]).tolist(),total_output_errors=total_error.tolist(),native_effects=effects[0].tolist(),predicted_effects=effects[1].tolist(),absolute_effect_errors=(effects[1]-effects[0]).abs().tolist(),row=index,tokens=len(rows[index]['ids']),child_error=rel(generated['child'],c),
    ...
    result=dict(cells=cells,seconds=time.perf_counter()-tic,
                scope='Fixed eight prefixes, one per existing group; native CPU prefix and head9 edits through attention10. '
                'Previously frozen GPU-produced scalar fields supplied identically to direct and predicted branches. '
                'Same five FP32-quantized initial states through FP32 and FP64 suffix; EPS32 enforced for every RMS. Input-generation and injection rounding remain. No fitting,freshOOD or CPU/GPU equivalence claim. '
                'Small real-state check; does not replace pending160-prefix GPU experiment.')
    (P/'COMPOSED_MLP10_BACKGROUND_PRECISION_V1_RESULT.json').write_text(json.dumps(result,indent=2)+'\n')
