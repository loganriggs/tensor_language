...
## Matched full-head regional control, 13 September 08:55

That comparison is now executed: the exact same full-native-head object and six paired-output probes on96historical regional prefixes, grouped into four families of24. [Receipt](SPARSE_INTERACTION_FULLHEAD_REGIONAL_V1_RESULT.json). It takes2.15seconds/96bodyforwards; coefficient/native-factor replay is $2.33\times10^{-15}$ and sparse/dense execution error $2.21\times10^{-7}$. The raw mixed-numerator error is3.44%.

Own-effect errors across groups are14.62%,10.41%,7.95%,11.94%; three groups miss10%, and there are18material sign reversals. The [matched-scope audit](SPARSE_FULLHEAD_SCOPE_V1_AUDIT.json) gives aggregate full-head effect error10.24%on regional text versus20.51%on FineWeb. This descriptive comparison holds operator and output scope fixed; text/prefix families still differ, and it is not a statistical or universal distribution-shift theorem.

There was another possible confound: these tests probe all six pairs, whereas the earlier retained-interaction regional test scores the designated pair for each prefix. Restricting the new regional result to that designated pair leaves13.69%,11.53%,6.22%,14.37%error. Thus expanding the output probes is not the sole explanation. The first24prefixes overlap the older retained-port group0, allowing a same-prefix/same-output comparison there; the later groups should not be presented as identical-prefix comparisons. The full-head and retained-port backgrounds remain different by definition.

The resulting interpretation is narrower and more useful than simply saying compression failed on other text: the operator is more faithful on the circuit's retained interaction ports than on the broader native head contribution, and the full-head test degrades further on this FineWeb panel. Preserve the interaction-specific result, but do not promote it to a reusable full-head replacement. Further compression should account for the actual producer-constrained inputs and validate any claimed extension of scope explicitly.
