...
def main():
    ...
    frozen=torch.load(P/'CITY_RESIDUAL6_SINGLE_INPUT_V1_WRITES.pt',weights_only=True)['writes']
    fixtures=[]
    for f,r,w in zip(prefix,reference,frozen,strict=True):
        city=f['city'];c=r['inputs']
        fixtures.append({'inputs':{'residual6':f['residual6'],'token_ids':f['token_ids'],'city':city,
                                   'destination':c['destination']},
                         'expected':w})
    with tempfile.TemporaryDirectory(prefix='city-residual6-isolated-') as directory:
        temp=Path(directory)
        for name in ['execute.py','attention7.py','readers.py','attention7.pt','readers.pt','head8.pt']:shutil.copyfile(source/name,temp/name)
...
r={'passes':max(errors)<=1e-10 and max(outside)==0 and finite and unknown and len(fixtures)==40,
   ...
   'imports_outside_repository':True,'seconds':time.perf_counter()-start,'scope':'Isolated CPU declared-input replay; only residual6 native input; compares frozen approximate generator writes, not exact native writes.'}
print(json.dumps(r))
''')
    ...
    (P/'CITY_RESIDUAL6_SINGLE_INPUT_V1_ISOLATED_RESULT.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
