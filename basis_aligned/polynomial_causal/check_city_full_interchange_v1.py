...
def main():
 ...
 if os.environ.get('BQLIB_NO_MODEL') or os.environ.get('BQLIB_DRYRUN'):
  print('800 sequence-equivalents;360 block calls;CPU fresh coupled interchange and16nulls');return
 ...
 from city_residual6_fields_v1 import execute,city_write
 from city_residual6_single_input_v1 import execute as removal
  ...
  for arm in range(19):
   ...
   for i,row in enumerate(groups):
    ...
    if arm<=2:d=writes[i][arm-1].float()
     ...
     gen=torch.Generator().manual_seed(18097000+1000*(arm-3)+row['context_id'])
 ...
 r={'pred_a':row_audit['passes'] and len(rows)==240 and all(preflight[k] for k in ['query_gate','self_removal_gate','native_swap_gate']) and outside==0 and max(norm_errors)<=1e-5 and bool(torch.isfinite(v).all()) and count==800 and calls==360 and not torch.cuda.is_initialized(),
    ...
    'scope':'Fresh20document coupled city K1/K2/fullV interchange with recipient queries. Two native residual6 contexts; approximate mixed8 RMS; native MLP8/suffix external. Fixed spelling probes. No independent composition or simplicity claim.','source_shas':binding}
