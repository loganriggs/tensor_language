#!/usr/bin/env python3
# BQGATE:280 sequence-equivalent forwards;126 batched block calls;180seconds;CPU only;no fitting.
"""pred_a replay/instruments; pred_b keymain explains four reverse documents;
pred_c valuemain positive>=.90,mean>=.02,controls<=.5target.
"""
...
STEM='CITY_SOURCE7_SLOT_FACTORIAL_V1'
...
def main():
 ...
 if os.environ.get('BQLIB_NO_MODEL') or os.environ.get('BQLIB_DRYRUN'):
  print('280 sequence-equivalents;126 block calls;CPU7 slot-factorial arms');return
 assert os.environ.get('CUDA_VISIBLE_DEVICES')=='' and not torch.cuda.is_initialized()
 cert=json.loads((P/'CITY_SOURCE7_SLOT_FACTORIAL_V1_CPU_RESULT.json').read_text())
 ...
 candidates=torch.load(P/'CITY_SOURCE7_SLOT_FACTORIAL_V1_WRITES.pt',weights_only=True)['writes']
 writes=candidates
 ...
 values=torch.zeros(7,40,10,dtype=torch.float64);count=0;calls=0;arm_seconds=[]
 try:
  for arm in range(7):
   ...
   print(f'arm {arm+1}/7: {arm_seconds[-1]:.3f}s; total {time.perf_counter()-start:.1f}s',flush=True)
 ...
 joint=v[6]-v[1];joint_abs=float(joint.abs().max());joint_rel=float(joint.norm()/v[1].norm())
 base=v[0,:,0][::2]-v[0,:,0][1::2];cap=base>=.1;rms=e.square().mean(1).sqrt();records={}
 for arm,name in [(1,'present'),(2,'keymain'),(3,'valuemain'),(4,'cross'),(5,'mains'),(6,'joint')]:
 ...
 for context in sorted({r['context_id'] for r in rows}):
  ...
  attenuation={}
  for arm,name in [(1,'present'),(2,'keymain'),(3,'valuemain'),(4,'cross')]:
   pair=v[arm,idx,0][::2]-v[arm,idx,0][1::2];attenuation[name]=((b[capable]-pair[capable])/b[capable]).tolist()
  docs.append({'context_id':context,'keymain_over_present_effect':float(e[2,idx,0].norm()/e[1,idx,0].norm()),'attenuations':attenuation})
 ...
 for label,ids_set in [('all',set(range(20))),('reversed',{4,8,11,19}),('other',set(range(20))-{4,8,11,19})]:
  ...
  interactions[label]={'suffix_main_interaction_over_smaller':float((e[5,idx,0]-e[2,idx,0]-e[3,idx,0]).norm()/min(e[2,idx,0].norm(),e[3,idx,0].norm())), 'cross_retention_change_over_present':float((e[1,idx,0]-e[5,idx,0]).norm()/e[1,idx,0].norm()),'cross_alone_over_present':float(e[4,idx,0].norm()/e[1,idx,0].norm())}
 r={'pred_a':absolute<=1e-4 and relative<=1e-5 and joint_abs<=1e-4 and joint_rel<=1e-5 and outside==0 and bool(torch.isfinite(v).all()) and count==280 and calls==126 and not torch.cuda.is_initialized(),
    'pred_b':len(reversed_docs)==4 and all(d['keymain_over_present_effect']>.5 and len(d['attenuations']['keymain'])==6 and all(a<0 for a in d['attenuations']['keymain']) for d in reversed_docs),
    'pred_c':int(cap.sum())==120 and records['valuemain']['positive_fraction']>=.90 and records['valuemain']['mean_attenuation']>=.02 and max(records['valuemain']['control_over_target'])<=.5,
    ...
    'scope':'Opened key/value role factorial in city-side MLP7 group; full native queries/denominators and suffix. No fresh/null/selectivity or independent composition promotion.','source_shas':binding}
