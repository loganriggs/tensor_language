...
| numerical precision / quantized bytes | smaller storage of the same numerical program | frozen dtype assignment; literal bytes; worst-position and OOD error | any interpretability consequence—the axes, graph, interfaces, and semantics are unchanged |
| prequential MDL bits / fit sample efficiency | fewer data-dependent bits and better statistical efficiency | frozen coding rule; held-out prequential codelength and fit-size/doubling curves | editability or compositionality |
