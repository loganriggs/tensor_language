...
from writer_span_coordinates_v1 import prepare as span_prepare,transform
...
def main():
 ...
 for cosine in [.9,.999999,1.]:
  ...
  coordinates=span_prepare(w);wo=coordinates['basis'];wo=torch.cat([wo,torch.zeros(2-wo.shape[0],1152,dtype=wo.dtype)]);bo=wo@z/1152;go=wo@wo.T/1152
  po=torch.stack([K(wi,z)-2*bi*m0 for wi,bi in zip(wo,bo)])
  qo=torch.stack([K(wo[i],wo[j])-2*go[i,j]*m0 for i,j in [(0,0),(0,1),(1,1)]])
  vo=torch.cat([wo,po,qo]);ob=prepare(vo.float(),ll.float(),rr.float(),dd.float(),rho0.reshape(1).float(),bo.float(),go.float())
  ...
  for av,bv in [(1.,1.),(1.,-1.),(.3,-.7),(2.,-2.)]:
   ...
   amplitudes=transform(torch.stack([a,b]),coordinates);amplitudes=torch.cat([amplitudes,torch.zeros(2-amplitudes.shape[0],dtype=amplitudes.dtype)]);oa,obeta=amplitudes
   op=scalar_functions(oa.float(),obeta.float(),den.float())@ob
   oe=float((op.double()-ref).norm())
   norm=float(ref.norm());err=float((predicted.double()-ref).norm());derr=float((direct.double()-ref).norm())
   rows.append(dict(orthogonal_relative_error=oe/norm if norm else None,orthogonal_scaled_absolute_error=oe/scale,cosine=cosine,a=av,b=bv,reference_norm=norm,constituent_scale=scale,material=norm>=1e-6*scale,bank_relative_error=err/norm if norm else None,direct_relative_error=derr/norm if norm else None,bank_scaled_absolute_error=err/scale,direct_scaled_absolute_error=derr/scale))
 result={'pred_a':all(x['orthogonal_relative_error']<=1e-3 for x in rows if x['material']),'rows':rows,'scope':'Actual MLP9/10 weights, synthetic common-input writer directions/context. FP32 bank and response contractions against FP64; not native BF16 or behavioral validation.'}
 (P/'TWO_WRITER_SVD_PRECISION_V1_CONTROL.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2))
