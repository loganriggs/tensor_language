"""Exact weight-derived affine-bias carrier correction; no fitted background.

Board preregistration10:41UTC: replay<=1e-6, price<=1%extra,
...
@torch.no_grad()
def main():
    out=P/'BIAS_CARRIER_CORRECTION_V1_RESULT.json'
    artifact=P/'BIAS_CARRIER_CORRECTION_V1_PROGRAM.pt'
    ...
    for layer in range(17):
        b=model.sd[f'transformer.h.{layer}.lambdas'][0].double()*b+model.sd[f'transformer.h.{layer}.mlp.Down_bias'].double()
    ...
    for layer in range(17):
        ...
        expanded+=scale*model.sd[f'transformer.h.{layer}.mlp.Down_bias'].double()
    ...
    result=dict(utc=datetime.now(timezone.utc).isoformat(),bias_rms=float(b.square().mean().sqrt()),
                ...
                scope='Weight-derived exact affine-bias carrier, no fit to native data. Sparse body plus small dense correction; native conditional raw scores only, no adoption.')
