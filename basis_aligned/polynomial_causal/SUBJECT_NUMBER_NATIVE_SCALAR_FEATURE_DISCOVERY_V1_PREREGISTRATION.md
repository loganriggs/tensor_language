# Subject-number native scalar feature discovery V1

Use only the already-open original direction-cardinality authority. For each recipient row and each of the 16 E/A/U/W background subsets, compute the base L11H3 projected write before the MLP6/7 number edit. Project that native write onto the first $k\in\{1,2,4,8\}$ left singular vectors of L11H3's native output-projection slice. Add one intercept and fit the held four-scalar program amplitude by ordinary least squares.
