...
| Full-input nested quadratic squares for the selected two-output quartic | [Terminal comparison](COUPLED_QUARTIC_WRITER_V1_MATH.md#terminal-comparison-12-september-0154): long LBFGS V2 gains8.96% over V1 but remains unconverged; native write29.32%, effects fail | No immediate identical continuation; fit readers for mixed interactions next |
| Absolute coefficient coverage | [Independent probe check](QUARTIC_TARGET_COVERAGE_V1_RESULT.json): V2 captures about10.8% of full-input two-output target | Relative gains are not total target or whole-model coverage |
| Mixed products of learned quadratics | [Full528 result](QUADRATIC_PRODUCT_CORE_LEARNED_V1_RESULT.json): +55.76% capture, write30.62%; greedy32edges +3.49%,write33.69%; effects fail | Joint240-step fit completed:+17.69%capture over mixed start,write26.75%; stationarity/effects fail |
| Independent starts and node restarts | [Native replacement](QUARTIC_NODE_REPLACEMENT_V1_RESULT.json): +0.277% capture,write26.50%, substantive bars fail | Existing bank does not exclude residual-derived initialization; stationary planted miss remains a global-recovery warning |
| Learned quadratic-core re-encoding | [Pencil check](LEARNED_QUADRATIC_PENCIL_V1_RESULT.json): exact22scalar+5pairblocks, rank16 reader truncation worsens write50.79% | Exact shared-parent DAG avoids truncation but saves only0.076% of counted component multiplications; not semantic reuse |
| Residual-derived quadratic initialization | [Native candidate](QUARTIC_RESIDUAL_EIGEN_CANDIDATE_V1_RESULT.json): eigensolves converge, +0.348%capture,write26.12%; effects fail | Matched180-update nonlinear refit queued to test initialization outcome |
...
**Current continuation (12 September02:39):** mixed graph and residual eigen-candidate runs are complete; both fail native-effect fidelity. The residual candidate is now the start of `run_quartic_residual_eigen_refit_v1.py`,180updates/900fit seconds, compared with the old continuation's180-update coefficient prefix. Read [the residual method and receipts](QUARTIC_RESIDUAL_EIGENMATRIX_V1_MATH.md). No new circuit promotion. Hourly02:27 ceremony repair remains: execute and interpret existing work, avoid more new toy/scaffolding.
...
[Joint routing/value polynomial](JOINT_ROUTING_VALUE_POLYNOMIAL_V1_MATH.md),12September09:47: exact q-degree2/source-degree3 coefficient contraction including QK1*QK2*value, plus diagonal full-U/MLP17 mixed and AA replay. CPU/native checks pass; planted tied-source cancellation distinguishes the object from older routing-overlap and OV-only studies. No trained factorization or promoted circuit.

[Shared cubic source variable projection](SHARED_CUBIC_SOURCE_PROJECTION_V1_MATH.md),12September10:10: new exact source/query factorization of the joint QK1*QK2*value*OV numerator. Private quadratic query-output functions are eliminated exactly; source CP3 factors are optimized on spheres. Dense controls and four cold planted starts pass. Native two-start rank16pilot is managed-live; no final outcome yet. Distinct from whole-function CCA and old common linear MLP15 interface.
