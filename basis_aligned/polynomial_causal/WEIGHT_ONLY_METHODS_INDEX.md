...
| Mixed products of learned quadratics | [Full528 result](QUADRATIC_PRODUCT_CORE_LEARNED_V1_RESULT.json): +55.76% capture, write30.62%; greedy32edges +3.49%,write33.69%; effects fail | Native price passed:3.188s gradient,10.69GiB;240-step/1200-second joint fit queued |
...
| Residual-derived quadratic initialization | [Matrix-free operator and eigensolver](QUARTIC_RESIDUAL_EIGENMATRIX_V1_MATH.md): planted recovery and native CPU operator controls pass; packed eigensolver checked | GPU operator/transfer pricing queued behind mixed fit; no native eigenfactor yet |
...
**Current continuation (12 September02:14):** [Native price](QUARTIC_SELECTED_EDGES_PRICE_V1_RESULT.json) passes all checks. `run_quartic_mixed_joint_v1.py` is queued through lane1 with240steps/1200fit seconds. It optimizes readers for the frozen32-edge mixed graph. Planted independent-start misses survive support changes and have numerically positive local curvature: local convergence is not recovery. Read [the current fit specification](COUPLED_QUARTIC_WRITER_V1_MATH.md#native-mixed-graph-fit-12-september-0214). The four circuit properties remain unresolved.
