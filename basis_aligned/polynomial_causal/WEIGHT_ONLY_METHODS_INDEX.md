...
| Mixed products of learned quadratics | [Full528 result](QUADRATIC_PRODUCT_CORE_LEARNED_V1_RESULT.json): +55.76% capture, write30.62%; greedy32edges +3.49%,write33.69%; effects fail | Joint selected-edge gradient implemented and CPU checked; native price submitted |
...
**Current continuation (12 September02:00):** all three prior jobs and their native-effect scoring are complete. The [mathematical review](THREE_HOURLY_MATHEMATICAL_REVIEW_2026-09-12_0200.md) selects joint optimization of readers under the learned mixed graph. Its CPU gradient control passes; `run_quartic_selected_edges_price_v1.py` is submitted through lane1. Check its receipt before choosing a fitting budget. The four circuit properties remain unresolved.
