...
### Follow-up: shared/private output branches

A weight-defined output split now greatly reduces neighboring inflection-choice effects, while still failing full CE preservation and losing more verb effect than predicted. Exact probability accounting separates the remaining lexical-pair mass change from the small binary grammatical-choice change. This is developmental validation, not a repaired verdict or new OOD confirmation. [Derivation, costs and results](NATIVE_RELATION_OUTPUT_SPLIT_V1_MATH.md).
