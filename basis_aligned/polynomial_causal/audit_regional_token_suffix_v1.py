...
@torch.no_grad()
def main():
    out=P/'REGIONAL_TOKEN_SUFFIX_V1_RESULT.json';assert not out.exists();torch.set_num_threads(2);torch.manual_seed(61209)
    ...
    for name,delta in edits:
        ...
        cells.append(dict(edit=name,relative_state_error=state_error,relative_full_logit_error=logit_error,relative_logit_effect_error=effect_error));assert max(state_error,logit_error,effect_error)<=1e-10
    result=dict(passed=True,cells=cells,vocabulary_rows=len(U),folded_LR_floats_per_context=program['left'][0].numel()+program['right'][0].numel(),scope='Two preselected contexts; exact FP64 conditional source-port algebra withnativeepsilon, notbitwisefloat32modelproof or newbehavioral/OODtest. K generation,Down,U andnativebackgroundremaincharged.')
