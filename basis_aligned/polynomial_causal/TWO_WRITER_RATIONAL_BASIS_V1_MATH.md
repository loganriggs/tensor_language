...
## Original-path check: transport cannot be skipped

The original head8.2 edit passes through MLP8 before reaching head9. [A cached native transport check](TRANSPORTED_WRITER_SPAN_V1_CONTROL.json) projects that actual raw9 change onto the weight-defined span of the original head8.2/head9.8 writers. Across three cue families in the existing 72-prompt panel, this shortcut has 45.2–53.9% state-change error. Recomputing RMS and checking head9 readers reduces errors to 21.0–30.4% for QK reads and 8.5–13.0% for the value read, still missing the declared 10% joint bar. No basis was fitted to these examples.
