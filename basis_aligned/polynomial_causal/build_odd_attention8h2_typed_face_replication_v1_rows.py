...
OUT_ROWS = HERE / "ODD_ATTENTION8H2_TYPED_FACE_REPLICATION_V1_ROWS.json"
OUT_CONTROL = HERE / "ODD_ATTENTION8H2_TYPED_FACE_REPLICATION_V1_CPU_CONTROL.json"
CITY_PAIRS = [
...
def main() -> None:
    ...
    for source_row, tensor_row in enumerate(source):
        ...
        for position, token in enumerate(tensor_row.tolist()):
            if token not in token_pair or position < LEFT or position + RIGHT > len(tensor_row):
                continue
            key = hashlib.sha256(
                f"odd8h2-typed-face-replication-v1|{source_row}|{position}|{token}".encode()
            ).hexdigest()
        ...
        for endpoint, (uk_id, us_id) in enumerate(endpoints):
            for cue in ("British", "American"):
                ...
                record = {
                    ...
                    "panel": "typed_face_replication_ood",
    ...
    output = {
        "schema": "odd_attention8h2.typed_face_replication.rows.v1",
    ...
    control = {
        ...
        "scope": (
            "Outcome-blind second natural FineWeb panel source-disjoint from V1 within skip39000; "
