...
def main():
 ...
 out=P/'QUARTIC_PILOT_NATIVE_EFFECTS_V1.json';assert not out.exists()
 bindings=json.loads((P/'COUPLED_QUARTIC_NONLINEAR_V1_BINDING.json').read_text())['files'];assert all(digest(k)==v for k,v in bindings.items())
 result=json.loads((P/'COUPLED_QUARTIC_NONLINEAR_V1_RESULT.json').read_text());ap=P/'COUPLED_QUARTIC_NONLINEAR_V1_PROGRAM.pt';assert digest(ap)==result['artifact_sha256']
 initial=torch.load(P/'COUPLED_QUARTIC_WRITER_V1_PROGRAMS.pt',weights_only=True)['programs'][0];pilot=torch.load(ap,weights_only=True)
 ...
 reports=[]
 for idx,name in [(1,'initial'),(2,'pilot')]:
