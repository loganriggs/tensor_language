"""Ranks frozen from weights; validate paired response on existing72pristineprompts, five signed amplitudes with analytic baseline, not native strength interventions."""
from pathlib import Path
import json,torch
import torch.nn.functional as F
from head17_source_interface_v1 import CHECKPOINT
from shared_key_consumer_complete_v1 import compile_program,scalar
...
def main():
 ...
 original_rows=json.loads((P/'SCALAR_CUE_CHANNEL_SHIFT_V1_ROWS.json').read_text())['rows'];rows=[dict(r,source_index=i,strength=strength) for strength in [-2.,-1.,.5,1.,2.] for i,r in enumerate(original_rows)]
 ...
 for row in rows:
  i=row['source_index']
  n=len(row['ids']);ids=torch.tensor([row['ids']]);z=cache['z8'][0,i,:n][None].double();h=cache['raw9'][0,i,:n][None].double();a=cache['amplitude8'][0,i,:n][None]*row['strength']
 ...
 for rank,(p,weight_error) in candidates.items():
  ...
  for family,strength in sorted(set((r['family'],r['strength']) for r in rows)):
   mask=torch.tensor([r['family']==family and r['strength']==strength for r in rows]);t=target[mask];e=pred[mask]-t
   cells.append(dict(family=family,strength=strength,response_error=float(e.norm()/t.norm()),reference_norm=float(t.norm())))
 ...
 out={'records':records,'orthogonality_error':ortherror,'baseline_program_scalars':size(full),'scope':'Frozen best converged complete even-key coefficient-error fit, not data fit. Paired scalar effects versus same rank64-transport program, existing72pristineprompts, five signed amplitudes with analytic baseline, not native strength interventions. No fresh/native-logit/semantic-selectivity claim.'}
 (P/'SHARED_KEY_COMPLETE_STRENGTH_V1_CONTROL.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
