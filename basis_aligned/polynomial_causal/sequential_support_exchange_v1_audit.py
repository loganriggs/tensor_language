...
def main():
    ...
    out=P/'SEQUENTIAL_SUPPORT_EXCHANGE_V1_AUDIT.json'; assert not out.exists()
    ...
    for prior in previous['rows']:
        ...
        rows.append(dict(side=side,product=j,gain=gain,cardinality=len(set(new_ids)),**report))
    ...
    predicted=sum(row['gain'] for row in rows)
    independent=sum(row['independent_cp_total_gain'] for row in previous['rows'])
    seconds=time.perf_counter()-started
    cache=Path('/dev/shm/bilin18_sequential_support_exchange_v1.pt'); assert not cache.exists()
    ...
    result=dict(predictions=dict(pred_a_instrument=abs(measured-predicted)<=1e-9
                    and all(row['gain']>=-1e-12 and row['cardinality']==128 for row in rows)
