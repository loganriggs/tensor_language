"""Frozen 256-context actual-producer ranking; see preregistration for bars."""
import hashlib
import json
import os
import signal
...
def main():
    ...
    out = P/'UNIFORM_PRODUCER_RANKING_V1_RESULT.json'
    ...
    replay = 0.
    for seed in range(170224000, 170224016):
        z, terms, denominator, diagnostic = model.sample(seed)
    ...
    for i, name in enumerate(NAMES):
        ...
        rows.append(dict(name=name, bytes=path.stat().st_size,
                         ...
                         paired_standard_error=float(difference.std()/16),
                         ...
                         contrast_paired_standard_error=float(cd.std()/16)))
    result = dict(utc=datetime.now(timezone.utc).isoformat(), contexts=256,
                  ...
                  pred_c=all(r['paired_difference']>3*r['paired_standard_error'] and
                             min(r['independent_half_differences'])>0 for r in rows[1:]),
