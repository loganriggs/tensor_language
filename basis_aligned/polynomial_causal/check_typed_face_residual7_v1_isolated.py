...
P=Path(__file__).resolve().parent;STEM='TYPED_FACE_RESIDUAL7_V1'
def main():
 with tempfile.TemporaryDirectory(prefix='coupled-isolated-') as tmp:
  root=Path(tmp);shutil.copytree(P/'extracted_circuits/typed_face_residual7_v1',root/'package',ignore=shutil.ignore_patterns('__pycache__'))
