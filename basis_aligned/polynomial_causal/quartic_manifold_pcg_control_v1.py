...
import torch,json
from quartic_manifold_pcg_v1 import fit,tangent,retract
...
r=dict(pred_a=orth<=1e-10 and sphere<=1e-10 and all(z['objective']<=h[i]['objective']+1e-12 for i,z in enumerate(h[1:])),pred_b=err<=1e-4,coefficient_error=err,orthogonality_error=orth,sphere_error=sphere,termination=reason,iterations=len(h),final_gradient=h[-1]['projected_gradient_norm'])
p=P/'QUARTIC_MANIFOLD_PCG_V1_CONTROL.json';assert not p.exists();p.write_text(json.dumps(r,indent=2)+'\n');print(r);assert r['pred_a'] and r['pred_b']
