...
@torch.no_grad()
def main():
    out=P/'REGIONAL_FIRST_MATCHED_WRITE_V1_RESULT.json';assert not out.exists();torch.set_num_threads(2);torch.manual_seed(61212)
    rows=json.loads((P/'REGIONAL_BEHAVIOR_CONTROLS_V1_ROWS.json').read_text())['rows'];a=torch.load(P/'REGIONAL_BEHAVIOR_CONTROLS_V1_ARTIFACT.pt',weights_only=True);delta=a['write_vertices'][:,1]-a['write_vertices'][:,0];pre=a['pre'];sd=torch.load(CK,mmap=True,weights_only=True,map_location='cpu')
