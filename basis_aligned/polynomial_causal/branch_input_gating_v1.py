"""No-fit paired input-gating test, registered before the larger screen.
...
@torch.no_grad()
def main(label):
    assert label in ('initial','separate')
    torch.set_num_threads(2);torch.set_default_dtype(torch.float64)
    root=Path(__file__).parent
    stem='SHARED_NODE_PARENT1_'+('FINEWEB_V1' if label=='initial' else 'SUPPRESSION_V1')
    output=root/f'BRANCH_INPUT_GATING_V1_{label.upper()}.json'
    ...
    result=dict(pred_a=valid,pred_b=valid and bool((standardized>=.25).all()),
        ...
        prospective_for_separate_panel=label=='separate',coefficient_replay=replay,
