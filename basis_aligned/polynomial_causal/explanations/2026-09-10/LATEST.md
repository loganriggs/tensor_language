...
**Current update, 22:08 — weight-first:** [Native ALS result and broader shared-input blocks](unsupervised_structure_campaign.md#weight-only-solver-benchmark-and-broader-blocks--2208-utc). ALS numerical checks passed; convergence and reference-quality predictions failed after120seconds. The new16-block, four-output-per-block weight fit is running. Signed squares are prepared, not yet submitted. FineWeb is the training-distribution validation corpus; Pile remains separately labelled OOD. Most structural hypotheses and stronger solvers remain untested.
