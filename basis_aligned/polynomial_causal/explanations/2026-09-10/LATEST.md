...
**Newest update, 23:32:** [Block result and overlapping token factors](unsupervised_structure_campaign.md#block-result-and-overlapping-token-factors--2328-utc). Varimax converged in62seconds: effective factors per token fell38.2→28.6, but top4 retention32.4%missed50%. The median token still needs53factors for90%of projected loading energy. Leading input functions remain complex. The block fit is still unconverged; equal-token varimax is queued. No identified circuits or new data fitting.
