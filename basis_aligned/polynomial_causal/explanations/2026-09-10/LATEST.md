...
**Latest results and explanation, 21:28:** [Unsupervised structure campaign](unsupervised_structure_campaign.md). The million-token panel is captured. Fixed input factors transferred to Pile: shared-reader validation error1.56%, improving to1.40% after training-only writer refit, versus affine2.37%. All three transfer-screen checks passed. The appended sections also explain the optimizer, convergence, costs and literature. Individual circuit identification and nonlinear fitting on the new panel remain pending.
