...
**Current direction, 21:53 — weight-first:** [Methods, assumptions and bottlenecks](unsupervised_structure_campaign.md#user-correction-weight-first-search-methods-and-bottlenecks--2153-utc). User correction: exhaust diverse weight-only factorizations before using data for discovery. FineWeb is in-distribution; Pile is shifted-corpus/OOD, with adapted fits distinguished. No further data-fitting jobs queued. Most of the25hypotheses remain untested. Exact implicit weight-only ALS update and preconditioned solve now pass CPU controls; native benchmarking and broader structured solvers are next.
