...
## First concrete experiment after auditing the old task results

The general 62-item behavior battery cannot be the only signal for this experiment. Its stored census says the
induction band is absent, and it contains no named copy or successor task. We therefore need dedicated task axes in
addition to broad collateral measurements.

The equality/copy executor already exposes four independent exact terms, one from each of L5H5, L7H3, L8H3, and
L8H4:

`e_h(q) = O_h sum_k 1[token[q] = token[k-1]] score1_h(q,k) score2_h(q,k) value_h(k)`.

All previous equality experiments changed these four terms together. The next experiment should evaluate all 16
subsets twice: subtract each subset from the native model, and add each subset to a background in which the four
complete heads are deleted. The complete subset table gives exact finite head-interaction terms, so it can
distinguish redundant alternatives from complementary or sequential contributions without using rank.

At the same time, capture every later attention and MLP write. A later module is only called a consumer if patching
its induced write change into the removal run predicts or repairs the task effect on held-out examples. If an MLP is
selected, expose its bilinear product contributions and group learned mixtures by task response and patch repair;
do not assume its native product channels are independent circuits.

This first experiment directly tests cross-head grouping, within-task splitting, redundancy, extraction, selective
removal, downstream MLP use, and held-out prediction. The old ordered-successor rank plan is deferred; successor is a
useful second task only after it is redesigned around order input, routing, output, and downstream use.
