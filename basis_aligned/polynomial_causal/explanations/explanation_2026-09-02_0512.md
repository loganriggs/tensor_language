...
## Rung 469: the factorization-independent test now running
...
The stronger check is predictive. Rung 469 freezes `Q` and one causal calibration scale on code discovery documents,
combines that frozen reader with states from held-out code and natural text, and predicts the actual effect of replacing
the full MLP contribution. It must beat simply copying the four code effects. A cosine alone is only a screen; the
prediction must also get magnitude and work in both natural waves.

The experiment is preregistered, implemented, tested, committed, and running through the managed GPU runner. It uses
1,440 forward passes and 768 context-specific gradient calculations. It opens no new data role and no sealed
attention-0 result.
...
ending the goal. If 469 supports the quadratic reader/state interface, the next action is an exact intervention built
from it. If it does not, the registered successor is a context-conditioned causal state model using distance,
predecessor count, and downstream use—not another product-count or rank sweep.
