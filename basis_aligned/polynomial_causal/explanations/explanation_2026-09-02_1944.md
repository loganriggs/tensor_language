...
Rung504 is now claimed and preregistered. It will remove every unordered pair among the same 18 sources, using the
same exact BF16 computation, and then recompute attention/MLP layers 10–17. Thus it measures the actual finite change
in copy cross-entropy rather than using a loss gradient as the causal result. For sources `s,t`, define the simultaneous
local removal effect `J_st` and the finite mixed difference
...
screen will also calculate the portion of the score action's copy benefit that disappears under each removal, and its
corresponding two-source inclusion–exclusion term. A pair must materially support both the MLP9 response and the
finite copy benefit, reject payload controls, and reproduce as the same complete pair set on fresh document halves
before any further validation opens. This can expose a genuine two-source interaction that the singleton screen
necessarily missed, while directly resolving whether M8's negative local gradient was misleading. If no pair
survives, the evidence will favor changing the observation or using the explicitly float32 explanatory tensor rather
than trying larger post-hoc subsets.
