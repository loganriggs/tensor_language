...
The proposed search is therefore **not** for a small subset of the 4,608 native hidden coordinates. It searches for
the smallest rotated subspace of those coordinates whose interchange has the registered causal effect. Native-unit
selection is included only as a dimension-matched control. In symbols, we want the smallest $k$ for which a learned
rank-$k$ projector $P$ passes the held-out swap, removal, specificity, and composition tests:

$$
k_\star
=\min\left\{
k:\ P=P^\top=P^2,\ \operatorname{rank}(P)=k,
\ \text{and }P\text{ passes the registered causal tests}
\right\}.
$$

This makes “minimal” operational rather than purely geometric. A smaller reconstruction rank does not win unless
the corresponding subspace transfers the intended computation and preserves the registered unrelated computations.
...
There is an important data-boundary correction before doing this optimization. The 62 circuit masks are saved as
member/control row indices on the original 1,000-row census; most are not portable semantic rules that can simply be
applied to thousands of new documents. In particular, `r.2.0.2` has only a weak surface classifier and no surviving
behavioral interpretation. Treating its old row indices as though they defined a new-document predicate would make
the large-data power test invalid.

The corrected design gives the two resources different jobs:

1. Use portable, exactly computed MLP0 targets on fresh documents—initially the token-only branch
   $T\equiv\mathrm{TT}$ and token-by-context branch $I\equiv\mathrm{X}$—to measure whether the product-space
   gradients and interchange effects are stable enough to fit.
2. On the original census rows only, use the 32 discovery circuit masks as downstream response fingerprints and
   penalize changes to the other discovery circuits.
3. Choose the smallest preregistered $k$ that passes; do not select $k$ merely by reconstruction error.
4. Freeze $U$, compile it to $(Q_\ell,d_\ell)$, and require the compiled quadratic implementation to reproduce the
   activation-space intervention.
5. Test the 30 held-out circuit masks on their frozen census rows, and separately test the portable branch/task
   effect on held-out natural documents and code OOD, all without refitting.
6. Compare against dimension-matched random subspaces, shuffled circuit labels, native-unit subsets, and multiple
   optimization seeds.
7. Require selective removal, bidirectional interchange between implementations, and predictable joint installation
...
larger-document stability measurement. A planning calculation based on the old split-half correlation
$r=0.10594$ estimates about $4{,}888$ total documents to target split-half reliability $0.70$ under the
Spearman--Brown sampling model. That is only a rough power estimate; it does not authorize fitting. The next
implementation must first verify that $T$ or $I$ supplies a portable, live causal target on those documents and that
its measured product-gradient direction is stable across the two halves. Real-model DAS remains unauthorized until
that gate passes.

### The exact controlled interchange we would fit

The label-boundary audit has now passed: all 311 census leaves store member and slice **indices** on the frozen
1,000-row dataset, and the 864 saved members of `r.2.0.2` agree bit-for-bit between the state file and its circuit
file. This confirms why those labels cannot be extended to fresh documents without a separately validated rule.

Instead, write a base MLP0 input as token part $p_b$ plus context part $q_b$. For a different donor token $p_d$,
construct the controlled hybrid input $(p_d,q_b)$. Its product-space change is exactly

$$
g(p_d,q_b)-g(p_b,q_b)
=\bigl[g_T(p_d)-g_T(p_b)\bigr]
+\bigl[g_I(p_d,q_b)-g_I(p_b,q_b)\bigr].
$$

The desired token-only interchange keeps only the first bracket. DAS therefore has a real separation problem: find
a projector $P$ such that

$$
P\bigl[g(p_d,q_b)-g(p_b,q_b)\bigr]
\approx g_T(p_d)-g_T(p_b),
$$

as judged by the downstream causal effect. Similarly, holding the token fixed and changing only context gives

$$
g(p_b,q_d)-g(p_b,q_b)=\Delta g_I+\Delta g_C,
$$

so an interaction projector must isolate $\Delta g_I$ from the accompanying context-only change $\Delta g_C$.
These hybrid identities now pass a deterministic planted calculation with relative errors between
$2.44\times10^{-16}$ and $2.98\times10^{-16}$. They specify the target exactly, but do not assume a small projector
...
- Rung 535 has now measured the exact two-piece interaction

  $$
  I_{S,R}=E_{\mathrm{native}}-E_S-E_R.
  $$

  It is substantial and code-stable, but not yet corpus-stable in the current $(S,R)$ coordinates.
- The next MLP step is the corrected label-and-power preflight for weight-compiled product-space DAS. It asks whether
  an exact, portable $T$ or $I$ intervention supplies a stable causal learning signal before any rank or optimizer
