...
## Answers to Logan's questions (24 September, 04:10 UTC)

**What the 100 output directions are.** They were chosen, not discovered; the lane then found what carries each one's second-order response.
- 16 *reader* directions: the 16 output "writer" directions w_g of the fitted quartic program for the MLP16→MLP17 path from the compression lane (21 September note on shared feature conditions, `EXPANDED_ROOT_EMPIRICAL_V1.pt`). Each is mapped to the residual direction whose unembedding read best matches w_g's logit pattern (R = Wᵀ(W w_g)/‖W w_g‖², normalised). They are the model's own late-block output features, so they were the natural "reader" targets.
- 84 *token* directions: the normalised unembedding rows of the 84 most frequent next-tokens in the FineWeb rows: ".", ",", "\n", " the", " and", " to", " of", " a", " in", "-", " for", " is", … — i.e. "how does a block-8 perturbation change the logit of *the*", as directly as possible.
The two families behaved alike on every measure (C1a pred_e passed; C1c and C3 report them separately), which is why the note pools them.

**Are the three heads the same for all 100, and is it QK or OV?** Only the **OV side** was attributed and patched: freezing or patching a head's *values* (the value projection of the source residual, which is what carries a perturbation from one position to another) while its attention pattern (QK) stayed free. The pattern's own second order was measured only at block level in the symmetric-DCT lane (rung 4: ~10% of the total), never per head. On the value side, per direction the top-3 heads carry a median 62% of the value-path share, and the value path is 79% of the whole. The identity of the top-3:
- exactly {8.2, 9.7, 9.8} for 43 of the 100 directions;
- 9.8 is in the top-3 for 81 directions, 9.7 for 73, 8.2 for 63;
- the recurring alternates are 11.6 (16 directions), 10.5 (27), 11.3 (11), 17.2 (5); the per-direction head-share vectors have mean pairwise cosine 0.65.
So it is one shared transport, with a second tier of heads that some directions use instead. This is head reuse on the *same 8 contexts* for every direction; whether the same heads carry a direction's response on fresh FineWeb contexts is what the run below tests.
