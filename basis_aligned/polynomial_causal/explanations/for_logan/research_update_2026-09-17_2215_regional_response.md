...
| Special composition of framing/clause destinations | edit | opened prospective panel | 1.1 times random median; beats1/9; gate.50 and8/9 | fails |
...
A second partition also fails to identify specially composable parts. Splitting the same complete write by framing versus clause destinations gives two live pieces and small interactions, yet its worst interaction is 1.1 times the median of nine same-cardinality random partitions; it beats only one of nine (**edit**, opened prospective panel). The registered requirement was at most half the median and better than at least eight. Near-additivity is again generic. The framing/clause split is a different object from the earlier physical-product split, so this neither rescues nor repeats its verdict.
...
Destination-partition result: [registration](../../DESTINATION_PARTITION_V1_PREREGISTRATION.md), [result](../../DESTINATION_PARTITION_V1_RESULT.json).880forwards in13.525780s. Native/full anchors exact; disjoint head9 writes sum with max relative error4.962250e-16. Both parts are live: clause/full target-norm ratios .2322–.3429; framing/full .6581–.7849. These unsigned ratios are not attribution fractions and need not sum to one. Behavioral interaction/smaller-single .003095–.008457 and joint prediction errors .000719–.002364 pass their .25/.10 gates. Specificity fails: canonical/null-median1.096903 versus.5, one of nine nulls beaten versus eight. No per-piece selective removal or fresh partition confirmation was scored.

Stop arbitrary repartition search at this boundary. The next [registered scope test](../../TYPED_FACE_NATIVE8_SCOPE_V1_PREREGISTRATION.md) applies the identical head8 write at the actual attention8 output, allowing MLP8 and every later consumer to respond, and compares it with the established conditional odd-value route. Existing source-edge and midpoint protocols explicitly restrict that route; its selectivity cannot be presumed for the broader intervention. The new runtime branch has a [CPU implementation preflight](../../TYPED_FACE_NATIVE8_SCOPE_V1_CPU_PREFLIGHT.json), with numerical validation and scoring still pending. The old response census cannot localize the new intervention's response.
