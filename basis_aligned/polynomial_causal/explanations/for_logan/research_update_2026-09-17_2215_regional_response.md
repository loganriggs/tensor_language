...
The retained city-dependent head8.2 write reaches regional spelling margins through the conditional head9.8 odd-value branch and a distributed native suffix. A new fixed-edit census rejects both direct-head9 sufficiency and the registered early-MLP response set. The line-break family has nearly the same response allocation as the other constructions, so its failed minimum-attenuation gate is not accompanied by a wholesale route change. These are response attributions, not independent downstream interventions. Separately, native and isolated replay now verify a head9 interface without inherited-first-value or initial-state inputs; the larger path still has external native states and suffix, failed composition specificity, and no whole-path simplicity result.
...
| Reduced head9 interface runs in isolation | fold | 40 opened native fixtures | 0.90 million stored scalars; raw-state and upstream-write inputs | passes |
...
Next-action evidence: [explicit head8/head9 composition CPU check](../../TYPED_FACE_COMPOSED_RAW_V1_CPU_RESULT.json) passes three synthetic lengths against the legacy conditional calculation. It accounts for1,788,419stored weight scalars including reentry lambda and74,880native-state scalars atT=32 across current8,donor-city8,raw9. This wider interface still needs native end-to-end replay; mathematical implementation composition does not imply small causal interactions or a passing composition property.
