...
Complete city-value removal now also passes its prospective strength test on the same opened documents: **2.3% prediction error**, **30% mean attenuation**, and **102/111 capable contrasts positive** (edit, opened data / new intervention strength). It beats all sixteen matched nulls and preserves all four readers. Twice the measured half-removal effect has **12% error**. This establishes useful nonlinear prediction across strengths, but does not isolate the local quadratic term from the nonlinear suffix. A registered matched-suffix ablation is testing that distinction.
