...
**The verb site (v216–v218, evening).** The gender state is re-detected at the verb by the same two units (3152 carries 78% and 3943 10.5% of MLP 8's write on 9.6's direction there), head 9.6 reads the verb (48%) as much as the noun (50%), and the feature is copied to the verb by head 4.5 (77% of block 4's contrast, all from the noun position) — the same copy head the number line uses, which is why zeroing it moves animacy and tense readers as well. But the he − she readout does not use that copy: zeroing 4.5 at the verb leaves the margin unchanged (−0.5% [v219]), unlike the number line's 7.7%. What feeds 3152's re-firing at the verb is the token copiers themselves: head 8.1 carries 34% of the detector's contrast there, block-6 attention 24% and MLP 6 31% [v220] — the gendered token travels with its copies, so a copy of the *feature* by 4.5 is redundant. Number, being computed rather than read from the token (8.1 carries −1% of it), needs 4.5's feature copy: the read-from-token / computed distinction reaches the verb site too. The edit decides it: zeroing 8.1 at the verb removes 29% of 3152's verb contrast and 3.5% of the he − she margin, the other eight block-8 heads at most 0.45% [v221]; head 12.4 reads the same two detectors as 9.6 at the noun (3152 53%, 3943 20% of MLP 8's write on its direction [v249]); the loss lands on head 9.6 (86% of the readers' coefficient drop), the unrelated readers stay inside their gates, and zeroing 8.1 at every position costs 12.9% [v222]. The full account of that head, including the single 128-d axis that carries number selectively, is §4.8 of `in_depth_circuit_number.md`.
