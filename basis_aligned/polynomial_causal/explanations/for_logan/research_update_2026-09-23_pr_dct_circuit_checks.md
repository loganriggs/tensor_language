...
So the variant fixes the training pathology and is honest about generalisation (OOD retention 0.82 on WikiText, 0.81–0.87 on FineWeb, reproduced), but what it finds is single source-block neurons ranked by a context-averaged downstream Jacobian. The same run on bilin18 (squared attention) is in progress; its receipt will be added below.
