...
The regional operator removes half of head8.2's complete city-token value contribution and approximates its coupled MLP8 response using one supplied residual7 state. Fresh prediction and selective-removal gates all pass: **2.2–13% effect error** and **10–13% attenuation** across families (**edit, fresh**). Its16.9million-float export passes CPU and isolated replay; native readout certification of this layout is pending. The upstream state, later suffix, corpus generalization and composition remain open.

```mermaid
flowchart LR
 R[External recipient residual7 state] -->|fold: one array, opened implementation| H[Head8.2 full city contribution]
 T[Token IDs and weight-derived tables] -->|fold: 53 supported tokens| H
 H -->|edit: inherited plus current half-removal, fresh| M[Coupled skip and approximate MLP8 response]
 M -->|edit: 2.2–13% error, fresh| S[External native suffix and spelling]
 I[Inherited-only removal] -.->|edit: 75% positive vs 90% gate, previous fresh| F[Consistent attenuation fails]
...
| Export needs one native state | implementation/replay | opened |40CPU and40isolated fixtures | passes these checks; native layout pending |
...
| Extracted | One-native-input export passes CPU/isolated replay. Native readout certificate pending; native prefix and suffix remain external. |
