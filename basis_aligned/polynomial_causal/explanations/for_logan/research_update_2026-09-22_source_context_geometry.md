...
## 5. A shared bank instead of output-local atoms (2 of 5 passed as written; 19:08 UTC)

One bank of 96 atoms shared by all 16 outputs with per-output coefficients (443,904 coefficients), fitted on the same 3,040 documents with the same loop and seeds, against the output-local fits of §4.

| arm (seeds 25001 / 25002) | fresh small-output value error | fresh response error | cross-start cosine 4–15 (mean / min) | span identification across seeds |
|---|---|---|---|---|
| output-local, 6 atoms per output (§4) | 34.3 / 34.1% | 36.3 / 36.0% | 0.89 / 0.76 | 0 of 12 outputs with all 6 CCs ≥ 0.9 |
| **shared bank of 96** | 33.9 / 33.9% | 35.9 / 35.9% | 0.87 / 0.74 | 12 of 96 bank directions with CC ≥ 0.9 (median CC 0.46) |

Sharing costs nothing and gains nothing (ratios 0.99), and it does not identify the dictionary either: about a dozen bank directions recur across seeds, the rest do not, while the correction function agrees at 0.87. So at this capacity the residual is a stable *function* implemented by unstable *atoms*, whether the atoms are typed, local or shared. That closes the atom-dictionary route on this branch as far as I have taken it; the remaining honest levers are capacity (more atoms) and the objective (Codex's Gaussian weight term as a regularizer, now with enough data that it may no longer be needed), and neither promises identification.
