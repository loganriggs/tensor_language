...
Requested direction, 13 September 2026. **Status at 06:44 UTC: campaign underway.** Initial interaction fits, a small exact graph simplification, full-U shared/private fits and sparse-core baselines have completed; see the progress note below. This supersedes the previous restriction against compression as a primary task. Discover structure from weights first, then validate on text. Earlier failed fits remain relevant controls.

### Initial progress

- **Setting 1:** Unrestricted-input projector/native-product baselines remain weak. Respecting the actual generated response exposes an exact simplification: three response vectors and six downstream symmetric products instead of four and ten. The product bank is40% smaller per context; native validation preserves all160 existing regional/FineWeb outcomes exactly. Background and attention dependencies remain. [Derivation and native result](../../RESPONSE_PRODUCT_BASIS_V2_MATH.md).
...
- **New setting 2 sparse-core check:** Partial output/head rotations are cheaper than rotating all three modes. At 2% coefficient error the best tested fixed frame saves 8.26% of dense folded-tensor storage; at 5%, 18.29%. Native behavioral fidelity and execution savings are untested. [Derivation, accounting correction and results](../../INTERACTION_SPARSE_CORE_BASELINE_V1_MATH.md).
