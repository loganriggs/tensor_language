...
Requested direction, 13 September 2026. **Status at 07:27 UTC: campaign underway.** Initial interaction fits, a small exact graph simplification, full-U shared/private fits and sparse-core baselines have completed; see the progress note below. This supersedes the previous restriction against compression as a primary task. Discover structure from weights first, then validate on text. Earlier failed fits remain relevant controls.
...
- **Setting 1 whole-interaction correction:** The small local-effect errors above do not describe the original two-edit interaction, where current error is11.9–16.1%. The main omission is an inherited mixed response before block10. A shared three-branch extension now preserves that response in an actual-weight algebra control; native-text validation remains pending. [Coverage audit](../../COMPOSED_ORIGINAL_INTERACTION_COVERAGE_V1_MATH.md) · [Extension and control](../../COMPOSED_JOINT_RESPONSE_V1_MATH.md).
