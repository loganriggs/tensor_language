...
**Final campaign status:** read the [13 September final compression report](research_update_2026-09-13_final_compression.md). The updates below are historical; the final report includes the96-prefix transfer, composition, selective-pair and package tests that were still pending in older entries.

**Newest result, 13 September13:51 UTC:** fitting sparse readers against the composed two-QK even numerator passes all five tested corpus aggregates at unchanged storage, after ten locally converged starts. Legal/patent error falls from16.52%to4.02%; other weighted errors3.60–5.06%. This is stronger than basis-only fitting, but six individual documents still fail10%, and fresh inputs/signed edits/child composition remain. Savings are16.31%ofreader representation,1.806%ofparent interface; no runtime gain. [Full derivation, comparisons and limits](../../DIRECT_PARENT_KEY_COMPRESSION_V1_MATH.md).

**Current status, 13 September 13:28 UTC:** the three settings below remain the campaign definition. Exact shared-tail algebra gives a real local simplification in setting1 and transfers to new lexical contrasts. However, removing the full cross interaction from the ordinary native state barely changes spelling capability; larger parent/remainder terms are more consequential. Compressing that parent saves2.77% of its declared interface and passes aggregate lexical checks, but fails FineWeb/legal transfer. The latest diagnostic rejects distant attention as the dominant error source. Keep the exact parent as the general reference. [Parent results and diagnostics](../../DIRECT_PARENT_KEY_COMPRESSION_V1_MATH.md) · [Shared-tail results and native-removal correction](../../SHARED_POLYNOMIAL_TAIL_V1_MATH.md).

**A concrete reuse lesson:** a standalone composed fit would save about10% if its original producer could disappear. Other consumers still need that producer, so installing the fit would instead increase joint storage by82.29%. This is why independent versus joint compression must include every surviving dependency. Ten starts reached locally stationary solutions but also missed the declared coefficient-error target. [Fit, convergence and joint price](../../COMPOSED_READER_REFIT_V1_MATH.md).

**Latest addition, 13 September 11:02 UTC:** the shared-denominator algebra generalizes to two distinct directions: seven response vectors need nineteen same-branch symmetric products instead of twenty-eight. A direct compiler verifies the identity and saves 32.1% of local output-bank storage; preparation speed is 1.07×/1.45× at CPU batches 1/8, missing the declared 1.1× threshold at both sizes. This is an algebraic test with transplanted head8.2/head9.8 directions at one common input, **not validation of their original circuit paths**. Separately, protected low-rank and sparse approximations of the upstream response map failed live response preservation, so that map remains exact. [Two-writer derivation, provenance correction and timing](../../TWO_WRITER_RATIONAL_BASIS_V1_MATH.md) · [Upstream compression tests](../../UPSTREAM_RESPONSE_MAP_V1_MATH.md).
...
Caching that shared component speeds product-bank preparation by about 1.27–1.35 times on tested CPU batches and approaches 20% bank-storage savings at large batches. This is a local gain. The full replacement still needs native weights and background computation; measured whole-branch speed does not improve. [Derivation and native validation](../../RESPONSE_DIAGONAL_CONIC_V1_MATH.md) · [Shared component, costs and intervention evidence](../../FIXED_WRITER_PRODUCTS_V1_MATH.md).

The shared term is behaviorally material: deleting it changes regional interaction effects by roughly 8–18%, including controls. It is therefore a useful computational primitive, not yet a selective spelling circuit. Its final-position and earlier-position removal effects approximately compose on the regional panel (under 1.5% target error), but one small-effect FineWeb group fails. Precision and intervention-strength counterchecks suggest a substantial numerical floor; they do not erase that original failure or establish fresh/OOD transfer.

In setting 2, a sparse folded tensor saves about 31% packed storage, but its CPU executor is slower and uses more working memory. Its 2.7–8.1% errors on the retained interaction do not transfer unchanged to the full head: matched full-head errors are about 10.2% on regional text and 20.5% on FineWeb. Shared-output and balanced common/difference fits have also completed; neither establishes a better adopted compression. Setting 3's full-unembedding shared/private fits remain unconverged and fail behavioral preservation. [Setting 2 results and scope checks](../../SPARSE_INTERACTION_EXECUTOR_V1_MATH.md) · [Shared-output fits](../../INTERACTION_SHARED_WRITES_V1_MATH.md) · [Setting 3 results](../../FULLU_SHARED_LOCAL_FIT_V1_MATH.md).

The priorities below remain: compare independent versus shared representations, count adapters and background, and distinguish algebraic simplicity from behavioral circuit identification. The initial-results chronology below is historical; this opening gives the current verdict.
