...
It therefore establishes a causal edge but not a standalone edge computation.
The next compression target is the scalar change along the fixed axis. On the
opened discovery panel, fit a small label-free polynomial of the frozen embedding
score (with pair/template cross-validation), freeze it, and evaluate both scalar
prediction and behavioral rescue on another untouched vocabulary panel. If no
small function predicts that scalar, retain the embedding node and L11H3 node as
separate graph regions connected by an unresolved distributed subgraph rather
than hiding the donor port.
