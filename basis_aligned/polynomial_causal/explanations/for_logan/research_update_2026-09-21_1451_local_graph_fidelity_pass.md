...
**Independent replication follow-up,15:05UTC.** The unchanged graph again passes all72absolute checks. It fails five baseline comparisons across four distinct cells. Pooling64FineWeb documents and32code files leaves two relative failures, both component3/code/spaced-word: natural ratio1.137 and donor-hybrid ratio1.106 versus the covariance baseline. All pooled comparisons against the isotropic baseline pass. Original panel failures remain; pooling does not overwrite them. We stop panel collection and turn to source-error versus nonlinear-readout diagnosis. [Replication comparison](../../direct_tensor_match/FRONTIER_REPLICATION_COMPARISON_V1.json), [pooled document audit](../../direct_tensor_match/FRONTIER_FRESH_AUDIT_POOLED_V1.json).
