...
Using the remaining legal arithmetic budget for fourteen additional private directions also fails: the best covariance allocation reaches **7.808%**, still above the **7.734%** limit, and that candidate fails the strict compiler reconstruction check. Both failures remain recorded. The structural null comparison finds much greater native alignment: average squared top-64 principal cosines are 0.971 in covariance geometry and 0.884 in original coordinates, versus at most 0.714 across 32 independently randomized orientations. This is compatibility evidence, not identified semantic reuse. The next control keeps the upstream MLP input weights fixed and permutes only folded channel coefficients, to test whether common architecture explains the alignment. Its source reconstruction and permutation-invariant checks pass; the null experiment is registered but has not run.

[Controlled native refit](../../direct_tensor_match/PENCIL_JOINT_REFIT_V1.json), [private-capacity result](../../direct_tensor_match/PRIVATE_CAPACITY_V1.json), [structural null result](../../direct_tensor_match/PAIR_SUPPORT_NULL_V1.json) and [architecture-control plan](../../direct_tensor_match/PAIR_SUPPORT_ARCHITECTURE_PLAN_V1.json), [three-hour math/literature review](../../THREE_HOURLY_MATHEMATICAL_REVIEW_2026-09-21_1350.md).
...
**Architecture-control follow-up, 14:20 UTC.** The completed control passes its registered comparisons, but common coefficient permutations retain nearly all the support alignment: .96625 versus native .97131 in covariance geometry, and .88061 versus .88360 in original coordinates. This sharply limits the interpretation of the earlier Haar-null excess. A subsequent coefficient-energy audit finds that aligned64-dimensional spaces contain87–96% of pair energy in covariance geometry but only13–21% in original coordinates, where substantial cross interactions remain. These are structural diagnostics, not a new accepted circuit. [Results and interpretation](../../direct_tensor_match/ARCHITECTURE_ALIGNMENT_INTERPRETATION_V1.md).
