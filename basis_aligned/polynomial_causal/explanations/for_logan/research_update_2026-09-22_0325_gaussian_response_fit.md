...
## 03:31 follow-up: how much does the Gaussian approximation matter?

A calibration-only diagnostic improves prediction without changing any feature directions. Using the full empirical calibration moments lowers larger-panel value errors from **7.33/7.52% to 5.80/5.85%**. Small-output response errors improve from **57.57/59.98% to 49.74/50.91%**, but still fail the registered primary improvement test on the original panel.

This method uses all 6,144 calibration output targets, so it is explicitly data-based regression, not a new weights-only result. The Gaussian and empirical methods have matching input mean and covariance; the difference is higher-order distribution information. The paper's metric $M$ lives on lifted polynomial coordinates and can encode such information—it is not synonymous with raw input covariance. A finite empirical metric does not provide global polynomial equality guarantees.

This qualifies the earlier negative: a failed Gaussian fit does not prove that no better readout exists in the same feature span. However, the improved readout remains far from accurate on the smaller components. [Full comparison, matching checks, and the connection to the paper](../../direct_tensor_match/MIXED_CP_CALIBRATION_MOMENTS_INTERPRETATION_V1.md).
