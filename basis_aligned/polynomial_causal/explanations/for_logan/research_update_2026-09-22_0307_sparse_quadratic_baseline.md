...
## Follow-up: let each output choose its own features

A separate spectral baseline reduces centered error to 24.16% at 4,608 products, versus 76.41% for the earlier fixed-basis sparse core. Optimally allocating the products by output energy lowers pooled error to 17.44%, but the worst output then has 42.83% error. The native shared-channel program is exact at the same product count. This shows why preserving cross-output reuse matters; independent output fits are still an uneconomical replacement. [Full derivation, allocation controls and results](../../direct_tensor_match/OUTPUT_LOCAL_BILINEAR_INTERPRETATION_V1.md).
