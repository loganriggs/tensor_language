...
The next backward step also rejects a tempting omission. At the input to MLP7, the earlier residual plus attention7 predicts the full source change with 22–70% error, failing its 20% gate (edit, eight opened contexts). The native token-embedding source added at block7 therefore remains explicit. Combining it with the earlier residual looks better in this panel, but that subset was not the registered passing hypothesis and is not promoted. These effects rerun MLP7 and both key factors; they are not rankings by weight magnitude.
...
| Earlier residual6 alone predicts the complete MLP7-input change | edit | eight opened contexts | 31–87% cue errors; gate 35% | fails |
| Earlier residual6 plus attention7, omitting initial7 | edit | same opened panel | 22–70% cue errors; gate 20% | fails |
...
Further backward-source screen: [registration](../../MLP7_INPUT_SOURCE_V1_PREREGISTRATION.md), [result](../../MLP7_INPUT_SOURCE_V1_RESULT.json). Here source bits are residual6=1, attention7=2, initial7=4; all corners recompute MLP7, with donor initial8 and inherited value already fixed. The reference is corner7 minus corner0 and differs from the prior source test's reference. Zero corner replays old source4; full corner replays the full face. Native anchors pass (max absolute 4.053116e-6, relative 6.399582e-7); complete Möbius closure is zero. Residual6-only cue errors .86835991/.30791828; residual6+attention7 .69737784/.21549617. Posthoc residual6+initial7 errors .21737491/.09810246 are exploratory. 160 forwards, 3.178530s; source claim 21:49 UTC to exit 21:51:06, roughly two minutes including authoring and queueing. No source-specific matched null or fresh confirmation accompanies these opened tests.

[Exact local response analysis](../../MLP7_EXACT_RESPONSE_V1_RESULT.json) expands MLP7 under every source change into normalization, bilinear cross, and quadratic terms, plus the residual skip. All local identities pass on the opened source states. Bias cancels explicitly. These are local response vectors, not downstream intervention results or a new selective component. The next circuit-hour handoff is structural template controls for the complete retained path; the failed source omissions stay disabled.
