...
against many small carriers [v202], so MLP 1 is a port at unit grain and the named chain starts at MLP 3; (half closed) why edits run 2–4× below carrier shares: downstream of the readers there is no compensation — under the MLP-5 edit the reader blocks carry 77% of the margin change and the later MLPs move with it [v224]. At the adjacent MLP-5 → MLP-6 stage the gap is pure arithmetic: a carrier share is the *linear* term of the product's change, and the exact in-place leave-out $u(\hat x)-u(\hat x-c)$ from the native trace predicts the edit to 0.4 points (6.0% vs 6.0%) while the linear term says 22% [v225]. At the MLP-6 → MLP-8 stage that same leave-out overshoots the edit 2.3–4× (15–28% vs 6.3% [v226]): the responder is MLP 7, which restores 71% of the removal's linear effect on the detector at the noun (direct −13.5%, MLP 7 +9.6%, block-7 attention −1.5%, exact −6.3% [v227]) — a homeostatic relay: the same block that carries 18% of the number (§4.4) also pushes back when its input is cut, and by unit the push-back is mostly one unit, MLP-7 unit 1779 (40% of MLP 7's response, top-10 units 68% [v228]), which is also MLP 7's main native relay of the number into the detector (49% of MLP 7's carriage [v229]) — relay and restorer are one unit; how it restores is open: at block grain MLP 6 feeds 1779 with the number (+26% carriage [v230]), so the push-back is not a damping by MLP 6 — the trio is in fact 1779's two largest inputs (57% of MLP 6's carriage into it, same sign [v231]) — and is not a damping input nor a normalisation artefact: an edit that removes only the trio's plural − singular difference and keeps its mean write leaves MLP 7's push-back unchanged (+0.10 vs +0.096 [v233]; the in-place algebra of v232 had suggested the mean terms and the RMS renormalisation, and that reading is falsified). MLP 7 responds to the missing number signal itself — under that edit unit 1779 alone is 65% of the push-back [v234] — and its own number contrast *shrinks* by 16% under that edit [v235], and its restoring push cannot act through the detector's normalisation (the detector reads an RMS-normalised input, so writes along the residual are invisible to it — the radial part is exactly zero [v236]); the carrier identity itself is cleared — recomputed with the true scale-invariant gradient it reproduces the fixed-rms census to within 2% [v237] — and the resolution is the detector's own bilinearity: the edit lowers 1779's value on *both* row classes (−2.2 on plural, −4.6 on singular, whose difference is the 16% shrink), and the detector's gradient along 1779's write has opposite signs on the two classes (−1.6 and +0.6), so the common-mode drop widens the detector's contrast on both sides (+0.037 and +0.029 [v238]). Under the original zeroing the shift is differential instead (−3.2 on plural rows, +2.4 on singular) and the restoring plural half wins because the detector's gradient there is 2.8× larger [v239]. The compensation is a product-gradient effect — the response is the class-wise sum of gradient × shift — not a damping input and not a normalisation; that same edit costs the margin 2.0%, three times the zeroing's 0.7%, so the trio's mean write is harmful to the behaviour elsewhere; the verb-position copy of the state (11.8% vs 5.3%) is the same two detectors re-firing at the verb (829 33%, 953 25% of MLP 8's write there [v203]), and at the verb the carriers are again the MLPs at that position (74%; attention blocks 4 and 5 lead the attention share at 9% and 8% [v204]) — the head that brings the feature to the verb is 4.5 (94% of block 4's contrast at the verb, 97% of it from the noun position, 26% token-only [v205]), with block 5's head 5.3 re-reading the verb position rather than copying. Zeroing 4.5 at the verb removes 7.7% of the margin, ten times any other block-4 head, but it is not selective (animacy and tense readers move five times their null [v206, v207b]): 4.5 is a shared subject-feature copier, as 8.1 is a shared token copier across families, so it is a carrier the number component uses, not a part of it. The number does ride a separable direction of that copy: removing only the component of 4.5's verb slice along $O_{4.5}^{\!\top} r$, with $r$ the plural detector's product gradient at the verb, takes 4.3% of the margin (55% of the whole-slice effect), beats sixteen random directions (≤ 0.6%) and leaves the animacy and tense readers inside their gates [v208] — a direction of a shared head is a component even when the head is not (with a norm-matched random null the picture holds: random directions ≤ 0.9%, readers at the null level [v209]; and a single fixed vector, the mean of the per-row directions, does 84% of the per-row job — 3.6% of the margin, selective, four times the matched null [v210] — so the number component at the verb is one 128-d direction of head 4.5; on natural text that fixed direction is weak, 0.8% of the congruent margin at twice the matched null, and behaves as a plural/they direction rather than a number contrast [v212] — the per-row directions are in fact one axis whose sign encodes the number (mean |cosine| 0.93; every singular row points opposite to every plural row [v213]), so the fixed axis is the component and its weakness out of the panel is real, not a construction artefact); and the MLP-6 trio's off-noun behaviour. Closed since: unit 1030 is a weak, plural-leaning third detector (11% of the trio's natural-text effect; the three singles add to 97% of the trio [v240]). Failed predictions on the way are on the scorecard: 35 of 129 registered readings on this line's receipts were false.
