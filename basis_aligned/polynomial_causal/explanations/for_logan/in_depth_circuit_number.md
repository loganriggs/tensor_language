...
The verb-copy stage copies the output of the agreement computation of MLPs 1–3, not the token. At unit grain (v361) MLP 3's half is led by 3465 (+172k on the reader through 4.5), 114 (+74k) and 493 (+32k), with a long tail (top ten 21%):
the lexical plural-noun detector fed by MLP 1's entry writes the state the copier carries, with the sign that favours 'they' for plurals. Edits agree at the margin (v362): zeroing the trio on the pronoun rows costs 3.6% of the they − he margin (16× the random-trio
null; the pair 2.2%, 114 alone 0.9%, additive) — small because the final margin rests on the later stages and the §4.9 compensation, but real and
named. The path MLP 1's entry → 3465 → head 4.5 → the pronoun reader is now followed at edit grain end to end. On 128 natural sentences (v364) head 4.5 leads block 4's number write at the cue noun with the panel's sign at 1.7 pooled std, so the copier
is live out of distribution at the noun; at the final token it leads only weakly (0.2 std), unlike the panel, where 4.4 and 4.8 led. Its writers on text (v365) match the frames within 0.03: MLP 3 0.53, the embedding path 0.20, MLP 2 0.19, MLP 1 0.08, attention 0.03.

*Added 19 Sep (v367–v368), the stage above:* MLP 8's plural detector 829 is fed by the MLPs just below it — MLPs 5, 6, 7 carry half of each of its
factors, the embedding path 0.17, MLP 3 0.06, the block-4 copier 0.03 — and within those layers by units this dossier already named: 1036 (MLP 5),
2483 (MLP 6, the verb-site unit) and 1779 (MLP 7, the compensation relay of §4.9) lead their layers on both factors, each above a spread tail. At
unit grain the noun-state chain reads MLP 1's entry → 3465 / 493 → 1036 → 2483 → 1779 → 829, rebuilt MLP by MLP. On 128 natural sentences (v369) every unit of that chain separates plural from singular cue nouns with the panel's sign (0.7–2.2 pooled std
against 2.6–7.3 on the panel) and none carries number to the answer position: the stage-by-stage account is consistent out of distribution. And the chain is causal as a whole (v370): zeroing the six named units on the pronoun rows costs 14% of the they − he margin, 56× a
layer-matched random null, with the upper four (1036, 2483, 1779, 829) alone at 11% and the MLP-3 pair adding its 3–4% roughly additively. On the 128 natural sentences (v371) the same six cost 8.8% of the labelled-pronoun margin (60× the null; the upper four 7.8%). Widening each layer's set (v372, v373) the MLP populations saturate: the top unit per layer costs 13%, the top five 31%, the top ten 34%, the top
twenty 32% — the MLP stack's causal share of the margin is bounded near a third at unit grain, the rest belonging to the attention readers and copiers. On natural text the fifty cost 18.8% (v374; 57× the null), twice the six leaders' 8.8% and half the panel figure.
