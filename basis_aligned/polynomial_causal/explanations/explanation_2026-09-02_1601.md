...
MLP1 is quadratic. If `theta` is one attention piece and `midpoint` is the average of the normal and branch-absent
MLP1 input states, its exact contribution through MLP1 is computed by

`Down1((Left1 theta)*(Right1 midpoint) + (Left1 midpoint)*(Right1 theta))`.

The 63 resulting vectors must exactly sum to the complete attention-caused change in MLP1. This accounts for the
interaction of each attention piece with the actual rest of MLP1's input without requiring one arbitrary head basis.

Finally, each piece receives a 62-number **downstream-use signature**. For each already catalogued circuit, we take
the gradient of its member-versus-matched-control CE with respect to MLP1's write and dot that gradient with the
piece's exact MLP1 vector. Two pieces from different heads become a candidate shared computation only if these
62-number signatures agree on one document half, predict the second half, beat shuffled-circuit and shifted-position
