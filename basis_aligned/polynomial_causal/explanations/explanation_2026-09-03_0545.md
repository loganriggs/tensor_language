...
circuit are withheld from **shared-projector** gradients. An independent target-specific control may use the fourth
mask, but it cannot select or modify a shared frame. The shared objective has two parts:
...
identity, stable cross-fold projector agreement for at least four optimizer seeds, reuse on the fourth circuit that
the shared fit never sees, separation from all other attention8 circuits, and selective mean-centered removal on
TEST.

Equally sized recovery-only, target-specific, random, and retrained label-permutation projectors are controls. Every
frame and control is frozen using FIT and VALIDATION before TEST is opened once; no TEST result can trigger another
fit or seed choice. Good response recovery without selectivity is explicitly a low-dimensional approximation, not
an interpretable circuit.
...
Because this quartet was proposed using older measurements on the same census, a positive result would be within-
census operational extraction, not fresh cluster discovery or out-of-distribution semantic identification. The
final test also measures every non-quartet attention8 circuit and all positions outside the quartet so a generic
CE-sensitive direction cannot pass as a circuit merely by fitting the selected masks.
