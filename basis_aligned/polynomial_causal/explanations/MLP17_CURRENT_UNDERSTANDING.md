...
11September22:01: exact-native input-normalization accounting preserves all target interaction signs with denominator fixed; correction norm44.1%verbs/9.72%nouns, so the small-correction prediction misses for verbs. The all-output six-path numerator replay passes5.23e-15. [New proposal and primary accounting](for_logan/interaction_path_decomposition_proposal_2026-09-11.md#7-normalization-is-a-shared-computation-not-a-disposable-detail). No sparse composed-path fit yet.
...
The G64 shared/private fit reconstructs243token functions within10%coefficienterror, but they contain only7distinct Urows (237IDs identical);47IDs are outside theGPT-2tokenizer. This extends the earlier196/50274duplicate observation, not a semantic discovery. All243landingroup5 andonefunctioncaptures99.9919%ofsubsetenergy. A separate energybound proves>=93.97%ofthe overall fitgain isoutsidethissubset, so duplication doesnotexplainawayallimprovement. See [frozen fit and counteraudit](../FULLU_SHARED_LOCAL_FIT_V1_MATH.md#accurate-token-subset-duplicate-audit-and-its-limit). No use-frequency or native-behavior claim.
