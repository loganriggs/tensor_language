...
to predict the downstream effects, even though the latter had previously explained some of MLP0's activation. The
new continuous-factor experiment found a much better candidate: T and I appear to make different changes to MLP1's
input while sharing the continuous state that MLP1 uses to interpret those changes. This held strongly in both
discovery halves, but the run's pre-registered BF16 numerical tolerance was too tight, so the result is not yet a
claim. A separately registered repair is rerunning from scratch and will open held-out documents only if discovery
repeats.
...
screen, not an identified circuit.
...
- whether the T–I shared continuous reader survives held-out documents;
...
The active GPU continuation is rung488. A pass licenses extraction and selective-swap tests for the T–I shared
reader. A failure sends the MLP0 work to separate within-branch finite-response models. No rank sweep is on either
route.
