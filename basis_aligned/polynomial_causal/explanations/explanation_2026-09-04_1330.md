...
### The revised time budget the reusable system should target

The original 20–40 minute target below was still too slow. As of 13:51 UTC, the operating target is one **causal
screen or honest null every ten serial minutes**, with deeper identification running only for candidates that pass.
This does not call a ten-minute screen a verified circuit. It separates cheap discovery from held-out identification
and adoption-grade hardening so new scientific hypotheses are not blocked behind a bespoke execution compiler.

The revised budget is:

| step | target time before GPU | target execution/review time |
|---|---:|---:|
| prior-result lookup and precise hypothesis | 1–3 minutes | machine-checked novelty receipt |
| linked counterfactual rows from templates | 2 minutes | automated semantic/token checks under 1 minute |
| native capability plus module/head/full-state screen | at most 2 minutes of configuration | normally seconds of batched GPU time |
| common scoring and dossier append | 2 minutes | automatic verdict and timing receipt |
| next claim/promotion handoff | 1 minute | deeper work runs in parallel only for passing screens |

The full operating contract is in `CIRCUIT_ONLY_TEN_MINUTE_OPERATING_PLAN_2026-09-04.md`. A deep DAS fit can
legitimately take longer after promotion, but new screens continue while it runs. Ten minutes is measured per serial
candidate; parallel agents and multiple arms cannot be used to disguise slow serial latency.
...
The task-14 compiler is now stopped rather than finished. Its 743,881-call conditional plan and repeated audit cycle
were disproportionate for discovering whether a subject-number state exists. The code and blocked reviews remain as
useful failure evidence, but it will not be frozen or enqueued. Adoption-grade execution hardening belongs after a
candidate has passed a cheap causal screen, not before its first localization result.
