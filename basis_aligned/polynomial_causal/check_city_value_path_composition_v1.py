...
STEM='CITY_VALUE_PATH_COMPOSITION_V1'
@torch.no_grad()
def main():
 ...
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('200sequence equivalents;90block calls;CPU direct/MLP8 value composition');return
 ...
 changes={2:pieces[1:5].sum(0).float(),3:pieces[0].float(),4:(pieces[0]+pieces[1:5].sum(0)).float()}
 ...
 prior_scores=torch.load(P/'CITY_FINEWEB_VALUE_MEDIATION_V1_ARTIFACT.pt',weights_only=True)['values'][[0,1,2,4]]
 diff=v[:4]-prior_scores;absolute=float(diff.abs().max());relative=float(diff.norm()/prior_scores.norm());e=v-v[1:2]
 prior=json.loads((P/'CITY_FINEWEB_RESPONSE_V1_RESULT.json').read_text());reversed_docs=set(prior['reversed_documents'])
 def metric(idx):
  a,b,j=[e[k,idx,0] for k in [3,2,4]];return {'interaction_over_smaller':float((j-a-b).norm()/min(a.norm(),b.norm())),'single_over_joint':[float(a.norm()/j.norm()),float(b.norm()/j.norm())],'control_rms_by_arm':e[:,idx,1:].square().mean(1).sqrt().tolist()}
 ...
 docs=[{'context_id':d,**metric([i for i,r in enumerate(rows) if r['context_id']==d])} for d in range(20)]
 ...
 'pred_b':groups_out['all']['interaction_over_smaller']<=.35,
 'pred_c':min(groups_out['all']['single_over_joint'])>=.10 and all(groups_out[g]['interaction_over_smaller']<=.35 for g in ['reversed','other']),
 'groups':groups_out,'documents':docs,'anchor_max_abs':absolute,'anchor_relative':relative,'max_subtraction_relative_error':max(errors),'other_head_error':max(other_errors),'outside':outside,'body_forwards':200,'batched_block_calls':calls,'seconds':time.perf_counter()-start,'scope':'Opened direct/MLP8 value-path composition screen. Fixed swap background and keys; fullsuffix. No random-split specificity/fresh joint promotion.','source_shas':binding}
 torch.save({'values':v,'arm_names':['native','swap','minus_mlp8','minus_direct','minus_joint']},P/(STEM+'_ARTIFACT.pt'));out.write_text(json.dumps(r,indent=2)+'\n');print(json.dumps({k:v for k,v in r.items() if k not in ['source_shas','documents']},indent=2));signal.alarm(0)
