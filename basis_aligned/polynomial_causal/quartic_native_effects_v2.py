...
@torch.no_grad()
def main():
 parser=argparse.ArgumentParser();parser.add_argument("--stem",required=True);parser.add_argument("--initial",required=True);parser.add_argument("--output",required=True);parser.add_argument("--format",choices=["square","mixed_full","mixed_sparse"],default="square");args=parser.parse_args()
 ...
 result=json.loads((P/(args.stem+'_RESULT.json')).read_text());ap=P/(args.stem+('_PROGRAM.pt' if args.format=='square' else '_PROGRAMS.pt'));assert digest(ap)==result['artifact_sha256']
 ...
 if args.format!='square':pilot=pilot['programs'][0]
