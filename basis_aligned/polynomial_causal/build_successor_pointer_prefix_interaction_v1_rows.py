#!/usr/bin/env python3
"""Freeze outcome-blind rows for the successor pointer x prefix screen."""
...
OUT = HERE / "SUCCESSOR_POINTER_PREFIX_INTERACTION_V1_ROWS.json"
FAMILIES = {
    "month": [
...
FINAL_INDICES = {"month": range(4, 10), "digit": range(4, 8)}
...
def main():
    ...
    for family, elements in FAMILIES.items():
        for final_index in FINAL_INDICES[family]:
            coherent = elements[final_index - 4:final_index + 1]
            orders = {
                "coherent": coherent,
                "early_swap_control": [coherent[1], coherent[0], *coherent[2:]],
                "late_swap_incoherent": [*coherent[:2], coherent[3], coherent[2], coherent[4]],
            }
            for condition in CONDITIONS:
                text, ids = prompt(orders[condition])
                row = {
                    "row_id": canonical([family, final_index, condition]),
                    "family": family,
                    "final_index": final_index,
                    "condition": condition,
                    "elements": orders[condition],
                    "text": text,
                    "token_ids": ids,
                    "last_position": len(ids) - 2,
    ...
    counts = {
        family: {condition: sum(
            row["family"] == family and row["condition"] == condition for row in rows
        ) for condition in CONDITIONS}
        for family in FAMILIES
    }
    payload = {
        "schema": "successor_pointer_prefix_interaction_v1_rows",
        "selection_rule": (
            "all length-five month endpoints at indices 4..9 and digit endpoints at "
            "indices 4..7; fixed early and late adjacent transpositions"
        ),
        "outcomes_opened": [],
        "model_loaded": False,
    ...
    print(json.dumps({
        "out": str(OUT), "rows": len(rows), "interventions": 2 * len(rows),
        "cells": counts, "row_manifest_sha256": payload["row_manifest_sha256"],
