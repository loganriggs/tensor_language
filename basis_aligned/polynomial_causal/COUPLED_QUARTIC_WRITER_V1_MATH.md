...
## Targeted node restarts, 12 September 00:59

The [single-node replacement control](QUARTIC_NODE_REPLACEMENT_V1_CONTROL.json) escapes the saved stationary miss in2/4 trials when replacing the weaker node, versus0/4 when replacing the other node. All fits maintain constraints and descent; all registered predictions hold. The weak node is selected without access to the planted decomposition, by its conditional contribution after refitting the other output coefficients. For invertible feature Gram K and optimal mixing A, removing node j increases squared coefficient error by the squared norm of row j of A divided by diagonal entry j of the inverse Gram. This is an optimization-restart criterion, not the behavioral importance of deleting a circuit without compensation.

An [executed native-program diagnosis](QUARTIC_CONDITIONAL_NODE_V1_DIAGNOSIS.json) checks this identity against explicit leave-one-out solves to within1e-15 relative program energy. Node13 is least conditionally contributing in V2, V3 and LBFGS V1, costing0.0320%,0.0335%,0.0460% of their captured coefficient energy respectively. The indices refer to the evolving fitted bank, not a native neuron or identified semantic unit. This provides a candidate restart location after the longer fit, but it does not predict that an arbitrary new reader in1152 dimensions will be useful. Candidate initialization and residual correlation must still be tested. The running continuation remains unchanged.

The positive recovery control is limited to one failed small planted target; it is not evidence that2/4 is a general success rate. Two weak-node replacements still fail, and some other-node fits remain unconverged. Keep all failures in the receipt. The reusable synthetic objective helper reproduces the saved endpoint exactly and supports further basin tests without copying its target construction.

## Residual-scored replacement experiment, 12 September 01:04

Random replacement can escape a small planted basin but is not automatically useful in1152 dimensions. The next managed test scores candidates against the residual left by31 retained quadratics. It uses64 rank16 quadratics from the two existing native spectral initializations,32 random rank16 quadratics, and the original deleted node. The target remains the fixed two-output target of saved LBFGS V1. The currently running longer continuation is a separate experiment and is not changed.

For each candidate, project its coefficient tensor onto the retained feature span. Let d be its two-output target correlation after subtracting that projection, and let v be its remaining squared coefficient norm. Adding it improves captured energy by the squared norm of d divided by v. Candidates numerically contained in the retained span are excluded. The [Schur-complement control](QUARTIC_RESIDUAL_ADDITION_V1_CONTROL.json) agrees with explicit augmented solves within1.94e-15. This is the same conditional-projection algebra already used for mixed-core pursuit, now applied to replacement of a nonlinear intermediate.

The experiment removes the least conditionally contributing node, chooses the highest-scoring candidate, and refits32 output coefficient rows exactly. Re-adding the old node is allowed, preventing a forced coefficient regression. Literal final size stays592704 floats. The registered bars are numerical score/replay agreement and descent; at least1% additional captured energy; and at least20% reduction in native write error on the frozen cache. These are separate predictions: a coefficient gain does not establish the behavioral gain. No native cache value selects candidates, and no candidate receives nonlinear refinement in this screen. A miss only excludes this bank and replacement rule, not all residual-aware initialization or node replacement.

[Runner](../bilinear_quotient/ops/run_quartic_node_replacement_v1.py) and [input bindings](QUARTIC_NODE_REPLACEMENT_V1_BINDING.json) preserve the experiment. Native results will be in QUARTIC_NODE_REPLACEMENT_V1_RESULT.json after managed execution.

## Learned-bank cross-product comparison, 12 September 01:10

The earlier528-product result used the initial spectral quadratic bank. A [new matched experiment](../bilinear_quotient/ops/run_quadratic_product_core_learned_v1.py) freezes the learned LBFGS V1 bank and compares its32 squares with the full528 pairwise products and a32-edge greedy selection. This tests whether distinct learned quadratics need to interact, rather than assuming that separate squares suffice. It is a red-team of the initial-bank negative, not a new result yet.

The full solve measures the available coefficient improvement within this fixed quadratic product space. The sparse selection uses the same32 outer multiplications as the square baseline, with explicit pair indices; it is not a globally optimal support search. Existing dense coefficient and node-removal controls apply unchanged. New native replay checks require the square-only objective and write error to reproduce the frozen endpoint within1e-8 before interpreting a change. Registered numerical, sparse-gain-plus-fidelity, and full-core-fidelity bars are preserved from the initial-bank experiment. Candidate selection uses weights only. The long nonlinear square fit and node replacement test remain separate comparisons; no candidate is chosen by cache behavior.

## Terminal comparison, 12 September 01:54

All three managed runs and all four frozen native-effect evaluations completed. Their numerical replay checks pass. None passes both behavioral-fidelity bars, and no circuit is promoted. Replacement and mixed-core fits start from **LBFGS V1**, not the new long-fit endpoint.

| Change from frozen LBFGS V1 | Extra coefficient capture | Native write error | Interpretation |
|---|---:|---:|---|
| 720 further LBFGS steps | 8.96% | 26.65% →29.32% | Gain bar passes; stationarity fails. |
| Replace weak node from existing spectral/random bank | 0.277% | 26.65% →26.50% | Both substantive gain/fidelity bars fail. |
| All528 products of learned quadratics | 55.76% | 26.65% →30.62% | Substantial missing cross interactions in coefficient space; native fidelity fails. |
| Greedy32 products of learned quadratics | 3.49% | 26.65% →33.69% |28 selected edges are cross products; sparse gain/fidelity bars fail. |

These are relative gains over the old captured projection, not fractions of the target explained. The [long-fit receipt](COUPLED_QUARTIC_LBFGS_V2_RESULT.json) stops at the720-step limit after3237fit seconds, with projected gradient0.1020 against1e-6. Its objective is−1.919149 on the original normalization. [Replacement](QUARTIC_NODE_REPLACEMENT_V1_RESULT.json) drops node13 and selects candidate33, an old spectral-bank quadratic, in6.74seconds. The [learned mixed-core solve](QUADRATIC_PRODUCT_CORE_LEARNED_V1_RESULT.json) takes24.88seconds and has normal-equation residual6.86e-14: the linear full-core solve is accurate, while its underlying learned readers are not globally optimized for this richer model.

The same native-background checks give the following ranges across agreement verbs, count nouns, past and progressive: long-fit swap relative RMS31.46–55.59%, replacement34.54–67.11%, full mixed21.26–40.10%, sparse mixed29.36–49.83%. Every model misses the10% swap bar. Mean absolute removal-effect CE disagreement is at least0.0310nats in every model/family and reaches0.1127; all miss the0.02nat requirement. Source receipts: [long](QUARTIC_LBFGS_V2_NATIVE_EFFECTS_V1.json), [replacement](QUARTIC_NODE_REPLACEMENT_NATIVE_EFFECTS_V1.json), [full mixed](QUARTIC_LEARNED_MIXED_FULL_EFFECTS_V1.json), [sparse mixed](QUARTIC_LEARNED_MIXED_SPARSE_EFFECTS_V1.json). These are reused developmental rows, not new OOD evidence.

The result-dependent next discriminator is **a changed quadratic basis for the learned mixed core**. A dense interaction matrix can be sparse after re-encoding its inputs, but the resulting quadratic readers may become expensive. Apply the existing real-pair pencil method to the two32-by32 learned core matrices, verify exact execution, then truncate the resulting quadratic input forms to the same inner rank16. This differs from the earlier unsuccessful pencil on native1152-by1152 MLP17 forms. It tests whether the large new coefficient gain admits a compact shared representation before assuming that more factors are necessary. Failure of this truncation would exclude only this re-encoding and fixed truncation, not joint reader/core optimization or other DAGs.

The [executed re-encoding](LEARNED_QUADRATIC_PENCIL_V1_RESULT.json) reconstructs the full mixed program within2.09e-13. Its22 scalar and5 pair blocks require37 distinct quadratic products, with frame condition666. But truncating the transformed quadratic readers to rank16 gives50.79% native write error; prediction C fails. Individual quadratic truncation errors range46.4–71.7%. This is not a jointly refitted candidate.
...
## Native mixed-graph fit, 12 September 02:14

The [native price test](QUARTIC_SELECTED_EDGES_PRICE_V1_RESULT.json) has completed with all bars held. Gradient and forward evaluations take3.188 and1.469seconds, peak GPU allocation is10.69GiB, tangent finite-difference relative error is1.08e-5, and baseline objective replay error is2.44e-15. Its projected gradient is0.6113: the square-trained readers are not stationary for the mixed graph.

The managed [joint fit](../bilinear_quotient/ops/run_quartic_mixed_joint_v1.py) now allows240 updates or1200fit seconds on the previously frozen32-edge graph. At the measured cost,240 gradient-plus-single-forward updates cost about1118seconds, before additional line-search trials. Registered A requires baseline replay within1e-8 and monotone coefficient objective; B requires10% additional capture over the **mixed** starting fit; C requires projected gradient at most1e-6. A time/iteration limit is not convergence. The baseline native write error for this actual fit is33.69%, distinct from the original square program's26.65%.

The frozen terminal candidate will use the existing mixed-sparse effect scorer against the original square comparison baseline. Its result explicitly records both that comparison baseline and the actual mixed fit start. No cached outcome selects an iterate. Fitted storage remains592704 floats. A standalone32-edge executor needs64 endpoint indices; the shared scorer artifact additionally uses a deterministic full-pair lookup and selected-row representation, totaling1088 serialized integer entries. These serialization entries are disclosed rather than counted as learned weights or omitted from artifact size.

**Terminal result,02:33:** the [joint mixed fit](QUARTIC_MIXED_JOINT_V1_RESULT.json) completes240updates in1080.7fit seconds. Coefficient capture increases17.69% over the mixed start, meeting B, but projected gradient0.09498 misses the1e-6 stationarity bar. Native write error improves33.69→26.75%; it is approximately the original square baseline's26.65%, rather than a new large improvement over that earlier model. The [frozen effect checks](QUARTIC_MIXED_JOINT_NATIVE_EFFECTS_V1.json) pass numerical replay and fail both substantive bars: swap relative RMS is46.71/45.71/39.82/19.44% for agreement verbs/count nouns/past/progressive; removal-effect CE disagreement is0.0514/0.0992/0.0292/0.0534nats. No new OOD, closed extraction or selective-circuit claim follows. The next selected experiment is the already drafted residual-derived replacement, matched to the old square baseline; another identical mixed continuation is not automatic.
