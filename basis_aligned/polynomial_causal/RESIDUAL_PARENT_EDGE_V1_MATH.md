...
### Completed local fits, 19:10 UTC

Both managed local fits converged under the registered fresh-coordinate checks. The original graph took10.33fit seconds/200acceptedsteps, ending with gradient3.37e-8 and recentprogress5.85e-10. The added-edge graph took21.05fit seconds/400steps, with gradient8.17e-9 andprogress2.33e-11. Numeric checks passed. Full objectives were0.8843379498 and0.8843630382: the added connection still costs2.50884e-5, exceeding1e-6. Its three parent1consumer roles remain3.13%,32.62%,41.35%, and it saves1137floats, but the error bar fails. The original arm's own1e-6gain bar also misses because it began close to its local optimum; the changed arm's gain bar holds. [Aggregate](CLOSED_COMPONENT_REFIT_V1_AGGREGATE.json).

This resolves local optimization uncertainty more strongly than another timed-out fit: the tested closed-component refits converge and still reject this connection. It is not global convergence of the full64-group model, nor optimization outside the fixed five-writer output span. The observed parent reader barely moves (before/after cosine0.999996/0.999922). Do not extrapolate this local failure to other parents or graph rewrites.
