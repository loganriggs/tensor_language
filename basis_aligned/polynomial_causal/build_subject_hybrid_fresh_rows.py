...
def main():
    ...
    templates=[('relative_called_yesterday','The {s} whom the {a} called yesterday'),
               ('relative_noticed_outside','The {s} that the {a} noticed outside'),
               ('front_station_greeted','Near the station the {s} who greeted the {a}'),
               ('front_office_helped','Outside the office the {s} whom the {a} helped')]
    ...
    old=json.loads((parent/'SUBJECT_JOINT_READER_FRESH_V667_ROWS.json').read_text())
    assert not {r['text'] for r in old}&{r['text'] for r in rows}
    path=parent/'SUBJECT_HYBRID_FRESH_V687_ROWS.json';path.write_text(json.dumps(rows,indent=2)+'\n')
