...
def main():
    ...
    captured=torch.load(P/'CITY_ATTENTION7_DROP3_FINEWEB_V1_ARTIFACT.pt',weights_only=True)['fixtures']
    ...
    with tempfile.TemporaryDirectory(prefix='city-residual6-isolated-') as directory:
        temp=Path(directory)
        for name in ['execute.py','attention7.py','readers.py','attention7.pt','readers.pt','head8.pt']:shutil.copyfile(P/'CITY_ATTENTION7_DROP3_FINEWEB_V1_PACKED_ATTENTION7.pt' if name=='attention7.pt' else source/name,temp/name)
...
r={'passes':max(errors)<=1e-10 and max(outside)==0 and finite and unknown and len(fixtures)==40,
   ...
   'imports_outside_repository':True,'seconds':time.perf_counter()-start,'scope':'Isolated CPU declared-input replay; FineWeb residual6 fixtures and weight-derived fresh token table; packed attention7 and drop3 executable; readers/head8 unchanged; compares generator writes.'}
print(json.dumps(r))
''')
    ...
    (P/'CITY_ATTENTION7_DROP3_FINEWEB_V1_ISOLATED_RESULT.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
