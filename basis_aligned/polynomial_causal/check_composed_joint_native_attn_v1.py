...
from composed_joint_native_attention_v1 import prepare, branch
...
def main():
    ...
    for index in (0,24,48,72,96,112,128,144):
        ...
        context=prepare(z9,m9.double()-b9.mlp.Down_bias.double(),raw0,v1,
                        program,float(b10.lambdas[0]),b10.attn)
        def post(z):
            z=z.float()
    ...
    result=dict(pred_a=max(max(c['state_errors']) for c in cells)<=1e-5,
                ...
                scope='Eight historical prefixes, original two-edit interaction with generated individual and joint states. Pristine context, frozen scalar fields and native weights/suffix retained. FP64 MLP9 response followed by rounded raw10 and native FP32 attention10/MLP10. FineWeb is training-domain text, no fresh OOD claim.')
    (P/'COMPOSED_JOINT_NATIVE_ATTN_V1_RESULT.json').write_text(json.dumps(result,indent=2)+'\n')
