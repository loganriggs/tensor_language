# Task 14 FIT localization v2: repaired physical compiler successor preregistration

**Prospective freeze:** 2026-09-04 UTC. **Status:** CPU-only, outcome-blind repaired compiler candidate. This object can license
...
The predecessor compiler at commit `ea16e22d28d125274ca4353f46e434c2826e0b02` is BLOCKED and must not be used.
Its independent BLOCK review is commit `45db7e2f2e2df3627c594b7df67dc0173aae318b`, exact review SHA-256
`673389c02ec4d7e9122557fe4fb44ab9f90950ccf8e6efbbd310ac6d543548b1`. That review demonstrated that the
predecessor trusted coherently rehashed mutable policy fields and lacked an executable conditional path. This
immutable successor repairs those defects; it is not a silent edit to the blocked object.
...
It additionally binds the BLOCK review above and the prospective producer-acceptance context at parent commit
`ecb37c0ab` / SHA-256 `1724fa6de7ece875cd633976841159302e04033ca008af6e6437ee159a935b46`, addendum
commit `ea50dcfdf` / SHA-256 `c28e6dc2a453a08027673a2420bbf2053e94a0cb02b18a6f0579f747c81a4d96`, and second
addendum commit `3b3920ac8` / SHA-256 `30f11f8a6c4efd8e9dc6e3eb97cbb79bfbdbed21f0d27a622d268002824be18b`.
These documents are design constraints only, not execution authority.
...
The manifest validator compares exact canonical constructors for every static policy section: identity/status,
authority closure, FIT-only scope, initialization, model and boundary semantics, physical batching, runtime and
publication, retained arrays/bytes, scientific thresholds and decisions, spectral diagnostic, DAG, and terminal
precedence. It independently requires the frozen chunk count/root and call-index count/hash and recomputes the entire
conditional price from the supplied chunks. The manifest's internal `contract_sha256` detects accidental corruption;
it is not an authority and cannot make a coherently changed policy valid.
...
Boundary -1 is edited before the block-0 auxiliary state exists: the edited normalized embedding becomes `x0`, and
the block derives `v1` live from that edited `x0`. For boundaries 0–17, the intervention preserves the target-prefix
`x0` and `v1` and edits only the registered residual coordinate. Donor `x0`/`v1` values are never copied. In a
two-site intervention, the later site runs on the trajectory produced by the earlier edit; it cannot restart from a
native target cache. Pure synthetic tripwires in the compiler distinguish all three cases before model access.
...
The manifest contains templates for every possible site chosen by a prior conditional. The compiler defines a typed
`ActivePlanState`; `evaluate_chunk_guard` parses each canonical chunk ID, checks its exact registered guard literal,
and derives the active site, fit configuration, and necessity path. A caller cannot choose arbitrary chunks with
self-consistent booleans. A separate typed `BranchState` and pure `project_terminal` function cover operational
abort, empty H/Q sets, exact joint-rank1 versus selected-family/rank health failures, higher-rank rescue, semantic
failure, single-site necessity, independently selected top-two-Q redundancy, H-before-Q ordering, and reader
pass/fail. Every inapplicable node is explicitly skipped. Single-site and redundancy successes are mutually
exclusive, and exactly one registered scientific terminal—or one operational abort with no scientific terminal—is
produced.

Before model load, runtime must safe-capture the whole binary index, verify its frozen byte count/hash, and regenerate
and compare all `743,881` descriptors. At each active chunk it then seeks and replays that chunk's exact offset/count
slice and root. Every inactive chunk receives a canonical `inactive_skip_zero_calls` receipt, and the ordered receipt
list defines the active-path root. The active path and terminal projection must pass the compiler's compatibility
validator. A runtime activates only the derived chunks. It cannot shorten a 400-step fit, reduce
...
The compact manifest contains exactly `3,821` conditional chunks with frozen root
`073ed886dd051aae2610d1aa771bce6c3012e25dca007c7614283ea9cac732ef` and an ordered call index with
...
checks around every model call. Injected-monotonic-clock pre/post guards make deadline expiry an operational abort
with no scientific terminal or partial package. A call may start only if an independently reviewed throughput receipt shows enough
...
and late races; the receipt is installed last. CPU preflight uses `lstat`, so a dangling symlink is occupied rather
than absent, and performs no namespace creation. Exceptions, deadline expiry, canary failure, source change, checkpoint
...
instrument invalidity. `instrument_invalid` is publishable only when every scheduled call for the exact joint-rank1
or selected-family/rank fit stage completed with finite evidence and its optimizer/seed health failed. Preflight,
hash, schema, nonfinite, incomplete-schedule, deadline, and runtime faults are operational aborts without a scientific
terminal or package. Only the two successful ordered-reader terminals can motivate a separate SELECT design; no
...
| Artifact | SHA-256 |
|---|---|
| compiler source | `6024009bc045200bc3525765dc1dd66261f84f9ccee0dbf9da7b2ddff3101415` |
| call manifest | `5f870a292e9e2db0830156d09d17af10d6d2c8201cb134c80aee12d9261f1b2e` |
| ordered binary call index | `ae399e393d03af9b6232b7fc5339dd892b418ec7c88943735f8b72fc064c8ad9` |
| deterministic dry run | `6cae7b207e372d82c061d189c67bff05bd4772da366d41ebf03a5cdd0c58c0dd` |
| focused adversarial tests | `462610026c34311f1b7a3074446bf9cc3b4c80ee96f89874523e4047b1aded46` |
