...
def main():
 ...
 for i,row in enumerate(rows):
  ...
  reads,rho2=features(z,h,u,a,p);oldreads,oldrho=features(z,h,u,a,old);feature_errors.append(rel(expand(reads,p),old_expand(oldreads,old)));out=scalar(reads,rho2,p);scalar_errors.append(rel(out,old_scalar(oldreads,oldrho,old)));full=predict(z,h,u,a,low['direction'],lam[0],left=low['left'],right=low['right']);reference=runtime.scalar(F.rms_norm(full.float(),(1152,)),ids,native,1);errs.append(rel(out,reference));norms.append(rel(rho2,full.square().mean(-1,keepdim=True)+EPS));base.append(rel(scalar(h@p['readers'].T,h.square().mean(-1,keepdim=True)+EPS,p),cache['scalar'][0,i,:n][None]))
 oldsize=sum(v.numel() for v in old.values() if torch.is_tensor(v));newsize=sum(v.numel() for v in p.values() if torch.is_tensor(v))
 result={'pred_a':max(feature_errors+scalar_errors+norms)<=1e-10,'pred_b':1-newsize/oldsize>=.07,'norm_error':max(norms),'feature_error':max(feature_errors),'scalar_error':max(scalar_errors),'native_baseline_error':max(base),'native_response_error':max(errs),'old_program_scalars':oldsize,'new_program_scalars':newsize,'saved_fraction':1-newsize/oldsize,'seconds':time.perf_counter()-tic,'scope':'Exact key-coordinate reuse within previously shared compiled rank64 directional response. Existing72cachedprompts; does not improve rank64 approximation or add fresh evidence. Expansion at consumer means no peak-memory/speed claim.'}
 torch.save({k:v.clone() for k,v in p.items()},P/'COMPOSED_KEY_SPAN_V1_PROGRAM.pt');(P/'COMPOSED_KEY_SPAN_V1_RESULT.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2))
