"""Saved-state numerical polish of conditional V1; original misses preserved."""
import hashlib
import json
...
def main():
    ...
    prior=json.loads((P/'FOLDED_SUPPORT_CONTINUE_V1_RESULT.json').read_text())
    assert prior['predictions']['pred_a_instrument']
    for item in (prior['source'],prior['cache']):
        assert hashlib.sha256(Path(item['path']).read_bytes()).hexdigest()==item['sha256']
    base=json.loads((P/'FOLDED_SUPPORT_READER_V1_AUDIT.json').read_text())['source']
    assert hashlib.sha256(Path(base['path']).read_bytes()).hexdigest()==base['sha256']
    saved=torch.load(base['path'],weights_only=True,map_location='cpu')
    ...
    for sweep in range(1,101):
        ...
        if converged or time.perf_counter()-fit_start>=70:break
    ...
    output=dict(initial_projected_residuals=initial_residuals,history=history,converged=converged,
        ...
        additional_gain=-independent,predictions=dict(
            ...
            pred_b_convergence=converged,pred_c_negligible_remaining_gain=-independent<=1e-7),
    ...
    path=Path('/dev/shm/bilin18_folded_support_continue_v2.pt');assert not path.exists()
    ...
    (P/'FOLDED_SUPPORT_CONTINUE_V2_RESULT.json').write_text(json.dumps(output,indent=2)+'\n')
