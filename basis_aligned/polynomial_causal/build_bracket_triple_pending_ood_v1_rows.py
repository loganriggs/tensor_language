#!/usr/bin/env python3
"""Freeze a fresh fourth bracket construction without loading the model."""
...
OUT = HERE / "BRACKET_TRIPLE_PENDING_OOD_V1_ROWS.json"
MARKS = {
...
GROUPS = (
    ("observatory log", "comet", "orbit", "spectrum"),
    ("theater ledger", "actor", "costume", "rehearsal"),
    ("harbor bulletin", "vessel", "tide", "berth"),
    ("wildlife survey", "herd", "migration", "sighting"),
    ("ceramics inventory", "glaze", "kiln", "vessel"),
    ("orchard register", "harvest", "variety", "shipment"),
    ("weather archive", "front", "pressure", "outlook"),
    ("railway notice", "platform", "signal", "departure"),
    ("film synopsis", "scene", "character", "ending"),
    ("garden plan", "border", "seedling", "season"),
    ("auction digest", "painting", "bidder", "valuation"),
    ("language workbook", "phrase", "exercise", "translation"),
)
    ...
    for group_index, (document, a, b, c) in enumerate(GROUPS):
        for left, right in UNORDERED:
            ...
            def target_text(inner):
                return (f"The {document} opened {{ a primary record, continued {MARKS[middle]['open']} "
                        f"a secondary note, then nested {MARKS[inner]['open']} the {a}, the {b}, "
                        f"and the {c} remained unresolved")
            ...
            row = {
                "family_id": "triple_pending_stack_top",
                "program_role": "target",
        ...
        for inner in MARKS:
            ...
            row = {
                "family_id": "middle_pending_type_change_inner_fixed",
...
def main():
    ...
    payload = {
        "schema": "bracket_triple_pending_ood_v1_rows",
        "model_loaded": False,
        "outcomes_opened": [],
        "row_count": len(rows),
