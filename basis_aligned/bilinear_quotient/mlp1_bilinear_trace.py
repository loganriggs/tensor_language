...
OUT = PT + 'mlp1_bilinear_trace_results.json'
...
def main():
    ...
    for k in top:
        ...
        if sel > 1.8: nsel += 1
        if bilin: nbilin += 1
        pcls_mean = np.array([np.abs(a[prevcls==c]).mean() if (prevcls==c).any() else 0 for c in range(nc)])
        ptopcls = int(pcls_mean.argmax()); psel = float(pcls_mean[ptopcls]/(pcls_mean.mean()+1e-9))
        driver = 'previous' if psel > 1.4*sel else 'current'
        ...
        units.append({'unit': int(k), 'class_write_mag': round(float(unit_mag[k]), 3),
                      ...
                      'prev_top_class': CLASSES[ptopcls], 'prev_selectivity': round(psel, 2), 'driver': driver,
                      'cvL': round(cvL, 2), 'cvR': round(cvR, 2), 'bilinear': bool(bilin),
                      'top_tokens': toptok})
    n_prevdriven = sum(1 for u in units if u['driver']=='previous')
    out = {'recon_err': round(err, 5), 'n_top_units': NUNIT,
           'n_class_selective': nsel, 'n_bilinear': nbilin, 'n_prev_driven': n_prevdriven,
    ...
    print(f"  prev-driven units: {n_prevdriven}/{NUNIT}", flush=True)
    for u in units[:12]:
        print(f"  unit {u['unit']}: {u['top_class']}(cur sel {u['class_selectivity']}) prev={u['prev_top_class']}({u['prev_selectivity']}) driver={u['driver']} bilin={u['bilinear']} | top {u['top_tokens'][:6]}", flush=True)
