# circuit_greedy2: CORRECTED ACCEPTANCE RULE (S1510: v1 maximized class damage
...
#   pred_a v2 beats top-5 verified selectivity on >= 2 of 4 classes.
#   pred_b median ensemble size <= 5.
#   pred_c digits >= 6x.
...
OUT = PT + 'circuit_greedy2_results.json'
H = m.transformer.h
...
def main():
    ...
    pa = wins >= 2
    pb = statistics.median(sizes) <= 5
    pc = res['digits']['verified_sel'] >= 6
    out = {'res': res, 'wins_vs_top5': wins, 'median_size': statistics.median(sizes),
           'pred_a_beats_top5_2of4': bool(pa), 'pred_b_size_le_5': bool(pb),
           'pred_c_digits_6x': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
