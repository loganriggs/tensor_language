# pronouns_joint: SECOND HEAD+MLP JOINT CIRCUIT — pronouns. (S1576: question
# heads + mlp11 slice compose 99.4% additively; S1575: the pronoun PAYLOAD is
# the positive eigenpart pos_r8 at mlp17, +.111 at zero global cost.) Heads =
# weights-only top-5 parsed from compression_rank2_results.json; slice = the
# pos_r8 payload part (NOT the |lambda| mix, which is net-suppressive). NR=960.
# Late-layer site + lower head selectivity (42x vs 186x) -> expect more
# interference than question.
# Registered predictions:
#   pred_a joint class rise >= .85 x (heads rise + slice rise).
#   pred_b joint global rise <= heads-only global rise + .003.
#   pred_c slice marginal on top of heads >= .05.
...
OUT = PT + 'pronouns_joint_results.json'
NR = 960
H = m.transformer.h
...
def main():
    ...
    order = lam.argsort(descending=True)[:8]   # pos_r8 payload part
    ...
    pa = (cj - c0) >= 0.85 * ((ch - c0) + (cf - c0))
    pb = (gj - g0) <= (gh - g0) + 0.003
    pc = (cj - ch) >= 0.05
    out = {'res': res, 'pred_a_additive_85': bool(pa),
           'pred_b_slice_free': bool(pb), 'pred_c_marginal_05': bool(pc),
