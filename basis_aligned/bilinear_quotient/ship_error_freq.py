# ship_error_freq: THE DAMAGE-vs-FREQUENCY CURVE (S1561). Per-target-token ship
# damage binned by log2 eval-frequency rank (8 bins).
...
#   pred_a mean per-token damage increases monotonically with rarity across the 8
#          bins (allow one inversion).
#   pred_b the rarest bin's per-token damage >= 5x the most frequent bin's.
#   pred_c tokens beyond rank 10000 carry >= .30 of total damage mass.
...
OUT = PT + 'ship_error_freq_results.json'
NR = 1920
...
def main():
    ...
    rise_w = torch.where(tn0 >= 5, ts1 / tn1.clamp_min(1) - ts0 / tn0.clamp_min(1),
                         torch.zeros(50257))
    dmg_w = (rise_w * tn0).clamp_min(0)
    order = ecnt.argsort(descending=True)
    ranks = torch.zeros(50257, dtype=torch.long)
    ranks[order] = torch.arange(50257)
    import math
    bins = {}
    edges = [0, 16, 64, 256, 1024, 4096, 10000, 25000, 50257]
    for b_ in range(8):
        sel = (ranks >= edges[b_]) & (ranks < edges[b_ + 1]) & (tn0 >= 5)
        w = tn0[sel]
        pt = float((rise_w[sel] * w).sum() / w.sum().clamp_min(1)) if int(sel.sum()) else 0.0
        bins[f'rank_{edges[b_]}_{edges[b_ + 1]}'] = {
            'per_token_damage': round(pt, 4),
            'mass_share': round(float(dmg_w[sel].sum() / dmg_w.sum()), 3),
            'n_tokens': int(sel.sum())}
        print(b_, bins[f'rank_{edges[b_]}_{edges[b_ + 1]}'], flush=True)
    pts = [v['per_token_damage'] for v in bins.values()]
    inversions = sum(1 for i in range(7) if pts[i + 1] < pts[i])
    pa = inversions <= 1
    pb = pts[-1] >= 5 * max(pts[0], 1e-6)
    beyond = sum(v['mass_share'] for k, v in bins.items()
                 if int(k.split('_')[1]) >= 10000)
    pc = beyond >= 0.30
    out = {'bins': bins, 'inversions': inversions,
           'mass_beyond_10k': round(beyond, 3),
           'pred_a_monotone_1inv': bool(pa), 'pred_b_rarest_5x': bool(pb),
           'pred_c_tail_30': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
