"""Registered in §1115: extend the skeleton-vs-tail split to ALL TEN deep MLPs (L5-14) simultaneously.
§1115 (3 layers): skeleton carries 78% of the content read, tail 12%. If the same holds band-wide, the WHOLE
content band's per-layer reads are 8-feature-sufficient and only the residual STREAM is genuinely high-rank
(reconciling §1042/§1051 full-rank with a low-rank read law). Same conditions: noop | skel (tail removed) |
tail (skeleton removed) | fullrem; CE + rare/freq.

REGISTERED PREDICTIONS:
  (0) SANITY: noop ~0; near-additive partition; fullrem cost > the 3-layer version's 0.043 (more readers hit).
  (a) SKELETON LAW BAND-WIDE: skel cost < 0.3x fullrem AND tail cost > 0.6x fullrem at all ten layers at once
      -> the content machine's read interface is ~8 stable features everywhere; the high rank is a property
      of the STREAM (accumulation), not of any reader;
  (b) if skel cost jumps toward fullrem band-wide (compounding through ten readers), the skeleton is only
      per-layer-sufficient and the tail matters cumulatively (the §1051 starvation reappears at read level;
      report plainly)."""
...
OUT = PT + 'content_skeleton_band_results.json'
NSEQ = 192; SEQ = 256; REF = list(range(5, 15)); K = 64; NATOM = 256; TOPK = 8; STEPS = 3000; RARE_MAX = 2
H = m.transformer.h
SUB = {'mode': None}
ST = {}
CUR = {}
