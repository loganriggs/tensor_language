"""CAPSTONE: HOW MUCH OF THE WHOLE MODEL DO WE UNDERSTAND? (user's core question). Replace EVERY component's
...
OUT = PT + 'whole_model_understanding_results.json'
NEVAL = 120; SEQ = 256; CONTENT_L = 15; K = 12; RTOK = 64; RPOS = 32
NLAYER = 18
...
def main():
    ...
    for t in TAGS:
        ...
        tt = table(O, tok_i, nu); r1 = O - tt[tik]
        Utop, _ = mean_subspace(r1, topic_i, K-1); top_term = (r1 @ Utop) @ Utop.T
    ...
    REPL['mode'] = 'off'; ce_full = ce_pass(blocks)
    REPL['mode'] = 'mean'; ce_mean = ce_pass(blocks)
    ...
    for lv in ['token', 'token+topic', 'token+topic+prev', 'shuftoken']:
        REPL['level'] = lv; ce = ce_pass(blocks)
