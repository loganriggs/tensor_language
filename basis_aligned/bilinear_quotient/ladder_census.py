"""LADDER CENSUS -- 400: the relay closure held on every head
tested (2.5: 0.999; 5.5: 0.837/0.860; 8.4 nested: 0.841/0.859).
Now the maximal one-code claim, all nine induction heads at once:
rebuild the residual with EVERY attention write below the head
replaced by its ITERATED RELAY (real patterns, values from the
growing reconstructed chain -- attention only ever moves ladder
code; MLP writes real). If this predicts every head's pattern,
nothing in any induction match code is non-ladder content.
Arms: ladder (no attention at all), iter_all (relays at every
block), shuf (iter_all with row-shuffled value sources; null).
REGISTERED PREDICTIONS (hit = argmax agreement at match
positions):
  (a) iter_all >= 0.90 for the early band 1.4, 2.5, 3.5, 3.8;
  (b) iter_all >= 0.75 for the deep band 5.5, 6.5, 7.3, 8.3, 8.4;
  (c) null: shuf's lift over ladder < 25% of iter_all's lift,
      averaged over heads;
  (d) report: rank corr per head (expect >0.6 early)."""
...
OUT=PT+'ladder_census_results.json'
NR=16
HEADS=[(1,4),(2,5),(3,5),(3,8),(5,5),(6,5),(7,3),(8,3),(8,4)]
EARLY=['1.4','2.5','3.5','3.8']; DEEP=['5.5','6.5','7.3','8.3','8.4']
...
def main():
    ...
    res={f'{li}.{hd}':{a:{'hit':0,'n':0,'corr':[]} for a in
         ('ladder','iter_all','shuf')} for li,hd in HEADS}
    for i in range(0,NR,4):
        ...
        def build(mode):
            """mode: 'ladder' (no attn), 'iter_all', 'shuf'."""
            xr=E.clone(); out={}
            for lj in range(maxli+1):
                ...
                if lj<maxli:
                    if mode!='ladder':
                        xr=xr+a_relay(lj,xr,mode=='shuf')
                    xr=xr+mout[lj]
            return out
        codes={a:build(a) for a in ('ladder','iter_all','shuf')}
        for li,hd in HEADS:
        ...
        for a,st in arms.items():
            outj[k][a]={'hit':round(st['hit']/max(st['n'],1),3),
                        'corr':round(sum(st['corr'])
                                     /max(len(st['corr']),1),3)}
        print(f"{k}: ladder {outj[k]['ladder']['hit']} | iter_all "
              f"{outj[k]['iter_all']['hit']} (corr "
              f"{outj[k]['iter_all']['corr']}) | shuf "
              f"{outj[k]['shuf']['hit']}",flush=True)
    pa=all(outj[k]['iter_all']['hit']>=0.90 for k in EARLY)
    pb=all(outj[k]['iter_all']['hit']>=0.75 for k in DEEP)
    fr=[]
    for k in outj:
        lift=outj[k]['iter_all']['hit']-outj[k]['ladder']['hit']
        sl=outj[k]['shuf']['hit']-outj[k]['ladder']['hit']
        if lift>0.02: fr.append(sl/lift)
    pc=(sum(fr)/max(len(fr),1))<0.25
    out={'heads':outj,'pred_a':bool(pa),'pred_b':bool(pb),
         'pred_c':bool(pc),
         'shuf_lift_fraction':round(sum(fr)/max(len(fr),1),3)}
    for nm,v in (('a','early band iter_all >=0.90'),
                 ('b','deep band iter_all >=0.75'),
                 ('c','shuf lift <25% of iter lift')):
        print(f"({nm}) {v}: "
