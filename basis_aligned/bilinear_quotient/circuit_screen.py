# circuit_screen: MANY CIRCUITS AT ONCE, TWO DISCOVERY METHODS, GRADED (user
...
#   pred_a >= 6 of 10 classes yield a selective circuit (class rise >= 2x global)
#          under at least one method.
#   pred_b the two methods pick overlapping ensembles (median Jaccard >= .25) —
#          they see the same circuits.
#   pred_c the DATA baseline wins or ties on >= 5 classes (honest expectation:
#          data-driven attribution is at least as good; the weights-only method's
#          value is not needing forward passes).
...
OUT = PT + 'circuit_screen_results.json'
NR = 480
...
def main():
    ...
    jac = []
    for cn in CLS:
        a = set(res[cn]['W']['heads']); b = set(res[cn]['B']['heads'])
        jac.append(len(a & b) / len(a | b))
    import statistics
    medj = statistics.median(jac)
    b_wins = sum(1 for cn in CLS
                 if res[cn]['B']['rise_class'] >= res[cn]['W']['rise_class'])
    pa = n_sel >= 6
    pb = medj >= 0.25
    pc = b_wins >= 5
    out = {'res': res, 'n_selective_2x': n_sel, 'median_jaccard': round(medj, 3),
           'baseline_wins': b_wins,
           'pred_a_6of10_selective': bool(pa), 'pred_b_overlap_25': bool(pb),
           'pred_c_baseline_5': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"selective {n_sel}/10 jaccard {medj:.2f} baseline_wins {b_wins}")
