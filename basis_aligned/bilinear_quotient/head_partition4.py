# head_partition4: CUT 4 — identity removal in BOTH coordinate systems. §1293 showed a
# per-position rank-1 mask along the raw v1 direction strips exactly the lambda-broadcast
# term (0.125 = cut 1's 0.122) because each layer's c_v RE-ENCODES block-0's stream write
# into its own coordinates. Here, per source position and head, remove the projection of
# the mixed value vv onto span{ v1 direction, c_v^L(block-0 stream write) direction }
# (Gram-Schmidt, rank <= 2). Null: the same removal at position-shuffled directions.
#
# Registered predictions:
#   pred_a TWO COORDINATES SUFFICE (main4): rank-2 removal >= 60% of main4's whole-head
#          induction damage (0.395) with elsewhere <= 40% of whole-head else (0.0605).
#   pred_b SHUFFLE NULL CLEAN: <= 25% of the aligned version (main4).
#   pred_c SECOND COORDINATE IS REAL: main4 rank-2 >= 2x the rank-1 anchor (0.125).
...
OUT = PT + 'head_partition4_results.json'
NR = 384; W = 128
H = m.transformer.h
...
def fwd(idx, which=None, mode=None, perm=None):
    """mode 'aligned': per position remove vv's projection onto span{v1 dir, c_v^L(d0)
    dir} (d0 = block-0 attention's stream write); 'shuffled': same at permuted positions."""
    x = F.rms_norm(m.transformer.wte(idx), (D,)); x0 = x; v1 = None; d0 = None
    B = idx.shape[0]
    for L, blk in enumerate(H):
        ...
        sel = selmask(L, which)
        if sel.any() and mode is not None and d0 is not None:
            u1 = v1v.float(); u2 = at.c_v(d0).view(B, T, 9, 128).float()
            if mode == 'shuffled':
                u1 = u1[:, perm]; u2 = u2[:, perm]
            u1 = u1 / u1.norm(dim=-1, keepdim=True).clamp_min(1e-6)
            u2 = u2 - (u2 * u1).sum(-1, keepdim=True) * u1
            u2 = u2 / u2.norm(dim=-1, keepdim=True).clamp_min(1e-6)
            vf = vv.float()
            comp = ((vf * u1).sum(-1, keepdim=True) * u1 +
                    (vf * u2).sum(-1, keepdim=True) * u2).to(vv.dtype)
...
def main():
    ...
    out = {'n_rows': NR, 'n_targets': int(TGT.sum()),
           ...
           'pred_c_second_coordinate': bool(pc),
           'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"pred_a identity {pa} | pred_b null {pb} | pred_c second-coord {pc}")
