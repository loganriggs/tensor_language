"""A5 ANOMALY -- v9 found replacing the certified induction owner attn5
with a class dictionary IMPROVES the assembly by 0.38 nats. Which part of
the dictionary does the work? Fit the full v9 stack, then evaluate the
v9-best context (a8 excluded) with attn5 as:
  dict  -- the class dictionary (v9-best itself);
  real  -- the actual attn5;
  zero  -- attn5 output zeroed (pure removal);
  mean  -- one global mean output, class-blind.
REGISTERED PREDICTIONS:
  (a) dict <= real - 0.2 (replicates the negative marginal);
  (b) zero < real (the real component actively harms on the substituted
      stream -- slack-in-context);
  (c) dict <= mean - 0.1 (class conditioning carries signal beyond mere
      quieting; if this FAILS the improvement is just removal)."""
...
OUT=PT+'a5_anomaly_results.json'
...
def main():
    ...
    S['a5z']=('attnz',5)
    S['a5m']=('attnm',5,S['a5'][2].mean(0))
    best=[n for n in order if n!='a8']
    base=evalC([],'oracle')
    arms={}
    arms['dict']=evalC(best,'oracle')-base
    arms['real']=evalC([n for n in best if n!='a5'],'oracle')-base
    arms['zero']=evalC([('a5z' if n=='a5' else n) for n in best],
                       'oracle')-base
    arms['mean']=evalC([('a5m' if n=='a5' else n) for n in best],
                       'oracle')-base
    for k,v in arms.items(): print(f'a5={k:5s} {v:+.4f}',flush=True)
    pa=arms['dict']<=arms['real']-0.2
    pb=arms['zero']<arms['real']
    pc=arms['dict']<=arms['mean']-0.1
    out={'base':round(base,4),
         'arms':{k:round(v,4) for k,v in arms.items()},
         'pred_a':bool(pa),'pred_b':bool(pb),'pred_c':bool(pc)}
    print(f"(a) dict beats real by >=0.2: {'HELD' if pa else 'FAILED'}")
    print(f"(b) zero < real (slack-in-context): "
          f"{'HELD' if pb else 'FAILED'}")
    print(f"(c) dict beats class-blind mean by >=0.1: "
