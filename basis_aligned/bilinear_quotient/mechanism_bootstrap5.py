"""MECHANISM BOOTSTRAP v5 -- the PATTERN-SIDE rung: for attention-probed
leaves, in a 2-token fold sequence the attention output at position 1
IS the pattern-composed value read (only the previous token is
attendable), so the bigram fold capturing at ATTENTION modules (layers
0-17) yields each pair's attention-path content; project onto the
leaf's attention-pca directions, rank pairs -- weights+architecture
only, member-blind. Scope: leaves with >=2 attention-pca probes.
REGISTERED PREDICTIONS:
  (a) >=4 tested leaves pass precision >=3x base at recall >=0.3;
  (b) random corpus-pair null <=1.2x (median);
  (c) trigger pairs printed."""
...
OUT=PT+'mechanism_bootstrap5_results.json'
CA=300; NB=78
MHL=list(range(2,10))
T=256
...
def main():
    ...
    FOLDC={}
    capsF={li:[] for li in range(18)}
    hsF=[]
    for li in range(18):
        def mkf(li=li):
            def h(mo,i_,o_):
                y=o_[0] if isinstance(o_,tuple) else o_
                capsF[li].append(y.detach()[:,1].float())
            return h
        hsF.append(m.transformer.h[li].attn.register_forward_hook(mkf()))
    ...
    print(f"(a) >=4 pass: {'HELD' if pa else 'FAILED'}")
