"""CROSS-MODEL CLASS-SHARPENING (DE-SHARED, settles 789 gpt2 confound) (does 782's "the first MLP COMPUTES/sharpens
...
OUT = PT + 'cross_model_class_whiten_results.json'
...
def fisher_deshared(V, lab):
    # remove the top principal direction (massive-activation / shared) before Fisher (782 robust metric)
    Vc = V - V.mean(0, keepdim=True)
    U = torch.linalg.svd(Vc, full_matrices=False)[2]
    Vd = Vc - (Vc @ U[:1].T) @ U[:1]
    return fisher(Vd, lab)
...
def run(model_id):
    ...
    fmd = fisher_deshared(Mk, labk); fed = fisher_deshared(Ek, labk)
    ...
    return {'d': d, 'n_tokens': len(kept), 'n_labelled': len(ki), 'eff_rank_mean': round(eff_rank(M), 2),
            ...
            'deshared_fisher_mean': round(fmd, 4), 'deshared_fisher_emb': round(fed, 4),
            'deshared_ratio': round(fmd/max(fed, 1e-9), 3)}
...
def main():
    ...
    for mid in MODELS:
        try:
            r = run(mid); out[mid] = r
            print(f'{mid}: {r["n_labelled"]} labelled | RAW Fisher ratio {r["fisher_ratio"]} (m {r["fisher_mean"]}/e {r["fisher_emb"]}) | DE-SHARED ratio {r["deshared_ratio"]} (m {r["deshared_fisher_mean"]}/e {r["deshared_fisher_emb"]})', flush=True)
        except Exception as e:
            out[mid] = {'error': str(e)[:200]}; print(f'{mid} FAILED: {e}', flush=True)
    ok = [mid for mid in out if 'fisher_ratio' in out[mid]]
    pa = len(ok) > 0 and all(out[mid]['deshared_ratio'] >= 1.2 for mid in ok)
    out['pred_a_deshared_class_sharpen_general'] = bool(pa); out['runtime_s'] = time.time()-t0
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f'\n(a) first MLP sharpens class cross-model on the DE-SHARED metric (>=1.2x both): {pa}', flush=True)
