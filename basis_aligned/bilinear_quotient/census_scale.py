"""CENSUS SCALE -- swarm prerequisite: rebuild the damage-tree
census at 2x corpus (the 212 fit rows + 212 never-seen fresh rows)
and save census_state_424.pt. Also the first cross-instance identity
measurement: match old leaves to new leaves by member-overlap
Jaccard restricted to the shared 212-row grid (LESSON 7: tags don't
transfer; overlap does).
REGISTERED PREDICTIONS:
  (a) >=80 leaves at 2x corpus (68 at 1x);
  (b) >=60% of old leaves have a Jaccard>=0.5 match in the new tree
      (circuits are corpus-stable objects, not window artifacts);
  (c) census_state_424.pt saved with rows/basev/members/scores."""
...
OUT=PT+'census_scale_results.json'
CA=300; NB=106
MHL=list(range(2,10))
T=256
...
def main():
    ...
    rows=torch.cat([FW[CA:CA+212],cl9.fresh_rows(212)])
    ...
    torch.save({'rows':rows,'basev':base.cpu(),
               ...
               PT+'census_state_424.pt')
    for lf in leaves:
        for k_ in ('_member','_score','_slice'): lf.pop(k_,None)
    for md in roots: md.pop('_member',None)
    json.dump(packs,open(PT+'circuit_tree_packs_424.json','w'),indent=1)
    ...
    print(f'\nleaves {nleaf} | new-structure {nnew} | child repl rate '
          f'{crate:.2f}')
    pa=nleaf>=80
    print(f"(a) >=80 leaves at 2x corpus: {'HELD' if pa else 'FAILED'}")
    # cross-instance identity: Jaccard on the shared 212-row grid
    old=torch.load(PT+'census_state.pt',map_location='cpu')['leaves']
    OLDGRID=212*256
    matched=0; pairs=[]
    newst=torch.load(PT+'census_state_424.pt',
                     map_location='cpu')['leaves']
    for ol in old:
        A=set(int(x) for x in ol['member'])
        bestJ=0; bestT=None
        for lf2 in newst:
            B=set(int(x) for x in lf2['member'] if int(x)<OLDGRID)
            if not B: continue
            J=len(A&B)/max(len(A|B),1)
            if J>bestJ: bestJ,bestT=J,lf2['tag']
        pairs.append({'old':ol['tag'],'new':bestT,'J':round(bestJ,3)})
        if bestJ>=0.5: matched+=1
    frac=matched/max(len(old),1)
    pb=frac>=0.6
    out['identity_pairs']=pairs
    out['identity_frac']=round(frac,3)
    print(f'cross-instance identity: {matched}/{len(old)} '
          f'({frac:.0%}) at J>=0.5')
    print(f"(b) >=60% matched: {'HELD' if pb else 'FAILED'}")
