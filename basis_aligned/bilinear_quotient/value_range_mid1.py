# value_range_mid1: §1186 drill-down — L5-9 one layer at a time @W64 (pos-0 visible) + joint.
# Registered: (a) DISTRIBUTED: max single layer <= 0.03; (b) singles sum <= mid1 joint
# (super-additive); (c) L5 NOT special once pos-0 visible (482's spike was pure sink):
# L5 single <= 1.5x the median single.
#
...
OUT = PT + 'value_range_mid1_results.json'
...
def main():
    ...
    BANDS = {'base': set(), 'L5': {5}, 'L6': {6}, 'L7': {7}, 'L8': {8}, 'L9': {9},
             'mid1': set(range(5, 10))}
    ...
    singles = [cost[f'L{L}'] for L in range(5, 10)]
    med = sorted(singles)[2]
    out = {'n_rows': NR, 'W': WIN, 'ce': CE, 'cost_vs_base': cost,
           'singles_sum': round(sum(singles), 4),
           'sanity': bool(abs(CE['base'] - CE['true_model']) <= 0.005),
           'pred_a_distributed': bool(max(singles) <= 0.03),
           'pred_b_superadditive': bool(sum(singles) <= cost['mid1']),
           'pred_c_L5_not_special': bool(cost['L5'] <= 1.5 * max(med, 1e-6)),
           'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"CE {CE}")
    print(f"costs {cost} | singles sum {out['singles_sum']} vs joint {cost['mid1']}")
    print(f"sanity {out['sanity']} | pred_a distributed {out['pred_a_distributed']} | pred_b superadd {out['pred_b_superadditive']} | pred_c L5 {out['pred_c_L5_not_special']}")
