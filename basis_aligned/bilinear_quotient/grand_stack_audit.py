# grand_stack_audit: §1184's 0.0385 at 2x FRESH rows (§1101 replication law).
# Registered: (a) 0.0385 +/- 0.012; (b) sub-additive vs parts sum.
...
OUT = PT + 'grand_stack_audit_results.json'
NR = 32; W = 128
...
def main():
    ...
    ROWS = cl.fineweb_rows(2 * NR)[NR:][:, :T + 1].contiguous()   # fresh draw (audit, §1101)
    ...
    out = {'n_rows': NR, 'W': W, 'ce': CE, 'grand_cost': cost, 'parts_sum': round(PARTS_SUM, 4),
           ...
           'pred_a_grand_cheap': bool(abs(cost - 0.0385) <= 0.012),
