"""v3: CLASS-CONTROLLED topic metric fixing §958's class confound. topic metric = gain on SOURCE-topic tokens
MINUS gain on BASE-topic tokens (both content words) at QP; the generic content-word boost from a class patch
cancels, isolating CONTENT-SPECIFIC topic control. Only pairs with different source/base topics are used.
REGISTERED PREDICTION (a): content_only and joint have POSITIVE class-controlled topic_net (content shifts
prediction toward source topic vs base topic) while class_only ~ 0 (class patch boosts both topics equally) and
random ~ 0 -> clean content-specific topic control + clean dissociation (class moves class, content moves topic).
...
OUT = PT + 'joint_interchange_v3_results.json'
...
def main():
    ...
    agg = {c: {'class_to_source': 0, 'topic_gain': 0.0, 'n': 0} for c in conds}
    def dset(topic):  # tensor of source-topic distinctive token ids
    ...
    for (bi, si) in pairs:
        ...
        src_topic = int(seq_topic[si]); base_topic = int(seq_topic[bi])
        Dsrc = dset(src_topic); Dbase = dset(base_topic)
        if Dsrc is None or Dbase is None or src_topic == base_topic: continue
        PATCH['on'] = False; lp0 = F.log_softmax(forward_logits(idx).float()[0, QP], -1)
        base_src = float(lp0[Dsrc].mean()); base_bas = float(lp0[Dbase].mean())
        for c, U in conds.items():
            PATCH['U'] = U; PATCH['vec'] = (src @ U) @ U.T; PATCH['on'] = True
            lg = forward_logits(idx).float()[0, QP]; PATCH['on'] = False
            lp = F.log_softmax(lg, -1); pc = predclass(lg)
            # CLASS-CONTROLLED topic metric: gain on SOURCE-topic tokens MINUS gain on BASE-topic tokens
            # (both content words; the generic content-word boost cancels, isolating content-specific topic shift)
            topic_net = (float(lp[Dsrc].mean()) - base_src) - (float(lp[Dbase].mean()) - base_bas)
            agg[c]['class_to_source'] += int(pc == src_cls_class)
            agg[c]['topic_gain'] += topic_net
            agg[c]['n'] += 1
    hh.remove()
    out = {'QP': QP, 'conditions': {}}
    for c in conds:
        a = agg[c]; out['conditions'][c] = {'pred_class_matches_source': round(a['class_to_source']/a['n'], 3),
                                            'topic_gain_nats_source': round(a['topic_gain']/a['n'], 4)}
        print(f"{c:>13}: class->source {out['conditions'][c]['pred_class_matches_source']} | topic-gain(nats, source) {out['conditions'][c]['topic_gain_nats_source']:+.4f}", flush=True)
    j = out['conditions']['joint']; r = out['conditions']['random']; co = out['conditions']['content_only']; cl_ = out['conditions']['class_only']
    # class-controlled metric: content should move topic MORE than class does (dissociation), class moves class
    out['pred_a_compositional'] = bool(j['pred_class_matches_source'] > r['pred_class_matches_source'] + 0.05 and
                                       co['topic_gain_nats_source'] > cl_['topic_gain_nats_source'] and
                                       co['topic_gain_nats_source'] > 0.05)
