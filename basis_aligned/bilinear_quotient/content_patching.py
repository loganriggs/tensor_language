...
OUT = PT + 'content_patching_results.json'
NEVAL = 160; SEQ = 256; REF_LAYERS = [8, 10, 12]; ABL = list(range(6, 15)); K = 256
...
def main():
    ...
    R = torch.linalg.qr(torch.randn(D, K, generator=g, device=DEV))[0]   # random-subspace control
    del caps, devsum, dev, devc

    # pair sources / targets
    n = blocks.shape[0] // 2; src = blocks[:n].contiguous(); tgt = blocks[n:2*n].contiguous()
    kl_c = kl_r = al_c = al_r = 0.0; npos = 0
    for i in range(0, n, 8):
        si = src[i:i+8].to(DEV)[:, :-1].contiguous(); ti = tgt[i:i+8].to(DEV)[:, :-1].contiguous()
        if si.shape[0] != ti.shape[0]: continue
        # source residuals (aligned to target positions)
        ST['mode'] = 'cap'; ST['store'] = {}; ls = fwd(si).float(); ST['mode'] = None
        srcres = {L: ST['store'][L] for L in ABL}
        ST['mode'] = None; lb = fwd(ti).float()               # target baseline
        base = F.log_softmax(lb, -1)
        lsrc = F.log_softmax(ls, -1)
        for U_, name in ((U, 'c'), (R, 'r')):
            ST['mode'] = 'patch'; ST['U'] = U_; ST['srcres'] = srcres
            lp = fwd(ti).float(); ST['mode'] = None
            patch = F.log_softmax(lp, -1)
            kl = (patch.exp() * (patch - base)).sum(-1)        # KL(patch||base) per pos
            dpatch = lp - lb; dsrc = ls - lb                   # movement vs source-direction (logit space)
            cos = F.cosine_similarity(dpatch.reshape(-1, V), dsrc.reshape(-1, V), dim=-1)
            if name == 'c': kl_c += float(kl.sum()); al_c += float(cos.sum())
            else: kl_r += float(kl.sum()); al_r += float(cos.sum())
        npos += si.shape[0] * si.shape[1]
    out = {'K': K, 'abl_range': [ABL[0], ABL[-1]], 'n_positions': npos,
           'content': {'kl': round(kl_c/npos, 4), 'alignment_to_source': round(al_c/npos, 4)},
           'random_subspace': {'kl': round(kl_r/npos, 4), 'alignment_to_source': round(al_r/npos, 4)}}
    out['pred_a_content_carries_topic'] = bool(out['content']['alignment_to_source'] > 0.05 and
                                               out['content']['alignment_to_source'] > 1.5*out['random_subspace']['alignment_to_source'] and
                                               out['content']['kl'] > out['random_subspace']['kl'])
    out['runtime_s'] = round(time.time()-t0, 1)
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"content:  KL {out['content']['kl']} | align->source {out['content']['alignment_to_source']}", flush=True)
    print(f"random:   KL {out['random_subspace']['kl']} | align->source {out['random_subspace']['alignment_to_source']}", flush=True)
    print(f"pred_a content carries topic: {out['pred_a_content_carries_topic']} | wrote {OUT} ({out['runtime_s']}s)")
