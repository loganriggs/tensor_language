# capitalized_mid_adds: WHICH LATE-MID DOES THE REGISTER PIPELINE NEED? (§1406:
# tabling mlp10-15 costs capitalized −.096 while HELPING the other three families.)
# Single-adds: keep-{4,6,7,8,9}+L for L in 10..15, scored on all families + else.
# Assumptions registered: same unified kit, same refit machinery; adds evaluated
# one at a time (no pairs — §1404 non-additivity noted, marginals only).
#
# Registered predictions (one-sided):
#   pred_a capitalized's top add is mlp14 or mlp15 (adjacent to the committee/register).
#   pred_b keep5 + top add lifts capitalized to >= 0.78 recovery.
#   pred_c the top add costs comparative <= 0.02 (no interference with the .9886).
...
OUT = PT + 'capitalized_mid_adds_results.json'
NFITT = 960
V = 50257
NMEAN = 24; NR = 1920
...
def main():
    ...
    for arm in ('full', 'ymean'):
        ...
        print(f"{arm}: " + " ".join(f"{k} {r[k]:.3f}" for k in MASKS) + f" | else {e:.4f}",
              flush=True)
    KEEP5 = {4, 6, 7, 8, 9}
    sets = [('keep5', KEEP5)] + [(f'keep5+{L}', KEEP5 | {L}) for L in range(10, 16)]
    ...
    adds = {L: table[f'keep5+{L}']['capitalized'] - table['keep5']['capitalized']
            for L in range(10, 16)}
    top = max(adds, key=lambda L: adds[L])
    pa = top in (14, 15)
    pb = table[f'keep5+{top}']['capitalized'] >= 0.78
    pc = (table['keep5']['comparative'] - table[f'keep5+{top}']['comparative']) <= 0.02
    out = {'ce': res, 'recovery_table': table,
           'cap_adds': {str(L): round(v, 4) for L, v in adds.items()}, 'top_add': int(top),
           'pred_a_top_14_15': bool(pa), 'pred_b_cap_78': bool(pb),
           'pred_c_comp_safe': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"cap adds {adds} top mlp{top}")
