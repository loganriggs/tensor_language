"""[FRESH-DATA + CLASS-BACKOFF: rare tokens (<MINCOUNT train) back off to their class mean, removing the §902 coverage confound so the generalizing number is not penalized by unseen tokens] certification of §900 CAPSTONE: HOW MUCH OF THE WHOLE MODEL DO WE UNDERSTAND? (user's core question). Tables/subspaces BUILT on the first 70% of rows, CE EVALUATED on the held-out 30% — certifies the 0.81 is not same-data optimism. Replace EVERY component's
...
OUT = PT + 'whole_model_understanding_fresh_backoff_results.json'
...
def main():
    ...
    ucls = torch.tensor(np.array([_clsf(_dd(int(t))) for t in uniq]), device=DEV); MINCOUNT=12
    ...
    for t in TAGS:
        ...
        tt = table(O[trmask], tok_i[trmask], nu)
        # class-backoff for rare tokens (train counts)
        ...
        _Otr = O[torch.tensor(trmask, device=DEV)]
        _cm.index_add_(0, ucls[_tik_tr], _Otr); _cc.index_add_(0, ucls[_tik_tr], torch.ones(_Otr.shape[0], device=DEV)); _cm = _cm/_cc.clamp_min(1).unsqueeze(1)
