"""MLP0 UNITS 600-900 -- how far down does mlp0's clean nameable
structure extend? 600 found the units ranked 300-600 cluster just as
cleanly as the top-300 (ARI 0.562 vs 0.58) with three NEW nameable
circuits (newline, pronoun, number-word), correcting the coverage
picture. This continues one band deeper (ranks 600-900) to bracket
where the clean structure finally decays -- if 600-900 still clusters
cleanly, mlp0 has many more circuits than the top-300 framing implied;
if it decays here, we have bracketed the layer's structured extent.

REGISTERED PREDICTIONS:
  (0) SANITY: exact by construction (h @ Down.T IS the real output);
  (a) STABILITY: report the split-half ARI against the trend so far
      (top-300 0.58, 300-600 0.562). Prediction: it stays > 0.1
      (still real) but is LOWER than 0.562 -- a decay as importance
      drops. Whether it holds above 0.4 (still substantial) or falls
      toward 0.1-0.3 (decaying) is the finding;
  (b) CONCENTRATION: report whether any of the 3 largest clusters
      reaches >= 5/8 single-class concentration;
  (c) NEW OR REPEATED? (no bar): read the top clusters' examples. Are
      they clean and NEW (more distinct circuits), clean but REPEATS
      of known classes, or degraded/mixed (structure decaying)?
  NULL: shuffling sample alignment drops the stability ARI to < 0.1."""
...
OUT = PT + 'mlp0_units_600_900_results.json'
NFRESH = 64
NSAMP = 4000
TOPK = 300
NCLUST = 20
...
def main():
    ...
    topk = order[2 * TOPK:3 * TOPK].numpy()  # ranks 600-900
