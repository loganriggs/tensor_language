# circuit_mlp_units3: CLASS-CONDITIONAL ACTIVATION SCORING (S1565: bilinear units
# are signed; weight ranking fails both directions). Unit score = (mean h_u |
# class positions - mean h_u overall) x (u_class . Down_col_u), 96 fit rows.
# Ablate the top-64 positive contributors at the best MLP.
# Original: THE SIGNED VERSION (S1563: |.|-ranking mixed promoters and
...
#   pred_a top-64 data-scored contributors' ablation RAISES class CE >= .10 at
#          >= 2 of 4 classes.
#   pred_b selectivity >= 2x at those (MLP-unit circuit membership).
#   pred_c top-64 carries >= .50 of positive class contribution at >= 2.
...
OUT = PT + 'circuit_mlp_units3_results.json'
NR = 960
H = m.transformer.h
...
def main():
    ...
    for cname, mask_v in CLS.items():
        ...
        conc = float(contrib[prom].clamp_min(0).sum()
                     / contrib.clamp_min(0).sum().clamp_min(1e-9))
        ...
        sels[cname] = sel; ratios[cname] = conc; layers_used[cname] = L
        res[cname] = {'mlp': L, 'rise_class': round(c1 - c0, 4),
                      'rise_global': round(g1 - g0, 4),
                      'selectivity': round(sel, 2),
                      'top64_concentration': round(conc, 3)}
        print(cname, res[cname], flush=True)
        json.dump({'partial': True, 'res': res}, open(OUT, 'w'), indent=1)
    for hk in hooks:
        hk.remove()

    pa = sum(1 for cn in CLS if res[cn]['rise_class'] >= 0.10) >= 2
    pb = sum(1 for cn in CLS
             if res[cn]['rise_class'] >= 0.10 and sels[cn] >= 2) >= 2
    pc = sum(1 for cn in CLS if ratios[cn] >= 0.50) >= 2
    out = {'res': res, 'pred_a_rise_2of4': bool(pa),
           'pred_b_selective_2of4': bool(pb), 'pred_c_concentrated': bool(pc),
