# mlp_ngram_complete: the REMAINING eight MLPs (6,8,10,11,13,14,16,17) @W=64, each alone —
# closes §1183's sampled map to all 18 layers ('every MLP is a window function' with no gaps).
# Registered predictions:
#   pred_a all eight <= 0.02;
#   pred_b readout MLPs 16/17 also cheap (<= 0.01 — their token-baseline term is local);
#   pred_c no context-hungry MLP anywhere (max <= 0.03).
#
...
OUT = PT + 'mlp_ngram_complete_results.json'
NR = 16; W = 128
...
MLG = {L: {'on': False, 'idx': None, 'cache': None} for L in (6, 8, 10, 11, 13, 14, 16, 17)}
MLW = {L: 64 for L in (6, 8, 10, 11, 13, 14, 16, 17)}
...
def main():
    ...
    hkL = [m.transformer.h[Lm].mlp.register_forward_hook(mk_mlpL_hook(Lm)) for Lm in LAYERS8]
    ...
    names = [f'm{Lm}ng' for Lm in (6, 8, 10, 11, 13, 14, 16, 17)]
    ...
    out = {'n_rows': NR, 'W': W, 'ce': CE, 'cost_vs_base': cost,
           ...
           'pred_a_all_cheap': bool(all(v <= 0.02 for v in vals)),
           'pred_b_readout_cheap': bool(cost['m16ng'] <= 0.01 and cost['m17ng'] <= 0.01),
           'pred_c_none_hungry': bool(max(vals) <= 0.03),
           'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"CE {CE}")
    print(f"costs {cost}")
    print(f"sanity {out['sanity_base_matches_true']} | pred_a cheap {out['pred_a_all_cheap']} | "
          f"pred_b readout {out['pred_b_readout_cheap']} | pred_c none-hungry {out['pred_c_none_hungry']}")
