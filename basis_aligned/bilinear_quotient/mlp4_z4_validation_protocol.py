...
def load_and_validate(path=PATH):
    ...
    for filename, expected in p["pinned_external_sources"].items():
        assert sha(HERE/filename) == expected, filename
    ...
    assert "paired frozen row" in p["metrics"]["uncertainty_unit"]
    assert p["metrics"]["confidence_intervals_are_descriptive_not_gates"]
    assert not p["metrics"]["source_document_ids_retained_by_freezer"]
    assert not p["metrics"]["source_document_independence_verified"]
    assert not p["metrics"]["inferential_corpus_coverage_claim"]
    assert not p["data"]["fit_rows_may_be_opened"]
    assert not p["data"]["combined_rows_may_be_opened"]
    assert not p["data"]["ood_may_be_opened"]
    assert p["checkpoint"]["cache_only"]
