# closer_pipe_test: SWARM LEAD (§1387 wave-2 worker A) — 13.8 appeared in selection
# regime at a closing '|' in a templated row. Is the PIPE the closer's THIRD delimiter
# surface? §1340 battery at a13, pipe targets (next token contains '|'; no parity — table
# pipes open and close, registered simplification).
#
# Registered predictions:
#   pred_a THIRD SURFACE: 13.8 solo damage >= 0.10 nats at pipe targets AND >= 5x its
#          elsewhere damage.
#   pred_b OWNERSHIP: 13.8 >= 60% of a13's pipe damage and both directions agree on it.
#   pred_c surgical: 13.8's elsewhere <= 10% of its pipe-target damage.
...
OUT = PT + 'closer_pipe_test_results.json'
NMEAN = 24; NR = 1920
L13 = 13
H = m.transformer.h
LAYERS = (13,)
...
def main():
    ...
    print(f"pipe ids {len(q)}", flush=True)
    ...
    TARGET = torch.isin(tgt_all, q_ids)   # no parity: pipes open AND close
    TARGET[:, :64] = False
    ...
    d8 = solo[8]
    conc8 = d8['target'] / max(abs(d8['else']), 1e-4)
    pa = d8['target'] >= 0.10 and conc8 >= 5.0
    pb = (d8['target'] >= 0.60 * max(layer_dmg, 1e-4)) and top_solo == 8 == top_keep
    pc = abs(d8['else']) <= 0.10 * max(d8['target'], 1e-4)
