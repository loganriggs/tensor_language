# circuit_greedy6: THE RECIPE ON SCREEN-4'S REMAINING CLASSES (days 4.6x, titles
# 7.5x, about 4.3x, ordinals 2.7x at top-5).
...
#   pred_b median ensemble size <= 3.
#   pred_c >= 1 single-head circuit among them.
...
OUT = PT + 'circuit_greedy6_results.json'
H = m.transformer.h
    ...
    def rx(pat):
        ...
        for t in range(V):
            if re.match(pat, ENC.decode([t])):
                v[t] = True
        return v
    return {'days': rx(r'^ (Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)$'),
            'titles': rx(r'^ (Mr|Mrs|Dr|Ms|Prof)$'),
            'about': rx(r'^ (about|approximately|around|roughly)$'),
            'ordinals': rx(r'^ (first|second|third|fourth|fifth|last|next)$')}
...
def main():
    ...
    TOP5_VERIFIED = {'days': 4.56, 'titles': 7.45, 'about': 4.34,
                     'ordinals': 2.66}
    res = {}
    sizes = []
    ...
    pa = wins >= 4
    pb = statistics.median(sizes) <= 3
    pc = min(sizes) == 1
    out = {'res': res, 'wins_vs_top5': wins, 'median_size': statistics.median(sizes),
           'pred_a_all4_beat': bool(pa), 'pred_b_size_le_3': bool(pb),
           'pred_c_one_single_head': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
