# attn_lowrank_r16: PRICE OPTIMIZATION OF THE WINNING CLASS (S1474: whitened r32 =
# median .937 at 23.6 Mbit/layer. Halve the rank: r16 = 11.8 Mbit/layer — how much
# fid survives? Same 11 layers, same whiteners.
# (S1472: roster-live + whitened r32 hit .92/.93 at a5/a8; S1469 hit .89-.92 at
# 10/13/14. The layers still on plain kernels or worse: 0,1,2,3,4,6,7,9,11,12,15 —
# kernel fids .21-.85). Whitened per-head rank-32, NO roster (these layers have no
# named specialists), one layer at a time, fid vs frozen anchors.
...
#   pred_a median r16 fid >= .85 (half price keeps the class viable).
#   pred_b every layer still beats its kernel.
#   pred_c median fid drop from r32 <= .06.
...
OUT = PT + 'attn_lowrank_r16_results.json'
NR = 960
are = sys.modules[type(m.transformer.h[0].attn).__module__].apply_rotary_emb
H = m.transformer.h
LAYERS = [0, 1, 2, 3, 4, 6, 7, 9, 11, 12, 15]
...
def main():
    ...
    import statistics
    KREF = {0: .8471, 1: .8165, 2: .678, 3: .6232, 4: .8109, 6: .5314, 7: .6018,
            9: .692, 11: .4172, 12: .2057, 15: .2793}
    R32 = {0: .9553, 1: .9639, 2: .9358, 3: .9097, 4: .9709, 6: .9367, 7: .9464,
           9: .9584, 11: .8443, 12: .8868, 15: .8638}
    med16 = statistics.median([fids[f'attn{L}_r16'] for L in LAYERS])
    med_drop = statistics.median([R32[L] - fids[f'attn{L}_r16'] for L in LAYERS])
    pa = med16 >= 0.85
    pb = all(fids[f'attn{L}_r16'] > KREF[L] for L in LAYERS)
    pc = med_drop <= 0.06
    out = {'ce': res, 'fid_opt': fids, 'median_r16': round(med16, 4),
           'median_drop_from_r32': round(med_drop, 4), 'kernel_refs': KREF,
           'mbits_per_layer': {'r16': 11.8, 'r32': 23.6, 'kernel': 0.037},
           'pred_a_med16_85': bool(pa), 'pred_b_all_beat_kernel': bool(pb),
           'pred_c_drop_le_06': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"med16 {med16:.4f} drop {med_drop:.4f}")
