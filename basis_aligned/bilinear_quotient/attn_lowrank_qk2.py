# attn_lowrank_qk2: INPUT-WHITENED per-head truncation (S1467: plain SVD went 0-for-3
...
#   pred_a whitened attn5 r32 >= 0 (at least fixes the sign; plain was -1.61).
#   pred_b whitened median r32 >= .85 (the original bar, now with the right metric).
#   pred_c whitened median r8 >= .50.
...
OUT = PT + 'attn_lowrank_qk2_results.json'
NR = 960
...
def main():
    ...
    out = {'ce': res, 'fid_opt': fids, 'median_r32': round(med32, 4),
           ...
           'mbits_per_layer': {'r8': 5.9, 'r32': 23.6, 'full_qk': 85.0,
                               'kernel': 0.037},
           'pred_a_attn5_r32_ge0': bool(pa), 'pred_b_med32_85': bool(pb),
           'pred_c_med8_50': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
