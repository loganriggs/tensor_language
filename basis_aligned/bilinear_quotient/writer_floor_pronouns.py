# writer_floor_pronouns: IS A *DIFFUSENESS* CLAIM ALSO UNINFORMATIVE?
# S1610 showed a CONCENTRATION claim is null: S1597 reported the question slice's
# top-4 carrying .718, and a matched-rank RANDOM basis scores .7223 while the
# certified slice scores .5946. S1598 makes the mirror-image claim at
# pronouns@mlp17: top-6 = {mlp16, x0, mlp15, mlp9, mlp14, attn9} carrying .482,
# and its pred_a (share >= .55) FAILED, concluding "WRITERS DIFFUSE".
# If a random rank-8 basis also lands near .48, then "diffuse" is as uninformative
# as "72% in four" was -- you cannot read either off a share without the null.
#
# Same harness as writer_floor_question, retargeted: pronouns @ mlp17, rank-8
# |lambda| (S1598's exact rule) vs matched-rank random, TOP=6 to match S1598's
# reported statistic. Local curated_rows.pt, 3 disjoint 333-row chunks
# (435/410/400 pronoun positions -- far above the question cell's 105/127/102).
# Rows are census (NOT fresh): shares are not comparable ACROSS corpora to
# S1598's .482, but every registered bar is a within-run arm-vs-arm comparison
# on identical rows.
#
# Registered predictions:
#   pred_a the share statistic carries no information here either: |random top-6
#          share - lambda top-6 share| <= .10, in >= 2 of 3 samples.
#   pred_b the S1606 floor shows up in the published set: >= 2 of S1598's six
#          named writers also appear in the RANDOM top-6, in 3/3 samples.
#   pred_c but membership is not entirely floor: >= 2 of S1598's six are ABSENT
#          from the random top-6 in 3/3 samples (the mirror of S1610, where 3 of
#          4 survived).
...
OUT = PT + 'writer_floor_pronouns_results.json'
NR = 960
SITE = 17
RANK = 8
...
# S1598's .482 figure.  What survives intact is every REGISTERED bar: pred_a,
...
SITE_Q = 17
RANK_Q = 8
QPAT = r'^ (he|she|they|He|She|They)$'
PUBLISHED = ['mlp16', 'x0', 'mlp15', 'mlp9', 'mlp14', 'attn9']   # S1598's named top-6
TOP = 6
...
def main():
    ...
    print(f"question@mlp{SITE_Q} rank-{RANK_Q} |lambda| eigs "
          f"{[round(float(x), 3) for x in lam_l]}  (S1598 ref: top-6 share .482, called DIFFUSE)",
          flush=True)
    ...
    for name, (V2, lam2) in arms.items():
        ...
        for skip in range(CHUNKS):
            ...
            per.append({'top': top, 'share': round(share, 4), 'class_n': r['class_n'],
                        'recon': r['recon_rel_err']})
            print(f"  {name:7s} skip={skip:6d} n={r['class_n']:3d} top={top} "
                  f"share={share:.4f} recon={r['recon_rel_err']:.1e}", flush=True)
        res[name] = per

    # consensus top-K per arm (rank-weighted across samples)
    def consensus(per):
        cnt = {}
        for p in per:
            for idx, c in enumerate(p['top']):
                cnt[c] = cnt.get(c, 0) + (TOP - idx)
    ...
    lam_shares = [p['share'] for p in res['lambda']]
    rnd_shares = [p['share'] for p in res['random']]
    lam_share = sum(lam_shares) / len(lam_shares)
    rnd_share = sum(rnd_shares) / len(rnd_shares)
    gaps = [abs(r - l) for l, r in zip(lam_shares, rnd_shares)]

    pub = set(PUBLISHED)
    in_rand = [len(pub & set(res['random'][k]['top'])) for k in range(CHUNKS)]
    absent = [len([c for c in PUBLISHED if c not in set(res['random'][k]['top'])])
              for k in range(CHUNKS)]

    pa = sum(1 for g in gaps if g <= 0.10) >= 2
    pb = all(v >= 2 for v in in_rand)
    pc = all(v >= 2 for v in absent)

    print(f"\nS1598 published top-6: {PUBLISHED}  (share .482, called DIFFUSE)")
    print(f"  of those, IN random top-{TOP} per sample: {in_rand}")
    print(f"  of those, ABSENT from random top-{TOP}:   {absent}")
    print(f"  share gaps |random - lambda|: {[round(g, 4) for g in gaps]}")
    print(f"  mean share: lambda {lam_share:.4f} vs random {rnd_share:.4f}")
    print(f"  consensus lambda {C['lambda']}")
    print(f"  consensus random {C['random']}")
    print(f"  overlap {overlap}/{TOP} | floor-corrected lambda -> {corrected}")
    ...
    out = {'config': {'class': 'question', 'site': SITE_Q, 'rank': RANK_Q,
                      ...
                      'S1598_reference': {'top6': PUBLISHED, 'share': 0.482,
                                          'verdict': 'DIFFUSE (its pred_a >=.55 FAILED)'}},
           'arms': res,
           'consensus': C, 'floor': sorted(floor),
           'lambda_top_floor_corrected': corrected,
           'overlap_lambda_random_ofTOP': overlap,
           'mean_share': {'lambda': round(lam_share, 4), 'random': round(rnd_share, 4)},
           'published_in_random': in_rand, 'published_absent_from_random': absent,
           'share_gaps': [round(g, 4) for g in gaps], 'S1598_published': PUBLISHED,
           'predictions': {'pred_a_share_gap_le10_2of3': bool(pa),
                           'pred_b_ge2_published_in_random_3of3': bool(pb),
                           'pred_c_ge2_published_absent_3of3': bool(pc)},
