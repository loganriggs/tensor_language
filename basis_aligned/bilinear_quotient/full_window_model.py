...
OUT = PT + 'full_window_model_results.json'
NR = 24; WS = [16, 32, 64, 128]; QSTART = 128; STRIDE = 2
...
def main():
    ...
    out = {'n_rows': NR, 'scored_positions': f'{QSTART}..{T} stride {STRIDE}', 'ce': CE,
           ...
           'pred_b_w64': bool(cost['W64'] <= 0.15),
           'pred_c_w128': bool(cost['W128'] <= 0.05),
           'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"base CE {CE['base']} | costs {cost}")
    print(f"pred_a monotone {out['pred_a_monotone']} | pred_b W64 {out['pred_b_w64']} | pred_c W128 {out['pred_c_w128']}")
