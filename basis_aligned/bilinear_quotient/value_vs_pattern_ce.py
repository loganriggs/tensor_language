...
OUT=PT+'value_vs_pattern_ce_results.json'
NR=32
PAIRS=[(6,3,0),(4,7,2),(6,5,7),(4,1,5)]
...
def main():
    ...
    c3=out['6.3']
    pa=(abs(c3['patswap']['dce_match'])<=0.02 and
        c3['valswap']['dce_match']>=0.8*c3['zero']['dce_match'])
    c7=out['4.7']
    pb=c7['patswap']['dce_match']<c7['valswap']['dce_match']
    others=[out['6.5'],out['4.1']]
    pc=all(o['patswap']['dce_match']<o['valswap']['dce_match']
           for o in others)
    pd=all(out[k]['zero']['dce_match']>0 for k in out)
    out.update({'pred_a':bool(pa),'pred_b':bool(pb),
                'pred_c_general':bool(pc),'pred_d':bool(pd)})
    for nm,v in (('a','a6.h3 replicates (pattern free, values cost)'),
                 ('b','a4.h7 same ordering'),
                 ('c_general','unnamed pairs same ordering'),
