"""[v3] CONTINUOUS-topic gap fill: §896 found the 12-cluster topic TABLE fills the middle little (+0.03-0.09); topic is CONTINUOUS (§874), so 12 discrete cells undercount it. Replace the discrete topic table with a keep-only projection onto the topic SUBSPACE (rank K-1, continuous) and re-measure how much topic fills — vs a random-subspace null of the same rank. FILL THE UNDERSTANDING GAPS with our named variables (user directive). §893: middle content MLPs are only
...
OUT = PT + 'layer_understanding_v3_results.json'
...
def main():
    ...
    def build_standins(O):
        ...
        # continuous topic SUBSPACE from r1's topic-cluster-mean deviations (rank K-1)
        Utop, _ = mean_subspace(r1, topic_i, K-1)
        gdev = torch.Generator(device=DEV).manual_seed(0); Urnd = torch.linalg.qr(torch.randn(D, K-1, generator=gdev, device=DEV))[0]
        top_term = (r1 @ Utop) @ Utop.T
        rnd_term = (r1 @ Urnd) @ Urnd.T
        r2 = r1 - top_term; pv = table(r2, prev_i, nu)
        s_tok = tt[torch.tensor(tok_i, device=DEV)]
        s_tt = s_tok + top_term
        s_ttp = s_tt + pv[torch.tensor(prev_i, device=DEV)]
        s_tsh = s_tok + rnd_term
        gm = O.mean(0)
