"""MODE DICTIONARY v2 -- hierarchical, not override (321's lesson): the
10 hand classes stay untouched except the two CONTEXTUAL classes (ind=8,
other=9), which are each SPLIT by the token's top root-mode among the 4
strongest modes -> labels 8/9 become (8,mode) and (9,mode): 10-2+2*4=16
labels... implemented as 10 base + 8 split labels = 18.
REGISTERED PREDICTIONS:
  (a) hierarchical recovery >= 10-class + 2 points;
  (b) shuffle (mode assignment permuted within the split classes)
      <= 10-class + 0.5;
  (c) per-split-label populations reported."""
...
OUT=PT+'mode_dictionary2_results.json'
...
def main():
    ...
    def labels(sc,base_cls):
        lab=base_cls.clone()
        best4=sc[:,:4].abs().argmax(1)
        for bc in (8,9):
            sel=base_cls==bc
            lab[sel]=10+(bc-8)*4+best4[sel]
        return lab
    ...
    print(f"(a) hierarchical >= +2pts: {'HELD' if pa else 'FAILED'}")
    print(f"(b) shuffle <= +0.5pt: {'HELD' if pb else 'FAILED'}")
