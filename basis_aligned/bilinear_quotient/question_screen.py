# question_screen: BEHAVIOUR-FIRST screen for INTERROGATIVE-STATE tracking — predicting
# "?" at sentence end when the sentence BEGAN with a question word. Like the delimiter
# circuit, this needs a carried state ("this sentence is a question"), but the opener is
# syntactic (a WH-word), not a paired symbol.
#
# Target: positions whose next token is a question-mark token AND the current sentence
# (since the last ./!/? token) begins with an interrogative opener (Who/What/When/Where/
# Why/How/Which/Is/Are/Do/Does/Did/Can/Could/Will/Would/Should). Controls per SOP: jitter
# +-3 and random matched.
#
# Registered predictions:
#   pred_a A CARRIER EXISTS: concentration >= 3 with jitter <= 1.5.
#   pred_b LATE-ATTENTION BET: the winner is attention at L >= 10 (state-tracking siblings
#          13.8/attn13 pattern; registered direction).
...
OUT = PT + 'question_screen_results.json'
NMEAN = 24; NR = 192
H = m.transformer.h
...
def main():
    ...
    print(f"?-ids {len(qm)} | sent-end {len(sent_end)} | wh {len(wh)}", flush=True)
    ...
    tgt_all = EVR[:, 1:]
    toks = EVR[:, :-1]
    is_end = torch.isin(toks, sent_end)
    is_wh = torch.isin(toks, wh)
    B2, T2 = toks.shape
