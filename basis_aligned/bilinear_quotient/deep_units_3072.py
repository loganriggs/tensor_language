# deep_units_3072: THE K=3072 POINT (S1539 law: log-linear until saturation ~.9).
# All 14 deep MLPs, K = 3072 (171 Mbit).
...
#   pred_a median K=3072 >= .85.
#   pred_b every module >= .75.
#   pred_c saturating modules (16, 17) gain <= .04 from K=2048 (the saturation law).
...
OUT = PT + 'deep_units_3072_results.json'
...
def main():
    ...
    K2048_REF = {4: .8582, 5: .8326, 6: .8576, 7: .7797, 8: .8031, 9: .7874,
                 10: .7456, 11: .768, 12: .7471, 13: .7305, 14: .6455, 15: .7019,
                 16: .912, 17: .9384}
    import statistics
    pa = statistics.median([RECS[L][3072] for L in TARGETS]) >= 0.85
    pb = all(RECS[L][3072] >= 0.75 for L in TARGETS)
    pc = all(RECS[L][3072] - K2048_REF[L] <= 0.04 for L in (16, 17))
    out = {'res': res, 'recs': {str(L): RECS[L] for L in TARGETS},
           'k2048_ref': K2048_REF,
           'pred_a_med_85': bool(pa), 'pred_b_all_75': bool(pb),
           'pred_c_saturation': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
