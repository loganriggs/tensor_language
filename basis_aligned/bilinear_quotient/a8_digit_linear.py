"""A8 RESCUE -- a8 (attn8, the digit owner) is the one dead rung: dict
+0.40 marginal, mean rescue failed. Its certified digit function is
contextual counting, which a per-class CONSTANT cannot carry. Rescue:
a8 dictionary with the digit class moved to the LINEAR arm (per-class
ridge read of the substituted stream at the site).
REGISTERED PREDICTIONS (evaluated in the round-2 best config, +2.749):
  (a) adding a8-digit-linear costs <= +0.10 marginal (rung viable -> 35/36);
  (b) it beats re-adding the plain a8 dict by >= 0.1."""
...
OUT=PT+'a8_digit_linear_results.json'
...
def main():
    ...
    for k in links:
        ...
        LW[k]=torch.linalg.solve(Xk.T@Xk+l2*torch.eye(D,device=DEV),
                                 Xk.T@Yk)
    S['a8dl']=('attnd2',8,CV,LW,links)
    S['a8d']=('attnd2',8,CV,LW,list(LINK))
    base=evalC([],'oracle')
    ...
    def with8(nm):
        ...
        for n in best:
            cand.append(n)
            if n=='c8m': cand.insert(-1,nm)
        return evalC(cand,'oracle')-base
    cdl=with8('a8dl'); cd=with8('a8d')
    print(f'best (no a8) {cost:+.4f} | +a8 digit-linear {cdl:+.4f} | '
          f'+a8 plain dict {cd:+.4f}')
    pa=(cdl-cost)<=0.10; pb=(cd-cdl)>=0.1
    out={'base':round(base,4),'no_a8':round(cost,4),
         'a8_digit_linear':round(cdl,4),'a8_plain_dict':round(cd,4),
         'pred_a':bool(pa),'pred_b':bool(pb)}
    print(f"(a) digit-linear marginal <= +0.10: {'HELD' if pa else 'FAILED'}")
    print(f"(b) beats plain dict by >=0.1: {'HELD' if pb else 'FAILED'}")
