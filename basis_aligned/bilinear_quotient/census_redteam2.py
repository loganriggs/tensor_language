"""CENSUS RED-TEAM ROUND 2 -- sign-aware joint-causal certification
(323: the round-1 bar was sign-blind; mode scores are signed and
several leaves showed 24x SIGNED effects scored as failures). For each
of the 20 blind-nameable leaves: members split by mode-score sign;
joint ablation of the leaf's implicated components must (i) move
pos-score members and neg-score members in OPPOSITE directions
consistent with their signs, (ii) pooled |member effect| >= 2x the
|effect| on base-CE-matched non-members, (iii) >=70% member-level sign
agreement (damage sign matches score sign).
REGISTERED PREDICTION: >=60% of tested leaves pass all three. If this
fails, the causal-unit claim for the census is genuinely in trouble."""
...
OUT=PT+'census_redteam2_results.json'
CA=300; NB=78
MHL=list(range(2,10))
T=256
...
def main():
    ...
    for lf in leaves:
        ...
        sl=lf['_slice']; sc_=lf['_score']
        thr=sc_.abs().quantile(0.85)
        memmask=sc_.abs()>=thr
        mem=sl[memmask]; msign=torch.sign(sc_[memmask])
        md=dmg[mem]
        agree=float((torch.sign(md)==msign).float().mean())
        pooled=float(md.abs().mean())
        mb=basev[mem]
        lo,hi=mb.quantile(0.1),mb.quantile(0.9)
        mset=set(mem.tolist())
        cand=torch.tensor([i for i in range(ntok) if i not in mset])
        candb=basev[cand]
        sel=cand[(candb>=lo)&(candb<=hi)]
        ctl=float(dmg[sel].abs().mean())
        pos_mean=float(md[msign>0].mean()) if (msign>0).any() else 0.0
        neg_mean=float(md[msign<0].mean()) if (msign<0).any() else 0.0
        opp=(pos_mean>0 and neg_mean<0) or \
            ((msign>0).all() and pos_mean>0) or \
            ((msign<0).all() and neg_mean<0)
        ok=opp and pooled>=2*max(ctl,1e-4) and agree>=0.7
        caus.append({'tag':lf['tag'],'pos_mean':round(pos_mean,3),
                     'neg_mean':round(neg_mean,3),
                     'pooled_abs':round(pooled,3),
                     'ctl_abs':round(ctl,3),
                     'sign_agree':round(agree,2),'pass':bool(ok)})
        tested+=1
        if ok: passed+=1
        print(f"causal2 {lf['tag']}: pos {pos_mean:+.3f} neg "
              f"{neg_mean:+.3f} pooled {pooled:.3f} ctl {ctl:.3f} "
              f"agree {agree:.2f} {'PASS' if ok else 'fail'}",flush=True)
    pdx=(passed>=0.6*tested) if tested else False
    out={'causal_tested':tested,'causal_passed':passed,'causal':caus,
         'pred_signaware':bool(pdx),
         'n_distinct':ndistinct,'sign_mixed':nmix}
    print(f"sign-aware causal {passed}/{tested}: "
          f"{'HELD' if pdx else 'FAILED'}")
