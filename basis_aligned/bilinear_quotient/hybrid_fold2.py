"""HYBRID + FOLD v2 -- the correct merge. v1 hot-swapped fold tables
into a stack whose every downstream piece was fit under the empirical
front (+3.76: the sequential-fitting rule, violated). v2 installs the
fold tables INSIDE the sequential chain: m0/m2/m3 become fold+absorber
rungs before anything downstream is fit, so the attention dictionaries,
tail spans, and gains all fit under the fold front.
REGISTERED PREDICTIONS (unchanged):
  (a) merged config <= +2.35 on fresh rows;
  (b) beats the empirical hybrid (+2.543) by >= 0.15;
  (c) window-C value reported."""
...
OUT=PT+'hybrid_fold2_results.json'
...
def main():
    ...
    # fold tables computed FIRST (weights-only)
    FOLD={}
    capsF={0:[],2:[],3:[]}
    hsF=[]
    for li in (0,2,3):
    ...
    S['m0']=fit_foldres(Y,X,ids,0); order.append('m0')
    ...
    S['m2']=fit_foldres(Y,X,ids,2); order.append('m2')
    S['a3']=fit_attnd(3,order); order.append('a3')
    Y,X,ids=runA(order,m.transformer.h[3].mlp)
    S['m3']=fit_foldres(Y,X,ids,3); order.append('m3')
    for li in MIDL:
    ...
    def evalHF(active,motif):
        ...
        return float(torch.cat(ces).mean())
    base=evalHF([],False)
    hyF=evalHF(hyb,True)-base           # fold hybrid (fits in-chain)
    hy=2.5432                           # empirical hybrid reference (303)
    baseC2=evalH([],False)
    hyFC=evalH(hyb,True)-baseC2
    print(f'fresh: empirical hybrid {hy:+.4f} | FOLD hybrid {hyF:+.4f} '
          f'| window C fold-hybrid {hyFC:+.4f}',flush=True)
    pa=hyF<=2.35; pb=(hy-hyF)>=0.15
    pc=True
    out={'base':round(base,4),'hybrid_emp':round(hy,4),
         'hybrid_fold':round(hyF,4),'hybrid_fold_C':round(hyFC,4),
         'n_heads_swapped':nsw,
         'pred_a':bool(pa),'pred_b':bool(pb)}
    print(f"(a) fold hybrid <= +2.35: {'HELD' if pa else 'FAILED'}")
    print(f"(b) beats empirical by >=0.15: {'HELD' if pb else 'FAILED'}")
