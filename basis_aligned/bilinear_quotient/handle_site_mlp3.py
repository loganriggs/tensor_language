# handle_site_mlp3: FOURTH POINT OF THE SITE MAP (S1494: mlp0 4.3x/1.2, mlp1
# 1.0/1.3, mlp2 1.7/1.9). mlp3 (fid .95, own-basis rank-256 structure) closes the
# front-stack map: basis = block-4 composed reads of h3; same classes and protocol.
...
#   pred_a w8 frag-keep >= .20 at mlp3 (7000).
#   pred_b selectivity (either basis) >= 1.5 at mlp3 (front sites stay class-
#          structured; mlp1 was the exception, not the rule).
...
OUT = PT + 'handle_site_mlp3_results.json'
NR = 960
...
def main():
    ...
    Wd1 = H[3].mlp.Down.weight.float().to(DEV)
    at2 = H[4].attn
    STACK = torch.cat([at2.c_q.weight.float().to(DEV) @ Wd1,
                       ...
                       H[4].mlp.Left.weight.float().to(DEV) @ Wd1,
                       H[4].mlp.Right.weight.float().to(DEV) @ Wd1], 0)
    ...
    for i in range(0, 480, 8):
        ...
        mo = (h1.to(H[3].mlp.Down.weight.dtype) @ H[3].mlp.Down.weight.T).float() \
            + H[3].mlp.Down_bias.float()
        mo_acc += mo.sum(0)
        if i < 240:
            hs.append(h1[::7].cpu())
    MU1 = a1s / n0
    ...
    out = {'res': res,
           ...
           'pred_a_frag_keep_20': bool(pa), 'pred_b_site_selective_15': bool(pb),
