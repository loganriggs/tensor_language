"""ASSEMBLED v6 -- THE ATTENTION RUNG: v5 (19/36 at +2.10) plus tail
attention 10-17 replaced by the section-247 dictionary materials, fit
sequentially under the whole v5 front:
  - structure classes (digit, bclose, sentend, comma, name, rep): per-class
    output CONSTANTS (247 showed 67-85% recoverable there);
  - contextual classes (newline, subword, ind, other): per-class ridge
    LINEAR reads of the (substituted) stream at the same position.
Labels: ORACLE arm uses the target-token classifier (fitting always uses
oracle labels); DEPLOY arm computes labels from a stream probe fit at
attn10's input (first surviving attention rung), like the tail dictionary.
REGISTERED PREDICTIONS:
  (a) >=4 of the 8 attention rungs survive the adaptive drop rule
      (oracle LOO marginal <= +0.4);
  (b) v6-best (oracle) <= +3.0 total;
  (c) a14 -- the tail attention component that is harmful-on-average
      (deletion-improves, section 233-235) -- has oracle marginal <= +0.10;
  (d) deploy-vs-oracle gap reported for v6-full and v6-best.
Adaptive rule (pre-registered): any attention rung with oracle LOO
marginal > +0.4 is dropped; v6-best re-evaluated on the survivors."""
...
OUT=PT+'assembled_v6_results.json'
...
def main():
            ...
            if kind=='cv':
                ...
                for li in TAILC:
                    ...
                    hs.append(m.transformer.h[li].mlp
                              .register_forward_hook(h))
        return hs
    def runA(active, cap_mod, tup=False):
        Ys=[]; Xs=[]; Ids=[]
        hs=install(active)
        def capf(mod,i_,o_):
    ...
    for li in ATT:
        ...
        for k in LINK:
            ...
            LW[k]=torch.linalg.solve(Xk.T@Xk+l2*torch.eye(D,device=DEV),
                                     Xk.T@Yk)
        nm=f'a{li}'
        S[nm]=('attnd',li,CV,LW,Wp2); order.append(nm)
        print(f'fit {nm} (n per const class '
              f'{[int((flatA==k).sum()) for k in CONSTK]})',flush=True)
    ...
    print(f'v6 full (oracle) {full:+.4f}',flush=True)
    marg={}
    for li in ATT:
        nm=f'a{li}'
    ...
    v6b=evalC(best,'oracle')-base if drop else full
    fullD=evalC(order,'probe')-base
    bestD=evalC(best,'probe')-base if drop else fullD
    surv=8-len(drop)
    pa=surv>=4; pb=v6b<=3.0; pc=marg['a14']<=0.10
    ncomp=len(best)-1+8-1  # front 11 + mids 6 + surviving attn + tail 8 comps
    out={'base':round(base,4),'v6_full_oracle':round(full,4),
         'att_marginals':{k:round(v,4) for k,v in marg.items()},
         'dropped':drop,'v6_best_oracle':round(v6b,4),
         'v6_full_deploy':round(fullD,4),'v6_best_deploy':round(bestD,4),
         'pred_a':bool(pa),'pred_b':bool(pb),'pred_c':bool(pc)}
    print(f'base {base:.3f} | v6 full {full:+.4f} | dropped {drop} | '
          f'v6-best {v6b:+.4f} | deploy full {fullD:+.4f} '
          f'best {bestD:+.4f}')
    print(f"(a) >=4/8 rungs survive ({surv}/8): {'HELD' if pa else 'FAILED'}")
    print(f"(b) v6-best <= +3.0: {'HELD' if pb else 'FAILED'}")
    print(f"(c) a14 marginal <= +0.10: {'HELD' if pc else 'FAILED'}")
