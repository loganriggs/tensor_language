"""FOLD-BASIS LADDER -- user direction: the early layers are the
best-understood part of the model (direct weight folds from the
embedding through attn0/mlp0/mlp1). Use them as BASIS FEATURES for
later circuits: fold each early MLP (0-3) over the vocabulary
(mlp_li(rms_norm(wte))), take the top-8 PCA directions of the fold
table, and add per-position above-median scores as 32 new library
predicates (shifted copies come free via the licensed SHIFT
operator). Then rerun the until-dry ladder. Baseline: cl2 converged
at 57/118.
REGISTERED PREDICTIONS:
  (a) converged programmable total >=65 of 118 (+8 over cl2);
  (b) shuffled-label null median <=0.6;
  (c) >=half of the passes gained over 57 cite a fold_ predicate."""
...
OUT=PT+'fold_basis_results.json'
CA=300; NB=78
MHL=list(range(2,10))
T=256
...
def main():
    ...
    # ---- fold-basis features from early-layer weight folds ----
    W_E=m.transformer.wte.weight.detach().float()
    XE=F.rms_norm(W_E,(W_E.shape[1],))
    for li9 in (0,1,2,3):
        mlp9=m.transformer.h[li9].mlp
        pdt=next(mlp9.parameters()).dtype
        outs9=[]
        for i9 in range(0,XE.shape[0],4096):
            xb=XE[i9:i9+4096].to(DEV).to(pdt)
            outs9.append(mlp9(xb).float().cpu())
        TB=torch.cat(outs9); TB=TB-TB.mean(0)
        g9f=torch.Generator().manual_seed(3)
        sub=torch.randperm(TB.shape[0],generator=g9f)[:20000]
        _,_,Vh9=torch.linalg.svd(TB[sub],full_matrices=False)
        SCR=TB@Vh9[:8].T
        vals=SCR[flat]
        for k9 in range(8):
            v9=vals[:,k9]
            LIB0[f'fold_m{li9}_d{k9}']=(v9>=v9.median())
        print(f'fold features m{li9} added',flush=True)
    LIB=dict(LIB0)
    ...
    nf=sum(1 for p in allp if 'fold_' in p)
    gainN=total-57
    pc=gainN>0 and nf>=max(1,gainN//2)
    out['pred_c']=bool(pc); out['n_fold_cited']=nf; out['gain_vs_57']=gainN
    print(f'fold-cited passes: {nf} | gain vs cl2: {gainN:+d}')
    print(f"(a) converged >=65: {'HELD' if pa else 'FAILED'}")
    print(f"(b) null <=0.6: {'HELD' if pb else 'FAILED'}")
    print(f"(c) fold cites >= half the gain: {'HELD' if pc else 'FAILED'}")
