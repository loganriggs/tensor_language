# extraction_v4: GOAL-1 RUNG 4 — THE PRINTABLE INDUCTION MACHINE. Compose the two proven
# pieces: (i) the route-grain skeleton (§1316: closure 33 heads + lambda-v1 route through
# all others = 79% of the induction gap), and (ii) the weights-computed stand-in for the
# four circuit heads (§1258-60: match scores from raw codes -> affine -> verdict axis;
# payload = successor's v1 code through 8.3/8.4's c_proj slices). In rung 4 the quad heads
# are READ-MASKED inside the skeleton (window-limited patterns + route-only values) and
# replaced by the computed operations. What remains live is: front band + sink + routes +
# means + CODE. Benchmark rows: W64 repeats (the stand-in program's standard), CE on
# q in [128, 256).
#
# Conditions: full | skeleton (quad live; rung-3 anchor on these rows) | skel_mask (quad
# read-masked, no stand-in) | skel_axis (axis only) | skel_standin (axis + payload) |
# skel_wrong (payload from argmax+2 — control).
#
# Registered predictions:
#   pred_a STAND-IN WORKS IN THE SKELETON: skel_standin recovers >= 50% of the
#          skel_mask -> skeleton gap.
#   pred_b PRINTABLE MODEL PRICE: skel_standin's damage vs full <= 2x skeleton's damage
#          vs full (coding the circuit costs at most one more skeleton-price).
#   pred_c WRONG-SUCCESSOR CONTROL: skel_wrong <= skel_axis recovery + 5 points.
...
OUT = PT + 'extraction_v4_results.json'
...
def main():
    ...
    pa = rec['skel_standin'] >= 0.50
    pb = d_stand <= 2 * max(d_skel, 1e-4)
    pc = rec['skel_wrong'] <= rec['skel_axis'] + 0.05
    out = {'n_rows': NR, 'pscale': best_ps, 'ce': CE, 'quad_gap_in_skeleton': round(gap, 4),
           ...
           'pred_a_standin_works': bool(pa), 'pred_b_printable_price': bool(pb),
           'pred_c_wrong_succ': bool(pc),
           'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"CE {CE}")
    print(f"quad gap {gap:.4f} | recovery {rec} | dmg skel {d_skel:.4f} standin {d_stand:.4f}")
    print(f"pred_a works {pa} | pred_b price {pb} | pred_c wrong {pc}")
