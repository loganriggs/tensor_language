...
"""THE MISSING CONTROL: bilin18's architecture trained 7000x longer (v788). Every rung so far says training tokens, not architecture, set how
much read-side rank a head needs: the softmax twin of bilin18 (QK-norm per head + rotary, ~5B tokens, 128-dim heads) needs rank 16 of 128 for
recovery 0.949, Pythia-160m needs 20 of 64 at 8.4B tokens, GPT-2 ~34 of 64 at ~40B, and Pythia-160m 46 of 64 at 300B. The one comparison the
report has been missing is a QK-NORMED ROTARY model trained long. Qwen3-0.6B is exactly that: per-head RMSNorm on q and k over the head
dimension (the same arrangement as bilin18, unlike OLMo-2 which normalises the whole projection), rotary, 128-dim heads, ~36T training tokens.
If the architecture is what makes the bilinear models simple, Qwen3 should be cheap at rank 16-32; if training length is, it should be expensive.
MODEL = Qwen/Qwen3-0.6B (28 layers x 16 heads, head dim 128, GQA with 8 kv heads, hidden 1024). Rows: the same FineWeb text as the other caches
re-tokenised with the Qwen3 tokenizer (472 fit / 189 / 190 held-out rows of 513). ROW-CENTRED logits throughout; column 0 and the diagonal native.
PREDICTIONS (scored as written; failures preserved)
    pred_a_instrument_replays   explicit native CE within 0.002 of the stock native CE (instrument, with per-head QK-norm applied in the low-rank path)
...
PREDICTIONS = {"pred_a_instrument_replays": "+-0.002", "pred_b_kernel_energy_top4": ">= 0.90", "pred_c_rank32_cost": "recovery < 0.96", "pred_d_rank16_cost": "recovery < 0.93", "pred_e_rank32_values_preserved_scale": "median ratio in [0.5, 2]"}
