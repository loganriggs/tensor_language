#!/usr/bin/env python
"""late_stack_shared_input_core_probe -- compositional reuse across the late stack: ONE k-dim input subspace (pooled centred input
covariance of mlp11-17; each block keeps its own mean and own weights) in place of seven per-block bases (§2739-§2741). CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_shared_const_256 pred_c_shared_const_768 pred_d_shared_tok_256
#                     pred_e_shared_core_captures_variance
...
Preregistration: polynomial_causal/LATE_STACK_SHARED_INPUT_CORE_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_stack_shared_input_core_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_stack_shared_input_core_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_STACK_SHARED_INPUT_CORE_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_stack_constant_filler_probe_results.json"   # §2739
PRIOR40 = ROOT / "late_stack_width_and_token_rank_probe_results.json"   # §2740
OUT = ROOT / "late_stack_shared_input_core_probe_results.json"
HASHES = {PREREG: "5aa4a57d79c5324cdbfbe5b3f5174b136395eee803ae1e1dddb998f4aa3937c0", PRIOR: "3b402bb3ccebd31396d3ad58c55e28e99f6596516a3183f8d7acdbd81be94f67", PRIOR40: "7b5cd1fa7b5a04ffed1f22e2f1c40b5f3754fecfc3ef53ff0320c2a593034b03",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_stack_shared_input_core_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_ALL7_CONST_256 = 0.397; LAM = 1e-2
PER_BLOCK = {"CONST_64": 0.797, "CONST_256": 0.397, "CONST_512": 0.186, "CONST_768": 0.079, "TOK_256": 0.297, "TOK_768": 0.065}   # §2739/§2740
KSH = [64, 256, 512, 768]; KT = [256, 768]; KCAP = 256; BLOCKS = list(range(11, 18))
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_max": 0.45, "c_max": 0.11, "d_max": 0.35, "e_min": 0.85}
NULLS = {"b_min": 0.55, "c_min": 0.20, "d_min": 0.45, "e_max": 0.60}
...
def main():
    ...
    Cavg = sum(heads[s_]["Cx"] for s_ in LATE7) / len(LATE7); evs, Ush_all = torch.linalg.eigh(Cavg); Ush_all = Ush_all.flip(1).float()
    def capture(l, U_):
        C = heads[("mlp", l)]["Cx"]; return float((U_.double().T @ C @ U_.double()).trace() / C.trace())
    cap = {str(l): {"shared": capture(l, Ush_all[:, :KCAP]), "own": capture(l, heads[("mlp", l)]["U"][:, :KCAP])} for l in BLOCKS}
    cap_ratio = {l: cap[l]["shared"] / cap[l]["own"] for l in cap}; mean_ratio = sum(cap_ratio.values()) / len(cap_ratio)
    log(stage="shared_core", cap_ratio={l: round(v, 3) for l, v in cap_ratio.items()}, mean_ratio=round(mean_ratio, 3))
    def run(parts):
        patch = {}; objs = []
        for p_ in parts:
    ...
    blk = lambda k, fill, U_=None: {("mlp", l): OwnHead(l, k, fill, U_) for l in BLOCKS}
    arms = {"ALL7_CONST_256": run([blk(256, "const")])}
    for k in KSH:
        arms[f"SHARED7_CONST_{k}"] = run([blk(k, "const", Ush_all[:, :k])])
    for k in KT:
        arms[f"SHARED7_TOK_{k}"] = run([blk(k, "tok", Ush_all[:, :k])])
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    gap = {f"CONST_{k}": arms[f"SHARED7_CONST_{k}"] - PER_BLOCK[f"CONST_{k}"] for k in KSH}
    gap.update({f"TOK_{k}": arms[f"SHARED7_TOK_{k}"] - PER_BLOCK[f"TOK_{k}"] for k in KT})
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["ALL7_CONST_256"] - PRIOR_ALL7_CONST_256) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_shared_const_256': bool(arms["SHARED7_CONST_256"] <= BARS["b_max"]),
        'pred_c_shared_const_768': bool(arms["SHARED7_CONST_768"] <= BARS["c_max"]),
        'pred_d_shared_tok_256': bool(arms["SHARED7_TOK_256"] <= BARS["d_max"]),
        'pred_e_shared_core_captures_variance': bool(mean_ratio >= BARS["e_min"]),
    }
    nulls = {"b_null_ge_.55": bool(arms["SHARED7_CONST_256"] >= NULLS["b_min"]), "c_null_ge_.20": bool(arms["SHARED7_CONST_768"] >= NULLS["c_min"]),
             "d_null_ge_.45": bool(arms["SHARED7_TOK_256"] >= NULLS["d_min"]), "e_null_ratio_le_.60": bool(mean_ratio <= NULLS["e_max"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "ks_shared": KSH, "ks_tok": KT, "blocks": BLOCKS,
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "all7_const_256": arms["ALL7_CONST_256"], "prior_all7_const_256": PRIOR_ALL7_CONST_256},
           "ce_added": arms, "shared_minus_per_block": gap, "per_block_prior": PER_BLOCK, "capture_256": cap, "capture_ratio_256": cap_ratio, "mean_capture_ratio": mean_ratio,
    ...
    if smoke:
        print(json.dumps({k: out[k] for k in ("preds", "nulls", "instrument", "ce_added", "shared_minus_per_block", "capture_ratio_256", "price")}, indent=1)); return
    from receipt import dump
    dump(out, OUT)
    print(json.dumps({"preds": preds, "nulls": nulls, "ce_added": arms, "shared_minus_per_block": gap, "mean_capture_ratio": mean_ratio}, indent=1))
