"""DOES THE ADOPTED CORRECTION TRANSPORT? A HELD-OUT FRESH WINDOW (Claude, LANE 1, RED TEAM)

**Every scalar this campaign has adopted was chosen by minimising L2 on the SAME 120 documents it is reported on.**
`FR` is built once, deterministically, from `dsf[di]` over `di in range(3000,10000)`, and `L2_F = evalM(FR,...) - baseF`.
SS2896 picked tail LW x0.25 by comparing L2_F; SS2902 picked CP Dk x0.5 by comparing L2_F; SS2904 reported their
composition as **+2.3522, a 0.3213 improvement on +2.6735** -- all on FR. SS2907 and SS2909 then searched 36 and 32 grid
cells against FR.

That is **selection on the evaluation set**, and its size has never been measured. Some of the reported 0.3213 is a real
property of the fitted stack and some is fitting the particular 120 documents that scored it. Nothing published so far can
separate the two, and this is the single largest threat to the only two adopted frontier results.

The test is the cheapest one available: continue the SAME document scan to 240 rows. `rows[:120]` is bit-identical to the
FR every previous rung used -- same order, same dedup -- so the reproduction gate still tests the published number.
`rows[120:240]` is a second 120-document window that **played no part in choosing any scalar**. Evaluate the baseline and
the frozen, already-adopted (tail .25, CP .5) configuration on both. No fitting, no grid, no selection: two arms.

**The quantity this rung exists to produce is the SELECTION BIAS, gain(FR) - gain(FR2), in nats.** It is reported whether
the predicates hold or fail, because a small bias vindicates the adoptions and a large one qualifies them -- and either way
the number belongs on the record next to +2.3522.

SIGN CONVENTION (SS2135): frontier L2 is CE ADDED ABOVE THE REAL MODEL, so **LOWER IS BETTER** (SS312: +2.6735 beating
+2.84/+2.93). A gain is `L2(baseline) - L2(arm)`, **POSITIVE = BETTER**. SS2128/SS2129/SS2133/SS2134 RETRACTED;
...
Preregistration: polynomial_causal/FRONTIER_SCALE_HOLDOUT_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run: the adopted (tail .25, CP .5) config on the selection window AND a held-out one")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_scale_holdout_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_SCALE_HOLDOUT_PREREGISTRATION.md'
PREREG_SHA = "c4490d577ce3e83916034a10408d7836925d1487d1454b116c2a2b7ae7d844c7"
...
if __name__=='__main__':
    ...
    ARMS[:] = [('baseline', {}), ('TC', {'tail_scale': 0.25, 'cp_scale': 0.5})]
    print('ONE pipeline run, 2 arms x 2 fresh windows against the same fitted stack',flush=True)
    out=main()

    bF, bF2 = out['baseline']['L2_F'], out['baseline']['L2_F2']
    gain_F  = round(bF  - out['TC']['L2_F'],  4)     # POSITIVE = the adopted scalars help
    gain_F2 = round(bF2 - out['TC']['L2_F2'], 4)
    bias    = round(gain_F - gain_F2, 4)             # POSITIVE = the selection window flattered the gain
    ...
    res={'rung':'frontier_scale_holdout','preds':preds,'nulls':nulls,'bars':BARS,
         ...
         'summary':{'L2_F_baseline':round(bF,4),'L2_F2_baseline':round(bF2,4),
                    ...
                    'gain_selection_window':gain_F,'gain_holdout_window':gain_F2,
    ...
    print(f"    SELECTION BIAS = {bias:+.4f} nats   (positive = the adopted number flatters itself)")
