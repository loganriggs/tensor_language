...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_skip_1416_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; per-class CE attribution of a14L vs attn14-real on FR (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'attn14_class_damage_results.json'
...
if __name__=='__main__':
    ...
    SEL['skipset']=(); SEL['clsdmg']=True
    print('ARM 1/2: full S2144 config (a14L installed), per-position CE captured',flush=True)
    r_full=main()
    cev_full=SEL['cev']; clsflat=SEL['clsflat']
    SEL['skipset']=(14,)
    print('ARM 2/2: skip-a14 (attn14 real), per-position CE captured',flush=True)
    r_skip=main()
    cev_skip=SEL['cev']
    import statistics as stt
    prev=json.load(open(PT+'attn14_headw8_results.json'))['base_skip14']['fresh8']
    ...
    constn_share=sum(shares[k] for k in CONSTK)
    pa=shares[CLS.index('ind')]<=0.30
    pb=constn_share>=0.20
    pc=stt.median(rp)<=0.005
    res={'full':r_full,'skip14':r_skip,'total_damage_sum':round(tot,2),
         'class_shares':{CLS[k]:round(shares[k],4) for k in range(10)},
         'constn_combined_share':round(constn_share,4),
         'per_window_skip14_vs_s2155_absdelta':rp,
         'convention':'per-position CE(full with a14L) - CE(attn14 real) on FR; positive = the dictionary hurts there',
         'pred_a_not_retrieval':bool(pa),'pred_b_constn_pay':bool(pb),'pred_c_reproduces_s2155':bool(pc),
    ...
    print(f"(b) CONSTN combined {constn_share:+.3f} >= 0.20: {'HELD' if pb else 'FAILED'}")
    print(f"(c) skip14 vs S2155 median |delta| {stt.median(rp):.4f} <= 0.005: {'HELD' if pc else 'FAILED'}")
