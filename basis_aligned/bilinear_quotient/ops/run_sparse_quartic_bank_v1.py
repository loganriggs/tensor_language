...
"""Exact sparse shared bank: two Muon random starts,100steps; price1088products1353728coeff.
Integrity normalresidual<1e-8/export<1e-4. Both explainedgain>=2/queryimprove.
Some start Gaussian<.9887785 and same coefficientqueries<.9777571; zero100% null.
...
def main():
 ...
 pairs=support(144,512,1100)
 def values(factors,coeff,x):return features(x,factors[0],factors[1],pairs.to(x.device))@coeff.T
 def normalize_factors(parameters):return normalize_bank(parameters[0].reshape(144,4,-1),parameters[1].reshape(144,4,-1))
 def native_objective(teacher,factors,ridge=1e-6):
  active=pairs.to(factors[0].device)
  return profile(gram(*factors,active),native_cross(teacher,*factors,active),ridge=ridge)
 def cp_entries(coeff,factors,indices):return entries(*factors,pairs.to(indices.device),indices)@coeff.T
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
  ...
  p=[torch.randn(576,12,dtype=dtype,requires_grad=True) for _ in range(2)];f=normalize_factors(p);loss,c=native_objective(teacher,f,ridge=1e-6);loss.backward();x=torch.randn(5,12,dtype=dtype)
  ...
  print(json.dumps(dict(controls=checked,rank512_shape_smoke=True,steps=100,seeds=[1101,1102],products=1088,coefficients=1353728)));return
 torch.backends.cuda.matmul.allow_tf32=False;start=time.monotonic();out=P/'SPARSE_QUARTIC_BANK_NATIVE_V1.json';assert not out.exists();scale=19054614563.464127
 ...
 for seed in [1101,1102]:
  gen=torch.Generator().manual_seed(seed);params=[torch.nn.Parameter((torch.randn(576,1152,generator=gen,dtype=torch.float64)/math.sqrt(1152)).cuda()) for _ in range(2)]
  ...
  with torch.no_grad():
   f,c=best[2],best[3];g=gram(*f,pairs.cuda());cross=native_cross(teacher,*f,pairs.cuda())
   ...
   artifact=dict(factors=[a.cpu().float() for a in f],pairs=pairs,coefficients=(c*scale).cpu().float(),writer=writer.cpu().float(),seed=seed,degree=4)
   path=P/f'SPARSE_QUARTIC_BANK_SEED{seed}_V1.pt';torch.save(artifact,path)
 ...
 pred=dict(pred_a_integrity=max(normals)<1e-8 and max(drifts)<1e-4,pred_b_learning=all(r['explained_gain']>=2 and r['final']['sampled_coefficient_error']<r['initial']['sampled_coefficient_error'] for r in rows),pred_c_comparison=any(r['final']['gaussian_error']<.9887785 and r['final']['sampled_coefficient_error']<.9777571 for r in rows))
 result=dict(predictions=pred,rows=rows,products=1088,coefficients=1353728,integer_indices=1024,seconds=time.monotonic()-start,peak_memory_bytes=torch.cuda.max_memory_allocated(),scope='Exact coefficient objective,16 fixed data-informed readers,random shared quadratic directions and fixed seeded sparse pair support; evaluation-only sampled coefficient/Gaussian/text metrics; no convergence or semantic/circuit claim.')
