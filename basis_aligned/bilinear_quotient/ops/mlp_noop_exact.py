# THE MLP NO-OP, TESTED DIRECTLY -- bit-identical predictions, not a ratio.
#
# §1891 claimed all eighteen MLP leave-one-out restorations are a no-op and gave a derivation: with the
# other 35 sites compiled the residual stream is a function of the current token alone (§1765), an MLP is
# position-wise, so a live MLP on a context-free stream returns precisely the value its own table stores.
# §1897 replicated it at 16,110 coverage and, in doing so, caught my wording: §1891 printed "EXACTLY
# +0.00" at TWO decimal places, and to four the largest MLP |gain| is 0.0023 / 0.0042 / 0.0000.
#
# I said in §1897 that the residual is the right order for the permutation null's sampling variance, and
# that I could not claim it because I had not recorded the numerator. This records the numerator -- and
# in a stronger form than agreement. The derivation says the restored program's OUTPUT is identical, so
# the test is not "does agreement move" but "does any single predicted token change".
#
# For each of the 18 MLP sites: argmax predictions with all 36 compiled, versus with that site LIVE, on
# every scored position. The derivation predicts ZERO differing positions. Anything else is a hole in it.
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY. Rung 2: closing §1897's named loose end.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
#   pred_a THE NO-OP IS EXACT: zero positions change their predicted token for any of the 18 MLP sites,
#          on any role. Not "few" -- ZERO, because the derivation says the tensors are identical, and a
#          derivation that is only approximately right is not a derivation. If FALSE, §1765's argument
#          has a hole and §1891's headline claim -- which I stated as provable and told Codex constrains
#          any per-site story -- is wrong, and I would say so at the top of the section.
#   pred_b AND ATTENTION IS NOT A NO-OP: at least one attention site changes at least 1% of positions, on
#          all three roles. The two-sided half: a test that returns zero for EVERYTHING is measuring
#          nothing, and this is the known-answer check that the comparison can detect a change at all.
#          §1889 is why this is registered -- a degenerate split read as a result there.
#   pred_c AND THE COLLAPSING SITES CHANGE THE MOST: attn5 changes more positions than attn13, on all
#          three roles. §1892 showed attn5's live output exceeds its table by 156.4x and drives the
#          program to chance while attn13 helps; the position-change count should order the same way. If
#          FALSE, "how much a site changes" and "how much it harms" are different quantities, which is
#          worth knowing before anyone uses one as a proxy for the other.
#   pred_d CONTROLS: coverage is exactly 16,110; all 36 sites are compared on identical positions; and
#          the all-compiled arm is compared against ITSELF as a zero-check, so a bug that reports zero
#          differences everywhere is caught by pred_b rather than passing as a result.
...
OUT = PT + 'ops/mlp_noop_exact_results.json'
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'THE MLP NO-OP, TESTED DIRECTLY | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    for si, s in enumerate(sites):
        ...
        for ename in evs:
            ...
            nd = int((p2 != basep[ename]).sum())
            row[ename] = {'changed': nd, 'n': int(p2.numel()),
                          'frac': nd / max(int(p2.numel()), 1)}
            p2 = None
        res['sites'][f'{s[0]}{s[1]}'] = row
        print(f'    restore {s[0]}{s[1]:<2d}  ' + '  '.join(
            f'{e} {row[e]["changed"]:6d} ({row[e]["frac"]:.4%})' for e in row)
    ...
    SK = list(res['sites'])
    MLPS = [k2 for k2 in SK if k2.startswith('mlp')]
    ATTNS = [k2 for k2 in SK if k2.startswith('attn')]

    def ch(k2, e):
        return res['sites'][k2][e]['changed']
    mlpmax = {e: max(ch(k2, e) for k2 in MLPS) for e in roles}
    attnmax = {e: max(res['sites'][k2][e]['frac'] for k2 in ATTNS) for e in roles}
    pa = all(mlpmax[e] == 0 for e in roles)
    pb = all(attnmax[e] >= 0.01 for e in roles)
    pc = all(ch('attn5', e) > ch('attn13', e) for e in roles)
    pd = ncov == NCOV and len(SK) == 36 and res['self_check'] == 0

    print(f'\n  THE MLP NO-OP IS EXACT (zero positions change, 18 sites) -> {pa}  ' + '  '.join(
        f'{e} max changed {mlpmax[e]}' for e in roles), flush=True)
    print(f'  and ATTENTION is not a no-op (>=1% somewhere) -> {pb}  ' + '  '.join(
        f'{e} max {attnmax[e]:.2%}' for e in roles), flush=True)
    print(f'  and attn5 changes MORE than attn13 -> {pc}  ' + '  '.join(
        f'{e} attn5 {ch("attn5", e)} vs attn13 {ch("attn13", e)}' for e in roles), flush=True)
    print(f'  coverage {ncov}, 36 sites, self-check {res["self_check"]} -> control {pd}', flush=True)
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_noop_is_exact_program_concentrated_on_frequent_targets': bool(pa),
                          'pred_b_attention_is_not_a_noop_more_concentrated_than_live': bool(pb),
                          'pred_c_attn5_changes_more_keeps_60pc_on_the_top_bucket': bool(pc),
