...
PRIOR=ROOT/'circuits/prior_art/NARRATIVE_TENSE_L11H3_SUFFIX_COMPRESSION_V1_PREREGISTRATION.md'
OUT=ROOT/'circuits/fast_screens/narrative_tense_l11h3_suffix_compression_v1_result.json'
PRIOR_SHA256='2cc91706cf33e087423905cf5c39c68a81ded58b23a06b6aaac352a23f148067'
...
def compile_plan():
 ...
 return {'schema':'narrative_tense_l11h3_suffix_compression_plan_v1','candidate_id':'narrative_tense.l11h3_suffix_compression_v1','model_loaded':False,'gpu_accessed':False,'queue_touched':False,'prior_art_sha256':PRIOR_SHA256,'capability_license_sha256':carrier.LICENSE_SHA256,'row_count':len(rows()),'layer':carrier.core.LAYER,'head':carrier.core.HEAD,'arms':list(ARMS),'bars':BARS,'registered_predictions':list(PREDICTION_REGISTRY),'price':{'model_forwards':6,'example_evaluations':576,'backwards':0,'parameter_updates':0}}
...
def evaluate(model,torch,F,facade):
 ...
 for start in range(0,len(rr),carrier.core.BATCH):
  ...
  plog,_=factor._factor_forward(model,btt[ix],bff[ix],torch,F,facade,replacement_heads=torch.stack(heads),replacement_head_mask=torch.ones(len(heads),dtype=torch.bool,device=device))
...
def main(argv:Sequence[str]|None=None):
 ...
 scored=score(ev,exact);term=terminal(scored['predictions']);result={'schema':'narrative_tense_l11h3_suffix_compression_result_v1','candidate_id':plan['candidate_id'],'terminal':term,'plan':plan,'checkpoint_weights_sha256':checkpoint.weights_sha256,'score':scored,'evidence':ev,'active_price':plan['price']};payload=managed.atomic_create_json(OUT,result);print(json.dumps({'terminal':term,'result_sha256':hashlib.sha256(payload).hexdigest()},sort_keys=True))
