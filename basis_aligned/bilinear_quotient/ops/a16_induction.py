"""THE INDUCTION POINTER AT a16 (rung 66): the single registered v2 after S2158.

CONVENTION (S2135): L2 is CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER.

S2158: a linear read of the TARGET's last occurrence recovers only +0.011 of a16L's ~0.21. The induction form
reads elsewhere: match the last previous occurrence q* of the CURRENT token idx[p], and read the stream at
q*+1 - the position whose token is the successor of the previous occurrence, i.e. the classic induction guess.
Output for ind positions = x[q*+1] @ W_ptr (ridge-fit on FW); all other classes keep the S2144 stand-in.
PRICE: +1.33M values; same oracle interface as the class labels. NULL (S2158): this too recovers ~0.01.

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) A QUARTER COMES BACK: median over the 8 windows of [L2_F_w(full) - L2_F_w(a16ind)] >= +0.05.
  ...
  TRIPWIRE: the pointer branch must fire > 0 times or the run aborts.

Writes a16_induction_results.json. Self-reviewed. RUNG-64 HEADER FOLLOWS.

POINTER-LINEAR STAND-IN FOR THE ind CLASS AT a16 (rung 64): the first constructive retrieval primitive.

CONVENTION (S2135): L2 is CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER.

S2151/S2157: both expensive tail dictionaries fail ~90-97% on retrieval classes, led by ind (target token seen
earlier in the document). The missing grammar element is a POINTER: for ind positions, output (stream at the
last occurrence of the target token) @ W_ptr instead of the current-position class-linear map. W_ptr is
ridge-fit on FW ind positions; every other class keeps the S2144 stand-in. PRICE: +1.33M values (one D x D
map); the pointer is computed from the token sequence (same oracle interface as the class labels). NULL
(S2148/S2151): retrieval needs more than the previous-occurrence stream and the map recovers little.

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) A QUARTER COMES BACK: median over the 8 windows of [L2_F_w(full) - L2_F_w(a16ptr)] >= +0.05
      (a16L's window cost vs real is ~0.21, S2146).
  (b) NOT ONE WINDOW: that difference is >= +0.02 on >= 6 of 8 windows.
  (c) REPRODUCTION: the full arm reproduces S2144 (FR L2_F 2.6662 within 0.01).
  TRIPWIRE: the pointer branch must fire > 0 times or the run aborts.

Writes a16_pointer_results.json. Self-reviewed. RUNG-63 HEADER FOLLOWS.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['a16_pointer_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; full vs a16-INDUCTION-pointer arms x eight windows (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'a16_induction_results.json'
...
def main():
    ...
    def install(active):
        ...
        for nm in active:
            ...
            if kind=='cv':
                ...
                _,li,CV,LW,Wp2,Wptr=S[nm]
                def h(mod,i_,o_,CV=CV,LW=LW,Wptr=Wptr):
                    y,v1=o_
                    xx=i_[0].float()
                    B2,T2,_=xx.shape
                    cflat=cur['lab']
                    ...
                    for k in LINK:
                        sel=cflat==k
                        if sel.any(): new[sel]=xf[sel]@LW[k]
                    idxb=cur['idx']
                    cb=cflat.view(B2,T2)
                    nrep=0
                    for b3 in range(B2):
                        for p3 in range(T2-1):
                            if int(cb[b3,p3])!=8: continue
                            tgt=idxb[b3,p3]
                            qs=(idxb[b3,:p3]==tgt).nonzero()
    ...
    for li in range(10,18):
        ...
        if li==16 and SEL.get('a16ptr'):
            TOK16=FW[CA:CB,:257]
            ptrX=[]; ptrY=[]
            for fi in range(len(flatA)):
                if int(flatA[fi])!=8: continue
                r5,pp=divmod(fi,256)
                tgt=int(TOK16[r5,pp])
                qs=(TOK16[r5,:pp]==tgt).nonzero()
                if len(qs)==0: continue
                q5=int(qs[-1])+1
                ptrX.append(X2[r5*256+q5]); ptrY.append(Y[r5*256+pp])
            Xp=torch.stack(ptrX); Yp=torch.stack(ptrY)
            lamp=1e-2*len(Xp)
            Wptr=torch.linalg.solve(Xp.T@Xp+lamp*torch.eye(D,device=DEV),Xp.T@Yp)
            S[f'a{li}L']=('attnp',16,CV,LW,Wp2,Wptr)
            print(f'a16 INDUCTION pointer map fit on {len(Xp)} ind positions',flush=True)
...
if __name__=='__main__':
    ...
    SEL['skipset']=(); SEL['a16ptr']=False
    print('ARM 1/2: full S2144 config (reproduction)',flush=True)
    r_f=main()
    SEL['a16ptr']=True; cur_nptr_before=0
    print('ARM 2/2: a16L with the INDUCTION-pointer ind stand-in',flush=True)
    r_p=main()
    ...
    print(f'recovery (full - a16ptr) per window: {d}  median {md:+.4f}')
    print(f"(a) median {md:+.4f} >= +0.05: {'HELD' if pa else 'FAILED'}")
    print(f"(b) >= +0.02 on {sum(g>=0.02 for g in d)}/8 (bar 6): {'HELD' if pb else 'FAILED'}")
    print(f"(c) full FR L2_F {r_f['L2_F']} vs 2.6662 (tol 0.01): {'HELD' if pc else 'FAILED'}")
