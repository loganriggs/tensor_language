# THE MAP-RANK LEVER AT THE DEPLOYED COVERAGE -- where it is worth 2.5x more.
#
# §1869 swept the map rank at 16,110 covered types and found it binding but partial: rank 64 -> 128 costs
# 5.309M and recovers 0.0948 nats per uncovered position, which at that build's 10.0% uncovered fraction
# is 0.00948 all-position nats -- **560 M reals per nat, 3.3x better than the cheapest comparable
# table-rank step**, and the best buy in §1829-§1869. It also found 73% of the fallback loss survives at
# map rank 512, so the residual is functional form rather than capacity.
#
# The DEPLOYED build (§1789, full rank over 5,419 covered types) has an uncovered fraction of **24.6%**,
# 2.5x §1869's. If the per-position recovery is similar there, the same 5.309M buys ~0.023 all-position
# nats -- and the lever is worth more on the build that actually exists than on the one §1869 measured.
# But §1867 is the reason to check rather than scale: it found the fallback PENALTY constant in coverage,
# which says nothing about whether the map-rank RECOVERY is.
#
# Docstring and predicate block written in ONE edit (LESSONS 63); the ladder holds numeric map ranks
# (LESSONS 64); filename tokenises against the banner (LESSONS 56).
#
# ROLES. All three. DISCOVERY ONLY. Rung 3: the deployed-build half of §1869.
#
...
# ceiling, and every cross-coverage comparison below is of that deficit, never of a raw CE.
#   pred_a THE RECOVERY TRANSFERS: map rank 64 -> 512 recovers at least 0.18 nats per uncovered position
#          on all three roles, within 0.06 of §1869's PUBLISHED +0.21926 / +0.23135 / +0.25424 at 16,110
#          types. If TRUE the map-rank lever is a property of the map and not of the covered set, and it
#          is worth 2.5x more on the deployed build than §1869 measured. If FALSE the recovery is
#          coverage-dependent -- which would be a second disanalogy with §1867's constant penalty and
#          would mean the lever has to be priced per build.
#   pred_b AND THE COVERED ARM IS UNTOUCHED: covered CE identical across every map rank to within 1e-6.
#          §1869 got exactly 0.00e+00; this is the same KNOWN-ANSWER check on a different covered set,
#          and if it moves nothing else in the run can be read.
#   pred_c AND THE LEVER IS WORTH MORE HERE: (uncovered fraction) x (rank 64 -> 128 recovery) exceeds
#          §1869's PUBLISHED 0.00948 all-position nats on all three roles. If FALSE the larger uncovered
#          fraction does not translate into more value, which would mean the per-position recovery falls
#          faster than the fraction rises.
#   pred_d CONTROLS: coverage is exactly 5,419, reproducing §1834's PUBLISHED figure; the covered arm
#          attains its ceiling exactly (an eleventh confirmation); every uncovered scored token has a
#          ceiling row; the uncovered fraction reproduces §1848's PUBLISHED 24.6% within 1pp.
...
OUT = PT + 'ops/map_rank_at_deployed_cov_results.json'
...
S1869_REC_16110 = {'skip7000': 0.21926, 'skip11000': 0.23135, 'skip1200': 0.25424}
S1869_VALUE_64_128 = 0.00948
...
def main():
    ...
    print(f'MAP RANK AT DEPLOYED COV | 5419 types, full table rank, map 64/128/256/512 | '
    ...
    rec128 = {e: fbloss['64'][e] - fbloss['128'][e] for e in roles2}
    value128 = {e: nfrac[e] * rec128[e] for e in roles2}
    ...
    pa = all(rec[e] >= 0.18 and abs(rec[e] - S1869_REC_16110[e]) <= 0.06 for e in roles2)
    pb = covspread <= 1e-6
    pc = all(value128[e] > S1869_VALUE_64_128 for e in roles2)
    pd = (NFULL == NCOV and attain
          and all(abs(nfrac[e] - S1848_UNC_FRAC_5419) <= 0.01 for e in roles2))

    print(f'\n  FALLBACK LOSS by MAP RANK at {NFULL} covered types (DEPLOYED), full table rank:',
          flush=True)
    for l2 in LADDER:
        print(f'    map {l2:5s} {mapcost[l2]:7.3f}M  ' + '  '.join(
            f'{e} {fbloss[l2][e]:+.5f}' for e in roles2), flush=True)
    print(f'\n  the RECOVERY transfers (>=0.18, within 0.06 of §1869) -> {pa}  ' + '  '.join(
        f'{e} {rec[e]:+.5f} vs §1869 {S1869_REC_16110[e]:+.5f}' for e in roles2), flush=True)
    print(f'  and the COVERED arm is untouched (<1e-6) -> {pb}  max spread {covspread:.2e}',
          flush=True)
    print(f'  and the 64->128 lever is worth MORE here (> §1869 {S1869_VALUE_64_128}) -> {pc}  '
          + '  '.join(f'{e} {value128[e]:.5f}' for e in roles2)
          + f'   at unc frac {nfrac[roles2[0]]:.1%}', flush=True)
    print(f'  coverage {NFULL}, attainment {attain}, unc frac ~ §1848 '
          f'{S1848_UNC_FRAC_5419:.1%} -> control {pd}', flush=True)

    json.dump({'run': 'map_rank_at_deployed_cov', 'n_covered': NFULL,
               'map_ranks': list(MAPRANKS), 'map_cost_M': mapcost, 'fallback_loss': fbloss,
               'covered_ce': covce, 'recovery_64_to_512': rec, 'recovery_64_to_128': rec128,
               'value_64_to_128': value128, 'uncovered_fraction': nfrac,
               'covered_spread': covspread, 'ceiling_uncovered': uceilv,
               'predictions': {'pred_a_recovery_transfers': bool(pa),
                               'pred_b_covered_untouched': bool(pb),
                               'pred_c_worth_more_here': bool(pc),
