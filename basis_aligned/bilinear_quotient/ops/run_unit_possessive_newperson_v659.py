...
"""v659: do the six separable possessive candidates survive the REGISTERED fit objective?
...
Smoke: V659_SMOKE=<out.json> (CPU, V659_SMOKE_ROWS=4, V659_SMOKE_NAMES=<cell>).
"""
...
OUT = ROOT / "circuits/followups/unit_possessive_newperson_v659_result.json"
...
NAMES = {"possessive_person_my_their": "possessive_person_my_their",
         "possessive_person_your_their": "possessive_person_your_their",
...
V659_ANCHOR = {"canon_ub": 0.0075, "weekly_ub": 0.0096}
...
BARS = {"ext_min": EXT_MIN, "rem_min": REM_MIN, "c_ub_max": C_UB_MAX, "row5_share_band": [0.5, 1.4], "k_reach": 3, "k_both": 2, "tol": 0.05, "max_dropped": 4}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 200000, 6400000


def _plan():
    return {"candidate_id": "corpus.unit_possessive_newperson_v659", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V659_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V659_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V659_SMOKE_NAMES", "verb_preposition_against_for,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_possessive_newperson_v659", "candidate_id": "corpus.unit_possessive_newperson_v659", "bars": BARS,
