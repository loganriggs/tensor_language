...
"""Pronoun number they/he DoD (v181): the PLURAL-noun detector 829's PRODUCT (v168 / v170), exactly, as writer-pair terms -- v178's fold on the number line; same registered questions (embedding x MLP-7 structure? MLP 6 damping? 8.1's copy?). u_3152(noun) = (L . x)(R . x) with x = x_8[noun] / rms
...
PREDICTIONS (scored as written; failures preserved; priors unsure except c, which v177 implies)
    pred_a_pair_closure                    sum of pair terms = captured u_3152(noun) within relative 1e-3, every row
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_unit829_pair_fold_v181_result.json"
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_unit829_pair_fold_v181"
UNIT, CLOSURE_TOL, H81_MIN = 829, 1e-3, 0.30
FORWARDS_MAX = 4
WRITERS = v16.WRITERS
...
def main() -> None:
    ...
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_unit829_pair_fold_result_v181", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "report": report, "shares": shares, "predictions": predictions, "forwards": forwards,
