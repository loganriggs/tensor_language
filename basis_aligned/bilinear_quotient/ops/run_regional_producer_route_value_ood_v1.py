...
"""pred_a native folded-producer and saved-group donor replay<=1e-5 relative;
pred_b full-group transfer>=40%whole-child reference and>=4/6positive EACHfamily;
pred_c value-only transfer>=90%full-group, unrelated<=.5regional EACHfamily.
Null: values alone may not carry the cue; joint QK routing or interaction can dominate.
"""
...
STEM='REGIONAL_PRODUCER_ROUTE_VALUE_OOD_V1'
@torch.no_grad()
def main():
    ...
    def producer_hook(layer,module,args,out):
        ...
        variants=torch.stack([torch.einsum('nhts,nsha->nta',routing[donor] if mask&1 else routing,values[donor] if mask&2 else values) for mask in range(4)],1)
    ...
    prior=torch.load(P/'REGIONAL_PAYLOAD_ATLAS_OOD_V1_ARTIFACT.pt',weights_only=True,map_location='cpu')['write_vertices'].cuda()
    prior_group=prior[:,0]+(prior[:,[10,11,15]]-prior[:,0,None]).sum(1)
    ...
    reference_cells=json.loads((P/'REGIONAL_PAYLOAD_ATLAS_OOD_V1_RESULT.json').read_text())['cells']
    for family in (0,1,2,3):
        ...
        cells.append(dict(family=family,whole_child_reference=reference_cells[family]['mean_transfer_by_arm'][38][0],mean_transfer_by_arm=transfer.mean(1).tolist(),positive_regional_by_arm=(transfer[:,:,0]>0).sum(1).tolist(),value_regional_meanabs=float(transfer[2,:,0].abs().mean()),value_unrelated_meanabs=float(transfer[2,:,1].abs().mean()),interaction_transfer=(transfer[3]-transfer[1]-transfer[2]).mean(0).tolist()))
    instrument=max(producer_errors+replays+[donor_error,recipient_error])<=1e-5
    full=instrument and all(c['mean_transfer_by_arm'][3][0]>=.4*c['whole_child_reference'] and c['positive_regional_by_arm'][3]>=4 for c in cells)
    result={'pred_a':instrument,'pred_b':full,'pred_c':full and all(c['mean_transfer_by_arm'][2][0]>=.9*c['mean_transfer_by_arm'][3][0] and c['value_unrelated_meanabs']<=.5*c['value_regional_meanabs'] for c in cells)}
    torch.save(dict(pre=pre.cpu(),write_vertices=writes.cpu(),arm_margins=arms.cpu()),art)
    result.update(recipient_replay_error=recipient_error,group_donor_replay_error=donor_error,producer_replay_errors=producer_errors,native_attention_replay_errors=replays,cells=cells,arm_names=['native','joint_QK_routing','projected_values','routing_and_values'],body_forwards=count,seconds=time.perf_counter()-tic,artifact_sha=digest(art),source_shas=binding,scope='Conditional upstream producer routing/value interchange. Current and first-stream values travel together, both QK scores travel together. All remaining upstream/downstream states fixed; not full-network module replacement or standalone extraction.')
