# IS THE RECOMMENDATION BUDGET-DEPENDENT?  -- the 65.950M iso-cost line, coverage 9054, rank 164.
#
...
# 9,054, 128 at 11,954 and 164 at 9054. This measures the 9054-type point.
#
...
#          §1852's PUBLISHED rank-256@16,110 figure of +0.09252 on skip7000, since rank 164 is far
...
#          9054 from fit rows [0:192]; the ceiling is finite and above live on every role.
...
OUT = PT + 'ops/low_budget_cov_9054_results.json'
...
TRANKS = (164,)          # None = full rank; §1786/§1813 use this ladder
...
FITLO, FITHI = 0, 192      # the draw giving 9054 covered types   # 5x the fit rows -> a larger covered set
H = m.transformer.h
NCOV = 5419
...
def main():
    ...
    assert NFULL == 9054, f'coverage {NFULL} != expected 9054'
    ...
    print(f'LOW BUDGET COV | 9054 types, rank 164, 65.950M line | '
    ...
    pd = (abs(cost - LOWBUDGET_M) / LOWBUDGET_M < 0.01 and NFULL == 9054
    ...
    json.dump({'run': 'low_budget_cov_9054', 'n_covered': NFULL, 'rank': R,
