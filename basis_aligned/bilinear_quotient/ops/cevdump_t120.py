"""THE SPECTRAL CLIFF (rung 250): corner + tail r120 - do the last 8 ranks still carry the damage?

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2348: with everything else exact,
tail r112 leaves +0.0531 - the final 16 ranks carry ~96% of the residual. If the fine structure is
concentrated at the very top of the spectrum's tail, r120 (93.75% rank) should STILL hold most of it.
REGISTERED PREDICTIONS (arms: THIS vs r112 0.0531/11):
  (a) CLIFF: census >= 0.035 (the last 8 ranks still carry the bulk).
  (b) valid <= 12 (no rank harvest left).
  (c) SANITY: census in [0.005, 0.12]; inert tripwires vs r112 AND r96 receipts.
NULL: smooth approach to exactness (census <= 0.030) - no cliff, just a slow tail. PRICE: 93.6M pattern
values (probe of spectrum structure, not a config proposal). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: spectral cliff r120')
    raise SystemExit(0)
import torch
...
OUT=PT+'cevdump_t120_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=120; SEL['qk_rmap']={li:128 for li in range(2,10)}; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    cev=SEL['cev']
    torch.save(cev,PT+'cev_t120.pt')
    print('cev saved: cev_t120.pt',flush=True)
    ...
    pb=nv<=12
    pc=0.005<=agg<=0.12
    if abs(agg-0.0531)<1e-4 or abs(agg-0.0553)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: tail120 map inert')
    ...
    print(f"(a) CLIFF - census {agg:+.4f} >= 0.035: {'HELD' if pa else 'FAILED'}")
