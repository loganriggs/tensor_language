...
OUT = PT + 'ops/downstream_sensitivity_ce_results.json'
...
REL = 0.50                     # inside the MEASURED 15-93% band of real table error (§1838)
SEEDS = (0, 1, 2)
CE_FLOOR = 0.005               # nats; pred_d's restated bar -- the probe must LAND at every site
STATE = {}
COV = {}
...
@torch.no_grad()
def ce(rows, hooks=()):
    """CROSS-ENTROPY readout. §1838 voided the top-1 version: an argmax has a discreteness floor, so a
    perturbation must flip a prediction before it registers, and 34 of 34 sites came in under it. CE is
    continuous and has no such floor."""
    tot, n = 0.0, 0
    for i in range(0, rows.shape[0], 8):
        bb = rows[i:i + 8]
        lg = forward_logits(bb[:, :-1].to(DEV).contiguous(), hooks)[:, 64:].float()
        tg = bb[:, 1:].to(DEV)[:, 64:]
        tot += float(F.cross_entropy(lg.reshape(-1, lg.shape[-1]), tg.reshape(-1),
...
def main():
    ...
    print(f'DOWNSTREAM SENSITIVITY, CE READOUT | {REL:.0%} relative noise, {len(SEEDS)} seeds | '
    ...
    sens = {}
    for st in SITES:
        drops = [ce(ev, [(st, noise_hook(REL, sd))]) - ce_base for sd in SEEDS]
    ...
    pc = rho_p > rho
    pd = (landed > CE_FLOOR and exdrift <= 0.02 and ncov == NCOV
          and abs(base - S1789_LIVE_TOP1_PP['skip7000']) <= 0.001
          and abs(base2 - S1789_LIVE_TOP1_PP['skip11000']) <= 0.001)

    print(f'\n  SENSITIVITY (CE NATS gained from {REL:.0%} noise, mean of {len(SEEDS)} seeds; '
          f'live CE {ce_base:.5f}):',
          flush=True)
    for k in ('attn', 'mlp'):
        print(f'    {k:4s} ' + ' '.join(
            f'L{L}:{sens[(k, L)]:+.4f}' for L in range(1, 18)), flush=True)
    print(f'\n  RMS table error / RMS output:', flush=True)
    for k in ('attn', 'mlp'):
        print(f'    {k:4s} ' + ' '.join(
            f'L{L}:{relerr[(k, L)]:.3f}' for L in range(1, 18)), flush=True)
    print(f'\n  dearest 6 by §1834 cost: ' + '  '.join(
        f'{nm} cost {S1834_COST[nm]:+.1%} sens {sens[SITES[names.index(nm)]]:+.4f}'
    ...
    print(f'  noise LANDED everywhere (min {landed:+.4f} nats vs floor {CE_FLOOR}), §1837 '
          f'explained reproduces '
          f'(max drift {exdrift:.4f}) -> control {pd}', flush=True)

    json.dump({'run': 'downstream_sensitivity_ce', 'ce_base': ce_base, 'rel_noise': REL, 'seeds': list(SEEDS),
