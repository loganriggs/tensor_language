#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the sixth eight spec-authored behaviours, plus one instrument.
"""v213: the AMENDED tier-3 battery (v196/v197 protocol, unchanged) over the SIXTH EIGHT spec-authored behaviours -- plus one instrument.

v204 4/8, v205 6/8, v207 4/8, v209 4/8 (+instrument each) at ~30 GPU-s per behaviour; v211 pending. Eight more, authored through
`circuit_fast_screen_behaviour_spec`, single-cue swap, matched final token, cue never final, 32 A1/A2/P/C rows, ABAB directions,
chosen to open three NEW families and extend three under separability: did_has_negation (`did/has not` -> lift/lifted, verb-form
family with question_bare_form), modal_perfect_form (`can/has surely` -> carry/carried, same family), mandative_subjunctive
(`demanded/noticed that the sailor` -> be/was, NEW mood family), polarity_tag (`did not/also leave, and` -> neither/so, polarity
family with polarity_addition), person_agreement_be (`As the pilot knows, I/you surely` -> am/are, person family),
person_agreement_past (`... recalls, I/you surely` -> was/were, person family), noun_complementizer (`The question/claim raised
by the pilot was` -> whether/that, complementizer family with verb_complementizer), to_gerund_selection (`used/objected to` ->
carry/carrying, complement family). Capability printed BEFORE the run on CPU (`g.prepare(valid_only=True)`, 32 rows per family):
zero A1 and zero A2 rows dropped on all eight (32 rows each); mean base/donor margins A1: did_has_negation -6.21/+6.03,
modal_perfect_form -6.74/+6.63, mandative_subjunctive -5.05/+5.21, person_agreement_be -4.64/+4.65, noun_complementizer
-4.58/+4.69, to_gerund_selection -4.46/+4.58, person_agreement_past -4.10/+3.98, polarity_tag -0.96/+0.58 (the WEAKEST cell by
far: the additive tag neither/so after `did not/also leave, and` is only weakly preferred, though every row has the right sign);
A2 weakest polarity_tag -0.77/+0.93, then person_agreement_past -2.51/+2.42 (all other A2 |means| >= 3.4).
Protocol identical to v197/v201/v204/v205/v207/v209/v211 (greedy pool 40, target 0.88, min_gain 0.005, max 30; row 2 = MIN over
...
REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 4 of the 8 new behaviours pass all four rows (v204 4/8, v205 6/8, v207 4/8, v209 4/8 with the
                           same recipe; polarity_tag is the expected miss from its CPU margin).                              prior 55%
...
Accounting: four-row passes here join the one-protocol standing; mandative_subjunctive is a new family and counts directly if
four-row; the other seven have corpus siblings (verb-form, polarity, person, complementizer, complement families) and wait for
the next separability rung before counting.
...
Smoke: V213_SMOKE=<out.json> (CPU, V213_SMOKE_ROWS=4 per split, V213_SMOKE_NAMES=did_has_negation; pool 3/max 3, steps 5).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec8f_v213_result.json"
...
NAMES = {"did_has_negation": "did_has_negation", "modal_perfect_form": "modal_perfect_form",
         "mandative_subjunctive": "mandative_subjunctive", "polarity_tag": "polarity_tag",
         "person_agreement_be": "person_agreement_be", "person_agreement_past": "person_agreement_past",
         "noun_complementizer": "noun_complementizer", "to_gerund_selection": "to_gerund_selection",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec8f_v213", "behaviours": 9, "constructions": 2,
...
def main() -> None:
    ...
    smoke = os.environ.get("V213_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V213_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V213_SMOKE_NAMES", "did_has_negation").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec8f_v213", "candidate_id": "corpus.unit_tier3_batch_amended_spec8f_v213", "bars": BARS,
