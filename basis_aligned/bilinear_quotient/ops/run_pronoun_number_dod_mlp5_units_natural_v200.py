...
"""Pronoun number they/he DoD (v200): the MLP-5 stage OUT OF THE PANEL. Zero the five MLP-5 units {1036, 2403, 2538, 3274, 3034} (v192 carrier census; v195
in-panel edit: 2483 -6%, 829 -3.9%, margin -1.3%, >= 6x the null) at the CUE of the same 128 natural rows; v197 / v199's design otherwise (16 random 5-unit
sets of MLP 5 at the cue, number-free readers). LIVE bar 0.005 as in v199.
PREDICTIONS (scored as written; failures preserved)
    pred_a_instrument_replays_native        hooked forward with no edit = producer native <= 1e-4
    pred_b_congruent_damage_live            congruent rows: damage fraction >= 0.005 and positive >= 0.60. Prior: unsure.
    pred_c_units_beat_random_unit_triples   congruent damage > max of the 16 random 5-unit sets
    pred_d_units_selective                  each reader |move| <= null mean |move| + 0.25 x damage
    pred_e_incongruent_shifts_toward_label  incongruent rows: mean oriented damage < 0. Prior: unsure.
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_mlp5_units_natural_v200_result.json"
...
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_mlp5_units_natural_v200"
LAYER, UNITS, CONGRUENT = 5, (1036, 2403, 2538, 3274, 3034), ("natural_plural_they", "natural_singular_he")
NULL_SEEDS = tuple(range(8001, 8017))
LIVE_FRACTION, LIVE_POSITIVE, GATE_RATIO, INSTRUMENT_TOL = 0.005, 0.60, 0.25, 1e-4
FORWARDS_MAX = 84
PREDICTIONS = {"pred_a_instrument_replays_native": "<= 1e-4", "pred_b_congruent_damage_live": ">= 0.005, positive >= 0.60", "pred_c_units_beat_random_unit_triples": "> random max", "pred_d_units_selective": "three gates", "pred_e_incongruent_shifts_toward_label": "< 0"}
