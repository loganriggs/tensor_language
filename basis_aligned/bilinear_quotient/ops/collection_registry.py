...
LINES = [
    ...
    ("temporal", "temporal will/had", ("will", "had"), ["11.3", "9.1", "15.5", "9.4"], "temporal_auxiliary_dod_removal_v28_result.json", "temporal_auxiliary_dod_random_set_null_v31_result.json", None, "temporal_auxiliary_dod_natural_v34_result.json", None, "TEMPORAL_DOD_SCORECARD.md"),
    ("temporal", "narrative was/is", ("was", "is"), None, "narrative_tense_dod_confirm_v43_result.json", "narrative_tense_dod_random_set_null_v45_result.json", None, None, None, "NARRATIVE_DOD_SCORECARD.md"),
    ...
    ("number", "lexical were/was", ("were", "was"), ["11.3", "5.7", "7.8", "9.7"], "number_family_dod_battery_v55_result.json", "number_family_dod_random_set_null_v60_result.json", None, None, None, "NUMBER_DOD_SCORECARD.md"),
    ...
    ("correlative", "correlative either/not", ("or", "but"), ["16.8", "14.8", "7.8", "8.1"], "correlative_either_not_dod_battery_v119_result.json", "correlative_either_not_dod_random_set_null_v121_result.json", "correlative_either_not_dod_response_census_v122_result.json", None, None, "CORRELATIVE_EITHER_NOT_DOD_SCORECARD.md"),
    ("correlative", "correlative both/neither", ("and", "nor"), ["8.1", "7.8", "16.8", "14.8"], "correlative_both_neither_dod_battery_v120_result.json", "correlative_both_neither_dod_random_set_null_v123_result.json", "correlative_both_neither_dod_response_census_v124_result.json", None, None, "CORRELATIVE_BOTH_NEITHER_DOD_SCORECARD.md"),
...
def census(r):
    if not r: return None
    s = r.get("summary", {}); return {"direct_share": s.get("direct_share_of_linear"), "downstream_net": s.get("downstream_net_mean"), "remainder": s.get("remainder"), "predictions": r.get("predictions")}


def natural(r):
    if not r: return None
    rep = r.get("report", {}).get("congruent")
    if rep: m = rep["removal"]; return {"congruent_damage": m["target_damage_mean"], "congruent_native": m["target_native_mean_margin"], "positive": m["target_damage_positive_fraction"], "null_max": rep["null_damage_max"], "predictions": r.get("predictions")}
    return {"predictions": r.get("predictions"), "keys": sorted(r.keys())[:12]}
