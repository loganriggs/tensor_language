# IS THE COVERAGE EFFECT MONOTONE? -- the one thing §1922 left unexplained.
#
# §1922 confirmed the reliability signal at the deployed 5,419 coverage and reported one detail it could
# not explain: the COVERED gradient is HIGHER at low coverage than at high, on all three roles --
# +9.69 / +10.46 / +10.17 at 5,419 against +9.50 / +9.79 / +9.28 at 16,110. The differences
# (0.19 / 0.67 / 0.89) sit at or above §1916's +-0.077 covered spread on two roles, so it is probably
# real. The UNCOVERED gradient moves the other way on two roles (+3.28 / +3.11 / +2.10 against
# +3.90 / +4.10 / +2.06) and is flat on the third.
#
# Two endpoints cannot distinguish a trend from a pair of measurements. This adds a third coverage,
# 9,054 types (the first 192 fit rows against the deployed 96 and the high 480), and asks whether both
# gradients interpolate. If they do, coverage is a smooth axis for this signal and the effect is a trend
# worth naming; if the middle sits outside both endpoints, it is not a coverage effect at all.
#
# NOTE ON THE COVERAGE FIGURES, because I nearly built this on the wrong ones: these scripts compute
# coverage from fit[:, :T] with the MODULE-LEVEL T = 256, not the full 512-token row width. At T = 256 the
# first 96 / 192 / 288 / 480 rows give 5,419 / 9,054 / 11,954 / 16,110 types; at the full width they give
# 8,172 / 12,959 / -- / 21,858. Measured, not assumed (PRE-FLIGHT B).
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY. Rung 3: §1922's open detail.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
#   pred_a THE COVERED GRADIENT INTERPOLATES: at 9,054 it lies within [min - 0.3, max + 0.3] of the two
#          PUBLISHED endpoints (+9.69 / +10.46 / +10.17 at 5,419; +9.50 / +9.79 / +9.28 at 16,110), on
#          all three roles. If TRUE the effect is a monotone trend in coverage and §1922's observation is
#          a real axis. If FALSE the middle escapes both endpoints and there is no coverage trend --
#          §1922's paragraph would then be describing two builds that differ for some other reason, and I
#          would say so there.
#   pred_b AND SO DOES THE UNCOVERED GRADIENT: same band against +3.28 / +3.11 / +2.10 and
#          +3.90 / +4.10 / +2.06. STATED WEAKNESS: on skip1200 the two endpoints are 2.10 and 2.06, so
#          the band is [1.76, 2.40] and that role tests almost nothing -- this predicate is carried by
#          skip7000 and skip11000, where the endpoints differ by 0.62 and 0.99. Registering the weakness
#          rather than letting a near-degenerate band read as a pass (§1889).
#   pred_c AND §1765 HOLDS A THIRD TIME: within-token margin spread below 1e-3 at 9,054, on both arms.
#          Exactly 0.00e+00 at 5,419 (§1922) and <= 2.86e-05 at 16,110 (§1913/§1918).
#   pred_d CONTROLS: coverage is exactly 9,054 -- the measured T = 256 figure for 192 fit rows; zero
#          lookup misses across the whole vocabulary; and the uncovered share is REPORTED, since it
#          should sit between 24% (5,419) and 10% (16,110) if the axis is smooth.
...
OUT = PT + 'ops/coverage_trend_results.json'
...
NCOV = 9054
FIT_LIMIT = 192   # first 192 rows of n480 -> 9,054 types at T=256 (measured, not assumed)
S1789_COV = 5419
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'COVERAGE TREND at 9054 | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    LO_C = {'skip7000': 9.69, 'skip11000': 10.46, 'skip1200': 10.17}   # §1922 at 5,419
    HI_C = {'skip7000': 9.50, 'skip11000': 9.79, 'skip1200': 9.28}     # §1918 at 16,110
    LO_U = {'skip7000': 3.28, 'skip11000': 3.11, 'skip1200': 2.10}
    HI_U = {'skip7000': 3.90, 'skip11000': 4.10, 'skip1200': 2.06}

    def band(v, a, b):
        return min(a, b) - 0.3 <= v <= max(a, b) + 0.3
    uncn = {e: sum(u(e, i8, 'n') for i8 in range(4)) for e in roles}
    totn = {e: uncn[e] + sum(q(e, 'tok', i8, 'n') for i8 in range(4)) for e in roles}
    pa = all(band(gcov[e], LO_C[e], HI_C[e]) for e in roles)
    pb = all(band(gunc[e], LO_U[e], HI_U[e]) for e in roles)
    pc = all(2.0 * dev[e] < 1e-3 for e in roles)
    pd = (ncov == NCOV and all(miss[e] == 0 for e in roles))

    print(f'\n  gradients at coverage {ncov}, against the two published endpoints:', flush=True)
    for e in roles:
        print(f'    {e:10s} COVERED   5,419 {LO_C[e]:+.2f}  ->  {ncov} {gcov[e]:+.2f}  ->  16,110 '
              f'{HI_C[e]:+.2f}', flush=True)
        print(f'    {"":10s} UNCOVERED 5,419 {LO_U[e]:+.2f}  ->  {ncov} {gunc[e]:+.2f}  ->  16,110 '
              f'{HI_U[e]:+.2f}   (n {uncn[e]})', flush=True)
    print(f'\n  the COVERED gradient interpolates -> {pa}  ' + '  '.join(
        f'{e} {gcov[e]:+.2f} in [{min(LO_C[e], HI_C[e]) - 0.3:+.2f}, {max(LO_C[e], HI_C[e]) + 0.3:+.2f}]'
        for e in roles), flush=True)
    print(f'  and so does the UNCOVERED one -> {pb}  ' + '  '.join(
        f'{e} {gunc[e]:+.2f} in [{min(LO_U[e], HI_U[e]) - 0.3:+.2f}, {max(LO_U[e], HI_U[e]) + 0.3:+.2f}]'
        for e in roles) + '   (skip1200 band near-degenerate, see docstring)', flush=True)
    print(f'  and §1765 holds a THIRD time (< 1e-3) -> {pc}  ' + '  '.join(
        f'{e} <= {2.0 * dev[e]:.2e}' for e in roles), flush=True)
    print(f'  coverage {ncov}, zero misses -> control {pd}; uncovered share ' + '  '.join(
        f'{e} {uncn[e] / max(totn[e], 1):.1%}' for e in roles) + '  (24% at 5,419, 10% at 16,110)',
        flush=True)
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_covered_interpolates': bool(pa),
                          'pred_b_uncovered_interpolates': bool(pb),
                          'pred_c_s1765_third_time': bool(pc),
