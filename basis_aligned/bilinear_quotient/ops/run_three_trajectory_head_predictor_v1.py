#!/usr/bin/env python3
# BQGATE:360bodyforwards;360finalreadouts;120prefixes<=248tokens;180seconds.
"""pred_a priorN/C/R native outcomes andfinaladditivebackground<=1e-4relative.
pred_b fullmixed directwrite effect predicts nativehead effect<=.02 allfivegroups.
pred_c compactmixed effect preserves priorvalidatedcompact<=.01 allfivegroups.
Null: removingP/A reference executions exposes hidden input dependency or numerical mismatch.
Price360fullforwards+360finalreadouts;120reusedprefixes;180seconds;no fitting.
"""
...
STEM='THREE_TRAJECTORY_HEAD_PREDICTOR_V1'
@torch.no_grad()
def main():
 ...
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('360forwards+360readouts;onlyN/C/R;projectedthree-cornerinterface');return
 ...
 try:
  for i,row in enumerate(rows):
   ...
   final=ctx['h17'];bar=final[1]+final[3]-final[0];ctx['readout_states']=[bar,bar+full,bar+small]
 ...
 for group in range(5):
  lo,hi=24*group,24*(group+1);z=readouts[lo:hi,:,0];prior=old['readouts'][lo:hi,:,0];full=z[:,1]-z[:,0];small=z[:,2]-z[:,0];native=prior[:,1]-prior[:,0];previous=prior[:,5]-prior[:,0]
  cells.append(dict(group=group,full_effect_error=rel(full,native),compact_preservation_error=rel(small,previous),compact_native_effect_error=rel(small,native),compact_same_nonzero_sign=int((small*native>0).sum()),native_zero_count=int((native==0).sum()),maxabs_full_error=float((full-native).abs().max())))
 result={'pred_a':max(replay)<=1e-4,'pred_b':all(c['full_effect_error']<=.02 for c in cells),'pred_c':all(c['compact_preservation_error']<=.01 for c in cells),'anchor_replay':replay,'cells':cells,'body_forwards':count,'extra_final_readouts':360,'seconds':time.perf_counter()-tic,'executed_arms':[0,1,3],'scope':'Actual three-trajectory conditional predictor. No P/A execution. Rawprojection/norm/crossscalar producers and nativefinalbackground remain.120reusedregionalprefixes, no newOOD or autonomous tokenmodel.'}
