...
"""v2: same five behaviours and greedy protocol; joint DAS now targets the EXACT-SET patch.

WHY A V2. v1 (`unit_greedy_protocol_v1_result.json`) fitted the joint direction to the DONOR'S
margin. A 2-4 unit set only reaches ~0.55 of that, so the optimizer was pushed past the set's own
effect and found steering directions: held-out recoveries 1.5-4.2, P 0.42 / C 0.61 on polarity.
That is not a subspace OF the set's effect. v2 fits the direction to reproduce what the exact
interchange of the chosen set does, which bounds it by the set, adds a random-direction baseline
of the same shape, and registers an upper bound as well as a lower one. Greedy results are
re-measured, not copied, so the receipt stands alone.
...
REGISTERED BEFORE THE RUN
    ...
    pred_c_chosen_set_is_selective
        ...
        patch of that set.
    pred_d_joint_rank1_is_a_subspace_of_the_set_effect
        rank-1 joint DAS on the set recovers between 0.50 and 1.20 of the exact-set joint on
        held-out A1 AND on A2, with P <= 0.20 and C <= 0.35 through the subspace, for every
        behaviour whose set reached the bar. Either bound failing fails the prediction.
    pred_f_random_direction_is_inert
        a random rank-1 direction of the same shape recovers <= 0.10 of the exact-set joint on
        held-out A1, on every behaviour.
...
OUT = ROOT / "circuits/followups/unit_greedy_protocol_v2_result.json"
...
SINGLE_BAR, P_BAR, C_BAR, TRANSFER_LO, TRANSFER_HI, RANDOM_MAX = 0.50, 0.20, 0.35, 0.50, 1.20, 0.10
DAS_STEPS = 200
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 6000, 200000


def _plan():
    return {"candidate_id": "corpus.unit_greedy_protocol_v2",
...
def main() -> None:
    ...
    for label, module, concentrated in BEHAVIOURS:
        ...
        for rank in sorted({1, len(chosen)}):
            ...
            das[f"rank_{rank}"] = {
                "objective": "match_exact_set_patch", "loss_history": hist,
                "random_baseline_a1_heldout": _das_eval(backend, held_prep, chosen, rq),
    ...
    def _tr(k):
        ...
        return (all(TRANSFER_LO <= (d[f]["fraction_of_exact"] or 0.0) <= TRANSFER_HI
                    for f in ("a1_heldout", "a2"))
                and d["p_effect"] <= P_BAR and d["c_effect"] <= C_BAR)
    pred_d = bool(reached) and all(_tr(k) for k in reached)
    pred_f = all((report[k]["das"]["rank_1"]["random_baseline_a1_heldout"]["fraction_of_exact"]
                  or 0.0) <= RANDOM_MAX for k in report)
    ...
    predictions = {
        ...
        "pred_d_joint_rank1_is_a_subspace_of_the_set_effect": bool(pred_d),
        "pred_e_sets_are_subadditive": bool(pred_e),
        "pred_f_random_direction_is_inert": bool(pred_f)}
    result = {"schema": "circuit_unit_greedy_protocol_result_v2",
              "candidate_id": "corpus.unit_greedy_protocol_v2",
              "instrument": instrument,
              "registered": {"pool": POOL, "target": TARGET, "min_gain": MIN_GAIN,
                             "max_units": MAX_UNITS, "single_bar": SINGLE_BAR, "p_bar": P_BAR,
                             "c_bar": C_BAR, "transfer_band": [TRANSFER_LO, TRANSFER_HI],
                             "random_max": RANDOM_MAX, "das_objective": "match_exact_set_patch",
                             "das_steps": DAS_STEPS},
