# DOES THE PROGRAM INHERIT THE MODEL'S DECIDEDNESS? -- §1911's open question.
#
# §1911 found the compiled program's agreement with the live model roughly DOUBLES with the model's own
# confidence: 5.36 / 5.06 / 5.80x on the least-decided quartile of live top-2 margin, 10.11 / 10.34 /
# 10.87x on the most-decided. Correctness does not modulate it (§1895) and sequence position does not
# (§1910), so confidence is the only variable of the three that does.
#
# That leaves an obvious question with a mechanical answer. §1911 measured only the MODEL's margin. If the
# program's own margin tracks it -- if the program is also decided exactly where the model is -- then the
# gradient is inheritance and there is nothing further to explain. If the two margins are largely
# independent and the gradient survives conditioning on the program's own confidence, then §1911 is a
# statement about the model's decidedness alone, which is stranger and more interesting.
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY. Rung 3: §1911's open question.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
# Covered arm throughout, named per LESSON 71. Predicates are written against THIS docstring, not
# inherited -- §1911's copied block encoded the opposite direction and was caught only by reading it.
#   pred_a THE MARGINS ARE CORRELATED: Spearman between the live top-2 margin and the PROGRAM's top-2
#          margin, over covered scored positions, is at least +0.4 on all three roles. If TRUE the
#          program is decided roughly where the model is and §1911's gradient is inheritance. If FALSE
#          the two decidedness signals are largely independent, and §1911 says the program follows the
#          model best where the MODEL is sure regardless of whether the program itself is -- which no
#          account in this arc predicts.
#   pred_b AND THE GRADIENT SURVIVES CONDITIONING: within the HIGHEST live-margin quartile, enrichment
#          in the top half by PROGRAM margin exceeds that in the bottom half by at least 1.0x, on all
#          three roles. This separates "the program inherits decidedness" from "the model's decidedness
#          alone drives it". If FALSE, the program's own confidence adds nothing once the model's is
#          fixed, and §1911 is entirely a statement about the model.
#   pred_c AND THE PROGRAM'S MARGIN PREDICTS ITS OWN ACCURACY: program top-1 accuracy in the top half by
#          program margin exceeds the bottom half by at least 20 percentage points, within that same
#          live-margin quartile. A known-answer check on the program-margin axis, exactly as §1910's and
#          §1911's pred_c were on theirs -- §1889 is why every new axis gets one.
#   pred_d CONTROLS: coverage is exactly 16,110; the four live-margin quartiles reproduce §1911's
#          PUBLISHED enrichments (5.36 / 5.74 / 6.23 / 10.11 on skip7000) within 0.05; and the
#          covered-arm baseline is computed in-run and reported as an enrichment.
...
OUT = PT + 'ops/margin_inheritance_results.json'
...
PB = ((0, 0), (1, 1), (2, 2), (3, 3))   # live-margin QUARTILES
...
def compare_by_bucket(rows, hooks, seenmask=None):
    ...
    both = {'lm': [], 'pm': []}
    sub = {h: {'l': [], 'p': [], 'n': 0, 'ok': 0} for h in ('hi', 'lo')}
    ...
    LM, PM = torch.cat(both['lm']).double(), torch.cat(both['pm']).double()
    def _rk(v):
        return torch.argsort(torch.argsort(v)).double()
    ra, rb = _rk(LM), _rk(PM)
    out['margin_spearman'] = float(((ra - ra.mean()) * (rb - rb.mean())).sum()
                                   / (ra.std(unbiased=False) * rb.std(unbiased=False) * ra.numel()))
    for h in ('hi', 'lo'):
        out[f'hiq_{h}'] = {**_en(sub[h]['l'], sub[h]['p']), 'n': sub[h]['n'],
                           'prog_acc': sub[h]['ok'] / max(sub[h]['n'], 1)}
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'MARGIN INHERITANCE: does the program inherit the model\'s decidedness? | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    rho = {e: top(e, 'margin_spearman') for e in roles}
    hi = {e: top(e, 'hiq_hi') for e in roles}
    lo = {e: top(e, 'hiq_lo') for e in roles}
    engap = {e: (hi[e]['enrichment'] or 0) - (lo[e]['enrichment'] or 0) for e in roles}
    accgap = {e: hi[e]['prog_acc'] - lo[e]['prog_acc'] for e in roles}
    pa = all(rho[e] >= 0.4 for e in roles)
    pb = all(engap[e] >= 1.0 for e in roles)
    pc = all(accgap[e] >= 0.20 for e in roles)
    pd = (ncov == NCOV
          and all(abs((cl2('skip7000', BK[i2], 'enrichment') or 0) - S1911_EN['skip7000'][i2]) <= 0.05
                  for i2 in range(4)))

    print(f'\n  live-margin quartiles (control, vs §1911):', flush=True)
    for e in roles:
        print(f'    {e:10s} ' + '  '.join(
            f'q{k4[1]} {(cl2(e, k4, "enrichment") or 0):5.2f}x' for k4 in BK), flush=True)
    print(f'\n  THE MARGINS ARE CORRELATED (Spearman >= +0.4) -> {pa}  ' + '  '.join(
        f'{e} {rho[e]:+.3f}' for e in roles), flush=True)
    print(f'  and the gradient SURVIVES conditioning (>= 1.0x within q3) -> {pb}  ' + '  '.join(
        f'{e} prog-hi {(hi[e]["enrichment"] or 0):.2f}x (n{hi[e]["n"]}) vs prog-lo '
        f'{(lo[e]["enrichment"] or 0):.2f}x (n{lo[e]["n"]}) = {engap[e]:+.2f}x' for e in roles),
        flush=True)
    print(f'  and the PROGRAM margin predicts its own accuracy (>= 20pp) -> {pc}  ' + '  '.join(
        f'{e} {lo[e]["prog_acc"]:.1%} -> {hi[e]["prog_acc"]:.1%} ({accgap[e] * 100:+.1f}pp)'
        for e in roles), flush=True)
    print(f'  coverage {ncov}, §1911 quartiles reproduce -> control {pd}; baseline ' + '  '.join(
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_margins_correlated': bool(pa),
                          'pred_b_gradient_survives_conditioning': bool(pb),
                          'pred_c_program_margin_predicts_accuracy': bool(pc),
