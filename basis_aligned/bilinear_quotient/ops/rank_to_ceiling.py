# IS §1768's 0.54 NATS REALLY THE TABLE RANK?  -- testing §1848's attribution instead of assuming it.
#
# §1848 found the settled FULL-RANK 36-site program equals the model's own per-token ceiling on covered
# positions EXACTLY -- program minus ceiling = -0.00000 nats on all three roles -- while §1768 published
# that program as 0.54 nats SHORT of the same ceiling. I attributed the difference to table rank: §1768's
# object is the rank-64 table with rank-128 corrections, mine was full rank. That attribution came from
# reading the two scripts, NOT from a measurement, and §1848's own pred_d failure was caused by exactly
# this kind of unstated build difference (LESSON 53, third instance).
#
# This measures it. Covered-position CE against the ceiling at table ranks full, 256, 64, 16 and 4, using
# the same build, the same ceiling lookup and the same population. If the rank-64 arm lands on §1768's
# published 6.57512 / 6.57289, the object is positively identified and §1848's attribution stops being an
# inference. If it does not, something other than rank separates the two programs and §1848's closing
# paragraph -- which declares the position-wise class exhausted -- is resting on a guess.
#
...
# failure branches enumerated per LESSON 44, anchors named with their BUILD per LESSONS 53's addendum:
#   pred_a RANK 64 REPRODUCES §1768's PUBLISHED PROGRAM: the rank-64 covered-position CE matches
#          §1768's 6.57512 (skip7000) and 6.57289 (skip11000) within 0.02 nats. If TRUE §1768's object is
#          positively identified as the rank-64 build, §1848's attribution is measured rather than
#          inferred, and the 0.54 nats is priced to the rank axis where §1813/§1816 already live. If
#          FALSE the two programs differ by something else -- corrections, fallback, or fit rows -- and
#          §1848's "the class has no internal headroom left" has to be withdrawn until that is found.
#   pred_b AND FULL RANK ATTAINS THE CEILING: the full-rank covered CE differs from the ceiling by under
#          0.001 nats on every role, reproducing §1848's -0.00000. If FALSE §1848 does not replicate and
#          nothing else in this run means anything.
#   pred_c AND THE APPROACH IS MONOTONE IN RANK: covered CE is non-increasing as rank rises through
#          4, 16, 64, 256, full, on every role. If FALSE the rank axis is not a clean ladder toward the
#          ceiling and "it is the rank" is too simple a statement even if pred_a passes.
#   pred_d CONTROLS: the ceiling reproduces §1768's PUBLISHED 6.03465 / 5.97902 within 0.01; live covered
#          CE reproduces §1768's PUBLISHED 3.29205 / 3.09711; the full-rank ALL-POSITION CE reproduces
#          §1811's PUBLISHED SETTLED FULL-RANK 6.01167 / 5.98477 / 6.00165 within 0.01 -- the second
#          anchor whose disagreement with the first localised §1848's error. Coverage 5419 of 50257.
...
OUT = PT + 'ops/rank_to_ceiling_results.json'
...
def main():
    ...
    print(f'RANK TO CEILING | covered CE vs the ceiling at table ranks full/256/64/16/4 | '
    ...
    pa = all(abs(curve['64'][e]['cov'] - S1768_PROG_RANK64_CORR128[e]) <= 0.02 for e in r64)
    pb = all(abs(curve['full'][e]['cov'] - ceilv[e]) < 0.001 for e in roles2)
    lad = ['4', '16', '64', '256', 'full']
    pc = all(curve[lad[i]][e]['cov'] <= curve[lad[i - 1]][e]['cov'] + 1e-9
             for e in roles2 for i in range(1, len(lad)))
    pd = (all(abs(ceilv[e] - S1768_CEIL_LENGTH1_LOOKUP[e]) <= 0.01 for e in r64)
          and all(abs(livev[e] - S1768_LIVE_COVERED[e]) <= 0.01 for e in r64)
          and all(abs(curve['full'][e]['allpos'] - S1811_SETTLED_FULLRANK_ALLPOS[e]) <= 0.01
                  for e in roles2 if e in S1811_SETTLED_FULLRANK_ALLPOS)
          and NFULL == NCOV)
    ...
    print(f'\n  rank 64 reproduces §1768 PUBLISHED rank-64 program -> {pa}  ' + '  '.join(
        f'{e} {curve["64"][e]["cov"]:.5f} vs {S1768_PROG_RANK64_CORR128[e]:.5f}' for e in r64),
        flush=True)
    print(f'  and FULL rank attains the ceiling (<0.001) -> {pb}  ' + '  '.join(
        f'{e} {curve["full"][e]["cov"] - ceilv[e]:+.6f}' for e in roles2), flush=True)
    print(f'  and the approach is MONOTONE in rank -> {pc}', flush=True)
    print(f'  ceiling/live §1768, full-rank all-position §1811 -> control {pd}', flush=True)

    json.dump({'run': 'rank_to_ceiling', 'ranks': [str(r) for r in TRANKS],
               'ceiling_covered': ceilv, 'live_covered': livev, 'curve': curve,
               'predictions': {'pred_a_rank64_reproduces_S1768': bool(pa),
                               'pred_b_full_rank_attains_ceiling': bool(pb),
