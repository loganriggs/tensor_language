...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_c69_perlayer_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; one arm + nine tail-prefix evals on FR (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_tail_prefix_results.json'
...
if __name__=='__main__':
    T00=time.time()
    SEL['mode']='norm'; SEL['K']=2304; SEL['K69']=576; SEL['K69MAP']={}; SEL['prefix_tail']=True
    print('SINGLE ARM: best middles (mlp45-2304 + c69-576) + nine tail-prefix evals',flush=True)
    r_b=main()
    marg=SEL['prefix_result']['marginals']; pl=SEL['prefix_result']['prefix']
    import statistics as stt
    med=stt.median(marg)
    pa=abs(r_b['L2_F']-2.6691)<=0.01
    pb=all(m>=-0.005 for m in marg)
    pc=max(marg)>=2*max(med,1e-9)
    res={'best':r_b,'prefix_L2F':pl,'marginals_a10L_to_a17L':marg,'median_marginal':round(med,4),
         'convention':'L2 = CE above the real model; lower is better; marginal = damage added by that replacement',
         'pred_a_reproduces_s2140':bool(pa),'pred_b_every_replacement_costs':bool(pb),
         'pred_c_concentrated':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'marginals a10L..a17L: {marg}  (median {med:+.4f}, max {max(marg):+.4f})')
    print(f"(a) full L2_F {r_b['L2_F']} vs 2.6691 (tol 0.01): {'HELD' if pa else 'FAILED'}")
    print(f"(b) all marginals >= -0.005: {'HELD' if pb else 'FAILED'}")
    print(f"(c) max {max(marg):+.4f} >= 2 x median {med:+.4f}: {'HELD' if pc else 'FAILED'}")
