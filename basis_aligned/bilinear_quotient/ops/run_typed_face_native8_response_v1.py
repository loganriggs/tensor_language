...
from regional_paired_write_runtime_v4 import measure,CONTROL_PAIRS
from run_even_value_factorial_native_v1 import setup
from typed_face_write_atoms_v1 import native
STEM='TYPED_FACE_NATIVE8_RESPONSE_V1'
...
def main():
 ...
 assert all(len(r['ids'])-1 not in r['destination_positions'] for r in groups)
 ...
 try:m=measure(model,graph,groups,['native','native8_midpoint'],write)
 finally:
  for handle in handles:handle.remove()
 v=expand(m['values'],mapping,6);old=torch.load(P/'TYPED_FACE_NATIVE8_SCOPE_V1_ARTIFACT.pt',weights_only=True)['values'][[0,2]]
