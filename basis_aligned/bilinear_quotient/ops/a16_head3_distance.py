"""IS HEAD 16.3 THE LONG-RANGE CARRIER (rung 69).

CONVENTION (S2135): per-position CE(skip-a16 with head 16.3 zeroed, bases excluded) - CE(skip-a16, attn16 real)
on FR; positive = head 16.3 was helping there. Signed-sum bars per S2159.

S2162: the a16 read rises with distance (rho 0.810, peak 32-64 tokens, half the mass beyond 32). S2153: 16.3 is
the top head (+0.054 window-median). Does 16.3 specifically carry the far mass? NULL: 16.3's damage has the
same ~49/51 near/far profile as the whole module.

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) FAR CARRIES: distance > 32 positions carry >= 0.6 of head 16.3's summed ind damage.
  (b) RISING PROFILE: Spearman rho >= 0.5 between log2-distance bin and mean damage (bins with >= 100).
  (c) REPRODUCTION: the skip-a16 base arm reproduces S2146 per-window (median |delta| <= 0.005).
  TRIPWIRE: the zeroed arm's per-position CE must differ from base.

Writes a16_head3_distance_results.json. Self-reviewed. RUNG-68 HEADER FOLLOWS.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['a16_distance_split_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; head-16.3 distance split on FR (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'a16_head3_distance_results.json'
...
def main():
    ...
    baseF=evalT(FR,120,[])
    _hz2=[]
    if SEL.get('zerohead') is not None:
        _hh=SEL['zerohead']; _HDz=D//9
        def _zp(mod,args):
            x=args[0].clone(); x[...,_hh*_HDz:(_hh+1)*_HDz]=0
            return (x,)+tuple(args[1:])
        _hz2.append(m.transformer.h[16].attn.c_proj.register_forward_pre_hook(_zp))
        print(f'attn16 head {_hh} zeroed for this arm (bases excluded)',flush=True)
...
if __name__=='__main__':
    ...
    SEL['clsdmg']=True; SEL['skip16']=True; SEL['zerohead']=None
    print('ARM 1/2: skip-a16 base (attn16 real), per-position CE captured',flush=True)
    r_base=main()
    cev_base=SEL['cev']; clsflat=SEL['clsflat']; FRtok=SEL['FRtok']
    SEL['zerohead']=3
    print('ARM 2/2: skip-a16 with head 16.3 zeroed, per-position CE captured',flush=True)
    r_z=main()
    cev_z=SEL['cev']
    import statistics as stt
    if float((cev_z-cev_base).abs().sum())<1e-3:
        raise SystemExit('INSTRUMENT FAIL: zeroed arm identical to base - hook inert')
    prev=json.load(open(PT+'frontier_skip_a16_results.json'))['skip_a16']['fresh8']
    rp=[abs(round(a-b,4)) for a,b in zip(r_base['fresh8'],prev)]
    diff=(cev_z-cev_base)
    IND=CLS.index('ind')
    dist=torch.full((len(clsflat),),-1,dtype=torch.long)
    for r6 in range(FRtok.shape[0]):
        toks=FRtok[r6,:257].tolist()
        last={}
        for p6 in range(256):
            tgt=toks[p6+1]
            if tgt in last and clsflat[r6*256+p6]==IND:
                dist[r6*256+p6]=p6-last[tgt]
            last[toks[p6]]=p6
    ok=(clsflat==IND)&(dist>0)
    far=ok&(dist>32)
    s_far=float(diff[far].sum()); s_all=float(diff[ok].sum())
    bins={}
    for b6 in range(9):
        lo,hi=2**b6,2**(b6+1)
        mb=ok&(dist>=lo)&(dist<hi)
        if int(mb.sum())>=100:
            bins[b6]={'n':int(mb.sum()),'mean':float(diff[mb].mean())}
    bk=sorted(bins); means=[bins[b]['mean'] for b in bk]
    def rank(v):
        o=sorted(range(len(v)),key=lambda i2:v[i2]); r7=[0]*len(v)
        for pos,i2 in enumerate(o): r7[i2]=pos
        return r7
    n7=len(means)
    rho=1-6*sum((a-b)**2 for a,b in zip(rank(means),list(range(n7))))/(n7*(n7*n7-1)) if n7>2 else 0.0
    pa=s_far>=0.6*s_all
    pb=rho>=0.5
    pc=stt.median(rp)<=0.005
    res={'base':r_base,'head3_zeroed':r_z,
         'sum_far32':round(s_far,1),'sum_all_ind':round(s_all,1),
         'bins_log2':{str(b):{'n':bins[b]['n'],'mean':round(bins[b]['mean'],5)} for b in bk},
         'spearman':round(rho,3),
         'convention':'per-position CE(head 16.3 zeroed) - CE(skip-a16 base) on FR; positive = the head was helping',
         'pred_a_far_carries':bool(pa),'pred_b_rising':bool(pb),'pred_c_reproduces_s2146':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print('log2 bins (n, mean): '+' '.join(f'2^{b}:{bins[b]["n"]}/{bins[b]["mean"]:+.4f}' for b in bk))
    print(f"(a) far(>32) {s_far:+.1f} >= 0.6 x all {s_all:+.1f}: {'HELD' if pa else 'FAILED'}")
    print(f"(b) Spearman {rho:+.3f} >= 0.5: {'HELD' if pb else 'FAILED'}")
