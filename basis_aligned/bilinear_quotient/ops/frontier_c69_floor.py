...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_halving_attrib_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; c69 floor arms (1152/576/288) x eight windows (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_c69_floor_results.json'
...
if __name__=='__main__':
    ...
    print('ARM 1/4: norm-2304 everywhere (S312 reproduction)',flush=True)
    r_norm=main()
    SEL['K69']=1152
    print('ARM 2/4: c6-c9 at 1152 (S2139 reproduction)',flush=True)
    r_h=main()
    SEL['K69']=576
    print('ARM 3/4: c6-c9 at 576',flush=True)
    r_q=main()
    SEL['K69']=288
    print('ARM 4/4: c6-c9 at 288 (descriptive)',flush=True)
    r_e=main()
    import statistics as stt
    d1=[round(a-b,4) for a,b in zip(r_h['fresh8'],r_norm['fresh8'])]
    d5=[round(a-b,4) for a,b in zip(r_q['fresh8'],r_norm['fresh8'])]
    d2=[round(a-b,4) for a,b in zip(r_e['fresh8'],r_norm['fresh8'])]
    pa=stt.median(d5)<=0.01
    pb=sum(g<=0.02 for g in d5)>=6
    pc=abs(stt.median(d1)-(-0.0118))<=0.015
    res={'norm2304':r_norm,'c69_1152':r_h,'c69_576':r_q,'c69_288':r_e,
         'per_window_damage_c69_1152':d1,'per_window_damage_c69_576':d5,'per_window_damage_c69_288':d2,
         'convention':'L2 = CE above the real model; lower is better; positive entries = damage added',
         'pred_a_quarter_free':bool(pa),'pred_b_windows_floor':bool(pb),'pred_c_reproduces_s2139':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'damage added, c69-1152, per window: {d1}')
    print(f'damage added, c69-576, per window: {d5}')
    print(f'damage added, c69-288, per window: {d2}')
    print(f"(a) c69-576 median {stt.median(d5):+.4f} <= +0.01: {'HELD' if pa else 'FAILED'}")
    print(f"(b) <= +0.02 on {sum(g<=0.02 for g in d5)}/8 (bar 6): {'HELD' if pb else 'FAILED'}")
    print(f"(c) c69-1152 median {stt.median(d1):+.4f} vs -0.0118 (tol 0.015): {'HELD' if pc else 'FAILED'}")
