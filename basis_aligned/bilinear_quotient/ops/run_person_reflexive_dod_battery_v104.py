...
"""Reflexive person I/you (myself vs yourself) DoD battery (v104): the atlas top-4 set ((8, 1), (13, 1), (10, 5), (15, 1)) on FRESH rows -- a PERSON readout family candidate.
...
a person-marked pronoun token). Frozen before access: F* = 0.46 +/- 0.15 (pred_g). Readers avoid person/number marking: will-would,
who-which, night-day (was-were is person-marked for I / you).

ROWS (fresh; `dod_lexicon.fresh`): 16 fresh agents x 16 fresh objects, three frames the module does not use, {I -> " myself", you -> " yourself"}.
96 rows in 3 batches. Battery as v71/v76/v85/v97 (`dod_battery.run`). Evidence tag: edit, fresh rows.

PREDICTIONS (scored as written by `dod_battery.run`; failures preserved): as v71, pred_g band 0.46 +/- 0.15 per construction.
...
CANDIDATE_ID = "reflexive_person.i_vs_you.dod_battery_v104"
OUT = "person_reflexive_dod_battery_v104_result.json"
HEADS = ((8, 1), (13, 1), (10, 5), (15, 1))
FROZEN, BAND = 0.46, 0.15
PREDICTIONS = {"pred_a_instrument_replays_native": "<= 1e-4", "pred_b_native_capability": ">= 0.85 per cell",
               ...
               "pred_g_set_fraction_within_band_of_frozen": "0.46 +/- 0.15 per construction"}
...
CONSTRUCTIONS = {"after": lambda agent, obj, cue: f"After the {obj} fell, {cue} clearly blamed",
                 "say": lambda agent, obj, cue: f"The {agent}s say that {cue} rarely trust",
                 "by": lambda agent, obj, cue: f"By the {obj} {cue} then reminded"}
    ...
    for construction, make in CONSTRUCTIONS.items():
        for g, agent in enumerate(agents):
            for present in (True, False):
                text = make(agent, objects[g], "I" if present else "you")
                ...
                rows.append(L.Row(hashlib.sha256(json.dumps(["person_reflexive_v104", construction, g, present, text]).encode()).hexdigest()[:24],
