#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_hub_three_replay pred_c_top_10_carry_two_thirds pred_d_top_30_carry_most pred_e_nested_sets_monotone
"""MLP 8's top agreement units swapped at the noun (v543). v542: at the noun's own position on the 43 adjacent rows MLP 8 swapped whole closes 0.295
of the agreement gap, while its three named hub units 829 / 953 / 1030 close 0.114 (v460 / v461). v454's exact per-unit fold of MLP 8's direct write on
u_v ranks 829, 1738, 3858, 149, 1030, ... (top 10 = 0.83 of the fold, top 30 = 0.90). Here the ranked units swapped at the noun in nested sets --
TOP3 [829, 1738, 3858], TOP10, TOP30 (v454's list) -- with the hub three as replay: how much of the block's 0.295 is carried by units the fold names?
PREDICTIONS (scored as written; failures preserved; priors from v454 / v460 / v542)
    pred_a_baseline_replays        the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_hub_three_replay        829 / 953 / 1030 alone close 0.114 +- 0.02 (v460 / v461)
    pred_c_top_10_carry_two_thirds the fold's top 10 units close >= 0.20 (two thirds of the block's 0.295)
    pred_d_top_30_carry_most       the fold's top 30 units close >= 0.25
    pred_e_nested_sets_monotone    TOP3 <= TOP10 <= TOP30 <= 0.295 + 0.03 (units add toward the block; no overshoot)
...
OUT = ROOT / "circuits/followups/mlp8_top_units_adjacent_v543_result.json"
CANDIDATE_ID = "chain.mlp8_top_units_adjacent_v543"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, HUB, BLOCK, BAND, TOP10_MIN, TOP30_MIN, OVER = 1e-3, 0.114, 0.295, 0.02, 0.20, 0.25, 0.03
import random
TOP = (829, 1738, 3858, 149, 1030, 1484, 4000, 953, 684, 3372, 3205, 2523, 2014, 1395, 1959, 2733, 1427, 2583, 2832, 3869, 368, 2576, 2596, 521, 3039, 2087, 3709, 4565, 2428, 1192)
UNIT_SETS = {"HUB3": {8: (829, 953, 1030)}, "TOP3": {8: TOP[:3]}, "TOP10": {8: TOP[:10]}, "TOP30": {8: TOP}}
...
FORWARDS_MAX = 11
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_hub_three_replay": "0.114 +- 0.02", "pred_c_top_10_carry_two_thirds": ">= 0.20", "pred_d_top_30_carry_most": ">= 0.25", "pred_e_nested_sets_monotone": "TOP3 <= TOP10 <= TOP30 <= 0.325"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "hub_v460": HUB, "block_v542": BLOCK, "band": BAND, "top10_min": TOP10_MIN, "top30_min": TOP30_MIN, "over": OVER}}
    ...
    def run(seqs, pos, mode):
        ...
        with torch.no_grad():
            ...
            for l, block in enumerate(blocks):
                ...
                if mode in UNIT_SETS and l in UNIT_SETS[mode]:
                    mlp = block.mlp; Lx, Rx = mlp.Left(xm), mlp.Right(xm); hdn = ((F.silu(Lx) * Rx) if model.config.gated else (Lx * Rx)).clone()
                    units = torch.tensor(UNIT_SETS[mode][l], device="cuda"); hdn[idx[:, None], pp[:, None], units[None, :]] = hdn[swap[:, None], pp[:, None], units[None, :]].clone()
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_hub_three_replay": abs(v["HUB3"] - HUB) <= BAND, "pred_c_top_10_carry_two_thirds": v["TOP10"] >= TOP10_MIN,
                   "pred_d_top_30_carry_most": v["TOP30"] >= TOP30_MIN, "pred_e_nested_sets_monotone": v["TOP3"] <= v["TOP10"] <= v["TOP30"] <= BLOCK + OVER}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "mlp8_top_units_adjacent_result_v543", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
