...
pred_b warmsharedbatched>=10%faster than nativebatched at12pairs bothprefixes.
pred_c warmsharedbatched no slower than nativebatched at1pair bothprefixes.
...
@torch.no_grad()
def main():
    files=json.loads((P/'COMPOSED_JOINT_GPU_WARM_V1_BINDING.json').read_text())['files']
    ...
    assert not (P/'COMPOSED_JOINT_GPU_WARM_V1_RESULT.json').exists()
    ...
    for index in (0,96):
        ...
        warm_context=prepare(*arguments)
        def shared_warm_batch(count):
            context=dict(warm_context);values=torch.cat(amplitudes(count),dim=0)
            context['first_values']=arguments[4].expand(values.shape[0],*arguments[4].shape[1:])
            return post(branch(values,context))
        variants={'native_serial':native_serial,'native_batch':native_batch,'shared_serial':shared_serial,'shared_batch':shared_batch,'shared_warm_batch':shared_warm_batch}
        for count in (1,4,12):
            ...
            cells.append(dict(row=index,tokens=len(rows[index]['ids']),pairs=count,state_errors=errors,
                              ...
                              matched_batched_speed_ratio=medians['native_batch']/medians['shared_warm_batch'],inclusive_speed_ratio=medians['native_batch']/medians['shared_batch']))
    ...
    result={'pred_a':all(max(c['state_errors'].values())<=1e-5 for c in cells),
            ...
            'scope':'Native FP32 MLP9/attention10/MLP10 vs validated FP64 generator followed by FP32 MLP10. Both serial and batched on fixed historical rows0/96,1/4/12amplitude pairs. Includes original shared preparation; additional warm variant excludes preparation for a repeated identical context. Synchronized wall time. Warm speed cannot be claimed for single new contexts. Pristine context and suffix excluded equally; no full-model performance or freshOOD claim.'}
    (P/'COMPOSED_JOINT_GPU_WARM_V1_RESULT.json').write_text(json.dumps(result,indent=2)+'\n')
