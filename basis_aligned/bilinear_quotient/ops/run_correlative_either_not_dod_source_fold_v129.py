...
"""Correlative either/not DoD (v129): SOURCE FOLD of the correlative set on its fresh rows (v80's exact fold). Parents: the correlative battery, v112 / v115 (the
person family's 8.1 / 13.1 / 15.1 are token-only readers of the pronoun token). Categories: the cue token ("either", "not"; the "noun" slot of v80's
...
OUT = v80.ROOT / "circuits/followups/correlative_either_not_dod_source_fold_v129_result.json"
CANDIDATE_ID = line.CANDIDATE_ID.rsplit(".", 1)[0] + ".dod_source_fold_v129"
CUES = ("either", "not",)
...
def main() -> None:
    ...
    result, forwards = v80.fold_line(fw, rows, pos, neg, line.HEADS, cues, objs, "correlative_either_not")
    H = result["heads"]
    ...
    OUT.write_text(json.dumps({"schema": "correlative_either_not_dod_source_fold_result_v129", "candidate_id": CANDIDATE_ID, "plan": plan, "line": result, "predictions": predictions, "forwards": forwards,
