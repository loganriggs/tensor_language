#!/usr/bin/env python
"""late_mlp_branch_width_probe -- below the MLP block: for blocks 8-17's MLPs Down[(Left x)(Right x)], truncate the input of ONE branch
only through the bus frame U_8 at k in {768, 896} (the other branch and all other reads exact), and the per-block MLP-only read cost
at 768. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_one_branch_is_cheaper_than_both pred_c_branches_symmetric pred_d_branch_costs_additive
#                     pred_e_mlp_only_compounds
...
Preregistration: polynomial_causal/LATE_MLP_BRANCH_WIDTH_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_mlp_branch_width_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_mlp_branch_width_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_MLP_BRANCH_WIDTH_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_width_by_kind_probe_results.json"   # §2773
OUT = ROOT / "late_mlp_branch_width_probe_results.json"
HASHES = {PREREG: "a2d23680e334de5967e57af8f34893f435b4ada39b0509ead746e6d3b87cb9e6", PRIOR: "8cb89e3fdd3c3ca609550480eeb0470100cfb7f95e5505cf326a51cb7b2a9ce4",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_mlp_branch_width_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_frac": 0.70, "c_lo": 0.5, "c_hi": 2.0, "d_lo": 0.8, "d_hi": 1.3, "e_frac": 0.70}
NULLS = {"b_frac": 0.95, "c_lo": 0.25, "c_hi": 4.0, "d_lo": 0.6, "d_hi": 1.6, "e_frac": 0.95}
...
def main():
    ...
    class BranchHead(OwnHead):
        """MLP l with only ONE branch (Left or Right) reading the projected input; the other branch reads the exact xhat."""
        def __init__(self, l, k, U_, branch):
            super().__init__(l, k, "const", U_); self.branch = branch
        def __call__(self, w, x):
            xh = F.rms_norm(x, (D,)); core_ = (xh @ self.U) @ self.U.T; fb = self.mx
            xp_ = core_ + fb - (fb @ self.U) @ self.U.T
            if self.branch == "left":
                return self.mlp.Down(self.mlp.Left(xp_) * self.mlp.Right(xh))
            return self.mlp.Down(self.mlp.Left(xh) * self.mlp.Right(xp_))
    def split8(k):
        ...
        return run(patch)
    def mlp_reads(k, ls):
        return run({("mlp", l): head(("mlp", l), k, U_8) for l in ls})
    def branch(k, which):
        return run({("mlp", l): BranchHead(l, k, U_8[:, :k], which) for l in range(SPLIT, NL)})
    LATE = list(range(SPLIT, NL))
    arms = {"SPLIT8_1024": split8(1024), "LATE_MLP_768": mlp_reads(768, LATE), "LEFT_768": branch(768, "left"), "RIGHT_768": branch(768, "right")}
    if not smoke:
        arms["LEFT_896"] = branch(896, "left"); arms["RIGHT_896"] = branch(896, "right")
    for l in ([17] if smoke else LATE):
        arms[f"ONE_MLP_{l}_768"] = mlp_reads(768, [l])
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    Mb, Lf, Rt = arms["LATE_MLP_768"], arms["LEFT_768"], arms["RIGHT_768"]
    per_block = {l: arms[f"ONE_MLP_{l}_768"] for l in LATE if f"ONE_MLP_{l}_768" in arms}
    ssum = sum(per_block.values())
    summ = {"mlp_768": Mb, "left_768": Lf, "right_768": Rt, "left_896": arms.get("LEFT_896"), "right_896": arms.get("RIGHT_896"),
            "max_branch_over_mlp": max(Lf, Rt) / Mb if abs(Mb) > 1e-9 else float("inf"), "left_over_right": Lf / Rt if abs(Rt) > 1e-9 else float("inf"),
            "sum_branches_over_mlp": (Lf + Rt) / Mb if abs(Mb) > 1e-9 else float("inf"), "per_block_sum": ssum, "per_block_sum_over_mlp": ssum / Mb if abs(Mb) > 1e-9 else float("inf"),
            "argmax_block": (max(per_block, key=per_block.get) if per_block else None)}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(Mb - PRIOR_LATE_MLP_768) <= BARS["repro_tol"]) if not smoke else True
    r_c = summ["left_over_right"]
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_one_branch_is_cheaper_than_both': bool(max(Lf, Rt) <= BARS["b_frac"] * Mb),
        'pred_c_branches_symmetric': bool(BARS["c_lo"] <= r_c <= BARS["c_hi"]),
        'pred_d_branch_costs_additive': bool(BARS["d_lo"] <= summ["sum_branches_over_mlp"] <= BARS["d_hi"]),
        'pred_e_mlp_only_compounds': bool(summ["per_block_sum_over_mlp"] <= BARS["e_frac"]),
    }
    nulls = {"b_null_branch_ge_.95": bool(max(Lf, Rt) >= NULLS["b_frac"] * Mb), "c_null_ratio_le_.25_or_ge_4": bool(r_c <= NULLS["c_lo"] or r_c >= NULLS["c_hi"]),
             "d_null_sum_le_.6_or_ge_1.6": bool(summ["sum_branches_over_mlp"] <= NULLS["d_lo"] or summ["sum_branches_over_mlp"] >= NULLS["d_hi"]),
             "e_null_sum_ge_.95": bool(summ["per_block_sum_over_mlp"] >= NULLS["e_frac"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "blocks 8-17 MLP: one branch (Left/Right) reads through U_8 at k, other branch exact; per-block MLP-only reads at 768",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "late_mlp_768": Mb, "prior_late_mlp_768": PRIOR_LATE_MLP_768},
           "ce_added": arms, "per_block_mlp_768": {str(k): v for k, v in per_block.items()}, "summary": summ,
