...
"""v329: verb_particle separability at FIFTEEN prepared -- the first test of whether PAIR DENSITY matters once a
readout pair has three occupants.

WHERE THIS SITS. The 2x2 is closed: v321 showed three cells sharing a full readout pair with a counted circuit are
separable from it (fam-arm leak into the original -0.0033/-0.0076/-0.0093), and v325 showed three cells reusing both
cue -> token mappings all FUSE, with every one of the five supra-bar fam-arm leaks landing on a cue donor and the
largest non-donor leak at 0.0174. The mapping is the identity; the pair is not. v327 was the first battery authored
under the relaxed gate and produced two four-row passes, BOTH of them the third occupant of their pair:
    verb_particle_away_up_c   whiled/freshened  (n11, held 0.884)  joins shied/sidled and faded/livened on ` away`/` up`
    verb_particle_down_out_c  bogged/conked     (n12, held 0.907)  joins found/settled and chickened/slowed on ` out`/` down`
v327's other two cells do not enter: up_down_c missed rows 3 AND 5, out_away_c missed row 2, and a row-3 miss means the
dim A1 effect is not established, so neither is interpretable as a control.
WHAT IS ACTUALLY NEW HERE. v321 tested pair density TWO. Nothing has tested THREE. If separability at a shared pair
degrades as occupants accumulate -- through the shared answer position, the shared token embeddings, or simply because
a rank-1 direction has less room -- density three is where it should first show, and the relevant comparison is
already on disk: away_up and away_up_b sat at fam-arm leak -0.0076 into each other at density two.
COST. Fifteen prepared, eight evaluated. Measured curve: 4 prepared 171 s, 7 -> 467, 10 -> 1143, 13 -> 1614. This is
the most expensive rung this family has run and proposal (xii) -- split the family or trim the control set to a fixed
deterministic sample -- is on the board unanswered, so the cost is paid rather than avoided.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
    pred_a_new_separable   at least k_new = 1 of the 2 new members is separable at the registered cross bar. The cue
                           account predicts both; the dead pair account predicted neither.               prior 80%
    pred_b_counted_kept    at least k_kept = 5 of the 6 re-checked counted members remain separable.     prior 80%
    ...
    pred_d_instrument      every evaluated member's own-arm extraction is within 0.03 of its parent battery's cdas
                           extraction_held. Fails automatically if any member errors; fifteen prepared is the largest
                           this family has run and v325 did thirteen with none.                          prior 85%
    pred_e_pair_density    verb_particle_away_up_c's fam-arm leak into the OTHER TWO OCCUPANTS of its own readout pair
                           -- away_up and away_up_b -- stays at or below pair_density_max = 0.02 in absolute value:
                           max(|leak into away_up|, |leak into away_up_b|) <= 0.02.
                           This is on the FAM arm deliberately. v325's partner clause was registered on the own arm,
                           where generic within-family collateral runs 0.35-0.44 and swamps the signal, and it failed
                           at 0.0125 while the hypothesis it tested was confirmed; the fam arm is where residual leak
                           identifies a partner. The 0.02 anchor is not invented: v325's SMALLEST genuine cue-donor
                           leak was 0.0286 and its largest non-donor leak was 0.0174, so 0.02 sits between the two
                           populations that rung actually measured. Worked example: leaks of 0.010 and 0.015 give
                           max 0.015 <= 0.02, TRUE, density three is as clean as density two; leaks of 0.030 and
                           0.010 give 0.030 > 0.02, FALSE, and the third occupant is starting to collide with the
                           first two even though it shares no cue with either.                           prior 70%
COUNTING. Each separable new member is +1 on 134. A fused pair-mate would be the first evidence that pair occupancy
matters at all, and would qualify the relaxed gate rather than overturn it -- the gate would become "a pair may be
reused, but not indefinitely", with density three as the measured limit.
BARS = {"cross_max": 0.05, "keep": 0.8, "ext_floor": 0.5, "c_ub_max": 0.01, "k_d": 4, "k_new": 1, "k_kept": 5, "instr_tol": 0.03, "pair_density_max": 0.02}
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v329_result.json"
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v329", "members": n, "families": list(FAMILIES),
...
def PREDS(R):
    ...
    tgt, mates = "verb_particle_away_up_c", ("verb_particle_away_up", "verb_particle_away_up_b")
    e = False
    if tgt in good:
        sibs = good[tgt]["arms"]["fam"]["siblings"]
        if all(s in sibs for s in mates):
            e = max(abs(sibs[s]) for s in mates) <= B["pair_density_max"]
    return {"pred_a_new_separable": bool(a), "pred_b_counted_kept": bool(b), "pred_c_row4_kept": bool(c),
            "pred_d_instrument": bool(d), "pred_e_pair_density": bool(e)}


def main() -> None:
    if os.environ.get("BQLIB_DRYRUN") or os.environ.get("BQLIB_NO_MODEL"):
        print(json.dumps(_plan(), indent=2, sort_keys=True)); return
    smoke = os.environ.get("V329_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V329_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    evaluate = [n for n in members if n in EVAL and (not smoke or n in os.environ.get("V329_SMOKE_NAMES", "verb_preposition_at_to,verb_preposition").split(","))]
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v329", "candidate_id": "corpus.unit_family_separability_spec_v329",
