# WHICH SITE BUYS THE LAYER-1 DROP -- attn1 or mlp1?
#
# §1829 measured the bottom-up recovery curve at every depth 0..7 for the first time and found the fall
# is at LAYER 1, not layer 5: sequential-arm gap fraction 64.8% at B0, 25.9% at B1, and then a slow
# decline to a floor, with depths 3..7 inside a 1.6pp band. The single B0->B1 step is **38.9 of the
# 53.1 percentage points** that are lost in total -- 73.3% of it, more than depths 2 through 7 combined.
#
# That step is one layer, and a layer has exactly two sites. This splits it. Compiling depth 0 plus
# attn1, and depth 0 plus mlp1, against depth 0 alone and against the full B1, is four arms and it
# settles which site the 38.9pp belongs to.
#
# Why this and not another summary of the stream: §1824-§1828 closed five accounts (magnitude, mean
# direction, single-layer localisation, second moment, cross-position routing shape and influence mass)
# and every one of them was measured at depths 0/3/5 -- which §1829 showed is a comparison across a FLAT
# region, since B3 through B7 differ by 1.6pp. This is measured on the loss, in the one place the curve
# actually moves.
#
# The gain correction is generalised accordingly: §1824's sequential calibration runs over the layers
# whose ATTENTION is still live in each arm, which differs between B0+attn1 (live attn 2..17) and
# B0+mlp1 (live attn 1..17). A gain on a compiled attention layer is a no-op -- its c_proj output is
# discarded by the substituting hook -- so the correctable set is exactly the live-attention set.
#
...
#   pred_a ONE SITE DOMINATES: the larger of the two single-site drops from B0 is at least 70% of the
#          full B0->B1 drop, on skip7000, in the SEQUENTIAL arm. If TRUE the 38.9pp belongs to one site
#          and this names it. If FALSE both sites carry a substantial share, and the layer-1 cost is a
#          property of compiling the LAYER rather than of either site -- which would mean there is no
#          smaller object to find here and the search should stop descending.
#   pred_b AND IT IS ATTENTION: attn1 costs more than mlp1, by at least 5pp. §1765 established that
#          substituting attention is what deletes cross-position structure, so attention is the
#          mechanism-consistent answer. If FALSE and mlp1 is the expensive site, the layer-1 damage is
#          NOT about attention deletion, which would be the most surprising result of the arc and would
#          redirect it away from every cross-position instrument built since §1765.
#   pred_c AND THE TWO OVERLAP: the sum of the two single-site drops EXCEEDS the full B0->B1 drop, so
#          they are sub-additive and partly destroy the same thing. If FALSE they are additive or worse,
#          each site removes something the other does not, and the layer-1 cost is two independent
#          losses that happen to land in one layer.
#   pred_d CONTROLS, cross-run per LESSON 42: B0 and B1 must reproduce §1829's PUBLISHED sequential gap
#          fractions (skip7000 64.8% and 25.9%) within 0.03, and B0 §1806's PUBLISHED raw 37.4%.
#          Endpoints must reproduce §1789's FULL-RANK top-1; the placement control must move top-1 by
...
OUT = PT + 'ops/layer1_site_split_results.json'
...
ARMS = {'B0': [('attn', 0), ('mlp', 0)],
        'B0_attn1': [('attn', 0), ('mlp', 0), ('attn', 1)],
        'B0_mlp1': [('attn', 0), ('mlp', 0), ('mlp', 1)],
        'B1': [('attn', 0), ('mlp', 0), ('attn', 1), ('mlp', 1)]}
...
def main():
    ...
    print(f'LAYER-1 SITE SPLIT | attn1 vs mlp1 on the 38.9pp B0->B1 drop (§1829) | '
    ...
    full_drop = seq['B0'] - seq['B1']
    d_attn = seq['B0'] - seq['B0_attn1']
    d_mlp = seq['B0'] - seq['B0_mlp1']
    bigger = 'attn1' if d_attn >= d_mlp else 'mlp1'
    share = max(d_attn, d_mlp) / full_drop if abs(full_drop) > 1e-9 else 0.0
    additivity = (d_attn + d_mlp) / full_drop if abs(full_drop) > 1e-9 else 0.0
    pa = share >= 0.70
    pb = (d_attn - d_mlp) >= 0.05
    pc = (d_attn + d_mlp) > full_drop
    pd = (all(abs(frac[e][f'{k}_seq'] - S1829_SEQ_GAPFRAC[k][e]) <= 0.03
              for e in roles for k in S1829_SEQ_GAPFRAC)
    ...
    print('\n  gap fraction recovered, sequential arm:', flush=True)
    for k in ARMS:
        for e in roles:
            print(f'    {k:10s} {e:10s} raw {frac[e][f"{k}_raw"]:8.1%}'
                  f'  SEQUENTIAL {frac[e][f"{k}_seq"]:8.1%}', flush=True)
    print(f'\n  THE SPLIT ({PICK_ROLE}, sequential): B0 {seq["B0"]:.1%}  '
          f'+attn1 {seq["B0_attn1"]:.1%}  +mlp1 {seq["B0_mlp1"]:.1%}  B1 {seq["B1"]:.1%}', flush=True)
    print(f'    drop from B0:  attn1 {d_attn:+.1%}   mlp1 {d_mlp:+.1%}   both {full_drop:+.1%}',
          flush=True)
    print(f'\n  ONE SITE dominates (>=70% of the B0->B1 drop) -> {pa}  '
          f'{bigger} holds {share:.1%}', flush=True)
    print(f'  and it is ATTENTION (attn1 costs mlp1 by >=5pp) -> {pb}  '
          f'attn1 {d_attn:+.1%} vs mlp1 {d_mlp:+.1%}, margin {d_attn - d_mlp:+.1%}', flush=True)
    print(f'  and the two OVERLAP (sum exceeds the joint drop) -> {pc}  '
          f'sum {d_attn + d_mlp:+.1%} vs joint {full_drop:+.1%} ({additivity:.2f}x)', flush=True)
    print(f'  B0/B1 reproduce §1829, B0 raw reproduces §1806, endpoints reproduce §1789 '
          f'-> control {pd}', flush=True)

    json.dump({'run': 'layer1_site_split', 'arms': {k: [list(s2) for s2 in v] for k, v in ARMS.items()},
               'role': PICK_ROLE, 'gap_fraction': frac,
               'sequential': {k: seq[k] for k in ARMS},
               'drop_attn1': d_attn, 'drop_mlp1': d_mlp, 'drop_joint': full_drop,
               'dominant_site': bigger, 'dominant_share': share, 'additivity_ratio': additivity,
               'layer_norm_live': ln_live, 'gain_sequential': gseq,
               'predictions': {'pred_a_one_site_dominates': bool(pa),
                               'pred_b_it_is_attention': bool(pb),
                               'pred_c_sites_overlap': bool(pc),
