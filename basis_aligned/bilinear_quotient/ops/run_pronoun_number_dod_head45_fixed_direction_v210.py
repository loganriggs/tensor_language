#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays_v206 pred_b_directions_are_aligned_across_rows pred_c_fixed_direction_damages_like_per_row pred_d_fixed_direction_beats_matched_random pred_e_fixed_direction_selective
"""Pronoun number they/he DoD (v210): is head 4.5's number direction ONE FIXED VECTOR? v208 / v209 removed, per row, the component of 4.5's verb slice along
d_i = O_{4.5}^T r_i with r_i the plural detector's product gradient at the verb of row i (-4.3% of the margin, selective, 5x the norm-matched null).
A component is Simple if that direction does not depend on the row: here the per-row d_i are computed (one trace per batch), their mean pairwise cosine
is reported, and the removal is repeated with the single normalized MEAN direction d_bar for every row (the per-row removal is repeated in the same run
for a like-for-like ratio), against 16 norm-matched random directions and the three unrelated readers.
PREDICTIONS (scored as written; failures preserved; priors unsure)
    ...
    pred_b_directions_are_aligned_across_rows  mean pairwise cosine of the normalized d_i over the 96 rows >= 0.80
    pred_c_fixed_direction_damages_like_per_row  the fixed-direction damage >= 0.80 x the per-row-direction damage
    pred_d_fixed_direction_beats_matched_random  the fixed-direction damage exceeds every one of the 16 norm-matched random directions
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_head45_fixed_direction_v210_result.json"
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_head45_fixed_direction_v210"
BLOCK, HEAD, U829, V206_MARGIN, DROP_MIN, GATE_RATIO, HALF, N_NULL, SEED, BATCH = 4, 5, 829, 196.62, 0.02, 0.25, 0.80, 16, 210, 32
FORWARDS_MAX = 63
import run_aspectual_dod_mlp8_pair_fold_v16 as v16
PREDICTIONS = {"pred_a_baseline_replays_v206": "<= 1e-3", "pred_b_directions_are_aligned_across_rows": ">= 0.80 cosine", "pred_c_fixed_direction_damages_like_per_row": ">= 0.80 x per-row", "pred_d_fixed_direction_beats_matched_random": "> 16 matched random", "pred_e_fixed_direction_selective": "three gates"}
...
def main() -> None:
    ...
    unit_dirs = torch.stack([d / d.norm() for d in all_dirs]); G = unit_dirs @ unit_dirs.T; n_ = len(rows)
    mean_cos = float((G.sum() - G.diagonal().sum()) / (n_ * (n_ - 1))); d_bar = unit_dirs.mean(0); d_bar = d_bar / d_bar.norm()
    print("mean pairwise cosine of per-row directions", round(mean_cos, 4), "| cosine of each with the mean: min", round(float((unit_dirs @ d_bar).min()), 3))
    for start in range(0, len(rows), BATCH):
    ...
    report = {"native_pooled": native, "mean_pairwise_cosine": mean_cos, "damage": damage, "moves": moves, "null_moves": null_moves, "gates": gates, "removed_norm_ratio_vs_number": norm_ratio, "null_damage_max": max(damage[f"rand{k}"] for k in range(N_NULL))}
    ...
    predictions = {"pred_a_baseline_replays_v206": abs(native - V206_MARGIN) / V206_MARGIN <= 1e-3, "pred_b_directions_are_aligned_across_rows": mean_cos >= 0.80, "pred_c_fixed_direction_damages_like_per_row": damage["fixed"] >= HALF * damage["number"],
    ...
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_head45_fixed_direction_result_v210", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
