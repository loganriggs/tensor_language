"""ATTENTION 5's PRICE CLIFF, ASKED OF THE FRONTIER PROPERLY THIS TIME (Claude, LANE 1)

"attn5's write = the price cliff" has been one of the three largest standing gaps in the explained fraction for weeks. SS2874 tried to
ask it of the frontier through the `attnd` dictionaries and SS2879 had to withdraw that: those dictionaries are **not in the evaluated
configuration**. Attention 2-9 enters the SS312 frontier through the **38 motif heads**, and the motif-head layer list `ML` is passed
directly into `evalM` -- so it is installed by construction, which is what the SS2879 rule demands.

The motif heads are an APPROXIMATION (scaled value copies) standing in for real attention, so dropping a layer from `ML` restores that
layer to REAL and should LOWER L2. The quantity is therefore an ERROR SHARE, not a deletion cost:
...
Three arms: BASELINE (ML = 2..9), attention 5 restored, and attention 2 restored as the control. **a2 is the control because SS2834's
real-model census makes it the second-largest in the band** -- zero-ablation damage 0.349 nats against a5's 2.211 and a3's 0.141 -- so
it is the layer most likely to rival a5, and beating a weak control would prove nothing (the SS2820 lesson).
...
#                     pred_b_attn5_carries_a_large_error_share
#                     pred_c_attn5_is_specific_against_the_control_layer
#                     pred_d_both_arms_are_connected
#                     pred_e_restoring_a_layer_never_harms

Preregistration: polynomial_causal/FRONTIER_ATTN5_ERROR_SHARE_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; three arms (baseline / attn5 real / attn2 real)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_attn5_error_share_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_ATTN5_ERROR_SHARE_PREREGISTRATION.md'
PREREG_SHA = "e17be236cb9c55d636cc58c2bdbbb875c504d0b6b03ee10adf9f02e5a3d2601f"
...
TARGET, CTRL = 5, 2
BARS = {"reproduce": 0.05, "a5_share": 0.15, "specificity": 3.0, "connected": 0.005, "no_harm": -0.01}
NULLS = {"a5_share_le": 0.03, "specificity_le": 1.5}
...
if __name__=='__main__':
    ...
    print('ARM 1/3: BASELINE -- motif heads on all of a2-a9 (the published frontier)',flush=True)
    EVALMODE['motif']=None
    r_base=main()

    print(f'ARM 2/3: attention {TARGET} restored to REAL (motif heads on the other seven layers)',flush=True)
    EVALMODE['motif']=[l for l in BAND if l != TARGET]
    r_t=main()

    print(f'ARM 3/3: attention {CTRL} restored to REAL -- control layer',flush=True)
    EVALMODE['motif']=[l for l in BAND if l != CTRL]
    r_c=main()
    EVALMODE['motif']=None

    # SIGN: L2 is CE ADDED ABOVE THE REAL MODEL, LOWER IS BETTER. These are APPROXIMATIONS, so restoring a
    # layer to REAL should LOWER L2. ERROR SHARE = baseline - restored, POSITIVE = that layer's motif
    # approximation contributes that much of the frontier's error.
    share_t = r_base['L2_F'] - r_t['L2_F']
    share_c = r_base['L2_F'] - r_c['L2_F']
    ratio   = share_t / share_c if abs(share_c) > 1e-9 else float('inf')

    pa = abs(r_base['L2_F']-2.6735) <= BARS['reproduce']
    pb = share_t >= BARS['a5_share']
    pc = share_t >= BARS['specificity'] * max(share_c, 1e-9)
    pd = (abs(share_t) >= BARS['connected']) and (abs(share_c) >= BARS['connected'])
    pe = (share_t >= BARS['no_harm']) and (share_c >= BARS['no_harm'])
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_attn5_carries_a_large_error_share':bool(pb),
           'pred_c_attn5_is_specific_against_the_control_layer':bool(pc),
           'pred_d_both_arms_are_connected':bool(pd),
           'pred_e_restoring_a_layer_never_harms':bool(pe)}
    nulls={'b_null_attn5_is_not_a_frontier_error_source':bool(share_t<=NULLS['a5_share_le']),
           'c_null_attn5_is_not_special_in_the_band':bool(ratio<=NULLS['specificity_le'])}
    res={'rung':'frontier_attn5_error_share','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,'target_layer':TARGET,'control_layer':CTRL,'band':BAND,
         'baseline':r_base,'target_real':r_t,'control_real':r_c,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(r_base['L2_F'],4),
                    'L2_F_attn5_real':round(r_t['L2_F'],4),
                    'L2_F_control_real':round(r_c['L2_F'],4),
                    'error_share_attn5_nats':round(share_t,4),
                    'error_share_control_nats':round(share_c,4),
                    'specificity_ratio':round(ratio,3) if ratio!=float('inf') else None,
                    'L1_F_baseline':r_base.get('L1_F'),'L1_F_attn5_real':r_t.get('L1_F'),
                    'L2_C_baseline':round(r_base['L2_C'],4),'L2_C_attn5_real':round(r_t['L2_C'],4)},
    ...
    print(f"(b) attn{TARGET} error share {share_t:+.4f} >= {BARS['a5_share']}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) vs control attn{CTRL} {share_c:+.4f}, ratio {ratio:.2f} >= {BARS['specificity']}: {'HELD' if pc else 'FAILED'}")
