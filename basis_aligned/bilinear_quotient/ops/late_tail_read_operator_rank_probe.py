#!/usr/bin/env python
"""late_tail_read_operator_rank_probe -- the exact RANK STRUCTURE of the late MLPs' core-gated tail read. Each late MLP's cross term is
Down(Lc*Rt + Lt*Rc) = J(c) t with J linear in the core input c (§2780-§2788). Weight-level: the effective rank of the family {J(c)}
under the fit-set core covariance (768 generators), the tail-input side E[J^T J] (384 tail dims) and the output side E[J J^T]
over real fit-set inputs, the constant-gate share |J(mean c)|^2 / E|J(c)|^2, and an exact split check. Decides whether a low-rank
W_l(c) surrogate of the read is admissible. CUDA lane-1 (two CE instrument arms; the rest is weight algebra).

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_gate_dependence_is_high_rank pred_c_tail_input_read_is_dense
#                     pred_d_cross_output_lands_in_core pred_e_constant_gate_is_a_minority

SIGN CONVENTION (§2135): the two CE numbers are CE ADDED ABOVE THE REAL MODEL on held-out docs 0-63 -- LOWER IS BETTER. Descriptive;
nothing installs into the §312 frontier; bases are data covariances scored by CE only (§2118 stays closed).
Preregistration: polynomial_causal/LATE_TAIL_READ_OPERATOR_RANK_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_tail_read_operator_rank_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_tail_read_operator_rank_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_TAIL_READ_OPERATOR_RANK_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_tail_writer_recency_probe_results.json"   # §2790
OUT = ROOT / "late_tail_read_operator_rank_probe_results.json"
HASHES = {PREREG: "43473a724b99c591b6f9a786174f89aeb14055b203cdbbd36974c2a80b75ac5b", PRIOR: "acd39cc8aaa38ca5b585e50b7c08628f662e9bbf669c0d9c5899faae240269c0",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_tail_read_operator_rank_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_SPLIT8_1024 = 0.0374; PRIOR_LATE_MLP_768 = 0.1249; N_MC = 512; JCH = 64; MCH = 32; LAM = 1e-2
...
def main():
    ...
    Uk = U_8[:, :768]; Ut = U_8[:, 768:]; NT = Ut.shape[1]
    # sample fit-set MLP inputs xh at every late site (first chunk of fit docs; N_MC tokens, pinned generator)
    grab = {}
    class Grab:
        def __init__(self, l): self.l = l
        def hook(self, idx): pass
        def __call__(self, w, x):
            grab[self.l] = F.rms_norm(x, (D,)).reshape(-1, D); return w
    idx = fit_rows[:CH, :TI]
    forward(m, idx, patch={("mlp", l): Grab(l) for l in LB})
    g = torch.Generator().manual_seed(0); sel = torch.randperm(idx.numel(), generator=g)[:N_MC].to(DEV)
    def jac(mlp, c, LU, RU):
        """J(c) [n, D, NT]: the late MLP's cross term as a linear map on the tail (in Ut coordinates) at core input c [n, D]."""
        Lc = mlp.Left(c); Rc = mlp.Right(c)
        return torch.einsum("oh,nhk->nok", mlp.Down.weight.float(), Lc[:, :, None] * RU[None] + Rc[:, :, None] * LU[None])
    per = {}
    for l in LB:
        mlp = m.transformer.h[l].mlp; h = heads[("mlp", l)]
        LU = mlp.Left(Ut.T).T.contiguous(); RU = mlp.Right(Ut.T).T.contiguous()          # [HID, NT]: Left Ut, Right Ut
        # (i) c-dependence rank: J(c) = sum_i a_i J_i (a = core coordinates); nonzero spectrum of Cov_a[vec J(c)] == spectrum of Ma^1/2 G Ma^1/2, Ma = core covariance
        Js = torch.cat([jac(mlp, Uk[:, i:i + JCH].T, LU, RU).reshape(min(JCH, 768 - i), -1) for i in range(0, 768, JCH)], 0)   # [768, D*NT]
        G = (Js @ Js.T).double(); del Js
        Ma = Uk.double().T @ h["Cx"].to(Uk.device) @ Uk.double()          # CENTERED core covariance: rank of the c-FLUCTUATION dependence (the mean gate is pred_e)
        ea, Va = torch.linalg.eigh(Ma); Sa = (Va * ea.clamp_min(0).sqrt()) @ Va.T
        S = Sa @ G @ Sa; S = (S + S.T) / 2
        sg = R.spectrum(S.cpu()); sG = R.spectrum(G.cpu())
        # (ii) Monte-Carlo over real fit-set inputs: tail-input side E[J^T J], output side E[J J^T], constant-gate share, exact-split check
        xs = grab[l][sel]; cs = (xs @ Uk) @ Uk.T
        Min = torch.zeros(NT, NT, dtype=torch.float64, device=DEV); Mout = torch.zeros(D, D, dtype=torch.float64, device=DEV); relerr = 0.0
        for i in range(0, cs.shape[0], MCH):
            Jc = jac(mlp, cs[i:i + MCH], LU, RU)
            Min += torch.einsum("nok,nol->kl", Jc, Jc).double(); Mout += torch.einsum("nok,npk->op", Jc, Jc).double()
            if i == 0:
                gt = torch.Generator(device="cpu").manual_seed(1); r = torch.randn(Jc.shape[0], NT, generator=gt).to(DEV); t = r @ Ut.T; c_ = cs[i:i + MCH]
                full = mlp.Down(mlp.Left(c_ + t) * mlp.Right(c_ + t)) - mlp.Down(mlp.Left(c_) * mlp.Right(c_)) - mlp.Down(mlp.Left(t) * mlp.Right(t))
                pred_ = torch.einsum("nok,nk->no", Jc, r); relerr = float((full - pred_).norm() / full.norm().clamp_min(1e-12))
        Min /= cs.shape[0]; Mout /= cs.shape[0]
        e_tot = float(torch.trace(Min)); e_mean = float(jac(mlp, ((h["mx"] @ Uk) @ Uk.T)[None], LU, RU).pow(2).sum())
        core_frac = float(torch.trace(Uk.double().T @ Mout @ Uk.double()) / torch.trace(Mout))
        per[l] = {"gate_dep_eff_rank": sg["eff_rank"], "gate_dep_rank_90": sg["rank_90"], "gate_dep_unweighted_eff_rank": sG["eff_rank"],
                  "tail_in_eff_rank": R.spectrum(Min.cpu())["eff_rank"], "tail_in_rank_90": R.spectrum(Min.cpu())["rank_90"],
                  "out_eff_rank": R.spectrum(Mout.cpu())["eff_rank"], "out_core_energy_frac": core_frac,
                  "const_gate_energy_share": e_mean / max(e_tot, 1e-12), "split_check_rel_err": relerr, "mean_read_energy_per_unit_tail": e_tot / NT}
        log(stage="block", l=l, **{k: (round(v, 4) if isinstance(v, float) else v) for k, v in per[l].items()})
        del G, S, Min, Mout
    ...
    C0 = arms["LATE_MLP_768"]
    def col(k_):
        return [per[l][k_] for l in LB]
    def med(v):
        return float(torch.tensor(v).median())
    summ = {"min_gate_dep_eff_rank": min(col("gate_dep_eff_rank")), "median_gate_dep_eff_rank": med(col("gate_dep_eff_rank")), "min_tail_in_eff_rank": min(col("tail_in_eff_rank")),
            "median_tail_in_eff_rank": med(col("tail_in_eff_rank")), "mean_out_core_energy_frac": sum(col("out_core_energy_frac")) / len(LB), "median_const_gate_share": med(col("const_gate_energy_share")),
            "max_split_check_rel_err": max(col("split_check_rel_err")), "core_dim": 768, "tail_dim": NT, "n_mc_tokens": int(N_MC)}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(C0 - PRIOR_LATE_MLP_768) <= BARS["repro_tol"]
                   and summ["max_split_check_rel_err"] <= BARS["split_tol"]) if not smoke else bool(summ["max_split_check_rel_err"] <= BARS["split_tol"])
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_gate_dependence_is_high_rank': bool(summ["min_gate_dep_eff_rank"] >= BARS["b_min"]),
        'pred_c_tail_input_read_is_dense': bool(summ["min_tail_in_eff_rank"] >= BARS["c_min"]),
        'pred_d_cross_output_lands_in_core': bool(summ["mean_out_core_energy_frac"] >= BARS["d_min"]),
        'pred_e_constant_gate_is_a_minority': bool(summ["median_const_gate_share"] <= BARS["e_max"]),
    }
    nulls = {"b_null_some_block_le_64": bool(summ["min_gate_dep_eff_rank"] <= NULLS["b_max"]), "c_null_some_block_le_96": bool(summ["min_tail_in_eff_rank"] <= NULLS["c_max"]),
             "d_null_core_frac_le_.5": bool(summ["mean_out_core_energy_frac"] <= NULLS["d_max"]), "e_null_const_share_ge_.8": bool(summ["median_const_gate_share"] >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           "sign_convention": "CE ADDED above the real model on held-out docs 0-63 (FRESH split; LOWER IS BETTER) for the two instrument arms; the rest are exact weight/second-moment quantities",
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "split": SPLIT,
           "program": "late MLP cross term Down(Lc*Rt + Lt*Rc) = J(c) t, J linear in the core input c; ranks of the c-dependence (768 generators, weighted by the fit-set core COVARIANCE (fluctuation about the mean gate)), of the tail-input side E[J^T J] and of the output side E[J J^T] over real fit-set inputs",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "late_mlp_768": C0, "prior_late_mlp_768": PRIOR_LATE_MLP_768, "max_split_check_rel_err": summ["max_split_check_rel_err"]},
           "ce_added": arms, "per_block": {str(l): per[l] for l in LB}, "summary": summ,
           "price": {"gpu_doc_forwards": 2 * int(fit_rows.shape[0]) + CH + int(ev.shape[0]) * (1 + len(arms)), "cpu_doc_forwards": 0, "gpu_seconds": time.time() - started},
