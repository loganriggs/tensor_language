#!/usr/bin/env python3
# BQGATE:200bodyforwards;40prefixes;300seconds;no fitting.
"""pred_a replay<=1e-4abs/1e-5relative,finite/outside0/200forwards;
pred_b omit correction error<=.35/live>=1e-5; pred_c frozen same;
pred_d collateral<=.5. Opened screen; no fresh/null-selectivity promotion."""
...
STEM='REDUCED_MLP8_NORMALIZATION_V1';ARMS=['native','native8_midpoint','compiled','omit_correction','freeze_denominator']
PACKAGE=P/'extracted_circuits/typed_face_reduced_fused_v1'
sys.path.insert(0,str(PACKAGE))
import execute as compiled
import typed_face_reduced_norm_variants_v1 as variants
...
def main():
 ...
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
  print('200bodyforwards;40prefixes;native/native8/compiled');return
 ...
 def pre9(module,args):
  ...
  if arm=='native':c['x9']=x.clone()
   ...
   for mode in ['omit_correction','freeze_denominator']:
    c[mode]=variants.execute(program,**inputs,mode=mode);outside.append(float(c[mode][:,~c['mask']].abs().max()))
  ...
  elif arm in ['compiled','omit_correction','freeze_denominator']:return x+c[arm].to(x.dtype),first,x0
 ...
 values=torch.zeros(5,40,10,dtype=torch.float64);count=0
 try:
 ...
 for family in doc['variants']:
  ...
  for ai,mode in [(3,'omit_correction'),(4,'freeze_denominator')]:
   prediction=e[ai,idx];rms=prediction.square().mean(0).sqrt();target=float(rms[0]);records[family][mode]={'target_error':float((prediction[:,0]-actual[:,0]).norm()/actual[:,0].norm()),'target_rms':target,'control_over_target':(rms[1:]/max(target,1e-30)).tolist()}
 result={'pred_a':anchor<=1e-4 and anchor_rel<=1e-5 and max(outside)==0 and bool(torch.isfinite(v).all()) and count==200,'pred_b':all(r['omit_correction']['target_error']<=.35 and r['omit_correction']['target_rms']>=1e-5 for r in records.values()),'pred_c':all(r['freeze_denominator']['target_error']<=.35 and r['freeze_denominator']['target_rms']>=1e-5 for r in records.values()),'pred_d':all(max(x['control_over_target'])<=.5 for r in records.values() for x in r.values()),'families':records,'anchor_max_abs':anchor,'anchor_relative':anchor_rel,'max_outside':max(outside),'body_forwards':count,'seconds':time.perf_counter()-start,'source_shas':binding,'scope':'Opened normalization approximation screen. Full native suffix recomputed; no fresh or random-null selective confirmation, no approximation adopted into exact package.'}
