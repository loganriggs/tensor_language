#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_joint_below_the_sum pred_c_joint_above_the_larger pred_d_late_units_replay pred_e_hub_units_replay
"""Hub units and late units swapped together (v461). v460: on adjacent rows 829 / 953 / 1030 (MLP 8) close 0.114 and 701 / 2059 (MLP 17) 0.079 of the
agreement margin. 701 reads the residual that 829 writes (v458: 829's column into 701's L input; MLPs 10-16 between), so the two sets are NESTED: v435
showed that nested joint swaps cancel in part. Here the joint swap of both sets at the noun, with each set alone as replay: the joint should sit between the
larger single effect and the sum.
PREDICTIONS (scored as written; failures preserved; priors from v435 / v460)
    pred_a_baseline_replays      the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_joint_below_the_sum   the joint swap closes <= 0.114 + 0.079 - 0.02 (nesting costs at least 0.02)
    pred_c_joint_above_the_larger  the joint swap closes >= 0.114 (the late set adds something the hub set does not carry)
    pred_d_late_units_replay     701 / 2059 alone close within 0.01 of 0.079
    pred_e_hub_units_replay      829 / 953 / 1030 alone close within 0.01 of 0.114
...
OUT = ROOT / "circuits/followups/hub_and_late_units_v461_result.json"
CANDIDATE_ID = "chain.hub_and_late_units_v461"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, HUB, LATE, NEST, BAND = 1e-3, 0.114, 0.079, 0.02, 0.01
import random
UNIT_SETS = {"LATE": {17: (701, 2059)}, "UNITS8": {8: (829, 953, 1030)}, "BOTH": {8: (829, 953, 1030), 17: (701, 2059)}}
...
FORWARDS_MAX = 9
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_joint_below_the_sum": "<= 0.173", "pred_c_joint_above_the_larger": ">= 0.114", "pred_d_late_units_replay": "0.079 +- 0.01", "pred_e_hub_units_replay": "0.114 +- 0.01"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "hub_v460": HUB, "late_v460": LATE, "nest": NEST, "band": BAND}}
    ...
    MODES = {"LATE": ({}, (0,)), "UNITS8": ({}, (0,)), "BOTH": ({}, (0,))}; forwards = 0
    def logits_margin(x):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_joint_below_the_sum": v["BOTH"] <= HUB + LATE - NEST, "pred_c_joint_above_the_larger": v["BOTH"] >= HUB,
                   "pred_d_late_units_replay": abs(v["LATE"] - LATE) <= BAND, "pred_e_hub_units_replay": abs(v["UNITS8"] - HUB) <= BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "hub_and_late_units_result_v461", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
