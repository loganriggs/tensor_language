...
"""Joint attention/MLP propagation control at fixed v665 width8.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v683_result.json'
PREDICTIONS=dict(pred_a_exact_replay='baseline and independent joint versus shared chain replay max absolute error <=1e-4',
 pred_b_effect_prediction='native projected-interface control matches dense compiled logits and modal logits<=1e-4',
 pred_c_capable_position_transfer='joint block arm A/B target<=10% and modal<=5% in every cell')


def main():
    plan=dict(prefix_calls=8,suffix_calls=56,sequences_per_arm=48,fits=0,backwards=0,additional_reduced_suffix_executions=96,additional_fused_attention_calls=144,additional_fused_mlp_calls=144,
    ...
    boundary_gaps=[];oracle_checks=[];reset_checks=[];fused_checks=[]
    fused_counts=dict(attention=0,mlp=0)
    fused_reports={mode:{name:{} for name in ['A','B','sum']} for mode in ['projected','joint']}
    reset_reports={str(k):{name:{} for name in ['A','B','sum']} for k in range(7)}
    chain_checks=[]
    ...
    def suffix(raw,x0,first,pos,answers,frozen=None,mode="freeze"):
        ...
        if frozen is None:
            context['baseline_states']=[];context['baseline_attention']=[];context['baseline_premlp']=[]
        ...
        for layer in range(11,18):
            ...
            if frozen is None and layer>=12:
                context['baseline_attention'].append(a.double().clone());context['baseline_premlp'].append(x.double().clone())
    ...
    def native_selected(raw,x0,first,readpos,start_layer=11):
        x=raw
        context['last_native_states']=[x.double().clone()]
        for layer in range(start_layer,18):
            block=model.transformer.h[layer]
            if layer>11:x=block.lambdas[0]*x+block.lambdas[1]*x0
            a,first=block.attn(F.rms_norm(x,(x.shape[-1],)),first)
            x=x+a;x=x+block.mlp(F.rms_norm(x,(x.shape[-1],)))
            context['last_native_states'].append(x.double().clone())
    ...
    for entries in batches:
        ...
        for source_name,delta in [('A',source_deltas[0]),('B',source_deltas[1]),('sum',source_deltas[0]+source_deltas[1])]:
            ...
            dense_logits,dense_z=run_dag(runtime,context['all_attention'],source_z,context['all_mlp_contexts'],case['final_context'],readpos)
            dense_ctrl=readout_prepared(context['control_fixed'],dense_z,context['control_context'])
            for mode in ['projected','joint']:
                fz=source_z.clone()
                for k in range(6):
                    block=model.transformer.h[k+12]
                    zin=fz*runtime['blocks'][k]['scale']
                    proposed=(context['all_background'][k]+zin@programs[k]['input_basis'].T).float()
                    fa,_=block.attn(F.rms_norm(proposed,(proposed.shape[-1],)),first);fused_counts['attention']+=1
                    if mode=='projected':
                        encoder=Q if k==0 else output_encoders[k-1]
                        da=(fa.double()-context['baseline_attention'][k])@encoder
                        fh=(context['baseline_premlp'][k]+(zin+da)@programs[k]['input_basis'].T).float()
                    else:fh=proposed+fa
                    fy=fh+block.mlp(F.rms_norm(fh,(fh.shape[-1],)));fused_counts['mlp']+=1
                    fz=(fy.double()-context['baseline_states'][k+1])@output_encoders[k]
                ffinal=fz[batch,readpos]
                flogits=readout_prepared(runtime['readout'],ffinal,case['final_context'])
                fctrl=readout_prepared(context['control_fixed'],ffinal,context['control_context'])
                if mode=='projected':
                    fused_checks.extend([float((flogits-dense_logits).abs().max()),float((fctrl-dense_ctrl).abs().max())])
                feffect=base-(flogits[:,0]-flogits[:,1])*direction
                fmodal=((cb-fctrl)[:,0::2]-(cb-fctrl)[:,1::2])
                for i,e in enumerate(entries):
                    fc=fused_reports[mode][source_name].setdefault(e[3],dict(target=[],prediction=[],native_modal=[],predicted_modal=[]))
                    fc['target'].append(float(native_effect[i]));fc['prediction'].append(float(feffect[i]))
                    fc['native_modal'].append(native_modal[i].tolist());fc['predicted_modal'].append(fmodal[i].tolist())
            for k in range(7):
                encoder=Q if k==0 else output_encoders[k-1]
                # Boundary0 is the identical original coordinate port, avoiding a second float32 add/subtract.
                reset_z=source_z if k==0 else (context['last_native_states'][k]-context['baseline_states'][k])@encoder
                reset_runtime=dict(sparse_runtime,blocks=sparse_runtime['blocks'][k:])
                reset_logits,reset_final=run_dag(reset_runtime,context['all_attention'][k:],reset_z,context['all_mlp_contexts'][k:],case['final_context'],readpos)
                reset_ctrl=readout_prepared(context['control_fixed'],reset_final,context['control_context'])
                reset_effect=base-(reset_logits[:,0]-reset_logits[:,1])*direction
                reset_modal=((cb-reset_ctrl)[:,0::2]-(cb-reset_ctrl)[:,1::2])
                if k==0:reset_checks.append(float((reset_ctrl-ctrl).abs().max()));reset_checks.append(float((reset_logits-predicted).abs().max()))
                if k==6:reset_checks.append(float((reset_ctrl-oracle_ctrl).abs().max()))
                for i,e in enumerate(entries):
                    rc=reset_reports[str(k)][source_name].setdefault(e[3],dict(target=[],prediction=[],native_modal=[],predicted_modal=[]))
                    rc['target'].append(float(native_effect[i]));rc['prediction'].append(float(reset_effect[i]))
                    rc['native_modal'].append(native_modal[i].tolist());rc['predicted_modal'].append(reset_modal[i].tolist())
    ...
    for reports in list(reset_reports.values())+list(fused_reports.values()):
    ...
    passing_boundaries=[k for k in range(1,6) if all(cell['relative_error']<=.10 and max(cell['modal_error'])<=.05 for name in ['A','B'] for cell in reset_reports[str(k)][name].values())]
    b=a and max(fused_checks)<=1e-4 and fused_counts==dict(attention=144,mlp=144)
    failed=[(cell['oracle_modal_error'][j],e) for name in ['A','B'] for cell in reuse_reports[name].values() for j,e in enumerate(cell['modal_error']) if e>.05]
    c=b and all(cell['relative_error']<=.10 and max(cell['modal_error'])<=.05 for name in ['A','B'] for cell in fused_reports['joint'][name].values())

    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,cells=cells,
                predictions=dict(zip(PREDICTIONS,[bool(a),bool(b),bool(c)])),selectivity=selectivity,reset_reports=reset_reports,fused_reports=fused_reports,fused_counts=fused_counts,max_fused_control_error=max(fused_checks),passing_interior_boundaries=passing_boundaries,max_reset_check=max(reset_checks),max_full_state_readout_error=max(oracle_checks),failed_modal_oracle_ratios=[o/e for o,e in failed],reuse_reports=reuse_reports,cross_effects=cross,reuse_counts=reuse_counts,pre11_joint_vs_post11_sum_gap=boundary_gaps,
