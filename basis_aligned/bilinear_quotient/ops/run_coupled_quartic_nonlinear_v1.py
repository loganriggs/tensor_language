#!/usr/bin/env python3
# BQGATE:0bodyforwards,0textsequences;12exact coefficient optimization steps,128cached validationvectors.
...
STEM='COUPLED_QUARTIC_NONLINEAR_V1'
def digest(path):
 h=hashlib.sha256()
...
def main():
 ...
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
  print(json.dumps(dict(gpu_accessed=False,body_forwards=0,text_sequences=0,optimization_steps=12,maximum_line_search=12,fitted_floats=592704)));return
 ...
 p=torch.load(P/'COUPLED_QUARTIC_WRITER_V1_PROGRAMS.pt',weights_only=True)['programs'][0];assert p['seed']==11511
 ...
 initial,_=evaluate(b,n,1.);divisor=-initial;assert divisor>0
 ...
 for step in range(12):
  norm2=float(gb.square().sum()+gn.square().sum());history.append(dict(iteration=step,objective=y,projected_gradient_norm=norm2**.5,wall_seconds=time.perf_counter()-tic))
  (P/(STEM+'_PROGRESS.json')).write_text(json.dumps(history,indent=2)+'\n');print(json.dumps(history[-1]),flush=True)
  if norm2**.5<=1e-6:break
  alpha=1.;accepted=False
  for trial in range(12):
   bn,nn=retract(b,n,-gb,-gn,alpha);yn,_=evaluate(bn,nn,divisor)
   if yn<=y-1e-4*alpha*norm2:accepted=True;break
   alpha*=.5
  if not accepted:line_failed=True;break
  b,n=bn.detach(),nn.detach();y,mix,gb,gn=evaluate(b,n,divisor,True)
 gradnorm=float((gb.square().sum()+gn.square().sum()).sqrt());history.append(dict(iteration=len(history),objective=y,projected_gradient_norm=gradnorm,wall_seconds=time.perf_counter()-tic))
 ...
 result={'pred_a':fd<=1e-4 and all(z['objective']<=history[i]['objective']+1e-10 for i,z in enumerate(history[1:])),'pred_b':-y>=1.1,'pred_c':gradnorm<=1e-6,'directional_gradient_relative_error':fd,'history':history,'capture_gain_fraction':-y-1,'line_search_failed':line_failed,'initial_native_error':initial_native,'final_native_error':native_error,'artifact_sha256':digest(ap),'wall_seconds':time.perf_counter()-tic,'peak_gpu_bytes':torch.cuda.max_memory_allocated(),'scope':'One-seed exact nonlinear pilot; localstationarity separate from gain, no OOD or selectivity evidence.'}
