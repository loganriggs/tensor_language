#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_2921_feeds_701 pred_c_random_unit_does_not pred_d_2921_moves_the_margin_a_little pred_e_2921_within_mlp16
"""Unit 2921 edited at the verb (v473). v471 / v472: MLP 16's 2921 leads the feed into 701's R input at a distant verb and shares its row condition. The
replace-edit: swap only 2921's activation at the answer position between the plural and singular rows and measure 701's activation and the agreement
margin; one random MLP-16 unit as the control; MLP 16's whole write at the answer position (v470: 0.137 on 701) for scale.
PREDICTIONS (scored as written; failures preserved; priors from v470-v472)
    pred_a_baseline_replays          the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_2921_feeds_701            the 2921 swap moves >= 0.05 of 701's pooled contrast at the verb. Prior: unsure
    pred_c_random_unit_does_not      a random MLP-16 unit (seed 0) moves <= 0.01 of 701's contrast
    pred_d_2921_moves_the_margin_a_little  the 2921 swap closes between 0.002 and 0.05 of the agreement margin gap (through 701 and its own column)
    pred_e_2921_within_mlp16         the 2921 swap moves no more of 701's contrast than MLP 16's whole write does (0.137 +- 0.02)
...
OUT = ROOT / "circuits/followups/unit_2921_edit_v473_result.json"
CANDIDATE_ID = "chain.unit_2921_edit_v473"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, FEED_MIN, RANDOM_MAX, MARGIN_LO, MARGIN_HI, M16_V470, BAND, UNIT = 1e-3, 0.05, 0.01, 0.002, 0.05, 0.137, 0.02, 701
import random
...
FORWARDS_MAX = 13
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_2921_feeds_701": ">= 0.05", "pred_c_random_unit_does_not": "<= 0.01", "pred_d_2921_moves_the_margin_a_little": "0.002-0.05", "pred_e_2921_within_mlp16": "<= 0.137 + 0.02"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "feed_min": FEED_MIN, "random_max": RANDOM_MAX, "margin": [MARGIN_LO, MARGIN_HI], "m16_v470": M16_V470, "band": BAND, "unit": UNIT, "u16": {k: list(v) for k, v in U16.items()}}}
    ...
    MODES = {"m16": ({}, (0,)), "u2921": ({}, (0,)), "urand": ({}, (0,))}; forwards = 0
    def logits_margin(x):
    ...
    mv, mc = verb["moved"], verb["margin_closed"]
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_2921_feeds_701": mv["u2921"] >= FEED_MIN, "pred_c_random_unit_does_not": abs(mv["urand"]) <= RANDOM_MAX,
                   "pred_d_2921_moves_the_margin_a_little": MARGIN_LO <= mc["u2921"] <= MARGIN_HI, "pred_e_2921_within_mlp16": mv["u2921"] <= M16_V470 + BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "unit_2921_edit_result_v473", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
