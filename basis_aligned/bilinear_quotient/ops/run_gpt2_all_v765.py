...
"""Replication, all-in-one, HF families (OPT / Llama-style), ROW-CENTRED logits (see v760: per-query-row constants are removed before kernels are estimated and added back in the program). MODEL = gpt2 (GPT-2 small, re-run under the row-centred protocol so that all softmax models share one instrument; v751-v754 used raw
logits). Same GPT-2-tokenizer rows as bilin18 (513 tokens = 512 predictions).
