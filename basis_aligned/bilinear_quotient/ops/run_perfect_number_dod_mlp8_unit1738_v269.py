...
"""Perfect-number have/has DoD (v269): what is MLP-8 unit 1738? v266: on 11.3's have - has direction at the noun, 1738 is the second unit (23% of MLP 8's write) after
829 (35%); on the pronoun line's 9.6 direction it appeared with the OPPOSITE sign (v168: -29k, v250: -5%). Same readings as v267 for unit 1738 on the v97 rows: u_1738 at
the noun and the final per row, the sign of (plural - singular), cos(Down8[:, 1738], r_11.3), and the pooled have - has margin change when 1738 is zeroed at the noun,
against 16 random single units, and at all positions.
PREDICTIONS (scored as written; failures preserved; priors unsure -- 1738 may be a singular detector read with a different sign, or a second plural unit)
    pred_a_unit_separates_plural_singular  u_1738(noun) has the same sign of (plural - singular) on >= 90% of the aligned pairs
    pred_b_down_aligns_with_reader         |cos(Down8[:, 1738], r_11.3)| >= 0.30
    pred_c_unit_fires_at_noun              the pooled |plural - singular| contrast of u_1738 at the noun is >= 1.5x that at the final
    pred_d_edit_live_and_beats_random_units  zeroing 1738 at the noun removes >= 0.01 of the pooled have - has margin and more than every one of the 16 random units
...
OUT = ROOT / "circuits/followups/perfect_number_dod_mlp8_unit1738_v269_result.json"
CANDIDATE_ID = "perfect_number.have_vs_has.dod_mlp8_unit1738_v269"
LAYER, UNIT, HEAD, AGREE_MIN, COS_MIN, RATIO_MIN, EDIT_MIN, N_NULL, SEED, BATCH = 8, 1738, (11, 3), 0.90, 0.30, 1.5, 0.01, 16, 269, 32
CUES = {L._single(" these"), L._single(" this")}
FORWARDS_MAX = 60
PREDICTIONS = {"pred_a_unit_separates_plural_singular": ">= 0.90", "pred_b_down_aligns_with_reader": ">= 0.30", "pred_c_unit_fires_at_noun": ">= 1.5x the final", "pred_d_edit_live_and_beats_random_units": ">= 0.01 and > 16 random"}
...
def main() -> None:
    ...
    OUT.write_text(json.dumps({"schema": "aspectual_dod_mlp8_unit1738_result_v269", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "report": report, "predictions": predictions, "forwards": forwards,
