#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over eight new spec-authored behaviours, plus one instrument.
"""v204: the AMENDED tier-3 battery (v196/v197 protocol, unchanged) over EIGHT NEW spec-authored behaviours -- plus one instrument.

The candidate-module pool was exhausted at v201 (the unused modules are variants and controls), so throughput now comes from
AUTHORING: eight behaviours written today through `circuit_fast_screen_behaviour_spec` (the ninth standing lesson), each a
single-cue swap with a matched final token, 32 A1/A2/P/C rows, ABAB directions: reflexive_gender (king/queen -> himself/herself),
pronoun_gender (same cue -> he/she), reflexive_number (pilots/pilot -> themselves/himself), pronoun_number (-> they/he),
demonstrative_number (these/this ADJ -> ones/one), comparative_frame (less/as ADJ -> than/as), deixis (came/went straight ->
here/there), polarity_addition (often/never ... often/never ADJ -> too/either). Nothing in the corpus read gender, deixis, the
comparative correlate or the additive particle before; the number and polarity readouts are siblings of existing circuits and
get a separability rung afterwards, not here. Capability printed BEFORE the run on CPU (`g.prepare(valid_only=True)`, 32 rows per
family): zero A1/A2 rows dropped on all eight; mean base/donor margins A1: reflexive_gender -1.75/+1.85, pronoun_gender
-2.14/+2.32, reflexive_number -3.43/+3.36, pronoun_number -2.00/+1.89, demonstrative_number -3.85/+4.10, comparative_frame
-7.32/+7.23, deixis -2.32/+2.56, polarity_addition -3.52/+3.71; A2 weakest is polarity_addition -1.15/+0.88. Protocol identical
to v197/v201 (greedy pool 40, target 0.88, min_gain 0.005, max 30; row 2 = MIN over the two held directions >= 0.80; row 3 dim
A1 CE LB975 > 0 and >= 0.10; row 4 constrained rank-1 own-C UB975 <= 0.01 on 16 held C rows -- v203 measured that width, the bar
stays until the board answers; row 5 = A2-own / A2 full-rank ceiling in [0.5, 1.4], LB975 > 0, kept for comparability).
finiteness_selection is re-run as the instrument.

REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 4 of the 8 new behaviours pass all four rows (v197 15/24, v201 2/9 -- the new-module rate is
                           the relevant prior; capability here is stronger than v201's).                        prior 45%
  ...
  pred_e_row5_amended   -> A2-own / A2 ceiling within [0.5, 1.4] with LB975 > 0 on at least 5 of 8.               prior 55%
...
Accounting: four-row passes here join the one-protocol standing (same protocol and split as v197/v201); distinct-circuit counts
wait for separability (gender pair; number readouts vs possessive_number/quantifier; polarity_addition vs polarity_licensing).
An error receipt per behaviour counts as a miss on every row. ~27 s GPU each, ~4.5 min.
A1/A2 preps use g.prepare(valid_only=True) as in v201; `n_dropped` is in every receipt (expected 0 everywhere).
Smoke: V204_SMOKE=<out.json> (CPU, V204_SMOKE_ROWS=4 per split, V204_SMOKE_NAMES=deixis; pool 3/max 3, steps 5).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec8_v204_result.json"
...
NAMES = {"reflexive_gender": "reflexive_gender", "pronoun_gender": "pronoun_gender", "reflexive_number": "reflexive_number",
         "pronoun_number": "pronoun_number", "demonstrative_number": "demonstrative_number", "comparative_frame": "comparative_frame",
         "deixis": "deixis", "polarity_addition": "polarity_addition", "finiteness_selection": "finiteness"}
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec8_v204", "behaviours": 9, "constructions": 2,
...
def main() -> None:
    ...
    smoke = os.environ.get("V204_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V204_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V204_SMOKE_NAMES", "deixis").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec8_v204", "candidate_id": "corpus.unit_tier3_batch_amended_spec8_v204", "bars": BARS,
