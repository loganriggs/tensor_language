...
OUT=PT+'truefisher_top8_fresh_results.json'
...
def main():
    ...
    pa=gain1>=0.6*0.1285; pb=stt.median(gains)>=0.6*0.082; pc=sum(g>0 for g in gains)>=6
    pd=abs(arms['plain']['profile'][6]-1.7415)<=0.10
    out={'windows':rows,'gain_median':round(stt.median(gains),4),'windows_positive':int(sum(g>0 for g in gains)),
         'window1_gain':round(gain1,4),'S2110_empirical_top8_gain':0.1285,'S2116_median':0.082,
         'profiles':{k:v['profile'] for k,v in arms.items()},
         'pred_a_window1':bool(pa),'pred_b_fresh_median':bool(pb),'pred_c_positive_6_of_8':bool(pc),
    ...
    print(f"(a) window-1 true-Fisher top-8 gain {gain1:+.4f} >= {0.6*0.1285:.4f}: {'HELD' if pa else 'FAILED'}")
    print(f"(b) fresh median {stt.median(gains):+.4f} >= {0.6*0.082:.4f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) positive on {sum(g>0 for g in gains)}/8: {'HELD' if pc else 'FAILED'}")
