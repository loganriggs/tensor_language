...
"""LONGER JOINT FIT on Pythia-160m final (v778): v777 showed the six costliest heads' rank-32 programs cost 60% less when fitted in isolation (sum +0.062 -> +0.024; head 1.10 +0.024 -> +0.003) than in v760's joint 300-step fit — so part of the ladder's descent may be a FITTING limit, not a program-class limit. Same protocol as v760 with the rank-32 arm only, 1200 Adam steps (4x), evaluation every 100 steps, snapshot at the validation minimum. If recovery rises from 0.932 toward the early-checkpoint values, the ladder's late-training numbers are lower bounds set by the fit; if it stays near 0.932, the class is the limit. ROW-CENTRED logits (Logan, 22 Sep: 'test the specific hypothesis of why we're seeing what we're seeing'). MODEL = EleutherAI/pythia-160m (12 x 12 x 768, softmax, ROTARY on 16 of 64 head dims, GPT-NeoX tokenizer;
...
PREDICTIONS (scored as written; failures preserved)
    ...
    pred_c_rank32_cost          rank-32 snapshot recovery >= 0.95 (v760, 300 steps: 0.932). Prior: unsure
    pred_d_rank16_cost          (renamed in spirit: fit length used) the validation minimum is reached AFTER step 300. Prior: likely
    pred_e_rank32_values_preserved_scale rank-32: median value ratio in [0.5, 2] over the 24 most valuable heads. Prior: likely
PRICE (registered maximum): stock 7 + explicit 7; means 16; kernels 2; 144 x 7 = 1008; all-ablated 7; kernels-only 7 + col0 7; ladder 35; 1 arm x (kappa_r 26 + step-0 10 + 1200 + 12 x (26 + 7 + 7) + snapshot 9) = 1725 + manipulability 168; total ~3050 forwards, 1200 backwards. Bars: forwards <= 3200, backwards <= 1200.
"""
...
FORWARDS_MAX, BACKWARDS_MAX = 3200, 1200
EBATCH, TBATCH, STEPS, EVAL_EVERY, N_VAL, N_MANIP = 32, 8, 1200, 100, 96, 24
LR_MAP, LR_MAP_MIN, LR_K, LR_K_MIN = 3e-4, 3e-5, 0.01, 0.001
KS, ARMS = (1, 2, 4, 8, 16), (32,)
REPLAY_TOL, E4_MIN, REC32, STEP_MIN, RATIO = 0.002, 0.90, 0.95, 300, (0.5, 2.0)
PREDICTIONS = {"pred_a_instrument_replays": "+-0.002", "pred_b_kernel_energy_top4": ">= 0.90", "pred_c_rank32_cost": "recovery >= 0.95 (v760: 0.932)", "pred_d_rank16_cost": "validation minimum after step 300", "pred_e_rank32_values_preserved_scale": "median ratio in [0.5, 2]"}
