#!/usr/bin/env python3
# BQGATE:1280bodyforwards;160extraMLP10;160prefixes<=248tokens;180seconds.
"""pred_a nativefour/boundary9/boundary10 replay and state partition <=1e-4 relative.
pred_b MLP-local effect approximates full block10 local effect <=.20 relative each4regionalcells.
...
Price1280fullforwards plus160MLP10 evaluations; reused160prefixes,180seconds,no fitting.
"""
...
STEM='CROSSFIRST_BLOCK10_SPLIT_V1'
@torch.no_grad()
def main():
 ...
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('1280forwards160extraMLP10;8arms, native4/boundary9/boundary10/MLPlocal/attentionpropagated');return
 ...
 for lo,hi in [(0,96),(96,160)]:replay.extend([rel(measures[lo:hi,:4],prior[lo:hi,:4]),rel(measures[lo:hi,4:6],prior[lo:hi,4:6])])
 cells=[]
 ...
 result={'pred_a':max(replay+statechecks)<=1e-4,'pred_b':all(c['MLP_only_error']<=.2 for c in cells[:4]),'pred_c':all(c['composition_error']<=.1 for c in cells[:4]),'anchor_replay':replay,'max_state_partition_error':max(statechecks),'cells':cells,'body_forwards':count,'extra_MLP10_evaluations':160,'seconds':time.perf_counter()-tic,'scope':'Finite local block10 generation split at additive output background: MLP own mixed response versus propagation of attention mixed input. Native borrowed states/fullweights/readout remain; no fitting/OOD/semantic promotion.'}
