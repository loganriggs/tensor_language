#!/usr/bin/env python
"""early_frame_smoothness_probe -- is the rotating early read frame (§2750) SMOOTH: sliding-window, leave-one-out and neighbour-only
cores for the 22 early sublayers; and does the settled region start at block 8 (own cores 0-7 + one core for 8-17 vs §2751)? CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_sliding_window_is_free pred_c_frame_predictable_from_neighbours pred_d_leave_one_out
#                     pred_e_settled_from_block_8
...
Preregistration: polynomial_causal/EARLY_FRAME_SMOOTHNESS_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/early_frame_smoothness_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("early_frame_smoothness_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "EARLY_FRAME_SMOOTHNESS_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "whole_model_width_program_probe_results.json"   # §2751
OUT = ROOT / "early_frame_smoothness_probe_results.json"
HASHES = {PREREG: "102b06cc804fcd2b7604ce6f8f68373cf09249965400e1cff639e3445bcbb9a6", PRIOR: "a3df6484be4f8973f7711b1f887be39e2d8ab0adceda5af51aadc2efdb98cafc",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "early_frame_smoothness_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_EARLY22_OWN_768 = 0.057; PRIOR_ALL36_768 = 0.197; LAM = 1e-2
K = 768; EARLY = list(range(0, 11)); LATE = list(range(11, 18)); SPLIT = 8
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_max": 0.02, "c_max": 0.04, "d_max": 0.03, "e_max": 0.03}
NULLS = {"b_min": 0.06, "c_min": 0.10, "d_min": 0.08, "e_min": 0.10}
...
def main():
    ...
    def nbr_core(kd, l, mode):
        if mode == "win3":
            mem = blocks([l - 1, l, l + 1])
        elif mode == "loo":
            mem = [s_ for s_ in blocks([l - 1, l, l + 1]) if s_ != (kd, l)]
        else:
            mem = blocks([l - 1, l + 1])
        return core_of(mem)
    U_late = core_of(LSITES); U_8 = core_of(blocks(range(SPLIT, NL)))
    arms = {"EARLY22_OWN_768": run({s_: head(s_, K) for s_ in ESITES})}
    full = {s_: head(s_, K) for s_ in ESITES}; full.update({s_: head(s_, K, U_late) for s_ in LSITES}); arms["ALL36_768"] = run(full)
    for mode in ("win3", "loo", "nbr"):
        arms[f"EARLY22_{mode.upper()}_768"] = run({s_: head(s_, K, nbr_core(s_[0], s_[1], mode)) for s_ in ESITES})
    split = {s_: head(s_, K) for s_ in blocks(range(0, SPLIT))}; split.update({s_: head(s_, K, U_8) for s_ in blocks(range(SPLIT, NL))}); arms["ALL36_SPLIT8_768"] = run(split)
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    gaps = {f"{mode}_minus_own": arms[f"EARLY22_{mode}_768"] - arms["EARLY22_OWN_768"] for mode in ("WIN3", "LOO", "NBR")}
    gaps["split8_minus_all36"] = arms["ALL36_SPLIT8_768"] - arms["ALL36_768"]
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["EARLY22_OWN_768"] - PRIOR_EARLY22_OWN_768) <= BARS["repro_tol"] and abs(arms["ALL36_768"] - PRIOR_ALL36_768) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_sliding_window_is_free': bool(gaps["WIN3_minus_own"] <= BARS["b_max"]),
        'pred_c_frame_predictable_from_neighbours': bool(gaps["NBR_minus_own"] <= BARS["c_max"]),
        'pred_d_leave_one_out': bool(gaps["LOO_minus_own"] <= BARS["d_max"]),
        'pred_e_settled_from_block_8': bool(gaps["split8_minus_all36"] <= BARS["e_max"]),
    }
    nulls = {"b_null_ge_.06": bool(gaps["WIN3_minus_own"] >= NULLS["b_min"]), "c_null_ge_.10": bool(gaps["NBR_minus_own"] >= NULLS["c_min"]),
             "d_null_ge_.08": bool(gaps["LOO_minus_own"] >= NULLS["d_min"]), "e_null_ge_.10": bool(gaps["split8_minus_all36"] >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "early22_own_768": arms["EARLY22_OWN_768"], "prior_early22_own_768": PRIOR_EARLY22_OWN_768, "all36_768": arms["ALL36_768"], "prior_all36_768": PRIOR_ALL36_768},
