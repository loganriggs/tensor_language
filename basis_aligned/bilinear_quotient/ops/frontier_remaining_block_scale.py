...
def main():
    ...
    _E0 = {k: {c: v.clone() for c, v in S[k][3].items()} if isinstance(S[k][3], dict) else S[k][3].clone()
           for k in order2 if k in S and S[k][0] == 'tail'}
    ...
    def _apply_taile(sc):
        for _k, _L0 in _E0.items():
            ...
            if isinstance(_L0, dict):
                _new = {c: (v.clone() if sc is None else (v.float()*float(sc)).to(v.dtype))
                        for c, v in _L0.items()}
            else:
                _new = _L0.clone() if sc is None else (_L0.float()*float(sc)).to(_L0.dtype)
            S[_k] = (_kind, _Wp, _DICT, _new)
