#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_five_writes_replay_on_fresh_rows pred_c_rest_of_their_blocks_small pred_d_all_attention_is_the_channel pred_e_five_below_all
"""The pronoun readers' writes on a THIRD natural set (v545). v485: on the 122 natural pairs of sets 1 + 2 (fineweb docs 0-16233, pile-10k) the five pronoun
readers {9.6, 10.1, 10.5, 12.4, 15.1} swapped jointly by their writes into the answer position close 0.803 of the they - he margin gap, the other 32 heads of
their blocks 0.026, all attention 0.973 (v481). Every number so far comes from those two sets; this is the untested OOD cell (review 42). Here the same
edits on v544's rows -- fineweb documents after the first 16234, disjoint from both earlier sets, mined outcome-blind with the same filters.
PREDICTIONS (scored as written; failures preserved; priors from v481 / v485)
    pred_a_baseline_replays               the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_five_writes_replay_on_fresh_rows  the five readers' joint write swap closes 0.803 +- 0.15 on the fresh rows
    pred_c_rest_of_their_blocks_small     the other 32 heads of blocks 9 / 10 / 12 / 15 close <= 0.10 jointly
    pred_d_all_attention_is_the_channel   every block's attention write into the answer position closes >= 0.90
    pred_e_five_below_all                 the five heads' joint swap is <= the all-attention swap
PRICE (registered maximum): 2 batches x 4 passes = 8 forwards (64 fresh pairs); 0 backwards; 0 fits. Bar <= 9.
"""
...
OUT = ROOT / "circuits/followups/third_set_pronoun_writes_v545_result.json"
CANDIDATE_ID = "chain.third_set_pronoun_writes_v545"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, FIVE_SUM, BAND, REST_MAX, ALL_ATTN, SHARE_MIN, ALL_MIN, UNIT = 1e-3, 0.803, 0.15, 0.10, 0.973, 0.80, 0.90, 701
...
FORWARDS_MAX = 9
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_five_writes_replay_on_fresh_rows": "0.803 +- 0.15", "pred_c_rest_of_their_blocks_small": "<= 0.10", "pred_d_all_attention_is_the_channel": ">= 0.90", "pred_e_five_below_all": "FIVE <= ALL"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "five_sum": FIVE_SUM, "band": BAND, "rest_max": REST_MAX, "all_attn": ALL_ATTN, "share_min": SHARE_MIN, "unit": UNIT}}
    ...
    MODES = {"FIVE_W": ({}, (0,)), "REST_W": ({}, (0,)), "ALL": ({}, (0,))}; SETS_W = {"FIVE_W": FIVE_W, "REST_W": REST_W, "ALL": {l: list(range(N_HEAD)) for l in range(18)}}; forwards = 0
    def logits_margin(x):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_five_writes_replay_on_fresh_rows": abs(mc["FIVE_W"] - FIVE_SUM) <= BAND, "pred_c_rest_of_their_blocks_small": abs(mc["REST_W"]) <= REST_MAX,
                   "pred_d_all_attention_is_the_channel": mc["ALL"] >= ALL_MIN, "pred_e_five_below_all": mc["FIVE_W"] <= mc["ALL"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "third_set_pronoun_writes_result_v545", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
