#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_no_single_stage_leads pred_c_late_stage_16_small pred_d_singles_sum_bounded pred_e_stage_12_or_10_largest
"""The pronoun slot's MLP rebuild by stage (v504). v503: MLPs 9-16 at the pronoun answer position jointly re-express 0.21 of the they - he margin (verb slot:
0.39 jointly, singles 0.05-0.10 each, v474). Here each MLP 9..16's write at the pronoun answer position swapped singly, 122 natural pairs: which stage
re-expresses the copied number for the pronoun readout, and is the late stage (MLP 16, which fed the verb detector) absent here?
PREDICTIONS (scored as written; failures preserved; priors from v464 / v474 / v503)
    pred_a_baseline_replays      the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_no_single_stage_leads no single MLP closes >= 0.12 of the gap
    pred_c_late_stage_16_small   MLP 16 closes <= 0.05 (no late detector for pronouns, v464). Prior: unsure
    pred_d_singles_sum_bounded   the eight singles sum to between 0.15 and 0.45 (nested around the joint 0.214)
    pred_e_stage_12_or_10_largest  the largest single stage is MLP 12 or MLP 10 (the readers' blocks). Prior: unsure
PRICE (registered maximum): 4 batches x 9 passes = 36 forwards; 0 backwards; 0 fits. Bar <= 37.
"""
...
OUT = ROOT / "circuits/followups/pronoun_slot_stages_v504_result.json"
CANDIDATE_ID = "chain.pronoun_slot_stages_v504"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, SINGLE_MAX, M16_MAX, SUM_LO, SUM_HI, UNIT = 1e-3, 0.12, 0.05, 0.15, 0.45, 701
import random
...
FORWARDS_MAX = 37
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_no_single_stage_leads": "< 0.12 each", "pred_c_late_stage_16_small": "<= 0.05", "pred_d_singles_sum_bounded": "0.15-0.45", "pred_e_stage_12_or_10_largest": "12 or 10"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "single_max": SINGLE_MAX, "m16_max": M16_MAX, "sum": [SUM_LO, SUM_HI], "unit": UNIT}}
    ...
    total = sum(mc.values())
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_no_single_stage_leads": max(mc.values()) < SINGLE_MAX, "pred_c_late_stage_16_small": abs(mc["m16"]) <= M16_MAX,
                   "pred_d_singles_sum_bounded": SUM_LO <= total <= SUM_HI, "pred_e_stage_12_or_10_largest": max(mc, key=mc.get) in ("m12", "m10")}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pronoun_slot_stages_result_v504", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
