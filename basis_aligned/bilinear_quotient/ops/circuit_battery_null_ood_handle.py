"""circuit_battery_null_ood_handle -- a THIRD, independent population for attn8's percentile, because a bar crossing of .027 is weak.

SS2862 measured attn8's median percentile among live components at **.214** (bar <= .20, pred_a FALSE by .014). SS2866 replicated at
1.5x the rows and got **.1875** -- pred_a TRUE. The two runs straddle the bar, and the whole difference is **.027**. Declaring attn8 an
outlier on the strength of that is exactly the kind of bar-crossing this lane keeps catching, so SS2866 recorded the flip and
deliberately declined to upgrade the claim.

This rung supplies the independent handle instead of another rerun of the same population: the same 36-component sweep scored on the
**OOD** split -- a population built from held-out vocabulary pools, disjoint from both SELECT and TEST -- with TEST retained only as the
comparison axis for the ranking clause. If attn8's percentile lands near .19-.21 again on genuinely different situations, the reading
is stable across populations and not an artefact of one; if it moves substantially, then "attn8's percentile" is not a single number and
neither SS2862's FALSE nor SS2866's TRUE should be carried forward.

Bars are carried over verbatim from the null-calibration preregistration.
...
Preregistration: polynomial_causal/CIRCUIT_BATTERY_NULL_OOD_PREREGISTRATION.md
"""
...
SELF = Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/"
            "circuit_battery_null_ood_handle.py")
...
ROOT = R.ROOT
PREREG = R.POLY / "CIRCUIT_BATTERY_NULL_OOD_PREREGISTRATION.md"
...
RUNG = "circuit_battery_null_ood_handle"
SMOKE = os.environ.get("SURROGATE_SMOKE") == "1"
OUT = ROOT / (f"{RUNG}_smoke_results.json" if SMOKE else f"{RUNG}_results.json")
HASHES = {PREREG: "bc0a9e36ce6a028de9a229e68f2a14a193ff8ea97aaecc77dc51c08021faddf2", V6RES: "940c1dc148d89d6458c6eecf1052f8f49701d1ffa149a8b3a0bffc4bd28d7e74",
...
def main():
    ...
    for tid, ps in results.items():
        vals = {k: v["selectivity"] for k, v in ps["OOD"].items() if v["selectivity"] is not None}
        me = vals.get(w)
        if me is None:
            continue
    ...
    result = {"rung": RUNG, "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS,
              ...
              "summary": {"median_writer_percentile": med_pct, "writer_percentile": pcts,
                          ...
                          "median_component_rank_rho_ood_test": med_rho, "component_rank_rho_ood_vs_test": ranks},
