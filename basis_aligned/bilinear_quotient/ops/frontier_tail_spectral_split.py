"""IS THE EXCESS REALLY ISOTROPIC? SCALE HALF THE SPECTRUM INSTEAD OF ALL OF IT (Claude, LANE 1)

SS2917 refuted the under-regularisation hypothesis decisively -- more ridge is monotonically WORSE at every level tested (+0.0195 to
+0.0972) while uniform x0.25 is worth -0.2287 -- and read that as: **the end-to-end excess is isotropic, not concentrated in the
low-eigenvalue directions ridge attacks.** That reading is an INFERENCE from one failed family, not a measurement. Ridge is a
particular anisotropic shrinkage; its failure rules ridge out, and hints at isotropy without demonstrating it.

This measures it directly. Each tail link map is `LW = U diag(s) V'`. Applying the SAME adopted scalar 0.25 to only the **top r**
singular values, or only the **bottom** ones, holds the amount of shrinkage per direction fixed and varies only WHICH directions get
it. Three ranks x two halves, against the uniform arm measured in the same run.

  isotropic excess    every direction is equally over-large, so no split beats uniform, BOTH halves carry gain,
                      and the two halves roughly ADD to the whole.
  concentrated excess some split beats uniform -- the gain lives in a subspace, and then the correction is not a
                      scalar at all but a projection, which is a completely different object to carry forward.

**Either answer is a real advance on SS2917's inference**, and a concentrated result would matter more: it would say the two adopted
parameters are a coarse stand-in for something with structure, and would point at exactly which subspace.
...
#                     pred_c_no_spectral_split_beats_uniform_shrinkage
#                     pred_d_both_halves_of_the_spectrum_carry_gain
#                     pred_e_the_two_halves_add_up_to_the_whole

Preregistration: polynomial_causal/FRONTIER_TAIL_SPECTRAL_SPLIT_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run: six anisotropic spectral splits of the tail link maps against the adopted uniform x0.25")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_tail_spectral_split_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_TAIL_SPECTRAL_SPLIT_PREREGISTRATION.md'
PREREG_SHA = "fc472ad534b1e81aa409cdfa32061493740498a4e8fa5d182dfa662e83432710"
...
BARS = {"reproduce": 0.05, "anchor": 0.01, "beats_uniform": 0.01, "additive": 0.05}
R_GRID = [64, 256, 576]          # 576 = half of 1152, so top and bot are equal-sized there
SPEC_SCALE = 0.25                # the ADOPTED scalar, applied to half the spectrum instead of all of it
...
if __name__=='__main__':
    ...
    # Eight arms against ONE fitted stack: the adopted uniform x0.25 as an in-run control, plus six
    # anisotropic splits that apply the SAME scalar to only half of each link map's spectrum.
    ARMS[:] = ([('baseline', {}), ('uniform', {'tail_scale': SPEC_SCALE})] +
               [(f'{h}{r}', {'tail_spec': (r, h)}) for r in R_GRID for h in ('top', 'bot')])
    ...
    uni = cost['uniform']
    splits = {n: v for n, v in cost.items() if n != 'uniform'}
    best_split = min(splits, key=lambda n: splits[n])
    RMID = R_GRID[len(R_GRID)//2]
    additivity = round(cost[f'top{RMID}'] + cost[f'bot{RMID}'] - uni, 4)
    both_negative = {r: (cost[f'top{r}'] < 0 and cost[f'bot{r}'] < 0) for r in R_GRID}

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = abs(uni - S2896_COST) <= BARS['anchor']
    pc = splits[best_split] >= uni - BARS['beats_uniform']
    pd = all(both_negative.values())
    pe = abs(additivity) <= BARS['additive']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_uniform_scaling_reproduces_S2896':bool(pb),
           'pred_c_no_spectral_split_beats_uniform_shrinkage':bool(pc),
           'pred_d_both_halves_of_the_spectrum_carry_gain':bool(pd),
           'pred_e_the_two_halves_add_up_to_the_whole':bool(pe)}
    nulls={'b_null_the_anchor_fails':bool(abs(uni - S2896_COST) >= 0.03),
           'c_null_the_excess_is_concentrated_in_a_subspace':bool(not pc),
           'd_null_one_half_of_the_spectrum_carries_nothing':bool(not pd),
           'e_null_the_spectral_halves_interact':bool(not pe)}
    res={'rung':'frontier_tail_spectral_split','preds':preds,'nulls':nulls,'bars':BARS,
         'arms':out,'published_L2_F':2.6735,'rank_grid':R_GRID,'spectral_scale':SPEC_SCALE,
         'summary':{'L2_F_baseline':round(bF,4),'L2_F2_baseline':round(b2,4),
                    'cost_fresh':cost,'cost_holdout':cost_h,'cost_fit':cost_c,
                    'uniform_cost':uni,'S2896_adopted_cost':S2896_COST,
                    'anchor_deviation':round(abs(uni-S2896_COST),4),
                    'best_split':best_split,'best_split_cost':splits[best_split],
                    'best_split_minus_uniform':round(splits[best_split]-uni,4),
                    'both_halves_negative':{str(k):bool(v) for k,v in both_negative.items()},
                    'additivity_at_r':RMID,'additivity_nats':additivity},
    ...
    print(f"(b) uniform x{SPEC_SCALE} {uni:+.4f} vs SS2896 {S2896_COST:+.4f}: {'HELD' if pb else 'FAILED'}")
    for r in R_GRID:
        print(f"    r={r}: top {cost[f'top{r}']:+.4f} | bot {cost[f'bot{r}']:+.4f} | sum {cost[f'top{r}']+cost[f'bot{r}']:+.4f}")
    print(f"(c) no split beats uniform: best {best_split} {splits[best_split]:+.4f} vs uniform {uni:+.4f}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) both halves carry gain at every r: {'HELD' if pd else 'FAILED'}  {both_negative}")
    print(f"(e) additivity at r={RMID}: {additivity:+.4f}, |.| <= {BARS['additive']}: {'HELD' if pe else 'FAILED'}")
