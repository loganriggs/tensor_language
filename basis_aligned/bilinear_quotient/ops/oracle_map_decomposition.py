# GENERALISATION OR REPRESENTATION?  -- an ORACLE map fitted on the uncovered rows themselves.
#
# §1872 found two fallbacks with nothing in common -- §1785's least-squares linear map from the token
# embedding, and §1780/§1781's output-NN neighbour -- lose +0.781 and +0.811 nats against §1868's
# uncovered-token ceiling. The residual is common to both, and §1869 showed capacity is not the binding
# constraint above map rank ~128.
#
# Two things it could still be, and nothing in the record separates them:
#   GENERALISATION   the map is fitted on COVERED tokens and applied to UNCOVERED ones, and simply does
#                    not transfer;
#   REPRESENTATION   a linear function of the token embedding cannot express those rows at any rank, no
#                    matter what it is fitted on.
#
# An uncovered token's ideal row is computable at exactly the same cost as a covered one -- it is that
# token's own length-1 site output; it is merely not STORED. So an ORACLE map, least-squares fitted on
# the uncovered rows themselves, is buildable. It is not deployable (it needs the answers it predicts),
# and it is not proposed as a build: it is the bound that separates the two accounts.
#
# ROLES. All three, deployed 5,419-type covered set. DISCOVERY ONLY. Rung 3: §1872's open question.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
# Units per LESSONS 49: fallback loss is a DEFICIT on the uncovered population against the uncovered
# ceiling.
#   pred_a THE LOSS IS MOSTLY GENERALISATION: the oracle map's fallback loss is below half the PUBLISHED
#          map-alone loss of +0.78075 / +0.86225 / +0.83997 (§1870/§1872) on all three roles. If TRUE a
#          linear function of the embedding CAN express these rows and the deployed map fails only to
#          transfer from covered to uncovered tokens -- which points at the fitting set, a thing one can
#          change. If FALSE the embedding cannot express them even with the answers in hand, the residual
#          is REPRESENTATIONAL, and no better-fitted linear map will help -- which would close the linear
#          family and make §1871's neighbour-plus-residual idea unpromising for the same reason.
#   pred_b AND THE ORACLE IS NOT PERFECT EITHER: its loss stays above 0.10 nats on all three roles. A
#          per-token linear map from a 1152-dimensional embedding to a 1152-dimensional row has enough
#          parameters to interpolate, so a loss near zero would mean the object is trivially expressible
#          and the whole fallback question reduces to fitting. If FALSE that is what has happened and the
#          finding is larger than pred_a's.
#   pred_c AND THE COVERED ARM IS UNTOUCHED: covered CE identical between the deployed and oracle arms to
#          within 1e-6. Both change uncovered rows only; §1869, §1870 and §1872 each returned exactly
#          0.00e+00, and this is the same KNOWN-ANSWER check on a fourth construction.
#   pred_d CONTROLS: the deployed MAP arm reproduces §1872's PUBLISHED +0.78075 / +0.86225 / +0.83997
#          within 0.005; coverage is exactly 5,419; the covered arm attains its ceiling (a thirteenth
...
OUT = PT + 'ops/oracle_map_decomposition_results.json'
...
MAPRANKS = (64,)
ARMS2 = ('MAP', 'ORACLE')          # None = full rank; §1786/§1813 use this ladder
...
def main():
    ...
    print(f'ORACLE MAP DECOMPOSITION | 5419 types, map fitted on covered vs on uncovered | '
    ...
    genshare = {e: (fbloss['MAP'][e] - fbloss['ORACLE'][e]) / max(fbloss['MAP'][e], 1e-9)
                for e in roles2}
    ...
    pa = all(fbloss['ORACLE'][e] < 0.5 * S1872_MAP_M64[e] for e in roles2)
    pb = all(fbloss['ORACLE'][e] > 0.10 for e in roles2)
    pc = covspread <= 1e-6
    ...
    print(f'\n  the loss is mostly GENERALISATION (oracle < half of map) -> {pa}  ' + '  '.join(
        f'{e} {fbloss["ORACLE"][e]:+.5f} vs {0.5 * S1872_MAP_M64[e]:.5f}' for e in roles2),
        flush=True)
    print(f'  and the ORACLE is not perfect either (>0.10) -> {pb}  ' + '  '.join(
        f'{e} {fbloss["ORACLE"][e]:+.5f}' for e in roles2), flush=True)
    ...
    json.dump({'run': 'oracle_map_decomposition', 'n_covered': NFULL, 'arms': list(ARMS2),
               ...
               'predictions': {'pred_a_mostly_generalisation': bool(pa),
                               'pred_b_oracle_imperfect': bool(pb),
