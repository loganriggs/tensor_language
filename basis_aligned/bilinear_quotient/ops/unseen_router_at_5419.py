# THE UNSEEN-TARGET ROUTER WHERE THE TRADE COULD GO THE OTHER WAY.
#
# §1955 established at 16,110 that unc_mass is a genuine input-side predictor of the unseen-target case
# (+23.3/+24.3/+19.8pp quartile separation) and that routing on it beats §1939's cosine -- but that NO
# routed build should be shipped there, because every msk arm costs 0.016-0.036 nats of CE, two to five
# times the converged build's whole margin over §1931, and pure_map matches them on the 0-0 bucket for
# less. §1955 named the caveat: that is a statement about the OPERATING POINT, not the signal. At 5,419
# the uncovered arm is ~24% of scored positions instead of ~10% (§1936) and the blend's CE margin over
# the deployed design is six times larger (§1951), so the same trade can come out differently.
#
# §1954 background: the converged build loses the unseen (0-0) target bucket on 5 of 6 role-coverage cells.
...
#   pred_b AND THE CE PRICE IS LOWER HERE: the best msk arm's pooled CE penalty against the unrouted
#          converged build is SMALLER at 5,419 than the +0.0159 / +0.0173 / +0.0275 nats §1955 measured
#          at 16,110, on at least 2 of 3 roles. If FALSE the routing costs as much or more where the
#          fallback matters more, and §1955's negative is the general answer rather than a 16,110 one.
#   pred_c AND IT IS FINALLY WORTH IT: some msk arm beats the unrouted converged build on pooled 0-0
#          kept-fraction AND costs less than 0.010 nats of pooled CE -- §1947's own spending threshold --
#          on at least 2 of 3 roles. This is the deployable claim §1955 could not make. If FALSE the
#          router is unprofitable at BOTH coverages and the unseen-bucket deficit is simply the price of
#          the blend, which is the more likely outcome and worth stating flatly.
...
t0 = time.time()
OUT = B.PT + 'ops/unseen_router_at_5419_results.json'
...
print(f'UNSEEN-TARGET ROUTER AT 5,419 | msk arms on unc_mass | the S1955 caveat tested | '
...
S1951_CE = (5.94788, 5.91648, 5.93606)
...
S1955_PEN = (0.0159, 0.0173, 0.0275)     # §1955's best-msk CE penalty vs the converged build, 16,110
pen = {r: min(ce(r, a) - ce(r, KNEE_LAB) for a in MSK) for r in B.ROLES}
beats_cos = sum(1 for i, r in enumerate(B.ROLES) if pen[r] < S1955_PEN[i])
pb = beats_cos >= 2
closes = sum(1 for r in B.ROLES
             if any(kf(r, a, 'pooled', BOT) > kf(r, KNEE_LAB, 'pooled', BOT)
                    and ce(r, a) - ce(r, KNEE_LAB) < 0.010 for a in MSK))
pc = closes >= 2
...
print(f'  and the CE price is lower here than §1955 measured at 16,110 (>=2 roles) -> {pb}  '
      f'{beats_cos}/3  (penalties ' + ' '.join(f'{pen[r]:+.4f}' for r in B.ROLES)
      + f' vs {S1955_PEN})', flush=True)
print(f'  and some msk arm gains the 0-0 bucket for under 0.010 nats (>=2 roles) -> {pc}  {closes}/3',
      flush=True)
...
B.report({'pred_a_signal_is_real': pa, 'pred_b_ce_price_is_lower': pb,
          'pred_c_finally_worth_it': pc, 'pred_d_controls': pd},
         {'config': {'arms': list(ARMS), 'coverages': [16110], 'table_rank': 'FULL',
                     'costs_M': COST,
                     'built_with': f'ops/bqlib.py v{B.LIB_VERSION}',
                     'ROLE_NOTE': 'DISCOVERY ONLY, rung 2 -- the S1955 caveat at the other coverage.',
