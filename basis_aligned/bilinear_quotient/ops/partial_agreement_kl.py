# IS A PARTIAL COMPILE FUNCTIONALLY CLOSER TO THE MODEL, OR JUST BETTER AT THE TASK?
#
# §1816 established that the cost/fidelity frontier is axis-invariant: the CE and top-1 Pareto sets are
# the same nine arms. But BOTH of those are task metrics -- they score the program against the true next
# token. §1788 introduced two instruments that score it against the MODEL instead, and ran them on the
# all-sites program only: top-1 AGREEMENT with the live model (23.57 / 22.71 / 24.21%) and
# KL(live || program) (2.88031 / 3.04866 / 2.75451 nats).
#
# A partial compile at L13 is far more accurate than the all-sites program (32.80% against 12.88%). The
# question those numbers cannot answer is whether it is also functionally NEARER the model, or whether
# it simply retains enough native machinery to score well while computing something else. Agreement and
# KL separate those.
#
# ROLES. skip7000, skip11000, skip1200; rank-64 tables at depths -1, 7, 10, 13. DISCOVERY ONLY.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39,
# failure branches enumerated per LESSON 44, constants named with UNITS per LESSON 49, NO RATIOS
# per LESSON 51:
#   pred_a IT BECOMES THE MODEL'S PREDICTOR: at L13 the program agrees with the live model's top-1 on at
#          least 50% of positions, against 23.57 / 22.71 / 24.21% for the all-sites program. If FALSE,
#          a program that reaches 83% of live top-1 accuracy still disagrees with live on most
#          positions -- it would be scoring well by a different route, which is the more interesting
#          negative and would say task accuracy is a poor proxy for functional similarity even at
#          depth.
#   pred_b AND DISTRIBUTIONALLY CLOSE: KL(live || program) at L13 is under 1.0 nat, against 2.88 / 3.05 /
#          2.75 for the all-sites program. Scored separately from pred_a because agreement is an argmax
#          statistic and KL is a distributional one; §1788 found them to move together on the all-sites
#          program, and a split here would locate where they part.
#   pred_c AND IT IMPROVES SMOOTHLY: agreement rises monotonically across depths -1 -> 7 -> 10 -> 13, at
#          every role. If FALSE there is a depth at which restoring more native machinery makes the
#          program agree with the model LESS, which given §1806's directional poisoning would be worth
#          isolating.
#   pred_d CONTROLS, cross-run per LESSON 42: the all-sites arm reproduces §1788's PUBLISHED agreement
#          within 0.03 and its KL within 0.25 (looser than usual because §1788 measured the FULL-RANK
#          all-sites program and this arm is rank-64 -- the tolerance is set to admit that known
#          difference and no more); the L-1 and L13 CE reproduce §1816's PUBLISHED 6.1733 / 6.1526 /
#          6.1446 and 4.1815 / 3.9736 / 4.2836 within 0.02; live CE reproduces 3.13704 / 2.93450 /
#          3.23027; coverage 5419 of 50257.
...
OUT = PT + 'ops/partial_agreement_kl_results.json'
PROBE_LS = ()
KIND_LS = ()
RANKS = (64,)                # §1816's frontier arm at every depth
...
def main():
    ...
    agr = {e: {k: res[e][k]['agree'] for k in keys} for e in roles}
    kl = {e: {k: res[e][k]['kl'] for k in keys} for e in roles}
    # NO RATIO ANYWHERE (LESSON 51)
    pa = all(agr[e]['r64_L13'] >= 0.50 for e in roles)
    pb = all(kl[e]['r64_L13'] < 1.0 for e in roles)
    pc = all(agr[e][f'r64_L{L2}'] >= agr[e][f'r64_L{L1}'] - 1e-12
             for e in roles for L1, L2 in zip(DEPTHS, DEPTHS[1:]))
    pd = (all(abs(res[e]['all_substituted']['top1'] - S1789_PROG[e]) <= 0.001
              and abs(res[e]['live_model']['top1'] - S1789_LIVE[e]) <= 0.001 for e in roles)
          and all(abs(agr[e]['r64_L-1'] - S1788_AGREE[e]) <= 0.03
                  and abs(kl[e]['r64_L-1'] - S1788_KL[e]) <= 0.25
                  and abs(ce[e]['r64_L-1'] - S1816_CE_R64[-1][e]) <= 0.02
                  and abs(ce[e]['r64_L13'] - S1816_CE_R64[13][e]) <= 0.02
    ...
    print('\n  four instruments across compile depth (rank-64 tables):', flush=True)
    for k in sorted(keys, key=lambda k: cost[k]):
        print(f'    {k:12s} {cost[k]/1e6:9.3f}M  ' + '  '.join(
            f'{e} CE {ce[e][k]:.4f} top1 {acc[e][k]:6.2%} agree {agr[e][k]:6.2%} '
            f'KL {kl[e][k]:.4f}' for e in roles), flush=True)
    print(f'\n  at L13 the program agrees with live on >=50% of positions -> {pa}  ' + '  '.join(
        f'{e} {agr[e]["r64_L13"]:.2%}' for e in roles), flush=True)
    print(f'  and KL(live||program) at L13 is below 1.0 nat -> {pb}  ' + '  '.join(
        f'{e} {kl[e]["r64_L13"]:.4f}' for e in roles), flush=True)
    print(f'  agreement rises monotonically with depth -> {pc}  ' + '  '.join(
        f'{e} ' + '/'.join(f'{agr[e][f"r64_L{L}"]:.1%}' for L in DEPTHS) for e in roles),
        flush=True)
    ...
    json.dump({'run': 'l5_cliff_probe',
               ...
               'ce_all': ce, 'agreement': agr, 'kl': kl,
               'predictions': {'pred_a_agreement_half_at_L13': bool(pa),
                               'pred_b_kl_under_one_nat_at_L13': bool(pb),
                               'pred_c_agreement_monotone_in_depth': bool(pc),
