# IS mlp5's COST CONCENTRATED IN A FEW CHANNELS?  -- testing Logan's outlier-dimension hypothesis.
#
# §1834 measured the single-site cost of all 34 sites and found one that dwarfs the rest: compiling
# `mlp5` on top of a compiled layer 0 costs **+61.2pp**, leaving 3.6% of the gap recovered out of B0's
# 64.8%. It beats the runner-up (mlp3, +48.5) by 12.7pp and is 95% of everything B0 has, in one site.
# Layer 5 is also the layer Logan independently flagged for its constant-bias head and outlier dimensions
# (§1089), and the layer §1804 found exploding 152x on a compiled stream -- though §1834 puts the
# destructive SITE in the MLP (+61.2) and not the attention (attn5, only +16.9).
#
# If 61.2pp lives in a handful of the 1152 residual channels, the object is nameable. This substitutes
# mlp5's output as usual but keeps k channels LIVE, for k in 4, 16, 64, 256, under three selections:
#
#   OUT   the k channels with the largest mean |live output|      -- outlier dimensions, §1089's object
#   DISC  the k with the largest mean |live - substituted row|    -- where the table is most wrong
#   RAND  k channels drawn uniformly at a fixed seed              -- the CONTROL that must be able to bite
#
# The RAND arm is the point of the design, per LESSON 46. "Keeping 256 of 1152 channels live helps" is
# not a finding unless keeping 256 ARBITRARY channels does not. Both principled selections are measured
# because they answer different questions: DISC is where the substitution errs, OUT is what the outlier
# hypothesis names, and they need not coincide.
#
# Channel statistics are measured IN THE B0 STREAM -- the stream the arm actually runs in -- not on the
# live model, so what is ranked is the discrepancy the arm will suffer.
#
# ROLES. All three. FULL RANK, settled program. §1824's sequential gain correction. DISCOVERY ONLY.
#
...
#   pred_a THE COST IS CONCENTRATED: keeping the 16 highest-DISC channels live -- 16 of 1152, 1.4% of
#          the residual width -- recovers at least half of mlp5's 61.2pp, i.e. the arm reaches a gap
#          fraction of at least 34.2%. If TRUE the most destructive site in the network is destructive
#          through a nameable handful of dimensions, which is the first concrete object this arc has
#          produced. If FALSE the cost is spread over many channels, there is no small set to name, and
#          mlp5 is expensive as a whole map rather than through a few directions.
#   pred_b AND IT IS THE OUTLIER CHANNELS: at k=16 the OUT selection recovers at least 80% of what DISC
#          recovers. If TRUE §1089's outlier dimensions are the mechanism and Logan's hypothesis carries
#          to this object. If FALSE the destructive channels are NOT the high-magnitude ones -- the
#          substitution errs somewhere other than where the signal is largest -- and the outlier framing
#          does not explain mlp5's cost however well it describes layer 5 otherwise.
#   pred_c AND SELECTION MATTERS: at every k, BOTH principled selections beat RAND by at least 10pp. If
#          FALSE, keeping any k channels live works as well as keeping the chosen ones, the effect is
#          about how much live signal survives rather than which part of it, and no channel set is
#          special -- which would make pred_a's concentration finding vacuous even if it passed.
#   pred_d CONTROLS, cross-run per LESSON 42: the k=0 arm reproduces §1834's PUBLISHED mlp5 figure (3.6%
#          gap fraction, +61.2pp) within 0.03; B0 reproduces §1829's PUBLISHED 64.8% and §1806's
#          PUBLISHED raw 37.4%. Endpoints must reproduce §1789's FULL-RANK top-1; the placement control
#          must move top-1 under 0.05pp. Coverage 5419 of 50257.
...
OUT = PT + 'ops/mlp5_channel_concentration_results.json'
...
TARGET = ('mlp', 5)                   # §1834's most expensive site, +61.2pp
KEEPS = (4, 16, 64, 256)              # channels kept LIVE, of D = 1152
SELECTORS = ('DISC', 'OUT', 'RAND')
# §1834's PUBLISHED sequential gap fraction for B0 + compiled mlp5, skip7000
S1834_MLP5_GAPFRAC = 0.0357
...
def partial_row_hook(full_rows, keep_ch):
    """Substitute the per-token row EXCEPT on `keep_ch`, where the LIVE module output is kept.

    keep_ch is a bool mask over the D residual channels. keep_ch all-False is exactly row_hook, which
    is what makes the k=0 arm a cross-run check against §1834 rather than a new object."""
    def hook(mod, args, out):
        y = out[0] if isinstance(out, tuple) else out
        sub = full_rows[STATE['idx'].reshape(-1)].reshape(y.shape).to(y.dtype)
        sub = torch.where(keep_ch.view(1, 1, -1), y, sub)
        return (sub,) + tuple(out[1:]) if isinstance(out, tuple) else sub
    return hook


def chan_probe(full_rows, store):
    """Per-channel mean |live output| and mean |live - row|, measured IN the stream that is running."""
    ...
    def hook(mod, args, out):
        ...
        s = store.setdefault('a', [torch.zeros(D, device=DEV), torch.zeros(D, device=DEV), 0])
        s[0] += o[:, 64:].abs().sum((0, 1))
        s[1] += (o[:, 64:] - rr[:, 64:]).abs().sum((0, 1))
...
def main():
    ...
    print(f'MLP5 CHANNEL CONCENTRATION | keep k of 1152 channels live | '
    ...
    # every arm here compiles the same SITE SET, so one calibration serves them all -- what varies
    # between arms is which CHANNELS of mlp5 are substituted, and gains act on attention c_proj only
    armsites = B0_SITES + [TARGET]
    gseq['arm'] = seq_gains(armsites)
    gseq['B0'] = seq_gains(list(B0_SITES))

    # channel statistics, measured in the B0 stream with the target's substitution in place
    store = {}
    evaluate(ev0, [(st, allh[st]) for st in B0_SITES] + [(TARGET, chan_probe(frr[TARGET], store))],
             keep_mask)
    acc = store['a']
    mag = (acc[0] / max(acc[2], 1)).cpu()
    disc = (acc[1] / max(acc[2], 1)).cpu()
    gg = torch.Generator().manual_seed(0)
    rnd = torch.randperm(D, generator=gg)
    rank = {'DISC': disc.argsort(descending=True), 'OUT': mag.argsort(descending=True),
            'RAND': rnd}
    print(f'\n  mlp5 channel stats in the B0 stream: mean|live| max {float(mag.max()):.3f} '
          f'median {float(mag.median()):.3f}; mean|live-row| max {float(disc.max()):.3f} '
          f'median {float(disc.median()):.3f}; DISC/OUT top-16 overlap '
          f'{len(set(rank["DISC"][:16].tolist()) & set(rank["OUT"][:16].tolist()))}/16', flush=True)

    print(f'\n  arms  ({time.time() - t0:.0f}s)', flush=True)
    run_g('B0', list(B0_SITES), gseq['B0'])
    zero = torch.zeros(D, dtype=torch.bool, device=DEV)
    run_g('K0', armsites, gseq['arm'],
          {TARGET: partial_row_hook(frr[TARGET], zero)})
    for selname in SELECTORS:
        for k in KEEPS:
            m2 = torch.zeros(D, dtype=torch.bool, device=DEV)
            m2[rank[selname][:k].to(DEV)] = True
            run_g(f'{selname}{k}', armsites, gseq['arm'],
                  {TARGET: partial_row_hook(frr[TARGET], m2)})
    ...
    seq = {k2: frac[PICK_ROLE][k2] for k2 in
           ['B0', 'K0'] + [f'{s3}{k3}' for s3 in SELECTORS for k3 in KEEPS]}
    b0, k0 = seq['B0'], seq['K0']
    stake = b0 - k0
    rec = {s3: {k3: (seq[f'{s3}{k3}'] - k0) / max(stake, 1e-9) for k3 in KEEPS}
           for s3 in SELECTORS}
    pa = seq['DISC16'] >= k0 + 0.5 * stake
    pb = (seq['OUT16'] - k0) >= 0.80 * max(seq['DISC16'] - k0, 1e-9)
    pc = all(seq[f'{s3}{k3}'] - seq[f'RAND{k3}'] >= 0.10
             for s3 in ('DISC', 'OUT') for k3 in KEEPS)
    pd = (abs(k0 - S1834_MLP5_GAPFRAC) <= 0.03
          and abs(b0 - S1829_B0_GAPFRAC) <= 0.03
    ...
    for k2 in ['B0', 'K0'] + [f'{s3}{k3}' for s3 in SELECTORS for k3 in KEEPS]:
        print(f'    {k2:10s} ' + '  '.join(
            f'{e} {frac[e][k2]:7.1%}' for e in roles), flush=True)
    print(f'\n  MLP5 CHANNELS KEPT LIVE ({PICK_ROLE}, sequential; B0 {b0:.1%}, '
          f'all-of-mlp5 compiled {k0:.1%}, stake {stake:.1%}):', flush=True)
    for s3 in SELECTORS:
        print(f'    {s3:5s} ' + '  '.join(
            f'k{k3} {seq[f"{s3}{k3}"]:6.1%} ({rec[s3][k3]:5.1%} of stake)' for k3 in KEEPS),
            flush=True)
    print(f'\n  the cost is CONCENTRATED (16 DISC channels buy >=half of 61.2pp) -> {pa}  '
          f'DISC16 {seq["DISC16"]:.1%} vs the {k0 + 0.5 * stake:.1%} needed, '
          f'{rec["DISC"][16]:.1%} of stake', flush=True)
    print(f'  and it is the OUTLIER channels (OUT16 >=80% of DISC16) -> {pb}  '
          f'OUT16 {rec["OUT"][16]:.1%} vs DISC16 {rec["DISC"][16]:.1%} of stake', flush=True)
    print(f'  and SELECTION matters (both beat RAND by >=10pp at every k) -> {pc}  ' + '  '.join(
        f'k{k3} DISC{seq[f"DISC{k3}"] - seq[f"RAND{k3}"]:+.1%} '
        f'OUT{seq[f"OUT{k3}"] - seq[f"RAND{k3}"]:+.1%}' for k3 in KEEPS), flush=True)
    print(f'  K0 reproduces §1834 ({S1834_MLP5_GAPFRAC:.1%}), B0 reproduces §1829 '
          f'-> control {pd}', flush=True)

    json.dump({'run': 'mlp5_channel_concentration', 'role': PICK_ROLE, 'target': list(TARGET),
               'keeps': list(KEEPS), 'selectors': list(SELECTORS),
               'B0_gapfrac': b0, 'K0_gapfrac': k0, 'stake': stake,
               'sequential': seq, 'fraction_of_stake': rec,
               'gap_fraction': frac,
               'predictions': {'pred_a_cost_is_concentrated': bool(pa),
                               'pred_b_outlier_channels': bool(pb),
                               'pred_c_selection_matters': bool(pc),
