#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_seven_heads_reach_the_bar pred_c_seven_heads_add pred_d_four_replay pred_e_seven_below_all
"""The person line's named set against the kill bar (v593). v586-v592: on 43 natural person pairs the four 18 Sep readers write 0.412 of the myself - yourself
gap into the answer, block 9's 9.6 / 9.7 0.058 / 0.035, block 5's 5.3 0.040; every head 0.943. Review 44 s5 set the kill criterion: person stands as a NAMED-SET
circuit only if some set of <= 8 heads writes >= 0.60 into the answer. Here the seven {8.1, 13.1, 10.5, 15.1, 9.6, 9.7, 5.3} jointly (SEVEN), the four (FOUR,
replay) and all heads (ALL, replay).
PREDICTIONS (scored as written; failures preserved; priors from v586 / v589 / v592)
    pred_a_baseline_replays        the unedited manual forward replays the model's myself - yourself margins within 1e-3
    pred_b_seven_heads_reach_the_bar  SEVEN closes >= 0.60 (the kill criterion). Prior: unsure -- the singles sum to 0.545
    pred_c_seven_heads_add         SEVEN is within 0.08 of 0.412 + 0.058 + 0.035 + 0.040 = 0.545 (writes into one position add)
    pred_d_four_replay             FOUR closes 0.412 +- 0.03
    pred_e_seven_below_all         SEVEN <= ALL
...
OUT = ROOT / "circuits/followups/person_named_set_kill_bar_v593_result.json"
CANDIDATE_ID = "person.named_set_kill_bar_v593"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, KILL_BAR, SEVEN_SUM, ADD, FOUR_V586, BAND, UNIT = 1e-3, 0.60, 0.412 + 0.058 + 0.035 + 0.040, 0.08, 0.412, 0.03, 701
...
FORWARDS_MAX = 9
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_seven_heads_reach_the_bar": ">= 0.60", "pred_c_seven_heads_add": "0.545 +- 0.08", "pred_d_four_replay": "0.412 +- 0.03", "pred_e_seven_below_all": "SEVEN <= ALL"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "kill_bar": KILL_BAR, "seven_sum": SEVEN_SUM, "add": ADD, "four_v586": FOUR_V586, "band": BAND, "unit": UNIT}}
    ...
    SEVEN = {8: [1], 13: [1], 10: [5], 15: [1], 9: [6, 7], 5: [3]}
    MODES = {"FOUR": ({}, (0,)), "SEVEN": ({}, (0,)), "ALL": ({}, (0,))}; SETS_W = {"FOUR": FIVE_W, "SEVEN": SEVEN, "ALL": {l: list(range(N_HEAD)) for l in range(18)}}; forwards = 0
    def logits_margin(x):
    ...
    print("person named set", {k: round(v, 3) for k, v in mc.items()}, "singles sum", round(SEVEN_SUM, 3), "kill bar", KILL_BAR)
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_seven_heads_reach_the_bar": mc["SEVEN"] >= KILL_BAR, "pred_c_seven_heads_add": abs(mc["SEVEN"] - SEVEN_SUM) <= ADD,
                   "pred_d_four_replay": abs(mc["FOUR"] - FOUR_V586) <= BAND, "pred_e_seven_below_all": mc["SEVEN"] <= mc["ALL"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "person_named_set_kill_bar_result_v593", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
