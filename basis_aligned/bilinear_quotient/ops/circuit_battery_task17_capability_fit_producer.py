#!/usr/bin/env python3
"""Model-facing producer for the frozen task-17 FIT capability contract.
...
NAMESPACE = "circuit_battery_task17_capability_fit_v1"
...
def call_inputs(
    ...
    if call.get("logical_batch_size") != 24 \
            or call.get("padded_sequence_length") != 13:
        raise ProducerError("physical task17 call shape changed")
    ...
    if sequences.shape != (24, 13) or targets.shape != (24,) \
            or len(foils) != 24 or any(not values for values in foils):
        raise ProducerError("task17 call inputs differ from exact physical shape")
    if sequences.min() < 0 or sequences.max() >= 50_257 \
        ...
        raise ProducerError("task17 call contains token outside tokenizer support")
...
def validate_call_arrays(
    ...
    for label, array in (("answer_logit", answer), ("max_foil_logit", maximum_foil)):
        if type(array) is not np.ndarray or array.shape != (24,) \
                or array.dtype != np.dtype("float32") or not array.flags.c_contiguous \
                or not bool(np.isfinite(array).all()):
            raise ProducerError(f"{label} differs from the exact float32[24] contract")
        values.append(array)
...
def execute_call_manifest(
    ...
    if type(manifest) is not list or type(metrics) is not list \
        ...
        raise ProducerError("task17 physical or metric call census changed")
    ...
    if len(primitives) != EXAMPLE_EVALUATIONS:
        raise ProducerError("task17 did not execute all 192 row-side evaluations")
...
def native_logits(model, tokens):
    """Exact native bilin18 forward for the contract's dynamic 24x13 batches."""
    import torch
    ...
    if tuple(logits.shape) != (24, 13, 50_304) or not bool(torch.isfinite(logits).all()):
...
def validate_result_surface(result: Mapping[str, object]) -> None:
    ...
    if type(decision) is not dict or decision.get("terminal") not in ("ok", "hard_abort"):
        raise ProducerError("task17 capability decision terminal is malformed")
...
def make_result(
    *, compiled: Mapping[str, object], decision: Mapping[str, object],
    ...
    result = {
        "schema": "circuit_battery_task17_capability_fit_result_v1",
        ...
        "task17_authority_sha256": capability.TASK17_AUTHORITY_SHA256,
    ...
    if len(completed) != FORWARD_CALLS \
        ...
        raise ProducerError("task17 literal call or evidence price changed")
    if checkpoint.revision != MODEL_REVISION \
        ...
        raise ProducerError("task17 checkpoint receipt differs from frozen model")
...
def run_dryrun(captured: Mapping[str, bytes]) -> dict[str, object]:
    ...
    def passing_evaluator(call, sequences, targets, foils):
        ...
        return (
            np.full(24, 1.0, dtype="<f4"),
            np.full(24, 0.0, dtype="<f4"),
        )
    ...
    if held["terminal"] != "ok" or rejected["terminal"] != "hard_abort" \
        ...
        raise ProducerError("task17 dry-run passing or capability-fail fixture changed")
    return {
        "schema": "circuit_battery_task17_capability_fit_producer_dryrun_v1",
...
def run_science(
    ...
    *, paths: package.PackagePaths = PATHS,
    ...
    return {
        "schema": "circuit_battery_task17_capability_fit_execution_receipt_v1",
