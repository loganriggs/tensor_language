#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_instrument pred_b_singular_subject_selectivity pred_c_singular_subject_prediction
"""Matched-strength native selective intervention control; frozen derivatives."""
import os,json,time,sys,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';OUT=A/'native_all_five_source_v1_result.json'
PREDICTIONS=dict(pred_a_instrument='oldunitB anddouble effectreplay1e-4;counts',pred_b_singular_subject_selectivity='all5 singularsubject native alignedretention>=80%unitB andmodalratio<=10% everycell',pred_c_singular_subject_prediction='all5 singularsubject quadratic number10%/modal5%numberbudget everycell')
PLAN=dict(prefix=24,double_suffix=48,native_suffix=48,gradient=0,hessian=0,fits=0,predictions=PREDICTIONS)
...
def main():
    ...
    for panel,digest in [('opposite','1869db8a18d4c89c28fa81a6bb57a19497e6f8692831eb4391f5d23c84556d73'),('congruent','bfba23ab066beb14f5cd469edce137db7ec1e8ccc26bfc36b66f896426b17511')]:
        ...
        for template in dict.fromkeys(r['template'] for r in rows):
            ...
            for role,key in [('subject','subject_position'),('attractor','control_position')]:
                ...
                arms=dict(unitB=torch.tensor([0,0,1,1,1],device='cuda',dtype=torch.float64).expand_as(zero),all5=torch.ones_like(zero))
                for arm,av in arms.items():
                    ...
                    for family in dict.fromkeys(r['family'] for r in entries):
                        ...
                        if arm=='unitB':oldreplays.append(float((y-torch.tensor(oldrecords[(panel,role,family,arm)]['target'],device='cuda')).abs().max()))
    ...
    for row in records:
        if row['arm']!='all5':continue
        effect=np.array(row['target']);control=np.array(lookup[(row['panel'],row['role'],row['family'],'unitB')]['target']);budget=max(np.linalg.norm(effect[:,0]),1e-30)
        retention=float(effect[:,0]@control[:,0]/max(control[:,0]@control[:,0],1e-30));modal=float(max(np.linalg.norm(effect[:,1:],axis=0))/budget)
        comparisons.append(dict(panel=row['panel'],role=row['role'],family=row['family'],retention=retention,modal_ratio=modal,number_error=row['number_error'],modal_error=max(row['modal_error'])))
    primary=[c for c in comparisons if c['role']=='subject' and c['family'].endswith('|singular')];assert len(primary)==8
    selective=instrument and all(c['retention']>=.8 and c['modal_ratio']<=.1 for c in primary)
    predictive=instrument and all(c['number_error']<=.1 and c['modal_error']<=.05 for c in primary)
