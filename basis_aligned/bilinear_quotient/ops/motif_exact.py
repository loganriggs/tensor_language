"""HALVES TEST I (rung 237): ALL motif blocks (2-9) exact, tail r64 - where is the certificate bottleneck?

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2332: exactness at the circuit-
loaded blocks bought zero certificates - the bottleneck is spread. This splits the stack: all 8 motif
blocks' patterns exact (r64 -> full rank), tail stays r64. If certificates unlock, the spread bottleneck
is front-half; if not, it is tail-half (or interaction), and rung 238 mirrors this with the tail.
REGISTERED PREDICTIONS (arms: THIS vs r64 0.0852/7, circuit-rank 0.0771/7, r96 0.0622/9):
  (a) census <= 0.070 (more heads exact than S2332).
  (b) SOME UNLOCK: valid >= 8.
  (c) SANITY: census in [0.01, 0.12]; inert tripwires vs r64 AND circuit-rank receipts.
NULL: motif exactness buys no certificates (valid <= 7) - the bottleneck is in the tail half.
PRICE: r64 config + 72 motif-block head-patterns at full rank. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: motif-exact halves test')
    raise SystemExit(0)
import torch
...
OUT=PT+'motif_exact_results.json'
...
if __name__=='__main__':
    ...
    pb=nv>=8
    pc=0.01<=agg<=0.12
    if abs(agg-0.0852)<1e-4 or abs(agg-0.0771)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: motif-exact map inert')
    ...
    print(f"(a) census {agg:+.4f} <= 0.070: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 8; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
