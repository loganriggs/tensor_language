...
"""pred_a prior/native replay<=1e-5;compiled64write<=1e-10/effect<=1e-5.
pred_b compiled32write<=1e-5/effect<=1e-3 EACHfamily.
pred_c childcompiled partition/referencewrite<=1e-10; suffix interaction reported.
9bodybatches48rows13–19tokens7suffixarms180sec. No fit or upstreamclosureclaim.
"""
...
STEM='COMPILED_SHARED_HEAD2_NATIVE_V1'
@torch.no_grad()
def main():
    ...
    program32={k:v.float() for k,v in program.items()};child_programs=[]
    for j in (0,1):
        child=dict(program);child['children']=program['children'].clone();child['children'][1-j]=0;child_programs.append(child)
    child_replays=[]
    ...
    try:
        for length,ids in batches:
            ...
            for pos in range(length):
                ...
                compiled=execute_head(query,source,rotation,program)
                compiled32=execute_head(query.float(),source.float(),rotation.float(),program32).double()
                child_writes=[execute_head(query,source,rotation,pp) for pp in child_programs]
                for j in (0,1):
    ...
    for arm,name in ((2,'compiled64'),(3,'compiled32')):
    ...
    aa=max(replays+[prior_replay])<=1e-5 and all(c['write_relative_error']<=1e-10 and c['effect_relative_error']<=1e-5 for c in cells if c['arm']=='compiled64')
    bb=aa and all(c['write_relative_error']<=1e-5 and c['effect_relative_error']<=1e-3 for c in cells if c['arm']=='compiled32')
    partition=relative(writes[:,4]+writes[:,5],writes[:,2]);cc=aa and max(child_replays+[partition])<=1e-10
    interactions=[]
    for family in range(4):
        ids=[i for i,row in enumerate(rows) if row['family']==family]
    ...
    result.update(seconds=time.perf_counter()-tic,body_forwards=count,artifact_sha=digest(art),source_shas=binding,
                  scope='Saved666656-scalar package executed directly on native q/source/R. FP64/FP32 and two child branches compared with original factors. Native input-state generators and final suffix remain; this validates local integration, not token-to-logit extraction.')
