"""PER-TOKEN TOP-K (rung 206): each token picks its own 1152 of 4608 units - the sparse-usage test.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. User-directed lane (SPD / parameter-
decomposition style): the shared unit dictionary stays FULL; each token activates only the top-1152 units
by |activation| x ||D_i|| (a top-k selection nonlinearity - still a tensor program). Same average active
count as rung 205's global arm; same r64-exact base (+0.0852). No fitted per-circuit parameters - selection
is data-determined, so the exactness type system is untouched.

REGISTERED PREDICTIONS (arms: TOPK = this run; GLOBAL = rung 205's receipt; anchor r64 +0.0852):
  (a) USAGE IS SPARSE: census(TOPK) <= 0.15 (predicted less than half of GLOBAL's predicted >= 0.30 floor).
  (b) CERTIFICATES SURVIVE SPARSIFICATION: valid >= 5 (vs <= 4 predicted for GLOBAL).
  (c) SANITY: census in [0.01, 1.2]; inert tripwire vs +0.0852.
NULL: unit usage is dense per token - top-k lands within 0.8x of GLOBAL. PRICE: no stored-value saving
(full dictionary kept); measures usage structure, the premise for a routed decomposition. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: per-token top-k sparse usage')
    raise SystemExit(0)
import torch
...
OUT=PT+'cp_topk1152_results.json'
...
if __name__=='__main__':
    ...
    print(f"(a) census {agg:+.4f} <= 0.15: {'HELD' if pa else 'FAILED'}")
