#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_four_units_do_little_on_text pred_c_random_units_do_not pred_d_four_units_below_the_panel pred_e_hub_units_at_the_noun_replay
...
OUT = ROOT / "circuits/followups/text_opposing_units_v517_result.json"
CANDIDATE_ID = "chain.text_opposing_units_v517"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, TEXT_BAND, RANDOM_MAX, PANEL_V516, HUB_V463, BAND = 1e-3, 0.03, 0.01, 0.078, 0.133, 0.02
import random
import random
...
FORWARDS_MAX = 17
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_four_units_do_little_on_text": "|.| <= 0.03", "pred_c_random_units_do_not": "|.| <= 0.01", "pred_d_four_units_below_the_panel": "<= 0.039", "pred_e_hub_units_at_the_noun_replay": "0.133 +- 0.02"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "text_band": TEXT_BAND, "random_max": RANDOM_MAX, "panel_v516": PANEL_V516, "hub_v463": HUB_V463, "band": BAND}}
    ...
    def study(items, batch):
        ...
        return out, nat_m[:, 0]
    verb, nat_t = study(text_items, 32); text = verb; panel = {"frame": "natural pairs (pronoun slot)", "rows": len(text_items)}
    ...
    with torch.no_grad():
        ...
        fin_ = torch.tensor([len(p) - 1 for p, _, _ in text_items[:32]] * 2, device="cuda"); z = hook["z"].float()[torch.arange(64, device="cuda"), fin_]; z = 30 * torch.tanh(z / 30); ref = (z[:, THEY] - z[:, HE]).cpu()
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_four_units_do_little_on_text": abs(v["DET_A"]) <= TEXT_BAND, "pred_c_random_units_do_not": abs(v["RANDOM_A"]) <= RANDOM_MAX,
                   "pred_d_four_units_below_the_panel": abs(v["DET_A"]) <= 0.5 * PANEL_V516, "pred_e_hub_units_at_the_noun_replay": abs(v["HUB_N"] - HUB_V463) <= BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "text_opposing_units_result_v517", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
