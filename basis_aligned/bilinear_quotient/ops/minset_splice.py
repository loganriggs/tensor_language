"""MINIMAL-SET TRAJECTORY SPLICE (rung 173): does fixing a circuit's ~5 components repair IT in the frontier config?

CONVENTION (S2135): per-position dCE = CE(config with spliced components) - CE(real model) on census rows;
recovery = (member damage plain - spliced)/plain on the circuit's own members. The reconnect rung: S2253
says the motif grammar is circuit-faithful alone and dies compositionally; S2260 says circuits are
~5-component objects. Here, inside the exact rung-127 frontier build, each of five sampled circuits'
battery-top-5 components are trajectory-spliced (real-pass outputs, all positions) - and per circuit a
seeded RANDOM-5 component splice is the specificity control. Splice hooks are registered AFTER install
(config hooks), li bound as defaults (S2238 rule), own eval loop with explicit batch counter.

REGISTERED PREDICTIONS:
  (a) MINSET REPAIRS: median own-circuit recovery (minset arm) >= 0.5.
  (b) SPECIFICITY: minset recovery >= 1.3 x random-5 recovery for >= 3 of 5 circuits.
  (c) REPRO: plain-arm member damages within 15% of the rung-132 rows receipt, median.
NULL: recovery is indiscriminate (ratio ~1) - the kill is stream-wide and per-circuit repair at component
grain is impossible (the positional-identity story's hard form). PRICE: none (oracle diagnostic).
Tripwire: INSTRUMENT FAIL on inert arms. Self-reviewed."""
import json, sys, time, os
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
if os.environ.get('BQLIB_DRYRUN')=='1':
    _bq='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
    _need=['probe_gate_results.json','minimality6_results.json','frontier_rows_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: minimal-set trajectory splice')
    raise SystemExit(0)
import torch
...
OUT=PT+'minset_splice_results.json'
        ...
        for _c in _UN:
            ...
            def _mkr(c=_c):
                def h(mo,i_,o_):
                    ...
                    _CAPR[c].append(y.detach().to(torch.float16).cpu())
                return h
...
if __name__=='__main__':
    ...
    pa=stt.median([r['recovery_minset'] for r in rows])>=0.5
    ...
    res={'rows':rows,'median_minset_recovery':round(stt.median([r['recovery_minset'] for r in rows]),3),
         ...
         'pred_a_minset_repairs':bool(pa),'pred_b_specific':bool(pb),'pred_c_repro':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) median minset recovery {res['median_minset_recovery']:.3f} >= 0.5: {'HELD' if pa else 'FAILED'}")
