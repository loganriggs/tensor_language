# IS THE 77% RECOVERABLE IN PRACTICE?  -- a table built from SHORT REAL CONTEXTS.
#
# §1844 established that 77.3% of mlp5's +61.2pp is the LENGTH-1 context-free table and only 13.9pp is
# the site. But the table that recovered it -- the empirical per-token mean over the EVAL rows -- is not
# deployable: it is fitted on the distribution it is scored on.
#
# A table built from the FIT rows is. §1834's pipeline already streams `fineweb_n96_skip80` to determine
# coverage, so averaging each site's output over a token's occurrences THERE costs no new data, stores
# exactly the same 5419 x D, and is honestly held out from all three eval roles. It sits between the two
# tables §1844 compared: real contexts like the empirical mean, held out like the length-1 row.
#
# This is the difference between a diagnosis and an improvement to the compiler.
#
# ROLES. All three, all held out from the fit rows. FULL RANK. DISCOVERY ONLY.
#
...
#   pred_a THE DEPLOYABLE TABLE RECOVERS MOST OF IT: FIT_mlp5 reaches a gap fraction of at least 27.3%
#          -- half of the 47.3pp the eval-fitted mean recovered, on top of the length-1 arm's 3.6%. If
#          TRUE the 77% is recoverable in practice, the compiler can be improved by changing its table
#          rather than its structure, and §1834's headline becomes a fixable defect. If FALSE the
#          recovery needs the eval distribution and is an upper bound only -- §1844 would then be a
#          statement about what a per-token table COULD do, not about what this compiler can be made to
#          do, and the length-1 row would stand as the best deployable choice.
#   pred_b AND IT GENERALISES ACROSS ROLES: FIT_mlp5's gap fraction varies by under 10pp across
#          skip7000, skip11000 and skip1200. The table is fitted on skip80 and every role is held out,
#          so a large spread would mean it has learned the fit rows rather than the token. If FALSE the
#          fit-context table overfits and the deployable gain is smaller than any single role suggests.
#   pred_c AND mlp4's INVERSION PERSISTS: FIT_mlp4 also recovers LESS than the length-1 arm's 20.3%.
#          §1844 found the eval-empirical mean is a worse substitute than the length-1 row at mlp4 in
#          both its runs, despite being 9.92x closer to the site's output. If TRUE that inversion is a
#          property of context-averaged tables at mlp4 and not of the eval fitting. If FALSE it was an
#          artefact of fitting on the eval distribution, and only the length-1-versus-eval-mean contrast
#          shows it.
#   pred_d CONTROLS, cross-run per LESSON 42: B0 reproduces §1829's PUBLISHED 64.8%; the length-1 arms
#          reproduce §1834's PUBLISHED 3.6% (mlp5) and 20.3% (mlp4); the eval-empirical arms reproduce
#          §1844's PUBLISHED 50.9% and -1.4%; endpoints reproduce §1789's FULL-RANK top-1; the placement
#          control moves top-1 under 0.05pp. Coverage 5419 of 50257.
...
OUT = PT + 'ops/deployable_table_results.json'
...
# §1844's PUBLISHED eval-empirical sequential gap fractions, skip7000
S1844_EM_GAPFRAC = {'EM_mlp5': 0.509, 'EM_mlp4': -0.014}
...
def main():
    ...
    print(f'DEPLOYABLE TABLE | length-1 vs FIT-CONTEXT vs eval-empirical rows | '
    ...
    # the DEPLOYABLE table: the same average, taken over the FIT rows, held out from every eval role
    ...
    fr5 = {e: frac[e]['FIT_mlp5_seq'] for e in roles}
    pa = seq['FIT_mlp5'] >= 0.273
    pb = max(fr5.values()) - min(fr5.values()) < 0.10
    pc = seq['FIT_mlp4'] < seq['L1_mlp4']
    pd = (abs(b0 - S1829_B0_GAPFRAC) <= 0.03
          and all(abs(seq[k2] - v) <= 0.03 for k2, v in S1834_L1_GAPFRAC.items())
          and all(abs(seq[k2] - v) <= 0.03 for k2, v in S1844_EM_GAPFRAC.items())
    ...
    print(f'\n  the DEPLOYABLE table recovers most of it (FIT_mlp5 >=27.3%) -> {pa}  '
          f'FIT_mlp5 {seq["FIT_mlp5"]:.1%} vs L1 {seq["L1_mlp5"]:.1%} and EM {seq["EM_mlp5"]:.1%}',
          flush=True)
    print(f'  and it GENERALISES across roles (<10pp spread) -> {pb}  ' + '  '.join(
        f'{e} {fr5[e]:.1%}' for e in roles), flush=True)
    print(f'  and mlp4 INVERSION persists with the fit table -> {pc}  '
          f'FIT {seq["FIT_mlp4"]:.1%} vs L1 {seq["L1_mlp4"]:.1%} (EM {seq["EM_mlp4"]:.1%})',
          flush=True)
    print(f'  B0 §1829, length-1 arms §1834, banks differ on exactly {NCOV} rows -> control {pd}',
          flush=True)

    json.dump({'run': 'deployable_table', 'role': PICK_ROLE,
               ...
               'predictions': {'pred_a_deployable_recovers_most': bool(pa),
                               'pred_b_generalises_across_roles': bool(pb),
                               'pred_c_mlp4_inversion_persists': bool(pc),
