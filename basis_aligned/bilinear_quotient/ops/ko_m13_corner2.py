"""ADDITIVITY CONTROL v2 (m13, rung 273): the knockout on the TRUE corner config, with a live config tripwire.

CONVENTION (S2135): per-position dCE = CE(corner config with m13 mean-ablated) - CE(real model); LOWER
IS BETTER. Rung 271 was INSTRUMENT-INVALID (S2371: dead SEL knobs in the mixed
lineage). This build descends from the corner lineage itself; a tripwire kills the run unless the
unablated L2F matches the corner's 0.0601. Mixed-baseline subtracted values to reproduce: own 0.486,
collateral 0.903.
REGISTERED PREDICTIONS (subtraction uses the CORNER baseline receipt):
  (a) BASELINE-INDEPENDENT: subtracted own median in [0.24, 0.74].
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: additivity control v2 m13')
    raise SystemExit(0)
import torch
...
OUT=PT+'ko_m13_corner2_results.json'
