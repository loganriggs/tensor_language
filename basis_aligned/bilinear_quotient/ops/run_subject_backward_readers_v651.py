...
"""Width8 backward-reader basis: two fixed readouts plus six adjoint features.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_joint_chain_v651_result.json'
...
def main():
    plan=dict(prefix_calls=12,suffix_calls=16,calibration_rows=128,evaluation_rows=48,width=8,
              additional_endpoint_chain_batches=6,basis_selection='two fixed answer readers plus six orthogonal calibration backward-reader SVD vectors',controls=['dense','random_seed647'],
    ...
    reader_samples=[backward_readers(reader_blocks,c[which]['backgrounds'],c[which]['states'][-1],U,eps)
                    for c in calibration for which in ('base','frozen')]
    bases=[];singular=[]
    ...
    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,
        ...
        scope='Analytical backward-reader width8 control, same calibration/evaluation data; final projection is oracle. Data-informed residual basis, no native circuit adoption.')
