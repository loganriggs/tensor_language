...
"""Aspectual has/had DoD (v253): the MLP-8 census AT THE BANK -- the correction of v251. Scorecard rows 26-27: heads 9.1 / 9.4 read the contextual bank state at
`last`, the period noun and `the` (row.source_positions), written by attention 8 (8.1's cue copy, 29% / 17%), MLP 8 (28% / 28%), MLP 7 and MLP 6; v251 censused MLP 8 at
the cue and the final instead, where the readers do not read, and found it spread. Same exact census (`dod_units.unit_census`) on 9.1's weight-only reader direction at
the three bank positions, pooled since - by, plus the three positions summed ("bank"). Kill criterion (review 26): top-10 < 0.30 -> the temporal MLP-8 port is spread.
PREDICTIONS (scored as written; failures preserved; priors unsure)
    ...
    pred_b_bank_top10_carry_half               summed over the three bank positions, the top-10 |pooled| units carry >= 0.50 of MLP 8's pooled write on r
    pred_c_one_unit_leads_bank                 the largest unit carries >= 0.15 of the bank total (a nameable detector, as 829 / 3152 were)
...
OUT = ROOT / "circuits/followups/aspectual_dod_mlp8_unit_census_bank_v253_result.json"
CANDIDATE_ID = "aspectual_anchor.has_vs_had.dod_mlp8_unit_census_bank_v253"
LAYER, HEAD, CLOSURE_TOL, TOP10_MIN, LEAD_MIN, JACCARD_MIN = 8, (9, 1), 1e-3, 0.50, 0.15, 1 / 3
...
def main() -> None:
    ...
    OUT.write_text(json.dumps({"schema": "aspectual_dod_mlp8_unit_census_bank_result_v253", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "positions": report, "predictions": predictions, "forwards": forwards,
