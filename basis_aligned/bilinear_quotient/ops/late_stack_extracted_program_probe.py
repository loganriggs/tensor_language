#!/usr/bin/env python
"""late_stack_extracted_program_probe -- the whole late MLP stack under the extraction recipe: pool blocks (mlp11-15) own weights on 32 input PCs +
token filler (§2730), mlp16/17 exact compile on the shared 8-dim square space + rank-8 token read (§2728/§2729); vs MEAN7 and §2725. CUDA lane-1 script.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_shared_square_program_beats_minimal pred_c_extracted_late_stack_beats_the_fitted_one
#                     pred_d_pool_output_truncation_to_256_is_cheap pred_e_composition_penalty_small
...
Preregistration: polynomial_causal/LATE_STACK_EXTRACTED_PROGRAM_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_stack_extracted_program_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_stack_extracted_program_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_STACK_EXTRACTED_PROGRAM_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_pool_own_weights_input_head_probe_results.json"   # §2730
PRIOR29 = ROOT / "late_core_square_features_probe_results.json"   # §2729
OUT = ROOT / "late_stack_extracted_program_probe_results.json"
HASHES = {PREREG: "c37701825604062daaa6dc8fbec46b4f3cccc24eda146be38488b09e34a161b5", PRIOR: "63024819f826504efcc6c6b9a30491adc787d9488c205485b2ef2663af56a93d", PRIOR29: "a3461898e90b84e5e8791e19cbca2c6f47be959826e73f659d79eb07054a8744",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_stack_extracted_program_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_MEAN7 = 1.885; PRIOR_POOL32TOK = 0.319; LAM = 1e-2
POOL = [("mlp", l) for l in range(11, 16)]
KIN = 32; OUTS = [256, 64]; RSQ = 8; RB = 8
BARS = {"ce_tol": 1e-4, "repro_tol7": 0.03, "repro_tol": 0.02, "b_max": 0.26, "c_max": 0.55, "d_max": 0.03, "e_max": 0.10}
NULLS = {"b_min": 0.30, "c_min": 0.65, "d_min": 0.10, "e_min": 0.25}
...
def main():
    ...
    wb = {}   # per pool block: write mean + write PCs (for output truncation)
    for s_ in POOL:
        C = bases[s_]["C"]; evw, Uw = torch.linalg.eigh(C); wb[s_] = {"mu": bases[s_]["mu"].to(DEV).float(), "U": Uw.flip(1).to(DEV).float()}
    ...
    class OwnHead:
        def __init__(self, l, k, out_r):
            h = heads[("mlp", l)]; self.mlp = m.transformer.h[l].mlp; self.U = h["U"][:, :k]; self.mx = h["mx"]; self.A = h["Afull"]
            self.out = None if out_r is None else wb[("mlp", l)]["U"][:, :out_r]; self.mu = wb[("mlp", l)]["mu"]
        def hook(self, idx):
            self.idx = idx
        def __call__(self, w, x):
            ...
            if self.out is not None:
                wr = self.mu + ((wr - self.mu) @ self.out) @ self.out.T
            return wr
    # mlp16/17: exact compile, quadratic core on the SHARED top-RSQ square space, rank-RB token read, no offset
    def compile_block(l):
        mlp = m.transformer.h[l].mlp; fill = fills[("mlp", l)]
    ...
    class Prog:
        def __init__(self, l):
            cb = comp[l]; self.A = Pi @ cb["A"] @ Pi; self.b0, self.B, self.me_, self.mu = cb["b0"], cb["B"], cb["me"], cb["mu"]; self.pmu = self.mu @ P
        def hook(self, idx):
            self.idx = idx
        def __call__(self, w, x):
            B_, T, _ = w.shape
    ...
    pool_part = lambda out_r: {("mlp", l): OwnHead(l, KIN, out_r) for l in range(11, 16)}
    prog_part = lambda: {("mlp", 16): Prog(16), ("mlp", 17): Prog(17)}
    arms = {"MEAN7": ce_of(m, ev, {s: meanpatch(bases[s]["mu"]) for s in LATE7}) - ce0}
    arms["POOL_OWN32_TOK"] = run([pool_part(None)])
    for r in OUTS:
        arms[f"POOL_OWN32_TOK_OUT{r}"] = run([pool_part(r)])
    arms["PROG16_17"] = run([prog_part()])
    arms["COMBINED"] = run([pool_part(None), prog_part()])
    arms["COMBINED_OUT256"] = run([pool_part(256), prog_part()])
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    pi = arms["COMBINED"] - arms["POOL_OWN32_TOK"] - arms["PROG16_17"]
    rec7 = {k: 1 - v / arms["MEAN7"] for k, v in arms.items()}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["MEAN7"] - PRIOR_MEAN7) <= BARS["repro_tol7"] and abs(arms["POOL_OWN32_TOK"] - PRIOR_POOL32TOK) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_shared_square_program_beats_minimal': bool(arms["PROG16_17"] <= BARS["b_max"]),
        'pred_c_extracted_late_stack_beats_the_fitted_one': bool(arms["COMBINED"] <= BARS["c_max"]),
        'pred_d_pool_output_truncation_to_256_is_cheap': bool(arms["POOL_OWN32_TOK_OUT256"] - arms["POOL_OWN32_TOK"] <= BARS["d_max"]),
        'pred_e_composition_penalty_small': bool(pi <= BARS["e_max"]),
    }
    nulls = {"b_null_prog_ge_.30": bool(arms["PROG16_17"] >= NULLS["b_min"]), "c_null_combined_ge_.65": bool(arms["COMBINED"] >= NULLS["c_min"]),
             "d_null_out256_ge_.10": bool(arms["POOL_OWN32_TOK_OUT256"] - arms["POOL_OWN32_TOK"] >= NULLS["d_min"]), "e_null_pi_ge_.25": bool(pi >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "k_in_pool": KIN, "out_ranks": OUTS, "square_space_rank": RSQ, "token_read_rank": RB,
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "mean7": arms["MEAN7"], "prior_mean7": PRIOR_MEAN7, "pool_own32_tok": arms["POOL_OWN32_TOK"], "prior_pool_own32_tok": PRIOR_POOL32TOK},
           "ce_added": arms, "recovery_of_mean7": rec7, "composition_penalty": pi,
           "program_numbers": {"prog16_17_per_block": KM * (RSQ * (RSQ + 1) // 2) + RB * (D + KM * KM) + KM * KM, "shared_core_basis": D * KM,
                               "pool_per_block_compiled_k32_full_out": D * KIN * KIN, "pool_per_block_compiled_k32_out256": 256 * KIN * KIN + D * 256, "pool_filler_ridge_per_block": D * D,
                               "note": "pool blocks evaluated with their own weights here (not compiled); the compiled counts are the exact-algebra sizes of §2727 applied at k=32"},
    ...
    if smoke:
        print(json.dumps({k: out[k] for k in ("preds", "nulls", "instrument", "ce_added", "composition_penalty", "price")}, indent=1)); return
    from receipt import dump
    dump(out, OUT)
    print(json.dumps({"preds": preds, "nulls": nulls, "ce_added": arms, "pi": pi}, indent=1))
