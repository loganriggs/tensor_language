#!/usr/bin/env python3
# BQGATE:128bodyforwards;32prefixes<=179tokens;180seconds;FineWeb preservation.
"""pred_a native/wholeheadzero CE andmarginreplay<=1e-4relative, tokenread<=1e-5.
pred_b allsourceKO meanabsCE<=.02,maxabsCE<=.1 each16rowfamily.
pred_c nativepositive>=14/16,meanmargin>0,wholeheadzeromeanabsCE>.1eachfamily.
Null: unconditional interaction removal damages known unrelated head8.2 service.
Price128forwards180sec; nativecontext required; reusedFineWebcontrols.
"""
...
STEM='FIRST_TOKEN_REMOVAL_NEWLINE_V1';FOLDER=P/'extracted_circuits/regional_even_key_producers_8_2_9_8_v1';TOKEN=P/'extracted_circuits/first_token_value_path_v1'
@torch.no_grad()
def main():
 files=json.loads((P/(STEM+'_BINDING.json')).read_text())['files'];assert all(digest(k)==v for k,v in files.items());rows=[r for r in json.loads((P/'SCALAR_PRODUCERS_NEWLINE_NATURAL_V2_ROWS.json').read_text())['rows'] if r['pool']=='fineweb'];assert len(rows)==32
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('128forwards32FineWebrows;native,allsourceKO,halfKO,head8.2zero');return
 ...
 assert count==128
 rel=lambda a,b:float((a-b).norm()/b.norm().clamp_min(1e-30));ref=torch.load(P/'SCALAR_PRODUCERS_NEWLINE_NATURAL_V2_ARTIFACT.pt',weights_only=True);replay=max(rel(ce[:,[0,3]],ref['ce'][16:][:,[0,4]]),rel(margins[:,[0,3]],ref['margins'][16:][:,[0,4]]));records=[]
 for k in range(2):
 ...
 A=replay<=1e-4 and max(errors)<=1e-5;B=A and all(r['meanabs_CE_change']<=.02 and r['maxabs_CE_change']<=.1 for r in records);C=A and all(r['native_positive']>=14 and r['native_mean_margin']>0 and r['wholehead_meanabs_CE_change']>.1 for r in records);result={'pred_a':A,'pred_b':B,'pred_c':C,'native_positive_control_replay_error':replay,'max_token_reader_error':max(errors),'records':records,'body_forwards':count,'seconds':time.perf_counter()-tic,'scope':'All-source crossfirst removal on32reusedcapableFineWebnewlinecontrols; no cueannotation. Head8.2zero positivecontrol. PositiveCEchange isdamage; preservation usesabsolutechanges. NotbroadcorpusOOD orglobalcomponentadoption.'}
