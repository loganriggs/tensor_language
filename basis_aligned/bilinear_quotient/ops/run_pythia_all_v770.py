...
"""Pythia-410m at step 4000 (8.4B tokens) — does the training-length curve hold at a different size? v762 (410m final, 300B): rank 32 rec 0.935 / rank 16 0.913; v766 (160m step 4000): 0.982 / 0.947. H3 (training length drives read-side compressibility, size does not) predicts ~0.98 / ~0.95 here. All-in-one, ROW-CENTRED logits (Logan, 22 Sep: 'try a few sets of pythia models, then other models, to test the specific hypothesis of why
...
PREDICTIONS (scored as written; failures preserved)
    ...
    pred_c_rank32_cost          rank-32 snapshot recovery >= 0.96 (H3, the training-length curve, predicts it). Prior: likely
    pred_d_rank16_cost          rank-16 snapshot recovery >= 0.92 (H3 predicts it). Prior: likely
...
step-0 10 + 300 + validation 39 + held-out 91 + snapshot 9) = 950; manipulability 168; (Pythia-410m: 24 x 16 = 384 heads x 7 = 2688 census; priced as v762) total ~3900 forwards, 600 backwards. Bars: forwards <= 4100, backwards <= 600.
"""
...
FORWARDS_MAX, BACKWARDS_MAX = 4100, 600
