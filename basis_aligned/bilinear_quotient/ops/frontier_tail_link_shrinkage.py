"""IS THE RIDGE SOLUTION THE END-TO-END OPTIMUM? A ONE-PARAMETER TEST. (Claude, LANE 1)

SS2890 established, from the fitting window itself, that it is not. The tail dictionaries' link maps are obtained by ridge regression
minimising PER-LAYER reconstruction of that layer's attention output; the frontier is scored by END-TO-END cross-entropy. A rank-1
truncation of the ridge solution beats it by **-0.0062 nats IN SAMPLE** and -0.0294 out of sample, and the fit-window and fresh-window
rank curves agree at Pearson **.962** -- so the effect is not overfitting, it is an objective mismatch.

Rank is a clumsy knob for that claim: it changes the model class as well as the magnitude. **Scale is the clean one.** Multiply every
fitted LW[k] by a scalar s and sweep s in {0, .25, .5, .75, .9, 1, 1.1, 1.25}. If the ridge solution overshoots for the end-to-end
objective then some s < 1 must beat s = 1, along a direction that changes nothing about the model class. If nothing below 1 improves,
the mismatch does not show along the scale direction and SS2890's account is bounded.

**SS2881 supplies a free anchor**: it measured `LW := {}` at **+0.1740 nats**, which is exactly this sweep's s = 0. pred_c requires the
sweep to land on it, making the arm a cross-rung reproduction rather than a fresh unanchored curve.

Nine arms differ only in what `evalM` sees, so they share ONE fitted stack (`ops/frontier_evalarms.py`, validated at baseline deviation
exactly 0.0 in SS2888/SS2889): **one pipeline run instead of nine.** The original maps are snapshotted after the refit loop and restored
before each arm.

SIGN CONVENTION (SS2135): frontier L2 is CE ADDED ABOVE THE REAL MODEL, so **LOWER IS BETTER** (SS312: +2.6735 beating +2.84/+2.93).
A cost is L2(arm) - L2(baseline), **POSITIVE = WORSE**. SS2128/SS2129/SS2133/SS2134 RETRACTED; SS2125 STANDS.
Both L2_F (fresh) and L2_C (the FITTING window) are reported per arm, because SS2890 showed the in-sample curve is the decisive one.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_some_shrunk_map_beats_the_ridge_fit
#                     pred_c_scale_zero_reproduces_S2881
#                     pred_d_the_arms_are_connected
#                     pred_e_the_curve_is_unimodal

Preregistration: polynomial_causal/FRONTIER_TAIL_LINK_SHRINKAGE_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run, nine evaluations (baseline + eight scales)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_tail_link_shrinkage_results.json'   # NOT the parent's receipt (SS2125 cites that file)
    ...
    def _apply_scale(sc):
        ...
        for _li in range(10, 18):
            ...
            if sc is None:
                ...
                _new = {c: (_M.float() * float(sc)) for c, _M in _LW0[_li].items()}
            S[f'a{_li}L'] = (_k, _l, _CV, _new, _Wp)
    def _cfg(spec):
        _apply_scale(spec.get('scale'))
...
PREREG = PT + '../polynomial_causal/FRONTIER_TAIL_LINK_SHRINKAGE_PREREGISTRATION.md'
PREREG_SHA = "69d33c7d33abad88b3dd000c195717c67b6f1683ef337994eb5be4460768bc40"
...
S2881 = PT + 'frontier_tail_refit_split_results.json'
S2881_SHA = "cd1a8028a8086706ba82a907436ae1acb74113d70c357d01bebaba21c81ecf95"
SCALES = [0.0, 0.25, 0.5, 0.75, 0.9, 1.0, 1.1, 1.25]
BARS = {"reproduce": 0.05, "anchor": 0.02, "improves": 0.0, "connected": 0.005, "turns": 1}
NULLS = {"improves_ge": 0.0, "anchor_ge": 0.05}
...
if __name__=='__main__':
    ...
    if _sha(S2881) != S2881_SHA:
        raise RuntimeError(f'frozen hash mismatch: {S2881}')
    prev = json.load(open(S2881))['summary']
    anchor0 = prev['cost_dropping_link_maps_nats']      # SS2881: +0.1740 for LW := {} , i.e. scale 0
    SEL['mode']='norm'

    ARMS[:] = [('baseline', {})] + [(f's{int(round(s*100)):03d}', {'scale': s}) for s in SCALES]
    ...
    key = lambda s: f's{int(round(s*100)):03d}'
    cF = {s: round(out[key(s)]['L2_F'] - bF, 4) for s in SCALES}
    cC = {s: round(out[key(s)]['L2_C'] - bC, 4) for s in SCALES}
    best_s = min(SCALES, key=lambda s: cF[s])
    below = [s for s in SCALES if s < 1.0 and cF[s] < 0]
    anchor_dev = abs(cF[0.0] - anchor0)
    seq = [cF[s] for s in SCALES]
    turns = sum(1 for i in range(1, len(seq)-1)
                if (seq[i]-seq[i-1] > 0) != (seq[i+1]-seq[i] > 0))

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = len(below) >= 1
    pc = anchor_dev <= BARS['anchor']
    pd = abs(cF[0.0]) >= BARS['connected']
    pe = turns <= BARS['turns']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_some_shrunk_map_beats_the_ridge_fit':bool(pb),
           'pred_c_scale_zero_reproduces_S2881':bool(pc),
           'pred_d_the_arms_are_connected':bool(pd),
           'pred_e_the_curve_is_unimodal':bool(pe)}
    nulls={'b_null_the_ridge_fit_is_optimal_along_this_direction':bool(len(below)==0),
           'c_null_this_rung_disagrees_with_S2881':bool(anchor_dev>=NULLS['anchor_ge'])}
    res={'rung':'frontier_tail_link_shrinkage','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,'scales':SCALES,'arms':out,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(bF,4),'L2_C_baseline':round(bC,4),
                    'cost_fresh_by_scale':{str(s):cF[s] for s in SCALES},
                    'cost_fit_by_scale':{str(s):cC[s] for s in SCALES},
                    'best_scale':best_s,'best_cost_fresh':cF[best_s],
                    'scales_below_one_that_improve':below,
                    'S2881_scale_zero_anchor':round(anchor0,4),
                    'scale_zero_measured':cF[0.0],'anchor_deviation':round(anchor_dev,4),
                    'turns_in_the_curve':turns,
                    'S2890_rank1_fresh':-0.0294,'S2890_rank1_fit':-0.0062,
                    'pipeline_runs_saved_vs_per_arm':len(SCALES)},
         ...
         'hashes':{PREREG:PREREG_SHA,S2881:S2881_SHA},'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {bF:.4f} vs 2.6735 (tol .05): {'HELD' if pa else 'FAILED'}")
    print("    fresh: " + "  ".join(f"s={s}:{cF[s]:+.4f}" for s in SCALES))
    print("    fit:   " + "  ".join(f"s={s}:{cC[s]:+.4f}" for s in SCALES))
    print(f"(b) scales below 1 that improve: {below}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) scale-0 {cF[0.0]:+.4f} vs SS2881 {anchor0:+.4f}, dev {anchor_dev:.4f}: {'HELD' if pc else 'FAILED'}")
    print(f'wrote {OUT} ({res["price"]["gpu_seconds"]:.0f}s, 1 pipeline run instead of {len(SCALES)+1})')
