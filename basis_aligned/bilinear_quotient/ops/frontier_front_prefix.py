"""FRONT PREFIX DECOMPOSITION + THE tailE CONTROL (rung 104).

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows; LOWER IS BETTER.
CODE-READ FINDING BEHIND THIS RUNG: cfgF = build_arm(False) returns stack+['tailE'] - the span-projected class
dictionaries on ALL EIGHT tail MLPs (m10-m17) were installed in every certificate arm, so rung 102's
"tables+middles only" and rung 103's "tables-only" labels were WRONG as attributions. This rung is the
independent physical control the process rule requires before any correction of S2196's family table.

ARMS (all on the 1000 census rows, all attention real, K=K69=4608):
  ctrl_no_tailE : cfgF minus 'tailE' (six front replacements + middles-at-4608) - the TRUE tables-only config
  tailE_only    : ONLY the tail-MLP span dictionaries installed, everything else real
  prefix_a0 .. prefix_m3E : cumulative front prefixes ['a0'],['a0','m0E'],...,all six (no middles, no tailE)

REGISTERED PREDICTIONS (arms named):
  (a) TABLES CARRY THE BULK (guards S2196): agg(ctrl_no_tailE) >= 1.4. If FAILED, S2196's attribution table
      requires a published correction (tailE was a material part of the +1.924).
  (b) CONCENTRATED: max front marginal (prefix_j - prefix_{j-1}, with prefix_0 = 0) >= 0.5 x agg(front-full).
  (c) SANITY: agg(front-full) in [0.5, 2.2].
NULL: the damage is spread across the six front replacements AND tailE was a material chunk. PRICE: none
(attribution). Tripwire: INSTRUMENT FAIL if the prefix aggregates are all identical (pstdev < 0.01).
Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: rung-103 artifact present; front prefix decomposition + tailE control')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_front_prefix_results.json'
...
if __name__=='__main__':
    ...
    ANCH=json.load(open(PT+'frontier_certificate5_results.json'))['aggregate_mean_dce']
    SEL['mode']='norm'; SEL['K']=4608; SEL['K69']=4608; SEL['K69MAP']={}
    SEL['skipset']=tuple(range(10,18)); SEL['motif_off']=tuple(range(2,10))
    SEL['front_prefix']=True; SEL['ext_rows']=CROWS
    print('ARMS: ctrl (cfgF minus tailE), tailE-only, six front prefixes - census rows',flush=True)
    main()
    A=SEL['front_result']
    out={}
    for k,cev in A.items():
        ...
        print(f"  {k:16s} agg {agg:+.4f}  valid {nv}/{len(CINFO)}",flush=True)
    FRONT=['a0','m0E','a1v','m1','m2E','m3E']
    pref=[out['prefix_'+n]['agg'] for n in FRONT]
    marg=[round(pref[0],4)]+[round(pref[j]-pref[j-1],4) for j in range(1,6)]
    for n,mg in zip(FRONT,marg): print(f'  marginal {n}: {mg:+.4f}')
    import statistics as stt
    if stt.pstdev(pref)<0.01: raise SystemExit('INSTRUMENT FAIL: prefix arms identical')
    ctrl=out['ctrl_no_tailE']['agg']; tonly=out['tailE_only']['agg']; full=pref[-1]
    pa=ctrl>=1.4
    pb=max(marg)>=0.5*full
    pc=0.5<=full<=2.2
    res={'arms':out,'front_order':FRONT,'marginals':marg,'anchor_rung103':ANCH,
         'convention':'per-position dCE = CE(config) - CE(real model) on census rows; lower is better',
         'pred_a_tables_carry_bulk':bool(pa),'pred_b_concentrated':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'ctrl(no tailE) {ctrl:+.4f} | tailE-only {tonly:+.4f} | front-full {full:+.4f} (rung-103 anchor {ANCH:+.4f})')
    print(f"(a) ctrl {ctrl:+.4f} >= 1.4 (tables carry the bulk; else S2196 needs correction): {'HELD' if pa else 'FAILED'}")
    print(f"(b) max marginal {max(marg):+.4f} >= 0.5 x full ({0.5*full:+.4f}): {'HELD' if pb else 'FAILED'}")
    print(f"(c) front-full {full:+.4f} in [0.5, 2.2]: {'HELD' if pc else 'FAILED'}")
