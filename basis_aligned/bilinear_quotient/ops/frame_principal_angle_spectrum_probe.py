#!/usr/bin/env python
"""frame_principal_angle_spectrum_probe -- construction-free measure of the read-frame drift: principal angles between consecutive
own 768-cores of the 36 sublayers and between each core and the settled frame U_8 (§2754). One covariance pass + one instrument arm.
CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_drift_is_broad pred_c_no_pair_is_nearly_shared pred_d_settled_sites_are_close_to_U8
#                     pred_e_drift_toward_U8_is_monotone

SIGN CONVENTION (§2135): the instrument CE number is CE ADDED ABOVE THE REAL MODEL on held-out docs 0-63 -- LOWER IS BETTER. Descriptive;
nothing installs into the §312 frontier; bases are data covariances (§2118 stays closed).
Preregistration: polynomial_causal/FRAME_PRINCIPAL_ANGLE_SPECTRUM_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/frame_principal_angle_spectrum_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("frame_principal_angle_spectrum_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "FRAME_PRINCIPAL_ANGLE_SPECTRUM_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "early_frame_drift_rank_probe_results.json"   # §2755
OUT = ROOT / "frame_principal_angle_spectrum_probe_results.json"
HASHES = {PREREG: "75fd28abbec8717eba7d5bfa8db6d6abd3bff79140f7ecfea5e1a4f661249287", PRIOR: "9f4823153389055aa5d5fc0f7b39865de62923a839625b0c6a272c5fef4702ca",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "frame_principal_angle_spectrum_probe"
...
K = 768; EARLY = list(range(0, 11)); LATE = list(range(11, 18)); SPLIT = 8; THRESH = [10, 30, 60]
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_min": 100, "c_min": 200, "d_max": 60, "e_max": -0.7}
NULLS = {"b_max": 30, "c_max": 80, "d_min": 150, "e_min": -0.2}
...
def main():
    ...
    import math
    seq = blocks(range(NL)); eseq = blocks(EARLY); set8 = blocks(range(SPLIT, NL))
    U_8 = core_of(set8)[:, :K]
    def angles(A, B):
        s = torch.linalg.svdvals((A[:, :K].T @ B[:, :K]).double()).clamp(-1, 1)
        deg = torch.rad2deg(torch.acos(s))
        return {f"n_gt_{t}": int((deg > t).sum()) for t in THRESH} | {"max_deg": float(deg.max()), "mean_deg": float(deg.mean())}
    consec = {f"{seq[i-1][0]}{seq[i-1][1]}->{seq[i][0]}{seq[i][1]}": angles(heads[seq[i - 1]]["U"], heads[seq[i]]["U"]) for i in range(1, len(seq))}
    vs8 = {f"{s_[0]}{s_[1]}": angles(heads[s_]["U"], U_8) for s_ in seq}
    early_pairs = [f"{eseq[i-1][0]}{eseq[i-1][1]}->{eseq[i][0]}{eseq[i][1]}" for i in range(1, len(eseq))]
    b_med = float(torch.tensor([consec[p]["n_gt_30"] for p in early_pairs], dtype=torch.float).median())
    c_min = min(consec[p]["n_gt_10"] for p in early_pairs)
    d_med = float(torch.tensor([vs8[f"{s_[0]}{s_[1]}"]["n_gt_30"] for s_ in set8], dtype=torch.float).median())
    e_rho = R.spearman(torch.arange(len(eseq), dtype=torch.float), torch.tensor([vs8[f"{s_[0]}{s_[1]}"]["n_gt_30"] for s_ in eseq], dtype=torch.float))
    arms = {"EARLY22_OWN_768": run({s_: head(s_, K) for s_ in ESITES})}
    log(stage="stats", b_med=b_med, c_min=c_min, d_med=d_med, e_rho=e_rho, own=round(arms["EARLY22_OWN_768"], 4))
    ...
    preds = {
        ...
        'pred_b_drift_is_broad': bool(b_med >= BARS["b_min"]),
        'pred_c_no_pair_is_nearly_shared': bool(c_min >= BARS["c_min"]),
        'pred_d_settled_sites_are_close_to_U8': bool(d_med <= BARS["d_max"]),
        'pred_e_drift_toward_U8_is_monotone': bool(e_rho <= BARS["e_max"]),
    }
    nulls = {"b_null_le_30": bool(b_med <= NULLS["b_max"]), "c_null_some_pair_le_80": bool(c_min <= NULLS["c_max"]),
             "d_null_ge_150": bool(d_med >= NULLS["d_min"]), "e_null_ge_-.2": bool(e_rho >= NULLS["e_min"])}
    gaps = {"early_consecutive_n_gt_30_median": b_med, "early_consecutive_n_gt_10_min": c_min, "settled_vs_U8_n_gt_30_median": d_med, "early_vs_U8_n_gt_30_spearman": e_rho}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           "sign_convention": "instrument: CE ADDED above the real model on held-out docs 0-63 (FRESH split; LOWER IS BETTER)",
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "k": K, "split": SPLIT, "thresholds_deg": THRESH,
           ...
           "ce_added": arms, "gaps": gaps, "consecutive": consec, "vs_settled_frame": vs8,
