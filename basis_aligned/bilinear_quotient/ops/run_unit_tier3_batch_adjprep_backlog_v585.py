...
"""v585: work the AUTHORED BACKLOG instead of authoring more cells.
...
REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here). THRESHOLDS SCALED FROM THE TWO
verb_preposition BATCHES, not inherited: v575 gave four rows 1 of 8, row 2 two of 8, rows 3 and 4 eight of 8, row 5
six of 8; v581 gave four rows 3 of 8, row 2 three of 8, rows 3 and 4 SEVEN of 8, row 5 six of 8. Nine cells here:
  pred_a_four_rows    at least k_four = 1 of the nine clears ALL FOUR tier rows. THIS IS THE amid TEST: the
                      verb_preposition yield of 4 of 16 came entirely from cells answering with one rare readout
                      token, and NO cell in this family uses it. If the token is what selects four-row passers, this
                      comes out FALSE at zero of nine.                                                prior 45%
  pred_b_row2_still_binding row 2 passes AT MOST row2_max = 4 of the nine, so row 2 stays the binding constraint it
                      was in both verb_preposition batches (2 of 8, then 3 of 8).                     prior 70%
  pred_c_row3         row 3 passes at least k_row3 = 7 of the nine, as it did at 8 of 8 then 7 of 8.  prior 75%
  pred_d_row4         row 4 passes at least k_row4 = 7 of the nine, on the same precedent.            prior 70%
  pred_e_row5_amended at least k_row5 = 5 of the nine clear row 5 under the amendment.                prior 70%
...
HOW IT READS. a FALSE at zero of nine: no cell in a family without the rare readout clears all four rows, which is
the strongest support the readout-token reading can get from a rung that does not manipulate the token -- and it
would mean the backlog's value is concentrated in a handful of rare-readout cells rather than spread across 175
unprocessed ones. a TRUE with two or more passing: the amid clustering in verb_preposition was coincidence or
family-specific, the one-in-four yield generalises, and the backlog is worth working family by family. a TRUE with
exactly one: too weak to separate the readings, and I will say so rather than pick the flattering one. b, c, d
holding alongside either: the row profile is a property of the protocol rather than of verb_preposition, which makes
the row-2 constraint worth attacking directly.
SCOPE. Nine behaviours, one family; units and tier rows only, no counting. The token reading is tested only by ABSENCE here -- no cell's readout is manipulated -- so a null is suggestive, not decisive.
Smoke: V585_SMOKE=<out.json> (CPU, V585_SMOKE_ROWS=4).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_adjprep_backlog_v585_result.json"
...
NAMES = {"adjective_preposition_from_about": "adjective_preposition_from_about",
         "adjective_preposition_in_of": "adjective_preposition_in_of",
         "adjective_preposition_in_to": "adjective_preposition_in_to",
         "adjective_preposition_in_with": "adjective_preposition_in_with",
         "adjective_preposition_on_beyond": "adjective_preposition_on_beyond",
         "adjective_preposition_over_for": "adjective_preposition_over_for",
         "adjective_preposition_to_against": "adjective_preposition_to_against",
...
BARS = {"ext_min": EXT_MIN, "rem_min": REM_MIN, "c_ub_max": C_UB_MAX, "row5_share_band": [0.5, 1.4], "k_four": 1, "row2_max": 4, "k_row3": 7, "k_row4": 7, "k_row5": 5, "instr_tol": 0.02}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 200000, 6400000


def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_adjprep_backlog_v585", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V585_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V585_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V585_SMOKE_NAMES", "adjective_preposition_in_to,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_adjprep_backlog_v585", "candidate_id": "corpus.unit_tier3_batch_adjprep_backlog_v585", "bars": BARS,
