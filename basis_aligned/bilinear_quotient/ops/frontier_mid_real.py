"""CORRECTED MIDDLES CONTROL (rung 133): offline real-module targets - pruning compensation vs exactness.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2225: rung 131 self-targeted (capture
hook downstream of the installed cp hook) and scored nothing. Here the target is computed OFFLINE per the
S2199 rule: inputs captured in the true config frame, hooks removed, then Y = real mlp(Xd) with ALL 4608
units - least-squares pruning compensation absorbed into the kept units' Down. Sign genuinely two-sided:
compensation usually helps aggregate; S2223 says refits at real-weight sites hurt.

REGISTERED PREDICTIONS (anchors: rung-127 receipt +1.9474 census / +1.8765 L2_F / +0.3256 increment):
  (a) COMPENSATION WINS: census agg <= 1.92 (>= 0.03 improvement).
  (b) L2_F fresh <= 1.86.
  (c) SANITY: increment in [0.25, 0.45] AND census >= 1.4.
NULL: exactness beats compensation (census >= 1.95). PRICE: none (same Down shapes + one D-bias per site).
Tripwire: INSTRUMENT FAIL if L2CF missing OR the rung-131 self-target signature (|L2F-1.8765|<1e-4 and
|census-1.9474|<1e-4). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_mid_ctrl_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: corrected middles control (offline real targets)')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_mid_real_results.json'
...
def main():
    ...
    if SEL.get('mid_traj'):
        _YRm={li:[] for li in MIDL}
        hs=[]
        for _li in MIDL:
            def _mkC(li=_li):
                def h(mo,i_,o_):
                    _YRm[li].append(o_.detach().reshape(-1,D).float())
                return h
            hs.append(m.transformer.h[_li].mlp.register_forward_hook(_mkC()))
        for _i in range(CA,CB,4):
            _bb=FW[_i:_i+4,:257].to(DEV)
            cur['idx']=_bb[:,:-1].contiguous(); cur['mode']='oracle'
            m(cur['idx'],_bb[:,1:].contiguous())
        ...
        for _li in MIDL:
            ...
            def _capD(mo,i_,o_):
                _Xs.append(i_[0].detach().reshape(-1,D).float())
            hs.append(m.transformer.h[_li].mlp.register_forward_hook(_capD))
            for _i in range(CA,CB,4):
                _bb=FW[_i:_i+4,:257].to(DEV)
                cur['idx']=_bb[:,:-1].contiguous(); cur['mode']='oracle'
                cur['lab']=clsA[_i-CA:_i-CA+4].reshape(-1)
            ...
            for h in hs: h.remove()
            _Xd=torch.cat(_Xs)
            _mlpR=m.transformer.h[_li].mlp
            _dt9=next(_mlpR.parameters()).dtype
            with torch.no_grad():
                _Y=torch.cat([_mlpR(_Xd[_j9:_j9+8192].to(_dt9)).float()
                              for _j9 in range(0,_Xd.shape[0],8192)])
            _t,_l2i,_Lk,_Rk,_Dk,_db=S[f'c{_li}']
            _G=(_Xd@_Lk.T)*(_Xd@_Rk.T)
            _Gb=_G.mean(0); _Yb=_Y.mean(0); _Gc=_G-_Gb
            _lam=1e-2*len(_G)
            _Wn=torch.linalg.solve(_Gc.T@_Gc+_lam*torch.eye(_G.shape[1],device=DEV),
                                   _Gc.T@(_Y-_Yb))
            _dbn=(_Yb-_Gb@_Wn)
            S[f'c{_li}']=('cp',_li,_Lk,_Rk,_Wn.T.contiguous(),_dbn.contiguous())
            _YRm[_li]=None
            print(f'  mid_traj: refit c{_li} Down (trajectory target)',flush=True)
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['mid_traj']=True
    print('ARM: rung-127 build + middles Down-refit to OFFLINE real targets (rung 133)',flush=True)
    main()
    ...
    agg=float((cev.cpu()-CBASE).mean())
    inc=L2F-L1F
    pa=agg<=1.92
    pb=L2F<=1.86
    pc=(0.25<=inc<=0.45) and agg>=1.4
    if abs(L2F-1.8765)<1e-4 and abs(agg-1.9474)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: rung-131 self-target signature')
    ...
    print(f"L2 fresh {L2F:+.4f} (was +1.8765); census {agg:+.4f} (was +1.9474); increment {inc:+.4f}")
    print(f"(a) census {agg:+.4f} <= 1.92: {'HELD' if pa else 'FAILED'}")
    print(f"(b) L2F {L2F:+.4f} <= 1.86: {'HELD' if pb else 'FAILED'}")
    print(f"(c) increment in [0.25, 0.45], census >= 1.4: {'HELD' if pc else 'FAILED'}")
