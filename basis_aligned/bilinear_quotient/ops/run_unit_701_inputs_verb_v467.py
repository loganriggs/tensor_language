#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_closure pred_b_both_inputs_move_at_the_verb pred_c_attention_carries_the_inputs_at_the_verb pred_d_block_11_attention_in_the_top_three pred_e_product_term_matters
"""What feeds unit 701 at a DISTANT verb (v467). v458 (at the noun): 701 is a product of two number-bearing inputs written by MLPs 15 / 16 / 10 / 12. v465:
at a distant verb 701 reads the number copied there by the verb readers (11.3 / 7.8 / 13.1 / 9.7) and the copier. Here the same exact product and writer
split of 701's activation change at the ANSWER position on the 79 distant rows: which side moves, and do the attention blocks (the copies) rather than the
MLPs write its inputs there?
PREDICTIONS (scored as written; failures preserved; priors from v458 / v465)
    pred_a_closure                              the three product terms sum to dh, and the writer terms to dL and dR, within relative 1e-3 per pair
    pred_b_both_inputs_move_at_the_verb         |dL| >= 0.30 |L_s| and |dR| >= 0.30 |R_s| pooled. Prior: unsure
    pred_c_attention_carries_the_inputs_at_the_verb  the attention blocks together carry >= 0.40 of dL or of dR (the copied number enters through attention here, not through MLPs as at the noun). Prior: unsure
    pred_d_block_11_attention_in_the_top_three  attn11 (head 11.3's block) is among the three largest |writer terms| of dL or of dR
    pred_e_product_term_matters                 |dL dR| >= 0.20 of |dh| pooled (the detector multiplies, as at the noun)
...
OUT = ROOT / "circuits/followups/unit_701_inputs_verb_v467_result.json"
CANDIDATE_ID = "chain.unit_701_inputs_verb_v467"
N_HEAD = 9
...
CLOSURE_TOL, SIDE_MIN, ATTN_MIN, PROD_MIN, UNIT = 1e-3, 0.30, 0.40, 0.20, 701
READERS = ((17, 0),)   # the residual entering block 17's MLP
FORWARDS_MAX = 4
PREDICTIONS = {"pred_a_closure": "<= 1e-3", "pred_b_both_inputs_move_at_the_verb": ">= 0.30 x 2", "pred_c_attention_carries_the_inputs_at_the_verb": ">= 0.40", "pred_d_block_11_attention_in_the_top_three": "top 3", "pred_e_product_term_matters": ">= 0.20"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"closure_tol": CLOSURE_TOL, "side_min": SIDE_MIN, "attn_min": ATTN_MIN, "prod_min": PROD_MIN, "unit": UNIT}}
    ...
    predictions = {"pred_a_closure": closure <= CLOSURE_TOL, "pred_b_both_inputs_move_at_the_verb": rep["rel_change"]["L"] >= SIDE_MIN and rep["rel_change"]["R"] >= SIDE_MIN,
                   "pred_c_attention_carries_the_inputs_at_the_verb": max(rep["L_attn_share"], rep["R_attn_share"]) >= ATTN_MIN, "pred_d_block_11_attention_in_the_top_three": "attn11" in list(rep["L_writers_top5"])[:3] or "attn11" in list(rep["R_writers_top5"])[:3],
                   "pred_e_product_term_matters": rep["prod_abs_share"] >= PROD_MIN}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "unit_701_inputs_verb_result_v467", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
