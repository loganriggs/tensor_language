...
"""Person family DoD (v108): matched-count random four-head-set null for the run_person_object_control_dod_battery_v105 set on its fresh rows (SIMPLE). Body: v72's; 16 random
...
within 0.46 +/- 0.05 (replay; the scored key keeps v72's literal name).
...
v72.OUT = v72.ROOT / "circuits/followups/person_object_control_dod_random_set_null_v108_result.json"
v72.CANDIDATE_ID = line.CANDIDATE_ID.rsplit(".", 1)[0] + ".dod_random_set_null_v108"
v72.EXCLUDED = set(line.HEADS)
v72.POOL = [(l, h) for l in range(18) for h in range(9) if (l, h) not in v72.EXCLUDED]
v72.SETS = [tuple(sorted(v72.random.Random(2026_09_18_108 + s).sample(v72.POOL, 4))) for s in range(16)]
