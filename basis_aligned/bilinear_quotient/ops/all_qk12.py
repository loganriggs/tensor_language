"""ALL-QK-12 (rung 186; math review 1907): the Hadamard-rank law, above-knee arm.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The law: the squared-attention pattern
is a Hadamard product of two Gram forms, and rank(A o B) <= rank(A) x rank(B) - so per-factor rank r gives
effective pattern rank ~r^2, and the critical rank is r* ~ sqrt(d_head) = sqrt(128) ~ 11.3. This explains
the sharp 8 -> 16 threshold (64 < 128 <= 256). Prediction: r = 12 (144 >= 128) still crosses.

REGISTERED PREDICTIONS (anchors: rung-185 census +1.2673 at r16; rung-183 +2.02 at r8):
  (a) CROSSES: census <= 1.55.
  (b) PLATEAU: census <= 1.2673 + 0.20 (above r*, performance plateaus).
  (c) SANITY: census >= 0.5; inert tripwire.
NULL: r12 breaks (>= 1.9) - r* is higher than sqrt(128) and the law's constant is wrong. PRICE: 148 x 4 x
12 x 1280 = 9.1M values (25% cheaper than r16 if it plateaus). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: all-QK-12 (Hadamard law above-knee)')
    raise SystemExit(0)
import torch
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['qk_r']=12; SEL['qk_tail']=True
    ...
    pb=agg<=1.2673+0.20
    pc=agg>=0.5
    if abs(agg-1.2673)<1e-3:
        raise SystemExit('INSTRUMENT FAIL: rank change inert')
    ...
    print(f"(a) census {agg:+.4f} <= 1.55: {'HELD' if pa else 'FAILED'}")
    print(f"(b) census {agg:+.4f} <= 1.4673: {'HELD' if pb else 'FAILED'}")
