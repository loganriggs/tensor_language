#!/usr/bin/env python3
# BQGATE:40prefixes;120suffixes;240seconds.
...
40prefixes;120suffixes;240seconds. Existing FineWeb/Pile corpus panels, unit parent removal,
the frozen complete-even-weight-optimizedtwo-of-four64-node candidate. No data fitting; all50304modeled outputs.
"""
...
@torch.no_grad()
def main():
    binding=json.loads((P/'STRUCTURED_PARENT_CORPUS_V1_BINDING.json').read_text())
    ...
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
        print('40prefixes;120suffixes;240seconds');return
    out=P/'STRUCTURED_PARENT_CORPUS_V1_RESULT.json';assert not out.exists()
    ...
    item=torch.load(P/'STRUCTURED_PARENT_FIT_V1_PROGRAM.pt',weights_only=True)['2of4']
    _,_,q=unpack(item)
    candidates['structured2of4']=dict(native,key_basis=[native['key_basis'][0],q.cuda()])
