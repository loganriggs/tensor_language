...
"""v655: is the registered-objective result possessive-specific? Eight verb_preposition mappings, same fit.
...
Smoke: V655_SMOKE=<out.json> (CPU, V655_SMOKE_ROWS=4, V655_SMOKE_NAMES=<cell>).
"""
...
OUT = ROOT / "circuits/followups/unit_uncounted_registered_objective2_v655_result.json"
...
NAMES = {"adjective_preposition_in_of": "adjective_preposition_in_of",
         "noun_preposition_at_beyond": "noun_preposition_at_beyond",
         "reciprocal_lenmatched": "reciprocal_lenmatched",
         "subcategorization": "subcategorization",
         "suppletive_comparative_better_worse": "suppletive_comparative_better_worse",
         "to_gerund_selection": "to_gerund_selection",
         "transitivity_lifted_said": "transitivity_lifted_said",
         "additive_scope_lenmatched": "additive_scope_lenmatched"}
ANCHOR = "additive_scope_lenmatched"
SIBLINGS = ['adjective_preposition_in_of', 'noun_preposition_at_beyond', 'reciprocal_lenmatched', 'subcategorization', 'suppletive_comparative_better_worse', 'to_gerund_selection', 'transitivity_lifted_said']
# v651 measured additive_scope_lenmatched under this exact arrangement: canonical +0.0035, weekly +0.0113,
# four rows clear, both-control FAIL. It rides along as a reproduction anchor.
V655_ANCHOR = {"canon_ub": 0.0035, "weekly_ub": 0.0113}
...
def _plan():
    return {"candidate_id": "corpus.unit_uncounted_registered_objective2_v655", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V655_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V655_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V655_SMOKE_NAMES", "verb_preposition_against_for,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_uncounted_registered_objective2_v655", "candidate_id": "corpus.unit_uncounted_registered_objective2_v655", "bars": BARS,
