...
@torch.no_grad()
def main():
 files=json.loads((P/'UPSTREAM_MAP_LIVE_V1_BINDING.json').read_text())['files'];assert all(sha256(Path(k).read_bytes()).hexdigest()==v for k,v in files.items())
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('48prefixes;432localresponses;120seconds');return
 assert not (P/'UPSTREAM_MAP_LIVE_V1_RESULT.json').exists();signal.alarm(120);torch.set_num_threads(2);tic=time.perf_counter()
 pr=torch.load(P/'MLP9_CROSSFIRST_RESPONSE_V1_PROGRAM.pt',weights_only=True);j=pr['mixed_map'].double();w=pr['direction'].double();v=w/w.norm();fixed=(j@v)[:,None]*v[None,:];u,s,vh=torch.linalg.svd(j-fixed,full_matrices=False)
 fits={rank:(fixed+(u[:,:rank-1]*s[:rank-1])@vh[:rank-1]).cuda() for rank in [32,128,256]};j=j.cuda();w=w.cuda();pr={k:v.cuda() for k,v in pr.items()}
 ...
 result={'pred_a':max(writer_errors)<=1e-10 and identity<=1e-10,'pred_b':bool(passing),'pred_c':any(all(g['max_context_error']<=.05 for g in groups if g['rank']==r) for r in passing),'writer_errors':writer_errors,'error_identity_max':identity,'groups':groups,'cells':cells,'seconds':time.perf_counter()-tic,'scope':'Constrained weights-only J9 factors on48live contexts and9signed branch responses; reference exact response algebra with native baseline. Local input-domain validation, not suffix/behavioral prediction or independent fresh OOD.'}
 (P/'UPSTREAM_MAP_LIVE_V1_RESULT.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps({k:v for k,v in result.items() if k!='cells'},indent=2))
