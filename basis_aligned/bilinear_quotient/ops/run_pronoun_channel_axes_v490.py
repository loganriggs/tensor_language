#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_five_writes_carry_the_they_she_margin pred_c_rest_small_on_they_she pred_d_five_writes_move_he_she_little pred_e_they_he_replays
"""The pronoun channel's output axes from the answer side (v490). v438 (by values at the noun): the five readers' swap closed the they - he and they - she
gaps alike and moved the he - she gap a sixth as much. Here by their WRITES into the answer (the channel end): the five heads' joint write swap scored on
the they - she gap and on the he - she change relative to they - he; the other 32 heads of their blocks on they - she; 122 natural pairs.
PREDICTIONS (scored as written; failures preserved; priors from v438 / v485)
    pred_a_baseline_replays                     the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_five_writes_carry_the_they_she_margin  the five heads' joint write swap closes >= 0.75 of the they - she gap
    pred_c_rest_small_on_they_she               the other 32 heads close <= 0.06 of the they - she gap
    pred_d_five_writes_move_he_she_little       the mean |change| of the he - she margin under the five-head swap is <= 0.20 x that of the they - he margin
    pred_e_they_he_replays                      the five-head swap closes 0.803 +- 0.03 of the they - he gap
PRICE (registered maximum): 4 batches x 3 passes = 12 forwards; 0 backwards; 0 fits. Bar <= 13.
"""
...
OUT = ROOT / "circuits/followups/pronoun_channel_axes_v490_result.json"
CANDIDATE_ID = "chain.pronoun_channel_axes_v490"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, SHE_MIN, REST_MAX, GENDER_MAX, FIVE_V485, BAND, UNIT = 1e-3, 0.75, 0.06, 0.20, 0.803, 0.03, 701
...
FORWARDS_MAX = 13
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_five_writes_carry_the_they_she_margin": ">= 0.75", "pred_c_rest_small_on_they_she": "<= 0.06", "pred_d_five_writes_move_he_she_little": "<= 0.20 x", "pred_e_they_he_replays": "0.803 +- 0.03"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "she_min": SHE_MIN, "rest_max": REST_MAX, "gender_max": GENDER_MAX, "five_v485": FIVE_V485, "band": BAND, "unit": UNIT}}
    ...
    MODES = {"FIVE_W": ({}, (0,)), "REST_W": ({}, (0,))}; SETS_W = {"FIVE_W": FIVE_W, "REST_W": REST_W}; forwards = 0
    def logits_margin(x):
    ...
    def study(items, batch):
        ...
        sgap = nat_m[0::2, 1] - nat_m[1::2, 1]; she_closed = {m: float(((nat_m[0::2, 1] - e[0::2, 1]) + (e[1::2, 1] - nat_m[1::2, 1])).sum() / (2 * sgap.sum())) for m, e in edm.items()}
        gender_ratio = {m: float((e[:, 2] - nat_m[:, 2]).abs().mean() / (e[:, 0] - nat_m[:, 0]).abs().mean()) for m, e in edm.items()}
        return {"pairs": len(items), "unit": UNIT, "contrast_mean": float(gap.mean()), "sign_constancy": sign, "moved": moved, "margin_closed": margin_closed, "they_she_closed": she_closed, "gender_over_number": gender_ratio, "native_margins": nat_m[:, 0].tolist()}, nat_m[:, 0]
    ...
    sc, gr = verb["they_she_closed"], verb["gender_over_number"]
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_five_writes_carry_the_they_she_margin": sc["FIVE_W"] >= SHE_MIN, "pred_c_rest_small_on_they_she": abs(sc["REST_W"]) <= REST_MAX,
                   "pred_d_five_writes_move_he_she_little": gr["FIVE_W"] <= GENDER_MAX, "pred_e_they_he_replays": abs(mc["FIVE_W"] - FIVE_V485) <= BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pronoun_channel_axes_result_v490", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
