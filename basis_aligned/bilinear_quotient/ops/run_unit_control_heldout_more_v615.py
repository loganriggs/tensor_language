...
"""v615: row 4 tests the control it was FITTED to spare. Measure the same quantity on a held-out control.
...
TEN MORE CELLS, again FIVE row-4 passers and FIVE failers so agreement can break in either direction. None
overlaps the ten in v603 or the five in v611, and every one is spec-authored with a readout outside the avoided
lane. This rung is INDEPENDENT of the v613 control swap running alongside it: v613 asks whether the effect follows
the objective, this asks how large and how consistent the effect is across more cells. The 15-cell figures it
extends are: verdicts agreeing 5 of 10 in v603, none of five surviving in v611, a median held-out-to-canonical
ratio of 4.30, the held-out control passing row 4 on five of fifteen and returning a negative upper bound on four.
...
Smoke: V615_SMOKE=<out.json> (CPU, V615_SMOKE_ROWS=4, V615_SMOKE_NAMES=<cell>).
"""
...
OUT = ROOT / "circuits/followups/unit_control_heldout_more_v615_result.json"
...
NAMES = {"benefactive_preposition_send": "benefactive_preposition_send",
         ...
         "determiner_number_barrels": "determiner_number_barrels",
         ...
         "comparative_superlative_two_three": "comparative_superlative_two_three"}
# row 4 verdicts under the CANONICAL control, from the v591-v607 lift receipts
V2_ROW4 = {
           "benefactive_preposition_send": True,
           "binding": True,
           "case_we_us": True,
           "comparative_equative_as_than": True,
           "determiner_number_barrels": True,
           "benefactive_preposition_give": False,
           "another_other_number": False,
           "container_surface_into_onto": False,
           "countability_much_many": False,
           "comparative_superlative_two_three": False,
           }
NEW = tuple(NAMES)
INSTR = ()
...
def _plan():
    return {"candidate_id": "corpus.unit_control_heldout_more_v615", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V615_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V615_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V615_SMOKE_NAMES", "verb_preposition_against_for,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_control_heldout_more_v615", "candidate_id": "corpus.unit_control_heldout_more_v615", "bars": BARS,
