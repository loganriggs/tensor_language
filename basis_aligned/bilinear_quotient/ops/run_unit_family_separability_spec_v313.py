...
"""v313: verb_particle separability for v311's single pass — the cheapest countable rung on the board, and deliberately so.

WHY THIS FAMILY. Separability on verb_preposition is HALTED: v289 prepared 72 family members and lost 6 to CUDA OOM,
v295 prepared 49 and lost 1, v297 prepared 55 and lost 4, and the memory goes on preparing every family member, which
IS the registered control set. The adjective family still completes (v305 ran 29 prepared with zero errors) but gains
2-5 members per batch and has roughly two batches of runway. The verb_particle family has FOUR members after this rung
prepares it — up_down (woke/calmed), out_down (found/settled), the new out_up (blurted/brightened), and up_on
(piped/rambled) as a three-row control after its v311 row-4 miss at C 0.0220. Four prepared is an eighth of the size
that first showed strain, so this rung is both cheap and safe, and it can actually COUNT a circuit, which the seven
verb_preposition passes sitting in the pending pool cannot.
WHAT IS NEW: verb_particle_out_up (blurted -> out, brightened -> up), 11 units, held 0.86, C 0.0050 at v311.
v311's other two cells do not enter as counted members: up_on missed ROW 4 so it joins the control set (row-4 misses
do, row-2 misses do not), and out_away missed ROW 2 at held 0.79 so it joins nothing.
WHAT THE FAMILY ALREADY SHOWED. At v250 up_down and out_down came out SEPARABLE from each other (sib 0.005 and 0.012)
despite sharing FOUR of five top units and both reading out a directional particle — the hardest unit-overlap case in
the corpus at the time, and one of the four independent lines showing unit overlap is not evidence of shared
computation. out_up shares its ` up` readout with up_down and its ` out` readout with out_down, so it is a
two-shared-readout case, which v250 already answered for the preposition class (at_to, separable at 0.006).
CUE FRESHNESS: out_up's cues (blurted, brightened) were script-verified against every kinds=() tuple in ops/ before
v311 was committed — that check caught five collisions in the first draft AND caught my sed fix silently failing, so
the verification is on the generated modules rather than the generator. A fusion here would therefore have no
cue-overlap explanation, and the corpus's fusions are currently six for six cue-sharing after v309 withdrew the one
apparent exception.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
    pred_a_new_separable   verb_particle_out_up is separable (k_new = 1 of 1). Prior is high because the family's only
                           previous test came out separable at the harder configuration.                    prior 80%
    pred_b_counted_kept    the re-checked counted members remain separable at the k_kept bar mkfamily computes from the
                           v250 receipt. With a four-member family this is up_down and out_down, whose v250 margins
                           were 0.005 and 0.012 — far from the 0.05 bar, and the family gains only two controls here,
                           so a loss would NOT have the control-set-growth explanation that v309 established for
                           adjective_preposition_for_toward.                                                prior 85%
    pred_c_row4_kept       fam-arm own-C UB975 <= 0.01 on at least k_d of the evaluated members.            prior 60%
    pred_d_instrument      every evaluated member's own-arm extraction is within 0.03 of its parent battery's cdas
                           extraction_held. Fails automatically if any member errors; at four prepared, an OOM here
                           would mean the wall is not about family size at all and the whole memory account needs
                           rewriting.                                                                        prior 95%
COUNTING. A separable out_up is +1 on the 128 the count tool reports. A fused out_up joins whichever counted incumbent
it leaks into (+0) and would be the first non-cue fusion to survive a control-set ablation, which it would then get.
BARS = {"cross_max": 0.05, "keep": 0.8, "ext_floor": 0.5, "c_ub_max": 0.01, "k_d": 2, "k_new": 1, "k_kept": 1, "instr_tol": 0.03}
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v313_result.json"
...
FAMILIES = {
    "verb_particle": ["verb_particle_up_down", "verb_particle_out_down", "verb_particle_out_up", "verb_particle_up_on"],
}
EVAL = ("verb_particle_out_up", "verb_particle_up_down", "verb_particle_out_down", )
COUNTED = ("verb_particle_up_down", "verb_particle_out_down", )
...
def _parent_cdas(n):
    ...
    for path in (V311, V257, V255, V253, V251, V249, V247, V245, V243, V241, V239, V237, V235, V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v313", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V313_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V313_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v313", "candidate_id": "corpus.unit_family_separability_spec_v313",
