"""METRIC-WEIGHTED FRONT v1b -- WHICH KNOB carried §2104's 0.125 nat: the residual bases or the unit selections?
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['stream_error_profile_results.json','metric_front_refit_results.json']
    ...
    print(f"DRYRUN OK: S2086 present (block 6 {_p['6']}); five arms (plain / metric / bases-only / units-only / random-metric)")
    raise SystemExit(0)
import json, sys, time, torch
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
import os
...
OUT=PT+'metric_front_refit_split_results.json'
...
def main():
    ...
    def gramian(TOKS,site):
        ...
        return G/n
    SITE_OF={'m0':1,'m2':3,'m3':4,'c4':5,'c5':6}
    ...
    def build_arm(mode):
        ...
        tag={'plain':'E','metric':'M','metric-bases':'B','metric-units':'U','random-metric':'R'}[mode]
        act=['a0']
    ...
    res={}
    for mode in ('plain','metric','metric-bases','metric-units','random-metric'):
        ts=time.time()
        cfg=build_arm(mode)
        ...
        E6=st[6]-real[6]
        obs_energy=float(((E6@P6)**2).sum()); tot_energy=float((E6**2).sum())
        res[mode]={'ce':round(ce,5),'gap':round(ce-base,5),'profile':[round(p,4) for p in prof],
                   'block6_error_energy_observable':obs_energy,'block6_error_energy_total':tot_energy,
                   'observable_share':round(obs_energy/max(tot_energy,1e-9),4)}
        print(f'{mode:14s}: gap {ce-base:+.4f} | b1 {prof[1]:.3f} b5 {prof[5]:.3f} b6 {prof[6]:.3f} b7 {prof[7]:.3f} b17 {prof[17]:.3f} '
              f'| b6 obs-energy {obs_energy:.3e} ({obs_energy/max(tot_energy,1e-9):.3f} of total) | {time.time()-ts:.0f}s',flush=True)
        del st; torch.cuda.empty_cache()
    E=res['plain']
    g=lambda k:E['gap']-res[k]['gap']
    joint,gb,gu,gr=g('metric'),g('metric-bases'),g('metric-units'),g('random-metric')
    pa=gu>=0.5*0.1253
    pb=abs((gb+gu)-joint)<=0.3*max(joint,1e-9)
    pc=abs(gr-0.0168)<=0.02
    pd=abs(E['profile'][6]-1.7415)<=0.10
    out={'base_ce':round(base,5),'arms':res,'gains':{'metric':round(joint,4),'metric-bases':round(gb,4),
         'metric-units':round(gu,4),'random-metric':round(gr,4)},'S2104_joint_gain':0.1253,'S2104_random_gain':0.0168,
         'eval_r90_block6':r90e,'pred_a_units_carry_it':bool(pa),'pred_b_additive':bool(pb),
         'pred_c_control_reproduces':bool(pc),'pred_d_reproduces_S2086':bool(pd),'self_reviewed':True,
         'runtime_s':round(time.time()-t0,1)}
    json.dump(out,open(OUT,'w'),indent=1)
    print(f"(d) cfgE block-6 rel-MSE {E['profile'][6]:.4f} vs 1.7415: {'HELD' if pd else 'FAILED'}")
    print(f"(a) units-only gain {gu:+.4f} >= 0.5 x 0.1253: {'HELD' if pa else 'FAILED'}   (bases-only {gb:+.4f}, joint {joint:+.4f})")
    print(f"(b) additive |({gb:+.4f})+({gu:+.4f}) - {joint:+.4f}| <= 30%: {'HELD' if pb else 'FAILED'}")
    print(f"(c) random-metric {gr:+.4f} vs S2104 +0.0168 (tol 0.02): {'HELD' if pc else 'FAILED'}")
