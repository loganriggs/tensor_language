#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_head_10_5_is_the_exit pred_c_one_head_carries_most pred_d_singles_add_to_the_block pred_e_no_head_is_negative
"""Block 10's exit by head (v432). v431: values at the noun for block 10 close 0.128 of the margin gap — a fourth high exit the reader set (9.6 / 12.4 /
15.1) did not include. Head 10.5 is a known reader on the person line and a VC2 writer (v400). Here each head of block 10 on its own (its value at the noun,
swapped within the pair), natural pairs only.
PREDICTIONS (scored as written; failures preserved; priors from v400 / v431)
    pred_a_baseline_replays      the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_head_10_5_is_the_exit head 10.5 has the largest single-head effect in block 10. Prior: unsure
    pred_c_one_head_carries_most the largest single head carries >= 0.60 of block 10's 0.128
    pred_d_singles_add_to_the_block  the nine single-head effects sum to within 0.05 of 0.128
...
OUT = ROOT / "circuits/followups/block10_heads_v432_result.json"
CANDIDATE_ID = "both_ends.block10_heads_v432"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1))
REPLAY_TOL, TOP_SHARE_MIN, ADD_TOL, BLOCK10_V431, NEG_MIN = 1e-3, 0.60, 0.05, 0.128, -0.02
FORWARDS_MAX = 41
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_head_10_5_is_the_exit": "10.5 first", "pred_c_one_head_carries_most": ">= 0.6 x 0.128", "pred_d_singles_add_to_the_block": "within 0.05 of 0.128", "pred_e_no_head_is_negative": "> -0.03 x 9"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "top_share_min": TOP_SHARE_MIN, "add_tol": ADD_TOL, "block10_v431": BLOCK10_V431, "neg_min": NEG_MIN}}
    ...
    c = text["closed"]
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_head_10_5_is_the_exit": max(c, key=c.get) == "h5", "pred_c_one_head_carries_most": max(c.values()) >= TOP_SHARE_MIN * BLOCK10_V431,
                   "pred_d_singles_add_to_the_block": abs(text["sum_singles"] - BLOCK10_V431) <= ADD_TOL, "pred_e_no_head_is_negative": all(v > NEG_MIN for v in c.values())}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "block10_heads_result_v432", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
