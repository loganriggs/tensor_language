#!/usr/bin/env python
# BQLANE: cpu
"""pairwise_fisher_subset_price_probe_v2 -- RERUN of the §2708 probe with ONE change: the certificate set list is de-duplicated
(v1 counted the pair (mlp16, mlp17) twice because it is also nested set A1, doubling that certificate and inflating J(A) by .1210 on
every superset; pred_a(ii) caught it). Same preregistration, bars, seeds, split. Addendum: PAIRWISE_FISHER_SUBSET_PRICE_PROBE_V2_ADDENDUM.md

v1 docstring: the second-order joint Fisher certificate is exactly pairwise, so 14 single certificates
...
OUT = ROOT / "pairwise_fisher_subset_price_probe_v2_results.json"
ADDENDUM = R.POLY / "PAIRWISE_FISHER_SUBSET_PRICE_PROBE_V2_ADDENDUM.md"
PRIOR_V1 = ROOT / "pairwise_fisher_subset_price_probe_results.json"   # §2708 as-run receipt (preserved, not overwritten)
HASHES = {PREREG: "36a777cbd7863a45f232bab297a97bc6f02542693577d8370f315e4059243a71",
          ...
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1", PRIOR_V1: "2d0dc207427e17395f3e3a93735f7d0444dd33e5e9cb66bf5d80c6d0559b0038",
          ADDENDUM: "1cc4377ec7197efd111621af5e7ea5170bccff80116cef6216028af916f27054"}
RUNG = "pairwise_fisher_subset_price_probe_v2"
...
def main():
    ...
    joint_sets = list(dict.fromkeys(pairs + [tuple(A) for A in named.values()]))   # v2: DEDUP -- v1 double-counted (mlp16, mlp17), which is both a pair and A1 (§2708)
