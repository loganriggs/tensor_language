#!/usr/bin/env python3
# BQGATE:412bodyforwards;12captureprefixes;180seconds;no fitting.
...
pred_c factored consumer<=.85expanded16 EACHprefix.
pred_d combined source/attention bytes<=independent EACHprefix.
Null: native suffix amplifies local arithmetic error or prices erase saving.
Price412bodyforwards,12captureprefixes,8recipients25strengths,180seconds.
...
from denominator_factored_attention10_v1 import compile_program as compile_attention,execute as execute_attention,MAPS
from inherited_source_positions_v1 import paired_masks
STEM='DENOMINATOR_FACTORED_NATIVE_V1'
...
def main():
    ...
    paired_masks(rows[:8]);assert len(rows)==12 and len(STRENGTHS)==25
    ...
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
        ...
        assert json.loads((P/'COUPLED_ATTENTION10_LAYOUT_V2_CONTROL.json').read_text())['max_layout_difference']==0
        ...
        factored=json.loads((P/'DENOMINATOR_FACTORED_ATTENTION10_V1_RESULT.json').read_text())
        assert factored['maxima']['write_error']<=1e-10
        print('412bodyforwards;12captureprefixes;4recipients25coupledstrengths native/compiled');return
    ...
    try:
        ...
        for i in RECIPIENTS:
            ...
            consumer=payload(attention_program);expanded_consumer=consumer+len(row['ids'])*(4*5*1152*8+(16*16-12*12)*8)
            prices.append(dict(row=i,pool=row['pool'],tokens=len(row['ids']),independent_bytes=sizes[0],combined_bytes=sizes[1],ratio=sizes[1]/sizes[0],factored_consumer_bytes=consumer,expanded_consumer_bytes=expanded_consumer,consumer_ratio=consumer/expanded_consumer))
            if not example.exists():torch.save(dict(source={k:v.cpu() for k,v in program.items()},attention={k:v.cpu() for k,v in attention_program.items()},x0=r['x0'].cpu(),lambdas=block10.lambdas.detach().cpu().clone()),example)
    ...
    result={'pred_a':all(r['state_error']<=1e-5 and r['post10_error']<=1e-5 and r['score_error']<=1e-5 for r in records),
            ...
            'pred_c':all(p['consumer_ratio']<=.85 for p in prices),
            'pred_d':all(p['ratio']<=1 for p in prices),
            ...
            'scope':'Combined coupled MLP9 and complete attention10 programs prepared once per pristine context; native suffix tested over200 signed cases on short and long prefixes, using12vector attention ports. Mixed strength tied to a*b. Context/source/MLPweights remain; no static model compression or independent role identification.'}
