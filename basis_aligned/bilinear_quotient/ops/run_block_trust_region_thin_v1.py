...
BQGATE:0forwards0seq. Same full-U lambda.01 overlapping16x16x4 family,600s.
"""
...
from block_trust_region_v3 import fit
...
def main():
    binding=json.loads((P/'BLOCK_TRUST_REGION_THIN_V1_BINDING.json').read_text())
    assert all(digest(f)==h for f,h in binding.items())
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
        print(json.dumps(dict(gpu_accessed=False,body_forwards=0,corpus_access=False,seconds=600,groups=16,rank=16,outputs=4,penalty=.01)));return
    out=P/'BLOCK_TRUST_REGION_THIN_V1_RESULT.json';assert not out.exists();signal.alarm(900)
    ...
    prior=json.loads((P/'BLOCK_TRUST_REGION_V1_RESULT.json').read_text())
    ...
    point,report=fit(objective,bank,packed,4,seconds=600,tolerance=1e-7)
    ...
    cache=Path('/dev/shm/bilin18_block_trust_region_thin_v1.pt');assert not cache.exists()
    ...
    result=dict(predictions={'pred_a_instrument':valid,'pred_b_converged':valid and accepted,
        ...
        wall_seconds=time.perf_counter()-start,inherited_fit_seconds=prior['inherited_fit_seconds']+prior['final']['seconds'],
        body_forwards=0,corpus_access=False,binding=binding,threadpools=threadpool_info(),
        scope='Same trust-region algorithm and Hessian, thin Stiefel contraction and2BLASthreads; latest native checkpoint, longer600sbudget. Same full-U lambda.01 objective and overlapping block family, all original convergence conditions plus stricter absolute norm. No global/circuit claim.')
    with out.open('x') as f:json.dump(result,f,indent=2);f.write('\n')
    print(json.dumps({k:v for k,v in result.items() if k not in ['final','binding']},indent=2),flush=True);assert valid

if __name__=='__main__':
    with threadpool_limits(limits=2,user_api='blas'):main()
