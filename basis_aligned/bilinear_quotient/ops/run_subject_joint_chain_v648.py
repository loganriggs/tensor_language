...
"""Registered width8 response-space test; managed queue only.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_joint_chain_v648_result.json'
...
def main():
    plan=dict(prefix_calls=12,suffix_calls=16,calibration_rows=128,evaluation_rows=48,width=8,
              basis_selection='calibration SVD, CPU float64',controls=['dense','random_seed647'],
    ...
    def capture(entries,with_truth):
        ...
        return dict(base=base,frozen=frozen,direction=direction,conditional=conditional,truth=truth,
                    families=[r['family'] for r in entries])
    calrows=[]
    ...
    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,
        ...
        scope='Data-informed basis, exact joint weight contractions, opened position evaluation; no complete extraction/selectivity/reuse claim.')
