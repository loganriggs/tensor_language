...
"""Pythia-1b FINAL (300B tokens; 16 x 8 x 2048, head dim 128, rotary on 32 dims) — size ladder extended 6x above 160m at fixed token count. Size was flat 160m -> 410m (v760 0.932 / v762 0.935 at rank 32; 0.876 / 0.913 at rank 16). H3 (training length, not size, sets read-side compressibility) predicts the 1b model stays LOW: rank 32 < 0.95, rank 16 < 0.92; if compressibility grew with size this would move up. All-in-one, ROW-CENTRED logits (Logan, 22 Sep: 'try a few sets of pythia models, then other models, to test the specific hypothesis of why
...
REPLAY_TOL, E4_MIN, REC32, REC16, RATIO = 0.002, 0.90, 0.95, 0.92, (0.5, 2.0)
PREDICTIONS = {"pred_a_instrument_replays": "+-0.002", "pred_b_kernel_energy_top4": ">= 0.90", "pred_c_rank32_cost": "recovery < 0.95", "pred_d_rank16_cost": "recovery < 0.92", "pred_e_rank32_values_preserved_scale": "median ratio in [0.5, 2]"}
