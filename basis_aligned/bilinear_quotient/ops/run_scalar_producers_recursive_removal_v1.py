...
def main():
 ...
 result=dict(checks=replay,regional_cells=rcells,newline_cells=ncells,regional_arms=['native','physical8','physical9','physicaljoint','zero8','zero9','zerojoint'],newline_arms=['native','physical8','physical9','physicaljoint','mean8'],body_forwards=body_count,seconds=time.perf_counter()-tic,scope='Unmerged frozen source components removed at their physical native producer output; subsequent states recompute. Reused regional/FineWeb rows, no new OOD or factor fit.',source_shas=binding)
 result.update({'pred_a':A,'pred_b':A and all(c['passed'] for c in rcells),'pred_c':A and all(c['native_capability'] and c['positive_control'] and c['preservation'] for c in ncells)})
