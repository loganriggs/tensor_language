#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_pronoun_reader_blocks_lead pred_c_no_second_hop_copier_for_pronouns pred_d_early_blocks_small pred_e_singles_add
"""The attention channel into the PRONOUN answer position, by block (v481). v476 (verb): the writes into the answer position are led by blocks 11 and 5,
with 5.3 a second-hop copier from the post-noun rebuild. Mirror for the pronoun readout: each block's attention write into the pronoun slot swapped singly
(122 natural pairs, they - he margin), plus all blocks jointly. Does the pronoun readout also have an early second-hop copier, or only the late readers?
PREDICTIONS (scored as written; failures preserved; priors from v431 / v476)
    pred_a_baseline_replays              the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_pronoun_reader_blocks_lead    blocks 9, 10 and 12 are the three largest single-block effects (the five readers' blocks). Prior: likely
    pred_c_no_second_hop_copier_for_pronouns  blocks 4-6 together close <= 0.06 (no early relay into the pronoun slot). Prior: unsure
...
PRICE (registered maximum): 4 batches x 20 passes = 80 forwards; 0 backwards; 0 fits. Bar <= 81.
"""
...
OUT = ROOT / "circuits/followups/attn_into_pronoun_blocks_v481_result.json"
CANDIDATE_ID = "chain.attn_into_pronoun_blocks_v481"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, MID_MAX, EARLY_MAX, ADD_TOL, UNIT = 1e-3, 0.06, 0.08, 0.25, 701
import random
...
FORWARDS_MAX = 81
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_pronoun_reader_blocks_lead": "{9, 10, 12}", "pred_c_no_second_hop_copier_for_pronouns": "<= 0.06", "pred_d_early_blocks_small": "<= 0.08", "pred_e_singles_add": "within 0.25 of the joint"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "mid_max": MID_MAX, "early_max": EARLY_MAX, "add_tol": ADD_TOL, "unit": UNIT}}
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_pronoun_reader_blocks_lead": set(sorted(singles, key=singles.get, reverse=True)[:3]) == {"a9", "a10", "a12"}, "pred_c_no_second_hop_copier_for_pronouns": mc["a4"] + mc["a5"] + mc["a6"] <= MID_MAX,
    ...
    OUT.write_text(json.dumps({"schema": "attn_into_pronoun_blocks_result_v481", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
