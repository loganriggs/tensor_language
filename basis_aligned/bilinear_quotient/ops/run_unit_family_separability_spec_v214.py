#!/usr/bin/env python3
# BQGATE: five frozen predictions; families, EVAL set, units (v201 + v211 + v213 receipts), recipe and bars fixed before the run.
"""v214: within-family separability for v213's passes -- the count test for did_has_negation, modal_perfect_form, noun_complementizer.

v213 put the sixth eight spec-authored behaviours through the four-row battery; its three four-row passes each have a corpus sibling
(did_has_negation and modal_perfect_form are both bare-vs-participle readouts, `did/has not lift/lifted`, `can/has surely
carry/carried`, sharing question_bare_form's family and most of their units: 07:08, 11:03, 04:01, 01:05 lead both; noun_complementizer
`The question/claim ... was whether/that` sits with v201's verb_complementizer `wondered/remarked ... whether/that`). Recipe as
v198/v202/v206/v208/v210/v212: per evaluated member, arm `own` (own C_fit control) and arm `fam` (own C_fit + EVERY sibling's A1 FIT
prep, control_weight 30 x len(controls)); evaluated on the member's held A1 (extraction), each sibling's held A1 (|CE| leak) and held C.
...
Families: verb_form = question_bare_form (v211, rows 2-3 only: a control), did_has_negation, modal_perfect_form;
complementizer = verb_complementizer (v201, not four-row: a control), noun_complementizer. All five are prepared; EVAL = the three
v213 passes (the non-four-row members are controls only; their verdicts would count for nothing). ~60-80 s per member (~4 min).
Count rule: each separable EVAL member is a distinct circuit (27 -> up to 30); if did_has_negation and modal_perfect_form leak into
each other they are ONE bare-vs-participle circuit with two surface constructions (counts +1 only if one of them is separable from
question_bare_form, else 0 -- fused with an uncounted construction stays uncounted until a four-row member of the family separates).

REGISTERED BEFORE THE RUN (each coded predicate is the sentence here):
    pred_a_did_has               did_has_negation is separable from question_bare_form and modal_perfect_form.        prior 40%
    pred_b_modal_perfect         modal_perfect_form is separable from question_bare_form and did_has_negation.        prior 40%
    pred_c_noun_complementizer   noun_complementizer is separable from verb_complementizer.                           prior 65%
    pred_d_row4_kept             fam-arm own-C UB975 <= 0.01 on >= 2 of the 3 evaluated members.                      prior 60%
    pred_e_instrument            every evaluated member's own-arm extraction is within +-0.03 of its v213 row-4 (cdas) extraction_held
                                 (0.996 / 1.008 / 1.003; same units, seed, split and code path -- v210's own arms reproduced v209's
                                 cdas values to the printed digit: a determinism check, NOT an independent run outcome).  prior 90%
Smoke: V214_SMOKE=<out.json> (CPU, V214_SMOKE_ROWS=4; all five members are prepared as controls, V214_SMOKE_NAMES=did_has_negation,noun_complementizer
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v214_result.json"
...
FAMILIES = {
    "verb_form": ["question_bare_form", "did_has_negation", "modal_perfect_form"],
    "complementizer": ["verb_complementizer", "noun_complementizer"],
}
EVAL = ("did_has_negation", "modal_perfect_form", "noun_complementizer")
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES, **v213.NAMES}


def _v213_cdas(n):
    return float(json.loads(V213.read_text())["behaviours"][n]["arms"]["cdas"]["extraction_held"])
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v214", "members": n, "families": list(FAMILIES),
...
def PREDS(R):
    ...
    a = ok and sep("did_has_negation")
    b = ok and sep("modal_perfect_form")
    c = ok and sep("noun_complementizer")
    ...
    e = ok and all(n in good and good[n]["arms"]["own"]["extraction"] is not None
                   and abs(good[n]["arms"]["own"]["extraction"] - _v213_cdas(n)) <= B["instr_tol"] for n in EVAL)
    return {"pred_a_did_has": bool(a), "pred_b_modal_perfect": bool(b), "pred_c_noun_complementizer": bool(c),
...
def main() -> None:
    ...
    smoke = os.environ.get("V214_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V214_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    evaluate = [n for n in members if n in EVAL and (not smoke or n in os.environ.get("V214_SMOKE_NAMES", "did_has_negation,noun_complementizer").split(","))]
    prior = {**json.loads(V197.read_text())["behaviours"], **json.loads(V201.read_text())["behaviours"], **json.loads(V204.read_text())["behaviours"], **json.loads(V205.read_text())["behaviours"], **json.loads(V207.read_text())["behaviours"], **json.loads(V209.read_text())["behaviours"], **json.loads(V211.read_text())["behaviours"], **json.loads(V213.read_text())["behaviours"]}
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v214", "candidate_id": "corpus.unit_family_separability_spec_v214",
              "separable_counts": nsep, "families": FAMILIES, "summary": summary, "members": R, "bars": BARS,
              "recipe": {"lambda": LAM, "steps": steps, "lr": LR, "complement_weight": CW, "rank": 1, "seed": 0, "units_source": "v197 + v201 + v204 + v205 + v207 + v209 + v211 + v213 receipts (later receipts override earlier ones)", "evaluated": list(EVAL),
