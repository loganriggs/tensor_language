...
"""v601: work the AUTHORED BACKLOG instead of authoring more cells.
...
Smoke: V601_SMOKE=<out.json> (CPU, V601_SMOKE_ROWS=4).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_crossconstruction601_v601_result.json"
...
NAMES = {"measure_wh_far_long": "measure_wh_far_long",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_crossconstruction601_v601", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V601_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V601_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V601_SMOKE_NAMES", "measure_wh_far_long,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_crossconstruction601_v601", "candidate_id": "corpus.unit_tier3_batch_crossconstruction601_v601", "bars": BARS,
