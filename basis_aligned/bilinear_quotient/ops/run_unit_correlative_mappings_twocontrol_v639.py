...
"""v639: the possessive recipe applied to correlatives -- two-control selectivity across five mappings.

WHY THIS FAMILY AND WHY NOW. The possessive sequence worked: ten cells differing by MAPPING gave seven that cleared
four tier rows and both controls (v633), and six of those seven separated from each other (v635) -- the first real
block of mutually distinct candidates in this corpus. Correlatives are the other family the standing DAS direction
names, and correlative_pair already has a receipt: v627 found it passing all four registered rows under the
canonical control while FAILING the weekly one at +0.0134, so its selectivity was canonical-specific. This rung asks
whether that was a property of correlatives or of that one cell, using the same arrangement that worked for
possessives.
FIVE NEW CELLS PLUS THE ANCHOR. canonical_v2, disjoint_either_not (or/but), open_neu and open_rec (and/nor), or_and
(or/and), with correlative_pair carried along so its v627 numbers can be checked for reproduction rather than
assumed.
ONE CELL EXCLUDED AND THE REASON RECORDED: correlative_state FAILS the known-good rebuild check -- my control-row
builder does not reproduce its own C panel byte-identically under the canonical control, so any v3 rows I built for
it would be untrustworthy. That is a defect in my builder's coverage of that cell's shape, not a reason to run it
anyway, and it is written down here so the exclusion is not silent and so the builder gap is on record.
A LIMIT I AM STATING UP FRONT: the correlative readout inventory is a small closed set of conjunctions -- and, but,
or, nor -- so these five mappings overlap heavily in vocabulary in a way the ten possessive mappings did not. If
they all pass, the separability rung matters MORE here than it did for possessives, not less, and I expect more
fusion for exactly that reason.
RANK 1, registered, and a null is not permission to raise it. Fitted against the WEEKLY control with the CANONICAL
one held out, the arrangement that transfers 64 percent against 32 across 39 cells. Both numbers reported per cell.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
  pred_a_controls_capable the weekly control is capable on all six: at most max_dropped = 4 of its 16 held rows
                        dropped.                                                                      prior 85%
  pred_b_all_reach      every one of the six reaches held-out extraction ext_min = 0.80.              prior 65%
  pred_c_siblings_reached at least k_reach = 4 of the six reach 0.80.                                 prior 80%
  pred_d_reached_are_two_control_selective at least k_both = 3 of the cells that reach are selective on BOTH
                        controls. v627 found the anchor failing the weekly control, so this is genuinely open.
                                                                                                      prior 45%
  pred_e_all_measured   zero cells error.                                                             prior 90%
HOW IT READS. d TRUE: two-control selectivity is not specific to possessives, correlatives supply more candidates,
and the separability rung follows immediately -- with the overlapping-conjunction caveat above making it the
decisive one. d FALSE with c true: correlatives carry the variable but their selectivity does not survive a second
control, v627's anchor result was the family's property rather than that cell's, and the possessive block stands out
as unusual rather than typical. THE ANCHOR IS THE CHECK ON BOTH READINGS: if correlative_pair does not reproduce
v627's canonical -0.0521 and weekly +0.0134, nothing else here is comparable and I will say the rung is void rather
than interpret it.
SCOPE. Six cells, independent fits, rank 1, two controls. No separability test, no counting.
Smoke: V639_SMOKE=<out.json> (CPU, V639_SMOKE_ROWS=4, V639_SMOKE_NAMES=<cell>).
"""
...
OUT = ROOT / "circuits/followups/unit_correlative_mappings_twocontrol_v639_result.json"
...
NAMES = {"correlative_pair": "correlative_pair",
         "correlative_canonical_v2": "correlative_canonical_v2",
         "correlative_disjoint_either_not": "correlative_disjoint_either_not",
         "correlative_open_neu": "correlative_open_neu",
         "correlative_open_rec": "correlative_open_rec",
         "correlative_or_and": "correlative_or_and"}
ANCHOR = "correlative_pair"
SIBLINGS = ['correlative_canonical_v2', 'correlative_disjoint_either_not', 'correlative_open_neu', 'correlative_open_rec', 'correlative_or_and']
# v627 measured the anchor under this same arrangement: canonical -0.0521, weekly +0.0134
V639_ANCHOR = {"canon_ub": -0.0521, "weekly_ub": 0.0134}
...
BARS = {"ext_min": EXT_MIN, "rem_min": REM_MIN, "c_ub_max": C_UB_MAX, "row5_share_band": [0.5, 1.4], "k_reach": 4, "k_both": 3, "tol": 0.05, "max_dropped": 4}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 200000, 6400000


def _plan():
    return {"candidate_id": "corpus.unit_correlative_mappings_twocontrol_v639", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V639_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V639_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V639_SMOKE_NAMES", "verb_preposition_against_for,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_correlative_mappings_twocontrol_v639", "candidate_id": "corpus.unit_correlative_mappings_twocontrol_v639", "bars": BARS,
