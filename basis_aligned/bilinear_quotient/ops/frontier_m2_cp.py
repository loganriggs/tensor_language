"""CP-VS-TABLE AT m2 (rung 109): does the model's OWN grammar dissolve the front-table problem?

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows; LOWER IS BETTER.
S2201: the table+featmap grammar hit a FEATURE wall at m2 (+0.31 flat in residual rank, 1/62 valid). But m2
IS a 4608-unit bilinear MLP - the CP-prune grammar used at c4-c9 was never tried at the front (tables were the
historical choice). Here m2 is replaced SINGLE-SITE by its own top-K units (norm importance, weights-only, NO
fitting rows), K in {288, 576, 1152, 2304, 4608}, with census certificates per arm.
Anchor (S2201): the table at m2 costs +0.3072, valid 1/62, and stores ~57.9M f16 values; CP-K stores K x 3456.

REGISTERED PREDICTIONS (arms named agg(K), valid(K)):
  (a) GRAMMAR BEATS TABLE: agg(576) <= 0.5 x 0.3072 = 0.1536 - at 29x fewer stored values.
  (b) VALIDITY MOVES: valid(1152) >= 10 of 62 (table: 1/62).
  (c) SANITY/MONOTONE: agg nonincreasing in K (tol 0.01) and agg(4608) in [-0.01, 0.02] (exactness check,
      S2198 confirmed CP at full K = the real MLP).
NULL: CP-K at a front site is as broken as the table (front damage is site-hardness, not grammar choice).
PRICE: CP-576 at m2 = 1,990,656 values vs table ~57.9M - a 29x cheaper front replacement IF it wins.
Tripwire: INSTRUMENT FAIL if all K arms identical (pstdev < 0.005). Self-reviewed."""
import json, sys, time, os
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
if os.environ.get('BQLIB_DRYRUN')=='1':
    _bq='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
    _need=['frontier_m2_rank_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: CP-vs-table at m2')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_m2_cp_results.json'
...
def main():
    ...
    if SEL.get('m2_cp'):
        _ROWS=SEL.get('ext_rows',FR)
        _mlp=m.transformer.h[2].mlp
        _L=_mlp.Left.weight.detach().float()
        _R=_mlp.Right.weight.detach().float()
        _D=_mlp.Down.weight.detach().float()
        _db=_mlp.Down_bias.detach().float()
        _imp=_D.norm(dim=0)*_L.norm(dim=1)*_R.norm(dim=1)
        _ord=_imp.argsort(descending=True)
        cur['clsmap']=classify2(_ROWS).to(DEV)
        _arms={}
        for _K in (288,576,1152,2304,4608):
            _kp=_ord[:_K]
            S['c2K']=('cp',2,_L[_kp].contiguous(),_R[_kp].contiguous(),
                      _D[:,_kp].contiguous(),_db)
            _arms[f'K_{_K}']=evalV(_ROWS,_ROWS.shape[0],['c2K'],ML).detach().cpu()
            print(f'  m2 CP K={_K} done',flush=True)
        del cur['clsmap']
        SEL['front_result']=_arms
...
if __name__=='__main__':
    ...
    TABLE_ANCHOR=json.load(open(PT+'frontier_m2_rank_results.json'))['arms']['rank_64']['agg']
    SEL['mode']='norm'; SEL['K']=4608; SEL['K69']=4608; SEL['K69MAP']={}
    SEL['skipset']=tuple(range(10,18)); SEL['motif_off']=tuple(range(2,10))
    SEL['m2_cp']=True; SEL['ext_rows']=CROWS
    print('ARMS: m2 single-site CP top-K, K in 288/576/1152/2304/4608 - census rows',flush=True)
    main()
    A=SEL['front_result']
    out={}
    for k,cev in A.items():
        ...
        print(f"  {k:7s} agg {agg:+.4f}  valid {nv}/{len(CINFO)}",flush=True)
    import statistics as stt
    aggs={int(k.split('_')[1]):v['agg'] for k,v in out.items()}
    if stt.pstdev(aggs.values())<0.005: raise SystemExit('INSTRUMENT FAIL: K arms identical')
    Ks=sorted(aggs)
    pa=aggs[576]<=0.5*TABLE_ANCHOR
    pb=out['K_1152']['valid']>=10
    mono=all(aggs[Ks[i]]>=aggs[Ks[i+1]]-0.01 for i in range(len(Ks)-1))
    pc=mono and -0.01<=aggs[4608]<=0.02
    res={'arms':out,'table_anchor_rank64':TABLE_ANCHOR,
         'values_cp576':576*3456,'values_table':50257*1152,
         'convention':'per-position dCE = CE(single m2 CP-K, weights-only) - CE(real model) on census rows; lower is better',
         'pred_a_grammar_beats_table':bool(pa),'pred_b_validity_moves':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"table anchor {TABLE_ANCHOR:+.4f} (57.9M values); CP-576 {aggs[576]:+.4f} (2.0M values)")
    print(f"(a) agg(576) {aggs[576]:+.4f} <= 0.5 x table ({0.5*TABLE_ANCHOR:+.4f}): {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid(1152) {out['K_1152']['valid']} >= 10: {'HELD' if pb else 'FAILED'}")
    print(f"(c) monotone + K4608 exact ({aggs[4608]:+.4f}): {'HELD' if pc else 'FAILED'}")
