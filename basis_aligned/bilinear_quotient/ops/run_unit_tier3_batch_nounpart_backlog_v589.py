...
"""v589: work the AUTHORED BACKLOG instead of authoring more cells.
...
REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here). Thresholds scaled from the 25
cells already lifted, where rows 3, 4 and 5 pass nearly everywhere (19 of 25, 24 of 25, 25 of 25) and ROW 2 passes
only 6 of 25. Eight cells here:
  pred_a_four_rows    at least k_four = 1 of the eight clears ALL FOUR tier rows.                     prior 50%
  pred_b_row2_still_binding row 2 passes AT MOST row2_max = 3 of the eight, holding the roughly one-in-four rate
                      seen across 25 cells. It FAILS if row 2 passes four or more, which would mean these two
                      families are less direction-asymmetric than the two already measured.           prior 75%
  pred_c_row3         row 3 passes at least k_row3 = 7 of the eight.                                  prior 75%
  pred_d_row4         row 4 passes at least k_row4 = 6 of the eight.                                  prior 70%
  pred_e_row5_amended at least k_row5 = 5 of the eight clear row 5 under the amendment.               prior 70%
...
HOW IT READS. This rung finishes the plain-mapping backlog, so its main job is to complete the denominator rather
than to test a new idea: after it, 33 of the 175 unprocessed cells will have been lifted and the yield of countable
behaviours per cell will rest on four families instead of two. b holding: row 2 is the binding constraint in
noun_preposition and verb_particle too, and direction asymmetry is a property of the protocol rather than of the two
families I happened to start with. b FAILING: these families are more symmetric, which would make them the place to
mine rather than verb_preposition. a false with c, d, e true: eight more clean lifts and nothing countable, which
sharpens the yield estimate downward and is worth having before anyone lifts the remaining 142.
SCOPE. Eight behaviours across two families; units and tier rows only, no counting. The _b and _c shape variants of already-counted cells are EXCLUDED, since lifting a variant of a counted cell would pad the denominator with near-duplicates.
Smoke: V589_SMOKE=<out.json> (CPU, V589_SMOKE_ROWS=4).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_nounpart_backlog_v589_result.json"
...
NAMES = {"noun_preposition_at_beyond": "noun_preposition_at_beyond",
         "noun_preposition_at_toward": "noun_preposition_at_toward",
         "noun_preposition_between_with": "noun_preposition_between_with",
         "noun_preposition_on_toward": "noun_preposition_on_toward",
         "noun_preposition_reason": "noun_preposition_reason",
         "noun_preposition_within_for": "noun_preposition_within_for",
         "verb_particle_on_away": "verb_particle_on_away",
...
BARS = {"ext_min": EXT_MIN, "rem_min": REM_MIN, "c_ub_max": C_UB_MAX, "row5_share_band": [0.5, 1.4], "k_four": 1, "row2_max": 3, "k_row3": 7, "k_row4": 6, "k_row5": 5, "instr_tol": 0.02}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 200000, 6400000


def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_nounpart_backlog_v589", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V589_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V589_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V589_SMOKE_NAMES", "noun_preposition_reason,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_nounpart_backlog_v589", "candidate_id": "corpus.unit_tier3_batch_nounpart_backlog_v589", "bars": BARS,
