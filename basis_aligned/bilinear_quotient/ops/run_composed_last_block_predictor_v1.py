...
"""pred_a previousN/C/R native outcomes andadditivefinalbackground<=1e-4relative.
pred_b g(zBar+fullhead2mixedwrite) predicts fulllastblocklocaleffect<=.10 eachfivegroups.
pred_c g(zBar+compacthead2mixedwrite) predicts fulllastblocklocaleffect<=.10 eachfivegroups.
Null: otherheads orMLP propagation errors prevent composingthelocal predictor.
Price360fullforwards+360extraMLPevaluations+480finalreadouts;120reusedprefixes;180seconds;no fitting.
"""
...
STEM='COMPOSED_LAST_BLOCK_PREDICTOR_V1'
@torch.no_grad()
def main():
 ...
 assert count==360
 old=torch.load(P/'ADDITIVE_HEAD_RAW_PORTS_NATIVE_V1_ARTIFACT.pt',weights_only=True);rel=lambda a,b:float((a-b).norm()/b.norm().clamp_min(1e-30));replay=[rel(measures[:,[0,1,3]],old['measures'][:,[0,1,3]]),rel(readouts[:,0],old['readouts'][:,0])];cells=[]
 for group in range(5):
  lo,hi=24*group,24*(group+1);z=readouts[lo:hi,:,0];prior=old['readouts'][lo:hi,:,0];full=z[:,1]-z[:,0];small=z[:,2]-z[:,0];native=old['measures'][lo:hi,4,0]-prior[:,0];previous=native
  cells.append(dict(group=group,full_effect_error=rel(full,native),compact_preservation_error=rel(small,previous),mlp_only_error=rel(z[:,3]-z[:,0],native),compact_native_effect_error=rel(small,native),compact_same_nonzero_sign=int((small*native>0).sum()),native_zero_count=int((native==0).sum()),maxabs_full_error=float((full-native).abs().max())))
 result={'pred_a':max(replay)<=1e-4,'pred_b':all(c['full_effect_error']<=.1 for c in cells),'pred_c':all(c['compact_preservation_error']<=.1 for c in cells),'anchor_replay':replay,'cells':cells,'body_forwards':count,'extra_final_readouts':480,'extra_mlp_evaluations':360,'seconds':time.perf_counter()-tic,'executed_arms':[0,1,3],'scope':'Three-trajectory composed head17.2 mixedwrite plusnativeMLP17 predictor ofwholelastblocklocalgeneration atadditivebackground. FullMLPweights andzN/C/R supplied; no Atrajectory orfreshOOD; nottotalchild/remainderinteraction.'}
 torch.save(dict(measures=measures[:,[0,1,3]],readouts=readouts),art);result['artifact_sha']=digest(art);out.write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result),flush=True)
