"""HEAD-PARITY MIRROR, v2 (rung 260): fine band missing at the ODD heads (par=0 exacts the evens).

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. Rerun of rung 259 after S2359's
tripwire fix (range prereg, no scalar-proximity check). Rung 258 (band missing at EVEN heads, labels per
the S2359 correction): 0.0544 - ~98% of the full-band 0.0553.
REGISTERED PREDICTIONS (arms: THIS vs full-band 0.0553/11 and even-deficient 0.0544/11):
  (a) GLOBAL-SYMMETRIC: census in [0.048, 0.062] (the mirror also costs ~full).
  (b) valid >= 11.
  (c) SANITY: census in [0.01, 0.12].
NULL: asymmetric localization (census < 0.048 - the odd heads carry less of the mechanism).
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: head-parity mirror v2')
    raise SystemExit(0)
import torch
...
OUT=PT+'band_mirror_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=96; SEL['qk_rmap']={li:128 for li in range(2,10)}; SEL['qk_hpar']=0; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    print(f"(a) GLOBAL-SYMMETRIC: census {agg:+.4f} in [0.048, 0.062]: {'HELD' if pa else 'FAILED'}")
