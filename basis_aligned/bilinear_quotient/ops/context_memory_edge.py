...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['front_m16_pipeline_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; b5-damage vs a16L-damage co-variation (context->memory edge; damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'context_memory_edge_results.json'
...
if __name__=='__main__':
    ...
    SEL['motif_off']=(5,)
    print('ARM 2/3: block-5 motifs REAL',flush=True)
    _=main(); cev_no5=SEL['cev']
    SEL['motif_off']=(); SEL['skipset']=(16,)
    print('ARM 3/3: skip-a16 (attn16 real)',flush=True)
    _=main(); cev_s16=SEL['cev']
    import statistics as stt
    for nm,cv in (('no5',cev_no5),('s16',cev_s16)):
        if float((cv-cev_full).abs().sum())<1e-3:
            raise SystemExit(f'INSTRUMENT FAIL: {nm} identical to full')
    d5=(cev_full-cev_no5); d16=(cev_full-cev_s16)
    def rank(v):
        o=sorted(range(len(v)),key=lambda i2:v[i2]); r7=[0]*len(v)
        for pos,i2 in enumerate(o): r7[i2]=pos
        return r7
    def binned_rho(mask):
        mm=mask.nonzero().squeeze(1)
        order=d5[mm].argsort(); mo=mm[order]
        B=200; x=[];y=[]
        for k0 in range(0,len(mo)-B+1,B):
            ii=mo[k0:k0+B]
            x.append(float(d5[ii].mean())); y.append(float(d16[ii].mean()))
        n7=len(x)
        if n7<=2: return 0.0,n7
        return 1-6*sum((a-b)**2 for a,b in zip(rank(x),rank(y)))/(n7*(n7*n7-1)), n7
    OTH=CLS.index('other'); IND=CLS.index('ind')
    ro,no=binned_rho(clsflat==OTH)
    ri,ni=binned_rho(clsflat==IND)
    pa=ro>=0.3
    pb=ri>=0.3
    pc=abs(r_full['L2_F']-2.6662)<=0.01
    res={'full':{'L2_F':r_full['L2_F']},'rho_other':round(ro,3),'bins_other':no,
         'rho_ind':round(ri,3),'bins_ind':ni,
         'convention':'d5 = CE(full)-CE(no5); d16 = CE(full)-CE(skip-a16); positive = the replacement hurts there',
         'pred_a_edge_other':bool(pa),'pred_b_edge_ind':bool(pb),'pred_c_reproduces_s2144':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'binned Spearman(d5,d16): other {ro:+.3f} ({no} bins) | ind {ri:+.3f} ({ni} bins)')
    print(f"(a) other rho {ro:+.3f} >= 0.3: {'HELD' if pa else 'FAILED'}")
    print(f"(b) ind rho {ri:+.3f} >= 0.3: {'HELD' if pb else 'FAILED'}")
