...
"""v333: THE REPLICATION RUNG. A new three-member family, outside verb_particle, carrying both arms of this morning's
result in one control set.

WHAT IS BEING REPLICATED, AND WHY IT NEEDS ITS OWN FAMILY. v321 and v325 closed a 2x2 inside verb_particle: cells
sharing a full readout PAIR with a counted circuit but no cue verb are separable from it (fam-arm leak into the
original -0.0033 / -0.0076 / -0.0093), while cells reusing both (cue -> token) MAPPINGS all fused, with all five
supra-bar fam-arm leaks landing on a cue donor and the largest non-donor leak at 0.0174. That is one readout class.
This rung repeats the whole design in the PREPOSITION class with three cells that exist only for the test:
    verb_preposition_repl_base    grumbled -> ` about`, lobbied -> ` for`
    verb_preposition_repl_twin    moaned   -> ` about`, pined   -> ` for`   same pair as the base, both cues fresh
    verb_preposition_repl_reuse   grumbled -> ` about`, pined   -> ` for`   one cue borrowed from each of the above
None of the 72 existing verb_preposition cells is in this family, so the verdicts here cannot contaminate, or be
contaminated by, the family whose separability is halted at the memory wall -- and the rung costs three prepared
members rather than forty. The LIMIT is stated plainly: this replicates across the readout class and the verb
inventory, not across frames; all three cells use the same bare_frame the particle cells use.
THE ARMS SIT IN ONE CONTROL SET ON PURPOSE. When the base is evaluated, the twin and the reuse cell are both its
controls; the reuse cell is a sibling the base's direction must spare. If base and twin still separate while reuse
fuses into both, the account survives the hardest arrangement I can build for it in a three-cell family.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
    pred_a_new_separable   at least k_new = 2 of the 3 members are separable at the registered cross bar.  prior 70%
    pred_b_pattern         the EXACT pattern: base separable AND twin separable AND reuse NOT separable. This is the
                           predicate that carries the replication, because pred_a's count of two can be reached by
                           the wrong pair -- base and reuse separable with the twin fused would satisfy pred_a while
                           refuting the account. Registered as the conjunction so it cannot be read either way after
                           the fact.                                                                       prior 55%
    pred_c_row4_kept       fam-arm own-C UB975 <= 0.01 on at least k_d = 2 of the 3 evaluated members.     prior 70%
    pred_d_instrument      every member's own-arm extraction is within 0.03 of its v331 cdas extraction_held. Fails
                           automatically if any member errors.                                             prior 90%
    pred_e_reuse_both      the reuse cell's fam-arm leak exceeds the 0.05 cross bar into BOTH of its cue donors, i.e.
                           min(|leak into base|, |leak into twin|) > cross_max = 0.05. Its only two siblings ARE its
                           donors, so this is not a partner-identification claim -- it is the claim that BOTH borrowed
                           mappings drive the fusion rather than one carrying it alone. Registered on the FAM arm
                           after v325's clause failed on the own arm, where generic within-family collateral of
                           0.35-0.44 swamps the signal. Worked example: leaks of 0.09 and 0.07 give min = 0.07 > 0.05,
                           TRUE, both mappings bind; leaks of 0.12 and 0.02 give 0.02, FALSE, and only the ` about`
                           mapping binds while ` for` is free -- which would be a real and reportable asymmetry, not a
                           null.                                                                           prior 60%
COUNTING. If the pattern holds, base and twin are +2 on 134 and the reuse cell is +0. If base and twin FUSE with each
other, the pair result does not leave verb_particle, this morning's relaxed gate applies to particles only, and I
would rather learn that from three cheap cells than after authoring fifty under it.
BARS = {"cross_max": 0.05, "keep": 0.8, "ext_floor": 0.5, "c_ub_max": 0.01, "k_d": 2, "k_new": 2, "instr_tol": 0.03}
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v333_result.json"
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v333", "members": n, "families": list(FAMILIES),
...
def PREDS(R):
    ...
    new_sep = sum(1 for n in NEW_MEMBERS if sep(n))
    a = ok and new_sep >= B["k_new"]
    BASE, TWIN, REUSE = "verb_preposition_repl_base", "verb_preposition_repl_twin", "verb_preposition_repl_reuse"
    b = ok and all(n in good for n in (BASE, TWIN, REUSE)) and sep(BASE) and sep(TWIN) and not sep(REUSE)
    ...
    e = False
    if REUSE in good:
        sibs = good[REUSE]["arms"]["fam"]["siblings"]
        if BASE in sibs and TWIN in sibs:
            e = min(abs(sibs[BASE]), abs(sibs[TWIN])) > B["cross_max"]
    return {"pred_a_new_separable": bool(a), "pred_b_pattern": bool(b), "pred_c_row4_kept": bool(c),
            "pred_d_instrument": bool(d), "pred_e_reuse_both": bool(e)}


def main() -> None:
    if os.environ.get("BQLIB_DRYRUN") or os.environ.get("BQLIB_NO_MODEL"):
        print(json.dumps(_plan(), indent=2, sort_keys=True)); return
    smoke = os.environ.get("V333_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V333_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v333", "candidate_id": "corpus.unit_family_separability_spec_v333",
