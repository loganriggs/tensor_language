#!/usr/bin/env python3
# BQLANE: gpu
# BQGATE: EXPERIMENT pred_a_exact_instrument pred_b_native_capability pred_c_instruction_subset_replays_cross pred_d_cross_live_and_directional pred_e_unrelated_reader_selectivity
"""Fresh recursive test of the instruction-source subset of the head9.8 cross term."""
...
PREREG=P/"SETTING2_REGIONAL_INSTRUCTION_CROSS_FRESH_V1_PREREGISTRATION.md"
ROWS=P/"SETTING2_REGIONAL_INSTRUCTION_CROSS_FRESH_V1_ROWS.json"
BINDING=P/"SETTING2_REGIONAL_INSTRUCTION_CROSS_FRESH_V1_BINDING.json"
...
OUT=ROOT/"basis_aligned/bilinear_quotient/circuits/fast_screens/setting2_regional_instruction_cross_fresh_v1_result.json"
SOURCE_NAMES=("embedding",)+tuple(x for layer in range(8) for x in (f"attn{layer}",f"mlp{layer}"))
LATE_NAMES=("attn5","mlp5","mlp6","mlp7")
ARMS=("native","additive_without_cross","instruction_cross","full_cross")
...
PRICE={"physical_model_executions":24,"sequences_per_arm":48,"arms":4,"fits":0,"backwards":0,"parameter_updates":0}
...
def load_bound():
    ...
    files={"preregistration":PREREG,"rows":ROWS,"row_check":P/"regional_cue_row_check_v1.py",
           ...
           "edge_program":EDGE,"parent_result":PARENT,"composition_result":COMPOSITION,"source_census":CENSUS}
...
def plan():
    ...
    return {"schema":"setting2_regional_instruction_cross_fresh_v1_plan","model_loaded":False,"gpu_accessed":False,"queue_touched":False,"rows":len(rows),"arms":ARMS,"late_names":LATE_NAMES,"price":PRICE,"row_checks":checks,"binding_sha256":sha(BINDING)}
...
def main():
    ...
    for _,indices in sorted(buckets.items()):
        for offset in range(0,len(indices),8):
            ...
            for arm_index,arm in enumerate(ARMS):
                ...
                edit={"additive_without_cross":route_delta+value_delta,"instruction_cross":route_delta+value_delta+instruction_cross,"full_cross":joint_delta}.get(arm)
                ...
                executions+=1
    effects=values-values[0]; pair_effect=paired_difference(effects,2); native_pairs=paired_difference(values[0],1); reports={}
    for family in sorted(set(row["family"] for row in rows)):
        row_ids=[i for i,row in enumerate(rows) if row["family"]==family]; pair_ids=[i//2 for i in row_ids[::2]]; full_vectors=full_cross_vectors[row_ids]; instruction_vectors=instruction_cross_vectors[row_ids]
        report={"native_positive_pairs":int((native_pairs[1,pair_ids,0]>0).sum()),"instruction_head_replay_relative_error":rel(instruction_vectors,full_vectors),"instruction_head_replay_cosine":cosine(instruction_vectors,full_vectors)}
        for scale,name in enumerate(("numerator","logit")):
            additive=pair_effect[1,scale,pair_ids,0]; instruction=pair_effect[2,scale,pair_ids,0]; full=pair_effect[3,scale,pair_ids,0]; instruction_cross_effect=instruction-additive; full_cross_effect=full-additive
            controls={READOUTS[j][0]:float(effects[3,scale,row_ids,j].square().mean().sqrt()/full.square().mean().sqrt().clamp_min(1e-30)) for j in range(1,len(READOUTS))}
            report[name]={"additive_target_rms":float(additive.square().mean().sqrt()),"instruction_joint_target_rms":float(instruction.square().mean().sqrt()),"full_joint_target_rms":float(full.square().mean().sqrt()),"instruction_cross_rms":float(instruction_cross_effect.square().mean().sqrt()),"full_cross_rms":float(full_cross_effect.square().mean().sqrt()),"instruction_cross_replay_relative_error":rel(instruction_cross_effect,full_cross_effect),"instruction_cross_replay_cosine":cosine(instruction_cross_effect,full_cross_effect),"full_cross_signed_mean":float(full_cross_effect.mean()),"unrelated_reader_ratios":controls}
    ...
    predictions={"pred_a_exact_instrument":bool(instrument),"pred_b_native_capability":bool(instrument and all(r["native_positive_pairs"]>=10 for r in reports.values())),"pred_c_instruction_subset_replays_cross":bool(instrument and all(r["instruction_head_replay_relative_error"]<=.30 and r["logit"]["instruction_cross_replay_relative_error"]<=.35 for r in reports.values())),"pred_d_cross_live_and_directional":bool(instrument and all(r["logit"]["full_cross_rms"]>=.01 and r["logit"]["instruction_cross_replay_cosine"]>=.90 for r in reports.values())),"pred_e_unrelated_reader_selectivity":bool(instrument and all(max(r["logit"]["unrelated_reader_ratios"].values())<=.75 for r in reports.values()))}
    ...
    result={"schema":"setting2_regional_instruction_cross_fresh_v1_result","terminal":"fresh_instruction_cross_subset" if all(predictions.values()) else "valid_null" if instrument else "invalid","predictions":predictions,"metrics":metrics,"family_reports":reports,"arms":ARMS,"readouts":[name for name,_ in READOUTS],"late_source_names":LATE_NAMES,"price":PRICE,"row_checks":checks,"binding_sha256":sha(BINDING),"runner_sha256":sha(RUNNER),"checkpoint_weights_sha256":"680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3","created_utc":datetime.now(timezone.utc).isoformat().replace("+00:00","Z"),"scope":"Fresh recursive comparison of instruction-only versus full source support for the explicit head9.8 late-QK1 by head8.2-induced-current-value cross term; fixed new templates/cities, five unrelated readers, no fit."}
