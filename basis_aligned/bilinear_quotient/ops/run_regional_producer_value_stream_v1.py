#!/usr/bin/env python3
# BQGATE:7 body forwards;32 sequences lengths4-12;4 suffix arms;no fit;180sec.
"""pred_a native folded-producer and prior both-value swap replay<=1e-5 relative;
pred_b both-value transfer>=90%prior full-group and>=5/8positive EACHfamily;
pred_c current-only transfer>=90%both-value, unrelated<=.5regional EACHfamily.
Null: the shared first-layer value stream can carry a substantial part of the cue.
"""
...
STEM='REGIONAL_PRODUCER_VALUE_STREAM_V1'
@torch.no_grad()
def main():
    ...
    pre=torch.empty(32,1152,device='cuda');writes=torch.zeros(32,4,1152,dtype=torch.float64,device='cuda');replays=[];count=0
    ...
    def producer_hook(layer,module,args,out):
        ...
        first_values=torch.einsum('nsd,ahd->nsha',captured['first'].double(),H)
    ...
    prior=torch.load(P/'REGIONAL_PRODUCER_ROUTE_VALUE_V1_ARTIFACT.pt',weights_only=True,map_location='cpu')['write_vertices'].cuda()
    prior_group=prior[:,2]
    ...
    reference_cells=json.loads((P/'REGIONAL_PRODUCER_ROUTE_VALUE_V1_RESULT.json').read_text())['cells']
    for family in (0,1):
