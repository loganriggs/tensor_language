"""MOTIF BAND IN ISOLATION (rung 263): motifs r96, TAIL EXACT - the cross-half overlap test.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. With tail r64 backdrops, the motif
band deficiency costs 0.0068 (economical 0.0676 vs corner 0.0608). Here the tail's global mechanism is
removed entirely (tail exact): if the two halves' bands are INDEPENDENT mechanisms, the isolated motif
band costs ~0.007 and the certificate ceiling without the tail mechanism becomes visible (many margins
were pinned by the tail's 0.05). Science point, not a config (tail uncompressed).
REGISTERED PREDICTIONS (arms: THIS vs economical-family receipts; motif-band-with-tail-r64 delta 0.0068):
  (a) INDEPENDENT MECHANISMS: census <= 0.012.
  (b) CERTIFICATE CEILING LIFTS: valid >= 20 (margins were tail-pinned).
  (c) SANITY: census in [0.001, 0.05].
NULL: cross-half overlap - the motif band shares the tail's mechanism (census >= 0.03), or the ceiling
stays (valid < 15 despite low census). PRICE: probe (motifs r96 35.4M + tail exact). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: motif band in isolation')
    raise SystemExit(0)
import torch
...
OUT=PT+'motif_band_iso_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=96; SEL['qk_rmap']={li:128 for li in range(10,18)}; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    print(f"(a) census {agg:+.4f} <= 0.012: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 20; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) census in [0.001, 0.05]; era rho {rho:.4f}: {'HELD' if pc else 'FAILED'}")
