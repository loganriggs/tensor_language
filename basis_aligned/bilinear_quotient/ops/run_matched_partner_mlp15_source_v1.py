...
"""pred_a native source/replay identities; pred_b background-only swaps<=.1;
pred_c producer-dependent swaps<=.1; sign>=.9/live>=4 on3 active families.
...
STEM='MATCHED_PARTNER_MLP15_SOURCE_V1'
...
def main():
    ...
    def producer_hook(module,args,value):captured['producer']=value.detach()
    ...
    handles=[block.register_forward_pre_hook(source_hook),block.attn.register_forward_hook(attention_hook),model.transformer.h[15].mlp.register_forward_hook(producer_hook),
    ...
    source={k:torch.empty(128,1152) for k in ('background','producer')};checks=[]
    try:
        for length,indices in sorted(buckets.items()):
            for offset in range(0,len(indices),8):
                ...
                aa=block.lambdas[0]*captured['producer'][:,-1];rr=captured['residual'][:,-1]+captured['attention'][:,-1]-aa;xx=captured['input16'][:,-1]
                ...
                for key,value in [('background',rr),('producer',aa)]:source[key][selected]=value.cpu()
                checks.extend([float((reconstructed-xx).norm()/xx.norm()),float((old-xx).norm()/old.norm()),float((manual-physical).norm()/physical.norm())])
    finally:
        for hook in handles:hook.remove()
    assert counts==[16,128]
    program=torch.load(P/'MATCHED_PARTNER_EXACT_INPUT_FOLD_V1_PROGRAM.pt',weights_only=True);rr=source['background'].double().cuda();aa=source['producer'].double().cuda()
    ...
    result.update(checks=checks,gain_errors=gain_errors,effects=effects,gain0_effects=gain0_effects,counts=counts,artifact_sha=digest(ap),source_shas=binding,
                  execution_seconds=time.perf_counter()-tic,scope='Conditional MLP15-to-MLP16 producer edge: scaled full MLP15 write versus remaining input; attention16 and downstream denominator remain native ports. Not whole-model MLP15 ablation.')
