"""ADDITIVITY CONTROL (m16, rung 272): the same knockout on a DIFFERENT baseline (corner config).
...
(mixed-baseline values: m16 subtracted own 0.755, collateral 0.90). The physical test of the
...
REGISTERED PREDICTIONS (arms: THIS (corner baseline) vs the mixed-baseline subtracted values):
  (a) BASELINE-INDEPENDENT: subtracted own median in [0.5, 1.0] (mixed value +/- 0.25 x ref-scale).
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: additivity control m16')
    raise SystemExit(0)
import torch
...
OUT=PT+'ko_m16_corner_results.json'
