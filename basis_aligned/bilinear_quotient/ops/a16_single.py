"""THE DEPLOYED TAIL-DICT GRAMMAR, REAL-FRAME, CIRCUIT-SCORED AT a16 (rung 158).

CONVENTION (S2135): per-position dCE = CE(real-frame a16L dictionary, all else real) - CE(real model) on the
census rows. The a16L class dictionary (probe + CV + LW link maps) is the frontier's deployed replacement for
its most circuit-loaded tail component (13 own circuits). Here it is FIT IN THE REAL FRAME (the aXL capture
installs nothing; fit_frame_real patches only the fit-loop capture at its first occurrence - the line-800
twin sits in a gated Fisher block, off in this config) and evaluated single-site: the grammar's intrinsic
circuit fidelity, uncontaminated by upstream drift.

REGISTERED PREDICTIONS:
  (a) a16-dict breadth >= 4 of 13 (test-half members >= 20; tau = 0.5 x battery ref).
  (b) SANITY/ANTI-INERT: agg in [0.005, 0.10].
  (c) COVERAGE: >= 8 a16 circuits have >= 20 test members (instrument guard).
NULL: breadth 0 - the class-dictionary grammar is circuit-unfaithful even in the real frame at its own
component, matching a16:clsdict 0/13 from S2251 (probe + LW link maps buy nothing at circuit grain).
PRICE: none (deployed grammar). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: real-frame a16L dict, circuit-scored')
    raise SystemExit(0)
import torch
...
OUT=PT+'a16_single_results.json'
...
def main():
    ...
    for li in range(10,18):
        ...
        Ys=[]; Xs=[]
        hs=install([] if SEL.get('fit_frame_real') else order2)+motif_hooks(ML)
        def cap2(mo_,i_,o_):
    ...
    if SEL.get('a16_single'):
        _ROWS=SEL.get('ext_rows',FR)
        cur['clsmap']=classify2(_ROWS).to(DEV)
        SEL['front_result']={'a16_dict':evalV(_ROWS,_ROWS.shape[0],['a16L'],[]).detach().cpu()}
        print('  a16L real-frame single-site done',flush=True)
...
if __name__=='__main__':
    ...
    SEL['skipset']=(10,11,12,13,14,15,17); SEL['motif_off']=tuple(range(2,10)); SEL['clsdmg']=False; SEL['ext_rows']=CROWS; SEL['a16_single']=True; SEL['fit_frame_real']=True
    SEL['K']=4608; SEL['K69']=4608; SEL['K69MAP']={}
    print('ARM: real-frame a16L dictionary single-site, circuit rows',flush=True)
    main()
    ...
    for t,v in BATC.items():
        ...
        REFD[t]=(mm,v['mean_ablation']['top'][0]['abs_dce_members'],
                 v['mean_ablation']['top'][0]['component'])
    out={}
    for k,cev in A.items():
        if cev.shape[0]!=NFLAT: raise SystemExit(f'INSTRUMENT FAIL: {k} cev {cev.shape[0]} != {NFLAT}')
        d=cev-CBASE
        te={t:round(float(d[HALF:][mm[HALF:]].abs().mean()),4)
            for t,(mm,ref,top) in REFD.items() if int(mm[HALF:].sum())>=20}
        out[k]={'agg':round(float(d.mean()),4),'test_members':te}
        print(f"  {k}: agg {out[k]['agg']:+.4f}",flush=True)
    te=out['a16_dict']['test_members']
    own=[t for t,(mm,ref,top) in REFD.items() if top=='a16']
    b16=sum(1 for t in own if t in te and te[t]<0.5*REFD[t][1]); n16=len(own)
    ag=out['a16_dict']['agg']
    cov=sum(1 for t in own if t in te)
    pa=b16>=4
    pb=0.005<=ag<=0.10
    pc=cov>=8
    res={'arms':out,'breadth_a16':[b16,n16],'test_covered_a16':cov,
         'convention':'per-position dCE = CE(real-frame a16L dict, all else real) - CE(real model) on census rows',
         'pred_a_a16_breadth':bool(pa),'pred_b_agg_sane':bool(pb),'pred_c_coverage':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'a16-dict breadth {b16}/{n16}; agg {ag:+.4f}; coverage {cov}')
    print(f"(a) a16 breadth {b16} >= 4: {'HELD' if pa else 'FAILED'}")
    print(f"(b) agg {ag:+.4f} in [0.005, 0.10]: {'HELD' if pb else 'FAILED'}")
    print(f"(c) coverage {cov} >= 8: {'HELD' if pc else 'FAILED'}")
