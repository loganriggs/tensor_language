...
"""Fixed-position forward folding: the slot's role (v607; approved 19 Sep 22:12 UTC). Logan's phrasing: "fix all positions but one and fold forward
(the slot's role)".

FIX (v607, after v604's instrument bug): v604's "content-free reference" averaged different sentences' tokens at each position INDEX (position 5
of one sentence with position 5 of another, regardless of content) -- a smeared, incoherent baseline that itself carried a large, inconsistent margin
(bigger in magnitude than the native margin on one sentence). Fixed here with a position-INDEPENDENT reference: the mean of the full vocabulary's own
embedding table (one D-vector, the same at every position) -- the standard "no information" baseline for an embedding space, and free of any
cross-sentence content leakage. Fraction reporting is also guarded: a slot's fraction of the native margin is only computed when the native margin's
magnitude clears a floor (0.5 logits), so a near-zero denominator cannot blow a small numerator into a meaningless four-digit percentage.

Method: for a natural sentence, replace EVERY position's embedding except one with the reference embedding above, leaving position t's own embedding
untouched, run the real forward, and read the they-he margin. Doing this once
...
OUT = ROOT / "circuits/followups/fixed_position_slot_role_fixed_v607_result.json"
CANDIDATE_ID = "slot_role.fixed_position_fixed_v607"
N_SENT = 4
...
def main() -> None:
    ...
    # global vocabulary-mean embedding: ONE D-vector, position-independent, no cross-sentence content leakage
    FRACTION_FLOOR = 0.5
    with torch.no_grad():
        vocab_mean = model.transformer.wte.weight.float().mean(0)   # [D]
    ...
    for r_ in recs:
        ...
        e_ref = vocab_mean[None, None, :].expand(1, T, D).clone()
        ...
        usable = abs(native_margin) >= FRACTION_FLOOR
        other_max = max(abs(v / native_margin) for k, v in slots.items() if k != noun_pos) if usable else None
        reports.append({"noun_pos": noun_pos, "T": T, "native_margin": native_margin, "true_margin": true_margin, "replay_err": replay_err, "all_ablated_margin": all_ablated_margin, "slots": slots,
                         "slot_sum": slot_sum, "noun_is_max_slot": noun_is_max, "other_slots_max_fraction": other_max, "usable": usable,
                         "noun_slot_fraction": slots[noun_pos] / native_margin if usable else None})
        print(f"native={native_margin:.3f} ablated={all_ablated_margin:.3f} noun_slot={slots[noun_pos]:.3f} noun_is_max={noun_is_max} slot_sum={slot_sum:.3f} other_max_frac={'NA' if other_max is None else f'{other_max:.3f}'} usable={usable}")

    usable_reports = [r for r in reports if r["usable"]]
    predictions = {"pred_a_baseline_replays": all(r["replay_err"] <= REPLAY_TOL for r in reports),
                   ...
                   "pred_c_other_slots_small": all(r["other_slots_max_fraction"] <= OTHER_MAX for r in usable_reports) if usable_reports else None,
                   "pred_d_slots_sub_additive": all(abs(r["slot_sum"]) < abs(r["native_margin"]) for r in reports),
                   "pred_e_mean_ablation_near_zero_margin": all(abs(r["all_ablated_margin"]) <= MEAN_MAX * abs(r["native_margin"]) for r in usable_reports) if usable_reports else None}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "fixed_position_slot_role_fixed_result_v607", "candidate_id": CANDIDATE_ID, "plan": plan, "reports": reports,
