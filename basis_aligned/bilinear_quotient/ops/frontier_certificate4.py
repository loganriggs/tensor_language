...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_certificate3_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: rung-101 artifact present; tables+middles-only certificate")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_certificate4_results.json'
...
if __name__=='__main__':
    ...
    print('ARM: tables+middles-only config (all attention real) on the census rows',flush=True)
    r_cfg=main()
    ...
    print(f"(a) valid {nvalid} >= 20: {'HELD' if pa else 'FAILED'}")
    print(f"(b) aggregate {agg:+.4f} <= 2.0: {'HELD' if pb else 'FAILED'}")
