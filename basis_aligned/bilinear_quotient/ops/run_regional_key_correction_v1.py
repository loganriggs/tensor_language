#!/usr/bin/env python3
# BQGATE:10 body forwards;48 sequences lengths6-14;7 suffix arms;no fit;180sec.
"""pred_a native/complete-correction branch replay<=1e-5 and CPUcontrol<=1e-12;
pred_b denominator-only correction write/effect error<=.1 eachfamily;
pred_c retained+mixed numerator withtrue denominator write/effect error<=.1 eachfamily.
Null: neither restricted correction suffices. No fittedscalar; bothQK factorsjoint.
"""
...
STEM='REGIONAL_KEY_CORRECTION_V1'
@torch.no_grad()
def main():
    ...
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('10 body forwards48sequences6-14tokens;7suffix arms;no fitting');return
    ...
    pre=torch.empty(48,1152,device='cuda');writes=torch.zeros(48,7,1152,dtype=torch.float64,device='cuda');replays=[];count=0
    ...
    def producer_hook(layer,module,args,out):
        ...
        cue_routes=[torch.stack([native_route[i,:,:,c] for i,c in enumerate(captured['cue_positions'])])]+[correction[key]*causal for key in ('base','denominator_only','numerator_only','mixed_true_denominator','full')]
        ...
        variants=torch.stack([torch.einsum('nht,nha->nta',routing,delta) for routing in cue_routes],1)
    ...
    try:
        for length,ids in batches:
            ...
            for pos in range(length):
                ...
                variants=[children]+[children+producer_variants[:,mask,pos]/rho[:,pos] for mask in range(6)]
    ...
    arms=torch.stack([margins(writes[:,j]-writes[:,0]) for j in range(7)])
    ...
    for arm,name in enumerate(('retained','denominator_only','numerator_only','mixed_true_denominator','complete'),2):
    ...
    instrument=max(producer_errors+replays+[replay]+[c['write_relative_error'] for c in cells if c['name']=='complete'])<=1e-5
    def faithful(name):return instrument and all(c['write_relative_error']<=.1 and c['regional_effect_relative_error']<=.1 for c in cells if c['name']==name)
    result={'pred_a':instrument,'pred_b':faithful('denominator_only'),'pred_c':faithful('mixed_true_denominator')}
    torch.save(dict(pre=pre.cpu(),write_vertices=writes.cpu(),arm_margins=arms.cpu()),art)
    result.update(native_branch_replay_error=replay,producer_replay_errors=producer_errors,native_attention_replay_errors=replays,cells=cells,body_forwards=count,frozen_supports=supports,seconds=time.perf_counter()-tic,artifact_sha=digest(art),source_shas=binding,scope='Exact joint-QK numerator/mixed-term/normalizer correction of frozen13 support on geographic48. Native omitted-port generators and queries retained; explanatory accounting, not closed extraction or fitted scalar repair.')
