#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_native_replays pred_b_cost_not_worse pred_c_values_preserved_rank pred_d_no_head_over_1p5 pred_e_values_under_33M
"""Attention lane, v715: bands 0-8 at rank 64 — the pattern program with every leaning band promoted.

v713 (band 6-8 at rank 64): +0.060, Spearman 0.82, median ratio 1.05, but 1.4 still 1.8x, 3.5 1.5x, 1.1 1.25x — v709 traced those to bands 0-2
and 3-5. Same protocol (SVD init, Adam 300 steps, validation stop, seed-703 split) with every head of layers 0-8 at rank 64 (layers 9-17: the
content heads at 64, the rest at 16; no native heads), then the 24-head manipulability check. CE ADDED, lower is better; joint recovery =
1 - cost / 3.996; ratio = in-program / native mean-ablation value.
PREDICTIONS (scored as written; failures preserved)
    pred_a_native_replays        native CE within 0.002 of 3.13241 (instrument)
    pred_b_cost_not_worse        held-out at the validation-chosen step <= 0.060 (v713). Prior: likely
    pred_c_values_preserved_rank Spearman over the 24 heads >= 0.8. Prior: likely
    pred_d_no_head_over_1p5      no head among the 24 has ratio > 1.5 (v713: 1.4 at 1.81). Prior: unsure
    pred_e_values_under_33M      the program's values <= 33M (native QK 95.6M). Prior: certain by construction (registered as the price)
...
manipulability 6 + 24 x 6 = 150; total ~605 forwards, 300 backwards; fit parameters ~33M. Bars: forwards <= 625, backwards <= 300.
"""
...
OUT = ROOT / "circuits/followups/attention_bands08_r64_fit_v715_result.json"
OUT_PT = ROOT / "circuits/followups/attention_bands08_r64_fit_v715_programs.pt"
...
CANDIDATE_ID = "attention.bands08_r64_fit_v715"
...
ARMS = ("bands08_r64",)
REPLAY_TOL, COST_MAX, RHO_MIN, RATIO_MAX, VALUES_MAX = 0.002, 0.060, 0.8, 1.5, 33e6
PREDICTIONS = {"pred_a_native_replays": "+-0.002", "pred_b_cost_not_worse": "<= 0.060", "pred_c_values_preserved_rank": "Spearman >= 0.8",
               "pred_d_no_head_over_1p5": "max ratio <= 1.5", "pred_e_values_under_33M": "values <= 33M"}


def main() -> None:
    plan = {"candidate_id": CANDIDATE_ID, "forwards_max": FORWARDS_MAX, "model_backwards": BACKWARDS_MAX, "model_updates": 0, "fit_parameters": 100 * 4 * 16 * 1280 + 62 * 4 * 64 * 1280 + 162 * 513,
            "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "steps": STEPS, "lr": [LR, LR_MIN],
            "bars": {"replay_tol": REPLAY_TOL, "cost_max": COST_MAX, "rho_min": RHO_MIN, "ratio_max": RATIO_MAX, "values_max": VALUES_MAX}, "band": list(BAND), "arms": list(ARMS), "n_manip": N_MANIP}
    ...
    for hh in nhooks:
        hh.remove()
    pre.remove()
    disk_guard.guard_torch_save(saved, str(OUT_PT), "v715 fitted hybrids, bands 0-8 at rank 64 (endpoint)")
    M = results["bands08_r64"]; max_ratio = max(manip[k]["ratio"] for k in top)
    ...
    predictions = {"pred_a_native_replays": abs(native - NATIVE_V615) <= REPLAY_TOL, "pred_b_cost_not_worse": M["heldout_at_chosen"] <= COST_MAX,
                   "pred_c_values_preserved_rank": rho >= RHO_MIN, "pred_d_no_head_over_1p5": max_ratio <= RATIO_MAX, "pred_e_values_under_33M": M["values"] <= VALUES_MAX}
    if forwards > FORWARDS_MAX or backwards > BACKWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} fwd / {backwards} bwd")
    OUT.write_text(json.dumps({"schema": "attention_bands08_r64_fit_result_v715", "candidate_id": CANDIDATE_ID, "plan": plan, "report": {"native": native, "joint_value": JOINT_VALUE, "arms": results, "manipulability": manip, "spearman": rho, "median_ratio": med_ratio, "program_endpoint_cost": base_prog - native},
