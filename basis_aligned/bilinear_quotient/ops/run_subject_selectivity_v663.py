...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v663_result.json'
PREDICTIONS=dict(pred_a_exact_replay='baseline and independent joint versus shared chain replay max absolute error <=1e-4',
 ...
 pred_c_capable_position_transfer='native effect error<=.10/capability>=.9 eachcell; modal collateral<=.10 and modal prediction error<=.05 of target norm; target norm>=5x median of eight matched random edits')
...
def main():
    ...
    c=a and b and all(c['relative_l2']<=.10 and c['native_accuracy']>=.9 for c in cells.values()) and max(collateral_ratios)<=.10 and max(collateral_prediction_errors)<=.05 and specificity>=5
