# THE DEPLOYABLE VERSION -- covered-fit, stream input, map rank 512.
#
# §1875 overturned §1873 and §1874: an ORACLE linear map from the length-1 residual stream, at map rank
# 512, reaches +0.11388 / +0.14094 / +0.14196 against §1868's uncovered-token ceiling, against the
# deployed map's +0.78075 / +0.86225 / +0.83997. Rank, input and fitting COMPOUND, and 85% of the
# fallback loss is expressible after all -- the "83% representational" and "71% function class"
# conclusions were artifacts of holding map rank at 64.
#
# But 0.114 is an ORACLE: fitted on the rows it predicts. It bounds expressibility, not achievability.
# The deployable object fits on COVERED tokens only and applies to uncovered ones -- and the stream input
# is computable offline for any token, so nothing here needs the answers at inference.
#
# §1873 measured a 16.6% generalisation gap, but at map rank 64 on the EMBEDDING, and §1875 has just
# shown a rank-64 measurement says nothing about rank 512 on the stream. This measures it where it
# matters.
#
# ROLES. All three, deployed 5,419-type covered set. DISCOVERY ONLY. Rung 3: §1875's open question.
#
...
# ANCHORS name the build they came from per LESSONS 53's addendum -- §1875's pred_d failed by quoting a
# rank-64 map against a rank-512 arm, so every constant below states its rank.
#   pred_a MOST OF THE ORACLE GAIN SURVIVES A COVERED-ONLY FIT: the covered-fit stream map at rank 512
#          comes in below 0.35 nats on all three roles -- i.e. it keeps more than half the distance from
#          the deployed +0.78075 (rank 64, embedding, covered-fit) down to §1875's PUBLISHED oracle
#          +0.11388 (rank 512, stream, oracle-fit). If TRUE the fallback is a solved engineering problem
#          and the largest open lever in the class closes with a build that already exists in parts. If
#          FALSE the oracle gain is mostly unattainable transfer, and §1875's 85% is a bound nobody can
#          reach -- which would restore something like §1873's conclusion for the right reason rather
#          than the wrong one.
#   pred_b AND IT BEATS THE RANK-512 EMBEDDING MAP: below §1870's PUBLISHED rank-512 covered-fit
#          embedding figures of +0.59560 / +0.67209 / +0.67172, on all three roles. This isolates the
#          INPUT at matched rank and matched fitting set, which no run has yet done. If FALSE the stream
#          input buys nothing once rank is high, and §1874's input term was itself a rank-64 artifact.
#   pred_c AND THE COVERED ARM IS UNTOUCHED: covered CE identical to the deployed arm within 1e-6.
#          Seventh KNOWN-ANSWER check; the previous six each returned exactly 0.00e+00.
#   pred_d CONTROLS: coverage is exactly 5,419; the covered arm attains its ceiling (a sixteenth
#          confirmation); every uncovered scored token has a ceiling row; the MAP arm at rank 512
#          reproduces §1870's PUBLISHED rank-512 row within 0.005 -- stated at its rank this time.
...
OUT = PT + 'ops/deployable_stream_map_results.json'
...
MAPRANKS = (512,)
ARMS2 = ('MAP', 'STREAMCOV')          # None = full rank; §1786/§1813 use this ladder
...
def main():
    ...
    print(f'DEPLOYABLE STREAM MAP | 5419 types, covered-fit stream map at rank 512 | '
    ...
    def build(n, rank=None, map_rank=MAP_RANK):
        ...
        for st in sites:
            ...
            if BASE['mode'] == 'STREAMCOV':
                # ORACLE: refit the map on the UNCOVERED rows themselves. Not deployable -- it needs the
                # answers it predicts -- and used only to bound how much of the loss is transfer.
                Uo = tables[st].double()
                Xo = covstream[st].double()       # COVERED tokens' streams: the fit set
                Ao = (Xo.T @ Xo
                      + RIDGE * torch.eye(D, device=DEV, dtype=torch.float64)
                      * (Xo.shape[0] / D))
                Wo = torch.linalg.solve(Ao, Xo.T @ Uo)
                Uu, So, Vo = torch.linalg.svd(Wo, full_matrices=False)
                mpo = (Uu[:, :map_rank] * So[:map_rank]) @ Vo[:map_rank]
                # ORC['stream'] covers only the NU tokens that occur at uncovered scored
                # positions, not all ~44.8k uncovered vocabulary rows. Write the embedding-map
                # base everywhere, then overwrite exactly the rows that are ever read. Assigning
                # the [NU, D] block straight into fr[unc] would be a shape error.
                fr[unc] = (Eunc @ mp).float()
                fr[utoks] = (ORC['stream'][st].double() @ mpo).float()   # applied to uncovered
            else:
                fr[unc] = (Eunc @ mp).float()
            out[st] = fr
    ...
    pa = all(fbloss['STREAMCOV'][e] < 0.35 for e in roles2)
    pb = all(fbloss['STREAMCOV'][e] < S1870_MAP_R512[e] for e in roles2)
    pc = covspread <= 1e-6
    pd = (all(abs(fbloss['MAP'][e] - S1870_MAP_R512[e]) <= 0.005 for e in roles2)
          and NFULL == NCOV and attain)
    ...
    print(f'\n  most of the ORACLE gain survives a covered-only fit (<0.35) -> {pa}  ' + '  '.join(
        f'{e} {fbloss["STREAMCOV"][e]:+.5f}' for e in roles2),
        flush=True)
    print(f'  and it beats the rank-512 EMBEDDING map (§1870) -> {pb}  ' + '  '.join(
        f'{e} {fbloss["STREAMCOV"][e]:+.5f}' for e in roles2), flush=True)
    print(f'  and the COVERED arm is untouched (<1e-6) -> {pc}  max spread {covspread:.2e}',
          flush=True)
    print(f'  MAP arm reproduces §1872, coverage {NFULL}, attainment {attain} -> control {pd}',
          flush=True)

    json.dump({'run': 'deployable_stream_map', 'n_covered': NFULL, 'arms': list(ARMS2),
               'fallback_loss': fbloss, 'covered_ce': covce, 'generalisation_share': genshare,
               'covered_spread': covspread, 'ceiling_uncovered': uceilv,
               'uncovered_fraction': nfrac, 'attainment_exact': bool(attain),
               'predictions': {'pred_a_survives_covered_fit': bool(pa),
                               'pred_b_beats_embedding_at_rank512': bool(pb),
