"""IS THE FRONTIER'S MLP STAGE ONE JOB DONE TWICE, OR TWO JOBS? (Claude, LANE 1)

SS2877 dropped each half of the MLP stage separately and found both load-bearing and nearly equally so: removing the token lookup costs
**+0.6814 nats**, removing the rank-64 quadratic residual costs **+0.7536**, asymmetry only .0722. It explicitly flagged the quantity it
did not measure: **dropping BOTH**. If the halves are independent the joint cost is the sum, **1.4350**; if they are largely redundant
with one another -- each able to stand in for the other -- it lands near the worse single drop, **0.7536**. Nothing measured so far
distinguishes those, and the difference decides whether the MLP stage is two jobs or one job done twice.

Two arms of SS312's published norm-selection pipeline (BASELINE, and `tb := 0` AND `A := 0` together), applied at BOTH `tableres`
construction sites. SS2877's single-drop costs are read from its receipt under a frozen hash rather than retyped, so the additivity
arithmetic cannot drift from the numbers it is compared against.

SIGN CONVENTION (SS2135): frontier L2 is CE ADDED ABOVE THE REAL MODEL, so **LOWER IS BETTER** (SS312: +2.6735 beating +2.84/+2.93).
A drop COST is L2_F(dropped) - L2_F(baseline), **POSITIVE = WORSE**; a SUBADDITIVITY GAP is (sum of singles) - (joint), **POSITIVE =
the halves are redundant with each other**. SS2128/SS2129/SS2133/SS2134 RETRACTED; SS2125 STANDS.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_the_halves_are_not_additive
#                     pred_c_the_halves_are_redundant_with_each_other
#                     pred_d_dropping_both_is_worse_than_dropping_either
#                     pred_e_the_baseline_reproduces_S2877_exactly

Preregistration: polynomial_causal/FRONTIER_MLP_JOINT_DROP_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; two arms (baseline / BOTH MLP halves dropped)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_mlp_joint_drop_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
MLPMODE={'drop':None}   # None | 'res' | 'table' | 'both'
CVSNAP={}
SKIP8=64
...
def main():
    ...
    def fit_tableres(Y,X,ids,li):
        ...
        A=torch.linalg.solve(ft.T@ft+lam*torch.eye(ft.shape[1],device=DEV),
                             ft.T@(Rr@P))
        if MLPMODE['drop'] in ('res', 'both'):
            A = torch.zeros_like(A)          # keep the token table, drop the quadratic residual
        if MLPMODE['drop'] in ('table', 'both'):
            tb = torch.zeros_like(tb)        # keep the quadratic residual, drop the token table
...
PREREG = PT + '../polynomial_causal/FRONTIER_MLP_JOINT_DROP_PREREGISTRATION.md'
PREREG_SHA = "6d6a7c7e51e46a65db7f1796cccbc51c20ce12551134a2cd16d476012db8a995"
...
MLPRES = PT + 'frontier_mlp_table_vs_residual_results.json'
MLPRES_SHA = "7e58624a7c98bdc64eb383a6393a0b69f4ff15a5b711648fd6a9d7bc9dc9893e"
BARS = {"reproduce": 0.05, "nonadditive": 0.20, "subadditive": 0.20, "worse_than_either": 0.10, "repro_base": 0.001}
NULLS = {"additive_le": 0.10, "fully_redundant_le": 0.05}
...
if __name__=='__main__':
    ...
    if _sha(MLPRES) != MLPRES_SHA:
        raise RuntimeError(f'frozen hash mismatch: {MLPRES}')
    prev = json.load(open(MLPRES))['summary']
    c_tab, c_res = prev['cost_dropping_table_nats'], prev['cost_dropping_residual_nats']
    SEL['mode']='norm'

    print('ARM 1/2: BASELINE -- reproduction of the published frontier',flush=True)
    MLPMODE['drop']=None
    r_base=main()

    print('ARM 2/2: BOTH halves dropped (tb := 0 AND A := 0) -- the MLP stage writes nothing',flush=True)
    MLPMODE['drop']='both'
    r_both=main()
    MLPMODE['drop']=None

    # SIGN: L2 is CE ADDED ABOVE THE REAL MODEL, LOWER IS BETTER -> cost POSITIVE = WORSE
    cost_both = r_both['L2_F'] - r_base['L2_F']
    additive  = c_tab + c_res
    gap       = additive - cost_both            # POSITIVE = SUBadditive, i.e. the halves are redundant
    over_max  = cost_both - max(c_tab, c_res)
    repro     = abs(r_base['L2_F'] - 2.6735)

    pa = repro <= BARS['reproduce']
    pb = abs(gap) >= BARS['nonadditive']
    pc = gap >= BARS['subadditive']
    pd = over_max >= BARS['worse_than_either']
    pe = abs(r_base['L2_F'] - prev['L2_F_baseline']) <= BARS['repro_base']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_halves_are_not_additive':bool(pb),
           'pred_c_the_halves_are_redundant_with_each_other':bool(pc),
           'pred_d_dropping_both_is_worse_than_dropping_either':bool(pd),
           'pred_e_the_baseline_reproduces_S2877_exactly':bool(pe)}
    nulls={'b_null_the_halves_are_additive':bool(abs(gap)<=NULLS['additive_le']),
           'c_null_the_halves_are_fully_redundant':bool(over_max<=NULLS['fully_redundant_le'])}
    res={'rung':'frontier_mlp_joint_drop','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,
         'baseline':r_base,'both_dropped':r_both,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(r_base['L2_F'],4),
                    'L2_F_both_dropped':round(r_both['L2_F'],4),
                    'cost_both_nats':round(cost_both,4),
                    'cost_table_only_S2877':round(c_tab,4),
                    'cost_residual_only_S2877':round(c_res,4),
                    'additive_prediction_nats':round(additive,4),
                    'subadditivity_gap_nats':round(gap,4),
                    'excess_over_worse_single_nats':round(over_max,4),
                    'L2_C_baseline':round(r_base['L2_C'],4),
                    'L2_C_both_dropped':round(r_both['L2_C'],4)},
         'price':{'gpu_forwards':0,'forwards_instrumented':False,'pipeline_runs':2,
                  'backwards':0,'fitted_parameters':0,
                  'gpu_seconds':round(time.time()-T00,1)},
         'hashes':{PREREG:PREREG_SHA,MLPRES:MLPRES_SHA},'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {r_base['L2_F']} vs 2.6735 (tol .05): {'HELD' if pa else 'FAILED'}")
    print(f"(b) |additive {additive:.4f} - both {cost_both:.4f}| = {abs(gap):.4f} >= {BARS['nonadditive']}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) subadditivity gap {gap:+.4f} >= {BARS['subadditive']}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) excess over the worse single drop {over_max:+.4f} >= {BARS['worse_than_either']}: {'HELD' if pd else 'FAILED'}")
