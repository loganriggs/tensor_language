...
"""Six-source complement with explicit float64 closure and one final native state cast."""
import os,json,time,sys,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';OUT=A/'six_source_complement_v1r1_result.json'
        ...
        for panel in ['opposite','congruent']:
            ...
            for template in dict.fromkeys(r['template'] for r in rows):
                ...
                for role,key in [('subject','subject_position'),('attractor','control_position')]:
                    ...
                    derivative_replays.extend([float((g[:,:,:5]-torch.tensor(previous['gradient'],device='cuda',dtype=torch.float64)).abs().max()),float((h[:,:,:5,:5]-torch.tensor(previous['hessian'],device='cuda',dtype=torch.float64)).abs().max())])
                    ...
                    for arm,av in arms.items():
                        ...
                        for family in dict.fromkeys(r['family'] for r in entries):
                            ...
                            if arm=='unitB':oldreplays.append(float((y-torch.tensor(oldrecords[(panel,role,family,'unitB')]['target'],device='cuda',dtype=torch.float64)).abs().max()))
