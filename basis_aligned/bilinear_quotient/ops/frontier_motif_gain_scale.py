"""DOES THE MISMATCH REACH THE MOTIF HEADS? SCALING THEIR PER-HEAD GAINS (Claude, LANE 1)

SS2902 broadened SS2900's claim: the local/end-to-end mismatch is not confined to ridge fits but comes from choosing a component by ANY
local criterion while scoring the construction end-to-end. Two blocks now carry adopted rescalings -- tail link maps x0.25 (SS2896,
-0.2287) and norm-selected CP units x0.5 (SS2902, -0.1075).

**The motif heads are the largest block not yet tested this way** (+0.3988, SS2882). Their per-head gains `ALPHA[li] = (ap, asf)` are
...
here too. If it does not, the claim is narrower than SS2902 states and the difference between the blocks that respond and those that do
not becomes the thing to explain.

SS2882 supplies a same-operation anchor: it measured `ML := []` (motif heads off, attention 2-9 real) at an error share of **+0.3988**,
i.e. a COST of **-0.3988** relative to the frontier -- note the sign, since restoring real attention there IMPROVED the frontier. That
arm is included.
...
SS2125 STANDS -- this rescales fitted gains; it neither selects nor reorders.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_the_motif_off_arm_reproduces_S2882
#                     pred_c_scaling_the_motif_gains_improves_the_frontier
#                     pred_d_the_arms_are_connected
#                     pred_e_both_windows_are_reported

Preregistration: polynomial_causal/FRONTIER_MOTIF_GAIN_SCALE_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run: baseline, motif heads off, and five ALPHA scales")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_motif_gain_scale_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_MOTIF_GAIN_SCALE_PREREGISTRATION.md'
PREREG_SHA = "2552387dbec30758449d79068fba679cb834e56a0219b8af61a14755495a64de"
...
if __name__=='__main__':
    ...
    if _sha(S2882) != S2882_SHA:
        raise RuntimeError(f'frozen hash mismatch: {S2882}')
    a82 = json.load(open(S2882))['summary']['error_share_motif_heads_nats']   # +0.3988 (motif off)
    SEL['mode']='norm'

    mk = lambda s: f'M{int(round(s*100)):03d}'
    ARMS[:] = ([('baseline', {}), ('motif_off', {'motif': []})]
               + [(mk(s), {'motif_scale': s}) for s in SCALES])
    ...
    c_off = round(out['motif_off']['L2_F'] - bF, 4)
    cF = {s: round(out[mk(s)]['L2_F'] - bF, 4) for s in SCALES}
    cC = {s: round(out[mk(s)]['L2_C'] - bC, 4) for s in SCALES}
    # SS2882's share is baseline - restored, so the motif_off arm's COST should be the negative of it
    anchor_target = -a82
    anchor_dev = abs(c_off - anchor_target)
    ...
    pc = len(below) >= 1
    pd = abs(c_off) >= BARS['connected']
    pe = True
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_motif_off_arm_reproduces_S2882':bool(pb),
           'pred_c_scaling_the_motif_gains_improves_the_frontier':bool(pc),
    ...
    nulls={'b_null_the_anchor_fails':bool(anchor_dev>=NULLS['anchor_ge']),
           'c_null_the_motif_gains_are_already_optimal':bool(len(below)==NULLS['no_improvement'])}
    res={'rung':'frontier_motif_gain_scale','preds':preds,'nulls':nulls,
         ...
         'summary':{'L2_F_baseline':round(bF,4),'L2_C_baseline':round(bC,4),
                    'cost_motif_off':c_off,'S2882_share':round(a82,4),
                    ...
                    'S2896_tail':-0.2287,'S2902_cp':-0.1075},
         ...
         'hashes':{PREREG:PREREG_SHA,S2882:S2882_SHA},'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {bF:.4f} vs 2.6735: {'HELD' if pa else 'FAILED'}")
    print(f"(b) motif off {c_off:+.4f} vs SS2882-implied {anchor_target:+.4f}, dev {anchor_dev:.4f}: {'HELD' if pb else 'FAILED'}")
