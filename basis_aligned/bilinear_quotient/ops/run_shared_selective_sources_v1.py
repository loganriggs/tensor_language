...
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';OUT=A/'shared_selective_sources_v1_result.json'
...
def main():
    ...
    for context in binding['contexts']:
        ...
        for role in ['subject','attractor']:
            pos,ds=c['source_components'][role];zero=torch.zeros(len(ds),6,device='cuda',dtype=torch.float64)
            def evaluate(a):
            ...
            old=next(v for v in prior['contexts'] if v['panel']==panel and v['template']==template and v['role']==role)
            replays.append(float((g[:,:4]-torch.tensor(old['gradient'],device='cuda',dtype=torch.float64)).abs().max()))
            v=torch.tensor([.3,-.2,.7,-.4,.5,.6],device='cuda',dtype=torch.float64).expand_as(zero)
    ...
    for context,c in cache:
        ...
        dspos=c['source_components']['subject'];baseline=native(dspos[1],dspos[0],torch.zeros(n,6,device='cuda',dtype=torch.float64))
        for role in ['subject','attractor']:
    ...
    instrument=counts==PLAN and max(replays)<=1e-8 and max(fdchecks)<=1e-3
