...
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';OUT=A/'native_quadratic_budgeted_v1_result.json'
...
def main():
    ...
    frozen=json.loads((P/'BUDGETED_MODAL_DIRECTIONS_V1.json').read_text());lookup_directions={(c['dataset'],c['panel'],c['role'],c['template'],c['kappa']):c for c in frozen['contexts']}
        ...
        for panel in ['opposite','congruent']:
            ...
            for template in dict.fromkeys(r['template'] for r in rows):
                ...
                for role,key in [('subject','subject_position'),('attractor','control_position')]:
                    ...
                    for kappa in [.05]:arms['budget'+str(kappa)]=torch.tensor(lookup_directions[(dataset,panel,role,template,kappa)]['amplitudes'],device='cuda',dtype=torch.float64)
