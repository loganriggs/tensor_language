# WHERE IS THE TABLE CROSSOVER?  -- one compiled site wants context, thirty-six want length-1.
#
# §1845: compiling ONE site into a live stream, the fit-context table beats the length-1 row by up to
# 43.5pp (mlp5, 3.6% -> 47.1%), on three held-out roles, at identical storage.
# §1846: compiling ALL 36, the same swap makes the program WORSE -- 13.55% -> 13.19% uniformly, 12.89%
# under the greedy per-site choice -- and §1765 already contains the derivation. A length-1 forward
# computes exactly the pure-token function the fully compiled program implements, so the length-1 table
# is its self-consistent FIXED POINT; a context table is a better predictor of the site in the MODEL and
# an off-manifold one in the PROGRAM.
#
# Both endpoints are now understood. §1832 established that partial compilation is a smooth
# dose-response between them with no frontier. So the practically useful question is where the two
# regimes cross, because that tells a HYBRID program which table to use -- and unlike §1846 it has a
# real chance of a positive answer over a wide range.
#
# This compares the two tables at compile depths B1, B3 and B5 (0..L compiled, L+1..17 live) against the
# all-36 result §1846 already published. The context table is applied at every compiled site in layers
# 1-6, exactly as §1846 applied it.
#
# ROLES. All three. FULL RANK. §1824's sequential gain correction. DISCOVERY ONLY.
#
...
#   pred_a THE CROSSOVER EXISTS: at B1 the context table beats the length-1 table by at least 2pp of gap
#          fraction. If TRUE there is a regime of partial compilation where the context table is the
#          right choice, and a hybrid program has a table decision worth making. If FALSE the context
#          table is a liability the moment more than one site is compiled, §1845's gain exists ONLY for a
#          single site, and the deployed length-1 choice is correct everywhere that matters.
#   pred_b AND IT IS MONOTONE IN DEPTH: the context-table advantage is non-increasing from B1 to B3 to
#          B5 to all-36 (allowing 2pp of slack per step). If FALSE the crossover is not a smooth function
#          of how much is compiled, and no single depth threshold describes it -- which would matter
#          because §1832 found the underlying damage IS a smooth dose-response, and a non-monotone table
#          advantage on top of it would need its own account.
#   pred_c AND THE CROSSOVER IS EARLY: the context table already HURTS at B5, i.e. its advantage there is
#          negative. If FALSE it still helps at B5 -- with 12 of 36 sites compiled -- and the liability
#          appears only near full compilation, which would make the context table the right default for
#          every hybrid program in the record and would be the most useful outcome available here.
#   pred_d CONTROLS, cross-run per LESSON 42: B0 reproduces §1829's PUBLISHED 64.8%; the LENGTH-1 arms at
#          B1, B3 and B5 reproduce §1829's PUBLISHED 25.9%, 11.9% and 12.3% within 0.03; ALL_L1
#          reproduces §1789's PUBLISHED full-rank top-1; FIT_mlp5 reproduces §1845's PUBLISHED 47.1%.
...
OUT = PT + 'ops/table_crossover_results.json'
...
PROBES = EARLY
DEPTHS = (1, 3, 5)
ARMS = {'B0': list(B0_SITES), 'FIT_mlp5': B0_SITES + [('mlp', 5)]}
for _L in DEPTHS:
    ARMS[f'B{_L}_L1'] = [st for st in [(k2, L2) for L2 in range(18) for k2 in ('attn', 'mlp')]
                         if st[1] <= _L]
    ARMS[f'B{_L}_FIT'] = list(ARMS[f'B{_L}_L1'])
# §1829's PUBLISHED sequential gap fractions for the bottom-up arms, skip7000
S1829_BOTUP = {'B1_L1': 0.259, 'B3_L1': 0.119, 'B5_L1': 0.123}
...
def main():
    ...
    print(f'TABLE CROSSOVER | length-1 vs fit-context at compile depths B1/B3/B5/all | '
    ...
    b0 = seq['B0']
    prog = {nm3: {e: res[e][nm3]['top1'] for e in roles} for nm3 in ('ALL_L1', 'ALL_FIT')}
    gapv0 = {e: res[e]['live_model']['top1'] - res[e]['all_substituted']['top1'] for e in roles}
    adv = {f'B{L2}': seq[f'B{L2}_FIT'] - seq[f'B{L2}_L1'] for L2 in DEPTHS}
    adv['ALL'] = (prog['ALL_FIT'][PICK_ROLE] - prog['ALL_L1'][PICK_ROLE]) / gapv0[PICK_ROLE]
    order = [f'B{L2}' for L2 in DEPTHS] + ['ALL']
    pa = adv['B1'] >= 0.02
    pb = all(adv[order[i]] <= adv[order[i - 1]] + 0.02 for i in range(1, len(order)))
    pc = adv['B5'] < 0.0
    pd = (abs(b0 - S1829_B0_GAPFRAC) <= 0.03
          and all(abs(seq[k2] - v) <= 0.03 for k2, v in S1829_BOTUP.items())
    ...
    print(f'\n  gap fraction by compile depth, {PICK_ROLE} (B0 {b0:.1%}, one site FIT_mlp5 '
          f'{seq["FIT_mlp5"]:.1%}):', flush=True)
    for L2 in DEPTHS:
        print(f'    B{L2}   length-1 {seq[f"B{L2}_L1"]:7.1%}   fit-context {seq[f"B{L2}_FIT"]:7.1%}'
              f'   advantage {adv[f"B{L2}"]:+7.1%}', flush=True)
    print(f'    ALL  length-1 {prog["ALL_L1"][PICK_ROLE]:7.2%}   fit-context '
          f'{prog["ALL_FIT"][PICK_ROLE]:7.2%}   advantage {adv["ALL"]:+7.1%}  (top-1, §1846)',
          flush=True)
    print(f'\n  the CROSSOVER exists (context wins at B1 by >=2pp) -> {pa}  '
          f'B1 advantage {adv["B1"]:+.1%}', flush=True)
    print(f'  and it is MONOTONE in depth -> {pb}  ' + '  '.join(
        f'{k3} {adv[k3]:+.1%}' for k3 in order), flush=True)
    print(f'  and the crossover is EARLY (context already hurts at B5) -> {pc}  '
          f'B5 advantage {adv["B5"]:+.1%}', flush=True)
    print(f'  B0/B1/B3/B5 §1829, ALL_L1 §1789, FIT_mlp5 §1845 -> control {pd}', flush=True)

    json.dump({'run': 'table_crossover', 'role': PICK_ROLE, 'depths': list(DEPTHS),
               'sequential_gapfrac': seq, 'whole_program_top1': prog,
               'context_advantage': adv, 'rows_changed': ndiff, 'gap_fraction': frac,
               'predictions': {'pred_a_crossover_exists': bool(pa),
                               'pred_b_monotone_in_depth': bool(pb),
                               'pred_c_crossover_is_early': bool(pc),
