"""REGISTERED FRONTIER CLAIM: RUNG-127 + OV RESIDUAL (rung 179).

CONVENTION (S2135): frontier L2 = CE ADDED ABOVE THE REAL MODEL on the S312 fresh windows; LOWER IS BETTER.
Rung 178 printed DESCRIPTIVELY L2_F +1.8222 / census +1.8950 for the rung-127 build plus rank-8 OV
residuals. Per the rung-123 precedent, the claim needs registration before the scoring run: identical
deterministic rebuild, claim bars as predictions. If held, the quotable 47.8M-class Pareto point moves
+1.8765 -> +1.8222 at +155,648 values.

REGISTERED PREDICTIONS (anchors: rung-178 receipt):
  (a) NEW POINT: L2_F fresh <= 1.84.
  (b) REPRODUCTION: |census - 1.8950| <= 0.015.
  (c) REPRODUCTION: |L2_F - 1.8222| <= 0.015 AND increment in [0.25, 0.45].
NULL: the 178 print was noise (L2_F >= 1.86). PRICE: 47.8M front + 155,648 OV values. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: registered claim rung-127 + OV residual')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_claim_ovres_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['ov_res']=True
    ...
    pa=L2F<=1.84
    pb=abs(agg-1.8950)<=0.015
    pc=abs(L2F-1.8222)<=0.015 and 0.25<=inc<=0.45
    ...
    print(f"(a) L2F {L2F:+.4f} <= 1.84: {'HELD' if pa else 'FAILED'}")
    print(f"(b) |census {agg:+.4f} - 1.8950| <= 0.015: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |L2F - 1.8222| <= 0.015, increment in band: {'HELD' if pc else 'FAILED'}")
