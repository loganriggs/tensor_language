#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_instrument pred_b_budget05_selectivity pred_c_prediction
"""Matched-strength native selective intervention control; frozen derivatives."""
import os,json,time,sys,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';OUT=A/'native_budgeted_modal_v1_result.json'
PREDICTIONS=dict(pred_a_instrument='oldunitB/null anddouble effectreplay1e-4;counts',pred_b_budget05_selectivity='budget05 alignedretention>=80%unitB andmodalratio<=10% all48cells',pred_c_prediction='bothbudgetarms quadratic number10%/modal5%numberbudget everycell')
...
def main():
        ...
        for panel in ['opposite','congruent']:
            ...
            for template in dict.fromkeys(r['template'] for r in rows):
                ...
                for role,key in [('subject','subject_position'),('attractor','control_position')]:
                    ...
                    for kappa in [.05,.1]:arms['budget'+str(kappa)]=torch.tensor(lookup_directions[(dataset,panel,role,template,kappa)]['amplitudes'],device='cuda',dtype=torch.float64)
    ...
    primary=[c for c in comparisons if c['arm']=='budget0.05'];assert len(primary)==48
