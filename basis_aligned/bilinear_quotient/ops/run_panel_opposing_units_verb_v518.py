#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_four_units_help_the_panel_verb_margin pred_c_pronoun_opposition_replays pred_d_random_units_do_not pred_e_verb_and_pronoun_effects_opposite
...
OUT = ROOT / "circuits/followups/panel_opposing_units_verb_v518_result.json"
CANDIDATE_ID = "chain.panel_opposing_units_verb_v518"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, VERB_MIN, PRON_V516, BAND, RANDOM_MAX = 1e-3, 0.03, -0.078, 0.02, 0.01
import random
import random
...
FORWARDS_MAX = 10
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_four_units_help_the_panel_verb_margin": ">= 0.03", "pred_c_pronoun_opposition_replays": "-0.078 +- 0.02", "pred_d_random_units_do_not": "|.| <= 0.01 x 2", "pred_e_verb_and_pronoun_effects_opposite": "opposite signs"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "verb_min": VERB_MIN, "pron_v516": PRON_V516, "band": BAND, "random_max": RANDOM_MAX}}
    ...
    def logits_margin(x):
        ...
        return torch.stack([z[:, THEY] - z[:, HE], (z[:, ARE] - z[:, IS]) + (z[:, WERE] - z[:, WAS]), z[:, HE] - z[:, SHE]], 1), z   # column 1 is the agreement margin here
    ...
    c = verb["closed"]
    v = {m: c[m]["verb"] for m in c}; agr = {m: c[m]["are_is"] for m in c}   # "verb" = the they - he column here, "are_is" = the agreement column
    report["pronoun_closed"] = v; report["agreement_closed"] = agr
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_four_units_help_the_panel_verb_margin": agr["DET_A"] >= VERB_MIN, "pred_c_pronoun_opposition_replays": abs(v["DET_A"] - PRON_V516) <= BAND,
                   "pred_d_random_units_do_not": abs(v["RANDOM_A"]) <= RANDOM_MAX and abs(agr["RANDOM_A"]) <= RANDOM_MAX, "pred_e_verb_and_pronoun_effects_opposite": agr["DET_A"] * v["DET_A"] < 0}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "panel_opposing_units_verb_result_v518", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
