#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_8_1_carries pred_c_blocks_7_8_carry_most_of_low pred_d_10_5_small pred_e_low_groups_add
"""The person line by head (v580). v579: on 43 natural person pairs every head's value at the cue carries 0.992 of the myself - yourself gap, the four
18 Sep readers {8.1, 13.1, 10.5, 15.1} 0.361, blocks 0-8 0.637, blocks 9-17 0.431. Here the four readers singly, and blocks 0-8 in three groups (0-4, 5-6,
7-8), values at the cue swapped.
PREDICTIONS (scored as written; failures preserved; priors from v579 and the 18 Sep person account (8.1 a token-only reader of I / you))
    pred_a_baseline_replays          the unedited manual forward replays the model's myself - yourself margins within 1e-3
    pred_b_8_1_carries               8.1 alone closes >= 0.15
    pred_c_blocks_7_8_carry_most_of_low  blocks 7-8 close >= 0.30 (half of the low blocks' 0.637)
    pred_d_10_5_small                10.5 alone closes <= 0.10 (its input was diffuse, 18 Sep)
    pred_e_low_groups_add            B04 + B56 + B78 is within 0.10 of 0.637
...
OUT = ROOT / "circuits/followups/person_heads_and_low_groups_v580_result.json"
CANDIDATE_ID = "person.heads_and_low_groups_v580"
ROWS = ROOT / "circuits/followups/person_reflexive_dod_natural_rows_v110.json"
N_HEAD = 9
FIVE = {8: [1], 13: [1], 10: [5], 15: [1]}   # the four person readers
REPLAY_TOL, H81_MIN, B78_MIN, H105_MAX, LOW_V579, ADD = 1e-3, 0.15, 0.30, 0.10, 0.637, 0.10
FORWARDS_MAX = 17
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_8_1_carries": ">= 0.15", "pred_c_blocks_7_8_carry_most_of_low": ">= 0.30", "pred_d_10_5_small": "<= 0.10", "pred_e_low_groups_add": "0.637 +- 0.10"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "text_pairs": len(text_items), "rows": str(ROWS.name), "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "h81_min": H81_MIN, "b78_min": B78_MIN, "h105_max": H105_MAX, "low_v579": LOW_V579, "add": ADD}}
    ...
    MODES = {"8.1": ({}, {8: [1]}, False), "13.1": ({}, {13: [1]}, False), "10.5": ({}, {10: [5]}, False), "15.1": ({}, {15: [1]}, False), "B04": ({}, {l: list(range(N_HEAD)) for l in range(0, 5)}, False), "B56": ({}, {l: list(range(N_HEAD)) for l in (5, 6)}, False), "B78": ({}, {l: list(range(N_HEAD)) for l in (7, 8)}, False)}; forwards = 0
    def logits_margin(x):
    ...
    c = text["closed"]
    print("person by head / low group", {k: round(v, 3) for k, v in c.items()}, "readers sum", round(c["8.1"] + c["13.1"] + c["10.5"] + c["15.1"], 3), "low groups sum", round(c["B04"] + c["B56"] + c["B78"], 3))
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_8_1_carries": c["8.1"] >= H81_MIN, "pred_c_blocks_7_8_carry_most_of_low": c["B78"] >= B78_MIN,
                   "pred_d_10_5_small": abs(c["10.5"]) <= H105_MAX, "pred_e_low_groups_add": abs(c["B04"] + c["B56"] + c["B78"] - LOW_V579) <= ADD}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "person_heads_and_low_groups_result_v580", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
