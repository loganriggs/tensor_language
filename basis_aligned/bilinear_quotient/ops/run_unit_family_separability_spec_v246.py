...
"""v246: within-family separability for the two v253 four-row passes, each under its FULL standing family, plus the re-check of every counted
member of the two grown families (v212/v216/v220/v222 rule, standing until board proposal (v)/(vii) is answered). Family membership was
settled from the v244 receipt (18/18 separable: verb_preposition now 6 counted, adjective_preposition 6 counted) and the v253 receipt BEFORE
authoring. v253 went 2/8: three row-4 misses (verb_preposition_at_from C 0.0125, verb_preposition_on_with 0.0206, verb_preposition_to_about
0.0121 -- all 1101) JOIN the verb_preposition control set as three-row members (standing rule); the three row-2 misses (in_with 0.70,
between_with 0.80 held dirA below the bar, from_with 0.67 -- all 0111) do NOT join any control set. noun_preposition does not grow (between_with
missed row 2; noun_preposition_reason is a v225 row-2 miss and is not a control) and is not exercised here.

Families here (ALL members prepared as controls; only EVAL evaluated):
  verb_preposition (6 counted after v244: verb_preposition (on/to), verb_preposition_insisted (of/on), verb_preposition_to_at,
    verb_preposition_with_about, verb_preposition_of_with, verb_preposition_from_for; three-row controls at_from, on_with, to_about)
    + verb_preposition_on_about (depended/complained -> on/about; 10 units 06:03/13:08/07:08/08:08/11:03; held 0.88, dim 0.598, cdas 0.927, C 0.0096, cext 0.991)
  adjective_preposition (6 counted after v244: fond, similar (to/from), identical_distinct (to/from), of_at, to_of, for_of; control same_different_preposition)
    + adjective_preposition_in_about (was interested/curious -> in/about; 11 units 13:08/08:08/06:03/07:08/11:03; held 0.82, dim 1.222, cdas 1.431, C -0.0003, cext 1.007)
Both new cells sit on the same hub set (06:03/07:08/08:08/11:03/13:08) as the 23 preposition cells that came out separable in v222/v240/v244
(max sibling 0.034). on_about shares its ` about` readout with with_about and its ` on` readout with verb_preposition (on/to) and insisted (of/on);
in_about shares ` about` with nothing in its family and ` in` with nothing counted. Shared-readout precedent: v236's leak (it_them 0.10/0.08)
for fusion, v226/v244 (with_about vs insisted 0.005) against. The three row-4-failed controls are the first verb_preposition controls with
C UB975 > 0.01: their A1 FIT rows enter the fam arm as controls, and a counted member losing separability ONLY under them would be a
control-set effect -- it is retracted all the same (v222 rule) and reported to the board.
Recipe as v198/.../v244: per evaluated member, arm `own` (own C_fit control) and arm `fam` (own C_fit + EVERY sibling's A1 FIT prep,
control_weight 30 x len(controls)); evaluated on the member's held A1 (extraction), each sibling's held A1 (|CE| leak) and held C.
Units frozen from the parent receipts (v253 for the new members; earlier batteries for the counted ones).
...
EVAL = 2 new members + 12 counted re-checks (14 fits x 2 arms; verb_preposition carries 9 controls, adjective_preposition 7: ~20-25 min).
Count rule (v242): a new member separable from every sibling +1; a re-checked counted member that fails is RETRACTED (-1).
86 (after v244) -> up to 88 from this rung.

REGISTERED BEFORE THE RUN (each coded predicate is the sentence here; bars in BARS):
    pred_a_new_separable   at least 1 of the 2 new members is separable (k_new). Expected: both (v240/v244 precedent 9/9 on these hubs);
                           the fusion risk is on_about against with_about / verb_preposition (two shared readouts).            prior 70%
    pred_b_counted_kept    at least 10 of the 12 re-checked counted members remain separable (k_kept; v244 kept 13/13, v240 5/5). prior 65%
    pred_c_row4_kept       fam-arm own-C UB975 <= 0.01 on at least 7 of the 14 evaluated members (k_d; v244 15/18 at <= 0.01; v240 9/12).                                                             prior 65%
...
Smoke: V246_SMOKE=<out.json> (CPU, V246_SMOKE_ROWS=4; all members are prepared as controls, V246_SMOKE_NAMES=verb_preposition_on_about,verb_preposition
are evaluated so a new member under the 9-control verb family, a re-check and the instrument read are exercised; steps 5).
BARS = {"cross_max": 0.05, "keep": 0.80, "ext_floor": 0.50, "c_ub_max": 0.01, "k_d": 7, "k_new": 1, "k_kept": 10, "instr_tol": 0.03}
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v246_result.json"
...
FAMILIES = {
    "verb_preposition": ["verb_preposition", "verb_preposition_insisted", "verb_preposition_to_at", "verb_preposition_with_about", "verb_preposition_of_with", "verb_preposition_from_for", "verb_preposition_on_about", "verb_preposition_to_about", "verb_preposition_at_from", "verb_preposition_on_with"],
...
EVAL = ("verb_preposition_on_about", "adjective_preposition_in_about", "verb_preposition", "verb_preposition_insisted", "verb_preposition_to_at", "verb_preposition_with_about", "verb_preposition_of_with", "verb_preposition_from_for", "adjective_preposition_fond", "adjective_preposition_similar", "identical_distinct_preposition", "adjective_preposition_of_at", "adjective_preposition_to_of", "adjective_preposition_for_of", )
COUNTED = ("verb_preposition", "verb_preposition_insisted", "verb_preposition_to_at", "verb_preposition_with_about", "verb_preposition_of_with", "verb_preposition_from_for", "adjective_preposition_fond", "adjective_preposition_similar", "identical_distinct_preposition", "adjective_preposition_of_at", "adjective_preposition_to_of", "adjective_preposition_for_of", )
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES, **v213.NAMES, **v215.NAMES, **v217.NAMES, **v219.NAMES, **v221.NAMES, **v223.NAMES, **v225.NAMES, **v227.NAMES, **v229.NAMES, **v231.NAMES, **v233.NAMES, **v235.NAMES, **v237.NAMES, **v239.NAMES, **v241.NAMES, **v243.NAMES, **v245.NAMES, **v247.NAMES, **v249.NAMES, **v251.NAMES, **v253.NAMES}


def _parent_cdas(n):
    # the parent battery receipt that produced this member's units (each behaviour name is unique across batteries)
    for path in (V253, V251, V249, V247, V245, V243, V241, V239, V237, V235, V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v246", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V246_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V246_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v246", "candidate_id": "corpus.unit_family_separability_spec_v246",
              "separable_counts": nsep, "families": FAMILIES, "summary": summary, "members": R, "bars": BARS,
              "recipe": {"lambda": LAM, "steps": steps, "lr": LR, "complement_weight": CW, "rank": 1, "seed": 0, "units_source": "v197 ... v253 battery receipts (later receipts override earlier ones)", "evaluated": list(EVAL),
