#!/usr/bin/env python
"""early_frame_drift_rank_probe -- is the change of read frame from one early sublayer to the next LOW-DIMENSIONAL? Each of the 22
early sites uses its predecessor's own 768-core with m directions swapped for its own top complement directions (m = 0 / 32 / 64 / 128),
vs its own core (§2753). CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_carry_over_costs pred_c_drift_is_low_rank_64 pred_d_drift_is_low_rank_128
#                     pred_e_drift_32
...
Preregistration: polynomial_causal/EARLY_FRAME_DRIFT_RANK_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/early_frame_drift_rank_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("early_frame_drift_rank_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "EARLY_FRAME_DRIFT_RANK_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "early_frame_smoothness_probe_results.json"   # §2753
OUT = ROOT / "early_frame_drift_rank_probe_results.json"
HASHES = {PREREG: "d4102fe4ca2dc8ee45e5a62a0d7d67b371d8156d5f58d006eeba36efb143eba0", PRIOR: "222af24d46002734229665bed56bfc5baad3d083bd39fa25e777dcb8c4b147c1",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "early_frame_drift_rank_probe"
...
K = 768; EARLY = list(range(0, 11)); LATE = list(range(11, 18)); MS = [0, 32, 64, 128]
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_min": 0.03, "c_max": 0.010, "d_max": 0.005, "e_max": 0.020}
NULLS = {"b_max": 0.01, "c_min": 0.040, "d_min": 0.020, "e_min": 0.050}
...
def main():
    ...
    def patched_core(s_, U_p, m_):
        C = heads[s_]["Cx"].to(U_p.dtype); U_p = U_p[:, :K]
        if m_ == 0:
            return U_p
        e = torch.einsum("dj,dk,kj->j", U_p, C, U_p)
        keep = U_p[:, torch.argsort(e, descending=True)[:K - m_]]
        Pc = torch.eye(D, device=U_p.device, dtype=U_p.dtype) - U_p @ U_p.T
        _, Uc = torch.linalg.eigh(Pc @ C @ Pc); Uc = Uc.flip(1)[:, :m_]
        return torch.cat([keep, Uc], 1)
    seq = [(kd, l) for l in EARLY for kd in ("attn", "mlp")]
    arms = {"EARLY22_OWN_768": run({s_: head(s_, K) for s_ in ESITES})}
    ortho = {}
    for m_ in MS:
        patch = {seq[0]: head(seq[0], K)}; worst = 0.0
        for i in range(1, len(seq)):
            U_s = patched_core(seq[i], heads[seq[i - 1]]["U"], m_); patch[seq[i]] = head(seq[i], K, U_s)
            worst = max(worst, float((U_s.T @ U_s - torch.eye(K, device=U_s.device, dtype=U_s.dtype)).abs().max()))
        ortho[m_] = worst
        arms[f"EARLY22_{'PREV' if m_ == 0 else f'PATCH{m_}'}_768"] = run(patch)
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    gaps = {f"{nm}_minus_own": arms[f"EARLY22_{nm}_768"] - arms["EARLY22_OWN_768"] for nm in ("PREV", "PATCH32", "PATCH64", "PATCH128")}
    ...
    preds = {
        ...
        'pred_b_carry_over_costs': bool(gaps["PREV_minus_own"] >= BARS["b_min"]),
        'pred_c_drift_is_low_rank_64': bool(gaps["PATCH64_minus_own"] <= BARS["c_max"]),
        'pred_d_drift_is_low_rank_128': bool(gaps["PATCH128_minus_own"] <= BARS["d_max"]),
        'pred_e_drift_32': bool(gaps["PATCH32_minus_own"] <= BARS["e_max"]),
    }
    nulls = {"b_null_le_.01": bool(gaps["PREV_minus_own"] <= NULLS["b_max"]), "c_null_ge_.04": bool(gaps["PATCH64_minus_own"] >= NULLS["c_min"]),
             "d_null_ge_.02": bool(gaps["PATCH128_minus_own"] >= NULLS["d_min"]), "e_null_ge_.05": bool(gaps["PATCH32_minus_own"] >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "k": K, "ms": MS, "orthonormality_max_dev": ortho,
