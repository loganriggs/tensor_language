...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; c8+c9@288 and @144 arms on the best x eight windows (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_c89_288_results.json'
...
if __name__=='__main__':
    ...
    r_b=main()
    SEL['K69MAP']={8:288,9:288}
    print('ARM 3/4: c8+c9@288, c6/c7 at 576',flush=True)
    r_2=main()
    SEL['K69MAP']={8:144,9:144}
    print('ARM 4/4: c8+c9@144 (descriptive floor)',flush=True)
    r_1=main()
    import statistics as stt
    db=[round(a-b,4) for a,b in zip(r_b['fresh8'],r_norm['fresh8'])]
    d2=[round(a-b,4) for a,b in zip(r_2['fresh8'],r_b['fresh8'])]
    d1=[round(a-b,4) for a,b in zip(r_1['fresh8'],r_b['fresh8'])]
    m2=stt.median(d2)
    pa=m2<=-0.005
    pb=abs(m2-(-0.0112))<=0.01
    pc=abs(stt.median(db)-(-0.0290))<=0.015
    res={'norm2304':r_norm,'best':r_b,'c89_288':r_2,'c89_144':r_1,
         'per_window_best_vs_norm':db,'per_window_c89_288_vs_best':d2,'per_window_c89_144_vs_best':d1,
         'additive_prediction':-0.0112,
         'convention':'L2 = CE above the real model; lower is better; positive entries = damage added',
         'pred_a_real_improvement':bool(pa),'pred_b_additive_number':bool(pb),'pred_c_reproduces_s2140':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'c89@288 - best per window: {d2}  median {m2:+.4f}')
    print(f'c89@144 - best per window: {d1}  median {stt.median(d1):+.4f}')
    print(f"(a) c89@288 median {m2:+.4f} <= -0.005: {'HELD' if pa else 'FAILED'}")
    print(f"(b) |{m2:+.4f} - (-0.0112)| = {abs(m2-(-0.0112)):.4f} <= 0.01: {'HELD' if pb else 'FAILED'}")
