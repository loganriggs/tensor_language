#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_unit_22_moves_the_pronoun_margin pred_c_random_unit_does_not pred_d_unit_22_within_mlp12 pred_e_hub_units_at_the_noun_replay
...
OUT = ROOT / "circuits/followups/unit_22_edit_v506_result.json"
CANDIDATE_ID = "chain.unit_22_edit_v506"
N_HEAD = 9
...
import random
import random
UNIT_SETS = {"U22_A": (12, (22,), "answer"), "RANDOM_A": (12, (random.Random(0).choice([j for j in range(4608) if j != 22]),), "answer"), "HUB_N": (8, (829, 953, 1030), "noun")}
...
FORWARDS_MAX = 17
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_unit_22_moves_the_pronoun_margin": ">= 0.04", "pred_c_random_unit_does_not": "<= 0.01", "pred_d_unit_22_within_mlp12": "<= 0.109", "pred_e_hub_units_at_the_noun_replay": "0.133 +- 0.02"}
...
def main() -> None:
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_unit_22_moves_the_pronoun_margin": v["U22_A"] >= U22_MIN, "pred_c_random_unit_does_not": abs(v["RANDOM_A"]) <= RANDOM_MAX,
                   "pred_d_unit_22_within_mlp12": v["U22_A"] <= M12_V504 + BAND, "pred_e_hub_units_at_the_noun_replay": abs(v["HUB_N"] - HUB_V463) <= BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "unit_22_edit_result_v506", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
