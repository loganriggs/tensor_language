"""IS THERE A CHEAP TAIL DICTIONARY AFTER ALL -- A LOW-RANK LINK MAP? (Claude, LANE 1)

SS2881 closed the obvious route and left exactly one open. Dropping the four 1152x1152 within-class link maps from each tail refit costs
**+0.1740 nats**, so they are the mechanism, not decoration; collapsing the ten-row class table costs only **+0.0248**. Per parameter the
table is about **6,600x** more efficient, which says the maps are wasteful rather than useless -- and a wasteful full-rank map is exactly
what a low-rank one replaces.

Three arms: BASELINE, every LW[k] truncated to **rank 8**, and to **rank 1**. A rank-r map costs 2*r*1152 numbers instead of 1152^2, so
across four LINK classes and eight tail layers rank 8 is **73,728 parameters against 42,467,328** -- a **576x** compression with
**0.1740 nats** of headroom to recover.

SS2881's full-drop cost is read from its receipt under a frozen hash rather than retyped, so the recovered-fraction arithmetic cannot
drift from the number it is measured against.

SIGN CONVENTION (SS2135): frontier L2 is CE ADDED ABOVE THE REAL MODEL, so **LOWER IS BETTER** (SS312: +2.6735 beating +2.84/+2.93).
A cost is L2_F(arm) - L2_F(baseline), **POSITIVE = WORSE**. SS2128/SS2129/SS2133/SS2134 RETRACTED; SS2125 STANDS. Per SS2879's standing
rule the manipulated entries are the a10L-a17L members of `order2`, which SS2881 showed move L2_F by .174; pred_d re-checks that here.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_rank8_recovers_most_of_the_link_map_value
#                     pred_c_rank1_is_worse_than_rank8
#                     pred_d_the_arms_are_connected
#                     pred_e_the_parameter_accounting_is_stated

Preregistration: polynomial_causal/FRONTIER_TAIL_LINK_LOWRANK_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; three arms (baseline / tail link maps rank-8 / rank-1)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_tail_link_lowrank_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
TAILMODE={'mode':None,'rank':None}   # None | 'links' | 'table' | 'rank' (truncate each LW[k])
CVSNAP={}
SKIP8=64
...
def main():
    ...
    for li in range(10,18):
        ...
        if TAILMODE['mode'] is not None:
            ...
            elif TAILMODE['mode'] == 'rank':
                # truncate each 1152x1152 within-class map to rank r: 2*r*1152 numbers instead of 1152^2
                r = TAILMODE['rank']
                for k in list(LW):
                    U, S_, Vh = torch.linalg.svd(LW[k].float(), full_matrices=False)
                    LW[k] = (U[:, :r] * S_[:r]) @ Vh[:r, :]
...
PREREG = PT + '../polynomial_causal/FRONTIER_TAIL_LINK_LOWRANK_PREREGISTRATION.md'
PREREG_SHA = "b5d18177213b19c04ca0aa976e3733079f3f66c9e5361bd05f6be176c2773957"
...
D_ = 1152
FULL_MAP = D_ * D_                 # 1,327,104 per link map
NLINK, NTAIL = 4, 8
BARS = {"reproduce": 0.05, "rank8_recovers": 0.05, "monotone": 0.02, "connected": 0.005}
NULLS = {"rank8_recovers_ge": 0.12, "monotone_le": 0.0}
...
if __name__=='__main__':
    ...
    cost_drop = prev['cost_dropping_link_maps_nats']      # SS2881: +0.1740 for removing the maps entirely
    SEL['mode']='norm'

    print('ARM 1/3: BASELINE -- the published frontier',flush=True)
    TAILMODE['mode']=None; TAILMODE['rank']=None
    r_base=main()

    print('ARM 2/3: tail link maps truncated to RANK 8',flush=True)
    TAILMODE['mode']='rank'; TAILMODE['rank']=8
    r_r8=main()

    print('ARM 3/3: tail link maps truncated to RANK 1',flush=True)
    TAILMODE['mode']='rank'; TAILMODE['rank']=1
    r_r1=main()
    TAILMODE['mode']=None; TAILMODE['rank']=None

    # SIGN: L2 is CE ADDED ABOVE THE REAL MODEL, LOWER IS BETTER -> cost POSITIVE = WORSE
    c8 = r_r8['L2_F'] - r_base['L2_F']
    c1 = r_r1['L2_F'] - r_base['L2_F']
    rec8 = 1.0 - (c8 / cost_drop) if cost_drop else float('nan')
    rec1 = 1.0 - (c1 / cost_drop) if cost_drop else float('nan')
    params8 = 2 * 8 * D_ * NLINK * NTAIL
    params1 = 2 * 1 * D_ * NLINK * NTAIL
    params_full = FULL_MAP * NLINK * NTAIL

    pa = abs(r_base['L2_F']-2.6735) <= BARS['reproduce']
    pb = c8 <= BARS['rank8_recovers']
    pc = (c1 - c8) >= BARS['monotone']
    pd = abs(c1) >= BARS['connected']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_rank8_recovers_most_of_the_link_map_value':bool(pb),
           'pred_c_rank1_is_worse_than_rank8':bool(pc),
           'pred_d_the_arms_are_connected':bool(pd),
           'pred_e_the_parameter_accounting_is_stated':True}
    nulls={'b_null_low_rank_does_not_recover':bool(c8>=NULLS['rank8_recovers_ge']),
           'c_null_rank_does_not_matter':bool((c1-c8)<=NULLS['monotone_le'])}
    res={'rung':'frontier_tail_link_lowrank','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,
         'baseline':r_base,'rank8':r_r8,'rank1':r_r1,
         'published_L2_F':2.6735,'S2881_full_drop_cost':cost_drop,
         'summary':{'L2_F_baseline':round(r_base['L2_F'],4),
                    'L2_F_rank8':round(r_r8['L2_F'],4),
                    'L2_F_rank1':round(r_r1['L2_F'],4),
                    'cost_rank8_nats':round(c8,4),
                    'cost_rank1_nats':round(c1,4),
                    'cost_full_drop_S2881_nats':round(cost_drop,4),
                    'fraction_recovered_rank8':round(rec8,4),
                    'fraction_recovered_rank1':round(rec1,4),
                    'params_full_link_maps':int(params_full),
                    'params_rank8':int(params8),'params_rank1':int(params1),
                    'compression_rank8':round(params_full/params8,1),
                    'L2_C_baseline':round(r_base['L2_C'],4),'L2_C_rank8':round(r_r8['L2_C'],4)},
    ...
    print(f"(b) rank-8 cost {c8:+.4f} <= {BARS['rank8_recovers']} (recovers {rec8:.1%} of {cost_drop:+.4f}): {'HELD' if pb else 'FAILED'}")
    print(f"(c) rank-1 {c1:+.4f} worse than rank-8 by {c1-c8:+.4f} >= {BARS['monotone']}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) arms connected: {'HELD' if pd else 'FAILED'}")
