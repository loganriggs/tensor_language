...
Replay<1e-5; Primary empirical-root error<.10, <=.9narrow26 and<=1.1parent656; baselineCEadded<.02 and<=parent+.002.
Conditional term only; fixed recipient RMS/background; no fitting or semantic adoption.
Prices: cheap26/48384, parent656/903168, roots384/322048+768indices.
Null: output sharing loses native response fidelity.
"""
...
def evaluate_program(arm,programs,x,teacher):
 ...
 if arm=='exact':return teacher(x)
 return pair_evaluate(programs[arm],x) if 'pairs' in programs[arm] else evaluate(programs[arm],x)
...
def main():
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
  ...
  torch.manual_seed(81);x=torch.randn(7,3,dtype=torch.float64);program=dict(U=torch.randn(2,2,3,dtype=torch.float64),V=torch.randn(2,2,3,dtype=torch.float64),C=torch.randn(4,3,dtype=torch.float64));programs={k:program for k in ['narrow26','parent656']}
  ...
  from paired_root_compiler import evaluate as pair_check
  root=dict(U=program['U'],V=program['V'],writer=torch.randn(4,2,dtype=x.dtype),pairs=[dict(kind='spectral',vectors=torch.eye(2,dtype=x.dtype).repeat(2,1,1),values=torch.randn(2,2,dtype=x.dtype))])
  for arm in ['empirical384','coefficient384']:programs[arm]=root;assert torch.equal(evaluate_program(arm,programs,x,None),pair_check(root,x))
  ...
  assert all(r['effect_error']==0 and r['cosine']==1 for r in summarize(rr));print(json.dumps(dict(captures=32,arms=5,strengths=[.5,1],aggregation_smoke='pass')));return
 import torch
 ...
 out=P/'EXPANDED_ROOT_NATIVE_V1.json';assert not out.exists();start=time.monotonic()
 files={'tokens': 'FULL_CHANNEL_FRESH_TOKENS_V1.pt', 'narrow26': 'WIDE_NATIVE_QUARTIC_4_INHERITED_LONG_V1.pt', 'parent656': 'QUARTIC_EXPANDED_STATE_V1.pt', 'empirical384': 'EXPANDED_ROOT_EMPIRICAL_V1.pt', 'coefficient384': 'EXPANDED_ROOT_COEFFICIENT_V1.pt'}
 hashes={k:hashlib.sha256((P/v).read_bytes()).hexdigest() for k,v in files.items()};assert hashes==json.loads((P/'EXPANDED_ROOT_NATIVE_INPUTS_V1.json').read_text())['sha256']
 def move(v):
  ...
  return v
 programs={k:move(torch.load(P/files[k],weights_only=True)) for k in ['narrow26','parent656','empirical384','coefficient384']};tokens=torch.load(P/files['tokens'],weights_only=True)
 ...
 for domain,documents in tokens.items():
  ...
  for doc,c in enumerate(captures):
   ...
   for arm in ['exact','narrow26','parent656','empirical384','coefficient384']:
 ...
 summary=summarize(rows);primary=[r for r in summary if r['arm']=='empirical384'];lookup={(r['domain'],r['alpha'],r['arm']):r for r in summary}
 pred=dict(pred_a_instrument=max(checks)<1e-5 and max(zeros)==0 and all(r['effect_error']==0 for r in summary if r['arm']=='exact'),pred_b_response=all(r['effect_error'] is not None and r['effect_error']<.1 for r in primary),pred_c_advantage=all(r['effect_error'] is not None and r['effect_error']<=.9*lookup[r['domain'],r['alpha'],'narrow26']['effect_error'] and r['effect_error']<=1.1*lookup[r['domain'],r['alpha'],'parent656']['effect_error'] and r['baseline_ce_added']<.02 and r['baseline_ce_added']<=lookup[r['domain'],r['alpha'],'parent656']['baseline_ce_added']+.002 for r in primary))
