# CAN A 36-NUMBER TABLE PRICE A COMPILED SET?  -- fitting and TESTING §1833's impression.
#
# §1829-§1833 produced a shape rather than a mechanism: a per-site cost that falls steeply with depth
# (§1831: 37.4pp at layer 1, 21.9 at layer 2, 0.7 at layer 9, -0.5 at layer 17), summed over the set,
# with a super-additive multiplier (§1832: ten deep sites cost 1.8x their single-site rate) and NO
# adjacency term (§1833: contiguity buys 1.5pp against a 10pp bar). §1830's layer 1 is the one exception,
# sub-additive to the point of redundancy at 1.96x of an exact-redundancy 2.00x.
#
# That shape has never been fitted or tested out of sample, and every per-site weight in it comes from
# TEN measured single sites out of thirty-six. This measures ALL of them -- B0 plus one site, for each of
# the 34 sites at layers 1..17 -- and then asks whether ONE constant on the sum of single-site costs
# predicts seven multi-site arms.
#
# Codex's 19:38 note states that §1832 "rules out independent per-site cost as a whole-program simplicity
# measure". That is right about INDEPENDENT (purely additive) per-site cost. This tests the weaker and
# more useful claim it leaves open: per-site cost plus a single interaction constant.
#
# Four test arms are measured IN THIS RUN (B1, TOP13, TOP9, SCATTER_ODD) and three more are taken from
# published figures (TOP11, TOP7, SCATTER_EVEN), giving seven points against one free parameter.
#
...
# failure branches enumerated per LESSON 44, ratio-not-share per LESSONS 57:
#   pred_a ONE MULTIPLIER SUFFICES: fitting cost(set) = alpha * sum of single-site costs, one parameter
#          by least squares over the seven multi-site arms, gives R^2 at least 0.90 against the measured
#          costs. If TRUE a compiled set of any shape can be priced from a 36-number table plus one
#          constant WITHOUT running it, which is the first thing in this arc that is useful rather than
#          merely true. If FALSE no per-site table plus a single interaction constant captures the cost,
#          the sites interact in a way that depends on WHICH sites, and §1833's separable-plus-multiplier
#          impression must be struck from the record rather than repeated.
#   pred_b AND ALPHA IS SUPER-ADDITIVE AND MODERATE: alpha lies in [1.2, 3.0]. If alpha <= 1.2 the joint
#          cost is at or below the sum of parts and §1832's super-additivity does not survive a proper
#          fit -- it would then have been an artefact of extrapolating from three in-band sites. If
#          alpha > 3.0 the interaction is too strong for one constant to be a useful summary even should
#          it fit, and the table would price nothing.
#   pred_c AND THE DEPTH PROFILE IS MONOTONE ENOUGH TO ACT ON: all twelve of the cheapest single sites
#          sit at layer 8 or deeper. If FALSE some early site is cheap, the profile has structure
#          §1831's seven sampled layers missed, and "compile the deepest sites" is not the right greedy
#          rule -- which would matter, because it is the rule anyone would reach for first.
#   pred_d CONTROLS, cross-run per LESSON 42, fifteen published reproductions: the ten single sites
#          §1831 published (attn at 1,2,3,5,9,13,17 and mlp at 1,9,17) within 0.03; B1 reproduces
#          §1829's 25.9%; TOP13 §1832's 40.6%; TOP9 §1832's 22.3%; SCATTER_ODD §1833's 44.0%; B0
#          §1806's raw 37.4%. Endpoints must reproduce §1789's FULL-RANK top-1; the placement control
...
OUT = PT + 'ops/site_cost_table_results.json'
...
ALL_SITES = [(k2, L2) for L2 in range(1, 18) for k2 in ('attn', 'mlp')]   # the 34 beyond B0
ARMS = {'B0': list(B0_SITES), 'B1': B0_SITES + [('attn', 1), ('mlp', 1)],
        'TOP13': B0_SITES + [(k2, L2) for L2 in range(13, 18) for k2 in ('attn', 'mlp')],
        'TOP9': B0_SITES + [(k2, L2) for L2 in range(9, 18) for k2 in ('attn', 'mlp')],
        'SCATTER_ODD': B0_SITES + [(k2, L2) for L2 in (9, 11, 13, 15, 17)
                                   for k2 in ('attn', 'mlp')]}
for _st in ALL_SITES:
    ARMS[f'S_{_st[0]}{_st[1]}'] = B0_SITES + [_st]
# PUBLISHED multi-site arms used as extra out-of-sample test points, with their exact site sets
S1832_EXTRA = {'TOP11': ([(k2, L2) for L2 in range(11, 18) for k2 in ('attn', 'mlp')], 0.318),
               'TOP7': ([(k2, L2) for L2 in range(7, 18) for k2 in ('attn', 'mlp')], 0.068),
               'SCATTER_EVEN': ([(k2, L2) for L2 in (8, 10, 12, 14, 16)
                                 for k2 in ('attn', 'mlp')], 0.401)}
# §1831's PUBLISHED single-site sequential gap fractions, skip7000 -- ten cross-run anchors
S1831_SINGLE = {'S_attn1': 0.274, 'S_attn2': 0.219, 'S_attn3': 0.449, 'S_attn5': 0.479,
                'S_attn9': 0.641, 'S_attn13': 0.642, 'S_attn17': 0.653,
                'S_mlp1': 0.261, 'S_mlp9': 0.607, 'S_mlp17': 0.608}
# PUBLISHED multi-site sequential gap fractions measured in THIS run, skip7000
S1833_INRUN = {'B1': 0.259, 'TOP13': 0.406, 'TOP9': 0.223, 'SCATTER_ODD': 0.440}
...
def main():
    ...
    print(f'SITE COST TABLE | all 34 single sites + a one-parameter pricing test | '
    ...
    b0 = seq['B0']
    w = {st: b0 - seq[f'S_{st[0]}{st[1]}'] for st in ALL_SITES}     # single-site cost, pp of gap
    pts = []
    for name in ('B1', 'TOP13', 'TOP9', 'SCATTER_ODD'):
        extra = [st for st in ARMS[name] if st not in B0_SITES]
        pts.append((name, sum(w[st] for st in extra), b0 - seq[name], 'in-run'))
    for name, (sl, val) in S1832_EXTRA.items():
        pts.append((name, sum(w[st] for st in sl), b0 - val, 'published'))
    sp = sum(p * p for _, p, _, _ in pts)
    alpha = sum(p * c2 for _, p, c2, _ in pts) / sp if sp > 1e-12 else 0.0
    cbar = sum(c2 for _, _, c2, _ in pts) / len(pts)
    ssres = sum((c2 - alpha * p) ** 2 for _, p, c2, _ in pts)
    sstot = sum((c2 - cbar) ** 2 for _, _, c2, _ in pts)
    r2 = 1.0 - ssres / sstot if sstot > 1e-12 else 0.0
    order = sorted(ALL_SITES, key=lambda st: w[st])
    cheap12 = order[:12]
    pa = r2 >= 0.90
    pb = 1.2 <= alpha <= 3.0
    pc = all(st[1] >= 8 for st in cheap12)
    pd = (all(abs(seq[k] - v) <= 0.03 for k, v in S1831_SINGLE.items())
          and all(abs(seq[k] - v) <= 0.03 for k, v in S1833_INRUN.items())
          and all(abs(frac[e]['B0_seq'] - S1829_SEQ_GAPFRAC['B0'][e]) <= 0.03 for e in roles)
    ...
    print(f'\n  SINGLE-SITE COST TABLE ({PICK_ROLE}, pp of gap lost vs B0 = {b0:.1%}):', flush=True)
    for k2 in ('attn', 'mlp'):
        print(f'    {k2:4s} ' + ' '.join(
            f'L{L2}:{w[(k2, L2)]:+6.1%}' for L2 in range(1, 18)), flush=True)
    print(f'\n  cheapest 12 sites: ' + '  '.join(
        f'{st[0]}{st[1]} {w[st]:+.1%}' for st in cheap12), flush=True)
    print(f'\n  ONE-PARAMETER PRICING TEST  cost = alpha * sum(single-site costs)', flush=True)
    print(f'    alpha {alpha:.3f}   R^2 {r2:.4f}   over {len(pts)} multi-site arms', flush=True)
    for name, p, c2, src2 in pts:
        print(f'      {name:13s} {src2:9s} sum {p:7.1%}  predicted {alpha * p:7.1%}  '
              f'measured {c2:7.1%}  resid {c2 - alpha * p:+6.1%}', flush=True)
    print(f'\n  ONE MULTIPLIER suffices (R^2 >= 0.90) -> {pa}  R^2 {r2:.4f}', flush=True)
    print(f'  and alpha is super-additive and moderate (1.2-3.0) -> {pb}  alpha {alpha:.3f}',
          flush=True)
    print(f'  and the 12 cheapest sites are all at layer >=8 -> {pc}  shallowest cheap site is '
          f'{cheap12[max(range(12), key=lambda i: -cheap12[i][1])][0]}'
          f'{min(st[1] for st in cheap12)}', flush=True)
    print(f'  ten §1831 single sites + four §1829/§1832/§1833 multi-site arms reproduce '
          f'-> control {pd}', flush=True)

    json.dump({'run': 'site_cost_table', 'role': PICK_ROLE,
               'B0_gapfrac': b0,
               'single_site_cost': {f'{st[0]}{st[1]}': w[st] for st in ALL_SITES},
               'cheapest_12': [f'{st[0]}{st[1]}' for st in cheap12],
               'alpha': alpha, 'r2': r2,
               'test_points': [{'arm': n, 'sum_singles': p, 'measured': c2, 'source': s2,
                                'predicted': alpha * p, 'residual': c2 - alpha * p}
                               for n, p, c2, s2 in pts],
               'gap_fraction': frac, 'sequential': seq,
               'predictions': {'pred_a_one_multiplier_suffices': bool(pa),
                               'pred_b_alpha_superadditive_moderate': bool(pb),
                               'pred_c_cheapest_are_deep': bool(pc),
