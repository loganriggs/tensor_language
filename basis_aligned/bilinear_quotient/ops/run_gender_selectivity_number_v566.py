#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_five_writes_leave_number_alone pred_c_all_writes_leave_number_alone pred_d_gender_effect_replays pred_e_rows_do_not_differ_in_number
"""Selectivity of the gender edit (v566). v563 / v565: the five pronoun readers' writes into the answer close 0.88-0.89 of the he - she gap on two natural
gender sets. The same five heads carry number (they vs he / she). Does the gender edit move number? The pronoun logits split into a gender axis, he - she,
and a number component, 2 they - he - she (unchanged if only the he / she balance shifts). Here v563's edits with both axes scored: the number component's
movement under each edit, pooled over pairs with the same sign convention, over the pooled he - she gap.
PREDICTIONS (scored as written; failures preserved; priors from v438-v440 (number's gender tilts cancel) and v563)
    pred_a_baseline_replays              the unedited manual forward replays the model's he - she margins within 1e-3
    pred_b_five_writes_leave_number_alone  |number component moved by FIVE_W| <= 0.10 of the gender gap
    pred_c_all_writes_leave_number_alone   |number component moved by ALL_W| <= 0.15 of the gender gap
    pred_d_gender_effect_replays         FIVE_W closes 0.878 +- 0.05 of the he - she gap (v563)
    pred_e_rows_do_not_differ_in_number  the native male - female difference of the number component is <= 0.10 of the gender gap (the pairs differ in gender only)
...
OUT = ROOT / "circuits/followups/gender_selectivity_number_v566_result.json"
CANDIDATE_ID = "gender.selectivity_number_v566"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, FIVE_NUM_MAX, ALL_NUM_MAX, FIVE_V563, BAND, NATIVE_MAX, UNIT = 1e-3, 0.10, 0.15, 0.878, 0.05, 0.10, 701
...
FORWARDS_MAX = 9
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_five_writes_leave_number_alone": "<= 0.10", "pred_c_all_writes_leave_number_alone": "<= 0.15", "pred_d_gender_effect_replays": "0.878 +- 0.05", "pred_e_rows_do_not_differ_in_number": "<= 0.10"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "five_num_max": FIVE_NUM_MAX, "all_num_max": ALL_NUM_MAX, "five_v563": FIVE_V563, "band": BAND, "native_max": NATIVE_MAX, "unit": UNIT}}
    ...
    MODES = {"FIVE_W": ({}, (0,)), "REST_W": ({}, (0,)), "ALL_W": ({}, (0,))}; SETS_W = {"FIVE_W": FIVE_W, "REST_W": REST_W, "ALL_W": {l: list(range(N_HEAD)) for l in range(18)}}; forwards = 0
    def logits_margin(x):
        z = 30.0 * torch.tanh(model.lm_head(F.rms_norm(x, (D,))) / 30.0).float()
        return torch.stack([z[:, HE] - z[:, SHE], 2 * z[:, THEY] - z[:, HE] - z[:, SHE], z[:, THEY] - z[:, HE]], 1), z   # column 0 = he - she (gender); column 1 = 2 they - he - she (number component)
    ...
    def study(items, batch):
        ...
        number_moved = {m: float(((nat_m[0::2, 1] - e[0::2, 1]) + (e[1::2, 1] - nat_m[1::2, 1])).sum() / (2 * mgap.sum())) for m, e in edm.items()}
        native_number_ratio = float((nat_m[0::2, 1] - nat_m[1::2, 1]).sum() / mgap.sum())
        return {"pairs": len(items), "unit": UNIT, "contrast_mean": float(gap.mean()), "sign_constancy": sign, "moved": moved, "margin_closed": margin_closed, "number_moved": number_moved, "native_number_ratio": native_number_ratio, "gender_gap_mean": float(mgap.mean()), "native_margins": nat_m[:, 0].tolist()}, nat_m[:, 0]
    ...
    nm = verb["number_moved"]; print("number component moved", {k: round(v, 3) for k, v in nm.items()}, "native number ratio", round(verb["native_number_ratio"], 3), "gender gap", round(verb["gender_gap_mean"], 2))
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_five_writes_leave_number_alone": abs(nm["FIVE_W"]) <= FIVE_NUM_MAX, "pred_c_all_writes_leave_number_alone": abs(nm["ALL_W"]) <= ALL_NUM_MAX,
                   "pred_d_gender_effect_replays": abs(mc["FIVE_W"] - FIVE_V563) <= BAND, "pred_e_rows_do_not_differ_in_number": abs(verb["native_number_ratio"]) <= NATIVE_MAX}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "gender_selectivity_number_result_v566", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
