...
"""v281: within-family separability for v277's single four-row pass, verb_preposition_by_with, under the largest family
in the corpus (21 counted members, 13 controls, 34 members prepared).

WHAT IS BEING ASKED. v277 went 1 of 8 and by_with is the survivor: abided/tinkered -> by/with, 7 units (the smallest
set in the family), held 0.86, C UB975 0.0031, cext 1.002. Its cue words are fresh -- `abided` and `tinkered` appear in
no other committed cell -- and that matters, because the v254-v262 line established that cue reuse is what makes cells
leak into each other and by_with was authored in a batch built to have none. So this rung asks the question that batch
was designed to make answerable: with no cue overlap anywhere, does a new cell separate cleanly from 21 counted
siblings, or is there leakage the cue account never explained?
Its ` with` readout is carried by FIVE counted siblings (with_about, to_with, of_with, over_with, into_with) and its
` by` by none -- so it is also another instance of the shared-readout case that v248 (one shared readout, sib 0.005),
v250 (two shared readouts, 0.006) and v252 (16 controls, 0.009) all found separable. If it fuses, the shared-readout
account revives and I will have to explain why it took 21 controls to show.

FAMILY. verb_preposition, 21 counted after v258/v262. Control-only members: at_from, on_with, to_about, by_for, of_by,
on_under, on_toward, of_over, at_under, of_beneath (row-4/row-5 misses that joined the control set), plus
with_beyond and in_beyond (v267 passes fused or counted at v258) and about_beyond (v277's row-4 miss, joining here).
The four v258/v260 fused incumbents (over_with, with_about, in_to, to_at) are KEPT as counted: each lost separability
only to its own cue-sharing partner, the pair is counted once with the incumbent holding the count, and v262 showed
that overlap is real rather than a control-set artifact (in the unconstrained arm the partner was the largest leak for
three of four, margins +0.48 to +0.62). That reading is on the board unanswered since 09:45 and is applied here as
registered rather than re-litigated.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
    pred_a_new_separable   verb_preposition_by_with is separable (k_new = 1 of 1). With no cue reuse and 21 counted
                           siblings this is the cleanest test of "does a fresh pair count" the corpus has.  prior 70%
    pred_b_counted_kept    at least 17 of the 21 re-checked counted members remain separable (k_kept). v258 lost four
                           incumbents to cue partners entering the control set; nothing new enters here except
                           by_with and three controls, so a loss now would NOT have a cue explanation.      prior 60%
    pred_c_row4_kept       fam-arm own-C UB975 <= 0.01 on at least 11 of the 22 evaluated members (k_d; v258 11/22). prior 55%
...
Watch, not a bar: v258 had verb_preposition itself at sib 0.043 and insisted at 0.035 with the leaks drifting up as
controls accumulate; this rung adds four more controls to that arm, and if either crosses 0.05 it is a retraction
caused entirely by control-set growth -- which is exactly what board proposal (ix) is about and it is still unanswered.
COUNTING. A separable by_with is +1 (107 -> 108). A fused by_with joins whichever counted incumbent it leaks into (+0).
A counted member that loses separability to something OTHER than a cue partner is a genuine retraction (-1) and gets
reported to the board rather than absorbed.
Recipe as v198/.../v262: per evaluated member, arm `own` (own C_fit control) and arm `fam` (own C_fit + EVERY sibling's
A1 FIT prep, control_weight 30 x len(controls)); evaluated on the member's held A1 (extraction), each sibling's held A1
(|CE| leak) and held C. Units frozen from the parent receipts (v277 for by_with; earlier batteries for the rest).
...
EVAL = 1 new + 21 counted re-checks (22 fits x 2 arms, 33 controls in the fam arm: ~2 h; the full re-check stands
because proposal (ix) is unanswered).
Smoke: V281_SMOKE=<out.json> (CPU, V281_SMOKE_ROWS=4; V281_SMOKE_NAMES=verb_preposition_by_with,verb_preposition).
BARS = {"cross_max": 0.05, "keep": 0.80, "ext_floor": 0.50, "c_ub_max": 0.01, "k_d": 11, "k_new": 1, "k_kept": 17, "instr_tol": 0.03}
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v281_result.json"
...
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES, **v213.NAMES, **v215.NAMES, **v217.NAMES, **v219.NAMES, **v221.NAMES, **v223.NAMES, **v225.NAMES, **v227.NAMES, **v229.NAMES, **v231.NAMES, **v233.NAMES, **v235.NAMES, **v237.NAMES, **v239.NAMES, **v241.NAMES, **v243.NAMES, **v245.NAMES, **v247.NAMES, **v249.NAMES, **v251.NAMES, **v253.NAMES, **v255.NAMES, **v257.NAMES, **v259.NAMES, **v261.NAMES, **v263.NAMES, **v265.NAMES, **v267.NAMES, **v277.NAMES}


def _parent_cdas(n):
    # the parent battery receipt that produced this member's units (each behaviour name is unique across batteries)
    for path in (V277, V267, V265, V263, V261, V259, V257, V255, V253, V251, V249, V247, V245, V243, V241, V239, V237, V235, V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v281", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V281_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V281_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v281", "candidate_id": "corpus.unit_family_separability_spec_v281",
