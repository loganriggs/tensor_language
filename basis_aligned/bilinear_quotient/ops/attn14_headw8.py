...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['attn16_headw8_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; per-head window-grain map at attn14 (10 x 8 evals; damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'attn14_headw8_results.json'
...
def main():
    ...
    for li in range(10,18):
        if li==SEL.get('skipli',-1): continue
        Ys=[]; Xs=[]
...
if __name__=='__main__':
    ...
    SEL['skipli']=14; SEL['headw8']=True; SEL['W8store']=[]
    ...
    dall=stt.median([v-b for v,b in zip(res_h['all'],r_n['fresh8'])])
    medh=stt.median(list(meds.values()))
    pa=max(meds.values())>=2*max(medh,1e-9)
    pb=abs(sum(meds.values())-dall)<=0.05
    pc=stt.median(saved)>=0.04
    res={'base_skip14':r_n,'per_head_windows':res_h,
         'window_median_d':{f'14.{h}':round(meds[h],4) for h in range(9)},'d_all_median':round(dall,4),
         'per_window_saving_vs_s2144_best':saved,
         'convention':'L2 = CE above the real model; lower is better; d_h = damage from zeroing head h (eval-scoped)',
         'pred_a_concentrated':bool(pa),'pred_b_additive':bool(pb),'pred_c_marginal_survives':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print('window-median d per head: '+' '.join(f'14.{h}:{meds[h]:+.4f}' for h in range(9))+f'  all:{dall:+.4f}')
    print(f"(a) max {max(meds.values()):+.4f} >= 2 x median {medh:+.4f}: {'HELD' if pa else 'FAILED'}")
    print(f"(b) |sum {sum(meds.values()):+.4f} - all {dall:+.4f}| = {abs(sum(meds.values())-dall):.4f} <= 0.05: {'HELD' if pb else 'FAILED'}")
    print(f"(c) saving vs S2144 best, median {stt.median(saved):+.4f} >= +0.04: {'HELD' if pc else 'FAILED'}")
