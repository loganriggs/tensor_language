...
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';OUT=A/'semantic_source_jet_v2_result.json'
            ...
            for role,key in [('subject','subject_position'),('attractor','control_position')]:
                ...
                def evaluate(amplitudes):
                    ...
                    for l in range(11,18):
                        ...
                        x=x+F.linear(F.linear(z,b.mlp.Left.weight.double())*F.linear(z,b.mlp.Right.weight.double()),b.mlp.Down.weight.double(),b.mlp.Down.bias.double() if b.mlp.Down.bias is not None else None)
