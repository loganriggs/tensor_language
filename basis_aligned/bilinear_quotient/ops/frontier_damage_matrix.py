"""PER-CIRCUIT DAMAGE MATRIX (rung 124): survivor identity and whether the four sites damage the SAME circuits.

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows; LOWER IS BETTER.
S2215: every CP pair keeps exactly 2/62 circuits; identity unknown. Here the four singles and the quad are
rerun SAVING the full per-circuit member mean|dCE| rows (5 x 62 damage matrix - the direction chapter's first
artifact), plus each arm's valid set.

REGISTERED PREDICTIONS (arms named single_cf0..3, quad):
  (a) MONOTONE DESTRUCTION: every circuit valid under quad is valid under ALL four singles.
  (b) SHARED-TARGET DAMAGE: median pairwise Spearman rho across the four singles' 62-circuit damage vectors
      >= 0.7 (the sites hurt the same circuits; the direction story is shared, not site-specific).
  (c) REPRODUCTION: |agg(quad) - 0.8302| <= 0.015 (rung-118 receipt).
NULL: site-specific damage (median rho < 0.7) and/or non-monotone validity (a circuit valid in quad but not
in some single). PRICE: none (attribution; the matrix is the product). Tripwire: INSTRUMENT FAIL if arms
identical (pstdev < 0.005). Self-reviewed."""
import json, sys, time, os
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
if os.environ.get('BQLIB_DRYRUN')=='1':
    _bq='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
    _need=['frontier_cp_pairs_results.json','frontier_pair_certs_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: per-circuit damage matrix')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_damage_matrix_results.json'
...
def main():
    ...
    if SEL.get('dmg_matrix'):
        ...
        _arms={}
        for _li in (0,1,2,3):
            _arms[f'single_cf{_li}']=evalV(_ROWS,_ROWS.shape[0],[f'cf{_li}'],ML).detach().cpu()
            print(f'  single cf{_li} done',flush=True)
        _arms['quad']=evalV(_ROWS,_ROWS.shape[0],['cf0','cf1','cf2','cf3'],ML).detach().cpu()
...
if __name__=='__main__':
    ...
    QA=json.load(open(PT+'frontier_cp_pairs_results.json'))['aggs']['quad']
    SEL['mode']='norm'; SEL['K']=4608; SEL['K69']=4608; SEL['K69MAP']={}
    SEL['skipset']=tuple(range(10,18)); SEL['motif_off']=tuple(range(2,10))
    SEL['dmg_matrix']=True; SEL['ext_rows']=CROWS
    print('ARMS: four CP singles + quad, per-circuit damage rows saved',flush=True)
    main()
    A=SEL['front_result']
    tags=sorted(CINFO)
    M={}; valid={}; aggs={}
    for k,cev in A.items():
        ...
        d=cev-CBASE
        aggs[k]=round(float(d.mean()),4)
        M[k]={t:round(float(d[CINFO[t]['mask']].abs().mean()),4) for t in tags}
        valid[k]=[t for t in tags if M[k][t]<0.5*CINFO[t]['ref']]
        print(f"  {k:11s} agg {aggs[k]:+.4f}  valid {len(valid[k])}/{len(tags)}",flush=True)
    import statistics as stt
    if stt.pstdev(aggs.values())<0.005: raise SystemExit('INSTRUMENT FAIL: arms identical')
    singles=[f'single_cf{i}' for i in (0,1,2,3)]
    pa=all(all(t in valid[sk] for sk in singles) for t in valid['quad'])
    def srho(u,v):
        import torch as T
        a=T.tensor(u).argsort().argsort().float(); b=T.tensor(v).argsort().argsort().float()
        a=a-a.mean(); b=b-b.mean()
        return float((a*b).sum()/((a.norm()*b.norm())+1e-9))
    rhos=[]
    import itertools as it
    for x,y in it.combinations(singles,2):
        rhos.append(round(srho([M[x][t] for t in tags],[M[y][t] for t in tags]),3))
    med=stt.median(rhos)
    pb=med>=0.7
    pc=abs(aggs['quad']-QA)<=0.015
    res={'aggs':aggs,'valid_sets':valid,'damage_matrix':M,'pairwise_spearman':rhos,
         'median_spearman':round(med,3),'quad_anchor_118':QA,
         'convention':'per-position dCE = CE(config) - CE(real model) on census rows; tau = 0.5 x battery ref',
         'pred_a_monotone':bool(pa),'pred_b_shared_targets':bool(pb),'pred_c_repro':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"quad survivors: {valid['quad']}")
    print(f'pairwise spearman {rhos}; median {med:.3f}')
    print(f"(a) monotone destruction: {'HELD' if pa else 'FAILED'}")
    print(f"(b) median rho {med:.3f} >= 0.7: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |quad {aggs['quad']:+.4f} - {QA:+.4f}| <= 0.015: {'HELD' if pc else 'FAILED'}")
