...
"""Export validated conditional response program with shared product banks.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_joint_chain_v655_result.json'
...
def main():
    ...
    artifact=OUT.parent/'subject_response_v655_program.pt'
    package=dict(runtime=exported_runtime,cases=exported_cases,
        producer=cpu(dict(programs=programs['calibration'],initial_encoder=encoders[0],
        ...
        metadata=dict(scope='Conditional model only; native fidelity gate fails and context generation remains native',
                      fixed_values=values,context_values_per_example=505,rank=8,
                      native_fidelity_gate=bool(c),predictions=result['predictions']))
