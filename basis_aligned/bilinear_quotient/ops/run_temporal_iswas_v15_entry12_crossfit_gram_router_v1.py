#!/usr/bin/env python3
"""Sign-invariant Gram A1/A2/off router for the entry12 construction union."""

# BQGATE: EXPERIMENT pred_a_authority_replay_scope_geometry_finiteness_and_price pred_b_gram_router_identifies_held_branches pred_c_routed_union_preserves_both_targets pred_d_routed_union_is_control_selective pred_e_router_retains_ungated_union_effect
...
PRIOR = ROOT / "circuits/prior_art/temporal_iswas_v15_entry12_crossfit_gram_router_v1.json"
REGULARIZED_RESULT = ROOT / "circuits/followups/temporal_iswas_v15_entry12_crossfit_regularized_source_router_v1_result.json"
ORACLES = ROOT / "circuits/followups/temporal_iswas_v15_construction_oracle_projective_bisector_v1_result.json"
OUT = ROOT / "circuits/followups/temporal_iswas_v15_entry12_crossfit_gram_router_v1_result.json"
CANDIDATE_ID = "temporal_auxiliary.iswas_v15_entry12_crossfit_gram_router_v1"
EXPECTED = {
    "prior": "40fd848a73f0eb91be2318bdd5cb0f29b85f6baff261f4ba50e4faf6fa6ad0e6",
    "regularized_result": "7e1bb0c549dc37f002909738cfa1e498a5c8734791cc9aa1fb4c9663fd5e182e",
    ...
    "router": "c3fdb42f84e96c057dbd20c56136100b27c784f041502365b13072104f15c266",
...
FILES = {
    "prior": PRIOR, "regularized_result": REGULARIZED_RESULT, "oracles": ORACLES,
    "finite_router": ROOT / "ops/entry12_finite_router_contract.py",
    "router": ROOT / "ops/entry12_gram_router_contract.py",
...
PRICE_MAX = {"native_capture_forwards": 10, "differentiable_transformer_forwards": 24,
             ...
             "example_evaluations": 2000, "fit_parameters": 30}
...
def main():
    ...
    prior, regularized_result = json.loads(PRIOR.read_text()), json.loads(REGULARIZED_RESULT.read_text())
    authority = (observed == EXPECTED and prior.get("candidate_id") == CANDIDATE_ID
                 and regularized_result.get("terminal") == "router_identification_failure")
    dry = {"candidate_id": CANDIDATE_ID, "dryrun": True, "authority_ok": authority,
           "router_classes": list(finite_router.CLASSES), "expected_differentiable_forwards": 24,
           "price_max": PRICE_MAX}
    ...
    direction_parity_ok = all(
        row["direction_id"] == ("present_to_past" if row["group_number"] % 2 == 0 else "past_to_present")
    ...
    for held in (0, 1):
        ...
        train_features = router.gram_features(
            backend.torch, train_off,
            {panel: on_cache[panel][train][1] for panel in ("A1", "A2")},
            train_context["base_batch"].semantic_positions)
        train_labels = finite_router.labels_for_rows(backend.torch, train_context["rows"], device=backend.device)
        router_fit = router.fit(backend.torch, train_features, train_labels)
        held_context = contexts[held]; off_output, off_entry = off_cache[held]
        held_features = router.gram_features(
            backend.torch, off_entry,
            {panel: on_cache[panel][held][1] for panel in ("A1", "A2")},
            held_context["base_batch"].semantic_positions)
        truth = finite_router.labels_for_rows(backend.torch, held_context["rows"], device=backend.device)
        predicted, scores = router.predict(backend.torch, held_features, router_fit)
        classification = finite_router.classification_report(backend.torch, truth, predicted)
        fits[str(held)] = {"basis": {"rank": int(union.shape[1]), **basis_geometry},
                           "router": {**classification, "predictions": predicted.cpu().tolist(),
                                      ...
                                      "train_features": train_features.cpu().tolist(),
                                      "held_features": held_features.cpu().tolist()}}
        ...
        learned_absolute = finite_router.routed_absolute(
            backend.torch, off_entry, expert_states, union, predicted,
            held_context["base_batch"].semantic_positions)
        gold_absolute = finite_router.routed_absolute(
    ...
    A = (authority and base_text_impossibility and direction_parity_ok and disjoint
    ...
    predictions = {
        ...
        "pred_b_gram_router_identifies_held_branches": B,
    ...
    gold_pass = all(reports[str(parity)]["gold_router"][panel]["metrics"]["signed_projection"] >= .75
                    for parity in (0, 1) for panel in ("A1", "A2"))
    terminal = ("invalid" if not A else "gram_router_candidate" if all((B, C, D, E))
    ...
    result = {"schema": "temporal_iswas_v15_entry12_crossfit_gram_router_result_v1",
              ...
              "instrument": {"manual_native_max_abs_error": native_closure,
                             ...
                             "direction_parity_ok": direction_parity_ok,
