...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['twin_anatomy_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: deps present; mean-ablation suite for all three twin pairs (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'twin_mean_suite_results.json'
...
def main():
    ...
    _tw=[]
    if SEL.get('mean_abl'):
        for _li9,_MU in SEL['mean_abl'].items():
            def _mkma(MU):
                def hma(mo,i_,o_):
                    return (MU.expand_as(o_[0]).to(o_[0].dtype),o_[1])
                return hma
            _tw.append(m.transformer.h[_li9].attn.register_forward_hook(_mkma(_MU)))
            print(f'attn{_li9} MEAN-ablated for this arm (bases excluded)',flush=True)
...
if __name__=='__main__':
    ...
    ZREF={'b23':9.0718,'p1416':0.2044}
    CONFIGS=[('b23',(2,3),(2,3),()),('b45',(4,5),(4,5),()),('p1416',(14,16),(),(14,16))]
    import statistics as stt
    allres={}
    base23=None
    for name,pair,moff,skips in CONFIGS:
        SEL['motif_off']=moff; SEL['skipset']=skips
        SEL['clsdmg']=True; SEL['cap1416']=True; SEL['cap_pair']=pair; SEL['mean_abl']=None
        print(f'== PAIR {name}: base (capture means) ==',flush=True)
        rb=main()
        MUa=torch.cat(SEL['y14']).mean(0).to(DEV); MUb=torch.cat(SEL['y16']).mean(0).to(DEV)
        if name=='b23': base23=rb['L2_F']
        SEL['clsdmg']=False; SEL['cap1416']=False
        arms={}
        for tag,mab in (('meanA',{pair[0]:MUa}),('meanB',{pair[1]:MUb}),('meanBoth',{pair[0]:MUa,pair[1]:MUb})):
            SEL['mean_abl']=mab
            print(f'== PAIR {name}: {tag} ==',flush=True)
            ra=main()
            d=ra['L2_F']-rb['L2_F']
            if abs(d)<1e-5:
                raise SystemExit(f'INSTRUMENT FAIL: {name}/{tag} identical to base')
            arms[tag]=round(d,4)
        SEL['mean_abl']=None
        arms['ratio']=round(arms['meanBoth']/max(arms['meanA']+arms['meanB'],1e-9),3)
        allres[name]={'base_L2F':rb['L2_F'],**arms}
        print(f'{name}: A {arms["meanA"]:+.4f} B {arms["meanB"]:+.4f} both {arms["meanBoth"]:+.4f} ratio {arms["ratio"]}',flush=True)
    pa=(allres['b23']['meanBoth']<=0.5*ZREF['b23']) and (allres['p1416']['meanBoth']<=0.5*ZREF['p1416'])
    pb=all(allres[n]['meanBoth']>=1.15*(allres[n]['meanA']+allres[n]['meanB']) for n in allres)
    pc=abs(base23-2.4410)<=0.01
    res={'pairs':allres,'zero_refs':ZREF,
         'convention':'damage on each leave-pair-real base; bases excluded; lower L2 better',
         'pred_a_mean_term_dominated':bool(pa),'pred_b_backup_survives':bool(pb),'pred_c_reproduces_r86_base':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    for n in allres: print(f"{n}: {allres[n]}")
    print(f"(a) mean-both <= 0.5 x zero-both (b23, 1416): {'HELD' if pa else 'FAILED'}")
    print(f"(b) super-additivity survives at all three pairs: {'HELD' if pb else 'FAILED'}")
    print(f"(c) b23 base {base23} vs 2.4410 (tol 0.01): {'HELD' if pc else 'FAILED'}")
