#!/usr/bin/env python
"""late_stack_width_and_token_rank_probe -- prices the two width knobs of the seven-block late program (own weights on each block's top-k
input PCs + token/constant filler, §2739): the PC width k beyond 256, and the rank of the per-block token read. Plus one allocation arm.
CUDA lane-1 script.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_all7_tok_512 pred_c_all7_tok_768 pred_d_token_read_rank_256_enough
#                     pred_e_token_read_is_high_rank
...
Preregistration: polynomial_causal/LATE_STACK_WIDTH_AND_TOKEN_RANK_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_stack_width_and_token_rank_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_stack_width_and_token_rank_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_STACK_WIDTH_AND_TOKEN_RANK_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_stack_constant_filler_probe_results.json"   # §2739
OUT = ROOT / "late_stack_width_and_token_rank_probe_results.json"
HASHES = {PREREG: "d19fb2d1a68b88a23dbec81e440866c663d268c49ee1cc899d3129f49b415163", PRIOR: "3b402bb3ccebd31396d3ad58c55e28e99f6596516a3183f8d7acdbd81be94f67",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_stack_width_and_token_rank_probe"
...
KS = [128, 256, 384, 512, 768]; KC = [512, 768]; RS = [16, 64, 256]; KPOOL_ALLOC = 512; KLAST_ALLOC = 256
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_max": 0.20, "c_max": 0.12, "d_max": 0.03, "e_min": 0.05}
NULLS = {"b_min": 0.28, "c_min": 0.20, "d_min": 0.07, "e_max": 0.02}
...
def main():
    ...
    class OwnHead:
        ...
        plus the ridge token read ('tok', §2730 recipe), optionally with the token map truncated to rank r; output unrestricted."""
        def __init__(self, l, k, fill, r=None):
            h = heads[("mlp", l)]; self.mlp = m.transformer.h[l].mlp; self.U = h["U"][:, :k]; self.mx = h["mx"]; self.fill = fill
            self.A = h["Afull"] if r is None else h["A_r"][r]
        def hook(self, idx):
            self.idx = idx
        def __call__(self, w, x):
    ...
    for s_ in LATE7:
        Us_, Ss_, Vh_ = torch.linalg.svd(heads[s_]["Afull"].double())
        heads[s_]["A_r"] = {r: ((Us_[:, :r] * Ss_[:r]) @ Vh_[:r]).float() for r in RS}
        heads[s_]["A_spec"] = R.spectrum(heads[s_]["Afull"].double().cpu().T @ heads[s_]["Afull"].double().cpu())
    log(stage="token_map_svd", **{f"A_eff_rank_{s_[1]}": round(heads[s_]["A_spec"]["eff_rank"], 1) for s_ in LATE7})
    def run(parts):
        patch = {}; objs = []
        for p_ in parts:
    ...
    pool_part = lambda k, fill, r=None: {("mlp", l): OwnHead(l, k, fill, r) for l in range(11, 16)}
    last_part = lambda k, fill, r=None: {("mlp", l): OwnHead(l, k, fill, r) for l in (16, 17)}
    arms = {}
    for k in KS:
        arms[f"ALL7_TOK_{k}"] = run([pool_part(k, "tok"), last_part(k, "tok")])
    for k in KC:
        arms[f"ALL7_CONST_{k}"] = run([pool_part(k, "const"), last_part(k, "const")])
    for r in RS:
        arms[f"ALL7_TOK256_r{r}"] = run([pool_part(256, "tok", r), last_part(256, "tok", r)])
    arms[f"POOL{KPOOL_ALLOC}+LAST2_{KLAST_ALLOC}_TOK"] = run([pool_part(KPOOL_ALLOC, "tok"), last_part(KLAST_ALLOC, "tok")])
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    rank_cost = {f"r{r}_minus_full": arms[f"ALL7_TOK256_r{r}"] - arms["ALL7_TOK_256"] for r in RS}
    ratios = {f"tok_{KS[i]}_over_{KS[i - 1]}": (arms[f"ALL7_TOK_{KS[i]}"] / arms[f"ALL7_TOK_{KS[i - 1]}"] if arms[f"ALL7_TOK_{KS[i - 1]}"] > 0 else None) for i in range(1, len(KS))}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["ALL7_TOK_256"] - PRIOR_ALL7_TOK_256) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_all7_tok_512': bool(arms["ALL7_TOK_512"] <= BARS["b_max"]),
        'pred_c_all7_tok_768': bool(arms["ALL7_TOK_768"] <= BARS["c_max"]),
        'pred_d_token_read_rank_256_enough': bool(rank_cost["r256_minus_full"] <= BARS["d_max"]),
        'pred_e_token_read_is_high_rank': bool(rank_cost["r64_minus_full"] >= BARS["e_min"]),
    }
    nulls = {"b_null_tok512_ge_.28": bool(arms["ALL7_TOK_512"] >= NULLS["b_min"]), "c_null_tok768_ge_.20": bool(arms["ALL7_TOK_768"] >= NULLS["c_min"]),
             "d_null_r256_cost_ge_.07": bool(rank_cost["r256_minus_full"] >= NULLS["d_min"]), "e_null_r64_cost_le_.02": bool(rank_cost["r64_minus_full"] <= NULLS["e_max"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "ks": KS, "ks_const": KC, "token_ranks": RS,
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "all7_tok_256": arms["ALL7_TOK_256"], "prior_all7_tok_256": PRIOR_ALL7_TOK_256},
           "ce_added": arms, "token_rank_cost": rank_cost, "tok_curve_ratios": ratios,
           "token_map_eff_rank": {str(s_[1]): heads[s_]["A_spec"] for s_ in LATE7},
    ...
    if smoke:
        print(json.dumps({k: out[k] for k in ("preds", "nulls", "instrument", "ce_added", "token_rank_cost", "tok_curve_ratios", "price")}, indent=1)); return
    from receipt import dump
    dump(out, OUT)
    print(json.dumps({"preds": preds, "nulls": nulls, "ce_added": arms, "token_rank_cost": rank_cost, "tok_curve_ratios": ratios}, indent=1))
