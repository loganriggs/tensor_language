"""EXACT-FRONT CLASS ATTRIBUTION (rung 136): are name/subword the CP-front's bill?

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows; LOWER IS BETTER.
S2228: the frontier pays name +3.13 and subword +2.32 - the classes the dense early MLPs assemble (S2205).
Here the identical build with the front at K=4608 (EXACT front; everything else unchanged) re-derives the
class table: whatever damage collapses is the front pruning's bill, class by class.

REGISTERED PREDICTIONS (anchors: S2228 class table; front solo at 3456 = +0.1193):
  (a) NAME IS FRONT-CAUSED: dCE(name) <= 1.6 (halves from +3.134).
  (b) SUBWORD IS FRONT-CAUSED: dCE(subword) <= 1.2 (halves from +2.319).
  (c) SANITY: census agg in [1.6, 1.9].
NULL: name/subword persist (>= 80% of S2228 values) - the assembly damage is motif/tail-caused and the next
move is NOT front capacity. PRICE: diagnostic arm at 63.7M front values (not a frontier claim).
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: exact-front class attribution')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_front4608_classes_results.json'
...
if __name__=='__main__':
    ...
    print('ARM: rung-127 build with EXACT front (K=4608), class table saved',flush=True)
    main()
    ...
    print(f"(a) name {PC.get('name',9.9):+.4f} <= 1.6: {'HELD' if pa else 'FAILED'}")
    print(f"(b) subword {_sw:+.4f} <= 1.2: {'HELD' if pb else 'FAILED'}")
    print(f"(c) census {agg:+.4f} in [1.6, 1.9]: {'HELD' if pc else 'FAILED'}")
