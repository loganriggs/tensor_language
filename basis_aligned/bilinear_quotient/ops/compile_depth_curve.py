# HOW MUCH OF THE MODEL CAN BE COMPILED?  -- the question §1804 ended on.
#
# §1804's prefix arms were a by-product of a cliff probe at four sampled depths, and they turned out to
# be the most informative thing in that section: holding depths 0..L live and compiling the rest gives
# +0.34pp at L4, +2.39 at L5, +3.81 at L6 and +19.29 at L13 -- that last is 32.84% against the live
# model's 39.32%, so compiling only the TOP FOUR layers costs 6.5pp where compiling all eighteen costs
# 25.8. This runs the curve at all eighteen depths.
#
# The question matters because every compilation figure in §1747-§1802 is for the ALL-sites program.
# A partial compile is a different and more useful object -- it says how much of a 546M transformer can
# be replaced by per-token tables before the cost becomes real -- and nothing in the thread has measured
# it as a curve.
#
# The live-vs-row norm diagnostic of §1804 is retained: it exceeded 1 at all eighteen layers (2.71 to
# 152.62), so the compiled program runs at a systematically smaller residual scale than the model, and
# the curve should be read next to it.
#
...
#   pred_a COMPILATION IS EXPENSIVE UNTIL VERY LATE: recovering half the 25.77 / 28.10 / 25.24 pp gap
#          requires at least twelve live layers (L >= 12), at every role. If FALSE, half the gap is
#          reachable with fewer live layers than that and a partial compile is a far better deal than
#          the all-sites figures suggest -- which would make the partial program, not the full one, the
#          object this thread should be characterising.
#   pred_b AND THE COST IS CONCENTRATED LATE: the gap-fraction gained from L9 to L13 exceeds that gained
#          from L5 to L9, at every role. If FALSE the curve is concave and early layers are the
#          expensive ones, which would contradict the shape §1804's four points suggested and mean those
#          points were sampled misleadingly.
#   pred_c THE CLIFF LAYER IS NOT SPECIAL HERE: the L5 prefix arm beats the L4 prefix arm, at every
#          role. §1804 showed L5's live module EXPLODES when fed a compiled stream (norm ratio 152.6);
#          in the prefix arm it is fed a LIVE stream instead, so it should behave. If FALSE, L5 damages
#          the program even on its own natural input and the cliff is a property of the layer rather
#          than of the mismatch -- a materially different reading of §1804.
#   pred_d CONTROLS, cross-run per LESSON 42: the all-substituted and live arms reproduce §1789's
#          PUBLISHED 0.1355 / 0.1425 / 0.1364 and 0.3932 / 0.4235 / 0.3888 within 0.001, and the L4, L5,
#          L6 and L13 prefix arms reproduce §1804's PUBLISHED deltas within 0.5pp; coverage 5419.
...
OUT = PT + 'ops/compile_depth_curve_results.json'
PROBE_LS = tuple(range(18))
S1804_PREFIX = {'skip7000': {4: 0.0034, 5: 0.0239, 6: 0.0381, 13: 0.1929},
                'skip11000': {4: 0.0047, 5: 0.0229, 6: 0.0362, 13: 0.2073},
                'skip1200': {4: 0.0040, 5: 0.0211, 6: 0.0329, 13: 0.1872}}
...
def main():
    ...
    print(f'COMPILE-DEPTH CURVE | depths 0..L live and L+1..17 compiled, all 18 L, plus the '
    ...
    for L in PROBE_LS:
        ...
        # PREFIX: depths 0..L live, depths L+1..17 compiled -- compile from the TOP DOWN
    ...
    # recovered fraction of the full gap at each compile depth
    frac = {e: {L: d(e, f'L{L}_prefix') / gap[e] for L in PROBE_LS} for e in roles}
    # the CHEAPEST useful depth: smallest L whose prefix recovers >=50% of the gap
    half = {e: next((L for L in PROBE_LS if frac[e][L] >= 0.5), None) for e in roles}
    pa = all(half[e] is not None and half[e] >= 12 for e in roles)
    pb = all(frac[e][13] - frac[e][9] > frac[e][9] - frac[e][5] for e in roles)
    pc = all(d(e, 'L5_prefix') > d(e, 'L4_prefix') for e in roles)
    pd = (all(abs(res[e]['all_substituted']['top1'] - S1789_PROG[e]) <= 0.001
              and abs(res[e]['live_model']['top1'] - S1789_LIVE[e]) <= 0.001 for e in roles)
          and all(abs(d(e, f'L{L}_prefix') - S1804_PREFIX[e][L]) <= 0.005
                  for e in roles for L in (4, 5, 6, 13))
          and NFULL == NCOV)

    print('\n  compile-from-the-top curve: depths 0..L live, L+1..17 compiled', flush=True)
    print('     L   ' + '  '.join(f'{e:>22s}' for e in roles), flush=True)
    for L in PROBE_LS:
        print(f'    {L:2d}   ' + '  '.join(
            f'{100*d(e, f"L{L}_prefix"):+7.2f}pp ({frac[e][L]:6.1%})' for e in roles), flush=True)
    print(f'\n  half the gap needs at least 12 live layers -> {pa}  ' + '  '.join(
        f'{e} L{half[e]}' for e in roles), flush=True)
    print(f'  the curve ACCELERATES with depth (L9->L13 gains more than L5->L9) -> {pb}  '
          + '  '.join(f'{e} {100*(frac[e][13] - frac[e][9]):+.1f} vs '
                      f'{100*(frac[e][9] - frac[e][5]):+.1f} points of gap' for e in roles),
          flush=True)
    print(f'  L5 live beats L4 live despite L5 being the cliff layer -> {pc}  ' + '  '.join(
        f'{e} {100*d(e, "L5_prefix"):+.2f} vs {100*d(e, "L4_prefix"):+.2f}pp' for e in roles),
        flush=True)
    ...
    json.dump({'run': 'l5_cliff_probe',
               ...
               'recovered_fraction': {e: {str(L): frac[e][L] for L in PROBE_LS}
                                      for e in roles},
               'half_gap_depth': {e: half[e] for e in roles},
               'predictions': {'pred_a_half_the_gap_needs_12_layers': bool(pa),
                               'pred_b_curve_accelerates_with_depth': bool(pb),
                               'pred_c_L5_prefix_beats_L4_prefix': bool(pc),
