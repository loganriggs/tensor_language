#!/usr/bin/env python3
"""Locality-density profile of the MLP1 write restoration.
...
OUT = ROOT / "mlp1_write_locality_density_profile_results.json"
BUNDLE = ROOT / "mlp1_write_locality_density_profile_per_token.pt"
HASHES = {
    PREREG: "01ee0350e6b09273fbe4c760277144347b01217f811f2344ea4dd2bc0642b675",
...
MIN_SCORED_PER_CELL_LOWEST = 3000
...
def main():
    ...
    if os.environ.get("BQLIB_DRYRUN") == "1":
        ...
        for path, expected in HASHES.items():
            if not path.is_file() or r493.sha256(path) != expected:
                raise RuntimeError(f"frozen hash mismatch: {path}")
        print(json.dumps({
            "status": "dry_run_passed", "rung": "mlp1_write_locality_density_profile",
    ...
    if OUT.exists() or BUNDLE.exists():
        raise RuntimeError("locality profile output namespace already exists")
    ...
    pred_a = bool(
        ...
        and instrument["min_lowest_density_positions_per_doc"] >= 1
    ...
    result = {
        "status": "complete", "rung": "mlp1_write_locality_density_profile",
