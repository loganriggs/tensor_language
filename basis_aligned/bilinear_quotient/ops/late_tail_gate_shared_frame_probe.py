#!/usr/bin/env python
"""late_tail_gate_shared_frame_probe -- is the late tail-read GATE FRAME SHARED across the late stack? §2804-§2805: each late MLP's tail
read is controlled by a gate on the core that truncates to 16-128 whitened modes per block, and the compression composes (full ten-block
program .0722) -- but every frame was fitted PER BLOCK. This rung asks whether one core subspace controls all ten tail reads: weight-side,
the adjacent blocks' read spaces span(M_c^{-1/2} V_k) either overlap far above a random pair (64/768 = .083) or not; CE-side, ONE pooled
frame (mean core moment, trace-normalised summed Grams) applied at all ten blocks at 32/64/128/256 modes is priced against the own
128-frames (§2799 .0627), against the end blocks' frames transferred to the whole stack, and against a seeded random frame. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_adjacent_read_spaces_overlap pred_c_shared_128_frame_prices_near_own_128
#                     pred_d_shared_beats_random pred_e_shared_256_frame_covers_the_stack
...
Preregistration: polynomial_causal/LATE_TAIL_GATE_SHARED_FRAME_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_tail_gate_shared_frame_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_tail_gate_shared_frame_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_TAIL_GATE_SHARED_FRAME_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_tail_gate_program_composition_probe_results.json"   # §2805
OUT = ROOT / "late_tail_gate_shared_frame_probe_results.json"
HASHES = {PREREG: "48c9d0707b7e3551bffc950389d792497d0db82dc92e52b208e810bc7186681d", PRIOR: "517e858bfa6e45882b79a29503543179a391ed9d575443dde27db7d6243dffea",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_tail_gate_shared_frame_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_SPLIT8_1024 = 0.0374; PRIOR_LATE_MLP_768 = 0.1249; PRIOR_EARLY_TAIL_ONLY = 0.0711; PRIOR_JOINT_GATE0 = 0.3668; PRIOR_GATE128 = 0.0627; PRIOR_SUM_GATE0 = 0.1895; PRIOR_B8_GATE0 = 0.0304; PRIOR_B9_GATE0 = 0.0209; PRIOR_B16_GATE0 = 0.0181; PRIOR_B17_GATE0 = 0.0458; PRIOR_B17_GATE16 = 0.0083; PRIOR_B16_GATE64 = 0.0051; PRIOR_TWO_BLOCK = 0.01491; PRIOR_FULL = 0.0722; ALT = (64, 96); LAM = 1e-2
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "exact_tol": 1e-3, "gram_tol": 1e-6, "frame_tol": 1e-6, "floor": 0.002, "b_ov": 0.5, "c_margin": 0.05, "d_margin": 0.05, "e_margin": 0.02}
NULLS = {"b_ov": 0.2, "c_margin": 0.10, "d_margin": 0.01, "e_margin": 0.05}
...
def main():
    ...
    pooled_spec = R.spectrum(Gwp.cpu()); pooled_captured = {k: float(evp[:k].sum() / evp.sum()) for k in (16, 32, 64, 128, 256)}
    def pi_shared(k):
    ...
    frames = {"SHARED_32": pi_shared(32), "SHARED_64": pi_shared(64), "SHARED_128": pi_shared(128), "SHARED_256": pi_shared(256),
              "TRANSFER_17_128": pi_own(NL - 1, 128), "TRANSFER_8_128": pi_own(SPLIT, 128), "RANDOM_128": pi_rand}
    ret = {nm: {str(l): retained(l, Pi) for l in LB} for nm, Pi in frames.items()}
    ret["OWN_128"] = {str(l): retained(l, pi_own(l, 128)) for l in LB}
    frame_check = max(abs(ret["OWN_128"][str(l)] - gates[l]["captured"][128]) for l in LB)
    shared_read = {k: orth(Mcb_ih @ Vp[:, :k]) for k in (128, 256)}
    own64_in_shared = {str(k): {str(l): overlap(read_space(l, 64), shared_read[k]) for l in LB} for k in (128, 256)}
    log(stage="frames", adj64=[round(v, 3) for v in adj[64].values()], adj_rand=adj_rand, far=far, pooled_captured=pooled_captured, frame_check=frame_check,
        ret_shared128=[round(ret["SHARED_128"][str(l)], 3) for l in LB], ret_own128=[round(ret["OWN_128"][str(l)], 3) for l in LB])
    class GateP(Gate):
        """MLP l with the cross term gated by a SUPPLIED core projector Pi (shared / transferred / random frame) instead of its own modes."""
    ...
    ov64_med = med(list(adj[64].values()))
    summ = {"adjacent_overlap": {str(k): adj[k] for k in KS}, "adjacent_overlap_median": {str(k): med(list(adj[k].values())) for k in KS},
            "random_pair_overlap": {str(k): adj_rand[k] for k in KS}, "random_pair_expected": {str(k): k / 768 for k in KS}, "overlap_8_vs_17": {str(k): far[k] for k in KS},
            "pooled_gram": {"eff_rank": pooled_spec["eff_rank"], "rank_90": pooled_spec["rank_90"], "captured": {str(k): v for k, v in pooled_captured.items()}},
            "retained_energy_per_block": ret, "own_captured_128_2799": {str(l): gates[l]["captured"][128] for l in LB}, "own64_read_space_inside_shared": own64_in_shared,
            "shared_128_minus_uniform_128": arms["SHARED_128"] - arms["UNIFORM_128"], "shared_256_minus_uniform_128": arms["SHARED_256"] - arms["UNIFORM_128"],
            "random_128_minus_shared_128": arms["RANDOM_128"] - arms["SHARED_128"], "prior_uniform_gate128_2799": PRIOR_GATE128, "prior_full_program_2805": PRIOR_FULL,
            "direct_check_rel_err": {"8": gates[SPLIT]["check"], "17": gates[NL - 1]["check"]}, "frame_check": frame_check, "n_fit_tokens": rec[("n", SPLIT)]}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(arms["GATE_EXACT"]) <= BARS["exact_tol"]
                   and abs(arms["UNIFORM_128"] - PRIOR_GATE128) <= BARS["repro_tol"] and max(gates[SPLIT]["check"], gates[NL - 1]["check"]) <= BARS["gram_tol"]
                   and frame_check <= BARS["frame_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_adjacent_read_spaces_overlap': bool(ov64_med >= BARS["b_ov"]),
        'pred_c_shared_128_frame_prices_near_own_128': bool(arms["SHARED_128"] - arms["UNIFORM_128"] <= BARS["c_margin"]),
        'pred_d_shared_beats_random': bool(arms["RANDOM_128"] - arms["SHARED_128"] >= BARS["d_margin"]),
        'pred_e_shared_256_frame_covers_the_stack': bool(arms["SHARED_256"] - arms["UNIFORM_128"] <= BARS["e_margin"]),
    }
    nulls = {"b_null_overlap_le_.2": bool(ov64_med <= NULLS["b_ov"]),
             "c_null_shared128_ge_uniform_plus_.10": bool(arms["SHARED_128"] - arms["UNIFORM_128"] >= NULLS["c_margin"]),
             "d_null_random_minus_shared_le_.01": bool(arms["RANDOM_128"] - arms["SHARED_128"] <= NULLS["d_margin"]),
             "e_null_shared256_ge_uniform_plus_.05": bool(arms["SHARED_256"] - arms["UNIFORM_128"] >= NULLS["e_margin"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "one pooled core frame (mean core second moment; trace-normalised summed exact gate Grams) gating every late MLP's tail-read cross term at 32/64/128/256 modes, against the own per-block 128-frames (§2799), the end blocks' frames transferred to the stack, and a seeded random frame; weight-side overlap of adjacent blocks' read spaces",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "gate_exact": arms["GATE_EXACT"],
                          "uniform_128": arms["UNIFORM_128"], "prior_uniform_128": PRIOR_GATE128, "direct_check_rel_err": summ["direct_check_rel_err"], "frame_check": frame_check},
