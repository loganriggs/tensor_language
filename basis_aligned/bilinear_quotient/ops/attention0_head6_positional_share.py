"""RUNG 428 (Claude red-team lane) -- IS HEAD 6 THE POSITIONAL HEAD? A REGISTERED MECHANISM TEST.
...
Frozen predictions
------------------
pred_a (instrument): float64 fold gate <= 1e-10; shuffled-pair control
    token-share <= .01 for every head; cross-sample Spearman of head
    offset-shares >= .8 per branch.
pred_b (positional maximum): head 6's branch-mean offset-share is the
    MAXIMUM of the 9 heads AND >= 1.5x the median head's, on BOTH pair
    samples.
pred_c (complement): head 6's branch-mean token-share is the MINIMUM of
    the 9 heads on both samples AND its offset-share >= .30 absolute.

Null: head 6 is not the offset-share maximum on either sample, or its
offset-share < .15 -- the carrier-outsider status is NOT positional and
the explanation must lie elsewhere (e.g., a private lexical family).

Price: identification screen only; no shipped object; no prior bar
(423/423b/426) is altered by any outcome.
"""
...
OPS = BQ / "ops"
OUT = BQ / "attention0_head6_positional_share_results.json"
...
def main() -> None:
    if os.environ.get("BQLIB_DRYRUN") == "1":
        assert TARGET_HEAD == 6 and PAIR_COUNT == 32_768
        assert 426_777 not in SEEDS and len(set(SEEDS)) == 2
        assert SV.exists() and ROWS_RECEIPT.exists()
        print("ATTENTION0 HEAD6 POSITIONAL SHARE | dry run: two-way variance, two samples, controls")
        return
    ...
    for seed in SEEDS:
        ...
        query = select_ids[torch.randint(
            len(select_ids), (PAIR_COUNT,), generator=generator).to(device)]
    ...
    pred_b = all(
        ...
        and offset_vectors[seed][TARGET_HEAD]
            >= 1.5 * median(offset_vectors[seed])
        for seed in SEEDS)
    pred_c = all(
        is_min(token_vectors[seed], TARGET_HEAD)
        and offset_vectors[seed][TARGET_HEAD] >= .30
        for seed in SEEDS)
    null = (
        not any(is_max(offset_vectors[seed], TARGET_HEAD) for seed in SEEDS)
        or max(offset_vectors[seed][TARGET_HEAD] for seed in SEEDS) < .15)

    result = {
        "status": "attention0_head6_positional_share_complete",
        "rung": 428,
        ...
        'pred_a_exact_instrument_and_stable_estimator': bool(pred_a),
        'pred_b_head6_is_positional_maximum': bool(pred_b),
        'pred_c_head6_token_minimum_and_absolute_bar': bool(pred_c),
        'null_head6_outsider_status_not_positional': bool(null),
        "FINAL_opened": 0,
