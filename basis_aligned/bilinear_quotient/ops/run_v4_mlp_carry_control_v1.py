...
"""V4_MLP_CARRY_CONTROL_V1_PREREGISTRATION.md; unchanged oracle source edits."""
import os,sys,json,time,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';PLAN=dict(prefix=12,native_suffix=16,joined_suffix=36)
def main():
 ...
 out=A/'v4_mlp_carry_control_v1_result.json';assert not out.exists();choicepath=P/'V4_FINITE_REFERENCE_CHOICES_V1.json';choices=json.loads(choicepath.read_text());prior=json.loads((A/'v4_finite_reference_v1_result.json').read_text());contexts=[];composition=json.loads((A/'v4_selective_composition_v1_result.json').read_text());field_errors=[];programs=[];cores=[];local_errors=[];export=None
 ...
 for context in contexts:
  ...
  for label,drop in [(10.8,'quadratic'),(10.9,'linear'),(10.95,'all')]:
   ...
   elif drop=='linear':reduced['coefficients'][:,:,:27]=0
   else:reduced['coefficients'].zero_()
  ...
  for family in dict.fromkeys(families):
   ...
   for boundary,estimate in joins.items():
    ...
    records.append(dict(panel=context['panel'],family=family,boundary=boundary,remaining_blocks=6 if boundary in [10.5,10.6,10.7,10.8,10.9,10.95] else 17-boundary,first_mlp=boundary in [10.5,10.6,10.7,10.8,10.9,10.95],errors=errors.tolist(),weak_norm=float(weak),passed=bool(errors[0]<=.1 and errors[1:].max()<=.05)))
 ...
 passing=[boundary for boundary in [10,10.5,10.6,10.7,10.8,10.9,10.95,11,17] if all(r['passed'] for r in records if r['boundary']==boundary)]
 result=dict(max_local_response_error=max(local_errors),response_values_per_context=sum(v.numel() for v in export['program'].values()),plan=PLAN,counts=counts,max_replay=max(replays),max_field_check=max(field_errors),records=records,passing_boundaries=passing,predictions=dict(pred_a_instrument=instrument,pred_b_carry_sufficient=instrument and 10.95 in passing,pred_c_linear_more_precise=instrument and max(max(r['errors']) for r in records if r['boundary']==10.8)<max(max(r['errors']) for r in records if r['boundary']==10.95)),seconds=time.perf_counter()-tic)
