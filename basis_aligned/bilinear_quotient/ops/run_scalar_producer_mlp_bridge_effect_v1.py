...
pred_b EACHtemplate direct+mixed serial effect error<=10%relative.
pred_c EACHtemplate direct-only serial effect error>=25%relative.
...
STEM='SCALAR_PRODUCER_MLP_BRIDGE_EFFECT_V1'
@torch.no_grad()
def main():
 ...
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('240bodyforwards48rows; exact/mixed/direct bridge signed effect validation');return
 ...
 predictions=torch.load(P/'SCALAR_PRODUCER_MLP_BRIDGE_NATIVE_V1_ARTIFACT.pt',weights_only=True,map_location='cpu')['predictions'].cuda()
 ...
 def hook(index,module,args,output):
  ...
  if context['arm']>=2 and index==1:
   scalar=predictions[{2:1,3:2,4:5}[context['arm']],context['row'],:args[0].shape[1]][None,:]
 ...
 result={'pred_a':A,'pred_b':A and all(c['serial_effect_errors'][1]<=.1 for c in cells),'pred_c':A and all(c['serial_effect_errors'][2]>=.25 for c in cells),'checks':checks,'cells':cells,'arms':['native','dynamicjoint','exactbridge','directmixed','directonly'],'body_forwards':counts,'seconds':time.perf_counter()-tic,'scope':'Conditional bridge scalar generation tested at signed native serial-effect boundary; reused contexts, external native inputs/background remain. No newline repair or global extraction.','source_shas':binding}
