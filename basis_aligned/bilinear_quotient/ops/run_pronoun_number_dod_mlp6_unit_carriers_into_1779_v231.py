#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_carrier_closure pred_b_top_10_units_carry_half pred_c_trio_carries_against pred_d_trio_share_at_least_010
"""Pronoun number they/he DoD (v231): does the MLP-6 TRIO {2483, 2826, 4131} damp MLP-7 unit 1779 while the rest of MLP 6 feeds it? v230: MLP 6 as a block
carries 1779's number contrast positively (+26%); v228: removing the trio RAISES 1779's contribution to the plural detector 829 (40% of MLP 7's push-back).
The two are consistent only if the trio's own carriage into 1779 has the opposite sign to MLP 6's total (or the response is not through 1779's inputs).
Carrier-level unit census (v192's identity) of MLP 6 into 1779 at the noun; the trio's summed carrier share is the registered quantity.
PREDICTIONS (scored as written; failures preserved; priors unsure)
    ...
    pred_b_top_10_units_carry_half   top-10 |pooled| units carry >= 0.50 of MLP 6's carrier total (in magnitude of the signed sum)
    pred_c_trio_carries_against      the trio's summed carrier term has the sign opposite to MLP 6's total (it damps 1779)
    pred_d_trio_share_at_least_010   |trio carrier| >= 0.10 x |MLP 6 total|
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_mlp6_unit_carriers_into_1779_v231_result.json"
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_mlp6_unit_carriers_into_1779_v231"
UNIT, SRC, DST, CLOSURE_TOL, TOP10_MIN, TRIO_MIN, BATCH = 1779, 6, 7, 1e-2, 0.50, 0.10, 32
TRIO = (2483, 2826, 4131)
FORWARDS_MAX = 5
PREDICTIONS = {"pred_a_carrier_closure": "<= 1e-2", "pred_b_top_10_units_carry_half": ">= 0.50", "pred_c_trio_carries_against": "opposite sign", "pred_d_trio_share_at_least_010": ">= 0.10"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "rows": len(rows), "rows_sha256": L.rows_sha256(rows), "unit": UNIT, "source_layer": SRC, "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"closure_tol": CLOSURE_TOL, "top10_min": TOP10_MIN, "trio_min": TRIO_MIN}, "trio": list(TRIO)}
    ...
    trio = sum(float(carrier[j]) for j in TRIO); trio_share = trio / total
    print("trio carrier", round(trio, 3), "share of MLP-6 total", round(trio_share, 4), "per unit", {j: round(float(carrier[j]), 3) for j in TRIO})
    predictions = {"pred_a_carrier_closure": closure <= CLOSURE_TOL, "pred_b_top_10_units_carry_half": share(10) >= TOP10_MIN, "pred_c_trio_carries_against": trio * total < 0, "pred_d_trio_share_at_least_010": abs(trio_share) >= TRIO_MIN}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_mlp5_unit_carriers_into_1779_result_v231", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "carrier_total": total, "shares": {str(k): share(k) for k in (5, 10, 20, 50, 100, 500)},
