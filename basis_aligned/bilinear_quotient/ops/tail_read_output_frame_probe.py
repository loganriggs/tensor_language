#!/usr/bin/env python
"""tail_read_output_frame_probe -- WHERE does the core-gated linear read of the tail WRITE? Its output Down[Lc*Rt + Lt*Rc] (§2780/§2783) is
split into the part inside the reader's own read frame (the bus U_8 top-768 late; the own top-512 frame early) and the part outside it;
each part kept alone, tail*tail dropped throughout. Fit-set energies of the two parts recorded from an exact forward. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_in_frame_output_carries_most_late pred_c_out_of_frame_output_alone_small_late
#                     pred_d_parts_near_additive_late pred_e_in_frame_output_carries_most_early
...
Preregistration: polynomial_causal/TAIL_READ_OUTPUT_FRAME_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/tail_read_output_frame_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("tail_read_output_frame_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "TAIL_READ_OUTPUT_FRAME_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "all18_tail_linear_program_probe_results.json"   # §2784
OUT = ROOT / "tail_read_output_frame_probe_results.json"
HASHES = {PREREG: "02b3ac5b386922c34b1a42b27376ef92a3ecf288a18c383fa2fdcb6abec06f63", PRIOR: "8b4fc5553b4c500ec98891fd48095f1863975f2dd226cb4cbd3bbc74a9e340d6",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "tail_read_output_frame_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_SPLIT8_1024 = 0.0374; PRIOR_LATE_MLP_768 = 0.1249; PRIOR_LATE_MLP_896 = 0.0662; PRIOR_DROP_TT_768 = 0.0087; PRIOR_EARLY_DROP_TT_768 = 0.0023; LAM = 1e-2
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_min": 0.60, "c_max": 0.40, "d_lo": 0.7, "d_hi": 1.3, "e_min": 0.60}
NULLS = {"b_max": 0.35, "c_min": 0.60, "d_max": 0.5, "e_max": 0.35}
...
def main():
    ...
    def split8(k):
        ...
        return run(patch)
    class OutHead(OwnHead):
        """MLP l reads a k-frame (own if U_ is None, else U_); c = in-frame input (rest constant), t = the tail; tail*tail dropped.
        The cross term's OUTPUT Down[Lc*Rt + Lt*Rc] is kept whole ('all'), only its part inside the frame ('in'), only outside ('perp'),
        or dropped ('none'); rec accumulates the fit-set energies of the two parts (exact forward)."""
        def __init__(self, l, k, mode, U_=None, rec=None):
            super().__init__(l, k, "const", None if U_ is None else U_[:, :k]); self.mode = mode; self.rec = rec
        def __call__(self, w, x):
            ...
            co = self.mlp.Down(Lc * Rt + Lt * Rc); ci = (co @ self.U) @ self.U.T; cp = co - ci
            if self.rec is not None:
                self.rec["in"] += float((ci.double() ** 2).sum()); self.rec["perp"] += float((cp.double() ** 2).sum()); self.rec["n"] += ci.shape[0] * ci.shape[1]
                return w
            base = self.mlp.Down(Lc * Rc)
            return base + (co if self.mode == "all" else ci if self.mode == "in" else cp if self.mode == "perp" else 0.0)
    recL = {"in": 0.0, "perp": 0.0, "n": 0}; recE = {"in": 0.0, "perp": 0.0, "n": 0}
    for i in range(0, fit_rows.shape[0], CH):
        idx = fit_rows[i:i + CH, :TI]
        pt = {("mlp", l): OutHead(l, 768, "all", U_8, recL) for l in range(SPLIT, NL)}; pt.update({("mlp", l): OutHead(l, 512, "all", None, recE) for l in range(0, SPLIT)})
        for o in pt.values():
            o.hook(idx)
        forward(m, idx, patch=pt)
    inshare_late = recL["in"] / max(recL["in"] + recL["perp"], 1e-12); inshare_early = recE["in"] / max(recE["in"] + recE["perp"], 1e-12)
    log(stage="cross_output_energy", in_share_late_768=round(inshare_late, 3), in_share_early_512=round(inshare_early, 3))
    def late(mode):
        return run({("mlp", l): OutHead(l, 768, mode, U_8) for l in range(SPLIT, NL)})
    def early(mode):
        return run({("mlp", l): OutHead(l, 512, mode) for l in range(0, SPLIT)})
    arms = {"SPLIT8_1024": split8(1024), "LATE_DROP_TT_768": late("all"), "LATE_MLP_768": late("none"), "LATE_CROSS_IN_768": late("in"), "LATE_CROSS_PERP_768": late("perp")}
    if not smoke:
        arms["EARLY_DROP_TT_512"] = early("all"); arms["EARLY_MLP_512"] = early("none"); arms["EARLY_CROSS_IN_512"] = early("in"); arms["EARLY_CROSS_PERP_512"] = early("perp")
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    def recov(j, full, arm):
        g = arms[j] - arms[full]; return (arms[j] - arms[arm]) / g if g > 1e-9 else None
    rin = recov("LATE_MLP_768", "LATE_DROP_TT_768", "LATE_CROSS_IN_768"); rperp = recov("LATE_MLP_768", "LATE_DROP_TT_768", "LATE_CROSS_PERP_768")
    ein = recov("EARLY_MLP_512", "EARLY_DROP_TT_512", "EARLY_CROSS_IN_512") if "EARLY_CROSS_IN_512" in arms else None
    eperp = recov("EARLY_MLP_512", "EARLY_DROP_TT_512", "EARLY_CROSS_PERP_512") if "EARLY_CROSS_PERP_512" in arms else None
    summ = {"late_768": {"recovery_in": rin, "recovery_perp": rperp, "sum": (rin + rperp) if (rin is not None and rperp is not None) else None, "energy_in_share": inshare_late},
            "early_512": {"recovery_in": ein, "recovery_perp": eperp, "sum": (ein + eperp) if (ein is not None and eperp is not None) else None, "energy_in_share": inshare_early}}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(arms["LATE_DROP_TT_768"] - PRIOR_DROP_TT_768) <= BARS["repro_tol"] and abs(arms["LATE_MLP_768"] - PRIOR_LATE_MLP_768) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_in_frame_output_carries_most_late': bool(rin is not None and rin >= BARS["b_min"]),
        'pred_c_out_of_frame_output_alone_small_late': bool(rperp is not None and rperp <= BARS["c_max"]),
        'pred_d_parts_near_additive_late': bool(rin is not None and rperp is not None and BARS["d_lo"] <= rin + rperp <= BARS["d_hi"]),
        'pred_e_in_frame_output_carries_most_early': bool(ein is not None and ein >= BARS["e_min"]),
    }
    nulls = {"b_null_in_le_.35": bool(rin is not None and rin <= NULLS["b_max"]), "c_null_perp_ge_.60": bool(rperp is not None and rperp >= NULLS["c_min"]),
             "d_null_sum_le_.5": bool(rin is not None and rperp is not None and rin + rperp <= NULLS["d_max"]), "e_null_early_in_le_.35": bool(ein is not None and ein <= NULLS["e_max"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "MLP reads through a k-frame (late: bus U_8 at 768; early: own frames at 512), tail*tail dropped; the gated linear read's OUTPUT Down[Lc*Rt + Lt*Rc] is kept whole / only inside the read frame / only outside it / dropped",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "late_drop_tt_768": arms["LATE_DROP_TT_768"], "prior_drop_tt_768": PRIOR_DROP_TT_768, "late_mlp_768": arms["LATE_MLP_768"], "prior_late_mlp_768": PRIOR_LATE_MLP_768},
