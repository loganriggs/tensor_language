...
def test_pooled_lookup_is_orientation_agnostic():
    ...
    x = B.Ctx({}, {}, {}, [('c', '', 0)], ['r'],
              pooled={'c': {('a', 'b'): {'mean': 0.5, 't': 3.0, 'n': 10}}})
    fwd, rev = x.tpool_full('c', 'a', 'b'), x.tpool_full('c', 'b', 'a')
    check('pooled: forward orientation', fwd['mean'] == 0.5 and fwd['t'] == 3.0)
    check('pooled: reverse orientation negates', rev['mean'] == -0.5 and rev['t'] == -3.0, str(rev))
    check('pooled: n is unchanged', rev['n'] == 10)
    try:
        x.tpool_full('c', 'a', 'zzz')
        check('pooled: a genuinely missing pair still raises', False, 'no raise')
    except KeyError as e:
        check('pooled: a genuinely missing pair still raises', 'paired_pairs' in str(e), str(e)[:60])


def test_run_refuses_a_vacuous_control_plan():
    """MEASURED 2026-08-30: six GPU runs this session died on an ill-formed covered-input control, every
    one decidable from the plan alone. run() now raises instead of warning-and-proceeding."""
    def raises(plan, why):
        try:
            B.run(name='probe', plan=plan, predicates=[('pred_a_x', 'x', lambda x: True)] * 3)
            check(f'plan pre-flight refuses {why}', False, 'run() accepted it')
        except ValueError as e:
            check(f'plan pre-flight refuses {why}', 'vacuous' in str(e), str(e)[:70])
        except Exception as e:                      # any other failure means it got past validation
            check(f'plan pre-flight refuses {why}', False, f'{type(e).__name__}: {e}')

    raises([('mix30m640', {'mlp': 768}, 'a'), ('mix30m640', {'mlp': 512}, 'b')], 'no same-spec pair')
    raises([('mix30m640', None, 'a'), ('map512', None, 'b')], 'no differing-spec pair')
    saved = B.DRYRUN
    try:                                            # and a well-formed plan gets through to the dry run
        B.DRYRUN = True
        ok = [('mix30m640', None, 'a'), ('map512', None, 'b'), ('mix30m640', {'mlp': 512}, 'c')]
        B.run(name='probe', plan=ok, predicates=[('pred_a_x', 'x', lambda x: True)] * 3)
        check('plan pre-flight passes a well-formed plan', True)
    except Exception as e:
        check('plan pre-flight passes a well-formed plan', False, f'{type(e).__name__}: {e}')
    finally:
        B.DRYRUN = saved
...
def test_inertness_requires_all36():
    ...
    fallback pair at a partial site set is NOT inert, and pairing it as same-spec asserts something false."""
    part = [('mix30m640', None, 'a', [('mlp', 2)]), ('map512', None, 'b', [('mlp', 2)])]
    inert, differ = B.inertness_pairs(part)
    check('all36: a partial-site fallback pair is NOT inert', ('a', 'b') in differ,
          f'inert={inert} differ={differ}')
    full = [('mix30m640', None, 'a'), ('map512', None, 'b')]
    i2, _d2 = B.inertness_pairs(full)
    check('all36: a full-site fallback pair still IS inert', ('a', 'b') in i2, str(i2))
