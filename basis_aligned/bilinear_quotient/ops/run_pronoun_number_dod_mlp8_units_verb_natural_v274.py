...
"""Pronoun number they/he DoD (v274): the MLP-8 number detectors {829, 953, 1030} zeroed at the ANNOTATED VERB of the natural rows. v248 found the verb-site units undetectable at cue + 1, where the
token after the noun is not reliably the verb; v272 / v273 mined 128 rows (FineWeb + Pile, 16 per cell) with `second_offset` = the first past-tense verb between the noun
and the pronoun. v197 / v199 / v200's design otherwise: damage on congruent rows, incongruent counter-case, 16 random same-size unit sets at the verb, number-free readers.
In the panel the same units at the verb cost: MLP-8 trio 5.9% (v215), MLP-6 trio (verb-site edit not run), MLP-5 verb-site pair 1.05% (v246).
PREDICTIONS (scored as written; failures preserved; priors from the panel)
    pred_a_instrument_replays_native        hooked forward with no edit = producer native <= 1e-4
    pred_b_congruent_damage_live            congruent rows: damage fraction >= 0.02 and positive >= 0.60
    pred_c_units_beat_random_unit_triples   congruent damage > max of the 16 random unit sets at the verb
...
PRICE (registered maximum): 4 batches x (native + verb + all-positions + 16 nulls) + producer replay 4 = 80 forwards; 0 backwards; 0 fits. Bar <= 84.
"""
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_mlp8_units_verb_natural_v274_result.json"
SOURCES = {"fineweb": ROOT / "circuits/followups/pronoun_number_dod_natural_verb_rows_v272.json", "pile": ROOT / "circuits/followups/pronoun_number_dod_natural_verb_rows_v273.json"}
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_mlp8_units_verb_natural_v274"
LAYER, UNITS, CONGRUENT = 8, (829, 953, 1030), ("natural_plural_they", "natural_singular_he")
NULL_SEEDS = tuple(range(8501, 8517))
LIVE_FRACTION, LIVE_POSITIVE, GATE_RATIO, INSTRUMENT_TOL = 0.02, 0.60, 0.25, 1e-4
FORWARDS_MAX = 84
PREDICTIONS = {"pred_a_instrument_replays_native": "<= 1e-4", "pred_b_congruent_damage_live": ">= 0.02, positive >= 0.60", "pred_c_units_beat_random_unit_triples": "> random max", "pred_d_units_selective": "three gates", "pred_e_incongruent_shifts_toward_label": "< 0"}


def main() -> None:
    rows, cue, shas = [], {}, {}
    for name, path in SOURCES.items():
        rs, sha, they, he = N.rows_from_receipt(path, "they", " they", " he", lambda r: f"natural_{r['cue']}_{r['label']}")
        ...
        for row, rec in zip(rs, recs):
            ...
            cue[row.row_id] = rec["second_offset"]      # the ANNOTATED VERB
    ...
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_mlp8_units_verb_natural_v274_result", "candidate_id": CANDIDATE_ID, "plan": plan, "instrument_max_abs_error": instrument, "arms": arms, "null_damage_max": null_max, "null_moves": null_moves, "nulls": nulls, "gates": gates,
