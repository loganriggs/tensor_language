#!/usr/bin/env python
"""early_stack_grouped_cores_probe -- does the early stack (blocks 0-10) share a FEW input coordinate systems, by depth or by kind
(§2749: own 768-cores .057, one core .180)? Depth groups of 2/3/4, kind groups, and the 36 x 36 capture matrix of every site's input
covariance under every site's 768-core. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_three_depth_groups_nearly_free pred_c_two_depth_groups pred_d_depth_not_kind
#                     pred_e_adjacent_blocks_read_alike
...
Preregistration: polynomial_causal/EARLY_STACK_GROUPED_CORES_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/early_stack_grouped_cores_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("early_stack_grouped_cores_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "EARLY_STACK_GROUPED_CORES_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "early_stack_width_map_probe_results.json"   # §2749
OUT = ROOT / "early_stack_grouped_cores_probe_results.json"
HASHES = {PREREG: "a5aa94aed1e8c89f0e2869b4f55797f1dc4443a79318e5f5718c0f23a47371d5", PRIOR: "745f13077b2292b8913e091938c2f496f991e691ecd7e7fdf746a0470df0368a",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "early_stack_grouped_cores_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_EARLY22_OWN_768 = 0.057; PRIOR_EARLY22_SHARED_768 = 0.180; LAM = 1e-2
K = 768; EARLY = list(range(0, 11)); LATE = list(range(11, 18))
SITES = [("mlp", l) for l in range(NL)]; ASITES = [("attn", l) for l in range(NL)]
ESITES = [(kd, l) for l in EARLY for kd in ("attn", "mlp")]; ALLS = [(kd, l) for l in range(NL) for kd in ("attn", "mlp")]
GROUPS = {"G2": [range(0, 6), range(6, 11)], "G3": [range(0, 4), range(4, 8), range(8, 11)], "G4": [range(0, 3), range(3, 6), range(6, 9), range(9, 11)]}
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_max": 0.04, "c_max": 0.06, "d_min": 0.02, "e_min": 0.95}
NULLS = {"b_min": 0.10, "c_min": 0.12, "d_max": 0.0, "e_max": 0.85}
...
def main():
    ...
    def cap(i, U_):
        C = heads[i]["Cx"]; Uk = U_[:, :K].double(); return float((Uk.T @ C @ Uk).trace() / C.trace())
    names = [f"{s_[0]}{s_[1]}" for s_ in ALLS]
    capm = [[cap(i, heads[j]["U"]) for j in ALLS] for i in ALLS]
    ratio = {}
    for (kd, l) in ESITES:
        own = cap((kd, l), heads[(kd, l)]["U"]); nb = [cap((kd, l), heads[(kd, l2)]["U"]) / own for l2 in (l - 1, l + 1) if 0 <= l2 < NL]
        ratio[f"{kd}{l}"] = min(nb)
    rmin = min(ratio.values()); rmin_site = min(ratio, key=ratio.get)
    log(stage="capture", adjacent_ratio={k_: round(v, 3) for k_, v in ratio.items()}, rmin=round(rmin, 4), rmin_site=rmin_site)
    U_all = core_of(ESITES)
    arms = {"EARLY22_OWN_768": run({s_: head(s_, K) for s_ in ESITES}), "EARLY22_SHARED_768": run({s_: head(s_, K, U_all) for s_ in ESITES})}
    for gname, grp in GROUPS.items():
        patch = {}
        for rg in grp:
            mem = [(kd, l) for l in rg for kd in ("attn", "mlp")]; Ug = core_of(mem)
            patch.update({s_: head(s_, K, Ug) for s_ in mem})
        arms[f"{gname}_768"] = run(patch)
    Ua = core_of([("attn", l) for l in EARLY]); Um = core_of([("mlp", l) for l in EARLY])
    arms["KIND2_768"] = run({s_: head(s_, K, Ua if s_[0] == "attn" else Um) for s_ in ESITES})
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    gaps = {f"{g}_minus_own": arms[f"{g}_768"] - arms["EARLY22_OWN_768"] for g in GROUPS}
    gaps["kind2_minus_own"] = arms["KIND2_768"] - arms["EARLY22_OWN_768"]; gaps["kind2_minus_g2"] = arms["KIND2_768"] - arms["G2_768"]; gaps["shared_minus_own"] = arms["EARLY22_SHARED_768"] - arms["EARLY22_OWN_768"]
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["EARLY22_OWN_768"] - PRIOR_EARLY22_OWN_768) <= BARS["repro_tol"] and abs(arms["EARLY22_SHARED_768"] - PRIOR_EARLY22_SHARED_768) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_three_depth_groups_nearly_free': bool(gaps["G3_minus_own"] <= BARS["b_max"]),
        'pred_c_two_depth_groups': bool(gaps["G2_minus_own"] <= BARS["c_max"]),
        'pred_d_depth_not_kind': bool(gaps["kind2_minus_g2"] >= BARS["d_min"]),
        'pred_e_adjacent_blocks_read_alike': bool(rmin >= BARS["e_min"]),
    }
    nulls = {"b_null_ge_.10": bool(gaps["G3_minus_own"] >= NULLS["b_min"]), "c_null_ge_.12": bool(gaps["G2_minus_own"] >= NULLS["c_min"]),
             "d_null_kind2_le_g2": bool(gaps["kind2_minus_g2"] <= NULLS["d_max"]), "e_null_lt_.85": bool(rmin < NULLS["e_max"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "k": K, "groups": {g: [[r.start, r.stop - 1] for r in grp] for g, grp in GROUPS.items()},
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "early22_own_768": arms["EARLY22_OWN_768"], "prior_early22_own_768": PRIOR_EARLY22_OWN_768, "early22_shared_768": arms["EARLY22_SHARED_768"], "prior_early22_shared_768": PRIOR_EARLY22_SHARED_768},
           "ce_added": arms, "gaps": gaps, "adjacent_ratio": ratio, "rmin": rmin, "rmin_site": rmin_site, "capture_matrix": {"sites": names, "cap_i_under_j": capm},
