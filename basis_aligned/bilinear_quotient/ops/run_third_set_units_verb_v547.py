#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_late_units_at_the_noun_inert pred_c_late_units_at_the_verb_replay pred_d_hub_units_at_the_noun_replay pred_e_hub_units_at_the_verb_replay
"""Unit components on the THIRD natural set, verb slot (v547). v462 (sets 1 + 2, 79 distant rows): 701 / 2059 swapped at the verb position close 0.069 of
the agreement gap and nothing at the noun (0.000); 829 / 953 / 1030 swapped at the noun 0.046, at the verb 0.031. v546: the five verb heads replay on the
fresh set (0.599). Here the same four unit edits on v544's 45 fresh distant rows, priors registered first.
PREDICTIONS (scored as written; failures preserved; priors from v462)
    pred_a_baseline_replays              the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_late_units_at_the_noun_inert  701 / 2059 swapped at the noun close <= 0.02 (they act at the answer position only)
    pred_c_late_units_at_the_verb_replay 701 / 2059 swapped at the verb close 0.069 +- 0.04
    pred_d_hub_units_at_the_noun_replay  829 / 953 / 1030 swapped at the noun close 0.046 +- 0.03
    pred_e_hub_units_at_the_verb_replay  829 / 953 / 1030 swapped at the verb close 0.031 +- 0.03
PRICE (registered maximum): 2 batches x 5 passes = 10 forwards (45 fresh distant rows); 0 backwards; 0 fits. Bar <= 11.
"""
...
OUT = ROOT / "circuits/followups/third_set_units_verb_v547_result.json"
CANDIDATE_ID = "chain.third_set_units_verb_v547"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, NOUN_MAX, LATE_A_V462, LATE_BAND, HUB_N_V462, HUB_A_V462, HUB_BAND = 1e-3, 0.02, 0.069, 0.04, 0.046, 0.031, 0.03
import random
UNIT_SETS = {"LATE_N": (17, (701, 2059), "noun"), "LATE_A": (17, (701, 2059), "answer"), "HUB_N": (8, (829, 953, 1030), "noun"), "HUB_A": (8, (829, 953, 1030), "answer")}
...
FORWARDS_MAX = 11
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_late_units_at_the_noun_inert": "<= 0.02", "pred_c_late_units_at_the_verb_replay": "0.069 +- 0.04", "pred_d_hub_units_at_the_noun_replay": "0.046 +- 0.03", "pred_e_hub_units_at_the_verb_replay": "0.031 +- 0.03"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "noun_max": NOUN_MAX, "late_a_v462": LATE_A_V462, "late_band": LATE_BAND, "hub_n_v462": HUB_N_V462, "hub_a_v462": HUB_A_V462, "hub_band": HUB_BAND}}
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_late_units_at_the_noun_inert": abs(v["LATE_N"]) <= NOUN_MAX, "pred_c_late_units_at_the_verb_replay": abs(v["LATE_A"] - LATE_A_V462) <= LATE_BAND,
                   "pred_d_hub_units_at_the_noun_replay": abs(v["HUB_N"] - HUB_N_V462) <= HUB_BAND, "pred_e_hub_units_at_the_verb_replay": abs(v["HUB_A"] - HUB_A_V462) <= HUB_BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "late_units_position_result_v547", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
