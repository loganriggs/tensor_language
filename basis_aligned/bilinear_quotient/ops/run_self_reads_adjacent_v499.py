#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_11_3_self_read_carries pred_c_verb_heads_self_read_carry pred_d_all_values_replay pred_e_self_reads_below_all_values
"""Self-reads at the noun for an adjacent verb (v499). v498: when the verb follows the noun directly (the answer position is the noun), 11.3's value at the
noun changes the agreement verbs (0.196) — the head attends to its own position. v446: every head's value at the noun closes 0.26 of the adjacent-row
margin. Here on the 43 adjacent rows: 11.3's value at the noun alone, the five verb heads' values, the five pronoun heads' values, and all heads' values.
PREDICTIONS (scored as written; failures preserved; priors from v446 / v498)
    pred_a_baseline_replays          the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_11_3_self_read_carries    11.3's value at the noun closes >= 0.04 of the adjacent-row agreement margin gap
    pred_c_verb_heads_self_read_carry  the five verb heads' values at the noun close >= 0.08
    pred_d_all_values_replay         every head's value at the noun closes 0.26 +- 0.03 (v446)
    pred_e_self_reads_below_all_values  the five verb heads' self-read is <= the all-heads value swap
...
OUT = ROOT / "circuits/followups/self_reads_adjacent_v499_result.json"
CANDIDATE_ID = "chain.self_reads_adjacent_v499"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, H113_MIN, V5_MIN, ALL_V446, BAND = 1e-3, 0.04, 0.08, 0.26, 0.03
...
FORWARDS_MAX = 11
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_11_3_self_read_carries": ">= 0.04", "pred_c_verb_heads_self_read_carry": ">= 0.08", "pred_d_all_values_replay": "0.26 +- 0.03", "pred_e_self_reads_below_all_values": "verb heads <= all"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "h113_min": H113_MIN, "v5_min": V5_MIN, "all_v446": ALL_V446, "band": BAND}}
    ...
    V5 = {11: [3], 7: [8], 13: [1], 9: [7], 5: [3]}; ALL_H = {l: list(range(N_HEAD)) for l in range(18)}
    MODES = {"h113": ({11: [3]}, (0,)), "V5": (V5, (0,)), "P5": (FIVE, (0,)), "ALL": (ALL_H, (0,))}; forwards = 0
    def logits_margin(x):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_11_3_self_read_carries": v["h113"] >= H113_MIN, "pred_c_verb_heads_self_read_carry": v["V5"] >= V5_MIN,
                   "pred_d_all_values_replay": abs(v["ALL"] - ALL_V446) <= BAND, "pred_e_self_reads_below_all_values": v["V5"] <= v["ALL"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "self_reads_adjacent_result_v499", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
