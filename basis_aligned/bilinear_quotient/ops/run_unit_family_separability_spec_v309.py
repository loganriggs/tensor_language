...
"""v309: is the for_toward retraction a real overlap or the control set tightening? A CONTROL-SET ABLATION.

WHAT HAPPENED. adjective_preposition_for_toward was separable at v289 (sib 0.033 into adjective_preposition_fond) and
again at v299, and came out NOT separable at v305 (0.058 into fond) after the family grew from 24 to 29 prepared. Under
the v222 rule that is a retraction, and it was reported to the board at 07:45 rather than absorbed. Two things make it
worth an instrument rather than a verdict:
  (1) It is the FIRST fusion in the corpus with NO shared cue. for_toward's cues are destined/inclined; fond's are
      fond/keen. Every previous fusion since v254 shared a cue word with the cell it leaked into.
  (2) The crossing is marginal and monotone in control-set size (0.033 -> 0.058 as five controls were added), which is
      equally consistent with the fitted direction being squeezed by more constraints as with a real overlap.
v262 built the instrument that separates those on the verb family; this is that instrument on the adjective family,
which is the one that still completes (v305 ran 29 prepared with zero OOM; verb separability is halted at 55).

THE ABLATION. Three arms on the SAME units, seed and split, for three members: own (own C_fit only), fam (own C_fit +
EVERY sibling as control -- reproduces v305), and fam_minus_partner (every sibling EXCEPT the cue/leak partner). The
partner is still EVALUATED in all three arms; it is only removed from the CONTROLS. Members and their partners:
  adjective_preposition_for_toward -> fond            (NO shared cue; the case in question)
  adjective_preposition_in_about   -> in_beneath      (shared cue `interested`; a known cue fusion, as a positive control)
  adjective_preposition_for_of     -> for_beneath     (shared cue `responsible`; second positive control)
The two cue fusions are included precisely so the arm has something that SHOULD behave one way if the instrument works:
at v262 the cue-sharing incumbents showed their partner as the largest unconstrained leak by margins of +0.48 to +0.62.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
  pred_a_all_three_arms_complete -> all three members produce all three arms with a non-null extraction. At 29 prepared
      v305 completed cleanly, so a failure here is memory, not evidence.                                  prior 90%
  pred_b_partner_special_unconstrained -> in the OWN arm (no sibling controls at all) the partner's leak exceeds the
      largest leak into any other sibling by >= 0.05, for at least 2 of 3. The two cue fusions should show this; the
      question is whether for_toward does. If it does NOT while both cue fusions do, the retraction is the control set
      tightening rather than a real overlap, and the corpus's fusions remain exactly the cue-sharing ones.  prior 70%
  pred_c_extraction_recovers -> fam_minus_partner extraction exceeds fam extraction by >= 0.02 for at least 2 of 3.
      This is the constraint-cost signature; at v262 it held for all four members tested.                  prior 70%
  pred_d_instrument_reproduces -> the fam arm reproduces v305's for_toward leak of 0.058 within 0.03. Same units, seed,
      split and code path, so this is a determinism check, NOT an independent outcome; if it fails, b and c are void.
                                                                                                          prior 90%
NOT REGISTERED, and deliberately: I am not putting a bar on for_toward's own-arm margin alone, because with one case a
bar is a coin flip dressed as a prediction. The three members are read together and the ledger will state all three
margins side by side.
COUNTING: this rung re-fits directions for members that are already counted or already fused and adds no cells, so it
CANNOT change the circuit total. It is spent entirely on whether a retraction I have already reported is real.
Smoke: V309_SMOKE=<out.json> (CPU, V309_SMOKE_ROWS=4; V309_SMOKE_NAMES=adjective_preposition_for_toward).
BARS = {"cross_max": 0.05, "partner_margin": 0.05, "k_special": 2, "recover_by": 0.02, "k_recovers": 2, "instr_tol": 0.03}
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v309_result.json"
...
EVAL = ("adjective_preposition_for_toward", "adjective_preposition_in_about", "adjective_preposition_for_of", )
COUNTED = ("adjective_preposition_for_toward", "adjective_preposition_in_about", "adjective_preposition_for_of", )
PARTNER = {"adjective_preposition_for_toward": "adjective_preposition_fond", "adjective_preposition_in_about": "adjective_preposition_in_beneath", "adjective_preposition_for_of": "adjective_preposition_for_beneath"}
...
PRED_NAMES = ("pred_a_all_three_arms_complete", "pred_b_partner_special_unconstrained",
              "pred_c_extraction_recovers", "pred_d_instrument_reproduces")
BARS = {"cross_max": 0.05, "partner_margin": 0.05, "k_special": 2, "recover_by": 0.02, "k_recovers": 2, "instr_tol": 0.03}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 20000, 640000
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v309", "members": n, "families": list(FAMILIES),
...
def PREDS(R):
    ...
    a = ok and all(good[n]["arms"]["fam_minus_partner"]["extraction"] is not None for n in good)
    def special(n):
    ...
    b = ok and sum(1 for n in good if special(n) >= B["partner_margin"]) >= B["k_special"]
    ...
    d = ok and abs(good["adjective_preposition_for_toward"]["leak_fam"] - 0.058) <= B["instr_tol"]
...
def main() -> None:
    ...
    smoke = os.environ.get("V309_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V309_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    evaluate = [n for n in members if n in EVAL and (not smoke or n in os.environ.get("V309_SMOKE_NAMES", "adjective_preposition_for_toward").split(","))]
        ...
        for n in mem:
            ...
            try:
                ...
                R[n] = {"family": fam, "units": units, "n_units": len(units), "exact_held": round(exact, 3), "arms": arms,
                        ...
                        "own_partner_margin": round(abs(own_sib.get(partner, 0.0))
                                                    - max([abs(v) for s2, v in own_sib.items() if s2 != partner] or [0.0]), 4), "separable": bool(sep),
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v309", "candidate_id": "corpus.unit_family_separability_spec_v309",
