...
p=prior.p
ROWS=prior.ROWS;PREREG=POLY/'INDUCTION_EARLY_MLP_FINITE_BILINEAR_FACTOR_V1_PREREGISTRATION.md';BINDING=POLY/'INDUCTION_EARLY_MLP_FINITE_BILINEAR_FACTOR_V1_BINDING.json';OUT=POLY/'INDUCTION_EARLY_MLP_FINITE_BILINEAR_FACTOR_V1_RESULT.json'
...
def plan():
 _r,f,rows,endpoints=load_bound();return {'schema':'induction_early_mlp_finite_bilinear_factor_v1_plan','model_loaded':False,'gpu_accessed':False,'queue_touched':False,'rows':len(rows),'endpoints':len(endpoints),'sites':list(SITES),'arms':list(ARMS),'candidate_order':list(CANDIDATES),'price':PRICE,'predicates':list(PREDICTION_REGISTRY)}
...
def execute(runtime,tokens,specs,target,native_states,arm):
 ...
 def mlp(event):
  ...
  if event.site in SITES and arm not in ('self','joint'):
   ...
   for local,spec in enumerate(specs):
    q=int(spec['final_position']);x1=event.state[local,q].float();x0=torch.as_tensor(native_states[local,si],device=device);d=x1-x0;lw=mlp.Left.weight.float();rw=mlp.Right.weight.float();dw=mlp.Down.weight.float();l0=F.linear(x0,lw);r0=F.linear(x0,rw);ld=F.linear(d,lw);rd=F.linear(d,rw);left=F.linear(ld*r0,dw);right=F.linear(l0*rd,dw);quad=F.linear(ld*rd,dw);joint=left+right+quad;direct=F.linear(F.linear(x1,lw)*F.linear(x1,rw)-l0*r0,dw);closures.append(float((joint-direct).square().sum()/direct.square().sum().clamp_min(1e-30)));component={'quadratic':quad,'left':left,'right':right,'cross':left+right,'full':joint}[arm];changed[local,q]=(changed[local,q].float()-component).to(changed.dtype);norms[local]+=float(component.double().norm().cpu())**2
   write=changed
  return write
