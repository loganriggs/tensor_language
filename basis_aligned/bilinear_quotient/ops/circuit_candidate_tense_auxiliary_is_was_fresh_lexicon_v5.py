"""Prospective fifth is/was lexicon for the frozen resid10 controller."""
...
_AGENTS = (
    "beekeeper", "blacksmith", "cartographer", "curator", "detective", "florist",
    "glassblower", "herbalist", "innkeeper", "lecturer", "mason", "optician",
    "paramedic", "ranger", "tailor", "zookeeper",
)
...
def _panel(group_number: int) -> list[dict[str, Any]]:
    ...
    group_id = f"FIT:{canonical_sha256([SCHEMA, TASK_ID, 'different_readout_v5', SEED, group_number])[:24]}"
    ...
    common_no_vocab = dict(seed=SEED, task_id=TASK_ID, group_number=group_number, group_id=group_id, reporter=agent, alternate_reporter=alternate, adjective="different_readout_v5", object_name="moment", spec=TASK_SPEC)
...
def _validate(rows: Sequence[Mapping[str, object]]) -> str:
    ...
    if materialized != _build():
        raise CandidateBankError("rows differ from sealed v5 authority")
    try:
    ...
    old_banks = (v1.build_rows(), v2.build_rows(), v3.build_rows(), v4.build_rows())
    old_ids = {str(row["row_id"]) for bank in old_banks for row in bank}
    old_agents = set(v1._AGENTS) | set(v3._AGENTS) | set(v4._AGENTS)
    ...
    if (
        ...
        raise CandidateBankError("v5 count, uniqueness, disjointness, agent novelty, or construction check failed")
    return digest
