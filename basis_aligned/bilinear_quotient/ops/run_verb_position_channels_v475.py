#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_attention_into_the_verb_carries_it_all pred_c_mlps_9_16_jointly_below_the_sum pred_d_mlps_9_16_jointly_above_the_largest pred_e_mlps_below_attention
"""The verb position's two channels (v475). v474: at a distant verb each MLP 9-16's write at the answer position carries 0.05-0.10 of the agreement margin
(singles sum 0.67). The verb position learns the noun's number only through what attention writes INTO it (the readers' copies). Here two joint edits at
the answer position, 79 distant rows: every block's attention output swapped within the pair (the whole channel from the rest of the sequence into the verb
position), and MLPs 9-16 swapped jointly (nested: later MLPs read earlier ones, v435).
PREDICTIONS (scored as written; failures preserved; priors from v429 / v435 / v474)
    pred_a_baseline_replays                   the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_attention_into_the_verb_carries_it_all  every block's attention output at the answer position closes >= 0.85 of the margin gap (the verb's only source is attention)
    pred_c_mlps_9_16_jointly_below_the_sum    MLPs 9-16 jointly close <= 0.67 - 0.05 (nesting)
    pred_d_mlps_9_16_jointly_above_the_largest  MLPs 9-16 jointly close >= 0.15 (more than any single, 0.102)
    pred_e_mlps_below_attention               the MLP joint is <= the attention joint
...
OUT = ROOT / "circuits/followups/verb_position_channels_v475_result.json"
CANDIDATE_ID = "chain.verb_position_channels_v475"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, ATTN_MIN, SUM_V474, NEST, JOINT_MIN, UNIT = 1e-3, 0.85, 0.671, 0.05, 0.15, 701
import random
...
FORWARDS_MAX = 10
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_attention_into_the_verb_carries_it_all": ">= 0.85", "pred_c_mlps_9_16_jointly_below_the_sum": "<= 0.621", "pred_d_mlps_9_16_jointly_above_the_largest": ">= 0.15", "pred_e_mlps_below_attention": "mlps <= attn"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "attn_min": ATTN_MIN, "sum_v474": SUM_V474, "nest": NEST, "joint_min": JOINT_MIN, "unit": UNIT}}
    ...
    MODES = {"attn_all": ({}, (0,)), "mlps9_16": ({}, (0,))}; forwards = 0
    def logits_margin(x):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_attention_into_the_verb_carries_it_all": mc["attn_all"] >= ATTN_MIN, "pred_c_mlps_9_16_jointly_below_the_sum": mc["mlps9_16"] <= SUM_V474 - NEST,
                   "pred_d_mlps_9_16_jointly_above_the_largest": mc["mlps9_16"] >= JOINT_MIN, "pred_e_mlps_below_attention": mc["mlps9_16"] <= mc["attn_all"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "verb_position_channels_result_v475", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
