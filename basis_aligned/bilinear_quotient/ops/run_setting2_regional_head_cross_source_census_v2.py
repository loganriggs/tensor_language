...
BINDING=P/"SETTING2_REGIONAL_HEAD_CROSS_SOURCE_CENSUS_V2_BINDING.json"
...
OUT=ROOT/"basis_aligned/bilinear_quotient/circuits/fast_screens/setting2_regional_head_cross_source_census_v2_result.json"
...
def load_bound():
    ...
    files={"preregistration":PREREG,"correction":P/"SETTING2_REGIONAL_HEAD_CROSS_SOURCE_CENSUS_V2_CORRECTION.md","rows":ROWS,"row_check":P/"regional_cue_row_check_v1.py",
...
def plan():
    ...
    return {"schema":"setting2_regional_head_cross_source_census_v2_plan","model_loaded":False,"gpu_accessed":False,"queue_touched":False,"rows":len(rows),"arms":ARMS,"late_names":LATE_NAMES,"price":PRICE,"row_checks":checks,"binding_sha256":sha(BINDING)}
...
def main():
    ...
    for family in families:
        ...
        for name in ("description","instruction"):
            vectors=torch.stack([source_vectors[i,:len(rows[i]["ids"])][roles[i][name]].sum(0) for i in row_ids])
