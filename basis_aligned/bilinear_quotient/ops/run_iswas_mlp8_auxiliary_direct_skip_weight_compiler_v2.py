...
PRIOR = ROOT / "circuits/prior_art/iswas_mlp8_auxiliary_direct_skip_weight_compiler_v2.json"
...
V1_RESULT = ROOT / "circuits/followups/iswas_mlp8_auxiliary_direct_skip_weight_compiler_v1_result.json"
V1_RUNNER = ROOT / "ops/run_iswas_mlp8_auxiliary_direct_skip_weight_compiler_v1.py"
...
OUT = ROOT / "circuits/followups/iswas_mlp8_auxiliary_direct_skip_weight_compiler_v2_result.json"
CANDIDATE_ID = "cross_task.iswas_mlp8_auxiliary_direct_skip_weight_compiler_v2"
LAYERS, SELECTED = (11, 15), {11: (1, 3), 15: (5,)}
CLAMP_ATTN, CLAMP_MLP = tuple(range(9, 15)), tuple(range(9, 15))
EXPECTED = {
    "prior": "353358aac6396923b096c122630b9e36da6d4c196971854e0464b6246861fc22",
    ...
    "v1_result": "250aa2f28949f47ab0bebdbf4560e81b7edaac9f1251d999f342e7ca455d4d48",
    "v1_runner": "9ee30409a9448ebd4a80ddf3f65fbd4a84cb9085ad2dd26abcfd70863d95729d"
}
...
def main():
    paths = {"prior": PRIOR, "parent": PARENT, "parent_runner": PARENT_RUNNER,
        ...
        "v1_result": V1_RESULT, "v1_runner": V1_RUNNER}
    if {name: sha(path) for name, path in paths.items()} != EXPECTED:
        raise RuntimeError("direct-skip compiler authority changed")
    prior, parent, atlas, local_result, capability, subspace, v1_result = [json.loads(path.read_text())
        for path in (PRIOR, PARENT, ATLAS, LOCAL, CAPABILITY, weight.SUBSPACE, V1_RESULT)]
    ...
    if (prior.get("candidate_id") != CANDIDATE_ID or parent.get("terminal") != "screen"
            ...
            or v1_result.get("terminal") != "null"
            or v1_result.get("predictions", {}).get("pred_b_exact_lambda_rms_cv_response_compiler") is not False
            or len(rows) != 30):
    ...
    for layer in LAYERS:
        ...
        predicted_value_delta = (1.0 - attention.lamb.detach().float()) * (predicted_direct - base_direct)
    ...
    pred_b = max_z <= .01 and max_current <= 1e-5 and max(max_value, max_response, max_cproj) <= .001
