"""REGISTERED CLAIM: RANK-64 (rung 202) - seven certificates.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. Rung 201 printed census +0.0852 /
fresh +0.0809 / valid 7/62. Identical rebuild; claim bars include the certificate count.

REGISTERED PREDICTIONS (anchors: rung-201 receipt):
  (a) REPRODUCTION: |census - 0.0852| <= 0.015.
  (b) CERTIFICATES: valid >= 7 of 62.
  (c) REPRODUCTION: |L2_F - 0.0809| <= 0.015.
NULL: noise. PRICE: ~186M values (r64 patterns 48.5M; only compression). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: registered claim r64')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_claim_qk64_results.json'
...
if __name__=='__main__':
    ...
    pa=abs(agg-0.0852)<=0.015
    pb=nv>=7
    pc=abs(L2F-0.0809)<=0.015
    ...
    print(f"(a) |census {agg:+.4f} - 0.0852| <= 0.015: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 7; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |L2F {L2F:+.4f} - 0.0809| <= 0.015: {'HELD' if pc else 'FAILED'}")
