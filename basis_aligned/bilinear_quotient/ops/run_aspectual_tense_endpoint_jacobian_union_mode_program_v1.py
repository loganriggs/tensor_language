#!/usr/bin/env python3
"""Complete base/donor endpoint-Jacobian plus static-weight union modes."""

# BQGATE: EXPERIMENT pred_a_authority_capability_projection_finiteness_and_exact_price pred_b_endpoint_union_modes_are_sufficient pred_c_endpoint_union_complements_are_secondary pred_d_endpoint_union_remains_compressive pred_e_no_causal_fit_or_model_update
...
import run_aspectual_tense_weight_jacobian_union_mode_program_v1 as parent
import run_aspectual_tense_weight_readable_mode_program_v1 as static
import subspace_weight_atlas as atlas


ROOT = Path(__file__).resolve().parents[1]
PRIOR = ROOT / "circuits/prior_art/aspectual_tense_endpoint_jacobian_union_mode_program_v1.json"
PARENT_RESULT = ROOT / "circuits/followups/aspectual_tense_weight_jacobian_union_mode_program_v1_result.json"
PARENT_RUNNER = ROOT / "ops/run_aspectual_tense_weight_jacobian_union_mode_program_v1.py"
...
OUT = ROOT / "circuits/followups/aspectual_tense_endpoint_jacobian_union_mode_program_v1_result.json"
CANDIDATE_ID = "aspectual_tense.endpoint_jacobian_union_mode_program_v1"
EXPECTED = {"prior": "0cc45e830fcb492d62b7f02648c70f6c73775bd03d19ebf8d6596856e0dc0d5c",
            "parent_result": "5239a47f124ca14b241e42cbeb4ecc28b56a5ea58d4eac4ac41676865d635494",
            "parent_runner": "2041b78b4debf43847a71ea3d3cd10cba7cf8d3c8a6fb61e8dd3b2b68bdff9c4",
...
PRICE = {"model_forwards": 24, "example_evaluations": 383, "records": 201,
...
def complete_basis(torch, gradients):
    _u, singular, vh = torch.linalg.svd(gradients.float(), full_matrices=False)
    if not singular.numel() or float(singular[0]) <= 0:
        raise ExperimentError("endpoint gradient span is empty")
    keep = singular > singular[0] * 1e-6
    q = torch.linalg.qr(vh[keep].T.double()).Q.float().to(gradients.device)
    return q, singular


def validate_static():
    paths = {"prior": PRIOR, "parent_result": PARENT_RESULT, "parent_runner": PARENT_RUNNER,
             "jacobian_runner": JACOBIAN_RUNNER, "static_runner": STATIC_RUNNER}
    if {name: sha(path) for name, path in paths.items()} != EXPECTED:
        raise ExperimentError("authority hash changed")
    result = json.loads(PARENT_RESULT.read_text())
    splits, chosen, pools, programs, subspaces = parent.validate_static()
    if result.get("terminal") != "null":
        raise ExperimentError("parent union result changed")
...
def main():
    ...
    dryrun = {"candidate_id": CANDIDATE_ID, "dryrun": True, "gpu_accessed": False,
              "model_loaded": False, "queue_touched": False, "endpoint_relative_rank_tolerance": 1e-6,
    ...
    for task in ("has", "is"):
        ...
        for component in components:
            ...
            endpoint_q, singular = complete_basis(
            ...
            q = parent.union_basis(torch, static_q, endpoint_q)
    ...
    for task in ("has", "is"):
        ...
        for panel in ("heldout", "a2"):
            ...
            outputs = {"full": positioned.patch_positioned_components(
                ...
                "endpoint_union": static.patch_projected(backend, base_batch, donor_batch,
    ...
    pred_a = bool(all_capable and finite and price == PRICE
                  and max(report["gram_max_abs_error"] for report in reports) <= 1e-5
                  and bank_widths == {"has_heldout": [3], "has_a2": [3],
                                      "is_heldout": [2], "is_a2": [2]})
    pred_b = all(metrics[task][panel]["endpoint_union"]["mean_recovery"]
                 >= 0.95 * metrics[task][panel]["full"]["mean_recovery"]
                 and metrics[task][panel]["endpoint_union"]["direction_fraction"] == 1.0
    ...
    pred_d = all(report["union_rank"] < report["width"]
                 and report["union_rank"] <= (114 if task == "has" else 35)
                 for task, task_reports in rank_reports.items() for report in task_reports.values())
    pred_e = backwards == 4 and price["model_updates"] == 0
    predictions = {
        "pred_a_authority_capability_projection_finiteness_and_exact_price": pred_a,
        "pred_b_endpoint_union_modes_are_sufficient": pred_b,
        "pred_c_endpoint_union_complements_are_secondary": pred_c,
        "pred_d_endpoint_union_remains_compressive": pred_d,
    ...
    result = {"schema": "aspectual_tense_endpoint_jacobian_union_mode_program_result_v1",
              "candidate_id": CANDIDATE_ID, "execution_policy": "managed_queue_only",
              "started_utc": started_utc, "finished_utc": utc_now(),
              "serial_seconds": time.perf_counter() - started, "authority_sha256": EXPECTED,
              "programs": programs, "rank_reports": rank_reports, "metrics": metrics,
              "bank_widths": bank_widths, "predictions": predictions,
              "price": price, "records": records, "terminal": terminal}
    atomic_create_json(OUT, result)
    print(json.dumps({key: result[key] for key in ("candidate_id", "rank_reports", "metrics",
