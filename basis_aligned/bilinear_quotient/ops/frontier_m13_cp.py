"""CP-VS-TABLE AT m1 AND m3 (rung 111): completing the per-site K map for the CP front.

CONVENTION (S2135): per-position dCE = CE(single-site CP-K, weights-only) - CE(real model) on the census rows;
LOWER IS BETTER. S2203/S2204 (m2: CP-576 crushes the table; m0: needs K=2304 but then dominates at 7x fewer
values). Here the remaining two front MLPs, single-site, K in {288,576,1152,2304,4608}.
Anchors (S2200 singles): m1's linear replacement +0.2628 (valid 2/62); m3E's table +0.4850 (valid 1/62).

REGISTERED PREDICTIONS (arms named agg(mL,K), valid(mL,K)):
  (a) GRAMMAR DOMINATES BY K=2304 AT BOTH SITES: agg(m1,2304) <= 0.5 x 0.2628 AND agg(m3,2304) <= 0.5 x 0.4850.
  (b) VALIDITY MOVES AT BOTH: valid(m1,2304) >= 5 AND valid(m3,2304) >= 5 (of 62).
  (c) SANITY: agg nonincreasing in K (tol 0.01) at both sites and agg(mL,4608) in [-0.01, 0.02] (exactness).
NULL: m1/m3 pattern like m0 at 576 (site-hardness at small K) AND fail even at 2304 (grammar loses somewhere).
PRICE: CP-2304 = 7,962,624 values/site vs table 57,896,064. Tripwire: INSTRUMENT FAIL if all arms of a site
are identical (pstdev < 0.005). Self-reviewed."""
import json, sys, time, os
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
if os.environ.get('BQLIB_DRYRUN')=='1':
    _bq='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
    _need=['frontier_m0_cp_results.json','frontier_front_singles_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: CP-vs-table at m1 and m3')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_m13_cp_results.json'
...
def main():
    ...
    if SEL.get('m13_cp'):
        ...
        for _li in (1,3):
            ...
            _ord=(_Dw.norm(dim=0)*_L.norm(dim=1)*_R.norm(dim=1)).argsort(descending=True)
            for _K in (288,576,1152,2304,4608):
                _kp=_ord[:_K]
                S['cXK']=('cp',_li,_L[_kp].contiguous(),_R[_kp].contiguous(),
                          _Dw[:,_kp].contiguous(),_db)
                _arms[f'm{_li}_K{_K}']=evalV(_ROWS,_ROWS.shape[0],['cXK'],ML).detach().cpu()
                print(f'  m{_li} CP K={_K} done',flush=True)
        del cur['clsmap']
        SEL['front_result']=_arms
...
if __name__=='__main__':
    ...
    SG=json.load(open(PT+'frontier_front_singles_results.json'))['arms']
    ANC={1:SG['single_m1']['agg'],3:SG['single_m3E']['agg']}
    SEL['mode']='norm'; SEL['K']=4608; SEL['K69']=4608; SEL['K69MAP']={}
    SEL['skipset']=tuple(range(10,18)); SEL['motif_off']=tuple(range(2,10))
    SEL['m13_cp']=True; SEL['ext_rows']=CROWS
    print('ARMS: m1 and m3 single-site CP top-K, K in 288/576/1152/2304/4608 - census rows',flush=True)
    main()
    A=SEL['front_result']
    out={}
    for k,cev in A.items():
        ...
        print(f"  {k:9s} agg {agg:+.4f}  valid {nv}/{len(CINFO)}",flush=True)
    import statistics as stt
    ok_a=ok_b=True; ok_c=True
    for li in (1,3):
        aggs={K:out[f'm{li}_K{K}']['agg'] for K in (288,576,1152,2304,4608)}
        if stt.pstdev(aggs.values())<0.005: raise SystemExit(f'INSTRUMENT FAIL: m{li} arms identical')
        ok_a&=aggs[2304]<=0.5*ANC[li]
        ok_b&=out[f'm{li}_K2304']['valid']>=5
        Ks=sorted(aggs)
        ok_c&=all(aggs[Ks[i]]>=aggs[Ks[i+1]]-0.01 for i in range(len(Ks)-1)) and -0.01<=aggs[4608]<=0.02
    res={'arms':out,'anchors_singles':{str(k):v for k,v in ANC.items()},
         'convention':'per-position dCE = CE(single-site CP-K, weights-only) - CE(real model) on census rows; lower is better',
         'pred_a_dominates_by_2304':bool(ok_a),'pred_b_validity_moves':bool(ok_b),'pred_c_sane':bool(ok_c),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) both sites agg(2304) <= 0.5 x anchor ({0.5*ANC[1]:+.4f}/{0.5*ANC[3]:+.4f}): {'HELD' if ok_a else 'FAILED'}")
    print(f"(b) both sites valid(2304) >= 5: {'HELD' if ok_b else 'FAILED'}")
    print(f"(c) monotone + exact at 4608: {'HELD' if ok_c else 'FAILED'}")
