"""THE UNIFORM GRAMMAR'S TWO-LEDGER ROW (rung 192): circuit rows + era profile for the new frontier.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The registered frontier (S2284,
+1.2673 census) has no saved per-circuit rows and no era-profile comparison. Identical rung-188 rebuild,
rows saved, Spearman against the rung-132 profile (the S2275 era question at the new grammar).

REGISTERED PREDICTIONS:
  (a) REPRODUCTION: |census - 1.2673| <= 0.015.
  (b) CERTIFICATE STAGNATION: valid <= 5 of 62 (FAILURE = news).
  (c) ERA INVARIANCE CONTINUES: Spearman vs rung-132 >= 0.85.
NULL (c): the uniform grammar moved which circuits pay (< 0.7). PRICE: none (re-instrumentation).
Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: uniform grammar two-ledger row')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_uniform_rows_results.json'
...
if __name__=='__main__':
    ...
    res={'L2F_fresh':round(L2F,4),'L1F':round(L1F,4),'increment':round(inc,4),'census_agg':round(agg,4),
         'n_valid':nv,'per_class':PC,'circuits':rows,'era_spearman_vs_132':round(rho,4),'link_mean':round(_link,4),'const_mean':round(_const,4),
    ...
    print(f"(a) |census {agg:+.4f} - 1.2673| <= 0.015: {'HELD' if pa else 'FAILED'}")
