"""THE CHEAP-END COMBINATION (rung 195): front 3456 + rank-12 patterns + no tailE.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The value-minimal uniform config:
47.8M front + 9.1M patterns, tailE dropped. r12 base was +1.3784; drop-tailE measured -0.152 on the r16
base.

REGISTERED PREDICTIONS:
  (a) census <= 1.28 (the drop composes at r12).
  (b) name <= 1.75.
  (c) SANITY: census >= 0.5; inert tripwire vs +1.3784.
NULL: non-additive at r12 (census > 1.33). PRICE: 56.9M values total - the cheap end. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: cheap-end combination')
    raise SystemExit(0)
import torch
...
OUT=PT+'combo_cheap_results.json'
...
if __name__=='__main__':
    ...
    pa=agg<=1.28
    pb=PC.get('name',9.9)<=1.75
    pc=agg>=0.5
    if abs(agg-1.3784)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: combo inert')
    ...
    print(f"(a) census {agg:+.4f} <= 1.28: {'HELD' if pa else 'FAILED'}")
    print(f"(b) name {PC.get('name',9.9):+.4f} <= 1.75: {'HELD' if pb else 'FAILED'}")
