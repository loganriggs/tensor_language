"""HALVES TEST II (rung 238): tail blocks (10-17) exact, motifs r64 - the mirror of S2335.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2335: motif-half exactness unlocked
7->9 certificates at +0.0608. Complementary claim: the tail tolerates r64, so tail-half exactness should
buy census (the tail has more replaced heads) but FEW certificates.
REGISTERED PREDICTIONS (arms: THIS vs r64 0.0852/7 and motif-exact 0.0608/9):
  (a) census <= 0.075 (72 tail heads upgraded).
  (b) TAIL BUYS LITTLE AT CERTIFICATE GRAIN: valid <= 8.
  (c) SANITY: census in [0.01, 0.12]; inert tripwires vs r64 AND motif-exact receipts.
NULL: both halves unlock equally (valid >= 9) - the bottleneck is not localized after all.
PRICE: r64 config + tail head-patterns at full rank. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: tail-exact halves mirror')
    raise SystemExit(0)
import torch
...
OUT=PT+'tail_exact_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=64; SEL['qk_rmap']={li:128 for li in range(10,18)}; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    pb=nv<=8
    pc=0.01<=agg<=0.12
    if abs(agg-0.0852)<1e-4 or abs(agg-0.0608)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: tail-exact map inert')
    ...
    print(f"(b) tail buys little - valid {nv} <= 8; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
