...
pred_b self removal changes native interaction target>=10%each family.
pred_c frozen compression error<=10%self removal effect, both endpoints each family.
...
from response_diagonal_conic_v1 import execute as self_product
@torch.no_grad()
def main():
 files=json.loads((P/'THREE_TERM_FRESH_SELF_REMOVAL_V1_BINDING.json').read_text())['files']
 ...
 out=P/'THREE_TERM_FRESH_SELF_REMOVAL_V1_RESULT.json';assert not out.exists()
 ...
 for index,row in enumerate(rows):
  ...
  for amplitude in (a,b,a+b):
   ...
   zgen=branch(amplitude,context).float().double();full.append(exact_post(zgen,amplitude,exact).float());rho=zgen.square().mean(-1,keepdim=True)+torch.finfo(torch.float32).eps;node=.5*self_product(coefficients(amplitude,context['response']),exact['bank'])/rho;compressed.append((exact_post(zgen,amplitude,exact)-node).float())
  outcomes=[]
 ...
 previous=json.loads((P/'THREE_TERM_FRESH_TEMPLATE_V1_RESULT.json').read_text())['cells']
 replay=max(abs(a-b) for c,old in zip(cells,previous) for arm in range(2) for rr,oo in zip(c['scores'][arm],old['scores'][arm]) for a,b in zip(rr,oo))
 own=[]
 for family in range(4):
  exact=torch.tensor([c['interactions'][1] for c in cells[family*12:(family+1)*12]],dtype=torch.float64)
  removed=torch.tensor([c['interactions'][2] for c in cells[family*12:(family+1)*12]],dtype=torch.float64)
  approximation=torch.tensor([c['interactions'][2] for c in previous[family*12:(family+1)*12]],dtype=torch.float64)
  native=torch.tensor([c['interactions'][0] for c in cells[family*12:(family+1)*12]],dtype=torch.float64)
  own.append(dict(family=family,self_effect_norms=(exact-removed).norm(dim=0).tolist(),relative_self_effect=((exact-removed).norm(dim=0)/native.norm(dim=0)).tolist(),compression_own_relative_error=((approximation-exact).norm(dim=0)/(exact-removed).norm(dim=0)).tolist()))
 result={'pred_a':A and replay==0,'pred_b':all(g['relative_self_effect'][0]>=.1 for g in own),'pred_c':all(max(g['compression_own_relative_error'])<=.1 for g in own),'pred_d':all(g['capability']>0 for g in groups),'anchors':anchors,'reference_replay_maxabs':replay,'own_groups':own,'groups':groups,'cells':cells,'seconds':time.perf_counter()-tic,'scope':'Same48 previously fresh templates, now reused for full residual-self node removal; frozen three-term approximation evaluated relative to own self-term removal effect. Native background/suffix, no new fit or independent OOD panel.'}
