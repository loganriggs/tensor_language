...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['attn2_motif_class_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; class maps of the block-3/4 motif replacements (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'attn34_motif_class_results.json'
...
if __name__=='__main__':
    ...
    SEL['skipset']=(); SEL['clsdmg']=True; SEL['motif_off']=()
    ...
    for b9 in (3,4):
        SEL['motif_off']=(b9,)
        print(f'ARM: block-{b9} motifs REAL',flush=True)
        _=main()
        cv=SEL['cev']
        if float((cev_full-cv).abs().sum())<1e-3:
            raise SystemExit(f'INSTRUMENT FAIL: b{b9} identical to full')
        diff=(cev_full-cv); tot=float(diff.sum())
        sh[b9]={CLS[k]:round(float(diff[clsflat==k].sum())/max(abs(tot),1e-9),4) for k in range(10)}
        sh[b9]['_total']=round(tot,1)
        print(f'  b{b9} total {tot:+.1f}: '+' '.join(f'{c}:{v:+.3f}' for c,v in sh[b9].items() if c!='_total'),flush=True)
    top3=max((c for c in sh[3] if c!='_total'),key=lambda c:sh[3][c])
    pa=top3 in ('subword','other')
    pb=0.453>sh[3]['subword']>sh[4]['subword']
    pc=abs(r_full['L2_F']-2.6662)<=0.01
    res={'full':{'L2_F':r_full['L2_F']},'shares_b3':sh[3],'shares_b4':sh[4],'b2_subword_ref':0.453,
         'convention':'per-position CE(full) - CE(block-b motifs real) on FR; positive = the replacement hurts there',
         'pred_a_b3_top':bool(pa),'pred_b_monotone_subword':bool(pb),'pred_c_reproduces_s2144':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) b3 top class {top3} in (subword,other): {'HELD' if pa else 'FAILED'}")
    print(f"(b) subword monotone 0.453 > {sh[3]['subword']} > {sh[4]['subword']}: {'HELD' if pb else 'FAILED'}")
