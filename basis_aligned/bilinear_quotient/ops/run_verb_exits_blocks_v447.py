#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_block4_leads_the_low_exits pred_c_a_block_9_to_12_leads_the_high_exits pred_d_singles_add pred_e_blocks_0_1_carry_little
"""The verb readout's exits from the noun, by block (v447). v446: for verbs >= 2 tokens after the noun, every head's value at the noun closes 1.03 of the
agreement margin gap, but the pronoun copiers carry little (readers 0.07, copier 0.14). Which blocks' values carry number to a distant verb? Every head's
value at the noun, one block at a time (0..17), on the 79 distant rows.
PREDICTIONS (scored as written; failures preserved; priors from v430 / v431 / v446)
    pred_a_baseline_replays              the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_block4_leads_the_low_exits    block 4 has the largest single-block effect among blocks 0-8 (the copier route feeds the rebuild the verb reads). Prior: likely
    pred_c_a_block_9_to_12_leads_the_high_exits  the largest single-block effect among blocks 9-17 is one of blocks 9-12. Prior: unsure
    pred_d_singles_add                   the 18 single-block effects sum to within 0.25 of the joint 1.03
    pred_e_blocks_0_1_carry_little       blocks 0 and 1 together close <= 0.05
...
OUT = ROOT / "circuits/followups/verb_exits_blocks_v447_result.json"
CANDIDATE_ID = "chain.verb_exits_blocks_v447"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, ADD_TOL, JOINT_V446, B01_MAX = 1e-3, 0.25, 1.029, 0.05
...
FORWARDS_MAX = 58
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_block4_leads_the_low_exits": "block 4 first of 0-8", "pred_c_a_block_9_to_12_leads_the_high_exits": "one of 9-12 first of 9-17", "pred_d_singles_add": "within 0.25 of 1.029", "pred_e_blocks_0_1_carry_little": "<= 0.05"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "add_tol": ADD_TOL, "joint_v446": JOINT_V446, "b01_max": B01_MAX}}
    ...
    MODES = {f"b{b}": ({b: list(range(N_HEAD))}, (0,)) for b in range(18)}; forwards = 0
    def logits_margin(x):
    ...
    c = verb["closed"]
    v = {m: c[m]["verb"] for m in c}; low = {m: v[m] for m in v if int(m[1:]) <= 8}; high = {m: v[m] for m in v if int(m[1:]) >= 9}; report["sum_singles"] = sum(v.values())
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_block4_leads_the_low_exits": max(low, key=low.get) == "b4", "pred_c_a_block_9_to_12_leads_the_high_exits": max(high, key=high.get) in {"b9", "b10", "b11", "b12"},
                   "pred_d_singles_add": abs(sum(v.values()) - JOINT_V446) <= ADD_TOL, "pred_e_blocks_0_1_carry_little": v["b0"] + v["b1"] <= B01_MAX}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "verb_exits_blocks_result_v447", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
