#!/usr/bin/env python3
# BQGATE: scale-adjusted donor control;34bodyforwards134seq,8readoutarms.
"""pred_a instrument<=1e-3abs/1e-5rel, pred_b adjusted mean/donor CE>=.005;
pred_c adjusted donor damage<=.10raw, pred_d unchanged control CEs<=1e-12.
Null: raw scalar donor damage is mostly context-scale mismatch. All native
parameters retained.34bodyforwards134seq, no fitting refinement. Full gates in
CALIBRATION_DIMENSIONLESS_DONOR_V1_PREREGISTRATION.md.
"""
...
import calibration_dimensionless_donor_v1 as N
import bilin18_observed_model_facade as facade
from induction_context_transport_v2 import digest
from circuit_fast_screen_managed_runner import atomic_create_json
OUT=POLY/'CALIBRATION_DIMENSIONLESS_DONOR_V1_RESULT.json'
BINDING=POLY/'CALIBRATION_DIMENSIONLESS_DONOR_V1_BINDING.json'
...
def fit_axis(outputs,rows):
    ...
    hist=np.bincount(targets,minlength=50304)
...
def main():
    ...
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
        print(json.dumps(dict(dryrun=True,model_loaded=False,gpu_accessed=False,forwards=34,sequences=134,controls=N.controls())));return
    ...
    try:
        with torch.inference_mode():
            fits=[r for r in rows if r['split']=='FIT'];fit_m=[];fit_taus=[]
            for start in range(0,48,4):
                ...
                fq=C.scalar(u,Q,beta);fit_taus.append((fq/N.units(h.double()-fq[...,None]*w)).cpu())
                if start==0:
            ...
            mean_tau=torch.cat(fit_taus).mean().cuda()
            fit_stats=dict(mean=float(mean_q),sd=float(fit_q.std(unbiased=False)),mean_tau=float(mean_tau),direction_replay=replay)
            del fit_m,fit_q
            artifact=POLY/'CALIBRATION_DIMENSIONLESS_DONOR_V1_AXES.pt';assert not artifact.exists()
            torch.save(dict(A=axes['A'].cpu(),B=axes['B'].cpu(),fit_mean_q=mean_q.cpu(),fit_mean_tau=mean_tau.cpu()),artifact)
            for cohort in ('FW_HOLDOUT','PILE_SHIFT'):
                ...
                background=hs.double()-q_all[...,None]*saved['w'];r_all=N.units(background);tau_all=q_all/r_all
                adjusted_q=r_all*tau_all.roll(-1,0)
                ...
                adjusted_expected=N.read_dimensionless(background[:4].cuda(),tau_all.roll(-1,0)[:4].cuda(),w,U)
                online,_,_=body(ids,donor=adjusted_q[:4].cuda(),compiled=True)
                bridges[cohort+'_online_adjusted_donor']=bridge(logits(online),adjusted_expected)
                del expected,online,adjusted_expected
                targets=np.array([r['tokens'][1:] for r in selected]).ravel();common=np.isin(targets,top)
                losses={k:[] for k in ('native','remove_ref','remove_A','remove_B','mean','donor','mean_tau','donor_tau')}
                ...
                for start in range(0,len(hflat),256):
                    ...
                    z['mean_tau']=N.read_dimensionless(b,mean_tau.expand(len(h)),w,U)
                    z['donor_tau']=N.read_dimensionless(b,tau_all.roll(-1,0).reshape(-1)[start:start+256].cuda(),w,U)
                ...
                reports[cohort]=dict(tau_mean=float(tau_all.mean()),tau_sd=float(tau_all.std(unbiased=False)),background_rms_min=float(r_all.min()),background_rms_max=float(r_all.max()),rows=len(selected),tokens=len(targets),mean_ce=means,ce_effects=effects,split_effect_relative_errors=errors,operational_stability=bool(stable),useful_pairing=bool(useful),q_mean=float(q_all.mean()),q_sd=float(q_all.std(unbiased=False)),q_centered_energy_fraction=float((q_all-q_all.mean()).square().sum()/q_all.square().sum()),donor_minus_mean_ce=effects['donor']['all']-effects['mean']['all'])
                for i,r in enumerate(selected):
                    sl=slice(i*256,(i+1)*256);mask=common[sl]
                    per_row.append(dict(row_id=r['row_id'],donor_row_id=selected[(i+1)%len(selected)]['row_id'],split=cohort,frequent_count=int(mask.sum()),rare_count=int((~mask).sum()),tau_mean=float(tau_all[i].mean()),tau_sd=float(tau_all[i].std(unbiased=False)),background_rms_mean=float(r_all[i].mean()),q_mean=float(q_all[i].mean()),q_sd=float(q_all[i].std(unbiased=False)),ce_sums={k:dict(all=float(v[sl].sum()),frequent=float(v[sl][mask].sum()),rare=float(v[sl][~mask].sum())) for k,v in losses.items()}))
    finally:handle.remove()
    instrument=counts==[34,134] and finite and replay['relative']<=1e-10 and all(v<=1e-5 for v in producer_errors.values()) and all(v['max_abs']<=1e-3 and v['relative']<=1e-5 for v in bridges.values())
    ...
    predictions={'pred_a_instrument':bool(instrument),'pred_b_residual_context_pairing':bool(all(r['ce_effects'][k]['all']>=.005 for r in reports.values() for k in ('mean_tau','donor_tau'))),'pred_c_scale_accounts_for_raw_damage':bool(all(r['ce_effects']['donor']['all']>0 and r['ce_effects']['donor_tau']['all']<=.10*r['ce_effects']['donor']['all'] for r in reports.values())),'pred_d_unchanged_arm_replay':bool(replay_max<=1e-12)}
    instrument=instrument and predictions['pred_d_unchanged_arm_replay']
    out=dict(unchanged_arm_replay_max_ce_error=replay_max,terminal='complete' if instrument else 'invalid',predictions=predictions,reports=reports,per_row=per_row,split_axis_cosine=split_cosine,fit_stats=fit_stats,producer_relative_errors=producer_errors,bridges=bridges,price=dict(body_forwards=counts[0],sequences=counts[1],native_parameters=sum(p.numel() for p in model.parameters()),weight_saving=0,axes_file_bytes=artifact.stat().st_size),runner_sha256=digest(RUNNER),binding_sha256=digest(BINDING),axes_sha256=digest(artifact),checkpoint_sha256=checkpoint.weights_sha256,wall_seconds=time.perf_counter()-tic,scope='Conditional interchange execution, disjoint-row fit stability, reused evaluation text; not semantic donor-task transfer or full extraction')
