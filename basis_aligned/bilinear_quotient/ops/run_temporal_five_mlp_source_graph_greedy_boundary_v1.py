#!/usr/bin/env python3
"""Resolve the exact frozen greedy-prefix boundary between ranks 45 and 55."""
# BQGATE: EXPERIMENT pred_a_authority_self_clamp_finiteness_and_price pred_b_boundary_contains_a_feasible_prefix pred_c_feasibility_is_monotone_above_boundary pred_d_boundary_is_at_most_rank50 pred_e_selected_arm_is_crossfit_stable
...
PRIOR = ROOT / "circuits/prior_art/temporal_five_mlp_source_graph_greedy_boundary_v1.json"
GREEDY = ROOT / "circuits/followups/temporal_five_mlp_source_graph_greedy_prefix_v1_result.json"
...
OUT = ROOT / "circuits/followups/temporal_five_mlp_source_graph_greedy_boundary_v1_result.json"
EXPECTED = {"greedy": "5bc58ebb5b1f79bb4b9d4b24a5a341b95ad1ab21402ed76f679b2be5a0cf9d56",
            "deletion": "b674de9220b3fb3b13ded3f48d908614ba363c10ffcc34faf9a6a8637cc3b10c",
            "helper": "2de68cab2af7e9f1619bedc9c0692780574e015a64c3c2aaf7fbc53fd5d0f32a"}
PREFIXES = tuple(range(46, 55))
...
def main():
    paths = {"greedy": GREEDY, "deletion": DELETION, "helper": HELPER}; observed = {k: sha(v) for k, v in paths.items()}
    if observed != EXPECTED: raise RuntimeError(f"greedy-boundary authority changed: {observed}")
    dry = {"candidate_id": "temporal_auxiliary.five_mlp_source_graph_greedy_boundary_v1", "dryrun": True,
           "gpu_accessed": False, "model_loaded": False, "queue_touched": False,
           "model_forwards_max": 71, "fit_updates": 0, "model_updates": 0, "transformer_backwards": 0}
    ...
    top20, ranking = deletion["top20"], deletion["ranking"]
    ...
    for n in PREFIXES:
        ...
        report["jointly_feasible"] = addback.feasible(report); reports[str(n)] = report
    ...
    feasible = [n for n in PREFIXES if reports[str(n)]["jointly_feasible"]]; selected = min(feasible) if feasible else None
    stable = False
    if selected:
        fits = reports[str(selected)]["fits"]; emin = min(fits["even_fit"]["coordinate"]["signed_projection"].values()); omin = min(fits["odd_fit"]["coordinate"]["signed_projection"].values())
    ...
    monotone = selected is not None and all(reports[str(n)]["jointly_feasible"] for n in PREFIXES if n >= selected)
    predictions = {"pred_a_authority_self_clamp_finiteness_and_price": bool(pa),
                   "pred_b_boundary_contains_a_feasible_prefix": selected is not None,
                   "pred_c_feasibility_is_monotone_above_boundary": bool(monotone),
                   "pred_d_boundary_is_at_most_rank50": selected is not None and selected <= 50,
                   "pred_e_selected_arm_is_crossfit_stable": bool(stable)}
    compact = {key: {"allowed_count": report["allowed_count"], "jointly_feasible": report["jointly_feasible"],
                     ...
                     "control_flip_max": max(f["control"]["top1_flip_fraction"] for f in report["fits"].values())} for key, report in reports.items()}
    terminal = "invalid" if not pa else "exact_greedy_boundary" if all(predictions.values()) else "nonmonotone_or_late_boundary"
    result = {"schema": "temporal_five_mlp_source_graph_greedy_boundary_result_v1", "started_utc": started, "finished_utc": now(),
              "serial_seconds": time.perf_counter()-tic, "authority_sha256": EXPECTED, "selected_rank": selected,
    ...
    atomic_create_json(OUT, result); print(json.dumps({k: result[k] for k in ("selected_rank", "arm_summary", "predictions", "terminal", "price")}, sort_keys=True))
