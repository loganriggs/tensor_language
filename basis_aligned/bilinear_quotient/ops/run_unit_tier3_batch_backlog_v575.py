...
"""v575: work the AUTHORED BACKLOG instead of authoring more cells.
...
REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here; SIX predicates, because this
protocol carries the row-5 amendment as its own row and I registered only five on the first draft -- pred_lint
caught the mismatch against the returned names before the rung ran):
  pred_a_four_rows    AT LEAST k_four = 1 of the eight new behaviours clears ALL FOUR tier rows at once.
                                                                                                      prior 80%
  pred_b_row2         at least k_row2 = 1 of the eight clears row 2.                                  prior 85%
  pred_c_row3         at least k_row3 = 2 clear row 3.                                                prior 80%
  pred_d_row4         at least k_row4 = 1 clears row 4, the C-upper-bound row that needs a usable control.
                                                                                                      prior 70%
  pred_e_row5_amended at least k_row5 = 1 clears row 5 under the amendment, with its removal share inside the
                      0.5 to 1.4 band.                                                                prior 70%
  pred_f_instrument   finiteness_selection rides along and reproduces its parent receipt: unit count matches and the
                      direction-B absolute difference is within instr_tol = 0.02, so a batch-wide drift is caught
                      rather than absorbed into the new members.                                      prior 90%
HOW IT READS. Most of the eight lifting: the backlog is real inventory and the route to the count is working what is
already authored, not writing more cells -- and the remaining eight plus the other families' 38 follow. Few lifting:
these cells were left unlifted for a reason I had forgotten, the backlog is smaller than it looks, and the honest
number of available circuits is closer to the count than to 286. e FALSE: the batteries drifted and no row here is
comparable to the corpus.
SCOPE. Eight behaviours, one family, one frame; units and tier rows only, no counting.
Smoke: V575_SMOKE=<out.json> (CPU, V575_SMOKE_ROWS=4).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_backlog_v575_result.json"
...
NAMES = {"verb_preposition_against_for": "verb_preposition_against_for",
         "verb_preposition_at_amid": "verb_preposition_at_amid",
         "verb_preposition_at_toward": "verb_preposition_at_toward",
         "verb_preposition_from_beyond": "verb_preposition_from_beyond",
         "verb_preposition_from_under": "verb_preposition_from_under",
         "verb_preposition_from_with": "verb_preposition_from_with",
         "verb_preposition_into_from": "verb_preposition_into_from",
         "verb_preposition_on_for": "verb_preposition_on_for",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_backlog_v575", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V575_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V575_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V575_SMOKE_NAMES", "verb_preposition_against_for,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_backlog_v575", "candidate_id": "corpus.unit_tier3_batch_backlog_v575", "bars": BARS,
