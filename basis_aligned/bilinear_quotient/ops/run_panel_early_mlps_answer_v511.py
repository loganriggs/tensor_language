#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_early_mlps_oppose_on_the_panel pred_c_mlps_9_16_replay pred_d_mlps_1_17_replay pred_e_early_plus_late_near_all
"""The early MLPs at the panel's answer position (v511). v510: on the panel MLPs 9-16 at the pronoun answer close 0.092 but MLPs 1-17 only 0.035, so the
early MLPs there must work against the margin (on text 1-17 = 9-16, so they are neutral there). Here MLPs 1-8 at the answer position alone on the 48 panel
pairs, with 9-16 and 1-17 as replays.
PREDICTIONS (scored as written; failures preserved; priors from v510)
    pred_a_baseline_replays          the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_early_mlps_oppose_on_the_panel  MLPs 1-8 at the answer position close <= -0.03 of the panel margin gap. Prior: likely (0.035 - 0.092)
    pred_c_mlps_9_16_replay          MLPs 9-16 close 0.092 +- 0.03
    pred_d_mlps_1_17_replay          MLPs 1-17 close 0.035 +- 0.03
    pred_e_early_plus_late_near_all  (MLPs 1-8) + (MLPs 9-16) is within 0.05 of MLPs 1-17 (MLP 17 alone adds little). Prior: unsure
...
OUT = ROOT / "circuits/followups/panel_early_mlps_answer_v511_result.json"
CANDIDATE_ID = "chain.panel_early_mlps_answer_v511"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, EARLY_MAX, M916_V510, MALL_V510, BAND, ADD_TOL, UNIT = 1e-3, -0.03, 0.092, 0.035, 0.03, 0.05, 701
import random
...
FORWARDS_MAX = 13
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_early_mlps_oppose_on_the_panel": "<= -0.03", "pred_c_mlps_9_16_replay": "0.092 +- 0.03", "pred_d_mlps_1_17_replay": "0.035 +- 0.03", "pred_e_early_plus_late_near_all": "within 0.05"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "early_max": EARLY_MAX, "m916_v510": M916_V510, "mall_v510": MALL_V510, "band": BAND, "add_tol": ADD_TOL, "unit": UNIT}}
    ...
    MODES = {"mlps1_8": ({}, (0,)), "mlps9_16": ({}, (0,)), "mlps1_17": ({}, (0,))}; forwards = 0
    def logits_margin(x):
    ...
    def run(seqs, pos, mode):
        ...
        with torch.no_grad():
            ...
            for l, block in enumerate(blocks):
                ...
                m_out = block.mlp(xm)
                if (mode == "mlps1_8" and 1 <= l <= 8) or (mode == "mlps9_16" and 9 <= l <= 16) or (mode == "mlps1_17" and 1 <= l <= 17):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_early_mlps_oppose_on_the_panel": mc["mlps1_8"] <= EARLY_MAX, "pred_c_mlps_9_16_replay": abs(mc["mlps9_16"] - M916_V510) <= BAND,
                   "pred_d_mlps_1_17_replay": abs(mc["mlps1_17"] - MALL_V510) <= BAND, "pred_e_early_plus_late_near_all": abs((mc["mlps1_8"] + mc["mlps9_16"]) - mc["mlps1_17"]) <= ADD_TOL}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "panel_early_mlps_answer_result_v511", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
