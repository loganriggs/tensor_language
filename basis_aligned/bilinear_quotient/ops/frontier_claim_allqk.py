"""REGISTERED FRONTIER CLAIM: ALL-QK-16 (rung 188).

CONVENTION (S2135): frontier L2 = CE ADDED ABOVE THE REAL MODEL on the S312 fresh windows; LOWER IS BETTER.
Rung 185 printed DESCRIPTIVELY L2_F +1.3498 / census +1.2673 for the uniform grammar (rank-16 QK patterns at
all 148 replaced heads, dictionaries retired). Identical deterministic rebuild; claim bars as predictions.

REGISTERED PREDICTIONS (anchors: rung-185 receipt):
  (a) NEW FRONTIER: L2_F fresh <= 1.37.
  (b) REPRODUCTION: |census - 1.2673| <= 0.015.
  (c) REPRODUCTION: |L2_F - 1.3498| <= 0.015.
NULL: 185's print was noise (L2_F >= 1.40). PRICE: 47.8M front + 12.1M patterns; dictionary store retired.
Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: registered claim all-QK-16')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_claim_allqk_results.json'
...
if __name__=='__main__':
    ...
    pa=L2F<=1.37
    pb=abs(agg-1.2673)<=0.015
    pc=abs(L2F-1.3498)<=0.015
    ...
    print(f"(a) L2F {L2F:+.4f} <= 1.37: {'HELD' if pa else 'FAILED'}")
    print(f"(b) |census {agg:+.4f} - 1.2673| <= 0.015: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |L2F - 1.3498| <= 0.015: {'HELD' if pc else 'FAILED'}")
