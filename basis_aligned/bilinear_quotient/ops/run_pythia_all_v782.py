...
"""HOW MUCH RANK does the late model need? (v782). The mechanism chapter closed the optimisation question: on Pythia-160m final the rank-32 joint program lands at recovery 0.932 (v760), 0.928 with 4x the steps (v778), 0.935 with 4x the tokens per step (v779), 0.915 if every head is refined alone (v780 - per-head optimality does not compose), and 0.9317 after polishing the refined programs jointly (v781, from either start). So ~0.93 at rank 32 is the class limit, not a fitting artifact. This rung prices the missing dimension: joint fits at rank 40 and rank 48 (rank 64 = the full head dim would make the content term exact and is trivially free). The question Logan's ladder asks in rank terms: how much more read-side rank does a 300B-token model need to reach the ~0.98 the 5B-token bilinear models reach at rank 16-64? ROW-CENTRED logits (Logan, 22 Sep: 'test the specific hypothesis of why we're seeing what we're seeing'). MODEL = EleutherAI/pythia-160m (12 x 12 x 768, softmax, ROTARY on 16 of 64 head dims, GPT-NeoX tokenizer;
...
PREDICTIONS (scored as written; failures preserved)
    ...
    pred_c_rank32_cost          (rank 48) snapshot recovery >= 0.96, i.e. half the remaining gap to 1.0 closed by 16 extra directions. Prior: unsure
    pred_d_rank16_cost          (rank 40) snapshot recovery >= 0.95 (rank 32: 0.932). Prior: unsure
...
TAG = "pythia160m_rankladder_v782"
...
REPLAY_TOL, E4_MIN, REC48, REC40, RATIO = 0.002, 0.90, 0.96, 0.95, (0.5, 2.0)
PREDICTIONS = {"pred_a_instrument_replays": "+-0.002", "pred_b_kernel_energy_top4": ">= 0.90", "pred_c_rank32_cost": "rank 48 recovery >= 0.96", "pred_d_rank16_cost": "rank 40 recovery >= 0.95", "pred_e_rank32_values_preserved_scale": "median ratio in [0.5, 2]"}
...
def main() -> None:
    ...
    r48, r40 = results[48], results[40]
    predictions = {"pred_a_instrument_replays": abs(native - native_stock) <= REPLAY_TOL, "pred_b_kernel_energy_top4": float(energy[3]) >= E4_MIN, "pred_c_rank32_cost": r48["recovery"] >= REC48,
                   "pred_d_rank16_cost": r40["recovery"] >= REC40, "pred_e_rank32_values_preserved_scale": RATIO[0] <= r48["median_ratio"] <= RATIO[1]}
