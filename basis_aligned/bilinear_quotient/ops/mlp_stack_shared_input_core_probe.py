#!/usr/bin/env python
"""mlp_stack_shared_input_core_probe -- does the WHOLE MLP stack share one input coordinate system? Early-block shared core, cross-depth
core transfer, and one core for all 18 MLPs (k = 768/896/1024), constant filler, own weights (§2742 extension). CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_early_blocks_share_a_core pred_c_cores_transfer_across_depth pred_d_one_core_for_all_18
#                     pred_e_whole_mlp_stack_on_1024
...
Preregistration: polynomial_causal/MLP_STACK_SHARED_INPUT_CORE_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/mlp_stack_shared_input_core_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("mlp_stack_shared_input_core_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "MLP_STACK_SHARED_INPUT_CORE_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_stack_width_and_token_rank_probe_results.json"   # §2740
OUT = ROOT / "mlp_stack_shared_input_core_probe_results.json"
HASHES = {PREREG: "d4955ff5a4b08035d696a69a0b4575420af4ba81930ab046299175e7867ff70b", PRIOR: "7b5cd1fa7b5a04ffed1f22e2f1c40b5f3754fecfc3ef53ff0320c2a593034b03",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "mlp_stack_shared_input_core_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_LATE7_OWN_768 = 0.079; LAM = 1e-2
K = 768; KALL = [768, 896, 1024]; EARLY = list(range(0, 11)); LATE = list(range(11, 18)); ALL = list(range(NL))
SITES = [("mlp", l) for l in ALL]
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_max": 0.03, "c_max": 0.10, "d_max": 0.05, "e_max": 0.10}
NULLS = {"b_min": 0.10, "c_min": 0.30, "d_min": 0.15, "e_min": 0.25}
...
def main():
    ...
    st = {s_: {"Gx": torch.zeros(D, D, dtype=torch.float64, device=DEV), "sx": torch.zeros(D, dtype=torch.float64, device=DEV)} for s_ in SITES}
    n = 0
    for idx, W_, X_ in collect_pass(m, fit_rows, SITES):
        n += X_[SITES[0]].shape[0]
        for s_ in SITES:
            xh = X_[s_].double(); st[s_]["Gx"] += xh.T @ xh; st[s_]["sx"] += xh.sum(0)
    me = None
    heads = {}
    for s_ in SITES:
    ...
    def core_of(ls):
        C = sum(heads[("mlp", l)]["Cx"] for l in ls) / len(ls); _, U_ = torch.linalg.eigh(C); return U_.flip(1).float()
    U_early, U_late, U_all = core_of(EARLY), core_of(LATE), core_of(ALL)
    def capture(l, U_):
        C = heads[("mlp", l)]["Cx"]; return float((U_.double().T @ C @ U_.double()).trace() / C.trace())
    cap = {str(l): {"own": capture(l, heads[("mlp", l)]["U"][:, :K]), "early": capture(l, U_early[:, :K]), "late": capture(l, U_late[:, :K]), "all": capture(l, U_all[:, :K])} for l in ALL}
    ratio = {nm: {str(l): cap[str(l)][nm] / cap[str(l)]["own"] for l in ALL} for nm in ("early", "late", "all")}
    log(stage="cores", eff_rank={str(l): round(heads[("mlp", l)]["eff_rank"], 1) for l in ALL}, ratio_all={l: round(v, 3) for l, v in ratio["all"].items()})
    def run(parts):
        patch = {}; objs = []
        for p_ in parts:
    ...
    blk = lambda ls, k, U_=None: {("mlp", l): OwnHead(l, k, "const", None if U_ is None else U_[:, :k]) for l in ls}
    arms = {"LATE7_OWN_768": run([blk(LATE, K)]), "EARLY11_OWN_768": run([blk(EARLY, K)]), "EARLY11_SHARED_768": run([blk(EARLY, K, U_early)]),
            "EARLY11_ON_LATE_CORE_768": run([blk(EARLY, K, U_late)]), "LATE7_ON_EARLY_CORE_768": run([blk(LATE, K, U_early)]), "ALL18_OWN_768": run([blk(ALL, K)])}
    for k in KALL:
        arms[f"ALL18_SHARED_{k}"] = run([blk(ALL, k, U_all)])
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    gaps = {"early_shared_minus_own": arms["EARLY11_SHARED_768"] - arms["EARLY11_OWN_768"], "early_on_late_minus_own": arms["EARLY11_ON_LATE_CORE_768"] - arms["EARLY11_OWN_768"],
            "late_on_early_minus_own": arms["LATE7_ON_EARLY_CORE_768"] - arms["LATE7_OWN_768"], "all18_shared_minus_own": arms["ALL18_SHARED_768"] - arms["ALL18_OWN_768"],
            "all18_own_minus_early_plus_late": arms["ALL18_OWN_768"] - arms["EARLY11_OWN_768"] - arms["LATE7_OWN_768"]}
    cross = max(gaps["early_on_late_minus_own"], gaps["late_on_early_minus_own"])
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["LATE7_OWN_768"] - PRIOR_LATE7_OWN_768) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_early_blocks_share_a_core': bool(gaps["early_shared_minus_own"] <= BARS["b_max"]),
        'pred_c_cores_transfer_across_depth': bool(cross <= BARS["c_max"]),
        'pred_d_one_core_for_all_18': bool(gaps["all18_shared_minus_own"] <= BARS["d_max"]),
        'pred_e_whole_mlp_stack_on_1024': bool(arms["ALL18_SHARED_1024"] <= BARS["e_max"]),
    }
    nulls = {"b_null_ge_.10": bool(gaps["early_shared_minus_own"] >= NULLS["b_min"]), "c_null_ge_.30": bool(cross >= NULLS["c_min"]),
             "d_null_ge_.15": bool(gaps["all18_shared_minus_own"] >= NULLS["d_min"]), "e_null_ge_.25": bool(arms["ALL18_SHARED_1024"] >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "k": K, "ks_all": KALL,
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "late7_own_768": arms["LATE7_OWN_768"], "prior_late7_own_768": PRIOR_LATE7_OWN_768},
           "ce_added": arms, "gaps": gaps, "cross_depth_max_gap": cross, "input_eff_rank": {str(l): heads[("mlp", l)]["eff_rank"] for l in ALL},
