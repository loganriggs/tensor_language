# WHERE DOES THE TRACKING COME FROM -- the tables, or the fallback map?
#
# §1885 and §1886 certified that the compiled 36-site program tracks the live model's top-1 at ~5.8x a
# permutation null on frequent targets and ~6.9x on targets never seen, on both the deployed build and
# §1882's 46%-cheaper one. That is the strongest positive statement in this arc about what the program
# reproduces, and it is currently an aggregate over two structurally different mechanisms.
#
# At a COVERED current token the program emits that token's exact length-1 site outputs, looked up. At an
# UNCOVERED current token it emits a rank-r linear map of the token embedding -- §1870's fallback, which
# §1866-§1880 spent fourteen sections pricing and which still carries +0.59560 / +0.67209 / +0.67172 nats
# of deficit against its own ceiling. These are not the same object and there is no reason they should
# track the model equally.
#
# Note the axis change, and it is the point: every bucket split since §1789 has been on the TRUE TARGET's
# frequency, which asks what the program can PRODUCE. This splits on the CURRENT token's coverage, which
# asks which MECHANISM produced it. No section has made that split on any instrument.
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY. Rung 3: §1886's open question, in-lane half.
#
...
#   pred_a THE TABLES CARRY THE TRACKING: enrichment at COVERED current tokens exceeds enrichment at
#          UNCOVERED ones by at least 1.5x, on all three roles. The table is the model's own length-1
#          output and the map is a rank-64 linear guess, so if the tracking is mechanistic the gap should
#          be large. If FALSE the fallback tracks the model about as well as an exact lookup does, which
#          would say the 5.8x is not coming from the compiled machinery at all but from something both
#          mechanisms share -- the most deflationary reading still standing after §1885.
#   pred_b AND THE FALLBACK STILL BEATS CHANCE: enrichment at UNCOVERED current tokens is at least 2.0x,
#          on all three roles. Even a rank-64 embedding map should carry SOME of the model's preference
#          if it is fitted to the model's own rows. If FALSE the fallback contributes nothing beyond
#          shared marginals and §1870's entire lever operates on CE without touching top-1 behaviour --
#          which would reframe §1866-§1882 as a CE story only.
#   pred_c AND THE COVERED SPLIT IS NOT A FREQUENCY PROXY: the covered/uncovered enrichment gap survives
#          within the 125+ target bucket alone, at least 1.2x, on all three roles. Covered current tokens
#          are commoner tokens, so a raw split could be measuring target frequency again by proxy; this
#          holds the target bucket fixed. If FALSE, pred_a is confounded and says nothing new.
#   pred_d CONTROLS: coverage is exactly 16,110; the covered and uncovered position counts sum to the
#          total; and the LIVE model's accuracy is reported on both halves, since it is the same model on
#          both and any difference there is a property of the POSITIONS, not of the program -- the
#          fifteenth known-answer check is that the two halves partition.
...
OUT = PT + 'ops/tracking_by_mechanism_results.json'
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'TRACKING BY MECHANISM | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    pb = all(mu[e]['enrichment'] >= 2.0 for e in roles)
    ...
    print(f'  and the FALLBACK still beats chance (>=2.0x) -> {pb}  ' + '  '.join(
        f'{e} {mu[e]["enrichment"]:.2f}x' for e in roles), flush=True)
