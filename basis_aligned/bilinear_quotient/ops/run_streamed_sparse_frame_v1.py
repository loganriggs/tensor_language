...
"""pred_a identities<=1e-8; pred_b both locally converge; pred_c25%gain;
pred_d cross-start functioncos>=.95. 600sec/200steps each; no data fit.
"""
...
def main():
    ...
    for arm in ('spectral','haar120463'):
        if arm=='spectral':q=frame
        else:q=torch.linalg.qr(torch.randn(1152,1152,device='cuda',generator=torch.Generator(device='cuda').manual_seed(120463))).Q
        ...
        q,e,report=fit(q,l,r,w,total,4096,max_steps=200,seconds=600,tolerance=1e-6,callback=callback)
        c=coefficients(q,l,r,w,e)
        errors.extend([float((q.T@q-eye).abs().max()),abs(float(c.square().sum()/total)-report['history'][-1]['capture'])])
        if arm=='spectral':errors.append(abs(report['history'][0]['capture']/prior['capture']['4096']-1))
    ...
    result={'pred_a':max(errors)<=1e-8,'pred_b':all(r['converged'] for r in reports),
            'pred_c':best>=1.25*prior['capture']['4096'],'pred_d':cosine>=.95}
    result.update(reports=reports,cosine=cosine,best_capture=best,identity_errors=errors,
                  controls=controls,execution_seconds=time.perf_counter()-tic,source_shas=binding,artifact_sha=digest(artifact),
                  scope='Full-input orthogonal sparse interaction search, paired coefficients; no causal circuit claim.')
