...
"""v250: within-family separability for the three v257 four-row passes. This is the FIRST rung that can test the verb_particle class against
itself: v255 opened it with one pre-screened cell (verb_particle_up_down, woke/calmed -> up/down) which passed all four rows and was counted as
a family-less SINGLETON, and v257 added verb_particle_out_down (found/settled -> out/down). A singleton is never separability-tested; two cells
are, so up_down is re-evaluated here under the v222 rule (a counted member is re-checked when its family grows) and out_down is scored under the
v242 rule (separable from every sibling +1; leaking into the counted incumbent joins it, +0). v257's third particle cell, verb_particle_on_away
(droned/slipped), missed row 2 (held 0.69) and row-2 failures do NOT join a control set, so the particle family here is exactly two cells.
The other two passes are verb_preposition cells under the family v248 left behind (9 counted, all 21 evals separable there, plus the three v253
row-4-failed three-row controls at_from / on_with / to_about). adjective_preposition_from_about is NOT here: it failed rows 2 and 4 (0101).

Families here (ALL members prepared as controls; only EVAL evaluated):
  verb_particle (1 counted after v255: verb_particle_up_down, 10 units 13:08/14:08/07:08/06:03/08:01, cext 0.991)
    + verb_particle_out_down (found/settled -> out/down; 12 units 13:08/07:08/06:03/14:08/08:01; held 0.86, dim 0.208, cdas 0.942, C 0.0059, cext 1.030)
  verb_preposition (9 counted after v248: verb_preposition (on/to), insisted (of/on), to_at, with_about, of_with, from_for, on_about, for_at,
    to_with; control-only: at_from, on_with, to_about)
    + verb_preposition_at_to (aimed/appealed -> at/to; 8 units 13:08/06:03/07:08/08:08/14:08; held 0.85, dim 0.485, cdas 1.043, C -0.0005, cext 1.008)
    + verb_preposition_from_about (suffered/boasted -> from/about; 15 units 06:03/07:08/13:08/08:08/11:03; held 0.88, dim 1.189, cdas 1.667, C 0.0069, cext 0.992)
The particle pair is the interesting one: up_down and out_down share FOUR of five top units (13:08/07:08/06:03/08:01 vs 13:08/14:08/07:08/06:03/08:01)
and both readouts are directional particles, so if any two cells in this corpus were going to share one direction it is these. Unit overlap has
not predicted fusion before (v214/v216/v220) and is not allowed to move the prior. The verb_preposition side has a sharper registered question
too: at_to shares ` at` with to_at and for_at and ` to` with to_at and to_with, i.e. BOTH of its readouts are already carried by counted siblings
in the same family -- v248 answered the one-shared-readout case (for_at vs to_at came out separable at sib 0.005), this is the two-shared case.
Recipe as v198/.../v248: per evaluated member, arm `own` (own C_fit control) and arm `fam` (own C_fit + EVERY sibling's A1 FIT prep,
control_weight 30 x len(controls)); evaluated on the member's held A1 (extraction), each sibling's held A1 (|CE| leak) and held C.
Units frozen from the parent receipts (v257 for the new members, v255 for up_down, earlier batteries for the rest).
...
EVAL = 3 new members + 10 counted re-checks (13 fits x 2 arms; verb_preposition carries 13 controls, verb_particle 1: ~25-30 min).
94 (after v248) -> up to 97 from this rung; a verb_particle fusion would instead make the class ONE circuit and leave the count at 94 + 2.

REGISTERED BEFORE THE RUN (each coded predicate is the sentence here; bars in BARS):
    pred_a_new_separable   at least 2 of the 3 new members are separable (k_new). Expected: at_to and from_about (v248 precedent 21/21);
                           the live fusion risk is out_down against up_down -- same class, four shared units, both directional particles.  prior 60%
    pred_b_counted_kept    at least 8 of the 10 re-checked counted members remain separable (k_kept; v248 kept 16/16, v246 12/12).          prior 65%
    pred_c_row4_kept       fam-arm own-C UB975 <= 0.01 on at least 6 of the 13 evaluated members (k_d; v248 17/21, v246 12/14).             prior 65%
...
Priors are not moved by unit overlap (v214/v216/v220) nor by a shared readout token (withdrawn at v224; v248 confirmed one shared readout inside
a family does not fuse).
Smoke: V250_SMOKE=<out.json> (CPU, V250_SMOKE_ROWS=4; all members are prepared as controls, V250_SMOKE_NAMES = the new particle cell plus a
verb_preposition re-check, so the two-cell family, the 13-control family and the instrument read are all exercised; steps 5).
BARS = {"cross_max": 0.05, "keep": 0.80, "ext_floor": 0.50, "c_ub_max": 0.01, "k_d": 6, "k_new": 2, "k_kept": 8, "instr_tol": 0.03}
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v250_result.json"
...
FAMILIES = {
    ...
    "verb_particle": ["verb_particle_up_down", "verb_particle_out_down"],
}
EVAL = ("verb_preposition_at_to", "verb_preposition_from_about", "verb_particle_out_down", "verb_preposition", "verb_preposition_insisted", "verb_preposition_to_at", "verb_preposition_with_about", "verb_preposition_of_with", "verb_preposition_from_for", "verb_preposition_on_about", "verb_preposition_for_at", "verb_preposition_to_with", "verb_particle_up_down", )
COUNTED = ("verb_preposition", "verb_preposition_insisted", "verb_preposition_to_at", "verb_preposition_with_about", "verb_preposition_of_with", "verb_preposition_from_for", "verb_preposition_on_about", "verb_preposition_for_at", "verb_preposition_to_with", "verb_particle_up_down", )
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES, **v213.NAMES, **v215.NAMES, **v217.NAMES, **v219.NAMES, **v221.NAMES, **v223.NAMES, **v225.NAMES, **v227.NAMES, **v229.NAMES, **v231.NAMES, **v233.NAMES, **v235.NAMES, **v237.NAMES, **v239.NAMES, **v241.NAMES, **v243.NAMES, **v245.NAMES, **v247.NAMES, **v249.NAMES, **v251.NAMES, **v253.NAMES, **v255.NAMES, **v257.NAMES}


def _parent_cdas(n):
    # the parent battery receipt that produced this member's units (each behaviour name is unique across batteries)
    for path in (V257, V255, V253, V251, V249, V247, V245, V243, V241, V239, V237, V235, V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v250", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V250_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V250_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v250", "candidate_id": "corpus.unit_family_separability_spec_v250",
