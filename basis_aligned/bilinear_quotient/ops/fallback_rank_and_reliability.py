# IS THE UNCOVERED SIGNAL LIMITED BY THE MAP'S RANK? -- connecting the reliability line to the cost arc.
#
# §1918 found the per-token reliability signal extends past the tables: at UNCOVERED positions the
# program's top-2 margin is still a pure token property (within-token spread 0.00e+00 / 0.00e+00 /
# 5.72e-06) and still orders agreement, at +3.90 / +4.10 / +2.06x against the covered arm's +9.50 /
# +9.79 / +9.28 -- 41% / 42% / 22% of the strength.
#
# The obvious explanation is that an uncovered row is a rank-64 linear map of the embedding (§1870) while
# a covered row is the model's own stored output, so the fallback simply carries less information. But
# §1877 showed the TABLE rank saturates at 512, and §1870's map has never been examined for what its rank
# does to anything except CE. If the weakness is the RANK, a rank-512 map should carry a stronger signal;
# if it is the map's linearity or its embedding input, rank will not help and the 41% is structural.
#
# §1880's law says a map above the table rank buys nothing, but that was measured in CE on covered-token
# reconstruction; it says nothing about whether a richer map makes the fallback's own CONFIDENCE more
# informative. This is the first thing to connect the reliability line to the cost arc since §1883.
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY. Rung 3: §1918's open question.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
# Populations named per LESSON 71; all gradients are under a GLOBAL null over their own arm.
#   pred_a RANK IS THE LIMIT: the rank-512 map's UNCOVERED gradient exceeds §1918's PUBLISHED rank-64
#          figures (+3.90 / +4.10 / +2.06) by at least 0.5 on all three roles. If TRUE the fallback's
#          confidence is rank-limited and buying map rank buys reliability as well as the CE §1870
#          measured -- a second reason to spend on it. If FALSE the 41% gap is structural to the map, and
#          no amount of rank makes an embedding-linear fallback speak for itself as well as a table does.
#   pred_b AND IT RAISES THE LEVEL TOO: the rank-512 map's top uncovered quartile exceeds §1918's
#          PUBLISHED 6.08 / 6.80 / 4.88 on all three roles. A gradient can widen because the bottom fell;
#          this checks the top rose. If FALSE and pred_a passes, rank sharpened the ordering without
#          making any uncovered token better tracked, which is a weaker and more specific claim.
#   pred_c AND THE COVERED ARM IS UNTOUCHED: covered gradient within 0.5 of §1914's PUBLISHED +9.50 /
#          +9.79 / +9.28. The map writes only uncovered rows, so a covered change means the build is
#          miswired. This is the arc's oldest control in its natural home -- the covered CE version of it
#          has returned exactly 0.00e+00 fourteen times.
#   pred_d CONTROLS: coverage is exactly 16,110; zero lookup misses across the whole vocabulary; and the
#          uncovered quartile counts are REPORTED, since §1918 flagged that at n = 349-1220 these carry
#          more null noise than §1916's +-0.08 and I have still not measured the spread at those n.
...
OUT = PT + 'ops/fallback_rank_and_reliability_results.json'
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'FALLBACK RANK AND RELIABILITY: is the uncovered signal rank-limited? | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    pa = all(gunc[e] >= S1918_UNC[e] + 0.5 for e in roles)
    pb = all(u(e, 3, 'global_enrichment') > S1918_Q3[e] for e in roles)
    pc = all(abs(gcov[e] - S1914_TOK[e]) <= 0.5 for e in roles)
    pd = (ncov == NCOV and all(miss[e] == 0 for e in roles))

    print(f'\n  agreement by PRECOMPUTED token margin, COVERED vs UNCOVERED arm:', flush=True)
    for e in roles:
        print(f'    {e:10s} COVERED   ' + '  '.join(
            f'q{i8} {q(e, "tok", i8, "global_enrichment"):5.2f}x' for i8 in range(4))
            + f'   grad {gcov[e]:+.2f}', flush=True)
        print(f'    {"":10s} UNCOVERED ' + '  '.join(
            f'q{i8} {u(e, i8, "global_enrichment"):5.2f}x (n{u(e, i8, "n"):5d})' for i8 in range(4))
            + f'   grad {gunc[e]:+.2f}', flush=True)
    print(f'\n  RANK IS THE LIMIT (uncovered gradient > §1918 + 0.5) -> {pa}  ' + '  '.join(
        f'{e} {gunc[e]:+.2f} vs §1918 {S1918_UNC[e]:+.2f}' for e in roles), flush=True)
    print(f'  and it RAISES THE LEVEL too (q3 > §1918) -> {pb}  ' + '  '.join(
        f'{e} {u(e, 3, "global_enrichment"):.2f} vs {S1918_Q3[e]:.2f}' for e in roles), flush=True)
    print(f'  and the COVERED arm is untouched (within 0.5 of §1914) -> {pc}  ' + '  '.join(
        f'{e} {gcov[e]:+.2f} vs {S1914_TOK[e]:+.2f}' for e in roles), flush=True)
    print(f'  coverage {ncov}, zero misses, uncovered quartile n reported -> control {pd}  ' + '  '.join(
        f'{e} n ' + '/'.join(str(u(e, i8, "n")) for i8 in range(4)) for e in roles), flush=True)
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_rank_is_the_limit': bool(pa),
                          'pred_b_raises_the_level': bool(pb),
