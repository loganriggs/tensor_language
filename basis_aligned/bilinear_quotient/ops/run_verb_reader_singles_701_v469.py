#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_11_3_alone_feeds_701 pred_c_7_8_alone_feeds_701_little pred_d_singles_add_to_the_four pred_e_11_3_is_most_of_the_four
"""Which verb reader feeds the detector, by edit (v469). v465: the four verb readers' value swap at the noun moves 0.48 of 701's pooled contrast at a distant
verb; v468 (fold): 11.3 is the largest writer of 701's inputs there and 7.8 barely writes them. Here each verb reader's value swap at the noun on its own
(11.3, 7.8, 13.1, 9.7) against the four together, measured on 701's activation at the answer position, 79 distant rows.
PREDICTIONS (scored as written; failures preserved; priors from v465 / v468)
    pred_a_baseline_replays          the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_11_3_alone_feeds_701      11.3's value swap alone moves >= 0.25 of 701's pooled contrast
    pred_c_7_8_alone_feeds_701_little  7.8's value swap alone moves <= 0.10
    pred_d_singles_add_to_the_four   the four singles sum to within 0.10 of the four-head swap (exits at one position add)
    pred_e_11_3_is_most_of_the_four  11.3 alone is >= 0.50 of the four-head effect. Prior: likely
PRICE (registered maximum): 3 batches x 6 passes = 18 forwards; 0 backwards; 0 fits. Bar <= 19.
"""
...
OUT = ROOT / "circuits/followups/verb_reader_singles_701_v469_result.json"
CANDIDATE_ID = "chain.verb_reader_singles_701_v469"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, H113_MIN, H78_MAX, ADD_TOL, SHARE_MIN, UNIT = 1e-3, 0.25, 0.10, 0.10, 0.50, 701
...
FORWARDS_MAX = 19
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_11_3_alone_feeds_701": ">= 0.25", "pred_c_7_8_alone_feeds_701_little": "<= 0.10", "pred_d_singles_add_to_the_four": "within 0.10", "pred_e_11_3_is_most_of_the_four": ">= 0.50 x V4"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "h113_min": H113_MIN, "h78_max": H78_MAX, "add_tol": ADD_TOL, "share_min": SHARE_MIN, "unit": UNIT}}
    ...
    MODES = {"V4": (V4, (0,)), "11.3": ({11: [3]}, (0,)), "7.8": ({7: [8]}, (0,)), "13.1": ({13: [1]}, (0,)), "9.7": ({9: [7]}, (0,))}; forwards = 0
    def logits_margin(x):
    ...
    mv = verb["moved"]; singles = mv["11.3"] + mv["7.8"] + mv["13.1"] + mv["9.7"]
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_11_3_alone_feeds_701": mv["11.3"] >= H113_MIN, "pred_c_7_8_alone_feeds_701_little": abs(mv["7.8"]) <= H78_MAX,
                   "pred_d_singles_add_to_the_four": abs(singles - mv["V4"]) <= ADD_TOL, "pred_e_11_3_is_most_of_the_four": mv["11.3"] >= SHARE_MIN * mv["V4"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "verb_reader_singles_701_result_v469", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
