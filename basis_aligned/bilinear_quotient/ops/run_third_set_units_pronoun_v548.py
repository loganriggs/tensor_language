#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_late_units_inert_at_the_pronoun_slot pred_c_random_units_do_not pred_d_hub_units_at_the_answer_small pred_e_hub_units_at_the_noun_replay
"""Unit components on the THIRD natural set, pronoun slot (v548). v463 (sets 1 + 2, 122 pairs, they - he margin): 829 / 953 / 1030 swapped at the noun close
0.133 of the gap, at the answer 0.009; 701 / 2059 at the answer 0.000 (the detector is verb-specific; v463's prior that it serves the pronoun slot FAILED and
is now the registered reading); two random MLP-17 units 0.000. v545: the five pronoun readers replay on the fresh set (0.730). Here the same four unit
edits on v544's 60 fresh pairs, priors registered first.
PREDICTIONS (scored as written; failures preserved; priors from v463)
    pred_a_baseline_replays                    the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_late_units_inert_at_the_pronoun_slot  701 / 2059 swapped at the answer close <= 0.02
    pred_c_random_units_do_not                 two random MLP-17 units swapped at the answer close <= 0.01
    pred_d_hub_units_at_the_answer_small       829 / 953 / 1030 swapped at the answer close <= 0.05
    pred_e_hub_units_at_the_noun_replay        829 / 953 / 1030 swapped at the noun close 0.133 +- 0.05
PRICE (registered maximum): 2 batches x 5 passes = 10 forwards (60 fresh pairs); 0 backwards; 0 fits. Bar <= 11.
"""
...
THIRD = [ROOT / "circuits/followups/pronoun_number_dod_natural_verb_rows_v544.json"]   # the third natural set (v544)
OUT = ROOT / "circuits/followups/third_set_units_pronoun_v548_result.json"
CANDIDATE_ID = "chain.third_set_units_pronoun_v548"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, LATE_MAX, RANDOM_MAX, HUB_A_MAX, HUB_V463, BAND = 1e-3, 0.02, 0.01, 0.05, 0.133, 0.05
import random
import random
UNIT_SETS = {"LATE_A": (17, (701, 2059), "answer"), "RANDOM_A": (17, tuple(random.Random(0).sample([j for j in range(4608) if j not in (701, 2059)], 2)), "answer"), "HUB_A": (8, (829, 953, 1030), "answer"), "HUB_N": (8, (829, 953, 1030), "noun")}
...
FORWARDS_MAX = 11
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_late_units_inert_at_the_pronoun_slot": "<= 0.02", "pred_c_random_units_do_not": "<= 0.01", "pred_d_hub_units_at_the_answer_small": "<= 0.05", "pred_e_hub_units_at_the_noun_replay": "0.133 +- 0.05"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "late_max": LATE_MAX, "random_max": RANDOM_MAX, "hub_a_max": HUB_A_MAX, "hub_v463": HUB_V463, "band": BAND}}
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_late_units_inert_at_the_pronoun_slot": abs(v["LATE_A"]) <= LATE_MAX, "pred_c_random_units_do_not": abs(v["RANDOM_A"]) <= RANDOM_MAX,
                   "pred_d_hub_units_at_the_answer_small": abs(v["HUB_A"]) <= HUB_A_MAX, "pred_e_hub_units_at_the_noun_replay": abs(v["HUB_N"] - HUB_V463) <= BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "late_units_pronoun_slot_result_v548", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
