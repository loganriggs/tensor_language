...
"""v613: swap which control enters the objective. Is row-4 selectivity the FIT, or the control?
...
Smoke: V613_SMOKE=<out.json> (CPU, V613_SMOKE_ROWS=4, V613_SMOKE_NAMES=<cell>).
"""
...
OUT = ROOT / "circuits/followups/unit_control_swap_v613_result.json"
...
def _plan():
    return {"candidate_id": "corpus.unit_control_swap_v613", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V613_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V613_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V613_SMOKE_NAMES", "verb_preposition_against_for,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_control_swap_v613", "candidate_id": "corpus.unit_control_swap_v613", "bars": BARS,
