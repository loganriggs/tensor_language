...
Two starts; factorinitialization<=60s, jointL-BFGS<=300s each. No text.
"""
import hashlib
import json
import os
...
def main():
    binding=json.loads((P/'STRUCTURED_BILINEAR_NATIVE_V1_BINDING.json').read_text())
    ...
    out=P/'STRUCTURED_BILINEAR_NATIVE_V1_RESULT.json';assert not out.exists()
    ...
    for seed in (0,937):
        ...
        expected=float(grad_norm*param_norm);eps=1e-4
        with torch.no_grad():
        ...
        finite=(plus-minus)/(2*eps);fd_error=abs(finite-expected)/max(abs(expected),1e-30)
        del original,direction
        preflight=dict(chunk_relative_error=chunk_error,parameter_fd_relative_error=fd_error,
                       parameter_fd_expected=expected,amplitude_solve=amplitudes)
        initial_cache=Path(f'/dev/shm/bilin18_structured_bilinear_v1_s{seed}_initial.pt');assert not initial_cache.exists()
        ...
        with (P/f'STRUCTURED_BILINEAR_NATIVE_V1_PREFLIGHT_{seed}.json').open('x') as f:json.dump(preflight,f,indent=2);f.write('\n')
        ...
        cache=Path(f'/dev/shm/bilin18_structured_bilinear_v1_s{seed}.pt');assert not cache.exists()
        ...
        with (P/f'STRUCTURED_BILINEAR_NATIVE_V1_SEED_{seed}.json').open('x') as f:json.dump(report,f,indent=2);f.write('\n')
