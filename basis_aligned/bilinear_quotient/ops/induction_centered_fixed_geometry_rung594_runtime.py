#!/usr/bin/env python3
"""Lazy model-backed call executor for R594.

Importing this file does not import torch or load a checkpoint.  Construction
of ``R594ModelExecutor`` is the only boundary that does so; owner tests use a
...
class R594ModelExecutor:
    ...
    def _intervene(self, tokens, specs, planned):
        ...
        layer_before = np.empty((len(specs),3,self.p.RESIDUAL), dtype="<f4")
        layer_after = np.empty_like(layer_before)
        layer_total = np.empty_like(layer_before)
        ...
        def attention(event):
            ...
            for local, spec in enumerate(specs):
                ...
                # Both layer-8 heads are applied in one transaction, observed as a layer total.
                total = planned_gpu[local, indices].sum(dim=0)
                modified[local, query] += total.to(dtype=modified.dtype)
                li = (5,7,8).index(event.site)
                layer_before[local,li] = before.detach().cpu().numpy()
                layer_after[local,li] = modified[local,query].float().detach().cpu().numpy()
                layer_total[local,li] = total.detach().cpu().numpy()
        ...
        return {
            "tokens.npy": np.ascontiguousarray(tokens, dtype="<i8"),
            "logits.npy": output,
            "planned_component_deltas.npy": np.ascontiguousarray(planned, dtype="<f4"),
            "layer_before.npy": layer_before, "layer_after.npy": layer_after, "layer_total.npy": layer_total,
            "layer_delta.npy": layer_after.astype(np.float64)-layer_before.astype(np.float64),
            "registered_component_deltas.npy": np.ascontiguousarray(planned, dtype="<f4"),
        }
