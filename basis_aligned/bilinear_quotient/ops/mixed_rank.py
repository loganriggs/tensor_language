"""MIXED-RANK ALLOCATION (rung 204): tail heads at r64, motif heads at r32.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The class table says the tail carries
the rank-hungry work (name/retrieval). Question: does {tail r64, motifs r32} - 36.0M pattern values, 26%
cheaper than uniform r64 - land nearer r64's 0.0852 than r32's 0.1492?

REGISTERED PREDICTIONS (anchors: r32 0.1492/2 certs; r64 0.0852/7 certs):
  (a) TAIL CARRIES THE NEED: census <= 0.11 (nearer r64 than the 0.117 midpoint).
  (b) CERTIFICATES MOSTLY KEPT: valid >= 5.
  (c) SANITY: census in [0.02, 0.16]; inert tripwire vs both uniform receipts.
NULL: rank need is uniform across blocks (census >= 0.117). PRICE: 36.0M pattern values. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: mixed-rank allocation')
    raise SystemExit(0)
import torch
...
OUT=PT+'mixed_rank_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=32; SEL['qk_rmap']={li:64 for li in range(10,18)}; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    pb=nv>=5
    pc=0.02<=agg<=0.16
    if abs(agg-0.0852)<1e-4 or abs(agg-0.1492)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: rank map inert')
    ...
    print(f"(a) census {agg:+.4f} <= 0.11: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 5; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) census in [0.02, 0.16]; era rho {rho:.4f}: {'HELD' if pc else 'FAILED'}")
