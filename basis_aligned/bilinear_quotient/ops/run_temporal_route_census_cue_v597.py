#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_all_values_at_the_cue_carry_tense pred_c_four_readers_carry pred_d_low_and_high_add pred_e_pairs_are_sign_constant
"""The TEMPORAL route census at the cue (v597): the number method on the will / had line. After 'tomorrow ...' the auxiliary tends to 'will', after
'earlier ...' to 'had'; the 17 Sep battery named {11.3, 9.1, 15.5, 9.4} on fresh rows. Here replace-edits on natural aligned pairs made by swapping the cue in
place (' tomorrow' <-> ' earlier', same adverb slot; v34's natural rows from both corpora, outcome-blind): every head's value at the cue, the four readers'
values, blocks 0-8, blocks 9-17 -- fraction of the tomorrow - earlier will - had gap closed. After v596 (since / by were not aligned pairs) the gap's sign
constancy is registered first.
PREDICTIONS (scored as written; failures preserved; priors from the number / gender / person censuses; temporal priors unsure)
    pred_a_baseline_replays                   the unedited manual forward replays the model's will - had margins within 1e-3
    pred_b_all_values_at_the_cue_carry_tense  every head's value at the cue swapped closes >= 0.80 of the gap
    pred_c_four_readers_carry                 {11.3, 9.1, 15.5, 9.4} values at the cue close >= 0.30. Prior: unsure
    pred_d_low_and_high_add                   blocks 0-8 + blocks 9-17 is within 0.10 of all heads. Prior: unsure
    pred_e_pairs_are_sign_constant            the tomorrow - earlier gap has the same sign on >= 0.75 of the pairs
PRICE (registered maximum): 4 batches x 5 passes = 20 forwards (<= 104 pairs); 0 backwards; 0 fits. Bar <= 21.
"""
...
OUT = ROOT / "circuits/followups/temporal_route_census_cue_v597_result.json"
CANDIDATE_ID = "temporal.route_census_cue_v597"
ROWS = ROOT / "circuits/followups/temporal_auxiliary_dod_natural_rows_v34.json"
N_HEAD = 9
FIVE = {11: [3], 9: [1, 4], 15: [5]}   # the four temporal readers (name kept)
REPLAY_TOL, ALL_MIN, FOUR_MIN, ADD, SIGN_MIN = 1e-3, 0.80, 0.30, 0.10, 0.75
FORWARDS_MAX = 21
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_all_values_at_the_cue_carry_tense": ">= 0.80", "pred_c_four_readers_carry": ">= 0.30", "pred_d_low_and_high_add": "|LOW + HIGH - ALL| <= 0.10", "pred_e_pairs_are_sign_constant": ">= 0.75"}


def temporal_pairs(rows_path=None):
    """Aligned pairs from v34's natural rows (both corpus panels): the cue swapped in place (' tomorrow' <-> ' earlier'); (tomorrow_row, earlier_row, cue_offset)."""
    E = L.ENCODING; partner = {" tomorrow": " earlier", " earlier": " tomorrow", "Tomorrow": "Earlier", "Earlier": "Tomorrow", " Tomorrow": " Earlier", " Earlier": " Tomorrow"}
    doc = json.loads((rows_path or ROWS).read_text()); rows = doc["rows"] if "rows" in doc else [r_ for p_ in doc["panels"].values() for r_ in p_["rows"]]
    items = []
    for r_ in rows:
        ...
        tom, ear = (r_["ids"], sw) if t.strip().lower() == "tomorrow" else (sw, r_["ids"])
        items.append((tom, ear, c))
    return items


gender_pairs = temporal_pairs   # name kept for the lineage's callers
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "text_pairs": len(text_items), "rows": str(ROWS.name), "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "all_min": ALL_MIN, "four_min": FOUR_MIN, "add": ADD, "sign_min": SIGN_MIN}}
    if os.environ.get("BQLIB_DRYRUN") or os.environ.get("BQLIB_NO_MODEL"):
        print(json.dumps(plan, indent=2, sort_keys=True)); return
    t0 = time.perf_counter()
    backend = producer.Bilin18TorchBackend.load("cuda"); torch, F, model = backend.torch, backend.F, backend.model; blocks = model.transformer.h
    from jacclust.tt_model import apply_rotary_emb  # noqa
    D = model.config.n_embd; hd = D // N_HEAD; HE, SHE = L._single(" will"), L._single(" had")   # temporal: the margin is will - had (names kept from the gender lineage)
    ...
    c = text["closed"]
    print("temporal census", {k: round(v, 3) for k, v in c.items()}, "gap", round(text["gap_mean"], 2), "sign", round(text["sign_constancy"], 3))
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_all_values_at_the_cue_carry_tense": c["ALL_N"] >= ALL_MIN, "pred_c_four_readers_carry": c["FIVE_N"] >= FOUR_MIN,
                   "pred_d_low_and_high_add": abs(c["LOW_N"] + c["HIGH_N"] - c["ALL_N"]) <= ADD, "pred_e_pairs_are_sign_constant": text["sign_constancy"] >= SIGN_MIN}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "temporal_route_census_cue_result_v597", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
