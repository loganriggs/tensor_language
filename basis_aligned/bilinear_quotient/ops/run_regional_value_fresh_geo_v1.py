#!/usr/bin/env python3
# BQGATE:9 body forwards;48 sequences lengths13-19;4 suffix arms;no fit;180sec.
"""pred_a producer/nativeattention/writecomposition relative<=1e-5.
pred_b eachfamily nativecuegap>.1,>=4/6positive;bothtransfer>.01,>=4/6positive.
pred_c eachfamily firsttransfer>=.1*nativecuegap,>=4/6positive,unrelated<=.5regional.
Null: frozen firstvalue circuit branch fails newcities/longertemplates. No fit.
"""
...
STEM='REGIONAL_VALUE_FRESH_GEO_V1'
@torch.no_grad()
def main():
    ...
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('9 body forwards48sequences6-14tokens;4suffix arms;no fitting');return
    ...
    for family in range(4):
        ...
        cells.append(dict(family=family,native_gap=float(native[:,0].mean()),native_positive=int((native[:,0]>0).sum()),
            mean_transfer_by_arm=transfer.mean(1).tolist(),positive_regional_by_arm=(transfer[:,:,0]>0).sum(1).tolist(),
            first_regional_meanabs=float(transfer[2,:,0].abs().mean()),first_unrelated_meanabs=float(transfer[2,:,1].abs().mean())))
    change=writes-writes[:,0,None];composition=float((change[:,3]-change[:,1]-change[:,2]).norm()/change[:,3].norm())
    instrument=max(producer_errors+replays+[composition])<=1e-5
    capability=all(c['native_gap']>.1 and c['native_positive']>=4 for c in cells)
    both=all(c['mean_transfer_by_arm'][3][0]>.01 and c['positive_regional_by_arm'][3]>=4 for c in cells)
    first=all(c['mean_transfer_by_arm'][2][0]>=.1*c['native_gap'] and c['positive_regional_by_arm'][2]>=4 and c['first_unrelated_meanabs']<=.5*c['first_regional_meanabs'] for c in cells)
    result={'pred_a':instrument,'pred_b':instrument and capability and both,'pred_c':instrument and capability and first}
    torch.save(dict(pre=pre.cpu(),write_vertices=writes.cpu(),arm_margins=arms.cpu()),art)
    result.update(cells=cells,native_capability=capability,composition_error=composition,producer_replay_errors=producer_errors,native_attention_replay_errors=replays,
                  seconds=time.perf_counter()-tic,body_forwards=count,artifact_sha=digest(art),source_shas=binding,
                  scope='Fresh cities/longer templates, frozen weight-derived source component and producer folds. Full native routing and background retained. First values computed from token identity; currentswap all positions. Not standalone extraction or repair of approximate-routing span failure.')
