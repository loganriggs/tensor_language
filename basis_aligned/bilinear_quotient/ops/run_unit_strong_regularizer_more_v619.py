...
"""v619: swap which control enters the objective. Is row-4 selectivity the FIT, or the control?
...
one way, 10 the other -- and the newer, more surprising half is the thin one. This repeats the v613 arrangement on
TEN FRESH CELLS, five that passed row 4 under the canonical fit and five that failed, none appearing in v603, v611,
...
Smoke: V619_SMOKE=<out.json> (CPU, V619_SMOKE_ROWS=4, V619_SMOKE_NAMES=<cell>).
"""
...
OUT = ROOT / "circuits/followups/unit_strong_regularizer_more_v619_result.json"
...
NAMES = {"determiner_number_ropes": "determiner_number_ropes",
         "imperative_mood": "imperative_mood",
         "number_anaphor_other_others": "number_anaphor_other_others",
         "numeral_dual_both_all": "numeral_dual_both_all",
         "one_anaphora_number": "one_anaphora_number",
         "coordination_or_nor": "coordination_or_nor",
         "definiteness_anaphor_needed": "definiteness_anaphor_needed",
         "definiteness_anaphor_wanted": "definiteness_anaphor_wanted",
         "deixis": "deixis",
...
V2_ROW4 = {
           "determiner_number_ropes": True,
           "imperative_mood": True,
           "number_anaphor_other_others": True,
           "numeral_dual_both_all": True,
           "one_anaphora_number": True,
           "coordination_or_nor": False,
           "definiteness_anaphor_needed": False,
           "definiteness_anaphor_wanted": False,
           "deixis": False,
           "existential_article_a_some": False,
           }
NEW = tuple(NAMES)
INSTR = ()
...
def _plan():
    return {"candidate_id": "corpus.unit_strong_regularizer_more_v619", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V619_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V619_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V619_SMOKE_NAMES", "verb_preposition_against_for,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_strong_regularizer_more_v619", "candidate_id": "corpus.unit_strong_regularizer_more_v619", "bars": BARS,
