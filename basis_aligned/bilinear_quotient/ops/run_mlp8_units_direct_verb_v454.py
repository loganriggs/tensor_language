#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_unit_closure pred_b_829_first_on_the_direct_path pred_c_953_or_1030_second pred_d_top_50_carry_half pred_e_top_20_overlap_with_pronoun_census
"""MLP 8 at unit grain on the DIRECT path to an adjacent verb (v454). v446: when the verb follows the noun directly, the answer position is the noun itself
and its own residual carries the agreement to the logits (values at the noun 0.26 of the margin only; MLP 8's three units 0.114 by replacement). Here the
exact per-unit split of MLP 8's write at the noun on the agreement direction u_v = W_U[are] - W_U[is] + W_U[were] - W_U[was], carried to the final residual
by the lambda chain of blocks 9-17 (T_j = lam * (u_v . Down[:, j]) h_j / rms), on the 43 adjacent natural rows (86 sequences, aligned pairs). Are the
verb-agreement units the same units that carry number to the pronoun readers (829 / 953 / 1030, v408)?
PREDICTIONS (scored as written; failures preserved; priors from v408 / v446)
    pred_a_unit_closure                     per-unit terms sum to MLP 8's term within relative 1e-3 on every row
    pred_b_829_first_on_the_direct_path     unit 829 has the largest |pooled contrast| on u_v. Prior: likely
    pred_c_953_or_1030_second               953 or 1030 is second
    pred_d_top_50_carry_half                the top 50 units carry >= 0.50 of MLP 8's pooled contrast on u_v
    pred_e_top_20_overlap_with_pronoun_census  the top-20 sets of this census and v408's share >= 12 units. Prior: unsure
...
OUT = ROOT / "circuits/followups/mlp8_units_direct_verb_v454_result.json"
CANDIDATE_ID = "chain.mlp8_units_direct_verb_v454"
N_HEAD, LAYER = 9, 8
CLOSURE_TOL, TOP50_MIN, OVERLAP_MIN = 1e-3, 0.50, 12
FORWARDS_MAX = 3
PREDICTIONS = {"pred_a_unit_closure": "<= 1e-3", "pred_b_829_first_on_the_direct_path": "rank 1", "pred_c_953_or_1030_second": "rank 2", "pred_d_top_50_carry_half": ">= 0.50", "pred_e_top_20_overlap_with_pronoun_census": ">= 12"}
...
def main() -> None:
    ...
    predictions = {"pred_a_unit_closure": closure <= CLOSURE_TOL, "pred_b_829_first_on_the_direct_path": rank[829] == 1, "pred_c_953_or_1030_second": 2 in (rank[953], rank[1030]), "pred_d_top_50_carry_half": share(50) >= TOP50_MIN, "pred_e_top_20_overlap_with_pronoun_census": overlap >= OVERLAP_MIN}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "mlp8_units_direct_verb_result_v454", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
