...
def main():
 ...
 torch.backends.cuda.matmul.allow_tf32=False;start=time.monotonic();out=P/'SENSITIVE_ROOT_FIT_V2.json';assert not out.exists()
 ...
 datasets=[];replay=0.;hashes={};epsilon_drifts=[]
 for name,file,n in [('calibration','fineweb_n96_skip1200.pt',96),('evaluation','fineweb_n192_skip7000.pt',32)]:
  ...
  for index in range(0,n,4):
   ...
   weights=sensitivity(final.double(),W.double(),den.double(),model.lm_head.weight.double(),eps=torch.finfo(final.dtype).eps)
   oldweights=sensitivity(final.double(),W.double(),den.double(),model.lm_head.weight.double());epsilon_drifts.append(float((weights-oldweights).norm()/weights.norm()))
 ...
 torch.save(dict(panels=[{k:v.cpu() for k,v in ds.items() if k!='x'} for ds in datasets],token_sha256=hashes,source_program_sha256=sha),P/'SENSITIVE_ROOT_CALIBRATION_V2.pt')
 ...
 result=dict(predictions=pred,results=results,prices=prices,compiler_diagnostics=diagnostics,capture_replay=replay,maximum_export_replay=max(drifts),program_sha256=exports,token_sha256=hashes,epsilon_weight_relative_drift=max(epsilon_drifts),writer_gram_offdiag=float((gram-torch.diag(gram.diag())).abs().max()),seconds=time.monotonic()-start)
