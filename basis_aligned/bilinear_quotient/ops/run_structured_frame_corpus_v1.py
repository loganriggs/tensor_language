...
@torch.no_grad()
def main():
    binding=json.loads((P/'STRUCTURED_FRAME_CORPUS_V1_BINDING.json').read_text())
    ...
    out=P/'STRUCTURED_FRAME_CORPUS_V1_RESULT.json';assert not out.exists()
    ...
    item=torch.load(P/'STRUCTURED_FRAME_REFIT_V1_PROGRAM.pt',weights_only=True)['2of4']
    _,_,q=unpack(item)
    candidates['learned2of4']=dict(native,key_basis=[native['key_basis'][0],q.cuda()])
