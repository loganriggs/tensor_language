...
def main():
 plan=json.loads((P/'JOINT_OVERLAP_PLAN_V1.json').read_text())
 ...
 assert not (P/'JOINT_OVERLAP_V1.json').exists()
 d=torch.load(P/'SHARED_PRODUCT_NATIVE_INPUTS_V1.pt',weights_only=True);templates_cpu=torch.load(P/'LOCAL_SHARED_READER_PROGRAMS_V1.pt',weights_only=True);Q=torch.stack([q for pair in d['pairs'] for q in pair['Qs']]);S=torch.linalg.inv(d['inverse_root']);I=torch.eye(1152,dtype=Q.dtype);records=[];programs={}
 ...
 torch.save(programs,P/'JOINT_OVERLAP_PROGRAMS_V1.pt');(P/'JOINT_OVERLAP_V1.json').write_text(json.dumps(dict(plan=plan,records=records,winners=winners,predictions=pred,seconds=time.perf_counter()-start),indent=2)+'\n');print(pred,flush=True)
