"""QK-TAIL (rung 183): replace the tail-attention DICTIONARIES with rank-8 real patterns.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The tail-attn family costs +0.37
increment even after trajectory refit, and S2239 showed its retrieval content is irreducibly contextual -
exactly what a REAL (if low-rank) pattern provides and no dictionary of the local stream can. Here blocks
10-17 get rank-8 QK patterns at ALL 72 heads (weights-only SVD, same construction as rung 181; motifs keep
their rank-8 patterns at 2-9; no aXL dicts fit - skipset covers the tail). Gating: tail hooks arm only
AFTER the real baselines are computed.

REGISTERED PREDICTIONS (anchors: rung-179 census +1.8950, increment +0.3724, name +3.021):
  (a) BEATS THE DICTIONARIES: census <= 1.78.
  (b) THE TAIL HEALS: increment <= 0.25 AND name <= 3.0.
  (c) SANITY: census >= 1.3; inert tripwire vs +1.8950.
NULL: retrieval needs sharp (high-rank) patterns - rank-8 is no better than the dicts (gain < 0.03) and the
dictionary grammar stands. PRICE: 72 x 4 x 8 x 1280 = 2,949,120 values, REPLACING the dictionaries' larger
CV/LW store - cheaper AND (if held) better. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: QK-tail rank-8')
    raise SystemExit(0)
import torch
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['qk_r']=8; SEL['qk_tail']=True
    print('ARM: rank-8 QK patterns at motif blocks 2-9 AND tail blocks 10-17 (dicts retired)',flush=True)
    main()
    ...
    pa=agg<=1.78
    pb=inc<=0.25 and PC.get('name',9.9)<=3.0
    pc=agg>=1.3
    ...
    print(f"(a) census {agg:+.4f} <= 1.78: {'HELD' if pa else 'FAILED'}")
    print(f"(b) increment {inc:+.4f} <= 0.25 and name {PC.get('name',9.9):+.4f} <= 3.0: {'HELD' if pb else 'FAILED'}")
