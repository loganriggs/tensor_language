#!/usr/bin/env python
"""late_attn_tail_read_probe -- which late-ATTENTION read needs the bus tail (dims 769-1152)? §2780-§2785 showed the MLP's tail use is a
core-gated linear read; late attention's tail use costs .0153 at 768 (§2772). Split by role: PATTERN reads (q, k, q2, k2; per-head normalised,
squared-bilinear) vs the VALUE read (c_v, bias-free, exactly linear in the tail), plus query-side vs key-side. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_value_read_carries_the_tail pred_c_pattern_read_is_cheap pred_d_sides_additive
#                     pred_e_same_ordering_at_896
...
Preregistration: polynomial_causal/LATE_ATTN_TAIL_READ_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_attn_tail_read_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_attn_tail_read_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_ATTN_TAIL_READ_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_tail_top2_direction_probe_results.json"   # §2786
OUT = ROOT / "late_attn_tail_read_probe_results.json"
HASHES = {PREREG: "5139b46546aceae498b61edd65b79b60e874e8a15bb5dc0f4415ea8470c5cf18", PRIOR: "a40141a961da596eb17c84c7c8656915896781bb74d51f0c34ee85b15fde81d7",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_attn_tail_read_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_min": 0.009, "c_max": 0.006, "d_frac": 0.25, "e_min": 0.004}
NULLS = {"b_max": 0.004, "c_min": 0.012, "d_lo": 0.5, "d_hi": 1.5, "e_ratio": 2.0}
...
def main():
    ...
    class SplitAttn(AttnHead):
        """Late attention block l with its READS split by role: the sides named in `sides` (subset of 'qkv') read the bus-core projection
        mx + U U^T (xh - mx); the other sides read the exact xh. sides='qkv' is exactly AttnHead(l, k, U_8)."""
        def __init__(self, l, k, sides):
            super().__init__(l, k, U_8); self.sides = sides
        def __call__(self, w, x):
            B, Tn = x.shape[:2]; xh = F.rms_norm(x, (D,)); a = self.a
    ...
    def late_attn(k, sides):
        return run({("attn", l): SplitAttn(l, k, sides) for l in range(SPLIT, NL)})
    arms = {"SPLIT8_1024": split8(1024), "LATE_ATTN_768": late_attn(768, "qkv"), "ATTN_PAT_768": late_attn(768, "qk"), "ATTN_VAL_768": late_attn(768, "v"),
            "ATTN_Q_768": late_attn(768, "q"), "ATTN_K_768": late_attn(768, "k"),
            "LATE_ATTN_896": late_attn(896, "qkv"), "ATTN_PAT_896": late_attn(896, "qk"), "ATTN_VAL_896": late_attn(896, "v")}
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    tot = arms["LATE_ATTN_768"]; sm = arms["ATTN_PAT_768"] + arms["ATTN_VAL_768"]
    summ = {"val_share_768": arms["ATTN_VAL_768"] / tot if tot else None, "pat_share_768": arms["ATTN_PAT_768"] / tot if tot else None,
            "sum_over_total_768": sm / tot if tot else None, "q_over_k_768": arms["ATTN_Q_768"] / arms["ATTN_K_768"] if arms["ATTN_K_768"] else None,
            "val_share_896": arms["ATTN_VAL_896"] / arms["LATE_ATTN_896"] if arms["LATE_ATTN_896"] else None}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(arms["LATE_ATTN_768"] - PRIOR_LATE_ATTN_768) <= BARS["repro_tol"] and abs(arms["LATE_ATTN_896"] - PRIOR_LATE_ATTN_896) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_value_read_carries_the_tail': bool(arms["ATTN_VAL_768"] >= BARS["b_min"]),
        'pred_c_pattern_read_is_cheap': bool(arms["ATTN_PAT_768"] <= BARS["c_max"]),
        'pred_d_sides_additive': bool(abs(sm - tot) <= BARS["d_frac"] * abs(tot)),
        'pred_e_same_ordering_at_896': bool(arms["ATTN_VAL_896"] >= arms["ATTN_PAT_896"] and arms["ATTN_VAL_896"] >= BARS["e_min"]),
    }
    nulls = {"b_null_val_768_le_.004": bool(arms["ATTN_VAL_768"] <= NULLS["b_max"]), "c_null_pat_768_ge_.012": bool(arms["ATTN_PAT_768"] >= NULLS["c_min"]),
             "d_null_sum_le_.5x_or_ge_1.5x": bool(tot != 0 and (sm <= NULLS["d_lo"] * tot or sm >= NULLS["d_hi"] * tot)),
             "e_null_pat_896_ge_2x_val_896": bool(arms["ATTN_PAT_896"] >= NULLS["e_ratio"] * arms["ATTN_VAL_896"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "late attention blocks 8-17 with reads split by role (pattern q/k vs value v; query vs key side) confined to the bus core top-k; everything else exact",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "late_attn_768": arms["LATE_ATTN_768"], "prior_late_attn_768": PRIOR_LATE_ATTN_768, "late_attn_896": arms["LATE_ATTN_896"], "prior_late_attn_896": PRIOR_LATE_ATTN_896},
