#!/usr/bin/env python
"""late_stack_write_routing_probe -- who reads the out-of-core part of the late writes (§2747)? Route the part of each late write
outside the shared read core either straight to the readout (invisible to later sublayers) or to later sublayers only (hidden from
the readout); plus the BUS program = reads on the 768 core + out-of-core writes routed to the readout. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_later_sublayers_do_not_need_it pred_c_readout_needs_it
#                     pred_d_bus_program_equals_read_program pred_e_narrower_bus
...
Preregistration: polynomial_causal/LATE_STACK_WRITE_ROUTING_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_stack_write_routing_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_stack_write_routing_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_STACK_WRITE_ROUTING_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_stack_write_core_probe_results.json"   # §2747
OUT = ROOT / "late_stack_write_routing_probe_results.json"
HASHES = {PREREG: "63212aa84bb6df22480cdd40653f781d6a258277effe067b15826c42a3d76067", PRIOR: "4bf02804d5b271fa636d1be512ddc43553880d14d9e60a5c6dc98ef4783d1028",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_stack_write_routing_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_LATE14_JOINT_768 = 0.109; PRIOR_DELETE_768 = 0.147; LAM = 1e-2
K = 768; KS = [512, 768]; LATE = list(range(11, 18))
SITES = [("mlp", l) for l in LATE]; ASITES = [("attn", l) for l in LATE]; FINAL = ("final", -1)
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_max": 0.05, "c_min": 0.08, "d_max": 0.02, "e_max": 0.10}
NULLS = {"b_min": 0.12, "c_max": 0.03, "d_min": 0.06, "e_min": 0.25}
...
def main():
    ...
    U_read = core_of(heads, "Cx")
    def outfrac(C, U_):
        return float(1.0 - (U_.double().T @ C @ U_.double()).trace() / C.trace())
    out_energy = {f"{s_[0]}{s_[1]}": {str(k): outfrac(wheads[s_]["Cw"], U_read[:, :k]) for k in KS} for s_ in SITES + ASITES}
    log(stage="cores", out_of_core_energy_768={s: round(v["768"], 3) for s, v in out_energy.items()})
    def run(patch, objs):
        def hook(idx):
            for o in objs:
                o.hook(idx)
        return ce_of(m, ev, patch, hook) - ce0
    reader = lambda s_: OwnHead(s_[1], K, "const", U_read[:, :K]) if s_[0] == "mlp" else AttnHead(s_[1], K, U_read)
    def routed(k, mode, with_reads=False):
        buf = Buf(); patch = {}
        for s_ in SITES + ASITES:
            r_ = Route(s_, k, U_read, mode, buf); patch[s_] = Both(reader(s_), r_) if with_reads else r_
        objs = [buf] + list(patch.values())
        if mode != "delete":
            patch[FINAL] = FinalAdd(buf, +1.0 if mode == "readout" else -1.0)
        return run(patch, objs)
    arms = {}
    rd = {s_: reader(s_) for s_ in SITES + ASITES}; arms["LATE14_JOINT_768"] = run(rd, list(rd.values()))
    for k in KS:
        arms[f"DELETE_{k}"] = routed(k, "delete")
    for k in KS:
        arms[f"TO_READOUT_{k}"] = routed(k, "readout")
    arms["HIDDEN_FROM_READOUT_768"] = routed(K, "hidden")
    arms["BUS_768"] = routed(K, "readout", with_reads=True)
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    gaps = {"bus_minus_joint": arms["BUS_768"] - arms["LATE14_JOINT_768"], "delete_minus_readout_plus_hidden_768": arms["DELETE_768"] - arms["TO_READOUT_768"] - arms["HIDDEN_FROM_READOUT_768"]}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["LATE14_JOINT_768"] - PRIOR_LATE14_JOINT_768) <= BARS["repro_tol"] and abs(arms["DELETE_768"] - PRIOR_DELETE_768) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_later_sublayers_do_not_need_it': bool(arms["TO_READOUT_768"] <= BARS["b_max"]),
        'pred_c_readout_needs_it': bool(arms["HIDDEN_FROM_READOUT_768"] >= BARS["c_min"]),
        'pred_d_bus_program_equals_read_program': bool(abs(gaps["bus_minus_joint"]) <= BARS["d_max"]),
        'pred_e_narrower_bus': bool(arms["TO_READOUT_512"] <= BARS["e_max"]),
    }
    nulls = {"b_null_ge_.12": bool(arms["TO_READOUT_768"] >= NULLS["b_min"]), "c_null_le_.03": bool(arms["HIDDEN_FROM_READOUT_768"] <= NULLS["c_max"]),
             "d_null_ge_.06": bool(abs(gaps["bus_minus_joint"]) >= NULLS["d_min"]), "e_null_ge_.25": bool(arms["TO_READOUT_512"] >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "k": K, "ks": KS,
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "late14_joint_768": arms["LATE14_JOINT_768"], "prior_late14_joint_768": PRIOR_LATE14_JOINT_768, "delete_768": arms["DELETE_768"], "prior_delete_768": PRIOR_DELETE_768},
           "ce_added": arms, "gaps": gaps, "out_of_core_energy": out_energy,
