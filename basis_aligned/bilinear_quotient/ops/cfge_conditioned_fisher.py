...
if os.environ.get('BQLIB_DRYRUN')=='1':
    if not os.path.exists(_bq0+'truefisher_top8_fresh_results.json'):
        print('DRYRUN FAIL: S2124 artifact absent'); raise SystemExit(1)
    _p0=json.load(open(_bq0+'truefisher_top8_fresh_results.json'))
    print(f"DRYRUN OK: S2124 median {_p0['gain_median']}; arms plain/top8/cond8 x eight windows")
    raise SystemExit(0)
if os.environ.get('BQLIB_DRYRUN')=='1':
...
OUT=PT+'cfge_conditioned_fisher_results.json'
...
def main():
    ...
    print(f'instrument: window-0 plain gap, inherited attnd labels {inh:+.4f} vs classified labels {prop:+.4f}',flush=True)
