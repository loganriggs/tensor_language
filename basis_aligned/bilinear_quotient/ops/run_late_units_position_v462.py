#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_late_units_at_the_noun_do_not_reach_a_distant_verb pred_c_late_units_at_the_verb_position_do pred_d_hub_units_at_the_noun_reach_it pred_e_hub_units_at_the_verb_position_too
"""Where the late units act: at the noun or at the answer position (v462). v457-v461: MLP 17's 701 / 2059 are a bilinear number detector that writes the
agreement margin when the answer position IS the noun. For a DISTANT verb (>= 2 tokens after the noun, 79 rows) the noun's write at block 17 can reach the
answer only through block 17's attention values; but the same units may fire at the answer position itself on the number copied there. Here the two-unit
swap on distant rows at the noun and at the answer position, and MLP 8's hub units at both positions for comparison.
PREDICTIONS (scored as written; failures preserved; priors from v446 / v460)
    pred_a_baseline_replays                       the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_late_units_at_the_noun_do_not_reach_a_distant_verb  701 / 2059 swapped at the noun close <= 0.02 of the distant-verb margin gap
    pred_c_late_units_at_the_verb_position_do     701 / 2059 swapped at the answer position close >= 0.04 (the detector fires where the prediction is made). Prior: unsure
    pred_d_hub_units_at_the_noun_reach_it         829 / 953 / 1030 at the noun close within 0.02 of 0.046 (v446)
    pred_e_hub_units_at_the_verb_position_too     829 / 953 / 1030 swapped at the answer position close >= 0.04 (MLP 8 rebuilds number at the answer position as at the post-noun token, v383-v385). Prior: unsure
...
OUT = ROOT / "circuits/followups/late_units_position_v462_result.json"
CANDIDATE_ID = "chain.late_units_position_v462"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, NOUN_MAX, ANSWER_MIN, HUB_V446, BAND = 1e-3, 0.02, 0.04, 0.046, 0.02
import random
...
FORWARDS_MAX = 16
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_late_units_at_the_noun_do_not_reach_a_distant_verb": "<= 0.02", "pred_c_late_units_at_the_verb_position_do": ">= 0.04", "pred_d_hub_units_at_the_noun_reach_it": "0.046 +- 0.02", "pred_e_hub_units_at_the_verb_position_too": ">= 0.04"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "noun_max": NOUN_MAX, "answer_min": ANSWER_MIN, "hub_v446": HUB_V446, "band": BAND}}
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_late_units_at_the_noun_do_not_reach_a_distant_verb": abs(v["LATE_N"]) <= NOUN_MAX, "pred_c_late_units_at_the_verb_position_do": v["LATE_A"] >= ANSWER_MIN,
                   "pred_d_hub_units_at_the_noun_reach_it": abs(v["HUB_N"] - HUB_V446) <= BAND, "pred_e_hub_units_at_the_verb_position_too": v["HUB_A"] >= ANSWER_MIN}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "late_units_position_result_v462", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
