...
"""Pythia-160m-DEDUPED (final, 300B tokens on the deduplicated Pile, ~1.5 epochs) — same size/architecture/token count as v760 (rank 32 rec 0.932, rank 16 0.876), different data. H3 (training tokens drive the descent) predicts recovery stays low: rank 32 < 0.95, rank 16 < 0.90; if data repetition/quality mattered instead, this would move. All-in-one, ROW-CENTRED logits (Logan, 22 Sep: 'try a few sets of pythia models, then other models, to test the specific hypothesis of why
...
PREDICTIONS (scored as written; failures preserved)
    ...
    pred_c_rank32_cost          rank-32 snapshot recovery < 0.95 (H3, the training-length curve, predicts it). Prior: likely
    pred_d_rank16_cost          rank-16 snapshot recovery < 0.90 (H3 predicts it). Prior: likely
...
REPLAY_TOL, E4_MIN, REC32, REC16, RATIO = 0.002, 0.90, 0.95, 0.9, (0.5, 2.0)
PREDICTIONS = {"pred_a_instrument_replays": "+-0.002", "pred_b_kernel_energy_top4": ">= 0.90", "pred_c_rank32_cost": "recovery < 0.95", "pred_d_rank16_cost": "recovery < 0.90", "pred_e_rank32_values_preserved_scale": "median ratio in [0.5, 2]"}
