...
"""Pronoun number they/he DoD (v168): MLP 8 at UNIT grain on the NUMBER line -- which units write the number state 9.6 reads at the noun (v103 / v82: the noun state is MLP-written, MLP 8 first)? v164's fold with the number line's rows, cue nouns (singular / plural agents) and 9.6's they-he reader direction. Registered question: are the number units different from the gender units 3152 / 3943 (reported: their rank here)?
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_mlp8_unit_census_v168_result.json"
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_mlp8_unit_census_v168"
LAYER = 8
...
def main() -> None:
    ...
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_mlp8_unit_census_result_v168", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "contrast_total": contrast,
