...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['merge_circuit_grain_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: battery present; census-row certification of the S2144 config")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_certificate_results.json'
...
if __name__=='__main__':
    ...
    print('ARM: S2144 config evaluated per-position on the census rows',flush=True)
    r_cfg=main()
    ...
    medfrac=stt.median([r['ind_other_frac'] for r in rows])
    fr_ret=sum(1 for r in fails if r['ind_other_frac']>medfrac)/max(len(fails),1)
    pa=nvalid>=40
    pb=fr_ret>=0.7
    pc=0.5<=agg<=5.0
    res={'aggregate_mean_dce':round(agg,4),'n_circuits':len(rows),'n_valid':nvalid,
         'fail_retrieval_fraction':round(fr_ret,3),'median_ind_other_frac':round(medfrac,3),
         'circuits':rows,
         'convention':'per-position dCE = CE(S2144 config) - CE(real model) on the census rows; tau = 0.5 x battery ref',
         'pred_a_broadly_valid':bool(pa),'pred_b_failures_retrieval':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'aggregate dCE {agg:+.4f}; valid {nvalid}/{len(rows)}; failing retrieval-frac {fr_ret:.2f}')
    print(f"(a) valid {nvalid} >= 40: {'HELD' if pa else 'FAILED'}")
    print(f"(b) failing retrieval fraction {fr_ret:.2f} >= 0.7: {'HELD' if pb else 'FAILED'}")
