"""QK RANK-16 (rung 182): does the pattern lever keep yielding?

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. Same build as rung 181 at rank 16;
the rung-181 receipt is read at RUN time (runner order). PRICE: 6,225,920 values.

REGISTERED PREDICTIONS:
  (a) STILL YIELDING: census(r16) <= census(r8) - 0.02.
  (b) MONOTONE: census(r16) <= census(r8) + 0.005.
  (c) SANITY: increment in [0.25, 0.50] AND census >= 1.3.
NULL (for a): saturation at rank 8 (the QK lever prices out early). Self-reviewed."""
...
OUT=PT+'motif_qkr16_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['qk_r']=16
    ...
    p181=PT+'motif_qkr_results.json'
    if not os.path.exists(p181): raise SystemExit('INSTRUMENT FAIL: rung-181 receipt missing at run time')
    A181=json.load(open(p181))['census_agg'] if 'census_agg' in json.load(open(p181)) else json.load(open(p181)).get('agg')
    if A181 is None: raise SystemExit('INSTRUMENT FAIL: rung-181 agg key missing')
    pa=agg<=A181-0.02
    pb=agg<=A181+0.005
    ...
    print(f"(a) census {agg:+.4f} <= r8-0.02 ({A181-0.02:+.4f}): {'HELD' if pa else 'FAILED'}")
    print(f"(b) census {agg:+.4f} <= r8+0.005 ({A181+0.005:+.4f}): {'HELD' if pb else 'FAILED'}")
