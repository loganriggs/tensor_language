...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_a16_linear_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; per-class CE attribution of a16L vs attn16-real on FR (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'attn16_class_damage_results.json'
...
if __name__=='__main__':
    ...
    for k in range(10):
        ...
        means[k]=float(diff[selk].mean()) if int(selk.sum())>0 else 0.0
    top=max(shares,key=lambda k:shares[k])
    pa=shares[top]>=0.40
    pb=top in LINK
    pc=abs(r_skip['L2_F']-2.5091)<=0.01
    res={'full':r_full,'skip16':r_skip,'total_damage_sum':round(tot,2),
         'class_shares':{CLS[k]:round(shares[k],4) for k in range(10)},
         'class_mean_damage':{CLS[k]:round(means[k],5) for k in range(10)},
         'top_class':CLS[top],'top_is_link':bool(pb),
         'convention':'per-position CE(full with a16L) - CE(attn16 real) on FR; positive = the dictionary hurts there',
         'pred_a_concentrated':bool(pa),'pred_b_top_is_link':bool(pb),'pred_c_reproduces_s2146':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print('class shares: '+' '.join(f'{CLS[k]}:{shares[k]:+.3f}' for k in range(10)))
    print(f"(a) top class {CLS[top]} share {shares[top]:+.3f} >= 0.40: {'HELD' if pa else 'FAILED'}")
    print(f"(b) top class in LINK: {'HELD' if pb else 'FAILED'}")
    print(f"(c) skip16 FR L2_F {r_skip['L2_F']} vs 2.5091 (tol 0.01): {'HELD' if pc else 'FAILED'}")
