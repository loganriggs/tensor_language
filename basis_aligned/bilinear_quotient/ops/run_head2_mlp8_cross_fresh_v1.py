...
pred_e>=2median/15nulls; pred_f<=.8eachbaseline.800forwards.
...
STEM='HEAD2_MLP8_CROSS_FRESH_V1';ARMS=['native','native8_midpoint','full_reentry','compiled']+[f'null:{k}' for k in range(16)]
...
def main():
 ...
 def pre9(module,args):
  ...
  if arm=='native':c['x9']=x.clone()
   ...
   row=groups[state['i']];seed=17092400+1000*int(arm.split(':')[1])+row['context_id'];generator=torch.Generator(device=x.device).manual_seed(seed)
 ...
 'pred_d':all(r['attenuation_positive_fraction']>=.75 and r['attenuation_mean']>=.02 for r in records.values()),'pred_e':all(r['target_over_null_median']>=2 and r['nulls_beaten']>=15 for r in records.values()),'pred_f':all(r['target_error']<=.8*r['cue_constant_error'] and r['target_error']<=.8*r['text_ols_error'] for r in records.values()),
 'families':records,'full_reentry_max_abs':anchor_abs,'full_reentry_relative':anchor_rel,'max_full_joint_input_error':max(joint_errors),'max_null_norm_error':max(norm_errors),'max_outside':max(outside),'body_forwards':count,'seconds':time.perf_counter()-start,'source_shas':binding,'scope':'Fresh reduced-cross confirmation with same-site block9 residual Gaussian nulls. Full normalization/background retained, native suffix recomputed. More native information than text baselines. No token-only/corpus/new-endpoint/donor-free/composition claim.'}
