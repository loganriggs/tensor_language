"""DECOMPOSE THE FRONTIER'S +2.6735 INTO THE ERROR EACH APPROXIMATED BLOCK CONTRIBUTES (Claude, LANE 1)

SS2879 forced a reset: SS2874/SS2875/SS2876 manipulated `fit_attnd` dictionaries that are **not in the evaluated configuration**, and
their zero costs were vacuous. The standing rule adopted there is to verify the manipulated entry is installed and to say so. This rung
manipulates only things that provably are: `ML`, the motif-head layer list passed straight into `evalM`, and membership of the
`a10L`-`a17L` entries in `order2` itself.

It also asks a different KIND of question. The blocks here are APPROXIMATIONS standing in for real components: `motif_hooks` replaces
attention 2-9's heads with scaled value copies, and the tail dictionaries replace attention 10-17. Turning one OFF therefore restores
the REAL component and should LOWER L2 -- so the quantity measured is not a deletion cost but an **error share**:

    error_share(block) = L2_F(baseline) - L2_F(block restored to real),   POSITIVE = that block contributes that much of the frontier's error

Four arms give the two shares and their joint, which is the first decomposition of the published +2.6735 into where its error actually
comes from -- and the two largest standing gaps in the explained fraction (tail dictionaries / coverage credit, and the price cliff at
attention 5, which sits inside the motif band) are both on that list.
...
#                     pred_b_the_motif_heads_carry_a_large_share_of_the_error
#                     pred_c_the_tail_dictionaries_carry_a_large_share
#                     pred_d_both_blocks_are_installed_and_move_the_number
#                     pred_e_the_two_shares_are_roughly_additive

Preregistration: polynomial_causal/FRONTIER_ERROR_DECOMPOSITION_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; four arms (baseline / motif heads off / tail dicts off / both off)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_error_decomposition_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
TAILMODE={'mode':None}
EVALMODE={'motif':None,'drop_tail':False}   # motif: None=all 8 layers, []=off; drop_tail: omit a10L-a17L
CVSNAP={}
SKIP8=64
...
def main():
    ...
    _ml = ML if EVALMODE['motif'] is None else EVALMODE['motif']
    _o2 = [x for x in order2 if not (EVALMODE['drop_tail'] and x.endswith('L'))]
...
PREREG = PT + '../polynomial_causal/FRONTIER_ERROR_DECOMPOSITION_PREREGISTRATION.md'
PREREG_SHA = "1718660c607a8a021711fb14f0923cf863e054b55d3467d9e04db17099a27a6d"
...
BARS = {"reproduce": 0.05, "motif_share": 0.30, "tail_share": 0.20, "installed": 0.01, "additive": 0.30}
NULLS = {"motif_share_le": 0.05, "tail_share_le": 0.05}
...
if __name__=='__main__':
    ...
    print('ARM 1/4: BASELINE -- the published frontier',flush=True)
    EVALMODE['motif']=None; EVALMODE['drop_tail']=False
    r_base=main()

    print('ARM 2/4: motif heads OFF -- attention 2-9 runs REAL instead of approximated',flush=True)
    EVALMODE['motif']=[]; EVALMODE['drop_tail']=False
    r_nomotif=main()

    print('ARM 3/4: tail dictionaries OFF -- attention 10-17 runs REAL instead of approximated',flush=True)
    EVALMODE['motif']=None; EVALMODE['drop_tail']=True
    r_notail=main()

    print('ARM 4/4: BOTH off -- attention 2-17 runs REAL',flush=True)
    EVALMODE['motif']=[]; EVALMODE['drop_tail']=True
    r_both=main()
    EVALMODE['motif']=None; EVALMODE['drop_tail']=False

    # SIGN: L2 is CE ADDED ABOVE THE REAL MODEL, LOWER IS BETTER. Replacing an APPROXIMATION by the REAL
    # component should LOWER L2, so a block's ERROR SHARE is baseline MINUS the restored arm: POSITIVE =
    # that block was contributing that much of the frontier's error.
    share_motif = r_base['L2_F'] - r_nomotif['L2_F']
    share_tail  = r_base['L2_F'] - r_notail['L2_F']
    share_both  = r_base['L2_F'] - r_both['L2_F']
    drift       = abs((share_motif + share_tail) - share_both)

    pa = abs(r_base['L2_F']-2.6735) <= BARS['reproduce']
    pb = share_motif >= BARS['motif_share']
    pc = share_tail  >= BARS['tail_share']
    pd = (abs(share_motif) >= BARS['installed']) and (abs(share_tail) >= BARS['installed'])
    pe = drift <= BARS['additive']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_motif_heads_carry_a_large_share_of_the_error':bool(pb),
           'pred_c_the_tail_dictionaries_carry_a_large_share':bool(pc),
           'pred_d_both_blocks_are_installed_and_move_the_number':bool(pd),
           'pred_e_the_two_shares_are_roughly_additive':bool(pe)}
    nulls={'b_null_the_motif_heads_are_error_free':bool(share_motif<=NULLS['motif_share_le']),
           'c_null_the_tail_dictionaries_are_error_free':bool(share_tail<=NULLS['tail_share_le'])}
    res={'rung':'frontier_error_decomposition','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,
         'baseline':r_base,'motif_off':r_nomotif,'tail_off':r_notail,'both_off':r_both,
         'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(r_base['L2_F'],4),
                    'L2_F_motif_off':round(r_nomotif['L2_F'],4),
                    'L2_F_tail_off':round(r_notail['L2_F'],4),
                    'L2_F_both_off':round(r_both['L2_F'],4),
                    'error_share_motif_heads_nats':round(share_motif,4),
                    'error_share_tail_dicts_nats':round(share_tail,4),
                    'error_share_both_nats':round(share_both,4),
                    'additivity_drift_nats':round(drift,4),
                    'L1_F_baseline':r_base.get('L1_F'),'L1_F_motif_off':r_nomotif.get('L1_F'),
                    'L2_C_baseline':round(r_base['L2_C'],4),'L2_C_motif_off':round(r_nomotif['L2_C'],4),
                    'residual_L2_F_after_both':round(r_both['L2_F'],4)},
         'price':{'gpu_forwards':0,'forwards_instrumented':False,'pipeline_runs':4,
    ...
    print(f"(b) motif-head error share {share_motif:+.4f} >= {BARS['motif_share']}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) tail-dict error share {share_tail:+.4f} >= {BARS['tail_share']}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) both blocks move the number: {'HELD' if pd else 'FAILED'}")
    print(f"(e) additivity drift {drift:.4f} <= {BARS['additive']}: {'HELD' if pe else 'FAILED'}")
