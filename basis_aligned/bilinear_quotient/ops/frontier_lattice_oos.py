"""OUT-OF-SAMPLE LATTICE EXTENSION (rung 120): does the composition calculus cross element TYPES?

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows; LOWER IS BETTER.
S2212/S2213: the MLP-CP interaction lattice is measured (sum J = +0.4419, K3 concentrated in 123/012). The
6-element front (+0.9427, S2206) adds a0 (exact table) and a1v (value table, single +0.0520). Here the four
cross-type pairs {a1v, cf_i} and one control pair {a0, cf3} are measured, and the ORDER-2 calculus predicts
the full 6-front OUT OF SAMPLE: pred2 = [sum cf singles +0.2350] + [a1v +0.0520] + [a0 = 0] + [sum J_cf
+0.4419] + [sum J_a1v,cf measured here] + [J_a0,* = 0 by the exactness hypothesis].

REGISTERED PREDICTIONS (arms named):
  (a) CROSS-TYPE ORDER-2 PREDICTS: |pred2(front6) - 0.9427| <= 0.10.
  (b) EXACTNESS EXTENDS TO INTERACTIONS: |J(a0,cf3)| = |agg(a0_cf3) - single_m3| <= 0.01.
  (c) SANITY: each a1v-pair agg >= max(+0.0520, its cf single) - 0.015 and <= 0.9427.
NULL: cross-type interactions are large/order-2 breaks across types - the calculus is MLP-only.
PRICE: none (attribution). Tripwire: INSTRUMENT FAIL if the four a1v-pair arms are identical (pstdev < 0.003).
Self-reviewed."""
import json, sys, time, os
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
if os.environ.get('BQLIB_DRYRUN')=='1':
    _bq='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
    _need=['frontier_cp_pairs_results.json','frontier_front_singles_results.json',
           'frontier_cp_front_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: out-of-sample lattice extension')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_lattice_oos_results.json'
...
def main():
    ...
    if SEL.get('lat_oos'):
        ...
        _arms={}
        for _li in (0,1,2,3):
            _arms[f'a1v_cf{_li}']=evalV(_ROWS,_ROWS.shape[0],['a1v',f'cf{_li}'],ML).detach().cpu()
            print(f'  pair (a1v, cf{_li}) done',flush=True)
        _arms['a0_cf3']=evalV(_ROWS,_ROWS.shape[0],['a0','cf3'],ML).detach().cpu()
        print('  pair (a0, cf3) done',flush=True)
        del cur['clsmap']
        SEL['front_result']=_arms
...
if __name__=='__main__':
    ...
    NFLAT=CN.nflat()
    R118=json.load(open(PT+'frontier_cp_pairs_results.json'))
    SGL={int(k):v for k,v in R118['singles'].items()}
    SUMJ=R118['sum_J']
    SA1V=json.load(open(PT+'frontier_front_singles_results.json'))['arms']['single_a1v']['agg']
    FR6=json.load(open(PT+'frontier_cp_front_results.json'))['agg']
    SEL['mode']='norm'; SEL['K']=4608; SEL['K69']=4608; SEL['K69MAP']={}
    SEL['skipset']=tuple(range(10,18)); SEL['motif_off']=tuple(range(2,10))
    SEL['lat_oos']=True; SEL['ext_rows']=CROWS
    print('ARMS: four a1v-cross pairs + a0_cf3 control - census rows',flush=True)
    main()
    A=SEL['front_result']
    agg={}
    for k,cev in A.items():
        ...
        print(f"  {k:8s} agg {agg[k]:+.4f}",flush=True)
    import statistics as stt
    a1p=[agg[f'a1v_cf{i}'] for i in (0,1,2,3)]
    if stt.pstdev(a1p)<0.003: raise SystemExit('INSTRUMENT FAIL: a1v-pair arms identical')
    Jx={i:round(agg[f'a1v_cf{i}']-SA1V-SGL[i],4) for i in (0,1,2,3)}
    Ja0=round(agg['a0_cf3']-SGL[3],4)
    pred2=round(sum(SGL.values())+SA1V+SUMJ+sum(Jx.values()),4)
    for i in (0,1,2,3): print(f'  J(a1v,cf{i}) = {Jx[i]:+.4f}')
    print(f'  J(a0,cf3) = {Ja0:+.4f}; pred2(front6) {pred2:+.4f} vs measured {FR6:+.4f}')
    pa=abs(pred2-FR6)<=0.10
    pb=abs(Ja0)<=0.01
    pc=all(agg[f'a1v_cf{i}']>=max(SA1V,SGL[i])-0.015 and agg[f'a1v_cf{i}']<=FR6 for i in (0,1,2,3))
    res={'aggs':agg,'J_a1v':Jx,'J_a0_cf3':Ja0,'pred2_front6':pred2,'front6_measured':FR6,
         'convention':'per-position dCE = CE(config) - CE(real model) on census rows; lower is better',
         'pred_a_crosstype_order2':bool(pa),'pred_b_exactness_extends':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) |pred2 - front6| = {abs(pred2-FR6):.4f} <= 0.10: {'HELD' if pa else 'FAILED'}")
    print(f"(b) |J(a0,cf3)| {abs(Ja0):.4f} <= 0.01: {'HELD' if pb else 'FAILED'}")
    print(f"(c) pair sanity: {'HELD' if pc else 'FAILED'}")
