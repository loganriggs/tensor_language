"""PAIR-GRAIN CERTIFICATES (rung 121): is certificate validity pairwise-fragile?

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows; LOWER IS BETTER.
Singles retain 8-12 of 62 valid circuits (m0@2304: 9, m1@2304: 12, m2@1152: 11, m3@576: 8) but the quad has 0
and every multi-site config ever built has <= 2. Where does validity die - already at pairs? The six pairs
rerun with per-circuit certificates (tau = 0.5 x battery ref; the aggs double as a rung-118 reproduction).

REGISTERED PREDICTIONS (arms named valid(pair_ij); singles' valids from the rung 109-111 receipts):
  (a) PAIRWISE FRAGILITY: median over the six pairs of valid(pair_ij) <= 0.5 x min(valid_i, valid_j).
  (b) RETENTION SOMEWHERE: max pair valid >= 6.
  (c) REPRODUCTION: every |agg(pair_ij) - rung-118 agg| <= 0.015 (CUDA-wobble tol).
NULL: (a) validity survives pairing (certificates die only at >= 3 sites) or (b) total collapse (max <= 2).
PRICE: none (attribution). Tripwire: INSTRUMENT FAIL if pair arms identical (pstdev < 0.005). Self-reviewed."""
import json, sys, time, os
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
if os.environ.get('BQLIB_DRYRUN')=='1':
    _bq='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
    _need=['frontier_cp_pairs_results.json','frontier_m0_cp_results.json','frontier_m2_cp_results.json',
           'frontier_m13_cp_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: pair-grain certificates')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_pair_certs_results.json'
...
if __name__=='__main__':
    ...
    R118=json.load(open(PT+'frontier_cp_pairs_results.json'))['aggs']
    SV={0:json.load(open(PT+'frontier_m0_cp_results.json'))['arms']['K_2304']['valid'],
        1:json.load(open(PT+'frontier_m13_cp_results.json'))['arms']['m1_K2304']['valid'],
        2:json.load(open(PT+'frontier_m2_cp_results.json'))['arms']['K_1152']['valid'],
        3:json.load(open(PT+'frontier_m13_cp_results.json'))['arms']['m3_K576']['valid']}
    SEL['mode']='norm'; SEL['K']=4608; SEL['K69']=4608; SEL['K69MAP']={}
    SEL['skipset']=tuple(range(10,18)); SEL['motif_off']=tuple(range(2,10))
    SEL['pair_certs']=True; SEL['ext_rows']=CROWS
    print('ARMS: six CP pairs with certificates - census rows',flush=True)
    main()
    A=SEL['front_result']
    out={}
    ...
    if stt.pstdev(v['agg'] for v in out.values())<0.005: raise SystemExit('INSTRUMENT FAIL: arms identical')
    fr=[]; repro=True
    for i,j in it.combinations((0,1,2,3),2):
        k=f'pair_{i}{j}'
        fr.append(out[k]['valid']<=0.5*min(SV[i],SV[j]))
        repro&=abs(out[k]['agg']-R118[k])<=0.015
    med=stt.median([out[f'pair_{i}{j}']['valid'] for i,j in it.combinations((0,1,2,3),2)])
    pa=sum(fr)>=3 and med<=0.5*min(SV.values())
    mx=max(v['valid'] for v in out.values())
    pb=mx>=6
    pc=repro
    res={'arms':out,'single_valids':SV,'median_pair_valid':med,'max_pair_valid':mx,
         'convention':'per-position dCE = CE(config) - CE(real model) on census rows; tau = 0.5 x battery ref',
         'pred_a_pairwise_fragile':bool(pa),'pred_b_retention':bool(pb),'pred_c_repro':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'median pair valid {med}; max {mx}; single valids {SV}')
    print(f"(a) pairwise fragility (median {med} <= 0.5 x min single {0.5*min(SV.values()):.1f}): {'HELD' if pa else 'FAILED'}")
    print(f"(b) max pair valid {mx} >= 6: {'HELD' if pb else 'FAILED'}")
    print(f"(c) all pair aggs reproduce rung 118 within 0.015: {'HELD' if pc else 'FAILED'}")
