# IS THE RECOMMENDATION BUDGET-DEPENDENT?  -- the 65.950M iso-cost line, coverage 16110, rank 97.
#
# §1861-§1863 measured the 230.087M iso-cost line at five coverages and concluded: spend the budget on
# RANK, at any coverage above ~12,000 types. Every one of those figures is at that SINGLE budget.
#
# §1853's grid is the reason to doubt it generalises. There, coverage HELPS at rank >= 16 and HURTS at
# rank 4 -- the coverage term changes sign at low rank. A smaller budget puts the whole iso-cost line at
# lower rank, so the recommendation could invert.
#
# This is the 65.950M line -- the cost of §1849's PUBLISHED rank-256 build at 5,419 types, all-position
# CE 6.06004 / 6.03311 / 6.04238. At that budget the affordable ranks are 256 at 5,419 types, 164 at
# 9,054, 128 at 11,954 and 97 at 16110. This measures the 16110-type point.
#
# Docstring and predicate block written in ONE edit (LESSONS 63); costs COMPUTED from §1754's model
# (§1858); a single finite rank in TRANKS (LESSONS 64).
#
# ROLES. All three. DISCOVERY ONLY. Rung 3: the open question §1863 ends on.
#
...
#   pred_a AT LOW BUDGET, COVERAGE STILL HELPS: this point's all-position CE is BELOW §1849's PUBLISHED
#          rank-256@5,419 figures of 6.06004 / 6.03311 / 6.04238 on all three roles. If TRUE the
#          §1862/§1863 recommendation survives a 3.5x smaller budget and can be stated without one. If
#          FALSE coverage HURTS at this budget, the recommendation is budget-dependent, and §1863's rule
#          must be qualified to the budget at which it was measured -- the outcome §1853's sign change
#          predicts and the one that would matter.
#   pred_b AND THE EFFECT IS SMALLER THAN AT 230M: whatever its sign, the gap to §1849's rank-256@5,419
#          is under 0.042 nats on skip7000 -- the gain §1862 measured at the same coverage step on the
#          large budget. If FALSE the coverage effect is at least as strong at low budget as at high,
#          which would make the two budgets differ in magnitude as well as possibly in sign.
#   pred_c AND THE BUILD IS SOUND AT THIS RANK: the covered CE lies above its own ceiling by more than
#          §1852's PUBLISHED rank-256@16,110 figure of +0.09252 on skip7000, since rank 97 is far
#          below 256 at the same coverage. If FALSE a much lower rank is not much worse, and the rank
#          ladder is flatter at this coverage than §1854 measured at 16,110.
#   pred_d CONTROLS: the cost computed from §1754's model is within 1% of 65.950M; coverage is exactly
#          16110 from fit rows [0:480]; the ceiling is finite and above live on every role.
...
OUT = PT + 'ops/low_budget_cov_16110_results.json'
...
TRANKS = (97,)          # None = full rank; §1786/§1813 use this ladder
...
def main():
    ...
    print(f'LOW BUDGET COV | 16110 types, rank 97, 65.950M line | '
    ...
    vs_ref = {e: S1849_R256_5419[e] - allpos[e] for e in roles2}
    pa = all(v > 0 for v in vs_ref.values())
    pb = abs(vs_ref['skip7000']) < 0.042
    pc = above['skip7000'] > S1852_R256_ABOVE_7000
    pd = (abs(cost - LOWBUDGET_M) / LOWBUDGET_M < 0.01 and NFULL == 16110
          and all(ceilv[e] > livev[e] for e in roles2))

    print(f'\\n  LOW-BUDGET point: {NFULL} covered types, rank {R}, {cost:.3f}M '
          f'(line budget {LOWBUDGET_M}M)', flush=True)
    for e in roles2:
        print(f'    {e:10s} ceiling {ceilv[e]:.5f}  covered {curve[R][e]["cov"]:.5f} '
              f'(+{above[e]:.5f})  ALLPOS {allpos[e]:.5f}  vs §1849 r256@5419 '
              f'{S1849_R256_5419[e]:.5f}', flush=True)
    print(f'\\n  at LOW budget coverage still HELPS -> {pa}  ' + '  '.join(
        f'{e} {vs_ref[e]:+.5f}' for e in roles2), flush=True)
    print(f'  and the effect is SMALLER than the 230M line\'s +0.042 -> {pb}  '
          f'skip7000 {vs_ref["skip7000"]:+.5f}', flush=True)
    print(f'  and rank {R} is well above the ceiling (> §1852 +{S1852_R256_ABOVE_7000}) -> {pc}  '
          f'+{above["skip7000"]:.5f}', flush=True)
    print(f'  cost {cost:.3f}M ~ {LOWBUDGET_M}M, coverage {NFULL} -> control {pd}', flush=True)

    json.dump({'run': 'low_budget_cov_16110', 'n_covered': NFULL, 'rank': R,
               'cost_M': cost, 'allpos': allpos, 'above_ceiling': above, 'vs_ref': vs_ref,
               'ceiling': ceilv, 'live': livev,
               'predictions': {'pred_a_coverage_still_helps': bool(pa),
                               'pred_b_effect_smaller': bool(pb),
                               'pred_c_rank_well_above_ceiling': bool(pc),
