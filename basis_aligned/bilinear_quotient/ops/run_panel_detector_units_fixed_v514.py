#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_detector_units_oppose_the_panel_pronoun_margin pred_c_random_units_do_not pred_d_detector_is_a_share_of_mlp17 pred_e_hub_units_at_the_noun_replay
...
OUT = ROOT / "circuits/followups/panel_detector_units_fixed_v514_result.json"
CANDIDATE_ID = "chain.panel_detector_units_fixed_v514"
N_HEAD = 9
...
import random
import random
UNIT_SETS = {"DET_A": (17, (701, 2059), "answer"), "RANDOM_A": (17, tuple(random.Random(0).sample([j for j in range(4608) if j not in (701, 2059)], 2)), "answer"), "HUB_N": (8, (829, 953, 1030), "noun")}
...
FORWARDS_MAX = 13
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_detector_units_oppose_the_panel_pronoun_margin": "<= -0.03", "pred_c_random_units_do_not": "|.| <= 0.01", "pred_d_detector_is_a_share_of_mlp17": "0.20-0.90 x 0.115", "pred_e_hub_units_at_the_noun_replay": "0.091 +- 0.02"}
...
def main() -> None:
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_detector_units_oppose_the_panel_pronoun_margin": v["DET_A"] <= DET_MAX, "pred_c_random_units_do_not": abs(v["RANDOM_A"]) <= RANDOM_MAX,
                   "pred_d_detector_is_a_share_of_mlp17": SHARE_LO * M17_V512 <= abs(v["DET_A"]) <= SHARE_HI * M17_V512, "pred_e_hub_units_at_the_noun_replay": abs(v["HUB_N"] - HUB_V423) <= BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "panel_detector_units_fixed_result_v514", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
