"""METRIC UNIT SELECTION: SECOND WINDOW + c6-c9 EXTENSION (rung 13).
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['stream_error_profile_results.json','metric_front_refit_split_results.json']
    ...
    print(f"DRYRUN OK: S2086 present (block 6 {_p['6']}); three arms x two windows (plain / metric-units / metric-units-all)")
    raise SystemExit(0)
import json, sys, time, torch
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
import os
...
OUT=PT+'metric_units_certify_results.json'
...
def main():
    ...
    res={}
    for mode in ('plain','metric-units','metric-units-all'):
        ts=time.time()
        cfg=build_arm(mode)
        ...
        print(f'{mode:17s}: gap w1 {ce1-base1:+.4f} w2 {ce2-base2:+.4f} | b5 {prof[5]:.3f} b6 {prof[6]:.3f} b7 {prof[7]:.3f} '
              f'b9 {prof[9]:.3f} b17 {prof[17]:.3f} | {time.time()-ts:.0f}s',flush=True)
        del st; torch.cuda.empty_cache()
    E=res['plain']
    g1=lambda k:E['gap_w1']-res[k]['gap_w1']; g2=lambda k:E['gap_w2']-res[k]['gap_w2']
    pa=g2('metric-units')>=0.062
    pb=g1('metric-units-all')>=g1('metric-units')+0.03
    pc=abs(g1('metric-units')-0.1241)<=0.02
    pd=abs(E['profile'][6]-1.7415)<=0.10
    out={'base_ce_w1':round(base1,5),'base_ce_w2':round(base2,5),'window2_rows':'FW[0:120]','arms':res,
         'gains_w1':{k:round(g1(k),4) for k in res if k!='plain'},'gains_w2':{k:round(g2(k),4) for k in res if k!='plain'},
         'S2105_units_gain_w1':0.1241,'pred_a_transfers_to_window2':bool(pa),'pred_b_c6_c9_add_more':bool(pb),
         'pred_c_reproduces_S2105':bool(pc),'pred_d_reproduces_S2086':bool(pd),'self_reviewed':True,
         'runtime_s':round(time.time()-t0,1)}
    json.dump(out,open(OUT,'w'),indent=1)
    print(f"(d) cfgE block-6 rel-MSE {E['profile'][6]:.4f} vs 1.7415: {'HELD' if pd else 'FAILED'}")
    print(f"(a) window-2 gain(metric-units) {g2('metric-units'):+.4f} >= 0.062: {'HELD' if pa else 'FAILED'}   (units-all w2 {g2('metric-units-all'):+.4f})")
    print(f"(b) window-1 units-all {g1('metric-units-all'):+.4f} >= units {g1('metric-units'):+.4f} + 0.03: {'HELD' if pb else 'FAILED'}")
    print(f"(c) window-1 units {g1('metric-units'):+.4f} vs S2105 0.1241 (tol 0.02): {'HELD' if pc else 'FAILED'}")
