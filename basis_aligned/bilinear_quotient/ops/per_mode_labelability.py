"""PER-MODE LABELABILITY -- BACKLOG rung 7's prerequisite, never actually measured.

Rung 7 (mode-conditioned stand-in selection) says: "Requires a deployable
mode-labeler (probe on the stream -> mode score) -- fit and validate that first."

S347 looks like it did this and did not. probe_gate2 regressed the 10 oracle mode
scores, then built an ANY-MODE gate from them and reported that gate's AUC
(0.551). It never reported a per-mode AUC. Its own diagnosis says the any-mode
construction "amplifies per-mode noise", so the number it published is about the
derived gate, not about whether any individual mode is labelable. Rung 7 needs
per-mode labels; the any-mode gate is not what it would deploy.

Two things also make S347's attempt unrepresentative of what is possible now.
It used QUADRATIC features (1680 dims) which S347 itself diagnosed as overfitting
the fit window, and it read at BLOCK 2 -- and S2079 subsequently found deeper
reads beat block 2 (AUC 0.6545 at block 9 against 0.621 at block 2) where richer
features had failed.

So: LINEAR per-mode probes, at block 2 (S342's site, as the control) and block 9
(S2079's best single site), each mode scored on its own held-out AUC.

REGISTERED PREDICTIONS:
  (a) SOME MODE IS LABELABLE: at block 9, at least 3 of the 10 modes reach
      held-out AUC >= 0.70. This is the number rung 7 actually needs and nobody
      has reported it. If FALSE, no individual damage mode is linearly readable
      from the stream at either depth, and rung 7's prerequisite fails -- the
      rung should be marked blocked on a labeler that does not exist rather than
      left looking merely unstarted.
  (b) DEPTH HELPS HERE TOO: block 9's mean per-mode AUC exceeds block 2's. S2079
      established this for the binary gate label; whether it carries to per-mode
      labels is a separate question, since the modes are finer-grained and the
      deeper stream has had more of the assembly's substitutions applied to it.
  (c) CONTROL: with mode labels SHUFFLED, mean per-mode AUC is < 0.55 at both
      depths. Ten separate ridges on this much data have ample capacity to fit
      noise, and (a) is a "best of ten" statistic, which is exactly the shape
      that needs a null.

Writes per_mode_labelability_results.json. No gate is built and no CE is claimed.
"""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    if not os.path.exists(_bq+'probe_gate3_results.json'):
        print('DRYRUN FAIL: S2079 arms absent'); raise SystemExit(1)
    _v3=json.load(open(_bq+'probe_gate3_results.json'))
    print(f"DRYRUN OK: S2079 present (blk2 AUC {_v3['arms']['blk2']['auc']}, "
          f"blk9 {_v3['arms']['blk9']['auc']}); per-mode linear probes at "
          f"depths 2 and 9, 10 modes, held out")
    raise SystemExit(0)
...
OUT=PT+'per_mode_labelability_results.json'
...
def main():
    ...
    # ---- per-mode LINEAR probes at two depths (rung 7 prerequisite) ----
    import random as _rnd
    DEPTHS=[('blk2',[2]),('blk9',[9])]
    ...
    NM=scA.shape[1]
    YA=(scA.abs()>=thr[None,:]).float().to(DEV)        # per-mode fit labels
    YF=(scF.abs()>=thr[None,:]).float()                # per-mode fresh labels
    def auc_of(score,lab):
        o=score.argsort()
        rk=torch.empty(len(score)); rk[o]=torch.arange(len(score)).float()
        pos=lab.bool(); npos=int(pos.sum()); nneg=len(lab)-npos
        if npos==0 or nneg==0: return float('nan')
        return float((rk[pos].sum()-npos*(npos-1)/2)/(npos*nneg))
    RES={}
    for nm,depths in DEPTHS:
        XA=stream_rows(rowsA,nA,depths); dim=XA.shape[1]
        lam=1e-2*len(XA)
        G=XA.T@XA+lam*torch.eye(dim,device=DEV)
        W=torch.linalg.solve(G,XA.T@YA)
        g=torch.Generator(device='cpu').manual_seed(11)
        perm=torch.randperm(YA.shape[0],generator=g).to(YA.device)
        Wsh=torch.linalg.solve(G,XA.T@YA[perm])
        del XA
        XF=stream_rows(FR,120,depths)
        P=(XF@W).cpu(); Psh=(XF@Wsh).cpu(); del XF
        aucs=[round(auc_of(P[:,k],YF[:,k]),4) for k in range(NM)]
        aucs_sh=[round(auc_of(Psh[:,k],YF[:,k]),4) for k in range(NM)]
        good=[k for k,a_ in enumerate(aucs) if a_>=0.70]
        mean=sum(aucs)/NM; mean_sh=sum(aucs_sh)/NM
        RES[nm]={'per_mode_auc':aucs,'shuffled_per_mode_auc':aucs_sh,
                 'mean_auc':round(mean,4),'shuffled_mean_auc':round(mean_sh,4),
                 'modes_at_070':good,'n_at_070':len(good)}
        print(f'  {nm}: mean AUC {mean:.4f} (shuffled {mean_sh:.4f}) | '
              f'modes >=0.70: {good}',flush=True)
        print(f'        per-mode {aucs}',flush=True)
    pa=RES['blk9']['n_at_070']>=3
    pb=RES['blk9']['mean_auc']>RES['blk2']['mean_auc']
    pc=max(RES[n]['shuffled_mean_auc'] for n,_ in DEPTHS)<0.55
    out={'n_modes':NM,'depths':RES,
         'pred_a_three_modes_labelable':bool(pa),
         'pred_b_depth_helps_per_mode':bool(pb),
         'pred_c_control_clean':bool(pc)}
    print(f"\n(c) CONTROL shuffled mean AUC "
          f"{max(RES[n]['shuffled_mean_auc'] for n,_ in DEPTHS):.4f} < 0.55: "
          f"{'HELD' if pc else 'FAILED'}")
    if not pc:
        print('    CONTROL FAILED -- ten ridges fit noise here; (a) and (b) '
              'may not be read.')
    else:
        print(f"(a) >=3 modes at AUC 0.70 on block 9: "
              f"{RES['blk9']['n_at_070']} : {'HELD' if pa else 'FAILED'}")
        print(f"(b) block 9 mean {RES['blk9']['mean_auc']:.4f} > block 2 "
              f"{RES['blk2']['mean_auc']:.4f}: {'HELD' if pb else 'FAILED'}")
        if not pa:
            print("    READING: no individual damage mode is linearly readable "
                  "-- BACKLOG rung 7 is BLOCKED on a labeler that does not "
                  "exist, not merely unstarted.")
