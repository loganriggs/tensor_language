#!/usr/bin/env python3
# BQGATE:96bodyforwards;120seconds;frozenoperator.
"""pred_a coefficient formula<=1e-10 and sparse/dense execution<=1e-5.
pred_b selected six-pair own mixed-term margin error<=10% each four groups of24.
pred_c no sign reversal where native own effect magnitude>=1e-5.
Price96bodyforwards/120seconds; existing frozen sparse packed operator.
Null: retained-port preservation does not extend even to full native head ports on regional text.
Historical regional prefixes, no fit, no freshOOD or full retained-circuit claim.
"""
...
@torch.no_grad()
def main():
    files=json.loads((P/'SPARSE_INTERACTION_FULLHEAD_REGIONAL_V1_BINDING.json').read_text())['files']
    assert all(sha256(Path(k).read_bytes()).hexdigest()==v for k,v in files.items())
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('96bodyforwards;120seconds;frozenoperator');return
    assert not (P/'SPARSE_INTERACTION_FULLHEAD_REGIONAL_V1_RESULT.json').exists()
    signal.alarm(120);torch.set_num_threads(2);torch.backends.cuda.matmul.allow_tf32=False;start=time.perf_counter()
    from fastload import load_model_fast
    model=load_model_fast().cuda().eval();rows=json.loads((P/'FIRST_TOKEN_PATH_FRESH_V1_ROWS.json').read_text())['rows'];assert len(rows)==96;validate(rows)
    ctx={};states=[]
    ...
    def margins(raw):
        scores=(30*torch.tanh(raw/30)).reshape(96,6,2);return scores[:,:,0]-scores[:,:,1]
    own=margins(raw)-margins(zero);pred=margins(changed)-margins(zero);cells=[]
    for group in range(4):
        ref=own[group*24:(group+1)*24];pp=pred[group*24:(group+1)*24];err=pp-ref
    ...
    result={'pred_a':formula_error<=1e-10 and replay<=1e-5,'pred_b':all(c['own_error']<=.1 for c in cells),'pred_c':all(c['material_sign_reversals']==0 for c in cells),
        'cells':cells,'formula_error':formula_error,'executor_replay_error':replay,'raw_mixed_error':float((value-exact).norm()/exact.norm()),'native_own_effects':own.tolist(),'predicted_own_effects':pred.tolist(),'body_forwards':96,'seconds':time.perf_counter()-start,
        'scope':'Historical regional, native full-head17.2 mixed numerator at selected12outputs; frozen sparse model, exact native normalizers/background. Six paired-output probes per prefix, not target-token CE, freshOOD or full retained circuit.'}
    (P/'SPARSE_INTERACTION_FULLHEAD_REGIONAL_V1_RESULT.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps({k:v for k,v in result.items() if not k.endswith('effects')},indent=2))
