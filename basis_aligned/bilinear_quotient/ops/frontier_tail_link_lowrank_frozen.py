"""THE LOW-RANK TAIL QUESTION, RE-ASKED WITH THE FITS FROZEN (Claude, LANE 1)

SS2884 is VOID by its own registered null. It truncated each tail refit's link maps INSIDE the sequential refit loop, so truncating
layer 10 changed what layer 11 was then fitted against: the three arms were three differently-fitted constructions rather than three
perturbations of one. The symptom was non-monotonicity -- rank 1 read **-0.0056** and rank 8 read **+0.1351**, so more rank was worse --
and `c_null_rank_does_not_matter` fired. SS2884 records the rank-1 figure but does NOT claim it.

This rung applies the identical truncation AFTER the `for li in range(10,18)` loop has completed, so every arm perturbs the SAME fitted
stack. Four arms -- baseline, rank 64, rank 8, rank 1 -- give a genuine rank sweep, and pred_b tests monotonicity directly, which is the
property whose absence voided SS2884.

The stake: the tail link maps are **42,467,328 parameters** across eight layers, and SS2881 measured their full removal at **+0.1740
nats**. Rank 1 is **73,728 parameters, a 576x compression**. If the cheap reading survives frozen fits, it is the largest simplification
the frontier has admitted; if it does not, SS2884's rank-1 number was an artifact of the sequential refit and the tail side closes.
...
#                     pred_b_cost_is_monotone_in_rank_once_the_fits_are_frozen
#                     pred_c_rank64_is_essentially_free
#                     pred_d_the_arms_are_connected
#                     pred_e_rank1_is_cheap

Preregistration: polynomial_causal/FRONTIER_TAIL_LINK_LOWRANK_FROZEN_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; four arms (baseline / frozen-truncated rank 64 / 8 / 1)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_tail_link_lowrank_frozen_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
TAILMODE={'mode':None,'rank':None}   # None | 'links' | 'table'
FROZEN={'rank':None}   # truncate every LW[k] AFTER all eight refits are complete
CVSNAP={}
SKIP8=64
...
def main():
    ...
    if FROZEN['rank'] is not None:
        # SS2884 was void because the in-loop truncation changed what LATER refits were fitted against --
        # the arms were three differently-fitted constructions, not three perturbations of one. Truncating
        # here, after the `for li in range(10,18)` loop has completed, makes every arm a perturbation of the
        # SAME fitted stack, which is what a rank sweep has to be to mean anything.
        _r = FROZEN['rank']
        for _li in range(10, 18):
            _k, _l, _CV, _LW, _Wp = S[f'a{_li}L']
            _new = {}
            for _c in _LW:
                _U, _S, _Vh = torch.linalg.svd(_LW[_c].float(), full_matrices=False)
                _new[_c] = (_U[:, :_r] * _S[:_r]) @ _Vh[:_r, :]
            S[f'a{_li}L'] = (_k, _l, _CV, _new, _Wp)
        print(f'FROZEN-TRUNCATED all eight tail refits to rank {_r}', flush=True)
...
PREREG = PT + '../polynomial_causal/FRONTIER_TAIL_LINK_LOWRANK_FROZEN_PREREGISTRATION.md'
PREREG_SHA = "c0fc3205efcfe9f4a01fa4f3678266109b6cecc51785f7901511a3fab34c4b3c"
...
S2884 = PT + 'frontier_tail_link_lowrank_results.json'
S2884_SHA = "7368b30e00f8f76a9b7c68b1968d9b5ef2083d204c90ca64559ab7213b7c8a25"
D_ = 1152; NLINK, NTAIL = 4, 8
BARS = {"reproduce": 0.05, "monotone_tol": -0.01, "rank64_free": 0.02, "connected": 0.005, "rank1_cheap": 0.05}
NULLS = {"still_nonmonotone": -0.05, "rank1_expensive_ge": 0.10}


def _params(r):
    return 2 * r * D_ * NLINK * NTAIL
...
if __name__=='__main__':
    ...
    if _sha(S2884) != S2884_SHA:
        raise RuntimeError(f'frozen hash mismatch: {S2884}')
    void = json.load(open(S2884))['summary']
    SEL['mode']='norm'
    ...
    print('ARM 2/4: link maps truncated to RANK 64, AFTER all eight refits are complete',flush=True)
    FROZEN['rank']=64
    r64=main()

    print('ARM 3/4: RANK 8, after all refits',flush=True)
    FROZEN['rank']=8
    r8=main()

    print('ARM 4/4: RANK 1, after all refits',flush=True)
    FROZEN['rank']=1
    r1=main()
    FROZEN['rank']=None

    # SIGN: L2 is CE ADDED ABOVE THE REAL MODEL, LOWER IS BETTER -> cost POSITIVE = WORSE
    c64 = r64['L2_F'] - r_base['L2_F']
    c8  = r8['L2_F']  - r_base['L2_F']
    c1  = r1['L2_F']  - r_base['L2_F']

    pa = abs(r_base['L2_F']-2.6735) <= BARS['reproduce']
    pb = (c1 - c8 >= BARS['monotone_tol']) and (c8 - c64 >= BARS['monotone_tol'])
    pc = c64 <= BARS['rank64_free']
    pd = abs(c1) >= BARS['connected']
    pe = c1 <= BARS['rank1_cheap']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_cost_is_monotone_in_rank_once_the_fits_are_frozen':bool(pb),
           'pred_c_rank64_is_essentially_free':bool(pc),
           'pred_d_the_arms_are_connected':bool(pd),
           'pred_e_rank1_is_cheap':bool(pe)}
    nulls={'b_null_still_nonmonotone':bool((c1-c8 < NULLS['still_nonmonotone']) or (c8-c64 < NULLS['still_nonmonotone'])),
           'e_null_rank1_was_an_artifact_of_the_sequential_refit':bool(c1>=NULLS['rank1_expensive_ge'])}
    res={'rung':'frontier_tail_link_lowrank_frozen','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,
         'baseline':r_base,'rank64':r64,'rank8':r8,'rank1':r1,'published_L2_F':2.6735,
         'S2884_void_costs':{'rank8':void['cost_rank8_nats'],'rank1':void['cost_rank1_nats']},
         'summary':{'L2_F_baseline':round(r_base['L2_F'],4),
                    'L2_F_rank64':round(r64['L2_F'],4),
                    'L2_F_rank8':round(r8['L2_F'],4),
                    'L2_F_rank1':round(r1['L2_F'],4),
                    'cost_rank64_nats':round(c64,4),
                    'cost_rank8_nats':round(c8,4),
                    'cost_rank1_nats':round(c1,4),
                    'monotone_gap_1_vs_8':round(c1-c8,4),
                    'monotone_gap_8_vs_64':round(c8-c64,4),
                    'S2884_sequential_rank8':round(void['cost_rank8_nats'],4),
                    'S2884_sequential_rank1':round(void['cost_rank1_nats'],4),
                    'params_full':int(D_*D_*NLINK*NTAIL),
                    'params_rank64':_params(64),'params_rank8':_params(8),'params_rank1':_params(1),
                    'compression_rank1':round(D_*D_*NLINK*NTAIL/_params(1),1),
                    'L2_C_baseline':round(r_base['L2_C'],4),'L2_C_rank1':round(r1['L2_C'],4)},
         ...
         'hashes':{PREREG:PREREG_SHA,S2884:S2884_SHA},'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {r_base['L2_F']} vs 2.6735 (tol .05): {'HELD' if pa else 'FAILED'}")
    print(f"(b) monotone: c1-c8 {c1-c8:+.4f}, c8-c64 {c8-c64:+.4f} (tol {BARS['monotone_tol']}): {'HELD' if pb else 'FAILED'}")
    print(f"(c) rank-64 cost {c64:+.4f} <= {BARS['rank64_free']}: {'HELD' if pc else 'FAILED'}")
    print(f"(e) rank-1 cost {c1:+.4f} <= {BARS['rank1_cheap']} (SS2884 sequential read {void['cost_rank1_nats']:+.4f}): {'HELD' if pe else 'FAILED'}")
