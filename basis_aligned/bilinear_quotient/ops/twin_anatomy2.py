...
OUT=PT+'twin_anatomy2_results.json'
...
def main():
    ...
    if SEL.get('twin_mode'):
        ...
        if _tm=='path_only':
            ...
            _tw.append(m.transformer.h[3].register_forward_pre_hook(_p3))
        if _tm=='direct_only':
            def _p3b(mo,args):
                return (args[0]-_st['d2'],)+tuple(args[1:])
            _tw.append(m.transformer.h[3].register_forward_pre_hook(_p3b))
...
if __name__=='__main__':
    ...
    if not (-0.02<=dpo<=dm2+0.05 and -0.02<=ddo<=dm2+0.05):
        raise SystemExit(f'INSTRUMENT FAIL: path arms out of sanity range (dpo {dpo:+.4f}, ddo {ddo:+.4f}, mean2 {dm2:+.4f})')
    pa=dpo<=0.5*dm2
    pb=ddo>=dpo
    ...
    res={'base':{'L2_F':r_base['L2_F']},
         ...
         'pred_a_path_carries_half':bool(pa),'pred_b_path_outweighs_residual':bool(pb),'pred_c_reproduces_r86_base':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'damage: mean2 {dm2:+.4f} | path_only {dpo:+.4f} | direct_only {ddo:+.4f}')
    print(f"(a) path_only {dpo:+.4f} <= 0.5 x mean2 {0.5*dm2:+.4f}: {'HELD' if pa else 'FAILED'}")
    print(f"(b) direct_only {ddo:+.4f} >= path_only {dpo:+.4f}: {'HELD' if pb else 'FAILED'}")
