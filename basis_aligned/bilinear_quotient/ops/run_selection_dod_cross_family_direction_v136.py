...
"""Adjective preposition in/of DoD (v136): SHARED HEADS, SEPARATE DIRECTIONS? v134's construction on this line: remove at the line's own set along the number were-was contrast (a family sharing heads with this one) and compare with the own direction; null = norm-matched random direction x 16. The arm named "person_myself_yourself" in the receipt is the OTHER-family direction (number were-was); "own_they_he" is this line's own contrast (names kept from the v134 body).
...
import run_selection_dod_battery_v85 as v76
import dod_battery

ROOT = dod_battery.ROOT
OUT = ROOT / "circuits/followups/selection_dod_cross_family_direction_v136_result.json"
CANDIDATE_ID = v76.CANDIDATE_ID.rsplit(".", 1)[0] + ".dod_cross_family_direction_v136"
NULL_SEEDS = tuple(range(13601, 13617))
V76_FRACTION, BAND, LITTLE, LIVE_FRACTION, LIVE_POSITIVE, INSTRUMENT_TOL = 0.57, 0.05, 0.25, 0.10, 0.75, 1e-4
FORWARDS_MAX = 63
...
ARMS = {"own_they_he": (" in", " of"), "person_myself_yourself": (" were", " was")}
