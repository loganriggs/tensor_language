#!/usr/bin/env python3
# BQGATE:18bodyforwards,144sequences,lengths8-11;frozen64pairvalidation,no fitting.
...
def main():
 ...
 assert hashlib.sha256(json.dumps(rows,sort_keys=True,separators=(',',':')).encode()).hexdigest()==authority['authority_sha256']=='76ae028307fae869f085c107b34e022b54938c3831c58de063066249e95342fe'
 sequences=[];buckets={}
 for row in rows:
 ...
 assert len(rows)==64 and len(sequences)==128 and set(buckets)=={8,9,10,11}
 assert sum((len(v)+7)//8 for v in buckets.values())==16
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
  print(json.dumps(dict(gpu_accessed=False,body_forwards=18,sequences=144,maximum_length=11,fitting=False)));return
 ...
 program=torch.load(P/'QUARTIC_OUTER32_V1_PROGRAM.pt',weights_only=True)
 ...
 def component(x16,pre):
  ...
  return exact_scalar@writer.T/den[...,None],scalar@writer.T/den[...,None],algebra
 captured={};count=[0,0]
 def input_hook(module,args):captured['x16']=args[0].detach()
 def counter(module,args):
  count[0]+=1;count[1]+=len(args[0]);assert count[0]<=18 and args[0].shape[1]<=11
 ...
 ports={k:torch.empty(128,1152) for k in ('input16','pre','native_output')}
 references=torch.empty(128,1152,dtype=torch.float64);approximations=torch.empty_like(references)
 ...
 try:
  ...
  for length,indices in sorted(buckets.items()):
   for off in range(0,len(indices),8):
    ...
    x16=captured['x16'];ref,approx,algebra=component(x16[:,-1],pre[:,-1]);algebra_errors.append(algebra)
    for key,value in [('input16',x16),('pre',pre),('native_output',native)]:ports[key][selected]=value[:,-1].cpu()
    references[selected]=ref.cpu();approximations[selected]=approx.cpu()
    if first:
 ...
 result=score([references,references,approximations],h,rows,cpu_u)
 families=[]
 ...
 torch.save(dict(ports=ports,reference_write=references,approximate_write=approximations,rows_sha256=digest(P/(STEM+'_ROWS.json')),program_sha256=digest(P/'QUARTIC_OUTER32_V1_PROGRAM.pt')),ap)
 ...
 result.update(dict(families=families,controls=controls,maximum_algebra_error=max(algebra_errors),counts=count,artifact_sha256=digest(ap),program_sha256=digest(P/'QUARTIC_OUTER32_V1_PROGRAM.pt'),rows_sha256=digest(P/(STEM+'_ROWS.json')),wall_seconds=time.perf_counter()-start,scope='Frozen component fresh lexical/construction validation; native parents, normalization and background retained. No fitting, corpus OOD or standalone selective-circuit claim.'))
