"""FRONT SINGLES (rung 106): intrinsic vs compounding damage of each front replacement.

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows; LOWER IS BETTER.
S2198: the prefix marginals are spread (0.25/0.09/0.45/0.63/0.33) but each marginal confounds the site with the
drifted input it receives. Here each front replacement is installed ALONE (everything else real): the intrinsic
per-site damage, plus certificates per arm. Anchor: front-full = +1.7474 (frontier_front_prefix_results.json,
arm prefix_m3E). a0 is known exact (S2198) and is skipped.

REGISTERED PREDICTIONS (arms named; singles = m0E,a1v,m1,m2E,m3E installed alone):
  (a) ADDITIVITY BAND: sum(single aggs) / front-full in [0.6, 1.0] (mild subadditivity; S2188's
      additive-signals frame). Null: strong compounding (< 0.6) or superadditivity (> 1.0).
  (b) DEPTH LAW SURVIVES ISOLATION: agg(single m2E) >= 1.5 x agg(single m0E).
  (c) SANITY: every single agg in [0.02, 1.2].
NULL: the prefix marginals were mostly input-drift (singles tiny, sum << full). PRICE: none (attribution).
Tripwire: INSTRUMENT FAIL if singles pstdev < 0.005. Self-reviewed."""
...
OUT=PT+'frontier_front_singles_results.json'
...
def main():
    ...
    if SEL.get('front_singles'):
        _ROWS=SEL.get('ext_rows',FR)
        cur['clsmap']=classify2(_ROWS).to(DEV)
        _arms={}
        for _nm in ['m0E','a1v','m1','m2E','m3E']:
            _arms['single_'+_nm]=evalV(_ROWS,_ROWS.shape[0],[_nm],ML).detach().cpu()
            print(f'  single {_nm} done',flush=True)
        del cur['clsmap']
        SEL['front_result']=_arms
...
if __name__=='__main__':
    ...
    FULL=json.load(open(PT+'frontier_front_prefix_results.json'))['arms']['prefix_m3E']['agg']
    SEL['mode']='norm'; SEL['K']=4608; SEL['K69']=4608; SEL['K69MAP']={}
    SEL['skipset']=tuple(range(10,18)); SEL['motif_off']=tuple(range(2,10))
    SEL['front_singles']=True; SEL['ext_rows']=CROWS
    print('ARMS: five front singles (each replacement alone, everything else real) - census rows',flush=True)
    main()
    A=SEL['front_result']
    out={}
    for k,cev in A.items():
        ...
        print(f"  {k:12s} agg {agg:+.4f}  valid {nv}/{len(CINFO)}",flush=True)
    import statistics as stt
    aggs=[v['agg'] for v in out.values()]
    if stt.pstdev(aggs)<0.005: raise SystemExit('INSTRUMENT FAIL: singles identical')
    tot=sum(aggs); ratio=tot/FULL
    pa=0.6<=ratio<=1.0
    pb=out['single_m2E']['agg']>=1.5*out['single_m0E']['agg']
    pc=all(0.02<=a<=1.2 for a in aggs)
    res={'arms':out,'sum':round(tot,4),'front_full_anchor':FULL,'ratio_sum_over_full':round(ratio,4),
         'convention':'per-position dCE = CE(config) - CE(real model) on census rows; lower is better',
         'pred_a_additivity_band':bool(pa),'pred_b_depth_law':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'sum(singles) {tot:+.4f} vs front-full {FULL:+.4f}  ratio {ratio:.3f}')
    print(f"(a) ratio {ratio:.3f} in [0.6, 1.0]: {'HELD' if pa else 'FAILED'}")
    print(f"(b) m2E single {out['single_m2E']['agg']:+.4f} >= 1.5 x m0E single ({1.5*out['single_m0E']['agg']:+.4f}): {'HELD' if pb else 'FAILED'}")
    print(f"(c) all singles in [0.02, 1.2]: {'HELD' if pc else 'FAILED'}")
