...
256 calibration plus96 balanced query forks;fresh64IID/64OOD/32fixed-point;8 ALS passes;B4 FP64;1800s;
...
OUT = POLY/'SHARED_JOIN_KERNEL_V2_COVERAGE_RESULT.json'
COEFFICIENTS = POLY/'SHARED_JOIN_KERNEL_V2_COVERAGE_COEFFICIENTS.pt'
ROWS = POLY/'SHARED_JOIN_KERNEL_V2_COVERAGE_ROWS.pt'
...
BOUND = [POLY/'shared_join_kernel_reference.py', POLY/'contextual_history_reference.py',
         POLY/'SHARED_JOIN_KERNEL_V2_COVERAGE_PREREGISTRATION.md',
         ROOT/'basis_aligned/bilinear_quotient/ops/run_equality_router_v1.py']
EXPECTED = '2453d5f66aa32b88a150a7322f33123af5eda2240d9844ef17b43a05705419fb'
...
def calibration_tokens(torch):
    original, _, _ = population(torch, 'iid', 256, 15909)
    support, _, _ = population(torch, 'iid', 1, 16909)
    # One independent calibration world, complete entity x hop query coverage.
    support = support.expand(96, -1).clone()
    support[:, 49] = torch.arange(24).repeat_interleave(4)
    support[:, 50] = torch.arange(25, 29).repeat(24)
    return torch.cat((original, support))


def coverage_controls(torch, R):
    tokens = calibration_tokens(torch)
    observed = set()
    for h in range(2):
        group, _, match = R.indices(tokens, h)
        observed.update(group[match].unique().tolist())
    expected = set()
    def add(h, qr, sr, lag):
        expected.add(((h*8+qr)*8+sr)*51+lag)
    for h in range(2):
        for distance in range(1, 24):
            if h == 0:
                add(h, 1, 0, 2*distance+1); add(h, 1, 1, 2*distance)
            else:
                add(h, 0, 1, 2*distance-1); add(h, 1, 1, 2*distance)
        for qr in range(3, 8):
            qpos = 49 if qr == 3 else 50
            for source in range(48):
                add(h, qr, source%2, qpos-source)
            add(h, qr, qr, 0)
            if qr != 3:
                add(h, qr, 3, 1)
    return {'passed': observed == expected, 'observed_cells': len(observed),
            'structurally_possible_cells': len(expected), 'missing': sorted(expected-observed),
            'unexpected': sorted(observed-expected), 'documents': len(tokens)}
...
def calibrate(torch, R, model):
    ...
    diagnostic = {'documents': len(tokens), 'matching_samples': len(group), 'seen_role_lag_cells': int(seen.sum()),
...
def run(torch, R, score, checks):
    ...
    results, row_data = {}, {}; exact_error = 0.; evaluated_batches = 0; finite = True; all_seen = True
    with torch.inference_mode():
        ...
        for pop, count, seed in (('iid', 64, 16910), ('ood_two_12cycles', 64, 16911), ('fixed_point', 32, 16912)):
    ...
    pred_a = checks['passed'] and all_seen and finite and evaluated_batches > 0 and exact_error <= 1e-9
    ...
    receipt = {'experiment': 'shared_join_kernel_v2_coverage', 'runner_sha256': score.digest(SOURCE), 'bound_sha256': EXPECTED,
               ...
               'exact_baseline_max_abs_logit': exact_error if evaluated_batches else None, 'evaluated_arm_batches': evaluated_batches, 'finite': finite,
...
def main():
    ...
    checks['coverage'] = coverage_controls(torch, R)
    checks['checks']['complete_calibration_support'] = checks['coverage']['passed']
