"""REGISTERED CLAIM: MOTIF-EXACT POINT (rung 239) - S2335 becomes official.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2335 measured {motifs exact, tail
r64} at census +0.0608 / fresh +0.0601 / valid 9 - the same price as uniform r96 (72.1M vs 72.7M pattern
values) with better census. Identical rebuild; claim bars registered.
REGISTERED PREDICTIONS (anchors: the S2335 receipt):
  (a) REPRODUCTION: |census - 0.0608| <= 0.015.
  (b) CERTIFICATES: valid >= 9.
  (c) REPRODUCTION: |L2_F - 0.0601| <= 0.015.
NULL: noise. PRICE: ~199M values (motif patterns exact 47.2M + tail r64 24.9M). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: registered claim motif-exact')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_claim_motif_results.json'
...
if __name__=='__main__':
    ...
    pa=abs(agg-0.0608)<=0.015
    pb=nv>=9
    pc=abs(L2F-0.0601)<=0.015
    ...
    print(f"(a) |census {agg:+.4f} - 0.0608| <= 0.015: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 9; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |L2F {L2F:+.4f} - 0.0601| <= 0.015: {'HELD' if pc else 'FAILED'}")
