"""LOW-RANK QK MOTIF HEADS (rung 181): the pattern-side lever, weights-only.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2278: the remaining ~0.50 motif
ceiling is attention-PATTERN structural. bilin18 uses SQUARED bilinear attention (pattern = (q.k)(q2.k2)/
128^2, RMS+rotary, causal tril, no softmax) - so each motif head's pattern is four D->128 projections,
SVD-truncatable WEIGHTS-ONLY to rank r. Here every motif head becomes a rank-8 REAL-pattern head: z =
pat_r @ vm (real values; alpha and the OV residual retired in this arm). Type-legal (truncation of real
weights, the CP analog at the pattern; no refit).

REGISTERED PREDICTIONS (anchors: rung-179 point census +1.8950; motifs-alone +0.553; subword +2.197):
  (a) THE PATTERN YIELDS: census <= 1.75 (>= 0.145 of the ~0.50 pattern ceiling at rank 8).
  (b) SUBWORD MOVES: <= 2.05.
  (c) SANITY: increment in [0.25, 0.50] AND census >= 1.3; inert tripwire as rung 178.
NULL: rank-8 patterns are no better than fixed prev/self (< 0.05 gain) - squared attention needs high-rank
patterns and the motif ceiling is irreducible below full QK. PRICE: 76 heads x 4 x 8 x (1152+128) =
3,112,960 values (vs the alpha grammar's ~2k/head). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: low-rank QK motif heads')
    raise SystemExit(0)
import torch
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['qk_r']=8
    print('ARM: rung-127 build + rank-8 QK patterns at all motif heads',flush=True)
    main()
    ...
    print(f"(c) increment {inc:+.4f} in [0.25, 0.50], census >= 1.3: {'HELD' if pc else 'FAILED'}")
