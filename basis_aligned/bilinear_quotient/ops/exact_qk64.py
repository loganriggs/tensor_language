"""RANK-64 ENDGAME (rung 201): the clean pattern-rank curve with certificates.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. With everything else exact (S2297),
the residual +0.149 is pattern truncation + interactions ONLY - the rank curve is finally confound-free.
r=64 = half of full head rank; certificate rows saved.

REGISTERED PREDICTIONS (anchor: rung-199 census +0.1492, valid 2):
  (a) STILL YIELDING: census <= 0.10.
  (b) CERTIFICATES CLIMB: valid >= 4 of 62.
  (c) SANITY: census in [0.02, 0.13]; inert tripwire.
NULL: the interaction floor dominates below 0.15 (census >= 0.12; certificates stuck at ~2). PRICE:
patterns 48.5M (r64). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: rank-64 endgame')
    raise SystemExit(0)
import torch
...
OUT=PT+'exact_qk64_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=64; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    print(f"(a) census {agg:+.4f} <= 0.10: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 4; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) census in [0.02, 0.13]; era rho {rho:.4f}: {'HELD' if pc else 'FAILED'}")
