"""MANIPULABILITY II (rung 265): knockout-transfer at m13 inside the compiled mixed config.

CONVENTION (S2135): per-position dCE = CE(mixed config with m13 mean-ablated) - CE(real model); LOWER
IS BETTER. The program goal includes MANIPULABLE: interventions on the compiled object should reproduce
the real model's measured effects. Protocol mirrors the battery: m13's output is replaced by its FW
mean (disjoint from census) INSIDE the mixed-spectrum config; per-circuit member damage compared to (i)
battery refs at m13-family circuits, (ii) removal_matrix's real m13 collateral row.
REGISTERED PREDICTIONS (arms: COMPILED-KO = this run; REAL-KO = battery/removal_matrix receipts):
  (a) OWN-FAMILY EFFECT TRANSFERS: median member/ref ratio over m13-top circuits in [0.7, 1.6].
  (b) COLLATERAL PROFILE TRANSFERS: Spearman vs the real m13 row >= 0.8 over ~62 circuits.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: knockout transfer m13')
    raise SystemExit(0)
import torch
...
OUT=PT+'ko_transfer_m13_results.json'
...
if __name__=='__main__':
    ...
    _own=sorted(r['member_absdce']/max(r['ref'],1e-9) for r in rows if _topK.get(r['tag'])=='m13')
