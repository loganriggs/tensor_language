#!/usr/bin/env python3
# BQLANE: cpu
"""Fake-facade end-to-end state-machine tests for R594; no torch/model import."""
...
MODULE = Path(__file__).with_name("induction_centered_fixed_geometry_rung594.py")
SPEC = importlib.util.spec_from_file_location("r594_fake", MODULE)
assert SPEC and SPEC.loader
r594 = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(r594)


def toy_bundle(phase: str) -> dict[str, object]:
    tokens = np.full((2, 30), r594.PAD_TOKEN, dtype="<i8")
    tokens[:, :3] = np.asarray([[3, 2, 3], [4, 5, 6]], dtype="<i8")
    specs = [
        {"final_position": 2, "payload_positions": [1, 2]},
        {"final_position": 2, "payload_positions": [1, 2]},
    ]
    expected_support = r594.expected_support_mask(specs, tokens)
    ...
    for index, kind in enumerate(("endpoint", "native", "replay", "score", "payload", "joint")):
        calls.append({
            ...
            "token_record_id": token_id, "token_sha256": r594.sha256_bytes(tokens.tobytes(order="C")),
            "batch_size": 2, "physical_width": 30,
            "authority_row_ids": ["e0", "e1"],
            "direction_ids": [] if kind == "endpoint" else ["d0", "d1"],
            "query_positions": [2, 2],
            **({
                "expected_support_sha256": r594.sha256_bytes(expected_support.tobytes(order="C")),
...
def arrays_for(call, tokens, planned=None, specs=None):
    ...
    common = {
        ...
        "logits.npy": np.zeros((b, r594.VOCAB), dtype="<f4"),
    }
    parent32 = {
        name: np.zeros((b, 4, r594.RESIDUAL), dtype="<f4")
        for name in (
    ...
    parent64 = {
        name: np.zeros((b, 4, r594.RESIDUAL), dtype="<f8")
        for name in (
    ...
    if call["call_kind"] == "endpoint":
        return common | {
            ...
            "factor_u.npy": np.zeros((b, 4, 2, r594.RESIDUAL), dtype="<f4"),
            "support.npy": r594.expected_support_mask(specs, tokens),
        } | parent
    if call["call_kind"] == "native":
        return common | {
            "live_e.npy": np.zeros((b, 4, 2), dtype="<f4"),
            "live_u.npy": np.zeros((b, 4, 2, r594.RESIDUAL), dtype="<f4"),
        } | parent
    delta = np.zeros((b, 4, r594.RESIDUAL), dtype="<f4") if planned is None else planned.copy()
    total=np.stack((delta[:,0],delta[:,1],np.add(delta[:,2],delta[:,3],dtype=np.float32)),axis=1)
    before=np.full_like(total,1500.)
    after=np.add(before,total,dtype=np.float32)
    return common | {"planned_component_deltas.npy": delta.copy(), "registered_component_deltas.npy": delta,
        "layer_before.npy":before,"layer_total.npy":total,"layer_after.npy":after,
        "layer_delta.npy":after.astype(np.float64)-before.astype(np.float64)}
...
class FakeExecutor:
    ...
    def execute(self, call, tokens, specs, planned):
        ...
        if self.mutation == (kind, "hook"):
            arrays["planned_component_deltas.npy"][0, 0, 0] = 2e-5
...
def contexts(bundle):
    ...
    zero_u = np.zeros((2, 4, 2, r594.RESIDUAL), dtype="<f4")
    native_logits = np.zeros((2, r594.VOCAB), dtype="<f4")
    output = {}
    for call in bundle["calls"]:
        ...
        if call["call_kind"] in r594.MACHINE_ARMS:
            cached["native_logits"] = native_logits
        output[call["call_id"]] = {
            "specs": bundle["specs"],
            "planned": None if call["call_kind"] in ("endpoint", "native") else np.zeros((2, 4, r594.RESIDUAL), dtype="<f4"),
            "cached": (
...
def public_paths(root: Path):
    return [root / path.name for path in r594.PUBLIC_NAMESPACES]
...
def test_fake_full_fit_scientific_null_stops_before_select(tmp_path: Path) -> None:
    ...
    observed = r594.run_manifest_calls(executor, fit, contexts(fit), stage=stage, public_root=tmp_path)
    ...
    if fit_gate_held:
        r594.run_manifest_calls(executor, select, contexts(select), stage=stage, public_root=tmp_path)
...
def test_fake_full_fit_held_opens_select_once(tmp_path: Path) -> None:
    ...
    assert r594.run_manifest_calls(executor, fit, contexts(fit), stage=fit_stage, public_root=tmp_path)["status"] == "complete"
    fit_gate_held = True
    if fit_gate_held:
        assert r594.run_manifest_calls(executor, select, contexts(select), stage=select_stage, public_root=tmp_path)["status"] == "complete"
...
def test_invalid_mid_arm_publishes_exact_prefix_and_stops(tmp_path: Path) -> None:
    ...
    observed = r594.run_manifest_calls(executor, bundle, contexts(bundle), stage=stage, public_root=tmp_path)
    ...
    assert (tmp_path / r594.INVALID_RECEIPT.name).is_file()
    assert not (tmp_path / r594.NORMAL_RESULT.name).exists()
    evidence = tmp_path / r594.INVALID_EVIDENCE.name
    ledger = evidence / "FIT" / "canonical_slice_ledger.jsonl"
    assert len(ledger.read_text(encoding="utf-8").splitlines()) == 1
    receipt = json.loads((tmp_path / r594.INVALID_RECEIPT.name).read_text())
    ...
    assert set(bounds["files"]) == set(r594.StreamingPhaseStore.ENDPOINT_MAP.values())
    assert not set(r594.StreamingPhaseStore.NATIVE_MAP.values()).intersection(bounds["files"])
...
def test_first_endpoint_invalid_publishes_raw_only_and_no_preallocation(tmp_path: Path) -> None:
    ...
    stage = tmp_path / "stage"; stage.mkdir()
    observed = r594.run_manifest_calls(
    ...
    assert raw.is_dir() and len(list(raw.glob("*.npy"))) == len(r594.mandatory_call_shapes(bundle["calls"][0]))
    receipt = json.loads((tmp_path / r594.INVALID_RECEIPT.name).read_text())
    bounds = receipt["canonical_written_bounds"]
    assert bounds == {
    ...
    for name, descriptor in receipt["evidence_files"].items():
        ...
        assert descriptor["sha256"] == r594.sha256_file(path)
...
def test_nonfinite_final_call_gets_distinct_mask_index(tmp_path: Path) -> None:
    ...
    call_dir = tmp_path / r594.INVALID_EVIDENCE.name / "calls" / "0003_FIT:score"
...
def test_hard_abort_publishes_nothing(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="incomplete"):
        r594.run_manifest_calls(executor, bundle, contexts(bundle), stage=stage, public_root=tmp_path)
...
def test_live_cache_component_drift_is_invalid(tmp_path: Path) -> None:
    ...
    class Drift(FakeExecutor):
        def execute(self, call, tokens, specs, planned):
            ...
            return response
    stage = tmp_path / "stage"; stage.mkdir()
    observed = r594.run_manifest_calls(Drift(), bundle, contexts(bundle), stage=stage, public_root=tmp_path)
    assert observed["diagnostic"]["failure_predicate"] == "factor_transport_failed"
...
def test_nonzero_literal_self_replay_is_invalid(tmp_path: Path) -> None:
    ...
    stage = tmp_path / "stage"; stage.mkdir()
    observed = r594.run_manifest_calls(FakeExecutor(), bundle, planned, stage=stage, public_root=tmp_path)
    assert observed["diagnostic"]["failure_predicate"] == "centered_hook_delta_failed"


def test_candidate_has_no_live_removal_or_renamed_machine_keys() -> None:
    producer_text = MODULE.read_text()
    runtime_text = Path(__file__).with_name("induction_centered_fixed_geometry_rung594_runtime.py").read_text()
    assert "live_removed" not in producer_text + runtime_text
    assert r594.MACHINE_ARMS == ("replay", "score", "payload", "joint")
    assert r594.SITES == ("L5H5", "L7H3", "L8H3", "L8H4")
    assert r594.ROLES == ("A", "C")
...
def test_normal_publication_is_receipt_last_and_collision_safe(tmp_path: Path) -> None:
    ...
    receipt = r594.publish_normal(stage, result, public_root=tmp_path)
    assert (tmp_path / r594.NORMAL_RECEIPT.name).is_file()
    assert receipt["result_sha256"] == r594.sha256_file(tmp_path / r594.NORMAL_RESULT.name)
    second = tmp_path / "second"; (second / "evidence").mkdir(parents=True)
    with pytest.raises(RuntimeError, match="occupied"):
        r594.publish_normal(second, result, public_root=tmp_path)


def test_model_runtime_module_has_no_eager_torch_import() -> None:
    text = Path(__file__).with_name("induction_centered_fixed_geometry_rung594_runtime.py").read_text()
    tree = ast.parse(text)
    eager = [
...
def test_observed_rounding_and_physical_mutations():
    bundle=toy_bundle('FIT');call=bundle['calls'][3]
    tokens=bundle['token_arrays'][call['token_record_id']]
    plan=np.zeros((2,4,r594.RESIDUAL),dtype=np.float32)
    plan[:,:,:]=np.array([.015,.021,.03,.04],dtype=np.float32)[None,:,None]
    arrays=arrays_for(call,tokens,plan,bundle['specs'])
    predicate,details=r594.evaluate_completed_call(call,arrays,tokens)
    assert predicate is None
    assert details['representability_residual_max_abs']>1e-5
    assert details['rounded_poststate_max_abs']==0
    mutations=[]
    skipped={k:v.copy() for k,v in arrays.items()};skipped['layer_after.npy']=skipped['layer_before.npy'].copy();mutations.append(skipped)
    sign={k:v.copy() for k,v in arrays.items()};sign['layer_after.npy']=sign['layer_before.npy']-sign['layer_total.npy'];mutations.append(sign)
    corrupt={k:v.copy() for k,v in arrays.items()};corrupt['layer_after.npy'][0,0,0]=np.nextafter(corrupt['layer_after.npy'][0,0,0],np.float32(np.inf));mutations.append(corrupt)
    site={k:v.copy() for k,v in arrays.items()};site['layer_total.npy'][:,[0,1]]=site['layer_total.npy'][:,[1,0]];mutations.append(site)
    for changed in mutations:
        # Do not rely on an inconsistent cached delta to reject a corrupted addition.
        changed['layer_delta.npy']=changed['layer_after.npy'].astype(np.float64)-changed['layer_before.npy'].astype(np.float64)
        predicate,_=r594.evaluate_completed_call(call,changed,tokens)
        assert predicate=='centered_hook_delta_failed'
