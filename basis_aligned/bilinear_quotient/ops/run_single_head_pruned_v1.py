...
STEM='SINGLE_HEAD_PRUNED_V1';ARMS=['native','native8_midpoint','compiled']
...
def main():
 ...
 saved=torch.load(P/'SINGLE_HEAD_FRESH_V1_ARTIFACT.pt',weights_only=True)['fixtures']
 ...
 v=expand(values,mapping,6);e=v-v[:1];old=torch.load(P/'SINGLE_HEAD_FRESH_V1_ARTIFACT.pt',weights_only=True)['values'];anchor=float((v[:2]-old[[0,1]]).abs().max());records={}
