#!/usr/bin/env python3
"""Prospective, CPU-testable contract and producer core for Rung 594.
...
import numpy as np
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "polynomial_causal"))
import fp32_add_observation_v1 as rounding
...
INSTRUMENT_AMENDMENT = POLY / "INDUCTION_CENTERED_FIXED_GEOMETRY_RUNG594_INSTRUMENT_REPAIR_AMENDMENT.md"
RUNTIME = OPS / "induction_centered_fixed_geometry_rung594_runtime.py"
...
SOURCE_HASHES = {
    ...
    INSTRUMENT_AMENDMENT: "2ba48978f0c9678d09fc053f6959de4cc8965a26b548109214a47faff3bb9270",
    RUNTIME: "cf28b62f5882ea84d4f4e8a5c5958ae38dc69328aab6e7f7f96e1ede230ff0bf",
    POLY / "fp32_add_observation_v1.py": "6ebfb929137e0dbaface93d8938e76f3e0c3e76951eb15e092706ba5d42816d1",
}

SCHEMA = "induction_centered_fixed_geometry_rung594_v1"
DRYRUN_SCHEMA = "induction_centered_fixed_geometry_rung594_dryrun_v1"
WIDTH = 30
BATCH = 32
VOCAB = 50_304
RESIDUAL = 1_152
PAD_TOKEN = 50_256
TOLERANCE = 1e-5
INITIAL_MINIMUM_FREE_BYTES = 11_010_562_560
SELECT_MINIMUM_FREE_BYTES = 4_473_957_888
MINIMUM_FREE_BYTES = INITIAL_MINIMUM_FREE_BYTES
COMPLETE_CANONICAL_DATA_BYTES = 9_804_907_008
LARGEST_CURRENT_CHUNK_DATA_BYTES = 45_652_480
MAXIMUM_STREAMING_DATA_BYTES = 9_850_559_488
...
NORMAL_RESULT = ROOT / "induction_centered_fixed_geometry_rung594_results.json"
NORMAL_RECEIPT = ROOT / "induction_centered_fixed_geometry_rung594_receipt.json"
NORMAL_EVIDENCE = ROOT / "induction_centered_fixed_geometry_rung594_evidence"
INVALID_RESULT = ROOT / "induction_centered_fixed_geometry_rung594_invalid_diagnostic.json"
INVALID_RECEIPT = ROOT / "induction_centered_fixed_geometry_rung594_invalid_receipt.json"
INVALID_EVIDENCE = ROOT / "induction_centered_fixed_geometry_rung594_invalid_evidence"
PUBLIC_NAMESPACES = (
...
def require_free_space(
    ...
    if observed < minimum:
        raise RuntimeError(
            f"R594 insufficient free space before {boundary} boundary: {observed} < {minimum}"
        )
...
def verify_sources() -> dict[str, str]:
    ...
    for path, expected in SOURCE_HASHES.items():
        if not path.is_file() or sha256_file(path) != expected:
            raise RuntimeError(f"frozen R594 authority changed: {path}")
...
def load_authority() -> tuple[object, dict[str, object]]:
    ...
    verify_sources()
    r591 = _immutable_module(R591, SOURCE_HASHES[R591], "r594_pinned_r591_authority")
    ...
    r585.__r594_runtime_loader__ = r591._load_runtime
...
def mandatory_call_shapes(call: Mapping[str, object]) -> dict[str, tuple[np.dtype, tuple[int, ...]]]:
    ...
    return common | {
        "planned_component_deltas.npy": (np.dtype("<f4"), (b, 4, RESIDUAL)),
        "registered_component_deltas.npy": (np.dtype("<f4"), (b, 4, RESIDUAL)),
        **{name: (np.dtype("<f4"), (b, 3, RESIDUAL)) for name in ("layer_before.npy", "layer_total.npy", "layer_after.npy")},
        "layer_delta.npy": (np.dtype("<f8"), (b, 3, RESIDUAL)),
    }
...
def first_failure(failures: Sequence[str]) -> str | None:
    ...
    if unknown or observed.intersection(FORBIDDEN_PREDICATES):
        raise ValueError(f"unknown/forbidden R594 predicates: {sorted(unknown)}")
...
def phase_evidence_schema(phase: str) -> dict[str, object]:
    ...
    return {
        ...
        **{f"directed_{name}": {"dtype": "float32", "shape": [nd, 4, RESIDUAL]} for name in (
        ...
        "planned_component_deltas.npy": {"dtype": "float32", "shape": [nd, 4, 4, RESIDUAL]},
        **{name: {"dtype": "float32", "shape": [nd, 4, 3, RESIDUAL]} for name in ("layer_before.npy", "layer_total.npy", "layer_after.npy")},
        "layer_delta.npy": {"dtype": "float64", "shape": [nd, 4, 3, RESIDUAL]},
...
def canonical_array_schema(bundle: Mapping[str, object]) -> dict[str, tuple[np.dtype, tuple[int, ...]]]:
    ...
    directed = {
        ...
        "planned_component_deltas.npy": (np.dtype("<f4"), (directed_rows, 4, 4, RESIDUAL)),
        **{name: (np.dtype("<f4"), (directed_rows, 4, 3, RESIDUAL)) for name in ("layer_before.npy", "layer_total.npy", "layer_after.npy")},
        "layer_delta.npy": (np.dtype("<f8"), (directed_rows, 4, 3, RESIDUAL)),
    ...
    def _derive_chunk_records(
        ...
        for local, directed_id_value in enumerate(native_call["direction_ids"]):
            ...
            for arm in ("score", "payload", "joint"):
                ...
                actual = arm_arrays["planned_component_deltas.npy"][local]
                records.append({
    ...
    def ingest_directed_chunk(
        ...
        for axis1, kind in enumerate(MACHINE_ARMS):
            descriptors_by_kind[kind].append(self._write_slice(
                "planned_component_deltas.npy", start, stop, by_kind[kind]["planned_component_deltas.npy"], axis1=axis1
            ))
            for name in ("layer_before.npy", "layer_total.npy", "layer_after.npy", "layer_delta.npy"):
                descriptors_by_kind[kind].append(self._write_slice(name, start, stop, by_kind[kind][name], axis1=axis1))
...
def publish_invalid_prefix(
    ...
    diagnostic = {
        "schema": "induction_centered_fixed_geometry_rung594_invalid_diagnostic_v1",
    ...
    receipt = {
        "schema": "induction_centered_fixed_geometry_rung594_invalid_receipt_v1",
...
def evaluate_completed_call(
    ...
    if kind in MACHINE_ARMS and not nonfinite:
        observation = rounding.observe(arrays["layer_before.npy"], arrays["layer_total.npy"], arrays["layer_after.npy"])
        planned = arrays["registered_component_deltas.npy"]
        expected_total = np.stack((planned[:,0], planned[:,1], np.add(planned[:,2], planned[:,3], dtype=np.float32)), axis=1)
        components_equal = np.array_equal(arrays["planned_component_deltas.npy"], planned)
        totals_equal = np.array_equal(expected_total.view(np.uint32), arrays["layer_total.npy"].view(np.uint32))
        delta_equal = np.array_equal(observation["actual_delta"], arrays["layer_delta.npy"])
        details["rounded_poststate_max_abs"] = observation["maximum_poststate_error"]
        details["representability_residual_max_abs"] = observation["maximum_rounding_residual"]
        details["rounding_envelope_passed"] = observation["rounding_envelope_passed"]
        details["registered_layer_totals_bitwise"] = totals_equal
        if not (observation["passed"] and components_equal and totals_equal and delta_equal) or (kind == "replay" and np.any(planned)):
...
def publish_normal(
    ...
    receipt = {
        "schema": "induction_centered_fixed_geometry_rung594_receipt_v1",
...
def run_science(*, public_root: Path = ROOT) -> dict[str, object]:
    ...
    started = time.time()
    implementation_sha256 = globals().get("__r594_immutable_sha256__")
    adapter_sha256 = globals().get("__r594_adapter_sha256__")
    if implementation_sha256 is None or adapter_sha256 is None:
        raise RuntimeError("R594 science requires immutable managed producer and adapter provenance")
    public = tuple(public_root / path.name for path in PUBLIC_NAMESPACES)
    occupied = [str(path) for path in public if path.exists()]
    if occupied:
        raise RuntimeError(f"R594 public namespace occupied: {occupied}")
    ...
    runtime = _immutable_module(RUNTIME, SOURCE_HASHES[RUNTIME], "r594_pinned_model_runtime")
    # This constructor is the first permitted torch/checkpoint/CUDA boundary.
    executor = runtime.R594ModelExecutor(types.SimpleNamespace(**globals()), r585)
    r585 = executor.r585
    invalid_provenance = {
    ...
    stage = Path(tempfile.mkdtemp(prefix=".r594-stage-", dir=public_root))
    ...
    try:
        ...
        result = {
            ...
            "insertion_activity_definition": "norm of registered planned per-head components; actual physical layer additions stored separately",
            "raw_storage": "RAM-backed scratch, volatile; compact result/receipt copied to workspace by wrapper",
            "independent_agent_review_claimed": False,
...
def build_dryrun() -> dict[str, object]:
    ...
    return {
        ...
        "streaming_storage": {
            ...
            "remaining_select_plus_chunk_bytes": 3_313_954_816,
