#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_instrument pred_b_absolute pred_c_improvement
"""Frozen mixed-CP root1 finite removal, two seeds, original32prefixes.
Replay<1e-5/zeroedit0. All16seed/domain/strength/conditioncells<=10%error.
Relativecontrol: allcells<=.8oldempirical384error. Same1536products2385920coeff.
...
def cp_values(program,x):
 z=x@program['factors'][0].T
 for a in program['factors'][1:]:z=z*(x@a.T)
 return z@program['coefficients'].T
...
def main():
 ...
 torch.backends.cuda.matmul.allow_tf32=False;out=P/'MIXED_CP_REMOVAL_V1.json';assert not out.exists();start=time.monotonic()
 programs={};hashes={}
 for seed in [1001,1002]:
  path=P/f'MIXED_CP_FEATURES_SEED{seed}_V1.pt';hashes[seed]=hashlib.sha256(path.read_bytes()).hexdigest();p=torch.load(path,weights_only=True)
  programs[seed]=dict(factors=[a.cuda().float() for a in p['factors']],coefficients=p['coefficients'].cuda().float(),writer=p['writer'].cuda().float())
 ...
 baseline=json.loads((P/'ROOT_CASE_ABLATION_V1.json').read_text());old={(r['domain'],r['alpha'],r['condition']):r for r in baseline['summary']}
 native_replay=max(abs(r['reference_energy']-old[r['domain'],r['alpha'],r['condition']]['reference_energy'])/old[r['domain'],r['alpha'],r['condition']]['reference_energy'] for r in summary)
 pred=dict(pred_a_instrument=max(checks)<1e-5 and reader_replay<1e-5 and max(zeros)==0 and native_replay<1e-4,pred_b_absolute=len(summary)==16 and all(r['error'] is not None and r['error']<=.1 for r in summary),pred_c_improvement=all(r['error']<=.8*old[r['domain'],r['alpha'],r['condition']]['error'] for r in summary))
 result=dict(predictions=pred,summary=summary,rows=rows,program_sha256=hashes,reader_replay=reader_replay,maximum_native_replay=max(checks),maximum_zero_edit=max(zeros),prior_reference_energy_replay=native_replay,seconds=time.monotonic()-start,scope='Frozen two CP candidates; native root1 projected quartic removal at quarter/full strength, actual final RMSNorm/softcap, same original denominator/otherbranches. OpenedFineWeb/code panels. Selectivity previouslyfailed andnotreset. No OOD or semanticadoption claim. Eachcandidate1536products2385920coefficients.')
