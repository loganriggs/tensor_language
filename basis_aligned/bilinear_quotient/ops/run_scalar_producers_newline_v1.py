#!/usr/bin/env python3
# BQGATE:32 body forwards;16 newline contexts13-19tokens;4suffixarms;180seconds;no fitting.
"""pred_a compiled/native head contributions<=1e-5, noedit logits/CE<=1e-4relative.
pred_b EACHfamily newline/comma mean>=.2,>=6/8positive,meanCE<=5;
actualwholehead8.2removal meanCEincrease>=.02 and>=4/8positive.
pred_c EACHcandidate/family meanabsnewlineCEchange<=.02,maxabs<=.1.
Null: missing native service or positive control makes preservation inconclusive.
Price32single-row native body forwards+4suffix evaluations/row;180sec,no fit.
"""
...
STEM='SCALAR_PRODUCERS_NEWLINE_V1'
@torch.no_grad()
def main():
 binding=json.loads((P/(STEM+'_BINDING.json')).read_text())['files'];assert all(digest(k)==v for k,v in binding.items())
 rows=json.loads((P/(STEM+'_ROWS.json')).read_text())['rows'];assert len(rows)==16 and all(sum(r['family']==f for r in rows)==8 for f in range(2));assert max(len(r['ids']) for r in rows)==19 and all(r['newline_id']==198 and r['comma_id']==11 for r in rows)
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('32bodyforwards16fixednewlinecontexts; wholeheadpositivecontrol+3conditionalregionalremovals');return
 ...
 for layer in (8,9):
  ...
  scales.append(scale)
 captured={};head_replay=[];subtract_replay=[];component_replay=[];branch_replay=[];logit_replay=[];ce_replay=[];ce=torch.zeros(16,5,dtype=torch.float64);margins=torch.zeros_like(ce);body_count=0
 def hook(index,head,module,args,output):
  ...
  if captured['mode']=='wholehead' and index==0:
   removed=output[0]-headwrite;zeroed=z.clone();zeroed[:,:,head]=0;direct=module.c_proj(zeroed.reshape_as(x));subtract_replay.append(float((removed-direct).norm()/direct.norm()));return removed,output[1]
  return output
 ...
 assert body_count==32
 cells=[]
 for family in range(2):
  ix=[i for i,r in enumerate(rows) if r['family']==family];delta=ce[ix]-ce[ix,:1];cap=float(margins[ix,0].mean())>=.2 and int((margins[ix,0]>0).sum())>=6 and float(ce[ix,0].mean())<=5;positive=float(delta[:,4].mean())>=.02 and int((delta[:,4]>0).sum())>=4;preserve=all(float(delta[:,arm].abs().mean())<=.02 and float(delta[:,arm].abs().max())<=.1 for arm in (1,2,3))
  cells.append(dict(family=family,native_capability=cap,wholehead_positive_control=positive,candidate_preservation=preserve,baseline_mean_ce=float(ce[ix,0].mean()),baseline_mean_margin=float(margins[ix,0].mean()),baseline_positive_margin_count=int((margins[ix,0]>0).sum()),mean_ce_change=delta.mean(0).tolist(),meanabs_ce_change=delta.abs().mean(0).tolist(),maxabs_ce_change=delta.abs().amax(0).tolist(),positive_ce_change_count=(delta>0).sum(0).tolist(),mean_margin_change=(margins[ix]-margins[ix,:1]).mean(0).tolist()))
 A=max(head_replay+subtract_replay+component_replay+branch_replay)<=1e-5 and max(logit_replay+ce_replay)<=1e-4;B=A and all(c['native_capability'] and c['wholehead_positive_control'] for c in cells);C=B and all(c['candidate_preservation'] for c in cells)
 result={'pred_a':A,'pred_b':B,'pred_c':C,'checks':dict(head_replay_max=max(head_replay),subtraction_replay_max=max(subtract_replay),component_replay_max=max(component_replay),branch_replay_max=max(branch_replay),logit_replay_max=max(logit_replay),ce_replay_max=max(ce_replay)),'cells':cells,'arms':['native','remove_regional_edge8_2','remove_regional_edge9_8','remove_joint_regional_edges','remove_wholehead8_2'],'body_forwards':body_count,'seconds':time.perf_counter()-tic,'source_shas':binding,'scope':'Dedicated newline capability and actual recursive wholehead8.2 positive control. Candidate removal affects only regional four-reader consumer edges, not all head consumers. Missing capability/control makes preservation inference inconclusive.'}
