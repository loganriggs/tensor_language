# THE RANK LEVER AT DEPLOYED COVERAGE -- second-class confirmation of §1919.
#
# §1919 showed that raising §1870's fallback map from rank 64 to 512 lifts the UNCOVERED reliability
# gradient by +0.78 / +0.63 / +0.70, and §1920 confirmed those gains clear their own seed noise by
# 3.9x / 2.0x / 2.2x. Both were measured at 16,110 covered types.
#
# §1922 then measured the rank-64 signal at the DEPLOYED 5,419 coverage and found the uncovered gradient
# is +3.28 / +3.11 / +2.10 there, on 2.4x the sample -- and §1923 found coverage is NOT a smooth axis for
# this quantity, with the uncovered gradient peaking mid-coverage on one role. So a lever measured only
# at 16,110 cannot be assumed to hold at 5,419, and 5,419 is the build anyone would ship.
#
...
# §1919's figures are at 16,110 and are what is being generalised, NOT controls -- anchoring a 5,419-type
# run to a 16,110-type constant is the trap that failed §1905's pred_d and cost §1882 a launch.
#   pred_a THE LEVER STILL WORKS: the rank-512 uncovered gradient exceeds §1922's PUBLISHED rank-64
#          +3.28 / +3.11 / +2.10 by at least 0.3 on all three roles. The bar is 0.3 rather than §1919's
#          0.5 because §1920 measured this gradient's seed spread at 0.202-0.319 and §1922's uncovered n
#          is 2.4x larger here, so 0.3 is roughly one spread -- a detection threshold, and I am NOT going
#          to name this prediction as though it were an explanation, which is what I got wrong at §1919.
#   pred_b AND THE COVERED ARM IS UNTOUCHED: the covered gradient is within 0.5 of §1922's PUBLISHED
#          +9.69 / +10.46 / +10.17. §1870's map writes only uncovered rows, so a covered change means the
#          build is miswired. The CE version of this control has returned exactly 0.00e+00 fifteen times.
#   pred_c AND §1765 HOLDS A FOURTH TIME: within-token margin spread below 1e-3 on both arms. It was
#          exactly 0.00e+00 at 5,419 (§1922) and 9,054 (§1923), and <= 2.86e-05 at 16,110.
#   pred_d CONTROLS: coverage is exactly 5,419; zero lookup misses across the whole vocabulary; and the
#          uncovered share is REPORTED and should be ~24%, matching §1922 rather than §1919's ~10%.
...
# THE LEVER: full table rank means program_rows(None) reads MAPRANK_OF[None]. §1922 measured this
# build at 64; this run raises it to 512, exactly as §1919 did at 16,110.
MAPRANK_OF = {None: 512, 512: 512}   # §1880/§1881: map_rank >= table_rank + 1
RIDGE = 1e-2
PT = '/workspace/tensor_language/basis_aligned/bilinear_quotient/'
OUT = PT + 'ops/rank_lever_at_deployed_results.json'
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'RANK LEVER AT DEPLOYED COVERAGE 5419 | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    S1922_UNC = {'skip7000': 3.28, 'skip11000': 3.11, 'skip1200': 2.10}
    S1922_COV = {'skip7000': 9.69, 'skip11000': 10.46, 'skip1200': 10.17}
    S1919_GAIN = {'skip7000': 0.78, 'skip11000': 0.63, 'skip1200': 0.70}   # §1919, at 16,110
    uncn = {e: sum(u(e, i8, 'n') for i8 in range(4)) for e in roles}
    totn = {e: uncn[e] + sum(q(e, 'tok', i8, 'n') for i8 in range(4)) for e in roles}
    gain = {e: gunc[e] - S1922_UNC[e] for e in roles}
    pa = all(gain[e] >= 0.3 for e in roles)
    pb = all(abs(gcov[e] - S1922_COV[e]) <= 0.5 for e in roles)
    pc = all(2.0 * dev[e] < 1e-3 for e in roles)
    pd = (ncov == NCOV and all(miss[e] == 0 for e in roles))

    print(f'\n  rank-512 fallback at DEPLOYED coverage {ncov}:', flush=True)
    for e in roles:
        print(f'    {e:10s} COVERED   grad {gcov[e]:+.2f}   (§1922 rank-64 {S1922_COV[e]:+.2f})',
              flush=True)
        print(f'    {"":10s} UNCOVERED grad {gunc[e]:+.2f}   (§1922 rank-64 {S1922_UNC[e]:+.2f})  '
              f'gain {gain[e]:+.2f}   (§1919 gain at 16,110 {S1919_GAIN[e]:+.2f})   n {uncn[e]}',
              flush=True)
    print(f'\n  THE LEVER still works (uncovered gain >= 0.3) -> {pa}  ' + '  '.join(
        f'{e} {gain[e]:+.2f}' for e in roles), flush=True)
    print(f'  and the COVERED arm is untouched (within 0.5) -> {pb}  ' + '  '.join(
        f'{e} {gcov[e]:+.2f} vs {S1922_COV[e]:+.2f}' for e in roles), flush=True)
    print(f'  and §1765 holds a FOURTH time (< 1e-3) -> {pc}  ' + '  '.join(
        f'{e} <= {2.0 * dev[e]:.2e}' for e in roles), flush=True)
    print(f'  coverage {ncov}, zero misses -> control {pd}; uncovered share ' + '  '.join(
        f'{e} {uncn[e] / max(totn[e], 1):.1%}' for e in roles) + '  (§1922 ~24%)', flush=True)
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_lever_still_works': bool(pa),
                          'pred_b_covered_untouched': bool(pb),
                          'pred_c_s1765_fourth_time': bool(pc),
