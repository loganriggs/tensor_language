...
"""V4_FIRST_BLOCK_JOIN_V1_PREREGISTRATION.md; unchanged oracle source edits."""
import os,sys,json,time,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';PLAN=dict(prefix=12,native_suffix=16,joined_suffix=16)
def main():
 ...
 out=A/'v4_first_block_join_v1_result.json';assert not out.exists();choicepath=P/'V4_FINITE_REFERENCE_CHOICES_V1.json';choices=json.loads(choicepath.read_text());prior=json.loads((A/'v4_finite_reference_v1_result.json').read_text());contexts=[];composition=json.loads((A/'v4_selective_composition_v1_result.json').read_text());field_errors=[];programs=[]
 ...
 for context in contexts:
  ...
  for family in dict.fromkeys(families):
   ...
   for boundary,estimate in joins.items():
    ...
    records.append(dict(panel=context['panel'],family=family,boundary=boundary,remaining_blocks=6 if boundary==10.5 else 17-boundary,first_mlp=boundary==10.5,errors=errors.tolist(),weak_norm=float(weak),passed=bool(errors[0]<=.1 and errors[1:].max()<=.05)))
 instrument=counts==PLAN and max(replays)<=1e-8 and max(field_errors)<=.001
 passing=[boundary for boundary in [10,10.5,11,17] if all(r['passed'] for r in records if r['boundary']==boundary)]
 result=dict(plan=PLAN,counts=counts,max_replay=max(replays),max_field_check=max(field_errors),records=records,passing_boundaries=passing,predictions=dict(pred_a_instrument=instrument,pred_b_post_attention_join=instrument and 10.5 in passing,pred_c_nontrivial_suffix=instrument and any(b>10 for b in passing)),seconds=time.perf_counter()-tic)
