#!/usr/bin/env python
"""late_tail_linear_read_spectrum_probe -- is the LINEAR quarter of the late tail write LOW-RANK (a cheap, installable correction of the
core) or spread? §2802: at the final readout, a ridge-linear read W (768 core coords -> 384 tail coords) recovers 24% of the late tail
channel's .1130 and 25% of its variance; the other three quarters are context-computed by the late MLPs. This rung takes W's singular value
decomposition and replaces A at the readout by its rank-r linear image for r in (1, 2, 4, 8, 16, 32, 64, 128, 768), pricing each on
held-out docs 0-63 per token, per position band and per target class; it also asks whether CE recovery tracks variance recovery (is
variance the right currency for the tail?), whether W's top tail direction is the §2801 frequency direction, and whether the core's
linear predictability of the tail falls with context length. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_rank8_keeps_half_of_the_linear_read pred_c_top_tail_direction_is_the_frequency_direction
#                     pred_d_ce_recovery_tracks_variance_recovery pred_e_linear_predictability_falls_with_position pred_f_rank64_reaches_90pct
...
Preregistration: polynomial_causal/LATE_TAIL_LINEAR_READ_SPECTRUM_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_tail_linear_read_spectrum_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R
from target_token_classes import target_token_classes

if not torch.cuda.is_available():
    raise RuntimeError("late_tail_linear_read_spectrum_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_TAIL_LINEAR_READ_SPECTRUM_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_tail_token_table_probe_results.json"   # §2802
OUT = ROOT / "late_tail_linear_read_spectrum_probe_results.json"
HASHES = {PREREG: "b76b5c3009628b7bcc769ef4585db3f1e08ef347ce3f97d1c3258758dfcd34c1", PRIOR: "fc8c5715f9e4afcabe6292f2e11840aa4086ec1cc72f7552e4629574cea9284a",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_tail_linear_read_spectrum_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_SPLIT8_1024 = 0.0374; PRIOR_LATE_MLP_768 = 0.1249; PRIOR_EARLY_TAIL_ONLY = 0.0711; PRIOR_ALL_TAILOUT = 0.1459; PRIOR_READERS_ONLY = 0.0404; PRIOR_FINAL_ONLY = 0.1130; RIDGE = 1e-2; PRIOR_LIN_CORE = 0.0854; RANKS = (1, 2, 4, 8, 16, 32, 64, 128, 768); PB = 32; LAM = 1e-2
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_half": 0.5, "c_cos": 0.8, "d_gap": 0.05, "e_drop": 0.05, "f_share": 0.9}
NULLS = {"b_quarter": 0.25, "c_cos": 0.3, "d_gap": 0.10, "e_flat": 0.02, "f_share": 0.75}
...
def main():
    ...
    a_mean = a_f.mean(0)
    # --- the ridge-linear read core -> tail (§2802's LIN_CORE) and its singular-value spectrum ---
    cm = c_f.mean(0); am = a_mean; Xc = c_f - cm; Yc = a_f - am; G = Xc.T @ Xc; lam = RIDGE * G.diagonal().mean()
    W = torch.linalg.solve(G + lam * torch.eye(768, dtype=G.dtype, device=DEV), Xc.T @ Yc)          # 768 x 384, core coords -> tail coords
    UW, sW, VhW = torch.linalg.svd(W, full_matrices=False); VW = VhW.T                                  # UW: core directions, VW: tail directions
    def W_r(r):
        return (UW[:, :r] * sW[:r]) @ VhW[:r]
    r2_fit = {r: 1.0 - float(((Xc @ W_r(r) - Yc) ** 2).sum() / (Yc ** 2).sum()) for r in RANKS}
    # explained-variance share of each truncation IN THE FIT (fraction of the full linear fit's explained variance) and singular energy share
    e_share = {r: (r2_fit[r] / r2_fit[RANKS[-1]]) if r2_fit[RANKS[-1]] > 0 else None for r in RANKS}
    s_energy = {r: float((sW[:r] ** 2).sum() / (sW ** 2).sum()) for r in RANKS}
    # the frequency direction d_f (§2801) and the top unembed tail direction, for comparison with W's top tail direction VW[:, 0]
    cnt = torch.bincount(fit_rows[:, 1:TI + 1].reshape(-1), minlength=V).double(); u = torch.log1p(cnt); u = u - u.mean(); u = u / u.norm()
    d_f = WU.T @ u; d_f = d_f / d_f.norm()
    cos_top = {"cos_VW1_df": float((VW[:, 0] @ d_f).abs()), "cos_VW1_WU1": float((VW[:, 0].float() @ VWU[:, 0]).abs()),
               "cos_VW2_WU2": float((VW[:, 1].float() @ VWU[:, 1]).abs())}
    fitfacts = {"n_fit_tokens": int(n_f), "r2_fit_by_rank": r2_fit, "fit_explained_share_by_rank": e_share, "singular_energy_share_by_rank": s_energy,
                "sW_top8": [float(v) for v in sW[:8]], "tail_rms": float(a_f.pow(2).sum(1).mean().sqrt())}
    log(stage="fit_models", **{f"r2_fit_{r}": round(v, 4) for r, v in r2_fit.items()}, **{k: (round(v, 4) if v is not None else None) for k, v in cos_top.items()})
    # --- predictors: rank-r linear reads evaluated at the final site on eval docs; out-of-sample R² accumulated per POSITION inside the arm ---
    class CFinalPred(CFinal):
        """Readout sees x - A + ((x·Uk - cm) W_r + am) Utᵀ: the late tail write replaced by its rank-r linear image of the core."""
        def __init__(self, box, Wr, acc): super().__init__(box); self.Wr = Wr; self.acc = acc
        def hook(self, idx): self.idx = idx
        def __call__(self, x, _):
            if self.box.A is None:
                return x
            B_, T_ = x.shape[0], x.shape[1]
            a = (self.box.A @ Ut).reshape(B_, T_, 384).double(); p = ((x @ Uk).double() - cm) @ self.Wr + am
            self.acc["ss"] += ((p - a) ** 2).sum((0, 2)); self.acc["st"] += ((a - a_mean) ** 2).sum((0, 2))
            return x - self.box.A + p.float() @ Ut.T
    preds_W = {f"LIN_{r}": W_r(r) for r in RANKS}
    ...
    def final_tok(name=None):
        ...
        acc = {"ss": torch.zeros(TI, dtype=torch.float64, device=DEV), "st": torch.zeros(TI, dtype=torch.float64, device=DEV)}
        patch[("final", -1)] = CFinalPred(box, preds_W[name], acc)
        out_ = tok_loss(patch, extra=(box,))
        r2_out[name] = 1.0 - float(acc["ss"].sum() / acc["st"].sum()) if float(acc["st"].sum()) > 0 else None
        r2_pos[name] = [1.0 - float(acc["ss"][b:b + PB].sum() / acc["st"][b:b + PB].sum()) for b in range(0, TI - 1, PB)]
        return out_
    arms = {"SPLIT8_1024": split8(1024)}
    base = tok_loss()
    tok = {"FINAL_ONLY": final_tok()}; tok.update({k: final_tok(k) for k in preds_W})
    ...
    F_ = arms["FINAL_ONLY"]; FULL = f"LIN_{RANKS[-1]}"
    rec_ = {k: 1.0 - sdiv(arms[k], F_) for k in preds_W}
    rec_full_pos = max(rec_[FULL], 0.0) if rec_[FULL] is not None else 0.0
    share_ = {k: sdiv(rec_[k], rec_[FULL]) for k in preds_W}
    ...
    gap_ = {k: (abs(rec_[k] - r2_out[k]) if (rec_[k] is not None and r2_out[k] is not None) else None) for k in preds_W}
    gap_max_ge8 = max([v for k, v in gap_.items() if v is not None and int(k.split("_")[1]) >= 8], default=None)
    pos_full = r2_pos[FULL]; pos_drop = pos_full[0] - pos_full[-1]
    summ = {"recovered_fraction_of_readout_consumption": rec_, "share_of_full_linear_read": share_, "r2_out_of_sample": r2_out, "r2_out_by_position_band": r2_pos,
            "position_band": PB, "r2_first_minus_last_band_full": pos_drop, "ce_minus_variance_recovery_abs": gap_, "max_gap_rank_ge_8": gap_max_ge8,
            "fit": fitfacts, "top_direction_cosines": cos_top, "rank_reaching_90pct": next((r for r in RANKS if share_[f"LIN_{r}"] is not None and share_[f"LIN_{r}"] >= 0.9), None),
            "per_token_by_class": {k: by_class(delta[k]) for k in ("FINAL_ONLY", "LIN_1", "LIN_8", FULL)},
            "novel_share_of_recovery": {k: novel_share(k) for k in preds_W}, "prior_final_only_2797": PRIOR_FINAL_ONLY, "prior_lin_core_2802": PRIOR_LIN_CORE}
    def ge(v, b):
        return bool(v is not None and v >= b)
    def le(v, b):
        return bool(v is not None and v <= b)
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(F_ - PRIOR_FINAL_ONLY) <= BARS["repro_tol"] and abs(arms[FULL] - PRIOR_LIN_CORE) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_rank8_keeps_half_of_the_linear_read': bool(rec_["LIN_8"] is not None and rec_["LIN_8"] - BARS["b_half"] * rec_full_pos >= 0.0),
        'pred_c_top_tail_direction_is_the_frequency_direction': ge(cos_top["cos_VW1_df"], BARS["c_cos"]),
        'pred_d_ce_recovery_tracks_variance_recovery': bool(gap_max_ge8 is not None and gap_max_ge8 <= BARS["d_gap"]),
        'pred_e_linear_predictability_falls_with_position': ge(pos_drop, BARS["e_drop"]),
        'pred_f_rank64_reaches_90pct': bool(rec_["LIN_64"] is not None and rec_["LIN_64"] - BARS["f_share"] * rec_full_pos >= 0.0),
    }
    nulls = {"b_null_rank8_le_quarter": bool(rec_["LIN_8"] is not None and rec_["LIN_8"] - NULLS["b_quarter"] * rec_full_pos <= 0.0),
             "c_null_cos_le_.3": le(cos_top["cos_VW1_df"], NULLS["c_cos"]),
             "d_null_gap_gt_.10": bool(gap_max_ge8 is not None and gap_max_ge8 > NULLS["d_gap"]),
             "e_null_flat": bool(abs(pos_drop) <= NULLS["e_flat"]),
             "f_null_rank64_below_75pct": bool(rec_["LIN_64"] is not None and rec_["LIN_64"] - NULLS["f_share"] * rec_full_pos < 0.0)}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "split": SPLIT, "ridge": RIDGE, "ranks": list(RANKS),
           "program": "the §2797 consumer instrument (late-MLP centred tail writes A withheld from the FINAL READOUT) with A REPLACED at the readout by the §2802 ridge-linear read of the final residual's 768 core coordinates TRUNCATED to its top-r singular pairs, r in RANKS; out-of-sample R² per position band inside each arm; per-token deltas by target class; W's top tail direction compared with the §2801 frequency direction d_f and the unembedding's top tail direction",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "final_only": F_, "prior_final_only": PRIOR_FINAL_ONLY, "lin_full": arms[FULL], "prior_lin_core": PRIOR_LIN_CORE, "tok_mean_baseline": float(base.mean())},
