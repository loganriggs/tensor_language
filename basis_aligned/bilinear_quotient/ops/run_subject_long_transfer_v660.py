...
48 newly frozen longer-position sequences, eight prefix calls and twenty-four suffix calls, zero fits.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v660_result.json'
...
def main():
    ...
    def suffix(raw,x0,first,pos,answers,frozen=None,mode="freeze"):
        ...
        if frozen is None:context['mlp_contexts']=[]
    ...
    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,cells=cells,
                ...
                scope='48 newly frozen longer-structure rows; original calibration and v644 nouns disjoint. Producer/basis fixed from v659, contexts generated natively for these new inputs. Six reduced MLP stages and folded attention12, no refit or row selection. Eight template-number cells. Historical vocabulary reuse not excluded. Selectivity, cross-task reuse and port-closed simplicity unestablished.')
