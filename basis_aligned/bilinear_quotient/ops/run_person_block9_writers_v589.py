#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_a_block_9_head_carries pred_c_block_9_singles_add pred_d_9_6_writes_person pred_e_at_most_two_heads_matter
"""Block 9's writers into the answer, person line (v589). v588: block 9 writes 0.114 of the myself - yourself gap straight into the answer, the largest block
outside the four named readers. Here block 9's nine heads singly (writes into the answer). 9.6 is the number / gender pronoun reader; 9.1 / 9.4 are the
temporal readers; 9.7 the verb-agreement reader.
PREDICTIONS (scored as written; failures preserved; priors from v588 and the readout atlas)
    pred_a_baseline_replays        the unedited manual forward replays the model's myself - yourself margins within 1e-3
    pred_b_a_block_9_head_carries  the largest single closes >= 0.05
    pred_c_block_9_singles_add     the nine singles sum to within 0.03 of 0.114
    pred_d_9_6_writes_person       9.6 closes >= 0.03 (the pronoun reader reads person too). Prior: unsure
    pred_e_at_most_two_heads_matter  at most two heads close >= 0.03
...
OUT = ROOT / "circuits/followups/person_block9_writers_v589_result.json"
CANDIDATE_ID = "person.block9_writers_v589"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, TOP_MIN, B9_V588, ADD, H96_MIN, MATTER, UNIT = 1e-3, 0.05, 0.114, 0.03, 0.03, 0.03, 701
...
FORWARDS_MAX = 21
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_a_block_9_head_carries": ">= 0.05", "pred_c_block_9_singles_add": "0.114 +- 0.03", "pred_d_9_6_writes_person": ">= 0.03", "pred_e_at_most_two_heads_matter": "<= 2 heads >= 0.03"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "top_min": TOP_MIN, "b9_v588": B9_V588, "add": ADD, "h96_min": H96_MIN, "matter": MATTER, "unit": UNIT}}
    ...
    SETS_W = {f"9.{h}": {9: [h]} for h in range(N_HEAD)}; MODES = {m: ({}, (0,)) for m in SETS_W}; forwards = 0
    def logits_margin(x):
    ...
    top = max(mc, key=lambda k: abs(mc[k])); print("block-9 heads into the answer", {k: round(v, 3) for k, v in mc.items()}, "sum", round(sum(mc.values()), 3), "top", top)
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_a_block_9_head_carries": abs(mc[top]) >= TOP_MIN, "pred_c_block_9_singles_add": abs(sum(mc.values()) - B9_V588) <= ADD,
                   "pred_d_9_6_writes_person": mc["9.6"] >= H96_MIN, "pred_e_at_most_two_heads_matter": sum(1 for v in mc.values() if v >= MATTER) <= 2}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "person_block9_writers_result_v589", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
