...
STEM='STRUCTURED_PRODUCER_CACHE_V1'
@torch.no_grad()
def main():
 ...
 try:
  for length,ids in batches:
   captured['group']=[];tokens=torch.tensor([rows[i]['ids'] for i in ids],device='cuda')
