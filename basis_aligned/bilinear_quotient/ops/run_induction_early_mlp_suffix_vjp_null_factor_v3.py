#!/usr/bin/env python3
# BQLANE: gpu
# BQGATE: 18forwards576seq3backwards; induction early-MLP suffix-VJP-null response factor;0updates.
"""Split the frozen early-MLP response into suffix-sensitive and suffix-null factors."""
...
p=prior.p
ROWS=prior.ROWS; PREREG=POLY/'INDUCTION_EARLY_MLP_SUFFIX_VJP_NULL_FACTOR_V3_PREREGISTRATION.md'; BINDING=POLY/'INDUCTION_EARLY_MLP_SUFFIX_VJP_NULL_FACTOR_V3_BINDING.json'; OUT=POLY/'INDUCTION_EARLY_MLP_SUFFIX_VJP_NULL_FACTOR_V3_RESULT.json'
MODULES=prior.CANDIDATE_GROUPS['early_mlp']; MODES=('self','joint','full','parallel','null'); BATCH=32
PREDICTION_REGISTRY={'pred_a_exact_instrument':None,'pred_b_suffix_null_answer_preserving':None,'pred_c_suffix_null_removes_collateral':None,'pred_d_vjp_null_partition':None}
def digest(path): return hashlib.sha256(Path(path).read_bytes()).hexdigest()
def clear_rotary_cache(model):
 for block in model.transformer.h:
  block.attn.rotary.seq_len_cached=None; block.attn.rotary.cos_cached=None; block.attn.rotary.sin_cached=None
def suffix_vjp(runtime,tokens,specs,target,answer_rows):
 torch=runtime.torch; device=next(runtime.model.parameters()).device; token_tensor=torch.as_tensor(tokens,dtype=torch.long,device=device); captured=[]; before=np.zeros((len(specs),3,p.RESIDUAL),np.float32); total=np.zeros_like(before); after=np.zeros_like(before); active=np.zeros(len(specs),bool)
 def attention(event):
  if event.site in (5,7,8):
   write,first,terms,_=runtime.r585.factorize_attention_event(event,specs,torch=torch,functional=runtime.functional,induction=runtime.induction); changed=write.clone(); indices=[i for i,s in enumerate(p.SITES) if int(s[1])==event.site]; li=(5,7,8).index(event.site)
   for local,spec in enumerate(specs):
    query=int(spec['final_position']); pieces=[np.subtract(target[local,i],terms[local][p.SITES[i]]['term'].numpy(),dtype=np.float32) for i in indices]; delta=torch.as_tensor(np.stack(pieces),device=device).sum(0); before[local,li]=changed[local,query].float().detach().cpu().numpy(); total[local,li]=delta.float().detach().cpu().numpy(); changed[local,query]+=delta.to(changed.dtype); after[local,li]=changed[local,query].float().detach().cpu().numpy(); active[local]|=bool(delta.float().norm().detach().cpu()>1e-6)
   write=changed
  else: write,first=event.block.attn(event.state,event.first_value)
  return write,first
 def mlp(event):
  write=event.block.mlp(event.state)
  if f'mlp{event.site}' in MODULES:
   write=write.detach().requires_grad_(True); captured.append(write)
  return write
 with torch.enable_grad():
  logits=runtime.facade.forward_with_dispatch(runtime.model,token_tensor,attention,mlp,require_production=False); finals=torch.as_tensor([int(x['final_position']) for x in specs],device=device); answers=torch.as_tensor([int(x['recipient_answer_id']) for x in answer_rows],device=device); selected=logits[torch.arange(len(specs),device=device),finals]; loss=runtime.functional.cross_entropy(selected,answers,reduction='sum'); gradients=torch.autograd.grad(loss,captured)
 axes=[]
 for write,gradient in zip(captured,gradients):
  rows=[]
  for local,spec in enumerate(specs):
   g=gradient[local,int(spec['final_position'])].float(); rows.append(g/g.norm().clamp_min(1e-30))
  axes.append(torch.stack(rows))
 observation=observe(before,total,after); assert observation['passed']
 return selected.detach().float().cpu().numpy(),torch.stack(axes,dim=1).detach(),active,observation
def execute(runtime,tokens,specs,target,native_writes,mode,suffix_axes):
...
def main():
 binding=json.loads(BINDING.read_text()); assert all(digest(path)==expected for path,expected in binding['files'].items()); r585,frozen,rows,endpoints=prior.load_rows(); assert binding['price']=={'forwards':18,'sequences':576,'backwards':3}
 ...
 try:
  endpoint_logits,endpoint_terms,endpoint_writes=prior.capture_native(runtime,endpoints); clear_rotary_cache(runtime.model); endpoint_index={row['endpoint_id']:i for i,row in enumerate(endpoints)}; recipient=np.array([endpoint_index[row['recipient_endpoint_id']] for row in rows]); donor=np.array([endpoint_index[row['donor_endpoint_id']] for row in rows]); specs=[endpoints[i] for i in recipient]; tokens=p.fixed_tokens(specs,'token_ids'); native=endpoint_logits[recipient]; recipient_terms=endpoint_terms[recipient]; donor_terms=endpoint_terms[donor]; native_writes=endpoint_writes[recipient]; arms={}; active={}; restored={}; parnorm={}; nullnorm={}; partitions=[]; observations=[]; axis_batches=[]; joint_batches=[]; joint_active=[]
  for start in range(0,len(rows),BATCH):
   stop=start+BATCH; logits,axis,act,obs=suffix_vjp(runtime,tokens[start:stop],specs[start:stop],donor_terms[start:stop],rows[start:stop]); joint_batches.append(logits); axis_batches.append(axis); joint_active.append(act); observations.append(obs)
  arms['joint']=np.concatenate(joint_batches); active['joint']=np.concatenate(joint_active); suffix_axes=runtime.torch.cat(axis_batches).to(next(runtime.model.parameters()).device)
  for mode in ('self','full','parallel','null'):
   target=recipient_terms if mode=='self' else donor_terms; batches=[]; aa=[]; rr=[]; pp=[]; nn=[]
   for start in range(0,len(rows),BATCH):
    stop=start+BATCH; values=execute(runtime,tokens[start:stop],specs[start:stop],target[start:stop],native_writes[start:stop],mode,suffix_axes[start:stop]); logits,act,response,pn,nnorm,part,obs=values; batches.append(logits); aa.append(act); rr.append(response); pp.append(pn); nn.append(nnorm); partitions.append(part); observations.append(obs)
 ...
 predictions={'pred_a_exact_instrument':bool(instrument),'pred_b_suffix_null_answer_preserving':bool(instrument and pred_b),'pred_c_suffix_null_removes_collateral':bool(instrument and pred_c),'pred_d_vjp_null_partition':bool(instrument and pred_d)}; terminal='suffix_vjp_null_screen' if all(predictions.values()) else ('invalid' if not instrument else 'suffix_vjp_null_factor_null')
 result={'schema':'induction_early_mlp_suffix_vjp_null_factor_v3_result','terminal':terminal,'predictions':predictions,'instrument':{'self_native_max_abs':max_abs,'self_native_relative':relative,'maximum_partition_error':max(partitions),'active_fraction':{x:float(np.mean(active[x])) for x in ('joint','full','parallel','null')},'response_active_fraction':{x:float(np.mean(restored[x]>1e-6)) for x in ('full','parallel','null')},'maximum_rounding_residual':float(max(o['maximum_rounding_residual'] for o in observations))},'reports':reports,'decisions':decisions,'component_norms':{x:{'mean_parallel_norm':float(np.mean(parnorm[x])),'mean_null_norm':float(np.mean(nullnorm[x])),'mean_installed_norm':float(np.mean(restored[x]))} for x in ('full','parallel','null')},'price':{'forwards':counts[0],'sequences':counts[1],'fits':0,'backwards':3,'weight_updates':0},'claim_boundary':'Opened-row explanatory factorization of fixed early MLP8-12 native response using per-row suffix CE VJPs at the joint-edited trajectory; no extracted autonomous consumer or fresh confirmation.','rows_sha256':digest(ROWS),'binding_sha256':digest(BINDING),'runner_sha256':digest(RUNNER),'checkpoint_sha256':runtime.checkpoint_sha256,'wall_seconds':time.perf_counter()-tic}
