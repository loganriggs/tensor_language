"""tailE REBUILD CONTROL (rung 130): in-frame targets — isolating what exploded in rung 128.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2222: the trajectory-target tailE
rebuild landed at census +3.0503 (baseline +1.9474) — beyond both registered outcomes. This control runs the
IDENTICAL rebuild machinery with IN-FRAME targets (config-frame captures for both inputs and outputs;
tailE_traj off). K-0-style discrimination: if this lands near baseline, the trajectory TARGET (mixed frames
in the 8-dim span) was the destroyer; if it still explodes, the rebuild machinery itself is buggy.

REGISTERED PREDICTIONS (anchors: rung-127 receipt +1.9474 census / +1.8765 L2_F / +0.3256 increment):
  (a) NEAR BASELINE (target was the destroyer): census agg <= 2.05.
  (b) L2_F fresh <= 1.95.
  (c) increment in [0.20, 0.70] AND census >= 1.4.
NULL: still exploded (census >= 2.6) — the rebuild machinery is buggy; a code audit follows before any
further tailE work. PRICE: none. Tripwire: INSTRUMENT FAIL if L2CF missing. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_taile_rebuild_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: tailE rebuild control (in-frame targets)')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_taile_ctrl_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['tailE_rebuild']=True
    print('ARM: rung-127 build + tailE rebuilt IN-FRAME (control for S2222)',flush=True)
    main()
    ...
    print(f"(a) census {agg:+.4f} <= 2.05: {'HELD' if pa else 'FAILED'}")
    print(f"(b) L2F {L2F:+.4f} <= 1.95: {'HELD' if pb else 'FAILED'}")
    print(f"(c) increment in [0.20, 0.70] and census >= 1.4: {'HELD' if pc else 'FAILED'}")
