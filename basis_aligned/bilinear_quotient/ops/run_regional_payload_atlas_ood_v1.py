#!/usr/bin/env python3
# BQGATE:10 body forwards;48 sequences lengths6-14;40 suffix arms;no fit;180sec.
"""pred_a projected residual/child/full-donor replay<=1e-5;
pred_b all-payload transplant>=10%native and>=4/6positive EACHfamily;
pred_c fixed A8/A9/A13 joint transfer>=40%all-payload, unrelated<=.5regional EACHfamily.
Null: a producer group selected on the original templates may fail to generalize.
"""
...
STEM='REGIONAL_PAYLOAD_ATLAS_OOD_V1'
@torch.no_grad()
def main():
    ...
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('10 body forwards48sequences6-14tokens;40suffix arms;no fitting');return
    ...
    pre=torch.empty(48,1152,device='cuda');writes=torch.zeros(48,39,1152,dtype=torch.float64,device='cuda');replays=[];count=0
    ...
    assert count==10
    joint=writes[:,0]+(writes[:,[10,11,15]]-writes[:,0,None]).sum(1)
    writes=torch.cat([writes,joint[:,None]],1)
    def margins(delta):
    ...
    prior=torch.load(P/'REGIONAL_FACTOR_INTERCHANGE_OOD_V1_ARTIFACT.pt',weights_only=True,map_location='cpu')['write_vertices'].cuda()
    recipient_error=float((writes[:,0]-prior[:,0]).norm()/prior[:,0].norm());donor_error=float((writes[:,38]-prior[:,4]).norm()/prior[:,4].norm())
    arms=torch.stack([margins(writes[:,j]-writes[:,0]) for j in range(40)]);cells=[]
    for family in (0,1,2,3):
        ...
        cells.append(dict(family=family,native_regional_gap=float(native[:,0].mean()),mean_transfer_by_arm=transfer.mean(1).tolist(),positive_regional_by_arm=(transfer[:,:,0]>0).sum(1).tolist(),group_regional_meanabs=float(transfer[39,:,0].abs().mean()),group_unrelated_meanabs=float(transfer[39,:,1].abs().mean())))
    instrument=max(producer_errors+child_errors+replays+[donor_error,recipient_error])<=1e-5;full=instrument and all(c['mean_transfer_by_arm'][38][0]>=.1*c['native_regional_gap'] and c['positive_regional_by_arm'][38]>=4 for c in cells)
    result={'pred_a':instrument,'pred_b':full,'pred_c':full and all(c['mean_transfer_by_arm'][39][0]>=.4*c['mean_transfer_by_arm'][38][0] and c['positive_regional_by_arm'][39]>=4 and c['group_unrelated_meanabs']<=.5*c['group_regional_meanabs'] for c in cells)}
    torch.save(dict(pre=pre.cpu(),write_vertices=writes.cpu(),arm_margins=arms.cpu()),art)
    result.update(recipient_replay_error=recipient_error,all_payload_donor_replay_error=donor_error,residual_unroll_errors=producer_errors,child_decomposition_errors=child_errors,native_attention_replay_errors=replays,cells=cells,arm_names=['native','embedding_numerator']+['attention_'+str(j) for j in range(17)]+['MLP_'+str(j) for j in range(17)]+['RMS_divisor','first_stream','all_payload_ports','joint_attention8_9_13'],body_forwards=count,seconds=time.perf_counter()-tic,artifact_sha=digest(art),source_shas=binding,scope='Exact conditional residual-write atlas for frozen regional payload. Isolated producer swaps retain recipient RMS and all downstream gates. Module labels are attribution ports, not identified semantic circuits.')
