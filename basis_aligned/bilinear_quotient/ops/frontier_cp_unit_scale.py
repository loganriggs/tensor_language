"""DOES THE MISMATCH REACH A BLOCK THAT IS NOT RIDGE-FITTED? SCALING THE CP UNITS (Claude, LANE 1)

SS2900 established that the local/end-to-end objective mismatch is **procedural**: scaling still helps after the front token table is
dropped and the quadratic residual is refitted to cover for it (+0.0378 at scale .9). It called the mismatch "a property of the ridge
fitting procedure itself".

**The CP units test that phrasing, because they are not ridge-fitted.** `c4`-`c9` are norm-selected CP factors -- the norm-2304
construction SS2118/SS2125 spent rungs on -- not least-squares solutions. If the mismatch really is about *ridge fitting*, scaling them
should NOT help. If it is about the more general fact that every component is chosen by a LOCAL criterion while the frontier is scored
END-TO-END, it should.

A `cp` entry is `('cp', li, Lk, Rk, Dk, db)` with forward `((x@Lk.T)*(x@Rk.T))@Dk.T + db`, so multiplying `Dk` scales the whole
quadratic reconstruction while leaving the bias -- the exact analogue of the `A` scaling SS2895/SS2900 applied to the front tables.

SS2883 supplies a same-operation anchor: it measured removing `c4`-`c9` from the evaluated config at an error share of **-0.2140**,
i.e. a COST of **+0.2140** relative to the frontier -- note the sign, since restoring the real MLPs there made the frontier WORSE. This
rung includes that arm.
...
SS2125 STANDS -- and note this rung does not select, reorder or re-derive anything, it rescales an already-fitted reconstruction.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_the_cp_drop_arm_reproduces_S2883
#                     pred_c_scaling_the_cp_units_improves_the_frontier
#                     pred_d_the_arms_are_connected
#                     pred_e_both_windows_are_reported

Preregistration: polynomial_causal/FRONTIER_CP_UNIT_SCALE_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run: baseline, CP dropped, and five Dk scales")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_cp_unit_scale_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_CP_UNIT_SCALE_PREREGISTRATION.md'
PREREG_SHA = "cab5f8807241bc14b54647e37b4f2cdf30506e6eb58129808bfe0ca6492de1a6"
...
SCALES = [0.5, 0.75, 0.9, 1.1, 1.25]
BARS = {"reproduce": 0.05, "anchor": 0.02, "improves": 0.0, "connected": 0.005}
NULLS = {"anchor_ge": 0.05, "no_improvement": 0.0}
...
if __name__=='__main__':
    ...
    if _sha(S2883) != S2883_SHA:
        raise RuntimeError(f'frozen hash mismatch: {S2883}')
    a83 = json.load(open(S2883))['summary']['error_share_cp_units_nats']   # -0.2140 (restoring real MLPs 4-9)
    SEL['mode']='norm'

    sk = lambda s: f'D{int(round(s*100)):03d}'
    ARMS[:] = ([('baseline', {}), ('cp_dropped', {'drop': frozenset(['c'])})]
               + [(sk(s), {'cp_scale': s}) for s in SCALES])
    ...
    c_drop = round(out['cp_dropped']['L2_F'] - bF, 4)
    cF = {s: round(out[sk(s)]['L2_F'] - bF, 4) for s in SCALES}
    cC = {s: round(out[sk(s)]['L2_C'] - bC, 4) for s in SCALES}
    # SS2883's share is baseline - restored, so its cost equivalent is the negative of that share
    anchor_target = -a83
    anchor_dev = abs(c_drop - anchor_target)
    below = [s for s in SCALES if cF[s] < 0]
    best_s = min(SCALES, key=lambda s: cF[s])

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = anchor_dev <= BARS['anchor']
    pc = len(below) >= 1
    pd = abs(c_drop) >= BARS['connected']
    pe = True
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_cp_drop_arm_reproduces_S2883':bool(pb),
           'pred_c_scaling_the_cp_units_improves_the_frontier':bool(pc),
           'pred_d_the_arms_are_connected':bool(pd),
           'pred_e_both_windows_are_reported':bool(pe)}
    nulls={'b_null_the_anchor_fails':bool(anchor_dev>=NULLS['anchor_ge']),
           'c_null_the_mismatch_is_confined_to_ridge_fits':bool(len(below)==NULLS['no_improvement'])}
    res={'rung':'frontier_cp_unit_scale','preds':preds,'nulls':nulls,
         ...
         'summary':{'L2_F_baseline':round(bF,4),'L2_C_baseline':round(bC,4),
                    'cost_cp_dropped':c_drop,'S2883_share':round(a83,4),
                    'anchor_target_as_cost':round(anchor_target,4),
                    ...
                    'scales_that_improve':below,'best_scale':best_s,
                    'best_cost_fresh':cF[best_s],'best_cost_fit':cC[best_s],
                    'S2896_tail_gain_for_reference':-0.2287,
                    'implied_L2_F_best':round(bF + cF[best_s], 4)},
         ...
         'hashes':{PREREG:PREREG_SHA,S2883:S2883_SHA},'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {bF:.4f} vs 2.6735: {'HELD' if pa else 'FAILED'}")
    print(f"(b) cp dropped {c_drop:+.4f} vs SS2883-implied {anchor_target:+.4f}, dev {anchor_dev:.4f}: {'HELD' if pb else 'FAILED'}")
    print("    fresh: " + "  ".join(f"s={s}:{cF[s]:+.4f}" for s in SCALES))
    print("    fit:   " + "  ".join(f"s={s}:{cC[s]:+.4f}" for s in SCALES))
    print(f"(c) scales improving: {below}: {'HELD' if pc else 'FAILED'}")
