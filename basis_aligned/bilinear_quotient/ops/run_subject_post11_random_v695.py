...
"""Post11 per-token norm-matched random controls on opened v691 rows.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v695_result.json'
PREDICTIONS=dict(pred_a_exact_replay='original instrument plus per-token random normmatch<=1e-5 and64calls',
 pred_b_effect_prediction='pooled native A/B effect norm / median matchedrandom number effect norm>=5',
 pred_c_capable_position_transfer='every matchedrandom cell/seed numbererror<=.10 andmodalerror<=.05 ofsource effect norm')


def main():
    plan=dict(prefix_calls=8,suffix_calls=56,sequences_per_arm=48,fits=0,backwards=0,matched_post11_suffix_calls=64,matched_reduced_calls=64,matched_seeds=list(range(69500,69508)),extra_attention_pullbacks=8,extra_reference_forwards=16,extra_token_local_mlp_pullbacks=24,extra_fulltoken_mlp_pullbacks=24,analytic_reverse_passes=4,attention_pullbacks=24,reference_attention_forwards=48,additional_tangent_suffix_calls=8,additional_initial_interface_suffix_calls=12,
    ...
    matched_calls=0;matched_norm_checks=[]
    matched_reports={name:{str(seed):{} for seed in range(69500,69508)} for name in ['A','B']}
    optimized_checks=[]
    ...
    for entries in batches:
        ...
        for source_name,delta in [('A',source_deltas[0]),('B',source_deltas[1]),('sum',source_deltas[0]+source_deltas[1])]:
            ...
            if source_name in ['A','B']:
                for seed in range(69500,69508):
                    rng=torch.Generator(device='cuda').manual_seed(seed)
                    rd=torch.randn(delta.shape,device='cuda',generator=rng)
                    desired=delta.norm(dim=-1,keepdim=True)
                    rd=rd/rd.norm(dim=-1,keepdim=True)*desired
                    matched_norm_checks.append(float(((rd.norm(dim=-1,keepdim=True)-desired).abs()/desired.clamp_min(1e-12)).max()))
                    actual=native_selected(context['baseline_after11']+rd,x0,first,readpos,start_layer=12);matched_calls+=1
                    reffect=base-(actual[:,0]-actual[:,1])*direction
                    rmodal=((cb-actual[:,2:])[:,0::2]-(cb-actual[:,2:])[:,1::2])
                    rl,_=run_dag(sparse_runtime,context['all_attention'],rd.double()@Q,context['all_mlp_contexts'],case['final_context'],readpos)
                    peffect=base-(rl[:,0]-rl[:,1])*direction
                    pmodal=-(qp*rd.double()).sum((-1,-2)).T
                    for i,e in enumerate(entries):
                        mc=matched_reports[source_name][str(seed)].setdefault(e[3],dict(target=[],prediction=[],native_modal=[],predicted_modal=[]))
                        mc['target'].append(float(reffect[i]));mc['prediction'].append(float(peffect[i]))
                        mc['native_modal'].append(rmodal[i].tolist());mc['predicted_modal'].append(pmodal[i].tolist())
    ...
    matched_specificity={};matched_cell_specificity={}
    for name,seed_reports in matched_reports.items():
        source_norm=np.linalg.norm([v for cell in reuse_reports[name].values() for v in cell['target']])
        random_norms=[]
        for report in seed_reports.values():
            random_norms.append(float(np.linalg.norm([v for cell in report.values() for v in cell['target']])))
            for family,cell in report.items():
                den=max(np.linalg.norm(reuse_reports[name][family]['target']),1e-20)
                cell['number_error_over_source']=float(np.linalg.norm(np.array(cell['prediction'])-cell['target'])/den)
                cell['modal_error_over_source']=(np.linalg.norm(np.array(cell['predicted_modal'])-cell['native_modal'],axis=0)/den).tolist()
        matched_specificity[name]=dict(source_norm=float(source_norm),random_norms=random_norms,ratio=float(source_norm/max(np.median(random_norms),1e-20)))
        matched_cell_specificity[name]={family:float(np.linalg.norm(cell['target'])/max(np.median([np.linalg.norm(report[family]['target']) for report in seed_reports.values()]),1e-20)) for family,cell in reuse_reports[name].items()}
    a=a and matched_calls==64 and max(matched_norm_checks)<=1e-5
    b=a and all(item['ratio']>=5 for item in matched_specificity.values())
    c=a and all(cell['number_error_over_source']<=.10 and max(cell['modal_error_over_source'])<=.05 for seed_reports in matched_reports.values() for report in seed_reports.values() for cell in report.values())
    failed=[]

    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,cells=cells,
                predictions=dict(zip(PREDICTIONS,[bool(a),bool(b),bool(c)])),selectivity=selectivity,matched_reports=matched_reports,matched_specificity=matched_specificity,matched_cell_specificity=matched_cell_specificity,matched_calls=matched_calls,max_matched_norm_error=max(matched_norm_checks),max_optimized_reader_difference=max(optimized_checks),hybrid_reports=hybrid_reports,tangent_reports=tangent_reports,max_tangent_relative_check=max(tangent_checks),reader_values_per_batch=reader_prices,tangent_suffix_calls=tangent_calls,initial_reports=initial_reports,initial_interface_suffix_calls=initial_counts,max_full_state_readout_error=max(oracle_checks),failed_modal_oracle_ratios=[o/e for o,e in failed],reuse_reports=reuse_reports,cross_effects=cross,reuse_counts=reuse_counts,pre11_joint_vs_post11_sum_gap=boundary_gaps,
                ...
                scope='Matched post11 random-control screen for sourceA/B, per-token norm/support matched,8fixed seeds,48opened v691 rows. Sparse-number and selectedmodal predictor frozen. All native background/source/reader generators external. Native specificity and control prediction are separate gates; no token-input closure or model adoption claim.')
