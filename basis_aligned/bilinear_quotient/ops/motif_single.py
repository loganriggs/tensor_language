"""THE DEPLOYED MOTIF GRAMMAR, CIRCUIT-SCORED AT ITS OWN COMPONENTS (rung 157).

CONVENTION (S2135): per-position dCE = CE(single-site motif replacement, all else real) - CE(real model) on
the census rows. S2251: every POSITION-LOCAL rung fails breadth at the attention components - fairly, since
attention is non-local. The fair test is the frontier's OWN non-local grammar: motif heads (alpha.v_prev/
self) at block 8 alone (a8 owns 16 circuits) and block 3 alone (a3 owns 5), everything else real; breadth on
test-half members (>= 20; tau = 0.5 x battery ref).

REGISTERED PREDICTIONS:
  (a) a8-motif breadth >= 4 of 16.
  (b) a3-motif breadth >= 2 of 5.
  (c) SANITY/ANTI-INERT: agg(motif@8) in [0.005, 0.30] AND agg(motif@3) in [0.005, 0.60].
NULL: breadth 0 at both - the deployed grammar is circuit-unfaithful at its own components, explaining the
global 0/62 bottom-up. PRICE: none (the motifs are already the deployed replacement). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: deployed motif grammar, circuit-scored')
    raise SystemExit(0)
import torch
...
OUT=PT+'motif_single_results.json'
...
def main():
    ...
    if SEL.get('motif_single'):
        _ROWS=SEL.get('ext_rows',FR)
        cur['clsmap']=classify2(_ROWS).to(DEV)
        _arms={}
        for _mb in (8,3):
            _arms[f'motif_at_{_mb}']=evalV(_ROWS,_ROWS.shape[0],[],[_mb]).detach().cpu()
            print(f'  motif at block {_mb} done',flush=True)
...
if __name__=='__main__':
    ...
    SEL['skipset']=tuple(range(10,18)); SEL['motif_off']=tuple(range(2,10)); SEL['clsdmg']=False; SEL['ext_rows']=CROWS; SEL['motif_single']=True
    SEL['K']=4608; SEL['K69']=4608; SEL['K69MAP']={}
    print('ARMS: motif@8 and motif@3 single-site, circuit rows',flush=True)
    main()
    ...
    def breadth(k,comp):
        te=out[k]['test_members']
        own=[t for t,(mm,ref,top) in REFD.items() if top==comp]
        return sum(1 for t in own if t in te and te[t]<0.5*REFD[t][1]),len(own)
    b8,n8=breadth('motif_at_8','a8'); b3,n3=breadth('motif_at_3','a3')
    a8g=out['motif_at_8']['agg']; a3g=out['motif_at_3']['agg']
    pa=b8>=4
    pb=b3>=2
    pc=(0.005<=a8g<=0.30) and (0.005<=a3g<=0.60)
    res={'arms':out,'breadth_a8':[b8,n8],'breadth_a3':[b3,n3],
         'convention':'per-position dCE = CE(single-site motif replacement) - CE(real model) on census rows',
         'pred_a_a8_breadth':bool(pa),'pred_b_a3_breadth':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'a8-motif breadth {b8}/{n8}; a3-motif breadth {b3}/{n3}')
    print(f"(a) a8 breadth {b8} >= 4: {'HELD' if pa else 'FAILED'}")
    print(f"(b) a3 breadth {b3} >= 2: {'HELD' if pb else 'FAILED'}")
    print(f"(c) aggs sane ({a8g:+.4f}, {a3g:+.4f}): {'HELD' if pc else 'FAILED'}")
