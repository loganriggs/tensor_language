#!/usr/bin/env python
"""late_tail_writer_pair_coherence_probe -- WHY the ten single-writer tail drops sum to only .52 of the joint (§2793): every PAIR of late
writers 8..16 is dropped together (36 pairs) alongside the singles and the joint; under §2788's quadratic loss law the cross term
kappa_jk = (cost_jk - cost_j - cost_k) / (2 sqrt(cost_j cost_k)) is the pair's cosine in the loss metric. Tests whether coherence is
generic, whether it concentrates on ADJACENT writers (the re-write chain of §2791) or is flat (a shared direction), whether pairwise
terms reconstruct the joint, and whether loss-metric coherence tracks the writers' centred input-space cosine. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_pairs_generically_super_additive pred_c_coherence_falls_with_distance
#                     pred_d_adjacent_writers_coherent pred_e_pairwise_quadratic_law_reconstructs_the_joint pred_f_loss_coherence_tracks_input_cosine
...
Preregistration: polynomial_causal/LATE_TAIL_WRITER_PAIR_COHERENCE_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_tail_writer_pair_coherence_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_tail_writer_pair_coherence_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_TAIL_WRITER_PAIR_COHERENCE_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_tail_writer_identity_probe_results.json"   # §2793
OUT = ROOT / "late_tail_writer_pair_coherence_probe_results.json"
HASHES = {PREREG: "689fe3194e87b4f2b66f5b1eb031787c0e2511be9c52c87713f46e8033d32b31", PRIOR: "b2908e2a3ee7c3c34fdabbda4012cf84e96c7feeb290b51627e46ff2d8005ad8",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_tail_writer_pair_coherence_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_SPLIT8_1024 = 0.0374; PRIOR_LATE_MLP_768 = 0.1249; PRIOR_EARLY_TAIL_ONLY = 0.0711; PRIOR_W = {8: 0.00592, 9: 0.00425, 10: 0.00566, 11: 0.00508, 12: 0.00413, 13: 0.00577, 14: 0.00333, 15: 0.00163, 16: 0.0011}; LAM = 1e-2
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "single_tol": 0.003, "b_frac": 0.75, "c_rho": -0.4, "d_kappa": 0.20, "e_lo": 0.75, "e_hi": 1.33, "f_rho": 0.5}
NULLS = {"b_frac": 0.5, "c_rho": 0.0, "d_kappa": 0.05, "e_lo": 0.5, "e_hi": 2.0, "f_rho": 0.1}
...
def main():
    ...
    WR = list(range(SPLIT, NL - 1))                      # the nine writers 8..16 (17 writes .0001, §2793; excluded from the pair grid)
    Uk = U_8[:, :768]
    def perp(z):
        return z - (z @ Uk) @ Uk.T
    def writers(l, win):
        """late writer blocks j <= l in the window: 'all' -> every late writer j <= l; ('set', js) -> those of js with j <= l; None -> nothing
        (exact). Block l's own attention write is the d = 0 writer."""
        if win is None:
            return []
        if win == "all":
            return [j for j in LB if j <= l]
        op, js = win
        return [j for j in js if j <= l]
    class Tracker:
    ...
    class RecHead(OwnHead):
        ...
        def __call__(self, w, x):
            ...
            return out
    PAIRS = [(j, k) for j in WR for k in WR if j < k]
    WIN = {"DROP_ALL": "all", "D_8_16": ("set", tuple(WR))}
    WIN.update({f"W{j}": ("set", (j,)) for j in WR}); WIN["W8_REPEAT"] = ("set", (8,))
    WIN.update({f"P{j}_{k}": ("set", (j, k)) for (j, k) in PAIRS})
    # fit pass: per-(reader, writer) means of the scaled component, and per reader the writer x writer Gram of the perp'd (tail) components
    acc = {(l, j): torch.zeros(D, dtype=torch.float64, device=DEV) for l in LB for j in LB if j <= l}
    gram = {l: torch.zeros(len(LB), len(LB), dtype=torch.float64, device=DEV) for l in LB}; ntok = 0
    class MeanRec(RecHead):
        def __init__(self, l, box): super().__init__(l, None, box, None)
        def __call__(self, w, x):
            xh = F.rms_norm(x, (D,)); scale = xh.norm(dim=-1, keepdim=True) / x.norm(dim=-1, keepdim=True).clamp_min(1e-12); b = self.box
            js = writers(self.l, "all")
            P = torch.stack([perp(b["c"][j] * scale).reshape(-1, D).double() for j in js])          # [nw, N, D]
            for a_, j in enumerate(js):
                acc[(self.l, j)] += (b["c"][j] * scale).reshape(-1, D).double().sum(0)
            gram[self.l][:len(js), :len(js)] += torch.einsum("and,bnd->ab", P, P)
    ...
    # centred, pooled-over-readers input-space cosine between writers j < k (readers l >= k see both): sum_l <c_j - mean, c_k - mean>
    def cgram(l, j, k):
        raw = float(gram[l][j - SPLIT, k - SPLIT])
        mj = perp(means[(l, j)]).double(); mk = perp(means[(l, k)]).double()
        return raw - ntok * float(mj @ mk)
    cos_in = {}
    for (j, k) in PAIRS:
        num = sum(cgram(l, j, k) for l in LB if l >= k); dj = sum(cgram(l, j, j) for l in LB if l >= k); dk = sum(cgram(l, k, k) for l in LB if l >= k)
        cos_in[(j, k)] = num / max((dj * dk) ** 0.5, 1e-12)
    log(stage="fit", ntok=ntok, cos_in_median=round(sorted(cos_in.values())[len(cos_in) // 2], 4))
    def rec(win):
        bx = {}
    ...
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items() if not k.startswith("P")})
    C0 = arms["LATE_MLP_768"]; A_ = arms["DROP_ALL"]; J_ = arms["D_8_16"]
    wcost = {j: arms[f"W{j}"] for j in WR}; noise = abs(arms["W8_REPEAT"] - arms["W8"])
    import math
    eps = 1e-6
    cross = {(j, k): arms[f"P{j}_{k}"] - wcost[j] - wcost[k] for (j, k) in PAIRS}
    kappa = {(j, k): cross[(j, k)] / (2.0 * math.sqrt(max(wcost[j], eps) * max(wcost[k], eps))) for (j, k) in PAIRS}
    dist = {(j, k): k - j for (j, k) in PAIRS}
    frac_pos = sum(1 for p in PAIRS if cross[p] > 0) / len(PAIRS)
    rho_dist = R.spearman([float(dist[p]) for p in PAIRS], [kappa[p] for p in PAIRS])
    def med(v):
        v = sorted(v); return v[len(v) // 2] if len(v) % 2 else 0.5 * (v[len(v) // 2 - 1] + v[len(v) // 2])
    kappa_by_d = {d: med([kappa[p] for p in PAIRS if dist[p] == d]) for d in sorted(set(dist.values()))}
    k_adj = kappa_by_d[1]
    pred_joint = sum(wcost.values()) + sum(cross.values())
    ratio = pred_joint / J_ if abs(J_) > 1e-9 else float("inf")
    rho_cos = R.spearman([kappa[p] for p in PAIRS], [cos_in[p] for p in PAIRS])
    summ = {"writer_cost": {str(j): wcost[j] for j in WR}, "repeat_noise_w8": noise,
            "pair_cost": {f"{j}_{k}": arms[f"P{j}_{k}"] for (j, k) in PAIRS}, "pair_cross": {f"{j}_{k}": cross[(j, k)] for (j, k) in PAIRS},
            "pair_kappa": {f"{j}_{k}": kappa[(j, k)] for (j, k) in PAIRS}, "pair_input_cosine": {f"{j}_{k}": cos_in[(j, k)] for (j, k) in PAIRS},
            "frac_pairs_super_additive": frac_pos, "spearman_distance_vs_kappa": rho_dist, "median_kappa_by_distance": {str(d): v for d, v in kappa_by_d.items()},
            "median_kappa_all": med(list(kappa.values())), "median_kappa_adjacent": k_adj,
            "sum_singles_8_16": sum(wcost.values()), "sum_cross_8_16": sum(cross.values()), "pairwise_quadratic_prediction": pred_joint, "measured_d_8_16": J_,
            "pairwise_prediction_over_measured": ratio, "d_8_16_over_drop_all": J_ / A_ if abs(A_) > 1e-9 else float("inf"),
            "spearman_kappa_vs_input_cosine": rho_cos, "median_input_cosine": med(list(cos_in.values()))}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(C0 - PRIOR_LATE_MLP_768) <= BARS["repro_tol"]
                   and abs(A_ - PRIOR_EARLY_TAIL_ONLY) <= BARS["repro_tol"] and max(abs(wcost[j] - PRIOR_W[j]) for j in WR) <= BARS["single_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_pairs_generically_super_additive': bool(frac_pos >= BARS["b_frac"]),
        'pred_c_coherence_falls_with_distance': bool(rho_dist <= BARS["c_rho"]),
        'pred_d_adjacent_writers_coherent': bool(k_adj >= BARS["d_kappa"]),
        'pred_e_pairwise_quadratic_law_reconstructs_the_joint': bool(BARS["e_lo"] <= ratio <= BARS["e_hi"]),
        'pred_f_loss_coherence_tracks_input_cosine': bool(rho_cos >= BARS["f_rho"]),
    }
    nulls = {"b_null_frac_le_.5": bool(frac_pos <= NULLS["b_frac"]), "c_null_rho_ge_0": bool(rho_dist >= NULLS["c_rho"]), "d_null_kappa_adj_le_.05": bool(k_adj <= NULLS["d_kappa"]),
             "e_null_ratio_outside_.5_2": bool(ratio <= NULLS["e_lo"] or ratio >= NULLS["e_hi"]), "f_null_rho_le_.1": bool(rho_cos <= NULLS["f_rho"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "split": SPLIT, "writers": WR, "n_pairs": len(PAIRS),
           "program": "blocks 8-17 MLP reads: core (top-768 of U_8) exact; tail drops the lambda-propagated writes (attention + MLP + bias) of ONE writer (W8..W16), of one PAIR of writers (P{j}_{k}, 36 pairs), of all nine (D_8_16), or of all ten (DROP_ALL) from every downstream reader, each replaced by its fit-set mean; kappa_jk = (cost_jk - cost_j - cost_k) / (2 sqrt(cost_j cost_k)) is the pair's cosine in the loss metric under the quadratic law (§2788)",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "late_mlp_768": C0, "prior_late_mlp_768": PRIOR_LATE_MLP_768,
                          "drop_all": A_, "prior_early_tail_only": PRIOR_EARLY_TAIL_ONLY, "prior_single_writers": {str(j): PRIOR_W[j] for j in WR}, "max_single_writer_repro_gap": max(abs(wcost[j] - PRIOR_W[j]) for j in WR)},
    ...
    if smoke:
        print(json.dumps({k: out[k] for k in ("preds", "nulls", "instrument", "summary", "price")}, indent=1)); return
    if smoke:
