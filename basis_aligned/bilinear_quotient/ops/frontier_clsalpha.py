"""CLASS-CONDITIONED MOTIF ALPHAS (rung 148): the untested cheap motif structure.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2220 tested frame-REFIT of the scalar
alphas (a no-op); per-CLASS alphas (9 heads x 10 classes per layer, ~1,520 scalars total) were never tested.
Fit: the same projection sums, accumulated per oracle class on the fit rows. Eval: the motif hook indexes
alpha by cur['lab'] when labels of the right length are present, falling back to the scalar alpha otherwise
(window evals without labels are mixed-frame and NOT scored; the census eval carries labels and is the
scored object). li default-bound per the S2238 rule.

REGISTERED PREDICTIONS (anchors: census +1.9474; subword +2.319; motifs carry ~36% of subword, S2232):
  (a) census <= 1.87 (>= 0.075 gain).
  (b) subword <= 2.15.
  (c) SANITY: increment in [0.25, 0.45] AND census >= 1.4.
NULL: alpha is class-invariant (gain < 0.02 - consistent with S2220's frame robustness). PRICE: +1,368
scalars (76 heads x 10 classes minus the scalar baseline) - negligible. Tripwire: INSTRUMENT FAIL if census
within 1e-3 of baseline (inert). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: class-conditioned motif alphas')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_clsalpha_results.json'
...
def main():
    ...
    for li in range(2,10):
        ...
        numC=torch.zeros(9,10,device=DEV); denC=torch.zeros(9,10,device=DEV)
        numsC=torch.zeros(9,10,device=DEV); densC=torch.zeros(9,10,device=DEV)
        for _bi9,(X2,v1) in enumerate(zip(caps[li]['x'],caps[li]['v1'])):
            ...
            if SEL.get('cls_alpha'):
                _B9=z.shape[0]
                _lb=clsA[_bi9*4:_bi9*4+_B9].reshape(-1)
                _zf=z.permute(0,2,1,3).reshape(-1,9,z.shape[3])
                _vpf=vp.permute(0,2,1,3).reshape(-1,9,z.shape[3])
                _vsf=vs.permute(0,2,1,3).reshape(-1,9,z.shape[3])
                for _k9 in range(10):
                    _s9=_lb==_k9
                    if _s9.any():
                        numC[:,_k9]+=(_zf[_s9]*_vpf[_s9]).sum((0,2))
                        denC[:,_k9]+=(_vpf[_s9]*_vpf[_s9]).sum((0,2))
                        numsC[:,_k9]+=(_zf[_s9]*_vsf[_s9]).sum((0,2))
                        densC[:,_k9]+=(_vsf[_s9]*_vsf[_s9]).sum((0,2))
        ALPHA[li]=(num/den.clamp_min(1e-9),nums/dens.clamp_min(1e-9))
        if SEL.get('cls_alpha'):
            SEL.setdefault('_AC',{})[li]=(numC/denC.clamp_min(1e-9),
                                          numsC/densC.clamp_min(1e-9))
        caps[li]=None
    def motif_hooks(layers):
        ...
        for li in layers:
            ...
            def h(mo_,args,out,at=at,ph=ph,sh=sh,ap=ap,asf=asf,li=li):
                ...
                vp=vp.permute(0,2,1,3); vs=vm.permute(0,2,1,3)
                _c9=cur.get('lab')
                if (SEL.get('cls_alpha') and _c9 is not None
                        and _c9.numel()==z.shape[0]*z.shape[2]):
                    _apc,_asc=SEL['_AC'][li]
                    _cc=_c9.view(z.shape[0],z.shape[2])
                    for hd in ph: z[:,hd]=_apc[hd][_cc].unsqueeze(-1)*vp[:,hd]
                    for hd in sh: z[:,hd]=_asc[hd][_cc].unsqueeze(-1)*vs[:,hd]
                else:
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['cls_alpha']=True
    print('ARM: rung-127 build + class-conditioned motif alphas',flush=True)
    main()
    ...
    if abs(agg-1.9474)<1e-3:
        raise SystemExit('INSTRUMENT FAIL: class-alpha inert (census unchanged)')
    ...
    print(f"(a) census {agg:+.4f} <= 1.87: {'HELD' if pa else 'FAILED'}")
    print(f"(b) subword {_sw:+.4f} <= 2.15: {'HELD' if pb else 'FAILED'}")
