#!/usr/bin/env python3
# BQGATE:7 body forwards;32 sequences lengths4-12;16 batched suffix arms;no fit;180sec.
"""pred_a frozen recipient/donor replay<=1e-5 and interaction reconstruction<=1e-10;
pred_b full-port transplant regional transfer>=10%native and >=5/8positive EACHfamily;
pred_c parent-only transfer>=50%full transfer and unrelated<=.5regional EACHfamily.
...
STEM='REGIONAL_FACTOR_INTERCHANGE_V1'
@torch.no_grad()
def main():
    ...
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('7 body forwards32sequences4-12tokens;16suffix arms;no fitting');return
    ...
    pre=torch.empty(32,1152,device='cuda');writes=torch.zeros(32,16,1152,dtype=torch.float64,device='cuda');replays=[];count=0
    ...
    prior=torch.load(P/'REGIONAL_SOURCE_BLOCK_V2_ARTIFACT.pt',weights_only=True,map_location='cpu')['writes']['collapsed_all'].cuda()
    recipient_error=float((writes[:,0]-prior).norm()/prior.norm());donor=prior[torch.arange(32,device='cuda')^1];donor_error=float((writes[:,15]-donor).norm()/donor.norm())
    ...
    for family in (0,1):
        ...
        cells.append(dict(family=family,native_regional_gap=float(native[:,0].mean()),mean_transfer_by_mask=transfer.mean(1).tolist(),positive_regional_by_mask=(transfer[:,:,0]>0).sum(1).tolist(),parent_regional_meanabs=float(transfer[2,:,0].abs().mean()),parent_unrelated_meanabs=float(transfer[2,:,1].abs().mean()),interaction_contrast_by_mask=((interactions[:,uk]-interactions[:,us])/2).mean(1).tolist()))
    instrument=recipient_error<=1e-5 and donor_error<=1e-5 and grid_error<=1e-10
    full=instrument and all(c['mean_transfer_by_mask'][15][0]>=.1*c['native_regional_gap'] and c['positive_regional_by_mask'][15]>=5 for c in cells)
    result={'pred_a':instrument,'pred_b':full,'pred_c':full and all(c['mean_transfer_by_mask'][2][0]>=.5*c['mean_transfer_by_mask'][15][0] and c['parent_unrelated_meanabs']<=.5*c['parent_regional_meanabs'] for c in cells)}
