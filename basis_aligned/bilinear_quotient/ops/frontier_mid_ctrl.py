"""MIDDLES REFIT CONTROL (rung 131): in-frame targets — scoring the exactness-vs-steering law.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2223: the trajectory Down-refit of the
pruned middles LOST 0.36 census. Candidate law: any 54k-sample ridge refit of a REAL-weight Down loses to
the model's own exact Down — trajectory or not. This control refits with IN-FRAME targets (the real module's
output on the config stream, captured in the same pass as the inputs; mid_frame on). If it ALSO loses, the
law stands and the middle-refit branch closes; if it recovers to ~baseline, the trajectory TARGET was the
specific harm at real-weight sites.

REGISTERED PREDICTIONS (anchors: rung-127 receipt +1.9474 census / +1.8765 L2_F):
  (a) THE LAW STANDS: census agg >= 2.10 (in-frame refit also loses >= 0.15).
  (b) LAW'S ORDERING: census(in-frame) <= +2.3052 (in-frame loses LESS than trajectory did).
  (c) SANITY: L2_F in [1.7, 2.6] and increment in [0.2, 0.8].
NULL: recovery to <= 2.00 — the trajectory target, not the refit, was the harm; refitting per se is safe.
PRICE: none. Tripwire: INSTRUMENT FAIL if L2CF missing. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_mid_traj_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: middles refit control (in-frame targets)')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_mid_ctrl_results.json'
    ...
    if SEL.get('mid_traj'):
        ...
        for _li in MIDL:
            ...
            def _capD(mo,i_,o_):
                ...
                _Ys.append(o_.detach().reshape(-1,D).float())
            ...
            for _i in range(CA,CB,4):
                ...
                m(cur['idx'],_bb[:,1:].contiguous())
            for h in hs: h.remove()
            _Xd=torch.cat(_Xs); _Y=(torch.cat(_Ys) if SEL.get('mid_frame') else torch.cat(_YRm[_li]))
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['mid_traj']=True; SEL['mid_frame']=True
    print('ARM: rung-127 build + middles IN-FRAME Down-refit (control for S2223)',flush=True)
    main()
    ...
    pa=agg>=2.10
    pb=agg<=2.3052
    pc=(1.7<=L2F<=2.6) and (0.2<=inc<=0.8)
    ...
    print(f"(a) census {agg:+.4f} >= 2.10: {'HELD' if pa else 'FAILED'}")
    print(f"(b) census {agg:+.4f} <= 2.3052 via L2F line {L2F:+.4f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) L2F in [1.7, 2.6], increment in [0.2, 0.8]: {'HELD' if pc else 'FAILED'}")
