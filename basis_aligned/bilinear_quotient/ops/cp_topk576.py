"""PER-TOKEN TOP-K DENSITY SWEEP (rung 208): k=576 - one-eighth density.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2304: per-token top-1152 (quarter
density) costs +0.1015 vs the fixed subset's +2.2048 at matched density - usage is sparse and rotating.
How sparse? This run halves the per-token budget on the same r64-exact base.

REGISTERED PREDICTIONS (arms: TOPK576 = this run; anchors +0.1015 @ k=1152, +0.0852 full):
  (a) USAGE SPARSER THAN QUARTER: census <= 0.20 at k=576.
  (b) SOME CERTIFICATES SURVIVE: valid >= 2.
  (c) SANITY: census in [0.01, 1.2]; inert tripwire vs both anchors.
NULL: quarter density was the knee - halving it at least doubles the excess over full base
(census >= 0.14). PRICE: usage-structure probe, no storage claim. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: per-token top-k density sweep k=576')
    raise SystemExit(0)
import torch
...
OUT=PT+'cp_topk576_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['cp_topk']=576; SEL['qk_r']=64; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    pb=nv>=2
    pc=0.01<=agg<=1.2
    if abs(agg-0.0852)<1e-4 or abs(agg-0.1015)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: topk inert')
    ...
    print(f"(a) census {agg:+.4f} <= 0.20: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 2; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) census in [0.01, 1.2]; era rho {rho:.4f}: {'HELD' if pc else 'FAILED'}")
