...
Replay<1e-5; paired response error<.10, <=1.1narrow26 and<=.8value656.
...
def main():
 ...
 from paired_root_compiler import evaluate as paired
 from circuit_fast_screen_producer import Bilin18TorchBackend
 torch.set_num_threads(2);torch.set_grad_enabled(False);torch.backends.cuda.matmul.allow_tf32=False
 out=P/'RESPONSE_FEATURE_INTERCHANGE_V1.json';assert not out.exists();start=time.monotonic()
 files={'tokens': 'FULL_CHANNEL_FRESH_TOKENS_V1.pt', 'narrow26': 'WIDE_NATIVE_QUARTIC_4_INHERITED_LONG_V1.pt', 'value656': 'QUARTIC_RESPONSE_FEATURES_32_WEIGHT_0_V1.pt', 'response656': 'QUARTIC_RESPONSE_FEATURES_32_WEIGHT_1_V1.pt'}
 hashes={k:hashlib.sha256((P/v).read_bytes()).hexdigest() for k,v in files.items()};assert hashes==json.loads((P/'RESPONSE_FEATURE_INTERCHANGE_INPUTS_V1.json').read_text())['sha256']
 def move(v):
  ...
  return v
 programs={k:move(torch.load(P/files[k],weights_only=True)) for k in ['narrow26','value656','response656']};tokens=torch.load(P/files['tokens'],weights_only=True)
 ...
 def evaluate(arm,x):return teacher(x) if arm=='exact' else paired(programs[arm],x) if arm=='response656' else bank(programs[arm],x)
 ...
 for domain,documents in tokens.items():
  ...
  for doc,c in enumerate(captures):
   ...
   for arm in ['exact','narrow26','value656','response656']:
 ...
 summary=summarize(rows);primary=[r for r in summary if r['arm']=='response656'];lookup={(r['domain'],r['alpha'],r['arm']):r for r in summary}
 pred=dict(pred_a_instrument=max(checks)<1e-5 and max(zeros)==0 and all(r['effect_error']==0 for r in summary if r['arm']=='exact'),pred_b_response=all(r['effect_error'] is not None and r['effect_error']<.1 for r in primary),pred_c_advantage=all(r['effect_error'] is not None and r['effect_error']<=1.1*lookup[r['domain'],r['alpha'],'narrow26']['effect_error'] and r['effect_error']<=.8*lookup[r['domain'],r['alpha'],'value656']['effect_error'] for r in primary))
