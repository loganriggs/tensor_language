...
"""Noun-number demonstrative these/this DoD (v256): is the plural-determiner detector 3892 READ from the token or COMPUTED? v254 / v255: MLP-8 unit 3892 carries 93% of
MLP 8's write on 11.2's ones - one direction at the cue (these / this) and is a clean these-detector. Carrier split (exact; `dod_units.carrier_split` identity) of
u_3892's these - this contrast at the cue over the 48 aligned pairs of the v157 rows, writers = embedding, attn / mlp totals of blocks 0-7, the nine heads of block 8.
The gender detectors were token-carried (~70%, v189 / v190); the number detector MLP-carried (66%, v188). A determiner is a function word: prior unsure.
PREDICTIONS (scored as written; failures preserved)
    pred_a_carrier_closure         carrier shares sum to 1 within 1e-3; per-pair identity within relative 1e-3
    pred_b_embedding_carries_040   the embedding carries >= 0.40 (read from the token)
    pred_c_mlp_stack_at_most_040   mlp:00..07 together carry <= 0.40
    pred_d_attention_at_most_020   attention writers together carry <= 0.20
    pred_e_top_carrier_is_embedding  the embedding is the single largest carrier
...
OUT = ROOT / "circuits/followups/noun_number_demonstrative_dod_unit3892_carrier_split_v256_result.json"
CANDIDATE_ID = "noun_number_demonstrative.these_vs_this.dod_unit3892_carrier_split_v256"
UNIT, CLOSURE_TOL, EMBED_MIN, MLP_MAX, ATTN_MAX = 3892, 1e-3, 0.40, 0.40, 0.20
FORWARDS_MAX = 4
WRITERS = v16.WRITERS
...
def main() -> None:
    rows, he, she, *_ = g.build()          # he = ones-token (positive), she = one-token (negative): names kept
    nouns = {L._single(" these"), L._single(" this")}      # the CUE (determiner) position
    ...
    OUT.write_text(json.dumps({"schema": "pronoun_gender_dod_unit3152_carrier_split_result_v256", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "report": report, "predictions": predictions, "forwards": forwards,
