"""REGISTERED PARETO CLAIM: THE CHEAP-END COMBINATION (rung 197).

CONVENTION (S2135): frontier L2 = CE ADDED ABOVE THE REAL MODEL on the S312 fresh windows; LOWER IS BETTER.
Rung 195 printed DESCRIPTIVELY L2_F +1.3089 / census +1.2219 at 56.9M values (front 3456 + rank-12 + no
tailE). Identical rebuild; claim bars as predictions.

REGISTERED PREDICTIONS (anchors: rung-195 receipt):
  (a) CHEAP POINT: L2_F fresh <= 1.33.
  (b) REPRODUCTION: |census - 1.2219| <= 0.015.
  (c) REPRODUCTION: |L2_F - 1.3089| <= 0.015.
NULL: 195's print was noise. PRICE: 56.9M values. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: registered claim cheap-end combo')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_claim_cheap_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['qk_r']=12; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    pa=L2F<=1.33
    pb=abs(agg-1.2219)<=0.015
    pc=abs(L2F-1.3089)<=0.015
    ...
    print(f"(a) L2F {L2F:+.4f} <= 1.33: {'HELD' if pa else 'FAILED'}")
    print(f"(b) |census {agg:+.4f} - 1.2219| <= 0.015: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |L2F - 1.3089| <= 0.015: {'HELD' if pc else 'FAILED'}")
