"""ALL-QK-24 (rung 189): the top of the rank curve.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. Curve so far: r10 1.52 / r12 1.38 /
r16 1.27. Does r24 keep yielding or is r16 the plateau?

REGISTERED PREDICTIONS (anchor: rung-188 census +1.2673):
  (a) STILL YIELDING: census <= 1.22.
  (b) MONOTONE: census <= 1.2673 + 0.005.
  (c) SANITY: census >= 0.5; inert tripwire vs 1.2673.
NULL (a): plateau (>= 1.25) - r16 is the operating point. PRICE: 18.2M pattern values. Self-reviewed."""
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['qk_r']=24; SEL['qk_tail']=True
    ...
    print(f"(a) census {agg:+.4f} <= 1.22: {'HELD' if pa else 'FAILED'}")
    print(f"(b) census {agg:+.4f} <= 1.2723: {'HELD' if pb else 'FAILED'}")
