"""OPTIMISE THE TAIL SCALES JOINTLY, NOT ONE AT A TIME (Claude, LANE 1)

SS2899 found per-layer tuning does not compose; SS2901 refuted the cascade explanation and located the real one: **the coupling is in
the optimal AMOUNT, not the sign.** Every layer's shrinkage helps -- the best prefix length is 8, all eight -- but uniform .05 gives
**-0.2048** while uniform .20 gives **-0.2290**. Each layer alone prefers .05; all eight together prefer ~.20. SS2901 closed with the
consequence: **any per-layer tuning must optimise jointly.**
...
one layer. a10L is the target because SS2898 and SS2899 both give it the largest single-layer gain (-0.0698 at .25, -0.0847 at .05).
Eleven arms, one fitted stack, **one pipeline run**.

If some (global, multiplier) pair beats the pure global, the joint direction is worth a proper coordinate descent. If the pure global
wins, SS2896's single adopted number is the whole story for this block and no further tuning is warranted -- which is a clean, cheap
close.

pred_b re-anchors the pure-global arm to SS2898's -0.2291, an eighth reproduction of the adopted effect.
...
#                     pred_b_the_global_arm_reproduces_S2898
#                     pred_c_a_joint_setting_beats_the_pure_global
#                     pred_d_the_best_arm_uses_a_nontrivial_multiplier
#                     pred_e_the_arms_are_connected

Preregistration: polynomial_causal/FRONTIER_TAIL_JOINT_SCALE_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run: baseline, global .20, and a 3x3 global-by-multiplier grid")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_tail_joint_scale_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_TAIL_JOINT_SCALE_PREREGISTRATION.md'
PREREG_SHA = "4c3b50a6804bfbee0198c3e97e0ad0af37dcdf65911111d5b5d866d6040ed368"
...
GLOBALS = [0.15, 0.20, 0.25]
MULTS = [0.5, 1.0, 2.0]
TARGET = 10                      # a10L -- SS2898/SS2899 give it the largest single-layer gain
BARS = {"reproduce": 0.05, "anchor": 0.01, "joint_beats": 0.005, "connected": 0.005}
NULLS = {"anchor_ge": 0.03, "joint_beats_le": 0.0}
...
if __name__=='__main__':
    ...
    g98 = json.load(open(S2898))['summary']['best_global_cost']     # -0.2291 at 0.20
    SEL['mode']='norm'

    nk = lambda g, m: f'g{int(round(g*100)):03d}m{int(round(m*100)):03d}'
    def spec(g, m):
        per = {l: g for l in LAYERS}
        per[TARGET] = g * m
        return {'per_layer': per}
    ARMS[:] = ([('baseline', {}), ('global020', {'scale': 0.20})]
               + [(nk(g, m), spec(g, m)) for g in GLOBALS for m in MULTS])
    ...
    grid = {(g, m): round(out[nk(g, m)]['L2_F'] - bF, 4) for g in GLOBALS for m in MULTS}
    best = min(grid, key=grid.get)
    best_cost = grid[best]
    beats = round(c_glob - best_cost, 4)          # POSITIVE = the joint arm is better
    anchor_dev = abs(c_glob - g98)
    # is the best joint arm just the pure global in disguise?
    pure_best = min(((g, 1.0) for g in GLOBALS), key=lambda k: grid[k])

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = anchor_dev <= BARS['anchor']
    pc = beats >= BARS['joint_beats']
    pd = best[1] != 1.0
    pe = abs(c_glob) >= BARS['connected']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_global_arm_reproduces_S2898':bool(pb),
           'pred_c_a_joint_setting_beats_the_pure_global':bool(pc),
           'pred_d_the_best_arm_uses_a_nontrivial_multiplier':bool(pd),
           'pred_e_the_arms_are_connected':bool(pe)}
    nulls={'b_null_the_anchor_fails':bool(anchor_dev>=NULLS['anchor_ge']),
           'c_null_the_pure_global_is_optimal':bool(beats<=NULLS['joint_beats_le'])}
    res={'rung':'frontier_tail_joint_scale','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,'globals':GLOBALS,'mults':MULTS,'target_layer':TARGET,
         'arms':out,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(bF,4),'L2_C_baseline':round(bC,4),
                    'grid_cost':{f'g{g}_m{m}':grid[(g,m)] for g in GLOBALS for m in MULTS},
                    'best_arm':{'global':best[0],'a10L_multiplier':best[1]},
                    'best_cost':best_cost,
                    'cost_global_020':c_glob,'S2898_global_best':round(g98,4),
                    'anchor_deviation':round(anchor_dev,4),
                    'joint_beats_global_by':beats,
                    'best_pure_global_in_grid':{'global':pure_best[0],'cost':grid[pure_best]},
    ...
    for g in GLOBALS:
        print("    g=%.2f: " % g + "  ".join(f"m={m}:{grid[(g,m)]:+.4f}" for m in MULTS))
    print(f"(b) global .20 {c_glob:+.4f} vs SS2898 {g98:+.4f}, dev {anchor_dev:.4f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) best {best} at {best_cost:+.4f} beats global by {beats:+.4f}: {'HELD' if pc else 'FAILED'}")
