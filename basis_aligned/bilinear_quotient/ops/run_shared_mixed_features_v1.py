...
"""Learn shared quadratic producers at fixed1088-product price, two starts.
...
def main():
 ...
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
  ...
  from check_shared_gradient_pullback import main as check
 ...
 torch.backends.cuda.matmul.allow_tf32=False;out=P/'SHARED_MIXED_FEATURES_NATIVE_V1.json';assert not out.exists();start=time.monotonic();scale=19054614563.464127;ridge=1e-6
 ...
 for seed in [1101,1102]:
  source=torch.load(P/f'GAUSSIAN_SUPPORT_EXCHANGE_SEED{seed}_V1.pt',weights_only=True);pairs=source['pairs'].cuda();lam=lambdas[seed];params=[torch.nn.Parameter(t.cuda().double().reshape(576,1152)) for t in source['factors']];rate=.1*math.sqrt(4/1152);opt=torch.optim.Muon(params,lr=rate,weight_decay=0.,adjust_lr_fn='match_rms_adamw');best=None;history=[];maxnormal=0.
  for step in range(steps+1):
   ...
   history.append(dict(step=step,objective=loss,elapsed=time.monotonic()-start));print(json.dumps(dict(seed=seed,**history[-1])),flush=True)
   if step==steps:break
   opt.zero_grad();backward(params,144,4,pairs,c,teacher,tr,loc,projection,S,mu,lam,normalizer,chunk=8);assert all(torch.isfinite(p.grad).all() for p in params);opt.step()
  ...
  with torch.no_grad():
   ...
   path=P/f'SHARED_MIXED_FEATURES_SEED{seed}_V1.pt';torch.save(dict(factors=[a.cpu().float(),b.cpu().float()],pairs=pairs.cpu(),coefficients=(c*scale).cpu().float(),writer=writer.cpu().float(),seed=seed,degree=4),path)
   rows.append(dict(seed=seed,coefficient_weight=lam,initial=initial,final=final,initial_objective=initial_loss,selected_objective=loss,relative_gain=(initial_loss-loss)/abs(initial_loss),selected_step=selected,history=history,baseline_replay=replay,normal_residual=maxnormal,export_error=drift,sha256=hashlib.sha256(path.read_bytes()).hexdigest()))
  del opt,params,best,a,b,c
 pred=dict(pred_a_integrity=all(r['baseline_replay']<1e-4 and r['normal_residual']<1e-8 and r['export_error']<1e-4 for r in rows),pred_b_learning=all(r['relative_gain']>=.01 and r['final']['text_error']<=r['initial']['text_error'] for r in rows),pred_c_component=all(r['final']['root1_same_token_error']<=.1 and r['final']['root1_sensitivity_error']<=.1 for r in rows))
 result=dict(predictions=pred,steps=steps,estimated_step_seconds=estimated_step,rows=rows,products=1088,coefficients=1353728,integer_indices=1024,seconds=time.monotonic()-start,peak_memory_bytes=torch.cuda.max_memory_allocated(),scope='Fixed512rootpairs, learn144normalizedrank<=8quadratics (4bilinear terms) andexact16readouts. FixedGaussian/coefficient mixture, not maintained coefficientconstraint. Warmstarttwoarchivedruns, no textlabels used in fitting/selection. Openedevaluationdiagnostic, no finite-removal/OOD/semanticadoption.')
