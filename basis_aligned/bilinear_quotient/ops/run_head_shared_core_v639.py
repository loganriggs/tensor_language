...
"""Joint folded head-space sparse-core baseline and native coordinate replay.
Full128 input span; output modes32/128; unordered core edges256/2048/8192.
...
OUT=BQ/'circuits/followups/head_shared_core_v639_result.json'
...
def main():
 plan=dict(output_ranks=[32,128],edge_budgets=[256,2048,8192],input_rank=128,forwards_max=64,fit_parameters=0,model_updates=0,model_backwards=0,execution_policy='managed_queue_only',
 ...
 for output_rank in [32,128]:
  ...
  for budget in [256,2048,8192]:
   kept=sparse_core(core,budget);program=compile_core(kept)
   floating=W.numel()+P.numel()+program['coefficients'].numel()
   indices=sum(program[k].numel() for k in ['p','q','alpha','pair'])
   candidates.append(dict(output_rank=output_rank,budget=budget,W=W,program=program,
    unique_products=len(program['p']),floating_values=floating,index_values=indices,
 ...
 try:
  for family,ids in samples:
   ...
   for candidate in candidates:
    approx_quads=[sparse_execute(z,candidate['W'],P,candidate['program']) for z in coordinates]
 ...
 result=dict(plan=plan,stats=stats,candidates=candidates,maximum_coordinate_replay=max(coordinate_checks),predictions=predictions,maximum_write_replay=max(write_checks),maximum_interaction_replay=max(interaction_checks),
  ...
  scope='Sparse joint quadratic core only; background, linear terms and normalization remain exact and expensive; opened prompts; not complete circuit price')
 disk_guard.guard_write(1000000,label='v639');OUT.write_text(json.dumps(result,indent=2)+'\n');print(predictions)
