#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_low_writes_into_next_carry pred_c_copier_4_5_carries pred_d_high_writes_into_next_small pred_e_copier_below_low
"""Who plants gender at the token after the noun (v571). v567: the gender readers' values at the token after the noun carry 0.132 of the he - she gap. That token
is not gendered itself, so the gender there was written by attention from the noun -- for number the copier 4.5 planted the noun's state at noun + 1 (v418,
0.19 of the pronoun gap). Here every head's write INTO noun + 1 swapped for blocks 0-8 (LOW_NEXT), for 4.5 alone (H45_NEXT), and for blocks 9-17 (HIGH_NEXT),
on the 61 pairs.
PREDICTIONS (scored as written; failures preserved; priors from v567 and the number line's copier)
    pred_a_baseline_replays          the unedited manual forward replays the model's he - she margins within 1e-3
    pred_b_low_writes_into_next_carry  blocks 0-8's writes into noun + 1 close >= 0.10 (the readers' value there: 0.132)
    pred_c_copier_4_5_carries        4.5's write into noun + 1 closes >= 0.05
    pred_d_high_writes_into_next_small  blocks 9-17's writes into noun + 1 close <= 0.05 (after the readers' blocks they cannot feed the readers)
    pred_e_copier_below_low          H45_NEXT <= LOW_NEXT
...
OUT = ROOT / "circuits/followups/gender_writes_into_next_v571_result.json"
CANDIDATE_ID = "gender.writes_into_next_v571"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, LOW_MIN, H45_MIN, HIGH_MAX, UNIT = 1e-3, 0.10, 0.05, 0.05, 701
...
FORWARDS_MAX = 9
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_low_writes_into_next_carry": ">= 0.10", "pred_c_copier_4_5_carries": ">= 0.05", "pred_d_high_writes_into_next_small": "<= 0.05", "pred_e_copier_below_low": "H45 <= LOW"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "low_min": LOW_MIN, "h45_min": H45_MIN, "high_max": HIGH_MAX, "unit": UNIT}}
    ...
    MODES = {"LOW_NEXT": ({}, (0,)), "H45_NEXT": ({}, (0,)), "HIGH_NEXT": ({}, (0,))}; SETS_W = {"LOW_NEXT": {l: list(range(N_HEAD)) for l in range(0, 9)}, "H45_NEXT": {4: [5]}, "HIGH_NEXT": {l: list(range(N_HEAD)) for l in range(9, 18)}}
    ...
    print("writes into noun + 1", {k: round(v, 3) for k, v in mc.items()})
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_low_writes_into_next_carry": mc["LOW_NEXT"] >= LOW_MIN, "pred_c_copier_4_5_carries": mc["H45_NEXT"] >= H45_MIN,
                   "pred_d_high_writes_into_next_small": abs(mc["HIGH_NEXT"]) <= HIGH_MAX, "pred_e_copier_below_low": mc["H45_NEXT"] <= mc["LOW_NEXT"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "gender_writes_into_next_result_v571", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
