...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v698_result.json'
...
def main():
    ...
    for name,report in two_site_reports.items():
        for family,cell in report.items():
            ...
            cross=y-np.array(cell['subject_effect'])-cell['attractor_effect']
            ...
            cell['native_cross_over_budget']=float(np.linalg.norm(cross)/budget)
            cell['cross_error_over_budget']=float(np.linalg.norm(pcross-cross)/budget)
            cell['native_cross']=cross.tolist();cell['predicted_cross']=pcross.tolist()
