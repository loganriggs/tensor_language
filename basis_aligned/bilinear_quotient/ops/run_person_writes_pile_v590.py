#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_four_writes_replay_on_pile pred_c_all_writes_into_the_answer pred_d_9_6_and_9_7_write_person pred_e_four_below_all
"""The person write side on the PILE rows (v590). v586-v589 (fineweb, 43 pairs): the four readers' writes into the answer close 0.412 of the myself - yourself
gap, every head's 0.943, block 9's 0.114 (9.6 0.058, 9.7 0.035). Here the same edits on v111's pile-10k rows (the second natural person set, 36 rows, pairs by the
same in-place swap): FOUR_W, ALL_W and the two block-9 writers jointly (NINE).
PREDICTIONS (scored as written; failures preserved; priors from v586 / v589)
    pred_a_baseline_replays              the unedited manual forward replays the model's myself - yourself margins within 1e-3
    pred_b_four_writes_replay_on_pile    FOUR_W closes 0.412 +- 0.15 on the pile pairs
    pred_c_all_writes_into_the_answer    ALL_W closes >= 0.80
    pred_d_9_6_and_9_7_write_person      NINE (9.6 + 9.7 writes) closes >= 0.05
    pred_e_four_below_all                FOUR_W <= ALL_W
PRICE (registered maximum): 2 batches x 4 passes = 8 forwards (<= 36 pairs); 0 backwards; 0 fits. Bar <= 9.
"""
...
OUT = ROOT / "circuits/followups/person_writes_pile_v590_result.json"
CANDIDATE_ID = "person.writes_pile_v590"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, FOUR_V586, BAND, ALL_MIN, NINE_MIN, UNIT = 1e-3, 0.412, 0.15, 0.80, 0.05, 701
PILE = dod_battery.ROOT / "circuits/followups/person_reflexive_dod_pile_rows_v111.json"
...
FORWARDS_MAX = 9
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_four_writes_replay_on_pile": "0.412 +- 0.15", "pred_c_all_writes_into_the_answer": ">= 0.80", "pred_d_9_6_and_9_7_write_person": ">= 0.05", "pred_e_four_below_all": "FOUR_W <= ALL_W"}
...
def main() -> None:
    ...
    def partner(tok):
        ...
        return None
    text_items = pv.person_pairs(PILE); verb_items = [(p, s_, c, 1) for p, s_, c in text_items]
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "four_v586": FOUR_V586, "band": BAND, "all_min": ALL_MIN, "nine_min": NINE_MIN, "unit": UNIT}}
    ...
    MODES = {"FIVE_W": ({}, (0,)), "NINE": ({}, (0,)), "ALL_W": ({}, (0,))}; SETS_W = {"FIVE_W": FIVE_W, "NINE": {9: [6, 7]}, "ALL_W": {l: list(range(N_HEAD)) for l in range(18)}}; forwards = 0
    def logits_margin(x):
    ...
    print("person writes on pile", {k: round(v, 3) for k, v in mc.items()})
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_four_writes_replay_on_pile": abs(mc["FIVE_W"] - FOUR_V586) <= BAND, "pred_c_all_writes_into_the_answer": mc["ALL_W"] >= ALL_MIN,
                   "pred_d_9_6_and_9_7_write_person": mc["NINE"] >= NINE_MIN, "pred_e_four_below_all": mc["FIVE_W"] <= mc["ALL_W"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "person_writes_pile_result_v590", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
