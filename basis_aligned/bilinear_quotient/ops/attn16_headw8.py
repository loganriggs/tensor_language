...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['attn16_three_heads_w8_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; per-head window-grain map at attn16 (10 x 8 evals; damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'attn16_headw8_results.json'
...
def main():
    ...
    meds={h:stt.median([v-b for v,b in zip(res_h[str(h)],r_n['fresh8'])]) for h in range(9)}
    top3=set(sorted(meds,key=lambda h:-meds[h])[:3])
    six_sum=sum(meds[h] for h in meds if h not in {0,3,4})
    pa=top3=={0,3,4}
    pb=six_sum>=0.015
    pc=stt.median(rp)<=0.005
    res={'base':r_n,'per_head_windows':res_h,
         'window_median_d':{f'16.{h}':round(meds[h],4) for h in range(9)},
         'top3':sorted(f'16.{h}' for h in top3),'six_summed_median':round(six_sum,4),
         'convention':'L2 = CE above the real model; lower is better; d_h = damage from zeroing head h (eval-scoped)',
         'pred_a_big_three':bool(pa),'pred_b_six_carry':bool(pb),'pred_c_reproduces_s2146':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print('window-median d per head: '+' '.join(f'16.{h}:{meds[h]:+.4f}' for h in range(9)))
