"""ARE THE CONTEXT MIXERS A PAIR TOO (rung 88): completing the pair census.

CONVENTION (S2135): per-position CE differences on FR; positive = the replacement hurts there. d4 = CE(full) -
CE(block-4 motifs real); d5 = CE(full) - CE(block-5 motifs real). OTHER positions (the mixers' lead class).

S2179/S2180: b2/b3 are a near-identity replicated pair; S2177: so are 14/16. The mixers b4/b5 are the third
candidate - but their signatures already DIVERGE (subword 0.393 vs 0.059; other 0.333 vs 0.464, S2174/S2167),
so the genuine expectation is mixed. NULL: a depth gradient, not a copy (low rho).

REGISTERED PREDICTIONS (arm-named; bins of 200 ordered by d4):
  (a) SAME POSITIONS: binned Spearman rho(d4, d5) >= 0.5 on other positions.
  (b) CONCENTRATION: other positions in the top half of d4 carry >= 0.6 of d5's summed damage.
  (c) REPRODUCTION: the full arm reproduces S2144 (FR L2_F 2.6662 within 0.01).

Writes mixer_pair_results.json. Self-reviewed. RUNG-85 HEADER FOLLOWS.

ARE THE ASSEMBLERS ALSO A REPLICATED PAIR (rung 85): the front symmetry test.

CONVENTION (S2135): per-position CE differences on FR; positive = the replacement hurts there. d2 = CE(full) -
CE(block-2 motifs real); d3 = CE(full) - CE(block-3 motifs real). Subword positions.

S2177: the tail readers are an N=2 same-position ensemble. b2/b3 have near-identical subword-led signatures
(0.453/0.448, S2174/S2175) - are they the front's replicated pair? NULL: division of assembly labor (low rho).

REGISTERED PREDICTIONS (arm-named; bins of 200 ordered by d2):
  (a) SAME POSITIONS: binned Spearman rho(d2, d3) >= 0.5 on subword positions.
  (b) CONCENTRATION: subword positions in the top half of d2 carry >= 0.6 of d3's summed damage.
  (c) REPRODUCTION: the full arm reproduces S2144 (FR L2_F 2.6662 within 0.01).

Writes assembler_pair_results.json. Self-reviewed. RUNG-84 HEADER FOLLOWS.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['assembler_backup_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: deps present; b4-damage vs b5-damage co-variation on other (mixer pair; damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'mixer_pair_results.json'
...
if __name__=='__main__':
    ...
    SEL['skipset']=(); SEL['clsdmg']=True; SEL['motif_off']=(); SEL['zero_m16']=False
    print('ARM 1/3: full S2144 config',flush=True)
    r_full=main()
    cev_full=SEL['cev']; clsflat=SEL['clsflat']
    SEL['motif_off']=(4,)
    print('ARM 2/3: block-4 motifs REAL',flush=True)
    _=main(); cev_no2=SEL['cev']
    SEL['motif_off']=(5,)
    print('ARM 3/3: block-5 motifs REAL',flush=True)
    _=main(); cev_no3=SEL['cev']
    import statistics as stt
    for nm,cv in (('no2',cev_no2),('no3',cev_no3)):
        if float((cv-cev_full).abs().sum())<1e-3:
            raise SystemExit(f'INSTRUMENT FAIL: {nm} identical to full')
    d2=(cev_full-cev_no2); d3=(cev_full-cev_no3)
    SW=CLS.index('other')
    swm=(clsflat==SW).nonzero().squeeze(1)
    order=d2[swm].argsort(); swo=swm[order]
    B=200; x=[];y=[]
    for k0 in range(0,len(swo)-B+1,B):
        ii=swo[k0:k0+B]
        x.append(float(d2[ii].mean())); y.append(float(d3[ii].mean()))
    def rank(v):
        o=sorted(range(len(v)),key=lambda i2:v[i2]); r7=[0]*len(v)
        for pos,i2 in enumerate(o): r7[i2]=pos
        return r7
    n7=len(x)
    rho=1-6*sum((a-b)**2 for a,b in zip(rank(x),rank(y)))/(n7*(n7*n7-1)) if n7>2 else 0.0
    med2=float(d2[swm].median())
    top=swm[d2[swm]>med2]
    tshare=float(d3[top].sum())/max(float(d3[swm].sum()),1e-9)
    pa=rho>=0.5
    pb=tshare>=0.6
    pc=abs(r_full['L2_F']-2.6662)<=0.01
    res={'full':{'L2_F':r_full['L2_F']},'n_subword':int(len(swm)),'n_bins':n7,
         'spearman_binned':round(rho,3),'top_half_share':round(tshare,4),
         'convention':'d2/d3 = CE(full) - CE(block-b motifs real); positive = the replacement hurts there',
         'pred_a_same_positions':bool(pa),'pred_b_concentrated':bool(pb),'pred_c_reproduces_s2144':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'subword n={len(swm)}; binned Spearman(d2,d3) {rho:+.3f}; top-half share {tshare:.3f}')
    print(f"(a) rho {rho:+.3f} >= 0.5: {'HELD' if pa else 'FAILED'}")
    print(f"(b) top-half share {tshare:.3f} >= 0.6: {'HELD' if pb else 'FAILED'}")
    print(f"(c) full FR L2_F {r_full['L2_F']} vs 2.6662 (tol 0.01): {'HELD' if pc else 'FAILED'}")
