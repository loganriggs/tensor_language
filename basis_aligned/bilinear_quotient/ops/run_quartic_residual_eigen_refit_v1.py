...
"""pred_a nativegradientFD<=1e-4 and monotonicobjective; pred_b captureoverold180updatebaseline>=1%; pred_c projectedgrad<=1e-6.
One seed11511,32rank16quadratics,592704floats. Exactcoefficientvariableprojection, no textfit.
Null: changedinitialization maynotimprove thebasin; anunconvergedfit isnotabsentstructure.
"""
...
STEM='QUARTIC_RESIDUAL_EIGEN_REFIT_V1'
def digest(path):
 h=hashlib.sha256()
...
def main():
 ...
 p=torch.load(P/'QUARTIC_RESIDUAL_EIGEN_CANDIDATE_V1_PROGRAM.pt',weights_only=True);assert p['seed']==11511
 ...
 previous=json.loads((P/'COUPLED_QUARTIC_LBFGS_V1_RESULT.json').read_text());candidate=json.loads((P/'QUARTIC_RESIDUAL_EIGEN_CANDIDATE_V1_RESULT.json').read_text());assert candidate['pred_a'];start_objective=previous['history'][-1]['objective']*(1+candidate['coefficient_gain_fraction']);assert abs(initial/divisor-start_objective)<=1e-8
 reference_180=json.loads((P/'COUPLED_QUARTIC_LBFGS_V2_RESULT.json').read_text())['history'][180]['objective']
 ...
 initial_native=rel(features(b,n,x)@mix@writer.T/den[:,None],reference);assert abs(initial_native-candidate['final_native_error'])<=1e-8;history=[];line_failed=False
 def callback(row,bank,weights,currentmix):
  print(json.dumps(row),flush=True)
  (P/(STEM+'_PROGRESS.json')).write_text(json.dumps(row,indent=2)+'\n')
 b,n,mix,history,reason=fit(b,n,evaluate,divisor,max_steps=180,max_seconds=900,callback=callback)
 ...
 result={'pred_a':fd<=1e-4 and all(z['objective']<=history[i]['objective']+1e-10 for i,z in enumerate(history[1:])),'pred_b':-y>=1.01*(-reference_180),'pred_c':gradnorm<=1e-6,'directional_gradient_relative_error':fd,'history':history,'termination':reason,'capture_gain_fraction':-y-1,'continuation_capture_gain_fraction':(-y)/(-start_objective)-1,'reference_180_objective':reference_180,'capture_gain_over_reference_180':y/reference_180-1,'line_search_failed':line_failed,'initial_native_error':initial_native,'final_native_error':native_error,'artifact_sha256':digest(ap),'wall_seconds':time.perf_counter()-tic,'peak_gpu_bytes':torch.cuda.max_memory_allocated(),'scope':'Residual-derived initialization,180updates/900fitsec, versus oldLBFGSV2 first180updates for coefficients only. No native artifact at that reference prefix. Localstationarity separate from gain, no OOD/selectivity evidence.'}
