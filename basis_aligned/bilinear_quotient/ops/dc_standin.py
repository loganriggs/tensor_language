"""THE DC STAND-IN (rung 98): attn3 -> its mean vector, with an optional small gain on attn2.

CONVENTION (S2135): L2 is CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER; damage on the motifs-2/3-real base
(bases excluded). S2187: mean-ablating attn3 costs only +0.0655 (vs +0.237 zeroed) - so "attn3 = its mean
vector" is a 1,152-value stand-in. Does a small gain on attn2 recover part of the remaining signal cost?
NULL (S2187): the b2/b3 signal costs are additive - the gain adds nothing on top of the mean.

REGISTERED PREDICTIONS (arm-named):
  (a) CROSS-RUN ANCHOR: the mean3 arm reproduces S2187 meanB (+0.0655 within 0.01).
  (b) THE GAIN HELPS: [L2_F(mean3 + alpha*) - base] <= +0.05.
  (c) REPRODUCTION: the base reproduces rung 86's base (FR L2_F 2.4410 within 0.01).

Writes dc_standin_results.json. Self-reviewed. RUNG-97 HEADER FOLLOWS.

THE DC LEDGER (rung 97): what share of each heavy component's knockout damage is its MEAN vector?

CONVENTION (S2135): L2 is CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER; damage on the full S2144 config
base (bases excluded). S2187: at b2/b3 the DC term carried ~98% of the joint zero-ablation damage. Does that
generalize? Components: a8 (the most circuit-dense), a5 (the cliff), m13/m14/m15/m16 (the band). For each:
zero-ablation vs mean-ablation; DC share = 1 - d(mean)/d(zero). NULL: DC share is small outside the
assemblers (b2/b3-specific).

REGISTERED PREDICTIONS (arm-named):
  (a) DC DOMINATES MODELWIDE: median DC share across the six components >= 0.5.
  (b) THE CIRCUIT-DENSE COMPONENT TOO: a8 DC share >= 0.5.
  (c) REPRODUCTION: the base reproduces S2144 (FR L2_F 2.6662 within 0.01).
  TRIPWIRE: every ablated arm must differ from base; every d(zero) >= d(mean) - 0.01.

Writes dc_ledger_results.json. Self-reviewed. RUNG-94 HEADER FOLLOWS.

MEAN-ABLATION SUITE FOR ALL THREE TWIN PAIRS (rung 94; user-requested update of the zero-ablation numbers).

CONVENTION (S2135): L2 is CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER. S2185: 73% of attn2's zero-ablation
damage was the MEAN term. Redo the knockout interaction numbers with MEAN-ablation for every pair, each on its
own leave-pair-real base: b2/b3 (motifs 2/3 real), b4/b5 (motifs 4/5 real), 14/16 (skip-1416). Arms per pair:
base (captures means) / meanA / meanB / meanBoth. ZERO-ablation references: b2/3 both = +9.0718 (S2180);
14/16 both = +0.2044 (S2165); b4/b5 previously unmeasured.

REGISTERED PREDICTIONS (arm-named):
  (a) THE MEAN TERM DOMINATED THE OLD NUMBERS: for b2/b3 and 14/16, dBoth(mean) <= 0.5 x dBoth(zero refs).
  (b) THE BACKUP LAW SURVIVES THE DC CORRECTION: for ALL THREE pairs, dBoth >= 1.15 x (dA + dB). NULL: the
      super-additivity was mean-term compounding and vanishes under mean-ablation.
  (c) REPRODUCTION: the b2/b3 base reproduces rung 86's base (FR L2_F 2.4410 within 0.01).
  TRIPWIRE: every ablated arm must differ from its base.

Writes twin_mean_suite_results.json. Self-reviewed. RUNG-93 HEADER FOLLOWS.

PATH DECOMPOSITION AT BLOCK GRAIN (rung 93; rung 92's path arms were mis-scaled - the attn pre-hook sees
the RMS-NORMED stream). The BLOCK receives the raw stream, so inject there: visible to attn3+m3 (granularity
change stated). Sanity bound: both path arms must lie in [0, mean2+0.05] or the run VOIDs itself.

REGISTERED PREDICTIONS (arm-named):
  (a) THE 2->3 BLOCK PATH CARRIES HALF: [L2_F(path_only) - base] <= 0.5 x [L2_F(mean2) - base].
  (b) THE PATH OUTWEIGHS THE RESIDUAL: [L2_F(direct_only) - base] >= [L2_F(path_only) - base].
  (c) REPRODUCTION: base reproduces rung 86's base (FR L2_F 2.4410 within 0.01).

Writes twin_anatomy2_results.json. Self-reviewed. RUNG-92 HEADER FOLLOWS.

TWIN ANATOMY (rung 92; user questions): does the survivor CHANGE when its twin is ablated, do they write
similar directions, and how much of attn2's value flows through attn3 vs the residual path?

CONVENTION (S2135): L2 is CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER. Base = S2144 config, blocks 2/3
motifs REAL; interventions registered after bases (bases excluded). MEAN-ablation of attn2 (gentler than the
zeroing used in S2180). Arms: base | mean2 (y2 -> batch-mean everywhere) | path_only (true y2 visible ONLY to
attn3's input; mean elsewhere) | direct_only (y2 everywhere EXCEPT attn3's input).

REGISTERED PREDICTIONS (arm-named):
  (a) THE 2->3 ATTENTION PATH CARRIES HALF: [L2_F(path_only) - base] <= 0.5 x [L2_F(mean2) - base]. NULL: the
      residual-stream path dominates (path_only ~ mean2).
  (b) ACTIVE RESPONSE, NOT PASSIVE REDUNDANCY: median per-position relative change of attn3's output under
      mean2, on subword positions, >= 0.10. NULL: y3 unchanged (passive twin).
  (c) REPRODUCTION: base reproduces rung 86's base (FR L2_F 2.4410 within 0.01).
  Descriptive: per-position cos(y2-mu2, y3-mu3) on subword vs elsewhere (do they write similar directions);
  cos(delta y3, -(y2-mu2)) (does y3 shift TOWARD replacing the lost y2); mean- vs zero-ablation gap vs S2180.

Writes twin_anatomy_results.json. Self-reviewed. RUNG-87 HEADER FOLLOWS.

DOES THE FRONT PAIR'S MAP TRANSFER (rung 87): the merge-licensing test at b2/b3.

On the S2144 config with blocks 2/3 motifs REAL, capture attn2 and attn3 outputs over FR. S2171: the tail
pair's map does NOT transfer (document-bound). The front's function is distance-flat and local (S2168), so the
derived prediction is the OPPOSITE: a document-independent map. If it transfers, b2/b3 is the program's first
legitimately mergeable pair. NULL: document-bound like the tail.

REGISTERED PREDICTIONS:
  (a) ALIGNED ON ASSEMBLY: mean top-8 canonical correlations on subword positions >= 0.6.
  (b) THE MAP TRANSFERS: the half-A linear map attn2 -> attn3 keeps >= 0.8 of its R^2 on half B.
  (c) REPRODUCTION: the base reproduces rung 86's base (FR L2_F 2.4410 within 0.01).
  TRIPWIRE: capture length must equal the position count.

Writes assembler_transfer_results.json. Self-reviewed. RUNG-77 HEADER FOLLOWS.

MINIMAL REALIZATION TEST FOR THE BACKUP PAIR (rung 77; math review 0140).

Alignment test, not a damage rung. On the skip-1416 config (both attentions real), capture attn14 and attn16
outputs over FR. If the two modules SHARE one read, the linear map between their outputs is document-
INDEPENDENT (fit on half A, transfers to half B); if they are independent duplicates, alignment exists but the
map does not transfer (the null). Sign of prediction derived pre-run per S2165's rule.

REGISTERED PREDICTIONS:
  (a) ALIGNED ON RETRIEVAL: mean of the top-8 canonical correlations on ind positions >= 0.6.
  (b) THE MAP TRANSFERS: the half-A linear map y14 -> y16 keeps >= 0.8 of its R^2 on half B.
  (c) RETRIEVAL-SPECIFIC: mean top-8 CC on ind exceeds the non-ind control by >= 0.2.
  TRIPWIRE: capture length must equal the position count.

Writes backup_pair_cca_results.json. Self-reviewed. RUNG-76 HEADER FOLLOWS.

THE POSITION LAW OF THE other-CLASS DAMAGE (rung 76): does the range law extend to novel tokens?

CONVENTION (S2135): per-position CE differences on FR; positive = the replacement hurts there. Cliff damage =
CE(full) - CE(block-5 motifs real); tail damage = CE(full) - CE(skip-a16, attn16 real). Signed-sum bars.

S2167: the cliff is OTHER-led (novel targets, no occurrence distance). Absolute position measures available
context. If the depth-adds-RANGE law (S2168) extends to novel tokens, the TAIL's other-damage should be more
late-concentrated than the cliff's. NULL: both flat in position (novel-token prediction is local everywhere).

REGISTERED PREDICTIONS (arm-named; damage convention; positions 0-255, late = p >= 128):
  (a) THE RANGE LAW EXTENDS: late-half share of the TAIL's other-damage >= late-half share of the CLIFF's
      other-damage + 0.10.
  (b) CONTEXT ACCUMULATION EXISTS AT BOTH: both late-half shares >= 0.50.
  (c) REPRODUCTION: the full arm reproduces S2144 (FR L2_F 2.6662 within 0.01).

Writes other_position_law_results.json. Self-reviewed. RUNG-73 HEADER FOLLOWS.

WHICH CLASSES PAY AT THE CLIFF (rung 73): the per-class attribution instrument moves to the FRONT - the
block-5 motif replacement.

CONVENTION (S2135): per-position CE(full S2144 config, motifs at blocks 2-9) - CE(same with block-5 motifs
REAL) on FR; positive = the block-5 motif replacement hurts there. Fits matched under each arm (leave-one-real
convention).

The tail's price is retrieval (S2151/S2157). The front's price cliff sits at attn5's write (S2113) and the
certified eight directions were newline/markup/place-name structure (S2111) - so the front's damage should be
DIFFERENT IN KIND. PRICE: none (attribution). NULL: retrieval again (ind/other dominate everywhere).

REGISTERED PREDICTIONS (arm-named; damage convention; share bars conditional on a sign-definite total, S2159):
  (a) NOT RETRIEVAL-CONCENTRATED: share(ind) <= 0.30.
  (b) STRUCTURAL CLASSES PAY: combined share of {newline, sentend, comma, bclose} >= 0.30.
  (c) REPRODUCTION: the full arm reproduces S2144 (FR L2_F 2.6662 within 0.01).

Writes attn5_motif_class_results.json. Self-reviewed. RUNG-63 HEADER FOLLOWS.

WHICH CLASSES PAY FOR a14L (rung 63): the retrieval-vs-energy dichotomy tested at the second layer.

CONVENTION (S2135): per-position CE(full S2144 config) - CE(skip-a14, attn14 real) on FR, grouped by the 10
oracle classes; positive = the a14L dictionary hurts there.

S2151: a16L's damage is ~90% ind+other (in-document retrieval). S2147/S2155: a14 is the energy-dominated,
diffuse layer. If the dichotomy is real, a14L's damage should NOT be retrieval-concentrated and the mean
(CONSTN) classes should pay too. PRICE: none (attribution). NULL: a14 is retrieval-like after all (ind >= 0.4).

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) NOT RETRIEVAL-CONCENTRATED: share(ind) <= 0.30 (a16L had 0.518).
  (b) THE MEAN CLASSES PAY TOO: combined share of the six CONSTN classes >= 0.20 (a16L: ~0).
  (c) REPRODUCTION: the skip-a14 arm reproduces S2155's base per-window values (median |delta| <= 0.005).

Writes attn14_class_damage_results.json. Self-reviewed. RUNG-56 HEADER FOLLOWS.

WHICH CLASSES PAY FOR a16L (rung 56): per-class damage attribution of the costliest replacement.

CONVENTION (S2135): L2 is CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER. Here: per-position CE of the full
S2144 config (a16L installed) minus per-position CE of the S2146 skip-a16 config (attn16 real), on FR, grouped
by the 10 oracle classes. The share tells the block-16 grammar design WHERE the 10-class code fails.
PRICE: none (attribution). NULL: uniform shares (~0.10 each).

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) CONCENTRATION: the top class carries >= 0.40 of the total positive damage share.
  (b) THE FAILURE IS IN A LINK CLASS: the top class is one of the 4 linear-mapped classes (even linearity
      fails there). If FAILED (a CONSTN mean class), a finer partition of that class is the targeted fix.
  (c) REPRODUCTION: the skip-a16 arm reproduces S2146 (FR L2_F 2.5091 within 0.01).

Writes attn16_class_damage_results.json. Self-reviewed. RUNG-55 HEADER FOLLOWS.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['merge_circuit_grain_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: deps present; mean3 stand-in +/- a2 gain arms (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'dc_standin_results.json'
...
def main():
    ...
    _tw=[]
    if SEL.get('abl'):
        for _mkey,(_mod,_mu) in SEL['abl'].items():
            def _mkab(MU):
                def hab(mo,i_,o_):
                    y=o_[0] if isinstance(o_,tuple) else o_
                    if isinstance(MU,tuple) and MU[0]=='scale':
                        r=(y*MU[1])
                    elif MU is not None:
                        r=MU.expand_as(y).to(y.dtype)
                    else:
                        r=torch.zeros_like(y)
                    return (r,o_[1]) if isinstance(o_,tuple) else r
                return hab
            _tw.append(_mod.register_forward_hook(_mkab(_mu)))
            print(f'{_mkey} ablated ({"mean" if _mu is not None else "zero"}) for this arm',flush=True)
    ...
    if SEL.get('clsdmg'):
        ...
        if SEL.get('cap1416'):
            SEL['y14']=[]; SEL['y16']=[]
            _hc.append(m.transformer.h[SEL['cap_pair'][0]].attn.register_forward_hook(
                lambda mo,i_,o_: SEL['y14'].append(o_[0].detach().reshape(-1,D).float().cpu())))
            _hc.append(m.transformer.h[SEL['cap_pair'][1]].attn.register_forward_hook(
                lambda mo,i_,o_: SEL['y16'].append(o_[0].detach().reshape(-1,D).float().cpu())))
        SEL['cev']=evalV(FR,120,order2,ML).detach().cpu()
        for _h8 in _hc: _h8.remove()
...
if __name__=='__main__':
    ...
    SEL['motif_off']=(2,3); SEL['skipset']=(); SEL['clsdmg']=False; SEL['cap1416']=False; SEL['abl']=None
    A2=m.transformer.h[2].attn; A3=m.transformer.h[3].attn
    caps=[]
    _hc2=[A3.register_forward_hook(lambda mo,i_,o_: caps.append(o_[0].detach().float().reshape(-1,D).mean(0).cpu()))]
    print('ARM base (capturing attn3 mean)',flush=True)
    rb=main()
    for h in _hc2: h.remove()
    MU3=torch.stack(caps).mean(0).to(DEV)
    import statistics as stt
    SEL['abl']={'a3':(A3,MU3)}
    print('ARM mean3',flush=True)
    rm=main()
    dmean=rm['L2_F']-rb['L2_F']
    best=(1.0,rm['L2_C'],rm)
    for al in (1.1,1.2,1.3):
        SEL['abl']={'a3':(A3,MU3),'a2':(A2,('scale',al))}
        print(f'ARM mean3 + {al} x a2',flush=True)
        ra=main()
        print(f'  alpha {al}: L2_C {ra["L2_C"]:+.4f} | L2_F {ra["L2_F"]:+.4f}',flush=True)
        if ra['L2_C']<best[1]: best=(al,ra['L2_C'],ra)
    SEL['abl']=None
    dbest=best[2]['L2_F']-rb['L2_F']
    pa=abs(dmean-0.0655)<=0.01
    pb=dbest<=0.05
    pc=abs(rb['L2_F']-2.4410)<=0.01
    res={'base':{'L2_F':rb['L2_F']},'mean3_d':round(dmean,4),'alpha_star':best[0],'best_d':round(dbest,4),
         'price':'1,152 values (the mean vector) + 1 scalar',
         'convention':'damage on the motifs-2/3-real base; lower L2 better',
         'pred_a_anchor':bool(pa),'pred_b_gain_helps':bool(pb),'pred_c_reproduces_r86_base':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'mean3 d={dmean:+.4f}; + alpha*={best[0]} -> d={dbest:+.4f}')
    print(f"(a) mean3 {dmean:+.4f} vs +0.0655 (tol 0.01): {'HELD' if pa else 'FAILED'}")
    print(f"(b) best {dbest:+.4f} <= +0.05: {'HELD' if pb else 'FAILED'}")
    print(f"(c) base {rb['L2_F']} vs 2.4410 (tol 0.01): {'HELD' if pc else 'FAILED'}")
