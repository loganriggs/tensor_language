...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_c69_prune_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; norm-2304/1152/576 x eight windows (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_norm_ksweep_results.json'
...
if __name__=='__main__':
    ...
    r_norm=main()
    SEL['mode']='norm'; SEL['K']=1152; SEL['K69']=1152
    print('ARM 2/3: norm-1152 at all six middles',flush=True)
    r_h=main()
    SEL['mode']='norm'; SEL['K']=576; SEL['K69']=576
    print('ARM 3/3: norm-576 at all six middles (descriptive)',flush=True)
    r_q=main()
    import statistics as stt
    g1=[round(a-b,4) for a,b in zip(r_h['fresh8'],r_norm['fresh8'])]
    g5=[round(a-b,4) for a,b in zip(r_q['fresh8'],r_norm['fresh8'])]
    pa=stt.median(g1)<=0.02
    pb=sum(g<=0.04 for g in g1)>=6
    pc=abs(r_norm['L2_F']-2.6735)<=0.01
    res={'norm2304':r_norm,'norm1152':r_h,'norm576':r_q,
         'per_window_damage_added_1152':g1,'per_window_damage_added_576':g5,
         'convention':'L2 = CE above the real model; lower is better; positive entries = damage added',
         'pred_a_half_units_cheap':bool(pa),'pred_b_windows_floor':bool(pb),'pred_c_reproduces_s312':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'damage added, norm-1152 vs norm-2304, per window: {g1}')
    print(f'damage added, norm-576 vs norm-2304, per window: {g5}')
    print(f"(a) 1152 median damage {stt.median(g1):+.4f} <= +0.02: {'HELD' if pa else 'FAILED'}")
    print(f"(b) <= +0.04 on {sum(g<=0.04 for g in g1)}/8 (bar 6): {'HELD' if pb else 'FAILED'}")
