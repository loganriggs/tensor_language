...
"""v637: do the six separable possessive candidates survive the REGISTERED fit objective?

WHERE THE SIX CAME FROM. v633: ten possessive cells differing by mapping all reached held-out extraction 0.883 to
0.897, and seven cleared four tier rows AND both controls. v635: six of those seven separate from each other and
from the counted family, with maximum sibling leaks 0.0043 to 0.0339 against a 0.05 bar -- disjoint_my_your, gender,
number_his_their, number_its_their, person_our_your, pronoun_number_hers_theirs. Only attractor_conflict fused, at
0.0641, which is the cell that deliberately pits number against an attractor. That is six candidates that clear four
rows, pass two controls and are mutually distinct: the first real block in this sequence.
THE ONE THING BETWEEN THEM AND BEING BOOKABLE. All of it was measured under a fit against the WEEKLY control. The
registered protocol fits against the CANONICAL control at control_weight 30, so as things stand these six are
candidates under an objective the protocol does not use, and booking them would need a ruling on the objective. That
ruling is not mine to make. But there is a version of the question that needs no ruling at all: refit them exactly as
the REGISTERED protocol does and see whether they still pass. If they do, they are bookable under current semantics
with nothing changed, and the fit-objective discussion becomes a separate matter rather than a blocker.
WHY THIS IS NOT A FOREGONE CONCLUSION, IN EITHER DIRECTION. Across 25 cells fitted against the canonical control,
selectivity transferred to the held-out control only 32 percent of the time against 64 percent for the weekly
arrangement, so refitting could well cost several of the six their held-out control. It could also cost nothing:
these are the strongest cells measured all day, their canonical bounds under the weekly fit were negative for four of
six, and a cell whose direction already improves the canonical control may keep doing so when the fit is pointed at
it. I am not predicting a clean sweep either way.
RANK 1, registered, unchanged, and a null is not permission to raise it. Both control numbers reported per cell, as
before. The ONLY change from v633 is which control enters the objective.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
  pred_a_controls_capable the weekly control -- now the HELD-OUT one -- is capable on all six: at most
                        max_dropped = 4 of its 16 held rows dropped.                                  prior 85%
  pred_b_all_reach      every one of the six reaches held-out extraction ext_min = 0.80 under the registered fit.
                        Extraction should not depend much on the control, so a failure here would be surprising and
                        would point at the fit rather than the sites.                                 prior 85%
  pred_c_siblings_reached at least k_reach = 5 of the six reach 0.80.                                 prior 90%
  pred_d_reached_are_two_control_selective at least k_both = 4 of the six are selective on BOTH controls under the
                        REGISTERED fit. This is the rung: it decides whether the six are bookable with no rule
                        change, or whether their standing depends on an objective the protocol does not use.
                                                                                                      prior 50%
  pred_e_all_measured   zero cells error.                                                             prior 90%
HOW IT READS. d TRUE: at least four of the six are two-control selective under the protocol exactly as registered,
they are bookable without any change to anyone's semantics, and the fit-objective question becomes a separate
improvement rather than a precondition. d FALSE with b true: the six carry the variable under the registered fit but
their selectivity depends on the weekly objective, which sharpens the case for revisiting the objective and is
itself the cleanest evidence yet for it -- but it leaves the six uncounted until that is ruled on. b FALSE:
extraction depends on the control in a way I did not expect, and the whole two-control framework needs re-reading
before any of these six means anything.
SCOPE. Six cells, one objective change, rank 1, two controls. No counting, no bar change, no separability re-test --
v635 already established mutual distinctness and that does not depend on the objective.
Smoke: V637_SMOKE=<out.json> (CPU, V637_SMOKE_ROWS=4, V637_SMOKE_NAMES=<cell>).
"""
...
OUT = ROOT / "circuits/followups/unit_possessive_registered_objective_v637_result.json"
...
NAMES = {"possessive_disjoint_my_your": "possessive_disjoint_my_your",
         "possessive_gender": "possessive_gender",
         "possessive_number_his_their": "possessive_number_his_their",
         "possessive_number_its_their": "possessive_number_its_their",
         "possessive_person_our_your": "possessive_person_our_your",
         "possessive_pronoun_number_hers_theirs": "possessive_pronoun_number_hers_theirs"}
ANCHOR = "possessive_gender"
SIBLINGS = ['possessive_disjoint_my_your', 'possessive_number_his_their', 'possessive_number_its_their', 'possessive_person_our_your', 'possessive_pronoun_number_hers_theirs']
V637_ANCHOR = None
# every one of the six cleared four rows and both controls in v633 and separated in v635,
# all under a fit against the WEEKLY control; this rung refits them against the CANONICAL one
...
BARS = {"ext_min": EXT_MIN, "rem_min": REM_MIN, "c_ub_max": C_UB_MAX, "row5_share_band": [0.5, 1.4], "k_reach": 5, "k_both": 4, "tol": 0.05, "max_dropped": 4}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 200000, 6400000


def _plan():
    return {"candidate_id": "corpus.unit_possessive_registered_objective_v637", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V637_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V637_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V637_SMOKE_NAMES", "verb_preposition_against_for,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_possessive_registered_objective_v637", "candidate_id": "corpus.unit_possessive_registered_objective_v637", "bars": BARS,
