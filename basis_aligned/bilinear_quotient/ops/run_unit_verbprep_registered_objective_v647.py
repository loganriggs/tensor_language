...
"""v647: is the registered-objective result possessive-specific? Eight verb_preposition mappings, same fit.
...
Smoke: V647_SMOKE=<out.json> (CPU, V647_SMOKE_ROWS=4, V647_SMOKE_NAMES=<cell>).
"""
...
OUT = ROOT / "circuits/followups/unit_verbprep_registered_objective_v647_result.json"
...
NAMES = {"verb_preposition_about_against": "verb_preposition_about_against",
         "verb_preposition_about_beyond": "verb_preposition_about_beyond",
         "verb_preposition_about_for": "verb_preposition_about_for",
         "verb_preposition_about_into": "verb_preposition_about_into",
         "verb_preposition_against_for": "verb_preposition_against_for",
         "verb_preposition_at_across": "verb_preposition_at_across",
         "verb_preposition_at_from": "verb_preposition_at_from",
         "verb_preposition_at_into": "verb_preposition_at_into"}
ANCHOR = "verb_preposition_about_for"
SIBLINGS = ['verb_preposition_about_against', 'verb_preposition_about_beyond', 'verb_preposition_about_into', 'verb_preposition_against_for', 'verb_preposition_at_across', 'verb_preposition_at_from', 'verb_preposition_at_into']
V647_ANCHOR = None
# none of these eight has been two-control tested; this is a fresh sample under the REGISTERED objective
...
def _plan():
    return {"candidate_id": "corpus.unit_verbprep_registered_objective_v647", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V647_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V647_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V647_SMOKE_NAMES", "verb_preposition_against_for,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_verbprep_registered_objective_v647", "candidate_id": "corpus.unit_verbprep_registered_objective_v647", "bars": BARS,
