...
Fresh full city-value removal confirmation; no causal-composition or complete-circuit promotion.
"""
...
STEM='CITY_FULL_PILE_V2';ARMS=['native','native8_midpoint','full_reentry','compiled']+[f'null:{k}' for k in range(16)]
PACKAGE=P/'extracted_circuits/typed_face_single_head_norm_v1'
sys.path.insert(0,str(PACKAGE))
import execute as compiled
import city_full_value_removal_v1 as candidate
from freeze_city_full_fresh_v1_baselines import features
from head2_mlp8_cross_edit_v1 import retained_delta
def digest(p):return hashlib.sha256(Path(p).read_bytes()).hexdigest()
@torch.no_grad()
def main():
 binding=json.loads((P/(STEM+'_BINDING.json')).read_text())['files'];assert all(digest(k)==v for k,v in binding.items())
 doc=json.loads((P/'CITY_FULL_PILE_V2_ROWS.json').read_text());rows=doc['rows'];groups,mapping=group_rows(rows);assert len(groups)==40
 ...
 tables={k:v.cuda() for k,v in torch.load(P/'CITY_FULL_PILE_V2_TABLES.pt',weights_only=True).items()}
 ...
 def pre9(module,args):
  ...
  if arm=='native':c['x9']=x.clone()
   ...
   row=groups[state['i']];seed=17093100+1000*int(arm.split(':')[1])+row['context_id'];generator=torch.Generator(device=x.device).manual_seed(seed)
  ...
  for ai,arm in enumerate(ARMS):
   ...
   for i,row in enumerate(groups):
    ...
    if arm=='native8_midpoint':
     ...
     value=p['mixture']*value+(1-p['mixture'])*F.linear(c['current'],p['current_value'])[:,city]
 ...
 for cell in doc['contexts']:
  idx=[i for i,r in enumerate(rows) if r['context_id']==cell['context_id']];base=v[0,idx,0][::2]-v[0,idx,0][1::2];got=v[3,idx,0][::2]-v[3,idx,0][1::2];cap=base>=.1
  untouched=[i for i in idx if rows[i]['is_untouched_natural_arm']]
  cells.append({'context_id':cell['context_id'],'document_sha256':cell['document_sha256'],'native_pair_margins':base.tolist(),'capable_endpoints':int(cap.sum()),'capable_attenuations':((base[cap]-got[cap])/base[cap]).tolist(),'untouched_natural_effects':e[3,untouched].tolist()})
 ...
 'pred_b':all(r['capable_pairs']>=90 and r['target_error']<=.35 and r['retained_target_rms']>=1e-5 for r in records.values()),'pred_c':all(max(r['control_over_target'])<=.5 for r in records.values()),
 ...
 'families':records,'document_diagnostics':cells,'full_reentry_max_abs':anchor_abs,'full_reentry_relative':anchor_rel,'max_full_joint_input_error':max(joint_errors),'max_null_norm_error':max(norm_errors),'max_outside':max(outside),'body_forwards':count,'seconds':time.perf_counter()-start,'source_shas':binding,'scope':'Filtered natural Pile contexts;20documents, one untouched arm and one city substitution. One native input and external suffix; diagnostic spelling readers, not natural next-token labels. No pretraining-disjointness/token-only/composition claim.'}
