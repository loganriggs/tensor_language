# HOW MANY NEIGHBOURS? -- k-nearest averaging for the uncovered quarter.
#
# §1777/§1778: sending each uncovered token to the SINGLE covered token with the most similar input
# embedding beat a mean over all 5,419 covered rows by 0.28-0.43 nats at every rank on all three
# roles, and made a fully standalone position-wise program the best all-position program in this arc.
#
# Those are the two extremes of one family: k=1 and k=5419. Neither is obviously right. A single
# neighbour is high-variance -- one token's row carries its idiosyncrasies -- and the global mean is
# maximally biased. The natural question is where in between, and it costs nothing extra to store:
# the program still holds one row per covered token, plus an index of k neighbours per uncovered id.
#
# ROLES. skip7000, skip11000 and skip1200, all-position and covered CE. skip11000's k=1 arm
...
# back per LESSON 39:
#   pred_a AVERAGING HELPS: at every rank and role, the best k in {1,2,4,8,16} beats k=1 by more than
#          0.005 nats. If FALSE the single nearest neighbour is already right and the variance
#          argument is wrong -- which would say the embedding neighbourhood is sharp rather than
#          noisy, and is worth knowing.
#   pred_b THE OPTIMUM IS INTERIOR: the best k is neither 1 nor 16 at every rank and role. If FALSE
#          at the top end the sweep is under-budgeted -- the defect §1756, §1770 and §1771 each
#          recorded -- and I report that rather than a design point.
#   pred_c COVERED CE IS UNTOUCHED by every k, on every role, within 1e-6. A wiring check that must
#          hold by construction.
#   pred_d CONTROLS: the k=1 arm reproduces §1777's 6.03786, 6.23480, 6.54141 and 6.69203 on
#          skip11000 within 0.002, the mean arm reproduces its 6.46948, 6.64292, 6.89892 and 7.02245,
#          and coverage is exactly 5419 of 50257.
...
OUT = PT + 'ops/knn_fallback_results.json'
KS = (1, 2, 4, 8, 16)
FAMILIES = ('contextfree',)
FALLBACKS = tuple(f'knn{k}' for k in KS) + ('mean',)
...
def table_hook(tbl, seen, standalone=False):
    ...
    def hook(mod, args, out):
        ...
        if k is None:
            ...
            nb = COV['topk'][STATE['idx'].reshape(-1), :k]        # [N, k] compact row ids
            sub = tbl[nb].mean(1).reshape(y.shape).to(y.dtype)
        if not standalone:
...
def main():
    ...
    topk = torch.zeros(V, max(KS), dtype=torch.long, device=DEV)
    topk[cov_idx] = torch.arange(nc, device=DEV).unsqueeze(1)
    for s0 in range(0, unc.numel(), 4096):
        u = unc[s0:s0 + 4096]
        sim = En[u] @ En[cov_idx].T
        topk[u] = sim.topk(max(KS), dim=-1).indices
    del En, sim
    torch.cuda.empty_cache()
    print(f'  built the top-{max(KS)} covered neighbours for {unc.numel()} uncovered ids '
    ...
    for fam in FAMILIES:
        ...
        for r in TABLE_RANKS:
            ...
            for fb in FALLBACKS:
                COV['k'] = None if fb == 'mean' else int(fb[3:])
                COV['idmap'] = mean_map
    ...
    def bestk(kk, e):
        return min(KS, key=lambda k2: out[kk][f'knn{k2}_{e}']['all_ce'])
    pa = all(out[kk][f'knn{bestk(kk, e)}_{e}']['all_ce']
             < out[kk][f'knn1_{e}']['all_ce'] - 0.005 for kk in keys for e in roles)
    pb = all(1 < bestk(kk, e) < max(KS) for kk in keys for e in roles)
    pc = all(abs(out[kk][f'knn{k2}_{e}']['cov_ce'] - out[kk][f'mean_{e}']['cov_ce']) <= 1e-6
             for kk in keys for e in roles for k2 in KS)
    pd = (all(abs(out[kk]['knn1_skip11000']['all_ce'] - v) <= 0.002
    ...
    print(f'\n  averaging k>1 neighbours beats k=1 by >0.005 everywhere -> {pa}', flush=True)
    print(f'  the best k is interior at every rank and role '
          f'{{e: [bestk(kk, e) for kk in keys] for e in roles}} -> {pb}'.replace('{e:', '{'),
          flush=True)
    print(f'  covered CE untouched by every k -> {pc}', flush=True)
    print(f'  knn1 reproduces §1777 + coverage {ncov} -> control {pd}', flush=True)
    ...
    r2 = {'config': {'ranks': [str(x) for x in TABLE_RANKS],
          ...
          'by_table_rank': out,
          'predictions': {'pred_a_averaging_beats_single_nearest': bool(pa),
                          'pred_b_best_k_is_interior': bool(pb),
