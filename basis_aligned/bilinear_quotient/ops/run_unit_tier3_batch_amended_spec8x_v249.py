#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the twenty-fourth eight spec-authored behaviours, plus one instrument.
"""v249: the AMENDED tier-3 battery (v196/v197 protocol, unchanged) over the TWENTY-FOURTH batch of spec-authored behaviours (EIGHT) -- plus one instrument.
...
v231 7/8, v233 3/8, v235 4/8, v237 2/8, v239 3/8, v241 4/6, v243 5/7, v245 2/6, v247 (pending at authoring) (+instrument each) at ~30-45 GPU-s per behaviour.
SECOND batch under the v245 lesson (pre-screen floor 1.0 on A1, mean base/donor on the donor axis; one repair or drop). v247 probes whether
the v245 row-4 own-C leak follows the readout class (determiner/pronoun vs other); this batch widens the SELECTION classes so that question
gets a real sample: four adjective/verb preposition-selection cells, one pronoun-case cell, one degree-marker cell, one correlative cell.
Recipe since v231: GRAMMATICAL cues with readout token pairs never screened before -- from v247 on the duplicate grep is ORDER-INSENSITIVE
(both `(' a', ' b')` and `(' b', ' a')`); they/them, with/about, from/of, of/at, to/of, with/for, as/than, or/and are all new pairs.
...
case_they_them (`The pilots near the lantern knew/thanked, of course,` -> they/them; case family);
verb_preposition_with_about (`Near the lantern the pilot dealt/worried, of course,` -> with/about; verb_preposition family);
verb_preposition_from_of (`Near the lantern the pilot suffered/died, of course,` -> from/of; verb_preposition family);
adjective_preposition_of_at (`Near the lantern the pilot was proud/good, of course,` -> of/at; adjective-preposition selection, v247 in_of);
adjective_preposition_to_of (`Near the lantern the pilot was similar/aware, of course,` -> to/of; adjective-preposition selection);
adjective_preposition_with_for (`Near the lantern the pilot was pleased/ready, of course,` -> with/for; adjective-preposition selection);
comparative_equative_as_than (`Near the lantern the pilot was as/more careful, of course,` -> as/than; degree family);
correlative_or_and (`Near the lantern either/both the pilot, of course,` -> or/and; correlative coordination, v245 or_nor failed row 4).
CPU capability margins BEFORE enqueue (mean base / mean donor on the donor axis; A1 | A2; dropped rows; floor 1.0 on A1 per the v245 lesson):
  case_they_them A1:-3.20/+3.00 drop0 A2:-2.94/+2.97 drop0 C:-1.43/+0.93 drop2
  verb_preposition_with_about A1:-4.92/+5.23 drop0 A2:-4.75/+4.96 drop0 C:-1.43/+0.93 drop2
  verb_preposition_from_of A1:-2.19/+2.07 drop0 A2:-1.58/+1.46 drop0 C:-1.43/+0.93 drop2
  adjective_preposition_of_at A1:-2.38/+2.67 drop0 A2:-1.68/+1.86 drop0 C:-1.43/+0.93 drop2
  adjective_preposition_to_of A1:-5.83/+5.75 drop0 A2:-4.18/+4.22 drop0 C:-1.43/+0.93 drop2
  adjective_preposition_with_for A1:-1.89/+1.75 drop0 A2:-1.61/+1.67 drop0 C:-1.43/+0.93 drop2
  comparative_equative_as_than A1:-3.27/+3.14 drop0 A2:-2.83/+2.83 drop0 C:-1.43/+0.93 drop2
  correlative_or_and A1:-3.18/+3.17 drop0 A2:-3.62/+3.72 drop0 C:-1.43/+0.93 drop2
  All eight A1 cells are >= 1.89/1.75 with zero dropped rows; every A2 cell >= 1.58/1.46. NO repairs, NO drops -- the first batch where every
  generated cell cleared the floor (the pairs were chosen from the classes that cleared it in v247: verb/adjective preposition selection,
  pronoun case, degree markers, correlatives). Batch size EIGHT; bars are the eight-cell bars (4/6/7/5/5 of 8).
Protocol identical to v197/v201/v204/.../v247 (greedy pool 40, target 0.88, min_gain 0.005, max 30; row 2 = MIN over the two held directions
...
REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 4 of the 8 new behaviours pass all four rows (pre-screened batches 11/19 = 0.58; all eight A1 cells >= 1.75). prior 55%
  pred_b_row2           -> MIN held-out per-direction exact-set recovery >= 0.80 on at least 6 of 8.              prior 65%
  pred_c_row3           -> dim A1 CE damage LB975 > 0 and >= 0.10 on at least 7 of 8 (17/17 of the >= 1.0 A1 cells across v241-v245 passed). prior 70%
  pred_d_row4           -> constrained rank-1 own-C CE UB975 <= 0.01 on at least 5 of 8 (v241 5/6, v243 5/7, v245 3/6; non-number/person readouts 3/6). prior 50%
  pred_e_row5_amended   -> A2-own / A2 ceiling within [0.5, 1.4] with LB975 > 0 on at least 5 of 8 (v241 6/6, v243 7/7, v245 6/6). prior 70%
...
Accounting: four-row passes here join the one-protocol standing. The three adjective_preposition_* cells and correlative_or_and have no
counted family (adjective_preposition_in_of in v247 would open one) and count as singletons on a four-row pass, subject to a within-batch
separability rung if two or more of a class pass; case_they_them (case), verb_preposition_* (verb_preposition) and comparative_equative_as_than
(degree) count only after a separability rung under the full family control set.
...
Smoke: V249_SMOKE=<out.json> (CPU, V249_SMOKE_ROWS=4 per split, V249_SMOKE_NAMES=case_they_them; pool 3/max 3, steps 5).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec8x_v249_result.json"
...
NAMES = {"case_they_them": "case_they_them", "verb_preposition_with_about": "verb_preposition_with_about",
         "verb_preposition_from_of": "verb_preposition_from_of", "adjective_preposition_of_at": "adjective_preposition_of_at",
         "adjective_preposition_to_of": "adjective_preposition_to_of", "adjective_preposition_with_for": "adjective_preposition_with_for",
         "comparative_equative_as_than": "comparative_equative_as_than", "correlative_or_and": "correlative_or_and",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec8x_v249", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V249_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V249_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V249_SMOKE_NAMES", "case_they_them").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec8x_v249", "candidate_id": "corpus.unit_tier3_batch_amended_spec8x_v249", "bars": BARS,
