#!/usr/bin/env python3
# BQGATE: five frozen predictions; set (v87 quantifier hub16), recipe, split and bars fixed before the run.
"""v88: the full-specificity direction on the quantifier hub16 set -- row 2 on odd extraction and cross-fitted row 4 on one direction.

quantifier_number on hub+8 sat at 3/5: the direction's extraction was 0.757 against an exact-set 0.816 (objective-limited, v82),
and its own-C residual is real (v86 cross-fitted: point 0.010, UB 0.018). v87 added eight heads; exact-set extraction is now
0.915 (LB 0.870) and odd removal 0.925. Standard recipe on the hub16 set: rank 1 per block, pooled EVEN A1 + verb-variant rows,
complement 1.0, own C EVEN + other five A1 EVEN as inertness controls (30 each), 120 steps, lr 0.05, seed 0; arms xdas (primary),
xdim, dim, exact, random; extraction and removal on ODD rows. Row 4 is cross-fitted as in v86: a second direction is fitted on the
ODD pool with ODD controls and evaluated on EVEN C; the two out-of-sample C damages are pooled (32 documents) before bootstrapping.

REGISTERED BEFORE THE RUN (extraction = fraction of full-ablation margin restored; removal = CE damage in nat)
    pred_a_reproduce      exact-set extraction A1 odd within 0.02 of v87's 0.915.
    pred_b_row2           xdas extraction A1 odd >= 0.80 with lb95 >= 0.60. Worked: 0.85 / 0.78 True; 0.79 / 0.75 False.
    pred_c_removal_grows  xdas A1 odd removal >= 0.831 (hub8 direction 0.731 + 0.10). Worked: 0.88 True; 0.80 False.
    ...
    Prior: a True; b ~75% (0.915 minus the 0.05-0.07 objective shortfall lands at 0.85); c ~85%; d ~35% -- the hub8 residual was
    real, not width, and more heads means more C-active heads for the penalty to cancel; e True.
    d False: quantifier stays at 2/3/5 with row 4 open for a reason that is not sample width; the next test is a second C sibling.
"""
...
OUT = ROOT / "circuits/followups/unit_quant_hub16_full_specificity_v88_result.json"
V87 = ROOT / "circuits/followups/unit_quant_dative_greedy_continuation_v87_result.json"
NAME = "quantifier_number"
LAM, STEPS, LR, CW = 30.0, 120, 0.05, 1.0
REPRO, REMOVAL_MIN, C_UB, ROW2, ROW2_LB, TRANSFER, CROSS_MAX, RAND_FRAC = 0.02, 0.831, 0.01, 0.80, 0.60, 0.50, 0.05, 0.10
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 700, 45000


def _plan():
    return {"candidate_id": "corpus.unit_quant_hub16_full_specificity_v88", "lambda": LAM,
...
def main() -> None:
    ...
    v87 = json.loads(V87.read_text())["sets"][NAME]
    sets = {NAME: v87["hub16"]}
    v87_exact = v87["sets"]["hub16"]["extraction"]["A1"]["point"]
    ...
    for name, units in sets.items():
        ...
        R[name] = {"units": units, "extraction": ext, "v87_exact_a1_odd": v87_exact, "crossfit": crossfit,
    ...
    CF = R[n]["crossfit"]
    predictions = {
        'pred_a_reproduce': abs(e(n, "A1", "exact")["point"] - R[n]["v87_exact_a1_odd"]) <= REPRO,
    ...
    result = {"predictions": predictions, "schema": "circuit_unit_quant_hub16_full_specificity_result_v1", "candidate_id": "corpus.unit_quant_hub16_full_specificity_v88",
