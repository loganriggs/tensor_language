...
"""Pronoun gender he/she DoD (v218): is head 4.5 the copy head on the GENDER line too? v205 (number): 4.5 carries 94% of block 4's contrast into the plural
detector at the verb, 97% of it from the noun position, 26% token-only; v207b: zeroing it moves animacy and tense readers too -- a shared subject-feature
copier, which predicts it also carries GENDER to the verb. Same fold on the v71 rows: every head of blocks 4 and 5 at the verb, reader = the male
detector 3152's product gradient at the verb (block-8 input), by source position and value branch. v216 / v217: 9.6 reads the verb 48% on gender and
3152 re-fires there (78%).
PREDICTIONS (scored as written; failures preserved; priors from v205)
    ...
    pred_d_leading_heads_are_contextual  for both leading heads the token-only branch carries <= 0.50 (gender is token-carried at the noun, v189: prior unsure)
...
OUT = ROOT / "circuits/followups/pronoun_gender_dod_verb_head_source_fold_v218_result.json"
CANDIDATE_ID = "pronoun_gender.he_vs_she.dod_verb_head_source_fold_v218"
UNIT, BLOCKS, CLOSURE_TOL, LEAD_MIN, NOUN_MIN, INHERITED_MAX, BATCH = 3152, (4, 5), 1e-3, 0.50, 0.60, 0.50, 32
FORWARDS_MAX = 12
...
def main() -> None:
    ...
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_verb_head_source_fold_result_v218", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "blocks": report, "predictions": predictions, "forwards": forwards,
