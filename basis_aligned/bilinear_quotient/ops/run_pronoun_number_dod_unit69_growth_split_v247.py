#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays_v206 pred_b_factor_closure pred_c_classes_shift_oppositely pred_d_one_factor_dominates pred_e_contrast_grows
"""Pronoun number they/he DoD (v247): WHY unit 69 grows when its feeders are removed. v246: zeroing the verb-site MLP-5 units {1036, 715} at the verb makes MLP-6 unit 69's
plural - singular contrast at the verb grow 2.2x (606 -> 1346) although both units feed it (v245: 52% + 30% of MLP 5's carriage). Two-pass runner (native, edited) reading
u_69 = (L.x)(R.x) at the verb from the normalised block-6 input; the exact change split (i) by FACTOR, delta u = (delta a) b_bar + a_bar (delta b), and (ii) by ROW CLASS
(the mean change of u_69 on plural rows and on singular rows separately). v238's law: a bilinear unit's contrast can grow under the loss of a feeder if the two classes
shift differently through its factors.
PREDICTIONS (scored as written; failures preserved)
    pred_a_baseline_replays_v206     native pooled they - he margin = 196.62 within relative 1e-3
    pred_b_factor_closure            the two factor terms sum to the exact change of u_69 within relative 1e-3, every row
    pred_c_classes_shift_oppositely  the mean change of u_69 on plural rows and on singular rows have opposite signs
    pred_d_one_factor_dominates      the larger |pooled factor term| is >= 1.5 x the smaller
    pred_e_contrast_grows            pooled delta u_69 / native contrast > 0 (the contrast grows, reproducing v246's +122% in place)
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_unit69_growth_split_v247_result.json"
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_unit69_growth_split_v247"
UNITS, LAYER, UNIT, U1779, V206_MARGIN, CLOSURE_TOL, DOMINATE, BATCH = (1036, 715), 5, 829, 69, 196.62, 1e-3, 1.5, 32
FORWARDS_MAX = 8
PREDICTIONS = {"pred_a_baseline_replays_v206": "<= 1e-3", "pred_b_factor_closure": "<= 1e-3", "pred_c_classes_shift_oppositely": "opposite signs", "pred_d_one_factor_dominates": ">= 1.5 x", "pred_e_contrast_grows": "> 0"}
    ...
    with torch.no_grad():
        ...
        for l, block in enumerate(blocks):
            ...
            if l == LAYER:
                ...
                m = block.mlp(xin)
            if l == 6:
                for i in range(len(chunk)): out[i]["xin7"] = xin[i, pos[i]].float().clone()      # key kept from v235: the read block's normalised input (block 6 here)
            if l == 8:
...
def main() -> None:
    ...
    target = [native[i]["h_trio"] * 0 for i, row in enumerate(rows)]      # ZEROING at the verb (v246)
    edited = []
    for start in range(0, len(rows), BATCH):
    ...
    mlp7 = model.transformer.h[6].mlp; L7, R7 = mlp7.Left.weight.detach().float()[U1779], mlp7.Right.weight.detach().float()[U1779]   # names kept: the read unit is MLP-6 unit 69
    per_row = []
    ...
    plural_mean = sum(per_row[i]["exact"] for i, j in pairs) / len(pairs); singular_mean = sum(per_row[j]["exact"] for i, j in pairs) / len(pairs)
    report = {"margin_native_pooled": m_native, "margin_drop_fraction": (m_native - m_edit) / m_native, "u829_drop_fraction": (u_native - u_edit) / u_native, "u69_contrast": c1779, "left_factor_term": left, "right_factor_term": right, "exact_change_fraction": exact,
              "plural_mean_change": plural_mean, "singular_mean_change": singular_mean, "max_closure": max(r["closure"] for r in per_row)}
    print({k: round(v, 4) for k, v in report.items()})
    big, small = max(abs(left), abs(right)), min(abs(left), abs(right))
    predictions = {"pred_a_baseline_replays_v206": abs(m_native - V206_MARGIN) / V206_MARGIN <= 1e-3, "pred_b_factor_closure": report["max_closure"] <= CLOSURE_TOL, "pred_c_classes_shift_oppositely": plural_mean * singular_mean < 0, "pred_d_one_factor_dominates": big >= DOMINATE * max(small, 1e-9), "pred_e_contrast_grows": exact > 0}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_unit69_growth_split_result_v247", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
