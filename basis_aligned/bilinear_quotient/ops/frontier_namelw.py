"""NAME-CLASS LINEAR MAPS IN THE TAIL DICTIONARIES (rung 139): the worst class was getting a CONSTANT.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2228: name is the frontier's worst class
(+3.134) - and a code-read shows WHY it might be: name is in CONSTN, so every tail-attention dictionary hands
name positions a constant per-class mean vector (only LINK classes get x@LW linear maps). This rung moves
name from CONSTN to LINK in the exact rung-127 build: name gets a fitted D x D linear map at each of the
eight tail sites (the scaffolding attnd fits shift consistently - same-config discipline).

REGISTERED PREDICTIONS (anchors: S2228 name +3.134, census +1.9474, increment +0.3256, L1F +1.5509):
  (a) NAME RECOVERS: dCE(name) <= 2.6 (>= 0.5 of its bill was the constant-value approximation).
  (b) AGGREGATE MOVES: census <= 1.92.
  (c) ISOLATION: |L1F - 1.5509| <= 0.02 AND increment <= 0.36.
NULL: name's bill is upstream of the tail dictionaries (rung 138's motif/tail split will cross-check) - name
unchanged within 0.1. PRICE: +8 linear maps (name at 8 tail sites) = 10,616,832 values.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: name-class linear maps in the tail dicts')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_namelw_results.json'
CA,CB=300,512; R0,R1=120,300
CONSTN={'digit','bclose','sentend','comma','rep'}
...
if __name__=='__main__':
    ...
    print('ARM: rung-127 build with name in LINK (linear maps at 8 tail sites)',flush=True)
    main()
    ...
    pa=PC.get('name',9.9)<=2.6
    pb=agg<=1.92
    pc=abs(L1F-1.5509)<=0.02 and inc<=0.36
    ...
    print(f"(a) name {PC.get('name',9.9):+.4f} <= 2.6: {'HELD' if pa else 'FAILED'}")
    print(f"(b) census {agg:+.4f} <= 1.92: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |L1F {L1F:+.4f} - 1.5509| <= 0.02, increment {inc:+.4f} <= 0.36: {'HELD' if pc else 'FAILED'}")
