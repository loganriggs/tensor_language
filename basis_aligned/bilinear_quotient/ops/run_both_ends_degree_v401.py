#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_closure pred_b_noun_source_carries_a_third pred_c_value_only_term_is_not_the_whole pred_d_third_order_small pred_e_leading_term_replays_across_frames
"""Both-ends contrast + degree expansion at the readers (v401; Logan's first pick among the folding families, 19 Sep).
...
PREDICTIONS (scored as written; failures preserved; priors from v379 / v381)
    ...
    pred_b_noun_source_carries_a_third        for 9.6 the noun-source term carries >= 0.35 of the head's total write contrast on u (panel, pooled)
    pred_c_value_only_term_is_not_the_whole   for 9.6 the value-only (pure copy, degree-1) term is <= 0.60 of the noun-source contrast. Prior: unsure (v379: product effect)
...
OUT = ROOT / "circuits/followups/both_ends_degree_v401_result.json"
CANDIDATE_ID = "both_ends.degree_expansion_v401"
N_HEAD, BATCH = 9, 32
CLOSURE_TOL, INSTR_TOL, NOUN_MIN, VALUE_MAX, THIRD_MAX = 1e-4, 2e-2, 0.35, 0.60, 0.25
READERS = ((9, 6), (12, 4), (15, 1))
FORWARDS_MAX = 4
PREDICTIONS = {"pred_a_closure": "<= 1e-4 / 2e-2", "pred_b_noun_source_carries_a_third": ">= 0.35", "pred_c_value_only_term_is_not_the_whole": "<= 0.60", "pred_d_third_order_small": "<= 0.25 x 3", "pred_e_leading_term_replays_across_frames": "same x 3"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "rows": len(rows), "rows_sha256": L.rows_sha256(rows), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"closure_tol": CLOSURE_TOL, "instr_tol": INSTR_TOL, "noun_min": NOUN_MIN, "value_max": VALUE_MAX, "third_max": THIRD_MAX}}
    ...
    predictions = {"pred_a_closure": closure <= CLOSURE_TOL and instr <= INSTR_TOL, "pred_b_noun_source_carries_a_third": r96["noun_source_share"] >= NOUN_MIN, "pred_c_value_only_term_is_not_the_whole": r96["term_shares"]["value_only"] <= VALUE_MAX,
    ...
    OUT.write_text(json.dumps({"schema": "both_ends_degree_result_v401", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
