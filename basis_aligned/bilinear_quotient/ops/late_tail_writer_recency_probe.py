#!/usr/bin/env python
"""late_tail_writer_recency_probe -- HOW FAR BACK the tail the late MLPs read was written: the late-origin residual is split exactly
(scalar lambda mixing) into one component per late writer block j; each late MLP l reads the top-768 bus core exactly and a tail with
the writers at distance d = l - j inside a window (d <= 0/1/2/4, d > 2/4, all) replaced by their fit-set means. Parent §2777/§2778
(writer kind); this rung asks banded (recent writers) vs spread (accumulated) communication. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_recent_two_blocks_are_a_minority pred_c_writers_five_or_more_blocks_back_carry_a_quarter
#                     pred_d_cost_share_tracks_energy_share pred_e_windows_complementary
...
Preregistration: polynomial_causal/LATE_TAIL_WRITER_RECENCY_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_tail_writer_recency_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_tail_writer_recency_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_TAIL_WRITER_RECENCY_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "early_attn_tail_read_probe_results.json"   # §2789
OUT = ROOT / "late_tail_writer_recency_probe_results.json"
HASHES = {PREREG: "dd88235d45678d1fe01ae62905a59e4bce55987d73151963ee20e2202778e77b", PRIOR: "5f30683d664e7bbd0268f53371409845a85019de8398ae4f79b6f456aa5cd7a3",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_tail_writer_recency_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_SPLIT8_1024 = 0.0374; PRIOR_LATE_MLP_768 = 0.1249; PRIOR_EARLY_TAIL_ONLY = 0.0711; PRIOR_DROP_OWN_ATTN_TAIL = 0.0023; LAM = 1e-2
...
def main():
    ...
    class RecHead(OwnHead):
        ...
        def __call__(self, w, x):
            ...
            return out
    WIN = {"DROP_ALL": "all", "D_LE0": ("le", 0), "D_LE1": ("le", 1), "D_LE2": ("le", 2), "D_LE4": ("le", 4), "D_GT2": ("gt", 2), "D_GT4": ("gt", 4)}
    ...
    en = {nm: 0.0 for nm in WIN}; ed = {d: 0.0 for d in range(len(LB))}; efull = 0.0; ntok = 0
    class MeanRec(RecHead):
        ...
        def __call__(self, w, x):
            ...
            for j in writers(self.l, "all"):
                cj = b["c"][j] * scale
                acc[(self.l, j)] += cj.reshape(-1, D).double().sum(0); ed[self.l - j] += float(perp(cj).pow(2).sum())
            for nm, win in WIN.items():
                js = writers(self.l, win)
                if js:
                    en[nm] += float(perp(sum(b["c"][j] * scale for j in js)).pow(2).sum())
            efull += float(perp(xh).pow(2).sum())
    ...
    edist = {str(d): (ed[d] / en["DROP_ALL"] if en["DROP_ALL"] > 0 else float("nan")) for d in ed}
    log(stage="tail_energy", late_origin_share_of_tail=round(en["DROP_ALL"] / max(efull, 1e-12), 3), window_share={k: round(v, 3) for k, v in eshare.items()}, by_distance={k: round(v, 3) for k, v in edist.items()})
    def rec(win):
        bx = {}
    ...
    summ = {"cost_share": {nm: frac(nm) for nm in WIN if nm != "DROP_ALL"}, "energy_share": eshare, "energy_by_distance": edist,
            "late_origin_share_of_tail_energy": en["DROP_ALL"] / max(efull, 1e-12), "drop_all_over_const": A_ / C0 if abs(C0) > 1e-9 else float("inf"),
            "sum_le2_gt2_over_all": frac("D_LE2") + frac("D_GT2"), "sum_le4_gt4_over_all": frac("D_LE4") + frac("D_GT4"),
            "le2_cost_minus_energy_share": frac("D_LE2") - eshare["D_LE2"]}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(C0 - PRIOR_LATE_MLP_768) <= BARS["repro_tol"]
                   and abs(A_ - PRIOR_EARLY_TAIL_ONLY) <= BARS["repro_tol"] and abs(arms["D_LE0"] - PRIOR_DROP_OWN_ATTN_TAIL) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_recent_two_blocks_are_a_minority': bool(frac("D_LE2") <= BARS["b_frac"]),
        'pred_c_writers_five_or_more_blocks_back_carry_a_quarter': bool(frac("D_GT4") >= BARS["c_frac"]),
        'pred_d_cost_share_tracks_energy_share': bool(abs(summ["le2_cost_minus_energy_share"]) <= BARS["d_gap"]),
        'pred_e_windows_complementary': bool(BARS["e_lo"] <= summ["sum_le2_gt2_over_all"] <= BARS["e_hi"]),
    }
    nulls = {"b_null_le2_ge_.75": bool(frac("D_LE2") >= NULLS["b_frac"]), "c_null_gt4_le_.10": bool(frac("D_GT4") <= NULLS["c_frac"]),
             "d_null_gap_ge_.40": bool(abs(summ["le2_cost_minus_energy_share"]) >= NULLS["d_gap"]), "e_null_sum_ge_2": bool(summ["sum_le2_gt2_over_all"] >= NULLS["e_hi"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "blocks 8-17 MLP reads: core (top-768 of U_8) exact; tail drops the lambda-propagated writes (attention + MLP + bias) of the late blocks at distance d = reader - writer inside the window, each replaced by its fit-set mean",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "late_mlp_768": C0, "prior_late_mlp_768": PRIOR_LATE_MLP_768,
                          "drop_all": A_, "prior_early_tail_only": PRIOR_EARLY_TAIL_ONLY, "d_le0": arms["D_LE0"], "prior_drop_own_attn_tail": PRIOR_DROP_OWN_ATTN_TAIL},
