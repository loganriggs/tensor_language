"""THE COMPUTE-SPARSE CORNER (rung 244): motif-exact patterns + per-token top-1152 MLPs.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The two best orthogonal moves meet:
the registered corner {motifs exact, tail r64} (0.0608 / 9 certs) plus 4x MLP compute sparsity, which on
the r64 base cost +0.016 census and 3 certificates (0.0852/7 -> 0.1013/4). Additive arithmetic: ~0.077.
REGISTERED PREDICTIONS (arms: THIS vs corner 0.0608/9 and topk-r64 0.1013/4):
  (a) NEAR-ADDITIVE: census <= 0.085.
  (b) CERTIFICATES: valid >= 6 (loses ~3 like the r64 base did).
  (c) SANITY: census in [0.01, 0.15]; inert tripwires vs corner AND topk receipts.
NULL: sparsity interacts with motif exactness (census >= 0.10 or valid <= 4). PRICE: corner storage
(~199M) + 4x compute sparsity in swapped MLPs. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: compute-sparse corner')
    raise SystemExit(0)
import torch
...
OUT=PT+'corner_topk_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['cp_topk']=1152; SEL['qk_r']=64; SEL['qk_rmap']={li:128 for li in range(2,10)}; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    pb=nv>=6
    pc=0.01<=agg<=0.15
    if abs(agg-0.0608)<1e-4 or abs(agg-0.1013)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: corner-topk inert')
    ...
    print(f"(b) valid {nv} >= 6; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
