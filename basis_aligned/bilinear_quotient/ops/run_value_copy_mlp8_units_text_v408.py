...
"""MLP 8 at unit grain on natural text, corrected closure (v408; v407 double-counted the Down bias in the reconstruction; ranks were unaffected). v406: MLP 8 writes half of the number the readers copy from the noun, on the 122 natural swapped pairs as
...
PREDICTIONS (scored as written; failures preserved; priors from v168 / v399)
    ...
    pred_e_gender_units_outside_top_20  units 3152 and 3943 (gender, v164) are both outside the top 20. Prior: v407 measured 3152 5th — expected to FAIL again; kept as the registered claim
...
OUT = ROOT / "circuits/followups/value_copy_mlp8_units_text_v408_result.json"
CANDIDATE_ID = "both_ends.value_copy_mlp8_units_text_v408"
...
def main() -> None:
    ...
    OUT.write_text(json.dumps({"schema": "value_copy_mlp8_units_text_result_v408", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
