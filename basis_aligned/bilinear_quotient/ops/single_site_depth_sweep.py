# IS THE FLOOR ABOUT LAYER 1, OR ABOUT ANY SINGLE COMPILED SITE ABOVE THE BOTTOM?
#
# §1829 found the bottom-up recovery curve falls at layer 1 (B0 64.8% -> B1 25.9%, 73.3% of the total
# fall) and is flat from B3 to B7 (1.6pp band). §1830 then found that EITHER site of layer 1 alone
# reaches that floor: attn1 +37.4pp, mlp1 +38.7pp against a joint +38.9pp, additivity 1.96x where 2.00x
# is exact redundancy. Two mechanistically different routes -- the attention arm is rescued +23.6pp by
# §1824's gain correction, the MLP arm is made 3.0pp worse by it -- arriving at the same ~26%.
#
# That leaves two opposite readings of the whole bottom-up record and they have not been separated:
#   (i)  LAYER 1 IS SPECIAL. The early layers carry something the live suffix needs, and compiling any
#        of it is what costs 38.9pp. Depth is the variable, §1829's curve means what it says.
#   (ii) ANY COMPILED SITE ABOVE THE BOTTOM DOES THIS. The floor is reached by inserting one
#        position-wise site anywhere into a live stack, and depth is not the variable at all -- in which
#        case §1829's curve is measuring "is anything compiled above layer 0" and every bottom-up figure
#        in the record needs recasting.
#
# The test is a single-site addition to B0 swept ACROSS DEPTH: B0 plus one compiled attention site at
# layer 1, 2, 3, 5, 9, 13 or 17, plus mlp at 1, 9 and 17 to check the site type behaves the same at
# depth as §1830 found it does at layer 1. This is a CURVE, not a bracket (LESSON 47) -- the mistake
# §1829 was built to correct.
#
...
#   pred_a DEPTH DOES NOT MATTER: the mean sequential recovery over the single-attention arms at layers
#          5, 9, 13 and 17 is within 5pp of the layer-1 arm's 27.4%. If TRUE, one compiled attention
#          site anywhere above a compiled layer 0 reaches the same floor, depth is not the variable, and
#          §1829's depth curve is measuring the presence of a compiled site rather than prefix depth --
#          which recasts every bottom-up figure in the record, including §1829's own. If FALSE the
#          collapse is specific to the shallow layers and layer 1 is a genuine object to attack.
#   pred_b AND SITE TYPE STILL DOES NOT MATTER AT DEPTH: |attn - mlp| sequential recovery is under 5pp
#          at each of layers 1, 9 and 17, replicating §1830's -1.3pp at layer 1 and extending it. If
#          FALSE the two site types diverge with depth, and §1830's interchangeability is a shallow-layer
#          accident rather than a property of compilation.
#   pred_c AND THE COMPILED SET IS MONOTONE: no single-site arm recovers LESS than B1's 25.9%, i.e.
#          compiling one site is never worse than compiling two. If FALSE some single compiled site is
#          worse than a strict superset of it, adding a compiled site can HELP, and recovery is not
#          monotone in the compiled set -- which would undermine reading a depth curve as a difficulty
#          ordering at all.
#   pred_d CONTROLS, cross-run per LESSON 42: B0 and B1 reproduce §1829's PUBLISHED sequential gap
#          fractions (skip7000 64.8% / 25.9%) within 0.03; B0+attn1 and B0+mlp1 reproduce §1830's
#          PUBLISHED 27.4% / 26.1% within 0.03; B0 reproduces §1806's PUBLISHED raw 37.4%. Endpoints
#          must reproduce §1789's FULL-RANK top-1; the placement control must move top-1 under 0.05pp.
...
OUT = PT + 'ops/single_site_depth_sweep_results.json'
...
ATTN_LS = (1, 2, 3, 5, 9, 13, 17)     # the depth sweep; 1 is §1830's published anchor
MLP_LS = (1, 9, 17)                   # site-type check at three depths
ARMS = {'B0': list(B0_SITES), 'B1': B0_SITES + [('attn', 1), ('mlp', 1)]}
for _L in ATTN_LS:
    ARMS[f'B0_attn{_L}'] = B0_SITES + [('attn', _L)]
for _L in MLP_LS:
    ARMS[f'B0_mlp{_L}'] = B0_SITES + [('mlp', _L)]
# §1830's PUBLISHED single-site sequential gap fractions at layer 1
S1830_SITE_GAPFRAC = {'B0_attn1': {'skip7000': 0.274, 'skip11000': 0.278, 'skip1200': 0.279},
                      'B0_mlp1': {'skip7000': 0.261, 'skip11000': 0.267, 'skip1200': 0.258}}
...
def main():
    ...
    print(f'SINGLE-SITE DEPTH SWEEP | one compiled site added to B0, across depth | '
    ...
    deep = [seq[f'B0_attn{L}'] for L in (5, 9, 13, 17)]
    mean_deep = sum(deep) / len(deep)
    anchor = seq['B0_attn1']
    typegap = {L: abs(seq[f'B0_attn{L}'] - seq[f'B0_mlp{L}']) for L in MLP_LS}
    singles = {k: seq[k] for k in ARMS if k not in ('B0', 'B1')}
    worst = min(singles, key=singles.get)
    pa = abs(mean_deep - anchor) <= 0.05
    pb = all(v < 0.05 for v in typegap.values())
    pc = singles[worst] >= seq['B1']
    pd = (all(abs(frac[e]['B0_seq'] - S1829_SEQ_GAPFRAC['B0'][e]) <= 0.03 for e in roles)
          ...
          and all(abs(frac[e][f'{k}_seq'] - S1830_SITE_GAPFRAC[k][e]) <= 0.03
                  for e in roles for k in S1830_SITE_GAPFRAC)
    ...
    print('\n  gap fraction recovered (B0 plus ONE compiled site), all roles:', flush=True)
    for k in ARMS:
        print(f'    {k:12s} ' + '  '.join(
            f'{e} raw {frac[e][f"{k}_raw"]:7.1%} seq {frac[e][f"{k}_seq"]:7.1%}'
            for e in roles), flush=True)
    print(f'\n  THE DEPTH SWEEP ({PICK_ROLE}, sequential, B0 = {seq["B0"]:.1%}, B1 = {seq["B1"]:.1%}):',
          flush=True)
    print('    +attn at L: ' + '  '.join(
        f'L{L} {seq[f"B0_attn{L}"]:.1%}' for L in ATTN_LS), flush=True)
    print('    +mlp  at L: ' + '  '.join(
        f'L{L} {seq[f"B0_mlp{L}"]:.1%}' for L in MLP_LS), flush=True)
    print(f'\n  DEPTH DOES NOT MATTER (deep mean within 5pp of L1) -> {pa}  '
          f'deep mean {mean_deep:.1%} vs L1 {anchor:.1%}, gap {mean_deep - anchor:+.1%}', flush=True)
    print(f'  and SITE TYPE does not matter at depth (<5pp at L1/L9/L17) -> {pb}  ' + '  '.join(
        f'L{L} {typegap[L]:.1%}' for L in MLP_LS), flush=True)
    print(f'  and the compiled set is MONOTONE (no single site below B1) -> {pc}  '
          f'worst single {worst} at {singles[worst]:.1%} vs B1 {seq["B1"]:.1%}', flush=True)
    print(f'  B0/B1 reproduce §1829, layer-1 sites reproduce §1830, endpoints §1789 '
          f'-> control {pd}', flush=True)

    json.dump({'run': 'single_site_depth_sweep', 'role': PICK_ROLE,
               'arms': {k: [list(s2) for s2 in v] for k, v in ARMS.items()},
               'gap_fraction': frac, 'sequential': seq,
               'deep_mean': mean_deep, 'layer1_anchor': anchor,
               'site_type_gap': {str(L): typegap[L] for L in MLP_LS},
               'worst_single_arm': worst, 'worst_single_value': singles[worst],
               'layer_norm_live': ln_live, 'gain_sequential': gseq,
               'predictions': {'pred_a_depth_does_not_matter': bool(pa),
                               'pred_b_site_type_does_not_matter': bool(pb),
                               'pred_c_compiled_set_is_monotone': bool(pc),
