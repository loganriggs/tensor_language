# BISECTING THE RIDGE -- the ~129.7M iso-cost line, coverage 5419, rank 525.
#
...
#   pred_d CONTROLS: coverage is exactly 5419 from fit rows [0:96]; the ceiling is finite and above
...
OUT = PT + 'ops/mid_budget_cov_5419_results.json'
...
TRANKS = (525,)          # None = full rank; §1786/§1813 use this ladder
...
FITLO, FITHI = 0, 96      # the draw giving 5419 covered types   # 5x the fit rows -> a larger covered set
H = m.transformer.h
NCOV = 5419
...
def main():
    ...
    assert NFULL == 5419, f'coverage {NFULL} != expected 5419'
    ...
    print(f'MID BUDGET COV | 5419 types, rank 525, 129.7M line | '
    ...
    pd = (NFULL == 5419 and all(ceilv[e] > livev[e] for e in roles2)
          and all(above[e] > 0 for e in roles2))
    ...
    json.dump({'run': 'mid_budget_cov_5419', 'n_covered': NFULL, 'rank': R,
