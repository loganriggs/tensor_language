"""HOW MUCH OF THE FRONTIER'S MLP STAGE IS THE TOKEN TABLE, AND HOW MUCH IS THE QUADRATIC RESIDUAL? (Claude, LANE 1)

SS2874 asked the analogous question of the ATTENTION stage and got a null: every `attnd` dictionary collapses to one constant for
-0.0001 nats, a5 and its a6 control alike, so nothing there was specific to any layer. The MLP stage is built differently -- both
`fit_tableres` and `fit_res`, its matched-context twin, return a TOKEN-INDEXED TABLE `tb[ids]` plus a LOW-RANK QUADRATIC RESIDUAL `A`
in a 64-dimensional projected subspace -- and that decomposes in a way the attention dictionaries do not.

Three runs of SS312's published pipeline, splitting the MLP stage in half at BOTH construction sites (patching only one would silently
mix the arms):

    ARM 2  A := 0        keep the token table, drop the quadratic residual
    ARM 3  tb := 0       keep the quadratic residual, drop the token table

SIGN CONVENTION (SS2135): frontier L2 is CE ADDED ABOVE THE REAL MODEL, so **LOWER IS BETTER** (SS312: +2.6735 beating +2.84/+2.93).
A drop COST is L2_F(dropped) - L2_F(baseline), **POSITIVE = WORSE**. SS2128/SS2129/SS2133/SS2134 RETRACTED; SS2125 STANDS.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_the_two_halves_are_not_equally_important
#                     pred_c_the_token_table_carries_the_stage
#                     pred_d_the_residual_is_cheap_to_drop
#                     pred_e_both_fitters_are_patched

Preregistration: polynomial_causal/FRONTIER_MLP_TABLE_VS_RESIDUAL_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; three arms (baseline / MLP residual dropped / MLP token table dropped)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_mlp_table_vs_residual_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
MLPMODE={'drop':None}   # None | 'res' | 'table'
CVSNAP={}
SKIP8=64
...
PREREG = PT + '../polynomial_causal/FRONTIER_MLP_TABLE_VS_RESIDUAL_PREREGISTRATION.md'
PREREG_SHA = "6c9e4558835046118bf60c443a12c3d5e6cdadba455fe05a5d9e0275ac4c30b1"
...
BARS = {"reproduce": 0.05, "asymmetry": 0.10, "table_dominates": 0.20, "residual_cheap": 0.10}
NULLS = {"asymmetry_le": 0.02, "residual_cheap_ge": 0.30}
...
if __name__=='__main__':
    ...
    print('ARM 2/3: quadratic residual dropped (A := 0), token tables kept',flush=True)
    MLPMODE['drop']='res'
    r_nores=main()

    print('ARM 3/3: token tables dropped (tb := 0), quadratic residual kept',flush=True)
    MLPMODE['drop']='table'
    r_notab=main()
    MLPMODE['drop']=None

    cost_nores = r_nores['L2_F'] - r_base['L2_F']     # POSITIVE = WORSE (LOWER L2 IS BETTER)
    cost_notab = r_notab['L2_F'] - r_base['L2_F']
    asym = abs(cost_notab - cost_nores)

    pa = abs(r_base['L2_F']-2.6735) <= BARS['reproduce']
    pb = asym >= BARS['asymmetry']
    pc = cost_notab >= BARS['table_dominates']
    pd = cost_nores <= BARS['residual_cheap']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_two_halves_are_not_equally_important':bool(pb),
           'pred_c_the_token_table_carries_the_stage':bool(pc),
           'pred_d_the_residual_is_cheap_to_drop':bool(pd),
           'pred_e_both_fitters_are_patched':True}
    nulls={'b_null_the_halves_are_interchangeable':bool(asym<=NULLS['asymmetry_le']),
           'd_null_the_residual_is_load_bearing':bool(cost_nores>=NULLS['residual_cheap_ge'])}
    res={'rung':'frontier_mlp_table_vs_residual','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,
         'baseline':r_base,'residual_dropped':r_nores,'table_dropped':r_notab,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(r_base['L2_F'],4),
                    'L2_F_residual_dropped':round(r_nores['L2_F'],4),
                    'L2_F_table_dropped':round(r_notab['L2_F'],4),
                    'cost_dropping_residual_nats':round(cost_nores,4),
                    'cost_dropping_table_nats':round(cost_notab,4),
                    'asymmetry_nats':round(asym,4),
                    'L2_C_baseline':round(r_base['L2_C'],4),
                    'L2_C_residual_dropped':round(r_nores['L2_C'],4)},
    ...
    print(f"(b) asymmetry {asym:+.4f} >= {BARS['asymmetry']}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) dropping the TABLE costs {cost_notab:+.4f} >= {BARS['table_dominates']}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) dropping the RESIDUAL costs {cost_nores:+.4f} <= {BARS['residual_cheap']}: {'HELD' if pd else 'FAILED'}")
