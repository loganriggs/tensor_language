#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_701_and_2059_move_the_adjacent_verb pred_c_random_two_units_do_not pred_d_late_units_beat_mlp8_units_here pred_e_values_at_noun_replay
"""The direct path's last units edited (v460). v457-v459: for a verb directly after the noun, MLP 17's units 701 and 2059 carry half of the last block's
agreement write, fed by MLP 16's 575 / 2921. The replace-edit that matches the claim: swap only 701 and 2059's activations at the noun between the plural
and singular rows (43 adjacent pairs; the answer position is the noun) — fraction of the agreement margin gap closed; two random MLP-17 units as the
control; MLP 8's 829 / 953 / 1030 swapped the same way (v446: 0.114 on adjacent rows) for scale; every head's value at the noun (0.26) as the replay.
PREDICTIONS (scored as written; failures preserved; priors from v446 / v457)
    pred_a_baseline_replays                   the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_701_and_2059_move_the_adjacent_verb  the two-unit swap closes >= 0.04 of the agreement margin gap on adjacent rows. Prior: unsure
    pred_c_random_two_units_do_not            two random MLP-17 units (seed 0) close <= 0.01
    pred_d_late_units_beat_mlp8_units_here    the two late units close at least 0.5 x what 829 / 953 / 1030 close on the same rows. Prior: unsure
    pred_e_values_at_noun_replay              every head's value at the noun closes within 0.05 of 0.26 (v446)
...
OUT = ROOT / "circuits/followups/late_units_edit_v460_result.json"
CANDIDATE_ID = "chain.late_units_edit_v460"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, LATE_MIN, RANDOM_MAX, RATIO_MIN, ALL_V446, BAND = 1e-3, 0.04, 0.01, 0.5, 0.26, 0.05
import random
UNIT_SETS = {"LATE": (17, (701, 2059)), "RANDOM": (17, tuple(random.Random(0).sample([j for j in range(4608) if j not in (701, 2059)], 2))), "UNITS8": (8, (829, 953, 1030))}
...
FORWARDS_MAX = 11
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_701_and_2059_move_the_adjacent_verb": ">= 0.04", "pred_c_random_two_units_do_not": "<= 0.01", "pred_d_late_units_beat_mlp8_units_here": ">= 0.5 x", "pred_e_values_at_noun_replay": "0.26 +- 0.05"}
...
def main() -> None:
    ...
    for r_ in recs:
        ...
        vs = r_["second_offset"]
        if vs - c == 1: verb_items.append((plural[:vs], singular[:vs], c))   # adjacent verbs: the answer position is the noun   # truncated so the answer position IS the verb slot (the verb is predicted there)
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "late_min": LATE_MIN, "random_max": RANDOM_MAX, "ratio_min": RATIO_MIN, "all_v446": ALL_V446, "band": BAND}}
    ...
    MODES = {"LATE": ({}, (0,)), "RANDOM": ({}, (0,)), "UNITS8": ({}, (0,)), "ALL_NOUN": (ALL_H, (0,))}; forwards = 0
    def logits_margin(x):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_701_and_2059_move_the_adjacent_verb": v["LATE"] >= LATE_MIN, "pred_c_random_two_units_do_not": abs(v["RANDOM"]) <= RANDOM_MAX,
                   "pred_d_late_units_beat_mlp8_units_here": v["LATE"] >= RATIO_MIN * v["UNITS8"], "pred_e_values_at_noun_replay": abs(v["ALL_NOUN"] - ALL_V446) <= BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "late_units_edit_result_v460", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
