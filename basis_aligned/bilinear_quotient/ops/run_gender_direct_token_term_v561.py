...
"""The direct token term, normalised (v561). v560: with the readers' signed pattern weights the weights-only token term has the right sign and ranking, but its
magnitude is ~30x the token-only singles because the logit is W_U x / rms(x) and the final rms was left out. Here the same native capture plus the final
residual's rms at the answer position per row; D_h = mean over rows of p_h x lamb_l (uO_h) W_v0 dn(e) / rms_final, over the mean he - she gap, against
v559's token-only singles. The direct term is the reader's own write; what the singles carry beyond it is re-expression by the blocks after the reader.
PREDICTIONS (scored as written; failures preserved; priors from v559 / v560; the number line's readers wrote 0.80 of their effect directly)
    ...
    pred_d_signs_agree                        sign(D_h) = sign(single_h) for 10.1, 12.4, 15.1 (replay of v560)
    pred_e_top_two_replay                     the two largest |D_h| are 10.1 and 12.4 (replay of v560)
...
OUT = ROOT / "circuits/followups/gender_direct_token_term_v561_result.json"
CANDIDATE_ID = "gender.direct_token_term_v561"
...
def main() -> None:
    ...
    def study(items, batch):
        ...
        rmsf = P.pop("rms_final"); direct = {k: float((P[k] * tok_rows[k] / rmsf).sum() / (2 * gap.sum())) for k in P}
        return {"pairs": len(items), "gap_mean": float(gap.mean()), "pattern_mean": {k: float(v.mean()) for k, v in P.items()}, "pattern_sign_constancy": {k: float(max((v > 0).float().mean(), (v < 0).float().mean())) for k, v in P.items()},
                "direct_term": direct, "rms_final_mean": float(rmsf.mean()), "native_margins": nat.tolist()}, nat
    ...
    OUT.write_text(json.dumps({"schema": "gender_direct_token_term_result_v561", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
