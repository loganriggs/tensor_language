...
pred_e replay of all prior scores <=1e-5.
...
@torch.no_grad()
def main():
 files=json.loads((P/'SHARED_TAIL_NATIVE_V1_BINDING.json').read_text())['files']
 ...
 out=P/'SHARED_TAIL_NATIVE_V1_RESULT.json';assert not out.exists()
 ...
 result={'pred_a':A,'pred_b':all(g['compressed_relative_errors'][0]<=.02 and g['compressed_relative_errors'][1]<=.05 for g in groups),'pred_c':all(g['incremental_relative_errors'][0]<=.01 and g['incremental_relative_errors'][1]<=.025 for g in groups),'pred_d':all(g['capability']>0 for g in groups),'anchors':anchors,'groups':groups,'cells':cells,'seconds':time.perf_counter()-tic,'scope':'Replay of 48 previously evaluated constructed templates, shared tail cache across contexts; six existing lexical contrasts, live assembled upstream scalar fields; native prefix/background/suffix. Three-term vs exact five-bank and native original interaction. Not independent text-only circuit or corpus OOD.'}
 old_result=json.loads((P/'THREE_TERM_FRESH_TEMPLATE_V1_RESULT.json').read_text())
 replay=max(abs(x-y) for new,old in zip(cells,old_result['cells']) for ns,os_ in zip(new['scores'],old['scores']) for nr,orr in zip(ns,os_) for x,y in zip(nr,orr))
 result['pred_e']=replay<=1e-5;result['prior_score_max_absolute_difference']=replay
