...
"""Source-reuse final-projection oracle at fixed v665 width8.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v680_result.json'
PREDICTIONS=dict(pred_a_exact_replay='baseline and independent joint versus shared chain replay max absolute error <=1e-4',
 ...
 pred_c_capable_position_transfer='final projection oracle retains >=75% of every failed A/B modal error; at least one failed cell required')
...
def main():
    ...
    c=b and bool(failed) and all(o>=.75*e for o,e in failed)

    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,cells=cells,
                predictions=dict(zip(PREDICTIONS,[bool(a),bool(b),bool(c)])),selectivity=selectivity,max_full_state_readout_error=max(oracle_checks),failed_modal_oracle_ratios=[o/e for o,e in failed],reuse_reports=reuse_reports,cross_effects=cross,reuse_counts=reuse_counts,pre11_joint_vs_post11_sum_gap=boundary_gaps,
