"""EIGENBASIS GENERALIZATION (rung 215): does the gauge-free advantage transfer to m14?

CONVENTION (S2135): per-position dCE = CE(m14 replaced, all else real) - CE(real model); LOWER IS BETTER.
S2309 published at m16: eigenfeatures beat neurons 0.770 own / 0.655 census at 3x fewer values. One module
is an anecdote; this runs the identical instrument at m14 (5 own circuits, mid-tail MLP). Exactness bars
follow the S2309 instrument rule (rel-Frobenius + scale-honest max at the known fp32 floor).
ARMS: NEUR = fixed top-1152/4608 neurons at m14; EIG = top-1152 eigenfeatures at m14.
REGISTERED PREDICTIONS:
  (a) EXACTNESS: rel-Frobenius <= 1e-3 AND max/RMS <= 5e-2.
  (b) ADVANTAGE TRANSFERS: own-circuit ratio EIG/NEUR <= 0.9.
  (c) GLOBAL: census ratio <= 0.8.
NULL: the m16 advantage was module-specific (either ratio >= 0.95). PRICE: probe (one module).
Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: m14 eigenbasis generalization'); raise SystemExit(0)
import torch
import torch.nn.functional as F
from bilin18_joint_removal import m, DEV
D=1152
PT='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
OUT=PT+'m14_eigenbasis_results.json'
...
def main():
    ...
    ownt=[t for t,v in CINFO.items() if v['top']=='m14']
    print(f'{len(CINFO)} circuits; m14 owns {len(ownt)}: {ownt}',flush=True)
    mlp=m.transformer.h[14].mlp
    ...
    pa=(frob<=1e-3 and mxs<=5e-2); pb=ra<=0.9; pc=rb<=0.8
