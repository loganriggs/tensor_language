# SWEEP THE MAP, NOT THE TABLES  -- the question §1813 ended on.
#
# §1813 found that at the efficient end the embedding->row MAP is 94.3% of the program's cost: at table
# rank 1 the tables are 0.320M reals and the rank-64 map is 5.308M. The whole table-rank line from
# §1751 onward has been optimising the minority of the bill.
#
# PRIOR ART, checked before building (PRE-FLIGHT A). `ops/map_rank_sweep.py` (§1786) already swept map
# ranks 8 / 16 / 64 / 256 and found a floor: rank 8 scored -0.055 nats against simply copying the
# neighbour's row, and fidelity was monotone in map rank. THIS IS NOT THAT RUN. §1786 measured CE, at
# TABLE RANK 64, where the map was 26% of the cost and its errors were diluted by 15.2M reals of table.
# This measures TOP-1 ACCURACY (the axis §1788 showed diverges from CE), at TABLE RANK 1 where the map
# IS the program, and scores the joint (table rank x map rank) grid on accuracy per real -- which
# nothing has done.
#
# ROLES. skip7000, skip11000, skip1200; table ranks 1, 8, 64 x map ranks 8, 16, 64, 256. All sites
# compiled. DISCOVERY ONLY.
#
...
#   pred_a A CHEAPER MAP IS NEARLY FREE AT TABLE RANK 1: dropping the map from rank 64 to 16 costs less
#          than 1 percentage point of top-1, at every role. That would cut the program from 5.628M to
#          1.647M reals -- a 3.4x saving on the object §1813 identified as 94% of the cost. If FALSE the
#          map cannot be cheapened where it dominates, and the cheap end of the frontier is fixed by
#          the map rather than by anything the table-rank line can reach.
#   pred_b AND THE JOINT OPTIMUM AGREES: the accuracy-per-real optimum over the twelve arms has map
#          rank at most 16, at every role. Scored separately from pred_a because a small accuracy cost
#          at t1 does not settle where the best buy sits once the table rank can also move -- the two
#          knobs interacted at depth in §1812 and may interact here.
#   pred_c BUT §1786'S FLOOR IS REAL: at table rank 64, map rank 8 loses at least 1 percentage point of
#          top-1 against map rank 64, at every role. §1786 found that floor in CE; if it does NOT
#          reproduce on accuracy, then the map's floor is a CE artifact and §1786's rank-64 choice was
#          made on the wrong axis -- which would be a correction to a certified design point.
#   pred_d CONTROLS, cross-run per LESSON 42: t1_m64 reproduces §1813's PUBLISHED 0.0990 / 0.1065 /
#          0.1007 and t64_m64 reproduces §1786's settled 0.1288 / 0.1349 / 0.1289, both within 0.01;
#          the endpoints reproduce §1789's 0.1355 / 0.1425 / 0.1364 and 0.3932 / 0.4235 / 0.3888 within
#          0.001; the t64_m64 cost reproduces §1786's 20.531M; coverage 5419 of 50257.
...
OUT = PT + 'ops/map_rank_at_low_table_results.json'
PROBE_LS = ()
KIND_LS = ()
RANKS = (1, 8, 64)           # table ranks: the efficient end, the middle, §1786's design point
MAPS = (8, 16, 64, 256)      # map ranks, matching §1786's sweep so its figures are comparable
...
S1813_T1_M64_PP = {'skip7000': 0.0990, 'skip11000': 0.1065, 'skip1200': 0.1007}
S1786_T64_M64_PP = {'skip7000': 0.1288, 'skip11000': 0.1349, 'skip1200': 0.1289}
...
def main():
    ...
    print(f'MAP RANK AT LOW TABLE RANK | table ranks {RANKS} x map ranks {MAPS} | '
    ...
    for r in RANKS:
        for mr in MAPS:
            ...
            n = len(sites)
            cost[f't{r}_m{mr}'] = n * (r * (NCOV + D) + 2 * D) + n * mr * 2 * D
            run(f't{r}_m{mr}', sites, hks)
            del frr, hks
            torch.cuda.empty_cache()
    ...
    keys = [f't{r}_m{mr}' for r in RANKS for mr in MAPS]
    ...
    pareto = {e: [k for k in keys
              ...
              for e in roles}
    drop16 = {e: acc[e]['t1_m64'] - acc[e]['t1_m16'] for e in roles}
    floor8 = {e: acc[e]['t64_m64'] - acc[e]['t64_m8'] for e in roles}
    best_map = {e: int(best_eff[e].split('_m')[1]) for e in roles}
    pa = all(drop16[e] < 0.01 for e in roles)
    pb = all(best_map[e] <= 16 for e in roles)
    pc = all(floor8[e] >= 0.01 for e in roles)
    pd = (all(abs(res[e]['all_substituted']['top1'] - S1789_PROG[e]) <= 0.001
              and abs(res[e]['live_model']['top1'] - S1789_LIVE[e]) <= 0.001 for e in roles)
          and all(abs(acc[e]['t1_m64'] - S1813_T1_M64_PP[e]) <= 0.01
                  and abs(acc[e]['t64_m64'] - S1786_T64_M64_PP[e]) <= 0.01 for e in roles)
    ...
    print('\n  absolute accuracy per million reals:', flush=True)
    for e in roles:
        print(f'    {e:10s} ' + '  '.join(
            f't{r}m{mr}:{eff[e][f"t{r}_m{mr}"]:.4f}' for r in RANKS for mr in MAPS), flush=True)
    print(f'\n  at table rank 1, map 64 -> 16 costs <1pp -> {pa}  ' + '  '.join(
        f'{e} {acc[e]["t1_m64"]:.2%} -> {acc[e]["t1_m16"]:.2%} ({-100*drop16[e]:+.2f}pp)'
        for e in roles), flush=True)
    print(f'  the accuracy-per-real optimum has map rank <=16 -> {pb}  ' + '  '.join(
        f'{e} {best_eff[e]} ({eff[e][best_eff[e]]:.4f})' for e in roles), flush=True)
    print(f'  §1786\'s map floor reproduces on accuracy (m8 loses >=1pp at t64) -> {pc}  '
          + '  '.join(f'{e} {acc[e]["t64_m8"]:.2%} vs {acc[e]["t64_m64"]:.2%} '
                      f'({-100*floor8[e]:+.2f}pp)' for e in roles), flush=True)
    ...
    json.dump({'run': 'l5_cliff_probe',
               ...
               'map_drop_64_to_16_at_t1': drop16, 'map_floor_at_t64': floor8,
               'predictions': {'pred_a_map16_costs_under_1pp_at_t1': bool(pa),
                               'pred_b_efficiency_optimum_map_le_16': bool(pb),
                               'pred_c_map_floor_reproduces_on_accuracy': bool(pc),
