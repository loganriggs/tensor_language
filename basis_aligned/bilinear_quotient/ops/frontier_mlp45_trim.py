...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_c69_floor_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; mlp45 trim arms on the c69-576 best x eight windows (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_mlp45_trim_results.json'
...
if __name__=='__main__':
    T00=time.time()
    SEL['mode']='norm'; SEL['K']=2304; SEL['K69']=2304
    print('ARM 1/4: norm-2304 everywhere (anchor)',flush=True)
    r_norm=main()
    SEL['K']=2304; SEL['K69']=576
    print('ARM 2/4: best = mlp45-2304 + c69-576 (S2140 reproduction)',flush=True)
    r_b=main()
    SEL['K']=1728; SEL['K69']=576
    print('ARM 3/4: mlp45-1728 on top',flush=True)
    r_t=main()
    SEL['K']=1152; SEL['K69']=576
    print('ARM 4/4: mlp45-1152 on top (descriptive)',flush=True)
    r_h2=main()
    import statistics as stt
    db=[round(a-b,4) for a,b in zip(r_b['fresh8'],r_norm['fresh8'])]
    dt=[round(a-b,4) for a,b in zip(r_t['fresh8'],r_b['fresh8'])]
    dh=[round(a-b,4) for a,b in zip(r_h2['fresh8'],r_b['fresh8'])]
    pa=stt.median(dt)<=0.015
    pb=sum(g<=0.03 for g in dt)>=6
    pc=abs(stt.median(db)-(-0.0290))<=0.015
    res={'norm2304':r_norm,'best':r_b,'trim1728':r_t,'trim1152':r_h2,
         'per_window_best_vs_norm':db,'per_window_trim1728_vs_best':dt,'per_window_trim1152_vs_best':dh,
         'convention':'L2 = CE above the real model; lower is better; positive entries = damage added',
         'pred_a_quarter_trim_cheap':bool(pa),'pred_b_windows_floor':bool(pb),'pred_c_reproduces_s2140':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'best - norm per window: {db}')
    print(f'trim1728 - best per window: {dt}')
    print(f'trim1152 - best per window: {dh}')
    print(f"(a) mlp45-1728 median {stt.median(dt):+.4f} <= +0.015: {'HELD' if pa else 'FAILED'}")
    print(f"(b) <= +0.03 on {sum(g<=0.03 for g in dt)}/8 (bar 6): {'HELD' if pb else 'FAILED'}")
    print(f"(c) best median {stt.median(db):+.4f} vs -0.0290 (tol 0.015): {'HELD' if pc else 'FAILED'}")
