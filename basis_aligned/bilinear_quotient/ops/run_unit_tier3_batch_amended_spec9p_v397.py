#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the fifty-seventh one spec-authored behaviour, plus one instrument.
"""v397: the AMENDED tier-3 battery over the FIFTY-SEVENTH batch (ONE cell) -- plus one instrument.
A construction class this corpus has never had: the cue is a DETERMINER and the readout is a NOUN'S NUMBER.
v285 2/7, ..., v361 2/4, v365 2/2, v369 2/3 (+instrument each).
WHY ONE CELL, AND WHY THIS ONE. Everything measured today about breadth, transfer and recency came from verb-cue
circuits -- a lexical verb selecting a particle or a preposition. The corpus's other entries are hand-written modules
from earlier work, so there has been no cheap way to ask whether any of it generalises to a different KIND of cue.
    verb_particle / verb_preposition   cue = a lexical verb, readout = a function word (particle or preposition)
    determiner_number_crates           cue = a FUNCTION WORD (several / each), readout = a CONTENT word's NUMBER
`Near the {object} the {agent} inspected several damaged` obliges ` crates`; `... inspected each damaged` obliges
` crate`. The determiner and the readout are separated by an adjective, which is the shared suffix, so the cue is not
the final input token and the candidate bank's matched_final_input_token requirement is met.
A second cell (several/every -> ` lanterns`/` lantern`) was authored and DROPPED before enqueue: the bank refused it
with `punctuation is not its exact standalone continuation token`, a tokenisation failure on the answer pair rather
than a capability failure, and it never entered ops/. One cell is enough to decide whether the class is worth a
family, and authoring three more before knowing that would be the mistake the ledger keeps recording.
CPU capability margins BEFORE enqueue (mean base / mean donor; A1 | A2; dropped rows; floor 1.0):
  determiner_number_crates  A1:-4.03/+3.70 drop0  A2:-3.49/+3.44 drop0
  Strong on both hypotheses with zero dropped rows -- the model does determiner-noun number agreement across an
  intervening adjective. Prior art search over determiner/number/several/each/plural/singular/agreement: 0 events.
Protocol identical to v197/v201/.../v369. finiteness_selection is the instrument and is EXCLUDED from four-row counts.
Batch size ONE; bars are the one-cell bars (1/1/1/1/1).

REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> the new behaviour passes all four rows. Its capability margins are among the strongest in
                           the corpus, but the day's evidence says the floor predicts studiability and not passing --
                           v349's four cells sat at 2.4-3.5 and produced one pass.                        prior 55%
  pred_b_row2           -> MIN held-out per-direction exact-set recovery >= 0.80.                          prior 65%
  pred_c_row3           -> dim A1 CE damage LB975 > 0 and >= 0.10.                                         prior 80%
  pred_d_row4           -> constrained rank-1 own-C CE UB975 <= 0.01. This is the row that fails most often in this
                           corpus -- 76 of 186 misses across every battery receipt are row 4 alone -- and for a new
                           construction with an unfamiliar control it is the one I would expect to bite. prior 50%
  pred_e_row5_amended   -> A2-own / A2 ceiling within [0.5, 1.4] with LB975 > 0.                           prior 70%
  pred_f_instrument     -> finiteness_selection: n_units identical to v197 and extraction_held_dirB within +-0.02.
Accounting: a pass makes this the first entry in a new construction class and earns a breadth pool of its own -- the
same frames the verb-cue entries were tested in, which cost a CPU screen each. A row-4 miss is NOT a null under
today's evidence: v393 recovered nine such cells by pooling, so the follow-up would be a pooled fit rather than an
abandonment. An honest four-row failure on rows 2 and 3 together would say the class needs different units.
An error receipt counts as a miss on every row. ~40 s GPU, ~1 min.
Smoke: V397_SMOKE=<out.json> (CPU, V397_SMOKE_ROWS=4 per split, V397_SMOKE_NAMES=determiner_number_crates).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec9p_v397_result.json"
...
NAMES = {"determiner_number_crates": "determiner_number_crates",
...
BARS = {"ext_min": EXT_MIN, "rem_min": REM_MIN, "c_ub_max": C_UB_MAX, "row5_share_band": [0.5, 1.4], "k_four": 1, "k_row2": 1, "k_row3": 1, "k_row4": 1, "k_row5": 1, "instr_tol": 0.02}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 200000, 6400000


def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v397", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V397_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V397_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V397_SMOKE_NAMES", "determiner_number_crates").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec9p_v397", "candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v397", "bars": BARS,
