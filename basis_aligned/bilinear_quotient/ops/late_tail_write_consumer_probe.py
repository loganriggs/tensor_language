#!/usr/bin/env python
"""late_tail_write_consumer_probe -- WHO CONSUMES the late MLPs' tail writes. §2796 priced the late-MLP tail write at the write site (.1459,
all downstream consumers affected) while the writer-kind probe priced it at the later MLP readers only (.0404); this rung measures the
decomposition directly: the centred tail writes of blocks 8-17's MLPs are accumulated in one lambda-propagated A, and a named consumer set
(later MLPs / later attention / the final readout) computes from x - A while the stream keeps every real write. Arms: NONE (exactness),
ALL (= §2796's write-site drop), MLP_ONLY, ATTN_ONLY, FINAL_ONLY, NOT_FINAL, and FINAL_W{j} (the readout's consumption of writer j alone).
CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_readout_consumes_more_than_later_mlps pred_c_late_attention_consumes_little
#                     pred_d_consumer_prices_additive pred_e_readout_is_the_majority_consumer pred_f_readout_consumption_concentrated_in_blocks_15_17
...
Preregistration: polynomial_causal/LATE_TAIL_WRITE_CONSUMER_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_tail_write_consumer_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_tail_write_consumer_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_TAIL_WRITE_CONSUMER_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_tail_write_origin_probe_results.json"   # §2796
OUT = ROOT / "late_tail_write_consumer_probe_results.json"
HASHES = {PREREG: "15ec293dedd268e5b38a29af93e130154d15c7aa5c37ce1ed40bda6ee7e89b58", PRIOR: "3a9716af474cb0ebe3d6e964ae45ea8d266f800b40741aafab31def03f3389cc",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_tail_write_consumer_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_SPLIT8_1024 = 0.0374; PRIOR_LATE_MLP_768 = 0.1249; PRIOR_EARLY_TAIL_ONLY = 0.0711; PRIOR_ALL_TAILOUT = 0.1459; PRIOR_READERS_ONLY = 0.0404; LAM = 1e-2
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "none_tol": 1e-3, "b_ratio": 1.5, "c_max": 0.03, "d_lo": 0.7, "d_hi": 1.4, "e_frac": 0.5, "f_frac": 0.5}
NULLS = {"b_ratio": 0.8, "c_min": 0.06, "d_lo": 0.5, "d_hi": 2.0, "e_frac": 0.25, "f_frac": 0.3}
...
def main():
    ...
    arms.update({"ALL": consumer(LB, LB, LB, True), "MLP_ONLY": consumer(LB, mlps=LB), "ATTN_ONLY": consumer(LB, attns=LB), "FINAL_ONLY": consumer(LB, final=True),
                 "NOT_FINAL": consumer(LB, LB, LB, False)})
    arms.update({f"FINAL_W{j}": consumer((j,), final=True) for j in LB})
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    def sdiv(a, b):
        return a / b if abs(b) > 1e-9 else float("inf")
    fw = {j: arms[f"FINAL_W{j}"] for j in LB}; fw_sum = sum(fw.values())
    summ = {"final_over_mlp": sdiv(arms["FINAL_ONLY"], arms["MLP_ONLY"]), "attn_only": arms["ATTN_ONLY"],
            "additivity_sum_three_over_all": sdiv(arms["MLP_ONLY"] + arms["ATTN_ONLY"] + arms["FINAL_ONLY"], arms["ALL"]),
            "final_marginal": arms["ALL"] - arms["NOT_FINAL"], "final_marginal_frac": sdiv(arms["ALL"] - arms["NOT_FINAL"], arms["ALL"]),
            "final_by_writer": {str(j): fw[j] for j in LB}, "final_by_writer_sum_over_final_only": sdiv(fw_sum, arms["FINAL_ONLY"]),
            "final_late_writer_frac_15_17": sdiv(sum(fw[j] for j in LB if j >= 15), fw_sum),
            "prior_all_tailout_2796": PRIOR_ALL_TAILOUT, "prior_drop_late_mlp_tail_readers_only": PRIOR_READERS_ONLY, "n_fit_tokens": rec[("n", SPLIT)]}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(arms["NONE"]) <= BARS["none_tol"]
                   and abs(arms["ALL"] - PRIOR_ALL_TAILOUT) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_readout_consumes_more_than_later_mlps': bool(summ["final_over_mlp"] >= BARS["b_ratio"]),
        'pred_c_late_attention_consumes_little': bool(arms["ATTN_ONLY"] <= BARS["c_max"]),
        'pred_d_consumer_prices_additive': bool(BARS["d_lo"] <= summ["additivity_sum_three_over_all"] <= BARS["d_hi"]),
        'pred_e_readout_is_the_majority_consumer': bool(summ["final_marginal_frac"] >= BARS["e_frac"]),
        'pred_f_readout_consumption_concentrated_in_blocks_15_17': bool(summ["final_late_writer_frac_15_17"] >= BARS["f_frac"]),
    }
    nulls = {"b_null_final_le_.8x_mlp": bool(summ["final_over_mlp"] <= NULLS["b_ratio"]), "c_null_attn_ge_.06": bool(arms["ATTN_ONLY"] >= NULLS["c_min"]),
             "d_null_additivity_outside_.5_2": bool(not (NULLS["d_lo"] <= summ["additivity_sum_three_over_all"] <= NULLS["d_hi"])),
             "e_null_final_marginal_le_.25": bool(summ["final_marginal_frac"] <= NULLS["e_frac"]), "f_null_late_frac_le_.3": bool(summ["final_late_writer_frac_15_17"] <= NULLS["f_frac"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "blocks 8-17 MLP tail writes (perp onto the 384 dims outside the bus-768 core, centred by fit means from docs 96-191) are accumulated in one lambda-propagated A; a named consumer set (later MLPs / later attention / final readout) computes from x - A while the stream keeps every real write; ALL reproduces §2796's write-site drop",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "none": arms["NONE"], "all": arms["ALL"], "prior_all_tailout": PRIOR_ALL_TAILOUT},
