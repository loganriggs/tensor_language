"""Embedding-forward folding, rung 33 (v641): the fixed-kernel atlas for LAYERS 6-8.
...
0.03-0.05; at layer 5 the bank jumps to 0.163 and head 5.7 (0.083) is the first content-critical head. Same instrument for layers 6, 7, 8:
...
PREDICTIONS (scored as written; failures preserved)
    ...
    pred_b_cheap_heads_persist   at each of layers 6-8 at least 3 heads have single cost <= 0.01 (positional taps persist mid-model). Prior: unsure
    ...
    pred_d_a_content_critical_head at least one head in layers 6-8 has single cost >= 0.05. Prior: likely
...
OUT = ROOT / "circuits/followups/embedding_forward_kernel_atlas_mid_v641_result.json"
OUT_PT = ROOT / "circuits/followups/embedding_forward_kernel_atlas_mid_v641_kernels.pt"
FIT_ROWS = ROOT / ".rowcache/fineweb_n480_skip80.pt"
EVAL_ROWS = ROOT / ".rowcache/fineweb_n192_skip7000.pt"
NATIVE_V615 = 3.13241
CANDIDATE_ID = "embedding_forward.kernel_atlas_mid_v641"
FORWARDS_MAX = 200
...
def main() -> None:
    ...
    with torch.no_grad():
        ...
        disk_guard.guard_torch_save({f"kbar_{l}_{h}": state["kbar"][(l, h)].cpu() for l in LAYERS for h in range(H)} | {"rowsum": {f"{l}.{h}": v for (l, h), v in rowsum.items()}}, str(OUT_PT), "v641 kernels")
    ...
    OUT.write_text(json.dumps({"schema": "embedding_forward_kernel_atlas_mid_result_v641", "candidate_id": CANDIDATE_ID, "plan": plan,
