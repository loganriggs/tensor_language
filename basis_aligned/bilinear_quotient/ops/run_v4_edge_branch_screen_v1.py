...
"""V4_EDGE_BRANCH_SCREEN_V1_PREREGISTRATION.md; unchanged oracle source edits."""
import os,sys,json,time,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';PLAN=dict(prefix=12,native_suffix=64,joined_suffix=252)
def main():
 ...
 out=A/'v4_edge_branch_screen_v1_result.json';assert not out.exists();choicepath=P/'V4_FINITE_REFERENCE_CHOICES_V1.json';choices=json.loads(choicepath.read_text());prior=json.loads((A/'v4_finite_reference_v1_result.json').read_text());contexts=[];composition=json.loads((A/'v4_selective_composition_v1_result.json').read_text());field_errors=[];programs=[];cores=[];local_errors=[];export=None
 ...
 for context in contexts:
  ...
  for sv,av in [(1.,1.),(.5,1.),(1.,.5),(-1.,1.),(1.,-1.),(2.,1.),(1.,2.)]:
   ...
   branches=(w*beta[:,None,:]).reshape(n,d,3,9).sum(-1)
   assert float((branches.sum(-1)-edge).abs().max())<1e-10
   for support in [(0,),(1,),(2,),(0,1),(0,2),(1,2)]:
    update=branches[:,:,list(support)].sum(-1);chosen=additive.double();chosen[batch,query]+=update;chosen=chosen.float()
    post=chosen+mlp(torch.nn.functional.rms_norm(chosen,(d,)));posts.append(('branch_'+''.join(map(str,support)),post))
 ...
 result=dict(plan=PLAN,counts=counts,max_replay=max(replays),max_local_edge_relative_error=max(local_errors),records=records,predictions=dict(pred_a_instrument=instrument,pred_b_single_branch_sufficient=instrument and any(all(r['passed'] for r in records if r['arm']==f'branch_{i}') for i in range(3)),pred_c_pair_sufficient=instrument and any(all(r['passed'] for r in records if r['arm']==f'branch_{ij}') for ij in ['01','02','12'])),seconds=time.perf_counter()-tic)
