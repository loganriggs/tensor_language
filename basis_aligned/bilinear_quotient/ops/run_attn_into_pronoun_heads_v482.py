#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_5_3_leads_block_5_into_the_pronoun pred_c_9_6_leads_block_9_into_the_pronoun pred_d_one_head_per_block pred_e_singles_add_to_blocks
"""Blocks 5 and 9 into the pronoun slot, by head (v482). v481: the writes into the pronoun slot are led by block 9 (0.39) with a small early relay from
blocks 4-6 (0.11; block 5 0.053). Is the early relay the same second-hop copier 5.3 that serves the verb (v477-v479), and is block 9's write the reader 9.6?
Each head's pre-projection write into the answer position swapped singly for blocks 5 and 9, 122 natural pairs, they - he margin.
PREDICTIONS (scored as written; failures preserved; priors from v477 / v481)
    pred_a_baseline_replays                 the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_5_3_leads_block_5_into_the_pronoun  head 5.3 has the largest single-head effect in block 5. Prior: likely
    pred_c_9_6_leads_block_9_into_the_pronoun  head 9.6 has the largest single-head effect in block 9
    pred_d_one_head_per_block               in each block the largest head carries >= 0.60 of the block's effect (0.053 / 0.391)
...
OUT = ROOT / "circuits/followups/attn_into_pronoun_heads_v482_result.json"
CANDIDATE_ID = "chain.attn_into_pronoun_heads_v482"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, TOP_SHARE_MIN, ADD_TOL, B5, B9, UNIT = 1e-3, 0.60, 0.04, 0.053, 0.391, 701
import random
...
FORWARDS_MAX = 77
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_5_3_leads_block_5_into_the_pronoun": "5.3 first", "pred_c_9_6_leads_block_9_into_the_pronoun": "9.6 first", "pred_d_one_head_per_block": ">= 0.60 x 2", "pred_e_singles_add_to_blocks": "within 0.04 x 2"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "top_share_min": TOP_SHARE_MIN, "add_tol": ADD_TOL, "b5_v481": B5, "b9_v481": B9, "unit": UNIT}}
    ...
    MODES = {f"{b}.{h}": ({}, (0,)) for b in (5, 9) for h in range(N_HEAD)}; forwards = 0
    def logits_margin(x):
    ...
    mc = verb["margin_closed"]; b5 = {m: mc[m] for m in mc if m.startswith("5.")}; b9 = {m: mc[m] for m in mc if m.startswith("9.")}; report["sums"] = {"5": sum(b5.values()), "9": sum(b9.values())}
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_5_3_leads_block_5_into_the_pronoun": max(b5, key=b5.get) == "5.3", "pred_c_9_6_leads_block_9_into_the_pronoun": max(b9, key=b9.get) == "9.6",
                   "pred_d_one_head_per_block": max(b5.values()) >= TOP_SHARE_MIN * B5 and max(b9.values()) >= TOP_SHARE_MIN * B9, "pred_e_singles_add_to_blocks": abs(sum(b5.values()) - B5) <= ADD_TOL and abs(sum(b9.values()) - B9) <= ADD_TOL}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "attn_into_pronoun_heads_result_v482", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
