"""BOOTSTRAP-AVERAGED DICTIONARY FITS (rung 151; math review 1307, move 2): is certificate dispersion estimation noise?

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. If circuit dispersion is partly
ESTIMATION noise in the dictionary fits, averaging B=4 bootstrap-refit parameter sets (each on a random half
of the fit rows; seeded per site) reduces it ~1/B at ZERO storage cost (parameters averaged, not ensembled).
If dispersion is intrinsic (drift-induced), averaging does nothing. Applied to the aXL CV/LW fits in the
exact rung-127 build; per-circuit member abs-dCE compared to the rung-132 receipt.

REGISTERED PREDICTIONS (anchors: rung-132 circuits receipt; census +1.9474):
  (a) ESTIMATION NOISE CONTRIBUTES: median over circuits of member_absdce(boot)/member_absdce(rung-132)
      <= 0.97 (>= 3% dispersion reduction).
  (b) AGGREGATE UNHARMED: census <= 1.97.
  (c) SANITY: increment in [0.25, 0.45] AND census >= 1.4.
NULL: ratio > 0.99 - dispersion is intrinsic; move 2 of the 1307 review closes. PRICE: none (same shapes,
averaged values). Tripwire: INSTRUMENT FAIL if L2CF missing. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: bootstrap-averaged dictionary fits')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_bootavg_results.json'
    ...
    for li in range(10,18):
        ...
        if Wp2 is None:
            ...
            Wp2=torch.linalg.solve(X2.T@X2+lam*torch.eye(D,device=DEV),
                                   X2.T@Yoh)
        if SEL.get('boot_avg'):
            _g=torch.Generator().manual_seed(1000+li)
            _N=Y.shape[0]
            _cvs=[]; _lws={k:[] for k in LINK}
            for _b in range(4):
                _sub=torch.randperm(_N,generator=_g)[:_N//2].to(DEV)
                _Yb=Y[_sub]; _Xb=X2[_sub]; _fb=flatA[_sub]
                _cvs.append(torch.stack([_Yb[_fb==k].mean(0) if (_fb==k).sum()>0
                                         else _Yb.mean(0) for k in range(10)]))
                for k in LINK:
                    _mkb=_fb==k
                    _Xk=_Xb[_mkb]; _Yk=_Yb[_mkb]
                    _l2=1e-2*max(len(_Xk),1)
                    _lws[k].append(torch.linalg.solve(_Xk.T@_Xk+_l2*torch.eye(D,device=DEV),
                                                      _Xk.T@_Yk))
            CV=torch.stack(_cvs).mean(0)
            LW={k:torch.stack(_lws[k]).mean(0) for k in LINK}
        else:
            CV=torch.stack([Y[flatA==k].mean(0) if (flatA==k).sum()>0
                            else Y.mean(0) for k in range(10)])
            LW={}
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['boot_avg']=True
    print('ARM: rung-127 build + bootstrap-averaged aXL dictionary fits (B=4)',flush=True)
    main()
    ...
    RATB=[r['member_absdce']/max(R132.get(r['tag'],9e9),1e-9) for r in rows if r['tag'] in R132]
    medB=stt.median(RATB)
    print(f'  median member-absdce ratio vs rung-132 baseline: {medB:.4f} over {len(RATB)} circuits',flush=True)
    pa=medB<=0.97
    pb=agg<=1.97
    ...
    print(f"(a) median dispersion ratio {medB:.4f} <= 0.97: {'HELD' if pa else 'FAILED'}")
    print(f"(b) census {agg:+.4f} <= 1.97: {'HELD' if pb else 'FAILED'}")
    print(f"(c) increment {inc:+.4f} in [0.25, 0.45], census >= 1.4: {'HELD' if pc else 'FAILED'}")
