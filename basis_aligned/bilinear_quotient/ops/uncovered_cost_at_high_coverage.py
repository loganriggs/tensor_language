...
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY. Rung 3 -- §2030's open question, at 16,110.
import os
import sys
...
LO, HI = 'c16110', 'c16110'   # S2031: the same decomposition at the higher coverage
...
UNC_TOTAL = (-0.734, -0.790, -4.693)   # S2029 at 5,419; NOT expected to hold here
...
def _the_total_reproduces(x):
    """the deficit at 16,110 is NOT §2029's 5,419 figure on any role -- differing by more than 0.05
    milli-nats. The uncovered arm halves between the coverages, so a deficit that reproduced exactly
    would mean it is not about the uncovered rows at all"""
...
def _the_tables_are_not(x):
    """and §2020's table raise is positive at uncovered inputs on >=2 roles here too -- at 5,419 it was
    +0.316 / +0.130 / -3.837, the deficit falling only on the half-sized skip1200. If the same two roles
    gain here, the skip1200 deficit is a property of that ROLE; if the pattern moves, it is a property of
    the uncovered arm's SIZE, which halves at this coverage"""
...
B.run(
    name='which_change_costs_the_uncovered_arm_hi',
    plan=PLAN,
    coverages=[(LO, B.FIT_16110, 16110)],
    predicates=[
