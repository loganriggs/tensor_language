"""FRAME DIAGNOSIS (rung 270): a16 knockout with the CONFIG-FRAME mean.

CONVENTION (S2135): per-position dCE = CE(mixed config, a16 ablated with the compiled model's OWN a16
mean) - CE(real model); LOWER IS BETTER. S2368/S2369: real-frame ablation means at replaced attention
sites produce an overshoot signature (own 2.40/1.74, global undershoot, compression fingerprint) - the
S2249 frame-mixing hazard in intervention form, hypothesized. Here the mean is captured from the COMPILED
a16 output with the full config active (64 FW rows, disjoint from census) - a config-frame statistic.
REGISTERED PREDICTIONS (arms: THIS vs real-frame a16 receipt own 2.404 / rho 0.727):
  (a) CONFOUND RESOLVED: own-family median ratio in [0.7, 1.6].
  (b) FINGERPRINT FLIPS to the knockout's: era Spearman vs rung-132 <= 0.45.
  (c) SANITY: ablated census in [0.08, 0.8] AND collateral Spearman >= 0.4.
NULL: overshoot persists (own > 1.8) - the distortion is the replaced-path dynamics, not the mean's frame.
PRICE: probe (+1 capture pass). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: a16 knockout, config-frame mean')
    raise SystemExit(0)
import torch
...
OUT=PT+'ko_a16_cfgmean_results.json'
...
def main():
    ...
    if SEL.get('clsdmg'):
        ...
        _cap2={'s':torch.zeros(D,device=DEV),'n':0}
        def _mc2(mo,i_,o_):
            _cap2['s']+=o_[0].detach().float().reshape(-1,D).sum(0); _cap2['n']+=o_[0].reshape(-1,D).shape[0]
        _h2=m.transformer.h[16].attn.register_forward_hook(_mc2)
        ...
        def _ab2(mo,i_,o_):
            ...
            y,v1=o_
            return (_mu2.expand_as(y).to(y.dtype),v1)
        SEL['abl_on']=True
        m.transformer.h[16].attn.register_forward_hook(_ab2)
...
if __name__=='__main__':
    ...
    _hmu=m.transformer.h[16].attn.register_forward_hook(_mucap)
    with torch.no_grad():
    ...
    SEL['_ablh']=_ablh
    print('a16 attn mean captured over FW; gated ablation hook armed',flush=True)
    main()
    ...
    _own=sorted(r['member_absdce']/max(r['ref'],1e-9) for r in rows if _topK.get(r['tag'])=='a16')
    _omed=_own[len(_own)//2]
    _MR=json.load(open(PT+'removal_matrix_results.json'))['matrix']['a16']
    ...
    print(f"(b) FINGERPRINT FLIPS - era rho {rho:.4f} <= 0.45; collateral {_rhoK:.3f}; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) ablated census {agg:+.4f} in [0.08, 0.8]; era rho {rho:.4f}: {'HELD' if pc else 'FAILED'}")
