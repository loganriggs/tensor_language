#!/usr/bin/env python3
# BQLANE: gpu
# BQGATE: 66forwards2112seq; downstream response-restoration screen;0fits.
"""A instrument; B held-out contextual mediator; C localized response."""
import hashlib
import json
import os
import signal
...
BINDING = POLY / "INDUCTION_CONTEXTUAL_CONSUMER_RESPONSE_V1_BINDING.json"
OUT = POLY / "INDUCTION_CONTEXTUAL_CONSUMER_RESPONSE_V1_RESULT.json"
CANDIDATES = ("mlp8",) + tuple(
    name for layer in range(9, 18) for name in (f"attn{layer}", f"mlp{layer}")
)
BATCH = 32
...
def execute(runtime, tokens, specs, target, native_writes, restore_name=None):
    torch = runtime.torch
    device = next(runtime.model.parameters()).device
    token_tensor = torch.as_tensor(tokens, dtype=torch.long, device=device)
    ...
    restore_index = None if restore_name is None else module_index(restore_name)
    ...
    def attention(event):
        ...
        if index == restore_index:
            ...
            for local, spec in enumerate(specs):
                ...
                restored[local] = float((new - old).double().norm().detach().cpu())
    ...
    return selected, active, restored, observation
...
def main():
    ...
    assert len(CANDIDATES) == 19 and binding["price"] == {"forwards": 66, "sequences": 2112}
    if REQUESTED_DRY:
        print(json.dumps({
        ...
        for name, target, candidate in (
            ...
            for start in range(0, len(rows), BATCH):
                ...
                logits, act, response, observation = execute(
                    ...
                    native_writes[start:stop], candidate,
                )
    ...
    for name in CANDIDATES:
        ...
        if passed:
            eligible.append((min(reductions.values()), -CANDIDATES.index(name), name))
    ...
    if selected is not None:
        ...
        for name in neighbors:
            neighbor_passes[name] = passes(confirm_reports[name], confirm_reports["joint"])[0]
    localized = bool(confirm_pass and not any(neighbor_passes.values()))
    instrument = (
        counts == [66, 2112]
    ...
    result = {
        "experiment": "induction_contextual_consumer_response_v1",
        "terminal": "complete" if instrument else "invalid_instrument",
        "predictions": {
            "pred_a_instrument": bool(instrument),
            "pred_b_confirm_contextual_mediator": bool(confirm_pass),
            "pred_c_localized_response": localized,
        },
        ...
        "discovery": {
            ...
            "eligible_candidates": [item[2] for item in sorted(eligible, reverse=True)],
        ...
        "claim_boundary": "Native-oracle complete-write response-restoration screen on frozen FIT subset; no unique consumer, extraction, SELECT/FINAL/OOD, or compression claim.",
