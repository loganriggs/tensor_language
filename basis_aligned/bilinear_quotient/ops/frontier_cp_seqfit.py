"""SEQUENTIAL-CONDITIONING REFIT OF THE CP FRONT (rung 116): the tables' 1.29x regime vs CP's 3.3x.

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model); LOWER IS BETTER. S2207: the CP-front
excess is a progressive ladder (each site's marginal is 1.0/2.5/5.8/5.8x its single). The table front was fit
SEQUENTIALLY (each site in the drifted prefix frame, S2198 compounding only 1.29x); plain CP is weights-only
and has no conditioning. Here each cf_i's Down readout (+bias) is refit in the drifted prefix frame on census
TRAIN rows (0-499 every 2nd, 250 rows), chained: refitted cf0 is installed when fitting cf1, etc. Mixed Ks
(S2206: 2304/2304/1152/576). Certificates on the census TEST half only (rows 500-999; full-census battery
refs; circuits with >= 20 test members).

ARMS (test half): plain = unrefit mixed-K cp_front; seq_traj = Down refit to the REAL-TRAJECTORY output
(position-aligned real m_li output - steers back toward the real stream); seq_frame = Down refit to
mlp(drifted input) (the tables' in-frame target).

REGISTERED PREDICTIONS:
  (a) CONDITIONING RESTORES THE TABLE REGIME: agg(seq_traj) <= 0.45 x agg(plain).
  (b) VALIDITY APPEARS: valid(seq_traj) >= 5.
  (c) IN-FRAME HELPS + POPULATION: agg(seq_frame) <= agg(plain) AND |agg(plain) - 0.9427| <= 0.05.
NULL: Down-refit cannot fix drift (the error lives in the unit ACTIVATIONS on the drifted stream, not the
readout). PRICE: none (Down refit swaps values, adds one D-bias per site).
Tripwire: INSTRUMENT FAIL if seq_traj cev is bitwise equal to plain. Self-reviewed."""
import json, sys, time, os
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
if os.environ.get('BQLIB_DRYRUN')=='1':
    _bq='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
    _need=['frontier_cp_prefix_results.json','frontier_cp_front_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: CP-front sequential-conditioning refit')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_cp_seqfit_results.json'
...
def main():
    ...
    if SEL.get('cp_seqfit'):
        _ROWS=SEL.get('ext_rows',FR)
        _TR=_ROWS[:500:2]; _TE=_ROWS[500:]
        ...
        for _li,_K in _KS.items():
            ...
            _Dw=_mlp.Down.weight.detach().float(); _db=_mlp.Down_bias.detach().float()
            _kp=(_Dw.norm(dim=0)*_L.norm(dim=1)*_R.norm(dim=1)).argsort(descending=True)[:_K]
            _W[_li]=(_L[_kp].contiguous(),_R[_kp].contiguous(),_Dw[:,_kp].contiguous(),_db)
            S[f'cf{_li}']=('cp',_li)+_W[_li]
        # real-trajectory targets at m0..m3 on the train rows
        _RT={li:[] for li in (0,1,2,3)}
        hs=[]
        for _li in (0,1,2,3):
            def _mk(li=_li):
                def h(mo,i_,o_): _RT[li].append(o_.detach().reshape(-1,D).float())
                return h
            hs.append(m.transformer.h[_li].mlp.register_forward_hook(_mk()))
        for _i in range(0,_TR.shape[0],4):
            _bb=_TR[_i:_i+4,:257].to(DEV)
            cur['idx']=_bb[:,:-1].contiguous(); cur['mode']='oracle'
            m(cur['idx'],_bb[:,1:].contiguous())
        for h in hs: h.remove()
        _RT={li:torch.cat(v) for li,v in _RT.items()}
        def _chain(_mode):
            # returns dict of refitted cp entries, fitting cf0..cf3 in order with refits installed
            _S2={}
            for _li in (0,1,2,3):
                _Lk,_Rk,_Dk,_db=_W[_li]
                _act=['a0','a1v']+[f'cx{_j}' for _j in range(_li)]
                for _j in range(_li): S[f'cx{_j}']=_S2[_j]
                _Xs=[]; _Ys=[]
                hs=install(_act)
                def _cap(mo,i_,o_):
                    _Xs.append(i_[0].detach().reshape(-1,D).float())
                    _Ys.append(o_.detach().reshape(-1,D).float())
                hs.append(m.transformer.h[_li].mlp.register_forward_hook(_cap))
                for _i in range(0,_TR.shape[0],4):
                    _bb=_TR[_i:_i+4,:257].to(DEV)
                    cur['idx']=_bb[:,:-1].contiguous(); cur['mode']='oracle'
                    m(cur['idx'],_bb[:,1:].contiguous())
                for h in hs: h.remove()
                _Xd=torch.cat(_Xs)
                _Y=_RT[_li] if _mode=='traj' else torch.cat(_Ys)
                _G=(_Xd@_Lk.T)*(_Xd@_Rk.T)
                _Gb=_G.mean(0); _Yb=_Y.mean(0)
                _Gc=_G-_Gb
                _lam=1e-2*len(_G)
                _Wn=torch.linalg.solve(_Gc.T@_Gc+_lam*torch.eye(_G.shape[1],device=DEV),
                                       _Gc.T@(_Y-_Yb))
                _dbn=(_Yb-_Gb@_Wn)
                _S2[_li]=('cp',_li,_Lk,_Rk,_Wn.T.contiguous(),_dbn.contiguous())
                print(f'  {_mode}: refit cf{_li}',flush=True)
            return _S2
        _CH={'seq_traj':_chain('traj'),'seq_frame':_chain('frame')}
        cur['clsmap']=classify2(_TE).to(DEV)
        _arms={}
        _arms['plain']=evalV(_TE,_TE.shape[0],['a0','a1v','cf0','cf1','cf2','cf3'],ML).detach().cpu()
        print('  arm plain done',flush=True)
        for _nm,_S2 in _CH.items():
            for _j in range(4): S[f'cx{_j}']=_S2[_j]
            _arms[_nm]=evalV(_TE,_TE.shape[0],['a0','a1v','cx0','cx1','cx2','cx3'],ML).detach().cpu()
            print(f'  arm {_nm} done',flush=True)
        del cur['clsmap']
        if torch.equal(_arms['seq_traj'],_arms['plain']):
            raise SystemExit('INSTRUMENT FAIL: seq_traj cev bitwise equal to plain')
        SEL['front_result']=_arms
...
if __name__=='__main__':
    ...
    SEL['cp_seqfit']=True; SEL['ext_rows']=CROWS
    print('ARMS: plain / seq_traj / seq_frame CP fronts, TEST half certificates',flush=True)
    main()
    A=SEL['front_result']
    CBT=CBASE[HALF:]
    out={}; dmap={}
    for k,cev in A.items():
        if cev.shape[0]!=HALF: raise SystemExit(f'INSTRUMENT FAIL: {k} cev {cev.shape[0]} != {HALF}')
        d=cev-CBT; dmap[k]=d
        out[k]={'agg':round(float(d.mean()),4)}
    rows=[]
    for t,info in CINFO.items():
        mte=info['mask'][HALF:]
        if int(mte.sum())<20: continue
        r={'tag':t,'ref':round(info['ref'],3)}
        for k in A:
            ma=float(dmap[k][mte].abs().mean())
            r['memberabs_'+k]=round(ma,4); r['valid_'+k]=bool(ma<0.5*info['ref'])
        rows.append(r)
    nv={k:sum(1 for r in rows if r['valid_'+k]) for k in A}
    for k in A: print(f"  {k:10s} agg {out[k]['agg']:+.4f}  valid {nv[k]}/{len(rows)} (test half)",flush=True)
    pa=out['seq_traj']['agg']<=0.45*out['plain']['agg']
    pb=nv['seq_traj']>=5
    pc=(out['seq_frame']['agg']<=out['plain']['agg']) and abs(out['plain']['agg']-ANCH)<=0.05
    res={'arms':out,'valid_counts':nv,'n_scored_circuits':len(rows),'anchor_s2206':ANCH,'circuits':rows,
         'convention':'per-position dCE = CE(config) - CE(real model), census TEST half; full-census refs',
         'pred_a_conditioning_works':bool(pa),'pred_b_validity_appears':bool(pb),'pred_c_frame_and_population':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) seq_traj {out['seq_traj']['agg']:+.4f} <= 0.45 x plain ({0.45*out['plain']['agg']:+.4f}): {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid(seq_traj) {nv['seq_traj']} >= 5: {'HELD' if pb else 'FAILED'}")
    print(f"(c) seq_frame <= plain and plain ~ anchor: {'HELD' if pc else 'FAILED'}")
