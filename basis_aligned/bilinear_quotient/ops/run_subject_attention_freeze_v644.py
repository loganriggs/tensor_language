...
"""Position stress: omit attention12–17 responses with separate edit/readout positions.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v644_result.json'
...
def main():
    ...
    a=max(replay)<=1e-4 and counts==dict(prefix_calls=8,suffix_calls=16)
    ...
    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,cells=cells,
                ...
                scope='New matched-position texts, same-task conditional effect test; six exact attention-write backgrounds plus prefix/first-value ports retained. No computational circuit adoption; template-position transfer tested with capability gate.')
