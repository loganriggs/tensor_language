...
"""v337: DOES A BEHAVIOUR'S IDENTITY CROSS FRAMES? The frame rung of the preposition replication family, five prepared.

WHAT IS SETTLED AND WHAT IS NOT. Inside verb_particle (v321, v325) and again in the preposition class (v333, 5/5
including the exact-pattern conjunction), a behaviour's identity is its (cue word -> readout token) MAPPING and not its
readout pair: cells sharing a whole pair with fresh cues separate (v333: base and twin at fam sib max 0.016 and
0.0039), and a cell reusing both mappings fuses with BOTH donors (0.0921 and 0.1163). Every one of those cells sits in
the SAME sentence frame, so "identity" has so far meant identity within a frame.
THE TWO NEW MEMBERS CHANGE THE FRAME AND NOTHING ELSE:
...
frame_same reuses BOTH of repl_base's cue -> token mappings verbatim while changing the matrix clause and the
parenthetical. It therefore shares its CUES with repl_base and its FRAME with frame_fresh, and the rung asks which of
those two it behaves like. Both passed all four rows at v335 (n14 held 0.897, n10 0.894), so the new frame supports
circuits as well as the old one and a null here cannot be blamed on a weak frame.
INSTRUMENT LIMIT, stated because it bounds the claim: my first draft of this frame removed the parenthetical so the
answer followed the verb directly, and the candidate bank refused every row with `matched_final_input_token` -- base
and donor must end on the same token, which the parenthetical is what guarantees. So the frame change here is the
matrix clause plus a different parenthetical, not an arbitrary sentence shape, and a fusion across THIS change does
not establish fusion across every change.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
    pred_a_new_separable   at least k_new = 1 of the 2 new members is separable. frame_fresh should be separable under
                           either account, so this is close to a floor check on the rung.                 prior 90%
    pred_b_counted_kept    at least k_kept = 1 of the 2 counted members (base, twin) remains separable. If frame_same
                           fuses with base, base can lose separability from the other side -- that is the same finding
                           seen from the donor's end, not a contradiction.                                prior 75%
    pred_c_row4_kept       fam-arm own-C UB975 <= 0.01 on at least k_d = 2 of the 4 evaluated members.    prior 70%
    pred_d_instrument      every evaluated member's own-arm extraction is within 0.03 of its v335/v331 cdas
                           extraction_held. Fails automatically if any member errors.                     prior 90%
    pred_e_cue_beats_frame frame_same's fam-arm leak into repl_base -- with which it shares both cue mappings and NOT
                           the frame -- exceeds its leak into frame_fresh -- with which it shares the frame and NO cue
                           -- by at least cue_over_frame = 0.02:
                           |leak into repl_base| - |leak into frame_fresh| >= 0.02.
                           This is the whole question in one number, it is measured whether or not anything fuses, and
                           it is on the FAM arm because v325's clause failed on the own arm where generic collateral
                           swamps the signal. Worked example: 0.08 into base and 0.01 into fresh gives 0.07 >= 0.02,
                           TRUE, identity travels with the cue mapping across the frame change; 0.01 and 0.06 gives
                           -0.05, FALSE, and identity is frame-bound, in which case every claim I made today acquires
                           the qualifier "within a frame". The 0.02 anchor is the same one v329 used successfully,
                           taken from the two populations v325 measured: smallest genuine cue-donor leak 0.0286,
                           largest non-donor leak 0.0174.                                                 prior 50%
                           I do not know which way this goes and the prior says so.
COUNTING. If frame_same fuses it is +0 and the corpus's inventory is mappings; if it is separable it is +1 on 138 and
the inventory is (frame, mapping) products, which multiplies the available space again but makes every existing
fusion claim frame-local.
BARS = {"cross_max": 0.05, "keep": 0.8, "ext_floor": 0.5, "c_ub_max": 0.01, "k_d": 2, "k_new": 1, "k_kept": 1, "instr_tol": 0.03, "cue_over_frame": 0.02}
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v337_result.json"
...
FAMILIES = {
    "verb_preposition_replication": ["verb_preposition_repl_base", "verb_preposition_repl_twin", "verb_preposition_repl_reuse", "verb_preposition_repl_frame_same", "verb_preposition_repl_frame_fresh"],
}
EVAL = ("verb_preposition_repl_frame_same", "verb_preposition_repl_frame_fresh", "verb_preposition_repl_base", "verb_preposition_repl_twin", )
COUNTED = ("verb_preposition_repl_base", "verb_preposition_repl_twin", )
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES, **v213.NAMES, **v215.NAMES, **v217.NAMES, **v219.NAMES, **v221.NAMES, **v223.NAMES, **v225.NAMES, **v227.NAMES, **v229.NAMES, **v231.NAMES, **v233.NAMES, **v235.NAMES, **v237.NAMES, **v239.NAMES, **v241.NAMES, **v243.NAMES, **v245.NAMES, **v247.NAMES, **v249.NAMES, **v251.NAMES, **v253.NAMES, **v255.NAMES, **v257.NAMES, **v311.NAMES, **v315.NAMES, **v319.NAMES, **v323.NAMES, **v327.NAMES, **v331.NAMES, **v335.NAMES}


def _parent_cdas(n):
    # the parent battery receipt that produced this member's units (each behaviour name is unique across batteries)
    for path in (V335, V331, V327, V323, V319, V315, V311, V257, V255, V253, V251, V249, V247, V245, V243, V241, V239, V237, V235, V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v337", "members": n, "families": list(FAMILIES),
...
def PREDS(R):
    ...
    SAME, BASE, FRESH = ("verb_preposition_repl_frame_same", "verb_preposition_repl_base",
                         "verb_preposition_repl_frame_fresh")
    e = False
    if SAME in good:
        sibs = good[SAME]["arms"]["fam"]["siblings"]
        if BASE in sibs and FRESH in sibs:
            e = abs(sibs[BASE]) - abs(sibs[FRESH]) >= B["cue_over_frame"]
    return {"pred_a_new_separable": bool(a), "pred_b_counted_kept": bool(b), "pred_c_row4_kept": bool(c),
            "pred_d_instrument": bool(d), "pred_e_cue_beats_frame": bool(e)}


def main() -> None:
    if os.environ.get("BQLIB_DRYRUN") or os.environ.get("BQLIB_NO_MODEL"):
        print(json.dumps(_plan(), indent=2, sort_keys=True)); return
    smoke = os.environ.get("V337_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V337_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v337", "candidate_id": "corpus.unit_family_separability_spec_v337",
