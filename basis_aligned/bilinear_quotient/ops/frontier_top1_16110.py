# THE FRONTIER IN TOP-1, NOT JUST CE  -- testing whether §1853-§1855's Pareto structure is metric-bound.
#
# §1852-§1855 measured the position-wise cost/fidelity frontier in ALL-POSITION CROSS-ENTROPY, found four
# of eighteen points dominated -- including full rank at 5,419 and at 9,054 covered types -- and §1855
# confirmed that dominated set is identical on all three roles.
#
# But §1788 established that CE and top-1 come APART for this program: it keeps ~34% of the live model's
# top-1 while recovering ~32% of the CE stake, and the two instruments disagreed about which arms were
# worth having throughout §1786-§1816. Every section from §1829 to §1846 is scored in TOP-1 gap
# fractions; the frontier is scored in CE. If the Pareto structure differs between the two metrics, then
# §1853's headline -- that the baseline build is dominated -- is a statement about CE only, and the
# registry entry needs qualifying.
#
# This measures both metrics on the same forwards, at the frontier's best coverage, across the full
# eight-rank ladder. The other two coverages follow as separate runs if this one shows a divergence.
#
# ROLES. All three. DISCOVERY ONLY.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39,
# anchors on THIS run's covered population only, per the defect §1851 and §1852 both hit:
#   pred_a THE TWO METRICS AGREE ON ORDERING: the rank ordering by all-position top-1 is the same as by
#          all-position CE, on all three roles. If TRUE the frontier is metric-robust at this coverage
#          and §1853-§1855 stand as written. If FALSE the two instruments rank the same builds
#          differently, §1853's dominations are CE-specific, and the registry entry must say so -- which
#          would matter because every cost figure in §1829-§1846 is top-1.
#   pred_b AND TOP-1 SATURATES EARLIER THAN CE: the top-1 gain from rank 256 to full rank is under 25% of
#          the gain from rank 4 to rank 256, i.e. proportionally smaller than CE's. §1788's divergence
#          predicts top-1 is the coarser instrument. If FALSE top-1 keeps improving where CE flattens,
#          and the expensive end of the frontier is worth MORE in top-1 than in CE, not less.
#   pred_c AND THE LADDER IS MONOTONE IN RANK ON BOTH METRICS. If FALSE on top-1 only, top-1 is too noisy
#          at this eval size to price a frontier with, which is itself worth knowing before any future
#          section quotes a top-1 frontier.
#   pred_d CONTROLS: rank 256 reproduces §1852's PUBLISHED CE above the ceiling (+0.09252 / +0.09844 /
#          +0.08049) within 0.005 at the same 16,110-type covered set; full-rank attainment is exact;
...
OUT = PT + 'ops/frontier_top1_16110_results.json'
...
# §1852's PUBLISHED rank-256 CE above the ceiling at THIS run's 16,110-type covered set
S1852_R256_16110 = {'skip7000': 0.09252, 'skip11000': 0.09844, 'skip1200': 0.08049}
NCOV_HIGH = 16110
...
def main():
    ...
    print(f'FRONTIER IN TOP-1 | 16,110 covered types, eight ranks, CE and top-1 | '
    ...
    COSTM = {'4': 7.877, '16': 15.334, '64': 45.163, '256': 164.478, '384': 243.966,
             '512': 323.454, '1024': 641.406, 'full': 673.464}
    pa = above['512']['skip7000'] <= 0.5 * S1852_R256_16110['skip7000']
    pb = not any(COSTM[l2] < COSTM['full']
                 and curve[l2][e]['allpos'] < curve['full'][e]['allpos']
                 for l2 in lad[:-1] for e in roles2)
    pc = all(above[lad[i]][e] <= above[lad[i - 1]][e] + 1e-9
             for e in roles2 for i in range(1, len(lad)))
    pd = (all(abs(above['256'][e] - S1852_R256_16110[e]) <= 0.005 for e in roles2)
          and NFULL == NCOV_HIGH and attain
          and all(ceilv[e] > livev[e] for e in roles2))

    print(f'\n  KNEE at {NFULL} types -- CE above the ceiling, all-position CE, and cost:', flush=True)
    for l2 in lad:
        print(f'    rank {l2:5s} {COSTM[l2]:8.2f}M  ' + '  '.join(
            f'{e} CE {curve[l2][e]["allpos"]:.5f} top1 {curve[l2][e]["top1_allpos"]:.4%}'
            for e in roles2), flush=True)
    print(f'\n  the curve BENDS EARLY (rank 512 <= half of rank 256) -> {pa}  '
          f'512 +{above["512"]["skip7000"]:.5f} vs the '
          f'{0.5 * S1852_R256_16110["skip7000"]:.5f} needed', flush=True)
    print(f'  and NOTHING in 384..1024 dominates full rank -> {pb}', flush=True)
    print(f'  and the ladder stays MONOTONE in rank -> {pc}', flush=True)
    print(f'  rank 256 reproduces §1852, coverage {NFULL}, attainment {attain} '
          f'-> control {pd}', flush=True)

    json.dump({'run': 'frontier_top1_16110', 'cost_M': COSTM, 'attainment_exact': bool(attain), 'n_covered': NFULL, 'ranks': [str(r) for r in TRANKS],
               'ceiling_covered': ceilv, 'live_covered': livev, 'curve': curve,
               'ladder_above_ceiling': above,
               'predictions': {'pred_a_curve_bends_early': bool(pa),
                               'pred_b_nothing_dominates_full': bool(pb),
