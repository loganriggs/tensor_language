...
"""v665: thirteen four-row passers on thirteen UNCOVERED stems -- how much of the backlog is real?

WHAT IS ON DISK, UNUSED. Sweeping every battery receipt for behaviours that already pass all four tier rows and
then removing everything any COUNTED tuple names, and everything whose STEM any counted cell shares, leaves 32
cells on 27 stems the corpus has never counted. They are already lifted: each has a parent receipt with a cdas
extraction_held between 0.87 and 1.05. Nothing has been done with them. This rung takes thirteen, one per stem,
and asks the only question that stands between them and being counted: are they distinct directions?
WHY NOW, AND WHY THIRTEEN IN ONE FAMILY. v663 put seven cells from four different stems in one family and every
leak came back at or under 0.019 against a 0.05 bar. Every fusion this corpus has recorded -- v631's number cells,
v609's numerals -- has been WITHIN one axis. If cross-stem non-fusion is real rather than a small-sample accident,
thirteen cells on thirteen different stems should nearly all separate, and the backlog converts into countable
circuits at roughly 84 GPU-seconds each. If it is an accident, this is where it breaks, and thirteen siblings is a
far harder test than four.
THE SHARPEST SUB-TEST, AND IT IS ADVERSARIAL. Two of the thirteen sit on different stems but share a readout pair
exactly: rather_prefer and let_want_complement both map onto (" carry", " to"). The standing finding is that
margins degrade with MAPPING-SHARING siblings. So this family contains a pair that the stem argument says will
separate and the mapping argument says will fuse. Whichever way that pair lands is informative, and it is the
reason to keep both in rather than tidy one away. case_he_him and gender_object_him_her half-share a readout
(" him") and are the weaker version of the same probe.
HOW IT READS. a TRUE: cross-stem non-fusion holds at scale, thirteen candidates become proposable, and the rule
"author onto an uncovered stem, not onto a crowded axis" is earned rather than asserted. a FALSE: the SET that
fuses is the result -- if it is exactly the mapping-sharing pair, then the readout mapping and not the stem is what
individuates a direction, which contradicts the reading I took from v663 and is the more valuable outcome.
NOTHING IS COUNTED BY THIS FILE. The count stays 139 whatever this returns; separability is a precondition for
proposing, not a count.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
  pred_a_new_separable at least k_new = 10 of the thirteen candidates are separable at cross_max = 0.05 while
                       sitting in one family with each other. Worked example: if eleven candidates are separable
                       and two -- say rather_prefer and let_want_complement -- have a largest sibling leak of 0.08,
                       the count is 11, 11 >= 10 is true, pred_a is TRUE. At nine separable it is FALSE. prior 60%
  pred_b_counted_kept  at least k_kept = 2 of the three counted members stay separable in the same run, so a null
                       on the candidates cannot be blamed on the family. Worked example: possessive_gender and
  ...
  pred_c_row4_kept     at least k_d = 2 members of EVAL keep a family-arm C upper bound at or below c_ub_max = 0.01.
                       Worked example: if twelve of the sixteen evaluated members have arms.fam.C.ce_ub975 at or
                       under 0.01, the count is 12, 12 >= 2 is true, pred_c is TRUE.                  prior 80%
  pred_d_instrument    every EVAL member reproduces its parent battery extraction within instr_tol = 0.03. Worked
                       example: animacy_place_anyone_anywhere scored 1.054 in its parent receipt; an own arm of
                       1.070 is a gap of 0.016 and holds, while one member off by 0.05 makes pred_d FALSE for the
                       whole run. These parents come from thirteen DIFFERENT batteries, so this is the widest
                       instrument check in the sequence and the one most likely to fail.              prior 60%
SCOPE. Thirteen uncounted candidates on thirteen uncovered stems, three counted controls, one family. No counting.
Smoke: V665_SMOKE=<out.json> (CPU) -- smoke proves the code path runs; its leak numbers are computed at reduced
steps and rows and are NOT measurements. v663 taught that the hard way.
"""
...
OUT = ROOT / "circuits/followups/unit_uncovered_stem_backlog_v665_result.json"
...
FAMILIES = {
    "uncovered_stems": ['possessive_gender', 'correlative_or_and', 'number_reflexive_myself_ourselves', 'animacy_place_anyone_anywhere', 'countability_fewer_less', 'equative_result', 'case_he_him', 'durativity_until_by', 'rather_prefer', 'let_want_complement', 'both_either', 'category_selection_because_of', 'numeral_dual_both_all', 'raising_extraposition_happened', 'gender_object_him_her', 'aux_copy_so'],
}
COUNTED_LIST = ['possessive_gender', 'correlative_or_and', 'number_reflexive_myself_ourselves']
COUNTED = tuple(COUNTED_LIST)
EVAL = tuple(COUNTED_LIST) + tuple(['animacy_place_anyone_anywhere', 'countability_fewer_less', 'equative_result', 'case_he_him', 'durativity_until_by', 'rather_prefer', 'let_want_complement', 'both_either', 'category_selection_because_of', 'numeral_dual_both_all', 'raising_extraposition_happened', 'gender_object_him_her', 'aux_copy_so'])
...
BARS = {"cross_max": 0.05, "keep": 0.8, "k_kept": 2, "ext_floor": 0.5, "c_ub_max": 0.01, "k_d": 2, "k_new": 10, "instr_tol": 0.03}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 20000, 640000


def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_uncovered_stem_backlog_v665", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V665_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V665_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    evaluate = [n for n in members if n in EVAL and (not smoke or n in os.environ.get("V665_SMOKE_NAMES", "both_either").split(","))]
    ...
    result = {"predictions": predictions, "schema": "unit_uncovered_stem_backlog_v665", "candidate_id": "corpus.unit_uncovered_stem_backlog_v665",
