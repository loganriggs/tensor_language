# IS §1874's CONCLUSION PREMATURE?  -- the stream oracle at map rank 512, not 64.
#
# §1874 decomposed the deployed fallback's 0.78075 nats into 16.6% fitting, 12.3% input representation
# and 71.1% residual, and concluded the binding constraint is the FUNCTION CLASS or the per-token
# restriction rather than the features. That conclusion has an untested assumption in it: the stream
# oracle was fitted at **map rank 64**, the deployed setting, while §1869 showed rank is a real lever
# inside the linear family (64 -> 512 recovers a fifth to a quarter of the loss).
#
# If rank and input compound, the stream oracle at rank 512 could fall well below §1874's 0.55527 and the
# "function class" reading would be premature -- the residual would still be shrinking under a lever that
# stays inside the linear per-token family. If it barely moves, §1874 stands.
#
# One constant changed from §1874's run, so the difference is attributable to map rank alone.
#
# ROLES. All three, deployed 5,419-type covered set. DISCOVERY ONLY. Rung 3: testing §1874's own weakest
# assumption before proposing anything larger.
#
...
#   pred_a §1874 STANDS: the stream oracle at map rank 512 stays above 0.40 nats on all three roles, so
#          rank does not compound with the richer input enough to overturn the reading that 71% of the
#          deployed loss is function class rather than features or fitting. If FALSE the residual is
#          still falling fast under a within-family lever, §1874's conclusion is premature, and the next
#          move is more rank rather than a different function class.
#   pred_b AND RANK STILL HELPS SOMETHING: it comes in below §1874's PUBLISHED rank-64 stream oracle of
#          +0.55527 / +0.62110 / +0.57685 on all three roles. If FALSE, eight times the map rank buys
#          nothing on this input -- a sharper statement than pred_a's, closing the rank lever for the
#          stream input specifically.
#   pred_c AND THE COVERED ARM IS UNTOUCHED: covered CE identical to the deployed arm within 1e-6. Sixth
#          KNOWN-ANSWER check on this invariant; the previous five each returned exactly 0.00e+00.
#   pred_d CONTROLS: the deployed MAP arm reproduces §1872's PUBLISHED +0.78075 / +0.86225 / +0.83997
#          within 0.005; coverage is exactly 5,419; the covered arm attains its ceiling (a fifteenth
#          confirmation); every uncovered scored token has a ceiling row.
...
OUT = PT + 'ops/oracle_stream_rank512_results.json'
...
MAPRANKS = (512,)
ARMS2 = ('MAP', 'STREAM')          # None = full rank; §1786/§1813 use this ladder
...
S1874_STREAM_R64 = {'skip7000': 0.55527, 'skip11000': 0.62110, 'skip1200': 0.57685}
...
def main():
    ...
    print(f'ORACLE STREAM RANK512 | 5419 types, stream-input oracle at map rank 512 | '
    ...
    def build(n, rank=None, map_rank=MAP_RANK):
        ...
        for st in sites:
            ...
            if BASE['mode'] == 'STREAM':
                ...
                Uo = ORC['rows'][st].double()
                Xo = ORC['stream'][st].double()   # the length-1 stream ENTERING this site
                Ao = (Xo.T @ Xo
                ...
                fr[utoks] = (ORC['stream'][st].double() @ mpo).float()
            else:
                fr[unc] = (Eunc @ mp).float()
            out[st] = fr
    ...
    genshare = {e: (fbloss['MAP'][e] - fbloss['STREAM'][e]) / max(fbloss['MAP'][e], 1e-9)
                for e in roles2}
    ...
    pa = all(fbloss['STREAM'][e] > 0.40 for e in roles2)
    pb = all(fbloss['STREAM'][e] < S1874_STREAM_R64[e] for e in roles2)
    pc = covspread <= 1e-6
    pd = (all(abs(fbloss['MAP'][e] - S1872_MAP_M64[e]) <= 0.005 for e in roles2)
    ...
    print(f'\n  §1874 STANDS (stream oracle at rank 512 still >0.40) -> {pa}  ' + '  '.join(
        f'{e} {fbloss["STREAM"][e]:+.5f}' for e in roles2),
        flush=True)
    print(f'  and RANK still helps (below §1874 rank-64) -> {pb}  ' + '  '.join(
        f'{e} {fbloss["STREAM"][e]:+.5f}' for e in roles2), flush=True)
    ...
    json.dump({'run': 'oracle_stream_rank512', 'n_covered': NFULL, 'arms': list(ARMS2),
               ...
               'predictions': {'pred_a_S1874_stands': bool(pa),
                               'pred_b_rank_still_helps': bool(pb),
