...
pred_b source-only signed margin-effect relative<=.1 against full query+source swap EACH8cells.
pred_c query-only same. Null: opposing query effect remains functionally material.
...
STEM='REGIONAL_QUERY_SOURCE_EFFECT_V1'
@torch.no_grad()
def main():
 binding=json.loads((P/(STEM+'_BINDING.json')).read_text())['files'];assert all(digest(k)==v for k,v in binding.items())
 a=torch.load(P/'REGIONAL_SOURCE_POSITIONS_V1_ARTIFACT.pt',weights_only=True,map_location='cpu');b=torch.load(P/'REGIONAL_QUERY_SOURCE_ACCOUNTING_V1_ARTIFACT.pt',weights_only=True,map_location='cpu')
 ...
 assert b['hybrid_writes'].shape==(2,2,96,1152)
 ...
 for k,bit in enumerate((1,2)):
  ...
  effects.append(torch.stack([margins(base[donor]-base)-baseline,margins(b['hybrid_writes'][k,0]-base)-baseline,margins(b['hybrid_writes'][k,1]-base)-baseline]))
 effects=torch.stack(effects)
 def rel(x,y):return float((x-y).norm()/y.norm().clamp_min(1e-30))
 replay=rel(baseline,a['baseline_margins']);cells=[]
 for k,cue in enumerate(('editor','tourist')):
  for assignment in range(2):
   for family in range(2):
    ...
    cells.append(dict(cue=cue,assignment=assignment,family=family,source_only_error=rel(v[2],v[0]),query_only_error=rel(v[1],v[0]),full_effect_norm=float(v[0].norm()),full_direction_positive=int((v[0]*direction>0).sum()),source_direction_positive=int((v[2]*direction>0).sum()),query_direction_positive=int((v[1]*direction>0).sum()),nonlinear_interaction_relative=rel(v[1]+v[2],v[0])))
 aa=replay<=1e-5;result={'pred_a':aa,'pred_b':aa and all(c['source_only_error']<=.1 for c in cells),'pred_c':aa and all(c['query_only_error']<=.1 for c in cells),'baseline_replay':replay,'cells':cells,'seconds':time.perf_counter()-tic,'body_forwards':0,'source_shas':binding,'scope':'Conditional component two-port replacements in fixed recipient native suffix. Does not regenerate upstream states or establish broad OOD.'}
