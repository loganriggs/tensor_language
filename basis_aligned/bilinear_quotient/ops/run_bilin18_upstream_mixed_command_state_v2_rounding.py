...
V2 FP32 per-coordinate rounding envelopes/relative1e-5, causal order,
...
import command_state_rounding_audit as Q
from circuit_fast_screen_managed_runner import atomic_create_json

P=N.P;C=N.D.C;R=N.R;M=N.M;POLY=P.POLY;RUNNER=Path(__file__).resolve()
PRIOR=POLY/'BILIN18_UPSTREAM_MIXED_COMMAND_STATE_V2_ROUNDING_PREREGISTRATION.md'
OUT=POLY/'BILIN18_UPSTREAM_MIXED_COMMAND_STATE_V2_ROUNDING_RESULT.json'
FILES={'prior':PRIOR,'parent':N.OUT,'parent_runner':Path(N.__file__),'primitive':Path(S.__file__),'rounding':Path(Q.__file__),'invalid_v1':POLY/'BILIN18_UPSTREAM_MIXED_COMMAND_STATE_V1_RESULT.json'}
EXPECTED={'prior': '766ba4a604b6280c7bf2b7c203e5298c22189f2d742cb1b0b7c9aefdcb950b28', 'parent': 'b10ef0d68c7e635bcb00bbd6d62ff195dfdbe21ef0700ac26ca7ffb5f83d5581', 'parent_runner': 'c37eb7776bc41d7c3d08d2acb4bed8dbedf02c208e6e324f6f9aaf4767ca963a', 'primitive': 'f24768ca40941e198c69e57bbb9b8dd9e1a48e38e8f02ba7c1b077b12e219cf7', 'rounding': 'e12bc01132aa6706d8546fdfebc61e8aa62e559b402a3a211aa2edb969132de3', 'invalid_v1': '272cb7f20b05519a39424305f3395b38302b5407805605fa7732ae7fd2df8f39'}
...
def main():
    ...
    controls={'state':S.controls(),'rounding':Q.controls()};assert all(c['passed'] for c in controls.values());rounding_audits=[];records=[];deployed=[];exact=[];causal=[];removed=0.;future=0.;forwards=0;layers=0;finite=True;restored=True
    with torch.inference_mode():
        for row in rows:
            ...
            rounding=Q.audit(ideal,after,cells);rounding_audits.append(rounding)
            for uv in ('00','10','01'):
                deployed.append(agreement(am[uv],bm[uv],atol=rounding['max_mode_envelope']+rounding['max_slack']));exact.append(M.correspondence(im[uv]+bm['00'],bm[uv]+bm['00']))
    ...
    a=finite and restored and forwards==64 and layers==192 and len(records)==32 and all(x['passed'] for x in rounding_audits) and future<=1e-8 and all(x['passed'] for x in deployed+exact+causal+replays)
    ...
    result={'scope':'Opened cross-cell residual intervention, not natural token edit or structurally reduced variable.',
        ...
        'rounding_audits':rounding_audits,'controls':controls,'summaries':summaries,'records':records,'authority_sha256':observed,'runner_sha256':P.sha(RUNNER),
