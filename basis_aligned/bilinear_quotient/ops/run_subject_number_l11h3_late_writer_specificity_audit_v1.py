...
"""Pruning and empirical-rank audit of the late-writer OOD graph."""
...
PARENT_RESULT = POLY / "SUBJECT_NUMBER_L11H3_LATE_WRITER_OOD_MOBIUS_V1_RESULT.json"
PREREG = POLY / "SUBJECT_NUMBER_L11H3_LATE_WRITER_SPECIFICITY_AUDIT_V1_PREREGISTRATION.md"
BINDING = POLY / "SUBJECT_NUMBER_L11H3_LATE_WRITER_SPECIFICITY_AUDIT_V1_BINDING.json"
OUT = POLY / "SUBJECT_NUMBER_L11H3_LATE_WRITER_SPECIFICITY_AUDIT_V1_RESULT.json"
LAYER, HEAD, HEAD_WIDTH = 11, 3, 128; NULLS, SEED, STEPS = 64, 20260928, 6
PORTS = parent.PORTS; EXPECTED_MASKS = (4, 1, 64, 8, 24, 80); THREE_MASKS = EXPECTED_MASKS[:3]
BARS = {"maximum_absolute_closure_error": 1e-4, "maximum_closed_reconstruction_error": 1e-10,
        ...
        "maximum_three_term_panel_relative_l2": .25, "maximum_three_term_error_increase": .02,
        "minimum_aligned_recovery": 0., "minimum_specificity_percentile": .90}
PRICE = {"partial_forwards": 4, "sequences": 256, "corners": 256, "mobius_terms": 255,
         "candidate_terms": 36, "readouts": 65, "greedy_steps": 6, "behavior_logits": 0,
...
def load_bound():
    ...
    if decoder["terminal"] != "embedding_number_decoder_frozen" or rank["terminal"] != "rank1_frozen_weights_only" \
            or parent_result["terminal"] != "specific_late_writer_ood_graph" \
...
def plan():
    _, decoder, rank, parent_result, rows = load_bound(); panels = parent.panel_indices(rows)
    return {"schema": "subject_number_l11h3_late_writer_specificity_audit_v1_plan", "model_loaded": False,
            ...
            "nulls": NULLS, "parent_terminal": parent_result["terminal"], "rank": rank["rank"],
...
def main():
    ...
    signal.alarm(900); binding, decoder, rank, parent_result, rows = load_bound(); panels = parent.panel_indices(rows)
    ...
    all_atoms = np.zeros((readouts, 255, len(rows)), dtype=np.float32)
    all_target = np.zeros((readouts, len(rows)), dtype=np.float32)
    ...
    expected = [m - 1 for m in EXPECTED_MASKS]; three = [m - 1 for m in THREE_MASKS]
    # Advanced indexing transposes the last expression; normalize to terms x rows.
    if all_atoms[0, :, panels["discovery"]].shape[0] == len(panels["discovery"]):
        discovery_atoms = all_atoms[0, :, panels["discovery"]].T
    else:
        discovery_atoms = all_atoms[0, :, panels["discovery"]]
    reproduced, curve = parent.select_terms(all_target[0, panels["discovery"]], discovery_atoms, candidates)
    ...
    for ai in range(1, readouts):
        disc = panels["discovery"]
        disc_atoms = all_atoms[ai, :, disc]
        if disc_atoms.shape[0] == len(disc): disc_atoms = disc_atoms.T
        selected, _ = parent.select_terms(all_target[ai, disc], disc_atoms, candidates)
    ...
    pred_b = bool(pred_a and all(three_pass))
    pred_c = bool(pred_b and max(increases.values()) <= BARS["maximum_three_term_error_increase"])
    ...
    result = {"schema": "subject_number_l11h3_late_writer_specificity_audit_v1_result", "terminal": terminal,
        ...
        "scope": "Prospective three-term pruning and 64-direction empirical-rank audit of the frozen V1 late-writer graph."}
    managed.atomic_create_json(OUT, result)
    print(json.dumps({k: result[k] for k in ("terminal", "predictions", "instrument", "target_writer", "specificity")}, indent=2, sort_keys=True))
    assert pred_a
