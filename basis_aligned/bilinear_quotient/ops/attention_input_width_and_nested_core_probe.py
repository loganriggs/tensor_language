#!/usr/bin/env python
"""attention_input_width_and_nested_core_probe -- attention's input-width curve (late seven at k = 64..768; all 18 at 256/128) and the
NESTED late program (MLPs read the first 768 columns of the joint core, attention the first 256/128 of the same core). Own weights,
own means, constant filler (§2745 extension). CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_attention_256 pred_c_attention_128 pred_d_all_attention_256 pred_e_nested_is_free
...
Preregistration: polynomial_causal/ATTENTION_INPUT_WIDTH_AND_NESTED_CORE_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/attention_input_width_and_nested_core_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("attention_input_width_and_nested_core_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "ATTENTION_INPUT_WIDTH_AND_NESTED_CORE_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_attention_shared_input_core_probe_results.json"   # §2745
OUT = ROOT / "attention_input_width_and_nested_core_probe_results.json"
HASHES = {PREREG: "87aeb83a20d11879a2af5eedffda26b78c15925d7ee770c76e3f1c1db3629ff8", PRIOR: "0711ac84c85961cf8263200e6fbec0e629dda8e85d524651c733f2a6941c0abe",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "attention_input_width_and_nested_core_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_ATTN7_OWN_768 = 0.0075; PRIOR_LATE14_JOINT_768 = 0.109; LAM = 1e-2
K = 768; KA = [64, 128, 256, 512]; KN = [256, 128]; LATE = list(range(11, 18)); ALL = list(range(NL))
SITES = [("mlp", l) for l in LATE]; ASITES = [("attn", l) for l in ALL]; ALATE = [("attn", l) for l in LATE]
BARS = {"ce_tol": 1e-4, "repro_tol_attn": 0.01, "repro_tol": 0.02, "b_max": 0.05, "c_max": 0.15, "d_max": 0.15, "e_max": 0.03}
NULLS = {"b_min": 0.15, "c_min": 0.40, "d_min": 0.40, "e_min": 0.10}
...
def main():
    ...
    U_joint = core_of(SITES + ALATE)
    log(stage="cores", eff_rank={f"{s_[0]}{s_[1]}": round(heads[s_]["eff_rank"], 1) for s_ in ASITES})
    def run(parts):
        patch = {}; objs = []
        for p_ in parts:
    ...
    ablk = lambda ls, k, U_=None: {("attn", l): AttnHead(l, k, U_) for l in ls}
    arms = {"ATTN7_OWN_768": run([ablk(LATE, K)])}
    for k in KA:
        arms[f"ATTN7_OWN_{k}"] = run([ablk(LATE, k)])
    arms["ATTN18_OWN_256"] = run([ablk(ALL, 256)]); arms["ATTN18_OWN_128"] = run([ablk(ALL, 128)])
    arms["LATE14_JOINT_768"] = run([mblk(K, U_joint), ablk(LATE, K, U_joint)])
    for k in KN:
        arms[f"LATE14_NESTED_768_{k}"] = run([mblk(K, U_joint), ablk(LATE, k, U_joint)])
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    gaps = {f"nested_{k}_minus_joint": arms[f"LATE14_NESTED_768_{k}"] - arms["LATE14_JOINT_768"] for k in KN}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["ATTN7_OWN_768"] - PRIOR_ATTN7_OWN_768) <= BARS["repro_tol_attn"] and abs(arms["LATE14_JOINT_768"] - PRIOR_LATE14_JOINT_768) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_attention_256': bool(arms["ATTN7_OWN_256"] <= BARS["b_max"]),
        'pred_c_attention_128': bool(arms["ATTN7_OWN_128"] <= BARS["c_max"]),
        'pred_d_all_attention_256': bool(arms["ATTN18_OWN_256"] <= BARS["d_max"]),
        'pred_e_nested_is_free': bool(gaps["nested_256_minus_joint"] <= BARS["e_max"]),
    }
    nulls = {"b_null_ge_.15": bool(arms["ATTN7_OWN_256"] >= NULLS["b_min"]), "c_null_ge_.40": bool(arms["ATTN7_OWN_128"] >= NULLS["c_min"]),
             "d_null_ge_.40": bool(arms["ATTN18_OWN_256"] >= NULLS["d_min"]), "e_null_ge_.10": bool(gaps["nested_256_minus_joint"] >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "k": K, "ks_attn": KA, "ks_nested": KN,
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "attn7_own_768": arms["ATTN7_OWN_768"], "prior_attn7_own_768": PRIOR_ATTN7_OWN_768, "late14_joint_768": arms["LATE14_JOINT_768"], "prior_late14_joint_768": PRIOR_LATE14_JOINT_768},
           "ce_added": arms, "gaps": gaps, "attn_input_eff_rank": {f"{s_[0]}{s_[1]}": heads[s_]["eff_rank"] for s_ in ASITES},
