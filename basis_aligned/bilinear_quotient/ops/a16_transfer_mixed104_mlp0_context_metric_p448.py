"""RUNG 330 -- SIGNED a16 ADOPTION GATE FOR MLP0 CONTEXT-RRR p448.

p448 alone passed the lower-rank shifted OOD tail gate.  Apply the identical
native a16 mean ablation and compare direct signed effects against its saved
unablated census CEV.

Frozen predictions
------------------
pred_a_live_baseline_price_and_identity:
    Baseline +.012662/43, OOD and fresh gates, p448 map, mixed104 identities,
    and 533,623,094-scalar / 2,018,550,380-byte bill are exact.
...
Null: cosine <.70 or collateral Spearman <.75.  Full pass formally adopts p448
as a new smaller Pareto point.  p384/p256 remain stopped by OOD tails.
"""
...
OUT = ROOT / "a16_transfer_mixed104_mlp0_context_metric_p448_results.json"
BASE_RESULT = ROOT / "mixed104_mlp0_context_metric_lower_rank_ood_results.json"
BASE_CEV = ROOT / "cev_mixed104_mlp0_context_rrr_lower_ranks.pt"
COMP_KO = ROOT / "cev_a16ko_mixed104_mlp0_context_rrr_p448.pt"
NATIVE_KO = ROOT / "cev_a16ko_native_context_rrr_p448_gate.pt"
RANK = 448
FIT_SLICE = (0, 24)
...
def main() -> None:
    if os.environ.get("BQLIB_DRYRUN") == "1":
        ...
        arm = baseline["arms"]["448"]
        assert arm["shifted_damage_row_max"] <= .090
        assert arm["certificates_valid"] >= 43 and arm["literal_standalone_scalars"] == SCALARS
        print("A16 TRANSFER CONTEXT-RRR P448 | dry run: baseline, fit, price, bars valid")
        return
    ...
    baseline = baselines["r448"].float().reshape(-1)
    ...
    del covariance, program0, _basis
    ...
    C.SEL["_ablh"] = ablate
    print("ARM: mixed104 + MLP0 context-RRR p448 with a16 mean ablated", flush=True)
    run = C.main()
    ...
    arm = baseline_result["arms"]["448"]
    pred_a = (arm["census_damage"] <= .014 and arm["certificates_valid"] >= 43
              and arm["shifted_damage_row_max"] <= .090 and run["L2_F"] <= .040
              and observed == {0: RANK} and widths == {104}
              and all(value == wanted_qk for value in index_sets.values())
              and not any(name in active for name in ("a0", "a1v", "tailE"))
              and SCALARS == 533_623_094 and BYTES == 2_018_550_380)
    ...
    result = {
        "status": "a16_transfer_mixed104_mlp0_context_metric_p448_complete",
        "rung": 330,
        "claim_level": "direct_signed_a16_p448_adoption_gate",
        "convention": "signed effect = KO CE minus unablated CE within each model",
        "unablated_census_damage": arm["census_damage"],
        "unablated_certificates_valid": arm["certificates_valid"],
        "unablated_shifted_damage_mean": arm["shifted_damage_mean"],
        ...
        "null_signed_p448_transport_fails": bool(null),
