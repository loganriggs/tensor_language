"""SKETCH-GATED TOP-K (rung 213): a 16x-cheaper gate picks each token's active units.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. Math-review move #3: convert the
sparse rotating usage (S2304/S2306/S2308) into executable structure. The gate no longer sees the true unit
activations: units are scored from a rank-64 sketch (P = top right-singular basis of [L;R]; score
|(xP@LP^T)*(xP@RP^T)|*||D_i||), then the selected top-1152 compute exactly. Gate cost ~0.66M vs 10.6M
flops/token/module. Same r64-exact base.

REGISTERED PREDICTIONS (arms: SKETCH = this run; ORACLE = +0.1015 (S2304); FIXED = +2.2048 (S2303)):
  (a) THE GATE WORKS: census <= 0.30 (keeps >= 90% of the ORACLE-vs-FIXED gap closed).
  (b) valid >= 1.
  (c) SANITY: census in [0.02, 2.5]; inert tripwire vs both ORACLE and FIXED receipts.
NULL: selection needs the full products - SKETCH >= 0.8 x FIXED (>= 1.76). PRICE: probe; gate flops 16x
down, storage unchanged. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: sketch-gated top-k')
    raise SystemExit(0)
import torch
...
OUT=PT+'cp_sketch64_results.json'
            ...
            if kind=='cv':
                ...
                def h(mod,i_,o_,Lk=Lk,Rk=Rk,Dk=Dk,db=db):
                    ...
                    if kk and u.shape[-1]>kk:
                        if SEL.get('cp_sketch'):
                            _key=id(Lk)
                            if _key not in SEL.setdefault('_SK',{}):
                                _,_,_Vh=torch.linalg.svd(torch.cat([Lk,Rk]),full_matrices=False)
                                _P=_Vh[:int(SEL['cp_sketch'])].T.contiguous()
                                SEL['_SK'][_key]=(Lk@_P,Rk@_P,_P)
                            _LP,_RP,_P=SEL['_SK'][_key]
                            _xs=x@_P
                            _us=(_xs@_LP.T)*(_xs@_RP.T)
                            _idx=(_us.abs()*Dk.norm(dim=0)).topk(kk,dim=-1).indices
                        else:
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['cp_topk']=1152; SEL['cp_sketch']=64; SEL['qk_r']=64; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    pb=nv>=1
    pc=0.02<=agg<=2.5
    if abs(agg-0.1015)<1e-4 or abs(agg-2.2048)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: sketch gate inert')
    ...
    print(f"(a) census {agg:+.4f} <= 0.30: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 1; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) census in [0.02, 2.5]; era rho {rho:.4f}: {'HELD' if pc else 'FAILED'}")
