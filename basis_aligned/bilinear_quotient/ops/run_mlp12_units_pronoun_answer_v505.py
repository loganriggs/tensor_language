#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_unit_closure pred_b_top_10_carry_half pred_c_top_50_carry_most pred_d_top_unit_is_a_number_unit pred_e_top_unit_column_reads_u
"""MLP 12 at the pronoun answer position, by unit (v505). v504: at the pronoun slot the copied number is re-expressed by the MLPs of the readers' own
blocks, MLP 12 and MLP 9 first (0.089 each). MLP 12's write at the answer position split exactly by unit on u = W_U[they] - W_U[he] (lambda chain of blocks
13-17; the final rms shared per row), plural - singular over the 122 natural pairs: is the re-expression a few units, as MLP 8's write at the noun was (829 /
953 / 1030), or a spread?
PREDICTIONS (scored as written; failures preserved; priors from v408 / v464)
    pred_a_unit_closure           per-unit terms sum to MLP 12's term within relative 1e-3 on every row
    pred_b_top_10_carry_half      the top 10 units carry >= 0.50 of MLP 12's pooled contrast on u at the answer. Prior: unsure
    pred_c_top_50_carry_most      the top 50 carry >= 0.80
    pred_d_top_unit_is_a_number_unit  the top unit's plural - singular activation contrast at the answer has one sign on >= 0.80 of the pairs
    pred_e_top_unit_column_reads_u  |cos(Down_12[:, top], u)| >= 0.10
...
OUT = ROOT / "circuits/followups/mlp12_units_pronoun_answer_v505_result.json"
CANDIDATE_ID = "chain.mlp12_units_pronoun_answer_v505"
N_HEAD, LAYER = 9, 12
CLOSURE_TOL, TOP10_MIN, TOP50_MIN, SIGN_MIN, COL_MIN = 1e-3, 0.50, 0.80, 0.80, 0.10
FORWARDS_MAX = 5
PREDICTIONS = {"pred_a_unit_closure": "<= 1e-3", "pred_b_top_10_carry_half": ">= 0.50", "pred_c_top_50_carry_most": ">= 0.80", "pred_d_top_unit_is_a_number_unit": ">= 0.80 one sign", "pred_e_top_unit_column_reads_u": "|cos| >= 0.10"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "pairs": len(items), "layer": LAYER, "reader": "direct (W_U), answer position", "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"closure_tol": CLOSURE_TOL, "top10_min": TOP10_MIN, "top50_min": TOP50_MIN, "sign_min": SIGN_MIN, "col_min": COL_MIN}}
    ...
    col = Dw[:, top].cpu(); col_cos = float(abs(col @ m.cpu()) / (col.norm() * m.cpu().norm()))
    report["top_unit"] = top; report["top_unit_sign_constancy"] = sign_const; report["top_unit_column_cos_u"] = col_cos
    predictions = {"pred_a_unit_closure": closure <= CLOSURE_TOL, "pred_b_top_10_carry_half": share(10) >= TOP10_MIN, "pred_c_top_50_carry_most": share(50) >= TOP50_MIN, "pred_d_top_unit_is_a_number_unit": sign_const >= SIGN_MIN, "pred_e_top_unit_column_reads_u": col_cos >= COL_MIN}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "mlp12_units_pronoun_answer_result_v505", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
