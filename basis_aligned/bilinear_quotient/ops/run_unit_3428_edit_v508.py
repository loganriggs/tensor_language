#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_unit_3428_moves_the_pronoun_margin pred_c_random_unit_does_not pred_d_unit_3428_within_mlp9 pred_e_hub_units_at_the_noun_replay
...
OUT = ROOT / "circuits/followups/unit_3428_edit_v508_result.json"
CANDIDATE_ID = "chain.unit_3428_edit_v508"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, U22_MIN, RANDOM_MAX, M12_V504, BAND, HUB_V463 = 1e-3, 0.04, 0.01, 0.089, 0.02, 0.133
import random
import random
UNIT_SETS = {"U22_A": (9, (3428,), "answer"), "RANDOM_A": (9, (random.Random(0).choice([j for j in range(4608) if j != 3428]),), "answer"), "HUB_N": (8, (829, 953, 1030), "noun")}
...
FORWARDS_MAX = 17
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_unit_3428_moves_the_pronoun_margin": ">= 0.04", "pred_c_random_unit_does_not": "<= 0.01", "pred_d_unit_3428_within_mlp9": "<= 0.109", "pred_e_hub_units_at_the_noun_replay": "0.133 +- 0.02"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "u22_min": U22_MIN, "random_max": RANDOM_MAX, "m12_v504": M12_V504, "band": BAND, "hub_v463": HUB_V463}}
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_unit_3428_moves_the_pronoun_margin": v["U22_A"] >= U22_MIN, "pred_c_random_unit_does_not": abs(v["RANDOM_A"]) <= RANDOM_MAX,
                   "pred_d_unit_3428_within_mlp9": v["U22_A"] <= M12_V504 + BAND, "pred_e_hub_units_at_the_noun_replay": abs(v["HUB_N"] - HUB_V463) <= BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "unit_3428_edit_result_v508", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
