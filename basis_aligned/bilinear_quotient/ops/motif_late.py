"""QUARTER TEST II (rung 242): only the LATE motif blocks (6-9) exact.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2339: early motifs (2-5) carry 71%
of the motif-half census gain and one certificate. The complement: blocks 6-9 exact, everything else r64.
REGISTERED PREDICTIONS (arms: THIS vs r64 0.0852/7, early-quarter 0.0680/8, motif-exact 0.0608/9):
  (a) LATE QUARTER CARRIES THE REST: census <= 0.080.
  (b) THE OTHER CERTIFICATE LIVES HERE: valid >= 8.
  (c) SANITY: census in [0.01, 0.12]; inert tripwires vs r64 AND early-quarter receipts.
NULL: late motifs carry nothing distinct (census >= 0.080 AND valid <= 7). PRICE: r64 config + blocks
6-9 patterns at full rank. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: late-motif quarter test')
    raise SystemExit(0)
import torch
...
OUT=PT+'motif_late_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=64; SEL['qk_rmap']={li:128 for li in range(6,10)}; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    pb=nv>=8
    pc=0.01<=agg<=0.12
    if abs(agg-0.0852)<1e-4 or abs(agg-0.0680)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: late-motif map inert')
    ...
    print(f"(a) census {agg:+.4f} <= 0.080: {'HELD' if pa else 'FAILED'}")
