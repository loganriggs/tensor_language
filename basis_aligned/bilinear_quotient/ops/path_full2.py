"""THE PATH CONTROL II (rung 281): same physical config via the CORNER lineage - implementation replication.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. Companion to rung 280: all patterns at
rank 128 (SEL knobs, which ARE live in this lineage - S2371 checked), values real. Two independent script
lineages building the same physical config; agreement isolates path cost from lineage bugs.
REGISTERED PREDICTIONS: (a) census <= 0.01; (b) valid >= 12; (c) census in [0.0, 0.12]; cev saved.
NULL: census >= 0.04 (path artifact) - and if 280 disagrees with 281 by > 0.01, a lineage bug, not physics.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: path control II (corner lineage, full rank)')
    raise SystemExit(0)
import torch
...
OUT=PT+'path_full2_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=128; SEL['qk_rmap']={li:128 for li in range(2,10)}; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    cev=SEL['cev']
    torch.save(cev,PT+'cev_pathfull2.pt')
    print('cev saved: cev_pathfull2.pt',flush=True)
    ...
    print(f"(c) census in [0.0, 0.12]: {'HELD' if pc else 'FAILED'}")
