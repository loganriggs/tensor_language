...
48 opened syntax-transfer sequences plus matched random site edits, eight prefix calls and twenty-four suffix calls, zero fits.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v664_result.json'
PREDICTIONS=dict(pred_a_exact_replay='baseline and independent joint versus shared chain replay max absolute error <=1e-4',
 ...
 pred_c_capable_position_transfer='final projection oracle retains >=75% of candidate collateral error on can/will and should/could')
...
def main():
    plan=dict(prefix_calls=8,suffix_calls=24,sequences_per_arm=48,fits=0,backwards=0,
              ...
              random_edit_seeds=[],reused_random_control_receipt="subject_attention_freeze_v663_result.json",modal_readers=[['can','will'],['may','might'],['should','could']],additional_native_attention_calls=24,execution_policy='managed_queue_only',predictions=PREDICTIONS)
    ...
    import hashlib
    rowfile=POLY/'SUBJECT_SYNTAX_TRANSFER_V662_ROWS.json'
    assert hashlib.sha256(rowfile.read_bytes()).hexdigest()=='f29cd6bebdfc57a0772a63a449b599014525774cb74c150771f20dbf0414e3be'
    ...
    oracle_collateral=[];native_collateral=[];predicted_collateral=[];target_effects=[];random_effects=json.loads((OUT.parent/'subject_attention_freeze_v663_result.json').read_text())['selectivity']['random_effects']
    ...
    def native_selected(raw,x0,first,readpos):
        ...
        return (30*torch.tanh(selected/30)).double(),x[torch.arange(len(x),device=x.device),readpos].double()
    ...
    for entries in batches:
        ...
        native_logits,native_final=native_selected(edited,x0,first,readpos);counts['suffix_calls']+=1
        ...
        oracle_z=(native_final-context['baseline_final'])@output_encoders[-1]
        oracle_controls=readout_prepared(context['control_fixed'],oracle_z,context['control_context'])
        ...
        oracle_collateral.extend(((cb-oracle_controls)[:,0::2]-(cb-oracle_controls)[:,1::2]).tolist())
    ...
    oracle_errors=np.linalg.norm(np.asarray(oracle_collateral)-native_control,axis=0)/target_norm
    ...
    selectivity=dict(oracle_collateral=oracle_collateral,oracle_error_over_target=oracle_errors.tolist(),oracle_to_candidate_error=(oracle_errors/collateral_prediction_errors).tolist(),readers=[['can','will'],['may','might'],['should','could']],
    ...
    c=a and b and all(oracle_errors[i]>=.75*collateral_prediction_errors[i] for i in [0,2])
    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,cells=cells,
                predictions=dict(zip(PREDICTIONS,[bool(a),bool(b),bool(c)])),selectivity=selectivity,
                ...
                scope='Final projection oracle diagnostic, not a legal predictor. 48 opened syntax-transfer rows, frozen v659 bases and unchanged v661 execution; new modal and matched-random controls. Relative clauses and fronted comma templates; vocabulary reused from v660. Eight template-number cells; no outcome-based selection. All attention12-17 and MLP12-17 responses dynamic in reduced full-sequence states. Native baseline context, first-value and initial post11 response retained as charged ports. No fresh generalization, selective removal or port-closed circuit claim.')
