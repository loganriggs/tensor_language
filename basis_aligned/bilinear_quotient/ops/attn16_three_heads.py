...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['attn16_head_marginals_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; skip-a16 config + joint zero of heads 1,2,5,6,7,8 (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'attn16_three_heads_results.json'
...
if __name__=='__main__':
    ...
    print('SINGLE ARM: S2146 skip-a16 config + joint six-head zero at attn16',flush=True)
    r_s=main()
    hr=SEL['head16_result']; d=hr['d']
    pa=abs(d-(-0.0012))<=0.01
    pb=d<=0.01
    pc=abs(r_s['L2_F']-2.5091)<=0.01
    res={'skip16':r_s,'six_zeroed':hr,'additive_prediction':-0.0012,
         'convention':'L2 = CE above the real model; lower is better; d = damage from zeroing heads 1,2,5,6,7,8',
         'pred_a_additive_number':bool(pa),'pred_b_nearly_free':bool(pb),'pred_c_reproduces_s2146':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) |{d:+.4f} - (-0.0012)| = {abs(d-(-0.0012)):.4f} <= 0.01: {'HELD' if pa else 'FAILED'}")
    print(f"(b) d {d:+.4f} <= +0.01: {'HELD' if pb else 'FAILED'}")
