...
"""v579: the step that actually moves the count -- within-family separability for verb_preposition_at_amid.

WHY THIS ONE. v575 lifted the eight verb_preposition mappings that were authored, screened and then left: all eight
produced units with held-out extraction 0.842 to 0.887 and no errors, rows 3 and 4 passed eight of eight, row 5 six
of eight, but ROW 2 passed only two of eight and exactly ONE behaviour cleared all four rows at once --
verb_preposition_at_amid (bristled -> ` at`, thrived -> ` amid`). Under the protocol a four-row pass is a candidate,
not a circuit: the corpus number moves only after the behaviour is shown SEPARABLE from the family it would join.
This rung is that check, and it is the first thing this session that could take the count off 139.
WHAT SEPARABLE MEANS HERE, AND WHY IT CAN FAIL. at_amid is fitted against ten counted verb_preposition members that
share its frame -- the same `Near the X the X CUE, of course,` shape 94 of 128 cells in this family use -- so the
only thing distinguishing it is its cue -> token mapping. v573 showed an hour ago that a restructured FRAME fuses
with its original at reach 0.95 to 1.02, so frame is not doing the separating work; if mappings do not separate
either, at_amid is a duplicate and must not be counted. The counted members are re-checked in the same run so a
family-wide instrument drift shows up instead of being read as a new member's success.
NOTHING IS COUNTED BY THIS FILE. It reports separability; the count is the portfolio owner's to move, and I have
asked on the board for the canonical counted list precisely so an increment can be booked without double-counting.
The count stays 139 in every ledger line I write for this rung.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
  pred_a_new_separable at least k_new = 1 of the new members -- there is exactly one, at_amid -- is separable from
                       the counted family at cross_max = 0.05.                                        prior 65%
  pred_b_counted_kept  at least k_kept = 8 of the ten counted members stay separable in the same run, so a null on
                       at_amid cannot be blamed on the family having stopped separating. IN v577 THIS BAR WAS
                       UNMEASURABLE: I registered it but set EVAL to at_amid alone, so the counted members were
                       prepared as controls and never evaluated, and the predicate could not have come out true
                       however the model behaved. EVAL now includes them, which is the only change in this rung.
                                                                                                      prior 88%
  pred_c_row4_kept     at least k_d = 2 members keep a C upper bound at or below c_ub_max = 0.01, so the control
                       row is still doing work in this batch.                                          prior 85%
  pred_d_instrument    every EVAL member reproduces its parent battery extraction within instr_tol = 0.03, which for
                       at_amid means the v575 receipt this rung is built on.                           prior 88%
HOW IT READS. a true with b: at_amid is a distinct behaviour, the first countable one from the backlog, and the
route from 175 unprocessed cells to counted circuits is real at roughly the one-in-eight yield v575 measured.
a FALSE with b true: at_amid fuses with the counted family despite clearing all four tier rows, which would mean a
four-row pass is NOT sufficient for distinctness and that the backlog yield is lower than one in eight -- the more
useful answer for planning, and the one I would rather have before authoring anything else.
b FALSE: the family stopped separating and nothing about at_amid is readable from this run.
SCOPE. One candidate, ten counted controls, one family, one frame.
Smoke: V579_SMOKE=<out.json> (CPU).
"""
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v579_result.json"
...
FAMILIES = {
    "verb_preposition": ['verb_preposition', 'verb_preposition_insisted', 'verb_preposition_to_at', 'verb_preposition_with_about', 'verb_preposition_of_with', 'verb_preposition_from_for', 'verb_preposition_on_about', 'verb_preposition_to_about', 'verb_preposition_at_from', 'verb_preposition_on_with', 'verb_preposition_at_amid'],
}
COUNTED_LIST = ['verb_preposition', 'verb_preposition_insisted', 'verb_preposition_to_at', 'verb_preposition_with_about', 'verb_preposition_of_with', 'verb_preposition_from_for', 'verb_preposition_on_about', 'verb_preposition_to_about', 'verb_preposition_at_from', 'verb_preposition_on_with']
COUNTED = tuple(COUNTED_LIST)
EVAL = tuple(COUNTED_LIST) + ("verb_preposition_at_amid", )
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES, **v213.NAMES, **v215.NAMES, **v217.NAMES, **v219.NAMES, **v221.NAMES, **v223.NAMES, **v225.NAMES, **v227.NAMES, **v229.NAMES, **v231.NAMES, **v233.NAMES, **v235.NAMES, **v237.NAMES, **v239.NAMES, **v241.NAMES, **v243.NAMES, **v245.NAMES, **v247.NAMES, **v249.NAMES, **v251.NAMES, **v253.NAMES, **v255.NAMES, **v257.NAMES, **v311.NAMES, **v315.NAMES, **v319.NAMES, **v323.NAMES, **v327.NAMES, **v349.NAMES, **v293.NAMES, **v353.NAMES, **v263.NAMES, **v265.NAMES, **v357.NAMES, **v339.NAMES, **v343.NAMES, **v361.NAMES, **v365.NAMES, **v575.NAMES}


def _parent_cdas(n):
    # the parent battery receipt that produced this member's units (each behaviour name is unique across batteries)
    for path in (V575R, V365, V361, V343, V339, V357, V263, V265, V353, V293, V349, V327, V323, V319, V315, V311, V257, V255, V253, V251, V249, V247, V245, V243, V241, V239, V237, V235, V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
BARS = {"cross_max": 0.05, "keep": 0.8, "k_kept": 8, "ext_floor": 0.5, "c_ub_max": 0.01, "k_d": 2, "k_new": 1, "instr_tol": 0.03}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 20000, 640000


def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v579", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V579_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V579_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v579", "candidate_id": "corpus.unit_family_separability_spec_v579",
