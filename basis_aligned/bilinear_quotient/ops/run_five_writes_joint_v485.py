#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_five_writes_jointly_add pred_c_rest_of_their_blocks_small pred_d_five_writes_are_most_of_the_channel pred_e_joint_below_all_attention
"""The five readers' writes into the pronoun slot, jointly (v485). v481-v484: by single-head swaps of the write into the answer position the five readers sum
to 0.823 of the they - he margin and all attention to 0.973. Writes into one position at different heads are exits of different states and should add
(v476). Here the five heads' writes swapped jointly; the other 32 heads of blocks 9 / 10 / 12 / 15 jointly; 122 natural pairs.
PREDICTIONS (scored as written; failures preserved; priors from v481-v484)
    pred_a_baseline_replays              the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_five_writes_jointly_add       the five readers' joint write swap closes 0.823 +- 0.05
    pred_c_rest_of_their_blocks_small    the other 32 heads of blocks 9 / 10 / 12 / 15 close <= 0.05 jointly
    pred_d_five_writes_are_most_of_the_channel  the five readers' joint swap closes >= 0.80 of 0.973 (all attention into the slot)
    pred_e_joint_below_all_attention     the five readers' joint swap closes <= 0.973
...
OUT = ROOT / "circuits/followups/five_writes_joint_v485_result.json"
CANDIDATE_ID = "chain.five_writes_joint_v485"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, FIVE_SUM, BAND, REST_MAX, ALL_ATTN, SHARE_MIN, UNIT = 1e-3, 0.823, 0.05, 0.05, 0.973, 0.80, 701
...
FORWARDS_MAX = 13
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_five_writes_jointly_add": "0.823 +- 0.05", "pred_c_rest_of_their_blocks_small": "<= 0.05", "pred_d_five_writes_are_most_of_the_channel": ">= 0.80 x 0.973", "pred_e_joint_below_all_attention": "<= 0.973"}
...
def main() -> None:
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_five_writes_jointly_add": abs(mc["FIVE_W"] - FIVE_SUM) <= BAND, "pred_c_rest_of_their_blocks_small": abs(mc["REST_W"]) <= REST_MAX,
                   "pred_d_five_writes_are_most_of_the_channel": mc["FIVE_W"] >= SHARE_MIN * ALL_ATTN, "pred_e_joint_below_all_attention": mc["FIVE_W"] <= ALL_ATTN}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "five_writes_joint_result_v485", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
