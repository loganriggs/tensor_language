"""INSIDE THE TAIL REFITS: IS THE COST IN THE TEN-ROW CLASS TABLE OR IN THE FIVE-MILLION-PARAMETER LINK MAPS? (Claude, LANE 1)

SS2878 found the eight tail refit dictionaries a10L-a17L are load-bearing -- collapsing them costs **+0.2011 nats** -- while the sixteen
`fit_attnd` dictionaries are free at 0.0000. It did not ask WHERE inside a tail refit that cost lives, and the parameter split makes
that question unusually valuable, because it is wildly asymmetric:

    class table  10 x 1152          =        11,520 parameters per layer   (0.22%)
    link maps     4 x 1152 x 1152   =     5,308,416 parameters per layer   (99.78%)

If the +0.2011 is carried by the CLASS TABLE, then the link maps -- **42,467,328 parameters across the eight tail layers** -- can be
dropped at no cost while the frontier keeps everything it was buying. If it is carried by the LINK MAPS, the tail dictionaries are
genuinely expensive and the simplification stops here.

Three arms of SS312's published norm-selection pipeline, applied at the INLINE tail-refit site only (the `fit_attnd` site is left alone
precisely because SS2876 measured it free, which isolates the refits):

    ARM 2  LW := {}                   keep the class table, drop the link maps
    ARM 3  CV := one mean vector      keep the link maps, collapse the class table
...
#                     pred_b_the_link_maps_are_free
#                     pred_c_the_class_table_carries_the_cost
#                     pred_d_the_two_pieces_account_for_S2878
#                     pred_e_the_parameter_split_is_stated

Preregistration: polynomial_causal/FRONTIER_TAIL_REFIT_SPLIT_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; three arms (baseline / tail link maps dropped / tail class table collapsed)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_tail_refit_split_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
TAILMODE={'mode':None}   # None | 'links' (drop LW only) | 'table' (collapse CV only)
CVSNAP={}
SKIP8=64
...
PREREG = PT + '../polynomial_causal/FRONTIER_TAIL_REFIT_SPLIT_PREREGISTRATION.md'
PREREG_SHA = "e6e14ec564657c93697ff162ed7f2f74622967c8f4b53d15f43e7b3cc16abd36"
...
S2878 = PT + 'frontier_all_dictionaries_collapse_tailrefits_results.json'
S2878_SHA = "5771215ea9e079572da82b0cbac5bbf1b30390ca4a63d374e8fd8666c369fc9a"
TABLE_PARAMS = 10 * 1152          # the ten class rows
LINK_PARAMS  = 4 * 1152 * 1152    # the four 1152x1152 maps for LINK = [2,7,8,9]
NTAIL = 8                         # a10L..a17L
BARS = {"reproduce": 0.05, "links_free": 0.05, "table_costly": 0.15, "additive": 0.05}
NULLS = {"links_free_ge": 0.15, "table_costly_le": 0.05}
...
if __name__=='__main__':
    ...
    if _sha(S2878) != S2878_SHA:
        raise RuntimeError(f'frozen hash mismatch: {S2878}')
    joint = json.load(open(S2878))['summary']['cost_all_dictionaries_nats']
    SEL['mode']='norm'
    ...
    print(f'ARM 2/3: tail refits a10L-a17L keep their class table, DROP the four link maps',flush=True)
    TAILMODE['mode']='links'
    r_nolink=main()

    print(f'ARM 3/3: tail refits a10L-a17L keep their link maps, COLLAPSE the class table',flush=True)
    TAILMODE['mode']='table'
    r_notab=main()
    TAILMODE['mode']=None

    # SIGN: L2 is CE ADDED ABOVE THE REAL MODEL, LOWER IS BETTER -> cost POSITIVE = WORSE
    cost_links = r_nolink['L2_F'] - r_base['L2_F']
    cost_table = r_notab['L2_F']  - r_base['L2_F']
    additive   = cost_links + cost_table
    drift      = abs(additive - joint)

    pa = abs(r_base['L2_F']-2.6735) <= BARS['reproduce']
    pb = cost_links <= BARS['links_free']
    pc = cost_table >= BARS['table_costly']
    pd = drift <= BARS['additive']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_link_maps_are_free':bool(pb),
           'pred_c_the_class_table_carries_the_cost':bool(pc),
           'pred_d_the_two_pieces_account_for_S2878':bool(pd),
           'pred_e_the_parameter_split_is_stated':True}
    nulls={'b_null_the_link_maps_are_load_bearing':bool(cost_links>=NULLS['links_free_ge']),
           'c_null_the_class_table_is_free':bool(cost_table<=NULLS['table_costly_le'])}
    res={'rung':'frontier_tail_refit_split','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,
         'baseline':r_base,'links_dropped':r_nolink,'table_collapsed':r_notab,
         'published_L2_F':2.6735,'S2878_joint_cost':joint,
         'summary':{'L2_F_baseline':round(r_base['L2_F'],4),
                    'L2_F_links_dropped':round(r_nolink['L2_F'],4),
                    'L2_F_table_collapsed':round(r_notab['L2_F'],4),
                    'cost_dropping_link_maps_nats':round(cost_links,4),
                    'cost_collapsing_class_table_nats':round(cost_table,4),
                    'sum_of_pieces_nats':round(additive,4),
                    'S2878_joint_cost_nats':round(joint,4),
                    'accounting_drift_nats':round(drift,4),
                    'L2_C_baseline':round(r_base['L2_C'],4),
                    'L2_C_links_dropped':round(r_nolink['L2_C'],4),
                    'params_class_table_per_layer':TABLE_PARAMS,
                    'params_link_maps_per_layer':LINK_PARAMS,
                    'params_link_maps_all_tail':LINK_PARAMS*NTAIL,
                    'link_share_of_layer_parameters':round(LINK_PARAMS/(LINK_PARAMS+TABLE_PARAMS),5)},
         ...
         'hashes':{PREREG:PREREG_SHA,S2878:S2878_SHA},'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {r_base['L2_F']} vs 2.6735 (tol .05): {'HELD' if pa else 'FAILED'}")
    print(f"(b) dropping the LINK MAPS costs {cost_links:+.4f} <= {BARS['links_free']}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) collapsing the CLASS TABLE costs {cost_table:+.4f} >= {BARS['table_costly']}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) pieces {additive:+.4f} vs SS2878 joint {joint:+.4f}, drift {drift:.4f} <= {BARS['additive']}: {'HELD' if pd else 'FAILED'}")
