...
"""GPT-2 small replication, v753: the fitted KERNEL + CONTENT + SINK program.

v751: per-offset kernels alone cost +5.80 on GPT-2 (+2.92 with the position-0 column native) — absolute positions and the first-token sink
break the pure offset template. Program per head on the pre-softmax logits (causal mask kept): column 0 NATIVE (the sink read), diagonal
native, and for 0 < j < i: logit(i, j) := kappa_h(i - j) + [P_r(i, j) - kappa_r(i - j)], where P_r is the head's logit from rank-r factored
q / k weight maps (biases native; no rotary in GPT-2) and kappa_r its per-offset mean on 64 fit rows (recomputed every 25 steps). Ranks: 64
for the 71 heads with mean-ablation value >= 0.005 (v751), 16 for the other 73; no native heads. kappa0 = v751's closed-form kernels, fitted as
kappa0 (1 + c). Adam 300 steps (kernels 0.01 -> 0.001, factors 3e-4 -> 3e-5, batch 8; 576 fit / 96 validation rows, seed 703), SNAPSHOT at
the validation minimum; manipulability over the 24 most valuable heads. Values: 73 x 2 x 16 x 832 + 71 x 2 x 64 x 832 + 144 x 513 = 9.6M vs
2 x 144 x 64 x 768 = 14.2M native QK (1.5x; GPT-2's heads are 64-wide so rank 64 is full rank on the head side — the saving is on the
768 side). Held-out skip7000 (native 3.39828); joint value 3.345 (v751). CE ADDED.
PREDICTIONS (scored as written; failures preserved)
    pred_a_native_replays        explicit-softmax native within 0.002 of 3.39828 (instrument)
    pred_b_closed_form_cost      the closed-form program (step 0) costs <= 1.0. Prior: unsure
    pred_c_fitted_cost           the snapshot costs <= 0.20 (recovery >= 0.94). Prior: unsure
...
PRICE (registered maximum): native 6; kappa_r 13 x 2 = 26; step-0 9; 300 forwards + 300 BACKWARDS; validation 36; held-out 72; snapshot 8; manipulability 144;
total ~601 forwards, 300 backwards; fit parameters ~10M. Bars: forwards <= 625, backwards <= 300.
"""
...
OUT = ROOT / "circuits/followups/gpt2_program_fit_v753_result.json"
OUT_PT = ROOT / "circuits/followups/gpt2_program_fit_v753_programs.pt"
...
CANDIDATE_ID = "gpt2.program_fit_v753"
...
def main() -> None:
    ...
    print(f"[gpt2 program] closed-form (step 0) {step0 - native:+.4f} -> snapshot (validation step {best[1]}) {c_snap - native:+.4f}, joint recovery {1 - (c_snap - native) / JOINT_VALUE:.3f}; curve {[round(ho_curve[k], 3) for k in sorted(ho_curve)]}")
    ...
    for hh in nhooks:
        hh.remove()
    disk_guard.guard_torch_save({f"{k[0]}.{k[1]}": {"kappa": (kappa0[k] * (1 + cmul[k])).detach().cpu(), "rank": ranks[k], **{n_: (fac[k][n_][0].detach().cpu(), fac[k][n_][1].detach().cpu()) for n_ in fac[k]}} for k in ALL}, str(OUT_PT), "v753 gpt2 program (snapshot)")
    values_n = sum(2 * min(ranks[k], hd) * (hd + D) + 513 for k in ALL)
    ...
    OUT.write_text(json.dumps({"schema": "gpt2_program_fit_result_v753", "candidate_id": CANDIDATE_ID, "plan": plan,
