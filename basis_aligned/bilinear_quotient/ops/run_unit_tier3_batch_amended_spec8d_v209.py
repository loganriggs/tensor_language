#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the fourth eight spec-authored behaviours, plus one instrument.
"""v209: the AMENDED tier-3 battery (v196/v197 protocol, unchanged) over the FOURTH EIGHT spec-authored behaviours -- plus one instrument.

v204 4/8, v205 6/8, v207 4/8 (+instrument) at ~30 GPU-s each; v207's misses were all row 4 (own-C UB975 0.012-0.018, just over
the bar). Eight more, authored through `circuit_fast_screen_behaviour_spec`, single-cue swap, matched final token, cue never final,
32 A1/A2/P/C rows, ABAB directions -- chosen to give v207's new families siblings for a separability pass: partitive_agreement
(`One/Two of the pilots near the lantern` -> has/have: head numeral past a plural attractor), reflexive_person_plural (`we/you
quickly blamed` -> ourselves/yourselves), object_pronoun_person_plural (`Because we/they had asked, ... gave the lantern to` ->
us/them), so_inversion (`left early, and so/then, naturally,` -> did/the), neither_inversion (`did not leave, and neither/then,
naturally,` -> did/the), gerund_selection (`kept/hoped, as always,` -> lifting/to), adjective_complement (`was busy/ready that
morning` -> lifting/to), perception_complement (`watched/persuaded the sailor` -> carry/to; causative_complement's sibling with the
bare side on a perception verb). Capability printed BEFORE the run on CPU (`g.prepare(valid_only=True)`, 32 rows per family):
zero A1/A2 rows dropped on all eight (C: 2 of 32 dropped, the shared C family); mean base/donor margins A1: partitive_agreement
-4.60/+4.60, reflexive_person_plural -5.09/+5.28, object_pronoun_person_plural -1.82/+1.68, so_inversion -0.53/+0.79 (WEAKEST:
the model barely prefers `did` after `and so, naturally,` -- the interposed adverb weakens the inversion cue; kept as a
capability-marginal cell, expected to miss), neither_inversion -3.02/+3.08, gerund_selection -2.54/+2.43, adjective_complement
-1.93/+1.92, perception_complement -6.11/+5.75; A2 weakest so_inversion -0.36/+0.53, then gerund_selection -1.88/+1.74.
Protocol identical to v197/v201/v204/v205/v207 (greedy pool 40, target 0.88, min_gain 0.005, max 30; row 2 = MIN over the two held
directions >= 0.80; row 3 dim A1 CE LB975 > 0 and >= 0.10; row 4 constrained rank-1 own-C UB975 <= 0.01 on 16 held C rows; row 5 =
A2-own / A2 full-rank ceiling in [0.5, 1.4], LB975 > 0). finiteness_selection is re-run as the instrument.

REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 4 of the 8 new behaviours pass all four rows (v204 4/8, v205 6/8, v207 4/8 with the same
                           recipe; the weak cells are the ones with the smallest CPU margins above).                          prior 50%
  ...
  pred_d_row4           -> constrained rank-1 own-C CE UB975 <= 0.01 on at least 5 of 8.                          prior 45%
...
Accounting: four-row passes here join the one-protocol standing; none counts as a new circuit directly -- every member has a
sibling in the corpus (partitive_agreement: the number-agreement sets; the person/inversion/complement sets: v207's) and waits for
the next separability rung (v210) before counting.
...
Smoke: V209_SMOKE=<out.json> (CPU, V209_SMOKE_ROWS=4 per split, V209_SMOKE_NAMES=so_inversion; pool 3/max 3, steps 5).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec8d_v209_result.json"
...
NAMES = {"partitive_agreement": "partitive_agreement", "reflexive_person_plural": "reflexive_person_plural",
         "object_pronoun_person_plural": "object_pronoun_person_plural", "so_inversion": "so_inversion",
         "neither_inversion": "neither_inversion", "gerund_selection": "gerund_selection",
         "adjective_complement": "adjective_complement", "perception_complement": "perception_complement",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec8d_v209", "behaviours": 9, "constructions": 2,
...
def main() -> None:
    ...
    smoke = os.environ.get("V209_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V209_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V209_SMOKE_NAMES", "so_inversion").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec8d_v209", "candidate_id": "corpus.unit_tier3_batch_amended_spec8d_v209", "bars": BARS,
