...
"""v258: THE DECISIVE TEST of the cue-lexeme hypothesis. Two new members, each of which REUSES A CUE VERB from a counted sibling, evaluated
against the full verb_preposition family (20 counted, 9 controls). The prediction is a FUSION, named in advance, at a named partner.

THE EVIDENCE SO FAR. v254 found the first fusion in the preposition class and it did not follow the readout token: verb_preposition_of_over
leaked 0.294 into of_by and 0.134 into over_with, and those are exactly the two cells that reuse one of its CUE VERBS (`consisted` is the ` of`
cue of both of_over and of_by; `presided` is the ` over` cue of both of_over and over_with). v256 then tested the other side and the hypothesis
held twice more:
  (a) all FIVE new cells that share a READOUT with counted siblings but reuse NO cue verb came out separable -- about_into 0.010,
      into_with 0.007, of_into 0.009, about_against 0.016, through_with 0.009. Shared readouts alone do not fuse.
  (b) a SECOND cue-sharing pair surfaced without being looked for: verb_preposition_with_about (dealt/WORRIED -> with/about) crossed the bar at
      sib 0.053, and its leak partner is verb_preposition_to_about (belonged/WORRIED -> to/about), which shares the cue verb `worried` for
      ` about`. Its leak has risen monotonically as to_about sat in the control set (0.038 -> 0.041 -> 0.047 -> 0.053 across v248/v252/v254/v256),
      which is what a real shared direction looks like as the control weight on it grows, not what noise looks like.
So three cue-sharing pairs leak (of_over/of_by, of_over/over_with, with_about/to_about) and eleven shared-readout non-cue-sharing pairs do not.

WHAT IS STILL MISSING, AND WHAT THIS RUNG ADDS. Every cue-sharing pair so far was found AFTER the fact -- I noticed the shared verb while reading
a receipt. That is exactly the shape of a story fitted to data. This rung is the pre-registered version: two cells were AUTHORED to reuse a cue,
their partners were named before either ran, and the prediction is stated as a bar the receipt can fail.
  + verb_preposition_with_beyond (DEALT/stretched -> with/beyond). `dealt` is verb_preposition_with_about's ` with` cue.
    PREDICTED: sib > 0.05 into with_about, and <= 0.05 into all 19 other counted siblings.
  + verb_preposition_in_beyond (RESULTED/persisted -> in/beyond). `resulted` is verb_preposition_in_to's ` in` cue.
    PREDICTED: sib > 0.05 into in_to, and <= 0.05 into all 19 other counted siblings.
Neither readout pair exists elsewhere in the corpus (with/beyond and in/beyond are 0 hits), and ` beyond` is carried by nothing counted, so the
ONLY thing linking either cell to the family is the reused cue verb. If both fuse at their named partner and nowhere else, the hypothesis is
confirmed on a pre-registered test. If either comes out separable from its cue partner, the hypothesis is WRONG as stated and the three observed
fusions need a different account -- and I would rather learn that here than keep the story.
A third cue-reuse cell, verb_preposition_at_under (LOOKED, from to_at), failed row 5 at v267 and one more, of_beneath (ACCUSED, from of_with),
failed row 4; both join the control set rather than being evaluated. on_within (INSISTED) failed all four rows. So the pre-registered test runs on
two arms, not five -- disclosed, and the missing arms are capability/row failures, not selections made after seeing separability.

COUNTING. Under the v242 rule a new cell that leaks into a counted incumbent JOINS it, +0. Both new cells are therefore expected to add ZERO, and
this rung is expected to leave the total unchanged at 104. It is run for the mechanism, not the count.
The two members that already fuse (over_with with of_over, with_about with to_about) keep ONE count each for their group: in both cases the
partner is an UNCOUNTED cell (of_over fused at v254; to_about is a three-row control), so no counted circuit is lost and none is double-counted.
That reading of the v222/v242 conflict is on the board (09:45) and remains unanswered.

Recipe as v198/.../v256: per evaluated member, arm `own` (own C_fit control) and arm `fam` (own C_fit + EVERY sibling's A1 FIT prep,
control_weight 30 x len(controls)); evaluated on the member's held A1 (extraction), each sibling's held A1 (|CE| leak) and held C.
Units frozen from the parent receipts (v267 for the new members; earlier batteries for the counted ones).
...
EVAL = 2 new members + 20 counted re-checks (22 fits x 2 arms, 29 controls in the fam arm: ~100 min; the full re-check stands because board
proposal (ix) is unanswered).

REGISTERED BEFORE THE RUN (each coded predicate is the sentence here; bars in BARS):
    pred_a_new_separable   at least 1 of the 2 new members is separable (k_new). THE CUE-LEXEME HYPOTHESIS PREDICTS THIS FAILS AT 0 of 2 --
                           this bar is written to be failed, and its failure is the positive result.                          prior 25%
    pred_b_counted_kept    at least 16 of the 20 re-checked counted members remain separable (k_kept; v256 kept 13/15).        prior 60%
    pred_c_row4_kept       fam-arm own-C UB975 <= 0.01 on at least 11 of the 22 evaluated members (k_d; v256 12/20).           prior 55%
...
pred_a is deliberately registered against my own hypothesis so the receipt cannot be read as confirming either way by choice of bar: if pred_a
PASSES, the hypothesis is refuted; if it FAILS at exactly 0 of 2 with each leak landing on the named partner, it is confirmed on a pre-registered
test. A mixed outcome (one fuses, one does not) confirms neither and gets reported as unresolved.
Watch, not a bar: verb_preposition itself reached sib 0.043 at v256 and verb_preposition_insisted 0.035; both are drifting up as controls
accumulate, which is what proposal (ix) is about.
Smoke: V258_SMOKE=<out.json> (CPU, V258_SMOKE_ROWS=4; V258_SMOKE_NAMES = a new member plus a re-check; steps 5).
BARS = {"cross_max": 0.05, "keep": 0.80, "ext_floor": 0.50, "c_ub_max": 0.01, "k_d": 11, "k_new": 1, "k_kept": 16, "instr_tol": 0.03}
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v258_result.json"
...
EVAL = ("verb_preposition_in_beyond", "verb_preposition_with_beyond", "verb_preposition", "verb_preposition_insisted", "verb_preposition_to_at", "verb_preposition_with_about", "verb_preposition_of_with", "verb_preposition_from_for", "verb_preposition_on_about", "verb_preposition_for_at", "verb_preposition_to_with", "verb_preposition_at_to", "verb_preposition_from_about", "verb_preposition_in_to", "verb_preposition_over_with", "verb_preposition_toward_from", "verb_preposition_with_against", "verb_preposition_about_into", "verb_preposition_into_with", "verb_preposition_of_into", "verb_preposition_about_against", "verb_preposition_through_with", )
...
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES, **v213.NAMES, **v215.NAMES, **v217.NAMES, **v219.NAMES, **v221.NAMES, **v223.NAMES, **v225.NAMES, **v227.NAMES, **v229.NAMES, **v231.NAMES, **v233.NAMES, **v235.NAMES, **v237.NAMES, **v239.NAMES, **v241.NAMES, **v243.NAMES, **v245.NAMES, **v247.NAMES, **v249.NAMES, **v251.NAMES, **v253.NAMES, **v255.NAMES, **v257.NAMES, **v259.NAMES, **v261.NAMES, **v263.NAMES, **v265.NAMES, **v267.NAMES}


def _parent_cdas(n):
    # the parent battery receipt that produced this member's units (each behaviour name is unique across batteries)
    for path in (V267, V265, V263, V261, V259, V257, V255, V253, V251, V249, V247, V245, V243, V241, V239, V237, V235, V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v258", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V258_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V258_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v258", "candidate_id": "corpus.unit_family_separability_spec_v258",
