...
"""Semantic attractor number-removal control on48 opened role-audited rows.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v696_result.json'
PREDICTIONS=dict(pred_a_exact_replay='original instrument plusattractor normpreservation1e-5 and4prefix/8block11/8suffixcalls',
 pred_b_effect_prediction='pooled subject/attractor numbernorm>=5 A/B andattractor-edited capability>=.90 everycell',
...
def main():
    ...
    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,cells=cells,
                ...
                scope='Semantic non-subject noun control on48opened v691rows; sameembedding-number removal atpairedattractor, A/Bsourceports patchedonlyatthatpre11site. Native subject labels fixed. Not normmatched tosubject (inputnormsreported). All native context/reader/sourcegenerators charged; no fullmodel adoption.')
