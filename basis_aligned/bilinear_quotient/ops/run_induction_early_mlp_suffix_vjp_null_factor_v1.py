...
p=prior.p
ROWS=prior.ROWS; PREREG=POLY/'INDUCTION_EARLY_MLP_SUFFIX_VJP_NULL_FACTOR_V1_PREREGISTRATION.md'; BINDING=POLY/'INDUCTION_EARLY_MLP_SUFFIX_VJP_NULL_FACTOR_V1_BINDING.json'; OUT=POLY/'INDUCTION_EARLY_MLP_SUFFIX_VJP_NULL_FACTOR_V1_RESULT.json'
...
def main():
 ...
 try:
  endpoint_logits,endpoint_terms,endpoint_writes=prior.capture_native(runtime,endpoints); endpoint_index={row['endpoint_id']:i for i,row in enumerate(endpoints)}; recipient=np.array([endpoint_index[row['recipient_endpoint_id']] for row in rows]); donor=np.array([endpoint_index[row['donor_endpoint_id']] for row in rows]); specs=[endpoints[i] for i in recipient]; tokens=p.fixed_tokens(specs,'token_ids'); native=endpoint_logits[recipient]; recipient_terms=endpoint_terms[recipient]; donor_terms=endpoint_terms[donor]; native_writes=endpoint_writes[recipient]; arms={}; active={}; restored={}; parnorm={}; nullnorm={}; partitions=[]; observations=[]; axis_batches=[]; joint_batches=[]; joint_active=[]
