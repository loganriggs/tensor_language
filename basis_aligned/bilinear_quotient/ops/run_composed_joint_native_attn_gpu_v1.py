#!/usr/bin/env python3
# BQGATE:2prefixes;5implementations;7timingrepeats;180seconds.
"""pred_a all state replay relative errors<=1e-5.
pred_b responsefoldednativeattention>=10%faster than nativebatched at12pairs bothprefixes.
pred_c responsefoldednativeattention no slower than nativebatched at1pair bothprefixes.
Null: FP64 generator overhead defeats algebraic savings on GPU.
Price2nativeprefixes,1/4/12pairs,5variants,7timingrepeats,180seconds.
"""
...
import composed_joint_native_attention_v1 as native_fold
from regional_cue_row_check_v1 import validate


@torch.no_grad()
def main():
    files=json.loads((P/'COMPOSED_JOINT_NATIVE_ATTN_GPU_V1_BINDING.json').read_text())['files']
    assert all(sha256(Path(k).read_bytes()).hexdigest()==v for k,v in files.items())
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
        print('2pristineprefixes,1/4/12pairs,5implementations,7timingrepeats;180seconds');return
    assert not (P/'COMPOSED_JOINT_NATIVE_ATTN_GPU_V1_RESULT.json').exists()
    ...
    for index in (0,96):
        ...
        def native_attention_folded(count):
            context=native_fold.prepare(z9,arguments[1],raw0,v1,program,float(b10.lambdas[0]),b10.attn)
            return post(native_fold.branch(torch.cat(amplitudes(count),dim=0),context))
        variants={'native_serial':native_serial,'native_batch':native_batch,'shared_serial':shared_serial,'shared_batch':shared_batch,'native_attention_folded':native_attention_folded}
        for count in (1,4,12):
            ...
            cells.append(dict(row=index,tokens=len(rows[index]['ids']),pairs=count,state_errors=errors,
                              ...
                              matched_batched_speed_ratio=medians['native_batch']/medians['native_attention_folded']))
    ...
    result={'pred_a':all(max(c['state_errors'].values())<=1e-5 for c in cells),
            ...
            'scope':'Native FP32 MLP9/attention10/MLP10 vs validated FP64 generator followed by FP32 MLP10. Both serial and batched on fixed historical rows0/96,1/4/12amplitude pairs. Includes preparation. Additional candidate: FP64 MLP9 response, native FP32 attention10/MLP10. Candidate has worse FineWeb pilot fidelity; timing is not adoption. Synchronized wall time. Pristine context and suffix excluded equally; no full-model performance or freshOOD claim.'}
    (P/'COMPOSED_JOINT_NATIVE_ATTN_GPU_V1_RESULT.json').write_text(json.dumps(result,indent=2)+'\n')
