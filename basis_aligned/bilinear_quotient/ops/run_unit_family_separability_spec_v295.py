...
"""v295: RE-RUN of the six v289 members that died with CUDA OOM, in a rung small enough to fit.

WHY. v289 evaluated 46 members across BOTH preposition families, preparing 72 members on device. Six of them raised
`OutOfMemoryError: CUDA out of memory` and the runner recorded each as `separable: false` WITH an error field --
verb_preposition_for_toward, from_onto and to_through (three of v289's eleven NEW members) and in_beyond,
through_with and toward_from (three counted incumbents). Those six verdicts are CRASHES, NOT MEASUREMENTS. They are
undecided, and ops/circuit_count.py was patched at 02:44 to ignore any member carrying an `error` so a crash cannot
be counted as a fusion. This rung decides them properly.
WHAT CHANGED TO MAKE IT FIT, and what did NOT. The recipe, bars and semantics are IDENTICAL to v289: same units frozen
from the same parent batteries, same own/fam arms, same control weight, same separability definition, same 0.05 cross
bar. The ONLY change is scope -- this rung prepares the verb_preposition family alone (44 members) instead of both
families (72), because the fam arm holds every prepared sibling's prep on device at once and that is what exhausted
memory. Each evaluated member still sees its FULL family as controls, so no member is judged against a smaller control
set than it would have had at v289; the adjective family is simply absent, and no evaluated member here belongs to it.
THIS IS THE MEMORY VERSION OF THE COST CURVE. The 22:59 measurement showed seconds per member growing linearly in
family size, making a full sweep quadratic; v289 shows the same growth hitting a MEMORY ceiling first, at family 44.
Board proposal (ix) -- sample the counted re-checks instead of sweeping them -- would have avoided both, and it is
still unanswered. What this rung demonstrates on its own is narrower and worth registering: at the current family size
a two-family sweep does not fit, so future rungs are per-family until (ix) is answered.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
    pred_a_new_separable   at least 2 of the 3 undecided NEW members are separable (k_new). v289's eight members that
                           DID complete were 8 of 8 separable, and all three of these were authored in batches whose
                           cue freshness was script-verified, so the cue-overlap account predicts all three.  prior 75%
    pred_b_counted_kept    all 3 of the undecided counted incumbents remain separable (k_kept = 3). in_beyond,
                           through_with and toward_from were separable when last measured (v258, v281, v254).  prior 70%
    ...
    pred_d_instrument      every evaluated member's own-arm extraction is within 0.03 of its parent battery's cdas
                           extraction_held. THIS IS THE ONE THAT MATTERS HERE: v289's pred_d failed only because the
                           OOM members had no extraction to compare, so a pass here says the six are measurable at
                           this scope and the failure was scope, not instrument.                               prior 90%
COUNTING. Each separable NEW member is +1 on top of the 121 the count tool reports (which excludes all six as
undecided). A counted incumbent that comes back non-separable is a genuine retraction and gets reported; one that
comes back separable simply keeps its count, which the tool already assumes.
EVAL = 6 members (3 new + 3 counted re-checks) against the full 44-member verb family: ~45 min at the measured
428 s/member.
Smoke: V295_SMOKE=<out.json> (CPU, V295_SMOKE_ROWS=4; V295_SMOKE_NAMES=verb_preposition_for_toward,verb_preposition_in_beyond).
BARS = {"cross_max": 0.05, "keep": 0.80, "ext_floor": 0.50, "c_ub_max": 0.01, "k_d": 3, "k_new": 2, "k_kept": 3, "instr_tol": 0.03}
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v295_result.json"
...
EVAL = ("verb_preposition_for_toward", "verb_preposition_from_onto", "verb_preposition_to_through", "verb_preposition_in_beyond", "verb_preposition_through_with", "verb_preposition_toward_from", )
COUNTED = ("verb_preposition_in_beyond", "verb_preposition_through_with", "verb_preposition_toward_from", )
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v295", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V295_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V295_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    evaluate = [n for n in members if n in EVAL and (not smoke or n in os.environ.get("V295_SMOKE_NAMES", "verb_preposition_for_toward,verb_preposition_in_beyond").split(","))]
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v295", "candidate_id": "corpus.unit_family_separability_spec_v295",
