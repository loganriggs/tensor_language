...
"""The templates with the POST-NOUN difference, block 0 guarded (v530; v529 scored nine NaN heads at block 0, where the post-noun residual is the same embedding in both rows and d = 0). v525 / v528 used d = the mean plural - singular residual at the NOUN. Two heads read the post-noun
...
PREDICTIONS (scored as written; failures preserved; priors from v478 / v525)
    ...
    pred_c_5_3_rank_improves                5.3's rank over 162 on the agreement template is <= 5 (noun-side d: 7, v525; v529 past the NaNs: 2)
    ...
    pred_e_copier_rank_improves             4.5's rank on the pronoun template is <= 6 or on the agreement template <= 5. Prior: v529 past the NaNs says NO (25 / 68) — expected to fail; kept as registered
...
OUT = ROOT / "circuits/followups/templates_post_noun_d_guarded_v530_result.json"
CANDIDATE_ID = "template.post_noun_d_v530"
N_HEAD = 9
...
def main() -> None:
    ...
    for l, h in READERS:
        ...
        for name, uu in (("pronoun", u.cpu()), ("verb", u_v.cpu())):
            T = torch.outer(uu, d)
            if float(T.norm()) < 1e-6:
                for hh in Ms: scores[name][f"{l}.{hh}"] = 0.0                      # d = 0 at this block (block 0 at the post-noun token): no template, score 0
                continue
            Tn = T / T.norm()
    ...
    OUT.write_text(json.dumps({"schema": "templates_post_noun_d_guarded_result_v530", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
