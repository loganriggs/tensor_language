#!/usr/bin/env python3
# BQGATE:6 body forwards + one14-block zero-input anchor;32 sequences lengths7-11;5 suffix arms;no fit;180sec.
"""pred_a native producer and saved first-branch write replay<=1e-5;
pred_b anchored-token routing branch write relative error<=.1;
pred_c anchored-token regional signed effect error<=.1 plus preserved controls.
Null: contextual routing input cannot be replaced by a weight-generated bias anchor plus exact embedding re-entry.
...
STEM='REGIONAL_ANCHORED_ROUTING_V2'
@torch.no_grad()
def main():
    ...
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('6 body forwards + one14-blockzeroanchor32sequences7-11tokens;5suffix arms;no fitting');return
    ...
    anchor_zero=torch.zeros(1,11,1152,device='cuda');anchor_x=anchor_zero.clone();anchor_v=None;anchor_raw={};embedding_coeff={};coefficient=anchor_zero.new_tensor(1.)
    for j,block in enumerate(model.transformer.h[:14]):
        coefficient=block.lambdas[0]*coefficient+block.lambdas[1]
        if j in (8,9,13):anchor_raw[j]=(block.lambdas[0]*anchor_x+block.lambdas[1]*anchor_zero).detach();embedding_coeff[j]=coefficient.clone()
        anchor_x,anchor_v=block(anchor_x,anchor_v,anchor_zero)
    producer_errors=[]
    ...
    def producer_hook(layer,module,args,out):
        ...
        nq,nk,nq2,nk2=qk(inp);tq,tk,tq2,tk2=qk(F.rms_norm(anchor_raw[layer][:,:length]+embedding_coeff[layer]*captured['token_embeddings'],(1152,)))
    ...
    for arm,name in enumerate(('anchored_queries_anchored_keys','native_queries_anchored_keys','anchored_queries_native_keys'),2):
    ...
    result.update(native_branch_replay_error=replay,producer_replay_errors=producer_errors,native_attention_replay_errors=replays,cells=cells,body_forwards=count,zero_anchor_blocks=14,embedding_reentry_coefficients={str(k):float(v) for k,v in embedding_coeff.items()},anchor_raw_norms={str(k):float(v.norm()) for k,v in anchor_raw.items()},seconds=time.perf_counter()-tic,artifact_sha=digest(art),source_shas=binding,scope='Zero-input weight-generated anchor plus exact embedding-reentry QK baseline and query/key hybrids. BothQK factorsjoint; projectedfirstvalues andremainingdownstreamcontextfixed. No datafit. Failure rejects this anchored-token routing approximation, not all context-independent or weight-based circuits.')
