#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_five_writes_carry_most_on_the_panel pred_c_rest_of_their_blocks_small_on_the_panel pred_d_all_attention_carries_the_panel_margin pred_e_five_are_most_of_the_channel_on_the_panel
"""The pronoun channel on the PANEL (v489). v485 (natural pairs): the five pronoun heads' writes into the answer jointly close 0.803 of the margin, the
other 32 heads of their blocks 0.026, all attention 0.973. The same three edits on the v76 panel pairs (48 pairs, a different frame): does the channel
replay across frames at the answer side as it did at the noun side (v433: 0.340)?
PREDICTIONS (scored as written; failures preserved; priors from v433 / v485)
    pred_a_baseline_replays                        the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_five_writes_carry_most_on_the_panel     the five heads' joint write swap closes >= 0.60 of the panel margin gap. Prior: unsure (values at the noun: 0.34)
    pred_c_rest_of_their_blocks_small_on_the_panel the other 32 heads of their blocks close <= 0.06 jointly
    pred_d_all_attention_carries_the_panel_margin  every block's attention write into the answer closes >= 0.85
    pred_e_five_are_most_of_the_channel_on_the_panel  the five heads' joint swap is >= 0.70 x the all-attention swap
...
OUT = ROOT / "circuits/followups/pronoun_channel_panel_v489_result.json"
CANDIDATE_ID = "chain.pronoun_channel_panel_v489"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, FIVE_MIN, REST_MAX, ALL_MIN, SHARE_MIN, UNIT = 1e-3, 0.60, 0.06, 0.85, 0.70, 701
...
FORWARDS_MAX = 13
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_five_writes_carry_most_on_the_panel": ">= 0.60", "pred_c_rest_of_their_blocks_small_on_the_panel": "<= 0.06", "pred_d_all_attention_carries_the_panel_margin": ">= 0.85", "pred_e_five_are_most_of_the_channel_on_the_panel": ">= 0.70 x all"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "five_min": FIVE_MIN, "rest_max": REST_MAX, "all_min": ALL_MIN, "share_min": SHARE_MIN, "unit": UNIT}}
    ...
    ALL_W = {l: list(range(N_HEAD)) for l in range(18)}
    MODES = {"FIVE_W": ({}, (0,)), "REST_W": ({}, (0,)), "ALL_W": ({}, (0,))}; SETS_W = {"FIVE_W": FIVE_W, "REST_W": REST_W, "ALL_W": ALL_W}; forwards = 0
    def logits_margin(x):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_five_writes_carry_most_on_the_panel": mc["FIVE_W"] >= FIVE_MIN, "pred_c_rest_of_their_blocks_small_on_the_panel": abs(mc["REST_W"]) <= REST_MAX,
                   "pred_d_all_attention_carries_the_panel_margin": mc["ALL_W"] >= ALL_MIN, "pred_e_five_are_most_of_the_channel_on_the_panel": mc["FIVE_W"] >= SHARE_MIN * mc["ALL_W"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pronoun_channel_panel_result_v489", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
