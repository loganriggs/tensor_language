#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_tilts_cancel_in_the_pair pred_c_pair_shares_the_axis pred_d_five_replay pred_e_pair_signs
"""Do the two gender tilts cancel (v493)? v492: 12.4 pushes masculine forms down 1.32x the feminine, 10.1 the feminine more (0.88). Here the pair 12.4 + 10.1
swapped together (values at the noun) scored by lemma, with the five-head swap as the replay (masculine / feminine fall 1.113); 122 natural pairs.
PREDICTIONS (scored as written; failures preserved; priors from v492)
    pred_a_baseline_replays      the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_tilts_cancel_in_the_pair  the pair's masculine / feminine fall ratio is between 0.95 and 1.25 (closer to even than 12.4's 1.32)
    pred_c_pair_shares_the_axis  the pair's lemma vector has cosine >= 0.95 with the five-head vector
    pred_d_five_replay           the five-head ratio replays 1.113 +- 0.05
    pred_e_pair_signs            under the pair's swap every third-person singular form falls and every third-person plural form rises
...
OUT = ROOT / "circuits/followups/tilt_pair_v493_result.json"
CANDIDATE_ID = "chain.tilt_pair_v493"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, PAIR_LO, PAIR_HI, COS_MIN, FIVE_V492, BAND = 1e-3, 0.95, 1.25, 0.95, 1.113, 0.05
...
FORWARDS_MAX = 22
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_tilts_cancel_in_the_pair": "0.95-1.25", "pred_c_pair_shares_the_axis": "cos >= 0.95", "pred_d_five_replay": "1.113 +- 0.05", "pred_e_pair_signs": "signs"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "pair": [PAIR_LO, PAIR_HI], "cos_min": COS_MIN, "five_v492": FIVE_V492, "band": BAND}}
    ...
    RD = dict(READERS)
    MODES = {"FIVE_N": ({l: hs for l, hs in FIVE.items()}, (0,)), "PAIR": ({12: [4], 10: [1]}, (0,))}; forwards = 0
    def logits_margin(x):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_tilts_cancel_in_the_pair": PAIR_LO <= text["PAIR"]["masc_over_fem"] <= PAIR_HI, "pred_c_pair_shares_the_axis": text["PAIR"]["cos_with_five"] >= COS_MIN,
                   "pred_d_five_replay": abs(text["FIVE_N"]["masc_over_fem"] - FIVE_V492) <= BAND, "pred_e_pair_signs": text["PAIR"]["signs_ok"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "tilt_pair_result_v493", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
