"""STREAM-DRIFT LEDGER OF THE CP FRONT (rung 115): is the 3.3x compounding amplification in STATE SPACE, or only in the CE surface?

CONVENTION (S2135): CE numbers are damage above the real model, lower is better. Here the measured object is
relative stream drift r_li = rms(h_config - h_real)/rms(h_real) at the output of blocks 0..9, on 200 census
rows, for: cp_front (S2206 Ks: m0 2304/m1 2304/m2 1152/m3 576 + a0 + a1v) and the four CP singles.

REGISTERED PREDICTIONS (arms named):
  (a) DRIFT IS SUPERADDITIVE TOO: r_3(cp_front) >= 1.5 x [r_3(m0)+r_3(m1)+r_3(m2)+r_3(m3)] (state-space
      amplification, not just CE curvature).
  (b) DRIFT PERSISTS INTO THE MIDDLES: r_9(cp_front) >= 0.8 x r_3(cp_front) (blocks 4-9, all real here, do
      not wash the drift out).
  (c) SANITY: r_3(single m3) <= r_3(cp_front); every r in [0.0005, 2].
NULL: drift is ADDITIVE (r_3(cp_front) ~ sum of singles) - then the 3.3x CE excess lives in the loss surface
(curvature), not in state amplification, and K-escalation (rung 114) should buy it back cheaply.
PRICE: none (diagnostic). Tripwire: INSTRUMENT FAIL if r_3(cp_front) < 0.005 (inert install). Self-reviewed."""
import json, sys, time, os
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
if os.environ.get('BQLIB_DRYRUN')=='1':
    _bq='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
    _need=['frontier_cp_front_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: CP-front stream-drift ledger')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_cp_drift_results.json'
...
def main():
    ...
    if SEL.get('drift'):
        ...
        for _li,_K in _KS.items():
            ...
            S[f'cf{_li}']=('cp',_li,_L[_kp].contiguous(),_R[_kp].contiguous(),
                           _Dw[:,_kp].contiguous(),_db)
        def _runcap(_act,_ref=None):
            hs=install(_act)
            caps={li:[] for li in range(10)} if _ref is None else None
            acc={li:0.0 for li in range(10)}; den={li:0.0 for li in range(10)}
            bi=0
            for _i in range(0,200,4):
                ...
                for _lj,blk in enumerate(m.transformer.h):
                    x,v1=blk(x,v1,x0)
                    if _lj<10:
                        xf=x.detach().reshape(-1,D).float()
                        if _ref is None:
                            caps[_lj].append(xf.to(torch.float16))
                        else:
                            rf=_ref[_lj][bi].float()
                            acc[_lj]+=float((xf-rf).pow(2).sum())
                            den[_lj]+=float(rf.pow(2).sum())
                    if _lj==9: break
                bi+=1
            for h in hs: h.remove()
            if _ref is None: return caps
            return {li:(acc[li]/max(den[li],1e-9))**0.5 for li in range(10)}
        REAL=_runcap([])
        _cfgs={'cp_front':['a0','a1v','cf0','cf1','cf2','cf3'],
               'single_m0':['cf0'],'single_m1':['cf1'],'single_m2':['cf2'],'single_m3':['cf3']}
        _dr={}
        for _nm,_act in _cfgs.items():
            _dr[_nm]=_runcap(_act,REAL)
            print(f"  {_nm:9s} r: "+" ".join(f"{_dr[_nm][li]:.4f}" for li in range(10)),flush=True)
        SEL['drift_result']=_dr
...
if __name__=='__main__':
    ...
    SEL['drift']=True; SEL['ext_rows']=CROWS
    print('ARMS: stream drift r_li (blocks 0-9) for cp_front + four CP singles, 200 census rows',flush=True)
    main()
    D_=SEL['drift_result']
    r3f=D_['cp_front'][3]; r9f=D_['cp_front'][9]
    if r3f<0.005: raise SystemExit('INSTRUMENT FAIL: cp_front drift inert at block 3')
    ssum=sum(D_[f'single_m{li}'][3] for li in (0,1,2,3))
    pa=r3f>=1.5*ssum
    pb=r9f>=0.8*r3f
    allr=[v for nm in D_ for v in D_[nm].values()]
    pc=(D_['single_m3'][3]<=r3f) and all(0.0005<=v<=2 for v in allr)
    res={'drift':{nm:{str(li):round(v,5) for li,v in d.items()} for nm,d in D_.items()},
         'r3_cp_front':round(r3f,5),'r3_singles_sum':round(ssum,5),'r9_cp_front':round(r9f,5),
         'convention':'r_li = rms(h_config - h_real)/rms(h_real), block outputs, 200 census rows',
         'pred_a_drift_superadditive':bool(pa),'pred_b_drift_persists':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'r3(cp_front) {r3f:.4f} vs singles sum {ssum:.4f}; r9 {r9f:.4f}')
    print(f"(a) r3 {r3f:.4f} >= 1.5 x sum ({1.5*ssum:.4f}): {'HELD' if pa else 'FAILED'}")
    print(f"(b) r9 {r9f:.4f} >= 0.8 x r3 ({0.8*r3f:.4f}): {'HELD' if pb else 'FAILED'}")
    print(f"(c) singles<=front + range: {'HELD' if pc else 'FAILED'}")
