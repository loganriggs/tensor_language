#!/usr/bin/env python3
# BQLANE: cpu
"""Model-free and adversarial tests for the task-21 capability producer."""
...
import circuit_battery_task21_capability_fit_producer as producer
import circuit_experiment_spec as framework


OPS = Path(__file__).resolve().parent
AUTHORITY = OPS / "circuit_battery_task21_copy_fit_authority.json"
SOURCE = OPS / "circuit_battery_task21_capability_fit_producer.py"
...
def passing_evaluator(records=None):
    ...
    def evaluate(call, sequences, targets, foils):
        ...
        return np.full(21, 2.0, dtype="<f4"), np.full(21, -1.0, dtype="<f4")
...
def test_every_row_side_is_physically_evaluated_without_prompt_deduplication() -> None:
    ...
    assert sum(item["sequences"].shape[0] for item in seen) == 168
    unique = {tuple(row) for item in seen for row in item["sequences"].tolist()}
    assert len(unique) == 105 < 168
    assert len(primitives) == 168
    assert framework.canonical_sha256(completed) == capability.CALL_MANIFEST_SHA256
    assert producer.evidence_numeric_bytes(evidence) == 1344
...
def test_malformed_model_arrays_fail_before_a_decision(attack: str) -> None:
    ...
    def malformed(_call, _sequences, _targets, _foils):
        answer = np.zeros(21, dtype="<f4")
        maximum = np.zeros(21, dtype="<f4")
        if attack == "shape":
            answer = answer[:-1]
        elif attack == "dtype":
    ...
    with pytest.raises(producer.ProducerError, match=r"float32\[21\]"):
...
def test_late_destination_race_is_never_overwritten_and_prior_moves_roll_back(
    ...
    with pytest.raises(FileExistsError, match="destination exists"):
        producer.publish_task21_package(stage, paths, before_move=plant)
    ...
    raced.unlink()
    producer.publish_task21_package(stage, paths)
...
def test_crash_after_each_install_rolls_back_to_a_retryable_complete_stage(
    ...
    with pytest.raises(RuntimeError, match="planted crash"):
        producer.publish_task21_package(stage, paths, crash=crash)
...
def test_rollback_never_moves_an_externally_substituted_inode(tmp_path: Path) -> None:
    ...
    with pytest.raises(producer.ProducerError, match="safe rollback was incomplete"):
        producer.publish_task21_package(stage, paths, crash=substitute)
...
def test_successful_publication_installs_receipt_last_and_is_complete(tmp_path: Path) -> None:
    ...
    producer.publish_task21_package(stage, paths, before_move=observe)
...
def test_fake_runtime_publishes_one_atomic_bound_package(
    ...
    assert receipt["example_evaluations"] == 168
    assert receipt["raw_numeric_evidence_bytes"] == 1344
