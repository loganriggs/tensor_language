# COMPRESSING THE COST TERM THAT ACTUALLY DOMINATES -- and the arc's first REGISTERED cost prediction
#
# §1754, caught by Codex: every cost figure in §1748-§1753 counted only the rank-8 factors (0.664M
# reals) and left out the per-token tables, which cost 36 x 5419 x 1152 = 224.737M -- 339x more. Two
# published claims reversed: the compiled program is not 26-37x more cost-efficient than the native
# allocation, it is 1.92x LESS. §1720 had already recorded that "the cost axis has had no instrument
# checks at all" because every registered prediction in this arc has been about fidelity. That is
# still true after four cost-axis errors, all four found by Codex.
#
# So: attack the dominant term, and register a prediction that a cost figure has to satisfy.
#
# The covered block of a site's table is 5419 x 1152. Rank-r truncating it costs r*(5419+1152) reals
# instead of 6.243M -- 14.8x smaller at r=64. Four table ranks, each with its own interleaved
# bottom-up compile of the rank-8 linear correction, each priced in FULL.
#
# pred_a below is the first prediction in this arc whose failure can be a COST failure rather than a
# fidelity failure, and it is a conjunction so that neither half can carry it alone.
#
# STILL A LOWER BOUND, and this matters more than the factor of 339: the hybrid hook (§1661) runs the
# LIVE module wherever the token was not covered at fit time, which is 24% of scored positions. None
# of these programs stands alone; all of them need the original 430.00M of modules present for that
# quarter, and none of these figures prices it.
#
...
#   pred_a A TENTH THE COST FOR FOUR FIFTHS THE FIDELITY: at table rank 64 the program keeps >= 80%
#          of the full-table program's held-out recovery AND costs <= 10% of its total reals. Both
#          halves are required, so the arithmetic half cannot carry it. If FALSE, the tables are not
#          compressible at a rate that helps and the cost axis stays where §1754 left it.
#   pred_b COST-EFFICIENCY IMPROVES MONOTONICALLY as the table rank falls from full through 256, 64,
#          16. If FALSE there is an efficiency optimum in the interior, which is a more useful thing
#          to know than a monotone trend and names the design point directly.
#   pred_c RANK 16 BREAKS IT: recovery at table rank 16 falls below half the full-table value. If
#          FALSE the tables are compressible far past where I expect and the sweep should go lower --
#          under-budgeting the sweep is the defect LESSONS 31's addendum records twice.
#   pred_d CONTROLS: the FULL-table arm reproduces §1748's +0.40631 and +0.38578 within 0.002 -- by
#          construction it is the same program, rebuilt in a fifth script -- plus table-only CE
#          7.35114, live CE 3.29205, and coverage exactly 5419 of 50257, asserted rather than
#          reported since every cost figure here is computed from it.
...
TABLE_RANKS = (None, 256, 64, 16)   # None = the full covered table
PT = '/workspace/tensor_language/basis_aligned/bilinear_quotient/'
OUT = PT + 'ops/table_rank_compression_results.json'
...
def main():
    ...
    ho = 'skip11000'
    full = out['full']['recovered'][ho]
    pa = (out['64']['recovered'][ho] >= 0.80 * full
          and out['64']['total_cost_M'] <= 0.10 * out['full']['total_cost_M'])
    eff = [out[k]['nats_per_Mreal'] for k in ('full', '256', '64', '16')]
    pb = all(eff[i] < eff[i + 1] for i in range(len(eff) - 1))
    pc = out['16']['recovered'][ho] < 0.50 * full
    pd = (all(abs(out['full']['recovered'][e] - v) <= 0.002 for e, v in S1748_A.items())
          and abs(base['skip7000']['table_only'] - S1738_PROGRAM_CE) <= 0.005
          and abs(base['skip7000']['live'] - 3.29205) <= 1e-3 and ncov == NCOV and fits_ok)

    print(f'\n  rank 64 keeps >=80% of full fidelity ({out["64"]["recovered"][ho]:+.5f} vs '
          f'{full:+.5f}) AND costs <=10% of it ({out["64"]["total_cost_M"]:.3f}M vs '
          f'{out["full"]["total_cost_M"]:.3f}M) -> {pa}', flush=True)
    print(f'  cost-efficiency improves monotonically as the table shrinks {eff} -> {pb}', flush=True)
    print(f'  rank 16 falls below half of full fidelity ({out["16"]["recovered"][ho]:+.5f}) -> {pc}',
          flush=True)
    print(f'  full-table arm reproduces §1748 + table-only CE + live CE + coverage {ncov} -> '
    ...
    r2 = {'config': {'correction_rank': RANK, 'table_ranks': [str(x) for x in TABLE_RANKS],
          ...
          'by_table_rank': out,
          'predictions': {'pred_a_rank64_keeps_fidelity_at_a_tenth_the_cost': bool(pa),
                          'pred_b_efficiency_monotone': bool(pb),
                          'pred_c_rank16_breaks': bool(pc),
