#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_two_verb_readers_carry_a_quarter pred_c_verb_readers_and_copier_add pred_d_verb_readers_and_pronoun_readers_add pred_e_named_verb_route_bounded
"""The verb readout's named route edited (v450). v448 / v449: heads 11.3 and 7.8 carry the noun's number to a distant verb as value copies (0.147 + 0.102
singly). Here, on the 79 distant rows at the verb slot, the named sets at the noun: the two verb readers together (V2); V2 + the copier 4.5; V2 + the five
pronoun readers; and V2 + copier + pronoun readers. Exits at one position add (v436), so the sums should match the parts.
PREDICTIONS (scored as written; failures preserved; priors from v446 / v448)
    pred_a_baseline_replays                   the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_two_verb_readers_carry_a_quarter   V2 closes >= 0.22 of the verb margin gap (singles 0.147 + 0.102)
    pred_c_verb_readers_and_copier_add        V2 + copier is within 0.05 of V2 + 0.138 (v446)
    pred_d_verb_readers_and_pronoun_readers_add  V2 + pronoun readers is within 0.05 of V2 + 0.072 (v446)
    pred_e_named_verb_route_bounded           V2 + copier + pronoun readers closes <= 0.60 (the rest is blocks 9 / 13 / 2 / 8 and the small exits)
...
OUT = ROOT / "circuits/followups/verb_named_route_v450_result.json"
CANDIDATE_ID = "chain.verb_named_route_v450"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, V2_MIN, ADD_TOL, COPIER_V446, READERS_V446, ALL_MAX = 1e-3, 0.22, 0.05, 0.138, 0.072, 0.60
...
FORWARDS_MAX = 16
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_two_verb_readers_carry_a_quarter": ">= 0.22", "pred_c_verb_readers_and_copier_add": "within 0.05", "pred_d_verb_readers_and_pronoun_readers_add": "within 0.05", "pred_e_named_verb_route_bounded": "<= 0.60"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "v2_min": V2_MIN, "add_tol": ADD_TOL, "copier_v446": COPIER_V446, "readers_v446": READERS_V446, "all_max": ALL_MAX}}
    ...
    V2 = {11: [3], 7: [8]}
    MODES = {"V2": (V2, (0,)), "V2_C": ({**V2, 4: [5]}, (0,)), "V2_P": ({**V2, **FIVE}, (0,)), "V2_C_P": ({**V2, **FIVE, 4: [5]}, (0,))}; forwards = 0
    def logits_margin(x):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_two_verb_readers_carry_a_quarter": v["V2"] >= V2_MIN, "pred_c_verb_readers_and_copier_add": abs(v["V2_C"] - (v["V2"] + COPIER_V446)) <= ADD_TOL,
                   "pred_d_verb_readers_and_pronoun_readers_add": abs(v["V2_P"] - (v["V2"] + READERS_V446)) <= ADD_TOL, "pred_e_named_verb_route_bounded": v["V2_C_P"] <= ALL_MAX}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "verb_named_route_result_v450", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
