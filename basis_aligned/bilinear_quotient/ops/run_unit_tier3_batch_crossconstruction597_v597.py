...
"""v597: work the AUTHORED BACKLOG instead of authoring more cells.
...
Smoke: V597_SMOKE=<out.json> (CPU, V597_SMOKE_ROWS=4).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_crossconstruction597_v597_result.json"
...
NAMES = {"comparative_superlative_two_three": "comparative_superlative_two_three",
         ...
         "determiner_number_ropes": "determiner_number_ropes",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_crossconstruction597_v597", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V597_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V597_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V597_SMOKE_NAMES", "comparative_superlative_two_three,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_crossconstruction597_v597", "candidate_id": "corpus.unit_tier3_batch_crossconstruction597_v597", "bars": BARS,
