#!/usr/bin/env python3
# BQGATE: 50forwards400seq128tokens; 384 frozen four-domain Pile endpoints, no fitting.
"""Frozen-branch four-domain corpus-shift validation of suppression.
...
Original support prediction remains failed. Corpus shift, not verified training-disjointness.
pred_d: own negative mean CE and collateralabs<=.05 in all four domains.
...
def main():
    ...
    domains = ['reference', 'discussion', 'biomedical', 'legal_patent']
    assert all(panel['domain_labels'].count(domain) == 32 for domain in domains)
    ...
    for i, record in enumerate(panel['metadata']):
        ...
        assert 128 <= pos < record['source_length'] <= 513
        assert record['domain'] == panel['domain_labels'][i % 128]
    ...
    indices = torch.randint(128, (2000, 128), generator=torch.Generator().manual_seed(5402))
    ...
    for domain in domains:
        selected = torch.tensor([i for i, label in enumerate(panel['domain_labels']) if label == domain])
        local = effects[:, selected]
        means_d = local.mean(1)
        own_d = torch.stack((means_d[0, 0], means_d[1, 1]))
        collateral_d = local[2, :, :2].abs().mean(0)
        per_domain[domain] = dict(documents=len(selected), mean_ce_added=means_d.tolist(),
            native_top20_fraction=(ranks[:, selected] <= 20).double().mean(1).tolist(),
            own_ce_added=own_d.tolist(), mean_absolute_control_ce_change=collateral_d.tolist(),
            held=bool((own_d < 0).all() and (collateral_d <= .05).all()))
    pred_d = valid and all(row['held'] for row in per_domain.values())
    ...
    result = dict(pred_a=valid, pred_b=pred_b, pred_c=pred_c, pred_d=pred_d, per_domain=per_domain,
        ...
        scope='Frozen original weight-derived branches on four Pile domains after FineWeb validation. Exact document hashes exclude the old2048document panel. Corpus shift does not prove training disjointness, historically untouched data, near-duplicate removal, sufficiency or circuit identification. No factor fitting; original support-direction failure retained.')
    with output.open('x') as f: json.dump(result, f, indent=2); f.write('\n')
    print(json.dumps({k:result[k] for k in ('pred_a', 'pred_b', 'pred_c', 'pred_d', 'mean_ce_added', 'per_domain', 'seconds')}), flush=True)
