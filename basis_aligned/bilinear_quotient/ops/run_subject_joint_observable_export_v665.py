...
"""Export same-width joint-observable response frame for target and preservation readers.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_joint_chain_v665_result.json'
...
def main():
    ...
    a=max(checks)<=1e-4 and max(absolute)<=1e-4 and max(reader_checks)<=1e-4 and max(encoder_checks)<=1e-8 and max(packed_checks)<=1e-8 and counts==dict(prefix_calls=16,suffix_calls=20)
    ...
    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,
        ...
        scope='Four equal-weight target/preservation finite-reader contrasts; balanced dual-space width8 with unchanged256row calibration, 9216 extra initial-encoder values charged, same calibration/evaluation data; final projection is oracle. Data-informed residual basis, no native circuit adoption.')
    artifact=OUT.parent/'subject_response_v665_program.pt'
