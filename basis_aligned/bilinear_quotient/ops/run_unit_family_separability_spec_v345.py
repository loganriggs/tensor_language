...
"""v345: HOW DIFFERENT MUST TWO FRAMES BE? Fourteen prepared, with the frame-A and frame-B copies of every new cell's
own cue mappings in the control set.

THE STATE OF THE ACCOUNT. A behaviour's identity is (frame, cue -> token mapping). Within a frame, reusing both
mappings fuses (v333: 0.0921 and 0.1163 into the two donors). Across a frame change it does not (v337: 0.0191), and
v341 confirmed that with the donors themselves as controls: fam-arm leak into the donor 0.0026 and 0.0243 against
own-arm leaks of 0.6577 and 1.0611 into those same donors. So the corpus's inventory is frames x mappings.
WHAT IS STILL UNMEASURED. Nothing says how DIFFERENT two frames must be for that to hold. Frame A is
`Near the {object} the {agent} {verb}, of course,`; frame B replaced the prefix with `Everyone agreed` and the
parenthetical with `frankly`; frame C uses `It turned out` and `apparently`. B and C differ from A in the same way
and from EACH OTHER in the same way -- a matrix clause and a parenthetical -- so if the frame axis is real at that
grain, the B and C copies of one mapping pair are two circuits; if the axis has a coarser grain, they fuse with each
other while both stay separable from A, and that would be the first measurement of the frame metric itself.
THE THREE NEW CELLS AND THEIR OLDER COPIES (all in this rung's control set, which is the point):
    verb_particle_fc_up_down       woke/calmed    copies: verb_particle_up_down (A, control-only), fb_up_down (B)
    verb_particle_fc_out_down      found/settled  copies: verb_particle_out_down (A, control-only), fb_out_down (B)
    verb_preposition_fc_about_for  cared/voted    copies: verb_preposition_about_for (A, control-only),
                                                          fb_about_for (B)
All three passed all four rows at v343 (n 14, 8, 12; held 0.900, 0.896, 0.878).
COST. Fourteen prepared, nine evaluated; the measured curve (19 + 9.3 x family_size) puts it near 35 min. Proposal
(xii) on the board -- split the family or trim the control set -- is unanswered, and this rung is exactly the case
that makes it urgent: the control set is no longer optional padding, it is the experiment.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
    pred_a_new_separable   at least k_new = 2 of the 3 new members are separable at the registered cross bar.
                                                                                                          prior 65%
    pred_b_counted_kept    at least k_kept = 5 of the 6 counted members remain separable.                 prior 75%
    pred_c_row4_kept       fam-arm own-C UB975 <= 0.01 on at least k_d = 4 of the 9 evaluated members.    prior 70%
    pred_d_instrument      every evaluated member's own-arm extraction is within 0.03 of its parent battery's cdas
                           extraction_held. Fails automatically if any member errors; fourteen prepared is the
                           largest this family has run.                                                    prior 85%
    pred_e_bc_distinct     verb_particle_fc_out_down's fam-arm leak into its FRAME-B copy, verb_particle_fb_out_down
                           -- the same two cue mappings one frame away -- stays under the cross bar in absolute
                           value: |leak into verb_particle_fb_out_down| < cross_max = 0.05.
                           This is the frame-grain question as one number, measured whether or not anything fuses,
                           on the FAM arm where all three of today's successful clauses were registered. Worked
                           example: 0.01 gives TRUE, B and C are two circuits and the frame axis is real at the grain
                           of a matrix clause plus a parenthetical; 0.11 -- the size v333 measured for within-frame
                           mapping reuse -- gives FALSE, and the honest reading would be that frames B and C are the
                           SAME frame as far as this model is concerned, that the earlier A-to-B result depended on
                           something specific to frame A's locative prefix, and that "frame" needs defining by
                           measurement rather than by my sentence-writing.                                prior 60%
COUNTING. Each separable new member is +1 on 142. A B/C fusion is +0 for that cell and is the more informative
outcome: it would bound the frame axis instead of extending it, and I would rather find that bound now, on three
cells, than after authoring a fourth and fifth frame under the assumption that any rewrite counts.
BARS = {"cross_max": 0.05, "keep": 0.8, "ext_floor": 0.5, "c_ub_max": 0.01, "k_d": 4, "k_new": 2, "k_kept": 5, "instr_tol": 0.03}
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v345_result.json"
...
FAMILIES = {
    "verb_preposition_replication": ["verb_preposition_repl_base", "verb_preposition_repl_twin", "verb_preposition_repl_reuse", "verb_preposition_repl_frame_same", "verb_preposition_repl_frame_fresh", "verb_particle_fb_out_down", "verb_preposition_fb_about_for", "verb_particle_fb_up_down", "verb_particle_out_down", "verb_preposition_about_for", "verb_particle_fc_up_down", "verb_particle_fc_out_down", "verb_preposition_fc_about_for", "verb_particle_up_down"],
}
EVAL = ("verb_particle_fc_up_down", "verb_particle_fc_out_down", "verb_preposition_fc_about_for", "verb_preposition_repl_base", "verb_preposition_repl_twin", "verb_preposition_repl_frame_same", "verb_preposition_repl_frame_fresh", "verb_particle_fb_out_down", "verb_preposition_fb_about_for", )
COUNTED = ("verb_preposition_repl_base", "verb_preposition_repl_twin", "verb_preposition_repl_frame_same", "verb_preposition_repl_frame_fresh", "verb_particle_fb_out_down", "verb_preposition_fb_about_for", )
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES, **v213.NAMES, **v215.NAMES, **v217.NAMES, **v219.NAMES, **v221.NAMES, **v223.NAMES, **v225.NAMES, **v227.NAMES, **v229.NAMES, **v231.NAMES, **v233.NAMES, **v235.NAMES, **v237.NAMES, **v239.NAMES, **v241.NAMES, **v243.NAMES, **v245.NAMES, **v247.NAMES, **v249.NAMES, **v251.NAMES, **v253.NAMES, **v255.NAMES, **v257.NAMES, **v311.NAMES, **v315.NAMES, **v319.NAMES, **v323.NAMES, **v327.NAMES, **v331.NAMES, **v335.NAMES, **v339.NAMES, **v343.NAMES}


def _parent_cdas(n):
    # the parent battery receipt that produced this member's units (each behaviour name is unique across batteries)
    for path in (V343, V339, V335, V331, V327, V323, V319, V315, V311, V257, V255, V253, V251, V249, V247, V245, V243, V241, V239, V237, V235, V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v345", "members": n, "families": list(FAMILIES),
...
def PREDS(R):
    ...
    CELL, BCOPY = "verb_particle_fc_out_down", "verb_particle_fb_out_down"
    e = False
    if CELL in good:
        sibs = good[CELL]["arms"]["fam"]["siblings"]
        if BCOPY in sibs:
            e = abs(sibs[BCOPY]) < B["cross_max"]
    return {"pred_a_new_separable": bool(a), "pred_b_counted_kept": bool(b), "pred_c_row4_kept": bool(c),
            "pred_d_instrument": bool(d), "pred_e_bc_distinct": bool(e)}


def main() -> None:
    if os.environ.get("BQLIB_DRYRUN") or os.environ.get("BQLIB_NO_MODEL"):
        print(json.dumps(_plan(), indent=2, sort_keys=True)); return
    smoke = os.environ.get("V345_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V345_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v345", "candidate_id": "corpus.unit_family_separability_spec_v345",
