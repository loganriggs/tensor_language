"""ATTENTION-SPLICE MECHANISM TEST (rung 117): do real a2/a3 transmit the CP-front compounding?

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows; LOWER IS BETTER.
S2207/S2209: the excess ladder grows at cf2/cf3 while state drift composes in quadrature and the middles
contract it. One mediator hypothesis: the REAL attention blocks a2/a3 read the drifted front stream and
transmit/amplify its damage. Splice: feed a2 and a3 their REAL (position-aligned) input streams, captured at
the same pre-hook point in a real pass (symmetric capture - the rung-92 pre-norm scale lesson), everything
else as in the mixed-K cp_front (S2206 Ks).

REGISTERED PREDICTIONS (arms named plain, spliced; full census):
  (a) ATTENTION IS A MEDIATOR: agg(spliced) <= 0.6 x agg(plain).
  (b) VALIDITY APPEARS: valid(spliced) >= 3 of 62.
  (c) REPRODUCTION: |agg(plain) - 0.9427| <= 0.015.
NULL: attention is a bystander - the drift transmits through the residual stream itself (agg(spliced) within
10% of plain). PRICE: none (mechanism test; a splice is not a deployable replacement).
Tripwire: INSTRUMENT FAIL if spliced cev is bitwise equal to plain (rung-61 inert-hook class). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: attention-splice mechanism test')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_attn_splice_results.json'
...
def main():
    ...
    if SEL.get('attn_splice'):
        ...
        for _li in (2,3):
            def _mkc(li=_li):
                def ph(mod,args):
                    _AI[li].append(args[0].detach().to(torch.float16))
                    return None
                return ph
            hs.append(m.transformer.h[_li].attn.register_forward_pre_hook(_mkc()))
        ...
        for h in hs: h.remove()
        cur['clsmap']=classify2(_ROWS).to(DEV)
        _act=['a0','a1v','cf0','cf1','cf2','cf3']
        _arms={'plain':evalV(_ROWS,_ROWS.shape[0],_act,ML).detach().cpu()}
        print('  arm plain done',flush=True)
        cur['abi']=0
        def _sp(li):
            def ph(mod,args):
                _rep=_AI[li][cur['abi']].to(args[0].dtype)
                if li==3: cur['abi']+=1
                return (_rep,)+tuple(args[1:])
            return ph
        hs2=[m.transformer.h[2].attn.register_forward_pre_hook(_sp(2)),
             m.transformer.h[3].attn.register_forward_pre_hook(_sp(3))]
        cur['abi']=0
        _arms['spliced']=evalV(_ROWS,_ROWS.shape[0],_act,ML).detach().cpu()
        for h in hs2: h.remove()
        del cur['clsmap']; del cur['abi']
        if torch.equal(_arms['spliced'],_arms['plain']):
            raise SystemExit('INSTRUMENT FAIL: spliced cev bitwise equal to plain (inert splice)')
        SEL['front_result']=_arms
...
if __name__=='__main__':
    ...
    SEL['attn_splice']=True; SEL['ext_rows']=CROWS
    print('ARMS: cp_front plain vs a2/a3-input splice - census rows',flush=True)
    main()
    A=SEL['front_result']
    out={}
    for k,cev in A.items():
        ...
        print(f"  {k:8s} agg {agg:+.4f}  valid {nv}/{len(CINFO)}",flush=True)
    pa=out['spliced']['agg']<=0.6*out['plain']['agg']
    pb=out['spliced']['valid']>=3
    pc=abs(out['plain']['agg']-ANCH)<=0.015
    res={'arms':out,'anchor_s2206':ANCH,
         'convention':'per-position dCE = CE(config) - CE(real model) on census rows; lower is better',
         'pred_a_attention_mediates':bool(pa),'pred_b_validity_appears':bool(pb),'pred_c_repro':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) spliced {out['spliced']['agg']:+.4f} <= 0.6 x plain ({0.6*out['plain']['agg']:+.4f}): {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid(spliced) {out['spliced']['valid']} >= 3: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |plain - anchor| <= 0.015: {'HELD' if pc else 'FAILED'}")
