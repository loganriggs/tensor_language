# SOURCE-CLOSED STREAM INPUT -- does the program own its own map input?
#
# §1876 cut the fallback loss from the deployed +0.78075 / +0.86225 / +0.83997 to
# +0.17427 / +0.21358 / +0.21419 by feeding the covered-fit map the length-1 RESIDUAL STREAM entering
# each site instead of the token embedding, at matched rank 512 and matched (covered-only) fitting set.
#
# Codex's 03:55Z source audit is CORRECT and this run exists because of it. §1876 populated that stream
# with a NATIVE length-1 forward pass through the real model. The FIT is genuinely covered-only, so
# §1876 does establish parameter transfer -- but a standalone compiled program does not own those
# site-entry vectors. To have them it must store another token x site table (the cost the program
# exists to avoid), call the original model (defeating the point), or REGENERATE them from its own
# compressed prefix. Only the third is a program. §1876 measured none of them.
#
# This measures the third. Every site is REPLACED by the map applied to the stream that site actually
# receives, so the input to site st is built from the program's own rows at all 36 sites beneath it and
# the native model is never consulted. That is one forward recursion, well-founded because a site's
# input depends only on sites strictly beneath it. Storage is UNCHANGED at 36 x rank x 2 x D -- the map
# is the same object, fed differently -- so if this holds, §1876's headline is source-closed for free.
#
# The risk is compounding: the map was fit on NATIVE covered streams and is here applied to
# PROGRAM-GENERATED uncovered streams, which drift a little at every one of 36 depths.
#
# ROLES. All three, deployed 5,419-type covered set. DISCOVERY ONLY. Rung 3: Codex's registered ask.
#
...
# ANCHORS name the build they came from per LESSONS 53's addendum, and their RANK per §1875.
#   pred_a THE CLOSURE SURVIVES: the source-closed stream map at rank 512 still beats §1870's PUBLISHED
#          rank-512 covered-fit EMBEDDING map (+0.59560 / +0.67209 / +0.67172) on all three roles. This
#          is the survival bar and it is deliberately weak, because the embedding map is the honest
#          fallback a program can already build with no extra input at all. If FALSE, §1876's gain was
#          an artifact of borrowing the native model's activations and the whole line collapses back to
#          §1870 -- which I would then say plainly, having published §1876 as "deployable".
#   pred_b AND IT KEEPS MOST OF THE ADVANTAGE: it retains at least half of §1876's PUBLISHED advantage
#          over that embedding map, i.e. lands below 0.38494 / 0.44283 / 0.44295 on the three roles.
#          Separates "closure costs a little" from "closure costs nearly everything"; the two have very
#          different consequences for the frontier §1853-§1865 mapped.
#   pred_c AND THE COVERED ARM IS UNTOUCHED: covered CE identical to the MAP arm within 1e-6. NINTH
#          KNOWN-ANSWER check; the previous eight each returned exactly 0.00e+00. Uncovered rows are
#          the only thing any arm changes, so a nonzero spread means the closure leaked into covered
#          positions and every number in the run is void.
#   pred_d CONTROLS: coverage is exactly 5,419; the covered arm attains its ceiling (a nineteenth
#          confirmation); the MAP arm reproduces §1870's rank-512 row within 0.005; and the NATIVE
#          STREAMCOV arm reproduces §1876's PUBLISHED +0.17427 / +0.21358 / +0.21419 within 0.005 --
#          the closure is measured in the SAME RUN as the claim it qualifies, so the comparison cannot
#          be a build difference.
...
OUT = PT + 'ops/stream_input_closure_results.json'
...
MAPRANKS = (512,)
ARMS2 = ('MAP', 'STREAMCOV', 'STREAMCLOSED')          # None = full rank; §1786/§1813 use this ladder
...
S1876_HALF = {'skip7000': 0.38494, 'skip11000': 0.44283, 'skip1200': 0.44295}
ORC = {}
COV = {}
...
def main():
    ...
    print(f'SOURCE-CLOSED STREAM INPUT | 5419 types, program-generated site-entry vectors, rank 512 | '
    ...
    def build(n, rank=None, map_rank=MAP_RANK):
        ...
        for st in sites:
            ...
            if BASE['mode'] in ('STREAMCOV', 'STREAMCLOSED'):
                Uo = tables[st].double()
                Xo = covstream[st].double()       # COVERED tokens' streams: the fit set, both arms
                Ao = (Xo.T @ Xo
                ...
                mpos[st] = (Uu[:, :map_rank] * So[:map_rank]) @ Vo[:map_rank]
            out[st] = fr
        if BASE['mode'] == 'STREAMCOV':
            # §1876: the NATIVE site-entry vector, from the real model's length-1 pass. Reproduced here
            # as the anchor pred_d checks, so the closure below is compared inside one build.
            for st in sites:
                out[st][utoks] = (ORC['stream'][st].double() @ mpos[st]).float()
        elif BASE['mode'] == 'STREAMCLOSED':
            # SOURCE-CLOSED. One length-1 pass over the uncovered tokens in which every site is
            # REPLACED by its own map output, so args[0] at site st is the stream the PROGRAM produces,
            # never the model's. Returning a value from a forward hook replaces the module output
            # (the same mechanism row_hook uses), which is what makes the recursion self-consistent
            # rather than a capture of the native stream.
            crow = {}

            def _mkc(st):
                def hook(mod, args, out_):
                    crow[st] = (args[0][:, 0].float().double() @ mpos[st]).float()
                    y = out_[0] if isinstance(out_, tuple) else out_
                    sub = crow[st].reshape(y.shape).to(y.dtype)
                    return (sub,) + tuple(out_[1:]) if isinstance(out_, tuple) else sub
                return hook
            for i5 in range(0, NU, 256):
                tt5 = utoks[i5:i5 + 256].unsqueeze(1)
    ...
    pa = all(fbloss['STREAMCLOSED'][e] < S1870_MAP_R512[e] for e in roles2)
    pb = all(fbloss['STREAMCLOSED'][e] < S1876_HALF[e] for e in roles2)
    pc = covspread <= 1e-6
    ...
    print(f'\n  the CLOSURE survives -- beats §1870 rank-512 embedding -> {pa}  ' + '  '.join(
        f'{e} {fbloss["STREAMCLOSED"][e]:+.5f} vs {S1870_MAP_R512[e]:+.5f}' for e in roles2), flush=True)
    print(f'  and keeps HALF of §1876 native advantage -> {pb}  ' + '  '.join(
        f'{e} {fbloss["STREAMCLOSED"][e]:+.5f} vs bar {S1876_HALF[e]:+.5f}' for e in roles2), flush=True)
    print(f'  and the COVERED arm is untouched (<1e-6) -> {pc}  max spread {covspread:.2e}',
          flush=True)
    print(f'  MAP reproduces §1870 and STREAMCOV reproduces §1876, coverage {NFULL}, '
          f'attainment {attain} -> control {pd}', flush=True)
    print(f'  CLOSURE COST (closed - native §1876): ' + '  '.join(
        f'{e} {fbloss["STREAMCLOSED"][e] - fbloss["STREAMCOV"][e]:+.5f}' for e in roles2), flush=True)

    json.dump({'run': 'stream_input_closure', 'n_covered': NFULL, 'arms': list(ARMS2),
               ...
               'predictions': {'pred_a_closure_beats_embedding': bool(pa),
                               'pred_b_keeps_half_of_native': bool(pb),
