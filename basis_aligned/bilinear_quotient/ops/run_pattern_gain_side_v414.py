#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_instrument pred_b_cross_closure pred_c_key_side_leads pred_d_query_side_small pred_e_interaction_small
"""Which END sets the readers' gain: the noun's key or the answer's query (v414). v413: token identities explain a fifth of p's variance each. p = s1 * s2 is
bilinear in (query of the answer row, key of the noun row). With the 244 sequences' queries q_i and keys k_j in hand, the CROSS matrix P_ij = p(q_i, k_j)
is exact for every pairing, and its variance over (i, j) splits exactly into a query-row main effect, a key-column main effect and an interaction (two-way
decomposition by row / column means, no fit). The diagonal is the observed p. Which side carries the row-level gain?
PREDICTIONS (scored as written; failures preserved; priors unsure)
    ...
    pred_b_cross_closure   the three variance components sum to the total variance of P within relative 1e-6 (exact identity), and P_ii replays p within 1e-6
    pred_c_key_side_leads  for 9.6 the key (noun-side) main effect is the largest of the three components. Prior: unsure
    pred_d_query_side_small  for 9.6 the query (answer-side) main effect is <= 0.30 of the total
    pred_e_interaction_small  for 9.6 the interaction is <= 0.30 of the total (the gain is close to a product of a query gain and a key gain). Prior: unsure
...
OUT = ROOT / "circuits/followups/pattern_gain_side_v414_result.json"
CANDIDATE_ID = "both_ends.pattern_gain_side_v414"
N_HEAD, BATCH = 9, 64
...
INSTR_TOL, CROSS_TOL, SIDE_MAX = 2e-2, 1e-6, 0.30
READERS = ((9, 6), (12, 4), (15, 1))
FORWARDS_MAX = 5
PREDICTIONS = {"pred_a_instrument": "<= 2e-2", "pred_b_cross_closure": "<= 1e-6", "pred_c_key_side_leads": "key largest", "pred_d_query_side_small": "<= 0.30", "pred_e_interaction_small": "<= 0.30"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "rows": len(rows), "dropped_records": dropped, "natural_records": len(recs), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"instr_tol": INSTR_TOL, "cross_tol": CROSS_TOL, "side_max": SIDE_MAX}}
                ...
                if l in layers:
                    ...
                    for (ll, h) in READERS:
                        ...
                        for i, row in enumerate(chunk):
                            n = int(nn_[i]); fac[(ll, h)][start + i] = {"p_noun": float(s1_all[i, n] * s2_all[i, n]), "qf": qf[i].cpu(), "q2f": q2f[i].cpu(), "kn": kh[i, n].cpu(), "k2n": k2h[i, n].cpu()}
                    v1_ = v1n
                else:
    ...
    for r in READERS:
        F_ = fac[r]; idx_ = range(n_text)
        Q = torch.stack([F_[i]["qf"] for i in idx_]); Q2 = torch.stack([F_[i]["q2f"] for i in idx_]); K_ = torch.stack([F_[i]["kn"] for i in idx_]); K2 = torch.stack([F_[i]["k2n"] for i in idx_])
        P = ((Q @ K_.T) / hd) * ((Q2 @ K2.T) / hd)                      # P_ij = p(query of row i, key of row j)
        ...
        cross_err = max(cross_err, abs(q_eff + k_eff + inter - total) / max(total, 1e-9))
        report["readers"][f"{r[0]}.{r[1]}"] = {"shares": {"query_main": q_eff / total, "key_main": k_eff / total, "interaction": inter / total}, "grand_mean": float(m), "diag_mean": float(P.diag().mean()), "n": n_text,
                                                "query_gain_range": [float(rm.min()), float(rm.max())], "key_gain_range": [float(cm.min()), float(cm.max())]}
    report["cross_closure_max"] = cross_err
    print(json.dumps(report, indent=1))
    R = report["readers"]; s96 = R["9.6"]["shares"]
    predictions = {"pred_a_instrument": instr <= INSTR_TOL, "pred_b_cross_closure": cross_err <= CROSS_TOL, "pred_c_key_side_leads": max(s96, key=s96.get) == "key_main", "pred_d_query_side_small": s96["query_main"] <= SIDE_MAX, "pred_e_interaction_small": s96["interaction"] <= SIDE_MAX}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pattern_gain_side_result_v414", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
