#!/usr/bin/env python
"""block_boundary_blend_rotation_probe -- is the early read-frame rotation (§2757) produced by the per-block embedding blend
x <- l0*x + l1*x0 or by the MLP write? Decomposes each mlp_l -> attn_{l+1} step through the pre-blend residual's frame (principal
angles) and tests causally whether attention reading through the pre-blend frame pays the blend's rotation. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_blend_step_dominates pred_c_blend_accounts_for_most pred_d_preblend_frame_costs
#                     pred_e_write_step_is_the_smaller_cost
...
Preregistration: polynomial_causal/BLOCK_BOUNDARY_BLEND_ROTATION_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/block_boundary_blend_rotation_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("block_boundary_blend_rotation_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "BLOCK_BOUNDARY_BLEND_ROTATION_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "frame_principal_angle_spectrum_probe_results.json"   # §2757
OUT = ROOT / "block_boundary_blend_rotation_probe_results.json"
HASHES = {PREREG: "759dc1527b1f255cdaf6858bbc684d15a731756ad2db79489a13fa89514efdca", PRIOR: "bc96dec9306508d8718d984f520a7e54d09612361a91ba70a263582a41f69729",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "block_boundary_blend_rotation_probe"
...
K = 768; EARLY = list(range(0, 11)); LATE = list(range(11, 18)); NB = 8
SITES = [("mlp", l) for l in range(NL)]; ASITES = [("attn", l) for l in range(NL)]; PRE = [("pre", l) for l in range(NL - 1)]
ESITES = [(kd, l) for l in EARLY for kd in ("attn", "mlp")]; LSITES = [("mlp", l) for l in LATE] + [("attn", l) for l in LATE]
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_min": 1.5, "c_min": 0.6, "d_min": 0.020, "e_max": 0.6}
NULLS = {"b_max": 1.0, "c_max": 0.3, "d_max": 0.005, "e_min": 0.9}
...
def forward(m, idx, patch=None, collect=None):
    ...
    for l, blk in enumerate(m.transformer.h):
        ...
        if collect is not None and l < NL - 1:
            collect(("pre", l), x)
    if collect is not None:
        collect(("final", -1), x)
    if collect is not None:
...
def main():
    ...
    pst = {s_: {"G": torch.zeros(D, D, dtype=torch.float64, device=DEV), "s": torch.zeros(D, dtype=torch.float64, device=DEV)} for s_ in PRE}
    class Rec:
    ...
    for i in range(0, fit_rows.shape[0], CH):
        ...
        forward(m, idx, patch={s_: Rec(s_) for s_ in ASITES}, collect=lambda s_, w: box.__setitem__(s_, w) if (s_[0] == "mlpin" and ("mlp", s_[1]) in SITES) or s_[0] == "pre" else None)
        for s_ in PRE:
            xp = F.rms_norm(box[s_], (D,)).reshape(-1, D).double(); pst[s_]["G"] += xp.T @ xp; pst[s_]["s"] += xp.sum(0)
        for s_ in SITES:
    ...
    pheads = {}
    for s_ in PRE:
        mp = pst[s_]["s"] / n; Cp = pst[s_]["G"] / n - torch.outer(mp, mp); _, Up = torch.linalg.eigh(Cp); pheads[s_[1]] = {"U": Up.flip(1).float(), "Cx": Cp}
    del st, pst
    ...
    def n30(A, B):
        s = torch.linalg.svdvals((A[:, :K].T @ B[:, :K]).double()).clamp(-1, 1); return int((torch.rad2deg(torch.acos(s)) > 30).sum())
    steps = {}
    for l in range(NL - 1):
        w_ = n30(heads[("mlp", l)]["U"], pheads[l]["U"]); b_ = n30(pheads[l]["U"], heads[("attn", l + 1)]["U"]); t_ = n30(heads[("mlp", l)]["U"], heads[("attn", l + 1)]["U"])
        steps[l] = {"write": w_, "blend": b_, "total": t_, "lam0": float(m.transformer.h[l + 1].lambdas[0]), "lam1": float(m.transformer.h[l + 1].lambdas[1])}
    ratio_bw = torch.tensor([steps[l]["blend"] / max(steps[l]["write"], 1) for l in range(NB)]); ratio_bt = torch.tensor([steps[l]["blend"] / max(steps[l]["total"], 1) for l in range(NB)])
    b_med = float(ratio_bw.median()); c_med = float(ratio_bt.median())
    arms = {"EARLY22_OWN_768": run({s_: head(s_, K) for s_ in ESITES})}
    pb = {s_: head(s_, K) for s_ in ESITES}; pb.update({("attn", l): head(("attn", l), K, pheads[l - 1]["U"]) for l in range(1, 11)}); arms["EARLY_ATTN_PREBLEND_768"] = run(pb)
    pv = {s_: head(s_, K) for s_ in ESITES}; pv.update({("attn", l): head(("attn", l), K, heads[("mlp", l - 1)]["U"]) for l in range(1, 11)}); arms["EARLY_ATTN_PREV_768"] = run(pv)
    log(stage="arms", b_med=b_med, c_med=c_med, **{k: round(v, 4) for k, v in arms.items()})
    gaps = {"preblend_minus_own": arms["EARLY_ATTN_PREBLEND_768"] - arms["EARLY22_OWN_768"], "prev_minus_own": arms["EARLY_ATTN_PREV_768"] - arms["EARLY22_OWN_768"],
            "blend_over_write_median": b_med, "blend_over_total_median": c_med}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["EARLY22_OWN_768"] - PRIOR_EARLY22_OWN_768) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_blend_step_dominates': bool(b_med >= BARS["b_min"]),
        'pred_c_blend_accounts_for_most': bool(c_med >= BARS["c_min"]),
        'pred_d_preblend_frame_costs': bool(gaps["preblend_minus_own"] >= BARS["d_min"]),
        'pred_e_write_step_is_the_smaller_cost': bool(gaps["preblend_minus_own"] <= BARS["e_max"] * gaps["prev_minus_own"]),
    }
    nulls = {"b_null_le_1": bool(b_med <= NULLS["b_max"]), "c_null_le_.3": bool(c_med <= NULLS["c_max"]),
             "d_null_le_.005": bool(gaps["preblend_minus_own"] <= NULLS["d_max"]), "e_null_ge_.9prev": bool(gaps["preblend_minus_own"] >= NULLS["e_min"] * gaps["prev_minus_own"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "k": K, "n_boundary_blocks": NB,
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "early22_own_768": arms["EARLY22_OWN_768"], "prior_early22_own_768": PRIOR_EARLY22_OWN_768},
           "ce_added": arms, "gaps": gaps, "steps_n_gt_30": steps,
