...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; full vs a16-pointer arms x eight windows (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'a16_pointer_results.json'
    ...
    for li in range(10,18):
        ...
        if li==16 and SEL.get('a16ptr'):
            ...
            for fi in range(len(flatA)):
                ...
                r5,pp=divmod(fi,256)
                tgt=int(TOK16[r5,pp+1])
                qs=(TOK16[r5,:pp+1]==tgt).nonzero()
                if len(qs)==0: continue
                q5=int(qs[-1])
            ...
            print(f'a16 pointer map fit on {len(Xp)} ind positions',flush=True)
