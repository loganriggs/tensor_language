#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_closure pred_b_postnoun_source_carries_a_third pred_c_value_only_term_leads pred_d_third_order_small pred_e_leading_term_replays_across_frames
"""Both-ends contrast + degree expansion at the readers, POST-NOUN source (v402; v401 did the noun source: 0.92 pure copy for 9.6). Same expansion at the token after the noun, where the number state is rebuilt by MLPs 4-8 (v383-v385).
Input end: plural vs singular noun (aligned v76 pairs, same length). Output end: u = W_U[they] - W_U[he]. For a bilinear head the noun-source term of the
head's final-token write on u is a PRODUCT of three factors of the noun state: s1 = (q_f . k_n)/d, s2 = (q2_f . k2_n)/d, w = (u O_h) . v_n. Its
plural-minus-singular contrast expands EXACTLY (no polynomial assumption; the QK norm and rotary sit inside the factors) into
    degree 1: one factor changes (value-only = pure copy; s1-only; s2-only), degree 2: two factors, degree 3: all three,
plus the query-change term (the final token's query differs too; evaluated at the singular keys). Sum of the eight = the noun-source contrast.
Also reported: the noun-source share of the whole head write contrast (v381: readers read the noun and the token after it about equally).
PREDICTIONS (scored as written; failures preserved; priors from v379 / v381)
    pred_a_closure                            eight terms sum to the noun-source contrast within relative 1e-4 (per reader, pooled); recomputed head writes match the module's within 2e-2
    pred_b_postnoun_source_carries_a_third    for 9.6 the post-noun source term carries >= 0.35 of the head's total write contrast on u (panel, pooled; v381: about half)
    pred_c_value_only_term_leads              for 9.6 the value-only (pure copy) term is >= 0.60 of the post-noun source contrast (v401 at the noun: 0.92). Prior: likely
    pred_d_third_order_small                  |degree-3 term| <= 0.25 of the noun-source contrast for each of 9.6 / 12.4 / 15.1
    pred_e_leading_term_replays_across_frames for 9.6 the largest |term| category is the same in all three frames. Prior: unsure.
...
OUT = ROOT / "circuits/followups/both_ends_degree_postnoun_v402_result.json"
CANDIDATE_ID = "both_ends.degree_expansion_postnoun_v402"
N_HEAD, BATCH = 9, 32
CLOSURE_TOL, INSTR_TOL, NOUN_MIN, VALUE_MIN, THIRD_MAX = 1e-4, 2e-2, 0.35, 0.60, 0.25
READERS = ((9, 6), (12, 4), (15, 1))
FORWARDS_MAX = 4
PREDICTIONS = {"pred_a_closure": "<= 1e-4 / 2e-2", "pred_b_postnoun_source_carries_a_third": ">= 0.35", "pred_c_value_only_term_leads": ">= 0.60", "pred_d_third_order_small": "<= 0.25 x 3", "pred_e_leading_term_replays_across_frames": "same x 3"}
...
def main() -> None:
    ...
    noun_of = lambda row: 1 + next(i for i, t in enumerate(row.ids) if t in nouns)   # the token AFTER the noun (v381: the other half of the readers' read)
    plan = {"candidate_id": CANDIDATE_ID, "rows": len(rows), "rows_sha256": L.rows_sha256(rows), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"closure_tol": CLOSURE_TOL, "instr_tol": INSTR_TOL, "noun_min": NOUN_MIN, "value_max": VALUE_MIN, "third_max": THIRD_MAX}}
    ...
    predictions = {"pred_a_closure": closure <= CLOSURE_TOL and instr <= INSTR_TOL, "pred_b_postnoun_source_carries_a_third": r96["noun_source_share"] >= NOUN_MIN, "pred_c_value_only_term_leads": r96["term_shares"]["value_only"] >= VALUE_MIN,
    ...
    OUT.write_text(json.dumps({"schema": "both_ends_degree_result_v402", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
