# DOES FREEZING THE DRAWS BEAT MORE PERMUTATIONS? -- pairing, for comparisons.
#
# §1915 and §1916 priced the permutation-null estimator: unbiased, spread 0.218 / 0.334 / 0.188 at
# NPERM = 8 and 0.061 / 0.123 / 0.096 at NPERM = 8, falling as 1/sqrt(NPERM). Codex noted at 10:22Z that
# tight comparisons should "freeze draws or use substantially more permutations". §1916 flagged that
# freezing is probably the stronger of the two and declined to claim it unmeasured. This measures it.
#
# The argument: a COMPARISON between two arms is a DIFFERENCE of two ratios. If each arm is scored
# against its own independent permutation set, the two null errors add. If both are scored against the
# SAME set, the shared component cancels and only the arms' own disagreement survives. Raising NPERM
# shrinks each LEVEL's error; pairing shrinks the DIFFERENCE's error directly, and costs nothing.
#
# The quantity under test is §1914's token-margin gradient (q3 - q0 enrichment), the number whose
# ranking against the live-margin gradient §1914 reversed and §1915 confirmed.
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY. Rung 3: §1916's open question.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
#   pred_a PAIRING CUTS THE COMPARISON ERROR AT LEAST 2x: at NPERM = 8, the seed-to-seed spread of the
#          token-margin gradient with a SHARED null is under half its spread with INDEPENDENT nulls, on
#          all three roles. If FALSE the two arms' null errors are not sharing enough structure to
#          cancel, freezing buys little, and Codex's two options are not equivalent in the way I implied
#          at §1916 -- which I would correct on the board, since they may act on it.
#   pred_b AND PAIRING DOES NOT BIAS: the paired and unpaired gradient MEANS agree within 0.1 on all
#          three roles. Pairing changes the variance of an estimator, not its expectation; if the means
#          move, the shared draw is correlating with something it should not and the trick is unsafe.
#          This is the control that makes pred_a worth acting on.
#   pred_c AND PAIRING AT NPERM=8 BEATS INDEPENDENCE AT NPERM=64: the paired NPERM = 8 gradient spread is
#          below the unpaired NPERM = 8 spread, on all three roles. This is the actionable claim -- it
#          says freezing draws is worth more than an eightfold increase in compute for any COMPARISON.
#          If FALSE, more permutations really is the better lever and §1916's speculation was wrong.
#   pred_d CONTROLS: coverage is exactly 16,110; both pairing regimes use the same eight seeds and the
#          same covered positions; and the unpaired NPERM = 8 gradient mean reproduces §1914's PUBLISHED
#          +9.50 / +9.79 / +9.28 within 0.3.
...
OUT = PT + 'ops/paired_vs_more_perms_results.json'
...
def compare_by_bucket(rows, hooks, seenmask=None):
    ...
    # the numerator is deterministic; only the NULL is estimated. Re-estimate it under 8 independent
    # seeds at two NPERM settings to price §1914's inferred +-0.2 directly.
    obs_cov = float((GL == GP).double().mean())
    out['covered_agreement'] = obs_cov
    for npm in (8, 64):
        ests = []
        for sd in range(8):
            g2 = torch.Generator(device='cpu').manual_seed(1000 + sd)
            nl = sum(float((GL == GP[torch.randperm(GP.numel(), generator=g2).to(GP.device)])
                           .double().mean()) for _ in range(npm)) / npm
            ests.append(obs_cov / max(nl, 1e-12))
        out[f'baseline_seeds_{npm}'] = ests

    # THE PAIRING TEST. The token-margin gradient is enrichment(q3) - enrichment(q0). Estimate it eight
    # times three ways: independent nulls per arm at NPERM=8, a SHARED null for both arms at NPERM=8,
    # and independent nulls at NPERM=64. Pairing should cancel the shared null error in the DIFFERENCE.
    A0, A3 = torch.cat(tokq[0]['l']), torch.cat(tokq[3]['l'])
    B0, B3 = torch.cat(tokq[0]['p']), torch.cat(tokq[3]['p'])
    o0 = float((A0 == B0).double().mean())
    o3 = float((A3 == B3).double().mean())

    def _null(L, P, seed, npm):
        g4 = torch.Generator(device='cpu').manual_seed(seed)
        return sum(float((L == P[torch.randperm(P.numel(), generator=g4).to(P.device)])
                         .double().mean()) for _ in range(npm)) / npm
    for tag, npm, shared in (('indep8', 8, False), ('paired8', 8, True), ('indep64', 64, False)):
        gs = []
        for sd in range(8):
            if shared:
                # ONE null, from the whole covered arm, used for BOTH quartiles
                nl = _null(GL, GP, 3000 + sd, npm)
                gs.append(o3 / max(nl, 1e-12) - o0 / max(nl, 1e-12))
            else:
                gs.append(o3 / max(_null(A3, B3, 4000 + sd, npm), 1e-12)
                          - o0 / max(_null(A0, B0, 5000 + sd, npm), 1e-12))
        out[f'grad_{tag}'] = {'seeds': gs, 'mean': sum(gs) / 8.0, 'spread': max(gs) - min(gs)}
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'PAIRED vs MORE PERMUTATIONS: which lever tightens a comparison? | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    for si, (lbl, g) in enumerate(arms):
        ...
        for ename in evs:
            ...
            row[ename] = {'enrichment': c['mech_cov']['enrichment'],
                          ...
                          'covered_agreement': c['covered_agreement'],
                          'baseline_seeds_8': c['baseline_seeds_8'],
                          'baseline_seeds_64': c['baseline_seeds_64'],
                          'grad_indep8': c['grad_indep8'], 'grad_paired8': c['grad_paired8'],
                          'grad_indep64': c['grad_indep64'],
    ...
    def g_(e, tag, f):
        return top(e, f'grad_{tag}')[f]
    pa = all(g_(e, 'paired8', 'spread') < 0.5 * g_(e, 'indep8', 'spread') for e in roles)
    pb = all(abs(g_(e, 'paired8', 'mean') - g_(e, 'indep8', 'mean')) <= 0.1 for e in roles)
    pc = all(g_(e, 'paired8', 'spread') < g_(e, 'indep64', 'spread') for e in roles)
    pd = (ncov == NCOV
          and all(abs(g_(e, 'indep8', 'mean') - S1914_TOK[e]) <= 0.3 for e in roles))

    print(f'\n  token-margin gradient (q3 - q0), 8 seeds, three regimes:', flush=True)
    for e in roles:
        print(f'    {e:10s} ' + '  '.join(
            f'{tag} {g_(e, tag, "mean"):+.3f} +-{g_(e, tag, "spread"):.3f}'
            for tag in ('indep8', 'paired8', 'indep64')), flush=True)
    print(f'\n  PAIRING cuts the comparison error >=2x -> {pa}  ' + '  '.join(
        f'{e} {g_(e, "paired8", "spread"):.3f} vs {g_(e, "indep8", "spread"):.3f}' for e in roles),
        flush=True)
    print(f'  and pairing does NOT bias (means within 0.1) -> {pb}  ' + '  '.join(
        f'{e} {abs(g_(e, "paired8", "mean") - g_(e, "indep8", "mean")):.3f}' for e in roles), flush=True)
    print(f'  and PAIRED-8 beats INDEP-64 -> {pc}  ' + '  '.join(
        f'{e} {g_(e, "paired8", "spread"):.3f} vs {g_(e, "indep64", "spread"):.3f}' for e in roles),
        flush=True)
    print(f'  coverage {ncov}, indep8 mean reproduces §1914 -> control {pd}  ' + '  '.join(
        f'{e} {g_(e, "indep8", "mean"):+.2f} vs {S1914_TOK[e]:+.2f}' for e in roles), flush=True)
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_pairing_cuts_error': bool(pa),
                          'pred_b_pairing_unbiased': bool(pb),
                          'pred_c_paired8_beats_indep64': bool(pc),
