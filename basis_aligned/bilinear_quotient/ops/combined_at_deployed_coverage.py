# THE COMBINED BUILD AT THE DEPLOYED COVERAGE -- the check §1931 named as wanted before shipping.
#
# §1931 certified a best-known build at 16,110 types: 18 attention sites at table rank 256, 18 MLP sites
# at 768, a rank-512 covered-fit fallback map, 360.724M reals. Against §1789's deployed design scaled up
# (673.464M) it is 46% cheaper and better on all-position CE, overall top-1 and the 125+ kept-fraction on
# all three roles, with one residual: 1.10pp of kept-fraction on unseen targets on skip1200.
#
# It has never been run through §1789's accuracy-by-frequency instrument at the DEPLOYED 5,419 coverage,
# which is the build anyone would actually ship. §1924 is a live example of a lever measured at 16,110
# that did not transfer to 5,419 at all, so this is not a formality. The costs differ too: at 5,419 the
# same design is 163.667M against the deployed 230.087M -- 29% cheaper rather than 46%, because the table
# cost scales with coverage while the map does not.
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY, 5,419 coverage. Rung 3: §1931's named question.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
# §1931's figures are at 16,110 and are what is being generalised, NOT controls -- anchoring a 5,419-type
# run to a 16,110-type constant is the trap that failed §1905's pred_d and cost §1930 a launch.
#   pred_a THE COMMON END STILL HOLDS: the combined build's kept-fraction in the 125+ bucket is within 2
#          percentage points of the deployed design's, on all three roles -- the same bar §1883 and §1931
#          both used. If FALSE the design does not transfer to the coverage anyone would ship at, and
#          §1931's "best-known build" needs "at 16,110" attached to it everywhere.
#   pred_b AND THE RARE END IS NO WORSE THAN §1931's: no more than 1.5pp below the deployed design. Same
#          bar as §1931, which I widened there from §1883's 1.0pp because §1883 had already failed 1.0 at
#          1.10pp -- carried unchanged so the two runs are comparable, and flagged again as a bar set
#          knowing where the answer sat.
#   pred_c AND THE OVERALL TOP-1 GAP IS SMALL: under 1 percentage point on all three roles, as at §1931
#          (0.08 / 0.04 / 0.01pp). At 5,419 the deployed design has full-rank tables over a smaller
#          covered set, so the comparison is less favourable to the combined build than at 16,110 and
#          this is the predicate most likely to move.
#   pred_d CONTROLS: coverage is exactly 5,419; the five buckets partition every scored position in both
#          arms; the LIVE model's per-bucket accuracy is identical between arms; and the costs are
#          REPORTED (230.087M deployed, 163.667M combined, 29% cheaper) rather than assumed.
...
COST_M = {'deployed': 230.087, 'combined': 163.667}   # §1858 / computed at 5,419
RIDGE = 1e-2
PT = '/workspace/tensor_language/basis_aligned/bilinear_quotient/'
OUT = PT + 'ops/combined_at_deployed_coverage_results.json'
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'COMBINED BUILD AT DEPLOYED COVERAGE 5419 | buckets {BUCKETS} on the fit-row count of the TRUE '
