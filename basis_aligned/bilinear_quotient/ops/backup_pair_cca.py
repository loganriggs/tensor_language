...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['other_position_law_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; backup-pair CCA/transfer captures on skip-1416 (alignment rung)")
    raise SystemExit(0)
import torch
...
OUT=PT+'backup_pair_cca_results.json'
...
def main():
    ...
    if SEL.get('clsdmg'):
        ...
        if SEL.get('cap1416'):
            SEL['y14']=[]; SEL['y16']=[]
            _hc.append(m.transformer.h[14].attn.register_forward_hook(
                lambda mo,i_,o_: SEL['y14'].append(o_[0].detach().reshape(-1,D).float().cpu())))
            _hc.append(m.transformer.h[16].attn.register_forward_hook(
...
if __name__=='__main__':
    ...
    SEL['skipset']=(14,16); SEL['clsdmg']=True; SEL['cap1416']=True; SEL['motif_off']=()
    print('SINGLE ARM: skip-1416, capturing attn14/attn16 outputs over FR',flush=True)
    r_b=main()
    clsflat=SEL['clsflat']
    Y14=torch.cat(SEL['y14']); Y16=torch.cat(SEL['y16'])
    if Y14.shape[0]!=len(clsflat) or Y16.shape[0]!=len(clsflat):
        raise SystemExit(f'INSTRUMENT FAIL: capture length {Y14.shape[0]}/{Y16.shape[0]} != {len(clsflat)}')
    import statistics as stt
    IND=CLS.index('ind')
    im=(clsflat==IND)
    gen=torch.Generator().manual_seed(77)
    non=(~im).nonzero().squeeze(1)
    non=non[torch.randperm(len(non),generator=gen)[:int(im.sum())]]
    ...
    cc_ind=cca8(Y14[im],Y16[im])
    cc_non=cca8(Y14[non],Y16[non])
    Ai=im&halfA; Bi=im&(~halfA)
    XA,YA=Y14[Ai],Y16[Ai]; XB,YB=Y14[Bi],Y16[Bi]
    lam=1e-2*len(XA)
    ...
    pc=(cc_ind-cc_non)>=0.2
    res={'base':{'L2_F':r_b['L2_F']},'cca8_ind':round(cc_ind,4),'cca8_nonind':round(cc_non,4),
         'procrustes_r2_halfA':round(r2A,4),'procrustes_r2_halfB':round(r2B,4),
         'n_ind':int(im.sum()),
         'pred_a_aligned':bool(pa),'pred_b_map_transfers':bool(pb),'pred_c_retrieval_specific':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'CCA top-8 mean: ind {cc_ind:.3f} | non-ind {cc_non:.3f}; linear map R^2: half-A {r2A:.3f} -> half-B {r2B:.3f}')
    print(f"(a) ind CCA {cc_ind:.3f} >= 0.6: {'HELD' if pa else 'FAILED'}")
    print(f"(b) transfer {r2B:.3f} >= 0.8 x {r2A:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) ind - non-ind = {cc_ind-cc_non:.3f} >= 0.2: {'HELD' if pc else 'FAILED'}")
