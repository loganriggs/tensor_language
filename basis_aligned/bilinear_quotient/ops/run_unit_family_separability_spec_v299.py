...
"""v299: the ADJECTIVE half of v293, under the per-family scope rule — one new member against the full adjective family.

v293 went 6 of 7 four-row and five of its passes are verb_preposition (running now as v297). Its sixth,
adjective_preposition_in_against (deficient/biased -> in/against), belongs to the other family and gets this rung,
because a two-family sweep no longer fits: v289 prepared 72 members and lost SIX to CUDA OOM, recording each as
`separable: false` with an error field; v295 re-ran them at verb-family scope and recovered five, all separable, with
only the 19-unit in_beyond still crashing. Per-family scope was adopted at 02:45 as a scope rule that changes no bar
and no verdict, and this is the first rung authored under it from the start rather than as a rescue.
The adjective family is 24 members with 17 counted (mkfamily computes the counted list from the v289 receipt, promoting members that were NEW there and came out separable). All 17 are re-checked here under the v222 rule because the family
grows by one; that is the full sweep the rules require and board proposal (ix) — sample the re-checks instead — is
still unanswered, so the full sweep stands.
CUE FRESHNESS: in_against comes from v293, whose cue words were script-verified against every kinds=() tuple in ops/
before commit (three collisions were caught in that draft and replaced), so a fusion here would have no cue-overlap
explanation. Every fusion the corpus has produced since v254 has had a shared cue; v297 and this rung are the two
places that run could break.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
    pred_a_new_separable   adjective_preposition_in_against is separable (k_new = 1 of 1). The cue account predicts
                           yes; the adjective family has produced exactly one fusion (for_of / for_beneath at v260),
                           and that pair shared the cue `responsible`.                                       prior 75%
    pred_b_counted_kept    at least 14 of the 17 re-checked counted members remain separable (k_kept). At v289 the
                           adjective side was 12 of 14, the two losses being in_about and for_of, both to their own
                           cue-sharing partners; those two are carried as `keep` because each pair counts once with
                           the incumbent holding the count.                                                  prior 65%
    pred_c_row4_kept       fam-arm own-C UB975 <= 0.01 on at least 9 of the 18 evaluated members (k_d).      prior 55%
    pred_d_instrument      every evaluated member's own-arm extraction is within 0.03 of its parent battery's cdas
                           extraction_held (a determinism check, NOT an independent run outcome). It fails
                           automatically if any member errors, which is how the v289 and v295 OOMs surfaced. prior 85%
COUNTING. A separable in_against is +1 on the count ops/circuit_count.py reports. A counted member that loses
separability to something OTHER than a cue partner is a genuine retraction (-1) and gets reported to the board rather
than absorbed.
EVAL = 1 new + 17 counted re-checks against the 24-member adjective family: ~1.8 h at the measured 428 s/member,
though that rate was measured on the 44-member verb family and this family is half that, so it should run faster.
Smoke: V299_SMOKE=<out.json> (CPU, V299_SMOKE_ROWS=4; V299_SMOKE_NAMES=adjective_preposition_in_against,adjective_preposition_fond).
BARS = {"cross_max": 0.05, "keep": 0.8, "ext_floor": 0.5, "c_ub_max": 0.01, "k_d": 9, "k_new": 1, "k_kept": 14, "instr_tol": 0.03}
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v299_result.json"
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v299", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V299_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V299_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    evaluate = [n for n in members if n in EVAL and (not smoke or n in os.environ.get("V299_SMOKE_NAMES", "verb_preposition_in_onto,verb_preposition").split(","))]
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v299", "candidate_id": "corpus.unit_family_separability_spec_v299",
