"""SKETCH-GATE RANK CURVE (rung 217): rank-256 gate - is selection fidelity sketch-rank-limited?

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2311: the rank-64 gate landed at
+1.0858 - between FIXED (+2.2048) and ORACLE (+0.1015), certificates dead. This runs the identical gate at
sketch rank 256 (selection still 4x cheaper than full products).

REGISTERED PREDICTIONS (arms: SKETCH256 = this run; SKETCH64 +1.0858; ORACLE +0.1015; FIXED +2.2048):
  (a) RANK-LIMITED: census <= 0.35 (most of the remaining gap closes with rank).
  (b) valid >= 1.
  (c) SANITY: census in [0.02, 2.5]; inert tripwire vs ORACLE and SKETCH64 receipts.
NULL: gate fidelity saturates (census >= 0.8) - cheap selection cannot reach the oracle regime; routing
needs the true activations or a learned gate. PRICE: probe; selection flops 4x down. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: sketch-gated top-k rank 256')
    raise SystemExit(0)
import torch
...
OUT=PT+'cp_sketch256_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['cp_topk']=1152; SEL['cp_sketch']=256; SEL['qk_r']=64; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    pb=nv>=1
    pc=0.02<=agg<=2.5
    if abs(agg-0.1015)<1e-4 or abs(agg-1.0858)<1e-4:
    ...
    print(f"(a) census {agg:+.4f} <= 0.35: {'HELD' if pa else 'FAILED'}")
