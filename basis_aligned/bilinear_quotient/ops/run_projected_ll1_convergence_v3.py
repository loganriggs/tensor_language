...
PREFIX='PROJECTED_LL1_CONVERGENCE_V3'
...
def main():
    ...
    assert all(control['pred_'+letter] for letter in ('a','b','c'))
    ...
    predictions={'pred_a': all(row['pred_a'] for row in rows), 'pred_b': all(row['pred_b'] for row in rows),
                 'pred_c': function_cos>=.95 and int((matched>=.8).sum())>=16}
    result=dict(**predictions,rows=rows,function_cosine=function_cos,
