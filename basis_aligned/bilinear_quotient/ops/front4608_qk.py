"""EXACT FRONT ON THE UNIFORM GRAMMAR (rung 191): re-pricing the front swap at the new frontier.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. At the old config the exact front
(K=4608) bought 0.23 census (S2231-era); interactions have since changed wholesale. Arm: rung-188 config
with cp_swap=4608.

REGISTERED PREDICTIONS (anchors: rung-188 census +1.2673; digit +0.942):
  (a) THE FRONT STILL PAYS: census <= 1.10.
  (b) digit <= 0.80.
  (c) SANITY: census >= 0.5; inert tripwire vs 1.2673.
NULL: the front's marginal shrank with the better downstream (recovery < 0.08). PRICE: +15.9M front values.
Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: exact front on uniform grammar')
    raise SystemExit(0)
import torch
...
OUT=PT+'front4608_qk_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=16; SEL['qk_tail']=True
    ...
    pa=agg<=1.10
    pb=PC.get('digit',9.9)<=0.80
    pc=agg>=0.5
    if abs(agg-1.2673)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: front swap inert')
    ...
    print(f"(a) census {agg:+.4f} <= 1.10: {'HELD' if pa else 'FAILED'}")
    print(f"(b) digit {PC.get('digit',9.9):+.4f} <= 0.80: {'HELD' if pb else 'FAILED'}")
