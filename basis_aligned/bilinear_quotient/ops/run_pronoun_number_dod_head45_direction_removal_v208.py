...
"""Pronoun number they/he DoD (v208): does the NUMBER ride a separable direction of head 4.5's copy? v206 / v207b: zeroing 4.5's whole slice at the verb
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_head45_direction_removal_v208_result.json"
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_head45_direction_removal_v208"
BLOCK, HEAD, U829, V206_MARGIN, DROP_MIN, GATE_RATIO, HALF, N_NULL, SEED, BATCH = 4, 5, 829, 196.62, 0.02, 0.25, 0.50, 16, 208, 32
FORWARDS_MAX = 63
...
def main() -> None:
    ...
    for start in range(0, len(rows), BATCH):
        ...
        o, rm = run(chunk, "project", lambda i: dirs[i]); per_row["number"].extend(o); removed_norms["number"].extend(rm)
        for k, rd in enumerate(rand_dirs):
            # norm-matched: a random direction removes a component of the same norm as the number removal for that row -> scale the slice's projection
            def direction_of(i, rd=rd): return rd
            o, rm = run(chunk, "project", direction_of); per_row[f"rand{k}"].extend(o); removed_norms[f"rand{k}"].extend(rm)
    ...
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_head45_direction_removal_result_v208", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
