"""possessive_number, distance SIX, prediction following a VERB rather than a PARTICLE.

Four matched controls have narrowed the distance-6 anomaly to a single untested difference.

    animacy              established as a disruptor (animate oblique fails at distance 4)
    token distance       ruled out (obliques pass at distance 7)
    argument prominence  ruled out (an inanimate direct object passes at distance 5)
    particle-final site   <- the only remaining difference

Every passing design ends on a verb; the one failing inanimate design ends on a particle,
"finished the work and put away" -> " their". This holds distance at SIX and keeps the
VP-plus-object structure, changing only the final element:

    fails    "The clerks packed the crate and put away"        -> " their"   particle-final
    here     "The clerks packed the large crate and signed"    -> " their"   VERB-final, same distance

Prediction registered before the run: a pass closes the investigation with one named factor; a
failure leaves an argument-type by distance interaction that none of these five controls can
separate, and I stop rather than run a sixth.
"""
...
TASK_ID = "possessive_number.verb_final_distance_six"
...
def _conjunct(noun: str, plural: bool, action: str) -> str:
    """Distance SIX, VP-plus-object held, prediction following a verb rather than a particle."""
    return f"The {noun}{'s' if plural else ''} packed the large crate and {action}"


def _notes(noun: str, plural: bool, action: str) -> str:
    return f"In the notes the {noun}{'s' if plural else ''} packed the large crate and {action}"
