"""DOES THE RANK-32 PROJECTION COMPOSE? THE ADOPTION CANDIDATE (Claude, LANE 1)

SS2921 established, with its controls passing (`svd_identity` +0.0001, both uniform routes agreeing to 0.0000), that the tail
correction is a **rank-32 projection**: shrinking 32 of 1152 singular directions reads **-0.2828** against the adopted uniform scalar's
-0.2287, interior in both axes, and **-0.2872 held out**. That was measured on the TAIL TERM ALONE.

SS2912's adopted frontier is a composition -- tail x0.30 uniform, CP x0.80, motif x1.25, +2.2999 in selection / +2.3171 held out -- and
SS2904 showed these terms INTERACT (tail x CP by +0.0149). A standalone gain of 0.0541 does not entitle anything to displace an adopted
composed result. This rung swaps ONLY the tail term, holding CP and motif at SS2912's adopted values, and measures the whole
configuration on both windows.

**This is the adoption candidate, and it is built so that it can fail.** Four anchors must land before the comparison means anything:
the reproduction gate, the SVD identity control, SS2912's configuration reproducing -0.3736, and SS2921's rank-32 projection
reproducing -0.2828 standalone. Then `pred_e` asks whether the swap helps IN COMPOSITION and `pred_f` asks whether that survives on a
window that chose nothing -- the test SS2914/SS2916 established as mandatory before any number is quoted.

The receipt also reports the **composition shortfall**: how much of the standalone 0.0541 survives being composed. SS2904 measured
0.2127 nats of non-additivity across three terms, so a shortfall is expected; its SIZE is the interesting number.
...
SS2125 STANDS -- this rescales singular values of already-fitted maps; it neither selects nor reorders.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_the_svd_path_at_scale_one_is_a_physical_no_op
#                     pred_c_the_adopted_configuration_reproduces_S2912
#                     pred_d_the_rank_projection_alone_reproduces_S2921
#                     pred_e_the_rank_projection_improves_the_composed_configuration
#                     pred_f_the_improvement_survives_on_the_held_out_window
#                     pred_g_the_optimum_is_interior_in_rank_and_scale

Preregistration: polynomial_causal/FRONTIER_RANK_COMPOSITION_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run: the rank projection swapped into SS2912's adopted configuration, on both windows")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_rank_composition_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_RANK_COMPOSITION_PREREGISTRATION.md'
PREREG_SHA = "d8b0f08be6e183b77f7946bc9edc92fa26b077da5d51605999e5eb0c119f058b"
...
BARS['improves'] = 0.01
R_GRID = [16, 32, 64]                       # SS2921's optimum was rank 32; it must be able to be interior
SC_GRID = [0.10, 0.25, 0.40]
S2912_G = -0.3736                           # SS2912's ADOPTED configuration (tail .30, CP .80, motif 1.25)
S2921_RANK32 = -0.2828                      # SS2921's rank-32 projection, standalone
...
if __name__=='__main__':
    ...
    # SS2912's adopted configuration, with its uniform tail term replaced by the SS2921 rank projection.
    # CP and motif are held at SS2912's adopted values throughout, so the ONLY thing varying is the tail term.
    G = {'cp_scale': 0.80, 'motif_scale': 1.25}
    ARMS[:] = ([('baseline', {}),
                ('svd_identity', {'tail_spec': (0, 'all', 1.0)}),
                ('G_uniform',    dict(G, tail_scale=0.30)),
                ('rank32_alone', {'tail_spec': (32, 'top', 0.25)})] +
               [(f'G_r{r}_s{sc}', dict(G, tail_spec=(r, 'top', sc)))
                for r in R_GRID for sc in SC_GRID])
    ...
    ident = cost['svd_identity']
    g_uni = cost['G_uniform']
    r_alone = cost['rank32_alone']
    cell = {(r,sc): cost[f'G_r{r}_s{sc}'] for r in R_GRID for sc in SC_GRID}
    best = min(cell, key=lambda k: cell[k])
    best_cost = cell[best]
    best_name = f'G_r{best[0]}_s{best[1]}'
    interior = (best[0] not in (R_GRID[0], R_GRID[-1])) and (best[1] not in (SC_GRID[0], SC_GRID[-1]))
    # the interaction: does swapping the tail term buy the same as it did standalone?
    standalone_gain = round(S2896_COST - r_alone, 4)      # rank32 vs the uniform tail scalar, alone
    composed_gain   = round(g_uni - best_cost, 4)         # the same swap, inside SS2912's configuration

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = abs(ident) <= BARS['identity']
    pc = abs(g_uni - S2912_G) <= BARS['anchor']
    pd = abs(r_alone - S2921_RANK32) <= BARS['anchor']
    pe = best_cost <= g_uni - BARS['improves']
    pf = cost_h[best_name] < cost_h['G_uniform']
    pg = bool(interior)
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_svd_path_at_scale_one_is_a_physical_no_op':bool(pb),
           'pred_c_the_adopted_configuration_reproduces_S2912':bool(pc),
           'pred_d_the_rank_projection_alone_reproduces_S2921':bool(pd),
           'pred_e_the_rank_projection_improves_the_composed_configuration':bool(pe),
           'pred_f_the_improvement_survives_on_the_held_out_window':bool(pf),
           'pred_g_the_optimum_is_interior_in_rank_and_scale':bool(pg)}
    nulls={'b_null_the_svd_path_is_not_faithful':bool(abs(ident) > 0.02),
           'c_null_the_adopted_configuration_fails':bool(abs(g_uni - S2912_G) >= 0.03),
           'd_null_S2921_does_not_reproduce':bool(abs(r_alone - S2921_RANK32) >= 0.03),
           'e_null_the_projection_adds_nothing_in_composition':bool(not pe),
           'f_null_the_improvement_is_selection':bool(not pf),
           'g_null_the_grid_is_too_narrow':bool(not interior)}
    res={'rung':'frontier_rank_composition','preds':preds,'nulls':nulls,'bars':BARS,
         'arms':out,'published_L2_F':2.6735,'grids':{'rank':R_GRID,'scale':SC_GRID},
         'summary':{'L2_F_baseline':round(bF,4),'L2_F2_baseline':round(b2,4),
                    'cost_fresh':cost,'cost_holdout':cost_h,'cost_fit':cost_c,
                    'svd_identity_cost':ident,
                    'G_uniform_cost':g_uni,'S2912_G':S2912_G,
                    'rank32_alone_cost':r_alone,'S2921_rank32':S2921_RANK32,
                    'best_cell':{'rank':best[0],'scale':best[1]},'best_cost':best_cost,
                    'best_holdout_cost':cost_h[best_name],
                    'G_uniform_holdout_cost':cost_h['G_uniform'],
                    'improves_composed_by':composed_gain,
                    'improves_standalone_by':standalone_gain,
                    'composition_shortfall':round(standalone_gain - composed_gain, 4),
                    'holdout_improvement':round(cost_h['G_uniform'] - cost_h[best_name], 4),
                    'optimum_interior':bool(interior),
                    'implied_L2_F_best':round(bF+best_cost,4),
                    'implied_L2_F2_best':round(b2+cost_h[best_name],4),
                    'S2912_L2_F':2.2999,'S2912_L2_F2':2.3171},
    ...
    print(f"(b) SVD path at scale 1.0 {ident:+.4f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) G uniform {g_uni:+.4f} vs SS2912 {S2912_G:+.4f}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) rank32 alone {r_alone:+.4f} vs SS2921 {S2921_RANK32:+.4f}: {'HELD' if pd else 'FAILED'}")
    for r in R_GRID:
        print(f"    r={r}: " + " | ".join(f"s{sc} {cell[(r,sc)]:+.4f}" for sc in SC_GRID))
    print(f"(e) composed improvement: best {best_name} {best_cost:+.4f} vs G_uniform {g_uni:+.4f} = {composed_gain:+.4f}: {'HELD' if pe else 'FAILED'}")
    print(f"(f) held out: {cost_h[best_name]:+.4f} vs {cost_h['G_uniform']:+.4f}: {'HELD' if pf else 'FAILED'}")
    print(f"(g) optimum interior: {'HELD' if pg else 'FAILED'}")
    print(f"    standalone swap bought {standalone_gain:+.4f}, composed {composed_gain:+.4f}, shortfall {standalone_gain-composed_gain:+.4f}")
    print(f"    implied L2_F {bF+best_cost:.4f} (SS2912: 2.2999) | held out {b2+cost_h[best_name]:.4f} (SS2912: 2.3171)")
