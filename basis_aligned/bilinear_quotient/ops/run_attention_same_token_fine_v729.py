...
"""Attention lane, v729: head 1.4 same-token indicator at a FINE beta grid (v728: |beta| >= 0.25 all hurt; beta = 0 was best on that grid).

v721 (exact single-token tables) found 1.4's content interaction dominated by identical-token pairs ('.', '.'), (' the', ' the'), (' of', ' of').
Test (same as v728, betas in [-0.1, 0.1]): replace 1.4's off-diagonal pattern by kappa(d) + beta * [tok_i == tok_j] (its v702 closed-form kernel plus a same-token indicator),
...
PRICE (registered maximum): native 6; 8 betas x 6 = 48; rank-16 program: kappa_r 2 + 6; total 62 forwards; 0 backwards; 0 fits. Bar <= 80.
"""
...
OUT = ROOT / "circuits/followups/attention_same_token_fine_v729_result.json"
...
NATIVE_V615 = 3.13241
CANDIDATE_ID = "attention.same_token_fine_v729"
FORWARDS_MAX = 80
EBATCH = 32
L_, H_ = 1, 4; H = 9
BETAS = (-0.1, -0.05, -0.02, 0.0, 0.01, 0.02, 0.05, 0.1)
