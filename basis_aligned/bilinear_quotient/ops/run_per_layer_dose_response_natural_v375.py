#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_native_margin_positive pred_b_k10_replays_v374 pred_c_every_k_beats_null_on_text pred_d_saturates_on_text pred_e_leaders_carry_a_third_of_k10
"""Saturation of the per-layer populations on natural text (v375). v373 (panel): top-1 / 5 / 10 / 20 per layer cost 13 / 31 / 34 / 32% of the margin. v374 (text):
the top ten cost 18.8%. Here k = 1 / 5 / 10 / 20 per layer on the 128 natural rows (census on the panel rows in-script), 3 random k-sets per k as null.
PREDICTIONS (scored as written; failures preserved; priors from v373 / v374)
    pred_a_native_margin_positive       the native oriented margin on the natural rows is positive
    pred_b_k10_replays_v374             |change(10)| replays v374's 0.450 within 0.02
    pred_c_every_k_beats_null_on_text   |change(k)| exceeds 3x the largest |change| among the random k-sets, every k
    pred_d_saturates_on_text            |change(20)| <= 1.15 x |change(10)| (no further gain past ten per layer, as on the panel)
    pred_e_leaders_carry_a_third_of_k10 |change(1)| >= 0.33 x |change(10)| (the five leaders carry a third of what the top ten carry). Prior: unsure.
PRICE (registered maximum): 3 census batches + 2 natural batches x (1 native + 4 edits + 12 null) = 37 forwards; 0 backwards; 0 fits. Bar <= 40.
"""
...
OUT = ROOT / "circuits/followups/per_layer_dose_response_natural_v375_result.json"
CANDIDATE_ID = "pronoun_number.per_layer_dose_response_natural_v375"
TARGETS = {3: (5, 1036), 5: (8, 829), 6: (8, 829), 7: (8, 829)}; N_NULL, SEED, BATCH, KS = 3, 375, 32, (1, 5, 10, 20)
V374, NULL_FACTOR, SAT, LEAD = 0.4503, 3.0, 1.15, 0.33
...
FORWARDS_MAX = 40
PREDICTIONS = {"pred_a_native_margin_positive": "> 0", "pred_b_k10_replays_v374": "0.450 +- 0.02", "pred_c_every_k_beats_null_on_text": "> 3x null x 4", "pred_d_saturates_on_text": "<= 1.15x", "pred_e_leaders_carry_a_third_of_k10": ">= 0.33x"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "rows": len(rows), "rows_sha256": L.rows_sha256(rows), "targets": {str(k): list(v) for k, v in TARGETS.items()}, "ks": list(KS), "n_null": N_NULL, "seed": SEED, "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"v374": V374, "null_factor": NULL_FACTOR, "sat": SAT, "lead": LEAD}, "natural": [p_.name for p_ in NATURAL]}
    ...
    def run_margin(edits):
        out = []
        for start in range(0, len(recs), 64): out += margins_tokens(backend, nat[start:start + 64], finals[start:start + 64], edits, readers)
        return sum(m["they_he"] * sg for m, sg in zip(out, sign)) / len(recs)
    base = run_margin(None); forwards += 2; per = {}
    for k in KS:
        d = run_margin({l: top[l][:k] for l in top}) - base; forwards += 2; nulls = []
        for _ in range(N_NULL):
            rs = {l: tuple(rng.sample([j for j in range(4608) if j not in top[l]], k)) for l in top}; nulls.append(run_margin(rs) - base); forwards += 2
        per[str(k)] = {"change": d, "change_rel": d / base, "null_max_abs": max(abs(v) for v in nulls)}
    ch = {k: abs(per[str(k)]["change"]) for k in KS}
    report = {"native_margin": base, "per_k": per, "top_units": {str(l): list(v[:5]) for l, v in top.items()}}
    print(json.dumps(report, indent=1))
    predictions = {"pred_a_native_margin_positive": base > 0, "pred_b_k10_replays_v374": abs(ch[10] - V374) <= 0.02, "pred_c_every_k_beats_null_on_text": all(ch[k] > NULL_FACTOR * per[str(k)]["null_max_abs"] for k in KS),
                   "pred_d_saturates_on_text": ch[20] <= SAT * ch[10], "pred_e_leaders_carry_a_third_of_k10": ch[1] >= LEAD * ch[10]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "per_layer_dose_response_natural_result_v375", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
