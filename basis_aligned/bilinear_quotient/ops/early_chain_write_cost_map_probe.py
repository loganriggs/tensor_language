#!/usr/bin/env python
"""early_chain_write_cost_map_probe -- the §2764 chain-write cost (+.020 at k = 1024) mapped per early site: SPLIT8_1024 reads +
ONE site's write confined to the next site's frame (delete), for each of the 16 early sites, plus the joint chain. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_per_site_costs_are_additive pred_c_cost_is_concentrated
#                     pred_d_no_single_write_carries_the_chain pred_e_block0_writes_are_free
...
Preregistration: polynomial_causal/EARLY_CHAIN_WRITE_COST_MAP_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/early_chain_write_cost_map_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("early_chain_write_cost_map_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "EARLY_CHAIN_WRITE_COST_MAP_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "chain_bus_program_statement_probe_results.json"   # §2764
OUT = ROOT / "early_chain_write_cost_map_probe_results.json"
HASHES = {PREREG: "bffcf3b1fc7f32cad25ad937d841dc321332375b6fc0c02e187a7b0cf476924b", PRIOR: "de512d007fede1615714d0160999327768de40ba2da352a04a064a870d6f047c",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "early_chain_write_cost_map_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_lo": 0.5, "b_hi": 2.0, "c_min": 0.60, "d_max": 0.010, "e_max": 0.002}
NULLS = {"b_lo": 0.25, "b_hi": 4.0, "c_max": 0.35, "d_min": 0.015, "e_min": 0.006}
...
def main():
    ...
    def next_frame(s_):
        kd, l = s_
        if kd == "attn":
            return heads[("mlp", l)]["U"]
        return heads[("attn", l + 1)]["U"] if l + 1 < SPLIT else U_8
    def program(k, chain_sites=()):
        patch = {s_: head(s_, k) for s_ in OWN8}; buf = Buf()
        for s_ in chain_sites:
            patch[s_] = Both(head(s_, k), Route(s_, k, next_frame(s_), "delete", buf))
        for s_ in SET8:
    ...
    per_site = {}
    for s_ in OWN8:
        nm = f"ONE_{s_[0]}{s_[1]}_1024"; arms[nm] = program(K, (s_,)); per_site[f"{s_[0]}{s_[1]}"] = arms[nm] - arms["SPLIT8_1024"]
        log(stage="site", site=f"{s_[0]}{s_[1]}", cost=round(per_site[f"{s_[0]}{s_[1]}"], 5))
    arms["CHAIN_ONLY_1024"] = program(K, tuple(OWN8))
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    joint = arms["CHAIN_ONLY_1024"] - arms["SPLIT8_1024"]; tot = sum(per_site.values())
    ranked = sorted(per_site.items(), key=lambda kv: -kv[1]); top4 = sum(v for _, v in ranked[:4])
    ratio = tot / joint if abs(joint) > 1e-9 else float("inf"); top4_share = top4 / tot if abs(tot) > 1e-9 else float("nan")
    mx = ranked[0][1] if ranked else 0.0; blk0 = per_site.get("attn0", 0.0) + per_site.get("mlp0", 0.0)
    summ = {"joint_chain_minus_split8": joint, "sum_per_site": tot, "additivity_ratio": ratio, "top4_share": top4_share, "top4": ranked[:4], "max_site": ranked[0] if ranked else None,
            "block0_sum": blk0, "attn_sum": sum(v for k_, v in per_site.items() if k_.startswith("attn")), "mlp_sum": sum(v for k_, v in per_site.items() if k_.startswith("mlp"))}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(arms["CHAIN_ONLY_1024"] - PRIOR_CHAIN_ONLY_1024) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_per_site_costs_are_additive': bool(BARS["b_lo"] <= ratio <= BARS["b_hi"]),
        'pred_c_cost_is_concentrated': bool(top4_share >= BARS["c_min"]),
        'pred_d_no_single_write_carries_the_chain': bool(mx <= BARS["d_max"]),
        'pred_e_block0_writes_are_free': bool(blk0 <= BARS["e_max"]),
    }
    nulls = {"b_null_ratio_ge_4_or_le_.25": bool(ratio >= NULLS["b_hi"] or ratio <= NULLS["b_lo"]), "c_null_top4_le_.35": bool(top4_share <= NULLS["c_max"]),
             "d_null_max_ge_.015": bool(mx >= NULLS["d_min"]), "e_null_block0_ge_.006": bool(blk0 >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "SPLIT8_1024 reads + ONE early site's write confined to the next site's frame (delete); CHAIN_ONLY_1024 = all 16 jointly",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024,
                          "chain_only_1024": arms["CHAIN_ONLY_1024"], "prior_chain_only_1024": PRIOR_CHAIN_ONLY_1024},
           "ce_added": arms, "per_site_cost": per_site, "summary": summ,
    ...
    if smoke:
        print(json.dumps({k: out[k] for k in ("preds", "nulls", "instrument", "ce_added", "per_site_cost", "summary", "price")}, indent=1)); return
    from receipt import dump
    dump(out, OUT)
    print(json.dumps({"preds": preds, "nulls": nulls, "ce_added": arms, "per_site_cost": per_site, "summary": summ}, indent=1))
