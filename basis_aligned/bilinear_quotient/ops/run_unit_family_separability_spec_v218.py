#!/usr/bin/env python3
# BQGATE: five frozen predictions; families, EVAL set, units (v217 receipt), recipe and bars fixed before the run.
"""v218: within-family separability for v217's passes -- the count test for adj_adv_feel, adjective_preposition_similar,
adjective_preposition_fond.

v217 put the eighth eight spec-authored behaviours through the four-row battery; its three four-row passes each have a sibling in
the same batch: adj_adv_feel (`felt/spoke rather` -> quiet/quietly) with adj_adv_seem (`seemed/moved rather`, rows 3-4 misses:
a control); adjective_preposition_similar (`similar/different in size` -> to/from) with adjective_preposition_fond (`fond/keen, of
course,` -> of/on), both four-row -- the two members of that family are evaluated against each other. The benefactive pair, the
pronoun-case single and wish_subjunctive have no four-row member and are not prepared (the mood family has four constructions and
no counted member; it is re-tried when one passes). Recipe as v198/v202/v206/v208/v210/v212/v214/v216: per evaluated member, arm
`own` (own C_fit control) and arm `fam` (own C_fit + EVERY sibling's A1 FIT prep, control_weight 30 x len(controls)); evaluated on the
member's held A1 (extraction), each sibling's held A1 (|CE| leak) and held C. Units frozen from the v217 receipt.
...
All four members are prepared; EVAL = the three v217 passes. ~60-80 s per evaluated member (3 fits, ~4 min).
Count rule: each separable EVAL member is a distinct circuit (36 -> up to 39); if similar and fond leak into each other they are ONE
adjective-preposition circuit with two surface constructions (counts +1 if either is separable from the other's direction, per the
v216 aux-copy precedent: an asymmetric pair counts once).

REGISTERED BEFORE THE RUN (each coded predicate is the sentence here):
    pred_a_adj_adv_feel          adj_adv_feel is separable from adj_adv_seem.                                          prior 55%
    pred_b_adjprep_similar       adjective_preposition_similar is separable from adjective_preposition_fond.           prior 55%
    pred_c_adjprep_fond          adjective_preposition_fond is separable from adjective_preposition_similar.           prior 55%
    pred_d_row4_kept             fam-arm own-C UB975 <= 0.01 on >= 2 of the 3 evaluated members.                       prior 65%
    pred_e_instrument            every evaluated member's own-arm extraction is within +-0.03 of its v217 row-4 (cdas)
                                 extraction_held (0.952 / 1.009 / 0.993; same units, seed, split and code path -- a determinism check, NOT an
                                 independent run outcome).                                                             prior 90%
Priors are not moved by unit overlap in either direction (v214: shared units separated; v216: shared units fused).
Smoke: V218_SMOKE=<out.json> (CPU, V218_SMOKE_ROWS=4; all four members are prepared as controls, V218_SMOKE_NAMES=adj_adv_feel,adjective_preposition_fond
are evaluated so both families and the instrument read are exercised; steps 5).
"""
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v218_result.json"
...
FAMILIES = {
    "predicate_category": ["adj_adv_seem", "adj_adv_feel"],
    "adjective_preposition": ["adjective_preposition_similar", "adjective_preposition_fond"],
}
EVAL = ("adj_adv_feel", "adjective_preposition_similar", "adjective_preposition_fond")
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES, **v213.NAMES, **v215.NAMES, **v217.NAMES}
...
BARS = {"cross_max": 0.05, "keep": 0.80, "ext_floor": 0.50, "c_ub_max": 0.01, "k_d": 2, "instr_tol": 0.03}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 20000, 640000


def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v218", "members": n, "families": list(FAMILIES),
...
def PREDS(R):
    ...
    a = ok and sep("adj_adv_feel")
    b = ok and sep("adjective_preposition_similar")
    c = ok and sep("adjective_preposition_fond")
    d = ok and sum(1 for n in good if good[n]["arms"]["fam"]["C"]["ce_ub975"] <= B["c_ub_max"]) >= B["k_d"]
    e = ok and all(n in good and good[n]["arms"]["own"]["extraction"] is not None
                   and abs(good[n]["arms"]["own"]["extraction"] - _parent_cdas(n)) <= B["instr_tol"] for n in EVAL)
    return {"pred_a_adj_adv_feel": bool(a), "pred_b_adjprep_similar": bool(b), "pred_c_adjprep_fond": bool(c),
...
def main() -> None:
    ...
    smoke = os.environ.get("V218_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V218_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v218", "candidate_id": "corpus.unit_family_separability_spec_v218",
              "separable_counts": nsep, "families": FAMILIES, "summary": summary, "members": R, "bars": BARS,
              "recipe": {"lambda": LAM, "steps": steps, "lr": LR, "complement_weight": CW, "rank": 1, "seed": 0, "units_source": "v197 + v201 + v204 + v205 + v207 + v209 + v211 + v213 + v215 + v217 receipts (later receipts override earlier ones)", "evaluated": list(EVAL),
