# SEQUENTIAL CALIBRATION  -- the fixed point §1823's first-order attempt jumped over.
#
# §1822: gains measured in the FULLY COMPILED stream repair the single-live-layer case (§1821, 99%) but
# not a bottom-up split -- at B0 they turn +37.4% into -4.1%. §1823: gains measured IN each arm fix B0
# (+55.7%, the best figure anywhere) but LOSE to the global gains at B3 by 33 points, because the arm
# they are measured in is itself pathological (-44.8%) and gains calibrated to a broken stream do not
# converge on the fixed point.
#
# Both failures are the same defect: a gain is only valid for a layer whose INPUT matches the stream the
# gain was measured in, and correcting a layer changes the stream for everything above it. Neither
# one-shot measurement can satisfy that for more than one layer at a time.
#
# SEQUENTIAL CALIBRATION satisfies it for all of them: correct the interface layer, RE-MEASURE, correct
# the next, and so on upward, so each gain is measured in a stream already corrected below it. That is
# the construction §1823 named and the only one left that is not a post-hoc selection between the two
# previous gain sources.
#
# Calibration uses CALROWS rows (a mean norm converges quickly); every scored arm uses the full role.
# BUILT FULL RANK to match §1806 / §1822 / §1823 (LESSON 53).
#
...
#   pred_a THE FIXED POINT IS BETTER THAN EITHER SHORTCUT: at B3, sequential beats the BETTER of the
#          global (+10.3 / +10.0 / +12.0%) and depth-matched (-22.8 / -23.0 / -24.1%) gains by at least
#          5 points, at every role. Scored against the better of the two, not a fixed number, so it
#          cannot be won by beating the weaker one. If FALSE, iterating buys nothing over a single
#          well-chosen measurement and the residual damage at B3 is not a gain error at all -- which
#          would close the magnitude account for the bottom-up direction and send the question back to
#          content.
#   pred_b AND IT STILL DOES NOT HARM B0: sequential is no worse than the RAW arm at B0 (+37.4 / +39.6 /
#          +38.1%). The global gains failed exactly here, and a correction that damages a healthy
#          configuration is unusable however well it does elsewhere.
#   pred_c AND THE CURVE ORDERS CORRECTLY: sequential recovery is monotone across B0 >= B3 >= B5.
#          §1806's signature defect was that compiling LESS scored worse than compiling everything;
#          neither previous gain source fixed the ordering, and a remedy that repairs magnitude without
#          repairing the ordering has not repaired the pathology.
#   pred_d CONTROLS, cross-run per LESSON 42, plus a KNOWN-ANSWER check per LESSON 34: raw arms
#          reproduce §1806's PUBLISHED full-rank fractions, global arms §1822's PUBLISHED -0.041 /
#          +0.103 / +0.115, and matched arms §1823's PUBLISHED +0.557 / -0.228 / -0.147, all within
#          tolerance; endpoints reproduce §1789's FULL-RANK figures; the placement control moves top-1
#          by under 0.05pp; AND the FIRST sequential gain at each depth must equal the global gain for
#          that layer to within 2%, because the interface layer sees the fully compiled stream under
#          both constructions -- §1823 already observed that identity exactly (0.1696 vs 0.1696), so if
#          it fails here the sequential machinery is wrong and nothing else is readable.
...
OUT = PT + 'ops/sequential_calibration_results.json'
...
S1823_MATCHED_GAPFRAC = {0: {'skip7000': 0.557, 'skip11000': 0.568, 'skip1200': 0.532},
                         3: {'skip7000': -0.228, 'skip11000': -0.230, 'skip1200': -0.241},
                         5: {'skip7000': -0.147, 'skip11000': -0.138, 'skip1200': -0.144}}
...
def main():
    ...
    print(f'SEQUENTIAL CALIBRATION | gains measured bottom-up in an already-corrected stream | '
    ...
    best_prior = {L: {e: max(frac[e][f'B{L}_global'], frac[e][f'B{L}_matched'])
                      for e in roles} for L in DEPTHS}
    pa = all(frac[e]['B3_seq'] - best_prior[3][e] >= 0.05 for e in roles)
    pb = all(frac[e]['B0_seq'] >= frac[e]['B0_raw'] - 1e-9 for e in roles)
    pc = all(frac[e]['B0_seq'] >= frac[e]['B3_seq'] >= frac[e]['B5_seq'] - 1e-9 for e in roles)
    ...
    print(f'\n  sequential beats the better prior gain by >=5 points at B3 -> {pa}  ' + '  '.join(
        f'{e} {frac[e]["B3_seq"]:.1%} vs best-prior {best_prior[3][e]:.1%}' for e in roles),
        flush=True)
    print(f'  and does NOT harm B0 -> {pb}  ' + '  '.join(
        f'{e} {frac[e]["B0_seq"]:.1%} vs raw {frac[e]["B0_raw"]:.1%}' for e in roles), flush=True)
    print(f'  and the sequential curve is MONOTONE in depth -> {pc}  ' + '  '.join(
        f'{e} ' + '/'.join(f'{frac[e][f"B{L}_seq"]:.1%}' for L in DEPTHS) for e in roles),
        flush=True)
    print(f'  raw/global/matched reproduce §1806/§1822/§1823, the first sequential gain equals the '
          f'global one at every depth, endpoints §1789, placement exact -> {pd}', flush=True)

    json.dump({'run': 'sequential_calibration', 'depths': list(DEPTHS), 'cal_rows': CALROWS,
               ...
               'gap_fraction': frac,
               'predictions': {'pred_a_sequential_beats_best_prior': bool(pa),
                               'pred_b_sequential_does_not_harm_B0': bool(pb),
                               'pred_c_sequential_curve_monotone': bool(pc),
