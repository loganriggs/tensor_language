...
48 opened noun/syntax-transfer sequences plus matched random site edits, eight prefix calls and twenty-four suffix calls, zero fits.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v668_result.json'
PREDICTIONS=dict(pred_a_exact_replay='baseline and independent joint versus shared chain replay max absolute error <=1e-4',
 pred_b_effect_prediction='top18/36 pair program full native effect error<=.10 eachcell and modal prediction error<=.05 of target norm',
 pred_c_capable_position_transfer='top18/36 worst-cell native effect error below median of three equal-price random18/36 arms')
...
def main():
    plan=dict(prefix_calls=8,suffix_calls=56,sequences_per_arm=48,fits=0,backwards=0,
              ...
              random_edit_seeds=list(range(66300,66308)),modal_readers=[['can','will'],['may','might'],['should','could']],additional_native_attention_calls=24,execution_policy='managed_queue_only',predictions=PREDICTIONS)
    ...
    sparse_runtimes={};arm_reports={}
    specifications=[('norm18',18,'tensor_norm',668),('norm9',9,'tensor_norm',668),('zero',0,'tensor_norm',668)]+[(f'random18_{seed}',18,'random',seed) for seed in [668,669,670]]
    for name,keep,strategy,seed in specifications:
        sparse_runtimes[name],_=prune_pairs(runtime,input_encoders,output_decoders,keep,strategy,seed)
        arm_reports[name]=dict(cells={},control_prediction=[],mlp_numerator_products_per_token=6*keep,normalization_dense_gram_forms=6,
            normalization_note="Dense norm geometry remains; pruning numerator pairs does not remove all quadratic work",
            quadratic_coefficients=6*8*keep,index_values=6*2*keep,
            retained_pairs=[list(zip(b['pair_i'].tolist(),b['pair_j'].tolist())) for b in sparse_runtimes[name]['blocks']])
    chain_checks=[]
    ...
    def native_selected(raw,x0,first,readpos):
        x=raw
        for layer in range(11,18):
            block=model.transformer.h[layer]
            if layer>11:x=block.lambdas[0]*x+block.lambdas[1]*x0
    ...
    for entries in batches:
        ...
        for name,sparse_runtime in sparse_runtimes.items():
            z=initial_all
            for rb,ap,mlpc,azero in zip(sparse_runtime['blocks'],context['all_attention'],context['all_mlp_contexts'],context['all_zero']):
                innovation=folded_attention(ap,z*rb['scale'])-azero
                z=shared_step(rb,z,mlpc,innovation)
            logits=readout_prepared(runtime['readout'],z[batch,readpos],case['final_context'])
            effect=base-(logits[:,0]-logits[:,1])*direction
            modal=readout_prepared(context['control_fixed'],z[batch,readpos],context['control_context'])
            modal_effect=((cb-modal)[:,0::2]-(cb-modal)[:,1::2])
            arm_reports[name]['control_prediction'].extend(modal_effect.tolist())
            for i,e in enumerate(entries):
                cell=arm_reports[name]['cells'].setdefault(e[3],dict(target=[],prediction=[]))
                cell['target'].append(float(base[i]-true[i]));cell['prediction'].append(float(effect[i]))
    ...
    for arm in arm_reports.values():
        for cell in arm['cells'].values():
            y=np.asarray(cell['target']);prediction=np.asarray(cell['prediction'])
            cell['relative_error']=float(np.linalg.norm(prediction-y)/np.linalg.norm(y))
        arm['worst_cell_error']=max(cell['relative_error'] for cell in arm['cells'].values())
        arm['modal_prediction_error']=(np.linalg.norm(np.asarray(arm['control_prediction'])-native_control,axis=0)/target_norm).tolist()
        arm['passes']=arm['worst_cell_error']<=.10 and max(arm['modal_prediction_error'])<=.05
    a=a and len(attention_checks)==24 and max(attention_checks)<=1e-4
    b=a and arm_reports['norm18']['passes']
    c=a and arm_reports['norm18']['worst_cell_error']<float(np.median([arm_reports[f'random18_{seed}']['worst_cell_error'] for seed in [668,669,670]]))

    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,cells=cells,
                predictions=dict(zip(PREDICTIONS,[bool(a),bool(b),bool(c)])),selectivity=selectivity,sparse_arms=arm_reports,
