#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_distant_verbs_read_values_at_the_noun pred_c_adjacent_verbs_do_not pred_d_distant_named_copiers_carry pred_e_distant_units_carry
"""The verb margin split by noun-verb distance (v446). v445: at the natural verb slot only 0.475 of the agreement margin passes through attention values at
the noun. In 43 of the 122 rows the verb follows the noun directly (distance 1): truncated at the verb slot, the answer position IS the noun, whose own
residual reaches the logits with no copy. Here the census on the two subsets — adjacent (distance 1, 43 rows) and distant (distance >= 2, 79 rows) — for
every head's value at the noun, the five readers, the copier and the three units.
PREDICTIONS (scored as written; failures preserved; priors from v428 / v445)
    pred_a_baseline_replays                   the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_distant_verbs_read_values_at_the_noun  on distant rows every head's value at the noun closes >= 0.85 of the verb margin gap
    pred_c_adjacent_verbs_do_not              on adjacent rows every head's value at the noun closes <= 0.15
    pred_d_distant_named_copiers_carry        on distant rows the five readers close >= 0.15 and the copier >= 0.05. Prior: unsure
    pred_e_distant_units_carry                on distant rows the three units close >= 0.12. Prior: unsure
PRICE (registered maximum): (3 + 2 batches) x 5 passes = 25 forwards; 0 backwards; 0 fits. Bar <= 26.
"""
...
OUT = ROOT / "circuits/followups/verb_margin_distance_v446_result.json"
CANDIDATE_ID = "chain.verb_margin_distance_v446"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, DIST_ALL_MIN, ADJ_ALL_MAX, READERS_MIN, COPIER_MIN, UNITS_MIN = 1e-3, 0.85, 0.15, 0.15, 0.05, 0.12
...
FORWARDS_MAX = 26
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_distant_verbs_read_values_at_the_noun": ">= 0.85", "pred_c_adjacent_verbs_do_not": "<= 0.15", "pred_d_distant_named_copiers_carry": "readers >= 0.15, copier >= 0.05", "pred_e_distant_units_carry": ">= 0.12"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "dist_all_min": DIST_ALL_MIN, "adj_all_max": ADJ_ALL_MAX, "readers_min": READERS_MIN, "copier_min": COPIER_MIN, "units_min": UNITS_MIN}}
    ...
    dist, nat_t = study(dist_items, 32); adj, _ = study(adj_items, 32); verb = dist; text = dist; panel = {"adjacent": {k: v for k, v in adj.items() if k != "native_margins"}, "adjacent_rows": len(adj_items), "distant_rows": len(dist_items)}; verb_items = dist_items
    ...
    c = verb["closed"]
    ca = adj["closed"]
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_distant_verbs_read_values_at_the_noun": c["ALL_NOUN"]["verb"] >= DIST_ALL_MIN, "pred_c_adjacent_verbs_do_not": ca["ALL_NOUN"]["verb"] <= ADJ_ALL_MAX,
                   "pred_d_distant_named_copiers_carry": c["FIVE_N"]["verb"] >= READERS_MIN and c["COPIER"]["verb"] >= COPIER_MIN, "pred_e_distant_units_carry": c["UNITS"]["verb"] >= UNITS_MIN}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "verb_margin_distance_result_v446", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
