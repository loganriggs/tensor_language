...
"""Frozen native DAG skip test, QUADRATIC_SKIP_PLAN_V1.md.
pred_a_exact: previous solve<1e-8, all native replays<1e-5.
pred_b_gaussian: previous independent full-rank Gaussian MSE gain>=10%.
...
Price ranks1/2/4/6:20790/21948/24264/26544coefficients,10products each.
...
def main():
 ...
 P=branch.P;out=P/'QUADRATIC_SKIP_NATIVE_V1.json';assert not out.exists();fit=json.loads((P/'QUADRATIC_SKIP_FIT_V1.json').read_text());records={};replays=[]
 for domain in ['fineweb','code']:
  branch.PANEL_PATH=None if domain=='fineweb' else P/'CODE_SHIFT_PANEL_V1.pt';branch.PROGRAM_SPECS={f'skip{r}':('QUADRATIC_SKIP_PROGRAM_V1.pt',r) for r in RANKS};branch.PRIMARY='skip2';branch.OUTPUT_STEM='QUADRATIC_SKIP_BRANCH_'+domain.upper()+'_V1';branch.PLAN=dict(branch.PLAN,documents=list(range(64,80)) if domain=='fineweb' else list(range(16)),arms=['exact','ablation']+[f'skip{r}' for r in RANKS],program_specs=branch.PROGRAM_SPECS)
  ...
  for rank in RANKS:
   mode.PANEL_PATH=None if domain=='fineweb' else P/'CODE_SHIFT_PANEL_V1.pt';mode.PROGRAM_FILE='QUADRATIC_SKIP_PROGRAM_V1.pt';mode.PROGRAM_KEY=rank;mode.OUTPUT_STEM=f'QUADRATIC_SKIP_MODE_{domain.upper()}_R{rank}_V1';mode.PLAN=dict(mode.PLAN,documents=list(range(80,96)) if domain=='fineweb' else list(range(16)),program_file=mode.PROGRAM_FILE,program_key=rank)
