# ISO-COST: what does the deployed build's storage buy if you spend it on RANK instead?
#
# §1853-§1860 established that the deployed full-rank build at 5,419 covered types -- 230.087M reals,
# all-position CE 6.01167 / 5.98477 / 6.00165 -- is Pareto-dominated, confirmed second class on both
# arms across seven builds. Every domination so far has been at a DIFFERENT cost, which makes the claim
# correct but awkward to act on: "cheaper and better" invites "how much better at the same price?"
#
# §1754's model answers what fits. At 16,110 covered types a rank-r table costs
# 36*(r*17262 + 2304) + 5.308M, so the deployed 230.087M budget buys up to **rank 361** (229.728M).
# Ranks 320 and 352 cost 204.250M and 224.135M -- also inside the budget.
#
# This measures 256, 320, 352 and 361 at 16,110 types and states the result the way a decision needs it:
# at IDENTICAL storage, how much better is the alternative build?
#
# Docstring and predicate block written in ONE edit (LESSONS 63); costs COMPUTED from §1754's model
# rather than typed (§1858's defect). Full rank is NOT built -- ceiling attainment is settled on seven
# covered sets and the rank-256 cross-run anchor serves as the control here.
#
# ROLES. All three. DISCOVERY ONLY. Rung 3: the question §1860's actionable claim raises.
#
...
#   pred_a AT THE DEPLOYED BUDGET, RANK WINS BY A MARGIN WORTH ACTING ON: rank 361 at 16,110 types costs
#          at most the PUBLISHED 230.087M and beats the PUBLISHED full@5,419 all-position CE of 6.01167 /
#          5.98477 / 6.00165 by at least 0.03 nats on all three roles. If TRUE the record can state an
#          iso-cost improvement rather than a Pareto relation, which is the form a deployment decision
#          actually takes. If FALSE the advantage is real but small at matched cost, and §1860's
#          "actionable" needs qualifying to "cheaper at equal fidelity" rather than "better at equal
#          cost" -- a weaker and more honest claim.
#   pred_b AND SPENDING THE WHOLE BUDGET BEATS STOPPING AT 256: rank 361 beats §1852's PUBLISHED
#          rank-256@16,110 all-position CE of 5.98851 / 5.94106 / 5.95781 on all three roles. If FALSE
#          the extra 65M reals between rank 256 and the budget ceiling buy nothing, and rank 256 is the
#          right recommendation regardless of budget -- a simpler answer than pred_a's.
#   pred_c AND THE LADDER IS MONOTONE across 256, 320, 352, 361 on every role, as §1849 and §1854 found
#          at coarser spacing. If FALSE the rank axis is not a ladder at this resolution and no
#          interpolated recommendation is safe.
#   pred_d CONTROLS: rank 256 reproduces §1852's PUBLISHED CE above the ceiling (+0.09252 / +0.09844 /
#          +0.08049) within 0.005 at the same 16,110-type covered set; every quoted cost is computed
#          from §1754's model and rank 361's is at or under 230.087M; coverage is 16,110; the ceiling is
#          finite and above live on every role.
...
OUT = PT + 'ops/iso_cost_rank_results.json'
...
TRANKS = (256, 320, 352, 361)          # None = full rank; §1786/§1813 use this ladder
...
FITLO, FITHI = 0, 480       # the full stream: §1852's 16,110-type covered set   # 5x the fit rows -> a larger covered set
H = m.transformer.h
NCOV = 5419
...
def main():
    ...
    assert NFULL == 16110, f'coverage {NFULL} != §1852 published 16110'
    ...
    print(f'ISO COST RANK | 16,110 types, ranks 256/320/352/361 at the deployed budget | '
    ...
    cost = {l2: rcost(l2) for l2 in LADDER}
    allpos = {l2: {e: curve[l2][e]['allpos'] for e in roles2} for l2 in LADDER}
    above = {l2: {e: curve[l2][e]['cov'] - ceilv[e] for e in roles2} for l2 in LADDER}
    gain = {e: DEPLOYED_ALLPOS[e] - allpos['361'][e] for e in roles2}
    over256 = {e: S1852_R256_ALLPOS[e] - allpos['361'][e] for e in roles2}
    pa = cost['361'] <= DEPLOYED_COST_M and all(v >= 0.03 for v in gain.values())
    pb = all(v > 0 for v in over256.values())
    pc = all(allpos[LADDER[i]][e] <= allpos[LADDER[i - 1]][e] + 1e-12
             for e in roles2 for i in range(1, len(LADDER)))
    pd = (all(abs(above['256'][e] - S1852_R256_ABOVE[e]) <= 0.005 for e in roles2)
          and NFULL == 16110 and all(ceilv[e] > livev[e] for e in roles2))

    print(f'\n  ISO-COST at {NFULL} types -- the deployed budget is {DEPLOYED_COST_M}M '
          f'(full rank at 5,419 types):', flush=True)
    for l2 in LADDER:
        print(f'    rank {l2:4s} {cost[l2]:8.3f}M  ' + '  '.join(
            f'{e} {allpos[l2][e]:.5f} (+{above[l2][e]:.5f})' for e in roles2), flush=True)
    print(f'    DEPLOYED {DEPLOYED_COST_M:8.3f}M  ' + '  '.join(
        f'{e} {DEPLOYED_ALLPOS[e]:.5f}' for e in roles2), flush=True)
    print(f'\n  AT THE DEPLOYED BUDGET rank wins by >=0.03 nats -> {pa}  rank 361 costs '
          f'{cost["361"]:.3f}M <= {DEPLOYED_COST_M}M, gain ' + '  '.join(
        f'{e} {gain[e]:+.5f}' for e in roles2), flush=True)
    print(f'  and spending the WHOLE budget beats stopping at 256 -> {pb}  ' + '  '.join(
        f'{e} {over256[e]:+.5f}' for e in roles2), flush=True)
    print(f'  and the ladder is MONOTONE at this resolution -> {pc}', flush=True)
    print(f'  rank 256 reproduces §1852 above-ceiling, coverage {NFULL} -> control {pd}', flush=True)

    json.dump({'run': 'iso_cost_rank', 'n_covered': NFULL, 'ladder': LADDER,
               'cost_M': cost, 'allpos': allpos, 'above_ceiling': above,
               'gain_over_deployed': gain, 'gain_over_r256': over256,
               'ceiling': ceilv, 'live': livev,
               'predictions': {'pred_a_iso_cost_rank_wins': bool(pa),
                               'pred_b_whole_budget_beats_256': bool(pb),
                               'pred_c_monotone_at_resolution': bool(pc),
