...
pred_b full-group transfer>=40%whole-child reference and>=5/8positive EACHfamily;
...
STEM='REGIONAL_PRODUCER_ROUTE_VALUE_V1'
@torch.no_grad()
def main():
    ...
    prior=torch.load(P/'REGIONAL_PAYLOAD_ATLAS_V1_ARTIFACT.pt',weights_only=True,map_location='cpu')['write_vertices'].cuda()
    ...
    reference_cells=json.loads((P/'REGIONAL_PAYLOAD_ATLAS_V1_RESULT.json').read_text())['cells']
    for family in (0,1):
    ...
    full=instrument and all(c['mean_transfer_by_arm'][3][0]>=.4*c['whole_child_reference'] and c['positive_regional_by_arm'][3]>=5 for c in cells)
