"""CLOSE THE DECOMPOSITION: PRICE THE LAST TWO cfgF BLOCKS AND SEE WHETHER THE SHARES ADD UP (Claude, LANE 1)

SS2882 and SS2883 priced four blocks of the SS312 frontier and left **+1.0978 nats -- 41.1% of the published +2.6735 -- unattributed**:

    front MLP tables   +1.0045      motif heads (attn 2-9)   +0.3988
    CP units c4-c9     -0.2140      tail dictionaries        +0.3864

The evaluated config is `order2 = cfgF + ['a10L'...'a17L']` with
`cfgF = ['a0','m0E','a1v','m1','m2E','m3E'] + ['c4'...'c9'] + ['tailE']`, so exactly two blocks remain unpriced: the EARLY-ATTENTION
entries `a0`/`a1v`, and the `tailE` entry. Pricing them partitions `cfgF` completely, and the sum of all six shares against the
frontier's own +2.6735 is then a **closure test** -- the first check of whether this whole decomposition is a partition at all or
whether interaction terms dominate it. SS2883 already measured a +0.2129 additivity drift on one pair and SS2880 found strong
superadditivity inside the MLP stage, so a large closure gap is a live outcome and is registered as such.

Arms restore an approximation to the REAL component, which should LOWER L2, so the quantity is an ERROR SHARE:

    error_share(block) = L2_F(baseline) - L2_F(block restored to real)
      POSITIVE = that block contributes that much error
      NEGATIVE = the approximation BEATS the real component in this stack -- which SS2883 measured for the CP units at -0.2140

The four prior shares are read from the SS2882/SS2883 receipts under FROZEN HASHES rather than retyped, so the closure arithmetic
cannot drift from the numbers it sums.

SIGN CONVENTION (SS2135): frontier L2 is CE ADDED ABOVE THE REAL MODEL, so **LOWER IS BETTER** (SS312: +2.6735 beating +2.84/+2.93).
SS2128/SS2129/SS2133/SS2134 RETRACTED; SS2125 STANDS. Per SS2879 every manipulated entry is a member of `cfgF` by construction, and
pred_d re-checks that as a measured predicate.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_the_early_attention_entries_carry_a_share
#                     pred_c_the_tailE_entry_carries_a_share
#                     pred_d_both_arms_are_connected
#                     pred_e_the_decomposition_closes

Preregistration: polynomial_causal/FRONTIER_DECOMPOSITION_CLOSURE_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; three arms (baseline / early attention off / tailE off)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_decomposition_closure_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
TAILMODE={'mode':None}
EVALMODE={'drop':frozenset()}   # prefixes to OMIT from the evaluated config: 'c' = c4-c9, 'm' = front MLP tables
CVSNAP={}
SKIP8=64
...
def main():
    ...
    L2C=evalM(FW[R0:R1],R1-R0,_o2,_ml)-baseC
    del cur['clsmap']
    ...
    L2F=evalM(FR,120,_o2,_ml)-baseF
    del cur['clsmap']
    inc=L2F-L1F
...
PREREG = PT + '../polynomial_causal/FRONTIER_DECOMPOSITION_CLOSURE_PREREGISTRATION.md'
PREREG_SHA = "3b0d5efd000dc6785e11d7ed08136b5f64f6be7a9404c04279e92574445ae341"
...
S2882 = PT + 'frontier_error_decomposition_results.json'
S2883 = PT + 'frontier_mlp_side_error_share_results.json'
S2882_SHA = "2a269d51e87f95d00e8f485e162b7a56710a185f2bf2e38947bfb29da715d2b3"
S2883_SHA = "cb3142892675744daa73220a0a6adcaad32d5f8e251813de66fbe6a393b90074"
BARS = {"reproduce": 0.05, "early_share": 0.10, "tail_e_share": 0.10, "connected": 0.005, "closure": 0.60}
NULLS = {"early_share_le": 0.02, "tail_e_share_le": 0.02, "closure_ge": 1.20}
...
if __name__=='__main__':
    ...
    for p_, h_ in ((S2882, S2882_SHA), (S2883, S2883_SHA)):
        if _sha(p_) != h_:
            raise RuntimeError(f'frozen hash mismatch: {p_}')
    prev82 = json.load(open(S2882))['summary']
    prev83 = json.load(open(S2883))['summary']
    sh_motif = prev82['error_share_motif_heads_nats']
    sh_tail  = prev82['error_share_tail_dicts_nats']
    sh_front = prev83['error_share_front_tables_nats']
    sh_cp    = prev83['error_share_cp_units_nats']
    SEL['mode']='norm'

    print('ARM 1/3: BASELINE -- the published frontier',flush=True)
    EVALMODE['drop']=frozenset()
    r_base=main()

    print("ARM 2/3: a0 and a1v OFF -- the early-attention entries run REAL",flush=True)
    EVALMODE['drop']=frozenset(['a'])
    r_noearly=main()

    print("ARM 3/3: tailE OFF -- the empirical tail entry runs REAL",flush=True)
    EVALMODE['drop']=frozenset(['t'])
    r_notaile=main()
    EVALMODE['drop']=frozenset()

    # SIGN: L2 is CE ADDED ABOVE THE REAL MODEL, LOWER IS BETTER. These are APPROXIMATIONS, so restoring one
    # should LOWER L2. ERROR SHARE = baseline - restored; POSITIVE = that block contributes that much error,
    # NEGATIVE = the approximation BEATS the real component in this stack (as SS2883 found for the CP units).
    sh_early = r_base['L2_F'] - r_noearly['L2_F']
    sh_tailE = r_base['L2_F'] - r_notaile['L2_F']
    total    = sh_motif + sh_tail + sh_front + sh_cp + sh_early + sh_tailE
    closure  = abs(total - r_base['L2_F'])

    pa = abs(r_base['L2_F']-2.6735) <= BARS['reproduce']
    pb = sh_early >= BARS['early_share']
    pc = sh_tailE >= BARS['tail_e_share']
    pd = (abs(sh_early) >= BARS['connected']) and (abs(sh_tailE) >= BARS['connected'])
    pe = closure <= BARS['closure']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_early_attention_entries_carry_a_share':bool(pb),
           'pred_c_the_tailE_entry_carries_a_share':bool(pc),
           'pred_d_both_arms_are_connected':bool(pd),
           'pred_e_the_decomposition_closes':bool(pe)}
    nulls={'b_null_the_early_entries_are_error_free':bool(sh_early<=NULLS['early_share_le']),
           'c_null_the_tailE_entry_is_error_free':bool(sh_tailE<=NULLS['tail_e_share_le']),
           'e_null_interactions_dominate':bool(closure>=NULLS['closure_ge'])}
    res={'rung':'frontier_decomposition_closure','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,
         'baseline':r_base,'early_off':r_noearly,'tailE_off':r_notaile,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(r_base['L2_F'],4),
                    'L2_F_early_attention_off':round(r_noearly['L2_F'],4),
                    'L2_F_tailE_off':round(r_notaile['L2_F'],4),
                    'error_share_early_attention_nats':round(sh_early,4),
                    'error_share_tailE_nats':round(sh_tailE,4),
                    'share_motif_heads_S2882':round(sh_motif,4),
                    'share_tail_dicts_S2882':round(sh_tail,4),
                    'share_front_tables_S2883':round(sh_front,4),
                    'share_cp_units_S2883':round(sh_cp,4),
                    'sum_of_all_six_blocks_nats':round(total,4),
                    'frontier_L2_F':round(r_base['L2_F'],4),
                    'closure_gap_nats':round(closure,4),
                    'L2_C_baseline':round(r_base['L2_C'],4),
                    'L2_C_early_off':round(r_noearly['L2_C'],4)},
         'price':{'gpu_forwards':0,'forwards_instrumented':False,'pipeline_runs':3,
                  'backwards':0,'fitted_parameters':0,
                  'gpu_seconds':round(time.time()-T00,1)},
         'hashes':{PREREG:PREREG_SHA,S2882:S2882_SHA,S2883:S2883_SHA},'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {r_base['L2_F']} vs 2.6735 (tol .05): {'HELD' if pa else 'FAILED'}")
    print(f"(b) early-attention share {sh_early:+.4f} >= {BARS['early_share']}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) tailE share {sh_tailE:+.4f} >= {BARS['tail_e_share']}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) both arms connected: {'HELD' if pd else 'FAILED'}")
    print(f"(e) six blocks sum {total:+.4f} vs frontier {r_base['L2_F']:+.4f}, gap {closure:.4f} <= {BARS['closure']}: {'HELD' if pe else 'FAILED'}")
