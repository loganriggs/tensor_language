#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the second eight spec-authored behaviours, plus one instrument.
"""v205: the AMENDED tier-3 battery (v196/v197 protocol, unchanged) over the SECOND EIGHT spec-authored behaviours -- plus one instrument.

v204 put four of eight spec-authored behaviours through all four rows in 275 GPU-s, so authoring is the throughput path. Eight
more, written today through `circuit_fast_screen_behaviour_spec`, single-cue swap, matched final token, 32 A1/A2/P/C rows,
ABAB directions: binding (Principle A/B: `The king near the lantern admired` -> himself / `The king said the queen admired` -> him),
possessive_gender (king/queen -> his/her), object_pronoun_gender (cue seven tokens back -> him/her), numeral_number (three/one ADJ
-> ones/one), subcategorization (put/saw the ADJ OBJ -> on/and: an argument-frame obligation, new family), person_possessive
(I/you -> my/your: person agreement, new family), equative_result (as/so ADJ -> as/that), reciprocal (two pilots/pilot greeted ->
each/himself). Capability printed BEFORE the run on CPU (`g.prepare(valid_only=True)`, 32 rows per family): zero A1/A2 rows
dropped on all eight; mean base/donor margins A1: binding -0.89/+0.85 (weakest), possessive_gender -2.94/+3.10,
object_pronoun_gender -2.54/+2.52, numeral_number -4.10/+4.43, subcategorization -1.97/+1.63, person_possessive -4.75/+4.48,
equative_result -6.19/+6.17, reciprocal -3.53/+3.14; A2 weakest object_pronoun_gender -1.64/+1.36. Protocol identical to
v197/v201/v204 (greedy pool 40, target 0.88, min_gain 0.005, max 30; row 2 = MIN over the two held directions >= 0.80; row 3 dim
A1 CE LB975 > 0 and >= 0.10; row 4 constrained rank-1 own-C UB975 <= 0.01 on 16 held C rows; row 5 = A2-own / A2 full-rank
...
REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 4 of the 8 new behaviours pass all four rows (v204 4/8 with the same recipe; binding and
                           subcategorization are the weak cells).                                              prior 50%
  ...
  pred_e_row5_amended   -> A2-own / A2 ceiling within [0.5, 1.4] with LB975 > 0 on at least 5 of 8.               prior 60%
...
Accounting: four-row passes here join the one-protocol standing; distinct-circuit counts wait for separability (the gender
readouts vs v204's; numeral vs demonstrative number; equative vs comparative_frame; reciprocal vs reflexive_number).
...
Smoke: V205_SMOKE=<out.json> (CPU, V205_SMOKE_ROWS=4 per split, V205_SMOKE_NAMES=binding; pool 3/max 3, steps 5).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec8b_v205_result.json"
...
NAMES = {"binding": "binding", "possessive_gender": "possessive_gender", "object_pronoun_gender": "object_pronoun_gender",
         "numeral_number": "numeral_number", "subcategorization": "subcategorization", "person_possessive": "person_possessive",
         "equative_result": "equative_result", "reciprocal": "reciprocal", "finiteness_selection": "finiteness"}
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec8b_v205", "behaviours": 9, "constructions": 2,
...
def main() -> None:
    ...
    smoke = os.environ.get("V205_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V205_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V205_SMOKE_NAMES", "binding").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec8b_v205", "candidate_id": "corpus.unit_tier3_batch_amended_spec8b_v205", "bars": BARS,
