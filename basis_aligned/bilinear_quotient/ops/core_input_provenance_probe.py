#!/usr/bin/env python
"""core_input_provenance_probe -- which upstream writes (attn0..15, mlp0..15) causally supply the 16 core input coordinates mlp16/17 compute on
(§2720)? Replace each upstream write's CORE component by its mean (other 1136 directions intact) and measure CE added; 32 singles + ALL + per-kind.
CUDA lane-1 script.
...
Preregistration: polynomial_causal/CORE_INPUT_PROVENANCE_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/core_input_provenance_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("core_input_provenance_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "CORE_INPUT_PROVENANCE_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_mlp_weights_on_core_input_probe_results.json"   # §2720
OUT = ROOT / "core_input_provenance_probe_results.json"
HASHES = {PREREG: "94d0831a2052224431a9dc49b9461060e05eb7b13425e1346cd5873cb1abe607", PRIOR: "577724584361261327348db32b41dda18aaff8eed6877089a995b9055fa0e55c",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "core_input_provenance_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_MEAN2 = 0.848; LAM = 1e-2   # LAM unused here (inherited ridge helpers)
UP = [(k, l) for l in range(16) for k in ("attn", "mlp")]
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_min": 0.50, "c_frac": 0.60, "d_rho": 0.60, "e_mult": 1.5}
NULLS = {"b_max": 0.15, "c_frac": 0.30, "d_rho": 0.20, "e_mult": 0.8}
...
def main():
    ...
    bases = fit(m, fit_rows, LATE7 + UP); log(stage="fit_cov")
    core = pooled([bases[s] for s in LATE7], False); P = core["U"][:, :KM].to(DEV).float()
    ce0 = ce_of(m, ev); log(stage="baseline", ce=ce0)
    def core_mean(mu):
        mu = mu.to(DEV); return lambda w, x: w - ((w - mu) @ P) @ P.T
    ref = ce_of(m, ev, {s_: meanpatch(bases[s_]["mu"]) for s_ in LAST2}) - ce0; log(stage="ref_mean_16_17", ce_added=ref)
    single = {}; energy = {}
    for s_ in UP:
        nm = f"{s_[0]}{s_[1]}"
        single[nm] = ce_of(m, ev, {s_: core_mean(bases[s_]["mu"])}) - ce0
        C = bases[s_]["C"].to(DEV).float(); energy[nm] = float(torch.trace(P.T @ C @ P))     # centred core energy of the write (fit set)
        log(stage="single", site=nm, ce_added=round(single[nm], 4), core_energy=round(energy[nm], 3))
    groups = {"ALL32": UP, "ATTN16": [s_ for s_ in UP if s_[0] == "attn"], "MLP16": [s_ for s_ in UP if s_[0] == "mlp"]}
    grp = {g: ce_of(m, ev, {s_: core_mean(bases[s_]["mu"]) for s_ in sites}) - ce0 for g, sites in groups.items()}
    log(stage="groups", **{k: round(v, 4) for k, v in grp.items()})
    costs = torch.tensor([single[f"{s_[0]}{s_[1]}"] for s_ in UP]); ens = torch.tensor([energy[f"{s_[0]}{s_[1]}"] for s_ in UP])
    pos = costs.clamp(min=0); top3 = float(pos.sort(descending=True).values[:3].sum()); tot = float(pos.sum())
    rho = R.spearman(costs, ens)
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(ref - PRIOR_MEAN2) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_core_input_is_load_bearing': bool(grp["ALL32"] >= BARS["b_min"]),
        'pred_c_few_suppliers': bool(tot > 0 and top3 >= BARS["c_frac"] * tot),
        'pred_d_energy_names_the_suppliers': bool(rho >= BARS["d_rho"]),
        'pred_e_attention_supplies_more_than_mlp': bool(grp["ATTN16"] >= BARS["e_mult"] * grp["MLP16"]),
    }
    nulls = {"b_null_all32_le_.15": bool(grp["ALL32"] <= NULLS["b_max"]), "c_null_top3_le_.30": bool(tot > 0 and top3 <= NULLS["c_frac"] * tot),
             "d_null_rho_le_.20": bool(rho <= NULLS["d_rho"]), "e_null_attn_le_.8_mlp": bool(grp["ATTN16"] <= NULLS["e_mult"] * grp["MLP16"])}
    ...
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "mean_16_17": ref, "prior_mean_16_17": PRIOR_MEAN2},
           "single_site_core_mean": single, "core_energy_of_write": energy, "groups": grp, "ranking": order[:10], "top3_over_total": (top3 / tot if tot > 0 else None), "spearman_cost_vs_energy": rho,
           "core_spectrum": core["spec"],
           "price": {"gpu_doc_forwards": int(fit_rows.shape[0]) + int(ev.shape[0]) * (2 + 32 + 3), "cpu_doc_forwards": 0, "gpu_seconds": time.time() - started},
           "hashes": {str(k): (v if v and len(v) == 64 else R.sha256(k)) for k, v in HASHES.items()}, "script_sha256": R.sha256(SELF)}
    if smoke:
        print(json.dumps({k: out[k] for k in ("preds", "nulls", "instrument", "groups", "ranking", "spearman_cost_vs_energy", "price")}, indent=1)); return
    from receipt import dump
    dump(out, OUT)
    print(json.dumps({"preds": preds, "nulls": nulls, "groups": grp, "ranking": order[:10], "rho": rho}, indent=1))
