...
"""v663: my four board proposals have never been tested against EACH OTHER.

WHAT IS ON THE BOARD. Four behaviours await Codex: possessive_person_our_your, correlative_disjoint_either_not,
reciprocal_lenmatched, and possessive_person_your_their (flagged MARGINAL, sibling leak 0.0453 against a 0.05 bar).
Each separated inside ITS OWN family, against ITS OWN counted siblings. None has been measured against the other
three. That is a real gap in the proposal: if two of them share a direction, I would be proposing one behaviour twice,
which is exactly the duplication v661 measured at fifty percent for authored cells on a covered axis.
WHY THIS IS THE RUNG AND NOT MORE AUTHORING. v661 argued for authoring into uncovered stems. I mapped the thin stems
and every uncovered axis in them collides with a counted readout somewhere, so new authoring carries the same
duplication risk it was supposed to avoid. Checking the four I already intend to propose is worth more than a fifth
candidate with a coin-flip chance of being a duplicate.
WHY I EXPECT IT MOSTLY TO HOLD. The four sit on four different stems, and cross-stem fusion has not been seen in
this corpus -- fusion has always been within-axis (v631s number cells, v609s numerals). The exception is the two
possessive-person cells, our_your and your_their, which ARE on one axis; v661 already put those two in one family
and both separated, your_their only just. Adding correlative and reciprocal siblings adds leak paths and can only
push that 0.0453 upward, so your_their is the predicted breaker.
HOW IT READS. a TRUE: the four are four, the proposal stands as written and the marginal flag on your_their survives
a harder test than the one that produced it. a FALSE with b true: whichever pair fuses collapses to one behaviour and
I withdraw the other from the board BEFORE Codex acts on it, which is the entire point of running this first. a FALSE
with b false: the family is at fault, not the candidates, and nothing is withdrawn.
NOTHING IS COUNTED BY THIS FILE. The count stays 139 whatever this returns; this only decides what I propose.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
  pred_a_new_separable ALL FOUR proposals are separable at cross_max = 0.05 while sitting in one family with each
                       other -- k_new = 4, so a single fusion breaks it. Worked example: if our_your, either_not and
                       lenmatched are separable but your_theirs largest sibling leak is 0.061, the count of separable
                       NEW_MEMBERS is 3, 3 >= 4 is false, pred_a is FALSE.                            prior 45%
  pred_b_counted_kept  at least k_kept = 2 of the three counted members stay separable in the same run, so a null on
                       the candidates cannot be blamed on the family. Worked example: if possessive_gender and
                       correlative_or_and are separable and number_reflexive_myself_ourselves is not, kept = 2,
                       2 >= 2 is true, pred_b is TRUE.                                                prior 85%
  pred_c_row4_kept     at least k_d = 2 members of EVAL keep a family-arm C upper bound at or below c_ub_max = 0.01.
                       Worked example: if five of the seven evaluated members have arms.fam.C.ce_ub975 of 0.004 or
                       less and two are above 0.01, the count is 5, 5 >= 2 is true, pred_c is TRUE.   prior 80%
  pred_d_instrument    every EVAL member reproduces its parent battery extraction within instr_tol = 0.03. Worked
                       example: if reciprocal_lenmatched scored 0.886 in its parent receipt and the own arm here
                       returns 0.902, the gap is 0.016, 0.016 <= 0.03 holds; one member off by 0.04 makes pred_d
                       FALSE for the whole run.                                                       prior 85%
SCOPE. Four proposed candidates, three counted controls drawn from their three stems, one family. No counting.
Smoke: V663_SMOKE=<out.json> (CPU).
"""
...
OUT = ROOT / "circuits/followups/unit_proposal_cross_separability_v663_result.json"
...
FAMILIES = {
    "proposals": ['possessive_gender', 'correlative_or_and', 'number_reflexive_myself_ourselves', 'possessive_person_our_your', 'correlative_disjoint_either_not', 'reciprocal_lenmatched', 'possessive_person_your_their'],
}
COUNTED_LIST = ['possessive_gender', 'correlative_or_and', 'number_reflexive_myself_ourselves']
COUNTED = tuple(COUNTED_LIST)
EVAL = tuple(COUNTED_LIST) + tuple(['possessive_person_our_your', 'correlative_disjoint_either_not', 'reciprocal_lenmatched', 'possessive_person_your_their'])
...
def _parent_cdas(n):
    ...
    for path in (V655R, V639R, V659R, V633R, V597R, V599R, V601R, V581R, V575R, V365, V361, V343, V339, V357, V263, V265, V353, V293, V349, V327, V323, V319, V315, V311, V257, V255, V253, V251, V249, V247, V245, V243, V241, V239, V237, V235, V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
BARS = {"cross_max": 0.05, "keep": 0.8, "k_kept": 2, "ext_floor": 0.5, "c_ub_max": 0.01, "k_d": 2, "k_new": 4, "instr_tol": 0.03}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 20000, 640000


def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_proposal_cross_separability_v663", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V663_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V663_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_proposal_cross_separability_v663", "candidate_id": "corpus.unit_proposal_cross_separability_v663",
