#!/usr/bin/env python3
# BQGATE:10 body forwards;48 sequences lengths6-14;4 batched suffix arms;no fit;180sec.
"""pred_a native replay<=1e-5, block collapse<=.1, eachfamily native cue mean>=.1 with>=4/6positive;
...
STEM='REGIONAL_SOURCE_BLOCK_OOD_V1'
@torch.no_grad()
def main():
    ...
    for coverage,candidate_key,reference_key in [('all_sources','collapsed_all','secant_all')]:
        candidate=writes[candidate_key];swap=candidate[torch.arange(48,device='cuda')^1];swap=swap*candidate.norm(dim=-1,keepdim=True)/swap.norm(dim=-1,keepdim=True).clamp_min(1e-30)
        ...
        collapse=float((candidate-writes[reference_key]).norm()/writes[reference_key].norm());instrument=max(replays)<=1e-5 and collapse<=.1;capability=all(c['native_mean']>=.1 and c['native_positive']>=4 for c in cells)
        verdict={'pred_a':instrument and capability,'pred_b':instrument and capability and all(c['removal_mean_reduction']>=.1*c['native_mean'] and c['removal_positive']>=4 for c in cells),'pred_c':instrument and capability and all(c['removal_mean_reduction']-c['control_mean_reduction']>=.05*c['native_mean'] and c['unrelated_meanabs_reduction']<=.5*c['regional_meanabs_reduction'] for c in cells)}
