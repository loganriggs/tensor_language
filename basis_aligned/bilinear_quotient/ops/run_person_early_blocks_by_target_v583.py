#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_early_copies_into_the_context_carry pred_c_early_writes_into_the_cue_carry pred_d_targets_add pred_e_all_targets_match_the_value_swap
"""The person line's early blocks by target (v583). v580 / v581: on 43 natural person pairs blocks 0-4's values at the cue carry 0.216 of the myself - yourself
gap jointly but 0.089 singly -- nested through the cue's own residual. Here blocks 0-4's WRITES swapped by target: into the cue itself (SELF: self-attention
rebuilding the cue's residual, which later blocks and the readers copy), into every position after the cue (POST: copies into the following context, as
8.1 / 6.1 do for gender), and into all positions from the cue on (ALL).
PREDICTIONS (scored as written; failures preserved; priors from v580 / v581 and the gender copiers)
    pred_a_baseline_replays                   the unedited manual forward replays the model's myself - yourself margins within 1e-3
    pred_b_early_copies_into_the_context_carry  POST closes >= 0.10
    pred_c_early_writes_into_the_cue_carry    SELF closes >= 0.05
    pred_d_targets_add                        SELF + POST is within 0.08 of ALL
    pred_e_all_targets_match_the_value_swap   ALL is within 0.08 of 0.216 (the same heads' values at the cue)
PRICE (registered maximum): 2 batches x 4 passes = 8 forwards (43 pairs); 0 backwards; 0 fits. Bar <= 9.
"""
...
OUT = ROOT / "circuits/followups/person_early_blocks_by_target_v583_result.json"
CANDIDATE_ID = "person.early_blocks_by_target_v583"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, POST_MIN, SELF_MIN, ADD, EARLY_V580, UNIT = 1e-3, 0.10, 0.05, 0.08, 0.216, 701
...
FORWARDS_MAX = 9
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_early_copies_into_the_context_carry": ">= 0.10", "pred_c_early_writes_into_the_cue_carry": ">= 0.05", "pred_d_targets_add": "|SELF + POST - ALL| <= 0.08", "pred_e_all_targets_match_the_value_swap": "0.216 +- 0.08"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "post_min": POST_MIN, "self_min": SELF_MIN, "add": ADD, "early_v580": EARLY_V580, "unit": UNIT}}
    ...
    MODES = {m: ({}, (0,)) for m in ("SELF", "POST", "ALL")}; SETS_W = {m: {l: list(range(N_HEAD)) for l in range(0, 5)} for m in MODES}
    RULES = {"SELF": lambda rel, tofin: rel == 0, "POST": lambda rel, tofin: rel >= 1, "ALL": lambda rel, tofin: rel >= 0}; forwards = 0
    def logits_margin(x):
    ...
    print("person early blocks by target", {k: round(v, 3) for k, v in mc.items()}, "SELF + POST", round(mc["SELF"] + mc["POST"], 3))
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_early_copies_into_the_context_carry": mc["POST"] >= POST_MIN, "pred_c_early_writes_into_the_cue_carry": mc["SELF"] >= SELF_MIN,
                   "pred_d_targets_add": abs(mc["SELF"] + mc["POST"] - mc["ALL"]) <= ADD, "pred_e_all_targets_match_the_value_swap": abs(mc["ALL"] - EARLY_V580) <= ADD}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "person_early_blocks_by_target_result_v583", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
