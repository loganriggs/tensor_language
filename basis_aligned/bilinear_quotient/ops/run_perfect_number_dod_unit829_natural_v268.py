...
"""Perfect-number have/has DoD (v268): the shared plural detector 829 OUT OF THE PANEL on the verb-agreement line. v266 / v267: 829 leads MLP 8's write on 11.3's
have - has direction at the noun and its removal costs 1.9% of the panel margin. Here 829 is zeroed at the CUE noun of the 128 natural rows (FineWeb v100 + Pile v101, 64
each; cells plural-have, plural-has, singular-have, singular-has; congruent = plural-have and singular-has) and at all positions; damage = oriented drop of the label
margin on congruent rows; incongruent rows should shift toward the label if the unit carries the noun's number and nothing else. Null: 16 seeded random single units
zeroed at the cue. Readers will-would / who-which / night-day, gate move <= null mean + 0.25 x damage. (On the pronoun line 829 alone was plural-only on natural text, v198.)
PREDICTIONS (scored as written; failures preserved; priors from v198 / v267)
...
OUT = ROOT / "circuits/followups/perfect_number_dod_unit829_natural_v268_result.json"
SOURCES = {"fineweb": ROOT / "circuits/followups/perfect_number_dod_natural_rows_v100.json", "pile": ROOT / "circuits/followups/perfect_number_dod_pile_rows_v101.json"}
CANDIDATE_ID = "perfect_number.have_vs_has.dod_unit829_natural_v268"
LAYER, UNITS, CONGRUENT = 8, (829,), ("natural_plural_have", "natural_singular_has")
NULL_SEEDS = tuple(range(8301, 8317))
...
def main() -> None:
    ...
    for name, path in SOURCES.items():
        rs, sha, they, he = N.rows_from_receipt(path, "have", " have", " has", lambda r: f"natural_{r['cue']}_{r['label']}")
    ...
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_unit829_natural_result_v268", "candidate_id": CANDIDATE_ID, "plan": plan, "instrument_max_abs_error": instrument, "arms": arms, "null_damage_max": null_max, "null_moves": null_moves, "nulls": nulls, "gates": gates,
