"""Sealed new-construction Next-year/Last-year authority for rank-seven confirmation."""

from __future__ import annotations

import circuit_fast_screen_behaviour_spec as bs
import circuit_candidate_temporal_auxiliary_fresh_cues_v8 as previous


lex = previous.lex
_head = lambda index: lex._REPORTERS[(index + 79) % len(lex._REPORTERS)][1]
_alternate = lambda index: lex._REPORTERS[(index + 79) % len(lex._REPORTERS)][0]
_object_a = lambda index: lex._OBJECTS[(index + 223) % len(lex._OBJECTS)]
_object_b = lambda index: lex._OBJECTS[(index + 239) % len(lex._OBJECTS)]
_cue = lambda future: "Next year" if future else "Last year"

SPEC = bs.BehaviourSpec(
    task_id="temporal_auxiliary.will_vs_had.fresh_next_last_year_memo",
    vocabulary=(" will", " had"),
    generator_role="generate_ninth_fresh_temporal_auxiliary_panels",
    answer_role="score_jointly_tokenized_will_versus_had",
    a1=bs.Family(
        "ninth_fresh_bare_frame", "fresh_next_last_year_beside_swap",
        lambda index, future: f"{_cue(future)} the {_head(index)} beside the {_object_a(index)}",
    ),
    a2=bs.Family(
        "ninth_fresh_memo_frame", "fresh_memo_next_last_year_swap",
        lambda index, future: (
            f"The memo records: {_cue(future)} the {_head(index)} beyond the {_object_b(index)}"
        ),
    ),
    ...
    p_generator_role="ninth_fresh_head_noun_rewrite",
)
...
def validate_rows(rows, *, task_id=TASK_ID, groups=bs.DEFAULT_GROUPS, seed=20261101):
    ...
    if len(targets) != 2 * groups:
        raise bs.CandidateBankError("ninth fresh target population changed")
    if any(row[key] in old_text for row in targets for key in ("base_text", "donor_text")):
        raise bs.CandidateBankError("ninth fresh target text overlaps v8")
    for row in targets:
        ...
        if len(row["base_ids"]) != len(row["donor_ids"]) or len(differences) != 1:
            raise bs.CandidateBankError("ninth fresh pair is not aligned at exactly one cue token")
    return digest


def build_rows(task_id=TASK_ID, groups=bs.DEFAULT_GROUPS, seed=20261101):
