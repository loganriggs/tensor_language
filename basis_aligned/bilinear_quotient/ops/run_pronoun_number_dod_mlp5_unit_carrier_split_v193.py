...
"""Pronoun number they/he DoD (v193): CARRIER split of MLP-5 units 1036 and 2403 (v192: the two largest MLP-5 carriers of the number into MLP-6 unit
2483, 21% and 19%) at the block-5 input. Writers: embedding, attn / mlp totals of blocks 0-4, the nine heads of block 5. Same exact identity as v188 /
v191. Registered question: does the MLP-carried number chain bottom out in the noun token here (embedding-carried), or continue down the stack?
PREDICTIONS (scored as written; failures preserved; priors unsure)
    ...
    pred_b_mlp_stack_carries_050     mlp:01..mlp:04 together carry >= 0.50, both units
    pred_c_block5_heads_carry_little every block-5 head carries <= 0.10, both units
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_mlp5_unit_carrier_split_v193_result.json"
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_mlp5_unit_carrier_split_v193"
UNITS, LAYER, CLOSURE_TOL, STACK_MIN, HEAD_MAX, EMBED_MIN, BATCH = (1036, 2403), 5, 1e-3, 0.50, 0.10, 0.20, 32
FORWARDS_MAX = 5
...
def main() -> None:
    ...
    for unit in UNITS:
        ...
        predictions["pred_c_block5_heads_carry_little"] &= all(abs(cs[f"attnhead:05:{h}"]) <= HEAD_MAX for h in range(9)); predictions["pred_d_embedding_carries_020"] &= cs["embed"] >= EMBED_MIN
