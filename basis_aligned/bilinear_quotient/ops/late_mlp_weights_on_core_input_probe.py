#!/usr/bin/env python
"""late_mlp_weights_on_core_input_probe -- extraction check for §2718: feed mlp16/mlp17's OWN Left/Right/Down a restricted input (the real 16 core
coordinates + a filler for the other 1136 directions: fit-set mean / token-predicted / random-position) and measure how much of the blocks'
value their own algebra reproduces. No block weights are fitted. CUDA lane-1 script.
...
Preregistration: polynomial_causal/LATE_MLP_WEIGHTS_ON_CORE_INPUT_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_mlp_weights_on_core_input_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_mlp_weights_on_core_input_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_MLP_WEIGHTS_ON_CORE_INPUT_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_message_core_input_map_probe_results.json"   # §2718
OUT = ROOT / "late_mlp_weights_on_core_input_probe_results.json"
HASHES = {PREREG: "3ce4575fbb71b2f0487d346f3666b87843593a319d9c73687c307b6dcbb93edf", PRIOR: "0993eb5dc2089cd33efbf2f6c752293f72ae7427ba95d10f7c1286d809d0f94b",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_mlp_weights_on_core_input_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_MEAN2 = 0.848; PRIOR_ORACLE = 0.158; LAM = 1e-2
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_min": 0.60, "c_gain": 0.15, "d_mult": 0.90, "e_drop": 0.15}
NULLS = {"b_max": 0.30, "c_gain": 0.03, "d_mult": 0.60, "e_drop": 0.03}
...
def main():
    ...
    wte = m.transformer.wte
    def warm(mode, restrict):
        objs = {}
        for s_ in LAST2:
            o = WeightsArm(m, s_[1], fills[s_], mode, bases[s_]["mu"], P, restrict); o.mlp_wte = wte; objs[s_] = o
        def hook(idx):
    ...
    for name, mode, restrict in (("W_MEANFILL", "MEAN", False), ("W_TOKFILL", "TOK", False), ("W_TOKFILL_M", "TOK", True), ("W_RANDFILL", "RAND", False)):
        o, h = warm(mode, restrict); arms[name] = ce_of(m, ev, o, h) - ce0
    ...
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["MEAN"] - PRIOR_MEAN2) <= BARS["repro_tol"] and abs(arms["ORACLE_M"] - PRIOR_ORACLE) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_own_weights_on_core_plus_token': bool(rec["W_TOKFILL"] >= BARS["b_min"]),
        'pred_c_token_filler_beats_mean_filler': bool(rec["W_TOKFILL"] >= rec["W_MEANFILL"] + BARS["c_gain"]),
        'pred_d_output_lives_in_the_core_here_too': bool(rec["W_TOKFILL_M"] >= BARS["d_mult"] * rec["W_TOKFILL"]),
        'pred_e_realistic_wrong_filler_hurts': bool(rec["W_RANDFILL"] <= rec["W_TOKFILL"] - BARS["e_drop"]),
    }
    nulls = {"b_null_rec_tokfill_le_.30": bool(rec["W_TOKFILL"] <= NULLS["b_max"]), "c_null_gain_le_.03": bool(rec["W_TOKFILL"] <= rec["W_MEANFILL"] + NULLS["c_gain"]),
             "d_null_tokfillM_le_.6_tokfill": bool(rec["W_TOKFILL_M"] <= NULLS["d_mult"] * rec["W_TOKFILL"]), "e_null_randfill_ge_tokfill_minus_.03": bool(rec["W_RANDFILL"] >= rec["W_TOKFILL"] - NULLS["e_drop"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "mean_last2": arms["MEAN"], "prior_mean_last2": PRIOR_MEAN2, "oracle_m": arms["ORACLE_M"], "prior_oracle_m": PRIOR_ORACLE},
           "ce_added": arms, "recovery": rec, "core_spectrum": core["spec"],
           "price": {"gpu_doc_forwards": 3 * int(fit_rows.shape[0]) + int(ev.shape[0]) * 7, "cpu_doc_forwards": 0, "gpu_seconds": time.time() - started},
           "hashes": {str(k): (v if v and len(v) == 64 else R.sha256(k)) for k, v in HASHES.items()}, "script_sha256": R.sha256(SELF)}
    if smoke:
        print(json.dumps({k: out[k] for k in ("preds", "nulls", "instrument", "ce_added", "recovery", "price")}, indent=1)); return
    from receipt import dump
    dump(out, OUT)
    print(json.dumps({"preds": preds, "nulls": nulls, "ce_added": arms, "recovery": rec}, indent=1))
