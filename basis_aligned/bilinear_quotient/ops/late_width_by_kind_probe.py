#!/usr/bin/env python
"""late_width_by_kind_probe -- which late reads need the width: blocks 8-17's ATTENTION reads alone or MLP reads alone through
the bus frame U_8 at k in {768, 896} (the other kind and all early sites untouched), vs both kinds. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_late_mlp_reads_carry_the_width pred_c_late_attention_reads_cheap_at_768
#                     pred_d_kinds_are_subadditive pred_e_bus_896_under_p06
...
Preregistration: polynomial_causal/LATE_WIDTH_BY_KIND_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_width_by_kind_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_width_by_kind_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_WIDTH_BY_KIND_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_width_control_probe_results.json"   # §2771
OUT = ROOT / "late_width_by_kind_probe_results.json"
HASHES = {PREREG: "706b5b9918d91a99561beda0e318ce61e6823dc2aacb8d86271f6fe47b585b05", PRIOR: "2f19a636206b0dae2ba86942ca50e06d523a22da7761f4f501fa784860bf47e7",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_width_by_kind_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_SPLIT8_1024 = 0.0374; PRIOR_BUS_768 = 0.1636; LAM = 1e-2
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_ratio": 2.0, "c_max": 0.040, "d_ratio": 1.2, "e_max": 0.060}
NULLS = {"c_min": 0.100, "d_ratio": 2.0, "e_min": 0.120}
...
def main():
    ...
    def split8(k):
        ...
        return run(patch)
    def bus_kind(k, kinds):
        return run({s_: head(s_, k, U_8) for s_ in SET8 if s_[0] in kinds})
    arms = {"SPLIT8_1024": split8(1024), "BUS_768": bus_kind(768, ("attn", "mlp"))}
    for k in ([768] if smoke else [768, 896]):
        arms[f"LATE_ATTN_{k}"] = bus_kind(k, ("attn",)); arms[f"LATE_MLP_{k}"] = bus_kind(k, ("mlp",))
    if not smoke:
        arms["BUS_896"] = bus_kind(896, ("attn", "mlp"))
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    A, M, B = arms["LATE_ATTN_768"], arms["LATE_MLP_768"], arms["BUS_768"]
    summ = {"attn_768": A, "mlp_768": M, "both_768": B, "mlp_over_attn_768": (M / A if abs(A) > 1e-9 else float("inf")), "sum_over_both_768": ((A + M) / B if abs(B) > 1e-9 else float("inf")),
            "attn_896": arms.get("LATE_ATTN_896"), "mlp_896": arms.get("LATE_MLP_896"), "both_896": arms.get("BUS_896")}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(B - PRIOR_BUS_768) <= BARS["repro_tol"]) if not smoke else True
    b896 = arms.get("BUS_896", B)
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_late_mlp_reads_carry_the_width': bool(M >= BARS["b_ratio"] * A),
        'pred_c_late_attention_reads_cheap_at_768': bool(A <= BARS["c_max"]),
        'pred_d_kinds_are_subadditive': bool(A + M <= BARS["d_ratio"] * B),
        'pred_e_bus_896_under_p06': bool(b896 <= BARS["e_max"]),
    }
    nulls = {"b_null_attn_ge_mlp": bool(A >= M), "c_null_attn_ge_.10": bool(A >= NULLS["c_min"]),
             "d_null_sum_ge_2x_both": bool(A + M >= NULLS["d_ratio"] * B), "e_null_bus896_ge_.12": bool(b896 >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "blocks 8-17: attention reads only / MLP reads only / both through U_8 at k; early sites untouched",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "bus_768": B, "prior_bus_768": PRIOR_BUS_768},
