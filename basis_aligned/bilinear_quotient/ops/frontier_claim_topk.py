"""REGISTERED CLAIM: THE DYNAMIC-MLP PARETO POINT (rung 235) - S2304 becomes official.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2304 measured r64 + per-token
top-1152 MLPs at census +0.1015 / fresh +0.1018 / valid 4 - the compute-sparse point (quarter active
units per token, storage unchanged). Identical rebuild; claim bars registered.
REGISTERED PREDICTIONS (anchors: the S2304 receipt):
  (a) REPRODUCTION: |census - 0.1015| <= 0.015.
  (b) CERTIFICATES: valid >= 4.
  (c) REPRODUCTION: |L2_F - 0.1018| <= 0.015.
NULL: noise. PRICE: r64 storage + 4x compute sparsity in every swapped MLP. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: registered claim - dynamic-MLP point')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_claim_topk_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['cp_topk']=1152; SEL['qk_r']=64; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    pa=abs(agg-0.1015)<=0.015
    pb=nv>=4
    pc=abs(L2F-0.1018)<=0.015
    ...
    print(f"(a) |census {agg:+.4f} - 0.1015| <= 0.015: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 4; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |L2F {L2F:+.4f} - 0.1018| <= 0.015: {'HELD' if pc else 'FAILED'}")
