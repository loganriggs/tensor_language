# PER-SITE TABLE CHOICE -- the first run in this arc that would CHANGE the artefact.
#
# §1845 established that a FIT-ROW context table -- each site's output averaged over its occurrences in
# `skip80`, the corpus §1834's pipeline already streams -- uses no new data, stores exactly the same
# 5419 x D, is held out from all three eval roles, and takes mlp5 from +61.2pp to +17.7pp, recovering 92%
# of what the (non-deployable) eval-fitted mean achieves. It also established the improvement is NOT
# uniform: at mlp4 the context table is WORSE than the single-token forward, 20.3% -> -3.1%, and that
# inversion appears with both context tables so it is not an artefact of eval fitting.
#
# Two tables over the identical interface and storage, and which is better is SITE-DEPENDENT. That is a
# choice a compiler can make, decided by one held-out measurement per site.
#
# SCOPE: the choice is offered at the TWELVE early sites, attn and mlp at layers 1-6. §1834 prices every
# site from layer 7 up at 7.4pp or less, so the choice cannot matter much there, and restricting it keeps
# the second row bank to 12 sites rather than 36 -- a real constraint, since §1807 already peaked at 26.4
# GiB holding two full banks. Stated as a scope, not hidden.
#
# PROTOCOL: the per-site choice is made from single-site arms on skip7000 and the mixed program is then
# scored on skip11000 and skip1200, which are HELD OUT from the choice as well as from the fit rows.
#
...
#   pred_a THE CONTEXT TABLE HELPS THE WHOLE PROGRAM: EARLY_FIT -- all 36 sites compiled, layers 1-6
#          using the fit-context table -- exceeds ALL_L1's top-1 by at least 2pp on skip7000. If TRUE the
#          single-site gains compose and the deployed compiler is simply using the wrong table at its
#          early sites. If FALSE they do NOT compose, which §1832's super-additivity makes a live
#          possibility, and §1845's improvement would be visible one site at a time and worth nothing to
#          a whole program -- a genuinely important negative for the compiler, not just for me.
#   pred_b AND THE PER-SITE CHOICE BEATS BOTH UNIFORM PROGRAMS: MIXED exceeds the better of ALL_L1 and
#          EARLY_FIT by at least 1pp on BOTH held-out roles, with the choice made on skip7000 only. If
#          FALSE the per-site choice does not transfer across roles -- it would then be fitting the
#          selection role, and one uniform table would be the honest recommendation.
#   pred_c AND THE CHOICE IS GENUINELY MIXED: at least 3 of the 12 early sites prefer length-1 and at
#          least 3 prefer the context table. If FALSE one table wins nearly everywhere, there is no
#          per-site choice worth making, and the recommendation collapses to "use that table" -- simpler,
#          and it would make pred_b moot however it scored.
#   pred_d CONTROLS, cross-run per LESSON 42: B0 reproduces §1829's PUBLISHED 64.8%; ALL_L1 reproduces
#          §1789's PUBLISHED full-rank top-1 (13.55 / 14.25 / 13.64%); the length-1 single-site arms
#          reproduce §1834's PUBLISHED mlp5 3.6% and mlp4 20.3%; FIT_mlp5 reproduces §1845's PUBLISHED
#          47.1% within 0.03. Coverage 5419 of 50257.
...
OUT = PT + 'ops/per_site_table_choice_results.json'
...
for _st in EARLY:
    ...
    ARMS[f'FIT_{_st[0]}{_st[1]}'] = B0_SITES + [_st]
# §1834's PUBLISHED length-1 single-site sequential gap fractions, skip7000
S1834_L1_GAPFRAC = {'L1_mlp5': 0.0357, 'L1_mlp4': 0.2026}
# §1845's PUBLISHED fit-context single-site sequential gap fraction, skip7000
S1845_FIT_MLP5 = 0.471
CHOOSE_ROLE = 'skip7000'
HELDOUT = ('skip11000', 'skip1200')
...
def main():
    ...
    print(f'PER-SITE TABLE CHOICE | 12 early sites, length-1 vs fit-context, mixed program | '
    ...
    for name, botup in ARMS.items():
        ...
        if name.startswith('FIT_'):
            ov = {botup[-1]: fith[botup[-1]]}
        run_g(f'{name}_raw', botup, {}, ov)
        run_g(f'{name}_seq', botup, gseq[name], ov)
    # ---- the choice, made on the CHOOSE_ROLE single-site arms only, then three whole-program arms
    gapv0 = {e: res[e]['live_model']['top1'] - res[e]['all_substituted']['top1'] for e in evs}

    def gf(e, k2):
        return (res[e][k2]['top1'] - res[e]['all_substituted']['top1']) / gapv0[e]
    prefer = {}
    for st in EARLY:
        nm2 = f'{st[0]}{st[1]}'
        prefer[nm2] = ('FIT' if gf(CHOOSE_ROLE, f'FIT_{nm2}_seq') > gf(CHOOSE_ROLE, f'L1_{nm2}_seq')
                       else 'L1')
    nfit = sum(1 for v in prefer.values() if v == 'FIT')
    print(f'\n  per-site preference on {CHOOSE_ROLE}: ' + '  '.join(
        f'{k3}:{v}' for k3, v in prefer.items()) + f'   ({nfit} FIT, {12 - nfit} L1)', flush=True)

    allsites_g = seq_gains(sites)
    fitov = {st: fith[st] for st in EARLY}
    mixov = {st: fith[st] for st in EARLY if prefer[f'{st[0]}{st[1]}'] == 'FIT'}
    run_g('ALL_L1', sites, allsites_g, {})
    run_g('EARLY_FIT', sites, allsites_g, fitov)
    run_g('MIXED', sites, allsites_g, mixov)
    ...
    prog = {nm3: {e: res[e][nm3]['top1'] for e in roles}
            for nm3 in ('ALL_L1', 'EARLY_FIT', 'MIXED')}
    b0 = seq['B0']
    lift_a = prog['EARLY_FIT'][PICK_ROLE] - prog['ALL_L1'][PICK_ROLE]
    lift_b = {e: prog['MIXED'][e] - max(prog['ALL_L1'][e], prog['EARLY_FIT'][e]) for e in HELDOUT}
    nfit2 = sum(1 for v in prefer.values() if v == 'FIT')
    pa = lift_a >= 0.02
    pb = all(v >= 0.01 for v in lift_b.values())
    pc = nfit2 >= 3 and (12 - nfit2) >= 3
    pd = (abs(b0 - S1829_B0_GAPFRAC) <= 0.03
          and all(abs(seq[k2] - v) <= 0.03 for k2, v in S1834_L1_GAPFRAC.items())
          and abs(seq['FIT_mlp5'] - S1845_FIT_MLP5) <= 0.03
          and all(abs(prog['ALL_L1'][e] - S1789_FULLRANK_TOP1_PP[e]) <= 0.001 for e in roles)
          and all(abs(res[e]['live_model']['top1'] - S1789_LIVE_TOP1_PP[e]) <= 0.001
                  for e in roles)
          and NFULL == NCOV)

    print(f'\n  single-site gap fraction, {PICK_ROLE} (B0 {b0:.1%}):', flush=True)
    for st in EARLY:
        nm2 = f'{st[0]}{st[1]}'
        print(f'    {nm2:6s} length-1 {seq[f"L1_{nm2}"]:7.1%}   fit-context {seq[f"FIT_{nm2}"]:7.1%}'
              f'   -> {prefer[nm2]}', flush=True)
    print(f'\n  WHOLE-PROGRAM top-1 (all 36 sites compiled):', flush=True)
    for nm3 in ('ALL_L1', 'EARLY_FIT', 'MIXED'):
        print(f'    {nm3:10s} ' + '  '.join(f'{e} {prog[nm3][e]:6.2%}' for e in roles), flush=True)
    print(f'\n  the CONTEXT table helps the whole program (>=2pp on {PICK_ROLE}) -> {pa}  '
          f'EARLY_FIT {prog["EARLY_FIT"][PICK_ROLE]:.2%} vs ALL_L1 '
          f'{prog["ALL_L1"][PICK_ROLE]:.2%}, lift {lift_a:+.2%}', flush=True)
    print(f'  and the PER-SITE choice beats both, on HELD-OUT roles -> {pb}  ' + '  '.join(
        f'{e} {lift_b[e]:+.2%}' for e in HELDOUT), flush=True)
    print(f'  and the choice is genuinely MIXED (>=3 each way) -> {pc}  '
          f'{nfit2} FIT, {12 - nfit2} L1', flush=True)
    print(f'  B0 §1829, ALL_L1 §1789, single-site arms §1834/§1845 -> control {pd}', flush=True)

    json.dump({'run': 'per_site_table_choice', 'role': PICK_ROLE,
               'choose_role': CHOOSE_ROLE, 'heldout': list(HELDOUT),
               'preference': prefer, 'n_fit': nfit2,
               'single_site_gapfrac': seq, 'whole_program_top1': prog,
               'lift_context_vs_l1': lift_a, 'lift_mixed_vs_best': lift_b,
               'rows_changed': ndiff, 'gap_fraction': frac,
               'predictions': {'pred_a_context_helps_program': bool(pa),
                               'pred_b_per_site_beats_uniform': bool(pb),
                               'pred_c_choice_is_mixed': bool(pc),
