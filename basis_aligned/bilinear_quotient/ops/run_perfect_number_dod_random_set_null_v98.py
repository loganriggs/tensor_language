...
"""Perfect have/has DoD (v98): matched-count random four-head-set null on the v97 fresh rows (SIMPLE). Body: v72's with this line's set
and rows; 16 random quadruples from the 158 other heads, each removed along its own weight-only have−has direction.
...
fraction within 0.66 +/- 0.05 (replay; the scored key keeps v72's literal name).
...
PREDICTIONS = {"pred_a_instrument_replays_native": "<= 1e-4", "pred_b_set_beats_every_random_quadruple": "> random max",
               "pred_c_random_quadruples_are_not_live": "none live", "pred_d_set_fraction_within_band_of_v71": "0.66 +/- 0.05 (this line's band)"}
v72.OUT = v72.ROOT / "circuits/followups/perfect_number_dod_random_set_null_v98_result.json"
v72.CANDIDATE_ID = "perfect_number.have_vs_has.dod_random_set_null_v98"
v72.EXCLUDED = set(line.HEADS)
v72.POOL = [(l, h) for l in range(18) for h in range(9) if (l, h) not in v72.EXCLUDED]
v72.SETS = [tuple(sorted(v72.random.Random(2026_09_18_98 + s).sample(v72.POOL, 4))) for s in range(16)]
v72.V71_FRACTION, v72.FORWARDS_MAX = 0.66, 60
