"""DO THE BACKUP READERS COVER THE SAME POSITIONS (rung 83).

CONVENTION (S2135): d16/d14 = per-position CE(zeroed arm) - CE(base) on the skip-1416 config (bases excluded);
positive = that attention was helping there. ind positions on FR.

S2171: the pair are independent duplicates (map non-transfer). Complementary question: do they help at the SAME
positions (interchangeable coverage - consistent with S2165 backup) or PARTITION the repeats (each owns some)?
NULL: partition (rho <= 0).

REGISTERED PREDICTIONS (arm-named):
  (a) SAME POSITIONS: binned Spearman rho(d16, d14) >= 0.5 on ind positions (bins of 200 ordered by d16).
  (b) CONCENTRATION: ind positions in the top half of d16 carry >= 0.6 of d14's summed ind help.
  (c) REPRODUCTION: the base arm reproduces S2156 per-window (median |delta| <= 0.005).
  TRIPWIRE: each zeroed arm must differ from base.

Writes backup_positions_results.json. Self-reviewed. RUNG-71 HEADER FOLLOWS.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; per-position coverage overlap of the backup readers (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'backup_positions_results.json'
...
if __name__=='__main__':
    ...
    arms={}
    for tag,za in (('base',()),('z16',(16,)),('z14',(14,))):
        SEL['zeroattn']=za
        print(f'ARM {tag}: attn zeroed at {za}',flush=True)
    ...
    d16=(arms['z16']['cev']-arms['base']['cev'])
    d14=(arms['z14']['cev']-arms['base']['cev'])
    IND=CLS.index('ind')
    im=(clsflat==IND).nonzero().squeeze(1)
    order=d16[im].argsort()
    imo=im[order]
    B=200
    b16=[];b14=[]
    for k0 in range(0,len(imo)-B+1,B):
        ii=imo[k0:k0+B]
        b16.append(float(d16[ii].mean())); b14.append(float(d14[ii].mean()))
    def rank(v):
        o=sorted(range(len(v)),key=lambda i2:v[i2]); r7=[0]*len(v)
        for pos,i2 in enumerate(o): r7[i2]=pos
        return r7
    n7=len(b16)
    rho=1-6*sum((a-b)**2 for a,b in zip(rank(b16),rank(b14)))/(n7*(n7*n7-1)) if n7>2 else 0.0
    med16=float(d16[im].median())
    top=im[d16[im]>med16]
    tshare=float(d14[top].sum())/max(float(d14[im].sum()),1e-9)
    pa=rho>=0.5
    pb=tshare>=0.6
    pc=stt.median(rp)<=0.005
    res={'base':{'L2_F':arms['base']['r']['L2_F']},'n_ind':int(len(im)),'n_bins':n7,
         'spearman_binned':round(rho,3),'top_half_share':round(tshare,4),
         'convention':'d = per-position CE(zeroed) - CE(base) on skip-1416; positive = that attention was helping',
         'pred_a_same_positions':bool(pa),'pred_b_concentrated':bool(pb),'pred_c_reproduces_s2156':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'ind n={len(im)}; binned Spearman(d16,d14) {rho:+.3f}; top-half share {tshare:.3f}')
