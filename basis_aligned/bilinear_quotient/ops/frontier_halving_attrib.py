...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_norm_ksweep_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; halving attribution arms x eight windows (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_halving_attrib_results.json'
...
if __name__=='__main__':
    ...
    print('ARM 1/3: norm-2304 everywhere (S312 reproduction)',flush=True)
    r_norm=main()
    SEL['mode']='norm'; SEL['K']=1152; SEL['K69']=2304
    print('ARM 2/3: halve mlp4/mlp5 only',flush=True)
    r_45=main()
    SEL['mode']='norm'; SEL['K']=2304; SEL['K69']=1152
    print('ARM 3/3: halve c6-c9 only',flush=True)
    r_69=main()
    import statistics as stt
    d45=[round(a-b,4) for a,b in zip(r_45['fresh8'],r_norm['fresh8'])]
    d69=[round(a-b,4) for a,b in zip(r_69['fresh8'],r_norm['fresh8'])]
    m45=stt.median(d45); m69=stt.median(d69)
    pa=abs(m45+m69-0.0292)<=0.01
    pb=m45>=m69
    pc=abs(r_norm['L2_F']-2.6735)<=0.01
    res={'norm2304':r_norm,'halve45':r_45,'halve69':r_69,
         'per_window_damage_halve45':d45,'per_window_damage_halve69':d69,
         'convention':'L2 = CE above the real model; lower is better; positive entries = damage added',
         'pred_a_additive':bool(pa),'pred_b_mlp45_pay_more':bool(pb),'pred_c_reproduces_s312':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'damage added, halve mlp4/5 only, per window: {d45}')
    print(f'damage added, halve c6-c9 only, per window: {d69}')
    print(f"(a) additivity |{m45:+.4f} + {m69:+.4f} - 0.0292| = {abs(m45+m69-0.0292):.4f} <= 0.01: {'HELD' if pa else 'FAILED'}")
    print(f"(b) mlp4/5 pay more: {m45:+.4f} >= {m69:+.4f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) norm-2304 L2_F {r_norm['L2_F']} vs 2.6735 (tol 0.01): {'HELD' if pc else 'FAILED'}")
