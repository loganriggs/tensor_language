#!/usr/bin/env python
"""whole_model_width_program_probe -- the complete width program of bilin18 (§2749): 22 early sublayers on own k-cores + 14 late
sublayers on one shared k-core, at k = 768 / 896 / 1024, with and without the §2748 BUS routing of the late out-of-core writes to the
readout. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_whole_model_1024 pred_c_whole_model_896 pred_d_composition_small_at_1024
#                     pred_e_bus_holds_in_the_whole_program
...
Preregistration: polynomial_causal/WHOLE_MODEL_WIDTH_PROGRAM_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/whole_model_width_program_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("whole_model_width_program_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "WHOLE_MODEL_WIDTH_PROGRAM_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "early_stack_width_map_probe_results.json"   # §2749
OUT = ROOT / "whole_model_width_program_probe_results.json"
HASHES = {PREREG: "a062e7956d7cac75dfcef1a57c9a72a3349e34e1151ddf807ed491771432a9b6", PRIOR: "745f13077b2292b8913e091938c2f496f991e691ecd7e7fdf746a0470df0368a",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "whole_model_width_program_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_ALL36_768 = 0.197; PRIOR_LATE14_JOINT_1024 = 0.023; PRIOR_EARLY22_OWN_1024 = 0.0084; LAM = 1e-2
K = 768; KS = [768, 896, 1024]; KB = [768, 1024]; EARLY = list(range(0, 11)); LATE = list(range(11, 18))
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "repro_tol_small": 0.015, "b_max": 0.05, "c_max": 0.12, "d_max": 0.015, "e_max": 0.02}
NULLS = {"b_min": 0.12, "c_min": 0.25, "d_min": 0.05, "e_min": 0.06}
...
def main():
    ...
    def whole(k, bus=False):
        patch = {s_: head(s_, k) for s_ in ESITES}; buf = Buf()
        for s_ in LSITES:
            r_ = head(s_, k, U_late); patch[s_] = Both(r_, Route(s_, k, U_late, "readout", buf)) if bus else r_
        if bus:
            patch[FINAL] = FinalAdd(buf, +1.0)
        return run(patch, (buf,))
    arms = {"EARLY22_OWN_1024": run({s_: head(s_, 1024) for s_ in ESITES}), "LATE14_JOINT_1024": run({s_: head(s_, 1024, U_late) for s_ in LSITES})}
    for k in KS:
        arms[f"ALL36_{k}"] = whole(k)
    for k in KB:
        arms[f"ALL36_BUS_{k}"] = whole(k, bus=True)
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    gaps = {"composition_1024": arms["ALL36_1024"] - arms["EARLY22_OWN_1024"] - arms["LATE14_JOINT_1024"], "bus_minus_plain_768": arms["ALL36_BUS_768"] - arms["ALL36_768"], "bus_minus_plain_1024": arms["ALL36_BUS_1024"] - arms["ALL36_1024"]}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["ALL36_768"] - PRIOR_ALL36_768) <= BARS["repro_tol"] and abs(arms["LATE14_JOINT_1024"] - PRIOR_LATE14_JOINT_1024) <= BARS["repro_tol_small"] and abs(arms["EARLY22_OWN_1024"] - PRIOR_EARLY22_OWN_1024) <= BARS["repro_tol_small"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_whole_model_1024': bool(arms["ALL36_1024"] <= BARS["b_max"]),
        'pred_c_whole_model_896': bool(arms["ALL36_896"] <= BARS["c_max"]),
        'pred_d_composition_small_at_1024': bool(gaps["composition_1024"] <= BARS["d_max"]),
        'pred_e_bus_holds_in_the_whole_program': bool(abs(gaps["bus_minus_plain_768"]) <= BARS["e_max"]),
    }
    nulls = {"b_null_ge_.12": bool(arms["ALL36_1024"] >= NULLS["b_min"]), "c_null_ge_.25": bool(arms["ALL36_896"] >= NULLS["c_min"]),
             "d_null_ge_.05": bool(gaps["composition_1024"] >= NULLS["d_min"]), "e_null_ge_.06": bool(abs(gaps["bus_minus_plain_768"]) >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "ks": KS, "ks_bus": KB,
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "all36_768": arms["ALL36_768"], "prior_all36_768": PRIOR_ALL36_768, "late14_joint_1024": arms["LATE14_JOINT_1024"], "prior_late14_joint_1024": PRIOR_LATE14_JOINT_1024, "early22_own_1024": arms["EARLY22_OWN_1024"], "prior_early22_own_1024": PRIOR_EARLY22_OWN_1024},
