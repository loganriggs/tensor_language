"""CP-VS-TABLE AT m0 (rung 110): the grammar test at the certificate-killer site.

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows; LOWER IS BETTER.
S2200/S2202: m0's table alone breaks 61/62 certificates at +0.2485 and both repair branches died there. Here
m0 is replaced SINGLE-SITE by its own top-K bilinear units (norm importance, weights-only, NO fitting rows),
K in {288, 576, 1152, 2304, 4608}, with census certificates per arm.
Anchor (S2198): the m0 table costs +0.2485, valid 1/62, and stores ~57.9M f16 values; CP-K stores K x 3456.

REGISTERED PREDICTIONS (arms named agg(K), valid(K)):
  (a) GRAMMAR BEATS TABLE: agg(576) <= 0.5 x 0.2485 = 0.1243 - at 29x fewer stored values.
...
PRICE: CP-576 at m0 = 1,990,656 values vs table ~57.9M - a 29x cheaper front replacement IF it wins.
Tripwire: INSTRUMENT FAIL if all K arms identical (pstdev < 0.005). Self-reviewed."""
import json, sys, time, os
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
if os.environ.get('BQLIB_DRYRUN')=='1':
    _bq='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
    _need=['frontier_m0_repair_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: CP-vs-table at m0')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_m0_cp_results.json'
...
def main():
    ...
    if SEL.get('m0_cp'):
        ...
        for _K in (288,576,1152,2304,4608):
            _kp=_ord[:_K]
            S['c0K']=('cp',0,_L[_kp].contiguous(),_R[_kp].contiguous(),
                      _D[:,_kp].contiguous(),_db)
            _arms[f'K_{_K}']=evalV(_ROWS,_ROWS.shape[0],['c0K'],ML).detach().cpu()
            print(f'  m0 CP K={_K} done',flush=True)
        del cur['clsmap']
        SEL['front_result']=_arms
...
if __name__=='__main__':
    ...
    TABLE_ANCHOR=json.load(open(PT+'frontier_front_prefix_results.json'))['arms']['prefix_m0E']['agg']
    SEL['mode']='norm'; SEL['K']=4608; SEL['K69']=4608; SEL['K69MAP']={}
    SEL['skipset']=tuple(range(10,18)); SEL['motif_off']=tuple(range(2,10))
    SEL['m0_cp']=True; SEL['ext_rows']=CROWS
    print('ARMS: m0 single-site CP top-K, K in 288/576/1152/2304/4608 - census rows',flush=True)
    main()
    A=SEL['front_result']
    out={}
    ...
    res={'arms':out,'table_anchor_prefix_m0E':TABLE_ANCHOR,
         'values_cp576':576*3456,'values_table':50257*1152,
         'convention':'per-position dCE = CE(single m0 CP-K, weights-only) - CE(real model) on census rows; lower is better',
