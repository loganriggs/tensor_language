...
PLAN=dict(bank_widths=[128,512],root_width=128,optimizers=['adam','muon'],seeds=[0,1],steps=300,batch_size=256,lr=.05,calibration_queries=2048,evaluation_queries=8192,function_queries=256,native_forwards=0)
def main():
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print(json.dumps(PLAN));return
 import torch
 from disk_guard import guard_torch_save
 sys.path.insert(0,str(P));from implicit_quartic import entries
 torch.set_num_threads(4);torch.backends.cuda.matmul.allow_tf32=False;out=P/'NATIVE_STOCHASTIC_QUARTIC_V1.json';assert not out.exists();start=time.perf_counter()
 ...
 for k,optname,seed in itertools.product(PLAN['bank_widths'],PLAN['optimizers'],PLAN['seeds']):
  torch.manual_seed(seed);r=128;C=torch.nn.Parameter(torch.randn(o,r,device='cuda')*.1/math.sqrt(o*r));L=torch.nn.Parameter(torch.randn(r,k,device='cuda'));R=torch.nn.Parameter(torch.randn(r,k,device='cuda'));A=torch.nn.Parameter(torch.randn(k,d,device='cuda'));B=torch.nn.Parameter(torch.randn(k,d,device='cuda'));D=torch.eye(k,device='cuda');parameters=[C,L,R,A,B];opt=torch.optim.Adam(parameters,lr=.05) if optname=='adam' else torch.optim.Muon(parameters,lr=.05,weight_decay=0.,adjust_lr_fn='match_rms_adamw');train_gen=torch.Generator(device='cuda');train_gen.manual_seed(17000+seed);history=[];finite=True;t0=time.perf_counter()
  def student():return [C,L/L.norm(dim=1,keepdim=True),R/R.norm(dim=1,keepdim=True),D,A/A.norm(dim=1,keepdim=True)*math.sqrt(d),B/B.norm(dim=1,keepdim=True)*math.sqrt(d)]
 ...
 guard_torch_save(dict(students=saved,teacher_scale=scale.cpu(),factor_order_without_identity=['C','L2','R2','L1','R1']),str(P/'NATIVE_STOCHASTIC_QUARTIC_V1.pt'));out.write_text(json.dumps(result,indent=2)+'\n')
