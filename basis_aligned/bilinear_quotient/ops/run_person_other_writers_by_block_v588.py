#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_early_singles_add pred_c_mid_singles_add pred_d_a_block_carries pred_e_block_4_writes
"""The other writers into the answer by block, person line (v588). v587: besides the four readers (0.412), blocks 0-7 write 0.216 of the myself - yourself gap
straight into the answer and blocks 9-12 (without 10.5) 0.220. Here those blocks singly: 0 ... 7, and 9, 10 (without 10.5), 11, 12.
PREDICTIONS (scored as written; failures preserved; priors from v587)
    pred_a_baseline_replays   the unedited manual forward replays the model's myself - yourself margins within 1e-3
    pred_b_early_singles_add  blocks 0-7 singly sum to within 0.08 of 0.216
    pred_c_mid_singles_add    blocks 9-12 singly sum to within 0.08 of 0.220
    pred_d_a_block_carries    the largest single closes >= 0.06
    pred_e_block_4_writes     block 4 (the copier block) closes >= 0.04. Prior: unsure
PRICE (registered maximum): 2 batches x 13 passes = 26 forwards (43 pairs); 0 backwards; 0 fits. Bar <= 27.
"""
...
OUT = ROOT / "circuits/followups/person_other_writers_by_block_v588_result.json"
CANDIDATE_ID = "person.other_writers_by_block_v588"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, EARLY_V587, MID_V587, ADD, TOP_MIN, B4_MIN, UNIT = 1e-3, 0.216, 0.220, 0.08, 0.06, 0.04, 701
...
FORWARDS_MAX = 27
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_early_singles_add": "0.216 +- 0.08", "pred_c_mid_singles_add": "0.220 +- 0.08", "pred_d_a_block_carries": ">= 0.06", "pred_e_block_4_writes": ">= 0.04"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "early_v587": EARLY_V587, "mid_v587": MID_V587, "add": ADD, "top_min": TOP_MIN, "b4_min": B4_MIN, "unit": UNIT}}
    ...
    SETS_W = {f"B{l}": grp(l, l) for l in list(range(0, 8)) + [9, 10, 11, 12]}; MODES = {m: ({}, (0,)) for m in SETS_W}; forwards = 0
    def logits_margin(x):
    ...
    early = sum(mc[f"B{l}"] for l in range(0, 8)); mid = sum(mc[f"B{l}"] for l in (9, 10, 11, 12)); top = max(mc, key=lambda k: abs(mc[k]))
    print("other writers by block", {k: round(v, 3) for k, v in mc.items()}, "early sum", round(early, 3), "mid sum", round(mid, 3), "top", top)
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_early_singles_add": abs(early - EARLY_V587) <= ADD, "pred_c_mid_singles_add": abs(mid - MID_V587) <= ADD,
                   "pred_d_a_block_carries": abs(mc[top]) >= TOP_MIN, "pred_e_block_4_writes": mc["B4"] >= B4_MIN}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "person_other_writers_by_block_result_v588", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
