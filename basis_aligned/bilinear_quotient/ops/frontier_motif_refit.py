"""MOTIF-ALPHA REFIT INSIDE THE FRONTIER CONFIG (rung 126): the cheapest possible repair of the eaten 0.9.

CONVENTION (S2135): frontier L2 and census numbers are CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER.
S2216/S2217: the new frontier carries a ~0.9-census front x downstream interaction; the motif heads' 76
scalar gains were fit on REAL streams and never refit. Here, inside the exact rung-123 build, the alphas are
refit by the same projection formula on the CP-front stream (fit rows CA:CB), BEFORE the tail dictionaries
are refit - so the whole downstream conditions on the swapped front. Everything else identical to rung 123.

REGISTERED PREDICTIONS (anchors: rung-123 receipt L2_F +2.0553, census +2.1359):
  (a) AGGREGATE RECOVERY: census agg <= 1.85 (>= 0.28 of the eaten interaction recovered by 76 scalars).
  (b) FRONTIER MOVES AGAIN: L2_F fresh <= 1.95.
  (c) SANITY: tail-attn increment in [0.25, 0.60] AND census agg >= 1.0.
NULL: alpha is frame-insensitive (a projection ratio of z onto v barely moves under drift): recovery < 0.05.
PRICE: none (the same 76 stored scalars, different values). Tripwire: INSTRUMENT FAIL if L2CF missing.
Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: motif-alpha refit in frontier config')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_motif_refit_results.json'
...
def main():
    ...
    for li in range(2,10):
        ...
        caps[li]=None
    if SEL.get('motif_refit'):
        _co={li:{'x':[],'v1':[]} for li in range(2,10)}
        hs=install(cfgF)
        for _li in range(2,10):
            def _mkc(li=_li):
                def h(mo_,args,out):
                    _co[li]['x'].append(args[0].detach())
                    _co[li]['v1'].append((args[1] if args[1] is not None
                                          else out[1]).detach())
                return h
            hs.append(m.transformer.h[_li].attn.register_forward_hook(_mkc()))
        ...
        for _li in range(2,10):
            ...
            _n=torch.zeros(9,device=DEV); _d=torch.zeros(9,device=DEV)
            _ns=torch.zeros(9,device=DEV); _ds=torch.zeros(9,device=DEV)
            for _X2,_v1 in zip(_co[_li]['x'],_co[_li]['v1']):
                _z,_vm=head_z(_at,_X2,_v1)
                _vp=torch.zeros_like(_vm); _vp[:,1:]=_vm[:,:-1]
                _vp=_vp.permute(0,2,1,3); _vs=_vm.permute(0,2,1,3)
                _n+=(_z*_vp).sum((0,2,3)); _d+=(_vp*_vp).sum((0,2,3))
                _ns+=(_z*_vs).sum((0,2,3)); _ds+=(_vs*_vs).sum((0,2,3))
            ALPHA[_li]=(_n/_d.clamp_min(1e-9),_ns/_ds.clamp_min(1e-9))
            _co[_li]=None
        print('  motif alphas refit under the CP-front stream',flush=True)
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['motif_refit']=True
    print('ARM: rung-123 build + motif-alpha refit under the CP front',flush=True)
    main()
    ...
    pa=agg<=1.85
    pb=L2F<=1.95
    pc=(0.25<=inc<=0.60) and agg>=1.0
    res={'L2F_fresh':round(L2F,4),'L1F':round(L1F,4),'increment':round(inc,4),'census_agg':round(agg,4),
         'anchor_rung123':{'L2F':ANCH['L2F_fresh'],'census':ANCH['census_agg']},
         'convention':'CE added above the real model; LOWER IS BETTER',
         'pred_a_aggregate_recovery':bool(pa),'pred_b_frontier_moves':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"L2 fresh {L2F:+.4f} (was +2.0553); census {agg:+.4f} (was +2.1359); increment {inc:+.4f}")
    print(f"(a) census {agg:+.4f} <= 1.85: {'HELD' if pa else 'FAILED'}")
    print(f"(b) L2F {L2F:+.4f} <= 1.95: {'HELD' if pb else 'FAILED'}")
    print(f"(c) increment in [0.25, 0.60] and census >= 1.0: {'HELD' if pc else 'FAILED'}")
