...
"""Pronoun number they/he DoD (v203): MLP 8 at unit grain at the VERB position (noun + 1: lost / wanted / bought). v82: the verb-position state 9.6 reads
is 51% MLP 8; v169b: zeroing {829, 953, 1030} at all positions removes 11.8% of the margin against 5.3% at the noun only, so the state is re-written
at the verb. v168's census (reader = 9.6's direction, exact per unit) at the verb: are the same two detectors firing there (the plural feature copied
to the verb by attention and re-detected), or different units?
PREDICTIONS (scored as written; failures preserved; priors unsure)
    ...
    pred_b_detectors_lead_at_verb         829 and 953 are both among the top-3 |pooled| units at the verb
    pred_c_top_10_units_carry_half        top-10 units carry >= 0.50 of the pooled contrast
    pred_d_verb_top50_overlaps_noun_top50 Jaccard(verb top-50, v168 noun top-50) >= 1/3
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_mlp8_unit_census_verb_v203_result.json"
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_mlp8_unit_census_verb_v203"
LAYER = 8
CLOSURE_TOL, TOP50_MIN, TOP200_MIN, JACCARD_MIN = 1e-3, 0.50, 0.80, 1 / 3
FORWARDS_MAX = 4
PREDICTIONS = {"pred_a_unit_closure": "<= 1e-3", "pred_b_detectors_lead_at_verb": "829, 953 in top-3", "pred_c_top_10_units_carry_half": ">= 0.50", "pred_d_verb_top50_overlaps_noun_top50": "Jaccard >= 1/3"}
...
def main() -> None:
    ...
    predictions = {"pred_a_unit_closure": closure <= CLOSURE_TOL, "pred_b_detectors_lead_at_verb": {829, 953} <= top3, "pred_c_top_10_units_carry_half": share(10) >= TOP50_MIN, "pred_d_verb_top50_overlaps_noun_top50": jac_noun >= JACCARD_MIN}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_mlp8_unit_census_verb_result_v203", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "contrast_total": contrast,
