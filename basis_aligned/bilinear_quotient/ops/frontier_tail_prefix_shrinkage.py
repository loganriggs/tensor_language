"""IS THE TAIL BAND A CASCADE? PREFIX SHRINKAGE (Claude, LANE 1)

SS2899 found that per-layer tuning does NOT compose: every one of the eight tail layers prefers a scale of .05-.10 when tuned alone, and
each gains more that way than at .25 -- but composing all eight optima gives **-0.2092**, WORSE than the global 0.20's **-0.2290**, with
the additive prediction (-0.2906) overshooting by .0814. It closed with a hypothesis it did not test: the tail dictionaries form a
**CASCADE** -- a10L's output is part of a11L's input -- so individually optimal shrinkage compounds down the chain into collective
over-shrinkage.

That hypothesis has a sharp, cheap consequence. Shrink a PREFIX of the band at the common per-layer optimum (.05) and lengthen it one
layer at a time. If the layers were independent the cost would fall monotonically with prefix length. If the cascade account is right,
the gain **saturates or reverses** once enough of the chain is shrunk.

Ten arms differ only in what `evalM` sees, so they share ONE fitted stack (`ops/frontier_evalarms.py`): **one pipeline run.** pred_c
re-anchors the global 0.20 arm to SS2898 (a seventh reproduction of the adopted effect) and pred_d re-anchors the single-layer arm to
SS2899's a10L, so the curve is tied at both ends to numbers other rungs measured.
...
#                     pred_b_the_prefix_gain_saturates_or_reverses
#                     pred_c_the_global_arm_reproduces_S2898
#                     pred_d_the_single_layer_arm_reproduces_S2899
#                     pred_e_the_arms_are_connected

Preregistration: polynomial_causal/FRONTIER_TAIL_PREFIX_SHRINKAGE_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run: baseline, global .20, and eight prefix lengths at .05")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_tail_prefix_shrinkage_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_TAIL_PREFIX_SHRINKAGE_PREREGISTRATION.md'
PREREG_SHA = "ec77985925e94b8c4a32a20f563aef44e658895346256244bbadcd74477fbc75"
...
S2899 = PT + 'frontier_tail_per_layer_optimum_results.json'
S2898_SHA = "3c98d82eb1d33194ba4aa0f0ad8914a2dfc17d6917be61d81d8349fb11794ec7"
S2899_SHA = "bbd01c3c87c073e0e136062f0377cb26de16e841e80a074708dec07bcdffa0b6"
LAYERS = list(range(10, 18))
PREFIX_SCALE = 0.05
GLOBAL_BEST = 0.20
BARS = {"reproduce": 0.05, "anchor": 0.01, "single_anchor": 0.01, "connected": 0.005}
NULLS = {"monotone_tol": 0.005}
...
if __name__=='__main__':
    ...
    for p_, h_ in ((S2898, S2898_SHA), (S2899, S2899_SHA)):
        if _sha(p_) != h_:
            raise RuntimeError(f'frozen hash mismatch: {p_}')
    g98 = json.load(open(S2898))['summary']['best_global_cost']                 # -0.2291
    a10_99 = json.load(open(S2899))['summary']['best_gain_per_layer']['a10L']   # -0.0847
    SEL['mode']='norm'

    pk = lambda n: f'P{n}'
    ARMS[:] = ([('baseline', {}), ('global020', {'scale': GLOBAL_BEST})]
               + [(pk(n), {'per_layer': {l: PREFIX_SCALE for l in LAYERS[:n]}}) for n in range(1, 9)])
    print(f'ONE pipeline run, {len(ARMS)} evaluations against the same fitted stack',flush=True)
    out=main()

    bF = out['baseline']['L2_F']
    c_glob = round(out['global020']['L2_F'] - bF, 4)
    cp = {n: round(out[pk(n)]['L2_F'] - bF, 4) for n in range(1, 9)}
    seq = [cp[n] for n in range(1, 9)]
    turns = sum(1 for i in range(1, len(seq)-1)
                if (seq[i]-seq[i-1] > NULLS['monotone_tol']) != (seq[i+1]-seq[i] > NULLS['monotone_tol']))
    best_n = min(range(1, 9), key=lambda n: cp[n])
    dev_glob = abs(c_glob - g98)
    dev_single = abs(cp[1] - a10_99)

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = best_n < 8
    pc = dev_glob <= BARS['anchor']
    pd = dev_single <= BARS['single_anchor']
    pe = abs(cp[1]) >= BARS['connected']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_prefix_gain_saturates_or_reverses':bool(pb),
           'pred_c_the_global_arm_reproduces_S2898':bool(pc),
           'pred_d_the_single_layer_arm_reproduces_S2899':bool(pd),
           'pred_e_the_arms_are_connected':bool(pe)}
    nulls={'b_null_each_added_layer_helps':bool(best_n == 8),
           'c_null_the_global_anchor_fails':bool(dev_glob >= 0.03)}
    res={'rung':'frontier_tail_prefix_shrinkage','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,'layers':LAYERS,'prefix_scale':PREFIX_SCALE,
         'arms':out,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(bF,4),
                    'cost_by_prefix_length':{str(n):cp[n] for n in range(1,9)},
                    'best_prefix_length':best_n,'best_prefix_cost':cp[best_n],
                    'cost_full_band_at_005':cp[8],
                    'cost_global_020':c_glob,'S2898_global_best':round(g98,4),
                    'global_anchor_deviation':round(dev_glob,4),
                    'S2899_a10L_alone':round(a10_99,4),'single_anchor_deviation':round(dev_single,4),
                    'turns_in_the_prefix_curve':turns,
                    'implied_L2_F_best_prefix':round(bF + cp[best_n], 4),
         ...
         'hashes':{PREREG:PREREG_SHA,S2898:S2898_SHA,S2899:S2899_SHA},'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {bF:.4f} vs 2.6735: {'HELD' if pa else 'FAILED'}")
    print("    prefix: " + "  ".join(f"n={n}:{cp[n]:+.4f}" for n in range(1,9)))
    print(f"(b) best prefix length {best_n} (< 8 means it saturates/reverses): {'HELD' if pb else 'FAILED'}")
    print(f"(c) global .20 {c_glob:+.4f} vs SS2898 {g98:+.4f}, dev {dev_glob:.4f}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) a10L alone {cp[1]:+.4f} vs SS2899 {a10_99:+.4f}, dev {dev_single:.4f}: {'HELD' if pd else 'FAILED'}")
