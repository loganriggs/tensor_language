...
import collections
FAMILIES = {"temporal": {"attn9_h1", "attn9_h4", "attn15_h5"}, "number": {"attn11_h3", "attn5_h7", "attn7_h8", "attn9_h7"}, "pronoun": {"attn9_h6", "attn12_h4", "attn15_h1"},
...
def main():
    ...
    if "--md" in sys.argv:
        ...
        lines = [f"Lines {len(rows)}, capable {len(cap)}, live {len(live)}, selective {len(sel)}. Family = >= 2 core heads in the top-4, largest overlap wins (temporal 9.1/9.4/15.5, number 11.3/5.7/7.8/9.7 (the union of the two batteried number sets), pronoun 9.6/12.4/15.1, selection 13.8/7.8/8.8, person 8.1/13.1/10.5/15.1): " + ", ".join(f"{k} {v}" for k, v in assigned.most_common()) + ".", "",
