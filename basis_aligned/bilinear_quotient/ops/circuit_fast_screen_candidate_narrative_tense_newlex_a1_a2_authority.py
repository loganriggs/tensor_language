#!/usr/bin/env python3
"""Frozen fresh-lexicon A1/A2 narrative-tense capability authority."""
...
SCHEMA = "narrative_tense_newlex_a1_a2_authority_v1"
TASK_ID = "narrative_tense.past_vs_present"
CAPABILITY_ID = "narrative_tense.newlex_a1_a2_native_capability_v1"
CAUSAL_CANDIDATE_ID = "narrative_tense.attn11_head3_newlex_a1_a2_carrier_v1"
...
SUBJECT_PAIRS = (
    ("orange", "mango"), ("peach", "onion"), ("carrot", "potato"),
    ("tomato", "pepper"), ("garlic", "bread"), ("cake", "cheese"),
    ("rice", "soup"), ("meat", "fish"), ("sheep", "goat"),
    ("deer", "bear"), ("lion", "tiger"), ("whale", "shark"),
    ("eagle", "crow"), ("owl", "ant"), ("bee", "fly"), ("worm", "frog"),
)
TAILS = ("bus", "plane", "bed", "desert", "sand", "steel", "page", "lock",
         "cup", "fork", "barrel", "mirror", "guitar", "horn", "shirt", "boot")
...
def _a1(subject: str, tail: str, past: bool) -> str:
    lead, verb, pointer = (("Yesterday", "served", "that") if past
                           else ("Today", "serves", "this"))
    return (f"{lead} the {subject} {verb} one role. At {pointer} time, "
            f"the role of the {tail}")


def _a2(subject: str, tail: str, past: bool) -> str:
    pointer, report = (("that", "showed") if past else ("this", "show"))
    return (f"At {pointer} time, records {report} the {subject} filling one role. "
            f"In {pointer} period, the recorded role of the {tail}")
...
def _p(subject: str, tail: str, past: bool) -> str:
    ...
    return (f"{lead} the {subject} {verb} one role. At {pointer} time, "
            f"the formal role of the {tail}")


def _c(subject: str, tail: str, past: bool) -> str:
    pointer, report = (("that", "showed") if past else ("this", "show"))
    return (f"At {pointer} time, records {report} the {subject} filling one role. "
            f"In {pointer} period, the documented role of the {tail}")
...
def _row(group: int, family: str, direction: str, base_text: str,
    ...
    return {
        ...
        "family": family, "template_id": "direct_recue" if family in {"A1", "P"}
        else "reported_recue", "direction_id": direction,
...
def _panel(group: int) -> list[dict]:
    ...
    return [
        ...
        _row(group, "P", control_direction,
             _p(base_subject, tail, control_past),
             _p(donor_subject, tail, control_past), control_past, control_past),
        _row(group, "C", control_direction,
             _c(base_subject, tail, control_past),
             _c(donor_subject, tail, control_past), control_past, control_past),
    ]
...
def validate_rows(rows: Sequence[Mapping[str, object]]) -> str:
    ...
    used = set(original._SUBJECTS + original._ALTERNATES + original._PLACES + original._FOCUS) \
        ...
        | set(a2c.TAILS) | set(a2c.LOCATIONS)
    if set(vocab) & used:
    ...
    prior_rows = original.build_rows() + fresh.build_rows() + newlex.build_rows() + a2c.build_rows()
