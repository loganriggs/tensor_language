...
"""GPT-2 on the RANK AXIS (v786). The ladder now reads, for recovery ~0.95-0.96: Pythia-160m at 8.4B tokens needs rank 20 of a 64-dim head (31%,
v783), the same model at 300B tokens needs 44-48 (70%, v782), and the softmax twin of bilin18 at ~5B tokens needs 16 of a 128-dim head (12.5%,
0.949; rank 24 -> 0.965, v784/v785). GPT-2 small (WebText, roughly 40B tokens, 64-dim heads) was measured only at ranks 16 and 32 (0.893 / 0.945,
v765). This rung fits ranks 40 and 48 on the same centred protocol so GPT-2 takes its place on the axis between the early and late Pythia points.
Row-centred logits as in v765 (the first-token column and diagonal native).
PREDICTIONS (scored as written; failures preserved)
    pred_a_instrument_replays   explicit native CE within 0.002 of the stock native CE (instrument)
    pred_b_kernel_energy_top4   top-4 SVD shapes carry >= 0.90 of the kernels' energy (v765: 0.9996). Prior: likely
    pred_c_rank32_cost          (rank 48) snapshot recovery >= 0.97 (rank 32: 0.945). Prior: unsure
    pred_d_rank16_cost          (rank 40) snapshot recovery >= 0.96. Prior: unsure
    pred_e_rank32_values_preserved_scale (rank 48) median value ratio over the 24 most valuable heads in [0.5, 2]. Prior: likely
PRICE (registered maximum): stock 7 + explicit 7; means 16; kernels 2; 144 x 7 = 1008; all-ablated 7; kernels-only 7 + col0 7; ladder 35;
2 arms x (kappa_r 26 + step-0 10 + 300 + 12 x (26 + 7 + 7) + snapshot 9) = 1650; manipulability 168; total ~2900 forwards, 600 backwards. Bars: forwards <= 3300, backwards <= 600.
"""
...
import dod_battery
import disk_guard
import gpt2_backend as HB

REPO = "gpt2"
TAG = "gpt2_rankladder_v786"
PREFIX = "gpt2"
...
CANDIDATE_ID = f"gpt2.all_{TAG}"
FORWARDS_MAX, BACKWARDS_MAX = 3300, 600
EBATCH, TBATCH, STEPS, EVAL_EVERY, N_VAL, N_MANIP = 32, 8, 300, 25, 96, 24
LR_MAP, LR_MAP_MIN, LR_K, LR_K_MIN = 3e-4, 3e-5, 0.01, 0.001
KS, ARMS = (1, 2, 4, 8, 16), (40, 48)
REPLAY_TOL, E4_MIN, REC48, REC40, RATIO = 0.002, 0.90, 0.97, 0.96, (0.5, 2.0)
PREDICTIONS = {"pred_a_instrument_replays": "+-0.002", "pred_b_kernel_energy_top4": ">= 0.90", "pred_c_rank32_cost": "rank 48 recovery >= 0.97", "pred_d_rank16_cost": "rank 40 recovery >= 0.96", "pred_e_rank32_values_preserved_scale": "median ratio in [0.5, 2]"}
...
def main() -> None:
    plan = {"candidate_id": CANDIDATE_ID, "forwards_max": FORWARDS_MAX, "model_backwards": BACKWARDS_MAX, "model_updates": 0, "fit_parameters": "2 arms of factored q/k maps + kernels",
            "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "model": REPO, "arms": list(ARMS), "ks": list(KS),
            "bars": {"replay_tol": REPLAY_TOL, "e4_min": E4_MIN, "rec48": REC48, "rec40": REC40, "ratio": RATIO}}
    ...
    r32, r16 = results[48], results[40]
    predictions = {"pred_a_instrument_replays": abs(native - native_stock) <= REPLAY_TOL, "pred_b_kernel_energy_top4": float(energy[3]) >= E4_MIN, "pred_c_rank32_cost": r32["recovery"] >= REC48,
                   "pred_d_rank16_cost": r16["recovery"] >= REC40, "pred_e_rank32_values_preserved_scale": RATIO[0] <= r32["median_ratio"] <= RATIO[1]}
    if forwards > FORWARDS_MAX or backwards > BACKWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} fwd / {backwards} bwd")
    OUT.write_text(json.dumps({"schema": f"gpt2_all_result_{TAG}", "candidate_id": CANDIDATE_ID, "plan": plan,
