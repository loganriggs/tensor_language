...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; class+position map of the block-2 motif replacement (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'attn2_motif_class_results.json'
...
if __name__=='__main__':
    ...
    print('ARM 1/2: full S2144 config, per-position CE captured',flush=True)
    r_full=main()
    cev_full=SEL['cev']; clsflat=SEL['clsflat']
    SEL['motif_off']=(2,)
    print('ARM 2/2: block-2 motifs REAL, per-position CE captured',flush=True)
    r_no2=main()
    cev_no2=SEL['cev']
    import statistics as stt
    if float((cev_full-cev_no2).abs().sum())<1e-3:
        raise SystemExit('INSTRUMENT FAIL: arms identical - motif_off inert')
    diff=(cev_full-cev_no2)
    tot=float(diff.sum())
    shares={}
    for k in range(10):
        selk=clsflat==k
        shares[k]=float(diff[selk].sum())/max(abs(tot),1e-9)
    ret=shares[CLS.index('ind')]+shares[CLS.index('other')]+shares[CLS.index('subword')]
    pos=torch.arange(len(clsflat))%256
    late=float(diff[pos>=128].sum()); early=float(diff[pos<128].sum())
    lshare=late/max(late+early,1e-9)
    pa=ret>=0.60
    pb=lshare<=0.45
    pc=abs(r_full['L2_F']-2.6662)<=0.01
    res={'full':{'L2_F':r_full['L2_F']},'total_damage_sum':round(tot,2),
         'class_shares':{CLS[k]:round(shares[k],4) for k in range(10)},
         'retrieval_combined':round(ret,4),'late_half_share':round(lshare,4),
         'convention':'per-position CE(full) - CE(block-2 motifs real) on FR; positive = the replacement hurts there',
         'pred_a_retrieval':bool(pa),'pred_b_early':bool(pb),'pred_c_reproduces_s2144':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'total {tot:+.1f}; shares: '+' '.join(f'{CLS[k]}:{shares[k]:+.3f}' for k in range(10))+f'; late-half {lshare:.3f}')
    print(f"(a) ind+other+subword {ret:+.3f} >= 0.60: {'HELD' if pa else 'FAILED'}")
    print(f"(b) late-half {lshare:.3f} <= 0.45: {'HELD' if pb else 'FAILED'}")
