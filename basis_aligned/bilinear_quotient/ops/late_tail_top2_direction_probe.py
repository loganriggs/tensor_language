#!/usr/bin/env python
"""late_tail_top2_direction_probe -- WHAT are the two dominant directions of the late-origin tail (§2779: eigenvalues 1.46 and .67 over a
flat floor of ~.065)? Position profile (a position-0 / sink-like spike?), alignment with the unembed row space, sharing across the ten late
blocks' own covariances, and the CE cost of removing exactly those two directions (mean-filled) from all late MLP reads, against two random
complement directions. Component identity below the block; ties the width story to known circuits if it lands. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_top_direction_is_a_position0_spike pred_c_top_directions_are_readout_aligned
#                     pred_d_dropping_top2_is_cheap pred_e_top_direction_shared_across_late_blocks
...
Preregistration: polynomial_causal/LATE_TAIL_TOP2_DIRECTION_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_tail_top2_direction_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_tail_top2_direction_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_TAIL_TOP2_DIRECTION_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "tail_read_output_frame_probe_results.json"   # §2785
OUT = ROOT / "late_tail_top2_direction_probe_results.json"
HASHES = {PREREG: "c00f8830d00c14f1e399d940be27318a92c8abdfa64de456ebf55715d61ba47e", PRIOR: "4cd9256f718555f42cf31958055a13614a367d5da24c127a0563dd9171ea3ac8",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_tail_top2_direction_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_SPLIT8_1024 = 0.0374; PRIOR_LATE_MLP_768 = 0.1249; PRIOR_LATE_MLP_896 = 0.0662; PRIOR_EIG1 = 1.46; LAM = 1e-2
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "eig_tol": 0.05, "b_min": 0.50, "c_min": 0.60, "d_max": 0.003, "e_cos": 0.9, "e_blocks": 8}
NULLS = {"b_max": 0.10, "c_max": 0.30, "d_min": 0.010, "e_blocks": 3}
...
def main():
    ...
    def perp(z):
        return z - (z @ Uk) @ Uk.T
    class Tracker:
        """Attention-site recorder for late block l: y (early origin), z (late origin = all writes of blocks 8..l incl. this attention)."""
        ...
        def __call__(self, w, x):
            b = self.box
            if self.l == SPLIT:
                b["y"] = x; b["z"] = torch.zeros_like(x)
            else:
                lam = m.transformer.h[self.l].lambdas; b["y"] = lam[0] * b["y"] + lam[1] * b["x0"]; b["z"] = lam[0] * b["z"]
            b["z"] = b["z"] + w
            return w
    class ZRec:
        """MLP-site recorder: per-block AND pooled covariance of the late-origin component off the core; optional per-position energy on given directions."""
        def __init__(self, l, box, acc, accb, dirs=None, pos=None):
            self.l = l; self.box = box; self.acc = acc; self.accb = accb; self.mlp = m.transformer.h[l].mlp; self.dirs = dirs; self.pos = pos
        def hook(self, idx): pass
        def __call__(self, w, x):
            xh = F.rms_norm(x, (D,)); scale = xh.norm(dim=-1, keepdim=True) / x.norm(dim=-1, keepdim=True).clamp_min(1e-12)
            tz = perp(self.box["z"] * scale)
            if self.dirs is not None:
                pr = (tz @ self.dirs) ** 2; self.pos["E"] += pr.sum(0).double(); self.pos["n"] += pr.shape[0]
                self.pos["tot"] += (tz ** 2).sum(-1).sum(0).double()
            else:
                t = tz.reshape(-1, D).double(); self.acc["G"] += t.T @ t; self.acc["s"] += t.sum(0); self.acc["n"] += t.shape[0]
                a = self.accb[self.l]; a["G"] += t.T @ t; a["s"] += t.sum(0); a["n"] += t.shape[0]
            self.box["z"] = self.box["z"] + w + self.mlp.Down_bias
            return w
    def newacc():
        return {"G": torch.zeros(D, D, dtype=torch.float64, device=DEV), "s": torch.zeros(D, dtype=torch.float64, device=DEV), "n": 0}
    acc = newacc(); accb = {l: newacc() for l in range(SPLIT, NL)}
    for i in range(0, fit_rows.shape[0], CH):
        idx = fit_rows[i:i + CH, :TI]; bx = {}
        pt = {("attn", l): Tracker(l, bx) for l in range(SPLIT, NL)}; pt.update({("mlp", l): ZRec(l, bx, acc, accb) for l in range(SPLIT, NL)})
        for o in pt.values():
            o.hook(idx)
        forward(m, idx, patch=pt)
    def top_of(a):
        mu = a["s"] / a["n"]; C = a["G"] / a["n"] - torch.outer(mu, mu); ev_, U_ = torch.linalg.eigh(C); return ev_.flip(0).clamp_min(0), U_.flip(1)
    evz, Uz = top_of(acc); u1 = Uz[:, 0].float(); u2 = Uz[:, 1].float(); dirs = torch.stack([u1, u2], 1)
    cos_blocks = []
    for l in range(SPLIT, NL):
        e_, U_ = top_of(accb[l]); cos_blocks.append({"block": l, "cos_top1": abs(float(U_[:, 0].float() @ u1)), "cos_top2_sub": float(((U_[:, :2].float().T @ dirs) ** 2).sum().sqrt() / 2 ** 0.5), "eig12": [float(e_[0]), float(e_[1])]})
    pos = {"E": torch.zeros(TI, 2, dtype=torch.float64, device=DEV), "tot": torch.zeros(TI, dtype=torch.float64, device=DEV), "n": 0}
    for i in range(0, fit_rows.shape[0], CH):
        idx = fit_rows[i:i + CH, :TI]; bx = {}
        pt = {("attn", l): Tracker(l, bx) for l in range(SPLIT, NL)}; pt.update({("mlp", l): ZRec(l, bx, acc, accb, dirs, pos) for l in range(SPLIT, NL)})
        for o in pt.values():
            o.hook(idx)
        forward(m, idx, patch=pt)
    E = pos["E"]; share_pos0 = [float(E[0, j] / E[:, j].sum()) for j in range(2)]; share_first8 = [float(E[:8, j].sum() / E[:, j].sum()) for j in range(2)]
    share_of_total_pos0 = float(E[0].sum() / pos["tot"][0])
    WU = m.lm_head.weight.float(); ev_u, V_u = torch.linalg.eigh(WU.T @ WU); V_u = V_u.flip(1); V16 = V_u[:, :16]; V128 = V_u[:, :128]
    wte_mean = F.normalize(m.transformer.wte.weight.float().mean(0), dim=0)
    def sub_cos(u, Vb):
        return float((u @ Vb).norm())
    dir_stats = {"u1": {"cos_unembed_top16": sub_cos(u1, V16), "cos_unembed_top128": sub_cos(u1, V128), "cos_wte_mean": abs(float(u1 @ wte_mean)), "share_pos0": share_pos0[0], "share_first8": share_first8[0], "eig": float(evz[0])},
                 "u2": {"cos_unembed_top16": sub_cos(u2, V16), "cos_unembed_top128": sub_cos(u2, V128), "cos_wte_mean": abs(float(u2 @ wte_mean)), "share_pos0": share_pos0[1], "share_first8": share_first8[1], "eig": float(evz[1])},
                 "floor_eig_3_16": [float(v) for v in evz[2:16]], "pos0_share_of_total_tail_energy": share_of_total_pos0}
    log(stage="directions", **{k: (round(v, 3) if isinstance(v, float) else v) for k, v in dir_stats["u1"].items()}, shared=[round(c["cos_top1"], 3) for c in cos_blocks])
    class DropDirs(OwnHead):
        """MLP l reads the exact input with the two pooled channel directions replaced by their fit-set mean (everything else exact)."""
        def __init__(self, l):
            super().__init__(l, 768, "const", Uk)
        def __call__(self, w, x):
            xh = F.rms_norm(x, (D,)); d = xh - self.mx; xp_ = xh - (d @ dirs) @ dirs.T
            return self.mlp.Down(self.mlp.Left(xp_) * self.mlp.Right(xp_))
    class DropRand(OwnHead):
        def __init__(self, l, R2):
            super().__init__(l, 768, "const", Uk); self.R2 = R2
        def __call__(self, w, x):
            xh = F.rms_norm(x, (D,)); d = xh - self.mx; xp_ = xh - (d @ self.R2) @ self.R2.T
            return self.mlp.Down(self.mlp.Left(xp_) * self.mlp.Right(xp_))
    g = torch.Generator().manual_seed(0); Rm = torch.randn(384, 2, generator=g); Rq, _ = torch.linalg.qr(Rm); R2 = (Uz[:, 2:386].float() @ Rq.to(DEV))
    arms = {"SPLIT8_1024": split8(1024), "BUS_896": run({("mlp", l): head(("mlp", l), 896, U_8) for l in range(SPLIT, NL)}), "DROP_TOP2_MLP": run({("mlp", l): DropDirs(l) for l in range(SPLIT, NL)})}
    if not smoke:
        arms["DROP_RAND2_MLP"] = run({("mlp", l): DropRand(l, R2) for l in range(SPLIT, NL)})
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    n_shared = sum(1 for c in cos_blocks if c["cos_top1"] >= BARS["e_cos"])
    summ = {"directions": dir_stats, "per_block_top": cos_blocks, "n_blocks_sharing_top1": n_shared, "drop_top2": arms["DROP_TOP2_MLP"], "drop_rand2": arms.get("DROP_RAND2_MLP")}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(arms["BUS_896"] - PRIOR_LATE_MLP_896) <= BARS["repro_tol"] and abs(float(evz[0]) - PRIOR_EIG1) <= BARS["eig_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_top_direction_is_a_position0_spike': bool(share_pos0[0] >= BARS["b_min"]),
        'pred_c_top_directions_are_readout_aligned': bool(max(dir_stats["u1"]["cos_unembed_top128"], dir_stats["u2"]["cos_unembed_top128"]) >= BARS["c_min"]),
        'pred_d_dropping_top2_is_cheap': bool(arms["DROP_TOP2_MLP"] <= BARS["d_max"]),
        'pred_e_top_direction_shared_across_late_blocks': bool(n_shared >= BARS["e_blocks"]),
    }
    nulls = {"b_null_pos0_share_le_.10": bool(share_pos0[0] <= NULLS["b_max"]), "c_null_cos128_le_.30": bool(max(dir_stats["u1"]["cos_unembed_top128"], dir_stats["u2"]["cos_unembed_top128"]) <= NULLS["c_max"]),
             "d_null_drop_top2_ge_.010": bool(arms["DROP_TOP2_MLP"] >= NULLS["d_min"]), "e_null_shared_le_3_blocks": bool(n_shared <= NULLS["e_blocks"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "identity of the two dominant directions of the pooled late-origin tail covariance (§2779): position profile, unembed alignment, sharing across late blocks, and the CE cost of removing them (mean-filled) from all late MLP reads vs 2 random complement directions",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "bus_896": arms["BUS_896"], "prior_late_mlp_896": PRIOR_LATE_MLP_896, "eig1": float(evz[0]), "prior_eig1": PRIOR_EIG1},
