...
"""Pronoun number they/he DoD (v207): WHERE is head 4.5's copy read? v206: zeroing 4.5's slice at the verb drops the they - he margin 7.7% (10x any
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_head45_readers_v207_result.json"
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_head45_readers_v207"
...
def main() -> None:
    ...
    def pooled_margin(lst): return sum((lst[i]["answer"] - lst[i]["foil"]) - (lst[j]["answer"] - lst[j]["foil"]) for i, j in pairs)
    ...
    moves = {r: sum(abs(a[r] - n[r]) for n, a in zip(margins["native"], margins["verb"])) / len(rows) for r in readers}
    null_moves = {r: sum(sum(abs(a[r] - n[r]) for n, a in zip(margins["native"], null_margins[h])) / len(rows) for h in null_margins) / len(null_margins) for r in readers}
    dmg_per_row = dmg_verb / len(pairs); gates = {r: moves[r] <= null_moves[r] + GATE_RATIO * dmg_per_row for r in readers}
    ...
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_head45_readers_result_v207", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
