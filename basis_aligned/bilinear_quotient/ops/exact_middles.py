"""EXACT MIDDLES ON THE DAY-CLOSE CONFIG (rung 199): decomposing the remaining +0.62.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. At census +0.62 the replaced parts are:
S2144-K middles (c4-c9), rank-32 pattern truncation, and interactions (front exact, tail MLPs real, no
tailE). This arm sets the middles to K=4608 (exact): what remains is pattern truncation + interactions -
the endgame decomposition. DIAGNOSTIC (middles-4608 adds ~74M values; not a config proposal).

REGISTERED PREDICTIONS (anchor: rung-196 census +0.6201):
  (a) MIDDLES-K HOLDS ITS OLD SHARE: census <= 0.50 (>= 0.12 recovery).
  (b) PATTERN TRUNCATION IS REAL: census >= 0.30.
  (c) SANITY: census >= 0.2; inert tripwire.
NULL (a): the middles marginal shrank at the new frontier (< 0.05 recovery). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: exact middles decomposition')
    raise SystemExit(0)
import torch
...
OUT=PT+'exact_middles_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=32; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    pb=agg>=0.30
    pc=agg>=0.2
    if abs(agg-0.6201)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: middles change inert')
    ...
    print(f"(a) census {agg:+.4f} <= 0.50 (middles-K held >= 0.12): {'HELD' if pa else 'FAILED'}")
    print(f"(b) census {agg:+.4f} >= 0.30 (pattern truncation is real): {'HELD' if pb else 'FAILED'}")
    print(f"(c) census >= 0.2: {'HELD' if pc else 'FAILED'}")
