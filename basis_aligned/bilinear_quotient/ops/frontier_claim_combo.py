"""REGISTERED FRONTIER CLAIM: THE QUALITY-END COMBINATION (rung 196).

CONVENTION (S2135): frontier L2 = CE ADDED ABOVE THE REAL MODEL on the S312 fresh windows; LOWER IS BETTER.
Rung 194 printed DESCRIPTIVELY L2_F +0.6412 / census +0.6201 (front 4608 + rank-32 patterns + no tailE).
Identical deterministic rebuild; claim bars as predictions.

REGISTERED PREDICTIONS (anchors: rung-194 receipt):
  (a) NEW FRONTIER: L2_F fresh <= 0.66.
  (b) REPRODUCTION: |census - 0.6201| <= 0.015.
  (c) REPRODUCTION: |L2_F - 0.6412| <= 0.015.
NULL: 194's print was noise (L2_F >= 0.70). PRICE: ~88M values (front 63.7M + patterns 24.2M).
Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: registered claim quality-end combo')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_claim_combo_results.json'
...
if __name__=='__main__':
    ...
    pa=L2F<=0.66
    pb=abs(agg-0.6201)<=0.015
    pc=abs(L2F-0.6412)<=0.015
    ...
    print(f"(a) L2F {L2F:+.4f} <= 0.66: {'HELD' if pa else 'FAILED'}")
    print(f"(b) |census {agg:+.4f} - 0.6201| <= 0.015: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |L2F - 0.6412| <= 0.015: {'HELD' if pc else 'FAILED'}")
