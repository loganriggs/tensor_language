...
"""Softmax replication, v750 (Logan, 21 Sep 23:50 UTC: 'that and our own softmax version of our model'): head values and closed-form
kernels on Elriggs/gpt2-bilinear-18l-9h-1152embd — bilin18's sibling with SOFTMAX attention (everything else identical: bilinear MLPs,
rotary, QK-norm, lambda-mixed values, tokenizer, 9726 training steps).

Same instruments as v701 / v702 / v724 applied at the PRE-SOFTMAX LOGIT level: (i) native CE on skip7000 with the stock sdpa and with the
explicit-softmax instrument (must agree); (ii) mean-ablation value of every head (z_h := its mean over the 480 skip80 rows) and the joint
value (all 162 at once); (iii) the closed-form kernel kappa_h(d) = mean pre-softmax logit at offset d (queries >= 8, 64 fit rows), installed
for every head at once as the OFF-DIAGONAL logits (diagonal native, causal mask kept) = the kernels-only program; (iv) the SVD of the 162 x 512
kernel matrix and the k-ladder (1, 2, 4, 8, 16 shapes) inside the kernels-only program; (v) the named basis {exp(-d/6), exp(-d/3), d^-0.5,
d^-1} inside it. Held-out skip7000 (192 x 512). CE ADDED, lower is better. bilin18 references: joint value 3.996; kernels-only closed-form
1.45 (fitted 0.956); top-4 energy 0.964; 4 shapes inside kernels-only +0.004.
PREDICTIONS (scored as written; failures preserved)
    pred_a_instrument_replays    explicit-softmax native CE within 0.002 of the stock-sdpa native CE (instrument)
    pred_b_joint_value_comparable the softmax model's joint value (all 162 mean-ablated) is within [2, 8] (bilin18: 3.996). Prior: unsure
    pred_c_kernels_only_cost     the closed-form kernels-only program costs <= 1.5 (bilin18: 1.45). Prior: unsure
    pred_d_kernel_energy_top4    the top-4 SVD shapes carry >= 0.90 of the kernels' energy (bilin18: 0.964). Prior: unsure
    pred_e_four_shapes_cheap     4 shapes inside the kernels-only program cost <= kernels-only + 0.05 (bilin18: +0.004). Prior: unsure
PRICE (registered maximum): stock native 6; instrumented native 6; means 15; kernels 2; 162 x 6 = 972; all-ablated 6; kernels-only 6; k-ladder 5 x 6 = 30;
named-4 6; total 1049 forwards; 0 backwards; 0 fits. Bar <= 1070.
"""
...
OUT = ROOT / "circuits/followups/softmax_values_kernels_v750_result.json"
OUT_PT = ROOT / "circuits/followups/softmax_values_kernels_v750_tensors.pt"
FIT_ROWS = ROOT / ".rowcache/fineweb_n480_skip80.pt"
EVAL_ROWS = ROOT / ".rowcache/fineweb_n192_skip7000.pt"
CANDIDATE_ID = "softmax.values_kernels_v750"
FORWARDS_MAX = 1070
EBATCH = 32
...
def main() -> None:
    plan = {"candidate_id": CANDIDATE_ID, "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0, "fit_parameters": 0,
            "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "model": SB.REPO, "ks": list(KS),
            "bars": {"replay_tol": REPLAY_TOL, "joint_range": JOINT_RANGE, "ko_max": KO_MAX, "e4_min": E4_MIN, "k4_tol": K4_TOL}}
    if os.environ.get("BQLIB_DRYRUN") or os.environ.get("BQLIB_NO_MODEL"):
        print(json.dumps(plan, indent=2, sort_keys=True)); return
    t0 = time.perf_counter()
    model, TT = SB.load("cuda"); dev = "cuda"; D = model.config.n_embd; hd = D // H; forwards = 0
    ...
    with torch.no_grad():
        for s in range(0, fit.shape[0], EBATCH):
            idx = fit[s:s + EBATCH, :-1].contiguous().to(dev); model(idx, fit[s:s + EBATCH, 1:].contiguous().to(dev)); forwards += 1
    cap["mode"] = "kernels"
    with torch.no_grad():
        for s in range(0, 64, EBATCH):
            idx = fit64[s:s + EBATCH, :-1].contiguous().to(dev); model(idx, fit64[s:s + EBATCH, 1:].contiguous().to(dev)); forwards += 1
    cap["mode"] = None
    ...
    state["_restore"]()
    disk_guard.guard_torch_save({"means": {f"{k[0]}.{k[1]}": means[k].cpu() for k in ALL}, "kappa": {f"{k[0]}.{k[1]}": kappa[k].cpu() for k in ALL}, "svd_S": S.cpu(), "svd_Vh": Vh[:16].cpu()}, str(OUT_PT), "v750 softmax means and kernels")
    ...
    OUT.write_text(json.dumps({"schema": "softmax_values_kernels_result_v750", "candidate_id": CANDIDATE_ID, "plan": plan,
                               "report": {"native_stock": native_stock, "native": native, "mean_ablation_cost": value, "median_head": median, "joint_value": joint, "kernels_only_cost": c_ko - native,
