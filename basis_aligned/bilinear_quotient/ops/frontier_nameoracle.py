"""NAME-ORACLE CEILING (rung 140): what would a perfect name-retriever at tail attention buy?

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2233: name (+3.081) is irreducible
retrieval - no local-stream function helps. Here the exact rung-127 build runs with an ORACLE SPLICE: at
every tail-attention dictionary site, positions with oracle class == name (k=5) pass through the REAL
attention output (available as the hooked module's own y); all other classes get the dictionary as usual.
Later dictionaries are fit under the spliced earlier sites (config-consistent). NOT deployable - this prices
the ceiling for a real retrieval module.

REGISTERED PREDICTIONS (anchors: S2228 name +3.134, census +1.9474, L1F +1.5509, increment +0.3256):
  (a) THE CEILING IS HIGH: dCE(name) <= 1.0 (>= 2/3 of name's bill is recoverable at the tail-attn grain).
  (b) AGGREGATE MOVES: census <= 1.85.
  (c) ISOLATION: |L1F - 1.5509| <= 0.02 AND increment in [0.15, 0.36].
NULL: name's bill sits in tailE/the front interactions, not tail attention (oracle moves name < 0.5).
PRICE: none (oracle diagnostic). Tripwire: INSTRUMENT FAIL if L2CF missing. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: name-oracle ceiling')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_nameoracle_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['name_oracle']=True
    print('ARM: rung-127 build + name-oracle splice at all tail-attn sites',flush=True)
    main()
    ...
    print(f"(a) name {PC.get('name',9.9):+.4f} <= 1.0: {'HELD' if pa else 'FAILED'}")
    print(f"(b) census {agg:+.4f} <= 1.85: {'HELD' if pb else 'FAILED'}")
