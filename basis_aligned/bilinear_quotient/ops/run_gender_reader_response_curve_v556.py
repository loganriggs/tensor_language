#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_half_swap_is_sub_linear pred_c_full_swap_replays pred_d_response_is_monotone pred_e_response_accelerates
"""The readout's response curve to the gender readers (v556). v555: inside the MLP-8 edit the five readers' values at the noun move by 0.23-0.34 of their
male - female change, yet the he - she margin closes 0.078 against 0.151 if it responded linearly to the readers' singles (v551). Here the readers' values at
the noun are swapped by a FRACTION alpha -- v <- (1 - alpha) v + alpha v_partner, for the five readers jointly, alpha in {0.25, 0.5, 0.75, 1} -- and the
margin closed is read as a function of alpha. Linear readout: closed(alpha) = alpha x 0.600. Quadratic (a bilinear stage multiplying the readers' write
with itself or with another moved signal): closed(alpha) ~ alpha^2 x 0.600.
PREDICTIONS (scored as written; failures preserved; priors from v549 / v555)
    pred_a_baseline_replays        the unedited manual forward replays the model's he - she margins within 1e-3
    pred_b_half_swap_is_sub_linear closed(0.5) <= 0.5 x 0.600 - 0.05 (v555's half-effect)
    pred_c_full_swap_replays       closed(1.0) = 0.600 +- 0.03 (v549's FIVE_N)
    pred_d_response_is_monotone    closed(0.25) < closed(0.5) < closed(0.75) < closed(1.0)
    pred_e_response_accelerates    closed(0.75) - closed(0.5) >= closed(0.5) - closed(0.25) (convex, as a quadratic stage predicts). Prior: unsure
...
OUT = ROOT / "circuits/followups/gender_reader_response_curve_v556_result.json"
CANDIDATE_ID = "gender.reader_response_curve_v556"
ROWS = ROOT / "circuits/followups/pronoun_gender_dod_natural_rows_v73.json"
N_HEAD = 9
FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, FIVE_V549, SUB, BAND = 1e-3, 0.600, 0.05, 0.03
FORWARDS_MAX = 11
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_half_swap_is_sub_linear": "<= 0.25", "pred_c_full_swap_replays": "0.600 +- 0.03", "pred_d_response_is_monotone": "increasing in alpha", "pred_e_response_accelerates": "convex"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "text_pairs": len(text_items), "rows": str(ROWS.name), "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "five_v549": FIVE_V549, "sub": SUB, "band": BAND}}
    ...
    ALPHAS = {"A025": 0.25, "A050": 0.5, "A075": 0.75, "A100": 1.0}
    MODES = {m: ({}, FIVE, False) for m in ALPHAS}; forwards = 0
    def logits_margin(x):
        ...
        with torch.no_grad():
            ...
            for l, block in enumerate(blocks):
                ...
                if mode != "native" and (l in MODES[mode][0] or l in MODES[mode][1] or MODES[mode][2]):
                    ...
                    al = ALPHAS[mode]
                    for h in hs0: v[idx, pp, h] = ((1 - al) * v[idx, pp, h] + al * v[swap, pp, h]).clone()   # fractional swap of the readers' values at the noun
                    if rest:
    ...
    c = text["closed"]
    r_ = [c["A025"], c["A050"], c["A075"], c["A100"]]; print("response", [round(x, 3) for x in r_], "linear", [round(a * FIVE_V549, 3) for a in (0.25, 0.5, 0.75, 1.0)])
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_half_swap_is_sub_linear": c["A050"] <= 0.5 * FIVE_V549 - SUB, "pred_c_full_swap_replays": abs(c["A100"] - FIVE_V549) <= BAND,
                   "pred_d_response_is_monotone": r_[0] < r_[1] < r_[2] < r_[3], "pred_e_response_accelerates": (r_[2] - r_[1]) >= (r_[1] - r_[0])}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "gender_reader_response_curve_result_v556", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
