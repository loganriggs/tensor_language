#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_instrument_replays pred_b_rank32_expensive pred_c_rank16_expensive pred_d_kernel_energy_top4 pred_e_rank32_values_preserved_scale
"""THE PAIR: same family, same data recipe, no QK-norm (v791). v788 falsified the training-length story as written: Qwen3-0.6B (per-head QK-norm
+ rotary, 128-dim heads, ~36T tokens) recovers 0.966 at rank 32 of 128 and 0.933 at rank 16 — on the softmax twin's curve (0.965 at 24 of 128,
0.949 at 16), NOT on the 300B-token Pythia curve (0.932 at 32 of 64). The one thing Qwen3 shares with bilin18 and its twin, and not with Pythia,
GPT-2, OPT or SmolLM, is per-head QK-norm. Qwen2.5-0.5B is the matched control: the previous generation of the same family, ~18T tokens of
a similar recipe, rotary, GQA, q/k/v BIASES and NO QK-norm, 64-dim heads (24 layers x 14 heads, 2 kv heads, hidden 896). It tokenises the FineWeb
rows IDENTICALLY to Qwen3 (checked: all three caches equal), so the pair is scored on the same 513-token rows. If QK-norm is what makes a
long-trained model cheap to read, Qwen2.5 should sit with Pythia-300B (rank 32 of 64 ~0.93); if the Qwen recipe is, it should sit with Qwen3.
MODEL = Qwen/Qwen2.5-0.5B. ROW-CENTRED logits throughout; column 0 and the diagonal native. Arms: rank 16 and 32 of 64 (Pythia's axis).
PREDICTIONS (scored as written; failures preserved)
    pred_a_instrument_replays   explicit native CE within 0.002 of the stock native CE (instrument; biases carried in the low-rank path)
    pred_b_rank32_expensive     rank-32-of-64 recovery < 0.945 (Pythia-160m/410m/1b at 300B: 0.932/0.935/0.928; Qwen3 at 32 of 128: 0.966). Prior: likely under "QK-norm"
    pred_c_rank16_expensive     rank-16-of-64 recovery < 0.93 (equal FRACTION to Qwen3's rank 32 of 128 = 0.966; Pythia-300B at 16: 0.876). Prior: likely under "QK-norm"
    pred_d_kernel_energy_top4   top-4 SVD shapes carry >= 0.90 of the kernels' energy (Qwen3: 0.729, sink-dominated). Prior: unlikely (family trait)
    pred_e_rank32_values_preserved_scale rank-32 median value ratio over the 24 most valuable heads in [0.5, 2]. Prior: likely
PRICE (registered maximum): as v788 (CE batch 8 on the 152k vocab): ~13700 forwards, 600 backwards; fewer heads (336 vs 448) so fewer value ablations.
...
REPLAY_TOL, E4_MIN, REC32, REC16, RATIO = 0.002, 0.90, 0.945, 0.93, (0.5, 2.0)
PREDICTIONS = {"pred_a_instrument_replays": "+-0.002", "pred_b_rank32_expensive": "recovery < 0.945", "pred_c_rank16_expensive": "recovery < 0.93", "pred_d_kernel_energy_top4": ">= 0.90", "pred_e_rank32_values_preserved_scale": "median ratio in [0.5, 2]"}
...
def main() -> None:
    ...
    model = HB.load(REPO, "cuda"); dev = "cuda"; L, H, D, hd, kv = HB.geometry(model); rot = None; forwards = 0; backwards = 0; ROTARY = True
    hd = getattr(model.config, "head_dim", None) or hd
    QKN = {l: tuple(t.to(dev) if torch.is_tensor(t) else t for t in HB.qk_norm(model, l)) for l in range(L)}
    ...
    print(f"[{TAG}] native: stock {native_stock:.5f} | explicit {native:.5f} | geometry L={L} H={H} D={D} hd={hd} kv={kv} rotary={ROTARY} qk_norm={QKN[0][0] is not None}")
    ...
    for R in ARMS:
        ...
        def lowrank_logits(l, h, n):
            ...
            if QKN[l][0] is not None:
                q = HB.apply_qk_norm(q, QKN[l][0], QKN[l][2]); k = HB.apply_qk_norm(k, QKN[l][1], QKN[l][2])
            if ROTARY:
    ...
    predictions = {"pred_a_instrument_replays": abs(native - native_stock) <= REPLAY_TOL, "pred_b_rank32_expensive": r32["recovery"] < REC32, "pred_c_rank16_expensive": r16["recovery"] < REC16,
                   "pred_d_kernel_energy_top4": float(energy[3]) >= E4_MIN, "pred_e_rank32_values_preserved_scale": RATIO[0] <= r32["median_ratio"] <= RATIO[1]}
