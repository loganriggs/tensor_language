...
import frozen_write_recurrence_v1 as R
import source_margin_gradient as G
import attention_write_factorial_executor_v1 as S
from circuit_fast_screen_managed_runner import atomic_create_json
BINDING=POLY/'MLP8_LATE_GROUPS_V2_BINDING.json';OUT=POLY/'MLP8_LATE_GROUPS_V2_RESULT.json'
...
def main():
            ...
            for world in bank['worlds']:
                ...
                def run(b,final=False):
                    ...
                    try:
                        ...
                        if final:
                            for key,site in [('final','resid:18'),('entry8','resid:08'),('embedding','resid:00'),('attention8','attn:08')]:
                                record[key]=torch.stack([out.captured[r,site] for r in b.row_ids])
                    finally:
                ...
                a_writes=[native['attention8']]+[native['writes']['attention',i][:,-1] for i in range(9,18)]
                m_writes=[(native['mlp'][:,-1].double()-delta[:,-1]).to(native['mlp'])]+[native['writes']['mlp',i][:,-1] for i in range(9,18)]
                recurrence=R.replay(native['entry8'],native['embedding'],a_writes,m_writes,[b.lambdas for b in model.transformer.h[8:]])
                recurrence_bitwise=torch.equal(recurrence,frozen_state)
                closed_form_state=S.bridge(frozen_state.cpu().numpy(),predicted.cpu().numpy())
                bridges={'native':S.bridge(z0[:,ids],old['native_logits']),
                    ...
                    'carry_state':S.bridge(frozen_state.cpu().numpy(),recurrence.cpu().numpy()),
                ...
                reports.append({'world_id':world['world_id'],'layout':world['layout'],'foil':world['foil'],
                    ...
                    'recurrence_bitwise':recurrence_bitwise,'closed_form_state_diagnostic':closed_form_state,
                    ...
                    'instrument_passed':recurrence_bitwise and first_ok and frozen_ok and np.isfinite(cube).all() and all(b['max_abs']<=1e-3 and b['relative']<=1e-5 for b in bridges.values())})
    ...
    result={'terminal':'late_groups_complete' if preds['pred_a_instrument'] else 'invalid','predictions':preds,'reports':reports,
        ...
        'scope':'Late attention/MLP responses recomputed or frozen under MLP8 source edit and A9 native clamp. V2 repairs raw-state validation using bitwise ordered residual recurrence; v1 invalid preserved. Architectural diagnostic groups, not independent semantic circuits.'}
