#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the forty-second three spec-authored behaviours, plus one instrument.
"""v311: the AMENDED tier-3 battery over the FORTY-SECOND batch (THREE cells) -- plus one instrument.
All three are VERB_PARTICLE cells, chosen because that family has TWO members and the two families I have been feeding
are at or over the memory wall.

v285 2/7, v287 5/6, v293 6/7, v301 4/6, v303 2/4 (+instrument each).
WHY THE PARTICLE FAMILY. Separability on verb_preposition is halted (v289 lost 6 members to CUDA OOM at 72 prepared,
v295 lost 1 at 49, v297 lost 4 at 55) and the adjective family, which still completes, is at 29 prepared with roughly
two batches of runway before it reaches the same size. Every preposition cell I author shortens that runway. The
verb_particle family has exactly TWO members -- up_down (woke/calmed) and out_down (found/settled), shown separable
from each other at v250 despite sharing four of five units -- so its separability rung is cheap now and will stay cheap
for many batches. Growing the SMALL family is the way to keep counting circuits while proposal (ix) is unanswered.
Three cells, all NEW pairs, word-boundary quote-agnostic grep 0 hits (up/on, out/up, out/away):
verb_particle_up_on (piped/rambled), out_up (blurted/brightened), out_away (leaked/withered).
CPU capability margins BEFORE enqueue (mean base / mean donor on the donor axis; A1 | A2; dropped rows; floor 1.0 on A1 per the v245 lesson):
  verb_particle_up_on A1:-2.27/+2.10 drop0 A2:-1.87/+1.43 drop0 C:-1.43/+0.93 drop2
  verb_particle_out_up A1:-2.64/+2.41 drop0 A2:-2.50/+2.59 drop0 C:-1.43/+0.93 drop2
  verb_particle_out_away A1:-1.28/+1.07 drop0 A2:-1.27/+1.22 drop0 C:-1.43/+0.93 drop2
  Three cells cleared the floor with zero dropped rows (weakest A1 1.07). TWO were dropped and never entered ops/:
  verb_particle_off_away (simmered/sneaked: A1 0.62/-0.40 with 29 of 32 rows dropped, A2 0.59/-0.43 with 31 dropped --
  the donor mean is NEGATIVE, so the frame does not oblige either particle) and verb_particle_on_down
  (soldiered/quietened: A1 0.87/0.94, A2 0.86/1.18 -- under the floor on both sides of A1, a two-sided failure).
  That is the same lesson v257 and v259 recorded: the particle class is capable only where the bare intransitive verb
  OBLIGES its particle, and `soldiered`/`quietened`/`simmered`/`sneaked` are all fine standing alone.
  Batch size THREE; bars are the three-cell bars (2/2/3/2/2).
Protocol identical to v197/v201/.../v303. finiteness_selection is the instrument and is EXCLUDED from four-row counts.

REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 2 of the 3 new behaviours pass all four rows. The particle class is 2 of 6 across
                           v255/v257 (up_down and out_down passed; on_away, off_out, up_off, in_out did not), so this
                           bar is above its historical rate and is set there deliberately: all three cells here cleared
                           a capability floor that the four failures did not.                              prior 55%
...
Accounting: passes here go to a verb_particle separability rung whose family would be 5 members -- far under the wall --
so unlike the verb_preposition passes piling up in the pending pool, these can actually be counted.
An error receipt per behaviour counts as a miss on every row. ~40 s GPU each, ~2 min.
Smoke: V311_SMOKE=<out.json> (CPU, V311_SMOKE_ROWS=4 per split, V311_SMOKE_NAMES=verb_particle_up_on).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec9p_v311_result.json"
...
NAMES = {"verb_particle_up_on": "verb_particle_up_on", "verb_particle_out_up": "verb_particle_out_up",
         "verb_particle_out_away": "verb_particle_out_away",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v311", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V311_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V311_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V311_SMOKE_NAMES", "verb_particle_up_on").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec9p_v311", "candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v311", "bars": BARS,
