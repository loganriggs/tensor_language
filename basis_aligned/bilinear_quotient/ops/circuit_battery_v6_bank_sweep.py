"""circuit_battery_v6_bank_sweep -- the re-specified selectivity metric across the WHOLE bank, at each behaviour's own writer.

SS2861 validated the v6 metric on the seven attn8 successor behaviours: the two controls measure different things (median .574),
P behaves as a POSITIVE control (.056), the writer opposes the copy answer 7 of 7, the score generalises to held-out TEST (.091) and
the behaviour ordering is stable (Spearman .714). Those seven share one writer and one task family, so nothing yet says whether the
metric's behaviour is a property of the METRIC or of attn8's successor circuit in particular.

This rung applies it to every behaviour the battery identified a writer for, **at that behaviour's own writer**, on the FULL arm SS2840
published (the calibrated per-behaviour arms exist for only eight behaviours, so using them here would silently restrict the sweep;
FULL is the arm the published numbers were taken at and it needs no fitting). Splits SELECT / TEST / OOD.
...
Preregistration: polynomial_causal/CIRCUIT_BATTERY_V6_BANK_SWEEP_PREREGISTRATION.md
"""
...
SELF = Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/"
            "circuit_battery_v6_bank_sweep.py")
...
ROOT = R.ROOT
PREREG = R.POLY / "CIRCUIT_BATTERY_V6_BANK_SWEEP_PREREGISTRATION.md"
BANK21 = ROOT / "circuit_battery_v2_results.json"
RUNG = "circuit_battery_v6_bank_sweep"
SMOKE = os.environ.get("SURROGATE_SMOKE") == "1"
OUT = ROOT / (f"{RUNG}_smoke_results.json" if SMOKE else f"{RUNG}_results.json")
HASHES = {PREREG: "a5e018447aea575b6d34283d90605252076f0c20cd72d9f76f5e8461232188ac", BANK21: "5924b2549d285175c80fbf7c8fc95a8a2fa06020acc1827bc472ddea69d9ec93",
