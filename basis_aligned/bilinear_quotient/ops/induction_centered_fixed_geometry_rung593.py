#!/usr/bin/env python3
"""Prospective, CPU-testable contract and producer core for Rung 593.
...
SCHEMA = "induction_centered_fixed_geometry_rung593_v1"
DRYRUN_SCHEMA = "induction_centered_fixed_geometry_rung593_dryrun_v1"
WIDTH = 30
BATCH = 32
VOCAB = 50_304
RESIDUAL = 1_152
PAD_TOKEN = 50_256
TOLERANCE = 1e-5
INITIAL_MINIMUM_FREE_BYTES = 9_455_639_040
SELECT_MINIMUM_FREE_BYTES = 3_954_175_488
MINIMUM_FREE_BYTES = INITIAL_MINIMUM_FREE_BYTES
COMPLETE_CANONICAL_DATA_BYTES = 8_252_195_328
LARGEST_CURRENT_CHUNK_DATA_BYTES = 43_440_640
MAXIMUM_STREAMING_DATA_BYTES = 8_295_635_968
...
SUPPORT_RECORD_SHA256 = {
    "FIT": "ad2e827af9d7fada09327aa27c9465173aa283ee918599bfd5cb5ee107f79d6a",
    "SELECT": "b33ebe9b6d971dd1d09cd3ab797b888703c718457e548ed8f2b244a6698397c9",
}
SUPPORT_COUNTS = {
    "FIT": {"records": 13_824, "true": 5_760, "false": 8_064},
    "SELECT": {"records": 6_912, "true": 2_880, "false": 4_032},
}
SUPPORT_DESCRIPTOR_SHA256 = {
    "FIT": "8ad3c99504273cb41873700bf240bca42b62cae7ae42942a063f7fe853ea0f5c",
    "SELECT": "904d2eb8327a7092b40b0ad6f80c242a4a90150a03b79fd83e48452dc806ede5",
}
...
NORMAL_RESULT = ROOT / "induction_centered_fixed_geometry_rung593_results.json"
NORMAL_RECEIPT = ROOT / "induction_centered_fixed_geometry_rung593_receipt.json"
NORMAL_EVIDENCE = ROOT / "induction_centered_fixed_geometry_rung593_evidence"
INVALID_RESULT = ROOT / "induction_centered_fixed_geometry_rung593_invalid_diagnostic.json"
INVALID_RECEIPT = ROOT / "induction_centered_fixed_geometry_rung593_invalid_receipt.json"
INVALID_EVIDENCE = ROOT / "induction_centered_fixed_geometry_rung593_invalid_evidence"
PUBLIC_NAMESPACES = (
...
def require_free_space(
    ...
    if observed < minimum:
        raise RuntimeError(
            f"R593 insufficient free space before {boundary} boundary: {observed} < {minimum}"
        )
...
def verify_sources() -> dict[str, str]:
    ...
    for path, expected in SOURCE_HASHES.items():
        if not path.is_file() or sha256_file(path) != expected:
            raise RuntimeError(f"frozen R593 authority changed: {path}")
...
def load_authority() -> tuple[object, dict[str, object]]:
    ...
    verify_sources()
    r591 = _immutable_module(R591, SOURCE_HASHES[R591], "r593_pinned_r591_authority")
    ...
    r585.__r593_runtime_loader__ = r591._load_runtime
...
def expected_support_mask(specs: Sequence[Mapping[str, object]], tokens: np.ndarray) -> np.ndarray:
    """Return the exact registered A/C equality support at all four sites."""
    values = np.asarray(tokens)
    if values.shape != (len(specs), WIDTH) or values.dtype != np.dtype("<i8"):
        raise ValueError("support tokens must be exact fixed-width int64")
    support = np.empty((len(specs), len(SITES), len(ROLES)), dtype=np.bool_)
    for local, spec in enumerate(specs):
        query = int(spec["final_position"])
        payloads = list(map(int, spec["payload_positions"]))
        if len(payloads) != len(ROLES) or not 0 <= query < WIDTH:
            raise ValueError("semantic support coordinates changed")
        row = [bool(values[local, payload - 1] == values[local, query]) for payload in payloads]
        support[local] = np.asarray([row] * len(SITES), dtype=np.bool_)
    return support


def support_records(execution: Mapping[str, object], phase: str | None = None) -> list[dict[str, object]]:
    records = []
    for endpoint in execution["endpoints"]:
        if phase is not None and endpoint["split"] != phase:
            continue
        token = fixed_tokens([endpoint], "token_ids")
        support = expected_support_mask([endpoint], token)[0]
        for site_index, site in enumerate(SITES):
            for role_index, role in enumerate(ROLES):
                records.append({
                    ...
                    "site": site,
                    "role": role,
                    "expected_support": bool(support[site_index, role_index]),
                })
...
def support_summary(execution: Mapping[str, object], phase: str) -> dict[str, object]:
    records = support_records(execution, phase)
    summary = {
        "records": len(records),
        "true": sum(bool(row["expected_support"]) for row in records),
        "false": sum(not bool(row["expected_support"]) for row in records),
    }
    if summary != SUPPORT_COUNTS[phase] or content_sha256(records) != SUPPORT_RECORD_SHA256[phase]:
        raise RuntimeError(f"{phase} authority-derived support census changed")
    return {**summary, "record_sha256": content_sha256(records)}


def high_precision_native_decomposition(
    pattern: np.ndarray, values: np.ndarray, projection: np.ndarray, support: np.ndarray,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Independently contract equality, complement, and full head in float64."""
    p = np.ascontiguousarray(pattern, dtype="<f8")
    v = np.ascontiguousarray(values, dtype="<f8")
    w = np.ascontiguousarray(projection, dtype="<f8")
    mask = np.asarray(support, dtype=np.bool_)
    if p.ndim != 2 or v.ndim != 3 or p.shape != v.shape[:2] or mask.shape != p.shape:
        raise ValueError("decomposition pattern/value/support shapes changed")
    if w.ndim != 2 or w.shape[1] != v.shape[2]:
        raise ValueError("decomposition projection shape changed")
    equality_head = np.einsum("bk,bkd->bd", p * mask, v, dtype=np.float64, optimize=False)
    remainder_head = np.einsum("bk,bkd->bd", p * ~mask, v, dtype=np.float64, optimize=False)
    full_head = np.einsum("bk,bkd->bd", p, v, dtype=np.float64, optimize=False)
    equality = np.einsum("bd,od->bo", equality_head, w, dtype=np.float64, optimize=False)
    remainder = np.einsum("bd,od->bo", remainder_head, w, dtype=np.float64, optimize=False)
    full = np.einsum("bd,od->bo", full_head, w, dtype=np.float64, optimize=False)
    return tuple(np.ascontiguousarray(item, dtype="<f8") for item in (equality, remainder, full))
...
def build_phase_manifest(execution: Mapping[str, object], phase: str) -> dict[str, object]:
    ...
    for chunk, batch in enumerate(endpoint_chunks):
        ...
        expected_support = expected_support_mask(batch, values)
        ...
        calls.append({
            ...
            "expected_support_sha256": sha256_bytes(expected_support.tobytes(order="C")),
    ...
    descriptors = [
        {
            "call_index": int(call["chunk_index"]),
            "true_count": int(call["expected_support_true_count"]),
            "false_count": int(call["expected_support_false_count"]),
            "mask_sha256": str(call["expected_support_sha256"]),
        }
        for call in calls if call["call_kind"] == "endpoint"
    ]
    if content_sha256(descriptors) != SUPPORT_DESCRIPTOR_SHA256[phase]:
        raise RuntimeError(f"{phase} endpoint support descriptor manifest changed")
    phase_support = support_summary(execution, phase)
    return {"phase": phase, "calls": calls, "token_records": tensors,
            ...
            "support_summary": phase_support,
            "support_descriptor_sha256": content_sha256(descriptors)}
...
def mandatory_call_shapes(call: Mapping[str, object]) -> dict[str, tuple[np.dtype, tuple[int, ...]]]:
    ...
    parent = {
        "native_equality_term.npy": (np.dtype("<f8"), (b, 4, RESIDUAL)),
        "factorized_equality_term.npy": (np.dtype("<f4"), (b, 4, RESIDUAL)),
        "native_non_equality_remainder.npy": (np.dtype("<f8"), (b, 4, RESIDUAL)),
        "native_head_write.npy": (np.dtype("<f8"), (b, 4, RESIDUAL)),
...
def first_failure(failures: Sequence[str]) -> str | None:
    ...
    if unknown or observed.intersection(FORBIDDEN_PREDICATES):
        raise ValueError(f"unknown/forbidden R593 predicates: {sorted(unknown)}")
...
def phase_evidence_schema(phase: str) -> dict[str, object]:
    ...
    return {
        ...
        **{name: {"dtype": "float64", "shape": [ne, 4, RESIDUAL]} for name in (
        ...
        **{f"directed_{name}": {"dtype": "float64", "shape": [nd, 4, RESIDUAL]} for name in (
            "native_equality_term.npy", "native_non_equality_remainder.npy", "native_head_write.npy",
        )},
...
def canonical_array_schema(bundle: Mapping[str, object]) -> dict[str, tuple[np.dtype, tuple[int, ...]]]:
    ...
    for name in (
        ...
        endpoint[name] = (np.dtype("<f8"), (endpoint_rows, 4, RESIDUAL))
    for name in (
        "factorized_equality_term.npy",
        "native_full_attention_write.npy", "independent_full_native_write.npy",
    ):
    ...
    for name in (
        ...
        directed["directed_" + name] = (np.dtype("<f8"), (directed_rows, 4, RESIDUAL))
    for name in (
        "factorized_equality_term.npy",
        "native_full_attention_write.npy", "independent_full_native_write.npy",
    ):
...
def publish_invalid_prefix(
    ...
    diagnostic = {
        "schema": "induction_centered_fixed_geometry_rung593_invalid_diagnostic_v1",
    ...
    receipt = {
        "schema": "induction_centered_fixed_geometry_rung593_invalid_receipt_v1",
...
def evaluate_completed_call(
    ...
    if kind in ("endpoint", "native") and not nonfinite:
        ...
        if kind == "endpoint":
            observed_support = np.asarray(arrays["support.npy"], dtype=np.bool_)
            expected_support = None if cached is None else cached.get("expected_support")
            if expected_support is None:
                details["support_contract_error"] = "missing authority-derived expected support"
                failures.append("factor_transport_failed")
            else:
                expected_support = np.asarray(expected_support, dtype=np.bool_)
                mismatch_count = (
                    int(np.count_nonzero(observed_support != expected_support))
                    if expected_support.shape == observed_support.shape else -1
                )
                details["support_true_count"] = int(observed_support.sum())
                details["support_false_count"] = int((~observed_support).sum())
                details["support_mismatch_count"] = mismatch_count
                details["expected_support_sha256"] = sha256_bytes(
                    np.ascontiguousarray(expected_support).tobytes(order="C")
                )
                if (
                    mismatch_count != 0
                    or details["support_true_count"] != int(call["expected_support_true_count"])
                    or details["support_false_count"] != int(call["expected_support_false_count"])
                    or details["expected_support_sha256"] != call["expected_support_sha256"]
                ):
                    failures.append("factor_transport_failed")
...
def run_manifest_calls(
    ...
    *, stage: Path, public_root: Path = ROOT, store: StreamingPhaseStore | None = None,
    invalid_provenance: Mapping[str, object] | None = None,
) -> dict[str, object]:
...
def make_context_factory(execution: Mapping[str, object], bundle: Mapping[str, object]):
    ...
    def factory(call: Mapping[str, object], outputs: Mapping[str, Path]) -> dict[str, object]:
        kind = str(call["call_kind"])
        if kind == "endpoint":
            specs = [endpoints[str(value)] for value in call["authority_row_ids"]]
            expected = expected_support_mask(specs, bundle["token_arrays"][call["token_record_id"]])
            return {"specs": specs, "cached": {"expected_support": expected}}
...
def validate_phase_support_evidence(
    phase_root: Path, execution: Mapping[str, object], phase: str,
) -> dict[str, object]:
    endpoints = [row for row in execution["endpoints"] if row["split"] == phase]
    expected = expected_support_mask(endpoints, fixed_tokens(endpoints, "token_ids"))
    observed = np.load(phase_root / "support.npy", mmap_mode="r", allow_pickle=False)
    if observed.dtype != np.dtype("bool") or observed.shape != expected.shape or not np.array_equal(observed, expected):
        raise RuntimeError(f"{phase} canonical support differs from authority-derived mask")
    summary = support_summary(execution, phase)
    if int(observed.sum()) != summary["true"] or int((~observed).sum()) != summary["false"]:
        raise RuntimeError(f"{phase} canonical support census changed")
    del observed
    return {
        "support_record_sha256": summary["record_sha256"],
        "support_record_count": summary["records"],
        "support_true_count": summary["true"],
        "support_false_count": summary["false"],
    }
...
def finalize_streamed_phase_evidence(
    ...
    gate_summary = {
        **_maximum_reconstruction_errors(phase_root),
        **validate_phase_support_evidence(phase_root, execution, phase),
    }
...
def publish_normal(
    ...
    receipt = {
        "schema": "induction_centered_fixed_geometry_rung593_receipt_v1",
...
def run_science(*, public_root: Path = ROOT) -> dict[str, object]:
    ...
    started = time.time()
    implementation_sha256 = globals().get("__r593_immutable_sha256__")
    adapter_sha256 = globals().get("__r593_adapter_sha256__")
    if implementation_sha256 is None or adapter_sha256 is None:
        raise RuntimeError("R593 science requires immutable managed producer and adapter provenance")
    public = tuple(public_root / path.name for path in PUBLIC_NAMESPACES)
    occupied = [str(path) for path in public if path.exists()]
    if occupied:
        raise RuntimeError(f"R593 public namespace occupied: {occupied}")
    ...
    runtime = _immutable_module(RUNTIME, SOURCE_HASHES[RUNTIME], "r593_pinned_model_runtime")
    # This constructor is the first permitted torch/checkpoint/CUDA boundary.
    executor = runtime.R593ModelExecutor(types.SimpleNamespace(**globals()), r585)
    r585 = executor.r585
    invalid_provenance = {
        "implementation_sha256": str(implementation_sha256),
        "adapter_sha256": str(adapter_sha256),
        "runtime_sha256": SOURCE_HASHES[RUNTIME],
        "checkpoint_weights_sha256": str(executor.checkpoint_sha256),
        "source_sha256": verify_sources(),
    }
    stage = Path(tempfile.mkdtemp(prefix=".r593-stage-", dir=public_root))
    ...
    try:
        fit = run_manifest_calls(
            ...
            stage=stage, public_root=public_root, invalid_provenance=invalid_provenance,
        )
...
def build_dryrun() -> dict[str, object]:
    ...
    audit_c, audit_r, audit_h = high_precision_native_decomposition(
        audit_pattern, audit_values, audit_projection, audit_support
    )
    audit_error = float(np.max(np.abs(audit_c + audit_r - audit_h)))
    planted_r = audit_r.copy()
    planted_r[0, 0] += 2e-5
    planted_error = float(np.max(np.abs(audit_c + planted_r - audit_h)))
    if audit_error > TOLERANCE or planted_error <= TOLERANCE:
        raise RuntimeError("high-precision decomposition falsifier is not live")
    return {
        ...
        "support_summary": {"FIT": fit["support_summary"], "SELECT": select["support_summary"]},
        "support_descriptor_sha256": {
            "FIT": fit["support_descriptor_sha256"],
            "SELECT": select["support_descriptor_sha256"],
        },
        ...
        "high_precision_decomposition_fixture": {
            "native_head_rms": float(np.sqrt(np.mean(np.square(audit_h)))),
        ...
        "streaming_storage": {
            ...
            "r592_prior_full_tree_peak_data_bytes": 10_677_399_552,
        },
