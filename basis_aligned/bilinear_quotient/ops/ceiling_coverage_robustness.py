# DOES THE CEILING RESULT SURVIVE A DIFFERENT COVERED SET?  -- second-class confirmation of §1848.
#
# §1848 and §1849 both found the full-rank 36-site composition attains the model's own per-token ceiling
# on covered positions EXACTLY, -0.000000 nats on three roles. That is the strongest positive claim in
# the §1829-§1850 arc and it is what §1850's frontier restatement rests on -- and every measurement of it
# so far uses ONE covered set: the 5419 token types in `fineweb_n96_skip80`.
#
# The house pattern for a claim this load-bearing is a second-class confirmation that varies the thing
# most likely to matter. Here that is the COVERED SET itself. `fineweb_n480_skip80` is five times the fit
# rows, so it covers more token types, changes which positions are scored as covered, refits the
# embedding->row map over a different basis, and rebuilds the output-NN fallback over a different
# neighbour pool. If the exact attainment is a property of the construction it must survive all of that;
# if it is an artefact of the particular 5419, it will not.
#
# §1765's derivation says it must survive -- a length-1 forward computes the fully compiled program's
# function for ANY token, so coverage cannot matter. This run exists to test that derivation rather than
# to trust it, which is the same discipline §1849 applied to §1848's attribution and which caught it.
#
# ROLES. All three. Ranks full and 64 only -- the ladder is §1849's object, attainment is this one.
# DISCOVERY ONLY.
#
...
#   pred_a ATTAINMENT SURVIVES A LARGER COVERED SET: with the n480 fit rows, the full-rank covered CE
#          differs from the ceiling by under 0.001 nats on every role. If TRUE §1848 is confirmed second
#          class and §1765's derivation is shown to hold independent of coverage, which is what licenses
#          §1850's frontier. If FALSE the exact attainment was a property of the 5419-type covered set,
#          §1848's headline needs restating as a coverage-specific result, and §1850 must be withdrawn.
#   pred_b AND COVERAGE ACTUALLY CHANGED: the n480 covered set exceeds 5419 types by at least 20%. This
#          is the known-answer check that the run varied what it claims to vary (LESSON 46) -- without
#          it, pred_a passing would mean nothing. If FALSE the two fit sets cover nearly the same types
#          and this is not a second class at all.
#   pred_c AND THE RANK-64 SHORTFALL MOVES LITTLE: the rank-64 CE above the ceiling stays within 0.05
#          nats of §1849's PUBLISHED +0.22327 / +0.23252 / +0.19774 (settled build, n96 fit rows). If
#          FALSE the rank ladder is coverage-dependent and §1850's frontier is specific to one fit set.
#   pred_d CONTROLS: live covered CE reproduces §1768's PUBLISHED 3.29205 / 3.09711 within 0.01 -- it is
#          a property of the model and the scored population, so a changed covered set moves the SCORED
#          SET and this control must be read on the same population it is quoted for; the ceiling is
#          rebuilt over the new covered set and must remain finite and above live on every role.
...
OUT = PT + 'ops/ceiling_coverage_robustness_results.json'
...
TRANKS = (None, 64)          # None = full rank; §1786/§1813 use this ladder
...
S1849_RANK64_ABOVE = {'skip7000': 0.22327, 'skip11000': 0.23252, 'skip1200': 0.19774}
...
def main():
    ...
    print(f'CEILING COVERAGE ROBUSTNESS | n480 fit rows, ranks full/64 | '
    ...
    pb = NFULL >= 1.20 * NCOV
    lad = ['64', 'full']
    pc = all(abs((curve['64'][e]['cov'] - ceilv[e]) - S1849_RANK64_ABOVE[e]) <= 0.05
             for e in roles2 if e in S1849_RANK64_ABOVE)
    ...
    print(f'  and the rank-64 shortfall moves <0.05 from §1849 -> {pc}  ' + '  '.join(
        f'{e} +{curve["64"][e]["cov"] - ceilv[e]:.5f} vs §1849 +{S1849_RANK64_ABOVE[e]:.5f}'
        for e in roles2 if e in S1849_RANK64_ABOVE), flush=True)
    print(f'  ceiling/live §1768, full-rank all-position §1811 -> control {pd}', flush=True)

    json.dump({'run': 'ceiling_coverage_robustness', 'n_covered': NFULL, 'ranks': [str(r) for r in TRANKS],
               ...
               'predictions': {'pred_a_attainment_survives_coverage': bool(pa),
                               ...
                               'pred_c_rank64_shortfall_stable': bool(pc),
