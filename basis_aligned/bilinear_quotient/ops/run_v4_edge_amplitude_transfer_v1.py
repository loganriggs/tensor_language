...
"""V4_EDGE_AMPLITUDE_TRANSFER_V1_PREREGISTRATION.md; unchanged oracle source edits."""
import os,sys,json,time,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';PLAN=dict(prefix=12,native_suffix=64,joined_suffix=84)
def main():
 ...
 out=A/'v4_edge_amplitude_transfer_v1_result.json';assert not out.exists();choicepath=P/'V4_FINITE_REFERENCE_CHOICES_V1.json';choices=json.loads(choicepath.read_text());prior=json.loads((A/'v4_finite_reference_v1_result.json').read_text());contexts=[];composition=json.loads((A/'v4_selective_composition_v1_result.json').read_text());field_errors=[];programs=[];cores=[];local_errors=[];export=None
 ...
 for context in contexts:
  ...
  for sv,av in [(1.,1.),(.5,1.),(1.,.5),(-1.,1.),(1.,-1.),(2.,1.),(1.,2.)]:
   ...
   estimates={}
   for label,post in [('carry',carry),('exact',exact),('linear',linear)]:
 ...
 result=dict(plan=PLAN,counts=counts,max_replay=max(replays),max_local_edge_relative_error=max(local_errors),records=records,predictions=dict(pred_a_instrument=instrument,pred_b_carry_transfer=instrument and all(r['passed'] for r in records if r['arm']=='carry'),pred_c_linear_transfer=instrument and all(r['passed'] for r in records if r['arm']=='linear')),seconds=time.perf_counter()-tic)
