#!/usr/bin/env python
"""all18_tail_linear_program_probe -- PROGRAM v4, TAIL-LINEAR: every MLP (0-17) quadratic on its k-core (own frames early, bus U_8 late) and
core-GATED LINEAR on the tail (the tail*tail term dropped; §2780/§2783: 7-10% of the tail's value, per half). Attention reads, writes and
the readout exact. Costs at k = 768 / 512 / 384, against the parts (§2780 .0087 late + §2783 .0023 early at 768). CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_all18_tail_linear_768_cheap pred_c_all18_tail_linear_512_cheap
#                     pred_d_late_tt_share_512_small pred_e_all18_near_additive_768
...
Preregistration: polynomial_causal/ALL18_TAIL_LINEAR_PROGRAM_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/all18_tail_linear_program_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("all18_tail_linear_program_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "ALL18_TAIL_LINEAR_PROGRAM_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "early_tail_product_term_probe_results.json"   # §2783
OUT = ROOT / "all18_tail_linear_program_probe_results.json"
HASHES = {PREREG: "c136f7cb38f102079aa984244973c9757e6406b75cf53c1a5a52028f88e7ef50", PRIOR: "02a246b246b2fc2607a13a84d63441a8aa6100747410cd16a0167598762f047d",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "all18_tail_linear_program_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_max": 0.020, "c_max": 0.050, "d_max": 0.15, "e_factor": 1.3}
NULLS = {"b_min": 0.050, "c_min": 0.100, "d_min": 0.30, "e_factor": 2.0}
...
def main():
    ...
    class TermHead(OwnHead):
        """MLP l reads a k-dim frame (own site frame if U_ is None, else U_): c = the truncated input (rest constant), t = the dropped tail.
        Left/Right bias-free: Left(c+t)*Right(c+t) = Lc*Rc + (Lc*Rt + Lt*Rc) + Lt*Rt exactly. keep in {'all','cross','tt','none'}."""
        def __init__(self, l, k, keep, U_=None):
            super().__init__(l, k, "const", None if U_ is None else U_[:, :k]); self.keep = keep
        def __call__(self, w, x):
            ...
            if self.keep == "all":
                prod = (Lc + Lt) * (Rc + Rt)
            elif self.keep == "cross":
                prod = Lc * Rc + Lc * Rt + Lt * Rc
            elif self.keep == "tt":
                prod = Lc * Rc + Lt * Rt
            else:
                prod = Lc * Rc
    ...
    def early(k, keep):
        return run({("mlp", l): TermHead(l, k, keep) for l in range(0, SPLIT)})
    def late(k, keep):
        return run({("mlp", l): TermHead(l, k, keep, U_8) for l in range(SPLIT, NL)})
    def all18(k, keep):
        patch = {("mlp", l): TermHead(l, k, keep) for l in range(0, SPLIT)}
        patch.update({("mlp", l): TermHead(l, k, keep, U_8) for l in range(SPLIT, NL)})
        return run(patch)
    arms = {"SPLIT8_1024": split8(1024), "LATE_DROP_TT_768": late(768, "cross"), "EARLY_DROP_TT_768": early(768, "cross"), "ALL18_DROP_TT_768": all18(768, "cross")}
    if not smoke:
        arms["ALL18_DROP_TT_512"] = all18(512, "cross"); arms["ALL18_DROP_TT_384"] = all18(384, "cross"); arms["ALL18_MLP_768"] = all18(768, "none")
        arms["LATE_MLP_512"] = late(512, "none"); arms["LATE_DROP_TT_512"] = late(512, "cross")
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    A768 = arms["ALL18_DROP_TT_768"]; A512 = arms.get("ALL18_DROP_TT_512"); A384 = arms.get("ALL18_DROP_TT_384"); parts = arms["LATE_DROP_TT_768"] + arms["EARLY_DROP_TT_768"]
    lj = arms.get("LATE_MLP_512"); lt = arms.get("LATE_DROP_TT_512"); late_tt_share_512 = (lt / lj) if (lj is not None and lt is not None and lj > 1e-9) else None
    summ = {"all18_drop_tt": {"768": A768, "512": A512, "384": A384}, "all18_core_only_768": arms.get("ALL18_MLP_768"), "parts_768": parts, "all18_over_parts_768": A768 / max(parts, 1e-9),
            "late_512": {"joint": lj, "drop_tt": lt, "tt_share": late_tt_share_512}}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(arms["LATE_DROP_TT_768"] - PRIOR_DROP_TT_768) <= BARS["repro_tol"] and abs(arms["EARLY_DROP_TT_768"] - PRIOR_EARLY_DROP_TT_768) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_all18_tail_linear_768_cheap': bool(A768 <= BARS["b_max"]),
        'pred_c_all18_tail_linear_512_cheap': bool(A512 is not None and A512 <= BARS["c_max"]),
        'pred_d_late_tt_share_512_small': bool(late_tt_share_512 is not None and late_tt_share_512 <= BARS["d_max"]),
        'pred_e_all18_near_additive_768': bool(A768 <= BARS["e_factor"] * parts),
    }
    nulls = {"b_null_all18_768_ge_.05": bool(A768 >= NULLS["b_min"]), "c_null_all18_512_ge_.10": bool(A512 is not None and A512 >= NULLS["c_min"]),
             "d_null_late_tt_share_512_ge_.30": bool(late_tt_share_512 is not None and late_tt_share_512 >= NULLS["d_min"]), "e_null_all18_768_ge_2x_parts": bool(A768 >= NULLS["e_factor"] * parts)}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "PROGRAM v4 (tail-linear): all 18 MLPs quadratic on their k-core (own frames for blocks 0-7, bus U_8 for 8-17) and core-gated LINEAR on the tail (tail*tail dropped); attention, writes, readout exact",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "late_drop_tt_768": arms["LATE_DROP_TT_768"], "prior_drop_tt_768": PRIOR_DROP_TT_768, "early_drop_tt_768": arms["EARLY_DROP_TT_768"], "prior_early_drop_tt_768": PRIOR_EARLY_DROP_TT_768},
