#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_native_replays pred_b_sink_program_recovers pred_c_sink_beats_kernel pred_d_sink_weight_positive pred_e_rank16_not_better
"""Attention lane, v733: the sink program at a fine grid of c (v732: c = 0.5 recovers 0.53, 0.25 and 1.0 are far worse).

v730: 5.7's kernel-only program is catastrophic (recovery -5.6) because an offset kernel cannot say 'position 0' (the read is absolute, not
relative; v700: argmax at position 0 for 99.5% of queries, value norm 771). Edit: 5.7's off-diagonal pattern := kappa(d) for keys j >= 1
and := c for key j = 0 (every query), c on a grid; one head inside the native model; against its value 0.0124 (v701) and its closed-form
rank-16 program. Response only. CE ADDED; recovery = 1 - cost / value.
PREDICTIONS (scored as written; failures preserved)
    pred_a_native_replays       native within 0.002 of 3.13241 (instrument)
    pred_b_sink_program_recovers at the best c the program recovers >= 0.8 of 5.7's value. Prior: likely (the sink write is the mean)
    pred_c_sink_beats_kernel     best cost <= kernel-only - 0.01. Prior: certain given v730 (registered anyway)
    pred_d_sink_weight_positive  the best c is > 0. Prior: likely
    pred_e_rank16_not_better     the rank-16 closed-form program does NOT beat the best one-number program by more than 0.001. Prior: unsure
PRICE (registered maximum): native 6; 9 values of c x 6 = 54; rank-16 kappa_r 2 + 6; total 68 forwards; 0 backwards; 0 fits. Bar <= 80.
"""
...
OUT = ROOT / "circuits/followups/attention_sink_fine_v733_result.json"
...
NATIVE_V615 = 3.13241
CANDIDATE_ID = "attention.sink_fine_v733"
FORWARDS_MAX = 80
EBATCH = 32
L_, H_ = 5, 7; H = 9
BETAS = (0.0, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.7, 0.8)
REPLAY_TOL, REC_MIN, GAIN_MIN, R16_MIN = 0.002, 0.8, 0.01, 0.001
PREDICTIONS = {"pred_a_native_replays": "+-0.002", "pred_b_sink_program_recovers": "recovery >= 0.8", "pred_c_sink_beats_kernel": "best - kernel-only <= -0.01",
               "pred_d_sink_weight_positive": "best c > 0", "pred_e_rank16_not_better": "rank-16 > best - 0.001"}
...
def main() -> None:
    ...
    def patched(q, k, v, q2, k2):
        ...
        if state["mode"] is not None:
            ...
            if state["mode"] == "indicator":
                prog = kappa[dmat][None].expand(Bn, Tn, Tn).clone(); prog[:, :, 0] = state["beta"]
            else:
    ...
    predictions = {"pred_a_native_replays": abs(native - NATIVE_V615) <= REPLAY_TOL, "pred_b_sink_program_recovers": (1 - costs[best] / value) >= REC_MIN,
                   "pred_c_sink_beats_kernel": costs[best] <= costs[0.0] - GAIN_MIN, "pred_d_sink_weight_positive": best > 0, "pred_e_rank16_not_better": c16 > costs[best] - R16_MIN}
