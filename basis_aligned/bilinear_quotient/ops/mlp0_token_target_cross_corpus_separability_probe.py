#!/usr/bin/env python3
"""Is the exact linear separator of R536's token target corpus-specific? (CPU probe, zero forwards)
...
Preregistration: polynomial_causal/MLP0_TOKEN_TARGET_CROSS_CORPUS_SEPARABILITY_PROBE_PREREGISTRATION.md
"""
...
PREREG = POLY / "MLP0_TOKEN_TARGET_CROSS_CORPUS_SEPARABILITY_PROBE_PREREGISTRATION.md"
LIB1 = ROOT / "ops" / "mlp0_hybrid_separability_lib.py"
LIB2 = ROOT / "ops" / "mlp0_hybrid_separability_corpus_lib.py"
FROZEN = ROOT / "mlp0_token_target_linear_separability_probe_results.json"
NAT = Path(CL.NAT); CODE = Path(CL.CODE)
OUT = ROOT / "mlp0_token_target_cross_corpus_separability_probe_results.json"
HASHES = {
    PREREG: "30ad933e41ff301b78686d52f03e9f9ed0d2da561ade272ef4fbe74a50e67d5b", LIB1: "0168d99083c003846b4f14e62dec645e3c801c725201a2bd0c681b0147ae28c8", LIB2: "c1f190905fab4955e26da98e5b026e98faa3f9e3c08988ae0709a69faf62b268", FROZEN: "a60fed8ad16c9db5c6c0f4b2a6fb387b86dbd904ea6ee81590dc97789146b302", NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1", CODE: "6cf514e75dfd03399f223a9ba5f6ebe5f4b1315bcb839a515e1c19e7b5474bd9",
}
RUNG = "mlp0_token_target_cross_corpus_separability_probe"
RHO = 1.0
...
def stats_for(L, R, p, w):
    _, Sp, _ = CL.weighted_token_moments(p, w)
    St, _ = CL.weighted_cov_gT(L, R, p, w)
    Sn = LIB.cov_gI_given_token_moment(L, R, Sp, RHO)
    return St, Sn
...
def main():
    ...
    result = {
        ...
        "target": "token", "rho": RHO, "distinct_tokens": distinct, "n_tokens": W["n_tokens"],
