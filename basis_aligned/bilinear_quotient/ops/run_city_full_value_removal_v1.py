...
pred_e>=2median/16nulls.800forwards.
...
STEM='CITY_FULL_VALUE_REMOVAL_V1';ARMS=['native','native8_midpoint','full_reentry','compiled']+[f'null:{k}' for k in range(16)]
...
def main():
 ...
 def pre9(module,args):
  ...
  if arm=='native':c['x9']=x.clone()
   ...
   row=groups[state['i']];seed=17092800+1000*int(arm.split(':')[1])+row['context_id'];generator=torch.Generator(device=x.device).manual_seed(seed)
 ...
 old=torch.load(P/'CITY_VALUE_BRANCH_FACTORIAL_V1_ARTIFACT.pt',weights_only=True)['values'];native_anchor=float((v[:2]-old[[0,3]]).abs().max())
 result={'native_factorial_anchor':native_anchor,'pred_a':native_anchor<=1e-5 and anchor_abs<=1e-4 and anchor_rel<=1e-5 and max(joint_errors)<=1e-4 and max(outside)==0 and max(norm_errors)<=1e-5 and bool(torch.isfinite(v).all()) and count==800,
