# IS THE INPUT THE CONSTRAINT?  -- the same oracle, fed the length-1 STREAM instead of the embedding.
#
# §1873 closed the linear family on the token embedding: an ORACLE map least-squares fitted on the
# uncovered rows it is asked to predict still loses +0.65138 / +0.72189 / +0.70037 nats against §1868's
# uncovered-token ceiling, so only ~17% of the deployed map's loss is generalisation and 83% is
# representational.
#
# The target is not mysterious -- it is the token's own length-1 output at that site. And the compiled
# program, by the time it needs site (kind, L)'s row, has already computed the residual stream at layers
# below L, which is a strictly RICHER function of the token than the raw embedding and free at inference.
#
# This runs §1873's oracle again with that input substituted: for each site, the map is fitted from the
# LENGTH-1 RESIDUAL STREAM entering that site to the site's row, on the uncovered tokens themselves. It
# is the same not-deployable bound §1873 used, changed in exactly one place, so the difference between
# the two numbers is attributable to the input representation and nothing else.
#
# ROLES. All three, deployed 5,419-type covered set. DISCOVERY ONLY. Rung 3: §1873's open question.
#
...
# ceiling.
#   pred_a THE INPUT IS THE CONSTRAINT: the stream-input oracle's loss is below half §1873's PUBLISHED
#          embedding-input oracle loss of +0.65138 / +0.72189 / +0.70037, on all three roles. If TRUE the
#          representational barrier is the embedding and not the per-token restriction -- a richer input
#          the program already has would carry these rows, and the fallback becomes an engineering
#          problem rather than a closed one. If FALSE the barrier survives a strictly richer input, per
#          -token functions are simply the limit whatever they are fed, and the fallback -- and with it
#          the last open lever in the position-wise class -- is closed.
#   pred_b AND IT STILL BEATS THE DEPLOYED MAP: below §1872's PUBLISHED +0.78075 / +0.86225 / +0.83997 on
#          all three roles. A strictly richer input fitted with the answers in hand should not do worse;
#          if it does, the fit is ill-conditioned and nothing else in the run can be read.
#   pred_c AND THE COVERED ARM IS UNTOUCHED: covered CE identical to the deployed arm within 1e-6. Fifth
#          KNOWN-ANSWER check on the same invariant; the previous four returned exactly 0.00e+00.
#   pred_d CONTROLS: the deployed MAP arm reproduces §1872's PUBLISHED values within 0.005; coverage is
#          exactly 5,419; the covered arm attains its ceiling (a fourteenth confirmation); every
...
OUT = PT + 'ops/oracle_stream_input_results.json'
...
def main():
    ...
    print(f'ORACLE STREAM INPUT | 5419 types, oracle fed the length-1 stream not the embedding | '
    ...
    pa = all(fbloss['STREAM'][e] < 0.5 * S1873_ORACLE_EMB[e] for e in roles2)
    pb = all(fbloss['STREAM'][e] < S1872_MAP_M64[e] for e in roles2)
    pc = covspread <= 1e-6
    ...
    print(f'\n  the INPUT is the constraint (stream oracle < half the embedding '
          f'oracle) -> {pa}  ' + '  '.join(
        f'{e} {fbloss["STREAM"][e]:+.5f} vs {0.5 * S1873_ORACLE_EMB[e]:.5f}' for e in roles2),
        flush=True)
    print(f'  and it still beats the DEPLOYED map -> {pb}  ' + '  '.join(
    ...
    json.dump({'run': 'oracle_stream_input', 'n_covered': NFULL, 'arms': list(ARMS2),
               ...
               'predictions': {'pred_a_input_is_the_constraint': bool(pa),
                               'pred_b_beats_deployed_map': bool(pb),
