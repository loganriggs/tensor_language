...
Null: five-bank full-branch execution amplifies numerical errors or breaks prior interaction fidelity.
...
@torch.no_grad()
def main():
    files=json.loads((P/'DIAGONAL_MLP10_NATIVE_V1_BINDING.json').read_text())['files']
    ...
    assert not (P/'DIAGONAL_MLP10_NATIVE_V1_RESULT.json').exists()
    ...
    previous=json.loads((P/'COMPOSED_JOINT_NATIVE_FULL_V1_RESULT.json').read_text())['cells']
    ...
    result={'pred_a':max(max(c['state_errors']) for c in cells)<=1e-5 and old_error<=1e-4,
            ...
            'scope':'160 historical prefixes, full original two-edit interaction; generated child/remainder/parent branches, native pristine context/scalar fields/dense weights/suffix. Five-bank normalized MLP10 in FP64 after rounded generated z10, output rounded FP32; no freshOOD, fitting or whole-model compression claim.'}
    (P/'DIAGONAL_MLP10_NATIVE_V1_RESULT.json').write_text(json.dumps(result,indent=2)+'\n')
