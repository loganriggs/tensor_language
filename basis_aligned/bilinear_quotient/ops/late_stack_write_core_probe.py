#!/usr/bin/env python
"""late_stack_write_core_probe -- do the fourteen late sublayers WRITE into one shared subspace, and is it the subspace they READ
(§2742/§2745)? Own / shared write cores at k = 256/512/768, writes on the read core, and reads + writes together. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_shared_write_core_is_free pred_c_write_768 pred_d_write_subspace_is_the_read_subspace
#                     pred_e_read_and_write_together
...
Preregistration: polynomial_causal/LATE_STACK_WRITE_CORE_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_stack_write_core_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_stack_write_core_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_STACK_WRITE_CORE_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_attention_shared_input_core_probe_results.json"   # §2745
OUT = ROOT / "late_stack_write_core_probe_results.json"
HASHES = {PREREG: "754a5dbe7967c1f933cf77e561ecc36e85292043ddb8ffb659c23ed47840a8da", PRIOR: "0711ac84c85961cf8263200e6fbec0e629dda8e85d524651c733f2a6941c0abe",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_stack_write_core_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_LATE14_JOINT_768 = 0.109; LAM = 1e-2
K = 768; KW = [256, 512, 768]; LATE = list(range(11, 18))
SITES = [("mlp", l) for l in LATE]; ASITES = [("attn", l) for l in LATE]
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_max": 0.03, "c_max": 0.15, "d_max": 0.05, "e_max": 0.30}
NULLS = {"b_min": 0.10, "c_min": 0.40, "d_min": 0.20, "e_min": 0.50}
...
def main():
    ...
    class WriteProj:
        """Site s's write replaced by mu_s + P (w - mu_s), P = U U^T on a k-dim write basis (own or shared)."""
        def __init__(self, s_, k, U_=None):
            h = wheads[s_]; self.U = (h["U"] if U_ is None else U_)[:, :k]; self.mu = h["mu"]
        def hook(self, idx):
            pass
        def __call__(self, w, x):
            return self.mu + ((w - self.mu) @ self.U) @ self.U.T
    class Both:
        """Read-side replacement (OwnHead / AttnHead) followed by a write-side projection on the same site."""
    ...
    def core_of(hs, key):
        C = sum(hs[s_][key] for s_ in SITES + ASITES) / len(SITES + ASITES); _, U_ = torch.linalg.eigh(C); return U_.flip(1).float()
    U_read = core_of(heads, "Cx"); U_write = core_of(wheads, "Cw")
    def capture(C, U_):
        return float((U_.double().T @ C @ U_.double()).trace() / C.trace())
    wcap = {f"{s_[0]}{s_[1]}": {"own": capture(wheads[s_]["Cw"], wheads[s_]["U"][:, :K]), "shared_write": capture(wheads[s_]["Cw"], U_write[:, :K]), "read_core": capture(wheads[s_]["Cw"], U_read[:, :K])} for s_ in SITES + ASITES}
    log(stage="cores", write_eff_rank={f"{s_[0]}{s_[1]}": round(wheads[s_]["eff_rank"], 1) for s_ in SITES + ASITES}, wcap_read_over_own={s: round(v["read_core"] / v["own"], 3) for s, v in wcap.items()})
    def run(parts):
        patch = {}; objs = []
        for p_ in parts:
    ...
    reader = lambda s_, U_: OwnHead(s_[1], K, "const", U_[:, :K]) if s_[0] == "mlp" else AttnHead(s_[1], K, U_)
    wblk = lambda k, U_=None: {s_: WriteProj(s_, k, U_) for s_ in SITES + ASITES}
    arms = {"LATE14_JOINT_768": run([{s_: reader(s_, U_read) for s_ in SITES + ASITES}])}
    for k in KW:
        arms[f"WRITE14_OWN_{k}"] = run([wblk(k)])
    for k in KW:
        arms[f"WRITE14_SHARED_{k}"] = run([wblk(k, U_write)])
    arms["WRITE14_ON_READ_CORE_768"] = run([wblk(K, U_read)])
    arms["READ_WRITE_768"] = run([{s_: Both(reader(s_, U_read), WriteProj(s_, K, U_write)) for s_ in SITES + ASITES}])
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    gaps = {"shared_minus_own_768": arms["WRITE14_SHARED_768"] - arms["WRITE14_OWN_768"], "on_read_core_minus_shared_768": arms["WRITE14_ON_READ_CORE_768"] - arms["WRITE14_SHARED_768"],
            "read_write_minus_read_plus_write": arms["READ_WRITE_768"] - arms["LATE14_JOINT_768"] - arms["WRITE14_SHARED_768"]}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["LATE14_JOINT_768"] - PRIOR_LATE14_JOINT_768) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_shared_write_core_is_free': bool(gaps["shared_minus_own_768"] <= BARS["b_max"]),
        'pred_c_write_768': bool(arms["WRITE14_SHARED_768"] <= BARS["c_max"]),
        'pred_d_write_subspace_is_the_read_subspace': bool(gaps["on_read_core_minus_shared_768"] <= BARS["d_max"]),
        'pred_e_read_and_write_together': bool(arms["READ_WRITE_768"] <= BARS["e_max"]),
    }
    nulls = {"b_null_ge_.10": bool(gaps["shared_minus_own_768"] >= NULLS["b_min"]), "c_null_ge_.40": bool(arms["WRITE14_SHARED_768"] >= NULLS["c_min"]),
             "d_null_ge_.20": bool(gaps["on_read_core_minus_shared_768"] >= NULLS["d_min"]), "e_null_ge_.50": bool(arms["READ_WRITE_768"] >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "k": K, "ks_write": KW,
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "late14_joint_768": arms["LATE14_JOINT_768"], "prior_late14_joint_768": PRIOR_LATE14_JOINT_768},
           "ce_added": arms, "gaps": gaps, "write_eff_rank": {f"{s_[0]}{s_[1]}": wheads[s_]["eff_rank"] for s_ in SITES + ASITES}, "write_capture_768": wcap,
