"""METRIC-WEIGHTED FRONT v1 -- refit the front's constrained pieces under site-local observability metrics.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['stream_error_profile_results.json','observable_correction_results.json',
           'front_piece_amplification_results.json']
    ...
    print(f"DRYRUN OK: S2086 present (block 6 {_p['6']}); three matched-context arms (plain / metric / random-metric)")
    raise SystemExit(0)
import json, sys, time, torch
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
import os
...
OUT=PT+'metric_front_refit_results.json'
...
def main():
    ...
    def build_arm(mode):
        ...
        tag={'plain':'E','metric':'M','random-metric':'R'}[mode]
        act=['a0']
    ...
    res={}
    for mode in ('plain','metric','random-metric'):
        ts=time.time()
        cfg=build_arm(mode)
    ...
    E=res['plain']
    gain=E['gap']-res['metric']['gap']; gain_r=E['gap']-res['random-metric']['gap']
    drop=1-res['metric']['block6_error_energy_observable']/max(E['block6_error_energy_observable'],1e-9)
    pa=gain>=0.15; pb=drop>=0.20; pc=gain_r<=0.05; pd=abs(E['profile'][6]-1.7415)<=0.10
    out={'base_ce':round(base,5),'arms':res,'gain_metric':round(gain,4),'gain_random_metric':round(gain_r,4),
         'observable_energy_drop_metric':round(drop,4),'eval_r90_block6':r90e,'S2086_block6':1.7415,
         'pred_a_metric_beats_cfgE':bool(pa),'pred_b_moves_observable_energy':bool(pb),
         'pred_c_random_metric_control':bool(pc),'pred_d_reproduces_S2086':bool(pd),'self_reviewed':True,
         'runtime_s':round(time.time()-t0,1)}
    json.dump(out,open(OUT,'w'),indent=1)
    print(f"(d) cfgE block-6 rel-MSE {E['profile'][6]:.4f} vs 1.7415: {'HELD' if pd else 'FAILED'}")
    print(f"(a) metric gain {gain:+.4f} >= 0.15: {'HELD' if pa else 'FAILED'}")
    print(f"(b) observable block-6 energy drop {drop:+.3f} >= 0.20: {'HELD' if pb else 'FAILED'}")
    print(f"(c) random-metric gain {gain_r:+.4f} <= 0.05: {'HELD' if pc else 'FAILED'}")
