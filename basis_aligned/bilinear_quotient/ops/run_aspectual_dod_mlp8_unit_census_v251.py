...
"""Aspectual has/had DoD (v251): the temporal family at UNIT grain -- does an MLP-8 'perfect/tense' detector exist, as the number detectors (829 / 953) and the gender
detectors (3152 / 3943) do? The readout set {8.1, 9.1, 9.4} reads since/by -> has/had; 8.1 copies the cue token, 9.1 / 9.4 are contextual. Exact unit census
(`dod_units.unit_census`) of MLP 8's write on 9.1's weight-only has - had reader direction r = V_{9.1}^T O_{9.1}^T (u_has - u_had), at TWO positions of the v1 rows
(64 rows, fronted / report constructions x 16 lexical groups x {since, by}): the CUE token (Since / By / since / by) and the FINAL token. Pooled since - by over
aligned pairs. Kill criterion (review 26): top-10 < 0.30 at both positions -> the temporal MLP-8 port is declared spread.
PREDICTIONS (scored as written; failures preserved; priors unsure)
    ...
    pred_b_cue_top10_carry_half                at the cue, the top-10 |pooled| units carry >= 0.50 of MLP 8's pooled write on r
    pred_c_final_top10_carry_half              at the final token, likewise >= 0.50
    pred_d_top_units_stable_across_constructions  at the cue, Jaccard(top-50 of the fronted rows, top-50 of the report rows) >= 1/3
...
OUT = ROOT / "circuits/followups/aspectual_dod_mlp8_unit_census_v251_result.json"
CANDIDATE_ID = "aspectual_anchor.has_vs_had.dod_mlp8_unit_census_v251"
LAYER, HEAD, CLOSURE_TOL, TOP10_MIN, JACCARD_MIN = 8, (9, 1), 1e-3, 0.50, 1 / 3
CUES = {L._single(" since"), L._single(" by"), L._single("Since"), L._single("By")}
FORWARDS_MAX = 4
PREDICTIONS = {"pred_a_unit_closure": "<= 1e-3", "pred_b_cue_top10_carry_half": ">= 0.50", "pred_c_final_top10_carry_half": ">= 0.50", "pred_d_top_units_stable_across_constructions": "Jaccard >= 1/3"}
...
def main() -> None:
    ...
    OUT.write_text(json.dumps({"schema": "aspectual_dod_mlp8_unit_census_result_v251", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "positions": report, "predictions": predictions, "forwards": forwards,
