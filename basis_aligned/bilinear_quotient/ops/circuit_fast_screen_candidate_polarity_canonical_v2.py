"""polarity_state.canonical_control -- polarity_state.negative_vs_positive scored against canonical control V2 (disjoint vocabulary).

A1, A2 and P are unchanged from the polarity_state.negative_vs_positive authority. Only the control changes: it becomes
...
them is a difference between the behaviours. This one is the licensing side.
...
TASK_ID = "polarity_state.canonical_control_v2"
POLARITY = (" any", " some")
...
def _panel(seed: int, group_number: int, case_index: int) -> list[dict[str, Any]]:
    ...
    common = dict(common_no_vocab, vocabulary=POLARITY)
    return [
