...
STEM='DIRECTIONAL_INTERACTION_STRENGTH_V1';FOLDER=P/'extracted_circuits/regional_even_key_producers_8_2_9_8_v1'
@torch.no_grad()
def main():
 ...
 def head9(module,args,output):
  ...
  if arm==0:
   ...
   z=context['z'].double();a=context['a'][...,None];d=direction.double();rho=z.square().mean(-1,keepdim=True)+torch.finfo(torch.float32).eps;mixed=(z@low['right'].T)@low['left'].T;delta=a*(-d+2*(z*d).mean(-1,keepdim=True)/rho*context['u0'].double()-mixed/rho);raw=context['raw9'].double()+gain*delta;context['linear']=runtime.scalar(F.rms_norm(raw.float(),(1152,)),context['tokens'],p,1)
   return output
 ...
 result={'pred_a':A,'pred_b':B,'pred_c':C,'records':records,'max_full_scalar_relative_error':max(field_checks),'body_forwards':count,'seconds':time.perf_counter()-tic,'arms':['native','remove8','remove8_actual9','remove8_predicted64_9','remove8_frozen9','remove8_predicted_full9','remove8_linear64_9'],'scope':'Frozen rank64, reused longer contexts with new strengths -1,.5,1.5,2. Same-rank linearized raw-state response is a mechanistic comparator. No new OOD panel. Pristine z8, bias-free MLP8 output, raw9 and head8 amplitude are inputs. Conditional surrogate-removal simulator with native prefix/background/suffix, not autonomous circuit extraction or corpus OOD.'}
