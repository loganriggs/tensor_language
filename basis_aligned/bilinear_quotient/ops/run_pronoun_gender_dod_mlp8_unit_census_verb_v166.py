...
"""Pronoun gender he/she DoD (v166): MLP 8 at UNIT grain at the VERB position -- which units write the gender state head 9.6 reads at the verb (51% MLP 8 there, v82)? v164's fold with the query position changed to the verb; registered question: are they the same two units (3152, 3943)?
...
PREDICTIONS (scored as written; failures preserved; priors unsure)
    ...
    pred_d_top_units_stable_across_frames the top-50 sets computed within each of the three frames pairwise share >= 25 units (Jaccard >= 1/3); the receipt also records whether 3152 and 3943 are in the top 10 (reported, not scored)
...
OUT = ROOT / "circuits/followups/pronoun_gender_dod_mlp8_unit_census_verb_v166_result.json"
CANDIDATE_ID = "pronoun_gender.he_vs_she.dod_mlp8_unit_census_verb_v166"
LAYER = 8
...
def main() -> None:
    ...
    noun_of = lambda row: next(i for i, t in enumerate(row.ids) if t in v81.VERB)   # the VERB position (name kept from v164)
    ...
    OUT.write_text(json.dumps({"schema": "pronoun_gender_dod_mlp8_unit_census_verb_result_v166", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "contrast_total": contrast,
