#!/usr/bin/env python
"""late_tail_product_term_probe -- BELOW THE MLP BLOCK: which TERM of the bilinear product needs the tail? With the late MLPs (blocks 8-17)
reading the bus core c (top-k of U_8, rest the fit-set constant) and t the dropped tail, Left(c+t)*Right(c+t) = Lc*Rc + (Lc*Rt + Lt*Rc) + Lt*Rt
exactly (bias-free). This rung keeps the cross terms only (DROP_TT), the tail*tail term only (DROP_CROSS), at 768 and 896, plus FULL
(all terms; must be 0 -- exactness check). CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_cross_terms_carry_the_bulk pred_c_tail_x_tail_is_small pred_d_terms_near_additive
#                     pred_e_tail_x_tail_negligible_at_896
...
Preregistration: polynomial_causal/LATE_TAIL_PRODUCT_TERM_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_tail_product_term_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_tail_product_term_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_TAIL_PRODUCT_TERM_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_tail_channel_rank_probe_results.json"   # §2779
OUT = ROOT / "late_tail_product_term_probe_results.json"
HASHES = {PREREG: "de8c79030819ca78bfc0cc8bd60c372d6a277da66a6910d8c3531eba2a56ab4b", PRIOR: "c5a85ff6174160d9080ba0197a4de5bb8790b17e1f9a77a6173024f6e9501e33",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_tail_product_term_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "exact_tol": 0.002, "b_min": 0.070, "c_max": 0.040, "d_lo": 0.7, "d_hi": 1.3, "e_max": 0.015}
NULLS = {"b_max": 0.040, "c_min": 0.070, "d_max": 0.5, "e_min": 0.040}
...
def main():
    ...
    class TermHead(OwnHead):
        """MLP l's normalised input xh = c + t, c = the truncated input (top-k of U_8 exact, rest the fit-set constant), t = the dropped tail.
        Left and Right are bias-free, so Left(xh)*Right(xh) = Lc*Rc + (Lc*Rt + Lt*Rc) + Lt*Rt exactly. keep in {'all','cross','tt','none'}."""
        def __init__(self, l, k, keep):
            super().__init__(l, k, "const", U_8[:, :k]); self.keep = keep
        def __call__(self, w, x):
    ...
    def term(k, keep):
        return run({("mlp", l): TermHead(l, k, keep) for l in range(SPLIT, NL)})
    arms = {"SPLIT8_1024": split8(1024), "LATE_MLP_768": term(768, "none"), "FULL_768": term(768, "all"), "DROP_TT_768": term(768, "cross"), "DROP_CROSS_768": term(768, "tt")}
    if not smoke:
        arms["LATE_MLP_896"] = term(896, "none"); arms["DROP_TT_896"] = term(896, "cross"); arms["DROP_CROSS_896"] = term(896, "tt")
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    C0 = arms["LATE_MLP_768"]; TT = arms["DROP_TT_768"]; CR = arms["DROP_CROSS_768"]; TT896 = arms.get("DROP_TT_896", TT); C896 = arms.get("LATE_MLP_896", C0)
    summ = {"drop_tt_768": TT, "drop_cross_768": CR, "sum_over_joint_768": (TT + CR) / max(C0, 1e-9), "tt_share_768": TT / max(C0, 1e-9), "cross_share_768": CR / max(C0, 1e-9),
            "drop_tt_896": TT896, "drop_cross_896": arms.get("DROP_CROSS_896"), "full_768_should_be_zero": arms["FULL_768"]}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(C0 - PRIOR_LATE_MLP_768) <= BARS["repro_tol"] and abs(C896 - PRIOR_LATE_MLP_896) <= BARS["repro_tol"] and abs(arms["FULL_768"]) <= BARS["exact_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_cross_terms_carry_the_bulk': bool(CR >= BARS["b_min"]),
        'pred_c_tail_x_tail_is_small': bool(TT <= BARS["c_max"]),
        'pred_d_terms_near_additive': bool(BARS["d_lo"] <= (TT + CR) / max(C0, 1e-9) <= BARS["d_hi"]),
        'pred_e_tail_x_tail_negligible_at_896': bool(TT896 <= BARS["e_max"]),
    }
    nulls = {"b_null_cross_le_.040": bool(CR <= NULLS["b_max"]), "c_null_tt_ge_.070": bool(TT >= NULLS["c_min"]),
             "d_null_sum_le_half_joint": bool((TT + CR) / max(C0, 1e-9) <= NULLS["d_max"]), "e_null_tt896_ge_.040": bool(TT896 >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "blocks 8-17 MLP reads through the bus (top-k of U_8; rest constant) with the bilinear product split exactly into core*core + cross + tail*tail; DROP_TT keeps core*core + cross, DROP_CROSS keeps core*core + tail*tail, LATE_MLP keeps core*core only, FULL keeps all (exactness check)",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "late_mlp_768": C0, "prior_late_mlp_768": PRIOR_LATE_MLP_768, "late_mlp_896": C896, "prior_late_mlp_896": PRIOR_LATE_MLP_896, "full_768": arms["FULL_768"]},
