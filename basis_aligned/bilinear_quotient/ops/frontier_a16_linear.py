"""ALL-LINEAR CLASSES AT a16 (rung 54): a grammar extension aimed at the identified bottleneck.

CONVENTION (S2135): L2 is CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER.

S2145/S2147: a16L is the assembly's most expensive replacement (+0.157) and its residual is unusually damaging
per unit energy. The cheapest in-grammar upgrade: give a16 (only) linear maps for ALL 10 classes instead of
linear-for-4-LINK + mean-for-6-CONSTN. PRICE: +6 x 1152^2 ~ +8.0M values at one layer. Arms: S2144 best full
tail (reproduction) | a16-all-linear. NULL (S2147): the damage is loss-specific structure, not class-mean
sloppiness - per-class linearity buys little.

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) A THIRD COMES BACK: median over the 8 windows of [L2_F_w(a16lin) - L2_F_w(full)] <= -0.05.
  (b) NOT ONE WINDOW: that difference is <= 0 on >= 7 of 8 windows.
  (c) REPRODUCTION: the full arm reproduces S2144 (FR L2_F 2.6662 within 0.01).

Writes frontier_a16_linear_results.json. Self-reviewed. RUNG-52 HEADER FOLLOWS.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_skip_a16_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; full vs a16-all-linear arms (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_a16_linear_results.json'
...
if __name__=='__main__':
    ...
    print('ARM 1/2: S2144 best config, full tail (reproduction)',flush=True)
    r_f=main()
    SEL['a16lin']=True
    print('ARM 2/2: same, a16L with linear maps for ALL 10 classes',flush=True)
    r_l=main()
    import statistics as stt
    d=[round(a-b,4) for a,b in zip(r_l['fresh8'],r_f['fresh8'])]
    md=stt.median(d)
    pa=md<=-0.05
    pb=sum(g<=0 for g in d)>=7
    pc=abs(r_f['L2_F']-2.6662)<=0.01
    res={'full':r_f,'a16_all_linear':r_l,'per_window_lin_minus_full':d,
         'convention':'L2 = CE above the real model; lower is better; negative = the upgrade helps',
         'price_note':'+6 x 1152^2 ~ +8.0M values at a16 only',
         'pred_a_third_comes_back':bool(pa),'pred_b_windows':bool(pb),'pred_c_reproduces_s2144':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'a16lin - full per window: {d}  median {md:+.4f}')
    print(f"(a) median {md:+.4f} <= -0.05: {'HELD' if pa else 'FAILED'}")
    print(f"(b) <= 0 on {sum(g<=0 for g in d)}/8 (bar 7): {'HELD' if pb else 'FAILED'}")
