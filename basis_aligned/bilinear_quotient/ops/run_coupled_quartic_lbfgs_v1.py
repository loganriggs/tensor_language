...
def main():
 ...
 p=torch.load(P/'COUPLED_QUARTIC_NONLINEAR_V2_PROGRAM.pt',weights_only=True);assert p['seed']==11511
 ...
 previous=json.loads((P/'COUPLED_QUARTIC_NONLINEAR_V2_RESULT.json').read_text());assert abs(initial/divisor-previous['history'][-1]['objective'])<=1e-8
