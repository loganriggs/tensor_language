#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_closure pred_b_noun_source_carries_most_on_text pred_c_copier_value_copy_replays_on_text pred_d_third_order_small pred_e_leading_term_same_on_both_halves
"""Copier head 4.5's degree expansion ON NATURAL TEXT (v410). v409 (panel): 4.5's noun-source write at the post-noun token on m (9.6's value direction) is
a value copy (0.91; pattern terms 0.07; noun source = the whole write). Here the same on the 122 natural swapped pairs (v405's instrument).
PREDICTIONS (scored as written; failures preserved; priors from v409)
    ...
    pred_b_noun_source_carries_most_on_text   the noun source carries >= 0.60 of 4.5's total write contrast on m at the post-noun token (panel 1.02). Prior: likely
    pred_c_copier_value_copy_replays_on_text  the value-only term is >= 0.60 of the noun-source contrast on text. Prior: likely
    pred_d_third_order_small                  |degree-3 term| <= 0.25 of the noun-source contrast
    pred_e_leading_term_same_on_both_halves   the largest |term| category is the same on the plural-cue and singular-cue halves
...
OUT = ROOT / "circuits/followups/both_ends_degree_copier_text_v410_result.json"
CANDIDATE_ID = "both_ends.degree_expansion_copier_text_v410"
N_HEAD, BATCH = 9, 64
...
CLOSURE_TOL, INSTR_TOL, NOUN_MIN, VALUE_MIN, THIRD_MAX = 1e-4, 2e-2, 0.60, 0.60, 0.25
READERS = ((4, 5),)
DOWNSTREAM = (9, 6)
FORWARDS_MAX = 5
PREDICTIONS = {"pred_a_closure": "<= 1e-4 / 2e-2", "pred_b_noun_source_carries_most_on_text": ">= 0.60", "pred_c_copier_value_copy_replays_on_text": ">= 0.60", "pred_d_third_order_small": "<= 0.25", "pred_e_leading_term_same_on_both_halves": "same x 2"}
...
def main() -> None:
    ...
    with torch.no_grad():
        for start in range(0, len(rows), BATCH):
            ...
            nn_ = torch.tensor([noun_of(r_) for r_ in chunk]); ff = nn_ + 1   # read at the token AFTER the noun
    ...
    predictions = {"pred_a_closure": closure <= CLOSURE_TOL and instr <= INSTR_TOL, "pred_b_noun_source_carries_most_on_text": r96["noun_source_share"] >= NOUN_MIN, "pred_c_copier_value_copy_replays_on_text": r96["term_shares"]["value_only"] >= VALUE_MIN,
                   "pred_d_third_order_small": all(abs(R[k]["degree_shares"]["degree3"]) <= THIRD_MAX for k in R), "pred_e_leading_term_same_on_both_halves": len(set(r96["leading_term_by_frame"].values())) == 1}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "both_ends_degree_copier_text_result_v410", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
