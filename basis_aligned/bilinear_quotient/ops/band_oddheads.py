"""HEAD-PARITY BAND TEST I (rung 258): fine band missing at odd heads only.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2353: WHICH fine directions are
missing does not matter within a head. Does WHICH HEADS carry the deficiency matter? Here tail heads with
odd index keep r96 while the others are exact (motifs exact throughout) - half the fine-band deficiency.
REGISTERED PREDICTIONS (arms: THIS vs all-tail-r96 0.0553/11 and r120 0.0526/11):
  (a) HEAD-ADDITIVE: census <= 0.040 (about half the full-band damage).
  (b) valid >= 11.
  (c) SANITY: census in [0.01, 0.12]; inert tripwires vs both anchors.
NULL: the cancellation mechanism is GLOBAL across heads too (census >= 0.048 - overlap, not additivity).
PRICE: probe (structure question, not a config). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: fine band at odd heads')
    raise SystemExit(0)
import torch
...
OUT=PT+'band_oddheads_results.json'
    ...
    if SEL.get('qk_r'):
        ...
        if SEL.get('qk_tail'):
            for _li in range(10,18):
                ...
                for _hd in range(9):
                    _fac=[]
                    _rh=128 if (SEL.get('qk_hpar') is not None and _hd%2==SEL['qk_hpar']) else _r
                    for _W in (_at.c_q,_at.c_k,_at.c_q2,_at.c_k2):
                        _M=_W.weight[_hd*128:(_hd+1)*128,:].detach().float()
                        _U,_S,_Vh=torch.linalg.svd(_M,full_matrices=False)
                        _fac.append(((_U[:,:_rh]*_S[:_rh]).contiguous(),_Vh[:_rh].contiguous()))
                    _d[_hd]=_fac
                SEL['_QKR'][_li]=_d
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=96; SEL['qk_rmap']={li:128 for li in range(2,10)}; SEL['qk_hpar']=1; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    pb=nv>=11
    pc=0.01<=agg<=0.12
    if abs(agg-0.0553)<1e-4 or abs(agg-0.0526)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: head-parity map inert')
    ...
    print(f"(a) census {agg:+.4f} <= 0.040: {'HELD' if pa else 'FAILED'}")
