...
"""Initial width8 interface with exact native suffix and final projection.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v684_result.json'
PREDICTIONS=dict(pred_a_exact_replay='baseline and independent joint versus shared chain replay max absolute error <=1e-4',
 pred_b_effect_prediction='independent full-state float64 selected-logit replay absolute error<=1e-4',
 pred_c_capable_position_transfer='initial-interface exact suffix A/B target<=10% and modal<=5% every cell')


def main():
    plan=dict(prefix_calls=8,suffix_calls=56,sequences_per_arm=48,fits=0,backwards=0,additional_initial_interface_suffix_calls=12,
    ...
    for entries in batches:
        ...
        for source_name,delta in [('A',source_deltas[0]),('B',source_deltas[1]),('sum',source_deltas[0]+source_deltas[1])]:
            ...
            for i,e in enumerate(entries):
                ic=initial_reports[source_name].setdefault(e[3],dict(target=[],prediction=[],native_modal=[],predicted_modal=[],unprojected_prediction=[]))
    ...
    b=a and max(oracle_checks)<=1e-4
