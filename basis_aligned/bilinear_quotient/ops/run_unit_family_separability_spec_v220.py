#!/usr/bin/env python3
# BQGATE: eight frozen predictions; families, EVAL set, units (v197 + v201 + v209 + v213 + v215 + v219 receipts), recipe and bars fixed before the run.
"""v220: within-family separability for v219's passes -- the count test for superlative_complement_best, article_countability,
number_of_agreement, for_complementizer_waited, for_complementizer_arranged -- plus the re-evaluation of the two COUNTED
complementizer members under a five-construction family.

v219 put the ninth eight spec-authored behaviours through the four-row battery (5/8 four-row). Families here: degree_complement =
superlative_complement_best + superlative_complement_largest (row 4 miss: control); countability = countability (v201, not four-row:
control), less_fewer_countability (row 4 miss: control), article_countability; number_agreement = partitive_agreement (v209),
perfect_number and lexical_number_pp (v197) -- all three four-row but FUSED at v210 (none separable from the others; the family counts
as one circuit) -- plus number_of_agreement; complementizer = verb_complementizer (v201, control), noun_complementizer (COUNTED at
v214, kept at v216), doubt_belief_complementizer (COUNTED at v216), for_complementizer_waited, for_complementizer_arranged. Recipe as
v198/.../v218: per evaluated member, arm `own` (own C_fit control) and arm `fam` (own C_fit + EVERY sibling's A1 FIT prep,
control_weight 30 x len(controls)); evaluated on the member's held A1 (extraction), each sibling's held A1 (|CE| leak) and held C.
Units frozen from the parent receipts.
...
All fourteen members are prepared; EVAL = the five v219 passes + noun_complementizer and doubt_belief_complementizer (counted members
re-checked under the enlarged family, v212 precedent: a counted member that loses separability is RETRACTED). ~60-80 s per evaluated
member (7 fits, ~9-11 min with the fourteen prepares).
Count rule: each separable new EVAL member is a distinct circuit (39 -> up to 44); the for-pair counts once if it fuses with itself
but separates from the whether/that members (v216 aux-copy precedent). number_of_agreement counts only if separable from ALL three
fused number-agreement members. A re-checked counted member that fails is retracted (-1).

REGISTERED BEFORE THE RUN (each coded predicate is the sentence here):
    pred_a_superlative_best      superlative_complement_best is separable from superlative_complement_largest.          prior 50%
    pred_b_article_count         article_countability is separable from countability and less_fewer_countability.       prior 55%
    pred_c_number_of             number_of_agreement is separable from partitive_agreement, perfect_number and
                                 lexical_number_pp (a fused trio; prior LOW because the trio itself fused).              prior 30%
    pred_d_for_waited            for_complementizer_waited is separable from the other four complementizer members.     prior 50%
    pred_e_for_arranged          for_complementizer_arranged is separable from the other four complementizer members.   prior 50%
    pred_f_counted_kept          noun_complementizer and doubt_belief_complementizer BOTH remain separable under the
                                 five-member family.                                                                    prior 70%
    pred_g_row4_kept             fam-arm own-C UB975 <= 0.01 on >= 4 of the 7 evaluated members.                        prior 60%
    pred_h_instrument            every evaluated member's own-arm extraction is within +-0.03 of its parent-receipt row-4 (cdas)
                                 extraction_held (1.010 / 0.983 / 1.012 / 1.000 / 1.019 / 1.003 / 0.981; same units, seed, split
                                 and code path -- a determinism check, NOT an independent run outcome).                prior 90%
Priors are not moved by unit overlap in either direction (v214 separated shared-unit members; v216 fused them).
Smoke: V220_SMOKE=<out.json> (CPU, V220_SMOKE_ROWS=4; all fourteen members are prepared as controls, V220_SMOKE_NAMES=superlative_complement_best,noun_complementizer
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v220_result.json"
...
FAMILIES = {
    "degree_complement": ["superlative_complement_best", "superlative_complement_largest"],
    "countability": ["countability", "less_fewer_countability", "article_countability"],
    "number_agreement": ["partitive_agreement", "perfect_number", "lexical_number_pp", "number_of_agreement"],
    "complementizer": ["verb_complementizer", "noun_complementizer", "doubt_belief_complementizer",
                       "for_complementizer_waited", "for_complementizer_arranged"],
}
EVAL = ("superlative_complement_best", "article_countability", "number_of_agreement", "for_complementizer_waited",
        "for_complementizer_arranged", "noun_complementizer", "doubt_belief_complementizer")
COUNTED = ("noun_complementizer", "doubt_belief_complementizer")
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES, **v213.NAMES, **v215.NAMES, **v217.NAMES, **v219.NAMES}


def _parent_cdas(n):
    # the parent battery receipt that produced this member's units (each behaviour name is unique across batteries)
    for path in (V219, V215, V213):
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v220", "members": n, "families": list(FAMILIES),
...
def PREDS(R):
    ...
    a = ok and sep("superlative_complement_best")
    b = ok and sep("article_countability")
    c = ok and sep("number_of_agreement")
    d = ok and sep("for_complementizer_waited")
    e = ok and sep("for_complementizer_arranged")
    ...
    return {"pred_a_superlative_best": bool(a), "pred_b_article_count": bool(b), "pred_c_number_of": bool(c),
            "pred_d_for_waited": bool(d), "pred_e_for_arranged": bool(e), "pred_f_counted_kept": bool(f),
...
def main() -> None:
    ...
    smoke = os.environ.get("V220_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V220_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v220", "candidate_id": "corpus.unit_family_separability_spec_v220",
              "separable_counts": nsep, "families": FAMILIES, "summary": summary, "members": R, "bars": BARS,
              "recipe": {"lambda": LAM, "steps": steps, "lr": LR, "complement_weight": CW, "rank": 1, "seed": 0, "units_source": "v197 + v201 + v204 + v205 + v207 + v209 + v211 + v213 + v215 + v217 + v219 receipts (later receipts override earlier ones)", "evaluated": list(EVAL),
