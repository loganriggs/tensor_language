...
"""pred_a anchor_abs<=1e-4/anchor_rel<=1e-5/write<=1e-4; pred_b targeterror<=.35/live>=1e-5;
pred_c collateral<=.5target; pred_d attenuation>=.02/positive>=.90;
pred_e>=2median/16nulls; pred_f<=.8eachbaseline.800forwards.
Opened Pile full removal; pred_g unedited/native baseline <=1e-5 absolute.
pred_h target error <=.8 of native half-effect linear scaling error.
No new fresh-confirmation, causal-composition or complete-circuit promotion.
"""
...
STEM='CITY_FULL_STRENGTH_V1';ARMS=['native','native8_full','full_reentry','compiled']+[f'null:{k}' for k in range(16)]
...
def main():
 ...
 cache=[{} for _ in groups];state={};joint_errors=[];outside=[];fixtures=[];norm_errors=[];analytic_errors=[];baseline_errors=[]
 ...
 def attnpost(module,args,result):
  c=cache[state['i']]
  if state['arm']=='native':c['a']=result[0].clone();c['g']=c['mixed']+result[0]
  elif state['arm']=='native8_full':return result[0]+c['delta'].to(result[0].dtype),result[1]
  return result
 ...
 def pre9(module,args):
  ...
  if arm=='native':c['x9']=x.clone()
   ...
   analytic_errors.append(float((c['complete']-actual).norm()/actual.norm().clamp_min(1e-30)))
   def block_end(g):
    normalized=F.rms_norm(g,(1152,));hidden=F.linear(normalized,m['left'])*F.linear(normalized,m['right'])
    return g+(F.linear(hidden,m['down'])+model.transformer.h[8].mlp.Down_bias)
   g0=c['mixed']+c['a'];g1=c['mixed']+(c['a']+c['delta'].to(c['a'].dtype))
   y0=block_end(g0);y1=block_end(g1);baseline_errors.append(float((y0-c['x9']).abs().max()))
   c['complete']=y1.double()-y0.double()
   joint_errors.append(float((c['complete']-actual).norm()/actual.norm().clamp_min(1e-30)))
   candidate_inputs={'residual7':c['residual7'],'token_ids':torch.tensor([row['ids']],device=x.device),'city':city,'destination':c['mask'],'strength':1.0}
  ...
  for ai,arm in enumerate(ARMS):
   ...
   for i,row in enumerate(groups):
    ...
    if arm=='native8_full':
     ...
     c['delta']=-1.0*F.linear(route[...,None]*value[:,None],p['output'])*mask[None,:,None]
 ...
 v=expand(values,mapping,6);e=v-v[:1];records={};baseline=json.loads((P/'CITY_FULL_PILE_V2_BASELINES.json').read_text())
 X=torch.tensor([features(r) for r in rows],dtype=torch.float64);fit=2*(X@torch.tensor(baseline['coefficients'],dtype=torch.float64));constant=2*torch.tensor([baseline['cue_constants'][r['cue']] for r in rows],dtype=torch.float64)
 old=torch.load(P/'CITY_FULL_PILE_V3_ARTIFACT.pt',weights_only=True)['values'];linear=2*(old[1,:,0]-old[0,:,0])
 ...
 behavior_replay=float((v[0]-old[0]).abs().max())
 result={'pred_h':all(r['target_error']<=.8*r['linear_half_effect_error'] for r in records.values()),'strength':1.0,'pred_g':behavior_replay<=1e-5 and max(baseline_errors)<=1e-5,'behavior_replay_max_abs':behavior_replay,'native_baseline_max_abs':max(baseline_errors),'old_analytic_max_relative_error':max(analytic_errors),'pred_a':anchor_abs<=1e-4 and anchor_rel<=1e-5 and max(joint_errors)<=1e-4 and max(outside)==0 and max(norm_errors)<=1e-5 and bool(torch.isfinite(v).all()) and count==800,
