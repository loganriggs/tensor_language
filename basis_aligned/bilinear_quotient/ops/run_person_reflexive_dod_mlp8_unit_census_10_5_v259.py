#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_unit_closure pred_b_pronoun_top10_carry_half pred_c_final_top10_carry_half pred_d_top_units_stable_across_constructions
"""Reflexive person I/you DoD (v259): the first in-depth document's open member, head 10.5, at MLP-8 unit grain. `in_depth_circuit.md` §4.7: 10.5 reads a contextual
mixture (52% pronoun position, 29% token-only) and is left open; its MLP inputs were never censused. Exact census (`dod_units.unit_census`) of MLP 8's write on 10.5's
weight-only myself - yourself reader direction at the PRONOUN position (I / you) and at the FINAL token of the v104 fresh rows (96 rows, three frames), pooled I - you
over aligned pairs. Kill criterion (review 26): top-10 < 0.30 at both positions -> the person family's MLP-8 port is spread.
PREDICTIONS (scored as written; failures preserved; priors unsure -- the person feature is a function-word token, like the temporal cue)
    pred_a_unit_closure                        sum_j T_j + bias = r . mlp(x) within 1e-3, every row and position
    pred_b_pronoun_top10_carry_half            at the pronoun, the top-10 |pooled| units carry >= 0.50 of MLP 8's pooled write on r
    pred_c_final_top10_carry_half              at the final token, likewise >= 0.50
    pred_d_top_units_stable_across_constructions  at the pronoun, min pairwise Jaccard of the three frames' top-50 sets >= 1/3
...
import run_person_reflexive_dod_battery_v104 as g
import dod_battery, dod_units

ROOT = dod_battery.ROOT
OUT = ROOT / "circuits/followups/person_reflexive_dod_mlp8_unit_census_10_5_v259_result.json"
CANDIDATE_ID = "reflexive_person.i_vs_you.dod_mlp8_unit_census_10_5_v259"
LAYER, HEAD, CLOSURE_TOL, TOP10_MIN, JACCARD_MIN = 8, (10, 5), 1e-3, 0.50, 1 / 3
CUES = {L._single(" I"), L._single("I"), L._single(" you"), L._single("you")}
FORWARDS_MAX = 5
PREDICTIONS = {"pred_a_unit_closure": "<= 1e-3", "pred_b_pronoun_top10_carry_half": ">= 0.50", "pred_c_final_top10_carry_half": ">= 0.50", "pred_d_top_units_stable_across_constructions": "min Jaccard >= 1/3"}
...
def main() -> None:
    ...
    predictions = {"pred_a_unit_closure": closure <= CLOSURE_TOL, "pred_b_pronoun_top10_carry_half": report["cue"]["shares"]["10"] >= TOP10_MIN, "pred_c_final_top10_carry_half": report["final"]["shares"]["10"] >= TOP10_MIN,
                   "pred_d_top_units_stable_across_constructions": report["cue"]["jaccard_fronted_report_top50"] >= JACCARD_MIN}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "aspectual_dod_mlp8_unit_census_10_5_result_v259", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "positions": report, "predictions": predictions, "forwards": forwards,
