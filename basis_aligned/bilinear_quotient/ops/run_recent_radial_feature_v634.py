...
"""Shared radial quadratic plus centered upstream products; full input span.
...
Fixed weights, no scalar refits.16forwards/256replays. Not optimal sparse DAG.
"""
import json,os,sys,time,hashlib
from pathlib import Path
from datetime import datetime,timezone
ROOT=Path(__file__).resolve().parents[3];BQ=ROOT/'basis_aligned/bilinear_quotient'
OUT=BQ/'circuits/followups/recent_radial_feature_v634_result.json'
...
def main():
    ...
    for family,rank in configs:
        ...
        if family!='dense':
            kept=orders[family][-rank:] if rank else orders[family][:0]
            row['global_tensor_error']=float((removal_energy(global_centered,kept).clamp_min(0)/global_gram.sum()).sqrt())
            row['parent_dictionary_error']=float((removal_energy(parent_centered,kept).clamp_min(0)/parent_gram.sum()).sqrt())
    ...
    result=dict(plan=plan,rows=rows,selected={k:v for k,v in selected.items() if k!='panels'},
        ...
        scope='One new shared radial quadratic plus centered native atoms; no fit; opened panels; upstream ports required',
        seconds=time.perf_counter()-tic,finished_utc=datetime.now(timezone.utc).isoformat())
    assert forwards<=plan['forwards_max']
    disk_guard.guard_write(1000000,label='v634 JSON');OUT.write_text(json.dumps(result,indent=2)+'\n')
