"""IS THE FRONTIER'S MOTIF-ATTENTION STAGE NEEDED AT ALL, OR ONLY ITS EIGHT CONSTANTS? (Claude, LANE 1)

SS2875 showed the whole motif band a2-a9 collapses to EIGHT CONSTANT VECTORS for **0.0000 nats**, removing 42,550,272 parameters --
ten class rows plus four full 1152x1152 link maps per layer, all contributing nothing measurable to L2. That raises the obvious next
question, which SS2875 did not ask: **are the eight constants themselves doing anything?**

Three arms of SS312's published norm-selection pipeline:

    ARM 2  band -> one mean vector per layer   (reproduces SS2875 exactly, and is its cross-rung instrument check)
    ARM 3  band -> the ZERO vector             (the stage writes nothing: deletion of its output)

The separation between them, `cost_zero - cost_const`, is what the eight constants are worth. If it is large the constants are real
structure and SS2875's simplification is the right one; if it is ~0 the entire motif-attention stage can be **deleted** from the
frontier, which is a far larger simplification than replacing it -- and one the price-constrained SS312 construction would care about,
since it frees eight component slots rather than 42.5M parameters.

Deletion is implemented as CV := 0 rather than by switching the entry to the parent's `attnz` kind, **deliberately**: `attnz` would
change which entry is `alist[0]` and therefore which hook computes `cur['lab']` for the downstream dictionaries. That is control flow.
CV := 0 leaves every control path identical and changes only what is written -- the same discipline as SS2874/SS2875.

SIGN CONVENTION (SS2135): frontier L2 is CE ADDED ABOVE THE REAL MODEL, so **LOWER IS BETTER** (SS312: +2.6735 beating +2.84/+2.93).
A cost is L2_F(arm) - L2_F(baseline), **POSITIVE = WORSE**. SS2128/SS2129/SS2133/SS2134 RETRACTED; SS2125 STANDS.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_the_constants_are_doing_work
#                     pred_c_the_constant_arm_reproduces_S2875
#                     pred_d_deleting_the_stage_is_expensive
#                     pred_e_the_parameter_accounting_is_stated

Preregistration: polynomial_causal/FRONTIER_BAND_CONSTANT_VS_DELETED_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; three arms (baseline / band -> constants / band -> zero)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_band_constant_vs_deleted_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
SEL={'mode':'norm','P8':{}}
WRITEMODE={'mode':None,'set':frozenset()}   # mode: None | 'const' (one mean vector) | 'zero' (writes nothing)
CVSNAP={}
SKIP8=64
...
def main():
    ...
    def fit_attnd(li,active):
        ...
        if li in WRITEMODE['set'] and WRITEMODE['mode'] is not None:
            ...
            # 'const' -> every class row is the overall mean write; 'zero' -> the layer writes nothing,
            # which is deletion of the stage's OUTPUT while leaving control flow (the probe label the hook
            # computes for downstream dictionaries) exactly intact. Using zeros rather than the parent's
            # 'attnz' kind is deliberate: 'attnz' would change which entry is `alist[0]` and therefore who
            # computes cur['lab'], and that is control flow.
            CV = (Y.mean(0) if WRITEMODE['mode'] == 'const'
                  else torch.zeros(CV.shape[1], device=CV.device, dtype=CV.dtype)
                  ).unsqueeze(0).repeat(CV.shape[0], 1)
            LW = {}
            print(f"{'ZEROED' if WRITEMODE['mode']=='zero' else 'COLLAPSED'} a{li}", flush=True)
...
PREREG = PT + '../polynomial_causal/FRONTIER_BAND_CONSTANT_VS_DELETED_PREREGISTRATION.md'
PREREG_SHA = "2ba5ff67a323464237d739ad8ee49dfb689eced4075778a100491db15c14fbce"
...
BAND = tuple(ATTM)                        # a2..a9, the motif-attention band SS2875 collapsed
BARS = {"reproduce": 0.05, "separation": 0.10, "const_repro": 0.01, "delete_costs": 0.20}
NULLS = {"separation_le": 0.02, "delete_costs_lt": 0.05}
PERLAYER = 5318784
...
if __name__=='__main__':
    ...
    print('ARM 1/3: BASELINE -- reproduction of the published frontier',flush=True)
    WRITEMODE['mode']=None; WRITEMODE['set']=frozenset()
    r_base=main()

    print(f'ARM 2/3: band a{BAND[0]}-a{BAND[-1]} -> ONE CONSTANT each (reproduces SS2875)',flush=True)
    WRITEMODE['mode']='const'; WRITEMODE['set']=frozenset(BAND)
    r_const=main()

    print(f'ARM 3/3: band a{BAND[0]}-a{BAND[-1]} -> ZERO (the stage writes nothing = deleted)',flush=True)
    WRITEMODE['mode']='zero'; WRITEMODE['set']=frozenset(BAND)
    r_zero=main()
    WRITEMODE['mode']=None; WRITEMODE['set']=frozenset()

    # SIGN: L2 is CE ADDED ABOVE THE REAL MODEL, LOWER IS BETTER -> cost POSITIVE = WORSE
    cost_const = r_const['L2_F'] - r_base['L2_F']
    cost_zero  = r_zero['L2_F']  - r_base['L2_F']
    sep        = cost_zero - cost_const          # POSITIVE = the constants are doing work
    repro      = abs(cost_const - 0.0)           # SS2875 measured 0.0000 for this exact arm

    pa = abs(r_base['L2_F']-2.6735) <= BARS['reproduce']
    pb = sep >= BARS['separation']
    pc = repro <= BARS['const_repro']
    pd = cost_zero >= BARS['delete_costs']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_constants_are_doing_work':bool(pb),
           'pred_c_the_constant_arm_reproduces_S2875':bool(pc),
           'pred_d_deleting_the_stage_is_expensive':bool(pd),
           'pred_e_the_parameter_accounting_is_stated':True}
    nulls={'b_null_the_constants_are_not_doing_work':bool(sep<=NULLS['separation_le']),
           'd_null_the_whole_stage_is_removable':bool(cost_zero<NULLS['delete_costs_lt'])}
    res={'rung':'frontier_band_constant_vs_deleted','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,'band':list(BAND),
         'baseline':r_base,'band_constant':r_const,'band_zero':r_zero,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(r_base['L2_F'],4),
                    'L2_F_band_constant':round(r_const['L2_F'],4),
                    'L2_F_band_zero':round(r_zero['L2_F'],4),
                    'cost_constant_nats':round(cost_const,4),
                    'cost_zero_nats':round(cost_zero,4),
                    'separation_nats':round(sep,4),
                    'L2_C_baseline':round(r_base['L2_C'],4),
                    'L2_C_band_constant':round(r_const['L2_C'],4),
                    'L2_C_band_zero':round(r_zero['L2_C'],4),
                    'parameters_saved_by_constants':int(PERLAYER*len(BAND)),
                    'vectors_remaining_if_constant':len(BAND),
                    'vectors_remaining_if_deleted':0},
    ...
    print(f"(b) separation {sep:+.4f} >= {BARS['separation']} (zero-cost minus const-cost): {'HELD' if pb else 'FAILED'}")
    print(f"(c) const arm reproduces SS2875's 0.0000 (dev {repro:.4f}): {'HELD' if pc else 'FAILED'}")
    print(f"(d) deleting the stage costs {cost_zero:+.4f} >= {BARS['delete_costs']}: {'HELD' if pd else 'FAILED'}")
