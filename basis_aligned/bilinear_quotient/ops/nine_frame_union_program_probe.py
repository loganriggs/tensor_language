#!/usr/bin/env python
"""nine_frame_union_program_probe -- the program v2 priced across k: 8 block frames + the bus, every early write confined to
span(F_next) U span(U_8) (union rule, §2768), late reads/writes through U_8 with the remainder routed to the readout; k in
{1024, 896, 768, 1088} plus BLOCK8_1088 reads-only. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_nine_frame_program_under_p045_at_1024 pred_c_union_writes_under_p045_at_768
#                     pred_d_nine_frame_program_under_p13_at_896 pred_e_nine_frame_program_under_p025_at_1088
...
Preregistration: polynomial_causal/NINE_FRAME_UNION_PROGRAM_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/nine_frame_union_program_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("nine_frame_union_program_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "NINE_FRAME_UNION_PROGRAM_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "attn67_handoff_probe_results.json"   # §2768
OUT = ROOT / "nine_frame_union_program_probe_results.json"
HASHES = {PREREG: "f0d0cc656594cfa2b68e6f3420a9786f53c2212d1a3cdd38e66b551f2ac9761c", PRIOR: "7ae23748391393bf1c0e639666eabcefde0036e30ff557bc1403193ea2e1d6fa",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "nine_frame_union_program_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_max": 0.045, "c_max": 0.045, "d_max": 0.130, "e_max": 0.025}
NULLS = {"b_min": 0.060, "c_min": 0.080, "d_min": 0.180, "e_min": 0.045}
...
def main():
    ...
    class RouteUnion:
        """Early write confined to span(U_next) ∪ span(U_bus): the remainder outside U_next is replaced by its projection onto U_bus."""
        def __init__(self, s_, k, U_next, U_bus):
            self.Un = U_next[:, :k]; self.Ub = U_bus[:, :k]; self.mu = wheads[s_]["mu"]
        def hook(self, idx):
            pass
        def __call__(self, w, x):
    ...
    def next_block_frame(s_):
        kd, l = s_
        if kd == "attn":
            return FB[l]
        return FB[l + 1] if l + 1 < SPLIT else U_8
    def block_reads(k):
    ...
    def p9(k):
        patch = {}; buf = Buf()
        for s_ in OWN8:
            patch[s_] = Both(head(s_, k, FB[s_[1]]), RouteUnion(s_, k, next_block_frame(s_), U_8))
        for s_ in SET8:
    ...
    KS = [1024] if smoke else [1024, 896, 768, 1088]
    for k in KS:
        arms[f"P9_{k}"] = p9(k); log(stage="p9", k=k, ce=round(arms[f"P9_{k}"], 4))
    arms["BLOCK8_1088"] = block_reads(1088)
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    PRIOR_BLOCK8 = {1024: 0.03744, 896: 0.1099, 768: 0.2253, 1088: arms["BLOCK8_1088"]}
    W = {f"W_{k}": arms[f"P9_{k}"] - PRIOR_BLOCK8[k] for k in KS}
    g = lambda k: arms.get(f"P9_{k}", arms["P9_1024"])
    ...
    preds = {
        ...
        'pred_b_nine_frame_program_under_p045_at_1024': bool(g(1024) <= BARS["b_max"]),
        'pred_c_union_writes_under_p045_at_768': bool(W.get("W_768", W["W_1024"]) <= BARS["c_max"]),
        'pred_d_nine_frame_program_under_p13_at_896': bool(g(896) <= BARS["d_max"]),
        'pred_e_nine_frame_program_under_p025_at_1088': bool(g(1088) <= BARS["e_max"]),
    }
    nulls = {"b_null_ge_.06": bool(g(1024) >= NULLS["b_min"]), "c_null_ge_.08": bool(W.get("W_768", W["W_1024"]) >= NULLS["c_min"]),
             "d_null_ge_.18": bool(g(896) >= NULLS["d_min"]), "e_null_ge_.045": bool(g(1088) >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "9 frames (8 block frames + bus U_8); early writes confined to span(F_next) U span(U_8) (union rule); late reads/writes through U_8, remainder to readout",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024},
           "ce_added": arms, "write_cost_over_block8_reads": W, "prior_block8_reads": {str(k): v for k, v in PRIOR_BLOCK8.items()},
    ...
    if smoke:
        print(json.dumps({k: out[k] for k in ("preds", "nulls", "instrument", "ce_added", "write_cost_over_block8_reads", "price")}, indent=1)); return
    from receipt import dump
    dump(out, OUT)
    print(json.dumps({"preds": preds, "nulls": nulls, "ce_added": arms, "write_cost_over_block8_reads": W}, indent=1))
