"""METRIC-CHOSEN TAIL SPANS (rung 26): choose each tail MLP's rank-8 span under the observability metric of the block after it.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['stream_error_profile_results.json','piece_price_ladder_results.json','metric_units_top8_fresh8_results.json']
    ...
    print(f"DRYRUN OK: S2086 present (block 6 {_p['6']}); three arms (plain / metric-spans / random-spans) x 9 windows")
    raise SystemExit(0)
import json, sys, time, torch
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
import os
...
OUT=PT+'metric_tail_spans_results.json'
...
def main():
    ...
    for mode in ('metric-spans','random-spans'):
        SPANS[mode]={}
        for li in TAILC:
            accs=[]
            for i in range(0,120,6):
            ...
            half,inv=MET[('metric' if mode=='metric-spans' else 'random-metric',f't{li}')]
            _,_,Vh=torch.linalg.svd((Y-Yb)@half, full_matrices=False)
            Q=orth((inv@Vh[:8].T).contiguous())
            SPANS[mode][li]=(Q,Yb)
    ...
    def build_arm(mode):
        ...
        tag={'plain':'E','metric-spans':'S','random-spans':'X'}[mode]
        act=['a0']
    ...
    res={}
    for mode in ('plain','metric-spans','random-spans'):
    ...
    gains8=[a-b for a,b in zip(E['gaps_fresh'],res['metric-spans']['gaps_fresh'])]
    pa=g1('metric-spans')>=0.05; pb=g1('random-spans')<=0.02; pc=stt.median(gains8)>=0.025
    pd=abs(E['profile'][6]-1.7415)<=0.10
    out={'base_ce_w1':round(base1,5),'arms':res,'gain_w1':{k:round(g1(k),4) for k in res if k!='plain'},
         'fresh_gains_metric':[round(g,4) for g in gains8],'fresh_gain_median':round(stt.median(gains8),4),
         'fresh_gains_random':[round(a-b,4) for a,b in zip(E['gaps_fresh'],res['random-spans']['gaps_fresh'])],
         'pred_a_metric_spans_beat_cfgE':bool(pa),'pred_b_random_control':bool(pb),'pred_c_transfers':bool(pc),
    ...
    print(f"(a) metric-spans gain w1 {g1('metric-spans'):+.4f} >= 0.05: {'HELD' if pa else 'FAILED'}")
    print(f"(b) random-spans gain w1 {g1('random-spans'):+.4f} <= 0.02: {'HELD' if pb else 'FAILED'}")
    print(f"(c) fresh median gain {stt.median(gains8):+.4f} >= 0.025: {'HELD' if pc else 'FAILED'}")
