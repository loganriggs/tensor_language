...
"""MORE DATA PER STEP, not more steps (v779). v778 ran the joint rank-32 fit for 1200 steps on Pythia-160m final and it OVERFIT: the validation minimum came at step 200 and held-out cost climbed from +0.226 to +0.565 by step 1200 (recovery 0.928, no better than v760's 0.932 at 300 steps). But v777 showed the same heads fitted ONE AT A TIME reach 60% lower cost, so the joint fit is gradient-starved per head rather than step-starved. This rung keeps 300 steps and quadruples the tokens per step (train batch 8 -> 32, 4x the gradient signal per head per step), everything else as v760. If recovery rises above 0.95 the joint numbers across the ladder are fitting-limited; if it stays near 0.93 the limit is the program class applied jointly. ROW-CENTRED logits (Logan, 22 Sep: 'test the specific hypothesis of why we're seeing what we're seeing'). MODEL = EleutherAI/pythia-160m (12 x 12 x 768, softmax, ROTARY on 16 of 64 head dims, GPT-NeoX tokenizer;
...
PREDICTIONS (scored as written; failures preserved)
    ...
    pred_c_rank32_cost          rank-32 snapshot recovery >= 0.95 (v760 batch 8: 0.932; v778 1200 steps: 0.928). Prior: unsure
    pred_d_rank16_cost          (fit length used) the validation minimum is reached AFTER step 100 — i.e. the bigger batch delays overfitting. Prior: likely
    pred_e_rank32_values_preserved_scale rank-32: median value ratio in [0.5, 2] over the 24 most valuable heads. Prior: likely
PRICE (registered maximum): stock 7 + explicit 7; means 16; kernels 2; 144 x 7 = 1008; all-ablated 7; kernels-only 7 + col0 7; ladder 35; 1 arm x (kappa_r 26 + step-0 10 + 300 + 12 x (26 + 7 + 7) + snapshot 9) = 825 + manipulability 168; total ~2150 forwards, 300 backwards. Bars: forwards <= 2500, backwards <= 300.
"""
...
FORWARDS_MAX, BACKWARDS_MAX = 2500, 300
EBATCH, TBATCH, STEPS, EVAL_EVERY, N_VAL, N_MANIP = 32, 32, 300, 25, 96, 24
LR_MAP, LR_MAP_MIN, LR_K, LR_K_MIN = 3e-4, 3e-5, 0.01, 0.001
KS, ARMS = (1, 2, 4, 8, 16), (32,)
REPLAY_TOL, E4_MIN, REC32, STEP_MIN, RATIO = 0.002, 0.90, 0.95, 100, (0.5, 2.0)
PREDICTIONS = {"pred_a_instrument_replays": "+-0.002", "pred_b_kernel_energy_top4": ">= 0.90", "pred_c_rank32_cost": "recovery >= 0.95 (v760: 0.932, v778: 0.928)", "pred_d_rank16_cost": "validation minimum after step 100", "pred_e_rank32_values_preserved_scale": "median ratio in [0.5, 2]"}
...
def main() -> None:
    plan = {"candidate_id": CANDIDATE_ID, "forwards_max": FORWARDS_MAX, "model_backwards": BACKWARDS_MAX, "model_updates": 0, "fit_parameters": "2 arms of factored q/k maps + kernels",
            ...
            "bars": {"replay_tol": REPLAY_TOL, "e4_min": E4_MIN, "rec32": REC32, "step_min": STEP_MIN, "ratio": RATIO}}
    ...
    predictions = {"pred_a_instrument_replays": abs(native - native_stock) <= REPLAY_TOL, "pred_b_kernel_energy_top4": float(energy[3]) >= E4_MIN, "pred_c_rank32_cost": r32["recovery"] >= REC32,
                   "pred_d_rank16_cost": r32["chosen_step"] > STEP_MIN, "pred_e_rank32_values_preserved_scale": RATIO[0] <= r32["median_ratio"] <= RATIO[1]}
