"""THE FRONT OF THE MOTIF BAND: HOW MUCH DO a2, a3 AND a4 OVERLAP? (Claude, LANE 1)

SS2889 profiled every motif layer and found the frontier's motif error is a FRONT-OF-BAND phenomenon: a2 .1946, a4 .1941, a3 .1579 --
73.5% of the band's .7441 summed share -- while a5, the "price cliff", is FIFTH of eight at .0597. It also found the layers strongly
SUBadditive: eight individual shares summing to .7441 against SS2882's .3988 for restoring the whole band at once.

Subadditivity decides what a fix has to look like, and SS2889 could not localise it: improving a2 alone will recover less than its
...
Eight arms differ only in the `ML` list passed to `evalM`, so they share ONE fitted stack under the fit-once/eval-many pattern
(`ops/frontier_evalarms.py`, validated at a baseline deviation of exactly 0.0 in both SS2888 and SS2889): **one pipeline run instead of
eight.** pred_c is the cross-rung check -- the three singles must land on SS2889's measured values.

SIGN CONVENTION (SS2135): frontier L2 is CE ADDED ABOVE THE REAL MODEL, so **LOWER IS BETTER** (SS312: +2.6735 beating +2.84/+2.93).
An ERROR SHARE = L2_F(baseline) - L2_F(subset restored to real), **POSITIVE = that subset's approximation costs that much**.
...
#                     pred_b_the_front_band_is_subadditive
#                     pred_c_the_singles_reproduce_S2889
#                     pred_d_the_arms_are_connected
#                     pred_e_most_pairs_are_subadditive

Preregistration: polynomial_causal/FRONTIER_FRONT_BAND_INTERACTION_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run, eight evaluations (baseline + all subsets of a2,a3,a4)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_front_band_interaction_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_FRONT_BAND_INTERACTION_PREREGISTRATION.md'
PREREG_SHA = "4b157e716c403714f196692e2c9434d358023e20fb6b3c576d18493e5c52541e"
...
S2889 = PT + 'frontier_motif_band_profile_results.json'
S2889_SHA = "444a40253595d9dd214ed30d4d609b1989d9deb3557f3133267ebdee96f33e29"
BAND = list(range(2,10))
FRONT = [2,3,4]
SUBSETS = [(2,), (3,), (4,), (2,3), (2,4), (3,4), (2,3,4)]
BARS = {"reproduce": 0.05, "subadditive": 0.10, "singles_repro": 0.01, "connected": 0.005, "n_pairs": 2}
NULLS = {"subadditive_le": 0.02, "singles_repro_ge": 0.03}
...
if __name__=='__main__':
    ...
    if _sha(S2889) != S2889_SHA:
        raise RuntimeError(f'frozen hash mismatch: {S2889}')
    prev = json.load(open(S2889))['summary']['error_share_by_layer']
    SEL['mode']='norm'

    def _name(sub): return "a" + "".join(str(x) for x in sub)
    ARMS[:] = [('baseline', {})] + [(_name(s), {'motif': [x for x in BAND if x not in s]}) for s in SUBSETS]
    print(f'ONE pipeline run, {len(ARMS)} evaluations against the same fitted stack',flush=True)
    out=main()

    base = out['baseline']['L2_F']
    # SIGN: L2 is CE ADDED ABOVE THE REAL MODEL, LOWER IS BETTER -> share = baseline - restored, POSITIVE
    # = that subset's motif approximation costs that much.
    sh = {_name(s): round(base - out[_name(s)]['L2_F'], 4) for s in SUBSETS}
    singles = {l: sh[f'a{l}'] for l in FRONT}
    sum_singles = round(sum(singles.values()), 4)
    triple = sh['a234']
    subadd = round(sum_singles - triple, 4)          # POSITIVE = subadditive (overlapping error)
    pair_inter = {f'a{a}{b}': round(sh[f'a{a}{b}'] - (singles[a] + singles[b]), 4)
                  for a, b in ((2,3), (2,4), (3,4))}
    n_neg = sum(1 for v in pair_inter.values() if v < 0)
    dev = max(abs(singles[l] - prev[f'a{l}']) for l in FRONT)

    pa = abs(base-2.6735) <= BARS['reproduce']
    pb = subadd >= BARS['subadditive']
    pc = dev <= BARS['singles_repro']
    pd = all(abs(singles[l]) >= BARS['connected'] for l in FRONT)
    pe = n_neg >= BARS['n_pairs']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_front_band_is_subadditive':bool(pb),
           'pred_c_the_singles_reproduce_S2889':bool(pc),
           'pred_d_the_arms_are_connected':bool(pd),
           'pred_e_most_pairs_are_subadditive':bool(pe)}
    nulls={'b_null_the_front_band_is_additive':bool(subadd<=NULLS['subadditive_le']),
           'c_null_this_rung_disagrees_with_S2889':bool(dev>=NULLS['singles_repro_ge'])}
    res={'rung':'frontier_front_band_interaction','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,'subsets':[list(s) for s in SUBSETS],
         'arms':out,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(base,4),
                    'share_by_subset':sh,
                    'singles':{f'a{l}':singles[l] for l in FRONT},
                    'sum_of_singles':sum_singles,
                    'share_triple_a234':triple,
                    'subadditivity_nats':subadd,
                    'pairwise_interactions':pair_inter,
                    'n_pairs_subadditive':n_neg,
                    'S2889_singles':{f'a{l}':prev[f'a{l}'] for l in FRONT},
                    'max_single_deviation':round(dev,4),
                    'S2889_whole_band_sum':0.7441,'S2882_whole_band_joint':0.3988,
                    'pipeline_runs_saved_vs_per_arm':len(SUBSETS)},
         ...
         'hashes':{PREREG:PREREG_SHA,S2889:S2889_SHA},'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {base:.4f} vs 2.6735 (tol .05): {'HELD' if pa else 'FAILED'}")
    print("    " + "  ".join(f"{k}={v:+.4f}" for k,v in sh.items()))
    print(f"(b) singles {sum_singles:+.4f} - triple {triple:+.4f} = {subadd:+.4f} >= {BARS['subadditive']}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) max single deviation vs SS2889 {dev:.4f} <= {BARS['singles_repro']}: {'HELD' if pc else 'FAILED'}")
    print(f"(e) {n_neg}/3 pairs subadditive: {'HELD' if pe else 'FAILED'}")
    print(f'wrote {OUT} ({res["price"]["gpu_seconds"]:.0f}s, 1 pipeline run instead of {len(SUBSETS)+1})')
