#!/usr/bin/env python3
# BQLANE: gpu
# BQGATE: 9forwards2144seq; fresh subject-number rank-one causal confirmation;0updates.
...
PREREG = POLY / "SUBJECT_NUMBER_RANK1_FRESH_CONFIRMATION_V1_PREREGISTRATION.md"
RANK1 = POLY / "SUBJECT_NUMBER_DIRECTION_CARDINALITY_RANK1_V1_ARTIFACT.json"
BINDING = POLY / "SUBJECT_NUMBER_RANK1_FRESH_CONFIRMATION_V1_BINDING.json"
OUT = ROOT / "circuits/fast_screens/subject_number_rank1_fresh_confirmation_v1_result.json"
...
PREDICTION_REGISTRY = {
    ...
    "pred_b_rank1_reproduces_original_fresh": None,
    "pred_c_rank1_substitutes_native_fresh": None,
...
def compile_plan():
    binding, rank = load_bound(); original.validate_preflight(); licensing.validate_causal_preflight(capability.build_gate(), capability.RESULT, capability.LICENSE, expected_license_sha256=binding["files"]["capability_license"], causal_candidate_id=authority.CAUSAL_CANDIDATE_ID)
    return {"schema": "subject_number_rank1_fresh_confirmation_v1_plan", "model_loaded": False, "gpu_accessed": False,
...
def score(evidence, exactness, plan):
    ...
    templates = {t: stats([x for x in evidence if x["template"] == t], "native_exact_q", "rank1_q") for t in ("behind_beside", "among_behind")}
...
def main():
    ...
    terminal = "rank1_fresh_confirmed" if all(scored["predictions"].values()) else ("invalid" if not scored["predictions"][PRED_KEYS[0]] else "rank1_fresh_confirmation_null")
    payload = managed.atomic_create_json(OUT, {"schema": "subject_number_rank1_fresh_confirmation_v1_result", "terminal": terminal,
