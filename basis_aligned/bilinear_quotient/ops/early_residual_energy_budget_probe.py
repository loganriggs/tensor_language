#!/usr/bin/env python
"""early_residual_energy_budget_probe -- measures the energy budget of the early residual stream (pre-blend residual, post-blend
residual, attention write, MLP write per block) so that §2759/§2760's inferences about the blend x <- l0*x + l1*x0 (inert from
block 2; block 1 restarts from 8*x0) rest on measured norms. One instrument CE arm. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_block1_blend_is_embedding_dominated pred_c_blend_is_minor_from_block_2
#                     pred_d_mlp_writes_carry_more_energy pred_e_mlp0_is_the_largest_early_write

SIGN CONVENTION (§2135): the CE number is CE ADDED ABOVE THE REAL MODEL on held-out docs 0-63 -- LOWER IS BETTER. Energies are
mean squared norms per token (||x0||^2 = 1152). Descriptive; nothing installs into the §312 frontier.
Preregistration: polynomial_causal/EARLY_RESIDUAL_ENERGY_BUDGET_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/early_residual_energy_budget_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("early_residual_energy_budget_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "EARLY_RESIDUAL_ENERGY_BUDGET_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "block_boundary_blend_rotation_probe_results.json"   # §2759
OUT = ROOT / "early_residual_energy_budget_probe_results.json"
HASHES = {PREREG: "52d4e8a7af26492bc27cb8f14c6f9da3af9770f7550bccc34dba26ef675664a3", PRIOR: "403dbd6f9412cfcbbeeb7aef6c727203e6cc953d4141cc212840e7679f672606",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "early_residual_energy_budget_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_min": 1.0, "c_max": 0.25, "d_min": 2.0, "e_min": 3.0}
NULLS = {"b_max": 0.25, "c_min": 1.0, "d_max": 1.0, "e_max": 1.2}
...
def main():
    ...
    en = {}
    def ecb(s_, w):
        if s_[0] in ("attn", "mlp", "pre", "post"):
            v = w.reshape(-1, D).double(); e = en.setdefault(s_, [0.0, 0]); e[0] += float((v * v).sum()); e[1] += int(v.shape[0])
    for i in range(0, fit_rows.shape[0], CH):
        forward(m, fit_rows[i:i + CH, :TI], collect=ecb)
    E = {f"{s_[0]}{s_[1]}": v[0] / max(v[1], 1) for s_, v in en.items()}
    lam = {l: (float(m.transformer.h[l].lambdas[0]), float(m.transformer.h[l].lambdas[1])) for l in range(NL)}
    r = {l: (lam[l][1] ** 2 * D) / max(lam[l][0] ** 2 * E[f"pre{l - 1}"], 1e-12) for l in range(1, NB)}
    r_med_2_7 = float(torch.tensor([r[l] for l in range(2, NB)]).median())
    mw_over_aw = [E[f"mlp{l}"] / max(E[f"attn{l}"], 1e-12) for l in range(NB)]; d_med = float(torch.tensor(mw_over_aw).median())
    others = [E[f"attn{l}"] for l in range(NB)] + [E[f"mlp{l}"] for l in range(1, NB)]; e_ratio = E["mlp0"] / max(max(others), 1e-12)
    log(stage="energy", r=({l: round(v, 3) for l, v in r.items()}), d_med=round(d_med, 3), e_ratio=round(e_ratio, 3),
        pre=[round(E[f"pre{l}"]) for l in range(NL - 1)], post=[round(E[f"post{l}"]) for l in range(NL)],
        aw=[round(E[f"attn{l}"]) for l in range(NL)], mw=[round(E[f"mlp{l}"]) for l in range(NL)])
    ...
    preds = {
        ...
        'pred_b_block1_blend_is_embedding_dominated': bool(r[1] >= BARS["b_min"]),
        'pred_c_blend_is_minor_from_block_2': bool(r_med_2_7 <= BARS["c_max"]),
        'pred_d_mlp_writes_carry_more_energy': bool(d_med >= BARS["d_min"]),
        'pred_e_mlp0_is_the_largest_early_write': bool(e_ratio >= BARS["e_min"]),
    }
    nulls = {"b_null_le_.25": bool(r[1] <= NULLS["b_max"]), "c_null_ge_1": bool(r_med_2_7 >= NULLS["c_min"]),
             "d_null_le_1": bool(d_med <= NULLS["d_max"]), "e_null_le_1.2": bool(e_ratio <= NULLS["e_max"])}
    gaps = {"r1": r[1], "r_median_2_7": r_med_2_7, "mw_over_aw_median_0_7": d_med, "mlp0_over_largest_other_early_write": e_ratio}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           "sign_convention": "CE ADDED above the real model on held-out docs 0-63 (FRESH split; LOWER IS BETTER); energies are mean squared norm per token, ||x0||^2 = 1152",
           ...
           "ce_added": arms, "gaps": gaps, "energies": E, "lambdas": {str(l): v for l, v in lam.items()}, "blend_ratio_r": {str(l): v for l, v in r.items()}, "mw_over_aw": mw_over_aw,
