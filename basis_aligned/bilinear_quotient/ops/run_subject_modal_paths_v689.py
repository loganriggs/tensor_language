...
"""Selected derivative paths on the now-opened v687 rows.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v689_result.json'
PREDICTIONS=dict(pred_a_exact_replay='original replay plus baseline tangent centralfinitechange relative error<=.01',
 pred_b_effect_prediction='full gradient modal prediction<=.05 for A/B everycell',
 pred_c_capable_position_transfer='some pruned derivative path has modal prediction<=.05 for A/B everycell')


def main():
    plan=dict(prefix_calls=8,suffix_calls=56,sequences_per_arm=48,fits=0,backwards=0,extra_attention_pullbacks=24,extra_reference_attention_forwards=48,extra_mlp_pullbacks=36,analytic_reverse_passes=4,attention_pullbacks=24,reference_attention_forwards=48,additional_tangent_suffix_calls=8,additional_initial_interface_suffix_calls=12,
    ...
    path_reports={name:{source:{} for source in ['A','B','sum']} for name in list(path_specs)+['full']}
    path_support=[]
