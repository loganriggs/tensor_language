#!/usr/bin/env python
"""late_tail_channel_rank_probe -- is the tail channel the late MLPs read LOW-RANK: the 768-complement of the bus re-ordered by the
pooled covariance of the late-origin component (lambda-propagated writes of blocks 8..l, scaled, off the core); late MLPs read the
core exactly plus the top-r channel directions, compared with the bus's own order at 768 + r. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_channel_order_beats_bus_order_at_128 pred_c_channel_order_beats_bus_order_at_64
#                     pred_d_256_channel_dims_close_the_gap pred_e_channel_is_low_rank
...
Preregistration: polynomial_causal/LATE_TAIL_CHANNEL_RANK_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_tail_channel_rank_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_tail_channel_rank_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_TAIL_CHANNEL_RANK_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_tail_writer_kind_probe_results.json"   # §2778
OUT = ROOT / "late_tail_channel_rank_probe_results.json"
HASHES = {PREREG: "57bee12112d1657995740602067d312ca99fdc7ba1fc6b3b3541b104ec4774e0", PRIOR: "09e55bea2b0fd5b9fb08df978bc119693dbd813afa2b6fe9ded46bb136c0b354",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_tail_channel_rank_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_SPLIT8_1024 = 0.0374; PRIOR_LATE_MLP_768 = 0.1249; PRIOR_LATE_MLP_896 = 0.0662; LAM = 1e-2
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_gain": 0.015, "c_gain": 0.010, "d_max": 0.010, "e_max": 150.0}
NULLS = {"gain_null": 0.003, "d_min": 0.030, "e_min": 300.0}
...
def main():
    ...
    class Tracker:
        ...
        def __call__(self, w, x):
            ...
            return w
    class ZRec:
        """MLP-site recorder: accumulates the pooled covariance of the late-origin component off the core; then adds its write to z."""
        def __init__(self, l, box, acc):
            self.l = l; self.box = box; self.acc = acc; self.mlp = m.transformer.h[l].mlp
        def hook(self, idx): pass
        def __call__(self, w, x):
            xh = F.rms_norm(x, (D,)); scale = xh.norm(dim=-1, keepdim=True) / x.norm(dim=-1, keepdim=True).clamp_min(1e-12)
            t = perp(self.box["z"] * scale).reshape(-1, D).double(); self.acc["G"] += t.T @ t; self.acc["s"] += t.sum(0); self.acc["n"] += t.shape[0]
            self.box["z"] = self.box["z"] + w + self.mlp.Down_bias
            return w
    acc = {"G": torch.zeros(D, D, dtype=torch.float64, device=DEV), "s": torch.zeros(D, dtype=torch.float64, device=DEV), "n": 0}
    for i in range(0, fit_rows.shape[0], CH):
        idx = fit_rows[i:i + CH, :TI]; bx = {}
        pt = {("attn", l): Tracker(l, bx) for l in LATE}; pt.update({("mlp", l): ZRec(l, bx, acc) for l in LATE})
        for o in pt.values():
            o.hook(idx)
        forward(m, idx, patch=pt)
    mz = acc["s"] / acc["n"]; Cz = acc["G"] / acc["n"] - torch.outer(mz, mz)
    evz, Uz = torch.linalg.eigh(Cz); Uz = Uz.flip(1).float(); evz = evz.flip(0).clamp_min(0)
    spec = R.spectrum(Cz.cpu())
    log(stage="channel_spectrum", eff_rank=round(spec["eff_rank"], 1), rank_90=spec["rank_90"], top8=[round(float(v), 5) for v in evz[:8]])
    class ChanHead(OwnHead):
        """MLP l reads the core (top-768 of U_8) exactly plus the top-r channel directions; the rest of the tail is the fit-set constant."""
        def __init__(self, l, r):
            super().__init__(l, 768, "const", Uk); self.Ur = Uz[:, :r]
        def __call__(self, w, x):
            xh = F.rms_norm(x, (D,)); t = perp(xh - self.mx)
            xp_ = (xh @ Uk) @ Uk.T + perp(self.mx) + (t @ self.Ur) @ self.Ur.T
            return self.mlp.Down(self.mlp.Left(xp_) * self.mlp.Right(xp_))
    def bus(k):
        return run({("mlp", l): head(("mlp", l), k, U_8) for l in range(SPLIT, NL)})
    def chan(r):
        return run({("mlp", l): ChanHead(l, r) for l in range(SPLIT, NL)})
    arms = {"SPLIT8_1024": split8(1024), "LATE_MLP_768": bus(768), "BUS_896": bus(896), "CH_128": chan(128)}
    if not smoke:
        arms["BUS_832"] = bus(832); arms["BUS_1024"] = bus(1024); arms["CH_64"] = chan(64); arms["CH_256"] = chan(256)
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    C0 = arms["LATE_MLP_768"]; B896 = arms["BUS_896"]; B832 = arms.get("BUS_832", B896); C128 = arms["CH_128"]; C64 = arms.get("CH_64", C128); C256 = arms.get("CH_256", C128)
    summ = {"gain_128_vs_bus896": B896 - C128, "gain_64_vs_bus832": B832 - C64, "ch_256": C256, "bus_1024_mlp_only": arms.get("BUS_1024"),
            "channel_eff_rank": spec["eff_rank"], "channel_rank_90": spec["rank_90"], "channel_top_eigs": [float(v) for v in evz[:16]]}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(C0 - PRIOR_LATE_MLP_768) <= BARS["repro_tol"] and abs(B896 - PRIOR_LATE_MLP_896) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_channel_order_beats_bus_order_at_128': bool(C128 <= B896 - BARS["b_gain"]),
        'pred_c_channel_order_beats_bus_order_at_64': bool(C64 <= B832 - BARS["c_gain"]),
        'pred_d_256_channel_dims_close_the_gap': bool(C256 <= BARS["d_max"]),
        'pred_e_channel_is_low_rank': bool(spec["eff_rank"] <= BARS["e_max"]),
    }
    nulls = {"b_null_no_gain": bool(C128 >= B896 - NULLS["gain_null"]), "c_null_no_gain": bool(C64 >= B832 - NULLS["gain_null"]),
             "d_null_ch256_ge_.03": bool(C256 >= NULLS["d_min"]), "e_null_eff_rank_ge_300": bool(spec["eff_rank"] >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "blocks 8-17 MLP reads: core (top-768 of U_8) exact + top-r directions of the pooled late-origin tail covariance (CH_r), vs the bus's own order at 768 + r (BUS_k); rest constant",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "late_mlp_768": C0, "prior_late_mlp_768": PRIOR_LATE_MLP_768, "bus_896": B896, "prior_late_mlp_896": PRIOR_LATE_MLP_896},
