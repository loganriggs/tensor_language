# DOES THE RELIABILITY SIGNAL ORDER CE? -- connecting it to the arc's native unit.
#
# §1911-§1924 built the per-token reliability signal entirely in AGREEMENT: top-1 match with the live
# model over a permutation null. That is the right instrument for "does the program reproduce the model",
# but every cost result in this program -- the frontier (§1853-§1865), the map-rank law (§1880/§1881),
# the 46% build (§1882), the fallback lever (§1870) -- is stated in NATS. The two instruments have
# already disagreed once: §1893 and §1894 found the scale that maximises agreement is fifth of six by CE.
#
# So whether the signal orders CE is not implied by anything measured so far, and it is what decides
# whether the signal is usable by the cost side of this program at all.
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY, deployed 5,419 coverage. Rung 3.
#
# Registered predictions, TWO-SIDED per LESSONS 31, absolute nats per LESSON 40, read back per LESSON 39.
# Units per LESSONS 49: these are per-position CEs on a fixed population, compared within the population,
# never a raw CE compared across populations -- the error §1866 corrected.
#   pred_a THE SIGNAL ORDERS CE: on covered positions, the LOWEST margin quartile's CE exceeds the
#          HIGHEST quartile's by at least 0.5 nats on all three roles. The agreement gradient there is
#          ~+10x, so if the two instruments agree at all this should be large. If FALSE the signal is an
#          agreement-only object, useless to the cost arc, and §1911-§1924 describe fidelity to the model
#          without describing correctness -- which §1895 already hinted at by finding the program
#          reproduces the model's MISTAKES.
#   pred_b AND ON THE UNCOVERED ARM TOO: the same comparison on uncovered positions gives at least 0.3
#          nats. Weaker bar because the uncovered agreement gradient is ~+3x rather than ~+10x. If FALSE
#          the signal orders CE only where the program has a table, which would matter for any use that
#          abstains on low-confidence tokens -- most of those are uncovered.
#   pred_c AND THE POOLED CE REPRODUCES §1858: the all-position CE over every scored position is within
#          0.002 of §1858's PUBLISHED 6.01167 / 5.98477 / 6.00165. This lineage has been an agreement-only
#          instrument since §1885 and has never computed a CE; anchoring it to the arc's oldest published
#          number is what makes the rest readable.
#   pred_d CONTROLS: coverage is exactly 5,419; the four margin quartiles partition every scored position
#          on each arm; and the covered and uncovered CEs are reported separately, never pooled into one
#          number and compared across the two populations.
...
OUT = PT + 'ops/signal_orders_ce_results.json'
...
def compare_by_bucket(rows, hooks, seenmask=None):
    ...
    for i in range(0, rows.shape[0], 8):
        ...
        for i8 in range(4):
            for nm9, m9 in (('cov', iscov & (qidx == i8)), ('unc', (~iscov) & (uidx == i8))):
                if int(m9.sum()):
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'DOES THE SIGNAL ORDER CE at 5419 | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    S1858_CE = {'skip7000': 6.01167, 'skip11000': 5.98477, 'skip1200': 6.00165}

    def ce_(e, nm9, i8, f='ce'):
        return res['sites'][A1][e]['ce'][f'{nm9}_{i8}'][f]
    dcov = {e: ce_(e, 'cov', 0) - ce_(e, 'cov', 3) for e in roles}
    dunc = {e: ce_(e, 'unc', 0) - ce_(e, 'unc', 3) for e in roles}
    allce = {e: res['sites'][A1][e]['ce_allpos'] for e in roles}
    parts = all(sum(ce_(e, nm9, i8, 'n') for i8 in range(4)) > 0 for e in roles for nm9 in ('cov', 'unc'))
    pa = all(dcov[e] >= 0.5 for e in roles)
    pb = all(dunc[e] >= 0.3 for e in roles)
    pc = all(abs(allce[e] - S1858_CE[e]) <= 0.002 for e in roles)
    pd = ncov == NCOV and parts

    print(f'\n  CE by PRECOMPUTED token-margin quartile (q0 = least reliable), coverage {ncov}:',
          flush=True)
    for e in roles:
        print(f'    {e:10s} COVERED   ' + '  '.join(
            f'q{i8} {ce_(e, "cov", i8):.4f} (n{ce_(e, "cov", i8, "n"):6d})' for i8 in range(4))
            + f'   q0-q3 {dcov[e]:+.4f}', flush=True)
        print(f'    {"":10s} UNCOVERED ' + '  '.join(
            f'q{i8} {ce_(e, "unc", i8):.4f} (n{ce_(e, "unc", i8, "n"):6d})' for i8 in range(4))
            + f'   q0-q3 {dunc[e]:+.4f}', flush=True)
    print(f'\n  THE SIGNAL ORDERS CE on covered positions (>= 0.5 nats) -> {pa}  ' + '  '.join(
        f'{e} {dcov[e]:+.4f}' for e in roles), flush=True)
    print(f'  and on the UNCOVERED arm too (>= 0.3) -> {pb}  ' + '  '.join(
        f'{e} {dunc[e]:+.4f}' for e in roles), flush=True)
    print(f'  and the pooled CE reproduces §1858 -> {pc}  ' + '  '.join(
        f'{e} {allce[e]:.5f} vs {S1858_CE[e]:.5f}' for e in roles), flush=True)
    print(f'  coverage {ncov}, quartiles partition both arms -> control {pd}', flush=True)
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_orders_ce_covered': bool(pa),
                          'pred_b_orders_ce_uncovered': bool(pb),
                          'pred_c_pooled_ce_reproduces_S1858': bool(pc),
