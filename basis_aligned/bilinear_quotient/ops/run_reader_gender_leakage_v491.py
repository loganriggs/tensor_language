#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_every_reader_writes_both_axes_alike pred_c_9_6_gender_leakage_small pred_d_one_reader_carries_the_gender_leakage pred_e_singles_add_on_they_he
"""Which reader leaks gender (v491)? v490: the five readers' joint write swap closes they - he 0.803 and they - she 0.754 and moves he - she 0.22 as much
as they - he. Here each reader's write into the answer swapped singly (9.6, 12.4, 10.5, 15.1, 10.1): its they - she / they - he closure ratio and its
he - she leakage; 122 natural pairs.
PREDICTIONS (scored as written; failures preserved; priors from v440 / v490)
    pred_a_baseline_replays                   the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_every_reader_writes_both_axes_alike  for each of the five heads the they - she closure is >= 0.80 x its they - he closure
    pred_c_9_6_gender_leakage_small           9.6's he - she change is <= 0.15 x its they - he change
    pred_d_one_reader_carries_the_gender_leakage  the reader with the largest gender ratio has >= 2 x the ratio of the median reader. Prior: unsure
    pred_e_singles_add_on_they_he             the five singles sum to within 0.05 of 0.803 on they - he (replay of v484's 0.823 vs joint 0.803)
PRICE (registered maximum): 4 batches x 6 passes = 24 forwards; 0 backwards; 0 fits. Bar <= 25.
"""
...
OUT = ROOT / "circuits/followups/reader_gender_leakage_v491_result.json"
CANDIDATE_ID = "chain.reader_gender_leakage_v491"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, ALIKE_MIN, G96_MAX, LEAK_RATIO, FIVE_V485, BAND, UNIT = 1e-3, 0.80, 0.15, 2.0, 0.803, 0.05, 701
...
FORWARDS_MAX = 25
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_every_reader_writes_both_axes_alike": ">= 0.80 x, x 5", "pred_c_9_6_gender_leakage_small": "<= 0.15", "pred_d_one_reader_carries_the_gender_leakage": ">= 2 x median", "pred_e_singles_add_on_they_he": "0.803 +- 0.05"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "alike_min": ALIKE_MIN, "g96_max": G96_MAX, "leak_ratio": LEAK_RATIO, "five_v485": FIVE_V485, "band": BAND, "unit": UNIT}}
    ...
    SINGLES = {"9.6": {9: [6]}, "12.4": {12: [4]}, "10.5": {10: [5]}, "15.1": {15: [1]}, "10.1": {10: [1]}}
    MODES = {k: ({}, (0,)) for k in SINGLES}; SETS_W = SINGLES; forwards = 0
    def logits_margin(x):
    ...
    sc, gr = verb["they_she_closed"], verb["gender_over_number"]; import statistics
    ratios = sorted(gr.values()); report["alike"] = {k: sc[k] / mc[k] if abs(mc[k]) > 1e-9 else None for k in mc}
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_every_reader_writes_both_axes_alike": all(sc[k] >= ALIKE_MIN * mc[k] for k in mc), "pred_c_9_6_gender_leakage_small": gr["9.6"] <= G96_MAX,
                   "pred_d_one_reader_carries_the_gender_leakage": max(ratios) >= LEAK_RATIO * statistics.median(ratios), "pred_e_singles_add_on_they_he": abs(sum(mc.values()) - FIVE_V485) <= BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "reader_gender_leakage_result_v491", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
