...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_asm_alllayers_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; arms norm-2304 / cond-2304 / cond-1152 x eight windows")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_cond_halfprice_results.json'
...
if __name__=='__main__':
    ...
    print('ARM 1/3: norm-2304 + assembly-conditioned Fisher at sites 5/6',flush=True)
    r_norm=main()
    SEL['collect_asm']=False
    SEL['mode']='fisher8'; SEL['K']=2304
    print('ARM 2/3: conditioned-2304 (S2129 reproduction)',flush=True)
    r_f=main()
    SEL['mode']='fisher8'; SEL['K']=1152
    print('ARM 3/3: conditioned-1152 (half price at mlp4/mlp5)',flush=True)
    r_h=main()
    import statistics as stt
    g22=[round(a-b,4) for a,b in zip(r_f['fresh8'],r_norm['fresh8'])]
    ghp=[round(a-b,4) for a,b in zip(r_h['fresh8'],r_norm['fresh8'])]
    pa=stt.median(ghp)>=0.0
    pb=sum(g>=-0.01 for g in ghp)>=6
    pc=abs(stt.median(g22)-0.0481)<=0.015
    res={'norm2304':r_norm,'cond2304':r_f,'cond1152':r_h,
         'per_window_cond2304_minus_norm':g22,'per_window_cond1152_minus_norm':ghp,
         'pred_a_half_price_no_loss':bool(pa),'pred_b_windows_floor':bool(pb),'pred_c_reproduces_cond2304':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'cond2304 - norm per window: {g22}')
    print(f'cond1152 - norm per window: {ghp}')
    print(f"(a) half-price median {stt.median(ghp):+.4f} >= 0: {'HELD' if pa else 'FAILED'}")
    print(f"(b) >= -0.01 on {sum(g>=-0.01 for g in ghp)}/8 (bar 6): {'HELD' if pb else 'FAILED'}")
    print(f"(c) cond2304 median {stt.median(g22):+.4f} vs 0.0481 (tol 0.015): {'HELD' if pc else 'FAILED'}")
