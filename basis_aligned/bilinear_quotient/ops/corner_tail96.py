"""THE TOP OF THE PARETO (rung 247): corner + tail r96.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The corner holds the tail at its
certificate knee (r64). Does buying the tail's next octave (r96, +12.4M) push census below 0.058 and
harvest the 1.03-margin thresholds?
REGISTERED PREDICTIONS (arms: THIS vs corner 0.0608/9 and uniform r96 0.0622/9):
  (a) census <= 0.058.
  (b) valid >= 9 (certificates at least kept).
  (c) SANITY: census in [0.01, 0.12]; inert tripwires vs corner AND uniform-r96 receipts.
NULL: tail rank beyond the knee buys nothing (census >= 0.060 AND valid <= 9). PRICE: 84.5M pattern
values. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: corner + tail r96')
    raise SystemExit(0)
import torch
...
OUT=PT+'corner_tail96_results.json'
...
if __name__=='__main__':
    ...
    pb=nv>=9
    pc=0.01<=agg<=0.12
    if abs(agg-0.0608)<1e-4 or abs(agg-0.0622)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: tail96 map inert')
