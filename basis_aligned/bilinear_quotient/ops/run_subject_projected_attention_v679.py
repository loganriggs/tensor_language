...
"""Exact factorized attention12–17 in frozen v665 response coordinates.

48 opened prompts,4 prefixes,4 block11,96 attention,24 MLP evaluations.
Native and expanded same-function controls; null: folding/norm/axis bug.
Charge all compiled tensors; no fitting or semantic promotion.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_projected_attention_v679_result.json'
PREDICTIONS=dict(pred_a_attention_replay='output relativeL2 and maxabs/max(1,RMS)<=1e-4',
 pred_b_response_replay='finite response relativeL2<=1e-4 for each nonzero amplitude and expanded absolute difference<=1e-10',
 pred_c_causal_prefix='changing final-token coordinates leaves earlier projected outputs exactly unchanged')


def main():
    plan=dict(prefix_calls=4,block11_calls=4,attention_calls=96,mlp_calls=24,rows=48,fits=0,backwards=0,
    ...
    from factorized_two_qk_attention import compile_attention,execute
    from projected_two_qk_attention import compile_attention as expanded_compile,execute as expanded_execute
    ...
    generator=torch.Generator(device='cuda').manual_seed(679)
    relative=[];absolute=[];scaled_absolute=[];responses=[];causal=[];prices=[];equivalence=[]
    counts=dict(prefix_calls=0,block11_calls=0,attention_calls=0,mlp_calls=0)
    def nvalues(obj):
    ...
    for template in dict.fromkeys(r['template'] for r in rows):
        ...
        for layer in range(12,18):
            block=model.transformer.h[layer];a=block.attn;heads=a.n_head;hd=a.head_dim
            P=package['producer']['programs'][layer-12]['input_basis'].cuda()
            Q=(package['producer']['initial_encoder'] if layer==12 else package['producer']['output_encoders'][layer-13]).cuda()
            ...
            expanded=expanded_compile(weights,background.double(),P,Q,first.reshape(b,t,heads,hd).double(),
                a.lamb.double(),heads,cos[0,:,0].double(),sin[0,:,0].double(),eps,eps)
            prices.append(dict(template=template,layer=layer,batch=b,length=t,compiled_values=nvalues(program),expanded_values=nvalues(expanded)))
            ...
            for amplitude in plan['amplitudes']:
                ...
                if amplitude==0:native_state=native
                native=native.double()@Q;folded=execute(program,amplitude*z)
                equivalence.append(float((folded-expanded_execute(expanded,amplitude*z)).abs().max()))
                err=folded-native
            ...
            x=background+native_state;x=x+block.mlp(F.rms_norm(x,(d,)));counts['mlp_calls']+=1
        print(template,'native',max(relative),'response',max(responses),flush=True)
    a_ok=max(relative+scaled_absolute)<=1e-4 and counts==dict(prefix_calls=4,block11_calls=4,attention_calls=96,mlp_calls=24)
    b_ok=a_ok and max(responses)<=1e-4 and max(equivalence)<=1e-10;c_ok=a_ok and max(causal)==0.
    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,
        ...
        max_expanded_absolute_difference=max(equivalence),max_causal_prefix_difference=max(causal),prices=prices,wall_seconds=time.perf_counter()-tic,
    ...
    OUT.write_text(payload);print(json.dumps(result['predictions']));assert a_ok and b_ok and c_ok
