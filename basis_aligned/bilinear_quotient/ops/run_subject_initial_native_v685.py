...
"""Initial width8 interface with exact native suffix, checking final-projection confound.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v685_result.json'
PREDICTIONS=dict(pred_a_exact_replay='baseline and independent joint versus shared chain replay max absolute error <=1e-4',
 ...
 pred_c_capable_position_transfer='initial-interface exact native logits WITHOUT final projection A/B target<=10% and modal<=5% every cell')
...
def main():
    ...
    c=b and initial_counts==12 and all(cell['unprojected_relative_error']<=.10 and max(cell['unprojected_modal_error'])<=.05 for name in ['A','B'] for cell in initial_reports[name].values())
