#!/usr/bin/env python
# BQLANE: cpu
"""radial_gauge_map_probe -- at every one of the 36 write sites, split the write in the PRE-write frame (x̂ = unit residual the
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/radial_gauge_map_probe.py")
...
OUT = ROOT / "radial_gauge_map_probe_results.json"
HASHES = {PREREG: "7a5bfdc9a17d504c109d47d30c5925e6a118169191b92b7c438e0ba2fd3eaae9",
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.003, "mid_drop_max": 0.03, "final_drop_min": 0.30, "early_scale_max": 0.05}
...
def main():
    ...
    if smoke:
        ...
        ev = torch.randint(0, 50000, (4, 257), generator=g); sites = [("attn", 0), ("mlp", 1), ("mlp", 17)]
        ...
        ev = nat[EVAL[0]:EVAL[1]]; sites = ALL_SITES
    ...
    preds = {
        'pred_a_instrument': bool(abs(ce0 - prior_base) <= BARS["ce_tol"] and max(ident.values()) <= BARS["ce_tol"] and abs(d1 - prior_drop1) <= BARS["repro_tol"]) if not smoke else bool(max(ident.values()) <= BARS["ce_tol"]),
    ...
    out = {"rung": RUNG, "status": "complete", "smoke": smoke, "sign_convention": "CE ADDED above the real model on held-out docs 96-159; LOWER IS BETTER",
           ...
           "price": {"gpu_forwards": 0, "cpu_doc_forwards": int(ev.shape[0]) * (1 + len(sites) * len(ARMS)) + 12, "cpu_seconds": time.time() - started},
