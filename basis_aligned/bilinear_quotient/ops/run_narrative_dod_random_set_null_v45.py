...
"""Narrative tense DoD battery, step 4 (v45): matched-count random four-head-set null for SIMPLE (as v25/v31).

Sixteen seeded random four-head sets from the 158 other heads, removed along their own weight-only was/is readout
directions on the v43 rows; compared with the set {15.5, 11.3, 9.4, 9.1}.

PREDICTIONS: pred_a instrument <= 1e-4; pred_b set damage > max random; pred_c no random set LIVE; pred_d set >= 10 x median |random|.
PRICE (registered maximum): native 2 + producer 2 + set 2 + 16 x 2 = 38 forwards; bar <= 44.
"""
...
OUT = ROOT / "circuits/followups/narrative_tense_dod_random_set_null_v45_result.json"
CANDIDATE_ID = "narrative_tense.past_vs_present.dod_random_set_null_v45"
LIVE_FRACTION, LIVE_POSITIVE, TENFOLD, INSTRUMENT_TOL = 0.10, 0.75, 10.0, 1e-4
FORWARDS_MAX = 44
EXCLUDED = {(15, 5), (11, 3), (9, 4), (9, 1)}
POOL = [(l, h) for l in range(18) for h in range(9) if (l, h) not in EXCLUDED]
SETS = [tuple(sorted(random.Random(2026_09_17_45 + s).sample(POOL, 4))) for s in range(16)]


def main() -> None:
    rows = v43.build()
    if os.environ.get("BQLIB_DRYRUN") or os.environ.get("BQLIB_NO_MODEL"):
        print(json.dumps({"candidate_id": CANDIDATE_ID, "rows": len(rows), "random_sets": SETS, "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0, "fit_parameters": 0,
                          "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only"}, indent=2)); return
    t0 = time.perf_counter()
    backend = producer.Bilin18TorchBackend.load("cuda")
    fw = L.ManualForward(backend)
    comps = v43.SET + tuple(c for s in SETS for c in v25.components_for(s))
    ...
    arm, n = v1._run_arm(fw, rows, components=v43.SET, mode="project"); forwards += n
    ...
    print("set", round(comp["target_damage_mean"], 4), "random max", round(max(dmg), 4), "median|.|", round(med, 4))
    ...
    result = {"schema": "narrative_tense_dod_random_set_null_result_v45", "candidate_id": CANDIDATE_ID, "instrument_max_abs_error": instrument, "set": comp, "random": randoms,
