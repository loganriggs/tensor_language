"""RUNG 344 -- SIGNED a16 ADOPTION GATE FOR CONTEXT-QK72.

Execute the audited signed context-QK harness at rank72 and rescore its exact
receipt under rank72-specific frozen baseline, identity, and price bars.

Frozen predictions
------------------
pred_a_live_baseline_price_and_identity:
    Baseline <=.008/50 certificates, OOD max <=.050, fresh <=.010, exact
    context-RRR rank72/440-map identity and 521,572,662-scalar bill.
...
rank72 QK point and licenses rank64.
"""
...
OUT = ROOT / "a16_transfer_mixed72_context_metric_qk_results.json"
BASE_RESULT = ROOT / "mixed72_context_metric_qk_ood_results.json"
BASE_CEV = ROOT / "cev_mixed72_context_metric_qk.pt"
COMP_KO = ROOT / "cev_a16ko_mixed72_context_metric_qk.pt"
NATIVE_KO = ROOT / "cev_a16ko_native_context_qk72_gate.pt"
...
def main() -> None:
    if os.environ.get("BQLIB_DRYRUN") == "1":
        ...
        assert baseline["census_damage"] <= .008
        assert baseline["certificates_valid"] >= 50
        assert baseline["literal_standalone_scalars"] == SCALARS
        print("A16 TRANSFER CONTEXT-QK72 | dry run: baseline, fit, bill, bars valid")
        return
    ...
    pred_a = (result["unablated_census_damage"] <= .008
              and result["unablated_certificates_valid"] >= 50
    ...
    for key in list(result):
        if key.startswith("pred_") or key.startswith("null_"):
            result.pop(key)
    result.update({
        "status": "a16_transfer_mixed72_context_metric_qk_complete",
        "rung": 344,
        "claim_level": "direct_signed_a16_context_qk72_adoption_gate",
        ...
        "null_signed_context_qk72_transport_fails": bool(null),
    })
    OUT.write_text(json.dumps(result, indent=2) + "\n")
    print(json.dumps({key: value for key, value in result.items()
                      if key != "own_effect_ratios"}, indent=2), flush=True)
    print(f"rescored rank72 signed receipt at {OUT}", flush=True)
