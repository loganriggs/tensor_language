#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_8_1_writes_into_the_noun pred_c_8_1_into_next_replays pred_d_8_1_later_small pred_e_8_1_positions_sum_to_its_value_single
"""Head 8.1's writes by target position (v574). v551: 8.1's value at the noun swapped closes 0.154 of the he - she gap (every position that reads it). v573:
its write into noun + 1 alone closes 0.056. Here 8.1's write swapped into the noun itself (SELF: the self-copy of the 18 Sep folds), into noun + 1 (NEXT),
into every later position (LATER), and into every position from the noun on (ALL_POS), on the 61 pairs.
PREDICTIONS (scored as written; failures preserved; priors from v551 / v573 and the 18 Sep folds)
    pred_a_baseline_replays                   the unedited manual forward replays the model's he - she margins within 1e-3
    pred_b_8_1_writes_into_the_noun           SELF closes >= 0.08 (the self-copy is the larger half of 8.1's 0.154)
    pred_c_8_1_into_next_replays              NEXT closes 0.056 +- 0.02
    pred_d_8_1_later_small                    LATER closes <= 0.05
    pred_e_8_1_positions_sum_to_its_value_single  SELF + NEXT + LATER is within 0.05 of 0.154 (one head, one source, disjoint targets)
...
OUT = ROOT / "circuits/followups/gender_copier_8_1_by_target_v574_result.json"
CANDIDATE_ID = "gender.copier_8_1_by_target_v574"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, SELF_MIN, NEXT_V573, NBAND, LATER_MAX, SINGLE_V551, ADD, UNIT = 1e-3, 0.08, 0.056, 0.02, 0.05, 0.154, 0.05, 701
...
FORWARDS_MAX = 11
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_8_1_writes_into_the_noun": ">= 0.08", "pred_c_8_1_into_next_replays": "0.056 +- 0.02", "pred_d_8_1_later_small": "<= 0.05", "pred_e_8_1_positions_sum_to_its_value_single": "0.154 +- 0.05"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "self_min": SELF_MIN, "next_v573": NEXT_V573, "nband": NBAND, "later_max": LATER_MAX, "single_v551": SINGLE_V551, "add": ADD, "unit": UNIT}}
    ...
    MODES = {m: ({}, (0,)) for m in ("SELF", "NEXT", "LATER", "ALL_POS")}; SETS_W = {m: {8: [1]} for m in MODES}
    ...
    print("8.1 writes by target", {k: round(v, 3) for k, v in mc.items()}, "sum of parts", round(mc["SELF"] + mc["NEXT"] + mc["LATER"], 3))
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_8_1_writes_into_the_noun": mc["SELF"] >= SELF_MIN, "pred_c_8_1_into_next_replays": abs(mc["NEXT"] - NEXT_V573) <= NBAND,
                   "pred_d_8_1_later_small": abs(mc["LATER"]) <= LATER_MAX, "pred_e_8_1_positions_sum_to_its_value_single": abs(mc["SELF"] + mc["NEXT"] + mc["LATER"] - SINGLE_V551) <= ADD}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "gender_copier_8_1_by_target_result_v574", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
