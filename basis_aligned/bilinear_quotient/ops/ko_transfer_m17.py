"""SIGNAL-TO-FLOOR TEST (m17, rung 266): the BIG component - fidelity predicted to HOLD.

CONVENTION (S2135): per-position dCE = CE(mixed config with m17 mean-ablated) - CE(real model); LOWER
...
compression floor. m17 (battery own-mean 2.52 - the largest) is the strong arm of the preregistered pair.
REGISTERED PREDICTIONS:
  (a) OWN-FAMILY TRANSFERS: median member/ref ratio over m17-top circuits in [0.7, 1.6].
  (b) BIG COMPONENT - collateral Spearman {_rhoK
  (c) sanity census band.
NULL: the floor hypothesis is wrong in the strong direction (rho < 0.7 despite a huge signal).
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: knockout transfer m17')
    raise SystemExit(0)
import torch
...
OUT=PT+'ko_transfer_m17_results.json'
...
if __name__=='__main__':
    ...
    _hmu=m.transformer.h[17].mlp.register_forward_hook(_mucap)
    with torch.no_grad():
    ...
    m.transformer.h[17].mlp.register_forward_hook(_ablh)
    print('m17 mean captured over FW; gated ablation hook armed',flush=True)
    main()
    ...
    _own=sorted(r['member_absdce']/max(r['ref'],1e-9) for r in rows if _topK.get(r['tag'])=='m17')
    _omed=_own[len(_own)//2]
    _MR=json.load(open(PT+'removal_matrix_results.json'))['matrix']['m17']
    ...
    print(f"(b) BIG COMPONENT - collateral Spearman {_rhoK:.3f} >= 0.8; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) ablated census {agg:+.4f} in [0.5, 5.0]; era rho {rho:.4f}: {'HELD' if pc else 'FAILED'}")
