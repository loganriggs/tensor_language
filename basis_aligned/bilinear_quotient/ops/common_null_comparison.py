# THE COMMON-NULL COMPARISON -- which confidence signal actually matters more.
#
# §1911 split agreement by the LIVE model's top-2 margin (5.36 -> 10.11x, +4.75/+5.27/+5.07x). §1913
# split it by the program's PRECOMPUTED per-token margin (4.22 -> 5.29x, +1.07/+1.94/+1.06x). Those look
# like a decisive win for the live signal and they are NOT comparable.
#
# §1913 said so in its own output: all twelve of its quartile enrichments (3.98-5.92x) sit BELOW the
# pooled baseline of 7.16 / 7.29 / 7.49x. A stratified average cannot fall below its own pool unless the
# strata change the statistic -- and enrichment divides agreement by a permutation null computed WITHIN
# the stratum. Stratifying on a TOKEN property groups positions that share tokens, so the shuffled
# predictions collide far more often, the null rises, and every ratio deflates. §1911's live-margin
# strata are not token-homogeneous and do not pay that cost.
#
# The fix is one line of statistics: shuffle against the WHOLE covered arm rather than within the
# stratum, for both splits, so the denominators are the same object. Then the two gradients can be
# compared. This run reports both splits under both nulls, so the size of the artifact is visible rather
# than assumed.
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY. Rung 3: §1913's open question.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
#   pred_a THE ARTIFACT IS REAL AND LARGE: under the GLOBAL null, the token-margin quartiles no longer
#          sit below the pooled baseline -- the highest quartile exceeds 7.0x on all three roles, against
#          §1913's PUBLISHED 5.29 / 5.92 / 5.73 under the within-stratum null. If FALSE the deflation was
#          not the stratum null and §1913's caveat is wrong, which I would correct in place: the token
#          signal would then simply be weak.
#   pred_b AND THE LIVE SIGNAL IS BARELY AFFECTED: its quartile gradient under the global null is within
#          1.0x of §1911's PUBLISHED +4.75 / +5.27 / +5.07. §1911's strata are not token-homogeneous, so
#          they should pay little of the same cost. This is the control that makes pred_a interpretable:
#          if BOTH splits move a lot, the global null is changing something else too.
#   pred_c AND THE RANKING SURVIVES THE FIX: under the common global null, the live-margin gradient still
#          exceeds the token-margin gradient on all three roles. The honest version of "which matters
#          more". If FALSE the ranking was entirely an artifact of the null and §1911 and §1913 have been
#          compared unfairly in every board note I have written about them -- which I would say plainly.
#   pred_d CONTROLS: coverage is exactly 16,110; the within-stratum arm reproduces §1913's PUBLISHED
#          token-margin quartiles (4.22 / 5.02 / 5.29 / 5.29 on skip7000) within 0.05; and the pooled
#          covered-arm baseline is computed in-run and reported.
...
OUT = PT + 'ops/common_null_comparison_results.json'
...
TOKQS = None     # the three quartile boundaries over covered tokens
...
def compare_by_bucket(rows, hooks, seenmask=None):
    ...
    tokq = {i8: {'l': [], 'p': []} for i8 in range(4)}   # split by PRECOMPUTED token margin
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'COMMON-NULL COMPARISON: which confidence signal matters more? | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    TOKQS = torch.quantile(TOKMARG[_ct].double(),
    ...
    print(f'  precomputed per-token program margin for {int(_ct.numel())} covered types '
          f'(quartile bounds {[round(float(x), 4) for x in TOKQS]})', flush=True)
    ...
    S1913_TOK = [4.22, 5.02, 5.29, 5.29]
    S1911_GRAD = {'skip7000': 4.75, 'skip11000': 5.27, 'skip1200': 5.07}
    gtok = {e: q(e, 'tok', 3, 'global_enrichment') - q(e, 'tok', 0, 'global_enrichment') for e in roles}
    gliv = {e: q(e, 'liv', 3, 'global_enrichment') - q(e, 'liv', 0, 'global_enrichment') for e in roles}
    pa = all(q(e, 'tok', 3, 'global_enrichment') > 7.0 for e in roles)
    pb = all(abs(gliv[e] - S1911_GRAD[e]) <= 1.0 for e in roles)
    pc = all(gliv[e] > gtok[e] for e in roles)
    pd = (ncov == NCOV
          and all(abs(q('skip7000', 'tok', i8, 'local_enrichment') - S1913_TOK[i8]) <= 0.05
                  for i8 in range(4)))

    print(f'\n  quartile enrichment under BOTH nulls (global null = shuffle over the whole covered arm):',
          flush=True)
    for e in roles:
        print(f'    {e:10s} global null {top(e, "global_null"):.4f}', flush=True)
        for nm, lab in (('tok', 'TOKEN margin'), ('liv', 'LIVE  margin')):
            print(f'      {lab}  ' + '  '.join(
                f'q{i8} {q(e, nm, i8, "global_enrichment"):5.2f}x (local {q(e, nm, i8, "local_enrichment"):5.2f}x)'
                for i8 in range(4)), flush=True)
    print(f'\n  THE ARTIFACT IS REAL (token q3 > 7.0x under the global null) -> {pa}  ' + '  '.join(
        f'{e} {q(e, "tok", 3, "global_enrichment"):.2f}x vs §1913 local {S1913_TOK[3]:.2f}x'
        for e in roles), flush=True)
    print(f'  and the LIVE signal is barely affected (within 1.0x of §1911) -> {pb}  ' + '  '.join(
        f'{e} {gliv[e]:+.2f}x vs §1911 {S1911_GRAD[e]:+.2f}x' for e in roles), flush=True)
    print(f'  and the RANKING survives the fix (live > token) -> {pc}  ' + '  '.join(
        f'{e} live {gliv[e]:+.2f}x vs token {gtok[e]:+.2f}x' for e in roles), flush=True)
    print(f'  coverage {ncov}, §1913 local quartiles reproduce -> control {pd}; baseline ' + '  '.join(
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_artifact_is_real': bool(pa),
                          'pred_b_live_barely_affected': bool(pb),
                          'pred_c_ranking_survives': bool(pc),
