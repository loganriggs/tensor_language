#!/usr/bin/env python
"""late_tail_read_gain_probe -- manipulation test of the late MLPs' tail read (§2780-§2785: a core-gated LINEAR read of the bus tail). Put a
gain alpha on the cross term (tail-linear program, self term dropped) and on the whole tail input (exact MLP on c + alpha t). A calibrated
linear read predicts a quadratic CE(alpha) around alpha = 1: half gain recovers ~3/4, double gain costs like removal, unit gain is the minimum.
CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_half_gain_recovers_three_quarters pred_c_double_gain_costs_like_removal
#                     pred_d_unit_gain_is_the_minimum pred_e_self_term_does_not_rescue_over_gain
...
Preregistration: polynomial_causal/LATE_TAIL_READ_GAIN_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_tail_read_gain_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_tail_read_gain_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_TAIL_READ_GAIN_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_tail_top2_direction_probe_results.json"   # §2786
OUT = ROOT / "late_tail_read_gain_probe_results.json"
HASHES = {PREREG: "6b124e00b429cc2adcc6f755ceef17f05dc789b547d628650122a677c6595eb3", PRIOR: "a40141a961da596eb17c84c7c8656915896781bb74d51f0c34ee85b15fde81d7",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_tail_read_gain_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_frac": 0.35, "c_lo": 0.6, "c_hi": 1.6}
NULLS = {"b_frac": 0.60, "c_lo": 0.30, "c_hi": 3.0, "d_gap": 0.003, "e_frac": 0.7}
...
def main():
    ...
    def split8(k):
        ...
        return run(patch)
    class GainHead(OwnHead):
        """MLP l on xh = c + t (c = top-k of U_8 exact, rest the fit-set constant). mode 'tl': Lc*Rc + alpha*(Lc*Rt + Lt*Rc) (self term
        dropped); mode 'sc': the exact MLP on c + alpha*t (cross scales by alpha, self term by alpha^2)."""
        def __init__(self, l, k, alpha, mode):
            super().__init__(l, k, "const", U_8[:, :k]); self.alpha = alpha; self.mode = mode
        def __call__(self, w, x):
            xh = F.rms_norm(x, (D,)); c = (xh @ self.U) @ self.U.T + self.mx - (self.mx @ self.U) @ self.U.T; t = xh - c
            if self.mode == "sc":
                xp = c + self.alpha * t
                return self.mlp.Down(self.mlp.Left(xp) * self.mlp.Right(xp))
            Lc = self.mlp.Left(c); Rc = self.mlp.Right(c); Lt = self.mlp.Left(t); Rt = self.mlp.Right(t)
            return self.mlp.Down(Lc * Rc + self.alpha * (Lc * Rt + Lt * Rc))
    def gain(alpha, mode):
        return run({("mlp", l): GainHead(l, 768, alpha, mode) for l in range(SPLIT, NL)})
    arms = {"SPLIT8_1024": split8(1024), "TL_A0": gain(0.0, "tl"), "TL_A05": gain(0.5, "tl"), "TL_A1": gain(1.0, "tl"), "TL_A15": gain(1.5, "tl"), "TL_A2": gain(2.0, "tl"),
            "SC_A05": gain(0.5, "sc"), "SC_A15": gain(1.5, "sc"), "SC_A2": gain(2.0, "sc")}
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    a0 = arms["TL_A0"]
    summ = {"half_over_zero": arms["TL_A05"] / a0 if a0 else None, "double_over_zero": arms["TL_A2"] / a0 if a0 else None,
            "a15_over_a05": arms["TL_A15"] / arms["TL_A05"] if arms["TL_A05"] else None, "sc2_over_tl2": arms["SC_A2"] / arms["TL_A2"] if arms["TL_A2"] else None,
            "sc05_minus_tl05": arms["SC_A05"] - arms["TL_A05"], "tl_grid": [arms[k] for k in ("TL_A0", "TL_A05", "TL_A1", "TL_A15", "TL_A2")]}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(arms["TL_A0"] - PRIOR_LATE_MLP_768) <= BARS["repro_tol"] and abs(arms["TL_A1"] - PRIOR_DROP_TT_768) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_half_gain_recovers_three_quarters': bool(arms["TL_A05"] <= BARS["b_frac"] * a0),
        'pred_c_double_gain_costs_like_removal': bool(BARS["c_lo"] * a0 <= arms["TL_A2"] <= BARS["c_hi"] * a0),
        'pred_d_unit_gain_is_the_minimum': bool(arms["TL_A1"] <= min(arms["TL_A05"], arms["TL_A15"])),
        'pred_e_self_term_does_not_rescue_over_gain': bool(arms["SC_A2"] >= arms["TL_A2"]),
    }
    nulls = {"b_null_half_ge_.60x": bool(arms["TL_A05"] >= NULLS["b_frac"] * a0), "c_null_double_le_.30x_or_ge_3x": bool(arms["TL_A2"] <= NULLS["c_lo"] * a0 or arms["TL_A2"] >= NULLS["c_hi"] * a0),
             "d_null_over_gain_helps": bool(arms["TL_A15"] <= arms["TL_A1"] - NULLS["d_gap"]), "e_null_sc2_le_.7x_tl2": bool(arms["SC_A2"] <= NULLS["e_frac"] * arms["TL_A2"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "late MLPs 8-17 at k=768 with a gain alpha on the tail's linear (cross) read or on the tail input; everything else exact",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "tl_a0": arms["TL_A0"], "prior_late_mlp_768": PRIOR_LATE_MLP_768, "tl_a1": arms["TL_A1"], "prior_drop_tt_768": PRIOR_DROP_TT_768},
