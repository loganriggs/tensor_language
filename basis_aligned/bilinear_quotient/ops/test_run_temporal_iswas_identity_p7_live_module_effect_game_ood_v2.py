...
import run_temporal_iswas_identity_p7_live_module_effect_game_ood_v2 as target
...
def test_v2_changes_replay_authority_to_parent_writer_only():
    source = Path(target.__file__).read_text()
    assert 'parent_by[(phase, "writer")]' in source
    assert 'parent_by[(phase, "identity_full")]' not in source
    assert "full_writer_replay_max_abs_error" in source
    assert target.PRIOR.name.endswith("_v2.json")
    assert target.OUT.name.endswith("_v2_result.json")
