...
"""v599: work the AUTHORED BACKLOG instead of authoring more cells.
...
REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here). THESE ARE v595's BARS VERBATIM, UNCHANGED.
This rung is a REPLICATION: same question, same thresholds, eight fresh constructions. Nothing is re-tuned
between batches, so the batches can be POOLED -- which is the point, since one batch of eight is a thin basis for
the row2/row4 inversion and four batches of eight is not. v591 lifted eight cells from eight constructions and
...
Smoke: V599_SMOKE=<out.json> (CPU, V599_SMOKE_ROWS=4).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_crossconstruction599_v599_result.json"
...
NAMES = {"each_all_agreement": "each_all_agreement",
         "existential_article_a_some": "existential_article_a_some",
         "gerund_subject_agreement": "gerund_subject_agreement",
         "imperative_mood": "imperative_mood",
         "infinitive_subject_agreement": "infinitive_subject_agreement",
         "kinship_age_older_younger": "kinship_age_older_younger",
         "lexical_number": "lexical_number_pp",
         "measure_phrase_agreement": "measure_phrase_agreement",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_crossconstruction599_v599", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V599_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V599_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V599_SMOKE_NAMES", "each_all_agreement,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_crossconstruction599_v599", "candidate_id": "corpus.unit_tier3_batch_crossconstruction599_v599", "bars": BARS,
