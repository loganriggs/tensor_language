#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the fifty-sixth three spec-authored behaviours, plus one instrument.
"""v369: the AMENDED tier-3 battery over the FIFTY-SIXTH batch (THREE cells) -- plus one instrument.
Three candidate NEW FRAME CLASSES for one mapping pair, after v367 showed that frames come in equivalence classes.
v285 2/7, ..., v357 4/6, v361 2/4, v365 2/2 (+instrument each).
WHAT v367 ESTABLISHED. Seven sentence frames of woke -> ` up` / calmed -> ` down` were prepared together. The four
report/evidential shapes -- `It turned out ... apparently,`, `Word spread that ... evidently,` and the two
recombinations that swap clause and parenthetical -- are MUTUALLY inseparable (leaks 0.13 to 0.23, including between
the two recombinations, which share neither clause nor adverb with each other). The locative frame A, the agreement
frame B and the denial frame D each separate from everything (leaks <= 0.04). Own-arm leaks ran 1.13 to 1.96
throughout. So a mapping pair supports one circuit per frame CLASS, not per sentence, and the corpus inventory is
(frame classes x mappings) with four classes known.
WHAT THIS BATCH IS FOR. The frame lane's ceiling is now a measurable quantity -- the number of distinguishable
registers -- and the way to raise it is to find a class that is none of the four. Three candidates, deliberately
picked from different syntactic positions rather than different wordings of the same one:
    verb_particle_fj_up_down  `Last winter the {agent} {verb}, admittedly,`          temporal adjunct
    verb_particle_fk_up_down  `Because the {agent} {verb}, obviously,`               causal subordinate clause
    verb_particle_fm_up_down  `Did anyone notice the {agent} {verb}, really,`        interrogative matrix
The temporal one is the interesting case rather than the safe one: frame A is `Near the {object} the {agent} {verb}`,
a LOCATIVE adjunct in the same position, so if adjunct position is what defines that class, fj should fuse with A --
which would be a sharper description of the class than "locative" and would predict the next collision. The causal
subordinate and the interrogative matrix are structurally further from anything already tested.
CPU capability margins BEFORE enqueue (mean base / mean donor; A1; dropped rows; floor 1.0):
  verb_particle_fm_up_down  (interrogative) A1:-6.18/+6.06 drop0
  verb_particle_fj_up_down  (temporal)      A1:-4.89/+4.64 drop0
  verb_particle_fk_up_down  (causal)        A1:-3.40/+3.40 drop0
  Three of three. This mapping pair has now cleared the floor in ten frames, from 2.0 in the locative frame A to 6.18
  in the interrogative -- a threefold spread for identical cue verbs.
Protocol identical to v197/v201/.../v365. finiteness_selection is the instrument and is EXCLUDED from four-row counts.
Batch size THREE; bars are the three-cell bars (2/2/3/2/2).

REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 2 of the 3 new behaviours pass all four rows. Every frame copy of this pair has
                           passed the battery so far, in six frames.                                      prior 80%
  pred_b_row2           -> MIN held-out per-direction exact-set recovery >= 0.80 on at least 2 of 3.       prior 75%
...
Accounting: the rung that follows prepares these three alongside one representative of each known class -- A, B, D and
one of the report cluster -- and asks, per cell, whether it separates from all four. A cell that separates from all
four is a new class and +1; a cell that fuses with one of them is +0 and NAMES that class better than I can from the
armchair, which is the outcome the temporal cell is there to produce.
An error receipt per behaviour counts as a miss on every row. ~40 s GPU each, ~3 min.
Smoke: V369_SMOKE=<out.json> (CPU, V369_SMOKE_ROWS=4 per split, V369_SMOKE_NAMES=verb_particle_fj_up_down).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec9p_v369_result.json"
...
NAMES = {"verb_particle_fj_up_down": "verb_particle_fj_up_down", "verb_particle_fk_up_down": "verb_particle_fk_up_down",
         "verb_particle_fm_up_down": "verb_particle_fm_up_down",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v369", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V369_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V369_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V369_SMOKE_NAMES", "verb_particle_fj_up_down").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec9p_v369", "candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v369", "bars": BARS,
