"""TRIPLE ARMS — COMPLETING THE MOBIUS LATTICE OF THE CP FRONT (rung 119).

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows; LOWER IS BETTER.
With rung 118's six pairs, the four 3-subsets complete the interaction lattice of {cf0,cf1,cf2,cf3}: either
order-2 predicts the triples (composition calculus validated at a THIRD order it was not fit on), or the
misses ARE the 3-way Mobius coefficients K_ijk, measured directly. Either outcome parameterizes the lattice.

ARMS: the four triples {cf_i,cf_j,cf_k} installed alone, mixed Ks (2304/2304/1152/576). At RUN time the
rung-118 receipt (frontier_cp_pairs_results.json) supplies singles and J_ij; the runner serializes, so it
lands first — its absence is an INSTRUMENT FAIL, not a scoring branch.

REGISTERED PREDICTIONS (pred2(ijk) = single_i+single_j+single_k + J_ij+J_ik+J_jk):
  (a) ORDER-2 PREDICTS THE TRIPLES: median over the 4 triples of |agg(triple) - pred2(triple)| <= 0.05.
  (b) LADDER CONSISTENCY: agg(triple_123) is the largest triple.
  (c) SANITY: each triple agg >= max(pair within) - 0.015 and <= agg(quad) + 0.015.
NULL: 3-way terms are material (median miss > 0.05); the lattice needs cubic coefficients K_ijk (then this
rung has measured them). PRICE: none (attribution). Tripwire: INSTRUMENT FAIL if triple arms identical
(pstdev < 0.005) or the 118 receipt is missing. Self-reviewed."""
import json, sys, time, os
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
if os.environ.get('BQLIB_DRYRUN')=='1':
    _bq='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
    _need=['frontier_cp_prefix_results.json','frontier_m0_cp_results.json','frontier_m2_cp_results.json',
           'frontier_m13_cp_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: triple arms (Mobius lattice completion)')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_cp_triples_results.json'
...
def main():
    ...
    if SEL.get('cp_triples'):
        _ROWS=SEL.get('ext_rows',FR)
        _KS={0:2304,1:2304,2:1152,3:576}
        for _li,_K in _KS.items():
            _mlp=m.transformer.h[_li].mlp
            _L=_mlp.Left.weight.detach().float(); _R=_mlp.Right.weight.detach().float()
            _Dw=_mlp.Down.weight.detach().float(); _db=_mlp.Down_bias.detach().float()
        ...
        for _t in _it.combinations((0,1,2,3),3):
            _arms['tri_'+''.join(map(str,_t))]=evalV(_ROWS,_ROWS.shape[0],[f'cf{_x}' for _x in _t],ML).detach().cpu()
            print(f'  triple {_t} done',flush=True)
        del cur['clsmap']
        SEL['front_result']=_arms
...
if __name__=='__main__':
    ...
    NFLAT=CN.nflat()
    p118=PT+'frontier_cp_pairs_results.json'
    if not os.path.exists(p118): raise SystemExit('INSTRUMENT FAIL: rung-118 receipt missing at run time')
    R118=json.load(open(p118))
    SGL={int(k):v for k,v in R118['singles'].items()}
    J=R118['J']; QUAD=R118['aggs']['quad']; PAIRA=R118['aggs']
    SEL['mode']='norm'; SEL['K']=4608; SEL['K69']=4608; SEL['K69MAP']={}
    SEL['skipset']=tuple(range(10,18)); SEL['motif_off']=tuple(range(2,10))
    SEL['cp_triples']=True; SEL['ext_rows']=CROWS
    print('ARMS: four CP triples, mixed Ks - census rows',flush=True)
    main()
    A=SEL['front_result']
    agg={}
    for k,cev in A.items():
        if cev.shape[0]!=NFLAT: raise SystemExit(f'INSTRUMENT FAIL: {k} cev {cev.shape[0]} != {NFLAT}')
        agg[k]=round(float((cev-CBASE).mean()),4)
    import statistics as stt, itertools as it
    if stt.pstdev(agg.values())<0.005: raise SystemExit('INSTRUMENT FAIL: triple arms identical')
    rows={}
    misses=[]
    for t in it.combinations((0,1,2,3),3):
        nm='tri_'+''.join(map(str,t))
        pred2=round(sum(SGL[x] for x in t)+sum(J[f'{a}{b}'] for a,b in it.combinations(t,2)),4)
        K3=round(agg[nm]-pred2,4)
        rows[nm]={'agg':agg[nm],'pred2':pred2,'K3':K3}
        misses.append(abs(K3))
        print(f'  {nm}: agg {agg[nm]:+.4f}  pred2 {pred2:+.4f}  K3 {K3:+.4f}',flush=True)
    med=stt.median(misses)
    pa=med<=0.05
    pb=agg['tri_123']==max(agg.values())
    ok=True
    for t in it.combinations((0,1,2,3),3):
        nm='tri_'+''.join(map(str,t))
        mp=max(PAIRA[f'pair_{a}{b}'] for a,b in it.combinations(t,2))
        ok&=agg[nm]>=mp-0.015 and agg[nm]<=QUAD+0.015
    pc=ok
    res={'triples':rows,'median_abs_K3':round(med,4),'quad_118':QUAD,
         'convention':'per-position dCE = CE(config) - CE(real model) on census rows; lower is better',
         'pred_a_order2_predicts':bool(pa),'pred_b_ladder':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) median |K3| {med:.4f} <= 0.05: {'HELD' if pa else 'FAILED'}")
    print(f"(b) tri_123 largest: {'HELD' if pb else 'FAILED'}")
    print(f"(c) triple sanity: {'HELD' if pc else 'FAILED'}")
