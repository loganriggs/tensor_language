...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; two arms (norm / true-Fisher top-8 at mlp4/mlp5)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_fisher8_results.json'
...
if __name__=='__main__':
    T00=time.time()
    SEL['mode']='norm'
    print('ARM 1/2: norm selection (reproduction of the published frontier)',flush=True)
    r_norm=main()
    SEL['P8']={site:fisher_top8(site) for site in (5,6)}
    SEL['mode']='fisher8'
    print('ARM 2/2: true-Fisher top-8 selection at mlp4/mlp5',flush=True)
    r_f=main()
    pa=abs(r_norm['L2_F']-2.6735)<=0.05
    pb=(r_norm['L2_F']-r_f['L2_F'])>=0.04
    ...
    res={'norm':r_norm,'fisher8':r_f,'published_L2_F':2.6735,
         ...
         'pred_a_reproduces_published':bool(pa),'pred_b_gain_installs':bool(pb),'pred_c_no_C_harm':bool(pc),
    ...
    print(f"(b) fresh gain {res['gain_fresh']:+.4f} >= 0.04: {'HELD' if pb else 'FAILED'}")
