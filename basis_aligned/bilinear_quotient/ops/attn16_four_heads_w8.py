"""WINDOW CERTIFICATION OF THE FOUR-HEAD attn16 (rung 60).

CONVENTION (S2135): L2 is CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER.

S2153: at window grain attn16 = heads 16.0/16.3/16.4/16.5; the other five sum to -0.0007. Certify at S2152's
standard: zero heads {16.1, 16.2, 16.6, 16.7, 16.8} THROUGHOUT (fits and window evals; bases excluded).
PRICE: the four-head description drops 5/9 of attn16. NULL: S2152's precedent - a hidden carrier among the five.

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) THE ADDITIVE NUMBER HOLDS: median over the 8 windows of [L2_F_w(four) - L2_F_w(nine)] <= +0.005
      (prediction -0.0007).
...
Writes attn16_four_heads_w8_results.json. Self-reviewed. RUNG-58 HEADER FOLLOWS.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; nine-head vs FOUR-head skip-a16 arms x eight windows (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'attn16_four_heads_w8_results.json'
...
if __name__=='__main__':
    ...
    def _mkzh(hh):
        def pre(mod,args):
            ...
            return (x,)+tuple(args[1:])
        return m.transformer.h[16].attn.c_proj.register_forward_pre_hook(pre)
    ZH=[_mkzh(k2) for k2 in (1,2,6,7,8)]
    ...
    r_n=main()
    SEL['zero6']=True
    print('ARM 2/2: skip-a16, heads 16.1/2/6/7/8 zeroed throughout (four-head attn16)',flush=True)
    r_t=main()
    SEL['zh_on']=False
