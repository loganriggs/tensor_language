#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_copiers_post_noun_add pred_c_copiers_below_readers_post_noun pred_d_copiers_and_noun_values_reach_most pred_e_copiers_and_noun_values_nested
"""Copiers to readers: composition (v576). v574 / v575: 8.1 and 6.1 copy the noun's gender into the positions after it (0.125 and 0.097 of the he - she gap by
their writes into positions >= noun + 1). v567: the readers' values at those positions carry 0.363 (0.132 + 0.231). Here the two copiers' writes into every
position after the noun swapped jointly (COPIERS_POST), and jointly with the five readers' values at the noun (COPIERS_POST_AND_NOUN) -- the two sources the
readers combine: the noun's own value and the copies planted after it.
PREDICTIONS (scored as written; failures preserved; priors from v549 / v567 / v574 / v575)
    pred_a_baseline_replays                    the unedited manual forward replays the model's he - she margins within 1e-3
    pred_b_copiers_post_noun_add               COPIERS_POST closes 0.222 +- 0.06 (8.1's 0.125 + 6.1's 0.097; two heads writing into the same positions add)
    pred_c_copiers_below_readers_post_noun     COPIERS_POST <= 0.363 (the readers' values after the noun; the copies are part of what they read there)
    pred_d_copiers_and_noun_values_reach_most  COPIERS_POST_AND_NOUN closes >= 0.70
    pred_e_copiers_and_noun_values_nested      COPIERS_POST_AND_NOUN <= 0.600 + COPIERS_POST (the joint is at most the sum)
...
OUT = ROOT / "circuits/followups/gender_copiers_to_readers_v576_result.json"
CANDIDATE_ID = "gender.copiers_to_readers_v576"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, COPIERS_SUM, CBAND, READERS_POST, MOST_MIN, NOUN_V549, UNIT = 1e-3, 0.222, 0.06, 0.363, 0.70, 0.600, 701
...
FORWARDS_MAX = 7
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_copiers_post_noun_add": "0.222 +- 0.06", "pred_c_copiers_below_readers_post_noun": "<= 0.363", "pred_d_copiers_and_noun_values_reach_most": ">= 0.70", "pred_e_copiers_and_noun_values_nested": "<= 0.600 + COPIERS_POST"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "copiers_sum": COPIERS_SUM, "cband": CBAND, "readers_post": READERS_POST, "most_min": MOST_MIN, "noun_v549": NOUN_V549, "unit": UNIT}}
    ...
    MODES = {"COPIERS_POST": ({}, (0,)), "COPIERS_POST_AND_NOUN": (FIVE, (0,))}; SETS_W = {m: {8: [1], 6: [1]} for m in MODES}
    ...
    print("copiers post-noun", round(mc["COPIERS_POST"], 3), "copiers + readers' noun values", round(mc["COPIERS_POST_AND_NOUN"], 3))
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_copiers_post_noun_add": abs(mc["COPIERS_POST"] - COPIERS_SUM) <= CBAND, "pred_c_copiers_below_readers_post_noun": mc["COPIERS_POST"] <= READERS_POST,
                   "pred_d_copiers_and_noun_values_reach_most": mc["COPIERS_POST_AND_NOUN"] >= MOST_MIN, "pred_e_copiers_and_noun_values_nested": mc["COPIERS_POST_AND_NOUN"] <= NOUN_V549 + mc["COPIERS_POST"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "gender_copiers_to_readers_result_v576", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
