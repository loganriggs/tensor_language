...
"""v633: does two-control selectivity hold for possessive cells that differ by MAPPING rather than structure?
...
Smoke: V633_SMOKE=<out.json> (CPU, V633_SMOKE_ROWS=4, V633_SMOKE_NAMES=<cell>).
"""
...
OUT = ROOT / "circuits/followups/unit_possessive_mappings_twocontrol_v633_result.json"
...
NAMES = {"possessive_disjoint_my_your": "possessive_disjoint_my_your",
         ...
         "possessive_number": "possessive_number",
         "possessive_number_attractor_conflict": "possessive_number_attractor_conflict",
...
SIBLINGS = ['possessive_disjoint_my_your', 'possessive_number', 'possessive_number_attractor_conflict', 'possessive_number_his_their', 'possessive_number_its_their', 'possessive_person_our_your', 'possessive_pronoun_mine_yours', 'possessive_pronoun_number_hers_theirs', 'possessive_pronoun_person_ours_theirs']
# no prior two-control numbers exist for these ten; the anchor check is dropped and pred_b is re-registered
V633_ANCHOR = None
...
BARS = {"ext_min": EXT_MIN, "rem_min": REM_MIN, "c_ub_max": C_UB_MAX, "row5_share_band": [0.5, 1.4], "k_reach": 7, "k_both": 5, "tol": 0.05, "max_dropped": 4}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 200000, 6400000


def _plan():
    return {"candidate_id": "corpus.unit_possessive_mappings_twocontrol_v633", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V633_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V633_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V633_SMOKE_NAMES", "verb_preposition_against_for,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_possessive_mappings_twocontrol_v633", "candidate_id": "corpus.unit_possessive_mappings_twocontrol_v633", "bars": BARS,
