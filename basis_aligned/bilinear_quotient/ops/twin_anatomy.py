...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['gauge_quotient_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: deps present; twin-anatomy arms (mean2/path_only/direct_only) + captures")
    raise SystemExit(0)
import torch
...
OUT=PT+'twin_anatomy_results.json'
...
def main():
    ...
    baseF=evalT(FR,120,[])
    _tw=[]
    if SEL.get('twin_mode'):
        _tm=SEL['twin_mode']; _MU2=SEL['mu2']; _st={}
        def _h2(mo,i_,o_):
            y2=o_[0]
            _st['d2']=(y2.float()-_MU2).to(y2.dtype)
            if _tm in ('mean2','path_only'):
                return (_MU2.expand_as(y2).to(y2.dtype),o_[1])
            return None
        _tw.append(m.transformer.h[2].attn.register_forward_hook(_h2))
        if _tm=='path_only':
            def _p3(mo,args):
                return (args[0]+_st['d2'],)+tuple(args[1:])
            _tw.append(m.transformer.h[3].attn.register_forward_pre_hook(_p3))
        if _tm=='direct_only':
            def _p3b(mo,args):
                return (args[0]-_st['d2'],)+tuple(args[1:])
            _tw.append(m.transformer.h[3].attn.register_forward_pre_hook(_p3b))
        print(f'twin mode {_tm} active (bases excluded)',flush=True)
    ...
    if SEL.get('clsdmg'):
        ...
        if SEL.get('cap1416'):
            SEL['y14']=[]; SEL['y16']=[]
            _hc.append(m.transformer.h[2].attn.register_forward_hook(
                lambda mo,i_,o_: SEL['y14'].append(o_[0].detach().reshape(-1,D).float().cpu())))
            _hc.append(m.transformer.h[3].attn.register_forward_hook(
...
if __name__=='__main__':
    ...
    SEL['skipset']=(); SEL['motif_off']=(2,3); SEL['clsdmg']=True; SEL['cap1416']=True
    SEL['twin_mode']=None; SEL['mu2']=None
    print('ARM 1/4: base (capture y2, y3)',flush=True)
    r_base=main()
    clsflat=SEL['clsflat']
    Y2b=torch.cat(SEL['y14']); Y3b=torch.cat(SEL['y16'])
    MU2=Y2b.mean(0).to(DEV); MU3=Y3b.mean(0)
    SEL['mu2']=MU2
    import statistics as stt
    SEL['twin_mode']='mean2'
    print('ARM 2/4: mean2 (capture y3)',flush=True)
    r_m2=main()
    Y3m=torch.cat(SEL['y16'])
    SEL['twin_mode']='path_only'
    print('ARM 3/4: path_only',flush=True)
    r_po=main()
    SEL['twin_mode']='direct_only'
    print('ARM 4/4: direct_only',flush=True)
    r_do=main()
    if float((Y3m-Y3b).abs().sum())<1e-3:
        raise SystemExit('INSTRUMENT FAIL: y3 identical under mean2 - hooks inert')
    dm2=r_m2['L2_F']-r_base['L2_F']
    dpo=r_po['L2_F']-r_base['L2_F']
    ddo=r_do['L2_F']-r_base['L2_F']
    SW=CLS.index('subword')
    swm=(clsflat==SW).nonzero().squeeze(1)
    oth=(clsflat!=SW).nonzero().squeeze(1)[:len(swm)]
    D3=(Y3m-Y3b)
    rel=(D3[swm].norm(dim=1)/(Y3b[swm].norm(dim=1)+1e-9))
    med_rel=float(rel.median())
    L2 = (Y2b-MU2.cpu())
    comp=torch.nn.functional.cosine_similarity(D3[swm], -L2[swm], dim=1)
    med_comp=float(comp.median())
    tc_sw=float(torch.nn.functional.cosine_similarity(L2[swm], (Y3b-MU3)[swm], dim=1).median())
    tc_ot=float(torch.nn.functional.cosine_similarity(L2[oth], (Y3b-MU3)[oth], dim=1).median())
    pa=dpo<=0.5*dm2
    pb=med_rel>=0.10
    pc=abs(r_base['L2_F']-2.4410)<=0.01
    res={'base':{'L2_F':r_base['L2_F']},
         'damage':{'mean2':round(dm2,4),'path_only':round(dpo,4),'direct_only':round(ddo,4),
                   'zero2_ref_s2180':0.4837},
         'y3_response':{'median_rel_change_subword':round(med_rel,4),'median_comp_cos':round(med_comp,4)},
         'twin_direction_cos':{'subword':round(tc_sw,4),'other_positions':round(tc_ot,4)},
         'pred_a_path_carries_half':bool(pa),'pred_b_active_response':bool(pb),'pred_c_reproduces_r86_base':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'damage: mean2 {dm2:+.4f} | path_only {dpo:+.4f} | direct_only {ddo:+.4f}  (zero2 ref +0.4837)')
    print(f'y3 under mean2: median rel change {med_rel:.3f}, compensation cos {med_comp:+.3f}')
    print(f'twin direction cos: subword {tc_sw:+.3f} vs elsewhere {tc_ot:+.3f}')
    print(f"(a) path_only {dpo:+.4f} <= 0.5 x mean2 {0.5*dm2:+.4f}: {'HELD' if pa else 'FAILED'}")
    print(f"(b) rel change {med_rel:.3f} >= 0.10: {'HELD' if pb else 'FAILED'}")
    print(f"(c) base FR L2_F {r_base['L2_F']} vs 2.4410 (tol 0.01): {'HELD' if pc else 'FAILED'}")
