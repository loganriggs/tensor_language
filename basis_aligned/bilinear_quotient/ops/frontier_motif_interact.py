"""MOTIF x FRONT INTERACTION (rung 125): who ate the 0.9 census nats in the global swap?

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows; LOWER IS BETTER.
S2216: the global swap realized only 0.72 of the front's ~1.63 solo swing; the tail-attn increment stayed in
its band, so the motif heads (weights-only scalar gains fit on REAL streams, never refit) are the suspects.
Arms (middles at K=4608 = real; no tail replacements anywhere): motifs_alone (motif hooks 2-9, all else
real), front_alone (CP-3456 front + a0/a1v, no motifs - repro of S2208's +0.1193), front_motifs (both).
J_FM = agg(front_motifs) - agg(motifs_alone) - agg(front_alone).

REGISTERED PREDICTIONS:
  (a) MOTIFS ARE THE EATER: J_FM >= 0.45 (at least half of the ~0.9 eaten is front x motif interaction).
  (b) STRONG SUPERADDITIVITY: agg(front_motifs) >= agg(motifs_alone) + agg(front_alone) + 0.30.
  (c) REPRODUCTION: |agg(front_alone) - 0.1193| <= 0.015.
NULL: motifs compose additively with the CP front (J_FM < 0.1) - the eater is the tail complex instead.
PRICE: none (attribution). Tripwire: INSTRUMENT FAIL if any two arms are bitwise equal. Self-reviewed."""
import json, sys, time, os
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
if os.environ.get('BQLIB_DRYRUN')=='1':
    _bq='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
    _need=['frontier_claim_results.json','frontier_cp_front2_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: motif x front interaction')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_motif_interact_results.json'
...
def main():
    ...
    if SEL.get('motif_interact'):
        ...
        for _li in (0,1,2,3):
            ...
            _kp=(_Dw.norm(dim=0)*_L.norm(dim=1)*_R.norm(dim=1)).argsort(descending=True)[:3456]
            S[f'cf{_li}']=('cp',_li,_L[_kp].contiguous(),_R[_kp].contiguous(),
                           _Dw[:,_kp].contiguous(),_db)
        cur['clsmap']=classify2(_ROWS).to(DEV)
        _FRT=['a0','a1v','cf0','cf1','cf2','cf3']
        _arms={}
        _arms['motifs_alone']=evalV(_ROWS,_ROWS.shape[0],[],ML).detach().cpu()
        print('  motifs_alone done',flush=True)
        _arms['front_alone']=evalV(_ROWS,_ROWS.shape[0],_FRT,[]).detach().cpu()
        print('  front_alone done',flush=True)
        _arms['front_motifs']=evalV(_ROWS,_ROWS.shape[0],_FRT,ML).detach().cpu()
        print('  front_motifs done',flush=True)
        del cur['clsmap']
        for _a in ('motifs_alone','front_alone'):
            if torch.equal(_arms[_a],_arms['front_motifs']):
                raise SystemExit(f'INSTRUMENT FAIL: {_a} bitwise equal to front_motifs')
        SEL['front_result']=_arms
...
if __name__=='__main__':
    ...
    NFLAT=CN.nflat()
    A3456=json.load(open(PT+'frontier_cp_front2_results.json'))['arms']['all_3456']['agg']
    SEL['mode']='norm'; SEL['K']=4608; SEL['K69']=4608; SEL['K69MAP']={}
    SEL['skipset']=tuple(range(10,18)); SEL['motif_off']=()
    SEL['motif_interact']=True; SEL['ext_rows']=CROWS
    print('ARMS: motifs_alone / front_alone / front_motifs - census rows',flush=True)
    main()
    A=SEL['front_result']
    agg={}
    for k,cev in A.items():
        ...
        print(f"  {k:13s} agg {agg[k]:+.4f}",flush=True)
    JFM=round(agg['front_motifs']-agg['motifs_alone']-agg['front_alone'],4)
    pa=JFM>=0.45
    pb=agg['front_motifs']>=agg['motifs_alone']+agg['front_alone']+0.30
    pc=abs(agg['front_alone']-A3456)<=0.015
    res={'aggs':agg,'J_front_motifs':JFM,'anchor_all3456':A3456,
         'convention':'per-position dCE = CE(config) - CE(real model) on census rows; lower is better',
         'pred_a_motifs_eat':bool(pa),'pred_b_superadditive':bool(pb),'pred_c_repro':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'J(front,motifs) = {JFM:+.4f}')
    print(f"(a) J_FM {JFM:+.4f} >= 0.45: {'HELD' if pa else 'FAILED'}")
    print(f"(b) front_motifs {agg['front_motifs']:+.4f} >= sum + 0.30 ({agg['motifs_alone']+agg['front_alone']+0.30:+.4f}): {'HELD' if pb else 'FAILED'}")
    print(f"(c) |front_alone {agg['front_alone']:+.4f} - {A3456:+.4f}| <= 0.015: {'HELD' if pc else 'FAILED'}")
