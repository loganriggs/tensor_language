#!/usr/bin/env python3
# BQLANE: gpu
# BQGATE: 23forwards230seq; fixed-pointer typed-group response screen;0fits.
"""A exact instrument; B live target; C typed-group response; D group null."""
...
PREREG = POLY / "SUCCESSOR_FIXED_POINTER_TYPED_GROUP_RESPONSE_V1_PREREGISTRATION.md"
SINGLETON = POLY / "SUCCESSOR_FIXED_POINTER_MODULE_RESPONSE_V2_RESULT.json"
BINDING = POLY / "SUCCESSOR_FIXED_POINTER_TYPED_GROUP_RESPONSE_V1_BINDING.json"
OUT = POLY / "SUCCESSOR_FIXED_POINTER_TYPED_GROUP_RESPONSE_V1_RESULT.json"
...
GROUPS = {
    "early_attention": tuple(f"attn{x}" for x in range(0, 8)),
    "pointer_attention": tuple(f"attn{x}" for x in range(8, 13)),
    "late_attention": tuple(f"attn{x}" for x in range(13, 18)),
    "pointer_mlp": tuple(f"mlp{x}" for x in range(8, 13)),
    "late_mlp": tuple(f"mlp{x}" for x in range(13, 18)),
    "pointer_all": tuple(name for x in range(8, 13) for name in (f"attn{x}", f"mlp{x}")),
    "late_all": tuple(name for x in range(13, 18) for name in (f"attn{x}", f"mlp{x}")),
    "all_attention": tuple(f"attn{x}" for x in range(18)),
    "all_mlp": tuple(f"mlp{x}" for x in range(18)),
}
BARS = {
...
PRICE = {"forwards": 23, "sequences": 230, "fits": 0}
...
def plan():
    ...
    return {
        "schema": "successor_fixed_pointer_typed_group_response_v1_plan",
        "model_loaded": False,
        "gpu_accessed": False,
        "queue_touched": False,
        ...
        "predicates": [
            ...
            "pred_c_typed_group_response",
            "pred_d_typed_group_null",
        ],
    }
...
def main():
    ...
    terminal = "invalid" if not (pred_a and pred_b) else ("typed_group_screen" if pred_c else "typed_group_null")
    result = {
        "schema": "successor_fixed_pointer_typed_group_response_v1_result",
        "terminal": terminal,
        "predictions": {
            "pred_a_exact_instrument": pred_a,
            "pred_b_live_target_and_capability": pred_b,
            "pred_c_typed_group_response": pred_c,
            "pred_d_typed_group_null": pred_d,
        },
        "bars": BARS,
        ...
        "selected_group": next((name for name in ranking if name in eligible), None),
