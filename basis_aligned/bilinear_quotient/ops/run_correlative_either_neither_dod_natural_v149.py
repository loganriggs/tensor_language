...
"""correlative_either_neither DoD (v149): the run_correlative_either_neither_dod_battery_v147 set on natural FineWeb rows (training corpus, out-of-panel) (PREDICTS OOD). Body: `dod_natural_line.run`; rows receipt `circuits/followups/correlative_either_neither_dod_natural_rows_v149.json` (outcome-blind
miner; any-sense cue filter stated). Congruent cells ('natural_either_or', 'natural_neither_nor') are the readout test; the other cells are corpus counter-cases. Emitted by dod_line.py.
...
CANDIDATE_ID = line.CANDIDATE_ID.rsplit(".", 1)[0] + ".dod_natural_v149"
ROWS = N.ROOT / "circuits/followups/correlative_either_neither_dod_natural_rows_v149.json"
OUT = "correlative_either_neither_dod_natural_v149_result.json"
CONGRUENT = ('natural_either_or', 'natural_neither_nor')
...
def main() -> None:
    rows, sha, pos, neg = N.rows_from_receipt(ROWS, "or", " or", " nor", lambda r: f"natural_{r['cue']}_{r['label']}")
