#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the forty-ninth three spec-authored behaviours, plus one instrument.
"""v339: the AMENDED tier-3 battery over the FORTY-NINTH batch (THREE cells) -- plus one instrument.
The first batch authored under the FRAME-BOUND account: every cell reuses BOTH cue -> token mappings of an existing
counted circuit, in the other frame.
v285 2/7, ..., v327 2/4, v331 3/3, v335 2/2 (+instrument each).
WHY THIS IS LEGITIMATE AUTHORING RATHER THAN DUPLICATION. v333 measured that reusing both cue mappings WITHIN a frame
fuses (fam-arm leak 0.0921 and 0.1163 into the two donors, cell not separable). v337 measured that the same reuse
ACROSS a frame change does NOT fuse: verb_preposition_repl_frame_same, which borrows both of repl_base's mappings and
changes only the matrix clause and the parenthetical, came out separable at fam sib max 0.0191 -- a sixfold drop from
the within-frame number and well under the 0.05 cross bar. So a behaviour's identity is (frame, cue -> token mapping),
not the mapping alone, and the same verbs in a new sentence shape are new circuits. These three cells put that to work
and simultaneously re-test it: a fusion here would refute what v337 just measured.
CELLS (donor cell in the OTHER frame in brackets; within frame B every mapping here is distinct):
  verb_particle_fb_up_down       woke/calmed    [verb_particle_up_down]
  verb_particle_fb_out_down      found/settled  [verb_particle_out_down]
  verb_preposition_fb_about_for  cared/voted    [verb_preposition_about_for]
CPU capability margins BEFORE enqueue (mean base / mean donor; A1; dropped rows; floor 1.0):
  verb_particle_fb_up_down       (woke/calmed)   A1:-5.73/+5.78 drop0
  verb_particle_fb_out_down      (found/settled) A1:-3.03/+3.16 drop0
  verb_preposition_fb_about_for  (cared/voted)   A1:-2.86/+2.97 drop0
  fb_up_down at 5.73 is the strongest A1 margin measured in this corpus. ONE of four was dropped and never entered
  ops/: verb_particle_fb_away_up (shied/sidled, A1 -0.61/+0.79), whose donor cell clears 1.26 in the OTHER frame --
  so CAPABILITY IS FRAME-DEPENDENT TOO, and a verb list that works in one sentence shape has to be re-screened in the
  next. That is a cheap CPU fact worth having before authoring fifty frame-B cells.
Protocol identical to v197/v201/.../v335. finiteness_selection is the instrument and is EXCLUDED from four-row counts.
Batch size THREE; bars are the three-cell bars (2/2/3/2/2).

REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 2 of the 3 new behaviours pass all four rows. All three clear the floor by more
                           than any batch today and their donors are counted circuits.                    prior 80%
...
Accounting: passes go to the frame-B family rung. Under the frame-bound account all three should be separable from
their cross-frame donors and from each other; the rung will also carry the donors as controls where they are already
in that family, so the test is not run against a control set that omits the cells most likely to collide.
An error receipt per behaviour counts as a miss on every row. ~40 s GPU each, ~2 min.
Smoke: V339_SMOKE=<out.json> (CPU, V339_SMOKE_ROWS=4 per split, V339_SMOKE_NAMES=verb_particle_fb_up_down).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec9p_v339_result.json"
...
NAMES = {"verb_particle_fb_up_down": "verb_particle_fb_up_down", "verb_particle_fb_out_down": "verb_particle_fb_out_down",
         "verb_preposition_fb_about_for": "verb_preposition_fb_about_for",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v339", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V339_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V339_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V339_SMOKE_NAMES", "verb_particle_fb_up_down").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec9p_v339", "candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v339", "bars": BARS,
