#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the forty-eighth two spec-authored behaviours, plus one instrument.
"""v335: the AMENDED tier-3 battery over the FORTY-EIGHTH batch (TWO cells) -- plus one instrument.
The FRAME arm. Everything decided today was decided inside one sentence shape; these two cells change the shape and
keep everything else fixed, so that the next rung can ask whether a behaviour's identity includes its frame.
v285 2/7, ..., v323 3/3, v327 2/4, v331 3/3 pending (+instrument each).
THE QUESTION. v321/v325 established, in the particle class, that a behaviour's identity is its (cue word -> readout
token) mapping and not its readout pair; v331/v333 replicate that in the preposition class. Every cell in both is the
SAME frame: `Near the {object} the {agent} {verb}, of course,` with the answer after a parenthetical. So "identity" so
far means identity WITHIN a frame. These two cells use a different matrix clause and a different parenthetical:
    verb_preposition_repl_frame_same   grumbled/lobbied   `Everyone agreed the {agent} {verb}, frankly,`
    verb_preposition_repl_frame_fresh  griped/hankered    same new frame, both cue verbs fresh
frame_same reuses BOTH cue -> token mappings of verb_preposition_repl_base verbatim. If the mapping alone is the
identity, frame_same fuses with repl_base ACROSS the frame change. If the identity is the frame together with the
mapping, it is separable from it, and today's account needs the qualifier "within a frame" written into it. frame_fresh
is the control: fresh mappings in the new frame, separable under either account, and it also tells me whether the new
frame supports circuits at all -- without it, a fusion-free result could just mean the frame is weak.
WHY THE PARENTHETICAL SURVIVED THE FRAME CHANGE. My first draft removed it entirely so the answer followed the verb
directly. The candidate bank refused every row with `matched_final_input_token`: base and donor must end on the same
token, which the parenthetical is what guarantees. That is a real constraint of the instrument, not a preference, so
the frame change is the matrix clause plus a different parenthetical, and I am stating the limit rather than implying
a bigger change than I made.
CPU capability margins BEFORE enqueue (mean base / mean donor; A1 | A2; dropped rows; floor 1.0):
  verb_preposition_repl_frame_same   (grumbled/lobbied)  A1:-2.40/+2.54 drop0  A2:-3.04/+3.20 drop0
  verb_preposition_repl_frame_fresh  (griped/hankered)   A1:-1.60/+1.89 drop0  A2:-2.50/+2.82 drop0
  Two of two cleared. The new frame is at least as capable as the old one, which is itself informative: the model does
  not need the locative prefix to carry preposition selection across a parenthetical.
Protocol identical to v197/v201/.../v331. finiteness_selection is the instrument and is EXCLUDED from four-row counts.
Batch size TWO; bars are the two-cell bars (1/1/2/1/1).

REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 1 of the 2 new behaviours passes all four rows.                        prior 80%
  pred_b_row2           -> MIN held-out per-direction exact-set recovery >= 0.80 on at least 1 of 2.       prior 75%
  pred_c_row3           -> dim A1 CE damage LB975 > 0 and >= 0.10 on both.                                 prior 80%
  pred_d_row4           -> constrained rank-1 own-C CE UB975 <= 0.01 on at least 1 of 2.                    prior 75%
  pred_e_row5_amended   -> A2-own / A2 ceiling within [0.5, 1.4] with LB975 > 0 on at least 1 of 2.         prior 75%
  pred_f_instrument     -> finiteness_selection: n_units identical to v197 and extraction_held_dirB within +-0.02.
Accounting: neither cell is counted until the frame rung reports. A frame_same that FUSES with repl_base is worth more
than a counted circuit: it would mean the identity travels with the cue mapping across sentence shapes, and that the
corpus's real inventory is mappings rather than (frame, mapping) products.
An error receipt per behaviour counts as a miss on every row. ~40 s GPU each, ~2 min.
Smoke: V335_SMOKE=<out.json> (CPU, V335_SMOKE_ROWS=4 per split, V335_SMOKE_NAMES=verb_preposition_repl_frame_same).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec9p_v335_result.json"
...
NAMES = {"verb_preposition_repl_frame_same": "verb_preposition_repl_frame_same",
         "verb_preposition_repl_frame_fresh": "verb_preposition_repl_frame_fresh",
...
BARS = {"ext_min": EXT_MIN, "rem_min": REM_MIN, "c_ub_max": C_UB_MAX, "row5_share_band": [0.5, 1.4], "k_four": 1, "k_row2": 1, "k_row3": 2, "k_row4": 1, "k_row5": 1, "instr_tol": 0.02}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 200000, 6400000


def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v335", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V335_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V335_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V335_SMOKE_NAMES", "verb_preposition_repl_frame_same").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec9p_v335", "candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v335", "bars": BARS,
