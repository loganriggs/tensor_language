#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_701_has_a_number_contrast_at_the_verb pred_c_verb_readers_feed_701 pred_d_copier_adds_to_701 pred_e_all_values_at_noun_carry_701
"""Do the verb readers feed the final detector (v465)? v462: MLP 17's unit 701 acts at the answer position for a distant verb (0.069 of the margin), reading
the number carried there; v448 / v450: heads 11.3 / 7.8 (+ 13.1 / 9.7) and the copier carry number from the noun to the verb. Composition by edit: swap the
verb readers' values at the noun and measure how much of 701's own plural - singular activation contrast AT THE ANSWER POSITION follows (79 distant rows);
the copier added; every head's value at the noun as the total.
PREDICTIONS (scored as written; failures preserved; priors from v450 / v462)
    pred_a_baseline_replays                   the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_701_has_a_number_contrast_at_the_verb  701's native plural - singular activation contrast at the answer position has one sign on >= 0.85 of the rows
    pred_c_verb_readers_feed_701              the four verb readers' value swap at the noun moves >= 0.25 of 701's contrast at the answer. Prior: unsure
    pred_d_copier_adds_to_701                 adding the copier moves >= 0.10 more than the four readers alone
    pred_e_all_values_at_noun_carry_701       every head's value at the noun moves >= 0.85 of 701's contrast (the detector reads only copied number)
PRICE (registered maximum): 3 batches x 4 passes = 12 forwards; 0 backwards; 0 fits. Bar <= 13.
"""
...
OUT = ROOT / "circuits/followups/readers_feed_701_v465_result.json"
CANDIDATE_ID = "chain.readers_feed_701_v465"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, SIGN_MIN, READERS_MIN, COPIER_GAIN, ALL_MIN, UNIT = 1e-3, 0.85, 0.25, 0.10, 0.85, 701
...
FORWARDS_MAX = 13
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_701_has_a_number_contrast_at_the_verb": ">= 0.85 one sign", "pred_c_verb_readers_feed_701": ">= 0.25", "pred_d_copier_adds_to_701": ">= 0.10 more", "pred_e_all_values_at_noun_carry_701": ">= 0.85"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "sign_min": SIGN_MIN, "readers_min": READERS_MIN, "copier_gain": COPIER_GAIN, "all_min": ALL_MIN, "unit": UNIT}}
    ...
    MODES = {"V4": (V4, (0,)), "V4_C": ({**V4, 4: [5]}, (0,)), "ALL_NOUN": (ALL_H, (0,))}; forwards = 0
    def logits_margin(x):
    ...
    mv = verb["moved"]
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_701_has_a_number_contrast_at_the_verb": verb["sign_constancy"] >= SIGN_MIN, "pred_c_verb_readers_feed_701": mv["V4"] >= READERS_MIN,
                   "pred_d_copier_adds_to_701": mv["V4_C"] >= mv["V4"] + COPIER_GAIN, "pred_e_all_values_at_noun_carry_701": mv["ALL_NOUN"] >= ALL_MIN}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "readers_feed_701_result_v465", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
