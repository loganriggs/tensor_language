"""Attention lane, v706: the mixed program at rank 16 / 64 (no native heads), and MANIPULABILITY — does the program preserve each head's value?

v705: ranks 4 / 16 / 64 per head give +0.124 at 12.4M values with no native heads; the residual sits with the 128 rank-4 heads. Arm MIXED16: those
128 heads at rank 16, the six at 16, the 28 content-heavy heads at 64 (all 162 hybrid; ~21M values), fitted as before. Then the manipulability
check Logan's gate needs: under the fitted MIXED16 program, mean-ablate each of the 24 most valuable heads (by native mean-ablation value, v701)
one at a time and compare the cost inside the program with the native value — Spearman rank correlation and the median ratio. A program that
reproduces the loss but not what each head is worth is not the model. CE ADDED, lower is better; joint recovery = 1 - cost / 3.996.
PREDICTIONS (scored as written; failures preserved)
    pred_a_native_replays        native CE within 0.002 of 3.13241 (instrument)
    pred_b_no_overfit            held-out at every evaluation after step 50 <= the step-50 value + 0.003. Prior: unsure
    pred_c_mixed16_recovery      MIXED16 held-out joint recovery >= 0.98 (cost <= 0.08). Prior: unsure
    pred_d_values_preserved_rank Spearman(native value, in-program value) over the 24 heads >= 0.8. Prior: unsure
    pred_e_values_preserved_scale the median ratio in-program / native value is within [0.5, 2.0]. Prior: unsure
PRICE (registered maximum): 1 arm x 300 steps = 300 forwards + 300 BACKWARDS; kappa_r recomputation 13 x 2 = 26; validation 13 x 3 = 39; held-out
13 x 6 + 6 = 84; manipulability 24 x 6 = 144; total ~600 forwards, 300 backwards; fit parameters ~20M. Bars: forwards <= 620, backwards <= 300.
"""
...
OUT = ROOT / "circuits/followups/attention_mixed16_manip_v706_result.json"
OUT_PT = ROOT / "circuits/followups/attention_mixed16_manip_v706_programs.pt"
...
CANDIDATE_ID = "attention.mixed16_manip_v706"
FORWARDS_MAX, BACKWARDS_MAX = 620, 300
...
ARMS = ("mixed16",)
REPLAY_TOL, UPTURN_TOL, REC16, RHO_MIN, RATIO = 0.002, 0.003, 0.98, 0.8, (0.5, 2.0)
PREDICTIONS = {"pred_a_native_replays": "+-0.002", "pred_b_no_overfit": "no upturn > 0.003", "pred_c_mixed16_recovery": ">= 0.98",
               "pred_d_values_preserved_rank": "Spearman >= 0.8", "pred_e_values_preserved_scale": "median ratio in [0.5, 2]"}
...
def main() -> None:
    plan = {"candidate_id": CANDIDATE_ID, "forwards_max": FORWARDS_MAX, "model_backwards": BACKWARDS_MAX, "model_updates": 0, "fit_parameters": 134 * 4 * 16 * 1280 + 134 * 513,
            ...
            "bars": {"replay_tol": REPLAY_TOL, "upturn_tol": UPTURN_TOL, "rec16": REC16, "rho_min": RHO_MIN, "ratio": RATIO}, "arms": list(ARMS), "n_manip": N_MANIP}
    ...
    for hh in nhooks:
        hh.remove()
    pre.remove()
    disk_guard.guard_torch_save(saved, str(OUT_PT), "v706 fitted hybrids (endpoint of training)")
    M = results["mixed16"]
    predictions = {"pred_a_native_replays": abs(native - NATIVE_V615) <= REPLAY_TOL,
                   "pred_b_no_overfit": all(M["heldout_curve"][str(st)] <= M["heldout_curve"]["50"] + UPTURN_TOL for st in range(75, STEPS + 1, EVAL_EVERY)),
                   "pred_c_mixed16_recovery": M["recovery_at_chosen"] >= REC16, "pred_d_values_preserved_rank": rho >= RHO_MIN, "pred_e_values_preserved_scale": RATIO[0] <= med_ratio <= RATIO[1]}
    if forwards > FORWARDS_MAX or backwards > BACKWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} fwd / {backwards} bwd")
    OUT.write_text(json.dumps({"schema": "attention_mixed16_manip_result_v706", "candidate_id": CANDIDATE_ID, "plan": plan, "report": {"native": native, "joint_value": JOINT_VALUE, "arms": results, "manipulability": manip, "spearman": rho, "median_ratio": med_ratio, "program_endpoint_cost": base_prog - native},
