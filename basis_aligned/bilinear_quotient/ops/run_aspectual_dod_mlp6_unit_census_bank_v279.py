...
"""Aspectual has/had DoD (v279): the temporal family ONE BLOCK DOWN -- MLP 6 at unit grain at the bank. v253: MLP 8's write on 9.1's has - had reader
...
Same exact census (`dod_units.unit_census`) for MLP 6's write on the same reader direction at the three bank positions and their sum, pooled since - by.
...
PREDICTIONS (scored as written; failures preserved; priors unsure)
    ...
    pred_b_bank_top10_carry_half               summed over the three bank positions, the top-10 |pooled| units carry >= 0.50 of MLP 6's pooled write on r
...
OUT = ROOT / "circuits/followups/aspectual_dod_mlp6_unit_census_bank_v279_result.json"
CANDIDATE_ID = "aspectual_anchor.has_vs_had.dod_mlp6_unit_census_bank_v279"
LAYER, HEAD, CLOSURE_TOL, TOP10_MIN, LEAD_MIN, JACCARD_MIN = 6, (9, 1), 1e-3, 0.50, 0.15, 1 / 3
...
def main() -> None:
    ...
    OUT.write_text(json.dumps({"schema": "aspectual_dod_mlp8_unit_census_bank_result_v279", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "positions": report, "predictions": predictions, "forwards": forwards,
