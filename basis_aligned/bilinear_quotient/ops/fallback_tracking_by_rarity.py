# THE FALLBACK'S TRACKING BY THE CURRENT TOKEN'S RARITY -- resolving §1888's one loose thread.
#
# §1887 and §1888 settled the mechanism attribution: table lookups track the live model's top-1 at
# ~7.2-7.6x a permutation null, §1870's rank-64 fallback map at ~2.7-3.0x, a ratio near 2.5x that
# survives holding the target bucket fixed and does not depend on coverage.
#
# One detail ran the other way and I declined to call it noise. EVERY fallback figure at 5,419 coverage
# is slightly below its 16,110 counterpart -- 2.91 vs 3.14, 2.97 vs 3.56, 2.70 vs 3.11 -- consistent in
# sign on all three roles, all inside pred_b's 1.0x bar. The obvious explanation is compositional: at
# lower coverage the UNCOVERED set contains commoner tokens (everything outside the smaller covered set),
# so if the map serves commoner uncovered tokens worse, the average falls exactly as observed.
#
# That is a testable statement about §1870's map and nobody has made it. This splits the UNCOVERED arm by
# the CURRENT token's own fit-row frequency and measures enrichment in each band. The covered arm is
# carried unchanged as the reference.
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY. Rung 3: §1888's loose thread.
#
...
#   pred_a THE COMPOSITIONAL EXPLANATION IS RIGHT: within the uncovered arm, enrichment on the COMMONEST
#          band is below that on the RAREST band, on all three roles. This is what §1888's consistent
#          sign requires, and it is the direction I expect only because the data pointed there -- I have
#          no mechanism for it in advance. If FALSE the sign in §1888 is not compositional and is either
#          noise after all or something about coverage itself, which I would then say plainly rather
#          than leave a §1888 paragraph standing that guessed wrong.
#   pred_b AND THE SPREAD IS MATERIAL: the gap between the best and worst uncovered band is at least
#          0.5x, on all three roles. §1888's whole effect is ~0.3x, so a compositional account needs a
#          within-arm spread at least that size to explain it. If FALSE the bands are too uniform to
#          produce §1888's shift by re-weighting, and the compositional story fails on magnitude even if
#          pred_a passes on direction -- which is the more interesting way for this to go.
#   pred_c AND EVERY UNCOVERED BAND STILL BEATS CHANCE: enrichment is at least 1.5x in every band on
#          every role. §1887's pred_b established the fallback is not inert in aggregate; this asks
#          whether that holds everywhere or is carried by one band. If FALSE the map contributes nothing
#          for some class of tokens, which localises §1870's remaining deficit.
#   pred_d CONTROLS: coverage is exactly 5,419; the uncovered bands partition the uncovered arm exactly;
#          the covered arm reproduces §1888's PUBLISHED 7.19 / 7.29 / 7.64x within 0.2x, since it is the
#          same build and the same positions with one extra split.
...
OUT = PT + 'ops/fallback_tracking_by_rarity_results.json'
...
UBANDS = ((0, 0), (1, 9), (10, 99), (100, 10 ** 9))   # the UNCOVERED current token's own fit count
...
def compare_by_bucket(rows, hooks, seenmask=None):
    ...
    for i in range(0, rows.shape[0], 8):
        ...
        fc = COV['freq'][cur]              # the CURRENT token's own fit-row count
        for b in UBANDS:
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'FALLBACK TRACKING BY CURRENT-TOKEN RARITY | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    live_bands = {e: [k2 for k2 in bk if ub[e][k2]['n'] >= 100] for e in roles}
    lo, hi = bk[-1], bk[0]        # commonest band, rarest band
    spread = {e: max(ub[e][k2]['enrichment'] for k2 in live_bands[e])
                 - min(ub[e][k2]['enrichment'] for k2 in live_bands[e]) for e in roles}
    pa = all(ub[e][lo]['enrichment'] < ub[e][hi]['enrichment'] for e in roles)
    pb = all(spread[e] >= 0.5 for e in roles)
    pc = all(ub[e][k2]['enrichment'] >= 1.5 for e in roles for k2 in live_bands[e])
    ...
    print(f'\n  UNCOVERED arm split by the CURRENT token\'s own fit count:', flush=True)
    for e in roles:
        print(f'    {e:10s} ' + '  '.join(
            f'{k2} n{ub[e][k2]["n"]:5d} {ub[e][k2]["enrichment"]:5.2f}x' for k2 in bk), flush=True)
    print(f'  COMMONEST band tracks WORSE than the rarest -> {pa}  ' + '  '.join(
        f'{e} {lo} {ub[e][lo]["enrichment"]:.2f}x vs {hi} {ub[e][hi]["enrichment"]:.2f}x'
        for e in roles), flush=True)
    print(f'  and the within-arm spread is material (>=0.5x) -> {pb}  ' + '  '.join(
        f'{e} {spread[e]:.2f}x' for e in roles), flush=True)
    print(f'  and EVERY band still beats chance (>=1.5x) -> {pc}  ' + '  '.join(
        f'{e} min {min(ub[e][k2]["enrichment"] for k2 in live_bands[e]):.2f}x' for e in roles),
        flush=True)
    print(f'  coverage {ncov}, bands partition the uncovered arm, covered arm reproduces §1888 '
          f'-> control {pd}  ' + '  '.join(
