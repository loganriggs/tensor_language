"""possessive_number with a LONG but structurally SIMPLE intervener.

The locality ladder conflates two things. Distance 1 passes with nothing intervening, distance 4
passes across a prepositional phrase, distance 6 fails across a full VP with its own object --
so token distance and intervening structure covary at every point and neither is isolated.

This is distance SEVEN, longer than the failing case, with an intervener made of prepositional
phrases only:

    "The clerks at the desk near the window signed" -> " their"     7 tokens, no verb, no object

Prediction registered before the run, and the two branches are opposite so the screen cannot come
out uninformative:

  * raw token distance is the operative variable -> this FAILS, seven exceeds the four-to-six break
  * an intervening verb phrase with its own object is the blocker -> this PASSES, despite being
    the longest gap tested

A pass means the four-to-six threshold I reported is an artefact of what filled the gap.
"""
...
TASK_ID = "possessive_number.long_simple_intervener"
...
def _conjunct(noun: str, plural: bool, action: str) -> str:
    """Antecedent SEVEN tokens before the pronoun, across two stacked prepositional phrases."""
    return f"The {noun}{'s' if plural else ''} at the desk near the window {action}"


def _notes(noun: str, plural: bool, action: str) -> str:
    return f"In the notes the {noun}{'s' if plural else ''} at the desk near the window {action}"
