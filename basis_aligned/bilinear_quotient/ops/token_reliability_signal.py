# IS PROGRAM CONFIDENCE A TOKEN PROPERTY? -- turning §1912's gradient into a build-time object.
#
# §1911 and §1912 found two near-independent modulators of the compiled program's agreement with the live
# model, each roughly doubling it: the MODEL's top-2 logit margin (5.36 -> 10.11x) and the PROGRAM's own
# (5.77 -> 9.84x within the model's top quartile), correlated with each other at only +0.11.
#
# The program's half of that should not be a per-position statistic at all. §1765 establishes the fully
# compiled program is a pure function of the CURRENT TOKEN -- verified exactly at §1899 (streams to
# 1.2e-07) and §1901 (site outputs to 3.2e-07), but never at the LOGITS, which is where a margin lives.
# If it holds there, the program's margin is a property of the token and can be precomputed from a single
# length-1 forward pass per covered type: 16,110 passes, once, at build time.
#
# That would convert §1912's diagnostic into a deployable per-token RELIABILITY SIGNAL -- which tokens the
# compiled program can speak for -- rather than something measurable only after the fact.
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY. Rung 3: §1912's open question.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
# Covered arm throughout, named per LESSON 71. Predicates written against THIS docstring; §1911's
# inherited block encoded the opposite direction and §1912's dropped three keys, both caught by reading.
#   pred_a §1765 HOLDS AT THE LOGITS: for covered current tokens, the maximum spread of the program's
#          top-2 margin across positions sharing a token is below 1e-3, MEASURED as twice the max
#          absolute deviation from that token's length-1 value (which bounds the spread), on all
#          three roles.
#          §1899 and §1901 verified the premise at the stream and the site output; the logits are one
#          lm_head away and nobody has checked. If FALSE the program's output is not a pure function of
#          the current token after all, which would be the largest single correction available to this
#          arc and would reach back to §1765 itself.
#   pred_b AND IT IS PRECOMPUTABLE: the per-token margin from a LENGTH-1 forward pass matches the margin
#          measured in context, maximum absolute difference below 1e-3 on all three roles. pred_a says
#          the quantity is constant per token; this says the constant is the one a length-1 pass gives,
#          which is what makes it a build-time object rather than merely a well-defined one.
#   pred_c AND THE SIGNAL IS USEFUL: splitting covered positions by the PRECOMPUTED per-token margin into
#          quartiles, the top quartile's agreement enrichment exceeds the bottom's by at least 1.0x on
#          all three roles -- i.e. the build-time signal recovers §1912's effect rather than merely being
#          well-defined. If FALSE the quantity is precomputable and uninformative, which is worth knowing
#          before anyone ships it as a reliability score.
#   pred_d CONTROLS: coverage is exactly 16,110; every covered scored position has a precomputed token
#          margin (zero lookup misses); and the covered-arm baseline is computed in-run and reported as
#          an enrichment.
...
OUT = PT + 'ops/token_reliability_signal_results.json'
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'TOKEN RELIABILITY SIGNAL: is program confidence a token property? | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    lowq, hiq = BK[0], BK[-1]
    gap = {e: (cl2(e, hiq, 'enrichment') or 0) - (cl2(e, lowq, 'enrichment') or 0) for e in roles}
    pa = all(2.0 * dev[e] < 1e-3 for e in roles)
    pb = all(dev[e] < 1e-3 for e in roles)
    pc = all(gap[e] >= 1.0 for e in roles)
    pd = (ncov == NCOV and all(miss[e] == 0 for e in roles))

    print(f'\n  agreement by PRECOMPUTED per-token program margin (q0 = least reliable):', flush=True)
    for e in roles:
        print(f'    {e:10s} ' + '  '.join(
            f'q{k4[1]} n{cl2(e, k4, "n"):6d} {(cl2(e, k4, "enrichment") or 0):5.2f}x'
            for k4 in BK), flush=True)
    print(f'\n  §1765 HOLDS AT THE LOGITS (spread < 1e-3) -> {pa}  ' + '  '.join(
        f'{e} <= {2.0 * dev[e]:.2e}' for e in roles), flush=True)
    print(f'  and it is PRECOMPUTABLE (length-1 match < 1e-3) -> {pb}  ' + '  '.join(
        f'{e} max dev {dev[e]:.2e}' for e in roles), flush=True)
    print(f'  and the SIGNAL is useful (q3 - q0 >= 1.0x) -> {pc}  ' + '  '.join(
        f'{e} {(cl2(e, lowq, "enrichment") or 0):.2f} -> {(cl2(e, hiq, "enrichment") or 0):.2f} '
        f'= {gap[e]:+.2f}x' for e in roles), flush=True)
    print(f'  coverage {ncov}, zero lookup misses -> control {pd}; baseline ' + '  '.join(
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_s1765_holds_at_logits': bool(pa),
                          'pred_b_precomputable': bool(pb),
                          'pred_c_signal_is_useful': bool(pc),
