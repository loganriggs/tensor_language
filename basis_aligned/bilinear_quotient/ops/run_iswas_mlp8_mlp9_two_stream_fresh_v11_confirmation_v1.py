...
PRIOR = ROOT / "circuits/prior_art/iswas_mlp8_mlp9_two_stream_fresh_v11_confirmation_v1.json"
...
SELECTION = ROOT / "circuits/followups/iswas_mlp8_mlp9_bilinear_correction_program_v2_result.json"
SELECTION_RUNNER = ROOT / "ops/run_iswas_mlp8_mlp9_bilinear_correction_program_v2.py"
...
OUT = ROOT / "circuits/followups/iswas_mlp8_mlp9_two_stream_fresh_v11_confirmation_v1_result.json"
CANDIDATE_ID = "cross_task.iswas_mlp8_mlp9_two_stream_fresh_v11_confirmation_v1"
FACTORS = ("left_change", "right_change", "bilinear_interaction")
EXPECTED = {"prior": "c7d24ef8b7c5407f8203eefbd35dfd7a0e03c2bdd7b41a5cbf681d32bf792553", "direct": "ee569e6cbf16a84dfcb615fc08c6c633d92a4854a867f054dc117af92bf41090",
    ...
    "selection": "1e68c77196ba65d7b4dc368d1e4a07945f5ed854254e1e3ba098adf97ef57eee",
    "selection_runner": "245687f87cfead5be677dcb8f6038e40e01d591b621c0f691772a2950412b4f3"}
...
def main():
    paths = {"prior": PRIOR, "direct": DIRECT, "direct_runner": DIRECT_RUNNER, "atlas": ATLAS,
        ...
        "selection": SELECTION, "selection_runner": SELECTION_RUNNER}
    if {name: sha(path) for name, path in paths.items()} != EXPECTED: raise RuntimeError("MLP9 factor authority changed")
    prior, direct, atlas, greedy, capability, subspace, selection = [json.loads(path.read_text())
        for path in (PRIOR, DIRECT, ATLAS, GREEDY, CAPABILITY, weight.SUBSPACE, SELECTION)]
    ...
    if (prior.get("candidate_id") != CANDIDATE_ID or direct.get("terminal") != "null"
            or atlas.get("terminal") != "screen" or greedy.get("terminal") != "screen"
            or selection.get("terminal") != "screen"
            or len(rows) != 30):
    ...
    pred_b = replay_logit<=1e-4 and replay_state<=2e-4 and cosine(target_effect,exact_effect)>=.999
