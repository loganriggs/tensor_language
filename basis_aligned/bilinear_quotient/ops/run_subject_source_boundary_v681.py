...
"""Source-reuse exact projected-state boundary resets at fixed v665 width8.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v681_result.json'
PREDICTIONS=dict(pred_a_exact_replay='baseline and independent joint versus shared chain replay max absolute error <=1e-4',
 pred_b_effect_prediction='full-state readout<=1e-4; start reset matches candidate and end reset matches oracle<=1e-10',
 pred_c_capable_position_transfer='some interior boundary reset restores A/B target<=10% and modal<=5% in every cell')


def main():
    plan=dict(prefix_calls=8,suffix_calls=56,sequences_per_arm=48,fits=0,backwards=0,additional_reduced_suffix_executions=84,
    ...
    boundary_gaps=[];oracle_checks=[];reset_checks=[]
    ...
    for reports in reset_reports.values():
    ...
    b=a and max(oracle_checks)<=1e-4 and max(reset_checks)<=1e-10
