#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_a_block_5_head_carries pred_c_block_5_singles_add pred_d_5_3_writes_person pred_e_at_most_two_heads_matter
"""Block 5's writers into the answer, person line (v592). v588: block 5 writes 0.059 of the myself - yourself gap straight into the answer, the second block
outside the named readers after block 9 (0.114: 9.6 / 9.7). Here block 5's nine heads singly (writes into the answer). 5.3 is the verb line's second-hop copier.
PREDICTIONS (scored as written; failures preserved; priors from v588)
    pred_a_baseline_replays        the unedited manual forward replays the model's myself - yourself margins within 1e-3
    pred_b_a_block_5_head_carries  the largest single closes >= 0.03
    pred_c_block_5_singles_add     the nine singles sum to within 0.03 of 0.059
    pred_d_5_3_writes_person       5.3 closes >= 0.02 (the verb line's relay copies person too). Prior: unsure
    pred_e_at_most_two_heads_matter  at most two heads close >= 0.02
PRICE (registered maximum): 2 batches x 10 passes = 20 forwards (43 pairs); 0 backwards; 0 fits. Bar <= 21.
"""
...
OUT = ROOT / "circuits/followups/person_block5_writers_v592_result.json"
CANDIDATE_ID = "person.block5_writers_v592"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, TOP_MIN, B5_V588, ADD, H53_MIN, MATTER, UNIT = 1e-3, 0.03, 0.059, 0.03, 0.02, 0.02, 701
FIVE_W = {8: [1], 13: [1], 10: [5], 15: [1]}; REST_W = {l: [h for h in range(9) if h not in FIVE_W[l]] for l in FIVE_W}   # the four person readers (name kept from the lineage)
import random
...
FORWARDS_MAX = 21
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_a_block_5_head_carries": ">= 0.03", "pred_c_block_5_singles_add": "0.059 +- 0.03", "pred_d_5_3_writes_person": ">= 0.02", "pred_e_at_most_two_heads_matter": "<= 2 heads >= 0.02"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "top_min": TOP_MIN, "b5_v588": B5_V588, "add": ADD, "h53_min": H53_MIN, "matter": MATTER, "unit": UNIT}}
    ...
    EX = {8: [1], 13: [1], 10: [5], 15: [1]}
    def grp(lo, hi): return {l: [h for h in range(N_HEAD) if h not in EX.get(l, [])] for l in range(lo, hi + 1)}
    SETS_W = {f"5.{h}": {5: [h]} for h in range(N_HEAD)}; MODES = {m: ({}, (0,)) for m in SETS_W}; forwards = 0
    def logits_margin(x):
    ...
    top = max(mc, key=lambda k: abs(mc[k])); print("block-5 heads into the answer", {k: round(v, 3) for k, v in mc.items()}, "sum", round(sum(mc.values()), 3), "top", top)
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_a_block_5_head_carries": abs(mc[top]) >= TOP_MIN, "pred_c_block_5_singles_add": abs(sum(mc.values()) - B5_V588) <= ADD,
                   "pred_d_5_3_writes_person": mc["5.3"] >= H53_MIN, "pred_e_at_most_two_heads_matter": sum(1 for v in mc.values() if v >= MATTER) <= 2}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "person_block5_writers_result_v592", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
