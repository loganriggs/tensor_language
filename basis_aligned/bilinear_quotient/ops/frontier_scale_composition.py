"""DO THE ADOPTED SCALINGS COMPOSE? THREE BLOCKS, ONE RUN (Claude, LANE 1)

Two frontier improvements are now adopted and a third is measured but not:

    TAIL   SS2896  link maps LW x 0.25   -0.2287   (adopted; six reproductions, max deviation .0003)
    CP     SS2902  CP units  Dk x 0.50   -0.1075   (adopted; anchor to SS2883 at deviation .0001)
    FRONT  SS2895  residual  A  x 0.50   -0.1648   (measured, NOT adopted -- its tb anchor failed by 4.95 nats)
...
record**: SS2888 found front x motif **super**additive (+0.3023), SS2892 found the motif layers **sub**additive (all three pairs
negative), SS2897 found a **one-way** compensation of 4.9479 vs 0.0215, and SS2901 found the tail layers coupled in the optimal AMOUNT
rather than the sign.

All eight subsets of {T, C, F} differ only in what `evalM` sees, so they share ONE fitted stack: **one pipeline run** gives every single,
every pair and the triple -- the complete interaction structure over the three blocks, which is the Moebius decomposition the 10:28Z
mathematical review ranked third, delivered constructively.

pred_b and pred_c re-anchor the two ADOPTED singles to their own sections, so the composition is measured against numbers already on the
ledger rather than against fresh ones.

SIGN CONVENTION (SS2135): frontier L2 is CE ADDED ABOVE THE REAL MODEL, so **LOWER IS BETTER** (SS312: +2.6735 beating +2.84/+2.93).
A cost is L2(arm) - L2(baseline), **POSITIVE = WORSE**, so a NEGATIVE cost is an improvement; NONADDITIVITY is triple - (sum of
singles), **POSITIVE = subadditive**. SS2128/SS2129/SS2133/SS2134 RETRACTED; SS2125 STANDS -- nothing here selects, reorders or
re-derives, it rescales already-fitted objects.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_the_tail_arm_reproduces_S2896
#                     pred_c_the_cp_arm_reproduces_S2902
#                     pred_d_the_combination_beats_the_best_single
#                     pred_e_the_three_blocks_are_not_additive

Preregistration: polynomial_causal/FRONTIER_SCALE_COMPOSITION_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run: all eight subsets of the three block scalings")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_scale_composition_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
def main():
    ...
    def _cfg(spec):
        ...
        return [x for x in order2 if x.endswith('L') or not (x[0] in _d)], ML
    _L2C = {}
    for _n, _s in _arms:
        _oo, _mm = _cfg(_s)
...
PREREG = PT + '../polynomial_causal/FRONTIER_SCALE_COMPOSITION_PREREGISTRATION.md'
PREREG_SHA = "4b1a26f37b52d3a76a0ed1514f93689e23df3e228045b1a04c6758a0095caa58"
...
S2896 = PT + 'frontier_tail_anchor_resolution_results.json'
S2902 = PT + 'frontier_cp_unit_scale_results.json'
S2895 = PT + 'frontier_front_table_shrinkage_results.json'
S2896_SHA = "5aef0a10442ce19305652c324d08b2d76200b0219d97738ea0323c9d11dab3d3"
S2902_SHA = "80c1d7934eee7c6abf329c7491a07022892367742b090b918099f5e38e28f976"
S2895_SHA = "b852f7f26bf55f65e6bebdceb0ed49bf3bd1f4021b93181d797620bc9f43fc14"
T_S, C_S, F_S = 0.25, 0.5, 0.5      # the adopted / best-known scale for each block
BARS = {"reproduce": 0.05, "anchor": 0.01, "combo_beats": 0.05, "nonadditive": 0.05}
NULLS = {"anchor_ge": 0.03, "combo_le": 0.0, "additive_le": 0.02}
...
if __name__=='__main__':
    ...
    for p_, h_ in ((S2896, S2896_SHA), (S2902, S2902_SHA), (S2895, S2895_SHA)):
        if _sha(p_) != h_:
            raise RuntimeError(f'frozen hash mismatch: {p_}')
    a96 = json.load(open(S2896))['summary']['cost_s025']                    # -0.2287  tail LW x .25
    a02 = json.load(open(S2902))['summary']['cost_fresh_by_scale']['0.5']   # -0.1075  CP Dk x .5
    a95 = json.load(open(S2895))['summary']['cost_fresh']['A050']           # -0.1648  front A x .5
    SEL['mode']='norm'

    def sp(t=False, c=False, f=False):
        d = {}
        if t: d['tail_scale'] = T_S
        if c: d['cp_scale'] = C_S
        if f: d['a_scale'] = F_S
        return d
    ARMS[:] = [('baseline', {}),
               ('T', sp(t=True)), ('C', sp(c=True)), ('F', sp(f=True)),
               ('TC', sp(t=True, c=True)), ('TF', sp(t=True, f=True)), ('CF', sp(c=True, f=True)),
               ('TCF', sp(t=True, c=True, f=True))]
    ...
    cF = {k: round(out[k]['L2_F'] - bF, 4) for k, _ in ARMS if k != 'baseline'}
    cC = {k: round(out[k]['L2_C'] - bC, 4) for k, _ in ARMS if k != 'baseline'}
    singles_sum = round(cF['T'] + cF['C'] + cF['F'], 4)
    triple = cF['TCF']
    nonadd = round(triple - singles_sum, 4)          # POSITIVE = subadditive (less than the sum)
    pair_inter = {p: round(cF[p] - (cF[p[0]] + cF[p[1]]), 4) for p in ('TC', 'TF', 'CF')}
    best_single = min(('T', 'C', 'F'), key=lambda k: cF[k])
    combo_beats = round(cF[best_single] - triple, 4)  # POSITIVE = the triple is better
    dev_T, dev_C, dev_F = abs(cF['T'] - a96), abs(cF['C'] - a02), abs(cF['F'] - a95)

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = dev_T <= BARS['anchor']
    pc = dev_C <= BARS['anchor']
    pd = combo_beats >= BARS['combo_beats']
    pe = abs(nonadd) >= BARS['nonadditive']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_tail_arm_reproduces_S2896':bool(pb),
           'pred_c_the_cp_arm_reproduces_S2902':bool(pc),
           'pred_d_the_combination_beats_the_best_single':bool(pd),
           'pred_e_the_three_blocks_are_not_additive':bool(pe)}
    nulls={'b_null_the_tail_anchor_fails':bool(dev_T>=NULLS['anchor_ge']),
           'c_null_the_cp_anchor_fails':bool(dev_C>=NULLS['anchor_ge']),
           'd_null_the_combination_does_not_help':bool(combo_beats<=NULLS['combo_le']),
           'e_null_the_blocks_are_additive':bool(abs(nonadd)<=NULLS['additive_le'])}
    res={'rung':'frontier_scale_composition','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,'scales':{'tail':T_S,'cp':C_S,'front':F_S},
         'arms':out,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(bF,4),'L2_C_baseline':round(bC,4),
                    'cost_fresh':cF,'cost_fit':cC,
                    'sum_of_singles':singles_sum,'cost_triple':triple,
                    'nonadditivity_nats':nonadd,'pairwise_interactions':pair_inter,
                    'best_single':best_single,'combination_beats_best_single_by':combo_beats,
                    'S2896_tail':round(a96,4),'tail_deviation':round(dev_T,4),
                    'S2902_cp':round(a02,4),'cp_deviation':round(dev_C,4),
                    'S2895_front':round(a95,4),'front_deviation':round(dev_F,4),
                    'implied_L2_F_triple':round(bF + triple, 4),
                    'adoption_gate_all_of_a_b_c':bool(pa and pb and pc)},
         ...
         'hashes':{PREREG:PREREG_SHA,S2896:S2896_SHA,S2902:S2902_SHA,S2895:S2895_SHA},
         'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {bF:.4f} vs 2.6735: {'HELD' if pa else 'FAILED'}")
    print("    fresh: " + "  ".join(f"{k}:{cF[k]:+.4f}" for k,_ in ARMS if k!='baseline'))
    print("    fit:   " + "  ".join(f"{k}:{cC[k]:+.4f}" for k,_ in ARMS if k!='baseline'))
    print(f"(b) T {cF['T']:+.4f} vs SS2896 {a96:+.4f}, dev {dev_T:.4f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) C {cF['C']:+.4f} vs SS2902 {a02:+.4f}, dev {dev_C:.4f}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) triple {triple:+.4f} beats best single {best_single} by {combo_beats:+.4f}: {'HELD' if pd else 'FAILED'}")
    print(f"(e) singles sum {singles_sum:+.4f}, nonadditivity {nonadd:+.4f}: {'HELD' if pe else 'FAILED'}")
    print(f"    implied L2_F(triple) = {bF+triple:.4f}   pairwise {pair_inter}")
