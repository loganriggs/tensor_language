#!/usr/bin/env python
"""attn67_handoff_probe -- is the out-of-frame part of attn6/attn7's write (the whole §2766 chain cost) a HAND-OFF into the bus?
SPLIT8_1024 reads + the two writes' next-frame remainder deleted / projected onto the bus frame U_8 / whole write onto U_8 /
routed to the readout. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_handoff_is_bus_bound pred_c_delete_reproduces_the_map
#                     pred_d_bus_frame_alone_carries_the_writes pred_e_readout_worse_than_delete
...
Preregistration: polynomial_causal/ATTN67_HANDOFF_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/attn67_handoff_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("attn67_handoff_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "ATTN67_HANDOFF_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "early_chain_write_cost_map_probe_results.json"   # §2766
OUT = ROOT / "attn67_handoff_probe_results.json"
HASHES = {PREREG: "406e651549c5b650c592fb253d82e349a215776479135bf06e1795205b353757", PRIOR: "a0c72b4e76437dc64721cfeceffd2676383fb867d710a165852371a6f6905324",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "attn67_handoff_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_max": 0.003, "c_lo": 0.010, "c_hi": 0.025, "d_max": 0.005}
NULLS = {"b_min": 0.010, "c_max": 0.005, "d_min": 0.015, "e_gap": 0.003}
...
def main():
    ...
    class RouteBus:
        """Split site s's centred write into its part inside U_next and the remainder; 'bus' keeps the remainder's projection onto
        U_bus (drops the rest); 'busonly' projects the whole centred write onto U_bus."""
        def __init__(self, s_, k, U_next, U_bus, mode):
            self.Un = U_next[:, :k]; self.Ub = U_bus[:, :k]; self.mu = wheads[s_]["mu"]; self.mode = mode
        def hook(self, idx):
            pass
        def __call__(self, w, x):
            c = w - self.mu
            if self.mode == "busonly":
                return self.mu + (c @ self.Ub) @ self.Ub.T
            inside = (c @ self.Un) @ self.Un.T; r = c - inside
            return self.mu + inside + (r @ self.Ub) @ self.Ub.T
    S67 = [("attn", 6), ("attn", 7)]
    def program(k, mode=None):
        patch = {s_: head(s_, k) for s_ in OWN8}; buf = Buf()
        for s_ in (S67 if mode else ()):
            Un = heads[("mlp", s_[1])]["U"]
            w_ = Route(s_, k, Un, mode, buf) if mode in ("delete", "readout") else RouteBus(s_, k, Un, U_8, mode)
            patch[s_] = Both(head(s_, k), w_)
        for s_ in SET8:
            patch[s_] = head(s_, k, U_8)
        if mode == "readout":
            patch[FINAL] = FinalAdd(buf, +1.0)
        return run(patch, (buf,))
    arms = {"SPLIT8_1024": program(K)}
    arms["DEL67"] = program(K, "delete"); arms["BUS67"] = program(K, "bus"); arms["BUSONLY67"] = program(K, "busonly"); arms["RO67"] = program(K, "readout")
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    cost = {kk: arms[kk] - arms["SPLIT8_1024"] for kk in ("DEL67", "BUS67", "BUSONLY67", "RO67")}
    ...
    preds = {
        ...
        'pred_b_handoff_is_bus_bound': bool(cost["BUS67"] <= BARS["b_max"]),
        'pred_c_delete_reproduces_the_map': bool(BARS["c_lo"] <= cost["DEL67"] <= BARS["c_hi"]),
        'pred_d_bus_frame_alone_carries_the_writes': bool(cost["BUSONLY67"] <= BARS["d_max"]),
        'pred_e_readout_worse_than_delete': bool(arms["RO67"] >= arms["DEL67"]),
    }
    nulls = {"b_null_ge_.010": bool(cost["BUS67"] >= NULLS["b_min"]), "c_null_le_.005": bool(cost["DEL67"] <= NULLS["c_max"]),
             "d_null_ge_.015": bool(cost["BUSONLY67"] >= NULLS["d_min"]), "e_null_ro_le_del_minus_.003": bool(arms["RO67"] <= arms["DEL67"] - NULLS["e_gap"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "SPLIT8_1024 reads + attn6/attn7 writes: remainder outside U_mlp_l deleted / projected onto U_8 / whole write onto U_8 / routed to readout",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024},
           "ce_added": arms, "cost_over_split8": cost, "prior_per_site": {"attn6": 0.0094, "attn7": 0.0070},
    ...
    if smoke:
        print(json.dumps({k: out[k] for k in ("preds", "nulls", "instrument", "ce_added", "cost_over_split8", "price")}, indent=1)); return
    from receipt import dump
    dump(out, OUT)
    print(json.dumps({"preds": preds, "nulls": nulls, "ce_added": arms, "cost_over_split8": cost}, indent=1))
