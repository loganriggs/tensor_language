"""IS THE CP EXCESS CONCENTRATED TOO? THE UNIT-SPLIT ANALOGUE OF SS2919 (Claude, LANE 1)

SS2919 found the TAIL excess is not spread evenly: scaling only the top 64 of 1152 singular directions reads -0.2614 against uniform
x0.25's -0.2287, and shrinking the rest of the spectrum actively HURTS (+0.0489). SS2918 found the CP scalar is invariant to how many
units survive, and SS2920 found the tail scalar is invariant to the regulariser -- so both corrections are properties of their objects
rather than of the procedures that made them. **The open question is whether they share a SHAPE.**

This is SS2919's experiment on the CP side. The CP units carry a natural importance ranking -- the very one `select_units` uses,
`||Dw[:,u]|| * ||L[u]|| * ||R[u]||` -- so "top r units" plays the role "top r singular directions" played for the tail. Scale one end of
that ranking by the ADOPTED 0.5, leave the other at 1.0, and see whether a subset beats the whole.

  concentrated (pred_d holds)  the CP correction is also a projection, on a small set of units, and the two
                               adopted scalars share a structure across completely different provenance --
                               ridge solutions on one side, the model's own untouched weights on the other.
  spread (pred_d fails)        the two corrections look alike as scalars but are NOT the same kind of object,
                               and the tail's low-rank structure is specific to fitted maps.

**The identity control is registered up front this time.** SS2919's uniform control went through a plain multiply rather than through
the path its split arms used, and that omission cost it its result. Here `cp_identity` scales every unit by 1.0 THROUGH THE SPLIT PATH
and must read 0.0000, and `cp_uniform` scales every unit by 0.5 through it and must agree with the plain multiply AND with SS2902.
...
SS2125 STANDS -- this rescales already-selected units; it does not reselect or reorder.
...
#                     pred_c_both_uniform_routes_reproduce_S2902
#                     pred_d_the_cp_excess_is_concentrated_like_the_tail
#                     pred_e_the_best_split_is_interior_to_the_rank_grid

Preregistration: polynomial_causal/FRONTIER_CP_UNIT_SPLIT_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run: the CP unit-split analogue of SS2919, with its identity control registered up front")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_cp_unit_split_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_CP_UNIT_SPLIT_PREREGISTRATION.md'
PREREG_SHA = "34205734f96caab2f0d2d900f31d2e9f8e3f290ee9d87149f502f9e63fb19ab0"
...
R_GRID = [64, 128, 256, 576, 1152]      # units of the retained 2304; 128/2304 mirrors SS2919's 64/1152
SPLIT_SCALE = 0.5                       # SS2902's ADOPTED scalar, applied to one end of the ranking
...
if __name__=='__main__':
    ...
    CP_ALONE = json.load(open(S2904))['summary']['cost_fresh']['C']   # SS2902's adopted CP Dk x0.5, -0.1074
    SEL['mode']='norm'

    ARMS[:] = ([('baseline', {}),
                ('cp_identity', {'cp_spec': (0, 'all', 1.0)}),
                ('cp_uniform',  {'cp_spec': (0, 'all', 0.5)}),
                ('plain_half',  {'cp_scale': 0.5})] +
               [(f'{h}{r}', {'cp_spec': (r, h, SPLIT_SCALE)}) for r in R_GRID for h in ('top','bot')])
    ...
    ident   = cost['cp_identity']
    uni     = cost['cp_uniform']
    plain   = cost['plain_half']
    ...
    tops = [cost[f'top{r}'] for r in R_GRID]
    interior = (best_split.startswith('top')
                and int(best_split[3:]) not in (R_GRID[0], R_GRID[-1]))

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = abs(ident) <= BARS['identity']
    pc = abs(uni - CP_ALONE) <= BARS['anchor'] and abs(uni - plain) <= BARS['anchor']
    ...
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           ...
           'pred_c_both_uniform_routes_reproduce_S2902':bool(pc),
           'pred_d_the_cp_excess_is_concentrated_like_the_tail':bool(pd),
    ...
    nulls={'b_null_the_split_path_is_not_faithful':bool(abs(ident) > 0.02),
           ...
           'd_null_the_cp_excess_is_spread_across_units':bool(not pd),
           'e_null_the_rank_grid_is_too_narrow':bool(not interior)}
    res={'rung':'frontier_cp_unit_split','preds':preds,'nulls':nulls,'bars':BARS,
         ...
         'summary':{'L2_F_baseline':round(bF,4),'L2_F2_baseline':round(b2,4),
                    ...
                    'S2902_cp_alone':CP_ALONE,'anchor_deviation':round(abs(uni-CP_ALONE),4),
                    ...
                    'S2919_tail_top64':-0.2614,'S2919_tail_uniform':-0.2287},
    ...
    print(f"(b) split path at scale 1.0 {ident:+.4f}, |.| <= 0.005: {'HELD' if pb else 'FAILED'}")
    print(f"(c) uniform via split {uni:+.4f} / plain multiply {plain:+.4f} vs SS2902 {CP_ALONE:+.4f}: {'HELD' if pc else 'FAILED'}")
    for r in R_GRID:
        print(f"    r={r}: top {cost[f'top{r}']:+.4f} | bot {cost[f'bot{r}']:+.4f}")
    print(f"(d) CP excess concentrated: best {best_split} {best_cost:+.4f} vs uniform {uni:+.4f}: {'HELD' if pd else 'FAILED'}")
    print(f"(e) best split interior to {R_GRID}: {'HELD' if pe else 'FAILED'}")
    print(f"    tail comparison (SS2919): top64 -0.2614 vs uniform -0.2287, a gain of 0.0327")
