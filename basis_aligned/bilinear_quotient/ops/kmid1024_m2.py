"""CODEBOOK SCALING (rung 229): km-1024 at m2 - how does the static plateau price with cells?

CONVENTION (S2135): per-position dCE = CE(m2 replaced, all else real) - CE(real model); LOWER IS BETTER.
S2326: km-256 -> 0.1406 vs oracle 0.0353. A 4x codebook prices the static-selection scaling law at depth.
ARMS: KM256-key arm = km-1024 cells (name kept for instrument reuse); KMHIER = blend (descriptive).
REGISTERED PREDICTIONS (anchors: km-256 0.1406, oracle 0.0353):
  (a) STILL IMPROVING: census|dCE|(KM1024) <= 0.125.
  (b) BUT PLATEAUED SHORT OF ORACLE: KM1024 >= 0.08.
  (c) CONSISTENCY: ORACLE <= KM1024 <= STATIC; non-inert; inert tripwire vs the km-256 receipt.
NULL: saturated already (KM1024 >= 0.135) - cells stop paying at 256; depth selection is irreducibly
dynamic. PRICE: 1024x1152 centroids + table + 2 census passes. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: km-1024 codebook scaling at m2'); raise SystemExit(0)
import torch
import torch.nn.functional as F
from bilin18_joint_removal import m, FW, DEV
D=1152
PT='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
OUT=PT+'kmid1024_m2_results.json'
MOD=2; NC=1024; K=1152
...
def main():
    ...
    pa=res['KM256']['aggabs']<=0.125
    pb=res['KM256']['aggabs']>=0.08
    if abs(res['KM256']['aggabs']-0.1406)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: codebook change inert')
    ...
    print(f"(a) KM1024 {res['KM256']['aggabs']:.4f} <= 0.125: {'HELD' if pa else 'FAILED'}")
    print(f"(b) plateau floor - KM1024 {res['KM256']['aggabs']:.4f} >= 0.08: {'HELD' if pb else 'FAILED'}")
