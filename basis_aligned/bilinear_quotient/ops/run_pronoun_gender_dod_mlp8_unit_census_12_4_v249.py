...
"""Pronoun gender he/she DoD (v249): what head 12.4 READS at the noun, at MLP-8 unit grain -- the open item since this morning (v207: 12.4 lost nothing when head 4.5 was
zeroed at the verb on the number line; v216: on gender 12.4 reads the noun 40%, the verb 23%). v164 censused MLP 8's write on 9.6's he - she reader direction at the noun
(3152 + 3943 = 76%). Same exact census with 12.4's weight-only reader direction r = V_{12.4}^T O_{12.4}^T (u_he - u_she): the same two detectors, or different units?
PREDICTIONS (scored as written; failures preserved; priors unsure)
    ...
    pred_b_detectors_lead_for_12_4    3152 and 3943 are both among the top-3 |pooled| units on 12.4's direction
    pred_c_top_10_units_carry_half    top-10 units carry >= 0.50 of the pooled contrast
    pred_d_top50_overlaps_9_6_top50   Jaccard(12.4 top-50, v164's 9.6 top-50) >= 1/3
PRICE (registered maximum): 2 batches x 1 forward = 2 forwards; 0 backwards; 0 fits. Bar <= 5.
"""
...
OUT = ROOT / "circuits/followups/pronoun_gender_dod_mlp8_unit_census_12_4_v249_result.json"
CANDIDATE_ID = "pronoun_gender.he_vs_she.dod_mlp8_unit_census_12_4_v249"
LAYER = 8
CLOSURE_TOL, TOP50_MIN, TOP200_MIN, JACCARD_MIN = 1e-3, 0.50, 0.80, 1 / 3
FORWARDS_MAX = 4
PREDICTIONS = {"pred_a_unit_closure": "<= 1e-3", "pred_b_detectors_lead_for_12_4": "3152, 3943 in top-3", "pred_c_top_10_units_carry_half": ">= 0.50", "pred_d_top50_overlaps_9_6_top50": "Jaccard >= 1/3"}
...
def main() -> None:
    ...
    predictions = {"pred_a_unit_closure": closure <= CLOSURE_TOL, "pred_b_detectors_lead_for_12_4": {3152, 3943} <= top3, "pred_c_top_10_units_carry_half": share(10) >= TOP50_MIN, "pred_d_top50_overlaps_9_6_top50": jac_noun >= JACCARD_MIN}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_mlp8_unit_census_12_4_result_v249", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "contrast_total": contrast,
