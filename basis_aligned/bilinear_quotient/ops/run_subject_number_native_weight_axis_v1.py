#!/usr/bin/env python3
# BQLANE: gpu
# BQGATE: 9forwards2144seq; subject-number native L11H3 weight axis;0updates.
"""A instrument; B native axis reproduces law axis; C native effect; D strata; E price."""
...
PREREG = POLY / "SUBJECT_NUMBER_NATIVE_WEIGHT_AXIS_V1_PREREGISTRATION.md"
...
NATIVE_AXIS = POLY / "SUBJECT_NUMBER_NATIVE_WEIGHT_AXIS_V1_ARTIFACT.json"
BINDING = POLY / "SUBJECT_NUMBER_NATIVE_WEIGHT_AXIS_V1_BINDING.json"
OUT = ROOT / "circuits/fast_screens/subject_number_native_weight_axis_v1_result.json"
METHODS = ("base", "exact", "law", "native_axis")
SUBSETS = factor_gate.BACKGROUND_SUBSETS
PATCH_CHUNK_ROWS = 256
MAXIMUM_ERROR = 5e-5
BARS = {
    "minimum_law_cosine": .90, "maximum_law_relative_l2_error": .50, "minimum_law_sign_agreement": .90,
    "minimum_native_cosine": .65, "maximum_native_relative_l2_error": .90, "minimum_native_sign_agreement": .70,
    "minimum_intermediate_cosine": .60, "maximum_intermediate_relative_l2_error": 1.0, "minimum_intermediate_sign_agreement": .65,
    "minimum_template_cosine": .50, "maximum_template_relative_l2_error": 1.1, "minimum_template_sign_agreement": .60,
}
PREDICTION_REGISTRY = {
    "pred_a_exact_native_axis_and_license": None,
    "pred_b_native_axis_reproduces_law_axis": None,
    "pred_c_native_axis_substitutes_native_effect": None,
    "pred_d_fresh_intermediate_and_templates": None,
    "pred_e_native_weight_provenance_and_price": None,
}
...
def load_bound():
    ...
    paths = {"preregistration": PREREG, "rank1_artifact": RANK1, "law_artifact": LAW, "native_axis_artifact": NATIVE_AXIS,
    ...
    native_axis = json.loads(NATIVE_AXIS.read_text())
    if native_axis["terminal"] != "native_weight_axis_frozen" or native_axis["causal_outcomes_read"] or native_axis["behavior_fit"]: raise ValueError("native axis artifact invalid")
    return binding, rank, law, native_axis


def compile_plan():
    binding, rank, law, native_axis = load_bound()
    ...
    return {"schema": "subject_number_native_weight_axis_v1_plan", "model_loaded": False, "gpu_accessed": False,
            ...
            "law_beta": law["beta"], "native_axis_prior_cosine": native_axis["prior_axis_cosine"],
...
def evaluate(model, torch, F, facade, rank, law, native_axis):
    ...
    weight_axis = torch.tensor(native_axis["axis"], dtype=torch.float32, device=device)
    native_axis_vectors = {key: float(value) * weight_axis for key, value in law["predicted_coefficients"].items()}
    heads = {}
    with torch.no_grad():
        for subset in SUBSETS:
            ...
            for index, row in enumerate(rows):
                ...
                heads[(index, subset, "native_axis")] = base[index] + native_axis_vectors[key]
    ...
    for index, row in enumerate(rows):
        for subset in SUBSETS:
            ...
            evidence.append({"row_id": row["row_id"], "direction": row["direction_id"], "template": row["template_id"], "background": subset, "cardinality": len(subset),
                             ...
                             "native_axis_q": margins[(index, subset, "native_axis")] - base})
...
def score(evidence, exactness, plan):
    weight_law = stats(evidence, "law_q", "native_axis_q"); native = stats(evidence, "native_exact_q", "native_axis_q")
    intermediate = stats([x for x in evidence if x["background"] not in {"", "EAUW"}], "native_exact_q", "native_axis_q")
    templates = {t: stats([x for x in evidence if x["template"] == t], "native_exact_q", "native_axis_q") for t in ("behind_beside", "among_behind")}
    instrument = len(evidence) == 512 and all(v <= MAXIMUM_ERROR for v in exactness.values())
    pred_b = passes(weight_law, BARS["minimum_law_cosine"], BARS["maximum_law_relative_l2_error"], BARS["minimum_law_sign_agreement"])
    ...
    pred_e = plan["native_axis_prior_cosine"] >= .94 and derive_price() == plan["price"]
    ...
    return {**exactness, "native_axis_vs_law_axis": weight_law, "native_axis_vs_native": native, "intermediate_native_axis_vs_native": intermediate,
            "template_native_axis_vs_native": templates, "predictions": predictions, "evidence": evidence}
...
def main():
    ...
    _, rank, law, native_axis = load_bound(); torch, F, facade = tangent.parent.factors._dependencies(); model, checkpoint = facade.load_bilin18(device="cuda", dtype=torch.float32, verify_weights_sha256=True)
    evidence, exactness = evaluate(model, torch, F, facade, rank, law, native_axis); scored = score(evidence, exactness, plan)
    terminal = "native_weight_axis_held" if all(scored["predictions"].values()) else ("invalid" if not scored["predictions"][PRED_KEYS[0]] else "native_weight_axis_null")
    payload = managed.atomic_create_json(OUT, {"schema": "subject_number_native_weight_axis_v1_result", "terminal": terminal,
