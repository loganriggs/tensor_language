"""ATTENTION-SIDE KNOCKOUT TRANSFER (a16, rung 268): the tail-attn replication.

CONVENTION (S2135): per-position dCE = CE(mixed config with a16 mean-ablated) - CE(real model); LOWER
IS BETTER. All four MLP knockouts transferred (S2364-S2367). a16 is an ATTENTION component (battery
own-mean 0.390) whose module is REPLACED by the compiled path (tail permanent hooks) - the ablation hook
...
REGISTERED PREDICTIONS:
  (a) OWN-FAMILY TRANSFERS: median member/ref ratio over a16-top circuits in [0.7, 1.6].
  (b) SMALL-REGIME COLLATERAL: Spearman in [0.45, 0.85).
  (c) SANITY: ablated census in [0.08, 0.8].
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: attn knockout transfer a16')
    raise SystemExit(0)
import torch
...
OUT=PT+'ko_transfer_a16_results.json'
...
def main():
    ...
    if SEL.get('clsdmg'):
        ...
        SEL['abl_on']=True
        m.transformer.h[16].attn.register_forward_hook(SEL['_ablh'])
