# IS THE MAP'S RANK THE BINDING CONSTRAINT ON THE FALLBACK?  -- sweeping map rank at full table rank.
#
# §1868 measured the fallback's own loss at **+0.82155 / +0.87058 / +0.87692** nats per uncovered
# position, against the model's length-1 ceiling for the 5,672 token types that occur there -- a ceiling
# that is EASIER to reach at uncovered positions than at covered ones (irreducible deficit 2.442 against
# 2.713). At 10.0% of scored positions that loss is worth ~0.082 nats all-position, 1.5x §1861's entire
# iso-cost gain, and it costs no extra storage to attack because the map occupies its 5.308M regardless.
#
# The map is §1785's rank-64 embedding->row least squares, and §1814 established rank(Ws) <= table_rank
# + 1, so at FULL table rank the map could carry up to D = 1152 and 64 may simply be too few. Its cost
# is 36 * map_rank * 2 * D: 5.308M at 64, 10.617M at 128, 21.234M at 256, 42.467M at 512.
#
# This sweeps map rank at full table rank and 16,110 covered types, holding everything else fixed.
#
# Docstring and predicate block written in ONE edit (LESSONS 63); the ladder holds map ranks, all
# numeric, and nothing indexes a full-rank key (LESSONS 64 and §1867's fifth tail failure).
#
# ROLES. All three. DISCOVERY ONLY. Rung 3: the open question §1868 ends on.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
# Units per LESSONS 49: the fallback loss is a DEFICIT on the uncovered population against the uncovered
# ceiling, never a CE compared across populations.
#   pred_a RANK IS THE BINDING CONSTRAINT: raising the map from rank 64 to rank 512 recovers at least
#          0.20 of the PUBLISHED 0.82155 / 0.87058 / 0.87692 fallback loss, on all three roles. If TRUE
#          the largest free lever in the class is opened by a parameter already in the build, and the
#          only question left is what to pay for it. If FALSE the map's rank is NOT what limits it --
#          the least-squares fit from embeddings simply cannot reach those rows -- and the fallback needs
#          a different functional form, not more capacity.
#   pred_b AND THE COVERED ARM IS UNTOUCHED: covered CE is identical across every map rank to within
#          1e-6 on all three roles. The map fills uncovered rows only, so this is a KNOWN-ANSWER check
#          (LESSON 34/46) on the whole sweep: if it moves, the map is reaching rows it should not and
#          nothing else in the run can be trusted.
#   pred_c AND THE RECOVERY IS MONOTONE IN MAP RANK across 64, 128, 256, 512 on every role. If FALSE the
#          map-rank axis is not a ladder and no interpolated recommendation is safe -- the same bar
#          §1849 and §1854 applied to table rank.
#   pred_d CONTROLS: map rank 64 reproduces §1868's PUBLISHED fallback loss within 0.005; coverage is
#          16,110; the covered arm attains its ceiling exactly (a tenth confirmation); every uncovered
...
OUT = PT + 'ops/map_rank_sweep_fallback_results.json'
...
MAPRANKS = (64, 128, 256, 512)          # None = full rank; §1786/§1813 use this ladder
...
LADDER = [str(m) for m in MAPRANKS]
...
# §1868's PUBLISHED fallback loss at map rank 64, full table rank, 16,110 covered types
S1868_FBLOSS_M64 = {'skip7000': 0.82155, 'skip11000': 0.87058, 'skip1200': 0.87692}
...
def main():
    ...
    print(f'MAP RANK SWEEP FALLBACK | 16110 types, full table rank, map 64/128/256/512 | '
    ...
    mapcost = {l2: 36 * int(l2) * 2 * D / 1e6 for l2 in LADDER}
    rec = {e: fbloss['64'][e] - fbloss['512'][e] for e in roles2}
    covspread = max(abs(covce[l2][e] - covce['64'][e]) for l2 in LADDER for e in roles2)
    attain = all(abs(covce['64'][e] - ceilv[e]) < 0.001 for e in roles2)
    pa = all(rec[e] >= 0.20 for e in roles2)
    pb = covspread <= 1e-6
    pc = all(fbloss[LADDER[i]][e] <= fbloss[LADDER[i - 1]][e] + 1e-12
             for e in roles2 for i in range(1, len(LADDER)))
    pd = (all(abs(fbloss['64'][e] - S1868_FBLOSS_M64[e]) <= 0.005 for e in roles2)
          and NFULL == 16110 and attain)

    print(f'\n  FALLBACK LOSS by MAP RANK at {NFULL} covered types, full table rank:', flush=True)
    for l2 in LADDER:
        print(f'    map {l2:5s} {mapcost[l2]:7.3f}M  ' + '  '.join(
            f'{e} {fbloss[l2][e]:+.5f}' for e in roles2), flush=True)
    print(f'\n  map RANK is the binding constraint (64->512 recovers >=0.20) -> {pa}  ' + '  '.join(
        f'{e} {rec[e]:+.5f}' for e in roles2), flush=True)
    print(f'  and the COVERED arm is untouched (<1e-6) -> {pb}  max spread {covspread:.2e}',
          flush=True)
    print(f'  and the recovery is MONOTONE in map rank -> {pc}', flush=True)
    print(f'  map 64 reproduces §1868 fallback loss, attainment {attain} -> control {pd}', flush=True)

    json.dump({'run': 'map_rank_sweep_fallback', 'n_covered': NFULL,
               'map_ranks': list(MAPRANKS), 'map_cost_M': mapcost,
               'fallback_loss': fbloss, 'covered_ce': covce, 'recovery_64_to_512': rec,
               'covered_spread': covspread, 'ceiling_uncovered': uceilv,
               'predictions': {'pred_a_rank_is_binding': bool(pa),
                               'pred_b_covered_untouched': bool(pb),
                               'pred_c_monotone_in_map_rank': bool(pc),
