"""THE NEXT TAIL OCTAVE (rung 249): corner + tail r112 - does the harvest continue toward majority?

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. Tail octaves from the corner: r64 ->
9 certs (0.0608), r96 -> 11 (0.0553). The tau-curve at the new point shows 43/62 under 1x - many margins
near the line. Half of the remaining tail gap (r112, 90.7M patterns).
REGISTERED PREDICTIONS (arms: THIS vs corner+tail96 0.0553/11):
  (a) census <= 0.050.
  (b) HARVEST CONTINUES: valid >= 12.
  (c) SANITY: census in [0.01, 0.12]; inert tripwires vs corner+tail96 AND corner+b16 receipts.
NULL: r96 was the last paying octave (valid <= 11). PRICE: 90.7M pattern values. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: corner + tail r112')
    raise SystemExit(0)
import torch
...
OUT=PT+'corner_tail112_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=112; SEL['qk_rmap']={li:128 for li in range(2,10)}; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    pb=nv>=12
    pc=0.01<=agg<=0.12
    if abs(agg-0.0553)<1e-4 or abs(agg-0.0606)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: tail112 map inert')
    ...
    print(f"(a) census {agg:+.4f} <= 0.050: {'HELD' if pa else 'FAILED'}")
