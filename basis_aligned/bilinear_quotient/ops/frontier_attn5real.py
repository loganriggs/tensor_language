"""BLOCK-5 ATTENTION REAL (rung 149): pricing the attn5 cliff inside the modern frontier.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. attn5's write is the program's standing
"price cliff" (S63-era lore: its motif replacement was the worst of the 38). Deployable-shaped arm: the
rung-127 build with block-5 attention REAL (motif_off={5}; tail dicts refit under it; everything else
unchanged). PRICE: attn5's real weights ~ 4 x 1152^2 = 5.3M values.

REGISTERED PREDICTIONS (anchors: census +1.9474; subword +2.319):
  (a) THE CLIFF IS REAL HERE TOO: census <= 1.80 (>= 0.15 recovery from one block's attention).
  (b) SUBWORD MOVES: dCE(subword) <= 2.05 (S2232: subword is partly motif-caused).
  (c) SANITY: increment in [0.25, 0.45] AND census >= 1.4.
NULL: attn5's motif is adequate in this config (recovery < 0.05) - the cliff was a table-era phenomenon.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: block-5 attention real')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_attn5real_results.json'
...
if __name__=='__main__':
    ...
    SEL['skipset']=(); SEL['motif_off']=(5,); SEL['clsdmg']=True; SEL['ext_rows']=CROWS
    SEL['cp_swap']=3456; SEL['tail_traj']=True
    print('ARM: rung-127 build with block-5 attention REAL',flush=True)
    main()
    ...
    print(f"(a) census {agg:+.4f} <= 1.80: {'HELD' if pa else 'FAILED'}")
    print(f"(b) subword {_sw:+.4f} <= 2.05: {'HELD' if pb else 'FAILED'}")
