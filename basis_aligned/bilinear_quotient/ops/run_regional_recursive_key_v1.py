#!/usr/bin/env python3
# BQGATE:10bodyforwards48rows;8prefixesx3supports;2zeroanchors;5suffixarms;180sec;no fit.
...
from recursive_key_prefix_v1 import generate
STEM='REGIONAL_RECURSIVE_KEY_V1'
@torch.no_grad()
def main():
    ...
    for i in range(0,len(rows),2):
        cue=[j for j,(a,b) in enumerate(zip(rows[i]['ids'],rows[i+1]['ids'])) if a!=b];assert len(cue)==1
        for row in rows[i:i+2]:prefixes.add(tuple(row['ids'][:cue[0]+1]))
    assert len(prefixes)==8
    support_states,generator_price=generate(model,sorted(prefixes),supports)
    records=torch.load(P/'REGIONAL_KEY_PREFIX_V3_ARTIFACT.pt',weights_only=True,map_location='cpu');input_comparison=[]
    for rec in records:
        ...
        for name,support in supports.items():
            ...
            conditional=F.rms_norm(raw.float(),(1152,)).cuda()
            actual=support_states[(prefix,rec['layer'],name)]
            input_comparison.append(dict(prefix=list(prefix),layer=rec['layer'],arm=name,relative_difference=float((actual-conditional).norm()/conditional.norm())))
    producer_errors=[]
    ...
    result.update(native_branch_replay_error=replay,producer_replay_errors=producer_errors,native_attention_replay_errors=replays,cells=cells,body_forwards=count,frozen_supports=supports,generator_price=generator_price,input_comparison=input_comparison,seconds=time.perf_counter()-tic,artifact_sha=digest(art),source_shas=binding,scope='Recursive selected-update key generator uses tokens and fixed zero-background only; no native actual-prefix port cache in candidate. Native queries and downstream context remain external. Geographic48 validation of stronger key-boundary extraction; no standalone full circuit claim.')
