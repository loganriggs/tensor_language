...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_asm_fresh8_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; three arms x eight windows; conditioned selection extended to c6-c9")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_asm_alllayers_results.json'
...
if __name__=='__main__':
    ...
    print('ARM 1/3: norm selection + assembly-conditioned Fisher at sites 5-10',flush=True)
    r_norm=main()
    SEL['collect_asm']=False
    SEL['mode']='fisher8'
    print('ARM 2/3: conditioned selection at mlp4/mlp5 (S2129 reproduction)',flush=True)
    r_f=main()
    SEL['mode']='fisher8all'
    print('ARM 3/3: conditioned selection at mlp4-mlp9',flush=True)
    r_a=main()
    import statistics as stt
    g45=[round(a-b,4) for a,b in zip(r_f['fresh8'],r_norm['fresh8'])]
    gall=[round(a-b,4) for a,b in zip(r_a['fresh8'],r_f['fresh8'])]
    pa=stt.median(gall)>=0.02
    pb=sum(g>=0 for g in gall)>=6
    pc=abs(stt.median(g45)-0.0481)<=0.015
    res={'norm':r_norm,'asm_mlp45':r_f,'asm_all':r_a,
         'per_window_gain_mlp45_minus_norm':g45,'per_window_gain_all_minus_mlp45':gall,
         'pred_a_all_layers_add':bool(pa),'pred_b_windows_nonneg':bool(pb),'pred_c_reproduces_mlp45':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'gain mlp45-norm per window: {g45}')
    print(f'gain all-mlp45 per window: {gall}')
    print(f"(a) all-layers median add {stt.median(gall):+.4f} >= 0.02: {'HELD' if pa else 'FAILED'}")
    print(f"(b) non-negative on {sum(g>=0 for g in gall)}/8 (bar 6): {'HELD' if pb else 'FAILED'}")
    print(f"(c) mlp45 median {stt.median(g45):+.4f} vs 0.0481 (tol 0.015): {'HELD' if pc else 'FAILED'}")
