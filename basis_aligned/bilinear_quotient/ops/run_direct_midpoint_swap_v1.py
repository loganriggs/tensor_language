...
def main():
 ...
 model=Bilin18TorchBackend.load('cuda').model.float();b16=model.transformer.h[16];b17=model.transformer.h[17];_,ru=torch.linalg.qr(model.lm_head.weight.double(),mode='reduced');package=torch.load(P/PROGRAM_FILE,weights_only=True);e=package.get('program',package);e={k:v.cuda().double() for k,v in e.items()};count=len(e['offset']);writer=torch.linalg.solve(ru,e['reduced_writers']);L=b17.mlp.Left.weight.double();R=b17.mlp.Right.weight.double();blocks=package.get('teacher_blocks');blocks=None if blocks is None else blocks.cuda().double();teacher_mean=package.get('true_block_means',e['offset']).cuda().double();C=e['scalar_readers'].T@ru@b17.mlp.Down.weight.double() if blocks is None else None;records=[];checks=[];coverage=[];donors=torch.load(P/f'{DONOR_PREFIX}_V1.pt',weights_only=True);meta=json.loads((P/f'{DONOR_PREFIX}_V1.json').read_text())
 ...
 for domain in ['fineweb','code']:
  ...
  for family in FAMILIES:
   mapping=donors[domain][family] if family!='removal' else torch.arange(len(flat));valid=mapping>=0
   if family=='removal':valid &= (torch.arange(len(flat))%256)>=16
   else:assert torch.all(doc[valid]!=doc[mapping[valid]])
   ...
   for document in range(len(tokens)):
    ...
    for mode in [*range(count),'joint']:
     sl=list(range(count)) if mode=='joint' else [mode];effects=[];r=dict(domain=domain,family=family,document=document,mode=str(mode),sites=len(ids))
     for label,amp in [('native',a),('predicted',b)]:
      difference=-amp[ids][:,sl] if family=='removal' else amp[dst][:,sl]-amp[ids][:,sl];delta=(difference@writer[:,sl].T).float();z=logits(state+delta);effects.append((z-native).double());r[label+'_ce_added']=float((F.cross_entropy(z,targets[ids],reduction='none')-basece).mean())
 ...
 for domain in ['fineweb','code']:
  ...
  for family in FAMILIES:
   ...
   for mode in [*[str(i) for i in range(count)],'joint']:
 ...
 pred=dict(pred_a_instrument=max(checks)==0 and min(coverage)>=.2,pred_b_features=all(summary[d]['same_token'][m]['centered_effect_relative_error']<.3 and summary[d]['same_token'][m]['centered_effect_cosine']>.95 for d in summary for m in [str(i) for i in range(count)]),pred_c_joint=all(summary[d]['same_token']['joint']['centered_effect_relative_error']<.2 for d in summary))
