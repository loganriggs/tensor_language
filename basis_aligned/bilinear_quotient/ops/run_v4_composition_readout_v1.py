...
"""V4_COMPOSITION_READOUT_V1_PREREGISTRATION.md; unchanged oracle source edits."""
import os,sys,json,time,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';PLAN=dict(prefix=12,native_suffix=16)
def main():
 ...
 out=A/'v4_composition_readout_v1_result.json';assert not out.exists();choicepath=P/'V4_FINITE_REFERENCE_CHOICES_V1.json';choices=json.loads(choicepath.read_text());prior=json.loads((A/'v4_finite_reference_v1_result.json').read_text());contexts=[];composition=json.loads((A/'v4_selective_composition_v1_result.json').read_text());field_errors=[];programs=[]
 ...
 for context in contexts:
  ...
  caps=[{} for _ in range(3)];base=native(0,0,capture=caps[0]);ys=native(1,0,capture=caps[1]);ya=native(0,1,capture=caps[2]);es=base-ys;ea=base-ya
  ...
  for family in dict.fromkeys(families):
   ...
   interaction=jj-predicted[ids];sn,an=ss[:,0].norm(),aa[:,0].norm();weak=torch.minimum(sn,an).clamp_min(1e-30);err=interaction.norm(dim=0)/weak;den=jj[:,0].norm().clamp_min(1e-30);ret=float(jj[:,0]@rr[:,0]/rr[:,0].square().sum().clamp_min(1e-30));leak=float(jj[:,1:].norm(dim=0).max()/den)
   records.append(dict(panel=context['panel'],family=family,weak_errors=err.tolist(),roundoff_relative=float(numerical[ids].norm(dim=0).max()/weak),subject_increment_errors=(interaction.norm(dim=0)/sn.clamp_min(1e-30)).tolist(),attractor_increment_errors=(interaction.norm(dim=0)/an.clamp_min(1e-30)).tolist(),subject_norm=float(sn),attractor_norm=float(an),joint_norm=float(den),reference_norm=float(rr[:,0].norm()),retention=ret,control_ratio=leak,subject=ss.tolist(),attractor=aa.tolist(),joint=jj.tolist(),reference=rr.tolist(),interaction=interaction.tolist()))
 instrument=counts==PLAN and max(replays)<=1e-8 and max(field_errors)<=1e-4 and max(r['roundoff_relative'] for r in records)<=.001
 result=dict(programs=programs,max_field_error=max(field_errors),choices_sha256=hashlib.sha256(choicepath.read_bytes()).hexdigest(),plan=PLAN,counts=counts,max_replay=max(replays),records=records,predictions=dict(pred_a_instrument=instrument,pred_b_weak_composition=instrument and all(r['weak_errors'][0]<=.1 and max(r['weak_errors'][1:])<=.05 for r in records),pred_c_joint_selectivity=instrument and all(r['retention']>=.8 and r['control_ratio']<=.1 for r in records)),seconds=time.perf_counter()-tic)
