# SECOND CLASS: does §1853's domination survive a different covered set?  -- fit slice [192:288].
#
# §1853's load-bearing claim is that **rank-256 at 16,110 covered types (164.478M reals, all-position CE
# 5.98851 / 5.94106 / 5.95781) DOMINATES full rank at 5,419 types (230.087M, 6.01167 / 5.98477 /
# 6.00165)** -- cheaper and better. §1855 confirmed it on all three eval roles, but all three roles read
# the SAME two builds: that is replication across eval sets, not across builds.
#
# The covered set is the thing not replicated. §1834's 5,419 types are the FIRST 96 rows of
# `fineweb_n480_skip80` (verified byte-identical to `fineweb_n96_skip80`), and the stream holds four more
# disjoint 96-row slices covering 5,932 / 6,142 / 5,914 / 5,709 types. If the domination is a property of
# the rank-versus-coverage trade rather than of that particular 5,419, it must survive each of them.
#
# This is slice [192:288], 6142 covered types. FULL RANK only: it is the dominated candidate, and the
# ladder beneath it is §1849's object and already measured.
#
# Docstring and predicate block rewritten in ONE edit (LESSONS 63); the cost is COMPUTED from §1754's
# model rather than typed (§1858's defect).
#
# ROLES. All three. DISCOVERY ONLY. Rung 2 of the autonomy order: second-class confirmation.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
# Every anchor names the build it came from, per LESSONS 53's addendum:
#   pred_a THE DOMINATION SURVIVES THIS COVERED SET: this slice's full-rank build costs more than
#          §1852's PUBLISHED 164.478M and has a HIGHER all-position CE than its PUBLISHED 5.98851 /
#          5.94106 / 5.95781, on all three roles. If TRUE the domination is a property of the trade and
#          not of §1834's particular types. If FALSE it depends on which 96 rows were fitted, §1853's
#          headline needs restating as a claim about one covered set, and the registry must say so.
#   pred_b AND THE COVERED SET IS COMPARABLE: coverage is within 15% of §1834's PUBLISHED 5,419, so this
#          is a like-for-like replacement rather than a different point on the coverage axis. If FALSE
#          the slice is not a replication of that cell and pred_a cannot be read as one.
#   pred_c AND FULL-RANK ATTAINMENT HOLDS AGAIN: covered CE equals the model's own per-token ceiling to
#          under 0.001 nats on all three roles -- an independent repeat of §1848/§1851/§1853/§1858 on a
#          covered set none of them used. If FALSE attainment depends on the fit rows, contradicting
#          §1765's derivation, and that would outrank everything else here.
#   pred_d CONTROLS: the ceiling is finite and above live on every role; coverage is recorded; the
#          full-rank cost is computed from §1754's model and reported alongside §1852's 164.478M.
...
OUT = PT + 'ops/second_class_dom_192_288_results.json'
...
# §1852's PUBLISHED rank-256 point at 16,110 covered types -- the dominating build
...
FITLO, FITHI = 192, 288      # a 96-row slice DISJOINT from §1834's first 96   # 5x the fit rows -> a larger covered set
H = m.transformer.h
NCOV = 5419
...
def main():
    ...
    assert 0.85 * NCOV <= NFULL <= 1.15 * NCOV, f'coverage {NFULL} off {NCOV}'
    ...
    print(f'SECOND CLASS DOM | fit slice [192:288], full rank only | '
    ...
    fullcost = (36 * (NFULL * D + D) + 36 * MAP_RANK * 2 * D) / 1e6
    worse = {e: curve['full'][e]['allpos'] - S1852_R256_ALLPOS[e] for e in roles2}
    pa = fullcost > S1852_R256_COST_M and all(v > 0 for v in worse.values())
    pb = 0.85 * NCOV <= NFULL <= 1.15 * NCOV
    pc = attain
    pd = all(ceilv[e] > livev[e] for e in roles2)

    print(f'\n  slice [{FITLO}:{FITHI}] -- {NFULL} covered types, full-rank cost {fullcost:.3f}M '
          f'(§1852 r256@16110: {S1852_R256_COST_M}M)', flush=True)
    for e in roles2:
        print(f'    {e:10s} ceiling {ceilv[e]:.5f}  covered {curve["full"][e]["cov"]:.5f}  '
              f'ALLPOS {curve["full"][e]["allpos"]:.5f}  vs §1852 {S1852_R256_ALLPOS[e]:.5f}  '
              f'delta {worse[e]:+.5f}', flush=True)
    print(f'\n  the DOMINATION survives this covered set -> {pa}  cost {fullcost:.1f}M > '
          f'{S1852_R256_COST_M}M and CE worse on all roles '
          f'{all(v > 0 for v in worse.values())}', flush=True)
    print(f'  and the covered set is COMPARABLE (within 15% of {NCOV}) -> {pb}  {NFULL} types',
          flush=True)
    print(f'  and full-rank ATTAINMENT holds again -> {pc}  ' + '  '.join(
        f'{e} {curve["full"][e]["cov"] - ceilv[e]:+.6f}' for e in roles2), flush=True)
    print(f'  ceiling above live on every role -> control {pd}', flush=True)

    json.dump({'run': 'second_class_dom_192_288', 'fit_slice': [192, 288],
               'n_covered': NFULL, 'full_cost_M': fullcost, 'worse_than_r256': worse,
               'ceiling': ceilv, 'live': livev, 'curve': curve, 'attainment_exact': bool(attain),
               'predictions': {'pred_a_domination_survives': bool(pa),
                               'pred_b_coverage_comparable': bool(pb),
                               'pred_c_attainment_holds': bool(pc),
