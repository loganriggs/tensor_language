#!/usr/bin/env python3
"""Frozen fourth-corpus authority for response-weighted subject-number testing."""
...
import circuit_fast_screen_candidate_subject_number_rank1_fresh_confirmation as prior


SCHEMA = "subject_number_response_weighted_fresh_authority_v1"
TASK_ID = "subject_verb.number_agreement"
CAPABILITY_ID = "subject_verb.number_agreement.response_weighted_fresh_capability_v1"
CAUSAL_CANDIDATE_ID = "subject_verb.number_agreement.response_weighted_prototype_fresh_v1"
SUBJECT_POSITION = 8
ROLES = ("recipient", "opposite_same_lemma", "same_number_different_lemma")
TEMPLATES = (
    ("across_beside", "Across the {a1} beside the {a2}, the {subject}"),
    ("beyond_alongside", "Beyond the {a1} alongside the {a2}, the {subject}"),
)
NOUN_PAIRS = (
    ("administrator", "administrators"), ("analyst", "analysts"),
    ("builder", "builders"), ("collector", "collectors"),
    ("consultant", "consultants"), ("consumer", "consumers"),
    ("contractor", "contractors"), ("creator", "creators"),
    ("customer", "customers"), ("designer", "designers"),
    ("founder", "founders"), ("guide", "guides"),
    ("inspector", "inspectors"), ("investor", "investors"),
    ("operator", "operators"), ("producer", "producers"),
)
EXPECTED_AUTHORITY_SHA256 = "b0ce33db34c2fce65f6afa54c71413e7aab3023628fb5ef8e1978fcd686ddc5d"
...
def _build_unvalidated():
    ...
    for group in range(len(NOUN_PAIRS)):
        ...
        for template_id, template in TEMPLATES:
            ...
            rows.append({"schema": SCHEMA, "task_id": TASK_ID, "capability_id": CAPABILITY_ID,
...
def validate_rows(rows, verify_hash=True):
    ...
    if len(forms) != 32 or forms & prior_vocabulary \
            or any(len(old_task14.ENCODING.encode(" " + word)) != 1 for word in forms):
    ...
    for row in rows:
        ...
        if recipient["subject_number"] == opposite["subject_number"] \
                ...
                or recipient["subject"] == lexical["subject"]:
    ...
    if len(set(prompts)) != 96 or len(set(tokens)) != 96 \
            or set(prompts) & prior_prompts or set(tokens) & prior_tokens:
