#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_late_units_serve_the_pronoun_slot pred_c_random_units_do_not pred_d_hub_units_at_the_answer_small pred_e_hub_units_at_the_noun_replay
...
OUT = ROOT / "circuits/followups/late_units_pronoun_slot_v463_result.json"
CANDIDATE_ID = "chain.late_units_pronoun_slot_v463"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, LATE_MIN, RANDOM_MAX, HUB_A_MAX, HUB_V423, BAND = 1e-3, 0.03, 0.01, 0.05, 0.124, 0.02
import random
import random
...
FORWARDS_MAX = 21
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_late_units_serve_the_pronoun_slot": ">= 0.03", "pred_c_random_units_do_not": "<= 0.01", "pred_d_hub_units_at_the_answer_small": "<= 0.05", "pred_e_hub_units_at_the_noun_replay": "0.124 +- 0.02"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "late_min": LATE_MIN, "random_max": RANDOM_MAX, "hub_a_max": HUB_A_MAX, "hub_v423": HUB_V423, "band": BAND}}
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_late_units_serve_the_pronoun_slot": v["LATE_A"] >= LATE_MIN, "pred_c_random_units_do_not": abs(v["RANDOM_A"]) <= RANDOM_MAX,
                   "pred_d_hub_units_at_the_answer_small": v["HUB_A"] <= HUB_A_MAX, "pred_e_hub_units_at_the_noun_replay": abs(v["HUB_N"] - HUB_V423) <= BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "late_units_pronoun_slot_result_v463", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
