...
"""Pronoun number they/he DoD (v197): the unit-grain story OUT OF THE PANEL. Zero the three MLP-8 number units {829 plural, 953 singular, 1030} (v168 census;
v169b edit: 5.3% of the margin at the noun on fresh rows, 11.8% at all positions) at the CUE position of the 128 natural rows (FineWeb v77 + Pile v78:
16 rows per (number, label) cell, outcome-blind miner), and read the label margin at the final token. Congruent rows (plural-they, singular-he) should
lose margin toward the label; incongruent rows (plural-he, singular-they) should gain it if the units carry the noun's number and nothing else (the
counter-case that the head-level set FAILED on both sources, v77/v78 pred_f). Null: 16 seeded random 3-unit sets zeroed at the cue. Readers
will-would / who-which / night-day (number-free), gate move <= null mean + 0.25 x damage.
PREDICTIONS (scored as written; failures preserved)
    pred_a_instrument_replays_native        hooked forward with no edit = producer native <= 1e-4
    pred_b_congruent_damage_live            congruent rows: damage fraction >= 0.03 of the native congruent margin and positive >= 0.65. Prior: unsure (5.3% in-panel).
    pred_c_units_beat_random_unit_triples   congruent damage > max of the 16 random 3-unit sets
    pred_d_units_selective                  each reader |move| <= null mean |move| + 0.25 x damage
    pred_e_incongruent_shifts_toward_label  incongruent rows: mean oriented damage < 0 (margin toward the label grows). Prior: unsure -- heads failed this.
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_mlp8_units_natural_v197_result.json"
...
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_mlp8_units_natural_v197"
LAYER, UNITS, CONGRUENT = 8, (829, 953, 1030), ("natural_plural_they", "natural_singular_he")
NULL_SEEDS = tuple(range(7901, 7917))
LIVE_FRACTION, LIVE_POSITIVE, GATE_RATIO, INSTRUMENT_TOL = 0.03, 0.65, 0.25, 1e-4
FORWARDS_MAX = 84
PREDICTIONS = {"pred_a_instrument_replays_native": "<= 1e-4", "pred_b_congruent_damage_live": ">= 0.03, positive >= 0.65", "pred_c_units_beat_random_unit_triples": "> random max", "pred_d_units_selective": "three gates", "pred_e_incongruent_shifts_toward_label": "< 0"}
