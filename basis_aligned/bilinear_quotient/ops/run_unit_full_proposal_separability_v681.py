...
"""v681: the WHOLE proposal in one family -- thirteen cells that each passed separability somewhere else.

THE QUESTION THIS ASKS, AND WHY IT IS NOT A REPEAT. Thirteen cells now clear four tier rows and the held-out control
under BOTH objectives: the eight already proposed to Codex, plus the five from v679 (comparative_frame,
comparative_complement_from_than, reflexive_number_itself_themselves, wh_adjunct_when_where, wh_argument_selection).
Every one of them has passed a separability test -- but in DIFFERENT families. our_your and either_not separated in
v663's seven-member family; reciprocal_lenmatched, durativity_until_by and both_either in v665's sixteen-member one
and again in v677's nine; adj_adv_feel, countability_few_little and definiteness_anaphor_needed only in v677. The
five new ones have never faced a separability test at all. Separability is a property of a SET, so thirteen
individual passes in six different families do not establish that the thirteen are thirteen directions. This run
puts all of them in ONE family and asks that question directly. If the proposal is coherent, this is the receipt
that says so; if two of them are one direction, better to find it here than after Codex counts them.
WHAT MAKES A FAILURE LIKELY, STATED WITHOUT NAMING A PAIR. Two structural facts, not semantic intuitions. First,
this family contains two comparative cells (comparative_frame, comparative_complement_from_than) and two wh cells
(wh_adjunct_when_where, wh_argument_selection) -- the first time this proposal has held two cells from one axis at
once, and every fusion this corpus has recorded has been WITHIN an axis (v631's number cells, v609's numerals,
v665's gender pair). Second, thirteen candidates means twelve sibling leak paths each, against v677's five and
v663's six; leak opportunity grows with family size and v665 already showed one cell (numeral_dual_both_all) falling
on the family-constrained extraction floor rather than on any leak.
I AM NOT NAMING A PREDICTED PAIR. Twice now I have registered a named fusion from readout adjacency -- v665's
rather_prefer/let_want_complement on an EXACT shared readout pair, v677's countability_few_little/
definiteness_anaphor_needed on a shared quantity axis -- and both times the named pair came back among the CLEANEST
in its family. The axis-level structure above is a different and better-supported signal than readout adjacency, so
it is stated; a specific pair is not, because the signal I would use to pick one has been measured twice and found
uninformative.
WHAT HAPPENS TO THE PROPOSAL, COMMITTED BEFORE THE RUN. Any cell that fails separability here is REMOVED from the
proposal, exactly as two cells were withdrawn after v673. A cell that fails only the family-constrained extraction
floor while keeping a small sibling leak is a NEAR-MISS, not a duplicate -- that distinction cost me a false
duplicate report in v665 and it is registered here so the verdict does not get read off the boolean.
NOTHING IS COUNTED BY THIS FILE.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
  pred_a_new_separable at least k_new = 11 of the thirteen candidates are separable at cross_max = 0.05 while
                       sitting in one family with each other. Worked example: if twelve separate and
                       comparative_frame has a largest sibling leak of 0.06, the count is 12, 12 >= 11 is true,
                       pred_a is TRUE -- and comparative_frame is still removed from the proposal. The bar measures
                       the batch; the per-cell action is separate and already committed.              prior 55%
  pred_b_counted_kept  at least k_kept = 2 of the three counted members stay separable in the same run, so a null on
                       ...
                       true, pred_b is TRUE. Note number_reflexive_myself_ourselves now sits beside
                       reflexive_number_itself_themselves, so this control is under more pressure than usual.
                                                                                                      prior 75%
  pred_c_row4_kept     at least k_d = 2 members of EVAL keep a family-arm C upper bound at or below c_ub_max = 0.01.
                       Worked example: ten of sixteen at or under 0.01 gives 10, 10 >= 2 is true, pred_c is TRUE.
                                                                                                      prior 85%
  pred_d_instrument    every EVAL member reproduces its parent battery extraction within instr_tol = 0.03. Worked
                       example: wh_adjunct_when_where scored 1.018 in its parent receipt, so an own arm of 1.002 is
                       a gap of 0.016 and holds; one member off by 0.04 makes pred_d FALSE for the whole run. With
                       sixteen members drawn from many batteries this is the widest instrument check yet.
                                                                                                      prior 70%
SCOPE. Thirteen both-objectives survivors, three counted controls, ONE family. No counting.
Smoke: V681_SMOKE=<out.json> (CPU) -- proves the code path runs; its numbers are not measurements.
"""
...
OUT = ROOT / "circuits/followups/unit_full_proposal_separability_v681_result.json"
...
FAMILIES = {
    "full_proposal": ['possessive_gender', 'correlative_or_and', 'number_reflexive_myself_ourselves', 'possessive_person_our_your', 'correlative_disjoint_either_not', 'reciprocal_lenmatched', 'durativity_until_by', 'both_either', 'adj_adv_feel', 'countability_few_little', 'definiteness_anaphor_needed', 'comparative_frame', 'comparative_complement_from_than', 'reflexive_number_itself_themselves', 'wh_adjunct_when_where', 'wh_argument_selection'],
}
COUNTED_LIST = ['possessive_gender', 'correlative_or_and', 'number_reflexive_myself_ourselves']
COUNTED = tuple(COUNTED_LIST)
EVAL = tuple(COUNTED_LIST) + tuple(['possessive_person_our_your', 'correlative_disjoint_either_not', 'reciprocal_lenmatched', 'durativity_until_by', 'both_either', 'adj_adv_feel', 'countability_few_little', 'definiteness_anaphor_needed', 'comparative_frame', 'comparative_complement_from_than', 'reflexive_number_itself_themselves', 'wh_adjunct_when_where', 'wh_argument_selection'])
...
BARS = {"cross_max": 0.05, "keep": 0.8, "k_kept": 2, "ext_floor": 0.5, "c_ub_max": 0.01, "k_d": 2, "k_new": 11, "instr_tol": 0.03}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 20000, 640000


def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_full_proposal_separability_v681", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V681_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V681_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    evaluate = [n for n in members if n in EVAL and (not smoke or n in os.environ.get("V681_SMOKE_NAMES", "wh_adjunct_when_where").split(","))]
    ...
    result = {"predictions": predictions, "schema": "unit_full_proposal_separability_v681", "candidate_id": "corpus.unit_full_proposal_separability_v681",
