...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; arms norm / cond-576 / cond-144 / K-0 x eight windows")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_cond_floor_results.json'
...
if __name__=='__main__':
    ...
    print('ARM 2/4: conditioned-576 (S2134 reproduction)',flush=True)
    r_s=main()
    SEL['mode']='fisher8'; SEL['K']=144
    print('ARM 3/4: conditioned-144 (sixteenth price)',flush=True)
    r_q=main()
    SEL['mode']='fisher8'; SEL['K']=0
    print('ARM 4/4: K-0 (Down-bias only, the null)',flush=True)
    r_z=main()
    import statistics as stt
    g5=[round(a-b,4) for a,b in zip(r_s['fresh8'],r_norm['fresh8'])]
    g1=[round(a-b,4) for a,b in zip(r_q['fresh8'],r_s['fresh8'])]
    g0=[round(a-b,4) for a,b in zip(r_z['fresh8'],r_s['fresh8'])]
    pa=stt.median(g1)>=-0.01
    pb=stt.median(g0)<=-0.05
    pc=abs(stt.median(g5)-0.1277)<=0.015
    res={'norm2304':r_norm,'cond576':r_s,'cond144':r_q,'K0':r_z,
         'per_window_cond576_minus_norm':g5,'per_window_cond144_minus_cond576':g1,
         'per_window_K0_minus_cond576':g0,
         'pred_a_sixteenth_price':bool(pa),'pred_b_units_carry_ce':bool(pb),'pred_c_reproduces_cond576':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'cond576 - norm per window: {g5}')
    print(f'cond144 - cond576 per window: {g1}')
    print(f'K0 - cond576 per window: {g0}')
    print(f"(a) cond-144 median {stt.median(g1):+.4f} >= -0.01: {'HELD' if pa else 'FAILED'}")
    print(f"(b) K0 median {stt.median(g0):+.4f} <= -0.05: {'HELD' if pb else 'FAILED'}")
    print(f"(c) cond-576 median {stt.median(g5):+.4f} vs 0.1277 (tol 0.015): {'HELD' if pc else 'FAILED'}")
