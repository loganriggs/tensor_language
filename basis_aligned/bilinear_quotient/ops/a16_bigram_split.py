...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['a16_induction_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; bigram-addressed split of the a16L damage on FR (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'a16_bigram_split_results.json'
...
if __name__=='__main__':
    ...
    big=torch.zeros(len(clsflat),dtype=torch.bool)
    for r6 in range(FRtok.shape[0]):
        seenbg=set()
        toks=FRtok[r6,:257].tolist()
        for p6 in range(1,256):
            bg=(toks[p6-1],toks[p6])
            if bg in seenbg: big[r6*256+p6]=True
            seenbg.add(bg)
    io=(clsflat==IND)|(clsflat==OTH)
    m_yes=io&big; m_no=io&(~big)
    s_yes=float(diff[m_yes].sum()); s_no=float(diff[m_no].sum()); s_all=s_yes+s_no
    mu_yes=float(diff[m_yes].mean()) if int(m_yes.sum())>0 else 0.0
    mu_no=float(diff[m_no].mean()) if int(m_no.sum())>0 else 0.0
    pa=s_yes>=0.6*s_all
    pb=mu_yes>=2*mu_no
    pc=stt.median(rp)<=0.005
    res={'full':r_full,'skip16':r_skip,
         'sums':{'bigram_matched':round(s_yes,1),'unmatched':round(s_no,1),'all_ind_other':round(s_all,1)},
         'means':{'bigram_matched':round(mu_yes,5),'unmatched':round(mu_no,5)},
         'counts':{'bigram_matched':int(m_yes.sum()),'unmatched':int(m_no.sum())},
         'convention':'per-position CE(full with a16L) - CE(attn16 real) on FR; positive = the dictionary hurts there',
         'pred_a_bigram_carries':bool(pa),'pred_b_concentrated':bool(pb),'pred_c_reproduces_s2146':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"ind+other damage: bigram-matched {s_yes:+.1f} ({int(m_yes.sum())} pos, mean {mu_yes:+.5f}) | unmatched {s_no:+.1f} ({int(m_no.sum())} pos, mean {mu_no:+.5f})")
    print(f"(a) matched sum >= 0.6 x all: {'HELD' if pa else 'FAILED'}")
    print(f"(b) matched mean >= 2 x unmatched mean: {'HELD' if pb else 'FAILED'}")
