...
OUT=POLY/'UNSUPERVISED_DATA_V1_RESULT.json';BIND=POLY/'UNSUPERVISED_DATA_V1_BINDING.json'
...
def main():
    ...
    backend=Bilin18TorchBackend.load('cuda');m=backend.model;counts=[0,0];cache={};xs=[];ys=[];rounding={};maxabs={}
    ...
    try:
        for start in range(0,1000,4):
            ...
            for key,store in [('x',xs),('y',ys)]:
                value=cache[key][bi,positions].float();rounded=value.half();assert torch.isfinite(rounded).all()
                error=float((rounded.float()-value).norm()/value.norm().clamp_min(1e-30))
    ...
    data=dict(x=torch.cat(xs),y=torch.cat(ys),**{k:spec[k] for k in ['positions','train_rows','validation_rows','test_rows']})
    ap=POLY/'UNSUPERVISED_DATA_V1_STATES.pt';assert not ap.exists();torch.save(data,ap)
    valid=counts==[250,1000] and data['x'].shape==data['y'].shape==(1000,64,1152)
    out=dict(schema='unsupervised.data.v1',predictions={'pred_a_capture_instrument':bool(valid),'pred_b_split_integrity':bool(split_ok),'pred_c_storage_fidelity':bool(valid and max(rounding.values())<=5e-4)},
        ...
        wall_seconds=time.perf_counter()-tic,scope='Unlabeled MLP17 normalized inputs and native outputs from historically opened cached corpus rows. Row-held-out splits, not fresh/OOD or verified document-level separation. FP16 rounding floor explicit.')
