#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_all_values_at_the_cue_carry_aspect pred_c_three_readers_carry pred_d_low_and_high_add pred_e_pairs_are_sign_constant
"""The ASPECTUAL route census at the cue (v596): the number method on the has / had line. After 'since ...' the auxiliary tends to 'has', after 'by ...' to
'had'; the 17 Sep battery named {8.1, 9.1, 9.4} (8.1 reads since / by token-only; 9.1 / 9.4 relay) at 50-66% of the fresh margin. Here replace-edits on natural
aligned pairs made by swapping the cue in place (' since' <-> ' by'; v20's 64 fineweb rows, outcome-blind): every head's value at the cue, the three readers'
values, blocks 0-8, blocks 9-17 -- fraction of the since - by has - had gap closed. The 'by' rows are often the passive 'by', so the swapped partner is not
always natural: the gap's sign constancy is registered as a prediction.
PREDICTIONS (scored as written; failures preserved; priors from the number / gender / person censuses; aspect priors unsure)
    pred_a_baseline_replays                   the unedited manual forward replays the model's has - had margins within 1e-3
    pred_b_all_values_at_the_cue_carry_aspect every head's value at the cue swapped closes >= 0.80 of the gap
    pred_c_three_readers_carry                {8.1, 9.1, 9.4} values at the cue close >= 0.30. Prior: unsure
    pred_d_low_and_high_add                   blocks 0-8 + blocks 9-17 is within 0.10 of all heads. Prior: unsure
    pred_e_pairs_are_sign_constant            the since - by gap has the same sign on >= 0.75 of the pairs
...
OUT = ROOT / "circuits/followups/aspect_route_census_cue_v596_result.json"
CANDIDATE_ID = "aspect.route_census_cue_v596"
ROWS = ROOT / "circuits/followups/aspectual_anchor_dod_natural_rows_v20.json"
N_HEAD = 9
FIVE = {8: [1], 9: [1, 4]}   # the three aspectual readers (name kept)
REPLAY_TOL, ALL_MIN, THREE_MIN, ADD, SIGN_MIN = 1e-3, 0.80, 0.30, 0.10, 0.75
FORWARDS_MAX = 11
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_all_values_at_the_cue_carry_aspect": ">= 0.80", "pred_c_three_readers_carry": ">= 0.30", "pred_d_low_and_high_add": "|LOW + HIGH - ALL| <= 0.10", "pred_e_pairs_are_sign_constant": ">= 0.75"}


def aspect_pairs(rows_path=None):
    """Aligned pairs from v20's natural rows: the cue swapped in place (' since' <-> ' by', 'Since' <-> 'By'); (since_row, by_row, cue_offset)."""
    E = L.ENCODING; partner = {" since": " by", " by": " since", "Since": "By", "By": "Since", " Since": " By", " By": " Since"}
    items = []
    for r_ in json.loads((rows_path or ROWS).read_text())["rows"]:
        ...
        since_row, by_row = (r_["ids"], sw) if t.strip().lower() == "since" else (sw, r_["ids"])
        items.append((since_row, by_row, c))
    return items


gender_pairs = aspect_pairs   # name kept for the lineage's callers
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "text_pairs": len(text_items), "rows": str(ROWS.name), "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "all_min": ALL_MIN, "three_min": THREE_MIN, "add": ADD, "sign_min": SIGN_MIN}}
    if os.environ.get("BQLIB_DRYRUN") or os.environ.get("BQLIB_NO_MODEL"):
        print(json.dumps(plan, indent=2, sort_keys=True)); return
    t0 = time.perf_counter()
    backend = producer.Bilin18TorchBackend.load("cuda"); torch, F, model = backend.torch, backend.F, backend.model; blocks = model.transformer.h
    from jacclust.tt_model import apply_rotary_emb  # noqa
    D = model.config.n_embd; hd = D // N_HEAD; HE, SHE = L._single(" has"), L._single(" had")   # aspect: the margin is has - had (names kept from the gender lineage)
    ...
    c = text["closed"]
    print("aspect census", {k: round(v, 3) for k, v in c.items()}, "gap", round(text["gap_mean"], 2), "sign", round(text["sign_constancy"], 3))
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_all_values_at_the_cue_carry_aspect": c["ALL_N"] >= ALL_MIN, "pred_c_three_readers_carry": c["FIVE_N"] >= THREE_MIN,
    ...
    OUT.write_text(json.dumps({"schema": "aspect_route_census_cue_result_v596", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
