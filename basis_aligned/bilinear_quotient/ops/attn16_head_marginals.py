...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; skip-a16 config + 10 per-head-zero evals on FR (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'attn16_head_marginals_results.json'
...
if __name__=='__main__':
    ...
    SEL['skip16']=True; SEL['head16']=True
    print('SINGLE ARM: S2146 skip-a16 config + per-head zero evals at attn16',flush=True)
    r_s=main()
    dh=SEL['head16_result']
    import statistics as stt
    heads=[dh[str(h)] for h in range(9)]
    dall=dh['all']
    medh=stt.median(heads)
    pa=max(heads)>=2*max(medh,1e-9)
    pb=abs(sum(heads)-dall)<=0.05
    pc=abs(r_s['L2_F']-2.5091)<=0.01
    res={'skip16':r_s,'per_head_damage':dh,'sum_heads':round(sum(heads),4),'d_all':dall,
         'convention':'L2 = CE above the real model; lower is better; d_h = damage added by zeroing head h',
         'pred_a_concentrated':bool(pa),'pred_b_additive':bool(pb),'pred_c_reproduces_s2146':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'per-head damage: {heads}  (median {medh:+.4f}, max {max(heads):+.4f}); all-zero {dall:+.4f}, sum {sum(heads):+.4f}')
    print(f"(a) max {max(heads):+.4f} >= 2 x median {medh:+.4f}: {'HELD' if pa else 'FAILED'}")
    print(f"(b) |sum - d_all| = {abs(sum(heads)-dall):.4f} <= 0.05: {'HELD' if pb else 'FAILED'}")
    print(f"(c) skip16 FR L2_F {r_s['L2_F']} vs 2.5091 (tol 0.01): {'HELD' if pc else 'FAILED'}")
