...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_mlp45_trim_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; per-layer c69@288 arms on the best x eight windows (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_c69_perlayer_results.json'
...
if __name__=='__main__':
    ...
    print('ARM 1/6: norm-2304 everywhere (anchor)',flush=True)
    r_norm=main()
    SEL['K69']=576; SEL['K69MAP']={}
    print('ARM 2/6: best = mlp45-2304 + c69-576 (S2140 reproduction)',flush=True)
    r_b=main()
    r_layer={}
    for _n,li0 in ((3,6),(4,7),(5,8),(6,9)):
        SEL['K69MAP']={li0:288}
        print(f'ARM {_n}/6: c{li0}@288, others 576',flush=True)
        r_layer[li0]=main()
    import statistics as stt
    db=[round(a-b,4) for a,b in zip(r_b['fresh8'],r_norm['fresh8'])]
    meds={}; perw={}
    for li0,r in r_layer.items():
        d=[round(a-b,4) for a,b in zip(r['fresh8'],r_b['fresh8'])]
        perw[li0]=d; meds[li0]=stt.median(d)
    tot=sum(meds.values())
    pa=min(meds.values())<=0.005
    pb=abs(tot-0.0217)<=0.015
    pc=abs(stt.median(db)-(-0.0290))<=0.015
    res={'norm2304':r_norm,'best':r_b,
         'per_layer':{f'c{li0}':{'arm':r_layer[li0],'per_window_vs_best':perw[li0],'median':round(meds[li0],4)} for li0 in r_layer},
         'per_window_best_vs_norm':db,'sum_of_medians':round(tot,4),
         'convention':'L2 = CE above the real model; lower is better; positive entries = damage added',
         'pred_a_someone_free':bool(pa),'pred_b_additive':bool(pb),'pred_c_reproduces_s2140':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    for li0 in sorted(meds): print(f'c{li0}@288 - best per window: {perw[li0]}  median {meds[li0]:+.4f}')
    print(f"(a) min layer median {min(meds.values()):+.4f} <= +0.005: {'HELD' if pa else 'FAILED'}")
    print(f"(b) additivity |{tot:+.4f} - 0.0217| = {abs(tot-0.0217):.4f} <= 0.015: {'HELD' if pb else 'FAILED'}")
