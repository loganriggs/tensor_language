"""TRAJECTORY SPLICE AT THE MOTIF GRAIN (rung 147): does front corruption TRANSIT through motif attention?

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2239 proved context corruption dominates
at tail attention. The motif heads (blocks 2-9) are the other attention family: their alpha.v approximation
reads the CP-front's drifted stream. Here real-model attn outputs at blocks 2-9 are captured position-aligned
on census rows and spliced during the census eval (all positions; li default-bound per the S2238 rule;
counter at li=9). The tail dicts stay as fit (frozen rung-127 build) - the arm prices motif-mediated
corruption + motif approximation jointly against the rung-127 baseline.

REGISTERED PREDICTIONS (anchors: census +1.9474; S2228 name +3.134; motifs-alone +0.553 solo):
  (a) LARGE JOINT SHARE: census <= 1.55 (>= 0.40 recovery).
  (b) NAME MOVES: dCE(name) <= 2.2 (motif-mediated context matters for name).
  (c) ISOLATION: |L1F - 1.5509| <= 0.02 AND census >= 1.4.
NULL: recovery < 0.15 - motif cost is direct approximation at the read position; corruption does not transit.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: trajectory splice at the motif grain')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_motiftraj_results.json'
...
def main():
    ...
    def motif_hooks(layers):
        ...
        for li in layers:
            ...
            def h(mo_,args,out,at=at,ph=ph,sh=sh,ap=ap,asf=asf,li=li):
                ...
                if SEL.get('mtraj_on'):
                    _mt=SEL['_MTJ'][li][cur['mbi']].to(X2.device)
                    if li==9: cur['mbi']=cur['mbi']+1
                    return (_mt.to(X2.dtype),v1r)
                if li==9 and 'mbi' in cur: cur['mbi']=cur['mbi']+1
                return (ynew,v1r)
    ...
    if SEL.get('mtraj'):
        ...
        _MT={li:[] for li in range(2,10)}
        hs=[]
        for _li in range(2,10):
            def _mkM(li=_li):
                def h(mo,i_,o_):
                    _MT[li].append(o_[0].detach().to(torch.float16).cpu())
                return h
            hs.append(m.transformer.h[_li].attn.register_forward_hook(_mkM()))
        ...
        for h in hs: h.remove()
        SEL['_MTJ']=_MT
        print(f'  captured real-trajectory motif-layer attn outputs ({len(_MT[2])} batches)',flush=True)
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['mtraj']=True
    print('ARM: rung-127 build + all-position trajectory splice at motif layers 2-9',flush=True)
    main()
    ...
    print(f"(a) census-bar-1.55; name {PC.get('name',9.9):+.4f} <= 1.0: {'HELD' if pa else 'FAILED'}")
    print(f"(b) name-bar-2.2 via census {agg:+.4f}: {'HELD' if pb else 'FAILED'}")
