#!/usr/bin/env python3
# BQLANE: gpu
# BQGATE: EXPERIMENT pred_a_exact_instrument pred_b_single_branches_live pred_c_failed_reader_singleton_attribution pred_d_additive_behavior_composes
"""Split additive routing/value collateral into its two single branches."""
...
PREREG=P/"SETTING2_REGIONAL_ADDITIVE_BRANCH_COLLATERAL_V1_PREREGISTRATION.md"
ROWS=P/"SETTING2_REGIONAL_INSTRUCTION_CROSS_FRESH_V1_ROWS.json"
BINDING=P/"SETTING2_REGIONAL_ADDITIVE_BRANCH_COLLATERAL_V1_BINDING.json"
...
OUT=ROOT/"basis_aligned/bilinear_quotient/circuits/fast_screens/setting2_regional_additive_branch_collateral_v1_result.json"
SOURCE_NAMES=("embedding",)+tuple(x for layer in range(8) for x in (f"attn{layer}",f"mlp{layer}"))
LATE_NAMES=("attn5","mlp5","mlp6","mlp7")
ARMS=("native","routing_only","value_only","additive")
...
def load_bound():
    ...
    files={"preregistration":PREREG,"rows":ROWS,"row_check":P/"regional_cue_row_check_v1.py",
           ...
           "edge_program":EDGE,"parent_result":PARENT,"composition_result":COMPOSITION,"source_census":CENSUS,"fresh_result":FRESH,"collateral_attribution":ATTRIBUTION}
...
def plan():
    ...
    return {"schema":"setting2_regional_additive_branch_collateral_v1_plan","model_loaded":False,"gpu_accessed":False,"queue_touched":False,"rows":len(rows),"arms":ARMS,"late_names":LATE_NAMES,"price":PRICE,"row_checks":checks,"binding_sha256":sha(BINDING)}
...
def main():
    ...
    for _,indices in sorted(buckets.items()):
        for offset in range(0,len(indices),8):
            ...
            for arm_index,arm in enumerate(ARMS):
                ...
                edit={"routing_only":route_delta,"value_only":value_delta,"additive":route_delta+value_delta}.get(arm)
    ...
    for family in sorted(set(row["family"] for row in rows)):
        ...
        for scale,name in enumerate(("numerator","logit")):
            route_target=pair_effect[1,scale,pair_ids,0]; value_target=pair_effect[2,scale,pair_ids,0]; additive_target=pair_effect[3,scale,pair_ids,0]; controls={}
            for j in range(1,len(READOUTS)):
                key=READOUTS[j][0]; route=effects[1,scale,row_ids,j]; value_effect=effects[2,scale,row_ids,j]; additive=effects[3,scale,row_ids,j]
                controls[key]={"routing_rms":float(route.square().mean().sqrt()),"value_rms":float(value_effect.square().mean().sqrt()),"additive_rms":float(additive.square().mean().sqrt()),"routing_to_additive_relative_error":rel(route,additive),"value_to_additive_relative_error":rel(value_effect,additive),"sum_to_additive_relative_error":rel(route+value_effect,additive)}
            report[name]={"routing_target_rms":float(route_target.square().mean().sqrt()),"value_target_rms":float(value_target.square().mean().sqrt()),"additive_target_rms":float(additive_target.square().mean().sqrt()),"target_sum_to_additive_relative_error":rel(route_target+value_target,additive_target),"controls":controls}
    ...
    failed=reports["1"]["logit"]["controls"]["work_jobs"]
    predictions={"pred_a_exact_instrument":bool(instrument),"pred_b_single_branches_live":bool(instrument and all(r["logit"]["routing_target_rms"]>=.002 and r["logit"]["value_target_rms"]>=.002 for r in reports.values())),"pred_c_failed_reader_singleton_attribution":bool(instrument and min(failed["routing_to_additive_relative_error"],failed["value_to_additive_relative_error"])<=.35),"pred_d_additive_behavior_composes":bool(instrument and all(r["logit"]["target_sum_to_additive_relative_error"]<=.35 and max(c["sum_to_additive_relative_error"] for c in r["logit"]["controls"].values())<=.35 for r in reports.values()))}
    ...
    result={"schema":"setting2_regional_additive_branch_collateral_v1_result","terminal":"additive_collateral_singleton" if all(predictions.values()) else "valid_additive_branch_split" if instrument else "invalid","predictions":predictions,"metrics":metrics,"family_reports":reports,"arms":ARMS,"readouts":[name for name,_ in READOUTS],"price":PRICE,"row_checks":checks,"binding_sha256":sha(BINDING),"runner_sha256":sha(RUNNER),"checkpoint_weights_sha256":"680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3","created_utc":datetime.now(timezone.utc).isoformat().replace("+00:00","Z"),"scope":"Opened fresh-panel recursive split of additive late-QK1 routing and head8.2-induced current-value branches, including target and five unrelated readers; no new transfer, fit, or parameter update."}
