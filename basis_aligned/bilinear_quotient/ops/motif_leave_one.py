...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['attn5_motif_distance_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; leave-one-real motif sweep over blocks 2-9 (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'motif_leave_one_results.json'
...
if __name__=='__main__':
    ...
    print('ARM full: motifs at 2-9',flush=True)
    r_full=main()
    import statistics as stt
    d={}
    for b8 in range(2,10):
        SEL['motif_off']=(b8,)
        print(f'ARM block-{b8} motifs REAL',flush=True)
        rb=main()
        d[b8]=round(r_full['L2_F']-rb['L2_F'],4)
        print(f'  d({b8}) = {d[b8]:+.4f}',flush=True)
    vals=list(d.values()); medb=stt.median(vals)
    mx=max(d,key=lambda b:d[b])
    pa=d[mx]>=2*max(medb,1e-9)
    pb=mx==5
    pc=abs(r_full['L2_F']-2.6662)<=0.01
    res={'full':{'L2_F':r_full['L2_F'],'L2_C':r_full['L2_C']},
         'per_block_fr_damage':{str(b):d[b] for b in sorted(d)},
         'max_block':int(mx),'median_damage':round(medb,4),
         'convention':'d(b) = L2_F(full) - L2_F(block-b motifs real); positive = the replacement costs damage',
         'pred_a_concentrated':bool(pa),'pred_b_cliff_leads':bool(pb),'pred_c_reproduces_s2144':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print('per-block motif damage: '+' '.join(f'b{b}:{d[b]:+.4f}' for b in sorted(d)))
    print(f"(a) max b{mx} {d[mx]:+.4f} >= 2 x median {medb:+.4f}: {'HELD' if pa else 'FAILED'}")
    print(f"(b) argmax = 5: {'HELD' if pb else 'FAILED'}")
