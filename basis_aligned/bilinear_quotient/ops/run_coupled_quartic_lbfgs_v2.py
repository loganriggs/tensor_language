#!/usr/bin/env python3
# BQGATE:0bodyforwards,0textsequences;720exact coefficient optimization steps,128cached validationvectors.
"""pred_a nativegradientFD<=1e-4 and monotonicobjective; pred_b additionalcapturegain>=5%; pred_c projectedgrad<=1e-6.
...
def main():
 ...
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
  print(json.dumps(dict(gpu_accessed=False,body_forwards=0,text_sequences=0,optimization_steps=720,maximum_line_search=20,fitted_floats=592704)));return
 out=P/(STEM+'_RESULT.json');ap=P/(STEM+'_PROGRAM.pt');assert not out.exists() and not ap.exists();signal.alarm(3900)
 ...
 p=torch.load(P/'COUPLED_QUARTIC_LBFGS_V1_PROGRAM.pt',weights_only=True);assert p['seed']==11511
 ...
 previous=json.loads((P/'COUPLED_QUARTIC_LBFGS_V1_RESULT.json').read_text());assert abs(initial/divisor-previous['history'][-1]['objective'])<=1e-8
 ...
 b,n,mix,history,reason=fit(b,n,evaluate,divisor,max_steps=720,max_seconds=3600,callback=callback)
 ...
 result={'pred_a':fd<=1e-4 and all(z['objective']<=history[i]['objective']+1e-10 for i,z in enumerate(history[1:])),'pred_b':-y>=1.05*(-previous['history'][-1]['objective']),'pred_c':gradnorm<=1e-6,'directional_gradient_relative_error':fd,'history':history,'termination':reason,'capture_gain_fraction':-y-1,'continuation_capture_gain_fraction':(-y)/(-previous['history'][-1]['objective'])-1,'line_search_failed':line_failed,'initial_native_error':initial_native,'final_native_error':native_error,'artifact_sha256':digest(ap),'wall_seconds':time.perf_counter()-tic,'peak_gpu_bytes':torch.cuda.max_memory_allocated(),'scope':'One-seed exact LBFGS continuation from LBFGS V1 with reset memory; localstationarity separate from gain, no OOD or selectivity evidence.'}
