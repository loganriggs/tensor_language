...
def main():
 ...
 import tiktoken
 plan=json.loads((P/'GENERATED_RESIDUAL_PLAN_V1.json').read_text())
 ...
 model=Bilin18TorchBackend.load('cuda').model.float();graph=move_preserving_dtype(torch.load(P/'FRONTIER_FRESH_GRAPH_V1.pt',weights_only=True),'cuda');bundles={name:move_preserving_dtype(torch.load(P/file,weights_only=True),'cuda') for name,file in [('separate','FRONTIER_FRESH_BASELINE_CALIBRATION_SHAPED_V1.pt'),('isotropic','FRONTIER_FRESH_BASELINE_NATIVE_ISOTROPIC_V1.pt')]}
 fold=move_preserving_dtype(torch.load(P/'PARTIAL_GRAPH_ORIGINAL_FORMS_V1.pt',weights_only=True),'cuda');A,B,alpha,beta=[fold[k] for k in ('A','B','alpha','beta')];enc=tiktoken.get_encoding('gpt2');checks=[];state_checks=[];records=[]
 for panel in (1,2):
  ...
  for domain in ('fineweb','stdlib'):
   ...
   for row in tokens[domain]:
    raw=capture(model,row[None,:256].cuda());z=raw['x16'].flatten(0,1)[16:].double();h=raw['h17'].flatten(0,1)[16:];m=(model.transformer.h[17].lambdas[0]*(raw['m16']-model.transformer.h[16].mlp.Down_bias)).flatten(0,1)[16:]
   ...
   for offset in range(0,len(valid),96):
    ids=valid[offset:offset+96];donors=mapping[ids];gi,gd=ids.cuda(),donors.cuda();h,m,md=cache['h'][gi],cache['m'][gi],cache['m'][gd];hybrid=h-m+md;vv={k:[] for k in ['true']+[f'{c}_{mode}' for c in ('graph','separate','isotropic') for mode in ('boundary','generated')]}
    for z,hh,mm in [(cache['z'][gi],h,m),(cache['z'][gd],hybrid,md)]:
     ...
     fitted={'graph':source_reads(z,graph)}
    ...
    for family,index in [('natural',0),('hybrid',1),('change',None)]:
     ...
     for cohort in ('all','continuation','spaced_word'):
      mask=torch.ones(len(ids),dtype=torch.bool) if cohort=='all' else torch.tensor([labels[i][cohort] for i in ids.tolist()]);mask=mask.cuda();n=int(mask.sum())
      if not n:continue
 ...
 absolute=all(max(c['candidates']['graph_generated']['relative_error'])<=(.20 if c['family']=='change' else .15) for c in summaries)
 relative=all(all(g<=1.1*b for g,b in zip(c['candidates']['graph_generated']['relative_error'],c['candidates'][base+'_generated']['relative_error'])) for c in summaries for base in ('separate','isotropic'))
 pred=dict(pred_a_replay=max(checks+state_checks)<1e-5,pred_b_absolute=absolute,pred_c_relative=relative)
 out=dict(predictions=pred,native_component_replay=max(checks),residual16_replay=max(state_checks),summaries=summaries,seconds=time.monotonic()-start,plan=plan,scope='Opened-panel scalar screen only. RMS17 scale and attention/embedding context remain native; not a full-model replacement. Donor source changes while recipient carry stays fixed.')
 (P/'GENERATED_RESIDUAL_V1.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(dict(predictions=pred,replay=max(checks),seconds=out['seconds'])),flush=True)
