...
"""v645: seven possessive mapping-cells clear everything -- seven behaviours, or three axes?
...
OUT = ROOT / "circuits/followups/unit_correlative_separability_v645_result.json"
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_correlative_separability_v645", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V645_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V645_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_correlative_separability_v645", "candidate_id": "corpus.unit_correlative_separability_v645",
