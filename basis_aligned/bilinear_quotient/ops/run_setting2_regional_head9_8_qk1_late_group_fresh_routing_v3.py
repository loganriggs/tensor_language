...
PREREG=P/"SETTING2_REGIONAL_HEAD9_8_QK1_LATE_GROUP_FRESH_ROUTING_V3_CORRECTION.md"
ROWS=P/"ODD_FRAMING_FRESH_V1_ROWS.json"
BINDING=P/"SETTING2_REGIONAL_HEAD9_8_QK1_LATE_GROUP_FRESH_ROUTING_V3_BINDING.json"
...
OUT=ROOT/"basis_aligned/bilinear_quotient/circuits/fast_screens/setting2_regional_head9_8_qk1_late_group_fresh_routing_v3_result.json"
...
def plan():
    ...
    return {"schema":"setting2_regional_head9_8_qk1_late_group_fresh_routing_v3_plan","model_loaded":False,"gpu_accessed":False,"queue_touched":False,"rows":len(rows),"arms":ARMS,"late_names":LATE_NAMES,"price":PRICE,"row_checks":bound["row_checks"],"binding_sha256":sha(BINDING)}


def rotate(x,cos,sin):
    d=x.shape[-1]//2
    c=(cos if x.ndim==4 else cos[:,:,0]).to(x.dtype); s=(sin if x.ndim==4 else sin[:,:,0]).to(x.dtype)
...
def main():
    ...
    target=effects[:,:,0][:,::2]-effects[:,:,0][:,1::2]
    native_pair=values[0,:,0][::2]-values[0,:,0][1::2]
    folded_sel=fold_selected[::2]-fold_selected[1::2]; folded_rr=fold_rr[::2]-fold_rr[1::2]; folded_parent=folded_sel+folded_rr; folded_complete=fold_complete[::2]-fold_complete[1::2]; predicted=-folded_sel; actual=target[1]
