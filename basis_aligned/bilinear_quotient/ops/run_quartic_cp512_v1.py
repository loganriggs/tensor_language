...
"""Exact CP512: two Muon random starts,25steps; price1536products2385920coeff.
...
def main():
 ...
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
  ...
  print(json.dumps(dict(controls=checked,rank512_shape_smoke=True,steps=25,seeds=[1001,1002],products=1536,coefficients=2385920)));return
 torch.backends.cuda.matmul.allow_tf32=False;start=time.monotonic();out=P/'QUARTIC_CP512_NATIVE_V1.json';assert not out.exists();scale=19054614563.464127
 ...
 for seed in [1001,1002]:
  ...
  with torch.no_grad():
   ...
   final=metrics(f,c);ref=values(f,c,text[:128]);fp=values([a.float() for a in f],c.float(),text[:128].float()).double();drift=float((fp-ref).norm()/ref.norm());drifts.append(drift)
   artifact=dict(factors=[a.cpu().float() for a in f],coefficients=(c*scale).cpu().float(),writer=writer.cpu().float(),seed=seed,degree=4)
   path=P/f'QUARTIC_CP512_SEED{seed}_V1.pt';torch.save(artifact,path)
