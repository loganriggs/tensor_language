...
"""Width8 snapshot-balanced trial/test spaces with exact norm geometry.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_joint_chain_v653_result.json'
...
def main():
    plan=dict(prefix_calls=12,suffix_calls=16,calibration_rows=128,evaluation_rows=48,width=8,
              additional_endpoint_chain_batches=6,basis_selection='snapshot-balanced response/finite-reader biorthogonal spaces',controls=['dense','random_seed647'],
    ...
    a=max(checks)<=1e-4 and max(absolute)<=1e-4 and max(reader_checks)<=1e-4 and max(encoder_checks)<=1e-8 and counts==dict(prefix_calls=12,suffix_calls=16)
    ...
    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,
        ...
        scope='Empirical balanced dual-space width8 control, 9216 extra initial-encoder values charged, same calibration/evaluation data; final projection is oracle. Data-informed residual basis, no native circuit adoption.')
