...
STEM='AMORTIZED_SPARSE_FRAME_FIDELITY_V1'
...
def main():
    ...
    fit=json.loads((P/'AMORTIZED_SPARSE_FRAME_NATIVE_V1_RESULT.json').read_text())
    ...
    initial=torch.load(P/'SPARSE_FRAME_NATIVE_V1_WRITES.pt',weights_only=True)['candidate'].cuda()
    ...
    prior_node=json.loads((P/'SPARSE_FRAME_NATIVE_V1_RESULT.json').read_text())['node']['index']
    prior_frames=torch.load(P/'STREAMED_SPARSE_FRAME_NATIVE_V1_FRAMES.pt',weights_only=True)
    prior_arm=json.loads((P/'SPARSE_FRAME_NATIVE_V1_RESULT.json').read_text())['best_arm']
    reader_cosine_to_prior=float(abs(a@prior_frames['frames'][prior_arm][:,prior_node].cuda()))
    ...
    result.update(best_arm=best,fit_converged=fit['reports'][best]['converged'],effects=effects,families=families,
                  ...
                  node=dict(index=node,reader_cosine_to_prior=reader_cosine_to_prior,edges=int(mask.sum()),readers=int(torch.unique(e[:,mask]).numel()),
