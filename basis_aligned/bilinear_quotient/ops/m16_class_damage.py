"""IS m16's FUNCTION RETRIEVAL-SHAPED TOO (rung 65): the cross-object test joining the m16 remainder to the
retrieval price.

CONVENTION (S2135): per-position CE(m16 zeroed throughout, bases excluded) - CE(full S2144 config, m16 real) on
FR, grouped by the 10 oracle classes; positive = m16 was helping there.

S2151/S2157: the tail-attention price is an in-document-retrieval price. m16 is an MLP - it CANNOT read other
positions - so if its deletion damage still concentrates on ind/other positions, block 16 is a coupled
retrieval unit (attn16 reads, m16 transforms what was read), unifying the m16 remainder (S2098-S2100, S2127)
with the retrieval story. PRICE: none (attribution). NULL: m16's damage is class-uniform (a generic MLP).

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) RETRIEVAL-SHAPED: combined share of ind+other+subword >= 0.60.
  (b) ind ALONE CARRIES: share(ind) >= 0.25.
  (c) REPRODUCTION: the full arm reproduces S2144 (FR L2_F 2.6662 within 0.01).

Writes m16_class_damage_results.json. Self-reviewed. RUNG-63 HEADER FOLLOWS.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['attn14_class_damage_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; per-class CE attribution of DELETING m16 on FR (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'m16_class_damage_results.json'
...
def main():
    ...
    _hz=[]
    if SEL.get('zero_m16'):
        _hz.append(m.transformer.h[16].mlp.register_forward_hook(
...
if __name__=='__main__':
    ...
    SEL['skipset']=(); SEL['clsdmg']=True; SEL['zero_m16']=False
    print('ARM 1/2: full S2144 config (m16 real), per-position CE captured',flush=True)
    r_full=main()
    cev_full=SEL['cev']; clsflat=SEL['clsflat']
    SEL['zero_m16']=True
    print('ARM 2/2: m16 zeroed throughout (bases excluded), per-position CE captured',flush=True)
    r_zero=main()
    cev_zero=SEL['cev']
    import statistics as stt
    diff=(cev_zero-cev_full)
    tot=float(diff.sum())
    shares={}
    for k in range(10):
        selk=clsflat==k
        shares[k]=float(diff[selk].sum())/max(tot,1e-9)
    ret_share=shares[CLS.index('ind')]+shares[CLS.index('other')]+shares[CLS.index('subword')]
    pa=ret_share>=0.60
    pb=shares[CLS.index('ind')]>=0.25
    pc=abs(r_full['L2_F']-2.6662)<=0.01
    res={'full':r_full,'m16_zeroed':r_zero,'total_damage_sum':round(tot,2),
         'class_shares':{CLS[k]:round(shares[k],4) for k in range(10)},
         'retrieval_combined_share':round(ret_share,4),
         'convention':'per-position CE(m16 zeroed) - CE(m16 real) on FR; positive = m16 was helping there',
         'pred_a_retrieval_shaped':bool(pa),'pred_b_ind_carries':bool(pb),'pred_c_reproduces_s2144':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print('class shares: '+' '.join(f'{CLS[k]}:{shares[k]:+.3f}' for k in range(10)))
    print(f"(a) ind+other+subword {ret_share:+.3f} >= 0.60: {'HELD' if pa else 'FAILED'}")
    print(f"(b) ind {shares[CLS.index('ind')]:+.3f} >= 0.25: {'HELD' if pb else 'FAILED'}")
