#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_15_1_leads_block_15 pred_c_15_1_carries_most pred_d_five_writes_exceed_five_values pred_e_singles_add_to_block
"""Block 15 into the pronoun slot, by head, and the five readers' writes vs their values (v484). v481-v483: into the pronoun slot the writes are 9.6 0.362,
12.4 0.244, 10.5 0.084, 10.1 0.060 and block 15 0.074. Here block 15 by head (15.1?), and the sum of the five readers' writes into the answer against their
values swapped at the noun (0.483, v433): the difference is what they read from the post-noun token and later positions.
PREDICTIONS (scored as written; failures preserved; priors from v433 / v481)
    pred_a_baseline_replays          the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_15_1_leads_block_15       head 15.1 has the largest single-head effect in block 15
    pred_c_15_1_carries_most         15.1 carries >= 0.60 of block 15's 0.074
    pred_d_five_writes_exceed_five_values  the five readers' writes into the answer sum to >= 0.483 + 0.10 (they read more than the noun). Prior: likely (v402: two sites)
    pred_e_singles_add_to_block      the nine single-head effects sum to within 0.03 of 0.074
PRICE (registered maximum): 4 batches x 10 passes = 40 forwards; 0 backwards; 0 fits. Bar <= 41.
"""
...
OUT = ROOT / "circuits/followups/attn_into_pronoun_heads_15_v484_result.json"
CANDIDATE_ID = "chain.attn_into_pronoun_heads_15_v484"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, TOP_SHARE_MIN, ADD_TOL, B15, FOUR_WRITES, FIVE_VALUES, UNIT = 1e-3, 0.60, 0.03, 0.074, 0.362 + 0.244 + 0.084 + 0.060, 0.483, 701
import random
...
FORWARDS_MAX = 41
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_15_1_leads_block_15": "15.1 first", "pred_c_15_1_carries_most": ">= 0.60 x 0.074", "pred_d_five_writes_exceed_five_values": ">= 0.583", "pred_e_singles_add_to_block": "within 0.03"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "top_share_min": TOP_SHARE_MIN, "add_tol": ADD_TOL, "b15_v481": B15, "four_writes": FOUR_WRITES, "five_values_v433": FIVE_VALUES, "unit": UNIT}}
    ...
    MODES = {f"{b}.{h}": ({}, (0,)) for b in (15,) for h in range(N_HEAD)}; forwards = 0
    def logits_margin(x):
    ...
    mc = verb["margin_closed"]; b15 = {m: mc[m] for m in mc if m.startswith("15.")}; report["sums"] = {"15": sum(b15.values())}; five_writes = FOUR_WRITES + b15.get("15.1", 0); report["five_writes_into_answer"] = five_writes
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_15_1_leads_block_15": max(b15, key=b15.get) == "15.1", "pred_c_15_1_carries_most": b15.get("15.1", 0) >= TOP_SHARE_MIN * B15,
                   "pred_d_five_writes_exceed_five_values": five_writes >= FIVE_VALUES + 0.10, "pred_e_singles_add_to_block": abs(sum(b15.values()) - B15) <= ADD_TOL}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "attn_into_pronoun_heads_15_result_v484", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
