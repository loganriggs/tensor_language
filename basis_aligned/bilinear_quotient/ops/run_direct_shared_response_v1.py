...
"""Shared-bank native response validation on the same registered input screen.
...
pred_b_order: shared quartic response error increases <=.02 versus original quartic at each learned amplitude.
...
Shared quartic price28912coefficients18products; no fitting; folded-function local responses, not semantic
...
def main():
 ...
 learned=[r for r in records if r['family']=='learned'];pred=dict(pred_a_response=all(m['error']<.35 and m['cosine']>.9 for r in learned for m in r['metrics'].values()),pred_b_order=all(r['metrics']['quartic']['error']<=next(old for old in json.loads((P/'DIRECTIONAL_RESPONSE_V1.json').read_text())['records'] if old['family']=='learned' and old['amplitude']==r['amplitude'])['metrics']['quartic']['error']+.02 for r in learned),pred_c_instrument=maxradius<1e-5 and max(precision)<1e-3 and all(d['target_rms']>0 for r in records for d in r['directions']));out.write_text(json.dumps(dict(plan=PLAN,records=records,predictions=pred,maximum_radius_error=maxradius,maximum_response_precision_error=max(precision),seconds=time.perf_counter()-start,scope='Input counterfactual responses of isolated folded function, not semantic/native full-model interventions. Random directions match pre-normalization perturbation L2; normalization can change realized tangent lengths.'),indent=2)+'\n');print(pred,flush=True)
