...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['a16_bigram_split_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; distance split of the a16L ind damage on FR (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'a16_distance_split_results.json'
...
if __name__=='__main__':
    ...
    SEL['clsdmg']=True
    print('ARM 1/2: full S2144 config (a16L installed), per-position CE captured',flush=True)
    r_full=main()
    cev_full=SEL['cev']; clsflat=SEL['clsflat']; FRtok=SEL['FRtok']
    SEL['skip16']=True
    print('ARM 2/2: skip-a16 (attn16 real), per-position CE captured',flush=True)
    r_skip=main()
    cev_skip=SEL['cev']
    import statistics as stt
    prev=json.load(open(PT+'frontier_skip_a16_results.json'))['skip_a16']['fresh8']
    rp=[abs(round(a-b,4)) for a,b in zip(r_skip['fresh8'],prev)]
    diff=(cev_full-cev_skip)
    IND=CLS.index('ind')
    ...
    near=ok&(dist<=32)
    s_near=float(diff[near].sum()); s_all=float(diff[ok].sum())
    import math
    bins={}
    for b6 in range(9):
        lo,hi=2**b6,2**(b6+1)
    ...
    res={'full':r_full,'skip16':r_skip,
         'sum_near32':round(s_near,1),'sum_all_ind':round(s_all,1),
         'bins_log2':{str(b):{'n':bins[b]['n'],'mean':round(bins[b]['mean'],5)} for b in bk},
         'spearman_bin_vs_mean':round(rho,3),
         'convention':'per-position CE(full with a16L) - CE(attn16 real) on FR; positive = the dictionary hurts there',
         'pred_a_near_carries':bool(pa),'pred_b_monotone':bool(pb),'pred_c_reproduces_s2146':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print('log2-distance bins (n, mean damage): '+' '.join(f'2^{b}:{bins[b]["n"]}/{bins[b]["mean"]:+.4f}' for b in bk))
    print(f"(a) near(<=32) {s_near:+.1f} >= 0.5 x all {s_all:+.1f}: {'HELD' if pa else 'FAILED'}")
    print(f"(b) |Spearman| {abs(rho):.3f} >= 0.5: {'HELD' if pb else 'FAILED'}")
    print(f"(c) skip16 vs S2146 median |delta| {stt.median(rp):.4f} <= 0.005: {'HELD' if pc else 'FAILED'}")
