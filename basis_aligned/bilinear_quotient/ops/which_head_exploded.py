# WHICH HEAD EXPLODED?  -- Logan's question, and my measurements cannot currently answer it.
#
# §1804 found that a live attention module fed a COMPILED (context-free) residual stream diverges:
# attn L5 emits a mean output norm 152.62x the row replacing it, L6 73.98x, against 25.77x at the
# adjacent L4. §1806/§1807 showed the damage is the INPUT, not the output, and not a magnitude error.
# EVERY ONE OF THOSE IS AT LAYER GRANULARITY -- the whole `attn` module -- so none of them says which
# of the nine heads is responsible. This does.
#
# THE RECORD MAKES IT A SHARP QUESTION. §1089 certified L5H7 as a CONSTANT-BIAS head: "its whole
# 0.88-nat function is one fixed vector, and that vector sits on the massive-activation/gain dims";
# §437 measured `5.7 zero 0.916, mean -0.007, constant explains 101%`; its bias value is 86% of the
# stack's total, no other head above ~0.04. §1469 separately lists the sink-ish layers as a5, a13, a14,
# a10 -- three of which are in §1802's harmful-restoration set (L5, L6, L13, L10).
#
# TWO HYPOTHESES PREDICTING OPPOSITE HEADS.
#   (i) 5.7, the constant-bias/sink head. Its constancy is a property of its attention PATTERN (mass on
#       position 0), not of its output map. A context-free stream destroys the key/query geometry, the
#       softmax disperses, and a head that normally contributes a small fixed vector starts averaging
#       values from everywhere. This would also explain an otherwise odd pairing: §1807 measured attn
#       L5 and L6 at live/row ratios of 1.00 and 1.02, the two BEST row-matches in the stack, which is
#       exactly what a per-token table should achieve against a near-constant head. Best-modelled and
#       worst-exploding would be the same fact.
#   (ii) 5.5, the induction head (§398/§399 ladder + a4 relay; §1525 primary context-matcher). It is
#       the input-SENSITIVE head there, and a constant head is by construction the least input-
#       sensitive thing in the layer.
#
# METHOD. Hook c_proj's INPUT to get the concatenated pre-projection; head h's contribution is its
# HD-slice through the matching COLUMNS of c_proj.weight. Measured in the LIVE stream and in the fully
# compiled stream on identical positions.
#
# ROLES. skip7000 only -- one role suffices to IDENTIFY a head, and if one is implicated the other two
# roles belong in the confirmation run, not this one. Layers 4, 5, 6, 13. DISCOVERY ONLY.
#
...
#   pred_a IT IS ONE HEAD: a single head carries at least 50% of L5's total excess output norm in the
#          compiled stream. If FALSE the explosion is distributed and is a property of the LAYER's
#          input rather than of any head -- which makes BOTH hypotheses above wrong and points at the
#          residual-stream geometry rather than at any circuit.
#   pred_b AND IT IS 5.7: the top head at L5 is head 7. Scored separately from pred_a because "one head
#          dominates" and "it is the bias head" are independent claims, and 5.5 is a specific rival
#          rather than a null.
#   pred_c AND IT IS CONCENTRATED: at L5 the largest per-head compiled/live norm ratio is at least 10x
#          the median head's. If FALSE every head inflates comparably and the explosion is a uniform
#          rescaling of the layer, which no head-level story explains.
#   pred_d CONTROLS: the decomposition must be EXACT -- heads plus bias reconstruct the layer's write to
#          a relative residual below 1e-3 in BOTH streams, a known-answer check (LESSON 34) without
#          which no share means anything; and the layer-level compiled/live ratios must reproduce
#          §1804's ORDERING (L5 and L6 both inflate, and both more than L4). The ordering rather than
#          §1804's absolute numbers, because §1804 measured against the ROW norm and this measures
#          against the LIVE norm -- related but not the same quantity, and a tolerance on the wrong
#          quantity is LESSON 49's error. Coverage 5419 of 50257.
...
OUT = PT + 'ops/which_head_exploded_results.json'
PROBE_LS = ()
KIND_LS = ()
RANKS = (64,)                # the settled table rank; only the STREAM matters here
PROBE_LS = (4, 5, 6, 13)     # §1804's cliff layers, the adjacent control, the -2pp layer
...
def main():
    ...
    print(f'PARTIAL COMPILE, FOUR INSTRUMENTS | rank-64 tables x depths {DEPTHS} | '
    ...
    live_n, rec_l = head_norms(ev, [], 'live')
    comp_n, rec_c = head_norms(ev, hks, 'compiled')
    del ev, frr, hks
    torch.cuda.empty_cache()

    ratio = {k2: comp_n[k2] / max(live_n[k2], 1e-9) for k2 in live_n}
    print('\n  per-head mean output norm, live -> compiled (ratio):', flush=True)
    for L in PROBE_LS:
        print(f'    L{L}:', flush=True)
        for h in range(NH):
            print(f'      h{h}  {live_n[f"L{L}h{h}"]:9.3f} -> {comp_n[f"L{L}h{h}"]:12.3f}'
                  f'   {ratio[f"L{L}h{h}"]:9.2f}x', flush=True)
    excess = {L: {h: comp_n[f'L{L}h{h}'] - live_n[f'L{L}h{h}'] for h in range(NH)}
              for L in PROBE_LS}
    tot_ex = {L: sum(max(v, 0.0) for v in excess[L].values()) for L in PROBE_LS}
    share = {L: {h: max(excess[L][h], 0.0) / max(tot_ex[L], 1e-9) for h in range(NH)}
             for L in PROBE_LS}
    top = {L: max(range(NH), key=lambda h: share[L][h]) for L in PROBE_LS}
    r5 = sorted(ratio[f'L5h{h}'] for h in range(NH))
    med5 = r5[NH // 2]
    lay = {L: sum(comp_n[f'L{L}h{h}'] for h in range(NH))
              / max(sum(live_n[f'L{L}h{h}'] for h in range(NH)), 1e-9) for L in PROBE_LS}

    pa = share[5][top[5]] >= 0.50
    pb = top[5] == 7
    pc = max(r5) >= 10.0 * med5
    pd = (all(rec_l[f'L{L}'] < 1e-3 and rec_c[f'L{L}'] < 1e-3 for L in PROBE_LS)
          and all(lay[L] > 1.0 for L in (4, 5, 6))
          and lay[5] > lay[4] and lay[6] > lay[4] and NFULL == NCOV)

    print(f'\n  ONE head carries >=50% of L5 excess norm -> {pa}  top h{top[5]} at '
          f'{share[5][top[5]]:.1%}  (' + ' '.join(f'h{h}:{share[5][h]:.0%}'
                                                  for h in range(NH)) + ')', flush=True)
    print(f'  and it is head 5.7, the §1089 constant-bias head -> {pb}  observed h{top[5]}',
          flush=True)
    print(f'  the explosion is CONCENTRATED (max >=10x median) -> {pc}  max {max(r5):.1f}x '
          f'median {med5:.1f}x', flush=True)
    print(f'  decomposition exact, and L5/L6 inflate more than L4 -> control {pd}  '
          + '  '.join(f'L{L} {lay[L]:.2f}x' for L in PROBE_LS), flush=True)
    print(f'\npred_a {pa} | pred_b {pb} | pred_c {pc} | pred_d {pd}', flush=True)

    json.dump({'run': 'which_head_exploded', 'probe_layers': list(PROBE_LS),
               'NH': NH, 'HD': HD, 'live_norm': live_n, 'compiled_norm': comp_n,
               'ratio': ratio, 'layer_ratio': {f'L{L}': lay[L] for L in PROBE_LS},
               'excess_share': {f'L{L}': share[L] for L in PROBE_LS},
               'top_head': {f'L{L}': top[L] for L in PROBE_LS},
               'reconstruction_live': rec_l, 'reconstruction_compiled': rec_c,
               'predictions': {'pred_a_one_head_carries_half': bool(pa),
                               'pred_b_it_is_head_5_7': bool(pb),
                               'pred_c_concentrated': bool(pc),
