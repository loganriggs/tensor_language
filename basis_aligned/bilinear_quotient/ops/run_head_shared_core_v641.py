...
"""Change output-frame objective: calibrated interaction versus weight energy.
Closed-form covariance basis on8separate calibration documents,128tokens each.
Freeze frames before evaluation. Ranks32/128 and matched random baseline.
65forwards, no model updates; fitted basis is NOT weights-only discovery.
"""
...
OUT=BQ/'circuits/followups/head_shared_core_v641_result.json'
...
def main():
 plan=dict(frames=['weight_energy','calibration_interaction','random'],calibration_rows='skip80 rows96:104 length128',output_ranks=[32,128],edge_budgets=["dense"],input_rank=128,forwards_max=65,fit_parameters=1152*128,model_updates=0,model_backwards=0,execution_policy='managed_queue_only',
 ...
 # Separate calibration rows; no evaluation-family vectors enter this covariance.
 state['mode']='native';cache['layers']={}
 cal=torch.load(BQ/'.rowcache/fineweb_n480_skip80.pt',weights_only=True)[96:104,:129].cuda()
 model(cal[:,:-1],cal[:,1:].contiguous())
 h,m,e=[cache[k].double() for k in ['h','mean','remainder']]
 cross=((m@L.T)*(e@R.T)+(e@L.T)*(m@R.T))@D.T/(h.square().mean(-1,keepdim=True)+eps)
 Y=cross.reshape(-1,D.shape[0]);_,empirical=torch.linalg.eigh(Y.T@Y)
 gen=torch.Generator(device='cuda').manual_seed(641)
 random_basis=torch.linalg.qr(torch.randn(D.shape[0],128,device='cuda',dtype=torch.float64,generator=gen)).Q
 frames=dict(weight_energy=Wo.flip(1),calibration_interaction=empirical.flip(1),random=random_basis)
 ...
 for family,frame in frames.items():
  for output_rank in [32,128]:
   ...
   budget=output_rank*128*129//2
   candidates.append(dict(frame=family,output_rank=output_rank,budget=budget,W=W,program=None,
    unique_products=128*129//2,floating_values=W.numel()+P.numel()+budget,index_values=0,
    calibration_cross_error=float((Y-(Y@W)@W.T).norm()/Y.norm()),
    tensor_relative_error=max(0.,(total-float(core.square().sum()))/total)**.5,stats={}))
 coordinate_checks=[]
 ...
 stats={};write_checks=[];interaction_checks=[];forwards=1
 try:
 ...
 result=dict(plan=plan,stats=stats,candidates=candidates,maximum_coordinate_replay=max(coordinate_checks),predictions=predictions,maximum_write_replay=max(write_checks),maximum_interaction_replay=max(interaction_checks),
  ...
  scope='Data-informed output-frame control versus weights and random; calibration frames frozen before evaluation; opened prompts; conditional branch only')
 disk_guard.guard_write(1000000,label='v641');OUT.write_text(json.dumps(result,indent=2)+'\n');print(predictions)
