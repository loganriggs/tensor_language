"""GAUGE-FREE EIGENBASIS AT m16, v2 (rung 209): fixed exactness instrument; the S2305 quarantine lifter.

CONVENTION (S2135): per-position dCE = CE(m16 replaced, all else real) - CE(real model); LOWER IS BETTER.
Rung 207 (S2305) failed its exactness tripwire on a scale-dishonest metric (max rel err with a 1e-3
denominator floor over 295k entries) with fp16 eigenvector storage; its ratios (0.770 own, 0.655 census)
are quarantined. This v2: full-fp32 eigenvectors and scale-honest exactness metrics. Same arms:
NEUR = fixed top-1152/4608 neurons; EIG = top-1152 eigenfeatures of the invariant tensor (3x fewer values).
REGISTERED PREDICTIONS:
  (a) EXACTNESS CERTIFIES (fixed metric): relative Frobenius <= 1e-3 AND max-abs/output-RMS <= 1e-2.
  (b) EIG ADVANTAGE IS REAL, not an fp16/instrument artifact: own-circuit ratio EIG/NEUR <= 0.9.
  (c) GLOBAL advantage holds: census ratio <= 0.8.
NULL: the quarantined advantage was artifact (either ratio >= 0.95), or a genuine pipeline bug survives
the metric fix (pred_a fails again -> hunt the bug, publish nothing). PRICE: probe (one module).
Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: m16 eigenbasis v2, fixed exactness instrument'); raise SystemExit(0)
import torch
import torch.nn.functional as F
from bilin18_joint_removal import m, DEV
D=1152
PT='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
OUT=PT+'m16_eigenbasis_v2_results.json'
...
def main():
    ...
    pa=(frob<=1e-3 and mxs<=1e-2); pb=ra<=0.9; pc=rb<=0.8
    ...
    print(f"(a) EXACTNESS frob {frob:.2e} <= 1e-3 and max/RMS {mxs:.2e} <= 1e-2: {'HELD' if pa else 'FAILED'}")
    print(f"(b) EIG own-mean <= 0.9 x NEUR (ratio {ra:.3f}): {'HELD' if pb else 'FAILED'}")
    print(f"(c) EIG census <= 0.8 x NEUR (ratio {rb:.3f}): {'HELD' if pc else 'FAILED'}")
