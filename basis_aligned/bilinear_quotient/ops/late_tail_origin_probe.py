#!/usr/bin/env python
"""late_tail_origin_probe -- where the contextual tail comes from: each late MLP's input off the top-768 bus directions is split EXACTLY
into an early-origin part (the residual at block 8's entry propagated through the scalar lambda mixing with the embedding) and a
late-origin part (writes of blocks 8..l-1 plus block l's own attention); arms keep one origin's tail and replace the other's by its
fit-set mean, core exact. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_late_origin_tail_carries_the_cost pred_c_early_origin_tail_is_minor
#                     pred_d_own_attention_tail_is_minor pred_e_origins_complementary
...
Preregistration: polynomial_causal/LATE_TAIL_ORIGIN_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_tail_origin_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_tail_origin_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_TAIL_ORIGIN_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_tail_token_fill_probe_results.json"   # §2776
OUT = ROOT / "late_tail_origin_probe_results.json"
HASHES = {PREREG: "6283a2679e3fcfe6e253f5045966dde4ab2ef3122f75d1cd20df4897daa68da0", PRIOR: "5500188542036eaf6763b45788c71a92821c64427990d37a860f8940dfe122d1",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_tail_origin_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_frac": 0.60, "c_frac": 0.50, "d_frac": 0.30, "e_lo": 0.7, "e_hi": 1.5}
NULLS = {"b_frac": 0.30, "c_frac": 0.90, "d_frac": 0.60, "e_hi": 2.0}
...
def main():
    ...
    def perp(z):
        return z - (z @ Uk) @ Uk.T
    class Tracker:
        """Attention-site recorder for late block l: propagates the early-origin residual y (y_8 = x at block 8's entry;
        y_l = lam0 y + lam1 x0), and stores block l's attention write. Returns the write unchanged."""
        ...
        def __call__(self, w, x):
            if self.l == SPLIT:
                ...
                lam = m.transformer.h[self.l].lambdas; self.box["y"] = lam[0] * self.box["y"] + lam[1] * self.box["x0"]
            self.box["aw"] = w; self.box["xpre"] = x
            return w
    class OriginHead(OwnHead):
        """MLP l reading the core exactly; the tail (off top-768) keeps the chosen origin's component, the rest replaced by its fit-set mean.
        mode: 'early' keeps y/rms(x); 'late' keeps (x - y)/rms(x); 'ownattn' keeps aw/rms(x); 'dropown' keeps (x - aw)/rms(x)."""
        def __init__(self, l, mode, box, means):
            super().__init__(l, 768, "const", Uk); self.mode = mode; self.box = box; self.mean = means[(l, mode)]
        def __call__(self, w, x):
            xh = F.rms_norm(x, (D,))
            scale = xh.norm(dim=-1, keepdim=True) / x.norm(dim=-1, keepdim=True).clamp_min(1e-12)
            b = self.box
            if self.mode == "early": part = b["y"] * scale
            elif self.mode == "late": part = (x - b["y"]) * scale
            elif self.mode == "ownattn": part = b["aw"] * scale
            else: part = (x - b["aw"]) * scale
            xp_ = (xh @ Uk) @ Uk.T + perp(self.mx) + perp(part - self.mean)
            return self.mlp.Down(self.mlp.Left(xp_) * self.mlp.Right(xp_))
    # fit pass: means of each origin component (scaled to the normalised input) per late block, and tail energy shares
    acc = {(l, md): torch.zeros(D, dtype=torch.float64, device=DEV) for l in LATE for md in ("early", "late", "ownattn", "dropown")}
    en = {(l, md): 0.0 for l in LATE for md in ("early", "late", "full")}
    ntok = 0
    box = {}
    class MeanRec:
        ...
        def __call__(self, w, x):
            ...
            comps = {"early": box["y"] * scale, "late": (x - box["y"]) * scale, "ownattn": box["aw"] * scale, "dropown": (x - box["aw"]) * scale}
            for md, c in comps.items():
                acc[(self.l, md)] += c.reshape(-1, D).double().sum(0)
            for md in ("early", "late"):
                en[(self.l, md)] += float(perp(comps[md]).pow(2).sum())
            en[(self.l, "full")] += float(perp(xh).pow(2).sum())
            return w
    for i in range(0, fit_rows.shape[0], CH):
        idx = fit_rows[i:i + CH, :TI]
        pt = {("attn", l): Tracker(l, box) for l in LATE}; pt.update({("mlp", l): MeanRec(l) for l in LATE})
        for o in pt.values():
            o.hook(idx)
        forward(m, idx, patch=pt); ntok += idx.numel()
    means = {k_: (v / ntok).float() for k_, v in acc.items()}
    energy = {str(l): {"early_share": en[(l, "early")] / en[(l, "full")], "late_share": en[(l, "late")] / en[(l, "full")]} for l in LATE}
    log(stage="origin_energy", early=round(sum(e["early_share"] for e in energy.values()) / len(LATE), 3), late=round(sum(e["late_share"] for e in energy.values()) / len(LATE), 3))
    def origin(mode):
        bx = {}
        patch = {("attn", l): Tracker(l, bx) for l in range(SPLIT, NL)}
        patch.update({("mlp", l): OriginHead(l, mode, bx, means) for l in range(SPLIT, NL)})
        return run(patch)
    arms = {"SPLIT8_1024": split8(1024), "LATE_MLP_768": run({("mlp", l): head(("mlp", l), 768, U_8) for l in LATE}),
            "EARLY_TAIL_ONLY": origin("early"), "LATE_TAIL_ONLY": origin("late")}
    if not smoke:
        arms["DROP_OWN_ATTN_TAIL"] = origin("dropown"); arms["OWN_ATTN_TAIL_ONLY"] = origin("ownattn")
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    C0 = arms["LATE_MLP_768"]; E_, L_ = arms["EARLY_TAIL_ONLY"], arms["LATE_TAIL_ONLY"]; Dn = arms.get("DROP_OWN_ATTN_TAIL", E_)
    summ = {"early_tail_only_over_const": E_ / C0, "late_tail_only_over_const": L_ / C0, "drop_own_attn_over_const": Dn / C0,
            "own_attn_only_over_const": arms.get("OWN_ATTN_TAIL_ONLY", float("nan")) / C0, "sum_origins_over_const": (E_ + L_) / C0,
            "mean_early_energy_share": sum(e["early_share"] for e in energy.values()) / len(LATE), "mean_late_energy_share": sum(e["late_share"] for e in energy.values()) / len(LATE)}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(C0 - PRIOR_LATE_MLP_768) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_late_origin_tail_carries_the_cost': bool(E_ >= BARS["b_frac"] * C0),
        'pred_c_early_origin_tail_is_minor': bool(L_ <= BARS["c_frac"] * C0),
        'pred_d_own_attention_tail_is_minor': bool(Dn <= BARS["d_frac"] * C0),
        'pred_e_origins_complementary': bool(BARS["e_lo"] <= (E_ + L_) / C0 <= BARS["e_hi"]),
    }
    nulls = {"b_null_early_only_le_.3": bool(E_ <= NULLS["b_frac"] * C0), "c_null_late_only_ge_.9": bool(L_ >= NULLS["c_frac"] * C0),
             "d_null_drop_own_ge_.6": bool(Dn >= NULLS["d_frac"] * C0), "e_null_sum_ge_2": bool((E_ + L_) / C0 >= NULLS["e_hi"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "blocks 8-17 MLP reads: core (top-768 of U_8) exact; tail keeps one origin (early-residual / late writes / own attention) and replaces the other by its fit-set mean",
           ...
           "ce_added": arms, "tail_energy_by_origin": energy, "summary": summ,
