#!/usr/bin/env python3
"""Zero-fit v12 transfer of the frozen auxiliary post-cue value source."""

# BQGATE: EXPERIMENT pred_a_authority_partition_value_replay_finiteness_and_price pred_b_frozen_postcue_source_is_sufficient pred_c_source_program_is_panel_stable pred_d_both_auxiliary_layers_have_material_value_reads pred_e_zero_fit_exact_arm_inventory
...
PRIOR = ROOT / "circuits/prior_art/iswas_mlp8_auxiliary_postcue_value_source_v12_transfer_v1.json"
...
OUT = ROOT / "circuits/followups/iswas_mlp8_auxiliary_postcue_value_source_v12_transfer_v1_result.json"
CANDIDATE_ID = "cross_task.iswas_mlp8_auxiliary_postcue_value_source_v12_transfer_v1"
RESULT_SCHEMA = "iswas_mlp8_auxiliary_postcue_value_source_v12_transfer_result_v1"
EXPECTED = {
    "prior": "28867adef8049883865b0bf58c16c932359eb633f7448f84f16816d390f8ed79",
...
MAX_FORWARDS, MAX_EVALUATIONS = 11, 330
...
def main() -> None:
    paths = {"prior": PRIOR, "parent": PARENT, "parent_runner": PARENT_RUNNER,
             "capability": CAPABILITY, "builder": BUILDER, "source_screen": SOURCE_SCREEN,
    ...
    source_screen = json.loads(SOURCE_SCREEN.read_text())
    ...
    if (prior.get("candidate_id") != CANDIDATE_ID or parent.get("terminal") != "screen"
            or source_screen.get("terminal") != "screen" or len(rows) != 30):
    ...
    value_closure, outside_causal_unscored = 0.0, 0.0
    for layer in LAYERS:
        ...
        difference = (all_value[layer] - expected_value).abs()
        for i, row in enumerate(rows):
            query = int(row["base_semantic_position"])
            selected = list(SELECTED[layer])
            value_closure = max(value_closure, float(difference[i, :query + 1, selected].max()))
            if query + 1 < difference.shape[1]:
                outside_causal_unscored = max(
                    outside_causal_unscored, float(difference[i, query + 1:, selected].max()))
    ...
    frozen = "post_cue_before_subject"
    identity_error = max(float((base_hidden18 - base18).abs().max()), float((self18 - base18).abs().max()))
    complete_core_logit_replay = float((das.head_logits(backend, states["complete_all_factors"])
        - das.head_logits(backend, core18)).abs().max())
    ...
    pred_a = bool(orientation_error <= 1e-6 and reconstruction <= .001 and value_closure <= .001
                  and complete_core_logit_replay <= .001 and identity_error <= .001 and finite
                  and forwards <= MAX_FORWARDS and evaluations <= MAX_EVALUATIONS)
    pred_b = all(metrics[p][frozen]["absolute_behavior_fraction_of_all_value"] >= .75
        and metrics[p][frozen]["q8_norm_fraction_of_all_value"] >= .75
        and metrics[p][frozen]["behavior_cosine_to_all_value"] >= .95
        and metrics[p][frozen]["q8_cosine_to_all_value"] >= .95 for p in ("A1", "A2"))
    pred_c = all(abs(metrics["A1"][frozen][key] - metrics["A2"][frozen][key]) <= .15
    ...
    result = {"schema": RESULT_SCHEMA,
        ...
        "instrument": {"f_linear_orientation_max_abs": orientation_error,
            ...
            "value_source_causal_closure_max_abs": value_closure,
            "outside_causal_unscored_max_abs": outside_causal_unscored,
            "complete_factor_to_live_core_full_logit_max_abs": complete_core_logit_replay,
            "native_self_clamp_max_abs": identity_error, "rows": len(rows)},
        "frozen_source_program": frozen, "metrics": metrics,
    ...
    print(json.dumps({key: result[key] for key in ("candidate_id", "instrument",
        "frozen_source_program", "metrics", "predictions", "terminal", "price")}, sort_keys=True))
