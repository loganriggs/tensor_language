"""PATH DECOMPOSITION (rung 284): direct weight-slice matmuls inside qkz - factor arithmetic vs recompute structure.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2381/S2382: the full-rank factor
path costs +0.0520 (bit-exact across both builds). This run keeps the SAME recompute structure (fp32
rotary/rms/einsum, zeroed-head layout) but computes Q/K/Q2/K2 by ONE direct matmul against the original
weight slices instead of two factor matmuls.
REGISTERED PREDICTIONS:
  (a) FACTOR ARITHMETIC WAS THE CULPRIT: census <= 0.01.
  (b) valid >= 12.
  (c) SANITY: census in [0.0, 0.12]; cev saved (cev_pathdirect.pt).
NULL: the RECOMPUTE STRUCTURE carries it (census >= 0.04) - the fix then lives in matching the native
bf16 op order/precision, not the factorization. PRICE: probe. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: path decomposition (direct weights)')
    raise SystemExit(0)
import torch
...
OUT=PT+'path_direct_results.json'
    ...
    if SEL.get('qk_r'):
        ...
        def qkz(at,X2,li,vm):
            ...
            for hd in SEL['_QKR'][li]:
                Xf=X2.float()
                Q[:,:,hd]=Xf@at.c_q.weight[hd*128:(hd+1)*128,:].float().T
                K[:,:,hd]=Xf@at.c_k.weight[hd*128:(hd+1)*128,:].float().T
                Q2[:,:,hd]=Xf@at.c_q2.weight[hd*128:(hd+1)*128,:].float().T
                K2[:,:,hd]=Xf@at.c_k2.weight[hd*128:(hd+1)*128,:].float().T
            cos,sin=at.rotary(Q)
            qn=F.rms_norm(Q,(128,)); kn=F.rms_norm(K,(128,))
            qn,kn=are(qn,cos,sin),are(kn,cos,sin)
            q2n=F.rms_norm(Q2,(128,)); k2n=F.rms_norm(K2,(128,))
...
if __name__=='__main__':
    ...
    cev=SEL['cev']
    torch.save(cev,PT+'cev_pathdirect.pt')
    print('cev saved: cev_pathdirect.pt',flush=True)
