...
"""pred_a folded native producer and downstream attention replay<=1e-5;
pred_b native capability allfamilies plusregional first-transfer>=2%native gap;
pred_c eachcontrol changeRMS<=5%native marginRMS,meanabs<=25%regional effectRMS,no capable flips.
Null: the branch may not transfer in fresh contexts or may alter control behaviors.
"""
...
STEM='REGIONAL_BEHAVIOR_CONTROLS_V2'
@torch.no_grad()
def main():
    ...
    arms=torch.stack([margins(writes[:,j]-writes[:,0]) for j in range(2)])
    reg_uk=[i for i,row in enumerate(rows) if row['family']==0 and row['cue']=='British'];reg_us=[i+1 for i in reg_uk]
    nativegap=arms[0,reg_uk,0]-arms[0,reg_us,0];transfer=(nativegap-arms[1,reg_uk,0]+arms[1,reg_us,0])/2
    effects=arms[1,:,0]-arms[0,:,0];regional_ids=reg_uk+reg_us;regrms=effects[regional_ids].square().mean().sqrt();cells=[]
    for family in (1,2,3):
        ids=[i for i,row in enumerate(rows) if row['family']==family];baseline=arms[0,ids,0];edited=arms[1,ids,0];effect=effects[ids];capable=baseline>0
        cells.append(dict(family=family,name=rows[ids[0]]['family_name'],native_margin_mean=float(baseline.mean()),native_margin_rms=float(baseline.square().mean().sqrt()),native_positive=int(capable.sum()),control_effect_rms=float(effect.square().mean().sqrt()),control_effect_meanabs=float(effect.abs().mean()),capable_flips=int((capable&(edited<=0)).sum()),per_prefix_native=baseline.tolist(),per_prefix_edited=edited.tolist()))
    instrument=max(producer_errors+replays)<=1e-5
    capability=instrument and float(nativegap.mean())>=.5 and int((nativegap>0).sum())>=3 and all(c['native_margin_mean']>=.5 and c['native_positive']>=6 for c in cells)
    positive=capability and float(transfer.mean())>=.02*float(nativegap.mean()) and int((transfer>0).sum())>=3
    result={'pred_a':instrument,'pred_b':positive,'pred_c':positive and all(c['control_effect_rms']<=.05*c['native_margin_rms'] and c['control_effect_meanabs']<=.25*float(regrms) and c['capable_flips']==0 for c in cells)}
    torch.save(dict(pre=pre.cpu(),write_vertices=writes.cpu(),arm_margins=arms.cpu()),art)
    result.update(capability_pass=capability,native_regional_gap_mean=float(nativegap.mean()),native_regional_gaps=nativegap.tolist(),regional_transfer_mean=float(transfer.mean()),regional_transfers=transfer.tolist(),regional_effect_rms=float(regrms),producer_replay_errors=producer_errors,native_attention_replay_errors=replays,control_cells=cells,body_forwards=count,seconds=time.perf_counter()-tic,artifact_sha=digest(art),source_shas=binding,scope='Fresh regional positive and three behavior control families. First-token branch computed from embeddings and frozen H maps with native routing/background fixed; no contextual first-value capture used. Interchange, not complete component removal or independent extraction.')
