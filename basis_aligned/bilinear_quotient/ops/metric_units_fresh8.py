"""FRESH-PILE CERTIFICATION of the metric-unit gain on the eight document-disjoint windows of S2083 (rung 22).
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['stream_error_profile_results.json','attn5_head_price_ladder_results.json','probe_gate7_results.json']
    ...
    print(f"DRYRUN OK: S2086 present (block 6 {_p['6']}); two arms x eight fresh pile-10k windows (+ window 1 reproduction)")
    raise SystemExit(0)
import json, sys, time, torch
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
import os
...
OUT=PT+'metric_units_fresh8_results.json'
...
def main():
    ...
    for wi,w in enumerate(WIN):
        ...
        rows.append({'window':wi,'docs':w['docs'],'base_ce':round(b,5),'gap_plain':round(g['plain'],5),
                     'gap_metric':round(g['metric-units'],5),'gain':round(g['plain']-g['metric-units'],5)})
    ...
    gains=[r['gain'] for r in rows]; gaps=[r['gap_plain'] for r in rows]
    import statistics as stt
    pa=sum(g>0 for g in gains)>=7
    pb=stt.median(gains)>=0.04
    pc=stt.pstdev(gains)<=0.5*stt.pstdev(gaps)
    gain1=gap1['plain']-gap1['metric-units']
    pd=abs(gain1-0.124)<=0.02 and abs(arms['plain']['profile'][6]-1.7415)<=0.10
    out={'windows':rows,'gain_median':round(stt.median(gains),4),'gain_mean':round(stt.mean(gains),4),
         'gain_sd':round(stt.pstdev(gains),4),'gap_sd':round(stt.pstdev(gaps),4),'windows_positive':int(sum(g>0 for g in gains)),
         'window1_gain':round(gain1,4),'window1_gap_plain':round(gap1['plain'],4),'profiles':{k:v['profile'] for k,v in arms.items()},
         'pred_a_positive_on_7_of_8':bool(pa),'pred_b_median_gain':bool(pb),'pred_c_gain_more_stable_than_gap':bool(pc),
         'pred_d_reproduction':bool(pd),'self_reviewed':True,'runtime_s':round(time.time()-t0,1)}
    json.dump(out,open(OUT,'w'),indent=1)
    print(f"(d) window-1 gain {gain1:+.4f} vs 0.124; cfgE b6 {arms['plain']['profile'][6]:.4f}: {'HELD' if pd else 'FAILED'}")
    print(f"(a) positive on {out['windows_positive']}/8 (bar 7): {'HELD' if pa else 'FAILED'}")
    print(f"(b) median gain {out['gain_median']:+.4f} >= 0.04: {'HELD' if pb else 'FAILED'}")
    print(f"(c) gain sd {out['gain_sd']:.4f} <= 0.5 x gap sd {out['gap_sd']:.4f}: {'HELD' if pc else 'FAILED'}")
