...
def main():
    ...
    result={"schema":"temporal_iswas_two_mode_weight_pullback_result_v1","candidate_id":CANDIDATE_ID,"execution_policy":"managed_queue_only","started_utc":started_utc,"finished_utc":utc_now(),"serial_seconds":time.perf_counter()-started,"authority_sha256":EXPECTED,"parent_authority_sha256":parent.EXPECTED,"dryrun":dryrun,"instrument":{"orientation_max_abs":orientation_error,"attention_reconstruction_max_abs":reconstruction,"capture_identity_max_abs":capture_identity,"q8_factor_closure_max_abs":q8_closure,"rank2_exact_response_relative_error":replay_rel,"orthogonal_gauge_projector_max_abs":gauge_error},"mode_artifacts":{"singular_values":[float(x) for x in modes["singular_values"][:8]],"physical_source_covectors_sha256":tensor_sha(modes["physical_source_covectors"]),"physical_reader_covectors_sha256":tensor_sha(modes["physical_reader_covectors"]),"source_coordinates":modes["source_coordinates"].cpu().tolist(),"reader_coordinates":modes["reader_coordinates"].cpu().tolist()},"known_components":known,"rank_correlations":rank_correlations,"upstream_rankings":upstream,"downstream_rankings":downstream,"predictions":predictions,"terminal":terminal,"price":PRICE}
