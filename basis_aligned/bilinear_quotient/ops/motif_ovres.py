"""MOTIF OV RESIDUAL, RANK-8 (rung 178; phase-2 opener): attacking the motif structural ceiling type-legally.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The motif family's +0.55 solo census is
the largest structural cost with no lever: alpha refit is a no-op (S2220), class-alpha nets +0.01 (S2244).
Untried: capacity in the VALUE MAP - each motif head becomes z = alpha.v + v @ R8 with R8 a rank-8 per-head
residual (fit by ridge to the real z on the SAME real-stream captures as alpha, SVD-truncated; a fitted-
full-output site, so the type system permits it; frame-insensitivity assumed per S2220's alpha result,
stated). Everything else = the exact rung-127 frontier build. li default-bound (S2238 rule).

REGISTERED PREDICTIONS (anchors: census +1.9474; subword +2.319; rep +1.708):
  (a) THE CEILING MOVES: census <= 1.87 (>= 0.075; 200x the parameter capacity of class-alpha).
  (b) MOTIF-OWNED CLASSES MOVE: subword <= 2.15 AND rep <= 1.55.
  (c) SANITY: increment in [0.25, 0.45] AND census >= 1.4.
NULL: the damage is in the ATTENTION PATTERN, not the value map - any per-head v->z map is capacity-futile
(gain < 0.02), and the motif ceiling is pattern-structural. PRICE: 76 heads x 2 x 128 x 8 = 155,648 values.
Tripwire: INSTRUMENT FAIL if census within 1e-3 of baseline. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: motif OV residual rank-8')
    raise SystemExit(0)
import torch
...
OUT=PT+'motif_ovres_results.json'
...
if __name__=='__main__':
    ...
    pa=agg<=1.87
    pb=_sw<=2.15 and PC.get('rep',9.9)<=1.55
    ...
    print(f"(b) subword {_sw:+.4f} <= 2.15 and rep {PC.get('rep',9.9):+.4f} <= 1.55: {'HELD' if pb else 'FAILED'}")
