...
p=prior.p
ROWS=prior.ROWS; PREREG=POLY/'INDUCTION_EARLY_MLP_SUFFIX_VJP_NULL_FACTOR_V2_PREREGISTRATION.md'; BINDING=POLY/'INDUCTION_EARLY_MLP_SUFFIX_VJP_NULL_FACTOR_V2_BINDING.json'; OUT=POLY/'INDUCTION_EARLY_MLP_SUFFIX_VJP_NULL_FACTOR_V2_RESULT.json'
...
def suffix_vjp(runtime,tokens,specs,target):
 ...
 with torch.enable_grad():
  logits=runtime.facade.forward_with_dispatch(runtime.model,token_tensor,attention,mlp,require_production=False); finals=torch.as_tensor([int(x['final_position']) for x in specs],device=device); answers=torch.as_tensor([int(x['recipient_answer_id']) for x in specs],device=device); selected=logits[torch.arange(len(specs),device=device),finals]; loss=runtime.functional.cross_entropy(selected,answers,reduction='sum'); gradients=torch.autograd.grad(loss,captured)
 axes=[]
...
def main():
 ...
 try:
  ...
  for start in range(0,len(rows),BATCH):
   stop=start+BATCH; logits,axis,act,obs=suffix_vjp(runtime,tokens[start:stop],specs[start:stop],donor_terms[start:stop]); joint_batches.append(logits); axis_batches.append(axis); joint_active.append(act); observations.append(obs)
