# HOW DOES THE RANK LADDER MOVE WITH COVERAGE?  -- §1851's implicit question, made explicit.
#
# §1851 confirmed §1848 second class: the full-rank composition attains the model's own per-token ceiling
# EXACTLY at 5,419 covered types and again at 16,110 (2.97x), on all three roles. But it also found the
# rank-64 shortfall RISES with coverage, +0.22327/+0.23252/+0.19774 at 5,419 becoming
# +0.27839/+0.29741/+0.25327 at 16,110 -- more token types compressed into the same rank-64 basis. That
# scoped §1850's cost-per-nat frontier to the covered set it was measured on.
#
# §1851 measured ONE rung of the ladder (rank 64) at the larger coverage. This measures ALL of it --
# ranks 256, 64, 16 and 4 -- so the SHAPE of the shift is visible rather than a single displacement, and
# §1850's frontier can be restated at a second coverage instead of merely being scoped away from one.
#
# ROLES. All three. Ranks full and 64. DISCOVERY ONLY.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39,
# anchors named with their BUILD per LESSONS 53's addendum, and the population caveat §1851's pred_d
# tripped over is now honoured: NOTHING here is anchored to a figure measured on a different covered set.
#   pred_a ATTAINMENT IS COVERAGE-INDEPENDENT: the full-rank covered CE equals the ceiling to under 0.001
#          nats at ALL THREE coverages, on all three roles. If TRUE §1765's derivation is confirmed at
#          three coverages spanning a factor of ~3 in token types and the class result is settled. If
#          FALSE attainment depends on how much was fitted, which would contradict a derivation that has
#          held twice, and the derivation would be the thing to re-examine.
#   pred_b AND THE COVERED SET IS THE LARGE ONE: 16,110 types, at least 20% above §1834's 5,419 -- the
#          known-answer check that this run is measuring the object it claims to (LESSON 46).
#   pred_c AND THE LADDER IS STILL MONOTONE IN RANK at this coverage, as §1849 found at 5,419. If FALSE
#          the rank axis stops being a clean ladder once the covered set is large, and §1850's frontier
#          would not merely shift with coverage but change shape -- a stronger scoping than §1851's.
#   pred_d CONTROLS: coverage rises strictly with the number of fit rows; the ceiling is finite and above
#          live at every coverage; live covered CE is REPORTED per coverage and not anchored across them,
#          since the scored population changes with the covered set -- the exact bar §1851's pred_d got
#          wrong after its own docstring warned of it.
...
OUT = PT + 'ops/ladder_at_high_coverage_results.json'
...
TRANKS = (None, 256, 64, 16, 4)          # None = full rank; §1786/§1813 use this ladder
...
S1849_LADDER_5419 = {'256': {'skip7000': 0.06580, 'skip11000': 0.06603, 'skip1200': 0.05599},
                     '64': {'skip7000': 0.22327, 'skip11000': 0.23252, 'skip1200': 0.19774},
                     '16': {'skip7000': 0.42066, 'skip11000': 0.44399, 'skip1200': 0.38663},
                     '4': {'skip7000': 0.70393, 'skip11000': 0.74134, 'skip1200': 0.67697}}
...
FIT_ROWS = PT + '.rowcache/fineweb_n480_skip80.pt'   # 5x the fit rows -> a larger covered set
H = m.transformer.h
NCOV = 5419
...
def main():
    ...
    assert NFULL > 1.20 * NCOV, f'coverage {NFULL} did not grow 20% over {NCOV}'
    ...
    print(f'LADDER AT HIGH COVERAGE | 16,110 covered types, ranks full/256/64/16/4 | '
    ...
    pa = all(abs(curve['full'][e]['cov'] - ceilv[e]) < 0.001 for e in roles2)
    pb = NFULL >= 1.20 * NCOV
    lad = ['4', '16', '64', '256', 'full']
    above = {l2: {e: curve[l2][e]['cov'] - ceilv[e] for e in roles2} for l2 in lad}
    rise = {l2: {e: above[l2][e] - S1849_LADDER_5419[l2][e] for e in roles2} for l2 in lad[:-1]}
    pc = all(above[lad[i]][e] <= above[lad[i - 1]][e] + 1e-9
             for e in roles2 for i in range(1, len(lad)))
    pd = (all(abs(livev[e] - S1768_LIVE_COVERED[e]) <= 0.01 for e in r64)
          and all(ceilv[e] > livev[e] for e in roles2))

    print(f'\n  COVERED CE above the ceiling, by table rank:', flush=True)
    for lbl in lad[::-1]:
        print(f'    rank {lbl:5s} ' + '  '.join(
            f'{e} +{curve[lbl][e]["cov"] - ceilv[e]:.5f}' for e in roles2), flush=True)
    print(f'\n  ATTAINMENT survives a larger covered set (<0.001) -> {pa}  ' + '  '.join(
        f'{e} {curve["full"][e]["cov"] - ceilv[e]:+.6f}' for e in roles2), flush=True)
    print(f'  and coverage actually GREW by >=20% -> {pb}  {NFULL} types vs §1834\'s {NCOV}'
          f' ({NFULL / NCOV:.2f}x)', flush=True)
    print(f'\n  LADDER at {NFULL} types vs §1849 at {NCOV} (CE above the ceiling):', flush=True)
    for l2 in lad[::-1]:
        if l2 == 'full':
            print(f'    rank {l2:5s} ' + '  '.join(
                f'{e} {above[l2][e]:+.6f}' for e in roles2), flush=True)
        else:
            print(f'    rank {l2:5s} ' + '  '.join(
                f'{e} +{above[l2][e]:.5f} (§1849 +{S1849_LADDER_5419[l2][e]:.5f}, '
                f'rise {rise[l2][e]:+.5f})' for e in roles2), flush=True)
    print(f'  and the ladder is MONOTONE in rank at this coverage -> {pc}', flush=True)
    print(f'  ceiling/live §1768, full-rank all-position §1811 -> control {pd}', flush=True)

    json.dump({'run': 'ladder_at_high_coverage', 'n_covered': NFULL, 'ranks': [str(r) for r in TRANKS],
               'ceiling_covered': ceilv, 'live_covered': livev, 'curve': curve,
               'ladder_above_ceiling': above, 'rise_vs_S1849': rise,
               'predictions': {'pred_a_attainment_survives_coverage': bool(pa),
                               'pred_b_coverage_grew': bool(pb),
                               'pred_c_ladder_monotone_at_high_coverage': bool(pc),
