#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_four_verb_readers_add pred_c_four_plus_copier_closes_half pred_d_full_named_set_closes_most pred_e_all_values_replay
"""The verb readout's full named set (v452). v448 / v451: the verb readout's heads from the noun are 11.3 (0.147), 7.8 (0.102), 13.1 (0.045), 9.7 (0.039);
the copier 4.5 (0.138) is shared with the pronoun readout; the pronoun readers add 0.072. Here at the noun, on the 79 distant rows at the verb slot: the four
verb readers (V4); V4 + copier; V4 + copier + the five pronoun readers (the full named set); and every head's value at the noun (the census total, 1.03).
PREDICTIONS (scored as written; failures preserved; priors from v446-v451)
    pred_a_baseline_replays          the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_four_verb_readers_add     V4 is within 0.05 of 0.147 + 0.102 + 0.045 + 0.039 = 0.333
    pred_c_four_plus_copier_closes_half  V4 + copier closes >= 0.45
    pred_d_full_named_set_closes_most  the full named set closes >= 0.52 (leaving <= half of the verb margin to the small exits). Prior: unsure
    pred_e_all_values_replay         every head's value at the noun closes within 0.05 of 1.029 (v446)
...
OUT = ROOT / "circuits/followups/verb_full_named_set_v452_result.json"
CANDIDATE_ID = "chain.verb_full_named_set_v452"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, V4_SUM, ADD_TOL, HALF_MIN, FULL_MIN, ALL_V446 = 1e-3, 0.333, 0.05, 0.45, 0.52, 1.029
...
FORWARDS_MAX = 16
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_four_verb_readers_add": "0.333 +- 0.05", "pred_c_four_plus_copier_closes_half": ">= 0.45", "pred_d_full_named_set_closes_most": ">= 0.52", "pred_e_all_values_replay": "1.029 +- 0.05"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "v4_sum": V4_SUM, "add_tol": ADD_TOL, "half_min": HALF_MIN, "full_min": FULL_MIN, "all_v446": ALL_V446}}
    ...
    V4 = {11: [3], 7: [8], 13: [1], 9: [7]}; FULL = {l: sorted(set(V4.get(l, []) + FIVE.get(l, []))) for l in set(V4) | set(FIVE)}; FULL[4] = [5]; ALL_H = {l: list(range(N_HEAD)) for l in range(18)}
    MODES = {"V4": (V4, (0,)), "V4_C": ({**V4, 4: [5]}, (0,)), "FULL": (FULL, (0,)), "ALL_NOUN": (ALL_H, (0,))}; forwards = 0
    def logits_margin(x):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_four_verb_readers_add": abs(v["V4"] - V4_SUM) <= ADD_TOL, "pred_c_four_plus_copier_closes_half": v["V4_C"] >= HALF_MIN,
                   "pred_d_full_named_set_closes_most": v["FULL"] >= FULL_MIN, "pred_e_all_values_replay": abs(v["ALL_NOUN"] - ALL_V446) <= ADD_TOL}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "verb_full_named_set_result_v452", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
