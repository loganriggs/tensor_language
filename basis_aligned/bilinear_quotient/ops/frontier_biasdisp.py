"""BIAS-DISPERSION DECOMPOSITION OF CERTIFICATE FAILURE (rung 150; math review 1307).

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The paradox this rung resolves: oracle
stream fidelity HALVES census damage (S2241: 1.9474 -> 0.9400) yet certificates stay 0/62. Statistical
decision theory splits a circuit's member mean|dCE| into BIAS (the signed member mean) and DISPERSION
(everything the absolute value adds). S2209's quadrature story predicts dispersion dominance: orthogonal
drifts add per-position variance everywhere. Two census evals in one build (base and all-class trajectory
splice), per-circuit signed and absolute member means for both.

REGISTERED PREDICTIONS:
  (a) DISPERSION-DOMINATED: median over circuits of |signed member mean| / member abs-mean <= 0.5 (base arm).
  (b) DISPERSION IS STICKY: median per-circuit abs-mean improvement under the splice <= 0.8 x the census
      improvement factor.
  (c) REPRODUCTION: base census within 0.015 of +1.9474 AND spliced within 0.015 of +0.9400.
NULL: BIAS-DOMINATED (median ratio > 0.5) - then per-circuit bias correction (~62 stored scalars) becomes
the cheapest certificate candidate ever proposed. Either branch redirects the direction chapter. PRICE:
none. Tripwire: INSTRUMENT FAIL if base cev is bitwise equal to spliced. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: bias-dispersion decomposition')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_biasdisp_results.json'
...
def main():
    ...
    if SEL.get('clsdmg'):
        ...
        if SEL.get('bias_disp'):
            SEL['cev_base']=evalV(_ROWS,_ROWS.shape[0],order2,ML).detach().cpu()
        if SEL.get('name_traj'):
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['name_traj']=True; SEL['traj_all']=True; SEL['bias_disp']=True
    print('ARM: base + spliced census evals; per-circuit signed and abs member means',flush=True)
    main()
    ...
    dB=SEL['cev_base'].cpu()-CBASE
    aggB=float(dB.mean())
    if torch.equal(SEL['cev_base'].cpu(),cev.cpu()):
        raise SystemExit('INSTRUMENT FAIL: base cev bitwise equal to spliced')
    RAT=[]; FIMP=[]
    for t,v in BATC.items():
        try: lf=CN.leaf(t)
        except Exception: continue
        mm=torch.zeros(NFLAT,dtype=torch.bool); mm[lf['member']]=True
        if mm.sum()==0: continue
        ms=float(dB[mm].mean()); ma=float(dB[mm].abs().mean())
        maS=float(d[mm].abs().mean())
        RAT.append(abs(ms)/max(ma,1e-9)); FIMP.append(ma/max(maS,1e-9))
    import statistics as stt
    medR=stt.median(RAT); medF=stt.median(FIMP)
    Fc=aggB/max(agg,1e-9)
    print(f'  median |signed|/abs ratio {medR:.3f}; median circuit abs-improvement {medF:.3f}x vs census {Fc:.3f}x',flush=True)
    pa=medR<=0.5
    pb=medF<=0.8*Fc
    pc=abs(aggB-1.9474)<=0.015 and abs(agg-0.9400)<=0.015
    ...
    res={'L2F_fresh':round(L2F,4),'L1F':round(L1F,4),'increment':round(inc,4),'census_agg':round(agg,4),
         ...
         'pred_a_dispersion_dominated':bool(pa),'pred_b_dispersion_sticky':bool(pb),'pred_c_repro':bool(pc),
         'median_bias_ratio':round(medR,4),'median_circuit_improvement':round(medF,4),'census_factor':round(Fc,4),'census_base':round(aggB,4),
    ...
    print(f"(a) median bias ratio {medR:.3f} <= 0.5: {'HELD' if pa else 'FAILED'}")
    print(f"(b) median circuit improvement {medF:.3f}x <= 0.8 x census factor ({0.8*Fc:.3f}x): {'HELD' if pb else 'FAILED'}")
    print(f"(c) base {aggB:+.4f} ~ 1.9474, spliced {agg:+.4f} ~ 0.9400: {'HELD' if pc else 'FAILED'}")
