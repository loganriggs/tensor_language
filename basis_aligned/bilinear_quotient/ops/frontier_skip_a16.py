...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_tail_prefix_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; full vs skip-a16 arms on the S2144 best middles (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_skip_a16_results.json'
...
if __name__=='__main__':
    ...
    r_f=main()
    SEL['skip16']=True
    print('ARM 2/2: same, with attn16 left REAL (a16L skipped, a17L refit under it)',flush=True)
    r_s=main()
    import statistics as stt
    d=[round(a-b,4) for a,b in zip(r_s['fresh8'],r_f['fresh8'])]
    md=stt.median(d)
    pa=md<=-0.10
    pb=sum(g<=-0.05 for g in d)>=7
    pc=abs(r_f['L2_F']-2.6662)<=0.01
    res={'full':r_f,'skip_a16':r_s,'per_window_skip_minus_full':d,
         'convention':'L2 = CE above the real model; lower is better; negative = leaving attn16 real saves damage',
         'coverage_note':'the skip arm replaces one fewer component - envelope point, not a free improvement',
         'pred_a_saving_survives':bool(pa),'pred_b_windows':bool(pb),'pred_c_reproduces_s2144':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'skip16 - full per window: {d}  median {md:+.4f}')
    print(f"(a) median {md:+.4f} <= -0.10: {'HELD' if pa else 'FAILED'}")
    print(f"(b) <= -0.05 on {sum(g<=-0.05 for g in d)}/8 (bar 7): {'HELD' if pb else 'FAILED'}")
