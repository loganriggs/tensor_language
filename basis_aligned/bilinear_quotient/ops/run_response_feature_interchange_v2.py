...
Replay<1e-5; response-trained error error<.10, <=1.1narrow26 and<=.8value656.
...
def main():
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
  ...
  torch.manual_seed(81);x=torch.randn(7,3,dtype=torch.float64);program=dict(U=torch.randn(2,2,3,dtype=torch.float64),V=torch.randn(2,2,3,dtype=torch.float64),C=torch.randn(4,3,dtype=torch.float64));programs={k:program for k in ['narrow26','value656','response656']}
 ...
 out=P/'RESPONSE_FEATURE_INTERCHANGE_V2.json';assert not out.exists();start=time.monotonic()
