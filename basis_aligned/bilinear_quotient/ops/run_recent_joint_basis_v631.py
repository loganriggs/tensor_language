...
Ranks512/1152; add independent CPU float64 SVD; include full-rank recovery in every family. No fitting.
Select using calibration only. Dense fallback cannot pass compressed validation.
16 native forwards,144 local replays. No package export for a failed baseline.
"""
import json,os,sys,time,hashlib
from pathlib import Path
from datetime import datetime,timezone
ROOT=Path(__file__).resolve().parents[3];BQ=ROOT/'basis_aligned/bilinear_quotient'
OUT=BQ/'circuits/followups/recent_joint_basis_v631_result.json'
...
def main():
    plan=dict(panels=['fineweb_n96_skip1200.pt','fineweb_n192_skip11000.pt'],ranks=[512,1152],
        families=['raw_input','raw_cpu64','balanced_input','joint_tensor_input'],scales=[1.,0.,.5,1.5],
    ...
    _,_,cpu_v=torch.linalg.svd(torch.cat([L,R]).double().cpu(),full_matrices=False)
    bases=dict(raw_input=raw_v.T,raw_cpu64=cpu_v.T.float().to(L.device),balanced_input=balanced_v.T,joint_tensor_input=joint_basis.flip(1))
    basis_audit={name:dict(orthogonality_relative=float((basis.T@basis-torch.eye(d,device=L.device)).norm()/d**.5),
        left_reconstruction=float((L-(L@basis)@basis.T).norm()/L.norm()),
        right_reconstruction=float((R-(R@basis)@basis.T).norm()/R.norm())) for name,basis in bases.items()}
    ...
    for family,rank in configs:
        conditional_errors=[];q=build(family,rank);stats={panel:{} for panel in plan['panels']}
        for batch in batches:
            base=None
            for scale in plan['scales']:
                alpha=execute(q,*batch['ports'],scale)['alpha'];local_calls+=1
                truth=batch['targets'][scale]
                conditional_errors.append(float((alpha-batch['conditional'][scale]).norm()/batch['conditional'][scale].norm()))
        ...
        row=dict(family=family,rank=rank,maximum_conditional_error=max(conditional_errors),stored_values=size(q),fraction_of_dense=size(q)/size(w),panels=panels,
    ...
    result=dict(plan=plan,rows=rows,selected={k:v for k,v in selected.items() if k!='panels'},
        ...
        basis_audit=basis_audit,gram_gauge_error=gram_gauge_error,balance_range=[float(balance.min()),float(balance.max())],
    ...
    disk_guard.guard_write(1000000,label='v631 JSON');OUT.write_text(json.dumps(result,indent=2)+'\n')
