...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['attn34_motif_class_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; b2-damage vs m16-help co-variation on subword positions (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'front_m16_pipeline_results.json'
...
if __name__=='__main__':
    ...
    SEL['motif_off']=(); SEL['zero_m16']=True
    print('ARM 3/3: m16 zeroed',flush=True)
    _=main(); cev_zm=SEL['cev']
    import statistics as stt
    for nm,cv in (('no2',cev_no2),('zm16',cev_zm)):
        if float((cv-cev_full).abs().sum())<1e-3:
            raise SystemExit(f'INSTRUMENT FAIL: {nm} identical to full')
    d2=(cev_full-cev_no2)      # b2 replacement damage (positive = hurts)
    hm=(cev_zm-cev_full)       # m16 help (positive = m16 was helping)
    SW=CLS.index('subword')
    swm=(clsflat==SW).nonzero().squeeze(1)
    order=d2[swm].argsort()
    swo=swm[order]
    B=200
    bd=[];bh=[]
    for k0 in range(0,len(swo)-B+1,B):
        ii=swo[k0:k0+B]
        bd.append(float(d2[ii].mean())); bh.append(float(hm[ii].mean()))
    def rank(v):
        o=sorted(range(len(v)),key=lambda i2:v[i2]); r7=[0]*len(v)
        for pos,i2 in enumerate(o): r7[i2]=pos
        return r7
    n7=len(bd)
    rho=1-6*sum((a-b)**2 for a,b in zip(rank(bd),rank(bh)))/(n7*(n7*n7-1)) if n7>2 else 0.0
    med2=float(d2[swm].median())
    top=swm[d2[swm]>med2]
    tshare=float(hm[top].sum())/max(float(hm[swm].sum()),1e-9)
    pa=rho>=0.3
    pb=tshare>=0.6
    pc=abs(r_full['L2_F']-2.6662)<=0.01
    res={'full':{'L2_F':r_full['L2_F']},'n_subword':int(len(swm)),'n_bins':n7,
         'spearman_binned':round(rho,3),'top_half_share_of_m16_help':round(tshare,4),
         'convention':'b2 damage = CE(full)-CE(no2); m16 help = CE(zm16)-CE(full); subword positions',
         'pred_a_covary':bool(pa),'pred_b_concentrated':bool(pb),'pred_c_reproduces_s2144':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'subword n={len(swm)}; binned Spearman {rho:+.3f}; top-half share of m16 help {tshare:.3f}')
    print(f"(a) rho {rho:+.3f} >= 0.3: {'HELD' if pa else 'FAILED'}")
