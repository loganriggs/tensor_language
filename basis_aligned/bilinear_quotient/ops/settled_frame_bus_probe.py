#!/usr/bin/env python
"""settled_frame_bus_probe -- the settled half of the model as ONE bus: §2754's SPLIT8 read program at k = 1024 with the writes of
blocks 8-17 also confined to the shared frame (remainder routed to the readout / deleted / hidden from the readout, §2748). CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_bus_is_free pred_c_remainder_matters pred_d_readout_is_the_main_consumer
#                     pred_e_out_of_frame_energy_small
...
Preregistration: polynomial_causal/SETTLED_FRAME_BUS_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/settled_frame_bus_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("settled_frame_bus_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "SETTLED_FRAME_BUS_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "settled_frame_split_point_probe_results.json"   # §2754
OUT = ROOT / "settled_frame_bus_probe_results.json"
HASHES = {PREREG: "2ed216c14aaa3849a21c6cc9ea1e67009919b674c0900de84d96bacc57e9c10f", PRIOR: "e4779f4bb162798609f15e3e7a793f99277d82edee5c7c4a2677b4bf79a5f75d",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "settled_frame_bus_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_ALL36_1024 = 0.0337; PRIOR_SPLIT8_1024 = 0.0374; LAM = 1e-2
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_max": 0.005, "c_min": 0.010, "d_min": 0.5, "d_denom_min": 0.003, "e_max": 0.10}
NULLS = {"b_min": 0.020, "c_max": 0.003, "d_max": 0.2, "e_min": 0.25}
...
def main():
    ...
    def split8(mode=None):
        patch = {s_: head(s_, K) for s_ in OWN8}; buf = Buf()
        for s_ in SET8:
            r_ = head(s_, K, U_8); patch[s_] = r_ if mode is None else Both(r_, Route(s_, K, U_8, mode, buf))
        if mode in ("readout", "hidden"):
            patch[FINAL] = FinalAdd(buf, +1.0 if mode == "readout" else -1.0)
        return run(patch, (buf,))
    full = {s_: head(s_, K) for s_ in ESITES}; full.update({s_: head(s_, K, U_late) for s_ in LSITES})
    arms = {"ALL36_1024": run(full), "SPLIT8_1024": split8(), "SPLIT8_BUS_1024": split8("readout"), "SPLIT8_DEL_1024": split8("delete"), "SPLIT8_HID_1024": split8("hidden")}
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    Uk = U_8[:, :K]
    def out_energy(s_):
        C = wheads[s_]["Cw"].to(Uk.dtype); return float(1.0 - torch.trace(Uk.T @ C @ Uk) / torch.trace(C))
    oe = {f"{s_[0]}{s_[1]}": out_energy(s_) for s_ in SET8}
    oe_med = float(torch.tensor(list(oe.values())).median())
    gaps = {"bus_minus_split8": arms["SPLIT8_BUS_1024"] - arms["SPLIT8_1024"], "del_minus_split8": arms["SPLIT8_DEL_1024"] - arms["SPLIT8_1024"],
            "hid_minus_split8": arms["SPLIT8_HID_1024"] - arms["SPLIT8_1024"]}
    ratio = gaps["hid_minus_split8"] / gaps["del_minus_split8"] if gaps["del_minus_split8"] >= BARS["d_denom_min"] else None
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["ALL36_1024"] - PRIOR_ALL36_1024) <= BARS["repro_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_bus_is_free': bool(gaps["bus_minus_split8"] <= BARS["b_max"]),
        'pred_c_remainder_matters': bool(gaps["del_minus_split8"] >= BARS["c_min"]),
        'pred_d_readout_is_the_main_consumer': bool(ratio is not None and ratio >= BARS["d_min"]),
        'pred_e_out_of_frame_energy_small': bool(oe_med <= BARS["e_max"]),
    }
    nulls = {"b_null_ge_.02": bool(gaps["bus_minus_split8"] >= NULLS["b_min"]), "c_null_le_.003": bool(gaps["del_minus_split8"] <= NULLS["c_max"]),
             "d_null_ratio_le_.2": bool(ratio is not None and ratio <= NULLS["d_max"]), "e_null_ge_.25": bool(oe_med >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "k": K, "split": SPLIT,
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "all36_1024": arms["ALL36_1024"], "prior_all36_1024": PRIOR_ALL36_1024, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024},
           "ce_added": arms, "gaps": gaps, "hid_over_del_ratio": ratio, "out_of_frame_write_energy_1024": oe, "out_of_frame_write_energy_median": oe_med,
