#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_five_verb_writes_small_at_the_noun_position pred_c_all_attention_small_at_the_noun_position pred_d_five_below_all pred_e_rest_small
"""The verb heads' writes at the noun's OWN position (v539). For a verb directly after the noun the answer position is the noun itself (43 adjacent rows):
its agreement comes mostly from its own residual (MLPs 0.45, attention into the position 0.31, v501 / v502) with the verb heads' self-read values worth 0.107
(v500). Here the write side: the five verb heads' writes into the noun's own position swapped jointly, the other 40 heads of their blocks, and every block's
attention write into that position (replay of v502's 0.313).
PREDICTIONS (scored as written; failures preserved; priors from v500 / v502)
    pred_a_baseline_replays                       the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_five_verb_writes_small_at_the_noun_position  the five verb heads' joint write swap closes <= 0.15 of the adjacent-row agreement margin gap
    pred_c_all_attention_small_at_the_noun_position  every block's attention write into the position closes 0.313 +- 0.05 (v502)
    pred_d_five_below_all                         the five heads' joint swap is <= the all-attention swap
    pred_e_rest_small                             the other 40 heads of their blocks close <= 0.10
...
OUT = ROOT / "circuits/followups/verb_writes_adjacent_v539_result.json"
CANDIDATE_ID = "chain.verb_writes_adjacent_v539"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, FIVE_MAX, ALL_V502, BAND, REST_MAX, UNIT = 1e-3, 0.15, 0.313, 0.05, 0.10, 701
...
FORWARDS_MAX = 9
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_five_verb_writes_small_at_the_noun_position": "<= 0.15", "pred_c_all_attention_small_at_the_noun_position": "0.313 +- 0.05", "pred_d_five_below_all": "five <= all", "pred_e_rest_small": "<= 0.10"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "five_max": FIVE_MAX, "all_v502": ALL_V502, "band": BAND, "rest_max": REST_MAX, "unit": UNIT}}
    ...
    MODES = {"FIVE_V": ({}, (0,)), "REST_V": ({}, (0,)), "ALL": ({}, (0,))}; SETS_W = {"FIVE_V": FIVE_V, "REST_V": REST_V, "ALL": ALL_H}; forwards = 0
    def logits_margin(x):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_five_verb_writes_small_at_the_noun_position": abs(mc["FIVE_V"]) <= FIVE_MAX, "pred_c_all_attention_small_at_the_noun_position": abs(mc["ALL"] - ALL_V502) <= BAND,
                   "pred_d_five_below_all": mc["FIVE_V"] <= mc["ALL"], "pred_e_rest_small": abs(mc["REST_V"]) <= REST_MAX}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "verb_writes_adjacent_result_v539", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
