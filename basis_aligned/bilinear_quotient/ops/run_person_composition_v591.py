#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_early_copies_replay pred_c_readers_cue_values_replay pred_d_two_sources_reach_half pred_e_two_sources_near_their_sum
"""Person composition: early copies and the readers' cue values (v591). v579-v583: on 43 natural person pairs the four readers' values at the cue carry 0.361 of
the myself - yourself gap and blocks 0-4's copies of I / you into the following context 0.154. Here blocks 0-4's post-cue writes swapped (COPIES), the four
readers' values at the cue (CUE), and both jointly (COPIES_AND_CUE) -- the gender line's two-source composition (v576: 0.60 + 0.26 -> 0.90) tried on person.
PREDICTIONS (scored as written; failures preserved; priors from v579 / v583 / v586)
    pred_a_baseline_replays          the unedited manual forward replays the model's myself - yourself margins within 1e-3
    pred_b_early_copies_replay       COPIES closes 0.154 +- 0.05
    pred_c_readers_cue_values_replay CUE closes 0.361 +- 0.05
    pred_d_two_sources_reach_half    COPIES_AND_CUE closes >= 0.50
    pred_e_two_sources_near_their_sum  COPIES_AND_CUE is within 0.10 of CUE + COPIES. Prior: unsure (the write side is spread, v586-v589)
...
OUT = ROOT / "circuits/followups/person_composition_v591_result.json"
CANDIDATE_ID = "person.composition_v591"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {8: [1], 13: [1], 10: [5], 15: [1]}   # the four person readers (name kept)
REPLAY_TOL, COPIES_V583, CUE_V579, BAND, HALF, NEAR, UNIT = 1e-3, 0.154, 0.361, 0.05, 0.50, 0.10, 701
...
FORWARDS_MAX = 9
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_early_copies_replay": "0.154 +- 0.05", "pred_c_readers_cue_values_replay": "0.361 +- 0.05", "pred_d_two_sources_reach_half": ">= 0.50", "pred_e_two_sources_near_their_sum": "|joint - sum| <= 0.10"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "copies_v583": COPIES_V583, "cue_v579": CUE_V579, "band": BAND, "half": HALF, "near": NEAR, "unit": UNIT}}
    ...
    EARLY = {l: list(range(N_HEAD)) for l in range(0, 5)}
    MODES = {"COPIES": ({}, (0,)), "CUE": (FIVE, (0,)), "COPIES_AND_CUE": (FIVE, (0,))}; SETS_W = {"COPIES": EARLY, "COPIES_AND_CUE": EARLY}
        ...
        with torch.no_grad():
            ...
            for l, block in enumerate(blocks):
                ...
                if mode != "native" and l in MODES[mode][0]:
                    ...
                    if mode != "native" and mode in SETS_W and l in SETS_W[mode]:
                        hsels = SETS_W[mode][l]
    ...
    print("person composition", {k: round(v, 3) for k, v in mc.items()}, "sum", round(mc["CUE"] + mc["COPIES"], 3))
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_early_copies_replay": abs(mc["COPIES"] - COPIES_V583) <= BAND, "pred_c_readers_cue_values_replay": abs(mc["CUE"] - CUE_V579) <= BAND,
                   "pred_d_two_sources_reach_half": mc["COPIES_AND_CUE"] >= HALF, "pred_e_two_sources_near_their_sum": abs(mc["COPIES_AND_CUE"] - mc["CUE"] - mc["COPIES"]) <= NEAR}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "person_composition_result_v591", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
