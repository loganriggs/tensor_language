"""COMPUTE-SPARSE MIXED POINT (rung 262): the best-value config - mixed patterns + per-token top-1152.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The mixed-spectrum point (180M ->
0.0573/11) plus the additive top-k surcharge (~+0.016 at all three prior anchors, S2350): predict ~0.073.
REGISTERED PREDICTIONS (arms: THIS vs mixed 0.0573/11 and ct96-topk 0.0718/8):
  (a) NEAR-ADDITIVE: census <= 0.078.
  (b) valid >= 8.
  (c) SANITY: census in [0.01, 0.15].
NULL: sparsity interacts with mixed maps (census >= 0.088 or valid <= 6). PRICE: 180M storage + 4x MLP
compute sparsity - the cheapest strong config on both axes if it holds. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: compute-sparse mixed point')
    raise SystemExit(0)
import torch
...
OUT=PT+'mixed_topk_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['cp_topk']=1152; SEL['qk_r']=96; SEL['qk_rmap']={}; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    print(f"(a) census {agg:+.4f} <= 0.078: {'HELD' if pa else 'FAILED'}")
