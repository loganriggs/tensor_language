#!/usr/bin/env python3
# BQLANE: gpu
# BQGATE: EXPERIMENT pred_a_exact_instrument pred_b_both_single_branches_live pred_c_multiplicative_overlap_is_material pred_d_joint_family_transfer pred_e_unrelated_reader_selectivity
"""Fresh factorial of late-touching QK1 routing and head8.2-induced current value."""
...
PREREG=P/"SETTING2_REGIONAL_QK1_VALUE_COMPOSITION_FRESH_V1_PREREGISTRATION.md"
ROWS=P/"SETTING2_REGIONAL_QK1_VALUE_COMPOSITION_FRESH_V1_ROWS.json"
BINDING=P/"SETTING2_REGIONAL_QK1_VALUE_COMPOSITION_FRESH_V1_BINDING.json"
...
OUT=ROOT/"basis_aligned/bilinear_quotient/circuits/fast_screens/setting2_regional_qk1_value_composition_fresh_v1_result.json"
SOURCE_NAMES=("embedding",)+tuple(x for layer in range(8) for x in (f"attn{layer}",f"mlp{layer}"))
LATE_NAMES=("attn5","mlp5","mlp6","mlp7")
ARMS=("native","routing_only","value_only","joint")
...
def load_bound():
    ...
    if rows_doc["prior_context_overlap"]!=0 or parent["terminal"]!="valid_qk1_late_group_fold" or not all(parent["predictions"].values()): raise ValueError("authority invalid")
...
def plan():
    ...
    return {"schema":"setting2_regional_qk1_value_composition_fresh_v1_plan","model_loaded":False,"gpu_accessed":False,"queue_touched":False,"rows":len(rows),"arms":ARMS,"late_names":LATE_NAMES,"price":PRICE,"row_checks":checks,"binding_sha256":sha(BINDING)}
...
def main():
    ...
    for _,indices in sorted(buckets.items()):
        for offset in range(0,len(indices),8):
            ...
            for arm_index,arm in enumerate(ARMS):
                ...
                edit={"routing_only":route_delta,"value_only":value_delta,"joint":joint_delta}.get(arm)
    ...
    for family in families:
        ...
        for scale,name in enumerate(("numerator","logit")):
            route=pair_effect[1,scale,pair_ids,0]; value_effect=pair_effect[2,scale,pair_ids,0]; joint=pair_effect[3,scale,pair_ids,0]; interaction=joint-route-value_effect
            controls={READOUTS[j][0]:float(effects[3,scale,row_ids,j].square().mean().sqrt()/joint.square().mean().sqrt().clamp_min(1e-30)) for j in range(1,len(READOUTS))}
            report[name]={"routing_target_rms":float(route.square().mean().sqrt()),"value_target_rms":float(value_effect.square().mean().sqrt()),"joint_target_rms":float(joint.square().mean().sqrt()),"interaction_target_rms":float(interaction.square().mean().sqrt()),"joint_additive_cosine":cosine(joint,route+value_effect),"signed_joint_mean":float(joint.mean()),"unrelated_reader_ratios":controls}
    ...
    predictions={
        ...
        "pred_c_multiplicative_overlap_is_material":bool(instrument and all(r["head_cross_to_smaller_single_mean_ratio"]>=.02 and r["recursive_interaction_to_smaller_single_ratio"]>=.01 for r in family_reports.values())),
    ...
    result={"schema":"setting2_regional_qk1_value_composition_fresh_v1_result","terminal":"routing_value_composition" if all(predictions.values()) else "valid_null" if instrument else "invalid","predictions":predictions,"metrics":metrics,"family_reports":family_reports,"arms":ARMS,"readouts":[name for name,_ in READOUTS],"late_source_names":LATE_NAMES,"price":PRICE,"row_checks":checks,"binding_sha256":sha(BINDING),"runner_sha256":sha(RUNNER),"checkpoint_weights_sha256":"680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3","created_utc":datetime.now(timezone.utc).isoformat().replace("+00:00","Z"),"scope":"Fresh 2x2 recursive composition of the frozen late-touching head9.8 QK1 removal with the half-strength head8.2-induced head9.8 current-value input; full-head current-value routing at framing sources, five unrelated readers, no fit."}
