...
"""Number family DoD, step 6 (v60): matched-count random four-head-set null for SIMPLE (as v31/v45), v55 rows, were−was contrast.
...
OUT = ROOT / "circuits/followups/number_family_dod_random_set_null_v60_result.json"
CANDIDATE_ID = "lexical_number.pp_intervener.dod_random_set_null_v60"
LIVE_FRACTION, LIVE_POSITIVE, TENFOLD, INSTRUMENT_TOL = 0.10, 0.75, 10.0, 1e-4
FORWARDS_MAX = 44
EXCLUDED = {(11, 3), (5, 7), (7, 8), (9, 7)}
POOL = [(l, h) for l in range(18) for h in range(9) if (l, h) not in EXCLUDED]
SETS = [tuple(sorted(random.Random(2026_09_18_60 + s).sample(POOL, 4))) for s in range(16)]
...
def main() -> None:
    ...
    fw.directions = L.readout_directions(backend.model, v55.SET + tuple(c for s in SETS for c in v25.components_for(s)), v55.WERE, v55.WAS)
    forwards = 0
    ...
    print("set", round(comp["target_damage_mean"], 4), "random max", round(max(dmg), 4), "median|.|", round(med, 4), "live randoms", sum(r["live"] for r in randoms))
    ...
    OUT.write_text(json.dumps({"schema": "number_family_dod_random_set_null_result_v60", "candidate_id": CANDIDATE_ID, "instrument_max_abs_error": instrument, "set": comp, "random": randoms, "random_damage_max": max(dmg),
                               "random_damage_median_abs": med, "predictions": predictions, "forwards": forwards, "serial_seconds": time.perf_counter() - t0, "finished_utc": datetime.now(timezone.utc).isoformat()}, indent=2, sort_keys=True) + "\n")
