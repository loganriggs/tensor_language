...
"""Selection DoD (v261): the selection family at MLP-8 unit grain. Readers {8.8, 6.3, 13.8, 7.8} read the verb (remained / looked / grew) -> the selected particle
(v85); 13.8 is the family's contextual reader. Exact census (`dod_units.unit_census`) of MLP 8's write on 13.8's weight-only reader direction at the CUE (the verb)
and at the FINAL token of the v85 rows (96 rows), pooled positive - negative over aligned pairs. Kill criterion (review 26): top-10 < 0.30 at both positions -> port.
PREDICTIONS (scored as written; failures preserved; priors unsure -- the cue is a content word (a verb), like the noun-number and gender families that had detectors)
...
OUT = ROOT / "circuits/followups/selection_dod_mlp8_unit_census_v261_result.json"
CANDIDATE_ID = "selection.particle.dod_mlp8_unit_census_v261"
LAYER, HEAD, CLOSURE_TOL, TOP10_MIN, JACCARD_MIN = 8, (13, 8), 1e-3, 0.50, 1 / 3
...
def main() -> None:
    ...
    OUT.write_text(json.dumps({"schema": "aspectual_dod_mlp8_unit_census_result_v261", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "positions": report, "predictions": predictions, "forwards": forwards,
