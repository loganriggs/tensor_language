"""THE 8-DIRECTION SELECTOR (rung 16): is the metric's gain reproducible from its top eigen-directions alone?
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['stream_error_profile_results.json','metric_units_mechanism2_results.json']
    ...
    print(f"DRYRUN OK: S2086 present (block 6 {_p['6']}); four selectors x two windows at K=2304 (norm / metric / top-8 / top-r50)")
    raise SystemExit(0)
import json, sys, time, torch
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
import os
...
OUT=PT+'metric_units_top8_results.json'
...
def main():
    ...
    def build_arm(mode,K=2304):
        def fit_res(li,piece,active):
            ...
            if mode in ('plain','metric-units','metric-units-all','top8','topr50'):
        ...
        tag={'plain':'E','metric':'M','metric-bases':'B','metric-units':'U','metric-units-all':'A','random-metric':'R','top8':'T','topr50':'F'}[mode]
        act=['a0']
    ...
    res={}
    for mode in ('plain','metric-units','top8','topr50'):
        ts=time.time()
        ...
        res[mode]={'gap_w1':round(ce1-base1,5),'gap_w2':round(ce2-base2,5),'profile':[round(p,4) for p in prof]}
        print(f'{mode:13s}: gap w1 {ce1-base1:+.4f} w2 {ce2-base2:+.4f} | b5 {prof[5]:.3f} b6 {prof[6]:.3f} b7 {prof[7]:.3f} | {time.time()-ts:.0f}s',flush=True)
        del st; torch.cuda.empty_cache()
    E=res['plain']
    g1=lambda k:E['gap_w1']-res[k]['gap_w1']; g2=lambda k:E['gap_w2']-res[k]['gap_w2']
    pa=g1('top8')>=0.8*0.124; pb=g2('top8')>=0.5*0.075; pc=g1('topr50')>=g1('top8')
    pd=abs(E['profile'][6]-1.7415)<=0.10 and abs(g1('metric-units')-0.124)<=0.02
    out={'base_ce_w1':round(base1,5),'base_ce_w2':round(base2,5),'arms':res,'r50':{k:HEAD[k]['r50'] for k in ('c4','c5')},
         'gains_w1':{k:round(g1(k),4) for k in res if k!='plain'},'gains_w2':{k:round(g2(k),4) for k in res if k!='plain'},
         'pred_a_top8_w1':bool(pa),'pred_b_top8_w2':bool(pb),'pred_c_topr50_ge_top8':bool(pc),'pred_d_reproduction':bool(pd),
         'self_reviewed':True,'runtime_s':round(time.time()-t0,1)}
    json.dump(out,open(OUT,'w'),indent=1)
    print(f"(d) cfgE b6 {E['profile'][6]:.4f}; full metric gain {g1('metric-units'):+.4f} vs 0.124: {'HELD' if pd else 'FAILED'}")
    print(f"(a) top-8 gain w1 {g1('top8'):+.4f} >= 0.0992: {'HELD' if pa else 'FAILED'}")
    print(f"(b) top-8 gain w2 {g2('top8'):+.4f} >= 0.0375: {'HELD' if pb else 'FAILED'}")
    print(f"(c) top-r50 gain w1 {g1('topr50'):+.4f} >= top-8 {g1('top8'):+.4f}: {'HELD' if pc else 'FAILED'}")
