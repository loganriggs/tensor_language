#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the thirty-ninth seven spec-authored behaviours, plus one instrument.
"""v293: the AMENDED tier-3 battery over the THIRTY-NINTH batch (SEVEN cells) -- plus one instrument.
Authored while v289 runs; the ` against` and ` into` readouts return with fresh cues, and ` across` gets a third cell.

v265 2/7, v267 2/5, v269 5/5, v277 1/8, v279 4/7, v285 2/7, v287 5/6 (+instrument each).
Seven cells, all NEW pairs, word-boundary quote-agnostic grep 0 hits (in/against, to/across, on/into, at/into, by/from,
for/into, in/against-adjective):
verb in_against (trusted/railed), to_across (resorted/stumbled), on_into (embarked/plunged), at_into (glanced/peeked),
by_from (swore/withdrew), for_into (opted/dived); adjective in_against (deficient/biased).
CPU capability margins BEFORE enqueue (mean base / mean donor on the donor axis; A1 | A2; dropped rows; floor 1.0 on A1 per the v245 lesson):
  verb_preposition_in_against A1:-2.00/+1.82 drop0 A2:-1.36/+1.24 drop0 C:-1.43/+0.93 drop2
  verb_preposition_to_across A1:-5.08/+5.16 drop0 A2:-4.70/+4.90 drop0 C:-1.43/+0.93 drop2
  verb_preposition_on_into A1:-4.06/+4.21 drop0 A2:-4.01/+3.86 drop0 C:-1.43/+0.93 drop2
  verb_preposition_at_into A1:-2.10/+1.95 drop0 A2:-1.98/+1.93 drop0 C:-1.43/+0.93 drop2
  verb_preposition_by_from A1:-2.54/+2.64 drop0 A2:-2.45/+2.47 drop0 C:-1.43/+0.93 drop2
  verb_preposition_for_into A1:-3.21/+3.29 drop0 A2:-3.60/+3.61 drop0 C:-1.43/+0.93 drop2
  adjective_preposition_in_against A1:-3.54/+3.63 drop0 A2:-2.42/+2.63 drop0 C:-1.43/+0.93 drop2
  All SEVEN cleared the floor with zero dropped rows (weakest A1 2.00). Tenth batch on the probe path.
  CUE FRESHNESS SCRIPT-VERIFIED: 14 cue words, zero reused, after `believed`, `rebelled` and `ventured` were caught
  colliding with insisted/against_for/to_beyond and replaced with trusted / railed / plunged. Three collisions in one
  draft is the norm rather than the exception -- the corpus has 404 distinct (cue -> readout token) mappings with 73
  already shared -- which is why this check runs on the GENERATED modules before anything is committed.
  Batch size SEVEN; bars are the seven-cell bars (3/5/6/5/5).
Protocol identical to v197/v201/.../v287. finiteness_selection is the instrument and is EXCLUDED from four-row counts.

REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 3 of the 7 new behaviours pass all four rows (v287 5/6, v285 2/7, v279 4/7). prior 55%
  pred_b_row2           -> MIN held-out per-direction exact-set recovery >= 0.80 on at least 5 of 7 (v287 6/6).   prior 60%
  pred_c_row3           -> dim A1 CE damage LB975 > 0 and >= 0.10 on at least 6 of 7 (v287 6/6).                  prior 80%
  pred_d_row4           -> constrained rank-1 own-C CE UB975 <= 0.01 on at least 5 of 7 (v287 5/6).               prior 65%
  pred_e_row5_amended   -> A2-own / A2 ceiling within [0.5, 1.4] with LB975 > 0 on at least 5 of 7 (v287 6/6).    prior 75%
  pred_f_instrument     -> finiteness_selection: n_units identical to v197 and extraction_held_dirB within +-0.02.
No row is predicted from a capability margin (retired 09:45), and no readout-token prediction is made: v279 refuted the
rare-token reading and v287's ` onto` block passing together was confounded with that batch's unusually high margins,
so token-level expectations are simply not registered any more.
Accounting: four-row passes count only after a separability rung. v289 is currently sweeping ELEVEN passes from three
batches in one rung (46 evaluated members, ~5.5 h at the measured rate of 428 s per member); this batch's passes queue
behind that and will be folded into the next combined rung on the same principle -- one sweep of the counted members
per group of batches, not one per batch.
An error receipt per behaviour counts as a miss on every row. ~40 s GPU each, ~5 min.
Smoke: V293_SMOKE=<out.json> (CPU, V293_SMOKE_ROWS=4 per split, V293_SMOKE_NAMES=verb_preposition_in_against).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec9m_v293_result.json"
...
NAMES = {"verb_preposition_in_against": "verb_preposition_in_against", "verb_preposition_to_across": "verb_preposition_to_across",
         "verb_preposition_on_into": "verb_preposition_on_into", "verb_preposition_at_into": "verb_preposition_at_into",
         "verb_preposition_by_from": "verb_preposition_by_from", "verb_preposition_for_into": "verb_preposition_for_into",
         "adjective_preposition_in_against": "adjective_preposition_in_against",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec9m_v293", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V293_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V293_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V293_SMOKE_NAMES", "verb_preposition_in_against").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec9m_v293", "candidate_id": "corpus.unit_tier3_batch_amended_spec9m_v293", "bars": BARS,
