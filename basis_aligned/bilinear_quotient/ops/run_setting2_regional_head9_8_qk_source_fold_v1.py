...
BINDING = P / "SETTING2_REGIONAL_HEAD9_8_QK_SOURCE_FOLD_V1_BINDING.json"
PARENT = ROOT / "basis_aligned/bilinear_quotient/circuits/fast_screens/setting2_regional_attn9_head_mlp16_fold_v2_result.json"
OUT = ROOT / "basis_aligned/bilinear_quotient/circuits/fast_screens/setting2_regional_head9_8_qk_source_fold_v1_result.json"
...
def plan() -> dict:
    ...
    return {"schema": "setting2_regional_head9_8_qk_source_fold_v1_plan",
...
def main() -> None:
    ...
    raw_native = torch.empty(96, 1152, dtype=torch.float64)
    raw_recomputed = torch.empty_like(raw_native)
    score_component_sums = [[], []]
    score_totals = [[], []]
    ...
    try:
        for _, indices in sorted(bound["buckets"].items()):
            for offset in range(0, len(indices), 8):
                ...
                r9 = sum(source_parts)
                raw_native[selected] = r9_native.double().cpu()
                raw_recomputed[selected] = r9
                ...
                for factor in range(2):
                    ...
                    score_component_sums[factor].append(components.sum(0))
                    score_totals[factor].append(total_score)
    ...
    raw_exact_error = rel(raw_recomputed, raw_recomputed)
    raw_native_bridge = rel(raw_recomputed, raw_native)
    score_errors = [rel(torch.cat(score_component_sums[f]), torch.cat(score_totals[f])) for f in range(2)]
    ...
    result = {"schema": "setting2_regional_head9_8_qk_source_fold_v1_result",
