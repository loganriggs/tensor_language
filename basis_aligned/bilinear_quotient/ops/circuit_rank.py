"""CIRCUIT-AWARE RANK ALLOCATION (rung 234): exact patterns at the three circuit-loaded attention blocks.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. Uniform curve: r64 0.0852/7 certs,
r96 0.0622/9 certs. Here rank goes where the CIRCUITS are: blocks 3, 8, 16 (a3/a8/a16 top 5+16+13
circuits) get full-rank (exact) patterns; everything else stays r64. Extra price ~1/3 of the r64->r96
step. This is NOT the dead block-grain split (S2302, low ranks by position); it is targeted exactness by
measured circuit load, aimed at the CERTIFICATE count.
REGISTERED PREDICTIONS (arms: THIS vs r64 0.0852/7 and r96 0.0622/9):
  (a) BEATS r64 AT A FRACTION OF r96 PRICE: census <= 0.075.
  (b) CERTIFICATE-TARGETED ALLOCATION PAYS: valid >= 9 (matches r96's count cheaper).
  (c) SANITY: census in [0.01, 0.12]; inert tripwire vs the r64 receipt.
NULL: circuit load does not predict rank need (census >= 0.082 or valid <= 7). PRICE: r64 config + 27
heads at full rank. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: circuit-aware rank allocation')
    raise SystemExit(0)
import torch
...
OUT=PT+'circuit_rank_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=64; SEL['qk_rmap']={3:128,8:128,16:128}; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    if abs(agg-0.0852)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: circuit-rank map inert')
