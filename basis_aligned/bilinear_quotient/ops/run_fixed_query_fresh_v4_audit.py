...
"""Prospective frozen source-column transfer; FIXED_QUERY_FRESH_V4_PREREGISTRATION.md."""
import os,json,sys,time,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';OUT=A/'fixed_query_fresh_v4_audit_result.json';PLAN=dict(prefix=18,native_suffix=60,baseline_attention=6)
def main():
 ...
 for context in contexts:
  ...
  fixed_queries={name:getattr(att,name)(F.rms_norm(c['raw'],(c['raw'].shape[-1],))) for name in ['c_q','c_q2']}
  def native(role,amplitudes,compiled=False):
   counts['native_suffix']+=1;pos,ds=c['source_components'][role]
   handles=[]
   if compiled==2:
    for name in ['c_q','c_q2']:handles.append(getattr(att,name).register_forward_hook(lambda module,args,output,name=name:fixed_queries[name]))
   elif compiled:att.forward=lambda z,v1=None:(evaluate(att,z,pos,background),v1)
  ...
  for role in ['subject','attractor']:
   ...
   ref=base-native(role,np.tile(reference,(n,1)));full=base-native(role,weights);compiled=base-native(role,weights,True);frozen=base-native(role,weights,2);pred=-torch.einsum('bok,bk->bo',g,torch.tensor(weights,device='cuda',dtype=torch.float64))
   for family in dict.fromkeys(families):
    ...
    if dataset=='v2_replay':
     old=next(r for r in installed['records'] if r['panel']==panel and r['role']==role and r['family']==family);replays.append(float((v-torch.tensor(old['effect'],device='cuda',dtype=torch.float64)).abs().max()))
    ...
    records.append(dict(dataset=dataset,panel=panel,family=family,role=role,full=score(truth),compiled=score(v),fidelity_errors=err.tolist(),compiled_vs_frozen=((v-frozen[ids]).norm(dim=0)/den).tolist(),frozen_fidelity_errors=((frozen[ids]-truth).norm(dim=0)/den).tolist(),bank_prediction_errors=((pred[ids]-truth).norm(dim=0)/den).tolist(),capability=float((base[ids,0]>0).double().mean()),baseline_margins=base[ids,0].tolist(),effect=v.tolist(),full_effect=truth.tolist(),reference_effect=ref[ids].tolist(),amplitudes=weights[ids].tolist()))
 ...
 instrument=instrument and max(max(r['compiled_vs_frozen']) for r in records)<=.001
