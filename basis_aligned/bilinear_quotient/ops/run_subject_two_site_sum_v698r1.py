...
"""Two-site post11 subject/attractor response composition on opened opposite-number rows.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v698r1_result.json'
PREDICTIONS=dict(pred_a_exact_replay='original instrument plusattractor normpreservation1e-5 and4prefix/8block11/8suffixcalls',
 pred_b_effect_prediction='two-site numbererror<=.10/modalerror<=.05 ofsum individualeffect norms eachcell',
 pred_c_capable_position_transfer='materialcross>=1%budget: crosspredictionerror<=5%budget andbeatsadditivezero, at leastonematerialcell')


def main():
    plan=dict(prefix_calls=8,suffix_calls=56,sequences_per_arm=48,fits=0,backwards=0,two_site_suffix_calls=8,two_site_reduced_calls=8,attractor_prefix_calls=4,attractor_block11_calls=8,attractor_suffix_calls=8,extra_attention_pullbacks=8,extra_reference_forwards=16,extra_token_local_mlp_pullbacks=24,extra_fulltoken_mlp_pullbacks=24,analytic_reverse_passes=4,attention_pullbacks=24,reference_attention_forwards=48,additional_tangent_suffix_calls=8,additional_initial_interface_suffix_calls=12,
    ...
    import hashlib
    rowfile=POLY/'SUBJECT_ATTRACTOR_CONTROL_V696_ROWS.json'
    assert hashlib.sha256(rowfile.read_bytes()).hexdigest()=='8f5dd61488ce13d8eea3de10e832b9e12ec5c6a3bbb0d4a6c7232d119e690be8'
    ...
    two_site_calls=0;two_site_reports={name:{} for name in ['A','B']}
    optimized_checks=[]
    ...
    for entries in batches:
        ...
        for source_name,delta in [('A',source_deltas[0]),('B',source_deltas[1]),('sum',source_deltas[0]+source_deltas[1])]:
            ...
            if source_name in ['A','B']:
                jd=delta+ad
                jactual=native_selected(context['baseline_after11']+jd,x0,first,readpos,start_layer=12);two_site_calls+=1
                jeffect=base-(jactual[:,0]-jactual[:,1])*direction
                jmodal=((cb-jactual[:,2:])[:,0::2]-(cb-jactual[:,2:])[:,1::2])
                jl,_=run_dag(sparse_runtime,context['all_attention'],jd.double()@Q,context['all_mlp_contexts'],case['final_context'],readpos)
                jpred=base-(jl[:,0]-jl[:,1])*direction
                jpmodal=-(qp*jd.double()).sum((-1,-2)).T
                for i,e in enumerate(entries):
                    jc=two_site_reports[source_name].setdefault(e[3],dict(target=[],prediction=[],native_modal=[],predicted_modal=[],subject_effect=[],attractor_effect=[],subject_prediction=[],attractor_prediction=[]))
                    jc['target'].append(float(jeffect[i]));jc['prediction'].append(float(jpred[i]))
                    jc['native_modal'].append(jmodal[i].tolist());jc['predicted_modal'].append(jpmodal[i].tolist())
                    jc['subject_effect'].append(float(native_effect[i]));jc['attractor_effect'].append(float(aeffect[i]))
                    jc['subject_prediction'].append(float(predicted_effect[i]));jc['attractor_prediction'].append(float(peffect[i]))
    ...
    b=a and min(attractor_specificity.values())>=5 and all(c['native_accuracy']>=.90 for rs in attractor_reports.values() for c in rs.values())
    ...
    material_cells=[]
    for name,report in two_site_reports.items():
        for family,cell in report.items():
            budget=max(np.linalg.norm(cell['subject_effect'])+np.linalg.norm(cell['attractor_effect']),1e-20)
            y=np.array(cell['target']);p=np.array(cell['prediction'])
            joint_cross=y-np.array(cell['subject_effect'])-cell['attractor_effect']
            pcross=p-np.array(cell['subject_prediction'])-cell['attractor_prediction']
            cell['source_budget']=float(budget)
            cell['number_error_over_budget']=float(np.linalg.norm(p-y)/budget)
            cell['number_error_over_own_effect']=float(np.linalg.norm(p-y)/max(np.linalg.norm(y),1e-20))
            cell['modal_error_over_budget']=(np.linalg.norm(np.array(cell['predicted_modal'])-cell['native_modal'],axis=0)/budget).tolist()
            cell['native_cross_over_budget']=float(np.linalg.norm(joint_cross)/budget)
            cell['cross_error_over_budget']=float(np.linalg.norm(pcross-joint_cross)/budget)
            cell['native_cross']=joint_cross.tolist();cell['predicted_cross']=pcross.tolist()
            if cell['native_cross_over_budget']>=.01:material_cells.append([name,family])
    a=a and two_site_calls==8
    b=a and all(cell['number_error_over_budget']<=.10 and max(cell['modal_error_over_budget'])<=.05 for rs in two_site_reports.values() for cell in rs.values())
    c=a and bool(material_cells) and all(two_site_reports[name][family]['cross_error_over_budget']<=.05 and two_site_reports[name][family]['cross_error_over_budget']<two_site_reports[name][family]['native_cross_over_budget'] for name,family in material_cells)
    failed=[]

    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,cells=cells,
                predictions=dict(zip(PREDICTIONS,[bool(a),bool(b),bool(c)])),selectivity=selectivity,two_site_reports=two_site_reports,two_site_calls=two_site_calls,material_cross_cells=material_cells,attractor_reports=attractor_reports,attractor_specificity=attractor_specificity,attractor_counts=attractor_counts,max_attractor_norm_error=max(attractor_norm_checks),max_optimized_reader_difference=max(optimized_checks),hybrid_reports=hybrid_reports,tangent_reports=tangent_reports,max_tangent_relative_check=max(tangent_checks),reader_values_per_batch=reader_prices,tangent_suffix_calls=tangent_calls,initial_reports=initial_reports,initial_interface_suffix_calls=initial_counts,max_full_state_readout_error=max(oracle_checks),failed_modal_oracle_ratios=[o/e for o,e in failed],reuse_reports=reuse_reports,cross_effects=cross,reuse_counts=reuse_counts,pre11_joint_vs_post11_sum_gap=boundary_gaps,
                ...
                scope='Post11 sum ofsubject/attractor responsevectors (not jointlyeditedprefix or pre11composition). Semantic non-subject noun control on48opened v691rows; sameembedding-number removal atpairedattractor, A/Bsourceports patchedonlyatthatpre11site. Native subject labels fixed. Not normmatched tosubject (inputnormsreported). All native context/reader/sourcegenerators charged; no fullmodel adoption.')
