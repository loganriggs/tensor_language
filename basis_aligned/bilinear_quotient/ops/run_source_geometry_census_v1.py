...
def controls():
    ...
    only_s = parent_pieces(S, torch.zeros_like(C), fac, coef, chunk=10)
    e4 = float((only_s[:4].abs().sum()))                     # with C = 0 only the pure-state piece survives
    assert e1 < 1e-12 and e2 < 1e-12 and e4 < 1e-12, (e1, e2, e4)
    return dict(quartic_piece_identity=e1, parent_piece_identity=e2, pure_state_only_when_context_zero=e4)
...
def main():
    ...
    parent = torch.stack([xn @ f.T for f in pf]).prod(0) @ pC.T
    ppieces = parent_pieces(S, C, pf, pC); parent_identity = float((ppieces.sum(0) - parent).norm() / parent.norm())
    ...
    out.write_text(json.dumps(dict(predictions=preds, controls=checks, manifest=manifest, replay=dict(state_context_reconstruction=recon, split_exact=split_exact, x16_capture=replay_x16, calibration_rows=replay_rows, target=replay_target, direct=replay_direct, parent_identity=parent_identity),
