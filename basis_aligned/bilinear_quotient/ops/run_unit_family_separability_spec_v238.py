...
"""v238: within-family separability for the v245 passes degree_complement_than_of (degree) and modal_complement_to_lift (modal_ellipsis),
plus the re-check of every counted member of the two families they enter (v212/v216/v220/v222 rule: a family that grows -- by a member OR
by a control -- re-checks its counted members; standing until board proposal (v)/(vii) is answered). Both families also GROW BY CONTROLS
here: the three-row (row-4-failed) members degree_enough (v207), degree_synthetic_periphrastic_adverb (v233) and modal_ellipsis_could_may
(v235) are added to their family control sets, as person_agreement_am_is was at v234 and relative_case_who_whom at v224.

Families here (ALL members prepared as controls; only EVAL evaluated):
  degree (5 counted: comparative_complement_superior, comparative_complement_inferior, degree_much_very (v224/v228),
    degree_synthetic_periphrastic_comparative, degree_synthetic_periphrastic_superlative (v228); controls superlative_complement_best,
    superlative_complement_largest, degree_so_such + NEW controls degree_enough, degree_synthetic_periphrastic_adverb)
    + degree_complement_than_of (9 units 08:01/09:01/12:08/10:05/07:08; held 0.83, dim 0.688, cdas 0.963, C 0.0089, cext 1.004; v245) --
      a comparative-vs-superlative MORPHOLOGY cue (taller/tallest) read out as the complement preposition (than/of); the counted
      periphrastic pair reads the same morphology into the degree word itself, and superior/inferior read a lexical comparative into
      `to`: the fusion question is whether the model's comparative/superlative axis is one direction across three readout slots.
  modal_ellipsis (2 counted: modal_ellipsis_can_must, modal_ellipsis_should_might (v230: did NOT fuse despite 5 shared hub units);
    NEW control modal_ellipsis_could_may (v235, rows 1101))
    + modal_complement_to_lift (11 units 11:03/07:08/06:03/04:01/03:06; held 0.87, dim 1.109, cdas 1.866, C 0.0007, cext 1.011; v245) --
      ought/must read out as `to` vs the bare verb: a complement-TYPE readout next to members that read the modal's identity back under
      ellipsis; the strongest cell of v245.
Recipe as v198/.../v236: per evaluated member, arm `own` (own C_fit control) and arm `fam` (own C_fit + EVERY sibling's A1 FIT prep,
control_weight 30 x len(controls)); evaluated on the member's held A1 (extraction), each sibling's held A1 (|CE| leak) and held C.
Units frozen from the parent receipts (v245 for the new members; earlier batteries for the counted ones).
...
EVAL = 2 new members + 7 counted re-checks (9 fits x 2 arms; degree carries 10 controls, modal_ellipsis 3: ~20 min).
...
fails is RETRACTED (-1). 73 (after v234; v236 pending) -> up to 75 from this rung.

REGISTERED BEFORE THE RUN (each coded predicate is the sentence here; bars in BARS):
    pred_a_new_separable   at least 1 of the 2 new members is separable (k_new). to_lift is the expected pass (complement type, not modal
                           identity); than_of is the fusion candidate (same morphology cue as the counted periphrastic pair).   prior 70%
    pred_b_counted_kept    at least 6 of the 7 re-checked counted members remain separable (k_kept; v228 kept 3/3 degree, v230 1/1 modal). prior 65%
    pred_c_row4_kept       fam-arm own-C UB975 <= 0.01 on at least 4 of the 9 evaluated members (k_d; v234 18/24).          prior 60%
...
Smoke: V238_SMOKE=<out.json> (CPU, V238_SMOKE_ROWS=4; all members are prepared as controls, V238_SMOKE_NAMES=modal_complement_to_lift,modal_ellipsis_can_must
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v238_result.json"
...
FAMILIES = {
    "degree": ["superlative_complement_best", "superlative_complement_largest", "comparative_complement_superior", "comparative_complement_inferior", "degree_much_very", "degree_so_such", "degree_synthetic_periphrastic_comparative", "degree_synthetic_periphrastic_superlative", "degree_enough", "degree_synthetic_periphrastic_adverb", "degree_complement_than_of"],
    "modal_ellipsis": ["modal_ellipsis_can_must", "modal_ellipsis_should_might", "modal_ellipsis_could_may", "modal_complement_to_lift"],
}
EVAL = ("degree_complement_than_of", "modal_complement_to_lift", "comparative_complement_superior", "comparative_complement_inferior", "degree_much_very", "degree_synthetic_periphrastic_comparative", "degree_synthetic_periphrastic_superlative", "modal_ellipsis_can_must", "modal_ellipsis_should_might", )
COUNTED = ("comparative_complement_superior", "comparative_complement_inferior", "degree_much_very", "degree_synthetic_periphrastic_comparative", "degree_synthetic_periphrastic_superlative", "modal_ellipsis_can_must", "modal_ellipsis_should_might", )
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES, **v213.NAMES, **v215.NAMES, **v217.NAMES, **v219.NAMES, **v221.NAMES, **v223.NAMES, **v225.NAMES, **v227.NAMES, **v229.NAMES, **v231.NAMES, **v233.NAMES, **v235.NAMES, **v237.NAMES, **v239.NAMES, **v241.NAMES, **v243.NAMES, **v245.NAMES}


def _parent_cdas(n):
    # the parent battery receipt that produced this member's units (each behaviour name is unique across batteries)
    for path in (V245, V243, V241, V239, V237, V235, V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
BARS = {"cross_max": 0.05, "keep": 0.80, "ext_floor": 0.50, "c_ub_max": 0.01, "k_d": 4, "k_new": 1, "k_kept": 6, "instr_tol": 0.03}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 20000, 640000


def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v238", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V238_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V238_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v238", "candidate_id": "corpus.unit_family_separability_spec_v238",
              "separable_counts": nsep, "families": FAMILIES, "summary": summary, "members": R, "bars": BARS,
              "recipe": {"lambda": LAM, "steps": steps, "lr": LR, "complement_weight": CW, "rank": 1, "seed": 0, "units_source": "v197 ... v245 battery receipts (later receipts override earlier ones)", "evaluated": list(EVAL),
