"""ALL-QK-32 (rung 193): the top of the rank curve.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. Curve: r8 2.02 / r10 1.52 / r12 1.38 /
r16 1.27 / r24 1.13 - still descending at 24. r32 = a quarter of full head rank.

REGISTERED PREDICTIONS (anchor: rung-189 census +1.1287):
  (a) STILL YIELDING: census <= 1.08.
  (b) MONOTONE: census <= 1.1337.
  (c) SANITY: census >= 0.5; inert tripwire.
NULL (a): plateau at last (>= 1.11). PRICE: 24.2M pattern values. Self-reviewed."""
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['qk_r']=32; SEL['qk_tail']=True
    ...
    print(f"(a) census {agg:+.4f} <= 1.08: {'HELD' if pa else 'FAILED'}")
    print(f"(b) census {agg:+.4f} <= 1.1337 (monotone): {'HELD' if pb else 'FAILED'}")
