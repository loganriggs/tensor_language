#!/usr/bin/env python3
# BQGATE:50304token initialization plus9 body forwards;48 sequences lengths13-19;7 suffix arms;no fit;180sec.
...
STEM='MIXED_TOKEN_HEAD2_NATIVE_V1'
@torch.no_grad()
def main():
    ...
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('9 body forwards48sequences13-19tokens;50304token initialization;7suffix arms;no fitting');return
    ...
    original_readers=torch.cat([program['parent'],program['children']])
    token_program['current_readers']=original_readers[:,:1152].clone()
    token_program['dual']=program['dual'].clone()
    vocab=model.transformer.wte.weight.shape[0];table=torch.empty(vocab,4,dtype=torch.float64,device='cuda')
    initial=model.transformer.h[0].lambdas
    for start in range(0,vocab,1024):
        token_ids=torch.arange(start,min(start+1024,vocab),device='cuda')
        xx=F.rms_norm(model.transformer.wte(token_ids),(1152,))
        first_input=F.rms_norm(initial[0]*xx+initial[1]*xx,(1152,))
        table[token_ids]=first_input.double()@original_readers[:,1152:].T
    token_program['token_reads']=table
    torch.save({k:v.cpu() for k,v in token_program.items()},P/(STEM+'_PROGRAM.pt'))
    ...
    result['program_storage_bytes']=sum(v.numel()*v.element_size() for v in token_program.values())
    result['program_field_dtypes']={k:str(v.dtype) for k,v in token_program.items()}
    ...
    result.update(seconds=time.perf_counter()-tic,body_forwards=count,artifact_sha=digest(art),source_shas=binding,
                  scope='Token lookup package executes on query/current/tokenID/R, without first-state inputs. OldFP32 and tokenFP32 compare to original factors. All-vocab first-state mixed-precision lookup closes that dependency; current/query generators and final suffix remain native.')
