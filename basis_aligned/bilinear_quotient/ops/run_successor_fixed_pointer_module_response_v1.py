...
BINDING = POLY / "SUCCESSOR_FIXED_POINTER_MODULE_RESPONSE_V1_BINDING.json"
OUT = POLY / "SUCCESSOR_FIXED_POINTER_MODULE_RESPONSE_V1_RESULT.json"
...
def load_bound():
    ...
    paths = {"rows": ROWS, "preregistration": PREREG, "prior_art": PRIOR}
...
def plan():
    ...
    return {
        "schema": "successor_fixed_pointer_module_response_v1_plan",
        "model_loaded": False,
        "gpu_accessed": False,
        "queue_touched": False,
...
def main():
    ...
    def count_forward(_module, arguments, _output):
        counts[0] += 1
        counts[1] += len(arguments[0])

    handle = model.transformer.h[0].attn.register_forward_hook(count_forward)
    started = time.perf_counter()
    try:
        native_full, caches = {}, {}
        for condition in CONDITIONS:
            native_full[condition], caches[condition] = sl.run(
                model, cfg, tensors[condition], collect=True,
            )
        ...
        self_full, _ = sl.run(model, cfg, tensors["late_swap_incoherent"], patch_head=self_head, patch_mlp=self_mlp)
        full_head, full_mlp = patches("coherent", "late_swap_incoherent")
        ceiling_full, _ = sl.run(
            model, cfg, tensors["late_swap_incoherent"], patch_head=full_head, patch_mlp=full_mlp,
        )
        ...
        for module in MODULES:
            ...
            for destination in ("late_swap_incoherent", "early_swap_control"):
                ...
                values, _ = sl.run(model, cfg, tensors[destination], patch_head=head or None, patch_mlp=mlp or None)
    ...
    result = {
        "schema": "successor_fixed_pointer_module_response_v1_result",
        "terminal": terminal,
        "predictions": {
