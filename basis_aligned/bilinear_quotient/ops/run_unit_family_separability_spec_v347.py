...
"""v347: IS THE CONTROL SET DOING WORK, OR IS IT PADDING? The same eight members v329 evaluated, with the control set
trimmed from fifteen prepared to those eight -- and the verdicts compared against v329's, member by member.

WHY. Verification cost, not authoring, is the binding constraint on this corpus. The measured curve is quadratic in
family size because every member's prep stays resident and every member is part of the registered control set:
4 prepared 171 s, 7 -> 467, 10 -> 1143, 13 -> 1614, 15 -> 1389 (fewer evaluated), 14 -> ~35 min running now. Proposal
(xii) on AGENT_BOARD -- split the family, or trim the control set to a fixed deterministic sample -- has been
unanswered since 10:51, and it should not be answered by argument. This rung answers it with a measurement, and it
changes NO registered semantics: it is an ordinary separability rung whose FAMILIES list happens to be a subset.
THE DESIGN. v329 evaluated eight members inside a fifteen-member family. This rung evaluates exactly those eight
members with exactly those eight as the family, so each fitted direction must spare seven siblings instead of
fourteen. Everything else -- recipe, bars, seeds, rows, the amended protocol -- is identical, and both receipts are
on disk, so the comparison is per member and not in aggregate.
WHAT EACH OUTCOME MEANS. If the verdicts agree, the seven omitted controls were padding for these members, proposal
(xii)(b) has its evidence, and every future rung in this family can be bounded at eight controls for roughly a third
of the time. If the verdicts DISAGREE -- some member separable here and fused there, or the fam-arm leak materially
larger with fewer controls -- then the control set is doing real work, trimming it would silently inflate the count,
and the honest conclusion is that the cost must be paid or the family split by construction rather than sampled.
I do not know which, and the priors say so.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
    pred_a_new_separable   at least k_new = 1 of the 2 members that were new at v329 is separable here.   prior 85%
    pred_b_counted_kept    at least k_kept = 5 of the 6 counted members remain separable.                 prior 85%
    pred_c_row4_kept       fam-arm own-C UB975 <= 0.01 on at least k_d = 4 of the 8 evaluated members.    prior 70%
    pred_d_instrument      every evaluated member's own-arm extraction is within 0.03 of its parent battery's cdas
                           extraction_held. The own arm does not involve the family at all, so this should be
                           unchanged from v329 and is the sanity check that the trim did not perturb the fit.
                                                                                                          prior 90%
    pred_e_verdicts_agree  every one of the eight members gets the SAME separability verdict as it did at v329, and
                           the fam-arm sibling maximum moves by less than trim_tol = 0.02 in absolute value on at
                           least six of the eight. Both halves are required, because verdicts alone are a coarse
                           readout -- a member sitting at 0.049 and one at 0.005 both read "separable" -- and the
                           margin is what a future rung would be relying on. Worked example: v329 gave away_up_c a fam
                           sib max of 0.0152; if this rung gives 0.0210 that is a move of 0.0058, inside the
                           tolerance, and if it gives 0.0400 that is 0.0248, outside it, and counts against the
                           predicate even though the verdict is unchanged. The 0.02 anchor is the same one v329's
                           density clause used, taken from the two populations v325 measured (smallest genuine
                           cue-donor leak 0.0286, largest non-donor leak 0.0174).                          prior 50%
COUNTING. NOTHING here is counted. Every member has already been counted at v329 under the full control set, and this
rung deliberately uses a weaker one; a "separable" verdict here is evidence about the instrument, not a circuit. If
pred_e fails, the finding is that trimmed control sets are not comparable to full ones and the receipts must not be
mixed -- which is itself the answer proposal (xii)(b) needs.
BARS = {"cross_max": 0.05, "keep": 0.8, "ext_floor": 0.5, "c_ub_max": 0.01, "k_d": 4, "k_new": 1, "k_kept": 5, "instr_tol": 0.03, "trim_tol": 0.02}
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v347_result.json"
...
FAMILIES = {
    "verb_particle": ["verb_particle_out_up", "verb_particle_away_up", "verb_particle_down_in", "verb_particle_up_down_b", "verb_particle_away_up_b", "verb_particle_out_down_b", "verb_particle_away_up_c", "verb_particle_down_out_c"],
}
EVAL = ("verb_particle_away_up_c", "verb_particle_down_out_c", "verb_particle_out_up", "verb_particle_away_up", "verb_particle_down_in", "verb_particle_up_down_b", "verb_particle_away_up_b", "verb_particle_out_down_b", )
COUNTED = ("verb_particle_out_up", "verb_particle_away_up", "verb_particle_down_in", "verb_particle_up_down_b", "verb_particle_away_up_b", "verb_particle_out_down_b", )
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES, **v213.NAMES, **v215.NAMES, **v217.NAMES, **v219.NAMES, **v221.NAMES, **v223.NAMES, **v225.NAMES, **v227.NAMES, **v229.NAMES, **v231.NAMES, **v233.NAMES, **v235.NAMES, **v237.NAMES, **v239.NAMES, **v241.NAMES, **v243.NAMES, **v245.NAMES, **v247.NAMES, **v249.NAMES, **v251.NAMES, **v253.NAMES, **v255.NAMES, **v257.NAMES, **v311.NAMES, **v315.NAMES, **v319.NAMES, **v323.NAMES, **v327.NAMES}


def _parent_cdas(n):
    # the parent battery receipt that produced this member's units (each behaviour name is unique across batteries)
    for path in (V327, V323, V319, V315, V311, V257, V255, V253, V251, V249, V247, V245, V243, V241, V239, V237, V235, V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v347", "members": n, "families": list(FAMILIES),
...
def PREDS(R):
    ...
    full = json.loads((ROOT / "circuits/followups/unit_family_separability_spec_v329_result.json").read_text())["members"]
    same = all(n in good and n in full and good[n]["separable"] == full[n]["separable"] for n in EVAL)
    close = sum(1 for n in EVAL if n in good and n in full
                and abs(abs(good[n]["arms"]["fam"]["sib_abs_max"]) - abs(full[n]["arms"]["fam"]["sib_abs_max"]))
                    < B["trim_tol"])
    e = ok and same and close >= 6
    return {"pred_a_new_separable": bool(a), "pred_b_counted_kept": bool(b), "pred_c_row4_kept": bool(c),
            "pred_d_instrument": bool(d), "pred_e_verdicts_agree": bool(e)}


def main() -> None:
    if os.environ.get("BQLIB_DRYRUN") or os.environ.get("BQLIB_NO_MODEL"):
        print(json.dumps(_plan(), indent=2, sort_keys=True)); return
    smoke = os.environ.get("V347_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V347_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v347", "candidate_id": "corpus.unit_family_separability_spec_v347",
