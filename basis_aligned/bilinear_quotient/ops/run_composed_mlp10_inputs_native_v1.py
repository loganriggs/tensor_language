#!/usr/bin/env python3
# BQGATE:3040bodyforwards;160extraMLP10;160prefixes<=248tokens;300seconds.
"""pred_a prior17armreplay<=1e-4 andinputproductpartition<=1e-10relative.
pred_b generatedinputerror<=.01 andjointnormerror<=.001 eachregional/FineWebpanel.
pred_c generatedfullandjointpreservation<=.02eachregionalgroup.
Null: folding attention partners or the joint denominator amplifies response error.
Price3040bodyforwards160extraMLP10+480FP64attention;160reusedprefixes;300seconds;nofit.
"""
...
from composed_mlp10_inputs_v1 import execute as generate_inputs
STEM='COMPOSED_MLP10_INPUTS_NATIVE_V1'
@torch.no_grad()
def main():
 ...
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('3040forwards160extraMLP10;19arms;integratedMLP9-attention10-inputs');return
 out=P/(STEM+'_RESULT.json');art=P/(STEM+'_ARTIFACT.pt');assert not out.exists() and not art.exists();signal.alarm(300);torch.set_num_threads(2);torch.backends.cuda.matmul.allow_tf32=False
 from fastload import load_model_fast
 model=load_model_fast().cuda().eval();tic=time.perf_counter();w=torch.load(P/'MLP9_CROSSFIRST_RESPONSE_V1_PROGRAM.pt',weights_only=True)['direction'].cuda();child={i:v.cuda() for i,v in torch.load(P/'CROSSFIRST_STATE_EXECUTOR_V1_ARTIFACT.pt',weights_only=True)['fields'].items()};old=torch.load(P/'CROSSFIRST_HIERARCHY_V1_ARTIFACT.pt',weights_only=True);parent={i:v.cuda() for i,v in old['parent_fields'].items()};ctx={};measures=torch.zeros(160,19,2,dtype=torch.float64);statechecks=[];count=0
 ...
 attention_matrices=[getattr(model.transformer.h[10].attn,k).weight.double() for k in ('c_q','c_k','c_q2','c_k2','c_v')];attention_output=model.transformer.h[10].attn.c_proj.weight.double();attention_mixture=float(model.transformer.h[10].attn.lamb);generated_checks=[]
 ...
 def pre10(module,args):
  if ctx['arm']==0:ctx['first10']=args[1].double()
 ...
 def after10(module,args,output):
  ...
  if arm==4:
   ...
   generated=generate_inputs(ctx['z9'],ctx['m9'],raw[0],z[0],ctx['first10'],child[i][...,None],(parent[i]-child[i])[...,None],response_program,float(module.lambdas[0]),attention_matrices,attention_mixture,attention_output)
   gc,gr=generated['child'],generated['remainder'];grho=generated['joint_rho']
   def generated_cross(x,y):
    L,R,D=mlpweights;return ((x@L.T)*(y@R.T)+(y@L.T)*(x@R.T))@D.T/grho
   ctx['generated_full']=generated_cross(gc,gr)
   ctx['generated_joint']=ctx['generated_full']-generated_cross(generated['child_attention'],generated['remainder_attention'])
   generated_checks.append(dict(error_sq=float((gc-(z[1]-z[0])).square().sum()+(gr-(z[3]-z[0])).square().sum()),reference_sq=float((z[1]-z[0]).square().sum()+(z[3]-z[0]).square().sum()),norm_error_sq=float((grho-rho).square().sum()),norm_reference_sq=float(rho.square().sum())))
   total=x.double()-bar;statechecks.append(float((gm+ga-total).norm()/total.norm().clamp_min(1e-30)));return output
  target=ctx['bar']+(ctx['gm'] if arm==6 else ctx['ga'] if arm==7 else ctx['parts']['cross'] if arm==8 else ctx['parts']['normalization'] if arm==9 else ctx['parts']['total'] if arm==10 else ctx['sources']['residual_residual'] if arm==11 else ctx['sources']['residual_attention'] if arm==12 else ctx['sources']['attention_attention'] if arm==13 else ctx['sources']['residual_residual']+ctx['sources']['residual_attention'] if arm==14 else ctx['folded_joint'] if arm==15 else ctx['folded_full'] if arm==16 else ctx['generated_full'] if arm==17 else ctx['generated_joint'] if arm==18 else 0)
 ...
 assert count==3040 and len(statechecks)==160
 prior=torch.load(P/'MLP9_TO_MLP10_RESIDUAL_FOLD_V1_ARTIFACT.pt',weights_only=True)['measures'];rel=lambda a,b:float((a-b).norm()/b.norm().clamp_min(1e-30));replay=[rel(measures[:96,:17],prior[:96]),rel(measures[96:,:17],prior[96:])]
 ...
 generated_errors=[(sum(c['error_sq'] for c in generated_checks[lo:hi])/sum(c['reference_sq'] for c in generated_checks[lo:hi]))**.5 for lo,hi in [(0,96),(96,160)]]
 generated_norm_errors=[(sum(c['norm_error_sq'] for c in generated_checks[lo:hi])/sum(c['norm_reference_sq'] for c in generated_checks[lo:hi]))**.5 for lo,hi in [(0,96),(96,160)]]
 cells=[]
 for lo,hi,label in [(24*k,24*(k+1),'regional'+str(k)) for k in range(4)]+[(96+16*k,112+16*k,'newline'+str(k)) for k in range(4)]:
  ...
  cells.append(dict(generated_full_effect_error=rel(z[:,17]-z[:,5],cross),generated_joint_effect_error=rel(z[:,18]-z[:,5],joint),joint_effect_error=rel(joint,cross),folded_joint_preservation_error=rel(folded,joint),folded_full_effect_error=rel(foldfull,cross),residual_only_error=rel(rr,cross),input_effect_composition_error=rel(rr+mixed+aa,cross),residual_aligned=float((rr*cross).sum()/cross.square().sum()),mixed_aligned=float((mixed*cross).sum()/cross.square().sum()),attention_squared_aligned=float((aa*cross).sum()/cross.square().sum()),formula_effect_error=rel(formula,mlp),cross_only_error=rel(cross,mlp),norm_only_error=rel(norm,mlp),cross_norm_composition_error=rel(cross+norm,formula),cross_aligned=float((cross*mlp).sum()/mlp.square().sum()),norm_aligned=float((norm*mlp).sum()/mlp.square().sum()),maxabs_formula_error=float((formula-mlp).abs().max()),cell=label,MLP_only_error=rel(mlp,full),attention_only_error=rel(att,full),composition_error=float(err.norm()/full.norm().clamp_min(1e-30)),MLP_aligned=float((mlp*full).sum()/full.square().sum().clamp_min(1e-30)),attention_aligned=float((att*full).sum()/full.square().sum().clamp_min(1e-30)),full_norm=float(full.norm()),maxabs_composition=float(err.abs().max())))
 result={'pred_a':max(replay+statechecks)<=1e-4 and max(input_partition_checks)<=1e-10,'pred_b':max(generated_errors)<=.01 and max(generated_norm_errors)<=.001,'pred_c':all(max(c['generated_full_effect_error'],c['generated_joint_effect_error'])<=.02 for c in cells[:4]),'generated_input_errors':generated_errors,'generated_norm_errors':generated_norm_errors,'folded_input_errors':fold_errors,'max_input_partition_error':max(input_partition_checks),'formula_state_errors':panel_errors,'anchor_replay':replay,'max_state_partition_error':max(statechecks),'cells':cells,'body_forwards':count,'extra_MLP10_evaluations':160,'extra_FP64_attention10_evaluations':480,'seconds':time.perf_counter()-tic,'scope':'MLP9 response generates both residual andattention10 input changes andjointMLP10norm. Pristinecontexts, scalarfields, fullweights, additivebackground andnative suffix retained.160reusedrows,nofit/newOOD.'}
 torch.save(dict(measures=measures,formula_checks=formula_checks,fold_checks=fold_checks,generated_checks=generated_checks),art);result['artifact_sha']=digest(art);out.write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result),flush=True)
