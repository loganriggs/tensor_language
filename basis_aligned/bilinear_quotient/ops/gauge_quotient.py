"""THE GAUGE-QUOTIENT TEST (rung 89): are the per-document maps conjugates of ONE shared map?
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['assembler_transfer_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: deps present; per-document map spectra vs raw entries (gauge-quotient rung)")
    raise SystemExit(0)
import torch
...
OUT=PT+'gauge_quotient_results.json'
...
if __name__=='__main__':
    ...
    if Y2.shape[0]!=len(clsflat):
        raise SystemExit('INSTRUMENT FAIL: capture length mismatch')
    import statistics as stt
    SW=CLS.index('subword')
    rows=torch.arange(len(clsflat))//256
    # global top-64 PC bases
    def pcs(Y):
        Yc=Y-Y.mean(0)
        _,_,V=torch.linalg.svd(Yc[:20000],full_matrices=False)
        return V[:64].T
    P2=pcs(Y2); P3=pcs(Y3)
    Z2=(Y2-Y2.mean(0))@P2; Z3=(Y3-Y3.mean(0))@P3
    docs={}
    for r6 in range(120):
    ...
    for i2 in range(n):
        for j2 in range(i2+1,n):
            a=specs[i2]; b=specs[j2]
            cosns.append(float((a@b)/(a.norm()*b.norm()+1e-9)))
