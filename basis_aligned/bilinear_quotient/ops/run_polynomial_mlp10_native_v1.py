...
"""pred_a post10 branch state relative errors<=1e-5 and old generated endpoint maxabs<=1e-4.
...
Null: three-term rational-numerator full-branch execution amplifies numerical errors or breaks prior interaction fidelity.
...
from polynomial_mlp10_branch_v1 import prepare as prepare_diagonal, evaluate as diagonal_post
from regional_cue_row_check_v1 import validate


@torch.no_grad()
def main():
    files=json.loads((P/'POLYNOMIAL_MLP10_NATIVE_V1_BINDING.json').read_text())['files']
    ...
    assert not (P/'POLYNOMIAL_MLP10_NATIVE_V1_RESULT.json').exists()
    ...
    for index in range(160):
        ...
        for amplitude in (a,b,a+b):
            ...
            native.append(post(state));generated.append(diagonal_post(branch(amplitude,context).float().double(),amplitude,diagonal).float())
        outcomes=[]
    ...
    result={'pred_a':max(max(c['state_errors']) for c in cells)<=1e-5 and old_error<=1e-4,
            ...
            'old_generated_endpoint_maxabs':old_error,'native_reference_replay_maxabs':native_replay,'groups':groups,'cells':cells,'row_check':row_check,'seconds':time.perf_counter()-tic,
            'scope':'160 historical prefixes, full original two-edit interaction; generated child/remainder/parent branches, native pristine context/scalar fields/dense weights/suffix. Three-vector degree-two numerator approximation in normalized MLP10 in FP64 after rounded generated z10, output rounded FP32; no freshOOD, fitting or whole-model compression claim.'}
    (P/'POLYNOMIAL_MLP10_NATIVE_V1_RESULT.json').write_text(json.dumps(result,indent=2)+'\n')
