"""circuit_battery_winners_curse_holdout -- select the most-selective component on FIT, pay for it on TEST.

SS2862 found, post-hoc, that the most-selective component under the v6 metric agrees between SELECT and TEST on **0 of 7** behaviours,
...
whichever component's noise was most favourable on the split it was chosen on. SS2862 reported it as exploratory and forbade the
campaign from reading "the most selective component" off a single split. This rung measures the effect honestly instead of avoiding it:
**select on FIT, evaluate on TEST**, and price the inflation.

It also asks the question that decides whether selection is worth anything at all -- does a FIT-selected component beat, on held-out
data, (i) the battery's NAMED writer for that behaviour and (ii) a random live component? Selection can be badly inflated and still
carry signal; those are separate facts and they are registered separately.
...
Preregistration: polynomial_causal/CIRCUIT_BATTERY_WINNERS_CURSE_PREREGISTRATION.md
"""
...
SELF = Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/"
            "circuit_battery_winners_curse_holdout.py")
...
ROOT = R.ROOT
PREREG = R.POLY / "CIRCUIT_BATTERY_WINNERS_CURSE_PREREGISTRATION.md"
...
RUNG = "circuit_battery_winners_curse_holdout"
SMOKE = os.environ.get("SURROGATE_SMOKE") == "1"
OUT = ROOT / (f"{RUNG}_smoke_results.json" if SMOKE else f"{RUNG}_results.json")
HASHES = {PREREG: "e58bcf58fd02760ea59fdcb56d1869bf02cf9843d4af65b7a52e766068353533", NULLRES: "b4bf278a10fea17e656db30e2d138fb219dc3c8cf5319282b307df5b08aa6295",
