...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['twin_mean_suite_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: deps present; DC-ledger zero-vs-mean arms for a8/a5/m13-m16 (damage convention)")
    raise SystemExit(0)
import torch
            ...
            def _mkab(MU):
                def hab(mo,i_,o_):
                    if isinstance(o_,tuple):
                        ...
                        return (r,o_[1])
                    r=(MU.expand_as(o_) if MU is not None else torch.zeros_like(o_)).to(o_.dtype)
                    return r
                return hab
...
if __name__=='__main__':
    ...
    SEL['motif_off']=(); SEL['skipset']=(); SEL['clsdmg']=False; SEL['cap1416']=False; SEL['abl']=None
    COMPS9={'a8':m.transformer.h[8].attn,'a5':m.transformer.h[5].attn,
            'm13':m.transformer.h[13].mlp,'m14':m.transformer.h[14].mlp,
            'm15':m.transformer.h[15].mlp,'m16':m.transformer.h[16].mlp}
    import statistics as stt
    # base arm with mean captures for all six
    caps={k:[] for k in COMPS9}
    _hc2=[]
    for k,mod in COMPS9.items():
        def _mkc(k=k):
            def hc(mo,i_,o_):
                y=o_[0] if isinstance(o_,tuple) else o_
                caps[k].append(y.detach().float().reshape(-1,D).mean(0).cpu())
            return hc
        _hc2.append(mod.register_forward_hook(_mkc()))
    print('ARM base (capturing means)',flush=True)
    rb=main()
    for h in _hc2: h.remove()
    MUS={k:torch.stack(v).mean(0).to(DEV) for k,v in caps.items()}
    res={}
    for k,mod in COMPS9.items():
        row={}
        for mode,mu in (('zero',None),('mean',MUS[k])):
            ...
            ra=main()
            d=ra['L2_F']-rb['L2_F']
            if abs(d)<1e-5: raise SystemExit(f'INSTRUMENT FAIL: {k}/{mode} identical to base')
            row[mode]=round(d,4)
        SEL['abl']=None
        if row['zero']<row['mean']-0.01:
            raise SystemExit(f'INSTRUMENT FAIL: {k} zero < mean by more than 0.01')
    ...
    shares=[res[k]['dc_share'] for k in res]
    pa=stt.median(shares)>=0.5
    pb=res['a8']['dc_share']>=0.5
    pc=abs(rb['L2_F']-2.6662)<=0.01
    out={'base_L2F':rb['L2_F'],'components':res,'median_dc_share':round(stt.median(shares),3),
         'convention':'damage on the full S2144 base; dc_share = 1 - d(mean)/d(zero)',
         'pred_a_dc_dominates':bool(pa),'pred_b_a8_too':bool(pb),'pred_c_reproduces_s2144':bool(pc),
    ...
    print(f"(b) a8 DC share {res['a8']['dc_share']:.3f} >= 0.5: {'HELD' if pb else 'FAILED'}")
    print(f"(c) base {rb['L2_F']} vs 2.6662 (tol 0.01): {'HELD' if pc else 'FAILED'}")
