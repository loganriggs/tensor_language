"""COMPUTE-SPARSE FRONTIER (rung 251): the 11-cert point + per-token top-1152 MLPs.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. Additivity held for the corner
(S2343: surcharge +0.0168). Same arithmetic on the new frontier {motifs exact, tail r96} (0.0553/11):
predict ~0.072.
REGISTERED PREDICTIONS (arms: THIS vs ct96 0.0553/11 and corner-topk 0.0776/7):
  (a) NEAR-ADDITIVE: census <= 0.075.
  (b) CERTIFICATES: valid >= 8.
  (c) SANITY: census in [0.01, 0.15]; inert tripwires vs ct96 AND corner-topk receipts.
NULL: sparsity interacts at the frontier (census >= 0.085 or valid <= 6). PRICE: 211M storage + 4x MLP
compute sparsity. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: compute-sparse frontier')
    raise SystemExit(0)
import torch
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['cp_topk']=1152; SEL['qk_r']=96; SEL['qk_rmap']={li:128 for li in range(2,10)}; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    pb=nv>=8
    pc=0.01<=agg<=0.15
    if abs(agg-0.0553)<1e-4 or abs(agg-0.0776)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: ct96-topk inert')
    ...
    print(f"(c) census in [0.01, 0.15]; era rho {rho:.4f}: {'HELD' if pc else 'FAILED'}")
