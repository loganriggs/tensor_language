#!/usr/bin/env python3
# BQGATE:0bodyforwards,0textsequences;4residualeigensolves,32targetmatrixactions,128cachedvalidationvectors.
"""pred_a rank16 replay<=1e-8 and eigen residual<=1e-5;
pred_b full-quadratic net capture gain>=30%; pred_c native write error<=20%.
One full symmetric quadratic plus31rank16quadratics; explicit higher price.
...
STEM='QUARTIC_FULL_QUADRATIC_CANDIDATE_V1'
@torch.no_grad()
def main():
 binding=json.loads((P/(STEM+'_BINDING.json')).read_text())['files'];assert all(digest(k)==v for k,v in binding.items())
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
  print(json.dumps(dict(gpu_accessed=False,body_forwards=0,text_sequences=0,residual_directions=4,target_matrix_actions=32,maximum_eigen_operator_actions=808)));return
 ...
 old=json.loads((P/'QUARTIC_RESIDUAL_EIGEN_CANDIDATE_V1_RESULT.json').read_text())
 replay=max(abs(best['rank16'][0]['net_capture_gain_fraction']-old['coefficient_gain_fraction']),abs(best['rank16'][0]['native_write_error']-old['final_native_error']))
 winner,program=best['full'];assert program is not None
 q,adjusted,alpha,write=program;coordinates=SymmetricCoordinates(1152,'cuda')
 torch.save(dict(seed=11511,input_readers=br.cpu(),inner_weights=nr.cpu(),mixing=adjusted.cpu(),full_quadratic_packed=coordinates.pack(q).cpu(),full_quadratic_mixing=alpha.cpu(),output_writers=writer.cpu(),native_write=write.cpu()),ap)
 result={'pred_a':replay<=1e-8,'pred_b':winner['net_capture_gain_fraction']>=.3,'pred_c':winner['native_write_error']<=.2}
 result.update(dict(rank16_replay_error=replay,old_capture=capture,removed_node=weak,baseline_native_error=baseline_error,best_rank16=best['rank16'][0],best_full=winner,rows=rows,eigen_reports=eigen_reports,
  fitted_floats=br.numel()+nr.numel()+adjusted.numel()+coordinates.size+alpha.numel()+writer.numel(),validation_write_floats=write.numel(),
  artifact_sha256=digest(ap),wall_seconds=time.perf_counter()-start,peak_gpu_bytes=torch.cuda.max_memory_allocated(),scope='Untruncated versus rank16 coefficient-selected quadratic candidate; full dense symmetric matrix explicitly charged. Cached native writes only, no nonlinear fitting/OOD or circuit claim.'))
