...
BINDING = POLY / "SUCCESSOR_POINTER_PREFIX_INTERACTION_V2_BINDING.json"
OUT = POLY / "SUCCESSOR_POINTER_PREFIX_INTERACTION_V2_RESULT.json"
...
def load_bound():
    ...
    bound_paths = {
        "rows": ROWS, "preregistration": PREREG, "prior_art": PRIOR,
        "correction": POLY / "SUCCESSOR_POINTER_PREFIX_INTERACTION_V2_INSTRUMENT_CORRECTION.md",
        "v1_binding": POLY / "SUCCESSOR_POINTER_PREFIX_INTERACTION_V1_BINDING.json",
        "v1_runner": OPS / "run_successor_pointer_prefix_interaction_v1.py",
        "v1_invalid_result": POLY / "SUCCESSOR_POINTER_PREFIX_INTERACTION_V1_RESULT.json",
    }
    for name, expected in binding["files"].items():
        path = bound_paths[name]
...
def compile_plan():
    ...
    return {
        "schema": "successor_pointer_prefix_interaction_v2_plan",
        "candidate_id": "successor_pointer_prefix_interaction_v2",
        "model_loaded": False,
        "gpu_accessed": False,
        "queue_touched": False,
        "rows": rows["row_count"],
...
def main():
    ...
    def full_batch_static_v1(token_matrix):
        # Match semlib.run's layer-0 GEMM shape and operation order exactly.
        dimension, heads = cfg["n_embd"], cfg["n_head"]
        state = F.rms_norm(model.transformer.wte(token_matrix), (dimension,))
        first_state = state
        block = model.transformer.h[0]
        state = block.lambdas[0] * state + block.lambdas[1] * first_state
        values = block.attn.c_v(F.rms_norm(state, (dimension,)))
        return values.view(*token_matrix.shape, heads, dimension // heads)

    native_static_v1 = full_batch_static_v1(tokens)
    recipient_v1 = native_static_v1[ar, last]
    self_sub = {
    ...
    donor_value_tokens = directed_tokens.clone()
    donor_value_tokens[torch.arange(len(directed), device=device), directed_last] = donor_tokens
    donor_static_v1 = full_batch_static_v1(donor_value_tokens)
    donor_v1 = donor_static_v1[torch.arange(len(directed), device=device), directed_last]
    donor_sub = {
    ...
    result = {
        "schema": "successor_pointer_prefix_interaction_v2_result",
        "candidate_id": "successor_pointer_prefix_interaction_v2",
        "terminal": terminal,
        ...
        "counts": {
            ...
            "static_value_projection_token_evaluations": 900,
            "fits": 0,
            "backwards": 0,
            "parameter_updates": 0,
        },
        "claim_boundary": (
            "V2 same-batch exactness correction; factorial screen of exact pointer substitution and matched prefix coherence; "
