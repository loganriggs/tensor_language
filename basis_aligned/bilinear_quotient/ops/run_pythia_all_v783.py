...
"""RANK NEEDED AT THE OTHER END OF TRAINING (v783). v782 priced the late model (Pythia-160m final, 300B tokens): recovery 0.932 at rank 32, 0.947 at rank 40, 0.955 at rank 48 of a 64-dim head - it needs most of its read-side directions. The early checkpoint reaches 0.947 at rank 16 and 0.982 at rank 32 (v766), so its crossing of 0.95 sits somewhere near rank 16-20. This rung fits rank 20 and rank 24 on step 4000 to pin 'the rank a model needs for recovery 0.95' at both ends of the training curve, which is the ladder's finding stated as a compression ratio rather than a CE. MODEL = EleutherAI/pythia-160m revision step4000. ROW-CENTRED logits (Logan, 22 Sep: 'test the specific hypothesis of why we're seeing what we're seeing'). MODEL = EleutherAI/pythia-160m (12 x 12 x 768, softmax, ROTARY on 16 of 64 head dims, GPT-NeoX tokenizer;
...
PREDICTIONS (scored as written; failures preserved)
    ...
    pred_c_rank32_cost          (rank 24) snapshot recovery >= 0.96 at step 4000 (rank 16: 0.947, rank 32: 0.982). Prior: likely
    pred_d_rank16_cost          (rank 20) snapshot recovery >= 0.95 at step 4000 - i.e. the early model needs ~2.5x less rank than the late one. Prior: likely
    pred_e_rank32_values_preserved_scale rank-32: median value ratio in [0.5, 2] over the 24 most valuable heads. Prior: likely
PRICE (registered maximum): stock 7 + explicit 7; means 16; kernels 2; 144 x 7 = 1008; all-ablated 7; kernels-only 7 + col0 7; ladder 35; 2 arms x (kappa_r 26 + step-0 10 + 300 + 12 x (26 + 7 + 7) + snapshot 9) = 1650 + manipulability 168; total ~3000 forwards, 600 backwards. Bars: forwards <= 3400, backwards <= 600.
"""
...
REVISION = "step4000"
TAG = "pythia160m_step4000_rank_v783"
...
FORWARDS_MAX, BACKWARDS_MAX = 3400, 600
EBATCH, TBATCH, STEPS, EVAL_EVERY, N_VAL, N_MANIP = 32, 8, 300, 25, 96, 24
LR_MAP, LR_MAP_MIN, LR_K, LR_K_MIN = 3e-4, 3e-5, 0.01, 0.001
KS, ARMS = (1, 2, 4, 8, 16), (20, 24)
REPLAY_TOL, E4_MIN, REC24, REC20, RATIO = 0.002, 0.90, 0.96, 0.95, (0.5, 2.0)
PREDICTIONS = {"pred_a_instrument_replays": "+-0.002", "pred_b_kernel_energy_top4": ">= 0.90", "pred_c_rank32_cost": "rank 24 recovery >= 0.96", "pred_d_rank16_cost": "rank 20 recovery >= 0.95", "pred_e_rank32_values_preserved_scale": "median ratio in [0.5, 2]"}
...
def main() -> None:
    plan = {"candidate_id": CANDIDATE_ID, "forwards_max": FORWARDS_MAX, "model_backwards": BACKWARDS_MAX, "model_updates": 0, "fit_parameters": "2 arms of factored q/k maps + kernels",
            ...
            "bars": {"replay_tol": REPLAY_TOL, "e4_min": E4_MIN, "rec24": REC24, "rec20": REC20, "ratio": RATIO}}
    ...
    r48, r40 = results[24], results[20]
    predictions = {"pred_a_instrument_replays": abs(native - native_stock) <= REPLAY_TOL, "pred_b_kernel_energy_top4": float(energy[3]) >= E4_MIN, "pred_c_rank32_cost": r48["recovery"] >= REC24,
                   "pred_d_rank16_cost": r40["recovery"] >= REC20, "pred_e_rank32_values_preserved_scale": RATIO[0] <= r48["median_ratio"] <= RATIO[1]}
    ...
    OUT.write_text(json.dumps({"schema": f"pythia_all_result_{TAG}", "candidate_id": CANDIDATE_ID, "plan": plan,
                               "report": {"model": REPO, "revision": REVISION, "geometry": {"L": L, "H": H, "D": D, "hd": hd, "rot": rot}, "native_stock": native_stock, "native": native, "mean_ablation_cost": value, "median_head": median, "joint_value": joint,
