#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_block_11_leads pred_c_verb_reader_blocks_carry_half pred_d_early_blocks_small pred_e_singles_add
"""The attention channel into the verb, by block (v476). v475: every block's attention write into a distant verb position closes 0.94 of the agreement
margin. Here each block's attention write into the answer position swapped singly (blocks 0..17), 79 distant rows: the verb-side mirror of v447 (exits from
the noun by block: 4, 11, 7, 9, 13). Attention writes into one position at different blocks are exits of different states, so they should add.
PREDICTIONS (scored as written; failures preserved; priors from v447)
    pred_a_baseline_replays          the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_block_11_leads            block 11's attention write into the verb is the largest single-block effect. Prior: likely (11.3)
    pred_c_verb_reader_blocks_carry_half  blocks 7 + 9 + 11 + 13 together close >= 0.45
    pred_d_early_blocks_small        blocks 0-3 together close <= 0.08
    pred_e_singles_add               the 18 singles sum to within 0.25 of 0.943
...
OUT = ROOT / "circuits/followups/attn_into_verb_blocks_v476_result.json"
CANDIDATE_ID = "chain.attn_into_verb_blocks_v476"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, READERS_MIN, EARLY_MAX, ALL_V475, ADD_TOL, UNIT = 1e-3, 0.45, 0.08, 0.943, 0.25, 701
import random
...
FORWARDS_MAX = 58
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_block_11_leads": "block 11 first", "pred_c_verb_reader_blocks_carry_half": ">= 0.45", "pred_d_early_blocks_small": "<= 0.08", "pred_e_singles_add": "within 0.25 of 0.943"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "readers_min": READERS_MIN, "early_max": EARLY_MAX, "all_v475": ALL_V475, "add_tol": ADD_TOL, "unit": UNIT}}
    ...
    MODES = {f"a{b}": ({}, (0,)) for b in range(18)}; forwards = 0
    def logits_margin(x):
    ...
    mc = verb["margin_closed"]; total = sum(mc.values())
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_block_11_leads": max(mc, key=mc.get) == "a11", "pred_c_verb_reader_blocks_carry_half": mc["a7"] + mc["a9"] + mc["a11"] + mc["a13"] >= READERS_MIN,
                   "pred_d_early_blocks_small": sum(mc[f"a{b}"] for b in range(4)) <= EARLY_MAX, "pred_e_singles_add": abs(total - ALL_V475) <= ADD_TOL}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "attn_into_verb_blocks_result_v476", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
