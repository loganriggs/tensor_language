#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_5_7_leads_block_5 pred_c_11_3_leads_block_11 pred_d_one_head_per_block pred_e_singles_add_to_blocks
"""Blocks 5 and 11 into the verb, by head (v477). v476: the attention writes into a distant verb position are led by block 11 (0.25) and block 5 (0.16);
block 5 was not a noun exit (0.03, v447), so it reads the post-noun rebuild. The atlas 'number' family lists 5.7 and 11.3. Here each head's write into the
answer position swapped singly for blocks 5 and 11 (the head's slice of the pre-projection activation swapped within the pair before c_proj), 79 distant rows.
PREDICTIONS (scored as written; failures preserved; priors from the atlas and v448 / v476)
    pred_a_baseline_replays     the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_5_7_leads_block_5    head 5.7 has the largest single-head effect in block 5. Prior: likely (atlas)
    pred_c_11_3_leads_block_11  head 11.3 has the largest single-head effect in block 11 (v448 by value at the noun; here by write into the verb)
    pred_d_one_head_per_block   in each block the largest head carries >= 0.60 of the block's effect (0.160 / 0.250)
    pred_e_singles_add_to_blocks  the nine single-head effects sum to within 0.04 of the block's effect, for both blocks
PRICE (registered maximum): 3 batches x 19 passes = 57 forwards; 0 backwards; 0 fits. Bar <= 58.
"""
...
OUT = ROOT / "circuits/followups/attn_into_verb_heads_v477_result.json"
CANDIDATE_ID = "chain.attn_into_verb_heads_v477"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, TOP_SHARE_MIN, ADD_TOL, B5, B11, UNIT = 1e-3, 0.60, 0.04, 0.160, 0.250, 701
import random
...
FORWARDS_MAX = 58
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_5_7_leads_block_5": "5.7 first", "pred_c_11_3_leads_block_11": "11.3 first", "pred_d_one_head_per_block": ">= 0.60 x 2", "pred_e_singles_add_to_blocks": "within 0.04 x 2"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "top_share_min": TOP_SHARE_MIN, "add_tol": ADD_TOL, "b5_v476": B5, "b11_v476": B11, "unit": UNIT}}
    ...
    mc = verb["margin_closed"]; b5 = {m: mc[m] for m in mc if m.startswith("5.")}; b11 = {m: mc[m] for m in mc if m.startswith("11.")}; report["sums"] = {"5": sum(b5.values()), "11": sum(b11.values())}
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_5_7_leads_block_5": max(b5, key=b5.get) == "5.7", "pred_c_11_3_leads_block_11": max(b11, key=b11.get) == "11.3",
                   "pred_d_one_head_per_block": max(b5.values()) >= TOP_SHARE_MIN * B5 and max(b11.values()) >= TOP_SHARE_MIN * B11, "pred_e_singles_add_to_blocks": abs(sum(b5.values()) - B5) <= ADD_TOL and abs(sum(b11.values()) - B11) <= ADD_TOL}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "attn_into_verb_heads_result_v477", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
