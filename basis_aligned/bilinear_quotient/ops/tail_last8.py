"""THE LAST-8 HYBRID (rung 252): tail maps = top-96 PLUS the smallest 8 singular directions.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The cliff decomposition (S2348/S2349):
missing the last 8 directions costs 0.0526; missing the 97-120 band costs ~0.003 (0.0553-0.0526, if
additive). So a 104-direction map keeping {top 96} + {121-128} should land near 0.005-0.01 at 74.2M
pattern values - CHEAPER than the 11-cert point and far better, if the spectrum decomposes additively.
REGISTERED PREDICTIONS (arms: THIS vs top96-only 0.0553/11 and top120 0.0526/11):
  (a) THE CLIFF MASS IS LITERALLY THE LAST 8: census <= 0.015.
  (b) CERTIFICATE HARVEST: valid >= 15.
  (c) SANITY: census in [0.001, 0.12]; inert tripwires vs both anchors.
NULL: the spectrum does not decompose additively - the 97-128 band matters jointly (census >= 0.045).
PRICE: 74.2M pattern values (tail 104/128 + motifs exact). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: tail last-8 hybrid')
    raise SystemExit(0)
import torch
...
OUT=PT+'tail_last8_results.json'
...
if __name__=='__main__':
    ...
    if abs(agg-0.0553)<1e-4 or abs(agg-0.0526)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: last8 map inert')
    ...
    print(f"(a) census {agg:+.4f} <= 0.015: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 15; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
