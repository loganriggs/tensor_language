"""DOES c6-c9 PRUNING GENERALIZE TO cfgE, AND DOES IT COMPOSE WITH THE TOP-8 SELECTOR (rung 48).

CONVENTION (S2135): cfgE gap numbers are CE added above the real model - lower is better.

S2139/S2140: on the S312 frontier, c6-c9's norm-ranked bottom 1728 units each are noise - pruning to 576 beats
the full config on 8/8 windows. The mechanism reading (approximation noise the downstream readers amplify) was
flagged unproven. If it is right, cfgE - whose downstream readers are REAL attention - should show the same
pruning benefit; if pruning only helps where readers are dictionaries, this fails and the scoping sharpens like
S2132 did for conditioning. Also tests whether the two standing positives compose. ARMS: plain cfgE (norm-2304
everywhere) | prune (c6-c9 at 576, norm rank) | top8 (S2124 reproduction, stream-matched) | top8+prune.
PRICE: -8.6M values for the pruned arms (4 x 1728 x 1245). NULL: S2132's precedent - frontier-only effects.

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) PRUNING GENERALIZES: median over the 8 windows of [gap_w(prune) - gap_w(plain)] <= 0.
  (b) THE POSITIVES COMPOSE: median [gap_w(top8+prune) - gap_w(top8)] <= 0.
  (c) REPRODUCTION: median [gap_w(plain) - gap_w(top8)] reproduces 0.0857 within 0.01 (stream matched).

Writes cfge_c69_prune_results.json. Self-reviewed. RUNG-44 HEADER FOLLOWS.

MC-SAMPLE ROBUSTNESS OF THE cfgE TOP-8 SELECTOR (rung 44).

CONVENTION (S2135): cfgE gap numbers are CE added above the real model - lower is better; a "gain" below is a
DAMAGE REDUCTION, gap(plain) - gap(arm).

The one standing positive result is the label-free top-8 selector on cfgE (-0.0857 median damage, S2116/S2124),
built from a 2-sample MC true Fisher; S2130/S2132 measured ~0.01 cross-stream sensitivity of that number. This
rung bounds the instrument: recompute the top-8 at sites 5/6 with FOUR samples per position (fresh generator,
seed 44) and compare subspace, selection and realized damage reduction against the stream-matched 2-sample eight.
Arms: plain (norm cfgE) | top8-2s (S2124 reproduction, stream-matched) | top8-4s. Price: none (instrument rung).
NULL: if the 4-sample eight moves the realized reduction by > 0.01, the certified 0.0857 carries an MC error bar
to be stated on the record.

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) SUBSPACE STABILITY: overlap(top8-2s, top8-4s) >= 0.7 at both sites (mean squared cosine of the 8 principal
      angles).
  (b) GAIN STABILITY: | median_w[gap_w(plain) - gap_w(top8-4s)] - median_w[gap_w(plain) - gap_w(top8-2s)] | <= 0.01.
  (c) REPRODUCTION: median_w[gap_w(plain) - gap_w(top8-2s)] reproduces 0.0857 within 0.01 (stream matched).

Writes cfge_top8_samples_results.json. Self-reviewed. RUNG-38 HEADER FOLLOWS.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    if not os.path.exists(_bq0+'cfge_top8_samples_results.json'):
        print('DRYRUN FAIL: S2130 artifact absent'); raise SystemExit(1)
    _p0=json.load(open(_bq0+'cfge_top8_samples_results.json'))
    print(f"DRYRUN OK: S2138 reduction {_p0['reduction_2s_median']}; cfgE pruning + composition arms x eight windows")
    raise SystemExit(0)
if os.environ.get('BQLIB_DRYRUN')=='1':
...
OUT=PT+'cfge_c69_prune_results.json'
...
def main():
    ...
    def build_arm(mode,K=2304):
        def fit_res(li,piece,active):
            ...
            if mode in ('plain','metric-units','metric-units-all','top8','topr50','prune','top8prune'):
        ...
        def cp_piece(li,piece):
            ...
            keep=imp.argsort(descending=True)[:(K if li in (4,5) else K69M[mode])]
            return ('cp',li,L[keep].contiguous(),Rw[keep].contiguous(),Dw[:,keep].contiguous(),db)
        tag={'plain':'E','metric':'M','metric-bases':'B','metric-units':'U','metric-units-all':'A','random-metric':'R','top8':'T','topr50':'F','prune':'P','top8prune':'X'}[mode]
        act=['a0']
    ...
    W1=FW[R0:R1]
    arms={}; OVL={}; CONDH={}
    K69M={'plain':2304,'prune':576,'top8':2304,'top8prune':576}
    for mode in ('plain','prune','top8','top8prune'):
    ...
    for wi,w in enumerate(WIN):
        ...
        rows.append({'window':wi,'docs':w['docs'],'base_ce':round(b,5),'gap_plain':round(g['plain'],5),
                     'gap_prune':round(g['prune'],5),'gap_top8':round(g['top8'],5),
                     'gap_top8prune':round(g['top8prune'],5)})
        print(f"window {wi}: docs {w['docs']:3d} base {b:.4f} | gaps plain {g['plain']:+.4f} prune {g['prune']:+.4f} "
              f"top8 {g['top8']:+.4f} top8+prune {g['top8prune']:+.4f}",flush=True)
    import statistics as stt
    dpr=[round(r['gap_prune']-r['gap_plain'],5) for r in rows]
    dcm=[round(r['gap_top8prune']-r['gap_top8'],5) for r in rows]
    red=[round(r['gap_plain']-r['gap_top8'],5) for r in rows]
    pa=stt.median(dpr)<=0.0
    pb=stt.median(dcm)<=0.0
    pc=abs(stt.median(red)-0.0857)<=0.01
    out={'windows':rows,'prune_effect_median':round(stt.median(dpr),4),'compose_effect_median':round(stt.median(dcm),4),
         'top8_reduction_median':round(stt.median(red),4),
         'convention':'gaps are CE above the real model; lower is better; negative effect = pruning helped',
         'pred_a_pruning_generalizes':bool(pa),'pred_b_positives_compose':bool(pb),'pred_c_reproduces_s2124':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-t0,1)}
    json.dump(out,open(OUT,'w'),indent=1)
    print(f'prune - plain per window: {dpr}')
    print(f'top8+prune - top8 per window: {dcm}')
    print(f"(a) pruning effect median {stt.median(dpr):+.4f} <= 0: {'HELD' if pa else 'FAILED'}")
    print(f"(b) composition effect median {stt.median(dcm):+.4f} <= 0: {'HELD' if pb else 'FAILED'}")
    print(f"(c) top8 reduction {stt.median(red):+.4f} vs 0.0857 (tol 0.01): {'HELD' if pc else 'FAILED'}")
