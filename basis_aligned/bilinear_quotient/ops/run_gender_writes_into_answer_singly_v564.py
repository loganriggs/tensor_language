#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_10_1_writes_most pred_c_singles_add pred_d_10_5_small pred_e_9_6_writes
"""The gender readers' writes into the answer, singly (v564). v563: the five pronoun readers' writes into the answer position close 0.878 of the he - she gap
jointly on 61 natural gender pairs; by their values at the noun (v551) the order was 10.1 0.255 > 12.4 0.170 > 9.6 0.107 > 15.1 0.055 > 10.5 0.015. Here each
reader's write into the answer swapped alone.
PREDICTIONS (scored as written; failures preserved; priors from v551 / v563)
    pred_a_baseline_replays   the unedited manual forward replays the model's he - she margins within 1e-3
    pred_b_10_1_writes_most   10.1's write into the answer closes more than any other reader's
    pred_c_singles_add        the five singles sum to 0.878 +- 0.10 (writes into one position add; v485)
    pred_d_10_5_small         10.5's write closes <= 0.05
    pred_e_9_6_writes         9.6's write closes >= 0.10
...
OUT = ROOT / "circuits/followups/gender_writes_into_answer_singly_v564_result.json"
CANDIDATE_ID = "gender.writes_into_answer_singly_v564"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, JOINT_V563, BAND, SMALL, MIN96, UNIT = 1e-3, 0.878, 0.10, 0.05, 0.10, 701
...
FORWARDS_MAX = 13
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_10_1_writes_most": "10.1 first", "pred_c_singles_add": "0.878 +- 0.10", "pred_d_10_5_small": "<= 0.05", "pred_e_9_6_writes": ">= 0.10"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "joint_v563": JOINT_V563, "band": BAND, "small": SMALL, "min96": MIN96, "unit": UNIT}}
    ...
    MODES = {f"{l}.{h}": ({}, (0,)) for l, h in HEADS}; SETS_W = {f"{l}.{h}": {l: [h]} for l, h in HEADS}; forwards = 0
    def logits_margin(x):
    ...
    print("writes into the answer singly", {k: round(v, 3) for k, v in mc.items()}, "sum", round(sum(mc.values()), 3))
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_10_1_writes_most": max(mc, key=mc.get) == "10.1", "pred_c_singles_add": abs(sum(mc.values()) - JOINT_V563) <= BAND,
                   "pred_d_10_5_small": abs(mc["10.5"]) <= SMALL, "pred_e_9_6_writes": mc["9.6"] >= MIN96}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "gender_writes_into_answer_singly_result_v564", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
