#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_replay pred_b_carry_mean pred_c_frozen
...
def main():
 ...
 from native_path_error_terms import split,control
 from pairwise_reader_graph import source_reads
 plan=json.loads((P/'NATIVE_PATH_ERROR_PLAN_V1.json').read_text())
 ...
 for family in ('natural','hybrid','change'):
  for cohort in ('all','continuation','spaced_word'):
   ...
   for candidate in ('graph','separate'):
    ...
    gram=sum((torch.tensor(r['gram'],dtype=torch.float64) for r in rows),torch.zeros(6,6,dtype=torch.float64))
    ...
    result[candidate]=dict(gram=gram.tolist(),total_energy=energy,diagonal_energy=float(gram.diag().sum()),cross_energy=float(gram.sum()-gram.diag().sum()),term_means=(sum((torch.tensor(r['term_sums'],dtype=torch.float64) for r in rows),torch.zeros(6,dtype=torch.float64))/sum(r['sites'] for r in rows)).tolist())
   summaries.append(result)
 failures=[r for r in summaries if r['cohort']=='spaced_word' and r['family'] in ('natural','hybrid')]
 pred=dict(pred_a_replay=max(term_checks+energy_replay)<1e-8,pred_b_carry_mean=all(abs(r['graph']['term_means'][0]-r['separate']['term_means'][0])>=max(abs(r['graph']['term_means'][i]-r['separate']['term_means'][i]) for i in range(1,6)) for r in failures),pred_c_frozen=True)
 out=dict(plan=plan,records=records,summaries=summaries,source_replay=max(checks),term_replay=max(term_checks),energy_replay=max(energy_replay),predictions=pred,seconds=time.monotonic()-start,term_order=['carry_times_second_error','source_times_second_error','alpha_times_second_error','first_error_times_source','beta_times_first_error','error_product'])
 (P/'NATIVE_PATH_ERROR_V1.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(dict(predictions=pred,summaries=summaries)),flush=True)
