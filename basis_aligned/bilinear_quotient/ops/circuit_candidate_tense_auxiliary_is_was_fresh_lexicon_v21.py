#!/usr/bin/env python3
"""Prospective history-disjoint v21 bank for the next read/write circuit test."""
...
_AGENTS = (
    "paleoclimatologist", "paleographer", "phytopathologist", "psychometrician",
    "dendrochronologist", "geodesist", "geophysicist", "paleobotanist",
    "paleozoologist", "petrographer", "phonologist", "taxonomist",
    "thermologist", "urbanologist", "xylologist", "chronologist",
)
...
def _panel(group_number: int) -> list[dict[str, Any]]:
    ...
    group_id = f"FIT:{canonical_sha256([SCHEMA, TASK_ID, 'read_write_confirm_v21', SEED, group_number])[:24]}"
    sentence_types = (
    ...
    common_no_vocab = dict(
        ...
        reporter=agent, alternate_reporter=alternate, adjective="read_write_confirm_v21",
        object_name="present_day_gone_by_current_former_age", spec=TASK_SPEC,
    )
    common = dict(common_no_vocab, vocabulary=READOUT)
    return [
        base.builder._row(
            **common, transform_id="A1", construction_id="present_day_days_gone_by_auxiliary_v21",
        ...
        base.builder._row(
            **common, transform_id="A2", construction_id="current_former_age_auxiliary_v21",
            direction_id=direction, matched_suffix=f", the {agent}",
        ...
        base.builder._row(
            **common, transform_id="P", construction_id="day_age_paraphrase_auxiliary_v21",
...
def _history_modules():
    modules = [
        importlib.import_module(f"circuit_candidate_aspectual_different_readout_is_was_v{i}")
        for i in range(1, 4)
    ]
    modules.extend(importlib.import_module(
        f"circuit_candidate_tense_auxiliary_is_was_fresh_lexicon_v{i}") for i in range(4, 21))
...
def _validate(rows: Sequence[Mapping[str, object]]) -> str:
    ...
    if materialized != _build():
        raise CandidateBankError("rows differ from sealed v21 authority")
    try:
    ...
    if (len(materialized) != 64 or len(ids) != 64 or ids & old_ids
        ...
        raise CandidateBankError("v21 count, novelty, or construction check failed")
    return digest
