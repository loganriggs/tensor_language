"""Capability-first temporal authority for multi-task cDAS confirmation."""
...
_head = lambda index: lex._REPORTERS[(index + 19) % len(lex._REPORTERS)][1]
_alternate = lambda index: lex._REPORTERS[(index + 19) % len(lex._REPORTERS)][0]
_object_a = lambda index: lex._OBJECTS[(index + 83) % len(lex._OBJECTS)]
_object_b = lambda index: lex._OBJECTS[(index + 97) % len(lex._OBJECTS)]
_cue = lambda future: "Next year" if future else "Last year"

SPEC = bs.BehaviourSpec(
    task_id="temporal_auxiliary.will_vs_had.fresh_next_last_year",
    vocabulary=(" will", " had"),
    generator_role="generate_capability_first_temporal_auxiliary_panels",
    answer_role="score_jointly_tokenized_will_versus_had",
    a1=bs.Family(
        "capability_first_bare_frame", "fresh_next_last_year_swap",
        lambda index, future: f"{_cue(future)} the {_head(index)} beyond the {_object_a(index)}",
    ),
    a2=bs.Family(
        "capability_first_schedule_frame", "fresh_schedule_next_last_year_swap",
        lambda index, future: (
    ...
    p_generator_role="capability_first_head_noun_rewrite",
)
...
def validate_rows(rows, *, task_id=TASK_ID, groups=bs.DEFAULT_GROUPS, seed=20261027):
    ...
    if len(targets) != 2 * groups:
        raise bs.CandidateBankError("capability-first target population changed")
    if any(row[key] in old_text for row in targets for key in ("base_text", "donor_text")):
        raise bs.CandidateBankError("capability-first target text overlaps an earlier authority")
    for row in targets:
        ...
        if len(row["base_ids"]) != len(row["donor_ids"]) or len(differences) != 1:
            raise bs.CandidateBankError("capability-first pair is not aligned at exactly one cue token")
    return digest


def build_rows(task_id=TASK_ID, groups=bs.DEFAULT_GROUPS, seed=20261027):
