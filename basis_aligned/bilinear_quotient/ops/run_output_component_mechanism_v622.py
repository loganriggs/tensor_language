...
"""Disambiguate the frozen leading output-block component.
...
OUT=BQ/'circuits/followups/output_component_mechanism_v622_result.json'
...
def main():
    ...
    for panel in plan['panels']:
        ...
        for start in range(0,8,4):
            ...
            n,y=cache['n'],cache['y'];alpha=cache['write']@reader
            ...
            if start==0:
                def edit(_m,args,out):return out-(out.float()@reader)[...,None]*v
    ...
    result=dict(plan=plan,rows=rows,trace_Q=float(trace),maximum_replay_relative_error=max(checks),
    ...
    disk_guard.guard_write(100000,label='v622 JSON');OUT.write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2))
