...
"""v577: the step that actually moves the count -- within-family separability for verb_preposition_at_amid.
...
REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
  ...
  pred_b_counted_kept  at least k_kept = 8 of the ten counted members stay separable in the same run, so a null on
                       at_amid cannot be blamed on the family having stopped separating.               prior 88%
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v577_result.json"
...
EVAL = ("verb_preposition_at_amid", )
COUNTED = tuple(['verb_preposition', 'verb_preposition_insisted', 'verb_preposition_to_at', 'verb_preposition_with_about', 'verb_preposition_of_with', 'verb_preposition_from_for', 'verb_preposition_on_about', 'verb_preposition_to_about', 'verb_preposition_at_from', 'verb_preposition_on_with'])
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v577", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V577_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V577_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    evaluate = [n for n in members if n in EVAL and (not smoke or n in os.environ.get("V577_SMOKE_NAMES", "verb_preposition_at_amid").split(","))]
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v577", "candidate_id": "corpus.unit_family_separability_spec_v577",
