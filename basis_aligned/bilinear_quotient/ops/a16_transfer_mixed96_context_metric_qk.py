"""RUNG 333 -- SIGNED a16 ADOPTION GATE FOR SPLIT-B CONTEXT-QK96.

Apply the identical native a16 mean ablation to the independently fitted
context-QK96 program and compare direct signed effects against its saved
unablated census CEV.

Frozen predictions
------------------
pred_a_live_baseline_price_and_identity:
    Baseline <=.004/58 certificates, OOD max <=.040, fresh <=.010, exact
    context-RRR rank96/440-map identity, and 535,089,462-scalar bill.
...
fixed split-B context-QK96 artifact as the new high-fidelity frontier point.
"""
...
OUT = ROOT / "a16_transfer_mixed96_context_metric_qk_results.json"
BASE_RESULT = ROOT / "mixed96_context_metric_qk_split_ood_results.json"
BASE_CEV = ROOT / "cev_mixed96_context_metric_qk_split_b.pt"
COMP_KO = ROOT / "cev_a16ko_mixed96_context_metric_qk.pt"
NATIVE_KO = ROOT / "cev_a16ko_native_context_qk96_gate.pt"
...
def main() -> None:
    if os.environ.get("BQLIB_DRYRUN") == "1":
        ...
        assert baseline["certificates_valid"] >= 58
        assert baseline["literal_standalone_scalars"] == SCALARS
        assert FIT_SLICE == (72, 96)
        print("A16 TRANSFER CONTEXT-QK96 | dry run: baseline, fit, price, bars valid")
        return
    ...
    C.SEL["_ablh"] = ablate
    print("ARM: split-B context-QK96 with a16 mean ablated", flush=True)
    run = C.main()
    ...
    pred_a = (baseline_result["census_damage"] <= .004
              and baseline_result["certificates_valid"] >= 58
              and baseline_result["shifted_damage_row_max"] <= .040
    ...
    result = {
        "status": "a16_transfer_mixed96_context_metric_qk_complete",
        "rung": 333,
        "claim_level": "direct_signed_a16_context_qk96_adoption_gate",
        ...
        "null_signed_context_qk96_transport_fails": bool(null),
