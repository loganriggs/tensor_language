#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_two_stages_jointly_below_the_sum pred_c_two_stages_jointly_above_the_larger pred_d_all_stages_replay pred_e_two_stages_are_most_of_all
"""The pronoun slot's two main rebuild stages together (v509). v504: at the pronoun answer MLPs 12 and 9 each re-express 0.089 of the margin; v506 / v508: neither
stage's top unit survives an edit. Here MLPs 9 + 12 at the answer swapped jointly (nested if 12 reads 9's write), against MLPs 9-16 jointly (replay 0.214);
122 natural pairs.
PREDICTIONS (scored as written; failures preserved; priors from v504)
    pred_a_baseline_replays              the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_two_stages_jointly_below_the_sum  MLPs 9 + 12 jointly close <= 0.178 - 0.02 (nesting)
    pred_c_two_stages_jointly_above_the_larger  MLPs 9 + 12 jointly close >= 0.089 + 0.02
    pred_d_all_stages_replay             MLPs 9-16 jointly close 0.214 +- 0.03
    pred_e_two_stages_are_most_of_all    MLPs 9 + 12 jointly are >= 0.60 x MLPs 9-16 jointly
...
OUT = ROOT / "circuits/followups/pronoun_slot_two_stages_v509_result.json"
CANDIDATE_ID = "chain.pronoun_slot_two_stages_v509"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, SUM_TWO, NEST, LARGER, ALL_V503, BAND, SHARE_MIN, UNIT = 1e-3, 0.178, 0.02, 0.089, 0.214, 0.03, 0.60, 701
import random
...
FORWARDS_MAX = 13
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_two_stages_jointly_below_the_sum": "<= 0.158", "pred_c_two_stages_jointly_above_the_larger": ">= 0.109", "pred_d_all_stages_replay": "0.214 +- 0.03", "pred_e_two_stages_are_most_of_all": ">= 0.60 x"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "sum_two": SUM_TWO, "nest": NEST, "larger": LARGER, "all_v503": ALL_V503, "band": BAND, "share_min": SHARE_MIN, "unit": UNIT}}
    ...
    MODES = {"m9_12": ({}, (0,)), "m9_16": ({}, (0,))}; forwards = 0
    def logits_margin(x):
    ...
    def run(seqs, pos, mode):
        ...
        with torch.no_grad():
            ...
            for l, block in enumerate(blocks):
                ...
                m_out = block.mlp(xm)
                if (mode == "m9_12" and l in (9, 12)) or (mode == "m9_16" and 9 <= l <= 16):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_two_stages_jointly_below_the_sum": mc["m9_12"] <= SUM_TWO - NEST, "pred_c_two_stages_jointly_above_the_larger": mc["m9_12"] >= LARGER + NEST,
                   "pred_d_all_stages_replay": abs(mc["m9_16"] - ALL_V503) <= BAND, "pred_e_two_stages_are_most_of_all": mc["m9_12"] >= SHARE_MIN * mc["m9_16"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pronoun_slot_two_stages_result_v509", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
