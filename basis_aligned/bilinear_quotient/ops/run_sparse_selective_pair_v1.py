...
pred_d selected target/control effect errors<=10% each family for remove9/joint/donorjoint.
Null: compressed head9 reader fails preservation of original selective pair.
Existing confirmation panel, not a new prospective panel.
...
STEM='SPARSE_SELECTIVE_PAIR_V1'
SELECTED=[0,4,5,6,7,8,9]

@torch.no_grad()
def main():
 binding=json.loads((P/(STEM+'_BINDING.json')).read_text())['files'];assert all(digest(k)==v for k,v in binding.items())
 assert json.loads((P/'SCALAR_JOINT_KEY_GRADES_CONFIRMATION_V1_SELECTION.json').read_text())['selected']['mask']==45
 ...
 compact={k:v.cuda() for k,v in torch.load(P/'extracted_circuits/regional_even_key_producers_8_2_9_8_v1/program.pt',weights_only=True).items()}
 item=torch.load(P/'SPARSE_COMPLETE_EVEN_FIT_V1_PROGRAM.pt',weights_only=True)['0.25'];_,_,q=unpack(item)
 compact['key_basis']=[compact['key_basis'][0],q.cuda()]
 ...
 def hook(index,head,module,args,output):
  ...
  if index==1:
   value=args[0].double()@compact['current_value_reader'];selected=(routing(args[0],compact,1)@value[...,None])[...,0]
  if arm==0:
 ...
 old=torch.load(P/'SCALAR_JOINT_KEY_GRADES_CONFIRMATION_V1_ARTIFACT.pt',weights_only=True)['regional']
 preservation=[]
 for family in range(3):
  ix=[i for i,r in enumerate(regional) if r['family']==family]
  for arm in (2,3,5):
   ref=old[ix,arm]-old[ix,0];pred=reg[ix,arm]-reg[ix,0]
   preservation.append(dict(family=family,arm=arm,relative_errors=((pred-ref).norm(dim=0)/ref.norm(dim=0)).tolist()))
 result={'pred_d':all(max(x['relative_errors'])<=.1 for x in preservation),'preservation':preservation,'pred_a':A,'pred_b':A and all(c['passed'] for c in cells),'pred_c':A and all(c['passed'] for c in ncells),'instrument':instrument,'capability':cap,'max_sum_error':max(checks),'max_meanhead_error':max(meanchecks),'regional':cells,'newline':ncells,'body_forwards':count,'seconds':time.perf_counter()-tic,'arms_regional':['native','remove8','remove9','removejoint','fullsector','donorjoint','selfdonorjoint'],'arms_natural':['native','removejoint','fullsector','meanhead8'],'scope':'Frozen compressed head9 reader in the known selectedmask45 pair, reused72regional/32newline panel. Fullsector/meanhead controls exact. No new fitting or prospective panel; externalnative context/normalizers remain.','source_shas':binding}
