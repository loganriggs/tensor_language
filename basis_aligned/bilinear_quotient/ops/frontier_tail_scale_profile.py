"""IS 0.25 OPTIMAL, AND IS THE MISMATCH UNIFORM ACROSS THE TAIL BAND? (Claude, LANE 1)

SS2896 ADOPTED the first frontier improvement of the campaign: scaling every tail-refit link map `LW[k]` by **0.25** takes the SS312
frontier from **+2.6736 to +2.4448** -- SIGN (SS2135): frontier L2 is CE ADDED ABOVE THE REAL MODEL, **LOWER IS BETTER**, so that is an
improvement of **0.2287 nats**. Both anchors resolved (refit-time +0.1740 at deviation 0.0000, frozen +0.1131 at 0.0001) and the curve
reproduced across four runs at a maximum deviation of 0.0003.

Two questions follow immediately and both are eval-only, so both fit in ONE pipeline run:

1. **Is 0.25 optimal?** SS2893's grid was coarse -- 0/.25/.5/.75/.9/1 -- and .25 was simply its lowest interior point. A finer grid
   (.10/.15/.20/.25/.30/.40) locates the optimum and says how much more is available.
2. **Is the mismatch uniform across the band?** Scaling ONE layer at a time, at 0.25, gives a per-layer profile. If the gain is
   concentrated in a few layers, the eventual end-to-end refit should be targeted rather than global; if it is spread, the mismatch is a
   property of the fitting procedure rather than of particular layers.

pred_b re-anchors to SS2896's adopted -0.2287, so this rung is a fifth reproduction of the adopted number as well as an extension of it.

SS2128/SS2129/SS2133/SS2134 RETRACTED; SS2125 STANDS.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_the_s025_arm_reproduces_S2896
#                     pred_c_the_optimum_is_interior
#                     pred_d_no_single_layer_matches_the_global_scale
#                     pred_e_the_per_layer_gains_are_not_additive

Preregistration: polynomial_causal/FRONTIER_TAIL_SCALE_PROFILE_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run: six global scales plus eight per-layer arms")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_tail_scale_profile_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
def main():
    ...
    _LW0 = {li: {k: v.clone() for k, v in S[f'a{li}L'][3].items()} for li in range(10, 18)}
    def _apply_scale(sc, only=None):
        # SCALE, not truncate: the whole fitted map is multiplied by a scalar. This is a ONE-PARAMETER
        # probe of the same claim the rank sweep raised -- if the ridge solution overshoots for the
        # END-TO-END objective, some scale below 1 must beat it. SS2881's LW:={} is exactly scale 0.
        for _li in range(10, 18):
            _k, _l, _CV, _LW, _Wp = S[f'a{_li}L']
            _sc = sc if (only is None or _li == only) else None
            if _sc == 'empty':
                # SS2881's ACTUAL operation: an EMPTY dict, so the hook's `for k in LW` never runs and
                # LINK-class positions keep the class constant CV[c]. SS2893 wrongly identified this with
                # scale 0, which sets LW[k]=0 and OVERWRITES those positions with zero. Both arms are
                # present here so the anchor is measured internally instead of imported across rungs.
                _new = {}
            elif _sc is None:
                _new = {k: v.clone() for k, v in _LW0[_li].items()}
            else:
                _new = {c: (_M.float() * float(_sc)) for c, _M in _LW0[_li].items()}
            S[f'a{_li}L'] = (_k, _l, _CV, _new, _Wp)
    def _cfg(spec):
        _apply_scale('empty' if spec.get('empty') else spec.get('scale'), spec.get('only'))
...
PREREG = PT + '../polynomial_causal/FRONTIER_TAIL_SCALE_PROFILE_PREREGISTRATION.md'
PREREG_SHA = "e7fa0507b0b2356a2df6da0fa36285e396d473d72f23d9faae51531ac0ab49a7"
...
GRID = [0.10, 0.15, 0.20, 0.25, 0.30, 0.40]
LAYERS = list(range(10, 18))
BARS = {"reproduce": 0.05, "anchor": 0.01, "interior": 0.22, "single_vs_global": 0.05, "nonadditive": 0.05}
NULLS = {"anchor_ge": 0.03, "additive_le": 0.02}
...
if __name__=='__main__':
    ...
    if _sha(S2896) != S2896_SHA:
        raise RuntimeError(f'frozen hash mismatch: {S2896}')
    adopted = json.load(open(S2896))['summary']['cost_s025']      # -0.2287
    SEL['mode']='norm'

    gk = lambda s: f'g{int(round(s*100)):03d}'
    lk = lambda l: f'L{l}'
    ARMS[:] = ([('baseline', {})]
               + [(gk(s), {'scale': s}) for s in GRID]
               + [(lk(l), {'scale': 0.25, 'only': l}) for l in LAYERS])
    print(f'ONE pipeline run, {len(ARMS)} evaluations against the same fitted stack',flush=True)
    out=main()

    bF = out['baseline']['L2_F']
    cg = {s: round(out[gk(s)]['L2_F'] - bF, 4) for s in GRID}
    cl = {l: round(out[lk(l)]['L2_F'] - bF, 4) for l in LAYERS}
    best_s = min(GRID, key=lambda s: cg[s])
    global_gain = cg[0.25]
    best_single = min(LAYERS, key=lambda l: cl[l])
    sum_singles = round(sum(cl.values()), 4)
    dev_anchor = abs(cg[0.25] - adopted)
    nonadd = abs(sum_singles - global_gain)

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = dev_anchor <= BARS['anchor']
    pc = cg[best_s] <= -BARS['interior'] and best_s not in (GRID[0], GRID[-1])
    pd = (cl[best_single] - global_gain) >= BARS['single_vs_global']
    pe = nonadd >= BARS['nonadditive']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_s025_arm_reproduces_S2896':bool(pb),
           'pred_c_the_optimum_is_interior':bool(pc),
           'pred_d_no_single_layer_matches_the_global_scale':bool(pd),
           'pred_e_the_per_layer_gains_are_not_additive':bool(pe)}
    nulls={'b_null_the_anchor_fails':bool(dev_anchor>=NULLS['anchor_ge']),
           'e_null_the_layers_are_additive':bool(nonadd<=NULLS['additive_le'])}
    res={'rung':'frontier_tail_scale_profile','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,'grid':GRID,'layers':LAYERS,'arms':out,
         'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(bF,4),
                    'global_cost_by_scale':{str(s):cg[s] for s in GRID},
                    'per_layer_cost_at_scale_025':{f'a{l}L':cl[l] for l in LAYERS},
                    'best_global_scale':best_s,'best_global_cost':cg[best_s],
                    'S2896_adopted_s025':round(adopted,4),'anchor_deviation':round(dev_anchor,4),
                    'best_single_layer':best_single,'best_single_cost':cl[best_single],
                    'sum_of_single_layer_gains':sum_singles,
                    'global_gain_at_025':global_gain,
                    'nonadditivity_nats':round(nonadd,4),
                    'implied_L2_F_at_best':round(bF + cg[best_s], 4),
         ...
         'hashes':{PREREG:PREREG_SHA,S2896:S2896_SHA},'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {bF:.4f} vs 2.6735: {'HELD' if pa else 'FAILED'}")
    print("    global: " + "  ".join(f"{s}:{cg[s]:+.4f}" for s in GRID))
    print("    layers: " + "  ".join(f"a{l}L:{cl[l]:+.4f}" for l in LAYERS))
    print(f"(b) s=.25 {cg[0.25]:+.4f} vs SS2896 {adopted:+.4f}, dev {dev_anchor:.4f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) best global s={best_s} at {cg[best_s]:+.4f} -> L2_F {bF+cg[best_s]:.4f}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) best single layer a{best_single}L {cl[best_single]:+.4f} vs global {global_gain:+.4f}: {'HELD' if pd else 'FAILED'}")
    print(f"(e) sum of singles {sum_singles:+.4f} vs global {global_gain:+.4f}, gap {nonadd:.4f}: {'HELD' if pe else 'FAILED'}")
