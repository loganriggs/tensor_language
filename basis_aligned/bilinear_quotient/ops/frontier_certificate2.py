...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_certificate_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: rung-99 artifact present; skip-1416 certificate")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_certificate2_results.json'
...
if __name__=='__main__':
    ...
    SEL['skipset']=(14,16); SEL['motif_off']=(); SEL['clsdmg']=True; SEL['ext_rows']=CROWS
    print('ARM: skip-1416 config evaluated per-position on the census rows',flush=True)
    r_cfg=main()
    ...
    prev={r['tag']:r for r in json.load(open(PT+'frontier_certificate_results.json'))['circuits']}
    a16tags=[t for t,v in BATC.items() if v['best_mean']=='a16' and t in {r['tag'] for r in rows}]
    def ratio(rr): return rr['member_absdce']/max(rr['ref'],1e-9)
    now={r['tag']:r for r in rows}
    impr=[ratio(prev[t])/max(ratio(now[t]),1e-9) for t in a16tags]
    med_impr=stt.median(impr)
    pa=med_impr>=2.0
    pb=nvalid>=10
    pc=0.5<=agg<=5.0
    fr_ret=med_impr
    res={'aggregate_mean_dce':round(agg,4),'n_circuits':len(rows),'n_valid':nvalid,
         'a16_median_improvement':round(fr_ret,3),
         'circuits':rows,
         'convention':'per-position dCE = CE(S2144 config) - CE(real model) on the census rows; tau = 0.5 x battery ref',
         'pred_a_a16_recovery':bool(pa),'pred_b_validity_appears':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'aggregate dCE {agg:+.4f}; valid {nvalid}/{len(rows)}; a16 median improvement {fr_ret:.2f}x')
    print(f"(a) a16 median improvement {fr_ret:.2f}x >= 2: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nvalid} >= 10: {'HELD' if pb else 'FAILED'}")
