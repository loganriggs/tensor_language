...
"""Tier-5: where is the noun's NUMBER value (the one 11:03 reads, v164) written? Per-unit interchange AT THE NOUN POSITION.

Units at the noun position (t-3 lexical, t quantifier): the token embedding (wte output, so both x and the per-block x0
injection), and every attn:l / mlp:l write for l = 0..10 (c_proj input / Down output rows). Each is clamped donor->base
at the base noun position (donor captures at the donor noun position); we capture V_h^11[noun] and inject
image(P_base[t,noun] (V' - V_base)[noun]) at t (11:03's value read only) and also read the plain patched forward.
Printed before registering (4 rows per parity, CPU): lexical -- embed alone = the whole donor (whole-model 0.988 / 1.000,
value swap 0.249 / 0.432 = the full noun value swap 0.246 / 0.432); embed donor with ALL 22 writes clamped to base:
value 0.007 / -0.007, whole 0.070 / 0.117; writes donor with embed base: value 0.238 / 0.442, whole 0.883 / 0.913; single
units: mlp:01 0.138 / 0.214 (whole 0.624 / 0.681), mlp:08 0.052 / 0.151, mlp:09 0.034 / 0.112, mlp:10 0.031 / 0.109,
every attn:l at the noun |value| <= 0.015. Quantifier (noun = t, same token on both sides so embed = 0 by construction):
attn:07 at t 0.041 / 0.051 (whole 0.336 / 0.393), mlp:07 0.016 / 0.039, mlp:08 0.025 / 0.022.
...
Registered before the run:
  pred_a_instrument     lexical, both parities: the noun-embedding patch alone recovers the whole donor within 0.95-1.05 and its
                        value swap equals the full noun value swap within 0.02
  pred_b_not_direct     lexical, both parities: embed donor with all 22 noun writes clamped to base gives |value swap| <= 0.03 and
                        whole-model <= 0.20 (the embedding's number is not read directly by 11:03)
  ...
  pred_d_mlp01_source   lexical, both parities: mlp:01 at the noun is the largest single unit, its value swap is 0.35-0.75 of the full
...
Reported, unregistered: every unit's value swap / whole-model / |dV| fraction; sum of single units vs the computed route.
Prior: all five printed above; the bands are the printed values +/- the row-count noise seen across v160-v164 (~0.1 of a share).
Smoke: V165_SMOKE=<out.json> -> CPU, 4 rows per parity (~40 s).
"""
...
OUT = ROOT / "circuits/followups/unit_tier5_hub_noun_value_source_v165_result.json"
V129_RECEIPT = ROOT / "circuits/followups/unit_tier5_head_read_map_v129_result.json"
SRC_LAYERS = 11
BARS = {"instr_whole": [0.95, 1.05], "instr_value_tol": 0.02, "direct_value_max": 0.03, "direct_whole_max": 0.20, "computed_band": [0.8, 1.2], "mlp01_band": [0.35, 0.75], "attn_value_max": 0.03, "attn07_band": [0.3, 0.7], "attn07_whole_band": [0.20, 0.60]}
...
def _plan():
    return {"candidate_id": "corpus.unit_tier5_hub_noun_value_source_v165", "behaviours": 2, "targets": 2,
...
def main() -> None:
    ...
    smoke = os.environ.get("V165_SMOKE")
    ...
    for sname, par in runs:
        ...
        everything = {"value_swap_at_t": rec(img((pb[:, None] * (vp - vb)).float())), "whole_model": whole_all,
                      "dv_frac": round(float(((vp - vb).norm(dim=1) / (vd - vb).norm(dim=1).clamp_min(1e-6)).mean()), 3)}
        S = {"rows": rows, "exact_hub": exact, "noun_value_swap_full": full_value_swap, "per_unit_at_noun": per_unit,
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier5_hub_noun_value_source_v165", "candidate_id": "corpus.unit_tier5_hub_noun_value_source_v165",
...
def PREDS(R):
    ...
    return {"pred_a_instrument": pred_a, "pred_b_not_direct": pred_b, "pred_c_computed": pred_c, "pred_d_mlp01_source": pred_d, "pred_e_quant_attn07": pred_e}
