#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_all_values_at_the_noun_carry_gender pred_c_five_readers_carry_a_third pred_d_low_and_high_add pred_e_high_at_least_low
"""The GENDER route census at the noun (v549): the number circuit's method on the pronoun-gender line. For number, every exit of the noun is a value at the
noun (0.98 of the they - he gap on 122 natural pairs; keys 0.0002; five readers 0.48; blocks 0-8 0.51 -- v428-v436). The gender line (he - she after a
gendered noun; readers 9.6 / 12.4 / 15.1 from the atlas, MLP-8 detectors 3152 male / 3943 female, 18 Sep) was closed by folds and zeroing edits only (2.6% /
7.8%). Here the replace-edit census on natural aligned pairs made by swapping the gendered noun in place (king <-> queen, ... ; v73's 64 fineweb rows,
outcome-blind): every head's value at the noun, the five pronoun readers' values, blocks 0-8, blocks 9-17 -- fraction of the male - female he - she gap closed.
PREDICTIONS (scored as written; failures preserved; priors from the number line v428-v436; gender priors unsure)
    pred_a_baseline_replays                   the unedited manual forward replays the model's he - she margins within 1e-3
    pred_b_all_values_at_the_noun_carry_gender  every head's value at the noun swapped closes >= 0.80 of the gap (number: 0.98)
    pred_c_five_readers_carry_a_third         the five pronoun readers' values at the noun close >= 0.30 (number: 0.48). Prior: unsure
    pred_d_low_and_high_add                   blocks 0-8 + blocks 9-17 at the noun is within 0.08 of all heads (exits at one position add; v436)
    pred_e_high_at_least_low                  blocks 9-17 close at least as much as blocks 0-8 (the gender readers are late; the detectors sit at MLP 8). Prior: unsure
PRICE (registered maximum): 2 batches x 5 passes = 10 forwards (<= 64 pairs); 0 backwards; 0 fits. Bar <= 11.
"""
...
OUT = ROOT / "circuits/followups/gender_route_census_noun_v549_result.json"
CANDIDATE_ID = "gender.route_census_noun_v549"
ROWS = ROOT / "circuits/followups/pronoun_gender_dod_natural_rows_v73.json"
N_HEAD = 9
FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, ALL_MIN, FIVE_MIN, ADD = 1e-3, 0.80, 0.30, 0.08
FORWARDS_MAX = 11
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "text_pairs": len(text_items), "rows": str(ROWS.name), "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "all_min": ALL_MIN, "five_min": FIVE_MIN, "add": ADD}}
    ...
    c = text["closed"]
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_all_values_at_the_noun_carry_gender": c["ALL_N"] >= ALL_MIN, "pred_c_five_readers_carry_a_third": c["FIVE_N"] >= FIVE_MIN,
                   "pred_d_low_and_high_add": abs(c["LOW_N"] + c["HIGH_N"] - c["ALL_N"]) <= ADD, "pred_e_high_at_least_low": c["HIGH_N"] >= c["LOW_N"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "gender_route_census_noun_result_v549", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
