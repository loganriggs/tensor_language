...
import numpy as np
RUNNER=Path(__file__).with_name('run_bracket_suffix_finite_bilinear_program_v2.py')
def load():
...
def test_v2_excludes_only_exact_reference_from_liveness():
 m=load();src=RUNNER.read_text();assert "for arm,v in active.items() if arm!='exact'" in src
