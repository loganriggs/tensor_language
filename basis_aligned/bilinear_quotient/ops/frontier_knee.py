...
OUT = PT + 'ops/frontier_knee_results.json'
...
TRANKS = (None, 1024, 512, 384, 256)          # None = full rank; §1786/§1813 use this ladder
...
def main():
    ...
    print(f'FRONTIER KNEE | 16,110 covered types, ranks full/1024/512/384/256 | '
    ...
    lad = ['256', '384', '512', '1024', 'full']
    above = {l2: {e: curve[l2][e]['cov'] - ceilv[e] for e in roles2} for l2 in lad}
    COSTM = {'256': 164.478, '384': 243.966, '512': 323.454, '1024': 641.406, 'full': 673.464}
    ...
    for l2 in lad:
        print(f'    rank {l2:5s} {COSTM[l2]:8.2f}M  ' + '  '.join(
            f'{e} +{above[l2][e]:.5f} (allpos {curve[l2][e]["allpos"]:.5f})' for e in roles2),
            flush=True)
    ...
    json.dump({'run': 'frontier_knee', 'cost_M': COSTM, 'attainment_exact': bool(attain), 'n_covered': NFULL, 'ranks': [str(r) for r in TRANKS],
