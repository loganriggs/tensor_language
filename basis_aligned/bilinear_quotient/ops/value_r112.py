"""VALUE RANK CURVE II (rung 257): v_r = 112 - the binary-structure test on the value side.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The pattern side is binary in
fine-rank deficiency (S2351): missing ANY chunk of the fine band costs ~0.05. If the value side shares
that structure, even 87.5% rank stays expensive; if values price smoothly, r112's surcharge is small.
REGISTERED PREDICTIONS (arms: THIS vs mixed base 0.0573/11 and value-r64 0.1295/3):
  (a) SMOOTH: census <= 0.070 (surcharge <= 0.013).
  (b) valid >= 10.
  (c) SANITY: census in [0.01, 0.15]; inert tripwires vs base AND r64 receipts.
NULL: value side binary too (census >= 0.09 at r112). PRICE: mixed patterns 52.9M + value factors 15.8M.
Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: value rank-112 on mixed base')
    raise SystemExit(0)
import torch
...
OUT=PT+'value_r112_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=96; SEL['qk_rmap']={}; SEL['v_r']=112; SEL['qk_tail']=True; SEL['drop_tailE']=True
