...
"""Preposition selection on/of (non-auxiliary contrast) reuse census (v54): blind readout sweep + top-4 set on the line's authored A1/A2 contexts (opened rows).
Contrast `O_h^T(u_on - u_of)`. Predictions as in `dod_reuse_census.run`; prior for pred_e: no family head in the top four (a non-auxiliary decision).
PRICE: 181 forwards per 32-row batch (registered in dod_reuse_census)."""
import circuit_fast_screen_candidate_preposition_selection as m
...
rows, pos, neg = R.rows_from_candidate(m, " on", " of", "preposition_selection")
if __name__ == "__main__":
    R.run("preposition_selection.on_vs_of.dod_reuse_census_v54", "preposition_selection_dod_reuse_census_v54_result.json", rows, pos, neg)
