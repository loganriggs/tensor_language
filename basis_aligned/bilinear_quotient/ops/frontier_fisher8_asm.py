"""ASSEMBLY-CONDITIONED FISHER (rung 32): recompute the metric THROUGH the deployed §312 assembly and re-select.

§2125: the cfgE-certified selector makes the §312 frontier WORSE (-0.048 fresh) because the eight directions were
computed for the REAL readers while the frontier deploys dictionary readers (38 motif heads, tail attention). The
falsifiable repair: run the norm arm first, and at its end compute the true-Fisher top-8 at blocks 5/6 WITH THE
FULL L2 CONFIGURATION INSTALLED (its hooks active during both Fisher forwards, labels sampled from the deployed
assembly's own predictions), then rerun the pipeline selecting mlp4/mlp5 units under that assembly-conditioned
metric.

REGISTERED PREDICTIONS:
  (a) REPRODUCTION GATE: the norm arm's L2_F reproduces 2.6735 within 0.01 (§2125 reproduced it exactly).
  (b) THE CONDITIONED METRIC INSTALLS: L2_F(norm) - L2_F(asm-fisher8) >= 0.02 (half of rung 30's failed 0.04 bar;
      the conditioning is the repair under test). If FAILED, the selector is certified as cfgE-only and the
      observability arc's frontier chapter closes with that scope statement.
  (c) NO WINDOW-C HARM: L2_C(norm) - L2_C(asm-fisher8) >= -0.01.

Writes frontier_fisher8_asm_results.json. Self-reviewed. RUNG-30 HEADER FOLLOWS.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_fisher8_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; two arms (norm / ASSEMBLY-conditioned true-Fisher top-8)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_fisher8_asm_results.json'
...
if __name__=='__main__':
    T00=time.time()
    SEL['mode']='norm'; SEL['collect_asm']=True
    print('ARM 1/2: norm selection + assembly-conditioned Fisher collection',flush=True)
    r_norm=main()
    SEL['collect_asm']=False
    SEL['mode']='fisher8'
    print('ARM 2/2: ASSEMBLY-conditioned true-Fisher top-8 selection at mlp4/mlp5',flush=True)
    r_f=main()
    pa=abs(r_norm['L2_F']-2.6735)<=0.01
    pb=(r_norm['L2_F']-r_f['L2_F'])>=0.02
    pc=(r_norm['L2_C']-r_f['L2_C'])>=-0.01
    res={'norm':r_norm,'fisher8':r_f,'published_L2_F':2.6735,
         'gain_fresh':round(r_norm['L2_F']-r_f['L2_F'],4),'gain_C':round(r_norm['L2_C']-r_f['L2_C'],4),
         'pred_a_reproduces_published':bool(pa),'pred_b_conditioned_metric_installs':bool(pb),'pred_c_no_C_harm':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) norm L2_F {r_norm['L2_F']} vs 2.6735 (tol 0.05): {'HELD' if pa else 'FAILED'}")
    print(f"(b) fresh gain {res['gain_fresh']:+.4f} >= 0.02: {'HELD' if pb else 'FAILED'}")
    print(f"(c) window-C gain {res['gain_C']:+.4f} >= -0.01: {'HELD' if pc else 'FAILED'}")
