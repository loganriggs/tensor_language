#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_answer_write_replays pred_c_all_position_writes_match_the_joint pred_d_intermediate_writes_oppose pred_e_positions_add
"""The gender readers' writes by position (v570). v563: the five readers' writes into the ANSWER position swapped close 0.878 of the he - she gap. v569: their
full outputs (pattern and values) swapped at every position close 0.745. The 0.13 between them must be the readers' writes into the positions between the
noun and the answer, and it must be negative. Here the readers' writes (c_proj input, per head) swapped at the answer only (W_ANSWER, replay), at the
positions from the noun to the one before the answer (W_INTER), and at every position from the noun on (W_ALLPOS).
PREDICTIONS (scored as written; failures preserved; priors from v563 / v569)
    pred_a_baseline_replays                 the unedited manual forward replays the model's he - she margins within 1e-3
    pred_b_answer_write_replays             W_ANSWER closes 0.878 +- 0.03
    pred_c_all_position_writes_match_the_joint  W_ALLPOS closes 0.745 +- 0.05 (v569's joint pattern-and-value swap)
    pred_d_intermediate_writes_oppose       W_INTER closes <= -0.05 (the readers' writes into the intermediate positions work against the answer)
    pred_e_positions_add                    W_ANSWER + W_INTER is within 0.08 of W_ALLPOS
...
OUT = ROOT / "circuits/followups/gender_writes_by_position_v570_result.json"
CANDIDATE_ID = "gender.writes_by_position_v570"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, ANSWER_V563, JOINT_V569, INTER_MAX, ADD, BAND, UNIT = 1e-3, 0.878, 0.745, -0.05, 0.08, 0.03, 701
...
FORWARDS_MAX = 9
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_answer_write_replays": "0.878 +- 0.03", "pred_c_all_position_writes_match_the_joint": "0.745 +- 0.05", "pred_d_intermediate_writes_oppose": "<= -0.05", "pred_e_positions_add": "|ANSWER + INTER - ALLPOS| <= 0.08"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "answer_v563": ANSWER_V563, "joint_v569": JOINT_V569, "inter_max": INTER_MAX, "add": ADD, "band": BAND, "unit": UNIT}}
    ...
    MODES = {"W_ANSWER": ({}, (0,)), "W_INTER": ({}, (0,)), "W_ALLPOS": ({}, (0,))}; SETS_W = {m: FIVE_W for m in MODES}
    RULES = {"W_ANSWER": lambda rel, tofin: tofin == 0, "W_INTER": lambda rel, tofin: (rel >= 0) & (tofin < 0), "W_ALLPOS": lambda rel, tofin: rel >= 0}; forwards = 0
    def logits_margin(x):
    ...
    print("writes by position", {k: round(v, 3) for k, v in mc.items()}, "answer + inter", round(mc["W_ANSWER"] + mc["W_INTER"], 3))
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_answer_write_replays": abs(mc["W_ANSWER"] - ANSWER_V563) <= BAND, "pred_c_all_position_writes_match_the_joint": abs(mc["W_ALLPOS"] - JOINT_V569) <= 0.05,
                   "pred_d_intermediate_writes_oppose": mc["W_INTER"] <= INTER_MAX, "pred_e_positions_add": abs(mc["W_ANSWER"] + mc["W_INTER"] - mc["W_ALLPOS"]) <= ADD}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "gender_writes_by_position_result_v570", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
