"""DOES THE SCALAR MERGE GENERALIZE (rung 95): drop-one-twin + survivor gain at the OTHER two pairs.

CONVENTION (S2135): L2 is CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER. S2184: at b2/b3 the merge recovers
55% (alpha*=1.45). Test the same construction at 14/16 (skip-1416 base; drop attn16, scale attn14) and b4/b5
(motifs-4/5-real base; drop attn5, scale attn4). Alpha grid {1.0,1.15,1.3,1.45,1.6} picked on window C.
NULL: S2184 was front-specific - the tail's weaker same-position coverage (0.689 vs 0.998) may not support
amplitude substitution.

REGISTERED PREDICTIONS (arm-named):
  (a) 14/16: [L2_F(drop16+alpha*) - base] <= 0.7 x [L2_F(drop16) - base].
  (b) b4/b5: [L2_F(drop5+alpha*) - base] <= 0.7 x [L2_F(drop5) - base].
  (c) REPRODUCTION: the skip-1416 base reproduces S2156 (FR L2_F 2.4230 within 0.01).

Writes merge_scalar_pairs_results.json. Self-reviewed. RUNG-91 HEADER FOLLOWS.

OUTPUT-SUBSTITUTION MERGE, THE COORDINATE-FREE VERSION (rung 91; user-suggested direction).

CONVENTION (S2135): L2 is CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER. Base = S2144 config with blocks 2/3
motifs REAL. S2180: dropping attn3 costs +0.237 (matched fits). S2181: a FIXED linear map from attn2's output
to attn3's is dead OOD (document gauge). The coordinate-free substitute that survives the gauge argument: a
SCALAR gain on attn2's output (alpha), fit by grid on the FW eval rows under the drop-attn3 build, then the
final arm rebuilt with alpha active throughout. NULL (S2126): scalar gains are not levers.

REGISTERED PREDICTIONS (arm-named):
  (a) THE SCALAR RECOVERS A THIRD: [L2_F(drop3+alpha*) - base] <= 0.7 x [L2_F(drop3) - base].
  (b) A REAL UPSCALING IS SELECTED: alpha* in [1.05, 1.8].
  (c) REPRODUCTION: the base reproduces rung 86's base (FR L2_F 2.4410 within 0.01).

Writes merge_scalar_results.json. Self-reviewed. RUNG-86 HEADER FOLLOWS.

IS THE FRONT PAIR ALSO MUTUAL BACKUP (rung 86): the interaction signature at b2/b3.

CONVENTION (S2135): L2 is CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER. Base = S2144 config with blocks 2/3
motifs REAL; dz = damage from zeroing the real attention(s) throughout (bases excluded). Sign derived pre-run
per S2165: replicated backup => SUPER-additive joint removal.

S2179: b2/b3 are a near-identity replicated pair (rho 0.998). The tail pair showed the backup interaction
(S2165, +40% super-additive). Same test at the front. NULL: additive (independent despite identical coverage).

REGISTERED PREDICTIONS (arm-named):
  (a) SUPER-ADDITIVE: [L2_F(both zeroed) - base] >= 1.15 x ([L2_F(z2) - base] + [L2_F(z3) - base]).
  (b) SAME POSITIONS: binned Spearman rho(dz2, dz3) >= 0.5 on subword positions.
  (c) REPRODUCTION: the full arm reproduces S2144 (FR L2_F 2.6662 within 0.01).
  TRIPWIRE: each zeroed arm must differ from base.

Writes assembler_backup_results.json. Self-reviewed. RUNG-84 HEADER FOLLOWS.

THE CONTEXT-TO-MEMORY EDGE (rung 84): do the b5 and a16 replacements fail at the same positions?

CONVENTION (S2135): per-position CE differences on FR; positive = the replacement hurts there. d5 = CE(full) -
CE(block-5 motifs real); d16 = CE(full) - CE(skip-a16, attn16 real).

S2176 measured the subword pipeline (b2 -> m16, rho 0.85). The retrieval chain's edge (b4/b5 context -> 14/16
memory) predicts shared failure points: positions needing context are positions needing memory. NULL:
independent failures (rho ~ 0) - context and memory serve different tokens.

REGISTERED PREDICTIONS (arm-named; bins of 200 ordered by d5):
  (a) THE EDGE ON other: binned Spearman rho(d5, d16) >= 0.3 on other positions.
  (b) THE EDGE ON ind: binned Spearman rho(d5, d16) >= 0.3 on ind positions.
  (c) REPRODUCTION: the full arm reproduces S2144 (FR L2_F 2.6662 within 0.01).

Writes context_memory_edge_results.json. Self-reviewed. RUNG-82 HEADER FOLLOWS.

DOES THE FRONT ASSEMBLE WHAT m16 FINISHES (rung 82): the cross-depth pipeline test.

CONVENTION (S2135): per-position CE differences on FR. b2 damage = CE(full) - CE(block-2 motifs real);
m16 help = CE(m16 zeroed) - CE(full) (positive where m16 was helping). Both on subword positions.

S2174/S2159: block 2's replacement and m16's deletion both hit SUBWORD continuation hardest. If they are one
pipeline (b2 assembles the word context that m16 finishes), their per-position profiles should co-vary. NULL:
independent subword services (rho ~ 0).

REGISTERED PREDICTIONS (arm-named):
  (a) CO-VARIATION: Spearman rho >= 0.3 between b2 damage and m16 help across subword positions, computed on
      200-position bins ordered by b2 damage.
  (b) CONCENTRATION: subword positions in the top half of b2 damage carry >= 0.6 of m16's summed subword help.
  (c) REPRODUCTION: the full arm reproduces S2144 (FR L2_F 2.6662 within 0.01).

Writes front_m16_pipeline_results.json. Self-reviewed. RUNG-81 HEADER FOLLOWS.

DO b3/b4 INTERPOLATE (rung 81): completing the front signature map.

CONVENTION (S2135): per-position CE(full S2144 config) - CE(block-b motifs REAL) on FR; positive = the
replacement hurts there.

S2174: b2 is subword-led (0.453); S2167: b5 is other-led (0.464); the tail is ind-led. If the class ordering
is a depth progression (assembly -> context -> memory), b3/b4 should interpolate. NULL: idiosyncratic
signatures, no ordering.

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) b3's top class is subword or other.
  (b) MONOTONE ASSEMBLY DECLINE: subword share falls b2 (0.453) > b3 > b4.
  (c) REPRODUCTION: the full arm reproduces S2144 (FR L2_F 2.6662 within 0.01).

Writes attn34_motif_class_results.json. Self-reviewed. RUNG-80 HEADER FOLLOWS.

THE PRICE LEADER'S SIGNATURE (rung 80): class and position map of the BLOCK-2 motif replacement.

CONVENTION (S2135): per-position CE(full S2144 config) - CE(block-2 motifs REAL) on FR; positive = the
replacement hurts there. Signed-sum accounting per S2159.

S2169: block 2 is the motif price leader (+0.189, 3x the cliff). S2167/S2170 characterized block 5
(retrieval-class, early-concentrated). Same instrument at the leader. NULLS: same signature as block 5.

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) RETRIEVAL-CLASS AGAIN: combined share of ind+other+subword >= 0.60 (the everything-is-retrieval law,
      now as a prediction).
  (b) EARLY-CONCENTRATED: late-half (position >= 128) share <= 0.45 (extending S2170's front law).
  (c) REPRODUCTION: the full arm reproduces S2144 (FR L2_F 2.6662 within 0.01).

Writes attn2_motif_class_results.json. Self-reviewed. RUNG-73 HEADER FOLLOWS.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['twin_anatomy2_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: deps present; scalar-merge arms at 14/16 and b4/b5 (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'merge_scalar_pairs_results.json'
...
def main():
    ...
    baseF=evalT(FR,120,[])
    _hza=[]
    if SEL.get('alpha2') is not None:
        _al=SEL['alpha2']
        _hza.append(m.transformer.h[SEL.get('alpha_li',2)].attn.register_forward_hook(
            lambda mo,i_,o_,_al=_al: (o_[0]*_al,o_[1])))
        print(f'attn2 output scaled by {_al} for this arm',flush=True)
    for _li9 in SEL.get('zeroattn',()):
        def _mkza(li9):
            def hza(mo,i_,o_):
                return (torch.zeros_like(o_[0]),o_[1])
            return hza
        _hza.append(m.transformer.h[_li9].attn.register_forward_hook(_mkza(_li9)))
        print(f'attn{_li9} zeroed for this arm (bases excluded)',flush=True)
    _hzm=[]
    if SEL.get('zero_m16'):
        _hzm.append(m.transformer.h[16].mlp.register_forward_hook(
            lambda mo,i_,o_: torch.zeros_like(o_)))
        print('m16 zeroed for this arm (bases excluded)',flush=True)
    ...
    out['fresh8']=W8RES
    for _h9 in _hzm: _h9.remove()
    for _hA in _hza: _hA.remove()
...
if __name__=='__main__':
    ...
    SEL['clsdmg']=False; SEL['zero_m16']=False
    import statistics as stt
    out_all={}
    for name,moff,skips,keep,drop,base_ref in (('p1416',(),(14,16),14,16,2.4230),('b45',(4,5),(),4,5,None)):
        SEL['motif_off']=moff; SEL['skipset']=skips
        SEL['zeroattn']=(); SEL['alpha2']=None; SEL['alpha_li']=keep
        print(f'== {name}: base ==',flush=True)
        rb=main()
        SEL['zeroattn']=(drop,)
        print(f'== {name}: drop attn{drop} ==',flush=True)
        rz=main()
        dz=rz['L2_F']-rb['L2_F']
        if abs(dz)<1e-5: raise SystemExit(f'INSTRUMENT FAIL: {name} drop identical to base')
        best=(1.0,rz['L2_C'],rz)
        for al in (1.15,1.3,1.45,1.6):
            SEL['alpha2']=al
            ra=main()
            print(f'  {name} alpha {al}: L2_C {ra["L2_C"]:+.4f} | L2_F {ra["L2_F"]:+.4f}',flush=True)
            if ra['L2_C']<best[1]: best=(al,ra['L2_C'],ra)
        SEL['alpha2']=None
        da=best[2]['L2_F']-rb['L2_F']
        out_all[name]={'base_L2F':rb['L2_F'],'drop_d':round(dz,4),'alpha_star':best[0],'merged_d':round(da,4),
                       'recovery':round(1-da/max(dz,1e-9),3)}
        print(f'{name}: drop {dz:+.4f} -> alpha*={best[0]} d={da:+.4f} (recovery {out_all[name]["recovery"]})',flush=True)
    pa=out_all['p1416']['merged_d']<=0.7*out_all['p1416']['drop_d']
    pb=out_all['b45']['merged_d']<=0.7*out_all['b45']['drop_d']
    pc=abs(out_all['p1416']['base_L2F']-2.4230)<=0.01
    res={'pairs':out_all,
         'convention':'damage on each leave-pair-real base; alpha picked on window C; lower L2 better',
         'pred_a_tail_merge':bool(pa),'pred_b_mixer_merge':bool(pb),'pred_c_reproduces_s2156':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    for n in out_all: print(f"{n}: {out_all[n]}")
    print(f"(a) 14/16 merged {out_all['p1416']['merged_d']:+.4f} <= 0.7 x {out_all['p1416']['drop_d']:+.4f}: {'HELD' if pa else 'FAILED'}")
    print(f"(b) b4/5 merged {out_all['b45']['merged_d']:+.4f} <= 0.7 x {out_all['b45']['drop_d']:+.4f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) skip-1416 base {out_all['p1416']['base_L2F']} vs 2.4230 (tol 0.01): {'HELD' if pc else 'FAILED'}")
