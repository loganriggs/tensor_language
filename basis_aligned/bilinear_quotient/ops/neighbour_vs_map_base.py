# IS THE DISCARDED NEIGHBOUR A COMPETITIVE BASE?  -- measured against the uncovered ceiling.
#
# §1871 found the settled build's uncovered rows are `Eunc @ mp` -- §1785's linear map ALONE. The
# output-NN neighbour of §1780/§1781 is computed inside build() and then discarded, matching §1785's own
# certification of the map "rather than the neighbour" (+0.03 in all-position CE).
#
# §1869/§1870 then showed the map's residual loss is functional form, not capacity: 73% survives at map
# rank 512, and the per-doubling recovery decays 0.095 -> 0.078 -> 0.047. The obvious next form is the
# neighbour as a BASE with the map fitted on the residual -- but that needs a leave-one-out neighbour for
# the covered fit set (a covered token's nearest covered neighbour is itself), which is a real build
# change and the riskiest thing available at this hour.
#
# This is the cheap half that decides whether that build is worth attempting. §1785 compared the two
# arms in ALL-POSITION CE, where covered positions dominate and dilute everything. Measured where it
# actually matters -- fallback loss against the uncovered-token ceiling of §1868 -- the neighbour may be
# much worse than the map (in which case it carries nothing and a residual fit is unpromising) or close
# to it (in which case the base point is worth changing).
#
# ROLES. All three, at the DEPLOYED 5,419-type covered set where §1870 showed the lever is worth most.
# DISCOVERY ONLY. Rung 3: the question §1871 ends on.
#
...
# ceiling, never a CE compared across populations.
#   pred_a §1785's ORDERING REPRODUCES ON THIS INSTRUMENT: the neighbour-alone fallback loss exceeds the
#          map-alone loss, whose PUBLISHED values at this coverage are +0.78075 / +0.86225 / +0.83997
#          (§1870, map rank 64), on all three roles. If FALSE the neighbour is actually BETTER when
#          measured against the ceiling, §1785's +0.03 was an all-position artefact in which covered
#          positions diluted the comparison, and the deployed build is using the wrong fallback
#          outright -- which would be a larger finding than the residual fit it was meant to motivate.
#   pred_b BUT THE NEIGHBOUR IS COMPETITIVE: it is worse by less than 0.15 nats on all three roles. If
#          TRUE the neighbour carries most of what the map does and is worth trying as a base for a
#          residual fit. If FALSE it is far behind, carries little the map does not, and the
#          neighbour-plus-residual build is not worth the risk -- the residual would then need a
#          different functional form entirely, not a better base point.
#   pred_c AND THE COVERED ARM IS UNTOUCHED: covered CE identical between the two arms to within 1e-6.
#          Both change uncovered rows only; §1869 and §1870 each got exactly 0.00e+00 and this is the
#          same KNOWN-ANSWER check on a third construction.
#   pred_d CONTROLS: the map arm reproduces §1870's PUBLISHED fallback loss within 0.005; coverage is
#          exactly 5,419; the covered arm attains its ceiling (a twelfth confirmation); every uncovered
#          scored token has a ceiling row.
...
OUT = PT + 'ops/neighbour_vs_map_base_results.json'
...
MAPRANKS = (64,)
ARMS2 = ('MAP', 'NEIGHBOUR')          # None = full rank; §1786/§1813 use this ladder
...
S1870_MAP_M64 = {'skip7000': 0.78075, 'skip11000': 0.86225, 'skip1200': 0.83997}
...
def main():
    ...
    print(f'NEIGHBOUR VS MAP BASE | 5419 types, uncovered rows from map or neighbour | '
    ...
    def build(n, rank=None, map_rank=MAP_RANK):
        ...
        for st in sites:
            ...
            fr[tk] = tables[st]
            if BASE['mode'] == 'NEIGHBOUR':
                # §1780/§1781's output-NN row, the object build() has been discarding
                fr[unc] = tables[st][nnrow[unc]]
            else:
    ...
    behind = {e: fbloss['NEIGHBOUR'][e] - fbloss['MAP'][e] for e in roles2}
    ...
    pa = all(behind[e] > 0 for e in roles2)
    pb = all(behind[e] < 0.15 for e in roles2)
    pc = covspread <= 1e-6
    pd = (all(abs(fbloss['MAP'][e] - S1870_MAP_M64[e]) <= 0.005 for e in roles2)
          and NFULL == NCOV and attain)

    print(f'\n  FALLBACK LOSS by uncovered-row construction at {NFULL} covered types:', flush=True)
    for l2 in LADDER:
        print(f'    {l2:10s} ' + '  '.join(
            f'{e} {fbloss[l2][e]:+.5f}' for e in roles2), flush=True)
    print(f'\n  §1785 ordering reproduces: neighbour is WORSE -> {pa}  ' + '  '.join(
        f'{e} {behind[e]:+.5f}' for e in roles2), flush=True)
    print(f'  but the neighbour is COMPETITIVE (<0.15 behind) -> {pb}  ' + '  '.join(
        f'{e} {behind[e]:+.5f}' for e in roles2), flush=True)
    print(f'  and the COVERED arm is untouched (<1e-6) -> {pc}  max spread {covspread:.2e}',
          flush=True)
    print(f'  MAP arm reproduces §1870, coverage {NFULL}, attainment {attain} -> control {pd}',
          flush=True)

    json.dump({'run': 'neighbour_vs_map_base', 'n_covered': NFULL, 'arms': list(ARMS2),
               'fallback_loss': fbloss, 'covered_ce': covce, 'neighbour_behind_map': behind,
               'covered_spread': covspread, 'ceiling_uncovered': uceilv,
               'uncovered_fraction': nfrac, 'attainment_exact': bool(attain),
               'predictions': {'pred_a_map_still_wins': bool(pa),
                               'pred_b_neighbour_competitive': bool(pb),
