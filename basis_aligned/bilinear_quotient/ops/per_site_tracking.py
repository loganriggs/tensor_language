# WHICH SITES CARRY THE TRACKING? -- leave-one-out restoration, 36 sites.
#
# §1887 and §1888 attributed the compiled program's top-1 fidelity to the live model to the TABLE
# lookups: 7.19 / 7.29 / 7.64x a permutation null at covered current tokens against 2.91 / 2.97 / 2.70x
# at uncovered ones, invariant across a 3x coverage change. That says WHICH MECHANISM, not WHICH SITE.
#
# The program substitutes 36 sites and §1829 found the entire bottom-up fall concentrated at layer 1, so
# there is a specific reason to expect the tracking is not spread evenly. This measures it by LEAVE-ONE-
# OUT RESTORATION: for each site s, run the program with all 36 sites compiled EXCEPT s, which stays
# live, and measure how much the agreement enrichment rises. A site whose restoration buys nothing is a
# site whose table already reproduces what the model does there.
#
# Restoration rather than single-site substitution on purpose: §1845 scoped a single-site result and
# §1840 showed the program's cost is NOT first-order along the substitution direction, so a one-site
# perturbation of the LIVE model does not predict a one-site relaxation of the COMPILED one. The
# question here is about the compiled program, so the baseline must be the compiled program.
#
# Offered to Codex on the board four times (06:36 / 06:44 / 06:52 / 07:04Z) and unclaimed; they are on
# the shared-RRR line. Taking it rather than idling the lane, and saying so on the board.
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY. Rung 3: §1885-§1888's open question.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
# LESSON 70 applied: enrichment is guarded at the computation site and every figure prints its n.
#   pred_a THE TRACKING IS CONCENTRATED: the single best-restoring site accounts for at least 15% of the
#          total enrichment gain summed over all 36 restorations, on all three roles. Register the SUM
#          over parts as the denominator, per LESSON 57 -- a share-of-total on the max alone cannot
#          separate dominance from redundancy. If FALSE the tracking is spread across sites and no small
#          set of tables carries the program's fidelity, which would say the compiled object is
#          irreducibly distributed and bound the value of any per-site compression.
#   pred_b AND IT IS THE EARLY LAYERS: the best-restoring site sits at layer <= 5 on all three roles, as
#          §1829's layer-1 concentration predicts. If FALSE the agreement is carried somewhere §1829's
#          CE-based account did not point to, which would be the more interesting result -- the two
#          instruments would then disagree about where the program is weakest.
#   pred_c AND EVERY RESTORATION HELPS: no site's restoration LOWERS the enrichment below the all-
#          compiled baseline by more than 0.1x, on any role. Restoring a site gives the program strictly
#          more of the true model, so a large drop would mean the compiled sites are compensating for
#          each other and the program is not a sum of independent substitutions. If FALSE that is a
#          structural fact about the program worth more than this run's headline.
#   pred_d CONTROLS: coverage is exactly 5,419; the all-compiled baseline reproduces §1888's PUBLISHED
#          covered-arm 7.19 / 7.29 / 7.64x within 0.05x (it reproduced to three digits at §1889 and
#          §1890); and all 36 restorations are measured on the identical position set.
...
OUT = PT + 'ops/per_site_tracking_results.json'
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'PER-SITE LEAVE-ONE-OUT RESTORATION | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    pa = all(share[e] >= 0.15 for e in roles)
    pb = all(lay[e] <= 5 for e in roles)
    pc = all(worst[e] >= -0.1 for e in roles)
    pd = (ncov == NCOV
          and all(abs((res['baseline'][e] or 0.0) - S1888_COV[e]) <= 0.05 for e in roles)
          and len(SK) == 36)

    print(f'\n  THE TRACKING IS CONCENTRATED (best site >=15% of summed gain) -> {pa}  ' + '  '.join(
        f'{e} {best[e]} {share[e]:.1%} of {tot[e]:.2f}x total' for e in roles), flush=True)
    print(f'  and it is an EARLY layer (<=5) -> {pb}  ' + '  '.join(
        f'{e} {best[e]} (layer {lay[e]})' for e in roles), flush=True)
    print(f'  and NO restoration hurts by >0.1x -> {pc}  ' + '  '.join(
        f'{e} worst {worst[e]:+.2f}x' for e in roles), flush=True)
    print(f'  coverage {ncov}, 36 sites, baseline reproduces §1888 -> control {pd}  ' + '  '.join(
        f'{e} {res["baseline"][e]:.2f}x vs {S1888_COV[e]:.2f}x' for e in roles), flush=True)
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_tracking_concentrated_program_concentrated_on_frequent_targets': bool(pa),
                          'pred_b_best_site_is_early_more_concentrated_than_live': bool(pb),
                          'pred_c_no_restoration_hurts_keeps_60pc_on_the_top_bucket': bool(pc),
