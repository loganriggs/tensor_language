# THE HEADLINE FIGURES AT NPERM = 64 -- converting "consistent within +-0.2" into tight numbers.
#
# §1914 found the permutation-null enrichment carries about +-0.2 at NPERM = 64, and §1915 measured it:
# seed-to-seed spread 0.218 / 0.334 / 0.188, falling to 0.061 / 0.123 / 0.096 at NPERM = 64, with the
# means differing by only 0.020 / 0.007 / 0.008 -- unbiased, merely noisy. Every central value in the arc
# is therefore right in expectation, but the anchor "reproductions" quoted to three digits were tighter
# than the instrument supported, and §1915 is the record of by how much rather than a fix.
#
# This is the fix, for the figures that carry the arc's conclusions. Three configurations, measured at
# NPERM = 64 under eight seeds each, so the record has values with stated precision instead of values
# with an unstated +-0.2:
#   the all-compiled COVERED baseline      (quoted throughout as 7.16 / 7.29 / 7.49; §1915 says 7.102 /
#                                           7.255 / 7.432)
#   restoring attn5                        (§1897: 1.06 / 1.03 / 1.06 -- the collapse)
#   restoring attn17 amplified x16         (§1908: 2.74 / 3.09 / 2.95 -- the CONSTRUCTED collapse, which
#                                           is what makes the alignment-x-magnitude result a designed
#                                           falsification rather than a survey)
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY. Rung 3: §1915's implied bookkeeping.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
#   pred_a THE COLLAPSES ARE FAR ABOVE THE NOISE: at NPERM = 64 the attn5 and attn17-x16 enrichments are
#          both below 3.5x on all three roles, against a baseline near 7.1-7.4x and a per-seed spread
#          under 0.15. §1908's conclusion rests on these two numbers; if either sits within a few spreads
#          of the baseline the designed falsification was weaker than I reported. If FALSE I would have
#          to soften §1908's headline, which is the most-cited result of the last two ticks.
#   pred_b AND THE PRECISION IS ACHIEVED: the seed-to-seed spread of every one of the nine measured
#          quantities is below 0.15. §1915 measured 0.061 / 0.123 / 0.096 for the baseline; the collapsed
#          configurations have far smaller enrichments and should be tighter still. If FALSE, NPERM = 64
#          is not enough for the numbers this record needs and the arc needs a higher setting.
#   pred_c AND THE NPERM=8 VALUES WERE UNBIASED: each configuration's NPERM = 64 mean is within 0.3 of
#          its PUBLISHED NPERM = 64 value (baseline 7.16 / 7.29 / 7.49; attn5 1.06 / 1.03 / 1.06;
#          attn17-x16 2.74 / 3.09 / 2.95). §1915 established unbiasedness on the baseline alone; this
#          extends it to the collapsed regime, where the null is estimated over a very different
#          agreement distribution. If FALSE the estimator is biased somewhere in that regime and §1915's
#          conclusion does not generalise from one configuration to three.
#   pred_d CONTROLS: coverage is exactly 16,110; all three configurations are measured on identical
#          covered positions; and the eight seeds are the same eight for every configuration.
...
OUT = PT + 'ops/headline_figures_at_nperm64_results.json'
NPERM = 64            # permutations averaged per bucket; the calibration above used 8
...
PROBE = [('attn', 17), ('attn', 5)]
MULTS = (16.0, 1.0)   # attn17 x16 (the constructed collapse); attn5 x1 (the natural one)
S1908_PUB = {'attn17_m16': [2.74, 3.09, 2.95], 'attn5_m1': [1.06, 1.03, 1.06],
             'baseline': [7.16, 7.29, 7.49]}   # §1897/§1908 PUBLISHED at NPERM = 8
...
def compare_by_bucket(rows, hooks, seenmask=None):
    ...
    def _en(dl, dp):
        ...
        pm = sum(float((L == P[torch.randperm(P.numel(), generator=gen).to(P.device)]).double().mean())
                 for _ in range(NPERM)) / NPERM
        # eight INDEPENDENT seeds at NPERM each, so the record carries a spread rather than a point
        seeds = []
        for sd in range(8):
            g3 = torch.Generator(device='cpu').manual_seed(2000 + sd)
            nl = sum(float((L == P[torch.randperm(P.numel(), generator=g3).to(P.device)])
                           .double().mean()) for _ in range(NPERM)) / NPERM
            seeds.append(ob / max(nl, 1e-12))
        return {'agreement': ob, 'permutation_null': pm,
                'enrichment': (ob / max(pm, 1e-12)) if L.numel() >= 100 else None,
                'enrichment_seeds': seeds,
                'enrichment_mean': sum(seeds) / 8.0,
                'enrichment_spread': max(seeds) - min(seeds),
                'n': int(L.numel())}
    for nm in ('cov', 'unc'):
        n2 = max(mech[nm]['n'], 1)
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'HEADLINE FIGURES AT NPERM=64 | buckets {BUCKETS} on the fit-row count of the TRUE '
        ...
        for mu in MULTS:
            ...
            for ename in evs:
                ...
                row[ename] = {'enrichment': c['mech_cov']['enrichment'], 'n': c['mech_cov']['n'],
                              'mean': c['mech_cov']['enrichment_mean'],
                              'spread': c['mech_cov']['enrichment_spread']}
    ...
    CFG = [('attn17', 'm16', 'attn17_m16'), ('attn5', 'm1', 'attn5_m1')]

    def st(k2, mk, e, f):
        return res['sites'][k2][mk][e][f]
    allsp = [st(k2, mk, e, 'spread') for k2, mk, _ in CFG for e in roles] \
        + [res['baseline_stat'][e]['spread'] for e in roles]
    pa = all(st(k2, mk, e, 'mean') < 3.5 for k2, mk, _ in CFG for e in roles)
    pb = all(x < 0.15 for x in allsp)
    pc = (all(abs(st(k2, mk, e, 'mean') - S1908_PUB[tag][i9]) <= 0.3
              for k2, mk, tag in CFG for i9, e in enumerate(roles))
          and all(abs(res['baseline_stat'][e]['mean'] - S1908_PUB['baseline'][i9]) <= 0.3
                  for i9, e in enumerate(roles)))
    pd = ncov == NCOV and len(allsp) == 9

    print(f'\n  headline figures at NPERM={NPERM}, 8 seeds (mean +- spread):', flush=True)
    for i9, e in enumerate(roles):
        print(f'    {e:10s} baseline {res["baseline_stat"][e]["mean"]:.3f} '
              f'+-{res["baseline_stat"][e]["spread"]:.3f} (§1908 pub {S1908_PUB["baseline"][i9]:.2f})  '
              + '  '.join(f'{tag} {st(k2, mk, e, "mean"):.3f} +-{st(k2, mk, e, "spread"):.3f} '
                          f'(pub {S1908_PUB[tag][i9]:.2f})' for k2, mk, tag in CFG), flush=True)
    print(f'\n  THE COLLAPSES ARE FAR ABOVE THE NOISE (< 3.5x) -> {pa}', flush=True)
    print(f'  and the PRECISION is achieved (all 9 spreads < 0.15) -> {pb}  max {max(allsp):.3f}',
          flush=True)
    print(f'  and the NPERM=8 values were UNBIASED (within 0.3) -> {pc}', flush=True)
    print(f'  coverage {ncov}, 9 quantities -> control {pd}', flush=True)
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_collapses_above_noise': bool(pa),
                          'pred_b_precision_achieved': bool(pb),
                          'pred_c_nperm8_unbiased': bool(pc),
