"""HOW BIG IS THE INTERACTION BETWEEN THE FRONTIER'S TWO LARGEST BLOCKS? (Claude, LANE 1)

SS2886 priced every member of `cfgF` and found the decomposition does NOT close: six single-block error shares sum to **+1.7928** of the
frontier's **+2.6735**, leaving **+0.8807 (32.9%)** that no single block accounts for. That missing third is not another block -- every
one is priced -- it is interaction structure, already visible as drifts of +0.0270 (SS2882) and +0.2129 (SS2883) and as outright
superadditivity inside the MLP stage (SS2880: +3.2104 against an additive 1.4350).

This rung measures one interaction term directly, between the two largest blocks: the FRONT MLP TABLES (+1.0045, SS2883) and the MOTIF
HEADS (+0.3988, SS2882). Restoring both together and comparing against the sum of the singles gives

    interaction = share_both - (share_front + share_motif)
      NEGATIVE = the two blocks explain OVERLAPPING error (restoring both recovers less than the sum)
      POSITIVE = superadditive, as SS2880 found inside the MLP stage
...
eval-only rungs overpay 4x). All four arms differ only in what is passed to `evalM`, so they are evaluated against ONE fitted stack:
**one pipeline run instead of four.** pred_e is the instrument check on that refactor -- the baseline arm must reproduce SS2883's
baseline to within .001, or the saving came at the cost of a different construction and nothing else here is readable.

SIGN CONVENTION (SS2135): frontier L2 is CE ADDED ABOVE THE REAL MODEL, so **LOWER IS BETTER** (SS312: +2.6735 beating +2.84/+2.93).
An ERROR SHARE = L2_F(baseline) - L2_F(restored), **POSITIVE = that block contributes that much error**.
...
#                     pred_b_the_interaction_term_is_large
#                     pred_c_the_joint_restoration_exceeds_either_alone
#                     pred_d_both_arms_are_connected
#                     pred_e_the_fit_once_refactor_reproduces_S2883s_baseline

Preregistration: polynomial_causal/FRONTIER_INTERACTION_FRONT_MOTIF_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run, four evaluations (baseline / front off / motif off / both off)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_interaction_front_motif_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_INTERACTION_FRONT_MOTIF_PREREGISTRATION.md'
PREREG_SHA = "ea786059fb00995c224e7cf39fe7a0abcbf5e231a90d32c7b453d8adc2ab410d"
...
BARS = {"reproduce": 0.05, "interaction": 0.15, "joint_exceeds": 0.10, "connected": 0.005, "refactor": 0.001}
NULLS = {"interaction_le": 0.05, "refactor_ge": 0.01}
...
if __name__=='__main__':
    ...
    for p_, h_ in ((S2883, S2883_SHA), (S2882, S2882_SHA)):
        if _sha(p_) != h_:
    ...
    # ONE fitted stack, four evaluations -- the fit-once/eval-many pattern from ops/frontier_evalarms.py.
    ARMS[:] = [('baseline',   {}),
               ('front_off',  {'drop': frozenset(['m'])}),
               ('motif_off',  {'motif': []}),
               ('both_off',   {'drop': frozenset(['m']), 'motif': []})]
    print(f'ONE pipeline run, {len(ARMS)} evaluations against the same fitted stack',flush=True)
    out=main()
    r_base, r_front, r_motif, r_both = (out['baseline'], out['front_off'],
                                        out['motif_off'], out['both_off'])

    # SIGN: L2 is CE ADDED ABOVE THE REAL MODEL, LOWER IS BETTER. Restoring an approximation to REAL should
    # LOWER L2, so ERROR SHARE = baseline - restored, POSITIVE = that block contributes that much error.
    sh_front = r_base['L2_F'] - r_front['L2_F']
    sh_motif = r_base['L2_F'] - r_motif['L2_F']
    sh_both  = r_base['L2_F'] - r_both['L2_F']
    inter    = sh_both - (sh_front + sh_motif)      # NEGATIVE = the two blocks explain overlapping error
    refactor = abs(r_base['L2_F'] - prev83['L2_F_baseline'])

    pa = abs(r_base['L2_F']-2.6735) <= BARS['reproduce']
    pb = abs(inter) >= BARS['interaction']
    pc = sh_both >= max(sh_front, sh_motif) + BARS['joint_exceeds']
    pd = (abs(sh_front) >= BARS['connected']) and (abs(sh_motif) >= BARS['connected'])
    pe = refactor <= BARS['refactor']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_interaction_term_is_large':bool(pb),
           'pred_c_the_joint_restoration_exceeds_either_alone':bool(pc),
           'pred_d_both_arms_are_connected':bool(pd),
           'pred_e_the_fit_once_refactor_reproduces_S2883s_baseline':bool(pe)}
    nulls={'b_null_the_two_blocks_are_additive':bool(abs(inter)<=NULLS['interaction_le']),
           'e_null_the_refactor_changed_the_construction':bool(refactor>=NULLS['refactor_ge'])}
    res={'rung':'frontier_interaction_front_motif','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,
         'arms':out,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(r_base['L2_F'],4),
                    'L2_F_front_off':round(r_front['L2_F'],4),
                    'L2_F_motif_off':round(r_motif['L2_F'],4),
                    'L2_F_both_off':round(r_both['L2_F'],4),
                    'share_front_tables_nats':round(sh_front,4),
                    'share_motif_heads_nats':round(sh_motif,4),
                    'share_both_nats':round(sh_both,4),
                    'sum_of_singles_nats':round(sh_front+sh_motif,4),
                    'interaction_nats':round(inter,4),
                    'S2883_share_front':round(prev83['error_share_front_tables_nats'],4),
                    'S2882_share_motif':round(prev82['error_share_motif_heads_nats'],4),
                    'refactor_baseline_deviation':round(refactor,6),
                    'S2886_closure_gap_for_reference':0.8807,
                    'pipeline_runs_saved_vs_per_arm':3},
         ...
         'hashes':{PREREG:PREREG_SHA,S2883:S2883_SHA,S2882:S2882_SHA},'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {r_base['L2_F']} vs 2.6735 (tol .05): {'HELD' if pa else 'FAILED'}")
    print(f"(b) interaction {inter:+.4f} (|.| >= {BARS['interaction']}), singles sum {sh_front+sh_motif:+.4f} vs joint {sh_both:+.4f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) joint exceeds either alone: {'HELD' if pc else 'FAILED'}")
    print(f"(e) fit-once refactor deviation {refactor:.6f} <= {BARS['refactor']}: {'HELD' if pe else 'FAILED'}")
    print(f'wrote {OUT} ({res["price"]["gpu_seconds"]:.0f}s, 1 pipeline run instead of 4)')
