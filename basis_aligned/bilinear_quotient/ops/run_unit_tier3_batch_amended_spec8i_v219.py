#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the ninth eight spec-authored behaviours, plus one instrument.
"""v219: the AMENDED tier-3 battery (v196/v197 protocol, unchanged) over the NINTH EIGHT spec-authored behaviours -- plus one instrument.

v204 4/8, v205 6/8, v207 4/8, v209 4/8, v211 3/8, v213 3/8, v215 5/8, v217 3/8 (+instrument each) at ~30-40 GPU-s per behaviour.
Eight more, authored through `circuit_fast_screen_behaviour_spec`, single-cue swap, matched final token, cue never final, 32
A1/A2/P/C rows, ABAB directions, opening ONE NEW family and extending four: superlative_complement_best (`The crate near the
lantern was best/better, of course,` -> of/than: the adjective's DEGREE selects the complement marker, NEW family) and
superlative_complement_largest (`largest/larger, of course,`, same family); less_fewer_countability (`carried less/fewer of the` ->
water/crates: quantifier-to-noun countability, the reverse readout of the countability family's noun-to-much/many) and
article_countability (`carried another/enough ADJ` -> crate/water, across an intervening adjective); number_of_agreement (`The/A
number of the pilots near the lantern` -> has/have: the same head noun agrees by determiner; agreement family, partitive sibling);
one_anaphora_number (`wanted the heavy crate/crates, not the light` -> one/ones: anaphoric pro-form number, number family, NEW
construction); for_complementizer_waited (`waited/claimed, of course,` -> for/that) and for_complementizer_arranged
(`arranged/insisted, of course,`), the for-infinitival vs finite-that contrast, complementizer family (whether/that siblings).
...
all eight; mean base/donor margins A1: for_complementizer_waited -5.72/+5.73, superlative_complement_largest -3.74/+3.40,
number_of_agreement -3.66/+3.63, superlative_complement_best -3.31/+2.73, article_countability -3.22/+2.79,
for_complementizer_arranged -2.90/+2.93, one_anaphora_number -2.49/+2.67, less_fewer_countability -1.82/+2.06 (the WEAKEST cell);
A2: waited -5.30/+5.17, arranged -3.75/+3.55, article -3.11/+2.71, number_of -2.82/+2.91, one_anaphora -2.78/+2.93, best -2.32/+2.06,
largest -2.32/+2.15, less_fewer -1.34/+1.57. No draft was replaced in this batch.
Protocol identical to v197/v201/v204/v205/v207/v209/v211/v213/v215/v217 (greedy pool 40, target 0.88, min_gain 0.005, max 30; row
2 = MIN over the two held directions >= 0.80; row 3 dim A1 CE LB975 > 0 and >= 0.10; row 4 constrained rank-1 own-C UB975 <= 0.01
...
REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 4 of the 8 new behaviours pass all four rows (v204 4/8, v205 6/8, v207 4/8, v209 4/8, v211 3/8,
                           v213 3/8, v215 5/8, v217 3/8 with the same recipe; less_fewer_countability is the expected miss from
                           its CPU margin; the four-row rate has been 3-5 of 8 for four batches).                     prior 50%
  pred_b_row2           -> MIN held-out per-direction exact-set recovery >= 0.80 on at least 6 of 8.              prior 75%
  pred_c_row3           -> dim A1 CE damage LB975 > 0 and >= 0.10 on at least 7 of 8 (v217 missed this at 5/8:
                           two dim rows NEGATIVE on 12- and 17-unit sets; bar kept, not tuned).                   prior 60%
...
(superlative pair, countability pair + countability, number_of + partitive_agreement, one_anaphora + the number family,
for_complementizer pair + the complementizer trio), so all wait for the next separability rung before counting.
...
Smoke: V219_SMOKE=<out.json> (CPU, V219_SMOKE_ROWS=4 per split, V219_SMOKE_NAMES=number_of_agreement; pool 3/max 3, steps 5).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec8i_v219_result.json"
...
NAMES = {"superlative_complement_best": "superlative_complement_best", "superlative_complement_largest": "superlative_complement_largest",
         "less_fewer_countability": "less_fewer_countability", "article_countability": "article_countability",
         "number_of_agreement": "number_of_agreement", "one_anaphora_number": "one_anaphora_number",
         "for_complementizer_waited": "for_complementizer_waited", "for_complementizer_arranged": "for_complementizer_arranged",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec8i_v219", "behaviours": 9, "constructions": 1,
...
def main() -> None:
    ...
    smoke = os.environ.get("V219_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V219_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V219_SMOKE_NAMES", "number_of_agreement").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec8i_v219", "candidate_id": "corpus.unit_tier3_batch_amended_spec8i_v219", "bars": BARS,
