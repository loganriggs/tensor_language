#!/usr/bin/env python3
# BQGATE:7 body forwards;32 sequences lengths4-12;4 batched suffix arms;no fit;180sec.
...
def main():
    ...
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('7 body forwards32sequences4-12tokens;4suffix arms;no fitting');return
    ...
    pre=torch.empty(32,1152,device='cuda');writes={name:torch.zeros(32,1152,dtype=torch.float64,device='cuda') for name in programs};replays=[];count=0
    ...
    try:
        for length,ids in sorted(buckets.items()):
            ...
            for pos in range(length):
                ...
                if pos not in (0,length-2):continue
                ...
                for name,(components,matrix) in programs.items():
                    terms=execute(query,source,components,matrix,cross_factors(components,*weights));writes[name][ids]+=(terms[:,:,-2:]*(gate/g0[None])[:,:,None,None]).sum((1,2))
            replays.append(float((native_sum-attention[:,-1]).norm()/attention[:,-1].norm()))
    finally:handle.remove()
    assert count==7
    candidate=writes['collapsed'];swap=candidate[torch.arange(32,device='cuda')^1];swap=swap*candidate.norm(dim=-1,keepdim=True)/swap.norm(dim=-1,keepdim=True).clamp_min(1e-30)
    def margins(removal):
    ...
    arms=torch.stack([margins(torch.zeros_like(pre)),margins(candidate),margins(swap),margins(writes['secant'])],1);cells=[]
    for family in (0,1):
    ...
    collapse=float((candidate-writes['secant']).norm()/writes['secant'].norm());instrument=max(replays)<=1e-5 and collapse<=.1;capability=all(c['native_mean']>=.1 and c['native_positive']>=5 for c in cells)
    ...
    torch.save(dict(pre=pre.cpu(),writes={k:v.cpu() for k,v in writes.items()},arm_margins=arms.cpu()),art)
    result.update(instrument_pass=instrument,native_capability_pass=capability,cells=cells,relative_block_collapse_error=collapse,native_replay_errors=replays,arm_order=['native','delete_block','delete_paired_context_direction_normmatched','delete_secant'],arm_margins=arms.cpu().tolist(),body_forwards=count,seconds=time.perf_counter()-tic,artifact_sha=digest(art),source_shas=binding,scope='Prospective semantic screen of frozen weight-discovered block. Source0/previous only. Paired direction control preserves local removal norm, not native causal structure. No OOD or full four-property promotion from this screen.')
