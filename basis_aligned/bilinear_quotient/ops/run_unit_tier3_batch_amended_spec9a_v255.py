#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the twenty-seventh eight spec-authored behaviours, plus one instrument.
"""v255: the AMENDED tier-3 battery (v196/v197 protocol, unchanged) over the TWENTY-SEVENTH batch of spec-authored behaviours (EIGHT) -- plus one instrument.
...
v231 7/8, v233 3/8, v235 4/8, v237 2/8, v239 3/8, v241 4/6, v243 5/7, v245 2/6, v247 4/6, v249 3/8, v251 5/8, v253 (running at authoring time)
(+instrument each) at ~30-45 GPU-s per behaviour.
FIFTH batch under the v245 lesson (pre-screen floor 1.0 on A1, mean base/donor on the donor axis; one repair or drop). Seven cells are
preposition-selection pairs under a grammatical cue (the selecting verb / adjective / noun) -- the class that has cleared four rows on 9 of 11
attempts since v247 and came out 23/23 separable (sib <= 0.034) in v222/v240/v244 -- each a NEW preposition PAIR (word-boundary, quote-agnostic
grep: for/at, to/with, about/for, into/from, of/about, at/about, for/about all 0 hits). The EIGHTH cell opens a NEW class, verb_particle
(phrasal-verb particle selection, woke -> up / calmed -> down; up/down 0 hits and a NEW readout pair), as a SINGLE pre-screened cell: after v253
lost three numeral_dual cells to capability at once, a new class gets one cell through the CPU floor and the battery before siblings are authored.
Cue words are fresh (referred/coped, apologized/laughed, cared/voted, turned/recovered, ashamed/serious, skilled/passionate, desire/concern,
woke/calmed: none appears in a committed spec module; `proud`, `dealt`, `listened`, `wanted` were avoided because they do).
...
verb_preposition_for_at (`Near the lantern the pilot apologized/laughed, of course,` -> for/at; verb_preposition family);
verb_preposition_to_with (`... referred/coped, of course,` -> to/with; verb_preposition family);
verb_preposition_about_for (`... cared/voted, of course,` -> about/for; verb_preposition family);
verb_preposition_into_from (`... turned/recovered, of course,` -> into/from; verb_preposition family);
adjective_preposition_of_about (`... was ashamed/serious, of course,` -> of/about; adjective_preposition family);
adjective_preposition_at_about (`... was skilled/passionate, of course,` -> at/about; adjective_preposition family);
noun_preposition_for_about (`... expressed a desire/concern, of course,` -> for/about; noun_preposition family);
verb_particle_up_down (`... woke/calmed, of course,` -> up/down; NEW class verb_particle, no family yet -- a four-row pass counts as a singleton).
CPU capability margins BEFORE enqueue (mean base / mean donor on the donor axis; A1 | A2; dropped rows; floor 1.0 on A1 per the v245 lesson):
  verb_particle_up_down A1:-3.40/+3.43 drop0 A2:-3.46/+3.42 drop0 C:-1.43/+0.93 drop2
  verb_preposition_for_at A1:-2.78/+2.90 drop0 A2:-2.63/+2.80 drop0 C:-1.43/+0.93 drop2
  verb_preposition_to_with A1:-4.78/+4.69 drop0 A2:-4.27/+4.04 drop0 C:-1.43/+0.93 drop2
  verb_preposition_about_for A1:-1.93/+1.76 drop0 A2:-2.08/+2.14 drop0 C:-1.43/+0.93 drop2
  verb_preposition_into_from A1:-3.40/+3.74 drop0 A2:-3.52/+3.23 drop0 C:-1.43/+0.93 drop2
  adjective_preposition_of_about A1:-2.89/+2.89 drop0 A2:-2.13/+2.18 drop0 C:-1.43/+0.93 drop2
  adjective_preposition_at_about A1:-3.56/+3.50 drop0 A2:-2.17/+2.03 drop0 C:-1.43/+0.93 drop2
  noun_preposition_for_about A1:-3.92/+4.04 drop0 A2:-3.38/+3.47 drop0 C:-1.43/+0.93 drop2
  All eight A1 cells are >= 1.76 with zero dropped rows (floor met); no drops, no repairs this batch. The v253 sink (three numeral_dual cells
  authored together, all failed capability) is answered by the pre-screen order: the single NEW-CLASS cell (verb_particle_up_down) was
  capchecked FIRST and alone (A1 3.40/3.43, A2 3.46/3.42) before any sibling of its class was considered; siblings (off/out) wait for its
  four-row result. Batch size EIGHT; bars are the eight-cell bars (4/6/7/5/5 of 8).
Protocol identical to v197/v201/v204/.../v253 (greedy pool 40, target 0.88, min_gain 0.005, max 30; row 2 = MIN over the two held directions
...
REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 4 of the 8 new behaviours pass all four rows (pre-screened batches 19/35 = 0.54 before v253). prior 55%
...
Accounting: four-row passes here join the one-protocol standing. The seven preposition cells have families with counted members
(verb_preposition 6 counted after v244; adjective_preposition 6 counted after v244; noun_preposition 2 counted) -- none counts before a
separability rung under its full family control set (v248 stem or later); verb_particle_up_down has NO family and counts as a singleton on a
four-row pass (standing rule) unless a second verb_particle cell is authored and passes, in which case both go to a separability rung.
...
Smoke: V255_SMOKE=<out.json> (CPU, V255_SMOKE_ROWS=4 per split, V255_SMOKE_NAMES=verb_particle_up_down; pool 3/max 3, steps 5).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec9a_v255_result.json"
...
NAMES = {"verb_particle_up_down": "verb_particle_up_down",
         "verb_preposition_for_at": "verb_preposition_for_at", "verb_preposition_to_with": "verb_preposition_to_with",
         "verb_preposition_about_for": "verb_preposition_about_for", "verb_preposition_into_from": "verb_preposition_into_from",
         "adjective_preposition_of_about": "adjective_preposition_of_about", "adjective_preposition_at_about": "adjective_preposition_at_about",
         "noun_preposition_for_about": "noun_preposition_for_about",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec9a_v255", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V255_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V255_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V255_SMOKE_NAMES", "verb_particle_up_down").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec9a_v255", "candidate_id": "corpus.unit_tier3_batch_amended_spec9a_v255", "bars": BARS,
