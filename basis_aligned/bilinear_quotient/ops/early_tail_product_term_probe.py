#!/usr/bin/env python
"""early_tail_product_term_probe -- COMPOSITIONALITY ACROSS DEPTH: does the late-block finding (the width beyond the read frame is a
core-GATED LINEAR read of the tail -- cross terms 83%, tail*tail 7%, §2780) hold for the EARLY blocks (0-7) reading through their OWN
site frames? Exact split Left(c+t)*Right(c+t) = Lc*Rc + (Lc*Rt + Lt*Rc) + Lt*Rt at k = 384 / 512 / 768; EARLY_FULL_512 must be 0. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_early_cross_share_512_large pred_c_early_tt_share_512_small
#                     pred_d_early_cross_share_768_large pred_e_early_terms_near_additive_512
...
Preregistration: polynomial_causal/EARLY_TAIL_PRODUCT_TERM_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/early_tail_product_term_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("early_tail_product_term_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "EARLY_TAIL_PRODUCT_TERM_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_tail_gate_rank_probe_results.json"   # §2782
OUT = ROOT / "early_tail_product_term_probe_results.json"
HASHES = {PREREG: "3535e8138ca30721f0c71290015383667e4146691eff9ea383aaee4e49b76aa4", PRIOR: "b3b494e10d301fa2161e731a73b584e59d0843d3e3052ca2d139d706f8fae1e4",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "early_tail_product_term_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "exact_tol": 0.002, "b_min": 0.60, "c_max": 0.30, "d_min": 0.60, "e_lo": 0.7, "e_hi": 1.3}
NULLS = {"b_max": 0.35, "c_min": 0.50, "d_max": 0.35, "e_max": 0.5}
...
def main():
    ...
    arms = {"SPLIT8_1024": split8(1024), "LATE_DROP_TT_768": late(768, "cross"), "EARLY_MLP_512": early(512, "none"), "EARLY_DROP_TT_512": early(512, "cross"), "EARLY_DROP_CROSS_512": early(512, "tt")}
    if not smoke:
        arms["EARLY_FULL_512"] = early(512, "all"); arms["EARLY_MLP_768"] = early(768, "none"); arms["EARLY_DROP_TT_768"] = early(768, "cross"); arms["EARLY_DROP_CROSS_768"] = early(768, "tt")
        arms["EARLY_MLP_384"] = early(384, "none"); arms["EARLY_DROP_TT_384"] = early(384, "cross"); arms["EARLY_DROP_CROSS_384"] = early(384, "tt")
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    def shares(k):
        j = arms.get(f"EARLY_MLP_{k}"); tt = arms.get(f"EARLY_DROP_TT_{k}"); cr = arms.get(f"EARLY_DROP_CROSS_{k}")
        if j is None or tt is None or cr is None or j <= 1e-9:
            return None
        return {"joint": j, "drop_tt": tt, "drop_cross": cr, "tt_share": tt / j, "cross_share": cr / j, "sum_over_joint": (tt + cr) / j}
    s512 = shares(512); s768 = shares(768); s384 = shares(384)
    summ = {"early_512": s512, "early_768": s768, "early_384": s384, "early_full_512_should_be_zero": arms.get("EARLY_FULL_512"), "late_drop_tt_768": arms["LATE_DROP_TT_768"]}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(arms["LATE_DROP_TT_768"] - PRIOR_DROP_TT_768) <= BARS["repro_tol"] and abs(arms.get("EARLY_FULL_512", 0.0)) <= BARS["exact_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_early_cross_share_512_large': bool(s512 is not None and s512["cross_share"] >= BARS["b_min"]),
        'pred_c_early_tt_share_512_small': bool(s512 is not None and s512["tt_share"] <= BARS["c_max"]),
        'pred_d_early_cross_share_768_large': bool(s768 is not None and s768["cross_share"] >= BARS["d_min"]),
        'pred_e_early_terms_near_additive_512': bool(s512 is not None and BARS["e_lo"] <= s512["sum_over_joint"] <= BARS["e_hi"]),
    }
    nulls = {"b_null_cross512_le_.35": bool(s512 is not None and s512["cross_share"] <= NULLS["b_max"]), "c_null_tt512_ge_.50": bool(s512 is not None and s512["tt_share"] >= NULLS["c_min"]),
             "d_null_cross768_le_.35": bool(s768 is not None and s768["cross_share"] <= NULLS["d_max"]), "e_null_sum512_le_.5": bool(s512 is not None and s512["sum_over_joint"] <= NULLS["e_max"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "blocks 0-7 MLP reads through their OWN k-frames (rest constant), the bilinear product split exactly into core*core + cross + tail*tail at k = 384/512/768; LATE_DROP_TT_768 and SPLIT8_1024 as instruments",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "late_drop_tt_768": arms["LATE_DROP_TT_768"], "prior_drop_tt_768": PRIOR_DROP_TT_768, "early_full_512": arms.get("EARLY_FULL_512")},
