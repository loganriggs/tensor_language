"""THE QUALITY-END COMBINATION (rung 194): front 4608 + rank-32 patterns + no tailE.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The three structure arms measured on
the r16 base: drop-tailE -0.152, exact front -0.298, r16->r32 -0.248. Naive-additive combination lands
~+0.57; the interaction ladder (S2207) says some is eaten. One config, all three.

REGISTERED PREDICTIONS (anchors: the S2288/S2289/S2291 receipts):
  (a) COMBINATION RETAINS 2/3: census <= 0.80.
  (b) NAME COMPOUNDS: name <= 1.35.
  (c) SANITY: census >= 0.4; inert tripwire vs +1.1153.
NULL: interactions eat >= half the additive gains (census > 0.92). PRICE: front 63.7M + patterns 24.2M ~
88M values (the quality end; the cheap end is rung 195). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: quality-end combination')
    raise SystemExit(0)
import torch
...
if __name__=='__main__':
    ...
    pa=agg<=0.80
    pb=PC.get('name',9.9)<=1.35
    pc=agg>=0.4
    if abs(agg-1.1153)<1e-4:
    ...
    print(f"(a) census {agg:+.4f} <= 0.80: {'HELD' if pa else 'FAILED'}")
    print(f"(b) name {PC.get('name',9.9):+.4f} <= 1.35: {'HELD' if pb else 'FAILED'}")
    print(f"(c) census >= 0.4: {'HELD' if pc else 'FAILED'}")
