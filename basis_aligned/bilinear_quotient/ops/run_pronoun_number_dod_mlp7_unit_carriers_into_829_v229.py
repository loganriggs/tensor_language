#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_carrier_closure pred_b_top_10_units_carry_half pred_c_top_50_units_carry_080 pred_d_largest_unit_supports pred_e_1779_is_a_top10_relay
"""Pronoun number they/he DoD (v229): MLP 7's NATIVE relay units into the plural detector 829, and whether the compensating unit 1779 (v228: 40% of MLP 7's
push-back under the MLP-6 removal) is one of them. v188: MLP 7 carries 18% of 829's plural - singular contrast at the noun (its pair mass is 1%: a carrier, not a
multiplier). Carrier-level unit census (v192's identity) of MLP 7 into 829: unit j's carrier term over a pair is da_j mB + mA db_j with c_j = lambda0_8 h7_j
Down7[:, j]; closure tolerance 1e-2 (fp32 sums over 4608 units).
PREDICTIONS (scored as written; failures preserved; priors unsure)
    pred_a_carrier_closure           unit terms sum to MLP 7's carrier term within relative 1e-2, every pair
    pred_b_top_10_units_carry_half   top-10 |pooled| units carry >= 0.50 of MLP 7's carrier total
    pred_c_top_50_units_carry_080    top-50 >= 0.80
    pred_d_largest_unit_supports     the largest |pooled| unit has the sign of MLP 7's carrier total
    pred_e_1779_is_a_top10_relay     unit 1779 is among the top-10 carriers (relay and restorer are the same unit). Prior: unsure.
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_mlp7_unit_carriers_into_829_v229_result.json"
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_mlp7_unit_carriers_into_829_v229"
UNIT, SRC, DST, CLOSURE_TOL, TOP10_MIN, TOP50_MIN, BATCH = 829, 7, 8, 1e-2, 0.50, 0.80, 32
FORWARDS_MAX = 5
PREDICTIONS = {"pred_a_carrier_closure": "<= 1e-2", "pred_b_top_10_units_carry_half": ">= 0.50", "pred_c_top_50_units_carry_080": ">= 0.80", "pred_d_largest_unit_supports": "sign of the MLP-7 carrier total", "pred_e_1779_is_a_top10_relay": "1779 in top-10"}
...
def main() -> None:
    ...
    predictions = {"pred_a_carrier_closure": closure <= CLOSURE_TOL, "pred_b_top_10_units_carry_half": share(10) >= TOP10_MIN, "pred_c_top_50_units_carry_080": share(50) >= TOP50_MIN, "pred_d_largest_unit_supports": (top[0][1] > 0) == (total > 0), "pred_e_1779_is_a_top10_relay": 1779 in [j for j, _ in top[:10]]}
    print("rank of 1779:", next((k for k, (j, _) in enumerate(top) if j == 1779), ">40"))
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_mlp5_unit_carriers_into_829_result_v229", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "carrier_total": total, "shares": {str(k): share(k) for k in (5, 10, 20, 50, 100, 500)},
