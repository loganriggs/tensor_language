#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_unit_closure pred_b_575_or_2921_in_top_three pred_c_top_10_carry_half pred_d_overlap_with_noun_census pred_e_top_unit_is_a_number_unit
"""MLP 16's units into 701's R input AT THE VERB (v471). v459 (at the noun, adjacent verbs): MLP 16's 575 / 2921 write 701's R input. v470: at a distant
verb MLP 16's whole write at the answer position moves 0.137 of 701's contrast. Are the same MLP-16 units at work at the verb position? MLP 16's write at the
answer position split exactly by unit on 701's R direction, 79 distant rows, compared with v459's census at the noun.
PREDICTIONS (scored as written; failures preserved; priors from v459)
    pred_a_unit_closure              per-unit terms sum to MLP 16's term within relative 1e-3 on every row
    pred_b_575_or_2921_in_top_three  575 or 2921 is among the three largest |pooled contrast| units at the verb
    pred_c_top_10_carry_half         the top 10 units carry >= 0.50
    pred_d_overlap_with_noun_census  the top-20 sets at the verb and at the noun (v459) share >= 8 units. Prior: unsure
    pred_e_top_unit_is_a_number_unit the top unit's plural - singular activation contrast at the verb has one sign on >= 0.80 of the pairs
...
OUT = ROOT / "circuits/followups/mlp16_units_into_701_verb_v471_result.json"
CANDIDATE_ID = "chain.mlp16_units_into_701_verb_v471"
N_HEAD, LAYER = 9, 16
CLOSURE_TOL, TOP10_MIN, OVERLAP_MIN, SIGN_MIN = 1e-3, 0.50, 8, 0.80
FORWARDS_MAX = 4
PREDICTIONS = {"pred_a_unit_closure": "<= 1e-3", "pred_b_575_or_2921_in_top_three": "rank <= 3", "pred_c_top_10_carry_half": ">= 0.50", "pred_d_overlap_with_noun_census": ">= 8 of 20", "pred_e_top_unit_is_a_number_unit": ">= 0.80 one sign"}
...
def main() -> None:
    ...
    for r_ in recs:
        ...
        if alt is None or r_["second_offset"] - c < 2: continue           # distant verbs only
        sw = list(r_["ids"]); sw[c] = alt; plural, singular = (r_["ids"], sw) if r_["cue"] == "plural" else (sw, r_["ids"])
        vs = r_["second_offset"]; items.append((plural[:vs], singular[:vs], vs - 1))   # position = the answer (verb slot)
    plan = {"candidate_id": CANDIDATE_ID, "adjacent_pairs": len(items), "layer": LAYER, "reader": "direct (W_U)", "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"closure_tol": CLOSURE_TOL, "top10_min": TOP10_MIN, "overlap_min": OVERLAP_MIN, "sign_min": SIGN_MIN}}
    ...
    prev = json.loads((ROOT / "circuits/followups/mlp16_units_into_701_v459_result.json").read_text())["report"]["top_units"][:20]; overlap = len({int(j) for j, _ in prev} & set(order[:20].tolist())); report["top20_overlap_with_v459"] = overlap
    r575 = {j: int((total.abs() > abs(total[j])).sum()) + 1 for j in (575, 2921)}; report["ranks_575_2921"] = r575
    predictions = {"pred_a_unit_closure": closure <= CLOSURE_TOL, "pred_b_575_or_2921_in_top_three": min(r575.values()) <= 3, "pred_c_top_10_carry_half": share(10) >= TOP10_MIN, "pred_d_overlap_with_noun_census": overlap >= OVERLAP_MIN, "pred_e_top_unit_is_a_number_unit": sign_const >= SIGN_MIN}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "mlp16_units_into_701_verb_result_v471", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
