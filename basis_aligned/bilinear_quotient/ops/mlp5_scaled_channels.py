# DOES SCALING RESCUE THE OUTLIER CHANNELS?  -- and is mlp5's live output actually exploded?
#
# §1835 kept k of mlp5's 1152 channels LIVE and found the cost is not channel-concentrated (256 channels,
# 22% of the width, buy 30.5% of the 61.2pp stake; 16 buy 0.2%) and that keeping the OUTLIER channels --
# §1089's object, Logan's hypothesis -- is WORSE THAN RANDOM at all four k (-1.7, -4.0, -2.6, -6.3pp).
#
# §1835 closed with an INTERPRETATION it explicitly did not test: §1804 found layer 5 exploding 152x when
# fed a compiled stream, so if mlp5's live module produces garbage in exactly its largest channels when
# its input is compiled, the table is PROTECTING against that explosion and preserving those channels
# reintroduces it. That would explain both the negative sign and the OUT arm's non-monotonicity (+3.4% at
# k=64, then -2.4% at k=256).
#
# This run does two things §1835 did not. It MEASURES the supposed explosion -- mean per-channel |mlp5
# output| in the B0 stream against the fully live model, on the very channels the OUT selection picks --
# and it gives the outlier hypothesis a fair second chance by keeping those channels live but RESCALED
# per channel to the substituted row's mean magnitude, so the direction is preserved and the magnitude
# fault removed.
#
...
#   pred_a SCALING RESCUES THE OUTLIER CHANNELS: with per-channel magnitude matching, OUT16 buys at least
#          10% of the 61.2pp stake, against -2.7% unscaled. If TRUE the outlier channels do carry usable
#          signal and §1835's negative was a magnitude fault, not a content one -- §1089's hypothesis is
#          partially rescued and §1804's explosion is shown to reach mlp5. If FALSE the outlier channels
#          are worthless even at the right scale, and the outlier framing is CLOSED for this object
#          rather than merely unsupported.
#   pred_b AND THE LIVE OUTPUT IS ACTUALLY EXPLODED: mean per-channel |mlp5 output| in the B0 stream
#          exceeds the fully live model's by at least 2.0x on the top-16 OUT channels. This measures the
#          mechanism §1835 could only suppose. If FALSE the live module is NOT inflated on those channels,
#          §1804's explosion does not extend to mlp5, and §1835's negative sign needs a different
#          explanation entirely -- which would matter, because I have already written the explosion
#          reading into the ledger as a hypothesis.
#   pred_c AND CONCENTRATION STILL FAILS: even scaled, DISC64 buys under 50% of the stake. §1835's core
#          negative should not be a scaling artefact, and this checks it directly. If FALSE, scaling
#          rescues concentration too, and §1835 understated how much of mlp5 lives in a few channels --
#          in which case §1835's headline needs correcting, not just extending.
#   pred_d CONTROLS, cross-run per LESSON 42: K0 reproduces §1834/§1835's PUBLISHED 3.6% gap fraction and
#          B0 §1829's PUBLISHED 64.8%, both within 0.03; the UNSCALED OUT16, OUT64, DISC64 and DISC256
#          arms reproduce §1835's PUBLISHED -2.7%, +3.4%, +8.0% and +30.5% of stake within 5pp; the
#          DISC/OUT top-16 overlap is again 0. Endpoints must reproduce §1789's FULL-RANK top-1; the
#          placement control must move top-1 under 0.05pp. Coverage 5419 of 50257.
...
OUT = PT + 'ops/mlp5_scaled_channels_results.json'
...
KEEPS = (16, 64, 256)                 # channels kept LIVE, of D = 1152
SELECTORS = ('DISC', 'OUT', 'RAND')
# §1835's PUBLISHED unscaled fractions of the 61.2pp stake, skip7000
S1835_UNSCALED = {'OUT16': -0.027, 'OUT64': 0.034, 'DISC64': 0.080,
                  'DISC256': 0.305}
...
def partial_row_hook(full_rows, keep_ch, scale=None):
    ...
    def hook(mod, args, out):
        y = out[0] if isinstance(out, tuple) else out
        sub = full_rows[STATE['idx'].reshape(-1)].reshape(y.shape).to(y.dtype)
        live = y if scale is None else y * scale.view(1, 1, -1).to(y.dtype)
        sub = torch.where(keep_ch.view(1, 1, -1), live, sub)
        return (sub,) + tuple(out[1:]) if isinstance(out, tuple) else sub
    return hook
...
def chan_probe(full_rows, store):
    ...
    def hook(mod, args, out):
        ...
        s = store.setdefault('a', [torch.zeros(D, device=DEV), torch.zeros(D, device=DEV),
                                   0, torch.zeros(D, device=DEV)])
        s[0] += o[:, 64:].abs().sum((0, 1))
        s[1] += (o[:, 64:] - rr[:, 64:]).abs().sum((0, 1))
        s[2] += int(o[:, 64:].shape[0] * o[:, 64:].shape[1])
        s[3] += rr[:, 64:].abs().sum((0, 1))
        return r
    return hook
...
def main():
    ...
    print(f'MLP5 SCALED CHANNELS | magnitude-matched live channels + explosion check | '
    ...
    # channel statistics in the B0 stream (the stream the arms run in) AND on the fully live model,
    # so the supposed explosion is a measured ratio rather than an assumption.
    store = {}
    evaluate(ev0, [(st, allh[st]) for st in B0_SITES] + [(TARGET, chan_probe(frr[TARGET], store))],
             keep_mask)
    acc = store['a']
    mag = (acc[0] / max(acc[2], 1)).cpu()          # |mlp5 live output| per channel, B0 stream
    disc = (acc[1] / max(acc[2], 1)).cpu()         # |live - row| per channel, B0 stream
    rowmag = (acc[3] / max(acc[2], 1)).cpu()       # |substituted row| per channel, B0 stream
    lstore2 = {}

    def live_probe(hook_store):
        def hook(mod, args, out):
            o = (out[0] if isinstance(out, tuple) else out).detach().float()
            s = hook_store.setdefault('a', [torch.zeros(D, device=DEV), 0])
            s[0] += o[:, 64:].abs().sum((0, 1))
            s[1] += int(o[:, 64:].shape[0] * o[:, 64:].shape[1])
            return None
        return hook
    evaluate(ev0, [(TARGET, live_probe(lstore2))], keep_mask)
    livemag = (lstore2['a'][0] / max(lstore2['a'][1], 1)).cpu()
    ...
    rank = {'DISC': disc.argsort(descending=True), 'OUT': mag.argsort(descending=True),
            'RAND': rnd}
    top16 = rank['OUT'][:16]
    blow = float((mag[top16] / livemag[top16].clamp_min(1e-9)).mean())
    overlap = len(set(rank['DISC'][:16].tolist()) & set(top16.tolist()))
    # per-channel magnitude match: scale the kept LIVE channel to the row's mean magnitude
    chscale = (rowmag / mag.clamp_min(1e-9)).to(DEV)
    print(f'\n  mlp5 in the B0 stream: mean|live| max {float(mag.max()):.3f} '
          f'median {float(mag.median()):.3f}; |live-row| max {float(disc.max()):.3f}; '
          f'DISC/OUT top-16 overlap {overlap}/16', flush=True)
    print(f'  EXPLOSION CHECK on the top-16 OUT channels: B0-stream |live| / fully-live |live| '
          f'= {blow:.3f}x   (live-model mean on those channels '
          f'{float(livemag[top16].mean()):.3f}, B0-stream {float(mag[top16].mean()):.3f})', flush=True)
    print(f'  magnitude-match scales on those channels: median '
          f'{float(chscale.cpu()[top16].median()):.4f}', flush=True)

    print(f'\n  arms  ({time.time() - t0:.0f}s)', flush=True)
    run_g('B0', list(B0_SITES), gseq['B0'])
    zero = torch.zeros(D, dtype=torch.bool, device=DEV)
    run_g('K0', armsites, gseq['arm'], {TARGET: partial_row_hook(frr[TARGET], zero)})
    for selname in SELECTORS:
        for k in KEEPS:
            ...
            run_g(f'{selname}{k}S', armsites, gseq['arm'],
                  {TARGET: partial_row_hook(frr[TARGET], m2, chscale)})
    ...
    names = ['B0', 'K0'] + [f'{s3}{k3}{sf}' for s3 in SELECTORS for k3 in KEEPS
                            for sf in ('', 'S')]
    seq = {k2: frac[PICK_ROLE][k2] for k2 in names}
    b0, k0 = seq['B0'], seq['K0']
    stake = b0 - k0
    rec = {n: (seq[n] - k0) / max(stake, 1e-9) for n in names if n not in ('B0', 'K0')}
    pa = rec['OUT16S'] >= 0.10
    pb = blow >= 2.0
    pc = rec['DISC64S'] < 0.50
    pd = (abs(k0 - S1834_MLP5_GAPFRAC) <= 0.03
          and abs(b0 - S1829_B0_GAPFRAC) <= 0.03
          and all(abs(rec[n] - v) <= 0.05 for n, v in S1835_UNSCALED.items())
          and overlap == 0
    ...
    print(f'\n  FRACTION OF THE {stake:.1%} STAKE BOUGHT ({PICK_ROLE}; B0 {b0:.1%}, K0 {k0:.1%}):',
          flush=True)
    for s3 in SELECTORS:
        print(f'    {s3:5s} raw    ' + '  '.join(
            f'k{k3} {rec[f"{s3}{k3}"]:+7.1%}' for k3 in KEEPS), flush=True)
        print(f'    {s3:5s} SCALED ' + '  '.join(
            f'k{k3} {rec[f"{s3}{k3}S"]:+7.1%}' for k3 in KEEPS), flush=True)
    print(f'\n  SCALING rescues the outlier channels (OUT16 scaled >=10% of stake) -> {pa}  '
          f'{rec["OUT16S"]:+.1%} scaled vs {rec["OUT16"]:+.1%} raw', flush=True)
    print(f'  and mlp5 IS exploded in the B0 stream (>=2.0x on top-16 OUT) -> {pb}  {blow:.3f}x',
          flush=True)
    print(f'  and CONCENTRATION still fails even scaled (DISC64 scaled <50%) -> {pc}  '
          f'{rec["DISC64S"]:+.1%}', flush=True)
    print(f'  K0 §1834, B0 §1829, four unscaled arms reproduce §1835, overlap {overlap} '
          f'-> control {pd}', flush=True)

    json.dump({'run': 'mlp5_scaled_channels', 'role': PICK_ROLE, 'target': list(TARGET),
               'keeps': list(KEEPS), 'selectors': list(SELECTORS),
               'B0_gapfrac': b0, 'K0_gapfrac': k0, 'stake': stake,
               'explosion_ratio_top16_out': blow, 'disc_out_overlap_16': overlap,
               'sequential': seq, 'fraction_of_stake': rec, 'gap_fraction': frac,
               'predictions': {'pred_a_scaling_rescues_outliers': bool(pa),
                               'pred_b_mlp5_is_exploded': bool(pb),
                               'pred_c_concentration_still_fails': bool(pc),
