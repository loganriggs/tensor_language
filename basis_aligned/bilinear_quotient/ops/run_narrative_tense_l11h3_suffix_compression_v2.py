...
PRIOR=ROOT/'circuits/prior_art/NARRATIVE_TENSE_L11H3_SUFFIX_COMPRESSION_V2_PREREGISTRATION.md'
OUT=ROOT/'circuits/fast_screens/narrative_tense_l11h3_suffix_compression_v2_result.json'
PRIOR_SHA256='25ad229b15ec2941b7b51da6d5f22ef36a46efdb44f27ef0ac7576406f1c76fb'
ARMS=('native_reinstall','complete_head','all_post_value','early_three_value','suffix_three_value','role_value','of_the_value')
...
def compile_plan():
 ...
 return {'schema':'narrative_tense_l11h3_suffix_compression_plan_v2','candidate_id':'narrative_tense.l11h3_suffix_compression_v2','model_loaded':False,'gpu_accessed':False,'queue_touched':False,'prior_art_sha256':PRIOR_SHA256,'capability_license_sha256':carrier.LICENSE_SHA256,'row_count':len(rows()),'layer':carrier.core.LAYER,'head':carrier.core.HEAD,'arms':list(ARMS),'bars':BARS,'registered_predictions':list(PREDICTION_REGISTRY),'price':{'model_forwards':6,'example_evaluations':576,'backwards':0,'parameter_updates':0}}
...
def evaluate(model,torch,F,facade):
 ...
 for start in range(0,len(rr),carrier.core.BATCH):
  ...
  for i,r in enumerate(chunk):
   ...
   hs={'native_reinstall':b['head'][i],'complete_head':d['head'][i],'all_post_value':repl['all_post'],'early_three_value':repl['early_three'],'suffix_three_value':repl['suffix_three'],'role_value':repl['role'],'of_the_value':repl['of_the']}
   for a in ARMS:inds.append(i);heads.append(hs[a]);spec.append((i,a))
  ix=torch.tensor(inds,dtype=torch.long,device=device)
  plog,_=factor._factor_forward(model,btt[ix],bff[ix],torch,F,facade,replacement_heads=torch.stack(heads),replacement_head_mask=torch.ones(len(heads),dtype=torch.bool,device=device),native_reinstall_mask=torch.tensor([a=='native_reinstall' for _,a in spec],dtype=torch.bool,device=device))
  for oi,(i,a) in enumerate(spec):
   r=chunk[i];q=int(bff[i]);native=metrics(blog[i],q,r,torch);changed=metrics(plog[oi],q,r,torch)
   if a=='native_reinstall':exact['native_reinstall_max_absolute_error']=max(exact['native_reinstall_max_absolute_error'],float((plog[oi]-blog[i]).abs().max()))
...
def main(argv:Sequence[str]|None=None):
 ...
 scored=score(ev,exact);term=terminal(scored['predictions']);result={'schema':'narrative_tense_l11h3_suffix_compression_result_v2','candidate_id':plan['candidate_id'],'terminal':term,'plan':plan,'checkpoint_weights_sha256':checkpoint.weights_sha256,'score':scored,'evidence':ev,'active_price':plan['price']};payload=managed.atomic_create_json(OUT,result);print(json.dumps({'terminal':term,'result_sha256':hashlib.sha256(payload).hexdigest()},sort_keys=True))
