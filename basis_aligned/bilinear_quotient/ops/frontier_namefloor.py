"""THE NAME FLOOR (rung 146): trajectory tail-attention + tailE bypass together.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2239: trajectory attn splice -> name
+1.321. S2237: the tailE bypass held +0.555 at module grade. Here both run together for name positions (the
whole tail complex idealized: real-trajectory attention values AND real tail-MLP span components). What
remains of name's bill is the front/motif DIRECT share (non-tail-mediated).

REGISTERED PREDICTIONS (anchors: S2239 name +1.321; S2237 tailE credit +0.555):
  (a) ROUGH ADDITIVITY: dCE(name) <= 0.9.
  (b) census <= 1.87.
  (c) ISOLATION: |L1F - 1.5509| <= 0.02 AND census >= 1.4.
NULL: the tailE credit was redundant with context corruption (name >= 1.15) - the same corrupted directions,
counted once. PRICE: none (oracle diagnostic). Tripwire: INSTRUMENT FAIL if census within 1e-3 of baseline.
Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: name floor (trajectory attn + tailE bypass)')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_namefloor_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['name_traj']=True; SEL['name_oracle_tail']=True
    print('ARM: rung-127 build + trajectory attn splice AND tailE bypass for name',flush=True)
    main()
    ...
    print(f"(a) [bar 0.9] name {PC.get('name',9.9):+.4f} <= 1.0: {'HELD' if pa else 'FAILED'}")
    print(f"(b) census {agg:+.4f} <= 1.87: {'HELD' if pb else 'FAILED'}")
