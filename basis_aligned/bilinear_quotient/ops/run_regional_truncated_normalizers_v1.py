...
Query32/key32 separate diagnostics; zero remainder; exact native nullspace preserved, no textfit.
...
from truncated_norm_quadratic_v1 import compile_norm,norm_mean
STEM='REGIONAL_TRUNCATED_NORMALIZERS_V1'
@torch.no_grad()
def main():
    ...
    result.update(cells=cells,prior_component_replay=replay,native_attention_replays=replays,norm_reader_scalars={str(k):4*9*k*1152+4*9*k for k in (32,64,128)},
                  seconds=time.perf_counter()-tic,body_forwards=count,artifact_sha=digest(art),source_shas=binding,
                  scope='Frozen shared component with top32/64 weight eigenreaders with zero complementary remainder;128 exact endpoint. No fit. Native input states retained; conditional norm extraction, not whole-model closure or arbitrary-input guarantee.')
