...
"""Exact five-port upstream decomposition of the dominant L11H3 subject U atom."""
...
import run_subject_number_l11h3_source_factor_mobius_v1 as parent
...
PARENT=POLY/"SUBJECT_NUMBER_L11H3_SOURCE_FACTOR_MOBIUS_V1_RESULT.json"
PREREG=POLY/"SUBJECT_NUMBER_L11H3_SUBJECT_VALUE_UPSTREAM_MOBIUS_V1_PREREGISTRATION.md"
BINDING=POLY/"SUBJECT_NUMBER_L11H3_SUBJECT_VALUE_UPSTREAM_MOBIUS_V1_BINDING.json"
OUT=POLY/"SUBJECT_NUMBER_L11H3_SUBJECT_VALUE_UPSTREAM_MOBIUS_V1_RESULT.json"
LAYER,HEAD,HEAD_WIDTH=11,3,128; NULLS,SEED=16,20260926
PORTS=("embedding_recurrence","early_writes_0_3","middle_writes_4_7","late_writes_8_10","first_value_bus")
BARS={"maximum_absolute_closure_error":1e-4,"maximum_component_reconstruction_error":1e-5,
      "maximum_native_value_replay_error":1e-4,"maximum_relative_closure_error":1e-5,
      "maximum_synthetic_error":1e-12,"maximum_term4_relative_l2":.25,
      "minimum_random_median_advantage":.10}
PRICE={"partial_forwards":4,"sequences":256,"corners":32,"mobius_terms":31,"readouts":17,
       "greedy_steps":4,"behavior_logits":0,"fits":0,"backwards":0,"parameter_updates":0}
PREDICTION_REGISTRY={"pred_a_exact_upstream_instrument":None,"pred_b_compact_upstream_terms":None,
                     "pred_c_writer_specificity":None}

def sha(path): return hashlib.sha256(Path(path).read_bytes()).hexdigest()

def greedy(target,atoms,names):
    residual=target.copy(); available=list(range(len(atoms))); selected=[]; curve=[]; denom=max(float(np.linalg.norm(target)),1e-30)
    for _ in range(4):
        choice=min(available,key=lambda i:float(np.linalg.norm(residual-atoms[i])))
        residual-=atoms[choice]; available.remove(choice); selected.append(names[choice]); curve.append(float(np.linalg.norm(residual)/denom))
    return {"selected":selected,"relative_l2_curve":curve,"final_relative_l2":curve[-1]}
...
def load_bound():
    binding=json.loads(BINDING.read_text()); paths={"authority":Path(authority.__file__),"source_primitive":Path(source_factor.__file__),
        "parent_runner":Path(parent.__file__),"decoder":DECODER,"rank1":RANK1,"parent_result":PARENT,"preregistration":PREREG}
    ...
    decoder,rank,parent_result=(json.loads(p.read_text()) for p in (DECODER,RANK1,PARENT))
    if decoder["terminal"]!="embedding_number_decoder_frozen" or rank["terminal"]!="rank1_frozen_weights_only" \
            or parent_result["terminal"]!="valid_source_factor_decomposition": raise ValueError("parent status changed")
    return binding,decoder,rank,parent_result,authority.build_rows()

def plan():
    _,decoder,rank,parent_result,rows=load_bound()
    return {"schema":"subject_number_l11h3_subject_value_upstream_mobius_v1_plan","model_loaded":False,"gpu_accessed":False,"queue_touched":False,
            "rows":len(rows),"batch_sizes":[64,64],"ports":list(PORTS),"corners":32,"mobius_terms":31,"readouts":17,"rank":rank["rank"],
            "decoder_axis_sha256":decoder["frozen_decoder"]["axis_float32_sha256"],"parent_terminal":parent_result["terminal"],
            "authority_sha256":authority.canonical(rows),"bars":BARS,"price":PRICE,"binding_sha256":sha(BINDING)}
...
def main():
    ...
    signal.alarm(600); binding,decoder,rank,parent_result,rows=load_bound()
    ...
    def capture(initial,positions):
        counts["partial_forwards"]+=1; counts["sequences"]+=len(initial); x=initial; x0=initial; first=None; components={"embedding_recurrence":initial.clone()}
        with torch.no_grad():
            for layer,block in enumerate(model.transformer.h):
                x=block.lambdas[0]*x+block.lambdas[1]*x0
                for key in components: components[key]=components[key]*block.lambdas[0]
                components["embedding_recurrence"]+=block.lambdas[1]*x0
                ...
                if layer==LAYER:
                    ...
                    grouped={"embedding_recurrence":components["embedding_recurrence"],
                             "early_writes_0_3":sum(components[k] for k in components if k.startswith(("attn_0","attn_1","attn_2","attn_3","mlp_0","mlp_1","mlp_2","mlp_3"))),
                             "middle_writes_4_7":sum(components[k] for k in components if k.startswith(("attn_4","attn_5","attn_6","attn_7","mlp_4","mlp_5","mlp_6","mlp_7"))),
                             "late_writes_8_10":sum(components[k] for k in components if k.startswith(("attn_8","attn_9","attn_10","mlp_8","mlp_9","mlp_10")))}
                    return x,grouped,first,factors
                attention,first=block.attn(state,first); x=x+attention; components[f"attn_{layer}"]=attention
                mlp=block.mlp(F.rms_norm(x,(x.shape[-1],))); x=x+mlp; components[f"mlp_{layer}"]=mlp
    ...
    for ids in group_ids:
        batch_rows=[rows[i] for i in ids]; tokens=torch.tensor([r["token_ids"] for r in batch_rows],dtype=torch.long,device=device); pos=torch.tensor([5]*len(ids),device=device); b=torch.arange(len(ids),device=device)
        ...
        def corner(mask):
            ...
            current=attention.c_v(state).view(len(ids),6,9,128)[:,:,HEAD]
            first=(first_r if mask&(1<<4) else first_b).view(len(ids),6,9,128)[:,:,HEAD]
    ...
    reports=[]
    for ai in range(17):
        atoms=np.concatenate(all_atoms[ai],axis=1); target=np.concatenate(all_target[ai]); report={"target_rms":float(np.sqrt(np.mean(target**2))),"greedy":greedy(target,atoms,names)}
    ...
    synthetic=parent.synthetic_fixture(); finite=bool(np.isfinite(np.asarray([component_error,value_replay,closure_abs,closure_rel,synthetic,*random_errors])).all())
    ...
    pred_b=bool(pred_a and target_report["greedy"]["final_relative_l2"]<=BARS["maximum_term4_relative_l2"])
    pred_c=bool(pred_b and advantage>=BARS["minimum_random_median_advantage"]); predictions=dict(zip(PREDICTION_REGISTRY,(pred_a,pred_b,pred_c)))
    terminal="compact_subject_value_upstream_candidate" if pred_c else "valid_subject_value_upstream_decomposition" if pred_a else "invalid"
    result={"schema":"subject_number_l11h3_subject_value_upstream_mobius_v1_result","terminal":terminal,"predictions":predictions,
            ...
            "wall_seconds":time.perf_counter()-started,"scope":"Opened-panel exact five-port upstream decomposition of the dominant L11H3 subject-value atom."}
    managed.atomic_create_json(OUT,result); print(json.dumps({k:result[k] for k in ("terminal","predictions","instrument","writer_axis","random_readout_specificity")},indent=2,sort_keys=True)); assert pred_a
