...
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';OUT=A/'native_matched_width_source_swap_v1_result.json'
PREDICTIONS=dict(pred_a_instrument='fiveport g/H1e-8,stateclosure1e-10,delta1e-5abs/1e-6rel,native/reference1e-4,priorquad6replay1e-4,counts',pred_b_selectivity='swap5 native80%alignedunitB retention/10%modal all48cells',pred_c_prediction='oldquad5/quad6/swap5 number10%/modal5%numberbudget everycell')
PLAN=dict(prefix=36,double_suffix=120,native_suffix=120,gradient=0,hessian=0,source_width=5,coefficients_per_context=80,predictions=PREDICTIONS)
...
def main():
    ...
    frozen_path=A/'six_source_complement_v1r1_result.json';frozen=json.loads(frozen_path.read_text());six_lookup={(c['dataset'],c['panel'],c['role'],c['template']):c for c in frozen['contexts']};six_records={(c['dataset'],c['panel'],c['role'],c['family'],c['arm']):c for c in frozen['records']}
    swaps=json.loads((P/'MATCHED_WIDTH_SOURCE_SWAP_V1.json').read_text());assert swaps['source_sha256']==hashlib.sha256(frozen_path.read_bytes()).hexdigest();swap_lookup={(c['dataset'],c['panel'],c['role'],c['template']):c for c in swaps['contexts'] if c['omitted_source']==1}
        ...
        for panel in ['opposite','congruent']:
            ...
            for template in dict.fromkeys(r['template'] for r in rows):
                ...
                for role,key in [('subject','subject_position'),('attractor','control_position')]:
                    ...
                    frozen_context=six_lookup[(dataset,panel,role,template)];g=torch.tensor(frozen_context['gradient'],device='cuda',dtype=torch.float64);h=torch.tensor(frozen_context['hessian'],device='cuda',dtype=torch.float64);base=evaluate(zero);baseline=native(zero)
                    ...
                    reference=np.array([0.,0.,1.,1.,1.,0.]);new=torch.tensor(frozen_context['amplitudes'],device='cuda',dtype=torch.float64);swap=torch.tensor(swap_lookup[(dataset,panel,role,template)]['amplitudes'],device='cuda',dtype=torch.float64)
                    old5=np.pad(np.array(quad_lookup[(dataset,panel,role,template)]['amplitudes']),((0,0),(0,1)))
                    contexts.append(dict(dataset=dataset,panel=panel,role=role,template=template,gradient=g.tolist(),hessian=h.tolist(),amplitudes=new.tolist(),optimizer_checks=[]))
                    arms=dict(unitB=torch.tensor(reference,device='cuda').expand_as(zero),oldquad5=torch.tensor(old5,device='cuda'),quad6=new,swap5=swap)
                    for arm,av in arms.items():
                        ...
                        for family in dict.fromkeys(r['family'] for r in entries):
                            ...
                            if arm=='quad6':oldreplays.append(float((y-torch.tensor(six_records[(dataset,panel,role,family,arm)]['target'],device='cuda',dtype=torch.float64)).abs().max()))
    ...
    instrument=counts=={k:PLAN[k] for k in counts} and max(replays)<=1e-4 and max(oldreplays)<=1e-4 and max(derivative_replays)<=1e-8 and max(closures)<=1e-10 and all(d['absolute']<=1e-5 and d['relative']<=1e-6 for d in deltas)
    ...
    primary=[c for c in comparisons if c['arm']=='swap5'];assert len(primary)==48
