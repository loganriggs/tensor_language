#!/usr/bin/env python3
"""Nested-regularized source-delta A1/A2/off router for the entry12 union."""

# BQGATE: EXPERIMENT pred_a_authority_nested_scope_replay_geometry_finiteness_and_price pred_b_regularized_router_identifies_held_branches pred_c_routed_union_preserves_both_targets pred_d_routed_union_is_control_selective pred_e_router_retains_ungated_union_effect
...
PRIOR = ROOT / "circuits/prior_art/temporal_iswas_v15_entry12_crossfit_regularized_source_router_v1.json"
SOURCE_ROUTER_RESULT = ROOT / "circuits/followups/temporal_iswas_v15_entry12_crossfit_source_delta_router_v1_result.json"
ORACLES = ROOT / "circuits/followups/temporal_iswas_v15_construction_oracle_projective_bisector_v1_result.json"
OUT = ROOT / "circuits/followups/temporal_iswas_v15_entry12_crossfit_regularized_source_router_v1_result.json"
CANDIDATE_ID = "temporal_auxiliary.iswas_v15_entry12_crossfit_regularized_source_router_v1"
EXPECTED = {
    "prior": "66926bb30ca3878533b319943755822f6c8a70074d869153efe2619b5ade0621",
    "source_router_result": "cf41b3c37a50813d71bc454abe2878cd4ec8701ec339610ac94fc81f690c34d8",
    ...
    "router": "cc396de977f8cdf70e367bbd4e68b620e7e823aed7eac620246a85dfa6946e49",
...
FILES = {
    "prior": PRIOR, "source_router_result": SOURCE_ROUTER_RESULT, "oracles": ORACLES,
    "finite_router": ROOT / "ops/entry12_finite_router_contract.py",
    "router": ROOT / "ops/regularized_source_router_contract.py",
...
PRICE_MAX = {"native_capture_forwards": 10, "differentiable_transformer_forwards": 24,
             ...
             "example_evaluations": 2000, "fit_parameters": 6915}
...
def main():
    ...
    prior, source_router_result = json.loads(PRIOR.read_text()), json.loads(SOURCE_ROUTER_RESULT.read_text())
    authority = (observed == EXPECTED and prior.get("candidate_id") == CANDIDATE_ID
                 and source_router_result.get("terminal") == "router_identification_failure")
    ...
    for held in (0, 1):
        ...
        train_features = finite_router.source_delta_features(
        ...
        train_groups = backend.torch.tensor([row["group_number"] for row in train_context["rows"]], device=backend.device)
        router_fit, nested = router.nested_select(
            backend.torch, train_features, train_labels, train_groups)
        held_context = contexts[held]; off_output, off_entry = off_cache[held]
        held_features = finite_router.source_delta_features(
        ...
        fits[str(held)] = {"basis": {"rank": int(union.shape[1]), **basis_geometry},
                           "router": {**classification, "nested": nested, "predictions": predicted.cpu().tolist(),
    ...
    nested_ok = all(
        len(fits[str(parity)]["router"]["nested"]["grid"]) == len(router.PENALTIES)
        and fits[str(parity)]["router"]["nested"]["selected_penalty"] in router.PENALTIES
        for parity in (0, 1))
    ...
    E = all(reports[str(parity)]["learned_router"][panel]["metrics"]["signed_projection"]
            ...
            for parity in (0, 1) for panel in ("A1", "A2"))
    predictions = {
        "pred_a_authority_nested_scope_replay_geometry_finiteness_and_price": A,
        "pred_b_regularized_router_identifies_held_branches": B,
    ...
    gold_pass = all(reports[str(parity)]["gold_router"][panel]["metrics"]["signed_projection"] >= .75
                    for parity in (0, 1) for panel in ("A1", "A2"))
    terminal = ("invalid" if not A else "regularized_source_router_candidate" if all((B, C, D, E))
    ...
    result = {"schema": "temporal_iswas_v15_entry12_crossfit_regularized_source_router_result_v1",
