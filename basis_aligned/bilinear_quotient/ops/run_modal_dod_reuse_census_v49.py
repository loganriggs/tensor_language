...
"""Modal would/will reuse census (v49): blind readout sweep + top-4 set on the line's authored A1/A2 contexts (opened rows).
Contrast `O_h^T(u_would - u_will)`. Predictions as in `dod_reuse_census.run`; prior for pred_e: yes (the 2026-09-06
greedy set for this line was {9.4, 11.3}). PRICE (registered maximum): 2+2+324+2+32 = 362 forwards; bar <= 380."""
...
if __name__ == "__main__":
    R.run("modal_remoteness.would_vs_will.dod_reuse_census_v49", "modal_remoteness_dod_reuse_census_v49_result.json", rows, pos, neg)
