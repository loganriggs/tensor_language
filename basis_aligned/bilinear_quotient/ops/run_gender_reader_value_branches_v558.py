#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_branches_add pred_c_token_only_branch_carries pred_d_current_branch_carries_most pred_e_mlp8_gap_is_the_token_branch
"""The gender readers' two value branches at the noun (v558). Each head's value is v = (1 - lamb) W_v n(live) + lamb v0, where v0 is block 0's value of the
same token (token-only) and lamb is a learned per-block scalar. On the number line the readers' copy was the current-state branch (0.94-1.0, v401-v406).
On the gender line the MLP-8 swap moves the readers' current-state values by 0.29-0.36 yet closes 0.096 of the margin through them against 0.152 from their
full-value singles (v557) -- as it would if a third of the readers' gender read sits in the token-only branch, which no write at the noun can touch. Here the
five readers' values at the noun swapped by branch: CUR (the current-state part only), TOK (the token-only part only), FULL (both; replay of 0.600).
PREDICTIONS (scored as written; failures preserved; priors from v549 / v557)
    pred_a_baseline_replays              the unedited manual forward replays the model's he - she margins within 1e-3
    pred_b_branches_add                  CUR + TOK = FULL +- 0.03 (the value is linear in its two branches; the readout is linear in the values, v556)
    pred_c_token_only_branch_carries     TOK closes >= 0.15 of the gap (the number line: <= 0.06 of the readers' copy)
    pred_d_current_branch_carries_most   CUR >= TOK
    pred_e_mlp8_gap_is_the_token_branch  CUR / FULL is within 0.10 of 0.096 / 0.152 = 0.63 (v557's readers' part over its full-value expectation)
...
OUT = ROOT / "circuits/followups/gender_reader_value_branches_v558_result.json"
CANDIDATE_ID = "gender.reader_value_branches_v558"
ROWS = ROOT / "circuits/followups/pronoun_gender_dod_natural_rows_v73.json"
N_HEAD = 9
FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, FIVE_V549, BAND, TOK_MIN, RATIO_V557, RBAND = 1e-3, 0.600, 0.03, 0.15, 0.096 / 0.152, 0.10
FORWARDS_MAX = 9
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_branches_add": "CUR + TOK = FULL +- 0.03", "pred_c_token_only_branch_carries": ">= 0.15", "pred_d_current_branch_carries_most": "CUR >= TOK", "pred_e_mlp8_gap_is_the_token_branch": "CUR/FULL = 0.63 +- 0.10"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "text_pairs": len(text_items), "rows": str(ROWS.name), "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "five_v549": FIVE_V549, "band": BAND, "tok_min": TOK_MIN, "ratio_v557": RATIO_V557, "rband": RBAND}}
    ...
    c = text["closed"]
    ratio = c["CUR"] / c["FULL"] if c["FULL"] else float("nan"); print("CUR", round(c["CUR"], 3), "TOK", round(c["TOK"], 3), "FULL", round(c["FULL"], 3), "CUR/FULL", round(ratio, 3), "v557 ratio", round(RATIO_V557, 3))
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_branches_add": abs(c["CUR"] + c["TOK"] - c["FULL"]) <= BAND, "pred_c_token_only_branch_carries": c["TOK"] >= TOK_MIN,
                   "pred_d_current_branch_carries_most": c["CUR"] >= c["TOK"], "pred_e_mlp8_gap_is_the_token_branch": abs(ratio - RATIO_V557) <= RBAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "gender_reader_value_branches_result_v558", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
