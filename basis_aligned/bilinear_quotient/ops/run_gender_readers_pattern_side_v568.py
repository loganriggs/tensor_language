#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_pattern_carries pred_c_pattern_and_values_make_the_write pred_d_pattern_below_values pred_e_all_positions_replay
"""The gender readers' pattern side (v568). v567: the readers' values swapped at every position from the noun on close 0.732 of the he - she gap; their writes
into the answer swapped close 0.878 (v563). A head's write is sum_t p_t v_t with p the SIGNED squared-attention pattern from the answer's query; the write swap
moves both p and v, the value swap only v. Here the readers' PATTERN swapped -- their queries and keys (both QK products) taken from the partner row, their
own values kept -- alone (PATTERN), and their values at every position from the noun on (ALL_POS, replay of 0.732).
PREDICTIONS (scored as written; failures preserved; priors from v563 / v567)
    pred_a_baseline_replays                the unedited manual forward replays the model's he - she margins within 1e-3
    pred_b_pattern_carries                 PATTERN closes >= 0.08 of the gap (the 0.146 the values do not carry)
    pred_c_pattern_and_values_make_the_write  PATTERN + ALL_POS is within 0.10 of 0.878 (the write swap)
    pred_d_pattern_below_values            PATTERN <= ALL_POS
    pred_e_all_positions_replay            ALL_POS closes 0.732 +- 0.03
...
OUT = ROOT / "circuits/followups/gender_readers_pattern_side_v568_result.json"
CANDIDATE_ID = "gender.readers_pattern_side_v568"
ROWS = ROOT / "circuits/followups/pronoun_gender_dod_natural_rows_v73.json"
N_HEAD = 9
FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, PATTERN_MIN, WRITE_V563, ADD, ALLPOS_V567, BAND = 1e-3, 0.08, 0.878, 0.10, 0.732, 0.03
FORWARDS_MAX = 7
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_pattern_carries": ">= 0.08", "pred_c_pattern_and_values_make_the_write": "0.878 +- 0.10", "pred_d_pattern_below_values": "PATTERN <= ALL_POS", "pred_e_all_positions_replay": "0.732 +- 0.03"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "text_pairs": len(text_items), "rows": str(ROWS.name), "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "pattern_min": PATTERN_MIN, "write_v563": WRITE_V563, "add": ADD, "allpos_v567": ALLPOS_V567, "band": BAND}}
    ...
    RULES = {"ALL_POS": lambda rel: rel >= 0, "PATTERN": lambda rel: rel < -10**6}
        ...
        with torch.no_grad():
            ...
            for l, block in enumerate(blocks):
                ...
                if mode != "native" and (l in MODES[mode][0] or l in MODES[mode][1] or MODES[mode][2]):
                    ...
                    if mode == "PATTERN":
                        y_own = attn.squared_attention(q, k, v, q2, k2); y_sw = attn.squared_attention(q[swap], k[swap], v, q2[swap], k2[swap])   # partner's pattern, own values
                        y = y_own.clone()
    ...
    c = text["closed"]
    print("pattern side", {k: round(v, 3) for k, v in c.items()}, "pattern + values", round(c["PATTERN"] + c["ALL_POS"], 3), "write swap", WRITE_V563)
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_pattern_carries": c["PATTERN"] >= PATTERN_MIN, "pred_c_pattern_and_values_make_the_write": abs(c["PATTERN"] + c["ALL_POS"] - WRITE_V563) <= ADD,
                   "pred_d_pattern_below_values": c["PATTERN"] <= c["ALL_POS"], "pred_e_all_positions_replay": abs(c["ALL_POS"] - ALLPOS_V567) <= BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "gender_readers_pattern_side_result_v568", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
