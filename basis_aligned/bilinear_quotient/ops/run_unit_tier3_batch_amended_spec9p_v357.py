#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the fifty-third six spec-authored behaviours, plus one instrument.
"""v357: the AMENDED tier-3 battery over the FIFTY-THIRD batch (SIX cells) -- plus one instrument.
...
v285 2/7, ..., v343 3/3, v349 1/4, v353 4/6 (+instrument each).
WHERE THIS SITS. v353 sent six preposition frame copies through the battery and four passed all four rows; v355 then
found all four separable from their own frame-A donors (fam-arm leaks 0.0091, 0.0223, 0.0078, -0.0055) while their
own-arm leaks into those donors ran 0.2869 to 0.9100. So the lane works and it is cheap -- 190 s of battery plus 341 s
of rung bought four counted circuits -- and the binding constraint is now how fast I can author, not verification.
This batch is six more copies drawn from the seventeen counted frame-A preposition circuits that still have none.
CELLS (frame in brackets; each borrows both cue -> token mappings from a counted frame-A circuit):
  verb_preposition_fb_from_about    suffered/boasted      [B]   from verb_preposition_from_about
  verb_preposition_fb_on_about      depended/complained   [B]   from verb_preposition_on_about
  verb_preposition_fb_into_with     crashed/flirted       [B]   from verb_preposition_into_with
  verb_preposition_fc_of_into       reeked/peered         [C]   from verb_preposition_of_into
  verb_preposition_fc_on_into       embarked/plunged      [C]   from verb_preposition_on_into
  verb_preposition_fc_through_with  rummaged/tinkered     [C]   from verb_preposition_through_with
Three in each frame, no mapping repeated within a frame, so the gate holds: a (cue -> token) mapping may be reused in
a DIFFERENT frame, never within one.
CPU capability margins BEFORE enqueue (mean base / mean donor; A1; dropped rows; floor 1.0):
  verb_preposition_fb_on_about      (depended/complained) A1:-6.72/+6.68 drop0
  verb_preposition_fb_from_about    (suffered/boasted)    A1:-5.59/+5.46 drop0
  verb_preposition_fc_of_into       (reeked/peered)       A1:-5.39/+5.32 drop0
  verb_preposition_fc_on_into       (embarked/plunged)    A1:-4.58/+4.39 drop0
  verb_preposition_fc_through_with  (rummaged/tinkered)   A1:-3.95/+3.78 drop0
  verb_preposition_fb_into_with     (crashed/flirted)     A1:-2.72/+2.84 drop0
  Six of six, and depended/complained at 6.72 is the strongest A1 margin measured in this corpus. That is NOT a
  forecast: v349's four particle cells sat at 2.4 to 3.5 and produced one four-row pass, and v353's two failures sat
  at 4.94 and 3.88. The floor says a cell can be studied; it has never said it will pass.
...
REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 3 of the 6 new behaviours pass all four rows. Preposition frame copies are 6 of 8
                           at this stage; the two misses were row-4 misses, not row-2.                     prior 70%
  pred_b_row2           -> MIN held-out per-direction exact-set recovery >= 0.80 on at least 5 of 6. No preposition
                           frame copy has missed row 2 yet; two particle ones did.                         prior 70%
...
Smoke: V357_SMOKE=<out.json> (CPU, V357_SMOKE_ROWS=4 per split, V357_SMOKE_NAMES=verb_preposition_fb_on_about).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec9p_v357_result.json"
...
NAMES = {"verb_preposition_fb_from_about": "verb_preposition_fb_from_about", "verb_preposition_fb_on_about": "verb_preposition_fb_on_about",
         "verb_preposition_fb_into_with": "verb_preposition_fb_into_with", "verb_preposition_fc_of_into": "verb_preposition_fc_of_into",
         "verb_preposition_fc_on_into": "verb_preposition_fc_on_into", "verb_preposition_fc_through_with": "verb_preposition_fc_through_with",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v357", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V357_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V357_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V357_SMOKE_NAMES", "verb_preposition_fb_on_about").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec9p_v357", "candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v357", "bars": BARS,
