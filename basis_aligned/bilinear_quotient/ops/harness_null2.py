"""HARNESS NULL v2 (rung 285): explicit empty actives at the census call - no knob semantics.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2383: knob-based "off" left legacy
replacements installed. Here the census evaluation itself is called with EMPTY active and motif lists
(evalV(rows, N, [], [])) - the one unambiguous "install nothing." Everything else in main runs as usual
(fits etc.) but cannot touch the census pass.
REGISTERED PREDICTIONS:
  (a) TRUE NULL: |census| <= 0.003 (CUDA wobble only).
  (b) valid >= 55 (zero damage -> members trivially certify).
  (c) |census| <= 0.02 (outer sanity); cev saved (cev_harnessnull2.pt).
NULL: harness leaks even with nothing installed (|census| > 0.02) - the census instrument itself is
biased and every number in the program needs re-anchoring. PRICE: probe. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: harness null v2 (explicit empty actives)')
    raise SystemExit(0)
import torch
...
OUT=PT+'harness_null2_results.json'
...
def main():
    ...
    if SEL.get('clsdmg'):
        ...
        SEL['cev']=evalV(_ROWS,_ROWS.shape[0],[],[]).detach().cpu()
...
if __name__=='__main__':
    ...
    cev=SEL['cev']
    torch.save(cev,PT+'cev_harnessnull2.pt')
    print('cev saved: cev_harnessnull2.pt',flush=True)
    ...
    print(f"(a) TRUE NULL - |census {agg:+.4f}| <= 0.003: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 55; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |census {agg:+.4f}| <= 0.02: {'HELD' if pc else 'FAILED'}")
