"""HALF-PRICE ON THE EIGHT FRESH WINDOWS (rung 24): metric-selected mlp4/mlp5 at K=1152 vs norm-selected at K=2304.

§2107 showed on two windows that metric selection at half the units matches norm selection at full units (window 1
+1.594 vs +1.581; window 2 +1.947 vs +1.907). §2116 certified the equal-price gain on §2083's eight fresh windows.
This certifies the half-price claim on the same eight windows.

REGISTERED PREDICTIONS (gaps vs the real model per window):
  (a) metric-1152 gap <= norm-2304 gap + 0.02 on >= 6 of the 8 windows.
  (b) median over windows of (norm-2304 gap - metric-1152 gap) >= -0.02.
  (c) REPRODUCTION: window 1 (R0:R1) gives metric-1152 +1.594 and norm-2304 +1.581 within 0.02 each (§2107).
  (d) cfgE block-6 rel-MSE 1.7415 within 0.10.

ORIGINAL HEADERS FOLLOW.


S2106/S2107 certified +0.124 / +0.075 nat (two windows of one token file) and a 2x unit compression for selecting
mlp4/mlp5's kept units by the block-5/6 loss-gradient metric. S2081-S2085 showed this program's "fresh" numbers can
collapse across windows (the gating arc's Nx random). This scores cfgE vs metric-units on the EIGHT pile-10k windows
probe_gate7 built (document-disjoint, 120 rows each, prefixes disjoint from every FW row), rebuilt with the same code.

REGISTERED PREDICTIONS:
  (a) gain(metric-units) > 0 on >= 7 of the 8 windows.
  (b) median gain over the 8 windows >= 0.04 nat (half of window 2's 0.075; the harder-text window).
  (c) the gain is more stable than the gap: sd(gain) <= 0.5 x sd(gap) across windows.
  (d) REPRODUCTION GATE: window-1 (R0:R1) gain reproduces 0.124 within 0.02; cfgE block-6 rel-MSE 1.7415 within 0.10.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['stream_error_profile_results.json','metric_units_fresh8_results.json','metric_units_ksweep_results.json']
    ...
    print(f"DRYRUN OK: S2086 present (block 6 {_p['6']}); norm-2304 vs metric-1152 x eight fresh windows (+ window 1 reproduction)")
    raise SystemExit(0)
import json, sys, time, torch
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
import os
...
OUT=PT+'metric_units_halfprice8_results.json'
...
def main():
    ...
    def build_arm(mode):
        ...
        def cp_piece(li,piece):
            ...
            KSEL=1152 if (mode=='metric-units' and li in (4,5)) else 2304
            keep=imp.argsort(descending=True)[:KSEL]
    ...
    for wi,w in enumerate(WIN):
        ...
        rows.append({'window':wi,'docs':w['docs'],'base_ce':round(b,5),'gap_plain':round(g['plain'],5),
                     'gap_metric':round(g['metric-units'],5),'diff':round(g['plain']-g['metric-units'],5)})
        print(f"window {wi}: docs {w['docs']:3d} base {b:.4f} | gap plain {g['plain']:+.4f} metric {g['metric-units']:+.4f} | gain {g['plain']-g['metric-units']:+.4f}",flush=True)
    diffs=[r['gap_plain']-r['gap_metric'] for r in rows]
    import statistics as stt
    pa=sum(r['gap_metric']<=r['gap_plain']+0.02 for r in rows)>=6
    pb=stt.median(diffs)>=-0.02
    pc=abs(gap1['metric-units']-1.5944)<=0.02 and abs(gap1['plain']-1.5808)<=0.02
    pd=abs(arms['plain']['profile'][6]-1.7415)<=0.10
    out={'windows':rows,'diff_median':round(stt.median(diffs),4),'diff_mean':round(stt.mean(diffs),4),
         'windows_within_0.02':int(sum(r['gap_metric']<=r['gap_plain']+0.02 for r in rows)),
         'window1':{'norm_2304':round(gap1['plain'],4),'metric_1152':round(gap1['metric-units'],4)},
         'S2107_window1':{'norm_2304':1.5808,'metric_1152':1.5944},'profiles':{k:v['profile'] for k,v in arms.items()},
         'pred_a_matches_on_6_of_8':bool(pa),'pred_b_median_diff':bool(pb),'pred_c_reproduces_S2107':bool(pc),
         'pred_d_reproduces_S2086':bool(pd),'self_reviewed':True,'runtime_s':round(time.time()-t0,1)}
    json.dump(out,open(OUT,'w'),indent=1)
    print(f"(d) cfgE b6 {arms['plain']['profile'][6]:.4f}: {'HELD' if pd else 'FAILED'}")
    print(f"(c) window 1: norm-2304 {gap1['plain']:+.4f} (1.581) metric-1152 {gap1['metric-units']:+.4f} (1.594): {'HELD' if pc else 'FAILED'}")
    print(f"(a) metric-1152 within +0.02 of norm-2304 on {out['windows_within_0.02']}/8: {'HELD' if pa else 'FAILED'}")
    print(f"(b) median(norm-2304 - metric-1152) {out['diff_median']:+.4f} >= -0.02: {'HELD' if pb else 'FAILED'}")
