#!/usr/bin/env python
"""late_square_directions_ablation_probe -- do the REAL mlp16/17 read their CE-relevant input through the five shared square directions
q_1..q_5 (§2734), in the same order the compiled program does? Input-ablation arms on the real blocks (single direction, top-5, whole
16-dim core, random 5-dim core subspaces) vs drop-one arms on the compiled program. CUDA lane-1 script.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_real_blocks_need_the_five pred_c_five_beat_random_in_the_real_blocks pred_d_same_order
#                     pred_e_five_carry_half_the_core
...
Preregistration: polynomial_causal/LATE_SQUARE_DIRECTIONS_ABLATION_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_square_directions_ablation_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_square_directions_ablation_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_SQUARE_DIRECTIONS_ABLATION_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_stack_extracted_program_probe_results.json"   # §2732
PRIOR34 = ROOT / "late_square_directions_identity_probe_results.json"   # §2734
OUT = ROOT / "late_square_directions_ablation_probe_results.json"
HASHES = {PREREG: "e12fbfd05110159d60d9205786bc1b1ca791a71c2818f6f1de8b7648ab54af8a", PRIOR: "a790b0f5e093e5625c690c19dbda9a4e36ab64a3ba41c4306b8d2de8e00dd68c", PRIOR34: "20aa36932f81f59bc402c47348083087a9517b19f8df860a3da2416d63e599cb",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_square_directions_ablation_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_PROG = 0.246; PRIOR_PROG5 = 0.273; LAM = 1e-2
POOL = [("mlp", l) for l in range(11, 16)]
KIN = 32; RSQ = 8; RB = 8; NTOP = 5; SEEDS = [0, 1, 2]
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_min": 0.30, "c_min": 2.0, "d_min": 0.7, "e_min": 0.5}
NULLS = {"b_max": 0.10, "c_max": 1.2, "d_max": 0.3, "e_max": 0.25}
...
def main():
    ...
    class RealAbl:
        """Real block with a core subspace (columns of Qs, orthonormal in stream coords) removed from its rms-normed input."""
        def __init__(self, l, Qs):
            self.mlp = m.transformer.h[l].mlp; self.Qs = Qs
        def hook(self, idx):
            pass
        def __call__(self, w, x):
            xh = F.rms_norm(x, (D,)); xp_ = xh - (xh @ self.Qs) @ self.Qs.T
    ...
    real_part = lambda Qs: {("mlp", 16): RealAbl(16, Qs), ("mlp", 17): RealAbl(17, Qs)}
    def drop(U_, j):
        keep = [i for i in range(U_.shape[1]) if i != j]; Uk = U_[:, keep]; return Uk @ Uk.T
    arms = {}
    for j in range(NTOP):
        arms[f"REAL_ABL_q{j + 1}"] = run([real_part(Q[:, j:j + 1])])
    arms["REAL_ABL_top5"] = run([real_part(Q[:, :NTOP])]); arms["REAL_ABL_core16"] = run([real_part(P)])
    rnd = []
    for sd in SEEDS:
        g = torch.Generator().manual_seed(sd); Rm = torch.linalg.qr(torch.randn(KM, NTOP, generator=g))[0].to(DEV)
        rnd.append(run([real_part(P @ Rm)]))
    arms.update({f"REAL_ABL_rand5_s{sd}": v for sd, v in zip(SEEDS, rnd)})
    arms["PROG_SHARED8"] = run([prog_part(Ush @ Ush.T)]); arms["PROG_SHARED5"] = run([prog_part(Ush[:, :NTOP] @ Ush[:, :NTOP].T)])
    for j in range(NTOP):
        arms[f"PROG_SHARED8_minus_u{j + 1}"] = run([prog_part(drop(Ush, j))])
        arms[f"PROG_SHARED5_minus_u{j + 1}"] = run([prog_part(drop(Ush[:, :NTOP], j))])
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    r_cost = [arms[f"REAL_ABL_q{j + 1}"] for j in range(NTOP)]
    p_cost = [arms[f"PROG_SHARED8_minus_u{j + 1}"] - arms["PROG_SHARED8"] for j in range(NTOP)]
    p5_cost = [arms[f"PROG_SHARED5_minus_u{j + 1}"] - arms["PROG_SHARED5"] for j in range(NTOP)]
    rho = R.spearman(torch.tensor(r_cost), torch.tensor(p_cost)); rho5 = R.spearman(torch.tensor(r_cost), torch.tensor(p5_cost))
    rand_med = float(torch.tensor(rnd).median()); top5 = arms["REAL_ABL_top5"]
    c_ratio = top5 / rand_med if rand_med > 0 else float("inf"); e_ratio = top5 / arms["REAL_ABL_core16"] if arms["REAL_ABL_core16"] > 0 else float("inf")
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["PROG_SHARED8"] - PRIOR_PROG) <= BARS["repro_tol"] and abs(arms["PROG_SHARED5"] - PRIOR_PROG5) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_real_blocks_need_the_five': bool(top5 >= BARS["b_min"]),
        'pred_c_five_beat_random_in_the_real_blocks': bool(c_ratio >= BARS["c_min"]),
        'pred_d_same_order': bool(rho >= BARS["d_min"]),
        'pred_e_five_carry_half_the_core': bool(e_ratio >= BARS["e_min"]),
    }
    nulls = {"b_null_top5_le_.10": bool(top5 <= NULLS["b_max"]), "c_null_ratio_le_1.2": bool(c_ratio <= NULLS["c_max"]),
             "d_null_rho_le_.3": bool(rho <= NULLS["d_max"]), "e_null_ratio_le_.25": bool(e_ratio <= NULLS["e_max"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "prog_shared8": arms["PROG_SHARED8"], "prior_prog": PRIOR_PROG, "prog_shared5": arms["PROG_SHARED5"], "prior_prog5": PRIOR_PROG5},
           "ce_added": arms, "real_per_direction": r_cost, "prog8_per_direction": p_cost, "prog5_per_direction": p5_cost,
           "spearman_real_vs_prog8": rho, "spearman_real_vs_prog5": rho5, "random5_median": rand_med, "top5_over_random": c_ratio, "top5_over_core16": e_ratio,
    ...
    if smoke:
        print(json.dumps({k: out[k] for k in ("preds", "nulls", "instrument", "ce_added", "real_per_direction", "prog8_per_direction", "spearman_real_vs_prog8", "top5_over_random", "top5_over_core16", "price")}, indent=1)); return
    from receipt import dump
    dump(out, OUT)
    print(json.dumps({"preds": preds, "nulls": nulls, "ce_added": arms, "rho": rho, "top5_over_random": c_ratio, "top5_over_core16": e_ratio}, indent=1))
