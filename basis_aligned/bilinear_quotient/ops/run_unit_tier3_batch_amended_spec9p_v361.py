#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the fifty-fourth four spec-authored behaviours, plus one instrument.
"""v361: the AMENDED tier-3 battery over the FIFTY-FOURTH batch (FOUR cells) -- plus one instrument.
The FRAME-CAPACITY test. Two mapping pairs that already carry counted circuits in three frames each get a fourth and a
fifth.
v285 2/7, ..., v349 1/4, v353 4/6, v357 4/6 (+instrument each).
THE QUESTION. Thirteen mapping pairs now support the frame-bound account: a cell reusing both cue -> token mappings of
a counted circuit in a different frame is separable from it, with own-arm leaks into the donor running 0.29 to 1.34
and fam-arm leaks under 0.024 (v341, v351, v355, v359). Every one of those tests compared a copy against the ORIGINAL.
None asked how many frames one mapping pair can carry before the copies start colliding with EACH OTHER. v345 got the
first data point -- the frame-B and frame-C copies of found/settled leak -0.0034 into each other -- but that is one
pair at density three.
    verb_particle_fd_up_down       woke/calmed   frame D   [A, B and C copies are all counted]
    verb_particle_fe_up_down       woke/calmed   frame E
    verb_preposition_fd_about_for  cared/voted   frame D   [A, B and C copies are all counted]
    verb_preposition_fe_about_for  cared/voted   frame E
Frame D is `Nobody doubted the {agent} {verb}, honestly,`; frame E is `Word spread that the {agent} {verb},
evidently,`. Both keep a parenthetical, which the candidate bank requires -- it enforces matched_final_input_token and
the parenthetical is what makes base and donor end on the same token -- so a "frame" here is always a matrix clause
plus a parenthetical, and that is the grain at which any capacity limit will be measured.
WHAT EACH OUTCOME MEANS, registered now rather than after the rung. If all five copies of a pair stay mutually
separable, the frame multiplier has no small ceiling and the corpus's inventory is frames x mappings with both factors
large. If a D or E copy FUSES with the B or C copy while both stay clear of A, there is a capacity limit and it is
measurable -- the model would have a bounded number of distinguishable ways to carry one mapping, which is a more
interesting result than four more circuits and is the reason to spend the batch here rather than on four more copies
of untouched pairs.
CPU capability margins BEFORE enqueue (mean base / mean donor; A1; dropped rows; floor 1.0):
  verb_particle_fe_up_down       (woke/calmed) A1:-4.87/+4.97 drop0
  verb_particle_fd_up_down       (woke/calmed) A1:-3.96/+3.97 drop0
  verb_preposition_fe_about_for  (cared/voted) A1:-2.63/+2.74 drop0
  verb_preposition_fd_about_for  (cared/voted) A1:-2.10/+2.15 drop0
  Four of four. woke/calmed has now cleared the floor in five different frames at 2.0 (A), 5.73 (B), 5.28 (C), 3.96 (D)
  and 4.87 (E) -- a 2.9x spread across frames for identical cue verbs, which is itself evidence that the frame is doing
  work rather than wrapping.
Protocol identical to v197/v201/.../v357. finiteness_selection is the instrument and is EXCLUDED from four-row counts.
Batch size FOUR; bars are the four-cell bars (2/3/3/3/3).

REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 2 of the 4 new behaviours pass all four rows. Frame copies of these two pairs are
                           4 of 4 so far at this stage.                                                    prior 75%
...
Accounting: the rung that follows will place each survivor in a family with EVERY prior copy of its own mapping pair
-- A, B and C -- so a capacity limit has every chance to show as a fusion between copies rather than against the
original. That is four or five prepared per pair, which the v347 measurement says is enough.
An error receipt per behaviour counts as a miss on every row. ~40 s GPU each, ~3 min.
Smoke: V361_SMOKE=<out.json> (CPU, V361_SMOKE_ROWS=4 per split, V361_SMOKE_NAMES=verb_particle_fd_up_down).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec9p_v361_result.json"
...
NAMES = {"verb_particle_fd_up_down": "verb_particle_fd_up_down", "verb_particle_fe_up_down": "verb_particle_fe_up_down",
         "verb_preposition_fd_about_for": "verb_preposition_fd_about_for", "verb_preposition_fe_about_for": "verb_preposition_fe_about_for",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v361", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V361_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V361_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V361_SMOKE_NAMES", "verb_particle_fd_up_down").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec9p_v361", "candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v361", "bars": BARS,
