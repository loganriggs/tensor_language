#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the third eight spec-authored behaviours, plus one instrument.
"""v207: the AMENDED tier-3 battery (v196/v197 protocol, unchanged) over the THIRD EIGHT spec-authored behaviours -- plus one instrument.

v204 (4/8) and v205 (6/8) put spec-authored behaviours through all four rows at ~35 GPU-s each; authoring stays the throughput path.
Eight more, written today through `circuit_fast_screen_behaviour_spec`, single-cue swap, matched final token, 32 A1/A2/P/C rows,
ABAB directions: reflexive_person (`Near the lantern I/you quickly blamed` -> myself/yourself), object_pronoun_person (`Because I/you had
asked, the pilot gave the lantern to` -> me/you; the you side is a copy, the I side a case mapping), person_possessive_plural (we/they ->
our/their), object_pronoun_number (`The lanterns/lantern belonged to the pilot who soon returned` -> them/it), causative_complement
(`made/allowed the sailor` -> carry/to: bare vs to-infinitive, new family), rather_prefer (`would rather/prefer not` -> carry/to),
negative_inversion (`Near the lantern, never/often before` -> had/the: fronted negative adverb -> subject-auxiliary inversion, new
family), degree_enough (`far too/barely ADJ` -> to/enough). Capability printed BEFORE the run on CPU (`g.prepare(valid_only=True)`,
32 rows per family): zero A1/A2 rows dropped on all eight; mean base/donor margins A1: reflexive_person -5.13/+5.11,
object_pronoun_person -1.61/+1.49 (weakest), person_possessive_plural -5.30/+5.19, object_pronoun_number -2.64/+2.26,
causative_complement -3.00/+2.76, rather_prefer -4.45/+4.42, negative_inversion -6.06/+6.06, degree_enough -5.18/+5.91; A2 weakest
object_pronoun_number -2.45/+2.45 (all A2 |means| >= 2.4).
Protocol identical to v197/v201/v204/v205 (greedy pool 40, target 0.88, min_gain 0.005, max 30; row 2 = MIN over the two held
...
REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 4 of the 8 new behaviours pass all four rows (v204 4/8, v205 6/8 with the same recipe;
                           object_pronoun_person and object_pronoun_number (smallest margins) are the weak cells).                   prior 55%
  pred_b_row2           -> MIN held-out per-direction exact-set recovery >= 0.80 on at least 6 of 8.              prior 70%
  pred_c_row3           -> dim A1 CE damage LB975 > 0 and >= 0.10 on at least 7 of 8.                             prior 80%
  pred_d_row4           -> constrained rank-1 own-C CE UB975 <= 0.01 on at least 5 of 8.                          prior 55%
...
Accounting: four-row passes here join the one-protocol standing; causative_complement and negative_inversion are new families
(count directly if four-row); the person/number readouts and rather_prefer / degree_enough wait for a separability pass against
their siblings (person_possessive, reflexive_number, object_pronoun_gender, causative_complement, degree_result).
...
Smoke: V207_SMOKE=<out.json> (CPU, V207_SMOKE_ROWS=4 per split, V207_SMOKE_NAMES=negative_inversion; pool 3/max 3, steps 5).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec8c_v207_result.json"
...
NAMES = {"reflexive_person": "reflexive_person", "object_pronoun_person": "object_pronoun_person",
         "person_possessive_plural": "person_possessive_plural", "object_pronoun_number": "object_pronoun_number",
         "causative_complement": "causative_complement", "rather_prefer": "rather_prefer",
         "negative_inversion": "negative_inversion", "degree_enough": "degree_enough", "finiteness_selection": "finiteness"}
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec8c_v207", "behaviours": 9, "constructions": 2,
...
def main() -> None:
    ...
    smoke = os.environ.get("V207_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V207_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V207_SMOKE_NAMES", "negative_inversion").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec8c_v207", "candidate_id": "corpus.unit_tier3_batch_amended_spec8c_v207", "bars": BARS,
