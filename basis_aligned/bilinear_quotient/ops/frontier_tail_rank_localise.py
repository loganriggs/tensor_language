"""LOCALISE THE RANK -- AND SUPPLY THE CONTROL SS2919 DID NOT HAVE (Claude, LANE 1)

SS2919 found that scaling ONLY the top 64 singular directions of each tail link map reads **-0.2614**, beating the adopted uniform
x0.25 at -0.2287, while the bottom of the spectrum is actively HARMFUL (bot64 +0.0489, bot256 +0.0114, bot576 +0.0005). That overturns
SS2917's isotropy INFERENCE and says the correction is a **low-rank projection, not a scalar**.

**SS2919 had a design gap and this rung exists first to close it.** Its uniform control went through `_apply_tail`, a plain multiply --
NOT through the SVD decompose-and-reconstruct path the split arms used. So nothing in that rung proved the SVD path is faithful, and a
lossy reconstruction could in principle manufacture the whole effect. `svd_identity` here scales ALL directions by 1.0 THROUGH THAT
PATH: it must read **0.0000**. `svd_uniform` scales all directions by 0.25 through the same path and must agree with the plain multiply.
**Until those two controls pass, SS2919's -0.2614 is not a result**, which is why nothing was adopted from it.

With the controls in place, this localises the rank: r in {16, 32, 64, 128, 256} x scale in {0.10, 0.25, 0.40}, with SS2919's top64
...
#                     pred_c_both_uniform_routes_reproduce_S2896
#                     pred_d_top64_reproduces_S2919
#                     pred_e_the_optimum_is_interior_in_rank_and_scale

Preregistration: polynomial_causal/FRONTIER_TAIL_RANK_LOCALISE_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run: a rank x scale sweep with the SVD-path identity control SS2919 lacked")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_tail_rank_localise_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_TAIL_RANK_LOCALISE_PREREGISTRATION.md'
PREREG_SHA = "0ce7965a5cb0d264495bb5aefbc25cdc7b556ebb6914861eb952561535decdc8"
...
R_GRID = [16, 32, 64, 128, 256]             # SS2919's top64 must be interior
SC_GRID = [0.10, 0.25, 0.40]
S2919_TOP64 = -0.2614                       # SS2919's best split, the cell this rung must reproduce
...
if __name__=='__main__':
    ...
    # The control SS2919 did not have: the SVD path at scale 1.0 over ALL directions must reconstruct the
    # fitted maps exactly and read 0.0000. Then a finer rank sweep x three scales around SS2919's top64.
    ARMS[:] = ([('baseline', {}),
                ('svd_identity', {'tail_spec': (0, 'all', 1.0)}),
                ('svd_uniform',  {'tail_spec': (0, 'all', 0.25)}),
                ('uniform',      {'tail_scale': 0.25})] +
               [(f'top{r}_s{sc}', {'tail_spec': (r, 'top', sc)}) for r in R_GRID for sc in SC_GRID])
    ...
    uni     = cost['uniform']
    svd_uni = cost['svd_uniform']
    cell = {(r,sc): cost[f'top{r}_s{sc}'] for r in R_GRID for sc in SC_GRID}
    best = min(cell, key=lambda k: cell[k])
    best_cost = cell[best]
    interior = (best[0] not in (R_GRID[0], R_GRID[-1])) and (best[1] not in (SC_GRID[0], SC_GRID[-1]))
    anchor = cost['top64_s0.25']

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = abs(ident) <= BARS['identity']
    pc = abs(uni - S2896_COST) <= BARS['anchor'] and abs(svd_uni - uni) <= BARS['anchor']
    pd = abs(anchor - S2919_TOP64) <= BARS['anchor']
    pe = bool(interior)
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           ...
           'pred_c_both_uniform_routes_reproduce_S2896':bool(pc),
           'pred_d_top64_reproduces_S2919':bool(pd),
           'pred_e_the_optimum_is_interior_in_rank_and_scale':bool(pe)}
    nulls={'b_null_the_svd_path_is_not_faithful':bool(abs(ident) > 0.02),
           'c_null_the_uniform_routes_disagree':bool(abs(svd_uni - uni) > 0.02),
           'd_null_S2919_does_not_reproduce':bool(abs(anchor - S2919_TOP64) >= 0.03),
           'e_null_the_grid_is_too_narrow':bool(not interior)}
    res={'rung':'frontier_tail_rank_localise','preds':preds,'nulls':nulls,'bars':BARS,
         ...
         'summary':{'L2_F_baseline':round(bF,4),'L2_F2_baseline':round(b2,4),
                    ...
                    'uniform_cost':uni,'svd_uniform_cost':svd_uni,
                    'route_disagreement':round(svd_uni-uni,4),
                    'S2896_adopted_cost':S2896_COST,'S2919_top64':S2919_TOP64,
                    'anchor_cell_cost':anchor,'anchor_deviation':round(abs(anchor-S2919_TOP64),4),
                    'best_cell':{'rank':best[0],'scale':best[1]},'best_cost':best_cost,
                    'best_beats_uniform_by':round(uni-best_cost,4),
                    'best_holdout_cost':cost_h[f'top{best[0]}_s{best[1]}'],
    ...
    print(f"(b) SVD path at scale 1.0 {ident:+.4f}, |.| <= 0.005: {'HELD' if pb else 'FAILED'}")
    print(f"(c) uniform {uni:+.4f} / via SVD {svd_uni:+.4f} vs SS2896 {S2896_COST:+.4f}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) top64 s0.25 {anchor:+.4f} vs SS2919 {S2919_TOP64:+.4f}: {'HELD' if pd else 'FAILED'}")
    for r in R_GRID:
        print(f"    r={r}: " + " | ".join(f"s{sc} {cell[(r,sc)]:+.4f}" for sc in SC_GRID))
    print(f"(e) optimum r={best[0]} s={best[1]} at {best_cost:+.4f} interior: {'HELD' if pe else 'FAILED'}")
    print(f"    beats uniform by {uni-best_cost:+.4f}; held out {cost_h[f'top{best[0]}_s{best[1]}']:+.4f}; implied L2_F {bF+best_cost:.4f}")
