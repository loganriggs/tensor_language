"""THE SHRINKAGE SWEEP, RE-RUN WITH AN INTERNAL ANCHOR (Claude, LANE 1)

SS2893 measured every scale below 1 improving the frontier, best **-0.2287 nats at s = 0.25** fresh and **-0.1530 in sample** -- which
would be the campaign's first real improvement to the published +2.6735. **It was not adopted**, because its registered anchor failed:
the s = 0 arm read -0.1863 against SS2881's +0.1740 for what the preregistration asserted was the same manipulation, a deviation of
0.3603 with opposite signs.

The cause is exact, and it is an error in my preregistration rather than a disagreement between measurements. The hook is
...
SS2881 set `LW = {}`, so the loop never runs and LINK-class positions **keep the class constant CV[c]**. SS2893's s = 0 set
...
INTERNALLY. pred_b checks the `{}` arm against SS2881; pred_c checks that the two zero-arms genuinely differ, confirming the diagnosis;
pred_e checks the scale grid against SS2893 so the earlier curve is reproduced rather than replaced. If all hold, the improvement rests
on a sound footing and can be adopted; SS2893's number stays unadopted regardless.
...
SS2125 STANDS. Both L2_F (fresh) and L2_C (the FITTING window) are reported per arm.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_the_empty_arm_reproduces_S2881
#                     pred_c_the_empty_and_zero_arms_differ
#                     pred_d_some_scale_below_one_improves
#                     pred_e_the_scale_grid_reproduces_S2893

Preregistration: polynomial_causal/FRONTIER_TAIL_SHRINKAGE_ANCHORED_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run, seven evaluations (baseline + LW empty + five scales)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_tail_shrinkage_anchored_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_TAIL_SHRINKAGE_ANCHORED_PREREGISTRATION.md'
PREREG_SHA = "258026c23779a51fa530e493cf58c4cbb240ad62f63a3e4a39dcf42ca4c19892"
...
S2893 = PT + 'frontier_tail_link_shrinkage_results.json'
S2881_SHA = "cd1a8028a8086706ba82a907436ae1acb74113d70c357d01bebaba21c81ecf95"
S2893_SHA = "4a24b6c46945e95d7547d930e7ae5aa9fe451cdbfa2f25ea8267afa89e88cbe7"
SCALES = [0.0, 0.25, 0.5, 0.75, 1.0]
BARS = {"reproduce": 0.05, "anchor": 0.02, "arms_differ": 0.10, "improves": 0.0, "repro_prior": 0.02}
NULLS = {"anchor_ge": 0.05, "arms_same_le": 0.02, "improves_none": 0}
...
if __name__=='__main__':
    ...
    for p_, h_ in ((S2881, S2881_SHA), (S2893, S2893_SHA)):
        if _sha(p_) != h_:
            raise RuntimeError(f'frozen hash mismatch: {p_}')
    a81 = json.load(open(S2881))['summary']['cost_dropping_link_maps_nats']   # +0.1740 for LW := {}
    prev93 = json.load(open(S2893))['summary']['cost_fresh_by_scale']
    SEL['mode']='norm'

    key = lambda s: f's{int(round(s*100)):03d}'
    ARMS[:] = ([('baseline', {}), ('lw_empty', {'empty': True})]
               + [(key(s), {'scale': s}) for s in SCALES])
    ...
    empty_cost = cF['lw_empty']
    zero_cost  = cF[key(0.0)]
    anchor_dev = abs(empty_cost - a81)
    arms_gap   = abs(empty_cost - zero_cost)
    below = [s for s in SCALES if s < 1.0 and cF[key(s)] < 0]
    best_s = min(SCALES, key=lambda s: cF[key(s)])
    repro_dev = max(abs(cF[key(s)] - prev93[str(s)]) for s in SCALES if str(s) in prev93)

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = anchor_dev <= BARS['anchor']
    pc = arms_gap >= BARS['arms_differ']
    pd = len(below) >= 1
    pe = repro_dev <= BARS['repro_prior']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_empty_arm_reproduces_S2881':bool(pb),
           'pred_c_the_empty_and_zero_arms_differ':bool(pc),
           'pred_d_some_scale_below_one_improves':bool(pd),
           'pred_e_the_scale_grid_reproduces_S2893':bool(pe)}
    nulls={'b_null_the_anchor_still_fails':bool(anchor_dev>=NULLS['anchor_ge']),
           'c_null_the_two_zero_arms_are_the_same':bool(arms_gap<=NULLS['arms_same_le']),
           'd_null_no_scale_improves':bool(len(below)==NULLS['improves_none'])}
    res={'rung':'frontier_tail_shrinkage_anchored','preds':preds,'nulls':nulls,
         ...
         'summary':{'L2_F_baseline':round(bF,4),'L2_C_baseline':round(bC,4),
                    ...
                    'cost_lw_empty':empty_cost,'cost_scale_zero':zero_cost,
                    'S2881_anchor':round(a81,4),'anchor_deviation':round(anchor_dev,4),
                    'gap_between_the_two_zero_arms':round(arms_gap,4),
                    'scales_below_one_that_improve':below,
                    'best_scale':best_s,'best_cost_fresh':cF[key(best_s)],
                    'best_cost_fit':cC[key(best_s)],
                    'S2893_scale_grid':prev93,'max_deviation_vs_S2893':round(repro_dev,4),
         ...
         'hashes':{PREREG:PREREG_SHA,S2881:S2881_SHA,S2893:S2893_SHA},'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {bF:.4f} vs 2.6735 (tol .05): {'HELD' if pa else 'FAILED'}")
    print(f"(b) LW:={{}} arm {empty_cost:+.4f} vs SS2881 {a81:+.4f}, dev {anchor_dev:.4f} <= {BARS['anchor']}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) LW:={{}} vs LW[k]:=0 gap {arms_gap:.4f} >= {BARS['arms_differ']}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) scales below 1 improving: {below}, best s={best_s} at {cF[key(best_s)]:+.4f} fresh / {cC[key(best_s)]:+.4f} fit: {'HELD' if pd else 'FAILED'}")
    print(f"(e) max deviation vs SS2893 grid {repro_dev:.4f}: {'HELD' if pe else 'FAILED'}")
