"""QUANTIFY THE REFIT COMPENSATION IN THE FRONT MLP STAGE (Claude, LANE 1)

SS2895 shrank the front MLP tables and found improvements on the fresh window (best **-0.1648 at a_scale .5**) -- and its `tb := 0`
anchor missed by **4.9484 nats**: **+5.6298** frozen against SS2877's **+0.6814** refit-time. The `A := 0` anchor held at .0215.

That asymmetry is the interesting part. SS2877 zeroed `tb` DURING FITTING, so the quadratic residual `A` was then fitted against a
residual computed with no token table and **absorbed the lookup's job**. Zeroing `tb` on a frozen stack gives `A` no chance to
compensate. The gap between the two is the size of that compensation -- and at ~4.95 nats it would be an order of magnitude larger than
any interaction this ledger has measured (SS2880 +3.2104, SS2888 +0.3023, SS2894 +0.0610).

Three runs settle it against ONE baseline:

    RUN 1  frozen stack : baseline, tb := 0, A := 0, a_scale = .5
    RUN 2  refit-time   : tb := 0 applied INSIDE the fitters -- exactly SS2877's operation
    RUN 3  refit-time   : A := 0 applied INSIDE the fitters -- exactly SS2877's operation

pred_d and pred_e make the asymmetry itself a prediction: the token table's compensation should be LARGE and the residual's SMALL,
which is what SS2895's two anchors (one failing by 4.95, one holding at .02) already imply.
...
#                     pred_b_the_refit_time_tb_arm_reproduces_S2877
#                     pred_c_the_refit_time_A_arm_reproduces_S2877
#                     pred_d_the_token_table_compensation_is_large
#                     pred_e_the_residual_compensation_is_small

Preregistration: polynomial_causal/FRONTIER_FRONT_ANCHOR_RESOLUTION_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; THREE pipeline runs: frozen stack plus the two refit-time arms")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_front_anchor_resolution_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
TAILMODE={'mode':None}
MLPMODE={'drop':None}   # refit-time: None | 'table' (tb:=0) | 'res' (A:=0), applied INSIDE the fitters
...
PREREG = PT + '../polynomial_causal/FRONTIER_FRONT_ANCHOR_RESOLUTION_PREREGISTRATION.md'
PREREG_SHA = "d3dad199091d07199b4b1e9cabadb2b9ac194f00f6c7e50f41d8e5375c8a37f0"
...
BARS = {"reproduce": 0.05, "anchor": 0.05, "tb_compensation": 3.0, "a_compensation": 0.10}
NULLS = {"anchor_ge": 0.10, "tb_compensation_le": 1.0}
...
if __name__=='__main__':
    ...
    for p_, h_ in ((S2877, S2877_SHA), (S2895, S2895_SHA)):
        if _sha(p_) != h_:
            raise RuntimeError(f'frozen hash mismatch: {p_}')
    p77 = json.load(open(S2877))['summary']
    p95 = json.load(open(S2895))['summary']
    refit_tb_81 = p77['cost_dropping_table_nats']       # +0.6814 refit-time tb:=0
    refit_A_81  = p77['cost_dropping_residual_nats']    # +0.7536 refit-time A:=0
    froz_tb_95  = p95['cost_fresh']['T000']             # +5.6298 frozen tb:=0
    froz_A_95   = p95['cost_fresh']['A000']             # +0.7321 frozen A:=0
    SEL['mode']='norm'

    print('RUN 1/3: frozen stack -- baseline / tb:=0 / A:=0 / a_scale=.5',flush=True)
    MLPMODE['drop']=None
    ARMS[:] = [('baseline', {}), ('froz_tb0', {'tb_scale': 0.0}),
               ('froz_A0', {'a_scale': 0.0}), ('a050', {'a_scale': 0.5})]
    out=main()
    bF = out['baseline']['L2_F']
    c_froz_tb = round(out['froz_tb0']['L2_F'] - bF, 4)
    c_froz_A  = round(out['froz_A0']['L2_F'] - bF, 4)
    c_a050    = round(out['a050']['L2_F'] - bF, 4)

    print('RUN 2/3: refit-time tb:=0 (SS2877 operation)',flush=True)
    ARMS[:] = []; MLPMODE['drop']='table'
    o2=main(); r2 = o2['single'] if 'single' in o2 else o2
    c_refit_tb = round(r2['L2_F'] - bF, 4)

    print('RUN 3/3: refit-time A:=0 (SS2877 operation)',flush=True)
    MLPMODE['drop']='res'
    o3=main(); r3 = o3['single'] if 'single' in o3 else o3
    c_refit_A = round(r3['L2_F'] - bF, 4)
    MLPMODE['drop']=None

    dev_tb = abs(c_refit_tb - refit_tb_81)
    dev_A  = abs(c_refit_A - refit_A_81)
    comp_tb = abs(c_froz_tb - c_refit_tb)     # how much the refit absorbs for the token table
    comp_A  = abs(c_froz_A  - c_refit_A)      # ... and for the quadratic residual

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = dev_tb <= BARS['anchor']
    pc = dev_A  <= BARS['anchor']
    pd = comp_tb >= BARS['tb_compensation']
    pe = comp_A  <= BARS['a_compensation']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_refit_time_tb_arm_reproduces_S2877':bool(pb),
           'pred_c_the_refit_time_A_arm_reproduces_S2877':bool(pc),
           'pred_d_the_token_table_compensation_is_large':bool(pd),
           'pred_e_the_residual_compensation_is_small':bool(pe)}
    nulls={'b_null_the_tb_anchor_fails_at_refit_time':bool(dev_tb>=NULLS['anchor_ge']),
           'c_null_the_A_anchor_fails_at_refit_time':bool(dev_A>=NULLS['anchor_ge']),
           'd_null_the_compensation_is_not_the_cause':bool(comp_tb<=NULLS['tb_compensation_le'])}
    res={'rung':'frontier_front_anchor_resolution','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,'frozen_arms':out,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(bF,4),
                    'cost_refit_time_tb0':c_refit_tb,'S2877_refit_tb0':round(refit_tb_81,4),
                    'tb_anchor_deviation':round(dev_tb,4),
                    'cost_refit_time_A0':c_refit_A,'S2877_refit_A0':round(refit_A_81,4),
                    'A_anchor_deviation':round(dev_A,4),
                    'cost_frozen_tb0':c_froz_tb,'S2895_frozen_tb0':round(froz_tb_95,4),
                    'cost_frozen_A0':c_froz_A,'S2895_frozen_A0':round(froz_A_95,4),
                    'token_table_refit_compensation_nats':round(comp_tb,4),
                    'residual_refit_compensation_nats':round(comp_A,4),
                    'cost_a_scale_050':c_a050,'S2895_a_scale_050':round(p95['cost_fresh']['A050'],4),
         ...
         'hashes':{PREREG:PREREG_SHA,S2877:S2877_SHA,S2895:S2895_SHA},'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {bF:.4f} vs 2.6735: {'HELD' if pa else 'FAILED'}")
    print(f"(b) refit tb:=0 {c_refit_tb:+.4f} vs SS2877 {refit_tb_81:+.4f}, dev {dev_tb:.4f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) refit A:=0 {c_refit_A:+.4f} vs SS2877 {refit_A_81:+.4f}, dev {dev_A:.4f}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) token-table refit compensation {comp_tb:.4f} >= {BARS['tb_compensation']}: {'HELD' if pd else 'FAILED'}")
    print(f"(e) residual refit compensation {comp_A:.4f} <= {BARS['a_compensation']}: {'HELD' if pe else 'FAILED'}")
    print(f"ADOPTION GATE (a,b,c): {'OPEN' if (pa and pb and pc) else 'CLOSED'}")
