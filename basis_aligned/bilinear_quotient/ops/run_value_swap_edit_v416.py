...
"""The value-copy account edited (v416). v401-v410 (folds): at the readers 9.6 / 12.4 / 15.1 number reaches the answer as a VALUE copy of the noun's state;
the patterns are number-blind. The counterfactual that matches the claim is a replace-edit, not a zero: on each aligned pair (plural noun / singular noun),
swap ONLY the three readers' value vectors at the noun position between the two rows (patterns untouched) -> the they - he margin should move toward the
partner's; swap ONLY their keys (k and k2) at the noun position (values untouched) -> little should happen. Restricted to the noun source of the three
heads; everything downstream recomputed natively. On the 122 natural swapped pairs (v405) and the v76 panel pairs.
PREDICTIONS (scored as written; failures preserved; priors from v380 / v401 / v405)
    ...
    pred_b_value_swap_moves_margin            the value swap closes >= 0.25 of the pair's margin gap (plural margin - singular margin), pooled over text pairs. Prior: the readers write ~0.7 of the signal (v380), the noun source ~half of that
    ...
    pred_e_effect_holds_on_panel              the value swap closes >= 0.25 of the gap on the panel pairs too
...
OUT = ROOT / "circuits/followups/value_swap_edit_v416_result.json"
CANDIDATE_ID = "both_ends.value_swap_edit_v416"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1))
REPLAY_TOL, VALUE_MIN, KEY_MAX, MOST_MIN = 1e-3, 0.25, 0.05, 0.80
FORWARDS_MAX = 22
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_value_swap_moves_margin": ">= 0.25", "pred_c_key_swap_moves_little": "<= 0.05", "pred_d_value_swap_beats_key_swap_on_most_pairs": ">= 0.80", "pred_e_effect_holds_on_panel": ">= 0.25"}
...
def main() -> None:
    ...
    OUT.write_text(json.dumps({"schema": "value_swap_edit_result_v416", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
