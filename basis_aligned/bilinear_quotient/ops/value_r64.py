"""THE OV CHAPTER OPENS (rung 255): value maps at rank 64 on the mixed-spectrum base.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. Patterns are settled (mixed-spectrum
point, S2352); the VALUE side (c_v) has never been touched - every replaced head still uses its full
128x1152 value map. Here each replaced head's value map is SVD-truncated to rank 64 (weights-only), the
same octave that cost the tail patterns +0.025. Values are ordinarily far more truncation-tolerant than
the squared score maps - the first cross-side comparison.
REGISTERED PREDICTIONS (arms: THIS vs the mixed base 0.0573/11):
  (a) VALUES TOLERATE HALF-RANK: census <= 0.075 (surcharge <= 0.018).
  (b) CERTIFICATES MOSTLY KEPT: valid >= 9.
  (c) SANITY: census in [0.01, 0.15]; inert tripwire vs the mixed receipt.
NULL: values are as rank-hungry as patterns (census >= 0.10). PRICE: mixed patterns 52.9M + value factors
~9.0M (110 replaced heads x 64 x 1280). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: value rank-64 on mixed base')
    raise SystemExit(0)
import torch
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=96; SEL['qk_rmap']={}; SEL['v_r']=64; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    print(f"(a) census {agg:+.4f} <= 0.075: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 9; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
