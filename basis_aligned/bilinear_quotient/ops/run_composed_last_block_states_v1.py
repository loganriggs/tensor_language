...
Cache continuation of COMPOSED_LAST_BLOCK_PREDICTOR_V1; same validated native predictions, plus saved final states and matching linear parts. No compressed readers installed.
...
STEM='COMPOSED_LAST_BLOCK_STATES_V1'
@torch.no_grad()
def main():
 ...
 result={'pred_a':max(replay)<=1e-4,'pred_b':all(c['full_effect_error']<=.1 for c in cells),'pred_c':all(c['compact_preservation_error']<=.1 for c in cells),'anchor_replay':replay,'cells':cells,'body_forwards':count,'extra_final_readouts':480,'extra_mlp_evaluations':360,'seconds':time.perf_counter()-tic,'executed_arms':[0,1,3],'scope':'Read-only cache of the existing three-trajectory composed predictor: states and corresponding linear parts.  FullMLPweights andzN/C/R supplied; no Atrajectory orfreshOOD; nottotalchild/remainderinteraction.'}
