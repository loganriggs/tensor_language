#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_5_3_leads_block_5_on_the_panel pred_c_other_block_5_heads_carry_the_rest pred_d_11_3_leads_block_11_on_the_panel pred_e_singles_add_to_blocks
"""Blocks 5 and 11 into the PANEL's pronoun slot, by head (v536). v535: on the panel block 5 writes 0.097 and block 11 0.044 into the answer; v534: 5.3's own
write is 0.040, so other block-5 heads carry the rest of the relayed number. Each head's write into the panel answer swapped singly for blocks 5 and 11, 48 pairs.
PREDICTIONS (scored as written; failures preserved; priors from v534 / v535)
    pred_a_baseline_replays                  the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_5_3_leads_block_5_on_the_panel    head 5.3 has the largest single-head effect in block 5
    pred_c_other_block_5_heads_carry_the_rest  the other eight heads of block 5 together close >= 0.04 (the relay beyond 5.3)
    pred_d_11_3_leads_block_11_on_the_panel  head 11.3 has the largest single-head effect in block 11 (the verb reader leaking into the pronoun slot). Prior: unsure
    pred_e_singles_add_to_blocks             the nine single-head effects sum to within 0.03 of the block's effect, for both blocks
PRICE (registered maximum): 3 panel batches x 19 passes = 57 forwards; 0 backwards; 0 fits. Bar <= 58.
"""
...
OUT = ROOT / "circuits/followups/attn_into_pronoun_heads_panel_5_11_v536_result.json"
CANDIDATE_ID = "chain.attn_into_pronoun_heads_panel_5_11_v536"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, OTHERS_MIN, ADD_TOL, B5, B11, UNIT = 1e-3, 0.04, 0.03, 0.097, 0.044, 701
import random
...
FORWARDS_MAX = 58
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_5_3_leads_block_5_on_the_panel": "5.3 first", "pred_c_other_block_5_heads_carry_the_rest": ">= 0.04", "pred_d_11_3_leads_block_11_on_the_panel": "11.3 first", "pred_e_singles_add_to_blocks": "within 0.03 x 2"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "others_min": OTHERS_MIN, "add_tol": ADD_TOL, "b5_v535": B5, "b11_v535": B11, "unit": UNIT}}
    ...
    MODES = {f"{b}.{h}": ({}, (0,)) for b in (5, 11) for h in range(N_HEAD)}; forwards = 0
    def logits_margin(x):
    ...
    mc = verb["margin_closed"]; b5 = {m: mc[m] for m in mc if m.startswith("5.")}; b9 = {m: mc[m] for m in mc if m.startswith("11.")}; report["sums"] = {"5": sum(b5.values()), "11": sum(b9.values())}; report["block5_others"] = sum(v for k, v in b5.items() if k != "5.3")
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_5_3_leads_block_5_on_the_panel": max(b5, key=b5.get) == "5.3", "pred_c_other_block_5_heads_carry_the_rest": report["block5_others"] >= OTHERS_MIN,
                   "pred_d_11_3_leads_block_11_on_the_panel": max(b9, key=b9.get) == "11.3", "pred_e_singles_add_to_blocks": abs(sum(b5.values()) - B5) <= ADD_TOL and abs(sum(b9.values()) - B11) <= ADD_TOL}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "attn_into_pronoun_heads_panel_5_11_result_v536", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
