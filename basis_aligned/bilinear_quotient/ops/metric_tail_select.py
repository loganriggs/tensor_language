"""SELECT vs CONSTRUCT (rung 27): at the tail, SELECT 8 of the plain PCA's top-32 directions by metric weight.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['stream_error_profile_results.json','metric_tail_spans_results.json']
    ...
    print(f"DRYRUN OK: S2086 present (block 6 {_p['6']}); three arms (plain / select-8-of-32 / random-8-of-32) x 9 windows")
    raise SystemExit(0)
import json, sys, time, torch
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
import os
...
OUT=PT+'metric_tail_select_results.json'
...
def main():
    ...
    genS=torch.Generator(device='cpu').manual_seed(27)
    for mode in ('select-8-of-32','random-8-of-32'):
        SPANS[mode]={}
        for li in TAILC:
            accs=[]
            for i in range(0,120,6):
            ...
            V32=Vh[:32].T.contiguous()                                   # D x 32, orthonormal
            half,_=MET[('metric',f't{li}')]
            w=((half@V32)**2).sum(0)                                     # u^T G u per direction
            if mode=='select-8-of-32': pick=w.argsort(descending=True)[:8]
            else: pick=torch.randperm(32,generator=genS)[:8].to(DEV)
    ...
    def build_arm(mode):
        ...
        tag={'plain':'E','select-8-of-32':'S','random-8-of-32':'X'}[mode]
        act=['a0']
    ...
    res={}
    for mode in ('plain','select-8-of-32','random-8-of-32'):
    ...
    E=res['plain']
    g1=lambda k:E['gap_w1']-res[k]['gap_w1']
    gains8=[a-b for a,b in zip(E['gaps_fresh'],res['select-8-of-32']['gaps_fresh'])]
    pa=res['select-8-of-32']['gap_w1']<=E['gap_w1']+0.01; pb=stt.median(gains8)>=0.02
    pc=res['random-8-of-32']['gap_w1']>=E['gap_w1']+0.02
    pd=abs(E['profile'][6]-1.7415)<=0.10
    out={'base_ce_w1':round(base1,5),'arms':res,'gain_w1':{k:round(g1(k),4) for k in res if k!='plain'},
         'fresh_gains_select':[round(g,4) for g in gains8],'fresh_gain_median':round(stt.median(gains8),4),
         'pred_a_no_harm':bool(pa),'pred_b_fresh_gain':bool(pb),'pred_c_random_subset_hurts':bool(pc),
    ...
    print(f"(a) select-8 gap w1 {res['select-8-of-32']['gap_w1']:+.4f} <= plain {E['gap_w1']:+.4f} + 0.01: {'HELD' if pa else 'FAILED'}")
    print(f"(b) fresh median gain {stt.median(gains8):+.4f} >= 0.02: {'HELD' if pb else 'FAILED'}")
    print(f"(c) random-8 gap w1 {res['random-8-of-32']['gap_w1']:+.4f} >= plain + 0.02: {'HELD' if pc else 'FAILED'}")
