...
@torch.no_grad()
def main():
 files=json.loads((P/'SHARED_TAIL_LEXICAL_V1_BINDING.json').read_text())['files']
 ...
 out=P/'SHARED_TAIL_LEXICAL_V1_RESULT.json';assert not out.exists()
 ...
 result={'pred_a':A,'pred_b':all(g['compressed_relative_errors'][0]<=.02 and g['compressed_relative_errors'][1]<=.05 for g in groups),'pred_c':all(g['incremental_relative_errors'][0]<=.01 and g['incremental_relative_errors'][1]<=.025 for g in groups),'pred_d':all(g['capability']>0 for g in groups),'anchors':anchors,'groups':groups,'cells':cells,'seconds':time.perf_counter()-tic,'scope':'48 new lexical-transfer rows, reused instruction styles, some related roots; shared tail cache across contexts; six existing lexical contrasts, live assembled upstream scalar fields; native prefix/background/suffix. Three-term vs exact five-bank and native original interaction. Not independent text-only circuit or corpus OOD.'}
