"""BRACKET THE MOTIF OPTIMUM, AND COMPOSE ALL THREE ADOPTED SCALINGS (Claude, LANE 1)

Three rescalings of the SS312 construction are now adopted, and one open limit blocks the third from being quoted as an optimum:

    TAIL   SS2896   LW    x 0.25    -0.2287
    CP     SS2902   Dk    x 0.50    -0.1075
    T+C    SS2904   both            -0.3213 fresh / -0.2899 in sample -> frontier +2.3522, interaction only +0.0149
    MOTIF  SS2905   ALPHA x 1.25    -0.1605 fresh / -0.1640 in sample -- but 1.25 was the TOP OF THE GRID, so the
                                    optimum is NOT bracketed and SS2905 recorded the number as a lower bound.

SS2905 also refuted my stated direction: every other block wants LESS, the motif heads want MORE, so the local/end-to-end mismatch is
**directional per block**. That makes the size of the motif correction genuinely unknown above 1.25.

This rung closes both questions in ONE pipeline run. A motif-only grid {1.25, 1.5, 2.0, 2.5, 3.0} brackets the optimum -- or shows it
is still beyond, which pred_c is written to detect -- and T+C+motif arms at {1.25, 1.5, 2.0} ask whether the third correction adds to
the adopted pair. SS2904's T+C and SS2905's motif-1.25 are both re-anchored, so the new arms are measured against numbers already on the
ledger.
...
#                     pred_c_the_motif_optimum_is_bracketed
#                     pred_d_adding_the_motif_scaling_beats_TC
#                     pred_e_the_motif_125_arm_reproduces_S2905

Preregistration: polynomial_causal/FRONTIER_MOTIF_BRACKET_AND_TRIPLE_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run: motif grid to bracket the optimum plus three-block combinations")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_motif_bracket_and_triple_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_MOTIF_BRACKET_AND_TRIPLE_PREREGISTRATION.md'
PREREG_SHA = "4abe22f8be2e835f7f15e56fa9ee8eceace6edf11084992e800b60842fada9c8"
...
S2905 = PT + 'frontier_motif_gain_scale_results.json'
S2904_SHA = "2ad1fc031037be32fd97431eef218d9dc22c382835b78e5409c967439d2850c5"
S2905_SHA = "f32ff524c39000b16dd692b5c8f5c3708a0d4feab731151425ca594621b6bc6a"
M_GRID = [1.25, 1.5, 2.0, 2.5, 3.0]
M_COMBO = [1.25, 1.5, 2.0]
T_S, C_S = 0.25, 0.5
BARS = {"reproduce": 0.05, "anchor": 0.01, "combo_beats": 0.05}
NULLS = {"anchor_ge": 0.03, "combo_le": 0.0}
...
if __name__=='__main__':
    ...
    for p_, h_ in ((S2904, S2904_SHA), (S2905, S2905_SHA)):
        if _sha(p_) != h_:
            raise RuntimeError(f'frozen hash mismatch: {p_}')
    a04 = json.load(open(S2904))['summary']['cost_fresh']['TC']                  # -0.3213
    a05 = json.load(open(S2905))['summary']['cost_fresh_by_scale']['1.25']       # -0.1605
    SEL['mode']='norm'

    mk = lambda m: f'M{int(round(m*100)):03d}'
    ck = lambda m: f'TCM{int(round(m*100)):03d}'
    ARMS[:] = ([('baseline', {}), ('TC', {'tail_scale': T_S, 'cp_scale': C_S})]
               + [(mk(m), {'motif_scale': m}) for m in M_GRID]
               + [(ck(m), {'tail_scale': T_S, 'cp_scale': C_S, 'motif_scale': m}) for m in M_COMBO])
    ...
    c_tc = round(out['TC']['L2_F'] - bF, 4)
    cM = {m: round(out[mk(m)]['L2_F'] - bF, 4) for m in M_GRID}
    cM_fit = {m: round(out[mk(m)]['L2_C'] - bC, 4) for m in M_GRID}
    cTCM = {m: round(out[ck(m)]['L2_F'] - bF, 4) for m in M_COMBO}
    cTCM_fit = {m: round(out[ck(m)]['L2_C'] - bC, 4) for m in M_COMBO}
    best_m = min(M_GRID, key=lambda m: cM[m])
    best_combo_m = min(M_COMBO, key=lambda m: cTCM[m])
    best_combo = cTCM[best_combo_m]
    beats = round(c_tc - best_combo, 4)              # POSITIVE = adding motif helps
    dev_tc = abs(c_tc - a04)
    dev_m = abs(cM[1.25] - a05)

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = dev_tc <= BARS['anchor']
    pc = best_m != M_GRID[-1]
    pd = beats >= BARS['combo_beats']
    pe = dev_m <= BARS['anchor']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_TC_arm_reproduces_S2904':bool(pb),
           'pred_c_the_motif_optimum_is_bracketed':bool(pc),
           'pred_d_adding_the_motif_scaling_beats_TC':bool(pd),
           'pred_e_the_motif_125_arm_reproduces_S2905':bool(pe)}
    nulls={'b_null_the_TC_anchor_fails':bool(dev_tc>=NULLS['anchor_ge']),
           'c_null_the_optimum_is_still_beyond_the_grid':bool(best_m==M_GRID[-1]),
           'd_null_the_motif_scaling_adds_nothing':bool(beats<=NULLS['combo_le'])}
    res={'rung':'frontier_motif_bracket_and_triple','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,'motif_grid':M_GRID,'combo_motif':M_COMBO,
         'arms':out,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(bF,4),'L2_C_baseline':round(bC,4),
                    'motif_only_fresh':{str(m):cM[m] for m in M_GRID},
                    'motif_only_fit':{str(m):cM_fit[m] for m in M_GRID},
                    'best_motif_scale':best_m,'best_motif_cost':cM[best_m],
                    'cost_TC':c_tc,'S2904_TC':round(a04,4),'TC_deviation':round(dev_tc,4),
                    'S2905_motif125':round(a05,4),'motif125_deviation':round(dev_m,4),
                    'triple_fresh':{str(m):cTCM[m] for m in M_COMBO},
                    'triple_fit':{str(m):cTCM_fit[m] for m in M_COMBO},
                    'best_triple_motif_scale':best_combo_m,'best_triple_cost':best_combo,
                    'triple_beats_TC_by':beats,
                    'implied_L2_F_best_triple':round(bF + best_combo, 4),
         ...
         'hashes':{PREREG:PREREG_SHA,S2904:S2904_SHA,S2905:S2905_SHA},'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {bF:.4f} vs 2.6735: {'HELD' if pa else 'FAILED'}")
    print("    motif only: " + "  ".join(f"m={m}:{cM[m]:+.4f}" for m in M_GRID))
    print("    T+C+motif : " + "  ".join(f"m={m}:{cTCM[m]:+.4f}" for m in M_COMBO))
    print(f"(b) TC {c_tc:+.4f} vs SS2904 {a04:+.4f}, dev {dev_tc:.4f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) best motif scale {best_m} (interior?): {'HELD' if pc else 'FAILED'}")
    print(f"(d) best triple {best_combo:+.4f} at m={best_combo_m} beats TC by {beats:+.4f}: {'HELD' if pd else 'FAILED'}")
    print(f"    implied L2_F = {bF+best_combo:.4f}")
