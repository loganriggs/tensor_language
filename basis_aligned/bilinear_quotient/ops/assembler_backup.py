...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['assembler_pair_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; b2/b3 zeroing interaction on motifs-2-3-real base (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'assembler_backup_results.json'
...
if __name__=='__main__':
    ...
    SEL['clsdmg']=True; SEL['zero_m16']=False; SEL['zeroattn']=(); SEL['motif_off']=()
    print('ARM 0/5: full S2144 config (reproduction)',flush=True)
    r_full=main()
    SEL['motif_off']=(2,3)
    arms={}
    for tag,za in (('base',()),('z2',(2,)),('z3',(3,)),('both',(2,3))):
        SEL['zeroattn']=za
        print(f'ARM {tag}: motifs 2/3 real, attn zeroed at {za}',flush=True)
        arms[tag]={'r':main(),'cev':SEL['cev']}
    clsflat=SEL['clsflat']
    import statistics as stt
    for tag in ('z2','z3','both'):
        if float((arms[tag]['cev']-arms['base']['cev']).abs().sum())<1e-3:
            raise SystemExit(f'INSTRUMENT FAIL: {tag} identical to base')
    d2f=arms['z2']['r']['L2_F']-arms['base']['r']['L2_F']
    d3f=arms['z3']['r']['L2_F']-arms['base']['r']['L2_F']
    djf=arms['both']['r']['L2_F']-arms['base']['r']['L2_F']
    dz2=(arms['z2']['cev']-arms['base']['cev'])
    dz3=(arms['z3']['cev']-arms['base']['cev'])
    SW=CLS.index('subword')
    swm=(clsflat==SW).nonzero().squeeze(1)
    order=dz2[swm].argsort(); swo=swm[order]
    B=200; x=[];y=[]
    for k0 in range(0,len(swo)-B+1,B):
        ii=swo[k0:k0+B]
        x.append(float(dz2[ii].mean())); y.append(float(dz3[ii].mean()))
    def rank(v):
    ...
    res={'full':{'L2_F':r_full['L2_F']},
         'arms':{t:{'L2_F':arms[t]['r']['L2_F']} for t in arms},
         'fr_damage':{'z2':round(d2f,4),'z3':round(d3f,4),'both':round(djf,4),'sum':round(d2f+d3f,4)},
         'spearman_binned_subword':round(rho,3),
         'convention':'damage on the motifs-2/3-real base; bases excluded; lower L2 better',
         'pred_a_superadditive':bool(pa),'pred_b_same_positions':bool(pb),'pred_c_reproduces_s2144':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"FR damage: z2 {d2f:+.4f} z3 {d3f:+.4f} both {djf:+.4f} (sum {d2f+d3f:+.4f})")
    print(f"(a) both {djf:+.4f} >= 1.15 x sum {1.15*(d2f+d3f):+.4f}: {'HELD' if pa else 'FAILED'}")
    print(f"(b) subword rho {rho:+.3f} >= 0.5: {'HELD' if pb else 'FAILED'}")
