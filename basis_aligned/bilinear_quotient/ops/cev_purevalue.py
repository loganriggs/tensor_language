"""CLEAN BAND-FREE CONTROL (rung 279): value r96 on EXACT patterns - the pure value-damage direction.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2378: the v96 control contained the
band deficiency. Here the hard-coded mixed selection is widened to ALL 128 directions (patterns exact),
leaving VALUE truncation as the only damage source; the saved cev gives the pure value direction.
REGISTERED PREDICTIONS (offline cosine bars preregistered in S2378):
  (a) PURE-VALUE DAMAGE SMALL: census in [0.005, 0.06] - doubles as the live config check (the band's
      ~0.055 must be ABSENT; ~0.076 would mean the edit was inert).
  (b) CERTIFICATES RECOVER: valid >= 10 (band cap lifted; only value damage remains).
  (c) placeholder (offline: cosine(d_pure_value, d_ct96) < 0.3; norm within 25% of 109.7).
NULL: value damage shares the band direction (offline cosine >= 0.5), or the edit is inert.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: pure-value control (exact patterns)')
    raise SystemExit(0)
import torch
...
OUT=PT+'cev_purevalue_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=96; SEL['qk_rmap']={}; SEL['v_r']=96; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    cev=SEL['cev']
    torch.save(cev,PT+'cev_purevalue.pt')
    print('cev saved: cev_purevalue.pt',flush=True)
    ...
    print(f"(a) PURE-VALUE census {agg:+.4f} in [0.005, 0.06]: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 10 (band cap lifted): {'HELD' if pb else 'FAILED'}")
