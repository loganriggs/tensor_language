...
PREREG=P/"SETTING2_REGIONAL_QK1_EDIT_DOWNSTREAM_RESPONSE_CENSUS_V1_PREREGISTRATION.md"
ROWS=P/"ODD_FRAMING_FRESH_V1_ROWS.json"
BINDING=P/"SETTING2_REGIONAL_QK1_EDIT_DOWNSTREAM_RESPONSE_CENSUS_V1_BINDING.json"
...
OUT=ROOT/"basis_aligned/bilinear_quotient/circuits/fast_screens/setting2_regional_qk1_edit_downstream_response_census_v1_result.json"
...
def load_bound():
    binding=json.loads(BINDING.read_text()); files={"preregistration":PREREG,"rows":ROWS,"row_check":P/"regional_cue_row_check_v1.py","parent_result":PARENT,"intervention_helper":HELPER}
...
def plan():
    bound,rows=load_bound(); return {"schema":"setting2_regional_qk1_edit_downstream_response_census_v1_plan","model_loaded":False,"gpu_accessed":False,"queue_touched":False,"rows":len(rows),"terms":NAMES,"price":PRICE,"row_checks":bound["row_checks"],"binding_sha256":sha(BINDING)}
...
def main():
    ...
    ranking=sorted(reports,key=lambda n:reports[n]["change_norm_ratio"],reverse=True); idx={n:i for i,n in enumerate(NAMES)}; top5_error=rel(sum(dt[idx[n]] for n in ranking[:5]),delta); direct_error=rel(dt[idx["attn9"]],delta); raw_cos=cosine(delta,actual); raw_sign=float(((delta*actual)>0).double().mean()); carry_error=math.sqrt(carry_num/max(carry_den,1e-30)); instrument=executions==12 and carry_error<=1e-6 and residual_closure<=1e-6 and attention_error_max<=1e-6 and bool(torch.isfinite(term_values).all()) and bool(torch.isfinite(actual_values).all())
