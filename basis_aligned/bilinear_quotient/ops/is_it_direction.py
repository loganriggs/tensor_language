# IF NOT MAGNITUDE, IS IT DIRECTION?  -- the question §1824 ended on.
#
# §1824 closed the magnitude account for deep-prefix compilation. Sequential calibration -- the best
# gain correction that exists for this program, each gain measured in a stream already corrected below
# it -- recovers 64.8% of the gap at B0 but only ~12% at B3 and B5, beating the better one-shot source
# there by +1.6 / +1.6 / +0.0 points against a 5-point bar. The remaining ~88% at depth is not a scale
# error, and no per-layer scalar reaches it.
#
# The next candidate is DIRECTION: the live layers may be receiving a stream whose directions are
# wrong, not merely its scale. §1819 measured direction at ONE site and found it preserved -- head
# 5.7's live and compiled mean output vectors had cosine +0.9990 -- but that was the SINGLE-INTERFACE
# case, which §1824 has now shown is the easy one. Nothing has measured direction across a prefix.
#
# METHOD. Mean attention-write VECTOR per layer (not just its norm), captured at c_proj's output with
# the sequential gains installed, so what is compared is the CORRECTED write against the live one. The
# single-interface arm is measured too, as a cross-run anchor against §1819.
#
# ROLES. skip7000 for the direction measurement (a mean vector needs no role replication to identify a
# direction; the gap fractions are still scored on all three). Depths 0, 3, 5. FULL RANK. DISCOVERY
# ONLY.
#
...
#   pred_a IT IS DIRECTION: in the sequentially corrected B3 arm the mean cosine between each live
#          layer's write and its live-model write is below 0.50. If FALSE the directions are largely
#          preserved and the deep-prefix residual is NEITHER scale NOR the direction of the layer
#          means -- which would rule out both first-order accounts and point at higher moments or at
#          position-dependent structure the mean vector cannot see.
#   pred_b AND IT COMPOUNDS: the cosine degrades monotonically with height above the interface at B3.
#          A single bad interface should corrupt the layer above it most and decay; compounding
#          corruption should do the opposite. If FALSE the damage does not accumulate upward and is
#          localised, which would make a targeted per-layer content correction plausible where a global
#          one is not.
#   pred_c AND IT EXPLAINS THE RECOVERY GAP: B0, which the same construction rescues to 64.8%, has a
#          HIGHER mean cosine than B3, which it rescues only to 11.9%. This ties the direction
#          measurement to the accuracy result; if FALSE, direction fidelity and recovery are unrelated
#          and pred_a's answer, whichever way it goes, explains nothing.
#   pred_d CONTROLS, cross-run per LESSON 42 and known-answer per LESSON 34: all four gain arms
#          reproduce §1806 / §1822 / §1823 / §1824's PUBLISHED gap fractions; the first sequential gain
#          equals the global gain at every depth; endpoints reproduce §1789's FULL-RANK figures; the
#          placement control moves top-1 by under 0.05pp; AND the single-interface L5 cosine must
#          exceed 0.90, since §1819 measured +0.9990 for head 5.7 in that same configuration -- a
#          layer-level figure far below that would mean this vector machinery is not measuring what
#          §1819 measured and nothing else here is readable.
...
OUT = PT + 'ops/is_it_direction_results.json'
...
def main():
    ...
    print(f'IS IT DIRECTION | cosine(live, gain-corrected) per layer, depths {DEPTHS} | '
    ...
    top_of = {L: cos_arm[L][17] for L in DEPTHS}
    pa = mean_cos[3] < 0.50
    pb = all(cos_arm[3][j3 + 1] <= cos_arm[3][j3] + 1e-9 for j3 in range(4, 17))
    pc = mean_cos[0] > mean_cos[3]
    ...
    print(f'\n  mean cosine(live, corrected) per arm: ' + '  '.join(
        f'B{L} {mean_cos[L]:+.3f} (top layer {top_of[L]:+.3f})' for L in DEPTHS), flush=True)
    print(f'\n  directions are WRONG at B3 (mean cosine <0.50) -> {pa}  {mean_cos[3]:+.4f}',
          flush=True)
    print(f'  and degrade monotonically with height above the interface -> {pb}  '
          + '  '.join(f'L{j3} {cos_arm[3][j3]:+.3f}' for j3 in (4, 8, 12, 17)), flush=True)
    print(f'  and B0, which recovers 65%, has HIGHER cosines than B3 -> {pc}  '
          f'B0 {mean_cos[0]:+.4f} vs B3 {mean_cos[3]:+.4f}', flush=True)
    print(f'  arms reproduce §1806/§1822/§1823/§1824, interface gain identity holds, the '
          f'single-interface cosine is {cos_single5:+.4f} -> control {pd}', flush=True)

    json.dump({'run': 'is_it_direction', 'depths': list(DEPTHS), 'cal_rows': CALROWS,
               ...
               'gap_fraction': frac,
               'predictions': {'pred_a_directions_wrong_at_B3': bool(pa),
                               'pred_b_cosine_degrades_with_height': bool(pb),
                               'pred_c_B0_has_higher_cosines': bool(pc),
