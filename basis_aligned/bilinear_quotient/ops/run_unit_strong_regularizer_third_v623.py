...
"""v623: swap which control enters the objective. Is row-4 selectivity the FIT, or the control?
...
one way, 10 the other -- and the newer, more surprising half is the thin one. This is the THIRD batch of the strong arm, repeating the v613/v619 arrangement on
TEN MORE FRESH CELLS, five that passed row 4 under the canonical fit and five that failed, none appearing in v603, v611,
...
Smoke: V623_SMOKE=<out.json> (CPU, V623_SMOKE_ROWS=4, V623_SMOKE_NAMES=<cell>).
"""
...
OUT = ROOT / "circuits/followups/unit_strong_regularizer_third_v623_result.json"
...
NAMES = {"polarity_ever_never": "polarity_ever_never",
         "polarity_yet_already": "polarity_yet_already",
         "predicate_category_well_good": "predicate_category_well_good",
         "quantity_change_more_less": "quantity_change_more_less",
         "raising_extraposition_seems": "raising_extraposition_seems",
         "kinship_age_older_younger": "kinship_age_older_younger",
         "measure_wh_far_long": "measure_wh_far_long",
         "numeral_number": "numeral_number",
         "passive_by_agent": "passive_by_agent",
         "polarity_anyone_someone": "polarity_anyone_someone"}
# row 4 verdicts under the CANONICAL control when FITTED against it, from the v591-v607 lift receipts
V2_ROW4 = {
           "polarity_ever_never": True,
           "polarity_yet_already": True,
           "predicate_category_well_good": True,
           "quantity_change_more_less": True,
           "raising_extraposition_seems": True,
           "kinship_age_older_younger": False,
           "measure_wh_far_long": False,
           "numeral_number": False,
           "passive_by_agent": False,
           "polarity_anyone_someone": False,
           }
NEW = tuple(NAMES)
INSTR = ()
...
def _plan():
    return {"candidate_id": "corpus.unit_strong_regularizer_third_v623", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V623_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V623_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V623_SMOKE_NAMES", "verb_preposition_against_for,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_strong_regularizer_third_v623", "candidate_id": "corpus.unit_strong_regularizer_third_v623", "bars": BARS,
