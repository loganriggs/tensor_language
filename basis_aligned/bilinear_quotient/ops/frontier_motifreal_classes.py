"""MOTIFS-REAL CLASS ATTRIBUTION (rung 138): is the assembly bill the motifs' or the tail dictionaries'?

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows; LOWER IS BETTER.
S2230: name/subword damage is NOT front-caused (persists 91%/86% under an exact front). Two suspects remain.
Here the rung-127 build runs with MOTIFS REAL (attention 2-9 real; motif hooks off; tail dicts refit under
real attention; CP-3456 front and everything else unchanged). Whatever name/subword damage collapses is the
motif heads' bill; what persists belongs to the tail dictionaries.

REGISTERED PREDICTIONS (anchors: S2228 class table name +3.134 / subword +2.319; motifs-alone +0.5530):
  (a) NAME IS MOTIF-CAUSED: dCE(name) <= 1.8 (>= 40% collapse).
  (b) SUBWORD IS MOTIF-CAUSED: dCE(subword) <= 1.5.
  (c) SANITY: census agg in [1.1, 1.7] (removing the motifs' +0.55 solo cost plus interaction).
NULL: name/subword persist (>= 80%) - the TAIL DICTIONARIES are the assembly bill and the automata/trie
design (math review 1008, move 3) targets the tail, not the motifs. PRICE: diagnostic (real attention at
blocks 2-9 is not a deployable replacement; attribution only). Tripwire: INSTRUMENT FAIL if L2CF missing.
Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: motifs-real class attribution')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_motifreal_classes_results.json'
...
if __name__=='__main__':
    ...
    print('ARM: rung-127 build with MOTIFS REAL, class table saved',flush=True)
    main()
    ...
    print(f"(a) name {PC.get('name',9.9):+.4f} <= 1.8: {'HELD' if pa else 'FAILED'}")
    print(f"(b) subword {_sw:+.4f} <= 1.5: {'HELD' if pb else 'FAILED'}")
    print(f"(c) census {agg:+.4f} in [1.1, 1.7]: {'HELD' if pc else 'FAILED'}")
