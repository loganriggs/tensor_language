...
def _unc(x, role, lab):
    """milli-nats by which `lab` beats §1959's build on the UNSEEN-TARGET bucket -- the cell the
    fallback exists to serve, and the one §2029 found the converged build losing at skip1200"""
    return 1000.0 * (x.ce(LO, role, S1959, 'pooled', '0-0')
                     - x.ce(LO, role, lab, 'pooled', '0-0'))


def _the_total_reproduces(x):
    """the converged build's unseen-bucket figures rebuild to §2029's +2.060 / +2.963 / −3.948
...
def _the_tables_are_not(x):
    """and §2020's table raise alone is POSITIVE on the unseen bucket on >=2 roles. §2031 showed its
    uncovered-input effect is unstable in sign, so this asks the same question one axis over, where
    §2029 measured +2.060 / +2.963 / -3.948 for the two changes combined"""
    return sum(1 for r in x.roles if _unc(x, r, TABS) > 0) >= 2


def _the_two_roughly_add(x):
    """and the two effects roughly add on the unseen bucket: their sum is within 1.0 milli-nat of the
    converged build's figure, on >=2 roles. §2014 measured the shipped program super-additive by 22% in
