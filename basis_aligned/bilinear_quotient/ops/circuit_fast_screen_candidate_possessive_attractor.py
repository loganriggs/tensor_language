"""possessive_number with an ANIMATE, number-mismatched attractor -- isolating plausibility.

The failing configurations both contain an intervening nominal that could be the controller: a
VP object in the six-token case, an expletive's notional subject in the existential one. That
suggested competition. But the distance-4 screen that PASSED already contains a
number-mismatched nominal -- "The clerks at the desk signed" -- so mere presence cannot be it.

What differs is plausibility: `desk` is inanimate and a poor antecedent for a possessive
referring to people. This holds distance at FOUR, identical to the passing screen, and makes the
intervener ANIMATE and number-mismatched:

    passes   "The clerks at the desk signed"      -> " their"    inanimate mismatched nominal
    here     "The clerks beside the manager signed" -> " their"  ANIMATE mismatched nominal
             "The clerk beside the managers signed" -> " his"

Prediction registered before the run: if plausibility as a controller is the blocker, this fails
where the inanimate version at the same distance passed. If it passes, plausibility is not it
either, and the VP-object failure stays unexplained -- which I will report as such rather than
reach for a fourth story.
"""
...
TASK_ID = "possessive_number.animate_attractor"
...
def _conjunct(noun: str, plural: bool, action: str) -> str:
    """Distance four, as in the passing screen, but the intervening nominal is ANIMATE and
    carries the OPPOSITE number from the antecedent."""
    attractor = "manager" if plural else "managers"
    return f"The {noun}{'s' if plural else ''} beside the {attractor} {action}"


def _notes(noun: str, plural: bool, action: str) -> str:
    attractor = "manager" if plural else "managers"
    return f"In the notes the {noun}{'s' if plural else ''} beside the {attractor} {action}"
