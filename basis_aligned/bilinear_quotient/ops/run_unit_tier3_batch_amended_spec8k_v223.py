#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the eleventh eight spec-authored behaviours, plus one instrument.
"""v223: the AMENDED tier-3 battery (v196/v197 protocol, unchanged) over the ELEVENTH EIGHT spec-authored behaviours -- plus one instrument.

v204 4/8, v205 6/8, v207 4/8, v209 4/8, v211 3/8, v213 3/8, v215 5/8, v217 3/8, v219 5/8, v221 5/8 (+instrument each) at ~30-40 GPU-s
per behaviour. Eight more, authored through `circuit_fast_screen_behaviour_spec`, single-cue swap, matched final token, cue never
final, 32 A1/A2/P/C rows, ABAB directions, opening ONE NEW family and extending four: raising_extraposition_seems (`Near the lantern,
it/the pilot seems, of course,` -> that/to: an expletive subject selects a that-clause, a referential subject selects raising `to`;
NEW family) and raising_extraposition_happened (`it/the pilot happened, of course,`; A2 `turned out`); degree_much_very (`was much/very,
of course,` -> better/good: the degree word selects the adjective's comparative form; degree family, new readout) and degree_so_such
(`was so/such, of course,` -> heavy/a: adjective vs determiner continuation); relative_case_who_whose (`thanked the sailor who/whose, of
course,` -> lifted/name: genitive relative case; case family, who_whom sibling); verb_preposition_insisted (`insisted/believed, of
course,` -> on/in; verb_preposition family, second pair); not_only_inversion (`Not only/Naturally near the lantern, of course,` ->
did/the; inversion family, fifth trigger); polarity_nor_and (`never/often lifted the crate, of course,` -> nor/and: negative-concord
coordinator; polarity family). TWO drafts were REPLACED after the capability check: infinitival_relative_first (`the first/tall pilot,
of course,` -> to/who; A1 -0.92/+0.60 with 2 rows dropped, A2 -0.21/+0.26 with 7 dropped) and infinitival_relative_only (A1 -0.58/+0.03,
9 dropped; A2 28 dropped) -- bilin18 does not carry the infinitival-relative licensing; recorded here as an honest capability null, not run.
...
the eight that run; mean base/donor margins A1: raising_extraposition_seems -4.83/+4.82, raising_extraposition_happened -4.46/+4.49,
relative_case_who_whose -3.43/+3.23, verb_preposition_insisted -2.76/+2.54, polarity_nor_and -2.22/+2.33, degree_much_very -2.00/+2.00,
degree_so_such -1.87/+1.50, not_only_inversion -0.70/+0.63 (the WEAKEST cell, 0 dropped; kept as a capability-limited probe like
v221's proximity_agreement_nor, which passed rows 2/3/5 at -0.97/+0.95);
A2: who_whose -4.33/+4.12, seems -4.18/+3.94, insisted -3.35/+3.08, happened -3.09/+3.03, nor_and -2.10/+2.18, much_very -2.08/+2.11,
so_such -2.01/+1.41, not_only -1.46/+1.48.
Protocol identical to v197/v201/v204/v205/v207/v209/v211/v213/v215/v217/v219/v221 (greedy pool 40, target 0.88, min_gain 0.005, max 30;
...
Accounting: four-row passes here join the one-protocol standing; every member has a sibling in this batch or the corpus (raising pair,
degree pair + the degree_complement family, who_whose + who_whom + pronoun_case, insisted + verb_preposition, not_only + the four
inversion members, nor_and + polarity_tag/polarity_addition), so all wait for the next separability rung before counting.
...
Smoke: V223_SMOKE=<out.json> (CPU, V223_SMOKE_ROWS=4 per split, V223_SMOKE_NAMES=raising_extraposition_seems; pool 3/max 3, steps 5).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec8k_v223_result.json"
...
NAMES = {"raising_extraposition_seems": "raising_extraposition_seems", "raising_extraposition_happened": "raising_extraposition_happened",
         "degree_much_very": "degree_much_very", "degree_so_such": "degree_so_such",
         "relative_case_who_whose": "relative_case_who_whose", "verb_preposition_insisted": "verb_preposition_insisted",
         "not_only_inversion": "not_only_inversion", "polarity_nor_and": "polarity_nor_and",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec8k_v223", "behaviours": 9, "constructions": 1,
...
def main() -> None:
    ...
    smoke = os.environ.get("V223_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V223_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V223_SMOKE_NAMES", "raising_extraposition_seems").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec8k_v223", "candidate_id": "corpus.unit_tier3_batch_amended_spec8k_v223", "bars": BARS,
