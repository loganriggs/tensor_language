#!/usr/bin/env python3
"""Exact E/A/U/W/Y/Z source factorial for MLP8's Task14 response."""

# BQGATE: EXPERIMENT pred_a_instrument_and_parent_closure pred_b_Z_mlp7_dominant pred_c_Y_mlp6_dominant pred_d_distributed_mlp6_7 pred_e_YZ_composition pred_f_direction_switch pred_g_number_specific
...
import run_task14_head11_3_fresh_matched_subject_mlp8_mlp4_7_source_factorial as parent
...
FAMILIES = ("E", "A", "U", "W", "Y", "Z")
SUBSETS = tuple("".join(parts) for size in range(1, 7)
...
BARS = {
    **parent.BARS,
    "maximum_X_grouping_closure_absolute_error": 5e-5,
    ...
    "minimum_YZ_interaction_fraction": .25,
    "maximum_number_specific_lexical_ratio": .25,
}


class MLP8MLP67SourceError(ValueError):
    pass
...
def validate_preflight():
    ...
    if parent_result.get("terminal") != "valid_causal_screen" \
            ...
            or predictions.get("pred_d_distributed_within_V") is not True \
            or predictions.get("pred_g_number_specific") is not True:
        raise MLP8MLP67SourceError("parent result no longer licenses MLP6--7 split")
...
def compile_plan():
    ...
    return {
        "schema": "task14_head11_3_fresh_matched_subject_mlp8_mlp6_7_source_factorial_plan_v1",
        ...
        "input_partition": {
            ...
            "W": "authoritative parent propagated MLP4--5 aggregate",
            "Y": "propagated MLP6 write; the X regrouping remainder follows Y",
            "Z": "propagated MLP7 write",
            "epsilon": "the parent raw-state remainder follows E",
        },
        "response_partition": "exact cross, quadratic, and full MLP8 responses at all 63 nonempty E/A/U/W/Y/Z donor corners",
...
def _decomposed_forward(model, tokens, finals, torch, F, facade, *,
    ...
    def attention(event):
        ...
        if event.site != expected_attention_site:
            raise MLP8MLP67SourceError("attention sites are not sequential")
    ...
    def mlp_dispatch(event):
        ...
        if event.site != expected_mlp_site:
            raise MLP8MLP67SourceError("MLP sites are not sequential")
        expected_mlp_site += 1
        if event.site == MLP_LAYER:
            ...
            y_value, z_value = slots[6], slots[7]
            x_group_remainder = x_value - (y_value + z_value)
            ...
            mlp8.update({
                ...
                "Y": y_value.detach().clone(), "Z": z_value.detach().clone(),
                "X_group_remainder": x_group_remainder.detach().clone(),
    ...
    input_required = {"E", "A", "U", "V", "W", "X", "Y", "Z",
                      "X_group_remainder",
    ...
    if set(captured) != required or set(mlp8) != input_required or projection is None \
        ...
        raise MLP8MLP67SourceError("decomposed forward audit failed")
    ...
    closure = {
        ...
        "V_grouping_closure_max_absolute_error": float((
            ...
            - mlp8["V"]).abs().max()),
        "X_grouping_closure_max_absolute_error": float((
            ((mlp8["Y"] + mlp8["Z"]) + mlp8["X_group_remainder"])
            - mlp8["X"]).abs().max()),
    }
...
def _hybrid_input(recipient, source, subset, F):
    if subset not in SUBSETS:
        raise MLP8MLP67SourceError("unknown E/A/U/W/Y/Z subset")
    ...
    has_u, has_w, has_y, has_z = (
        family in subset for family in ("U", "W", "Y", "Z"))
    if has_y == has_z:
        chosen_x = source["X"] if has_y else recipient["X"]
        x_role = has_y
    else:
        x_remainder = source["X_group_remainder"] if has_y \
            else recipient["X_group_remainder"]
        chosen_x = (chosen["Y"] + chosen["Z"]) + x_remainder
        x_role = None
    if x_role is not None and has_w == x_role:
        chosen_v = source["V"] if has_w else recipient["V"]
        v_role = has_w
    else:
        v_remainder = source["V_group_remainder"] if has_w \
            else recipient["V_group_remainder"]
        chosen_v = (chosen["W"] + chosen_x) + v_remainder
        v_role = None
    v_is_source = v_role is True
    v_is_recipient = v_role is False
...
def evaluate(model, torch, F, facade):
    ...
    exactness = {
        ...
        "X_grouping_closure_max_absolute_error": closure["X_grouping_closure_max_absolute_error"],
    ...
    for source_name in SOURCES:
        ...
        for subset in SUBSETS:
            ...
            subset_slots, subset_outputs, output_diag = polarized_v2._propagated_slots(
                ...
                native_source_output=source_input["output"] if subset == "EAUWYZ" else None)
        ...
        for parent_subset, child_subset in PARENT_TO_CHILD.items():
            ...
            parent_slots, parent_outputs, _ = polarized_v2._propagated_slots(
                ...
                native_recipient_output=input_roles["recipient"]["output"],
                native_source_output=source_input["output"] if parent_subset == "EAUWX" else None)
    ...
    exactness["parent_head_endpoint_max_absolute_error"] = max(
        ...
        for condition, expected in {
            ...
            "opposite_EAUWYZ_full": polarized_v1._parent_head(
                recipient, opposite, "rrrrorr", attention, projection, torch, F),
            "lexical_EAUWYZ_full": grandparent._head_from_slot(
...
def _aggregate_terms(terms, parent_subset, depth_filter=None):
    ...
    for key, value in terms.items():
        ...
        if not families & {"Y", "Z"}:
            continue
        if depth_filter == "Y_only" and ("Y" not in families or "Z" in families):
            continue
        if depth_filter == "Z_only" and ("Z" not in families or "Y" in families):
            continue
        if depth_filter == "YZ" and not {"Y", "Z"}.issubset(families):
            continue
...
def score(evidence, exactness, bars=BARS):
    ...
    if len(observed) != len(expected) or set(observed) != expected \
        ...
        raise MLP8MLP67SourceError("evidence does not cover exact 379-condition screen")
    if any(type(item.get(key)) not in (int, float)
           or not math.isfinite(float(item[key]))
           for item in evidence for key in metric_keys):
        raise MLP8MLP67SourceError("task metric is missing or non-finite")
    ...
    exact_pairs = (
        ...
        ("X_grouping_closure_max_absolute_error", "maximum_X_grouping_closure_absolute_error"),
                ...
                for metric in ("margin", "CE"):
                    ...
                    for child_key, value in child_terms.items():
                        ...
                        parent_key = "".join(
                            ...
                            or (family == "X" and bool(families & {"Y", "Z"})))
            ...
            for metric in ("margin", "CE"):
                ...
                for aggregate in PARENT_AGGREGATES:
                    ...
                    for depth in ("Y_only", "Z_only", "YZ"):
                        ...
                        absolute_ratio = sum(
                            abs(value) for key, value in terms.items()
                            if abs(_aggregate_terms({key: value}, aggregate, depth)) > 0
                        ) / max(abs(denominator), 1e-12)
                        direction_stats[direction][(
                            component, metric, aggregate, f"{depth}_absolute")].append(
                                absolute_ratio)
    ...
    def dominates(depth, selected_directions=None):
        ...
        selected = f"{depth}_only"
        other_only = "Y_only" if depth == "Z" else "Z_only"
        return instrument and all(
            direction_stats[direction][(component, metric, aggregate, selected)]
            ...
            and abs(direction_stats[direction][(
                component, metric, aggregate, "YZ")])
                <= bars["maximum_minor_recovery_fraction"]
            for direction in selected_directions for component in COMPONENTS
            for metric in ("margin", "CE") for aggregate in ("M", "EM", "AM"))

    z_dominant, y_dominant = dominates("Z"), dominates("Y")
    distributed = instrument and not z_dominant and not y_dominant and any(all(
        abs(entries[(component, metric, aggregate, depth)])
            >= bars["minimum_distributed_recovery_fraction"]
        for entries in direction_stats.values() for metric in ("margin", "CE")
        for depth in ("Y_only", "Z_only")) and all(
        abs(entries[(component, metric, aggregate, "YZ")])
            < bars["minimum_YZ_interaction_fraction"]
        for entries in direction_stats.values() for metric in ("margin", "CE"))
    ...
    for direction in direction_stats:
        choices = [depth for depth in ("Y", "Z")
                   if dominates(depth, (direction,))]
        winners[direction] = choices[0] if len(choices) == 1 else None
    direction_switch = instrument and any(
        (all((direction_stats["plural_to_singular"][(component, metric, aggregate, "Y_only")]
              > direction_stats["plural_to_singular"][(component, metric, aggregate, "Z_only")])
             != (direction_stats["singular_to_plural"][(component, metric, aggregate, "Y_only")]
                 > direction_stats["singular_to_plural"][(component, metric, aggregate, "Z_only")])
             for metric in ("margin", "CE"))
         or all(direction_stats["plural_to_singular"][(component, metric, aggregate, "YZ")]
                * direction_stats["singular_to_plural"][(component, metric, aggregate, "YZ")] < 0
                for metric in ("margin", "CE")))
        for component in COMPONENTS for aggregate in ("M", "EM", "AM"))
        ...
        for subset in SUBSETS:
            for component in COMPONENTS:
                for metric in ("margin", "CE"):
                    ...
                    opposite_scale = cell["opposite"][component][metric]["effects"]["EAUWYZ"]
    ...
    return {**exactness,
        ...
        "pred_b_Z_mlp7_dominant": bool(z_dominant),
        "pred_c_Y_mlp6_dominant": bool(y_dominant),
        "pred_d_distributed_mlp6_7": bool(distributed),
        "pred_e_YZ_composition": bool(cross_depth),
...
def main(argv=None):
    ...
    if args.dry_run or os.environ.get("BQLIB_DRYRUN") == "1" \
        ...
        return
    if OUT.exists():
        raise MLP8MLP67SourceError(f"refusing to overwrite {OUT}")
    ...
    result = {
        "schema": "task14_head11_3_fresh_matched_subject_mlp8_mlp6_7_source_factorial_result_v1",
