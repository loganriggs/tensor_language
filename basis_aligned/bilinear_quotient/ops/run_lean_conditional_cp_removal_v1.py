...
Relativecontrol: allcells<=1.10frozenCP512parenterror.1536products848912floats.
...
def cp_values(program,x):
 from lean_conditional_cp import evaluate
...
def main():
 ...
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
  ...
  from lean_conditional_cp import compile_program
  program=compile_program(program,0)
  ...
  actual=dict(input_projection=torch.randn(256,1152),factors=[torch.randn(512,256) for _ in range(4)],biases=[torch.randn(512) for _ in range(4)],matching=0,pair_weights=torch.randn(2,512),coefficients=torch.randn(16,512),constant=torch.randn(16))
 ...
 torch.backends.cuda.matmul.allow_tf32=False;out=P/'LEAN_CONDITIONAL_CP_REMOVAL_V1.json';assert not out.exists();start=time.monotonic()
 frozen=json.loads((P/'LEAN_CONDITIONAL_CP_REMOVAL_INPUTS_V1.json').read_text())
 for file,sha in frozen.items():assert hashlib.sha256((P/file).read_bytes()).hexdigest()==sha
 programs={};hashes={}
 for seed in [1001,1002]:
  path=P/f'LEAN_CONDITIONAL_CP_SEED{seed}_RANK256_V1.pt';hashes[seed]=hashlib.sha256(path.read_bytes()).hexdigest();p=torch.load(path,weights_only=True)
