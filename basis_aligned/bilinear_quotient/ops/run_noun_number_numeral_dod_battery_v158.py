...
"""Numeral number three/one (ones vs one) DoD battery (v158): the atlas top-4 set {11.2, 7.8, 15.1, 10.5} on FRESH rows -- the noun-number candidate's
SECOND line (v157 demonstrative these/this passed 7/7 at 33%; this line decides candidate vs family).

Lane: Claude circuit lane. Frozen before access: F* = 0.22 +/- 0.15 (pred_g). Readers set explicitly and number-free: will-would, who-which, night-day.
...
module does not use, {three -> " ones", one -> " one"}:
...
CANDIDATE_ID = "numeral_number.three_vs_one.dod_battery_v158"
OUT = "noun_number_numeral_dod_battery_v158_result.json"
HEADS = ((11, 2), (7, 8), (15, 1), (10, 5))
FROZEN, BAND = 0.22, 0.15
PREDICTIONS = {"pred_a_instrument_replays_native": "<= 1e-4", "pred_b_native_capability": ">= 0.85 per cell",
               ...
               "pred_g_set_fraction_within_band_of_frozen": "0.22 +/- 0.15 per construction"}
ADJ_CANDIDATES = ("shiny", "dusty", "heavy", "tiny", "faded", "sturdy", "glossy", "rusty", "narrow", "smooth", "rough", "sharp", "blunt", "pale", "dark", "bright", "cheap", "fancy",
                  ...
                  "crisp", "shabby", "sleek", "sturdy", "bulky", "dented", "glossy", "grimy", "jagged", "lumpy", "murky", "musty", "rigid", "rotten", "scruffy", "slimy", "soggy", "spiky", "tattered", "wobbly", "brittle", "coarse", "creaky", "dingy", "frayed")
...
def build():
    own = ("noun_number_v158_agents", "noun_number_v158_objects", "noun_number_v158_adjectives")   # registered after the run; excluded so replay is stable
    ...
    for construction, make in CONSTRUCTIONS.items():
        for g, agent in enumerate(agents):
            for present in (True, False):
                text = make(agent, objects[g], adjectives[g], "three" if present else "one")
                ...
                rows.append(L.Row(hashlib.sha256(json.dumps(["noun_number_v158", construction, g, present, text]).encode()).hexdigest()[:24],
