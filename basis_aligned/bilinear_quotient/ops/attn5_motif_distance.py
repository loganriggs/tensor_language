...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['attn5_motif_class_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; distance split of the block-5 motif ind damage on FR (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'attn5_motif_distance_results.json'
...
if __name__=='__main__':
    ...
    SEL['motif_off']=(5,)
    print('ARM 2/2: block-5 motifs REAL, per-position CE captured',flush=True)
    r_no5=main()
    cev_no5=SEL['cev']
    ...
    print(f"(a) near(<=32) {s_near:+.1f} >= 0.6 x all {s_all:+.1f}: {'HELD' if pa else 'FAILED'}")
    print(f"(b) peak bin 2^{peak} < 16: {'HELD' if pb else 'FAILED'}")
