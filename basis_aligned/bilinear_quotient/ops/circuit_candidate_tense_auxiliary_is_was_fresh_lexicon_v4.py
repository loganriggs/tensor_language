"""Prospective fourth is/was lexicon for the bracketed-root actuator."""
...
_AGENTS = (
    "acrobat", "barber", "carpenter", "dancer", "ecologist", "firefighter",
    "grocer", "illustrator", "jeweler", "locksmith", "musician", "navigator",
    "potter", "sailor", "technician", "upholsterer",
)
...
def _panel(group_number: int) -> list[dict[str, Any]]:
    ...
    group_id = f"FIT:{canonical_sha256([SCHEMA, TASK_ID, 'different_readout_v4', SEED, group_number])[:24]}"
    ...
    common_no_vocab = dict(seed=SEED, task_id=TASK_ID, group_number=group_number, group_id=group_id, reporter=agent, alternate_reporter=alternate, adjective="different_readout_v4", object_name="moment", spec=TASK_SPEC)
...
def _validate(rows: Sequence[Mapping[str, object]]) -> str:
    ...
    if materialized != _build():
        raise CandidateBankError("rows differ from sealed v4 authority")
    try:
    ...
    old_rows = v1.build_rows() + v2.build_rows() + v3.build_rows()
    old_agents = set(v1._AGENTS) | set(v3._AGENTS)
    if (
        len(materialized) != 64
        or len({str(row["row_id"]) for row in materialized}) != 64
        or {str(row["row_id"]) for row in materialized} & {str(row["row_id"]) for row in old_rows}
        or set(_AGENTS) & old_agents
        or any(not all(row["construction_checks"].values()) for row in materialized)
    ):
        raise CandidateBankError("v4 count, uniqueness, disjointness, agent novelty, or construction check failed")
    return digest
