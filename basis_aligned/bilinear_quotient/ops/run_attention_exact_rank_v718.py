...
"""Attention lane, v718: EXACT RANK — do the fitted pattern programs rely on off-rank components, and does a factored (exact-rank) refit
reach the same cost?

v716: re-truncating the jointly fitted pattern maps to their nominal ranks cost +0.08. The v706-v715 protocol fitted each head's four QK maps
as free 128 x 1152 tensors initialised at rank r; they keep 99.9% of their energy in-rank, but the bilinear pattern may be sensitive to
the rest. Part (i), replay: the saved v706 / v713 / v715 endpoint maps evaluated as saved and after exact re-truncation to their nominal
ranks (kappa_r recomputed). Part (ii), the fix: v713's configuration (111 heads at 16, 51 at 64, no native heads) refitted with FACTORED
maps U_r V_r (U 128 x r, V r x 1152; exact rank by construction; balanced SVD init), Adam 300 steps (v706 rates), validation-minimum
SNAPSHOT, manipulability over the 24 most valuable heads on the snapshot. Values (now earned exactly): 25.9M vs 95.6M. CE ADDED, lower is
better; joint recovery = 1 - cost / 3.996; ratio = in-program / native mean-ablation value.
PREDICTIONS (scored as written; failures preserved)
    pred_a_native_replays        native CE within 0.002 of 3.13241 (instrument)
    pred_b_offrank_matters       v713's maps re-truncated cost >= their as-saved cost + 0.02 (the off-rank part carries real loss). Prior: likely
    pred_c_factored_cost         factored refit snapshot held-out <= 0.080 (v713 as saved: 0.060 reported / 0.073 endpoint). Prior: unsure
    pred_d_factored_values_preserved_rank Spearman over the 24 heads >= 0.8 (v713: 0.82). Prior: unsure
...
OUT = ROOT / "circuits/followups/attention_exact_rank_v718_result.json"
OUT_PT = ROOT / "circuits/followups/attention_exact_rank_v718_programs.pt"
...
CANDIDATE_ID = "attention.exact_rank_v718"
...
ARMS = ("factored",)
REPLAY_TOL, OFFRANK_MIN, COST_MAX, RHO_MIN, RATIO_MAX = 0.002, 0.02, 0.080, 0.8, 2.0
PREDICTIONS = {"pred_a_native_replays": "+-0.002", "pred_b_offrank_matters": "re-truncated >= as-saved + 0.02 (v713)", "pred_c_factored_cost": "<= 0.080",
               "pred_d_factored_values_preserved_rank": "Spearman >= 0.8", "pred_e_factored_no_head_over_2": "max ratio <= 2.0"}


def main() -> None:
    plan = {"candidate_id": CANDIDATE_ID, "forwards_max": FORWARDS_MAX, "model_backwards": BACKWARDS_MAX, "model_updates": 0, "fit_parameters": 111 * 4 * 16 * 1280 + 51 * 4 * 64 * 1280 + 162 * 513, "replay": list(REPLAY),
    ...
    OUT.write_text(json.dumps({"schema": "attention_exact_rank_result_v718", "candidate_id": CANDIDATE_ID, "plan": plan, "report": {"native": native, "joint_value": JOINT_VALUE, "arms": results, "manipulability": manip, "spearman": rho, "median_ratio": med_ratio, "program_endpoint_cost": base_prog - native},
