...
def main():
 ...
 full_ref=json.loads((ROOT/'circuits/followups/temporal_iswas_five_mlp_rank16_gain_curve_v1_result.json').read_text())['reports']['1.15'];replay=max(abs(reports['full']['response'][t][m]-full_ref['response'][t][m]) for t in TASKS for m in ('signed_projection','relative_squared_error','norm_ratio'))
