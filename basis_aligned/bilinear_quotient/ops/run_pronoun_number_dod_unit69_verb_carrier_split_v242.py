#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_carrier_closure pred_b_attention_4_5_carries_030 pred_c_head_4_5_leads_attention pred_d_embedding_carries_at_most_010
"""Pronoun number they/he DoD (v242): what MLP-6 unit 69 reads AT THE VERB. v241: at the verb, unit 69 is the largest MLP-6 input of the plural detector 829 (38%),
although at the noun it damps it (v182: -172). The number reaches the verb through head 4.5's copy (v205 / v206: 94% of block 4's contrast at the verb, 7.7% of the
margin under edit). Carrier split (exact, `dod_units.carrier_split`) of u_69's plural - singular contrast at the block-6 input AT THE VERB: writers = embedding, attn /
mlp totals of blocks 0-5, the nine heads of block 6 -- and, because attention is the only route to the verb, the attention totals of blocks 4 and 5 are the registered carriers.
PREDICTIONS (scored as written; failures preserved; priors unsure)
    ...
    pred_b_attention_4_5_carries_030  attn:04 + attn:05 carry >= 0.30 of unit 69's contrast at the verb
    pred_c_head_4_5_leads_attention   attn:04 is the largest attention writer (4.5's block)
    pred_d_embedding_carries_at_most_010  the embedding (the verb token) carries <= 0.10 (the verb token is the same in both members of a pair)
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_unit69_verb_carrier_split_v242_result.json"
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_unit69_verb_carrier_split_v242"
UNITS, LAYER, CLOSURE_TOL, ATTN45_MIN, EMBED_MAX, BATCH = (69,), 6, 1e-3, 0.30, 0.10, 32
FORWARDS_MAX = 5
WRITERS = ["embed"] + [f"{k}:{l:02d}" for l in range(LAYER) for k in ("attn", "mlp")] + [f"attnhead:{LAYER:02d}:{h}" for h in range(9)]
PREDICTIONS = {"pred_a_carrier_closure": "<= 1e-3", "pred_b_attention_4_5_carries_030": ">= 0.30", "pred_c_head_4_5_leads_attention": "attn:04 largest", "pred_d_embedding_carries_at_most_010": "<= 0.10"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "rows": len(rows), "rows_sha256": L.rows_sha256(rows), "units": UNITS, "layer": LAYER, "writers": WRITERS, "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"closure_tol": CLOSURE_TOL, "attn45_min": ATTN45_MIN, "embed_max": EMBED_MAX}, "position": "verb"}
    ...
    predictions = {"pred_a_carrier_closure": True, "pred_b_attention_4_5_carries_030": True, "pred_c_head_4_5_leads_attention": True, "pred_d_embedding_carries_at_most_010": True}
    ...
    for unit in UNITS:
        ...
        attn_writers = {w: v for w, v in cs.items() if w.startswith("attn:")}; lead_attn = max(attn_writers, key=lambda w: abs(attn_writers[w]))
        print("   attn:04 + attn:05", round(cs["attn:04"] + cs["attn:05"], 3), "largest attention writer", lead_attn, round(attn_writers[lead_attn], 3))
        predictions["pred_a_carrier_closure"] &= abs(sum(cs.values()) - 1) <= CLOSURE_TOL; predictions["pred_b_attention_4_5_carries_030"] &= (cs["attn:04"] + cs["attn:05"]) >= ATTN45_MIN
        predictions["pred_c_head_4_5_leads_attention"] &= lead_attn == "attn:04"; predictions["pred_d_embedding_carries_at_most_010"] &= cs["embed"] <= EMBED_MAX
    predictions["pred_a_carrier_closure"] &= closure <= CLOSURE_TOL
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_mlp6_unit_carrier_split_result_v242", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "units": report, "predictions": predictions, "forwards": forwards,
