"""PROBE GATE v4 -- is the two-site gain DEPTH or WIDTH? A control, not a new idea.

§2079 (rung 4) found the concatenated [2;9] probe gates best of everything tried:
efficiency 6.113x random and 66.4% of the oracle gain, against the best single
site's 5.488x (blk5) and the v1 block-2 baseline's 3.867x. It also found that
arm SECOND-WORST on AUC (0.6382 vs blk9's 0.6545), reproducing S1365's two-site
negative, and concluded that AUC is the wrong headline for a gate that only ever
uses the top 17.25% of the ranking.

§2079 registered its own open question and declined to claim two sites help:
"[2;9] is 2304-dim against the single sites' 1152, so whether the gain is depth
or width is unseparated -- I am not claiming two sites help until that control
exists." This is that control. All three pairs drawn from {2, 5, 9} are 2304-dim,
so width is held constant and only the site choice varies.

REGISTERED PREDICTIONS (fresh rows, doc-disjoint, against §2079's MEASURED arms):
  (a) WIDTH HYPOTHESIS: all three 2304-dim pairs exceed the best 1152-dim single
      site's efficiency of 5.488x (blk5). If TRUE, doubling the feature count
      helps whichever sites are used, and §2079's [2;9] result is not evidence
      that two SITES carry complementary information.
  (b) AND THE SITES DO NOT MATTER: the spread among the three pairs is small --
      max/min efficiency ratio < 1.20. Registered separately from (a) because
      they are not the same claim: (a) could hold while one pair still clearly
      beats the others, which would mean width sets a floor and site choice
      still buys something on top of it.
  (c) INSTRUMENT CONTROL: the blk2 single-site arm reproduces §2079's 3.867x
      within 0.15x, and §2079 itself reproduced §342's 3.826x. Two independent
      reproductions of the same baseline across three runs is what licenses
      comparing arms BETWEEN runs; without it every cross-run number here is
      uncontrolled.

Writes probe_gate4_results.json. No claim about which configuration to deploy is
made here -- that needs the fresh-window certification §2079 already flagged.
"""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['probe_gate3_results.json','experiments/gating/probe_gate.py']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _v3=json.load(open(_bq+'probe_gate3_results.json'))
    if 'arms' not in _v3 or 'blk2+9' not in _v3['arms']:
        print('DRYRUN FAIL: S2079 arms missing'); raise SystemExit(1)
    print(f"DRYRUN OK: S2079 present (blk2 {_v3['arms']['blk2']['eff_vs_random']}x, "
          f"blk5 {_v3['arms']['blk5']['eff_vs_random']}x, "
          f"blk2+9 {_v3['arms']['blk2+9']['eff_vs_random']}x); "
          f"width control = three 2304-dim pairs")
    raise SystemExit(0)
...
OUT=PT+'probe_gate4_results.json'
...
def main():
    ...
    S2079={'blk2':3.867,'blk5':5.488,'blk9':4.574,'blk2+9':6.113}
    PAIRS=('blk2+5','blk2+9','blk5+9')
    pe=[ARMR[n]['eff_vs_random'] for n in PAIRS]
    best_single_eff=max(ARMR[n]['eff_vs_random'] for n in ('blk2','blk5','blk9'))
    pa=all(x>5.488 for x in pe)
    pb=(max(pe)/max(min(pe),1e-4))<1.20
    pc=abs(ARMR['blk2']['eff_vs_random']-3.867)<=0.15
    out={'base':round(baseC2,4),'oracle':round(gat,4),'random':round(rnd,4),
         'fraction':round(frac,4),'oracle_gain':round(gain,4),
         'random_gain':round(rgain,4),'S2079_measured':S2079,'arms':ARMR,
         'pair_efficiencies':{n:ARMR[n]['eff_vs_random'] for n in PAIRS},
         'best_single_site_efficiency':best_single_eff,
         'pred_a_all_pairs_beat_best_single':bool(pa),
         'pred_b_pairs_are_interchangeable':bool(pb),
         'pred_c_blk2_reproduces_S2079':bool(pc)}
    print(f"(a) all 2304-dim pairs > 5.488x (best 1152-dim single): "
          f"{ {n: ARMR[n]['eff_vs_random'] for n in PAIRS} } : "
          f"{'HELD' if pa else 'FAILED'}")
    print(f"(b) pair spread max/min = {max(pe)/max(min(pe),1e-4):.3f} < 1.20 : "
          f"{'HELD' if pb else 'FAILED'}")
    print(f"(c) blk2 {ARMR['blk2']['eff_vs_random']:.3f}x reproduces S2079's "
          f"3.867x within 0.15 : {'HELD' if pc else 'FAILED'}")
    if pa and pb:
        print('    READING: width, not site choice -- S2079 must not be quoted '
              'as evidence that two SITES carry complementary information.')
    elif pa and not pb:
        print('    READING: width sets a floor and site choice buys more on top.')
