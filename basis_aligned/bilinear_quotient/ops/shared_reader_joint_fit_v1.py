"""Shared implementation for four separately managed matched local fits."""
...
PREFIX='SHARED_READER_JOINT_FIT_V1'
...
def main(label,family):
    assert label in ('spectral','native') and family in ('original','graph')
    ...
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
        print(json.dumps(dict(model_loaded=False,gpu_accessed=False,body_forwards=0,corpus_access=False,
                              ...
                              raw_entry_bounds=[-1,1],reencode_every=200,penalty=.01,
    ...
    del check
    fitted,stats=fit(target,graph,total,seconds=1200,penalty=.01,max_cycles=100)
    with torch.no_grad():
    ...
    predictions={'pred_a':bool(numeric),'pred_b':bool(stats['local_converged']),
                 'pred_c':bool(stats['initial']-stats['final']>=1e-4)}
    final=cpu(fitted);torch.save(final,artifact)
    result=dict(**predictions,label=label,family=family,capture=stats['capture'],initial_cpu_replay=initial_replay,
                executor_replay=replay,price=price(final,parts),optimization=stats,
                artifact=dict(path=str(artifact),sha256=digest(artifact)),
                scope='Bounded/re-encoded local fitting at fixed shared topology, matched to original group family. Full folded weight metric; no global recovery, corpus fitting, or behavioral circuit claim.')
