"""ASSEMBLY ERROR QUOTIENT v1 -- what does the assembly's OWN block-6 error cost, and is that cost in the
observable subspace?

The two observability artifacts (OBSERVABILITY_QUOTIENT_V1_RESULT.md, STREAM_ERROR_PRICE_V1_RESULT.md) priced
RANDOM stream error: direction matters at most 3.5x at first order, but depth matters enormously -- a half-norm
random error costs 0.058 nat at block 5 and 1.48 nat at block 6, and the certified empirical assembly's stream
error (S2086) peaks at block 6 at rel-MSE 1.74 (relative norm ~1.3), where a random error of norm 1.0 costs 3.6
nat. The assembly's whole gap is +2.9. So its structured error must be far cheaper than random error of the same
size, and the question this run asks is WHICH PART of it is cheap: the quotient relative to the program's own
error, which the alternate entry point (explanation_1745 s8.3) named as the object to factor.

METHOD. Rebuild S2086's certified empirical matched-context arm verbatim (same fits, same rows, same tail refit),
collect real and assembly streams at every block input on the S2086 rows (R0:R1), and at sites 5, 6, 7 form the
assembly's error E_k = assembly - real per position. Then, with NO hooks installed (the real model), inject at
block k alone: (i) E_k itself; (ii) 8 random errors matched to E_k's per-position norm; (iii) E_k projected on
the top-r90 subspace of the loss-gradient Gramian G_k (computed on the same rows, real model); (iv) its
complement; (v) E_k projected on a random r90-dimensional subspace. Price = mean CE increase over positions >= 64
against the real model on the same rows.

REGISTERED PREDICTIONS (site 6, the hump and the cliff; sites 5 and 7 reported descriptively):
  (a) STRUCTURED IS CHEAPER THAN RANDOM: dCE(E_6) <= 0.5 * mean dCE(random, matched norm). A random error of the
      assembly's block-6 norm costs more than the assembly's entire gap, so the assembly's error must sit in cheap
      directions; 0.5 is the design bar at which "structured" is a usable statement. If FALSE the assembly's
      error is priced like noise and no quotient helps -- only norm reduction does.
  (b) THE COST LIVES IN THE OBSERVABLE PART: dCE(P_obs E_6) >= 0.8 * dCE(E_6). If FALSE the first-order observable
      subspace does not capture what makes the assembly's error expensive, and a finite/nonlinear quotient is
      needed even for the program's own error.
  (c) THE COMPLEMENT IS CHEAP AND THE NULL IS BEATEN: dCE(P_cmp E_6) <= 0.3 * dCE(E_6) AND dCE(P_obs E_6) >
      dCE(P_rand E_6). Without the second clause, (b) could be subspace dimension rather than direction.
  (d) REPRODUCTION GATE (LESSON 42): block-6 rel-MSE of the rebuilt arm reproduces S2086's 1.7415 within 0.10.
      If it fails, the arm is not S2086's and nothing above is comparable.

Self-reviewed. Writes assembly_error_quotient_results.json.
"""
...
OUT=PT+'assembly_error_quotient_results.json'
...
def main():
    ...
    # ================= new analysis: price the assembly's OWN error =================
    ...
    SKIP=64
    def run_real(site=None, delta=None):
        with torch.no_grad():
            idx=TOK[:,:-1]; tg=TOK[:,1:].reshape(-1)
            x=F.rms_norm(m.transformer.wte(idx),(D,)); x0=x; v1=None
            for li,blk in enumerate(m.transformer.h):
                if li==site and delta is not None: x=x+delta
                x,v1=blk(x,v1,x0)
            lg=(30*torch.tanh(m.lm_head(F.rms_norm(x,(D,)))/30)).float()
            ce=F.cross_entropy(lg.view(-1,lg.size(-1)),tg,reduction='none').view(NRW,TT)
    ...
    base=run_real()
    gen=torch.Generator(device='cpu').manual_seed(6)
    res={}
    for site in (5,6,7):
        E=(empS[site]-real[site]).view(NRW,TT,D)
        pn=E.norm(dim=-1,keepdim=True)
        xn=real[site].view(NRW,TT,D).norm(dim=-1).mean()
        P,r90=gramian(site)
        own=run_real(site,E)-base
        rnd=[]
        for _ in range(8):
            u=torch.randn(NRW,TT,D,generator=gen).to(DEV); u=u/u.norm(dim=-1,keepdim=True).clamp_min(1e-9)
            rnd.append(run_real(site,u*pn)-base)
        Eobs=E@P@P.T; Ecmp=E-Eobs
        Rr=torch.linalg.qr(torch.randn(D,r90,generator=gen))[0].to(DEV); Ernd=E@Rr@Rr.T
        d_obs=run_real(site,Eobs)-base; d_cmp=run_real(site,Ecmp)-base; d_rnd=run_real(site,Ernd)-base
        res[str(site)]={'rel_mse':round(prof[site],4),'rel_norm_mean':round(float((pn.squeeze(-1)/xn).mean()),4),
            'r90_obs':r90,'energy_share_in_observable':round(float((Eobs**2).sum()/(E**2).sum()),4),
            'dCE_own':round(own,5),'dCE_random_matched_norm':{'mean':round(sum(rnd)/8,5),'min':round(min(rnd),5),'max':round(max(rnd),5)},
            'dCE_observable_projection':round(d_obs,5),'dCE_complement_projection':round(d_cmp,5),
            'dCE_random_subspace_projection':round(d_rnd,5)}
        print(f"site {site}: rel-MSE {prof[site]:.3f} rel-norm {res[str(site)]['rel_norm_mean']:.3f} | own {own:+.4f} | "
              f"random {sum(rnd)/8:+.4f} | obs {d_obs:+.4f} ({res[str(site)]['energy_share_in_observable']:.3f} of energy) "
              f"cmp {d_cmp:+.4f} rndsub {d_rnd:+.4f} | r90 {r90}",flush=True)
        del E,Eobs,Ecmp,Ernd; torch.cuda.empty_cache()
    s6=res['6']
    pa=s6['dCE_own']<=0.5*s6['dCE_random_matched_norm']['mean']
    pb=s6['dCE_observable_projection']>=0.8*s6['dCE_own']
    pc=(s6['dCE_complement_projection']<=0.3*s6['dCE_own']) and (s6['dCE_observable_projection']>s6['dCE_random_subspace_projection'])
    pd=abs(prof[6]-1.7415)<=0.10
    out={'base_ce_real_rows':round(base,5),'rows':NRW,'profile_rel_mse':{str(k):round(v,4) for k,v in prof.items()},
         'sites':res,'S2086_block6_rel_mse':1.7415,
         'pred_a_structured_cheaper_than_random':bool(pa),'pred_b_cost_in_observable_part':bool(pb),
         'pred_c_complement_cheap_and_null_beaten':bool(pc),'pred_d_reproduces_S2086':bool(pd),
         'self_reviewed':True,'runtime_s':round(time.time()-t0,1)}
    json.dump(out,open(OUT,'w'),indent=1)
    print(f"(d) block-6 rel-MSE {prof[6]:.4f} vs S2086 1.7415 (tol 0.10): {'HELD' if pd else 'FAILED'}")
    print(f"(a) own {s6['dCE_own']:+.4f} <= 0.5 x random {s6['dCE_random_matched_norm']['mean']:+.4f}: {'HELD' if pa else 'FAILED'}")
    print(f"(b) observable projection {s6['dCE_observable_projection']:+.4f} >= 0.8 x own: {'HELD' if pb else 'FAILED'}")
    print(f"(c) complement {s6['dCE_complement_projection']:+.4f} <= 0.3 x own and obs > random-subspace {s6['dCE_random_subspace_projection']:+.4f}: {'HELD' if pc else 'FAILED'}")
