#!/usr/bin/env python
"""late_width_control_probe -- independent physical control before publishing a correction: is the k = 768 bus read cost
(.164, late_width_control_probe) the late blocks' own width use (each late site through its OWN 768-frame) or an
artifact of sharing one frame? Plus early own frames at 768 and the bus cost per late block. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_late_width_use_is_real_not_a_sharing_artifact pred_c_early_own_frames_cheap_at_768
#                     pred_d_late_per_block_costs_are_additive pred_e_late_cost_is_spread
...
Preregistration: polynomial_causal/LATE_WIDTH_CONTROL_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_width_control_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_width_control_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_WIDTH_CONTROL_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "early_block_read_cost_map_probe_results.json"   # §2770
OUT = ROOT / "late_width_control_probe_results.json"
HASHES = {PREREG: "1cbdd8a73aa3e7cc5170ef92cc8dd05e35858f6faf1bbc319716c804449740ea", PRIOR: "95de78d8b653c4724457c759f0e45cfbbc4f92925e4d2932334ecd3311085c35",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_width_control_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_min": 0.100, "c_max": 0.050, "d_lo": 0.5, "d_hi": 1.5, "e_max": 0.60}
NULLS = {"b_max": 0.050, "c_min": 0.100, "d_lo": 0.25, "d_hi": 3.0, "e_min": 0.80}
...
def main():
    ...
    def own_of(sites, k):
        return run({s_: head(s_, k) for s_ in sites})
    def one_late(l, k):
        return run({s_: head(s_, k, U_8) for s_ in blocks([l])})
    arms = {"SPLIT8_1024": split8(1024), f"BUS_{KM}": bus_only(KM), f"LATE_OWN_{KM}": own_of(SET8, KM), f"EARLY_OWN_{KM}": own_of(OWN8, KM)}
    per_block = {}
    for l in (range(SPLIT, SPLIT + 2) if smoke else range(SPLIT, NL)):
        arms[f"ONE_L{l}_{KM}"] = one_late(l, KM); per_block[str(l)] = arms[f"ONE_L{l}_{KM}"]
    ...
    ratio = tot / arms[f"BUS_{KM}"] if abs(arms[f"BUS_{KM}"]) > 1e-9 else float("inf")
    top3_share = sum(v for _, v in ranked[:3]) / tot if abs(tot) > 1e-9 else float("nan")
    summ = {"sum_late_blocks": tot, "bus": arms[f"BUS_{KM}"], "additivity_ratio": ratio, "top3_share": top3_share, "ranked": ranked}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(arms[f"BUS_{KM}"] - PRIOR_BUS_768) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_late_width_use_is_real_not_a_sharing_artifact': bool(arms[f"LATE_OWN_{KM}"] >= BARS["b_min"]),
        'pred_c_early_own_frames_cheap_at_768': bool(arms[f"EARLY_OWN_{KM}"] <= BARS["c_max"]),
        'pred_d_late_per_block_costs_are_additive': bool(BARS["d_lo"] <= ratio <= BARS["d_hi"]),
        'pred_e_late_cost_is_spread': bool(top3_share <= BARS["e_max"]),
    }
    nulls = {"b_null_late_own_le_.05": bool(arms[f"LATE_OWN_{KM}"] <= NULLS["b_max"]), "c_null_early_own_ge_.10": bool(arms[f"EARLY_OWN_{KM}"] >= NULLS["c_min"]),
             "d_null_ratio_ge_3_or_le_.25": bool(ratio >= NULLS["d_hi"] or ratio <= NULLS["d_lo"]), "e_null_top3_ge_.80": bool(top3_share >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "control: late sites through OWN 768-frames vs the shared bus; early sites through own 768-frames; bus cost per late block",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024,
                          "bus_768": arms[f"BUS_{KM}"], "prior_bus_768": PRIOR_BUS_768},
           "ce_added": arms, "per_late_block_cost_768": per_block, "summary": summ,
    ...
    if smoke:
        print(json.dumps({k: out[k] for k in ("preds", "nulls", "instrument", "ce_added", "per_late_block_cost_768", "summary", "price")}, indent=1)); return
    from receipt import dump
    dump(out, OUT)
    print(json.dumps({"preds": preds, "nulls": nulls, "ce_added": arms, "per_late_block_cost_768": per_block, "summary": summ}, indent=1))
