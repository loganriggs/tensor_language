"""CERTIFICATE-TARGETED REPAIR AT m0 (rung 108): is the circuit breakage an OBJECTIVE problem or a CAPACITY problem?

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model); LOWER IS BETTER. S2198: m0's table alone
breaks 61/62 circuits at only +0.2485 aggregate. The S2195 hand-off asked for repair-targeted compilation.
Here the m0 residual (SAME grammar: quadfeat -> rank-64) is refit on the census TRAIN half (rows 0-499) twice:
unweighted, and with weight 10 on every position that is a member of ANY battery circuit. Certificates are
scored on the census TEST half (rows 500-999) only — populations stated per S2190; battery refs are full-census
(noted). Three single-site arms (each installed with exact a0): plain m0E (FW-fit), refit_unw, refit_wt.

REGISTERED PREDICTIONS (arms named; test half only; circuits with >= 20 test members):
  (a) OBJECTIVE MATTERS: median over circuits of memberabs(refit_unw)/memberabs(refit_wt) >= 1.5.
  (b) VALIDITY APPEARS: refit_wt valid for >= 10 circuits (anchor: prefix_m0E was 1/62 full-census, S2198).
  (c) BOUNDED PRICE + POPULATION SANITY: agg(refit_wt) <= 2 x agg(refit_unw), AND |agg(plain) - 0.2485| <= 0.10.
NULL: weighting moves nothing — m0's breakage is capacity/grammar-limited, closing the objective-repair branch
(rung 107 tests the capacity branch at m2). PRICE: none (same 64-rank residual; weights are free).
Tripwire: INSTRUMENT FAIL if refit_wt's cev is bitwise equal to plain's (S2189 inert-hook class). Self-reviewed."""
import json, sys, time, os
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
if os.environ.get('BQLIB_DRYRUN')=='1':
    _bq='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
    _need=['frontier_front_prefix_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: m0 certificate-targeted repair')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_m0_repair_results.json'
...
def main():
    ...
    if SEL.get('m0_repair'):
        _ROWS=SEL.get('ext_rows',FR)
        _TR=_ROWS[:500]; _TE=_ROWS[500:]
        _mem=SEL['mem_flat']
        _wtr=(1.0+9.0*_mem[:500*256].float()).to(DEV)
        ...
        hs.append(m.transformer.h[0].mlp.register_forward_hook(_cap))
        ...
        for h in hs: h.remove()
        _X=torch.cat(_Xs); _Y=torch.cat(_Ys)
        _tb=S['m0E'][2]
        _ids=_TR[:,:256].reshape(-1).to(DEV)
        _Rr=_Y-_tb[_ids].float()
        _,_,_Vh=torch.linalg.svd(_Rr[:30000],full_matrices=False)
        _P=orth(_Vh[:64].T)
        _ft=quadfeat(_X,0)
        _lam=1e-2*len(_X)
        def _solve(w):
            fw=_ft*w[:,None]
            M=fw.T@_ft+_lam*torch.eye(_ft.shape[1],device=DEV)
            return torch.linalg.solve(M,fw.T@(_Rr@_P))
        _Au=_solve(torch.ones(len(_X),device=DEV))
        _Aw=_solve(_wtr)
        del cur['clsmap']
        cur['clsmap']=classify2(_TE).to(DEV)
        _arms={}
        for _nm,_A in [('plain',None),('refit_unw',_Au),('refit_wt',_Aw)]:
            S['m0X']=S['m0E'] if _A is None else ('tableres',0,_tb,_A,_P)
            _arms[_nm]=evalV(_TE,_TE.shape[0],['a0','m0X'],ML).detach().cpu()
            print(f'  arm {_nm} done',flush=True)
        del cur['clsmap']
        if torch.equal(_arms['refit_wt'],_arms['plain']):
            raise SystemExit('INSTRUMENT FAIL: refit_wt cev bitwise equal to plain (inert hook)')
        SEL['front_result']=_arms
...
if __name__=='__main__':
    ...
    CINFO={}
    MEM=torch.zeros(NFLAT,dtype=torch.bool)
    ...
    HALF=NFLAT//2
    ANCH=json.load(open(PT+'frontier_front_prefix_results.json'))['arms']['prefix_m0E']['agg']
    SEL['mode']='norm'; SEL['K']=4608; SEL['K69']=4608; SEL['K69MAP']={}
    SEL['skipset']=tuple(range(10,18)); SEL['motif_off']=tuple(range(2,10))
    SEL['m0_repair']=True; SEL['ext_rows']=CROWS; SEL['mem_flat']=MEM
    print(f'ARMS: m0 plain / census-refit / member-weighted refit; train rows 0-499, TEST rows 500-999 ({int(MEM.sum())} member positions of {NFLAT})',flush=True)
    main()
    A=SEL['front_result']
    CB_TE=CBASE[HALF:]
    out={}; dmap={}
    ...
    for t,info in CINFO.items():
        ...
        n=int(mte.sum())
        if n<20: continue
        r={'tag':t,'n_test_members':n,'ref':round(info['ref'],3)}
        for k in A:
            r['memberabs_'+k]=round(float(dmap[k][mte].abs().mean()),4)
            r['valid_'+k]=bool(r['memberabs_'+k]<0.5*info['ref'])
        r['ratio_unw_over_wt']=round(r['memberabs_refit_unw']/max(r['memberabs_refit_wt'],1e-6),3)
        rows.append(r)
    import statistics as stt
    med=stt.median([r['ratio_unw_over_wt'] for r in rows])
    ...
    pa=med>=1.5
    pb=nv['refit_wt']>=10
    pc=(out['refit_wt']['agg']<=2*out['refit_unw']['agg']) and abs(out['plain']['agg']-ANCH)<=0.10
    res={'arms':out,'n_scored_circuits':len(rows),'valid_counts':nv,'median_ratio_unw_over_wt':round(med,3),
         'anchor_prefix_m0E_fullcensus':ANCH,'circuits':rows,
         'convention':'per-position dCE = CE(config) - CE(real model); TEST half (census rows 500-999); refs are full-census battery refs',
         'pred_a_objective_matters':bool(pa),'pred_b_validity_appears':bool(pb),'pred_c_price_and_population':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'median memberabs ratio unw/wt {med:.3f}; valid wt {nv["refit_wt"]}/{len(rows)}')
    print(f"(a) median ratio {med:.3f} >= 1.5: {'HELD' if pa else 'FAILED'}")
    print(f"(b) refit_wt valid {nv['refit_wt']} >= 10: {'HELD' if pb else 'FAILED'}")
    print(f"(c) agg(wt) {out['refit_wt']['agg']:+.4f} <= 2 x agg(unw) ({2*out['refit_unw']['agg']:+.4f}) and plain {out['plain']['agg']:+.4f} ~ anchor {ANCH:+.4f}: {'HELD' if pc else 'FAILED'}")
