...
"""Widths4/32, inherited/random,1000Adamsteps. True native purequartic targets.
...
def main():
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
  print(json.dumps(dict(forwards=0,widths=[4,32],starts=['inherited','random'],steps=1000)));return
 import torch
 ...
 start=time.monotonic();out=P/'WIDE_NATIVE_QUARTIC_LONG_V1.json';assert not out.exists();control=json.loads((P/'EMPIRICAL_QUARTIC_DICTIONARY_CONTROLS_V1.json').read_text());long=json.loads((P/'EMPIRICAL_QUARTIC_DICTIONARY_LONG_V1.json').read_text());assert all(r['validation_error']<.01 for r in long['warm_start_controls']);assert max(max(r['solve_error'],r['gradient_error']) for r in control['controls'])<1e-9
