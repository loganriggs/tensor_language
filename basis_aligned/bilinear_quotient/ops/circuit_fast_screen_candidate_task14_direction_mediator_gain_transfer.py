#!/usr/bin/env python3
"""Frozen fourth-corpus authority for the six-scalar mediator program."""
...
import circuit_fast_screen_candidate_task14_cardinality_prototype_transfer as prior


SCHEMA = "task14_direction_mediator_gain_transfer_authority_v1"
TASK_ID = "subject_verb.number_agreement"
CAPABILITY_ID = "subject_verb.number_agreement.direction_mediator_gain_transfer_capability_v1"
CAUSAL_CANDIDATE_ID = "subject_verb.number_agreement.direction_mediator_gain_fourth_corpus_transfer_v1"
SUBJECT_POSITION = 8
ROLES = ("recipient", "opposite_same_lemma", "same_number_different_lemma")
TEMPLATES = (
    ("under_beyond", "Under the {a1} beyond the {a2}, the {subject}"),
    ("beyond_under", "Beyond the {a1} under the {a2}, the {subject}"),
)
NOUN_PAIRS = (
    ("guardian", "guardians"), ("analyst", "analysts"),
    ("mechanic", "mechanics"), ("detective", "detectives"),
    ("diplomat", "diplomats"), ("scientist", "scientists"),
    ("technician", "technicians"), ("designer", "designers"),
    ("developer", "developers"), ("programmer", "programmers"),
    ("researcher", "researchers"), ("inspector", "inspectors"),
    ("instructor", "instructors"), ("supervisor", "supervisors"),
    ("referee", "referees"), ("astronaut", "astronauts"),
)
EXPECTED_AUTHORITY_SHA256 = "5a64e8c09b20623350ad95681d496658533a31f04041cc145a7469fccd5f20b3"
...
def _build_unvalidated():
    ...
    for group in range(len(NOUN_PAIRS)):
        ...
        lexical_group = (0 if recipient_number == 0 else half) + (within + 1) % half
        a1_group, a2_group = (group + 4) % len(NOUN_PAIRS), (group + 9) % len(NOUN_PAIRS)
        ...
        for template_id, template in TEMPLATES:
            subjects = {"recipient": NOUN_PAIRS[group][recipient_number], "opposite_same_lemma": NOUN_PAIRS[group][1-recipient_number], "same_number_different_lemma": NOUN_PAIRS[lexical_group][recipient_number]}
            endpoints = {}
            for role in ROLES:
            ...
            rows.append({"schema": SCHEMA, "task_id": TASK_ID, "capability_id": CAPABILITY_ID, "causal_candidate_id": CAUSAL_CANDIDATE_ID, "phase": "PROSPECTIVE", "group_number": group, "row_id": canonical_sha256(identity), "template_id": template_id, "direction_id": direction, "attractor_state": list(attractor_state), "subject_position": SUBJECT_POSITION, "endpoints": endpoints})
    return rows


def _prior_material():
    vocabulary, prompts, tokens = prior._prior_material()
    vocabulary.update(word for pair in prior.NOUN_PAIRS for word in pair)
...
def validate_rows(rows, verify_hash=True):
    ...
    if sorted(Counter((x["direction_id"], x["template_id"]) for x in rows).values()) != [8] * 4:
        raise ValueError("balance changed")
    vocabulary, prior_prompts, prior_tokens = _prior_material()
    forms = {word for pair in NOUN_PAIRS for word in pair}
    if len(forms) != 32 or forms & vocabulary or any(len(old_task14.ENCODING.encode(" " + word)) != 1 for word in forms):
    ...
    for row in rows:
        ...
        for alternate in (opposite, lexical):
            if [i for i, pair in enumerate(zip(recipient["ids"], alternate["ids"])) if pair[0] != pair[1]] != [SUBJECT_POSITION]:
...
def compile_plan():
    rows = build_rows()
    return {"schema": SCHEMA, "task_id": TASK_ID, "capability_id": CAPABILITY_ID, "causal_candidate_id": CAUSAL_CANDIDATE_ID, "row_count": len(rows), "prompt_endpoints": 96, "templates": [x[0] for x in TEMPLATES], "authority_sha256": canonical_sha256(rows)}
