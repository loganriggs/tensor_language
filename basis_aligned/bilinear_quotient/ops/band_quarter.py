"""QUARTER-DEFICIENCY (rung 261): fine band missing at only heads hd%4==0 (18 of 72 tail heads).

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. Half-deficiency costs ~98% of full
(S2358). If even a QUARTER of the heads deficient still costs ~full, the cancellation mechanism is
maximally delicate - broken by any deficiency anywhere.
REGISTERED PREDICTIONS (arms: THIS vs full-band 0.0553/11 and half-deficient 0.0544/11):
  (a) EXTREME GLOBALITY: census >= 0.045.
  (b) valid >= 11.
  (c) SANITY: census in [0.01, 0.12].
NULL: partial additivity emerges below half (census <= 0.035). PRICE: probe. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: quarter-deficiency band test')
    raise SystemExit(0)
import torch
...
OUT=PT+'band_quarter_results.json'
...
if __name__=='__main__':
    ...
    print(f"(a) EXTREME GLOBALITY: census {agg:+.4f} >= 0.045: {'HELD' if pa else 'FAILED'}")
