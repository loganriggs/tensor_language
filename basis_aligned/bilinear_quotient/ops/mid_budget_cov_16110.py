# BISECTING THE RIDGE -- the ~129.7M iso-cost line, coverage 16110, rank 200.
#
# §1864 found the coverage term CHANGES SIGN with budget. At 230.087M, moving from 5,419 to 16,110
# covered types gains 0.056 / 0.079 / 0.071 nats (the affordable rank at 16,110 is 361). At 65.950M the
# same move COSTS 0.044 / 0.031 / 0.022 (affordable rank 97). So the threshold rank for that step lies
# between **97 and 361**, and §1864 could say no more than that.
#
# This bisects it. At a budget of about 129.7M the affordable ranks are 200 at
# 16,110 types and 525 at 5,419. Measuring both ends of that line places the
# ridge on one side or the other of rank 200.
#
# Docstring and predicate block written in ONE edit (LESSONS 63); costs COMPUTED from §1754's model
# (§1858); a single finite rank in TRANKS (LESSONS 64); the filename tokenises against the banner
# (LESSONS 56's check caught the previous naming).
#
# ROLES. All three. DISCOVERY ONLY. Rung 3: the open question §1864 ends on.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39:
#   pred_a THE BUILD IS SOUND AND ON BUDGET: cost computed from §1754's model is within 2% of 129.7M and
#          the covered CE lies strictly above its own ceiling. If FALSE the point is not on the intended
#          iso-cost line and cannot be compared with its partner.
#   pred_b IT BEATS THE 65.950M LINE'S BEST: all-position CE is below §1849's PUBLISHED rank-256@5,419
#          figures of 6.06004 / 6.03311 / 6.04238 on all three roles -- a 2x budget should buy something
#          wherever it sits on the ridge. If FALSE, doubling the budget along this coverage buys nothing,
#          which would be a stronger statement about the ridge than its location.
#   pred_c AND IT IS BETWEEN THE TWO PUBLISHED LINES: all-position CE lies between §1849's rank-256@5,419
#          (6.06004 on skip7000) and §1861's rank-361@16,110 (5.95599), i.e. a mid budget buys mid
#          fidelity. If FALSE the fidelity/cost relation is not monotone in budget along this axis, which
#          no section has yet suggested and would need its own investigation.
#   pred_d CONTROLS: coverage is exactly 16110 from fit rows [0:480]; the ceiling is finite and above
#          live on every role; the CE above the ceiling is positive as it must be at finite rank.
...
OUT = PT + 'ops/mid_budget_cov_16110_results.json'
...
TRANKS = (200,)          # None = full rank; §1786/§1813 use this ladder
...
def main():
    ...
    print(f'MID BUDGET COV | 16110 types, rank 200, 129.7M line | '
    ...
    pa = (abs(cost - MIDBUDGET_M) / MIDBUDGET_M < 0.02
          and all(above[e] > 0 for e in roles2))
    pb = all(v > 0 for v in vs_ref.values())
    pc = S1861_R361_16110_7000 < allpos['skip7000'] < S1849_R256_5419['skip7000']
    pd = (NFULL == 16110 and all(ceilv[e] > livev[e] for e in roles2)
          and all(above[e] > 0 for e in roles2))

    print(f'\\n  MID-BUDGET point: {NFULL} covered types, rank {R}, {cost:.3f}M '
          f'(line {MIDBUDGET_M}M)', flush=True)
    for e in roles2:
        print(f'    {e:10s} ceiling {ceilv[e]:.5f}  covered {curve[R][e]["cov"]:.5f} '
              f'(+{above[e]:.5f})  ALLPOS {allpos[e]:.5f}', flush=True)
    print(f'\\n  on budget and sound -> {pa}  cost {cost:.3f}M vs {MIDBUDGET_M}M', flush=True)
    print(f'  beats the 65.950M line best (§1849 r256@5419) -> {pb}  ' + '  '.join(
        f'{e} {vs_ref[e]:+.5f}' for e in roles2), flush=True)
    print(f'  and sits BETWEEN the two published lines -> {pc}  '
          f'{S1861_R361_16110_7000} < {allpos["skip7000"]:.5f} < '
          f'{S1849_R256_5419["skip7000"]}', flush=True)
    print(f'  coverage {NFULL}, ceiling above live -> control {pd}', flush=True)

    json.dump({'run': 'mid_budget_cov_16110', 'n_covered': NFULL, 'rank': R,
               'cost_M': cost, 'allpos': allpos, 'above_ceiling': above, 'vs_ref': vs_ref,
               'ceiling': ceilv, 'live': livev,
               'predictions': {'pred_a_on_budget_and_sound': bool(pa),
                               'pred_b_beats_low_budget_best': bool(pb),
                               'pred_c_between_published_lines': bool(pc),
