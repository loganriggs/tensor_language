"""ATTENTION-SIDE KNOCKOUT TRANSFER (a14, rung 269): the tail-attn replication.

CONVENTION (S2135): per-position dCE = CE(mixed config with a14 mean-ablated) - CE(real model); LOWER
IS BETTER. All four MLP knockouts transferred (S2364-S2367). a14 is an ATTENTION component (battery
own-mean 0.436) whose module is REPLACED by the compiled path (tail permanent hooks) - the ablation hook
registers at the census site, after all install hooks, so it overrides the compiled output. Small-signal
regime predicted for collateral.
REGISTERED PREDICTIONS:
  (a) OWN-FAMILY TRANSFERS: median member/ref ratio over a14-top circuits in [0.7, 1.6].
  (b) SMALL-REGIME COLLATERAL: Spearman in [0.45, 0.85).
  (c) SANITY: ablated census in [0.08, 0.6].
NULL: attention knockouts do not transfer through the compiled pattern path (own ratio outside [0.5, 2]).
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: attn knockout transfer a14')
    raise SystemExit(0)
import torch
...
OUT=PT+'ko_transfer_a14_results.json'
...
if __name__=='__main__':
    ...
    def _mucap(mo,i_,o_):
        MUCAP['s']+=o_[0].detach().float().reshape(-1,D).sum(0); MUCAP['n']+=o_[0].reshape(-1,D).shape[0]
    _hmu=m.transformer.h[14].attn.register_forward_hook(_mucap)
    with torch.no_grad():
    ...
    _own=sorted(r['member_absdce']/max(r['ref'],1e-9) for r in rows if _topK.get(r['tag'])=='a14')
    _omed=_own[len(_own)//2]
    _MR=json.load(open(PT+'removal_matrix_results.json'))['matrix']['a14']
    ...
    print(f"(b) small-regime collateral - Spearman {_rhoK:.3f} in [0.45, 0.85); margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) ablated census {agg:+.4f} in [0.08, 0.6]; era rho {rho:.4f}: {'HELD' if pc else 'FAILED'}")
