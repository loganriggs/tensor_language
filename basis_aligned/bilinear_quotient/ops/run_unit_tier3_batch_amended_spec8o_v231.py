#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the fifteenth eight spec-authored behaviours, plus one instrument.
"""v231: the AMENDED tier-3 battery (v196/v197 protocol, unchanged) over the FIFTEENTH batch of spec-authored behaviours (eight) -- plus one instrument.
...
(+instrument each) at ~30-45 GPU-s per behaviour. v225 and v229 were the two ZERO batteries and both were world-knowledge INFERENCE
readouts (container shape, temporal order, level change, cardinality, generation, result state): every A1 full-set ceiling was <= 0.24 CE,
against 0.96 for the grammatical instrument. Standing reading: bilin18 reads GRAMMATICAL cues at 10-40x the CE of any inference. This batch
therefore returns to grammatical cues and, per the v225/v227 lesson (variant frames of a known readout lose row 5; NEW readout tokens keep it),
opens EIGHT readout token pairs never screened before, authored through `circuit_fast_screen_behaviour_spec`, single-cue swap, matched final
token, cue never final, 32 A1/A2/P/C rows, ABAB directions:
degree_synthetic_periphrastic_comparative (`The pilot near the lantern seemed tall/careful, of course, and the sailor seemed even` -> taller/more);
degree_synthetic_periphrastic_superlative (`The pilots near the lantern were tall/careful, of course, and the sailor was the` -> tallest/most;
  sibling pair: NEW degree-morphology family -- the degree family reads much/very, than/as, best/better frames, never the -er/more choice);
countability_few_little (`The crates/water near the lantern ran low, of course, so the pilot took only a` -> few/little);
countability_number_amount (`The pilot checked the crates/water near the lantern, of course, and was struck by the sheer` -> number/amount;
  both countability family, whose screened readouts are many/much, water/crates, crate/water);
possessive_number_its_their (`The crate/crates near the lantern slipped off the cart, of course, and landed on` -> its/their; number family,
  first inanimate possessive);
polarity_any_some (`The pilot near the lantern never/often brought, of course,` -> any/some; polarity family: anything/something, too/either);
modal_ellipsis_can_must (`The pilot near the lantern can/must lift the crate, of course, and so` -> can/must; NEW family -- modal IDENTITY
  under ellipsis; the modal family reads complement type, bare/to and bare/participle, never the modal itself);
relative_animacy_whom_which (`The pilots/crates near the lantern, of course, all of` -> whom/which; relative-animacy family: who/which, who/whom).
Capability printed BEFORE the run on CPU (`g.prepare(valid_only=True)`, 32 rows per family, 18:52 UTC): ZERO A1 and ZERO A2 rows dropped
on all eight, no repairs, and the margins are back at grammatical strength (mean base/donor on the donor axis) A1:
degree_synthetic_periphrastic_comparative -5.90/+5.62 (the largest margin of any screened behaviour), polarity_any_some -3.18/+3.25,
modal_ellipsis_can_must -3.05/+2.77, degree_synthetic_periphrastic_superlative -2.76/+2.31, relative_animacy_whom_which -2.71/+2.79,
possessive_number_its_their -1.76/+1.75, countability_few_little -0.51/+0.35, countability_number_amount -0.30/+0.31 (the two countability
readouts are the weak cells -- count/mass is half lexical -- and are the registered expectation for the misses);
A2: comparative -5.65/+5.30, polarity -3.52/+3.64, superlative -2.97/+2.59, whom_which -2.03/+2.62, modal -1.69/+1.85,
its_their -1.52/+1.31, few_little -0.05/+0.42, number_amount -0.21/+0.15. C (shared night frame) -1.43/+0.93, 2 dropped, as always.
Protocol identical to v197/v201/v204/.../v229 (greedy pool 40, target 0.88, min_gain 0.005, max 30; row 2 = MIN over the two held directions
...
REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 4 of the 8 new behaviours pass all four rows (grammatical batches v204-v223: 3-6 of 8; the two
                           inference batches 0/8 and 0/7). prior 55%
  ...
  pred_d_row4           -> constrained rank-1 own-C CE UB975 <= 0.01 on at least 5 of 8 (v221 5/8, v223 4/8, v227 5/8).   prior 50%
  pred_e_row5_amended   -> A2-own / A2 ceiling within [0.5, 1.4] with LB975 > 0 on at least 5 of 8 (new readouts: v227 8/8). prior 60%
...
Accounting: four-row passes here join the one-protocol standing. Six of the eight have screened family members (countability x2, number/
possessive, polarity, relative animacy) or a sibling in this batch (the degree-morphology pair) and so count only after a separability
rung under the full family control set; modal_ellipsis_can_must is family-less and counts as a singleton (verb_preposition precedent).
...
Smoke: V231_SMOKE=<out.json> (CPU, V231_SMOKE_ROWS=4 per split, V231_SMOKE_NAMES=modal_ellipsis_can_must; pool 3/max 3, steps 5).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec8o_v231_result.json"
...
NAMES = {"degree_synthetic_periphrastic_comparative": "degree_synthetic_periphrastic_comparative",
         "degree_synthetic_periphrastic_superlative": "degree_synthetic_periphrastic_superlative",
         "countability_few_little": "countability_few_little", "countability_number_amount": "countability_number_amount",
         "possessive_number_its_their": "possessive_number_its_their", "polarity_any_some": "polarity_any_some",
         "modal_ellipsis_can_must": "modal_ellipsis_can_must", "relative_animacy_whom_which": "relative_animacy_whom_which",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec8o_v231", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V231_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V231_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V231_SMOKE_NAMES", "modal_ellipsis_can_must").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec8o_v231", "candidate_id": "corpus.unit_tier3_batch_amended_spec8o_v231", "bars": BARS,
