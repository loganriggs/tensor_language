"""THE SUPPRESSOR AS AN OPPONENT PROCESS (rung 78; math review 0140).

On the skip-1416 config (attentions and m16 all real), capture attn14/attn16 and m16 outputs over FR. If m16
implements read-minus-brake by DIRECT logit opposition, its unembedding-projected output should be
anti-aligned with the attentions' on ind positions and ~orthogonal elsewhere. NULL: the suppression is routed
through later blocks (no direct logit opposition).

REGISTERED PREDICTIONS (unembedding-space cosines, per-position, median over samples of 2000):
  (a) OPPOSITION ON RETRIEVAL: median cos(U y_m16, U (y_a14+y_a16)) <= -0.30 on ind positions.
  (b) SPECIFIC: |median cos| <= 0.15 on a non-ind control sample.
  (c) REPRODUCTION: the base reproduces S2156 (skip-1416 FR L2_F 2.4230 within 0.01).
  TRIPWIRE: capture length must equal the position count.

Writes m16_opponent_results.json. Self-reviewed. RUNG-77 HEADER FOLLOWS.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['backup_pair_cca_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; m16 opponent-process cosines on skip-1416 (alignment rung)")
    raise SystemExit(0)
import torch
...
OUT=PT+'m16_opponent_results.json'
...
def main():
    ...
    if SEL.get('clsdmg'):
        ...
        if SEL.get('cap1416'):
            ...
            SEL['ym']=[]
            _hc.append(m.transformer.h[16].mlp.register_forward_hook(
                lambda mo,i_,o_: SEL['ym'].append(o_.detach().reshape(-1,D).float().cpu())))
...
if __name__=='__main__':
    ...
    print('SINGLE ARM: skip-1416, capturing attn14/attn16/m16 outputs over FR',flush=True)
    r_b=main()
    clsflat=SEL['clsflat']
    Y14=torch.cat(SEL['y14']); Y16=torch.cat(SEL['y16']); YM=torch.cat(SEL['ym'])
    if YM.shape[0]!=len(clsflat):
        raise SystemExit(f'INSTRUMENT FAIL: capture length {YM.shape[0]} != {len(clsflat)}')
    import statistics as stt
    IND=CLS.index('ind')
    gen=torch.Generator().manual_seed(78)
    im=(clsflat==IND).nonzero().squeeze(1)
    im=im[torch.randperm(len(im),generator=gen)[:2000]]
    non=(clsflat!=IND).nonzero().squeeze(1)
    non=non[torch.randperm(len(non),generator=gen)[:2000]]
    WU=m.lm_head.weight.detach().float().cpu()   # (V, D)
    def med_cos(idx):
        cs=[]
        for k0 in range(0,len(idx),200):
            ii=idx[k0:k0+200]
            um=YM[ii]@WU.T
            ua=(Y14[ii]+Y16[ii])@WU.T
            c=torch.nn.functional.cosine_similarity(um,ua,dim=1)
            cs.extend(c.tolist())
        return stt.median(cs)
    ci=med_cos(im); cn=med_cos(non)
    pa=ci<=-0.30
    pb=abs(cn)<=0.15
    pc=abs(r_b['L2_F']-2.4230)<=0.01
    res={'base':{'L2_F':r_b['L2_F']},'median_cos_ind':round(ci,4),'median_cos_nonind':round(cn,4),
         'pred_a_opposition':bool(pa),'pred_b_specific':bool(pb),'pred_c_reproduces_s2156':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'median unembedding-space cos(m16, a14+a16): ind {ci:+.3f} | non-ind {cn:+.3f}')
    print(f"(a) ind cos {ci:+.3f} <= -0.30: {'HELD' if pa else 'FAILED'}")
    print(f"(b) |non-ind cos| {abs(cn):.3f} <= 0.15: {'HELD' if pb else 'FAILED'}")
    print(f"(c) base FR L2_F {r_b['L2_F']} vs 2.4230 (tol 0.01): {'HELD' if pc else 'FAILED'}")
