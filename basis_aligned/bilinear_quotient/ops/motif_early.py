"""QUARTER TEST (rung 241): only the EARLY motif blocks (2-5) exact, everything else r64.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The motif half carries the
certificate bottleneck (S2335); is it the early motifs (local grammar onset) or late motifs? Blocks 2-5
exact, 6-9 + tail r64.
REGISTERED PREDICTIONS (arms: THIS vs r64 0.0852/7 and motif-exact 0.0608/9):
  (a) EARLY MOTIFS CARRY MOST OF IT: census <= 0.075.
  (b) valid >= 8 (at least one of the two motif-half certificates comes from blocks 2-5).
  (c) SANITY: census in [0.01, 0.12]; inert tripwires vs motif-exact AND r64 receipts.
NULL: the motif need is spread evenly within the half (census >= 0.073 AND valid <= 7).
PRICE: r64 config + blocks 2-5 patterns at full rank. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: early-motif quarter test')
    raise SystemExit(0)
import torch
...
OUT=PT+'motif_early_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=64; SEL['qk_rmap']={li:128 for li in range(2,6)}; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    pb=nv>=8
    pc=0.01<=agg<=0.12
    if abs(agg-0.0608)<1e-4 or abs(agg-0.0852)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: early-motif map inert')
