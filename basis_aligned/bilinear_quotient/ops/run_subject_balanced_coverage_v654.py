...
"""Width8 balanced response spaces with matched preposed/postposed calibration.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_joint_chain_v654_result.json'
...
def main():
    plan=dict(prefix_calls=16,suffix_calls=20,calibration_rows=256,evaluation_rows=48,width=8,
              additional_endpoint_chain_batches=8,basis_selection='snapshot-balanced response/finite-reader biorthogonal spaces',controls=['dense','random_seed647'],
    ...
    for l in range(7):
        ...
        reads=torch.cat([q[l] for q in reader_samples]).cpu()
    ...
    a=max(checks)<=1e-4 and max(absolute)<=1e-4 and max(reader_checks)<=1e-4 and max(encoder_checks)<=1e-8 and counts==dict(prefix_calls=16,suffix_calls=20)
    ...
    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,
        ...
        max_encoder_duality_error=max(encoder_checks),max_reader_closure_absolute_error=max(reader_checks),max_local_relative_error=max(checks),max_local_absolute_error=max(absolute),fixed_values=values,
        ...
        scope='Balanced dual-space width8 with broader256row calibration, 9216 extra initial-encoder values charged, same calibration/evaluation data; final projection is oracle. Data-informed residual basis, no native circuit adoption.')
