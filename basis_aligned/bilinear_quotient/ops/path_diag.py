"""LOCALIZATION DUMP (rung 286): native-vs-replaced outputs captured in situ at every tail block.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2384: the +0.0520 is deterministic
recompute-structure error. The tail hook holds the NATIVE module output (ground truth) at the moment it
replaces it: this run (full rank, values real) saves (y_native, y_replaced) pairs plus z slices per tail
block on the first hooked batch, and scores the per-block output relative error in-script; offline
analysis then walks the sub-steps.
REGISTERED PREDICTIONS:
  (a) DIVERGENCE EXISTS at output level: max per-block rel err >= 0.01.
  (b) UNIFORM across blocks: max/min rel err <= 5 (one shared structural cause).
  (c) INSTRUMENT: all 8 tail blocks captured, finite.
NULL: rel errs < 0.005 everywhere (the damage enters NOT through the tail replacement outputs -
then the motif path or the merge is the carrier and the hunt moves there). PRICE: probe. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: localization dump (native vs replaced)')
    raise SystemExit(0)
import torch
        ...
        if SEL.get('qk_tail'):
            for _li in range(10,18):
                ...
                def _th(mo_,args,out,at=_at,li=_li):
                    ...
                    if li not in SEL.setdefault('_diag',{}):
                        zn=z.permute(0,2,1,3) if z.shape[1]==9 else z
                        SEL['_diag'][li]={'ynat':y[:2].detach().float().cpu(),
                                          'yrep':ynew[:2].detach().float().cpu(),
                                          'zq':zq[:1,:, :64].detach().float().cpu(),
                                          'zh':zn[:1,:, :64].detach().float().cpu() if zn.dim()==4 else None}
                    return (ynew,v1r)
...
if __name__=='__main__':
    ...
    cev=SEL['cev']
    torch.save(SEL.get('_diag',{}),PT+'path_diag.pt')
    print(f"diag saved: {len(SEL.get('_diag',{}))} blocks",flush=True)
    ...
    _errs={li:float((d['yrep']-d['ynat']).norm()/(d['ynat'].norm()+1e-9)) for li,d in SEL.get('_diag',{}).items()}
    print('  per-block output rel err:',{li:round(e,4) for li,e in sorted(_errs.items())},flush=True)
    _mx=max(_errs.values()) if _errs else 0.0; _mn=min(_errs.values()) if _errs else 0.0
    pa=_mx>=0.01
    pb=(_mx/max(_mn,1e-9))<=5.0
    pc=len(_errs)==8 and all(e==e and e<10 for e in _errs.values())
    ...
    print(f"(a) DIVERGENCE EXISTS - max block rel err {_mx:.4f} >= 0.01: {'HELD' if pa else 'FAILED'}")
    print(f"(b) UNIFORM - max/min {_mx/max(_mn,1e-9):.2f} <= 5; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) 8 blocks, finite: {len(_errs)}: {'HELD' if pc else 'FAILED'}")
