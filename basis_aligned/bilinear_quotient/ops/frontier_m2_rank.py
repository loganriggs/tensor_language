"""M2 RESIDUAL-RANK CAPACITY CURVE (rung 107): does the table grammar have a capacity path at its worst site?

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows; LOWER IS BETTER.
S2199: the front-table error is high-rank context signal; the current tableres residual is 64-dim (quadfeat ->
64 SVD dims of the residual). Here m2's replacement is refit in its exact S2198 prefix frame
(act = [a0,m0E,a1v,m1]) at residual ranks r in {0,16,64,256,512} and each variant is installed ALONE
(single-site arm, everything else real) on the census rows with certificates.

REGISTERED PREDICTIONS (arms named agg(r) = aggregate dCE of the single-m2 variant at rank r):
  (a) RANK WALL: agg(0) - agg(64) >= 2 x [agg(64) - agg(512)] (returns diminish; the S2118 wall's analogue).
  (b) THE GRAMMAR CANNOT BUY ITS WAY OUT: agg(512) >= 0.5 x agg(64) (8x residual rank recovers less than half
      of m2's single-site damage).
  (c) SANITY/MONOTONE: agg(0) >= agg(16) >= agg(64) (tol 0.02) and all aggs in [0.05, 1.5].
NULL: capacity path OPEN (agg keeps halving with rank - repair = bigger residual, no new grammar needed).
PRICE: rank-512 residual at one site = 512 x (1152 + ~700) extra values vs rank-64. Tripwire: INSTRUMENT FAIL
if all rank arms are identical (pstdev < 0.005). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: m2 residual-rank capacity curve')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_m2_rank_results.json'
...
def main():
    ...
    if SEL.get('m2_rank'):
        _ROWS=SEL.get('ext_rows',FR)
        _act=['a0','m0E','a1v','m1']
        _Y2,_X2,_ids2=runA(_act,m.transformer.h[2].mlp)
        _tb=fit_table(_Y2,_ids2)
        _Rr=_Y2-_tb[_ids2].float()
        _,_,_Vh=torch.linalg.svd(_Rr[:30000],full_matrices=False)
        _ft=quadfeat(_X2,2)
        _lam=1e-2*len(_X2)
        _M=_ft.T@_ft+_lam*torch.eye(_ft.shape[1],device=DEV)
        cur['clsmap']=classify2(_ROWS).to(DEV)
        _arms={}
        for _r in (0,16,64,256,512):
            if _r==0:
                _P=torch.zeros(D,1,device=DEV); _A=torch.zeros(_ft.shape[1],1,device=DEV)
            else:
                _P=orth(_Vh[:_r].T)
                _A=torch.linalg.solve(_M,_ft.T@(_Rr@_P))
            S['m2R']=('tableres',2,_tb,_A,_P)
            _arms[f'rank_{_r}']=evalV(_ROWS,_ROWS.shape[0],['m2R'],ML).detach().cpu()
            print(f'  m2 residual rank {_r} done',flush=True)
        del cur['clsmap']
        SEL['front_result']=_arms
...
if __name__=='__main__':
    ...
    SEL['m2_rank']=True; SEL['ext_rows']=CROWS
    print('ARMS: m2 single-site tableres at residual ranks 0/16/64/256/512 - census rows',flush=True)
    main()
    A=SEL['front_result']
    out={}
    ...
    if stt.pstdev(aggs.values())<0.005: raise SystemExit('INSTRUMENT FAIL: rank arms identical')
    g_low=aggs[0]-aggs[64]; g_high=aggs[64]-aggs[512]
    pa=g_low>=2*g_high
    pb=aggs[512]>=0.5*aggs[64]
    pc=(aggs[0]>=aggs[16]-0.02 and aggs[16]>=aggs[64]-0.02
        and all(0.05<=a<=1.5 for a in aggs.values()))
    res={'arms':out,'gain_0_to_64':round(g_low,4),'gain_64_to_512':round(g_high,4),
         'convention':'per-position dCE = CE(config) - CE(real model) on census rows; lower is better',
         'pred_a_rank_wall':bool(pa),'pred_b_no_capacity_path':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"gains: 0->64 {g_low:+.4f} | 64->512 {g_high:+.4f}")
    print(f"(a) rank wall {g_low:+.4f} >= 2 x {g_high:+.4f}: {'HELD' if pa else 'FAILED'}")
    print(f"(b) agg(512) {aggs[512]:+.4f} >= 0.5 x agg(64) ({0.5*aggs[64]:+.4f}): {'HELD' if pb else 'FAILED'}")
    print(f"(c) monotone + all in [0.05, 1.5]: {'HELD' if pc else 'FAILED'}")
