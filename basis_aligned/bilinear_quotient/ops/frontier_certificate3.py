...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_certificate2_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: rung-100 artifact present; all-motifs-real certificate")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_certificate3_results.json'
...
if __name__=='__main__':
    ...
    print('ARM: all-motifs-real config evaluated per-position on the census rows',flush=True)
    r_cfg=main()
    ...
    fails=[r for r in rows if not r['valid']]
    TAILLOC={t for t,v in BATC.items() if v['best_mean'] in ('a16','m16','a14')}
    tailfrac=sum(1 for r in fails if r['tag'] in TAILLOC)/max(len(fails),1)
    pa=nvalid>=30
    pb=tailfrac>=0.6
    pc=0.3<=agg<=5.0
    fr_ret=tailfrac
    res={'aggregate_mean_dce':round(agg,4),'n_circuits':len(rows),'n_valid':nvalid,
         'failing_tail_fraction':round(fr_ret,3),
         'circuits':rows,
         'convention':'per-position dCE = CE(S2144 config) - CE(real model) on the census rows; tau = 0.5 x battery ref',
         'pred_a_large_recovery':bool(pa),'pred_b_residue_is_tail':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'aggregate dCE {agg:+.4f}; valid {nvalid}/{len(rows)}; failing tail-localized fraction {fr_ret:.2f}')
    print(f"(a) valid {nvalid} >= 30: {'HELD' if pa else 'FAILED'}")
    print(f"(b) failing tail fraction {fr_ret:.2f} >= 0.6: {'HELD' if pb else 'FAILED'}")
