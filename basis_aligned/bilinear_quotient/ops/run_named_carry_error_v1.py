#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_replay pred_b_inherited_mean pred_c_frozen
...
def main():
 ...
 from named_carry_error_terms import split,control,projected_carry
 from pairwise_reader_graph import source_reads
 plan=json.loads((P/'NAMED_CARRY_ERROR_PLAN_V1.json').read_text())
 ...
 for version in (1,2):
  tokens=torch.load(P/f'FRONTIER_FRESH_TOKENS_V{version}.pt',weights_only=True)['stdlib'];mapping=torch.load(P/f'FRONTIER_FRESH_DONORS_V{version}.pt',weights_only=True)['stdlib'];documents=json.loads((P/f'FRONTIER_FRESH_PLAN_V{version}.json').read_text())['recipient_documents']['stdlib'];cache={k:[] for k in ('z','h','m','carry')};labels=[]
  for row in tokens:
   ...
   carry=projected_carry(model,raw,A[:,2]).flatten(0,1)[16:]
   for key,value in (('z',z),('h',h),('m',m),('carry',carry)):cache[key].append(value)
  ...
  for offset in range(0,len(valid),96):
   ...
   for z,hh,mm in ((cache['z'][gi],h,m),(cache['z'][gd],hybrid,md)):
    ...
    for candidate,reads in (('graph',source_reads(z,graph)[...,4:6]),('separate',baseline_reads(z,baseline['2']))):
     terms=split(hh@A[:,2],scale[:,0],alpha[2],beta[2],true_reads,reads,cache['carry'][gi]);term_values[candidate].append(terms);actual=values[candidate][-1]-native[:,2];term_checks.append(float((terms.sum(-1)-actual).norm()/actual.norm().clamp_min(1e-30)))
 ...
 for family in ('natural','hybrid','change'):
  for cohort in ('all','continuation','spaced_word'):
   ...
   for candidate in ('graph','separate'):
    ...
    gram=sum((torch.tensor(r['gram'],dtype=torch.float64) for r in rows),torch.zeros(10,10,dtype=torch.float64))
    ...
    result[candidate]=dict(gram=gram.tolist(),total_energy=energy,diagonal_energy=float(gram.diag().sum()),cross_energy=float(gram.sum()-gram.diag().sum()),term_means=(sum((torch.tensor(r['term_sums'],dtype=torch.float64) for r in rows),torch.zeros(10,dtype=torch.float64))/sum(r['sites'] for r in rows)).tolist())
   summaries.append(result)
 failures=[r for r in summaries if r['cohort']=='spaced_word' and r['family'] in ('natural','hybrid')]
 pred=dict(pred_a_replay=max(term_checks+energy_replay)<1e-8,pred_b_inherited_mean=all(abs(r['graph']['term_means'][0]-r['separate']['term_means'][0])>=max(abs(r['graph']['term_means'][i]-r['separate']['term_means'][i]) for i in range(1,4)) for r in failures),pred_c_frozen=True)
 out=dict(plan=plan,records=records,summaries=summaries,source_replay=max(checks),term_replay=max(term_checks),energy_replay=max(energy_replay),predictions=pred,seconds=time.monotonic()-start,term_order=['preceding_residual_times_second_error','embedding_skip_times_second_error','attention17_times_second_error','mlp16_bias_times_second_error','fp32_closure_times_second_error','source_times_second_error','alpha_times_second_error','first_error_times_source','beta_times_first_error','error_product'])
 (P/'NAMED_CARRY_ERROR_V1.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(dict(predictions=pred,summaries=summaries)),flush=True)
