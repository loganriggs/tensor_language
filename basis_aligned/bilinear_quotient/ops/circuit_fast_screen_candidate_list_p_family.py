...
PRIOR_ART = ROOT / "circuits/fast_screen_list_p_family_prior_art.json"
CONTROL_PATH = ROOT / "pending_opener_controls_rung537.json"

SCHEMA = "circuit_fast_screen_candidate_v1"
TASK_ID = "numbered_list.p_family_discriminator"
...
FAMILY_MAP = {
    # A1/A2/P are UNCHANGED from the 21:15Z screen -- same frozen rows, same constructions. The ONLY change
    # is C, and that is the whole experiment.
    "list_two_line_state_shift": ("A1", "two_line"),
    "list_three_line_state_shift": ("A2", "three_line"),
    # P SWAPPED, and that is the whole experiment. The 02:15Z screen used `list_surface_preserved`
    # (step-1 lists, item words shuffled) and attn:08 passed at P 0.0224. `list_step_two_conflict` is the
    # same answer-preserving shape over STEP-2 lists (21, 23, 25 -> 26). For the sibling numeric-sequence
    # behaviour the P family flipped a verdict (digit 0.2517 vs word 0.0560), so this hypothesis is known
    # to be sensitive and the list claim has only ever been tested against one P family.
    "list_step_two_conflict": ("P", "step_two_conflict"),
    # C was `sequence_digit_state_shift`, another NUMERAL behaviour, and it recovered 0.959 at the top site
    # -- the sole reason that screen returned no_selective_causal_site. Here C is the bracket
    # non-opener punctuation control, a NON-numeral behaviour, which recovered 0.034 in the 01:46Z positive
    # control. If the null was caused by choosing a numeral control, this run returns a selective site.
...
TASK_SPEC = battery.BatteryTaskSpec(
    ...
    transforms=(
        ...
        battery.TransformSpec("P", "step_two_conflict_item_rewrite", False, "invariant"),
...
def _checks(row, base_foil, donor_foil, transform_id):
    ...
    return {
        ...
        # Derived from TASK_SPEC rather than hardcoded: C's `answer_changes` differs BETWEEN SCREENS -- it
        # was True when C was another numeral behaviour, and is False here because the bracket punctuation
        # control preserves its answer. Hardcoding it rejected every C row on the first build.
    ...
    for direction_id in ("base_to_donor", "donor_to_base"):
        for index in range(groups):
            ...
            for transform_id, (family, construction_id) in (
                ...
                if transform_id == "C":
                    # the bracket controls file carries one `answer_id`, identical on both sides;
                    # its foil is the other closer (the endpoint vocabulary there is {")"=8, '"'=1})
                    b_ans = d_ans = int(row["answer_id"])
                    row = dict(row, base_answer_id=b_ans, donor_answer_id=d_ans,
