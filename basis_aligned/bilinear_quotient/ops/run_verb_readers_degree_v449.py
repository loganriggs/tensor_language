#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_closure pred_b_noun_source_carries_a_quarter pred_c_verb_readers_are_value_copies pred_d_third_order_small pred_e_both_heads_value_led
"""Degree expansion of the VERB readout's heads (v449). v448: heads 11.3 and 7.8 carry the noun's number to a distant verb. Are they value copiers like
the pronoun readers (v401-v410), or does their attention pattern carry number? The exact eight-term expansion of each head's noun-source write on the
agreement direction u_v = W_U[are] - W_U[is] + W_U[were] - W_U[was], on the 79 distant natural rows truncated at their verb slot (aligned pairs by the
in-place noun-number swap).
PREDICTIONS (scored as written; failures preserved; priors from v401 / v405)
    ...
    pred_b_noun_source_carries_a_quarter  for 11.3 the noun-source term carries >= 0.25 of the head's total write contrast on u_v. Prior: unsure
    pred_c_verb_readers_are_value_copies  for 11.3 the value-only term is >= 0.60 of the noun-source contrast. Prior: likely
    pred_d_third_order_small           |degree-3 term| <= 0.25 of the noun-source contrast for both heads
    pred_e_both_heads_value_led        the largest |term| is value-only for both heads on both cue halves
PRICE (registered maximum): 158 sequences in 3 batches = 3 forwards; 0 backwards; 0 fits. Bar <= 4.
"""
...
OUT = ROOT / "circuits/followups/verb_readers_degree_v449_result.json"
CANDIDATE_ID = "chain.verb_readers_degree_v449"
N_HEAD, BATCH = 9, 64
...
READERS = ((11, 3), (7, 8))
FORWARDS_MAX = 4
PREDICTIONS = {"pred_a_closure": "<= 1e-4 / 2e-2", "pred_b_noun_source_carries_a_quarter": ">= 0.25", "pred_c_verb_readers_are_value_copies": ">= 0.60", "pred_d_third_order_small": "<= 0.25 x 2", "pred_e_both_heads_value_led": "value-only x 4"}
...
def main() -> None:
    ...
    for gi, r_ in enumerate(recs):
        ...
        vs = r_["second_offset"]
        if vs - c < 2: dropped += 1; continue                          # distant verbs only: the verb is predicted at position vs - 1 >= c + 1
        swapped = list(r_["ids"]); swapped[c] = alt; plural_first = r_["cue"] == "plural"
        rows.append(Row(r_["ids"][:vs], c, r_["cue"], plural_first, gi)); rows.append(Row(swapped[:vs], c, r_["cue"], not plural_first, gi))
    ...
    D = model.config.n_embd; hd = D // N_HEAD; WU = model.lm_head.weight.detach().float(); u = (WU[L._single(" are")] - WU[L._single(" is")] + WU[L._single(" were")] - WU[L._single(" was")]).cuda()
    ...
    R = report["readers"]; r96 = R["11.3"]
    predictions = {"pred_a_closure": closure <= CLOSURE_TOL and instr <= INSTR_TOL, "pred_b_noun_source_carries_a_quarter": r96["noun_source_share"] >= NOUN_MIN, "pred_c_verb_readers_are_value_copies": r96["term_shares"]["value_only"] >= VALUE_MIN,
                   "pred_d_third_order_small": all(abs(R[k]["degree_shares"]["degree3"]) <= THIRD_MAX for k in R), "pred_e_both_heads_value_led": all(set(R[k]["leading_term_by_frame"].values()) == {"value_only"} for k in R)}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "verb_readers_degree_result_v449", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
