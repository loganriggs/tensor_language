"""GLOBAL QUARTER-DENSITY (rung 205): one fixed top-1152 unit set per MLP, front + middles.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. Comparator arm for the sparse-usage
question (rung 206). Base = the r64-exact config (census +0.0852); here every CP-swapped MLP keeps ONE
GLOBAL top-1152-of-4608 unit set (norm score), same set for all tokens.

REGISTERED PREDICTIONS (anchor: r64 receipt +0.0852; amplified-marginals law S2297):
  (a) GLOBAL PRUNING IS EXPENSIVE HERE: census >= 0.30.
  (b) CERTIFICATES LOST: valid <= 4.
  (c) SANITY: census in [0.10, 2.5]; inert tripwire vs +0.0852.
NULL: front/middle pruning stays cheap at low damage (census < 0.20). PRICE: MLP values 4x down at the
swapped modules. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: global quarter-density comparator')
    raise SystemExit(0)
import torch
...
OUT=PT+'cp_global1152_results.json'
...
if __name__=='__main__':
    ...
    SEL['mode']='norm'; SEL['K']=1152; SEL['K69']=1152; SEL['K69MAP']={}
    SEL['skipset']=tuple(range(10,18)); SEL['motif_off']=(); SEL['clsdmg']=True; SEL['ext_rows']=CROWS
    SEL['cp_swap']=1152; SEL['qk_r']=64; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    if abs(agg-0.0852)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: K change inert')
    ...
    print(f"(c) census in [0.10, 2.5]; era rho {rho:.4f}: {'HELD' if pc else 'FAILED'}")
