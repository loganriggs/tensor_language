#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_attention_into_the_panel_slot_replays pred_c_mlps_at_the_panel_slot_in_band pred_d_all_mlps_at_the_panel_slot_bounded pred_e_attention_dominates_on_the_panel
"""The pronoun slot's two channels on the PANEL frame (v510). v503 (natural pairs): attention into the pronoun answer 0.973, MLPs 9-16 there 0.214, MLPs 1-17
0.213. The same three edits on the 48 v76 panel pairs: does the answer-side re-expression replay across frames as the attention channel did (v489: 0.801 /
0.965)?
PREDICTIONS (scored as written; failures preserved; priors from v489 / v503)
    pred_a_baseline_replays                    the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_attention_into_the_panel_slot_replays  every block's attention write into the answer position closes 0.965 +- 0.03 (v489)
    pred_c_mlps_at_the_panel_slot_in_band      MLPs 9-16 at the answer position jointly close between 0.12 and 0.35 on the panel. Prior: unsure
    pred_d_all_mlps_at_the_panel_slot_bounded  MLPs 1-17 at the answer position jointly close <= 0.40
    pred_e_attention_dominates_on_the_panel    the attention joint is >= 2.5 x the MLPs 1-17 joint
...
OUT = ROOT / "circuits/followups/pronoun_slot_channels_panel_v510_result.json"
CANDIDATE_ID = "chain.pronoun_slot_channels_panel_v510"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, ATTN_V489, BAND, M916_LO, M916_HI, MALL_MAX, RATIO_MIN, UNIT = 1e-3, 0.965, 0.03, 0.12, 0.35, 0.40, 2.5, 701
import random
...
FORWARDS_MAX = 13
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_attention_into_the_panel_slot_replays": "0.965 +- 0.03", "pred_c_mlps_at_the_panel_slot_in_band": "0.12-0.35", "pred_d_all_mlps_at_the_panel_slot_bounded": "<= 0.40", "pred_e_attention_dominates_on_the_panel": ">= 2.5 x"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "attn_v489": ATTN_V489, "band": BAND, "m916": [M916_LO, M916_HI], "mall_max": MALL_MAX, "ratio_min": RATIO_MIN, "unit": UNIT}}
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_attention_into_the_panel_slot_replays": abs(mc["attn_all"] - ATTN_V489) <= BAND, "pred_c_mlps_at_the_panel_slot_in_band": M916_LO <= mc["mlps9_16"] <= M916_HI,
                   "pred_d_all_mlps_at_the_panel_slot_bounded": abs(mc["mlps1_17"]) <= MALL_MAX, "pred_e_attention_dominates_on_the_panel": mc["attn_all"] >= RATIO_MIN * abs(mc["mlps1_17"])}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pronoun_slot_channels_panel_result_v510", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
