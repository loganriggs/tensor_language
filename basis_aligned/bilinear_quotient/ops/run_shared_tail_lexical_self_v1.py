...
@torch.no_grad()
def main():
 files=json.loads((P/'SHARED_TAIL_LEXICAL_SELF_V1_BINDING.json').read_text())['files']
 ...
 out=P/'SHARED_TAIL_LEXICAL_SELF_V1_RESULT.json';assert not out.exists()
 ...
 previous=json.loads((P/'SHARED_TAIL_LEXICAL_V1_RESULT.json').read_text())['cells']
 ...
 result={'pred_a':A and replay==0,'pred_b':all(g['relative_self_effect'][0]>=.1 for g in own),'pred_c':all(max(g['compression_own_relative_error'])<=.1 for g in own),'pred_d':all(g['capability']>0 for g in groups),'anchors':anchors,'reference_replay_maxabs':replay,'own_groups':own,'groups':groups,'cells':cells,'seconds':time.perf_counter()-tic,'scope':'Same48 lexical-transfer rows, now reused for full residual-self node removal; frozen three-term approximation evaluated relative to own self-term removal effect. Native background/suffix, no new fit or independent OOD panel.'}
