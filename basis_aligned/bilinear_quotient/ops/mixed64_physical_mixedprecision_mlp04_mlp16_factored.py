"""RUNG 436 -- MIXED-PRECISION REPAIR OF THE SUB-500M COMPOSITE.
...
pred_d_shifted_and_fresh_hold:
    Untouched WT103 [470824,501664) full original-native mean/p95/max
    <=.075/.140/.220 and conditional fresh max <=.040.

Strong null: identity failure, census >=.10, <=5 certificates, shifted mean
>=.10, inert L16 hook, or no max-error improvement over rung414. Pass licenses
one signed gate; no tuning.
"""
...
OUT = ROOT / "mixed64_physical_mixedprecision_mlp04_mlp16_factored_results.json"
CEV = ROOT / "cev_mixed64_physical_mixedprecision_mlp04_mlp16_factored.pt"
...
BF16_RESULT = ROOT / "mixed64_physical_bf16_mlp04_mlp16_factored_results.json"
SOURCE_FACTOR = ROOT / "mlp16_rank2_quadratic_factored.pt"
PHYSICAL_FACTOR = ROOT / "mlp16_rank2_quadratic_factored_fp32_physical.pt"
WIKI_SKIP = 470_824
N_ROWS = 120
...
L16_SCALARS = 14_984
L16_BYTES = 59_936
RUNG414_MAX = .11764144897460938
RUNG = 436


def _fp32_factored_prediction(x: torch.Tensor,
...
def main() -> None:
    if os.environ.get("BQLIB_DRYRUN") == "1":
        for path in (PARENT_RESULT, PARENT_CEV, SIGNED_PARENT, BF16_RESULT, SOURCE_FACTOR):
        ...
        failed = json.loads(BF16_RESULT.read_text())
        assert failed["parent_cev_delta_max_abs"] == RUNG414_MAX
        assert WIKI_STOP == 501_664 and WIKI_STOP < 675_457
        assert BYTES == 2 * SCALARS + 2 * L16_SCALARS
        assert L16_BYTES == 4 * L16_SCALARS
        print("MIXED64 PHYSICAL MIXEDPRECISION | dry run: parents, population, bills valid")
        return
    ...
    factor_fp32 = {name: value.to(torch.float32).contiguous()
                   for name, value in factor_source.items()}
    torch.save(factor_fp32, PHYSICAL_FACTOR)
    ...
    parent._factored_prediction = _fp32_factored_prediction
    parent.OUT = OUT
    parent.CEV = CEV
    ...
    failed_bf16 = json.loads(BF16_RESULT.read_text())
    ...
    factor_exact_source = (
        set(factor_stored) == set(factor_source)
        and all(torch.equal(factor_stored[name], factor_source[name].float().contiguous())
                for name in factor_source))
    ...
    physical_identity = (
        ...
        and factor_dtypes == ["torch.float32"] and factor_no_dense and factor_exact_source
        ...
        and BYTES == 2 * SCALARS + 2 * L16_SCALARS
    )
    pred_a = physical_identity
    pred_b = (
        result["physical_census_damage"] <= .070
        and result["physical_certificates"] >= 10
        and parent_delta_mean <= .010 and parent_delta_max <= .050
        and parent_delta_max <= .5 * RUNG414_MAX)
    pred_c = (
    ...
    null = (
        ...
        or parent_delta_max >= RUNG414_MAX)

    for key in list(result):
        if key.startswith("pred_") or key.startswith("null_"):
            result.pop(key)
    result.update({
        "status": "mixed64_physical_mixedprecision_mlp04_mlp16_factored_complete",
        "rung": RUNG,
        "claim_level": "physical_mixedprecision_sub500_adoption_repair_screen",
        ...
        "mlp16_factor_exact_source_fp32": factor_exact_source,
        ...
        "rung414_bf16_parent_cev_delta_max_abs": failed_bf16["parent_cev_delta_max_abs"],
        "parent_max_improvement_fraction": (
            1.0 - parent_delta_max / failed_bf16["parent_cev_delta_max_abs"]),
    ...
    print("MIXED64 PHYSICAL MIXEDPRECISION REPAIR DONE", flush=True)
