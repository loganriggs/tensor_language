#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays_v183 pred_b_2483_contrast_drops_015 pred_c_829_contrast_drops_005 pred_d_edit_beats_random_sets pred_e_both_drop_not_grow
"""Pronoun number they/he DoD (v195): EDIT deciding the v192 nomination. Zero the five MLP-5 units the carrier census named -- 1036, 2403, 2538, 3274,
3034 (64% of MLP 5's carrier term into MLP-6 unit 2483; MLP 5 is 35% of 2483's plural - singular contrast, v191) -- at the noun only, in a plain
forward, and read three things: the pooled plural - singular contrast of u_2483 at the noun (block 6), of u_829 at the noun (block 8, the plural
detector), and the they - he margin at the final token. Null: 16 seeded random 5-unit sets of MLP 5 (disjoint from the five), same positions.
The expected margin effect is far below the null (0.35 x 0.22 x 0.45 x 0.05 of the margin), so the margin is reported, not scored.
PREDICTIONS (scored as written; failures preserved)
    pred_a_baseline_replays_v183   the unedited pooled contrast of u_2483 at the noun matches v183 (-1043.5) within relative 1e-3
    pred_b_2483_contrast_drops_015 the edit removes >= 0.15 of |u_2483 contrast| (carriage 0.35 x 0.64 = 0.22 predicted; the edit also removes mass)
    pred_c_829_contrast_drops_005  the edit removes >= 0.05 of |u_829 contrast| (0.22 of 2483, which is 0.22 of MLP 6's 0.45 of 829 -> 0.02 predicted through 2483 alone; the five units may also feed 2826 / 4131). Prior: unsure.
    pred_d_edit_beats_random_sets  the edit's |fractional change of the 2483 contrast| exceeds 3x the largest among the 16 random sets
    pred_e_both_drop_not_grow      the 2483 and 829 contrasts shrink in magnitude (not grow, as 3152 did under the v177 edit)
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_mlp5_carrier_edit_v195_result.json"
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_mlp5_carrier_edit_v195"
UNITS, LAYER, U2483, U829, V183_CONTRAST, DROP_2483_MIN, DROP_829_MIN, NULL_FACTOR, N_NULL, SEED, BATCH = (1036, 2403, 2538, 3274, 3034), 5, 2483, 829, -1043.5, 0.15, 0.05, 3.0, 16, 195, 32
FORWARDS_MAX = 60
PREDICTIONS = {"pred_a_baseline_replays_v183": "<= 1e-3", "pred_b_2483_contrast_drops_015": ">= 0.15", "pred_c_829_contrast_drops_005": ">= 0.05", "pred_d_edit_beats_random_sets": "> 3x null max", "pred_e_both_drop_not_grow": "both shrink"}
...
def main() -> None:
    ...
    predictions = {"pred_a_baseline_replays_v183": abs(base["u2483"] - V183_CONTRAST) / abs(V183_CONTRAST) <= 1e-3, "pred_b_2483_contrast_drops_015": -frac["u2483"] >= DROP_2483_MIN, "pred_c_829_contrast_drops_005": -frac["u829"] >= DROP_829_MIN,
