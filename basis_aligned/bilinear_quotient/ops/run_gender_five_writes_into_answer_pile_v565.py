#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_five_writes_replay_on_pile pred_c_rest_of_their_blocks_small pred_d_all_writes_into_the_answer pred_e_five_below_all
"""The gender readers' writes into the answer on the PILE rows (v565). v563: on 61 fineweb gender pairs the five pronoun readers' writes into the answer close
0.878 of the he - she gap (rest of their blocks 0.001, all attention 1.042). Here the same edits on the second natural gender set, v75's pile-10k rows (mined
outcome-blind by the same miner on a different corpus), aligned pairs by the same in-place noun swap -- the gender line's out-of-sample cell.
PREDICTIONS (scored as written; failures preserved; priors from v563)
    pred_a_baseline_replays          the unedited manual forward replays the model's he - she margins within 1e-3
    pred_b_five_writes_replay_on_pile  the five readers' joint write swap closes 0.878 +- 0.15 on the pile pairs
    pred_c_rest_of_their_blocks_small  the other 32 heads of blocks 9 / 10 / 12 / 15 close <= 0.10 jointly
    pred_d_all_writes_into_the_answer  every head's write into the answer closes >= 0.80
    pred_e_five_below_all            FIVE_W <= ALL_W
...
OUT = ROOT / "circuits/followups/gender_five_writes_into_answer_pile_v565_result.json"
CANDIDATE_ID = "gender.five_writes_into_answer_pile_v565"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, FIVE_V563, BAND, REST_MAX, ALL_MIN, UNIT = 1e-3, 0.878, 0.15, 0.10, 0.80, 701
...
FORWARDS_MAX = 9
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_five_writes_replay_on_pile": "0.878 +- 0.15", "pred_c_rest_of_their_blocks_small": "<= 0.10", "pred_d_all_writes_into_the_answer": ">= 0.80", "pred_e_five_below_all": "FIVE_W <= ALL_W"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "five_v563": FIVE_V563, "band": BAND, "rest_max": REST_MAX, "all_min": ALL_MIN, "unit": UNIT}}
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_five_writes_replay_on_pile": abs(mc["FIVE_W"] - FIVE_V563) <= BAND, "pred_c_rest_of_their_blocks_small": abs(mc["REST_W"]) <= REST_MAX,
                   "pred_d_all_writes_into_the_answer": mc["ALL_W"] >= ALL_MIN, "pred_e_five_below_all": mc["FIVE_W"] <= mc["ALL_W"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "gender_five_writes_into_answer_pile_result_v565", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
