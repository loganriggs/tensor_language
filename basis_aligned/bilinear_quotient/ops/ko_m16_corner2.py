"""ADDITIVITY CONTROL v2 (m16, rung 274): the knockout on the TRUE corner config, with a live config tripwire.

CONVENTION (S2135): per-position dCE = CE(corner config with m16 mean-ablated) - CE(real model); LOWER
IS BETTER. Rung 272 was INSTRUMENT-INVALID (S2371: dead SEL knobs in the mixed
lineage). This build descends from the corner lineage itself; a tripwire kills the run unless the
unablated L2F matches the corner's 0.0601. Mixed-baseline subtracted values to reproduce: own 0.755,
collateral 0.934.
REGISTERED PREDICTIONS (subtraction uses the CORNER baseline receipt):
  (a) BASELINE-INDEPENDENT: subtracted own median in [0.5, 1.0].
  (b) subtracted collateral Spearman >= 0.85.
  (c) SANITY: ablated census in [0.2, 5.0].
NULL: subtraction is baseline-dependent (outside bands) - additivity fails; the S2370 provisional
correction does NOT publish. PRICE: probe. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['removal_matrix_results.json','frontier_claim_motif_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: additivity control v2 m16')
    raise SystemExit(0)
import torch
...
OUT=PT+'ko_m16_corner2_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=64; SEL['qk_rmap']={li:128 for li in range(2,10)}; SEL['qk_tail']=True; SEL['drop_tailE']=True
    print('ARM: rank-16 QK patterns at ALL replaced heads, blocks 2-17 (dicts retired)',flush=True)
    MUCAP={'s':torch.zeros(D,device=DEV),'n':0}
    def _mucap(mo,i_,o_):
        MUCAP['s']+=o_.detach().float().reshape(-1,D).sum(0); MUCAP['n']+=o_.reshape(-1,D).shape[0]
    _hmu=m.transformer.h[16].mlp.register_forward_hook(_mucap)
    with torch.no_grad():
        for _i in range(0,128,4):
            _ix=FW[_i:_i+4,:256].to(DEV)
            _x=F.rms_norm(m.transformer.wte(_ix),(D,)); _x0=_x; _v1=None
            for _blk in m.transformer.h: _x,_v1=_blk(_x,_v1,_x0)
    _hmu.remove()
    MUV=(MUCAP['s']/max(MUCAP['n'],1)).clone()
    def _ablh(mo,i_,o_):
        if not SEL.get('abl_on'): return None
        return MUV.expand_as(o_).to(o_.dtype)
    m.transformer.h[16].mlp.register_forward_hook(_ablh)
    print('m16 mean captured over FW; gated ablation hook armed',flush=True)
    main()
    if 'L2CF' not in SEL: raise SystemExit('INSTRUMENT FAIL: L2CF capture missing')
    L1F,L2C,L2F=SEL['L2CF']
    if abs(L2F-0.0601)>0.01: raise SystemExit(f'INSTRUMENT FAIL: not the corner config (L2F {L2F:+.4f})')
    cev=SEL['cev']
    ...
    _topK={t:v['mean_ablation']['top'][0]['component'] for t,v in BATC.items()}
    _BL={r['tag']:r['member_absdce'] for r in json.load(open(PT+'frontier_claim_motif_results.json'))['circuits']}
    _own=sorted((r['member_absdce']-_BL.get(r['tag'],0))/max(r['ref'],1e-9) for r in rows if _topK.get(r['tag'])=='m16')
    _omed=_own[len(_own)//2]
    _MR=json.load(open(PT+'removal_matrix_results.json'))['matrix']['m16']
    _tags2=[r['tag'] for r in rows if r['tag'] in _MR]
    _rowd={r['tag']:r['member_absdce'] for r in rows}
    _u2=torch.tensor([_MR[t] for t in _tags2]).argsort().argsort().float()
    _v2=torch.tensor([_rowd[t]-_BL.get(t,0) for t in _tags2]).argsort().argsort().float()
    _u2=_u2-_u2.mean(); _v2=_v2-_v2.mean()
    _rhoK=float((_u2*_v2).sum()/((_u2.norm()*_v2.norm())+1e-9))
    pa=0.5<=_omed<=1.0
    pb=_rhoK>=0.85
    pc=0.2<=agg<=5.0
    ...
    print(f"(a) BASELINE-INDEPENDENT - subtracted own {_omed:.3f} in [0.5, 1.0]: {'HELD' if pa else 'FAILED'}")
    print(f"(b) subtracted collateral Spearman {_rhoK:.3f} >= 0.85; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) ablated census {agg:+.4f} in [0.2, 5.0]: {'HELD' if pc else 'FAILED'}")
