"""ALL-CLASS TRAJECTORY SPLICE AT TAIL ATTENTION (rung 145): the family's total context-corruption share.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2239: for name, context corruption at
tail attention (0.96) exceeds the module-replacement share (0.85). Here the trajectory splice covers EVERY
position (all classes) at all eight tail-attn sites during the census eval: the arm prices what perfect
stream fidelity into the tail attention would buy for the WHOLE config, with per-class recoveries free.

REGISTERED PREDICTIONS (anchors: census +1.9474; S2239 name +1.321 at name-only splice):
  (a) TOTAL SHARE MATERIAL: census <= 1.70 (>= 0.25 recovery).
  (b) CONSISTENCY: dCE(name) <= 1.37 (within 0.05 of the name-only splice - no cross-class interference).
  (c) ISOLATION: |L1F - 1.5509| <= 0.02 AND census >= 1.4.
NULL: the non-name classes' tail-attn content is local (total recovery < 0.10 beyond the name-only 0.051).
PRICE: none (oracle diagnostic). Tripwire: INSTRUMENT FAIL if census within 1e-3 of baseline. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: all-class trajectory splice')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_trajall_results.json'
            ...
            if kind=='cv':
                ...
                def h(mod,i_,o_,CV=CV,LW=LW,Wp2=Wp2,first=first,li=li):
                    ...
                    if SEL.get('name_traj_on'):
                        _selN=(c>=0) if SEL.get('traj_all') else (c==5)
                        if _selN.any():
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['name_traj']=True; SEL['traj_all']=True
    print('ARM: rung-127 build + ALL-CLASS trajectory splice at tail attention',flush=True)
    main()
    ...
    print(f"(a) census-bar-1.70; name {PC.get('name',9.9):+.4f} <= 1.0: {'HELD' if pa else 'FAILED'}")
    print(f"(b) name-bar-1.37 via census {agg:+.4f}: {'HELD' if pb else 'FAILED'}")
