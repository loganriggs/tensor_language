"""PER-TOKEN TOP-K DENSITY SWEEP (rung 210): k=288 - one-sixteenth density; curve completion.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. Curve so far on the r64-exact base:
full 4608 -> +0.0852; per-token 1152 -> +0.1015 (fixed-subset comparator +2.2048). This point and rung
208's k=576 locate the usage-sparsity knee.

REGISTERED PREDICTIONS (arms: TOPK288 = this run; anchors +0.0852 full, +0.1015 @ k=1152):
  (a) STILL FAR BELOW THE FIXED SUBSET: census <= 0.45 at 1/16 density.
  (b) valid >= 1.
  (c) SANITY: census in [0.01, 1.5]; inert tripwire vs both anchors.
NULL: the knee sits above 1/16 (census >= 0.5). PRICE: usage-structure probe, no storage claim.
Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: per-token top-k density sweep k=288')
    raise SystemExit(0)
import torch
...
OUT=PT+'cp_topk288_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['cp_topk']=288; SEL['qk_r']=64; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    print(f"(b) valid {nv} >= 1; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) census in [0.01, 1.5]; era rho {rho:.4f}: {'HELD' if pc else 'FAILED'}")
