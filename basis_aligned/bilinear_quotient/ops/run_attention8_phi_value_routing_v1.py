...
def main():
 ...
 result=dict(pred_a=A,pred_b=B,pred_c=B and errors[2]<=.2,margin_replay_error=marginerr,head_reading_replay_error=readerr,field_replay_error=ferr,first_generated_sector_field_errors=errors,body_forwards=count,seconds=time.perf_counter()-tic,scope='Native jointQK routing plus current/first sourcevalues; generatedH sector donation fields, not physical sector input swap. Reused96prefixes; native contextual inputs/background.')
