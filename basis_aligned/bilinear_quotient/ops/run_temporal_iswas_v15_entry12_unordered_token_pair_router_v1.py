#!/usr/bin/env python3
"""Transparent unordered token-pair router for the entry12 construction union."""

# BQGATE: EXPERIMENT pred_a_authority_alignment_scope_replay_geometry_finiteness_and_price pred_b_token_router_identifies_held_branches pred_c_routed_union_preserves_both_targets pred_d_routed_union_is_control_selective pred_e_router_matches_gold_union
...
PRIOR = ROOT / "circuits/prior_art/temporal_iswas_v15_entry12_unordered_token_pair_router_v1.json"
GRAM_RESULT = ROOT / "circuits/followups/temporal_iswas_v15_entry12_crossfit_gram_router_v1_result.json"
ORACLES = ROOT / "circuits/followups/temporal_iswas_v15_construction_oracle_projective_bisector_v1_result.json"
OUT = ROOT / "circuits/followups/temporal_iswas_v15_entry12_unordered_token_pair_router_v1_result.json"
CANDIDATE_ID = "temporal_auxiliary.iswas_v15_entry12_unordered_token_pair_router_v1"
EXPECTED = {
    "prior": "d7f6208b369e287c54a11d0e2eebfb1d55c68079573369196258d9e7592d3f71",
    "gram_result": "8543717ad765068be8677f093d9bef6f204d6dc8a8d3ccca317985c4f9f2b7de",
...
FILES = {
    "prior": PRIOR, "gram_result": GRAM_RESULT, "oracles": ORACLES,
...
PRICE_MAX = {"native_capture_forwards": 10, "differentiable_transformer_forwards": 24,
             ...
             "example_evaluations": 2000, "fit_parameters": 2}
...
def main():
    ...
    prior, gram_result = json.loads(PRIOR.read_text()), json.loads(GRAM_RESULT.read_text())
    authority = (observed == EXPECTED and prior.get("candidate_id") == CANDIDATE_ID
                 and gram_result.get("terminal") == "router_identification_failure")
    ...
    for held in (0, 1):
        ...
        train_signatures = router.signatures(train_context["base_batch"], train_context["donor_batch"])
        train_labels = finite_router.labels_for_rows(backend.torch, train_context["rows"], device=backend.device)
        router_fit = router.fit(train_signatures, train_labels.detach().cpu().tolist())
        held_context = contexts[held]; off_output, off_entry = off_cache[held]
        held_signatures = router.signatures(held_context["base_batch"], held_context["donor_batch"])
        truth = finite_router.labels_for_rows(backend.torch, held_context["rows"], device=backend.device)
        predicted = backend.torch.tensor(router.predict(router_fit, held_signatures), device=backend.device)
        scores = backend.torch.nn.functional.one_hot(predicted, num_classes=3).float()
        ...
        fits[str(held)] = {"basis": {"rank": int(union.shape[1]), **basis_geometry},
                           "router": {**classification, "predictions": predicted.cpu().tolist(),
                                      ...
                                      "train_signatures": [repr(item) for item in train_signatures],
                                      "held_signatures": [repr(item) for item in held_signatures],
                                      "train_map": {repr(key): value for key, value in router_fit.items()}}}
        ...
        learned_gold_replay = max(
            learned_gold_replay, maxdiff(learned["logits"], gold["logits"]))
    ...
    E = learned_gold_replay <= 1e-4
    predictions = {
        "pred_a_authority_alignment_scope_replay_geometry_finiteness_and_price": A,
        "pred_b_token_router_identifies_held_branches": B,
        "pred_c_routed_union_preserves_both_targets": C,
        "pred_d_routed_union_is_control_selective": D,
        "pred_e_router_matches_gold_union": E,
    }
    gold_pass = all(reports[str(parity)]["gold_router"][panel]["metrics"]["signed_projection"] >= .75
                    for parity in (0, 1) for panel in ("A1", "A2"))
    terminal = ("invalid" if not A else "token_pair_router_candidate" if all((B, C, D, E))
    ...
    result = {"schema": "temporal_iswas_v15_entry12_unordered_token_pair_router_result_v1",
              ...
              "instrument": {"manual_native_max_abs_error": native_closure,
                             ...
                             "learned_gold_logit_max_abs_error": learned_gold_replay,
