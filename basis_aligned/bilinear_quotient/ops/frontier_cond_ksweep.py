...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_cond_halfprice_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; arms norm / cond-1152 / cond-576 / cond-288 x eight windows")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_cond_ksweep_results.json'
...
def select_units(li,L,Rw,Dw):
    ...
    return imp.argsort(descending=True)[:(SEL.get('K',2304) if li in (4,5) else 2304)]
...
if __name__=='__main__':
    T00=time.time()
    SEL['mode']='norm'; SEL['collect_asm']=True; SEL['K']=2304
    print('ARM 1/4: norm-2304 + assembly-conditioned Fisher at sites 5/6',flush=True)
    r_norm=main()
    SEL['collect_asm']=False
    SEL['mode']='fisher8'; SEL['K']=1152
    print('ARM 2/4: conditioned-1152 (S2133 reproduction)',flush=True)
    r_h=main()
    SEL['mode']='fisher8'; SEL['K']=576
    print('ARM 3/4: conditioned-576 (quarter price)',flush=True)
    r_q=main()
    SEL['mode']='fisher8'; SEL['K']=288
    print('ARM 4/4: conditioned-288 (eighth price, descriptive)',flush=True)
    r_e=main()
    import statistics as stt
    g11=[round(a-b,4) for a,b in zip(r_h['fresh8'],r_norm['fresh8'])]
    gq=[round(a-b,4) for a,b in zip(r_q['fresh8'],r_h['fresh8'])]
    ge=[round(a-b,4) for a,b in zip(r_e['fresh8'],r_h['fresh8'])]
    pa=stt.median(gq)>=-0.01
    pb=sum(g>=-0.02 for g in gq)>=6
    pc=abs(stt.median(g11)-0.0861)<=0.015
    res={'norm2304':r_norm,'cond1152':r_h,'cond576':r_q,'cond288':r_e,
         'per_window_cond1152_minus_norm':g11,'per_window_cond576_minus_cond1152':gq,
         'per_window_cond288_minus_cond1152':ge,
         'pred_a_quarter_price':bool(pa),'pred_b_windows_floor':bool(pb),'pred_c_reproduces_cond1152':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'cond1152 - norm per window: {g11}')
    print(f'cond576 - cond1152 per window: {gq}')
    print(f'cond288 - cond1152 per window: {ge}')
    print(f"(a) quarter-price median {stt.median(gq):+.4f} >= -0.01: {'HELD' if pa else 'FAILED'}")
    print(f"(b) >= -0.02 on {sum(g>=-0.02 for g in gq)}/8 (bar 6): {'HELD' if pb else 'FAILED'}")
    print(f"(c) cond-1152 median {stt.median(g11):+.4f} vs 0.0861 (tol 0.015): {'HELD' if pc else 'FAILED'}")
