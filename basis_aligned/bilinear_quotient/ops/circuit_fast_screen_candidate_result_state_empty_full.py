...
`The pilot drained the tank near the lantern, of course, so it was` obliges ` empty`; `The pilot filled the tank near the lantern, of course, so it was` obliges ` full`. Sibling of result_state_open_closed (result-state family, second verb pair).
...
SPEC = bs.BehaviourSpec(
    ...
    a2=bs.Family("dawn_frame", "dawn_frame_verb_swap", lambda i, pos: f"Before dawn the {_adj(i)} {_agent(i)} {'drained' if pos else 'filled'} it, of course, leaving it"),
