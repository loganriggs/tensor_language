...
"""Self-reads at the noun for an adjacent verb, rows corrected (v500; v499 ran on the distant rows by a derive leftover). v498: when the verb follows the noun directly (the answer position is the noun), 11.3's value at the
...
PRICE (registered maximum): 3 batches x 5 passes = 15 forwards (43 rows in batches of 32 -> 2 batches of pairs = 3 token batches); 0 backwards; 0 fits. Bar <= 16.
"""
...
OUT = ROOT / "circuits/followups/self_reads_adjacent_fixed_v500_result.json"
CANDIDATE_ID = "chain.self_reads_adjacent_fixed_v500"
N_HEAD = 9
...
def main() -> None:
    ...
    dist, nat_t = study(adj_items, 32); verb = dist; text = dist; panel = {"rows": "adjacent (answer = noun)", "adjacent_rows": len(adj_items)}; verb_items = adj_items
    ...
    OUT.write_text(json.dumps({"schema": "self_reads_adjacent_fixed_result_v500", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
