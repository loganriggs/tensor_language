...
PLAN=dict(widths=[32,128,512],optimizers=['adam','muon'],seeds=[0,1],steps=600,lr=.05,metric='gaussian',native_forwards=0)
def main():
 ...
 out=P/'NATIVE_FULL_QUADRATIC_V1.json';assert not out.exists();start=time.perf_counter()
 ...
 for width in PLAN['widths']:
  for optimizer in PLAN['optimizers']:
   for seed in PLAN['seeds']:
    ...
    opt=torch.optim.Adam([d,l,r],lr=.05) if optimizer=='adam' else torch.optim.Muon([d,l,r],lr=.05,weight_decay=0.,adjust_lr_fn='match_rms_adamw')
    ...
    for step in range(PLAN['steps']):
     ...
     loss=(teacher_g+inner(d,ll,rr,d,ll,rr)-2*inner(c,a,b,d,ll,rr))/teacher_g
     ...
     lr=.05*(.01+.99*.5*(1+math.cos(math.pi*(step+1)/PLAN['steps'])))
     for group in opt.param_groups:group['lr']=lr
    with torch.no_grad():
     dd,ll,rr=saved;f=(teacher_f+float(inner(dd,ll,rr,dd,ll,rr,False))-2*float(inner(c,a,b,dd,ll,rr,False)))/teacher_f
    row=dict(width=width,optimizer=optimizer,seed=seed,gaussian_relative_error=math.sqrt(max(0,valuebest)),frobenius_relative_error=math.sqrt(max(0,f)),reduced_parameter_values=3*1152*width,expanded_parameter_values=width*(50304+2*6912),seconds=time.perf_counter()-t0,history=history);records.append(row)
    if str(width) not in best or valuebest<best[str(width)]['loss']:best[str(width)]=dict(loss=valuebest,factors=[v.cpu() for v in saved])
    result=dict(plan=PLAN,replay=replay,records=records,seconds=time.perf_counter()-start,scope='Full lastMLP unnormalized joint numerator, exactQR frames; no native activation optimization or circuit claim')
 ...
 guard_torch_save(dict(best=best,output_frame=qu.cpu(),input_frame=qe.cpu(),teacher_scale=scale.cpu()),str(P/'NATIVE_FULL_QUADRATIC_V1.pt'));out.write_text(json.dumps(result,indent=2)+'\n')
