#!/usr/bin/env python3
"""Frozen combined capable-construction narrative-tense authority."""
...
import circuit_fast_screen_candidate_narrative_tense_answer_anchor_authority as anchor_v2


SCHEMA = "narrative_tense_combined_authority_v1"
TASK_ID = "narrative_tense.past_vs_present"
CAPABILITY_ID = "narrative_tense.combined_native_capability_v1"
CAUSAL_CANDIDATE_ID = "narrative_tense.attn11_head3_combined_carrier_v1"
...
SUBJECT_PAIRS = (
    ("glove", "belt"), ("sock", "toy"), ("doll", "ball"),
    ("engine", "motor"), ("pump", "cable"), ("wire", "brick"),
    ("tile", "beam"), ("board", "post"), ("fence", "ceiling"),
    ("stair", "ladder"), ("tunnel", "mine"), ("well", "port"),
    ("shed", "hut"), ("store", "court"), ("lawn", "mug"), ("pan", "pot"),
)
TAILS = ("tray", "bin", "tub", "rack", "shelf", "stool", "couch", "pillow",
         "towel", "comb", "razor", "duty", "plan", "goal", "rule", "law")
...
def validate_rows(rows: Sequence[Mapping[str, object]]) -> str:
    ...
    used = set(original._SUBJECTS + original._ALTERNATES + original._PLACES + original._FOCUS) \
        ...
        | set(a2c.TAILS) | set(a2c.LOCATIONS) | set(x for pair in fresh_v2.SUBJECT_PAIRS for x in pair) | set(fresh_v2.TAILS) | set(x for pair in anchor_v2.SUBJECT_PAIRS for x in pair) | set(anchor_v2.TAILS)
    if set(vocab) & used:
    ...
    prior_rows = (original.build_rows() + fresh.build_rows() + newlex.build_rows()
                  + a2c.build_rows() + fresh_v2.build_rows() + anchor_v2.build_rows())
