...
"""V4_EDGE_HEAD3_RESPONSE_V1_PREREGISTRATION.md; unchanged oracle source edits."""
import os,sys,json,time,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';PLAN=dict(prefix=12,native_suffix=64,joined_suffix=196)
def main():
 ...
 out=A/'v4_edge_head3_response_v1_result.json';assert not out.exists();choicepath=P/'V4_FINITE_REFERENCE_CHOICES_V1.json';choices=json.loads(choicepath.read_text());prior=json.loads((A/'v4_finite_reference_v1_result.json').read_text());contexts=[];composition=json.loads((A/'v4_selective_composition_v1_result.json').read_text());field_errors=[];programs=[];cores=[];local_errors=[];export=None
 ...
 for context in contexts:
  ...
  for sv,av in [(1.,1.),(.5,1.),(1.,.5),(-1.,1.),(1.,-1.),(2.,1.),(1.,2.)]:
   ...
   chosen_input=additive.double();chosen_input[batch,query]+=chosen;chosen_input=chosen_input.float()
   chosen_exact=chosen_input+mlp(torch.nn.functional.rms_norm(chosen_input,(d,)))
   # Prepare the response from only three writer columns, not a masked27-core.
   select=torch.tensor([3,12,21],device=w.device);w3=w[:,:,select];z3=beta[:,select]
   g3=compile_linear(mlp.Left.weight.double(),mlp.Right.weight.double(),mlp.Down.weight.double(),additive[batch,query].double(),w3,torch.finfo(torch.float32).eps)
   chosen_linear=bg.clone();chosen_linear[batch,query]=(chosen_linear[batch,query].double()+evaluate_linear(g3,z3)).float()
   posts.extend([('head3_exact',chosen_exact),('head3_linear',chosen_linear)])
 ...
 result=dict(plan=PLAN,counts=counts,max_replay=max(replays),max_local_edge_relative_error=max(local_errors),records=records,predictions=dict(pred_a_instrument=instrument,pred_b_head3_exact=instrument and all(r['passed'] for r in records if r['arm']=='head3_exact'),pred_c_head3_linear=instrument and all(r['passed'] for r in records if r['arm']=='head3_linear')),seconds=time.perf_counter()-tic)
