...
"""Mixed partials / eigenfilters on the number circuit (v603; approved 19 Sep 22:12 UTC; Logan asked what this means -- explained in the docstring
...
OUT = ROOT / "circuits/followups/mixed_partials_number_v603_result.json"
CANDIDATE_ID = "eigenfilters.mixed_partials_number_v603"
N_SENT = 4
...
def main() -> None:
    ...
    for r_ in recs:
        ...
        def margin_of_alpha(alpha):   # alpha: [T], requires_grad handled by caller
            x = F.rms_norm(e0 * (1 + alpha)[None, :, None], (D,))
            x0, v1_ = x, None
            for block in blocks:
    ...
    OUT.write_text(json.dumps({"schema": "mixed_partials_number_result_v603", "candidate_id": CANDIDATE_ID, "plan": plan, "reports": reports,
