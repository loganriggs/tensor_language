...
"""Pronoun gender he/she DoD (v176): head 6.1's write AT THE NOUN POSITION, as read by the two factors of MLP-6 unit 3230 (v175: 54% / 43% of each factor). v172's construction at block 6, head 1, along d = O_{6.1}^T L_3230 and O_{6.1}^T R_3230.
...
OUT = ROOT / "circuits/followups/pronoun_gender_dod_6_1_at_noun_for_unit3230_v176_result.json"
CANDIDATE_ID = "pronoun_gender.he_vs_she.dod_6_1_at_noun_for_unit3230_v176"
UNIT, CLOSURE_TOL, SELF_MIN, INH_MIN = 3230, 1e-3, 0.80, 0.50
FORWARDS_MAX = 6
...
def main() -> None:
    ...
    mlp = model.transformer.h[6].mlp; O = model.transformer.h[6].attn.c_proj.weight.detach().float()[:, 128:256]
    ...
    for start in range(0, len(rows), v1.BATCH):
        ...
        f = L.attention_factors(fw, chunk, 6); forwards += 1
        for name, d in dirs.items():
            out, lamb = L.head_source_terms_at(fw, chunk, 6, noun_of, {1: d}); forwards += 1 if name == "L" else 0
    ...
    OUT.write_text(json.dumps({"schema": "pronoun_gender_dod_6_1_at_noun_for_unit3230_result_v176", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "factors": report, "predictions": predictions, "forwards": forwards,
