...
pred_c collateral<=.5target; pred_d attenuation>=.02/positive>=.75;
pred_e>=2median/15nulls.800forwards.
Opened new removal counterfactual; no causal-composition or complete-circuit promotion.
"""
...
STEM='CITY_INHERITED_REMOVAL_V1';ARMS=['native','native8_midpoint','full_reentry','compiled']+[f'null:{k}' for k in range(16)]
...
def main():
 ...
 doc=json.loads((P/'SINGLE_HEAD_FRESH_V1_ROWS.json').read_text());rows=doc['rows'];groups,mapping=group_rows(rows);assert len(groups)==40
 ...
 tables={k:v.cuda() for k,v in torch.load(P/'SINGLE_HEAD_FRESH_V1_TABLES.pt',weights_only=True).items()}
 ...
 def pre9(module,args):
  ...
  if arm=='native':c['x9']=x.clone()
   ...
   row=groups[state['i']];seed=17092600+1000*int(arm.split(':')[1])+row['context_id'];generator=torch.Generator(device=x.device).manual_seed(seed)
 ...
 v=expand(values,mapping,6);e=v-v[:1];records={}
 for family in doc['variants']:
  ...
  records[family]={'target_error':float((prediction[:,0]-actual[:,0]).norm()/actual[:,0].norm()),'retained_target_rms':target,'control_over_target':(rms[3,1:]/max(target,1e-30)).tolist(),'capable_pairs':int(cap.sum()),'attenuation_positive_fraction':float((atten>0).double().mean()) if len(atten) else 0.,'attenuation_mean':float(atten.mean()) if len(atten) else 0.,'target_over_null_median':target/max(median,1e-30),'nulls_beaten':int((nr<target).sum()),'null_readout_rms':rms[4:].tolist()}
 ...
 'pred_d':all(r['attenuation_positive_fraction']>=.75 and r['attenuation_mean']>=.02 for r in records.values()),'pred_e':all(r['target_over_null_median']>=2 and r['nulls_beaten']>=15 for r in records.values()),
 'families':records,'full_reentry_max_abs':anchor_abs,'full_reentry_relative':anchor_rel,'max_full_joint_input_error':max(joint_errors),'max_null_norm_error':max(norm_errors),'max_outside':max(outside),'body_forwards':count,'seconds':time.perf_counter()-start,'source_shas':binding,'scope':'Opened donor-free city inherited-value removal. One recipient residual7 input. Same-site matched nulls/four readers; no fresh/OOD/paired-swap/composition claim.'}
