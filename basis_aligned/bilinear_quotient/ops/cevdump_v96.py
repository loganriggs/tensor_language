"""VALUE RANK CURVE I (rung 256): v_r = 96 on the mixed base.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2355: value r64 costs +0.0722 and
collapses certificates to 3. The curve upward: does the value side price smoothly, or show the patterns'
binary fine-band structure (S2351)?
REGISTERED PREDICTIONS (arms: THIS vs mixed base 0.0573/11 and value-r64 0.1295/3):
  (a) census <= 0.085 (surcharge <= 0.028 at 3/4 rank).
  (b) valid >= 9.
  (c) SANITY: census in [0.01, 0.15]; inert tripwires vs base AND r64 receipts.
NULL: binary like patterns (census >= 0.11 even at r96). PRICE: mixed patterns 52.9M + value factors
13.5M. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: value rank-96 on mixed base')
    raise SystemExit(0)
import torch
...
OUT=PT+'cevdump_v96_results.json'
...
if __name__=='__main__':
    ...
    cev=SEL['cev']
    torch.save(cev,PT+'cev_v96.pt')
    print('cev saved: cev_v96.pt',flush=True)
    ...
    print(f"(a) census {agg:+.4f} <= 0.085: {'HELD' if pa else 'FAILED'}")
