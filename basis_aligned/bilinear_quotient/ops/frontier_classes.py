"""PER-CLASS TABLE OF THE FRONTIER (rung 134): who pays the remaining +1.9474?

CONVENTION (S2135): per-position dCE = CE(frontier config) - CE(real model) on the census rows; LOWER IS
BETTER. The exact rung-127 build, with the 10-class census breakdown saved. Directs the next structural
target: does the S2145 retrieval-price law survive into the new frontier, or did the class structure flatten?

REGISTERED PREDICTIONS:
  (a) RETRIEVAL LAW SURVIVES: mean dCE over {ind, other} >= 2 x mean over {digit,bclose,sentend,comma,name,rep}.
  (b) ASSEMBLY CHEAP: dCE(subword) <= the overall aggregate.
  (c) REPRODUCTION: |census - 1.9474| <= 0.015.
NULL: the class structure flattened (link < 2x const) - the next structural move is NOT class-targeted.
PRICE: none (re-instrumentation of a deterministic build). Tripwire: INSTRUMENT FAIL if L2CF missing.
Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: per-class table of the frontier')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_classes_results.json'
...
if __name__=='__main__':
    ...
    print('ARM: exact rung-127 build, per-class census table saved',flush=True)
    main()
    ...
    print(f"(a) link {_link:+.4f} >= 2 x const ({2*_const:+.4f}): {'HELD' if pa else 'FAILED'}")
    print(f"(b) subword {_sw:+.4f} <= agg {agg:+.4f}: {'HELD' if pb else 'FAILED'}")
