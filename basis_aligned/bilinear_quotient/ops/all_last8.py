"""ALL-MIXED SPECTRUM (rung 253): top-96 + last-8 at EVERY replaced head - motifs included.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. If the smallest-directions cliff is a
property of squared-attention score maps generally (not just the retrieval tail), the 104-direction mixed
map should serve the motifs too: price 52.9M pattern values - cheaper than the ECONOMICAL shape - with
near-corner-or-better damage.
REGISTERED PREDICTIONS (arms: THIS vs economical 0.0676/8 and 11-cert point 0.0553/11):
  (a) MOTIF SPECTRA HAVE THE CLIFF TOO: census <= 0.030.
  (b) valid >= 12.
  (c) SANITY: census in [0.001, 0.12]; inert tripwires vs the two anchor receipts.
NULL: the cliff is tail-specific (census >= 0.05) - motif maps are norm-ordered after all.
PRICE: 52.9M pattern values. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: all-mixed spectrum last-8')
    raise SystemExit(0)
import torch
...
if __name__=='__main__':
    ...
    print(f"(a) census {agg:+.4f} <= 0.030: {'HELD' if pa else 'FAILED'}")
