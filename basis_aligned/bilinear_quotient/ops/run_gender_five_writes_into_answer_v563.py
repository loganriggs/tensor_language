#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_five_writes_into_the_answer_carry pred_c_rest_of_their_blocks_small pred_d_all_writes_into_the_answer pred_e_five_below_all
"""The gender readers' writes INTO the answer (v563). v549-v551: the five pronoun readers' VALUES at the noun carry 0.600 of the he - she gap on 61 natural gender
pairs (10.1 0.255, 12.4 0.170, 9.6 0.107). v560-v562: the weights-only direct token term (pattern x token value x chain / rms) is ~2% of the token-only singles.
Either the readers' effect is relayed (their writes into the positions between the noun and the answer, re-read later) or the instrument is wrong. The number
line's readers wrote 0.80 of the channel straight into the answer (v485). Here the edit that decides: the five readers' writes into the answer position swapped
jointly (FIVE_W), the other 32 heads of their blocks (REST_W), and every head's write into the answer (ALL_W), by the c_proj pre-hook, on the 61 pairs.
PREDICTIONS (scored as written; failures preserved; priors from v485 (number) and v549 (gender values))
    pred_a_baseline_replays                 the unedited manual forward replays the model's he - she margins within 1e-3
    pred_b_five_writes_into_the_answer_carry  the five readers' joint write swap closes >= 0.45 of the gap (their values at the noun: 0.600; the direct write is most of it, as for number)
    pred_c_rest_of_their_blocks_small       the other 32 heads of blocks 9 / 10 / 12 / 15 close <= 0.10 jointly
    pred_d_all_writes_into_the_answer       every head's write into the answer closes >= 0.80 (number: 0.973)
...
OUT = ROOT / "circuits/followups/gender_five_writes_into_answer_v563_result.json"
CANDIDATE_ID = "gender.five_writes_into_answer_v563"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, FIVE_MIN, REST_MAX, ALL_MIN, UNIT = 1e-3, 0.45, 0.10, 0.80, 701
...
FORWARDS_MAX = 9
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_five_writes_into_the_answer_carry": ">= 0.45", "pred_c_rest_of_their_blocks_small": "<= 0.10", "pred_d_all_writes_into_the_answer": ">= 0.80", "pred_e_five_below_all": "FIVE_W <= ALL_W"}
...
def main() -> None:
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_five_writes_into_the_answer_carry": mc["FIVE_W"] >= FIVE_MIN, "pred_c_rest_of_their_blocks_small": abs(mc["REST_W"]) <= REST_MAX,
                   "pred_d_all_writes_into_the_answer": mc["ALL_W"] >= ALL_MIN, "pred_e_five_below_all": mc["FIVE_W"] <= mc["ALL_W"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "gender_five_writes_into_answer_result_v563", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
