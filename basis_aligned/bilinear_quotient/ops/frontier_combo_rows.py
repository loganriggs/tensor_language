"""THE DAY-CLOSE FRONTIER'S CIRCUIT LEDGER (rung 198): rows, tau-margins, era profile at census +0.62.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. At census 2.855 the median circuit sat
at 5.44x its tau reference; the frontier is now 4.6x lower. This rung documents how close the circuits are:
identical rung-196 rebuild with rows, member/ref ratios, the tau-curve, and the era Spearman.

REGISTERED PREDICTIONS:
  (a) REPRODUCTION: |census - 0.6201| <= 0.015.
  (b) THE MARGIN CLOSED PROPORTIONALLY: median member/ref <= 1.5 (from 5.44).
  (c) ERA INVARIANCE: Spearman vs rung-132 >= 0.85.
NULL (b): circuits closed slower than aggregate again (> 2.0 - the S2275 pattern). PRICE: none.
Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: day-close circuit ledger')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_combo_rows_results.json'
...
if __name__=='__main__':
    ...
    print(f"(a) |census {agg:+.4f} - 0.6201| <= 0.015: {'HELD' if pa else 'FAILED'}")
    print(f"(b) median member/ref {medrat:.3f} <= 1.5 (was 5.44 at census 2.855); valid {nv}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) era Spearman {rho:.4f} >= 0.85: {'HELD' if pc else 'FAILED'}")
