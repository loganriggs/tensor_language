...
"""v409: the AMENDED tier-3 battery over the FIFTY-NINTH batch (TWO cells) -- plus one instrument.
A SECOND attempt at siblings for the determiner-number class, with a principled change rather than a cosmetic one.
v285 2/7, ..., v397 1/1, v405 0/2 (+instrument each).
WHY THE FIRST ATTEMPT FAILED, AS FAR AS THE RECEIPTS SAY. v397's cell (several / each -> ` crates` / ` crate`) passed
all four rows first try with 8 units and held-out recovery 0.898. v405's two siblings both missed row 2 --
determiner_number_barrels (numerous / another) at 0.429 and determiner_number_ropes (many / one) at 0.878 -- despite
capability margins of 4.31 to 4.55 with zero dropped rows, so the model does the task and a six-unit greedy set
fitted on sixteen rows does not carry it. v407 then showed that pooling barrels with three shape variants of itself
covers it at 1.001, which says the failure was in the FIT rather than in the model.
The difference I can name between the pass and the failures is grammatical: several / each contrasts a plural
quantifier with a DISTRIBUTIVE singular, while `one` and `another` are discourse-anaphoric as much as number-marking,
so base and donor there may differ in more than the number feature the screen attributes the effect to. These two keep
the distributive contrast and change only the nouns:
    determiner_number_ropes_d    several / every -> ` ropes` / ` rope`      (frayed)
    determiner_number_barrels_d  many / each     -> ` barrels` / ` barrel`  (cracked)
Both cue -> token mappings are fresh: `each` already appears in the corpus as each -> ` crate`, and each -> ` barrel`
is a different mapping under the gate the corpus measured today (a mapping is a cue word PAIRED WITH a readout token).
I am registering the grammatical story as the REASON I chose these cells, not as a finding -- it is a hypothesis about
why v405 failed, and the only thing this battery can tell me is whether these two pass.
CPU capability margins BEFORE enqueue (mean base / mean donor; A1 | A2; dropped rows; floor 1.0):
  determiner_number_barrels_d  A1:-4.79/+4.78 drop0  A2:-4.33/+4.10 drop0
  determiner_number_ropes_d    A1:-3.69/+3.42 drop0  A2:-3.17/+2.98 drop0
  The class is now 5 of 5 on capability. Capability has never been the problem here.
Protocol identical to v197/v201/.../v405. finiteness_selection is the instrument and is EXCLUDED from four-row counts.
Batch size TWO; bars are the two-cell bars (1/1/2/1/1).

REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 1 of the 2 new behaviours passes all four rows. v405's identical bar failed at 0
                           of 2, and the only thing that has changed is the choice of determiner, so this is a real
                           test of the grammatical story rather than a formality.                          prior 55%
  pred_b_row2           -> MIN held-out per-direction exact-set recovery >= 0.80 on at least 1 of 2. This is the row
                           that failed at v405 (0.429 and 0.878) and the one the story predicts should change.
                                                                                                          prior 55%
  pred_c_row3           -> dim A1 CE damage LB975 > 0 and >= 0.10 on both. v405 passed this on both cells, so the
                           units do carry SOME of the effect even when they miss row 2.                    prior 85%
  pred_d_row4           -> constrained rank-1 own-C CE UB975 <= 0.01 on at least 1 of 2. v405 passed this too
                           (0.0049 and 0.0014).                                                            prior 75%
  pred_e_row5_amended   -> A2-own / A2 ceiling within [0.5, 1.4] with LB975 > 0 on at least 1 of 2.        prior 70%
  pred_f_instrument     -> finiteness_selection: n_units identical to v197 and extraction_held_dirB within +-0.02.
Accounting: a pass gives the determiner class a second four-row member and makes its separability rung possible, which
is the only route by which this class can ever count. A second failure at 0 of 2 says the class supports exactly one
four-row circuit on this frame, which would itself be worth knowing and would end the attempt rather than licensing a
third pair of determiners.
An error receipt per behaviour counts as a miss on every row. ~40 s GPU each, ~2 min.
Smoke: V409_SMOKE=<out.json> (CPU, V409_SMOKE_ROWS=4 per split, V409_SMOKE_NAMES=determiner_number_ropes_d).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec9p_v409_result.json"
...
NAMES = {"determiner_number_ropes_d": "determiner_number_ropes_d", "determiner_number_barrels_d": "determiner_number_barrels_d",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v409", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V409_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V409_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V409_SMOKE_NAMES", "determiner_number_ropes_d").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec9p_v409", "candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v409", "bars": BARS,
