#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_9_7_leads_block_9 pred_c_one_head_leads_block_13 pred_d_one_head_per_block pred_e_singles_add_to_blocks
"""The verb readout's remaining exits by head: blocks 9 and 13 (v451). v447 / v448: the verb readout's exits from the noun are blocks 4 (copier), 11 (11.3), 7
(7.8), 9 (0.079) and 13 (0.058). The atlas 'number' family also lists 9.7. Here blocks 9 and 13 by head on the 79 distant rows at the verb slot.
PREDICTIONS (scored as written; failures preserved; priors from the atlas and v447)
    ...
    pred_b_9_7_leads_block_9     head 9.7 has the largest single-head effect in block 9 (not 9.6, the pronoun reader). Prior: unsure
    pred_c_one_head_leads_block_13  the largest head of block 13 is >= 2 x the second
    pred_d_one_head_per_block    in each block the largest head carries >= 0.60 of the block's effect (0.079 / 0.058)
...
OUT = ROOT / "circuits/followups/verb_heads_9_13_v451_result.json"
CANDIDATE_ID = "chain.verb_heads_9_13_v451"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, TOP_SHARE_MIN, ADD_TOL, B11, B7 = 1e-3, 0.60, 0.04, 0.079, 0.058
...
FORWARDS_MAX = 58
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_9_7_leads_block_9": "9.7 first", "pred_c_one_head_leads_block_13": "top >= 2 x second", "pred_d_one_head_per_block": ">= 0.60 x 2", "pred_e_singles_add_to_blocks": "within 0.04 x 2"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "top_share_min": TOP_SHARE_MIN, "add_tol": ADD_TOL, "b9_v447": B11, "b13_v447": B7}}
    ...
    MODES = {f"{b}.{h}": ({b: [h]}, (0,)) for b in (9, 13) for h in range(N_HEAD)}; forwards = 0
    def logits_margin(x):
    ...
    c = verb["closed"]
    v = {m: c[m]["verb"] for m in c}; b11 = {m: v[m] for m in v if m.startswith("9.")}; b7 = {m: v[m] for m in v if m.startswith("13.")}; report["sums"] = {"9": sum(b11.values()), "13": sum(b7.values())}
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_9_7_leads_block_9": max(b11, key=b11.get) == "9.7", "pred_c_one_head_leads_block_13": max(b7.values()) >= 2 * sorted(b7.values())[-2],
    ...
    OUT.write_text(json.dumps({"schema": "verb_heads_9_13_result_v451", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
