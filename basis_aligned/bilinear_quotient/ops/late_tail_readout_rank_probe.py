#!/usr/bin/env python
"""late_tail_readout_rank_probe -- the RANK of what the unembedding reads in the late tail. §2797: the late MLPs' tail writes cost .1130
when withheld from the final readout alone. Here the readout is denied those writes EXCEPT their component in a k-dim tail frame
(k = 8/32/128 of 384), in two frames: the tail's activation PCs at the final site (fit set) and the unembedding's OWN tail frame (right
singular vectors of W_U restricted to the tail, a weight-only frame). Recovered fraction = 1 - KEEP/FINAL_ONLY. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_unembed_frame_k32_recovers_most pred_c_unembed_frame_beats_activation_pca_at_k32
#                     pred_d_unembed_frame_k8_recovers_a_large_share pred_e_k128_recovers_nearly_all_in_either_frame

SIGN CONVENTION (§2135): every CE number is CE ADDED ABOVE THE REAL MODEL on held-out docs 0-63 -- LOWER IS BETTER. Descriptive;
nothing installs into the §312 frontier; the frames are a data covariance and a weight SVD, scored by CE only (§2118 stays closed).
Preregistration: polynomial_causal/LATE_TAIL_READOUT_RANK_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_tail_readout_rank_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_tail_readout_rank_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_TAIL_READOUT_RANK_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_tail_write_consumer_probe_results.json"   # §2797
OUT = ROOT / "late_tail_readout_rank_probe_results.json"
HASHES = {PREREG: "f2523e94dfefaf811bf9e24febd62cc3cc7a06d6082046fd903f6547eea42bd9", PRIOR: "002f99919ef998efc44471bb1ff61ed681c0bd847cdb07d80d74ea5f9f9f1017",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_tail_readout_rank_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_rec": 0.7, "c_margin": 0.1, "d_rec": 0.4, "e_rec": 0.9}
NULLS = {"b_rec": 0.4, "d_rec": 0.15, "e_rec": 0.6}
...
def main():
    ...
    KS = (8, 32, 128)
    frames = {"PCA": UA, "WU": VWU}
    log(stage="frames", eff_rank_A=round(specA["eff_rank"], 1), rank90_A=specA["rank_90"], eff_rank_WU_tail=round(eff_WU, 1), cap_WU32=round(cap(VWU[:, :32]), 3), cap_PCA32=round(cap(UA[:, :32]), 3),
        overlap32=round(float((VWU[:, :32].T @ UA[:, :32]).pow(2).sum() / 32), 3))
    arms = {"SPLIT8_1024": split8(1024), "FINAL_ONLY": consumer(LB, final=True)}
    for fr in frames:
        for k in KS:
            arms[f"KEEP_{fr}{k}"] = consumer(LB, final=True, keep=frames[fr][:, :k])
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    def sdiv(a, b):
        return a / b if abs(b) > 1e-9 else float("inf")
    rec_ = {f"{fr}{k}": 1.0 - sdiv(arms[f"KEEP_{fr}{k}"], arms["FINAL_ONLY"]) for fr in frames for k in KS}
    summ = {"recovered_fraction_of_readout_consumption": rec_, "tail_cov_energy_captured": {f"{fr}{k}": cap(frames[fr][:, :k]) for fr in frames for k in KS},
            "A_tail_spectrum": specA, "WU_tail_eff_rank": eff_WU, "WU_tail_top_sv_share": float(pr_[0]), "frame_overlap_32": float((VWU[:, :32].T @ UA[:, :32]).pow(2).sum() / 32),
            "prior_final_only_2797": PRIOR_FINAL_ONLY, "n_fit_tokens": acc["n"]}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(arms["FINAL_ONLY"] - PRIOR_FINAL_ONLY) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_unembed_frame_k32_recovers_most': bool(rec_["WU32"] >= BARS["b_rec"]),
        'pred_c_unembed_frame_beats_activation_pca_at_k32': bool(rec_["WU32"] >= rec_["PCA32"] + BARS["c_margin"]),
        'pred_d_unembed_frame_k8_recovers_a_large_share': bool(rec_["WU8"] >= BARS["d_rec"]),
        'pred_e_k128_recovers_nearly_all_in_either_frame': bool(max(rec_["WU128"], rec_["PCA128"]) >= BARS["e_rec"]),
    }
    nulls = {"b_null_WU32_le_.4": bool(rec_["WU32"] <= NULLS["b_rec"]), "c_null_WU32_le_PCA32": bool(rec_["WU32"] <= rec_["PCA32"]),
             "d_null_WU8_le_.15": bool(rec_["WU8"] <= NULLS["d_rec"]), "e_null_k128_le_.6": bool(max(rec_["WU128"], rec_["PCA128"]) <= NULLS["e_rec"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "the late MLPs' centred tail writes A (blocks 8-17, lambda-propagated) are withheld from the FINAL READOUT except for their component in a k-dim tail frame: PCA = top-k activation PCs of A at the final site (docs 96-191); WU = top-k right singular vectors of W_U restricted to the tail; recovered fraction = 1 - KEEP/FINAL_ONLY",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "final_only": arms["FINAL_ONLY"], "prior_final_only": PRIOR_FINAL_ONLY},
           "ce_added": arms, "summary": summ,
           "price": {"gpu_doc_forwards": int(fit_rows.shape[0]) * 2 + int(ev.shape[0]) * (1 + len(arms)), "cpu_doc_forwards": 0, "gpu_seconds": time.time() - started},
