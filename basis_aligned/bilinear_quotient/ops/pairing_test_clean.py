# THE PAIRING TEST, BUILT CORRECTLY -- what §1917 should have been.
#
# §1916 speculated that freezing the permutation draws would tighten a COMPARISON more than raising NPERM
# tightens each level. §1917 tried to test it and varied the null DEFINITION (global versus
# within-stratum) instead, so it re-derived §1913 versus §1914 and answered nothing; its pred_b caught the
# mis-build because pairing cannot move a mean by 8.3. §1920 then measured the effect by accident: its
# gradient is a paired difference, and against the sqrt(q0^2 + q3^2) that independence would give it
# came in at 0.63 / 0.56 / 0.42 -- suggestive, but resting on an approximation, since the two quartile
# spreads are not identically distributed.
#
# This is the clean version. ONE estimator (the global null over the uncovered arm), ONE quantity (the
# q3 - q0 gradient), and the ONLY thing varied is whether the two quartiles are scored against the SAME
# draw or two INDEPENDENT draws. Eight seeds each, everything else held.
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY. Rung 3: §1920's named open question, and the
# honest completion of something I got wrong twice.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
#   pred_a PAIRING TIGHTENS THE DIFFERENCE: the shared-draw gradient spread is smaller than the
#          independent-draw spread on all three roles. If FALSE, freezing draws does nothing for a
#          comparison, §1916's speculation was wrong, §1920's incidental 0.42-0.63 was an artifact of its
#          approximation, and I would correct all three on the board -- Codex cited the freezing option in
#          their 10:22Z methodology note.
#   pred_b AND BY THE AMOUNT §1920 ESTIMATED: the ratio of shared to independent spread is between 0.3
#          and 0.9 on all three roles, bracketing §1920's PUBLISHED 0.63 / 0.56 / 0.42. A two-sided bar:
#          below 0.3 would mean pairing helps far more than the sqrt approximation suggested, above 0.9
#          that it barely helps and §1920's reading was too generous.
#   pred_c AND PAIRING DOES NOT BIAS: the shared and independent gradient MEANS agree within 0.1 on all
#          three roles. Pairing changes a variance, not an expectation. This is the predicate that caught
#          §1917's mis-build -- it failed there by 8.3 - and it is registered again unchanged, because a
#          control that has already caught one structural error in this exact experiment earns its place.
#   pred_d CONTROLS: coverage is exactly 16,110; both regimes use the same eight seeds and identical
#          positions; and the shared-draw mean reproduces §1920's PUBLISHED +4.736 / +4.707 / +2.659
#          within 0.3.
...
OUT = PT + 'ops/pairing_test_clean_results.json'
...
def compare_by_bucket(rows, hooks, seenmask=None):
    ...
    def _unull_seed(seed):
        g6 = torch.Generator(device='cpu').manual_seed(seed)
        return sum(float((UL == UP[torch.randperm(UP.numel(), generator=g6).to(UP.device)])
                         .double().mean()) for _ in range(NPERM)) / NPERM
    unulls = []
    for sd in range(8):
    ...
    # SHARED draw: both quartiles scored against the same null within a seed.
    gsh = [obs_q[3] / max(nl, 1e-12) - obs_q[0] / max(nl, 1e-12) for nl in unulls]
    # INDEPENDENT draws: a separate null per quartile within each seed. Same estimator, same data,
    # same NPERM -- the ONLY difference is which draw each arm is scored against.
    gin = []
    for sd in range(8):
        n3 = _unull_seed(7000 + sd)
        n0 = _unull_seed(8000 + sd)
        gin.append(obs_q[3] / max(n3, 1e-12) - obs_q[0] / max(n0, 1e-12))
    out['unc_grad_seeds'] = {'mean': sum(gsh) / 8.0, 'spread': max(gsh) - min(gsh)}
    out['grad_shared'] = {'mean': sum(gsh) / 8.0, 'spread': max(gsh) - min(gsh)}
    out['grad_independent'] = {'mean': sum(gin) / 8.0, 'spread': max(gin) - min(gin)}
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'PAIRING TEST, CLEAN: does sharing the draw tighten a difference? | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    for si, (lbl, g) in enumerate(arms):
        ...
        for ename in evs:
            ...
            row[ename] = {'enrichment': c['mech_cov']['enrichment'],
                          ...
                          'grad_shared': c['grad_shared'],
                          'grad_independent': c['grad_independent'],
    ...
    S1920_GRAD = {'skip7000': 4.736, 'skip11000': 4.707, 'skip1200': 2.659}

    def gr(e, tag, f):
        return res['sites'][A1][e][f'grad_{tag}'][f]
    ratio = {e: gr(e, 'shared', 'spread') / max(gr(e, 'independent', 'spread'), 1e-12) for e in roles}
    pa = all(gr(e, 'shared', 'spread') < gr(e, 'independent', 'spread') for e in roles)
    pb = all(0.3 <= ratio[e] <= 0.9 for e in roles)
    pc = all(abs(gr(e, 'shared', 'mean') - gr(e, 'independent', 'mean')) <= 0.1 for e in roles)
    pd = (ncov == NCOV
          and all(abs(gr(e, 'shared', 'mean') - S1920_GRAD[e]) <= 0.3 for e in roles))

    print(f'\n  uncovered gradient (q3 - q0), 8 seeds, SHARED vs INDEPENDENT draws:', flush=True)
    for e in roles:
        print(f'    {e:10s} shared {gr(e, "shared", "mean"):+.3f} +-{gr(e, "shared", "spread"):.3f}   '
              f'independent {gr(e, "independent", "mean"):+.3f} '
              f'+-{gr(e, "independent", "spread"):.3f}   ratio {ratio[e]:.2f}', flush=True)
    print(f'\n  PAIRING TIGHTENS the difference -> {pa}  ' + '  '.join(
        f'{e} {gr(e, "shared", "spread"):.3f} < {gr(e, "independent", "spread"):.3f}'
        for e in roles), flush=True)
    print(f'  and by the amount §1920 estimated (0.3-0.9) -> {pb}  ' + '  '.join(
        f'{e} {ratio[e]:.2f} (§1920 approx 0.63/0.56/0.42)' for e in roles), flush=True)
    print(f'  and pairing does NOT bias (means within 0.1) -> {pc}  ' + '  '.join(
        f'{e} {abs(gr(e, "shared", "mean") - gr(e, "independent", "mean")):.3f}' for e in roles),
        flush=True)
    print(f'  coverage {ncov}, shared mean reproduces §1920 -> control {pd}  ' + '  '.join(
        f'{e} {gr(e, "shared", "mean"):+.3f} vs {S1920_GRAD[e]:+.3f}' for e in roles), flush=True)
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_pairing_tightens': bool(pa),
                          'pred_b_ratio_matches_S1920': bool(pb),
                          'pred_c_pairing_unbiased': bool(pc),
