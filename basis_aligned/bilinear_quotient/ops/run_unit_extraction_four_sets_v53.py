"""v53: v52 rerun -- pred_e re-registered on the RIGHT axis (null base-donor separation), per-document margins saved.

v52 pred_e measured mean |m_null| / mean |m_native| (0.41/0.43/0.19/0.23) -- but a CONSTANT answer bias at the final position
(e.g. was-over-were regardless of quantifier) gives |m_null| > 0 without the null carrying any base/donor information: a
wrong-axis predicate (design-error instance 10). The quantity that says whether the background is null is the SEPARATION
between a base sentence and its donor along the base axis, m_b + m_d (donor axis = -base axis on answer-changing rows):
a constant bias cancels in the sum, retained information does not. v52 a/b/c/d are unchanged and re-reported.
    pred_e_null_is_null (v53)  every set: mean(m_null_b + m_null_d) <= 0.25 x mean(m_nat_b + m_nat_d). Worked: bias c gives
                               c + (-c) = 0 -> True; null keeping 40% of the separation -> False.

--- v52 docstring (arms, a-d unchanged) ---
...
OUT = ROOT / "circuits/followups/unit_extraction_four_sets_v53_result.json"
SETS = v23.SETS
...
def _plan():
    return {"candidate_id": "corpus.unit_extraction_four_sets_v53", "sets": {k: v[1] for k, v in SETS.items()},
...
def main() -> None:
    ...
    for n, (m, units) in SETS.items():
        ...
        def family(prep, names):
            ...
            half = len(nat) // 2   # first half = base sentences, second half = their donors, same row order
            sep_nat = [nat[i] + nat[half + i] for i in range(half)]
            sep_null = [m_null[i] + m_null[half + i] for i in range(half)]
            out = {"null_abs_over_native_abs": sum(abs(x) for x in m_null) / sum(abs(x) for x in nat), "documents": len(nat),
                   "null_separation_over_native": sum(sep_null) / sum(sep_nat), "per_doc": {"native": nat, "null": m_null}}
            for a in names:
                su, qs = arms[a]
        ...
        report[n] = r
        print(n, "null sep %.3f null|.| %.2f  exact %.3f [%.3f,%.3f]  dim %.3f  rand %.3f  set_r %.3f  interchange %.3f | A2 exact %.3f dim %.3f" % (
            r["A1"]["null_separation_over_native"], r["A1"]["null_abs_over_native_abs"], r["A1"]["keep_exact"]["point"], r["A1"]["keep_exact"]["lb95"], r["A1"]["keep_exact"]["ub95"],
    ...
    predictions = {
        ...
        'pred_e_null_is_null': all(report[n]["A1"]["null_separation_over_native"] <= NULL_FRAC for n in SETS),
    }
    result = {"predictions": predictions, "schema": "circuit_unit_extraction_result_v2", "candidate_id": "corpus.unit_extraction_four_sets_v53",
