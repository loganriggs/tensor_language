...
"""modal DoD (v153): the run_modal_dod_battery_v66 set on natural FineWeb rows (training corpus, out-of-panel) (PREDICTS OOD). Body: `dod_natural_line.run`; rows receipt `circuits/followups/modal_dod_natural_rows_v153.json` (outcome-blind
miner; any-sense cue filter stated). Congruent cells ('natural_remote_would', 'natural_present_will') are the readout test; the other cells are corpus counter-cases. Emitted by dod_line.py.
...
import run_modal_dod_battery_v66 as line

CANDIDATE_ID = line.CANDIDATE_ID.rsplit(".", 1)[0] + ".dod_natural_v153"
ROWS = N.ROOT / "circuits/followups/modal_dod_natural_rows_v153.json"
OUT = "modal_dod_natural_v153_result.json"
CONGRUENT = ('natural_remote_would', 'natural_present_will')
HEADS = ((9, 4), (11, 3), (9, 1), (15, 5))
...
def main() -> None:
    rows, sha, pos, neg = N.rows_from_receipt(ROWS, "would", " would", " will", lambda r: f"natural_{r['cue']}_{r['label']}")
