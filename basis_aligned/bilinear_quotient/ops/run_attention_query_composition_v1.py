...
"""Finite baseline Q2 reuse; ATTENTION_QUERY_COMPOSITION_V1_PREREGISTRATION.md."""
import os,sys,json,time,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';PLAN=dict(prefix=12,native_suffix=40)
def main():
 ...
 out=A/'attention_query_composition_v1_result.json';assert not out.exists()
 ...
 for context in binding['contexts']:
  ...
  def native(role,aa,freeze=None):
   counts['native_suffix']+=1;pos,ds=c['source_components'][role];handles=[]
   for key in (() if freeze is None else ((freeze,) if isinstance(freeze,str) else freeze)):
    handles.append(getattr(att,key).register_forward_hook(lambda module,args,output,key=key:fixed[key]))
  ...
  zeros=torch.zeros(n,23,device='cuda');baseline=native('subject',zeros);replays.append(float((baseline-native('subject',zeros,('c_q','c_q2'))).abs().max()))
  for role in ['subject','attractor']:
   ...
   effects={name:baseline-native(role,aa,freeze) for name,freeze in [('full',None),('freeze_Q2','c_q2'),('freeze_Q1','c_q'),('freeze_both',('c_q','c_q2'))]}
   ...
   for family in dict.fromkeys(families):
    ...
    for name,effect in effects.items():
     v=effect[ids]
     if name!='freeze_both':
      previous=next(r for r in singles['records'] if r['panel']==context['panel'] and r['role']==role and r['family']==family and r['arm']==name);replays.append(float((v-torch.tensor(previous['effect'],device='cuda',dtype=torch.float64)).abs().max()))
     error=(v-full).norm(dim=0)/den;ret=float(v[:,0]@reference/reference.square().sum());leak=float(v[:,1:].norm(dim=0).max()/v[:,0].norm().clamp_min(1e-30))
 ...
 instrument=counts==PLAN and max(replays)<=1e-8;q2=[r for r in records if r['arm']=='freeze_both']
 mixed=[]
 for row in q2:
  match=lambda name:next(r for r in records if r['panel']==row['panel'] and r['family']==row['family'] and r['role']==row['role'] and r['arm']==name)
  full=torch.tensor(match('full')['effect']);one=torch.tensor(match('freeze_Q1')['effect']);two=torch.tensor(match('freeze_Q2')['effect']);both=torch.tensor(row['effect']);delta=both-one-two+full;scale=full[:,0].norm().clamp_min(1e-30);small=min((one-full)[:,0].norm(),(two-full)[:,0].norm()).clamp_min(1e-30)
  mixed.append(dict(panel=row['panel'],family=row['family'],role=row['role'],relative_to_full=(delta.norm(dim=0)/scale).tolist(),number_relative_to_smaller_singleton=float(delta[:,0].norm()/small)))
 result=dict(mixed=mixed,plan=PLAN,counts=counts,prior_sha256=hashlib.sha256(priorpath.read_bytes()).hexdigest(),max_replay=max(replays),records=records,predictions=dict(pred_a_instrument=instrument,pred_b_q2_fidelity=instrument and all(r['fidelity'] for r in q2),pred_c_q2_selectivity=instrument and all(r['selectivity'] for r in q2)),seconds=time.perf_counter()-tic)
