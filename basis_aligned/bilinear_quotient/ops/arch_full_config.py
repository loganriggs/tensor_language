"""ARCHITECTURE COMPOSITION II (rung 232): the full S2327 architecture in one config.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The chapter-verdict architecture:
static token tables at m0+m1, ONE dynamic top-1152 node per remaining MLP, r64 patterns. Additive
prediction: 0.1015 (dynamic config) + 0.113 (m0 gap) + 0.142 (m1 gap) ~ 0.36.

REGISTERED PREDICTIONS (anchors: dynamic config +0.1015/4 certs; single-site gaps 0.113/0.142):
  (a) NEAR-ADDITIVE: census <= 0.45.
  (b) REAL COST: census >= 0.20.
  (c) valid >= 1; sanity range; inert tripwires vs both receipts.
NULL: interactions blow past additivity (census >= 0.55). PRICE: two front tables + dynamic deep nodes +
r64 patterns. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: architecture composition II - full config')
    raise SystemExit(0)
import torch
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['cp_topk']=1152; SEL['tokid_front']=(0,1); SEL['qk_r']=64; SEL['qk_tail']=True; SEL['drop_tailE']=True
    SEL['TOKIDX']={}
    ...
    pa=agg<=0.45
    pb=agg>=0.20
    pc=nv>=1 and 0.01<=agg<=1.5
    if abs(agg-0.1015)<1e-4 or abs(agg-0.0852)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: arch config inert')
    ...
    print(f"(a) census {agg:+.4f} <= 0.45: {'HELD' if pa else 'FAILED'}")
    print(f"(b) additive floor - census {agg:+.4f} >= 0.20; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) valid {nv} >= 1; era rho {rho:.4f}: {'HELD' if pc else 'FAILED'}")
