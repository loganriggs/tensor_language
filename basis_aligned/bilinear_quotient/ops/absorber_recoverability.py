"""IS THE INJECTED ERROR LINEARLY RECOVERABLE? The question that precedes rung 2's build.

S2087 named the target: attention rungs `a6` (+0.8846), `a5` (+0.8523) and `a1`
(+0.8384) inject 2.575 of the band's +1.862 rel-MSE, while the other five
attention rungs in 2..9 total +0.287 between them. It also stated what it had NOT
shown: "whether a rank-r absorber can recover the +0.85 an injector adds is
unmeasured -- that is the build, and I am not claiming it."

Building the absorber properly means a sequential matched-context merge (S307),
which is expensive and has failed twice on that rule. But the question that
DECIDES whether the merge is worth attempting is cheaper and offline: at each
named rung, is the error the dictionary injects LINEARLY READABLE from the stream
the rung already sees? If it is not, no linear absorber of any rank recovers it
and rung 2 closes regardless of how carefully the merge is done.

METHOD. On the same certified config, capture at each target rung: the assembly's
attention input (the rms-normed pre-attention stream) and BOTH the assembly's and
the real model's attention output. The injected residual is (real out - assembly
out). Fit a ridge read from input to residual on the FIRST half of the rows and
score on the SECOND half -- held out, because a fit scored on its own rows is the
shape LESSON 106 and S2061/S2069 were written about and it would flatter any
absorber.

REGISTERED PREDICTIONS:
  (a) IT IS RECOVERABLE AT ALL: at `a5`, a rank-32 ridge read captures >= 50% of
      the injected residual variance on HELD-OUT rows. Registered in the
      direction rung 2 needs, so the risk sits on the rung. If FALSE the
      injection is not linearly readable from what the rung sees and the
      absorber programme should close before the merge is built.
  (b) IT GENERALISES ACROSS THE THREE TARGETS: at least 2 of a1/a5/a6 clear the
      same 50% bar. One rung could be idiosyncratic; the re-specified rung is
      three rungs.
  (c) CONTROL, and nothing may be read without it: the SAME rank-32 read fitted
      on SHUFFLED residuals (rows permuted, destroying the input-residual
      correspondence) captures < 10% held out. A ridge with 32 free directions
      on this much data could fit noise; if the shuffled control also scores
      high, the numbers in (a) and (b) are capacity, not signal.

Writes absorber_recoverability_results.json. No absorber is installed and no CE
is claimed -- this measures readability only.
"""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    if not os.path.exists(_bq+'sublayer_stream_error_results.json'):
        print('DRYRUN FAIL: S2087 sublayer split absent'); raise SystemExit(1)
    _p=json.load(open(_bq+'sublayer_stream_error_results.json'))
    print(f"DRYRUN OK: S2087 present (block5 attn {_p['block5_attn_step']}, "
          f"band attn {_p['band_attn_total']}); testing linear recoverability "
          f"at a1/a5/a6, held out")
    raise SystemExit(0)
...
OUT=PT+'absorber_recoverability_results.json'
...
def main():
    ...
    RANK=32
    def capture(active):
        """per target rung: attention input (rms-normed pre-attn stream) and output."""
        inp={li:[] for li in TARGETS}; out={li:[] for li in TARGETS}
        hold={}
        hs=install(active)
        for li in TARGETS:
            def mkpre(li=li):
                def h(mo,args):
                    hold[li]=(args[0].detach().float(),args[2].detach().float())
                return h
            def mkattn(li=li):
                def h(mo,args,outp):
                    x,x0=hold[li]
                    lam=m.transformer.h[li].lambdas
                    pre=lam[0].float()*x+lam[1].float()*x0
                    inp[li].append(F.rms_norm(pre,(D,)).reshape(-1,D))
                    y=(outp[0] if isinstance(outp,tuple) else outp).detach().float()
                    out[li].append(y.reshape(-1,D))
                return h
        ...
        for i in range(R0,R1,8):
            ...
            m(cur['idx'], bb[:,1:].contiguous())
        for h in hs: h.remove()
        return ({li:torch.cat(v) for li,v in inp.items()},
                {li:torch.cat(v) for li,v in out.items()})
    realI,realO=capture([])
    empI,empO=capture(cfgE)

    def fit_score(X,Rres,rank,shuffle=False):
        n=X.shape[0]; h=n//2
        Xf,Xe=X[:h],X[h:]
        Rf,Re=Rres[:h],Rres[h:]
        if shuffle:
            g=torch.Generator(device='cpu').manual_seed(7)
            Rf=Rf[torch.randperm(Rf.shape[0],generator=g).to(Rf.device)]
        ...
        pred=Ze@W
        num=float(((Re-pred)**2).sum()); den=float(((Re-Re.mean(0,keepdim=True))**2).sum())
        return 1.0-num/max(den,1e-12)

    res={}
    for li in TARGETS:
        Rres=realO[li]-empO[li]                 # what the dictionary failed to produce
        inj=float(Rres.var(0).sum())/max(float(realO[li].var(0).sum()),1e-12)
        r2=fit_score(empI[li],Rres,RANK)
        r2s=fit_score(empI[li],Rres,RANK,shuffle=True)
        res[f'a{li}']={'injected_rel_var':round(inj,4),
                       'heldout_r2_rank32':round(r2,4),
                       'shuffled_control_r2':round(r2s,4)}
        print(f'  a{li}: injected rel-var {inj:.4f} | held-out R2 {r2:.4f} '
              f'| shuffled control {r2s:.4f}',flush=True)
    ok=[k for k,v in res.items() if v['heldout_r2_rank32']>=0.50]
    ctrl=max(v['shuffled_control_r2'] for v in res.values())
    pa=res['a5']['heldout_r2_rank32']>=0.50
    pb=len(ok)>=2
    pc=ctrl<0.10
    out={'rank':RANK,'targets':res,'clearing_50pct':ok,
         'max_shuffled_control':round(ctrl,4),
         'pred_a_a5_recoverable':bool(pa),
         'pred_b_two_of_three':bool(pb),
         'pred_c_control_clean':bool(pc)}
    print(f"\n(c) CONTROL shuffled R2 {ctrl:.4f} < 0.10: "
          f"{'HELD' if pc else 'FAILED'}")
    if not pc:
        print('    CONTROL FAILED -- a rank-32 read fits noise on this data and '
              'neither (a) nor (b) may be read.')
    else:
        print(f"(a) a5 held-out R2 {res['a5']['heldout_r2_rank32']:.4f} >= 0.50: "
              f"{'HELD' if pa else 'FAILED'}")
        print(f"(b) >=2 of a1/a5/a6 clear 0.50: {ok} : "
              f"{'HELD' if pb else 'FAILED'}")
        if not pa and not pb:
            print("    READING: the injected error is not linearly readable "
                  "from what the rung sees -- rung 2's absorber programme "
                  "should CLOSE before the merge is built.")
