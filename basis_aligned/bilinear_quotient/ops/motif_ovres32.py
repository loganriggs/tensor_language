"""OV RANK-32 SATURATION CHECK (rung 180): is the value-map share exhausted at rank 8?

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2276: rank-8 OV residuals buy 0.052
census. If rank 32 buys no more, the value map is saturated and the remaining ~0.50 motif ceiling is
ATTENTION-PATTERN structural (low-rank QK becomes the next lever). If rank 32 keeps yielding, rank-8
underfit the map.

REGISTERED PREDICTIONS (anchor: rung-178 census +1.8950):
  (a) SATURATION: |census(r=32) - 1.8950| <= 0.02.
  (b) NO WORSENING: census(r=32) <= 1.9000.
  (c) SANITY: increment in [0.25, 0.45] AND census >= 1.4.
NULL: census(r=32) <= 1.86 - capacity still yielding; the pattern reading weakens and an OV rank sweep
replaces QK work. PRICE: 4x the rung-178 values (622,592) if adopted. Tripwire: inert guard as rung 178.
Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: OV rank-32 saturation check')
    raise SystemExit(0)
import torch
...
OUT=PT+'motif_ovres32_results.json'
    ...
    for li in range(2,10):
        ...
        if SEL.get('ov_res'):
            ...
            for _k9 in _AG:
                ...
                _r9=int(SEL.get('ov_rank',8))
                _WR[_k9]=(_U[:,:_r9]*_S9[:_r9])@_Vh9[:_r9]
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['ov_res']=True; SEL['ov_rank']=32
    print('ARM: rung-127 build + rank-8 OV residuals at all 76 motif heads',flush=True)
    main()
    ...
    print(f"(a) |census {agg:+.4f} - 1.8950| <= 0.02 (saturation): {'HELD' if pa else 'FAILED'}")
    print(f"(b) census {agg:+.4f} <= 1.9000 (no worsening): {'HELD' if pb else 'FAILED'}")
