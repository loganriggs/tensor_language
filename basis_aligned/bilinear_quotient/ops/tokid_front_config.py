"""ARCHITECTURE COMPOSITION I (rung 231): the m0 static token table inside the r64 frontier config.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. Single-site numbers (all else real):
m0 TOKID 0.1478 vs ORACLE 0.0349 - a 0.113 gap. This run swaps ONLY m0 to the static table inside the
r64-exact config (+0.0852 base, 7/62 certs): does the cost compose additively (~0.20)?

REGISTERED PREDICTIONS (arms: THIS = r64 base + m0 tokid; anchors r64 +0.0852, single-site gap 0.113):
  (a) NEAR-ADDITIVE: census <= 0.22.
  (b) NOT FREE IN-CONFIG: census >= 0.13.
  (c) CERTIFICATES SURVIVE PARTIALLY: valid >= 2; sanity range; inert tripwire vs the r64 receipt.
NULL: super-additive interaction (census >= 0.30). PRICE: r64 config + 58M-index m0 table (naive; rows
compressible later). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: architecture composition I - m0 table in r64 config')
    raise SystemExit(0)
import torch
...
OUT=PT+'tokid_front_results.json'
    ...
    def install(active):
        ...
        for nm in active:
            ...
            if kind=='cv':
                ...
                def h(mod,i_,o_,li=li,Lk=Lk,Rk=Rk,Dk=Dk,db=db):
                    x=i_[0].float()
                    u=(x@Lk.T)*(x@Rk.T)
                    if li in SEL.get('tokid_front',()):
                        _ti=SEL['TOKIDX'][li][cur['idx']].long()
                        u=u*torch.zeros_like(u).scatter_(-1,_ti,1.0)
                    else:
                        kk=int(SEL.get('cp_topk') or 0)
                        if kk and u.shape[-1]>kk:
                            _idx=(u.abs()*Dk.norm(dim=0)).topk(kk,dim=-1).indices
                            u=u*torch.zeros_like(u).scatter_(-1,_idx,1.0)
                    return ((u@Dk.T)+db).to(o_.dtype)
                hs.append(m.transformer.h[li].mlp
                          .register_forward_hook(h))
            elif kind=='attnd':
                _,li,CV,LW,Wp2=S[nm]
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['tokid_front']=(0,); SEL['qk_r']=64; SEL['qk_tail']=True; SEL['drop_tailE']=True
    SEL['TOKIDX']={}
    _Wte=m.transformer.wte.weight.detach().float()
    for _li in SEL['tokid_front']:
        _mlp2=m.transformer.h[_li].mlp
        _L2=_mlp2.Left.weight.detach().float(); _R2=_mlp2.Right.weight.detach().float()
        _dn2=_mlp2.Down.weight.detach().float().norm(dim=0)
        _I=torch.empty(_Wte.shape[0],1152,dtype=torch.int32,device=DEV)
        for _v0 in range(0,_Wte.shape[0],4096):
            _xe=F.rms_norm(_Wte[_v0:_v0+4096],(D,))
            _I[_v0:_v0+4096]=(((_xe@_L2.T)*(_xe@_R2.T)).abs()*_dn2).topk(1152,dim=-1).indices.int()
        SEL['TOKIDX'][_li]=_I
    print('token tables built for front modules',flush=True)
    ...
    pa=agg<=0.22
    pb=agg>=0.13
    pc=nv>=2 and 0.01<=agg<=1.2
    if abs(agg-0.0852)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: tokid front inert')
    ...
    print(f"(a) census {agg:+.4f} <= 0.22: {'HELD' if pa else 'FAILED'}")
    print(f"(b) additive floor - census {agg:+.4f} >= 0.13; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) valid {nv} >= 2; era rho {rho:.4f}: {'HELD' if pc else 'FAILED'}")
