"""IS THE FRONT EXCESS CONCENTRATED? RERUN WITH THE ORDERING FIXED (Claude, LANE 1)

SS2927 asked this and could not answer it: `_apply_front_split` ran BEFORE `_apply_front`, which resets A from its snapshot whenever
`a_scale` is None, so the split was wiped on every arm and all thirteen read exactly **0.0000**. The identity arm read 0.0000 too -- and
was therefore satisfied by precisely the failure it exists to exclude. What caught it was the ROUTE-AGREEMENT predicate: `f_uniform`
(x0.5 through the split path) read 0.0000 while `plain_half` (x0.5 through the plain multiply) read -0.1649.

Identical question, identical predicates, one line moved. The scientific content is unchanged from SS2927's preregistration:
    ...
    front A  (quadratic residuals)      ???                                     ???     vs uniform -0.1648   this rung

SS2895's front scalar is worth -0.1648 standalone and FAILED to compose (SS2904: TCF -0.2885 against TC -0.3213). A scalar that damages
directions the other terms rely on is exactly what SS2921 found for the tail, where shrinking the bottom of the spectrum HURT by
+0.0489 -- so if the front excess is concentrated, that composition failure has a candidate explanation.

**Carrying SS2926's standing caution forward:** whatever this finds is a STANDALONE number. SS2922's CP split won standalone by 0.0206
and then lost by 0.035 in composition. Nothing here may be adopted without a composed, held-out measurement.
...
Preregistration: polynomial_causal/FRONTIER_FRONT_SPLIT_FIXED_PREREGISTRATION.md
...
OUT=PT+'frontier_front_split_fixed_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
def main():
    ...
    def _cfg(spec):
        ...
        # SS2927: this MUST run after `_apply_front`, which resets A from the _F0 snapshot when `a_scale` is
        # None. Placed before it, the split was silently wiped and every arm read exactly 0.0000 -- a no-op
        # the identity control could not detect, because a no-op is what the identity arm is meant to see.
...
PREREG = PT + '../polynomial_causal/FRONTIER_FRONT_SPLIT_FIXED_PREREGISTRATION.md'
PREREG_SHA = "29e60893e9ac9171200fa1d02fac3f5a37bccd54e6e3d918c48b42e72706036c"
...
if __name__=='__main__':
    ...
    res={'rung':'frontier_front_split_fixed','preds':preds,'nulls':nulls,'bars':BARS,
