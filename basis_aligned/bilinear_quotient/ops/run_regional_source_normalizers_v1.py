...
"""pred_a nativeattention/priorcomponent replay<=1e-5.
pred_b queryconstant write/removal-effect error<=.1 EACHfamily.
pred_c keyconstant same; pred_d bothconstant same. No datafitting.
Constants from per-head Frobeniusnorm²/128, assuming isotropic normalizedinput.
Null: context-dependent norms are essential.9batches48rows13–19tokens5suffixarms.
"""
...
STEM='REGIONAL_SOURCE_NORMALIZERS_V1'
@torch.no_grad()
def main():
    ...
    nominal=[t.square().sum((1,2))/128+eps for t in (q1,q2,k1,k2)]
    ...
    try:
        for length,ids in batches:
            ...
            for pos in range(length):
                ...
                nq=nominal[0]*nominal[1];nk=nominal[2]*nominal[3]
                gates=[gate,1/(128**2*(nq[None]*knorm).sqrt()),1/(128**2*(qnorm*nk[None]).sqrt()),(1/(128**2*(nq*nk).sqrt()))[None].expand_as(gate)]
    ...
    for arm,name in enumerate(('query_constant','key_constant','both_constant'),1):
    ...
    instrument=max(replays+[replay])<=1e-5
    ...
    result={'pred_a':instrument,'pred_b':passed('query_constant'),'pred_c':passed('key_constant'),'pred_d':passed('both_constant')}
    torch.save(dict(pre=pre.cpu(),write_vertices=writes.cpu(),baseline_margins=baseline.cpu(),removal_margins=arms.cpu()),art)
    result.update(cells=cells,prior_component_replay=replay,native_attention_replays=replays,nominal_norms=[n.cpu().tolist() for n in nominal],
                  seconds=time.perf_counter()-tic,body_forwards=count,artifact_sha=digest(art),source_shas=binding,
                  scope='Frozen shared component, trace-derived isotropic norm constants separately on query/key/both. No fit. Full numerator and native input states retained; tests normalization dependency, not arbitrary-input equivalence or standalone extraction.')
