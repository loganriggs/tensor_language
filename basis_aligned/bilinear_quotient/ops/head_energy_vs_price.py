"""ENERGY vs PRICE AT HEAD GRAIN: oracle-correct head 7 alone vs the other eight heads in the certified arm.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['stream_error_profile_results.json','attn5_error_by_head_results.json']
...
OUT=PT+'head_energy_vs_price_results.json'
...
def main():
    ...
    h7=list(range(7*HD,8*HD)); others=[c for c in range(NH*HD) if c not in h7]
    arms={'none':None,'h7_only':h7,'other_eight':others,'all_nine':list(range(NH*HD))}
    res={}
    for k,cols in arms.items():
        ce,prof=run_arm(cols); res[k]={'gap':round(ce-base,5),'profile':[round(p,4) for p in prof]}
        print(f"{k:12s}: gap {ce-base:+.4f} | b5 {prof[5]:.3f} b6 {prof[6]:.3f} b7 {prof[7]:.3f} b17 {prof[17]:.3f}",flush=True)
    g0=res['none']['gap']; gall=res['all_nine']['gap']
    rec={k:(g0-res[k]['gap'])/max(g0-gall,1e-9) for k in arms}
    d6={k:res['none']['profile'][6]-res[k]['profile'][6] for k in arms}
    pa=rec['h7_only']<=0.3*rec['all_nine']; pb=rec['other_eight']>=0.6*rec['all_nine']
    pc=d6['h7_only']>=0.5*d6['all_nine']; pd=abs(prof6-1.7415)<=0.10
    out={'base_ce':round(base,5),'arms':res,'recovery':{k:round(v,4) for k,v in rec.items()},
         'block6_rel_mse_drop':{k:round(v,4) for k,v in d6.items()},'pred_a_loud_error_is_cheap':bool(pa),
         'pred_b_quiet_errors_are_the_price':bool(pb),'pred_c_energy_is_h7':bool(pc),'pred_d_reproduces_S2086':bool(pd),
    ...
    print(f"(a) recovery h7-only {rec['h7_only']:.3f} <= 0.3 x all {rec['all_nine']:.3f}: {'HELD' if pa else 'FAILED'}")
    print(f"(b) recovery other-eight {rec['other_eight']:.3f} >= 0.6 x all: {'HELD' if pb else 'FAILED'}")
    print(f"(c) block-6 drop h7-only {d6['h7_only']:+.4f} >= 0.5 x all {d6['all_nine']:+.4f}: {'HELD' if pc else 'FAILED'}")
