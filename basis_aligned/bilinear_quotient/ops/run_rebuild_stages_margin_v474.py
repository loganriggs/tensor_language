#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_mlp16_margin_replays pred_c_mlp12_carries_a_share pred_d_mlp11_opposes_or_nothing pred_e_singles_sum_bounded
"""The verb-position rebuild stages on the agreement MARGIN (v474). v470 measured each MLP 9..16's whole write at a distant verb on 701's activation
(distributed: 12 / 16 / 14 / 9 / 15 each 0.12-0.15, 11 opposing); v473 gave MLP 16's margin effect (0.091). Here the same eight single-MLP swaps at the
answer position measured on the agreement margin gap itself, 79 distant rows: which rebuild stages carry the verb's number to the logits.
PREDICTIONS (scored as written; failures preserved; priors from v470 / v473)
    pred_a_baseline_replays      the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_mlp16_margin_replays  MLP 16's swap closes 0.091 +- 0.02 of the margin gap
    pred_c_mlp12_carries_a_share MLP 12's swap closes >= 0.05 (it led on 701). Prior: unsure
    pred_d_mlp11_opposes_or_nothing  MLP 11's swap closes <= 0.02 (it opposed on 701)
    pred_e_singles_sum_bounded   the eight singles sum to between 0.30 and 1.20 of the margin gap. Prior: unsure
...
OUT = ROOT / "circuits/followups/rebuild_stages_margin_v474_result.json"
CANDIDATE_ID = "chain.rebuild_stages_margin_v474"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, M16_V473, BAND, M12_MIN, M11_MAX, SUM_LO, SUM_HI, UNIT = 1e-3, 0.091, 0.02, 0.05, 0.02, 0.30, 1.20, 701
import random
...
FORWARDS_MAX = 28
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_mlp16_margin_replays": "0.091 +- 0.02", "pred_c_mlp12_carries_a_share": ">= 0.05", "pred_d_mlp11_opposes_or_nothing": "<= 0.02", "pred_e_singles_sum_bounded": "0.30-1.20"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "m16_v473": M16_V473, "band": BAND, "m12_min": M12_MIN, "m11_max": M11_MAX, "sum": [SUM_LO, SUM_HI], "unit": UNIT}}
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_mlp16_margin_replays": abs(mc["m16"] - M16_V473) <= BAND, "pred_c_mlp12_carries_a_share": mc["m12"] >= M12_MIN,
                   "pred_d_mlp11_opposes_or_nothing": mc["m11"] <= M11_MAX, "pred_e_singles_sum_bounded": SUM_LO <= total <= SUM_HI}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "rebuild_stages_margin_result_v474", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
