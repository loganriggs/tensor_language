...
STEM='QUADRATIC_PRODUCT_CORE_LEARNED_V1'
def digest(p):
 h=hashlib.sha256()
 with open(p,'rb') as f:
...
def main():
 ...
 programs=[torch.load(P/'COUPLED_QUARTIC_LBFGS_V1_PROGRAM.pt',weights_only=True)]
 ...
 for idx,p in enumerate(programs[:1]):
  ...
  ij=pairs(32,b.device);old=torch.zeros(528,2,device='cuda');old[ij[0]==ij[1]]=p['mixing'].cuda()
  ...
  previous=json.loads((P/'COUPLED_QUARTIC_LBFGS_V1_RESULT.json').read_text());objective_replay=abs(oldobj/p['divisor']-previous['history'][-1]['objective']);assert objective_replay<=1e-8
 ...
 result={'pred_a':all(r['diagnostics']['normal_residual']<=1e-8 and r['relative_objective_delta']<=1e-10 for r in reports),'pred_b':all(r['sparse_capture_ratio']>=1.10 and r['sparse_native_error']<=.5*r['baseline_native_error'] for r in reports),'pred_c':all(r['coupled_native_error']<=.1 for r in reports),'reports':reports,'artifact_sha256':digest(ap),'wall_seconds':time.perf_counter()-tic,'scope':'Full528mixed core ceiling and matched32edgegreedyprogram on frozen learned LBFGS V1 bank; no textfit or OOD claim.'}
