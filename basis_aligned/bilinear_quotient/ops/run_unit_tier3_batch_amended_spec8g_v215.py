#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the seventh eight spec-authored behaviours, plus one instrument.
"""v215: the AMENDED tier-3 battery (v196/v197 protocol, unchanged) over the SEVENTH EIGHT spec-authored behaviours -- plus one instrument.

v204 4/8, v205 6/8, v207 4/8, v209 4/8, v211 3/8 (+instrument each) at ~30 GPU-s per behaviour; v213 pending. Eight more, authored
through `circuit_fast_screen_behaviour_spec`, single-cue swap, matched final token, cue never final, 32 A1/A2/P/C rows, ABAB
directions, opening two NEW families and extending three: aux_copy_so (`can/should leave, and so` -> can/should: the auxiliary is
COPIED into the elliptical tag, NEW family) and aux_copy_ellipsis (`... leave, and the sailor` -> can/should, same family);
wh_argument_selection (`What/Where did the pilot near the lantern` -> carry/go, NEW family: wh-type to verb selection) and
wh_argument_perfect (`What/Where had ...` -> carried/gone, same family); adjective_subjunctive (`it was essential/obvious that the
sailor` -> be/was) and requested_subjunctive (`requested/noticed that the sailor` -> be/was), mood family with mandative_subjunctive
(v213); doubt_belief_complementizer (`The doubt/belief of the pilot was` -> whether/that, complementizer family); rarely_inversion
(`Near the lantern, rarely/often, of course,` -> did/the, inversion family). Capability printed BEFORE the run on CPU
(`g.prepare(valid_only=True)`, 32 rows per family):
zero A1 and zero A2 rows dropped on all eight (32 rows each); mean base/donor margins A1: requested_subjunctive -5.22/+5.37,
adjective_subjunctive -4.18/+4.34, aux_copy_ellipsis -3.41/+3.44, rarely_inversion -3.02/+2.79, aux_copy_so -2.86/+2.82,
wh_argument_selection -2.05/+2.81, doubt_belief_complementizer -2.24/+2.36, wh_argument_perfect -1.11/+1.56 (the WEAKEST cell:
`Where had the pilot near the lantern` prefers `gone` over `carried` only mildly, though no row has the wrong sign; it replaced
a Where/When draft at -1.23/+0.25 with 2 A2 rows dropped); A2 weakest wh_argument_perfect -1.55/+1.29, all other A2 |means| >= 2.1.
Protocol identical to v197/v201/v204/v205/v207/v209/v211/v213 (greedy pool 40, target 0.88, min_gain 0.005, max 30; row 2 = MIN over
the two held directions >= 0.80; row 3 dim A1 CE LB975 > 0 and >= 0.10; row 4 constrained rank-1 own-C UB975 <= 0.01 on 16 held C
rows; row 5 = A2-own / A2 full-rank ceiling in [0.5, 1.4], LB975 > 0). finiteness_selection is re-run as the instrument.

REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 4 of the 8 new behaviours pass all four rows (v204 4/8, v205 6/8, v207 4/8, v209 4/8, v211 3/8 with
                           the same recipe; wh_argument_perfect is the expected miss from its CPU margin).                              prior 55%
...
(aux-copy pair, wh pair, mood trio, complementizer, inversion), so all wait for the next separability rung before counting.
...
Smoke: V215_SMOKE=<out.json> (CPU, V215_SMOKE_ROWS=4 per split, V215_SMOKE_NAMES=aux_copy_so; pool 3/max 3, steps 5).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec8g_v215_result.json"
...
NAMES = {"aux_copy_so": "aux_copy_so", "aux_copy_ellipsis": "aux_copy_ellipsis",
         "wh_argument_selection": "wh_argument_selection", "wh_argument_perfect": "wh_argument_perfect",
         "adjective_subjunctive": "adjective_subjunctive", "requested_subjunctive": "requested_subjunctive",
         "doubt_belief_complementizer": "doubt_belief_complementizer", "rarely_inversion": "rarely_inversion",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec8g_v215", "behaviours": 9, "constructions": 2,
...
def main() -> None:
    ...
    smoke = os.environ.get("V215_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V215_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V215_SMOKE_NAMES", "aux_copy_so").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec8g_v215", "candidate_id": "corpus.unit_tier3_batch_amended_spec8g_v215", "bars": BARS,
