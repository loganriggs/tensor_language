# SECOND CLASS on the WINNING arm -- rank-256 at a different large covered set.
#
# §1859 replicated §1853's domination across three disjoint covered sets, but only on the LOSING side:
# the dominating build -- rank-256 at 16,110 types, 164.478M reals, all-position CE 5.98851 / 5.94106 /
# 5.95781 -- was still a single build, and §1859 named that asymmetry as the next thing to remove.
#
# `fineweb_n480_skip80` rows [96:480] give 14,405 covered types (0.894x of 16,110) from a 384-row draw
# that EXCLUDES §1834's first 96 rows entirely. Rank-256 there costs ~148.8M by §1754's model -- cheaper
# still than 164.478M -- so if its CE also beats every published full-rank build, the winning arm is
# replicated on an independent draw and the domination no longer rests on one build on either side.
#
# FULL RANK is built alongside, giving a seventh independent test of §1848's exact attainment.
#
# Docstring and predicate block written in ONE edit (LESSONS 63); costs COMPUTED from §1754's model,
# never typed (§1858's defect).
#
# ROLES. All three. DISCOVERY ONLY. Rung 2: second-class confirmation, winning side.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39,
# every anchor named with the build that produced it per LESSONS 53's addendum:
#   pred_a THE WINNING ARM REPLICATES: this rank-256 build has a LOWER all-position CE than every one of
#          the four PUBLISHED full-rank builds -- 5,419 (§1853: 6.01167 / 5.98477 / 6.00165), 5,914
#          (§1859: 6.01418 / 5.96771 / 5.97623), 5,932 (§1859: 6.00504 / 5.97544 / 5.99141) and 6,142
#          (§1859: 5.99851 / 5.95722 / 5.98174) -- on all three roles, twelve comparisons, while costing
#          less than each. If TRUE the domination rests on independent builds on BOTH sides and §1859's
#          asymmetry is closed. If FALSE this draw's rank-256 does not dominate, the frontier point is
#          draw-sensitive, and §1853's headline must be stated for the specific 16,110-type build.
#   pred_b AND THE COVERED SET IS LARGE AND INDEPENDENT: coverage is at least 2.5x §1834's PUBLISHED
#          5,419 and the fit rows exclude §1834's first 96 entirely. The known-answer check that this is
#          a different large draw and not a re-run of §1852 (LESSON 46).
#   pred_c AND THE TWO LARGE-COVERAGE RANK-256 BUILDS AGREE: this build's all-position CE is within 0.03
#          nats of §1852's PUBLISHED 5.98851 / 5.94106 / 5.95781 on every role. If FALSE, large-coverage
#          rank-256 builds are draw-sensitive at the 0.03-nat scale, which would matter for every
#          frontier figure quoted from a single draw -- including §1854's whole curve.
#   pred_d CONTROLS: full-rank attainment is exact (<0.001) on all three roles, a seventh independent
#          covered set; this build's CE above its own ceiling is within 0.03 of §1852's PUBLISHED
#          +0.09252 / +0.09844 / +0.08049 for rank 256; the ceiling is finite and above live.
...
OUT = PT + 'ops/second_class_win_96_480_results.json'
...
TRANKS = (None, 256)          # None = full rank; §1786/§1813 use this ladder
...
# the four PUBLISHED full-rank builds this one must beat (§1853 and §1859), cost M and all-position CE
FULLRANK_BUILDS = {
    '5419': (230.087, {'skip7000': 6.01167, 'skip11000': 5.98477, 'skip1200': 6.00165}),
    '5914': (250.585, {'skip7000': 6.01418, 'skip11000': 5.96771, 'skip1200': 5.97623}),
    '5932': (251.362, {'skip7000': 6.00504, 'skip11000': 5.97544, 'skip1200': 5.99141}),
    '6142': (260.076, {'skip7000': 5.99851, 'skip11000': 5.95722, 'skip1200': 5.98174})}
...
FITLO, FITHI = 96, 480      # a 384-row draw EXCLUDING §1834's first 96 rows   # 5x the fit rows -> a larger covered set
H = m.transformer.h
NCOV = 5419
...
def main():
    ...
    assert NFULL >= 2.5 * NCOV, f'coverage {NFULL} not >= 2.5x {NCOV}'
    ...
    print(f'SECOND CLASS WIN | fit rows [96:480], 14,405 types, ranks full and 256 | '
    ...
    r256cost = (36 * (256 * (NFULL + D) + 2 * D) + 36 * MAP_RANK * 2 * D) / 1e6
    mine = {e: curve['256'][e]['allpos'] for e in roles2}
    above256 = {e: curve['256'][e]['cov'] - ceilv[e] for e in roles2}
    beats = {b: {e: v[1][e] - mine[e] for e in roles2} for b, v in FULLRANK_BUILDS.items()}
    pa = all(r256cost < FULLRANK_BUILDS[b][0] and all(d > 0 for d in beats[b].values())
             for b in FULLRANK_BUILDS)
    pb = NFULL >= 2.5 * NCOV
    pc = all(abs(mine[e] - S1852_R256_ALLPOS[e]) <= 0.03 for e in roles2)
    pd = (attain
          and all(abs(above256[e] - S1852_R256_ABOVE[e]) <= 0.03 for e in roles2)
          and all(ceilv[e] > livev[e] for e in roles2))

    print(f'\n  rows [{FITLO}:{FITHI}] -- {NFULL} covered types, rank-256 cost {r256cost:.3f}M '
          f'(§1852 build: {S1852_R256_COST_M}M at 16,110)', flush=True)
    for e in roles2:
        print(f'    {e:10s} ceiling {ceilv[e]:.5f}  r256 covered {curve["256"][e]["cov"]:.5f} '
              f'(+{above256[e]:.5f}, §1852 +{S1852_R256_ABOVE[e]:.5f})  ALLPOS {mine[e]:.5f} '
              f'(§1852 {S1852_R256_ALLPOS[e]:.5f})', flush=True)
    print(f'\n  margin over each PUBLISHED full-rank build (positive = this build is better):',
          flush=True)
    for b in FULLRANK_BUILDS:
        print(f'    vs full@{b:5s} ({FULLRANK_BUILDS[b][0]:7.3f}M)  ' + '  '.join(
            f'{e} {beats[b][e]:+.5f}' for e in roles2), flush=True)
    print(f'\n  the WINNING arm replicates on an independent draw -> {pa}', flush=True)
    print(f'  and the covered set is LARGE and independent (>=2.5x {NCOV}) -> {pb}  {NFULL} types',
          flush=True)
    print(f'  and the two large-coverage rank-256 builds AGREE (<0.03) -> {pc}  ' + '  '.join(
        f'{e} {mine[e] - S1852_R256_ALLPOS[e]:+.5f}' for e in roles2), flush=True)
    print(f'  full-rank attainment exact (7th covered set) {attain}, r256 above-ceiling reproduces '
          f'-> control {pd}', flush=True)

    json.dump({'run': 'second_class_win_96_480', 'fit_rows': [FITLO, FITHI],
               'n_covered': NFULL, 'r256_cost_M': r256cost, 'r256_allpos': mine,
               'r256_above_ceiling': above256, 'margin_over_fullrank': beats,
               'ceiling': ceilv, 'live': livev, 'curve': curve, 'attainment_exact': bool(attain),
               'predictions': {'pred_a_winning_arm_replicates': bool(pa),
                               'pred_b_large_independent_draw': bool(pb),
                               'pred_c_two_r256_builds_agree': bool(pc),
