...
if os.environ.get('BQLIB_DRYRUN')=='1':
    if not os.path.exists(_bq0+'cfge_conditioned_fisher2_results.json'):
        print('DRYRUN FAIL: S2130 artifact absent'); raise SystemExit(1)
    _p0=json.load(open(_bq0+'cfge_conditioned_fisher2_results.json'))
    print(f"DRYRUN OK: S2132 top8 median {_p0['gain_top8_median']}; 2-sample vs 4-sample Fisher arms")
    raise SystemExit(0)
if os.environ.get('BQLIB_DRYRUN')=='1':
...
OUT=PT+'cfge_top8_samples_results.json'
...
def main():
    ...
    HEAD4={}
    genF4=torch.Generator(device=DEV).manual_seed(44)
    def gramian4(TOKS,site):
        G=torch.zeros(D,D,device=DEV,dtype=torch.float64); n=0
        for b0 in range(0,TOKS.shape[0],4):
            idx=TOKS[b0:b0+4,:-1]
            with torch.no_grad():
                ...
                p=torch.softmax((30*torch.tanh(m.lm_head(F.rms_norm(x,(D,)))/30))[:,:-1].float(),-1)
            for _s in range(4):
                y=torch.multinomial(p.reshape(-1,p.shape[-1]),1,generator=genF4).view(p.shape[0],p.shape[1])
                with torch.enable_grad():
                    x=F.rms_norm(m.transformer.wte(idx),(D,)); x0=x; v1=None; leaf=None
                    for li,blk in enumerate(m.transformer.h):
                        if li==site:
    ...
    for piece,site in (('c4',5),('c5',6)):
        G4=gramian4(TOKF,site); e4,Q4=torch.linalg.eigh(G4); Q4=Q4.flip(1)
        HEAD4[piece]=Q4[:,:8].float().contiguous()
        OV24[piece]=round(float(((HEAD[piece]['top8'].double().T@Q4[:,:8])**2).sum()/8),4)
        print(f'4-sample top-8 for {piece} (site {site}): overlap with 2-sample eight {OV24[piece]:.3f}',flush=True)
    ...
    g4s=[round(r['gain_top8']+r['gain_cond'],5) for r in rows]   # gap(plain)-gap(4s arm)
    m2=stt.median(g45); m4=stt.median(g4s)
    pa=all(v>=0.7 for v in OV24.values())
    pb=abs(m4-m2)<=0.01
    pc=abs(m2-0.0857)<=0.01
    out={'windows':rows,'reduction_2s_median':round(m2,4),'reduction_4s_median':round(m4,4),
         'overlap_2s_vs_4s':OV24,
         'convention':'gaps are CE above the real model; a gain is a damage reduction',
         'pred_a_subspace_stable':bool(pa),'pred_b_gain_stable':bool(pb),'pred_c_reproduces_s2124':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-t0,1)}
    json.dump(out,open(OUT,'w'),indent=1)
    print(f'damage reduction per window, 2-sample: {g45}')
    print(f'damage reduction per window, 4-sample: {g4s}')
    print(f"(a) 2s/4s top-8 overlap {OV24} all >= 0.7: {'HELD' if pa else 'FAILED'}")
    print(f"(b) |{m4:+.4f} - {m2:+.4f}| = {abs(m4-m2):.4f} <= 0.01: {'HELD' if pb else 'FAILED'}")
    print(f"(c) 2s median {m2:+.4f} vs 0.0857 (tol 0.01): {'HELD' if pc else 'FAILED'}")
