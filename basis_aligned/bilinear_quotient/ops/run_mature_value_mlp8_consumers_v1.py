#!/usr/bin/env python3
# BQGATE: MLP8 output branches with all native consumers live; 320 forwards/5120 sequences.
"""A instrument; B value-path fidelity; C new; D inherited; E composition, .10 all worlds.
...
import module_output_delta_v1 as A
import source_margin_gradient as G
import attention_write_factorial_executor_v1 as S
from circuit_fast_screen_managed_runner import atomic_create_json
BINDING=POLY/'MATURE_VALUE_MLP8_CONSUMERS_V1_BINDING.json';OUT=POLY/'MATURE_VALUE_MLP8_CONSUMERS_V1_RESULT.json'
...
def main():
    ...
    parent=json.loads((POLY/'MATURE_VALUE_MLP8_ORIGIN_V1_RESULT.json').read_text());assert parent['predictions']['pred_a_instrument']
            ...
            for world in source['worlds']:
                ...
                for arm,component in components.items():
                    ...
                    for start in (0,16):
                        ...
                        with A.subtract(mlp,state['mlp_outputs'][start:start+16],delta[start:start+16],audit):
                ...
                reference=q_np@z0
                path={k:q_np@(z0-np.asarray(z)) for k,z in old['arm_logits'].items()}
                ...
                path_errors={k:norms(path[k]-projected[k],projected[k]) for k in ('full','new','inherited')}
                materiality=norms(full,reference)
                ...
                report={'world_id':world['world_id'],'layout':world['layout'],'foil':world['foil'],
                    ...
                    'path_fidelity_errors':path_errors,'source_materiality':materiality,'branch_fidelity_errors':errors,
                    'composition_errors':composition,
                    'path_dominance_passed':all(S.passed(v,.10) for e in path_errors.values() for v in e.values()),
    ...
    preds={'pred_a_instrument':restored and counts==[320,5120] and all(r['instrument_passed'] for r in reports),
        'pred_b_value_path':all(r['path_dominance_passed'] for r in reports),
    ...
    result={'terminal':'mlp8_all_consumers_complete' if preds['pred_a_instrument'] else 'invalid','predictions':preds,'reports':reports,
        ...
        'scope':'MLP8 mixed branches removed at actual module output with all downstream consumers live. Compared to prior partial value-path effects; differences include interactions, not isolated other-consumer circuits. No independent source production or new OOD evidence.'}
    atomic_create_json(OUT,result)
    print(json.dumps({'predictions':preds,'wall_seconds':result['wall_seconds'],'worlds':[{k:r[k] for k in ('world_id','path_dominance_passed','new_passed','inherited_passed','composition_passed','source_materiality')} for r in reports]}));assert preds['pred_a_instrument']
