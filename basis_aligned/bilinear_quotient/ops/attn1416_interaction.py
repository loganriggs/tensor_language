"""ARE attn14 AND attn16 ACTUALLY REDUNDANT (rung 71): the interaction test behind S2164's word.

CONVENTION (S2135): L2 is CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER; d(X) = damage added on the skip-1416
config (both attentions real; S2156) by zeroing X throughout (bases excluded).

S2164 called the two layers "redundant document memory" from matching profiles - an inference, not an
interaction measurement. If they back each other up, zeroing BOTH should cost less than the sum of zeroing each
(sub-additive). NULL (S2149-S2154 precedent): near-additive, independent reads with the same law.

REGISTERED PREDICTIONS (arm-named; damage convention; FR aggregates and ind-position signed sums):
  (a) SUB-ADDITIVE: [L2_F(both zeroed) - L2_F(base)] <= 0.85 x ([L2_F(z16)-base] + [L2_F(z14)-base]).
  (b) THE OVERLAP IS ON ind: I_ind = S_ind(both) - S_ind(z16) - S_ind(z14) <= -0.10 x min(S_ind(z16),
      S_ind(z14)) (signed per-position sums on ind positions).
  (c) REPRODUCTION: the base arm reproduces S2156's skip-1416 per-window values (median |delta| <= 0.005).
  TRIPWIRE: each zeroed arm's CE must differ from base.

Writes attn1416_interaction_results.json. Self-reviewed. RUNG-70 HEADER FOLLOWS.

a14's DISTANCE PROFILE (rung 70): division of labor along distance, or redundant document memory?

CONVENTION (S2135): per-position CE(full S2144 config with a14L) - CE(skip-a14, attn14 real) on FR; positive =
the dictionary hurts there. Signed-sum bars per S2159.

S2162: a16's read RISES with distance (rho 0.810, peak 32-64). a14 fails on the same classes (S2157) but with a
diffuse head map (S2155). If the two layers divide labor along distance, a14 should be the SHORT-range reader.
NULL: the same rising profile as a16 (redundant document memory at two layers).

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) NEAR CARRIES AT a14: distance <= 32 positions carry >= 0.6 of the summed ind damage.
  (b) THE PEAK IS NEAR: the max-mean log2 bin (among bins with >= 100 positions) is at distance < 16.
  (c) REPRODUCTION: the skip-a14 arm reproduces S2155's base per-window values (median |delta| <= 0.005).

Writes a14_distance_split_results.json. Self-reviewed. RUNG-68 HEADER FOLLOWS.

IS THE a16 READ SHORT- OR LONG-RANGE (rung 68): the distance split that decides between a truncated-window
attention stand-in and genuine long-range memory.

CONVENTION (S2135): per-position CE(full S2144 config with a16L) - CE(skip-a16, attn16 real) on FR; positive =
the dictionary hurts there. Signed-sum bars per S2159.

S2161 paused the pointer program. One structural question remains cheap and decisive: the DISTANCE profile.
Split ind-position damage by p - (last occurrence of the target). NULL: distance-uniform.

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) NEAR CARRIES: positions with distance <= 32 carry >= 0.5 of the summed ind damage. If HELD, a
      truncated-window 3-4-head attention is the licensed next construction; if FAILED, block 16 is genuine
      long-range memory.
  (b) MONOTONIC PROFILE: Spearman |rho| >= 0.5 between log2-distance bin and mean damage over bins with
      >= 100 positions.
  (c) REPRODUCTION: the skip-a16 arm reproduces S2146 per-window (median |delta| <= 0.005).

Writes a16_distance_split_results.json. Self-reviewed. RUNG-67 HEADER FOLLOWS.

IS THE a16 DAMAGE BIGRAM-ADDRESSED (rung 67): the licensing test for a bigram pointer.

CONVENTION (S2135): per-position CE(full S2144 config with a16L) - CE(skip-a16, attn16 real) on FR; positive =
the dictionary hurts there. Signed-SUM bars per S2159's lesson.

S2158/S2160: neither unigram pointer (target-occurrence, induction-successor) is attn16's read. Hypothesis: the
retrieval is BIGRAM-addressed - copying licensed by a repeated (t[p-1], t[p]) context. Split the per-position
a16L damage on ind+other positions by whether the current bigram occurred earlier in the document.
PRICE: none (attribution). NULL: unmatched positions carry it (the read is not bigram-addressed either).

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) BIGRAM-MATCHED CARRIES: sum of damage on bigram-matched ind+other positions >= 0.6 x sum on all
      ind+other positions.
  (b) CONCENTRATION PER POSITION: mean damage on bigram-matched ind+other >= 2 x mean on unmatched ind+other.
  (c) REPRODUCTION: the skip-a16 arm reproduces S2146 per-window (median |delta| <= 0.005).

Writes a16_bigram_split_results.json. Self-reviewed. RUNG-56 HEADER FOLLOWS.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['a14_distance_split_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; attn14 x attn16 zeroing interaction on skip-1416 (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'attn1416_interaction_results.json'
...
def main():
    ...
    for _li9 in SEL.get('zeroattn',()):
        def _mkza(li9):
            ...
            return hza
        _hz3.append(m.transformer.h[_li9].attn.register_forward_hook(_mkza(_li9)))
...
if __name__=='__main__':
    ...
    SEL['clsdmg']=True; SEL['skipset']=(14,16)
    arms={}
    for tag,za in (('base',()),('z16',(16,)),('z14',(14,)),('both',(14,16))):
        SEL['zeroattn']=za
        print(f'ARM {tag}: skip-1416 with attn zeroed at {za}',flush=True)
        arms[tag]={'r':main(),'cev':SEL['cev']}
    clsflat=SEL['clsflat']
    import statistics as stt
    for tag in ('z16','z14','both'):
        if float((arms[tag]['cev']-arms['base']['cev']).abs().sum())<1e-3:
            raise SystemExit(f'INSTRUMENT FAIL: {tag} identical to base')
    prev=json.load(open(PT+'frontier_skip_1416_results.json'))['skip1416']['fresh8']
    rp=[abs(round(a-b,4)) for a,b in zip(arms['base']['r']['fresh8'],prev)]
    IND=CLS.index('ind')
    mask=clsflat==IND
    d16=arms['z16']['r']['L2_F']-arms['base']['r']['L2_F']
    d14=arms['z14']['r']['L2_F']-arms['base']['r']['L2_F']
    dj=arms['both']['r']['L2_F']-arms['base']['r']['L2_F']
    S16=float((arms['z16']['cev']-arms['base']['cev'])[mask].sum())
    S14=float((arms['z14']['cev']-arms['base']['cev'])[mask].sum())
    Sb=float((arms['both']['cev']-arms['base']['cev'])[mask].sum())
    Iind=Sb-S16-S14
    pa=dj<=0.85*(d16+d14)
    pb=Iind<=-0.10*min(S16,S14)
    pc=stt.median(rp)<=0.005
    res={'arms':{t:{'L2_F':arms[t]['r']['L2_F'],'L2_C':arms[t]['r']['L2_C']} for t in arms},
         'fr_damage':{'z16':round(d16,4),'z14':round(d14,4),'both':round(dj,4),'sum':round(d16+d14,4)},
         'ind_sums':{'z16':round(S16,1),'z14':round(S14,1),'both':round(Sb,1),'interaction':round(Iind,1)},
         'convention':'damage on the skip-1416 config; bases excluded; lower L2 better',
         'pred_a_subadditive':bool(pa),'pred_b_overlap_on_ind':bool(pb),'pred_c_reproduces_s2156':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"FR damage: z16 {d16:+.4f} z14 {d14:+.4f} both {dj:+.4f} (sum {d16+d14:+.4f})")
    print(f"ind sums: z16 {S16:+.1f} z14 {S14:+.1f} both {Sb:+.1f} -> interaction {Iind:+.1f}")
    print(f"(a) both {dj:+.4f} <= 0.85 x sum {0.85*(d16+d14):+.4f}: {'HELD' if pa else 'FAILED'}")
    print(f"(b) I_ind {Iind:+.1f} <= {-0.10*min(S16,S14):+.1f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) base vs S2156 median |delta| {stt.median(rp):.4f} <= 0.005: {'HELD' if pc else 'FAILED'}")
