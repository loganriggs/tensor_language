#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_five_verb_writes_carry_half pred_c_rest_of_their_blocks_small pred_d_joint_below_all_attention pred_e_joint_at_most_the_block_sum
"""The five verb readers' writes into the verb, jointly (v486). v476-v478: into a distant verb the attention writes are led by 11.3 (0.238) and 5.3 (0.113),
with blocks 9 / 7 / 13 (0.116 / 0.095 / 0.073) behind; v485: the pronoun readers' joint write swap closed 0.80 with the rest of their blocks at 0.03. Here
the five verb heads 11.3 / 5.3 / 9.7 / 7.8 / 13.1 swapped jointly by their writes into the answer position, and the other 40 heads of blocks 5 / 7 / 9 / 11 / 13
jointly; 79 distant rows.
PREDICTIONS (scored as written; failures preserved; priors from v476-v478 / v485)
    pred_a_baseline_replays           the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_five_verb_writes_carry_half  the five verb heads' joint write swap closes >= 0.50 of the agreement margin gap
    pred_c_rest_of_their_blocks_small the other 40 heads of their five blocks close <= 0.12 jointly (block 9 also holds 9.6 / 9.1, v451). Prior: unsure
    pred_d_joint_below_all_attention  the five heads' joint swap closes <= 0.943 (all attention into the verb, v475)
    pred_e_joint_at_most_the_block_sum  the five heads' joint swap closes <= 0.25 + 0.16 + 0.116 + 0.095 + 0.073 (their blocks' singles, v476)
PRICE (registered maximum): 3 batches x 3 passes = 9 forwards; 0 backwards; 0 fits. Bar <= 10.
"""
...
OUT = ROOT / "circuits/followups/five_verb_writes_joint_v486_result.json"
CANDIDATE_ID = "chain.five_verb_writes_joint_v486"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, FIVE_MIN, REST_MAX, ALL_ATTN, BLOCK_SUM, UNIT = 1e-3, 0.50, 0.12, 0.943, 0.25 + 0.16 + 0.116 + 0.095 + 0.073, 701
...
FORWARDS_MAX = 10
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_five_verb_writes_carry_half": ">= 0.50", "pred_c_rest_of_their_blocks_small": "<= 0.12", "pred_d_joint_below_all_attention": "<= 0.943", "pred_e_joint_at_most_the_block_sum": "<= 0.694"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "five_min": FIVE_MIN, "rest_max": REST_MAX, "all_attn": ALL_ATTN, "block_sum": BLOCK_SUM, "unit": UNIT}}
    ...
    MODES = {"FIVE_V": ({}, (0,)), "REST_V": ({}, (0,))}; SETS_W = {"FIVE_V": FIVE_V, "REST_V": REST_V}; forwards = 0
    def logits_margin(x):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_five_verb_writes_carry_half": mc["FIVE_V"] >= FIVE_MIN, "pred_c_rest_of_their_blocks_small": abs(mc["REST_V"]) <= REST_MAX,
                   "pred_d_joint_below_all_attention": mc["FIVE_V"] <= ALL_ATTN, "pred_e_joint_at_most_the_block_sum": mc["FIVE_V"] <= BLOCK_SUM}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "five_verb_writes_joint_result_v486", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
