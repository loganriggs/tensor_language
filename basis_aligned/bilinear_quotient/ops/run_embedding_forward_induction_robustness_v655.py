"""Embedding-forward folding, rung 44 (v655): robustness of the induction-head numbers on disjoint rows.

v636 measured head 5.5's induction enrichment (9.6x), head 5.7's position-0 share (0.26) and the key-restricted edit costs on the FIRST 64 skip80 rows
(and skip7000 for CE). This rung repeats the response statistics on skip80 rows 64-127 (disjoint) and the edits on the same held-out rows, and
scores replay tolerances. CE ADDED, lower is better.
PREDICTIONS (scored as written; failures preserved)
    pred_a_native_replays      native CE within 0.002 of 3.13241 (instrument)
    pred_b_55_enrichment_replays 5.5's induction enrichment on the disjoint rows within +-30% of 9.6x. Prior: likely
    pred_c_57_share_replays    5.7's share on positions 0-7 within +-0.10 of 0.269. Prior: likely
    pred_d_55_keys_cost_replays zeroing 5.5 on induction keys costs >= 2 x zeroing it elsewhere (v636: 4.3x) (same held-out rows: exact replay expected). Prior: must hold
    pred_e_base_rate_replays   base rate of induction-eligible keys within +-30% of 0.0098. Prior: likely
PRICE (registered maximum): 4 capture forwards (64 rows x 2 conditions) + 3 configs x 6 eval batches = 22 forwards; 0 backwards; 0 fits. Bar <= 26.
"""
...
OUT = ROOT / "circuits/followups/embedding_forward_induction_robustness_v655_result.json"
FIT_ROWS = ROOT / ".rowcache/fineweb_n480_skip80.pt"
EVAL_ROWS = ROOT / ".rowcache/fineweb_n192_skip7000.pt"
NATIVE_V615 = 3.13241
CANDIDATE_ID = "embedding_forward.induction_robustness_v655"
FORWARDS_MAX = 26
EBATCH, N_CAP, Q_MIN, ROW_OFFSET = 32, 64, 8, 64
L_IND, H_IND, L_PREV, H_PREV, H_SINK = 5, 5, 0, 3, 7
REPLAY_TOL, ENRICH_REF, SHARE_REF, COST_RATIO, BASE_REF = 0.002, 9.6, 0.269, 2.0, 0.0098
PREDICTIONS = {"pred_a_native_replays": "+-0.002", "pred_b_55_enrichment_replays": "9.6x +-30%", "pred_c_57_share_replays": "0.269 +-0.10",
               "pred_d_55_keys_cost_replays": ">= 2x", "pred_e_base_rate_replays": "0.0098 +-30%"}
...
def main() -> None:
    plan = {"candidate_id": CANDIDATE_ID, "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0, "fit_parameters": 0,
            ...
            "bars": {"replay_tol": REPLAY_TOL, "enrich_ref": ENRICH_REF, "share_ref": SHARE_REF, "cost_ratio": COST_RATIO, "base_ref": BASE_REF}, "row_offset": ROW_OFFSET}
    ...
    predictions = {"pred_a_native_replays": abs(native - NATIVE_V615) <= REPLAY_TOL, "pred_b_55_enrichment_replays": abs(enrich[H_IND] - ENRICH_REF) <= 0.3 * ENRICH_REF,
                   "pred_c_57_share_replays": abs(nat[H_SINK]["share_first8"] - SHARE_REF) <= 0.10,
                   "pred_d_55_keys_cost_replays": e["zero_induction_keys"] >= COST_RATIO * e["zero_other_keys"],
                   "pred_e_base_rate_replays": abs(base_ind - BASE_REF) <= 0.3 * BASE_REF}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "embedding_forward_induction_robustness_result_v655", "candidate_id": CANDIDATE_ID, "plan": plan,
