# DOES THE RELIABILITY SIGNAL EXTEND TO UNCOVERED TOKENS? -- §1913's other open question.
#
# §1913 showed the compiled program's top-2 logit margin is a pure property of the current token
# (within-token spread <= 2.86e-05) and precomputable from a length-1 pass, and §1914/§1915 showed that
# under a common global null it orders agreement better than the live model's own margin does
# (+9.32/+9.82/+9.40x versus +5.91/+6.38/+6.10x). Everything there was measured on COVERED tokens, where
# the program emits a stored table row.
#
# At an UNCOVERED token the program emits §1870's rank-64 fallback map output instead. §1909 measured
# those rows as well aligned with the live outputs (median cosine +0.9587) but only 1.18x-1.76x their
# scale, and §1903 showed they are where the MLP restorations actually change predictions. Nothing has
# asked whether the margin is still a token property there, or whether it still orders agreement.
#
# It matters practically: a reliability signal defined only on the 16,110 covered types covers about 76%
# of scored positions. If it extends, the signal covers the whole vocabulary and a program can score its
# own answer for ANY token at build time.
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY. Rung 3: §1913's second open question, and a
# return to the object after four sections (§1914-§1917) on estimator precision.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
# Populations are named per LESSON 71: every quantity below is the UNCOVERED arm unless it says covered.
#   pred_a IT IS STILL A TOKEN PROPERTY: the within-token spread of the program's top-2 margin at
#          UNCOVERED positions is below 1e-3, measured as twice the max deviation from that token's
#          length-1 value. §1765 is about the compiled program, not about which row a site emits, so it
#          should hold -- but the fallback row is produced by a MAP from the embedding rather than looked
#          up, and no section has checked that the argument survives that substitution. If FALSE the
#          signal cannot be precomputed for uncovered tokens at all and the rest of the run is moot.
#   pred_b AND IT STILL ORDERS AGREEMENT: under a GLOBAL null over the uncovered arm, the top margin
#          quartile's enrichment exceeds the bottom's by at least 1.0x on all three roles. If FALSE the
#          signal is covered-only, a program can score its own answer for 76% of positions and not the
#          rest, and any deployment has to fall back to refusing on every uncovered token.
#   pred_c BUT MORE WEAKLY THAN ON COVERED TOKENS: the uncovered gradient is SMALLER than §1914's
#          PUBLISHED covered gradient of +9.50 / +9.79 / +9.28, on all three roles. The fallback row is a
#          rank-64 linear map of the embedding rather than the model's own output, so its confidence
#          should carry less information. If FALSE the fallback's confidence is as informative as the
#          table's, which would be a surprising and useful fact about §1870's map.
#   pred_d CONTROLS: coverage is exactly 16,110; every UNCOVERED scored position has a precomputed token
#          margin (zero lookup misses); and the covered arm reproduces §1914's PUBLISHED +9.50 / +9.79 /
#          +9.28 within 0.5, measured in this same run so the two arms are not compared across builds.
...
OUT = PT + 'ops/uncovered_reliability_results.json'
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'UNCOVERED RELIABILITY: does the token signal extend past the tables? | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    pb = all(gunc[e] >= 1.0 for e in roles)
    pc = all(gunc[e] < S1914_TOK[e] for e in roles)
    pd = (ncov == NCOV and all(miss[e] == 0 for e in roles)
          and all(abs(gcov[e] - S1914_TOK[e]) <= 0.5 for e in roles))
    ...
    print(f'\n  STILL A TOKEN PROPERTY at uncovered positions (< 1e-3) -> {pa}  ' + '  '.join(
        f'{e} <= {2.0 * dev[e]:.2e}' for e in roles), flush=True)
    print(f'  and it STILL ORDERS agreement there (>= 1.0x) -> {pb}  ' + '  '.join(
        f'{e} {gunc[e]:+.2f}x' for e in roles), flush=True)
    print(f'  but MORE WEAKLY than on covered tokens -> {pc}  ' + '  '.join(
        f'{e} unc {gunc[e]:+.2f} vs §1914 cov {S1914_TOK[e]:+.2f}' for e in roles), flush=True)
    print(f'  coverage {ncov}, zero misses, covered arm reproduces §1914 -> control {pd}  ' + '  '.join(
        f'{e} {gcov[e]:+.2f}' for e in roles), flush=True)
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_still_token_property': bool(pa),
                          'pred_b_still_orders_agreement': bool(pb),
                          'pred_c_weaker_than_covered': bool(pc),
