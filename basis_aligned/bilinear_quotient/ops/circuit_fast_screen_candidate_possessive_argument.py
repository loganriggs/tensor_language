"""possessive_number with an INANIMATE nominal in ARGUMENT position.

The corpus now implies two disruptors and only one is established. Animacy is: an animate
oblique breaks agreement at distance four where an inanimate oblique does not. But the distance-6
failure ("finished the work and put away") has an INANIMATE nominal, so a second factor is at
work, and the candidate is argument prominence -- there the nominal is a verb's direct object,
while every passing intervener has been the object of a preposition.

    "The clerks at the desk signed"                -> " their"   inanimate OBLIQUE, dist 4   PASSES
    "The clerks at the desk near the window signed" -> " their"  inanimate OBLIQUE, dist 7   PASSES
    "The clerks packed the crate and signed"        -> " their"  inanimate OBJECT,  dist 5   this screen

Distance five is SHORTER than the seven-token oblique chain that passes, so distance cannot
account for a failure here.

Prediction registered before the run: if argument prominence is a real second disruptor, this
fails despite obliques passing at distance seven. If it passes, the distance-6 failure was not
about argument position and I have no explanation for it -- which I will say, rather than invent
a third factor.
"""
...
TASK_ID = "possessive_number.inanimate_argument"
...
def _conjunct(noun: str, plural: bool, action: str) -> str:
    """Distance five, with the intervening nominal a verb's DIRECT OBJECT rather than an oblique."""
    return f"The {noun}{'s' if plural else ''} packed the crate and {action}"


def _notes(noun: str, plural: bool, action: str) -> str:
    return f"In the notes the {noun}{'s' if plural else ''} packed the crate and {action}"
