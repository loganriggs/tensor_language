...
"""pred_a postMLP9 state andall-vocabulary scores<=1e-5relative EACHcase.
pred_b baseline-subtracted readout errors<=1e-5+1e-4abs(effect); zeroexact.
pred_c coupled prepared bytes<=.93generic EACHprefix.
...
def main():
    ...
    def attention_hook(module,args,output):
        if context['mode']=='capture':
            captures[context['row']]=dict(g=graph.routing(args[0]),v=graph.values(args[0]),z=(context['raw9']+output[0]).double())
            return output
    ...
    try:
        ...
        for i in RECIPIENTS:
            ...
            context.update(program=program,changes=changes)
            sizes=[sum(v.numel()*v.element_size() for v in p.values()) for p in [generic,program]]
            prices.append(dict(row=i,tokens=len(row['ids']),generic_bytes=sizes[0],coupled_bytes=sizes[1],ratio=sizes[1]/sizes[0]))
            ...
            for j,strengths in enumerate(STRENGTHS):
                ...
                native=forward(i,'native');native_state=context['post9']
                compiled=forward(i,'compiled');compiled_state=context['post9']
                ...
                records.append(dict(row=i,strengths=strengths,state_error=rel(compiled_state,native_state),score_error=rel(compiled,native),
    ...
    result={'pred_a':all(r['state_error']<=1e-5 and r['score_error']<=1e-5 for r in records),
            ...
            'pred_c':all(p['ratio']<=.93 for p in prices),
            ...
            'scope':'Exact coupled routing/value program prepared once per pristine context; native suffix tested over100 signed cases. Mixed strength tied to a*b. Context/source/MLPweights remain; no static model compression or independent role identification.'}
