#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_instrument pred_b_joint_reader pred_c_integrated_reader
"""Exact mixed edge replay and finite removal; opened composition diagnosis."""
import os,json,sys,time,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';OUT=A/'attention_edge_transport_native_v1_result.json'
PLAN=dict(prefix=12,suffix=232,attention=68,reader_gradients=16,directional_jvp=128)
def main():
    ...
    counts=dict(prefix=0,suffix=0,attention=0,reader_gradients=0,directional_jvp=0);groups=[];checks=[]
    ...
    for context in binding['contexts']:
        ...
        for s,t in binding['coordinates']:
            ...
            post=state+writes[label]
            def tangent(alpha):
                with torch.enable_grad():
                    _,response=torch.autograd.functional.jvp(suffix,post-alpha*delta,-delta,create_graph=False)
                counts['directional_jvp']+=1
                return response
            joint=tangent(0.);midpoint=tangent(.5)
            node=(3./5.)**.5/2
            integrated=(5*tangent(.5-node)+8*midpoint+5*tangent(.5+node))/18
            effect=removed-values[label]
            estimates=dict(baseline=-local,joint=joint,midpoint=midpoint,integrated=integrated)
            ...
            for family in dict.fromkeys(old['families']):
                ...
                errors={mode:[float((estimate-effect)[ids,o].norm()) for o in range(4)] for mode,estimate in estimates.items()}
                norms=[float(effect[ids,o].norm()) for o in range(4)]
                records.append(dict(family=family,coordinate=[s,t],errors=errors,target_norms=norms,relative_errors={mode:[e/max(n,1e-12) for e,n in zip(err,norms)] for mode,err in errors.items()},effect=effect[ids].tolist(),predictions={mode:value[ids].tolist() for mode,value in estimates.items()}))
    ...
    instrument=counts==PLAN and max(c['saved_replay'] for c in checks)<1e-10 and max(c['edge_error'] for c in checks)<1e-10
    joint_pass=all(r['errors']['joint'][0]<=max(1e-8,.1*r['target_norms'][0]) for r in records)
    integrated_pass=all(all(e<=max(1e-8,1e-4*n) for e,n in zip(r['errors']['integrated'],r['target_norms'])) for r in records)
    result=dict(plan=PLAN,counts=counts,checks=checks,groups=groups,predictions=dict(pred_a_instrument=instrument,pred_b_joint_reader=instrument and joint_pass,pred_c_integrated_reader=instrument and integrated_pass),seconds=time.perf_counter()-tic)
