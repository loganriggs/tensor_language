#!/usr/bin/env python
"""asymmetric_width_program_probe -- program v2 (8 block frames + bus, union write rule) priced with SEPARATE early width k_e
(block frames + union writes' next-frame half) and bus width k_b (U_8: late reads/writes and the bus half of the union writes),
after §2770/§2771 found the 768 cliff is the late blocks'. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_early768_bus1024_under_p06 pred_c_early640_bus1024_under_p10
#                     pred_d_early512_bus1024_under_p25 pred_e_bus_width_costs_more_than_early_width
...
Preregistration: polynomial_causal/ASYMMETRIC_WIDTH_PROGRAM_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/asymmetric_width_program_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("asymmetric_width_program_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "ASYMMETRIC_WIDTH_PROGRAM_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_width_control_probe_results.json"   # §2771
OUT = ROOT / "asymmetric_width_program_probe_results.json"
HASHES = {PREREG: "247debe7d1a1144770a065beb56c9be8604131ec13b24eab0be242bd22eb5935", PRIOR: "2f19a636206b0dae2ba86942ca50e06d523a22da7761f4f501fa784860bf47e7",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "asymmetric_width_program_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_SPLIT8_1024 = 0.0374; PRIOR_CHAIN_ONLY_1024 = 0.0572; LAM = 1e-2
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_max": 0.060, "c_max": 0.100, "d_max": 0.250}
NULLS = {"b_min": 0.100, "c_min": 0.180, "d_min": 0.400, "e_gap": 0.020}
...
def main():
    ...
    class RouteUnion2:
        """Early write confined to span(U_next[:, :kn]) ∪ span(U_bus[:, :kb]): the remainder outside U_next is replaced by its projection onto U_bus."""
        def __init__(self, s_, kn, U_next, kb, U_bus):
            self.Un = U_next[:, :kn]; self.Ub = U_bus[:, :kb]; self.mu = wheads[s_]["mu"]
        def hook(self, idx):
            pass
        def __call__(self, w, x):
            c = w - self.mu; inside = (c @ self.Un) @ self.Un.T; r = c - inside
            return self.mu + inside + (r @ self.Ub) @ self.Ub.T
    ...
    def v3(ke, kb):
        ...
        for s_ in OWN8:
            ...
            if kd == "attn":
                ...
                Un, kn = U_8, kb
            patch[s_] = Both(head(s_, ke, FB[l]), RouteUnion2(s_, kn, Un, kb, U_8))
        for s_ in SET8:
            patch[s_] = Both(head(s_, kb, U_8), Route(s_, kb, U_8, "readout", buf))
        patch[FINAL] = FinalAdd(buf, +1.0)
        return run(patch, (buf,))
    arms = {"SPLIT8_1024": split8(1024)}
    GRID = [(768, 1024)] if smoke else [(768, 1024), (640, 1024), (512, 1024), (768, 960), (768, 1088)]
    for ke, kb in GRID:
        arms[f"V3_E{ke}_B{kb}"] = v3(ke, kb); log(stage="v3", ke=ke, kb=kb, ce=round(arms[f"V3_E{ke}_B{kb}"], 4))
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    g = lambda ke, kb: arms.get(f"V3_E{ke}_B{kb}", arms["V3_E768_B1024"])
    deltas = {"bus_1024_to_960_at_e768": g(768, 960) - g(768, 1024), "early_768_to_640_at_b1024": g(640, 1024) - g(768, 1024),
              "early_640_to_512_at_b1024": g(512, 1024) - g(640, 1024), "bus_1024_to_1088_at_e768": g(768, 1088) - g(768, 1024)}
    ...
    preds = {
        ...
        'pred_b_early768_bus1024_under_p06': bool(g(768, 1024) <= BARS["b_max"]),
        'pred_c_early640_bus1024_under_p10': bool(g(640, 1024) <= BARS["c_max"]),
        'pred_d_early512_bus1024_under_p25': bool(g(512, 1024) <= BARS["d_max"]),
        'pred_e_bus_width_costs_more_than_early_width': bool(deltas["bus_1024_to_960_at_e768"] >= deltas["early_768_to_640_at_b1024"]),
    }
    nulls = {"b_null_ge_.10": bool(g(768, 1024) >= NULLS["b_min"]), "c_null_ge_.18": bool(g(640, 1024) >= NULLS["c_min"]),
             "d_null_ge_.40": bool(g(512, 1024) >= NULLS["d_min"]), "e_null_early_drop_costs_.02_more": bool(deltas["early_768_to_640_at_b1024"] - deltas["bus_1024_to_960_at_e768"] >= NULLS["e_gap"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "split": SPLIT, "grid": GRID,
           "program": "program v2 with early width k_e (block frames, union writes' next-frame half) and bus width k_b (U_8 reads/writes, bus half of union writes)",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024},
           "ce_added": arms, "deltas": deltas, "priors": {"P9_1024": 0.0389, "P9_768": 0.2419},
    ...
    if smoke:
        print(json.dumps({k: out[k] for k in ("preds", "nulls", "instrument", "ce_added", "deltas", "price")}, indent=1)); return
