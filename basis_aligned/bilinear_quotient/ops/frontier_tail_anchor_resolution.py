"""RESOLVE THE ANCHOR: IS THE .061 GAP THE REFIT-TIME / EVAL-TIME DISTINCTION? (Claude, LANE 1)

SS2893 and SS2894 both measured a large apparent improvement to the SS312 frontier -- **-0.2288 nats at scale 0.25**, reproducing across
two independent runs to **0.0003** and present on the fitting window as well as fresh -- and **neither adopted it**, because in both the
`LW := {}` anchor missed. SS2894 narrowed the cause to one number: the anchor reads **+0.1130** when `LW := {}` is applied to a FROZEN
stack, against SS2881's **+0.1740** when it is applied INSIDE the sequential refit loop, where the downstream fits adapt to it. Gap
**0.0610**.

That is the same refit-time/eval-time distinction SS2884 raised and SS2887 tested for the RANK knob, where freezing did **not** change
the answer. Here it apparently does. This rung measures both applications **in the same script against the same baseline**, which is
the one thing that can settle it:

    RUN 1  frozen stack   : baseline, LW := {} applied AFTER all eight refits, and s = 0.25
    RUN 2  refit-time     : LW := {} applied INSIDE the refit loop -- exactly SS2881's operation

If the refit-time arm lands on SS2881's +0.1740 and the frozen arm on SS2894's +0.1130, the distinction is quantified, the frozen-stack
anchor is valid for a frozen-stack curve, and the improvement can be adopted on its own footing. If the refit-time arm does NOT
reproduce SS2881, something other than this distinction is wrong and the whole scale line stays closed.
...
#                     pred_b_the_refit_time_arm_reproduces_S2881
#                     pred_c_the_frozen_arm_reproduces_S2894
#                     pred_d_the_two_applications_genuinely_differ
#                     pred_e_the_s025_arm_reproduces_the_curve

Preregistration: polynomial_causal/FRONTIER_TAIL_ANCHOR_RESOLUTION_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; TWO pipeline runs: frozen stack (3 arms) and the refit-time empty-LW arm")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_tail_anchor_resolution_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
def main():
    ...
    def _cfg(spec):
        _apply_scale('empty' if spec.get('empty') else spec.get('scale'))
...
PREREG = PT + '../polynomial_causal/FRONTIER_TAIL_ANCHOR_RESOLUTION_PREREGISTRATION.md'
PREREG_SHA = "a7b339c25760c3afc7f551d5819d5d3604e8442a4e5cd482026124b1a80f7e62"
...
S2894 = PT + 'frontier_tail_shrinkage_anchored_results.json'
S2881_SHA = "cd1a8028a8086706ba82a907436ae1acb74113d70c357d01bebaba21c81ecf95"
S2894_SHA = "4d3e7fdd91828d96cb5056e3bd3c2e5ac96140b571f2927cb25d0c5f273d37b5"
BARS = {"reproduce": 0.05, "refit_anchor": 0.02, "frozen_anchor": 0.02, "distinction": 0.04, "curve": 0.01}
NULLS = {"refit_anchor_ge": 0.05, "distinction_le": 0.01}
...
if __name__=='__main__':
    ...
    for p_, h_ in ((S2881, S2881_SHA), (S2894, S2894_SHA)):
        if _sha(p_) != h_:
            raise RuntimeError(f'frozen hash mismatch: {p_}')
    a81 = json.load(open(S2881))['summary']['cost_dropping_link_maps_nats']      # +0.1740, refit-time LW:={}
    p94 = json.load(open(S2894))['summary']
    frozen81 = p94['cost_lw_empty']                                             # +0.1130, frozen LW:={}
    curve25  = p94['cost_fresh']['s025']                                        # -0.2288
    SEL['mode']='norm'

    # RUN 1 -- the FROZEN stack: baseline, LW:={} applied AFTER all refits, and s=.25.
    print('RUN 1/2: frozen stack -- baseline / LW:={} after refits / s=0.25',flush=True)
    TAILMODE['mode']=None
    ARMS[:] = [('baseline', {}), ('frozen_empty', {'empty': True}), ('s025', {'scale': 0.25})]
    out=main()
    bF = out['baseline']['L2_F']
    c_frozen = round(out['frozen_empty']['L2_F'] - bF, 4)
    c_s025   = round(out['s025']['L2_F'] - bF, 4)

    # RUN 2 -- the REFIT-TIME arm: LW:={} applied INSIDE the refit loop, exactly as SS2881 did.
    print('RUN 2/2: refit-time -- LW:={} applied inside the refit loop (SS2881 operation)',flush=True)
    ARMS[:] = []
    TAILMODE['mode']='links'
    out2=main()
    TAILMODE['mode']=None
    # out2 is a single-arm dict when ARMS is empty
    r2 = out2['single'] if 'single' in out2 else out2
    c_refit = round(r2['L2_F'] - bF, 4)

    dev_refit  = abs(c_refit - a81)
    dev_frozen = abs(c_frozen - frozen81)
    distinction = abs(c_refit - c_frozen)
    dev_curve  = abs(c_s025 - curve25)

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = dev_refit  <= BARS['refit_anchor']
    pc = dev_frozen <= BARS['frozen_anchor']
    pd = distinction >= BARS['distinction']
    pe = dev_curve <= BARS['curve']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_refit_time_arm_reproduces_S2881':bool(pb),
           'pred_c_the_frozen_arm_reproduces_S2894':bool(pc),
           'pred_d_the_two_applications_genuinely_differ':bool(pd),
           'pred_e_the_s025_arm_reproduces_the_curve':bool(pe)}
    nulls={'b_null_the_refit_arm_does_not_reproduce_S2881':bool(dev_refit>=NULLS['refit_anchor_ge']),
           'd_null_the_distinction_is_not_the_cause':bool(distinction<=NULLS['distinction_le'])}
    res={'rung':'frontier_tail_anchor_resolution','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,'frozen_arms':out,'refit_arm':r2,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(bF,4),
                    'cost_refit_time_empty':c_refit,'S2881_refit_time':round(a81,4),
                    'refit_anchor_deviation':round(dev_refit,4),
                    'cost_frozen_empty':c_frozen,'S2894_frozen':round(frozen81,4),
                    'frozen_anchor_deviation':round(dev_frozen,4),
                    'distinction_nats':round(distinction,4),
                    'cost_s025':c_s025,'S2894_s025':round(curve25,4),
                    'curve_deviation':round(dev_curve,4),
                    'adoption_gate_all_of_a_b_c_e':bool(pa and pb and pc and pe)},
         ...
         'hashes':{PREREG:PREREG_SHA,S2881:S2881_SHA,S2894:S2894_SHA},'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {bF:.4f} vs 2.6735 (tol .05): {'HELD' if pa else 'FAILED'}")
    print(f"(b) refit-time LW:={{}} {c_refit:+.4f} vs SS2881 {a81:+.4f}, dev {dev_refit:.4f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) frozen LW:={{}} {c_frozen:+.4f} vs SS2894 {frozen81:+.4f}, dev {dev_frozen:.4f}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) distinction {distinction:.4f} >= {BARS['distinction']}: {'HELD' if pd else 'FAILED'}")
    print(f"(e) s=.25 {c_s025:+.4f} vs SS2894 {curve25:+.4f}, dev {dev_curve:.4f}: {'HELD' if pe else 'FAILED'}")
    print(f"ADOPTION GATE (a,b,c,e): {'OPEN' if (pa and pb and pc and pe) else 'CLOSED'}")
