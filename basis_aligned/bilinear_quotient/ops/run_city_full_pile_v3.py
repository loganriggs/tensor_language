...
Opened Pile precision repair; pred_g unchanged behavior and native baseline <=1e-5 absolute.
...
STEM='CITY_FULL_PILE_V3';ARMS=['native','native8_midpoint','full_reentry','compiled']+[f'null:{k}' for k in range(16)]
...
def main():
 ...
 old=torch.load(P/'CITY_FULL_PILE_V2_ARTIFACT.pt',weights_only=True)['values'];keep=[0,1]+list(range(3,20));behavior_replay=float((v[keep]-old[keep]).abs().max())
 result={'pred_g':behavior_replay<=1e-5 and max(baseline_errors)<=1e-5,'behavior_replay_max_abs':behavior_replay,'native_baseline_max_abs':max(baseline_errors),'old_analytic_max_relative_error':max(analytic_errors),'pred_a':anchor_abs<=1e-4 and anchor_rel<=1e-5 and max(joint_errors)<=1e-4 and max(outside)==0 and max(norm_errors)<=1e-5 and bool(torch.isfinite(v).all()) and count==800,
