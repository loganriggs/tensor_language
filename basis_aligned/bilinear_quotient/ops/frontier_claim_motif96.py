"""REGISTERED CLAIM: THE ECONOMICAL SHAPE (rung 245) - {motifs r96, tail r64} becomes official.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2341 measured the second tier of the
allocation menu at census +0.0676 / fresh +0.0660 / valid 8 at 60.3M pattern values. Identical rebuild;
claim bars registered.
REGISTERED PREDICTIONS (anchors: the S2341 receipt):
  (a) REPRODUCTION: |census - 0.0676| <= 0.015.
  (b) CERTIFICATES: valid >= 8.
  (c) REPRODUCTION: |L2_F - 0.0660| <= 0.015.
NULL: noise. PRICE: ~187M values (motif r96 35.4M + tail r64 24.9M patterns). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: registered claim economical shape')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_claim_motif96_results.json'
...
if __name__=='__main__':
    ...
    pa=abs(agg-0.0676)<=0.015
    pb=nv>=8
    pc=abs(L2F-0.0660)<=0.015
    ...
    print(f"(a) |census {agg:+.4f} - 0.0676| <= 0.015: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 8; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |L2F {L2F:+.4f} - 0.0660| <= 0.015: {'HELD' if pc else 'FAILED'}")
