#!/usr/bin/env python3
"""Prospective seventh is/was lexicon matched to has/had v6 occupations."""
...
def _panel(group_number: int) -> list[dict[str, Any]]:
    ...
    group_id = f"FIT:{canonical_sha256([SCHEMA, TASK_ID, 'different_readout_v7_dual', SEED, group_number])[:24]}"
    ...
    common_no_vocab = dict(seed=SEED, task_id=TASK_ID, group_number=group_number, group_id=group_id, reporter=agent, alternate_reporter=alternate, adjective="different_readout_v7_dual", object_name="moment", spec=TASK_SPEC)
...
def _prior_rows():
    return tuple(module.build_rows() for module in (v1, v2, v3, v4, v5, v6))


def _validate(rows: Sequence[Mapping[str, object]]) -> str:
    materialized = [dict(row) for row in rows]
    if materialized != _build():
        raise CandidateBankError("rows differ from sealed v7 authority")
    try:
    ...
    if len(materialized) != 64 or len(ids) != 64 or ids & old_ids or set(_AGENTS) & old_agents or any(not all(row["construction_checks"].values()) for row in materialized):
        raise CandidateBankError("v7 count, uniqueness, disjointness, agent novelty, or construction check failed")
    return digest
