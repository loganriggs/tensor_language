# IS THE FALLBACK'S PENALTY REDUCIBLE?  -- the ceiling for UNCOVERED positions.
#
# §1867 established that the fallback costs +0.55 to +0.75 nats per uncovered position, constant in
# coverage, and that even at 16,110 covered types closing it would be worth as much as §1861's entire
# iso-cost improvement. But that 0.55 is a deficit against the LIVE model, and §1867 said plainly what it
# therefore does not establish: **how much of it any per-token fallback could recover.**
#
# §1848's ceiling answers that for covered tokens -- the model's own length-1 logits, which the full-rank
# program attains EXACTLY. The same object exists for uncovered tokens: an uncovered token still has a
# length-1 forward. So the deficit decomposes:
#
#     program_unc - live_unc  =  (ceiling_unc - live_unc)  +  (program_unc - ceiling_unc)
#                                 irreducible in the class      the FALLBACK's own loss
#
# The first term is the price of being position-wise at all (§1765). The second is what a better map or
# neighbour rule could recover at zero extra storage. Nothing in the record separates them.
#
# The ceiling lookup is built only for the tokens that actually occur at uncovered scored positions --
# a few thousand, not the 50,257 vocabulary, which would need 10 GiB.
#
# Docstring and predicate block written in ONE edit (LESSONS 63); LADDER holds 'full' and nothing calls
# int() on it (LESSONS 64 and §1867's fifth tail failure).
#
# ROLES. All three. DISCOVERY ONLY. Rung 3: the open question §1867 ends on.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
# Units named per LESSONS 49: every quantity is a DEFICIT on ONE population, never a CE compared across
# two -- the error §1866 had to correct.
#   pred_a THE FALLBACK'S OWN LOSS IS LARGE: program_unc minus ceiling_unc is at least 0.30 nats on all
#          three roles. If TRUE most of §1867's 0.55-0.75 is the fallback's and a better uncovered rule
#          is worth building at zero extra storage -- the first genuinely open engineering lever this arc
#          has found. If FALSE the fallback is already near the best a per-token function can do for an
#          uncovered token, the 0.55 is mostly the price of the class itself, and §1866/§1867's "largest
#          open lever" must be restated as "largest open COST", which is not the same thing.
#   pred_b AND IT IS MOST OF THE GAP: the fallback's own loss is at least half of §1867's PUBLISHED
#          per-position gap (+0.55027 / +0.64486 / +0.75301 at this coverage). If FALSE the irreducible
#          term dominates and the recoverable part is a minority of a number I have twice called the
#          largest lever in the class.
#   pred_c AND THE COVERED ARM STILL ATTAINS ITS CEILING: covered CE equals the covered ceiling to under
#          0.001 nats -- a ninth independent confirmation of §1848, and the check that the ceiling
#          machinery is behaving on this build before its uncovered half is trusted.
#   pred_d CONTROLS: every uncovered scored position's token has a ceiling row (zero lookup misses);
#          coverage is 16,110; program CE exceeds live CE on both populations; §1867's PUBLISHED
#          uncovered fraction of 10.0% reproduces within 1pp.
...
OUT = PT + 'ops/uncovered_ceiling_results.json'
...
TRANKS = (None,)          # None = full rank; §1786/§1813 use this ladder
...
S1867_GAP_16110 = {'skip7000': 0.55027, 'skip11000': 0.64486, 'skip1200': 0.75301}
S1867_UNC_FRAC = 0.100
...
FITLO, FITHI = 0, 480      # the draw giving 16110 covered types   # 5x the fit rows -> a larger covered set
H = m.transformer.h
NCOV = 5419
...
def main():
    ...
    assert NFULL == 16110, f'coverage {NFULL} != expected 16110'
    ...
    print(f'UNCOVERED CEILING | 16110 types, ceiling for uncovered tokens too | '
    ...
    R = LADDER[0]
    # rcost() does int(r); this run's LADDER holds 'full', which has no integer rank. Inherited from
    # the iso-cost lineage where every entry was numeric -- the fifth tail-inheritance failure here.
    cost = ((36 * (NFULL * D + D) + 36 * MAP_RANK * 2 * D) / 1e6 if R == 'full' else rcost(R))
    ...
    defc = {e: curve['full'][e]['cov'] - livev[e] for e in roles2}
    defu = {e: curve['full'][e]['unc'] - liveu[e] for e in roles2}
    gap = {e: defu[e] - defc[e] for e in roles2}
    irred = {e: uceilv[e] - liveu[e] for e in roles2}          # ceiling_unc - live_unc
    fbloss = {e: curve['full'][e]['unc'] - uceilv[e] for e in roles2}   # program_unc - ceiling_unc
    share = {e: fbloss[e] / max(S1867_GAP_16110[e], 1e-9) for e in roles2}
    attain = all(abs(curve['full'][e]['cov'] - ceilv[e]) < 0.001 for e in roles2)
    pa = all(fbloss[e] >= 0.30 for e in roles2)
    pb = all(share[e] >= 0.5 for e in roles2)
    pc = attain
    pd = (NFULL == 16110 and all(abs(nfrac[e] - S1867_UNC_FRAC) <= 0.01 for e in roles2)
          and all(curve['full'][e]['cov'] > livev[e] and curve['full'][e]['unc'] > liveu[e]
                  for e in roles2))

    print(f'\n  UNCOVERED DEFICIT DECOMPOSED at {NFULL} covered types ({NU} uncovered token types):',
          flush=True)
    for e in roles2:
        print(f'    {e:10s} live_unc {liveu[e]:.5f}  ceiling_unc {uceilv[e]:.5f}  '
              f'program_unc {curve["full"][e]["unc"]:.5f}   irreducible {irred[e]:+.5f}  '
              f'FALLBACK LOSS {fbloss[e]:+.5f}  ({share[e]:.1%} of §1867 gap {S1867_GAP_16110[e]:.5f})',
              flush=True)
    print(f'\n  the FALLBACK\'s own loss is large (>=0.30) -> {pa}  ' + '  '.join(
        f'{e} {fbloss[e]:+.5f}' for e in roles2), flush=True)
    print(f'  and it is MOST of §1867\'s gap (>=50%) -> {pb}  ' + '  '.join(
        f'{e} {share[e]:.1%}' for e in roles2), flush=True)
    print(f'  and the covered arm still ATTAINS its ceiling -> {pc}  ' + '  '.join(
        f'{e} {curve["full"][e]["cov"] - ceilv[e]:+.6f}' for e in roles2), flush=True)
    print(f'  uncovered fraction {nfrac[roles2[0]]:.1%} ~ §1867 {S1867_UNC_FRAC:.1%}, '
          f'coverage {NFULL} -> control {pd}', flush=True)

    json.dump({'run': 'uncovered_ceiling', 'n_covered': NFULL, 'n_uncovered_types': NU,
               'live_uncovered': liveu, 'ceiling_uncovered': uceilv,
               'irreducible': irred, 'fallback_loss': fbloss, 'share_of_gap': share,
               'deficit_covered': defc, 'deficit_uncovered': defu, 'gap': gap,
               'uncovered_fraction': nfrac, 'attainment_exact': bool(attain),
               'predictions': {'pred_a_fallback_loss_large': bool(pa),
                               'pred_b_most_of_gap': bool(pb),
                               'pred_c_covered_attains': bool(pc),
