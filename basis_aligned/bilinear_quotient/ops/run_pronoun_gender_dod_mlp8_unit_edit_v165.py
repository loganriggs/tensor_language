#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_instrument_replays_native pred_b_two_units_at_noun_are_live pred_c_two_units_beat_random_unit_pairs pred_d_two_units_selective pred_e_all_positions_removes_more
"""Pronoun gender he/she DoD (v165): EDIT the two MLP-8 units that v164 nominated (3152, 3943) -- folds nominate, edits decide (better_circuits).
...
PREDICTIONS (scored as written; failures preserved; priors unsure -- 9.6 is one of four heads and MLP 8 one of its inputs)
    ...
    pred_b_two_units_at_noun_are_live     fraction >= 0.10 and positive >= 0.75
    pred_c_two_units_beat_random_unit_pairs  damage > max of 16 random 2-unit zeroings at the noun
    pred_d_two_units_selective            each reader |move| <= null mean |move| + 0.25 x damage
...
OUT = ROOT / "circuits/followups/pronoun_gender_dod_mlp8_unit_edit_v165_result.json"
V164 = ROOT / "circuits/followups/pronoun_gender_dod_mlp8_unit_census_v164_result.json"
CANDIDATE_ID = "pronoun_gender.he_vs_she.dod_mlp8_unit_edit_v165"
LAYER, UNITS = 8, (3152, 3943)
NULL_SEEDS = tuple(range(6501, 6517))
LIVE_FRACTION, LIVE_POSITIVE, GATE_RATIO, MORE, INSTRUMENT_TOL = 0.10, 0.75, 0.25, 1.5, 1e-4
FORWARDS_MAX = 46
PREDICTIONS = {"pred_a_instrument_replays_native": "<= 1e-4", "pred_b_two_units_at_noun_are_live": "live", "pred_c_two_units_beat_random_unit_pairs": "> random max", "pred_d_two_units_selective": "three gates", "pred_e_all_positions_removes_more": ">= 1.5 x noun-only"}
...
def main() -> None:
    ...
    for seed in NULL_SEEDS:
        gen = torch.Generator(device="cpu").manual_seed(seed); units = tuple(torch.randperm(4608, generator=gen)[:2].tolist())
    ...
    predictions = {"pred_a_instrument_replays_native": instrument <= INSTRUMENT_TOL, "pred_b_two_units_at_noun_are_live": two["fraction"] >= LIVE_FRACTION and two["positive"] >= LIVE_POSITIVE,
                   "pred_c_two_units_beat_random_unit_pairs": two["damage"] > null_max, "pred_d_two_units_selective": all(gates.values()),
                   "pred_e_all_positions_removes_more": arms["two_units_all_positions"]["damage"] >= MORE * two["damage"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pronoun_gender_dod_mlp8_unit_edit_result_v165", "candidate_id": CANDIDATE_ID, "plan": plan, "instrument_max_abs_error": instrument, "arms": arms, "null_damage_max": null_max, "null_moves": null_moves,
