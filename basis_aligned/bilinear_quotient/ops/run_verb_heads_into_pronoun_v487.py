#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_verb_heads_write_little_into_the_pronoun_slot pred_c_pronoun_heads_replay pred_d_union_adds pred_e_verb_heads_a_small_fraction
"""Do the verb heads write into the pronoun slot (v487)? v485 / v486: each readout's five heads carry most of the attention channel into their own answer
slot. Cross-check from the answer side at the PRONOUN slot: the five verb heads' (11.3 / 5.3 / 9.7 / 7.8 / 13.1) writes into the pronoun slot swapped
jointly, the five pronoun heads' writes (replay 0.803), and the union of both sets; 122 natural pairs.
PREDICTIONS (scored as written; failures preserved; priors from v453 / v485)
    pred_a_baseline_replays                        the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_verb_heads_write_little_into_the_pronoun_slot  the five verb heads' joint write swap closes <= 0.05 of the they - he margin gap
    pred_c_pronoun_heads_replay                    the five pronoun heads' joint write swap closes 0.803 +- 0.03
    pred_d_union_adds                              the union closes within 0.04 of the sum of the two sets
    pred_e_verb_heads_a_small_fraction             the verb heads' joint effect is <= 0.07 x the pronoun heads' joint effect
...
OUT = ROOT / "circuits/followups/verb_heads_into_pronoun_v487_result.json"
CANDIDATE_ID = "chain.verb_heads_into_pronoun_v487"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, VERB_MAX, FIVE_V485, BAND, ADD_TOL, RATIO_MAX, UNIT = 1e-3, 0.05, 0.803, 0.03, 0.04, 0.07, 701
FIVE_W = {9: [6], 10: [1, 5], 12: [4], 15: [1]}; FIVE_V = {11: [3], 5: [3], 9: [7], 7: [8], 13: [1]}; UNION = {l: sorted(set(FIVE_W.get(l, [])) | set(FIVE_V.get(l, []))) for l in set(FIVE_W) | set(FIVE_V)}
import random
...
FORWARDS_MAX = 17
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_verb_heads_write_little_into_the_pronoun_slot": "<= 0.05", "pred_c_pronoun_heads_replay": "0.803 +- 0.03", "pred_d_union_adds": "within 0.04 of the sum", "pred_e_verb_heads_a_small_fraction": "<= 0.07 x"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "verb_max": VERB_MAX, "five_v485": FIVE_V485, "band": BAND, "add_tol": ADD_TOL, "ratio_max": RATIO_MAX, "unit": UNIT}}
    ...
    MODES = {"FIVE_V": ({}, (0,)), "FIVE_W": ({}, (0,)), "UNION": ({}, (0,))}; SETS_W = {"FIVE_V": FIVE_V, "FIVE_W": FIVE_W, "UNION": UNION}; forwards = 0
    def logits_margin(x):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_verb_heads_write_little_into_the_pronoun_slot": abs(mc["FIVE_V"]) <= VERB_MAX, "pred_c_pronoun_heads_replay": abs(mc["FIVE_W"] - FIVE_V485) <= BAND,
                   "pred_d_union_adds": abs(mc["UNION"] - (mc["FIVE_V"] + mc["FIVE_W"])) <= ADD_TOL, "pred_e_verb_heads_a_small_fraction": abs(mc["FIVE_V"]) <= RATIO_MAX * mc["FIVE_W"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "verb_heads_into_pronoun_result_v487", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
