"""MIDDLES TRAJECTORY DOWN-REFIT IN THE FRONTIER CONFIG (rung 129): the trajectory move at c4-c9.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The S2144 K-reduction at c4-c9 costs
~0.15 solo (S2198) plus its interactions. The trajectory-teacher move is 2-for-2 (front Downs S2210, tail
dicts S2221). Here each pruned middle's Down map (+bias) is refit sequentially in the true config frame
(CP front + motifs installed, earlier refit middles in place), targeting the REAL-trajectory m_li outputs.
Otherwise the exact rung-127 build (trajectory aXL dicts refit downstream of the new middles).

REGISTERED PREDICTIONS (anchors: rung-127 receipt L2_F +1.8765, census +1.9474, increment +0.3256):
  (a) CENSUS RECOVERY: census agg <= 1.87 (>= 0.08 recovered).
  (b) FRONTIER MOVES: L2_F fresh <= 1.85.
  (c) SANITY: increment in [0.20, 0.55] AND census >= 1.4.
NULL: the middle K-cost is unit-capacity, not readout frame (recovery < 0.03; the closed K-metric lore
extends to targets). PRICE: none (same Down shapes + one D-bias per site).
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: middles trajectory Down-refit')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_mid_traj_results.json'
    ...
    if SEL.get('mid_traj'):
        ...
        for _li in MIDL:
            ...
            for _i in range(CA,CB,4):
                ...
                m(cur['idx'],_bb[:,1:].contiguous())
            for h in hs: h.remove()
            _Xd=torch.cat(_Xs); _Y=torch.cat(_YRm[_li])
...
if __name__=='__main__':
    ...
    print('ARM: rung-127 build + middles trajectory Down-refit',flush=True)
    main()
    ...
    print(f"(b) L2F {L2F:+.4f} <= 1.85: {'HELD' if pb else 'FAILED'}")
