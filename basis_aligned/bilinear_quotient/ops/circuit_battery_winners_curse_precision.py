"""circuit_battery_winners_curse_precision -- SS2864's two failing clauses BOTH landed between bar and null; more data decides.

SS2864 selected the most-selective component on FIT and evaluated it on TEST. Two clauses failed, and both failed into the undecided
zone rather than onto the null:

    pred_a  inflation = .063        bar >= .15   null <= .05    -> mild, but not "none"
    pred_b  pick - named writer = -.020   bar >= 0   null <= -.05  -> indistinguishable, in either direction

A measurement that lands between its own bar and its own null has not decided anything, and SS2865 has just shown this design's
sample-size sensitivity is real (SS2863's pred_e flipped from .556 to .609 at 2.7x the rows while its pred_c got worse). This rung is
the SAME design at **PER_CELL=24** (1.5x SS2864's rows), everything else identical and **every bar carried over verbatim**.
Re-registering a bar for a replication would make the replication meaningless.

The clauses that DID resolve in SS2864 -- pred_c (selection beats random at -.266) and pred_d (argmin agrees on 0 of 7) -- are re-run
unchanged as instrument checks: if either moves, the replication itself is suspect.
...
Preregistration: polynomial_causal/CIRCUIT_BATTERY_WINNERS_CURSE_PRECISION_PREREGISTRATION.md
"""
...
SELF = Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/"
            "circuit_battery_winners_curse_precision.py")
...
ROOT = R.ROOT
PREREG = R.POLY / "CIRCUIT_BATTERY_WINNERS_CURSE_PRECISION_PREREGISTRATION.md"
...
RUNG = "circuit_battery_winners_curse_precision"
SMOKE = os.environ.get("SURROGATE_SMOKE") == "1"
OUT = ROOT / (f"{RUNG}_smoke_results.json" if SMOKE else f"{RUNG}_results.json")
HASHES = {PREREG: "af831e25a2b3bd36d2ff3bb255d362d28f599160cecd4f167a3cc207ad43cb20", NULLRES: "b4bf278a10fea17e656db30e2d138fb219dc3c8cf5319282b307df5b08aa6295",
...
def main():
    ...
    for tid in tasks:
        ...
        tv = {k: v["selectivity"] for k, v in per_split["TEST"].items() if v["selectivity"] is not None}
    ...
    for tid, ps in results.items():
        ...
        tv = {k: v["selectivity"] for k, v in ps["TEST"].items() if v["selectivity"] is not None}
        if not fv or not tv:
            continue
