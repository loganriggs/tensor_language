"""FRONT ERROR ANATOMY (rung 105): where do the front replacements err - DC vs signal, and on which classes?

CONVENTION (S2135): errors here are output-space (not CE): E = replaced_output - real_module(same input), computed
in the full-front frame (all six front replacements installed; middles at 4608; no tailE; all attention real) on
200 census rows. rel = rms(E)/rms(real output); dcfrac = ||mean_pos(E)|| / rms_pos(||E||) (S2190/S2191 DC frame).
This is the repair-targeted-compilation diagnostic: it says WHAT a table repair must fix (means vs per-class signal).

REGISTERED PREDICTIONS (arms named):
  (a) TABLES PRESERVE DC: dcfrac <= 0.15 for >= 5 of 6 sites {a0,m0E,a1v,m1,m2E,m3E}.
  (b) RETRIEVAL CONCENTRATION: at the max-rel site, mean per-class rel over {ind,other} >= 1.3 x mean over the
      const classes {digit,bclose,sentend,comma,name,rep} (S2145's retrieval-price law, output-space form).
  (c) SANITY: every site rel in [0.02, 1.5].
NULL: errors are DC-contaminated and class-uniform (repair must fix means everywhere). PRICE: none (diagnostic).
Tripwire: INSTRUMENT FAIL if max rel < 0.02 (replacements inert - the S2189-class bug). Self-reviewed."""
import json, sys, time, os
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
if os.environ.get('BQLIB_DRYRUN')=='1':
    _bq='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
    _need=['frontier_certificate5_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: front error anatomy (DC vs signal, per class)')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_front_anatomy_results.json'
...
def main():
    ...
    del cur['clsmap']
    if SEL.get('front_anatomy'):
        _AR=SEL.get('ext_rows',FR)[:200]
        cur['clsmap']=classify2(_AR).to(DEV)
        _SITES=[('a0',m.transformer.h[0].attn.c_v),('m0E',m.transformer.h[0].mlp),
                ('a1v',m.transformer.h[1].attn.c_v),('m1',m.transformer.h[1].mlp),
                ('m2E',m.transformer.h[2].mlp),('m3E',m.transformer.h[3].mlp)]
        _cx={k:[] for k,_ in _SITES}; _cy={k:[] for k,_ in _SITES}
        hs=install(['a0','m0E','a1v','m1','m2E','m3E'])
        for _nm,_mod in _SITES:
            def _mk(nm=_nm):
                def h(mo,i_,o_):
                    _cx[nm].append(i_[0].detach().reshape(-1,D).to(torch.float16).cpu())
                    _oo=o_[0] if isinstance(o_,tuple) else o_
                    _cy[nm].append(_oo.detach().reshape(-1,D).to(torch.float16).cpu())
                return h
            hs.append(_mod.register_forward_hook(_mk()))
        for _i in range(0,_AR.shape[0],4):
            _bb=_AR[_i:_i+4,:257].to(DEV)
            cur['idx']=_bb[:,:-1].contiguous()
            cur['mode']='oracle'; cur['lab']=cur['clsmap'][_i:_i+4].reshape(-1)
            m(cur['idx'], _bb[:,1:].contiguous())
        for h in hs: h.remove()
        _clsf=cur['clsmap'].reshape(-1).cpu()
        _anat={}
        for _nm,_mod in _SITES:
            _X=torch.cat(_cx[_nm]).to(DEV); _Yr=torch.cat(_cy[_nm]).to(DEV).float()
            if _clsf.shape[0]!=_X.shape[0]:
                raise SystemExit(f'INSTRUMENT FAIL: {_nm} cap {_X.shape[0]} != cls {_clsf.shape[0]}')
            _dt=next(_mod.parameters()).dtype
            with torch.no_grad():
                _Yt=torch.cat([_mod(_X[_j:_j+8192].to(_dt)).float() for _j in range(0,_X.shape[0],8192)])
            _E=_Yr-_Yt
            _rms=lambda t: float(t.float().pow(2).mean().sqrt())
            _rel=_rms(_E)/max(_rms(_Yt),1e-9)
            _dc=float(_E.mean(0).norm())/max(float(_E.pow(2).sum(1).mean().sqrt()),1e-9)
            _pc={}
            for _k in range(10):
                _mkk=_clsf==_k
                if int(_mkk.sum())>0:
                    _pc[CLS[_k]]=round(_rms(_E[_mkk])/max(_rms(_Yt[_mkk]),1e-9),4)
            _anat[_nm]={'rel':round(_rel,4),'dcfrac':round(_dc,4),'per_class_rel':_pc,'n':int(_X.shape[0])}
            print(f"  {_nm:4s} rel {_rel:.4f}  dcfrac {_dc:.4f}",flush=True)
            _cx[_nm]=None; _cy[_nm]=None
        del cur['clsmap']
        SEL['anat_result']=_anat
...
if __name__=='__main__':
    ...
    SEL['front_anatomy']=True; SEL['ext_rows']=CROWS
    print('ARM: error anatomy of the six front replacements on 200 census rows',flush=True)
    main()
    A=SEL['anat_result']
    if max(v['rel'] for v in A.values())<0.02:
        raise SystemExit('INSTRUMENT FAIL: all front replacements inert (max rel < 0.02)')
    dcok=sum(1 for v in A.values() if v['dcfrac']<=0.15)
    top=max(A,key=lambda k:A[k]['rel'])
    pcr=A[top]['per_class_rel']
    linkm=sum(pcr.get(c,0.0) for c in ('ind','other'))/2
    cvals=[pcr[c] for c in ('digit','bclose','sentend','comma','name','rep') if c in pcr]
    constm=sum(cvals)/max(len(cvals),1)
    pa=dcok>=5
    pb=linkm>=1.3*constm
    pc=all(0.02<=v['rel']<=1.5 for v in A.values())
    res={'sites':A,'max_rel_site':top,'link_mean_rel':round(linkm,4),'const_mean_rel':round(constm,4),
         'convention':'E = replaced - real(same input), full-front frame, 200 census rows; rel=rms(E)/rms(real)',
         'pred_a_dc_clean':bool(pa),'pred_b_retrieval_concentration':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'max-rel site {top}; link mean rel {linkm:.4f} vs const {constm:.4f}; dc-clean sites {dcok}/6')
    print(f"(a) dcfrac <= 0.15 on {dcok}/6 (bar 5): {'HELD' if pa else 'FAILED'}")
    print(f"(b) link {linkm:.4f} >= 1.3 x const ({1.3*constm:.4f}) at {top}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) all rel in [0.02, 1.5]: {'HELD' if pc else 'FAILED'}")
