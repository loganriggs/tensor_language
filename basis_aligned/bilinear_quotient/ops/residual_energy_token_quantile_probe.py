#!/usr/bin/env python
"""residual_energy_token_quantile_probe -- per-token distribution (median / p99 / max / top-1% share / position-0 share) of the
squared norms whose MEANS §2762 tabulated (post-blend residual, pre-blend residual, attention write, MLP write per block), so the
energy budget is re-stated at the typical token and the massive-token share is known. One instrument CE arm. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_massive_tokens_carry_mlp0 pred_c_position0_is_the_sink
#                     pred_d_mlp0_is_large_at_the_typical_token pred_e_blend_is_minor_at_the_typical_token

SIGN CONVENTION (§2135): the CE number is CE ADDED ABOVE THE REAL MODEL on held-out docs 0-63 -- LOWER IS BETTER. Norms are squared
per token (||x0||^2 = 1152). Descriptive; nothing installs into the §312 frontier.
Preregistration: polynomial_causal/RESIDUAL_ENERGY_TOKEN_QUANTILE_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/residual_energy_token_quantile_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("residual_energy_token_quantile_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "RESIDUAL_ENERGY_TOKEN_QUANTILE_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "early_residual_energy_budget_probe_results.json"   # §2762
OUT = ROOT / "residual_energy_token_quantile_probe_results.json"
HASHES = {PREREG: "7e9cd1cc6c897eddfb4b5b10d78647378e243f9e6823e4c97a0687af4a7a95d2", PRIOR: "fd2f9981ced38a278731af3e1139f847b0ac03d57d84ef2950c4c47f750f8a03",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "residual_energy_token_quantile_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_min": 0.50, "c_min": 0.30, "d_min": 100 * 1152.0, "e_max": 0.25}
NULLS = {"b_max": 0.05, "c_max": 0.02, "d_max": 10 * 1152.0, "e_min": 1.0}
...
def main():
    ...
    def qcb(s_, w):
        if s_[0] in ("attn", "mlp", "pre", "post"):
            sq.setdefault(s_, []).append((w.double() ** 2).sum(-1).cpu())
    for i in range(0, fit_rows.shape[0], CH):
        forward(m, fit_rows[i:i + CH, :TI], collect=qcb)
    Q = {}
    for s_, parts in sq.items():
        A = torch.cat(parts, 0); flat = A.reshape(-1); tot = float(flat.sum()); ntop = max(1, int(round(0.01 * flat.numel())))
        Q[f"{s_[0]}{s_[1]}"] = {"mean": float(flat.mean()), "median": float(flat.median()), "p99": float(flat.quantile(0.99)), "max": float(flat.max()),
                               "top1pct_share": float(flat.topk(ntop).values.sum() / tot), "pos0_share": float(A[:, 0].sum() / tot), "n_tokens": int(flat.numel())}
        sq[s_] = A
    lam = {l: (float(m.transformer.h[l].lambdas[0]), float(m.transformer.h[l].lambdas[1])) for l in range(NL)}
    r_tok = {l: float((lam[l][1] ** 2 * D / (lam[l][0] ** 2 * sq[("pre", l - 1)].reshape(-1).clamp_min(1e-12))).median()) for l in range(1, NB)}
    r_med_2_7 = float(torch.tensor([r_tok[l] for l in range(2, NB)]).median())
    log(stage="quant", pre0=Q["pre0"], mlp0=Q["mlp0"], r_tok={l: round(v, 4) for l, v in r_tok.items()},
        med_pre=[round(Q[f"pre{l}"]["median"]) for l in range(NL - 1)], top1_pre=[round(Q[f"pre{l}"]["top1pct_share"], 3) for l in range(NL - 1)],
        pos0_pre=[round(Q[f"pre{l}"]["pos0_share"], 3) for l in range(NL - 1)])
    ...
    preds = {
        ...
        'pred_b_massive_tokens_carry_mlp0': bool(Q["pre0"]["top1pct_share"] >= BARS["b_min"]),
        'pred_c_position0_is_the_sink': bool(Q["pre0"]["pos0_share"] >= BARS["c_min"]),
        'pred_d_mlp0_is_large_at_the_typical_token': bool(Q["mlp0"]["median"] >= BARS["d_min"]),
        'pred_e_blend_is_minor_at_the_typical_token': bool(r_med_2_7 <= BARS["e_max"]),
    }
    nulls = {"b_null_le_.05": bool(Q["pre0"]["top1pct_share"] <= NULLS["b_max"]), "c_null_le_.02": bool(Q["pre0"]["pos0_share"] <= NULLS["c_max"]),
             "d_null_le_11520": bool(Q["mlp0"]["median"] <= NULLS["d_max"]), "e_null_ge_1": bool(r_med_2_7 >= NULLS["e_min"])}
    gaps = {"pre0_top1pct_share": Q["pre0"]["top1pct_share"], "pre0_pos0_share": Q["pre0"]["pos0_share"], "mlp0_median": Q["mlp0"]["median"], "r_tok_median_2_7": r_med_2_7}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           "sign_convention": "CE ADDED above the real model on held-out docs 0-63 (FRESH split; LOWER IS BETTER); norms are squared per token, ||x0||^2 = 1152",
           ...
           "ce_added": arms, "gaps": gaps, "quantiles": Q, "lambdas": {str(l): v for l, v in lam.items()}, "blend_ratio_r_typical_token": {str(l): v for l, v in r_tok.items()},
