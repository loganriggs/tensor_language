"""tailE REBUILD IN THE FRONTIER CONFIG (rung 128): the last table-front-frame residue, with trajectory targets.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2221 frontier: L2_F +1.8765, census
+1.9474 - but its tailE (tail-MLP span dictionaries m10-17) is still fit inside build_arm under the OLD table
front and without motifs (the S2216 stated limitation, priced ~0.14). Here tailE is rebuilt after the swap:
inputs captured in the true config frame (CP front + motif hooks), targets from a REAL pass (the trajectory
move, 2-for-2 so far), probe/spans unchanged. Trajectory aXL tail dicts kept (rung-127 build otherwise).

REGISTERED PREDICTIONS (anchors: rung-127 receipt L2_F +1.8765, census +1.9474, increment +0.3256):
  (a) CENSUS RECOVERY: census agg <= 1.85 (>= 0.10 recovered).
  (b) FRONTIER MOVES: L2_F fresh <= 1.83.
  (c) SANITY: increment in [0.20, 0.55] AND census >= 1.4. Certificate rows saved (descriptive).
NULL: the tailE frame/target is immaterial (recovery < 0.03). PRICE: none (same shapes).
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: tailE rebuild in frontier config')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_taile_rebuild_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['tailE_rebuild']=True; SEL['tailE_traj']=True
    print('ARM: rung-127 build + tailE rebuilt (config-frame inputs, trajectory targets)',flush=True)
    main()
    ...
    print(f"(b) L2F {L2F:+.4f} <= 1.83: {'HELD' if pb else 'FAILED'}")
    print(f"(c) increment in [0.20, 0.55] and census >= 1.4: {'HELD' if pc else 'FAILED'}")
