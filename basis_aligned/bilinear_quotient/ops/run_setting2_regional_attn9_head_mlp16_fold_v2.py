...
BINDING = P / "SETTING2_REGIONAL_ATTN9_HEAD_MLP16_FOLD_V2_BINDING.json"
OUT = ROOT / "basis_aligned/bilinear_quotient/circuits/fast_screens/setting2_regional_attn9_head_mlp16_fold_v2_result.json"
...
def load_bound() -> tuple[dict, list[dict]]:
    ...
    files = {"preregistration": PREREG, "rows": ROWS,
             ...
             "correction": P / "SETTING2_REGIONAL_ATTN9_HEAD_MLP16_FOLD_V2_CORRECTION.md",
             "invalid_v1": ROOT / "basis_aligned/bilinear_quotient/circuits/fast_screens/setting2_regional_attn9_head_mlp16_fold_v1_result.json"}
...
def plan() -> dict:
    ...
    return {"schema": "setting2_regional_attn9_head_mlp16_fold_v2_plan",
...
def main() -> None:
    ...
    projected_attention9 = sum(heads)
    direct = cross(projected_attention9)
    native_attention_cross_error = rel(direct, cross(coefficient9 * attention9))
    ...
    result = {"schema": "setting2_regional_attn9_head_mlp16_fold_v2_result",
              ...
              "metrics": {"head_write_reconstruction_relative_error": head_reconstruction_error,
                          ...
                          "native_rounded_attention_cross_relative_error": native_attention_cross_error,
