# IS THE TRACKING UNIFORM ACROSS POSITION? -- testing §1765 behaviourally on an unused axis.
#
# Everything from §1884 to §1909 has pooled scored positions >= 64 and never split them. That leaves a
# clean, unexamined prediction. §1765 established that a fully compiled program is a pure function of the
# CURRENT TOKEN -- substituting every site deletes attention, and cross-position Jacobians are exactly
# zero. The live model is not: it accumulates context as the sequence proceeds.
#
# So the two predictors should diverge WITH POSITION. Early in a sequence the model has little context
# and is closer to being a function of the current token itself; late in the sequence it has a great deal
# and the compiled program cannot follow. **If §1765 has a behavioural signature, it is that agreement
# should be highest at the earliest scored positions and fall as context accumulates.** No section has
# looked, and the answer is a direct behavioural test of the arc's founding structural result.
#
# Selection: rung 3 is exhausted in the attention line (§1909 closed it) and the remaining named question
# -- uneven per-site rank allocation -- has been with Codex since 07:57Z and is untouched by me. This is a
# fresh axis in my own lane that duplicates nothing on theirs.
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY. Rung 3, new axis.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
# Every quantity is COVERED-arm and names its population per LESSON 71; the baseline is computed in-run
# and reported as an enrichment, never anchored across coverages.
#   pred_a AGREEMENT FALLS WITH POSITION: the earliest position bucket has higher enrichment than the
#          latest, on all three roles. This is §1765's behavioural signature. If FALSE the compiled
#          program tracks the model no better early than late, which would say the agreement measured
#          since §1885 is insensitive to how much context the model is using -- a genuinely surprising
#          result about what the 6-8x is made of, and the more interesting outcome.
#   pred_b AND THE EFFECT IS MATERIAL: the earliest bucket exceeds the latest by at least 1.0x enrichment
#          on all three roles. Separates a real gradient from a detectable one; the pooled figure is
#          ~7.2x, so 1.0x is about 14% of it. If FALSE the direction may hold while the magnitude is
#          negligible, and the pooled number is a fair summary after all.
#   pred_c AND THE LIVE MODEL IMPROVES WITH POSITION: its top-1 accuracy in the latest bucket exceeds the
#          earliest, on all three roles. A known-answer check on the position axis itself -- more context
#          must help the real model -- so that a null result in pred_a cannot be blamed on a broken
#          split. §1889 is why this is registered.
#   pred_d CONTROLS: coverage is exactly 16,110; the position buckets partition every scored covered
#          position; and the covered-arm baseline is computed here and reported as an enrichment.
...
OUT = PT + 'ops/tracking_by_position_results.json'
...
def compare_by_bucket(rows, hooks, seenmask=None):
    ...
    for i in range(0, rows.shape[0], 8):
        ...
        hib = (f >= BUCKETS[-1][0])
        pos = torch.arange(64, 64 + cur.shape[1], device=DEV).unsqueeze(0).expand_as(cur)
        for a4, b4 in PB:
            k4 = f'p{a4}_{b4}'
            m5 = iscov & (pos >= a4) & (pos <= b4)
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'TRACKING BY POSITION | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    first, last = BK[0], BK[-1]
    drop = {e: (cl2(e, first, 'enrichment') or 0) - (cl2(e, last, 'enrichment') or 0) for e in roles}
    pa = all(drop[e] > 0 for e in roles)
    pb = all(drop[e] >= 1.0 for e in roles)
    pc = all(cl2(e, last, 'live_acc') > cl2(e, first, 'live_acc') for e in roles)
    pd = (ncov == NCOV
          and all(sum(cl2(e, k4, 'n') for k4 in BK) > 0 for e in roles))

    print(f'\n  enrichment and live accuracy by scored position, covered arm:', flush=True)
    for e in roles:
        print(f'    {e:10s} ' + '  '.join(
            f'{k4.replace("p", "")[:7]} n{cl2(e, k4, "n"):6d} '
            f'{(cl2(e, k4, "enrichment") or 0):5.2f}x live {cl2(e, k4, "live_acc"):5.1%}'
            for k4 in BK), flush=True)
    print(f'\n  AGREEMENT FALLS WITH POSITION -> {pa}  ' + '  '.join(
        f'{e} {(cl2(e, first, "enrichment") or 0):.2f} -> {(cl2(e, last, "enrichment") or 0):.2f}'
        for e in roles), flush=True)
    print(f'  and the effect is MATERIAL (>= 1.0x) -> {pb}  ' + '  '.join(
        f'{e} {drop[e]:+.2f}x' for e in roles), flush=True)
    print(f'  and the LIVE model improves with position -> {pc}  ' + '  '.join(
        f'{e} {cl2(e, first, "live_acc"):.1%} -> {cl2(e, last, "live_acc"):.1%}' for e in roles),
        flush=True)
    print(f'  coverage {ncov}, buckets partition -> control {pd}; baseline ' + '  '.join(
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_agreement_falls_program_concentrated_on_frequent_targets': bool(pa),
                          'pred_b_effect_material_more_concentrated_than_live': bool(pb),
                          'pred_c_live_improves_keeps_60pc_on_the_top_bucket': bool(pc),
