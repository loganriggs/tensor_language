#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_singles_sum_to_mid pred_c_each_block_small pred_d_block_5_rest_matches pred_e_all_replays
"""The MID blocks singly at the noun's own position (v541). v540: on the 43 adjacent rows the blocks outside the five verb heads' blocks write
EARLY 0.013, MID {6, 8, 10, 12} 0.053, LATE 0.032 of the agreement gap into the noun's own position. Here each mid block's attention write swapped
singly (all nine heads), plus ALL for the replay.
PREDICTIONS (scored as written; failures preserved; priors from v540)
    pred_a_baseline_replays        the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_singles_sum_to_mid      B6 + B8 + B10 + B12 = 0.053 +- 0.03
    pred_c_each_block_small        every single mid block closes <= 0.05
    pred_d_block_5_rest_matches    unused slot kept for the gate: block 12 (the pronoun reader 12.4's block) is the largest of the four. Prior: unsure.
    pred_e_all_replays             ALL closes 0.313 +- 0.05 (v539 / v540)
PRICE (registered maximum): 2 batches x 6 passes = 12 forwards; 0 backwards; 0 fits. Bar <= 13.
"""
...
OUT = ROOT / "circuits/followups/mid_blocks_adjacent_v541_result.json"
CANDIDATE_ID = "chain.mid_blocks_adjacent_v541"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, MID_V540, MID_BAND, ALL_V502, BAND, SINGLE_MAX, UNIT = 1e-3, 0.053, 0.03, 0.313, 0.05, 0.05, 701
...
FORWARDS_MAX = 13
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_singles_sum_to_mid": "0.053 +- 0.03", "pred_c_each_block_small": "<= 0.05 each", "pred_d_block_5_rest_matches": "block 12 largest", "pred_e_all_replays": "0.313 +- 0.05"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "mid_v540": MID_V540, "mid_band": MID_BAND, "all_v502": ALL_V502, "band": BAND, "single_max": SINGLE_MAX, "unit": UNIT}}
    ...
    GRP = {"B6": (6,), "B8": (8,), "B10": (10,), "B12": (12,)}
    MODES = {m: ({}, (0,)) for m in ("B6", "B8", "B10", "B12", "ALL")}; SETS_W = {m: {l: list(range(N_HEAD)) for l in GRP[m]} for m in GRP}; SETS_W["ALL"] = ALL_H; forwards = 0
    def logits_margin(x):
    ...
    singles = {m: mc[m] for m in ("B6", "B8", "B10", "B12")}; tot = sum(singles.values())
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_singles_sum_to_mid": abs(tot - MID_V540) <= MID_BAND, "pred_c_each_block_small": all(abs(v) <= SINGLE_MAX for v in singles.values()),
                   "pred_d_block_5_rest_matches": max(singles, key=singles.get) == "B12", "pred_e_all_replays": abs(mc["ALL"] - ALL_V502) <= BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "mid_blocks_adjacent_result_v541", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
