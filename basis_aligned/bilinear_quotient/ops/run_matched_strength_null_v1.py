#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_instrument pred_b_strength_match pred_c_collateral_advantage
"""Matched-strength native selective intervention control; frozen derivatives."""
import os,json,time,sys,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';OUT=A/'matched_strength_null_v1_result.json'
PREDICTIONS=dict(pred_a_instrument='native versusdouble effects andoldunit/null replay1e-4;counts',pred_b_strength_match='matched_B versusnull_full native number vector relativeerror<=10%everycell',pred_c_collateral_advantage='null collateral<=10%number allcells and<=half matched collateral everymaterialcell;requires strengthmatch')
PLAN=dict(prefix=24,double_suffix=64,native_suffix=64,gradient=0,hessian=0,fits=0,predictions=PREDICTIONS)
...
def main():
    ...
    oldpath=A/'five_source_modal_null_v1_result.json';old=json.loads(oldpath.read_text())
    oldcontexts={(c['panel'],c['role'],c['template']):c for c in old['contexts']}
    oldrecords={(c['panel'],c['role'],c['family'],c['arm']):c for c in old['records']}
    matched=json.loads((P/'MATCHED_STRENGTH_AMPLITUDES_V1.json').read_text());assert hashlib.sha256(oldpath.read_bytes()).hexdigest()==matched['source_sha256']
    matches={(c['panel'],c['role'],c['template']):c for c in matched['contexts']}
    for panel,digest in [('opposite','1869db8a18d4c89c28fa81a6bb57a19497e6f8692831eb4391f5d23c84556d73'),('congruent','bfba23ab066beb14f5cd469edce137db7ec1e8ccc26bfc36b66f896426b17511')]:
        ...
        for template in dict.fromkeys(r['template'] for r in rows):
            ...
            for role,key in [('subject','subject_position'),('attractor','control_position')]:
                ...
                arms=dict(unitB=torch.tensor([0,0,1,1,1],device='cuda',dtype=torch.float64).expand_as(zero),null_full=torch.tensor(previous['candidate_amplitudes'],device='cuda',dtype=torch.float64),matched_B=torch.tensor(matches[(panel,role,template)]['amplitudes'],device='cuda',dtype=torch.float64))
                for arm,av in arms.items():
                    ...
                    for family in dict.fromkeys(r['family'] for r in entries):
                        ...
                        if arm!='matched_B':oldreplays.append(float((y-torch.tensor(oldrecords[(panel,role,family,arm)]['target'],device='cuda')).abs().max()))
                        records.append(dict(panel=panel,role=role,family=family,arm=arm,target=y.tolist(),number_error=float((prediction[ids,0]-y[:,0]).norm()/den),modal_error=((prediction[ids,1:]-y[:,1:]).norm(dim=0)/den).tolist(),number_norm=float(den),native_capability=float((baseline[ids,0]>0).double().mean())))
    instrument=max(replays)<=1e-4 and max(oldreplays)<=1e-4 and counts=={k:PLAN[k] for k in counts}
    lookup={(r['panel'],r['role'],r['family'],r['arm']):r for r in records};comparisons=[]
    for row in records:
        if row['arm']!='null_full':continue
        null=np.array(row['target']);control=np.array(lookup[(row['panel'],row['role'],row['family'],'matched_B')]['target']);budget=max(np.linalg.norm(null[:,0]),1e-30)
        strength=np.linalg.norm(control[:,0]-null[:,0])/budget;nmodal=max(np.linalg.norm(null[:,1:],axis=0));cmodal=max(np.linalg.norm(control[:,1:],axis=0));material=cmodal>=.01*budget
        comparisons.append(dict(panel=row['panel'],role=row['role'],family=row['family'],strength_error=float(strength),null_modal_over_number=float(nmodal/budget),matched_modal_over_null_number=float(cmodal/budget),null_to_matched_modal=float(nmodal/max(cmodal,1e-30)),material=bool(material)))
    strength=instrument and all(c['strength_error']<=.1 for c in comparisons)
    advantage=strength and any(c['material'] for c in comparisons) and all(c['null_modal_over_number']<=.1 and (not c['material'] or c['null_to_matched_modal']<=.5) for c in comparisons)
    result=dict(plan=PLAN,counts=counts,predictions=dict(zip(PREDICTIONS,[bool(instrument),bool(strength),bool(advantage)])),max_native_reference_replay=max(replays),max_prior_replay=max(oldreplays),missing_analytic_roots=matched['missing_roots'],comparisons=comparisons,records=records,seconds=time.perf_counter()-tic)
