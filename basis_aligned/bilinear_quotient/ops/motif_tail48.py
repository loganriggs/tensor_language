"""CHEAPEN THE SATURATED HALF (rung 240): motifs exact, tail r48.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2336: the tail is nearly saturated
at r64 (exactness there bought only 0.0076). If saturated, r48 should cost little: {motifs exact, tail
r48} = 65.9M pattern values, 9% below the registered motif-exact point (72.1M).
REGISTERED PREDICTIONS (arms: THIS vs motif-exact 0.0608/9 and r64 0.0852/7):
  (a) TAIL TOLERATES r48: census <= 0.075.
  (b) CERTIFICATES MOSTLY KEPT: valid >= 8.
  (c) SANITY: census in [0.01, 0.12]; inert tripwires vs motif-exact AND r64 receipts.
NULL: saturation was one-sided (r64 is the knee; census >= 0.085 or valid <= 7). PRICE: 65.9M pattern
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: motifs exact, tail r48')
    raise SystemExit(0)
import torch
...
OUT=PT+'motif_tail48_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=48; SEL['qk_rmap']={li:128 for li in range(2,10)}; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    if abs(agg-0.0608)<1e-4 or abs(agg-0.0852)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: tail-r48 map inert')
