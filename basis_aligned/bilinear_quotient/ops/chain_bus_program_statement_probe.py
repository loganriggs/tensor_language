#!/usr/bin/env python
"""chain_bus_program_statement_probe -- the whole model as ONE frame program, priced: blocks 0-7 read through their own top-k frame
and write INTO the next site's frame (remainder deleted; mlp_7 writes into the bus), blocks 8-17 read and write through the bus
frame with the remainder routed to the readout (§2756 + §2761). No parameter beyond the 16 early frames and the bus. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_early_chain_is_cheaper_at_1024 pred_c_whole_program_under_p08_at_1024
#                     pred_d_whole_program_under_p35_at_768 pred_e_bus_writes_add_nothing_on_top_of_the_chain
...
Preregistration: polynomial_causal/CHAIN_BUS_PROGRAM_STATEMENT_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/chain_bus_program_statement_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("chain_bus_program_statement_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "CHAIN_BUS_PROGRAM_STATEMENT_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "early_write_frame_chain_probe_results.json"   # §2761
OUT = ROOT / "chain_bus_program_statement_probe_results.json"
HASHES = {PREREG: "4c25eee21321a861067bfc34ffafbb4c1edbe49b1972322ba981f55da18bc030", PRIOR: "33630d046c2fca0f14cc3a2779ef077697d9bcf707892ea13c1ce51b5213a66e",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "chain_bus_program_statement_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_max": 0.030, "c_max": 0.080, "d_max": 0.350, "e_max": 0.005}
NULLS = {"b_min": 0.080, "c_min": 0.150, "d_min": 0.600, "e_min": 0.020}
...
def main():
    ...
    def program(k, chain=False, bus=False):
        patch = {s_: head(s_, k) for s_ in OWN8}; buf = Buf()
        if chain:
            for s_ in OWN8:
                patch[s_] = Both(head(s_, k), Route(s_, k, next_frame(s_), "delete", buf))
        for s_ in SET8:
            r_ = head(s_, k, U_8); patch[s_] = Both(r_, Route(s_, k, U_8, "readout", buf)) if bus else r_
        if bus:
            patch[FINAL] = FinalAdd(buf, +1.0)
        return run(patch, (buf,))
    arms = {"SPLIT8_1024": program(1024)}
    arms["CHAIN_ONLY_1024"] = program(1024, chain=True)
    arms["CHAIN_BUS_1024"] = program(1024, chain=True, bus=True)
    arms["CHAIN_BUS_768"] = program(768, chain=True, bus=True)
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    gaps = {"chain_only_minus_split8_1024": arms["CHAIN_ONLY_1024"] - arms["SPLIT8_1024"], "chain_bus_minus_chain_only_1024": arms["CHAIN_BUS_1024"] - arms["CHAIN_ONLY_1024"],
            "chain_bus_minus_split8_1024": arms["CHAIN_BUS_1024"] - arms["SPLIT8_1024"]}
    ...
    preds = {
        ...
        'pred_b_early_chain_is_cheaper_at_1024': bool(gaps["chain_only_minus_split8_1024"] <= BARS["b_max"]),
        'pred_c_whole_program_under_p08_at_1024': bool(arms["CHAIN_BUS_1024"] <= BARS["c_max"]),
        'pred_d_whole_program_under_p35_at_768': bool(arms["CHAIN_BUS_768"] <= BARS["d_max"]),
        'pred_e_bus_writes_add_nothing_on_top_of_the_chain': bool(gaps["chain_bus_minus_chain_only_1024"] <= BARS["e_max"]),
    }
    nulls = {"b_null_ge_.08": bool(gaps["chain_only_minus_split8_1024"] >= NULLS["b_min"]), "c_null_ge_.15": bool(arms["CHAIN_BUS_1024"] >= NULLS["c_min"]),
             "d_null_ge_.6": bool(arms["CHAIN_BUS_768"] >= NULLS["d_min"]), "e_null_ge_.02": bool(gaps["chain_bus_minus_chain_only_1024"] >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "blocks 0-7: read own top-k frame, write into the next site's frame (remainder deleted; mlp_7 -> bus); blocks 8-17: read and write through the bus frame U_8, remainder routed to the readout",
