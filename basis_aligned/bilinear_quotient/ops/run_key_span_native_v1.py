...
pred_e regional/newline scores replay original uncompressed pair <=1e-6relative.
pred_f live scalar relative error<=1e-10, both heads, actual edited contexts.
pred_d selected target/control effect errors<=10% each family for remove9/joint/donorjoint.
Null: exact key-coordinate rewrite fails native numerical or intervention preservation.
...
def main():
 ...
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('632bodyforwards72regional7arms32natural4arms; frozenmask45reusedconfirmation');return
 ...
 from extracted_circuits.sparse_even_key_producers_8_2_9_8_v1.execute import load_program,scalar as packaged_scalar
 ...
 compiled=KeySpanParent(compact)
 reference_program={k:v.cuda() for k,v in torch.load(P/'extracted_circuits/regional_even_key_producers_8_2_9_8_v1/program.pt',weights_only=True).items()}
 live_scalar_errors=[]
 ...
 def hook(index,head,module,args,output):
  ...
  selected=compiled.scalar(args[0],context['tokens'],index)
  scalar_reference=packaged_scalar(args[0],context['tokens'],reference_program,index)
  live_scalar_errors.append(float((selected-scalar_reference).norm()/scalar_reference.norm().clamp_min(1e-30)))
  if arm==0:
 ...
 prior=torch.load(P/'SCALAR_JOINT_KEY_GRADES_CONFIRMATION_V1_ARTIFACT.pt',weights_only=True)
 package_reg_error=float((reg-prior['regional']).norm()/prior['regional'].norm())
 package_ce_error=float((ce-prior['newline_ce']).norm()/prior['newline_ce'].norm())
 result={'pred_f':bool(live_scalar_errors) and max(live_scalar_errors)<=1e-10,'max_live_scalar_error':max(live_scalar_errors),'live_scalar_calls':len(live_scalar_errors),'pred_e':max(package_reg_error,package_ce_error)<=1e-6,'package_regional_replay':package_reg_error,'package_newline_replay':package_ce_error,'pred_d':all(max(x['relative_errors'])<=.1 for x in preservation),'preservation':preservation,'pred_a':A,'pred_b':A and all(c['passed'] for c in cells),'pred_c':A and all(c['passed'] for c in ncells),'instrument':instrument,'capability':cap,'max_sum_error':max(checks),'max_meanhead_error':max(meanchecks),'regional':cells,'newline':ncells,'body_forwards':count,'seconds':time.perf_counter()-tic,'arms_regional':['native','remove8','remove9','removejoint','fullsector','donorjoint','selfdonorjoint'],'arms_natural':['native','removejoint','fullsector','meanhead8'],'scope':'Exact original key-span coordinates in the known selectedmask45 pair, reused72regional/32newline panel. Fullsector/meanhead controls exact. No new fitting or prospective panel; externalnative context/normalizers remain.','source_shas':binding}
