"""TRAJECTORY-TARGET TAIL DICTIONARIES IN THE FRONTIER CONFIG (rung 127): attacking the +0.5045 increment.

CONVENTION (S2135): frontier L2 and census numbers are CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER.
S2217: the new frontier's tail-attn increment is +0.5045 - now the largest single family cost in the config.
S2210: trajectory-teacher targets beat in-frame targets for the front Down refits. Here the same move is
applied to the tail attention dictionaries inside the exact rung-123 build: the a10L-a17L CV/LW fits keep
their config-frame INPUTS but take their TARGETS from a real-model pass (position-aligned real attention
outputs), so each dictionary steers the drifted stream back toward the real trajectory. Everything else
identical to rung 123 (CP-3456 swap, original motif alphas).

REGISTERED PREDICTIONS (anchors: rung-123 receipt L2_F +2.0553, L1F +1.5508, census +2.1359):
  (a) INCREMENT DROPS: tail-attn increment (L2_F - L1F) <= 0.38 (>= 25% cut from +0.5045).
  (b) FRONTIER MOVES: L2_F fresh <= 1.95.
  (c) SANITY: |L1F - anchor L1F| <= 0.02 (front/motifs untouched) AND census in [1.4, 2.2].
NULL: the 10-class dictionary map is the binding constraint, not the target frame (increment cut < 0.05).
PRICE: none (same dictionary shapes and value counts; different values). Tripwire: INSTRUMENT FAIL if L2CF
missing or _YR capture empty. Self-reviewed."""

import json, sys, time, os
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
if os.environ.get('BQLIB_DRYRUN')=='1':
    _bq='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
    _need=['frontier_claim_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: trajectory-target tail dictionaries')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_tail_traj_results.json'
...
if __name__=='__main__':
    ...
    NFLAT=CN.nflat()
    ANCH=json.load(open(PT+'frontier_claim_results.json'))
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True
    print('ARM: rung-123 build + trajectory-target tail dictionaries',flush=True)
    main()
    ...
    pa=inc<=0.38
    pb=L2F<=1.95
    pc=abs(L1F-ANCH['L1F'])<=0.02 and 1.4<=agg<=2.2
    res={'L2F_fresh':round(L2F,4),'L1F':round(L1F,4),'increment':round(inc,4),'census_agg':round(agg,4),
         'anchor_rung123':{'L2F':ANCH['L2F_fresh'],'L1F':ANCH['L1F'],'census':ANCH['census_agg'],
                           'increment':ANCH['increment']},
         'convention':'CE added above the real model; LOWER IS BETTER',
         'pred_a_increment_drops':bool(pa),'pred_b_frontier_moves':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"L2 fresh {L2F:+.4f} (was +2.0553); increment {inc:+.4f} (was +0.5045); census {agg:+.4f}")
    print(f"(a) increment {inc:+.4f} <= 0.38: {'HELD' if pa else 'FAILED'}")
    print(f"(b) L2F {L2F:+.4f} <= 1.95: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |L1F {L1F:+.4f} - {ANCH['L1F']:+.4f}| <= 0.02 and census in [1.4, 2.2]: {'HELD' if pc else 'FAILED'}")
