"""circuit_battery_v6_ood_population -- the bank sweep's generalisation clauses re-read against a DIFFERENT population.

SS2863 and SS2865 ran the v6 metric over 14 behaviours at PER_CELL 24 and 64. Their pred_d (held-out gap) and pred_e (behaviour
ordering) were both scored SELECT vs TEST. SS2865 showed the design is sample-size sensitive -- pred_e flipped .556 -> .609 at 2.7x the
rows -- but sample size and population are different failure modes and only the first has been probed.
...
from FIT, SELECT and TEST. Everything else is identical, PER_CELL=24 (SS2863's value, so the comparison is against a run whose numbers
are already published), and **every bar is carried over verbatim**. A metric that generalises to TEST but not to OOD is measuring
something tied to the shared pools rather than to the behaviour.
...
Preregistration: polynomial_causal/CIRCUIT_BATTERY_V6_OOD_PREREGISTRATION.md
"""
...
SELF = Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/"
            "circuit_battery_v6_ood_population.py")
...
ROOT = R.ROOT
PREREG = R.POLY / "CIRCUIT_BATTERY_V6_OOD_PREREGISTRATION.md"
BANK21 = ROOT / "circuit_battery_v2_results.json"
RUNG = "circuit_battery_v6_ood_population"
SMOKE = os.environ.get("SURROGATE_SMOKE") == "1"
OUT = ROOT / (f"{RUNG}_smoke_results.json" if SMOKE else f"{RUNG}_results.json")
HASHES = {PREREG: "dee6bb18c7735c9c46e67373b9b9237f130bceff103c55ea8263ab789ef2dc33", BANK21: "5924b2549d285175c80fbf7c8fc95a8a2fa06020acc1827bc472ddea69d9ec93",
...
def main():
    ...
    sel_s, sel_t, pos_s = col("SELECT", "selectivity"), col("OOD", "selectivity"), col("SELECT", "positive_control")
