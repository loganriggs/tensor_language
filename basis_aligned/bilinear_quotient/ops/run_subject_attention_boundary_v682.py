...
"""Source-reuse post-attention projected-state resets at fixed v665 width8.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v682_result.json'
PREDICTIONS=dict(pred_a_exact_replay='baseline and independent joint versus shared chain replay max absolute error <=1e-4',
 ...
 pred_c_capable_position_transfer='some post-attention boundary in layers12-14 restores A/B target<=10% and modal<=5% in every cell')


def main():
    plan=dict(prefix_calls=8,suffix_calls=56,sequences_per_arm=48,fits=0,backwards=0,additional_reduced_suffix_executions=156,
    ...
    attention_reset_reports={str(k):{name:{} for name in ['A','B','sum']} for k in range(6)}
    ...
    def native_selected(raw,x0,first,readpos,start_layer=11):
        x=raw
        context['last_native_states']=[x.double().clone()];context['last_native_premlp']=[]
        for layer in range(start_layer,18):
            block=model.transformer.h[layer]
            if layer>11:x=block.lambdas[0]*x+block.lambdas[1]*x0
            a,first=block.attn(F.rms_norm(x,(x.shape[-1],)),first)
            x=x+a
            context['last_native_premlp'].append(x.double().clone())
            x=x+block.mlp(F.rms_norm(x,(x.shape[-1],)))
    ...
    for entries in batches:
        ...
        for source_name,delta in [('A',source_deltas[0]),('B',source_deltas[1]),('sum',source_deltas[0]+source_deltas[1])]:
            ...
            for k in range(6):
                ...
                zh=(context['last_native_premlp'][k]-context['baseline_premlp'][k])@encoder
                # zh already contains residual scaling and the attention write.
                post_mlp=shared_step(dict(sparse_runtime['blocks'][k],scale=torch.ones_like(sparse_runtime['blocks'][k]['scale'])),zh,context['all_mlp_contexts'][k])
                partial=dict(sparse_runtime,blocks=sparse_runtime['blocks'][k+1:])
                alogits,az=run_dag(partial,context['all_attention'][k+1:],post_mlp,context['all_mlp_contexts'][k+1:],case['final_context'],readpos)
                actrl=readout_prepared(context['control_fixed'],az,context['control_context'])
                aeffect=base-(alogits[:,0]-alogits[:,1])*direction
                amodal=((cb-actrl)[:,0::2]-(cb-actrl)[:,1::2])
                for i,e in enumerate(entries):
                    ac=attention_reset_reports[str(k)][source_name].setdefault(e[3],dict(target=[],prediction=[],native_modal=[],predicted_modal=[]))
                    ac['target'].append(float(native_effect[i]));ac['prediction'].append(float(aeffect[i]))
                    ac['native_modal'].append(native_modal[i].tolist());ac['predicted_modal'].append(amodal[i].tolist())
            for k in range(7):
                encoder=Q if k==0 else output_encoders[k-1]
    ...
    for reports in list(reset_reports.values())+list(attention_reset_reports.values()):
    ...
    passing_attention=[k+12 for k in range(6) if all(cell['relative_error']<=.10 and max(cell['modal_error'])<=.05 for name in ['A','B'] for cell in attention_reset_reports[str(k)][name].values())]
    c=b and any(layer<=14 for layer in passing_attention)

    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,cells=cells,
                predictions=dict(zip(PREDICTIONS,[bool(a),bool(b),bool(c)])),selectivity=selectivity,reset_reports=reset_reports,attention_reset_reports=attention_reset_reports,passing_attention_layers=passing_attention,passing_interior_boundaries=passing_boundaries,max_reset_check=max(reset_checks),max_full_state_readout_error=max(oracle_checks),failed_modal_oracle_ratios=[o/e for o,e in failed],reuse_reports=reuse_reports,cross_effects=cross,reuse_counts=reuse_counts,pre11_joint_vs_post11_sum_gap=boundary_gaps,
