...
Frozen fresh confirmation; no causal-composition or complete-circuit promotion.
"""
...
STEM='SINGLE_HEAD_FRESH_V1';ARMS=['native','native8_midpoint','full_reentry','compiled']+[f'null:{k}' for k in range(16)]
...
def main():
 ...
 context={k:v.cuda() for k,v in torch.load(P/'extracted_circuits/typed_face_reduced_fused_v1/context_program.pt',weights_only=True).items()}
 context['token_ids']=tables['token_ids'];context['first_table']=tables['first_table']
 candidate_program={'local':program,'context':context,'reentry':{k:tables[k] for k in ['token_ids','initial_table','lambdas8']}}
 ...
 def pre9(module,args):
  ...
  if arm=='native':c['x9']=x.clone()
   ...
   candidate_inputs={'residual7':c['residual7'],'donor_city7':cache[i^1]['residual7'][:,city],'token_ids':torch.tensor([row['ids']],device=x.device),'recipient_token':row['ids'][city],'donor_token':groups[i^1]['ids'][city],'city':city,'destination':c['mask']}
   ...
   row=groups[state['i']];seed=17092500+1000*int(arm.split(':')[1])+row['context_id'];generator=torch.Generator(device=x.device).manual_seed(seed)
 ...
 v=expand(values,mapping,6);e=v-v[:1];records={};baseline=json.loads((P/'HEAD2_MLP8_CROSS_FRESH_V1_BASELINES.json').read_text())
