"""Embedding-forward folding, rung (v642): the fixed-kernel atlas for LAYERS 9-11.
...
0.03-0.05; at layer 5 the bank jumps to 0.163 and head 5.7 (0.083) is the first content-critical head. Same instrument for layers 9, 10, 11 (v641: layers 6-8 were all cheap, banks 0.02-0.04):
...
PREDICTIONS (scored as written; failures preserved)
    ...
    pred_b_cheap_heads_persist   at each of layers 9-11 at least 3 heads have single cost <= 0.01 (positional taps persist mid-model). Prior: unsure
    ...
    pred_d_a_content_critical_head at least one head in layers 9-11 has single cost >= 0.05. Prior: likely
...
OUT = ROOT / "circuits/followups/embedding_forward_kernel_atlas_v642_result.json"
OUT_PT = ROOT / "circuits/followups/embedding_forward_kernel_atlas_v642_kernels.pt"
FIT_ROWS = ROOT / ".rowcache/fineweb_n480_skip80.pt"
EVAL_ROWS = ROOT / ".rowcache/fineweb_n192_skip7000.pt"
NATIVE_V615 = 3.13241
CANDIDATE_ID = "embedding_forward.kernel_atlas_v642"
FORWARDS_MAX = 200
...
def main() -> None:
    ...
    with torch.no_grad():
        ...
        disk_guard.guard_torch_save({f"kbar_{l}_{h}": state["kbar"][(l, h)].cpu() for l in LAYERS for h in range(H)} | {"rowsum": {f"{l}.{h}": v for (l, h), v in rowsum.items()}}, str(OUT_PT), "v642 kernels")
    ...
    OUT.write_text(json.dumps({"schema": "embedding_forward_kernel_atlas_result_v642", "candidate_id": CANDIDATE_ID, "plan": plan,
