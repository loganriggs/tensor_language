...
"""The verb readers' verb vectors, replay reference corrected (v495). v494's four content predictions held but its replay check compared the manual native
margins on the distant verb rows against the module's forward on the PRONOUN-slot rows (a derive leftover), so pred_a failed as an instrument artefact. Same
measurement, reference aligned to the verb rows.
...
OUT = ROOT / "circuits/followups/verb_reader_vectors_fixed_v495_result.json"
CANDIDATE_ID = "chain.verb_reader_vectors_fixed_v495"
N_HEAD = 9
...
def main() -> None:
    ...
    OUT.write_text(json.dumps({"schema": "verb_reader_vectors_fixed_result_v495", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
