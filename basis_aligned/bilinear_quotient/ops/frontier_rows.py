"""CERTIFICATE ROWS FOR THE STANDING FRONTIER (rung 132): the two-ledger row rung 128 failed to deliver.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The quotable frontier (S2221, registered)
is the rung-127 build: L2_F fresh +1.8765, census +1.9474. Its per-circuit certificate rows were never saved
(rung 128 was to carry them and exploded, S2222). This rung reruns the EXACT rung-127 build (deterministic)
saving the full 62-circuit rows - the frontier's entry in the two-ledger table.

REGISTERED PREDICTIONS (anchors: rung-127 receipt):
  (a) REPRODUCTION: |census - 1.9474| <= 0.015.
  (b) CERTIFICATE STAGNATION (the S2208 lesson): valid <= 5 of 62. A FAILURE here (valid >= 6) would be NEWS:
      the aggregate path incidentally repairing circuits.
  (c) REPRODUCTION: |L2_F - 1.8765| <= 0.015 AND increment in [0.25, 0.40].
NULL: (b)'s failure branch as stated. PRICE: none (re-instrumentation). Tripwire: INSTRUMENT FAIL if L2CF
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: certificate rows for the standing frontier')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_rows_results.json'
...
if __name__=='__main__':
    ...
    print('ARM: exact rung-127 build, certificate rows saved',flush=True)
    main()
    ...
    pa=abs(agg-1.9474)<=0.015
    pb=nv<=5
    pc=abs(L2F-1.8765)<=0.015 and 0.25<=inc<=0.40
    ...
    print(f"(a) |census {agg:+.4f} - 1.9474| <= 0.015: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} <= 5 via L2F {L2F:+.4f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |L2F - 1.8765| <= 0.015, increment in [0.25, 0.40]: {'HELD' if pc else 'FAILED'}")
