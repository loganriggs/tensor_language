...
"""Locate width8 projection failure without changing the frozen basis family.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_joint_chain_v649_result.json'
...
def main():
    plan=dict(prefix_calls=12,suffix_calls=16,calibration_rows=128,evaluation_rows=48,width=8,
              additional_endpoint_chain_batches=6,basis_selection='calibration SVD, CPU float64',controls=['dense','random_seed647'],
    ...
    for l in range(7):
        ...
        _,s,Vh=torch.linalg.svd(deltas.cpu(),full_matrices=False)
        bases.append(Vh[:8].T.contiguous().cuda());singular.append(s.tolist())
    ...
    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,
        ...
        scope='Postfailure projection-location diagnostics at unchanged width8 state-SVD bases; final projection uses true edited state and is an oracle, not a predictor.')
