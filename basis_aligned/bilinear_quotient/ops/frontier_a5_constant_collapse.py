"""COLLAPSE a5's FRONTIER DICTIONARY TO A SINGLE CONSTANT VECTOR (Claude, LANE 1).

SS2835 established that attention 5's write in the REAL model is one fixed vector: |cos| .9999996 with its own mean, gain CV .081
(rank 1 of 36 for constancy, SS2834's census), and replacing it with that constant costs .1286 nats of document CE against the 2.2109
nats that deleting it costs -- 94.2% recovered. SS2830 made it the price cliff: 3rd of 36 in document CE and 20.4x disproportionate per
unit written. "attn5's write = the price cliff" has stood as one of the three largest gaps in the explained fraction for weeks.

Inside SS312's frontier a5 is not the raw attention: it is a fitted `attnd` dictionary -- a 10-row class-conditional constant table
CV[c] chosen by an input probe, plus linear maps LW[k] for the non-constant classes. If SS2835 is right about the real model, those ten
rows should be near-identical and the linear links should buy nothing, so the whole dictionary should collapse to ONE vector at no cost
in L2.

This reruns SS312's published (norm-selection) pipeline three times: BASELINE, a5 collapsed, and a6 collapsed as a like-for-like control
from the same motif band ATTM. The collapse changes only FITTED VALUES -- CV becomes ten copies of Y.mean(0) and LW becomes empty --
never control flow, so `cur['lab']` is still computed for downstream dictionaries exactly as before.

SIGN CONVENTION (SS2135, the rule that matters most here): frontier L2 numbers are CE ADDED ABOVE THE REAL MODEL, so **LOWER IS
BETTER** (SS312: "+2.6735 beating +2.84/+2.93"). A collapse COST is therefore L2_F(collapsed) - L2_F(baseline), POSITIVE = WORSE.
SS2128/SS2129/SS2133/SS2134 are RETRACTED for reading higher L2 as better; SS2125 STANDS and the frontier is norm-2304 at 2.6735.
pred_a is SS2125 rung 30's reproduction gate carried over verbatim: if the baseline does not reproduce 2.6735 nothing else here is
readable.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_collapsing_a5_is_free
#                     pred_c_the_control_layer_is_not_free
#                     pred_d_a5_rows_are_near_identical
#                     pred_e_the_parameter_saving_is_stated

Preregistration: polynomial_causal/FRONTIER_A5_CONSTANT_COLLAPSE_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; three arms (baseline / a5 collapsed / a6 collapsed)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_a5_constant_collapse_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_A5_CONSTANT_COLLAPSE_PREREGISTRATION.md'
PREREG_SHA = "127dfa8e945f001d656676f56df60196fb5acf282a47fc104a64ee012b9c679d"
A5, CTRL = 5, 6
BARS = {"reproduce": 0.05, "a5_free": 0.02, "ctrl_ratio": 5.0, "ctrl_abs": 0.05, "row_cos": 0.05}
NULLS = {"a5_free_ge": 0.05, "ctrl_ratio_le": 2.0}
...
if __name__=='__main__':
    ...
    print(f'ARM 2/3: a{A5} collapsed to a single constant',flush=True)
    COLLAPSE['li']=A5
    r_a5=main()

    print(f'ARM 3/3: a{CTRL} collapsed -- like-for-like control from the same motif band',flush=True)
    COLLAPSE['li']=CTRL
    r_ctrl=main()

    # structural: how near-identical are the dictionary rows, measured on the BASELINE fit
    def row_spread(li):
        CV=CVSNAP.get(li)
        if CV is None: return float('nan')
        X=F.normalize(CV.float(),dim=1)
        C=X@X.T
        n=C.shape[0]
        return float(1.0-C[torch.triu_indices(n,n,1)[0],torch.triu_indices(n,n,1)[1]].min())
    spread_a5, spread_ctrl = row_spread(A5), row_spread(CTRL)

    # SIGN: L2 is CE ADDED ABOVE THE REAL MODEL, LOWER IS BETTER -> cost POSITIVE = WORSE
    cost_a5   = r_a5['L2_F']   - r_base['L2_F']
    cost_ctrl = r_ctrl['L2_F'] - r_base['L2_F']
    saved = 10*D + len(LINK)*D*D - D          # ten class rows + link matrices -> one vector

    pa = abs(r_base['L2_F']-2.6735) <= BARS['reproduce']
    pb = cost_a5 <= BARS['a5_free']
    pc = (cost_ctrl >= BARS['ctrl_abs']) and (cost_ctrl >= BARS['ctrl_ratio']*max(cost_a5,1e-4))
    pd = (spread_a5 <= BARS['row_cos']) and (spread_a5 < spread_ctrl)
    pe = saved > 0
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_collapsing_a5_is_free':bool(pb),
           'pred_c_the_control_layer_is_not_free':bool(pc),
           'pred_d_a5_rows_are_near_identical':bool(pd),
           'pred_e_the_parameter_saving_is_stated':bool(pe)}
    nulls={'b_null_a5_class_structure_is_load_bearing':bool(cost_a5>=NULLS['a5_free_ge']),
           'c_null_collapsing_anything_is_free':bool(cost_ctrl<=NULLS['ctrl_ratio_le']*max(cost_a5,1e-4))}
    res={'rung':'frontier_a5_constant_collapse','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,'collapsed_layer':A5,'control_layer':CTRL,
         'baseline':r_base,'a5_collapsed':r_a5,'control_collapsed':r_ctrl,
         'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(r_base['L2_F'],4),
                    'L2_F_a5_collapsed':round(r_a5['L2_F'],4),
                    'L2_F_control_collapsed':round(r_ctrl['L2_F'],4),
                    'cost_a5_nats':round(cost_a5,4),'cost_control_nats':round(cost_ctrl,4),
                    'L2_C_baseline':round(r_base['L2_C'],4),'L2_C_a5_collapsed':round(r_a5['L2_C'],4),
                    'row_spread_a5':spread_a5,'row_spread_control':spread_ctrl,
                    'parameters_saved_per_collapsed_layer':int(saved)},
    ...
    print(f"(b) a{A5} collapse cost {cost_a5:+.4f} <= {BARS['a5_free']} (LOWER L2 IS BETTER): {'HELD' if pb else 'FAILED'}")
    print(f"(c) a{CTRL} control cost {cost_ctrl:+.4f}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) row spread a{A5} {spread_a5:.4f} vs a{CTRL} {spread_ctrl:.4f}: {'HELD' if pd else 'FAILED'}")
