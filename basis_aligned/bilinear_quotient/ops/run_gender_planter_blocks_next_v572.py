#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_blocks_7_8_carry pred_c_blocks_5_6_carry pred_d_blocks_0_4_small pred_e_groups_add
"""The planter of gender at noun + 1, by block group (v572). v571: blocks 0-8's writes into the token after the noun close 0.131 of the he - she gap; 4.5 alone
0.011. Here blocks 0-4 (B04), 5-6 (B56) and 7-8 (B78) writes into noun + 1 swapped, on the 61 pairs. Prior: the self-copy blocks (6, 8) hold heads that
read the noun's token; their neighbours at noun + 1 likely do the same one position on.
PREDICTIONS (scored as written; failures preserved; priors from v571 and the self-copies' blocks)
    pred_a_baseline_replays   the unedited manual forward replays the model's he - she margins within 1e-3
    pred_b_blocks_7_8_carry   B78 closes >= 0.05
    pred_c_blocks_5_6_carry   B56 closes >= 0.03
    pred_d_blocks_0_4_small   B04 closes <= 0.05 (the copier blocks are not the gender planter)
    pred_e_groups_add         B04 + B56 + B78 is within 0.05 of 0.131
...
OUT = ROOT / "circuits/followups/gender_planter_blocks_next_v572_result.json"
CANDIDATE_ID = "gender.planter_blocks_next_v572"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, B78_MIN, B56_MIN, B04_MAX, LOW_V571, ADD, UNIT = 1e-3, 0.05, 0.03, 0.05, 0.131, 0.05, 701
...
FORWARDS_MAX = 9
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_blocks_7_8_carry": ">= 0.05", "pred_c_blocks_5_6_carry": ">= 0.03", "pred_d_blocks_0_4_small": "<= 0.05", "pred_e_groups_add": "0.131 +- 0.05"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "b78_min": B78_MIN, "b56_min": B56_MIN, "b04_max": B04_MAX, "low_v571": LOW_V571, "add": ADD, "unit": UNIT}}
    ...
    MODES = {"B04": ({}, (0,)), "B56": ({}, (0,)), "B78": ({}, (0,))}; SETS_W = {"B04": {l: list(range(N_HEAD)) for l in range(0, 5)}, "B56": {l: list(range(N_HEAD)) for l in (5, 6)}, "B78": {l: list(range(N_HEAD)) for l in (7, 8)}}
    ...
    print("block groups into noun + 1", {k: round(v, 3) for k, v in mc.items()}, "sum", round(sum(mc.values()), 3))
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_blocks_7_8_carry": mc["B78"] >= B78_MIN, "pred_c_blocks_5_6_carry": mc["B56"] >= B56_MIN,
                   "pred_d_blocks_0_4_small": abs(mc["B04"]) <= B04_MAX, "pred_e_groups_add": abs(sum(mc.values()) - LOW_V571) <= ADD}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "gender_planter_blocks_next_result_v572", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
