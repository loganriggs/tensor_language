#!/usr/bin/env python
"""circuit_battery_reader_unit_localisation -- BELOW the MLP block: which hidden units read the write, and are THEY selective?
...
# BQGATE: EXPERIMENT  pred_a_unit_decomposition_is_exact pred_b_a_few_units_carry_the_read
#                     pred_c_the_unit_set_is_more_specific_than_its_block pred_d_the_unit_set_is_shared
...
Preregistration: polynomial_causal/CIRCUIT_BATTERY_READER_UNIT_LOCALISATION_PREREGISTRATION.md
"""
...
SELF = Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/circuit_battery_reader_unit_localisation.py")
...
ROOT = R.ROOT
PREREG = R.POLY / "CIRCUIT_BATTERY_READER_UNIT_LOCALISATION_PREREGISTRATION.md"
BATTERY = ROOT / "circuit_battery_v2_results.json"
RUNG = "circuit_battery_reader_unit_localisation"
...
@torch.no_grad()
def run_units(m, tokens, finals, layer, units, *, contrib=False):
    ...
    for site, blk in enumerate(m.transformer.h):
        ...
        x = x + write
        if site == layer and W is not None and (units is not None or contrib):
            nat = F.rms_norm(x, (D,))
            rem = F.rms_norm(x - W, (D,))
            hn = blk.mlp.Left(nat) * blk.mlp.Right(nat)
            hr = blk.mlp.Left(rem) * blk.mlp.Right(rem)
            if contrib:
                col = blk.mlp.Down.weight.norm(dim=0)               # (hidden,)
                info = ((hn - hr)[ar, finals].abs() * col).float()  # (B, hidden)
    ...
    return (logits, info) if contrib else logits
...
def main():
        ...
        for layer in READERS:
            ...
            for fam, sign in (("A1", 1.0), ("C", -1.0)):
                ...
                for b in CB.batches([r for r in rows if r["family"] == fam and r["split"] == "FIT"]):
                    ...
                    _lg, info = run_units(m, ids, fin, layer, None, contrib=True); fwd[0] += 1
    ...
    preds = {
        ...
        'pred_b_a_few_units_carry_the_read': bool(med("top_share") >= BARS["topk_share"]),
        ...
        'pred_d_the_unit_set_is_shared': bool(jac and float(np.median(list(jac.values()))) >= BARS["jaccard"]),
