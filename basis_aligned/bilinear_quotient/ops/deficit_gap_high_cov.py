# THE DEFICIT GAP AT HIGH COVERAGE -- does the fallback penalty shrink as coverage rises?
#
# §1866 corrected §1848: measured as DEFICIT AGAINST LIVE -- the only quantity comparable across two
# populations of different difficulty -- uncovered positions are +0.547 / +0.663 / +0.868 nats worse than
# covered ones on the 5,419-type build, not "not a deficit" as §1848 concluded from raw CE. At 24.6% of
# scored positions that is worth ~0.135 nats of all-position CE, 2.4x §1861's entire iso-cost gain.
#
# What §1866 could not say is whether the PER-POSITION gap shrinks with coverage, because every script
# after §1848 records live CE on covered positions only. This is that one line of instrumentation, at the
# largest covered set available: 16,110 types, full rank, live and program CE on BOTH populations.
#
# Docstring and predicate block written in ONE edit (LESSONS 63); a single arm in TRANKS (LESSONS 64);
# filename tokenises against the banner (LESSONS 56).
#
# ROLES. All three. DISCOVERY ONLY. Rung 3: the open question §1866 ends on.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
# NOTE the unit: every prediction below is about DEFICIT (program minus live) on a population, never raw
# CE across populations -- the LESSON 49 error §1866 had to correct.
#   pred_a THE DEFICIT GAP PERSISTS AT HIGH COVERAGE: the uncovered-minus-covered deficit gap at 16,110
#          types is at least 0.30 nats on all three roles -- more than half the +0.547 / +0.663 / +0.868
#          §1866 computed at 5,419. If TRUE the fallback is worth attacking at every coverage and its
#          value is set purely by the uncovered FRACTION, which is the simple story. If FALSE the gap
#          collapses as coverage rises, high coverage already fixes the fallback, and §1848's conclusion
#          is right for the builds that matter even though its reasoning was not.
#   pred_b AND THE UNCOVERED SLICE IS SUBSTANTIALLY SMALLER HERE: uncovered positions are under 15% of
#          scored positions, against §1848's PUBLISHED 24.6% at 5,419 types. The known-answer check that
#          coverage did what it is supposed to (LESSON 46); without it pred_a's result cannot be turned
#          into a value.
#   pred_c AND THE VALUE OF CLOSING IT IS SMALLER THAN AT 5,419: the product (uncovered fraction) x
#          (deficit gap) is below §1866's PUBLISHED 0.135 nats on skip7000. If FALSE the fallback is worth
#          MORE at high coverage than at low, which would be surprising and would make it the first lever
#          in this arc that improves with coverage rather than saturating.
#   pred_d CONTROLS: coverage is exactly 16,110; live covered CE reproduces §1852's build; the ceiling is
#          finite and above live on both populations; program CE exceeds live CE on both, as it must.
...
OUT = PT + 'ops/deficit_gap_high_cov_results.json'
...
S1866_GAP_5419 = {'skip7000': 0.54749, 'skip11000': 0.66269, 'skip1200': 0.86773}
S1866_VALUE_5419_7000 = 0.135
...
def main():
    ...
    print(f'DEFICIT GAP HIGH COV | 16110 types, full rank, both populations | '
    ...
    value = {e: nfrac[e] * gap[e] for e in roles2}
    pa = all(gap[e] >= 0.30 for e in roles2)
    pb = all(nfrac[e] < 0.15 for e in roles2)
    pc = value['skip7000'] < S1866_VALUE_5419_7000
    pd = (NFULL == 16110 and all(ceilv[e] > livev[e] for e in roles2)
          and all(curve['full'][e]['cov'] > livev[e] and curve['full'][e]['unc'] > liveu[e]
                  for e in roles2))

    print(f'\n  DEFICIT BY POPULATION at {NFULL} covered types, full rank:', flush=True)
    for e in roles2:
        print(f'    {e:10s} covered  prog {curve["full"][e]["cov"]:.5f} live {livev[e]:.5f} '
              f'(+{defc[e]:.5f})   uncovered  prog {curve["full"][e]["unc"]:.5f} '
              f'live {liveu[e]:.5f} (+{defu[e]:.5f})   GAP {gap[e]:+.5f}   '
              f'unc frac {nfrac[e]:.1%}', flush=True)
    print(f'\n  the DEFICIT GAP persists at high coverage (>=0.30) -> {pa}  ' + '  '.join(
        f'{e} {gap[e]:+.5f} (§1866 at 5,419: {S1866_GAP_5419[e]:+.5f})' for e in roles2), flush=True)
    print(f'  and the uncovered slice is smaller (<15%, §1848 had {S1848_UNC_FRAC_5419:.1%}) -> {pb}  '
          + '  '.join(f'{e} {nfrac[e]:.1%}' for e in roles2), flush=True)
    print(f'  and the VALUE of closing it is below §1866 0.135 -> {pc}  ' + '  '.join(
        f'{e} {value[e]:.5f}' for e in roles2), flush=True)
    print(f'  coverage {NFULL}, ceiling above live, program above live on both -> control {pd}',
          flush=True)

    json.dump({'run': 'deficit_gap_high_cov', 'n_covered': NFULL,
               'deficit_covered': defc, 'deficit_uncovered': defu, 'gap': gap,
               'uncovered_fraction': nfrac, 'value_of_closing': value,
               'live_covered': livev, 'live_uncovered': liveu, 'ceiling': ceilv, 'curve': curve,
               'predictions': {'pred_a_gap_persists': bool(pa),
                               'pred_b_slice_smaller': bool(pb),
                               'pred_c_value_smaller': bool(pc),
