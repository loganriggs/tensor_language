...
"""pred_a entire FP32 four-variant score panel replays exactly.
pred_b FP64 final-readout composition error<=20% target/control.
pred_c FP64 composition relative error<=half FP32 error both endpoints.
Price16prefixes,256suffixes,512readouts,120seconds. SameFP32body/states.
Null: final readout precision cannot explain the observed composition failure.
Original FP32 result retained; no freshOOD or FP64body claim.
"""
...
@torch.no_grad()
def main():
    files=json.loads((P/'SELF_CONSUMER_READOUT64_V1_BINDING.json').read_text())['files']
    ...
    assert not (P/'SELF_CONSUMER_READOUT64_V1_RESULT.json').exists()
    ...
    result={'pred_a':torch.equal(scores,prior),'pred_b':max(second['composition_error'])<=.2,
        'pred_c':all(b<=.5*a for a,b in zip(first['composition_error'],second['composition_error'])),
        'fp32_replay_maxabs':float((scores-prior).abs().max()),'final_precision_endpoint_maxabs':float((scores64-scores).abs().max()),
        'comparisons':comparisons,'cells':cells,'seconds':time.perf_counter()-tic,
        'scope':'Same16FineWeb0prefixes, sameFP32transformer/interventions/finalstates, differentfinalRMS/lm_head/softcap/logsoftmaxprecision. OriginalFP32failurepreserved; noFP64bodyornewOODclaim.'}
    (P/'SELF_CONSUMER_READOUT64_V1_RESULT.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps({k:v for k,v in result.items() if k!='cells'},indent=2))
