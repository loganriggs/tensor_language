#!/usr/bin/env python3
"""Token-keyed transplant v2b -- overlap-maximized donors, four-action format.
...
V2_RESULT = ROOT / "mlp1_write_token_keyed_transplant_v2_results.json"
OUT = ROOT / "mlp1_write_token_keyed_transplant_v2b_results.json"
BUNDLE = ROOT / "mlp1_write_token_keyed_transplant_v2b_per_token.pt"
HASHES = {
    PREREG: "299fa7777e3d6485a368bb922ad20f5c508188dee62b11cda40248880dd9a93a",
    ...
    V2_RESULT: "cc78ccf4b925d3a2d2b3699a8120b83b088100c17ddf1a9f88d38aa871c2f94d",
}
...
def validate_inputs():
    ...
    if v2.get("pred_a_exact_lawful_live_supplied_instrument") is not False \
            or v2.get("strong_null") is not True:
        raise RuntimeError("v2 receipt does not license the v2b price repair")
...
def main():
    ...
    if os.environ.get("BQLIB_DRYRUN") == "1":
        assert EXPECTED_FORWARDS == 1500
        assert EXPECTED_MERGE == 1000
        global HALF
        real_half = HALF
        HALF = 6
        try:
        ...
        for path, expected in HASHES.items():
            if not path.is_file() or r493.sha256(path) != expected:
                raise RuntimeError(f"frozen hash mismatch: {path}")
        print(json.dumps({
            "status": "dry_run_passed", "rung": "mlp1_write_token_keyed_transplant_v2b",
    ...
    if OUT.exists() or BUNDLE.exists():
        raise RuntimeError("v2b output namespace already exists")
    ...
    result = {
        "status": "complete", "rung": "mlp1_write_token_keyed_transplant_v2b",
        ...
        "next_step": (
            "register_cross_corpus_token_matched_validation" if not strong_null
            else ("instrument_breach_repair_only" if not pred_a else
