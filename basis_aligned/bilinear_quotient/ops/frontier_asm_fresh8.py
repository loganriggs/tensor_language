...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_fisher8_asm_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; two arms x eight fresh windows, asm-conditioned selection")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_asm_fresh8_results.json'
...
if __name__=='__main__':
    ...
    gains=[round(a-b,4) for a,b in zip(r_f['fresh8'],r_norm['fresh8'])]
    gmed=stt.median(gains); npos=sum(g>0 for g in gains)
    pa=gmed>=0.04
    pb=npos>=7
    pc=abs(r_norm['L2_F']-2.6735)<=0.01
    res={'norm':r_norm,'asm_fisher8':r_f,'published_L2_F':2.6735,
         'per_window_gain_asm_minus_norm':gains,'gain_median':round(gmed,4),
         'windows_positive':int(npos),
         'pred_a_median_gain':bool(pa),'pred_b_windows_positive':bool(pb),
         'pred_c_reproduction':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'per-window gain (asm - norm): {gains}')
    print(f"(a) median gain {gmed:+.4f} >= 0.04: {'HELD' if pa else 'FAILED'}")
    print(f"(b) positive on {npos}/8 (bar 7): {'HELD' if pb else 'FAILED'}")
    print(f"(c) norm FR L2_F {r_norm['L2_F']} vs 2.6735 (tol 0.01): {'HELD' if pc else 'FAILED'}")
