#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_they_she_opposition_matches_they_he pred_c_pronoun_opposition_replays pred_d_random_units_do_not pred_e_gender_margin_barely_moves
...
OUT = ROOT / "circuits/followups/panel_opposing_units_axes_v519_result.json"
CANDIDATE_ID = "chain.panel_opposing_units_axes_v519"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, SHE_BAND, PRON_V516, BAND, RANDOM_MAX, GENDER_MAX = 1e-3, 0.03, -0.078, 0.02, 0.01, 0.25
import random
import random
UNIT_SETS = {"DET_A": (17, (3547, 1747, 3093, 4448), "answer"), "RANDOM_A": (17, tuple(random.Random(0).sample([j for j in range(4608) if j not in (3547, 1747, 3093, 4448)], 4)), "answer")}
...
FORWARDS_MAX = 10
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_they_she_opposition_matches_they_he": "within 0.03", "pred_c_pronoun_opposition_replays": "-0.078 +- 0.02", "pred_d_random_units_do_not": "|.| <= 0.01 x 2", "pred_e_gender_margin_barely_moves": "<= 0.25 x"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "she_band": SHE_BAND, "pron_v516": PRON_V516, "band": BAND, "random_max": RANDOM_MAX, "gender_max": GENDER_MAX}}
    ...
    def logits_margin(x):
        ...
        return torch.stack([z[:, THEY] - z[:, HE], z[:, THEY] - z[:, SHE], z[:, HE] - z[:, SHE]], 1), z   # column 1 is they - she here
    ...
    def study(items, batch):
        ...
        for m, em in ed.items():
            out["closed"][m] = {"verb": float(((nat_m[0::2, 0] - em[0::2, 0]) + (em[1::2, 0] - nat_m[1::2, 0])).sum() / (2 * gap[:, 0].sum())), "are_is": float(((nat_m[0::2, 1] - em[0::2, 1]) + (em[1::2, 1] - nat_m[1::2, 1])).sum() / (2 * gap[:, 1].sum())),
                                "gender_over_number": float((em[:, 2] - nat_m[:, 2]).abs().mean() / (em[:, 0] - nat_m[:, 0]).abs().mean())}
    ...
    c = verb["closed"]
    v = {m: c[m]["verb"] for m in c}; she = {m: c[m]["are_is"] for m in c}; gr = {m: c[m]["gender_over_number"] for m in c}   # "verb" = they - he, "are_is" = they - she here
    report["they_he_closed"] = v; report["they_she_closed"] = she; report["gender_over_number"] = gr
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_they_she_opposition_matches_they_he": abs(she["DET_A"] - v["DET_A"]) <= SHE_BAND, "pred_c_pronoun_opposition_replays": abs(v["DET_A"] - PRON_V516) <= BAND,
                   "pred_d_random_units_do_not": abs(v["RANDOM_A"]) <= RANDOM_MAX and abs(she["RANDOM_A"]) <= RANDOM_MAX, "pred_e_gender_margin_barely_moves": gr["DET_A"] <= GENDER_MAX}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "panel_opposing_units_axes_result_v519", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
