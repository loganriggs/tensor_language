...
"""Shared upstream bilinear-product pruning without restricting input span.

Compare trace-aware isotropic-input functional pruning (global and parent) and
...
Budgets256/1024/2304/4608. Same opened native intervention panels as v629.
Fixed weights, no scalar refits.16forwards/208replays. Not optimal sparse DAG.
"""
import json,os,sys,time,hashlib
from pathlib import Path
from datetime import datetime,timezone
ROOT=Path(__file__).resolve().parents[3];BQ=ROOT/'basis_aligned/bilinear_quotient'
OUT=BQ/'circuits/followups/recent_sparse_products_v633_result.json'
...
def main():
    plan=dict(panels=['fineweb_n96_skip1200.pt','fineweb_n192_skip11000.pt'],ranks=[256,1024,2304,4608],
        families=['global_sphere','parent_sphere','random_products'],scales=[1.,0.,.5,1.5],
    ...
    trace=(L*R).sum(-1)
    global_sphere=2*global_gram+(D.T@D)*torch.outer(trace,trace)
    parent_sphere=2*parent_gram+(C.T@C)*torch.outer(trace,trace)
    orders=dict(global_sphere=greedy_removal_order(global_sphere),
        parent_sphere=greedy_removal_order(parent_sphere),
    ...
    def build(family,rank):
        ...
        kept=orders[family][-rank:]
        supports[family+':'+str(rank)]=kept.cpu().tolist()
        return dict(w,left16=L[kept],right16=R[kept],down16=D[:,kept])
    ...
    for family,rank in configs:
        ...
        if family!='dense':
            kept=orders[family][-rank:]
            row['global_sphere_error']=float((removal_energy(global_sphere,kept).clamp_min(0)/global_sphere.sum()).sqrt())
            row['parent_sphere_error']=float((removal_energy(parent_sphere,kept).clamp_min(0)/parent_sphere.sum()).sqrt())
            row['global_tensor_error']=float((removal_energy(global_gram,kept).clamp_min(0)/global_gram.sum()).sqrt())
            row['parent_dictionary_error']=float((removal_energy(parent_gram,kept).clamp_min(0)/parent_gram.sum()).sqrt())
    ...
    result=dict(plan=plan,rows=rows,selected={k:v for k,v in selected.items() if k!='panels'},
        ...
        scope='Isotropic quadratic functional metric including trace, not native distribution optimum; fixed dictionary; opened panels',
        seconds=time.perf_counter()-tic,finished_utc=datetime.now(timezone.utc).isoformat())
    assert forwards<=plan['forwards_max']
    disk_guard.guard_write(1000000,label='v633 JSON');OUT.write_text(json.dumps(result,indent=2)+'\n')
