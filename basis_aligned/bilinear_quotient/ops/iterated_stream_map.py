# ITERATED SELF-CONSISTENT STREAM MAP -- is §1878 a fitting error or an expressibility limit?
#
# §1878 killed §1876's deployability. A covered-fit map from the length-1 residual stream reaches
# +0.17427 / +0.21358 / +0.21419 when the stream comes from a NATIVE forward pass, but +1.08978 /
# +1.27276 / +1.26133 when the program regenerates it from its own compressed prefix -- worse than
# §1870's rank-512 EMBEDDING map (+0.59560 / +0.67209 / +0.67172) and worse than the deployed rank-64
# build (+0.78075 / +0.86225 / +0.83997). I retracted §1876's headline on that.
#
# But §1878 confounds two things. The map was FITTED on native covered streams and APPLIED to
# program-generated uncovered streams. That is a train/test distribution shift, and a distribution shift
# is a FITTING error, not a statement about what a linear map on this input can express. The fix is to
# fit where you apply: generate the COVERED tokens' streams the same way the uncovered ones will be
# generated -- every site replaced by the map -- refit on those, and iterate map -> streams -> map.
#
# This is the last rescue for the stream-input line. Storage is still unchanged (36 x rank x 2 x D), and
# the procedure is deployable: the iteration happens once, offline, at build time.
#
# ROLES. All three, deployed 5,419-type covered set. DISCOVERY ONLY. Rung 3: §1878's open question.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
# Units per LESSONS 49: fallback loss is a DEFICIT on the uncovered population against its ceiling.
# ANCHORS name the build AND the rank they came from, per LESSONS 53's addendum and §1875.
#   pred_a REFITTING HELPS AT ALL: the iterated self-consistent map beats §1878's PUBLISHED closed
#          figures (+1.08978 / +1.27276 / +1.26133) on all three roles. This is the weakest possible
#          bar and it only asks whether the distribution shift was ANY of the problem. If FALSE the
#          shift explains none of §1878 and the collapse is intrinsic to running the recursion.
#   pred_b AND THE LINE LIVES: it beats §1870's PUBLISHED rank-512 covered-fit EMBEDDING map
#          (+0.59560 / +0.67209 / +0.67172) on all three roles. This is the bar that matters. The
#          embedding map needs no input the program does not already own, so a stream map that cannot
#          beat it is not worth its extra machinery at equal storage. If TRUE, part of §1876 is
#          recoverable in a genuinely source-closed form and I will say so as loudly as I retracted it.
#          If FALSE the stream-input line is CLOSED and §1870 is the deployable answer.
#   pred_c AND THE COVERED ARM IS UNTOUCHED: covered CE identical to the MAP arm within 1e-6. TENTH
#          KNOWN-ANSWER check; the previous nine each returned exactly 0.00e+00.
#   pred_d CONTROLS: coverage is exactly 5,419; the covered arm attains its ceiling (a twentieth
#          confirmation); the MAP arm reproduces §1870 and the STREAMCOV arm reproduces §1876, both
#          within 0.005 and both in THIS build; the STREAMCLOSED arm reproduces §1878 within 0.005 via
#          an independently written code path; and the iteration CONVERGES -- the last relative map
#          change is smaller than the first. A diverging iteration invalidates pred_a and pred_b
#          whatever they read.
...
OUT = PT + 'ops/iterated_stream_map_results.json'
...
MAPRANKS = (512,)
ARMS2 = ('MAP', 'STREAMCOV', 'STREAMCLOSED', 'STREAMITER')
NITER = 3
ITER_DELTA = []          # None = full rank; §1786/§1813 use this ladder
...
LADDER = list(ARMS2)
# §1849's PUBLISHED rank-256 build at 5,419 types -- the reference point of the 65.950M line
# §1866's PUBLISHED deficit gaps and value at 5,419 covered types
# §1867's PUBLISHED per-position deficit gap at 16,110 covered types
S1848_UNC_FRAC_5419 = 0.246
# §1869's PUBLISHED map-rank recovery (64 -> 512) at 16,110 covered types
# §1870's PUBLISHED map-alone fallback loss at map rank 64, 5,419 covered types
S1872_MAP_M64 = {'skip7000': 0.78075, 'skip11000': 0.86225, 'skip1200': 0.83997}
# §1873's PUBLISHED embedding-input oracle loss -- the bound this run's input change is measured against
S1873_ORACLE_EMB = {'skip7000': 0.65138, 'skip11000': 0.72189, 'skip1200': 0.70037}
# §1874's PUBLISHED stream-input oracle at map rank 64 -- the point this run changes rank from
S1875_ORACLE_STREAM_R512 = {'skip7000': 0.11388, 'skip11000': 0.14094, 'skip1200': 0.14196}
# §1870's PUBLISHED covered-fit EMBEDDING map at rank 512 -- the input-isolating comparison
S1870_MAP_R512 = {'skip7000': 0.59560, 'skip11000': 0.67209, 'skip1200': 0.67172}
S1849_R256_5419 = {'skip7000': 6.06004, 'skip11000': 6.03311, 'skip1200': 6.04238}
S1852_R256_ABOVE_7000 = 0.09252
...
FITLO, FITHI = 0, 96       # §1834's covered set: 5,419 types, the DEPLOYED build   # 5x the fit rows -> a larger covered set
H = m.transformer.h
NCOV = 5419
...
STATE = {}
BASE = {'mode': 'MAP'}
# §1876 PUBLISHED, rank 512, covered-fit, NATIVE stream input -- the claim this run qualifies.
S1876_STREAM_R512 = {'skip7000': 0.17427, 'skip11000': 0.21358, 'skip1200': 0.21419}
# half of §1876's advantage over §1870's rank-512 embedding map: the pred_b bar.
S1878_CLOSED = {'skip7000': 1.08978, 'skip11000': 1.27276, 'skip1200': 1.26133}   # §1878 PUBLISHED
ORC = {}
COV = {}
...
def main():
    ...
    assert NFULL == NCOV, f'coverage {NFULL} != §1834 published {NCOV}'
    ...
    print(f'ITERATED SELF-CONSISTENT STREAM MAP | 5419 types, fit where applied, rank 512 | '
    ...
    def build(n, rank=None, map_rank=MAP_RANK):
        ...
        covstream = {st: torch.zeros(n, D, device=DEV) for st in sites}
        cap, capin = {}, {}

        def mk(st):
            def hook(mod, args, out):
                cap[st] = (out[0] if isinstance(out, tuple) else out)[:, 0].float()
                capin[st] = args[0][:, 0].float()      # the stream ENTERING this site, covered tokens
                return None
            return hook
        for i in range(0, n, 256):
            t = tk[i:i + 256].unsqueeze(1)
            ...
            for st in sites:
                ...
                covstream[st][i:i + t.shape[0]] = capin[st]
        if rank is not None:
        ...
        for st in sites:
            ...
            fr[tk] = tables[st]
            fr[unc] = (Eunc @ mp).float()          # the embedding-map base, written for EVERY arm
            out[st] = fr

        def fit_maps(X):
            """rank-`map_rank` map from a per-site input matrix to that site's covered table rows."""
            mm = {}
            for st in sites:
                Xo = X[st].double()
                Ao = (Xo.T @ Xo + RIDGE * torch.eye(D, device=DEV, dtype=torch.float64)
                      * (Xo.shape[0] / D))
                Wo = torch.linalg.solve(Ao, Xo.T @ tables[st].double())
                Uu, So, Vo = torch.linalg.svd(Wo, full_matrices=False)
                mm[st] = (Uu[:, :map_rank] * So[:map_rank]) @ Vo[:map_rank]
            return mm

        def prog_streams(toks, mm, want_rows=True):
            """The site-entry vectors the PROGRAM produces for `toks`: one length-1 pass in which every
            site is REPLACED by mm applied to the stream that site receives, so site st's input is built
            from the program's own rows at all 36 sites beneath it and the model is never consulted."""
            nt = toks.shape[0]
            X = {st: torch.zeros(nt, D, device=DEV) for st in sites}
            R = {st: torch.zeros(nt, D, device=DEV) for st in sites} if want_rows else None
            cin, crw = {}, {}

            def _mk2(st):
                def hook(mod, args, out_):
                    cin[st] = args[0][:, 0].float()
                    crw[st] = (cin[st].double() @ mm[st]).float()
                    y = out_[0] if isinstance(out_, tuple) else out_
                    sub = crw[st].reshape(y.shape).to(y.dtype)
                    return (sub,) + tuple(out_[1:]) if isinstance(out_, tuple) else sub
                return hook
            for i6 in range(0, nt, 256):
                tt6 = toks[i6:i6 + 256].unsqueeze(1)
                forward_logits(tt6, [(st, _mk2(st)) for st in sites])
                for st in sites:
                    X[st][i6:i6 + tt6.shape[0]] = cin[st]
                    if want_rows:
                        R[st][i6:i6 + tt6.shape[0]] = crw[st]
            return X, R

        if BASE['mode'] in ('STREAMCOV', 'STREAMCLOSED', 'STREAMITER'):
            mpos = fit_maps({st: covstream[st] for st in sites})   # iteration 0 == §1876's map
        if BASE['mode'] == 'STREAMCOV':
            # §1876: the NATIVE site-entry vector. Reproduced as pred_d's anchor.
            for st in sites:
                out[st][utoks] = (ORC['stream'][st].double() @ mpos[st]).float()
        elif BASE['mode'] == 'STREAMCLOSED':
            # §1878, re-derived through prog_streams -- an independently written path to the same claim.
            _, Ru = prog_streams(utoks, mpos)
            for st in sites:
                out[st][utoks] = Ru[st]
            del Ru
        elif BASE['mode'] == 'STREAMITER':
            # fit where you apply: regenerate the COVERED streams the same way the uncovered ones will
            # be, refit on those, repeat. The iteration is offline and happens once, at build time.
            for it in range(NITER):
                Xc, _ = prog_streams(tk, mpos, want_rows=False)
                nm = fit_maps(Xc)
                dlt = max(float((nm[st] - mpos[st]).norm() / mpos[st].norm().clamp_min(1e-12))
                          for st in sites)
                ITER_DELTA.append(dlt)
                print(f'      iter {it + 1}/{NITER}: max relative map change {dlt:.5f}', flush=True)
                mpos = nm
                Xc, nm = None, None
                torch.cuda.empty_cache()
            _, Ru = prog_streams(utoks, mpos)
    ...
    # the UNCOVERED half: the same object for every token that actually occurs at an uncovered scored
    # position. A few thousand rows, not the 50,257 vocabulary (which would need ~10 GiB).
    useen = torch.zeros(V, dtype=torch.bool, device=DEV)
    for ename in evs:
        rr = evs[ename]
        for i3 in range(0, rr.shape[0], 8):
            idx3 = rr[i3:i3 + 8, :-1].to(DEV)[:, 64:]
            useen[idx3[~seen2[idx3]]] = True
    utoks = useen.nonzero(as_tuple=True)[0]
    NU = int(utoks.numel())
    uidmap = torch.full((V,), -1, dtype=torch.long, device=DEV)
    uidmap[utoks] = torch.arange(NU, device=DEV)
    uceil_lp = torch.zeros(NU, W, device=DEV)
    for i2 in range(0, NU, 256):
        tt = utoks[i2:i2 + 256].unsqueeze(1)
        uceil_lp[i2:i2 + tt.shape[0]] = torch.log_softmax(forward_logits(tt)[:, 0].float(), -1)
    print(f'  uncovered-token ceiling built for {NU} token types', flush=True)

    ceilv, livev, liveu, nfrac, uceilv = {}, {}, {}, {}, {}
    for ename in evs:
        ceilv[ename] = split_ce(evs[ename], [], seen2, ceil_lp, idmap2)['cov'][0]
        _lv = split_ce(evs[ename], [], seen2)
        livev[ename] = _lv['cov'][0]
        liveu[ename] = _lv['unc'][0]
        nfrac[ename] = _lv['unc'][1] / (_lv['cov'][1] + _lv['unc'][1])
        uceilv[ename] = split_ce(evs[ename], [], ~seen2, uceil_lp, uidmap)['cov'][0]
    print(f'\n  coverage {NFULL} types ({NFULL / NCOV:.2f}x §1834); ceiling CE ' + '  '.join(
        f'{e} {ceilv[e]:.5f}' for e in evs) + f' ({time.time() - t0:.0f}s)', flush=True)

    curve = {}
    # the ORACLE targets: each uncovered scored token's own length-1 site output, and its embedding.
    ORC['emb'] = m.transformer.wte.weight.detach()[utoks].float().double()
    ORC['rows'] = {st: torch.zeros(NU, D, device=DEV) for st in sites}
    _cap, _capin = {}, {}

    ORC['stream'] = {st: torch.zeros(NU, D, device=DEV) for st in sites}

    def _mk(st):
        def hook(mod, args, out):
            _cap[st] = (out[0] if isinstance(out, tuple) else out)[:, 0].float()
            _capin[st] = args[0][:, 0].float()      # the stream ENTERING this site
            return None
        return hook
    for i4 in range(0, NU, 256):
        tt4 = utoks[i4:i4 + 256].unsqueeze(1)
        forward_logits(tt4, [(st, _mk(st)) for st in sites])
        for st in sites:
            ORC['rows'][st][i4:i4 + tt4.shape[0]] = _cap[st]
            ORC['stream'][st][i4:i4 + tt4.shape[0]] = _capin[st]
    print(f'  oracle target rows built for {NU} uncovered token types '
    ...
    for lbl in ARMS2:
        ...
        if False:
            ...
            frr, allh = None, None
            torch.cuda.empty_cache()
            bank = build(NFULL, rank=None, map_rank=rk)[0]
    ...
    # no cost column here: LADDER holds ARM NAMES, not ranks, and nothing in this run
    # prices storage. rcost() does int(r) -- the sixth tail-inheritance failure.
    R = LADDER[0]
    ...
    fbloss = {l2: {e: curve[l2][e]['unc'] - uceilv[e] for e in roles2} for l2 in LADDER}
    covce = {l2: {e: curve[l2][e]['cov'] for e in roles2} for l2 in LADDER}
    genshare = {e: (fbloss['MAP'][e] - fbloss['STREAMCOV'][e]) / max(fbloss['MAP'][e], 1e-9)
                for e in roles2}
    covspread = max(abs(covce[l2][e] - covce['MAP'][e]) for l2 in LADDER for e in roles2)
    attain = all(abs(covce['MAP'][e] - ceilv[e]) < 0.001 for e in roles2)
    conv = len(ITER_DELTA) >= 2 and ITER_DELTA[-1] < ITER_DELTA[0]
    pa = all(fbloss['STREAMITER'][e] < S1878_CLOSED[e] for e in roles2)
    pb = all(fbloss['STREAMITER'][e] < S1870_MAP_R512[e] for e in roles2)
    pc = covspread <= 1e-6
    pd = (all(abs(fbloss['MAP'][e] - S1870_MAP_R512[e]) <= 0.005 for e in roles2)
          and all(abs(fbloss['STREAMCOV'][e] - S1876_STREAM_R512[e]) <= 0.005 for e in roles2)
          and all(abs(fbloss['STREAMCLOSED'][e] - S1878_CLOSED[e]) <= 0.005 for e in roles2)
          and conv and NFULL == NCOV and attain)

    print(f'\n  FALLBACK LOSS: fitted on COVERED (deployed) vs on UNCOVERED (oracle), '
          f'{NFULL} covered types:', flush=True)
    for l2 in LADDER:
        print(f'    {l2:8s} ' + '  '.join(
            f'{e} {fbloss[l2][e]:+.5f}' for e in roles2), flush=True)
    print(f'    generalisation share of the deployed loss: ' + '  '.join(
        f'{e} {genshare[e]:.1%}' for e in roles2), flush=True)
    print(f'\n  REFITTING HELPS AT ALL -- beats §1878 closed -> {pa}  ' + '  '.join(
        f'{e} {fbloss["STREAMITER"][e]:+.5f} vs {S1878_CLOSED[e]:+.5f}' for e in roles2), flush=True)
    print(f'  and THE LINE LIVES -- beats §1870 rank-512 embedding -> {pb}  ' + '  '.join(
        f'{e} {fbloss["STREAMITER"][e]:+.5f} vs {S1870_MAP_R512[e]:+.5f}' for e in roles2), flush=True)
    print(f'  and the COVERED arm is untouched (<1e-6) -> {pc}  max spread {covspread:.2e}', flush=True)
    print(f'  anchors §1870/§1876/§1878 reproduce, coverage {NFULL}, attainment {attain}, '
          f'iteration converges {conv} (deltas ' + ' '.join(f'{x:.5f}' for x in ITER_DELTA)
          + f') -> control {pd}', flush=True)
    print(f'  RESIDUAL GAP to the NATIVE-stream §1876: ' + '  '.join(
        f'{e} {fbloss["STREAMITER"][e] - fbloss["STREAMCOV"][e]:+.5f}' for e in roles2), flush=True)

    json.dump({'run': 'iterated_stream_map', 'n_covered': NFULL, 'arms': list(ARMS2),
               'fallback_loss': fbloss, 'covered_ce': covce, 'generalisation_share': genshare,
               'covered_spread': covspread, 'iter_delta': ITER_DELTA, 'ceiling_uncovered': uceilv,
               'uncovered_fraction': nfrac, 'attainment_exact': bool(attain),
               'predictions': {'pred_a_refit_beats_closed': bool(pa),
                               'pred_b_beats_embedding_map': bool(pb),
