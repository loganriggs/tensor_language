...
"""pred_a exact expansion and frozen native/full anchors;pred_b capability/live.
...
STEM='ODD_SOURCE_SWAP_INTERACTION_NATIVE_V1';ARMS=['native','full_swap','routing_swap','value_swap','additive_no_mixed'];READOUTS=[('target',None),('work_jobs',(670,3946)),('cat_dog',(3797,3290)),('red_blue',(2266,4171)),('monday_tuesday',(3321,3431)),('apple_orange',(17180,10912))]
...
def main():
    ...
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
        assert json.loads((P/'ODD_SOURCE_SWAP_INTERACTION_V1_CPU_CONTROL.json').read_text())['pred_a'];print('240bodyforwards;48 captures+4x48 swaps;CPUcontrol',cpu_control());return
    ...
    assert count==240 and bool(torch.isfinite(values).all());effect=values-values[0];anchor=max(float((values[0,:,:2]-prior[0,:,:2]).abs().max()),float((values[1,:,:2]-prior[1,:,:2]).abs().max()));cells=[]
    ...
    torch.save({'values':values,'effects':effect},artifact);result={'pred_a':anchor<=1e-5 and max(replays)<=1e-10 and max(algebra)<=1e-10 and min(mixed_norms)>1e-8 and count==240 and bool(torch.isfinite(values).all()),'pred_b':all(c['capability_pass'] for c in cells),'pred_c':all(c['additive_no_mixed_pass'] for c in cells),'pred_d':all(c['value_hypothesis_pass'] for c in cells),'pred_e':all(c['routing_hypothesis_pass'] for c in cells),'max_frozen_anchor_error':anchor,'max_source_helper_replay_error':max(replays),'max_swap_expansion_error':max(algebra),'min_mixed_source_norm':min(mixed_norms),'families':cells,'body_forwards':count,'seconds':time.perf_counter()-start,'artifact_sha256':digest(artifact),'source_shas':binding,'arm_order':ARMS,'readout_order':[x[0] for x in READOUTS],'scope':'Exact routing/value/mixed decomposition of paired O framing-source interchange with native suffix. No fitting, upstream necessity, corpus OOD, compression adoption, or quantization.'};out.write_text(json.dumps(result,indent=2)+'\n');print(json.dumps({k:v for k,v in result.items() if k!='source_shas'}),flush=True);signal.alarm(0)
