...
#
#
# v2 ADDS A SPECIFICITY CONTROL, answering the sharpest of Codex's three audit points (§1721):
...
# which one it is.
#
# WHERE THE CONTROL CANNOT BE FORMED, it is reported as null rather than faked: a circuit naming
# every site of its kind has no same-kind elsewhere, so the control is drawn from the complementary
# kind; a circuit naming all 36 sites has no elsewhere at all and gets `specificity: null`.
#
# The v1 limitation stands and is NOT addressed here: the audit still scores component SETS, not
# CLAIMS (§1722). Entries naming the same sites still return identical rows.
#
...
OUT = PT + 'ops/circuit_audit_v2_results.json'
...
def control_sites(sites):
    """A matched-size set of components the circuit does NOT name. Same kind where possible;
    complementary kind when the circuit names every site of its own; None when it names all 36."""
    named = set(sites)
    ...
    # deterministic spread through the pool rather than a random draw, so the control is
    # reproducible across runs and not a lucky or unlucky sample
    step = max(len(pool) // len(sites), 1)
    return [pool[(i * step) % len(pool)] for i in range(len(sites))]
...
def main():
    ...
    print(f'CIRCUIT AUDIT v2 (+specificity control) | registry {len(reg)} entries, {len(certified)} certified | '
    ...
    for ename, epath in EVAL_SETS:
        ...
        for name in auditable:
            ...
            ctrl = control_sites(sites)
            if ctrl is None:
                rem_c, spec = None, None
            else:
                ccc = ce(ev, seen, hooks=[mod_of(*st).register_forward_hook(
                    const_hook(K[f'{st[0]}{st[1]}'].to(DEV).float(), st[0] == 'attn'))
                    for st in ctrl])
                rem_c = ccc - cl
                spec = removal / rem_c if rem_c > 1e-6 else None
            rows.setdefault(name, {})[ename] = {
                'n_sites': len(sites), 'ce_live': round(cl, 5),
                'removal_nats': round(removal, 5), 'extraction': round(extraction, 5),
                'control_removal_nats': None if rem_c is None else round(rem_c, 5),
                'specificity': None if spec is None else round(spec, 4)}
            if ename == 'skip7000':
                sp = 'n/a  ' if spec is None else f'{spec:5.2f}'
                print(f'    {name[:42]:42s} {len(sites):2d}s  removal {removal:7.4f}  '
                      f'extract {extraction:7.2%}  specificity {sp}', flush=True)
        if ename == 'skip7000':
    ...
    sp = [rows[n]['skip7000']['specificity'] for n in auditable
          if rows[n]['skip7000']['specificity'] is not None]
    ctrl_rems = [rows[n]['skip7000']['control_removal_nats'] for n in auditable
                 if rows[n]['skip7000']['control_removal_nats'] is not None]
    pa = (len(sp) >= 2) and (max(sp) / max(min(sp), 1e-9) >= 2.0)
    pb = (len(sp) > 0) and (sorted(sp)[len(sp) // 2] > 1.0)
    ...
    pd = (len(ctrl_rems) > 0) and all(r > 0 for r in ctrl_rems)
    ...
    print(f'\n  SPECIFICITY (named set removal / matched-size control set removal):', flush=True)
    for n in sorted([a for a in auditable if rows[a]['skip7000']['specificity'] is not None],
                    key=lambda a: -rows[a]['skip7000']['specificity']):
        r = rows[n]['skip7000']
        print(f'    {n[:42]:42s} {r["specificity"]:6.2f}   ({r["removal_nats"]:.4f} vs control '
              f'{r["control_removal_nats"]:.4f})', flush=True)
    print(f'\n  extraction spread {min(ex):.2%} .. {max(ex):.2%}', flush=True)
    print(f'  specificity spread {min(sp):.2f} .. {max(sp):.2f} -> discriminating {pa}', flush=True)
    print(f'  median specificity {sorted(sp)[len(sp)//2]:.2f} -> named beats arbitrary {pb}',
          flush=True)
    print(f'  worst OOD removal move {max(ood):.2%} | v1 rows reproduce -> control {pc}', flush=True)
    print(f'  every control set has positive removal -> non-vacuous {pd}', flush=True)
    ...
    res = {'config': {'registry': REG, 'eval_sets': [n for n, _ in EVAL_SETS],
           ...
           'predictions': {'pred_a_specificity_discriminates': bool(pa),
                           'pred_b_named_beats_arbitrary': bool(pb),
                           'pred_c_v1_rows_reproduce': bool(pc),
                           'pred_d_controls_non_vacuous': bool(pd)},
