#!/usr/bin/env python3
# BQLANE: gpu
# BQGATE: EXPERIMENT pred_a_exact_instrument pred_b_instruction_cross_selective pred_c_full_cross_selective pred_d_failed_reader_mainly_additive
"""Attribute fresh instruction-cross collateral against the additive branches."""
...
PREREG=P/"SETTING2_REGIONAL_INSTRUCTION_CROSS_COLLATERAL_ATTRIBUTION_V1_PREREGISTRATION.md"
ROWS=P/"SETTING2_REGIONAL_INSTRUCTION_CROSS_FRESH_V1_ROWS.json"
BINDING=P/"SETTING2_REGIONAL_INSTRUCTION_CROSS_COLLATERAL_ATTRIBUTION_V1_BINDING.json"
...
OUT=ROOT/"basis_aligned/bilinear_quotient/circuits/fast_screens/setting2_regional_instruction_cross_collateral_attribution_v1_result.json"
...
def load_bound():
    ...
    files={"preregistration":PREREG,"rows":ROWS,"row_check":P/"regional_cue_row_check_v1.py",
           ...
           "edge_program":EDGE,"parent_result":PARENT,"composition_result":COMPOSITION,"source_census":CENSUS,"fresh_result":FRESH}
...
def plan():
    ...
    return {"schema":"setting2_regional_instruction_cross_collateral_attribution_v1_plan","model_loaded":False,"gpu_accessed":False,"queue_touched":False,"rows":len(rows),"arms":ARMS,"late_names":LATE_NAMES,"price":PRICE,"row_checks":checks,"binding_sha256":sha(BINDING)}
...
def main():
    ...
    for _,indices in sorted(buckets.items()):
        for offset in range(0,len(indices),8):
            ...
            for arm_index,arm in enumerate(ARMS):
                ...
                executions+=1
    effects=values-values[0]; pair_effect=paired_difference(effects,2); reports={}
    for family in sorted(set(row["family"] for row in rows)):
        ...
        for scale,name in enumerate(("numerator","logit")):
            additive_target=pair_effect[1,scale,pair_ids,0]; instruction_target=pair_effect[2,scale,pair_ids,0]-additive_target; full_target=pair_effect[3,scale,pair_ids,0]-additive_target
            instruction_controls={}; full_controls={}; additive_controls={}; joint_controls={}
            for j in range(1,len(READOUTS)):
                key=READOUTS[j][0]; additive=effects[1,scale,row_ids,j]; instruction=effects[2,scale,row_ids,j]-additive; full=effects[3,scale,row_ids,j]-additive
                instruction_controls[key]=float(instruction.square().mean().sqrt()/instruction_target.square().mean().sqrt().clamp_min(1e-30)); full_controls[key]=float(full.square().mean().sqrt()/full_target.square().mean().sqrt().clamp_min(1e-30)); additive_controls[key]=float(additive.square().mean().sqrt()); joint_controls[key]=float(effects[3,scale,row_ids,j].square().mean().sqrt())
            failed_fraction=additive_controls["work_jobs"]/max(joint_controls["work_jobs"],1e-30)
            report[name]={"instruction_cross_target_rms":float(instruction_target.square().mean().sqrt()),"full_cross_target_rms":float(full_target.square().mean().sqrt()),"instruction_cross_control_ratios":instruction_controls,"full_cross_control_ratios":full_controls,"additive_control_rms":additive_controls,"full_joint_control_rms":joint_controls,"work_jobs_additive_to_joint_rms_ratio":failed_fraction}
    ...
    predictions={"pred_a_exact_instrument":bool(instrument),"pred_b_instruction_cross_selective":bool(instrument and all(max(r["logit"]["instruction_cross_control_ratios"].values())<=.75 for r in reports.values())),"pred_c_full_cross_selective":bool(instrument and all(max(r["logit"]["full_cross_control_ratios"].values())<=.75 for r in reports.values())),"pred_d_failed_reader_mainly_additive":bool(instrument and reports["1"]["logit"]["work_jobs_additive_to_joint_rms_ratio"]>=.75)}
    ...
    result={"schema":"setting2_regional_instruction_cross_collateral_attribution_v1_result","terminal":"cross_specific_selectivity" if all(predictions.values()) else "valid_collateral_attribution" if instrument else "invalid","predictions":predictions,"metrics":metrics,"family_reports":reports,"arms":ARMS,"readouts":[name for name,_ in READOUTS],"price":PRICE,"row_checks":checks,"binding_sha256":sha(BINDING),"runner_sha256":sha(RUNNER),"checkpoint_weights_sha256":"680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3","created_utc":datetime.now(timezone.utc).isoformat().replace("+00:00","Z"),"scope":"Opened fresh-panel subtraction attribution of unrelated-reader effects to additive routing/value branches versus instruction/full head9.8 cross terms; no new transfer, fit, or parameter update."}
