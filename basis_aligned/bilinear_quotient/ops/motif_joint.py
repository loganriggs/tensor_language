"""THE JOINT MOTIF NUMBER (rung 79): closing S2169's flagged open number.

CONVENTION (S2135): L2 is CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER; d(all) = L2_F(full) - L2_F(ALL
motifs real) on FR (the total cost of the 38-motif-head replacement, fits matched per arm).

S2169: the eight leave-one-real marginals sum to +0.741 and are strongly sub-additive. Measure the joint.
PRICE: none (accounting). NULL: joint ~ +0.741 (additive after all).

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) STRONG SUB-ADDITIVITY: d(all) <= 0.5 x 0.741.
  (b) COHERENCE FLOOR: d(all) >= max single marginal (+0.1888).
  (c) REPRODUCTION: the full arm reproduces S2144 (FR L2_F 2.6662 within 0.01).

Writes motif_joint_results.json. Self-reviewed. RUNG-75 HEADER FOLLOWS.

WHICH BLOCKS CARRY THE MOTIF PRICE (rung 75): the leave-one-real sweep over blocks 2-9.

CONVENTION (S2135): L2 is CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER; d(b) = L2_F(full) - L2_F(block-b
motifs real) on FR (positive = that block's motif replacement costs damage; fits matched per arm).

S2167/S2168: block 5's motif replacement costs ~+0.064 FR, retrieval-class, distance-flat. Which of the eight
motif blocks carry the price? NULL: uniform (~+0.02 each). PRICE: none (attribution).

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) CONCENTRATION: max_b d(b) >= 2 x median_b d(b).
  (b) THE CLIFF LEADS: argmax_b d(b) = 5.
  (c) REPRODUCTION: the full arm reproduces S2144 (FR L2_F 2.6662 within 0.01).

Writes motif_leave_one_results.json. Self-reviewed. RUNG-74 HEADER FOLLOWS.

THE DISTANCE LAW AT THE CLIFF (rung 74): does the model divide retrieval by DEPTH?

CONVENTION (S2135): per-position CE(full S2144 config) - CE(block-5 motifs REAL) on FR; positive = the
replacement hurts there. Signed-sum bars per S2159.

S2167: the cliff's damage is 84.5% retrieval-class. S2162/S2164: the tail's ind damage RISES with distance
(peak 32-64). Motif heads are fixed-pattern and plausibly LOCAL - if the front's ind damage is NEAR-dominated,
the model divides retrieval by depth (front short-range, tail long-range): the first cross-depth architectural
law. NULL: the same rising profile (retrieval is depth-homogeneous).

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) NEAR CARRIES AT THE CLIFF: distance <= 32 positions carry >= 0.6 of the summed ind damage.
  (b) THE PEAK IS NEAR: the max-mean log2 bin (bins with >= 100 positions) is at distance < 16.
  (c) REPRODUCTION: the full arm reproduces S2144 (FR L2_F 2.6662 within 0.01).

Writes attn5_motif_distance_results.json. Self-reviewed. RUNG-73 HEADER FOLLOWS.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['motif_leave_one_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; full vs ALL-motifs-real arms (joint accounting; damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'motif_joint_results.json'
...
if __name__=='__main__':
    ...
    SEL['skipset']=(); SEL['clsdmg']=False; SEL['motif_off']=()
    print('ARM 1/2: full S2144 config (motifs at 2-9)',flush=True)
    r_full=main()
    SEL['motif_off']=tuple(range(2,10))
    print('ARM 2/2: ALL motifs real (no motif hooks anywhere)',flush=True)
    r_all=main()
    dall=round(r_full['L2_F']-r_all['L2_F'],4)
    pa=dall<=0.5*0.741
    pb=dall>=0.1888
    pc=abs(r_full['L2_F']-2.6662)<=0.01
    res={'full':{'L2_F':r_full['L2_F'],'L2_C':r_full['L2_C']},
         'all_motifs_real':{'L2_F':r_all['L2_F'],'L2_C':r_all['L2_C']},
         'joint_damage':dall,'sum_of_marginals':0.741,
         'convention':'d(all) = L2_F(full) - L2_F(all motifs real); positive = the 38-head replacement costs damage',
         'pred_a_subadditive':bool(pa),'pred_b_floor':bool(pb),'pred_c_reproduces_s2144':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'joint motif damage d(all) = {dall:+.4f}  (sum of marginals +0.741)')
    print(f"(a) d(all) {dall:+.4f} <= +0.3705: {'HELD' if pa else 'FAILED'}")
    print(f"(b) d(all) {dall:+.4f} >= +0.1888: {'HELD' if pb else 'FAILED'}")
