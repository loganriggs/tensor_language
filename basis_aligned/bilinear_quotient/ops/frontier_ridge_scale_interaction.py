"""IS THE SHRINKAGE INTRINSIC, OR IS IT COMPENSATING FOR THE RIDGE PENALTY? PENALTY x SCALE (Claude, LANE 1)

SS2917 established that INCREASING the ridge penalty is monotonically worse, and declared its own gap out loud: the grid was one-sided,
so `lambda < 1` -- LESS ridge than the fitted penalty -- was never tested, and the rung could rule out larger penalties without naming
an optimum. This closes that gap and asks a sharper question with the same machinery.

If the adopted x0.25 exists because the RIDGE BIAS makes the solution wrong, then how much shrinkage is needed should DEPEND on the
penalty: a solution fitted with less ridge would need a different scale. If instead the excess is intrinsic to the LOCAL OBJECTIVE
itself -- least squares against a local target, whatever the penalty -- the optimal scale should be the SAME at every penalty.

  optimum moves with the penalty   the shrinkage is entangled with the ridge, and x0.25 is not a clean object
  optimum is penalty-independent   the excess belongs to the local objective, not to the regulariser, and the
                                   scalar is a property of WHAT was fitted rather than HOW

Three penalties x five scales, one fitted stack, no refit -- the normal equations are cached, so each cell is a re-solve.

**Two controls carry this rung.** The cell (lambda x1, scale 1.0) must read **0.0000**: the fitted penalty with no scaling is the
baseline stack rebuilt from its own normal equations, so it is a physical no-op that proves the re-solve path is faithful -- the control
SS2879 taught me to register. The cell (lambda x1, scale 0.25) must reproduce **SS2896's -0.2287**.
...
SS2125 STANDS -- this re-solves an already-specified estimator and rescales it; it neither selects nor reorders.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_the_identity_cell_is_a_physical_no_op
#                     pred_c_the_fitted_penalty_quarter_scale_cell_reproduces_S2896
#                     pred_d_the_optimal_scale_is_the_same_at_every_penalty
#                     pred_e_the_scale_grid_brackets_every_optimum

Preregistration: polynomial_causal/FRONTIER_RIDGE_SCALE_INTERACTION_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run: a penalty x scale grid on the tail link maps, closing the gap SS2917 declared")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_ridge_scale_interaction_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
def main():
    ...
    def _apply_tail_lam_scale(spec):
        # SS2918: `_apply_tail_lambda` REBUILDS LW from the normal equations, so it must run BEFORE any
        # scaling or it silently discards it. This one knob does both, in that order.
        if spec is None:
            return
        _mult, _sc = spec
        ...
        for _li in ATTT:
            ...
            for _c in LINK:
                ...
                _sol = torch.linalg.solve(_XtX.to(DEV) + float(_mult)*1e-2*_n*_eye, _XtY.to(DEV))
                _new[_c] = (_sol * float(_sc)).to(_LW[_c].dtype)
    ...
    def _cfg(spec):
        ...
        _apply_tail_lam_scale(spec.get('tail_lam_scale'))
...
PREREG = PT + '../polynomial_causal/FRONTIER_RIDGE_SCALE_INTERACTION_PREREGISTRATION.md'
PREREG_SHA = "94aa740ea3c280228f9766dd9b2707f7f483352a8da974dd13ec58c7a5f75d26"
...
LM_GRID = [0.25, 1.0, 4.0]                  # x1 is the FITTED penalty; x0.25 closes the gap SS2917 declared
SC_GRID = [0.15, 0.25, 0.40, 0.60, 1.00]    # 0.25 must be interior
...
if __name__=='__main__':
    ...
    ARMS[:] = [('baseline', {})] + [(f'l{lm}_s{sc}', {'tail_lam_scale': (lm, sc)})
                                    for lm in LM_GRID for sc in SC_GRID]
    ...
    cost   = {(lm,sc): round(out[f'l{lm}_s{sc}']['L2_F']  - bF, 4) for lm in LM_GRID for sc in SC_GRID}
    cost_h = {(lm,sc): round(out[f'l{lm}_s{sc}']['L2_F2'] - b2, 4) for lm in LM_GRID for sc in SC_GRID}
    cost_c = {(lm,sc): round(out[f'l{lm}_s{sc}']['L2_C']  - bC, 4) for lm in LM_GRID for sc in SC_GRID}
    best_scale = {lm: min(SC_GRID, key=lambda sc: cost[(lm,sc)]) for lm in LM_GRID}
    best_cost  = {lm: cost[(lm, best_scale[lm])] for lm in LM_GRID}
    ident  = cost[(1.0, 1.0)]                    # fitted penalty, no scaling: must be a physical no-op
    anchor = cost[(1.0, 0.25)]                   # SS2896's adopted tail scalar, re-measured here
    interior = all(best_scale[lm] not in (SC_GRID[0], SC_GRID[-1]) for lm in LM_GRID)
    same_optimum = len(set(best_scale.values())) == 1

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = abs(ident) <= BARS['identity']
    pc = abs(anchor - S2896_COST) <= BARS['anchor']
    pd = bool(same_optimum)
    pe = bool(interior)
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_identity_cell_is_a_physical_no_op':bool(pb),
           'pred_c_the_fitted_penalty_quarter_scale_cell_reproduces_S2896':bool(pc),
           'pred_d_the_optimal_scale_is_the_same_at_every_penalty':bool(pd),
           'pred_e_the_scale_grid_brackets_every_optimum':bool(pe)}
    nulls={'b_null_the_machinery_is_not_a_noop':bool(abs(ident) > 0.02),
           'c_null_the_anchor_fails':bool(abs(anchor - S2896_COST) >= 0.03),
           'd_null_the_shrinkage_is_compensating_for_the_ridge_penalty':bool(not same_optimum),
           'e_null_the_scale_grid_is_too_narrow':bool(not interior)}
    res={'rung':'frontier_ridge_scale_interaction','preds':preds,'nulls':nulls,'bars':BARS,
         'arms':out,'published_L2_F':2.6735,
         'grids':{'lambda_multiplier':LM_GRID,'scale':SC_GRID},
         'summary':{'L2_F_baseline':round(bF,4),'L2_F2_baseline':round(b2,4),
                    'cost_fresh':{f'l{l}_s{c}':v for (l,c),v in sorted(cost.items())},
                    'cost_holdout':{f'l{l}_s{c}':v for (l,c),v in sorted(cost_h.items())},
                    'cost_fit':{f'l{l}_s{c}':v for (l,c),v in sorted(cost_c.items())},
                    'best_scale_per_lambda':{str(k):v for k,v in best_scale.items()},
                    'best_cost_per_lambda':{str(k):v for k,v in best_cost.items()},
                    'identity_cell_cost':ident,'anchor_cell_cost':anchor,
                    'S2896_adopted_cost':S2896_COST,
                    'anchor_deviation':round(abs(anchor-S2896_COST),4),
                    'optimum_is_penalty_independent':bool(same_optimum),
    ...
    print(f"(b) identity cell (lambda x1, scale 1.0) {ident:+.4f}, |.| <= 0.005: {'HELD' if pb else 'FAILED'}")
    print(f"(c) cell (lambda x1, scale 0.25) {anchor:+.4f} vs SS2896 {S2896_COST:+.4f}: {'HELD' if pc else 'FAILED'}")
    for lm in LM_GRID:
        print(f"    lambda x{lm}: " + " | ".join(f"s{sc} {cost[(lm,sc)]:+.4f}" for sc in SC_GRID)
              + f"   -> best scale {best_scale[lm]} at {best_cost[lm]:+.4f}")
    print(f"(d) optimal scale independent of the penalty: {best_scale}: {'HELD' if pd else 'FAILED'}")
