...
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';OUT=A/'native_independent_source_swap_v1_result.json'
...
def main():
    ...
    swaps=json.loads((P/'INDEPENDENT_SOURCE_SWAP_V1.json').read_text());assert swaps['source_sha256']==hashlib.sha256(frozen_path.read_bytes()).hexdigest();swap_lookup={(c['dataset'],c['panel'],c['role'],c['template']):c for c in swaps['contexts'] if c['omitted_source']==1}
