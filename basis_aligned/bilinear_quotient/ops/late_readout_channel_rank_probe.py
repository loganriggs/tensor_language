#!/usr/bin/env python
"""late_readout_channel_rank_probe -- how wide is the late stack's readout side-channel (§2748)? The routed out-of-core write sum
(384-dim complement of the 768 read core) is truncated to its top-r fit-set PCs before it reaches the final residual, r = 32..256,
alone and inside the BUS program. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_side_channel_128 pred_c_side_channel_64 pred_d_bus_with_128_side_channel
#                     pred_e_side_channel_is_low_rank
...
Preregistration: polynomial_causal/LATE_READOUT_CHANNEL_RANK_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_readout_channel_rank_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_readout_channel_rank_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_READOUT_CHANNEL_RANK_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "whole_model_width_program_probe_results.json"   # §2751
OUT = ROOT / "late_readout_channel_rank_probe_results.json"
HASHES = {PREREG: "d9618fa92ffdb3056789aca6cd0b551085322c69dd961c4ba68a9f15ed0828c5", PRIOR: "a3df6484be4f8973f7711b1f887be39e2d8ab0adceda5af51aadc2efdb98cafc",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_readout_channel_rank_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_TO_READOUT_768 = 0.050; PRIOR_BUS_768 = 0.105; LAM = 1e-2
K = 768; RS = [32, 64, 128, 256]; LATE = list(range(11, 18))
SITES = [("mlp", l) for l in LATE]; ASITES = [("attn", l) for l in LATE]; FINAL = ("final", -1)
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_max": 0.03, "c_max": 0.06, "d_max": 0.03, "e_max": 150.0}
NULLS = {"b_min": 0.10, "c_min": 0.15, "d_min": 0.10, "e_min": 300.0}
...
def main():
    ...
    class FinalAdd:
        """Adds sign * buffer to the final residual; with Q (D x r) and mean q0 the buffer is first truncated: q0 + Q Q^T (acc - q0)."""
        def __init__(self, buf, sign, Q=None, q0=None): self.buf = buf; self.sign = sign; self.Q = Q; self.q0 = q0
        def hook(self, idx):
            pass
        def __call__(self, x, _):
            if self.buf.acc is None:
                return x
            acc = self.buf.acc if self.Q is None else self.q0 + ((self.buf.acc - self.q0) @ self.Q) @ self.Q.T
            return x + self.sign * acc
    class Both:
    ...
    out_energy = {f"{s_[0]}{s_[1]}": {str(k): outfrac(wheads[s_]["Cw"], U_read[:, :k]) for k in (512, K)} for s_ in SITES + ASITES}
    ...
    # second fit pass: covariance of the routed out-of-core sum (the readout side-channel)
    Gq = torch.zeros(D, D, dtype=torch.float64, device=DEV); sq = torch.zeros(D, dtype=torch.float64, device=DEV); nq = 0
    for i in range(0, fit_rows.shape[0], CH):
        idx = fit_rows[i:i + CH, :TI]; buf = Buf()
        forward(m, idx, patch={s_: Route(s_, K, U_read, "readout", buf) for s_ in SITES + ASITES})
        q = buf.acc.reshape(-1, D).double(); Gq += q.T @ q; sq += q.sum(0); nq += q.shape[0]
    q0 = sq / nq; Cq = Gq / nq - torch.outer(q0, q0); evq, Uq = torch.linalg.eigh(Cq); Uq = Uq.flip(1).float(); q0 = q0.float()
    qspec = R.spectrum(Cq.cpu()); q_mean_energy = float((q0.double() ** 2).sum() / (Cq.trace() + (q0.double() ** 2).sum()))
    log(stage="side_channel", eff_rank=round(qspec["eff_rank"], 1), rank_90=qspec["rank_90"], mean_energy_frac=round(q_mean_energy, 4))
    def routed(k, mode, with_reads=False, r=None):
        buf = Buf(); patch = {}
        for s_ in SITES + ASITES:
            r_ = Route(s_, k, U_read, mode, buf); patch[s_] = Both(reader(s_), r_) if with_reads else r_
        objs = [buf] + list(patch.values())
        patch[FINAL] = FinalAdd(buf, +1.0, None if r is None else Uq[:, :r], None if r is None else q0)
        return run(patch, objs)
    arms = {"TO_READOUT_768": routed(K, "readout"), "BUS_768": routed(K, "readout", with_reads=True)}
    for r in RS:
        arms[f"TO_READOUT_768_R{r}"] = routed(K, "readout", r=r)
    arms["BUS_768_R128"] = routed(K, "readout", with_reads=True, r=128)
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    gaps = {f"R{r}_minus_full": arms[f"TO_READOUT_768_R{r}"] - arms["TO_READOUT_768"] for r in RS}
    gaps["bus_R128_minus_bus"] = arms["BUS_768_R128"] - arms["BUS_768"]
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["TO_READOUT_768"] - PRIOR_TO_READOUT_768) <= BARS["repro_tol"] and abs(arms["BUS_768"] - PRIOR_BUS_768) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_side_channel_128': bool(gaps["R128_minus_full"] <= BARS["b_max"]),
        'pred_c_side_channel_64': bool(gaps["R64_minus_full"] <= BARS["c_max"]),
        'pred_d_bus_with_128_side_channel': bool(gaps["bus_R128_minus_bus"] <= BARS["d_max"]),
        'pred_e_side_channel_is_low_rank': bool(qspec["eff_rank"] <= BARS["e_max"]),
    }
    nulls = {"b_null_ge_.10": bool(gaps["R128_minus_full"] >= NULLS["b_min"]), "c_null_ge_.15": bool(gaps["R64_minus_full"] >= NULLS["c_min"]),
             "d_null_ge_.10": bool(gaps["bus_R128_minus_bus"] >= NULLS["d_min"]), "e_null_ge_300": bool(qspec["eff_rank"] >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "k": K, "rs": RS,
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "to_readout_768": arms["TO_READOUT_768"], "prior_to_readout_768": PRIOR_TO_READOUT_768, "bus_768": arms["BUS_768"], "prior_bus_768": PRIOR_BUS_768},
           "ce_added": arms, "gaps": gaps, "side_channel": {"eff_rank": qspec["eff_rank"], "rank_90": qspec["rank_90"], "mean_energy_frac": q_mean_energy, "max_dim": D - K},
