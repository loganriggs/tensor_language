#!/usr/bin/env python3
# BQGATE: five frozen predictions; family, EVAL set, units (v197 + v207 + v211 receipts), recipe and bars fixed before the run.
"""v212: within-family separability for v211's complement-type passes -- the count test for modal_ought, let_want_complement, adjective_finiteness.

v211 put the fifth eight spec-authored behaviours through the four-row battery; its three four-row passes are all complement-type
siblings (bare vs to / to vs that readouts), so none counts before this pass. Recipe as v198/v202/v206/v208/v210: per evaluated
member, arm `own` (own C_fit control) and arm `fam` (own C_fit + EVERY sibling's A1 FIT prep, control_weight 30 x len(controls));
evaluated on the member's held A1 (extraction), each sibling's held A1 (|CE| leak) and held C. Units frozen from the parent receipts.
...
Family (FULL nine-member control set, per proposal (iv)): complement_type = causative_complement, rather_prefer, finiteness_selection
(v208's three), gerund_selection, adjective_complement, perception_complement (v209/v210), modal_ought, let_want_complement,
adjective_finiteness (v211). All nine are prepared as controls; only FIVE are evaluated (EVAL): the three v211 passes and the two
already-counted members (rather_prefer, finiteness_selection), whose count depends on staying separable under the enlarged set --
v210's log showed reflexive_person losing separability when the person family grew from four to six controls. The non-four-row
members are controls only (their verdicts would count for nothing). ~80-100 s per member (~8 min).

REGISTERED BEFORE THE RUN (each coded predicate is the sentence here):
    pred_a_modal_ought           modal_ought is separable from the other eight complement-type members.               prior 55%
    pred_b_new_passes            at least 2 of the 3 v211 passes (modal_ought, let_want_complement, adjective_finiteness) are
                                 separable (v210: 6 of 6 complement members separable under six controls).           prior 60%
    pred_c_counted_kept          rather_prefer and finiteness_selection are BOTH still separable under the nine-member set (they
                                 were under three, v208, and six, v210 log).                                          prior 65%
    pred_d_row4_kept             fam-arm own-C UB975 <= 0.01 on >= 3 of the 5 evaluated members.                      prior 55%
    pred_e_instrument            finiteness_selection own-arm extraction within +-0.03 of its v208 own-arm value 1.015 (same units,
                                 seed, split and code path: a determinism check, NOT an independent run outcome).     prior 90%
Smoke: V212_SMOKE=<out.json> (CPU, V212_SMOKE_ROWS=4; all nine members are prepared as controls, V212_SMOKE_NAMES=modal_ought,finiteness_selection
are evaluated so the instrument read and the controls-beyond-EVAL path are both exercised; steps 5).
"""
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v212_result.json"
...
FAMILIES = {
    "complement_type": ["causative_complement", "rather_prefer", "finiteness_selection", "gerund_selection",
                        "adjective_complement", "perception_complement", "modal_ought", "let_want_complement",
                        "adjective_finiteness"],
}
EVAL = ("modal_ought", "let_want_complement", "adjective_finiteness", "rather_prefer", "finiteness_selection")
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES}
...
BARS = {"cross_max": 0.05, "keep": 0.80, "ext_floor": 0.50, "c_ub_max": 0.01, "k_b": 2, "k_d": 3, "instr_tol": 0.03}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 20000, 640000


def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v212", "members": n, "families": list(FAMILIES),
...
def PREDS(R):
    ...
    a = ok and sep("modal_ought")
    b = ok and sum(1 for n in ("modal_ought", "let_want_complement", "adjective_finiteness") if sep(n)) >= B["k_b"]
    c = ok and sep("rather_prefer") and sep("finiteness_selection")
    d = ok and sum(1 for n in good if good[n]["arms"]["fam"]["C"]["ce_ub975"] <= B["c_ub_max"]) >= B["k_d"]
    e = ok and INSTRUMENT in good and abs(good[INSTRUMENT]["arms"]["own"]["extraction"] - _v208_own()) <= B["instr_tol"]
    return {"pred_a_modal_ought": bool(a), "pred_b_new_passes": bool(b), "pred_c_counted_kept": bool(c),
...
def main() -> None:
    ...
    smoke = os.environ.get("V212_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V212_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    evaluate = [n for n in members if n in EVAL and (not smoke or n in os.environ.get("V212_SMOKE_NAMES", "modal_ought,finiteness_selection").split(","))]
    prior = {**json.loads(V197.read_text())["behaviours"], **json.loads(V201.read_text())["behaviours"], **json.loads(V204.read_text())["behaviours"], **json.loads(V205.read_text())["behaviours"], **json.loads(V207.read_text())["behaviours"], **json.loads(V209.read_text())["behaviours"], **json.loads(V211.read_text())["behaviours"]}
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v212", "candidate_id": "corpus.unit_family_separability_spec_v212",
              "separable_counts": nsep, "families": FAMILIES, "summary": summary, "members": R, "bars": BARS,
              "recipe": {"lambda": LAM, "steps": steps, "lr": LR, "complement_weight": CW, "rank": 1, "seed": 0, "units_source": "v197 + v201 + v204 + v205 + v207 + v209 + v211 receipts (later receipts override earlier ones)", "evaluated": list(EVAL),
