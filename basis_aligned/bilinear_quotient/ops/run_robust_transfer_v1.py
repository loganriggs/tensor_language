...
def main():
 ...
 import tiktoken
 plan=json.loads((P/'ROBUST_TRANSFER_PLAN_V1.json').read_text())
 ...
 model=Bilin18TorchBackend.load('cuda').model.float();graph=move_preserving_dtype(torch.load(P/'ROBUST_DIRECTION_DIRECTED_PROGRAM_V1.pt',weights_only=True),'cuda');bundles=move_preserving_dtype(torch.load(P/'ROBUST_MATCHED_BASELINES_V1.pt',weights_only=True),'cuda')
 ...
 for panel in (1,2):
  ...
  for domain in ('fineweb','stdlib'):
   ...
   for offset in range(0,len(valid),96):
    ids=valid[offset:offset+96];donors=mapping[ids];gi,gd=ids.cuda(),donors.cuda();h,m,md=cache['h'][gi],cache['m'][gi],cache['m'][gd];hybrid=h-m+md;vv={k:[] for k in ['true']+[f'{c}_{mode}' for c in ('graph',*bundles) for mode in ('boundary','generated')]}
 ...
 relative=all(all(g<=1.1*b for g,b in zip(c['candidates']['graph_generated']['relative_error'],c['candidates'][base+'_generated']['relative_error'])) for c in summaries for base in bundles)
