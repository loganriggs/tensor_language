...
"""DOES A WIDER HEAD NEED A SMALLER SHARE OF ITS DIRECTIONS? (v787). The rank axis now reads, for recovery ~0.95-0.97: softmax twin 16 of a
128-dim head (12.5%, 5B tokens), Pythia-160m 20 of 64 (31%, 8.4B tokens), GPT-2 small ~34 of 64 (~53%, ~40B tokens), Pythia-160m 46 of 64 (70%,
300B tokens). The twin wins partly because its heads are twice as wide, and the v775 census hinted that wider heads use a smaller FRACTION of
their directions (input-weighted participation ratio ~10% for Pythia-1b's 256-dim heads against ~15% for 64-dim heads). This rung controls for
that inside one family: Pythia-1b final (256-dim heads, the same 300B tokens as Pythia-160m) at rank 64 and rank 96 — 25% and 37.5% of its head.
If a wide head needs a much smaller share, 1b should clear 0.95 at rank 64 where 160m needed 70% of its width. ROW-CENTRED logits (Logan, 22 Sep:
'test the specific hypothesis of why we're seeing what we're seeing'). MODEL = EleutherAI/pythia-160m (12 x 12 x 768, softmax, ROTARY on 16 of 64 head dims, GPT-NeoX tokenizer;
...
PREDICTIONS (scored as written; failures preserved)
    ...
    pred_c_rank32_cost          (rank 96 = 37.5% of the head) snapshot recovery >= 0.97 (rank 32: 0.928, v773). Prior: unsure
    pred_d_rank16_cost          (rank 64 = 25% of the head) snapshot recovery >= 0.95 — the head-width control: 160m needed 70% of its width for this. Prior: unsure
    pred_e_rank32_values_preserved_scale rank-32: median value ratio in [0.5, 2] over the 24 most valuable heads. Prior: likely
PRICE (registered maximum): stock 7 + explicit 7; means 16; kernels 2; 128 x 7 = 896 (Pythia-1b: 16 layers x 8 heads); all-ablated 7; kernels-only 7 + col0 7; ladder 35; 2 arms x (kappa_r 26 + step-0 10 + 300 + 12 x (26 + 7 + 7) + snapshot 9) = 1650 + manipulability 168; total ~3000 forwards, 600 backwards. Bars: forwards <= 3400, backwards <= 600.
"""
...
KS, ARMS = (1, 2, 4, 8, 16), (64, 96)
REPLAY_TOL, E4_MIN, REC96, REC64, RATIO = 0.002, 0.90, 0.97, 0.95, (0.5, 2.0)
PREDICTIONS = {"pred_a_instrument_replays": "+-0.002", "pred_b_kernel_energy_top4": ">= 0.90", "pred_c_rank32_cost": "rank 96 recovery >= 0.97", "pred_d_rank16_cost": "rank 64 recovery >= 0.95", "pred_e_rank32_values_preserved_scale": "median ratio in [0.5, 2]"}
...
def main() -> None:
    plan = {"candidate_id": CANDIDATE_ID, "forwards_max": FORWARDS_MAX, "model_backwards": BACKWARDS_MAX, "model_updates": 0, "fit_parameters": "2 arms of factored q/k maps + kernels",
            ...
            "bars": {"replay_tol": REPLAY_TOL, "e4_min": E4_MIN, "rec96": REC96, "rec64": REC64, "ratio": RATIO}}
    ...
    r48, r40 = results[96], results[64]
    predictions = {"pred_a_instrument_replays": abs(native - native_stock) <= REPLAY_TOL, "pred_b_kernel_energy_top4": float(energy[3]) >= E4_MIN, "pred_c_rank32_cost": r48["recovery"] >= REC96,
                   "pred_d_rank16_cost": r40["recovery"] >= REC64, "pred_e_rank32_values_preserved_scale": RATIO[0] <= r48["median_ratio"] <= RATIO[1]}
