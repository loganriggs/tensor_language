#!/usr/bin/env python3
# BQGATE:204bodyforwards;4captureprefixes;180seconds;no fitting.
...
pred_c combined source/attention bytes<=independent EACHprefix.
Null: native suffix amplifies local arithmetic error or prices erase saving.
Price204bodyforwards,4captureprefixes,4recipients25strengths,180seconds.
...
from coupled_attention10_ports_v2 import compile_program as compile_attention,execute as execute_attention,MAPS
from inherited_source_positions_v1 import paired_masks
STEM='COUPLED_NATURAL_LENGTH_V1'
RECIPIENTS=[0,1,2,3]
GRID=[-1,0,.5,1,2]
...
def main():
    ...
    rows=json.loads((P/'COUPLED_NATURAL_LENGTH_V1_ROWS.json').read_text())['rows']
    assert len(rows)==4 and len(STRENGTHS)==25
    assert all(len(rows[i]['ids'])==len(rows[i^1]['ids']) for i in range(4))
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
        ...
        print('204bodyforwards;4captureprefixes;4recipients25coupledstrengths native/compiled');return
    ...
    def readouts(scores,row):
        return torch.stack([-scores.log_softmax(-1)[row['target_id']],scores[198]-scores[11]]).double().cpu()
    rel=lambda a,b:float((a.double()-b.double()).norm()/b.double().norm().clamp_min(1e-30))
    try:
        for i in RECIPIENTS:forward(i,'capture')
        for i in RECIPIENTS:
    ...
    result={'pred_a':all(r['state_error']<=1e-5 and r['post10_error']<=1e-5 and r['score_error']<=1e-5 for r in records),
            ...
            'pred_c':all(p['ratio']<=1 for p in prices),
            ...
            'scope':'Combined coupled MLP9 and complete attention10 programs prepared once per pristine context; native suffix tested over100 signed cases on84/98-token natural text pairs. Mixed strength tied to a*b. Context/source/MLPweights remain; no static model compression or independent role identification.'}
