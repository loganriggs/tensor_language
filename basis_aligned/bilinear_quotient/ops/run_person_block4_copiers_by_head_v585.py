#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_a_block_4_head_carries pred_c_block_4_singles_add pred_d_4_5_replays pred_e_at_most_two_heads_matter
"""The person line's block-4 copiers by head (v585). v584: block 4's writes into the positions after the cue carry 0.069 of the myself - yourself gap, 4.5 alone
0.018. Here block 4's nine heads singly (post-cue writes) on the 43 pairs.
PREDICTIONS (scored as written; failures preserved; priors from v584)
    pred_a_baseline_replays        the unedited manual forward replays the model's myself - yourself margins within 1e-3
    pred_b_a_block_4_head_carries  the largest single closes >= 0.03
    pred_c_block_4_singles_add     the nine singles sum to within 0.03 of 0.069 (one block, disjoint heads, same targets)
    pred_d_4_5_replays             4.5 closes 0.018 +- 0.015
...
OUT = ROOT / "circuits/followups/person_block4_copiers_by_head_v585_result.json"
CANDIDATE_ID = "person.block4_copiers_by_head_v585"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, TOP_MIN, B4_V584, ADD, H45_V584, HBAND, MATTER, UNIT = 1e-3, 0.03, 0.069, 0.03, 0.018, 0.015, 0.02, 701
...
FORWARDS_MAX = 21
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_a_block_4_head_carries": ">= 0.03", "pred_c_block_4_singles_add": "0.069 +- 0.03", "pred_d_4_5_replays": "0.018 +- 0.015", "pred_e_at_most_two_heads_matter": "<= 2 heads >= 0.02"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "top_min": TOP_MIN, "b4_v584": B4_V584, "add": ADD, "h45_v584": H45_V584, "hband": HBAND, "matter": MATTER, "unit": UNIT}}
    ...
    SETS_W = {f"4.{h}": {4: [h]} for h in range(N_HEAD)}; MODES = {m: ({}, (0,)) for m in SETS_W}
    ...
    top = max(mc, key=lambda k: abs(mc[k])); print("block-4 heads (post-cue writes)", {k: round(v, 3) for k, v in mc.items()}, "sum", round(sum(mc.values()), 3), "top", top)
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_a_block_4_head_carries": abs(mc[top]) >= TOP_MIN, "pred_c_block_4_singles_add": abs(sum(mc.values()) - B4_V584) <= ADD,
                   "pred_d_4_5_replays": abs(mc["4.5"] - H45_V584) <= HBAND, "pred_e_at_most_two_heads_matter": sum(1 for v in mc.values() if v >= MATTER) <= 2}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "person_block4_copiers_by_head_result_v585", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
