...
STEM='CITY_FULL_FRESH_V1';ARMS=['native','native8_midpoint','full_reentry','compiled']+[f'null:{k}' for k in range(16)]
...
def main():
 ...
 tables={k:v.cuda() for k,v in torch.load(P/'CITY_FULL_FRESH_V1_TABLES.pt',weights_only=True).items()}
 ...
 def pre9(module,args):
  ...
  if arm=='native':c['x9']=x.clone()
   ...
   row=groups[state['i']];seed=17092900+1000*int(arm.split(':')[1])+row['context_id'];generator=torch.Generator(device=x.device).manual_seed(seed)
