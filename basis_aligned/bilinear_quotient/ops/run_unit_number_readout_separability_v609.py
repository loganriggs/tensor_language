...
"""v609: three cross-construction four-row passers, all in ONE family -- distinct, or one circuit counted thrice?

WHERE THEY CAME FROM. Five pooled batches lifted 40 non-prepositional cells (v591, v595, v597, v599, v601). Exactly
THREE cleared all four tier rows: determiner_number_crates (v597), lexical_number (v599) and numeral_dual_both_all
(v601). Under the protocol a four-row pass is a candidate, not a circuit -- the corpus number moves only after the
behaviour is shown separable from the family it would join.
WHY THIS ONE NEEDS CARE. All three fall under `number_readout`, a family with 17 counted members. They came from
three different constructions -- a determiner, a lexical number word and a numeral quantifier -- but they answer on
the same axis, and the corpus record is that identity is the cue -> token MAPPING and that margins degrade against
mapping-sharing siblings. So the live worry is the one the amid rung had: three candidates that are really ONE
number direction counted three times. Booking three duplicates would be worse than booking none, because a duplicate
is invisible once it is in the count.
HOW IT CAN REFUTE. All three sit in the SAME family list as six counted number_readout members, so the separability
metric measures each candidate's leak against the other two candidates as well as against the counted six. If they
encode one direction their mutual leaks exceed cross_max = 0.05 and `separable` goes false; if they are three
behaviours, all three clear it while sitting beside each other. The six counted members are re-evaluated in the same
run so a null cannot be blamed on the family -- the control row I left unmeasured in v577 and had to re-run as v579.
A CAVEAT I AM CARRYING FORWARD FROM v603. Row 4, one of the four rows these three passed, is a binary threshold
placed inside its own spread: the same cdas direction evaluated on a second same-answer control flipped five of ten
verdicts, with the two controls' C upper bounds correlating at only r = 0.604. So "cleared all four rows" is a
weaker statement than it looks, and separability here does not repair that. If these three separate, they are
candidates whose row 4 rests on one control -- and I will say so when reporting them rather than presenting a clean
four-row pass.
NOTHING IS COUNTED BY THIS FILE. The count stays 139.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
  pred_a_new_separable ALL THREE candidates are separable at cross_max = 0.05 while sitting in one family with each
                       other and with six counted members -- k_new = 3, so a single fusion breaks it. prior 50%
  pred_b_counted_kept  at least k_kept = 4 of the six counted members stay separable in the same run, so a null on
                       the candidates cannot be blamed on the family having stopped separating.       prior 85%
  ...
  pred_d_instrument    every EVAL member reproduces its parent battery extraction within instr_tol = 0.03 -- v597,
                       v599 and v601 for the three candidates.                                        prior 85%
HOW IT READS. a TRUE with b: three constructions that answer on the same axis are nonetheless three behaviours, the
cross-construction backlog yields countable candidates at roughly three per forty cells, and the route to inventory
outside the prepositions is real. a FALSE with b true: they fuse, the three four-row passes are one number direction
seen three times, and the yield from 40 non-prepositional cells is ONE candidate rather than three -- which is the
more valuable answer and the reason this runs before anything is booked. a partly false: whichever pair fuses names
the two that share a direction, which is finer than either.
SCOPE. Three candidates, six counted controls, one family. No counting, and row 4 for these three still rests on a
single control.
Smoke: V609_SMOKE=<out.json> (CPU).
"""
...
OUT = ROOT / "circuits/followups/unit_number_readout_separability_v609_result.json"
...
FAMILIES = {
    "number_readout": ['agreement_do_does', 'agreement_lifts_lift', 'demonstrative_number', 'demonstrative_number_that_those', 'number_anaphor_it_them', 'number_reflexive_myself_ourselves', 'determiner_number_crates', 'lexical_number', 'numeral_dual_both_all'],
}
COUNTED_LIST = ['agreement_do_does', 'agreement_lifts_lift', 'demonstrative_number', 'demonstrative_number_that_those', 'number_anaphor_it_them', 'number_reflexive_myself_ourselves']
COUNTED = tuple(COUNTED_LIST)
EVAL = tuple(COUNTED_LIST) + tuple(['determiner_number_crates', 'lexical_number', 'numeral_dual_both_all'])
...
BARS = {"cross_max": 0.05, "keep": 0.8, "k_kept": 4, "ext_floor": 0.5, "c_ub_max": 0.01, "k_d": 2, "k_new": 3, "instr_tol": 0.03}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 20000, 640000


def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_number_readout_separability_v609", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V609_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V609_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    evaluate = [n for n in members if n in EVAL and (not smoke or n in os.environ.get("V609_SMOKE_NAMES", "determiner_number_crates,lexical_number").split(","))]
    ...
    result = {"predictions": predictions, "schema": "unit_number_readout_separability_v609", "candidate_id": "corpus.unit_number_readout_separability_v609",
