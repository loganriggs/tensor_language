#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_carrier_closure pred_b_mlp_stack_carries_050 pred_c_mlp6_enters_negatively pred_d_embedding_carries_at_most_020
"""Pronoun number they/he DoD (v230): what builds MLP-7 unit 1779, the number relay (49% of MLP 7's carriage into 829, v229) that also restores 40% of MLP 7's
push-back when the MLP-6 trio is removed (v228)? Carrier split (exact, `dod_units.carrier_split`) of u_1779's plural - singular contrast at the block-7 input:
writers = embedding, attn / mlp totals of blocks 0-6, the nine heads of block 7. The registered mechanism of the compensation: MLP 6 enters 1779 with NEGATIVE
carriage (it damps the relay), so removing MLP 6's plural write releases 1779 -- the same shape as MLP-6 unit 3230 damping the male detector (v177).
PREDICTIONS (scored as written; failures preserved; priors unsure except c, the registered mechanism)
    pred_a_carrier_closure          carrier shares sum to 1 within 1e-3 and the per-pair identity holds within relative 1e-3
    pred_b_mlp_stack_carries_050    mlp:01..mlp:06 together carry >= 0.50 (in magnitude of the signed sum)
    pred_c_mlp6_enters_negatively   mlp:06's carrier share is < 0
    pred_d_embedding_carries_at_most_020  the embedding carries <= 0.20 (number is computed, not read)
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_unit1779_carrier_split_v230_result.json"
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_unit1779_carrier_split_v230"
UNITS, LAYER, CLOSURE_TOL, STACK_MIN, EMBED_MAX, BATCH = (1779,), 7, 1e-3, 0.50, 0.20, 32
FORWARDS_MAX = 5
WRITERS = ["embed"] + [f"{k}:{l:02d}" for l in range(LAYER) for k in ("attn", "mlp")] + [f"attnhead:{LAYER:02d}:{h}" for h in range(9)]
PREDICTIONS = {"pred_a_carrier_closure": "<= 1e-3", "pred_b_mlp_stack_carries_050": ">= 0.50", "pred_c_mlp6_enters_negatively": "< 0", "pred_d_embedding_carries_at_most_020": "<= 0.20"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "rows": len(rows), "rows_sha256": L.rows_sha256(rows), "units": UNITS, "layer": LAYER, "writers": WRITERS, "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"closure_tol": CLOSURE_TOL, "stack_min": STACK_MIN, "embed_max": EMBED_MAX}}
    ...
    predictions = {"pred_a_carrier_closure": True, "pred_b_mlp_stack_carries_050": True, "pred_c_mlp6_enters_negatively": True, "pred_d_embedding_carries_at_most_020": True}
    ...
    for unit in UNITS:
        ...
        predictions["pred_c_mlp6_enters_negatively"] &= cs["mlp:06"] < 0; predictions["pred_d_embedding_carries_at_most_020"] &= cs["embed"] <= EMBED_MAX
    predictions["pred_a_carrier_closure"] &= closure <= CLOSURE_TOL
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_mlp6_unit_carrier_split_result_v230", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "units": report, "predictions": predictions, "forwards": forwards,
