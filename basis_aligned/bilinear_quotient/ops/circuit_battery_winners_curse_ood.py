"""circuit_battery_winners_curse_ood -- does selection survive a change of POPULATION, not just a change of sample size?

SS2864 selected the most-selective component on FIT and evaluated on TEST: the argmin agreed on 0 of 7, selection still beat a random
live component by -.266, inflation was a mild .063, and the picked component was indistinguishable from the named writer (-.020).
A precision replication at 1.5x the rows is already queued for the two clauses that landed between bar and null.

This rung varies the other axis. It selects on FIT exactly as before but evaluates on **OOD** -- situations built from held-out
vocabulary pools, disjoint from FIT and TEST alike. Sample size and population are different failure modes and SS2865/SS2866 have shown
this family of designs is sensitive to the first; nothing yet tests the second. If selection still beats random on OOD, the signal in
the metric survives a genuine distribution change; if it collapses, then what selection was tracking was specific to the pools the
selection split shares with TEST.

Bars carried over verbatim from the winner's-curse preregistration.
...
Preregistration: polynomial_causal/CIRCUIT_BATTERY_WINNERS_CURSE_OOD_PREREGISTRATION.md
"""
...
SELF = Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/"
            "circuit_battery_winners_curse_ood.py")
...
ROOT = R.ROOT
PREREG = R.POLY / "CIRCUIT_BATTERY_WINNERS_CURSE_OOD_PREREGISTRATION.md"
...
RUNG = "circuit_battery_winners_curse_ood"
SMOKE = os.environ.get("SURROGATE_SMOKE") == "1"
OUT = ROOT / (f"{RUNG}_smoke_results.json" if SMOKE else f"{RUNG}_results.json")
HASHES = {PREREG: "2f0fdc1251c7edb16bc725da2228fb4a5c414bcad3fe5a2f240470e863346fd2", NULLRES: "b4bf278a10fea17e656db30e2d138fb219dc3c8cf5319282b307df5b08aa6295",
...
def main():
    ...
    for tid in tasks:
        ...
        fv = {k: v["selectivity"] for k, v in per_split["FIT"].items() if v["selectivity"] is not None}
        tv = {k: v["selectivity"] for k, v in per_split["OOD"].items() if v["selectivity"] is not None}
        pick = min(fv, key=fv.get) if fv else None
        print(f"[wc] {tid:30s} FIT-pick={str(pick):8s} fit={fv.get(pick, float('nan')):.3f} "
              f"test={tv.get(pick, float('nan')):.3f} live={len(fv)}/{len(comps)}", flush=True)

    w = f"{WRITER[0]}{WRITER[1]}"
    rng = np.random.default_rng(20260904)
    picks, infl, vs_writer, vs_random, argmin_agree = {}, {}, {}, {}, {}
    for tid, ps in results.items():
        fv = {k: v["selectivity"] for k, v in ps["FIT"].items() if v["selectivity"] is not None}
        tv = {k: v["selectivity"] for k, v in ps["OOD"].items() if v["selectivity"] is not None}
        if not fv or not tv:
            continue
    ...
    preds = {
        ...
        'pred_e_inert_components_are_gated_out': bool(
            any(v["selectivity"] is None for ps in results.values() for v in ps["FIT"].values())),
    }
    nulls = {
