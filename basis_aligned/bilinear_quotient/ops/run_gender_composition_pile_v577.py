#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_composition_replays_on_pile pred_c_copiers_post_noun_replay pred_d_noun_values_carry pred_e_joint_at_least_the_sum
"""The gender composition on the PILE rows (v577). v576 (fineweb, 61 pairs): the two copiers' post-noun writes close 0.256, and with the five readers' values at
the noun 0.898 -- the whole channel. Here the same three edits (plus the readers' noun values alone) on v75's 64 pile-10k pairs, the second natural gender set.
PREDICTIONS (scored as written; failures preserved; priors from v549 / v576)
    ...
    pred_b_composition_replays_on_pile COPIERS_POST_AND_NOUN closes 0.898 +- 0.10 on the pile pairs
...
OUT = ROOT / "circuits/followups/gender_composition_pile_v577_result.json"
CANDIDATE_ID = "gender.composition_pile_v577"
N_HEAD = 9
...
PILE = dod_battery.ROOT / "circuits/followups/pronoun_gender_dod_pile_rows_v75.json"
...
FORWARDS_MAX = 9
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_composition_replays_on_pile": "0.898 +- 0.10", "pred_c_copiers_post_noun_replay": "0.256 +- 0.08", "pred_d_noun_values_carry": ">= 0.45", "pred_e_joint_at_least_the_sum": ">= NOUN + COPIERS - 0.05"}
...
def main() -> None:
    ...
    print("pile: noun values", round(mc["NOUN"], 3), "copiers post-noun", round(mc["COPIERS_POST"], 3), "joint", round(mc["COPIERS_POST_AND_NOUN"], 3))
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_composition_replays_on_pile": abs(mc["COPIERS_POST_AND_NOUN"] - JOINT_V576) <= JBAND, "pred_c_copiers_post_noun_replay": abs(mc["COPIERS_POST"] - COPIERS_V576) <= CBAND,
    ...
    OUT.write_text(json.dumps({"schema": "gender_composition_pile_result_v577", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
