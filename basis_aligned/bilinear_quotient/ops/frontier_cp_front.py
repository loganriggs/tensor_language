"""THE CP-FRONT CONFIG (rung 112): the first multi-site config built from the winning grammar.

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows; LOWER IS BETTER.
S2203/S2204: CP-K dominates the tables at every tested front MLP by K=2304. Config: m0-m3 replaced by their
own top-K bilinear units (weights-only) + the a0 table (exact, S2198) + the a1v table (+0.052 alone, 11/62);
EVERYTHING else real. Per-site K: read from frontier_m13_cp_results.json at RUN time if it exists (registered
rule: smallest K with agg <= 0.10, else 2304); fallback defaults {m0:2304, m1:2304, m2:1152, m3:2304} from
S2203/S2204. Anchor: the six-table front cost +1.7474, valid 0/62 (S2198 prefix_m3E).

REGISTERED PREDICTIONS (arm named cp_front):
  (a) AGGREGATE COLLAPSE: agg(cp_front) <= 0.5 (>= 3.5x better than the table front's +1.7474).
  (b) VALIDITY APPEARS IN COMBINATION: valid(cp_front) >= 5 of 62 - the first multi-site config ever above 0.
  (c) SANITY: agg(cp_front) in [0.05, 1.0].
NULL: superadditive compounding (S2200's 22% was at table grain) destroys the single-site gains; validity 0.
PRICE: at defaults, front stores (2304+2304+1152+2304) x 3456 = 27,869,184 values + a1v table vs four tables
231,584,256 - 8.3x cheaper. Tripwire: INSTRUMENT FAIL if cev length mismatch. Self-reviewed."""
import json, sys, time, os
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
if os.environ.get('BQLIB_DRYRUN')=='1':
    _bq='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
    _need=['frontier_m0_cp_results.json','frontier_m2_cp_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: CP-front config')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_cp_front_results.json'
...
if __name__=='__main__':
    ...
    KS={0:2304,1:2304,2:1152,3:2304}
    src='defaults'
    p13=PT+'frontier_m13_cp_results.json'
    if os.path.exists(p13):
        arms13=json.load(open(p13))['arms']
        for li in (1,3):
            for K in (288,576,1152,2304):
                if arms13[f'm{li}_K{K}']['agg']<=0.10:
                    KS[li]=K; break
        src='m13 results (smallest K with agg <= 0.10, else 2304)'
    ANCH=json.load(open(PT+'frontier_front_prefix_results.json'))['arms']['prefix_m3E']['agg']
    SEL['mode']='norm'; SEL['K']=4608; SEL['K69']=4608; SEL['K69MAP']={}
    SEL['skipset']=tuple(range(10,18)); SEL['motif_off']=tuple(range(2,10))
    SEL['cp_front']=True; SEL['ext_rows']=CROWS; SEL['cpks']=KS
    print(f'ARM: CP-front config, K={KS} (source: {src}); a0+a1v tables; all else real',flush=True)
    main()
    cev=SEL['front_result']['cp_front']
    ...
    vals=sum(KS[li]*3456 for li in KS)
    pa=agg<=0.5
    pb=nv>=5
    pc=0.05<=agg<=1.0
    res={'agg':round(agg,4),'n_valid':nv,'Ks':{str(k):v for k,v in KS.items()},'k_source':src,
         'front_values':vals,'table_front_values':4*50257*1152,'anchor_table_front':ANCH,
         'circuits':rows,
         'convention':'per-position dCE = CE(CP-front config) - CE(real model) on census rows; lower is better',
         'pred_a_aggregate_collapse':bool(pa),'pred_b_validity_appears':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'CP-front agg {agg:+.4f}; valid {nv}/{len(rows)} (table front: {ANCH:+.4f}, 0/62); {vals/1e6:.1f}M values vs 231.6M')
    print(f"(a) agg {agg:+.4f} <= 0.5: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 5: {'HELD' if pb else 'FAILED'}")
    print(f"(c) agg in [0.05, 1.0]: {'HELD' if pc else 'FAILED'}")
