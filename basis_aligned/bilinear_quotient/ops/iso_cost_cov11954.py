# WHERE DOES THE ISO-COST PLATEAU BEGIN?  -- the middle point of §1862's two-point bracket.
#
# §1862 measured three points on the 230.087M iso-cost line and found every one beats the deployed build
# (full rank over 5,419 types, 6.01167 / 5.98477 / 6.00165), but that the advantage SATURATES:
# 9,054 -> 14,405 buys +0.014 to +0.026 nats, and 14,405 -> 16,110 buys +/-0.003, i.e. nothing.
#
# "The plateau starts somewhere between 9,054 and 14,405" is a TWO-POINT BRACKET, and LESSON 47 says
# points chosen to bracket a feature do not locate it -- the mistake §1829 was built to correct and one
# I have now made twice more this session. This is the middle point: rows [0:288] give 11954 covered
# types, affordable rank 476 at 230.087M budget (229.976M).
#
# Docstring and predicate block written in ONE edit (LESSONS 63); costs COMPUTED from §1754's model
# (§1858); a single finite rank in TRANKS, so nothing indexes the full-rank key (LESSONS 64).
#
# ROLES. All three. DISCOVERY ONLY. Rung 3: the open question §1862's plateau raises.
#
...
#   pred_a IT BEATS THE DEPLOYED BUILD, like every other point on the line: all-position CE below the
#          PUBLISHED 6.01167 / 5.98477 / 6.00165 on all three roles at no more than 230.087M. If FALSE
#          the line is not uniformly better than the deployed build and §1862's headline needs a caveat
#          at this coverage.
#   pred_b THE PLATEAU HAS ALREADY BEEN REACHED BY 11954: this point is within 0.01 nats of §1862's
#          PUBLISHED 14,405-type point (5.95604 / 5.90496 / 5.93050) on all three roles. If TRUE the
#          plateau begins at or below 11954 types and the operative threshold is ~12k, not ~14k. If
#          FALSE it lies between 11954 and 14,405, the threshold is higher than this point, and the
#          rule should quote 14k.
#   pred_c AND IT IS CLEARLY BETTER THAN 9,054: it beats §1862's PUBLISHED 9,054-type point (5.97012 /
#          5.93014 / 5.95397) by at least 0.005 nats on all three roles. If FALSE the gain between 9,054
#          and 14,405 is concentrated ABOVE this point rather than below it, which would place the
#          plateau's onset nearer 14,405 and make the two-point bracket's midpoint uninformative.
#   pred_d CONTROLS: the cost computed from §1754's model is at most 230.087M; coverage is exactly
#          11954 from fit rows [0:288]; the ceiling is finite and above live on every role; the
#          covered CE lies strictly above its own ceiling, as it must at finite rank.
...
OUT = PT + 'ops/iso_cost_cov11954_results.json'
...
TRANKS = (476,)          # None = full rank; §1786/§1813 use this ladder
...
LADDER = ['476']
# §1862's PUBLISHED iso-cost points, same 230.087M budget
S1862_COV14405 = {'skip7000': 5.95604, 'skip11000': 5.90496, 'skip1200': 5.93050}
S1862_COV9054 = {'skip7000': 5.97012, 'skip11000': 5.93014, 'skip1200': 5.95397}
...
FITLO, FITHI = 0, 288      # the draw giving 11954 covered types   # 5x the fit rows -> a larger covered set
H = m.transformer.h
NCOV = 5419
...
def main():
    ...
    assert NFULL == 11954, f'coverage {NFULL} != expected 11954'
    ...
    print(f'ISO COST COV | 11954 types, rank 476, plateau onset | '
    ...
    vs_14405 = {e: allpos[e] - S1862_COV14405[e] for e in roles2}
    vs_9054 = {e: S1862_COV9054[e] - allpos[e] for e in roles2}
    pa = cost <= DEPLOYED_COST_M and all(v > 0 for v in vs_dep.values())
    pb = all(abs(v) <= 0.01 for v in vs_14405.values())
    pc = all(v >= 0.005 for v in vs_9054.values())
    pd = (cost <= DEPLOYED_COST_M and NFULL == 11954
    ...
    print(f'  and the PLATEAU is already reached here (<0.01 from 14,405) -> {pb}  ' + '  '.join(
        f'{e} {vs_14405[e]:+.5f}' for e in roles2), flush=True)
    print(f'  and it clearly beats 9,054 (>=0.005) -> {pc}  ' + '  '.join(
        f'{e} {vs_9054[e]:+.5f}' for e in roles2), flush=True)
    print(f'  cost <= budget, coverage {NFULL}, ceiling above live -> control {pd}', flush=True)

    json.dump({'run': 'iso_cost_cov11954', 'n_covered': NFULL, 'rank': R,
               'cost_M': cost, 'allpos': allpos, 'above_ceiling': above,
               'vs_deployed': vs_dep, 'vs_14405': vs_14405, 'vs_9054': vs_9054,
               'ceiling': ceilv, 'live': livev,
               'predictions': {'pred_a_beats_deployed': bool(pa),
                               'pred_b_plateau_reached_here': bool(pb),
                               'pred_c_clearly_beats_9054': bool(pc),
