#!/usr/bin/env python3
# BQGATE:432bodyforwards;72freshlongerprefixes;6arms;120seconds.
...
STEM='DIRECTIONAL_INTERACTION_LOGIT_V1';FOLDER=P/'extracted_circuits/regional_even_key_producers_8_2_9_8_v1'
@torch.no_grad()
def main():
 binding=json.loads((P/(STEM+'_BINDING.json')).read_text())['files'];assert all(digest(k)==v for k,v in binding.items());rows=json.loads((P/(STEM+'_ROWS.json')).read_text())['rows'];validate(rows);assert len(rows)==72
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('432bodyforwards;72freshlongerprefixes;6arms;120seconds');return
 ...
 def head8(module,args,output):
  s=runtime.scalar(args[0],context['tokens'],p,0)
 ...
 try:
  for i,row in enumerate(rows):
   ids=torch.tensor([row['ids']],device='cuda');context.clear();context['tokens']=ids
   for arm in range(6):
 ...
 result={'pred_a':A,'pred_b':B,'pred_c':C,'records':records,'max_full_scalar_relative_error':max(field_checks),'body_forwards':count,'seconds':time.perf_counter()-tic,'arms':['native','remove8','remove8_actual9','remove8_predicted64_9','remove8_frozen9','remove8_predicted_full9'],'scope':'Frozen rank64 selected on earlier short cue panel; fresh longer contexts, same cue families and endpoints. Pristine z8, bias-free MLP8 output, raw9 and head8 amplitude are inputs. Conditional surrogate-removal simulator with native prefix/background/suffix, not autonomous circuit extraction or corpus OOD.'}
