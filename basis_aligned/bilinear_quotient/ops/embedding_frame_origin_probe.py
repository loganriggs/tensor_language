#!/usr/bin/env python
"""embedding_frame_origin_probe -- attn_0 reads the token-embedding frame (its input is x0 itself). Is that frame derivable from wte
alone (unweighted vocabulary covariance), does each block's blend x <- l0*x + l1*x0 pull the read frame back toward it, and how
deep can sites read through the embedding frame at k=768 before it costs? CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_weights_only_frame_suffices pred_c_blend_pulls_toward_embedding
#                     pred_d_blocks_012_read_the_embedding_frame pred_e_embedding_frame_does_not_reach_block_10

SIGN CONVENTION (§2135): every CE number is CE ADDED ABOVE THE REAL MODEL on held-out docs 0-63 -- LOWER IS BETTER. Descriptive;
nothing installs into the §312 frontier; U_w is a weight-covariance frame scored by CE only (§2118 stays closed).
Preregistration: polynomial_causal/EMBEDDING_FRAME_ORIGIN_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/embedding_frame_origin_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("embedding_frame_origin_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "EMBEDDING_FRAME_ORIGIN_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "frame_principal_angle_spectrum_probe_results.json"   # §2757
OUT = ROOT / "embedding_frame_origin_probe_results.json"
HASHES = {PREREG: "fa897ae5c032410d4afc8208aa5794d43e6147334e50c77f0b9fd3c9d07db9aa", PRIOR: "bc96dec9306508d8718d984f520a7e54d09612361a91ba70a263582a41f69729",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "embedding_frame_origin_probe"
...
SITES = [("mlp", l) for l in range(NL)]; ASITES = [("attn", l) for l in range(NL)]
ESITES = [(kd, l) for l in EARLY for kd in ("attn", "mlp")]; LSITES = [("mlp", l) for l in LATE] + [("attn", l) for l in LATE]
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_cap_min": 0.93, "b_gap_max": 0.010, "c_min": 6, "d_max": 0.030, "e_min": 0.060}
NULLS = {"b_cap_max": 0.80, "b_gap_min": 0.040, "c_max": 3, "d_min": 0.080, "e_max": 0.020}
...
def main():
    ...
    def capture(C, U_):
        Uk = U_[:, :K].to(C.dtype); return float(torch.trace(Uk.T @ C @ Uk) / torch.trace(C))
    U_emb = heads[("attn", 0)]["U"]
    Wn = F.rms_norm(m.transformer.wte.weight.detach().float().to(DEV), (D,)).double(); Wn = Wn - Wn.mean(0, keepdim=True)
    Cw_ = (Wn.T @ Wn) / Wn.shape[0]; _, U_w = torch.linalg.eigh(Cw_); U_w = U_w.flip(1).float(); del Wn
    cap_w = capture(heads[("attn", 0)]["Cx"], U_w); cap_self = capture(heads[("attn", 0)]["Cx"], U_emb)
    d_emb = {f"{s_[0]}{s_[1]}": n30(U_emb, heads[s_]["U"]) for s_ in ESITES}
    pulls = [int(d_emb[f"attn{l + 1}"] < d_emb[f"mlp{l}"]) for l in range(NB)]; n_pull = int(sum(pulls))
    log(stage="geom", cap_w=round(cap_w, 4), cap_self=round(cap_self, 4), n_pull=n_pull, emb_vs_w_n30=n30(U_emb, U_w), d_emb=d_emb)
    B012 = blocks(range(0, 3))
    arms = {"EARLY22_OWN_768": run({s_: head(s_, K) for s_ in ESITES})}
    p_ = {s_: head(s_, K) for s_ in ESITES}; p_.update({s_: head(s_, K, U_emb) for s_ in B012}); arms["EMB_BLOCKS012_768"] = run(p_)
    p_ = {s_: head(s_, K) for s_ in ESITES}; p_.update({s_: head(s_, K, U_w) for s_ in B012}); arms["EMBW_BLOCKS012_768"] = run(p_)
    arms["EARLY22_EMB_768"] = run({s_: head(s_, K, U_emb) for s_ in ESITES})
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    gaps = {"emb012_minus_own": arms["EMB_BLOCKS012_768"] - arms["EARLY22_OWN_768"], "embw012_minus_emb012": arms["EMBW_BLOCKS012_768"] - arms["EMB_BLOCKS012_768"],
            "emb22_minus_own": arms["EARLY22_EMB_768"] - arms["EARLY22_OWN_768"], "capture_attn0_by_Uw": cap_w, "capture_attn0_by_own": cap_self, "n_pull_of_8": n_pull}
    ...
    preds = {
        ...
        'pred_b_weights_only_frame_suffices': bool(cap_w >= BARS["b_cap_min"] and gaps["embw012_minus_emb012"] <= BARS["b_gap_max"]),
        'pred_c_blend_pulls_toward_embedding': bool(n_pull >= BARS["c_min"]),
        'pred_d_blocks_012_read_the_embedding_frame': bool(gaps["emb012_minus_own"] <= BARS["d_max"]),
        'pred_e_embedding_frame_does_not_reach_block_10': bool(gaps["emb22_minus_own"] >= BARS["e_min"]),
    }
    nulls = {"b_null_cap_le_.8_or_gap_ge_.04": bool(cap_w <= NULLS["b_cap_max"] or gaps["embw012_minus_emb012"] >= NULLS["b_gap_min"]),
             "c_null_le_3": bool(n_pull <= NULLS["c_max"]), "d_null_ge_.08": bool(gaps["emb012_minus_own"] >= NULLS["d_min"]),
             "e_null_le_.02": bool(gaps["emb22_minus_own"] <= NULLS["e_max"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "ce_added": arms, "gaps": gaps, "d_emb_n_gt_30": d_emb, "pulls": pulls, "emb_vs_weights_frame_n_gt_30": n30(U_emb, U_w),
