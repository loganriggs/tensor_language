...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: deps present; drop-attn3 + scalar-gain merge arms (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'merge_scalar_results.json'
...
def main():
    ...
    if SEL.get('alpha2') is not None:
        _al=SEL['alpha2']
        _hza.append(m.transformer.h[2].attn.register_forward_hook(
...
if __name__=='__main__':
    ...
    SEL['clsdmg']=False; SEL['zero_m16']=False; SEL['motif_off']=(2,3); SEL['alpha2']=None
    SEL['zeroattn']=()
    print('ARM 1/3: base (motifs 2/3 real)',flush=True)
    r_base=main()
    SEL['zeroattn']=(3,)
    print('ARM 2/3: drop attn3 (S2180 reproduction of z3)',flush=True)
    r_z3=main()
    d3=r_z3['L2_F']-r_base['L2_F']
    # grid the scalar on the window-C eval of the drop3 build: cheap proxy - use each candidate's L2_C
    best=(None,1e9)
    for al in (1.0,1.1,1.2,1.3,1.45,1.6,1.8):
        SEL['alpha2']=al
        rc=main()
        print(f'  alpha {al}: L2_C {rc["L2_C"]:+.4f} | L2_F {rc["L2_F"]:+.4f}',flush=True)
        if rc['L2_C']<best[1]: best=(al,rc['L2_C'],rc)
    al_star=best[0]; r_al=best[2]
    dal=r_al['L2_F']-r_base['L2_F']
    pa=dal<=0.7*d3
    pb=1.05<=al_star<=1.8
    pc=abs(r_base['L2_F']-2.4410)<=0.01
    res={'base':{'L2_F':r_base['L2_F']},'drop3':{'L2_F':r_z3['L2_F'],'d':round(d3,4)},
         'alpha_star':al_star,'drop3_alpha':{'L2_F':r_al['L2_F'],'d':round(dal,4)},
         'convention':'damage on the motifs-2/3-real base; lower L2 better; alpha selected on window C',
         'pred_a_scalar_recovers':bool(pa),'pred_b_real_upscaling':bool(pb),'pred_c_reproduces_r86_base':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'drop3 d={d3:+.4f}; alpha*={al_star} -> d={dal:+.4f}')
    print(f"(a) {dal:+.4f} <= 0.7 x {d3:+.4f}: {'HELD' if pa else 'FAILED'}")
    print(f"(b) alpha* {al_star} in [1.05,1.8]: {'HELD' if pb else 'FAILED'}")
