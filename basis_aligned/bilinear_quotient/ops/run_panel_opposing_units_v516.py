#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_four_units_oppose_the_panel_pronoun_margin pred_c_random_units_do_not pred_d_four_units_are_a_share_of_mlp17 pred_e_hub_units_at_the_noun_replay
"""Do the final detector units serve the pronoun slot too (v463)? v462: MLP 17's 701 / 2059 act at the answer position for a distant verb (0.069 of the
agreement margin), nothing at the noun. Here at the PRONOUN slot (122 natural pairs, they - he margin): 701 / 2059 swapped at the answer position; two random
MLP-17 units (seed 0) at the answer position; the hub units 829 / 953 / 1030 at the answer position and at the noun (replay of 0.124, v423).
PREDICTIONS (scored as written; failures preserved; priors from v423 / v462)
    pred_a_baseline_replays               the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_late_units_serve_the_pronoun_slot  701 / 2059 at the answer position close >= 0.03 of the pronoun margin gap. Prior: unsure
    pred_c_random_units_do_not            two random MLP-17 units at the answer position close <= 0.01
    pred_d_hub_units_at_the_answer_small  829 / 953 / 1030 at the answer position close <= 0.05
    pred_e_hub_units_at_the_noun_replay   829 / 953 / 1030 at the noun close within 0.02 of 0.124 (v423)
PRICE (registered maximum): 4 batches x 5 passes = 20 forwards; 0 backwards; 0 fits. Bar <= 21.
"""
...
OUT = ROOT / "circuits/followups/panel_opposing_units_v516_result.json"
CANDIDATE_ID = "chain.panel_opposing_units_v516"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, DET_MAX, RANDOM_MAX, M17_V512, SHARE_LO, SHARE_HI, HUB_V423, BAND = 1e-3, -0.03, 0.01, 0.115, 0.20, 0.90, 0.091, 0.02
import random
import random
UNIT_SETS = {"DET_A": (17, (3547, 1747, 3093, 4448), "answer"), "RANDOM_A": (17, tuple(random.Random(0).sample([j for j in range(4608) if j not in (3547, 1747, 3093, 4448)], 4)), "answer"), "HUB_N": (8, (829, 953, 1030), "noun")}
...
FORWARDS_MAX = 13
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_four_units_oppose_the_panel_pronoun_margin": "<= -0.03", "pred_c_random_units_do_not": "|.| <= 0.01", "pred_d_four_units_are_a_share_of_mlp17": "0.20-0.90 x 0.115", "pred_e_hub_units_at_the_noun_replay": "0.091 +- 0.02"}
...
def main() -> None:
    ...
    for r_ in recs:
        ...
        vs = r_["second_offset"]
        if vs - c >= 2: verb_items.append((plural[:vs], singular[:vs], c))   # distant verbs: the answer position is not the noun   # truncated so the answer position IS the verb slot (the verb is predicted there)
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "det_max": DET_MAX, "random_max": RANDOM_MAX, "m17_v512": M17_V512, "share": [SHARE_LO, SHARE_HI], "hub_v423": HUB_V423, "band": BAND}}
    ...
    MODES = {m: ({}, (0,)) for m in UNIT_SETS}; forwards = 0
    def logits_margin(x):
        ...
        with torch.no_grad():
            ...
            for l, block in enumerate(blocks):
                ...
                if mode in UNIT_SETS and l == UNIT_SETS[mode][0]:
                    ...
                    units = torch.tensor(UNIT_SETS[mode][1], device="cuda"); at = pp if UNIT_SETS[mode][2] == "noun" else fin; hdn[idx[:, None], at[:, None], units[None, :]] = hdn[swap[:, None], at[:, None], units[None, :]].clone()
    ...
    def study(items, batch):
        ...
        return out, nat_m[:, 0]
    verb, nat_t = study(panel_items, 16); text = verb; panel = {"frame": "v76 panel pairs", "rows": len(panel_items)}
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_four_units_oppose_the_panel_pronoun_margin": v["DET_A"] <= DET_MAX, "pred_c_random_units_do_not": abs(v["RANDOM_A"]) <= RANDOM_MAX,
                   "pred_d_four_units_are_a_share_of_mlp17": SHARE_LO * M17_V512 <= abs(v["DET_A"]) <= SHARE_HI * M17_V512, "pred_e_hub_units_at_the_noun_replay": abs(v["HUB_N"] - HUB_V423) <= BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "panel_opposing_units_result_v516", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
