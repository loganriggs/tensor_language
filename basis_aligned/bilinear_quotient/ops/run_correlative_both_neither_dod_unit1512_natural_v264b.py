...
"""Correlative both/neither DoD (v264b; v264 ran to completion but its receipt was withheld by the price guard -- 80 forwards against a registered 64: the Pile file has 64 rows, not 48, so the panel is 112 rows / 4 batches): MLP-8 unit 1512 OUT OF THE PANEL. v262 / v263: 1512 is a neither-context detector at the FINAL token (48 / 48 on the panel), fed by
...
PRICE (registered maximum): 4 batches x (native + final + all-positions + 16 nulls) + producer replay 4 = 80 forwards; 0 backwards; 0 fits. Bar <= 84.
"""
...
OUT = ROOT / "circuits/followups/correlative_both_neither_dod_unit1512_natural_v264b_result.json"
...
CANDIDATE_ID = "correlative_both_neither.and_vs_nor.dod_unit1512_natural_v264b"
...
def main() -> None:
    ...
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_unit1512_natural_result_v264b", "candidate_id": CANDIDATE_ID, "plan": plan, "instrument_max_abs_error": instrument, "arms": arms, "null_damage_max": null_max, "null_moves": null_moves, "nulls": nulls, "gates": gates,
