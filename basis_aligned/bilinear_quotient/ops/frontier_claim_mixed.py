"""REGISTERED CLAIM: THE MIXED-SPECTRUM POINT (rung 254) - S2352 becomes official.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2352 measured top-96+last-8 at every
replaced head: census +0.0573 / fresh +0.0584 / valid 11 at 52.9M pattern values - supersedes the
economical shape on every axis. Identical rebuild; claim bars registered.
REGISTERED PREDICTIONS (anchors: the S2352 receipt):
  (a) REPRODUCTION: |census - 0.0573| <= 0.015.
  (b) CERTIFICATES: valid >= 11.
  (c) REPRODUCTION: |L2_F - 0.0584| <= 0.015.
NULL: noise. PRICE: ~180M values (52.9M mixed-spectrum patterns). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: registered claim mixed-spectrum point')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_claim_mixed_results.json'
...
if __name__=='__main__':
    ...
    pa=abs(agg-0.0573)<=0.015
    pb=nv>=11
    pc=abs(L2F-0.0584)<=0.015
    ...
    print(f"(a) |census {agg:+.4f} - 0.0573| <= 0.015: {'HELD' if pa else 'FAILED'}")
