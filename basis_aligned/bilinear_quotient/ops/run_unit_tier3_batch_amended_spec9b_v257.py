#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the twenty-eighth five spec-authored behaviours, plus one instrument.
"""v257: the AMENDED tier-3 battery (v196/v197 protocol, unchanged) over the TWENTY-EIGHTH batch of spec-authored behaviours (FIVE) -- plus one instrument.

v231 7/8, v233 3/8, v235 4/8, v237 2/8, v239 3/8, v241 4/6, v243 5/7, v245 2/6, v247 4/6, v249 3/8, v251 5/8, v253 2/8, v255 6/8
(+instrument each) at ~30-45 GPU-s per behaviour.
SIXTH batch under the v245 lesson (pre-screen floor 1.0 on A1, mean base/donor on the donor axis; one repair or drop). v255's new class
verb_particle (woke -> up / calmed -> down) passed all four rows (n=10, held 0.86, dim 1.331, cdas 1.326, C 0.0027, cext 0.991) as a
single pre-screened cell, so this batch AUTHORS ITS SIBLINGS -- the class can only be tested for within-family separability once two or more
of its cells pass (a single family-less pass stands as a singleton and is never separability-tested). Of four sibling pairs authored, only
two survived the CPU floor (see the capability block): out/down and on/away. The other three cells are new preposition pairs under grammatical
cues (word-boundary, quote-agnostic grep: out/down, on/away, from/about, at/to all 0 hits).
...
verb_particle_out_down (`Near the lantern the pilot found/settled, of course,` -> out/down; verb_particle family, 1 counted after v255);
verb_particle_on_away (`... droned/slipped, of course,` -> on/away; verb_particle family);
verb_preposition_from_about (`... suffered/boasted, of course,` -> from/about; verb_preposition family, 6 counted after v244);
verb_preposition_at_to (`... aimed/appealed, of course,` -> at/to; verb_preposition family);
adjective_preposition_from_about (`... was absent/adamant, of course,` -> from/about; adjective_preposition family, 6 counted after v244).
CPU capability margins BEFORE enqueue (mean base / mean donor on the donor axis; A1 | A2; dropped rows; floor 1.0 on A1 per the v245 lesson):
  verb_particle_out_down A1:-2.27/+2.27 drop0 A2:-2.60/+2.78 drop0 C:-1.43/+0.93 drop2
  verb_preposition_from_about A1:-6.33/+6.24 drop0 A2:-5.32/+5.09 drop0 C:-1.43/+0.93 drop2
  verb_preposition_at_to A1:-3.90/+3.53 drop0 A2:-3.27/+2.98 drop0 C:-1.43/+0.93 drop2
  verb_particle_on_away A1:-1.52/+1.42 drop0 A2:-1.34/+1.31 drop0 C:-1.43/+0.93 drop2
  adjective_preposition_from_about A1:-4.08/+4.16 drop0 A2:-3.07/+3.20 drop0 C:-1.43/+0.93 drop2
  Five cells at >= 1.34 with zero dropped rows. THREE cells were DROPPED as two-sided capability failures (A1 and A2 both below 1.0) and their
  modules deleted before any battery: verb_particle_off_out (dozed/dropped: A1 all 32 rows dropped, A2 0.14/0.15 drop23), verb_particle_up_off
  (showed/set: 0.85/0.96 drop31 | 0.28/0.04 drop20), verb_particle_in_out (gave/passed: 0.27/0.74 drop1 | 0.31/0.58) -- bare intransitive
  `gave/dozed/showed, of course,` does not oblige a particle without an object, so those collocations are not in the model. verb_particle_on_away
  was ONE-SIDED (A1 0.75/0.75, A2 1.14/1.05) and took its ONE licensed repair (carried/ran -> droned/slipped: 1.52/1.42, 0 dropped); disclosed.
  adjective_preposition_at_with is NOT re-attempted: it was a two-sided drop at v253 (good/patient) and a two-sided drop is not iterated.
  Batch size FIVE; bars are scaled to five cells (3/4/4/3/3).
Protocol identical to v197/v201/.../v255 (greedy pool 40, target 0.88, min_gain 0.005, max 30; row 2 = MIN over the two held directions
...
REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 3 of the 5 new behaviours pass all four rows (pre-screened batches 25/43 = 0.58). prior 55%
  pred_b_row2           -> MIN held-out per-direction exact-set recovery >= 0.80 on at least 4 of 5 (v255 6/8, v251 8/8).  prior 60%
  pred_c_row3           -> dim A1 CE damage LB975 > 0 and >= 0.10 on at least 4 of 5 (v255 8/8).                           prior 75%
  pred_d_row4           -> constrained rank-1 own-C CE UB975 <= 0.01 on at least 3 of 5 (v255 8/8, v251 7/8).              prior 70%
  pred_e_row5_amended   -> A2-own / A2 ceiling within [0.5, 1.4] with LB975 > 0 on at least 3 of 5 (v255 8/8).             prior 70%
...
Accounting: four-row passes here join the one-protocol standing. The three preposition cells have families with counted members and count only
after a separability rung under the full family control set (v248 covers the v255 passes; a later stem covers these). The two verb_particle
cells: if BOTH pass, the class has >= 2 members and goes to a separability rung with up_down before any of them is counted beyond the standing
singleton; if exactly one passes, the v242 rule applies to it against up_down at the next separability rung. An error receipt per behaviour
counts as a miss on every row. ~40 s GPU each, ~4 min.
A1/A2 preps use g.prepare(valid_only=True); `n_dropped` is in every receipt.
Smoke: V257_SMOKE=<out.json> (CPU, V257_SMOKE_ROWS=4 per split, V257_SMOKE_NAMES=verb_particle_out_down; pool 3/max 3, steps 5).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec9b_v257_result.json"
...
NAMES = {"verb_particle_out_down": "verb_particle_out_down", "verb_particle_on_away": "verb_particle_on_away",
         "verb_preposition_from_about": "verb_preposition_from_about", "verb_preposition_at_to": "verb_preposition_at_to",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec9b_v257", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V257_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V257_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V257_SMOKE_NAMES", "verb_particle_out_down").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec9b_v257", "candidate_id": "corpus.unit_tier3_batch_amended_spec9b_v257", "bars": BARS,
