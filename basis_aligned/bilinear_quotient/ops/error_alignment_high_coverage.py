# §1895 AT HIGH COVERAGE -- second-class confirmation of "the program makes the model's mistakes".
#
# §1895 found that on the ~60% of covered scored positions where BOTH the program and the live model are
# WRONG, the program's top-1 matches the model's specific wrong token at 22.76 / 20.75 / 24.06% against a
# permutation null of 3.73 / 3.33 / 3.70% -- 6.10 / 6.24 / 6.50x. That excluded the alternative §1885
# could not: that the aggregate 5-8x tracking is carried by easy positions any two competent predictors
# share. It is the strongest positive statement in this arc about what the compiled program reproduces.
#
# It was measured at the DEPLOYED 5,419 coverage, on one build. §1886 and §1888 both showed this family
# of results is stable across builds and coverages, but neither tested the both-wrong cell, which is the
# one that carries §1895's conclusion. At 16,110 types the covered arm grows and its composition changes
# -- more token types are covered, so the covered positions include rarer tokens the model handles worse.
# If the both-wrong enrichment is a property of the compiled program it should survive; if it is a
# property of which positions happened to be covered at 5,419, it should move.
#
# House pattern: a just-certified result gets a second-class confirmation before anything is built on it
# (§1595, §1598, §1603, §1886, §1888).
#
...
# ANCHORS: §1895's are at 5,419 and are what is being generalised. §1858's CE and §1888's enrichment are
# both coverage-specific and are NOT used here -- the trap that cost §1882 a launch and that §1889 walked
# into a second time.
#   pred_a THE MISTAKE-SHARING SURVIVES: both-wrong agreement is at least 3.0x the permutation null on
#          all three roles, the same bar §1895 passed. If FALSE, §1895's conclusion is specific to the
#          deployed coverage and the whole §1884-§1895 line needs "at 5,419 types" attached -- which
#          would matter, because I have just written it up as a statement about the program.
#   pred_b AND IT IS THE SAME NUMBER: within 1.5x of §1895's PUBLISHED 6.10 / 6.24 / 6.50x. A deliberately
#          loose bar, because the covered POPULATION differs at 16,110 and I am testing whether the effect
#          is of the same order, not whether it is identical. If FALSE the effect is real at both
#          coverages but sensitive to which positions are covered, which is itself worth knowing.
#   pred_c AND PROGRAM-ONLY-RIGHT STAYS REAL: at least 10% of the program's correct predictions, as at
#          §1895 (16.9 / 15.8 / 18.7%) and §1884 (19%). Three populations, one claim.
#   pred_d CONTROLS: coverage is exactly 16,110; the four cells partition every covered scored position;
#          and the both-wrong cell is the largest, as the marginals require.
...
OUT = PT + 'ops/error_alignment_high_coverage_results.json'
...
S1858_CE = {'skip7000': 6.01167, 'skip11000': 5.98477, 'skip1200': 6.00165}   # §1858, 5,419 -- NOT a bar here
S1895_BW = {'skip7000': 6.10, 'skip11000': 6.24, 'skip1200': 6.50}   # §1895 PUBLISHED, 5,419 types
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'ERROR ALIGNMENT AT HIGH COVERAGE | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    pb = all(abs((bw[e]['enrichment'] or 0.0) - S1895_BW[e]) <= 1.5 for e in roles)
    pc = all(ponly[e] >= 0.10 for e in roles)
    pd = (ncov == NCOV
          and all(sum(cl2(e, k4) for k4 in CELLS) == res['sites'][A1][e]['cells']['both_right']['n']
                  + res['sites'][A1][e]['cells']['prog_only']['n']
                  + res['sites'][A1][e]['cells']['model_only']['n']
                  + res['sites'][A1][e]['cells']['both_wrong']['n'] for e in roles)
          and all(biggest[e] == 'both_wrong' for e in roles))
    ...
    print(f'  and it MATCHES §1895 within 1.5x -> {pb}  ' + '  '.join(
        f'{e} {bw[e]["enrichment"]:.2f}x vs §1895 {S1895_BW[e]:.2f}x' for e in roles), flush=True)
    print(f'  and PROGRAM-ONLY-RIGHT is >=10% of its correct predictions -> {pc}  ' + '  '.join(
        f'{e} {cl2(e, "prog_only")}/{progright[e]} = {ponly[e]:.1%}' for e in roles), flush=True)
    print(f'  coverage {ncov}, cells partition, both-wrong largest -> control {pd}  ' + '  '.join(
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_mistake_sharing_survives_program_concentrated_on_frequent_targets': bool(pa),
                          'pred_b_matches_S1895_more_concentrated_than_live': bool(pb),
                          'pred_c_prog_only_right_stays_real_keeps_60pc_on_the_top_bucket': bool(pc),
