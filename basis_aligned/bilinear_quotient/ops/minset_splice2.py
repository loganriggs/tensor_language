"""MINSET SPLICE v2 (rung 174): the rung-173 control fixed - bf16 captures, sets logged, NaN tripwire.

CONVENTION (S2135): recovery = (plain - spliced)/plain member damage; frontier build; census rows. S2270:
three of five random-5 arms NaN'd on f16-capture overflow; the specificity pred was partially void. v2
stores captures in bfloat16 (f32 range), saves the rand5 sets in the receipt, and hard-fails on any NaN.

REGISTERED PREDICTIONS:
  (a) REPRO OF 173's VALID HALF: median minset recovery within 0.05 of 0.404.
  (b) SPECIFICITY (the rescored pred): minset recovery >= 1.3 x rand5 recovery for >= 3 of 5 circuits.
  (c) PLAIN REPRO: plain member damages vs the rung-132 receipt, median ratio in [0.85, 1.15].
NULL (for b): indiscriminate recovery - the buy-back is generic stream fidelity, not circuit-specific.
PRICE: none (oracle diagnostic). Tripwire: INSTRUMENT FAIL on any NaN recovery or inert arm. Self-reviewed."""

import json, sys, time, os
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
if os.environ.get('BQLIB_DRYRUN')=='1':
    _bq='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
    _need=['minset_splice_results.json','frontier_rows_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: minset splice v2 (bf16)')
    raise SystemExit(0)
import torch
...
OUT=PT+'minset_splice2_results.json'
...
def main():
    ...
    if SEL.get('minset_splice'):
        _ROWS=SEL.get('ext_rows',FR)
        cur['clsmap']=classify2(_ROWS).to(DEV)
        _UN=sorted({c for cs in SEL['ms_sets'].values() for c in cs})
        _CAPR={c:[] for c in _UN}
        hs=[]
        for _c in _UN:
            _li=int(_c[1:]); _mod=(m.transformer.h[_li].attn if _c[0]=='a' else m.transformer.h[_li].mlp)
            def _mkr(c=_c):
                def h(mo,i_,o_):
                    y=o_[0] if isinstance(o_,tuple) else o_
                    _CAPR[c].append(y.detach().to(torch.bfloat16).cpu())
                return h
            hs.append(_mod.register_forward_hook(_mkr()))
        for _i in range(0,_ROWS.shape[0],4):
            _bb=_ROWS[_i:_i+4,:257].to(DEV)
            cur['idx']=_bb[:,:-1].contiguous(); cur['mode']='oracle'
            m(cur['idx'],_bb[:,1:].contiguous())
        for h in hs: h.remove()
        print(f'  real-pass captures at {_UN}',flush=True)
        def _evalVS(_comps):
            hs=install(order2)+motif_hooks(ML)
            for _c in _comps:
                _li=int(_c[1:]); _mod=(m.transformer.h[_li].attn if _c[0]=='a' else m.transformer.h[_li].mlp)
                def _sp(c=_c):
                    def h(mo,i_,o_):
                        _r=_CAPR[c][cur['msb']].to(DEV).float()
                        if isinstance(o_,tuple): return (_r.to(o_[0].dtype),o_[1])
                        return _r.to(o_.dtype)
                    return h
                hs.append(_mod.register_forward_hook(_sp()))
            ces=[]
            for _i in range(0,_ROWS.shape[0],4):
                cur['msb']=_i//4
                _bb=_ROWS[_i:_i+4,:257].to(DEV)
                cur['idx']=_bb[:,:-1].contiguous(); _tg=_bb[:,1:].reshape(-1)
                cur['mode']='oracle'; cur['lab']=cur['clsmap'][_i:_i+4].reshape(-1)
                x=F.rms_norm(m.transformer.wte(cur['idx']),(D,)); x0=x; v1=None
                for blk in m.transformer.h: x,v1=blk(x,v1,x0)
                lg=(30*torch.tanh(m.lm_head(F.rms_norm(x,(D,)))/30)).float()
                ces.append(F.cross_entropy(lg.view(-1,lg.size(-1)),_tg,reduction='none'))
            for h in hs: h.remove()
            return torch.cat(ces).detach().cpu()
        _res={'plain':_evalVS([])}
        print('  plain done',flush=True)
        for _nm,_cs in SEL['ms_sets'].items():
            _res[_nm]=_evalVS(_cs)
            print(f'  splice {_nm} done',flush=True)
        SEL['ms_results']=_res
...
if __name__=='__main__':
    ...
    PICKS=['r.2.0','r.3.0','r.0.0','r.1.2','r.6.2.2']
    MASKS={}; SETS={}
    g=torch.Generator().manual_seed(31)
    for i9,t in enumerate(PICKS):
        lf=CN.leaf(t)
        mm=torch.zeros(NFLAT,dtype=torch.bool); mm[lf['member']]=True
        MASKS[t]=mm
        tops=[e['component'] for e in BATC[t]['mean_ablation']['top'][:5]]
        SETS[f'{t}:minset']=tops
        SETS[f'{t}:rand5']=[SIX[j] for j in torch.randperm(len(SIX),generator=g)[:5].tolist()]
    SEL['mode']='norm'; SEL['K']=2304; SEL['K69']=2304; SEL['K69MAP']={6:576,7:576,8:288,9:288}
    SEL['skipset']=(); SEL['motif_off']=(); SEL['clsdmg']=False; SEL['ext_rows']=CROWS
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['minset_splice']=True; SEL['ms_sets']=SETS
    print(f'ARMS: plain + {len(SETS)} splice arms',flush=True)
    main()
    A=SEL['ms_results']
    ...
    for t in PICKS:
        mm=MASKS[t]
        base=float(dpl[mm].abs().mean())
        recm=(base-float((A[f'{t}:minset']-CBASE)[mm].abs().mean()))/max(base,1e-9)
        recr=(base-float((A[f'{t}:rand5']-CBASE)[mm].abs().mean()))/max(base,1e-9)
        if recm!=recm or recr!=recr:
            raise SystemExit(f'INSTRUMENT FAIL: NaN recovery at {t} (capture overflow?)')
        rows.append({'tag':t,'plain_member':round(base,4),'r132':round(R132.get(t,float('nan')),4),
                     'rand5':SETS[f'{t}:rand5'],
                     'recovery_minset':round(recm,3),'recovery_rand5':round(recr,3),
                     'minset':SETS[f'{t}:minset']})
        print(f"  {t}: plain {base:.3f} | minset rec {recm:.2f} | rand5 rec {recr:.2f}",flush=True)
    if any(torch.equal(A[k],A['plain']) for k in A if k!='plain'):
        raise SystemExit('INSTRUMENT FAIL: a splice arm is bitwise equal to plain')
    pa=abs(stt.median([r['recovery_minset'] for r in rows])-0.404)<=0.05
    pb=sum(1 for r in rows if r['recovery_minset']>=1.3*max(r['recovery_rand5'],1e-9))>=3
    reps=[r['plain_member']/max(r['r132'],1e-9) for r in rows]
    pc=0.85<=stt.median(reps)<=1.15
    res={'rows':rows,'median_minset_recovery':round(stt.median([r['recovery_minset'] for r in rows]),3),
         'convention':'recovery = (plain - spliced)/plain member damage; frontier build; census rows',
         'pred_a_repro_173':bool(pa),'pred_b_specific':bool(pb),'pred_c_repro':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) median minset recovery {res['median_minset_recovery']:.3f} within 0.05 of 0.404: {'HELD' if pa else 'FAILED'}")
    print(f"(b) minset >= 1.3 x rand5 for {sum(1 for r in rows if r['recovery_minset']>=1.3*max(r['recovery_rand5'],1e-9))} >= 3: {'HELD' if pb else 'FAILED'}")
    print(f"(c) plain vs rung-132 median ratio {stt.median(reps):.3f} in [0.85, 1.15]: {'HELD' if pc else 'FAILED'}")
