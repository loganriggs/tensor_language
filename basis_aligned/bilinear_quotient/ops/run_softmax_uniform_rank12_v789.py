...
at UNIFORM rank 12 -- 9.4% of its 128-dim head, against Pythia's 31% at rank 20 and 75% at rank 48 -- with the same protocol as v752
...
PREDICTIONS (scored as written; failures preserved)
    ...
    pred_b_closed_form_cost     rank-12 closed-form (SVD-truncated, unfitted) cost <= 0.7. Prior: likely
    pred_c_fitted_cost          rank-12 fitted cost <= 0.24, i.e. recovery >= 0.929 — pins the twin's crossing of 0.95 between rank 12 and 16 (rank 16: 0.949). Prior: unsure
    pred_d_values_preserved_rank rank-12 manipulability Spearman >= 0.7. Prior: likely
    pred_e_values_preserved_scale rank-12 median value ratio in [0.5, 2]. Prior: likely
...
OUT = ROOT / "circuits/followups/softmax_uniform_rank12_v789_result.json"
OUT_PT = ROOT / "circuits/followups/softmax_uniform_rank12_v789_programs.pt"
...
CANDIDATE_ID = "softmax.uniform_rank12_v789"
...
VALUE_FLOOR = 0.005
RANK = 12
REPLAY_TOL, CF_MAX, FIT_MAX, RHO_MIN, RATIO = 0.002, 0.7, 0.24, 0.7, (0.5, 2.0)
PREDICTIONS = {"pred_a_native_replays": "+-0.002", "pred_b_closed_form_cost": "rank 12 <= 0.7", "pred_c_fitted_cost": "rank 12 <= 0.24", "pred_d_values_preserved_rank": "rank 12 Spearman >= 0.7", "pred_e_values_preserved_scale": "rank 12 median ratio in [0.5, 2]"}
