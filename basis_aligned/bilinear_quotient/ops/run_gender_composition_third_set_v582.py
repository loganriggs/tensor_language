#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_composition_replays_on_third-set pred_c_copiers_post_noun_replay pred_d_noun_values_carry pred_e_joint_at_least_the_sum
"""The gender composition on the THIRD gender set (v582). v576 (fineweb set 1, 61 pairs) and v577 (pile, 64 pairs: noun values 0.626, copiers 0.232, joint 0.899): the two copiers' post-noun writes close 0.256, and with the five readers' values at
the noun 0.898 -- the whole channel. Here the same three edits (plus the readers' noun values alone) on v578's third natural gender set (fineweb documents after the 7617 that v73 scanned).
PREDICTIONS (scored as written; failures preserved; priors from v549 / v576)
    pred_a_baseline_replays            the unedited manual forward replays the model's he - she margins within 1e-3
    pred_b_composition_replays_on_third-set COPIERS_POST_AND_NOUN closes 0.898 +- 0.10 on the third-set pairs
    pred_c_copiers_post_noun_replay    COPIERS_POST closes 0.256 +- 0.08
    pred_d_noun_values_carry           the five readers' values at the noun (NOUN) close >= 0.45 (fineweb: 0.600)
    pred_e_joint_at_least_the_sum      COPIERS_POST_AND_NOUN >= NOUN + COPIERS_POST - 0.05 (as on fineweb: no nesting between the two sources)
PRICE (registered maximum): 2 batches x 4 passes = 8 forwards (64 pairs); 0 backwards; 0 fits. Bar <= 9.
"""
...
OUT = ROOT / "circuits/followups/gender_composition_third_set_v582_result.json"
CANDIDATE_ID = "gender.composition_third_set_v582"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, JOINT_V576, JBAND, COPIERS_V576, CBAND, NOUN_MIN, NEST, UNIT = 1e-3, 0.898, 0.10, 0.256, 0.08, 0.45, 0.05, 701
PILE = dod_battery.ROOT / "circuits/followups/pronoun_gender_dod_natural_rows_v578.json"   # third gender set: fineweb docs after the first 7617 (v578)
...
FORWARDS_MAX = 9
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_composition_replays_on_third-set": "0.898 +- 0.10", "pred_c_copiers_post_noun_replay": "0.256 +- 0.08", "pred_d_noun_values_carry": ">= 0.45", "pred_e_joint_at_least_the_sum": ">= NOUN + COPIERS - 0.05"}
...
def main() -> None:
    ...
    def partner(tok):
        ...
        return None
    text_items = gv.gender_pairs(PILE); verb_items = [(p, s_, c, 1) for p, s_, c in text_items]
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "joint_v576": JOINT_V576, "jband": JBAND, "copiers_v576": COPIERS_V576, "cband": CBAND, "noun_min": NOUN_MIN, "nest": NEST, "unit": UNIT}}
    ...
    MODES = {"NOUN": (FIVE, (0,)), "COPIERS_POST": ({}, (0,)), "COPIERS_POST_AND_NOUN": (FIVE, (0,))}; SETS_W = {"COPIERS_POST": {8: [1], 6: [1]}, "COPIERS_POST_AND_NOUN": {8: [1], 6: [1]}}
    ...
    print("third-set: noun values", round(mc["NOUN"], 3), "copiers post-noun", round(mc["COPIERS_POST"], 3), "joint", round(mc["COPIERS_POST_AND_NOUN"], 3))
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_composition_replays_on_third-set": abs(mc["COPIERS_POST_AND_NOUN"] - JOINT_V576) <= JBAND, "pred_c_copiers_post_noun_replay": abs(mc["COPIERS_POST"] - COPIERS_V576) <= CBAND,
                   "pred_d_noun_values_carry": mc["NOUN"] >= NOUN_MIN, "pred_e_joint_at_least_the_sum": mc["COPIERS_POST_AND_NOUN"] >= mc["NOUN"] + mc["COPIERS_POST"] - NEST}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "gender_composition_third_set_result_v582", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
