#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the sixteenth eight spec-authored behaviours, plus one instrument.
"""v233: the AMENDED tier-3 battery (v196/v197 protocol, unchanged) over the SIXTEENTH batch of spec-authored behaviours (eight) -- plus one instrument.
...
v231 7/8 (+instrument each) at ~30-45 GPU-s per behaviour. v231 confirmed the v225/v229 reading in one battery: grammatical cues with NEW
readout tokens pass 7/8 (the one miss was the weakest countability cell), inference readouts 0/15. This batch keeps that recipe and opens
EIGHT readout token pairs never screened before, authored through `circuit_fast_screen_behaviour_spec`, single-cue swap, matched final
token, cue never final, 32 A1/A2/P/C rows, ABAB directions:
degree_synthetic_periphrastic_adverb (`The pilot near the lantern ran fast/gracefully, of course, and the sailor ran even` -> faster/more;
  third member of the v231 degree-morphology pair, first ADVERB member);
modal_ellipsis_should_might (`The pilot near the lantern should/might lift the crate, of course, and so` -> should/might; second member of
  the modal-ellipsis family whose can/must singleton is counted -- this member decides whether that family fuses);
predicate_category_well_good (`The pilot near the lantern sang/looked, of course, remarkably` -> well/good; predicate-category family,
  suppletive adverb);
category_selection_during_while, category_selection_despite_although, category_selection_because_of
  (`The pilot near the lantern slept soundly during/while, of course,` -> the/it; `... kept working despite/although, of course,` -> the/it;
  `... stayed home because of/because, of course,` -> the/it; NEW family -- a connective's CATEGORY selects an NP or a clause complement;
  three siblings sharing one readout pair, so the family's within-batch separability is the interesting question);
duration_point_since_for (`The pilot near the lantern has been waiting for/since, of course,` -> hours/noon; NEW family, span vs point);
reflexive_animacy_himself_itself (`The pilot/crate near the lantern stood there, of course, all by` -> himself/itself; animacy family,
  he/it, who/which, whom/which screened, first reflexive readout).
Capability printed BEFORE the run on CPU (`g.prepare(valid_only=True)`, 32 rows per family, 19:12 UTC): ZERO A1 and ZERO A2 rows dropped
on all eight, no repairs; margins (mean base/donor on the donor axis) A1: reflexive_animacy_himself_itself -2.95/+3.29,
modal_ellipsis_should_might -2.94/+2.79, degree_synthetic_periphrastic_adverb -2.53/+1.89, duration_point_since_for -2.00/+1.57,
category_selection_because_of -1.92/+2.01, category_selection_despite_although -1.61/+1.68, predicate_category_well_good -1.15/+0.99,
category_selection_during_while -0.96/+1.00 (the weakest cell, still 3-4x the inference readouts of v225/v229);
A2: well_good -2.73/+2.77, adverb -2.84/+2.44, himself_itself -2.31/+2.70, because_of -1.87/+2.04, since_for -1.92/+1.84,
despite_although -1.52/+1.59, should_might -1.55/+1.44, during_while -1.20/+1.23. C (shared night frame) -1.43/+0.93, 2 dropped, as always.
Protocol identical to v197/v201/v204/.../v231 (greedy pool 40, target 0.88, min_gain 0.005, max 30; row 2 = MIN over the two held directions
...
Accounting: four-row passes here join the one-protocol standing. Seven of the eight have screened family members (degree morphology,
modal ellipsis, predicate category, animacy) or siblings in this batch (the three category_selection frames) and so count only after a
separability rung under the full family control set; duration_point_since_for is family-less and counts as a singleton.
...
Smoke: V233_SMOKE=<out.json> (CPU, V233_SMOKE_ROWS=4 per split, V233_SMOKE_NAMES=duration_point_since_for; pool 3/max 3, steps 5).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec8p_v233_result.json"
...
NAMES = {"degree_synthetic_periphrastic_adverb": "degree_synthetic_periphrastic_adverb",
         "modal_ellipsis_should_might": "modal_ellipsis_should_might", "predicate_category_well_good": "predicate_category_well_good",
         "category_selection_during_while": "category_selection_during_while",
         "category_selection_despite_although": "category_selection_despite_although",
         "category_selection_because_of": "category_selection_because_of", "duration_point_since_for": "duration_point_since_for",
         "reflexive_animacy_himself_itself": "reflexive_animacy_himself_itself",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec8p_v233", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V233_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V233_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V233_SMOKE_NAMES", "duration_point_since_for").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec8p_v233", "candidate_id": "corpus.unit_tier3_batch_amended_spec8p_v233", "bars": BARS,
