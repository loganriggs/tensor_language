# HOW NOISY IS THE ESTIMATOR? -- pricing §1914's +-0.2 and checking the ranking survives.
#
# §1914 found that re-running §1913's within-stratum arm on IDENTICAL data and code gave
# 4.25 / 5.19 / 5.25 / 5.30 against 4.22 / 5.02 / 5.29 / 5.29 -- a 0.17 shift at q1 -- because the run
# drew extra permutations for its global null and left the shared torch.Generator at a different state.
# At NPERM = 64 the permutation-null estimate therefore carries roughly +-0.2 of enrichment, which is at
# or above several of the "reproduces exactly" anchor checks written since §1885.
#
# That figure was inferred from ONE discrepancy. This measures it: run the covered-arm baseline at
# NPERM = 64 under eight independent seeds, and again at NPERM = 64, and report the spread of each. It
# also re-checks §1914's headline ranking at the higher NPERM, since a ranking established at +-0.2
# should be confirmed where the noise is a quarter of that.
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY. Rung 3: §1914's open precision question.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
#   pred_a §1914's ESTIMATE IS RIGHT IN ORDER: the seed-to-seed spread of the covered-arm baseline at
#          NPERM = 64 is between 0.05 and 0.60 on all three roles -- i.e. §1914's inferred +-0.2 is the
#          right order rather than an accident of one comparison. If the spread is BELOW 0.05, the 0.17
#          discrepancy §1914 saw was not RNG and I have mis-attributed it; if ABOVE 0.60, the estimator
#          is far noisier than I said and more anchor claims are affected than §1914 admits.
#   pred_b AND NPERM = 64 CUTS IT AS SQRT(8) SHOULD: the spread at NPERM = 64 is below half the spread at
#          NPERM = 64, on all three roles. Permutation-null error should fall as 1/sqrt(NPERM), so an 8x
#          increase should give ~2.8x; below 2x would mean something other than sampling dominates.
#   pred_c AND THE §1914 RANKING SURVIVES: at NPERM = 64 the TOKEN-margin gradient still exceeds the
#          LIVE-margin gradient on all three roles, as §1914's PUBLISHED +9.50/+9.79/+9.28 vs
#          +6.03/+6.36/+6.02 found at NPERM = 64. If FALSE the reversal I posted at 10:19Z was itself
#          noise, and I would retract it as loudly as I posted it.
#   pred_d CONTROLS: coverage is exactly 16,110; the NPERM = 64 central baseline is within 0.3 of the
#          NPERM = 64 one (unbiased, merely noisy); and all seeds are run on identical positions.
...
OUT = PT + 'ops/estimator_noise_results.json'
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'ESTIMATOR NOISE: how big is the permutation-null error? | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    S1914_LIV = {'skip7000': 6.03, 'skip11000': 6.36, 'skip1200': 6.02}
    sp8 = {e: max(top(e, 'baseline_seeds_8')) - min(top(e, 'baseline_seeds_8')) for e in roles}
    sp64 = {e: max(top(e, 'baseline_seeds_64')) - min(top(e, 'baseline_seeds_64')) for e in roles}
    m8 = {e: sum(top(e, 'baseline_seeds_8')) / 8.0 for e in roles}
    m64 = {e: sum(top(e, 'baseline_seeds_64')) / 8.0 for e in roles}
    ...
    pa = all(0.05 <= sp8[e] <= 0.60 for e in roles)
    pb = all(sp64[e] < 0.5 * sp8[e] for e in roles)
    pc = all(gtok[e] > gliv[e] for e in roles)
    pd = (ncov == NCOV and all(abs(m8[e] - m64[e]) <= 0.3 for e in roles))

    print(f'\n  covered-arm baseline enrichment under 8 independent seeds:', flush=True)
    for e in roles:
        print(f'    {e:10s} NPERM=8  mean {m8[e]:.3f} spread {sp8[e]:.3f}   '
              f'NPERM=64 mean {m64[e]:.3f} spread {sp64[e]:.3f}', flush=True)
    print(f'\n  §1914\'s +-0.2 is the right ORDER (0.05 <= spread8 <= 0.60) -> {pa}  ' + '  '.join(
        f'{e} {sp8[e]:.3f}' for e in roles), flush=True)
    print(f'  and NPERM=64 halves it -> {pb}  ' + '  '.join(
        f'{e} {sp64[e]:.3f} vs {0.5 * sp8[e]:.3f}' for e in roles), flush=True)
    print(f'  and the §1914 RANKING survives at NPERM=64 -> {pc}  ' + '  '.join(
        f'{e} token {gtok[e]:+.2f} vs live {gliv[e]:+.2f} (§1914 {S1914_TOK[e]:+.2f}/{S1914_LIV[e]:+.2f})'
        for e in roles), flush=True)
    print(f'  coverage {ncov}, NPERM=8 unbiased vs 64 (<=0.3) -> control {pd}  ' + '  '.join(
        f'{e} {abs(m8[e] - m64[e]):.3f}' for e in roles), flush=True)
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_noise_order_right': bool(pa),
                          'pred_b_nperm64_halves_it': bool(pb),
                          'pred_c_ranking_survives_at_64': bool(pc),
