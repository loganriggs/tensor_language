#!/usr/bin/env python3
# BQLANE: gpu
# BQGATE: EXPERIMENT pred_a_exact_instrument pred_b_top10_concentration pred_c_cross_module_live pred_d_typed_writes_live pred_e_stable_leader
"""Split regional head9.8 QK1 carry×carry into embedding/layer0-7 source pairs."""
...
PREREG=P/"SETTING2_REGIONAL_HEAD9_8_QK1_CARRY_SOURCE_FOLD_V1_PREREGISTRATION.md"
ROWS=P/"FIRST_TOKEN_PATH_FRESH_V1_ROWS.json"
BINDING=P/"SETTING2_REGIONAL_HEAD9_8_QK1_CARRY_SOURCE_FOLD_V1_BINDING.json"
PARENT=ROOT/"basis_aligned/bilinear_quotient/circuits/fast_screens/setting2_regional_head9_8_qk_source_fold_v3_result.json"
OUT=ROOT/"basis_aligned/bilinear_quotient/circuits/fast_screens/setting2_regional_head9_8_qk1_carry_source_fold_v1_result.json"
NAMES=("embedding",)+tuple(x for layer in range(8) for x in (f"attn{layer}",f"mlp{layer}"))
PRICE={"physical_prefix_forwards":14,"sequences":96,"carry_sources":17,"ordered_source_terms":289,"fits":0,"backwards":0,"parameter_updates":0}
...
def load_bound():
    ...
    if parent["terminal"]!="valid_qk_source_fold" or parent["rankings"]["qk1"][0]!="carry8_x_carry8": raise ValueError("parent QK1 carry path is not leading")
...
def plan():
    ...
    return {"schema":"setting2_regional_head9_8_qk1_carry_source_fold_v1_plan","model_loaded":False,"gpu_accessed":False,"queue_touched":False,"rows":len(rows),"sources":NAMES,"price":PRICE,"row_checks":bound["row_checks"],"binding_sha256":sha(BINDING)}
...
def main():
    ...
    folded=torch.empty(289,96,dtype=torch.float64); parent_fold=torch.empty(96,dtype=torch.float64); complete_fold=torch.empty(96,dtype=torch.float64)
    ...
    for _,indices in sorted(bound["buckets"].items()):
        for offset in range(0,len(indices),8):
            ...
            parts=[embed_c*x0.double().cpu()]+[write_c[name]*writes[name].double().cpu() for name in NAMES[1:]]
            ...
            qp,qtotal=head_project(normalized,total,q1); kp,ktotal=head_project(normalized,total,k1)
            ...
            components=torch.stack([torch.einsum("bd,btd->bt",qp[u][:,-1],kp[v])/128 for u in range(17) for v in range(17)])
            ...
            q2c,q2total=head_project([],total,q2); k2c,k2total=head_project([],total,k2)
    ...
    reports={}; families=sorted(set(r["family_name"] for r in rows)); term_index={}
    for term,(u,v) in enumerate((u,v) for u in range(17) for v in range(17)):
        name=f"{NAMES[u]}_x_{NAMES[v]}"; term_index[name]=term; fr={}
    ...
    ranking=sorted(reports,key=lambda n:reports[n]["change_norm_ratio"],reverse=True); top10_error=rel(sum(dt[term_index[n]] for n in ranking[:10]),delta)
    cross_ids=[i for i,(u,v) in enumerate((u,v) for u in range(17) for v in range(17)) if u!=v]
    attn_ids=[i for i,(u,v) in enumerate((u,v) for u in range(17) for v in range(17)) if NAMES[u].startswith("attn") or NAMES[v].startswith("attn")]
    mlp_ids=[i for i,(u,v) in enumerate((u,v) for u in range(17) for v in range(17)) if NAMES[u].startswith("mlp") or NAMES[v].startswith("mlp")]
    cross_ratio=float(dt[cross_ids].sum(0).norm()/delta.norm().clamp_min(1e-30)); attn_ratio=float(dt[attn_ids].sum(0).norm()/delta.norm().clamp_min(1e-30)); mlp_ratio=float(dt[mlp_ids].sum(0).norm()/delta.norm().clamp_min(1e-30))
    parent_ratio=float(delta.norm()/delta_complete.norm().clamp_min(1e-30)); expected=parent["term_reports"]["qk1"]["carry8_x_carry8"]["change_norm_ratio"]; parent_bridge=abs(parent_ratio-expected)
    leader=reports[ranking[0]]; instrument=forward_count==14 and carry_error<=1e-6 and score_error<=1e-8 and fold_error<=1e-8 and parent_bridge<=1e-6
    predictions={"pred_a_exact_instrument":bool(instrument),"pred_b_top10_concentration":bool(instrument and top10_error<=.50),"pred_c_cross_module_live":bool(instrument and cross_ratio>=.20),"pred_d_typed_writes_live":bool(instrument and max(attn_ratio,mlp_ratio)>=.10),"pred_e_stable_leader":bool(instrument and leader["change_norm_ratio"]>=.10 and min(leader["family_change_norm_ratios"].values())>=.03)}
    result={"schema":"setting2_regional_head9_8_qk1_carry_source_fold_v1_result","terminal":"valid_qk1_carry_source_fold" if instrument else "invalid","predictions":predictions,"metrics":{"carry_source_reconstruction_relative_error":carry_error,"qk1_carry_score_sum_relative_error":score_error,"folded_sum_relative_error":fold_error,"parent_change_norm_ratio":parent_ratio,"parent_ratio_absolute_bridge":parent_bridge,"top10_replay_relative_error":top10_error,"cross_source_aggregate_change_norm_ratio":cross_ratio,"attention_containing_aggregate_change_norm_ratio":attn_ratio,"mlp_containing_aggregate_change_norm_ratio":mlp_ratio,"physical_prefix_forwards":forward_count,"sequences":96,"pair_count":48},"source_names":NAMES,"embed_coefficient":embed_c,"write_coefficients":write_c,"ranking":ranking,"top10_terms":ranking[:10],"term_reports":reports,"price":PRICE,"row_checks":planned["row_checks"],"outcome_access":{"behavioral_logits":False,"new_rows":False,"fits":False},"checkpoint_weights_sha256":"680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3","binding_sha256":sha(BINDING),"runner_sha256":sha(RUNNER),"created_utc":datetime.now(timezone.utc).isoformat().replace("+00:00","Z"),"scope":"Exact embedding/layer0-7 ordered source attribution inside the QK1 carry×carry term of the regional head9.8×MLP16×MLP17 path; fixed denominators, QK2, value, suffix, and reader."}
    managed.atomic_create_json(OUT,result); print(json.dumps({"terminal":result["terminal"],"predictions":predictions,"metrics":result["metrics"],"top10":ranking[:10],"top10_ratios":{n:reports[n]["change_norm_ratio"] for n in ranking[:10]}},indent=2)); assert instrument
