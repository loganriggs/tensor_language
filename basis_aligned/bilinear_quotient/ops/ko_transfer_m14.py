"""SIGNAL-TO-FLOOR TEST (m14, rung 267): the SMALL component - degradation predicted (like m13).

CONVENTION (S2135): per-position dCE = CE(mixed config with m14 mean-ablated) - CE(real model); LOWER
IS BETTER. S2364/S2365: knockout transfer held for m16 (own-mean 1.353; collateral rho 0.858) and
degraded for m13 (0.590; rho 0.649) - fidelity appears to track intervention signal vs the 0.0573
compression floor. m14 (battery own-mean 0.596 - small, like m13) is the degradation arm of the preregistered pair.
REGISTERED PREDICTIONS:
  (a) OWN-FAMILY TRANSFERS: median member/ref ratio over m14-top circuits in [0.7, 1.6].
  (b) SIGNAL-TO-FLOOR - collateral Spearman {_rhoK
  (c) sanity census band.
NULL: fidelity holds even for the small component (rho >= 0.85 - the floor hypothesis dies).
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: knockout transfer m14')
    raise SystemExit(0)
import torch
...
OUT=PT+'ko_transfer_m14_results.json'
...
if __name__=='__main__':
    ...
    _hmu=m.transformer.h[14].mlp.register_forward_hook(_mucap)
    with torch.no_grad():
    ...
    m.transformer.h[14].mlp.register_forward_hook(_ablh)
    print('m14 mean captured over FW; gated ablation hook armed',flush=True)
    main()
    ...
    _own=sorted(r['member_absdce']/max(r['ref'],1e-9) for r in rows if _topK.get(r['tag'])=='m14')
    _omed=_own[len(_own)//2]
    _MR=json.load(open(PT+'removal_matrix_results.json'))['matrix']['m14']
    ...
    print(f"(b) SIGNAL-TO-FLOOR - collateral Spearman {_rhoK:.3f} < 0.8 predicted; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) ablated census {agg:+.4f} in [0.08, 0.5]; era rho {rho:.4f}: {'HELD' if pc else 'FAILED'}")
