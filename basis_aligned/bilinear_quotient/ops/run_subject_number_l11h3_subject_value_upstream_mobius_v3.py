...
"""Float64 closed-gauge audit of the V2 upstream subject-value decomposition."""
...
import run_subject_number_l11h3_subject_value_upstream_mobius_v2 as prior
...
DECODER = base.DECODER; RANK1 = base.RANK1; PARENT = base.PARENT
PRIOR_RESULT = POLY / "SUBJECT_NUMBER_L11H3_SUBJECT_VALUE_UPSTREAM_MOBIUS_V2_RESULT.json"
PREREG = POLY / "SUBJECT_NUMBER_L11H3_SUBJECT_VALUE_UPSTREAM_MOBIUS_V3_PREREGISTRATION.md"
BINDING = POLY / "SUBJECT_NUMBER_L11H3_SUBJECT_VALUE_UPSTREAM_MOBIUS_V3_BINDING.json"
OUT = POLY / "SUBJECT_NUMBER_L11H3_SUBJECT_VALUE_UPSTREAM_MOBIUS_V3_RESULT.json"
PORTS = base.PORTS; PRICE = base.PRICE; NULLS = base.NULLS; SEED = base.SEED
LAYER, HEAD, HEAD_WIDTH = base.LAYER, base.HEAD, base.HEAD_WIDTH
BARS = {k: v for k, v in base.BARS.items() if k != "maximum_component_reconstruction_error"}
BARS.update(maximum_closed_reconstruction_error=1e-10, maximum_late_gauge_correction_relative_l2=1e-5)
...
def load_bound():
    ...
    paths = {"authority": Path(authority.__file__), "source_primitive": Path(source_factor.__file__),
             "prior_runner": Path(prior.__file__), "prior_result": PRIOR_RESULT, "decoder": DECODER,
             "rank1": RANK1, "parent_result": PARENT, "preregistration": PREREG}
    ...
    decoder, rank, parent_result, prior_result = (json.loads(p.read_text()) for p in (DECODER, RANK1, PARENT, PRIOR_RESULT))
    if decoder["terminal"] != "embedding_number_decoder_frozen" or rank["terminal"] != "rank1_frozen_weights_only" \
            or parent_result["terminal"] != "valid_source_factor_decomposition" or prior_result["terminal"] != "invalid":
        raise ValueError("parent status changed")
    return binding, decoder, rank, parent_result, prior_result, authority.build_rows()

def plan():
    _, decoder, rank, parent_result, prior_result, rows = load_bound()
    return {"schema": "subject_number_l11h3_subject_value_upstream_mobius_v3_plan", "model_loaded": False,
            "gpu_accessed": False, "queue_touched": False, "rows": len(rows), "batch_sizes": [64, 64],
            "ports": list(PORTS), "correction": "float64_closed_late_gauge", "prior_terminal": prior_result["terminal"],
...
def main():
    ...
    signal.alarm(600); binding, decoder, rank, parent_result, prior_result, rows = load_bound()
    ...
    counts = {"partial_forwards": 0, "sequences": 0, "corners": 32, "mobius_terms": 31, "readouts": 17,
    ...
    all_atoms = [[] for _ in range(17)]; all_target = [[] for _ in range(17)]
        ...
        with torch.no_grad():
            for layer, block in enumerate(model.transformer.h):
                ...
                if layer == LAYER:
                    ...
                    explicit = {"embedding_recurrence": components[("embedding", -1)],
                        "early_writes_0_3": sum((v for (kind, idx), v in components.items() if kind != "embedding" and idx in range(0, 4)), zero),
                        "middle_writes_4_7": sum((v for (kind, idx), v in components.items() if kind != "embedding" and idx in range(4, 8)), zero),
                        "late_writes_8_10": sum((v for (kind, idx), v in components.items() if kind != "embedding" and idx in range(8, 11)), zero)}
                    groups = {k: v.double() for k, v in explicit.items()}
                    prefix = groups["embedding_recurrence"] + groups["early_writes_0_3"] + groups["middle_writes_4_7"]
                    late_explicit = groups["late_writes_8_10"]
                    groups["late_writes_8_10"] = x.double() - prefix
                    correction = groups["late_writes_8_10"] - late_explicit
                    return x, groups, first, factors, correction, late_explicit
    ...
    for ids in group_ids:
        ...
        raw_b, groups_b, first_b, factors_b, corr_b, late_b = capture(base_input, pos)
        raw_r, groups_r, first_r, factors_r, corr_r, late_r = capture(removed_input, pos)
        reconstruction = max(reconstruction, float((sum(groups_b.values()) - raw_b.double()).abs().max()),
                             float((sum(groups_r.values()) - raw_r.double()).abs().max()))
        correction_abs = max(correction_abs, float(corr_b.abs().max()), float(corr_r.abs().max()))
        correction_rel = max(correction_rel, float(corr_b.norm() / late_b.norm().clamp_min(1e-30)),
                             float(corr_r.norm() / late_r.norm().clamp_min(1e-30)))
        ...
        def corner(mask):
            raw = sum((groups_r if mask & (1 << i) else groups_b)[PORTS[i]] for i in range(4)).float()
        ...
        for ai in range(17):
            all_atoms[ai].append(torch.stack([-dividends[m][:, ai] for m in range(1, 32)]).cpu().numpy())
            all_target[ai].append(target[:, ai].cpu().numpy())
    names = ["*".join(PORTS[i] for i in range(5) if m & (1 << i)) for m in range(1, 32)]; reports = []
    for ai in range(17):
        atoms = np.concatenate(all_atoms[ai], axis=1); target = np.concatenate(all_target[ai])
        report = {"target_rms": float(np.sqrt(np.mean(target ** 2))), "greedy": base.greedy(target, atoms, names)}
        if ai == 0:
            report["terms"] = [{"mask": m + 1, "ports": [PORTS[i] for i in range(5) if (m + 1) & (1 << i)],
                "order": (m + 1).bit_count(), "rms": float(np.sqrt(np.mean(atoms[m] ** 2))),
                "aligned_recovery": float(atoms[m] @ target / max(target @ target, 1e-30))} for m in range(31)]
        reports.append(report)
    target_report = reports[0]; random_errors = np.asarray([r["greedy"]["final_relative_l2"] for r in reports[1:]])
    advantage = float(np.median(random_errors) - target_report["greedy"]["final_relative_l2"])
    synthetic = base.parent.synthetic_fixture()
    finite = bool(np.isfinite(np.asarray([reconstruction, correction_abs, correction_rel, value_replay, closure_abs, closure_rel, synthetic, *random_errors])).all())
    pred_a = bool(finite and reconstruction <= BARS["maximum_closed_reconstruction_error"]
        and correction_rel <= BARS["maximum_late_gauge_correction_relative_l2"]
    ...
    pred_c = bool(pred_b and advantage >= BARS["minimum_random_median_advantage"])
    ...
    result = {"schema": "subject_number_l11h3_subject_value_upstream_mobius_v3_result", "terminal": terminal, "predictions": predictions,
        "instrument": {"finite": finite, "closed_reconstruction_max_abs_error": reconstruction,
            "late_gauge_correction_max_abs": correction_abs, "late_gauge_correction_max_relative_l2": correction_rel,
        ...
        "writer_axis": target_report, "random_readout_specificity": {"count": NULLS, "seed": SEED,
            "term4_relative_l2": random_errors.tolist(), "median_term4_relative_l2": float(np.median(random_errors)),
            "target_term4_relative_l2": target_report["greedy"]["final_relative_l2"], "median_advantage": advantage},
        "correction": {"prior_terminal": prior_result["terminal"], "hypothesis": "float32 summation association",
            "fix": "float64 closed late gauge with explicit correction measurement"},
        ...
        "scope": "Prospective closed-gauge audit of V2; unchanged scientific gates; V1 and V2 remain invalid."}
    managed.atomic_create_json(OUT, result)
    print(json.dumps({k: result[k] for k in ("terminal", "predictions", "instrument", "writer_axis", "random_readout_specificity", "correction")}, indent=2, sort_keys=True))
    assert pred_a
