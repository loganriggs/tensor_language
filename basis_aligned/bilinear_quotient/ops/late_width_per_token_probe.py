#!/usr/bin/env python
"""late_width_per_token_probe -- per-token decomposition of the late width cost: is blocks 8-17's MLP-read cost at 768 (§2773/§2774)
concentrated on a few tokens, on rare targets (fit-set count 0), on early positions, and on the same tokens as the whole program?
CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_width_cost_is_concentrated pred_c_rare_targets_overweighted pred_d_not_a_position_effect
#                     pred_e_same_tokens_as_whole_program
...
Preregistration: polynomial_causal/LATE_WIDTH_PER_TOKEN_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_width_per_token_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_width_per_token_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_WIDTH_PER_TOKEN_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_mlp_branch_width_probe_results.json"   # §2774
OUT = ROOT / "late_width_per_token_probe_results.json"
HASHES = {PREREG: "58b276e429d4f13df1a86ffdaafc72a6cf42c8d7a7a9676be679aad4bfce5b6b", PRIOR: "a94f3ca4ff2abf00b722233006794aa5d93e27a0fefa5c05253e947dcb677768",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_width_per_token_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_min": 0.60, "c_ratio": 1.5, "d_ratio": 2.0, "e_min": 0.30}
NULLS = {"b_max": 0.35, "c_ratio": 1.0, "d_ratio": 4.0, "e_max": 0.10}
...
def main():
    ...
    def split8(k):
        ...
        return patch
    def per_token(patch):
        objs = list(patch.values()) if patch else []
        outs = []
        for i in range(0, ev.shape[0], CH):
            idx = ev[i:i + CH, :TI]; tgt = ev[i:i + CH, 1:TI + 1]
            for o in objs:
                o.hook(idx)
            lg = forward(m, idx, patch if patch else None)
            outs.append(F.cross_entropy(lg.reshape(-1, V), tgt.reshape(-1), reduction="none").view(idx.shape[0], TI).float().cpu())
        return torch.cat(outs, 0)
    LATE = list(range(SPLIT, NL))
    tok = {"BASE": per_token(None), "SPLIT8_1024": per_token(split8(1024)),
           "LATE_MLP_768": per_token({("mlp", l): head(("mlp", l), 768, U_8) for l in LATE}),
           "BUS_768": per_token({s_: head(s_, 768, U_8) for s_ in SET8})}
    arms = {k: float((v - tok["BASE"]).mean()) for k, v in tok.items() if k != "BASE"}
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    tgt_all = ev[:, 1:TI + 1].cpu()
    cnt = torch.bincount(fit_rows[:, :TI + 1].reshape(-1).cpu(), minlength=V)
    tc = cnt[tgt_all]
    rare = (tc == 0); freq = (tc >= 100)
    pos = torch.arange(TI).unsqueeze(0).expand_as(tgt_all); early = (pos < 16)
    def share(delta, mask):
        tot = float(delta.sum()); return float(delta[mask].sum()) / tot if abs(tot) > 1e-12 else float("nan")
    def top_share(delta, frac=0.10):
        d = delta.reshape(-1); n = max(1, int(round(frac * d.numel()))); tot = float(d.sum())
        return float(d.sort(descending=True).values[:n].sum()) / tot if abs(tot) > 1e-12 else float("nan")
    D_ = {k: (tok[k] - tok["BASE"]) for k in arms}
    per_arm = {}
    for k, d in D_.items():
        per_arm[k] = {"ce_added": arms[k], "top10_share": top_share(d), "top1_share": top_share(d, 0.01), "rare_share": share(d, rare), "freq_share": share(d, freq),
                      "early_pos_share": share(d, early), "mean_rare": float(d[rare].mean()), "mean_freq": float(d[freq].mean()), "mean_early": float(d[early].mean()),
                      "by_pos_bucket": [float(d[(pos >= a) & (pos < a + 32)].mean()) for a in range(0, TI, 32)],
                      "frac_tokens_hurt": float((d > 0).float().mean())}
    rare_tok_share = float(rare.float().mean()); early_tok_share = float(early.float().mean()); freq_tok_share = float(freq.float().mean())
    rho = R.spearman(D_["LATE_MLP_768"].reshape(-1), D_["SPLIT8_1024"].reshape(-1))
    rho_bus = R.spearman(D_["LATE_MLP_768"].reshape(-1), D_["BUS_768"].reshape(-1))
    pm = per_arm["LATE_MLP_768"]
    summ = {"rare_token_share": rare_tok_share, "freq_token_share": freq_tok_share, "early_token_share": early_tok_share, "n_tokens": int(tgt_all.numel()),
            "rare_ratio": pm["rare_share"] / rare_tok_share if rare_tok_share > 0 else float("nan"), "early_ratio": pm["early_pos_share"] / early_tok_share,
            "spearman_mlp768_vs_split8": rho, "spearman_mlp768_vs_bus768": rho_bus, "base_mean": float(tok["BASE"].mean())}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(summ["base_mean"] - ce0) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(arms["LATE_MLP_768"] - PRIOR_LATE_MLP_768) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_width_cost_is_concentrated': bool(pm["top10_share"] >= BARS["b_min"]),
        'pred_c_rare_targets_overweighted': bool(summ["rare_ratio"] >= BARS["c_ratio"]),
        'pred_d_not_a_position_effect': bool(summ["early_ratio"] <= BARS["d_ratio"]),
        'pred_e_same_tokens_as_whole_program': bool(rho >= BARS["e_min"]),
    }
    nulls = {"b_null_top10_le_.35": bool(pm["top10_share"] <= NULLS["b_max"]), "c_null_rare_ratio_le_1": bool(summ["rare_ratio"] <= NULLS["c_ratio"]),
             "d_null_early_ratio_ge_4": bool(summ["early_ratio"] >= NULLS["d_ratio"]), "e_null_rho_le_.10": bool(rho <= NULLS["e_max"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "program": "per-token CE for BASE / SPLIT8_1024 / late MLP reads through U_8 at 768 / bus 768; decomposed by concentration, fit-set target frequency, position",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "base_mean_per_token": summ["base_mean"], "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "late_mlp_768": arms["LATE_MLP_768"], "prior_late_mlp_768": PRIOR_LATE_MLP_768},
           "ce_added": arms, "per_arm": per_arm, "summary": summ,
           "price": {"gpu_doc_forwards": int(fit_rows.shape[0]) + int(ev.shape[0]) * len(tok), "cpu_doc_forwards": 0, "gpu_seconds": time.time() - started},
