...
"""Pronoun number they/he DoD (v225): WHERE the carrier-vs-edit gap sits. v192 (carrier census) said the five MLP-5 units {1036, 2403, 2538, 3274, 3034} carry
64% of MLP 5's 35% = 22% of u_2483's plural - singular contrast at the noun; v195 (edit) removed 6%; v224 showed no downstream compensation. Both numbers
are functions of the same native trace, so the gap can be decomposed exactly without a forward-with-edit: with c = the five units' summed write as it
reaches block 6 and x = the block-6 input at the noun,
    D_joint = u(x) - u(x - c) = (L.c)(R.x) + (L.x)(R.c) - (L.c)(R.c)   over rms(x)^2 held      [the in-place leave-out]
and the carrier term is  da mB + mA db  with a = (L.c)/rms, b = (R.c)/rms  (v188's identity, the five units' own change x the pair-mean partner).
Reported per pooled plural - singular contrast of u_2483: the carrier term (first-order), D_joint (rms held), D_joint with rms recomputed (rms of x - c),
and the first-order cross-terms difference (carrier - D_joint). v195's edit (-0.060) is the reference.
PREDICTIONS (scored as written; failures preserved)
    pred_a_joint_leave_out_matches_edit  D_joint with rms recomputed, pooled, is within 0.02 of v195's -0.060 (fraction of the u_2483 contrast)
    pred_b_first_order_overshoots        the carrier term (first-order) is >= 1.5 x |D_joint| in magnitude
    pred_c_cross_terms_explain_gap       carrier term - D_joint (rms held) accounts for >= 0.70 of the carrier - edit gap
    pred_d_rms_effect_small              |D_joint(rms recomputed) - D_joint(rms held)| <= 0.10 x |D_joint(rms held)|
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_joint_leave_out_v225_result.json"
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_joint_leave_out_v225"
UNITS, SRC, DST, UNIT, V195_EDIT, MATCH_TOL, OVERSHOOT, EXPLAIN_MIN, RMS_MAX, BATCH = (1036, 2403, 2538, 3274, 3034), 5, 6, 2483, -0.0599, 0.02, 1.5, 0.70, 0.10, 32
FORWARDS_MAX = 5
PREDICTIONS = {"pred_a_joint_leave_out_matches_edit": "within 0.02 of -0.060", "pred_b_first_order_overshoots": ">= 1.5 x", "pred_c_cross_terms_explain_gap": ">= 0.70 of the gap", "pred_d_rms_effect_small": "<= 0.10"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "rows": len(rows), "rows_sha256": L.rows_sha256(rows), "units": list(UNITS), "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0, "fit_parameters": 0,
            "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"match_tol": MATCH_TOL, "overshoot": OVERSHOOT, "explain_min": EXPLAIN_MIN, "rms_max": RMS_MAX, "v195_edit": V195_EDIT}}
    ...
    report = {"u2483_contrast": contrast, "carrier_first_order": carrier, "D_joint_rms_held": D_held, "D_joint_rms_recomputed": D_rms, "v195_edit": -V195_EDIT, "gap_carrier_minus_edit": gap, "cross_terms_share_of_gap": explained}
    ...
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_joint_leave_out_result_v225", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
