#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_replay pred_b_full pred_c_mode pred_d_natural
...
def main():
 ...
 out=P/'LEARNED_FULL_QUADRATIC_NATIVE_V1.json';assert not out.exists()
 e={k:v.cuda().double() for k,v in torch.load(P/'MIDPOINT_CONTINUATION_NATIVE_OPERATOR_V1.pt',weights_only=True).items()};p={k:v.cuda() for k,v in torch.load(P/'LEARNED_FULL_QUADRATIC_PROGRAM_V1.pt',weights_only=True).items()}
 ...
 result=dict(predictions=pred,summary=summary,records=records,natural_records=natural,natural_summary=natural_summary,rms_ratios=scale_rows,maximum_replay=max(checks),seconds=time.monotonic()-start,scope='Path-specific MLP16 polynomial residual swap, recipient attention17 and other carry held fixed; h17/RMS17/MLP17/final RMS/softcap recomputed. Full effect compares true direct-path endpoints. Mode effect separately isolates scalar mode delta in each original background, not a claim to explain the whole path effect. Opened token-matched panel, no new fit/fresh validation. Upstream source extraction still external.')
