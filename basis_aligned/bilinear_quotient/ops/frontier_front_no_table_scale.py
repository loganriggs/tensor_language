"""IS THE LOCAL/END-TO-END MISMATCH PROCEDURAL, OR SPECIFIC TO THE TABLE+RESIDUAL PAIR? (Claude, LANE 1)

SS2897 established that the front MLP stage's compensation is **one-way**: given a refit, the rank-64 quadratic residual `A` absorbs
**4.9479** of the 5.629 nats that removing the token table costs, while the token table absorbs only **0.0215** of the residual's. Its
practical reading was "drop the TABLE, not the residual" -- the opposite of what parameter counts suggest.

That sets up a sharp question about SS2890's central claim. SS2895 found that scaling the residual improves the frontier while the table
is present. **Does it still improve once the table is gone and `A` has been refitted to cover for it?**
  ...
  - If YES, the local/end-to-end objective mismatch is a property of **the ridge fitting procedure itself** -- it reappears in whatever
    configuration is fitted, and the end-to-end refitting move SS2890 argued for is general.
  - If NO, the mismatch was specific to the **table+residual pair** -- an artefact of two co-fitted blocks dividing one job -- and
    SS2890's account is much narrower than it looks.

    RUN 1  reference : the published frontier, unmodified
    RUN 2  tb := 0 applied at REFIT time (so `A` refits to cover it), then `A` scaled at .25/.5/.75/.9 -- eval-only on that stack

pred_b anchors run 2's unscaled arm to SS2897's **+0.6811**, which SS2897 measured to a ten-thousandth against SS2877.

SIGN CONVENTION (SS2135): frontier L2 is CE ADDED ABOVE THE REAL MODEL, so **LOWER IS BETTER** (SS312: +2.6735 beating +2.84/+2.93).
A cost is L2(arm) - L2(baseline), **POSITIVE = WORSE**; a GAIN here is cost(tb dropped) - cost(tb dropped and scaled), **POSITIVE = the
scaling helps**. SS2128/SS2129/SS2133/SS2134 RETRACTED; SS2125 STANDS.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_the_table_drop_reproduces_S2897
#                     pred_c_scaling_still_helps_after_the_table_is_gone
#                     pred_d_the_gain_is_smaller_than_with_the_table_present
#                     pred_e_the_arms_are_connected

Preregistration: polynomial_causal/FRONTIER_FRONT_NO_TABLE_SCALE_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; TWO pipeline runs: reference, then table-dropped-at-refit with residual scaling")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_front_no_table_scale_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_FRONT_NO_TABLE_SCALE_PREREGISTRATION.md'
PREREG_SHA = "9f3dcf41100de384012b2ccce4600f22d5b74661a7dcafb27c06e734e564b53d"
...
S2897 = PT + 'frontier_front_anchor_resolution_results.json'
S2895 = PT + 'frontier_front_table_shrinkage_results.json'
S2897_SHA = "467bcec26283290bcc40c4ee9930173795dfb243a2858d0b77ddf77da56caac1"
S2895_SHA = "b852f7f26bf55f65e6bebdceb0ed49bf3bd1f4021b93181d797620bc9f43fc14"
A_SCALES = [0.25, 0.5, 0.75, 0.9]
BARS = {"reproduce": 0.05, "anchor": 0.02, "still_improves": 0.02, "connected": 0.005}
...
if __name__=='__main__':
    ...
    for p_, h_ in ((S2897, S2897_SHA), (S2895, S2895_SHA)):
        if _sha(p_) != h_:
            raise RuntimeError(f'frozen hash mismatch: {p_}')
    a97 = json.load(open(S2897))['summary']['cost_refit_time_tb0']       # +0.6811
    with_table = json.load(open(S2895))['summary']['best_A_cost_fresh']  # -0.1648, table present
    SEL['mode']='norm'

    print('RUN 1/2: reference baseline -- the published frontier, unmodified',flush=True)
    MLPMODE['drop']=None; ARMS[:] = [('baseline', {})]
    out1=main(); bF = out1['baseline']['L2_F']

    print('RUN 2/2: token table dropped at REFIT time; then scale the refitted residual',flush=True)
    MLPMODE['drop']='table'
    ak = lambda s: f'A{int(round(s*100)):03d}'
    ARMS[:] = [('tb_dropped', {})] + [(ak(s), {'a_scale': s}) for s in A_SCALES]
    out2=main()
    MLPMODE['drop']=None

    c_drop = round(out2['tb_dropped']['L2_F'] - bF, 4)
    c_sc   = {s: round(out2[ak(s)]['L2_F'] - bF, 4) for s in A_SCALES}
    gains  = {s: round(c_drop - c_sc[s], 4) for s in A_SCALES}   # POSITIVE = scaling helps
    best_s = max(A_SCALES, key=lambda s: gains[s])
    best_gain = gains[best_s]
    anchor_dev = abs(c_drop - a97)

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = anchor_dev <= BARS['anchor']
    pc = best_gain >= BARS['still_improves']
    pd = best_gain < abs(with_table)
    pe = abs(c_drop) >= BARS['connected']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_table_drop_reproduces_S2897':bool(pb),
           'pred_c_scaling_still_helps_after_the_table_is_gone':bool(pc),
           'pred_d_the_gain_is_smaller_than_with_the_table_present':bool(pd),
           'pred_e_the_arms_are_connected':bool(pe)}
    nulls={'b_null_the_anchor_fails':bool(anchor_dev>=NULLS['anchor_ge']),
           'c_null_the_mismatch_was_pair_specific':bool(best_gain<=NULLS['no_improvement'])}
    res={'rung':'frontier_front_no_table_scale','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,'a_scales':A_SCALES,
         'run1':out1,'run2':out2,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(bF,4),
                    'cost_table_dropped_refit':c_drop,'S2897_anchor':round(a97,4),
                    'anchor_deviation':round(anchor_dev,4),
                    'cost_by_A_scale_vs_frontier':{str(s):c_sc[s] for s in A_SCALES},
                    'gain_vs_table_dropped':{str(s):gains[s] for s in A_SCALES},
                    'best_A_scale':best_s,'best_gain':best_gain,
                    'S2895_gain_with_table_present':round(abs(with_table),4),
                    'implied_L2_F_best':round(bF + c_sc[best_s], 4)},
         ...
         'hashes':{PREREG:PREREG_SHA,S2897:S2897_SHA,S2895:S2895_SHA},'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {bF:.4f} vs 2.6735: {'HELD' if pa else 'FAILED'}")
    print(f"(b) tb dropped {c_drop:+.4f} vs SS2897 {a97:+.4f}, dev {anchor_dev:.4f}: {'HELD' if pb else 'FAILED'}")
    print("    scaled: " + "  ".join(f"s={s}:{c_sc[s]:+.4f} (gain {gains[s]:+.4f})" for s in A_SCALES))
    print(f"(c) best gain {best_gain:+.4f} at s={best_s} >= {BARS['still_improves']}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) vs SS2895 with-table gain {abs(with_table):.4f}: {'HELD' if pd else 'FAILED'}")
