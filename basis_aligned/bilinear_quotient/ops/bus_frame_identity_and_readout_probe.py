#!/usr/bin/env python
"""bus_frame_identity_and_readout_probe -- (i) is the blocks-8-17 bus frame (§2754/§2756) the LATE frame: the late-7 (blocks 11-17) or
last-4 (14-17) 1024-core serving blocks 8-17 in the SPLIT8 program; (ii) does the UNEMBED read through the bus: the final rms-normed
residual projected onto U_8 vs onto its own top-1024 frame before lm_head. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_bus_is_the_late_frame pred_c_last4_frame_still_serves pred_d_unembed_reads_through_the_bus
#                     pred_e_bus_is_nearly_the_readout_frame
...
Preregistration: polynomial_causal/BUS_FRAME_IDENTITY_AND_READOUT_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/bus_frame_identity_and_readout_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("bus_frame_identity_and_readout_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "BUS_FRAME_IDENTITY_AND_READOUT_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "settled_frame_bus_probe_results.json"   # §2756
OUT = ROOT / "bus_frame_identity_and_readout_probe_results.json"
HASHES = {PREREG: "03765266956a4f5bf0b3ed9b389cf14366a23ad2461bb222f8b1d7fbddc1402e", PRIOR: "6c637a4743260c579e449f3e6f4317a2bb6cd12bd6b1347318e211341874c5ed",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "bus_frame_identity_and_readout_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_max": 0.005, "c_max": 0.015, "d_max": 0.030, "e_max": 0.015}
NULLS = {"b_min": 0.020, "c_min": 0.040, "d_min": 0.080, "e_min": 0.050}
...
def main():
    ...
    fst = {"G": torch.zeros(D, D, dtype=torch.float64, device=DEV), "s": torch.zeros(D, dtype=torch.float64, device=DEV)}
    class Rec:
    ...
    for i in range(0, fit_rows.shape[0], CH):
        ...
        forward(m, idx, patch={s_: Rec(s_) for s_ in ASITES}, collect=lambda s_, w: box.__setitem__(s_, w) if (s_[0] == "mlpin" and ("mlp", s_[1]) in SITES) or s_ in SITES + ASITES or s_ == FINAL else None)
        xf = F.rms_norm(box[FINAL], (D,)).reshape(-1, D).double(); fst["G"] += xf.T @ xf; fst["s"] += xf.sum(0)
        for s_ in SITES:
    ...
    mf = fst["s"] / n; Cf = fst["G"] / n - torch.outer(mf, mf); _, Uf = torch.linalg.eigh(Cf); fhead = {"U": Uf.flip(1).float(), "mx": mf.float(), "Cx": Cf}
    del st, wst, fst
    ...
    class FinalRead:
        def __init__(self, U_): self.U = U_[:, :K]; self.mx = fhead["mx"]
        def hook(self, idx): pass
        def __call__(self, x, _):
            xh = F.rms_norm(x, (D,)); return self.mx + ((xh - self.mx) @ self.U) @ self.U.T
    OWN8 = blocks(range(0, SPLIT)); SET8 = blocks(range(SPLIT, NL))
    frames = {"U8": core_of(SET8), "L7": core_of(blocks(range(11, NL))), "L4": core_of(blocks(range(14, NL)))}
    def split8(Ush):
        patch = {s_: head(s_, K) for s_ in OWN8}; patch.update({s_: head(s_, K, Ush) for s_ in SET8}); return run(patch)
    arms = {"SPLIT8_1024": split8(frames["U8"]), "SPLIT8_L7FRAME_1024": split8(frames["L7"]), "SPLIT8_L4FRAME_1024": split8(frames["L4"]),
            "FINAL_ON_BUS_1024": run({FINAL: FinalRead(frames["U8"])}), "FINAL_OWN_1024": run({FINAL: FinalRead(fhead["U"])})}
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    def capture(C, U_):
        C = C.to(U_.dtype); Uk = U_[:, :K]; return float(torch.trace(Uk.T @ C @ Uk) / torch.trace(C))
    CU = (m.lm_head.weight.detach().float().to(DEV).T @ m.lm_head.weight.detach().float().to(DEV))
    caps = {f"{nm}_captures_{cn}": capture(C, U_) for nm, U_ in list(frames.items()) + [("Uf", fhead["U"])] for cn, C in (("final_input", fhead["Cx"]), ("unembed_gram", CU))}
    s = torch.linalg.svdvals((frames["U8"][:, :K].T @ frames["L7"][:, :K]).double()).clamp(-1, 1); deg = torch.rad2deg(torch.acos(s))
    ang = {"U8_vs_L7_n_gt_10": int((deg > 10).sum()), "U8_vs_L7_n_gt_30": int((deg > 30).sum()), "U8_vs_L7_max_deg": float(deg.max())}
    gaps = {"l7_minus_u8": arms["SPLIT8_L7FRAME_1024"] - arms["SPLIT8_1024"], "l4_minus_u8": arms["SPLIT8_L4FRAME_1024"] - arms["SPLIT8_1024"],
            "final_on_bus_minus_final_own": arms["FINAL_ON_BUS_1024"] - arms["FINAL_OWN_1024"]}
    ...
    preds = {
        ...
        'pred_b_bus_is_the_late_frame': bool(gaps["l7_minus_u8"] <= BARS["b_max"]),
        'pred_c_last4_frame_still_serves': bool(gaps["l4_minus_u8"] <= BARS["c_max"]),
        'pred_d_unembed_reads_through_the_bus': bool(arms["FINAL_ON_BUS_1024"] <= BARS["d_max"]),
        'pred_e_bus_is_nearly_the_readout_frame': bool(gaps["final_on_bus_minus_final_own"] <= BARS["e_max"]),
    }
    nulls = {"b_null_ge_.02": bool(gaps["l7_minus_u8"] >= NULLS["b_min"]), "c_null_ge_.04": bool(gaps["l4_minus_u8"] >= NULLS["c_min"]),
             "d_null_ge_.08": bool(arms["FINAL_ON_BUS_1024"] >= NULLS["d_min"]), "e_null_ge_.05": bool(gaps["final_on_bus_minus_final_own"] >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "ce_added": arms, "gaps": gaps, "captures_1024": caps, "angles_1024": ang, "final_input_eff_rank": R.spectrum(fhead["Cx"].cpu())["eff_rank"],
