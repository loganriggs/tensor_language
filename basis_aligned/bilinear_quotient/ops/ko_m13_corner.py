"""ADDITIVITY CONTROL (m13, rung 271): the same knockout on a DIFFERENT baseline (corner config).

CONVENTION (S2135): per-position dCE = CE(corner config with m13 mean-ablated) - CE(real model); LOWER
IS BETTER. S2370's provisional re-analysis says baseline-SUBTRACTED knockout metrics are the honest ones
(mixed-baseline values: m13 subtracted own 0.486, collateral 0.90). The physical test of the
subtraction: rerun on the CORNER baseline (0.0608 profile, different from mixed 0.0573) subtracting the
corner receipt instead - baseline-independent quantities must agree.
REGISTERED PREDICTIONS (arms: THIS (corner baseline) vs the mixed-baseline subtracted values):
  (a) BASELINE-INDEPENDENT: subtracted own median in [0.24, 0.74] (mixed value +/- 0.25 x ref-scale).
  (b) subtracted collateral Spearman >= 0.85.
  (c) SANITY: ablated census in [0.08, 0.5].
NULL: subtraction is baseline-dependent (outside the band) - additivity fails, the raw two-regime reading
of S2365/S2367 stands, no correction publishes. PRICE: probe. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: additivity control m13')
    raise SystemExit(0)
import torch
...
OUT=PT+'ko_m13_corner_results.json'
...
def main():
    ...
    if SEL.get('qk_r'):
        ...
        for _li in range(2,10):
            ...
            for _hd in _heads:
                _fac=[]
                _idx=torch.cat([torch.arange(96),torch.arange(120,128)])
...
if __name__=='__main__':
    ...
    _hmu=m.transformer.h[13].mlp.register_forward_hook(_mucap)
    with torch.no_grad():
    ...
    m.transformer.h[13].mlp.register_forward_hook(_ablh)
    print('m13 mean captured over FW; gated ablation hook armed',flush=True)
    main()
    ...
    _own=sorted((r['member_absdce']-_BL.get(r['tag'],0))/max(r['ref'],1e-9) for r in rows if _topK.get(r['tag'])=='m13')
    _omed=_own[len(_own)//2]
    _MR=json.load(open(PT+'removal_matrix_results.json'))['matrix']['m13']
    ...
    pb=_rhoK>=0.85
    pc=0.08<=agg<=0.5
    if abs(agg-0.0553)<1e-4 or abs(agg-0.0676)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: all-last8 map inert')
