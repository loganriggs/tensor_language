#!/usr/bin/env python3
"""Prospective v11 construction/lexicon bank for five-head source confirmation."""
from __future__ import annotations

from typing import Any, Mapping, Sequence
import circuit_candidate_tense_auxiliary_is_was_fresh_lexicon_v10 as v10

SCHEMA, SPLIT, GROUPS = v10.SCHEMA, v10.SPLIT, 16
SEED = 20261103
TASK_ID, TASK_SPEC, READOUT = v10.TASK_ID, v10.TASK_SPEC, v10.READOUT
canonical_sha256, CandidateBankError = v10.canonical_sha256, v10.CandidateBankError
_AGENTS = ("actuary", "anthropologist", "apothecary", "arbitrator", "archaeologist",
    "bailiff", "biochemist", "calligrapher", "cardiologist", "ceramicist", "climatologist",
    "conservator", "dramaturge", "epidemiologist", "ethnographer", "geographer")
...
def _panel(group_number: int) -> list[dict[str, Any]]:
    ...
    group_id = f"FIT:{canonical_sha256([SCHEMA, TASK_ID, 'different_readout_v11_source_confirm', SEED, group_number])[:24]}"
    ...
    common_no_vocab = dict(seed=SEED, task_id=TASK_ID, group_number=group_number,
        ...
        adjective="different_readout_v11_source_confirm", object_name="interval_phase", spec=TASK_SPEC)
    common = dict(common_no_vocab, vocabulary=READOUT)
    return [
        v10.v9.v8.v1.builder._row(**common, transform_id="A1",
            construction_id="accordingly_throughout_this_that_interval_auxiliary_v11",
        ...
        v10.v9.v8.v1.builder._row(**common, transform_id="A2",
            construction_id="bulletin_during_this_that_phase_auxiliary_v11",
            direction_id=direction, matched_suffix=f" phase, the {agent}",
            base_text=embedded(base_present), donor_text=embedded(donor_present),
            base_answer=answer(base_present), donor_answer=answer(donor_present), sentence_types=sentence_types),
        v10.v9.v8.v1.builder._row(**common, transform_id="P",
            construction_id="accordingly_temporal_interval_paraphrase_auxiliary_v11",
        ...
        v10.v9.v8.v1.builder._row(**common_no_vocab, transform_id="C",
            **v10.v9.v8.v1.canonical.row_kwargs(group_number, forward)),
    ]
...
def _validate(rows: Sequence[Mapping[str, object]]) -> str:
    ...
    if materialized != _build():
        raise CandidateBankError("rows differ from sealed v11 authority")
    try:
        digest = v10.v9.v8.v1.battery.validate_rows(TASK_SPEC, materialized, required_phases=(SPLIT,))
    except v10.v9.v8.v1.battery.BatteryContractError as error:
        raise CandidateBankError(str(error)) from error
    old_modules = (v10.v9.v8.v1, v10.v9.v8.v2, v10.v9.v8.v3, v10.v9.v8.v4,
                   v10.v9.v8.v5, v10.v9.v8.v6, v10.v9.v8.v7, v10.v9.v8, v10.v9, v10)
    ...
    if (len(materialized) != 64 or len(ids) != 64 or ids & old_ids or set(_AGENTS) & old_agents
        ...
        raise CandidateBankError("v11 count, uniqueness, novelty, or construction check failed")
    return digest
