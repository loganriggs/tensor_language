...
"""V4_MLP_RESPONSE_FOLD_V1_PREREGISTRATION.md; unchanged oracle source edits."""
import os,sys,json,time,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';PLAN=dict(prefix=12,native_suffix=16,joined_suffix=24)
def main():
 ...
 for context in contexts:
  ...
  for family in dict.fromkeys(families):
   ...
   for boundary,estimate in joins.items():
    ...
    records.append(dict(panel=context['panel'],family=family,boundary=boundary,remaining_blocks=6 if boundary in [10.5,10.6,10.7] else 17-boundary,first_mlp=boundary in [10.5,10.6,10.7],errors=errors.tolist(),weak_norm=float(weak),passed=bool(errors[0]<=.1 and errors[1:].max()<=.05)))
 instrument=counts==PLAN and max(replays)<=1e-8 and max(field_errors)<=.001 and max(local_errors)<=1e-8
 passing=[boundary for boundary in [10,10.5,10.6,10.7,11,17] if all(r['passed'] for r in records if r['boundary']==boundary)]
 result=dict(max_local_response_error=max(local_errors),response_values_per_context=sum(v.numel() for v in export['program'].values()),plan=PLAN,counts=counts,max_replay=max(replays),max_field_check=max(field_errors),records=records,passing_boundaries=passing,predictions=dict(pred_a_instrument=instrument,pred_b_compiled_join=instrument and 10.7 in passing,pred_c_precise_join=instrument and max(max(r['errors']) for r in records if r['boundary']==10.7)<=.001),seconds=time.perf_counter()-tic)
 guard_torch_save(export,str(A/'v4_mlp_response_fold_v1_program.pt'))
 guard_torch_save(dict(groups=cores,scope='Conditional mixed edge, no native source or suffix closure'),str(A/'v4_mlp_response_fold_v1_edge_cores.pt'))
