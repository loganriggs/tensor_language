...
# BQLANE: cpu
"""Pronoun number they/he: natural rows with a VERB annotation (v544, fineweb, docs after the first 16234 -- the ones v272 scanned). Same cells and filters as v77 / v78 (a position whose next token is ' they' or ' he',
...
`second_offset`. Purpose (v544): a THIRD natural set, disjoint in documents from v272 (fineweb docs 0-16233) and v273 (pile-10k), for the untested OOD cell of both reader sets (review 42). Original purpose of the lineage: the verb-site units (MLP-6 {2483, 2826, 4131}, MLP-5 {1036, 715}) were undetectable at cue + 1 on the v77 / v78 rows (v248), where the
...
OUT = Path(__file__).resolve().parent.parent / "circuits/followups/pronoun_number_dod_natural_verb_rows_v544.json"

if __name__ == "__main__":
    cues = {tid: number for tid, (w, number) in base.NOUNS.items()}
    labels = {base.THEY: "they", base.HE: "he"}
    M.mine(cues, labels, "fineweb", OUT, "pronoun_number_dod_natural_verb_rows_v544", exclude=base.PRONOUNS, cue_window=base.CUE_WINDOW, context=base.CONTEXT, per_cell=base.PER_CELL,
           doc_budget=base.DOC_BUDGET, second={"plural": VERB_IDS, "singular": VERB_IDS}, skip=16234)   # third natural set: fresh documents (v272 stopped at doc 16233)
