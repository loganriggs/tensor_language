...
"""v321: THE PAIR-GATE TEST. verb_particle separability at TEN prepared, with three TOKEN-PAIR TWINS evaluated in a
family that contains their own same-pair originals.

THE QUESTION. Before authoring any cell I grep ops/ for the readout pair and refuse a pair already in use. That gate
is mine; it has never been measured. The measured account is weaker and different: six fusions in the corpus, six of
them cue-sharing, zero surviving exceptions after v309 -- behaviours fuse when they share a (cue word -> readout
token) MAPPING, not when they share a token. Here each twin has the SAME readout pair as a counted circuit and an
entirely different cue verb on each side:
    verb_particle_up_down_b   cheered/knuckled   vs  verb_particle_up_down   woke/calmed      (` up` / ` down`)
    verb_particle_away_up_b   faded/livened      vs  verb_particle_away_up   shied/sidled     (` away` / ` up`)
    verb_particle_out_down_b  chickened/slowed   vs  verb_particle_out_down  found/settled    (` out` / ` down`)
All three passed all four rows at v319 (n 7, 14, 10; held 0.882, 0.883, 0.913). The fourth twin, out_up_b, missed
row 3 and is EXCLUDED entirely rather than added as a control: a row-3 miss means its dim A1 effect is not
established, so it is not a behaviour whose damage I can interpret, unlike the row-4 misses (up_on, on_out) that sit
in the control set.
THE READING, REGISTERED IN THE v319 DOCSTRING BEFORE EITHER RECEIPT EXISTED AND REPEATED HERE VERBATIM IN SUBSTANCE:
separable twins mean the pair gate is too conservative by roughly the verb inventory, and the corpus's real space is
its 404 distinct cue->token mappings (73 already shared) rather than the 15 unordered token pairs over the six
particle tokens this model can produce -- of which nine are used, four have failed capability, two collide with other
families, and exactly ONE is fresh. Fused twins mean the gate is measured rather than assumed, the readout pair is
the identity of the behaviour, and I stop authoring against spent pairs in every family.
WHY THIS IS A FAIR TEST AND NOT A RIGGED ONE. The twins share the readout pair, the frame, the P control, the cue
CLASS and the answer position with their originals; they differ only in the cue words. That is the cleanest available
separation of "shares a token" from "shares a cue -> token mapping", and it is a test the cue-overlap account can
LOSE: if the account were wrong and the token pair were the identity, these three are exactly the cells that should
fuse. The own-arm leak table from v317 does not prejudge it -- I read it first and it came out at P(|leak| shared >
|leak| disjoint) = 0.62 over 209 comparisons, i.e. raw collateral does not track shared tokens, and I registered no
bar on it.
COST. Ten prepared. v317 ran seven in 467 s with zero errors; the measured curve (s/member = 19 + 9.3 x family_size)
puts ten at ~112 s/member, ~19 min. That is well under the 49-to-72-member sizes where verb_preposition lost members
to CUDA OOM, and this rung carries the crash-verdict fix, so an OOM would record `separable: None` and could never be
read as a fusion.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
    pred_a_new_separable   at least k_new = 2 of the 3 twins are separable at the registered cross bar. Prior is set
                           BELOW the family's 5-for-5 record because these three are the hardest members the family
                           has had: each shares its whole readout pair with a sibling.                    prior 60%
    pred_b_counted_kept    at least k_kept = 4 of the 5 re-checked counted members remain separable. The one to watch
                           is each twin's ORIGINAL: if the pair is the identity, the fusion shows up as the original
                           losing separability against its twin, not only as the twin failing.            prior 75%
...
COUNTING. Each separable twin is +1 on 131. A fused twin is +0 and settles the gate. I will report the own-arm leak
between each twin and its original next to the fam-arm number, because a twin that is separable only after the
controls are added is a different fact from a twin that never touched its original, and v317 showed this family's
own-arm leaks run 0.11 to 0.47.
BARS = {"cross_max": 0.05, "keep": 0.8, "ext_floor": 0.5, "c_ub_max": 0.01, "k_d": 4, "k_new": 2, "k_kept": 4, "instr_tol": 0.03}
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v321_result.json"
...
FAMILIES = {
    "verb_particle": ["verb_particle_up_down", "verb_particle_out_down", "verb_particle_out_up", "verb_particle_up_on", "verb_particle_away_up", "verb_particle_down_in", "verb_particle_on_out", "verb_particle_up_down_b", "verb_particle_away_up_b", "verb_particle_out_down_b"],
}
EVAL = ("verb_particle_up_down_b", "verb_particle_away_up_b", "verb_particle_out_down_b", "verb_particle_up_down", "verb_particle_out_down", "verb_particle_out_up", "verb_particle_away_up", "verb_particle_down_in", )
COUNTED = ("verb_particle_up_down", "verb_particle_out_down", "verb_particle_out_up", "verb_particle_away_up", "verb_particle_down_in", )
...
def _parent_cdas(n):
    ...
    for path in (V319, V315, V311, V257, V255, V253, V251, V249, V247, V245, V243, V241, V239, V237, V235, V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v321", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V321_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V321_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    evaluate = [n for n in members if n in EVAL and (not smoke or n in os.environ.get("V321_SMOKE_NAMES", "verb_preposition_at_to,verb_preposition").split(","))]
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v321", "candidate_id": "corpus.unit_family_separability_spec_v321",
