#!/usr/bin/env python3
# BQGATE: six frozen predictions; arms fixed; readers from v131; random control seeded; one-cue rows by rule.
"""Tier-5: the two routes into the CUE key -- the token embedding itself and the layer writes -- do they add?

v136 = v135 corrected. v135's three cue-key arms ran `capture_with_clamp` on B/D (semantic_positions = t) with
caches captured on Bc/Dc (positions = cue), so the writes were clamped at t and every cue number was void
(v134 cross-check: embedding-only 0.335/0.228/0.132 vs v134's 1 - all_below 0.323/0.389/0.663 on the same rows).
Here the cue arms run on Bc/Dc, and pred_f registers the cross-check against the v134 receipt on disk.
...
Registered before the run:
  ...
  pred_f_matches_v134 |keep_emb(cue) - (1 - all_below removed in v134's receipt)| <= 0.02 on 3/3 (same rows,
                      same clamp, different code path -- the check v135 lacked)
...
Smoke: V136_SMOKE=<out.json>, V136_SMOKE_SET=<set> -> CPU, 4 rows.
"""
...
OUT = ROOT / "circuits/followups/unit_tier5_cue_two_routes_v136_result.json"
FAR_GAP = 3
READERS = dict(v131.READERS)
INSTR_MIN, ADD_TOL, WRITES_MAJ, EMB_MAJ, COS_MIN, CTRL_MAX = 0.9, 0.15, 0.5, 0.4, 0.7, 0.15
V134_TOL = 0.02
V134_RECEIPT = ROOT / "circuits/followups/unit_tier5_cue_value_relay_v134_result.json"
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 200, 6000


def _plan():
    return {"candidate_id": "corpus.unit_tier5_cue_two_routes_v136", "behaviours": 3,
...
def main() -> None:
    ...
    smoke = os.environ.get("V136_SMOKE")
    ...
    sets = dict(READERS)
    if smoke:
        pick = os.environ.get("V136_SMOKE_SET") or "lexical_number_pp"
    ...
    # cross-check against v134's all_below arm (same rows, same clamp, older code path); smoke rows differ -> skipped there
    v134 = json.loads(V134_RECEIPT.read_text())["behaviours"] if V134_RECEIPT.exists() and not smoke else {}
    for n in ran:
        ref = v134.get(n, {}).get("arms", {}).get("all_below")
        R[n]["arms"]["v134_keep_emb"] = round(1 - ref["removed_cue"], 3) if ref else None
    f = [n for n in ran if v(n, "v134_keep_emb") != -9 and abs(v(n, "cue_embedding_only") - v(n, "v134_keep_emb")) <= V134_TOL]
    full = len(READERS)
    predictions = {"pred_a_instrument": len(a) == full, "pred_b_routes_add": len(b) == full, "pred_c_reader_type": len(c) == full,
                   "pred_d_same_axis": len(d) == full, "pred_e_control": len(e) == full, "pred_f_matches_v134": len(f) == full}
    result = {"predictions": predictions, "schema": "unit_tier5_cue_two_routes_v136", "candidate_id": "corpus.unit_tier5_cue_two_routes_v136",
              "bars": {"instr_min": INSTR_MIN, "add_tol": ADD_TOL, "writes_maj": WRITES_MAJ, "emb_maj": EMB_MAJ, "cos_min": COS_MIN, "ctrl_max": CTRL_MAX, "v134_tol": V134_TOL},
              "counts": {"ran": len(ran), "instrument": len(a), "routes_add": len(b), "reader_type": len(c), "same_axis": len(d), "control": len(e), "matches_v134": len(f)},
