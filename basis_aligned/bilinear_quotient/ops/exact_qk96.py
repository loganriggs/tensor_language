"""RANK-96 (rung 203): the pattern curve's last point before exactness.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. r96 = 3/4 of full head rank (at r=128
the truncation is exact and the config IS the real model, trivially). Curve so far at the clean config:
r32 0.149 / r64 0.085. Does the certificate count keep tracking rank toward majority?

REGISTERED PREDICTIONS (anchor: rung-201):
  (a) census <= 0.055.
  (b) valid >= 12 of 62.
  (c) census >= 0.005; inert tripwire.
NULL: an interaction floor emerges (census >= 0.07; certificates stall). PRICE: patterns 72.7M (25%
compression only - the curve's science end, not a config proposal). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: rank-96 curve point')
    raise SystemExit(0)
import torch
...
OUT=PT+'exact_qk96_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=96; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    print(f"(a) census {agg:+.4f} <= 0.055: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 12; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) census >= 0.005; era rho {rho:.4f}: {'HELD' if pc else 'FAILED'}")
