...
def load_bound():
 b=json.loads(BINDING.read_text());paths={'six_rows':SIX_ROWS,'seven_rows':SEVEN_ROWS,'six_result':SIX_RESULT,'seven_result':SEVEN_RESULT,'preregistration':PREREG};assert all(digest(paths[k])==v for k,v in b['files'].items());assert b['sites']==list(SITES) and b['candidates']==list(CANDIDATES) and b['bars']==BARS and b['price']==PRICE
 six=json.loads(SIX_ROWS.read_text());seven=json.loads(SEVEN_ROWS.read_text());sr=json.loads(SIX_RESULT.read_text());tr=json.loads(SEVEN_RESULT.read_text());assert sr['predictions']['pred_b_exact_joint_parent_live'] and tr['predictions']['pred_b_exact_joint_parent_live'];return {'sixth':six,'seventh':seven}
def plan():
 panels=load_bound();return {'schema':'bracket_suffix_finite_bilinear_program_v1_plan','model_loaded':False,'gpu_accessed':False,'queue_touched':False,'panels':{k:v['endpoint_count'] for k,v in panels.items()},'sites':list(SITES),'candidate_order':list(CANDIDATES),'bars':BARS,'price':PRICE,'predicates':list(PREDICTION_REGISTRY)}
