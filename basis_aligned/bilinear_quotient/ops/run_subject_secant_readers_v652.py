...
"""Width8 exact finite-secant reader basis with two fixed readout anchors.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_joint_chain_v652_result.json'
...
def main():
    plan=dict(prefix_calls=12,suffix_calls=16,calibration_rows=128,evaluation_rows=48,width=8,
              additional_endpoint_chain_batches=6,basis_selection='two fixed answer readers plus six orthogonal calibration finite-secant reader SVD vectors',controls=['dense','random_seed647'],
    ...
    for l in range(7):
        deltas=torch.cat([readers[l] for readers in reader_samples])
    ...
    a=max(checks)<=1e-4 and max(absolute)<=1e-4 and max(reader_checks)<=1e-4 and counts==dict(prefix_calls=12,suffix_calls=16)
    ...
    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,
        ...
        max_reader_closure_absolute_error=max(reader_checks),max_local_relative_error=max(checks),max_local_absolute_error=max(absolute),fixed_values=values,
        ...
        scope='Exact finite-secant reader width8 control, same calibration/evaluation data; final projection is oracle. Data-informed residual basis, no native circuit adoption.')
