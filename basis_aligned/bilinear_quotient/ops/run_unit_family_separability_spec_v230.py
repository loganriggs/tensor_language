...
"""v230: within-family separability for v233's two four-row passes that have families (modal_ellipsis_should_might, category_selection_because_of)
plus the re-check of the counted modal_ellipsis singleton (v212/v216/v220/v222 rule, standing until board proposal (v) is answered).
duration_point_since_for is family-less at this point (v235's within_by, if it passes, opens the question later) and counts on v233 alone.

v233 (sixteenth spec batch): 3/8 four-row, preds 4/6 (pred_a 3 < 4, pred_b row 2 on 4/8). Families here:
  modal_ellipsis = modal_ellipsis_can_must (counted at v231 as a singleton; 21 units on the late hub 13:08/09:01/15:05/14:00/08:01) +
    modal_ellipsis_should_might (18 units 13:08/14:00/15:05/08:01/09:01/09:04; dim 1.117, cdas 1.163, C 0.0067, cext 1.006). The two share
    at least five units and the same frame -- the strongest fusion candidate of the day; a fused pair counts ONCE (v216 aux-copy precedent),
    and if should_might leaks into can_must one way only, can_must's count stands and should_might adds nothing.
  category_selection = category_selection_because_of (10 units on EARLY heads 03:06/07:08/05:08/04:01/05:03/02:03; dim 0.314, cdas 0.341,
    C 0.0025, cext 1.002) + category_selection_during_while (rows 0111, 4 units, control) + category_selection_despite_although (rows 0101,
    16 units, control). All three read the same the/it slot from a connective's category; the siblings are the natural leak targets.
Recipe as v198/.../v228: per evaluated member, arm `own` (own C_fit control) and arm `fam` (own C_fit + EVERY sibling's A1 FIT prep,
control_weight 30 x len(controls)); evaluated on the member's held A1 (extraction), each sibling's held A1 (|CE| leak) and held C.
Units frozen from the parent receipts (v233 for the new members, v231 for can_must).
...
EVAL = 2 new members + 1 counted re-check (3 fits x 2 arms, small control sets: ~5 min).
...
fails is RETRACTED (-1). 61 (after since_for) -> up to 63.

REGISTERED BEFORE THE RUN (each coded predicate is the sentence here; bars in BARS):
    pred_a_new_separable   at least 1 of the 2 new members is separable (k_new). because_of is the expected pass (its siblings are
                           weaker sets); should_might is the expected fusion with can_must.                              prior 65%
    pred_b_counted_kept    the 1 re-checked counted member (modal_ellipsis_can_must) remains separable (k_kept = 1; NOT 0 -- a bar
                           of 0 could not fail).                                                                          prior 55%
    pred_c_row4_kept       fam-arm own-C UB975 <= 0.01 on at least 2 of the 3 evaluated members (k_d; v228 15/22).       prior 55%
...
Smoke: V230_SMOKE=<out.json> (CPU, V230_SMOKE_ROWS=4; all members are prepared as controls, V230_SMOKE_NAMES=category_selection_because_of,modal_ellipsis_can_must
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v230_result.json"
...
FAMILIES = {
    "modal_ellipsis": ["modal_ellipsis_can_must", "modal_ellipsis_should_might"],
    "category_selection": ["category_selection_during_while", "category_selection_despite_although", "category_selection_because_of"],
}
EVAL = ("modal_ellipsis_should_might", "category_selection_because_of", "modal_ellipsis_can_must", )
COUNTED = ("modal_ellipsis_can_must", )
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES, **v213.NAMES, **v215.NAMES, **v217.NAMES, **v219.NAMES, **v221.NAMES, **v223.NAMES, **v225.NAMES, **v227.NAMES, **v229.NAMES, **v231.NAMES, **v233.NAMES}


def _parent_cdas(n):
    # the parent battery receipt that produced this member's units (each behaviour name is unique across batteries)
    for path in (V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
BARS = {"cross_max": 0.05, "keep": 0.80, "ext_floor": 0.50, "c_ub_max": 0.01, "k_d": 2, "k_new": 1, "k_kept": 1, "instr_tol": 0.03}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 20000, 640000


def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v230", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V230_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V230_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v230", "candidate_id": "corpus.unit_family_separability_spec_v230",
              "separable_counts": nsep, "families": FAMILIES, "summary": summary, "members": R, "bars": BARS,
              "recipe": {"lambda": LAM, "steps": steps, "lr": LR, "complement_weight": CW, "rank": 1, "seed": 0, "units_source": "v197 ... v233 battery receipts (later receipts override earlier ones)", "evaluated": list(EVAL),
