...
"""v583: the refuting test for the amid pattern -- four behaviours, or one counted four times?

THE OBSERVATION THIS EXISTS TO ATTACK. Every countable behaviour the backlog has produced shares ONE readout token.
v575 lifted eight verb_preposition mappings and exactly one cleared all four tier rows: at_amid. v581 lifted eight
more and three cleared them: of_amid, on_amid, to_amid. Four of four countable behaviours answer with the same
low-frequency preposition, while sixteen mappings with commoner readouts produced none. The tempting reading is that
what survives all four rows is selected by the READOUT TOKEN rather than by the cue verb, and I would be booking four
new circuits on it.
WHY I AM TESTING INSTEAD OF BOOKING. The corpus record says a behaviour's identity is its cue -> token MAPPING, and
that margins degrade against MAPPING-SHARING siblings. Four cells that share a readout token and differ only in the
cue verb are exactly that configuration, so the competing reading is that they are not four behaviours at all: one
direction, counted four times. Booking four duplicates would be worse than booking none, because the controlling
goal is hundreds of NONDUPLICATED circuits and a duplicate is invisible once it is in the count.
HOW THE TEST CAN REFUTE IT. All four amid cells are placed in the SAME family list as six counted verb_preposition
members, so the separability metric measures each amid cell's leak against the other three amid cells as well as
against the counted six. If they encode one direction, their mutual leaks exceed cross_max = 0.05 and `separable`
goes false for them; if they are four behaviours, all four clear it while sitting beside each other. The counted six
are re-evaluated in the same run so a null cannot be blamed on the family, which is the control row I left
unmeasured in v577 and had to re-run as v579.
NOTHING IS COUNTED BY THIS FILE whatever it returns. The count stays 139.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
  pred_a_new_separable ALL FOUR amid behaviours are separable at cross_max = 0.05 while sitting in one family --
                       k_new = 4, not the inherited 1, so a single fusion breaks it.                  prior 45%
  pred_b_counted_kept  at least k_kept = 5 of the six counted members stay separable in the same run, so a null on
                       the amid four cannot be blamed on the family having stopped separating.        prior 85%
  pred_c_row4_kept     at least k_d = 2 members keep a C upper bound at or below c_ub_max = 0.01, so the control row
                       is still doing work in this batch.                                             prior 85%
  pred_d_instrument    every EVAL member reproduces its parent battery extraction within instr_tol = 0.03 -- v575
                       for at_amid, v581 for the other three.                                         prior 85%
HOW IT READS. a TRUE with b: the four are genuinely distinct despite sharing a readout token, the amid pattern is
about which mappings SURVIVE the rows rather than about them being one direction, and four candidates stand. a FALSE
with b true: they fuse, the four-of-four pattern is one circuit seen four times, and the backlog yield I just revised
upward to one in four collapses back toward one in sixteen -- the more valuable answer, and the reason this runs
before anything else is booked. a partly false: whichever amid cells fuse name the pairs that share a direction, which
is a finer result than either.
SCOPE. Four candidates, six counted controls, one family, one frame.
Smoke: V583_SMOKE=<out.json> (CPU).
"""
...
OUT = ROOT / "circuits/followups/unit_amid_mutual_separability_v583_result.json"
...
FAMILIES = {
    "verb_preposition": ['verb_preposition', 'verb_preposition_insisted', 'verb_preposition_to_at', 'verb_preposition_with_about', 'verb_preposition_of_with', 'verb_preposition_from_for', 'verb_preposition_at_amid', 'verb_preposition_of_amid', 'verb_preposition_on_amid', 'verb_preposition_to_amid'],
}
COUNTED_LIST = ['verb_preposition', 'verb_preposition_insisted', 'verb_preposition_to_at', 'verb_preposition_with_about', 'verb_preposition_of_with', 'verb_preposition_from_for']
COUNTED = tuple(COUNTED_LIST)
EVAL = tuple(COUNTED_LIST) + tuple(['verb_preposition_at_amid', 'verb_preposition_of_amid', 'verb_preposition_on_amid', 'verb_preposition_to_amid'])
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES, **v213.NAMES, **v215.NAMES, **v217.NAMES, **v219.NAMES, **v221.NAMES, **v223.NAMES, **v225.NAMES, **v227.NAMES, **v229.NAMES, **v231.NAMES, **v233.NAMES, **v235.NAMES, **v237.NAMES, **v239.NAMES, **v241.NAMES, **v243.NAMES, **v245.NAMES, **v247.NAMES, **v249.NAMES, **v251.NAMES, **v253.NAMES, **v255.NAMES, **v257.NAMES, **v311.NAMES, **v315.NAMES, **v319.NAMES, **v323.NAMES, **v327.NAMES, **v349.NAMES, **v293.NAMES, **v353.NAMES, **v263.NAMES, **v265.NAMES, **v357.NAMES, **v339.NAMES, **v343.NAMES, **v361.NAMES, **v365.NAMES, **v575.NAMES, **v581.NAMES}


def _parent_cdas(n):
    # the parent battery receipt that produced this member's units (each behaviour name is unique across batteries)
    for path in (V581R, V575R, V365, V361, V343, V339, V357, V263, V265, V353, V293, V349, V327, V323, V319, V315, V311, V257, V255, V253, V251, V249, V247, V245, V243, V241, V239, V237, V235, V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
BARS = {"cross_max": 0.05, "keep": 0.8, "k_kept": 5, "ext_floor": 0.5, "c_ub_max": 0.01, "k_d": 2, "k_new": 4, "instr_tol": 0.03}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 20000, 640000


def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_amid_mutual_separability_v583", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V583_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V583_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_amid_mutual_separability_v583", "candidate_id": "corpus.unit_amid_mutual_separability_v583",
