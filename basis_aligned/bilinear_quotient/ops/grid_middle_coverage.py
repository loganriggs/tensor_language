# THE COVERAGE x RANK GRID, MIDDLE POINT  -- completing a frontier that has two rows and needs three.
#
# §1852 found the position-wise frontier is two-dimensional and that its two axes INTERACT: tripling the
# covered set buys -0.106 all-position nats at full rank and COSTS +0.061 at rank 4. It also found the
# object §1848 certified -- full rank at 5,419 covered types, 230.1M reals, CE 6.01167 -- is
# **Pareto-dominated** by rank-256 at 16,110 types (164.5M reals, 5.98851): cheaper and better.
#
# That domination rests on TWO coverage rows measured from TWO DIFFERENT FILES (`n96_skip80` at §1849,
# `n480_skip80` at §1852), so the coverage contrast is confounded with whatever differs between the two
# streams. This run takes the middle point from a single stream -- the first 192 rows of `n480_skip80` --
# so that this row and §1852's share a file and differ only in how much of it was used. Three rows from
# one stream is a grid; two rows from two files is a comparison with a confound in it.
#
# Nothing here re-opens §1848: full-rank attainment is exact at every coverage measured so far and is
# checked again as pred_a. What is open is the SHAPE of the frontier between the rows.
#
# ROLES. All three. Ranks full/256/64/16/4. DISCOVERY ONLY.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39,
# anchors named with their BUILD per LESSONS 53's addendum. NOTE: no control here anchors a figure
# measured on a DIFFERENT covered population -- the defect §1851's pred_d and §1852's pred_d both hit,
# the second time by copying the first script without re-reading its controls.
#   pred_a ATTAINMENT HOLDS AT THE MIDDLE COVERAGE TOO: full-rank covered CE equals the ceiling to under
#          0.001 nats on all three roles. Third independent coverage. If FALSE, §1848 and §1851 were both
#          coverage-lucky and §1765's derivation is what needs re-examining.
#   pred_b AND THE COVERAGE IS GENUINELY INTERMEDIATE: the covered set from 192 rows lies strictly
#          between §1834's 5,419 and §1852's 16,110. The known-answer check that this is a middle point
#          (LESSON 46) -- without it the grid has two rows, not three.
#   pred_c AND THE LADDER SHIFT IS MONOTONE IN COVERAGE: at every finite rank, the CE above the ceiling
#          at this coverage lies between §1849's 5,419-type value and §1852's PUBLISHED 16,110-type value
#          (+0.09252 / +0.27839 / +0.48135 / +0.82377 on skip7000). If FALSE the ladder does not move
#          monotonically with coverage, the two-row reading in §1852 was an accident of which two, and no
#          smooth frontier can be fitted through the grid.
#   pred_d CONTROLS: coverage lies strictly between the two published values; the ceiling is finite and
#          above live on every role; the ladder is monotone in rank at this coverage as at both others.
...
OUT = PT + 'ops/grid_middle_coverage_results.json'
...
S1852_LADDER_16110 = {'256': {'skip7000': 0.09252, 'skip11000': 0.09844, 'skip1200': 0.08049},
                      '64': {'skip7000': 0.27839, 'skip11000': 0.29741, 'skip1200': 0.25327},
                      '16': {'skip7000': 0.48135, 'skip11000': 0.51846, 'skip1200': 0.45230},
                      '4': {'skip7000': 0.82377, 'skip11000': 0.88141, 'skip1200': 0.81035}}
...
FITN = 192                 # take the FIRST 192 rows: same stream as §1852, less of it
NCOV_HIGH = 16110          # §1852's PUBLISHED coverage from all 480 rows of this stream
H = m.transformer.h
NCOV = 5419
...
def main():
    t0 = time.time()
    fit = load(FIT_ROWS)[:FITN]
    T = fit.shape[1] - 1
    full_seen = torch.zeros(V, dtype=torch.bool)
    full_seen[fit[:, :T].reshape(-1).long()] = True
    NFULL = int(full_seen.sum())
    assert NCOV < NFULL < NCOV_HIGH, f'coverage {NFULL} not strictly between {NCOV} and {NCOV_HIGH}'
    ...
    print(f'GRID MIDDLE COVERAGE | 192 of the n480 fit rows, ranks full/256/64/16/4 | '
    ...
    mono_rank = all(above[lad[i]][e] <= above[lad[i - 1]][e] + 1e-9
                    for e in roles2 for i in range(1, len(lad)))
    pc = all(S1849_LADDER_5419[l2][e] - 1e-9 <= above[l2][e] <= S1852_LADDER_16110[l2][e] + 1e-9
             for l2 in lad[:-1] for e in roles2)
    # NO anchor to a figure measured on a different covered population -- see the header note.
    pd = (NCOV < NFULL < NCOV_HIGH
    ...
    print(f'\n  LADDER at {NFULL} types, bracketed by §1849 ({NCOV}) and §1852 ({NCOV_HIGH}):',
          flush=True)
    for l2 in lad[::-1]:
        if l2 == 'full':
            ...
            print(f'    rank {l2:5s} ' + '  '.join(
                f'{e} §1849 +{S1849_LADDER_5419[l2][e]:.5f} < HERE +{above[l2][e]:.5f} < '
                f'§1852 +{S1852_LADDER_16110[l2][e]:.5f}' for e in roles2), flush=True)
    print(f'  and the ladder shift is MONOTONE in coverage (between §1849 and §1852) -> {pc}',
          flush=True)
    print(f'    monotone in RANK at this coverage: {mono_rank}', flush=True)
    print(f'  ceiling/live §1768, full-rank all-position §1811 -> control {pd}', flush=True)

    json.dump({'run': 'grid_middle_coverage', 'fit_rows': FITN, 'n_covered': NFULL, 'ranks': [str(r) for r in TRANKS],
               ...
               'predictions': {'pred_a_attainment_survives_coverage': bool(pa),
                               'pred_b_coverage_is_intermediate': bool(pb),
                               'pred_c_shift_monotone_in_coverage': bool(pc),
