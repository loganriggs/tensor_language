...
"""v367: WHAT CARRIES A FRAME? Seven copies of one mapping pair, and the two recombined frames that split the
collision v363 found.

WHERE THIS COMES FROM. v363 prepared five frames of woke -> ` up` / calmed -> ` down` and found a pair-specific
resolution limit: the frame-D copy separated from every other copy (largest fam-arm leak 0.0064) while the frame-E
copy FUSED with the frame-C copy at 0.0773, staying clear of A (0.0242), B (0.0008) and D (0.0112). Own-arm leaks
between copies ran 1.20 to 1.82 throughout, so both the collision and the separations are real.
    frame C   `It turned out the {agent} {verb}, apparently,`     counted
    frame E   `Word spread that the {agent} {verb}, evidently,`   fused with C, not counted
    frame G   `It turned out the {agent} {verb}, evidently,`      C's clause + E's parenthetical    NEW
    frame H   `Word spread that the {agent} {verb}, apparently,`  E's clause + C's parenthetical    NEW
All seven copies are prepared here -- A, B, C, D, E as controls, G and H evaluated -- so every possible collision is
visible in one receipt. v347 measured that control-set size is inert and only relatives matter; all seven of these
ARE relatives, so seven prepared is both affordable and necessary.
THE THREE OUTCOMES ARE CODED, NOT NARRATED. pred_b and pred_e are mutually exclusive in practice and both can fail,
which is the third outcome:
  * the PARENTHETICAL carries the frame -> G collides with E and H collides with C (pred_e true)
  * the MATRIX CLAUSE carries it        -> G collides with C and H collides with E (pred_b true)
  * neither                            -> both false, and C and E collide as wholes rather than through a part
I am not predicting which. The post-hoc story after v363 was that C and E are both report frames with an evidential
-ly adverb while B and D are speaker-stance frames; that is exactly the kind of similarity story my ledger records me
getting wrong, so it is written here as the thing being tested rather than as an expectation, and both priors are set
below even.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
    pred_a_new_separable   at least k_new = 1 of the 2 new members is separable at the registered cross bar. Under
                           the "parenthetical carries it" outcome G fuses with E and H fuses with C, so BOTH could
                           fail; under "clause carries it" the same; under "neither" both pass.           prior 55%
    pred_b_clause_carries  the matrix clause carries the frame identity: the SMALLER of the two same-clause fam-arm
                           leaks clears the cross bar, min(|G -> C|, |H -> E|) >= cross_max = 0.05, where G shares C's
                           clause and H shares E's. Worked example: leaks of 0.08 and 0.11 give a minimum of 0.08
                           >= 0.05, TRUE; 0.08 and 0.02 give 0.02, FALSE, because one collision is not the claim --
                           the claim is that sharing a clause is what collides, and it has to hold on both.
                                                                                                          prior 35%
    pred_c_row4_kept       fam-arm own-C UB975 <= 0.01 on at least k_d = 2 of the 2 evaluated members.    prior 65%
    pred_d_instrument      each evaluated member's own-arm extraction is within 0.03 of its v365 cdas
                           extraction_held. Fails automatically on an error receipt.                      prior 90%
    pred_e_adverb_carries  the PARENTHETICAL carries the frame identity: min(|G -> E|, |H -> C|) >= cross_max = 0.05,
                           where G shares E's parenthetical and H shares C's. Same form as pred_b on the other
                           diagonal, so the two are directly comparable and the receipt reports both numbers whichever
                           way it falls. If this one holds, a four-token parenthetical is carrying more of the
                           behaviour's identity than the entire matrix clause, which would be the most surprising
                           thing the frame axis has produced.                                             prior 35%
COUNTING. Each separable new member is +1 on 153. A fused one is +0 and localises the collision, which is the point.
BARS = {"cross_max": 0.05, "keep": 0.8, "ext_floor": 0.5, "c_ub_max": 0.01, "k_d": 2, "k_new": 1, "instr_tol": 0.03}
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v367_result.json"
...
FAMILIES = {
    "frame_capacity_up_down": ["verb_particle_fd_up_down", "verb_particle_fe_up_down", "verb_particle_up_down", "verb_particle_fb_up_down", "verb_particle_fc_up_down", "verb_particle_fg_up_down", "verb_particle_fh_up_down"],
}
EVAL = ("verb_particle_fg_up_down", "verb_particle_fh_up_down", )
COUNTED = ()
...
def _parent_cdas(n):
    ...
    for path in (V365, V361, V343, V339, V357, V263, V265, V353, V293, V349, V327, V323, V319, V315, V311, V257, V255, V253, V251, V249, V247, V245, V243, V241, V239, V237, V235, V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v367", "members": n, "families": list(FAMILIES),
...
def PREDS(R):
    ...
    def leak(a_, b_):
        if a_ not in good:
            return None
        s = good[a_]["arms"]["fam"]["siblings"]
        return abs(s[b_]) if b_ in s else None

    same_clause = [leak(G, C), leak(H, E)]      # G shares C's matrix clause, H shares E's
    same_adverb = [leak(G, E), leak(H, C)]      # G shares E's parenthetical, H shares C's
    b = all(x is not None for x in same_clause) and min(same_clause) >= B["cross_max"]
    e = all(x is not None for x in same_adverb) and min(same_adverb) >= B["cross_max"]
    return {"pred_a_new_separable": bool(a), "pred_b_clause_carries": bool(b), "pred_c_row4_kept": bool(c),
            "pred_d_instrument": bool(d), "pred_e_adverb_carries": bool(e)}


def main() -> None:
    if os.environ.get("BQLIB_DRYRUN") or os.environ.get("BQLIB_NO_MODEL"):
        print(json.dumps(_plan(), indent=2, sort_keys=True)); return
    smoke = os.environ.get("V367_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V367_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v367", "candidate_id": "corpus.unit_family_separability_spec_v367",
