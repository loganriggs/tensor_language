#!/usr/bin/env python
"""early_block_read_cost_map_probe -- the k = 768 READ cost of program v2 mapped per early block: block l's attention and MLP read
through the block frame F_l at 768 with every other site untouched, l = 0..7; plus the bus reads alone. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_block_read_costs_are_additive pred_c_read_cost_is_concentrated
#                     pred_d_bus_reads_cheap_at_768 pred_e_no_single_block_is_the_cliff
...
Preregistration: polynomial_causal/EARLY_BLOCK_READ_COST_MAP_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/early_block_read_cost_map_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("early_block_read_cost_map_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "EARLY_BLOCK_READ_COST_MAP_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "nine_frame_union_program_probe_results.json"   # §2769
OUT = ROOT / "early_block_read_cost_map_probe_results.json"
HASHES = {PREREG: "17c7719f58deb7cf93c16ee96e5d97774ecbcf309e55a329eabf2253747d13c2", PRIOR: "6e807004d834e79b92a09ea3db2c9fb8ade775c5aed9cd901d7012db466aaefc",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "early_block_read_cost_map_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_SPLIT8_1024 = 0.0374; PRIOR_BLOCK8_768 = 0.2253; LAM = 1e-2
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_lo": 0.5, "b_hi": 1.5, "c_min": 0.50, "d_max": 0.050, "e_max": 0.100}
NULLS = {"b_lo": 0.25, "b_hi": 3.0, "c_max": 0.30, "d_min": 0.100, "e_min": 0.150}
...
def main():
    ...
    KM = 768
    def block_reads(k):
        patch = {s_: head(s_, k, FB[s_[1]]) for s_ in OWN8}
        for s_ in SET8:
    ...
    def split8(k):
        ...
        return run(patch)
    def one_block(l, k):
        return run({s_: head(s_, k, FB[l]) for s_ in blocks([l])})
    def bus_only(k):
        return run({s_: head(s_, k, U_8) for s_ in SET8})
    arms = {"SPLIT8_1024": split8(1024), f"BLOCK8_{KM}": block_reads(KM)}
    per_block = {}
    for l in (range(2) if smoke else range(SPLIT)):
        arms[f"ONE_B{l}_{KM}"] = one_block(l, KM); per_block[str(l)] = arms[f"ONE_B{l}_{KM}"]
        log(stage="block", l=l, cost=round(per_block[str(l)], 5))
    arms[f"BUS_{KM}"] = bus_only(KM)
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    tot = sum(per_block.values()); ranked = sorted(per_block.items(), key=lambda kv: -kv[1])
    ratio = (tot + arms[f"BUS_{KM}"]) / arms[f"BLOCK8_{KM}"] if abs(arms[f"BLOCK8_{KM}"]) > 1e-9 else float("inf")
    top2_share = sum(v for _, v in ranked[:2]) / tot if abs(tot) > 1e-9 else float("nan"); mx = ranked[0][1]
    summ = {"sum_blocks": tot, "bus": arms[f"BUS_{KM}"], "block8_joint": arms[f"BLOCK8_{KM}"], "additivity_ratio": ratio, "top2_share": top2_share, "ranked": ranked}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(arms[f"BLOCK8_{KM}"] - PRIOR_BLOCK8_768) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_block_read_costs_are_additive': bool(BARS["b_lo"] <= ratio <= BARS["b_hi"]),
        'pred_c_read_cost_is_concentrated': bool(top2_share >= BARS["c_min"]),
        'pred_d_bus_reads_cheap_at_768': bool(arms[f"BUS_{KM}"] <= BARS["d_max"]),
        'pred_e_no_single_block_is_the_cliff': bool(mx <= BARS["e_max"]),
    }
    nulls = {"b_null_ratio_ge_3_or_le_.25": bool(ratio >= NULLS["b_hi"] or ratio <= NULLS["b_lo"]), "c_null_top2_le_.30": bool(top2_share <= NULLS["c_max"]),
             "d_null_bus_ge_.10": bool(arms[f"BUS_{KM}"] >= NULLS["d_min"]), "e_null_max_ge_.15": bool(mx >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "split": SPLIT, "k_map": KM,
           "program": "block l's attention+MLP read through F_l at 768, all else untouched; BUS_768 = late reads through U_8 alone; BLOCK8_768 = all jointly",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024,
                          "block8_768": arms[f"BLOCK8_{KM}"], "prior_block8_768": PRIOR_BLOCK8_768},
           "ce_added": arms, "per_block_read_cost_768": per_block, "summary": summ,
    ...
    if smoke:
        print(json.dumps({k: out[k] for k in ("preds", "nulls", "instrument", "ce_added", "per_block_read_cost_768", "summary", "price")}, indent=1)); return
    from receipt import dump
    dump(out, OUT)
    print(json.dumps({"preds": preds, "nulls": nulls, "ce_added": arms, "per_block_read_cost_768": per_block, "summary": summ}, indent=1))
