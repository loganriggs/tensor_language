...
def run_batch(model,tokens,edit,embed_c,write_c):
    ...
    for layer in range(10,18):
        ...
        if layer==17:
            ...
            heads17=projected_head_writes(q17,k17,q217,k217,value17,at17.c_proj.weight).double(); rounding17=a.double()-heads17.sum(1); head17_error=float(rounding17.norm()/a.double().norm().clamp_min(1e-30)); captures["attn17_heads"]=heads17; captures["attn17_rounding"]=rounding17
...
def main():
    ...
    term_values=torch.empty(18,48,dtype=torch.float64); head_values=torch.empty(9,48,dtype=torch.float64); rounding_values=torch.empty(48,dtype=torch.float64); final_values=torch.empty(48,dtype=torch.float64); carry_num=carry_den=0.; attention_error_max=head17_output_error=0.; executions=0
    for _,indices in sorted(bound["buckets"].items()):
        for offset in range(0,len(indices),8):
            ...
            head_delta=(edited["writes"]["attn17_heads"][:,:,-1]-native["writes"]["attn17_heads"][:,:,-1]).double().cpu(); head_values[:,selected]=torch.einsum("bhd,bd->hb",head_delta,readers); rounding_delta=(edited["writes"]["attn17_rounding"][:,-1]-native["writes"]["attn17_rounding"][:,-1]).double().cpu(); rounding_values[selected]=(rounding_delta*readers).sum(-1)
    dt=head_values[:,::2]-head_values[:,1::2]; rounding_dt=rounding_values[::2]-rounding_values[1::2]; total17=term_values[NAMES.index("attn17"),::2]-term_values[NAMES.index("attn17"),1::2]; final_delta=final_values[::2]-final_values[1::2]; response_closure=rel(dt.sum(0)+rounding_dt,total17); families=sorted(set(r["family"] for r in rows)); reports={}
    for head in range(9):
        family_reports={}
    ...
    ranking=sorted(range(9),key=lambda h:reports[str(h)]["to_attention17_norm_ratio"],reverse=True); top=ranking[0]; top3_error=rel(sum(dt[h] for h in ranking[:3])+rounding_dt,total17); carry_error=math.sqrt(carry_num/max(carry_den,1e-30)); downstream=json.loads(DOWNSTREAM.read_text()); parent_ratio=downstream["term_reports"]["attn17"]["change_norm_ratio"]; observed_ratio=float(total17.norm()/final_delta.norm().clamp_min(1e-30)); parent_error=abs(observed_ratio-parent_ratio); rounding_ratio=float(rounding_dt.norm()/total17.norm().clamp_min(1e-30)); instrument=executions==12 and carry_error<=1e-6 and attention_error_max<=1e-6 and head17_output_error<=.005 and response_closure<=2e-6 and parent_error<=1e-5 and bool(torch.isfinite(head_values).all())
    ...
    metrics={"carry_source_reconstruction_relative_error":carry_error,"manual_attention9_reconstruction_relative_error":attention_error_max,"attention17_projection_rounding_relative_norm":head17_output_error,"attention17_rounding_response_norm_ratio":rounding_ratio,"attention17_head_plus_rounding_response_partition_relative_error":response_closure,"attention17_change_norm_ratio":observed_ratio,"parent_attention17_ratio_absolute_error":parent_error,"top3_plus_rounding_attention17_replay_relative_error":top3_error,"physical_model_executions":executions,"rows":48,"pairs":24}
