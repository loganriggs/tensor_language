...
"""v252: within-family separability for the two v259 four-row passes under the verb_preposition family, which is now the largest in the corpus:
11 counted members (all separable at v250) plus five three-row controls. Two of those controls are NEW here — verb_preposition_by_for
(stood/longed, 1101, C UB975 0.0167) and verb_preposition_of_by (consisted/abided, 1101, C 0.0105) missed row 4 at v259 and row-4 failures
JOIN the control set (row-2/row-3/row-5 failures do not), so they enter as controls without being scored. v259's third miss,
adjective_preposition_in_to, is a row-2 failure (held 0.80) and joins nothing.

Family here (ALL members prepared as controls; only EVAL evaluated):
  verb_preposition (11 counted after v250: verb_preposition (on/to), insisted (of/on), to_at, with_about, of_with, from_for, on_about, for_at,
    to_with, at_to, from_about; control-only: at_from, on_with, to_about, by_for, of_by)
    + verb_preposition_in_to (resulted/amounted -> in/to; 8 units; held 0.86, dim 0.563, cdas 0.859, C 0.0016, cext 1.033)
    + verb_preposition_over_with (presided/tampered -> over/with; 6 units; held 0.85, dim 1.058, cdas 1.395, C 0.0088, cext 1.024)
The registered question: in_to's ` to` readout is already carried by to_at, to_with and at_to, and its ` in` by nothing counted; over_with's
` with` is carried by with_about, to_with and of_with. v248 answered the ONE-shared-readout case (for_at vs to_at separable at sib 0.005) and
v250 answered the TWO-shared case (at_to shares both its readouts with counted siblings and came out separable at sib 0.006). This rung asks
it at the largest control set yet: 16 controls in the fam arm, where the fitted direction has to stay inert on every one of them at once.
The failure mode to watch is not fusion but the KEEP bar — v250's with_about fell to fam 0.936 (sib 0.040 into to_about), the lowest kept
extraction on any rung so far, and each added control pushes that number down.
Recipe as v198/.../v250: per evaluated member, arm `own` (own C_fit control) and arm `fam` (own C_fit + EVERY sibling's A1 FIT prep,
control_weight 30 x len(controls)); evaluated on the member's held A1 (extraction), each sibling's held A1 (|CE| leak) and held C.
Units frozen from the parent receipts (v259 for the new members; earlier batteries for the counted ones).
...
EVAL = 2 new members + 11 counted re-checks (13 fits x 2 arms, 16 controls each: ~30-35 min).
97 (after v250) -> up to 99 from this rung.

REGISTERED BEFORE THE RUN (each coded predicate is the sentence here; bars in BARS):
    pred_a_new_separable   at least 1 of the 2 new members is separable (k_new). Expected: both (v250 13/13, v248 21/21).      prior 70%
    pred_b_counted_kept    at least 9 of the 11 re-checked counted members remain separable (k_kept; v250 kept 11/11).         prior 65%
    pred_c_row4_kept       fam-arm own-C UB975 <= 0.01 on at least 6 of the 13 evaluated members (k_d; v250 9/13, v248 17/21). prior 60%
...
Priors are not moved by unit overlap (v214/v216/v220) nor by a shared readout token (withdrawn at v224; v248 and v250 both confirmed shared
readouts inside a family do not fuse, including the two-shared case).
Smoke: V252_SMOKE=<out.json> (CPU, V252_SMOKE_ROWS=4; all members are prepared as controls, V252_SMOKE_NAMES = a new member plus a re-check,
so the 16-control arm and the instrument read are both exercised; steps 5).
BARS = {"cross_max": 0.05, "keep": 0.80, "ext_floor": 0.50, "c_ub_max": 0.01, "k_d": 6, "k_new": 1, "k_kept": 9, "instr_tol": 0.03}
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v252_result.json"
...
FAMILIES = {
    "verb_preposition": ["verb_preposition", "verb_preposition_insisted", "verb_preposition_to_at", "verb_preposition_with_about", "verb_preposition_of_with", "verb_preposition_from_for", "verb_preposition_on_about", "verb_preposition_to_about", "verb_preposition_at_from", "verb_preposition_on_with", "verb_preposition_for_at", "verb_preposition_to_with", "verb_preposition_at_to", "verb_preposition_from_about", "verb_preposition_by_for", "verb_preposition_of_by", "verb_preposition_in_to", "verb_preposition_over_with"],
}
EVAL = ("verb_preposition_in_to", "verb_preposition_over_with", "verb_preposition", "verb_preposition_insisted", "verb_preposition_to_at", "verb_preposition_with_about", "verb_preposition_of_with", "verb_preposition_from_for", "verb_preposition_on_about", "verb_preposition_for_at", "verb_preposition_to_with", "verb_preposition_at_to", "verb_preposition_from_about", )
COUNTED = ("verb_preposition", "verb_preposition_insisted", "verb_preposition_to_at", "verb_preposition_with_about", "verb_preposition_of_with", "verb_preposition_from_for", "verb_preposition_on_about", "verb_preposition_for_at", "verb_preposition_to_with", "verb_preposition_at_to", "verb_preposition_from_about", )
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES, **v213.NAMES, **v215.NAMES, **v217.NAMES, **v219.NAMES, **v221.NAMES, **v223.NAMES, **v225.NAMES, **v227.NAMES, **v229.NAMES, **v231.NAMES, **v233.NAMES, **v235.NAMES, **v237.NAMES, **v239.NAMES, **v241.NAMES, **v243.NAMES, **v245.NAMES, **v247.NAMES, **v249.NAMES, **v251.NAMES, **v253.NAMES, **v255.NAMES, **v257.NAMES, **v259.NAMES}


def _parent_cdas(n):
    # the parent battery receipt that produced this member's units (each behaviour name is unique across batteries)
    for path in (V259, V257, V255, V253, V251, V249, V247, V245, V243, V241, V239, V237, V235, V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v252", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V252_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V252_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v252", "candidate_id": "corpus.unit_family_separability_spec_v252",
              "separable_counts": nsep, "families": FAMILIES, "summary": summary, "members": R, "bars": BARS,
              "recipe": {"lambda": LAM, "steps": steps, "lr": LR, "complement_weight": CW, "rank": 1, "seed": 0, "units_source": "v197 ... v259 battery receipts (later receipts override earlier ones)", "evaluated": list(EVAL),
