#!/usr/bin/env python
"""early_frame_count_probe -- how many EARLY frames does the k = 1024 program need? Own (16, SPLIT8) vs per-block (8) vs per-pair
(4) vs one early frame, late reads through the bus U_8 in every arm; then the 9-frame program (8 block frames + bus) with the
chain writes and bus writes of §2764. CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_block_frames_nearly_free_at_1024 pred_c_pair_frames_under_p03_at_1024
#                     pred_d_one_early_frame_is_the_cliff pred_e_nine_frame_program_under_p08_at_1024
...
Preregistration: polynomial_causal/EARLY_FRAME_COUNT_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/early_frame_count_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("early_frame_count_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "EARLY_FRAME_COUNT_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "chain_bus_program_statement_probe_results.json"   # §2764
OUT = ROOT / "early_frame_count_probe_results.json"
HASHES = {PREREG: "9570ac2191941389f058b2a9fe614ba146f541e7bd06ce7b2ee9cef810544aed", PRIOR: "de512d007fede1615714d0160999327768de40ba2da352a04a064a870d6f047c",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "early_frame_count_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_SPLIT8_1024 = 0.0374; LAM = 1e-2
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_max": 0.010, "c_max": 0.030, "d_min": 0.050, "e_max": 0.080}
NULLS = {"b_min": 0.030, "c_min": 0.080, "d_max": 0.015, "e_min": 0.150}
...
def main():
    ...
    def frame_of_blocks(ls):
        return core_of(blocks(ls))
    FB = [frame_of_blocks([l]) for l in range(SPLIT)]
    FP = [frame_of_blocks([2 * p, 2 * p + 1]) for p in range(SPLIT // 2)]
    F1 = frame_of_blocks(range(SPLIT))
    def read_program(k, frame_for):
        patch = {s_: head(s_, k, frame_for(s_)) for s_ in OWN8}
        for s_ in SET8:
            patch[s_] = head(s_, k, U_8)
        return run(patch)
    def block_chain_bus(k):
        patch = {}; buf = Buf()
        for s_ in OWN8:
            kd, l = s_
            wf = FB[l] if kd == "attn" else (FB[l + 1] if l + 1 < SPLIT else U_8)
            patch[s_] = Both(head(s_, k, FB[l]), Route(s_, k, wf, "delete", buf))
        for s_ in SET8:
            patch[s_] = Both(head(s_, k, U_8), Route(s_, k, U_8, "readout", buf))
        patch[FINAL] = FinalAdd(buf, +1.0)
        return run(patch, (buf,))
    arms = {"SPLIT8_1024": read_program(K, lambda s_: None)}
    arms["BLOCK8_1024"] = read_program(K, lambda s_: FB[s_[1]])
    arms["PAIR4_1024"] = read_program(K, lambda s_: FP[s_[1] // 2])
    arms["EARLY1_1024"] = read_program(K, lambda s_: F1)
    arms["BLOCK8_CHAIN_BUS_1024"] = block_chain_bus(K)
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    gaps = {kk + "_minus_split8_1024": arms[kk] - arms["SPLIT8_1024"] for kk in ("BLOCK8_1024", "PAIR4_1024", "EARLY1_1024")}
    def frame_angles(A, B):
        sv = torch.linalg.svdvals(A[:, :K].T.double() @ B[:, :K].double()).clamp(-1, 1); return int((torch.rad2deg(torch.acos(sv)) > 30).sum())
    n30 = {"block_vs_own": [[frame_angles(FB[l], heads[s_]["U"]) for s_ in blocks([l])] for l in range(SPLIT)],
           "pair_vs_block": [[frame_angles(FP[p], FB[2 * p]), frame_angles(FP[p], FB[2 * p + 1])] for p in range(SPLIT // 2)],
           "early1_vs_block": [frame_angles(F1, FB[l]) for l in range(SPLIT)]}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_block_frames_nearly_free_at_1024': bool(gaps["BLOCK8_1024_minus_split8_1024"] <= BARS["b_max"]),
        'pred_c_pair_frames_under_p03_at_1024': bool(gaps["PAIR4_1024_minus_split8_1024"] <= BARS["c_max"]),
        'pred_d_one_early_frame_is_the_cliff': bool(gaps["EARLY1_1024_minus_split8_1024"] >= BARS["d_min"]),
        'pred_e_nine_frame_program_under_p08_at_1024': bool(arms["BLOCK8_CHAIN_BUS_1024"] <= BARS["e_max"]),
    }
    nulls = {"b_null_ge_.03": bool(gaps["BLOCK8_1024_minus_split8_1024"] >= NULLS["b_min"]), "c_null_ge_.08": bool(gaps["PAIR4_1024_minus_split8_1024"] >= NULLS["c_min"]),
             "d_null_le_.015": bool(gaps["EARLY1_1024_minus_split8_1024"] <= NULLS["d_max"]), "e_null_ge_.15": bool(arms["BLOCK8_CHAIN_BUS_1024"] >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "split": SPLIT, "k": K,
           "program": "early frames shared per block (8) / per pair (4) / one (1) vs own (16); BLOCK8_CHAIN_BUS: 8 block frames + bus, chain writes into the next block frame (delete), bus writes with readout remainder",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024},
           "ce_added": arms, "gaps": gaps, "n30_angles_over_30deg": n30,
