#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_attention_into_the_pronoun_slot_replays pred_c_mlps_at_the_pronoun_slot_small pred_d_all_mlps_at_the_pronoun_slot_small pred_e_attention_dominates
"""The pronoun slot's two channels (v503). v475 (verb): attention into the verb position closes 0.94 of the agreement margin and MLPs 9-16 there jointly
0.39 (a rebuild the detector reads). v463 / v464: the pronoun readout has no MLP-17 final step. Here at the PRONOUN slot (122 natural pairs, they - he):
every block's attention write into the answer position (replay of 0.973, v481); MLPs 9-16 there jointly; MLPs 1-17 there jointly.
PREDICTIONS (scored as written; failures preserved; priors from v464 / v475 / v481)
    pred_a_baseline_replays                      the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_attention_into_the_pronoun_slot_replays  every block's attention write into the answer position closes 0.973 +- 0.03
    pred_c_mlps_at_the_pronoun_slot_small        MLPs 9-16 at the answer position jointly close <= 0.15 (no rebuild for the pronoun readout). Prior: unsure
    pred_d_all_mlps_at_the_pronoun_slot_small    MLPs 1-17 at the answer position jointly close <= 0.25
    pred_e_attention_dominates                   the attention joint is >= 4 x the MLPs 1-17 joint
PRICE (registered maximum): 4 batches x 4 passes = 16 forwards; 0 backwards; 0 fits. Bar <= 17.
"""
...
OUT = ROOT / "circuits/followups/pronoun_slot_channels_v503_result.json"
CANDIDATE_ID = "chain.pronoun_slot_channels_v503"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, ATTN_V481, BAND, M916_MAX, MALL_MAX, RATIO_MIN, UNIT = 1e-3, 0.973, 0.03, 0.15, 0.25, 4.0, 701
import random
...
FORWARDS_MAX = 17
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_attention_into_the_pronoun_slot_replays": "0.973 +- 0.03", "pred_c_mlps_at_the_pronoun_slot_small": "<= 0.15", "pred_d_all_mlps_at_the_pronoun_slot_small": "<= 0.25", "pred_e_attention_dominates": ">= 4 x"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "attn_v481": ATTN_V481, "band": BAND, "m916_max": M916_MAX, "mall_max": MALL_MAX, "ratio_min": RATIO_MIN, "unit": UNIT}}
    ...
    MODES = {"attn_all": ({}, (0,)), "mlps9_16": ({}, (0,)), "mlps1_17": ({}, (0,))}; forwards = 0
    def logits_margin(x):
    ...
    def run(seqs, pos, mode):
        ...
        with torch.no_grad():
            ...
            for l, block in enumerate(blocks):
                ...
                m_out = block.mlp(xm)
                if (mode == "mlps9_16" and 9 <= l <= 16) or (mode == "mlps1_17" and 1 <= l <= 17):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_attention_into_the_pronoun_slot_replays": abs(mc["attn_all"] - ATTN_V481) <= BAND, "pred_c_mlps_at_the_pronoun_slot_small": abs(mc["mlps9_16"]) <= M916_MAX,
                   "pred_d_all_mlps_at_the_pronoun_slot_small": abs(mc["mlps1_17"]) <= MALL_MAX, "pred_e_attention_dominates": mc["attn_all"] >= RATIO_MIN * abs(mc["mlps1_17"])}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pronoun_slot_channels_result_v503", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
