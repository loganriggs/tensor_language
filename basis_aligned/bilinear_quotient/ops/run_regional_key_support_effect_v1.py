...
pred_b forward13-key routing branch write relative error<=.1;
pred_c forward13-key regional signed effect error<=.1 plus preserved controls.
...
STEM='REGIONAL_KEY_SUPPORT_EFFECT_V1'
@torch.no_grad()
def main():
    ...
    def producer_hook(layer,module,args,out):
        ...
        for name in ('forward','backward','all'):
            approx=inp.clone()
            for i,prefix in enumerate(captured['cue_prefixes']):approx[i,1]=support_states[(prefix,layer,name)]
    ...
    try:
        for length,ids in batches:
            ...
            captured['token_embeddings']=x0;captured['cue_prefixes']=[tuple(rows[i]['ids'][:2]) for i in ids]
    ...
    instrument=max(producer_errors+replays+[replay,cells[-1]['write_relative_error']])<=1e-5;candidate=cells[0];regscale=float(regref.square().mean().sqrt())
