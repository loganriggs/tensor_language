#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_branches_add pred_c_token_only_branch_small pred_d_current_branch_carries pred_e_five_values_carry
"""The VERB readers' two value branches at the noun (v595): v594's edit on the verb readout. Number's pronoun readers copy a computed state (current-state
0.411, token-only 0.075 of 0.483; v594), gender's readers read half the token (0.291 / 0.314 of 0.600; v558). Here the five verb heads {11.3, 7.8, 13.1, 9.7, 5.3}
values AT THE NOUN swapped by branch on the 79 distant natural rows (verb two or more tokens after the noun; agreement margin): CUR, TOK, FULL. 5.3 reads the
token after the noun rather than the noun, so its share here should be small; the four late readers read the noun's rebuilt state.
PREDICTIONS (scored as written; failures preserved; priors from v445-v448 / v594)
    ...
    pred_c_token_only_branch_small TOK closes <= 0.10 of the agreement gap
    pred_d_current_branch_carries  CUR closes >= 0.25
    pred_e_five_values_carry       FULL closes >= 0.30. Prior: unsure (their writes into the verb: 0.557)
PRICE (registered maximum): 3 batches x 4 passes = 12 forwards (79 distant rows); 0 backwards; 0 fits. Bar <= 13.
"""
...
OUT = ROOT / "circuits/followups/verb_reader_value_branches_v595_result.json"
CANDIDATE_ID = "verb.reader_value_branches_v595"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, BAND, TOK_MAX, CUR_MIN, FULL_MIN, UNIT = 1e-3, 0.03, 0.10, 0.25, 0.30, 701
...
FORWARDS_MAX = 13
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_branches_add": "CUR + TOK = FULL +- 0.03", "pred_c_token_only_branch_small": "<= 0.10", "pred_d_current_branch_carries": ">= 0.25", "pred_e_five_values_carry": ">= 0.30"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "band": BAND, "tok_max": TOK_MAX, "cur_min": CUR_MIN, "full_min": FULL_MIN, "unit": UNIT}}
    ...
    MODES = {m: (FIVE_V, (0,)) for m in ("CUR", "TOK", "FULL")}; SETS_W = {}; forwards = 0
    def logits_margin(x):
        ...
        with torch.no_grad():
            ...
            for l, block in enumerate(blocks):
                ...
                if mode != "native" and l in MODES[mode][0]:
                    ...
                    for h in hs:
                        for o in offs:
                            if mode == "CUR": v[idx, pp + o, h] = (vcur[swap, pp + o, h] + vtok[idx, pp + o, h]).clone()
                            elif mode == "TOK": v[idx, pp + o, h] = (vcur[idx, pp + o, h] + vtok[swap, pp + o, h]).clone()
                            else: v[idx, pp + o, h] = v[swap, pp + o, h].clone()
    ...
    dist_items = [(p, s_, c) for p, s_, c, d in verb_items if d >= 2]   # distant rows (79): the verb two or more tokens after the noun
    ...
    print("verb readers by branch", {k: round(v, 3) for k, v in mc.items()})
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_branches_add": abs(mc["CUR"] + mc["TOK"] - mc["FULL"]) <= BAND, "pred_c_token_only_branch_small": abs(mc["TOK"]) <= TOK_MAX,
                   "pred_d_current_branch_carries": mc["CUR"] >= CUR_MIN, "pred_e_five_values_carry": mc["FULL"] >= FULL_MIN}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "verb_reader_value_branches_result_v595", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
