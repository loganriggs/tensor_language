...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v674_result.json'
...
def main():
    ...
    package=torch.load(OUT.parent/'subject_response_v673_program.pt',map_location='cpu',weights_only=True)
    def gpu(value):
    ...
    reuse_reports={name:{} for name in ['A','B','sum']};dense_reuse_reports={name:{} for name in ['A','B','sum']};reuse_counts=dict(block11=0,post11_suffix=0)
    boundary_gaps=[]
    chain_checks=[]
    ...
    for entries in batches:
        ...
        for source_name,delta in [('A',source_deltas[0]),('B',source_deltas[1]),('sum',source_deltas[0]+source_deltas[1])]:
            ...
            dense_predicted,dense_final_z=run_dag(runtime,context['all_attention'],source_z,context['all_mlp_contexts'],case['final_context'],readpos)
            dense_effect=base-(dense_predicted[:,0]-dense_predicted[:,1])*direction
            dense_ctrl=readout_prepared(context['control_fixed'],dense_final_z,context['control_context'])
            dense_modal=((cb-dense_ctrl)[:,0::2]-(cb-dense_ctrl)[:,1::2])
            ...
            for i,e in enumerate(entries):
                ...
                dc=dense_reuse_reports[source_name].setdefault(e[3],dict(target=[],prediction=[],native_modal=[],predicted_modal=[]))
                dc['target'].append(float(native_effect[i]));dc['prediction'].append(float(dense_effect[i]))
                dc['native_modal'].append(native_modal[i].tolist());dc['predicted_modal'].append(dense_modal[i].tolist())
    ...
    for report in list(reuse_reports.values())+list(dense_reuse_reports.values()):
    ...
    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,cells=cells,
                predictions=dict(zip(PREDICTIONS,[bool(a),bool(b),bool(c)])),selectivity=selectivity,reuse_reports=reuse_reports,dense_reuse_reports=dense_reuse_reports,cross_effects=cross,reuse_counts=reuse_counts,pre11_joint_vs_post11_sum_gap=boundary_gaps,
