#!/usr/bin/env python
"""early_stack_width_map_probe -- the EARLY stack's width map (blocks 0-10, 22 sublayers): every early sublayer alone on its own
256-core, the early stack on own / shared k-cores, and the complete 36-sublayer width program (early own 768 + late joint 768,
§2744/§2745/§2746). CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_early_stack_768 pred_c_whole_model_768 pred_d_no_early_sublayer_is_narrow_critical
#                     pred_e_early_sharing_cost
...
Preregistration: polynomial_causal/EARLY_STACK_WIDTH_MAP_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/early_stack_width_map_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("early_stack_width_map_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "EARLY_STACK_WIDTH_MAP_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_stack_write_core_probe_results.json"   # §2747
OUT = ROOT / "early_stack_width_map_probe_results.json"
HASHES = {PREREG: "ce9da60efdd3df9201856fcf324a70693f21e90fe0df8787c64935cbc235ed97", PRIOR: "4bf02804d5b271fa636d1be512ddc43553880d14d9e60a5c6dc98ef4783d1028",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "early_stack_width_map_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_LATE14_JOINT_768 = 0.109; PRIOR_EARLY11_OWN_768 = 0.0435; LAM = 1e-2
K = 768; KS1 = 256; KOWN = [512, 768, 1024]; KSH = [768, 1024]; EARLY = list(range(0, 11)); LATE = list(range(11, 18))
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_max": 0.08, "c_max": 0.25, "d_max": 0.10, "e_max": 0.10}
NULLS = {"b_min": 0.20, "c_min": 0.45, "d_min": 0.30, "e_min": 0.20}
...
def main():
    ...
    U_late = core_of(LSITES); U_early = core_of(ESITES)
    def run(patch):
    ...
    late = {s_: head(s_, K, U_late) for s_ in LSITES}
    arms = {"EARLY11_OWN_768": run({("mlp", l): head(("mlp", l), K) for l in EARLY}), "LATE14_JOINT_768": run(late)}
    single = {}
    for s_ in ESITES:
        single[f"{s_[0]}{s_[1]}"] = run({s_: head(s_, KS1)})
    log(stage="single_256", **{k: round(v, 4) for k, v in single.items()})
    for k in KOWN:
        arms[f"EARLY22_OWN_{k}"] = run({s_: head(s_, k) for s_ in ESITES})
    for k in KSH:
        arms[f"EARLY22_SHARED_{k}"] = run({s_: head(s_, k, U_early) for s_ in ESITES})
    full36 = {s_: head(s_, K) for s_ in ESITES}; full36.update({s_: head(s_, K, U_late) for s_ in LSITES})
    arms["ALL36_768"] = run(full36)
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    smax = max(single.values()); smax_site = max(single, key=single.get)
    gaps = {"early_shared_minus_own_768": arms["EARLY22_SHARED_768"] - arms["EARLY22_OWN_768"], "early_shared_minus_own_1024": arms["EARLY22_SHARED_1024"] - arms["EARLY22_OWN_1024"],
            "all36_minus_parts": arms["ALL36_768"] - arms["EARLY22_OWN_768"] - arms["LATE14_JOINT_768"], "sum_single_256": sum(single.values()), "max_single_256": smax, "max_single_site": smax_site}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["LATE14_JOINT_768"] - PRIOR_LATE14_JOINT_768) <= BARS["repro_tol"] and abs(arms["EARLY11_OWN_768"] - PRIOR_EARLY11_OWN_768) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_early_stack_768': bool(arms["EARLY22_OWN_768"] <= BARS["b_max"]),
        'pred_c_whole_model_768': bool(arms["ALL36_768"] <= BARS["c_max"]),
        'pred_d_no_early_sublayer_is_narrow_critical': bool(smax <= BARS["d_max"]),
        'pred_e_early_sharing_cost': bool(gaps["early_shared_minus_own_768"] <= BARS["e_max"]),
    }
    nulls = {"b_null_ge_.20": bool(arms["EARLY22_OWN_768"] >= NULLS["b_min"]), "c_null_ge_.45": bool(arms["ALL36_768"] >= NULLS["c_min"]),
             "d_null_ge_.30": bool(smax >= NULLS["d_min"]), "e_null_ge_.20": bool(gaps["early_shared_minus_own_768"] >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "k": K, "k_single": KS1, "ks_own": KOWN, "ks_shared": KSH,
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "late14_joint_768": arms["LATE14_JOINT_768"], "prior_late14_joint_768": PRIOR_LATE14_JOINT_768, "early11_own_768": arms["EARLY11_OWN_768"], "prior_early11_own_768": PRIOR_EARLY11_OWN_768},
           "ce_added": arms, "single_256": single, "gaps": gaps, "input_eff_rank": {f"{s_[0]}{s_[1]}": heads[s_]["eff_rank"] for s_ in ASITES + SITES},
           "price": {"gpu_doc_forwards": int(fit_rows.shape[0]) + int(ev.shape[0]) * (1 + len(arms) + len(single)), "cpu_doc_forwards": 0, "gpu_seconds": time.time() - started},
