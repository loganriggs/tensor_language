"""TABLES-ONLY CERTIFICATE (rung 103): completing the family decomposition.

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows. Config: ALL attention
real AND all CP units kept (K = K69 = 4608 - the middles are effectively the real MLPs); ONLY the front tables
(m0-m3) replaced.

REGISTERED PREDICTIONS:
  (a) VALIDITY APPEARS: >= 30 of 62 circuits valid at tau = 0.5 x ref.
  (b) AGGREGATE: mean dCE <= 1.0.
  (c) SANITY: aggregate in [0.1, 5.0].
NULL: even the tables alone break everything. PRICE: none. Self-reviewed. RUNG-102 HEADER FOLLOWS.

CERTIFICATE FOR TABLES+MIDDLES ONLY (rung 102): the curve's last cheap point.

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows. Config: ALL attention
real (motifs off 2-9, tail aXL skipped 10-17); only the front tables (m0-m3) and CP middles (c4-c9) replaced.

REGISTERED PREDICTIONS:
  (a) VALIDITY APPEARS: >= 20 of 62 circuits valid at tau = 0.5 x ref.
  (b) AGGREGATE: mean dCE <= 2.0.
  (c) SANITY: aggregate in [0.2, 5.0].
NULL: still 0/62 - no partial replacement preserves the certified circuits. PRICE: none. Self-reviewed.
RUNG-101 HEADER FOLLOWS.

CERTIFICATE FOR THE ALL-MOTIFS-REAL CONFIG (rung 101): the decisive front attribution.

CONVENTION (S2135): per-position dCE = CE(all-motifs-real config) - CE(real model) on the census rows. S2193:
the tail retreat restores almost nothing - the breakage is front-caused. Here the front attention is REAL
(motif hooks off everywhere) with the tail dictionaries in place. S2169/S2189 predict a large recovery.

REGISTERED PREDICTIONS:
  (a) LARGE RECOVERY: >= 30 of 62 circuits valid at tau = 0.5 x ref.
  (b) THE RESIDUE IS THE TAIL: >= 60% of still-failing circuits are a16/m16/a14-localized.
  (c) SANITY: aggregate mean dCE in [0.3, 5.0].
NULL: breakage persists (the CP middles/front tables also break circuits). PRICE: none. Self-reviewed.
RUNG-100 HEADER FOLLOWS.

CERTIFICATE FOR THE ENVELOPE CONFIG (rung 100): does skip-1416 restore the a16-localized circuits?

CONVENTION (S2135): per-position dCE = CE(skip-1416 config) - CE(real model) on the census rows. S2192: the
full S2144 config is valid for 0/62 circuits at tau = 0.5 x ref. The deepest envelope retreat keeps both tail
attentions real - the 13 a16-localized circuits (and a14's) should recover if the tail dictionaries were their
breakage. NULL: the breakage is front-caused; the retreat restores little.

REGISTERED PREDICTIONS:
  (a) a16 RECOVERY: the a16-localized circuits' median damage ratio improves >= 2x vs rung 99.
  (b) SOME VALIDITY APPEARS: >= 10 of 62 circuits valid at tau = 0.5 x ref.
  (c) SANITY: aggregate mean dCE in [0.5, 5.0].
NULL as above. PRICE: none (certification). Self-reviewed. RUNG-99 HEADER FOLLOWS.

CIRCUIT-GRAIN CERTIFICATION OF THE FRONTIER CONFIG (rung 99; math review 0437).

CONVENTION (S2135): per-position dCE = CE(S2144 config) - CE(real model) on the CENSUS rows; positive = the
assembly hurts there. The battery (62 circuits, member masks over the census grid) becomes a causal-abstraction
certificate: the config is VALID for a circuit at tau iff its member mean|dCE| < tau = 0.5 x that circuit's
battery reference (mean-ablation of its own top component).

REGISTERED PREDICTIONS:
  (a) BROADLY VALID: >= 40 of 62 circuits are abstraction-valid at tau.
  (b) FAILURES ARE RETRIEVAL: >= 70% of failing circuits have an (ind+other) member fraction above the
      all-circuit median fraction.
  (c) SANITY: the config's aggregate mean dCE on the census rows lies in [0.5, 5.0].
NULL: broad breakage beyond retrieval. PRICE: none (certification). Self-reviewed. RUNG-73 HEADER FOLLOWS.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_certificate4_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: rung-102 artifact present; tables-only certificate")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_certificate5_results.json'
...
if __name__=='__main__':
    ...
    print(f'{len(CINFO)} circuits with masks; census rows {tuple(CROWS.shape)}',flush=True)
    SEL['mode']='norm'; SEL['K']=4608; SEL['K69']=4608; SEL['K69MAP']={}
    SEL['skipset']=tuple(range(10,18)); SEL['motif_off']=tuple(range(2,10)); SEL['clsdmg']=True; SEL['ext_rows']=CROWS
    print('ARM: tables-only config (all attention real, all CP units kept) on the census rows',flush=True)
    r_cfg=main()
    cev=SEL['cev']; clsflat=SEL['clsflat']
    if cev.shape[0]!=NFLAT:
        raise SystemExit(f'INSTRUMENT FAIL: cev {cev.shape[0]} != nflat {NFLAT}')
    import statistics as stt
    dce=cev-CBASE
    agg=float(dce.mean())
    IND=CLS.index('ind'); OTH=CLS.index('other')
    rows=[]
    for t,info in CINFO.items():
        mm=info['mask']
        md=float(dce[mm].abs().mean())
        iof=float(((clsflat[mm]==IND)|(clsflat[mm]==OTH)).float().mean())
        rows.append({'tag':t,'ref':round(info['ref'],3),'member_absdce':round(md,4),
                     'valid':bool(md<0.5*info['ref']),'ind_other_frac':round(iof,3)})
    nvalid=sum(1 for r in rows if r['valid'])
    pa=nvalid>=30
    pb=agg<=1.0
    pc=0.1<=agg<=5.0
    fr_ret=float(nvalid)
    res={'aggregate_mean_dce':round(agg,4),'n_circuits':len(rows),'n_valid':nvalid,
         'n_valid_float':round(fr_ret,1),
         'circuits':rows,
         'convention':'per-position dCE = CE(S2144 config) - CE(real model) on the census rows; tau = 0.5 x battery ref',
         'pred_a_validity_appears':bool(pa),'pred_b_aggregate':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'aggregate dCE {agg:+.4f}; valid {nvalid}/{len(rows)}')
    print(f"(a) valid {nvalid} >= 30: {'HELD' if pa else 'FAILED'}")
    print(f"(b) aggregate {agg:+.4f} <= 1.0: {'HELD' if pb else 'FAILED'}")
    print(f"(c) aggregate {agg:+.4f} in [0.5, 5.0]: {'HELD' if pc else 'FAILED'}")
