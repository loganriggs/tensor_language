...
import numpy as np
RUNNER=Path(__file__).with_name('run_induction_early_mlp_finite_bilinear_factor_v1.py')
def load():
 spec=importlib.util.spec_from_file_location('finite_bilinear',RUNNER);m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m);return m
def test_bound_plan():
