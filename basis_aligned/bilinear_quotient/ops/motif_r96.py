"""GRADED MOTIF RANK (rung 243): motifs r96, tail r64 - is full exactness required, or just rank?

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. The registered corner uses EXACT
motif patterns. If the motif hunger is a smooth rank effect, r96 motifs (3/4 rank) should keep most of
the gain at ~3/4 the motif spend (~35.4M + tail 24.9M = 60.3M vs the corner's 72.1M).
REGISTERED PREDICTIONS (arms: THIS vs r64 0.0852/7 and motif-exact corner 0.0608/9):
  (a) MOST OF THE GAIN SURVIVES: census <= 0.070.
  (b) CERTIFICATES MOSTLY KEPT: valid >= 8.
  (c) SANITY: census in [0.01, 0.12]; inert tripwires vs r64 AND corner receipts.
NULL: motif certificates need exactness (valid <= 7) - the corner is sharp in rank too. PRICE: 60.3M
pattern values. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: motifs r96, tail r64')
    raise SystemExit(0)
import torch
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=64; SEL['qk_rmap']={li:96 for li in range(2,10)}; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    if abs(agg-0.0852)<1e-4 or abs(agg-0.0608)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: motif-r96 map inert')
