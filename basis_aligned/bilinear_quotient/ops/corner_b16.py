"""TARGETED TAIL EXACTNESS (rung 246): the corner + block 16 exact - aimed at the a16/m16 failure family.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2342's failure map: a16-family 13 of
53 failures; nearest threshold r.6.2.2 (1.03, m16-family) and r.3.0.0 (1.25, a16). Block 16 is the tail's
most circuit-loaded block; the corner leaves it at r64. Here: corner + block-16 patterns exact (+2.9M).
REGISTERED PREDICTIONS (arms: THIS vs corner 0.0608/9; tail-exact-all gave only +1 cert from r64 base):
  (a) census <= 0.058.
  (b) THRESHOLD HARVEST: valid >= 10 (r.6.2.2 at 1.03 crosses).
  (c) SANITY: census in [0.01, 0.12]; inert tripwire vs the corner receipt.
NULL: the a16/m16 family is not block-16-rank-limited (valid <= 9) - matching S2332's lesson at the tail.
PRICE: corner + 9 head-patterns full rank (75.0M pattern values). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: corner + block-16 exact')
    raise SystemExit(0)
import torch
...
OUT=PT+'corner_b16_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['qk_r']=64; _rm={li:128 for li in range(2,10)}; _rm[16]=128; SEL['qk_rmap']=_rm; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    pb=nv>=10
    pc=0.01<=agg<=0.12
    if abs(agg-0.0608)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: b16 map inert')
    ...
    print(f"(a) census {agg:+.4f} <= 0.058: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 10; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
