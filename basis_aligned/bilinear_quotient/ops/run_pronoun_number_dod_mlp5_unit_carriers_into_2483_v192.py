...
"""Pronoun number they/he DoD (v192): which MLP-5 UNITS carry the number into MLP-6 unit 2483? v191: MLP 5 is 2483's largest carrier (35% of the
plural - singular contrast at the noun) although its mass share is negative; v184's leave-one-unit-out census (mass + carriage) found MLP 5 diffuse
and cancelling. Carrier-level unit census: with a_j = (L . c_j)/rms, b_j = (R . c_j)/rms for unit j's write c_j = lam0_6 h5_j Down5[:, j] as it reaches
block 6, and A, B the totals of the two factors, unit j's carrier term over an aligned pair is  da_j mB + mA db_j  (exact identity per pair; the
terms over all writers sum to the contrast). Pooled over pairs; shares relative to MLP 5's carrier total (sum over units + the bias, which has no change).
PREDICTIONS (scored as written; failures preserved; priors unsure)
    pred_a_carrier_closure           sum over units of carrier terms = MLP 5's carrier term (the v191 quantity) within relative 1e-3, every pair
    pred_b_top_10_units_carry_half   top-10 |pooled| units carry >= 0.50 of MLP 5's carrier total
    pred_c_top_50_units_carry_080    top-50 >= 0.80
    pred_d_largest_unit_supports     the largest |pooled| unit has the sign of MLP 5's carrier total (it carries with, not against)
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_mlp5_unit_carriers_into_2483_v192_result.json"
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_mlp5_unit_carriers_into_2483_v192"
UNIT, SRC, DST, CLOSURE_TOL, TOP10_MIN, TOP50_MIN, BATCH = 2483, 5, 6, 1e-3, 0.50, 0.80, 32
FORWARDS_MAX = 5
PREDICTIONS = {"pred_a_carrier_closure": "<= 1e-3", "pred_b_top_10_units_carry_half": ">= 0.50", "pred_c_top_50_units_carry_080": ">= 0.80", "pred_d_largest_unit_supports": "sign of the MLP-5 carrier total"}
...
def main() -> None:
    ...
    fw = L.ManualForward(backend); blocks = model.transformer.h; mlp6, mlp5 = blocks[DST].mlp, blocks[SRC].mlp
    ...
    predictions = {"pred_a_carrier_closure": closure <= CLOSURE_TOL, "pred_b_top_10_units_carry_half": share(10) >= TOP10_MIN, "pred_c_top_50_units_carry_080": share(50) >= TOP50_MIN, "pred_d_largest_unit_supports": (top[0][1] > 0) == (total > 0)}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_mlp5_unit_carriers_into_2483_result_v192", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "carrier_total": total, "shares": {str(k): share(k) for k in (5, 10, 20, 50, 100, 500)},
