"""IS THE MAGIC 0.25 JUST UNDER-REGULARISATION? SWEEP THE RIDGE LAMBDA THAT PRODUCED THE LINK MAPS (Claude, LANE 1)

SS2914 established that the shrinkage is REAL: 98.6% of the gain transports to documents that chose nothing, bias +0.0044 nats. So the
question stops being "is it an artefact" and becomes **"what is it"**.

The tail link maps are ridge solutions: `LW[k] = (Xk'Xk + 1e-2*n I)^-1 Xk'Yk`, a penalty chosen once, for LOCAL reconstruction, and
never revisited. SS2890/SS2902/SS2905 say components fitted to a local criterion are systematically **too large** for the end-to-end
score. **A ridge penalty is exactly the knob that makes a least-squares solution smaller.** So there is an obvious hypothesis the
campaign has never tested: the adopted `x0.25` is a crude stand-in for a lambda that was simply too small.

This sweeps `lambda -> mult * lambda` for mult in {4, 16, 64, 256, 1024}, re-solving the SAME estimator on the SAME data with only the
penalty changed, and compares against the adopted uniform `x0.25` measured in the same run. The normal equations `(Xk'Xk, Xk'Yk, n)` are
captured during the fit and kept on CPU (~340 MB), so every lambda is a re-solve, **not a refit** -- seven arms for one pipeline run.

**The two outcomes are both worth having, which is why this is the rung to run:**
  ridge REACHES uniform (pred_d holds)  -> the correction has a principled reading: **the local ridge is under-regularised for the
                                           end-to-end objective**, and the magic scalar becomes a lambda with a standard meaning.
  ridge CANNOT reach it (pred_d fails)  -> uniform shrinkage is a genuinely DIFFERENT operator from ridge. Ridge shrinks along
                                           data-aligned directions by eigenvalue; uniform scaling shrinks every direction equally.
                                           Failing pred_d would say the end-to-end excess is **isotropic**, not concentrated in the
                                           low-eigenvalue directions ridge attacks -- a real structural fact about the error.
...
SS2125 STANDS -- this re-solves an already-specified estimator; it neither selects nor reorders.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_uniform_scaling_reproduces_S2896
#                     pred_c_more_ridge_improves_the_frontier_at_all
#                     pred_d_ridge_reaches_what_uniform_shrinkage_achieves
#                     pred_e_the_best_lambda_is_interior_to_the_grid

Preregistration: polynomial_causal/FRONTIER_TAIL_RIDGE_LAMBDA_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run: a ridge-lambda sweep on the tail link maps against the adopted uniform x0.25")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_tail_ridge_lambda_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_TAIL_RIDGE_LAMBDA_PREREGISTRATION.md'
PREREG_SHA = "a0b8ef779d2ba163411856c2d3146052bd989c5a3ede7840dd60826ca5733006"
...
BARS = {"reproduce": 0.05, "anchor": 0.01, "ridge_helps": -0.05, "reaches_uniform": 0.02}
LAM_GRID = [4, 16, 64, 256, 1024]
...
if __name__=='__main__':
    ...
    tc  = json.load(open(S2904))['summary']['cost_fresh']['TC']        # -0.3213 on FR (T .25, C .5)
    SEL['mode']='norm'

    # Seven arms against ONE fitted stack: the adopted uniform x0.25 as an in-run control, plus five
    # re-solves of the same estimator at a larger ridge penalty. No refit -- the normal equations are cached.
    ARMS[:] = ([('baseline', {}), ('scale25', {'tail_scale': 0.25})] +
               [(f'lam{v}', {'tail_lambda': v}) for v in LAM_GRID])
    ...
    fitc = {n: round(out[n]['L2_C'] - out['baseline']['L2_C'], 4) for n, _ in ARMS if n != 'baseline'}
    lam_cost = {v: cost[f'lam{v}'] for v in LAM_GRID}
    # the holdout window comes free from the SS2914 machinery this rung was derived from: every arm is
    # already evaluated on both windows, so each lambda's transport is reported without a second run.
    b2 = out['baseline']['L2_F2']
    cost_h = {n: round(out[n]['L2_F2'] - b2, 4) for n, _ in ARMS if n != 'baseline'}
    best_lam = min(lam_cost, key=lambda v: lam_cost[v])
    best_lam_cost = lam_cost[best_lam]
    ref = cost['scale25']                              # the adopted uniform shrinkage, re-measured here
    interior = best_lam not in (LAM_GRID[0], LAM_GRID[-1])

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = abs(ref - S2896_COST) <= BARS['anchor']
    pc = best_lam_cost <= BARS['ridge_helps']
    pd = best_lam_cost <= S2896_COST + BARS['reaches_uniform']
    pe = bool(interior)
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_uniform_scaling_reproduces_S2896':bool(pb),
           'pred_c_more_ridge_improves_the_frontier_at_all':bool(pc),
           'pred_d_ridge_reaches_what_uniform_shrinkage_achieves':bool(pd),
           'pred_e_the_best_lambda_is_interior_to_the_grid':bool(pe)}
    nulls={'b_null_the_anchor_fails':bool(abs(ref - S2896_COST) >= 0.03),
           'c_null_more_ridge_does_nothing':bool(best_lam_cost > BARS['ridge_helps']),
           'd_null_ridge_cannot_reach_uniform_shrinkage':bool(not pd),
           'e_null_the_lambda_grid_is_too_narrow':bool(not interior)}
    res={'rung':'frontier_tail_ridge_lambda','preds':preds,'nulls':nulls,'bars':BARS,
         'arms':out,'published_L2_F':2.6735,'lambda_grid':LAM_GRID,
         'summary':{'L2_F_baseline':round(bF,4),
                    'cost_fresh':cost,'cost_fit':fitc,'cost_holdout':cost_h,
                    'L2_F2_baseline':round(b2,4),
                    'holdout_bias_best_lambda':round(cost_h[f'lam{best_lam}'] - best_lam_cost, 4),
                    'holdout_bias_uniform':round(cost_h['scale25'] - ref, 4),
                    'lambda_cost_fresh':lam_cost,
                    'best_lambda_multiplier':best_lam,'best_lambda_cost':best_lam_cost,
                    'uniform_scale_0.25_cost':ref,'S2896_adopted_cost':S2896_COST,
                    'ridge_minus_uniform':round(best_lam_cost - ref, 4),
                    'best_lambda_interior':bool(interior),
                    'implied_L2_F_best_lambda':round(bF + best_lam_cost, 4)},
    ...
    print(f"(b) uniform x0.25 {ref:+.4f} vs SS2896 {S2896_COST:+.4f}: {'HELD' if pb else 'FAILED'}")
    print("    lambda sweep: " + " | ".join(f"x{v} {lam_cost[v]:+.4f}" for v in LAM_GRID))
    print(f"(c) more ridge helps at all: best x{best_lam} at {best_lam_cost:+.4f} <= -0.05: {'HELD' if pc else 'FAILED'}")
    print(f"(d) ridge REACHES uniform shrinkage: {best_lam_cost:+.4f} <= {S2896_COST+BARS['reaches_uniform']:+.4f}: {'HELD' if pd else 'FAILED'}")
    print(f"(e) best lambda interior to {LAM_GRID}: {'HELD' if pe else 'FAILED'}")
    print("    held-out: " + " | ".join(f"{n} {cost_h[n]:+.4f}" for n,_ in ARMS if n!='baseline'))
    print(f"    RIDGE - UNIFORM = {best_lam_cost - ref:+.4f} nats (negative = ridge is BETTER than the adopted scalar)")
