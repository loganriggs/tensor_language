# DOES THE CE/TOP-1 SWAP REPRODUCE AT THE ORIGINAL COVERAGE?  -- completing the top-1 frontier.
#
# §1856 measured both metrics on the same forwards at 16,110 covered types and found they order the TOP
# of the ladder differently: CE says full rank beats rank 1024 (5.90522 vs 5.90669), top-1 says rank 1024
# beats full rank (13.9567% vs 13.9323% on skip7000, and the same sign on both other roles). Rank 1024 is
# also cheaper -- 641.4M reals against 673.5M -- so it DOMINATES full rank on top-1.
#
# That was measured at one coverage. §1853's and §1854's dominations of full@5,419 and full@9,054 are CE
# claims; whether the top-1 frontier agrees at those coverages is unmeasured. This run takes the original
# 5,419-type covered set -- §1834's, and the one every section from §1829 to §1846 used -- and asks
# whether the swap is a property of the ladder or of the large covered set.
#
# THE DOCSTRING AND THE PREDICATE BLOCK WERE REWRITTEN IN ONE EDIT (LESSONS 63): §1856's own defect was
# a fresh docstring over an inherited predicate block, and §1857's audit found 14 such lines across 9
# scripts. The gate check added there now covers this file.
#
# ROLES. All three. Ranks full/1024/512/384/256/64/16/4. DISCOVERY ONLY.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39,
# anchors on THIS run's 5,419-type covered population only:
#   pred_a THE SWAP REPRODUCES: at 5,419 covered types, rank 1024's all-position top-1 exceeds full
#          rank's on all three roles, as it did at 16,110. If TRUE the CE/top-1 disagreement is a
#          property of the rank ladder itself and no full-rank build is on the top-1 frontier at any
#          coverage measured. If FALSE the swap needs the large covered set, §1856's finding is
#          coverage-specific, and the top-1 frontier must be stated per coverage rather than generally.
#   pred_b AND FULL RANK IS DOMINATED IN TOP-1 HERE TOO: some rank cheaper than full rank's 230.087M
#          reaches a higher all-position top-1 on all three roles. If FALSE full rank survives on the
#          top-1 frontier at this coverage even if pred_a's swap is small, and the CE and top-1 frontiers
#          differ in WHICH points they exclude rather than in whether they exclude full rank.
#   pred_c AND TOP-1 IS MONOTONE UP TO RANK 1024, as at 16,110 -- the ladder should be clean everywhere
#          except the final step. If FALSE top-1 is too noisy at this eval size to price a frontier with,
#          which would retire the top-1 frontier as an instrument rather than qualify it.
#   pred_d CONTROLS, same population throughout: rank 64 reproduces §1849's PUBLISHED CE above the
#          ceiling at this 5,419-type covered set (+0.22327 / +0.23252 / +0.19774) within 0.005;
#          full-rank attainment is exact; coverage is 5,419; the ceiling is finite and above live.
...
OUT = PT + 'ops/frontier_top1_5419_results.json'
...
TRANKS = (None, 1024, 512, 384, 256, 64, 16, 4)          # None = full rank; §1786/§1813 use this ladder
...
# §1849's PUBLISHED rank-64 CE above the ceiling at THIS run's 5,419-type covered set
S1849_R64_5419 = {'skip7000': 0.22327, 'skip11000': 0.23252, 'skip1200': 0.19774}
...
FIT_ROWS = PT + '.rowcache/fineweb_n96_skip80.pt'   # §1834's covered set: 5,419 types   # 5x the fit rows -> a larger covered set
H = m.transformer.h
NCOV = 5419
...
def main():
    ...
    print(f'FRONTIER IN TOP-1, ORIGINAL COVERAGE | 5,419 covered types, eight ranks | '
    ...
    roles2 = [e for e in curve['full']]
    r64 = [e for e in roles2 if e in S1768_PROG_RANK64_CORR128]
    attain = all(abs(curve['full'][e]['cov'] - ceilv[e]) < 0.001 for e in roles2)
    lad = ['4', '16', '64', '256', '384', '512', '1024', 'full']
    above = {l2: {e: curve[l2][e]['cov'] - ceilv[e] for e in roles2} for l2 in lad}
    # COMPUTED from §1754's model, not hand-typed: three of my hand-typed entries were wrong, and at
    # this coverage rank 1024 costs MORE than the full table (1024 x 6571 > 5419 x 1152), which a wrong
    # cost column would have hidden from pred_b.
    COSTM = {l3: (36 * ((NFULL * D + D) if l3 == 'full' else (int(l3) * (NFULL + D) + 2 * D))
                  + 36 * MAP_RANK * 2 * D) / 1e6
             for l3 in ['4', '16', '64', '256', '384', '512', '1024', 'full']}
    t1 = {l2: {e: curve[l2][e]['top1_allpos'] for e in roles2} for l2 in lad}
    pa = all(t1['1024'][e] > t1['full'][e] for e in roles2)
    pb = any(COSTM[l2] < COSTM['full'] and all(t1[l2][e] > t1['full'][e] for e in roles2)
             for l2 in lad[:-1])
    upto = lad[:lad.index('1024') + 1]
    pc = all(t1[upto[i]][e] >= t1[upto[i - 1]][e] - 1e-12
             for e in roles2 for i in range(1, len(upto)))
    pd = (all(abs(above['64'][e] - S1849_R64_5419[e]) <= 0.005 for e in roles2)
          and NFULL == NCOV and attain
          and all(ceilv[e] > livev[e] for e in roles2))

    print(f'\n  LADDER at {NFULL} types -- cost, CE, top-1:', flush=True)
    for l2 in lad:
        print(f'    rank {l2:5s} {COSTM[l2]:8.2f}M  ' + '  '.join(
            f'{e} CE {curve[l2][e]["allpos"]:.5f} top1 {t1[l2][e]:.4%}' for e in roles2), flush=True)
    print(f'\n  the CE/top-1 SWAP reproduces at 5,419 types -> {pa}  ' + '  '.join(
        f'{e} 1024 {t1["1024"][e]:.4%} vs full {t1["full"][e]:.4%}' for e in roles2), flush=True)
    print(f'  and FULL RANK is dominated in top-1 here too -> {pb}', flush=True)
    print(f'  and top-1 is MONOTONE up to rank 1024 -> {pc}', flush=True)
    print(f'  rank 64 reproduces §1849, coverage {NFULL}, attainment {attain} -> control {pd}',
          flush=True)

    json.dump({'run': 'frontier_top1_5419', 'top1': t1, 'cost_M': COSTM, 'attainment_exact': bool(attain), 'n_covered': NFULL, 'ranks': [str(r) for r in TRANKS],
               'ceiling_covered': ceilv, 'live_covered': livev, 'curve': curve,
               'ladder_above_ceiling': above,
               'predictions': {'pred_a_swap_reproduces': bool(pa),
                               'pred_b_full_dominated_in_top1': bool(pb),
                               'pred_c_top1_monotone_to_1024': bool(pc),
