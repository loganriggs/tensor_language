...
"""Zero/one-attention derivative path expansion on opened v687 rows.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v690_result.json'
PREDICTIONS=dict(pred_a_exact_replay='original replay plus baseline tangent centralfinitechange relative error<=.01',
 pred_b_effect_prediction='at-most-one-attention expansion modalprediction<=.05 A/B everycell',
 pred_c_capable_position_transfer='some single-attention-layer correction modalprediction<=.05 A/B everycell')


def main():
    plan=dict(prefix_calls=8,suffix_calls=56,sequences_per_arm=48,fits=0,backwards=0,extra_attention_pullbacks=48,extra_reference_attention_forwards=96,extra_mlp_reader_triplet_equivalents=120,analytic_reverse_passes=4,attention_pullbacks=24,reference_attention_forwards=48,additional_tangent_suffix_calls=8,additional_initial_interface_suffix_calls=12,
    ...
    import hashlib
    rowfile=POLY/'SUBJECT_HYBRID_FRESH_V687_ROWS.json'
    assert hashlib.sha256(rowfile.read_bytes()).hexdigest()=='c0b8214f736ffc4aa77016a9364ac740b80c2a07f7f1010427a484577264e6e0'
    ...
    from selected_suffix_readers import baseline_reader,single_attention_terms
    ...
    path_specs=dict(carry=([],[]),mlp17=([5],[]),mlp16_17=([4,5],[]),all_mlp=(list(range(6)),[]),attention_only=([],list(range(6))))
    path_reports={name:{source:{} for source in ['A','B','sum']} for name in list(path_specs)+['full','one_attention_sum']+[f'one_attention_{k+12}' for k in range(6)]}
    path_support=[];expansion_checks=[]
    tangent_checks=[];tangent_calls=0;reader_prices=[]
    tangent_reports={mode:{name:{} for name in ['A','B','sum']} for mode in ['linear','corrected']}
    ...
    for entries in batches:
        ...
        path_readers={name:baseline_reader(blocks,tr,first.reshape(b,t,9,128).double(),reader_pairs[1:],readpos,eps,set(mlps),set(attns)) for name,(mlps,attns) in path_specs.items()}
        no_attention,terms=single_attention_terms(blocks,tr,first.reshape(b,t,9,128).double(),reader_pairs[1:],readpos,eps)
        expansion_checks.append(float((no_attention-path_readers['all_mlp']).abs().max()))
        path_readers['one_attention_sum']=no_attention+terms.sum(0)
        for k in range(6):path_readers[f'one_attention_{k+12}']=no_attention+terms[k]
        path_readers['full']=q0[1:]
        path_support.append({name:int((q.abs().sum((0,3))>0).sum()) for name,q in path_readers.items()})
        reader_prices.append(q0.numel())
        ...
        for source_name,delta in [('A',source_deltas[0]),('B',source_deltas[1]),('sum',source_deltas[0]+source_deltas[1])]:
            ...
            for path_name,pq in path_readers.items():
                path_prediction=-(pq*delta.double()).sum((-1,-2)).T
                for i,e in enumerate(entries):
                    pc=path_reports[path_name][source_name].setdefault(e[3],dict(target=[],native_modal=[],predicted_modal=[]))
                    pc['target'].append(float(native_effect[i]));pc['native_modal'].append(native_modal[i].tolist());pc['predicted_modal'].append(path_prediction[i].tolist())
            full_linear=(q0*delta.double()).sum((-1,-2)).T
            lost_linear=(q0*(delta.double()-source_z@programs[0]['input_basis'].T)).sum((-1,-2)).T
            for mode in ['linear','corrected']:
    ...
    for reports in path_reports.values():
        for report in reports.values():
            for cell in report.values():
                cell['modal_error']=(np.linalg.norm(np.array(cell['predicted_modal'])-cell['native_modal'],axis=0)/max(np.linalg.norm(cell['target']),1e-20)).tolist()
    passing_paths=[name for name,rs in path_reports.items() if all(max(c['modal_error'])<=.05 for source in ['A','B'] for c in rs[source].values())]
    a=a and max(expansion_checks)<=1e-10
    b=a and 'one_attention_sum' in passing_paths
    hybrid_reports={}
    for name in ['A','B','sum']:
        hybrid_reports[name]={}
        for family,cell in tangent_reports['corrected'][name].items():
            lc=tangent_reports['linear'][name][family]
    ...
    c=a and any(f'one_attention_{k+12}' in passing_paths for k in range(6))
    failed=[]

    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,cells=cells,
                predictions=dict(zip(PREDICTIONS,[bool(a),bool(b),bool(c)])),selectivity=selectivity,max_zero_attention_expansion_check=max(expansion_checks),path_specs=path_specs,path_reports=path_reports,passing_paths=passing_paths,path_support=path_support,hybrid_reports=hybrid_reports,tangent_reports=tangent_reports,max_tangent_relative_check=max(tangent_checks),reader_values_per_batch=reader_prices,tangent_suffix_calls=tangent_calls,initial_reports=initial_reports,initial_interface_suffix_calls=initial_counts,max_full_state_readout_error=max(oracle_checks),failed_modal_oracle_ratios=[o/e for o,e in failed],reuse_reports=reuse_reports,cross_effects=cross,reuse_counts=reuse_counts,pre11_joint_vs_post11_sum_gap=boundary_gaps,
