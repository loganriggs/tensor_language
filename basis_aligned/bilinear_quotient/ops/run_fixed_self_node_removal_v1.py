...
Null: shared fixed self-product cannot be removed without violating interaction preservation.
...
@torch.no_grad()
def main():
    files=json.loads((P/'FIXED_SELF_NODE_REMOVAL_V1_BINDING.json').read_text())['files']
    ...
    assert not (P/'FIXED_SELF_NODE_REMOVAL_V1_RESULT.json').exists()
    ...
    for index in range(160):
        ...
        fixed_product=diagonal['bank'][...,0,:].clone();diagonal['bank'][...,0,:]=0
        def post(z):
            z=z.float()
        ...
        for amplitude in (a,b,a+b):
            ...
            zgen=branch(amplitude,context).float().double();candidate=diagonal_post(zgen,amplitude,diagonal)
            ll,rr,dd=mlp_matrices;rho=zgen.square().mean(-1,keepdim=True)+torch.finfo(torch.float32).eps
            expected=zgen+((zgen@ll.T)*(zgen@rr.T))@dd.T/rho+b10.mlp.Down_bias.double()-.5*amplitude.square()*fixed_product/rho
            node_errors.append(rel(candidate,expected));generated.append(candidate.float())
        outcomes=[]
    ...
    result={'pred_a':max(node_errors)<=1e-10 and native_replay==0,
            ...
            'scope':'160 historical prefixes, full original two-edit interaction; generated child/remainder/parent branches, native pristine context/scalar fields/dense weights/suffix. Fixed self-product node removed from all changed branches; other five-bank terms, whole normalizer and background retained; no freshOOD, fitting or whole-model compression claim.'}
    (P/'FIXED_SELF_NODE_REMOVAL_V1_RESULT.json').write_text(json.dumps(result,indent=2)+'\n')
