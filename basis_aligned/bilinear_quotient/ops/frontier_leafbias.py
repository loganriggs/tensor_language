"""LEAF-CONDITIONED FINAL-STREAM BIAS VECTORS (rung 152): the bias-branch certificate candidate.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2246: the certificate statistic is
~69% signed bias at the median circuit. Candidate: 62 per-circuit bias vectors added to the block-17 output
at member positions, each fit as the mean (real - config) h17 drift over that circuit's TRAIN-half members
(census rows 0-499; >= 20 train members required). Certificates scored on the TEST half only (rows 500-999;
full-census battery refs; >= 20 test members). PRICE: 62 x 1152 = 71,424 values - DIAGNOSTIC-GRADE: leaf
membership is census-indexed, not computable on new text; a deployable version needs leaf classifiers.

REGISTERED PREDICTIONS:
  (a) CERTIFICATES MOVE: TEST-half valid >= 5 (every prior multi-site config: 0-2).
  (b) AGGREGATE UNHARMED: census <= 1.93.
  (c) ISOLATION: |L1F - 1.5509| <= 0.02 AND census >= 1.4.
NULL: member damage is not leaf-DC at the final stream (valid < 5) - the bias branch needs finer grain than
62 leaf vectors. Tripwire: INSTRUMENT FAIL if census within 1e-3 of +1.9474 (inert bias). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: leaf-conditioned final-stream biases')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_leafbias_results.json'
...
def main():
    ...
    if SEL.get('leaf_bias'):
        _HALF=SEL['half']
        def _caph17(_hooked):
            _cap=[]
            def h17(mo,i_,o_):
                _cap.append(o_[0].detach().reshape(-1,D).float())
            hs=(install(order2)+motif_hooks(ML)) if _hooked else []
            hs.append(m.transformer.h[17].register_forward_hook(h17))
            _TR=SEL.get('ext_rows',FR)[:500]
            if _hooked:
                cur['clsmap']=classify2(_TR).to(DEV)
            for _i in range(0,500,4):
                _bb=_TR[_i:_i+4,:257].to(DEV)
                cur['idx']=_bb[:,:-1].contiguous(); cur['mode']='oracle'
                if _hooked:
                    cur['lab']=cur['clsmap'][_i:_i+4].reshape(-1)
                m(cur['idx'],_bb[:,1:].contiguous())
            for h in hs: h.remove()
            if _hooked: del cur['clsmap']
            return torch.cat(_cap)
        _hcfg=_caph17(True)
        _hreal=_caph17(False)
        _drift=_hreal-_hcfg
        del _hcfg,_hreal
        _NF9=SEL['nflat9']
        _BI=torch.zeros(_NF9,D,dtype=torch.float16)
        _nfit=0; _nover=0; _seen=torch.zeros(_NF9,dtype=torch.bool)
        for _t,_mi in SEL['leafmembers'].items():
            _trm=_mi[_mi<_HALF]
            if _trm.numel()<20: continue
            _b=_drift[_trm].mean(0).cpu().to(torch.float16)
            _nover+=int(_seen[_mi].sum()); _seen[_mi]=True
            _BI[_mi]=_b; _nfit+=1
        SEL['_BI']=_BI
        print(f'  leaf biases fit for {_nfit} circuits; {int(_seen.sum())} biased positions; {_nover} overlaps',flush=True)
        def _bh(mo,i_,o_):
            if not SEL.get('lb_on'): return None
            _x,_v1=o_
            _B9=_x.shape[0]
            _sl=SEL['_BI'][cur['bbi']*1024:cur['bbi']*1024+_B9*_x.shape[1]].to(_x.device).float()
            cur['bbi']=cur['bbi']+1
            return ((_x.float()+_sl.view(_x.shape[0],_x.shape[1],D)).to(_x.dtype),_v1)
        m.transformer.h[17].register_forward_hook(_bh)

    if SEL.get('clsdmg'):
        _ROWS=SEL.get('ext_rows',FR)
        cur['clsmap']=classify2(_ROWS).to(DEV)
        if SEL.get('leaf_bias'):
            SEL['lb_on']=True; cur['bbi']=0
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['leaf_bias']=True; SEL['half']=NFLAT//2; SEL['nflat9']=NFLAT
    print('ARM: rung-127 build + 62 leaf-conditioned h17 bias vectors (train-half fit)',flush=True)
    BATC0=json.load(open(PT+'circuits/BATTERY.json'))['by_tag']
    LM={}
    for t0 in BATC0:
        try: lf0=CN.leaf(t0)
        except Exception: continue
        mi0=torch.as_tensor(lf0['member'],dtype=torch.long)
        if mi0.numel()>0: LM[t0]=mi0
    SEL['leafmembers']=LM
    main()
    ...
    HALF=NFLAT//2
    nvT=0; nT=0
    for r9,(t,v) in zip(rows,[(t,v) for t,v in BATC.items() if True]):
        pass
    nvT=0; nT=0
    for t,v in BATC.items():
        try: lf=CN.leaf(t)
        ...
        mm=torch.zeros(NFLAT,dtype=torch.bool); mm[lf['member']]=True
        mte=mm[HALF:]
        if int(mte.sum())<20: continue
        nT+=1
        mdT=float(d[HALF:][mte].abs().mean())
        ref=v['mean_ablation']['top'][0]['abs_dce_members']
        if mdT<0.5*ref: nvT+=1
    print(f'  TEST-half certificates: {nvT}/{nT} valid',flush=True)
    if abs(agg-1.9474)<1e-3:
        raise SystemExit('INSTRUMENT FAIL: leaf bias inert (census unchanged)')
    pa=nvT>=5
    pb=agg<=1.93
    pc=abs(L1F-1.5509)<=0.02 and agg>=1.4
    res={'L2F_fresh':round(L2F,4),'L1F':round(L1F,4),'increment':round(inc,4),'census_agg':round(agg,4),
         'n_valid_full':nv,'n_valid_test':nvT,'n_test_scored':nT,'circuits':rows,
    ...
    print(f"(a) TEST-half valid {nvT}/{nT} >= 5: {'HELD' if pa else 'FAILED'}")
    print(f"(b) census {agg:+.4f} <= 1.93 via L2F {L2F:+.4f}: {'HELD' if pb else 'FAILED'}")
