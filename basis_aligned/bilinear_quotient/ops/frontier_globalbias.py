"""GLOBAL h17 BIAS (rung 156): the operator-endorsed "optimal bias on all data," one vector.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2249: 62 per-circuit biases HURT
(+0.19 census). The legitimate object is the GLOBAL component-level bias: one h17 drift-mean vector (1,152
values) fit on the census TRAIN half, applied everywhere. Prediction is near-null by design - the global
drift mean should be small - and the contrast with S2249 is the point: shared beats specific.

REGISTERED PREDICTIONS:
  (a) NEAR-NULL: |census - 1.9474| <= 0.02.
  (b) NO HARM: census <= 1.9524 (the per-circuit version added +0.19).
  (c) ISOLATION: |L1F - 1.5509| <= 0.02 AND census >= 1.4.
NULL: even the global bias hurts (> 1.9524) - h17 DC additions are frame-illegal like the S2224 span edits.
PRICE: 1,152 values. Tripwire: INSTRUMENT FAIL if census within 1e-3 of +1.9474... waived here: a genuine
near-null IS the registered pred_a, so the inertness tripwire is REMOVED for this rung (recorded).
Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['frontier_leafbias_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: global h17 bias')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_globalbias_results.json'
...
def main():
    ...
    if SEL.get('leaf_bias'):
        ...
        del _hcfg,_hreal
        _NF9=SEL['nflat9']
        _bg=_drift.mean(0).cpu().to(torch.float16)
        _BI=_bg.unsqueeze(0).expand(_NF9,D).contiguous()
        SEL['_BI']=_BI
        print(f'  GLOBAL h17 bias fit (norm {float(_bg.float().norm()):.3f}) applied at all positions',flush=True)
        def _bh(mo,i_,o_):
...
if __name__=='__main__':
    ...
    print('ARM: rung-127 build + ONE global h17 bias vector (train-half fit)',flush=True)
    ...
    print(f"(a) |census - 1.9474| <= 0.02; TEST valid {nvT}/{nT}: {'HELD' if pa else 'FAILED'}")
    print(f"(b) census {agg:+.4f} <= 1.9524 (no harm) via L2F {L2F:+.4f}: {'HELD' if pb else 'FAILED'}")
