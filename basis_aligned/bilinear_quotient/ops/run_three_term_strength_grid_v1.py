#!/usr/bin/env python3
# BQGATE:52prefixes;2304suffixes;240seconds.
"""pred_a anchors hold, unit-strength reference replay<=1e-6, zero identities exact.
pred_b each family/nonzero pair native error<=2%target/5%control.
pred_c each family/nonzero pair incremental error<=1%target/2.5%control.
pred_d no material target/control sign flips, native magnitude>=1e-5.
...
@torch.no_grad()
def main():
 files=json.loads((P/'THREE_TERM_STRENGTH_GRID_V1_BINDING.json').read_text())['files']
 ...
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('52prefixes;2304suffixes;240seconds');return
 out=P/'THREE_TERM_STRENGTH_GRID_V1_RESULT.json';assert not out.exists()
 ...
 strengths=(-1,0,1,2);grid=[(0,0)]+[(s,t) for s in strengths for t in strengths if (s,t)!=(0,0)];lookup={pair:i for i,pair in enumerate(grid)}
 def post(z):
 ...
 for index,row in enumerate(rows):
  ...
  for alpha,beta in grid[1:]:
   amplitude=alpha*a+beta*b
  ...
  scores=torch.tensor(outcomes,dtype=torch.float64).reshape(3,16,2)
  effects=torch.stack([scores[:,lookup[(s,t)]]-scores[:,lookup[(s,0)]]-scores[:,lookup[(0,t)]]+scores[:,0] for s,t in grid],dim=1)
 ...
 for family in range(4):
  sub=cells[family*12:(family+1)*12];effects=torch.tensor([c['interactions'] for c in sub],dtype=torch.float64)
  for j,(alpha,beta) in enumerate(grid):
   n,e,c=effects[:,:,j].unbind(1)
   if alpha==0 or beta==0:
    zero_error=max(zero_error,float(effects[:,:,j].abs().max()));continue
   groups.append(dict(family=family,strengths=[alpha,beta],compressed_relative_errors=[rel(c[:,k],n[:,k]) for k in range(2)],exact_relative_errors=[rel(e[:,k],n[:,k]) for k in range(2)],incremental_relative_errors=[rel(c[:,k],e[:,k]) for k in range(2)],reference_norms=n.norm(dim=0).tolist(),material_sign_flips=(((n*c)<0)&(n.abs()>=1e-5)).sum(0).tolist()))
 previous=json.loads((P/'THREE_TERM_FRESH_TEMPLATE_V1_RESULT.json').read_text())['cells'];corner_indices=[lookup[x] for x in [(0,0),(1,0),(0,1),(1,1)]]
 replay=max(abs(x-y) for c,old in zip(cells,previous) for arm in range(3) for old_index,new_index in enumerate(corner_indices) for x,y in zip(c['scores'][arm][new_index],old['scores'][arm][old_index]))
 result={'pred_a':A and replay<=1e-6 and zero_error==0,'pred_b':all(g['compressed_relative_errors'][0]<=.02 and g['compressed_relative_errors'][1]<=.05 for g in groups),'pred_c':all(g['incremental_relative_errors'][0]<=.01 and g['incremental_relative_errors'][1]<=.025 for g in groups),'pred_d':all(sum(g['material_sign_flips'])==0 for g in groups),'anchors':anchors,'grid':grid,'unit_strength_reference_replay_maxabs':replay,'zero_interaction_maxabs':zero_error,'groups':groups,'cells':cells,'seconds':time.perf_counter()-tic,'scope':'48 previously fresh prompts, reused for signed strength transfer; {-1,0,1,2} independent multipliers. One context and16joint branch outputs reused per variant for16interaction queries. Zero identity follows shared-state lookup, not an independent precision test. Native states/background/suffix retained; not new text/corpus OOD or arbitrary-composition certificate.'}
