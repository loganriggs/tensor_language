...
STEM='TYPED_FACE_CONTEXT_GENERATED_V1';ARMS=['native','native8_midpoint','compiled']
PACKAGE=P/'extracted_circuits/typed_face_context_generated_v1'
...
def main():
 ...
 def pre8(module,args):
  if state['arm']=='native':
   c=cache[state['i']];c['mixed']=(module.lambdas[0]*args[0]+module.lambdas[1]*args[2]).clone();c['rho8']=(c['mixed'].double().square().mean(-1,keepdim=True)+torch.finfo(torch.float32).eps).sqrt()
 ...
 def pre9(module,args):
  ...
  if arm=='native':c['x9']=x.clone()
   ...
   inputs={'current8':c['current'],'donor_city8':cache[i^1]['current'][:,city],'rho8':c['rho8'],'token_ids':torch.tensor([row['ids']],device=x.device),'recipient_token':row['ids'][city],'donor_token':groups[i^1]['ids'][city],'city':city,'destination':c['mask']}
 ...
 handles=[model.transformer.h[8].register_forward_pre_hook(pre8),model.transformer.h[8].attn.register_forward_pre_hook(attnpre),model.transformer.h[8].attn.register_forward_hook(attnpost),model.transformer.h[9].register_forward_pre_hook(pre9)]
 ...
 for family in doc['variants']:
  idx=[i for i,r in enumerate(rows) if r['variant']==family];actual=e[1,idx];prediction=e[2,idx]
  records[family]=((prediction-actual).norm(dim=0)/actual.norm(dim=0).clamp_min(1e-30)).tolist()
 diff=v[2]-v[1];abs_error=float(diff.abs().max());relative_error=float(diff.norm()/v[1].norm())
