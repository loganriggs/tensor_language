...
"""Coordination agreement were/was (and vs or) reuse census (v53): blind readout sweep + top-4 set on the line's authored A1/A2 contexts (opened rows).
Contrast `O_h^T(u_were - u_was)`. Predictions as in `dod_reuse_census.run`; prior for pred_e: 11.3 yes (number), temporal relay no.
PRICE: 181 forwards per 32-row batch (registered in dod_reuse_census)."""
import circuit_fast_screen_candidate_coordination_agreement as m
...
rows, pos, neg = R.rows_from_candidate(m, " were", " was", "coordination_agreement")
if __name__ == "__main__":
    R.run("coordination_agreement.and_vs_or.dod_reuse_census_v53", "coordination_agreement_dod_reuse_census_v53_result.json", rows, pos, neg)
