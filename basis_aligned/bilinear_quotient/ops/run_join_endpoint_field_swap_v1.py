#!/usr/bin/env python3
"""Test whether endpoint embedding interchange retargets two forward joins.
...
pred_c_reuse: own single transfers meet retarget bars; otherquery goldP change<=.1.
...
7arms,B4FP64,1800s,<256MiB/tensor;managedGPU. Exported387968 native constants,
...
OUT=POLY/'JOIN_ENDPOINT_FIELD_SWAP_V1_RESULT.json';ROWS=POLY/'JOIN_ENDPOINT_FIELD_SWAP_V1_ROWS.pt'
...
BOUND=[POLY/n for n in ('join_endpoint_field_swap_reference.py','join_contribution_context_reference.py',
                       'exact_source_edit_reference.py','JOIN_ENDPOINT_FIELD_SWAP_V1_PREREGISTRATION.md')]+[ROOT/'model.py',ROOT/'deep_model.py']
EXPECTED='2d86bc88224e680300ae0976e2bfe495cec097cd2dd139bc90ea58fe78ed70c6'
...
def edits(own,swapped,full):
    ...
    return {'native':zero,'identity':-own[0]-own[1]+own[0]+own[1],
            'swap_a':a,'swap_b':b,'both_swap':a+b,'cut_a':-full[0],'cut_b':-full[1]}
...
def run(torch,R,E,J,checks):
    ...
    with torch.inference_mode():
        for pop,worlds in R.populations().items():
            ...
            for query in range(2):
                ...
                single='swap_a' if query==0 else 'swap_b'
                other_single='swap_b' if query==0 else 'swap_a'
                groups[str(query)]={'native_accuracy':float((logits['native'][:,-1].argmax(-1)==answer)[select].double().mean()),
                                    ...
                                    'other_single_gold_change':float((gold[other_single]-gold['native'])[select].mean())}
            lower={}
            for q in range(2):
                for h in range(3):
    ...
    pred_c=pred_a and all(q['single_target_accuracy']>=.8 and q['single_target_probability_gain']>=.5 and abs(q['other_single_gold_change'])<=.10 for q in groups)
    ...
    receipt={'experiment':'join_endpoint_field_swap_v1','runner_sha256':digest(SOURCE),'bound_sha256':EXPECTED,'package_sha256':EXPECTED_PACKAGE,
             ...
             'terminal':'instrument_invalid' if not pred_a else ('endpoint_field_interchange_supported' if pred_b and pred_c and pred_d else 'endpoint_field_interchange_not_established'),
