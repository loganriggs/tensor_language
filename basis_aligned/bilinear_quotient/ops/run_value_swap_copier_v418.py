#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_value_swap_moves_margin pred_c_key_swap_moves_little pred_d_value_swap_beats_key_swap_on_most_pairs pred_e_effect_holds_on_panel
"""The copier's value edited (v418). v409 / v410: head 4.5's write at the post-noun token is a value copy of the noun's state; v383-v385: the post-noun
number state is rebuilt by MLPs 4-8 from a small attention seed; v417: the readers' two-site value swap closes half the margin. Here the replace-edit one
step upstream: swap ONLY 4.5's value vector at the noun position between the plural and singular rows (its patterns untouched) and let everything downstream
(MLPs 4-8 at the post-noun token, the readers) recompute; keys-only as the control. How much of the they - he margin gap follows the copier's value?
PREDICTIONS (scored as written; failures preserved; priors from v385 / v409)
    pred_a_baseline_replays                   the unedited manual forward replays the model's they - he margins within 1e-3 (max abs over rows)
    pred_b_value_swap_moves_margin            swapping 4.5's value at the noun closes >= 0.10 of the pair's margin gap, pooled over text pairs. Prior: unsure (v385: the seed is small, the rebuild large)
    pred_c_key_swap_moves_little              the key swap closes <= 0.05 of the gap, pooled over text pairs
    pred_d_value_swap_beats_key_swap_on_most_pairs  |value-swap effect| > |key-swap effect| on >= 0.70 of the text pairs
    pred_e_effect_holds_on_panel              the value swap closes >= 0.10 of the gap on the panel pairs too
...
OUT = ROOT / "circuits/followups/value_swap_copier_v418_result.json"
CANDIDATE_ID = "both_ends.value_swap_copier_v418"
N_HEAD = 9
READERS = ((4, 5),)
REPLAY_TOL, VALUE_MIN, KEY_MAX, MOST_MIN = 1e-3, 0.10, 0.05, 0.70
FORWARDS_MAX = 22
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_value_swap_moves_margin": ">= 0.10", "pred_c_key_swap_moves_little": "<= 0.05", "pred_d_value_swap_beats_key_swap_on_most_pairs": ">= 0.70", "pred_e_effect_holds_on_panel": ">= 0.10"}
...
def main() -> None:
        ...
        with torch.no_grad():
            ...
            for l, block in enumerate(blocks):
                ...
                if l in heads_at and mode != "native":
                    ...
                    if mode == "value":
                        v = v.clone(); v[idx, pp, h] = v[swap, pp, h].clone()
                    else:
                        k = k.clone(); k2 = k2.clone(); k[idx, pp, h] = k[swap, pp, h].clone(); k2[idx, pp, h] = k2[swap, pp, h].clone()
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_value_swap_moves_margin": text["value_closed"] >= VALUE_MIN, "pred_c_key_swap_moves_little": abs(text["key_closed"]) <= KEY_MAX,
                   "pred_d_value_swap_beats_key_swap_on_most_pairs": text["value_beats_key_share"] >= MOST_MIN, "pred_e_effect_holds_on_panel": panel["value_closed"] >= VALUE_MIN}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "value_swap_copier_result_v418", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
