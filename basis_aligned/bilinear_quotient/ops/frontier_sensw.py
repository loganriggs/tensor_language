"""SENSITIVITY-WEIGHTED TAIL-DICT FITS (rung 135; math review 1008): GN-weighted estimation of the dictionaries.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. All dictionary fits so far minimize
UNWEIGHTED output MSE; the loss metric is Gauss-Newton, and constant output metrics provably do not change
multi-output LSQ - only PER-POSITION weights do. Here per-position sensitivity weights w_t = |dCE/dh| at the
block-10 input (one real-model backprop over the fit rows, true targets) enter the aXL CV class means
(weighted means differ within class) and LW ridge fits. CLOSED-LIST DISTINCTION: this is per-position
weighting of DICTIONARY PARAMETER ESTIMATION - not Fisher unit-selection (S2125, closed) and not metric-K
reduction (S2118, closed). Everything else = the exact rung-127 frontier build.

REGISTERED PREDICTIONS (anchors: rung-127 receipt +1.9474 census / +0.3256 increment / L1F +1.5509):
  (a) census agg <= 1.92 (weighting buys >= 0.03).
  (b) tail-attn increment <= 0.30.
  (c) L1F invariant (|L1F - 1.5509| <= 0.02) AND census >= 1.4.
NULL: the 10-class structure already captures loss sensitivity (gain < 0.01). PRICE: none (same shapes).
Tripwire: INSTRUMENT FAIL if the rung-131 inert signature appears. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: sensitivity-weighted tail-dict fits')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_sensw_results.json'
...
def main():
    ...
    if SEL.get('sens_w'):
        ...
        for _i in range(CA,CB,4):
            _bb=FW[_i:_i+4,:257].to(DEV)
            _idx=_bb[:,:-1].contiguous(); _tg=_bb[:,1:].contiguous()
            with torch.enable_grad():
                x=F.rms_norm(m.transformer.wte(_idx),(D,)); x0=x; v1=None; leaf=None
                for _lj,blk in enumerate(m.transformer.h):
                    if _lj==10:
                        x=x.detach().requires_grad_(True); leaf=x
                    x,v1=blk(x,v1,x0)
                lg=(30*torch.tanh(m.lm_head(F.rms_norm(x,(D,)))/30)).float()
                F.cross_entropy(lg.view(-1,lg.size(-1)),_tg.reshape(-1)).backward()
            _ws.append(leaf.grad.detach().reshape(-1,D).norm(dim=1))
            m.zero_grad(set_to_none=True)
        _w=torch.cat(_ws); _w=(_w/_w.mean()).clamp(max=10.0)
        SEL['WSEN']=_w
        print(f'  sensitivity weights: p90 {float(_w.quantile(0.9)):.2f}, max {float(_w.max()):.2f}',flush=True)
    ...
    for li in range(10,18):
        ...
        if SEL.get('sens_w'):
            ...
            for k in range(10):
                ...
                if int(_sel.sum())>0:
                    _wk=_wA[_sel][:,None]
                    _cvl.append((Y[_sel]*_wk).sum(0)/_wk.sum())
                else: _cvl.append(Y.mean(0))
            CV=torch.stack(_cvl)
        else:
            CV=torch.stack([Y[flatA==k].mean(0) if (flatA==k).sum()>0
                            else Y.mean(0) for k in range(10)])
        ...
        for k in LINK:
            ...
            l2=1e-2*max(len(Xk),1)
            if SEL.get('sens_w'):
                _wk=SEL['WSEN'][mk_][:,None]
                LW[k]=torch.linalg.solve((Xk*_wk).T@Xk+l2*torch.eye(D,device=DEV),
                                         (Xk*_wk).T@Yk)
            else:
                LW[k]=torch.linalg.solve(Xk.T@Xk+l2*torch.eye(D,device=DEV),
                                         Xk.T@Yk)
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['tail_traj']=True; SEL['sens_w']=True
    print('ARM: rung-127 build + sensitivity-weighted aXL dictionary fits',flush=True)
    main()
    ...
    if abs(agg-1.9474)<1e-4 and abs(L2F-1.8765)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: weights inert (rung-131 signature)')
    ...
    print(f"(b) increment {inc:+.4f} <= 0.30: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |L1F {L1F:+.4f} - 1.5509| <= 0.02, census >= 1.4: {'HELD' if pc else 'FAILED'}")
