...
"""Two-site post11 subject/attractor response composition on opened same-number rows.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v699_result.json'
...
def main():
    ...
    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,cells=cells,
                ...
                scope='Post11 sum ofsubject/attractor responsevectors (not jointlyeditedprefix or pre11composition). Same-number semantic non-subject noun control on48opened v697rows; sameembedding-number removal atpairedattractor, A/Bsourceports patchedonlyatthatpre11site. Native subject labels fixed. Not normmatched tosubject (inputnormsreported). All native context/reader/sourcegenerators charged; no fullmodel adoption.')
