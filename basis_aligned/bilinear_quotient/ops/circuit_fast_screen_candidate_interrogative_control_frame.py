"""interrogative_licensing with a SECOND same-answer control frame, to isolate a confound.

At 11:33Z I reported that the same-answer control statistic orders by dependency type:
obligations 0.034-0.080, narrative_tense 0.142, and the two licensing behaviours highest at
0.173 and 0.230. But those two licensing screens share BOTH their answer vocabulary (" any" /
" some") AND their control frame ("The leader hoped to manage" / "The member offered to
manage"), so the ordering could belong to either, or to the dependency type. This isolates the
frame.

A1, A2 and P are byte-identical to the v1 authority. Only C changes, from an infinitival frame
to a locative transitive one:

    v1   "The leader hoped to manage"        /  "The member offered to manage"   -> " some"
    here "Beside the harbor the leader gathered" / "Inside the market the member gathered" -> " some"

Both still answer " some" and both sides of a row still end on the same token.

PREDICTION, registered before the run: if the control frame is what produced C 0.230, C falls
into the 0.034-0.080 obligation cluster here. If C stays above 0.15, the frame is not the cause
and the live explanations are the vocabulary or the dependency type -- which the next screen, an
obligation on the any/some vocabulary, would separate.
...
TASK_ID = "interrogative_licensing.control_frame_variant"
MOOD = (" any", " some")
...
TASK_SPEC = battery.BatteryTaskSpec(
    ...
    transforms=(
        ...
        battery.TransformSpec("C", "locative_transitive_same_answer_frame", False, "registered_active"),
    ),
)
...
def _control(agent: str, verb: str, first: bool, index: int = 0) -> str:
    """A locative transitive frame, replacing v1's infinitival one. Still answers " some".

    The only difference from the v1 authority; A1, A2 and P are untouched.
    """
    place = _PLACES[(index + (0 if first else 5)) % len(_PLACES)]
    where = "Beside" if first else "Inside"
    return f"{where} the {place} the {agent} {_participle(verb)}"
...
def _panel(seed: int, group_number: int, case_index: int) -> list[dict[str, Any]]:
    ...
    return [
        ...
        builder._row(**common, transform_id="C",
                     construction_id="locative_transitive_same_answer_frame",
                     direction_id="base_to_donor" if forward else "donor_to_base",
                     matched_suffix=f" {_participle(verb)}",
                     base_text=_control(agent if forward else alternate, verb, True, case_index),
                     donor_text=_control(alternate if forward else agent, verb, False, case_index),
