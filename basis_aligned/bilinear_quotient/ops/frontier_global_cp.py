"""THE CP-FRONT GLOBAL CONFIG (rung 122): swap tables -> CP-3456 inside the FULL S2144 config.

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows; LOWER IS BETTER.
S2192 measured the full S2144 config at +2.8553 census aggregate, 0/62 (rung-99 receipt = the registered
anchor; no repro arm). S2198 showed the front tables carry ~1.75 of it; S2208 showed a CP-3456 front costs
+0.1193 alone. Here the four front MLP table/linear entries inside cfgF are REPLACED IN PLACE by CP top-3456
(weights-only) BEFORE the tail-attention dictionaries are refit, so a10-17L condition on the swapped front.
Stated limitation: tailE (tail-MLP span dicts) and the front residual spans remain table-front-fit inside
build_arm - a frame approximation worth ~0.14 (S2198).

Everything else exactly S2144: motif heads at blocks 2-9, c4/c5 at K=2304, c6/c7 at 576, c8/c9 at 288, full
tail attention dictionaries, tailE.

REGISTERED PREDICTIONS (arm named swap; anchor = rung-99 receipt +2.8553):
  (a) NEW GLOBAL LEVEL: agg(swap) <= 1.6 (>= 1.25 census nats recovered by the front swap).
  (b) CERTIFICATES MOVE OFF ZERO: valid(swap) >= 3 of 62.
  (c) SANITY/ANTI-INERTNESS: agg(swap) in [0.8, 2.2] AND agg(swap) >= 0.62 (front-alone +0.1193 + 0.5: the
      motifs/middles/tail must still add real damage - a bare-front result here means an inert config).
NULL: (a) interactions between the CP front and motifs/tail eat the gain; (b) certificates stay 0-2 (S2208's
lesson). PRICE: front stores 47.8M values vs the tables' 231.6M - the global config gets cheaper AND better
if (a) holds. FR fresh-window L2 is printed descriptively (frontier comparison next rung if warranted).
Tripwire: INSTRUMENT FAIL on cev length mismatch. Self-reviewed."""
import json, sys, time, os
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
if os.environ.get('BQLIB_DRYRUN')=='1':
    _bq='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
    _need=['frontier_cp_front2_results.json','frontier_certificate_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: CP-front global config')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_global_cp_results.json'
...
if __name__=='__main__':
    ...
    ANCH=json.load(open(PT+'frontier_certificate_results.json'))['aggregate_mean_dce']
    ...
    SEL['cp_swap']=3456
    print('ARM: full S2144 config with CP-3456 front swap - census rows',flush=True)
    main()
    cev=SEL['cev']
    ...
    for t,info in CINFO.items():
        md=float(d[info['mask']].abs().mean())
        rows.append({'tag':t,'ref':round(info['ref'],3),'member_absdce':round(md,4),
                     'valid':bool(md<0.5*info['ref'])})
    nv=sum(1 for r in rows if r['valid'])
    pa=agg<=1.6
    pb=nv>=3
    pc=0.8<=agg<=2.2 and agg>=0.62
    res={'agg':round(agg,4),'n_valid':nv,'anchor_rung99':ANCH,'front_values':4*3456*3456,
         'circuits':rows,
         'convention':'per-position dCE = CE(S2144 config with CP-3456 front) - CE(real model) on census rows; lower is better',
         'pred_a_new_global_level':bool(pa),'pred_b_certs_move':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'CP-front global config: agg {agg:+.4f} (rung-99 anchor {ANCH:+.4f}); valid {nv}/{len(rows)}')
    print(f"(a) agg {agg:+.4f} <= 1.6: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 3: {'HELD' if pb else 'FAILED'}")
    print(f"(c) agg in [0.8, 2.2] and >= 0.62: {'HELD' if pc else 'FAILED'}")
