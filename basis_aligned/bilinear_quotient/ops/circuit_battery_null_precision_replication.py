"""circuit_battery_null_precision_replication -- attn8's percentile missed its bar by .014; does that survive more data?

SS2862 computed the v6 selectivity metric with all 36 components standing in as the writer, at PER_CELL=16, and found attn8's median
percentile among live components was **.214** against a bar of <= .20 -- pred_a FALSE **by .014**. Everything else held: the null spread
was 1.457, a median of 6 live components cleared the retired .25 bar, and the component ranking reproduced at Spearman .596.
...
(SS2852's value, 1.5x the rows), everything else identical and **bars unchanged** -- re-registering them would make the replication
meaningless. If .214 moves below .20 the original was a sampling artefact and the ledger will record the reversal; if it holds or rises,
attn8 is genuinely not the outlier I predicted, and that is the more interesting outcome.
...
Preregistration: polynomial_causal/CIRCUIT_BATTERY_NULL_PRECISION_PREREGISTRATION.md
"""
...
SELF = Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/"
            "circuit_battery_null_precision_replication.py")
...
ROOT = R.ROOT
PREREG = R.POLY / "CIRCUIT_BATTERY_NULL_PRECISION_PREREGISTRATION.md"
...
RUNG = "circuit_battery_null_precision_replication"
SMOKE = os.environ.get("SURROGATE_SMOKE") == "1"
OUT = ROOT / (f"{RUNG}_smoke_results.json" if SMOKE else f"{RUNG}_results.json")
HASHES = {PREREG: "5875eaac518d5dba8a1e7ce486d14eebfce0c2aa476787d4dc821112efa55883", V6RES: "940c1dc148d89d6458c6eecf1052f8f49701d1ffa149a8b3a0bffc4bd28d7e74",
