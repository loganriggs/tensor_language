...
"""Selection DoD (v261b; v261 crashed on a zero contrast -- the aligned pairs share the verb, so the verb is not the cue): the selection family at MLP-8 unit grain. The CUE is the position where the two members of a pair differ (the adjective, e.g. interested / afraid, which selects the particle in / of). Readers {8.8, 6.3, 13.8, 7.8} read the verb (remained / looked / grew) -> the selected particle
(v85); 13.8 is the family's contextual reader. Exact census (`dod_units.unit_census`) of MLP 8's write on 13.8's weight-only reader direction at the CUE (the adjective)
...
OUT = ROOT / "circuits/followups/selection_dod_mlp8_unit_census_v261b_result.json"
