...
pred_b EACHtemplate value-only serial effect error<=10%relative.
pred_c EACHtemplate routing-only serial effect error>=25%relative.
...
STEM='SCALAR_PRODUCER_JOINT_QK_VALUE_EFFECT_V1'
@torch.no_grad()
def main():
 ...
 if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('240bodyforwards48rows; joint-QK/value generated-component effect validation');return
 ...
 predictions=torch.load(P/'SCALAR_PRODUCER_JOINT_QK_VALUE_V2_ARTIFACT.pt',weights_only=True,map_location='cpu')['scalars'].cuda()
 ...
 result={'pred_a':A,'pred_b':A and all(c['serial_effect_errors'][1]<=.1 for c in cells),'pred_c':A and all(c['serial_effect_errors'][2]>=.25 for c in cells),'checks':checks,'cells':cells,'arms':['native','dynamicjoint','joint_field','value_field','routing_field'],'body_forwards':counts,'seconds':time.perf_counter()-tic,'scope':'Conditional joint-QK/value generation tested at signed native serial-effect boundary; removed component field is changed, not the full head routing. Reused contexts; no newline repair or global extraction.','source_shas':binding}
