...
TERMINAL_COMPLEMENT_SOURCE_V1_PREREGISTRATION.md fixes semantics.
"""
...
OUT=POLY/'TERMINAL_COMPLEMENT_SOURCE_V1_RESULT.json';BIND=POLY/'TERMINAL_COMPLEMENT_SOURCE_V1_BINDING.json'
def main():
    ...
    engine=ScalarWriteNetwork(backend,e,16,256);visits=[0,0,0];checks={};reports={};saved={}
    def perp(x):
    ...
    def body(rows,side,label,scalars=None,restore=None,own_cache=None):
        ...
        hooks=[mlp.register_forward_pre_hook(inp),mlp.Down.register_forward_pre_hook(product),mlp.register_forward_hook(clamp),model.transformer.h[0].attn.register_forward_hook(first)]
        ...
        for name in ['A1','A2']:
            ...
            for label,v in arms.items():
                ...
                if label in ['B','X','Y','FB','FX']:
                    ...
                    err=(predicted-actual).abs();checks[name+'_'+label+'_fold']=dict(max_abs=float(err.max()),max_scaled=float((err/(1e-4+1e-5*actual.abs())).max()))
                saved[name]['states'][label]=dict(h=v['h'].cpu(),r=v['raw17'].cpu(),m=m.cpu(),u=v['u'].cpu(),products=v['products'].cpu(),selected_scores=score.cpu())
    ...
    valid=engine.valid() and visits==[16,16,16]
    for k,v in checks.items():
        if k.endswith('_fold'):valid=valid and v['max_scaled']<=1
    ...
    artifact=POLY/'TERMINAL_COMPLEMENT_SOURCE_V1_STATES.pt';assert not artifact.exists();torch.save(saved,artifact)
    result=dict(schema='terminal.complement_source.v1',predictions={'pred_a_instrument':bool(valid),'pred_b_mlp_complement_sufficient':bool(passes('01')),'pred_c_carried_complement_sufficient':bool(passes('10'))},reports=reports,checks=checks,engine_bridges=engine.bridges,scalar_checks=engine.scalar_checks,visits=visits,
        artifact_sha256=digest(artifact),runner_sha256=digest(RUNNER),binding_sha256=digest(BIND),wall_seconds=time.perf_counter()-tic,price=dict(body_forwards=engine.counts[0],sequences=engine.counts[1],physical_readouts=16,native_parameters=sum(p.numel() for p in model.parameters()),artifact_bytes=artifact.stat().st_size,native_weight_saving=0),scope='Conditional complementary-source sufficiency on opened rows. Full native contexts and weights retained. Old capability, selectivity and joint-choice failures unchanged.')
