...
"""v359: four more preposition frame copies against their own frame-A donors, in an eight-member purpose-built family.

THE DESIGN FOLLOWS TWO MEASUREMENTS RATHER THAN CONVENIENCE. v347 showed control-set SIZE is inert (fifteen prepared
trimmed to eight changed no verdict and no margin by more than 0.0096); v345 showed that what degrades a margin is
having your own MAPPING-SHARING relatives present, with repl_frame_fresh -- seven same-pair siblings, zero shared
mappings, 0.0114 -- as the within-rung control. So this family is exactly the four new cells and the four counted
frame-A circuits whose cue -> token mappings they borrow, and nothing else. The four new cells are mapping-disjoint
from each other, so they contribute no mutual interference; each one's only relative in the room is its own donor,
which is precisely the cell that must fail to collide for the frame account to hold:
    verb_preposition_fb_from_about    suffered/boasted    frame B  <- verb_preposition_from_about   (A, counted)
    verb_preposition_fb_into_with     crashed/flirted     frame B  <- verb_preposition_into_with    (A, counted)
    verb_preposition_fc_of_into       reeked/peered       frame C  <- verb_preposition_of_into      (A, counted)
    verb_preposition_fc_through_with  rummaged/tinkered   frame C  <- verb_preposition_through_with (A, counted)
All four passed all four rows at v357 (n 14, 12, 12, 15; held 0.897, 0.914, 0.898, 0.894). Eight prepared, about six
minutes at the rate v355 measured (341 s for the same shape).
WHY IT MATTERS BEYOND FOUR CIRCUITS. v355 put four preposition frame copies through this exact design and all four
came out separable (fam-arm leaks 0.0091, 0.0223, 0.0078, -0.0055) with own-arm leaks into the same donors of 0.2869
to 0.9100. The account now rests on nine mapping pairs; these four take it to thirteen. The value of continuing is no
longer confirmation -- it is that each new pair is an independent chance for the account to FAIL, and a single fused
copy would bound it by mapping pair, which is why pred_e is registered as a MAXIMUM over the four rather than a mean.
Preposition frame copies are 10 of 14 at the battery stage; the four failures were two row-4 misses and two row-2
misses, so the battery is still filtering and these four are not a foregone conclusion.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
    pred_a_new_separable   at least k_new = 3 of the 4 new members are separable at the registered cross bar.
                                                                                                          prior 75%
    pred_b_own_leak_large  in the OWN arm -- own C only, no sibling controls -- the SMALLEST leak from any of the four
                           cells into its own frame-A donor is still large:
                           min over the four of |own-arm leak into own donor| >= own_leak_min = 0.20.
                           This keeps pred_e honest. "Separable" is only interesting when the controls did real work;
                           if a cell never touched its donor uncontrolled, its verdict says nothing about frames.
                           v341 measured own-arm donor leaks of 0.6577 and 1.0611 against fam-arm leaks of 0.0026 and
                           0.0243; v351 measured 0.7828 against -0.0095. Worked example: own-arm leaks of 0.55, 0.80,
                           0.35 and 0.22 give a minimum of 0.22 >= 0.20, TRUE; if one came in at 0.05 the minimum is
                           0.05, FALSE, and that cell's separability would be uninformative even though its verdict
                           reads the same as the others'.                                                 prior 70%
    pred_c_row4_kept       fam-arm own-C UB975 <= 0.01 on at least k_d of the evaluated members. v351 missed this by 0.0002 on one member and
                           v355 passed it, so it is not a formality.                                prior 65%
    pred_d_instrument      every evaluated member's own-arm extraction is within 0.03 of its v357 cdas
                           extraction_held. Fails automatically if any member errors.                     prior 90%
    pred_e_donors_below    the LARGEST fam-arm leak from any of the four cells into its OWN donor stays under
                           cross_max = 0.05 in absolute value: max over the four of |leak into own donor| < 0.05.
                           On the FAM arm, where every clause of mine that succeeded today was registered. Worked
                           example: donor leaks of 0.01, 0.02, 0.01, 0.03 give 0.03, TRUE, all four mapping pairs
                           survive the frame change; one at 0.09 -- near v333's within-frame fusion of 0.0921 and
                           0.1163 -- gives FALSE, and that pair is the first to fail it, which is a bound on the
                           account by mapping pair rather than a null.                                    prior 70%
COUNTING. Each separable new member is +1 on 148.
BARS = {"cross_max": 0.05, "keep": 0.8, "ext_floor": 0.5, "c_ub_max": 0.01, "k_d": 2, "k_new": 3, "instr_tol": 0.03, "own_leak_min": 0.2}
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v359_result.json"
...
FAMILIES = {
    "frame_prep_copies2": ["verb_preposition_fb_from_about", "verb_preposition_fb_into_with", "verb_preposition_fc_of_into", "verb_preposition_fc_through_with", "verb_preposition_from_about", "verb_preposition_into_with", "verb_preposition_of_into", "verb_preposition_through_with"],
}
EVAL = ("verb_preposition_fb_from_about", "verb_preposition_fb_into_with", "verb_preposition_fc_of_into", "verb_preposition_fc_through_with", )
COUNTED = ()
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES, **v213.NAMES, **v215.NAMES, **v217.NAMES, **v219.NAMES, **v221.NAMES, **v223.NAMES, **v225.NAMES, **v227.NAMES, **v229.NAMES, **v231.NAMES, **v233.NAMES, **v235.NAMES, **v237.NAMES, **v239.NAMES, **v241.NAMES, **v243.NAMES, **v245.NAMES, **v247.NAMES, **v249.NAMES, **v251.NAMES, **v253.NAMES, **v255.NAMES, **v257.NAMES, **v311.NAMES, **v315.NAMES, **v319.NAMES, **v323.NAMES, **v327.NAMES, **v349.NAMES, **v293.NAMES, **v353.NAMES, **v263.NAMES, **v265.NAMES, **v357.NAMES}


def _parent_cdas(n):
    # the parent battery receipt that produced this member's units (each behaviour name is unique across batteries)
    for path in (V357, V263, V265, V353, V293, V349, V327, V323, V319, V315, V311, V257, V255, V253, V251, V249, V247, V245, V243, V241, V239, V237, V235, V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v359", "members": n, "families": list(FAMILIES),
            "model_forwards_max": MODEL_FORWARDS_MAX, "example_evaluations_max": EXAMPLE_EVALUATIONS_MAX,
            "model_backwards": 2 * n * STEPS, "model_updates": 0, "fit_parameters": 2 * n * 30 * 128,
            "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only"}


DONOR = {"verb_preposition_fb_from_about": "verb_preposition_from_about",
         "verb_preposition_fb_into_with": "verb_preposition_into_with",
         "verb_preposition_fc_of_into": "verb_preposition_of_into",
         "verb_preposition_fc_through_with": "verb_preposition_through_with"}
...
def PREDS(R):
    ...
    own_leaks = [abs(good[n]["arms"]["own"]["siblings"][DONOR[n]])
                 for n in DONOR if n in good and DONOR[n] in good[n]["arms"]["own"]["siblings"]]
    fam_leaks = [abs(good[n]["arms"]["fam"]["siblings"][DONOR[n]])
                 for n in DONOR if n in good and DONOR[n] in good[n]["arms"]["fam"]["siblings"]]
    b = len(own_leaks) == len(DONOR) and min(own_leaks) >= B["own_leak_min"]
    e = len(fam_leaks) == len(DONOR) and max(fam_leaks) < B["cross_max"]
    return {"pred_a_new_separable": bool(a), "pred_b_own_leak_large": bool(b), "pred_c_row4_kept": bool(c),
            "pred_d_instrument": bool(d), "pred_e_donors_below": bool(e)}


def main() -> None:
    if os.environ.get("BQLIB_DRYRUN") or os.environ.get("BQLIB_NO_MODEL"):
        print(json.dumps(_plan(), indent=2, sort_keys=True)); return
    smoke = os.environ.get("V359_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V359_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v359", "candidate_id": "corpus.unit_family_separability_spec_v359",
