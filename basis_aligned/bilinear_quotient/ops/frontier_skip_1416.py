"""DO THE TWO SKIPS COMPOSE (rung 62): skip-a14 + skip-a16 together, measured per S2154's rule.

CONVENTION (S2135): L2 is CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER.

S2146: skip-a16 saves 0.2126. S2155: skip-a14 saves 0.2327. Naive sum ~ -0.44, but S2154 showed joint-throughout
removals pick up interaction/refit terms - so measure. Arms: S2144 best full tail (reproduction) | skip-a16
(S2146 window reproduction) | skip-a14+a16 (a15L and a17L refit with both attentions real). PRICE: -10.6M
stored values; coverage retreats TWO components (stated, as always). NULL: sub-additive composition (S2154).

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) MOST OF THE SUM SURVIVES: median over the 8 windows of [L2_F_w(skip1416) - L2_F_w(full)] <= -0.30.
  (b) STRICTLY BETTER THAN skip-a16 ALONE: [L2_F_w(skip1416) - L2_F_w(skip16)] <= 0 on >= 7 of 8 windows.
  (c) REPRODUCTION: the skip-a16 arm reproduces S2146 per-window (median |delta| <= 0.005).

Writes frontier_skip_1416_results.json. Self-reviewed. RUNG-61 HEADER FOLLOWS.

attn14 PER-HEAD MAP AT WINDOW GRAIN (rung 61): the second-costliest tail dictionary, same instrument.

CONVENTION (S2135): L2 is CE ADDED ABOVE THE REAL MODEL - LOWER IS BETTER; d_h(w) = damage added on window w by
zeroing head h of the real attn14 (eval-scoped) under a skip-a14 config (a14L skipped, a15L..a17L refit under
a14-real; everything else = the S2144 best).

S2145: a14L costs +0.073 (second only to a16L); S2147: its within-class residual is the HIGHEST (0.933). Name
its heads at window grain from the start (S2152's lesson). PRICE: none (attribution). NULL: uniform heads.

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) CONCENTRATION: max_h window-median d_h >= 2 x median_h window-median d_h.
  (b) ADDITIVITY: |sum_h window-median d_h - window-median d_all| <= 0.05.
  (c) THE S2145 MARGINAL SURVIVES REFITS: median over windows of [L2_F_w(S2144 best, stored) - L2_F_w(skip14)]
      >= +0.04 (a14L's +0.073 FR marginal, allowing refit absorption; S2146 precedent was super-additive).

Writes attn14_headw8_results.json. Self-reviewed. RUNG-59 HEADER FOLLOWS.
...
S2152: the FR-only head map (S2149) mispriced the six "free" heads (+0.0239 median at window grain). Re-measure
the per-head map on the eight windows: one build, eval-scoped zeroing of each head (and all nine) per window.
PRICE: none (attribution). NULL: the FR ordering transfers wholesale.

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) THE BIG THREE SURVIVE: the top three heads by window-median d_h are {16.0, 16.3, 16.4}.
  (b) THE SIX CARRY: the other six heads' summed window-median d_h >= +0.015 (per S2152).
  (c) REPRODUCTION: the base arm reproduces S2146's per-window values (median |delta| <= 0.005).

Writes attn16_headw8_results.json. Self-reviewed. RUNG-58 HEADER FOLLOWS.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['attn14_headw8_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; full / skip-a16 / skip-a14+a16 arms x eight windows (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_skip_1416_results.json'
...
if __name__=='__main__':
    ...
    SEL['skipset']=(); SEL['headw8']=False
    print('ARM 1/3: S2144 best, full tail (reproduction)',flush=True)
    r_f=main()
    SEL['skipset']=(16,)
    print('ARM 2/3: skip-a16 (S2146 reproduction)',flush=True)
    r_16=main()
    SEL['skipset']=(14,16)
    print('ARM 3/3: skip-a14+a16',flush=True)
    r_b2=main()
    import statistics as stt
    prev=json.load(open(PT+'frontier_skip_a16_results.json'))['skip_a16']['fresh8']
    rp=[abs(round(a-b,4)) for a,b in zip(r_16['fresh8'],prev)]
    dj=[round(a-b,4) for a,b in zip(r_b2['fresh8'],r_f['fresh8'])]
    d16=[round(a-b,4) for a,b in zip(r_b2['fresh8'],r_16['fresh8'])]
    mj=stt.median(dj)
    pa=mj<=-0.30
    pb=sum(g<=0 for g in d16)>=7
    pc=stt.median(rp)<=0.005
    res={'full':r_f,'skip16':r_16,'skip1416':r_b2,
         'per_window_joint_minus_full':dj,'per_window_joint_minus_skip16':d16,
         'naive_sum_prediction':-0.4453,
         'convention':'L2 = CE above the real model; lower is better',
         'pred_a_most_survives':bool(pa),'pred_b_beats_skip16':bool(pb),'pred_c_reproduces_s2146':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'joint - full per window: {dj}  median {mj:+.4f}  (naive sum -0.445)')
    print(f'joint - skip16 per window: {d16}')
    print(f"(a) median {mj:+.4f} <= -0.30: {'HELD' if pa else 'FAILED'}")
    print(f"(b) <= 0 vs skip16 on {sum(g<=0 for g in d16)}/8 (bar 7): {'HELD' if pb else 'FAILED'}")
