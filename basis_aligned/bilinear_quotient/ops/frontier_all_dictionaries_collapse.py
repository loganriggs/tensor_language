"""COLLAPSE EVERY ATTENTION DICTIONARY IN THE FRONTIER, AND MEASURE THE INSTRUMENT'S OWN RESOLUTION (Claude, LANE 1).

SS2874 collapsed a5's fitted `attnd` dictionary to one constant and measured a cost of **-0.0001 nats** -- and the a6 control collapsed
for exactly the same -0.0001, firing the registered null `c_null_collapsing_anything_is_free`. Two things follow, and this rung is
built around both.

First, **-0.0001 is at the reproduction noise floor**: SS2874's own baseline differs from the published 2.6735 by +0.0001, so a single
layer's collapse cannot be resolved. SS2874 recorded that caveat rather than leaving it implicit. **Nothing in this family is readable
until the floor is measured**, so arm 2 here is simply the BASELINE RUN AGAIN, identical configuration, and its difference from arm 1
IS the resolution of the instrument.

Second, if every motif dictionary is already near-constant (SS2874: row spreads .0416 for a5 and .0422 for a6, indistinguishable), the
interesting question is no longer which layer is special -- none is -- but **how much of the frontier is a table of constants**. Arm 3
collapses ALL sixteen attention dictionaries, a2-a17. Each carries ten class rows plus four 1152x1152 link maps for the LINK classes
[2,7,8,9], so each collapse removes **5,318,784 parameters**, and sixteen remove **85,100,544**.

SIGN CONVENTION (SS2135): frontier L2 is CE ADDED ABOVE THE REAL MODEL, so **LOWER IS BETTER** (SS312: +2.6735 beating +2.84/+2.93).
A collapse COST is L2_F(collapsed) - L2_F(baseline), **POSITIVE = WORSE**. SS2128/SS2129/SS2133/SS2134 RETRACTED; SS2125 STANDS.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_the_instrument_resolution_is_finer_than_one_milli_nat
#                     pred_c_all_sixteen_dictionaries_collapse_cheaply
#                     pred_d_the_joint_cost_is_resolvable_above_the_floor
#                     pred_e_the_parameter_saving_is_stated

Preregistration: polynomial_causal/FRONTIER_ALL_DICTIONARIES_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; three arms (baseline / baseline repeat = resolution / all 16 dictionaries collapsed)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_all_dictionaries_collapse_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_ALL_DICTIONARIES_PREREGISTRATION.md'
PREREG_SHA = "b22f82669d8016d4716581de0fa91612fffc43448e98c2dfd0508baffc2fa726"
...
ALLD = tuple(ATTM) + tuple(ATTT)          # a2..a17: every attention dictionary in the construction
BARS = {"reproduce": 0.05, "floor": 0.001, "all_free": 0.05, "resolvable": 2.0}
NULLS = {"all_free_ge": 0.20, "floor_ge": 0.01}
PERLAYER = 5318784
...
if __name__=='__main__':
    ...
    print('ARM 2/3: BASELINE AGAIN, identical config -- this difference IS the instrument resolution',flush=True)
    COLLAPSE['set']=frozenset()
    r_rep=main()

    print(f'ARM 3/3: ALL {len(ALLD)} attention dictionaries a{ALLD[0]}-a{ALLD[-1]} collapsed to constants',flush=True)
    COLLAPSE['set']=frozenset(ALLD)
    r_all=main()

    floor    = abs(r_rep['L2_F'] - r_base['L2_F'])          # the instrument's own reproducibility
    cost_all = r_all['L2_F'] - r_base['L2_F']               # POSITIVE = WORSE (LOWER L2 IS BETTER)
    saved    = PERLAYER * len(ALLD)

    pa = abs(r_base['L2_F']-2.6735) <= BARS['reproduce']
    pb = floor <= BARS['floor']
    pc = cost_all <= BARS['all_free']
    pd = abs(cost_all) >= BARS['resolvable'] * max(floor, 1e-6)
    pe = saved > 0
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_instrument_resolution_is_finer_than_one_milli_nat':bool(pb),
           'pred_c_all_sixteen_dictionaries_collapse_cheaply':bool(pc),
           'pred_d_the_joint_cost_is_resolvable_above_the_floor':bool(pd),
           'pred_e_the_parameter_saving_is_stated':bool(pe)}
    nulls={'c_null_the_dictionaries_are_jointly_load_bearing':bool(cost_all>=NULLS['all_free_ge']),
           'b_null_the_instrument_cannot_resolve_this_family':bool(floor>=NULLS['floor_ge']),
           'd_null_the_joint_cost_is_below_the_floor':bool(abs(cost_all)<BARS['resolvable']*max(floor,1e-6))}
    res={'rung':'frontier_all_dictionaries_collapse','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,'dictionaries':list(ALLD),
         'baseline':r_base,'baseline_repeat':r_rep,'all_collapsed':r_all,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(r_base['L2_F'],4),
                    'L2_F_baseline_repeat':round(r_rep['L2_F'],4),
                    'L2_F_all_collapsed':round(r_all['L2_F'],4),
                    'instrument_resolution_nats':round(floor,6),
                    'cost_all_dictionaries_nats':round(cost_all,4),
                    'L2_C_baseline':round(r_base['L2_C'],4),'L2_C_all_collapsed':round(r_all['L2_C'],4),
                    'n_dictionaries_collapsed':len(ALLD),
                    'parameters_saved_per_layer':PERLAYER,
                    'parameters_saved_total':int(saved)},
    ...
    print(f"(b) instrument resolution {floor:.6f} <= {BARS['floor']}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) all-{len(ALLD)} collapse cost {cost_all:+.4f} <= {BARS['all_free']} (LOWER L2 IS BETTER): {'HELD' if pc else 'FAILED'}")
    print(f"(d) resolvable above the floor: {'HELD' if pd else 'FAILED'}")
