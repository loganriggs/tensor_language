"""MANIPULABILITY I (rung 264): knockout-transfer at m16 inside the compiled mixed config.

CONVENTION (S2135): per-position dCE = CE(mixed config with m16 mean-ablated) - CE(real model); LOWER
IS BETTER. The program goal includes MANIPULABLE: interventions on the compiled object should reproduce
the real model's measured effects. Protocol mirrors the battery: m16's output is replaced by its FW
mean (disjoint from census) INSIDE the mixed-spectrum config; per-circuit member damage compared to (i)
battery refs at m16-family circuits, (ii) removal_matrix's real m16 collateral row.
REGISTERED PREDICTIONS (arms: COMPILED-KO = this run; REAL-KO = battery/removal_matrix receipts):
  (a) OWN-FAMILY EFFECT TRANSFERS: median member/ref ratio over m16-top circuits in [0.7, 1.6].
  (b) COLLATERAL PROFILE TRANSFERS: Spearman vs the real m16 row >= 0.8 over ~62 circuits.
  (c) SANITY: ablated census in [0.2, 5.0] (the knockout bites; config baseline 0.0573).
NULL: the compiled object does not support faithful knockouts (ratio outside [0.5, 2] or rho < 0.5).
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['removal_matrix_results.json','frontier_rows_results.json','circuits/BATTERY.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    print('DRYRUN OK: knockout transfer m16')
    raise SystemExit(0)
import torch
...
OUT=PT+'ko_transfer_m16_results.json'
...
if __name__=='__main__':
    ...
    _own=sorted(r['member_absdce']/max(r['ref'],1e-9) for r in rows if _topK.get(r['tag'])=='m16')
    ...
    _v2=torch.tensor([{r['tag']:r['member_absdce'] for r in rows}[t] for t in _tags2]).argsort().argsort().float()
    ...
    print(f"(a) KNOCKOUT TRANSFER - own-median ratio {_omed:.3f} in [0.7, 1.6]: {'HELD' if pa else 'FAILED'}")
    print(f"(b) collateral Spearman {_rhoK:.3f} >= 0.8; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) ablated census {agg:+.4f} in [0.2, 5.0]; era rho {rho:.4f}: {'HELD' if pc else 'FAILED'}")
