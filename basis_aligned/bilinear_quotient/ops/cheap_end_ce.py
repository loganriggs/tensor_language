# PUT THE CHEAP END ON THE CE FRONTIER  -- the question §1814 ended on.
#
# Everything since §1788 has been measured in TOP-1 ACCURACY, because §1788 showed CE and accuracy
# diverge. But the thread's cost/fidelity record from §1754 to §1787 is entirely CE, and the corrected
# cheap end has never been placed on it. §1814 found the rank-1 program costs 0.485M reals -- 886x
# compression against the live model's 430.00M -- retaining 9.90 / 10.65 / 10.07% top-1 against live
# 39.32 / 42.35 / 38.88%. Its CE is unmeasured.
#
# The map is built at its CARRYABLE rank min(64, r+1) throughout, per §1814: rank(Ws) <= r+1, so a
# higher map rank is the same matrix at a higher price. That is the corrected accounting, applied from
# the start rather than patched afterwards.
#
# ROLES. skip7000, skip11000, skip1200; table ranks 1, 2, 4, 8, 64; all sites compiled. DISCOVERY ONLY.
#
...
#   pred_a THE CHEAP END IS EXPENSIVE IN NATS: the rank-1 program's all-position CE exceeds the rank-64
#          program's by at least 0.30 nats, at every role. Top-1 differs by only 2.98 / 2.84 / 2.82 pp
#          between them; if CE differs by less than 0.30 the two axes agree that rank-1 is nearly free,
#          and the 42x cost saving is close to a free lunch. If it differs by more, CE sees a
#          degradation top-1 does not -- the mirror of §1788, where CE flattered the program and
#          accuracy did not.
#   pred_b CE IS MONOTONE IN TABLE RANK: all-position CE does not increase as rank goes 1 -> 2 -> 4 ->
#          8 -> 64, at every role. §1813 found top-1 monotone at all sites; if CE is not, the two axes
#          disagree about the ORDERING and not merely the magnitude, which would make "rank r is better
#          than rank s" an axis-dependent statement everywhere in §1751-§1813.
#   pred_c AND THE CE OPTIMUM IS ALSO CHEAP: the CE-nats-bought-per-real optimum is at table rank 4 or
#          below, at every role. §1813 found the top-1 efficiency optimum at rank 1 with no interior
#          optimum. Scored separately because the two metrics are differently shaped -- CE is unbounded
#          above while top-1 is bounded in [0,1], so a metric that saturates on one axis need not on
#          the other.
#   pred_d CONTROLS, cross-run per LESSON 42: t1 reproduces §1813's PUBLISHED top-1 of 0.0990 / 0.1065 /
#          0.1007 and t64 reproduces §1786's 0.1288 / 0.1349 / 0.1289, both within 0.01; t64's
#          ALL-POSITION CE reproduces §1786's PUBLISHED 6.17330 / 6.15261 / 6.14463 within 0.02 -- the
#          first CE cross-check in this sub-thread and the one that proves the new CE path is the same
#          measurement §1754-§1787 used; the live model's all-position CE reproduces 3.13704 / 2.93450 /
#          3.23027 within 0.01; coverage 5419 of 50257.
...
OUT = PT + 'ops/cheap_end_ce_results.json'
PROBE_LS = ()
KIND_LS = ()
RANKS = (1, 2, 4, 8, 64)     # the cheap end plus §1786's design point
...
def main():
    ...
    print(f'CHEAP END ON THE CE FRONTIER | table ranks {RANKS}, map at carryable rank | '
    ...
    cost = {}
    for r in RANKS:
        mr = min(64, r + 1)                 # the CARRYABLE map rank (§1814)
        frr = build(NFULL, r, mr)[0]
        hks = {st: row_hook(frr[st]) for st in sites}
        n = len(sites)
        cost[f't{r}'] = n * (r * (NCOV + D) + 2 * D) + n * mr * 2 * D
        run(f't{r}', sites, hks)
        del frr, hks
        torch.cuda.empty_cache()
    ...
    tabled = {e: res[e]['all_substituted']['ce_all'] for e in roles}
    ce_gap = {e: ce[e]['t1'] - ce[e]['t64'] for e in roles}
    ce_mono = {e: all(ce[e][f't{r2}'] <= ce[e][f't{r1}'] + 1e-9
                      for r1, r2 in zip(RANKS, RANKS[1:])) for e in roles}
    # CE bought per million reals, against the fully-tabled program
    ce_eff = {e: {k: (tabled[e] - ce[e][k]) / (cost[k] / 1e6) for k in keys} for e in roles}
    best_ce_eff = {e: max(ce_eff[e], key=lambda k: ce_eff[e][k]) for e in roles}
    pa = all(ce_gap[e] >= 0.30 for e in roles)
    pb = all(ce_mono[e] for e in roles)
    pc = all(int(best_ce_eff[e][1:]) <= 4 for e in roles)
    pd = (all(abs(res[e]['all_substituted']['top1'] - S1789_PROG[e]) <= 0.001
              and abs(res[e]['live_model']['top1'] - S1789_LIVE[e]) <= 0.001 for e in roles)
          and all(abs(acc[e]['t1'] - S1813_T1_PP[e]) <= 0.01
                  and abs(acc[e]['t64'] - S1786_T64_PP[e]) <= 0.01
                  and abs(ce[e]['t64'] - S1786_T64_CE[e]) <= 0.02
    ...
    print('\n  all-position CE and top-1, with the map at its carryable rank:', flush=True)
    for k in keys:
        print(f'    {k:5s} {cost[k]/1e6:8.3f}M  ' + '  '.join(
            f'{e} CE {ce[e][k]:.5f} top1 {acc[e][k]:6.2%}' for e in roles), flush=True)
    print(f'    live         430.000M  ' + '  '.join(
        f'{e} CE {res[e]["live_model"]["ce_all"]:.5f}' for e in roles), flush=True)
    print('\n  CE nats bought per million reals, against the fully-tabled program:', flush=True)
    for e in roles:
        print(f'    {e:10s} ' + '  '.join(f'{k}:{ce_eff[e][k]:.4f}' for k in keys), flush=True)
    print(f'\n  the rank-1 program costs >=0.30 nats against rank 64 -> {pa}  ' + '  '.join(
        f'{e} {ce[e]["t1"]:.5f} vs {ce[e]["t64"]:.5f} (+{ce_gap[e]:.5f})' for e in roles),
        flush=True)
    print(f'  CE is monotone in table rank -> {pb}  ' + '  '.join(
        f'{e} ' + '/'.join(f'{ce[e][k]:.4f}' for k in keys) for e in roles), flush=True)
    print(f'  the CE-per-real optimum is at table rank <=4 -> {pc}  ' + '  '.join(
        f'{e} {best_ce_eff[e]} ({ce_eff[e][best_ce_eff[e]]:.4f})' for e in roles), flush=True)
    ...
    json.dump({'run': 'l5_cliff_probe',
               ...
               'ce_all': ce, 'ce_per_M': ce_eff, 'ce_gap_t1_vs_t64': ce_gap,
               'predictions': {'pred_a_rank1_costs_030_nats': bool(pa),
                               'pred_b_ce_monotone_in_rank': bool(pb),
                               'pred_c_ce_efficiency_optimum_le_4': bool(pc),
