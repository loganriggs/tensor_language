#!/usr/bin/env python3
# BQLANE: gpu
# BQGATE: 22forwards154seq90readouts; successor cross-type residual low-rank transfer;0updates.
...
BINDING = POLY / "SUCCESSOR_POINTER_INTERACTION_LOW_RANK_V1_BINDING.json"
OUT = POLY / "SUCCESSOR_POINTER_INTERACTION_LOW_RANK_V1_RESULT.json"
...
def plan():
    ...
    return {"schema": "successor_pointer_interaction_low_rank_v1_plan", "model_loaded": False,
...
def main():
    ...
    def readout(residual):
        counts["readout_rows"] += len(residual)
        value = torch.as_tensor(residual, dtype=torch.float32, device=device)
        with torch.no_grad():
    ...
    def rank_report(data, basis, rank):
        ...
        for destination in ("late_swap_incoherent", "early_swap_control"):
            ...
            synthetic[destination] = readout(additive + projection)
        target = margin(native["coherent"], rows["coherent"], "backward") - margin(native["late_swap_incoherent"], rows["late_swap_incoherent"], "backward")
        reports = {}; passes = True
        for family in FAMILIES:
    ...
    fit_readout_error = float(np.max(np.abs(readout(fit["residual"]["late_swap_incoherent"]) - fit["native"]["late_swap_incoherent"])))
    ...
    if fit_instrument and selected is not None:
        ...
        hold_readout_error = float(np.max(np.abs(readout(hold["residual"]["late_swap_incoherent"]) - hold["native"]["late_swap_incoherent"])))
    ...
    result = {
        "schema": "successor_pointer_interaction_low_rank_v1_result", "terminal": terminal,
