#!/usr/bin/env python3
"""Zero-forward leave-one-head-out test of the v23 rank-one context core."""
...
PRIOR = ROOT / "circuits/prior_art/temporal_iswas_v23_rank1_shared_context_leave_one_head_out_v1.json"
...
OUT = ROOT / "circuits/followups/temporal_iswas_v23_rank1_shared_context_leave_one_head_out_v1_result.json"
EXPECTED = {
    "prior": "c7d8c97e6428000eb6e1356c18074b1dc1b0dbef26ee5c1a914e3a17e46f0c52",
    ...
    "library": "514be335690aae4cb100fb9f58f3f00b8d55449a57b3b253ea1600a33153a6a6",
}
RANK = 1
ROTATION_SEED = 2308
RANDOM_SEED = 2401
...
def random_capture_q99(value, *, generator):
    directions = torch.randn(RANDOM_DRAWS, value.shape[0], generator=generator)
...
def main():
    paths = {"prior": PRIOR, "arrays": ARRAYS, "grouping": GROUPING,
             "gauge": GAUGE, "library": LIBRARY}
    observed = {name: sha(path) for name, path in paths.items()}
    dry = {"candidate_id": "cross_task.temporal_iswas.v23_rank1_shared_context_leave_one_head_out_v1",
           "dryrun": True, "gpu_accessed": False, "model_loaded": False,
           "queue_touched": False, "authority_ok": observed == EXPECTED,
           "rank": RANK, "random_draws": RANDOM_DRAWS, "price": PRICE}
    import os
    if os.environ.get("BQLIB_DRYRUN") or os.environ.get("BQLIB_NO_MODEL"):
        print(json.dumps(dry, sort_keys=True)); return
    if observed != EXPECTED: raise RuntimeError("rank-one shared-context authority changed")
    if OUT.exists(): raise FileExistsError(f"refusing overwrite: {OUT}")

    started_utc, started = now(), time.perf_counter()
    frozen = np.load(ARRAYS, allow_pickle=False)
    maps = torch.from_numpy(frozen["contracted"]).float()
    ...
    rotated_maps = []
    for value in maps:
        rotation, _ = torch.linalg.qr(torch.randn(128, 128, generator=rotation_generator))
    ...
    for route, value, fold in zip(routes, maps, report["folds"]):
        ...
        fold_reports[route] = {"heldout_captured_energy_fraction": capture,
                               "random_capture_q99": random_q99,
                               "random_capture_mean": random_mean,
                               "exceeds_random_q99": capture > random_q99}

    instrument_error = max(reconstruction_error, certificate_error,
                           raw_gram_replay_error, max(gauge_errors))
    A = bool(observed == EXPECTED and instrument_error <= BARS["instrument"]
    ...
    result = {"schema": "temporal_iswas_v23_rank1_shared_context_leave_one_head_out_result_v1",
        ...
        "routes": routes, "rank": RANK, "component_weights": [1.0] * len(routes),
        "instrument": {"reconstruction_max_abs": reconstruction_error,
        ...
        "selected_rank_weight_or_threshold_after_outcome": None}
    atomic_create_json(OUT, result)
    print(json.dumps({key: result[key] for key in ("instrument", "fold_reports",
        "minimum_leave_one_out_projector_overlap", "raw_context_gram_median",
        "tail_context_gram_median", "context_gram_median_reduction", "predictions",
