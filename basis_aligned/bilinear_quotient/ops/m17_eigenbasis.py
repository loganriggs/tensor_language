"""THE DECISIVE BASIS TEST (rung 218): m17, the highest-damage module - predicted EIGEN WINS.

CONVENTION (S2135): per-position dCE = CE(m17 replaced, all else real) - CE(real model); LOWER IS BETTER.
Standing 2-1 for neurons (m16: eigen 0.77; m14: neuron 1.26; m13: neuron 1.54). Sharpened hypothesis
(S2314): the gauge-free eigenbasis wins where module function is concentrated/high-damage. m17 has the
battery's largest knockout (own-mean 2.52, single own circuit) - the hypothesis' strongest prediction.
ARMS: NEUR = fixed top-1152/4608 neurons at m17; EIG = top-1152 eigenfeatures at m17.
REGISTERED PREDICTIONS:
  (a) EXACTNESS: rel-Frobenius <= 1e-3 AND max/RMS <= 5e-2.
  (b) EIGEN WINS AT THE CONCENTRATED MODULE: own-circuit ratio EIG/NEUR <= 0.9.
  (c) census ratio <= 0.85.
NULL: neuron holds (own ratio >= 0.95) - the m16 win was not about damage concentration; the hypothesis
dies. PRICE: probe (one module). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: m17 decisive basis test'); raise SystemExit(0)
import torch
import torch.nn.functional as F
from bilin18_joint_removal import m, DEV
D=1152
PT='/workspace/tensor_language/basis_aligned/bilinear_quotient/'
OUT=PT+'m17_eigenbasis_results.json'
...
def main():
    ...
    ownt=[t for t,v in CINFO.items() if v['top']=='m17']
    print(f'{len(CINFO)} circuits; m17 owns {len(ownt)}: {ownt}',flush=True)
    ...
    pa=(frob<=1e-3 and mxs<=5e-2); pb=ra<=0.9; pc=rb<=0.85
    ...
    print(f"(c) EIG census <= 0.85 x NEUR (ratio {rb:.3f}): {'HELD' if pc else 'FAILED'}")
