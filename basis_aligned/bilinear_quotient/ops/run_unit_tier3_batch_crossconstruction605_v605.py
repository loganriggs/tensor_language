...
"""v605: work the AUTHORED BACKLOG instead of authoring more cells.
...
REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here). THESE ARE v595's BARS VERBATIM.
A replication batch: same question, same thresholds, fresh constructions, nothing re-tuned between batches so all
batches POOL. As of the 18:43 tick the pooled figure over 40 non-prepositional cells is row2 72%, row3 85%,
row4 42%, row5 65%, against 27/97/79/88 for the 33 prepositional ones. v591 lifted eight cells from eight constructions and
...
Smoke: V605_SMOKE=<out.json> (CPU, V605_SMOKE_ROWS=4).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_crossconstruction605_v605_result.json"
...
NAMES = {"person_agreement_be": "person_agreement_be",
         "person_agreement_past": "person_agreement_past",
         "polarity_ever_never": "polarity_ever_never",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_crossconstruction605_v605", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V605_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V605_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V605_SMOKE_NAMES", "person_agreement_be,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_crossconstruction605_v605", "candidate_id": "corpus.unit_tier3_batch_crossconstruction605_v605", "bars": BARS,
