...
"""Upstream boundary-source census for frozen v621 quadratic component.

Observe all embedding/attention/MLP writes and carry each exactly once through
native lambdas to h17. Biases stay in their producing writes. Rank sources by
mean absolute half-cross attribution on skip1200; freeze order for skip11000.
...
OUT=BQ/'circuits/followups/output_component_sources_v625_result.json'
...
def main():
    ...
    from quadratic_source_census import carried_sources,pair_table,read_quadratic
    ...
    for panel_index,panel in enumerate(plan['panels']):
        ...
        if panel_index==0:order=scores.argsort(descending=True)
        frontier=[]
    ...
    disk_guard.guard_write(100000,label='v625 JSON');OUT.write_text(json.dumps(result,indent=2)+'\n')
