#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays_v221 pred_b_one_reader_loses_half pred_c_loser_is_9_6 pred_d_edit_selective pred_e_all_positions_removes_more
"""Pronoun gender he/she DoD (v222): the head 8.1-at-verb edit (v221: he - she -3.5%, 3152's verb contrast -29%) -- WHICH reader loses it, and is it SELECTIVE?
The four gender readers' weight-only coefficients c_h = v_h . z_h(final) (h in {10.1, 9.6, 12.4, 15.1}; `fw.capture` + `readout_directions`) under native,
8.1 zeroed at the verb, 8.1 zeroed at ALL positions (this also removes its self-copy at the noun and its copy at the final query: v174 / v146), pooled male -
female over the 48 pairs; the he - she margin and the three gender-free readers (will-would, who-which, night-day) for the verb edit, with the other eight
block-8 heads at the verb as the matched null. Margin = he - she for every row (v207's orientation lesson).
PREDICTIONS (scored as written; failures preserved; priors from v216: 9.6 reads the verb 48%, 12.4 23%, 15.1 11%, 10.1 3%)
    pred_a_baseline_replays_v221     the native pooled margin matches v221 (103.46) within relative 1e-3
    pred_b_one_reader_loses_half     one reader head's coefficient drop is >= 0.50 of the summed coefficient drop over the four
    pred_c_loser_is_9_6              that head is 9.6
    pred_d_edit_selective            each unrelated reader |move| <= mean |move| over the eight other heads + 0.25 x the margin damage (4.5's whole-slice edit failed this on number)
    pred_e_all_positions_removes_more  zeroing 8.1 at all positions removes >= 1.5 x the verb-only margin damage
PRICE (registered maximum): 2 batches x (3 captures + 2 margin forwards + 8 null forwards) = 26 forwards; 0 backwards; 0 fits. Bar <= 28.
"""
...
OUT = ROOT / "circuits/followups/pronoun_gender_dod_head81_readers_v222_result.json"
CANDIDATE_ID = "pronoun_gender.he_vs_she.dod_head81_readers_v222"
BLOCK, HEAD, V221_MARGIN, HALF, GATE_RATIO, MORE, BATCH = 8, 1, 103.46, 0.50, 0.25, 1.5, 32
HEADS = ((10, 1), (9, 6), (12, 4), (15, 1))
FORWARDS_MAX = 28
PREDICTIONS = {"pred_a_baseline_replays_v221": "<= 1e-3", "pred_b_one_reader_loses_half": ">= 0.50", "pred_c_loser_is_9_6": "9.6", "pred_d_edit_selective": "three gates", "pred_e_all_positions_removes_more": ">= 1.5x"}
...
def main() -> None:
    ...
    predictions = {"pred_a_baseline_replays_v221": abs(m_native - V221_MARGIN) / V221_MARGIN <= 1e-3, "pred_b_one_reader_loses_half": bool(report["loser_share"] is not None and report["loser_share"] >= HALF), "pred_c_loser_is_9_6": loser == "9.6",
                   "pred_d_edit_selective": all(gates.values()), "pred_e_all_positions_removes_more": dmg_all >= MORE * dmg_verb}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_head45_readers_result_v222", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
