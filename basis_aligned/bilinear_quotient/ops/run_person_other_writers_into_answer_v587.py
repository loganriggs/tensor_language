#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_groups_make_the_remainder pred_c_late_group_carries pred_d_early_group_small pred_e_four_replay
"""The other writers into the answer, person line (v587). v586: on 43 natural person pairs the four readers' writes into the answer close 0.412 of the
myself - yourself gap, every head's 0.943. Here the remaining heads' writes into the answer by block group -- blocks 0-7 (G0_7), block 8 without 8.1 (G8_REST),
blocks 9-12 without 10.5 (G9_12_REST), blocks 13-17 without 13.1 / 15.1 (G13_17_REST) -- with the four readers as replay.
PREDICTIONS (scored as written; failures preserved; priors from v586 and the number / gender write censuses)
    pred_a_baseline_replays          the unedited manual forward replays the model's myself - yourself margins within 1e-3
    pred_b_groups_make_the_remainder the four groups sum to within 0.10 of 0.943 - 0.412 = 0.531
    pred_c_late_group_carries        G13_17_REST closes >= 0.15. Prior: unsure
    pred_d_early_group_small         G0_7 closes <= 0.15 (early heads write little straight into the answer)
    pred_e_four_replay               FOUR_W closes 0.412 +- 0.03
PRICE (registered maximum): 2 batches x 6 passes = 12 forwards (43 pairs); 0 backwards; 0 fits. Bar <= 13.
"""
...
OUT = ROOT / "circuits/followups/person_other_writers_into_answer_v587_result.json"
CANDIDATE_ID = "person.other_writers_into_answer_v587"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, REMAINDER, ADD, LATE_MIN, EARLY_MAX, FOUR_V586, BAND, UNIT = 1e-3, 0.943 - 0.412, 0.10, 0.15, 0.15, 0.412, 0.03, 701
...
FORWARDS_MAX = 13
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_groups_make_the_remainder": "0.531 +- 0.10", "pred_c_late_group_carries": ">= 0.15", "pred_d_early_group_small": "<= 0.15", "pred_e_four_replay": "0.412 +- 0.03"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "remainder": REMAINDER, "add": ADD, "late_min": LATE_MIN, "early_max": EARLY_MAX, "four_v586": FOUR_V586, "band": BAND, "unit": UNIT}}
    ...
    SETS_W = {"FOUR_W": FIVE_W, "G0_7": grp(0, 7), "G8_REST": grp(8, 8), "G9_12_REST": grp(9, 12), "G13_17_REST": grp(13, 17)}; MODES = {m: ({}, (0,)) for m in SETS_W}; forwards = 0
    def logits_margin(x):
    ...
    groups = {k: mc[k] for k in ("G0_7", "G8_REST", "G9_12_REST", "G13_17_REST")}; print("other writers into the answer", {k: round(v, 3) for k, v in mc.items()}, "groups sum", round(sum(groups.values()), 3))
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_groups_make_the_remainder": abs(sum(groups.values()) - REMAINDER) <= ADD, "pred_c_late_group_carries": mc["G13_17_REST"] >= LATE_MIN,
                   "pred_d_early_group_small": abs(mc["G0_7"]) <= EARLY_MAX, "pred_e_four_replay": abs(mc["FOUR_W"] - FOUR_V586) <= BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "person_other_writers_into_answer_result_v587", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
