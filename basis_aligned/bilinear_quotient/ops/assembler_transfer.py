...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; attn2/attn3 CCA/transfer on motifs-2-3-real base (merge-licensing rung)")
    raise SystemExit(0)
import torch
...
OUT=PT+'assembler_transfer_results.json'
...
if __name__=='__main__':
    ...
    print('SINGLE ARM: motifs 2/3 real, capturing attn2/attn3 outputs over FR',flush=True)
    r_b=main()
    clsflat=SEL['clsflat']
    Y2=torch.cat(SEL['y14']); Y3=torch.cat(SEL['y16'])
    if Y2.shape[0]!=len(clsflat) or Y3.shape[0]!=len(clsflat):
        raise SystemExit(f'INSTRUMENT FAIL: capture length {Y2.shape[0]}/{Y3.shape[0]} != {len(clsflat)}')
    import statistics as stt
    SW=CLS.index('subword')
    im=(clsflat==SW)
    rows=torch.arange(len(clsflat))//256
    halfA=rows<60
    def cca8(X,Y):
        Xc=X-X.mean(0); Yc=Y-Y.mean(0)
        Ux,_,_=torch.linalg.svd(Xc,full_matrices=False)
        Uy,_,_=torch.linalg.svd(Yc,full_matrices=False)
        k=64
        C=Ux[:,:k].T@Uy[:,:k]
        sv=torch.linalg.svdvals(C)
        return float(sv[:8].mean())
    cc=cca8(Y2[im],Y3[im])
    Ai=im&halfA; Bi=im&(~halfA)
    XA,YA=Y2[Ai],Y3[Ai]; XB,YB=Y2[Bi],Y3[Bi]
    lam=1e-2*len(XA)
    W=torch.linalg.solve(XA.T@XA+lam*torch.eye(XA.shape[1]),XA.T@YA)
    def r2(X,Y):
        pred=X@W
        return 1-float(((Y-pred)**2).sum())/max(float(((Y-Y.mean(0))**2).sum()),1e-9)
    r2A=r2(XA,YA); r2B=r2(XB,YB)
    pa=cc>=0.6
    pb=(r2A>1e-6) and (r2B>=0.8*r2A)
    pc=abs(r_b['L2_F']-2.4410)<=0.01
    res={'base':{'L2_F':r_b['L2_F']},'cca8_subword':round(cc,4),
         'map_r2_halfA':round(r2A,4),'map_r2_halfB':round(r2B,4),
         'pred_a_aligned':bool(pa),'pred_b_map_transfers':bool(pb),'pred_c_reproduces_r86_base':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'subword CCA {cc:.3f}; map R^2 half-A {r2A:.3f} -> half-B {r2B:.3f}')
    print(f"(a) CCA {cc:.3f} >= 0.6: {'HELD' if pa else 'FAILED'}")
    print(f"(b) transfer {r2B:.3f} >= 0.8 x {r2A:.3f}: {'HELD' if pb else 'FAILED'}")
