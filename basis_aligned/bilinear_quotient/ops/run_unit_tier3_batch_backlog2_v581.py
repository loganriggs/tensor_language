...
"""v581: work the AUTHORED BACKLOG instead of authoring more cells.
...
REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here). THE THRESHOLDS ARE DERIVED
FROM v575's MEASURED FRACTIONS, NOT INHERITED: v575 reused v409's k_four=1, k_row2=1, k_row3=2, k_row4=1, k_row5=1,
which "at least one of eight" satisfies trivially, so 6/6 there said almost nothing. v575 measured four rows 1 of 8,
row 2 TWO of 8, row 3 8 of 8, row 4 8 of 8, row 5 6 of 8, and these bars are set so that a different batch fails
them:
  pred_a_four_rows    at least k_four = 1 of the eight clears ALL FOUR tier rows, as at_amid did in v575.
                                                                                                      prior 70%
  pred_b_row2_still_binding row 2 passes AT MOST row2_max = 3 of the eight, so row 2 remains the binding constraint
                      it was in v575 (two of eight). This FAILS if row 2 passes four or more, which would mean the
                      first batch was unlucky rather than row 2 being hard.                           prior 70%
  pred_c_row3_unanimous row 3 passes ALL EIGHT, k_row3 = 8, as it did in v575. One failure breaks it. prior 75%
  pred_d_row4_unanimous row 4 passes ALL EIGHT, k_row4 = 8, as it did in v575. One failure breaks it. prior 70%
  pred_e_row5_amended at least k_row5 = 5 of the eight clear row 5 under the amendment, against six of eight in
                      v575.                                                                            prior 70%
...
HOW IT READS. a true with b, c, d: the one-in-eight yield replicates on a second batch, row 2 is confirmed as the
thing that gates these cells, and the backlog's real value is roughly two countable behaviours per sixteen cells --
a number I can plan with. b FALSE: row 2 passes four or more, v575's two-of-eight was a small-sample accident, and
the yield is better than I booked. c or d FALSE: rows 3 and 4 are not automatic after all, and v575's unanimity was
the accident instead. a FALSE with c and d true: eight more cells lift cleanly and none is countable, which caps the
backlog's value near zero and is worth knowing before lifting the other families' 38.
SCOPE. Eight behaviours, one family, one frame; units and tier rows only, no counting.
Smoke: V581_SMOKE=<out.json> (CPU, V581_SMOKE_ROWS=4).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_backlog2_v581_result.json"
...
NAMES = {"verb_preposition_from_of": "verb_preposition_from_of",
         "verb_preposition_in_amid": "verb_preposition_in_amid",
         "verb_preposition_of_amid": "verb_preposition_of_amid",
         "verb_preposition_off_with": "verb_preposition_off_with",
         "verb_preposition_on_amid": "verb_preposition_on_amid",
         "verb_preposition_on_within": "verb_preposition_on_within",
         "verb_preposition_to_amid": "verb_preposition_to_amid",
         "verb_preposition_to_beyond": "verb_preposition_to_beyond",
...
BARS = {"ext_min": EXT_MIN, "rem_min": REM_MIN, "c_ub_max": C_UB_MAX, "row5_share_band": [0.5, 1.4], "k_four": 1, "row2_max": 3, "k_row3": 8, "k_row4": 8, "k_row5": 5, "instr_tol": 0.02}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 200000, 6400000


def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_backlog2_v581", "behaviours": 9, "constructions": 5,
...
def PREDS(R):
    ...
    return {"pred_a_four_rows": bool(a), "pred_b_row2_still_binding": bool(b), "pred_c_row3_unanimous": bool(c),
            "pred_d_row4_unanimous": bool(d), "pred_e_row5_amended": bool(e), "pred_f_instrument": bool(f)}


def main() -> None:
    if os.environ.get("BQLIB_DRYRUN") or os.environ.get("BQLIB_NO_MODEL"):
        print(json.dumps(_plan(), indent=2, sort_keys=True)); return
    smoke = os.environ.get("V581_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V581_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V581_SMOKE_NAMES", "verb_preposition_from_of,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_backlog2_v581", "candidate_id": "corpus.unit_tier3_batch_backlog2_v581", "bars": BARS,
