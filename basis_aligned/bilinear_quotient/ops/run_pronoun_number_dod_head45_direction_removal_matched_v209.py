#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays_v206 pred_b_direction_removal_damages pred_c_direction_beats_random_directions pred_d_direction_removal_selective pred_e_direction_carries_half_of_full_zeroing
"""Pronoun number they/he DoD (v209): v208 with a NORM-MATCHED random null. v208 (5/5) removed the number direction of head 4.5's verb slice (-4.3% of the margin, selective)
against 16 random directions that removed only 23-76% of the norm the number direction removed. Here each random direction d_k removes n_i d_k, with
n_i the norm the number direction removed on row i (sign of d_k as drawn), so every null removal has exactly the number removal's norm.
Original question: v206 / v207b: zeroing 4.5's whole slice at the verb
removes 7.7% of the they - he margin but also moves the animacy and tense readers five times their null -- 4.5 copies the subject's features wholesale.
Here only the component of 4.5's verb slice along one 128-d direction is removed: d = O_{4.5}^T r with r = the block-8 input gradient of the plural
detector 829 at the verb (v205's reader; a fold object built from weights and the native trace, fixed per row), scaled to reach block 8 -- the number
direction of the copy as 829 sees it. Null: 16 seeded random 128-d directions, each removal norm-matched to the number-direction removal per row.
Readers will-would / who-which / night-day; gate move <= null mean + 0.25 x damage.
PREDICTIONS (scored as written; failures preserved; priors unsure)
    ...
    pred_b_direction_removal_damages          the number-direction removal drops the margin by >= 0.02 (v206 full zeroing: 0.077)
    pred_c_direction_beats_random_directions  its damage exceeds every one of the 16 norm-matched random directions
    pred_d_direction_removal_selective        each unrelated reader |move| <= null mean |move| + 0.25 x damage (the property the full zeroing failed)
    pred_e_direction_carries_half_of_full_zeroing  its damage >= 0.50 x the full-zeroing damage measured in the same run
PRICE (registered maximum): 3 batches x (native + full zeroing + direction removal + 16 random directions + 1 trace for r) = 60 forwards; 0 backwards; 0 fits. Bar <= 63.
"""
...
OUT = ROOT / "circuits/followups/pronoun_number_dod_head45_direction_removal_matched_v209_result.json"
CANDIDATE_ID = "pronoun_number.they_vs_he.dod_head45_direction_removal_matched_v209"
BLOCK, HEAD, U829, V206_MARGIN, DROP_MIN, GATE_RATIO, HALF, N_NULL, SEED, BATCH = 4, 5, 829, 196.62, 0.02, 0.25, 0.50, 16, 209, 32
FORWARDS_MAX = 63
import run_aspectual_dod_mlp8_pair_fold_v16 as v16
PREDICTIONS = {"pred_a_baseline_replays_v206": "<= 1e-3", "pred_b_direction_removal_damages": ">= 0.02", "pred_c_direction_beats_random_directions": "> 16 random", "pred_d_direction_removal_selective": "three gates", "pred_e_direction_carries_half_of_full_zeroing": ">= 0.50 x full"}
...
def main() -> None:
    ...
    predictions = {"pred_a_baseline_replays_v206": abs(native - V206_MARGIN) / V206_MARGIN <= 1e-3, "pred_b_direction_removal_damages": damage["number"] >= DROP_MIN, "pred_c_direction_beats_random_directions": damage["number"] > report["null_damage_max"],
    ...
    OUT.write_text(json.dumps({"schema": "pronoun_number_dod_head45_direction_removal_matched_result_v209", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
