"""RE-OPTIMISE AROUND THE NEW OBJECTS: RANK-32 TAIL x CP SPLIT x MOTIF (Claude, LANE 1)

SS2923 adopted a rank-32 tail projection (+2.2732 / +2.2953 held out) but **inherited CP x0.80 and motif x1.25 from SS2912**, where
both were chosen against a UNIFORM tail term. SS2923 said so in its own text and registered this as the next step: the composition
shortfall was **0.0275 of a 0.0542 standalone gain**, i.e. the terms are correcting partly the same error, so the inherited constants
are the wrong ones for the new tail object.

SS2922 then showed the CP correction is itself **not a scalar**: leaving the top 128 units alone and halving the rest reads -0.1280
against uniform's -0.1074, and it transports (-0.1123 vs -0.0908 held out). So the CP axis here is CATEGORICAL -- the incumbent scalar
against three splits -- rather than a scale sweep.

Three axes, one fitted stack: tail-subspace scale x {uniform CP, three CP splits} x motif. Both identity paths are controlled in the
same run.

**This is an adoption candidate and it is built to fail**: SS2923's own configuration is an arm, so `pred_c` anchors the comparison, and
`pred_e` requires the winner to still win on a window that chose nothing -- the test SS2914/SS2916 made mandatory.
...
#                     pred_b_both_identity_paths_are_physical_no_ops
#                     pred_c_the_incumbent_reproduces_S2923
#                     pred_d_re_optimising_around_the_new_objects_improves_on_S2923
#                     pred_e_the_improvement_survives_on_the_held_out_window
#                     pred_f_the_optimum_is_interior_in_both_continuous_axes

Preregistration: polynomial_causal/FRONTIER_NEW_OBJECTS_JOINT_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run: a joint re-optimisation around the new tail and CP object types")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_new_objects_joint_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_NEW_OBJECTS_JOINT_PREREGISTRATION.md'
PREREG_SHA = "a8a30c8f9d134e842f0895bd6203bff6f1e21b75f38e7c19d9f488527d79c366"
...
BARS['improves'] = 0.01
TS_GRID = [0.20, 0.25, 0.30]                # scale on the tail's rank-32 subspace; SS2923 adopted 0.25
M_GRID  = [1.15, 1.25, 1.35]                # SS2912 adopted 1.25
CP_OPTS = [('u0.8',     {'cp_scale': 0.80}),                    # the INCUMBENT, a uniform scalar
           ('b128s0.5', {'cp_spec': (128, 'bot', 0.5)}),        # SS2922's best CP split
           ('b128s0.8', {'cp_spec': (128, 'bot', 0.8)}),
           ('b256s0.5', {'cp_spec': (256, 'bot', 0.5)})]
S2923_COST = -0.4003                        # SS2923's ADOPTED configuration, the number to beat
...
if __name__=='__main__':
    ...
    ARMS[:] = ([('baseline', {}),
                ...
                ('cp_identity',  {'cp_spec': (0, 'all', 1.0)}),
                ('S2923', {'tail_spec': (32,'top',0.25), 'cp_scale': 0.80, 'motif_scale': 1.25})] +
               [(f't{ts}_{cn}_m{mm}', dict(co, tail_spec=(32,'top',ts), motif_scale=mm))
                for ts in TS_GRID for cn, co in CP_OPTS for mm in M_GRID])
    ...
    ident_svd, ident_cp = cost['svd_identity'], cost['cp_identity']
    inc = cost['S2923']
    cell = {(ts, cn, mm): cost[f't{ts}_{cn}_m{mm}']
            for ts in TS_GRID for cn, _ in CP_OPTS for mm in M_GRID}
    best = min(cell, key=lambda k: cell[k])
    best_cost = cell[best]
    best_name = f't{best[0]}_{best[1]}_m{best[2]}'
    interior = (best[0] not in (TS_GRID[0], TS_GRID[-1])) and (best[2] not in (M_GRID[0], M_GRID[-1]))

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = abs(ident_svd) <= BARS['identity'] and abs(ident_cp) <= BARS['identity']
    pc = abs(inc - S2923_COST) <= BARS['anchor']
    pd = best_cost <= inc - BARS['improves']
    pe = cost_h[best_name] < cost_h['S2923']
    pf = bool(interior)
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_both_identity_paths_are_physical_no_ops':bool(pb),
           'pred_c_the_incumbent_reproduces_S2923':bool(pc),
           'pred_d_re_optimising_around_the_new_objects_improves_on_S2923':bool(pd),
           'pred_e_the_improvement_survives_on_the_held_out_window':bool(pe),
           'pred_f_the_optimum_is_interior_in_both_continuous_axes':bool(pf)}
    nulls={'b_null_an_identity_path_is_not_faithful':bool(abs(ident_svd)>0.02 or abs(ident_cp)>0.02),
           'c_null_the_incumbent_fails':bool(abs(inc - S2923_COST) >= 0.03),
           'd_null_S2923_is_already_jointly_optimal':bool(not pd),
           'e_null_the_improvement_is_selection':bool(not pe),
           'f_null_a_continuous_grid_is_too_narrow':bool(not interior)}
    res={'rung':'frontier_new_objects_joint','preds':preds,'nulls':nulls,'bars':BARS,
         'arms':out,'published_L2_F':2.6735,
         'grids':{'tail_scale':TS_GRID,'cp_options':[c for c,_ in CP_OPTS],'motif':M_GRID},
         'summary':{'L2_F_baseline':round(bF,4),'L2_F2_baseline':round(b2,4),
                    'cost_fresh':cost,'cost_holdout':cost_h,'cost_fit':cost_c,
                    'svd_identity_cost':ident_svd,'cp_identity_cost':ident_cp,
                    'S2923_incumbent_cost':inc,'S2923_reference':S2923_COST,
                    'anchor_deviation':round(abs(inc-S2923_COST),4),
                    'best_cell':{'tail_scale':best[0],'cp':best[1],'motif':best[2]},
                    'best_cost':best_cost,'best_holdout_cost':cost_h[best_name],
                    'S2923_holdout_cost':cost_h['S2923'],
                    'improves_by':round(inc-best_cost,4),
                    'holdout_improvement':round(cost_h['S2923']-cost_h[best_name],4),
                    'selection_bias':round(best_cost-cost_h[best_name],4)*-1,
                    'optimum_interior':bool(interior),
                    'implied_L2_F_best':round(bF+best_cost,4),
                    'implied_L2_F2_best':round(b2+cost_h[best_name],4),
                    'S2923_L2_F':2.2732,'S2923_L2_F2':2.2953},
    ...
    print(f"(b) identities: svd {ident_svd:+.4f} cp {ident_cp:+.4f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) SS2923 incumbent {inc:+.4f} vs {S2923_COST:+.4f}: {'HELD' if pc else 'FAILED'}")
    for cn, _ in CP_OPTS:
        print(f"    CP {cn}: " + " | ".join(f"t{ts}m{mm} {cell[(ts,cn,mm)]:+.4f}" for ts in TS_GRID for mm in M_GRID))
    print(f"(d) best {best_name} {best_cost:+.4f} beats SS2923 by {inc-best_cost:+.4f}: {'HELD' if pd else 'FAILED'}")
    print(f"(e) held out {cost_h[best_name]:+.4f} vs {cost_h['S2923']:+.4f}: {'HELD' if pe else 'FAILED'}")
    print(f"(f) interior in tail-scale and motif: {'HELD' if pf else 'FAILED'}")
    print(f"    implied L2_F {bF+best_cost:.4f} (SS2923: 2.2732) | held out {b2+cost_h[best_name]:.4f} (SS2923: 2.2953)")
