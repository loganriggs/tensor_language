"""ALL-QK-16 (rung 185): the uniform-grammar candidate - rank-16 patterns at EVERY replaced attention head.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2280: rank 16 crosses the pattern
threshold at the motifs (+1.6507 with tail dicts). Here the tail dictionaries are RETIRED and blocks 10-17
also get rank-16 QK patterns (all 72 tail heads + the 76 motif heads; weights-only; hooks armed after real
baselines). One grammar everywhere the attention is replaced: z = pat_16 @ v_real, y = c_proj(z).

REGISTERED PREDICTIONS (anchors: rung-182 census +1.6507 with dicts; rung-183 r8-tail failure +2.0241):
  (a) THE TAIL CROSSES TOO: census <= 1.75 (within 0.10 of motif-QK-16+dicts despite retiring the dicts).
  (b) RETRIEVAL SURVIVES: name <= 3.2.
  (c) SANITY: census >= 1.3; inert tripwire.
NULL: the tail needs dictionaries even at rank 16 (census > 1.9) - retrieval patterns are sharper than
composition patterns. PRICE: (76+72) x 4 x 16 x 1280 = 12.1M values, REPLACING the dict store.
Self-reviewed."""
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['qk_r']=16; SEL['qk_tail']=True
    ...
    pa=agg<=1.75
    pb=PC.get('name',9.9)<=3.2
    pc=agg>=1.3
    if abs(agg-1.8950)<1e-3:
        raise SystemExit('INSTRUMENT FAIL: tail QK inert')
    ...
    print(f"(a) census {agg:+.4f} <= 1.75: {'HELD' if pa else 'FAILED'}")
    print(f"(b) name {PC.get('name',9.9):+.4f} <= 3.2: {'HELD' if pb else 'FAILED'}")
    print(f"(c) census >= 1.3: {'HELD' if pc else 'FAILED'}")
