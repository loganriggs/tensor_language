"""DOES PER-LAYER TUNING COMPOSE? THE CONSTRUCTIVE TEST OF SS2898's ADDITIVITY (Claude, LANE 1)

SS2896 adopted a global scale of 0.25 on the tail-refit link maps, taking the SS312 frontier from **+2.6736 to +2.4448** -- SIGN
(SS2135): frontier L2 is CE ADDED ABOVE THE REAL MODEL, **LOWER IS BETTER**. SS2898 then found the tail band is **the only nearly-additive
block in this construction**: eight per-layer gains summing to -0.2512 against a global -0.2288, a gap of 0.0224 (under 10%), while every
other block carries interactions an order of magnitude larger -- the motif band subadditive (SS2889/SS2892), the front MLP stage
superadditive and one-way directional (SS2880/SS2897: 4.9479 vs 0.0215).

Additivity is a claim with a **constructive** consequence: if the layers really are independent knobs, then tuning each one separately
and composing the result should beat the single global scale by about the sum of the individual improvements. This rung tests that by
building it.

    RUN 1  per-layer grid : every tail layer at every scale in {.05,.10,.20,.35,.60}, one fitted stack, 41 arms
    RUN 2  composition    : the per-layer argmin composed into ONE arm, against the global 0.20 and the baseline

pred_c is the additivity test in its sharpest form -- not a correlation but a **prediction of the composed number** from the singles.
pred_d re-anchors the global arm to SS2898's -0.2291, making this a sixth reproduction of the adopted effect.

SS2128/SS2129/SS2133/SS2134 RETRACTED; SS2125 STANDS; the published frontier remains norm-2304 at 2.6735.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_the_composed_per_layer_scale_beats_the_global
#                     pred_c_the_composed_gain_is_predicted_by_additivity
#                     pred_d_the_global_arm_reproduces_S2898
#                     pred_e_several_layers_prefer_a_different_scale

Preregistration: polynomial_causal/FRONTIER_TAIL_PER_LAYER_OPTIMUM_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; TWO pipeline runs: a per-layer grid, then the composed optimum")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_tail_per_layer_optimum_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
def main():
    ...
    def _apply_scale(sc, only=None, per=None):
        ...
        for _li in range(10, 18):
            ...
            _sc = (per.get(_li) if per is not None else (sc if (only is None or _li == only) else None))
            if _sc == 'empty':
    ...
    def _cfg(spec):
        _apply_scale('empty' if spec.get('empty') else spec.get('scale'), spec.get('only'), spec.get('per_layer'))
...
PREREG = PT + '../polynomial_causal/FRONTIER_TAIL_PER_LAYER_OPTIMUM_PREREGISTRATION.md'
PREREG_SHA = "5f36ae612d736eb702b7f8298ec61cc23212364edd322ece0883a582a43d9c6d"
...
S2898 = PT + 'frontier_tail_scale_profile_results.json'
S2898_SHA = "3c98d82eb1d33194ba4aa0f0ad8914a2dfc17d6917be61d81d8349fb11794ec7"
LAYERS = list(range(10, 18))
GRID = [0.05, 0.10, 0.20, 0.35, 0.60]
GLOBAL_BEST = 0.20
BARS = {"reproduce": 0.05, "beats_global": 0.01, "additive": 0.05, "anchor": 0.01, "n_differ": 3}
NULLS = {"beats_global_le": 0.0, "additive_ge": 0.10}
...
if __name__=='__main__':
    ...
    if _sha(S2898) != S2898_SHA:
        raise RuntimeError(f'frozen hash mismatch: {S2898}')
    p98 = json.load(open(S2898))['summary']
    global_best_cost = p98['best_global_cost']          # -0.2291 at s=0.20
    SEL['mode']='norm'

    lk = lambda l, s: f'L{l}s{int(round(s*100)):03d}'
    print('RUN 1/2: per-layer grid -- every tail layer at every scale, one fitted stack',flush=True)
    ARMS[:] = [('baseline', {})] + [(lk(l, s), {'scale': s, 'only': l}) for l in LAYERS for s in GRID]
    out=main()
    bF = out['baseline']['L2_F']
    grid = {l: {s: round(out[lk(l, s)]['L2_F'] - bF, 4) for s in GRID} for l in LAYERS}
    best_per_layer = {l: min(GRID, key=lambda s: grid[l][s]) for l in LAYERS}
    per_layer_gains = {l: grid[l][best_per_layer[l]] for l in LAYERS}
    additive_pred = round(sum(per_layer_gains.values()), 4)

    print(f'RUN 2/2: composed per-layer optimum {best_per_layer} vs the global {GLOBAL_BEST}',flush=True)
    ARMS[:] = [('baseline', {}),
               ('composed', {'per_layer': dict(best_per_layer)}),
               ('global_best', {'scale': GLOBAL_BEST})]
    out2=main()
    b2 = out2['baseline']['L2_F']
    c_comp   = round(out2['composed']['L2_F'] - b2, 4)
    c_global = round(out2['global_best']['L2_F'] - b2, 4)

    beats = round(c_global - c_comp, 4)                 # POSITIVE = composed is better (lower L2)
    add_dev = abs(c_comp - additive_pred)
    anchor_dev = abs(c_global - global_best_cost)
    n_differ = sum(1 for l in LAYERS if best_per_layer[l] != GLOBAL_BEST)

    pa = abs(b2-2.6735) <= BARS['reproduce']
    pb = beats >= BARS['beats_global']
    pc = add_dev <= BARS['additive']
    pd = anchor_dev <= BARS['anchor']
    pe = n_differ >= BARS['n_differ']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_composed_per_layer_scale_beats_the_global':bool(pb),
           'pred_c_the_composed_gain_is_predicted_by_additivity':bool(pc),
           'pred_d_the_global_arm_reproduces_S2898':bool(pd),
           'pred_e_several_layers_prefer_a_different_scale':bool(pe)}
    nulls={'b_null_per_layer_tuning_buys_nothing':bool(beats<=NULLS['beats_global_le']),
           'c_null_additivity_fails':bool(add_dev>=NULLS['additive_ge'])}
    res={'rung':'frontier_tail_per_layer_optimum','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,'layers':LAYERS,'grid':GRID,
         'run1_arms':out,'run2_arms':out2,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(b2,4),
                    'per_layer_grid':{f'a{l}L':{str(s):grid[l][s] for s in GRID} for l in LAYERS},
                    'best_scale_per_layer':{f'a{l}L':best_per_layer[l] for l in LAYERS},
                    'best_gain_per_layer':{f'a{l}L':per_layer_gains[l] for l in LAYERS},
                    'additive_prediction':additive_pred,
                    'cost_composed':c_comp,'cost_global_best':c_global,
                    'composed_beats_global_by':beats,
                    'additivity_deviation':round(add_dev,4),
                    'S2898_global_best_cost':round(global_best_cost,4),
                    'anchor_deviation':round(anchor_dev,4),
                    'n_layers_preferring_a_different_scale':n_differ,
                    'implied_L2_F_composed':round(b2 + c_comp, 4),
                    'S2896_adopted_L2_F':2.4448},
         ...
         'hashes':{PREREG:PREREG_SHA,S2898:S2898_SHA},'self_reviewed':True}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) baseline L2_F {b2:.4f} vs 2.6735: {'HELD' if pa else 'FAILED'}")
    print(f"    best scale per layer: {best_per_layer}")
    print(f"(b) composed {c_comp:+.4f} vs global {c_global:+.4f}, beats by {beats:+.4f} >= {BARS['beats_global']}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) additive prediction {additive_pred:+.4f}, dev {add_dev:.4f} <= {BARS['additive']}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) global arm vs SS2898 {global_best_cost:+.4f}, dev {anchor_dev:.4f}: {'HELD' if pd else 'FAILED'}")
    print(f"    implied L2_F composed = {b2 + c_comp:.4f} (SS2896 adopted 2.4448)")
