...
"""Joint blockgroup projection control for signed/mixed source effects.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v694_result.json'
PREDICTIONS=dict(pred_a_exact_replay='original instrument plus group6 oracle replay<=1e-8',
 pred_b_effect_prediction='group2 target relative error<=.10 acrossall8arms/cells',
 pred_c_capable_position_transfer='group2 worst target error lower than group1')
SOURCE_ARMS=dict(A=(1.,0.),B=(0.,1.),sum=(1.,1.),half_sum=(.5,.5),negA=(-1.,0.),negB=(0.,-1.),difference=(1.,-1.),twiceA=(2.,0.))


def main():
    plan=dict(prefix_calls=8,suffix_calls=56,sequences_per_arm=48,fits=0,backwards=0,grouped_attention_calls=576,grouped_mlp_calls=576,extra_attention_pullbacks=8,extra_reference_forwards=16,extra_token_local_mlp_pullbacks=24,extra_fulltoken_mlp_pullbacks=24,analytic_reverse_passes=4,attention_pullbacks=24,reference_attention_forwards=48,additional_tangent_suffix_calls=8,additional_initial_interface_suffix_calls=32,
    ...
    group_checks=[];group_counts=dict(attention=0,mlp=0)
    group_reports={str(size):{name:{} for name in SOURCE_ARMS} for size in [1,2,6]}
    optimized_checks=[]
    ...
    for entries in batches:
        ...
        for source_name,(alpha,beta) in SOURCE_ARMS.items():
            ...
            for group_size in [1,2,6]:
                gx=initial_state.clone()
                for start in range(0,6,group_size):
                    stop=min(start+group_size,6)
                    for k in range(start,stop):
                        block=model.transformer.h[k+12]
                        gx=block.lambdas[0]*gx+block.lambdas[1]*x0
                        ga,_=block.attn(F.rms_norm(gx,(gx.shape[-1],)),first);group_counts['attention']+=1
                        gx=gx+ga;gx=gx+block.mlp(F.rms_norm(gx,(gx.shape[-1],)));group_counts['mlp']+=1
                    gz=(gx.double()-context['baseline_states'][stop])@output_encoders[stop-1]
                    if stop<6:gx=(context['baseline_states'][stop]+gz@programs[stop]['input_basis'].T).float()
                glogits=readout_prepared(runtime['readout'],gz[batch,readpos],case['final_context'])
                geffect=base-(glogits[:,0]-glogits[:,1])*direction
                if group_size==6:group_checks.append(float((geffect-initial_effect).abs().max()))
                for i,e in enumerate(entries):
                    gc=group_reports[str(group_size)][source_name].setdefault(e[3],dict(target=[],prediction=[]))
                    gc['target'].append(float(native_effect[i]));gc['prediction'].append(float(geffect[i]))
    ...
    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,cells=cells,
                ...
                scope='Native blockgroup projection control (full native weights and computation charged), alongside dense36-pair number predictor plus selected attention12/allMLP modal reader. Eight signed/mixed post11 source arms on48opened v691 rows. Fixed v665 frames; full native baseline/context/reader/source generators remain external. Restores quadratic pairs only. No fresh-text, token-input closure, residual-state replacement or full-model adoption claim.')
