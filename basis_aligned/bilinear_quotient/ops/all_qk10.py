"""ALL-QK-10 (rung 187): the Hadamard-rank law, below-knee arm.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. r = 10 gives effective pattern rank
~100 < 128: the law predicts DEGRADATION. The rung-186 (r12) receipt is read at RUN time (runner order).

REGISTERED PREDICTIONS:
  (a) BELOW THE KNEE: census >= 1.55.
  (b) MONOTONE KNEE: census(r10) >= census(r12) + 0.10.
  (c) SANITY: census >= 0.5 AND census <= 2.6; inert tripwire.
NULL: r10 also crosses (<= 1.45) - r* is below sqrt(128); the law's constant is off (the knee sits in
[8, 10]). PRICE: attribution. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: all-QK-10 (Hadamard law below-knee)')
    raise SystemExit(0)
import torch
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['qk_r']=10; SEL['qk_tail']=True
    ...
    p186=PT+'all_qk12_results.json'
    if not os.path.exists(p186): raise SystemExit('INSTRUMENT FAIL: rung-186 receipt missing at run time')
    A186=json.load(open(p186)).get('census_agg') or json.load(open(p186)).get('agg')
    pa=agg>=1.55
    pb=agg>=A186+0.10
    pc=0.5<=agg<=2.6
    ...
    print(f"(b) census {agg:+.4f} >= r12+0.10 ({A186+0.10:+.4f}): {'HELD' if pb else 'FAILED'}")
    print(f"(c) census in [0.5, 2.6]: {'HELD' if pc else 'FAILED'}")
