...
def main():
 ...
 torch.set_num_threads(4);torch.backends.cuda.matmul.allow_tf32=False;probe=torch.arange(16,dtype=torch.float64).reshape(4,4)/17;device_error=float((symmetric_square(probe.cuda()).cpu()-symmetric_square(probe)).norm());assert device_error<1e-12;out=P/'NATIVE_MIXED_ROOT_V2.json';assert not out.exists();start=time.perf_counter();source=torch.load(P/'ROOT_BASIS_REFINE_V1.pt',weights_only=True)['students'];old={(r['optimizer'],r['lr'],r['seed']):r for r in json.load(open(P/'NATIVE_LEARNED_SHARED_BANK_V1.json'))['records']};state=torch.load('/workspace/.hf_home/hub/models--Elriggs--gpt2-bilinear-sqrd-attn-18l-9h-1152embd/snapshots/ed9146549ee6dc8ed8cd75e9d48fcfe4278f4240/pytorch_model.bin',weights_only=True)
