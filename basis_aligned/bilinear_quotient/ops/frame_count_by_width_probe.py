#!/usr/bin/env python
"""frame_count_by_width_probe -- the (frames x k) trade-off: the early-frame sharing penalty (16 own / 8 per block / 4 per pair / 1)
at k = 896 and k = 768, reads only, late reads through the bus U_8 (§2765 measured it at 1024: +.00005 / +.0028 / +.0133). CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_block_frames_cheap_at_896 pred_c_block_frames_under_p03_at_768
#                     pred_d_one_early_frame_is_a_cliff_at_768 pred_e_sharing_penalty_grows_as_k_shrinks
...
Preregistration: polynomial_causal/FRAME_COUNT_BY_WIDTH_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/frame_count_by_width_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("frame_count_by_width_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "FRAME_COUNT_BY_WIDTH_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "early_frame_count_probe_results.json"   # §2765
OUT = ROOT / "frame_count_by_width_probe_results.json"
HASHES = {PREREG: "5bc842df1a107084bcec09cc633d12ca38f0fd16ddf6dda310822683b99fe1cd", PRIOR: "f177a210a58bc477f42ae7a2840487ca3fb05be76e1e563e945f8783e8125ed2",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "frame_count_by_width_probe"
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "b_max": 0.010, "c_max": 0.030, "d_min": 0.100, "e_min": 0.027}
NULLS = {"b_min": 0.030, "c_min": 0.060, "d_max": 0.030, "e_max": 0.015}
...
def main():
    ...
    def read_program(k, frame_for):
        ...
        return run(patch)
    arms = {"SPLIT8_1024": read_program(1024, lambda s_: None)}
    KS = [1024] if smoke else [896, 768]
    for k in KS:
        arms[f"OWN16_{k}"] = read_program(k, lambda s_: None)
        arms[f"BLOCK8_{k}"] = read_program(k, lambda s_: FB[s_[1]])
        arms[f"PAIR4_{k}"] = read_program(k, lambda s_: FP[s_[1] // 2])
        arms[f"EARLY1_{k}"] = read_program(k, lambda s_: F1)
        log(stage="width", k=k, **{n: round(arms[f"{n}_{k}"], 4) for n in ("OWN16", "BLOCK8", "PAIR4", "EARLY1")})
    pen = {f"P_{k}_{n}": arms[f"{nm}_{k}"] - arms[f"OWN16_{k}"] for k in KS for n, nm in ((8, "BLOCK8"), (4, "PAIR4"), (1, "EARLY1"))}
    ka, kb = (KS[0], KS[-1])
    ...
    preds = {
        ...
        'pred_b_block_frames_cheap_at_896': bool(pen[f"P_{ka}_8"] <= BARS["b_max"]),
        'pred_c_block_frames_under_p03_at_768': bool(pen[f"P_{kb}_8"] <= BARS["c_max"]),
        'pred_d_one_early_frame_is_a_cliff_at_768': bool(pen[f"P_{kb}_1"] >= BARS["d_min"]),
        'pred_e_sharing_penalty_grows_as_k_shrinks': bool(pen[f"P_{ka}_1"] >= BARS["e_min"] and pen[f"P_{kb}_1"] >= pen[f"P_{ka}_1"]),
    }
    nulls = {"b_null_ge_.03": bool(pen[f"P_{ka}_8"] >= NULLS["b_min"]), "c_null_ge_.06": bool(pen[f"P_{kb}_8"] >= NULLS["c_min"]),
             "d_null_le_.03": bool(pen[f"P_{kb}_1"] <= NULLS["d_max"]), "e_null_896_le_.015": bool(pen[f"P_{ka}_1"] <= NULLS["e_max"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "split": SPLIT, "widths": KS,
           "program": "reads only: early frames own (16) / per block (8) / per pair (4) / one (1) at k in {896, 768}; late reads through U_8; penalty P_k(n) = arm - OWN16_k",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024},
           "ce_added": arms, "penalties": pen, "prior_1024_penalties": {"P_1024_8": 0.00005, "P_1024_4": 0.0028, "P_1024_1": 0.0133},
    ...
    dump(out, OUT)
    print(json.dumps({"preds": preds, "nulls": nulls, "ce_added": arms, "penalties": pen}, indent=1))
