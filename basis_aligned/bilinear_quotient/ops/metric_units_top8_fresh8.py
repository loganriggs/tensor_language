"""CERTIFY THE EIGHT-DIRECTION SELECTOR on the eight fresh windows (rung 25).
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['stream_error_profile_results.json','metric_units_halfprice8_results.json','metric_units_top8_results.json']
    ...
    print(f"DRYRUN OK: S2086 present (block 6 {_p['6']}); norm vs top-8 at K=2304 x eight fresh windows")
    raise SystemExit(0)
import json, sys, time, torch
sys.path.insert(0,'/workspace/tensor_language/basis_aligned/bilinear_quotient')
import os
...
OUT=PT+'metric_units_top8_fresh8_results.json'
...
def main():
    ...
    for wi,w in enumerate(WIN):
        ...
        rows.append({'window':wi,'docs':w['docs'],'base_ce':round(b,5),'gap_plain':round(g['plain'],5),
                     'gap_top8':round(g['top8'],5),'gain':round(g['plain']-g['top8'],5)})
        print(f"window {wi}: docs {w['docs']:3d} base {b:.4f} | gap plain {g['plain']:+.4f} top8 {g['top8']:+.4f} | gain {g['plain']-g['top8']:+.4f}",flush=True)
    gains=[r['gain'] for r in rows]
    import statistics as stt
    gain1=gap1['plain']-gap1['top8']
    pa=sum(g>0 for g in gains)>=7; pb=stt.median(gains)>=0.6*0.082; pc=abs(gain1-0.1285)<=0.02
    pd=abs(arms['plain']['profile'][6]-1.7415)<=0.10
    out={'windows':rows,'gain_median':round(stt.median(gains),4),'gain_mean':round(stt.mean(gains),4),'windows_positive':int(sum(g>0 for g in gains)),
         'window1_gain':round(gain1,4),'S2110_window1_gain':0.1285,'S2116_full_metric_median':0.082,'profiles':{k:v['profile'] for k,v in arms.items()},
         'pred_a_positive_on_7_of_8':bool(pa),'pred_b_median_ge_sixty_percent':bool(pb),'pred_c_reproduces_S2110':bool(pc),'pred_d_reproduces_S2086':bool(pd),
         'self_reviewed':True,'runtime_s':round(time.time()-t0,1)}
    json.dump(out,open(OUT,'w'),indent=1)
    print(f"(d) cfgE b6 {arms['plain']['profile'][6]:.4f}: {'HELD' if pd else 'FAILED'}")
    print(f"(c) window-1 top-8 gain {gain1:+.4f} vs 0.1285: {'HELD' if pc else 'FAILED'}")
    print(f"(a) positive on {out['windows_positive']}/8: {'HELD' if pa else 'FAILED'}")
    print(f"(b) median {out['gain_median']:+.4f} >= {0.6*0.082:.4f}: {'HELD' if pb else 'FAILED'}")
