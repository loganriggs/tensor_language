#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_11_3_leads_block_11 pred_c_7_8_leads_block_7 pred_d_one_head_per_block pred_e_singles_add_to_blocks
"""The verb readout's heads (v448). v447: for a distant verb the number leaves the noun through blocks 4 (the copier), 11, 7, 9 and 13. The readout atlas's
'number' family is 11.3 / 5.7 / 7.8 / 9.7. Here blocks 11 and 7 by head (each head's value at the noun, swapped within the pair) on the 79 distant rows.
PREDICTIONS (scored as written; failures preserved; priors from the atlas and v447)
    pred_a_baseline_replays     the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_11_3_leads_block_11  head 11.3 has the largest single-head effect in block 11. Prior: likely (atlas)
    pred_c_7_8_leads_block_7    head 7.8 has the largest single-head effect in block 7. Prior: likely (atlas)
    pred_d_one_head_per_block   in each of the two blocks the largest head carries >= 0.60 of the block's effect (0.154 / 0.095)
...
OUT = ROOT / "circuits/followups/verb_heads_v448_result.json"
CANDIDATE_ID = "chain.verb_heads_v448"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, TOP_SHARE_MIN, ADD_TOL, B11, B7 = 1e-3, 0.60, 0.04, 0.154, 0.095
...
FORWARDS_MAX = 58
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_11_3_leads_block_11": "11.3 first", "pred_c_7_8_leads_block_7": "7.8 first", "pred_d_one_head_per_block": ">= 0.60 x 2", "pred_e_singles_add_to_blocks": "within 0.04 x 2"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "top_share_min": TOP_SHARE_MIN, "add_tol": ADD_TOL, "b11_v447": B11, "b7_v447": B7}}
    ...
    MODES = {f"{b}.{h}": ({b: [h]}, (0,)) for b in (11, 7) for h in range(N_HEAD)}; forwards = 0
    def logits_margin(x):
    ...
    c = verb["closed"]
    v = {m: c[m]["verb"] for m in c}; b11 = {m: v[m] for m in v if m.startswith("11.")}; b7 = {m: v[m] for m in v if m.startswith("7.")}; report["sums"] = {"11": sum(b11.values()), "7": sum(b7.values())}
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_11_3_leads_block_11": max(b11, key=b11.get) == "11.3", "pred_c_7_8_leads_block_7": max(b7, key=b7.get) == "7.8",
                   "pred_d_one_head_per_block": max(b11.values()) >= TOP_SHARE_MIN * B11 and max(b7.values()) >= TOP_SHARE_MIN * B7, "pred_e_singles_add_to_blocks": abs(sum(b11.values()) - B11) <= ADD_TOL and abs(sum(b7.values()) - B7) <= ADD_TOL}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "verb_heads_result_v448", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
