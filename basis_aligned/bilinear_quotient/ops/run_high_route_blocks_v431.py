#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_reader_blocks_are_the_top_three pred_c_blocks_16_17_carry_a_share pred_d_singles_add_to_the_half pred_e_no_high_block_is_negative
"""The high route by block (v431). v429: values at the noun for blocks 9-17 close 0.505 of the margin gap (text); v427: the three readers 0.368, all 27
heads of blocks 9 / 12 / 15 0.381. Here each block 9..17 on its own (every head's value at the noun, swapped within the pair), natural pairs only: are the
readers' blocks the top three, what do blocks 16-17 add, and do the singles add.
PREDICTIONS (scored as written; failures preserved; priors from v427 / v429)
    pred_a_baseline_replays              the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_reader_blocks_are_the_top_three  blocks 9, 12 and 15 are the three largest single-block effects. Prior: likely
    pred_c_blocks_16_17_carry_a_share    blocks 16 and 17 together close >= 0.05 (the 0.12 gap between 0.505 and 0.381 must sit somewhere). Prior: unsure
    pred_d_singles_add_to_the_half       the nine single-block effects sum to within 0.15 of the joint 0.505. Prior: unsure
    pred_e_no_high_block_is_negative     no single block closes <= -0.03
...
OUT = ROOT / "circuits/followups/high_route_blocks_v431_result.json"
CANDIDATE_ID = "both_ends.high_route_blocks_v431"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1))
REPLAY_TOL, B1617_MIN, ADD_TOL, HIGH_V429, NEG_MIN = 1e-3, 0.05, 0.15, 0.505, -0.03
FORWARDS_MAX = 41
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_reader_blocks_are_the_top_three": "{9, 12, 15}", "pred_c_blocks_16_17_carry_a_share": ">= 0.05", "pred_d_singles_add_to_the_half": "within 0.15 of 0.505", "pred_e_no_high_block_is_negative": "> -0.03 x 9"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "b1617_min": B1617_MIN, "add_tol": ADD_TOL, "high_v429": HIGH_V429, "neg_min": NEG_MIN}}
    ...
    MODES = {f"b{b}": b for b in range(9, 18)}; forwards = 0
    def logits_margin(x):
    ...
    c = text["closed"]
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_reader_blocks_are_the_top_three": set(sorted(c, key=c.get, reverse=True)[:3]) == {"b9", "b12", "b15"}, "pred_c_blocks_16_17_carry_a_share": c["b16"] + c["b17"] >= B1617_MIN,
                   "pred_d_singles_add_to_the_half": abs(text["sum_singles"] - HIGH_V429) <= ADD_TOL, "pred_e_no_high_block_is_negative": all(v > NEG_MIN for v in c.values())}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "high_route_blocks_result_v431", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
