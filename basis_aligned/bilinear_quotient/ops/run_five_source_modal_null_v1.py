...
24prefix,128double+64native suffix forwards,64gradient+320Hessian-row reverse;192 bounded LPs.
All native generators charged. Null: number prediction hides modal collateral.
"""
import os,json,time,sys,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';OUT=A/'five_source_modal_null_v1_result.json'
ARMS=dict(unitB=[0,0,1,1,1],null_full=None,null_half=None)
PREDICTIONS=dict(pred_a_instrument='native effect replay1e-4; FDgradient1%/Hessian15%; symmetry1e-4; Bderivative replay1e-8; LP residual1e-8; counts',pred_b_selective_retention='null_full native aligned number retention>=.80 unitB and modal ratio<=.10 every cell',pred_c_prediction='number error10% and modal error5%number-effect every arm/cell')
PLAN=dict(prefix=24,double_suffix=128,native_suffix=64,gradient=64,hessian=320,fits=0,linear_programs=192,amplitudes=ARMS,coefficients_per_context=80,predictions=PREDICTIONS)
...
def main():
    ...
    old=json.loads((A/'semantic_source_multiobservable_v1_result.json').read_text())
    ...
    for panel,digest in [('opposite','1869db8a18d4c89c28fa81a6bb57a19497e6f8692831eb4391f5d23c84556d73'),('congruent','bfba23ab066beb14f5cd469edce137db7ec1e8ccc26bfc36b66f896426b17511')]:
        ...
        for template in dict.fromkeys(r['template'] for r in rows):
            ...
            for role,key in [('subject','subject_position'),('attractor','control_position')]:
                ...
                checks.append(check)
                previous=oldcontexts[(panel,role,template)]
                derivative_replay.append(float((g[:,:,2:]-torch.tensor(previous['gradient'],device='cuda',dtype=torch.float64)).abs().max()))
                derivative_replay.append(float((h[:,:,2:,2:]-torch.tensor(previous['hessian'],device='cuda',dtype=torch.float64)).abs().max()))
                candidates=[]
                ...
                for arm,amplitude in ARMS.items():
                    av=torch.tensor(amplitude,device='cuda',dtype=torch.float64).expand(len(a),5) if arm=='unitB' else candidate*(.5 if arm=='null_half' else 1.)
    ...
    a=max(replays)<=1e-4 and max(lp_checks)<=1e-8 and max(derivative_replay)<=1e-8 and counts=={k:PLAN[k] for k in counts} and all(c['symmetry']<=1e-4 and all(max(f['gradient_relative'])<=.01 and max(f['hessian_relative'])<=.15 for f in c['finite']) for c in checks)
    selected=[]
    for c in records:
    ...
    result=dict(plan=PLAN,counts=counts,predictions=dict(zip(PREDICTIONS,[bool(a),bool(a and all(c['aligned_retention']>=.8 and c['modal_ratio']<=.1 for c in selected)),bool(a and all(c['number_error']<=.1 and max(c['modal_error'])<=.05 for c in records))])),selection=selected,max_lp_residual=max(lp_checks),max_B_derivative_replay=max(derivative_replay),max_native_effect_replay=max(replays),checks=checks,records=records,contexts=contexts,seconds=time.perf_counter()-tic)
