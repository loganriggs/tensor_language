#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the fifty-eighth two spec-authored behaviours, plus one instrument.
"""v405: the AMENDED tier-3 battery over the FIFTY-EIGHTH batch (TWO cells) -- plus one instrument.
Two SIBLING mappings for the determiner-number class, authored so the class can have a separability rung at all.
v285 2/7, ..., v369 2/3, v397 1/1 (+instrument each).
WHY THESE TWO. v397 gave the corpus its first non-verb-cue circuit (several / each -> ` crates` / ` crate`, four rows
first try, 8 units, held 0.898, own-C 0.0026) and v401 and v403 gave it a breadth of four shapes in sample and
out-of-sample transfer of 0.983 and 0.974. It still counts ZERO, because a four-row pass with no family sits at R6
until a separability rung exists, and a separability rung needs a SIBLING to be separable against. These are the
siblings:
    determiner_number_ropes    many / one         -> ` ropes` / ` rope`      (frayed)
    determiner_number_barrels  numerous / another -> ` barrels` / ` barrel`  (cracked)
Both use a fresh quantifier pair and a fresh noun, so under the measured gate -- a (cue word -> readout token) mapping
may be reused in a different FRAME but never within one -- each is a distinct behaviour rather than a shape variant of
the first. All three share the construction, the frame and the P control, which is what makes the separability rung
that follows a real test: the three directions must avoid each other while doing the same job on different words.
CPU capability margins BEFORE enqueue (mean base / mean donor; A1 | A2; dropped rows; floor 1.0):
  determiner_number_barrels  A1:-4.55/+4.61 drop0  A2:-4.38/+4.30 drop0
  determiner_number_ropes    A1:-4.31/+4.05 drop0  A2:-4.29/+4.16 drop0
  Both stronger than the parent cell (-4.03/+3.70), and the class is now 3 of 3 on capability with zero dropped rows
  anywhere -- determiner-noun number agreement across an intervening adjective is something this model does reliably.
Protocol identical to v197/v201/.../v397. finiteness_selection is the instrument and is EXCLUDED from four-row counts.
Batch size TWO; bars are the two-cell bars (1/1/2/1/1).

REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 1 of the 2 new behaviours passes all four rows. The parent passed all four first
                           try, but one cell is not a rate.                                               prior 70%
  pred_b_row2           -> MIN held-out per-direction exact-set recovery >= 0.80 on at least 1 of 2.       prior 70%
  pred_c_row3           -> dim A1 CE damage LB975 > 0 and >= 0.10 on both.                                 prior 80%
  pred_d_row4           -> constrained rank-1 own-C CE UB975 <= 0.01 on at least 1 of 2. Row 4 is where this corpus
                           fails most (76 of 186 misses), and for these cells the control is a lexical rewrite of the
                           same determiner frame, which is the hardest kind to spare.                      prior 55%
...
Accounting: a pass by either sibling makes a determiner_number FAMILY possible, and the rung after it decides whether
the class contributes one entry or three. Under the day's evidence I expect three -- fresh cue -> token mappings in
one frame have been separable in every family tested -- but that is a prediction the rung will settle, not an
assumption I get to count on now.
An error receipt per behaviour counts as a miss on every row. ~40 s GPU each, ~2 min.
Smoke: V405_SMOKE=<out.json> (CPU, V405_SMOKE_ROWS=4 per split, V405_SMOKE_NAMES=determiner_number_ropes).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec9p_v405_result.json"
...
NAMES = {"determiner_number_ropes": "determiner_number_ropes", "determiner_number_barrels": "determiner_number_barrels",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v405", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V405_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V405_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V405_SMOKE_NAMES", "determiner_number_ropes").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec9p_v405", "candidate_id": "corpus.unit_tier3_batch_amended_spec9p_v405", "bars": BARS,
