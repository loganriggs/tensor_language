# IS THE q1/q2 CE INVERSION A FREQUENCY CONFOUND? -- measuring it instead of guessing.
#
# §1925 found the reliability signal orders CE strongly at the extremes (+1.2107 / +1.2329 / +1.2553 nats
# between the best and worst covered quartile) but NOT monotonically: q1 sits BELOW q2 on all six
# arm-role pairs. Six of six is not noise, and §1925 declined to propose a mechanism -- §1888, §1890,
# §1898-§1900 and §1923 are six sections of what happens when I explain a pattern before measuring it.
#
# There is one confound cheap enough to just measure. The quartiles are cut over TYPES but weighted by
# POSITIONS: the covered counts are 6,489 / 4,780 / 7,479 / 9,226, so the quartiles hold very different
# numbers of positions and therefore very different token-frequency mixtures. If CE tracks token
# frequency independently of the margin -- which §1789 makes likely, since the program keeps ~63% of the
# model on frequent targets and ~3% on rare ones -- then a quartile holding more frequent tokens gets a
# lower CE for a reason that has nothing to do with reliability.
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY, deployed 5,419. Rung 3: §1925's one open detail.
#
# Registered predictions, TWO-SIDED per LESSONS 31, absolute nats per LESSON 40, read back per LESSON 39.
#   pred_a THE FREQUENCY MIXTURE IS INVERTED THE SAME WAY: the mean current-token fit-frequency of the
#          covered q1 quartile EXCEEDS that of q2, on all three roles -- the same q1-over-q2 inversion
#          the CE shows. If FALSE the frequency mixture does not have the shape that would explain the CE
#          inversion, the confound is ruled out, and §1925's pattern needs a different account that I
#          would then leave open rather than invent.
#   pred_b AND CONTROLLING FOR FREQUENCY REMOVES IT: within the highest target-frequency bucket (§1789's
#          125+), the covered CE is monotone across the four margin quartiles on at least two of three
#          roles. This is the test. If TRUE the inversion is a mixture artifact and the signal IS a
#          ranking once frequency is held; if FALSE it survives the control and is a real property of the
#          margin, which would make §1925's "threshold, not ranking" the permanent statement.
#   pred_c AND THE EXTREME GAP SURVIVES THE CONTROL: within that same bucket, the q0-minus-q3 covered CE
#          gap is at least 0.3 nats on all three roles. §1925's +1.2 was measured across all target
#          frequencies; if it vanishes once frequency is fixed, then the signal was largely a frequency
#          proxy and the handover to the cost side is much weaker than §1925 reported.
#   pred_d CONTROLS: coverage is exactly 5,419; the pooled all-position CE reproduces §1858's PUBLISHED
#          6.01167 / 5.98477 / 6.00165 within 0.002, as it did at §1925; and the per-quartile position
#          counts reproduce §1925's PUBLISHED covered 6489 / 4780 / 7479 / 9226 exactly.
...
OUT = PT + 'ops/frequency_confound_results.json'
...
def main():
    ...
    COV['freq'] = torch.bincount(fit[:, 1:T + 1].reshape(-1).long(),
                                 minlength=V).to(DEV)
    print(f'IS THE q1/q2 INVERSION A FREQUENCY CONFOUND | buckets {BUCKETS} on the fit-row count of the TRUE '
    ...
    S1925_N = [6489, 4780, 7479, 9226]   # §1925 PUBLISHED covered quartile position counts
    ...
    pa = all(fq(e, 1) > fq(e, 2) for e in roles)
    pb = sum(1 for e in roles if mono[e]) >= 2
    pc = all(higap[e] >= 0.3 for e in roles)
    pd = (ncov == NCOV
          and all(abs(allce[e] - S1858_CE[e]) <= 0.002 for e in roles)
          and all(ce_(roles[0], 'cov', i8, 'n') == S1925_N[i8] for i8 in range(4)))
    ...
    print(f'\n  the FREQUENCY mixture is inverted the same way (q1 > q2) -> {pa}  ' + '  '.join(
        f'{e} {fq(e, 1):.1f} vs {fq(e, 2):.1f}' for e in roles), flush=True)
    print(f'  and controlling for frequency REMOVES the inversion (>=2 roles monotone) -> {pb}  '
          + '  '.join(f'{e} {mono[e]}' for e in roles), flush=True)
    print(f'  and the extreme gap SURVIVES the control (>= 0.3 nats) -> {pc}  ' + '  '.join(
        f'{e} {higap[e]:+.4f}' for e in roles), flush=True)
    print(f'  coverage {ncov}, §1858 CE and §1925 counts reproduce -> control {pd}  ' + '  '.join(
        f'{e} {allce[e]:.5f}' for e in roles), flush=True)
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_frequency_inverted_too': bool(pa),
                          'pred_b_control_removes_inversion': bool(pb),
                          'pred_c_gap_survives_control': bool(pc),
