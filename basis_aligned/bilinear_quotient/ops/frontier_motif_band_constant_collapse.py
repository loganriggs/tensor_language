"""COLLAPSE THE WHOLE MOTIF-ATTENTION BAND TO CONSTANTS IN THE SS312 FRONTIER (Claude, LANE 1).

Companion to `frontier_a5_constant_collapse`, which asks the same question of a5 alone. SS2834's census of the REAL model -- an
independent receipt, different data and a different measurement from anything here -- gives the motif band a2-a9 a summed deletion cost
of **3.234 nats** against **0.796** when each write is replaced by its own constant, and gives the TAIL band a10-a17 a summed deletion
cost of only **0.157 nats**, i.e. the tail attention components are very nearly inert in document CE and would make a control that
cannot discriminate. The control here is therefore the band MINUS a5, not the tail.

a5 alone carries 2.211 of the band's 3.234 nats (68%), so the two arms decompose the band: if the whole band collapses for free the
frontier's entire motif-attention stage is a table of eight constant vectors, and if only a5 does, the price cliff is a single
component rather than a stage.

SIGN CONVENTION (SS2135): frontier L2 is CE ADDED ABOVE THE REAL MODEL, so **LOWER IS BETTER** (SS312: +2.6735 beating +2.84/+2.93).
A collapse COST is L2_F(collapsed) - L2_F(baseline), **POSITIVE = WORSE**. SS2128/SS2129/SS2133/SS2134 RETRACTED; SS2125 STANDS and the
frontier is norm-2304 at 2.6735. pred_a is SS2125 rung 30's reproduction gate carried over verbatim.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_the_whole_motif_band_collapses_cheaply
#                     pred_c_the_band_without_a5_also_collapses
#                     pred_d_frontier_row_spread_tracks_the_real_model_census
#                     pred_e_the_parameter_saving_is_stated

Preregistration: polynomial_causal/FRONTIER_MOTIF_BAND_COLLAPSE_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; three arms (baseline / motif band a2-a9 collapsed / band without a5)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_motif_band_constant_collapse_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_MOTIF_BAND_COLLAPSE_PREREGISTRATION.md'
PREREG_SHA = "e7ae6d6c33ac9e97bedbae89b5944acfc68d3964fc8e40e4b571e4d413e21549"
...
BAND = tuple(ATTM)                       # a2..a9, the motif-attention band
A5 = 5
BAND_NO_A5 = tuple(l for l in BAND if l != A5)
BARS = {"reproduce": 0.05, "band_free": 0.10, "no_a5_free": 0.10, "census_rho": 0.50, "row_cos": 0.05}
NULLS = {"band_free_ge": 0.30, "no_a5_free_ge": 0.30, "census_rho_le": 0.10}
CENSUS = PT + 'circuit_battery_constant_write_census_results.json'
...
if __name__=='__main__':
    ...
    print(f'ARM 2/3: the whole motif band a{BAND[0]}-a{BAND[-1]} collapsed to constants',flush=True)
    COLLAPSE['set']=frozenset(BAND)
    r_band=main()

    print(f'ARM 3/3: the band WITHOUT a{A5} collapsed -- isolates a{A5}\'s share',flush=True)
    COLLAPSE['set']=frozenset(BAND_NO_A5)
    r_noa5=main()

    def row_spread(li):
        CV=CVSNAP.get(li)
        if CV is None: return float('nan')
        X=F.normalize(CV.float(),dim=1); C=X@X.T; n=C.shape[0]
        ti=torch.triu_indices(n,n,1)
        return float(1.0-C[ti[0],ti[1]].min())
    spreads={li: row_spread(li) for li in BAND}

    # cross-artifact: does the FRONTIER dictionary's row spread track the REAL model's constancy census?
    cen=json.load(open(CENSUS))['components']
    pairs=[(spreads[li], cen[f'attn{li}']['gain_cv']) for li in BAND
           if f'attn{li}' in cen and not (spreads[li]!=spreads[li])]
    def spear(a,b):
        import numpy as _np
        ra=_np.argsort(_np.argsort(_np.asarray(a,dtype=float))).astype(float)
        rb=_np.argsort(_np.argsort(_np.asarray(b,dtype=float))).astype(float)
        if _np.std(ra)==0 or _np.std(rb)==0: return float('nan')
        return float(_np.corrcoef(ra,rb)[0,1])
    rho=spear([x for x,_ in pairs],[y for _,y in pairs]) if len(pairs)>=4 else float('nan')

    # SIGN: LOWER L2 IS BETTER -> cost POSITIVE = WORSE
    cost_band = r_band['L2_F']  - r_base['L2_F']
    cost_noa5 = r_noa5['L2_F']  - r_base['L2_F']
    saved_per_layer = 10*D + len(LINK)*D*D - D

    pa = abs(r_base['L2_F']-2.6735) <= BARS['reproduce']
    pb = cost_band <= BARS['band_free']
    pc = cost_noa5 <= BARS['no_a5_free']
    pd = (rho==rho) and rho >= BARS['census_rho']
    pe = saved_per_layer > 0
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_whole_motif_band_collapses_cheaply':bool(pb),
           'pred_c_the_band_without_a5_also_collapses':bool(pc),
           'pred_d_frontier_row_spread_tracks_the_real_model_census':bool(pd),
           'pred_e_the_parameter_saving_is_stated':bool(pe)}
    nulls={'b_null_the_band_class_structure_is_load_bearing':bool(cost_band>=NULLS['band_free_ge']),
           'c_null_a5_is_the_only_collapsible_one':bool(cost_noa5>=NULLS['no_a5_free_ge']),
           'd_null_frontier_spread_is_unrelated_to_the_census':bool(rho==rho and rho<=NULLS['census_rho_le'])}
    res={'rung':'frontier_motif_band_constant_collapse','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,'band':list(BAND),'band_without_a5':list(BAND_NO_A5),
         'baseline':r_base,'band_collapsed':r_band,'band_without_a5_collapsed':r_noa5,
         'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(r_base['L2_F'],4),
                    'L2_F_band_collapsed':round(r_band['L2_F'],4),
                    'L2_F_band_without_a5_collapsed':round(r_noa5['L2_F'],4),
                    'cost_band_nats':round(cost_band,4),'cost_band_without_a5_nats':round(cost_noa5,4),
                    'a5_implied_share_nats':round(cost_band-cost_noa5,4),
                    'L2_C_baseline':round(r_base['L2_C'],4),'L2_C_band_collapsed':round(r_band['L2_C'],4),
                    'frontier_row_spread_by_layer':{str(k):v for k,v in spreads.items()},
                    'spearman_frontier_spread_vs_census_gain_cv':rho,
                    'parameters_saved_per_collapsed_layer':int(saved_per_layer),
                    'parameters_saved_whole_band':int(saved_per_layer*len(BAND))},
    ...
    print(f"(b) whole-band cost {cost_band:+.4f} <= {BARS['band_free']} (LOWER L2 IS BETTER): {'HELD' if pb else 'FAILED'}")
    print(f"(c) band-without-a5 cost {cost_noa5:+.4f} <= {BARS['no_a5_free']}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) spread vs census rho {rho:.3f} >= {BARS['census_rho']}: {'HELD' if pd else 'FAILED'}")
