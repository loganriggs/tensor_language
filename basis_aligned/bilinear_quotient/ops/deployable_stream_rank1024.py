# DOES THE STREAM INPUT KEEP PAYING ABOVE RANK 512?  -- the deployable map at rank 1024.
#
# §1876 established the deployable fix: a covered-fit map from the length-1 residual stream at rank 512
# reaches +0.17427 / +0.21358 / +0.21419 against §1868's uncovered-token ceiling, versus the deployed
# rank-64 embedding map's +0.78075 / +0.86225 / +0.83997 -- and the INPUT half of that is free, worth
# +0.10161 / +0.11650 / +0.11069 all-position nats at identical map storage.
#
# §1869 measured strongly diminishing returns in map rank ON THE EMBEDDING (per-doubling recovery 0.095,
# 0.078, 0.047). §1875 then showed rank is worth 2.4x more on the stream than on the embedding, so
# §1869's decay curve does not transfer and rank 512 may not be where the stream input saturates.
#
# Map rank 1024 costs 84.934M against 512's 42.467M, so the question is decision-relevant: another
# 42.5M reals is 18% of the deployed build's 230.087M.
#
# One constant changed from §1876's run.
#
# ROLES. All three, deployed 5,419-type covered set. DISCOVERY ONLY. Rung 3: §1876's open question.
#
...
# ANCHORS state their rank, per §1875's pred_d failure.
#   pred_a RANK KEEPS PAYING: the covered-fit stream map at rank 1024 comes in below 0.14 nats on all
#          three roles -- i.e. it recovers at least a fifth of the distance from §1876's PUBLISHED
#          rank-512 +0.17427 down to zero. If TRUE the stream input has not saturated at 512 and the
#          fallback's remaining loss is still a rank question, so the recommendation should quote the
#          largest rank the budget allows rather than 512. If FALSE rank 512 is where the stream input
#          saturates, the residual is not a capacity matter, and §1876's 0.174 is close to the practical
#          floor for a linear per-token map.
#   pred_b AND IT APPROACHES §1875's ORACLE: within 0.03 nats of the PUBLISHED rank-512 stream ORACLE of
#          +0.11388 / +0.14094 / +0.14196 on all three roles. §1876 sat 0.060 above it at rank 512; if
#          more rank closes that gap, the generalisation penalty is itself rank-limited. If FALSE the
#          covered-fit build cannot reach the oracle however much rank it is given, and the ~0.06 is a
#          genuine transfer cost.
#   pred_c AND THE COVERED ARM IS UNTOUCHED: covered CE identical to the MAP arm within 1e-6. Eighth
#          KNOWN-ANSWER check; the previous seven each returned exactly 0.00e+00.
#   pred_d CONTROLS: coverage is exactly 5,419; the covered arm attains its ceiling (a seventeenth
#          confirmation); every uncovered scored token has a ceiling row; the MAP arm -- an EMBEDDING
#          map at rank 1024, which no section has published -- is reported, not anchored.
...
OUT = PT + 'ops/deployable_stream_rank1024_results.json'
...
# §1876's PUBLISHED covered-fit STREAM map at rank 512 -- the point this run changes rank from
S1876_STREAMCOV_R512 = {'skip7000': 0.17427, 'skip11000': 0.21358, 'skip1200': 0.21419}
...
def main():
    ...
    print(f'DEPLOYABLE STREAM R1024 | 5419 types, covered-fit stream map at rank 1024 | '
    ...
    pa = all(fbloss['STREAMCOV'][e] < 0.14 for e in roles2)
    pb = all(abs(fbloss['STREAMCOV'][e] - S1875_ORACLE_STREAM_R512[e]) <= 0.03
             for e in roles2)
    pc = covspread <= 1e-6
    pd = (NFULL == NCOV and attain
    ...
    print(f'\n  RANK keeps paying above 512 (<0.14) -> {pa}  ' + '  '.join(
        f'{e} {fbloss["STREAMCOV"][e]:+.5f}' for e in roles2),
        flush=True)
    print(f'  and it APPROACHES §1875 oracle (within 0.03) -> {pb}  ' + '  '.join(
    ...
    json.dump({'run': 'deployable_stream_rank1024', 'n_covered': NFULL, 'arms': list(ARMS2),
               ...
               'predictions': {'pred_a_rank_keeps_paying': bool(pa),
                               'pred_b_approaches_oracle': bool(pb),
