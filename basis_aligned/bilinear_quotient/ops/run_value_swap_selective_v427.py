#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_other_heads_carry_little pred_c_readers_are_most_of_their_blocks pred_d_all_heads_bounded pred_e_selective_on_panel
"""Selectivity of the value copy among the readers' blocks' heads (v427). v416: swapping the three readers' values at the noun closes 0.37 of the natural
margin gap. Are they the heads that carry number out of blocks 9 / 12 / 15, or does the rest of each block copy it too? Same replace-edit, values only,
at the noun: the three readers (R), all 27 heads of blocks 9 / 12 / 15 (ALL), and the 24 non-reader heads (OTHER).
PREDICTIONS (scored as written; failures preserved; priors from v380 (the readers write 0.73 of the final number signal) / v416)
    pred_a_baseline_replays                 the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_other_heads_carry_little         OTHER closes <= 0.10 of the gap on text
    pred_c_readers_are_most_of_their_blocks R >= 0.75 x ALL on text
    pred_d_all_heads_bounded                ALL closes <= 0.55 on text (the noun-site value copy of three blocks cannot exceed the two-site readers' read by much)
    pred_e_selective_on_panel               OTHER <= 0.10 on the panel too
...
OUT = ROOT / "circuits/followups/value_swap_selective_v427_result.json"
CANDIDATE_ID = "both_ends.value_swap_selective_v427"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1))
REPLAY_TOL, OTHER_MAX, READER_SHARE_MIN, ALL_MAX = 1e-3, 0.10, 0.75, 0.55
FORWARDS_MAX = 29
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_other_heads_carry_little": "<= 0.10", "pred_c_readers_are_most_of_their_blocks": ">= 0.75 x ALL", "pred_d_all_heads_bounded": "<= 0.55", "pred_e_selective_on_panel": "<= 0.10"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "other_max": OTHER_MAX, "reader_share_min": READER_SHARE_MIN, "all_max": ALL_MAX}}
    ...
    RD = dict(READERS)
    MODES = {"R": {l: [RD[l]] for l in RD}, "ALL": {l: list(range(N_HEAD)) for l in RD}, "OTHER": {l: [hh for hh in range(N_HEAD) if hh != RD[l]] for l in RD}}; forwards = 0
    def logits_margin(x):
    ...
    def study(items, batch):
        ...
        res = {m: [] for m in ("native", "R", "ALL", "OTHER")}
        ...
        out = {m: closed(res[m]) for m in ("R", "ALL", "OTHER")}
        return {"pairs": len(items), "gap_mean": float(gap.mean()), "closed": out, "reader_share_of_all": out["R"] / out["ALL"] if abs(out["ALL"]) > 1e-9 else float("nan"), "native_margins": nat.tolist()}, nat
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_other_heads_carry_little": abs(text["closed"]["OTHER"]) <= OTHER_MAX, "pred_c_readers_are_most_of_their_blocks": text["reader_share_of_all"] >= READER_SHARE_MIN,
                   "pred_d_all_heads_bounded": text["closed"]["ALL"] <= ALL_MAX, "pred_e_selective_on_panel": abs(panel["closed"]["OTHER"]) <= OTHER_MAX}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "value_swap_selective_result_v427", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
