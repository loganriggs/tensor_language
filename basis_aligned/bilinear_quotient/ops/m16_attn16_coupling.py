"""IS m16's ANTI-RETRIEVAL COUPLED TO attn16 (rung 72): the interaction test behind S2159's word "fights".

CONVENTION (S2135): damage on the skip-1416 config (both attentions real); bases excluded; per-position CE
differences; on ind positions a NEGATIVE sum for zeroing m16 = m16 was hurting there (S2159).

S2159 found deleting m16 HELPS ind positions. Does that harm require attn16's output (a coupled pair), or does
m16 fight retrieval generally? Arms: base | m16 zeroed | attn16 zeroed | both zeroed.
A = S_ind[cev(zm16) - cev(base)] (m16's ind effect WITH attn16); B = S_ind[cev(both) - cev(z16)] (WITHOUT).
NULL: B ~ A (m16's ind-harm is attn16-independent).

REGISTERED PREDICTIONS (arm-named; damage convention):
  (a) THE S2159 EFFECT REPLICATES ON THIS BASE: A <= -100 summed nats.
  (b) COUPLING: B - A >= 0.5 x |A| (the ind benefit of deleting m16 shrinks by at least half when attn16 is
      already gone).
...
Writes m16_attn16_coupling_results.json. Self-reviewed. RUNG-71 HEADER FOLLOWS.
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; m16 x attn16 coupling on skip-1416 (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'m16_attn16_coupling_results.json'
...
def main():
    ...
    _hz3=[]
    if SEL.get('zerom16'):
        _hz3.append(m.transformer.h[16].mlp.register_forward_hook(
...
if __name__=='__main__':
    ...
    arms={}
    for tag,za,zm in (('base',(),False),('zm16',(),True),('z16',(16,),False),('both',(16,),True)):
        SEL['zeroattn']=za; SEL['zerom16']=zm
        print(f'ARM {tag}: attn-zero {za}, m16-zero {zm}',flush=True)
        arms[tag]={'r':main(),'cev':SEL['cev']}
    clsflat=SEL['clsflat']
    import statistics as stt
    for tag in ('zm16','z16','both'):
    ...
    IND=CLS.index('ind')
    mask=clsflat==IND
    A=float((arms['zm16']['cev']-arms['base']['cev'])[mask].sum())
    B=float((arms['both']['cev']-arms['z16']['cev'])[mask].sum())
    pa=A<=-100
    pb=(B-A)>=0.5*abs(A)
    pc=stt.median(rp)<=0.005
    res={'arms':{t:{'L2_F':arms[t]['r']['L2_F'],'L2_C':arms[t]['r']['L2_C']} for t in arms},
         'A_m16_ind_effect_with_attn16':round(A,1),'B_m16_ind_effect_without_attn16':round(B,1),
         'convention':'damage on skip-1416; bases excluded; negative A = m16 was hurting ind positions',
         'pred_a_effect_replicates':bool(pa),'pred_b_coupled':bool(pb),'pred_c_reproduces_s2156':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"m16 ind effect: WITH attn16 A={A:+.1f} | WITHOUT B={B:+.1f}")
    print(f"(a) A {A:+.1f} <= -100: {'HELD' if pa else 'FAILED'}")
    print(f"(b) B-A {B-A:+.1f} >= 0.5|A| {0.5*abs(A):+.1f}: {'HELD' if pb else 'FAILED'}")
