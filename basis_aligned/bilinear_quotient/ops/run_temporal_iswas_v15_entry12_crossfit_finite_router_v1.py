#!/usr/bin/env python3
"""Held-parity A1/A2/off router for the sufficient entry12 construction union."""

# BQGATE: EXPERIMENT pred_a_authority_replay_scope_geometry_finiteness_and_price pred_b_router_identifies_held_branches pred_c_routed_union_preserves_both_targets pred_d_routed_union_is_control_selective pred_e_router_retains_ungated_union_effect
...
PRIOR = ROOT / "circuits/prior_art/temporal_iswas_v15_entry12_crossfit_finite_router_v1.json"
...
OUT = ROOT / "circuits/followups/temporal_iswas_v15_entry12_crossfit_finite_router_v1_result.json"
CANDIDATE_ID = "temporal_auxiliary.iswas_v15_entry12_crossfit_finite_router_v1"
EXPECTED = {
    "prior": "ccc5353995f7befb8f40d9af4f765a1eb58de99613d372c4d8f687aa38c042cd",
    ...
    "router": "638da03d39238abe9f9373b912b7f446e171be85ee33dcad8fc5762f09109bf9",
...
FILES = {
    "prior": PRIOR, "basis_result": BASIS_RESULT, "oracles": ORACLES,
...
def main():
    ...
    prior, basis_result = json.loads(PRIOR.read_text()), json.loads(BASIS_RESULT.read_text())
    ...
    for held in (0, 1):
        ...
        train_features = router.semantic_features(
            backend.torch, train_off, train_context["base_batch"].semantic_positions)
        ...
        held_features = router.semantic_features(
            backend.torch, off_entry, held_context["base_batch"].semantic_positions)
    ...
    A = (authority and disjoint and native_closure <= 1e-4 and parent_replay <= 1e-4
    ...
    predictions = {
        ...
        "pred_b_router_identifies_held_branches": B,
    ...
    gold_pass = all(reports[str(parity)]["gold_router"][panel]["metrics"]["signed_projection"] >= .75
                    for parity in (0, 1) for panel in ("A1", "A2"))
    terminal = ("invalid" if not A else "finite_router_candidate" if all((B, C, D, E))
    ...
    result = {"schema": "temporal_iswas_v15_entry12_crossfit_finite_router_result_v1",
