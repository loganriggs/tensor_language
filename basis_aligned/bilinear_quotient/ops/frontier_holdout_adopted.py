"""DOES THE **ADOPTED** SS2912 OPTIMUM TRANSPORT, AND DID THE 95-CELL SEARCH BUY ANYTHING REAL? (Claude, LANE 1, RED TEAM)

SS2912 adopted **tail .30 / CP .80 / motif 1.25 -> L2_F +2.2999** on a registered rule whose predicates all held, and the section
recorded its own qualification in advance: **all 95 cells of that three-axis search were scored on the same fixed 120 documents**
(`FR`). Its sibling `frontier_scale_holdout` measures the transport of the SS2904 **two-parameter** config, which is a LOWER BOUND on
the bias here -- two single-axis rungs are far less selected than a 95-cell grid.

This rung measures the thing that actually matters for the headline number: **baseline, SS2904 TC, and the SS2912 adopted config, each on
the selection window AND on a held-out window**, in one run. Two quantities come out that nothing else can produce:

  selection bias   gain(FR) - gain(FR2)   per config -- how much each number flatters itself
  EXTRA            gain(G) - gain(TC)     on each window -- the 95-cell search bought **0.0523 nats on FR**;
                                          `pred_f` asks whether ANY of it exists off the window that chose it

If EXTRA is ~0.05 on FR and ~0.00 on FR2, the grid search was fitting documents and the honest frontier is SS2904's two-parameter
object. **That is a real possible outcome of this rung and it costs me my own most recent adoption**, which is why it is registered
before the numbers are seen. Re-measuring TC also cross-checks `frontier_scale_holdout` on an independently-run pipeline.

SIGN CONVENTION (SS2135): frontier L2 is CE ADDED ABOVE THE REAL MODEL, so **LOWER IS BETTER** (SS312: +2.6735 beating +2.84/+2.93).
A gain is `L2(baseline) - L2(arm)`, **POSITIVE = BETTER**. SS2128/SS2129/SS2133/SS2134 RETRACTED; SS2125 STANDS.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_the_adopted_gain_reproduces_on_the_selection_window
#                     pred_c_at_least_three_quarters_of_the_gain_transports
#                     pred_d_the_holdout_window_is_a_comparable_comparator
#                     pred_e_the_gain_does_not_change_sign_off_the_selection_window
#                     pred_f_the_extra_search_beyond_TC_survives_off_the_selection_window

Preregistration: polynomial_causal/FRONTIER_HOLDOUT_ADOPTED_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; ONE pipeline run: SS2904 TC and the SS2912 adopted config, each on the selection window AND a held-out one")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_holdout_adopted_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_HOLDOUT_ADOPTED_PREREGISTRATION.md'
PREREG_SHA = "2419c8e38499b8c05a360b7b72201d006cb67c4372dda90956579568a7ff2b1e"
...
BARS = {"reproduce": 0.05, "gain_reproduces": 0.25, "transport_frac": 0.75, "comparable": 0.50}
...
if __name__=='__main__':
    ...
    # NO grid and NO selection: the ONE adopted SS2904 configuration, transported unchanged to a window it
    # never saw. Two arms, so the extra cost over a plain rerun is two forward passes.
    ARMS[:] = [('baseline', {}),
               ('TC', {'tail_scale': 0.25, 'cp_scale': 0.5}),                              # SS2904, 2 params, ~2 rungs of search
               ('G',  {'tail_scale': 0.30, 'cp_scale': 0.80, 'motif_scale': 1.25})]        # SS2912, 3 params, 95 cells of search
    print('ONE pipeline run, 3 arms x 2 fresh windows against the same fitted stack',flush=True)
    out=main()

    bF, bF2 = out['baseline']['L2_F'], out['baseline']['L2_F2']
    gF  = {k: round(bF  - out[k]['L2_F'],  4) for k in ('TC','G')}    # POSITIVE = the scalars help
    gF2 = {k: round(bF2 - out[k]['L2_F2'], 4) for k in ('TC','G')}
    bias = {k: round(gF[k] - gF2[k], 4) for k in ('TC','G')}          # POSITIVE = the selection window flattered it
    gain_F, gain_F2 = gF['G'], gF2['G']               # the predicates are about the ADOPTED SS2912 config
    window  = round(bF2 - bF, 4)
    # the extra search bought 0.0523 nats ON the selection window; does any of it survive OFF it?
    extra_F  = round(gF['G']  - gF['TC'],  4)
    extra_F2 = round(gF2['G'] - gF2['TC'], 4)

    pa = abs(bF-2.6735) <= BARS['reproduce']
    pb = gain_F >= BARS['gain_reproduces']            # pins the sign of pred_c's reference quantity
    pc = pb and (gain_F2 >= BARS['transport_frac'] * gain_F)
    pd = abs(window) <= BARS['comparable']
    pe = gain_F2 > 0.0
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_adopted_gain_reproduces_on_the_selection_window':bool(pb),
           'pred_c_at_least_three_quarters_of_the_gain_transports':bool(pc),
           'pred_d_the_holdout_window_is_a_comparable_comparator':bool(pd),
           'pred_e_the_gain_does_not_change_sign_off_the_selection_window':bool(pe),
           'pred_f_the_extra_search_beyond_TC_survives_off_the_selection_window':bool(extra_F2 > 0.0)}
    nulls={'b_null_the_adopted_gain_does_not_reproduce':bool(gain_F < BARS['gain_reproduces']),
           'c_null_the_gain_is_an_artefact_of_the_selection_window':bool(gain_F2 <= 0.5*gain_F),
           'e_null_the_correction_is_window_specific':bool(gain_F2 <= 0.0),
           'f_null_the_95_cell_search_bought_nothing_transportable':bool(extra_F2 <= 0.0)}
    res={'rung':'frontier_holdout_adopted','preds':preds,'nulls':nulls,'bars':BARS,
         'arms':out,'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(bF,4),'L2_F2_baseline':round(bF2,4),
                    'L2_F_TC':out['TC']['L2_F'],'L2_F2_TC':out['TC']['L2_F2'],
                    'L2_F_G':out['G']['L2_F'],'L2_F2_G':out['G']['L2_F2'],
                    'gain_selection_window':gF,'gain_holdout_window':gF2,
                    'selection_bias_nats':bias,
                    'extra_from_search_selection_window':extra_F,
                    'extra_from_search_holdout_window':extra_F2,
                    'transported_fraction':(round(gain_F2/gain_F,4) if gain_F>0 else None),
                    'window_offset_baseline':window,
                    'S2904_TC_on_FR_for_reference':round(tc,4),
                    'S2904_adopted_L2_F':2.3522,
                    'implied_holdout_L2':round(bF2 - gain_F2, 4)},
    ...
    print(f"(b) gain on the SELECTION window {gain_F:+.4f} >= 0.25: {'HELD' if pb else 'FAILED'}")
    print(f"(c) gain on the HELD-OUT window {gain_F2:+.4f} >= 0.75 x {gain_F:+.4f} = {0.75*gain_F:+.4f}: {'HELD' if pc else 'FAILED'}")
    print(f"(d) baselines comparable: holdout {bF2:.4f} vs selection {bF:.4f}, offset {window:+.4f}: {'HELD' if pd else 'FAILED'}")
    print(f"(e) gain keeps its sign off the selection window: {'HELD' if pe else 'FAILED'}")
    print(f"(f) the extra search survives off the selection window: {extra_F2:+.4f} > 0: {'HELD' if extra_F2>0 else 'FAILED'}")
    print(f"    SELECTION BIAS   TC {bias['TC']:+.4f} | G {bias['G']:+.4f} nats   (positive = the number flatters itself)")
    print(f"    EXTRA FROM 95-CELL SEARCH   on FR {extra_F:+.4f} | on the HELD-OUT window {extra_F2:+.4f}")
    print(f'wrote {OUT} ({res["price"]["gpu_seconds"]:.0f}s, 2 arms x 2 windows in one run)')
