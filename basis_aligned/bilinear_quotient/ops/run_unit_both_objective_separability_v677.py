...
"""v677: the last gate on the both-objectives survivors -- do they fuse with each other?

WHAT IS ALREADY ESTABLISHED ABOUT THESE SIX. Three cells cleared the held-out control under BOTH objectives in v675
(adj_adv_feel, countability_few_little, definiteness_anaphor_needed) in a run whose known-good arm reproduced eight
recorded values exactly. Three more cleared both objectives in v673 (reciprocal_lenmatched, durativity_until_by,
both_either) and are ALREADY separable from v665. What the three new ones have never had is a separability pass:
nobody has checked whether they fuse with a sibling. Four rows and both controls say a cell carries its variable and
spares unrelated behaviours; they say nothing about whether it is the SAME direction as another cell.
WHY THAT GATE IS NOT A FORMALITY, MEASURED NOT ASSUMED. v665 ran thirteen cells that all looked fine on their
existing receipts: eleven separated and two did not, and one of the two fused across a stem-name boundary I had
assumed made it safe (gender_object_him_her with the counted possessive_gender, mutual 0.095 and 0.083). So roughly
one in six cells that pass the earlier gates still fails this one, and the failures are not predictable from the
names.
WHY THE THREE ALREADY-SEPARABLE CELLS ARE IN THE FAMILY. They are not being re-litigated; they are here because
separability is a property of a SET, not of a cell. The three v673 survivors separated in v665's 16-member family,
which did not contain adj_adv_feel, countability_few_little or definiteness_anaphor_needed. Putting all six in one
family asks the question that matters for a proposal: do the cells that pass the FULL standard fuse with each other?
A pass in an old family is not a pass in this one.
THE PAIR TO WATCH, NAMED IN ADVANCE. countability_few_little (" few"/" little") and definiteness_anaphor_needed
(" one"/" it") both read out on a quantity-or-anaphor axis over a mass/count noun, and both sit downstream of a
determiner. If anything in this family fuses I expect it to be that pair. adj_adv_feel is the outlier -- an
adjective/adverb category distinction -- and I expect it to be the cleanest.
HOW IT READS. a TRUE: all six pass the full standard and become proposable together, which would be the first batch
this lane has taken end to end through four rows, both objectives AND separability. a FALSE: the SET that fuses is
the result, and any cell that fuses is not proposed -- the same commitment I made in v673 and honoured by
withdrawing two cells.
NOTHING IS COUNTED BY THIS FILE.

REGISTERED BEFORE THE RUN (bars in BARS; each coded predicate is the sentence here):
  pred_a_new_separable at least k_new = 5 of the six candidates are separable at cross_max = 0.05 while sitting in
                       one family with each other. Worked example: if five separate and definiteness_anaphor_needed
                       has a largest sibling leak of 0.07, the count is 5, 5 >= 5 is true, pred_a is TRUE -- and I
                       still do not propose that cell. The bar measures the batch; the per-cell action is separate.
                       At four separable it is FALSE.                                                 prior 65%
  pred_b_counted_kept  at least k_kept = 2 of the three counted members stay separable in the same run, so a null on
                       the candidates cannot be blamed on the family. Worked example: possessive_gender and
                       correlative_or_and separable, number_reflexive_myself_ourselves not, kept = 2, 2 >= 2 is
                       true, pred_b is TRUE.                                                          prior 85%
  pred_c_row4_kept     at least k_d = 2 members of EVAL keep a family-arm C upper bound at or below c_ub_max = 0.01.
                       Worked example: seven of the nine evaluated members at or under 0.01 gives 7, 7 >= 2 is true,
                       pred_c is TRUE.                                                                prior 85%
  pred_d_instrument    every EVAL member reproduces its parent battery extraction within instr_tol = 0.03. Worked
                       example: adj_adv_feel scored 0.952 in its parent receipt, so an own arm of 0.968 is a gap of
                       0.016 and holds; one member off by 0.04 makes pred_d FALSE for the whole run.   prior 80%
SCOPE. Six both-objectives survivors, three counted controls, one family. No counting.
Smoke: V677_SMOKE=<out.json> (CPU) -- proves the code path runs; its numbers are not measurements.
"""
...
import run_unit_possessive_newperson_v659 as v659
import run_unit_reciprocal_separability_v657 as v657

ROOT = Path(__file__).resolve().parent.parent
OUT = ROOT / "circuits/followups/unit_both_objective_separability_v677_result.json"
...
V659R = ROOT / "circuits/followups/unit_possessive_newperson_v659_result.json"
...
FAMILIES = {
    "both_objective_survivors": ['possessive_gender', 'correlative_or_and', 'number_reflexive_myself_ourselves', 'reciprocal_lenmatched', 'durativity_until_by', 'both_either', 'adj_adv_feel', 'countability_few_little', 'definiteness_anaphor_needed'],
}
COUNTED_LIST = ['possessive_gender', 'correlative_or_and', 'number_reflexive_myself_ourselves']
COUNTED = tuple(COUNTED_LIST)
EVAL = tuple(COUNTED_LIST) + tuple(['reciprocal_lenmatched', 'durativity_until_by', 'both_either', 'adj_adv_feel', 'countability_few_little', 'definiteness_anaphor_needed'])
NAMES = {**v197.NAMES, **v201.NAMES, **v204.NAMES, **v205.NAMES, **v207.NAMES, **v209.NAMES, **v211.NAMES, **v213.NAMES, **v215.NAMES, **v217.NAMES, **v219.NAMES, **v221.NAMES, **v223.NAMES, **v225.NAMES, **v227.NAMES, **v229.NAMES, **v231.NAMES, **v233.NAMES, **v235.NAMES, **v237.NAMES, **v239.NAMES, **v241.NAMES, **v243.NAMES, **v245.NAMES, **v247.NAMES, **v249.NAMES, **v251.NAMES, **v253.NAMES, **v255.NAMES, **v257.NAMES, **v311.NAMES, **v315.NAMES, **v319.NAMES, **v323.NAMES, **v327.NAMES, **v349.NAMES, **v293.NAMES, **v353.NAMES, **v263.NAMES, **v265.NAMES, **v357.NAMES, **v339.NAMES, **v343.NAMES, **v361.NAMES, **v365.NAMES, **v575.NAMES, **v581.NAMES, **v597.NAMES, **v599.NAMES, **v601.NAMES, **v633.NAMES, **v659.NAMES, **v657.NAMES}


def _parent_cdas(n):
    # the parent battery receipt that produced this member's units (each behaviour name is unique across batteries)
    for path in (V651R, V655R, V639R, V659R, V633R, V597R, V599R, V601R, V581R, V575R, V365, V361, V343, V339, V357, V263, V265, V353, V293, V349, V327, V323, V319, V315, V311, V257, V255, V253, V251, V249, V247, V245, V243, V241, V239, V237, V235, V233, V231, V229, V227, V225, V223, V221, V219, V217, V215, V213, V211, V209, V207, V205, V204, V201, V197):
...
BARS = {"cross_max": 0.05, "keep": 0.8, "k_kept": 2, "ext_floor": 0.5, "c_ub_max": 0.01, "k_d": 2, "k_new": 5, "instr_tol": 0.03}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 20000, 640000


def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_both_objective_separability_v677", "members": n, "families": list(FAMILIES),
...
def main() -> None:
    ...
    smoke = os.environ.get("V677_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V677_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    result = {"predictions": predictions, "schema": "unit_both_objective_separability_v677", "candidate_id": "corpus.unit_both_objective_separability_v677",
