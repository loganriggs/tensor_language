"""REGISTERED PARETO CLAIM: THE EXACT-FRONT CONFIG (rung 137).

CONVENTION (S2135): frontier L2 = CE ADDED ABOVE THE REAL MODEL on the S312 fresh windows; LOWER IS BETTER.
Rung 136 printed DESCRIPTIVELY L2_F fresh +1.6599 for the exact-front (K=4608) variant of the rung-127 build.
Per process (the rung-123 precedent) a claim needs registration before the scoring run: this rung reruns the
identical deterministic build with the claim bars as predictions. If held, the program's Pareto set gains a
SECOND quotable point - it does NOT replace the 47.8M-value frontier line (S2221).

REGISTERED PREDICTIONS (anchors: rung-136 receipt L2_F +1.6599, census +1.7202):
  (a) L2_F fresh <= 1.70.
  (b) REPRODUCTION: |census - 1.7202| <= 0.015.
  (c) CERTIFICATE STAGNATION: valid <= 5 of 62 (FAILURE here = news).
NULL: the 136 print was noise (L2_F >= 1.80). PRICE: front 63,700,992 values (vs 47.8M at the S2221 point).
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: registered exact-front Pareto claim')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_claim4608_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=4608; SEL['tail_traj']=True
    print('ARM: identical rung-136 build; Pareto claim bars registered',flush=True)
    main()
    ...
    print(f"(a) L2F {L2F:+.4f} <= 1.70: {'HELD' if pa else 'FAILED'}")
    print(f"(b) |census {agg:+.4f} - 1.7202| <= 0.015: {'HELD' if pb else 'FAILED'}")
    print(f"(c) valid {nv} <= 5: {'HELD' if pc else 'FAILED'}")
