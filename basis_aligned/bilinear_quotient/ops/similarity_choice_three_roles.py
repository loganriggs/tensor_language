# THREE-ROLE CONFIRMATION OF THE OUTPUT-SIMILARITY FALLBACK
#
# §1780: choosing the uncovered token's stand-in by the model's own LENGTH-1 NEXT-TOKEN DISTRIBUTION
# beat input-embedding cosine at every rank -- 0.037 to 0.052 nats -- and the two similarities pick
# the same neighbour for only 7.53% of the 44,838 uncovered ids. It was measured on skip11000 alone,
# while the embedding map it replaces was confirmed on three roles at §1778. A better map with a
# thinner confirmation should not stand as the design point.
#
# ROLES. skip7000, skip11000, skip1200; all-position and covered CE. skip11000 reproduces §1780 as a
# control. DISCOVERY plus SECOND-CLASS CONFIRMATION.
#
# Registered predictions, TWO-SIDED per LESSONS 31, absolute nats with margins per LESSON 40, read
# back per LESSON 39:
#   pred_a IT REPLICATES: output similarity beats embedding similarity at every rank on ALL THREE
#          roles. If FALSE the advantage is role-specific and §1780 should not be the design point.
#   pred_b THE MAGNITUDE TRANSFERS: each role's gain is within 0.02 nats of skip11000's at every
#          rank. Scored independently of pred_a, since the sign can replicate while the size does not
#          -- which is what §1774 found for the frontier's cheap end.
...
OUT = PT + 'ops/similarity_choice_three_roles_results.json'
...
def main():
    ...
    g = {e: [out[kk][f'embedding_{e}']['all_ce'] - out[kk][f'output_{e}']['all_ce'] for kk in keys]
         for e in roles}
    pb = all(abs(g[e][i] - g['skip11000'][i]) <= 0.02 for e in roles for i in range(len(keys)))
    ...
    print(f'\n  output-distribution similarity beats input-embedding similarity everywhere -> {pa}',
          flush=True)
    print(f'  the gain transfers across roles: ' + ' | '.join(
        f'{e} {[round(x, 4) for x in g[e]]}' for e in roles) + f' -> within 0.02 {pb}', flush=True)
    ...
    r2 = {'config': {'ranks': [str(x) for x in TABLE_RANKS],
          ...
          'predictions': {'pred_a_output_similarity_wins': bool(pa),
                          'pred_b_gain_transfers_within_0p02': bool(pb),
