"""DOES a1'S ABSORBER BUY ANYTHING? The one candidate S2088 left standing.

S2088 closed BACKLOG rung 2: at the two largest attention injectors the injected
error is not linearly readable (held-out R^2 -0.0645 at a5, +0.0657 at a6,
shuffled control max 0.0148). It left exactly one candidate -- `a1`, injection
+0.8384, held-out R^2 **0.5973** -- and named the question it could not answer:
"whether recovering 60% of its residual variance converts into CE or into
downstream stream fidelity is unmeasured, and S309 dissociates the two."

S309 is the reason this needs measuring rather than assuming: it found stream
fidelity and CE dissociate completely (~100% relative stream error at +1.4 nats).
A 60% variance recovery is a fact about a1's output and predicts NEITHER.

METHOD, and its one honest caveat. The rank-32 read is fitted exactly as in
S2088 (first half of rows) and then INSTALLED at a1's output at inference on the
held-out half. Nothing downstream is refitted. That is deliberate -- it is the
cheap deployment, the one an absorber would actually get without re-running the
whole sequential merge -- but it means every downstream rung is now operating
slightly off the context it was fitted under, which is precisely the mismatch
S307 documented. **If the absorber helps here it helps in the hard case; if it
hurts, S307 predicts why and the verdict is "needs the full merge", not "a1 is
worthless".** Both readings are stated in advance so neither is a post-hoc rescue.

REGISTERED PREDICTIONS (all on held-out rows):
  (a) THE LOCAL FIX LANDS: rel-MSE at block 2's input -- the site immediately
      downstream of a1 -- falls by >= 30%. a1's residual is 60% readable, so the
      stream it feeds should improve substantially. If FALSE the recovery does
      not even survive one block and the candidate closes on the spot.
  (b) AND IT SURVIVES TO THE PEAK: rel-MSE at block 6's input, S2086's maximum,
      is lower with the absorber than without. A local repair that is undone
      within four blocks buys nothing a deep reader can use.
  (c) CE DOES NOT WORSEN: mean CE with the absorber is <= CE without, on the same
      held-out rows. Registered on the optimistic side on purpose -- with no
      downstream refit, S307 says this is where a real absorber most plausibly
      regresses, and putting the risk here rather than on (a) keeps the
      dissociation S309 measured from being quietly assumed away.

Writes a1_absorber_effect_results.json.
"""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    if not os.path.exists(_bq+'absorber_recoverability_results.json'):
        print('DRYRUN FAIL: S2088 recoverability absent'); raise SystemExit(1)
    _p=json.load(open(_bq+'absorber_recoverability_results.json'))['targets']
    if _p['a1']['heldout_r2_rank32'] < 0.5:
        print('DRYRUN FAIL: S2088 does not leave a1 as a candidate'); raise SystemExit(1)
    print(f"DRYRUN OK: S2088 present (a1 held-out R2 "
          f"{_p['a1']['heldout_r2_rank32']}, a5 {_p['a5']['heldout_r2_rank32']}); "
          f"installing a1's rank-32 absorber, no downstream refit")
    raise SystemExit(0)
...
OUT=PT+'a1_absorber_effect_results.json'
...
def main():
    ...
    def cap_a1(active):
        ...
        hs=install(active)
        def pre(mo,args):
            hold['x']=(args[0].detach().float(),args[2].detach().float())
        def af(mo,args,outp):
            x,x0=hold['x']
            lam=m.transformer.h[LI].lambdas
            inp.append(F.rms_norm(lam[0].float()*x+lam[1].float()*x0,(D,)).reshape(-1,D))
            y=(outp[0] if isinstance(outp,tuple) else outp).detach().float()
            out.append(y.reshape(-1,D))
        hs.append(m.transformer.h[LI].register_forward_pre_hook(pre))
        hs.append(m.transformer.h[LI].attn.register_forward_hook(af))
        for i in range(R0,R1,8):
            bb=FW[i:i+4,:257].to(DEV)
            cur['idx']=bb[:,:-1].contiguous(); cur['mode']='oracle'
            m(cur['idx'], bb[:,1:].contiguous())
        for h in hs: h.remove()
        return torch.cat(inp), torch.cat(out)
    rI,rO=cap_a1([]); eI,eO=cap_a1(cfgE)
    Rres=rO-eO
    n=Rres.shape[0]; half=n//2
    Xf=eI[:half]; Rf=Rres[:half]
    U,S1,Vh=torch.linalg.svd(Xf-Xf.mean(0,keepdim=True),full_matrices=False)
    P=Vh[:RANK].T.contiguous()
    Zf=Xf@P; lam=1e-2*len(Zf)
    W=torch.linalg.solve(Zf.T@Zf+lam*torch.eye(RANK,device=DEV),Zf.T@Rf)
    print(f'  fitted a1 absorber: rank {RANK} on {half} of {n} positions',flush=True)

    ABS={'on':False}
    def absorb_hook(mo,args,outp):
        if not ABS['on']:
            return None
        x,x0=ABS['hold']
        lam2=m.transformer.h[LI].lambdas
        z=F.rms_norm(lam2[0].float()*x+lam2[1].float()*x0,(D,))
        corr=((z.reshape(-1,D)@P)@W).view_as(x)
        if isinstance(outp,tuple):
            return (outp[0]+corr.to(outp[0].dtype), outp[1])
        return outp+corr.to(outp.dtype)
    def pre_hold(mo,args):
        ABS['hold']=(args[0].detach().float(),args[2].detach().float())

    ROWS=list(range(R0,R1,8))
    EVAL=ROWS[len(ROWS)//2:]                    # held-out half, matching the fit split
    def run(active,use_abs):
        ABS['on']=use_abs
        caps={li:[] for li in PROBES}; ces=[]
        hs=install(active)
        hs.append(m.transformer.h[LI].register_forward_pre_hook(pre_hold))
        hs.append(m.transformer.h[LI].attn.register_forward_hook(absorb_hook))
        for li in PROBES:
            def mk(li=li):
                def h(mo,args): caps[li].append(args[0].detach().float().reshape(-1,D))
                return h
        ...
        for h in hs: h.remove()
        ABS['on']=False
        return {li:torch.cat(v) for li,v in caps.items()}, float(torch.cat(ces).mean())
    realC,realCE=run([],False)
    offC,offCE=run(cfgE,False)
    onC,onCE=run(cfgE,True)
    def rel(a_,b_):
        return float(((a_-b_)**2).mean(0).sum())/max(float(b_.var(0).sum()),1e-12)
    prof={}
    for li in PROBES:
        off=rel(offC[li],realC[li]); on=rel(onC[li],realC[li])
        prof[str(li)]={'off':round(off,4),'on':round(on,4),
                       'delta_pct':round(100*(on-off)/max(off,1e-12),2)}
        print(f'  block {li:2d} input: off {off:.4f} -> on {on:.4f} '
              f'({prof[str(li)]["delta_pct"]:+.2f}%)',flush=True)
    d2=prof['2']['delta_pct']; d6=prof['6']['delta_pct']
    pa=d2<=-30.0
    pb=prof['6']['on']<prof['6']['off']
    pc=onCE<=offCE
    out={'rank':RANK,'probes':prof,'ce_off':round(offCE,4),'ce_on':round(onCE,4),
         'ce_delta':round(onCE-offCE,4),'ce_real':round(realCE,4),
         'pred_a_block2_falls_30pct':bool(pa),
         'pred_b_survives_to_peak':bool(pb),
         'pred_c_ce_does_not_worsen':bool(pc)}
    print(f"\nCE: off {offCE:.4f} | on {onCE:.4f} | delta {onCE-offCE:+.4f} "
          f"(real model {realCE:.4f})")
    print(f"(a) block-2 rel-MSE falls >=30% ({d2:+.2f}%): "
          f"{'HELD' if pa else 'FAILED'}")
    print(f"(b) block-6 rel-MSE lower with absorber: {'HELD' if pb else 'FAILED'}")
    print(f"(c) CE does not worsen: {'HELD' if pc else 'FAILED'}")
    if pa and not pc:
        print('    S307 READING: the stream improves locally while CE regresses '
              '-- the downstream rungs are off-context, so the verdict is '
              '"needs the full matched-context merge", not "a1 is worthless".')
