...
def main():
 ...
 base_response={'attention':{k:v.to(backend.device) for k,v in base_full['attention'].items() if k in (8,9,11)},'mlp':{k:v.to(backend.device) for k,v in base_full['mlp'].items() if k in (1,3,4,6)}};donor_response={'attention':{k:v.to(backend.device) for k,v in donor_full['attention'].items() if k in (8,9,11)},'mlp':{k:v.to(backend.device) for k,v in donor_full['mlp'].items() if k in (1,3,4,6)}}
 mask30={s:(top[s] if s=='MLP0' else full[s]) for s in SUPPORT};fresh={'rows':rows,'batch':batch};base_state=atlas.states(torch,backend,base_output,rows);donor_state=atlas.states(torch,backend,donor_output,rows);target=pair.response_vectors(backend,fresh,base_response,donor_response,qs,modes);reports={};finite=[]
 for name,masks in (('top50',top),('mask30',mask30),('full',full)):
  out,changed=parent.run_selected(backend,batch,base_full,{s:bh1[s]-bh0[s] for s in SUPPORT},bases,maps,masks);vectors=pair.response_vectors(backend,fresh,base_response,changed,qs,modes);response={t:pair.vector_metrics(torch,vectors[t],target[t]) for t in TASKS};behavior=joint.behavior_report(backend,rows,base_state,donor_state,atlas.states(torch,backend,out,rows));reports[name]={'response':response,'behavior_signed_projection':behavior};finite+=list(behavior.values())+[x for t in TASKS for x in response[t].values()]
