"""K-ESCALATION OF THE CP FRONT (rung 114): does raising K buy back the compounding?

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows; LOWER IS BETTER.
S2206: mixed-K CP front = +0.9427, 0/62, 3.3x its singles sum. If the excess scales with per-site damage,
richer sites collapse it superlinearly; if the compounding REGIME persists, all-2304 lands near 3.3x its
singles sum (3.3 x [.0647+.0454+.0207+.0223+.052] ~ +0.68). Arms: all four front MLPs at K=2304; all at
K=3456 (75% of units). a0+a1v tables in both; all else real.

REGISTERED PREDICTIONS (arms named agg(all2304), agg(all3456)):
  (a) K BUYS IT BACK: agg(all2304) <= 0.4 (between collapse ~0.2 and persist ~0.68 - discriminative bar).
  (b) VALIDITY APPEARS: valid(all2304) >= 5 of 62.
  (c) MORE K, LESS DAMAGE: agg(all3456) < agg(all2304) and agg(all3456) <= 0.20.
NULL: the compounding regime persists (~+0.68, 0 valid) - K cannot fix what drift composition breaks.
PRICE: all-2304 = 31,850,496 values; all-3456 = 47,775,744 (vs four tables 231,584,256).
Tripwire: INSTRUMENT FAIL if the two arms are identical. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: CP-front K escalation')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_cp_front2_results.json'
...
def main():
    ...
    if SEL.get('cp_front2'):
        _ROWS=SEL.get('ext_rows',FR)
        cur['clsmap']=classify2(_ROWS).to(DEV)
        _arms={}
        for _KK in (2304,3456):
            for _li in (0,1,2,3):
                _mlp=m.transformer.h[_li].mlp
                _L=_mlp.Left.weight.detach().float(); _R=_mlp.Right.weight.detach().float()
                _Dw=_mlp.Down.weight.detach().float(); _db=_mlp.Down_bias.detach().float()
                ...
                S[f'cf{_li}']=('cp',_li,_L[_kp].contiguous(),_R[_kp].contiguous(),
                               _Dw[:,_kp].contiguous(),_db)
            _arms[f'all_{_KK}']=evalV(_ROWS,_ROWS.shape[0],['a0','a1v','cf0','cf1','cf2','cf3'],ML).detach().cpu()
            print(f'  all-{_KK} done',flush=True)
        del cur['clsmap']
        SEL['front_result']=_arms
...
if __name__=='__main__':
    ...
    SEL['cp_front2']=True; SEL['ext_rows']=CROWS
    print('ARMS: CP front all-2304 and all-3456 - census rows',flush=True)
    main()
    A=SEL['front_result']
    out={}
    ...
    if abs(out['all_2304']['agg']-out['all_3456']['agg'])<1e-6:
        raise SystemExit('INSTRUMENT FAIL: arms identical')
    a24=out['all_2304']['agg']; a34=out['all_3456']['agg']
    pa=a24<=0.4
    pb=out['all_2304']['valid']>=5
    pc=(a34<a24) and a34<=0.20
    res={'arms':out,'values':{'all_2304':4*2304*3456,'all_3456':4*3456*3456},
         'convention':'per-position dCE = CE(config) - CE(real model) on census rows; lower is better',
         'pred_a_k_buys_back':bool(pa),'pred_b_validity_appears':bool(pb),'pred_c_monotone':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) agg(all2304) {a24:+.4f} <= 0.4: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid(all2304) {out['all_2304']['valid']} >= 5: {'HELD' if pb else 'FAILED'}")
    print(f"(c) agg(all3456) {a34:+.4f} < all2304 and <= 0.20: {'HELD' if pc else 'FAILED'}")
