"""REGISTERED MILESTONE CLAIM: THE EXACT-MIDDLES CONFIG WITH THE FIRST CERTIFICATE BAR (rung 200).

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. Rung 199 printed census +0.1492 /
fresh +0.1440 with valid 2/62 - the first nonzero certificate count in ~50 certified configs. Identical
deterministic rebuild with rows saved; the claim includes, for the first time, a CERTIFICATE prediction.

REGISTERED PREDICTIONS (anchors: rung-199 receipt):
  (a) REPRODUCTION: |census - 0.1492| <= 0.015.
  (b) THE FIRST CERTIFICATE CLAIM: valid >= 2 of 62.
  (c) REPRODUCTION: |L2_F - 0.1440| <= 0.015.
NULL: 199's print was noise. PRICE: ~162M values (high-fidelity end; the only compressed objects are the
148 attention patterns at rank 32). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: milestone claim exact-middles + first certificate bar')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_claim_exact_results.json'
...
if __name__=='__main__':
    ...
    pa=abs(agg-0.1492)<=0.015
    pb=nv>=2
    pc=abs(L2F-0.1440)<=0.015
    ...
    print(f"(a) |census {agg:+.4f} - 0.1492| <= 0.015: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 2 (THE FIRST CERTIFICATE BAR); margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |L2F {L2F:+.4f} - 0.1440| <= 0.015; era rho {rho:.4f}: {'HELD' if pc else 'FAILED'}")
