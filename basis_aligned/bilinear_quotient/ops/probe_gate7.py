"""PROBE GATE v7 -- EIGHT fresh windows, to find what actually drives the spread.

S2081 found the gating arc's numbers do not transfer between fresh windows.
S2082 found FR is seven documents and that on FR3 the ORACLE gain is negative
(-0.1305) -- gating with perfect causal labels makes the assembly worse. S2082
then presented the document count as the mechanism, and I corrected that in place
before writing this: the three windows do NOT order by document count.

    FR   7 docs  +0.0776      FR3  17 docs  -0.1305      FR2  42 docs  +0.0578

The most diverse window is not the worst; the middle one is. So document count is
a fact about FR and not an explanation of anything, and with three points nothing
can be distinguished. This runs EIGHT document-disjoint windows of 120 rows each
and records, per window, the candidate explanatory variables alongside the gain:
document count, base CE, and gated fraction.

Arms are cut to blk2 (S342's published configuration) and blk2+9 (the best pair
from S2080) because the question is about WINDOWS, not arms, and every window
costs a full 36-probe oracle sweep.

REGISTERED PREDICTIONS:
  (a) IS FR3 RARE OR IS GATING BROADLY UNRELIABLE: the oracle gain is positive on
      at least 6 of the 8 windows. If TRUE, negative-gain windows are a minority
      case and the arc's numbers are optimistic-but-real; if FALSE, gating with
      perfect causal labels fails on a large fraction of fresh text and every
      section from S337 onward is reporting a minority outcome as the result.
      This is the load-bearing predicate and I do not know which way it goes.
  (b) DOCUMENT COUNT DOES NOT EXPLAIN IT: |Pearson r| between oracle gain and
      document count across the eight windows is < 0.5. Registered in the
      direction my own self-correction implies, so the risk sits on the position
      I just took rather than the one I abandoned. If FALSE, doc count DOES
      order the windows once there are eight of them, S2082's original framing
      was right, and my correction of it was the error.
  (c) BASE CE DOES: |Pearson r| between oracle gain and the window's base CE is
      >= 0.5 -- the obvious alternative, that harder text gates differently. If
      (b) and (c) both fail, then nothing measured here explains a spread that
      runs from +0.078 to -0.131, and I will report that as the finding rather
      than reaching for a fourth variable post hoc.

Writes probe_gate7_results.json.
"""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['probe_gate6_results.json','experiments/gating/probe_gate.py']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _v3=json.load(open(_bq+'probe_gate6_results.json'))
    if 'oracle_gain_by_window' not in _v3:
        print('DRYRUN FAIL: S2082 results missing'); raise SystemExit(1)
    print(f"DRYRUN OK: S2082 present (oracle gains "
          f"{_v3['oracle_gain_by_window']}); sweeping eight windows")
    raise SystemExit(0)
...
OUT=PT+'probe_gate7_results.json'
...
def main():
    ...
    npos=sum(1 for g in og if g>0)
    r_doc=pearson(og,dc); r_base=pearson(og,bc)
    pa=npos>=6
    pb=abs(r_doc)<0.5
    pc=abs(r_base)>=0.5
    out={'windows':ROWS,'n_windows':len(ROWS),
         'oracle_gain_positive_count':npos,
         'pearson_oracle_vs_docs':round(r_doc,4),
         'pearson_oracle_vs_base_ce':round(r_base,4),
         'oracle_gain_min':min(og),'oracle_gain_max':max(og),
         'pred_a_oracle_positive_on_6_of_8':bool(pa),
         'pred_b_docs_do_not_explain':bool(pb),
         'pred_c_base_ce_explains':bool(pc)}
    print(f"\noracle gain: min {min(og):+.4f} max {max(og):+.4f}; "
          f"positive on {npos}/{len(ROWS)}")
    print(f"(a) oracle positive on >=6 of 8: {'HELD' if pa else 'FAILED'}")
    print(f"(b) |r(oracle, docs)| {abs(r_doc):.3f} < 0.5: "
          f"{'HELD' if pb else 'FAILED'}")
    print(f"(c) |r(oracle, baseCE)| {abs(r_base):.3f} >= 0.5: "
          f"{'HELD' if pc else 'FAILED'}")
    if pb and not pc:
        print('    NEITHER measured variable explains a spread from '
              f'{min(og):+.4f} to {max(og):+.4f}. Reporting that as the finding '
              'rather than reaching for a fourth variable post hoc.')
