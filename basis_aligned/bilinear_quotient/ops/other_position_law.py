...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; position law of the other-class damage, cliff vs tail (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'other_position_law_results.json'
...
if __name__=='__main__':
    ...
    print('ARM 1/3: full S2144 config, per-position CE captured',flush=True)
    r_full=main()
    ...
    dc=(cev_full-cev_no5); dt=(cev_full-cev_s16)
    OTH=CLS.index('other')
    om=clsflat==OTH
    pos=torch.arange(len(clsflat))%256
    late=om&(pos>=128); early=om&(pos<128)
    def share(d):
        sl=float(d[late].sum()); se=float(d[early].sum())
        return sl/max(sl+se,1e-9), sl, se
    shc,slc,sec=share(dc); sht,slt,set_=share(dt)
    pa=sht>=shc+0.10
    pb=(shc>=0.50) and (sht>=0.50)
    pc=abs(r_full['L2_F']-2.6662)<=0.01
    res={'full':{'L2_F':r_full['L2_F']},
         'cliff_other':{'late_share':round(shc,4),'late_sum':round(slc,1),'early_sum':round(sec,1)},
         'tail_other':{'late_share':round(sht,4),'late_sum':round(slt,1),'early_sum':round(set_,1)},
         'convention':'per-position CE differences; positive = the replacement hurts there; late = position >= 128',
         'pred_a_range_law_extends':bool(pa),'pred_b_both_late':bool(pb),'pred_c_reproduces_s2144':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'other-damage late-half share: cliff {shc:.3f} (late {slc:+.1f} / early {sec:+.1f}) | tail {sht:.3f} (late {slt:+.1f} / early {set_:+.1f})')
    print(f"(a) tail {sht:.3f} >= cliff {shc:.3f} + 0.10: {'HELD' if pa else 'FAILED'}")
    print(f"(b) both >= 0.50: {'HELD' if pb else 'FAILED'}")
