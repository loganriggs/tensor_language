...
Replay<1e-5; paired response error<.10, <=.9narrow26 and<=1.1parent656.
Conditional term only; fixed recipient RMS/background; no fitting or semantic adoption.
Prices26/48384,656/903168,384/322048+768indices. Null:no wider-bank advantage.
"""
...
def main():
 ...
 out=P/'PAIRED_ROOT_INTERCHANGE_V1.json';assert not out.exists();start=time.monotonic()
 files={'tokens':'FULL_CHANNEL_FRESH_TOKENS_V1.pt','narrow26':'WIDE_NATIVE_QUARTIC_4_INHERITED_LONG_V1.pt','parent656':'QUARTIC_JOINT_READOUT_LAMBDA_1_V1.pt','paired384':'PAIRED_ROOT_RANK_16_V2.pt','spectral':'QUARTIC_RANK_BLOCK_16_V1.pt'}
 hashes={k:hashlib.sha256((P/v).read_bytes()).hexdigest() for k,v in files.items()};assert hashes==json.loads((P/'PAIRED_ROOT_BRANCH_INPUTS_V1.json').read_text())['sha256']
 def move(v):
  ...
  return v
 programs={k:move(torch.load(P/files[k],weights_only=True)) for k in ['narrow26','parent656','paired384']};tokens=torch.load(P/files['tokens'],weights_only=True)
 ...
 def evaluate(arm,x):return teacher(x) if arm=='exact' else paired(programs[arm],x) if arm=='paired384' else bank(programs[arm],x)
 ...
 for domain,documents in tokens.items():
  ...
  for doc,c in enumerate(captures):
   ...
   for arm in ['exact','narrow26','parent656','paired384']:
   ...
   for alpha in [.5,1.]:
    ...
    ref=changed['exact']
    for arm,effect in changed.items():rows.append(dict(domain=domain,document=doc,donor=(doc+1)%len(captures),alpha=alpha,arm=arm,tokens=len(x),reference_energy=float(ref.square().sum()),estimate_energy=float(effect.square().sum()),error_energy=float((effect-ref).square().sum()),dot=float((effect*ref).sum()),native_ce_response=ce['exact'],candidate_ce_response=ce[arm]))
   print(json.dumps(dict(domain=domain,document=doc)),flush=True)
 summary=summarize(rows);primary=[r for r in summary if r['arm']=='paired384'];lookup={(r['domain'],r['alpha'],r['arm']):r for r in summary}
 pred=dict(pred_a_instrument=max(checks)<1e-5 and max(zeros)==0 and all(r['effect_error']==0 for r in summary if r['arm']=='exact'),pred_b_response=all(r['effect_error'] is not None and r['effect_error']<.1 for r in primary),pred_c_advantage=all(r['effect_error'] is not None and r['effect_error']<=.9*lookup[r['domain'],r['alpha'],'narrow26']['effect_error'] and r['effect_error']<=1.1*lookup[r['domain'],r['alpha'],'parent656']['effect_error'] for r in primary))
