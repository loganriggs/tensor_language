#!/usr/bin/env python3
# BQGATE: six frozen predictions; the amended tier-3 battery (v197 protocol, unchanged) over the twenty-third six spec-authored behaviours, plus one instrument.
"""v247: the AMENDED tier-3 battery (v196/v197 protocol, unchanged) over the TWENTY-THIRD batch of spec-authored behaviours (SIX) -- plus one instrument.
...
v231 7/8, v233 3/8, v235 4/8, v237 2/8, v239 3/8, v241 4/6, v243 5/7, v245 2/6 (+instrument each) at ~30-45 GPU-s per behaviour.
FIRST batch under the v245 lesson: the pre-screen floor is 1.0 on A1 (mean base/donor on the donor axis), not 0.5 -- across v241/v243/v245
every A1 cell >= 1.0/1.0 passed row 3 (17/17) and both cells below 1.0 failed it. v245 also showed non-number/person readouts leaking own-C
at row 4 (3/6 vs 1/13 for number/person), so this batch returns to DETERMINER/PRONOUN-type readouts OUTSIDE the number and person families:
gender (three pairs), case (one pair) -- plus two preposition-selection readouts as a probe of whether the leak follows the readout class.
Recipe since v231: GRAMMATICAL cues with readout token pairs never screened before (every pair grepped against every `vocabulary=` in the
corpus; his/her, him/her, his/hers, we/us, to/at, in/of are all new pairs). Authored through `circuit_fast_screen_behaviour_spec`,
single-cue swap, matched final token, cue never final, 32 A1/A2/P/C rows, ABAB directions:
gender_possessive_his_her (`Near the lantern he/she kept the crate, of course, in` -> his/her; gender readout, no counted family);
gender_object_him_her (`Near the lantern he/she lifted the crate, of course, and the sailor thanked` -> him/her; gender);
gender_possessive_pronoun_his_hers (`Near the lantern he/she kept the crate, of course, since it was` -> his/hers; gender);
case_we_us (`The pilots near the lantern believed/thanked, of course,` -> we/us; case family);
verb_preposition_to_at (`Near the lantern the pilot listened/looked, of course,` -> to/at; verb_preposition family);
adjective_preposition_in_of (`Near the lantern the pilot was interested/afraid, of course,` -> in/of; selection readout, no counted family).
CPU capability margins BEFORE enqueue (mean base / mean donor on the donor axis; A1 | A2; dropped rows; floor 1.0 on A1 per the v245 lesson):
  gender_possessive_his_her A1:-3.19/+3.27 drop0 A2:-1.38/+1.37 drop0 C:-1.43/+0.93 drop2
  gender_object_him_her A1:-2.60/+2.67 drop0 A2:-3.12/+3.02 drop0 C:-1.43/+0.93 drop2
  gender_possessive_pronoun_his_hers A1:-2.43/+2.65 drop0 A2:-2.57/+2.46 drop0 C:-1.43/+0.93 drop2
  case_we_us A1:-2.18/+1.97 drop0 A2:-1.94/+1.93 drop0 C:-1.43/+0.93 drop2
  verb_preposition_to_at A1:-2.79/+2.94 drop0 A2:-2.54/+2.68 drop0 C:-1.43/+0.93 drop2
  adjective_preposition_in_of A1:-4.68/+4.68 drop0 A2:-3.20/+3.33 drop0 C:-1.43/+0.93 drop2
  All six A1 cells are >= 2.18/1.97 with zero dropped rows -- the first batch with every A1 cell above 2.0; every A2 cell >= 1.37/1.38. NO repairs.
  DROPPED without repair (both sides incapable, so no single-side repair applies): case_she_her (`hoped/helped, of course,` -> she/her:
  A1 -0.46/+0.29 drop3, A2 -0.84/+0.41 drop1 -- `her` doubles as a possessive determiner, so the object side never separates) and
  numeral_dual_either_any (`Of the two/three crates ... could lift, of course,` -> either/any: A1 -0.31/+0.21, A2 -0.47/+0.42). Batch size SIX;
  bars are the six-cell bars (3/5/5/4/4 of 6).
Protocol identical to v197/v201/v204/.../v245 (greedy pool 40, target 0.88, min_gain 0.005, max 30; row 2 = MIN over the two held directions
...
REGISTERED (bars in BARS; each coded predicate is the sentence here):
  pred_a_four_rows      -> at least 3 of the 6 new behaviours pass all four rows (pre-screened batches 11/19 = 0.58; all six A1 cells >= 2.0 here). prior 60%
  pred_b_row2           -> MIN held-out per-direction exact-set recovery >= 0.80 on at least 5 of 6.              prior 65%
  pred_c_row3           -> dim A1 CE damage LB975 > 0 and >= 0.10 on at least 5 of 6 (17/17 of the >= 1.0 A1 cells across v241-v245 passed). prior 75%
  pred_d_row4           -> constrained rank-1 own-C CE UB975 <= 0.01 on at least 4 of 6 (v241 5/6, v243 5/7, v245 3/6; determiner/pronoun readouts 12/13). prior 60%
  pred_e_row5_amended   -> A2-own / A2 ceiling within [0.5, 1.4] with LB975 > 0 on at least 4 of 6 (v241 6/6, v243 7/7, v245 6/6). prior 70%
...
Accounting: four-row passes here join the one-protocol standing. gender_* (x3) and adjective_preposition_in_of have no counted family and
count as singletons on a four-row pass (a within-batch gender separability rung follows if two or more gender cells pass); case_we_us (case,
2 counted) and verb_preposition_to_at (verb_preposition, counted) count only after a separability rung under the full family control set.
...
Smoke: V247_SMOKE=<out.json> (CPU, V247_SMOKE_ROWS=4 per split, V247_SMOKE_NAMES=case_we_us; pool 3/max 3, steps 5).
"""
...
OUT = ROOT / "circuits/followups/unit_tier3_batch_amended_spec8w_v247_result.json"
...
NAMES = {"gender_possessive_his_her": "gender_possessive_his_her", "gender_object_him_her": "gender_object_him_her",
         "gender_possessive_pronoun_his_hers": "gender_possessive_pronoun_his_hers", "case_we_us": "case_we_us",
         "verb_preposition_to_at": "verb_preposition_to_at", "adjective_preposition_in_of": "adjective_preposition_in_of",
...
def _plan():
    return {"candidate_id": "corpus.unit_tier3_batch_amended_spec8w_v247", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V247_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V247_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V247_SMOKE_NAMES", "case_we_us").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_tier3_batch_amended_spec8w_v247", "candidate_id": "corpus.unit_tier3_batch_amended_spec8w_v247", "bars": BARS,
