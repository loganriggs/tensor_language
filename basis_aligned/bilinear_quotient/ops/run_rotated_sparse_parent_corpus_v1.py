...
the frozen weight-optimized25%entry-pruned64-node candidate. No data fitting; all50304modeled outputs.
"""
...
@torch.no_grad()
def main():
    binding=json.loads((P/'ROTATED_SPARSE_PARENT_CORPUS_V1_BINDING.json').read_text())
    ...
    out=P/'ROTATED_SPARSE_PARENT_CORPUS_V1_RESULT.json';assert not out.exists()
    ...
    item=torch.load(P/'SPARSE_READER_ROTATION_V1_PROGRAM.pt',weights_only=True)['0.25']
    _,_,q=unpack(item)
    candidates['rotated_sparse25']=dict(native,key_basis=[native['key_basis'][0],q.cuda()])
