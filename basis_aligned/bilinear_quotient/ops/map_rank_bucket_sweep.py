# IS THERE A MAP RANK THAT WINS BOTH BUCKETS? -- sweeping the tradeoff §1933/§1934 found.
#
# §1933 and §1934 established, at two coverages, that raising §1870's fallback map from rank 64 to 512
# buys unseen-target kept-fraction (+1.3 / +0.5 / +0.4pp at 5,419; +0.68 / +0.47 / +0.22pp at 16,110) and
# costs a little on the 125+ bucket (63.5 -> 63.1, 62.9 -> 62.1, 63.4 -> 62.8 at 5,419). Both were
# two-point comparisons: rank 64 against rank 512, nothing in between.
#
# That leaves a practical question the cost arc cares about. At the DEPLOYED 5,419 coverage no build using
# a rank-512 map matches §1789's deployed design on the common bucket -- §1933's full512 arm, which is the
# most expensive build in that sweep at 267.245M, still comes in at 63.1 / 62.1 / 62.8 against 63.5 / 62.9
# / 63.4. If the tradeoff is monotone in map rank there may be an INTERMEDIATE rank that takes most of the
# rare-target gain without the common-target loss, and nobody has looked.
#
# Table rank is held at FULL in every arm so only the map varies; the rank-64 arm is §1789's deployed
# design and is the published anchor.
#
# ROLES. skip7000, skip11000, skip1200. DISCOVERY ONLY, 5,419 coverage. Rung 3.
#
# Registered predictions, TWO-SIDED per LESSONS 31, margins per LESSON 40, read back per LESSON 39.
# LESSON 72 applied: the questions here are directional and about existence, so the predicates are SIGNED
# comparisons and an explicit search, never a tolerance on |delta|.
#   pred_a THE RARE GAIN IS MONOTONE IN MAP RANK: the unseen-target kept-fraction at rank 512 exceeds
#          that at 256, which exceeds 128, which exceeds 64 -- on at least 2 of 3 roles. If FALSE the
#          benefit is not a smooth function of rank and an intermediate point cannot be reasoned about
#          from the endpoints, which would itself be worth knowing before anyone interpolates.
#   pred_b AND THE COMMON COST IS MONOTONE TOO: the 125+ kept-fraction at rank 512 is BELOW that at 256,
#          below 128, below 64, on at least 2 of 3 roles. The other half of the tradeoff. If FALSE the
#          common-target cost is not rank-driven and §1933's reading of it needs revisiting.
#   pred_c AND A SWEET SPOT EXISTS: at rank 128 or 256 the unseen kept-fraction is at least 0.3pp ABOVE
#          the deployed rank-64 arm AND the 125+ kept-fraction is no more than 0.2pp BELOW it, on at
#          least 2 of 3 roles. This is the practical question -- most of the rare gain for almost none of
#          the common loss. If FALSE the tradeoff is unavoidable at every rank and the choice is a genuine
#          deployment call rather than a free lunch, which is the more likely outcome and worth stating
#          plainly rather than hunting for.
#   pred_d CONTROLS: coverage is exactly 5,419; the rank-64 arm reproduces §1932's PUBLISHED deployed
#          figures -- 125+ 63.5 / 62.9 / 63.4% and unseen 2.7 / 6.2 / 3.6% -- within 0.5pp; buckets
#          partition; and the LIVE per-bucket accuracy is identical across all four arms.
...
RANKS = ('map64', 'map128', 'map256', 'map512')
MAPRANK_OF = {'map64': 64, 'map128': 128, 'map256': 256, 'map512': 512}
# UNIFORM ranks only -- the question is about the table axis, not the allocation.
ALLOC = {k9: None for k9 in ('map64', 'map128', 'map256', 'map512')}   # FULL table rank throughout
...
def main():
    ...
    ORD = ['map64', 'map128', 'map256', 'map512']
    monup = sum(1 for e in roles
                if all(kept(ORD[i9 + 1], e, bot) > kept(ORD[i9], e, bot) for i9 in range(3)))
    mondn = sum(1 for e in roles
                if all(kept(ORD[i9 + 1], e, top) < kept(ORD[i9], e, top) for i9 in range(3)))
    sweet = sum(1 for e in roles
                if any(kept(a2, e, bot) - kept('map64', e, bot) >= 0.003
                       and kept(a2, e, top) - kept('map64', e, top) >= -0.002
                       for a2 in ('map128', 'map256')))
    livespread = max(abs(res[a2][e][b]['top1_acc_live'] - res['map64'][e][b]['top1_acc_live'])
                     for a2 in ORD for e in roles
                     for b in [f'{x}-{y}' for x, y in BUCKETS] + ['overall'])
    partition = all(sum(res[a2][e][f'{x}-{y}']['n'] for x, y in BUCKETS)
                    == res[a2][e]['overall']['n'] for a2 in ORD for e in roles)
    pa = monup >= 2
    pb = mondn >= 2
    pc = sweet >= 2
    pd = (ncov == NCOV and partition and livespread <= 1e-9
          and all(abs(kept('map64', e, top) - S1932_TOP[e]) <= 0.005 for e in roles)
          and all(abs(kept('map64', e, bot) - S1932_BOT[e]) <= 0.005 for e in roles))

    print(f'\n  kept-fraction by MAP rank at FULL table rank, {ncov} types:', flush=True)
    for e in roles:
        print(f'    {e:10s} 125+   ' + '  '.join(
            f'{a2[3:]} {kept(a2, e, top):.1%}' for a2 in ORD), flush=True)
        print(f'    {"":10s} unseen ' + '  '.join(
            f'{a2[3:]} {kept(a2, e, bot):.1%}' for a2 in ORD)
            + f'   (vs 64: ' + ' '.join(
                f'{a2[3:]}{(kept(a2, e, bot) - kept("map64", e, bot)) * 100:+.2f}'
                for a2 in ORD[1:]) + 'pp)', flush=True)
    print(f'\n  the RARE gain is monotone in map rank (>=2 roles) -> {pa}  {monup}/3', flush=True)
    print(f'  and the COMMON cost is monotone too (>=2 roles) -> {pb}  {mondn}/3', flush=True)
    print(f'  and a SWEET SPOT exists at 128 or 256 (>=2 roles) -> {pc}  {sweet}/3', flush=True)
    print(f'  coverage {ncov}, map64 reproduces §1932, partitions, LIVE identical -> control {pd}',
          flush=True)
    ...
    r2 = {'config': {'table_ranks': ['full' if r is None else str(r) for r in RANKS],
          ...
          'results': res,
          'predictions': {'pred_a_rare_monotone': bool(pa),
                          'pred_b_common_monotone': bool(pb),
                          'pred_c_sweet_spot_exists': bool(pc),
