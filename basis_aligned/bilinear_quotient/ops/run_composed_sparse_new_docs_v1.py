#!/usr/bin/env python3
# BQGATE:96prefixes;288suffixes;240seconds.
...
96prefixes;288suffixes;240seconds. Existing FineWeb/Pile corpus panels, unit parent removal,
the frozen complete-even-weight-optimized25%entry-pruned64-node candidate. No data fitting; all50304modeled outputs.
"""
...
@torch.no_grad()
def main():
    binding=json.loads((P/'COMPOSED_SPARSE_NEW_DOCS_V1_BINDING.json').read_text())
    assert all(sha256(Path(k).read_bytes()).hexdigest()==v for k,v in binding['files'].items())
    panel=json.loads((P/'COMPOSED_SPARSE_NEW_DOCS_V1_ROWS.json').read_text());rows=panel['rows']
    assert len(rows)==96
    assert all(sha256(Path(k).read_bytes()).hexdigest()==v for k,v in panel['sources'].items())
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):
        print('96prefixes;288suffixes;240seconds');return
    out=P/'COMPOSED_SPARSE_NEW_DOCS_V1_RESULT.json';assert not out.exists()
