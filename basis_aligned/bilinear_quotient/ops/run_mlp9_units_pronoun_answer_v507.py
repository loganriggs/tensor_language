...
"""MLP 9 at the pronoun answer position, by unit (v507). v504: at the pronoun slot the copied number is re-expressed by the MLPs of the readers' own
blocks, MLP 12 and MLP 9 first (0.089 each). MLP 9's write at the answer position split exactly by unit (v506: MLP 12's top unit was refuted by edit; is MLP 9's write, the other 0.089 stage, a spread too?) on u = W_U[they] - W_U[he] (lambda chain of blocks
...
PREDICTIONS (scored as written; failures preserved; priors from v408 / v464)
    pred_a_unit_closure           per-unit terms sum to MLP 9's term within relative 1e-3 on every row
    pred_b_top_10_carry_half      the top 10 units carry >= 0.50 of MLP 9's pooled contrast on u at the answer. Prior: unsure
    ...
    pred_e_top_unit_column_reads_u  |cos(Down_9[:, top], u)| >= 0.10
...
OUT = ROOT / "circuits/followups/mlp9_units_pronoun_answer_v507_result.json"
CANDIDATE_ID = "chain.mlp9_units_pronoun_answer_v507"
N_HEAD, LAYER = 9, 9
...
def main() -> None:
    ...
    OUT.write_text(json.dumps({"schema": "mlp9_units_pronoun_answer_result_v507", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
