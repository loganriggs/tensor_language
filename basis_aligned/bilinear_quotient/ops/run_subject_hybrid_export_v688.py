...
"""Portable hybrid export on the now-opened v687 combinations.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v688_result.json'
PREDICTIONS=dict(pred_a_exact_replay='original replay plus baseline tangent centralfinitechange relative error<=.01',
 pred_b_effect_prediction='tangent-only A/B target<=.10 and modal<=.05 everycell',
 pred_c_capable_position_transfer='hybrid A/B target<=.10,modal<=.05; everycell native capability>=.90 and native modal collateral<=.10')


def main():
    plan=dict(prefix_calls=8,suffix_calls=56,sequences_per_arm=48,fits=0,backwards=0,analytic_reverse_passes=4,attention_pullbacks=24,reference_attention_forwards=48,additional_tangent_suffix_calls=8,additional_initial_interface_suffix_calls=12,
    ...
    from hybrid_response_runtime import fold_readers,execute as run_hybrid
    from conditional_attention_mlp_runtime import select_attention_examples
    ...
    for entries in batches:
        ...
        exported=dict(families=[entries[i][3] for i in [0,1]],tokens=tokens[idx],
            attention=[select_attention_examples(ap,idx) for ap in context['all_attention']],
            contexts=[{k:v[idx] for k,v in mc.items()} for mc in context['all_mlp_contexts']],
            final_context={k:v[idx] for k,v in case['final_context'].items()},positions=readpos[idx],
            baseline_margin=base[idx],direction=direction[idx],readers=fold_readers(q0,programs[0]['input_basis'],Q)[:,idx],sources={})
        per_batch={}
        for source_name,delta in [('A',source_deltas[0]),('B',source_deltas[1]),('sum',source_deltas[0]+source_deltas[1])]:
            ...
            exported['sources'][source_name]=dict(delta=delta[idx].double(),
                reference=torch.cat([(predicted_effect-lost_linear[:,0]*direction)[idx,None],-full_linear[idx,1:]],dim=1))
            packed=run_hybrid(sparse_runtime,Q,exported,exported['sources'][source_name]['delta'])
            export_checks.append(float((packed-exported['sources'][source_name]['reference']).abs().max()))
        ...
        exported_cases.append(cpu(exported))
    ...
    artifact=OUT.parent/'subject_hybrid_v688.pt'
    assert not artifact.exists()
    package_export=cpu(dict(runtime=sparse_runtime,encoder=Q,cases=exported_cases,
        scope='Conditional output predictor with full source-response and native background/gradient ports; not a residual-state replacement'))
    def values(v):
        ...
        return 0
    guard_write(values(package_export)*8,str(OUT.parent),'hybrid conditional export')
    assert max(export_checks)<=1e-8
    torch.save(package_export,artifact)
    result=dict(export_max_error=max(export_checks),artifact_bytes=artifact.stat().st_size,export_values=values(package_export),created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,cells=cells,
                predictions=dict(zip(PREDICTIONS,[bool(a),bool(b),bool(c)])),selectivity=selectivity,hybrid_reports=hybrid_reports,tangent_reports=tangent_reports,max_tangent_relative_check=max(tangent_checks),reader_values_per_batch=reader_prices,tangent_suffix_calls=tangent_calls,initial_reports=initial_reports,initial_interface_suffix_calls=initial_counts,max_full_state_readout_error=max(oracle_checks),failed_modal_oracle_ratios=[o/e for o,e in failed],reuse_reports=reuse_reports,cross_effects=cross,reuse_counts=reuse_counts,pre11_joint_vs_post11_sum_gap=boundary_gaps,
