#!/usr/bin/env python
"""late_last_two_error_correction_probe -- do the real mlp16/17 error-correct the pool's non-core write error (§2733)? CLEAN-WRITE arm (pool
perturbed, mlp16/17 write their unperturbed-stream outputs) measures the compensation; OWN_k arms (mlp16/17 own weights on top-k input PCs +
token filler, k = 16..256) measure the composition penalty vs input width. CUDA lane-1 script.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_real_blocks_compensate pred_c_penalty_falls_with_input_width
#                     pred_d_wide_own_weights_beat_the_core_program_alone pred_e_extracted_stack_with_wide_last_blocks_beats_the_fitted_stack
...
Preregistration: polynomial_causal/LATE_LAST_TWO_ERROR_CORRECTION_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_last_two_error_correction_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_last_two_error_correction_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_LAST_TWO_ERROR_CORRECTION_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_stack_composition_penalty_anatomy_probe_results.json"   # §2733
OUT = ROOT / "late_last_two_error_correction_probe_results.json"
HASHES = {PREREG: "5542e8df0cf22a1b3e94868cf565cc853b0f281b986107705267256e4fbb5f39", PRIOR: "7b60cea19e4c4d4c58cb4161eff76edee30b93a379f9ec9f183d11d8fa10cd6d",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_last_two_error_correction_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_POOL32TOK = 0.319; PRIOR_PROG = 0.246; PRIOR_COMB = 0.745; LAM = 1e-2
POOL = [("mlp", l) for l in range(11, 16)]
KIN = 32; RSQ = 8; RB = 8; KS = [16, 32, 64, 128, 256]
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "clean_tol": 0.002, "b_min": 0.10, "c_max": 0.09, "d_max": 0.20, "e_max": 0.55}
NULLS = {"b_max": 0.03, "c_min": 0.15, "d_min": 0.26, "e_min": 0.65}
...
def main():
    ...
    class OwnHead:
        """Block l with its own weights on its top-k input PCs + token filler (§2730 recipe); output unrestricted."""
    ...
    class CleanStore:
        """One clean (unpatched) forward per chunk; serves mlp16/17's clean writes to the patched forward."""
        def __init__(self):
            self.key = None; self.w = {}
        def hook(self, idx):
            key = (int(idx[0, 0]), int(idx.shape[0]), int(idx[-1, -1]))
            if key != self.key:
                self.w = {}
                forward(m, idx, collect=lambda s_, w: self.w.__setitem__(s_, w) if s_ in LAST2 else None); self.key = key
    class CleanWrite:
        def __init__(self, store, s_):
            self.store = store; self.s = s_
        def hook(self, idx):
            self.store.hook(idx)
        def __call__(self, w, x):
            return self.store.w[self.s]
    ...
    pool_part = lambda: {("mlp", l): OwnHead(l, KIN) for l in range(11, 16)}
    prog_part = lambda: {("mlp", 16): Prog(comp[16]), ("mlp", 17): Prog(comp[17])}
    own_part = lambda k: {("mlp", 16): OwnHead(16, k), ("mlp", 17): OwnHead(17, k)}
    def clean_part():
        store = CleanStore(); return {s_: CleanWrite(store, s_) for s_ in LAST2}
    arms = {"MEAN7": ce_of(m, ev, {s: meanpatch(bases[s]["mu"]) for s in LATE7}) - ce0}
    arms["POOL"] = run([pool_part()]); arms["CLEAN16_17"] = run([clean_part()]); arms["POOL+CLEAN16_17"] = run([pool_part(), clean_part()])
    arms["PROG"] = run([prog_part()]); arms["POOL+PROG"] = run([pool_part(), prog_part()])
    for k in KS:
        arms[f"OWN_{k}"] = run([own_part(k)]); arms[f"POOL+OWN_{k}"] = run([pool_part(), own_part(k)])
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    kappa = arms["POOL+CLEAN16_17"] - arms["POOL"]
    pis = {"pi_prog": arms["POOL+PROG"] - arms["POOL"] - arms["PROG"]}
    pis.update({f"pi_{k}": arms[f"POOL+OWN_{k}"] - arms["POOL"] - arms[f"OWN_{k}"] for k in KS})
    rec7 = {k: 1 - v / arms["MEAN7"] for k, v in arms.items()}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["POOL"] - PRIOR_POOL32TOK) <= BARS["repro_tol"] and abs(arms["PROG"] - PRIOR_PROG) <= BARS["repro_tol"]
                   and abs(arms["POOL+PROG"] - PRIOR_COMB) <= BARS["repro_tol"] and abs(arms["CLEAN16_17"]) <= BARS["clean_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_real_blocks_compensate': bool(kappa >= BARS["b_min"]),
        'pred_c_penalty_falls_with_input_width': bool(pis["pi_128"] <= BARS["c_max"]),
        'pred_d_wide_own_weights_beat_the_core_program_alone': bool(arms["OWN_64"] <= BARS["d_max"]),
        'pred_e_extracted_stack_with_wide_last_blocks_beats_the_fitted_stack': bool(arms["POOL+OWN_256"] <= BARS["e_max"]),
    }
    nulls = {"b_null_kappa_le_.03": bool(kappa <= NULLS["b_max"]), "c_null_pi128_ge_.15": bool(pis["pi_128"] >= NULLS["c_min"]),
             "d_null_own64_ge_.26": bool(arms["OWN_64"] >= NULLS["d_min"]), "e_null_pool_own256_ge_.65": bool(arms["POOL+OWN_256"] >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           "sign_convention": "CE ADDED above the real model on held-out docs 0-63 (FRESH split; LOWER IS BETTER); pi(X,Y) = CE(X+Y) - CE(X) - CE(Y); kappa = CE(POOL+CLEAN16_17) - CE(POOL)",
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "k_in_pool": KIN, "ks_last_two": KS, "square_space_rank": RSQ, "token_read_rank": RB,
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "pool": arms["POOL"], "prog": arms["PROG"], "pool_prog": arms["POOL+PROG"], "clean_alone": arms["CLEAN16_17"],
                          "priors": {"pool": PRIOR_POOL32TOK, "prog": PRIOR_PROG, "pool_prog": PRIOR_COMB}},
           "ce_added": arms, "recovery_of_mean7": rec7, "compensation_kappa": kappa, "penalties": pis,
           "price": {"gpu_doc_forwards": 3 * int(fit_rows.shape[0]) + int(ev.shape[0]) * (1 + len(arms) + 2), "cpu_doc_forwards": 0, "gpu_seconds": time.time() - started},
           "hashes": {str(k): (v if v and len(v) == 64 else R.sha256(k)) for k, v in HASHES.items()}, "script_sha256": R.sha256(SELF)}
    if smoke:
        print(json.dumps({k: out[k] for k in ("preds", "nulls", "instrument", "ce_added", "compensation_kappa", "penalties", "price")}, indent=1)); return
    from receipt import dump
    dump(out, OUT)
    print(json.dumps({"preds": preds, "nulls": nulls, "ce_added": arms, "kappa": kappa, "penalties": pis}, indent=1))
