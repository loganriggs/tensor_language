"""REGISTERED CLAIM: RANK 96 (rung 236) - the 9-certificate point becomes official.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. S2301 measured r96 at census +0.0622 /
fresh +0.0626 / valid 9. Identical rebuild; claim bars registered.
REGISTERED PREDICTIONS (anchors: the S2301 receipt):
  (a) REPRODUCTION: |census - 0.0622| <= 0.015.
  (b) CERTIFICATES: valid >= 9.
  (c) REPRODUCTION: |L2_F - 0.0626| <= 0.015.
NULL: noise. PRICE: ~200M values (r96 patterns 72.7M). Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: registered claim r96')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_claim_qk96_results.json'
...
if __name__=='__main__':
    ...
    pa=abs(agg-0.0622)<=0.015
    pb=nv>=9
    pc=abs(L2F-0.0626)<=0.015
    ...
    print(f"(a) |census {agg:+.4f} - 0.0622| <= 0.015: {'HELD' if pa else 'FAILED'}")
    print(f"(b) valid {nv} >= 9; margin {medrat:.3f}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |L2F {L2F:+.4f} - 0.0626| <= 0.015: {'HELD' if pc else 'FAILED'}")
