...
OUT=BQ/'circuits/followups/recent_sparse_products_v632_result.json'
...
def main():
    plan=dict(panels=['fineweb_n96_skip1200.pt','fineweb_n192_skip11000.pt'],ranks=[256,1024,2304,4608],
        families=['global_products','parent_products','random_products'],scales=[1.,0.,.5,1.5],
    ...
    orders=dict(global_products=greedy_removal_order(global_gram),
        parent_products=greedy_removal_order(parent_gram),
    ...
    result=dict(plan=plan,rows=rows,selected={k:v for k,v in selected.items() if k!='panels'},
        ...
        scope='Shared native-product dictionary pruning; not new-feature sparse DAG optimum; opened panels; upstream ports required',
        seconds=time.perf_counter()-tic,finished_utc=datetime.now(timezone.utc).isoformat())
    assert forwards<=plan['forwards_max']
    disk_guard.guard_write(1000000,label='v632 JSON');OUT.write_text(json.dumps(result,indent=2)+'\n')
