#!/usr/bin/env python3
# BQGATE: five frozen predictions; set, split and bars fixed before the run.
"""v90: greedy continuation (+8 heads) for verb_complementizer, whose row 4 is open for a real reason.

verb_complementizer's full-specificity direction has a real own-C residual with the matched that-only sibling C (v86 cross-fitted:
point 0.025, UB 0.038). v87/v88 found quantifier's real 0.010 residual vanished once the set grew from 10 to 18 heads: with more
of the behaviour's own heads in the set, the constrained direction has more room to keep the sibling inert. v66 stopped the
complementizer greedy at hub+8 with the last gain still 0.037 nat. Same protocol as v83/v87: removal-greedy on EVEN A1 with the
diff-in-means direction, eight additions, then exact-set extraction (A1, A2) and dim removal (A1, A2, C, other five) on ODD rows.

REGISTERED BEFORE THE RUN (extraction = fraction of full-ablation margin restored; removal = CE damage in nat; ODD rows)
    pred_a_reproduce      hub8 exact extraction within 0.02 of v68 (0.893) and hub8 odd removal within 0.02 of v66 (1.119).
    pred_b_not_plateaued  even removal gain over the eight additions >= 0.10. Worked: 0.15 True; 0.08 False.
    pred_c_row2_quant     hub16 exact extraction A1 >= 0.85 with lb95 >= 0.75 (already 0.893 at hub8; this registers that it does not FALL). Worked: 0.90 True; 0.84 False.
    pred_d_transfer       odd removal gain >= 0.50 x even gain AND hub16 A2 exact extraction >= hub8 A2 - 0.02.
    pred_e_cross_clean    hub16 dim removal on the other five behaviours <= 0.05 (signed, as v83) and own sibling C <= 0.05.
    Prior: a True; b ~80%; c ~85%; d ~70%; e ~40% -- the sibling C already takes 0.029 from the hub8 dim direction (v75) and greedy
    selects on A1 only, so the added heads may well raise it; e False is informative: it says the set-level C leak grows with the
    set and the constrained direction (next run) has more to cancel, the opposite of the quantifier case.
"""
...
OUT = ROOT / "circuits/followups/unit_complementizer_greedy_continuation_v90_result.json"
...
NAMES, MAX_ADD = ("verb_complementizer",), 8
REPRO_EXT, REPRO_REM, GAIN_MIN, ROW2, ROW2_LB, HOLD, A2_SLACK, CROSS_MAX = 0.02, 0.02, 0.10, 0.85, 0.75, 0.50, 0.02, 0.05
V68_EXACT, V67_ODD = {"verb_complementizer": 0.893}, {"verb_complementizer": 1.119}


def _plan():
    return {"candidate_id": "corpus.unit_complementizer_greedy_continuation_v90", "sets": list(NAMES), "max_add": MAX_ADD,
...
def main() -> None:
    ...
    predictions = {
        ...
        'pred_c_row2_quant': ext(ALL["verb_complementizer"], "hub16")["A1"]["point"] >= ROW2 and ext(ALL["verb_complementizer"], "hub16")["A1"]["lb95"] >= ROW2_LB,
    ...
    result = {"predictions": predictions, "schema": "circuit_unit_greedy_continuation_one_set_result_v1", "candidate_id": "corpus.unit_complementizer_greedy_continuation_v90",
