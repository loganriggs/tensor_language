#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_early_and_late_routes_add pred_c_five_verb_readers_add pred_d_full_verb_named_set pred_e_11_3_replays
"""Early and late verb routes are parallel (v480). v478 / v479: 5.3 carries the copier's seed from the post-noun token to the verb (early route); 11.3 / 7.8 /
13.1 / 9.7 read the noun directly (late route). Parallel routes should add under joint swaps; nested ones do not (v435, v479). 79 distant rows, verb margin:
11.3 at the noun + 5.3 at the post-noun; the four late readers at the noun + 5.3 at the post-noun; the same plus the copier (the full named verb set).
PREDICTIONS (scored as written; failures preserved; priors from v450 / v478 / v479)
    pred_a_baseline_replays        the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_early_and_late_routes_add  11.3 (noun) + 5.3 (post-noun) closes within 0.03 of 0.147 + 0.076
    pred_c_five_verb_readers_add   the four late readers (noun) + 5.3 (post-noun) closes within 0.04 of 0.334 + 0.076
    pred_d_full_verb_named_set     the five readers + the copier closes between 0.45 and 0.56 (5.3 and the copier overlap by ~0.065, v479)
    pred_e_11_3_replays            11.3 at the noun closes 0.147 +- 0.02
...
OUT = ROOT / "circuits/followups/verb_routes_parallel_v480_result.json"
CANDIDATE_ID = "chain.verb_routes_parallel_v480"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, H113, H53, V4, BAND2, BAND4, FULL_LO, FULL_HI, BAND = 1e-3, 0.147, 0.076, 0.334, 0.03, 0.04, 0.45, 0.56, 0.02
...
FORWARDS_MAX = 16
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_early_and_late_routes_add": "0.223 +- 0.03", "pred_c_five_verb_readers_add": "0.410 +- 0.04", "pred_d_full_verb_named_set": "0.45-0.56", "pred_e_11_3_replays": "0.147 +- 0.02"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "h113": H113, "h53": H53, "v4": V4, "band2": BAND2, "band4": BAND4, "full": [FULL_LO, FULL_HI], "band": BAND}}
    ...
    JOINTS = {"j113_53": {11: ([3], (0,)), 5: ([3], (1,))}, "j4_53": {11: ([3], (0,)), 7: ([8], (0,)), 13: ([1], (0,)), 9: ([7], (0,)), 5: ([3], (1,))}, "jfull": {11: ([3], (0,)), 7: ([8], (0,)), 13: ([1], (0,)), 9: ([7], (0,)), 5: ([3], (1,)), 4: ([5], (0,))}}
    MODES = {"h113_n": ({11: [3]}, (0,)), **{k: ("JOINT", None) for k in JOINTS}}; forwards = 0
    def logits_margin(x):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_early_and_late_routes_add": abs(v["j113_53"] - (H113 + H53)) <= BAND2, "pred_c_five_verb_readers_add": abs(v["j4_53"] - (V4 + H53)) <= BAND4,
                   "pred_d_full_verb_named_set": FULL_LO <= v["jfull"] <= FULL_HI, "pred_e_11_3_replays": abs(v["h113_n"] - H113) <= BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "verb_routes_parallel_result_v480", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
