"""THE MLP SIDE OF THE FRONTIER: HOW MUCH OF THE +2.6735 DO THE CP UNITS AND THE FRONT TABLES CONTRIBUTE? (Claude, LANE 1)

Companion to `frontier_error_decomposition`, which prices the two ATTENTION approximations (the 38 motif heads and the tail
dictionaries). The evaluated configuration is

    order2 = cfgF + ['a10L'...'a17L'],   cfgF = ['a0','m0E','a1v','m1','m2E','m3E'] + ['c4'...'c9'] + ['tailE']

so the MLP side is two blocks: the FRONT tables `m0E`/`m1`/`m2E`/`m3E` and the CP-unit reconstructions `c4`-`c9`. Both are
APPROXIMATIONS standing in for real MLPs, so omitting one from the evaluated config restores the real component and should LOWER L2:

    error_share(block) = L2_F(baseline) - L2_F(block restored to real),   POSITIVE = that block contributes that much of the error

Every entry manipulated here is a member of `cfgF` by construction -- the SS2879 rule -- and pred_d requires each to move the number as
a measured check rather than a code reading. The tail refits are explicitly excluded from the prefix drop so this rung and its
companion partition the construction rather than overlapping on it.

SIGN CONVENTION (SS2135): frontier L2 is CE ADDED ABOVE THE REAL MODEL, so **LOWER IS BETTER** (SS312: +2.6735 beating +2.84/+2.93).
SS2128/SS2129/SS2133/SS2134 RETRACTED; SS2125 STANDS. Note SS2131 CLOSED c6-c9 REORDERING; this rung does not reorder anything, it
prices the block.

# BQGATE: EXPERIMENT  pred_a_the_baseline_reproduces_the_published_frontier
#                     pred_b_the_cp_units_carry_a_large_share_of_the_error
#                     pred_c_the_front_tables_carry_a_measurable_share
#                     pred_d_both_blocks_are_installed_and_move_the_number
#                     pred_e_the_two_shares_are_roughly_additive

Preregistration: polynomial_causal/FRONTIER_MLP_SIDE_ERROR_SHARE_PREREGISTRATION.md
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; four arms (baseline / CP units off / front tables off / both off)")
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_mlp_side_error_share_results.json'   # NOT the parent's receipt (SS2125 cites that file)
...
PREREG = PT + '../polynomial_causal/FRONTIER_MLP_SIDE_ERROR_SHARE_PREREGISTRATION.md'
PREREG_SHA = "cd2125a9b60b5a312c583dde87afd6d25a27f62b1cb6625bb9b6f06431f0f7c3"
...
BARS = {"reproduce": 0.05, "cp_share": 0.20, "front_share": 0.10, "installed": 0.01, "additive": 0.30}
NULLS = {"cp_share_le": 0.05, "front_share_le": 0.02}
...
if __name__=='__main__':
    ...
    print('ARM 2/4: c4-c9 OFF -- MLPs 4-9 run REAL instead of CP-unit reconstructions',flush=True)
    EVALMODE['drop']=frozenset(['c'])
    r_nocp=main()

    print('ARM 3/4: front tables OFF -- m0E/m2E/m3E and m1 run REAL',flush=True)
    EVALMODE['drop']=frozenset(['m'])
    r_nofront=main()

    print('ARM 4/4: BOTH off -- the whole MLP side runs REAL',flush=True)
    EVALMODE['drop']=frozenset(['c','m'])
    r_both=main()
    EVALMODE['drop']=frozenset()

    # SIGN: L2 is CE ADDED ABOVE THE REAL MODEL, LOWER IS BETTER. These entries are APPROXIMATIONS, so
    # dropping one restores the real component and should LOWER L2. ERROR SHARE = baseline - restored,
    # POSITIVE = that block contributes that much of the frontier's error.
    share_cp    = r_base['L2_F'] - r_nocp['L2_F']
    share_front = r_base['L2_F'] - r_nofront['L2_F']
    share_both  = r_base['L2_F'] - r_both['L2_F']
    drift       = abs((share_cp + share_front) - share_both)

    pa = abs(r_base['L2_F']-2.6735) <= BARS['reproduce']
    pb = share_cp >= BARS['cp_share']
    pc = share_front >= BARS['front_share']
    pd = (abs(share_cp) >= BARS['installed']) and (abs(share_front) >= BARS['installed'])
    pe = drift <= BARS['additive']
    preds={'pred_a_the_baseline_reproduces_the_published_frontier':bool(pa),
           'pred_b_the_cp_units_carry_a_large_share_of_the_error':bool(pb),
           'pred_c_the_front_tables_carry_a_measurable_share':bool(pc),
           'pred_d_both_blocks_are_installed_and_move_the_number':bool(pd),
           'pred_e_the_two_shares_are_roughly_additive':bool(pe)}
    nulls={'b_null_the_cp_units_are_error_free':bool(share_cp<=NULLS['cp_share_le']),
           'c_null_the_front_tables_are_error_free':bool(share_front<=NULLS['front_share_le'])}
    res={'rung':'frontier_mlp_side_error_share','preds':preds,'nulls':nulls,
         'bars':BARS,'null_bars':NULLS,
         'baseline':r_base,'cp_off':r_nocp,'front_off':r_nofront,'both_off':r_both,
         'published_L2_F':2.6735,
         'summary':{'L2_F_baseline':round(r_base['L2_F'],4),
                    'L2_F_cp_off':round(r_nocp['L2_F'],4),
                    'L2_F_front_off':round(r_nofront['L2_F'],4),
                    'L2_F_both_off':round(r_both['L2_F'],4),
                    'error_share_cp_units_nats':round(share_cp,4),
                    'error_share_front_tables_nats':round(share_front,4),
                    'error_share_both_nats':round(share_both,4),
                    'additivity_drift_nats':round(drift,4),
                    'L1_F_baseline':r_base.get('L1_F'),'L1_F_cp_off':r_nocp.get('L1_F'),
                    'L2_C_baseline':round(r_base['L2_C'],4),'L2_C_cp_off':round(r_nocp['L2_C'],4)},
    ...
    print(f"(b) CP-unit error share {share_cp:+.4f} >= {BARS['cp_share']}: {'HELD' if pb else 'FAILED'}")
    print(f"(c) front-table error share {share_front:+.4f} >= {BARS['front_share']}: {'HELD' if pc else 'FAILED'}")
