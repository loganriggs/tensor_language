...
"""Correlative both/neither DoD (v260): the correlative family at MLP-8 unit grain. Readers {8.1, 7.8, 16.8, 14.8} read both / neither -> and / nor (v120); 16.8 is the
family-specific contextual reader. Exact census (`dod_units.unit_census`) of MLP 8's write on 16.8's weight-only and - nor reader direction at the CUE (both / neither)
and at the FINAL token of the v120 rows (96 rows, three verbs), pooled both - neither. Kill criterion (review 26): top-10 < 0.30 at both positions -> port.
PREDICTIONS (scored as written; failures preserved; priors unsure -- the cue is a function word, as for person and temporal (ports), but the family's readers are
contextual and the cue decides a later conjunction)
...
OUT = ROOT / "circuits/followups/correlative_both_neither_dod_mlp8_unit_census_v260_result.json"
CANDIDATE_ID = "correlative_both_neither.and_vs_nor.dod_mlp8_unit_census_v260"
LAYER, HEAD, CLOSURE_TOL, TOP10_MIN, JACCARD_MIN = 8, (16, 8), 1e-3, 0.50, 1 / 3
...
def main() -> None:
    ...
    OUT.write_text(json.dumps({"schema": "aspectual_dod_mlp8_unit_census_result_v260", "candidate_id": CANDIDATE_ID, "plan": plan, "closure_max": closure, "positions": report, "predictions": predictions, "forwards": forwards,
