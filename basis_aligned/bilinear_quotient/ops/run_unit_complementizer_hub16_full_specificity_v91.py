#!/usr/bin/env python3
# BQGATE: five frozen predictions; set (v90 complementizer hub16), recipe, split and bars fixed before the run.
"""v91: the full-specificity direction on the verb_complementizer hub16 set -- does the real row-4 residual close with more heads?

verb_complementizer on hub+8 sits at 2/3/5 with the only REAL open row 4 in the tier list: cross-fitted own-C residual against the
that-only sibling is 0.025 (UB 0.038, v86), not sample width. v88 found quantifier's real residual (0.010) vanished when the set
grew from 10 to 18 heads. v90 grew the complementizer set from 11 to 19 heads (exact-set extraction 0.951, odd removal 1.291) and
found the unconstrained dim direction leaks 0.39 into the sibling C at both sizes. Standard recipe on the hub16 set: rank 1 per
block, pooled EVEN A1 + verb-variant rows, complement 1.0, own C EVEN + other five A1 EVEN as inertness controls (30 each), 120
steps, lr 0.05, seed 0; arms xdas (primary), xdim, dim, exact, random; extraction and removal on ODD rows. Row 4 is cross-fitted
as in v86/v88: a second direction fitted on the ODD pool with ODD controls, evaluated on EVEN C; 32 out-of-sample documents pooled.

REGISTERED BEFORE THE RUN (extraction = fraction of full-ablation margin restored; removal = CE damage in nat)
    pred_a_reproduce      exact-set extraction A1 odd within 0.02 of v90's 0.951 (read from the v90 receipt).
    pred_b_row2           xdas extraction A1 odd >= 0.80 with lb95 >= 0.60. Worked: 0.88 / 0.84 True; 0.79 / 0.75 False.
    pred_c_removal_grows  xdas A1 odd removal >= 1.125 (hub8 direction 1.025 + 0.10). Worked: 1.20 True; 1.10 False.
    ...
    Prior: a True; b ~85% (0.951 minus the 0.05-0.07 objective shortfall); c ~85%; d ~40% -- the dim leak into C is 0.39, thirteen
    times quantifier's, so the penalty has more to cancel; the quantifier precedent says more heads help, the leak size says not enough.
    d False: verb_complementizer stays at 2/3/5 and row 4 is closed off as a set-size question; the next test is a second C sibling.
"""
...
OUT = ROOT / "circuits/followups/unit_complementizer_hub16_full_specificity_v91_result.json"
V90 = ROOT / "circuits/followups/unit_complementizer_greedy_continuation_v90_result.json"
NAME = "verb_complementizer"
LAM, STEPS, LR, CW = 30.0, 120, 0.05, 1.0
REPRO, REMOVAL_MIN, C_UB, ROW2, ROW2_LB, TRANSFER, CROSS_MAX, RAND_FRAC = 0.02, 1.125, 0.01, 0.80, 0.60, 0.50, 0.05, 0.10
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 700, 45000


def _plan():
    return {"candidate_id": "corpus.unit_complementizer_hub16_full_specificity_v91", "lambda": LAM,
...
def main() -> None:
    ...
    v90 = json.loads(V90.read_text())["sets"][NAME]
    sets = {NAME: v90["hub16"]}
    v90_exact = v90["sets"]["hub16"]["extraction"]["A1"]["point"]
    ...
    for name, units in sets.items():
        ...
        R[name] = {"units": units, "extraction": ext, "v90_exact_a1_odd": v90_exact, "crossfit": crossfit,
    ...
    CF = R[n]["crossfit"]
    predictions = {
        'pred_a_reproduce': abs(e(n, "A1", "exact")["point"] - R[n]["v90_exact_a1_odd"]) <= REPRO,
    ...
    result = {"predictions": predictions, "schema": "circuit_unit_hub16_full_specificity_result_v1", "candidate_id": "corpus.unit_complementizer_hub16_full_specificity_v91",
