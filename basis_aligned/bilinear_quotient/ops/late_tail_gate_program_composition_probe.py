#!/usr/bin/env python
"""late_tail_gate_program_composition_probe -- does the §2804 per-block gate compression COMPOSE across the late stack? §2804: one late
MLP at a time, the tail-read gate truncates cheaply (block 17 at 16 whitened modes .0083, block 16 at 64 .0051, block 15 at 64 .0063) but
the ten cross terms' REMOVALS compound super-additively (singles sum .190, joint .3668). A program that compresses every late block's
gate must survive that compounding. This rung adds the compression block by block from 17 downward (16 @ 17, 64 @ 16, 128 @ 8-15),
prices the singles at those ranks, the full ten-block program against §2799's uniform GATE_128 (.0627), tests whether compounding is
already present in a PAIR of removals (16+17; 8+9 as the early pair), and re-prices §2804's marginal two-block program (.01491 vs bar
.015) on the disjoint docs 64-95 (in no fit). CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_two_block_price_replicates_on_a_disjoint_split pred_c_full_program_compounds_weakly
#                     pred_d_pair_16_17_removal_already_compounds pred_e_full_program_near_uniform_128

SIGN CONVENTION (§2135): every CE number is CE ADDED ABOVE THE REAL MODEL on held-out docs 0-63 (ALT arms: docs 64-95) -- LOWER IS BETTER.
Descriptive; nothing installs into the §312 frontier; bases are data second moments scored by CE only (§2118 stays closed).
Preregistration: polynomial_causal/LATE_TAIL_GATE_PROGRAM_COMPOSITION_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/late_tail_gate_program_composition_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("late_tail_gate_program_composition_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "LATE_TAIL_GATE_PROGRAM_COMPOSITION_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "late_tail_gate_per_block_probe_results.json"   # §2804
OUT = ROOT / "late_tail_gate_program_composition_probe_results.json"
HASHES = {PREREG: "4433976712d3de636bae653f12e4a4436dcf2c7fc229be99b1d7b299c2f85b95", PRIOR: "036020dcf0cb1cba497f08498627b6c31791d660df8ad9817374957c859f48fb",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "late_tail_gate_program_composition_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_SPLIT8_1024 = 0.0374; PRIOR_LATE_MLP_768 = 0.1249; PRIOR_EARLY_TAIL_ONLY = 0.0711; PRIOR_JOINT_GATE0 = 0.3668; PRIOR_GATE128 = 0.0627; PRIOR_SUM_GATE0 = 0.1895; PRIOR_B8_GATE0 = 0.0304; PRIOR_B9_GATE0 = 0.0209; PRIOR_B16_GATE0 = 0.0181; PRIOR_B17_GATE0 = 0.0458; PRIOR_B17_GATE16 = 0.0083; PRIOR_B16_GATE64 = 0.0051; PRIOR_TWO_BLOCK = 0.01491; ALT = (64, 96); LAM = 1e-2
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.015, "exact_tol": 1e-3, "gram_tol": 1e-6, "floor": 0.002, "b_alt": 0.03, "c_factor": 1.5, "d_factor": 1.5, "e_margin": 0.01}
NULLS = {"b_alt": 0.04, "c_factor": 1.9, "d_factor": 1.1, "e_margin": 0.03}
...
def main():
    ...
    def one(l, k, extra_=None):
        ...
        return run(patch)
    # --- the per-block ranks of the compressed late tail-read program (§2804: 16 @ block 17, 64 @ block 16; 128 elsewhere) ---
    RANK_OF = {l: (16 if l == 17 else 64 if l == 16 else 128) for l in LB}
    def many(kmap):
        patch = {("mlp", j): Gate(j, None) for j in LB}
        for j, kj in kmap.items():
            patch[("mlp", j)] = Gate(j, kj)
        return run(patch)
    # ALT split: docs 64-95 -- in no fit (frames/second moments 96-191; eval 0-63); smoke: the same random rows
    ev2 = nat[ALT[0]:ALT[1]].to(DEV) if not smoke else ev
    ce0_alt = ce_of(m, ev2)
    def run_alt(patch):
        ...
        def hook(idx):
            for o in objs:
                o.hook(idx)
        return ce_of(m, ev2, patch, hook) - ce0_alt
    arms = {"SPLIT8_1024": split8(1024), "GATE_EXACT": many({})}
    arms.update({f"B{l}_GATE_{RANK_OF[l]}": many({l: RANK_OF[l]}) for l in LB})                      # singles at the program ranks
    order = list(range(NL - 1, SPLIT - 1, -1))                                                       # 17, 16, ..., 8
    cum = {}
    for n_ in range(1, len(order) + 1):
        kmap = {l: RANK_OF[l] for l in order[:n_]}; cum[n_] = many(kmap); arms[f"CUM_{n_}_{order[n_ - 1]}"] = cum[n_]
    arms["PAIR_16_17_GATE_0"] = many({16: 0, 17: 0})
    arms["PAIR_8_9_GATE_0"] = many({8: 0, 9: 0})
    two_patch = {("mlp", j): Gate(j, None) for j in LB}; two_patch[("mlp", 16)] = Gate(16, 64); two_patch[("mlp", 17)] = Gate(17, 16)
    arms["ALT_GATE_EXACT"] = run_alt({("mlp", j): Gate(j, None) for j in LB}); arms["ALT_TWO_BLOCK"] = run_alt(two_patch)
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    sum_singles = float(sum(arms[f"B{l}_GATE_{RANK_OF[l]}"] for l in LB))
    full = cum[len(order)]
    factor_full = full / max(sum_singles, BARS["floor"])
    pair_sum_16_17 = PRIOR_B16_GATE0 + PRIOR_B17_GATE0; pair_sum_8_9 = PRIOR_B8_GATE0 + PRIOR_B9_GATE0
    factor_pair_16_17 = arms["PAIR_16_17_GATE_0"] / max(pair_sum_16_17, BARS["floor"]); factor_pair_8_9 = arms["PAIR_8_9_GATE_0"] / max(pair_sum_8_9, BARS["floor"])
    marg = {str(order[n_ - 1]): cum[n_] - (cum[n_ - 1] if n_ > 1 else 0.0) for n_ in range(1, len(order) + 1)}
    summ = {"ranks": {str(l): RANK_OF[l] for l in LB}, "singles_at_program_rank": {str(l): arms[f"B{l}_GATE_{RANK_OF[l]}"] for l in LB}, "sum_singles": sum_singles,
            "cumulative_from_17_down": {str(order[n_ - 1]): cum[n_] for n_ in range(1, len(order) + 1)}, "marginal_cost_of_adding_block": marg,
            "full_program": full, "compounding_factor_full": factor_full, "prior_uniform_gate128_2799": PRIOR_GATE128, "full_minus_uniform128": full - PRIOR_GATE128,
            "pair_16_17": {"joint_gate0": arms["PAIR_16_17_GATE_0"], "sum_singles_2804": pair_sum_16_17, "factor": factor_pair_16_17},
            "pair_8_9": {"joint_gate0": arms["PAIR_8_9_GATE_0"], "sum_singles_2804": pair_sum_8_9, "factor": factor_pair_8_9},
            "alt_split": {"docs": list(ALT), "baseline_ce": ce0_alt, "gate_exact": arms["ALT_GATE_EXACT"], "two_block": arms["ALT_TWO_BLOCK"], "two_block_main_split_2804": PRIOR_TWO_BLOCK},
            "prior_gate0_factor_2799_over_2804": PRIOR_JOINT_GATE0 / PRIOR_SUM_GATE0, "direct_check_rel_err": {"8": gates[SPLIT]["check"], "17": gates[NL - 1]["check"]}, "n_fit_tokens": rec[("n", SPLIT)]}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["SPLIT8_1024"] - PRIOR_SPLIT8_1024) <= BARS["repro_tol"] and abs(arms["GATE_EXACT"]) <= BARS["exact_tol"]
                   and abs(arms["ALT_GATE_EXACT"]) <= BARS["exact_tol"] and abs(arms["B17_GATE_16"] - PRIOR_B17_GATE16) <= BARS["repro_tol"] and abs(arms["B16_GATE_64"] - PRIOR_B16_GATE64) <= BARS["repro_tol"]
                   and max(gates[SPLIT]["check"], gates[NL - 1]["check"]) <= BARS["gram_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_two_block_price_replicates_on_a_disjoint_split': bool(arms["ALT_TWO_BLOCK"] <= BARS["b_alt"]),
        'pred_c_full_program_compounds_weakly': bool(full <= BARS["c_factor"] * sum_singles + BARS["floor"]),
        'pred_d_pair_16_17_removal_already_compounds': bool(arms["PAIR_16_17_GATE_0"] >= BARS["d_factor"] * pair_sum_16_17 - BARS["floor"]),
        'pred_e_full_program_near_uniform_128': bool(full <= PRIOR_GATE128 + BARS["e_margin"]),
    }
    nulls = {"b_null_alt_ge_.04": bool(arms["ALT_TWO_BLOCK"] >= NULLS["b_alt"]),
             "c_null_factor_ge_1.9": bool(full >= NULLS["c_factor"] * sum_singles - BARS["floor"]),
             "d_null_pair_le_1.1x": bool(arms["PAIR_16_17_GATE_0"] <= NULLS["d_factor"] * pair_sum_16_17 + BARS["floor"]),
             "e_null_full_ge_uniform_plus_.03": bool(full >= PRIOR_GATE128 + NULLS["e_margin"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           "sign_convention": "CE ADDED above the real model on held-out docs 0-63 (FRESH split; LOWER IS BETTER); ALT arms on docs 64-95",
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "split": SPLIT, "alt": list(ALT),
           "program": "the §2804 per-block gate compression composed across the late stack: each late MLP's cross term gated by its top-k whitened gate modes (k = 16 at block 17, 64 at block 16, 128 at blocks 8-15), added cumulatively from block 17 downward, against the sum of single-block costs; pairwise cross-term removals (16+17, 8+9) against §2804's singles; the two-block program re-priced on the disjoint docs 64-95",
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "split8_1024": arms["SPLIT8_1024"], "prior_split8_1024": PRIOR_SPLIT8_1024, "gate_exact": arms["GATE_EXACT"], "alt_gate_exact": arms["ALT_GATE_EXACT"],
                          "b17_gate_16": arms["B17_GATE_16"], "prior_b17_gate_16": PRIOR_B17_GATE16, "b16_gate_64": arms["B16_GATE_64"], "prior_b16_gate_64": PRIOR_B16_GATE64, "direct_check_rel_err": summ["direct_check_rel_err"]},
           "ce_added": arms, "summary": summ,
           "price": {"gpu_doc_forwards": int(fit_rows.shape[0]) + int(ev.shape[0]) * (1 + len(arms) - 2) + int(ev2.shape[0]) * 3, "cpu_doc_forwards": 0, "gpu_seconds": time.time() - started},
