#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_detector_writes_the_readers_verb_vector pred_c_detector_signs pred_d_detector_change_sizeable pred_e_11_3_replays
"""Does the detector write the readers' verb vector (v497)? v496: 11.3, 7.8, 5.3 and the copier all write one verb vector. v462: MLP 17's 701 / 2059 act at the
verb position (0.069 of the margin). Here the two units' activations swapped at the answer position, scored by verb against 11.3's value swap at the noun;
79 distant rows.
PREDICTIONS (scored as written; failures preserved; priors from v444 / v479)
    ...
    pred_a_baseline_replays                   the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_detector_writes_the_readers_verb_vector  cosine over the eight verbs between the detector swap's and 11.3's signed changes >= 0.90
    pred_c_detector_signs                     under the detector swap is / was / has / does fall and are / were / have / do rise
    pred_d_detector_change_sizeable           the detector swap's mean |verb change| is >= 0.03
    pred_e_11_3_replays                       11.3's mean |verb change| replays 0.127 +- 0.02
...
OUT = ROOT / "circuits/followups/detector_verb_vector_v497_result.json"
CANDIDATE_ID = "chain.detector_verb_vector_v497"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, COS_MIN, DET_MIN, H113_V496, BAND = 1e-3, 0.90, 0.03, 0.127, 0.02
...
FORWARDS_MAX = 16
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_detector_writes_the_readers_verb_vector": "cos >= 0.90", "pred_c_detector_signs": "signs", "pred_d_detector_change_sizeable": ">= 0.03", "pred_e_11_3_replays": "0.127 +- 0.02"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "cos_min": COS_MIN, "det_min": DET_MIN, "h113_v496": H113_V496, "band": BAND}}
    ...
    def study(items, batch):
        ...
        out["verb_cos_detector_vs_11_3"] = cosv("UNITS", "11.3")
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_detector_writes_the_readers_verb_vector": verb["verb_cos_detector_vs_11_3"] >= COS_MIN, "pred_c_detector_signs": verb["UNITS"]["verb_signs_ok"],
                   "pred_d_detector_change_sizeable": verb["UNITS"]["verb_mean_abs_change"] >= DET_MIN, "pred_e_11_3_replays": abs(verb["11.3"]["verb_mean_abs_change"] - H113_V496) <= BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "detector_verb_vector_result_v497", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
