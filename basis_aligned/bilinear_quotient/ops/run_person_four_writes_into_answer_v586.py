#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_four_writes_into_the_answer_carry pred_c_rest_of_their_blocks_small pred_d_all_writes_into_the_answer pred_e_four_below_all
"""The person readers' writes INTO the answer (v586). v579-v580: on 43 natural person pairs the four 18 Sep readers' VALUES at the cue carry 0.361 of the
myself - yourself gap (8.1 0.148, 13.1 0.102, 15.1 0.060, 10.5 0.048); the early copier route carries 0.15 more into the sentence. For gender the readers'
writes into the answer (0.88) exceeded their values at the noun (0.60) because they also read the copies. Here the four readers' writes into the answer
position swapped jointly (FOUR_W), the other 32 heads of blocks 8 / 10 / 13 / 15 (REST_W), and every head's write into the answer (ALL_W).
PREDICTIONS (scored as written; failures preserved; priors from v579-v585 and the gender line)
    pred_a_baseline_replays                 the unedited manual forward replays the model's myself - yourself margins within 1e-3
    pred_b_four_writes_into_the_answer_carry  FOUR_W closes >= 0.45 (values 0.36 plus what they read from the copies)
    pred_c_rest_of_their_blocks_small       REST_W closes <= 0.15
    pred_d_all_writes_into_the_answer       ALL_W closes >= 0.85
...
OUT = ROOT / "circuits/followups/person_four_writes_into_answer_v586_result.json"
CANDIDATE_ID = "person.four_writes_into_answer_v586"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, FOUR_MIN, REST_MAX, ALL_MIN, UNIT = 1e-3, 0.45, 0.15, 0.85, 701
...
FORWARDS_MAX = 9
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_four_writes_into_the_answer_carry": ">= 0.45", "pred_c_rest_of_their_blocks_small": "<= 0.15", "pred_d_all_writes_into_the_answer": ">= 0.85", "pred_e_four_below_all": "FOUR_W <= ALL_W"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "four_min": FOUR_MIN, "rest_max": REST_MAX, "all_min": ALL_MIN, "unit": UNIT}}
    ...
    print("person writes into the answer", {k: round(v, 3) for k, v in mc.items()})
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_four_writes_into_the_answer_carry": mc["FIVE_W"] >= FOUR_MIN, "pred_c_rest_of_their_blocks_small": abs(mc["REST_W"]) <= REST_MAX,
                   "pred_d_all_writes_into_the_answer": mc["ALL_W"] >= ALL_MIN, "pred_e_four_below_all": mc["FIVE_W"] <= mc["ALL_W"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "person_four_writes_into_answer_result_v586", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
