"""REGISTERED FRONTIER CLAIM: MOTIF QK-16 (rung 184).

CONVENTION (S2135): frontier L2 = CE ADDED ABOVE THE REAL MODEL on the S312 fresh windows; LOWER IS BETTER.
Rung 182 printed DESCRIPTIVELY L2_F +1.6428 / census +1.6507 for the rung-127 build with rank-16 QK motif
heads (weights-only SVD). Identical deterministic rebuild; claim bars as predictions. If held, the
48M-class Pareto point moves +1.8222 -> +1.6428 (and beats the 63.7M point's +1.6599 at 16M fewer values).

REGISTERED PREDICTIONS (anchors: rung-182 receipt):
  (a) NEW POINT: L2_F fresh <= 1.67.
  (b) REPRODUCTION: |census - 1.6507| <= 0.015.
  (c) REPRODUCTION: |L2_F - 1.6428| <= 0.015 AND increment in [0.35, 0.55].
NULL: 182's print was noise (L2_F >= 1.70). PRICE: 47.8M front + 6,225,920 QK-16 values. Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: registered claim motif QK-16')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_claim_qk16_results.json'
...
if __name__=='__main__':
    ...
    pa=L2F<=1.67
    pb=abs(agg-1.6507)<=0.015
    pc=abs(L2F-1.6428)<=0.015 and 0.35<=inc<=0.55
    ...
    print(f"(a) L2F {L2F:+.4f} <= 1.67: {'HELD' if pa else 'FAILED'}")
    print(f"(b) |census {agg:+.4f} - 1.6507| <= 0.015: {'HELD' if pb else 'FAILED'}")
    print(f"(c) |L2F - 1.6428| <= 0.015, increment in [0.35, 0.55]: {'HELD' if pc else 'FAILED'}")
