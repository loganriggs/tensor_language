#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_copier_replays pred_c_5_3_replays pred_d_joint_below_the_sum pred_e_joint_above_the_copier
"""Does 5.3 relay the copier's seed (v479)? v478: head 5.3 reads number at the post-noun token (0.076 of the verb margin) where MLPs 4-8 rebuild it from
the copier 4.5's seed (v409 / v419). If 5.3 relays that seed to the verb, swapping the copier's value at the noun AND 5.3's value at the post-noun token
together should close less than the sum of the two (nested, v435). 79 distant rows, verb margin: copier alone (replay 0.138), 5.3 at the post-noun alone
(replay 0.076), both.
PREDICTIONS (scored as written; failures preserved; priors from v446 / v478)
    pred_a_baseline_replays     the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_copier_replays       the copier's value at the noun closes 0.138 +- 0.02
    pred_c_5_3_replays          5.3's value at the post-noun token closes 0.076 +- 0.02
    pred_d_joint_below_the_sum  the joint closes <= 0.138 + 0.076 - 0.03 (5.3 relays at least 0.03 of the copier's effect). Prior: unsure
    pred_e_joint_above_the_copier  the joint closes >= 0.138 (5.3 also carries number the copier did not plant)
...
OUT = ROOT / "circuits/followups/copier_to_5_3_v479_result.json"
CANDIDATE_ID = "chain.copier_to_5_3_v479"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, COPIER_V446, H53_V478, BAND, NEST = 1e-3, 0.138, 0.076, 0.02, 0.03
...
FORWARDS_MAX = 13
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_copier_replays": "0.138 +- 0.02", "pred_c_5_3_replays": "0.076 +- 0.02", "pred_d_joint_below_the_sum": "<= 0.184", "pred_e_joint_above_the_copier": ">= 0.138"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "copier_v446": COPIER_V446, "h53_v478": H53_V478, "band": BAND, "nest": NEST}}
    ...
    MODES = {"copier": ({4: [5]}, (0,)), "h53_n1": ({5: [3]}, (1,)), "joint": ("JOINT", None)}; forwards = 0
    def logits_margin(x):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_copier_replays": abs(v["copier"] - COPIER_V446) <= BAND, "pred_c_5_3_replays": abs(v["h53_n1"] - H53_V478) <= BAND,
                   "pred_d_joint_below_the_sum": v["joint"] <= COPIER_V446 + H53_V478 - NEST, "pred_e_joint_above_the_copier": v["joint"] >= COPIER_V446}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "copier_to_5_3_result_v479", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
