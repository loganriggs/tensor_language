# HOW LOW CAN THE TABLE RANK GO?  -- the question §1812 ended on.
#
# §1812 found the accuracy-per-real optimum among ranks 4-64 is rank 8 at all sites (0.0065 / 0.0054 /
# 0.0084 pp per million reals), beating every partial compile: depth buys higher ABSOLUTE accuracy but
# costs 23.889M reals per retained layer. Two things it could not settle. Rank 4 was the ORIGIN of that
# efficiency metric, so it could not be scored against rank 8. And nothing below 4 was measured.
#
# This sweeps ranks 1, 2, 4, 8, 16 at all sites, and scores efficiency as ABSOLUTE accuracy per real,
# which has no arbitrary origin and puts every arm including the cheapest on the same footing.
#
...
# failure branches enumerated per LESSON 44, constants named with their UNITS per LESSON 49:
#   pred_a ACCURACY IS MONOTONE IN RANK: top-1 does not decrease as rank goes 1 -> 2 -> 4 -> 8 -> 16, at
#          every role. §1812 found a non-monotonicity at depth (rank 32 was worse than rank 16 at L7 on
#          all three roles); if the same happens at all sites, rank is not a clean fidelity knob
#          anywhere and every "rank r is best" statement in §1786-§1812 needs the caveat.
#   pred_b THE EFFICIENCY OPTIMUM IS AT THE CHEAP END: the best absolute accuracy per real is at rank 4
#          or below, at every role. If FALSE the optimum is at 8 or 16 even on a metric that can see
#          rank 1 and 2, which would mean §1812's rank-8 answer was not an artifact of its origin and
#          the cheap end genuinely falls off.
#   pred_c RANK 1 IS NOT USELESS: a rank-1 table at every site retains at least 50% of the settled
#          rank-64 program's top-1 (12.88 / 13.49 / 12.89%). If FALSE the program collapses below some
#          rank between 1 and 4 and there is a floor after all -- which §1812's pred_c failed to find
#          at depth but which may exist at all sites, where §1812 measured a 2.13pp span across ranks
#          4-64 against 0.50pp at L13.
#   pred_d CONTROLS, cross-run per LESSON 42: r4_L-1, r8_L-1 and r16_L-1 reproduce §1812's PUBLISHED
#          top-1 accuracies (0.1075 / 0.1156 / 0.1095, 0.1136 / 0.1208 / 0.1174, 0.1168 / 0.1245 /
#          0.1210) within 0.01, quoted as PERCENTAGE-POINT accuracies with the units in the constant's
#          name; the endpoints reproduce §1789's 0.1355 / 0.1425 / 0.1364 and 0.3932 / 0.4235 / 0.3888
#          within 0.001; the rank-64 all-sites cost reproduces §1786's 20.531M; coverage 5419.
...
OUT = PT + 'ops/low_rank_floor_results.json'
PROBE_LS = ()
KIND_LS = ()
DEPTHS = (-1,)   # all-sites only: §1812 put the efficiency optimum here
RANKS = (1, 2, 4, 8, 16)     # the cheap end; §1812's optimum was rank 8 and rank 4 was its
                             # metric origin, so neither could be scored against the other
...
S1812_PP = {'r4_L-1': {'skip7000': 0.1075, 'skip11000': 0.1156, 'skip1200': 0.1095},
            'r8_L-1': {'skip7000': 0.1136, 'skip11000': 0.1208, 'skip1200': 0.1174},
            'r16_L-1': {'skip7000': 0.1168, 'skip11000': 0.1245, 'skip1200': 0.1210}}
S1786_R64_PP = {'skip7000': 0.1288, 'skip11000': 0.1349, 'skip1200': 0.1289}
...
def main():
    ...
    print(f'LOW-RANK FLOOR | all-sites compile at ranks {RANKS}, finding where it breaks | '
    ...
    pareto = {e: [k for k in keys
              ...
              for e in roles}
    mono = {e: all(acc[e][f'r{r2}_L-1'] >= acc[e][f'r{r1}_L-1'] - 1e-12
                   for r1, r2 in zip(RANKS, RANKS[1:])) for e in roles}
    r1frac = {e: acc[e]['r1_L-1'] / S1786_R64_PP[e] for e in roles}
    pa = all(mono[e] for e in roles)
    pb = all(int(best_eff[e].split('_')[0][1:]) <= 4 for e in roles)
    pc = all(r1frac[e] >= 0.50 for e in roles)
    pd = (all(abs(res[e]['all_substituted']['top1'] - S1789_PROG[e]) <= 0.001
              and abs(res[e]['live_model']['top1'] - S1789_LIVE[e]) <= 0.001 for e in roles)
          and all(abs(acc[e][k] - v[e]) <= 0.01 for k, v in S1812_PP.items() for e in roles)
    ...
    for e in roles:
        print(f'    {e:10s} ' + '  '.join(
            f'r{r}:{eff[e][f"r{r}_L-1"]:.4f}' for r in RANKS), flush=True)
    print(f'\n  accuracy is MONOTONE in rank from 1 to 16 -> {pa}  ' + '  '.join(
        f'{e} ' + '/'.join(f'{acc[e][f"r{r}_L-1"]:.2%}' for r in RANKS) for e in roles),
        flush=True)
    print(f'  the accuracy-per-real optimum is at rank <=4 -> {pb}  ' + '  '.join(
        f'{e} {best_eff[e]} ({eff[e][best_eff[e]]:.4f})' for e in roles), flush=True)
    print(f'  rank 1 retains >=50% of the settled rank-64 accuracy -> {pc}  ' + '  '.join(
        f'{e} {acc[e]["r1_L-1"]:.2%} of {S1786_R64_PP[e]:.2%} = {r1frac[e]:.0%}'
    ...
    json.dump({'run': 'l5_cliff_probe',
               ...
               'rank1_fraction_of_r64': r1frac,
               'predictions': {'pred_a_monotone_in_rank': bool(pa),
                               'pred_b_efficiency_optimum_at_rank_le_4': bool(pb),
                               'pred_c_rank1_keeps_half': bool(pc),
