...
"""V4_EDGE_HEAD_SCREEN_V1_PREREGISTRATION.md; unchanged oracle source edits."""
import os,sys,json,time,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[3];P=ROOT/'basis_aligned/polynomial_causal';A=ROOT/'basis_aligned/bilinear_quotient/circuits/followups';PLAN=dict(prefix=12,native_suffix=16,joined_suffix=84)
def main():
 ...
 for context in contexts:
  ...
  for sv,av in [(1.,1.)]:
   ...
   head_writes=(w*beta[:,None,:]).reshape(n,d,3,9).sum(2)
   assert float((head_writes.sum(-1)-edge).abs().max())<1e-10
   for head in range(9):
    for mode in ['only','drop']:
     update=head_writes[:,:,head] if mode=='only' else edge-head_writes[:,:,head]
     post=bg.clone();post[batch,query]=(post[batch,query].double()+update).float();posts.append((f'{mode}_{head}',post))
 ...
 result=dict(plan=PLAN,counts=counts,max_replay=max(replays),max_local_edge_relative_error=max(local_errors),records=records,predictions=dict(pred_a_instrument=instrument,pred_b_single_head_sufficient=instrument and any(all(r['passed'] for r in records if r['arm']==f'only_{h}') for h in range(9)),pred_c_some_head_needed=instrument and any(not r['passed'] for r in records if r['arm'].startswith('drop_'))),seconds=time.perf_counter()-tic)
