"""DROP tailE (rung 190): the legacy span-dicts cost +0.14 and compress NOTHING.

CONVENTION (S2135): CE added above the real model; LOWER IS BETTER. tailE modifies only an 8-dim span of
the REAL tail MLPs' outputs - the full modules still run, so it saves no compute and ~no values, while
S2198 priced it at ~+0.14 census and S2237 showed it holds +0.55 of name's bill. COVERAGE NOTE, stated: the
config then replaces nothing of m10-17 - a claimed-coverage reduction the ledger records explicitly. Arm:
the rung-188 config with tailE dropped from the active list.

REGISTERED PREDICTIONS (anchors: rung-188 census +1.2673; name +2.204):
  (a) FREE RECOVERY: census <= 1.16.
  (b) NAME HEALS: name <= 2.05.
  (c) SANITY: census >= 0.5; inert tripwire vs 1.2673.
NULL: interactions make tailE non-additive here (recovery < 0.05). PRICE: negative (removes a component;
coverage note above). Self-reviewed."""
...
OUT=PT+'drop_taile_results.json'
...
if __name__=='__main__':
    ...
    SEL['cp_swap']=3456; SEL['qk_r']=16; SEL['qk_tail']=True; SEL['drop_tailE']=True
    ...
    pa=agg<=1.16
    pb=PC.get('name',9.9)<=2.05
    pc=agg>=0.5
    if abs(agg-1.2673)<1e-4:
        raise SystemExit('INSTRUMENT FAIL: tailE drop inert')
    ...
    print(f"(a) census {agg:+.4f} <= 1.16: {'HELD' if pa else 'FAILED'}")
    print(f"(b) name {PC.get('name',9.9):+.4f} <= 2.05: {'HELD' if pb else 'FAILED'}")
    print(f"(c) census >= 0.5: {'HELD' if pc else 'FAILED'}")
