...
endpoint readouts share a forward. Historical hash-bound runtimes are untouched.
"""
import torch
...
@torch.no_grad()
def measure(model,graph,rows,arms,write):
    ...
    def descendant(module,args,result):
        if state['arm']=='native':return result
        current,first=args
