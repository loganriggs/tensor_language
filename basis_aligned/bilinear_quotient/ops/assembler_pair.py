...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['context_memory_edge_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; b2-damage vs b3-damage co-variation on subword (assembler pair; damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'assembler_pair_results.json'
...
if __name__=='__main__':
    ...
    SEL['motif_off']=(2,)
    print('ARM 2/3: block-2 motifs REAL',flush=True)
    _=main(); cev_no2=SEL['cev']
    SEL['motif_off']=(3,)
    print('ARM 3/3: block-3 motifs REAL',flush=True)
