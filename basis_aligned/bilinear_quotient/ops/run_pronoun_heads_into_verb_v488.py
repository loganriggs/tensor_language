#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_pronoun_heads_write_little_into_the_verb pred_c_verb_heads_replay pred_d_union_adds pred_e_pronoun_heads_a_small_fraction
"""Do the pronoun heads write into the verb (v488)? v487: the verb heads write 0.041 into the pronoun slot against the pronoun heads' 0.803. The mirror at a
distant verb: the five pronoun heads' (9.6 / 12.4 / 10.5 / 15.1 / 10.1) writes swapped jointly, the five verb heads' writes (replay 0.557), and the union;
79 distant rows, agreement margin. v450 by their values at the noun: 0.072.
PREDICTIONS (scored as written; failures preserved; priors from v450 / v486)
    pred_a_baseline_replays                   the unedited manual forward replays the model's verb margins within 1e-3
    pred_b_pronoun_heads_write_little_into_the_verb  the five pronoun heads' joint write swap closes <= 0.12 of the agreement margin gap
    pred_c_verb_heads_replay                  the five verb heads' joint write swap closes 0.557 +- 0.03
    pred_d_union_adds                         the union closes within 0.04 of the sum of the two sets
    pred_e_pronoun_heads_a_small_fraction     the pronoun heads' joint effect is <= 0.25 x the verb heads' joint effect
...
OUT = ROOT / "circuits/followups/pronoun_heads_into_verb_v488_result.json"
CANDIDATE_ID = "chain.pronoun_heads_into_verb_v488"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, PRON_MAX, FIVE_V486, BAND, ADD_TOL, RATIO_MAX, UNIT = 1e-3, 0.12, 0.557, 0.03, 0.04, 0.25, 701
FIVE_V = {11: [3], 5: [3], 9: [7], 7: [8], 13: [1]}; FIVE_W = {9: [6], 10: [1, 5], 12: [4], 15: [1]}; UNION = {l: sorted(set(FIVE_W.get(l, [])) | set(FIVE_V.get(l, []))) for l in set(FIVE_W) | set(FIVE_V)}
import random
...
FORWARDS_MAX = 13
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_pronoun_heads_write_little_into_the_verb": "<= 0.12", "pred_c_verb_heads_replay": "0.557 +- 0.03", "pred_d_union_adds": "within 0.04 of the sum", "pred_e_pronoun_heads_a_small_fraction": "<= 0.25 x"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "pron_max": PRON_MAX, "five_v486": FIVE_V486, "band": BAND, "add_tol": ADD_TOL, "ratio_max": RATIO_MAX, "unit": UNIT}}
    ...
    MODES = {"FIVE_W": ({}, (0,)), "FIVE_V": ({}, (0,)), "UNION": ({}, (0,))}; SETS_W = {"FIVE_W": FIVE_W, "FIVE_V": FIVE_V, "UNION": UNION}; forwards = 0
    def logits_margin(x):
    ...
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_pronoun_heads_write_little_into_the_verb": abs(mc["FIVE_W"]) <= PRON_MAX, "pred_c_verb_heads_replay": abs(mc["FIVE_V"] - FIVE_V486) <= BAND,
                   "pred_d_union_adds": abs(mc["UNION"] - (mc["FIVE_V"] + mc["FIVE_W"])) <= ADD_TOL, "pred_e_pronoun_heads_a_small_fraction": abs(mc["FIVE_W"]) <= RATIO_MAX * mc["FIVE_V"]}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "pronoun_heads_into_verb_result_v488", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
