#!/usr/bin/env python
"""settled_frame_split_point_probe -- at k = 896 / 1024, the cost of "blocks 0..p-1 own read frames + ONE frame for blocks p..17"
over §2751's ALL36 program (22 own + one late-7 frame), for split p = 6 / 8 / 10 (§2753 measured +.022 at p = 8, k = 768). CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_split8_cheap_at_1024 pred_c_split8_cheap_at_896 pred_d_earlier_split_costs
#                     pred_e_settled_by_8
...
Preregistration: polynomial_causal/SETTLED_FRAME_SPLIT_POINT_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/settled_frame_split_point_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("settled_frame_split_point_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "SETTLED_FRAME_SPLIT_POINT_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "whole_model_width_program_probe_results.json"   # §2751
OUT = ROOT / "settled_frame_split_point_probe_results.json"
HASHES = {PREREG: "1115f80a3cb8cd42fcec25ca1ab96cc6d584da69cbaa62eec7bc0dbc47ad9cc1", PRIOR: "a3df6484be4f8973f7711b1f887be39e2d8ab0adceda5af51aadc2efdb98cafc",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "settled_frame_split_point_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_ALL36_1024 = 0.0337; PRIOR_ALL36_896 = 0.096; LAM = 1e-2
KS = [1024, 896]; EARLY = list(range(0, 11)); LATE = list(range(11, 18)); SPLITS = [6, 8, 10]
...
BARS = {"ce_tol": 1e-4, "repro_tol_1024": 0.015, "repro_tol_896": 0.02, "b_max": 0.010, "c_max": 0.015, "d_min": 0.003, "e_min": -0.003}
NULLS = {"b_min": 0.030, "c_min": 0.040, "d_max": 0.0, "e_max": -0.010}
...
def main():
    ...
    U_late = core_of(LSITES); U_split = {p: core_of(blocks(range(p, NL))) for p in SPLITS}
    arms = {}
    for k in KS:
        full = {s_: head(s_, k) for s_ in ESITES}; full.update({s_: head(s_, k, U_late) for s_ in LSITES}); arms[f"ALL36_{k}"] = run(full)
        for p in (SPLITS if k == 1024 else [8]):
            sp = {s_: head(s_, k) for s_ in blocks(range(0, p))}; sp.update({s_: head(s_, k, U_split[p]) for s_ in blocks(range(p, NL))})
            arms[f"SPLIT{p}_{k}"] = run(sp)
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    gaps = {"split8_minus_all36_1024": arms["SPLIT8_1024"] - arms["ALL36_1024"], "split8_minus_all36_896": arms["SPLIT8_896"] - arms["ALL36_896"],
            "split6_minus_split8_1024": arms["SPLIT6_1024"] - arms["SPLIT8_1024"], "split10_minus_split8_1024": arms["SPLIT10_1024"] - arms["SPLIT8_1024"]}
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(arms["ALL36_1024"] - PRIOR_ALL36_1024) <= BARS["repro_tol_1024"] and abs(arms["ALL36_896"] - PRIOR_ALL36_896) <= BARS["repro_tol_896"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_split8_cheap_at_1024': bool(gaps["split8_minus_all36_1024"] <= BARS["b_max"]),
        'pred_c_split8_cheap_at_896': bool(gaps["split8_minus_all36_896"] <= BARS["c_max"]),
        'pred_d_earlier_split_costs': bool(gaps["split6_minus_split8_1024"] >= BARS["d_min"]),
        'pred_e_settled_by_8': bool(gaps["split10_minus_split8_1024"] >= BARS["e_min"]),
    }
    nulls = {"b_null_ge_.03": bool(gaps["split8_minus_all36_1024"] >= NULLS["b_min"]), "c_null_ge_.04": bool(gaps["split8_minus_all36_896"] >= NULLS["c_min"]),
             "d_null_le_0": bool(gaps["split6_minus_split8_1024"] <= NULLS["d_max"]), "e_null_le_-.01": bool(gaps["split10_minus_split8_1024"] <= NULLS["e_max"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "ks": KS, "splits": SPLITS,
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "all36_1024": arms["ALL36_1024"], "prior_all36_1024": PRIOR_ALL36_1024, "all36_896": arms["ALL36_896"], "prior_all36_896": PRIOR_ALL36_896},
           "ce_added": arms, "gaps": gaps,
