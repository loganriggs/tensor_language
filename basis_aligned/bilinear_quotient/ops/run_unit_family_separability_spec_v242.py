...
"""v242: separability of the three v247 pronoun-cued gender cells under the FULL gender family -- the v206 fused gender circuit
(object_pronoun_gender him/her and possessive_gender his/her, four-row at v205; pronoun_gender he/she 1110 and reflexive_gender
himself/herself 1101 at v204; all king/queen LEXICAL-noun cued; v206 found the four mutually leaking 0.065-0.117 -> counted as ONE
circuit with four constructions) plus gender_possessive_his_her, gender_object_him_her, gender_possessive_pronoun_his_hers (v247;
he/she PRONOUN cue; the first two REUSE the incumbents' readout pairs his/her and him/her -- my quote-blind pair grep missed that,
23:52 ledger row). v240 tested the three new cells only against each other (within-batch) and counts nothing; this rung is the count.

Family here (ALL seven prepared as controls; EVAL = the three new cells + the two four-row incumbents as re-checks):
  gender: object_pronoun_gender (6 units, cext 0.989, v205), possessive_gender (7 units, cext 0.990, v205),
          pronoun_gender (3 units, v204), reflexive_gender (9 units, v204)   -- the fused incumbent, king/queen cue
        + gender_possessive_his_her (7 units 12:04/10:01/09:06/15:01/16:01; held 0.88, dim 0.543, cdas 0.669, C 0.0047, cext 0.993)
        + gender_object_him_her (9 units 12:04/10:01/09:06/13:01/15:01; held 0.90, dim 0.479, cdas 0.589, C 0.0004, cext 0.967)
        + gender_possessive_pronoun_his_hers (7 units same hubs; held 0.89, dim 0.639, cdas 0.726, C 0.0049, cext 0.992)
Question: is "the referent's gender" one direction whether the cue is a gendered NOUN (king/queen) or a PRONOUN (he/she)? The new cells
sit on the 12:04/10:01/09:06/15:01 hubs; the v205 incumbents' units are read from their receipts at run time.
Recipe as v198/.../v240: per evaluated member, arm `own` (own C_fit control) and arm `fam` (own C_fit + EVERY sibling's A1 FIT prep,
control_weight 30 x len(controls)); evaluated on the member's held A1 (extraction), each sibling's held A1 (|CE| leak) and held C.
Units frozen from the parent receipts (v247 for the new cells; v205 for the incumbents).
...
EVAL = 3 new + 2 incumbent re-checks (5 fits x 2 arms; 6 controls each: ~9 min).
Count rule (registered here, per-sibling CE read from the receipt by hand): a new cell separable from EVERY sibling counts +1; a new
cell that leaks (>0.05) into ANY king/queen construction joins the fused incumbent (+0, a fifth/sixth/seventh construction); new cells
that are separable from every king/queen construction but leak only into each other form ONE new fused circuit (+1 for the group).
An incumbent that comes out separable under the six controls is REPORTED (v206's lesson: more siblings constrain the direction harder,
and the verdict can flip) and goes to the board under proposals (v)/(vii); it is not re-scored here. 76 -> up to 79 from this rung.

REGISTERED BEFORE THE RUN (each coded predicate is the sentence here; bars in BARS):
    pred_a_new_separable      at least 1 of the 3 new cells is separable from every sibling (k_new). his_hers is the expected pass (unique
                              pair); his_her and him_her share pairs with the incumbents (v236: shared pair leaked 0.10/0.08; v226: shared
                              pair separated).                                                                              prior 50%
    pred_b_incumbents_fused   at least 1 of the 2 four-row incumbents is NOT separable under the six controls (k_fused; v206: all four
                              leaked into each other with one to three controls).                                          prior 75%
    pred_c_row4_kept          fam-arm own-C UB975 <= 0.01 on at least 2 of the 5 evaluated members (k_d; v240 pending, v238 9/9).  prior 65%
...
Smoke: V242_SMOKE=<out.json> (CPU, V242_SMOKE_ROWS=4; all members are prepared as controls, V242_SMOKE_NAMES=gender_possessive_pronoun_his_hers,possessive_gender
are evaluated so a new cell, an incumbent and the instrument read are exercised; steps 5).
"""
...
OUT = ROOT / "circuits/followups/unit_family_separability_spec_v242_result.json"
...
FAMILIES = {
    "gender": ["object_pronoun_gender", "possessive_gender", "pronoun_gender", "reflexive_gender", "gender_possessive_his_her", "gender_object_him_her", "gender_possessive_pronoun_his_hers"],
}
EVAL = ("gender_possessive_his_her", "gender_object_him_her", "gender_possessive_pronoun_his_hers", "object_pronoun_gender", "possessive_gender", )
COUNTED = ("object_pronoun_gender", "possessive_gender", )
...
BARS = {"cross_max": 0.05, "keep": 0.80, "ext_floor": 0.50, "c_ub_max": 0.01, "k_d": 2, "k_new": 1, "k_fused": 1, "instr_tol": 0.03}
MODEL_FORWARDS_MAX, EXAMPLE_EVALUATIONS_MAX = 20000, 640000


def _plan():
    n = len(EVAL)
    return {"candidate_id": "corpus.unit_family_separability_spec_v242", "members": n, "families": list(FAMILIES),
...
def PREDS(R):
    ...
    new_sep = sum(1 for n in NEW_MEMBERS if sep(n)); fused = sum(1 for n in COUNTED if n in good and not sep(n))
    a = ok and new_sep >= B["k_new"]
    b = ok and fused >= B["k_fused"]
    ...
    return {"pred_a_new_separable": bool(a), "pred_b_incumbents_fused": bool(b), "pred_c_row4_kept": bool(c), "pred_d_instrument": bool(d)}


def main() -> None:
    if os.environ.get("BQLIB_DRYRUN") or os.environ.get("BQLIB_NO_MODEL"):
        print(json.dumps(_plan(), indent=2, sort_keys=True)); return
    smoke = os.environ.get("V242_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V242_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    ...
    evaluate = [n for n in members if n in EVAL and (not smoke or n in os.environ.get("V242_SMOKE_NAMES", "gender_possessive_pronoun_his_hers,possessive_gender").split(","))]
    ...
    result = {"predictions": predictions, "schema": "unit_family_separability_spec_v242", "candidate_id": "corpus.unit_family_separability_spec_v242",
