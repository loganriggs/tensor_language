...
48 newly frozen irregular-noun sequences plus matched random site edits, eight prefix calls and twenty-four suffix calls, zero fits.
...
OUT=ROOT/'basis_aligned/bilinear_quotient/circuits/followups/subject_attention_freeze_v670_result.json'
...
def main():
    ...
    import hashlib
    rowfile=POLY/'SUBJECT_IRREGULAR_FRESH_V669_ROWS.json'
    assert hashlib.sha256(rowfile.read_bytes()).hexdigest()=='39e808a080116eb77c33eb90844b07db6a560384e4a378338b6e77977ee6cd83'
    ...
    from conditional_attention_mlp_runtime import execute as run_dag,select_attention_examples
    def cpu(value):
    ...
    for entries in batches:
        ...
        for name,sparse_runtime in sparse_runtimes.items():
            ...
            if name=='norm18':
                idx=torch.tensor([0,1],device=initial_all.device)
                aps=[select_attention_examples(ap,idx) for ap in context['all_attention']]
                mlpcs=[{k:v[idx] for k,v in c.items()} for c in context['all_mlp_contexts']]
                finalc={k:v[idx] for k,v in case['final_context'].items()}
                packed,packed_z=run_dag(sparse_runtime,aps,initial_all[idx],mlpcs,finalc,readpos[idx])
                export_checks.append(float((packed-logits[idx]).abs().max()))
                exported_cases.append(cpu(dict(families=[entries[i][3] for i in [0,1]],tokens=tokens[idx],
                    attention=aps,contexts=mlpcs,final_context=finalc,positions=readpos[idx],initial=initial_all[idx],
                    control_fixed=context['control_fixed'],control_context={k:v[idx] for k,v in context['control_context'].items()},
                    reference_logits=packed,reference_controls=modal[idx])))
    ...
    result=dict(created_utc=datetime.now(timezone.utc).isoformat(),plan=plan,counts=counts,cells=cells,
                ...
                scope='Portable conditional DAG export on48 opened irregular-noun rows, frozen joint-observable v665 bases and unchanged v661 execution; new modal and matched-random controls. Irregular noun pairs disjoint from calibration; four structural families. Historical vocabulary reuse not excluded; rows frozen before this run, no support refit. Eight template-number cells; no outcome-based selection. All attention12-17 and MLP12-17 responses dynamic in reduced full-sequence states. Native baseline context, first-value and initial post11 response retained as charged ports. No fresh generalization, selective removal or port-closed circuit claim.')
    assert max(export_checks)<=1e-8
    artifact=OUT.parent/'subject_conditional_dag_v670.pt'
    package_out=dict(runtime=cpu(sparse_runtimes['norm18']),cases=exported_cases,
        metadata=dict(scope='Conditional response DAG; native context and initial-response generators remain external',
                      rows=8,selection='first singular/plural row of each template; no outcome selection',
                      source_frames='subject_response_v665_program.pt',native_capability_gate=False,
                      max_export_replay_error=max(export_checks)))
    guard_torch_save(package_out,str(artifact),label='conditional attention MLP DAG export')
    result['export_artifact']=str(artifact.relative_to(ROOT));result['max_export_replay_error']=max(export_checks)
