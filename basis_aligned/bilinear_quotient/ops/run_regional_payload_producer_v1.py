#!/usr/bin/env python3
# BQGATE:7 body forwards;32 sequences lengths4-12;7 batched suffix arms;no fit;180sec.
"""pred_a compiled producer/child/all-donor replay<=1e-5;
pred_b all-payload transplant>=10%native cue and>=5/8positive EACHfamily;
pred_c MLP16-only transfer>=50%all-payload, unrelated<=.5regional EACHfamily.
Null: most cue payload may already exist in the preceding residual.
"""
...
STEM='REGIONAL_PAYLOAD_PRODUCER_V1'
@torch.no_grad()
def main():
    ...
    if os.environ.get('BQLIB_DRYRUN') or os.environ.get('BQLIB_NO_MODEL'):print('7 body forwards32sequences4-12tokens;7suffix arms;no fitting');return
    ...
    pre=torch.empty(32,1152,device='cuda');writes=torch.zeros(32,7,1152,dtype=torch.float64,device='cuda');replays=[];count=0
    fold=torch.load(P/'REGIONAL_PAYLOAD_MLP16_FOLD_V1_ARTIFACT.pt',weights_only=True,map_location='cpu');fold={k:v.cuda() for k,v in fold.items()};current_readers=fold['current_readers'];first_readers=fold['first_readers'];left16=w('transformer.h.16.mlp.Left.weight');right16=w('transformer.h.16.mlp.Right.weight');producer_errors=[];child_errors=[]
    handles=[model.transformer.h[0].attn.register_forward_pre_hook(lambda module,args:captured.update(first=args[0].detach())),model.transformer.h[16].mlp.register_forward_pre_hook(lambda module,args:captured.update(mlp_input=args[0].detach())),model.transformer.h[16].mlp.register_forward_hook(lambda module,args,out:captured.update(mlp_output=out.detach()))]
    try:
        for length,ids in sorted(buckets.items()):
            ...
            rho=(r.double().square().mean(-1,keepdim=True)+eps).sqrt();mi=captured['mlp_input'].double();mo=captured['mlp_output'].double();mlp_numerator=((mi@left16.T)*(mi@right16.T))@fold['folded_down'].T+fold['folded_bias'];native_mlp=fold['block17_lambdas'][0]*(mo@current_readers.T);producer_errors.append(float((mlp_numerator-native_mlp).norm()/native_mlp.norm()));pre_numerator=fold['block17_lambdas'][0]*((x.double()-mo)@current_readers.T);entry_numerator=fold['block17_lambdas'][1]*(x0.double()@current_readers.T)
            ...
            for pos in range(length):
                ...
                parts=[mlp_numerator[:,pos],pre_numerator[:,pos],entry_numerator[:,pos],rho[:,pos],first[:,pos]@first_readers.T]
                def child(parts):return (parts[0]+parts[1]+parts[2])/parts[3]+parts[4]
                child_errors.append(float((child(parts)-children).norm()/children.norm()))
                variants=[children]+[child([v[donor_indices] if j==changed else v for j,v in enumerate(parts)]) for changed in range(5)]+[child([v[donor_indices] for v in parts])]
    ...
    recipient_error=float((writes[:,0]-prior[:,0]).norm()/prior[:,0].norm());donor_error=float((writes[:,6]-prior[:,4]).norm()/prior[:,4].norm())
    arms=torch.stack([margins(writes[:,j]-writes[:,0]) for j in range(7)]);cells=[]
    for family in (0,1):
        ...
        cells.append(dict(family=family,native_regional_gap=float(native[:,0].mean()),mean_transfer_by_arm=transfer.mean(1).tolist(),positive_regional_by_arm=(transfer[:,:,0]>0).sum(1).tolist(),mlp_regional_meanabs=float(transfer[1,:,0].abs().mean()),mlp_unrelated_meanabs=float(transfer[1,:,1].abs().mean())))
    instrument=max(producer_errors+child_errors+[donor_error,recipient_error])<=1e-5;full=instrument and all(c['mean_transfer_by_arm'][6][0]>=.1*c['native_regional_gap'] and c['positive_regional_by_arm'][6]>=5 for c in cells)
    result={'pred_a':instrument,'pred_b':full,'pred_c':full and all(c['mean_transfer_by_arm'][1][0]>=.5*c['mean_transfer_by_arm'][6][0] and c['mlp_unrelated_meanabs']<=.5*c['mlp_regional_meanabs'] for c in cells)}
    torch.save(dict(pre=pre.cpu(),write_vertices=writes.cpu(),arm_margins=arms.cpu()),art)
    result.update(recipient_replay_error=recipient_error,all_payload_donor_replay_error=donor_error,compiled_producer_errors=producer_errors,child_decomposition_errors=child_errors,native_attention_replay_errors=replays,cells=cells,arm_names=['native','MLP16_numerator','pre_MLP16_numerator','embedding_reentry','RMS_divisor','first_stream','all_payload_ports'],body_forwards=count,seconds=time.perf_counter()-tic,artifact_sha=digest(art),source_shas=binding,scope='Conditional regional payload producer mediation; isolated numerator swaps retain recipient divisor. No independent upstream extraction or four-property promotion.')
