...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    _need=['m16_attn16_coupling_results.json','empirical_L2_results.json']
    _miss=[f for f in _need if not os.path.exists(_bq+f)]
    if _miss:
        print(f'DRYRUN FAIL: missing {_miss}'); raise SystemExit(1)
    _p=json.load(open(_bq+'empirical_L2_results.json'))
    print(f"DRYRUN OK: published L2_F {_p['L2_F']}; per-class CE attribution of the block-5 motif replacement on FR (damage convention)")
    raise SystemExit(0)
import torch
...
OUT=PT+'attn5_motif_class_results.json'
...
if __name__=='__main__':
    ...
    print('ARM 1/2: full S2144 config (motifs at 2-9), per-position CE captured',flush=True)
    r_full=main()
    cev_full=SEL['cev']; clsflat=SEL['clsflat']
    SEL['motif_off']=(5,)
    print('ARM 2/2: block-5 motifs REAL (attn5 untouched), per-position CE captured',flush=True)
    r_no5=main()
    cev_no5=SEL['cev']
    import statistics as stt
    if float((cev_full-cev_no5).abs().sum())<1e-3:
        raise SystemExit('INSTRUMENT FAIL: arms identical - motif_off inert')
    diff=(cev_full-cev_no5)
    tot=float(diff.sum())
    shares={}
    for k in range(10):
        selk=clsflat==k
        shares[k]=float(diff[selk].sum())/max(abs(tot),1e-9)
    STRUCT=[CLS.index(n) for n in ('newline','sentend','comma','bclose')]
    struct_share=sum(shares[k] for k in STRUCT)
    pa=shares[CLS.index('ind')]<=0.30
    pb=struct_share>=0.30
    pc=abs(r_full['L2_F']-2.6662)<=0.01
    res={'full':r_full,'block5_motifs_real':r_no5,'total_damage_sum':round(tot,2),
         'class_shares':{CLS[k]:round(shares[k],4) for k in range(10)},
         'struct_combined_share':round(struct_share,4),
         'convention':'per-position CE(full) - CE(block-5 motifs real) on FR; positive = the replacement hurts there',
         'pred_a_not_retrieval':bool(pa),'pred_b_structural':bool(pb),'pred_c_reproduces_s2144':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f'total {tot:+.1f}; class shares: '+' '.join(f'{CLS[k]}:{shares[k]:+.3f}' for k in range(10)))
    print(f"(a) ind share {shares[CLS.index('ind')]:+.3f} <= 0.30: {'HELD' if pa else 'FAILED'}")
    print(f"(b) structural combined {struct_share:+.3f} >= 0.30: {'HELD' if pb else 'FAILED'}")
