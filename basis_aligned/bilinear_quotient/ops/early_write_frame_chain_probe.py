#!/usr/bin/env python
"""early_write_frame_chain_probe -- is the early program a CHAIN of frames: does each early MLP write land in the NEXT site's read
frame (§2759: MLP writes carry the whole frame rotation), with the remainder outside that frame readout-bound or dead, and do
attention writes sit inside the next frame more fully than MLP writes? Write routing as §2756 (delete / readout). CUDA lane-1.

# BQGATE: EXPERIMENT  pred_a_instrument pred_b_next_frame_adopts_the_write pred_c_next_frame_is_the_writes_home
#                     pred_d_early_chain_with_readout_is_cheap pred_e_attention_writes_sit_inside_the_next_frame
...
Preregistration: polynomial_causal/EARLY_WRITE_FRAME_CHAIN_PROBE_PREREGISTRATION.md
"""
...
SELF = Path(os.path.abspath(sys.argv[0])) if sys.argv and sys.argv[0].endswith(".py") else Path("/workspace/tensor_language/basis_aligned/bilinear_quotient/ops/early_write_frame_chain_probe.py")
sys.path.insert(0, str(SELF.parent))
import mlp_in_situ_usage_rank_map_probe as R

if not torch.cuda.is_available():
    raise RuntimeError("early_write_frame_chain_probe is a lane-1 CUDA script; refusing to fall back to CPU")
DEV = torch.device("cuda")
ROOT = R.ROOT
PREREG = R.POLY / "EARLY_WRITE_FRAME_CHAIN_PROBE_PREREGISTRATION.md"
PRIOR = ROOT / "block_boundary_blend_rotation_probe_results.json"   # §2759
OUT = ROOT / "early_write_frame_chain_probe_results.json"
HASHES = {PREREG: "358fc13d4711b8b33fe17bfb42b86602eab7867250333462c075266d45d151fa", PRIOR: "403dbd6f9412cfcbbeeb7aef6c727203e6cc953d4141cc212840e7679f672606",
          R.BLOB: "680d6c26cf05af2e9b5eaac1d52fa1c9e4ea443f60a7c74ad211740e317d6de3",
          R.NAT: "666a32015c8ab3dcbabca4a859f5a0c8a3e1b9b9cc8f0b7f7c9e5211d903e2a1"}
RUNG = "early_write_frame_chain_probe"
D, NH, HD, NL, V = R.D, R.NH, R.HD, R.NL, R.V
TI = 256; FIT = (96, 192); EVAL = (0, 64); CH = 8
KM = 16
LATE7 = [("mlp", l) for l in range(11, 18)]; LAST2 = [("mlp", 16), ("mlp", 17)]
PRIOR_BASE = 3.0322401; PRIOR_EARLY22_OWN_768 = 0.057; LAM = 1e-2
K = 768; SPLIT = 8; EARLY = list(range(0, 11)); LATE = list(range(11, 18))
...
BARS = {"ce_tol": 1e-4, "repro_tol": 0.02, "b_min": 1.5, "c_max": 0.5, "d_max": 0.040, "e_max": 0.5}
NULLS = {"b_max": 1.1, "c_min": 0.9, "d_min": 0.120, "e_min": 1.0}
...
def main():
    ...
    def oe(s_, U_):
        C = wheads[s_]["Cw"]; Uk = U_[:, :K].to(C.dtype); return float(1.0 - torch.trace(Uk.T @ C @ Uk) / torch.trace(C))
    NB = 8
    oe_own = [oe(("mlp", l), heads[("mlp", l)]["U"]) for l in range(NB)]
    oe_next = [oe(("mlp", l), heads[("attn", l + 1)]["U"]) for l in range(NB)]
    oe_attn_next = [oe(("attn", l), heads[("mlp", l)]["U"]) for l in range(NB)]
    ratio_oe = torch.tensor([oe_own[l] / max(oe_next[l], 1e-9) for l in range(NB)]); ratio_med = float(ratio_oe.median())
    all_gt = bool(all(oe_own[l] > oe_next[l] for l in range(NB)))
    med_attn = float(torch.tensor(oe_attn_next).median()); med_mlp = float(torch.tensor(oe_next).median())
    log(stage="geom", oe_own=[round(v, 4) for v in oe_own], oe_next=[round(v, 4) for v in oe_next], oe_attn_next=[round(v, 4) for v in oe_attn_next], ratio_med=round(ratio_med, 3))
    base = lambda: {s_: head(s_, K) for s_ in ESITES}
    def with_routes(kd, frame_of, mode):
        patch = base(); buf = Buf()
        for l in range(NB):
            s_ = (kd, l); patch[s_] = Both(head(s_, K), Route(s_, K, frame_of(l), mode, buf))
        if mode == "readout":
            patch[FINAL] = FinalAdd(buf, +1.0)
        return run(patch, (buf,))
    arms = {"EARLY22_OWN_768": run(base())}
    arms["MLPW_OWNFRAME_DEL_768"] = with_routes("mlp", lambda l: heads[("mlp", l)]["U"], "delete")
    arms["MLPW_NEXTFRAME_DEL_768"] = with_routes("mlp", lambda l: heads[("attn", l + 1)]["U"], "delete")
    arms["MLPW_NEXTFRAME_RO_768"] = with_routes("mlp", lambda l: heads[("attn", l + 1)]["U"], "readout")
    arms["ATTNW_NEXTFRAME_DEL_768"] = with_routes("attn", lambda l: heads[("mlp", l)]["U"], "delete")
    log(stage="arms", **{k: round(v, 4) for k, v in arms.items()})
    own = arms["EARLY22_OWN_768"]
    gaps = {"mlpw_ownframe_del_minus_own": arms["MLPW_OWNFRAME_DEL_768"] - own, "mlpw_nextframe_del_minus_own": arms["MLPW_NEXTFRAME_DEL_768"] - own,
            "mlpw_nextframe_ro_minus_own": arms["MLPW_NEXTFRAME_RO_768"] - own, "attnw_nextframe_del_minus_own": arms["ATTNW_NEXTFRAME_DEL_768"] - own,
            "oe_own_over_next_median": ratio_med, "oe_next_median_mlp": med_mlp, "oe_next_median_attn": med_attn}
    c_ratio = gaps["mlpw_nextframe_del_minus_own"] / gaps["mlpw_ownframe_del_minus_own"] if gaps["mlpw_ownframe_del_minus_own"] > 0 else None
    e_ratio = gaps["attnw_nextframe_del_minus_own"] / gaps["mlpw_nextframe_del_minus_own"] if gaps["mlpw_nextframe_del_minus_own"] > 0 else None
    inst_ok = bool(abs(ce0 - PRIOR_BASE) <= BARS["ce_tol"] and abs(own - PRIOR_EARLY22_OWN_768) <= BARS["repro_tol"]) if not smoke else True
    preds = {
        'pred_a_instrument': bool(inst_ok),
        'pred_b_next_frame_adopts_the_write': bool(all_gt and ratio_med >= BARS["b_min"]),
        'pred_c_next_frame_is_the_writes_home': bool(c_ratio is not None and c_ratio <= BARS["c_max"]),
        'pred_d_early_chain_with_readout_is_cheap': bool(gaps["mlpw_nextframe_ro_minus_own"] <= BARS["d_max"]),
        'pred_e_attention_writes_sit_inside_the_next_frame': bool(e_ratio is not None and e_ratio <= BARS["e_max"] and med_attn <= med_mlp),
    }
    nulls = {"b_null_ratio_le_1.1": bool(ratio_med <= NULLS["b_max"]), "c_null_ge_.9": bool(c_ratio is not None and c_ratio >= NULLS["c_min"]),
             "d_null_ge_.12": bool(gaps["mlpw_nextframe_ro_minus_own"] >= NULLS["d_min"]), "e_null_ge_1": bool(e_ratio is not None and e_ratio >= NULLS["e_min"])}
    out = {"rung": RUNG, "status": "complete", "smoke": smoke,
           ...
           "preds": preds, "nulls": nulls, "bars": BARS, "null_bars": NULLS, "device": "cuda", "k": K, "n_blocks": NB,
           "instrument": {"baseline_ce": ce0, "prior_baseline": PRIOR_BASE, "early22_own_768": own, "prior_early22_own_768": PRIOR_EARLY22_OWN_768},
           "ce_added": arms, "gaps": gaps, "c_ratio": c_ratio, "e_ratio": e_ratio,
           "out_of_frame_energy": {"mlp_vs_own_frame": oe_own, "mlp_vs_next_frame": oe_next, "attn_vs_next_frame": oe_attn_next},
