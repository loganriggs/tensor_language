"""PAIRWISE INTERACTION EXPANSION OF THE CP FRONT (rung 118): does order-2 composition calculus predict configs?

CONVENTION (S2135): per-position dCE = CE(config) - CE(real model) on the census rows; LOWER IS BETTER.
Math frame (review 0707): CE(f + sum_i delta_i) - CE(f) ~ sum_i d_i + (1/2) sum_{i!=j} delta_i^T H delta_j.
S2209 showed the delta_i are nearly L2-orthogonal but CE is superadditive - so the cross terms live in the
Fisher/GN metric H, and J_ij = agg(pair_ij) - single_i - single_j measures them directly. If order-2 suffices,
excess(quad) = sum_{i<j} J_ij and EVERY one of the 2^4 front subsets becomes predictable before running - a
composition calculus for the program. The S2207 ladder gives the row sums for free (J_01 = .0676;
J_02+J_12 = .2395; J_03+J_13+J_23 = .3568); this rung measures the six J_ij individually.

ARMS: six pairs {cf_i,cf_j} installed alone + quad {cf0,cf1,cf2,cf3} alone, mixed Ks (2304/2304/1152/576).
Singles (receipts, alone-frame): m0 +0.0647, m1 +0.0454, m2 +0.0504, m3 +0.0745.

REGISTERED PREDICTIONS:
  (a) ORDER-2 SUFFICES: |excess(quad) - sum J_ij| <= 0.25 x excess(quad), excess(quad) = agg(quad) - sum singles.
  (b) LATE-PAIR DOMINANCE: J_23 is the largest J and >= 0.35 x sum J (the S2207 ladder shape).
  (c) SANITY: each pair agg >= max(its two singles) - 0.015 and <= agg(quad) + 0.015; agg(quad) in [0.7, 1.1].
NULL: three-way-and-higher terms dominate (order-2 fails; the pair calculus dies; composition needs the full
lattice). PRICE: none (attribution). Tripwire: INSTRUMENT FAIL if pair arms identical (pstdev < 0.005).
Self-reviewed."""
...
if os.environ.get('BQLIB_DRYRUN')=='1':
    ...
    print('DRYRUN OK: pairwise interaction expansion')
    raise SystemExit(0)
import torch
...
OUT=PT+'frontier_cp_pairs_results.json'
...
def main():
    ...
    if SEL.get('cp_pairs'):
        ...
        for _i,_j in _it.combinations((0,1,2,3),2):
            _arms[f'pair_{_i}{_j}']=evalV(_ROWS,_ROWS.shape[0],[f'cf{_i}',f'cf{_j}'],ML).detach().cpu()
            print(f'  pair ({_i},{_j}) done',flush=True)
...
if __name__=='__main__':
    ...
    SEL['cp_pairs']=True; SEL['ext_rows']=CROWS
    print('ARMS: six CP pairs + quad, mixed Ks - census rows',flush=True)
    main()
    A=SEL['front_result']
    agg={}
    for k,cev in A.items():
        ...
        print(f"  {k:9s} agg {agg[k]:+.4f}",flush=True)
    import statistics as stt, itertools as it
    pairs=list(it.combinations((0,1,2,3),2))
    if stt.pstdev([agg[f'pair_{i}{j}'] for i,j in pairs])<0.005:
        raise SystemExit('INSTRUMENT FAIL: pair arms identical')
    J={f'{i}{j}':round(agg[f'pair_{i}{j}']-SGL[i]-SGL[j],4) for i,j in pairs}
    sumJ=round(sum(J.values()),4)
    exq=round(agg['quad']-sum(SGL.values()),4)
    for k,v in J.items(): print(f'  J_{k} = {v:+.4f}')
    print(f'sum J {sumJ:+.4f} vs quad excess {exq:+.4f}')
    pa=abs(exq-sumJ)<=0.25*exq
    pb=(J['23']==max(J.values())) and J['23']>=0.35*sumJ
    ok=True
    for i,j in pairs:
        ok&=agg[f'pair_{i}{j}']>=max(SGL[i],SGL[j])-0.015 and agg[f'pair_{i}{j}']<=agg['quad']+0.015
    pc=ok and 0.7<=agg['quad']<=1.1
    res={'aggs':agg,'singles':SGL,'J':J,'sum_J':sumJ,'quad_excess':exq,
         'convention':'per-position dCE = CE(config) - CE(real model) on census rows; lower is better',
         'pred_a_order2_suffices':bool(pa),'pred_b_late_pair_dominance':bool(pb),'pred_c_sane':bool(pc),
         'self_reviewed':True,'runtime_s':round(time.time()-T00,1)}
    json.dump(res,open(OUT,'w'),indent=1)
    print(f"(a) |{exq:+.4f} - {sumJ:+.4f}| <= 0.25 x excess ({0.25*exq:+.4f}): {'HELD' if pa else 'FAILED'}")
    print(f"(b) J_23 {J['23']:+.4f} max and >= 0.35 x sumJ ({0.35*sumJ:+.4f}): {'HELD' if pb else 'FAILED'}")
    print(f"(c) pair sanity + quad in [0.7, 1.1]: {'HELD' if pc else 'FAILED'}")
