#!/usr/bin/env python3
# BQGATE: EXPERIMENT pred_a_baseline_replays pred_b_5_6_small_on_text pred_c_11_1_small_on_text pred_d_relays_together_small_on_text pred_e_5_3_replays_on_text
"""The panel's small relays on natural text (v537). v536: on the panel heads 5.6 (0.028) and 11.1 (0.032) join 5.3 (0.040) as small writers into the pronoun slot.
Here their writes into the NATURAL pronoun answer (122 pairs): are they panel-specific relays, like the MLP-17 late population (v517), or a small permanent part
of the channel?
PREDICTIONS (scored as written; failures preserved; priors from v481 / v482 / v536)
    pred_a_baseline_replays          the unedited manual forward replays the model's they - he margins within 1e-3
    pred_b_5_6_small_on_text         5.6's write into the natural pronoun answer closes <= 0.03 of the margin gap (v482: 0.014)
    pred_c_11_1_small_on_text        11.1's write closes <= 0.03 (block 11 as a whole: 0.027, v481)
    pred_d_relays_together_small_on_text  5.3 + 5.6 + 11.1 (three single effects summed) <= 0.07 on text (panel: 0.100)
    pred_e_5_3_replays_on_text       5.3's write closes 0.021 +- 0.01 (v482)
PRICE (registered maximum): 4 text batches x 4 passes = 16 forwards; 0 backwards; 0 fits. Bar <= 17.
"""
...
OUT = ROOT / "circuits/followups/panel_relays_on_text_v537_result.json"
CANDIDATE_ID = "chain.panel_relays_on_text_v537"
N_HEAD = 9
READERS = ((9, 6), (12, 4), (15, 1)); FIVE = {9: [6], 10: [1, 5], 12: [4], 15: [1]}
REPLAY_TOL, SMALL_MAX, TOGETHER_MAX, H53_V482, BAND, UNIT = 1e-3, 0.03, 0.07, 0.021, 0.01, 701
import random
...
FORWARDS_MAX = 17
PREDICTIONS = {"pred_a_baseline_replays": "<= 1e-3", "pred_b_5_6_small_on_text": "<= 0.03", "pred_c_11_1_small_on_text": "<= 0.03", "pred_d_relays_together_small_on_text": "<= 0.07", "pred_e_5_3_replays_on_text": "0.021 +- 0.01"}
...
def main() -> None:
    ...
    plan = {"candidate_id": CANDIDATE_ID, "panel_pairs": len(panel_items), "text_pairs": len(text_items), "readers": [f"{l}.{h}" for l, h in READERS], "forwards_max": FORWARDS_MAX, "model_backwards": 0, "model_updates": 0,
            "fit_parameters": 0, "gpu_accessed": False, "model_loaded": False, "execution_policy": "managed_queue_only", "bars": {"replay_tol": REPLAY_TOL, "small_max": SMALL_MAX, "together_max": TOGETHER_MAX, "h53_v482": H53_V482, "band": BAND, "unit": UNIT}}
    ...
    MODES = {"5.3": ({}, (0,)), "5.6": ({}, (0,)), "11.1": ({}, (0,))}; forwards = 0
    def logits_margin(x):
    ...
    mc = verb["margin_closed"]; report["relays_sum"] = mc["5.3"] + mc["5.6"] + mc["11.1"]
    predictions = {"pred_a_baseline_replays": replay <= REPLAY_TOL, "pred_b_5_6_small_on_text": abs(mc["5.6"]) <= SMALL_MAX, "pred_c_11_1_small_on_text": abs(mc["11.1"]) <= SMALL_MAX,
                   "pred_d_relays_together_small_on_text": abs(report["relays_sum"]) <= TOGETHER_MAX, "pred_e_5_3_replays_on_text": abs(mc["5.3"] - H53_V482) <= BAND}
    if forwards > FORWARDS_MAX:
        raise SystemExit(f"price exceeded: {forwards} > {FORWARDS_MAX}")
    OUT.write_text(json.dumps({"schema": "panel_relays_on_text_result_v537", "candidate_id": CANDIDATE_ID, "plan": plan, "report": report, "predictions": predictions, "forwards": forwards,
