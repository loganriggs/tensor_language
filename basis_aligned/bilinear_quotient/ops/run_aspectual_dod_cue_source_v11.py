...
"""Aspectual has/had definition-of-done battery, step 11: head 8.1 as a token-only cue reader (EDIT).
...
PRICE (registered maximum): 2 batches x (native + producer + recompute-capture 1 + 8 arms) = 22
forwards; 0 backwards; 0 fits. Bar <= 30.
"""
...
OUT = ROOT / "circuits/followups/aspectual_anchor_dod_cue_source_v11_result.json"
CANDIDATE_ID = "aspectual_anchor.has_vs_had.dod_cue_source_v11"
...
def main() -> None:
    ...
    result = {"schema": "aspectual_anchor_dod_cue_source_result_v11", "candidate_id": CANDIDATE_ID, "plan": _plan(rows),
