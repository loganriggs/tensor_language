...
"""v641: do the six separable possessive candidates survive the REGISTERED fit objective?
...
Smoke: V641_SMOKE=<out.json> (CPU, V641_SMOKE_ROWS=4, V641_SMOKE_NAMES=<cell>).
"""
...
OUT = ROOT / "circuits/followups/unit_correlative_registered_objective_v641_result.json"
...
NAMES = {"correlative_disjoint_either_not": "correlative_disjoint_either_not",
         "correlative_or_and": "correlative_or_and",
         "correlative_open_neu": "correlative_open_neu",
         "correlative_pair": "correlative_pair"}
ANCHOR = "correlative_pair"
SIBLINGS = ['correlative_disjoint_either_not', 'correlative_or_and', 'correlative_open_neu']
V641_ANCHOR = None
# v639, weekly fit: disjoint_either_not and or_and were two-control selective and cleared four rows;
# open_neu cleared four rows but failed the weekly control; correlative_pair is the reproduction anchor
...
def _plan():
    return {"candidate_id": "corpus.unit_correlative_registered_objective_v641", "behaviours": 9, "constructions": 5,
...
def main() -> None:
    ...
    smoke = os.environ.get("V641_SMOKE")
    backend = producer.Bilin18TorchBackend.load("cpu" if smoke else "cuda")
    torch = backend.torch
    t0 = time.perf_counter()
    cut = (lambda rows: rows[:int(os.environ.get("V641_SMOKE_ROWS", "4"))]) if smoke else (lambda rows: rows)
    pool, max_units, steps = (3, 3, 5) if smoke else (POOL, MAX_UNITS, STEPS)
    which = [n for n in NAMES if not smoke or n in os.environ.get("V641_SMOKE_NAMES", "verb_preposition_against_for,finiteness_selection").split(",")]
    ...
    predictions = PREDS(R)
    result = {"predictions": predictions, "schema": "unit_correlative_registered_objective_v641", "candidate_id": "corpus.unit_correlative_registered_objective_v641", "bars": BARS,
