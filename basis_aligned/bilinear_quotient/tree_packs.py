"""TREE PACKS -- rerun the v2 tree, additionally saving per-leaf naming
packs: 20 member tokens each with +/-10-token context windows, member
counts, probe lists. Output: circuit_tree_packs.json, one pack per
replicated leaf, for the stateless naming wave.
Same registered bars as v2 (should reproduce 68 +/- a few; the run is
deterministic up to GPU nondeterminism):
  (a) >= 40 leaves; (b) >= half taxonomy-invisible; (c) descendant
  replication >= 0.5; (d) every leaf pack has >= 12 context examples."""
...
OUT=PT+'tree_packs_results.json'
CA=300; NB=53
MHL=list(range(2,10))
T=256
