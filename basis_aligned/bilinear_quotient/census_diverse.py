...
REGISTERED PREDICTIONS:
  (a) >=100 leaves at 5x diverse corpus;
  (b) DIVERSITY REFLECTED: median leaf's top-single-document member
      share <=50% (the old guide-dominance broken);
  (c) NEW-IDENTITY-RULE leg: split the corpus into halves A/B;
      for >=60% of A-leaves, the machinery damage-profile (its 4
      probes' member-mean dCE vector) replicates on B (cosine of
      the 4-vector >=0.7 on B-members selected by A's program) --
      deferred to a follow-up if runtime forces it; minimum: state
      + per-leaf top-document shares saved."""
...
OUT=PT+'census_diverse_results.json'
CA=300; NB=250
MHL=list(range(2,10))
T=256
...
def main():
    ...
    for ex in dsc:
        ...
        for s0 in range(0,len(tk)-513,513):
            row=tk[s0:s0+513]
            if tuple(row[:32]) in seenc: continue
            rowsl.append(row); docid.append(di); got+=1
            if got>=2 or len(rowsl)>=NB*4: break
        if len(rowsl)>=NB*4: break
    rows=torch.tensor(rowsl,dtype=torch.long)
    docid=torch.tensor(docid)
    torch.save({'rows':rows,'docid':docid},PT+'curated_rows.pt')
    print(f'curated {rows.shape[0]} rows from '
          f'{len(set(docid.tolist()))} documents',flush=True)
    ...
    torch.save({'rows':rows,'basev':base.cpu(),
               ...
               PT+'census_state_diverse.pt')
    for lf in leaves:
        for k_ in ('_member','_score','_slice'): lf.pop(k_,None)
    for md in roots: md.pop('_member',None)
    json.dump(packs,open(PT+'circuit_tree_packs_diverse.json','w'),indent=1)
    ...
    print(f'\nleaves {nleaf} | new-structure {nnew} | child repl rate '
          f'{crate:.2f}')
    pa=nleaf>=100
    print(f"(a) >=100 leaves: {'HELD' if pa else 'FAILED'}")
    # (b) per-leaf top-document member share
    st9=torch.load(PT+'census_state_diverse.pt',
                   map_location='cpu')['leaves']
    dflat=docid[:,None].expand(-1,256).reshape(-1)
    shares=[]
    for lf9 in st9:
        dm9=dflat[lf9['member']]
        u,c=dm9.unique(return_counts=True)
        shares.append(float(c.max())/max(len(lf9['member']),1))
    shares.sort()
    med=shares[len(shares)//2] if shares else 1.0
    pb=med<=0.5
    out['median_top_doc_share']=round(med,3)
    out['doc_share_q']=[round(shares[int(q*len(shares))],3)
                        for q in (0.1,0.5,0.9)] if shares else []
    print(f'median top-document share: {med:.0%}')
    print(f"(b) median <=50%: {'HELD' if pb else 'FAILED'}")
