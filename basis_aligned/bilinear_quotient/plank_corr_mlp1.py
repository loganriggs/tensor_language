# plank_corr_mlp1: TRAINED CORRECTION FOR THE mlp1 PLANK (the S1489 move —
# the one method that has been reliably additive — applied to the board's top
# items). Stand-in = tier-2000 token table + ridge on [attn1, mlp0] + a TRAINABLE
# rank-64 correction on the same inputs, trained vs full-model CE with ONLY
# mlp1 replaced (solo context, everything else live). 400 steps, Adam 3e-3,
# correction init zero (the regime where CE-training works, S1533).
#
# Registered predictions:
#   pred_a corrected fid_opt >= 0.980 (plank alone: .9752 unpriced / .9507 priced).
#   pred_b held-out CE gain from the correction >= 0.03.
...
OUT = PT + 'plank_corr_mlp1_results.json'
