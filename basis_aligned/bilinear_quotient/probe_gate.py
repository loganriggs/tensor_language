"""PROBE GATE -- deploy-legal gating rung between surface programs
(1.5x random, deploy_gated2) and oracle causal labels (9.4x, 337).
The oracle gate needs 36 ablation sweeps on the eval data itself; a
ridge probe reading the residual stream after block 2 (the cheap
lexical rungs every frontier config keeps real) is available at
inference. Fit probe on fit-window oracle labels, gate fresh rows at
the oracle-matched fraction, doc-disjoint throughout.
REGISTERED PREDICTIONS (fresh):
  (a) probe AUC vs oracle labels >= 0.75;
  (b) probe-gated efficiency >= 2.5x random at matched fraction;
  (c) probe recovers >= 50% of the oracle gain at the same fraction.
Caveat on record: reading the stream costs keeping blocks 0-2
computed; those are the assembly's cheapest, most exact rungs."""
...
def main():
    ...
    XA=stream_rows(rowsA,nA)
    yA=gateA.float()*2-1
    lamp=1e-2*len(XA)
    Wp=torch.linalg.solve(XA.T@XA+lamp*torch.eye(D,device=DEV),
                          XA.T@yA)
    del XA
    XF=stream_rows(FR,120)
