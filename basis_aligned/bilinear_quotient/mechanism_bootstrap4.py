"""MECHANISM BOOTSTRAP v4 -- extend the bigram fold to layers 0-9 so
middle-MLP leaves get their mechanism rung measured (331: the bigram
rung certifies at 33-36x precision when expressible; only front layers
were folded).
REGISTERED PREDICTIONS:
  (a) >=6 of tested leaves (mlp-pca probes at layers <=9, >=2 of them)
      pass precision >=3x base at recall >=0.3;
  (b) random corpus-pair null <=1.2x (median);
  (c) pass-rate by probe depth reported (front 0-3 vs middle 4-9)."""
...
OUT=PT+'mechanism_bootstrap4_results.json'
CA=300; NB=78
MHL=list(range(2,10))
T=256
...
def main():
    ...
    pa=npass>=6
    fr_=[r for r in res if all(int(p.split(chr(39))[3][1:])<=3
         for p in [str(x) for x in []])] if False else None
    ...
    print(f"(a) >=6 pass: {'HELD' if pa else 'FAILED'}")
