# d2_circuit: THE FIRST CIRCUIT THROUGH THE COMPRESSION (user directive 2026-08-26:
...
#   pred_a EXTRACTION keeps >= .50 of mlp0's frag-class effect while keeping <= .30
#          of its global effect (selective retention = a circuit, not a blob).
#   pred_b REMOVAL damages frag-class >= 3x its global damage.
#   pred_c GENERALIZATION: all class-conditional effects replicate on skip=2000 with
#          the same sign and >= half the magnitude.
...
OUT = PT + 'd2_circuit_results.json'
NR = 960
...
def main():
    ...
    _, _, Vt = torch.linalg.svd(PATC * rms.unsqueeze(0), full_matrices=False)
    d2 = Vt[1]                                          # D2 axis (S1470 pattern dir1)
    U_OUT = (H[0].mlp.Down.weight.float().to(DEV) @ (d2 * rms))   # [D] output image
    ...
    # mean mlp0 OUTPUT (for the extraction arm's base)
    ...
    def fwd_circ(idx, mode):
        ...
        for L, blk in enumerate(H):
            ...
            if L == 0 and mode is not None:
                h0 = (blk.mlp.Left(z).float() * blk.mlp.Right(z).float())
                s = ((h0 - MU0['mu']) / rms) @ d2                  # [B, T]
                mo_full = blk.mlp(z).float()
                axis_part = s.unsqueeze(-1) * U_OUT
                if mode == 'removal':
                    mo = mo_full - axis_part
                else:
                    mo = MEAN_OUT.expand(B, T, D) + axis_part
                x = x + mo.to(x.dtype)
            else:
                x = x + blk.mlp(z)
        return 30.0 * torch.tanh(m.lm_head(F.rms_norm(x, (D,))) / 30.0)

    def ce_run2(rows, mode):
        s_ = 0.0; n_ = 0; sf = 0.0; nf = 0; sd = 0.0; nd = 0
        NRr = rows.shape[0]
        for i in range(0, NRr, 8):
            bb = rows[i:i + 8].to(DEV)
            idx = bb[:, :-1].contiguous(); tg = bb[:, 1:].contiguous()
            lo = fwd_circ(idx, mode).float()
    ...
    for setname, rows in (('s7000', EVR), ('s2000', EV2)):
        res[setname] = {}
        for mode in (None, 'removal', 'extract', 'mean_only'):
            md = mode
            if mode == 'mean_only':
                # extraction baseline: mean out, NO axis (what does the axis add?)
                # implemented as extract with s zeroed via a flag: use removal of
                # everything = mean; reuse fwd_circ 'extract' minus axis: cheap hack:
                continue
            r_ = ce_run2(rows, md)
            res[setname][str(mode)] = {k: round(v, 4) for k, v in r_.items()}
            print(setname, mode, res[setname][str(mode)], flush=True)
            json.dump({'partial': True, 'res': res}, open(OUT, 'w'), indent=1)

    # mean-ablation reference for "mlp0's effect" per class (from anchors: use
    # mean-out arm) — run it on both sets
    @torch.no_grad()
    def fwd_meanout(idx):
        x = F.rms_norm(m.transformer.wte(idx), (D,)); x0 = x; v1 = None
        B = idx.shape[0]
        for L, blk in enumerate(H):
            ...
            z = F.rms_norm(x, (D,))
            if L == 0:
                x = x + MEAN_OUT.expand(B, T, D).to(x.dtype)
            else:
                x = x + blk.mlp(z)
        return 30.0 * torch.tanh(m.lm_head(F.rms_norm(x, (D,))) / 30.0)
    ...
    for setname, rows in (('s7000', EVR), ('s2000', EV2)):
        ...
        for i in range(0, rows.shape[0], 8):
            ...
            sf += float(ce[isf].sum()); nf += int(isf.sum())
            sd += float(ce[isd].sum()); nd += int(isd.sum())
        res[setname]['mean_out'] = {'global': round(s_ / max(n_, 1), 4),
                                    'frag': round(sf / max(nf, 1), 4),
                                    'det': round(sd / max(nd, 1), 4)}
        print(setname, 'mean_out', res[setname]['mean_out'], flush=True)

    def scores(setname):
        c = res[setname]['None']; mo = res[setname]['mean_out']
        ex = res[setname]['extract']; rm = res[setname]['removal']
        eff = {k: mo[k] - c[k] for k in c}              # mlp0's full effect
        keep = {k: (mo[k] - ex[k]) / max(eff[k], 1e-6) for k in c}
        rise = {k: rm[k] - c[k] for k in c}
        return eff, keep, rise
    eff7, keep7, rise7 = scores('s7000')
    eff2, keep2, rise2 = scores('s2000')

    pa = keep7['frag'] >= 0.50 and keep7['global'] <= 0.30
    pb = rise7['frag'] >= 3 * max(rise7['global'], 1e-6)
    ok = []
    for (a7v, a2v) in ((keep7['frag'], keep2['frag']),
                       (rise7['frag'], rise2['frag'])):
        ok.append((a7v > 0) == (a2v > 0) and abs(a2v) >= 0.5 * abs(a7v))
    pc = all(ok)
    out = {'res': res,
           'effect_7000': {k: round(v, 4) for k, v in eff7.items()},
           'keep_extract_7000': {k: round(v, 4) for k, v in keep7.items()},
           'rise_removal_7000': {k: round(v, 4) for k, v in rise7.items()},
           'keep_extract_2000': {k: round(v, 4) for k, v in keep2.items()},
           'rise_removal_2000': {k: round(v, 4) for k, v in rise2.items()},
           'pred_a_selective_extract': bool(pa), 'pred_b_removal_3x': bool(pb),
           'pred_c_generalizes': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"keep {keep7} rise {rise7}")
