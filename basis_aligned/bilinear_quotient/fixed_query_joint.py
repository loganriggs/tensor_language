"""FIXED QUERY JOINT -- does per-head fixed-query tolerance compose?
574 found 161/162 heads individually tolerate a fixed (mean) query
at < 0.02 nats. The program's recurring question is whether such
per-component results COMPOSE (they mostly have not: interface
compression 540/541, prefix accumulation 543). This tests it for
the query computation: replace EVERY head's query with its own
mean simultaneously, and measure the whole-model cost.
If the joint cost is small, the model's entire attention reduces
to FIXED QUERIES resolved by the double-QK AND and rotary -- a
strong, clean, model-wide simplification. If it is large and
superadditive versus the sum of the 162 individual costs, then
per-head fixed-query tolerance does not compose, consistent with
the distributed-computation theme.
Also swept: cumulative by layer (replace all queries in layers
0..k), to see where the cost accumulates -- 574 flagged the early
layers (2.5, 3.5, 1.1) as the most content-dependent, so the
prediction is that early layers contribute disproportionately.
REGISTERED PREDICTIONS:
  (0) IDENTITY: reinjecting the real queries for all heads costs
      < 1e-3 nats, verifying the machinery;
  (a) THE JOINT NUMBER: report the cost of replacing all 162
      queries with their means. No bar -- the number is the
      result;
  (b) COMPOSITION: report the ratio of the joint cost to the sum
      of the 162 individual costs from 574. Ratio near 1 means it
      composes additively; ratio >> 1 means superadditive
      (does not compose);
  (c) LAYER CUMULATIVE: report the cost of fixing all queries in
      layers 0..k for k = 0,2,5,8,11,14,17, to locate where the
      cost accrues;
  NULL: fixing all queries to RANDOM matched-norm vectors costs
      more than fixing them to the means -- the means are
      meaningful."""
...
OUT=PT+'fixed_query_joint_results.json'
NFRESH=48
...
def main():
    ...
    for i in range(0,NFRESH,4):
        ...
        for li in range(NL):
            qp=F.rms_norm(m.transformer.h[li].attn.c_q(caps[li])
                .view(B,T,NH,128),(128,)).float()
            for h in range(NH):
    ...
    base=price([])
    prev=json.load(open(PT+'fixed_query_census_results.json'))
    sum_indiv=sum(prev['costs'].values())
    allL=list(range(NL))
    joint=price(allL)-base
    print(f'baseline {base:.4f}',flush=True)
    print(f'JOINT (all 162 queries fixed to means): {joint:+.4f}',
          flush=True)
