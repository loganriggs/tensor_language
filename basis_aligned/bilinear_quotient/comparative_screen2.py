# comparative_screen2: BEHAVIOUR-FIRST screen for a carried SYNTACTIC EXPECTATION at mid
# range — predicting "than" after a comparative ("bigger", "more", "easier") appeared
# earlier in the sentence. Unlike the delimiter/question states (opener -> closing symbol),
# this expectation licenses a specific FUNCTION WORD at variable distance.
#
# Target: positions whose next token is a "than" token AND a comparative token appeared in
# the previous 20 positions. Controls per SOP: jitter +-3 and random matched.
#
# Registered predictions:
#   pred_a A CARRIER EXISTS: concentration >= 3 with jitter <= 1.5.
#   pred_b MID-ATTENTION BET: the winner is attention in L4-9 (fetch-range, nearer than
#          the state heads at L10/13 — the dependency is short).
...
OUT = PT + 'comparative_screen2_results.json'
NMEAN = 24; NR = 960
H = m.transformer.h
...
def main():
    ...
    out = {'n_targets': ntar, 'base': {k: round(v, 4) for k, v in base.items()},
           ...
           'pred_a_carrier': bool(pa), 'pred_b_attn8': bool(pb), 'pred_c_rand_flat': bool(pc),
           'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"top5 {out['top5']}")
    print(f"pred_a carrier {pa} | pred_b attn8 {pb} | pred_c rand {pc}")
