# handle_score_mlp1: THE THREE-PROPERTY HANDLE SCORE AT SITE #2 (user directive
# 2026-08-26: judge compressions by how well they support extraction, removal, and
# generalization — so the suite must be a repeatable scorecard, not a one-off).
# Same protocol as S1486/88 but at mlp1 (the model's biggest module): basis under
# test = top-8 whitened right-vectors of block-2's stacked composed reads of h1
# (pat(4)+val+mlp(2) @ Down1); baselines = activation-PCA-8 of h1 and random-8.
# Extraction (mean + subspace) and removal at mlp1's output; every arm scored on
# skip=7000 AND fresh skip=2000 in one run. Classes: det / name-fragment masks.
...
#   pred_a weight-channel-8 extraction keeps >= .30 of mlp1's frag effect (7000).
#   pred_b the selectivity split replicates at site 2: weight ratio >= 2x PCA ratio.
#   pred_c generalization: frag-keep and removal-rise replicate on skip=2000 with
#          same sign and >= half magnitude.
...
OUT = PT + 'handle_score_mlp1_results.json'
NR = 960
...
def main():
    ...
    # h1 stats (mlp1 hidden), block-2 composed reads
    Wd1 = H[1].mlp.Down.weight.float().to(DEV)
    at2 = H[2].attn
    STACK = torch.cat([at2.c_q.weight.float().to(DEV) @ Wd1,
                       ...
                       H[2].mlp.Left.weight.float().to(DEV) @ Wd1,
                       H[2].mlp.Right.weight.float().to(DEV) @ Wd1], 0)

    @torch.no_grad()
    def run_to_h1(idx):
        """Return (h1 [B,T,HD], mlp1_out) plus keeps model prefix consistent."""
        x = F.rms_norm(m.transformer.wte(idx), (D,)); x0 = x; v1 = None
        B = idx.shape[0]
        for L in (0, 1):
            blk = H[L]
    ...
    for i in range(0, 480, 8):
        ...
        mo = (h1.to(H[1].mlp.Down.weight.dtype) @ H[1].mlp.Down.weight.T).float() \
            + H[1].mlp.Down_bias.float()
        mo_acc += mo.sum(0)
        if i < 240:
            hs.append(h1[::7].cpu())
    MU1 = a1s / n0
    ...
    out = {'res': res,
           ...
           'pred_a_frag_keep_30': bool(pa), 'pred_b_selectivity_2x': bool(pb),
