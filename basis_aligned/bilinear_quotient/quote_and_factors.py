"""QUOTE AND FACTORS -- a third head, to test the general account.
560/562 gave a general account of structural attention: a fixed
query selects a token class through the double-QK soft-AND, and the
head either discriminates a specific member by position (brackets)
or detects the class indifferently (newlines). Two heads is a
pattern, not a law. This tests a third: head 10.7, which leads
opening-quote prediction (524).
Opening quotes are a DETECTION task like newlines -- predicting a
quote needs "is quotation-like context present", not "which
specific earlier token". So the general account predicts head 10.7
is a TOKEN detector: the double-QK AND holds, but selectivity is
token-driven (dies under key-mean) not position-driven (survives
rotary removal). That is a real advance prediction and can fail.
Target: an opening-quote-predicting position. Match: the most
recent preceding quote-or-quote-context key. Distractor: a
non-quote neighbour. Same factor probes as 560/562.
REGISTERED PREDICTIONS:
  (0) EXACTNESS: f1*f2 reproduces the score to 1e-4;
  (a) AND: product match/distractor ratio >= 1.3x the larger
      factor -- the soft-AND holds for a third head;
  (b) TOKEN MODALITY (advance prediction): the factors' selectivity
      survives rotary removal (drops < 30%) but collapses under
      key-mean (drops > 50%), matching the newline head and the
      detection-needs-identity rule. If instead it is
      position-driven, the account is wrong for detection tasks
      and that is reported;
  (c) report all ratios;
  NULL: control (jittered query) product ratio < 1.3."""
...
OUT=PT+'quote_and_factors_results.json'
NFRESH=192
    ...
    for r in range(NFRESH):
        ...
        for q in range(T):
            if chr(10) in cl.d1(int(nxt[r,q])):
                ...
                if prev:
                    mt=prev[-1]              # most recent newline key
                    ds=mt-1                  # non-newline just before it
                    if ds>=0 and not isnl[r,ds]:
