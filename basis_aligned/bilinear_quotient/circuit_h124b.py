...
# Registered:
# RETRY (S1557: the 64-token window left only 39 antecedent-absent positions).
# Window narrowed to 16 tokens.
#   pred_a antecedent-absent positions >= 300 (the split is now powered).
#   pred_b antecedent-present damage >= 1.5x absent.
#   pred_c global rise <= .005.
...
OUT = PT + 'circuit_h124b_results.json'
NR = 960
H = m.transformer.h
...
def main():
    ...
    def measure(on):
        ...
        for i in range(0, NR, 8):
            ...
            past64 = cum - torch.roll(cum, 16, dims=1)
            past64[:, :16] = cum[:, :16]
            hascap = past64 > 0
    ...
    pa = nn['no_ante'] >= 300
    pb = rises['ante'] >= 1.5 * max(rises['no_ante'], 1e-6)
    pc = gl <= 0.005
    out = {'global_rise': gl, 'rises': rises, 'n_positions': nn,
           'pred_a_powered_300': bool(pa), 'pred_b_ante_15x': bool(pb),
           'pred_c_global_005': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
