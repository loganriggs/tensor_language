"""[v2 distinctive-token read, fixes §865 frequency confound] DATA-DRIVEN: what does the CONTENT residual encode? (naming attempt via unsupervised discovery, §864).
...
OUT = PT + 'content_clusters2_results.json'; NEVAL = 260; RTOK = 64; RPOS = 32; K = 16; READ_L = 15
...
def main():
    ...
    # corpus baseline freqs
    base_cur = Counter(cur); base_nxt = Counter(tgt[tgt>=0]); Ncur=len(cur); Nnxt=int((tgt>=0).sum())
    def distinctive(counter, base, N_all, n_cl, topn=8, mincount=4):
        scored=[]
        for t,c in counter.items():
            if c<mincount: continue
            ratio = (c/n_cl)/((base.get(t,0)+1)/N_all)
            scored.append((ratio, t, c))
        scored.sort(reverse=True)
        return [repr(d(t)) for _,t,_ in scored[:topn]]
    clusters=[]
    for j in range(K):
        mk=a==j; n=int(mk.sum())
        if n<10: continue
        ...
        clusters.append({'cluster':j,'n':n,
                         'distinctive_current':distinctive(curc,base_cur,Ncur,n),
                         'distinctive_next':distinctive(nxtc,base_nxt,Nnxt,int((tgt[mk]>=0).sum()))})
    out={'k':K,'read_layer':READ_L,'clusters':clusters,'runtime_s':round(time.time()-t0,1)}
    json.dump(out,open(OUT,'w'),indent=1)
    for c in clusters: print(f"cluster {c['cluster']} (n={c['n']}): DISTINCTIVE cur {c['distinctive_current'][:6]} -> next {c['distinctive_next'][:6]}",flush=True)
    print(f"wrote {OUT}")
