# digit_heads: §1340-PATTERN HEAD STAGE on a8 (digit owner, §1352 certified: +0.210 at
# n=9056, conc 5.9, cross-method). Solo + all-but-one over a8's 9 heads at digit targets.
#
# Registered predictions (modest owner bar — digits are a broad class):
#   pred_a top solo head >= 50% of the layer's target damage.
#   pred_b knockout and keep-only AGREE on the top head (§3.1 both-directions check).
...
def main():
    ...
    # target mask: next tok is pure digits
    ...
    print(f"layer dmg {layer_dmg:+.4f} (vs §1352 +0.210)", flush=True)
    solo = {}; keep = {}
    for h in range(9):
        r = ce_all({h})
    ...
    pa = solo[top_solo]['target'] >= 0.50 * max(layer_dmg, 1e-4)
