# scalar_recalibrate: USER-PROPOSED — can a learned PER-HEAD OUTPUT SCALAR repair the
# read-mask damage? Rationale: masking reads leaves surviving/local contributions slightly
# miscalibrated (§1213/§1225-27 interference: some reads are net harmful in the restricted
# regime). A scalar per head adds ZERO description complexity to the certificate — it folds
# exactly into c_proj's head slice — so any recovery it buys is free understanding.
#
# Setup: all-162-heads read-mask @W64 on prose (gap 0.1758, §1186/§1222 replicated 4x —
# the largest clean restriction residual). Learn s in R^162 (init 1.0), head h's pre-c_proj
# output y_h *= s_(L,h), by Adam on CE over TRAIN rows (8); evaluate on DISJOINT eval rows
# (16). Controls: base/masked CE on eval rows; PERMUTED scalars (same multiset, shuffled
# across heads) — parameter-count-matched null.
#
# Registered predictions:
#   pred_a FREE RECOVERY IS REAL: held-out masked-CE gap shrinks >= 20% with scalars
#          (0.1758 -> <= 0.141 on eval rows).
#   pred_b THE FIX IS WHERE THE CUTS ARE: mean |s-1| over the 12 §1222 core heads >= 2x
#          mean |s-1| over all other heads (recalibration concentrates on the heads whose
#          reads were severed).
#   pred_c PERMUTED NULL FAILS: shuffled scalars recover <= 5% of the gap (it is the
#          assignment, not the magnitudes).
...
OUT = PT + 'scalar_recalibrate_results.json'
NTRAIN = 8; NEVAL = 16; WIN = 64; QSTART = 128; STEPS = 60; LR = 0.03
...
def main():
    ...
    out = {'n_train': NTRAIN, 'n_eval': NEVAL, 'W': WIN, 'steps': STEPS,
           ...
           'pred_a_free_recovery': bool(rec >= 0.20),
