# state_in_full_ship: DO THE COMPUTATIONS SURVIVE THE TOTAL-GLASS MODEL? (S1552:
# they survive the attention replacement exactly; the final certification is the
# full 36-module v4 ship.) Metrics: is/are agreement accuracy + quote-parity CE
# gap, clean vs all-36-replaced.
...
#   pred_a is/are accuracy under the full ship >= .88 (clean ~.957).
#   pred_b the parity gap retains >= .60 of its clean value.
#   pred_c global matches the v4 record within .05 (sanity: 3.843).
...
OUT = PT + 'state_in_full_ship_results.json'
NR = 960
...
def main():
    ...
    acc0, gap0, g0 = metric(frozenset(), frozenset())
    acc1, gap1, g1 = metric(ALLL, ALLM)
    pa = acc1 >= 0.88
    pb = gap1 >= 0.60 * gap0
    pc = abs(g1 - 3.8431) <= 0.05
    out = {'clean': {'acc': round(acc0, 4), 'gap': round(gap0, 4),
                     'global': round(g0, 4)},
           'full_ship': {'acc': round(acc1, 4), 'gap': round(gap1, 4),
                         'global': round(g1, 4)},
           'pred_a_acc_88': bool(pa), 'pred_b_gap_60': bool(pb),
           'pred_c_global_sanity': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
