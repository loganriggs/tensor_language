# grand_stack: EVERYTHING CERTIFIED, JOINTLY (§1183 capstone) — folded patterns (162) +
# sink constant + m0@8 + mlp1@16 + mlp2@32 + mlp3/4@64 + mlp5/7/9/12/15@64.
# Registered: (a) grand total <= 0.06 nats; (b) sub-additive vs parts' sum ~0.075 (6th time);
# (c) sanity exact.
#
...
OUT = PT + 'grand_stack_results.json'
NR = 16; W = 128
...
def main():
    ...
    CONDS = {'base': (set(), False, False, set()),
             'grand': (set(range(18)), True, True, NG_ALL)}
    ...
    cost = round(CE['grand'] - CE['base'], 4)
    PARTS_SUM = 0.0141 + 0.0132 + 0.0139 + 0.0094 + 0.0129 + 0.0124 + 0.004 + 0.0062 + 0.0062 + 0.0027 + 0.0025 + 0.0019
    out = {'n_rows': NR, 'W': W, 'ce': CE, 'grand_cost': cost, 'parts_sum': round(PARTS_SUM, 4),
           'sanity_base_matches_true': bool(abs(CE['base'] - CE['true_model']) <= 0.02),
           'pred_a_grand_cheap': bool(cost <= 0.06),
           'pred_b_subadditive': bool(cost <= PARTS_SUM),
           'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"CE {CE}")
    print(f"grand {cost} vs parts sum {out['parts_sum']}")
    print(f"sanity {out['sanity_base_matches_true']} | pred_a cheap {out['pred_a_grand_cheap']} | pred_b subadd {out['pred_b_subadditive']}")
