# mlp1_clusters: THE SIMPLICITY LADDER for the module-replacement benchmark (user
# clarification 2026-08-24: the score is reconstruction AND simplicity — a 50k-row table
# is the complex end, not the answer; if mlp1 is a categorizer mapping tokens into ~16
# classes, THAT is the description). Replace mlp1's output with K-cluster approximations:
# k-means over per-token mean outputs, token -> its cluster's centroid. K in {1, 4, 16,
# 64, 256, 1024}; the full unigram table (mlp1_table.py, K=50k) is the ceiling anchor.
# Description length ~ K vectors + a 50k->K assignment (log2 K bits/token).
#
# Registered predictions:
#   pred_a CATEGORIZER: K=16 already recovers >= 60% of the full-table recovery.
#   pred_b SATURATION: K=1024 adds <= 10 points over K=64.
#   pred_c SEMANTIC CLUSTERS ("numbers to numbers"): at K=16, single-digit tokens land in
#          <= 2 distinct clusters.
...
OUT = PT + 'mlp1_clusters_results.json'
...
def main():
    ...
    pa = rec['k16'] >= 0.6 * max(rec['table50k'], 1e-4)
    pb = (rec['k1024'] - rec['k64']) <= 0.10
    pc = len(dig_clusters) <= 2
    out = {'n_fit': NFIT, 'n_eval': NR, 'ce': res, 'stake': round(stake, 4),
           'recovery': rec, 'digit_clusters_at_k16': sorted(dig_clusters),
           'pred_a_categorizer': bool(pa), 'pred_b_saturation': bool(pb),
           'pred_c_digits_together': bool(pc),
           'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"stake {stake:.4f} | recovery {rec} | digit clusters@16 {sorted(dig_clusters)}")
    print(f"pred_a categorizer {pa} | pred_b saturation {pb} | pred_c digits {pc}")
