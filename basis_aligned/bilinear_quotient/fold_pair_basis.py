"""PAIR-FOLD BASIS -- user direction: the early layers are EXACTLY
computable; fold_basis used only unigram folds (mlp(rms_norm(wte)))
and gained +1. This version computes the exact 2-token contextual
object: run the REAL model on every corpus-support token pair [p,t]
(2-token sequences) and capture mlp0/mlp1 outputs at position 1 --
the true early-stack state given a 2-token context, including
attn0's lexical composition and the bilinear cross terms that
unigram folds + binary shifts cannot represent. Top-8 PCA directions
per layer -> 16 pairfold predicates (+32 shifted). Ladder rerun
until dry. Baseline: 58/118 (fold_basis).
REGISTERED PREDICTIONS:
  (a) converged programmable total >=63 of 118 (+5 over 58);
  (b) shuffled-label null median <=0.6;
  (c) >=3 passes cite a pairfold_ predicate."""
...
OUT=PT+'fold_pair_basis_results.json'
CA=300; NB=78
MHL=list(range(2,10))
T=256
...
def main():
    ...
    # ---- EXACT 2-token pair-fold features ----
    PRS=torch.stack([PREV.reshape(-1),flat],1)
    valid=PRS[:,0]>=0
    uniqp,inv=torch.unique(PRS[valid],dim=0,return_inverse=True)
    print(f'unique pairs {len(uniqp)}',flush=True)
    caps9={0:[],1:[]}
    hs9=[]
    for li9 in (0,1):
        def mk9(li9=li9):
            def h(mo,i_,o_):
                caps9[li9].append(o_.detach().float()[:,1].cpu())
            return h
        hs9.append(m.transformer.h[li9].mlp
                   .register_forward_hook(mk9()))
    for i9 in range(0,len(uniqp),1024):
        bb=uniqp[i9:i9+1024].to(DEV)
        m(bb,bb)
    for h in hs9: h.remove()
    for li9 in (0,1):
        T9=torch.cat(caps9[li9]); T9=T9-T9.mean(0)
        g9f=torch.Generator().manual_seed(3)
        sub=torch.randperm(len(T9),generator=g9f)[:20000]
        _,_,Vh9=torch.linalg.svd(T9[sub],full_matrices=False)
        SCRP=T9@Vh9[:8].T
        pv=SCRP[inv]
        for k9 in range(8):
            fb=torch.zeros(len(flat),dtype=torch.bool)
            fb[valid]=pv[:,k9]>=pv[:,k9].median()
            LIB0[f'pairfold_m{li9}_d{k9}']=fb
        caps9[li9]=None
        print(f'pairfold m{li9} added',flush=True)
    LIB=dict(LIB0)
    ...
    nf=sum(1 for p in allp if 'pairfold_' in p)
    gainN=total-58
    pc=nf>=3
    out['pred_c']=bool(pc); out['n_pairfold_cited']=nf; out['gain_vs_58']=gainN
    print(f'pairfold-cited passes: {nf} | gain vs fold_basis: {gainN:+d}')
    print(f"(a) converged >=63: {'HELD' if pa else 'FAILED'}")
    print(f"(b) null <=0.6: {'HELD' if pb else 'FAILED'}")
    print(f"(c) >=3 pairfold cites: {'HELD' if pc else 'FAILED'}")
