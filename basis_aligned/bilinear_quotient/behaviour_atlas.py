"""BEHAVIOUR ATLAS -- does the method that found the newline head
generalize, or was that one lucky behaviour?
...
REGISTERED PREDICTIONS:
  ...
  NULL: for every class, the top component measured against the
      ...
      fires on random position sets is measuring nothing.
...
OUT=PT+'behaviour_atlas_results.json'
NFRESH=48
...
def main():
    ...
    for key in list(MODS):
        ...
        row={}
        for nm,mask in M.items():
            other=~(mask|CT[nm]|RN[nm])
            do=float(d[other].mean())
            row[nm]={
