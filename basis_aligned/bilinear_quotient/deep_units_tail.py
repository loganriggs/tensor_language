# deep_units_tail: PRICE THE TAIL AT THE STRAGGLERS (S1534: K 64->256 adds ~.08;
# where does the diffuse half live?). K in {2048, 4608} (full-K = exactness sanity)
# at mlps 10, 12, 13, 14.
...
#   pred_a K=2048 adds >= .10 over K=1024 at all four (the tail is broad but real).
#   pred_b mlp14 K=2048 >= .55.
#   pred_c full-K >= .95 everywhere (sub-MLP machinery is exact at full rank).
...
OUT = PT + 'deep_units_tail_results.json'
...
def main():
    ...
    K1024_REF = {10: .5939, 12: .5818, 13: .5661, 14: .417}
    pa = all(RECS[L][2048] >= K1024_REF[L] + 0.10 for L in TARGETS)
    pb = RECS[14][2048] >= 0.55
    pc = all(RECS[L][4608] >= 0.95 for L in TARGETS)
    out = {'res': res, 'recs': {str(L): RECS[L] for L in TARGETS},
           'k1024_ref': K1024_REF,
           'pred_a_2048_adds_10': bool(pa), 'pred_b_mlp14_55': bool(pb),
           'pred_c_full_exact_95': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
