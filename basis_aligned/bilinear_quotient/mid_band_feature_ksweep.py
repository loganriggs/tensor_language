# mid_band_feature_ksweep: PATH OR PLATEAU? — how far the model's own features can go
#
# §1709 found that augmenting the middle-band linear maps with the module's own bilinear features
# HURTS at k=8 (-0.16 [-0.25,-0.07]) and HELPS at k=64 (+0.77 [+0.65,+0.89]), giving a new best
# whole-model program of 55.80%. It left one question, decisive either way:
#
#   PATH    -- the gain keeps climbing past k=64 and several more points are available by spending
#              more features; the band's quadratic content is reachable through the module's own
#              basis, just not cheaply.
#   PLATEAU -- the gain flattens and 64 features already extract what this basis offers. Then the
#              band's 37.7% quadratic remainder (§1668) is NOT recoverable from the module's own
#              feature basis at any tractable k, which is the stronger and more useful statement.
#
# §1679 is not a guide here: its k=512 arm was catastrophic (-49.93%) but that REPLACED the module,
# pinning the unselected features. This AUGMENTS, so the unselected features keep contributing
# through the live module and the cancellation structure §1679 broke is left intact.
#
# k in {64, 256, 512} plus the k=0 control. 64 is carried over as a direct replication of §1709's
# +0.77 in the same run, so the sweep is anchored rather than compared across runs.
#
# Registered predictions, TWO-SIDED per LESSONS 31:
#   pred_a THE k=64 REPLICATION HOLDS: its gain lands within 0.15 points of §1709's +0.77. Without
#          this the larger arms are not comparable to anything.
#   pred_b PATH RATHER THAN PLATEAU: k=512 exceeds k=64 by at least 0.30 points, with the
#          difference's 95% interval excluding zero. Failure here is the PLATEAU result and is
#          reported as the finding, not as a null.
#   pred_c DIMINISHING RETURNS EITHER WAY: the gain from 64 to 512 is smaller than the 0.93 points
#          the run went from k=8 to k=64. A larger jump would mean the family is still in its steep
#          region at 512 and the sweep has not found the shape.
...
OUT = PT + 'mid_band_feature_ksweep_results.json'
...
S1709_K64_GAIN = 0.00768
S1709_K8_TO_K64 = 0.00932
FEAT = {'k': 0, 'sel': {}}
...
def main():
    ...
    print(f'MID BAND FEATURE K-SWEEP | linear + k own bilinear features at '
    ...
    d_lo = ci_g['k512'][0] - ci_g['k64'][1]
    d_hi = ci_g['k512'][1] - ci_g['k64'][0]
    pa = abs(g64 - S1709_K64_GAIN) <= 0.0015
    pb = ((g512 - g64) >= 0.0030) and ((d_lo > 0) or (d_hi < 0))
    pc = (g512 - g64) < S1709_K8_TO_K64
    pd = (abs(base - S1696_BOTH) <= 0.005 and abs(cl - S1683_CE_LIVE) <= 1e-3)

    for k in KS[1:]:
        g = arms[f'k{k}']['ceiling_exact'] - base
        print(f'  k={k:4d} gain {g:+.2%}  95% CI [{ci_g[f"k{k}"][0]:+.2%}, '
              f'{ci_g[f"k{k}"][1]:+.2%}]  ceiling {arms[f"k{k}"]["ceiling"]:.2%}  '
              f'{k * D * len(MID) / 1e6:.2f}M extra reals total', flush=True)
    print(f'  k=64 replicates §1709 (+{S1709_K64_GAIN:.2%}) -> {pa}', flush=True)
    print(f'  k=512 over k=64: {g512 - g64:+.2%} -> PATH rather than plateau {pb}', flush=True)
    print(f'  vs the k8->k64 step of +{S1709_K8_TO_K64:.2%} -> diminishing {pc}', flush=True)
    print(f'  fraction of the band gain +{S1703_MID_BAND_GAIN:.2%} recovered at k=512: '
          f'{g512 / S1703_MID_BAND_GAIN:.1%}', flush=True)
    ...
    res = {'config': {'mid_sites': MID, 'ks': KS, 'hidden_dim': DH, 'ridge': RIDGE,
           ...
           'fraction_of_band_recovered_k512': round(g512 / S1703_MID_BAND_GAIN, 4),
           'predictions': {'pred_a_k64_replicates_s1709': bool(pa),
                           'pred_b_path_not_plateau': bool(pb),
