"""CENSUS RED-TEAM (reviewer-2) -- four attacks on the 118-circuit
census, each with a bar that can fail:
  A. NESTING: parents and children share members by construction; count
     DISTINCT circuits after collapsing Jaccard>0.5 member overlaps.
  B. SEVERITY ARTIFACT: an all-positive loading vector is a damage-
     severity gradient, not a contrastive circuit; count sign-mixed
     leaves (both signs carrying >=10% of loading L1 mass).
  C. CONFOUNDS: per leaf, corr(mode score, base CE); top-token share of
     members; corr(membership, position-in-row).
  D. JOINT CAUSAL: for up to 20 blind-nameable leaves, ablate the
     leaf's implicated components TOGETHER; member damage must be >=2x
     that of base-CE-MATCHED non-members (kills the "just fragile
     tokens" story; jointness tests coherence).
REGISTERED PREDICTIONS:
  (a) >=60 distinct circuits survive dedup;
  (b) >=40% of leaves sign-mixed;
  (c) medians: |corr(score, baseCE)| <=0.4, top-token share <=0.5,
      |corr(member, position)| <=0.3;
  (d) >=70% of tested leaves pass the 2x matched-control causal bar."""
...
OUT=PT+'census_redteam_results.json'
CA=300; NB=78
MHL=list(range(2,10))
T=256
...
def main():
    ...
    for md in roots:
        ...
        leaves.append(md)
        recurse(md,1)
    print(f'tree built: {len(leaves)} leaves; auditing',flush=True)
    # ---- A. nesting dedup ----
    msets=[set(lf['_member'].tolist()) for lf in leaves]
    keep=[]
    for i in range(len(leaves)):
        dup=False
        for j in keep:
            inter=len(msets[i]&msets[j]); uni=len(msets[i]|msets[j])
            if inter/max(uni,1)>0.5: dup=True; break
    ...
    for lf in leaves:
        L=lf['_loading']
        pos=float(L.clamp(min=0).abs().sum()); neg=float(L.clamp(max=0).abs().sum())
        tot=pos+neg
        if min(pos,neg)/max(tot,1e-9)>=0.10: nmix+=1
    # ---- C. confounds ----
    basev=base.cpu()
    import math
    ccebase=[]; ttok=[]; cpos=[]
    posidx=(torch.arange(ntok)%256).float()
    for lf in leaves:
        sl=lf['_slice']; sc_=lf['_score']
        bv=basev[sl]
        c1=float(torch.corrcoef(torch.stack([sc_.abs(),bv]))[0,1])
        ccebase.append(abs(c1))
        mem=lf['_member']
        tt_=toks[mem]
        top=tt_.mode().values
        ttok.append(float((tt_==top).float().mean()))
        memflag=torch.zeros(len(sl))
        mset=set(mem.tolist())
        memflag=torch.tensor([1.0 if int(x) in mset else 0.0
                              for x in sl])
        c2=float(torch.corrcoef(torch.stack([memflag,
                                             posidx[sl]]))[0,1])
        cpos.append(abs(c2) if not math.isnan(c2) else 0.0)
    med=lambda v: sorted(v)[len(v)//2]
    # ---- D. joint causal on blind-nameable leaves ----
    try:
        qs=json.load(open(PT+'v4_quiz_scores.json'))['scores']
        passtags=[r['tag'] for r in qs if r['true_pos']>=5][:20]
    except Exception:
        passtags=[lf['tag'] for lf in leaves[:20]]
    tested=0; passed=0; caus=[]
    for lf in leaves:
        if lf['tag'] not in passtags: continue
        comps=set()
        for ps in lf['top_probes']:
        ...
        dmg=(ce_vec(hooks)-base).cpu()
        mem=lf['_member']
        bv=basev
        memdmg=float(dmg[mem].mean())
        mb=basev[mem]
        lo,hi=mb.quantile(0.1),mb.quantile(0.9)
        mset=set(mem.tolist())
        cand=[i for i in range(ntok) if i not in mset]
        cand=torch.tensor(cand)
        candb=basev[cand]
        sel=cand[(candb>=lo)&(candb<=hi)]
        ctldmg=float(dmg[sel].mean())
        ratio=memdmg/max(ctldmg,1e-4)
        caus.append({'tag':lf['tag'],'member_dmg':round(memdmg,4),
                     'matched_ctl':round(ctldmg,4),
                     'ratio':round(ratio,2)})
        tested+=1
        if ratio>=2: passed+=1
        print(f"causal {lf['tag']}: mem {memdmg:+.3f} ctl {ctldmg:+.3f} "
              f"ratio {ratio:.2f}",flush=True)
    pa=ndistinct>=60
    pb=nmix>=0.4*len(leaves)
    pc=(med(ccebase)<=0.4 and med(ttok)<=0.5 and med(cpos)<=0.3)
    pdx=(passed>=0.7*tested) if tested else False
    out={'n_leaves':len(leaves),'n_distinct':ndistinct,
         'sign_mixed':nmix,
         'median_corr_baseCE':round(med(ccebase),3),
         'median_top_token_share':round(med(ttok),3),
         'median_corr_position':round(med(cpos),3),
         'causal_tested':tested,'causal_passed':passed,'causal':caus,
         'pred_a':bool(pa),'pred_b':bool(pb),'pred_c':bool(pc),
         'pred_d':bool(pdx)}
    print(f"A distinct {ndistinct}/{len(leaves)}: "
          f"{'HELD' if pa else 'FAILED'}")
    print(f"B sign-mixed {nmix}/{len(leaves)}: "
          f"{'HELD' if pb else 'FAILED'}")
    print(f"C confound medians baseCE {med(ccebase):.2f} token "
          f"{med(ttok):.2f} pos {med(cpos):.2f}: "
          f"{'HELD' if pc else 'FAILED'}")
    print(f"D causal {passed}/{tested}: {'HELD' if pdx else 'FAILED'}")
