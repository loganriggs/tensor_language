# site_depth_separation: WHAT MAKES A SITE DIFFUSE? §1634 showed the SITE, not the
# class and not certification, governs whether an eigen-slice separates from a
# random basis. This asks whether that is a DEPTH effect.
#
# §1634 left one class measured at two sites: `at` separates 17/60 at mlp11 (gap
# −.0075, essentially no signal) and 60/60 at mlp17 (gap −.1094, tying the certified
# pronouns class). Same class, same statistic, same rows, same 20 seeds -- only the
# site differs. This holds the CLASS fixed at `at` and sweeps five sites in depth
# order: mlp5, mlp8, mlp11, mlp14, mlp17, rank-2 TOP-4 throughout.
#
# A CONFOUND I AM DELIBERATELY NOT TRIPPING OVER. The upstream component set grows
# with depth: 12 components at mlp5 (x0 + attn0..5 + mlp0..4) against 36 at mlp17.
# A TOP-4 SHARE is therefore mechanically larger at shallow sites and raw shares are
# NOT comparable across sites. The SEPARATION COUNT is, because the lambda and
# random arms at a given site see the identical component set, so the count is a
# within-site comparison and the component-count term cancels. All scored quantities
# below are separation counts and within-site gaps; no cross-site share is scored.
#
# SITE is now a SINGLE module-level global read by the forward stop, the component
# set and the coefficient chain alike, so a forward-extent/component-set mismatch
# (LESSONS 20) is structurally impossible rather than merely gate-checked.
#
# 20 independent random rank-2 bases (seeds 1729-1748) x 3 disjoint 160-row chunks
# = 60 trials per site, corrected quantity.
#
# Registered predictions:
#   pred_a DEPTH DRIVES IT: separation at mlp17 exceeds separation at mlp5 by >= 20
#          of 60.
#   pred_b IT IS MONOTONE: separation is non-decreasing across mlp5, mlp8, mlp11,
#          mlp14, mlp17 in depth order. (Risky -- mlp11 is already known to be 17/60,
#          so any shallower site scoring above 17 breaks this.)
#   pred_c THE GAP DEEPENS WITH DEPTH: the mean gap at mlp17 is more negative than
#          at mlp5 by >= .05.
...
OUT = PT + 'site_depth_separation_results.json'
NR = 960
...
PAT_AT = r'^ at$'
CELLS = {
    'mlp5':  {'site': 5,  'rank': 2, 'top': 4, 'certified': False, 'pat': PAT_AT},
    'mlp8':  {'site': 8,  'rank': 2, 'top': 4, 'certified': False, 'pat': PAT_AT},
    'mlp11': {'site': 11, 'rank': 2, 'top': 4, 'certified': False, 'pat': PAT_AT},
    'mlp14': {'site': 14, 'rank': 2, 'top': 4, 'certified': False, 'pat': PAT_AT},
    'mlp17': {'site': 17, 'rank': 2, 'top': 4, 'certified': False, 'pat': PAT_AT},
}
DEPTH_ORDER = ['mlp5', 'mlp8', 'mlp11', 'mlp14', 'mlp17']
...
def main():
    ...
    seq = [out_cells[k]['separation'] for k in DEPTH_ORDER]
    gaps_seq = [out_cells[k]['mean_gap'] for k in DEPTH_ORDER]
    pa = seq[-1] - seq[0] >= 20
    pb = all(seq[i] <= seq[i + 1] for i in range(len(seq) - 1))
    pc = gaps_seq[-1] - gaps_seq[0] <= -0.05

    print(f"\n  CLASS ` at` HELD FIXED, site swept in DEPTH ORDER, "
          f"{N_RANDOM*CHUNKS} trials each:", flush=True)
    for k in DEPTH_ORDER:
        v = out_cells[k]
        print(f"    {k:6s} separation {v['separation']:2d}/{v['trials']}  "
              f"mean gap {v['mean_gap']:+.4f}  lambda {v['lambda']}", flush=True)
    print(f"  depth sequence {seq}   monotone non-decreasing: {pb}", flush=True)
    print(f"  deepest - shallowest: separation {seq[-1]-seq[0]:+d}/60, "
          f"gap {gaps_seq[-1]-gaps_seq[0]:+.4f}", flush=True)
    print(f"  (§1634 anchors: ` at` was 17/60 at mlp11 and 60/60 at mlp17)", flush=True)
    ...
    out = {'config': {'cells': {k: {kk: vv for kk, vv in v.items() if kk != 'pat'}
           ...
           'cells': out_cells,
           'predictions': {'pred_a_deep_minus_shallow_ge_20': bool(pa),
                           'pred_b_monotone_in_depth': bool(pb),
                           'pred_c_gap_deepens_by_05': bool(pc)},
           'depth_order': DEPTH_ORDER, 'separation_sequence': seq,
           'gap_sequence': gaps_seq,
