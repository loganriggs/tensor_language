"""One watch-list per layer? Section 130: a layer's heads share a genuine filter
...
OUT=('/workspace/tensor_language/basis_aligned/bilinear_quotient/'
     'bilin18_one_watchlist_results.json')
...
def main():
    ...
    for li in LAYERS:
        ...
        print(f'L{li:2d}: attention-lexicon vs MLP-watchlist median cos {per[li]:.2f}',
              flush=True)
    wins=sum(1 for li in LAYERS if per[li]-rm>=0.15)
    pa=wins>=3
    ...
    print(f"(a) one watch-list per layer (>= null+0.15 at >=3/5): "
