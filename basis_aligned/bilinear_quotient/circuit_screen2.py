# circuit_screen2: TWELVE MORE CLASSES (S1505: 8/10 selective, weights-only method
# won 6/10 — now the registered direction). Same protocol.
...
#   pred_a >= 7 of 12 classes yield a selective circuit (>= 2x) under some method.
#   pred_b the weights-only method wins on class effect for >= 6 of 12 (the S1505
#          direction repeats).
#   pred_c head 12.6 appears in >= 3 of the 12 weight-method ensembles (a
#          formatting/punctuation generalist, not a class specialist).
...
OUT = PT + 'circuit_screen2_results.json'
NR = 480
...
def main():
    ...
    n126 = sum(1 for cn in CLS if '12.6' in res[cn]['W']['heads'])
    pa = n_sel >= 7
    pb = w_wins >= 6
    pc = n126 >= 3
    out = {'res': res, 'n_selective_2x': n_sel, 'w_wins': w_wins,
           'head126_count': n126,
           'pred_a_7of12_selective': bool(pa), 'pred_b_w_wins_6': bool(pb),
           'pred_c_126_generalist': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
