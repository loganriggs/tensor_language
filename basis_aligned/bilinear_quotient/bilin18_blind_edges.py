...
OUT=('/workspace/tensor_language/basis_aligned/bilinear_quotient/'
     'bilin18_blind_edges_results.json')
...
def main():
    ...
    for i,j in EDGES:
        ...
        scores.append(float((C*G2).sum()))
        null_scores.append(float(C.trace())*float(G2.trace())/D)
    ...
    pa=sK>=0.6; pb=top>=3*bot; pc=(sK-sN)>=0.2
    ...
    print(f"(a) rank prediction >=0.6: {'HELD' if pa else 'FAILED'}")
    print(f"(b) top/bottom >=3x: {'HELD' if pb else 'FAILED'}")
