...
#
# Registered predictions (content-copy retest, S1520 fix):
#   pred_a content-copy yields a >= 2x-selective ensemble under SOME method.
#   pred_b the content-copy P-ensemble overlaps induction's P-ensemble
#          ({5.5, 10.8, 12.6, 4.2, 8.6}) with Jaccard >= .3.
#   pred_c P beats T for content-copy (it is attention-defined).
...
OUT = PT + 'circuit_copy2_results.json'
NR = 480
...
def main():
    ...
    def best_sel(cn):
        ...
        return max(res[k]['rise_class'] / max(res[k]['rise_global'], 1e-6)
                   for k in keys)
    pa = best_sel('copy') >= 2
    IND_P = {'5.5', '10.8', '12.6', '4.2', '8.6'}
    a_ = set(res['copy_P']['heads'])
    jac = len(a_ & IND_P) / len(a_ | IND_P)
    pb = jac >= 0.3
    pc = res['copy_P']['rise_class'] > res['copy_T']['rise_class']
    out = {'res': res, 'copyP_inductionP_jaccard': round(jac, 3),
           'pred_a_content_copy_2x': bool(pa), 'pred_b_overlap_3': bool(pb),
           'pred_c_P_beats_T': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
