# pronoun_rank2_recheck: THE HOLD ON PENDING ITEM 2 — re-running the pronoun
# comparison in the DISCRIMINATING regime instead of the saturated one.
#
# §1638 concluded that pronouns@mlp17 has no distinctiveness against four other
# personal pronoun classes, and it is the sole basis for the second pending
# withdrawal in PENDING_RETRACTION_S1612.md. It was measured entirely at rank-8
# TOP-6.
#
# §1639 then showed the separation COUNT saturates at rank-8 TOP-6 (six unrelated
# cells all landing at 57-60/60), and §1640 showed rank drives that saturation 7:1
# over TOP, that the statistic is monotone in rank, and -- most relevant here -- that
# the GAP SIGN itself flips between rank-2 and rank-4 at mlp11 (` at` sits at −.0075
# at rank-2 and +.0527 at rank-8, same class, same site).
#
# So §1638's evidence sits in the saturated regime, and at least at one cell the sign
# of the very quantity it relies on is rank-dependent. The mlp17 gaps are large
# (pronouns −.1320, ` I` −.1795) and unlike the near-zero mlp11 case where the flip
# occurred, so I do not expect the conclusion to move -- but "I do not expect it to
# move" is the reasoning that failed six times today, and Logan is being asked to
# withdraw a published claim on this basis.
#
# Identical design to §1638 -- same five classes, same site, same rows, same seeds --
# changing ONLY the configuration to rank-2 TOP-4, which §1640 established as
# discriminating (17/60 for ` at`@mlp11 against 52/60 at rank-8).
#
# Registered predictions:
#   pred_a THE CONCLUSION SURVIVES THE REGIME CHANGE: pronouns' separation exceeds the
#          BEST other-pronoun class by <= 5 of 60, as it did at rank-8 TOP-6 (margin 0).
#   pred_b THE DIRECTION SURVIVES: all four other pronoun classes still have a
#          NEGATIVE mean gap at rank-2 TOP-4.
#   pred_c THE REGIME IS ACTUALLY DIFFERENT, so this is a real re-test and not a
#          repeat: at least one of the five classes has separation <= 45/60, which no
#          class managed at rank-8 TOP-6 (all were 58-60).
...
OUT = PT + 'pronoun_rank2_recheck_results.json'
NR = 960
...
CELLS = {
    'pronouns': {'site': 17, 'rank': 2, 'top': 4, 'certified': True, 'published': 0.482,
                 'pat': r'^ (he|she|they|He|She|They)$'},
    'it':       {'site': 17, 'rank': 2, 'top': 4, 'certified': False, 'pat': r'^ (it|It)$'},
    'we':       {'site': 17, 'rank': 2, 'top': 4, 'certified': False, 'pat': r'^ (we|We)$'},
    'you':      {'site': 17, 'rank': 2, 'top': 4, 'certified': False, 'pat': r'^ (you|You)$'},
    'I':        {'site': 17, 'rank': 2, 'top': 4, 'certified': False, 'pat': r'^ I$'},
}
...
def main():
    ...
    print(f"\n  SEPARATION at mlp17 rank-2 TOP-4 (DISCRIMINATING regime), exact-subtype pronoun controls, {N_RANDOM*CHUNKS} trials each:",
          flush=True)
    ...
    print(f"  other-pronoun classes with NEGATIVE gap: {n_neg}/4 | classes at <= 45/60 "
    ...
    out = {'config': {'cells': {k: {kk: vv for kk, vv in v.items() if kk != 'pat'}
           ...
           'cells': out_cells,
           'predictions': {'pred_a_margin_le_5_at_rank2': bool(pa),
                           'pred_b_all_4_negative_gap': bool(pb),
                           'pred_c_regime_is_unsaturated': bool(pc)},
           'n_negative_gap': n_neg, 'n_unsaturated': n_unsat,
