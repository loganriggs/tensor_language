# unified_bill_mlp: THE JOINT ATTN+MLP ONE-MODEL BILL (§1403-04: 28 heads serve 4
# families .64-.92 + else .81 with ALL MLPs live; §1395: question mid set {4,6,7,8,9},
# mlp5/10 dead weight). This run: unified ungated kit (28 heads) x mid-MLP treatments —
# all-mid-live / keep-{4,6,7,8,9} (union mid set, gmean stand-ins refit UNDER the
# unified kit for 5,10-15) / all-mid-gmean. Scored on all four families + else.
# Assumptions registered: keep set = {4,6,7,8,9} (union of comparative's {4,5,6,7}
# minus dead-weight 5 plus question's 8,9); commons ungated everywhere (§1402);
# specialists = the §1403 six.
#
# Registered predictions (one-sided):
#   pred_a keep-{4,6,7,8,9} holds comparative >= 0.80 recovery.
#   pred_b keep-{4,6,7,8,9} holds question >= 0.55 recovery.
#   pred_c keep-{4,6,7,8,9} holds elsewhere >= 0.60 recovery.
...
OUT = PT + 'unified_bill_mlp_results.json'
NFITT = 960
V = 50257
NMEAN = 24; NR = 1920
...
def main():
    ...
    for arm in ('full', 'ymean'):
        ...
        print(f"{arm}: " + " ".join(f"{k} {r[k]:.3f}" for k in MASKS) + f" | else {e:.4f}",
              flush=True)
    for mode, tag in ((None, 'live'), ('mid_live_set', 'keep5'), ('mid_mean', 'mean')):
        MLPMODE['cur'] = mode
        if mode == 'mid_live_set':
            MLPTAB['live_set'] = {4, 6, 7, 8, 9}
    ...
    table = {}
    for tag in ('live', 'keep5', 'mean'):
        a = f'unified_{tag}'
        table[tag] = {**{f: round(rec(a, f), 4) for f in MASKS},
                      'else': round(rece(a), 4)}
        print(f"{tag}: {table[tag]}", flush=True)
    pa = table['keep5']['comparative'] >= 0.80
    pb = table['keep5']['question'] >= 0.55
    pc = table['keep5']['else'] >= 0.60
    out = {'ce': res, 'recovery_table': table,
           'pred_a_comparative_80': bool(pa), 'pred_b_question_55': bool(pb),
           'pred_c_else_60': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
