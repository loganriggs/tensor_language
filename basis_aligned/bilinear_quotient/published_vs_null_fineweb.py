# published_vs_null_fineweb: TEST S1597's .718 AND S1598's .482 AS PUBLISHED, ON
# CANONICAL FINEWEB ROWS.
#
# Every section from S1609 to S1619 ran on curated_rows.pt (census rows, NOT
# fresh), because the FineWeb path was dead on this box from ~05:00 (8 rows at
# offset 40 did not stream in 876 s). Each of those sections therefore carries the
# caveat that shares are NOT comparable across corpora to the published figures.
# Codex removed that blocker at 08:24 via standard-HTTP download with an
# order-proof receipt (authority `pinned_local_ordered_manifest`, scored-work flag
# true, receipt fineweb_oracle_v2_receipt.json in .rowcache/).
#
# This is the first run able to compare a certified slice's share to its PUBLISHED
# value on the corpus that value came from.
#   question @ mlp11, rank-2 |lambda|, TOP 4   -- S1597 published .718
#   pronouns @ mlp17, rank-8 |lambda|, TOP 6   -- S1598 published .482 ("DIFFUSE")
# Rows: .rowcache/fineweb_n480_skip80.pt (480 x 513; 215 question and 809 pronoun
# positions), 3 disjoint 160-row chunks. Absolute attribution mass, matching
# slice_writers.py:216 exactly. Each cell against its own matched-rank random arm
# on identical rows -- S1612 established a bare share is uninterpretable without
# one, and S1613 that the null cannot be tabulated.
# NOTE: uses .rowcache ONLY, never .rowcache_shadow (authority `none`, cannot
# authorise scored work). Nothing in either directory is modified.
#
# Registered predictions:
#   pred_a the |lambda| share at question@mlp11 lands within .10 of the published
#          .718 on these rows (i.e. the published figure reproduces).
#   pred_b at question@mlp11 the |lambda| share EXCEEDS its matched-rank null by
#          >= .05 -- the published concentration claim has real content.
#   pred_c at pronouns@mlp17 the |lambda| share is BELOW its null, confirming
#          S1612's direction on the corpus the .482 was computed from.
...
OUT = PT + 'published_vs_null_fineweb_results.json'
NR = 960
...
def main():
    ...
    pa = sum(1 for g in q['gaps'] if g <= 0.10) >= 2
    pb = sum(1 for g in p['gaps'] if g <= 0.10) >= 2
    pc = all(v >= 3 for v in q['absent_from_random'])
    ...
    out = {'config': {'cells': {k: {kk: vv for kk, vv in v.items() if kk != 'pat'}
           ...
           'cells': out_cells,
           'predictions': {'pred_a_question_share_gap_le10_2of3': bool(pa),
                           'pred_b_pronouns_share_gap_le10_2of3': bool(pb),
                           'pred_c_question_membership_ge3_absent_3of3': bool(pc)},
