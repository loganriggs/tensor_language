"""[RTOPIC=128 frontier probe] Does FINER content (128-dim topic subspace vs 24) let a smooth map capture the middle, or is it genuinely interaction/high-dim? Note: the topic feature is the L15 content proj (DOWNSTREAM of the middle) so the middle number is an UPPER BOUND — if 128-dim STILL fails, the middle output is not even a function of the full downstream content. GAP-FILL: is the middle content a SMOOTH (generalizing) function of named variables, or genuinely
...
OUT = PT + 'learned_map_r128_results.json'
NEVAL = 200; SEQ = 256; CONTENT_L = 15; K = 12; RTOK = 64; RPOS = 32; RTOPIC = 128; RIDGE = 1e2
...
def main():
    ...
    topic_proj = content @ Utopic                                   # (N, RTOPIC)
