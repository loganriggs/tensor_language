"""MLP0 CLUSTER13 LOGIT MARGIN V2 -- redo of cluster 13's causal test
(584), fixing an unequal-token-count confound: the first version
summed 2 positive-class tokens against 4 negative-class tokens, so
the negative side's raw logit sum was inflated by having twice as
many terms, independent of context (direct check: period+exclaim mean
logit sums to 10.1 across ordinary positions, the four dash variants
to 14.9, purely from term count). Caught because the margin at the
class's OWN target positions came back negative -- an impossible
reading -- before it was written up as a refutation (584 withdrew the
verdict rather than reporting it). Fix: use the MEAN logit per class,
not the sum, so class size cannot bias the comparison. Cluster 7 is
rerun alongside as a check that its already-confirmed result (584)
survives the corrected metric (its own token counts were 3-vs-1, a
smaller version of the same imbalance, and its baseline signs already
looked sane, but the fix should be verified, not assumed, on both).
...
REGISTERED PREDICTIONS (per cluster, same structure as 583):
  (-1) BASELINE SANITY (the guard 584 needed and didn't have): the
      base margin at a class's OWN target positions must have the
      sensible sign (positive for the positive class, negative for
      the negative class) for BOTH clusters -- if not, the token
      choice is still confounded and no verdict below should be
      trusted;
...
OUT = PT + 'mlp0_cluster13_logit_margin_v2_results.json'
NFRESH = 64
NSAMP = 4000
TOPK = 300
NCLUST = 20
...
def main():
    ...
    def margins(unit_ids, pos_tok, neg_tok):
        ...
        for i in range(0, NFRESH, 4):
            ...
            pos = sum(lg[..., t] for t in pos_tok) / len(pos_tok)
            neg = sum(lg[..., t] for t in neg_tok) / len(neg_tok)
    ...
    for k, cfg in CLUSTERS.items():
        ...
        print(f'(-1) baseline sanity: base_pos={base_pos:.3f} (want >0), '
              f'base_neg={base_neg:.3f} (want <0): '
