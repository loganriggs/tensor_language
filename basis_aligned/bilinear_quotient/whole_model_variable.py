...
OUT = PT + 'whole_model_variable_results.json'
...
def main():
    ...
    out = {'ce_full': round(ce_full, 4), 'front_benefit': round(ben, 4), 'keep_classpos_recovery': round(rec, 4),
           'keep_random_recovery': round(rec_r, 4), 'pred_0': bool(p0), 'pred_a_whole_is_class_position': bool(pa),
           'runtime_s': time.time()-t0}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f'\n(a) the whole model reduces to a class+position computer end-to-end (keep>=0.7 & >>random): {pa}', flush=True)
