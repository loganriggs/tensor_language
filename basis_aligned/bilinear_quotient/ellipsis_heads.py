# ellipsis_heads: THE PAIR BET (§1359). a17 leads ellipsis at 2.5x; its named heads are
# the EXCLAIM PAIR 17.2+17.3. If the pair carries the ellipsis service, that is its THIRD
# capability surface (!, ..., both expressive terminals) and the §1354 heads-are-functions
# heuristic scores again. §1340 pattern at ellipsis targets (next token contains '...' or
# unicode ellipsis).
#
# Registered predictions:
#   pred_a THE PAIR CARRIES IT: 17.2 + 17.3 jointly >= 60% of the layer's target damage
#          (solo-sum grain).
...
OUT = PT + 'ellipsis_heads_results.json'
NMEAN = 24; NR = 1920
L13 = 17
H = m.transformer.h
LAYERS = (13,)
...
def main():
    ...
    for tok in range(50257):
        ...
        if '...' in d or '\u2026' in d:
            e_.add(tok)
    e_ids = torch.tensor(sorted(e_))
    print(f"ellipsis ids {len(e_)}", flush=True)
    ...
    # target mask: next tok contains an ellipsis
    toks = EVR[:, :-1]; tgt_all = EVR[:, 1:]
    TARGET = torch.isin(tgt_all, e_ids)
    TARGET[:, :64] = False
    ...
    print(f"layer dmg {layer_dmg:+.4f} (vs §1359 screen +0.302)", flush=True)
    solo = {}; keep = {}
    for h in range(9):
        r = ce_all({h})
    ...
    pair_sum = solo[2]['target'] + solo[3]['target']
    pa = pair_sum >= 0.60 * max(layer_dmg, 1e-4)
    ...
    out = {'n_targets': k, 'n_rows': NR, 'layer_dmg': round(layer_dmg, 4),
           ...
           'pair_sum_share': round(pair_sum / max(layer_dmg, 1e-4), 4),
           'pred_a_pair_carries': bool(pa), 'pred_b_directions_agree': bool(pb),
