# unit_screen2: 2x-DATA RERUN with the CLEANED jitter control (§1361). The v1 jitter
# (1.70) is suspected FUNCTION-SURFACE OVERLAP: unit targets adjoin numbers, so jitter
# positions land on a8's own digit surface. This run excludes digit-target positions
# from the jitter set (overlap fraction reported) and doubles the data.
#
# Registered predictions:
#   pred_a concentration reproduces: a8 >= 0.12 nats and conc >= 5.
#   pred_b CLEANED controls pass: jitter (digit-excluded) and random each <= 1.5. The
#          raw jitter is also reported — dirty-raw + clean-cleaned confirms the
#          contamination account in one run.
#   pred_c ownership at 2x data: a8 >= 2x each of a7/a9/a13/a5. Honest FALSE = units
#          are a8-led but shared; the thread parks.
...
OUT = PT + 'unit_screen2_results.json'
NMEAN = 24; NR = 3840
H = m.transformer.h
LAYERS = (8, 7, 9, 13, 5)
...
def main():
    ...
    JIT[:, 2:] = TARGET[:, :-2]
    JIT &= ~TARGET
    DIGT = torch.isin(tgt_all, dg_ids)
    raw_n = int(JIT.sum())
    overlap = int((JIT & DIGT).sum())
    JIT &= ~DIGT
    print(f"jitter digit-overlap {overlap}/{raw_n} = {overlap/max(raw_n,1):.2f} "
          f"(excluded)", flush=True)
    # random control: count-matched draw from non-target positions
    g = torch.Generator().manual_seed(211)
