# ship_glue: JOINT CE-TRAINED CORRECTIONS FOR ALL THREE MLP SLOTS (S1489: a single
# mlp2 correction recovered .80 CE. Now: rank-32 + bias corrections on the mlp0
# slot (input a0), mlp1 slot (input [a1, m0_stream]) and mlp2 slot (input [a2,
# m1_stream]), trained JOINTLY vs full-model CE inside the all4 ship. ~300k params
# ~ 5 Mbit of glue total. CAVEAT registered: glue is composite patching, partly
# compensating other planks — the S1489 transfer test showed both components exist.
...
#   pred_a all4 with joint glue <= 3.35 (from 3.56 with mlp2-only glue).
#   pred_b joint glue beats mlp2-only glue by >= .10.
#   pred_c the glued ship's gap to the attention-only composite (3.107) <= .30 —
#          i.e., three glued MLP planks cost <= .30 CE on top of the attn stack.
...
GLUE = {'on': False}   # per-slot: (b, U, V) keyed 0/1/2
USE_CLEAN = {'v': False}
    ...
    for L, blk in enumerate(H):
        ...
        if L == 0:
            ...
            if 0 in mlps:
                ...
                st_ = ym0_ + (a0_out.reshape(-1, D) - xm0_) @ W0
                if GLUE['on']:
                    b0, U0, V0 = GLUE[0]
                    st_ = st_ + b0 + ((a0_out.reshape(-1, D) - xm0_) @ V0) @ U0.T
                st = SHIP['t0'][toks].view(B, T, D) + st_.view(B, T, D)
                mo = st.to(x.dtype)
            else:
                mo = blk.mlp(z)
            m0_stream = mo.float()
            ...
            if 1 in mlps:
                ...
                if GLUE['on']:
                    b1, U1, V1 = GLUE[1]
                    st_ = st_ + b1 + ((X2 - xm1_) @ V1) @ U1.T
                st = SHIP['t1'][toks].view(B, T, D) + st_.view(B, T, D)
                mo = st.to(x.dtype)
            else:
                mo = blk.mlp(z)
...
def main():
    ...
    res['all4_noglue'] = round(ce2(ALLL, M4), 4)
    print(res, flush=True)

    # joint glue: rank-32 + bias per slot (slot0 input D; slots 1/2 input 2D)
    params = []
    for slot, ind in ((0, D), (1, 2 * D), (2, 2 * D)):
        b_ = torch.nn.Parameter(torch.zeros(D, device=DEV))
        U_ = torch.nn.Parameter(torch.zeros(D, 32, device=DEV))
        V_ = torch.nn.Parameter(torch.randn(ind, 32, device=DEV) * 0.001)
        GLUE[slot] = (b_, U_, V_)
        params += [b_, U_, V_]
    ...
    res['all4_glued'] = round(ce2(ALLL, M4), 4)
    GLUE['on'] = False
    print(res, flush=True)

    pa = res['all4_glued'] <= 3.35
    pb = (3.56 - res['all4_glued']) >= 0.10
    pc = (res['all4_glued'] - 3.1074) <= 0.30
    out = {'ce': res, 'glue_gain_vs_noglue': round(res['all4_noglue']
                                                   - res['all4_glued'], 4),
           'gap_to_attn_only': round(res['all4_glued'] - 3.1074, 4),
           'loss_curve': curve,
           'data_budget': {'steps': 300, 'batch': 8, 'lr': 3e-3, 'rows': 480},
           'pred_a_glued_le_335': bool(pa), 'pred_b_beats_mlp2only_10': bool(pb),
           'pred_c_gap_le_30': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"glued {res['all4_glued']} gain {out['glue_gain_vs_noglue']}")
