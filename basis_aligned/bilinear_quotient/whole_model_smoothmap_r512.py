...
OUT = PT + 'whole_model_smoothmap_r512_results.json'
NEVAL = 200; SEQ = 256; CONTENT_L = 15; K = 12; RTOK = 64; RPOS = 32; RPCA = 512; RIDGE = 1e2; NLAYER = 18
...
def main():
    ...
    _cc = content - content.mean(0, keepdim=True); Vpca = torch.linalg.svd(_cc, full_matrices=False)[2][:RPCA].T.contiguous()
    feat = torch.cat([wte[torch.tensor(cur, device=DEV)], wte[torch.tensor(np.where(prev>=0,prev,0), device=DEV)],
                      content @ Vpca, torch.ones(len(cur), 1, device=DEV)], 1)
