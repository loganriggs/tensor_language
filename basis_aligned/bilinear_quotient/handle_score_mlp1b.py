# handle_score_mlp1b: SELF-DERIVED CLASSES (S1490: mlp1's channel was UNSELECTIVE
# for mlp0's det/frag classes — handle scores are class-relative. Here the classes
# come from the channel ITSELF: project mlp1's token table onto channel directions
# 0-2, take each direction's top-40 tokens (freq >= 5) as a class mask, and re-run
# extraction/removal scored on THOSE classes. If the channel is selective for its
# own classes, per-site class discovery becomes a standard step of the handle lane;
# if not, mlp0's selectivity was special.
...
#   pred_a w8 keeps >= .30 of the effect on its OWN top class (class0, 7000).
#   pred_b selectivity on own classes >= 2.0 (class-keep / global-keep).
#   pred_c replicates on skip=2000 (sign + half magnitude for class0 keep).
...
OUT = PT + 'handle_score_mlp1b_results.json'
NR = 960
...
def main():
    ...
    # self-derived classes: token table of h1 -> project onto channel dirs 0-2
    ttsum = torch.zeros(50257, 8); ttcnt = torch.zeros(50257)
    for i in range(0, 480, 8):
        idx = FR2[i:i + 8, :-1].to(DEV).contiguous()
        h1 = run_to_h1(idx).reshape(-1, HD)
        sc = (((h1 - MU1) / RMS1) @ W8.T).cpu()          # [n, 8]
        toks = FR2[i:i + 8, :-1].reshape(-1)
        ttsum.index_add_(0, toks, sc)
        ttcnt.index_add_(0, toks, torch.ones(toks.shape[0]))
    proj = torch.where(ttcnt.unsqueeze(1) > 0,
                       ttsum / ttcnt.clamp_min(1).unsqueeze(1),
                       torch.zeros(1, 8))
    ok = ttcnt >= 5
    CLS = {}
    import tiktoken
    ...
    for d_ in range(3):
        p = torch.where(ok, proj[:, d_], torch.full_like(proj[:, d_], -1e9))
        top = p.argsort(descending=True)[:40]
        CLS[f'class{d_}'] = torch.tensor(top.tolist())
        print(f"class{d_}:", '|'.join(ENC.decode([t]) for t in top[:15].tolist()),
              flush=True)
    FRAG_IDS = CLS['class0']; DET_IDS = CLS['class1']    # reuse mask plumbing
    CLS2_IDS = CLS['class2']
    ...
    out = {'res': res,
           ...
           'pred_a_class0_keep_30': bool(pa), 'pred_b_own_selectivity_2': bool(pb),
