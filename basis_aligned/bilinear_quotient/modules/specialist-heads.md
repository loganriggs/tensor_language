...
## 2026-09-12 21:08 — head13.0 full source-cubic projection screen

Distinct from the earlier leading-value split: two frozen rank16 source-polynomial banks projected before physical O, with original QK norms/relative positions. On48existing regional prompts, native instrument holds but paired cue coverage is approximatelyzero (range−0.0147% to0.0053%); wholehead coverage1.41/2.36%. Replica effectrelativeRMS5.23/7.55% passes its subtest, but overall B/C fail for negligible cue transfer. Do not infer privatehead instability from whole27bank coefficient instability; earlier head13functions agreed~1.5%. No other-task absence claim; existing capitalized-committee interpretation untouched. [Receipt](../../polynomial_causal/CUBIC_BANK_HEAD13_SCREEN_V1_RESULT.json) · [Method note](../../polynomial_causal/FOLDED_CUBIC_TRUST_PILOT_V1_MATH.md).
