# deep_mlp_ngram: §1182's deep-band extension of the n-gram MLP map — mlp5/7/9/12/15@W=64,
# each ALONE (no stack). Long-range value rides attention pooling (§1180), which stays live
# outside the replaced MLP, so deep entries should stay cheap.
# Registered predictions:
#   pred_a all five <= 0.03;
#   pred_b flat in depth (max - min <= 0.02);
#   pred_c alternative: any >= 0.10 -> a genuinely context-hungry MLP (localized values-side
#          long-range consumer) — informative either way.
#
...
OUT = PT + 'deep_mlp_ngram_results.json'
NR = 16; W = 128
...
MLG = {L: {'on': False, 'idx': None, 'cache': None} for L in (5, 7, 9, 12, 15)}
MLW = {5: 64, 7: 64, 9: 64, 12: 64, 15: 64}
...
def main():
    ...
    for Lm in (5, 7, 9, 12, 15):
        CONDS[f'm{Lm}ng'] = (set(), False, False, {Lm})
    hk = m.transformer.h[0].mlp.register_forward_hook(m0_bigram_hook)   # inert (never enabled)
    hkL = [m.transformer.h[Lm].mlp.register_forward_hook(mk_mlpL_hook(Lm)) for Lm in (5, 7, 9, 12, 15)]
    ...
    names = [f'm{Lm}ng' for Lm in (5, 7, 9, 12, 15)]
    cost = {c: round(CE[c] - CE['base'], 4) for c in names}
    vals = [cost[c] for c in names]
    out = {'n_rows': NR, 'W': W, 'ce': CE, 'cost_vs_base': cost,
           'sanity_base_matches_true': bool(abs(CE['base'] - CE['true_model']) <= 0.02),
           'pred_a_all_cheap': bool(all(v <= 0.03 for v in vals)),
           'pred_b_flat': bool(max(vals) - min(vals) <= 0.02),
           'pred_c_hungry_found': bool(any(v >= 0.10 for v in vals)),
           'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"CE {CE}")
    print(f"costs {cost}")
    print(f"sanity {out['sanity_base_matches_true']} | pred_a cheap {out['pred_a_all_cheap']} | "
          f"pred_b flat {out['pred_b_flat']} | pred_c hungry {out['pred_c_hungry_found']}")
