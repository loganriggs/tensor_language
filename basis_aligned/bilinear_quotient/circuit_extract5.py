# circuit_extract5: NARROW THE INPUT SET (S1514: layers 0-6 attention exact
# suffices for class recovery but is non-selective). Arms per circuit: bg /
# bg_ens / early_exact (heads + attention layers 0-3 exact) / late_exact (heads +
# attention layers 4-6 exact).
# Registered predictions:
#   pred_a early_exact (0-3) recovers >= .5x the half-depth reference (S1514:
#          .79/.72/.57) for >= 2 of 3 circuits.
#   pred_b early beats late for >= 2 of 3 (the inputs are early-stack).
#   pred_c a first selectivity gain: early_exact class recovery >= 1.2x its global
#          recovery for >= 2 of 3.
...
OUT = PT + 'circuit_extract5_results.json'
NR = 960
...
def main():
    ...
    for cname, spec in CIRCUITS.items():
        ...
        for nm, kw in (('clean', {}), ('bg', {'bg': True}),
                       ...
                       ('early_exact', {'bg': True, 'ens_exact': True,
                                        'exact_layers': frozenset(range(4))}),
                       ('late_exact', {'bg': True, 'ens_exact': True,
                                       'exact_layers': frozenset(range(4, 7))})):
    ...
    for cname in CIRCUITS:
        ...
        recs[cname] = {}
        for arm in ('bg_ens', 'early_exact', 'late_exact'):
            e = res[cname][arm]
            recs[cname][arm] = {k: round((b[k] - e[k]) / max(b[k] - c[k], 1e-6), 4)
                                for k in c}
        print(cname, recs[cname], flush=True)
    HALF_REF = {'question': 0.7895, 'semicolon': 0.7238, 'close_paren': 0.5698}
    n_a = sum(1 for cn in recs
              if recs[cn]['early_exact']['cls'] >= 0.5 * HALF_REF[cn])
    pa = n_a >= 2
    n_b = sum(1 for cn in recs
              if recs[cn]['early_exact']['cls'] > recs[cn]['late_exact']['cls'])
    pb = n_b >= 2
    n_c = sum(1 for cn in recs
              if recs[cn]['early_exact']['cls']
              >= 1.2 * max(recs[cn]['early_exact']['global'], 1e-6))
    pc = n_c >= 2
    out = {'res': res, 'recovery': recs, 'half_ref': HALF_REF,
           'pred_a_early_half_2of3': bool(pa), 'pred_b_early_beats_late': bool(pb),
           'pred_c_selectivity_12': bool(pc),
