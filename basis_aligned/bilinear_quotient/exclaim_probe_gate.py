# exclaim_probe_gate: BACKLOG #7's FIRST INSTANTIATION (§1363) — a mode-labeler probe
# gate for capabilities with no token-computable window. Three threads block on this
# (exclaim-specific kit §1350, ellipsis kit §1360, backlog #7).
#
# §105-SAFE DESIGN (capture and apply in the same code path): the probe is FIT on the
# ROUTE-ONLY hybrid stream — the kit's own base state — captured at the entry to block 3
# during the route forward on DISJOINT fit rows, and APPLIED at the same site in the
# same forward implementation. Never fit on the full model's stream.
#
# Probe: ridge regression on the rms-normed L3-entry stream -> label "an '!'-token
# occurs within the next 16 positions". Threshold set on fit rows for ~10% positive
# rate. Gate: positions with probe score above threshold; band a05 + pair (17.2, 17.3)
# live inside the gate, v1-route elsewhere (§1316 grain).
#
# Arms: route | pair_solo | band05 ungated (the §1350 generalist anchor, 0.860/0.880) |
# probe-gated band05+pair.
#
# Registered predictions:
#   pred_a THE GATE KEEPS THE CAPABILITY: gated kit keeps >= 60% of band05's TARGET
#          increment over route.
#   pred_b THE GATE SHEDS THE GENERALIST BILL: gated kit sheds >= 65% of band05's
#          ELSEWHERE increment over route.
#   pred_c THE MODE IS STREAM-LEGIBLE: probe AUC >= 0.75 on held-out fit rows.
...
OUT = PT + 'exclaim_probe_gate_results.json'
...
def fwd_arm(idx, arm, vmeans, ymeans, gatemask, capture_l3=None):
    ...
    for L, blk in enumerate(H):
        if L == 3 and capture_l3 is not None:
...
def main():
    ...
    for i in range(0, NFITP, 8):
        ...
        xs = F.rms_norm(cap[0], (D,)).reshape(-1, D)
    ...
    pa = t_inc_gate >= 0.60 * max(t_inc_band, 1e-6)
    pb = e_inc_gate <= 0.35 * max(e_inc_band, 1e-6)
    pc = auc >= 0.75
