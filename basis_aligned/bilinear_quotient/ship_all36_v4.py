# ship_all36_v4: K=3072 UNIT PLANKS (S1539: log-linear until saturation; price
# rises to 171 Mbit/module — measuring where the ship's price-performance knee is).
...
#   pred_a ship36_v4 <= 4.00.
#   pred_b beats v3 (4.1214) by >= .10.
#   pred_c fresh-row band <= .07.
...
OUT = PT + 'ship_all36_v4_results.json'
NR = 960
...
def main():
    ...
    pa = res['ship36'] <= 4.00
    pb = (4.1214 - res['ship36']) >= 0.10
    pc = abs(res['ship36_fresh2000'] - res['ship36']) <= 0.07
    out = {'ce': res, 'delta36': round(d36, 4),
           ...
           'pred_a_le_400': bool(pa), 'pred_b_beats_v3_10': bool(pb),
           'pred_c_fresh_le_07': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
