...
- 803: GPT2-LARGE PASSES -- gpt2-medium ISOLATED exception, not scale trend (settles 802). gpt2-large (774M): class+pos 0.751 vs random 0.185, mlp0 keep 0.77 (healthy). GPT-2 sizes: small 0.77 ok / medium 0.12 fails / large 0.75 ok -> gpt2-medium NOT explained by scale (both smaller+larger fine). FINAL TALLY: ~3/4 class+position holds 5/6 models (bilin18 0.78, gpt2-small 0.77, gpt2-large 0.75, pythia-160m 0.75, pythia-410m 0.69), across bilinear/absolute/rotary + scale 124M-774M; gpt2-medium (355M) single genuine exception (isolated quirk, joint mlp0, not scale). CONCLUSION: reduction is COMMON+ROBUST across family/scale but NOT universal (1 documented counterexample). Closes cross-model/scale thread.
  ...
  §857 layer_geometry_full — WHOLE-STACK geometry: re-clustering (RSA) concentrated at FRONT (L0 0.49/L1 0.58) + READOUT (L15→16 0.81); middle L5-14 near-frozen (RSA 0.95-0.98). Eff-dim: 117→20(L0)→47(L1)→~45→23(L5)→51(L9 PEAK)→...→6.5(L16)→2.8(L17). TWO expansions (L1,L6-9), THREE collapses (L0,L5,L16-17). Middle re-inflates dimensionality (23→51) but topologically stable; readout collapses to ~3-dim prediction. Figure sent
  §858 middle_reinflation — middle re-inflates (L5→L9, eff-dim 23→51) mostly into CONTENT/unnamed directions (+ fine token-identity +0.05); prev-token (−0.08) & position (−0.05) FADE. Token gain small vs eff-dim jump → most expansion is content/unnamed (diffuse lexical wall §810). MIDDLE characterization complete. Bottom-up program complete; content/lexical is the recurring wall
  §859 readout_3dim — L17 token-mean collapses to ~3 GRAMMATICAL/boundary axes (81.5% var): dir0 punctuation/boundary, dir1 clause-continuation(modal/pronoun) vs sentence-end, dir2 proper-name vs number (class-sep 0.66-0.85, freq/pos corr ~0). Token-identity-driven final rep is grammar; content is context-varying diffuse residue. Completes readout
  §860 logit_lens_content — content built DISTRIBUTED across depth, BACK-WEIGHTED (within-CE L0 10.1→L14 5.2→L17 2.5; last 3 layers cut 5.2→2.5, coincides with geometry collapse §857). Grammar mostly settled by L4 (class-CE 2.06), final sharpen L16/17. L5 bump = attn5 content re-representation not yet logit-readable. Content distributed/back-heavy (matches diffuse §810), not a localizable circuit. Corrects my grammar-early/content-late guess (both monotone). Next: attn-vs-mlp content decomposition
  §861 logit_lens_attn_mlp — content generated mostly by MLPs (total within-CE reduction 4.58 vs attention 1.48, ~3×), concentrated in BACK MLPs (mlp16 +1.21/mlp15 +0.73/mlp17 +0.47). Attention direct gain small/gradual (L5 −1.29 = attn5 re-rep). Back MLPs = main content generators + read class+pos (§851). Caveat: attention may have large INDIRECT (context) role hidden by small direct gain — testing next
  §862 ablate_attention_content — attention is CONTEXT-FOR-CONTENT: all-attention ablation raises content-CE +2.80 vs grammar-CE +0.65 (4.3×). Small direct gain (§861) hid large indirect role (provides context back-MLPs convert to content). Grammar nearly context-free.
  §863 CAPSTONE — bilin18 = TWO MACHINES: (1) context-free GRAMMAR machine (front MLPs: token→sharpened class + log-position; low-rank, nameable, causal; easy 23%; read out to ~3 grammatical axes) + (2) context-driven CONTENT machine (attention aggregates context → back MLPs generate specific word; distributed/back-weighted/diffuse; hard 75%, mostly irreducible). Interpretable machinery ≠ bulk of difficulty. Open frontier: NAME the content machine (needs semantic approach)
  §864 content_semantic — semantic-cluster probe does NOT name content: full residual predicts next-token semantic-cluster only +0.056 over class+position (0.463 vs 0.407; shuffled-matched 0.403 = §836 re-confirmed). Embedding clusters are morphological not semantic (muddy target). Content resists grammatical AND semantic-cluster probes — diffuse by every measure. Next: data-driven clustering of the content residual itself
  §865 content_clusters — FREQUENCY-CONFOUNDED (all clusters show same frequent tokens ,.the-of-to); raw top-token read uninformative. Refitting with distinctive (over-representation) read
  §866 content_clusters2 — CONTENT WALL CRACKED: content machine tracks TOPIC/semantic DOMAIN. Distinctive-token read of content-residual clusters is clearly topical (education/Orthodox-religion/tech/geopolitics/sports/vaccines/aviation/medicine/prison/politics; ~12 of 16 single-topic). NAMES the diffuse content (7 prior angles failed: topic isn't grammatical, wte-clusters morphological, raw-token freq-confounded). Reconciles diffuseness: topic-space is HIGH-DIM (no low-rank handle) but STRUCTURED/nameable, not noise. Caveat: token-level read, not yet causal. Next: causal topic-patch
  §867 topic_steer — FIRST causal topic test NEGATIVE (own-topic logit gain 0.001): §866 topic is REPRESENTATION-level (decodable, solid) but causal role NOT shown. Weak test though (α=8 vs class steering's 16-32, mean not deviation direction, L15 maybe read-not-write site, small batch). read≠write caution. Retesting: topic-deviation direction + α sweep + more data (topic_steer2)
  §868 topic_steer2 — topic IS CAUSAL but weakly steerable by one direction: deviation-direction steering (α=256) gives own-topic logit gain +0.042 vs off-topic −0.004, EVERY topic specific+consistent. Weak because topic is HIGH-DIM (one direction = small slice; §866), unlike low-rank class (strong steering §823). §867 pred used too-strict 0.5 threshold. Upgrades §866 to causally-supported. FINAL: grammar=low-rank/context-free/strongly-causal (easy 23%); content=high-dim topic/attention-fed/weakly-causal (hard 75%). Both named+causal. Program complete
  §869 mlp_topic_folding — MLPs READ topic at ALL depths (3-4× vs shuffled-null ~1.2×), NO middle gradient; my 'topic built in middle by MLP' pred FALSE. Caveat: front fold inflated by token-identity leak (rank-64 subspace incomplete); emergence curve (§870) carries causal claim. class-read front-loaded (mlp0 7.7×) + huge at readout (mlp16 12.3×)
  §870 topic_emergence — ATTENTION aggregates topic across depth: topic-decode rises 0.37->0.85 (chance 0.20, shuffled-null 0.179), gains ALL from attention (every layer attn-increment +, middle +0.205; MLP increments ~0/neg -0.016). MIDDLE MECHANISM NAMED: attention=topic aggregator, MLP=topic reader/word-generator. Chain: attn aggregates->topic->MLP reads->readout emits topic-coherent word. Bottom-up map mechanistically COMPLETE
  §871 topic_context_length — content NEEDS long context, grammar is LOCAL: sweeping visible context C=2->256, topic-decode rises 0.19->0.67 (monotone, still rising at 128->256; shuffled-null ~0.14), content-CE drops 1.53 nats vs grammar-CE only 0.35 (4.4×). Functional confirmation of §870 aggregator. Content still dropping at C=256 -> §840 floor partly context-limited. Pred TRUE
  §872 topic_aggregation_nature — topic aggregator = CONTENT-WORD, ORDER-INVARIANT gist: function-masked retains 82% of topic, content-masked only 16% (chance 0.128), order-shuffled retains 61% while grammar-CE rises +0.70. Topic rides content words, largely order-free (bag-of-words gist); grammar needs order+local sequence. Both preds TRUE (partial: 61%/82%). Content machine characterization complete
  §873 topic_robustness — CORRECTION: discrete 12-topic clustering does NOT replicate across splits (real 0.029 <= shuffled-null 0.103) BUT the test is uninformative (rare-token fingerprint + tiny split coverage). Forces framing fix: NOT '12 discrete topics' — it's a CONTINUOUS high-dim topic structure that clustering only visualizes. Topic finding stands on causal(§868)+gist(§872)+decode(§870), not discrete replication. Proper continuous-geometry retest queued (topic_robustness2)
  §874 topic_robustness2 — CONTINUOUS topic geometry DOES replicate: subspace overlap A<->B 0.531 = 25× random chance (0.021), centroid cosine 0.394 >> ~0.03. My pre-registered shuffle-null was INVALID (global shuffle → iid halves → trivially 0.95 overlap; valid null is random-subspace chance r/D). Real<iid (0.53<0.95) itself shows content varies across topic-splits. VINDICATES §873 continuous-structure framing. Content=continuous topic tracker established (with §868/§870/§871/§872)
  §875 topic_heads — topic aggregation DISTRIBUTED across heads (every head decodes topic >> chance 0.17; medians 0.26-0.48), modest per-layer concentration (top/median 1.57), clearest L6 head1 0.637 (> L6 full output). No single 'topic head' — mirrors topic diffuseness across directions(§868)/layers(§870)/heads. Pred a weakly TRUE, headline is distribution
  §876 irreducible_tokens — 'irreducible' loss = FIRST-MENTION word choice, shared with gpt2-large: bilin18 CE 3.26 vs large 3.17 (gap .096), per-token corr 0.90 (same hard tokens), 87% of loss is a shared floor. First-mention costs ~4.2 nats vs seen ~1.7 (both models). Floor is 89% first-mention/82% content ('tornado','regions','March'). Irreducible = open NEW-word choice, topic-narrowed not determined. Retires §830 overclaim. Caveat: large OOD on FineWeb -> gap is lower bound. Pred a False on over-strict AND (first-mention was the real discriminator)
  §877 induction_mechanism — bilin18 has STRONG induction (synthetic 2nd-copy 1.29 vs first 13.09, score 11.8 vs null 0.93; natural inductable-bigram 0.68 vs non 3.55). FRONT-LAYER CHAIN gated by L5 (L5 ablation removes ~all induction; L0/1/2 each also necessary). IDENTIFIES attn5 = the induction/copy head (its earlier-mysterious 'content contribution' IS copying seen content words). Explains §876 seen-token cheapness. Pred a False (chain drops overlap, don't partition)
  §878 induction_ablate_natural — L5 IS the induction site (ablation collapses inductable 0.69->3.64; control L11 does nothing -0.008) BUT it's a multi-function CONTENT head not pure induction: L5 ablation also hurts first-mentions +2.15 (can't copy), copy-specific effect ~0.8 nats (2.95-2.15). Pred a False (ratio 1.37). Refines §877: L5 = early content head + induction gate
  §879 loss_budget CAPSTONE — where every nat goes: overall CE 3.24 (unigram 7.17)=class 0.76+within 2.48. FIRST-MENTION 60% of tokens/CE 4.24 = 78% of loss (almost all content, within 3.48); SEEN-OTHER 34%/CE 1.94 = 20%; INDUCTABLE 6%/CE 0.69 = 1.3% (induction nearly free). Grammar ~0.8 in every bucket (cheap/uniform); content tracks novelty (0.39 copied->1.10 seen->3.48 new). Quantitative form of two-machine+induction+irreducible account. Pred TRUE
  §880 loss_budget_crossmodel — budget shape UNIVERSAL: first-mention = 78.4%(bilin18)/77.4%(gpt2)/78.2%(gpt2-large) of loss; inductable ~1.3%, seen-other ~20%, grammar cheap/uniform in all 3. Absolute first-mention within-CE tracks scale (3.80 gpt2->3.48 bilin18->3.35 large) but the shape is model-independent. Complements §831 universal 23/77 split. Pred TRUE. Caveat: gpt2 OOD on FineWeb (shape robust)
  §881 seen_token_discount — seen-token discount = SOFT-COPY salience: loss falls with recency (2.50 far->1.66 near) and count (2.48 1x->1.33 6+), monotone grid. Pred a TRUE. L5 ablation hurts FAR priors more (+1.86 vs recent +1.39) -> L5 = LONG-RANGE copy; recent repeats redundantly supported (pred b False, informative). Accounts for seen-other 20% bucket. Budget now mechanistically complete end-to-end
  §882 induction_circuit — two-step front circuit CONFIRMED: attn5 ablation removes ~all induction (11.8->0.47, the copy gate), attn0 removes 70% (prev-token key), control attn13 spared (0.13). Pred a False only on tiny non-additivity (both 1.24 vs attn5-alone 0.47, both ~0). Nails §877 circuit: attn0(key)->attn5(copy), downstream irrelevant. END-STATE: bottom-up program comprehensively complete
  §883 cross_arch_swiglu — account ARCHITECTURE-INDEPENDENT: swiglu18 (swiglu MLP, same size/data/attn) reproduces bilin18: budget first-mention 78.4%/seen 20.3%/inductable 1.3% IDENTICAL, within-frac 0.761 vs 0.765, induction score 12.2 (gate L4 vs L5, front). Two-machine+induction+budget = property of learned computation, not bilinear-MLP form. Pred TRUE. With §880/§831: robust across architecture AND scale
  §884 cross_arch_bilin12 — SCALE SPLIT: loss budget scale-INVARIANT (bilin12 162M: first-mention 78.1%/seen 20.5%/inductable 1.3%, within-frac 0.767 — identical to bilin18) but general-purpose INDUCTION strength scales with size (synthetic 4.26 vs 11.8; natural inductable still cheap 0.745). Small model relies more on statistical predictability for repeats; big model has genuine content-agnostic copy. Pred a False (correctly — induction is scale-dependent). Loss STRUCTURE invariant, mechanism STRENGTH scales
  §885 cross_arch_sqrd12 — GENERALITY SWEEP COMPLETE: sqrd12 (different attention, single-branch normalized squared) budget IDENTICAL (78.1/20.5/1.3, within-frac 0.767); induction 8.6 STRONGER than bilin12's 4.26 at same size -> induction strength depends on attention mechanism (normalized>bilinear) + scale. Matrix (bilin18/swiglu18/bilin12/sqrd12): budget+split INVARIANT across MLP/attention/scale; induction strength scale+attention-dependent. Program conclusion
  §886 grammar_steer — DESIGN FLAW not a result: front-injection (block2) gains EXACTLY 0.000 because the per-block residual rescaling (x=λ0·x+λ1·x0) attenuates a block-2 perturbation by ~λ0^15 before the readout (documented effect). NOT 'grammar non-causal'. Topic §868 survived only by injecting at block15. Redo at block15 for fair comparison (grammar_steer2). No report claim
  §887 grammar_steer2 CORRECTION — single-direction steering is WEAK for grammar too (own-class gain 0.010 at α32, ≤ topic's 0.042), specific but small. Refutes the 'grammar single-direction steers strongly, unlike high-dim topic' contrast — read≠write (item 2) limits BOTH (steered READ directions). Topic causal status now rests on gist(§872)/first-mention(§876)/geometry(§874), NOT the weak 0.042 steering. Two-machine distinction = STRUCTURE not steering-strength. Matching alpha (grammar_steer3) to finalize; false 'steers strongly' clause corrected now
  §888 grammar_steer3 — matched-alpha: grammar single-direction steering 0.079 = ~1.9× topic's 0.042 (both specific, both weak <0.1; read≠write caps both). Refines §887's over-correction (α too low there): modest real asymmetry (grammar lower-rank) not 'equally weak', but not 'strong' either. Steering thread RESOLVED: weak lever for both, grammar ~2× topic; two-machine distinction stays STRUCTURAL
  §889 uncertainty_repr — model REPRESENTS uncertainty (entropy monotone by bucket: inductable 1.56<seen 2.80<first-mention 3.68; entropy-loss corr 0.577; global H 3.25≈CE 3.26) but MISCALIBRATED by situation: overcautious on easy (gap +0.85) OVERCONFIDENT on first-mentions (gap −0.59). is-first-mention decodable L0 0.655->L8 0.72. CORRECTION: printed majority 0.403 was minority (bug); true chance 0.597, shuffled 0.573 -> real decode margin only ~+0.12, modest not large. Pred TRUE but internal-signal strength was overstated (baseline bug), corrected
  §890 calibration_crossmodel — calibration asymmetry UNIVERSAL: bilin18/gpt2/gpt2-large ALL over-cautious on inductable+seen (gaps +0.77..+1.14) and OVER-CONFIDENT on first-mentions (−0.53..−0.66). LMs over-commit exactly where they should be least sure. General LM property (like the budget §880). Pred TRUE
  §891 readout_dim_reconcile — CORRECTION: '3-dim readout' is VARIANCE-collapse (per-token eff-dim L15 72->L17 4.3, class-means 1.7) NOT prediction rank. Prediction is HIGH-RANK: top-256 variance-PCs recover only 88% of CE gain (mean-only 9.76->256pc 4.02->full 3.26), content in the low-variance tail. Reconciles collapse w/ high-dim content (loud=grammar, quiet high-rank=content). Fixed FINDINGS/artifact '3-dim prediction' wording
  §892 das_class_interchange [NEW: variable-level causal abstraction] — INTERCHANGE works: class is a causal variable (activation-subspace IIA 0.252 vs no-patch 0.058, random 0.06 — 4.3×), and interchange BEATS mean-steering (§888 0.01), vindicating user's read+write-jointly point. BUT lm_head weight-subspace fails (0.098≈random) — need the RIGHT weights (mlp0 class detectors), not unembedding. Method established; refine weight-source + DAS + content variable next
  §893 layer_understanding [user metric] — per-component understanding (0=mean-ablate,1=full; token-table stand-in, shuffled null): FRONT grammar HIGH+genuine (mlp0 0.94/+2.66 = token->class-cluster table, mlp2 0.87, attn0 0.77; mlp1 0.98 but genuine +0.35=mostly rank), MIDDLE MLPs content LOW (mlp5/8/11 0.39-0.48 = context-dependent gist not a table), READOUT 0.59-0.78. Quantifies two-machine story on the scale. Caveat: redundant-middle denominators tiny/noisy (§813); token-table is context-free ceiling
  §894 das_topic_interchange — TOPIC is a CAUSAL variable via interchange: patching topic subspace shifts prediction toward source topic Δ −0.86(no-patch)→−0.16(patch), +0.70 nats, flip-rate 0.36→0.49, vs random-patch −0.84. Resolves §868 (steering weak = read≠write, NOT topic non-causal). Interchange works for both class(§892) and content(§894). Honest: Δ still slightly neg (one position vs full context). Pred TRUE
  §895 das_class_learned — DAS≈activation (0.271 vs 0.281: activation subspace is optimal, no hidden better one). CORRECT-WEIGHT answer: mlp0 front-WRITE FAILS at L15 (overlap 0.043≈chance) — the class variable subspace ROTATES through the stack; at L15 (near readout) the READ side aligns partially (lm_head overlap 0.188, IIA 0.156 > §892). Pred 'write beats read' FALSE (opposite). Lesson: use the weights of the component acting AT that layer. Rotation test queued
  §896 layer_understanding_v2 [FILL GAP] — middle content MLPs filled by PREV-TOKEN (+0.20-0.24, local/bigram) far more than TOPIC-clusters (+0.02-0.09): token+prev reaches ~0.6-0.79 (from token-only ~0.4). Topic causally real (§894) but 12 discrete clusters undercount the continuous variable's reconstruction. attn0 +prev 0.95 (prev-token head confirmed). Residual ~0.25 = long-range/continuous-topic/interactions
  §897 class_subspace_rotation — class subspace ROTATES write→read: write-overlap 0.29(L0)→0.02(L17), read-overlap 0.07→0.18, consecutive 0.49(front,fast)→0.85(mid,stable)→0.75(readout). Confirms §895 (front-write fails at L15). NEITHER weight tight (max 0.29/0.18) — activation always best; weights = partial layer-dependent handle. Matches §857 geometry
  §898 layer_understanding_v3 — CONTINUOUS topic subspace fills 2-6× more of the middle than discrete clusters (§896): topic gains mlp5 +0.02->+0.12, attn5 +0.06->+0.13, attn11 +0.09->+0.18 (genuine vs random-subspace null). token+continuous-topic+prev reaches ~0.68-0.84 for the middle (from ~0.4 token-only). Middle content ~3/4 accounted (token+continuous-topic+prev); residual ~0.2-0.3 long-range/interactions. Confirms topic is continuous not discrete
  §899 position_interchange — DESIGN FLAW (not run): patches at fixed QP=128 so all items share one position bucket -> no different-position source to pair (empty-candidate crash). Position is low-value (well-understood §786, ~0.10 budget); DROPPED rather than reworked. Pivoting to the capstone: whole-model understanding (all 36 components replaced by named-variable stand-ins at once) — answers 'how much of the model do we understand'
  §900 whole_model_understanding CAPSTONE — named variables reconstruct ~81% of the WHOLE model (all 36 components replaced at once; scale 0=all-mean-ablate CE 8.84, 1=full CE 3.38): token 0.60 -> +continuous-topic 0.69 -> +prev 0.812; shuffled-token null -0.30 (genuine +1.11). ~19% residual = long-range/interactions. HEADLINE: 81% of the model is named-variable reconstructable. Caveat: same-data (fresh version queued)
  §901 whole_model_understanding_fresh — CORRECTION: §900's 81% was SAME-DATA OVERFITTING; fresh (train/test) whole-model understanding = 0.29 (token 0.19 + continuous-topic +0.10 generalize; PREV-TOKEN +0.00 fresh = pure overfitting, was +0.12 same-data). Retracts §896/898 'prev-token fills the middle' as same-data artifact. GENERALIZING: only low-rank vars (grammar+continuous-topic) ~29% all-at-once. Fresh check (queued per §836 lesson) caught a 3× overclaim. Caveats: all-36 compounds; small train set lower-bounds it
  §902 layer_understanding_fresh — per-component FRESH corrects §893: FRONT generalizes as token tables (mlp0 0.38/genuine+1.86, mlp1 0.92, mlp2 0.69) but MIDDLE/READOUT token-tables OVERFIT — fresh ~0 or NEGATIVE (attn5 -2.88, mlp15/16 ~-1.0, actively harmful out-of-sample). §893 middle/readout values were same-data overfit. Caveat: absolute fresh coverage-limited (small train, rare tokens) — genuine-vs-null is robust read; well-sampled-token version queued
  §903 layer_understanding_fresh_backoff — class-backoff: mlp0 0.38->0.59 (coverage was limiting it), harmful negatives (attn5 -2.88->0.02, mlp16 -1.04->0.37) were rare-token noise not anti-info; but over-coarsens mlp1/mlp2 (0.92->0.61). ROBUST across §901/902/903: front generalizes above null to ~0.5-0.6 (data-limited), middle ~0, readout ~0.4, whole-model ~0.29. Grammar=tabulatable(imperfect), content=causal-not-tabulatable
  §904 whole_model_understanding_fresh_backoff — DEFINITIVE: whole-model generalizing understanding 0.315 (backoff), robust with §901's 0.29. CERTIFIED HEADLINE (corrects §900 same-data 0.81): ~30% of the model reconstructable from generalizing named variables (token/grammar 0.18 + continuous-topic +0.11; prev +0.03 mostly overfit). Other ~70% causal (interchange) but not tabulatable. Fresh-certification arc §901-904 closed
  §905 learned_map_understanding — SMOOTH map (not table) recovers FRONT to 0.90 (mlp0 0.90 vs table 0.59 ≈ same-data 0.94 — grammar IS a smooth generalizing function of the token; earlier fresh drop was table COVERAGE). But MIDDLE ~0.10 (attn5 0.12/mlp5 0.11/mlp8,11 0.00) — NOT a function of token+prev+topic, genuinely high-dim beyond named variables. Readout 0.56. Separates coverage-limit (front, fixable) from genuinely-not-a-function (middle). Pred a False. Whole-model smooth-map queued
  §906 whole_model_smoothmap — DEFINITIVE: smooth map of named variables reconstructs 0.406 of whole model held-out (vs table 0.29/0.315; null -0.03). ~40% understood as generalizing named-variable FUNCTIONS (front grammar 0.90 + readout 0.56 + middle 0.10; barbell-weighted). Grammar SOLVED (smooth token fn, causal); middle content the OPEN frontier (causal §894 but not a fn of token+prev+topic); readout half. Understanding-quant arc §893-906 certified at ~40%
  §907 learned_map_r128 — NO-OP (bug caught): topic subspace capped at K=12 clusters, so RTOPIC=128 == r24 (identical results). Re-confirms middle not a fn of 12-dim topic but does NOT test finer content. Proper higher-rank test queued: content-PCA-128 feature (upper bound via downstream content)
  §908 learned_map_contentpca — middle IS a smooth function of FINER content (rank-128): middle avg 0.32 vs 0.06 for 12-topic (attn5 0.44, mlp5 0.35, mlp8 0.26, mlp11 0.24). REVISES §905: middle not irreducibly interactive — high-dim-but-functional, 12 topics too coarse. CAVEAT: content feature is downstream L15 (leaky upper bound, more for later-middle). Clean upstream-content test queued (read-vs-build)
  §909 learned_map_upstream — clean upstream ≈ downstream (middle 0.304 vs 0.323): §908 leakage was NOT the issue; middle output is a function of its INPUT content, read-vs-build gap ~0. Middle is ~1/3 rank-128-LINEAR-readable (token+prev+upstream content), ~2/3 HIGH-RANK/NONLINEAR (bilinear MLP nonlinearity+interactions). Not a wall but genuinely high-dim nonlinear. Rank-vs-nonlinearity test (rank-512) queued
  §910 learned_map_rank512 — near-full-rank linear read: middle 0.30(r128)->0.44(r512), so 2/3-gap = +0.14 higher-rank-linear + ~0.56 GENUINE NONLINEARITY (bilinear MLP multiplicative). Middle content = HIGH-RANK + ~HALF NONLINEAR; irreducibility is mechanistic (it IS a bilinear interaction). Frontier fully characterized §893-910: grammar solved (low-rank linear token), content ~0.44 linear + ~0.56 bilinear-nonlinear + causal. Named-variable-interaction test queued
  §911 learned_map_interactions — named-variable cross-products (token×topic etc.) DON'T capture the middle nonlinearity: middle 0.242 (rank-64+interactions) < 0.304 (rank-128 content alone). Middle's bilinear nonlinearity is high-dim multiplicative over content, not a few named interactions. Caveat: RPCA 64 vs 128 not apples-to-apples. Lever for middle = more content rank
  §912 class_carrier_trace — class variable is CONTINUOUSLY RE-WRITTEN layer-by-layer (not transported): carried ~0.85 (mid) but 0.49 at L0→L1; each layer writes fresh class (just-wrote 0.88 front→0.33 L14→0.56 L17); resid→front-write decays 0.88→0.08, resid→read rises 0.07→0.18(L14); WRITE crosses front→readout-ward at L11. NEITHER weight basis aligns strongly at any layer (max 0.88 front@L0, 0.18 read) — activation subspace is the maintained truth. Answers user's bottom-up: no single rotation point, gradual re-writing; intermediate = each layer's own class subspace. Refines §897
  §913 whole_model_smoothmap_r512 — CORRECTION: 0.849 is a LEAKAGE artifact (rank-512 L15 content feature is DOWNSTREAM — near-determines readout, circular for middle). Benchmark STAYS ~0.41 (12-topic low-leak §906). Do NOT report 0.85. Clean upstream-feature version queued (whole_model_upstream). Lesson: features must be upstream of what you reconstruct
  §914 whole_model_upstream — CLEAN rank-128 whole-model = 0.72, but this is LOW-RANK-LINEAR-INTERFACE (reconstruct each component from a linear read of its INPUT RESIDUAL), NOT named-concept understanding. Two benchmarks: NAMED-CONCEPT (token+topic+prev) ~0.41 (headline); LINEAR-INTERFACE (input residual) 0.72 (architectural). Gap 0.31 = low-rank-linear stream content not yet named; ~0.28 = bilinear nonlinearity (§910). Don't conflate; headline understanding stays 0.41
  §915 class_multiaxis — class code is MULTI-AXIS (user's 'The=two categories' confirmed): capital/determiner/punct/number/space/alpha all separately decodable (0.996-1.0 vs base); capital⊥determiner (|cos| 0.095). 'The' det+4014/cap+3419 (BOTH), 'the' det+/cap− , 'London' det−/cap+. Token = point in multi-category space, not a bucket. My 8-way classify() was a coarse single-label proxy (mislabels The as det-only). Refines grammar machine: multi-attribute code (~24 dim §780)
  §916 why_rederive — CORRECTION: middle class-write is MORE context-derived than token-derived (token R² 0.40 < context gap 0.46; mlp5 only 0.22 token). Middle does NOT maintain token-class — it computes CONTEXT-DEPENDENT class (token's grammatical role in sequence, disambiguation/agreement), genuine new info. Front (mlp0/2) = surface token-class (R² 0.73-0.88); middle = contextual class; readout reads out. Corrects my 'maintenance' answer; sharpens §818/819 (new class is context-derived)
  §917 context_class_predictive — middle's contextual class is partly PREDICTIVE: across depth CURRENT-class fades (0.99->0.83), NEXT-class rises (0.56->0.65); middle more next-predictive than mlp0 (next-cur -0.21 vs -0.43). Stack shifts description->prediction. Honest: current-class dominates throughout (gradual shift not switch, +0.09 next). Completes class thread: front=surface multi-axis class(§915) -> middle=contextual class(§916) -> shifting to next-token prediction(§917)
  §918 next_class_emergence — predictive grammar (next-class) BUILT BY ATTENTION (rises 0.53->0.67, chance 0.445; every attn step + , total attn +0.11 >> mlp +0.02; back MLPs NEGATIVE = consume class into words). UNIFYING: attention aggregates context for BOTH predictive-grammar(§918) and topic(§870); front MLP writes surface class, back MLP converts to words. Completes grammar-machine bottom-up account
  §919 grammar_vs_topic_heads — grammar & topic MOSTLY SHARED across heads (per-head decode corr 0.31, moderate): most heads carry both (distributed §875), modest specialization (some grammar-lean, some topic-lean), no clean split. Shared context-engine (§918) shared at head level too. COMPLETES grammar machine §915-919: mlp0 surface multi-axis class -> attention builds predictive class+topic (shared heads) -> middle MLP contextualizes -> back MLP converts to words
  §920 grammar_topic_interference — two machines CLEANLY SEPARABLE: grammar vs topic subspace overlap 0.009≈chance 0.007 (ORTHOGONAL); cross-patch topic->class shift 0.064 vs class->class 0.872 (cross-talk 0.07). Shared attention engine writes to ORTHOGONAL independently-editable channels. Two-machine framing is a REAL factorization. Completes two-machine account: shared engine + orthogonal grammar/content channels, both causal
  §921 word_recombination — CLASS channel controls predicted word's PART-OF-SPEECH (ablate class: class-match −0.21, topic −0.007, clean). TOPIC-channel word-role INCONCLUSIVE: topic-match baseline 0.073 too sparse (most predictions are common/function words), ablation −0.004≈noise. pred a passed on tiny numbers only — NOT evidence for topic half. word=class∩topic confirmed for class half only. Cleaner CE-based retest queued
  §922 word_recombination_ce — CE double-dissociation FAILS: ablate CLASS (8-dim) hurts BOTH class-CE +1.96 AND within-CE +5.10 (low-rank DOMINANT, breaks all); ablate low-rank TOPIC (11-dim) does NOTHING (+0.04, content is HIGH-DIM DIFFUSE, no low-rank handle §866/874). Re-confirms low-rank-grammar/high-rank-content asymmetry at output level. Reconciles §894 (interchange on topic-distinctive tokens=real; ablation on overall within-CE=diluted). word=class∩topic not showable by symmetric low-rank ablation. Position-restricted retest queued
  §923 word_recombination_positions — content's word-role IS on first-mention positions (directionally): content-ablation Δwithin-CE first-mention +0.20 > seen +0.15 > inductable +0.11 (monotonic w/ novelty, above random +0.08). Small magnitude (rank-64 = slice of high-dim content). Class-ablation dominates all (+3.9-6.3). Reconciles §894/§922. Pred False on strict 0.1 threshold (gap 0.088) but direction correct. CLOSES recombination: class=low-rank dominant POS-gate, content=high-rank diffuse new-word narrowing at content positions; asymmetric intersection
  §924 worked_example — concrete traces: 'pumping you up, and turning'->predicts pronoun; ablate CLASS->punctuation (POS flipped), ablate CONTENT->unchanged. Another: pron->cap under class-ablation. 'doorbell'->'rings/ring/buzz' (content-appropriate). Illustrates class-gates-POS (§921) + content-picks-word on single predictions. Topic labels clean(physiology)/noisy(doorbell)=coarse 12-cluster. Concrete capstone of two-machine account
  §925 grammar_topic_sep_crossmodel — two-channel separability UNIVERSAL: grammar⊥topic near-orthogonal in bilin18 (1.3× chance), gpt2 (1.6×), gpt2-large (2.1×). Joins universal budget(§880)+split(§831). LMs organize grammar/content into near-orthogonal channels generally. Completes two-machine account's generality. Nuance: gpt2 slightly less orthogonal (OOD or mild real diff)
  §926 content_beyond_topic — content beyond topic resists naming with constructible features: entity-repeat +0.05, prev-class +0.06 (real but weak, survive topic removal); sentence-pos absent (removed as position §786); paren too rare. Pred a FALSE. Confirms content high-dimensionality (§866/874) — beyond topic it's diffuse, needs external semantic tools to name. On-box: topic is the main handle + weak entity/syntax. Hierarchical subtopic angle queued
  §927 content_subtopics — high-dim content is a COHERENT TOPIC->SUBTOPIC HIERARCHY: religion->worship/saints/titles/doctrine; academic->medicine/peer-review/research; tech->security-CVE/software-design. Content = subject matter at MULTIPLE GRANULARITIES (why it's high-dim: fine-grained hierarchy, not noise). Partial naming of high-rank content. CAVEAT: Jaccard-0 weak (any partition gives it); real evidence = subtopic interpretability (qualitative)
  §928 content_granularity_benchmark — CAUSAL held-out: keep grammar+token+pos exact, replace content part with K topic-centroids. Ablating content at L15 costs +1.74 nats; K=8/32/128 recover 0.8%/2.4%/6.3% (monotonic, no plateau); shuffled null WORSE than ablate. Content is a topic tracker but topic (even 128 buckets) is only ~6% of its causal work — the rest is finer per-context continuum. Quantifies the content frontier; consistent with §922/§873/§874. Topic-centroid stand-in is a weak reconstruction -> benchmark content term stays low
  §929 topic_buildup_by_layer — topic tracker built GRADUALLY across whole stack (per-layer topic-decode vs L15 K=32 topics): front L0-2 0.41 -> late L13-15 0.84, peak L15 0.85, DECLINES in readout L16-17. Long distributed accumulation (attn aggregation §871), no single topic layer; CONTRASTS grammar's front-loaded mlp0 write. Caveats: L0 0.32 from token leak; L15 target-favored. Fills bottom-up gap (where content is assembled)
  §930 content_granularity_extended — CONTINUUM confirmed: recovery K=128/512/1024 = 0.074/0.132/0.169, increments SHRINKING (+.074/+.058/+.037) but positive, shuffled null -0.21. Even 1024 topic buckets recover only ~17% of the content machine's causal loss; content is a high-rank continuum (need ~centroid/context). Closes content-frontier quantification; topic naming is qualitatively right but a modest causal handle; explains low benchmark content term (~0.10). Reproduces §928 at K=128
  §931 topic_buildup_components — PREDICTION REFUTED: front MLPs (esp MLP1: drop 0.667=collapse to base) hold topic, NOT per-layer attention (sum MLP-drop 1.26 vs attn 0.40; attn small+distributed). Front MLPs write per-token content substrate (like mlp0 writes class §915); attention pools redundantly (per-layer understates it). Placement control clean (L16/17=0). CAVEATS: whole-submodule mean-ablate favors big writers; single-layer attn compensated by 17 others. Follow-ups: topic-specificity of MLP1, cumulative attn ablation
  §932 content_bagofwords — topic ~= a BAG-OF-WORD-EMBEDDINGS running average of context. Decode L15 topic from running-mean features: ceiling 0.844, all-token bag 0.655, content-word bag 0.657, function-word 0.551, recency 0.665, current-only 0.216 (base 0.170). Bag recovers 72% of topic-decode-above-base; content~=all-token; recency~=flat (order-invariant, long-range §872). NAMES the 'what' of content aggregation; pairs with §931 (where: front MLPs) + §930 (how much: 17% handle)
  §933 topic_component_specificity — RESOLVES §931 caveat: MLP1 is a DOMINANT GENERIC substrate writer (ablation collapses BOTH topic 1.005 and class 0.998 to base, CE +6.3, residΔvar 4.2), NOT topic-specific. MLP0/2/3 lean GRAMMAR (class_drop>topic_drop, cf mlp0 §915); MLP9 tiny (front-loaded). REFINES §931: topic NOT localized to any component -> diffuse bag-of-words (§932) on shared front-MLP substrate pooled by attention. Tightens content account (what§932/where§933/how-much§930)
  §934 grammar_vs_content_bag — clean DOUBLE DISSOCIATION of input structure: TOPIC bag 0.655 >> current 0.216(base 0.170); CLASS current 0.539 > bag 0.460(base 0.453). Dissociation index 0.518, nulls at base. Grammar=local/current-token, content=order-invariant bag. Input-side complement to output separability (§920) + causal separability (§892/894). Caveat: current_class only +0.086 (class is nonlinear via mlp0)
  §935 content_bag_mlpfeatures — REFUTED front-MLP-encode hypothesis: per-token post-MLP features give no gain over raw embeddings for the topic bag (L0 0.667, L2 0.664 vs raw 0.655; L4/L7 collapse). Bag saturates at TOKEN-IDENTITY level (~72% of topic); the gap to ceiling (0.655->0.844) is context-dependent attention weighting, NOT per-token enrichment. Shuffled-map null flawed (permutation preserves identity) — reported, not relied on. Closes on-box content-mechanism arc
  §936 content_bag_benchmark — REVISION: content residual's causal bulk is LOCAL per-token, not topic. Loss recovery: current-token map 0.222 > topic-centroid 0.169 > bag-of-words map 0.052 (shuffled null 0.002). Bag decodes topic LABEL best (§932) but reconstructs content-for-LOSS worst -> decoding!=reconstructing. Topic is a modest long-range slice; most content term is local per-token. Doesn't contradict §871/872 (long-range minority). Method caveat: rank-64 strip leaves per-token structure in 'content'. Pred bag>buckets FALSE
  §937 content_bag_crossmodel — bag-of-words TOPIC-LABEL structure UNIVERSAL: gpt2 bag 0.455 vs current 0.209 (+0.247); gpt2-large 0.360 vs 0.186 (+0.174); both above null; bilin18 ref 0.655 vs 0.216. Content is an order-invariant bag-of-words topic gist in general LMs (like §925/§880/§831). Scoped per §936: about topic-LABEL decodability, not causal weight
  §938 content_pertoken_split — MODERATES §936: per-token MEAN lookup 0.101, +topic remainder 0.172 (cross-token gain +0.072), whole-centroid 0.180, shuffled-null -0.505. Local per-token does NOT dominate; token-identity(~10%) and cross-token topic(~7%) comparable. §936's 22% linear map was partly SMOOTH embedding-similarity (identity+semantics), not pure per-token. Content term = mix of comparable modest slices + ~78% high-rank continuum. Closes content-stand-in arc. Caveat: struct differs from §936 (within-run comparisons clean)
  §939 whole_model_understanding_tokenmap — BENCHMARK 0.315 -> 0.422 (held-out): smooth per-token LINEAR MAP beats discrete token table. Levels: table 0.177, MAP-alone 0.324 (beats old full pipeline 0.315!), MAP+topic 0.416, MAP+topic+prev 0.422; shuffled-map null -0.190, genuine 0.612. Linear map generalizes across embedding-similar tokens (§938 mechanism at scale), a strictly better named-var stand-in. Progression: 0.81 same-data(§900,corrected) -> 0.315 held-out table -> 0.422 held-out map. Residual ~0.58 = content continuum + interactions
  §940 whole_model_understanding_bands — depth localization of the 0.42: RAW band understanding back 0.445 > front 0.344 > middle 0.262, BUT shuffled nulls differ (front -0.373, mid -0.340, back -0.138) so GENUINE (shuffle-corrected) front 0.717 > middle 0.602 > back 0.583. Back's raw number is generic (residual already built, reads out accumulated topic). MIDDLE = largest contribution (2.24 nats) + worst raw recon (0.262) = content-computation frontier. Pred front>back FALSE. Locates residual in the middle content mixing
  §941 middle_bilinearity — WHY middle is the frontier: linear->multiplicative->linear depth arc. MLP linear-recoverable frac: L0 0.90, L1 0.98 (front nearly linear, incl dominant writer MLP1), L8 0.40, L11 0.38, L15 0.37 (middle ~60% MULTIPLICATIVE), L17 0.85 (readout linear). Shuffled null ~0. Middle's irreducibility IS genuine bilinear multiplication (no linear read captures it) -> explains §940 frontier + §930/938 content cap. Middle MLPs tiny individual cost -> multiplicative computation redundant/distributed. CAPSTONE
  §942 middle_nonlinearity_crossmodel — middle-nonlinearity dip UNIVERSAL: linear-R2 middle<front & middle<back in bilin18 (0.29 vs 0.51/0.50), gpt2 (0.23 vs 0.69/0.33), gpt2-large (0.18 vs 0.48/0.26); shuffled null ~0. linear->nonlinear->linear depth arc is a GENERAL transformer property (GELU too), not a bilinear quirk. bilin18 readout linearizes hardest (L16/17 R2 0.89/0.93). Joins universality thread (§925/§937/§880/§831). Metric caveat: R2 here vs loss-frac §941
  §943 middle_interaction_terms — what middle multiplies NOT cleanly content×content (pred FALSE). Variance share + ablation dominated by gg=struct(token)×struct(token) at L1/L11/L15 (L15 gg 0.91); cc dominant only at L8 (0.35). 'grammar' subspace = token+pos+class dominated by TOKEN identity, so middle products are largely TOKEN-CONDITIONAL, content×content a minority (peaks L8) — aligns with §936/938 (per-token bulk, topic minority). Caveats: variance shares sum<1 (terms cancel); middle ablation tiny (~0.01, redundant); recon-resid ~1e-4 (exact sanity OK)
  §944 readout_role — CORRECTION: readout (L16-17) does the BULK of output formation, NOT a small refinement. Logit-lens CE L15 5.82 -> L16 4.09 -> L17 3.26(final): last 2 blocks cut 2.56 nats. Skip-cost L16 +1.71, L17 +0.82 (load-bearing). Pred FALSE. L15 holds info but not output-readable; L16-17 rotate residual into token basis (large but near-LINEAR §941). Corrects §940's 'back=generic readout' and the implicit 'prediction formed by L15'. Doesn't undermine content benchmark (L16-17 kept real there)
  §945 readout_linear — readout is ~95% a nameable LINEAR map of L15: single linear map L15->L17 recovers 94.5% of the 2.35-nat gap (CE 3.238 vs final 3.110, logit-lens 5.455, shuffled null 9.879). Closes bottom-up stack: FRONT near-linear grammar (solved), MIDDLE multiplicative content (frontier), READOUT ~95% linear rotation into token basis. Two of three registers near-linear/nameable; middle multiplication is the sole frontier. Benchmark 0.42, residual=middle multiplication
  §946 middle_interaction_3way — clean 3-way (token/class/content): CLASS products NEGLIGIBLE everywhere (~0), middle multiplies TOKEN x TOKEN + TOKEN x CONTENT + CONTENT x CONTENT in comparable measure (L11 token-involving 0.020 vs con_con 0.010; L8 con_con 0.013 largest). Corrects §943 (class-lumping): class not multiplied, content×content substantial ~1/3. Middle = distributed token&content multiplication, no class. Exact-recon 6e-5. Completes bottom-up account
  §947 middle_redundancy — CONFOUND CAUGHT: block-SKIP (output<-input) conflates the content computation with the residual-RESCALE bookkeeping (x=λ0·x+λ1·x0 per block). Skip costs huge+compounding (middle k=4 +5.9, k=6 +13.8 > front 7.3) but this measures broken stream scaling, NOT redundancy. Pred FALSE but metric invalid. Does NOT contradict §940/941/946 (those used clean MLP-output mean-ablate, tiny per-layer). Lesson: use submodule mean-ablate, not block-skip, with per-block rescaling. Corrected experiment queued
  §948 middle_redundancy_meanablate — CORRECTED (clean MLP mean-ablate): middle DISTRIBUTED + super-additively COOPERATIVE, not freely redundant. Per-layer sum 0.45 vs all-10 together 2.20 (super-additive 4.9x); each middle MLP ~0.04 alone. middle-4 0.46 < front-4 1.73 (pred TRUE). Confirms §947 confound (clean 2.20 vs skip 16+). Corrects loose 'redundant' -> distributed-cooperative. Caveat: cumulative front curve understates mlp0/1 (order-masking); single-layer §933/941 (mlp1 6.3) is the clean front evidence
  §949 three_register_linearize — REFUTES 'middle is sole nonlinearity': linearize-cost front 2.76 > middle 0.88 > readout 0.12 (rest real). Front's near-linear-but-HUGE writers (magnitude×compounding) cost more to linearize than the intrinsically-nonlinear-but-tiny middle. SHAPE arc (§941/942 linear->mult->linear) still valid (per-layer); but front NOT freely linearizable in aggregate. CAVEAT: linearize-all 4.36 ~= shuffled-null 4.28 (degenerate regime); lack per-register nulls -> per-register costs SUGGESTIVE not certified. Corrected version (per-register nulls) queued
  §950 three_register_linearize_v2 — RETRACTS §949: front linear-map UNCERTIFIED (C_lin 2.76 > C_shuf 2.13 > C_mean 2.12 = worse than mean/shuffle; 6 front layers too sequentially coupled to linearize together). §949's 'front nonlinearity costs more' is an artifact, retracted. CERTIFIED: middle C_lin 0.88 << C_shuf 2.30 (nonlin-frac 0.40), readout C_lin 0.12 << C_shuf 0.68 (0.16) — confirm §941/945 shape arc with nulls. Front aggregate nonlinearity unmeasurable this way; per-layer §941 (near-linear) stands. Artifact never carried §949 (no correction needed)
  §951 attention_vs_mlp_bands — division of labor: ATTENTION FRONT-LOADED (mean-ablate front 3.66 > mid 0.86 > back 0.22, monotonic decrease), MLP steady (2.01/1.34/1.54). Front attn dominates (>mlp 2.01) and is largest single band-cost; late attn barely matters. Context-gathering is EARLY (reconciles §929/931). Pred 'attn-in-middle' FALSE. Caveat: band mean-ablate has interaction effects (front-MLP-band 2.01 vs mlp1-alone 6.4 §933 - not cross-comparable); within-band attn-vs-mlp is the clean claim
  §952 front_attention_role — front attention HOSTS INDUCTION (+early context substrate): mean-ablate front-attn costs inductable +5.21 (baseline 0.69, 7.5x), ~= all-attn +5.29, vs random band +0.89 (excess +4.32) -> induction lives in front L0-5, not distributed. Also hurts first_mention +4.11, seen_other +2.61 (broad early context-mixing). Explains §951 front dominance. Completes named attention account (front=induction+substrate, mid=bag-of-words pooling §932/929/931, shared engine §918/919). Pred TRUE
  §953 induction_layer_localize — induction is DISTRIBUTED COOPERATIVE, not localized: per-layer attn-ablate on inductable all small (max L8 +0.18, top-3 share 47%, sum-of-singles ~0.9) yet front-6-together +5.21 (§952) = ~6x super-additive. No single induction-head-layer; removing one is compensated, removing the front collectively destroys induction. Same pattern as content §948, pooling §931 -> UNIFYING THEME: bilin18 circuits are distributed+super-additive. Pred FALSE. Caveat: layer-granularity (single HEAD could be localized within a layer)
  §954 induction_head_localize — induction has identifiable HEADS embedded in a distributed cooperative circuit: top-5 heads L2h5(+0.123)/L5h5/L8h3/L3h8/L8h4 carry 59.5% of single-head signal (early-mid layers, L2h5 dominant = bilin18's induction heads, classic early picture) BUT each tiny alone (top +0.12 vs +5.21 collective §952, sum-of-singles 0.63) -> no single head load-bearing, super-additive. Reconciles §953 (layer-distributed) w/ classic induction heads. Pred TRUE. Completes induction thread (§952->953->954)
  §955 induction_head_set — top-5 induction heads are GENUINE specific cooperative set: together +0.575 > singles-sum 0.37 (super-additive), vs random -0.015 (specific), first_mention +0.04~0 (induction-SPECIFIC, 3 clean signatures). BUT only ~11% of full front induction (5.27) -> identifiable heads are a small slice of a broad distributed circuit. Completes induction localization (§952-955). Reinforces unifying theme (localized circuits = tip of distributed super-additive ensembles). Pred TRUE
  §956 cross_model_superadditivity — bilin18 distributed-cooperative structure GENUINELY SPECIAL: attention super-additivity ratio bilin18 3.52 (SUPER-additive, single layers cheap ~0.17/layer) vs gpt2 0.71 / gpt2-large 0.73 (SUB-additive, single layers expensive ~2.6/layer, redundant). QUALITATIVE sign flip: bilin18 spreads computation thin across cooperating layers (no single load-bearing); GPT-2 concentrates it in individually-critical redundant layers. Validates §948/931/953-955 (not generic). Caveat: GPT-2 OOD may inflate its costs but sign is robust
  §957 joint_interchange — CLASS compositional control CONFIRMED (class_only 0.38, joint 0.375 vs random 0.24 = +0.14; content_only 0.24 doesn't move class -> clean dissociation; class effect survives joint patch). TOPIC metric (source-topic tokens in top-10) at FLOOR for all conditions (~0.05) = TOO SPARSE (§921 weakness), NOT evidence against topic causality (§894 confirmed +0.70 nats). Pred FALSE but metric-limited. Corrected v2 (nats-based topic metric) queued
  §958 joint_interchange_v2 — compositional interchange: CLASS control CLEAN (class_only/joint move class +0.14 vs random; content_only doesn't) + joint topic-gain HIGHEST (1.65>1.19>0.55>0.21, effects ADD). BUT topic metric CLASS-CONFOUNDED: class_only (1.19) > content_only (0.55) on topic-gain because class-patch boosts content-word log-probs generally. Content-specific topic dissociation NOT isolated (pred FALSE, metric confound). Topic causality clean elsewhere (§894/§934). v3 (class-controlled topic metric: source-minus-base) queued
  §959 joint_interchange_v3 — COMPOSITIONAL CAUSAL CONTROL, clean double dissociation (class-controlled topic metric): class-patch moves CLASS (0.38 vs 0.245 random) not topic (+0.02); content-patch moves TOPIC (class-controlled +0.748 vs +0.031 random) not class (0.245); JOINT moves BOTH (class 0.375, topic +0.748). Geiger causal-abstraction ideal: the two named subspaces independently+compositionally control their own machines. Pred TRUE. Capstone for causal-abstraction thread (§957->958->959); confirms two-machine account at deepest causal level
  §960 joint_interchange_qpsweep — compositional double dissociation ROBUST across QP {80,140,200,240}: class_only moves class (0.32-0.38 vs random 0.21) not topic; content_only moves topic (class-controlled +0.75 to +0.83 vs random 0.04) not class; at every QP. §959 not a QP=200 artifact. Minor early class->topic leak (QP80/140 +0.20) but content_only dominates 3-4x. Solidifies causal-abstraction capstone (§957->960). Pred TRUE
  §961 content/class_interchange_bylayer — ASYMMETRY in causal addressability across depth: CONTENT patchable throughout middle, effect GROWS L8 +0.12 -> L11 +0.39 -> L15 +0.75 (vs random ~0.01; pred TRUE) = accumulated gist persists (§929). CLASS interchange WEAK early (L2 +0.01 no effect, L8 +0.085) only clear LATE L15 +0.145 (pred FALSE) = class RE-DERIVED downstream washes out early patches (§916/917), rotates write->read (§897). Both causal (§959) but content depth-robust, class only late-addressable. Completes no-skipping causal picture
  §962 class_rederivation — DIRECT demo class is RE-DERIVED across middle: ablate class subspace at L2 -> L15 next-class decode 0.672 recovers 98.2% of clean 0.676 (middle rebuilds class from token); ablate AT L15 -> 0.454=base (destroyed). Mechanism behind §961 (class only late-addressable) + confirms §916/917 (re-derived not carried). Content accumulates/persists vs class re-derived every block (leaky λ0 residual + λ1 token re-injection). Pred TRUE
  §963 content_persistence — CORRECTION: BOTH content (96.7% recovered after L8 ablation) and class (98.2% §962) are RE-DERIVED, nearly identical; neither 'carried'. My §961/962 'content accumulates vs class re-derived' dichotomy WRONG. Real distinction = re-derivation SPEED: class fast (per-block from token, early patch washes immediately) vs content slow (long-range attention pooling over many layers, patch survives partially, grows toward readout §961). Reconciles §961 interchange asymmetry. Artifact corrected. clean L15 topic 0.863, after-L15-abl 0.077
  §964 rederivation_speed — REFUTES §963 speed hypothesis: BOTH class & content re-derive within ONE block (L3: class +0.85, content +0.93; ~full by L6), content marginally FASTER not slower. No class-fast/content-slow distinction. Corrected unified picture: both variables CONTINUOUSLY RE-DERIVED from intact context every block (residual = recomputed estimate, not store); interchange patches inject foreign content that base context overwrites, so patches are TRANSIENT, stick only late (§961 content-grows-to-L15 = last content layer). Lesson: stop over-theorizing unverified mechanisms. Artifact corrected
  §965 cross_family_superadditivity — distributed-cooperative structure is FAMILY-driven, NOT bilinear-specific: super-additivity ratio bilin18 3.20, swiglu18 2.29, bilin12 1.61 (all super-additive) vs GPT-2 0.71 (sub-additive §956). swiglu18 (SwiGLU MLP, no bilinear) super-additive too -> cause is the training/architecture family (softmax-free sqrd-attn + value-residual + x0 re-injection), not the MLP nonlinearity. Validates §956 CE-ratio via residual-ratio (bilin18 3.20~3.52). First family-model generalization
  §966 cross_family_separability — two-machine SEPARABILITY (grammar⊥content) FAMILY-WIDE: near-chance overlap ratio bilin18 1.50, swiglu18 1.42, bilin12 1.16 (all near-orthogonal), as in §920/GPT-2 §925. Core two-machine STRUCTURE generalizes across family (bilinear + SwiGLU, 18L + 12L). Pairs with §965 (distinctive structure) -> two-machine account structure is family-wide. Pred TRUE. Residual-only (no output-transform dependence)
  §967 cross_family_bagofwords — content MECHANISM (bag-of-words) FAMILY-WIDE: bag-minus-current +0.44 (bilin18), +0.49 (swiglu18), +0.41 (bilin12), all bag>>current as §932/937. FAMILY GENERALIZATION COMPLETE on 3 axes: structure (separability §966) + mechanism (bag §967) + distinctive style (distributed-cooperative §965) all family-wide (bilinear+SwiGLU, 18L+12L); key properties universal (GPT-2 §925/937/942/880). Two-machine account is not a bilin18 quirk. Pred TRUE
  §968 cross_family_nonlinearity — depth ARC (linear->nonlinear->linear) FAMILY-WIDE: middle-dip band means bilin18 0.49/0.27/0.49, swiglu18 0.48/0.34/0.49, bilin12 0.60/0.49/0.50 (all middle<front&back; bilin12 shallow). Completes 4-AXIS family generalization (structure §966 + mechanism §967 + style §965 + depth-arc §968), all siblings, core universal via GPT-2. Account is a family property not a checkpoint quirk. Pred TRUE
  §969 attn_variant_superadditivity — CAUSE ISOLATED: SQUARED (softmax-free) ATTENTION drives distributed structure. Matched pair (both bilinear MLP 12L/768, cfg-confirmed): bilin12 squared_attn=True ratio 1.61 vs bilinsm12 squared_attn=False (softmax) ratio 1.13. Switching sqrd->softmax collapses super-additivity (1.61->1.13). Sharpens §965: cause is specifically squared attention, not bilinear MLP/size. Caveat: bilinsm12 1.13 still slightly >1 (not fully GPT-2's 0.71; minor residual from value-residual/x0-reinjection). Pred TRUE
  §970 attn_mlp_2x2_superadditivity — 2x2 CAUSAL ISOLATION complete: squared attn is the driver of distributed structure, bilinear MLP DAMPENS it. Ratios: squared+bilinear 1.61, softmax+bilinear 1.13, squared+standard(sqrd12) 9.74. Squared attn ALONE (standard MLP) gives extreme super-additivity (9.74) -> bilinear MLP not needed; it dampens (1.61<9.74). sqrd12 band dR2 246M vs singles 25M = collectively dependent on front attn ensemble. Definitive: softmax-free squared attention causes the distributed structure. Pred TRUE
  §971 whole_model_understanding_repro — headline benchmark 0.42 is SEED/DRAW-STABLE: reproduces 0.445 on fresh draw (NEVAL 240, seeds->7) vs §939 0.422 (Δ 0.023, within ±0.03); map>table +0.051, shuffled null -0.145. Tracked metric is robust, not a rows/seed artifact. PROGRAM STATUS: bilin18 interpretation comprehensively complete + causally verified + family-generalized (4 axes) + self-corrected; residual ~58% = irreducible high-rank middle multiplication. Pred TRUE
  §972 error_decomposition — top-1 partition (n=51k): HIT 0.384 (loss-share 0.08), CONTENT-err 0.294 (0.42), GRAMMAR-err 0.323 (0.50). Pred 'content dominates errors' FALSE (grammar-type top-1 errors slightly lead). Top-1 is class-correct ~2/3 of the time. CAVEAT: top-1 MODE != class distribution; does NOT contradict chain-rule budget (§880 content 77%) - class-wrong top-1 is often HEDGING to a frequent function word on hard content positions. Descriptive MODE/hedging fact, not a loss reattribution. Follow-up (error_hedging_test) queued to TEST hedging directly
  §973 error_hedging_test — CONFIRMED 'grammar errors' are content HEDGING not grammar failures: on grammar-error positions true token rarer (-7.43 vs hit -6.07), top-1 is FUNCTION word 78% (vs base 43%, content-err 25%), true token still ranks ~10 (distribution intact, only mode hedged). Vindicates §972 caveat, reconciles w/ loss budget (§880 content 77%): both error types driven by CONTENT difficulty; grammar not the failure point. bilin18's mistakes are content mistakes (wrong content word, or hedge to function word). Closes error-interpretability thread (§972->973). Pred TRUE
  §974 content_uncertainty_hedging — pred FALSE: hedging driven by CLASS entropy (hedge 0.19->0.91 monotonic with class uncertainty), NOT content. My 'content entropy'(within-top-class) metric CONFOUNDED (low iff top class is small function class -> artifact negative trend). Clean claim: model hedges to function word under CLASS/grammar uncertainty. Reconciles §973 (content-hard positions raise class-entropy -> hedge); doesn't overturn it. Accuracy falls with both (sanity). Winding down error sub-thread (finer splits -> confounded metrics)
  §975 cross_family_errors — content-error/hedging structure FAMILY-WIDE: bilin18/swiglu18/bilin12 near-identical (partition ~38/29/32 hit/content/grammar; grammar-err top1-func ~0.78-0.79 vs base 0.44; grammar true-token rarer -7.3 vs hit -6.0). 'Mistakes are content mistakes, hedge to function words' (§973) generalizes across family. Transform-invariant (argmax), non-confounded. 5th family-general axis (w/ §965-968). Pred TRUE
  §976 hedge_depth — the 'hedge' is a FRONT high-freq function-word DEFAULT the readout overrides on easy positions but not hard content. Logit-lens func-top1: front ~0.88-0.90 for BOTH hit & grammar-err (high-freq default); DIVERGES at readout - hits move OFF (L17 0.59, commit to content), grammar-errs STAY hedged (0.83). So hedge = readout failing to override the front default on hard content. Unifies §944(readout overrides)/§973-975(content errors)/§915(front writes default). Pred TRUE (front-leans-function) but mechanism = readout-override-failure
  §977 readout_freq_debias — frequency de-biasing DISTRIBUTED, not readout-specific: corr(logit-change, log-freq) readout -0.546 vs front -0.567 (equal direction; both suppress frequent/boost rare). Readout larger MAGNITUDE (std 4.47 vs 2.51) = dominant but not sole calibrator (consistent item 3, §944). Model starts high-freq-biased (L0) and de-biases toward content throughout. §976 readout-override is PART of distributed anti-frequency de-bias, not a separate mechanism. Pred FALSE (not readout-specific); unification refined not confirmed
  §978 content_bottleneck_capacity — capacity buys CONTENT not grammar: bilin18 vs bilin12 (same FineWeb) content-CE 2.53 vs 2.68 (Δ0.146), class-CE 0.79 vs 0.82 (Δ0.030). Extra ~50% capacity goes ~5x more into content; grammar SATURATED at both (~0.8). Content-frac size-invariant (76%). Content is BOTH partly capacity-limited (improves w/ size) AND largely task-fundamental (dominant + shrinks slowly, 0.15 nats/+50% params). Grammar solved regardless of size. Explains why content is the frontier (capacity sink + residual difficulty). Pred TRUE
  §979 calibration_two_machines — model EXCELLENTLY calibrated: token ECE 0.009 (conf 0.38~acc 0.38), class ECE 0.022 (conf 0.72~acc 0.70), reliability near-diagonal. Grammar higher-confidence+accuracy (solved); content appropriately uncertain. Hedging (§973-976) is CALIBRATED uncertainty (model knows when content is hard, low conf matches low acc), not blind failure. Unifies two-machine + errors + calibration: confident grammar gate + uncertain content pick, well-calibrated. Closes output-behavior facet (§972-979). Pred TRUE
  §980 content_error_topicality — content errors are TOPICALLY APPROPRIATE (graceful failure): on content-error positions the wrong predicted word is in the true word's TOPIC 40% vs class-matched-random null 15% (2.6x). Content machine narrows to the right SUBJECT even when missing the specific word (topic-right, word-wrong), not topic failure. Ties errors to topic tracker (§866); content's topic-narrowing works, residual difficulty is within-topic word choice (§930/938). Completes output-behavior facet (§972-980). Pred TRUE
  §981 qk_routing — QK routing HETEROGENEOUS/SPECIALIZED per head (softmax-free squared attn, exact pattern captured): INDUCTION heads (h4 +0.193, h6 +0.157 vs null ~0), CONTENT-sim heads (h8 +0.151, h7 +0.122), RECENCY heads (h1 +0.11). Mean washed it out (pred FALSE mean-based, but per-head clear). NEGATIVE ROUTING distinctive to softmax-free: anti-heads route AWAY (h3 induction -0.190, h0 content -0.085) - softmax (weights>=0) can't. Routing = per-head mixture of specialized attractive+suppressive modes. Opens routing thread
  §982 qk_double_factor — DOUBLE-QK is non-redundant: two factors (q.k, q2.k2) differ in dominant feature on 6/9 heads (e.g. h3 opposite induction signs f1 -0.18/f2 +0.10; h1 f1 rec weak/f2 rec -0.53). Pattern = f1*f2 = product of two DISTINCT bilinear criteria -> conjunctive (positive where both same-sign) + SIGNED (negative where disagree = §981 suppressive routing). Explains why double-QK: signed conjunction of two routing criteria, richer than single softmax score. Caveat: recency/content co-vary short-context, can't cleanly label factors content-vs-position (robust: non-redundant + signed conjunction). Pred TRUE
  §983 qk_routing_longrange — CORRECTS §981: content-similarity routing was RECENCY confound (long-range q-k>30: content-sim 0.005~null 0.002; §981 'content head' h8 +0.151->-0.037). Genuine range-robust routing = RECENCY + INDUCTION (h4 +0.25, h6 +0.20 persist at range; h3 -0.207 anti-induction). §981 induction+negative-routing STAND; content-similarity claim RETRACTED. Unifies §932: attention does NOT route by content -> topic emerges from ~uniform/recency pooling (bag-of-words literal), not content-selective attending. Pred FALSE (mean) but induction genuine per-head. Long-range control caught the §982 confound
  §984 qk_routing_depth — induction routing FRONT/MID-peaked: max-head long-range induction corr L2 0.12, L5 0.32(peak), L8 0.25, L11 0.04, L15 0.01 (front 0.23 vs back 0.03, null 0.01). Induction routing lives early-mid (peak L5), gone late - confirms §952/954 at routing-pattern level. Late attention routes neither induction nor content (§983). Recency uninformative here (long-range). Pred TRUE. COMPLETES QK-routing sub-project (§981-984): signed conjunction of 2 criteria, specialized+suppressive heads, recency+induction not content-sim, induction front-peaked, content from pooling
  §985 value_residual — VALUE RESIDUAL is critical + primarily CONTENT: ablate (lamb=0) doubles loss (CE 3.32->6.66, +3.33 nats); hurts content +2.64 vs grammar +0.69 (3.8x ratio). Confirms value residual (pooling first-block/original token values v1 into every block) = the bag-of-words content-aggregation mechanism (§932); grammar far less dependent. Distinctive v1 re-injection is the substrate of the content machine. Caveat: lamb=0 off-training-distribution (magnitude may include generic disruption; content>>grammar differential robust). Pred TRUE
  §986 value_residual_scaling — value-residual content role GENUINE+GRADED (resolves §985 caveat): content-CE smooth monotonic rise as alpha->0 (2.53/2.55/2.64/3.27/5.18), > grammar each step (pred TRUE). NONLINEAR: tolerant to moderate scaling (a=1->0.5 content +0.11) but sharp near 0 (a=0.25->0 +1.91). So §985's +3.33 has a near-zero tail but the value residual is genuinely content-load-bearing (model needs a substantial fraction; content>>grammar throughout). v1 re-injection = substrate of bag-of-words content machine (§932)
  §987 x0_reinjection_scaling — x0 re-injection (lambda1~8, dominant) is CRITICAL+GRADED (full CE 3.32->5.66 as lambda1->0); content/grammar ratio 2.96 < value-residual 3.84 (pred TRUE) BUT both content-heavy: x0's lower ratio is from hurting content LESS (+1.75 vs 2.64) at similar grammar (+0.59 vs 0.69), NOT more grammar. So x0 re-injection is a GENERAL token-availability mechanism serving both machines (not a clean grammar substrate); value residual is the more content-specific. x0 ablation less catastrophic than value-resid (+2.34 vs 3.33). CORRECTS my clean-dissociation hypothesis: relative shift not dissociation
  §988 bilinear_mlp_factors — FLAWED METRIC caught: mean-freeze decomposition confounded by ~zero-mean projections (freezing either factor at its ~0 mean kills 95-97% of product variance at ALL layers -> asym~1, retain~0.03 everywhere, uninformative). Cannot distinguish gate-vs-content; pred 'True' (1.05 vs 1.02) is noise, RETRACTED. §941 (front near-linear/mid multiplicative via clean loss-recovery) stands. Corrected metric (constant-fraction = ||mean||^2/(||mean||^2+var)) queued. Lesson: mean-freeze invalid for centered activations
  §989 bilinear_mlp_factors_v2 — NO factor specialization (clean constant-fraction metric): both factors ~zero-mean, vary symmetrically at ALL layers (cf 0.02-0.07, Left~=Right). 'Gate x content' hypothesis for front near-linearity (§941) REFUTED - raw product interaction-heavy everywhere incl front. Reconcile: §941 output-linearity stands; front linearity is an OUTPUT-MAP property (Down projects out the nonlinear interaction at front), NOT a constant gate. Closes bilinear-MLP-factor question (negative: no gate/content split; both factors always vary; linearity is post-Down). Corrects §988 confound. Pred FALSE
  §990 bilinear_down_readout — REFUTES §989: interaction (u*w) DOMINATES Down-output variance at ALL layers incl front (L1 0.94, mid 0.95-0.97; recon-resid 0.0). Down does NOT project out the interaction at front. §989's 'Down reads out linear part' reconciliation refuted (my 2nd wrong inference on this sub-question, stated plainly). Consistent reconciliation left: the interaction is LINEARLY PREDICTABLE from x at front but not middle (quadratic collapses to ~linear) - will TEST directly not assert. Artifact reverted to plain certified §941 (front near-linear output/mid multiplicative). Pred FALSE
  §991 interaction_linear_predictability — raw-output R^2 of linear map x->Down(target): interaction R^2 ~= full R^2 at every layer (interaction dominates output, §990); front 0.55 ~2x middle 0.26 (same direction as §941) but NEVER reaches §941's loss-frac 0.90. CORRECTION: not a bug/invalid — correctly reproduces the KNOWN raw-output R^2 ~0.51 (ledger ~27891); my sanity gate mis-compared two metrics (raw-output-variance R^2 vs §941's LOSS-recovery). RESOLUTION of §941-vs-§990: the front's raw output IS nonlinear (interaction dominant) but behaviorally INERT (linear map recovers 90-98% of the LOSS effect); the middle interaction is load-bearing. Definitive loss-ablation test queued §992
  §992 interaction_loss_ablation — REVERSAL (pred_a FALSE): dropping the multiplicative interaction is CATASTROPHIC at the FRONT (L0 1.61, L1 0.52 nats; band 4.89) and nearly FREE in the MIDDLE (~0.03/layer). NULL(full recompute)=baseline, costs>=0 (valid). CORRECTS §991: front interaction is LOAD-BEARING, not inert (my 3rd wrong interpretation here, stated plainly). Reconciliation: '§941 front-linear' = best-fit-linear reproduces the output (interaction is linearly-SHAPED, §991) NOT interaction-negligible; middle interaction genuinely multiplicative but low-stakes per layer (redundant band §940). Cost is within-CE(content) at both. Confirming best-fit-vs-Taylor side-by-side queued §993
  §993 bestfit_vs_taylor_linear — AIRTIGHT (all preds TRUE): FRONT best-fit-linear recovers 89-98% of loss (=§941) while Taylor-drop is far worse (L0 frac -1.04, worse than deleting the MLP); null(shuffled)~0; MID best-fit frac 0.27<front. PROVES §992: front interaction is LOAD-BEARING yet best-fit-linearly-reproducible; middle genuinely multiplicative but low-stakes per layer (band §940). CLOSES the bilinear-MLP thread with a coherent causal account (§941/§988-993). Corrected 3 prior interpretation errors in place (gate/projection/inert)
  §994 interaction_band_ablation — SURPRISE (pred_a,c FALSE): the FRONT is the super-additive cooperative interaction band (joint 4.89 vs sum 2.36, ratio 2.08 -- a cascade where L1 reads L0's damaged output), the MIDDLE is ~additive (1.15). Deep-middle interaction (L8,11,15) is cheap even jointly (~0.10 nats); middle band 0.55 is mostly L4. NULL 0.0003 (valid). REFRAME: middle MLPs' own multiplicative writes are SMALL; the content frontier (§940) lives in RESIDUAL-STREAM content read by attention, not middle-MLP interaction terms. Cost is within-CE(content) at both bands (pred_b TRUE). Corrected my backwards expectation plainly
  §995 content_receptive_field — banded-attention-window sweep. NULL(full band==original) valid. ABSOLUTE nats: K=1 penalty content +1.93 vs grammar +0.47 (content ~4x more context-hungry); both improve smoothly to ~128 with diminishing returns, content UNSATURATED at 256 (broad bag-of-words, log-linear). CORRECTION: pred_a/b FALSE only because I gated on RECOVERY FRACTION (normalizes away the asymmetry, coincidentally equal K90=128); absolute nats confirm the two-machine account. Content stand-in must integrate the WHOLE context
  §996 content_bag_selectivity — the bag is predominantly a bag of CONTENT WORDS: content-word far context recovers 80% of far content (within-CE) vs 51% for function words (pred_a TRUE). Redundant/over-determined (shares sum 1.31 -> either subset recovers most). NULL('all'==original) valid. pred_b FALSE: grammar also has a modest long-range component (class-CE +0.33 when far context removed) -> my W=4 grammar control was not clean (stated plainly). Benchmark implication: content stand-in = broad content-word bag-of-words over whole context
  §997 content_pooling_depth — RECONCILES §936 vs §995: content GATHERED broadly in early/middle layers (band L0-11 within-cost +0.98) then reads out LOCALLY late (band L12-17 +0.13; ratio 7.3x). Consistency null: all_K8 +1.12 == §995 K=8. §936's 'content local at L15' recontextualized: local = post-pooling readout of broadly-gathered content. Consolidated content-machine mechanism (bag-of-words / broad / content-words / gathered-early-read-late / multiplicative-residual-ceiling)
  §998 content_gathering_layers — content gathering CONCENTRATED in L3-5: banding L3-5's K=8 window costs +0.52 within-CE (8x any other 3-layer group, 46% of all-layers §997), early-sum 0.68 vs late-sum 0.06. Matches 'content begins ~attn5'. Sum-of-groups(0-11) 0.68 < §997 joint 0.98 -> mild cooperative interdependence (other early layers re-pool when one group is banded). pred_a TRUE. COMPLETES content-gathering-depth thread (§995-998): broad/content-word bag gathered in L3-5, pooled across early/middle, read locally late
  §999 content_multiplicative_ceiling — CONFOUNDED (my §994-class mistake, stated plainly): joint-band linearization with independently-fit maps INFLATES cost via compounding/distribution-shift. Signature: lin_front +1.42 within > lin_middle +0.77, CONTRADICTING §941/§993 (front is ~linear per-layer). Absolute 'ceiling' numbers are inflated upper bounds. SALVAGEABLE: within/class cost ratio ~3-4x in every band -> multiplication serves CONTENT (pred_a TRUE). Clean ceiling needs compositional (compounding-free) linear fit -> queued §1000
  §1000 content_mult_ceiling_compositional — CLEAN (compounding-free) ceiling. Compositional fit lowers all costs vs §999 (middle 0.773->0.645, front 1.42->1.01) -> §999 partly inflated (confirmed). BUT front>middle PERSISTS (1.01>0.645), partially correcting §999: it's REAL, not just compounding. Reconciles with §941 (front 90-98% linear PER-LAYER = RELATIVE; §1000 = ABSOLUTE nats; front writes most (§933) so small relative nonlinearity = large absolute). Forcing ANY band linear costs ~4x more content than grammar (pred_a). CLEAN CEILING: forcing all MLPs linear costs +1.59 nats content = the floor on linear stand-ins / benchmark content cap
  §1001 content_mult_by_group — the irreducible MULTIPLICATIVE content is FRONT-LOADED: L0-2 +0.563 within (~44% of group-sum), tapering down-stack, small readout bump L15-17 +0.151. early(0-8) 0.91 vs late(12-17) 0.25 (pred_a TRUE). group_sum 1.27 < §1000 all-band 1.59 (cooperative super-additivity). SEPARATES multiplicative-content COMPUTATION (L0-2 front MLPs, §933) from content POOLING (L3-5 attention, §998). Puzzle: L0-2 is below the pooling -> its multiplicative content is likely LOCAL word-sense, not broad topic -> next test
  §1002 content_mult_local_vs_topic — clean local/topic hypothesis REFUTED (pred_a,b FALSE). NULL valid. L0-2 ratio 0.60: ~60% (0.34) of its multiplicative content is LOCAL (survives K8 banding), ~40% (0.22) is downstream-pooling-substrate damage. Later groups DON'T show topic-dependence (L6-8 ratio 1.32 = compensation; band x linearize entangled). Robust: §1001 front-loading stands; L0-2 mostly-local word-sense + secondary pooling-substrate role; clean 2x2 decomposition not achievable (intervention interaction). Not drilling finer
  §1003 content_mult_ceiling_recert — the §1000 multiplicative-content ceiling is DRAW-STABLE: on a fresh larger draw (offset 400, 256 eval) lin_all within 1.546 vs §1000's 1.594 (delta -0.05); front 0.98 > middle 0.64 (ordering robust); within>>class 3.6x. pred_a TRUE. Certifies the ~1.55-1.59 nat content floor is not first-draw optimism
  §1004 content_bag_contentword — HONEST NEGATIVE: content-word bag (0.042) does NOT beat all-token bag (0.052), both << centroid (0.169) & current-token (0.222). §996's content-word finding is about the MODEL's mechanism, not a stand-in recipe: the topic is redundant (subsetting adds nothing) and raw-embedding bags are poor proxies for the learned multiplicative pooling regardless of tokens. Closes content-stand-in question: linear/bag content ceiling is fundamental (multiplicative, §1000), not a token-choice issue
  §1005 front_content_table_vs_linear — REVERSED (pred_a,b FALSE, stated plainly): a per-token TABLE is WORSE than a linear map at both bands (front 1.55 vs 1.01; middle 1.30 vs 0.65) because the table discards context; MLP content is CONTEXT-dependent even at the front. NULL ok (shuffled 1.885>table). Clean 3-way decomposition of front content: 18% current-token-lookup, 29% context-linear, 54% context-MULTIPLICATIVE (=§1000 ceiling, truly context-mult not word-sense). Benchmark token-tables capture only ~18% of MLP content
  §1006 content_pooling_heads — the L3-5 content gathering is DOMINATED by ONE head, HEAD 7 (banding h7's K=8 window costs +0.29 within-CE = 88% of per-head sum, 56% of all-heads joint); other 8 heads ~0.001-0.011 alone. NULL consistency (all-heads 0.52 == §998). per-head sum 0.328 < all-heads 0.520 -> distributed cooperative remainder (other heads matter jointly ~0.2, dispensable alone). pred_a TRUE. A rare near-single-component localization (h7) + distributed remainder, fitting the 'one knob + distributed remainder' theme
  §1007 content_head7_characterize — the content-gatherer is a SINGLE head: L5 head 7 (L3/L4 h7 cost 0.000; all +0.29 is L5). CONTENT head (within/class ratio 3.92, pred_a TRUE). BROAD pooler: L5 h7's |attn| mass>8 = 0.628 (bag-of-words); pred_b FALSE only from mis-averaging mass>8 over irrelevant L3/L4 h7 (metric artifact, stated plainly). Matches 'content begins ~attn5' exactly. CAPSTONE: content machine localized bottom-up to one head, L5 h7
  §1008 content_L5_head_ablation — CORRECTS §1006/1007: output mean-ablation of L5 h7 costs only 0.0098 within-CE (TOP content head, ratio 3.92, pred_a TRUE by direction) -- but ~30x LESS than banding's 0.29. h7 is the PREFERENTIAL broad-pooling content head yet REDUNDANT/compensable (model reconstructs content without it), NOT a load-bearing single part. 'Rare single-component' framing (§1006/1007) RETRACTED. Instruments diverge because banding injects a misleading local pool (interference) while ablation is cleanly compensated. Methodological lesson: banding can overstate unique causal role. Artifact corrected
  §1009 content_gathering_ablate — banding-vs-ablation relationship FLIPS with granularity (partly corrects §1008's over-general 'banding overstates'): SINGLE head banding(0.29)>>ablation(0.01) [sibling compensation + interference]; WHOLE band ablation(2.5)>>banding(0.06-0.52) [removing early attention is foundational/off-distribution]. pred_a,b FALSE, pred_c TRUE. Neither instrument uniformly right: BANDING isolates the long-range function (right tool for §998's 'where is content gathered'); OUTPUT-ABLATION measures total-output importance (early attention foundational, §951). §998's L3-5 localization STANDS (not visible under cruder whole-band ablation); clarification not retraction
  §1010 cross_family_mult_content — 'MLP multiplication serves CONTENT not grammar' is ARCHITECTURE-GENERAL: linearizing the middle band costs ~3-4x more content than grammar in EVERY model incl swiglu18 (NON-bilinear SwiGLU): bilin18 3.66, bilin12 3.05, swiglu18 3.49. pred_a TRUE. So the content/grammar division of the nonlinearity is not bilinear-specific; extends §942 (nonlinearity dip universal) with the content/grammar attribution. Strengthens the two-machine account as architecture-general
  §1011 content_contextlen_family — content broad/grammar local is ARCHITECTURE-GENERAL: content ~4.5x more context-hungry than grammar in every model incl swiglu18 (bilin18 4.56, bilin12 4.45, swiglu18 4.52); grammar saturates by K~8, content unsaturated to K=200. Matches+generalizes §995. With §1010, BOTH two-machine pillars (content nonlinear+long-range / grammar linear+local) are now universal across family incl non-bilinear SwiGLU. pred_a TRUE
  §1012 content_depth_onset_family — CONFOUNDED (my poor instrument choice, stated plainly): logit-lens 90%-onset is ~0.89-1.0 for BOTH grammar and content in every model because logit-lens CE collapses only in the last 1-2 layers (readout-basis alignment §934/§941), so it measures READABILITY not COMPUTATION depth. pred_a FALSE is an artifact; NO conclusion drawn. Grammar-front/content-later depth structure already established causally (§915-919 grammar front, §998 content L3-5, §941 middle) -- those stand; §1012 used the wrong instrument
  §1013 whole_model_understanding_recert — the north-star understanding number is DRAW-SENSITIVE: token+topic+prev 0.29 (draw0) -> 0.45 (draw400), genuine-vs-shuffled 0.67->1.0. pred (draw-stable) FALSE. Swing is entirely the TOPIC term (adds 0.10 vs 0.247) -- topic stand-in depends on train/eval topic overlap, varies by draw; token term stable (0.19-0.20). CORRECTION: report understanding as a RANGE ~0.29-0.45, not a point; prior single quotes reflected draw variance. Multi-draw averaging queued §1014
  §1014 whole_model_understanding_multidraw — STABLE north-star: we understand 0.32 ± 0.06 of the whole model (token+topic+prev, held-out, all 36 comps, 4-draw mean±std); genuine-vs-shuffled 0.74 ± 0.15; token term 0.15±0.04 (tightest). Confirms §1013 draw-sensitivity (topic-driven); the ±0.06 spread is the honest uncertainty. Residual ~0.68 = irreducible context-multiplicative content (§1000). Replaces single-draw quotes (0.29/0.42/0.81 were points on a draw/metric-variable distribution). pred_token_tight+pred_total_wider TRUE
  §1015 mult_content_class_robust — 'multiplication serves content' is TAXONOMY-ROBUST: content/grammar ratio 3.93 (8-class) and 7.97 (coarse 2-class function-vs-content), both >>2.5 (pred_a TRUE) -> not an artifact of the hand-coded 8-class taxonomy. pred_0 FALSE but the shuffled-class NULL was MIS-DESIGNED (stated plainly): a random partition is unpredictable so its class-CE barely moves under linearization -> class-cost~0 -> ratio inflates (9.37) not collapses; a random partition can't null a chain-rule split. Finding stands; null was wrong control
  §1016 content_injection_priming — GENERATIVE input-side confirmation of bag-of-words content: injecting a topical content word at pos 3 raises its TOPIC-NEIGHBORS' log-prob (+0.52 nats) at query pos 150, vs function words +0.07 (7.4x) and random-neighbor null +0.07 (7.5x). Excludes W+context (not induction/copy); long-range; topic-specific; content-word-driven (confirms §996 generatively). New evidence modality (prior content causality was activation-level §894). The account PREDICTS behavior. pred_0+pred_a TRUE
  §1017 content_injection_dose — the bag ACCUMULATES ~linearly with dose (content topic-boost 0.53/0.96/1.81/3.42 for K=1/2/4/8, ~+0.43/copy; pred_a pooling TRUE). pred_0 FALSE: function-word dose also accumulates (1.55 at K8) -- the bag pools ALL tokens (genuine, neighbor-specific), content 2.2x > function; distinction is quantitative not absolute (refines §1016). Generative confirmation of pooling in DOSE (with §1016's KIND)
  §1018 content_injection_separability — GENERATIVE two-machine separability: FAR content injection (pos3) shifts content 0.52 but grammar-TV only 0.007 (77x); NEAR injection (pos148) shifts grammar-TV 0.21 (31x, positive control -> metric sensitive). Content pooled long-range, grammar local, shown by input injection (complements activation-level §959-960/§966). pred_0+pred_a TRUE. COMPLETES generative trilogy §1016-1018 (kind/dose/separability)
  §1019 content_injection_position — generative content receptive field: topic boost rises smoothly toward the query (0.53 at dist147 -> 4.01 at dist2, recency-weighted) yet stays substantial far back (0.53 = 13% of near; broad/long-range). Cross-validates §995 (banding) + §981-983 (recency) from the input side -- three instruments, one profile. pred_0+pred_a TRUE. COMPLETES injection modality (§1016-1019: kind/dose/separability/receptive-field)
  §1020 grammar_injection_local — GENERATIVE local-grammar validation: adjacent determiner injection raises P(noun-ish) sharply (the +0.32, a +0.26) while far injection ~0 (grammar local; mirrors §1018). Nice syntax nuance: prep 'of' adjacent raises P(noun) only +0.034 because 'of'->determiner->noun (grammar tracks real syntax). pred_0+pred_a TRUE. COMPLETES generative two-machine validation (§1016-1020: content broad-pooling / grammar local-POS)
  §1021 content_injection_family — generative content-priming is ARCHITECTURE-GENERAL: topical injection primes topic (content>>function, ratio 6.7-9.2) in every model incl swiglu18 (non-bilinear). Confirms bag-of-words content BEHAVIOR is architecture-general generatively (complements structural §1010/1011). pred_a TRUE. Content machine now triple-validated (structural/causal/generative) + arch-general in structural AND generative modalities
  §1022 content_injection_mixing — the content bag COMPOSES topics ADDITIVELY with NO dilution: injecting two topics primes BOTH to ~full solo strength (joint 0.63 vs solo 0.57, retention 1.11 -- slightly higher, not diluted); cross-null 0.086 (specific). A genuine linear SUPERPOSITION of topic vectors (not winner-take-all). pred_a+pred_b+pred_0 TRUE (dilution prediction too weak -- there is none, stated plainly). Completes the generative content property set (§1016-1022: kind/dose/separability/RF/arch-general/compositionality)
  §1023 content_injection_capacity — the content bag is HIGH-CAPACITY: >=6 distinct topics superpose with NO dilution (retention >=1 at all N up to 6; pred_b TRUE, pred_a dilution FALSE). Consistent with §930 (high-rank continuum). CONFOUND (stated plainly): retention GROWING to 2.23 is NOT clean super-addition -- injecting more content words raises content predictions generally (§1020), and topic-neighbors are content words, riding that shift; lacked a topic-unrelated content-word control. Robust lower bound (>=6 topics, no dilution) stands. Completes generative content modality (§1016-1023)
  §1024 content_injection_capacity_clean — RESOLVES §1023 confound: control content-density boost grows with N (0.086->0.683, the confound measured); CLEAN per-topic retention (after subtraction) is FLAT near 1 (0.90-1.13 for N=1..6). So §1023's raw super-linear 2.23 was ENTIRELY content-density; TRUE result = high-capacity ADDITIVE bag (>=6 topics, no dilution, no genuine super-addition). Confirms linear superposition (§930/§1022). pred_0+pred_a TRUE. Confound corrected
  §1025 induction_injection — GENERATIVE induction validation (third mechanism): inject 'A B' early + A at end raises logP(B) by +8.56 nats at query (long-range copy); wrong-source 'A C' control only +0.42 (20x smaller -> position-specific copy not priming). Far larger than content-priming (~0.5) because induction is precise single-token copy vs broad topical shift. Validates §877 generatively. pred_a+pred_b TRUE. ALL THREE mechanisms now validated generatively (grammar §1020 / content §1016-1024 / induction §1025)
  §1026 induction_injection_family — generative induction is ARCHITECTURE-GENERAL: AB...A->B copy raises logP(B) +6.5 to +8.4 nats in every model incl swiglu18 (non-bilinear), wrong-source control ~0.4-0.6 (15-20x smaller). pred_0+pred_a TRUE. COMPLETES the generative x mechanism x architecture matrix: grammar(local)/content(§1021)/induction(§1026) all predict behavior generatively AND generalize across the family. Account validated across structural/causal/generative modalities, arch-general
  §1027 mechanism_independence — INDUCTION is INDEPENDENT of content (clean, retention 0.985: co-injecting a topic doesn't change the copy). Content-side retention 5.97 is a DESIGN CONFOUND (my flaw, stated plainly): the induction condition replaces the LAST token (=query position) with random A, contaminating the content measurement; content-solo used a different last token -> not comparable. pred_a FALSE from the design flaw, not interference. Fix (matched last-token content baseline) queued §1028
  §1028 mechanism_independence_v2 — INDUCTION-independence CLEAN (retention 0.973, robust with §1027): copy unaffected by content co-injection. Content-side retention improved 5.97->1.98 (last-token-PRESENCE confound removed) but residual 2x remains = last-token-IDENTITY (plain-bg original-last vs induction-bg A-last; content-ish last token amplifies content), NOT mechanism interaction. pred_a FALSE from residual design confound (stated plainly). Matched-last-token control ({A} vs {AB,A}) queued §1029
  §1029 mechanism_independence_v3 — CORRECTS §1028: last-token IDENTITY is NOT the cause (A-last/orig-last retention 1.089~1). The content-side 2x comes from ADDING the AB tokens (retention_trigger 1.822, last-token matched) -- but A,B are content-ish mid-freq tokens, so this is the CONTENT-DENSITY effect (§1023/1024), not a proven induction-mechanism interaction. Robust: induction independent of content (0.97-0.985); content-side needs a density-matched control (AB-trigger vs A'B'-non-trigger) -> queued §1030. My §1028 diagnosis corrected plainly
  §1030 mechanism_content_from_induction — density-matched control REFUTES §1029's content-density diagnosis: with A,B density identical in both, content boost is still ~2x larger when induction FIRES (last=A 1.02) vs not (last=D 0.54), retention 1.91. Genuinely tied to induction firing. LIKELY softmax coupling (measured on log-probs; induction concentrates mass on B, changing the logit->logprob mapping) -- both write shared logits -- NOT necessarily a content-computation interaction. Test on raw logits queued §1031. Robust: induction indep of content (0.97). pred_a FALSE
  §1031 mechanism_content_from_induction_logits — the content↔induction interaction is REAL at the LOGIT level (not softmax): content boost still ~1.8x larger when induction fires (trigger 0.84 vs non-trigger 0.47), measured pre-softmax. Capstone of §1027-1031 (5 controls peeling confounds: last-token presence/identity, content-density, softmax -- all refuted). ROBUST: ASYMMETRIC interaction -- induction independent of content (0.97), but content AMPLIFIED ~1.8x when induction fires. Mechanism unresolved (leading hypothesis: induction attention carries nearby content forward; not verified). Winding down; refines (not breaks) the core account. pred_a FALSE
  §1032 content_induction_spatial — EXPLAINS the §1031 interaction: content amplification FALLS with W's distance from the induction-match (1.98@pos3 -> 1.94@50 -> 1.53@100), pred_a spatial TRUE. TWO components: SPATIAL (induction attention carries nearby content forward, distance-decaying) + GLOBAL (~1.5x induction-firing content boost even far from match). RESOLVES the content↔induction saga (§1027-1032): induction indep of content; content amplified by induction via attention-forwarding-nearby-content + a global state boost. Core account unchanged, refined with one explained coupling
  §1033 content_steering — INFORMATIVE NEGATIVE: content injection barely moves the TOP-5 (content top-5 topic-hit 0.002->0.021 at K=8, ΔK8 +0.019) despite +3.4 nat log-prob boosts (§1017), because top-k is dominated by HIGH-FREQ grammatical tokens and content-neighbors sit deep in the tail (logP ~-15 -> -11.6, still below top-5). Function injection DOES move top-5 (0.27->0.41, head=high-freq). So CONTENT owns the TAIL, GRAMMAR owns the HEAD -- behavioral confirmation of the chain-rule split; bounds understanding->control (content shapes the tail not the top-k). pred_a/pred_0 FALSE, my steering framing naive (stated plainly)
  §1034 content_target_rank — NATURAL-TEXT confirmation of grammar-head/content-tail (§1033): function/grammar targets median rank 0, mean 11.6, 80% in top-5 (HEAD); content targets median rank 6, mean 286.7 (25x deeper), 46% in top-5 (TAIL). Grammar slot resolved precisely; content is a broad tail distribution over which specific word fills it. Chain-rule split read off per-token ranks. pred_a+pred_b TRUE. Confirms §1033 on real data (injection + natural-text ranks agree)
  §1035 per_component_understanding — the benchmark graph across all 36 components is BIMODAL: FRONT MLPs (grammar) understood (mlp0-3 = 0.67/0.93/0.74/0.63, mlp1 the dominant writer essentially solved); attn5 = -4.84 CATASTROPHIC OUTLIER (the content-pooling head-layer §998/§1007 -- a broad context pool that named per-position stand-ins actively MISPREDICT, worse than mean-ablate -> ANTI-understood); MIDDLE (content) ~0 to slightly negative (median -0.035, frontier); readout partly understood (mlp16/17 0.43/0.63). Medians front 0.45/front-MLP 0.65, middle -0.035, back 0.027. pred_a TRUE. Two-machine split read component-by-component; attn5 the single most anti-understood component
  §1036 bilinear_content_standin — INVALID (my optimization failure, stated plainly): gradient-fit rank-256 bilinear R² only ~0.26 at middle, but the MLP is LITERALLY bilinear so full-rank must be R²=1 -> the Adam fit is underpowered. Tell: R=0 Adam linear R² 0.29 < closed-form ridge linear 0.55 (§991/1005) -- doesn't even reach the linear baseline. Does NOT test the bilinear hypothesis; user's point remains OPEN. Correct test = truncate the model's OWN bilinear neurons (exact at full rank) -> §1037
  §1037 bilinear_neuron_rank — prong1 (VALID, R²=1 at full rank): the middle is bilinear but HIGH-RANK in the neuron basis by OUTPUT VARIANCE (R²@256 0.16, @1024 0.41, need ~all 4608 for 0.9; pred_a FALSE). Low-rank bilinear in neuron basis does NOT capture it (consistent §930/§940 distributed content). CAVEATS: (1) R² weights output variance incl loss-IRRELEVANT directions (§737/748) -> loss-relevant rank could be far lower -> §1038; (2) greedy neuron-truncation is a suboptimal upper bound; optimal/compositional factorization (prong 4) open. 'Irreducible' still wrong (it IS bilinear); 'irreducibly high-rank' is the sharper open claim
  §1038 bilinear_neuron_lossrank — prong1 nuanced: loss-relevant middle content is PARTLY low-rank bilinear. Loss-recovery@256 0.36 vs output-R²@256 0.16 (2.3x -> much output-rank is loss-irrelevant §737/748). L15 GENUINELY LOW-RANK (16 neurons -> 72% loss-recovery, a handful of nameable neurons carry the last content layer). Deep-middle L6/8/11 stays HIGH-rank even for loss (256->0.2, 1024->0.4-0.56). pred_a FALSE (no clean 0.9). Corrected claim: NOT 'irreducible' -- exact bilinear structure, low-rank at L15, high-rank deep-middle. Greedy ranking is upper bound; prong 4 (compositional) may compress further
  §1039 prong2_pooling_standin — CORRECTION: §1035's attn5=-4.84 was a HARNESS ARTIFACT (token+topic+prev topic-subspace off-distribution for the pooler); clean token table -> attn5 ~+0.01, bag ~+0.07 (near zero, NOT -4.84). Crude bag stand-in does NOT rescue poolers (worse than token for attn3/4: 0.13/0.12 vs 0.23/0.31; attn5 0.067). Reason: attn5 pools the VALUE RESIDUAL recency-weighted (§985/1019), not uniform-mean-of-embeddings. pred_a FALSE. Pooler is MECHANISTICALLY understood (§998/1007/1019) but not reconstructible by a simple stand-in -- the two come apart. Figure/§1035 corrected
  §1040 bottomup_mlp_rank — bottom-up effective bilinear loss-rank per MLP (R for 90%): READOUT mlp16 = 64 (compact low-rank bilinear, understood!); front mlp0/1 ~1024 (DOMINANT writers meanabl 2.17/1.08, moderate-rank -- corrects 'front=low-rank'; they're token-table-captured §1035 but bilinear-map rank ~1024); mlp4/15 mostly-low-rank+tail (16 neurons->0.77/0.72); DEEP-MIDDLE mlp5-14 FULL-rank (rec@1024 0.39-0.56, tiny budgets) = genuine high-rank frontier. pred_a FALSE. Modules understood by DIFFERENT structures; deep-middle needs prong 4 (bilinear over named subspaces/earlier writes, not raw input)
  §1041 prong4_bilinear_crossterms — PRONG 4 WIN: the deep-middle bilinear form is STRUCTURED = CONTEXT×CONTEXT. Identity decomposition exact. Output-variance: context×context 65-73%, token×context 20-26%, token×token only 7-9%. LOSS: loc×loc recovers ~nothing (~= mean-ablate), token×context barely helps; CONTEXT×CONTEXT carries ~all loss-relevant content. So the middle multiplies POOLED CONTENT BY ITSELF (not the token) -- structured, compositional understanding, even though flat-rank is full (§1040). The right basis is loc/ctx. pred_a TRUE. Next: is context×context low-rank in the named topic subspace (=topic×topic)?
  §1042 prong4_context_rank — LIMIT: the deep-middle's context×context is HIGH-RANK in the content subspace too (recovery@128 0.33, @512 0.77, need ~full 1152 for 90%; pred_a FALSE). The pooled content is GENUINELY HIGH-DIMENSIONAL (confirms §930 at the bilinear-input level, loss-relevant + optimal-PCA). PRONGS 1-4 CONCLUSION: 'irreducible' was WRONG (middle is exactly bilinear = content×content §1041; modules understood by matched structures §1040; readout rank-64; pooler mechanistic). But a REAL wall: deep-middle content is genuinely high-rank (confirmed 3 ways: §1000 linear, §1038 neuron, §1042 context) -- a real model property, not a stand-in artifact. Corrected claim: deep-middle = high-rank content×content bilinear
  §1043 bottomup_attn_window — BOTTOM-UP win: front attention understood as LOCAL-WINDOW routing (attn0 0.94, attn1 0.83, attn2-4 0.49-0.64; front mean 0.70), FAR above §1035's per-position 0.3-0.5. Shuffled-window null strongly negative (genuine). attn5 pooler EXCEPTION (0.089, broad pool, §1039). pred_a+pred_b TRUE. 90%-status: attn0/mlp1/mlp16 all ~0.93-0.94; attn1 0.83; attn2-4 need wider window/induction; deep-middle bounded by high-rank content
  §1044 bottomup_attn_induction — induction-copy + wider window barely lift attn2-4 (gains <=0.017, pred_a FALSE); my induction hypothesis WRONG. REINTERPRETATION (per §998): attn3-5 are the L3-5 CONTENT-POOLING band (broad, not window/induction). Front attn splits: attn0-2 LOCAL-WINDOW (0.65-0.94, understood), attn3-5 POOLERS (broad, mechanistic §998/1007). Bottom-up 90% map: attn0/mlp1/mlp16 ~0.93-0.94 (local/low-rank, DONE); poolers + deep-middle structurally understood but bounded by REAL properties (broad pooling / high-rank content), not stand-in failures
  §1045 bottomup_mlp_window — front MLPs are TOKEN + LOCAL-WINDOW functions: window lifts mlp0 0.59->0.885, mlp1 (dominant writer meanabl 6.5) 0.65->0.95 -> FRONT L0-1 ~90% understood. mlp2/3 partial (0.65-0.72); mlp4 window HURTS (-0.32, transition/pooling zone). pred_a TRUE. BOTTOM-UP 90% STATUS: ~90% = attn0(0.94)/attn1(0.83)/mlp0(0.885)/mlp1(0.95)/mlp16(0.94) via matched stand-ins; partial = attn2/mlp2/mlp3; BOUNDED by real properties = attn3-5 poolers (broad) + deep-middle high-rank content×content. Modules hit 90% when scored with the MATCHED stand-in
  §1046 bottomup_readout — readout MLPs are near-LINEAR reads (mlp17 0.85, mlp16 0.78/0.94-bilinear); readout ATTENTION is INERT (attn16/17 meanabl ~0.01). pred_a FALSE only due to inert attn (not a real gap). Readout UNDERSTOOD as near-linear/low-rank MLP reads + negligible attn (§941 L17 ~85% linear). BOTTOM-UP MAP COMPLETE: front L0-1 ~90% (token+window); transition L2-4 partial; attn3-5 poolers (broad); deep-middle high-rank content×content; readout near-linear. 90% reached with MATCHED stand-ins; hold-outs (poolers, deep-middle) bounded by REAL properties not instruments
  §1047 bottomup_attn_all — complete attention map: attn0-2 LOCAL-WINDOW (0.64-0.94, understood); attn3-14 BROAD (window-rec 0.03-0.62, content pooling/long-range, low-stakes); attn15-17 INERT (meanabl ~0.01). Bands: front 0.80, gatherers 0.40, middle 0.10, late 0.10. pred_a TRUE. Attention = local-window front + broad-pooling middle + inert late
  §1048 bottomup_mlp_transition — bag (pooled content) does NOT lift transition MLPs: mlp2 token+win+bag 0.712 == §1045 window-only 0.716 (bag adds nothing), mlp3 0.608<0.648 (worse), mlp4 -2.88 (catastrophic). pred_a TRUE is MISLEADING (vs token-only, not window). My 'transition reads pooled content' hypothesis WRONG. mlp2/3 plateau ~0.65-0.72 local; residual is bilinear HIGH-RANK content (§1042), smaller-scale. Front-MLP map: mlp0/1 ~90%, mlp2/3 local plateau, mlp4 enters multiplicative regime. Understanding-gap is UNIFORMLY the high-rank bilinear content (absent front L0-1/readout, growing mlp2->deep-middle)
§1049 middle_content_sharing: deep-middle content subspaces SHARED — pairwise top-64 overlap mean 0.577 (null 0.054), adjacent 0.65-0.82 decaying to 0.32 at 6-14 → one content computation on a slowly-drifting shared subspace, not 10 independent high-rank maps.
§1050 bottomup_middle_shared: replacing ALL 9 deep-middle MLPs at once = negative recovery (stand-ins fit on clean stream, compound errors on corrupted stream; per-module %s dont compose). But SHARED content basis >> per-layer at high K (-0.17 vs -0.60 @512) = functional confirmation of §1049 shared subspace; recovery rises with K (high-rank band).
§1051 bottomup_middle_greedy: greedy corrupted-stream fit does NOT rescue band composition (K=512 -0.70, no better than clean-fit); §1050 negative was not a distribution-shift artifact. Deep-middle transports LOAD-BEARING high-rank content through the residual stream — cannot bottleneck to rank-K per layer by any fit strategy. Strongest form of the high-rank-content frontier.
§1052 content_onset: the shared deep-middle content object is BORN GRADUALLY through the transition — overlap with deep-middle content ref rises monotonically L1 0.16 -> L5 0.44 -> L10 0.91 (null 0.056). The frontier is not a wall at L6; it emerges from L3. Transition MLPs resist token+window (§1048) because they catch the birth of the one content flow.
§1053 attn_writes_content: middle attention output overlaps the deep-middle MLP-read content subspace only modestly (band mean 0.23, 3-6x null) vs mlp-input 0.53-0.91 -> attention is NOT the primary author of the content-read subspace; it is a CUMULATIVE residual-stream object (reinforces §1051). The tidy attention-gather->MLP-multiply-same-subspace DAG is too simple. Caveat: increment-vs-accumulated.
§1054 pooling_standin_all: CORRECTED attention lane -- window of the RESIDUAL (not embeddings, as §1047 wrongly used) recovers front+gatherer attn L0-5 at 0.66-0.94 (attn3/4/5: 0.82/0.92/0.66 vs §1047 0.49/0.62/0.09). Pool does NOT beat window (pred_b FALSE) -> middle attn is local-window routing of the residual, not a loss-bearing broad pool (pooling lives in MLP-content path, §1053). Deep attn L12-17 near-zero stakes. Attention lane much better understood than benchmark showed.
§1055 content_dimensions: NAMED the content frontier -- its top principal axes are interpretable topic/register contrasts (PC0 informal-conversational vs formal-encyclopedic; PC1 emotional-narrative vs technical-financial; PC2 casual-social vs analytical; PC3 personal vs moral/religious). Top-10 PCs = only 11.6% variance -> MANY interpretable axes = high-dimensional semantically-organized topic/register manifold. From "high-rank content" to "high-dimensional interpretable topic space."
§1056 content_ablation: causal ablation of content-PC bands from deep-middle stream. Top-64 catastrophic (+8.4 nats, 36% var); bottom-64 still +1.6 (2.4x random control 0.67) despite 0.8% var -> content subspace causally privileged over random across whole spectrum (why rank-K stand-in fails). But loss-per-variance NOT flat (spread 20.7x, pred_a FALSE); bottom-band interpretation confounded by non-content overlap. Clean: content is causally real, top-heavy, full-spectrum privileged.
§1057 crossmodel_content: the high-dimensional interpretable topic/register content manifold is UNIVERSAL across the bilinear family. bilin12 (top10 var 0.186) and swiglu18 (0.158) both high-rank like bilin18 (0.116); the SAME topic/register axes recur (encyclopedic vs casual, moral vs business, personal vs economic) and even the SAME extremal contexts appear across all three independently-trained models. The frontier is the family way of representing context, not a bilin18 quirk.
§1058 crossmodel_content_cost: LOSS-LEVEL confirmation of universality. bilin12 & swiglu18 both replicate bilin18 front-linear (0.89/0.86) vs middle-nonlinear (0.59/0.46) split. Load-bearing high-rank content is a family property in behavior, not just subspace variance (closes §1057 caveat).
§1059 content_patching: ACTIVATION PATCHING (read/write-independent) — injecting a source text content-subspace coords into a target run moves the target output ~90% toward the source (alignment 0.90, KL 7.35) vs random-subspace control (0.67, 2.08; content KL 3.5x). The content subspace CAUSALLY transports topic downstream. pred_a false only on strict 1.5x alignment gate (random baseline inflated by large patch dim); KL 3.5x and 0.90 alignment support the claim.
§1060 content_patching_sweep: K-sweep closes §1059 caveat. Content-vs-random KL ratio 62x@K16, 22x@K64, 3.5x@K256; just 16 content dirs move target 0.58 toward source vs 0.14 random. Topic transport concentrated in top ~16-64 (interpretable) content directions. §1059 K=256 near-tie was a large-patch artifact.
§1061 crossmodel_cca: bilin18 vs swiglu18 (independently trained, different MLP type) encode the SAME content INFORMATION. CCA top corrs 0.95-0.97, mean 0.845, 58/64 dirs >0.7; shuffled control ~0.03. The content manifold is a convergent model-independent representation (same info up to linear map), not just same structure (§1057).
§1062 crossmodel_transfer: content is FUNCTIONALLY INTERCHANGEABLE across independently-trained models. swiglu18 source content, mapped via linear W into bilin18, patched into bilin18 transports topic at alignment 0.764 = 96% of the within-model upper bound (0.798), vs random-map control 0.348. One model context representation causally drives another. Strongest form of universality.
§1063 crossmodel_cca_width: CROSS-WIDTH universality. bilin12 (D=768) vs bilin18/swiglu18 (D=1152) share content info at top CCA 0.96-0.97, mean 0.76-0.80, ~50/64 dirs >0.7 (vs same-width 0.97/0.845/58; shuffled 0.03). Content representation is architecture-independent (different width, depth, MLP type).
§1064 content_axis_features: content axes are GENUINE SEMANTIC TOPIC, not surface register. Six surface features (digit/punct/caps/token-len/word-rate) explain only ~2% of content-coord variance (mean R2 0.02; best single |r|~0.2 mean-token-len on PC0; shuffled null ~0.006). §1055 "informal/formal" axes are semantic/genre distinctions, not shallow text statistics. Strengthens deep-semantic-content claim; corrects naive surface-register reading.
§1065 content_doc_coherence: content is MULTI-SCALE. Document-eta2 0.167 (vs shuffled null 0.004, and 1.64x random subspace 0.102) -> carries real document-level topic, but 83% within-document = predominantly running local context. Consistent with recency-weighted content-bag (§894/930): "what the recent context is about" at both doc and local scale, not a static per-doc topic label. pred_a strict >0.3 gate was wrong expectation for a running-context signal.
§1066 crossmodel_transfer_width: cross-ARCHITECTURE causal transfer. bilin12 (D=768,12L) content mapped via linear W drives bilin18 (D=1152,18L) at alignment 0.691 = 93% of within-model (0.745), vs random-map 0.354. Same pattern as §1062 same-width (96%). FIXED an initial scale-mismatch bug (injected deviation vs removed raw block-output -> all conditions identical 0.264; fixed to block-output raw projection). Content is functionally interchangeable across width AND depth.
§1067 content_ablation_by_tokentype: content is BROAD prediction support, NOT content-word-specific (pred_a FALSE). Both content+random ablation hurt rare tokens more (general difficulty); random steeper (3.04x vs content 1.69x low/high). Content excess-over-random is RELATIVELY larger for FREQUENT/function tokens (2.74x vs 1.52x). Content used heavily even for function words. Caveat: K=256 destructive/compounding regime; low-K check queued (§1068).
§1068 content_ablation_lowk: CORRECTS §1067. At clean K=16 (random control tiny +0.04-0.26), content ablation hurts rare/content targets 7.6x more than frequent (+8.5 vs +1.1 nats) > random difficulty baseline 6.0x -> content DOES preferentially support content/topical-word prediction. §1067 K=256 "broad" conclusion was a destructive-regime artifact (K>=64 saturates function words too, masking selectivity). Content mechanism tied to function.
§1069 middle_attn_window_width: middle attention (L5-11) gap is NON-LOCAL/content-dependent, not wider-window. Recovery 0.54@W3 -> 0.50@W15 (slightly worse, overfit); front flat high (0.8-0.94, W3 suffices). No window width reconstructs the middle-attn remainder -> it is entangled with the content-carrying residual stream (§1053/1054). Bottom-up map now complete: every module band characterized.
§1070 whole_model_greedy: CAPSTONE. End-to-end greedy substitution of all 36 matched stand-ins recovers only 12% (CE 6.83 vs meanabl 7.36 vs full 3.085). Curve declines monotonically from 0.90 (module 1) through 0.12, INCLUDING through the isolation-high readout -> ERROR COMPOUNDING dominates, not a content wall. Per-module understanding does NOT compose (extends §1050/1051 to whole model). Valid understanding measure = per-module-IN-ISOLATION (benchmark_matched figure); composed number is a compounding lower bound.
§1071 whole_model_content_passthrough: passing deep-middle content MLPs through (greedy-fit the rest) TRIPLES whole-model recovery (0.124->0.392, CE 6.83->5.68) -> content is a MAJOR isolated gap at whole-model scale. BUT scaffold stand-ins still compound (curve 0.88->0.28 even with content intact, through isolation-high readout). Low whole-model number = content gap + scaffold-approximation compounding, both. Per-module-in-isolation remains the valid understanding measure.
§1072 content_from_bag: deep-middle content is NOT a linear bag-of-words of context embeddings. Recency-weighted bag features (cur/cummean/exp-decay) predict the content coordinate at R2 ~0 (best exp9 0.019; shuffled null -0.10). Content coordinate = contextual deviation (token identity partialled out) is near-orthogonal to bag-of-identities -> content is a nonlinearly-computed contextual/semantic representation, not a shallow linear bag. Refines §894/930 (bag intuition right about FUNCTION, wrong about the vector being a linear bag).
§1073 content_construction: content is NOT a raw-word bag (§1072 R2~0) but is progressively/linearly built from the PROCESSED residual stream -- R2 predicting L8-12 content rises 0.38@L1 -> 0.61@L4 -> 0.80@L6 -> 0.88@L7. Nonlinearity is FRONT-LOADED (one layer already carries 38%); middle refines a linearly-determined representation. Completes the content-origin account (reconciles §1072/1052/1042).
§1074 content_seed: the content seed is created by the early ATTENTION (context gathering), NOT the MLP (pred REVERSED). Attn steps drive all content-R2 gains (+0.64 total; L0 alone -0.01->0.25); MLP steps slightly DECREASE it (-0.10, they write orthogonal grammar). Completes two-machine origin: attention GATHERS content, front MLP writes grammar, deep-middle MLPs MULTIPLY the gathered content (§1041), readout reads it.
§1075 arch_features: bilin18 two distinctive features feed the two machines. Removing x0 re-injection +2.34 nats, value-residual +3.37, both +5.09 (base 3.26) -> enormously load-bearing. VALUE-RESIDUAL disproportionately helps CONTENT/rare words (rare/freq 2.69) + per-layer concentrates at L2-4 (content-onset transition, §1052) = routes block-0 value into content-building. x0 RE-INJECTION flatter/grammar (1.56) + per-layer at L0-1 (grammar front). Both distributed/super-additive. Architecture <-> two machines.
§1076 blockzero_value: block-0 value is EXACTLY static per-token content (R2=1.0 from embedding, token-identity var frac 1.0; static-per-token-table override costs 0.0 nats; shuffled-token null +0.52). Structural: at block 0 x=x0=embedding, no context yet, so c_v(input) is a pure per-token value. The value-residual broadcasts each word static c_v content to every layer. COMPLETES content-construction: block-0 static values -> broadcast by value-residual -> pooled by attention (§1074) -> multiplied by deep-middle MLPs (§1041). Reconciles §1072 (content = bag of c_v VALUES not raw embeddings).
§1077 mlp_gate_leftright: bilinear gate. Weight cos ~0 all layers (Left/Right orthogonal). FRONT MLPs (mlp0/4) = genuine two-distinct-factor CONJUNCTION (forcing self-product Right:=Left catastrophic +2.4/+2.1 nats). DEEP-MIDDLE content MLPs = DIFFUSE gate (force-self-product cheap +0.09-0.14, ~2-3x permute-null; low stakes). pred_a REVERSED (strong conjunction is in FRONT/grammar, not deep-middle). Tension with §842 mlp0-self-product flagged (different methods: class-subspace form vs full-output replaceability).
§1078 gate_reconcile: RESOLVED §842<->§1077 tension. mlp0 is a MIX -- 1% self-square neurons (corr>0.7, §842 class units) + conjunction majority. Force-self-product on HIGH-corr quartile +0.026 (free) vs LOW-corr quartile +1.703 (67x) vs random +0.041. Both prior results stand (different subsets). mlp8 content: 0.04% self-square (diffuse gate, §1077).
§1075-CORRECTION: §1075 duplicated §985-987 (1k); clean value-residual->content/x0->grammar dissociation was OVERSTATED (§987 already corrected: both content-heavy, x0 only relatively more grammar-weighted). Report paragraph fixed. §1075 genuine-new = per-layer localization + freq split; §1076 (static per-token block-0 value) genuine. Dedup-first lesson reinforced.
§1079 content_ood_code: FIRST OOD test. Content subspace is largely REGISTER-SPECIFIC -- prose content subspace captures code content 0.166 (3x random null 0.055, but only 32% of code own top-64 0.515; prose<->code subspace overlap 0.19). Code content also LOWER-rank (top-10 0.238 vs prose 0.124). Nuances universality: content is universal ACROSS MODELS on in-distribution prose (§1057-1063), but register-ADAPTIVE within a model (code & prose use mostly different content directions + ~3x-random shared core).
§1080 register_generality_by_depth: register-generality gradient by depth. GRAMMAR front (L0-2) prose<->code subspace overlap 0.41 (46% of ceiling) vs CONTENT deep-middle (L6-14) 0.20 (28% of ceiling); ~2x more register-general (random null 0.057). Two-machine split holds along a THIRD axis: grammar=register-general syntactic machine, content=register-adaptive topic machine. Natural OOD extension of §1079.
§1081 ood_band_importance: on OOD code the model leans on LOCAL/per-token prediction. Both deep machines relied on LESS (content-band cost-frac 0.69->0.28, grammar-band 2.00->0.65) while value-residual costs MORE (1.01->1.10, per-token broadcast). Both preds FALSE (no clean grammar-holds/content-leans-out; BOTH deep bands lean out, local carries more; code is locally predictable). Representation-vs-reliance: grammar REPRESENTATION transfers (§1080) but is RELIED on less for code prediction.
§1082 — THREAD A: readout merge additive-linear (lin-cos 0.77) but head not content-insulated: argmax flips 38% (ctrl 14%); interchange worse than removal (content = real additive vote); redteam: variance-matched control pending (readout_merge.py)
§1083 — THREAD B first per-head map: super-head L5H7 = 54% of all attention causal weight (0.91 nats, broad pooler = likely THE content gatherer); induction is a different head L5H5 (0.25 share); top-5 heads = 67%; late all-inert (attn_head_map.py)
§1084 — THREAD C: NO binding band (cross share flat ~0.2, pred_a false); smooth tok→dev gradient (pred_b true); L4 = first true context MLP (nothing partial works); L1 token-term carries 98% of a 6.6-nat stake; readout mlp16 = 91% token-variance, token baseline load-bearing; redteam: held-out means pending (transition_terms.py)
§1085 — THREAD D: middle pool is positional/recency-first (log-dist r −0.39..−0.61 tops every layer); content-sim bias real but second-order, grows with depth (r 0.08→0.18), causal 1.8-4.1x; head-mean caveat (pooler_criterion.py)
...
§1092 — RE-DERIVATION NOTICE: thread B (§1083/87/89/91) duplicated the prior sink arc §429-432+§954 (position-0 constant head, mlp4 source, 162-head const sweep); marginal new: 62/38 bias-split table, rare/freq 2.16, sparse-truncation; report needs no update; dedup lesson recorded
...
§1111 — markdown FALSIFIES the 2.05 ratio law (1.62) and interpolation; direction survives (grammar>content generality 1.6-2.1x all pairs); raw-cost table added (fractions difficulty-confounded — method rule); content band prose-specialized, vresid most register-robust (register_markdown.py)
§1112 — coverage NOT the gap: matched-vocab ratios [0.61,0.58,0.12,0.83]; mlp0 tables are register-CONTEXTUAL with breadth asymmetry (prose-built travel, code-built don't); register axis closed (grammar_transfer_vmatch.py)
§1113 — content dictionary: stable (0.77 vs §763's 0.40) but weakly sparse (R² 0.71@k8), half-nameable, NOT individuated (3/8, controls exceed own twice); manifold's units stay continuous axes; k-sweep queued (content_dictionary.py)
§1114 — content code = stable sparse SKELETON (k≈8, 70% var, 3.6x PCA, stability 0.76-0.81) + dense TAIL (30%, stability dies 0.37); k₉₀=25.6; §763 instability = the dense limit; skeleton-vs-tail causal split queued (content_dict_ksweep.py)
§1115 — SKELETON IS THE CAUSAL CORE: 8 stable features carry 78% of the content read at REF layers (tail: 30% var, 12% CE — variance≠CE inverted); §1113 revised UP; whole-band version queued (content_skeleton_causal.py)
§1116 — skeleton law scale-invariant (13% band-wide); deep MLPs are MINOR content consumers (0.34 nats total read vs stream-level +8.4 §1056 — main consumer downstream); 6/8 skeleton atoms named (topic/register/discourse mix); naming thread concluded (content_skeleton_band.py)
§1117 — CAPSTONE: skeleton everywhere (deep 13% / readout-MLP 15% / logit path 10%; exit consumer 0.90 nats cross-checks §1082's 0.885); tail read nowhere — §1056 reinterpreted as construction disruption; stream-level closing test queued (content_skeleton_readout.py)
§1118 — ARC CLOSED: skeleton = API (reads 10-15% tail), tail = SCRATCH (46% of stream-level function in transit); construction skeleton-primary (97%); every content-line tension resolved; report update next (content_skeleton_stream.py)
§1119 — content API substantially universal: 7/8 atoms match cross-architecture, 5/8 cross-width (nulls ~0.014); code density a family constant (R² 0.710±0.001 @k8); arc §1113-1119 complete; stability audit queued (skeleton_family.py)
§1120 — skeleton arc AUDITED: split-half stable (0.125/0.137 vs 0.122; 0.794/0.788 vs 0.778); arc hardens as published; benchmark-refresh with skeleton content term is the registered next (skeleton_stability.py)
§1121 — skeleton does NOT lift tabulation (0.18 vs topic 0.26): construction-limitation quantified at north-star level; PLUS: the benchmark's topic term is partially self-referential — fully-external understanding is ~0.16-0.18 (annotation to §939/§1013, not a retraction) (benchmark_skeleton.py)
...
§1129 — CAPSTONE: package value-vector = smallest causal unit (atom interchange 0/8, wrong-dir null sometimes beats own); unified law: separately-authored, individually-nameable, jointly-causal population codes in BOTH machines (skeleton_atom_interchange.py)
§1130 — L3 certifies (0.83@64/0.95@256 — crosses 90%); L4/L5 own-64 leak-corrected (0.48/0.54); rank-256 certifies band 0.80-0.95; L4-not-token confirmed sharpest (mtok NEGATIVE held-out); readout certification queued (transition_standin_heldout.py)
...
§1135 — FIRST free-running result: three readable failure phenotypes (function-word soup / fragment salad / unanchored rare-token stream at rep4=0.000); ordering dissociates, ratios 1.25-1.52 partial; metric-sharpened rerun queued (gen_two_machines.py)
§1136 — generation wave closes: content axis quantified (cw-rate 0.44→0.20, 1.6x; soup confirmed), grammar axis under-instrumented at bigram grain (instrument gap, stated); vres signature = content-rich unanchored stream (cw ABOVE natural, rep4 0.000); phenotypes to report (gen_two_machines2.py)
...
§1144 — #2 NAMED: state-dependent NEAR-FIELD re-weighting (templates 42-60% var, 75-92% near-mass, coeffs decodable R² 0.63-0.88); middle-attention account COMPLETE (kernel × saturation-mass + near-field shaping); open problems 6/6 addressed (pattern_templates.py)
§1145 — decodable ≠ reconstructible: probe-named terms add ZERO over kernel (variance≠function at pattern level, third appearance); §1141/§1144 downgraded to representation-level; middle-attn stays kernel 0.58 + interaction-borne 42% PROVEN (attn_named_standin.py)
§1146 — #6 closed pred_c: causal codeword > 8-atom skeleton (k=8 swap generic 0.97); grain map three-tiered (read=skeleton / address=16-64 pattern / reconstruction=never); published pages corrected; open problems 6/6 (skeleton_pattern_grain.py)
§1147 — ADDRESS = COHERENCE not rank (uniform swaps generic at ALL K incl 64, ratios ≈1.00; §1059-60 specificity was position-matching); §1146 refined; pages corrected again; address story closed (address_grain.py)
§1148 — INSTRUMENT RECALL: affinity readout blind (positive control 1.02 = shuffled); §1128-29(content)/§1146/§1147 re-scoped; pages corrected 3rd time; sensitive-readout ladder queued (coherent_control.py)
§1149 — SUB-THREAD NULLIFIED: rotated null (1.41) matches coherent (1.39) — protocol never reproduced the known positive; §1146-1148 withdrawn to protocol-bound-null; standing facts revert to §1059-60; method law: replicate the positive FIRST; prime suspect = intervention locus (§1066 rule violated) (grain_kl.py)
§1150 — RESOLVED IN-PROTOCOL: positive replicates 4th-decimal on fresh rows (c256 0.8994 vs 0.8995); transport is POSITION-BOUND (shuffle excess negative), rank-8 carries 78% of rank-16 alignment, bag-average carries zero excess; open problem #6 answered validly (content_grain_ladder.py)
§1151 — position class IRRELEVANT (content-pos advantage 0.009 ≈ func 0.006, both « bar); half-coverage patching = ZERO net excess regardless of which half — transport non-additive across positions, coverage curve queued (pos_masked_transport.py)
§1152 — CORRECTION §1151 framing (coverage-mismatched null): coverage curve smooth concave 0.42→0.90, transport GRADED not all-or-nothing; scattered > contiguous at equal count (0.62 vs 0.55/0.45) = local-window reading signature; prefix > suffix (coverage_curve_transport.py)
§1153 — MECHANISM: transport is LOCAL (own-position coords 0.891 ≈ full 0.899) + small causal pooled leak (0.358 half-density, distance-decaying); two-term model reproduces all §1152 numbers ±0.005 incl suffix causal-mask zero-leak; transport arc §1150-53 COMPLETE (local_window_transport.py)
§1154 — depth IRRELEVANT / density EVERYTHING: address persists once written (thirds interchangeable ±0.025; L14 alone = 50% of full excess; excess ~ #patch points); registered alternative confirmed, preds a-c false (layer_ladder_transport.py)
§1155 — clamp nearly FROZEN (0.985-0.99/block, depth-uniform; random persists equally = generic stream linearity) but density effect ≠ coordinate decay: p14 final coords 0.96 of full9 yet 50% of logit excess — half of transport = PROCESSED CONSEQUENCES; consequence_split queued (patch_relaxation.py)
§1156 — §1155 OVERTURNED by own registered test: stripping final coords kills 68% of KL (coords DOMINANT, consequences minor; strip null exact 0); §1155's S-curve was cosine = magnitude-blind — magnitude re-measurement queued; method law: cosine ≠ magnitude (consequence_split.py)
...
§1165 — fixed map: global law CONFIRMED (sink 0.998, L5 0.887, flat 0.86-0.89 bands; only L1 0.68 lags) — attention selection window-computable model-wide; causal capstone fold_pattern_loss queued (pattern_fold_map2.py)
...
§1247 — janitor is GENERAL: injected random dirs scrubbed hardest (0.029 vs matcher 0.185) — block 5 = off-manifold normalizer, not a matcher-keyed service; §1244 consumption unaffected; framing corrected (janitor_general.py)
...
§1292 — cut 2 FALSIFIED: identity code is FULL-RANK (top-16 = 23-45% energy; masks ~= nulls) — partition works at bottleneck variables (verdict axis), not transport pipes; data-dependent rank-1 cut 3 queued (head_partition2.py)
§1293 — cut 3: PERFECT null (60:1) but catches only raw-coordinate identity (0.125 = cut 1's 0.122); fresh route re-encodes the code; tail = identity-transport crowd (0.963, spec 1.78x); rank-2 two-coordinate cut 4 queued (head_partition3.py)
§1294 — partition arc CLOSED: 4 cuts, 4 registered outcomes — task-parts are weights-derivable at bottlenecks, not at transport (depth-recoded full-rank content); keeper instrument = per-position identity mask (60:1 null); tail certified as identity crowd (head_partition4.py)
§1295 — ANNOTATION SERVICE CERTIFIED: 1.1/1.8 sufficient at match sources too (84%/70%, pair 96.5% of a 4.33-nat dependence) — one pair, THREE consumers (matchers, 13.8, 10.5); 1.1=self-head, 1.8=diffuse — different algorithms, same function (writer_cross2.py)
§1296 — annotators use OPPOSITE algorithms: 1.1 local (86% within +-8), 1.8 global (76% far); pair sufficient-NOT-necessary (only-pair ablation ~free); identity-mark vs context-signature discriminating cell queued (pair_profile.py)
§1297 — DISSOCIATION 77 points: context-free repeats — 1.1 (identity mark) 110% vs 1.8 (context signature) 33%, reversing natural text; adding 1.8 to 1.1 HURTS there; mirror arm queued (pair_algo.py)
...
§1308 — GOAL-3 CLOSED: stem-matcher prediction certified on natural text (0.186 @ conc 4.5; 78% of identity = inside the registered band; ctrl 0.8%) — first full weights->prediction->causal loop (stem_induction.py)
§1309 — GOAL-2 matrix: strongly diagonal (own-behaviour 20-300x foreign) but preds FALSE via position-overlap in target sets + small-n noise — disjoint-mask v2 queued (selectivity_matrix.py)
§1310 — GOAL-2 verdict: single-head circuits cleanly removable (20-300x); copy = shared subroutine (0.03-0.08 leak into all structured text, real); question->than = sentence coupling; matrix closed (selectivity_matrix2.py)
...
§1326 — FULL-DEPTH MLP LADDER: three regimes (front 0-3 token-resolved high-ceiling and carrying all the work; middle 4-15 inert at 0.64 nats TOTAL, below the instrument's ~0.15-nat floor; top 16-17 half-contextual + categorical, mlp16's K=16 BEATS the 50k table); preds a-c all TRUE as written, a & b re-scored on the evaluable subset and stronger there; §1325's "frequency is a late-model signature" WITHDRAWN — it tracks work, not depth (mlp_ladder_depth.py)
§1327 — context arm buys big everywhere (mlp17 tok16xctx16 0.612 > the 50k table's 0.497; ctx alone 0.563) but the FRONT CONTROL FAILED (mlp1 +0.260, larger than the top) — pre-registered as indicting the instrument, so all claims held provisional; cell-count confound ruled out by a clean rand null, TOKEN-IDENTITY confound not; purity diagnostic retired as powerless; within-token null queued (mlp_context_tables.py)
§1328 — within-token null: TOP CONTEXT IS REAL (mlp17 +0.183 / mlp16 +0.139 survive, inside un-tableable headroom; 16 ctx classes reach ~1/3 of the contextual half; it is FAST stream state, not doc-state — doc/prev buy little) but mlp1 survives too at +0.144 vs headroom 0.055 -> IMPOSSIBLE, convicts the null's rare-token leg; CEILING BOUND (incr <= 1-ceiling) adopted as standing gate; preds b TRUE, a & c FALSE; null-scaling test queued (mlp_context_tables2.py)
§1329 — comparative G1+G2: extraction reaches 70% but kept set = GENERALIST band + one conditional specialist (elsewhere-rec 0.679 ~ target-rec 0.697 convicts a02 as universal, not comparative); 8.1's increment 26x target-selective (0.361 vs 0.014 nats) = G2 verified; 8.1 without annotator worthless (+0.025) = first fully closed conditional chain both directions; preds a,b FALSE by letter, c TRUE; mark-slice refinement logged (comparative_extraction.py)
§1330 — scaling REFUTES the artifact account (incr GROWS 0.092->0.142 as sparse tokens halve; ceiling steady): §1328's ceiling-bound gate RETRACTED (mean table is L2-optimal, not CE-optimal — it lower-bounds token inadequacy, doesn't upper-bound context value); mlp1's within-token context REINSTATED (~1.0 nats at 4-bit grain) — mlp1 = context-indexed per-token MENU; draw spread ~+-0.05 quoted; residual-ctx direct test queued (mlp1_null_scaling.py)
§1331 — annotator service decomposed: key-side mark small (+0.070 of +0.263, rank-1 carries ~55% of it), QUERY-side dominates 3:1; both slices PERFECTLY selective (else = route +-0.002) -> [route + comp-position outputs + 8.1] = capability-specific 50% at zero elsewhere cost; preds a,b FALSE, c TRUE; query-side arm queued (comparative_mark_slice.py)
§1332 — residual test OVERTURNS §1330: on top of the full token table ctx16 adds +0.0066 at mlp1 (menu WITHDRAWN; coarse increment = CONSISTENT-HASH fine-token routing, which also grows with data — growth criterion struck) but +0.1328 at mlp17 (null -0.001, 42% of residual variance) = bound-free proof of real fast context at the top; preds a,b FALSE, c TRUE (mlp1_residual_ctx.py)
§1333 — comparative circuit CLOSED: [route + 2 zero-bit positional gates + 8.1] = 0.659 target recovery (85% of the band's contribution) at else route+0.014; query-side dominant 2.7:1; preds a-c ALL TRUE (first clean sweep); recipe -> question circuit next (comparative_query_side.py)
§1334 — question template: HALF-carried (band 0.508 < 0.60 bar -> a02-annotator assumption partly wrong; missing service outside front attention), query-side dominates again (+0.128, 2nd circuit), and FIRST INVERSION: clause-gated a02 BEATS whole live band (0.545 vs 0.508) at 0.25 less elsewhere — capability-gating is the better description, not just cheaper; both-arm void (key subset of qry, disclosed); band scan queued (question_gates.py)
§1335 — question annotator = L0-5 (clause-gated a05 0.714, best number, bar cleared; front+mid sub-additive state carriers); INVERSION repeats in band width (a08 0.677 < a05 — L6-8 gated liveness hurts; optimal live set is not "more"); preds a-c ALL TRUE (2nd sweep); per-layer/per-head localization queued with registered DIFFUSE bet (question_band_scan.py)
§1336 — QUESTION CLOSED: mid service = LAYER 4 (63%) but crowd-carried within it (top head 32% — DIFFUSE bet WON); [route + clause gate L0-5 + 10.5] = 0.714 final; comparative=owner vs question=crowd now both itemized; preds a-c ALL TRUE, 3rd sweep (question_mid_heads.py)
§1337 — close-bracket screen at n=1779: a13 +0.711 nats target dmg, conc 45.9 (largest clean concentration on record), ownership 3x-clear; VERDICT WITHHELD on random control 1.62 vs 1.5 (jitter clean 1.35; small-denominator noise suspected); preds a,c TRUE, b FAILED as written; 2x rerun running (close_bracket_screen.py)
§1338 — close-bracket CERTIFIED at 2x data (a13 +0.706, conc 46.8, controls 1.18/0.99 clean; damage stable across draws, ratios need data); head stage queued with registered OWNER bet (close_bracket_screen2.py)
§1339 — capitalized: confound fix cleans the controls (1.38/0.88 vs atlas 12.5 — sentence-initial exclusion was right) but NO layer-owner (a17 +0.109 vs a16 +0.098, 1.1x): first LATE-BAND shared capability (a15-17); atlas leader column can name a band member; parked behind close-bracket (capitalized_screen.py)
§1340 — NEW NAMED HEAD: 13.8 = close-bracket head (96.5% of layer solo; other 8 heads jointly +0.008; both directions agree; ~570x surgical) — first fully-new head from the registry pipeline; owner/crowd/pair scoreboard updated; subtype decomposition queued per user conjecture (compound closes ")),")." need helpers) (close_bracket_heads.py)
§1341 — subtypes: 13.8 owns EVERY close form inside L13 (shares 0.95-1.04; user conjecture FAILS within-layer) but is vindicated ACROSS layers — a14 assists exactly the compound closes (+0.17..+0.20 on comma/period/other) and is NEGATIVE on plain (−0.060): cross-layer division of labor, close vs trailing punctuation; double-close unevaluable (n=4, disclosed) (close_bracket_subtypes.py)
§1342 — question kit slims 36->16 heads for 1 point (0.641); 8 heads BEAT the 27-head band (0.584 vs 0.545); L4 "crowd" = a trio (4.0/4.1/4.7 by drop-cost — add-grain masked it); CROSS-CIRCUIT GUESTS in the top-8: 0.3 (bigram router) + 1.1 (stem matcher); preds a-c ALL TRUE (question_kit_slim.py)
§1343 — entropy-setter REFUTED 3 ways (rank collapse 95->51% top-5 = missing CONTENT; gain-freeze HURTS 0.578; temperature s*=0.95 buys nothing); missing 0.34 = target-specific dynamic content from live values; first gain-frozen leg of the template (closes the both-regimes gap); missing-band scan queued (comparative_gap_anatomy.py)
§1344 — missing content SPLITS: L3-5 wins raw (+0.135) but GENERIC (else +0.225, pred_c's registered failure = the sorting instrument); L10-12 is 1.8x TARGET-SPECIFIC (+0.117/+0.065) = a POST-FETCH REFINE stage downstream of 8.1 — 4-stage circuit (annotate->fetch->refine->readout); §1310's question->than off-diagonal sits in this band; refine-head localization queued with 10.5-moonlights bet (comparative_missing_band.py)
§1345 — refine stage = {10.5, 12.8} (+11.7/11.6): THE QUESTION HEAD IS THE TOP COMPARATIVE-REFINE CARRIER (+0.343 drop — §1310's off-diagonal called it); top-4 beat all-27 (0.778 vs 0.776); §1313's "question-specific" label narrowed, not retracted; pred_c FAILED by 0.0006, quoted; comparative kit final 0.76-0.78; thread rests — close-bracket extraction next (comparative_refine_heads.py)
§1346 — close-bracket template SWEEPS (3rd circuit): kit = [route + depth>0 gate on a02 + 13.8] = 0.657 at else +0.009; a02-annotator HOLDS here (unlike question); query-side 3-for-3; 13.8 solo is BELOW route (owner without mark = liability, sharpest conditionality yet); both-arm void repeated + gate-disjointness now a mandatory template precondition; ~0.65 plateau across all three kits noted; a14 subtype rung queued (close_bracket_gates.py)
§1347 — a14 does NOT port into the kit (+0.005 vs its 0.17-0.20 full-model ablation role): ABLATION NAMES ROLES, CONSTRUCTION PRICES KITS — standing rule; assistants are redundancy-embedded, owners port; close-bracket kit stands at 0.657; bracket refine band-scan queued with downstream bet (close_bracket_a14.py)
§1348 — bracket refine scan: motif HOLDS in the data (L14-17 downstream, ratio 2.4, like L10-12 for comparative) but preds b,c FALSE as written — bars keyed to the raw winner (generic b35) instead of the specificity-sorted band, §1344's lesson half-encoded; fix standing: register on the specificity-passing band; CROSS-CIRCUIT INVARIANT: generic-pool elsewhere increment +0.227 == +0.225 across capabilities (one shared MDL line item); bracket thread rests, exclaim template queued (close_bracket_missing_band.py)
§1349 — exclaim v1 under an infra race (degenerate gates executed; qry/both void, disclosed): pair CONDITIONAL 4-for-4 (+0.026 solo), band 0.710 = highest of any circuit, route floor 0.540 = highest (L17 owner as predicted), sentence-initial gate useless -> "!" state accumulates, no token-computable gate exists — mode-labeler (backlog #7) is the continuation; EDIT-BEFORE-QUEUE ops rule added; rewritten band arms requeued (exclaim_gates.py)
§1350 — exclaim v2 sweeps: a05+pair 0.860 = highest number in program BUT elsewhere 0.880 (generalist — quoted up front; no gate exists); depth matters +0.150 -> CRITERION SCOPE PREDICTS ANNOTATOR DEPTH (2 circuits per side); thread rests, G1-specific blocked on backlog-#7 probe gate (exclaim_gates.py)
§1351 — open_quote FAILS clean screen on every bar (a10 +0.067 vs atlas +0.272; conc 2.7; jitter dirty; a11/a13 NEGATIVE at targets) — DEMOTED, mask mismatch disclosed (n=90 atlas vs n=1087 clean); distributed-no-owner class grows; digit->a8 screen queued (open_quote_screen.py)
§1352 — digit CERTIFIES (a8 +0.210 > atlas's +0.134, n=9056, conc 5.9, controls clean, ownership 2x) — CROSS-METHOD: atlas + a8-counting arc name the same owner; a13 flat at digits = clean dissociation; head stage queued (digit_screen.py)
§1353 — digits = REDUNDANT PAIR 8.3+8.7 (solo 38/28% vs keep-only 65/46% — both grains measured, redundancy quantified); 8.3 IS the §1207-09 copy station -> suspicion: digit capability is substantially COPYING; pred_a FALSE (no owner), b,c TRUE; label artifact disclosed; copy-split queued (digit_heads.py)
§1354 — digit pair divides by EVIDENCE TYPE with opposite signs: 8.3 copy-flavored (2.9x per-position), 8.7 fresh-specialist NEGATIVE on copyable (interference inside a pair, first sighting); "redundant" -> COMPLEMENTARY; moonlighting ledger = 4 -> new heuristic: heads are functions, capabilities are contexts; pred_a FALSE (bar mixed class sizes — per-position bars standing); digit thread rests; behaviour_atlas2 generator queued (digit_copy_split.py)
§1355 — generator refills pool (ellipsis->a17 23.8x, unit->a8 7.5x, quote_close->a13 30.5x!) and pred_b's failure VALIDATES heads-are-functions: new capabilities land on known function-layers; CLOSER HYPOTHESIS raised (does 13.8 own quote-closing too? -> delimiter-general close function); possessive distributed (grammar prior misfired); quote_close_heads queued (behaviour_atlas2.py)
§1356 — 13.8 = THE CLOSER (renamed): 107.9% solo share at quote targets — keeping only 13.8 BEATS the intact layer (neighbors net-negative, interference at its extreme); one function, two capability surfaces (brackets 46.8x + quotes 30.5x were the same head); preds a-c ALL TRUE; union-kit MDL test queued (quote_close_heads.py)
§1357 — ONE KIT TWO CAPABILITIES lands (union == dedicated to 0.001 on both families; kit accounting goes per-FUNCTION — first measured MDL merge) but quote side under-carries (0.468, route floor 0.273 = half of brackets') — the §1350 scope-predicts-depth taxonomy CALLED the miss (quote spans are long); depth rung queued (closer_kit.py)
§1358 — TAXONOMY IS LAW (4-for-4): a05-parity kit 0.630 clears the bar (mid +0.162); width inversion 5th sighting (a08 < a05); CLOSER THREAD CLOSES — final form [route + (depth-gate a02) UNION (parity-gate a05) + 13.8] = brackets 0.657 / quotes 0.630, one function head, one composite zero-bit gate; ellipsis screen queued (closer_quote_depth.py)
§1359 — ellipsis: clean screen, a17 leads at 2.5x (conc 22.7; random control passes by 0.01, quoted) but 3x bar broken by a13 = THE CLOSER's layer (delimited-span ellipses suspicion) — two function-heads at one behavior is now the EXPECTED shape; PAIR BET registered: 17.2+17.3 (the exclaim pair) carry it -> third surface, EXPRESSIVE-TERMINAL function; heads stage queued (ellipsis_screen.py)
§1360 — pair bet WINS at 93% with ASYMMETRIC split: 17.2 = BROAD expressive-terminal (78% of "…" vs 49% of "!"), 17.3 = "!"-specialist (15% vs 43%) — §1319's additive halves gain a breadth axis; moonlighting entry 5; ellipsis resolved in 2 runs; unit screen queued with 8.7-fresh expectation (ellipsis_heads.py)
§1361 — unit: conc real (+0.203, 5.1) but verdict withheld (jitter 1.70, ownership fails at n=161) — NEW CONTAMINATION CLASS FOUND: function-surface overlap (jitter lands on a8's own digit territory); cleaned-control 2x rerun queued (unit_screen.py)
§1362 — cleaned rerun REFUTES my §1361 contamination story (5% overlap, jitter unchanged 1.70) — a8 is PHRASE-SCOPED numeric-context (jitter positions genuinely load-bearing); unit DEMOTED (two strikes); jitter control scoped: position-spiked capabilities only; atlas3 generator queued with prospective a8 bet (thousands/decimal) (unit_screen2.py)
§1363 — atlas3 EMPTY (0/7; a8 bet loses: numeric FORMATTING is front glue — a8 narrowed to numeric-CONTENT; two-machines division reproduced inside one phrase); token-class generator EXHAUSTED — specialist inventory ~8 functions, a completeness statement; queue redirects to the blocking infra: exclaim probe gate queued (backlog #7 first instantiation, §105-safe design) (behaviour_atlas3.py)
§1364 — probe gate WORKS as instrument (sheds 93% of the generalist bill; 6x better target-per-elsewhere) but recall-capped by probe AUC 0.618 (keeps 41% < 60% bar) — first capability-specific exclaim kit exists; backlog #7 instantiated and reusable; two-site probe upgrade queued (exclaim_probe_gate.py)
§1365 — scoped negative EARNED: AUC ceiling ~0.61 across two feature sets — the "!" mode is carried by the live values the kit ablates (the §1350 loop closes on itself); deploy-legal recall ceiling ~44% named; EXCLAIM CLOSED; mdl_bill queued (the per-function bill, shared heads priced once) (exclaim_probe_gate2.py)
§1366 — THE BILL FLIPS THE FRAME: kits = cheap specialists (1-5 heads) + expensive SHARED annotator bands (27-54 heads); dedup 61.2%; TWO-TIER accounting adopted (infrastructure priced once ~50M / capability marginals 1-4M); question cheapest per-nat BECAUSE slimmed -> band slimming is the lever; closer band-slim queued (mdl_bill.py)
§1367 — band-16 BEATS band-54 (6th inversion, largest; 38 of 54 heads dead weight); NEVER GATE A CONSTANT (sink 5.7 drop −0.44 — gate-boundary seam); THE COMMONS MEASURED: 10/16 carriers shared between question & closer (union 22) — Tier-1 re-prices to ~19.5M; one-sided-bar lesson #4; capstone shared-band experiment queued (closer_band_slim.py)
§1368 — CAPSTONE LANDS ABOVE ITS BARS: one 22-head commons serves ALL kits BETTER than their own bands (question 0.696, comparative 0.874, closer 0.767/0.657); headline: commons (~19.5M) + 6 specialists (~5.3M) + zero-bit gates = 66-87% of four families at ~4.6% of the model; pred_c missed by 0.0006 (ungated refine heads); extraction arc RESTS (shared_band_kit.py)
§1369 — BILL FINAL: commons 19.5M + specialists 5.3M = 24.8M (4.5% of model) for 24.2 nats at 1.03M/nat (2.4x better); marginals: question 0.44M, closer 0.88M, comparative 3.98M (pred_c FALSE on the un-trimmed refine tail — fork recorded); extraction arc RESTS at capstone (mdl_bill2.py)
§1370 — two-sided description FAILS at naive composition (all preds FALSE, both modes pre-named): the inert middle is LOAD-BEARING inside the kit (−0.42 — redundancy is context-dependent, per-module stakes scoped to the intact model); intact-fit tables collapse the kit stream (else 0.037 — the §105 catastrophe measured at scale); SEQUENTIAL REFIT version queued per HARNESS.md (full_description_kit.py)
§1371 — refit restores the LM (else 0.037->0.376) but not the capability (0.474 < 0.60; refit middle still −0.334): the mid MLPs' in-kit service is DYNAMIC — THE GENERIC MIDDLE IS ONE REDUNDANT SERVICE WITH TWO IMPLEMENTATIONS (mid attn + mid MLPs); extraction can remove either half, never both; two-sided bill final: attention compresses (25M), MLP side does not; ARC RESTS; refine-gate loose end queued (full_description_kit2.py)
§1372 — loose end CLOSED: refine-gating free at targets (0.875), elsewhere 0.022 (§1368 miss closed); pred_c FALSE = moonlighting confined by design (bar-lesson #5: exempt the gated head's other surfaces); FINAL comparative kit; EXTRACTION ARC CLOSES (§1311-1372) — FINDINGS updated; bilin12 transfer thread opens (comparative_refine_gate.py)
§1373 — THE CLOSER TRANSFERS: bilin12 a7 = concentrated close-bracket layer (+0.953 > bilin18's +0.71; conc 21.1; controls clean; rel depth 0.64 vs 0.72) — first specialist-grain universality evidence; all 3 transfer bets TRUE; owner-head stage queued (bilin12_closer_screen.py)
§1374 — OWNER STRUCTURE TRANSFERS: bilin12's 7.1 = closer at 105% solo (neighbors net-negative — the exact §1356 signature); 13.8 and 7.1 are the same part in two machines; delimiter-general test queued (bilin12_closer_heads.py)
§1375 — THE CLOSER IS REPRODUCIBLY BUILT: 7.1 owns quotes at 105.3% (cross-model table complete — function, single-owner grain, depth, and interference signature ALL transfer); heads-are-functions is now a claim about TRAINING; transfer thread 3-for-3; sqrd12 boundary test queued (bilin12_closer_quotes.py)
§1376 — closer SURVIVES the score-function boundary at half strength (sqrd12 a7: +0.402, conc 8.9, clean, rel depth 0.64 == bilin12's) under a bigger generalist front; preds a,b FALSE by raw-argmax keying — THIRD bar-keying failure -> ops rule: patch templates, not memories; owner question queued (sqrd12_closer_screen.py)
§1377 — OWNER SHAPE CROSSES THE SCORE FUNCTION: sqrd12's 7.0 = closer at 103.4% solo, neighbors net-negative — third model, same part; only magnitude varies; final 3x2 cell queued (sqrd12_closer_heads.py)
§1378 — 3x2 CLOSER TABLE COMPLETE (every cell owner-carried; sqrd12 quotes 75.7% + helper = the one softened cell, where §1376 found attenuation): THE TRAINING PROBLEM DETERMINES the closer's existence, grain, and depth; architecture modulates strength; transfer thread RESTS; swiglu18 extension queued with specificity-keyed bars (sqrd12_closer_quotes.py)
§1379 — swiglu18 screen: pred_a FALSE on a selector hair (a14 lost to a10 by 0.008 dmg at 5.5x the concentration; a14 passes ALL raw bars: +0.244, conc 18.8, depth 0.82) — selector refined (rank passing layers by CONCENTRATION), patched into template; weakest family closer if confirmed -> MLP type also modulates strength; head stage queued (swiglu18_closer_screen.py)
§1380 — FOUR MODELS ONE PART: swiglu18's 14.1 = closer at 113.6% solo (6th layer with the exact signature); existence/grain/depth/interference = training-problem properties; strength modulated by score fn AND MLP type; TRANSFER THREAD CLOSES (swiglu18_closer_heads.py)
§1381 — capstone DRAW-ROBUST: all four scores within +-0.025 on disjoint rows, ordering exact, selectivity holds — the headline claim gains a measured error bar; bilin12 kit-recipe port queued (shared_band_kit_seed2.py)
§1382 — extraction grammar TRANSFERS IN STRUCTURE (query-side 5th confirmation; owner-as-liability in 2nd model; gates selective) but carry bar missed by 0.011 (0.539, within measured draw noise, scored FALSE); transfer thread RESTS COMPLETELY (§1373-82); flagship-table error-bar leg queued (bilin12_closer_kit.py)
§1383 — flagship share error-bar: 13.8 bracket solo 0.977 +- 0.012 across 3 disjoint draws, surgical every draw; both headline results now carry measured bars; USER: bilin18-only, middle-MLP wall is the frontier — stand-in ladder running with his outlier-dims hypothesis as pred_a (closer_share_spread.py)
§1384 — ladder v1: 2 of 3 arms were MY bugs (gain fit/apply rms mismatch -> NaN; uncentered ridge -> −0.44) and the third confounded by one-shot fitting (mean baseline 0.342 vs refit 0.540 — §158 gap measured a 3rd time); hypothesis verdicts WITHHELD; sequential-refit rerun queued at the same bars (mid_standin_ladder.py)
§1385 — cheap fixes FAIRLY DEAD (outlier +0.008 — user hypothesis refuted at a fair test; gain HURTS; rank-1 nil; jointly <3% of the 0.328 gap): the middle's in-kit service is genuinely distributed conditional computation; next: does the crowd collapse? mid-MLP drop-cost queued (mid_standin_ladder2.py)
§1386 — MIDDLE COLLAPSES TO MLP4-7: keep-only-4 = 0.882 > all-live 0.874 (7th inversion; mlp8-15 net interference; grains AGREE 4/4 — owner-band not crowd); bottleneck re-scopes by 2/3; both middle implementations (attn3-5 + mlp4-7) at the SAME depths (mid_mlp_dropcost.py)
§1387 — swarm calibration: 9/9 well-predicted datapoints mlp1-dominated (datapoint-grain confirmation of easy-distribution ownership); QK/OV instrument works; 2 new micro-functions (11.6 roster-surname, 6.3 compound-cache); sampler lesson -> nontable sampler for wave 2 (swarm_lib.py, swarm2/)
§1388 — owner-band RESISTS low-rank: 0/4 rank-1, 1/4 rank-64-marginal; shallow flat dose-response = real high-rank inside the kit; middle's floor = FOUR LIVE MODULES (~64M); two-sided kit closes at 0.84-0.88 (mlp47_anatomy.py)
§1389 — wave-2 consolidated: non-table easy territory IS the structure layer (finding, not bug); candidates 6.5 + 7.8; reviewer downgraded 11.6 (one-sample story) and enriched mlp1 (2nd mode: topical recall); magnitude-ranking-picks-wrong-story lesson -> wave-3 protocol (separate head/MLP ranking, two-skip rule, stratified sampler); instrument self-validated via 0.3/8.1/13.8 rediscoveries (swarm2/)
§1390 — PIPE = closer's THIRD surface (83.2% solo on an opening-diluted mask, conc 25x): brackets+quotes+pipes, one head; swarm->queue pipeline closes its first loop (datapoint observation -> registered confirmation in ~1h); two-skip candidate confirmations queued (closer_pipe_test.py)
§1391 — two-skip rule kills BOTH candidates (6.5 delim-frac 0.00/0.33; 7.8 one shared type): THE MAGNITUDE FLOOR — heads below ~0.02 scope-dCE have no nameable categories (3 deaths, 1 success at 5-40x that size); wave-3 rule: no naming below the floor; registry struck (candidate_confirm_65_78.py)
§1392 — wave-3 consolidated: 2.5 enrichment held (fetcher = rare-name/code continuation); head-star hunt = strongest §1391 confirmation (1 of 20 candidates, and it was struck-6.5 reconfirmed dead); EASY-DISTRIBUTION OWNERSHIP MAP COMPLETE (mlp1 + thin structure layer + specialists at triggers); wave-4 pivot: medium-CE behavior-conditioned enrichment (swarm2/)
§1393 — keep-{4,5,6,7} mid-band is comparative-specific: reproduces width-inversion on comparative (.882>.874), rides on closer (gap .019), FAILS question (−.094, 3× bar); dropping mlp8-11 costs ~0.2 CE elsewhere in every kit (generic background service). Width inversion = property of (band,capability) pair (mid_owners_crosskit)
§1394 — 7.8 CLOSED: verdict test at 2x power fails floor both skips (.0190/.0195 < .02), top-30 overlap 1/30; "coherent flavor, zero surface overlap" = recognized third outcome (flavor-only helper) (rec_w4C_78verdict)
§1395 — question's mid set = {4,6,7,8,9}: mlp4 dominant (+.724), mlp5/mlp10 dead weight (~0), keep4+8+9 reaches .679 vs live .696 (all preds PASSED); mlp4 = the middle's universal first layer; drop-4/6 improves elsewhere ~0.2 = width-inversion echo #8 (question_mid_dropcost)
§1396 — wave-4 consolidated: medium band adds ZERO new categories (mlp1 owns 19/22; specialists = secondaries at documented triggers; mlp16 "candidate" = wave-2 register rediscovery, Cubs surnames + URL enrichment); floor amended: necessary-not-sufficient for whole-MLP specs; swarm SATURATED at position grain, idles by default (swarm2/)
§1397 — capitalized = 7-head COMMITTEE {17.2,17.1,16.5,15.3,17.0,16.4,16.0} across a15-17, NO owner (shares .49/.30/.41; top-3 = 41% of band; all 3 crowd preds PASSED); 17.2 moonlight #3; anti-closer shape (capitalized_band_heads)
§1398 — committee-of-7 = clean unit: 71% of band damage, surgical (else 2%), rest-20 unselective generic service, near-additive; REMOVAL GOAL DONE for capitalized (7/162 heads, −.21 target / .004 else); cross-layer super-additivity ~.08 (capitalized_band_slim)
§1399 — committee grain: QK-mean costs 67% of OV (pred_a payload-dominance FAILED, 1.49 < 2.0); a16 trio conjunctive (either grain destroys all), only a17 trio payload-dominant (2.6x); extraction must keep live QK for 15.3/16.x members (capitalized_committee_grain)
§1400 — capitalized kit .644 (weakest of 4 families, matches deepest-context ranking); committee marginal INSIDE kit only .029 vs .21 removal value (pred_b FAILED — strongest owner-conditionality sighting); CAPGATE .4846 coverage (pred_c FAILED by .015); half of capitalized targets have no input-side cue (capitalized_kit)
§1401 — gate was the binder: ungated commons lifts capitalized .644→.813 (2nd best family); elsewhere improves too; committee marginal in-context .008 (pred_c FAILED) — removal handles ≠ construction components, cleanest dissociation yet (capitalized_kit2)
§1402 — gates dropped for 3 of 4 families (comparative .917 = program record, closer .810, capitalized .796 ungated) but question WORSE ungated (.696→.633, pred_a FAILED): question's gate is FUNCTIONAL (protects long-range state) — width inversion #9, at gate grain (commons_ungated_crosskit)
§1403 — ONE-MODEL BILL: 28/162 heads serve question .644 / comparative .918 / closer .846 / capitalized .826 / else .812; co-residence SYNERGISTIC (closer +.036, capitalized +.030 over solo — pred_a failed on a two-sided bar, 5th bar miss); gate belongs to question only (unified_kit)
§1404 — THE CLOSER IS CONSTRUCTION-REDUNDANT (drop cost −.01 in unified kit!) and only 10.5 of the specialists is load-bearing; simultaneous drop of 14 individually-cheap heads collapses (−.07-.11): cheapness doesn't compose — redundancy is a shared resource (unified_kit_slim)
§1405 — sequential greedy: 20-head kit (q .638/comp .903/closer .838/cap .813/else .786); 13.8 dropped FIRST (redundancy confirmed); drift ~.003/drop breaks hold bar (pred_a FAILED) (unified_kit_slim2)
...
§1412 — all-12 removal SUPER-additive: .603 vs .403 sum (pred_c failed opposite-direction); a13-14 and a15-17 crews = redundant implementations, either covers ~2/3; honest handle = 12 heads / .60 nats / 6.7% spillover (capitalized_committee12)
§1413 — no small removal set: greedy marginals INCREASE (.035→.060), 6 heads = exactly half of .603 (pred_b FAILED); redundancy = a pool that depletes (3rd face of one law); CAPITALIZED THREAD CLOSED; head-grain mine queued (capitalized_removal_greedy)
§1414 — head-grain mine: 20 novel pairs (8.2 newline .749! 13.3 punct .678, 10.6 quote .540, 11.0 newline .535); 13.8 sanity .899; 9/12 committee blind-confirmed; damage mining = primary generator (damage_cluster_mine_heads)
§1415 — 8.2 = NEWLINE SPECIALIST (+.146, cleanest first screen since the closer), 11.0 second newline head, 10.6 quote +.036 (pred_b missed bar by .004); punct pairs were argmax mirages (2/2 dead); screen is the arbiter (head_screen5)
§1416 — newline pair = third two-crew instance (joint 1.70x max solo); subtype split VACUOUS (corpus emits only bare '\n' — disclosed); 10.6 INVERTED: second close-quote server, open-quote stays unserved; two-crew motif registered (newline_pair_quote)
§1417 — kit28 serves newline .863 WITHOUT the newline heads (pred_a failed spectacularly); two-inventory law now GENERAL: removal handles are construction-redundant in 4/4 tests, sole exception 10.5/question; pair not added to bill (newline_bill)
...
§1429 — quadratic residual is DIRECTION-AGNOSTIC: random dirs (.056 gain) beat function-aware (.043); two basis families failed; mlp4 = linear-on-2 (62%) + isotropic quadratic (+.06) + 26% tail; capacity-vs-direction test queued (mlp4_quad)
...
§1600 — OUTPUT CHANNEL CERTIFIED (2-for-3, held-out replicated): WU@v1+ rank-1 token is '?' itself (top-10 all question variants); final-residual cut 2.02 = 2.5x source cut; decomposition doesn't close (readers offset) — channel budget queued
