"""RSPD CLUSTER R80 -- fair per-cluster rank at the SAME 80% tolerance as
the global r80 (708 used a strict 0.02-nat/99% bar, not comparable). For
each cluster: benefit_c = CE_ablate_c - CE_full_c (that cluster's tokens
only), and r80_c = smallest r with per-cluster recovered >= 0.80. Compares
per-cluster r80 to the global r80=128. Prediction: most clusters have r80
FAR below 128 at the 80% bar (clustering does reduce functional rank).
ORIG NAMING DOC BELOW.
...
OUT = PT + 'rspd_cluster_r80_results.json'
LAYER = 1
NFIT = 24
NEVAL = 48
K = 8
...
@torch.no_grad()
def ce_masked_ablate(rows, n, target):
    """CE over target-cluster tokens with that cluster's mlp1 output ZEROED."""
    global _ABLATE_TGT
    _ABLATE_TGT = target
    ce_s = 0.0; cnt = 0
    for i in range(0, n, 4):
        ...
        x = F.rms_norm(m.transformer.wte(idx), (D,)); x0 = x; v1 = None
        cap = {}
        h1 = m.transformer.h[LAYER].mlp.register_forward_hook(
            lambda mo, i_, o_: cap.__setitem__('o', o_.detach().float().reshape(-1, D)))
        # second hook zeroes target-cluster tokens
        def zhook(mo, i_, o_):
            flat = o_.float().reshape(-1, D)
            on = flat / flat.norm(dim=1, keepdim=True).clamp_min(1e-9)
            a = (on @ CFG['C'].T).argmax(1)
            flat = flat.clone(); flat[a == target] = 0.0
            return flat.reshape(o_.shape).to(o_.dtype)
        h2 = m.transformer.h[LAYER].mlp.register_forward_hook(zhook)
        for li, blk in enumerate(m.transformer.h):
            x, v1 = blk(x, v1, x0)
        h1.remove(); h2.remove()
        on = cap['o'] / cap['o'].norm(dim=1, keepdim=True).clamp_min(1e-9)
        ...
        tt = tgt.reshape(-1)
        if amask.any():
            ce_s += float(F.nll_loss(lp[amask], tt[amask], reduction='sum')); cnt += int(amask.sum())
    return ce_s / max(cnt, 1)
...
def main():
    ...
    for j in range(K):
        ...
        # per-cluster r80 at the 80% bar (same as global r80)
        CFG['on'] = False
        ce_full = ce_masked(ev, NEVAL, j)
        CFG['on'] = True; CFG['r'] = 0; CFG['target'] = j   # r=0 subspace -> ablate cluster
        ce_abl = ce_masked_ablate(ev, NEVAL, j)
        ben = ce_abl - ce_full
        frank = RANKS[-1]
        for r in RANKS:
            CFG['on'] = True; CFG['r'] = r; CFG['target'] = j
            rec = (ce_abl - ce_masked(ev, NEVAL, j)) / max(ben, 1e-6)
            if rec >= 0.80:
                frank = r; break
        CFG['on'] = False
        # output readout
