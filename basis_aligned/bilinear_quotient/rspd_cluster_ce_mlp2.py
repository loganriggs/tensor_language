...
OUTPUT subspaces reproduce a high-rank layer (mlp2) better than a single
global low-rank subspace at the same rank r?

Method: fit clusters on mlp2's Down-OUTPUT direction over a fit set; per
cluster, take the top-r left-singular directions of that cluster's outputs
(its own subspace). At eval, hook mlp2's output; for each token assign it
...
zeroes mlp2's output (keeps bias). If clustered > global at small r, the
...
OUT = PT + 'rspd_cluster_ce_mlp2_results.json'
LAYER = 2
NFIT = 24
NEVAL = 48
K = 8
...
def main():
    ...
    O = capture_out(fit, NFIT)                       # (Nfit, D) mlp2 outputs
