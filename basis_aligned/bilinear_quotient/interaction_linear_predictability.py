...
NTRAIN = 3000; NTEST = 1500; RIDGE = 1e-2
CAP = {}
...
@torch.no_grad()
def ridge_r2(X, Y, ntr):
    # X:(N,D) Y:(N,Dy); fit W on first ntr rows, R^2 on the rest
    Xtr, Xte = X[:ntr], X[ntr:]; Ytr, Yte = Y[:ntr], Y[ntr:]
    # center Y by train mean; augment X with bias column
    Xtr1 = torch.cat([Xtr, torch.ones(Xtr.shape[0], 1, device=X.device)], 1)
    Xte1 = torch.cat([Xte, torch.ones(Xte.shape[0], 1, device=X.device)], 1)
    A = Xtr1.T @ Xtr1 + RIDGE * torch.eye(Xtr1.shape[1], device=X.device)
    W = torch.linalg.solve(A, Xtr1.T @ Ytr)
    pred = Xte1 @ W
    ss_res = float(((Yte - pred)**2).sum())
    ss_tot = float(((Yte - Yte.mean(0, keepdim=True))**2).sum())
    return round(1.0 - ss_res/max(ss_tot, 1e-9), 4)
...
def main():
    ...
    for L in LAYERS:
        ...
        r2_int = ridge_r2(X, tgt_int, ntr)
        r2_full = ridge_r2(X, tgt_full, ntr)
        out['layers'][str(L)] = {'r2_int': r2_int, 'r2_full': r2_full, 'ntrain': ntr, 'ntest': X.shape[0]-ntr}
        print(f"L{L:>2}: R^2 interaction {r2_int} | R^2 full {r2_full} (ntr {ntr})", flush=True)
    ...
    out['pred_a_interaction_linear_at_front'] = bool(fi > 0.7 and fi > mi + 0.2)
