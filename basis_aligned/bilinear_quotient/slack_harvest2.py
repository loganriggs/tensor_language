"""SLACK HARVEST v2 -- sign-calibrated (328: SVD mode signs are
arbitrary; v1 assumed negative=improves and hurt the model). v2
calibrates per mode ON THE FIT WINDOW: ablate the mode's top-2
components on fit rows, measure each polarity's extreme members, and
harvest ONLY a polarity whose fit-window members IMPROVE (skip modes
with none -- reported). Applied to fresh rows at the calibrated
polarity's extreme 8%.
REGISTERED PREDICTIONS (fresh):
  (a) net Delta-CE < 0 (calibrated policy improves the real model);
  (b) random-position control (matched fractions) >= 0;
  (c) per-mode polarity + contribution reported."""
...
OUT=PT+'slack_harvest2_results.json'
...
def main():
    ...
    qlo=scA.quantile(0.08,dim=0)
    # per-mode: top-2 loading components + negative-score mask on fresh
    modes=[]
    qloA=scA.quantile(0.08,dim=0); qhiA=scA.quantile(0.92,dim=0)
    qloF=scF.quantile(0.08,dim=0); qhiF=scF.quantile(0.92,dim=0)
    for k in range(10):
        L=Vh9[k]
        topc=[P36[i] for i in L.abs().argsort(descending=True)[:2]
              .tolist()]
        hooks=[]
        for c in topc: hooks+=mp(c)
        dfit=(ce_vec_rows(rowsA,nA,hooks)-baseA).cpu()
        negA=scA[:,k]<qloA[k]; posA=scA[:,k]>qhiA[k]
        effneg=float(dfit[negA].mean()) if negA.any() else 9
        effpos=float(dfit[posA].mean()) if posA.any() else 9
        pol=None
        if effneg<0 and effneg<=effpos: pol='neg'
        elif effpos<0: pol='pos'
        if pol is None:
            print(f'mode {k}: no improving polarity (neg {effneg:+.3f} '
                  f'pos {effpos:+.3f}) -- skipped',flush=True)
            continue
        maskk=((scF[:,k]<qloF[k]) if pol=='neg'
               else (scF[:,k]>qhiF[k])).view(120,256)
        modes.append((k,topc,maskk))
        print(f'mode {k}: comps {topc} polarity {pol} (fit neg '
              f'{effneg:+.3f} pos {effpos:+.3f}) frac '
              f'{float(maskk.float().mean()):.2%}',flush=True)
    if not modes:
        print('no harvestable modes -- (a) FAILED vacuously',flush=True)
    def policy_hooks(masks):
        # masks: dict comp-key -> (120,256) bool (union over modes)
        hs=[]
        for key,mk_ in masks.items():
            mu=mus2[key]; mod=MODS2[key]
            if key[0]=='a':
                def h(mo,i_,o_,mu=mu,key=key):
                    y,v1=o_
                    gv=cur['pmask'][key].view(-1)[:,None].to(DEV)
                    yf=y.float().reshape(-1,D)
                    new=torch.where(gv,mu[None,:].expand_as(yf),yf)
                    return (new.view(y.shape).to(y.dtype),v1)
            else:
                def h(mo,i_,o_,mu=mu,key=key):
                    gv=cur['pmask'][key].view(-1)[:,None].to(DEV)
        ...
        return hs
    def build_masks(mode_list):
        mk={}
        for k,topc,mask in mode_list:
            for c in topc:
                mk[c]=mk.get(c,torch.zeros(120,256,dtype=torch.bool))                    |mask
        return mk
    def evalP(mode_list,masks):
        hs=policy_hooks(masks)
        ces=[]
        for i in range(0,120,4):
            bb=FR[i:i+4,:257].to(DEV)
            idx=bb[:,:-1].contiguous(); tg=bb[:,1:].reshape(-1)
            cur['pmask']={key:mm[i:i+4] for key,mm in masks.items()}
            x=F.rms_norm(m.transformer.wte(idx),(D,)); x0=x; v1=None
            for blk in m.transformer.h:
                x,v1=blk(x,v1,x0)
            lg=(30*torch.tanh(m.lm_head(F.rms_norm(x,(D,)))/30)).float()
        ...
        for h in hs: h.remove()
        return float(torch.cat(ces).mean())
    b0=float(baseFr.mean())
    masks=build_masks(modes)
    touched=float(torch.stack(list(masks.values())).any(0)
                  .float().mean())
    dpol=evalP(modes,masks)-b0
    g8=torch.Generator().manual_seed(9)
    rmodes=[]
    for k,topc,mask in modes:
        fr2=float(mask.float().mean())
        rmodes.append((k,topc,
                       torch.rand(120,256,generator=g8)<fr2))
    rmasks=build_masks(rmodes)
    drnd=evalP(rmodes,rmasks)-b0
    print(f'policy dCE {dpol:+.4f} | random-position control '
          f'{drnd:+.4f} | touched {touched:.2%}')
    pa=dpol<0; pb=drnd>=0
    out={'delta_policy':round(dpol,4),'delta_random':round(drnd,4),
         'touched_fraction':round(touched,4),
         'pred_a':bool(pa),'pred_b':bool(pb)}
    print(f"(a) policy improves the real model: "
          f"{'HELD' if pa else 'FAILED'}")
    print(f"(b) random-position control >= 0: {'HELD' if pb else 'FAILED'}")
