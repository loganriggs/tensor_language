"""MLP1 UNIT CLUSTER -- does the validated method (578 toy, 579/581
mlp0 real, three named groups, two causally confirmed in 584/583)
generalize to a SECOND layer, and do its groups differ from mlp0's?

Same recipe as mlp0_unit_cluster.py + mlp0_unit_cluster_examples.py,
combined and applied to mlp1 (LJ=1) instead of mlp0 (LJ=0), with
context-based naming from the start -- 579 already established that
a logit-lens naming probe is the wrong lens for these input-selective
units, so this skips straight to reading real activating examples.

The question this adds beyond "does mlp0 have reusable groups": is
mlp1 doing similar work to mlp0 (same kind of groups: article choice,
punctuation, contraction), or does the FIRST distinctive structure
appear at layer 1 because layer 0 already handled the token-level
stuff (503, 507: layer 0 = token-indexed lookup table, exactly)? If
mlp1's groups read as genuinely different classes of behaviour from
mlp0's three (article/punctuation/aux-contraction), that is evidence
the model allocates DIFFERENT layers to different linguistic
sub-problems rather than every early MLP doing generic overlapping
work -- a piece of "where is what computed" this program has been
building toward since the depth map.

REGISTERED PREDICTIONS:
  (0) SANITY: full-component reconstruction (all units kept) exactly
      reproduces the model (cost < 1e-3 nats) -- inherited from
      mlp0_units' identity check, re-verified here for mlp1;
  (a) STABILITY: split-half reclustering of the top-300 units gives
      Adjusted Rand Index >= 0.3 versus a permutation null of ~0 --
      579's bar (it got 0.58); report whether mlp1 clusters as
      cleanly as mlp0 did or more weakly;
  (b) CONCENTRATION: for the 3 largest clusters, at least 1 of 3 has
      >= 5/8 positive-example concentration on one surface class
      (mlp0_units' fine_class taxonomy) -- 581's bar, applied fresh;
  (c) DISTINCTNESS (the new question): compare mlp1's 3 largest
      clusters' example classes against mlp0's three named readings
      (article choice, sentence-final-vs-dash punctuation, aux-vs-
      contraction) -- report whether they overlap or are visibly
      different behaviours. No bar, this is the finding;
  NULL: shuffling sample alignment before clustering drops the
      stability ARI to < 0.1 (579's null, re-run here)."""
...
OUT = PT + 'mlp1_unit_cluster_results.json'
NFRESH = 64
NSAMP = 4000
TOPK = 300
NCLUST = 20
...
def main():
    ...
    print(f'(c) distinctness vs mlp0 (article/punct-subtype/aux-contraction) '
          f'-- read the printed examples above.', flush=True)
