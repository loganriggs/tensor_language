# payload_live160: generalization certification of the FULL reduction (§1260) — same
# conditions on PERIOD-160 repeat rows (source at t-160, an offset the axis map and the
# §1239 coefficients never saw; §1258 certified the matcher half generalizes).
#
# Registered predictions:
#   pred_a FULL REDUCTION GENERALIZES: recovery(static payload) >= 0.60 on period-160.
#   pred_b STATIC ≈ LIVE there too: |recovery(static) − recovery(live)| <= 0.05.
#   pred_c AXIS-ONLY BASELINE: static beats axis_only by >= 0.08 (payload adds off-offset too).
...
OUT = PT + 'payload_live160_results.json'
...
def main():
    ...
    qp = torch.arange(160, T, device=DEV)
    ...
    out = {'n_rows': NR, 'ce': CE, 'quad_damage': round(dmg, 4), 'recovery': rec,
           'pred_a_generalizes': bool(rec['static'] >= 0.60),
           'pred_b_static_eq_live': bool(abs(rec['static'] - rec['live']) <= 0.05),
           'pred_c_payload_adds': bool(rec['static'] >= rec['axis_only'] + 0.08),
           'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"CE {CE} | damage {dmg:.4f} | recovery {rec}")
    print(f"pred_a gen {out['pred_a_generalizes']} | pred_b eq {out['pred_b_static_eq_live']} | pred_c adds {out['pred_c_payload_adds']}")
