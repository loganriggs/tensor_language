"""BOTTOM-UP: push the front MLPs below 90% (mlp0 0.67, mlp2/3/4) with a matched stand-in. The front MLP output is a
...
OUT = PT + 'bottomup_mlp_window_results.json'
NEVAL = 200; SEQ = 256; LAYERS = [0, 1, 2, 3, 4]; RIDGE = 1e3; NPREV = 3
...
def main():
    ...
    for L in LAYERS:
        ...
        out['layers'][str(L)] = {'meanabl_cost': round(ce_ma-ce_full,3), 'recovery_token': rt, 'recovery_token_window': rtw, 'gain': round(rtw-rt,3)}
        print(f"mlp{L}: token {rt} | token+window {rtw} | gain {out['layers'][str(L)]['gain']} (meanabl {ce_ma-ce_full:.3f})", flush=True)
    for h in hooks: h.remove()
    out['pred_a_window_lifts'] = bool(out['layers']['0']['gain'] > 0.05 and out['layers']['0']['recovery_token_window'] > out['layers']['0']['recovery_token'])
    out['runtime_s'] = round(time.time()-t0, 1)
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"pred_a window lifts front MLPs: {out['pred_a_window_lifts']}", flush=True)
