# pattern_fold_map2: the 162-head window-foldability map, EXACT-PREFIX FIX (§1164).
#
# v1's window_resid clamp-repeated token 0 for positions t < W-1, corrupting all early-
# position keys (sink 5.7 read 0.002 — impossible; whole map understated). Fix: for t < W
# the true window IS the prefix, so one forward over the first W tokens gives exact x_hat
# for positions 0..W-1; sliding windows cover t >= W.
#
# Registered predictions (re-registered from §1164):
#   pred_a GLOBAL LAW STRENGTHENS: every layer's mean hit @W128 rises or holds vs v1
#          (none falls by > 0.01); all >= 0.68.
#   pred_b SINK RECOVERS: head 5.7 >= 0.9 at BOTH widths (constant position-0 fetch).
#   pred_c L5 RECOVERS: layer-5 mean @W128 >= 0.85.
# Nulls: shuffled-row x_hat at W=32 for layers {2, 9, 16} as v1.
#
# --- v1 header below for provenance ---
#
...
OUT = PT + 'pattern_fold_map2_results.json'
...
def window_resid(tokens, W, nblocks):
    ...
    # EXACT-PREFIX FIX: positions 0..W-1 from one true-prefix forward
    Wp = min(W, Tn)
...
def main():
    ...
    out = {'n_rows': NR, 'per_head': table, 'nulls_W32_shuffled': nulls,
           ...
           'pred_a_global_law': bool(all(mean128[L] >= 0.68 for L in range(18))),
           'pred_b_sink_recovers': bool(sink57[0] >= 0.9 and sink57[1] >= 0.9),
           'pred_c_L5_recovers': bool(mean128[5] >= 0.85),
    ...
    print(f"pred_a global {out['pred_a_global_law']} | pred_b sink {out['pred_b_sink_recovers']} | pred_c L5 {out['pred_c_L5_recovers']}")
