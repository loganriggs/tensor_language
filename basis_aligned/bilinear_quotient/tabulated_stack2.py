...
OUT = PT + 'tabulated_stack2_results.json'
NR = 16; W = 128
...
def m0_bigram_hook(mo, i_, o_):
    ...
    mask = torch.tril(torch.ones(Tn, Tn, device=DEV)) * ((ar[:, None] - ar[None, :]) < 2).float()
    pat = (sc * sc2) * mask
...
def main():
    ...
    out = {'n_rows': NR, 'W': W, 'ce': CE, 'cost_vs_base': cost,
           ...
           'pred_a_m0_replicates': bool(cost['m0big'] <= 0.02),
           'pred_b_compatible': bool(cost['stack3'] <= cost['stack2'] + cost['m0big'] + 0.01),
           'pred_c_stack3_cheap': bool(cost['stack3'] <= 0.05),
