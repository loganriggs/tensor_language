# full_window_family: §1180's whole-model window curve on swiglu18 (family symmetry).
# Registered: (a) monotone; (b) W=128 total within 2x of bilin18's 0.0816;
# (c) implied values share @128 = total - 0.0148 (selection, §1170) stays the dominant term.
#
...
OUT = PT + 'full_window_family_results.json'
...
def main():
    ...
    out = {'n_rows': NR, 'scored_positions': f'{QSTART}..{T} stride {STRIDE}', 'ce': CE,
           ...
           'pred_b_w128_2x': bool(cost['W128'] <= 2 * 0.0816),
           'pred_c_values_dominant': bool(cost['W128'] - 0.0148 > 0.0148),
           'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"base CE {CE['base']} | costs {cost}")
    print(f"pred_a monotone {out['pred_a_monotone']} | pred_b 2x {out['pred_b_w128_2x']} | pred_c values-dom {out['pred_c_values_dominant']}")
