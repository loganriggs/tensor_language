"""Covariance-matched null for section 129. The 0.71 within-layer filter
...
OUT=('/workspace/tensor_language/basis_aligned/bilinear_quotient/'
     'bilin18_filter_null_results.json')
...
def main():
    ...
    mpairs=[(orth(Ch2@torch.randn(D,8,device=DEV,generator=g)),
            ...
            for _ in range(100)]
    rm=med_cos(mpairs)
    pa=(w-rm)>=0.2; void=rm>=0.5
    out={'within':w,'across':x,'random_iso':r,'random_matched':rm,
         'pred_a_lexicon':bool(pa),'voided':bool(void)}
    print(f'within {w:.2f} | across {x:.2f} | iso null {r:.2f} | '
          f'MATCHED null {rm:.2f}')
    print(f"(a) lexicon real (within - matched >= 0.2): {'HELD' if pa else 'FAILED'}")
    if void: print('alternative fired: sharing is covariance concentration -- claim voided')
