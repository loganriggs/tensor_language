# attn_composite7: CONFIG-MATCHED KERNEL TRAINING + OVERFIT-vs-SHIFT DIAGNOSIS
# (S1460: CE-trained kernels set the record 4.013 but (1) were trained in kernel_all
# config and evaluated with roster — a mismatch, and (2) showed a .24 train/eval gap
# whose cause is unresolved. Here: train WITH the roster live (matching eval), double
# data and steps (960 rows, 600 steps), and evaluate the trained kernels on THREE row
# sets: held-out skip=7000, FRESH same-corpus skip=2000 (never trained), and the
# training rows themselves. fineweb skip regions are the same distribution, so
# skip2000 ~ skip7000 => the S1460 gap was memorization of the 480 rows, not shift.)
# Original header: TRAIN THE KERNELS AGAINST CE IN-COMPOSITE (S1458: the fixed-point
...
#   pred_a roster-config-trained kernels + roster <= 3.95 (config match + 2x data beats 4.013).
#   pred_b |CE(skip2000 fresh) - CE(skip7000 fresh)| <= .03 (no distribution shift —
#          the S1460 gap was training-row memorization).
#   pred_c CE(training rows) at least .10 BELOW CE(fresh rows) (the memorization is real).
...
OUT = PT + 'attn_composite7_results.json'
NMEAN = 24; NR = 960; NTR = 960; STEPS = 600; LR = 3e-3
...
def main():
    ...
    for step in range(STEPS):
        ...
        bb = TRR[sel].to(DEV)
        idx = bb[:, :-1].contiguous(); tg = bb[:, 1:].contiguous()
        lo = fwd_comp(idx, curves, roster=True).float()
        ce = F.cross_entropy(lo.reshape(-1, lo.shape[-1]), tg.reshape(-1),
                             reduction='none').view(tg.shape)
        loss = ce[:, 64:].mean()
    ...
    EVR2 = cl.fineweb_rows(NR, skip=2000)[:, :T + 1].contiguous()
    res['roster_trained_7000'] = round(ce_eval(EVR, trained, roster=True), 4)
    res['roster_trained_2000'] = round(ce_eval(EVR2, trained, roster=True), 4)
    res['roster_trained_trainrows'] = round(ce_eval(TRR[:NR], trained, roster=True), 4)
    print(f"7000 {res['roster_trained_7000']} 2000 {res['roster_trained_2000']} "
          f"train {res['roster_trained_trainrows']}", flush=True)

    fresh_gap = abs(res['roster_trained_2000'] - res['roster_trained_7000'])
    memo = min(res['roster_trained_2000'], res['roster_trained_7000']) \
        - res['roster_trained_trainrows']
    pa = res['roster_trained_7000'] <= 3.95
    pb = fresh_gap <= 0.03
    pc = memo >= 0.10
    out = {'ce': res, 'fresh_gap': round(fresh_gap, 4), 'memo_gap': round(memo, 4),
           'loss_curve': curve_log,
           'data_budget': {'train_rows': NTR, 'steps': STEPS, 'batch': 8, 'lr': LR},
           'pred_a_roster_le_395': bool(pa), 'pred_b_no_shift_03': bool(pb),
           'pred_c_memo_ge_10': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
