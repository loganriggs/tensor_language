"""Completes the family phenotype sweep (§1137: swiglu18 = same width, different MLP — soup near-verbatim,
stream exact, content dissociation 6.9x). Now CROSS-WIDTH: bilin12 (12 layers, D=768). Bands remapped to its
depth: content L3-9, grammar L0-1, random control L5+L8, content basis from its own L5/6/7 (the §1057/§1063
reference layers). Same prompts, seed, metrics.

REGISTERED PREDICTIONS:
  (0) SANITY: base near natural; random control ≈ base.
  (a) PHENOTYPES CROSS WIDTH: content-abl craters cw-rate >= 1.5x grammar-abl AND vres-off shows the stream
      signature (rep4 <= 0.005, cw-rate >= natural) -> soup + stream are properties of the TRAINING
      DISTRIBUTION + architecture family at any width; report whether the soup text is again near-verbatim;
  (b) grammar-failure attractor reported (bilin18 fragments / swiglu18 loops / bilin12 = ?) — the flavor is the
      model-specific part;
  (c) divergence -> the phenotypes need scale (report plainly)."""
...
OUT = PT + 'gen_family12_results.json'
NP = 96; PLEN = 64; GLEN = 64; REF = [5, 6, 7]; K = 64
CONTENT_BAND = list(range(3, 10)); GRAMMAR_BAND = [0, 1]; RAND_BAND = [5, 8]
DEV = 'cuda'
...
def main():
    ...
    print(f"bilin12 natural: cw {nat_cw:.3f} | legal {legal_rate(natural):.3f}", flush=True)
    ...
    out = {'model': 'bilin12', 'natural_cw': round(nat_cw, 4), 'conditions': res, 'texts': texts,
