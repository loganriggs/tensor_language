# ship_all36_v3: K=2048 UNIT PLANKS (S1537: each K-doubling adds ~.07-.15; K=2048
# planks should buy another ~.2-.4 CE).
...
#   pred_a ship36_v3 <= 4.30 on skip=7000.
#   pred_b beats v2 (4.5540) by >= .20.
#   pred_c fresh-row band <= .05.
...
OUT = PT + 'ship_all36_v3_results.json'
NR = 960
...
def main():
    ...
    pa = res['ship36'] <= 4.30
    pb = (4.5540 - res['ship36']) >= 0.20
    pc = abs(res['ship36_fresh2000'] - res['ship36']) <= 0.05
    out = {'ce': res, 'delta36': round(d36, 4),
           ...
           'pred_a_le_430': bool(pa), 'pred_b_beats_v2_20': bool(pb),
           'pred_c_fresh_le_05': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
