"""[v2: topic-DEVIATION direction + alpha sweep + more batch] CAUSAL verification of the TOPIC finding (§866): does the content machine's topic representation
...
OUT = PT + 'topic_steer2_results.json'; NEVAL = 260; RTOK = 64; RPOS = 32; K = 12; READ_L = 15; ALPHAS = [16.0, 64.0, 256.0]; NDISTINCT = 30
...
def mk_hook():
    def hook(mo, i_, o_):
        ...
        v2 = v + globals()['ALPHA'] * ST['vec'].to(v.dtype)
...
def main():
    ...
    for j in range(K):
        ...
        vec = content[torch.tensor(mk, device=DEV)].mean(0) - content.mean(0); vec = vec/ (vec.norm()+1e-9)
    ...
    idxb = rows[:32, :257].to(DEV)[:, :-1].contiguous()
    ...
    for ALPHA in ALPHAS:
        ...
        for A in tids:
            ...
            row = {B: round(mean_logit_on(lg, topics[B]['dtok']) - mean_logit_on(base_logits, topics[B]['dtok']), 3) for B in tids}
            mat[A] = row
        dg = float(np.mean([mat[A][A] for A in tids])); og = float(np.mean([mat[A][B] for A in tids for B in tids if B!=A]))
        print(f"alpha {ALPHA}: mean diagonal {dg:.3f} | off-diag {og:.3f}", flush=True)
        if best is None or (dg-og) > best[3]: best = (ALPHA, mat, dg, dg-og, og)
    hh.remove()
    ALPHA, mat, _, _, _ = best
    diag = [mat[A][A] for A in tids]; off = [mat[A][B] for A in tids for B in tids if B != A]
    out = {'k': K, 'best_alpha': ALPHA, 'n_topics': len(tids), 'topic_names': {A: topics[A]['name'] for A in tids},
