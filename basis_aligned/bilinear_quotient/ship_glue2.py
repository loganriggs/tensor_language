# ship_glue2: SEQUENTIAL (GREEDY) GLUE (S1492: joint 3-slot training UNDERPERFORMED
# the single-slot run — interference + unconverged. Coordinate descent instead:
# train mlp2's slot 400 steps, FREEZE it, then train mlp1's slot 400 steps, then
# mlp0's 200. Same param shapes, lr 2e-3.)
# Original header: JOINT CE-TRAINED CORRECTIONS FOR ALL THREE MLP SLOTS (S1489: a single
...
#   pred_a sequential-glued all4 <= 3.50 (beats both joint 3.615 and single 3.560).
#   pred_b the mlp1-slot stage adds >= .05 on top of the mlp2 stage.
#   pred_c final gap to attn-only (3.107) <= .38.
...
OUT = PT + 'ship_glue2_results.json'
NR = 960
...
def main():
    ...
    def zero_slot(ind):
        return (torch.zeros(D, device=DEV), torch.zeros(D, 32, device=DEV),
                torch.zeros(ind, 32, device=DEV))

    GLUE[0] = zero_slot(D); GLUE[1] = zero_slot(2 * D); GLUE[2] = zero_slot(2 * D)
    GLUE['on'] = True

    def train_slot(slot, ind, steps, seed):
        GLUE[slot] = make_slot(ind)
        opt = torch.optim.Adam(list(GLUE[slot]), lr=2e-3)
        ...
        for step in range(steps):
            ...
            cur.append(round(float(loss), 4))
            if step % 80 == 0:
                print(f"  slot{slot} step {step} loss {float(loss):.4f}", flush=True)
        GLUE[slot] = tuple(t.detach() for t in GLUE[slot])
        return cur

    curves = {}
    curves['s2'] = train_slot(2, 2 * D, 400, 5)
    res['after_mlp2'] = round(ce2(ALLL, M4), 4)
    print(f"after mlp2 {res['after_mlp2']}", flush=True)
    json.dump({'partial': True, 'res': res}, open(OUT, 'w'), indent=1)
    curves['s1'] = train_slot(1, 2 * D, 400, 6)
    res['after_mlp1'] = round(ce2(ALLL, M4), 4)
    print(f"after mlp1 {res['after_mlp1']}", flush=True)
    json.dump({'partial': True, 'res': res}, open(OUT, 'w'), indent=1)
    curves['s0'] = train_slot(0, D, 200, 7)
    res['after_mlp0'] = round(ce2(ALLL, M4), 4)
    GLUE['on'] = False
    print(res, flush=True)

    final = res['after_mlp0']
    pa = final <= 3.50
    pb = (res['after_mlp2'] - res['after_mlp1']) >= 0.05
    pc = (final - 3.1074) <= 0.38
    out = {'ce': res, 'stage_gains': {'mlp2': round(res['all4_noglue']
                                                    - res['after_mlp2'], 4),
                                      'mlp1': round(res['after_mlp2']
                                                    - res['after_mlp1'], 4),
                                      'mlp0': round(res['after_mlp1'] - final, 4)},
           'gap_to_attn_only': round(final - 3.1074, 4), 'curves': curves,
           'data_budget': {'steps': '400+400+200', 'batch': 8, 'lr': 2e-3},
           'pred_a_seq_le_350': bool(pa), 'pred_b_mlp1_stage_05': bool(pb),
           'pred_c_gap_le_38': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"final {final} stages {out['stage_gains']}")
