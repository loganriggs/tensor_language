# ship_error_mine: WHAT DOES THE GLASS SHIP STILL GET WRONG? (The remaining ~.9 CE
# mapped to vocabulary — the generator for the next round.) Per-target-token CE
# rise under the full v4 ship vs clean (NR=1920); top-100 damaged tokens decoded;
# damage split by target frequency (eval-count rank </> 500) and copy status
# (target in previous 64 context tokens).
...
#   pred_a the novel-and-rare cell carries >= .45 of total ship damage (S1554
#          quantified).
#   pred_b copied targets (any frequency) carry <= .15 combined.
#   pred_c the top-100 damaged tokens carry >= .20 of total damage (actionably
#          concentrated).
...
OUT = PT + 'ship_error_mine_results.json'
NR = 1920
...
def main():
    ...
    def per_token(layers, mlps):
        tsum = torch.zeros(50257); tn = torch.zeros(50257)
        csum = {k: 0.0 for k in ('copy', 'novel_freq', 'novel_rare')}
        cn = {k: 0 for k in ('copy', 'novel_freq', 'novel_rare')}
        ...
        for i in range(0, NR, 8):
            ...
            ecnt.index_add_(0, tgf, mk.cpu().reshape(-1).float())
        return tsum, tn, ecnt

    ts0, tn0, ecnt = per_token(frozenset(), frozenset())
    ts1, tn1, _ = per_token(ALLL, ALLM)
    print("both passes done", flush=True)

    # copy-status needs positionwise masks — one more joint pass computing cells
    thresh = ecnt.sort(descending=True).values[500]
    RARE = (ecnt < thresh)

    def cells(layers, mlps):
        cs = {k: 0.0 for k in ('copy', 'novel_freq', 'novel_rare')}
        cn = {k: 0 for k in ('copy', 'novel_freq', 'novel_rare')}
        for i in range(0, NR, 8):
            bb = EVR[i:i + 8].to(DEV)
            idx = bb[:, :-1].contiguous(); tg = bb[:, 1:].contiguous()
            ...
            ce = F.cross_entropy(lo.reshape(-1, lo.shape[-1]), tg.reshape(-1),
                                 reduction='none').view(tg.shape)
            mk = torch.ones_like(tg, dtype=torch.bool); mk[:, :64] = False
            copy = torch.zeros_like(mk)
            for lag in range(1, 65):
                past = torch.roll(idx, lag, dims=1)
                past[:, :lag] = -1
                copy |= (past == tg)
            copy &= mk
            rare = RARE.to(DEV)[tg] & mk
            masks = {'copy': copy, 'novel_freq': mk & ~copy & ~rare,
                     'novel_rare': mk & ~copy & rare}
            for k, ms in masks.items():
                cs[k] += float(ce[ms].sum()); cn[k] += int(ms.sum())
        return cs, cn

    cs0, cn0 = cells(frozenset(), frozenset())
    cs1, _ = cells(ALLL, ALLM)
    total_dmg = sum(cs1[k] - cs0[k] for k in cs0)
    shares = {k: round((cs1[k] - cs0[k]) / max(total_dmg, 1e-6), 3) for k in cs0}
    print("cells:", shares, flush=True)

    rise_w = torch.where(tn0 >= 20, ts1 / tn1.clamp_min(1) - ts0 / tn0.clamp_min(1),
                         torch.zeros(50257))
    dmg_w = rise_w * tn0
    top = dmg_w.argsort(descending=True)[:100]
    top_share = float(dmg_w[top].sum() / dmg_w.clamp_min(0).sum())
    top_list = [[ENC.decode([int(t)]), round(float(rise_w[t]), 3),
                 int(tn0[t])] for t in top[:40]]
    print("top damaged:", [t[0] for t in top_list[:20]], flush=True)

    pa = shares['novel_rare'] >= 0.45
    pb = shares['copy'] <= 0.15
    pc = top_share >= 0.20
    out = {'cell_shares': shares,
           'cell_n': cn0, 'top40_tokens': top_list,
           'top100_damage_share': round(top_share, 3),
           'pred_a_novel_rare_45': bool(pa), 'pred_b_copy_le_15': bool(pb),
           'pred_c_top100_20': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
