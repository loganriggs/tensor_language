"""PROBE GATE v2 -- close the AUC gap. v1 (linear ridge on the
block-2 stream -> binary label) got AUC 0.621, eff 3.8x random
(HELD), 42% of oracle. v2 upgrades the probe, same read point and
deploy story: (i) quadratic features in mlp3's own read directions
(quadfeat, 1680 dims) -- the model is bilinear, give the probe its
native basis; (ii) regress the 10 oracle mode scores directly and
gate on any-mode threshold crossing (matching the oracle gate's
construction) instead of collapsing to one binary.
REGISTERED PREDICTIONS (fresh):
  (a) probe AUC vs oracle labels >= 0.70;
  (b) probe-gated efficiency >= 3x random at matched fraction;
  (c) probe recovers >= 60% of the oracle gain at the same fraction."""
...
def main():
    ...
    # ---- deploy-legal probe: stream after block 2 -> oracle label ----
    IREAD=2
    def stream_rows(TOK,N):
        outs=[]
        for i in range(0,N,4):
            bb=TOK[i:i+4,:257].to(DEV)
            idx=bb[:,:-1].contiguous()
            ...
            for lj,blk in enumerate(m.transformer.h):
                x,v1=blk(x,v1,x0)
                if lj==IREAD: break
            outs.append(x.float().reshape(-1,D))
        return torch.cat(outs)
    XA=stream_rows(rowsA,nA); FAq=quadfeat(XA,3); del XA
    Yt=scA.to(DEV)
    lamp=1e-2*len(FAq)
    Wp=torch.linalg.solve(FAq.T@FAq+lamp*torch.eye(FAq.shape[1],
                                                   device=DEV),
                          FAq.T@Yt)
    predA=(FAq@Wp); del FAq
    thrq=predA.abs().quantile(0.97,dim=0).cpu(); del predA
    XF=stream_rows(FR,120); FFq=quadfeat(XF,3); del XF
    predF=(FFq@Wp).cpu(); del FFq
    sFp=(predF.abs()/thrq[None,:].clamp_min(1e-6)).max(1).values
    ...
    aucP=float((rk[pos].sum()-npos*(npos-1)/2)/(npos*nneg))
    print(f'probe AUC (fresh vs oracle labels): {aucP:.3f}',flush=True)
    thrP=sFp.quantile(1-frac)
    gateP=(sFp>=thrP).view(120,256)
    print(f'probe gated fraction: {float(gateP.float().mean()):.2%}',
          flush=True)
    ...
    prb=evalG(order2,ML,gateP)-float(baseFr.mean())
    del cur['clsmap']
    gain=baseC2-gat; rgain=baseC2-rnd; pgain=baseC2-prb
    print(f'base config {baseC2:+.4f} | oracle {gat:+.4f} (gain '
          f'{gain:+.4f}) | probe {prb:+.4f} (gain {pgain:+.4f}) | '
          f'random {rnd:+.4f} (gain {rgain:+.4f})')
    effP=pgain/max(rgain,1e-4)
    pa=aucP>=0.75
    pb=effP>=2.5
    pc=pgain>=0.5*max(gain,1e-4)
    out={'base':round(baseC2,4),'oracle':round(gat,4),
         'probe':round(prb,4),'random':round(rnd,4),
         'fraction':round(frac,4),'auc':round(aucP,4),
         'probe_eff_vs_random':round(effP,3),
         'probe_frac_of_oracle':round(pgain/max(gain,1e-4),3),
         'pred_a':bool(pa),'pred_b':bool(pb),'pred_c':bool(pc)}
    print(f"(a) probe AUC >= 0.75: {'HELD' if pa else 'FAILED'}")
    print(f"(b) probe eff >= 2.5x random: {'HELD' if pb else 'FAILED'}")
    print(f"(c) probe >= 50% of oracle gain: "
