# exclaim_screen2: BEHAVIOUR-FIRST screen for EXCLAMATORY-REGISTER tracking — predicting
...
#   pred_b ATTN17 BET (registered from the withheld v1): winner is attn17.
...
OUT = PT + 'exclaim_screen2_results.json'
NMEAN = 24; NR = 960
H = m.transformer.h
...
def main():
    ...
    out = {'n_targets': ntar, 'base': {k: round(v, 4) for k, v in base.items()},
           ...
           'pred_a_carrier': bool(pa), 'pred_b_attn17': bool(pb), 'pred_c_rand_flat': bool(pc),
           'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"top5 {out['top5']}")
    print(f"pred_a carrier {pa} | pred_b attn17 {pb} | pred_c rand {pc}")
