"""VARIABLE-LEVEL causal abstraction, step 2: is the TOPIC/gist variable causal via INTERCHANGE INTERVENTION?
...
OUT = PT + 'das_topic_interchange_results.json'
NEVAL = 400; SEQ = 256; QP = 128; PATCH_L = 15; K = 12; RTOK = 64; RPOS = 32; NDIST = 40
...
def main():
    ...
    topic_all = kmeans(cn, K).cpu().numpy().reshape(nb, SEQ)
    # topic subspace = content-cluster mean deviations (rank ~K-1), orthonormalized
    Utopic_raw, gc = mean_subspace(content, topic_all.reshape(-1), K)
    ...
    out['pred_a_topic_causal_interchange'] = bool(out['mean_delta_topic_patch'] > out['mean_delta_random_patch'] + 0.05 and
                                                  out['flip_rate_topic'] > out['flip_rate_random'] + 0.1 and
                                                  out['mean_delta_topic_patch'] > out['mean_delta_nopatch'] + 0.05)
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"INTERCHANGE on TOPIC subspace (base<-source at L{PATCH_L}, pos {QP}); Δ = source-topic minus base-topic distinctive-logit gain:", flush=True)
    ...
    print(f"(a) topic is a causal variable via interchange: {out['pred_a_topic_causal_interchange']}", flush=True)
