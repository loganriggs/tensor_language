"""CORRECTED v2 of §957 with a NATS-based topic metric (log-prob gain on source-topic distinctive tokens, not top-10 membership which was too sparse). CAPSTONE causal-abstraction test (Geiger): can we COMPOSITIONALLY control both machines? Patch the CLASS
...
OUT = PT + 'joint_interchange_v2_results.json'
...
def main():
    ...
    for (bi, si) in pairs:
        ...
        src_topic = int(seq_topic[si]); Dsrc = dset(src_topic)
        PATCH['on'] = False; lg0 = forward_logits(idx).float()[0, QP]
        lp0 = F.log_softmax(lg0, -1)
        base_src_mass = float(lp0[Dsrc].mean()) if Dsrc is not None else 0.0
        for c, U in conds.items():
            ...
            # NATS-based topic metric: change in mean log-prob on SOURCE-topic distinctive tokens (§894-style)
            topic_gain = (float(lp[Dsrc].mean()) - base_src_mass) if Dsrc is not None else 0.0
            agg[c]['class_to_source'] += int(pc == src_cls_class)
            agg[c]['topic_gain'] += topic_gain
            agg[c]['n'] += 1
    hh.remove()
    ...
    out['pred_a_compositional'] = bool(j['pred_class_matches_source'] > r['pred_class_matches_source'] + 0.05 and
                                       j['topic_gain_nats_source'] > r['topic_gain_nats_source'] + 0.05 and
                                       co['topic_gain_nats_source'] > cl_['topic_gain_nats_source'])
