"""GREEDY SELECTION, ROUND 2 -- start from the round-1 winner (5 attention
means adopted, +2.779) and keep going: (i) try MEAN stand-ins for the six
middle MLPs (currently CP truncations) -- if the scaffolding phenomenon
extends to MLPs the middle description collapses to six vectors; (ii)
re-try the remaining attention dicts as means in the new context; (iii)
re-try a8 as a mean after all other swaps. Two full passes or until no
change.
REGISTERED PREDICTIONS:
  (a) >=1 middle MLP adopts the mean;
  (b) final <= +2.70;
  (c) the description census is reported (what kind each slot uses)."""
...
OUT=PT+'standin_select2_results.json'
...
def main():
    ...
    # mlp mean stand-ins: capture means under the current front
    best=[n for n in order if n!='a8']
    for nm in ('a2','a5','a9','a14','a16'):
        best=[(nm+'m' if n==nm else n) for n in best]
    Ym={}
    hs=install(best)
    caps={li:torch.zeros(D,device=DEV) for li in MIDL}; cnt=0
    for li in MIDL:
        def mk(li=li):
            def h(mod,i_,o_):
                caps[li]+=o_.detach().float().reshape(-1,D).sum(0)
            return h
        hs.append(m.transformer.h[li].mlp.register_forward_hook(mk()))
    for i in range(CA,CB,4):
        bb=FW[i:i+4,:257].to(DEV)
    ...
    for h in hs: h.remove()
    for li in MIDL:
        S[f'c{li}m']=('attnm',li,caps[li]/cnt)  # reuse mean kind? no: mlp
    # mlp mean hook kind: reuse 'linear' with W=0
    for li in MIDL:
        mu=caps[li]/cnt
        S[f'c{li}m']=('linear',li,torch.zeros(D,D,device=DEV),mu)
    base=evalC([],'oracle')
    cost=evalC(best,'oracle')-base
    print(f'start (round-1 winner) {cost:+.4f}',flush=True)
    swaps=[]
    for rnd in range(2):
        changed=False
        for li in MIDL:
            nm=f'c{li}'
            if nm not in best: continue
            cand=[(nm+'m' if n==nm else n) for n in best]
            c=evalC(cand,'oracle')-base
            tag='ADOPT' if c<cost-0.005 else 'keep'
            print(f'r{rnd} {nm}->mean {c:+.4f} ({tag})',flush=True)
            if c<cost-0.005:
                best=cand; cost=c; swaps.append(nm); changed=True
        for li in range(2,18):
            nm=f'a{li}'
            if nm not in best: continue
            cand=[(nm+'m' if n==nm else n) for n in best]
            ...
            if c<cost-0.005:
                best=cand; cost=c; swaps.append(nm); changed=True
        if 'a8m' not in best and 'a8' not in best:
            cand=[]
            for n in best:
                cand.append(n)
                if n=='c8' or n=='c8m': cand.insert(-1,'a8m')
            c=evalC(cand,'oracle')-base
            tag='ADOPT' if c<cost-0.005 else 'keep'
            print(f'r{rnd} a8->mean(rescue) {c:+.4f} ({tag})',flush=True)
            if c<cost-0.005:
                best=cand; cost=c; swaps.append('a8'); changed=True
        if not changed: break
    dep=evalC(best,'probe')-base
    census={n:S[n][0] for n in best}
    nmid=sum(1 for nm in swaps if nm.startswith('c'))
    pa=nmid>=1; pb=cost<=2.70
    out={'base':round(base,4),'final':round(cost,4),
         'final_deploy':round(dep,4),'swaps':swaps,'census':census,
         'pred_a':bool(pa),'pred_b':bool(pb)}
    print(f'final {cost:+.4f} | deploy {dep:+.4f} | new swaps {swaps}')
    print(f"(a) >=1 middle MLP mean: {'HELD' if pa else 'FAILED'}")
    print(f"(b) final <= +2.70: {'HELD' if pb else 'FAILED'}")
