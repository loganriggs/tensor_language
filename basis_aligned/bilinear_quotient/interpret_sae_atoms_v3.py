"""INTERPRET SAE ATOMS v3 (fix 752 broken max-lift with KL). fix 751's confound). Monosemanticity via
...
OUT = PT + 'interpret_sae_atoms_v3_results.json'
...
def lift(code_col, toks, base, topn=TOPN):
    # KL divergence of the top-activation token distribution from base (robust
    # to rare-token singletons, dominated by the atom's DOMINANT tokens).
    idx=np.argsort(-code_col)[:topn]
    idx=idx[code_col[idx]>0]
    if len(idx)<10: return 0.0
    c=Counter(toks[idx].tolist()); tot=len(idx)
    kl=0.0
    for t,cnt in c.items():
        pt=cnt/tot; pb=base.get(t,1e-9)
        kl += pt*np.log(pt/max(pb,1e-9))
    return float(kl)
...
def main():
    ...
    pa = best_sae > 1.3*svd_lift and res[str(PS[-1])]['lift_low_usage'] > res[str(PS[-1])]['lift_high_usage']
    null_ok = rand_lift < 0.5*svd_lift
    print(f'(a) SAE KL > 1.3x SVD & low-usage>high-usage: {pa}; NULL random low: {null_ok}', flush=True)
