# mid_pair_synergy: DEPTH OR FUNCTION? — the arm that separates §1706's two explanations
#
# §1706 established that two-site MLP pairs at opposite ends of the stack have OPPOSITE synergy
# signs: mlp0+mlp1 is +41.8% super-additive, mlp16+mlp17 is -20.1% sub-additive, at matched band
# size and with the early excess interval excluding zero. It also flagged the confound plainly:
# in bilin18, depth and function are the same contrast. The front MLPs are tabular and
# token-driven, the late MLPs are nearly linear readouts (§1668: quadratic remainder 31.3% front
# against 16.4% late), so "early vs late" cannot be told apart from "tabular vs readout".
#
# A MIDDLE pair separates them, because the two explanations disagree about where it lands:
#
#   DEPTH says the excess is set by how much model lies downstream to amplify the error. mlp8 and
#     mlp9 have nine blocks below them, between the early pair's sixteen and the late pair's one,
#     so the excess should fall BETWEEN +41.8% and -20.1%.
#   FUNCTION says the excess is set by what the modules compute. The middle MLPs are the LEAST
#     tabular band in the model (§1666: joint table ceiling 21.73% against the front's 76.45%) and
#     the MOST bilinear (§1668: 37.7% quadratic remainder against 31.3% and 16.4%), so on a
#     functional reading they are the extreme case, not the intermediate one, and there is no
#     reason for the excess to land between the other two.
#
# mlp8 and mlp9 are chosen as the middle of the middle band, away from both boundaries.
#
# Two compiles: the no-exemption control and the pair, plus the two singles, so four arms. The
# singles cannot be reused from an earlier section the way §1706 reused §1704's -- no run has
# exempted mlp8 or mlp9 individually -- so they are measured here.
#
# Registered predictions, pred_a and pred_b both TWO-SIDED per LESSONS 31:
#   pred_a THE MIDDLE PAIR IS INTERMEDIATE, as depth predicts: its excess fraction lies strictly
#          between the late pair's -20.1% and the early pair's +41.8%. Landing outside that
#          interval on either side favours the functional reading.
#   pred_b THE SIGN IS RESOLVED: the 95% interval on (pair gain - singles sum) excludes zero.
#          §1705's ordering claim failed exactly this test and §1706's passed it.
#   pred_c CONTROLS: the no-exemption arm reproduces §1696's 55.04% within 0.5 points, the
#          baseline CE reproduces 3.29205 (§1695), and all three gains are positive.
#   pred_d NON-VACUITY: the two singles differ from each other by at least 0.05 points, so the
#          pair is not simply one site carrying everything. If they are indistinguishable the
#          excess is still readable but the "two interacting sites" framing is not.
...
OUT = PT + 'mid_pair_synergy_results.json'
...
S1696_BOTH = 0.5504
S1698_RANK8 = 0.5475
BANDS = {'mlp8': ('mlp', range(8, 9)), 'mlp9': ('mlp', range(9, 10)),
         'mlp8_mlp9_pair': ('mlp', range(8, 10))}
S1705_LATE_EXCESS_FRAC = -0.201   # late pair
S1706_EARLY_EXCESS_FRAC = 0.418   # early pair
...
def main():
    ...
    print(f'MID PAIR SYNERGY | mlp8, mlp9 and the pair, RECOMPILED | '
    ...
    pair = gains['mlp8_mlp9_pair']
    singles = gains['mlp8'] + gains['mlp9']
    excess = pair - singles
    excess_frac = excess / pair if abs(pair) > 1e-9 else float('nan')
    lo, hi = ci_g['mlp8_mlp9_pair']
    ex_lo, ex_hi = lo - singles, hi - singles

    pa = S1705_LATE_EXCESS_FRAC < excess_frac < S1706_EARLY_EXCESS_FRAC
    pb = (ex_lo > 0) or (ex_hi < 0)
    pc = (abs(base - S1696_BOTH) <= 0.005 and abs(cl - S1683_CE_LIVE) <= 1e-3
          and all(v > 0 for v in gains.values()))
    pd = abs(gains['mlp8'] - gains['mlp9']) >= 0.0005
    ...
    print(f'  singles: mlp8 {gains["mlp8"]:+.2%}  mlp9 {gains["mlp9"]:+.2%}  sum {singles:+.2%}',
          flush=True)
    print(f'  excess {excess:+.2%} = {excess_frac:+.1%} of the pair  95% CI '
          f'[{ex_lo:+.2%}, {ex_hi:+.2%}]', flush=True)
    print(f'  late {S1705_LATE_EXCESS_FRAC:+.1%} < middle {excess_frac:+.1%} < early '
          f'{S1706_EARLY_EXCESS_FRAC:+.1%} -> INTERMEDIATE, as depth predicts {pa}', flush=True)
    print(f'  excess CI excludes zero -> sign resolved {pb}', flush=True)
    print(f'  singles distinguishable -> two interacting sites {pd}', flush=True)
    ...
    res = {'config': {'sites': ALL18, 'ridge': RIDGE,
                      ...
                      'design': 'MIDDLE two-site pair (mlp8, mlp9) at band size matched to the early and '
                                'late pairs; singles measured here since no prior run exempted them',
                      'discriminates': 'DEPTH predicts an excess between the late -20.1% and early +41.8%; '
                                       'FUNCTION does not, since the middle MLPs are the LEAST tabular '
                                       '(S1666: 21.73% joint table ceiling vs the front 76.45%) and MOST '
                                       'bilinear (S1668: 37.7% quadratic remainder) band -- the extreme '
                                       'case on a functional reading, not the intermediate one',
           ...
           'predictions': {'pred_a_intermediate_as_depth_predicts': bool(pa),
                           'pred_b_sign_resolved': bool(pb),
