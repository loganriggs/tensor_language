# comparative_refine_gate: THE LOOSE END (§1368 pred_c missed by 0.0006 — the four
# refine heads run ungated). Gate them by the comparative query window and re-score.
# Arms: kit (anchor, refine ungated) | kit_rg (refine heads live only in the qry window;
# 8.1 stays live everywhere — its fetch precedes the window).
#
# Registered predictions:
#   pred_a target recovery within 0.02 of the anchor (one-sided down).
#   pred_b the elsewhere increment over route falls under the 0.05 bar (closes §1368).
#   pred_c no other family regresses by more than 0.02 (question/closer diagnostics).
...
OUT = PT + 'comparative_refine_gate_results.json'
NMEAN = 24; NR = 1920
...
SPEC = {'question': {(10, 5)},
        ...
        'closer': {(13, 8)}}
CUR = {'kit': None, 'refine_gated': False}
REFINE = {(10, 5), (12, 8), (11, 7), (11, 6)}
    ...
    for L, blk in enumerate(H):
        ...
        if arm == 'full':
            ...
            for h in range(9):
                ...
                elif CUR['refine_gated'] and (L, h) in REFINE:
                    vr[:, :, h] = vmeans[L][h].to(vr.dtype)   # routed base; re-lit below
            vvr = (1 - at.lamb) * vr + at.lamb * v1.view_as(vr)
            y = torch.einsum('bhqk,bkhd->bqhd', pat.to(vvr.dtype), vvr)
            if arm == 'kit' and CUR['refine_gated'] and any((L, h) in (spec & REFINE) for h in range(9)):
                vv_live = (1 - at.lamb) * v + at.lamb * v1.view_as(v)
                y_live = torch.einsum('bhqk,bkhd->bqhd', pat.to(vv_live.dtype), vv_live)
                gm2 = gatemask.view(B, T, 1)
                for h in range(9):
                    if (L, h) in (spec & REFINE):
                        y[:, :, h] = torch.where(gm2, y_live[:, :, h], y[:, :, h])
...
def main():
    ...
    for arm in ('full', 'ymean', 'route'):
        ...
        print(f"{arm}: " + " ".join(f"{k} {r[k]:.3f}" for k in MASKS) + f" | else {e:.4f}",
              flush=True)
    for name, rg in (('kit_comparative', False), ('kit_comp_rg', True)):
        CUR['refine_gated'] = rg
    ...
    r_anchor = rec('kit_comparative', 'comparative')
    r_rg = rec('kit_comp_rg', 'comparative')
    e_anchor = rece('kit_comparative') - rece('route')
    e_rg = rece('kit_comp_rg') - rece('route')
    dq = abs(rec('kit_comp_rg', 'question') - rec('kit_comparative', 'question'))
    db = abs(rec('kit_comp_rg', 'closer_b') - rec('kit_comparative', 'closer_b'))
    pa = r_rg >= r_anchor - 0.02
    pb = e_rg <= 0.05
    pc = dq <= 0.02 and db <= 0.02
    out = {'ce': res,
           'comparative': {'anchor': round(r_anchor, 4), 'refine_gated': round(r_rg, 4)},
           'else_incr': {'anchor': round(e_anchor, 4), 'refine_gated': round(e_rg, 4)},
           'pred_a_target_holds': bool(pa), 'pred_b_else_under_bar': bool(pb),
           'pred_c_no_regressions': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"\ncomparative: anchor {r_anchor:.3f} -> gated {r_rg:.3f} | "
          f"else incr {e_anchor:.4f} -> {e_rg:.4f}")
    print(f"pred_a holds {pa} | pred_b under-bar {pb} | pred_c clean {pc}")
