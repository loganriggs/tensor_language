"""DAMAGE MODES v2 -- v1's modes were stable but document-confounded
(48 rows; one travel-guide doc dominated; even/odd token halves shared
documents). Fixes: 144 rows (36k tokens), per-column winsorization at
|z|<=3 (CE-delta tails inflated the null), and DOCUMENT-DISJOINT split
halves (first 72 rows vs last 72).
REGISTERED PREDICTIONS:
  (a) top-8 winsorized energy >= 1.5x the shuffle null;
  (b) >= 2 of the top-8 modes align with known classes (R^2 >= 0.20);
  (c) >= 1 top-8 mode with class R^2 <= 0.15 AND document-disjoint
      split-half loading cos >= 0.6 (a NEW label that survives the
      confound control);
  (d) per-mode top probes and sample tokens reported."""
...
OUT=PT+'damage_modes2_results.json'
CA=300; NB=36   # 36 batches x 4 rows x 256 = 36864 tokens
MHL=list(range(2,10))
...
def main():
    ...
    mu_=M.mean(0,keepdim=True)
    M=torch.clamp((M-mu_)/sd,-3,3)          # winsorized z-scores
    ...
    half=(NB*4//2)*256
    Me=M[:half]; Mo=M[half:]                # document-disjoint halves
    ...
    for k in range(8):
        ...
        if r2best>=0.20: naligned+=1
        if r2best<=0.15 and repl>=0.6: nnew+=1
    pa=top8>=1.5*top8n; pb=naligned>=2; pc=nnew>=1
    out={'top8_energy':round(top8,3),'null':round(top8n,3),
         'modes':report,'n_aligned':naligned,'n_new':nnew,
         'pred_a':bool(pa),'pred_b':bool(pb),'pred_c':bool(pc)}
    print(f"(a) structure >= 1.5x null: {'HELD' if pa else 'FAILED'}")
