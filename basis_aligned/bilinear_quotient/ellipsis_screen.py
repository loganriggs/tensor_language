# ellipsis_screen: POOL CANDIDATE at the §1302 standard. Generator flag (§1355):
# ellipsis -> a17 at +0.318, ratio 23.8. RISK REGISTERED: a17 also led capitalized,
# which resolved late-band-shared with NO owner (§1339) — pred_c is the owner-vs-band
# question, and a FALSE there means a second late-band shared capability.
# Targets: next token contains '...' or the unicode ellipsis.
#
# Registered predictions (verdict withheld if controls dirty):
#   pred_a CONCENTRATION: a17 target damage >= 0.15 nats AND >= 8x elsewhere.
#   pred_b BOTH CONTROLS CLEAN: jitter and random each <= 1.5x.
#   pred_c OWNERSHIP: a17 >= 3x every competitor (a15, a16, a13, a5).
...
OUT = PT + 'ellipsis_screen_results.json'
NMEAN = 24; NR = 1920
H = m.transformer.h
...
def main():
    ...
    print(f"ellipsis token ids: {len(oq)}", flush=True)
