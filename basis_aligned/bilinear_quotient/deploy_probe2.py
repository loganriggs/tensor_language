"""DEPLOY PROBE UPGRADE -- the oracle/deploy gap is now ~0.2 nats and the
label probe is the whole gap. The current probe is a ridge read of the
stream at attn2's input (acc 0.69). At deploy time the CURRENT token id
is also legal input, and it strongly predicts the class of the next
token. New probe: logits = stream-ridge + per-current-token class
log-prior (Laplace-smoothed counts from window A).
REGISTERED PREDICTIONS:
  (a) probe accuracy >= 0.78 (from 0.69);
  (b) deploy total (round-1 winner config) <= oracle + 0.12."""
...
OUT=PT+'deploy_probe2_results.json'
...
def main():
            ...
            if kind=='cv':
                ...
                def h(mod,i_,o_,CV=CV,LW=LW,Wp2=Wp2,first=first):
                    ...
                    if first and cur['mode']=='probe':
                        lg=x@Wp2
                        if 'PRI' in cur: lg=lg+cur['PRI'][cur['idx']
                            .reshape(-1)]
                        cur['lab']=lg.argmax(1)
                    c=cur['lab']
                    new=CV[c].clone()
    ...
    # per-current-token class prior from window A
    ids=[]
    for i in range(CA,CB,4):
        ids.append(FW[i:i+4,:256].reshape(-1))
    ids=torch.cat(ids).to(DEV)
    PRI=torch.zeros(V,10,device=DEV)
    PRI.index_put_((ids,flatA),torch.ones_like(flatA,dtype=torch.float),
                   accumulate=True)
    PRI=torch.log(PRI+0.5)-torch.log(PRI.sum(1,keepdim=True)+5.0)
    base=evalC([],'oracle')
    orc=evalC(best,'oracle')-base
    dep0=evalC(best,'probe')-base
    cur['PRI']=PRI
    dep1=evalC(best,'probe')-base
    del cur['PRI']
    # probe accuracy on window A captures (attn2 input under front)
    hs=install([n for n in best if n in
                ('a0','m0','a1v','m1')])
    Xs=[]
    def capf(mod,i_,o_): Xs.append(i_[0].detach().reshape(-1,D).float())
    hs.append(m.transformer.h[2].attn.register_forward_hook(capf))
    for i in range(CA,CB,4):
        bb=FW[i:i+4,:257].to(DEV)
        cur['idx']=bb[:,:-1].contiguous()
        cur['mode']='oracle'; cur['lab']=clsA[i-CA:i-CA+4].reshape(-1)
    ...
    X=torch.cat(Xs)
    Wp2=S['a2'][4]
    acc0=float(((X@Wp2).argmax(1)==flatA).float().mean())
    acc1=float(((X@Wp2+PRI[ids]).argmax(1)==flatA).float().mean())
    print(f'probe acc stream-only {acc0:.3f} | +token-prior {acc1:.3f}')
    print(f'oracle {orc:+.4f} | deploy old {dep0:+.4f} | '
          f'deploy new {dep1:+.4f}')
    pa=acc1>=0.78; pb=dep1<=orc+0.12
    out={'base':round(base,4),'oracle':round(orc,4),
         'deploy_old':round(dep0,4),'deploy_new':round(dep1,4),
         'acc_stream':round(acc0,3),'acc_prior':round(acc1,3),
         'pred_a':bool(pa),'pred_b':bool(pb)}
    print(f"(a) probe acc >= 0.78: {'HELD' if pa else 'FAILED'}")
    print(f"(b) deploy <= oracle+0.12: {'HELD' if pb else 'FAILED'}")
