# channel_circuit: CIRCUITS AT CHANNEL GRAIN + RED-TEAM BASELINES (S1484: single
...
#   pred_a channel extraction is monotone in k and k=8 keeps >= .30 of mlp0's
#          frag-class effect (channels fix what single axes could not).
#   pred_b RED-TEAM: weight-channel-8 keeps at least (activation-PCA-8 - .05) of the
#          frag effect — the weight structure is at least competitive; if PCA wins
#          big, the composition machinery was not needed for THIS job.
#   pred_c channel-8 removal costs >= .01 CE global while random-8 removal <= .003
#          (subspace-grain causality is real and specific).
...
OUT = PT + 'channel_circuit_results.json'
NR = 960
...
def main():
    ...
    ARMS = [('mean', 'extract', torch.zeros(1, HD, device=DEV)),
            ('ex_w1', 'extract', W1), ('ex_w8', 'extract', W8),
            ('ex_w32', 'extract', W32), ('ex_pca8', 'extract', P8),
    ...
    keeps = {nm: {k: round(keep(nm, k), 4) for k in c}
             for nm in ('ex_w1', 'ex_w8', 'ex_w32', 'ex_pca8')}
    rises = {nm: {k: round(res[nm][k] - c[k], 4) for k in c}
             for nm in ('rm_w8', 'rm_pca8', 'rm_rand8')}
    mono = keeps['ex_w1']['frag'] <= keeps['ex_w8']['frag'] <= keeps['ex_w32']['frag']
    pa = mono and keeps['ex_w8']['frag'] >= 0.30
    pb = keeps['ex_w8']['frag'] >= keeps['ex_pca8']['frag'] - 0.05
    pc = rises['rm_w8']['global'] >= 0.01 and rises['rm_rand8']['global'] <= 0.003
    out = {'res': res, 'effect': {k: round(v, 4) for k, v in eff.items()},
           'keeps': keeps, 'rises': rises,
           'pred_a_channel8_30_monotone': bool(pa),
           'pred_b_weight_competitive': bool(pb),
           'pred_c_subspace_causal_specific': bool(pc),
