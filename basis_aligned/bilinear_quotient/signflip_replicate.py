"""Replication of the section-193 sign flip: swiglu18's k=4 interchange excess
was NEGATIVE (-0.070, compensation regime). Replicate on disjoint eval rows
(384-448). REGISTERED: (a) the sign holds (excess < 0); (b) magnitude >= 40% of
-0.070; (c) bilin12's k=4 positive excess (+0.055) also replicates in sign.

Prior context -- coupling-constant stability check for section 192's normalized comparison
...
OUT=('/workspace/tensor_language/basis_aligned/bilinear_quotient/'
     'signflip_replicate_results.json')
...
def main():
    ...
    cs={}
    specs=[('swiglu18','swiglu18',16,17),('bilin12','bilin12',10,11)]
    ...
    esw=cs['swiglu18'][4]['excess']; e12=cs['bilin12'][4]['excess']
    pa=esw<0
    pb=esw<=0.4*(-0.070)
    pc=e12>0
    out['pred_a']=bool(pa); out['pred_b']=bool(pb); out['pred_c']=bool(pc)
    print(f"\n(a) swiglu18 k=4 sign holds (<0): {'HELD' if pa else 'FAILED'} ({esw:+.4f})")
    print(f"(b) magnitude >=40%: {'HELD' if pb else 'FAILED'}")
    print(f"(c) bilin12 k=4 positive replicates: {'HELD' if pc else 'FAILED'} ({e12:+.4f})")
