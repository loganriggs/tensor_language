"""POWER RECHECK 2 -- completes the audit under the N-stable
power definition of writeup 500. The first pass (499: are the census's mechanism negatives
...
bar at the 1.3 floor) get the same treatment. If widening flips
those, it is not measuring anything.
The first pass tested the four candidates picked out by the
max-of-N separation statistic; that statistic was retracted (500)
and the N-stable one -- minimum detectable enrichment above 1.35
-- names five, of which three were already tested and all three
regressed toward 1.0. The two never tested are r.6.0.0 m17 (signal
1.843 against a 1.977 bar, the largest unresolved hint in the
census) and r.5.3.2 a15 (1.18 against 1.804). Same design, same
control logic, two fresh decisive negatives as the control.
REGISTERED PREDICTIONS:
  ...
  (b) THE WIDENING WORKS: both bars fall to 1.35 or below at 20
      draws, i.e. the extra draws shrink the null's spread enough
      that the test can see a 35% enrichment;
...
OUT=PT+'power_recheck2_results.json'
HINT=[('r.6.0.0','m17'),('r.5.3.2','a15')]
CTRL=[('r.12.0.2','a5'),('r.11.3.0','a3')]
NS=20
...
def main():
    ...
    pb=bool(H) and all(v['min_detectable']<=1.35 for v in H)
    ...
    for nm,v in (('a','a hinted negative flips at 20 seeds'),
                 ('b','both bars fall to <=1.35x at 20 draws'),
