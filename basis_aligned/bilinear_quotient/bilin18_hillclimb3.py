"""HILLCLIMB round 3: the computation-class lever. A factored bilinear stand-in
...
OUT=('/workspace/tensor_language/basis_aligned/bilinear_quotient/'
     'bilin18_hillclimb3_results.json')

def fit_bilinear(X,Y,h,steps=400,lr=3e-3):
    ...
    for i in range(384,448,4):
        ...
        for lj in range(18):
            ...
            if lj==li and mp is not None:
                ...
                mo=(((xi@mp['L'].T)*(xi@mp['R'].T))@mp['Dn'].T+mp['by'])
...
def main():
    ...
    base=ce_with(0,None)
    out={}
    for li,h,bar in ((1,64,0.60),(16,16,0.059)):
    ...
    pa=out['L1']['cost']<=0.60
    pb=out['L16']['cost']<=0.059
    pc=all(v['mse0']/max(v['msef'],1e-6)>=3 for v in out.values())
    out['pred_a']=bool(pa); out['pred_b']=bool(pb); out['pred_c']=bool(pc)
    print(f"\n(a) L1 class lever works (<=0.60): {'HELD' if pa else 'FAILED'}")
    print(f"(b) L16 width-16 <= linear r8: {'HELD' if pb else 'FAILED'}")
