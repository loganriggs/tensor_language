"""LINEAR-ARM RANK REDUCTION -- the bits-metric examples doc identifies
the D x D linear arms (attention dictionaries' contextual classes, and
m1's full linear map) as the fitted-bits hogs of the census: the circuit
content (constants, spans) is cheap; the escape-hatch linear maps are
~1.3M fitted numbers each. If they rank-reduce cheaply the honest
description shrinks by an order of magnitude.
Arms: truncate ALL dictionary linear maps + m1's map to rank r via SVD,
r in (128, 32, 8), evaluated in the round-2 best config (+2.749).
REGISTERED PREDICTIONS:
  (a) rank-128 within +0.02 of full (+2.749);
  (b) rank-32 within +0.05;
  (c) rank-8 cost reported (expected to break: the arms carry real
...
OUT=PT+'linarm_rank_results.json'
...
def main():
    ...
    def trunc(W,r):
        ...
        return (U[:,:r]*Sg[:r])@Vh[:r]
    import copy
    FULL={k:v for k,v in S.items()}
    base=evalC([],'oracle')
    full=evalC(best,'oracle')-base
    print(f'full-rank best {full:+.4f}',flush=True)
    out={'base':round(base,4),'full':round(full,4),'ranks':{}}
    for r in (128,32,8):
        for nm in list(S.keys()):
            v=FULL[nm]
            if v[0]=='attnd':
                _,li,CV,LW,Wp2=v
                S[nm]=('attnd',li,CV,{k:trunc(W,r) for k,W in LW.items()},
                       Wp2)
            elif v[0]=='linear' and nm=='m1':
                _,li,W,b=v
                S[nm]=('linear',li,trunc(W,r),b)
        c=evalC(best,'oracle')-base
        out['ranks'][r]=round(c,4)
        print(f'rank {r:4d}: {c:+.4f}',flush=True)
    for nm in list(S.keys()): S[nm]=FULL[nm]
    pa=out['ranks'][128]<=full+0.02
    pb=out['ranks'][32]<=full+0.05
    out['pred_a']=bool(pa); out['pred_b']=bool(pb)
    print(f"(a) rank-128 within +0.02: {'HELD' if pa else 'FAILED'}")
    print(f"(b) rank-32 within +0.05: {'HELD' if pb else 'FAILED'}")
    print(f"(c) rank-8 cost {out['ranks'][8]:+.4f} (informational)")
