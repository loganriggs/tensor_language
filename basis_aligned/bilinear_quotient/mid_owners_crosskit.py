# mid_owners_crosskit: IS THE MID OWNER-BAND CAPABILITY-GENERAL? (§1386-88 found
# mlp4-7 = the middle's owners INSIDE THE COMPARATIVE KIT, with keep-4 BEATING all-live.)
# This run repeats the three-arm comparison (all-mid-live / mid-refit-mean / keep-{4,5,6,7})
# for the QUESTION and CLOSER kits, each scored on its own family.
#
# Registered predictions (one-sided, §1367 bar-lesson):
#   pred_a keep-4 within 0.03 BELOW all-live on QUESTION targets.
#   pred_b keep-4 within 0.03 BELOW all-live on CLOSER-bracket targets.
#   pred_c selectivity: keep-4's elsewhere within 0.05 of all-live's, both kits.
...
OUT = PT + 'mid_owners_crosskit_results.json'
NFITT = 960
V = 50257
NMEAN = 24; NR = 1920
...
def main():
    ...
    for arm in ('full', 'ymean', 'route'):
        ...
        print(f"{arm}: " + " ".join(f"{k} {r[k]:.3f}" for k in MASKS) + f" | else {e:.4f}",
              flush=True)
    MLPMODE['cur'] = None
    KITS3 = ('comparative', 'question', 'closer')
    for kitname in KITS3:
        for mode, tag in ((None, 'live'), ('mid_mean', 'mean'), ('mid_live_set', 'keep4')):
            MLPMODE['cur'] = mode
            if mode == 'mid_live_set':
                MLPTAB['live_set'] = {4, 5, 6, 7}
            r, e = ce_run('kit', kitname)
            res[f'{kitname}_{tag}'] = {**{k: round(v, 4) for k, v in r.items()},
                                       'else': round(e, 4)}
            print(f"{kitname}_{tag}: " + " ".join(f"{k} {r[k]:.3f}" for k in MASKS)
    ...
    fam_of = {'comparative': 'comparative', 'question': 'question', 'closer': 'closer_b'}
    table = {}
    for kitname in KITS3:
        fam = fam_of[kitname]
        table[kitname] = {tag: round(rec(f'{kitname}_{tag}', fam), 4)
                          for tag in ('live', 'mean', 'keep4')}
        print(f"{kitname}: live {table[kitname]['live']:.3f} | mean "
              f"{table[kitname]['mean']:.3f} | keep4 {table[kitname]['keep4']:.3f}",
              flush=True)
    pa = table['question']['keep4'] >= table['question']['live'] - 0.03
    pb = table['closer']['keep4'] >= table['closer']['live'] - 0.03
    pc = all(abs(rece(f'{k}_keep4') - rece(f'{k}_live')) <= 0.05
             for k in ('question', 'closer'))
    out = {'ce': res, 'recovery_table': table,
           'pred_a_question_holds': bool(pa), 'pred_b_closer_holds': bool(pb),
           'pred_c_selective': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"pred_a question {pa} | pred_b closer {pb} | pred_c selective {pc}")
