# question_punct_control: THE DIRECT PARALLEL TO §1641 — is §1597's certified
# `question` cell distinctive among PUNCTUATION, or is it the weakest of its subtype
# the way the certified pronoun class turned out to be?
#
# §1641 re-ran the certified pronoun cell against four other personal pronouns in the
# discriminating rank-2 TOP-4 regime and found the certified class WEAKEST of five --
# margin -17, with ` I` nearly 4x more below-null. That resolved pending item 2.
#
# §1637 already hinted the same question for §1597's cell: `question` (^\?$| \?$) is
# SENTENCE-TERMINAL PUNCTUATION, and against the single punctuation control `period`
# its margin at mlp11 rank-2 TOP-4 was 2/60, not the 13/60 §1633 reported against
# function words. One control is one control -- §1641 needed four to be convincing,
# and this runs the same test properly.
#
# Five punctuation classes at question's own cell (mlp11, rank-2 TOP-4 -- the regime
# §1640 established as discriminating), 20 independent bases x 3 disjoint 160-row
# chunks = 60 trials each. Same rows and seeds as every section in this arc.
#
# THIS CAN CREATE A THIRD PENDING ITEM AND I AM NOT PREJUDGING IT. If `question` is
# the weakest or middling among punctuation, §1597's distinctiveness at this cell is
# in the same position §1612's pronoun reading was, and that goes to Logan as a new
# drafted-not-applied item. If `question` stands clear, §1597 gains support it has not
# previously had, because no same-type control has ever been run for it.
#
# NOTE ON SCOPE: this tests the SEPARATION statistic only. §1597's share (.718,
# replicated .7179), its writer graph and its 20:1 head-grain are different
# measurements and are untouched either way.
#
# Registered predictions:
#   pred_a `question` IS NOT DISTINCTIVE AMONG PUNCTUATION: its separation exceeds the
#          BEST other punctuation class by <= 5 of 60, mirroring §1641's pronoun result.
#   pred_b THE DIRECTION IS SHARED: at least 3 of the 4 other punctuation classes have
#          the SAME gap sign as `question`.
#   pred_c THE REGIME DISCRIMINATES, so this is a real test: at least one of the five
#          classes has separation <= 45/60.
...
OUT = PT + 'question_punct_control_results.json'
NR = 960
...
CELLS = {
    'question':  {'site': 11, 'rank': 2, 'top': 4, 'certified': True, 'published': 0.718,
                  'pat': r'^\?$| \?$'},
    'period':    {'site': 11, 'rank': 2, 'top': 4, 'certified': False, 'pat': r'^\.$|^ \.$'},
    'comma':     {'site': 11, 'rank': 2, 'top': 4, 'certified': False, 'pat': r'^,$|^ ,$'},
    'exclaim':   {'site': 11, 'rank': 2, 'top': 4, 'certified': False, 'pat': r'^!$|^ !$'},
    'semicolon': {'site': 11, 'rank': 2, 'top': 4, 'certified': False, 'pat': r'^;$|^ ;$'},
}
...
def main():
    ...
    csign = cert['mean_gap'] >= 0
    n_same = sum(1 for v in fresh.values() if (v['mean_gap'] >= 0) == csign)
    all_sep = [cert['separation']] + [v['separation'] for v in fresh.values()]
    n_unsat = sum(1 for x in all_sep if x <= 45)
    pa = cert['separation'] - best_fresh <= 5
    pb = n_same >= 3
    pc = n_unsat >= 1

    print(f"\n  SEPARATION at mlp17 rank-2 TOP-4 (DISCRIMINATING regime), exact-subtype PUNCTUATION controls, {N_RANDOM*CHUNKS} trials each:",
          flush=True)
    ...
    print(f"  question {cert['separation']}/{NT}  vs  best other punctuation "
          f"{best_fresh}/{NT}  -> MARGIN {cert['separation']-best_fresh}", flush=True)
    print(f"  other punctuation sharing question's gap sign "
          f"({'positive' if csign else 'negative'}): {n_same}/4 | classes at <= 45/60 "
          f"(regime check): {n_unsat}/5", flush=True)
    ...
    out = {'config': {'cells': {k: {kk: vv for kk, vv in v.items() if kk != 'pat'}
           ...
           'cells': out_cells,
           'predictions': {'pred_a_question_margin_le_5': bool(pa),
                           'pred_b_3of4_share_gap_sign': bool(pb),
                           'pred_c_regime_is_unsaturated': bool(pc)},
           'n_same_sign': n_same, 'n_unsaturated': n_unsat,
