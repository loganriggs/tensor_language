# writer_floor_absmass: TEST THE PUBLISHED SHARE STATISTICS AS THEY WERE ACTUALLY
# COMPUTED.  S1611 found that the earlier floor harness ranked components by
# POSITIVE-ONLY signed contribution, while S1597 (slice_writers.py:216) and S1598
# (slice_writers_p.py:205) both rank by ABSOLUTE attribution mass:
#
#     delta = {c: (coef[c] * (cmu[c] - mu[c])).abs().sum()}
#     share = sum(delta[c] for c in top) / sum(delta.values())
#
# So S1610's cross-reference to .718 was withdrawn and BOTH published figures are
# currently UNTESTED against a matched-rank null.  This tests them as written, at
# both cells, in one run.
#
#   question @ mlp11, rank-2 |lambda|, TOP=4   -- S1597 reported share .718
#   pronouns @ mlp17, rank-8 |lambda|, TOP=6   -- S1598 reported share .482 and
#                                                 concluded "WRITERS DIFFUSE"
#
# Each cell against a matched-RANK random orthonormal basis on IDENTICAL rows.
# Local curated_rows.pt, 3 disjoint 333-row chunks (question ~110 class positions
# per chunk, pronouns ~420).  Rows are census, NOT fresh: shares are not
# comparable ACROSS corpora to the published figures, but every registered bar is
# a within-run arm-vs-arm comparison on the same rows and the same statistic.
#
# Registered predictions:
#   pred_a question cell: |random share - lambda share| <= .10 in >= 2 of 3
#          samples, i.e. the .718-style statistic does not discriminate.
#   pred_b pronouns cell: same bar at TOP=6, i.e. the .482 "DIFFUSE" verdict is
#          equally undiscriminating.
#   pred_c membership DOES discriminate even where share does not: at the question
#          cell, >= 3 of the |lambda| absolute-mass top-4 are ABSENT from the
#          random top-4, in 3/3 samples.
...
OUT = PT + 'writer_floor_absmass_results.json'
NR = 960
...
LOCAL_ROWS, LOCAL_KEY = 'curated_rows.pt', 'rows'
...
def main():
    ...
    raw = torch.load(PT + LOCAL_ROWS, map_location='cpu')
    raw = raw[LOCAL_KEY] if isinstance(raw, dict) else raw
    allr = raw[:CHUNKS * ROWS_PER_CHUNK, :T + 1].contiguous()
    chunks = [allr[c * ROWS_PER_CHUNK:(c + 1) * ROWS_PER_CHUNK] for c in range(CHUNKS)]
    print(f'LOCAL {LOCAL_ROWS}: {CHUNKS} chunks x {ROWS_PER_CHUNK} rows (census rows, NOT fresh)', flush=True)
    ...
    out = {'config': {'cells': {k: {kk: vv for kk, vv in v.items() if kk != 'pat'}
                      ...
                      'row_source': LOCAL_ROWS, 'rows_are_fresh': False,
