"""CIRCUIT TREE v3 -- the third data tranche: 212 window-A rows PLUS
100 never-seen pile-document rows (dedup vs FW), 312 rows / 80k tokens.
Document diversity was v2's binding constraint; this targets the
100-circuit mark. Packs with contexts saved for the naming wave.
REGISTERED PREDICTIONS:
  (a) >= 90 document-disjoint-replicated leaves;
  (b) >= half taxonomy-invisible (class R^2 <= 0.15);
  (c) descendant replication rate >= 0.5;
  (d) packs written with >= 12 contexts each."""
...
OUT=PT+'circuit_tree3_results.json'
CA=300; NB=78
MHL=list(range(2,10))
T=256
...
def main():
    ...
    json.dump(packs,open(PT+'circuit_tree3_packs.json','w'),indent=1)
