# circuit_extract4: WHY DID EXTRACTION-BY-RESTORATION FAIL MID-STACK? (S1512: all
# three punctuation circuits' class recovery went NEGATIVE.) Hypothesis: the
# ensemble heads' INPUTS need exactness — extraction must include input paths.
# Arms per circuit: bg (rank-32 everywhere) / bg_ens (heads exact; S1512 ref) /
# below_exact (heads exact + ALL layers below the ensemble's lowest layer exact) /
# half_exact (heads exact + layers 0-6 exact only).
# Registered predictions:
#   pred_a below_exact turns class recovery POSITIVE for >= 2 of 3 circuits.
#   pred_b below_exact class recovery >= .30 for >= 1 circuit.
#   pred_c half_exact recovers >= half of below_exact's class recovery for >= 2 of
#          3 (the needed input fidelity is mostly in layers 0-6).
...
OUT = PT + 'circuit_extract4_results.json'
NR = 960
...
def main():
    ...
    for cname, spec in CIRCUITS.items():
        ...
        min_L = min(L for (L, hh) in spec['heads'])
        res[cname] = {}
        for nm, kw in (('clean', {}), ('bg', {'bg': True}),
                       ('bg_ens', {'bg': True, 'ens_exact': True}),
                       ('below_exact', {'bg': True, 'ens_exact': True,
                                        'exact_layers': frozenset(range(min_L))}),
                       ('half_exact', {'bg': True, 'ens_exact': True,
                                       'exact_layers': frozenset(range(7))})):
    ...
    for cname in CIRCUITS:
        c = res[cname]['clean']; b = res[cname]['bg']
        recs[cname] = {}
        for arm in ('bg_ens', 'below_exact', 'half_exact'):
            e = res[cname][arm]
            recs[cname][arm] = {k: round((b[k] - e[k]) / max(b[k] - c[k], 1e-6), 4)
                                for k in c}
        print(cname, recs[cname], flush=True)
    n_pos = sum(1 for cn in recs if recs[cn]['below_exact']['cls'] > 0)
    pa = n_pos >= 2
    pb = any(recs[cn]['below_exact']['cls'] >= 0.30 for cn in recs)
    n_half = sum(1 for cn in recs
                 if recs[cn]['half_exact']['cls']
                 >= 0.5 * max(recs[cn]['below_exact']['cls'], 1e-6))
    pc = n_half >= 2
    out = {'res': res, 'recovery': recs,
           'pred_a_below_positive_2of3': bool(pa),
           'pred_b_below_30_1of3': bool(pb), 'pred_c_half_suffices_2of3': bool(pc),
