# attn_lowrank_sweep: THE WINNING CLASS SWEPT ACROSS THE REMAINING 11 LAYERS
...
#   pred_a median r32 fid across the 11 layers >= .85.
#   pred_b EVERY layer beats its distance-kernel fid (S1447 refs).
#   pred_c attn12 (worst kernel, .206) reaches >= .70.
...
OUT = PT + 'attn_lowrank_sweep_results.json'
NR = 960
...
def main():
    ...
    pa = med32 >= 0.85
    pb = all(fids[f'attn{L}_r32'] > KREF[L] for L in LAYERS)
    pc = fids['attn12_r32'] >= 0.70
    out = {'ce': res, 'fid_opt': fids, 'median_r32': round(med32, 4),
           'kernel_refs': KREF,
           'mbits_per_layer': {'r32': 23.6, 'full_qk': 85.0, 'kernel': 0.037},
           'pred_a_med32_85': bool(pa), 'pred_b_all_beat_kernel': bool(pb),
           'pred_c_a12_70': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"med32 {med32:.4f}")
