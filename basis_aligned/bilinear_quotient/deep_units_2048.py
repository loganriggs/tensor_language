# deep_units_2048: K=2048 COMPLETION at the remaining deep MLPs (S1537: log-uniform
# tail). K = 2048 only, mlps 4, 5, 6, 7, 8, 9, 11, 15, 16, 17.
# Original: THE UNIT-TRUNCATION CURVE (S1533: top-64 units recover .30-.42
...
#   pred_a K=2048 adds >= .08 over K=1024 at every module.
#   pred_b median K=2048 >= .72.
#   pred_c the log-linear law holds: each module's 1024->2048 gain >= .6x its
#          256->1024 gain.
...
OUT = PT + 'deep_units_2048_results.json'
NR = 960; NTR = 480; STEPS = 300; RK = 64
H = m.transformer.h
TARGETS = [4, 5, 6, 7, 8, 9, 11, 15, 16, 17]
LINALL_REF = {}
...
def main():
    ...
    K1024_REF = {4: .7032, 5: .6577, 6: .7098, 7: .6119, 8: .6446, 9: .6474,
                 11: .6072, 15: .5147, 16: .8497, 17: .8876}
    K256_REF = {4: .4297, 5: .3394, 6: .525, 7: .4058, 8: .4793, 9: .4892,
                11: .4496, 15: .321, 16: .78, 17: .7943}
    import statistics
    pa = all(RECS[L][2048] >= K1024_REF[L] + 0.08 for L in TARGETS)
    pb = statistics.median([RECS[L][2048] for L in TARGETS]) >= 0.72
    pc = all((RECS[L][2048] - K1024_REF[L])
             >= 0.6 * max(K1024_REF[L] - K256_REF[L], 1e-6) for L in TARGETS)
    out = {'res': res, 'recs': {str(L): RECS[L] for L in TARGETS},
           'pred_a_adds_08': bool(pa), 'pred_b_med_72': bool(pb),
           'pred_c_loglinear': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
