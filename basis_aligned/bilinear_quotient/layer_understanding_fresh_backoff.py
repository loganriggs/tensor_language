"""[FRESH per-component, CLASS-BACKOFF for rare tokens — removes the §902 coverage confound so absolute fresh values are meaningful] token means backing off to CLASS mean for tokens with <MINCOUNT train occurrences (exactly the user's "token means AND known clusters"). [certification of §893] HOW MUCH OF EACH LAYER DO WE UNDERSTAND, out-of-sample? §893
...
OUT = PT + 'layer_understanding_fresh_backoff_results.json'
NEVAL = 200; SEQ = 256
...
def main():
    ...
    # class labels per unique token for backoff
    import tiktoken; enc = tiktoken.get_encoding('gpt2'); dd = lambda i: enc.decode([int(i)])
    ...
    def clsf(sx):
        ...
        if low in CONJ: return 2
        if low in PRON: return 3
        return 6 if t[0].isupper() else 7
    ucls = np.array([clsf(dd(int(t))) for t in uniq])
    MINCOUNT = 12
    tables = {}
    for t in tags:
        tab = sums[t]/cnts[t].clamp_min(1).unsqueeze(1)
        # class-mean table (8 classes) from train sums
        cmean = torch.zeros(8, D, device=DEV); ccnt = torch.zeros(8, device=DEV)
        uc = torch.tensor(ucls, device=DEV); cmean.index_add_(0, uc, sums[t]); ccnt.index_add_(0, uc, cnts[t])
        cmean = cmean/ccnt.clamp_min(1).unsqueeze(1)
        rare = torch.tensor(cnts[t].cpu().numpy() < MINCOUNT, device=DEV)
        tab[rare] = cmean[torch.tensor(ucls, device=DEV)][rare]
        tables[t] = tab
