"""REPRODUCIBILITY of the §939 headline benchmark (0.42) on a FRESH data draw (NEVAL 240 vs 200) and DIFFERENT random seeds (split/kmeans/shuffle 0->7). Registered prediction: token-linear-map + topic + prev held-out understanding reproduces ~0.42 (+-0.03); shuffled-map null negative -> the tracked metric is seed/draw-stable. ORIGINAL: BENCHMARK REFRESH: does a SMOOTH per-token LINEAR MAP (from the token embedding) beat the discrete token TABLE
...
OUT = PT + 'whole_model_understanding_repro_results.json'
NEVAL = 240; SEQ = 256; CONTENT_L = 15; K = 12; RTOK = 64; RPOS = 32; RIDGE_MAP = 1e3; MINCOUNT = 12
NLAYER = 18
...
def main():
    ...
    topic_i = kmeans(cn, K, seed=7).cpu().numpy().astype(np.int64)
    rng = np.random.RandomState(7); tok_sh = rng.permutation(nu)[tok_i]
