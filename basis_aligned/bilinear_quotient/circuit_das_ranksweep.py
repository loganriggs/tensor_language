...
#   pred_c  RANK SWEEP (§2060's registered open question). §2060 measured rank 1 and 4 only and found
#           8-23% of the component's member damage recovered, which said the circuits are not
#           four-dimensional but not where they DO saturate. This sweeps 1/4/16/64. Registered: rank 64
#           (5.6% of the 1152 dimensions) recovers >= 80% of the full component's held-out member damage.
#           If TRUE the circuit is contained in a subspace two orders of magnitude below the component and
#           §2060's negative becomes a quantitative answer. If FALSE the effect is genuinely spread across
#           most of the component's dimensions and no subspace story will capture it.
#
...
#
# Writes circuits/DAS_RANKSWEEP.json (read-only artifact; modifies no circuit file -- Codex works the same folder).
import json
import time
...
json.dump(rep, open('circuits/DAS_RANKSWEEP.json', 'w'), indent=1)
...
print(f'\nwrote circuits/DAS_RANKSWEEP.json ({time.time()-t0:.0f}s)')
