# ship_all36: THE TOTAL GLASS NUMBER (S1497: 26 modules = 3.902 at 2.4x-solo
# marginal cost for weak planks. This run replaces EVERYTHING: all 18 attention
# layers + all 18 MLPs — mlps 4..13 added as stream-linalls fit on ship-context
# captures like 14-17). Whatever the number is, it is the spec's ultimate composite
# and the honest frontier of the whole program.
...
#   pred_a the 36-module ship <= 5.5 CE on skip=7000.
#   pred_b total delta <= 2x the sum of all per-module in-ship marginals measured so
#          far plus solo stakes of 4-13 (compounding stays gentle).
#   pred_c fresh-row band <= .15.
...
OUT = PT + 'ship_all36_results.json'
NR = 960
...
def main():
    ...
    ALLM = frozenset(range(18)) - frozenset({3}) | frozenset({3})
    ALLM = frozenset(range(18))
    res = {'clean': round(ce2(frozenset(), frozenset()), 4)}
    res['ship26'] = round(ce2(ALLL, frozenset({0, 1, 2, 3, 14, 15, 16, 17})), 4)
    print(res, flush=True)
    json.dump({'partial': True, 'res': res}, open(OUT, 'w'), indent=1)
    res['ship36'] = round(ce2(ALLL, ALLM), 4)
    print(res, flush=True)
    json.dump({'partial': True, 'res': res}, open(OUT, 'w'), indent=1)
    EV2 = cl.fineweb_rows(NR, skip=2000)[:, :T + 1].contiguous()
    res['ship36_fresh2000'] = round(ce2(ALLL, ALLM, rows=EV2), 4)
    CORR['on'] = False
    print(res, flush=True)

    solo_sum = 0.593 + 0.0153 + 0.0683 + 0.0621 + 0.0736 + 0.0808 \
        + sum([0.021, 0.019, 0.018, 0.028, 0.027, 0.029, 0.024, 0.026, 0.022, 0.020])
    d36 = res['ship36'] - res['clean']
    pa = res['ship36'] <= 5.5
    pb = d36 <= 2 * solo_sum
    pc = abs(res['ship36_fresh2000'] - res['ship36']) <= 0.15
    out = {'ce': res, 'delta36': round(d36, 4),
           'solo_sum_estimate': round(solo_sum, 3),
           'compounding_vs_estimate': round(d36 / max(solo_sum, 1e-6), 2),
           'fresh_band': round(res['ship36_fresh2000'] - res['ship36'], 4),
           'pred_a_le_55': bool(pa), 'pred_b_compound_le_2x': bool(pb),
           'pred_c_fresh_le_15': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"ship36 {res['ship36']} delta {d36:.4f}")
