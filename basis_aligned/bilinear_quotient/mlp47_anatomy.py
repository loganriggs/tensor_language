# mlp47_anatomy: INSIDE THE OWNER-BAND (§1386). The middle collapsed to mlp4-7 (keep-4 =
# 0.882 > all-live 0.874). This run asks what each of the four actually is: per-module
# rank-k dose-response — swap ONE of mlp4-7 for a rank-k stand-in (k = 1, 4, 16, 64;
# out = gmean + Uk @ (ridge(x_hat) coefs), fit on kit captures with the base config
# installed) while the other three stay live and mlp8-15 stay refit-meaned.
#
# Base config everywhere: attention kit (commons+8.1+refine, gated) + front/top MLPs
# live + mlp8-15 refit means + mlp4-7 live except the swapped one.
#
# Registered predictions (comparative-target recovery; keep-4 anchor ~0.882):
#   pred_a at least 2 of the 4 admit a rank-64 stand-in within 0.03 of the anchor.
#   pred_b rank-1 recovers >= 30% of the module's own drop-cost for >= 2 modules
#          (some of the four are low-rank calibrator-like; §650 shape).
#   pred_c selectivity: every arm's elsewhere within 0.05 of the anchor's.
...
OUT = PT + 'mlp47_anatomy_results.json'
NFITT = 960
V = 50257
NMEAN = 24; NR = 1920
...
MLPTAB = {'gmean': {}, 'rmsref': {}, 'pc1': {}, 'beta': {}, 'outdims': {},
          'xmean': {}, 'Uk': {}, 'Beta': {}, 'installed': set(), 'swapL': None, 'swapk': 64}
CAPL = {'cur': None}
MIDL = tuple(range(4, 16))
OWNERS = (4, 5, 6, 7)
MEANED = tuple(range(8, 16))
KS = (1, 4, 16, 64)
    ...
    for L, blk in enumerate(H):
        ...
        if mode == 'refit_fit' and 4 <= L <= 15:
            ...
            if L in MLPTAB['installed']:
                ...
                if L == CAPL.get('statL') and CAPL.get('collect'):
                    CAPL['xin'] = mlp_in.detach().float()
                    CAPL['out'] = blk.mlp(mlp_in).detach().float()
                x = x + blk.mlp(mlp_in)
        elif mode == 'swap':
            if L == MLPTAB['swapL']:
                coefs = ((mlp_in.float() - MLPTAB['xmean'][L]) @ MLPTAB['Beta'][L])
                k = MLPTAB['swapk']
                rec_ = MLPTAB['gmean'][L] + coefs[..., :k] @ MLPTAB['Uk'][L][:, :k].T
                x = x + rec_.to(x.dtype)
            elif L in MLPTAB['installed']:
                x = x + MLPTAB['gmean'][L].to(x.dtype)
            else:
                x = x + blk.mlp(mlp_in)
        else:
...
def main():
    ...
    # ---- fit: sequential means for mlp8-15 (owners live), then rank-k stats for 4-7
    ...
    for L in OWNERS:
        ...
        Uk = evecs[:, -64:].flip(-1)          # top-64 PCs, descending
        MLPTAB['Uk'][L] = Uk
        Xc = XT - torch.outer(SX, SX) / npos
        lam = 0.01 * float(torch.diagonal(Xc).mean())
        rhs = XO @ Uk - torch.outer(SX, gmean @ Uk)
        MLPTAB['Beta'][L] = torch.linalg.solve(Xc + lam * torch.eye(D, device=DEV), rhs)
        print(f"stats mlp{L} fit (top-64 PCs + ridge)", flush=True)
    CAPL['collect'] = False
    MLPMODE['cur'] = None
    ...
    res['anchor_keep4'] = {**{k: round(v, 4) for k, v in r.items()}, 'else': round(e, 4)}
    print(f"anchor_keep4: comparative {r['comparative']:.3f} | else {e:.4f}", flush=True)
    for L in OWNERS:
        for k in KS:
            MLPMODE['cur'] = 'swap'
            MLPTAB['swapL'] = L; MLPTAB['swapk'] = k
            r, e = ce_run('kit', 'comparative')
            res[f'swap_m{L}_k{k}'] = {**{kk: round(v, 4) for kk, v in r.items()},
                                      'else': round(e, 4)}
            print(f"swap mlp{L} k={k}: comparative {r['comparative']:.3f} | else {e:.4f}",
                  flush=True)
    ...
    r_anchor = rec('anchor_keep4', 'comparative')
    drop_costs = {4: None, 5: None, 6: None, 7: None}
    swap = {L: {k: rec(f'swap_m{L}_k{k}', 'comparative') for k in KS} for L in OWNERS}
    n_r64_ok = sum(1 for L in OWNERS if swap[L][64] >= r_anchor - 0.03)
    # rank-1 vs the module's own contribution (anchor - swap at k=1... approximate the
    # module's in-config value as anchor - rank1? Use anchor - min over k as full drop proxy)
    n_r1_ok = 0
    for L in OWNERS:
        full_drop = r_anchor - min(swap[L].values())
        if full_drop > 1e-3 and (r_anchor - swap[L][1]) <= 0.7 * full_drop:
            n_r1_ok += 1
    pa = n_r64_ok >= 2
    pb = n_r1_ok >= 2
    pc = all(abs(rece(f'swap_m{L}_k{k}') - rece('anchor_keep4')) <= 0.05
             for L in OWNERS for k in KS)
    out = {'ce': res, 'anchor': round(r_anchor, 4),
           'swap_recovery': {f'mlp{L}': {str(k): round(v, 4) for k, v in swap[L].items()}
                             for L in OWNERS},
           'n_rank64_within_003': n_r64_ok, 'n_rank1_30pct': n_r1_ok,
           'pred_a_two_rank64': bool(pa), 'pred_b_two_rank1': bool(pb),
           'pred_c_selective': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    for L in OWNERS:
        print(f"mlp{L}: " + " ".join(f"k{k}={swap[L][k]:.3f}" for k in KS))
    print(f"anchor {r_anchor:.3f} | rank64-ok {n_r64_ok} | rank1-ok {n_r1_ok}")
