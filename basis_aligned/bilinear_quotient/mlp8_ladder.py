# mlp8_ladder: THE LADDER WALKED TO mlp8 (board #3 after S1479: .047 unexplained,
# zoo-only). mlp7/mlp9 profile (.52 quad ceiling) predicts the same here; bars set
# accordingly — this run is COVERAGE (a seed for a deep-mid module), not a bet the
# template revives. Arms for mlp9's output (everything
...
#   pred_a lin2 [attn8, mlp7] >= .35 of mlp8's held-out stake.
#   pred_b linall >= .45.
...
OUT = PT + 'mlp8_ladder_results.json'
NFIT = 960; NEV = 960
H = m.transformer.h
STAND = {'mode': None, 'tensor': None}
CAP = {'on': False, 'store': None}
NAMES = ('m0', 'm1', 'm2', 'm3', 'm4', 'm5', 'm6', 'm7', 'm8', 'a8')
...
def main():
    ...
    hooks = [H[L].mlp.register_forward_hook(cap_hook_for(f'm{L}')) for L in range(9)]
    hooks.append(H[8].attn.c_proj.register_forward_hook(cap_hook_for('a8')))
    FT = capture(FITR)
    print("fit capture done", flush=True)

    MLPS = ['m0', 'm1', 'm2', 'm3', 'm4', 'm5', 'm6', 'm7']
    X2f = torch.cat([FT['a8'], FT['m7']], -1).reshape(-1, 2 * D)
    XAf = torch.cat([FT['a8']] + [FT[n] for n in MLPS], -1).reshape(-1, 9 * D)
    Yf = FT['m8'].reshape(-1, D)
    ...
    def fit_quad():
        ...
        NPOS = FT['m8'].shape[0] * T
        CH = 16384
        Aa = FT['a8'].reshape(-1, D); Mm = FT['m7'].reshape(-1, D)
    ...
    mh = H[8].mlp.register_forward_hook(stand_hook)
    ...
    def ce_run(mode):
        ...
        for i in range(0, NEV, 8):
            ...
            if mode is None:
                ...
                B = idx.shape[0]
                XeA = torch.cat([E['a8']] + [E[n] for n in MLPS],
                                -1).reshape(-1, 9 * D).to(DEV)
                linAp = ymA + (XeA - xmA) @ WA
                if mode == 'mean':
                    st = gmean.expand(B, T, D)
                elif mode == 'lin2':
                    Xe2 = torch.cat([E['a8'], E['m7']], -1).reshape(-1, 2 * D).to(DEV)
                    st = (ym2 + (Xe2 - xm2) @ W2).view(B, T, D)
                elif mode == 'linall':
                    st = linAp.view(B, T, D)
                else:
                    Wq, qm, rm = QFIT
                    q = quad_feats(E['a8'].reshape(-1, D), E['m7'].reshape(-1, D))
    ...
    sw = json.load(open(PT + 'optimal_ablation_all_results.json'))['results']['mlp8']
    ...
    out = {'ce': res, 'stake': round(stake, 4),
           ...
           'pred_a_lin2_35': bool(pa), 'pred_b_linall_45': bool(pb),
