# mlp_ladder_stack: WHERE IS THE WALL? The n-gram MLP construction climbed one layer at a
# time: mlp2@W32, mlp3@W64, mlp4@W64, each alone; stack6 = stack4 + {mlp2, mlp3}.
# The transition dossier says content is born L3-5; mlp4 reads the pooled precursor (§1094).
# Registered predictions:
#   pred_a mlp2@W32 <= 0.05;
#   pred_b mlp3@W64 <= 0.10;
#   pred_c mlp4@W64 >= 0.20 (the content wall);
#   pred_d stack6 <= 0.10 and sub-additive (stack6 <= stack4 + mlp2 + mlp3).
#
...
OUT = PT + 'mlp_ladder_stack_results.json'
NR = 16; W = 128
...
MLG = {L: {'on': False, 'idx': None, 'cache': None} for L in (1, 2, 3, 4)}
MLW = {1: 16, 2: 32, 3: 64, 4: 64}
...
def main():
    ...
    ALLF = set(range(18))
    CONDS = {'base': (set(), False, False, set()),
             'stack4': (ALLF, True, True, {1}),
             'm2ng': (set(), False, False, {2}),
             'm3ng': (set(), False, False, {3}),
             'm4ng': (set(), False, False, {4}),
             'stack6': (ALLF, True, True, {1, 2, 3})}
    hk = m.transformer.h[0].mlp.register_forward_hook(m0_bigram_hook)
    hkL = [m.transformer.h[Lm].mlp.register_forward_hook(mk_mlpL_hook(Lm)) for Lm in (1, 2, 3, 4)]
    ...
    for i in range(0, NR, 4):
        ...
        for cname, (layers, use_sink, use_m0, ng_layers) in CONDS.items():
    ...
    cost = {c: round(CE[c] - CE['base'], 4) for c in ('stack4', 'm2ng', 'm3ng', 'm4ng', 'stack6')}
    out = {'n_rows': NR, 'W': W, 'ce': CE, 'cost_vs_base': cost,
           'refs': {'stack4_1178': 0.0297},
           'sanity_base_matches_true': bool(abs(CE['base'] - CE['true_model']) <= 0.02),
           'pred_a_mlp2_cheap': bool(cost['m2ng'] <= 0.05),
           'pred_b_mlp3_cheap': bool(cost['m3ng'] <= 0.10),
           'pred_c_mlp4_wall': bool(cost['m4ng'] >= 0.20),
           'pred_d_stack6': bool(cost['stack6'] <= 0.10 and
                                 cost['stack6'] <= cost['stack4'] + cost['m2ng'] + cost['m3ng']),
           'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"CE {CE}")
    print(f"costs {cost}")
    print(f"sanity {out['sanity_base_matches_true']} | pred_a m2 {out['pred_a_mlp2_cheap']} | "
          f"pred_b m3 {out['pred_b_mlp3_cheap']} | pred_c m4-wall {out['pred_c_mlp4_wall']} | pred_d stack6 {out['pred_d_stack6']}")
