# class_range_multiseed: DO MY OWN §1625/§1627 CONCLUSIONS SURVIVE THE METHODOLOGY
# THAT JUST OVERTURNED §1612?
#
# §1628 and §1629 established that a null from a SINGLE random basis is a sample of
# size one, and that seed 1729 specifically is unrepresentative in a CELL-DEPENDENT
# direction: it ran ~.03 HIGH at question@mlp11 and ~.041 LOW at pronouns@mlp17,
# understating the published effect both times.
#
# §1625's class range (.1084) and §1627's 12/12 sign stability were BOTH computed
# against that same single basis. If seed 1729's bias also varies by CLASS MASK
# within one site, then the class range I reported may be partly an artifact of one
# basis interacting with six different masks, and the below-null signs could move.
# Applying the correction to old work and not to my own would be selective.
#
# Design: six classes at mlp11 rank-2 TOP-4, corrected quantity, 20 INDEPENDENT
# random rank-2 bases (seeds 1729-1748) x 3 disjoint 160-row chunks per class.
# The per-class null becomes a 60-trial mean instead of a 3-trial one.
#
# Registered predictions:
#   pred_a CLASS-DEPENDENCE SURVIVES A PROPER NULL: the range of 20-seed mean nulls
#          across the six classes is >= .05.
#   pred_b THE SINGLE SEED WAS DISTORTING IT: that range differs from §1625's
#          single-seed .1084 by >= .01 in absolute value.
#   pred_c §1627's BELOW-NULL FINDING SURVIVES: `the` still has a negative signed
#          gap (λ minus the 20-seed mean null) on all three chunks.
...
OUT = PT + 'class_range_multiseed_results.json'
NR = 960
...
def main():
    ...
    for cname, cfg in CELLS.items():
        ...
        out_cells[cname] = {'lambda': [round(x, 4) for x in lam_share],
        ...
        print(f"  {cname:9s} lambda {[round(x,4) for x in lam_share]} | null20 {null20:.4f} "
              f"| null_1729 {null1:.4f} | gap20 {gaps20}", flush=True)

    nulls20 = {k: v['null_20seed'] for k, v in out_cells.items()}
    rng20 = round(max(nulls20.values()) - min(nulls20.values()), 4)
    S1625_RANGE = 0.1084

    pa = rng20 >= 0.05
    pb = abs(rng20 - S1625_RANGE) >= 0.01
    pc = all(g < 0 for g in out_cells['the']['gaps_20seed'])

    print(f"\n  20-SEED mean null per class (mlp11 r2 TOP4, {N_RANDOM*CHUNKS} trials each):",
          flush=True)
    for k in sorted(nulls20, key=lambda x: -nulls20[x]):
        v = out_cells[k]
        print(f"    {k:9s} null20 {v['null_20seed']:.4f}  (seed1729 {v['null_seed1729']:.4f}, "
              f"delta {v['null_20seed']-v['null_seed1729']:+.4f})  gap20 {v['gaps_20seed']}",
              flush=True)
    print(f"  class range under 20-seed null: {rng20}   (§1625 single-seed range {S1625_RANGE})",
          flush=True)
    print(f"  'the' negative on all 3 chunks under 20-seed null: {pc}", flush=True)
    ...
    out = {'config': {'cells': {k: {kk: vv for kk, vv in v.items() if kk != 'pat'}
           ...
           'cells': out_cells, 'class_range_20seed': rng20, 'S1625_single_seed_range': S1625_RANGE,
           'predictions': {'pred_a_class_range_ge_05': bool(pa),
                           'pred_b_differs_from_S1625_range_ge_01': bool(pb),
                           'pred_c_the_still_below_null': bool(pc)},
