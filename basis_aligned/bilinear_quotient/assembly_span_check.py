"""ASSEMBLY SPAN CHECK -- the real model range-regulates its private
span via the attn5->mlp5 channel (292). The validated assembly bypasses
that junction entirely (attn5 = a mean vector, mlp5 = a CP truncation)
yet matches behavior within 3 nats. Does it reproduce the internal
regulation, or is it behaviorally right with different internal
statistics? Measure the mlp6-slot span-coefficient variance (real
model's span basis) in the assembled model vs the real model on window
A.
REGISTERED PREDICTIONS:
  (a) the assembly's span variance is OUTSIDE [0.5x, 2x] of the real
      model's (different internal traffic, extending 287's
      observation);
  (b) per-band variance table reported (span, next8, full)."""
...
OUT=PT+'assembly_span_results.json'
...
def main():
    ...
    def capture_m6(active):
        m6=[]
        hs=install(active)
        hs.append(m.transformer.h[6].mlp.register_forward_hook(
            lambda mo,i_,o_: m6.append(o_.detach().float()
                                       .reshape(-1,D))))
        ...
        return torch.cat(m6)
    Yreal=capture_m6([])
    Yasm=capture_m6(best)
    _,_,V6h=torch.linalg.svd((Yreal-Yreal.mean(0))[:40000],
                             full_matrices=False)
    S6=orth(V6h[:8].T); S16=orth(V6h[8:16].T)
    tabv={}
    for nm2,Pv in (('span',S6),('next8',S16),('full',None)):
        vr=float((Yreal@Pv).var(0).sum()) if Pv is not None             else float(Yreal.var(0).sum())
        va=float((Yasm@Pv).var(0).sum()) if Pv is not None             else float(Yasm.var(0).sum())
        tabv[nm2]={'real':round(vr,1),'assembly':round(va,1),
                   'ratio':round(va/vr,3)}
        print(f'{nm2:6s}: real {vr:.0f} assembly {va:.0f} '
              f'ratio {va/vr:.2f}',flush=True)
    r=tabv['span']['ratio']
    pa=r<0.5 or r>2.0
    out={'bands':tabv,'pred_a':bool(pa)}
    print(f"(a) span ratio outside [0.5,2] ({r:.2f}): "
