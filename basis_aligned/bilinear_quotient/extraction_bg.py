# extraction_bg: EXTRACTION INSIDE THE COMPRESSED ATTENTION BACKGROUND — the
# direct "does compression make extraction easier" cell (user's benchmark
# framing). S1592: in an optimal-CONSTANT background the 5 certified question
# heads recover only .159 of the class attention gap. Here the background is
# the certified ship attention tier (per-head rank-32 whitened QK everywhere,
# plain SVD at layers {8,16,17}, SPEC roster exact — the S1567-era composite;
# the question circuit heads are NOT in SPEC), MLPs intact, and the extraction
# adds the 5 question heads exactly on top.
#   rec_in_bg = (cB - cC) / (cB - c0): the fraction of the compressed
#   background's class deficit closed by the exact circuit heads.
# Reference: rec_const = .159 (extraction_q_results.json, parsed).
# NR=960, question class.
# Registered predictions:
#   pred_a the compressed background alone keeps most class function: its
#          class-CE rise <= .5 x the all-constants class rise (parsed from
#          extraction_q_results.json).
#   pred_b the exact circuit heads close >= .30 of the remaining background
#          class deficit (vs .159 in the constant background).
#   pred_c rec_in_bg >= 1.5 x rec_const (compression makes extraction
#          strictly easier, by half again).
...
OUT = PT + 'extraction_bg_results.json'
NR = 960
...
def main():
    ...
    xq = json.load(open(PT + 'extraction_q_results.json'))['res']
    ...
    pb = rec_bg >= 0.30
    pc = rec_bg >= 1.5 * rec_const
    out = {'res': res, 'pred_a_bg_keeps_half': bool(pa),
           'pred_b_rec_30': bool(pb), 'pred_c_easier_15x': bool(pc),
