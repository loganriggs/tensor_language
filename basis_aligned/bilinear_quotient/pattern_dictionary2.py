"""PATTERN DICTIONARY v2 -- 364: v1 fit RAW unnormalized patterns
(squared products, no softmax) and magnitude variance swamped shape
(1/162 at R^2>=0.7 despite functional swaps working). v2 separates
SHAPE from GAIN: each pattern row is normalized over keys (unit
sum, sign-preserving via abs-sum), then fit on the 6 archetypes.
REGISTERED PREDICTIONS:
  (a) >=100/162 heads reach shape-R^2 >= 0.5;
  (b) motif-census agreement of the dominant atom >= 80%;
  (c) worst heads listed (targets for new archetypes)."""
...
OUT=PT+'pattern_dictionary2_results.json'
NR=24
...
def main():
    ...
    for ri in range(NR):
        ...
        for li,blkm in enumerate(m.transformer.h):
            ...
            pat=pat/pat.abs().sum(-1,keepdim=True).clamp_min(1e-6)
            for hd in range(9):
                gi9=li*9+hd
                pf=pat[hd].reshape(-1)
    ...
    n07=sum(1 for v in R2.values() if v>=0.5)
    pa=n07>=100
