"""MIXED-SUBSTRATE FRONT -- the pricing law says pick each rung's
substrate by where downstream readers look. m2/m3's outputs feed the
sensitive blocks 4-9 (where the motif heads read), so they should be
EMPIRICAL (faithful); m0's output is heavily post-processed by
m1/m2/m3 fits before reaching the mid-band, so it can be FOLD (cheap,
weights-derived). Config: fold-m0 + empirical-m2/m3, matched-context
throughout, then the 38 motif-head gains on top.
REGISTERED PREDICTIONS (fresh):
  (a) mixed base (20 comps) <= +1.65 (near the pure-fold +1.46, far
      under the pure-empirical +1.86);
  (b) mixed base + 38 heads <= +2.20 (beats the pure-empirical L1's
      +2.29 -- a new 24-effective envelope point);
  (c) the motif marginal on the mixed base <= +0.60 (the mid-band
      faithfulness is what the heads price)."""
...
OUT=PT+'mixed_front_results.json'
...
def main():
    ...
    def build_arm(fold_set):
        ...
        def fit_res(li,tb,active):
            ...
            A=torch.linalg.solve(ft.T@ft+lam*torch.eye(ft.shape[1],
                                 ...
                                 ft.T@(Rr@P))
            return ('tableres',li,tb,A,P)
        pre=''.join(str(x) for x in sorted(fold_set)) or 'E'
        S[f'm0{pre}']=fit_res(0,FOLD[0] if 0 in fold_set else None,
                              ['a0'])
        act=['a0',f'm0{pre}','a1v','m1']
        S[f'm2{pre}']=fit_res(2,FOLD[2] if 2 in fold_set else None,act)
        act=act+[f'm2{pre}']
        S[f'm3{pre}']=fit_res(3,FOLD[3] if 3 in fold_set else None,act)
    ...
    cfgF=build_arm({0})   # MIXED: fold m0 + empirical m2/m3
    ...
    print(f'mixed base: C {baseline:+.4f} fresh {baseF2:+.4f}')
    print(f'mixed + 38 heads: C {L1C:+.4f} fresh {L1F:+.4f} '
          f'(marginal {margF:+.4f})')
    pa=baseF2<=1.65; pb=L1F<=2.20; pc=margF<=0.60
    out={'base_C':round(baseline,4),'base_F':round(baseF2,4),
         'L1_C':round(L1C,4),'L1_F':round(L1F,4),
         'marg_F':round(margF,4),
         'pred_a':bool(pa),'pred_b':bool(pb),'pred_c':bool(pc)}
    print(f"(a) mixed base <= +1.65: {'HELD' if pa else 'FAILED'}")
    print(f"(b) +38 heads <= +2.20: {'HELD' if pb else 'FAILED'}")
    print(f"(c) motif marginal <= +0.60: {'HELD' if pc else 'FAILED'}")
