...
OUT=PT+'fold_tables_results.json'
R0,R1=120,300
LAYERS=[0,1,2,3]

@torch.no_grad()
def build_tables():
    tabs={li:torch.zeros(V,D,device=DEV,dtype=torch.float16)
          for li in LAYERS}
    caps={}
    hs=[]
    for li in LAYERS:
        def mk(li=li):
            return lambda mo_,i_,o_: caps.__setitem__(li,o_.detach())
    ...
    for s0 in range(0,V,4096):
        ...
        for blk in m.transformer.h[:max(LAYERS)+1]:
            x,v1=blk(x,v1,x0)
        for li in LAYERS:
            tabs[li][s0:s0+idx.shape[0]]=caps[li][:,0].to(torch.float16)
...
def main():
    ...
    pa=recs[0]>=0.80; pb=recs[1]>=0.50
    pc=all(recs[i]>recs[i+1] for i in range(3))
    pd=res[0].get('shuffle_recovery',1)<=0.0
    out={'layers':{str(k):v for k,v in res.items()},
         'pred_a':bool(pa),'pred_b':bool(pb),'pred_c':bool(pc),
         'pred_d':bool(pd)}
    print(f"(a) mlp0 >=80%: {'HELD' if pa else 'FAILED'}")
    print(f"(b) mlp1 >=50%: {'HELD' if pb else 'FAILED'}")
    print(f"(c) monotone decay: {'HELD' if pc else 'FAILED'}")
