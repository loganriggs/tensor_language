...
OUT = PT + 'value_range_bands_results.json'
...
def main():
    ...
    out = {'n_rows': NR, 'W': WIN, 'ce': CE, 'cost_vs_base': cost,
           ...
           'pred_a_mid_carries': bool(cost['mid1'] + cost['mid2'] >= 0.6 * cost['all18']),
           'pred_b_late_local': bool(cost['late'] <= 0.02),
           'pred_c_superadditive': bool(bandsum < cost['all18']),
           'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"CE {CE}")
    print(f"costs {cost} | band sum {out['band_sum']} vs all18 {cost['all18']}")
    print(f"sanity {out['sanity']} | pred_a mid {out['pred_a_mid_carries']} | pred_b late {out['pred_b_late_local']} | pred_c superadd {out['pred_c_superadditive']}")
