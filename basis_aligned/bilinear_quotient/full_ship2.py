# full_ship2: THE SHIP EXTENDED — + context-fit mlp2 (lin2 ridge from [a2, m1_stream],
# solo .9202 S1437) and mlp17 (stream-linall ridge, solo .878 S1478). Stand-ins
# computed inline; stream mlp outputs stored during the pass for mlp17's read. Fits
# on captures under the all3 ship (context-matched, lesson 3).
...
#   pred_a all4 (+mlp2) <= 4.15.
#   pred_b all5 (+mlp2+mlp17) <= 4.45.
#   pred_c ship compounding stays <= 1.8x over the five solo deltas.
...
OUT = PT + 'full_ship2_results.json'
NR = 960
...
def main():
    ...
    ARMS = [('clean', frozenset(), frozenset()),
            ('mlp2_solo', frozenset(), frozenset({2})),
            ('mlp17_solo', frozenset(), frozenset({17})),
            ('all3', ALLL, frozenset({0, 1})),
            ('all4', ALLL, frozenset({0, 1, 2})),
            ('all5', ALLL, frozenset({0, 1, 2, 17}))]
    ...
    cl_ = res['clean']
    solos = {'attn': 0.1619, 'mlp1': 0.3125, 'mlp0': 0.1184,
             'mlp2': res['mlp2_solo'] - cl_, 'mlp17': res['mlp17_solo'] - cl_}
    d_all5 = res['all5'] - cl_
    factor = d_all5 / max(sum(solos.values()), 1e-6)
    pa = res['all4'] <= 4.15
    pb = res['all5'] <= 4.45
    pc = factor <= 1.8
    out = {'ce': res, 'solo_deltas': {k: round(v, 4) for k, v in solos.items()},
           'all5_delta': round(d_all5, 4), 'ship_compounding': round(factor, 3),
           'pred_a_all4_le_415': bool(pa), 'pred_b_all5_le_445': bool(pb),
           'pred_c_factor_le_18': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"solos {out['solo_deltas']} all5 {d_all5:.4f} factor {factor:.2f}")
