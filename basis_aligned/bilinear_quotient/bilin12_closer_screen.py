# bilin12_closer_screen: THE TRANSFER THREAD OPENS (the arc's n=1-model caveat). Does
# bilin12 (12L, 6 heads, D=768 — same family, §FINDINGS "universal" content results) have
# the same specialist structure? First rung: the close-bracket §1302-standard screen,
# ported. Per-layer y-mean ablation over ALL 12 attention layers (small model — the full
# sweep is affordable), bracket-depth targets, both controls.
#
# Registered predictions (transfer bets, from bilin18's structure):
#   pred_a A CONCENTRATED CLOSE-BRACKET LAYER EXISTS: some layer with target damage
#          >= 0.20 nats and concentration >= 8.
#   pred_b IT SITS MID-STACK: the winner's relative depth in [0.5, 0.85] (bilin18's
#          closer sits at 13/18 = 0.72).
#   pred_c BOTH CONTROLS CLEAN at the winner (<= 1.5x) — else verdict withheld per
#          standard.
...
OUT = PT + 'bilin12_closer_screen_results.json'
NMEAN = 24; NR = 1920
