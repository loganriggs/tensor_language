"""MECHANISM BOOTSTRAP v2 -- v1's trigger sets were hijacked by GPT-2
glitch tokens (anomalous embeddings, globally loud in every direction:
SourceFile, cffff, zero-width space...). Instrument fixes, both
mechanism-faithful and member-blind: (i) score = FRACTION of the
token's fold output that lands in the leaf's probe directions
(projection^2 / total output norm^2 -- cancels global loudness);
(ii) candidate vocabulary restricted to corpus-supported tokens
(frequency >= 2 in the corpus rows -- a corpus property, not a member
property). Same registered bars as v1:
  (a) >=50% of tested front-MLP leaves reach precision >= 3x base rate
      at recall >= 0.3; (b) random-set null <= 1.2x (median);
  (c) trigger sets printed."""
...
OUT=PT+'mechanism_bootstrap2_results.json'
CA=300; NB=78
MHL=list(range(2,10))
T=256
...
def main():
    ...
    for lf in leaves:
        ...
        u,c=flat.unique(return_counts=True)
        support=set(u[c>=2].tolist())
        score_cpu=score.cpu()
        cand=[t for t in score_cpu.argsort(descending=True).tolist()
              if t in support]
        q=max(8,int(rate*len(support)))
        trig=set(cand[:q])
