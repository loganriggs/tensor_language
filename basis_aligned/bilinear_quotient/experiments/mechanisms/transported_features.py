"""TRANSPORTED FEATURES v1 -- the attention-transported description
language, first rung. The programmable frontier is stuck at 55-58
across four input vocabularies (357): the standing hypothesis is
that the ~60 unprogrammable circuits depend on content ATTENTION
TRANSPORTS from far positions. The certified induction motif is a
token-MATCHING rule, so its transport is computable from the raw
token stream with no model: for position i with current token t,
let j = the most recent previous occurrence of t. New predicates:
  has_match (j exists within 256), match_close (i-j<=32),
  after_match_F = F[j+1] for each base feature F ("what followed
  last time"), at_match_F = F[j] shifted context.
These reach arbitrarily far back -- exactly what shifts and pair
folds cannot. Ladder rerun until dry.
REGISTERED PREDICTIONS:
  (a) converged programmable total >=64 of 118 (breaks the 55-58
      plateau by more than search noise);
  (b) shuffled-label null median <=0.6;
  (c) >=5 passes cite a match-transported predicate. If (a) FAILS,
      the attention-transport hypothesis loses its input-computable
      form and the remaining candidates are stream-level."""
...
OUT=PT+'transported_features_results.json'
CA=300; NB=78
MHL=list(range(2,10))
T=256
...
def main():
    ...
    # ---- match-transported features (induction-rule transport) ----
    ntok9=len(flat)
    matchj=torch.full((ntok9,),-1,dtype=torch.long)
    for r_ in range(ntokr):
        last={}
        row=tok2d[r_].tolist()
        for t_ in range(Tr):
            tk9=row[t_]
            if tk9 in last: matchj[r_*Tr+t_]=r_*Tr+last[tk9]
            last[tk9]=t_
    hasm=matchj>=0
    dist=torch.where(hasm,torch.arange(ntok9)-matchj,
                     torch.full((ntok9,),9999))
    LIB0['has_match']=hasm
    LIB0['match_close']=hasm&(dist<=32)
    LIB0['match_far']=hasm&(dist>32)
    base_names=list(LIB0.keys())
    for nm9 in ('is_newline','is_digit','is_punct','upper_initial',
                'starts_space','mid_word','seen_before'):
        v=LIB0[nm9]
        for lab,off in (('after_match_',1),('at_match_',0)):
            fb=torch.zeros(ntok9,dtype=torch.bool)
            src9=matchj+off
            ok9=hasm&(src9<ntok9)&((src9//Tr)==(torch.arange(ntok9)//Tr))
            fb[ok9]=v[src9[ok9]]
            LIB0[lab+nm9]=fb
    print(f'match-transported features added ({len(LIB0)} base)',
          flush=True)
    LIB=dict(LIB0)
    ...
    def run_prog_lib(f,prog):
        ...
        for conj in prog:
            ...
            mm|=cm
        return mm
    passes_all={r['tag']:r['program'] for r in res if r['bacc']>=0.75}
    fails=set(r['tag'] for r in res if r['bacc']<0.75)
    rounds=[]
    for rnd in range(6):
        added=0
        newly={}
        for lf in leaves:
            if lf['tag'] not in fails: continue
            mem=lf['_member']
            memflat=torch.zeros(len(flat),dtype=torch.bool)
            memflat[mem]=True
            f,_=leaf_feats(memflat)
            for tg9,pg9 in passes_all.items():
                f['circ_'+tg9.replace('.','_')]=run_prog_lib(f,pg9)
            bv=basev
            mb=bv[mem]
            lo,hi=mb.quantile(0.1),mb.quantile(0.9)
            nonm=(~memflat)&(bv>=lo)&(bv<=hi)
            ...
            nonidx=nonidx[torch.randperm(len(nonidx),generator=g9)
                          [:len(mem)]]
            posA=mem[halfmask[mem]]; posB=mem[~halfmask[mem]]
            negA=nonidx[halfmask[nonidx]]
            negB=nonidx[~halfmask[nonidx]]
            if min(len(posA),len(posB),len(negA),len(negB))<30: continue
            mask,prog=rule_search(f,posA,negA)
            tp=float(mask[posB].float().mean())
            fp=float(mask[negB].float().mean())
            ...
            if bacc2>=0.75:
                added+=1
                newly[lf['tag']]=prog
                print(f"round {rnd} PASS {lf['tag']}: {bacc2:.2f} "
                      f"{prog}",flush=True)
        rounds.append(added)
        passes_all.update(newly)
        fails-=set(newly)
        print(f'round {rnd}: +{added} (total {len(passes_all)})',
              flush=True)
        if added<3: break
    it2=sum(rounds)
    try:
    ...
    print(f"converged programmable total: {total}/118 "
          f"(rounds {rounds})")
    allp=[str(p) for p in passes_all.values()]
    nf=sum(1 for p in allp if 'match' in p)
    gainN=total-58
    pc=nf>=5
    out['pred_c']=bool(pc); out['n_match_cited']=nf; out['gain_vs_58']=gainN
    print(f'match-cited passes: {nf} | gain vs 58: {gainN:+d}')
    print(f"(a) converged >=64: {'HELD' if pa else 'FAILED'}")
    print(f"(b) null <=0.6: {'HELD' if pb else 'FAILED'}")
    print(f"(c) >=5 match cites: {'HELD' if pc else 'FAILED'}")
