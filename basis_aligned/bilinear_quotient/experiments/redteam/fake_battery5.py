...
census leaf -- catches mixtures; cannot hit reals, whose pairwise
Jaccard is <0.5 by the 323 dedup). Specificity-ratio reported but not
gated (broad-interface physics makes a hard bar misclassify reals).
REGISTERED PREDICTIONS:
  (a) 5/5 reals pass (sel, smix, not-composite);
  (b) 5/5 fakes rejected: F1 sel, F2 structural, F3 composite,
      F4 sel, F5 smix. Any deviation reported as-is; the battery
      CONVERGES after this run regardless of outcome."""
...
def main():
    ...
    for i,cd in enumerate(cands):
        ...
        e_forctl=float(foreign[ctlho].abs().mean())
        selv=e_own/max(e_ctl,1e-4)
        self_for=e_for/max(e_forctl,1e-4)
        spec=selv/max(self_for,1e-4)
        memfit=mem[fit[mem]]
        ctlfit=ctl[fit[ctl]]
        selv_in=float(own[memfit].abs().mean())/            max(float(own[ctlfit].abs().mean()),1e-4)
        msets={lf2['tag']:set(lf2['_member'].tolist())
               for lf2 in leaves}
    ...
    def ok_real(r):
        sv=r['selectivity']
        if sv!=sv: return False   # nan == structural rejection
        return (sv>=2 and r.get('sign_minority',0)>=0.15
                and r.get('composite',0)<0.6)
    realpass=sum(1 for t in reals if ok_real(R[t]))
    pa=realpass>=5
    f2s=R['F2_severity']['selectivity']
    pb=(R['F1_random']['selectivity']<=1.3 and
        (R['F2_severity'].get('structural_reject',False) or f2s!=f2s))
    pc=R['F3_franken'].get('composite',0)>=0.6
