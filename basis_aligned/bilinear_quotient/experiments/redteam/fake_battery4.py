"""FAKE BATTERY v4 -- final specificity form (332): specificity =
selectivity(own)/selectivity(foreign), each vs matched controls, so
component strength cancels. nan selectivity == structural rejection.
REGISTERED PREDICTIONS:
  (a) >=4/5 reals pass (sel>=2, spec-ratio>=1.5, sign-minority>=0.15);
  (b) all five fakes rejected on >=1 dimension (hole stays closed)."""
...
OUT=PT+'fake_battery4_results.json'
CA=300; NB=78
MHL=list(range(2,10))
T=256
...
def main():
    ...
    def ok_real(r):
        ...
        return (sv>=2 and r['specificity']>=1.5
