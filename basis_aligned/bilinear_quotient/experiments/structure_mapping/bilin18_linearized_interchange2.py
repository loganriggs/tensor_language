"""v2 -- v1 was VOID: the linear-replacement hook ran after the span-ablation
hook and overwrote it, so d17-under-linearization was structurally zero and the
excess "drop" was an artifact (recorded in section 102). This version registers
the replacement FIRST so ablations apply to the linear map's output. Predictions
unchanged.
...
OUT=('/workspace/tensor_language/basis_aligned/bilinear_quotient/'
     'bilin18_linearized_interchange2_results.json')
...
def ce_eval():
    ...
    if LIN17:
        ...
        hs+=[h_pre,blk.mlp.register_forward_hook(mlp_hook)]
