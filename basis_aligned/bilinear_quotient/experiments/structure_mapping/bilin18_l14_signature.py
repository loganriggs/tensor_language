"""Signature of the late-attention harm (section 138). Is L14's negative the
...
OUT=('/workspace/tensor_language/basis_aligned/bilinear_quotient/'
     'bilin18_l14_signature_results.json')
...
def main():
    ...
    for li in (14,13):
        ...
        d=ce1-ce0
        tot=float(d.sum())
        hshare=float(d[hard].sum())/tot if abs(tot)>1e-6 else float('nan')
        emean=float(d[easy].mean()); hmean=float(d[hard].mean())
        out[str(li)]={'total':tot/d.numel(),'hard_share':hshare,
                      'easy_mean':emean,'hard_mean':hmean}
        print(f'L{li}: mean {tot/d.numel():+.4f} | hard-mean {hmean:+.4f} | '
              f'easy-mean {emean:+.4f} | hard share {hshare:.2f}',flush=True)
    pa=out['14']['hard_share']>=0.6 and out['14']['total']<0
    pb=out['14']['easy_mean']>=-0.001
    pc=out['13']['hard_mean']>0
    out['pred_a']=bool(pa); out['pred_b']=bool(pb); out['ctrl_c']=bool(pc)
    print(f"\n(a) benefit concentrated on hard (>=60%): {'HELD' if pa else 'FAILED'}")
    print(f"(b) easy not helped: {'HELD' if pb else 'FAILED'}")
    print(f"(c) L13 hurts hard tokens: {'HELD' if pc else 'VIOLATED'}")
