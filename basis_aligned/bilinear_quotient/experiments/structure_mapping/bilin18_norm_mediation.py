"""v2, control fixed. v1 froze the hybrid's gain to the REAL model's clean rms
(wrong reference; base shifted 0.0097). This version measures on the REAL model
(no linear L17) and freezes each arm's final gain to the SAME model's own
no-damage per-token rms. REGISTERED: (a) control exact (<=0.002); (b)
content-level excess >= 0.05 (the 16->17 interaction is real at content level);
(c) norm-mediated share of individual damage >= 60% at both spans.
...
OUT=('/workspace/tensor_language/basis_aligned/bilinear_quotient/'
     'bilin18_norm_mediation_results.json')
...
def fwd_arm(idx, spans, span_lis, lin17, clamp17):
    ...
    if clamp17:   # freeze final per-token gain to THIS model's no-damage run
...
def main():
    ...
    exc_c=out['norm_frozen']['excess']
    share16=1-out['norm_frozen']['d16']/out['free']['d16']
    share17=1-out['norm_frozen']['d17']/out['free']['d17']
    pa=ctrl<=0.002; pb=exc_c>=0.05; pcx=share16>=0.6 and share17>=0.6
    out['content_excess']=exc_c; out['norm_share_16']=share16
    out['norm_share_17']=share17
    print(f"(a) control exact (<=0.002): {'HELD' if pa else 'VIOLATED'} ({ctrl:.4f})")
    print(f"(b) content-level excess >=0.05: {'HELD' if pb else 'FAILED'} ({exc_c:+.4f})")
    print(f"(c) norm share >=60% both: {'HELD' if pcx else 'FAILED'} "
          f"({share16:.0%}/{share17:.0%})")
    out['kill']=kill; out['ctrl_gap']=ctrl
    out['pred_a']=bool(pa); out['pred_b']=bool(pb); out['pred_c']=bool(pcx)
