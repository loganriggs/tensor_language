"""Reference submission v2: push toward the Pareto frontier. Improvements over
...
OUT=('/workspace/tensor_language/basis_aligned/bilinear_quotient/'
     'bilin18_reference_v2_results.json')
...
def main():
    ...
    # naive v1-style assignment (rank-16) for the refit comparison
    assign={}
    for li in const_layers: assign[li]=fit_rank(li,0)
    for li in rank8_layers: assign[li]=fit_rank(li,16)
    PR.LINS=assign
    joint_naive=ce()-base
    # sequential refit: refit each stand-in with upstream installed
    PR.LINS={}
    refit={}
    ...
    for li in sorted(const_layers+rank8_layers):
        r=0 if li in const_layers else 16
        PR.LINS=dict(refit)
        refit[li]=fit_rank_live(li,r)
    PR.LINS=refit
    joint_refit=ce()-base
    PR.LINS={}
    params=len(rank8_layers)*2*D*16
    pa=joint_refit<=1.8 and params<=0.4e6
    pb=joint_refit<=0.75*joint_naive
    out={'r0cost':{str(k):v for k,v in r0cost.items()},
         'const_layers':const_layers,'rank16_layers':rank8_layers,
         'joint_naive':joint_naive,'joint_refit':joint_refit,
         'params':params,'pred_a':bool(pa),'pred_b':bool(pb),'base':base}
    print(f'joint naive +{joint_naive:.3f} | joint REFIT +{joint_refit:.3f} '
          f'({params/1e6:.2f}M params)')
    print(f"(a) refit <= 1.8 at <= 0.4M: {'HELD' if pa else 'FAILED'}")
    print(f"(b) refit buys >= 25%: {'HELD' if pb else 'FAILED'}")
