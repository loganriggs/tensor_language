...
OUT=('/workspace/tensor_language/basis_aligned/bilinear_quotient/'
     'bilin18_l1_span_linearization_results.json')
...
def main():
    ...
    pa=span>=0.6*full; pb=comp<=0.4*full
    pc=abs((span+comp)-full)<=0.15*full
    out={'base':base,'full':full,'span':span,'comp':comp,'random':rnd,
         'span_energy_share':espan,'pred_a':bool(pa),'pred_b':bool(pb),
         'pred_c':bool(pc)}
    print(f"(a) span carries >=60%: {'HELD' if pa else 'FAILED'} ({span/max(full,1e-9):.0%})")
    print(f"(b) complement <=40%: {'HELD' if pb else 'FAILED'} ({comp/max(full,1e-9):.0%})")
    print(f"(c) additive within 15%: {'HELD' if pc else 'FAILED'}")
