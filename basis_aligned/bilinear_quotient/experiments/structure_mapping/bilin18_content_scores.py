"""Content scores for the tail (section 141): re-rank the layers' top-8 MLP
...
OUT=('/workspace/tensor_language/basis_aligned/bilinear_quotient/'
     'bilin18_content_scores_results.json')
...
def main():
    ...
    CONTENT120={5:0.0095,6:-0.0164,7:0.0139,8:0.0020,9:-0.0230,10:0.0050,
                11:0.0185,12:-0.0053,13:0.0089,14:0.0034,15:-0.0150,16:0.1480,
                17:0.0928}
    hm={int(n[1:]): out[n]['hard_mean'] for n in out if n.startswith('L')}
    pa=(hm[16]>=0.05) and all(hm[li]<0.05 for li in hm if li!=16)
    ls=sorted(hm)
    conc=0
    for i in range(len(ls)):
        for j in range(i+1,len(ls)):
            s1=(hm[ls[i]]-hm[ls[j]])*(CONTENT120[ls[i]]-CONTENT120[ls[j]])
            conc+=1 if s1>0 else -1
    tau=conc/(len(ls)*(len(ls)-1)/2)
    pb=tau<=0.7
    out['pred_a']=bool(pa); out['kendall_vs_net']=tau; out['pred_b']=bool(pb)
    print(f"\n(a) L16 only content span (hard>=+0.05): {'HELD' if pa else 'FAILED'}")
    print(f"(b) content ranking reorders net (tau<=0.7): {'HELD' if pb else 'FAILED'} ({tau:+.2f})")
