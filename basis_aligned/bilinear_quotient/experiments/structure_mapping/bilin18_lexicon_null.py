"""Origin of the attention lexicon (section 148): is the within-layer filter
ALIGNMENT (0.71 vs 0.35 matched null) trained, or would shuffled weights'
filters also cluster? Same pipeline with row-shuffled Wq/Wk per head.
REGISTERED: (a) shuffled within-layer alignment <= matched-null + 0.1 (the
lexicon is trained); alternative: generic clustering scope-notes section 130.
...
OUT=('/workspace/tensor_language/basis_aligned/bilinear_quotient/'
     'bilin18_lexicon_null_results.json')
...
def main():
    ...
    rm=med_cos(mpairs)
    pa=w<=rm+0.1
    out={'shuffled_within':w,'shuffled_across':x,'matched_null':rm,
         'pred_a_trained':bool(pa)}
    print(f'SHUFFLED within {w:.2f} | across {x:.2f} | matched null {rm:.2f}')
    print(f"(a) lexicon is trained (shuffled <= null+0.1): {'HELD' if pa else 'FAILED'}")
