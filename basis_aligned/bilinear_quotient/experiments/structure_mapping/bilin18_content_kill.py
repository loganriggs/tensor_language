"""Content-level skin test. Section 117: at content level (per-arm frozen final
...
OUT=('/workspace/tensor_language/basis_aligned/bilinear_quotient/'
     'bilin18_content_kill_results.json')
...
def main():
    ...
    for tag,l17,clamp in (('real_frozen',None,True),('linear_frozen','fit',True)):
        L=lin17_map if l17=='fit' else None
        b_=ce(spans,[],L,clamp)
        d16=ce(spans,[16],L,clamp)-b_
        d17=ce(spans,[17],L,clamp)-b_
        joint=ce(spans,[16,17],L,clamp)-b_
        exc=joint-d16-d17
        out[tag]={'base':b_,'d16':d16,'d17':d17,'excess':exc}
        print(f'{tag:14s}: base {b_:.4f} | d16 {d16:+.4f} | d17 {d17:+.4f} | '
              f'excess {exc:+.4f}',flush=True)
    kill=1-out['linear_frozen']['excess']/out['real_frozen']['excess'] \
         if out['real_frozen']['excess']>1e-6 else float('nan')
    ctrl=0.0
