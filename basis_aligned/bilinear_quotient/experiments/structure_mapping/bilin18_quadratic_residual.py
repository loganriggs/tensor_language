"""User-suggested (2026-08-17): compressed quadratic on the residual of the
...
OUT=('/workspace/tensor_language/basis_aligned/bilinear_quotient/'
     'bilin18_quadratic_residual_results.json')
...
def main():
    ...
    for li in (17,9):
        ...
        P=orth(Vh[:16].T)
        rt=Rt@P; re=Re@P
        _,_,Vhx=torch.linalg.svd(Xc[:20000],full_matrices=False)
        row={}
        for k in (16,32,64):
            U=orth(Vhx[:k].T)
    ...
    print(f"\n(a) L17 skin compact (>=0.5 at k=64): {'HELD' if pa else 'FAILED'}")
    print(f"(b) L9 residual high-rank (<=0.3): {'HELD' if pb else 'FAILED'}")
