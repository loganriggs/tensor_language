"""Round 3c (section 201, ledger #15): the class lever with an HONESTLY CHEAP
warm base -- rank-32 linear base + width-64 bilinear residual at L1, total
0.30M params, all counted. REGISTERED: (a) L1 combo <= +0.60 individually (the
class lever stands at honest cost, beating rank-64 linear +1.11 and shading
toward full linear +0.29 at 1/4 its params); (b) L16 with rank-4 base +
width-16 bilinear (0.08M) <= +0.05; failure closes the class question.

Prior context -- HILLCLIMB round 3b (section 199): the class lever, fit properly. Stand-in =
...
OUT=('/workspace/tensor_language/basis_aligned/bilinear_quotient/'
     'bilin18_hillclimb3c_results.json')

def fit_bilinear(X,Y,h,steps=800,lr=3e-3):
    bx=X.mean(0); by=Y.mean(0)
    Xc=(X-bx)
    # closed-form linear warm base, TRUNCATED to cheap rank
    ...
    _rk=fit_bilinear.base_rank
    U_,S_,Vh_=torch.linalg.svd(Wl)
    Wl=U_[:,:_rk]@torch.diag(S_[:_rk])@Vh_[:_rk]
    Rres=(Y-by)-Xc@Wl
...
def main():
    ...
    base=ce_with(0,None)
    out={}
    for li,h,bar,rk in ((1,64,0.60,32),(16,16,0.05,4)):
        fit_bilinear.base_rank=rk
        X=torch.cat([a[li] for a,b in tri]); Y=torch.cat([b[li] for a,b in tri])
        mp=fit_bilinear(X.float(),Y.float(),h)
        cost=ce_with(li,mp)-base
        out[f'L{li}']={'width':h,'cost':cost,'params':3*D*h+2*D*rk,
                       'mse0':mp['mse0'],'msef':mp['msef']}
        print(f'L{li} w{h}+r{rk}base: +{cost:.3f} ({(3*D*h+2*D*rk)/1e6:.2f}M honest) '
    ...
    print(f"\n(a) L1 honest combo (<=0.60): {'HELD' if pa else 'FAILED'}")
