"""CENSUS RED-TEAM ROUND 3 -- the SELECTIVITY bar, registered fresh
(324): on the NEXT twenty untouched blind-nameable leaves (quiz passers
21-40 by score order, none tested in rounds 1-2), joint ablation of
each leaf's implicated components must give pooled |member effect| >=
2x the |effect| on base-CE-matched non-members.
REGISTERED PREDICTION: >=60% pass. Sign agreement is reported but NOT
scored (the linear-unit claim already fell in 324). If selectivity
fails too, the census narrows to its replication tier alone."""
...
OUT=PT+'census_redteam3_results.json'
CA=300; NB=78
MHL=list(range(2,10))
T=256
