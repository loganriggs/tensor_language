"""CIRCUIT TREE v2 -- scale the validated recursion (v1: 17 leaves,
child replication 0.60): all 212 window-A rows (54k tokens), 24 root
modes, TWO recursion levels (pca-block parents refined by
block-halving on the child slice).
REGISTERED PREDICTIONS:
  (a) >= 40 document-disjoint-replicated leaves;
  (b) >= half taxonomy-invisible (class R^2 <= 0.15);
  (c) descendant replication rate >= 0.5 across both levels;
  (d) full tree in circuit_tree2.json for the naming wave."""
...
OUT=PT+'circuit_tree2_results.json'
CA=300; NB=53
MHL=list(range(2,10))
T=256
...
def main():
    ...
    def factor(pspecs,slice_idx,nmodes,tag):
        ...
        for k in range(min(nmodes,len(Sg))):
            ...
            ii=sc.abs().argsort(descending=True)[:6]
    ...
    json.dump(out,open(PT+'circuit_tree2.json','w'),indent=1)
