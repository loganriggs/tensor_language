"""CIRCUIT TREE -- scale the data-dual to ~100 supervised circuits by
RECURSIVE probe refinement (user direction): a replicated damage mode
names components; refine probes INSIDE those components (MLP -> 4
slice-conditioned output-PCA-block ablations; attention layer -> its 9
heads); re-factor the damage matrix restricted to the mode's own data;
split; recurse. Document-disjoint replication is the gate at every
node, so the worst failure is an honestly-too-broad cluster.
Level 0: the 108 coarse probes (36 components + 72 mid-band heads),
top-16 modes. Level 1: for each replicated mode, refined probes from
its top-4 loading components, factored on its top-15% token slice,
top-4 submodes. Leaves = all replicated modes/submodes.
REGISTERED PREDICTIONS:
  (a) >= 40 leaves pass document-disjoint replication (cos >= 0.6);
  (b) >= half the leaves have 10-class taxonomy R^2 <= 0.15 (mostly
      structure the taxonomy cannot see);
  (c) recursion works: >= 50% of children of replicated parents
      themselves replicate;
  (d) full tree written to circuit_tree.json (probes, sample tokens,
      replication, class-R^2 per leaf) for the naming wave."""
...
OUT=PT+'circuit_tree_results.json'
CA=300; NB=36
MHL=list(range(2,10))
T=256
...
def main():
    ...
    for md in roots:
        ...
        if not ok: continue
        leaves.append(md)
        # ---- level 1: refined probes on this mode's slice ----
        fine=[]
        seen=set()
        ...
        for sm in subs:
            ...
            if okc:
                child_stats[0]+=1
                leaves.append(sm)
            print(f"  {sm['tag']}: repl {sm['repl']} class "
    ...
    json.dump(out,open(PT+'circuit_tree.json','w'),indent=1)
