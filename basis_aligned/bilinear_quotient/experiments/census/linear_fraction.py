"""LINEAR-UNIT FRACTION, BATCH 3 -- the 325 registered follow-up as a
population estimate. Batches 1-2 (leaves 0-40 by name score) give 4/20
and 5/20 sign-aware linear units: pooled 22.5%. Batch 3 = leaves 40-60,
same sign-aware bar (opposite-direction movement by score sign, pooled
|effect| >= 2x matched controls, >=70% member sign agreement).
REGISTERED PREDICTIONS:
  (a) batch-3 count in [1, 8] of 20 (binomially consistent with 22.5%);
  (b) pooled 60-leaf estimate reported with a Wilson interval -- THE
      quotable linear-unit fraction for the census."""
...
OUT=PT+'linear_fraction_results.json'
CA=300; NB=78
MHL=list(range(2,10))
T=256
...
def main():
    ...
    try:
        ...
        allp=[r['tag'] for r in qs if r['true_pos']>=4]
        passtags=allp[40:60]
    except Exception:
    ...
    import math
    tot_n=40+tested; tot_k=9+passed
    p=tot_k/tot_n; z=1.96
    den=1+z*z/tot_n
    ctr=(p+z*z/(2*tot_n))/den
    hw=z*math.sqrt(p*(1-p)/tot_n+z*z/(4*tot_n*tot_n))/den
    print(f'pooled linear-unit fraction: {tot_k}/{tot_n} = {p:.1%} '
          f'(Wilson 95%: {ctr-hw:.1%}-{ctr+hw:.1%})')
    pdx=1<=passed<=8
    out={'causal_tested':tested,'causal_passed':passed,'causal':caus,
         'pred_signaware':bool(pdx),
         'n_distinct':ndistinct,'sign_mixed':nmix}
    print(f"(a) batch-3 in [1,8]: {passed}/{tested}: "
