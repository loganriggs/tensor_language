...
OUT=PT+'assembled_v3_results.json'
...
def main():
    ...
    S['m0']=('table',0,fit_table(Y,ids)); order.append('m0')
    ...
    for li,nm in ((2,'m2'),(3,'m3')):
        ...
        S[nm]=('table',li,fit_table(Y,ids)); order.append(nm)
    ...
    m1ctx=full-nom1
    pa=full<=0.90; pb=full<1.85 and full<2.69; pc=m1ctx<=0.24
    out={'base':round(base,4),'assembled_v3':round(full,4),
         'without_m1':round(nom1,4),'m1_in_context':round(m1ctx,4),
         'pred_a':bool(pa),'pred_b':bool(pb),'pred_c':bool(pc)}
    print(f'base {base:.3f} | ASSEMBLED v3 {full:+.4f} | without m1 '
          f'{nom1:+.4f} | m1 in-context {m1ctx:+.4f}')
    print(f"(a) <=0.90: {'HELD' if pa else 'FAILED'}")
    print(f"(b) beats v1 and v2: {'HELD' if pb else 'FAILED'}")
