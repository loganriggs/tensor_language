"""ASSEMBLED v10 -- FRONTIER CONSOLIDATION: the v9-best recipe (34/36,
+2.868) with two upgrades from the ladders: front absorbers at write-rank
P=128 (270: monotone to +1.899 on the 19-comp assembly) and a8 (the digit
owner, the one dropped rung) excluded from the start so its budget is
never spent.
REGISTERED PREDICTIONS:
  (a) v10 oracle <= +2.75 (beats v9-best 2.868 via the P128 front);
  (b) deploy gap <= +0.20;
  (c) informational: per-band cost split reported (eval minus-front,
      minus-middle-attn arms)."""
...
OUT=PT+'assembled_v10_results.json'
...
def main():
    ...
    fullD=evalC(order,'probe')-base
    noatm=evalC([n for n in order if not (n[0]=='a' and n[1:].isdigit()
                 and 2<=int(n[1:])<=9)],'oracle')-base
    pa=full<=2.75; pb=(fullD-full)<=0.20
    out={'base':round(base,4),'v10_oracle':round(full,4),
         'v10_deploy':round(fullD,4),
         'without_middle_attn':round(noatm,4),
         'pred_a':bool(pa),'pred_b':bool(pb)}
    print(f'base {base:.3f} | v10 {full:+.4f} | deploy {fullD:+.4f} | '
          f'without middle-attn {noatm:+.4f}')
    print(f"(a) v10 <= +2.75: {'HELD' if pa else 'FAILED'}")
    print(f"(b) deploy gap <= +0.20: {'HELD' if pb else 'FAILED'}")
