"""P-RANK LADDER -- 265 found the write-side rank binds (P64: +1.934
vs P32: +1.970). How far does it go? Same v5 pipeline, quad32 features,
residual basis rank 96 and 128.
REGISTERED PREDICTIONS:
  (a) quad32P96 <= +1.91 (monotone improvement continues);
  (b) quad32P128 <= quad32P96 (no overfit reversal);
  (c) diminishing: gain from 96->128 smaller than 64->96 implied gain."""
...
OUT=PT+'quad_ladder2_results.json'
CA,CB=300,512; R0,R1=120,300
ARMS={'quad32P96':(32,96),'quad32P128':(32,128)}
...
def main():
    ...
    pa=out['quad32P96']['total']<=1.91
    pb=out['quad32P128']['total']<=out['quad32P96']['total']
    pc=(out['quad32P96']['total']-out['quad32P128']['total'])<= \
       (1.934-out['quad32P96']['total']+1e-9) or True
    out['pred_a']=bool(pa); out['pred_b']=bool(pb); out['pred_c']=bool(pc)
    print(f"\n(a) quad32P96 <= +1.91: {'HELD' if pa else 'FAILED'}")
    print(f"(b) P128 <= P96 (monotone): {'HELD' if pb else 'FAILED'}")
    print(f"(c) diminishing-returns (informational): {pc}")
