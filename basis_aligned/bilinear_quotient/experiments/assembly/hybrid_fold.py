"""HYBRID + FOLD -- merge the two certified wins: the head-level hybrid
(26 comps + 38 motif heads, +2.543 fresh) with the fold front (m0, m2,
m3 as weights-derived tables, 305: strictly dominant, -0.32 fresh).
REGISTERED PREDICTIONS:
  (a) merged config <= +2.35 on fresh rows (new frontier);
  (b) merged beats the empirical hybrid (+2.543) by >= 0.15;
...
OUT=PT+'hybrid_fold_results.json'
...
def main():
    ...
    # fold tables for m0/m2/m3 (weights-only, then refit absorbers)
    FOLD={}
    capsF={0:[],2:[],3:[]}
    hsF=[]
    for li in (0,2,3):
    ...
    def evalHF(active,motif):
        ...
        return float(torch.cat(ces).mean())
    base=evalHF([],False)
    hy=evalHF(hyb,True)-base            # empirical hybrid (reference)
    hyF=evalHF(hybF,True)-base          # fold hybrid (the merge)
    baseC2=evalH([],False)
    hyFC=evalH(hybF,True)-baseC2
