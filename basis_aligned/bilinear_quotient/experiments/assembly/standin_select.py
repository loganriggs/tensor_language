"""GREEDY STAND-IN SELECTION -- the a5 anomaly resolved: a constant mean
output BEAT both the real attn5 (+3.47) and its class dictionary (+2.87)
at +2.72; zeroing was catastrophic (+4.75). Some attention components are
scaffolding: the assembly needs their average contribution, and their
input-dependence only misfires on substituted streams. Sweep all 16
attention rungs front-to-back, swapping each dict for its mean (and
trying dropped a8 as a mean), keeping any swap that improves the total.
REGISTERED PREDICTIONS:
  (a) final total <= +2.60 (from v9-best +2.868);
  (b) >=2 rungs prefer the mean over the dictionary;
  (c) a8-as-mean is adopted (rescuing the dropped rung -> 35/36)."""
...
OUT=PT+'standin_select_results.json'
...
def main():
    ...
    print(f'start (v9-best) {cost:+.4f}',flush=True)
    swaps=[]
    for li in range(2,18):
        nm=f'a{li}'
        if li==8:
            ...
            for n in best:
                cand.append(n)
                if n=='c8': cand.insert(-1,'a8m')
        else:
            cand=[(f'a{li}m' if n==nm else n) for n in best]
        c=evalC(cand,'oracle')-base
        tag='ADOPT' if c<cost-0.005 else 'keep'
        print(f'{nm}->mean {c:+.4f} ({tag})',flush=True)
        if c<cost-0.005:
            best=cand; cost=c; swaps.append(nm)
    dep=evalC(best,'probe')-base
    pa=cost<=2.60; pb=len(swaps)>=2; pc='a8' in swaps
    out={'base':round(base,4),'final':round(cost,4),
         'final_deploy':round(dep,4),'swaps':swaps,
         'pred_a':bool(pa),'pred_b':bool(pb),'pred_c':bool(pc)}
    print(f'final {cost:+.4f} | deploy {dep:+.4f} | swaps {swaps}')
    print(f"(a) final <= +2.60: {'HELD' if pa else 'FAILED'}")
    print(f"(b) >=2 mean-swaps: {'HELD' if pb else 'FAILED'}")
    print(f"(c) a8 rescued as mean: {'HELD' if pc else 'FAILED'}")
