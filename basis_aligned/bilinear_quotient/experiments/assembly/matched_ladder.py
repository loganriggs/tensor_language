"""MATCHED LADDER -- rebuild coverage on top of the 308 base (20 comps
at +1.46 fresh), each rung fit under its own eval context:
  L0: base = fold front + CP middles + tail (308 replication);
  L1: + 38 motif-head gains (alphas fit under L0);
  L2: + tail-attention dictionaries a10-17 (fit under L1's stack);
  L3: + middle-attention dictionaries a2-a9 except a8 (fit under L2,
      replacing the real band the motif heads lived in -- heads swap
      out where their component swaps in).
REGISTERED PREDICTIONS (fresh):
  (a) L1 <= +1.65; (b) L2 <= +2.05; (c) L3 <= +2.55 (a full-census
      config beating the old +2.925 by >= 0.37); (d) window-C values
      reported per rung."""
...
OUT=PT+'matched_ladder_results.json'
...
def main():
    ...
    # ---- L2: tail-attention dicts fit under cfgF+motifs ----
    Yoh=torch.zeros(len(flatA),10,device=DEV)
    Yoh[torch.arange(len(flatA)),flatA]=1.0
    order2=list(cfgF)
    Wp2=None
    ...
    del cur['clsmap']
    print(f'L2 (+tail attn): C {L2C:+.4f} | fresh {L2F:+.4f}',flush=True)
    pa=L1F<=1.65; pb=L2F<=2.05
    out={'base_C':round(baseC,4),'base_F':round(baseF,4),
         'L1':{'C':round(L1C,4),'fresh':round(L1F,4)},
         'L2':{'C':round(L2C,4),'fresh':round(L2F,4)},
         'pred_a':bool(pa),'pred_b':bool(pb)}
    print(f"(a) L1 <= +1.65 fresh: {'HELD' if pa else 'FAILED'}")
    print(f"(b) L2 <= +2.05 fresh: {'HELD' if pb else 'FAILED'}")
