"""FRESH FRONTIER LADDER -- ledger 22 made the fresh window the quotable
one; the frontier graph needs every rung re-quoted there. Evaluate the
nested assembly ladder on the untouched validation half (FR-val):
  FRONT: a0, m0, a1v, m1, m2, m3, tail dictionary (the v4-recipe point)
  +MIDDLE: + CP-2304 mlp4-9 (the v7-recipe point)
  +TAILATT: + attention 10-17 dictionaries (the v8-recipe point)
  +MIDATT: + attention 2-9 dicts with validated means (the 34/36 point,
  = the validated config; its FR-val number +2.925 replicates by
  construction).
REGISTERED PREDICTIONS (fresh-window bars = window-C value + ~0.35):
  (a) FRONT <= +1.90; (b) +MIDDLE <= +2.30; (c) +TAILATT <= +2.70;
  (d) monotone increase along the ladder."""
...
OUT=PT+'fresh_frontier_results.json'
...
def main():
    ...
    def evalF(TOK,active,mode,clsT=None):
        ...
        for h in hs: h.remove()
        return float(torch.cat(ces).mean())
    FRv=FR[88:176]; clsV=clsFR[88:176]
    base_v=evalF(FRv,[],'oracle')
    FRONT=['a0','m0','a1v','m1','m2','m3','tail']
    MID=[f'c{li}' for li in MIDL]
    TATT=[f'a{li}' for li in range(10,18)]
    MATT=[f'a{li}' for li in range(2,10) if li!=8]
    val=[n for n in order if n!='a8']
    for nm in ('a5','a14','a16'):
        val=[(nm+'m' if n==nm else n) for n in val]
    ladder={}
    ladder['front']=evalF(FRv,FRONT,'oracle',clsV)-base_v
    ladder['front+middle']=evalF(FRv,FRONT+MID,'oracle',clsV)-base_v
    ladder['front+middle+tailatt']=evalF(FRv,FRONT+MID+TATT,'oracle',
                                         clsV)-base_v
    ladder['validated34']=evalF(FRv,val,'oracle',clsV)-base_v
    for k,v in ladder.items(): print(f'{k:24s} {v:+.4f}',flush=True)
    vals=list(ladder.values())
    pa=vals[0]<=1.90; pb=vals[1]<=2.30; pc=vals[2]<=2.70
    pd=all(vals[i]<=vals[i+1]+1e-9 for i in range(3))
    out={'base':round(base_v,4),
         'ladder':{k:round(v,4) for k,v in ladder.items()},
         'pred_a':bool(pa),'pred_b':bool(pb),'pred_c':bool(pc),
         'pred_d':bool(pd)}
    print(f"(a) front <= +1.90: {'HELD' if pa else 'FAILED'}")
    print(f"(b) +middle <= +2.30: {'HELD' if pb else 'FAILED'}")
    print(f"(c) +tailatt <= +2.70: {'HELD' if pc else 'FAILED'}")
    print(f"(d) monotone: {'HELD' if pd else 'FAILED'}")
