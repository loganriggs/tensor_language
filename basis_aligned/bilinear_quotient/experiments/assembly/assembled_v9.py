"""ASSEMBLED v9 -- INTO THE MIDDLE ATTENTION: v8 plus attention 2-9 as
class-dictionary rungs (constants at structure classes, per-class linear
reads elsewhere), fit sequentially in TRUE BLOCK ORDER (a2 before m2, a4
before c4, ...). If all rungs survive, every component of the model is
replaced except the pattern sides of attn0/attn1.
The induction band lives at attn3-5 (certified owners) and induction is
contextual transport that no same-position read can reproduce, so these
rungs should be the expensive ones.
REGISTERED PREDICTIONS:
  (a) a3, a4, a5 carry the three largest LOO marginals among a2-a9
      (mechanistic ordering claim from the induction ownership result);
  (b) >=3 of the 8 new rungs survive the drop rule (marginal <= +0.4);
  (c) v9-best oracle <= +3.2; deploy gap <= +0.15 (probe now sits at
      attn2's input, earlier and presumably less class-readable)."""
...
OUT=PT+'assembled_v9_results.json'
...
def main():
    ...
    base=evalC([],'oracle')
    full=evalC(order,'oracle')-base
    print(f'v9 full (oracle) {full:+.4f}',flush=True)
    marg={}
    for li in ATTM:
        nm=f'a{li}'
        marg[nm]=full-(evalC([n for n in order if n!=nm],'oracle')-base)
        print(f'LOO marginal {nm}: {marg[nm]:+.4f}',flush=True)
    drop=[nm for nm in marg if marg[nm]>0.4]
    best=[n for n in order if n not in drop]
    v9b=evalC(best,'oracle')-base if drop else full
    fullD=evalC(best,'probe')-base
    surv=8-len(drop)
    ranks=sorted(marg,key=lambda n:-marg[n])[:3]
    pa=set(ranks)=={'a3','a4','a5'}
    pb=surv>=3; pc=v9b<=3.2
    out={'base':round(base,4),'v9_full_oracle':round(full,4),
         'attm_marginals':{k:round(v,4) for k,v in marg.items()},
         'dropped':drop,'v9_best_oracle':round(v9b,4),
         'v9_best_deploy':round(fullD,4),
         'pred_a':bool(pa),'pred_b':bool(pb),'pred_c':bool(pc)}
    print(f'base {base:.3f} | v9 full {full:+.4f} | dropped {drop} | '
          f'best {v9b:+.4f} | deploy(best) {fullD:+.4f}')
    print(f"(a) top-3 marginals = a3,a4,a5 (got {ranks}): "
          f"{'HELD' if pa else 'FAILED'}")
    print(f"(b) >=3/8 survive ({surv}/8): {'HELD' if pb else 'FAILED'}")
    print(f"(c) v9-best <= +3.2: {'HELD' if pc else 'FAILED'}")
