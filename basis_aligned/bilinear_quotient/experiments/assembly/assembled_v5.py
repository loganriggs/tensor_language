"""ASSEMBLED v5 -- INTO THE MIDDLE: v4 plus mlp4-9 as firewall+absorber
rungs (sequential). The middle was the old flat track s incompressible
region; the grammar says try firewalled tables with absorbers there too.
REGISTERED: (a) v5 total <= +2.6 with 19/36 components replaced; (b) the
six middle components combined in-context marginal <= +1.2; (c) per-
component leave-one-out marginals reported; (d) PRE-REGISTERED ADAPTIVE
RULE: any middle component with LOO marginal > +0.4 is dropped and the
best-subset assembly re-evaluated (reported as v5-best).
...
OUT=PT+'assembled_v5_results.json'
...
def main():
    ...
    MIDS=['m4','m5','m6','m7','m8','m9']
    marg={}
    for nm in MIDS:
        marg[nm]=full-(evalC([n for n in order if n!=nm])-base)
        print(f'LOO marginal {nm}: {marg[nm]:+.4f}',flush=True)
    drop=[nm for nm in MIDS if marg[nm]>0.4]
    best=order; v5b=full
    if drop:
        best=[n for n in order if n not in drop]
        v5b=evalC(best)-base
        print(f'adaptive drop {drop} -> v5-best {v5b:+.4f}',flush=True)
    midsum=sum(marg.values())
    pa=full<=2.6; pb=midsum<=1.2
    out={'base':round(base,4),'assembled_v5':round(full,4),
         'middle_marginals':{k:round(v,4) for k,v in marg.items()},
         'dropped':drop,'v5_best':round(v5b,4),
         'pred_a':bool(pa),'pred_b':bool(pb)}
    print(f'base {base:.3f} | v5 full {full:+.4f} | middle sum '
          f'{midsum:+.4f} | v5-best {v5b:+.4f}')
    print(f"(a) v5 <=2.6: {'HELD' if pa else 'FAILED'}")
    print(f"(b) middle sum <=1.2: {'HELD' if pb else 'FAILED'}")
