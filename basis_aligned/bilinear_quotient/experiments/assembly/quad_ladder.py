"""QUAD-ABSORBER CAPACITY LADDER -- 263's QUAD arm (16 weight directions,
136 pairwise products) improved the v5 assembly from +2.095 to +1.993.
Which capacity is binding: the number of quadratic feature directions, or
the rank-32 residual basis P the absorber writes into? Three arms, same
sequential v5 pipeline, absorbers only:
  quad32    -- 32 weight directions (528 quad features), P rank 32
  quad48    -- 48 directions (1176 features), P rank 32
  quad32P64 -- 32 directions, P rank 64
REGISTERED PREDICTIONS:
  (a) quad32 <= +1.94 (more weight directions keep paying);
  (b) quad32P64 <= quad32 - 0.03 (the write-side rank binds too);
  (c) quad48 <= quad32 + 0.02 (no overfit blowup; gains saturating)."""
...
OUT=PT+'quad_ladder_results.json'
CA,CB=300,512; R0,R1=120,300
ARMS={'quad32':(32,32),'quad48':(48,32),'quad32P64':(32,64)}
...
def main():
    ...
    for li in range(0,10):
        ...
        _,_,Vh=torch.linalg.svd(
            ...
            full_matrices=False)
        vdir[li]=Vh[:48].T.contiguous()
    tri={r:torch.triu_indices(r,r) for r in (32,48)}
    def quadfeat(X,li,r):
        Z=X@vdir[li][:,:r]
        iu,il=tri[r]
    ...
    def build(arm):
            ...
            for nm in active:
                ...
                if kind=='cv':
                    ...
                    def h(mod,i_,o_,tb=tb,A=A,P=P,li=li):
                        ...
                        ft=quadfeat(x,li,rq)
                        new=(tb[cur['idx']].float().reshape(-1,D)
                             +(ft@A)@P.T)
        ...
        print(f'{arm:10s} total {tot:+.4f} | R2 {r2s}',flush=True)
        return tot,r2s
    out={}
    for arm in ARMS:
        tot,r2s=build(arm)
        out[arm]={'total':round(tot,4),'r2':r2s}
    pa=out['quad32']['total']<=1.94
    pb=out['quad32P64']['total']<=out['quad32']['total']-0.03
    pc=out['quad48']['total']<=out['quad32']['total']+0.02
    out['pred_a']=bool(pa); out['pred_b']=bool(pb); out['pred_c']=bool(pc)
    print(f"\n(a) quad32 <= +1.94: {'HELD' if pa else 'FAILED'}")
    print(f"(b) quad32P64 beats quad32 by >=0.03: "
          f"{'HELD' if pb else 'FAILED'}")
    print(f"(c) quad48 no blowup (<= quad32+0.02): "
