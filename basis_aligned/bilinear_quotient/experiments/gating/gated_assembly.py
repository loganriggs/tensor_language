"""GATED ASSEMBLY -- the census cashes into the benchmark (user
directive): per-position component gating. Base config = the empirical
high-coverage stack (28 comps + 38 heads, +2.674 fresh). Gate: compute
damage-mode scores causally on the fresh eval rows (108 probes,
loadings from the fit window -- oracle arm, stated), and at positions
belonging to any of the top-10 modes (|score| >= its fit-window 92nd
percentile), the CP middles and tail-attention dictionary components
RUN REAL -- targeted spending exactly where the census says the data
depends on real machinery.
REGISTERED PREDICTIONS (fresh rows):
  (a) gated total <= base config - 0.25 nats, at gated fraction <= 25%
      (fraction reported);
  (b) random-position gating at MATCHED fraction recovers <= 40% of the
      gated improvement;
  (c) per-arm numbers + gated fraction reported."""
...
OUT=PT+'gated_assembly_results.json'
...
def main():
    ...
    thr=scA.abs().quantile(0.92,dim=0)
    ...
    pa=gain>=0.25 and frac<=0.25
    pb=rgain<=0.4*max(gain,1e-4)
    out={'base':round(baseC2,4),'gated':round(gat,4),
         'random':round(rnd,4),'fraction':round(frac,4),
         'pred_a':bool(pa),'pred_b':bool(pb)}
    print(f"(a) gain >= 0.25 at frac <= 25%: {'HELD' if pa else 'FAILED'}")
    print(f"(b) random <= 40% of gain: {'HELD' if pb else 'FAILED'}")
