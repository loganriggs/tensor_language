"""DEPLOY-GATED v2 -- gate with the FULL converged compositional
program set (~50-64 circuits vs 14), all input-only: surface + class +
trigger-pair + shifted features, with circ_ predicates resolved by
fixed-point iteration over the program DAG.
REGISTERED PREDICTIONS (fresh rows):
  (a) gain >= 0.05 (vs 0.014 with 14 surface programs), random-matched
      <= 40% of gain;
  (b) efficiency >= 2.5x random (vs 1.5x)."""
...
OUT=PT+'deploy_gated2_results.json'
...
def main():
    ...
    import re as re9, ast as ast9
    progd={}
    try:
        d9=js9.load(open(PT+'compositional_ladder2_results.json'))
        for r9 in d9['programs']:
            if r9['bacc']>=0.75: progd[r9['tag']]=r9['program']
    except Exception: pass
    pat9=re9.compile(r"round \d+ PASS (r[\w.]+): [\d.]+ (\[\[.*\]\])")
    try:
        for line in open(PT+'runlogs/compositional_ladder2.log'):
            mm9=pat9.search(line)
            if mm9: progd[mm9.group(1)]=ast9.literal_eval(mm9.group(2))
    except Exception: pass
    progs=list(progd.values())
    print(f'{len(progs)} converged programs',flush=True)
    tok2d=FR[:,:256]
    nR,Tr=tok2d.shape
    ...
    # class labels (mechanical)
    from circuit_dictionary import CLS as CLS9
    CM=torch.zeros(nR,Tr,dtype=torch.long)
    for r_ in range(nR):
        tt_=FR[r_,:257].tolist()
        for pos in range(256):
            t=tt_[pos+1]; p=tt_[pos]
            tg=dec.get(int(tt_[pos+1]),enc9.decode([t]))
            tg=enc9.decode([t]); pv=enc9.decode([p]); st=tg.strip()
            if st.isdigit() and not tg.startswith(' '): k=0
            elif st in (')',']') and any(bch in enc9.decode(
                tt_[max(0,pos-60):pos+1]) for bch in ('(','[')): k=1
            ...
            CM[r_,pos]=k
    for k in range(10):
        F_[f'class_{CLS9[k]}']=(CM==k)
    # trigger-pair features from the induction-grade mechanisms
    for path,key in ((PT+'mechanism_bootstrap3_results.json','p'),
                     (PT+'mechanism_bootstrap5_results.json','a')):
        try: d8=js9.load(open(path))
        except Exception: continue
        for r8 in d8['results']:
            if not r8.get('pass'): continue
            prs=[tuple(t8.split('|',1)) for t8 in r8['trigger_top']
                 if '|' in t8]
            mm=torch.zeros(nR,Tr,dtype=torch.bool)
            for r_ in range(nR):
                row=FR[r_,:256].tolist()
                for t_ in range(1,Tr):
                    if any(enc9.decode([row[t_-1]])==a8 and
                           enc9.decode([row[t_]])==b8
                           for a8,b8 in prs):
                        mm[r_,t_]=True
            F_[f'trig{key}_'+r8['tag'].replace('.','_')]=mm
    base_names=list(F_)
    for nm8 in base_names:
        F_['prev1_'+nm8]=torch.roll(F_[nm8],1,dims=1)
        F_['prev2_'+nm8]=torch.roll(F_[nm8],2,dims=1)
    def run_prog(prog):
        mm=torch.zeros(nR,Tr,dtype=torch.bool)
        for conj in prog:
            ...
            mm|=cm
        return mm
    # fixed-point resolution of circ_ predicates
    for _ in range(8):
        for tg9,pg9 in progd.items():
            F_['circ_'+tg9.replace('.','_')]=run_prog(pg9)
    ...
    pa=gain>=0.05 and rgain<=0.4*max(gain,1e-4)
    pb=(gain/max(frac,1e-4))>=2.5*(rgain/max(frac,1e-4))
