"""GATED ASSEMBLY v2 -- tighter gates (328: census positions worth 3.1x
random per gated token; the 92nd-pct union gated 36% for +0.14). v2
gates at the 97th percentile per mode (union ~8-12%).
REGISTERED PREDICTIONS (fresh):
  (a) gain >= 0.12 at gated fraction <= 12%;
  (b) random gating at matched fraction <= 40% of the gain;
  (c) efficiency (gain per gated token) >= 2.5x random."""
...
OUT=PT+'gated_assembly2_results.json'
...
def main():
    ...
    pa=gain>=0.12 and frac<=0.12
    eff=(gain/max(frac,1e-4))/max(rgain/max(frac,1e-4),1e-4)
    ...
    print(f"(a) gain >= 0.12 at frac <= 12%: {'HELD' if pa else 'FAILED'}")
    print(f"(b) random <= 40% of gain: {'HELD' if pb else 'FAILED'}")
    print(f"(c) efficiency vs random: {eff:.1f}x")
