"""DEPLOY-GATED ASSEMBLY -- the fully input-only gating rung: gate
positions predicted by the 14 certified circuit PROGRAMS (329), all of
which use only base token predicates -- executable from raw tokens, no
oracle anywhere. At program-predicted positions the CP middles and
tail-attention dictionaries run real.
REGISTERED PREDICTIONS (fresh rows):
  (a) deploy-gated gain >= 0.03 at reported fraction, random-matched
      gate <= 40% of the gain;
  (b) efficiency (gain per gated token) >= 2x random."""
...
OUT=PT+'deploy_gated_results.json'
...
def main():
    ...
    # ---- program gate: base predicates on FR tokens ----
    import json as js9, tiktoken as tk9
    enc9=tk9.get_encoding('gpt2')
    progs=[r['program'] for r in
           js9.load(open(PT+'program_names_results.json'))['programs']
           if r['bacc']>=0.75]
    tok2d=FR[:,:256]
    nR,Tr=tok2d.shape
    flat=tok2d.reshape(-1)
    uniq=flat.unique()
    dec={int(u):enc9.decode([int(u)]) for u in uniq.tolist()}
    def sf(fn):
        vals={u:fn(dec[u]) for u in dec}
        return torch.tensor([vals[int(t)] for t in flat.tolist()])            .view(nR,Tr)
    NL=sf(lambda x: chr(10) in x)
    F_={'is_digit':sf(lambda x: x.strip().isdigit()),
        'is_punct':sf(lambda x: bool(x.strip()) and
                      not any(c.isalnum() for c in x.strip())),
        'upper_initial':sf(lambda x: x.startswith(' ') and
                           x.strip()[:1].isupper()),
        'mid_word':sf(lambda x: (not x.startswith(' ')) and
                      x.strip().isalpha()),
        'starts_space':sf(lambda x: x.startswith(' ')),
        'is_newline':NL,
        'prev_newline':torch.roll(NL,1,dims=1)}
    SEEN=torch.zeros(nR,Tr,dtype=torch.bool)
    DNL=torch.zeros(nR,Tr,dtype=torch.long)
    for r_ in range(nR):
        seen=set(); dn=99
        row=tok2d[r_].tolist()
        for t_ in range(Tr):
            ...
            seen.add(row[t_])
            if chr(10) in dec[row[t_]]: dn=0
            else: dn=min(dn+1,99)
            DNL[r_,t_]=dn
    F_['seen_before']=SEEN
    F_['dist_nl_le2']=DNL<=2
    F_['dist_nl_ge6']=DNL>=6
    def run_prog(prog):
        mm=torch.zeros(nR,Tr,dtype=torch.bool)
        for conj in prog:
            cm=torch.ones(nR,Tr,dtype=torch.bool)
            for pred in conj:
                neg=pred.startswith('NOT ')
                nm2=pred[4:] if neg else pred
                v=F_[nm2]
                cm&=(~v if neg else v)
            mm|=cm
        return mm
    gateF=torch.zeros(nR,Tr,dtype=torch.bool)
    for pg in progs: gateF|=run_prog(pg)
    frac=float(gateF.float().mean())
    print(f'deploy-program gated fraction: {frac:.2%}',flush=True)
    baseFr=torch.zeros(1)  # placeholder replaced below
    g8=torch.Generator().manual_seed(8)
    rmask=(torch.rand(nR,Tr,generator=g8)<frac)
    ...
    print(f'base config {baseC2:+.4f} | gated {gat:+.4f} (gain '
          f'{gain:+.4f}) | random-gated {rnd:+.4f} (gain {rgain:+.4f})')
    pa=gain>=0.03 and rgain<=0.4*max(gain,1e-4)
    pb=(gain/max(frac,1e-4))>=2*(rgain/max(frac,1e-4))
    out={'base':round(baseC2,4),'gated':round(gat,4),
         'random':round(rnd,4),'fraction':round(frac,4),
         'pred_a':bool(pa),'pred_b':bool(pb)}
    print(f"(a) deploy gain >= 0.03, random <= 40%: {'HELD' if pa else 'FAILED'}")
    print(f"(b) efficiency >= 2x random: {'HELD' if pb else 'FAILED'}")
