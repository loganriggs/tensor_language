...
#
# circuit_readouts: ARE THE COPY READOUT HEADS SHARED WITH CAPITALIZATION? (S1522:
# copy's late heads {16.3, 16.4, 17.1} overlap the capitalization committee.)
# Arms: rm {16.3, 16.4, 17.1} (copy readouts) vs rm {13.0, 13.5, 14.4, 14.6, 14.7,
# 15.3, 16.0, 16.5, 17.0, 17.2} (committee MINUS the readouts), scored on copy /
# induction / capitalized / global.
# Registered predictions:
#   pred_a the readouts damage BOTH copy (>= .06) and capitalized (>= .04) classes.
#   pred_b the committee-minus-readouts damages capitalized (>= .15) but copy <= .04
#          (division of labor, not one blob).
#   pred_c both arms' global rise <= .05.
...
OUT = PT + 'circuit_readouts_results.json'
NR = 960
...
def main():
    ...
    ENS = {'readouts': [(16, 3), (16, 4), (17, 1)],
           'committee_rest': [(13, 0), (13, 5), (14, 4), (14, 6), (14, 7),
                              (15, 3), (16, 0), (16, 5), (17, 0), (17, 2)]}

    def measure(hset):
        HSET['set'] = hset
        gs = 0.0; gn = 0
        cs = {k2: 0.0 for k2 in ('copy', 'induction', 'cap')}
        cn = {k2: 0 for k2 in ('copy', 'induction', 'cap')}
        for i in range(0, NR, 8):
            bb = EVR[i:i + 8].to(DEV)
            idx = bb[:, :-1].contiguous(); tg = bb[:, 1:].contiguous()
            mm = masks_for(idx, tg)
            mm['cap'] = CAPV[tg].clone()
            mm['cap'][:, :64] = False
    ...
    pa = rises['readouts']['copy'] >= 0.06 and rises['readouts']['cap'] >= 0.04
    pb = (rises['committee_rest']['cap'] >= 0.15
          and rises['committee_rest']['copy'] <= 0.04)
    pc = all(rises[nm]['global'] <= 0.05 for nm in rises)
    out = {'res': res, 'pred_a_readouts_shared': bool(pa),
           'pred_b_division_of_labor': bool(pb), 'pred_c_global_le_05': bool(pc),
