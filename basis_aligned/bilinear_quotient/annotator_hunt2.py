# annotator_hunt2: §1273's rerun with the share metric fixed (sum/count accumulation —
# v1 NaN'd on batches with zero close targets). Same design; predictions re-registered on
# the working metric:
#   pred_a FETCH-COLLAPSE NAMES THE ANNOTATOR: some band's opener-side ablation cuts 13.8's
#          delimiter-key share by >= 50%; its random control <= 20% of that drop.
#   pred_b THE ANNOTATOR IS SPECIFIC, NOT GENERIC: that band is NOT attn0-2 (whose §1273 CE
#          effect was mostly generic) — registered bet: mlp0-2 or attn3-8.
#   pred_c COHERENCE: the share-collapse winner = the specificity-adjusted CE winner
#          (open-minus-random excess).
...
OUT = PT + 'annotator_hunt2_results.json'
NR = 96
