# unified_kit_slim: SLIM THE ONE-MODEL BILL (§1403: 28 live heads serve 4 families at
# .64-.92 + else .81, co-residence synergistic). Drop-cost each of the 28 live heads from
# the unified ungated kit (one at a time -> 28 arms), then a SLIM arm dropping every head
# whose worst family cost is < 0.01 recovery. Scored on all four families + else.
# Assumptions registered: costs measured in recovery units per family; slim rule fixed
# in advance at worst-cost < 0.01; one-sided bars ONLY (§1403 lesson).
#
# Registered predictions (one-sided):
#   pred_a >= 5 of the 22 commons heads are droppable (worst family+else cost < 0.01).
#   pred_b every specialist survives: dropping 13.8 costs closer >= 0.10, dropping 10.5
#          costs question >= 0.10, dropping 8.1 costs comparative >= 0.10.
#   pred_c the slim kit holds: every family and else within 0.02 BELOW the full 28-head
#          kit (improvements pass).
...
OUT = PT + 'unified_kit_slim_results.json'
NMEAN = 24; NR = 1920
...
def main():
    ...
    for pair in LIVE:
        ...
        res[f'drop_{name}'] = {**{kk: round(v, 4) for kk, v in r.items()},
                               'else': round(e, 4)}
        cc = {}
        for f in MASKS:
            g = res['ymean'][f] - res['full'][f]
            cc[f] = round(ref[f] - (res['ymean'][f] - r[f]) / max(g, 1e-6), 4)
        ge = res['ymean']['else'] - res['full']['else']
        cc['else'] = round(ref['else'] - (res['ymean']['else'] - e) / max(ge, 1e-6), 4)
        costs[name] = cc
        print(f"drop {name}: worst {max(cc.values()):+.4f} ({cc})", flush=True)
        json.dump({'partial': True, 'costs': costs}, open(OUT, 'w'), indent=1)

    droppable = [n for n, cc in costs.items() if max(cc.values()) < 0.01]
    CUR['drop'] = {tuple(int(x) for x in n.split('.')) for n in droppable}
    r, e = ce_run2('kit', 'ungated')
    CUR['drop'] = set()
    slim = {**{f: rec('kit28', f) for f in ()}, }
    slim = {}
    for f in MASKS:
        g = res['ymean'][f] - res['full'][f]
        slim[f] = round((res['ymean'][f] - r[f]) / max(g, 1e-6), 4)
    ge = res['ymean']['else'] - res['full']['else']
    slim['else'] = round((res['ymean']['else'] - e) / max(ge, 1e-6), 4)
    n_slim = len(LIVE) - len(droppable)
    comm_dropped = [n for n in droppable
                    if tuple(int(x) for x in n.split('.')) in COMMONS]
    pa = len(comm_dropped) >= 5
    sp_cost = {'13.8': costs.get('13.8', {}).get('closer', 0),
               '10.5': costs.get('10.5', {}).get('question', 0),
               '8.1': costs.get('8.1', {}).get('comparative', 0)}
    pb = all(v >= 0.10 for v in sp_cost.values())
    pc = all(slim[k] >= ref[k] - 0.02 for k in slim)
    out = {'ref28': {k: round(v, 4) for k, v in ref.items()}, 'costs': costs,
           'droppable': droppable, 'n_slim': n_slim, 'slim_recovery': slim,
           'specialist_costs': sp_cost,
           'pred_a_5_commons_droppable': bool(pa), 'pred_b_specialists_survive': bool(pb),
           'pred_c_slim_holds': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"droppable {len(droppable)} ({droppable}) -> slim {n_slim} heads: {slim}")
