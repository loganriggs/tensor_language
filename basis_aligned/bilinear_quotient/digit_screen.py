# digit_screen: POOL CANDIDATE #5 at the §1302 standard. Atlas (§513): digit -> a8
# (+0.134 target vs +0.035 elsewhere, n=223) — and the a8-counting arc (backlog #3)
# independently predicts a8, making this a cross-method confirmation opportunity.
# Targets = next token decodes to pure digits (any length, optional leading space).
#
# Registered predictions (honest bars for a weaker-concentration candidate):
#   pred_a CONCENTRATION: a8 target damage >= 0.08 nats AND >= 3x elsewhere.
...
OUT = PT + 'digit_screen_results.json'
NMEAN = 24; NR = 1920
H = m.transformer.h
LAYERS = (8, 7, 9, 13, 5)
...
def main():
    ...
    for tok in range(50257):
        try:
            ...
            continue
        ds = d.strip()
        if ds.isdigit() and len(ds) >= 1:
            oq.add(tok)
    oq_ids = torch.tensor(sorted(oq))
    print(f"digit token ids: {len(oq)}", flush=True)
    ...
    pa = d13['target'] >= 0.08 and conc >= 3.0
