...
OUT = PT + 'bottomup_attn_all_results.json'
...
def main():
    ...
    out['window_recovery_by_layer'] = {str(L): out['layers'][str(L)]['recovery_window'] for L in LAYERS}
    out['front_local'] = round(float(np.mean([out['layers'][str(L)]['recovery_window'] for L in [0,1,2]])), 3)
    out['gatherers_L3_5'] = round(float(np.mean([out['layers'][str(L)]['recovery_window'] for L in [3,4,5]])), 3)
    out['middle_L6_14'] = round(float(np.mean([out['layers'][str(L)]['recovery_window'] for L in range(6,15)])), 3)
    out['late_L15_17'] = round(float(np.mean([out['layers'][str(L)]['recovery_window'] for L in [15,16,17]])), 3)
    out['pred_a_front_local_high_middle_low'] = bool(out['front_local'] > 0.6 and out['middle_L6_14'] < out['front_local'])
    out['runtime_s'] = round(time.time()-t0, 1)
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"window-recovery by band: front(0-2) {out['front_local']} | gatherers(3-5) {out['gatherers_L3_5']} | middle(6-14) {out['middle_L6_14']} | late(15-17) {out['late_L15_17']}", flush=True)
    print(f"pred_a front-local-high/middle-low {out['pred_a_front_local_high_middle_low']}", flush=True)
