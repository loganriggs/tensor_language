...
budget account is a property of the learned computation (not of the bilinear-MLP form), swiglu18 should show
the same signatures. Run on swiglu18: (1) the loss budget by position type (§879), (2) synthetic induction +
...
REGISTERED PREDICTIONS:
  (0) SANITY: swiglu18 runs and gives a sane overall CE (~3-4 nats);
  (a) ARCHITECTURE-INDEPENDENT: swiglu18 reproduces the budget SHAPE (first-mention dominates >60%, induction
  ...
  (b) if swiglu18 differs markedly (weak induction, different budget shape, or induction gated at a very
...
OUT = PT + 'cross_arch_swiglu_results.json'
...
def main():
    t0 = time.time()
    m, cfg = load_elriggs('swiglu18', device=DEV, dtype=torch.float32)
    ...
    out = {'model': 'swiglu18', 'overall_ce': round(float(tot_f.mean()), 3),
    ...
    print(f"swiglu18 overall CE {out['overall_ce']} = class {out['overall_class_ce']} + within {out['overall_within_ce']} (within frac {out['within_frac']})", flush=True)
