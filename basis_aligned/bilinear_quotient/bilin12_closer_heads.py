# bilin12_closer_heads: THE SHARPEST TRANSFER CLAIM (§1373). bilin12's a7 is the
# closer layer (+0.953, conc 21.1). Does it have a single OWNER head like bilin18's 13.8
# (96.5% solo)? §1340 pattern: solo + all-but-one over a7's 6 heads at bracket targets.
#
# Registered predictions:
#   pred_a AN OWNER EXISTS: top solo head >= 60% of the layer's target damage.
...
OUT = PT + 'bilin12_closer_heads_results.json'
L7 = 7
NH = 6
HD = 128
NMEAN = 24; NR = 1920
m12, cfg = load_elriggs('bilin12')
H = m12.transformer.h
...
def main():
    ...
    print(f"layer dmg {layer_dmg:+.4f} (vs §1373 +0.953)", flush=True)
    solo = {}; keep = {}
    for h in range(NH):
        r = ce_all({h})
