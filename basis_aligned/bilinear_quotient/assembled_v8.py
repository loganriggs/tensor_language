"""ASSEMBLED v8 -- MERGE THE WINNERS: front tables with quad32/P64
absorbers (265: write-rank binds, 32 weight-directions suffice), middle
mlp4-9 as weights-only CP k=2304 truncations (264: 19@+1.68), tail
attention 10-17 as class dictionaries (262), tail MLP dictionary as
always. 27/36 components.
REGISTERED PREDICTIONS:
  (a) v8 oracle total <= +2.20 (v6 was +2.548 with weaker middle/front);
  (b) deploy (probe-labeled) gap <= +0.10 over oracle;
  (c) all 8 attention rungs survive the pre-registered drop rule
...
OUT=PT+'assembled_v8_results.json'
...
def main():
    ...
    def fit_tableres(Y,X,ids,li):
        ...
        print(f'absorber m{li} (quad32/P64): R^2 {r2:.2f}',flush=True)
    ...
    print(f'v8 full (oracle) {full:+.4f}',flush=True)
    marg={}
    for li in ATT:
        nm=f'a{li}'
    ...
    v8b=evalC(best,'oracle')-base if drop else full
    fullD=evalC(order,'probe')-base
    surv=8-len(drop)
    pa=full<=2.20; pb=(fullD-full)<=0.10; pc=surv==8
    out={'base':round(base,4),'v8_full_oracle':round(full,4),
         'att_marginals':{k:round(v,4) for k,v in marg.items()},
         'dropped':drop,'v8_best_oracle':round(v8b,4),
         'v8_full_deploy':round(fullD,4),
         'pred_a':bool(pa),'pred_b':bool(pb),'pred_c':bool(pc)}
    print(f'base {base:.3f} | v8 full {full:+.4f} | dropped {drop} | '
          f'best {v8b:+.4f} | deploy {fullD:+.4f}')
    print(f"(a) v8 <= +2.20: {'HELD' if pa else 'FAILED'}")
    print(f"(b) deploy gap <= +0.10: {'HELD' if pb else 'FAILED'}")
    print(f"(c) 8/8 survive ({surv}/8): {'HELD' if pc else 'FAILED'}")
