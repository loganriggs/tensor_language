"""FRONT + TAIL IN-ASSEMBLY MARGINALS -- the module-relevance figure has
no in-assembly cost for the front MLPs (the five most important
components) or the tail span dictionaries, because those rungs were fit
but never individually priced. Measure LOO marginals in the current best
config (round-2 greedy winner: 34/36 at +2.749) for a0, m0, a1v, m1, m2,
m3 and each tail MLP rung 10-17.
REGISTERED PREDICTIONS:
  (a) m1's in-assembly marginal <= 0.35 (solo relevance 5.32 nats -- the
      table+absorber captures >=93% of the model's most important
      component);
  (b) the four front MLP marginals sum <= 0.8;
  (c) every tail rung marginal <= 0.15."""
...
OUT=PT+'front_tail_marginals_results.json'
...
def main():
    ...
    # c8 -> mean (round-2 winner): capture mlp8 mean under current config
    hs=install(best)
    acc=[torch.zeros(D,device=DEV),0]
    def capm(mod,i_,o_):
        acc[0]+=o_.detach().float().reshape(-1,D).sum(0); acc[1]+=1
    ...
    for i in range(CA,CB,4):
        ...
        m(cur['idx'], bb[:,1:].contiguous())
    for h in hs: h.remove()
    S['c8m']=('linear',8,torch.zeros(D,D,device=DEV),acc[0]/((CB-CA)*256))
    best=[('c8m' if n=='c8' else n) for n in best]
    def evalskip(active,skip=None):
        cur['tailskip']=skip
        r=evalC(active,'oracle')
        cur['tailskip']=None
        return r
    base=evalC([],'oracle')
    full=evalskip(best)-base
    print(f'best config full {full:+.4f}',flush=True)
    marg={}
    for nm in ('a0','m0','a1v','m1','m2','m3'):
        marg[nm]=full-(evalskip([n for n in best if n!=nm])-base)
        print(f'LOO {nm}: {marg[nm]:+.4f}',flush=True)
    for li in TAILC:
        marg[f't{li}']=full-(evalskip(best,skip=li)-base)
        print(f'LOO tail m{li}: {marg[f"t{li}"]:+.4f}',flush=True)
    fs=sum(marg[n] for n in ('m0','m1','m2','m3'))
    pa=marg['m1']<=0.35; pb=fs<=0.8
    pc=all(marg[f't{li}']<=0.15 for li in TAILC)
    out={'base':round(base,4),'full':round(full,4),
         'marginals':{k:round(v,4) for k,v in marg.items()},
         'pred_a':bool(pa),'pred_b':bool(pb),'pred_c':bool(pc)}
    print(f"(a) m1 <= 0.35: {'HELD' if pa else 'FAILED'}")
    print(f"(b) front MLP sum {fs:+.3f} <= 0.8: {'HELD' if pb else 'FAILED'}")
    print(f"(c) all tail <= 0.15: {'HELD' if pc else 'FAILED'}")
