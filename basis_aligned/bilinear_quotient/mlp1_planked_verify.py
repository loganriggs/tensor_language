# mlp1_planked_verify: VERIFY THE COMPOSED GLASS PLANK (frontier_seed carries an
# ESTIMATED tier2000+ridge entry at .956 flagged "verify M1" — estimates never stand,
# Invariant 1). Arms (held-out, frozen anchors): tier2000 table alone (ref .922);
# tier2000 + FULL residual ridge from [attn1, mlp0] (ridge REFIT on the tiered table's
# residual — sequential-refit discipline); tier2000 + rank-128 truncated ridge.
# Fit skip=80, EVAL skip=7000.
#
# Registered predictions:
#   pred_a tier2000 + full ridge >= .95 fid_opt (the estimate holds).
#   pred_b tier2000 + rank-128 ridge >= .94 at ~96 Mbit total.
#   pred_c the composed entry beats table-only by >= .025.
...
OUT = PT + 'mlp1_planked_verify_results.json'
NFIT = 960; NEV = 960
K = 64
H = m.transformer.h
...
def main():
    ...
    TIER = base64.clone(); keep = freq_order[:2000]; TIER[keep] = TOKTAB[keep]
    ...
    W128 = (Ur[:, :128] * Sr[:128]) @ Vtr[:128]
    MBITS = {'tier': (2000 * D + 50257 * 64 + 64 * D) * 16 / 1e6,
             'tier_ridge': (2000 * D + 50257 * 64 + 64 * D + 2 * D * D) * 16 / 1e6,
             'tier_ridge128': (2000 * D + 50257 * 64 + 64 * D + 128 * 3 * D) * 16 / 1e6}
    ...
    pa = fids['tier_ridge'] >= 0.95
    pb = fids['tier_ridge128'] >= 0.94
    pc = (fids['tier_ridge'] - fids['tier']) >= 0.025
    out = {'ce': res, 'fid_opt': fids, 'mbits': {k: round(v, 1) for k, v in MBITS.items()},
           'frozen_anchor': {'ce_mean': sw['ce_mean'], 'ce_opt': sw['ce_opt']},
           'pred_a_composed_95': bool(pa), 'pred_b_rank128_94': bool(pb),
           'pred_c_ridge_adds_025': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
