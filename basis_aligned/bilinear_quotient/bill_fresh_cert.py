# bill_fresh_cert: FRESH-WINDOW CERTIFICATION OF THE §1408 BILL (backlog rule: every
# new winner certifies on fresh rows). Identical config — 28-head unified ungated kit +
# keep7 mids {4,6,7,8,9,11,13} with gmean stand-ins refit on THESE rows' fit stream —
# evaluated on rows the bill has never touched (skip=7000).
#
# Registered predictions (one-sided):
#   pred_a comparative >= 0.95 on fresh rows.
#   pred_b every family within 0.04 BELOW its §1408 number (q .690 / comp .984 /
#          closer .864 / cap .797).
#   pred_c else within 0.03 of §1408's .750.
...
OUT = PT + 'bill_fresh_cert_results.json'
NFITT = 960
V = 50257
NMEAN = 24; NR = 1920
...
def main():
    ...
    ROWS = cl.fineweb_rows(NMEAN + NR, skip=7000)[:, :T + 1].contiguous()
    ...
    FITT = cl.fineweb_rows(NFITT, skip=9200)[:, :T + 1].contiguous()
    ...
    REF = {'question': 0.690, 'comparative': 0.984, 'closer_b': 0.864,
           'capitalized': 0.797}
    t7 = table['keep7']
    pa = t7['comparative'] >= 0.95
    pb = all(t7[f] >= REF[f] - 0.04 for f in REF)
    pc = t7['else'] >= 0.750 - 0.03
    out = {'ce': res, 'recovery_table': table, 'ref_1408': REF,
           'pred_a_comp_95': bool(pa), 'pred_b_within_04': bool(pb),
           'pred_c_else_03': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
