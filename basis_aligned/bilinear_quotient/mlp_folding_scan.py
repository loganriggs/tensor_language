"""FOLD scan across DEPTH's bilinear form onto layer-0's computed features (user's 2nd directive; gets SPECIFICS
...
OUT = PT + 'mlp_folding_scan_results.json'
SCAN_LAYERS = [0,1,2,3,4,5,15,16,17]
...
def capture(rows):
    ...
    Xs = {L: [] for L in SCAN_LAYERS}; Os = {L: [] for L in SCAN_LAYERS}; seqs = []
    hs = []
    ...
    return X, O, np.concatenate(seqs, 0)
...
def main():
    ...
    for L in SCAN_LAYERS:
        ...
        Utok = mean_subspace(Xl, toks, RTOK); Utokfine = orth_against(Utok, Uclass)
        Uprev = orth_against(mean_subspace(Xl, prev, RPREV), Uclass, Utok)
        Upos = mean_subspace(Xl, pos.astype(np.int64), RPOS)
        feats = {'class': Uclass, 'token_fine': Utokfine, 'prev': Uprev, 'pos': Upos}
        chance = {k: v.shape[1]/D for k, v in feats.items()}
        Ucls_out = mean_subspace(Ol, toks, NCLASS_DIR); unit_mag = (Ucls_out.T @ Dw).norm(dim=0)
        top = torch.topk(unit_mag, NUNIT).indices.tolist()
        Lk = Lw[top]; Rk = Rw[top]; Lk = Lk/(Lk.norm(dim=1,keepdim=True)+1e-9); Rk = Rk/(Rk.norm(dim=1,keepdim=True)+1e-9)
        row = {}
        for name, U in feats.items():
            lf = energy_frac(Lk, U); rf = energy_frac(Rk, U)
            row[name] = round((lf+rf)/2/max(chance[name],1e-9), 2)
        out['layers'][f'mlp{L}'] = row
        print(f"mlp{L}: class x{row['class']} | pos x{row['pos']} | token_fine x{row['token_fine']} | prev x{row['prev']}", flush=True)
