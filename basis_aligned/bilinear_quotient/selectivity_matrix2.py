# selectivity_matrix2: §1309 v2 — GOAL-2 certification with the two confounds fixed:
# (1) behaviour masks MUTUALLY EXCLUSIVE: copy-supported positions (IDENT or VAR) are
# subtracted from succ/than/delim/qmark sets, so foreign-circuit damage cannot enter
# through shared positions; (2) 1920 rows for the thin classes; (3) the control bar and
# off-diagonal bar are RELATIVE (<= 10% of the removing circuit's own diagonal), per the
# §1305 separation-ratio instrument note.
#
# Registered predictions:
#   pred_a DIAGONAL DOMINANCE: every circuit's own-behaviour damage >= 5x every
#          non-overlap cell.
#   pred_b OFF-DIAGONAL QUIET (relative): every non-overlap cell <= 10% of that row's
#          own-diagonal minimum.
#   pred_c CONTROL ROW FLAT (relative): control <= 10% of each column's diagonal.
...
OUT = PT + 'selectivity_matrix2_results.json'
NMEAN = 24; NR = 1920; W = 128
H = m.transformer.h
...
def main():
    ...
    COPY = IDENT | VAR
    SUCC &= ~COPY; THAN &= ~COPY; DELIM &= ~COPY; QMARK &= ~COPY
    ...
    for c in CIRCUITS:
        if c == 'control':
            continue
        own = OWN[c]
        diag_min[c] = min(MAT[c][b] for b in own)
        for b in [x for x in beh if x not in own]:
            if diag_min[c] < 5 * max(MAT[c][b], 1e-4):
                viol_a.append((c, b, MAT[c][b]))
            if MAT[c][b] > 0.10 * max(diag_min[c], 1e-4):
                viol_b.append((c, b, MAT[c][b]))
    pa = len(viol_a) == 0
    pb = len(viol_b) == 0
    coldiag = {'ident': MAT['matchers']['ident'], 'var': MAT['matchers']['var'],
               'succ': MAT['successor']['succ'], 'than': MAT['comparative']['than'],
               'delim': MAT['delimiter']['delim'], 'qmark': MAT['question']['qmark']}
    pc = all(MAT['control'][b] <= 0.10 * max(coldiag[b], 1e-4) for b in beh)
