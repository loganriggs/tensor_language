# separation_selection_mlp17: DOES §1633's CERTIFIED-vs-FRESH SEPARATION GAP
# REPLICATE AT THE SECOND CERTIFIED CELL?
#
# §1633 held the site fixed at mlp11 and found the certified class (question)
# separating 60/60 against a 20-seed null while five fresh classes separated
# 17-47/60. That is one site and one certified class. The house pattern is to
# second-class confirm (S1595, S1598, S1603), and the registry contains exactly one
# other certified slice cell: pronouns@mlp17, which §1629 measured at 60/60.
#
# This repeats §1633 there. The SAME five fresh classes are reused so that the class
# set is held constant and only the cell changes -- site 11 -> 17, rank 2 -> 8,
# TOP 4 -> 6, matching pronouns' published configuration. If the certified/fresh gap
# is a property of certification it should reappear; if it was a fact about mlp11 or
# about rank-2, it should not.
#
# Note the direction differs: at mlp11 the certified class sits far ABOVE its null,
# at mlp17 pronouns sits far BELOW its null (§1629). The separation count is
# computed in whichever direction each class actually points, so both are comparable.
#
# 20 independent random rank-8 bases (seeds 1729-1748) x 3 disjoint 160-row chunks
# = 60 trials per class, corrected quantity (forward stops at 17, upstream
# components, site-relative coefficients).
#
# Registered predictions:
#   pred_a NO FRESH CLASS AT mlp17 REACHES PERFECT SEPARATION: each of the five has
#          at least one of its 60 random trials beating its lambda arm.
#   pred_b THE CERTIFIED CELL IS CLEARLY ABOVE THEM: pronouns' separation count
#          exceeds the BEST fresh class's by at least 3 of 60.
#   pred_c FRESH CLASSES ARE BROADLY WEAK, NOT JUST THE BEST ONE: the MEDIAN fresh
#          separation is <= 47/60, the level of mlp11's BEST fresh class.
...
OUT = PT + 'separation_selection_mlp17_results.json'
NR = 960
...
CELLS = {
    ...
    'with':     {'site': 17, 'rank': 8, 'top': 6, 'certified': False, 'pat': r'^ with$'},
    'from':     {'site': 17, 'rank': 8, 'top': 6, 'certified': False, 'pat': r'^ from$'},
    'at':       {'site': 17, 'rank': 8, 'top': 6, 'certified': False, 'pat': r'^ at$'},
    'by':       {'site': 17, 'rank': 8, 'top': 6, 'certified': False, 'pat': r'^ by$'},
    'as':       {'site': 17, 'rank': 8, 'top': 6, 'certified': False, 'pat': r'^ as$'},
}
...
def main():
    ...
    seps = sorted(v['separation'] for v in fresh.values())
    median_fresh = seps[len(seps) // 2]
    ...
    print(f"  certified pronouns {cert['separation']}/{NT}  vs  best fresh {best_fresh}/{NT}"
          f"  -> margin {cert['separation']-best_fresh}  | median fresh {median_fresh}/{NT}",
          flush=True)
    print(f"  (§1633 at mlp11: certified 60/60, best fresh 47/60, margin 13)", flush=True)
    ...
    out = {'config': {'cells': {k: {kk: vv for kk, vv in v.items() if kk != 'pat'}
           ...
           'predictions': {'pred_a_no_fresh_class_perfect': bool(pa),
                           ...
                           'pred_c_median_fresh_le_47': bool(pc)},
           'median_fresh_separation': median_fresh,
