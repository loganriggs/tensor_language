...
OUT=PT+'assembled_dictionary_results.json'
...
def main():
    ...
    def pertok(parts):
        ...
        if 'm1' in parts:
            def h_m1(mod,i_,o_):
                return tab_m1[cur['idx']].to(o_.dtype)
    ...
    pa=comb<=1.5*ssum; pb=comb<=2.0
    pc=abs(solos['m1']-1.216)<=0.18 and abs(solos['a1v']-0.032)<=0.05
    ...
    print(f"(a) <=1.5x sum: {'HELD' if pa else 'FAILED'}")
    print(f"(b) assembled <=2.0 nats: {'HELD' if pb else 'FAILED'}")
