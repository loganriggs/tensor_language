# type_config_robustness: DOES THE CLASS-TYPE DEPENDENCE SURVIVE A DIFFERENT
# (rank, TOP)? Strengthening a weakness I declared myself.
#
# FINDINGS entry 12 marks the class-type depth result MEDIUM confidence with the
# reason stated plainly: "seven classes, one rank, one TOP". Every cell in
# §1635-§1637 was rank-2 TOP-4, so the whole type story could be an artifact of that
# one configuration. This re-runs the type-discriminating comparison at rank-8 TOP-6.
#
# One class per type, at the two sites where the type signature was clearest:
#   at      FUNCTION WORD    rank-2 TOP-4 gave mlp11 17/60, mlp17 51/60
#   period  PUNCTUATION      rank-2 TOP-4 gave mlp11 58/60, mlp17 54/60
#   cap     CAPITALISED      rank-2 TOP-4 gave mlp11 52/60, mlp17 59/60
#
# The type signature at rank-2 TOP-4 lives at mlp11, where the three types spread
# 17/52/58 -- a range of 41 of 60. At mlp17 they were 51/54/59, a range of only 8.
# So the registered claim is not merely "type matters" but "type matters AT mlp11 and
# much less at mlp17", and pred_c tests that contrast rather than just the levels.
#
# 20 independent random rank-8 bases (seeds 1729-1748) x 3 disjoint 160-row chunks
# = 60 trials per cell, 6 cells. No cross-site SHARE scored (component count grows
# with depth); only within-site separation counts. SITE remains one global.
#
# Registered predictions:
#   pred_a THE ORDERING SURVIVES: at rank-8 TOP-6, ` at` has the LOWEST separation of
#          the three classes at mlp11.
#   pred_b TYPE STILL DISCRIMINATES: the spread across the three types at mlp11 is
#          >= 15 of 60. (rank-2 TOP-4 gave 41.)
#   pred_c THE CONTRAST IS SITE-SPECIFIC: the spread across types at mlp17 is SMALLER
#          than the spread at mlp11. (rank-2 TOP-4 gave 8 vs 41.)
...
OUT = PT + 'type_config_robustness_results.json'
NR = 960
...
CLASS_PATS = {'at': r'^ at$', 'period': r'^\.$|^ \.$', 'cap': r'^ [A-Z][a-z]+$'}
SITES = [11, 17]
CELLS = {f'{c}@mlp{st}': {'site': st, 'rank': 8, 'top': 6, 'certified': False,
                          'pat': pat, 'cls': c}
...
def main():
    ...
    i11 = SITES.index(11); i17 = SITES.index(17)
    at11 = {c: by_cls[c]['sep'][i11] for c in by_cls}
    at17 = {c: by_cls[c]['sep'][i17] for c in by_cls}
    spread11 = max(at11.values()) - min(at11.values())
    spread17 = max(at17.values()) - min(at17.values())
    pa = at11['at'] == min(at11.values())
    pb = spread11 >= 15
    pc = spread17 < spread11

    print(f"\n  per-class profile, sites {SITES}, {N_RANDOM*CHUNKS} trials each:", flush=True)
    for c in CLASS_PATS:
        v = by_cls[c]
        seps = ' '.join(f'{x:2d}' for x in v['sep'])
        gps = ' '.join(f'{x:+.3f}' for x in v['gap'])
        print(f"    {c:7s} sep [{seps}]  gap [{gps}]", flush=True)
    print(f"\n  RANK-8 TOP-6 separation by class type:", flush=True)
    for c in by_cls:
        print(f"    {c:7s} mlp11 {at11[c]:2d}/60   mlp17 {at17[c]:2d}/60", flush=True)
    print(f"  spread across types: mlp11 {spread11}/60 | mlp17 {spread17}/60", flush=True)
    print(f"  rank-2 TOP-4 reference: mlp11 at 17 / cap 52 / period 58 (spread 41); "
          f"mlp17 at 51 / period 54 / cap 59 (spread 8)", flush=True)
    print(f"  ` at` lowest at mlp11: {pa}", flush=True)
    ...
    out = {'config': {'cells': {k: {kk: vv for kk, vv in v.items() if kk != 'pat'}
           ...
           'cells': out_cells,
           'predictions': {'pred_a_at_lowest_at_mlp11': bool(pa),
                           'pred_b_mlp11_type_spread_ge_15': bool(pb),
                           'pred_c_mlp17_spread_lt_mlp11': bool(pc)},
           'sep_at_mlp11': at11, 'sep_at_mlp17': at17,
           'spread_mlp11': spread11, 'spread_mlp17': spread17,
