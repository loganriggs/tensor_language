# late_mlp_site_synergy: DOES §1704's SYNERGY GENERALISE, OR IS IT THE FRONT BAND'S?
#
# §1704 found the four front MLPs strongly SYNERGISTIC inside the 36-site program: exempting them
# one at a time buys +4.97 points summed, exempting the band as a whole buys +7.53 -- 52% more.
# The reading was that leaving one site real buys little while its neighbours are still programs
# feeding it error, and that removing the whole band's cascade at once is worth more than the
# parts. If that is the mechanism, it should appear wherever a band is exempted, not only at the
# front.
#
# The LATE MLP band is the cheapest place to test it: two sites (mlp16, mlp17), so three compiles
# against §1703's already-measured band gain of +1.987. A middle band would be twelve sites and
# five hours.
#
# It is also the sharper test, because the mechanism makes a SIZE prediction as well as a sign
# one. A cascade argument says the excess should grow with the number of sites whose errors can
# compound -- four front sites offer six pairwise interactions, two late sites offer one. If the
# late band shows the same 52% excess, the cascade reading is wrong and something size-independent
# is going on.
#
# §1703 measured the late-MLP band gain at +1.987 with the SAME program, protocol and eval rows,
# so it is a direct comparator rather than a cross-protocol one.
#
# Registered predictions:
#   pred_a THE SYNERGY IS NOT SPECIFIC TO THE FRONT: the two site gains sum to LESS than §1703's
#          band gain of +1.987, i.e. the late band is super-additive too.
#   pred_b BUT IT IS SMALLER, as a cascade with fewer interacting sites should be: the late band's
#          excess (band minus sum-of-singles, as a fraction of the band) is below §1704's 34.0%
#          (2.56 of 7.53). If it matches or exceeds the front band's, the cascade reading fails.
#   pred_c CONTROLS: the no-exemption arm reproduces §1696's 55.04% within 0.5 points, the
#          baseline CE reproduces 3.29205 (§1695), and both site gains are positive.
#   pred_d SPREAD: the larger site gain's 95% interval excludes the smaller's, so the ordering is
...
OUT = PT + 'late_mlp_site_synergy_results.json'
...
S1696_BOTH = 0.5504
S1698_RANK8 = 0.5475
BANDS = {f'mlp{L}': ('mlp', range(L, L + 1)) for L in (16, 17)}
S1703_LATE_BAND_GAIN = 0.01987
S1704_FRONT_EXCESS_FRAC = 0.340   # (7.53 - 4.97) / 7.53
...
def main():
    ...
    print(f'LATE MLP SITE SYNERGY | exempt-one over mlp16-17, RECOMPILED | '
    ...
    excess = S1703_LATE_BAND_GAIN - total
    excess_frac = excess / S1703_LATE_BAND_GAIN if S1703_LATE_BAND_GAIN > 0 else float('nan')
    top = max(gains, key=lambda k: gains[k]); bot = min(gains, key=lambda k: gains[k])

    pa = total < S1703_LATE_BAND_GAIN
    pb = excess_frac < S1704_FRONT_EXCESS_FRAC
    ...
    for k in sorted(gains, key=lambda k: -gains[k]):
        print(f'    {k:11s} {gains[k]:+7.2%}  95% CI [{ci_g[k][0]:+.2%}, {ci_g[k][1]:+.2%}]',
              flush=True)
    print(f'  two site gains sum {total:+.2%} vs §1703 band {S1703_LATE_BAND_GAIN:+.2%} '
          f'-> late band super-additive too {pa}', flush=True)
    print(f'  excess {excess:+.2%} = {excess_frac:.1%} of the band, vs the front band\'s '
          f'{S1704_FRONT_EXCESS_FRAC:.1%} -> smaller, as a cascade predicts {pb}', flush=True)
    ...
    res = {'config': {'sites': ALL18, 'ridge': RIDGE,
                      ...
                      'why_late_band': 'two sites -- the cheapest place to test whether S1704 synergy '
                                       'generalises (3 compiles vs 12 sites / 5h for a middle band)',
                      's1703_late_band_gain': S1703_LATE_BAND_GAIN,
                      's1704_front_excess_frac': S1704_FRONT_EXCESS_FRAC,
                      'size_prediction': 'a cascade argument says the excess should grow with the number '
                                         'of interacting sites: 4 front sites offer 6 pairwise '
                                         'interactions, 2 late sites offer 1',
           ...
           'sum_of_site_gains': round(total, 5), 'excess': round(excess, 5),
           'excess_fraction_of_band': round(excess_frac, 5),
           'predictions': {'pred_a_late_band_superadditive_too': bool(pa),
                           'pred_b_excess_smaller_than_front': bool(pb),
