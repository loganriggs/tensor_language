...
REGISTERED PREDICTIONS:
  (0) SANITY: bilin12 runs and gives a sane overall CE (~3-4 nats);
  (a) ARCHITECTURE-INDEPENDENT: bilin12 reproduces the budget SHAPE (first-mention dominates >60%, induction
  ...
  (b) if bilin12 differs markedly (weak induction, different budget shape, or induction gated at a very
...
OUT = PT + 'cross_arch_bilin12_results.json'
...
def main():
    t0 = time.time()
    m, cfg = load_elriggs('bilin12', device=DEV, dtype=torch.float32)
    ...
    out = {'model': 'bilin12', 'overall_ce': round(float(tot_f.mean()), 3),
    ...
    print(f"bilin12 overall CE {out['overall_ce']} = class {out['overall_class_ce']} + within {out['overall_within_ce']} (within frac {out['within_frac']})", flush=True)
