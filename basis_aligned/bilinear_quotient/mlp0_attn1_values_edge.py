# mlp0_attn1_values_edge: COMPLETE BLOCK-1'S READ OF mlp0 (S1464: the PATTERN edge is
...
#   pred_a the values edge >= .10 CE (values usually carry more than patterns).
#   pred_b rank-32 >= .80 recovery.
#   pred_c rank-8 >= .50 recovery.
...
OUT = PT + 'mlp0_attn1_values_edge_results.json'
NR = 960
...
def fwd_arm(idx, deltas):
    ...
    for L, blk in enumerate(H):
        ...
        if L == 1 and deltas is not None:
            ...
            tril = torch.tril(torch.ones(T, T, device=DEV, dtype=torch.bool))
            pat = pat.masked_fill(~tril, 0.0)
            vpre = at.c_v(xin).float() - corr * (h0 @ deltas['v'].T)
...
def main():
    ...
    COMP = {'v': at1.c_v.weight.float().to(DEV) @ Wd0}
    ...
    out = {'ce': res, 'edge_gap_cut': round(gap, 4), 'recovery': recs,
           'pred_a_edge_ge_10': bool(pa), 'pred_b_rank32_80': bool(pb),
           'pred_c_rank8_50': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
