"""STACK WRITER DECOMP -- 414: mechanism inputs for the heads
that RESIST readable code. The 4-read frontier left ~40 heads whose
truncation is expensive; per the mechanism-first standard, ask what
their scores COMPARE (exact writer-pair decomposition, as 387/411)
rather than how many reads they need. Targets: the 8 costliest
heads under 4-read code: ['1.1', '12.6', '5.7', '1.8', '11.6', '10.5', '3.8', '9.8'].
REGISTERED PREDICTIONS:
  (a) DIFFUSE-INPUT hypothesis: these heads are code-resistant
      because their scores have MANY inputs -- top-5 writer-pair
      mass < 0.5 for >=6/8 (contrast: early induction 0.6+);
  (b) at least 2/8 still have a dominant pair that is m0|m0 (the
      universal substrate reaches even the diffuse heads);
  (c) dominant-pair table written for all 8 (the swarm's
      mechanism worklist)."""
...
OUT=PT+'stack_writer_decomp_results.json'
IND=[(1, 1), (12, 6), (5, 7), (1, 8), (11, 6), (10, 5), (3, 8), (9, 8)]
NR=16
...
def main():
    ...
    ndiff=sum(1 for k in outj if outj[k]['top5_share']<0.5)
    pa=ndiff>=6
    nm0=sum(1 for k in outj if outj[k]['dominant_pairs'] and
            outj[k]['dominant_pairs'][0][0]=='m0|m0')
    pb=nm0>=2
    out={'heads':outj,'n_diffuse':ndiff,'n_m0m0_dominant':nm0,
         'pred_a':bool(pa),'pred_b':bool(pb),'pred_c':True}
    print(f"(a) >=6/8 top5<0.5 (diffuse inputs): "
          f"{'HELD' if pa else 'FAILED'}")
    print(f"(b) >=2/8 m0|m0 dominant: {'HELD' if pb else 'FAILED'}"
          f" ({nm0})")
