# quote_screen: BEHAVIOUR-FIRST screen (SOP v4) for QUOTE-STATE TRACKING — predicting a
# closing quotation mark requires carrying "a quote is open" across a span. Target class:
# positions whose TRUE NEXT token is a quote token AND >= 1 quote token appears in the
# prior 64 positions (quote context exists). Controls (both mandatory per SOP): the same
# positions jittered ±3 (position-matched) and a fully random position set of equal size.
#
# Screen: mean-ablate each of the 36 components (attention/MLP x 18 layers; output replaced
# by its global mean vector, computed on separate rows) and rank by CONCENTRATION =
# damage(target) / damage(elsewhere) — never absolute damage (SOP: magnitude finds the
# front for every behaviour).
#
# Registered predictions:
#   pred_a A CARRIER EXISTS: some component reaches concentration >= 3 at the target with
#          its jitter concentration <= 1.5 (behaviour-specific, not position-generic).
#   pred_b IT IS ATTENTION IN L2-L8: quote state needs range; register the direction.
#   pred_c RANDOM CONTROL FLAT: the winner's random-set concentration <= 1.5.
...
OUT = PT + 'quote_screen_results.json'
NMEAN = 24; NR = 96
H = m.transformer.h
...
def main():
    ...
    for s in ['"', ' "', '."', ',"', '!"', '?"', '\u201c', '\u201d', ' \u201c', '\u201d.']:
        try:
            for t in enc.encode(s):
                dec = enc.decode([t])
                if '"' in dec or '\u201c' in dec or '\u201d' in dec:
                    qids.add(t)
        except Exception:
            pass
    qids = torch.tensor(sorted(qids))
    print(f"quote token ids: {qids.tolist()}", flush=True)
    ...
    tgt_all = EVR[:, 1:]
    isq = torch.isin(tgt_all, qids)
    ctx = torch.zeros_like(isq)
    tok_isq = torch.isin(EVR[:, :-1], qids)
    for w in range(1, 65):
