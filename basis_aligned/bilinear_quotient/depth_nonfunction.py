# depth_nonfunction: DOES THE mlp11 SEPARATION MINIMUM SURVIVE OUTSIDE FUNCTION WORDS?
#
# §1636 found the separation minimum at mlp11 in 4 of 4 classes and I flagged the
# limitation in the same section: ` at`, ` the`, ` and`, ` is` are ALL FUNCTION WORDS,
# so the minimum could be a property of function-word prediction rather than of the
# network. Testing my own stated limitation rather than leaving it as a caveat.
#
# Three classes that are NOT function words, chosen to be structurally different from
# each other as well as from the first four:
#   period    punctuation, sentence boundary
#   digit     numeric tokens
#   cap       capitalised words (proper nouns and sentence-initial content)
#
# Same five sites mlp5/8/11/14/17, rank-2 TOP-4, 20 independent bases x 3 disjoint
# 160-row chunks = 60 trials per cell, 15 cells. As in §1635/§1636 no cross-site SHARE
# is scored, because the upstream component count grows 12 -> 36 with depth and a
# TOP-4 share shrinks mechanically; only within-site separation counts and gaps are
# scored. SITE remains one global driving forward stop, component set and coefficients.
#
# Prior data the bars are set from: function-word separations at mlp11 were 17 (at),
# 24 (the), 33 (is), 45 (and) -- the highest being 45/60.
#
# Registered predictions:
#   pred_a THE MINIMUM GENERALISES BEYOND FUNCTION WORDS: at least 2 of the 3 classes
#          have their LOWEST separation of the five sites at mlp11.
#   pred_b NON-MONOTONICITY IS GENERAL: none of the 3 shows a monotone profile.
#   pred_c mlp11 IS LOW IN ABSOLUTE TERMS, not merely lowest relatively: every one of
#          the 3 classes has mlp11 separation <= 45/60, the weakest function-word
#          result. (This can fail even if pred_a passes -- a class could bottom out at
#          mlp11 while still separating strongly there.)
...
OUT = PT + 'depth_nonfunction_results.json'
NR = 960
...
CLASS_PATS = {'period': r'^\.$|^ \.$', 'digit': r'^ ?[0-9]+$',
              'cap': r'^ [A-Z][a-z]+$'}
SITES = [5, 8, 11, 14, 17]
...
def main():
    ...
    n_low_at_11 = sum(1 for c in by_cls if by_cls[c]['sep'][mlp11_i] <= 45)
    pa = n_min_at_11 >= 2
    pb = n_monotone == 0
    pc = n_low_at_11 == 3
    ...
    print(f"  §1636 FUNCTION-WORD reference at mlp11: at 17, the 24, is 33, and 45 "
          f"(all min@11)", flush=True)
    print(f"  min at mlp11: {n_min_at_11}/3 | monotone: {n_monotone}/3 | "
          f"mlp11 sep <= 45: {n_low_at_11}/3 | positive gap at mlp14: {n_pos_at_14}/3",
          flush=True)
    ...
    out = {'config': {'cells': {k: {kk: vv for kk, vv in v.items() if kk != 'pat'}
           ...
           'predictions': {'pred_a_min_at_mlp11_ge_2of3': bool(pa),
                           'pred_b_no_monotone_profile': bool(pb),
                           'pred_c_mlp11_sep_le_45_all3': bool(pc)},
           'n_low_at_mlp11': n_low_at_11,
