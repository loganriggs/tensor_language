# mlp9_ladder: THE LADDER WALKED TO mlp9 (board #5: .050 CE unexplained, NO stand-in
# yet). S1451 showed the local chain WEAKENS at mlp7 (lin2 .43/linall .47/quad .52),
# so bars are calibrated DOWN from the mid-template. Arms for mlp9's output (everything
# else live, held-out): lin2 = ridge from [attn9, mlp8]; linall = ridge from
# [attn9, mlp0..mlp8]; linall + r=256/F=8192 sampled-pair quadratic over [attn9; mlp8].
# Fit skip=80, EVAL skip=7000. Frozen anchors from the 198-sweep score fid_opt too.
#
# Registered predictions (calibrated to the S1451 mlp7 outcome):
#   pred_a lin2 [attn9, mlp8] >= .40 of mlp9's held-out stake.
#   pred_b linall >= .50.
#   pred_c the quadratic adds >= .04 over linall.
...
OUT = PT + 'mlp9_ladder_results.json'
NFIT = 960; NEV = 960
H = m.transformer.h
STAND = {'mode': None, 'tensor': None}
CAP = {'on': False, 'store': None}
NAMES = ('m0', 'm1', 'm2', 'm3', 'm4', 'm5', 'm6', 'm7', 'm8', 'm9', 'a9')
...
def main():
    ...
    hooks = [H[L].mlp.register_forward_hook(cap_hook_for(f'm{L}')) for L in range(10)]
    hooks.append(H[9].attn.c_proj.register_forward_hook(cap_hook_for('a9')))
    FT = capture(FITR)
    print("fit capture done", flush=True)

    MLPS = ['m0', 'm1', 'm2', 'm3', 'm4', 'm5', 'm6', 'm7', 'm8']
    X2f = torch.cat([FT['a9'], FT['m8']], -1).reshape(-1, 2 * D)
    XAf = torch.cat([FT['a9']] + [FT[n] for n in MLPS], -1).reshape(-1, 10 * D)
    ...
    def fit_quad():
        ...
        NPOS = FT['m9'].shape[0] * T
        CH = 16384
        Aa = FT['a9'].reshape(-1, D); Mm = FT['m8'].reshape(-1, D)
    ...
    mh = H[9].mlp.register_forward_hook(stand_hook)
    ...
    def ce_run(mode):
        ...
        for i in range(0, NEV, 8):
            ...
            if mode is None:
                ...
                B = idx.shape[0]
                XeA = torch.cat([E['a9']] + [E[n] for n in MLPS],
                                -1).reshape(-1, 10 * D).to(DEV)
                linAp = ymA + (XeA - xmA) @ WA
                if mode == 'mean':
                    st = gmean.expand(B, T, D)
                elif mode == 'lin2':
                    Xe2 = torch.cat([E['a9'], E['m8']], -1).reshape(-1, 2 * D).to(DEV)
                    st = (ym2 + (Xe2 - xm2) @ W2).view(B, T, D)
                elif mode == 'linall':
                    st = linAp.view(B, T, D)
                else:
                    Wq, qm, rm = QFIT
                    q = quad_feats(E['a9'].reshape(-1, D), E['m8'].reshape(-1, D))
    ...
    sw = json.load(open(PT + 'optimal_ablation_all_results.json'))['results']['mlp9']
    ...
    out = {'ce': res, 'stake': round(stake, 4),
           ...
           'pred_c_quad_adds_04': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
