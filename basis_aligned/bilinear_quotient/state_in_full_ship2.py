# state_in_full_ship2: THIRD AND FOURTH BEHAVIORS FOR THE CERTIFICATION SET —
# contextual COPYING and INDUCTION under the total-glass ship. Metric: the
# copy/induction ADVANTAGE (how much easier class positions are than the global
# average: global CE minus class CE), clean vs full ship — does the replaced model
# still exploit context repetition?
...
#   pred_a the copy advantage retains >= .60 of its clean value under the ship.
#   pred_b the induction advantage retains >= .60.
#   pred_c global sanity within .05 of 3.8431.
...
OUT = PT + 'state_in_full_ship2_results.json'
NR = 960
...
def main():
    ...
    def class_masks(idx, tg):
        ...
        copy[:, :64] = False; induc[:, :64] = False
        return copy, induc

    def metric(layers, mlps):
        gs = 0.0; gn = 0
        cs = 0.0; cn_ = 0; is_ = 0.0; in_ = 0
        for i in range(0, NR, 8):
            bb = EVR[i:i + 8].to(DEV)
            idx = bb[:, :-1].contiguous(); tg = bb[:, 1:].contiguous()
            lo = fwd_arm(idx, layers, TWALL, mlps).float()
            ce = F.cross_entropy(lo.reshape(-1, lo.shape[-1]), tg.reshape(-1),
                                 reduction='none').view(tg.shape)
            mk = torch.ones_like(tg, dtype=torch.bool); mk[:, :64] = False
            cm, im = class_masks(idx, tg)
            gs += float(ce[mk & ~cm].sum()); gn += int((mk & ~cm).sum())
            cs += float(ce[cm].sum()); cn_ += int(cm.sum())
            is_ += float(ce[im].sum()); in_ += int(im.sum())
        g = gs / max(gn, 1)
        return (g - cs / max(cn_, 1), g - is_ / max(in_, 1), g)

    adv_c0, adv_i0, g0 = metric(frozenset(), frozenset())
    ALLM = frozenset(range(18))
    adv_c1, adv_i1, g1 = metric(ALLL, ALLM)
    pa = adv_c1 >= 0.60 * adv_c0
    pb = adv_i1 >= 0.60 * adv_i0
    pc = abs(g1 - 3.8431) <= 0.06
    out = {'clean': {'copy_adv': round(adv_c0, 4), 'induction_adv': round(adv_i0, 4),
                     'global': round(g0, 4)},
           'full_ship': {'copy_adv': round(adv_c1, 4),
                         'induction_adv': round(adv_i1, 4), 'global': round(g1, 4)},
           'retention': {'copy': round(adv_c1 / max(adv_c0, 1e-6), 3),
                         'induction': round(adv_i1 / max(adv_i0, 1e-6), 3)},
           'pred_a_copy_60': bool(pa), 'pred_b_induction_60': bool(pb),
