"""NUMBER WORD VERIFY -- causally test the number-word cluster (the
cleanest CURRENT-TOKEN-driven circuit, 604) and ground its triggers to
the current token, giving a token-driven counterpart to the context-
driven article circuit.

600 found, in mlp0 ranks 300-600, a 37-unit cluster firing positive
before spelled-out numbers (six/seven/nine/Five) and negative before
single-character tokens (C/2). 604 placed it at the current-token end
of the lexical-contextual spectrum (attn0-fold correlation 0.802 --
it barely needs prior context). This gives it (1) the validated 592
patching causal test -- does it push toward number words -- adding to
the correct/reversed tally (3 correct, 2 reversed so far), and (2) a
current-token trigger census -- which current tokens make it fire,
grounding it to the token level (its clean input side, no context
confound).

Margin = P(spelled number: six/seven/eight/nine/ten/two/three/four/
five/one) - P(single-char control: C/2). SOURCE = number-favoring
positions, TARGET = control-favoring positions.

REGISTERED PREDICTIONS:
  (0) REPRODUCIBILITY + IDENTITY: reclustering mlp0 ranks 300-600
      reproduces 600's sizes; the number-word cluster is the 3rd
      largest (37 units); self-patch changes nothing -- VOIDS on
      failure;
  (a) CAUSAL SIGN (adds to the tally): patching a number-favoring
      source into a control target moves the margin toward number
      words (delta > 0 = correct-signed, joining the article
      clusters) or away (< 0 = reversed, joining newline/aux). No
      pre-set pass -- the sign is a data point on the correct-vs-
      reversed pattern;
  (b) SPECIFICITY: the cluster's patch effect exceeds a size-matched
      random-unit patch (|ratio| >= 3) -- the effect is real and
      specific whichever sign it has;
  (c) CURRENT-TOKEN TRIGGERS (the grounding, no bar): at number-
      target positions, report which CURRENT tokens have the highest
      cluster firing -- the token-level triggers, expected to be
      number-adjacent tokens (digits, other number words, quantity
      words) consistent with 604's current-token-driven finding;
  NULL: the same source patch at an unrelated class (article-target
      positions) produces a shift under 30% of the number-target
      effect -- number-specific."""
...
OUT = PT + 'number_word_verify_results.json'
NFRESH = 64
NSAMP = 4000
TOPK = 300
NCLUST = 20
NPAIRS = 80
NUM_TOK = [2237, 3598, 3624, 5193, 3478, 734, 1115, 1440, 1936, 530]
CTRL_TOK = [327, 362]  # ' C', ' 2'
...
def recover_number_cluster(fresh):
    ...
    return ranked[2][1], sizes == expect, sizes  # 3rd largest = number-word (37)
