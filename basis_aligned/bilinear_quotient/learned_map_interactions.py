"""[INTERACTIONS: add pairwise products of named-variable projections (token-subspace x topic, prev x topic, token x prev) to the feature — a BILINEAR map of NAMED variables — testing whether the middle nonlinearity (~0.56, §910) is interactions of the variables we named vs unnamed structure] CLEAN (non-leaky) middle-frontier test: is the middle a smooth function of UPSTREAM content — what it READS
...
OUT = PT + 'learned_map_interactions_results.json'
NEVAL = 200; SEQ = 256; RPCA = 64; RIDGE = 1e2
...
def main():
    ...
    for (L, k) in COMPS:
        ...
        cph = (Xin - gci) @ Vp                     # content proj (RPCA)
        tk = wte_cur @ Vp[:min(Vp.shape[0], D)] if False else wte_cur  # keep token emb
        # low-rank projections for interactions (reuse content basis Vp for a shared low-rank space)
        tkl = wte_cur @ Vp; pvl = wte_prev @ Vp    # token/prev in the content low-rank space (RPCA each)
        NI = 16
        inter = torch.cat([tkl[:, :NI]*cph[:, :NI], pvl[:, :NI]*cph[:, :NI], tkl[:, :NI]*pvl[:, :NI]], 1)  # 3*NI cross terms
        feat = torch.cat([wte_cur, wte_prev, cph, inter, ones], 1); featperm = feat[permidx]
