# comparative_screen: BEHAVIOUR-FIRST screen for a carried SYNTACTIC EXPECTATION at mid
...
OUT = PT + 'comparative_screen_results.json'
NMEAN = 24; NR = 192
H = m.transformer.h
...
def main():
    ...
    pb = win.startswith('attn') and 4 <= Lw <= 9
    pc = wr['conc_rand'] <= 1.5
    out = {'n_targets': ntar, 'base': {k: round(v, 4) for k, v in base.items()},
           ...
           'pred_a_carrier': bool(pa), 'pred_b_mid_attn': bool(pb), 'pred_c_rand_flat': bool(pc),
           'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"top5 {out['top5']}")
    print(f"pred_a carrier {pa} | pred_b mid-attn {pb} | pred_c rand {pc}")
