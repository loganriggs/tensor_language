"""HANDOFF 0 DISSECTION -- completes the junction trilogy. Block 5 is a
narrow additive channel; block 1 is a coherence junction (partial cuts
worse than full removal). Which type is block 0 (+0.96)? Note block 0's
attention input is exactly rms(wte) -- a pure token function -- so its
handoff to mlp0 is token-conditioned by construction.
REGISTERED PREDICTIONS: (a) top-16 >= 60% of the full cut; (b)
random-16 <= 20%; (c) ladder shape reported -- monotone-rising = channel
type, inverted = coherence type (typology assignment, either HELD)."""
...
OUT=PT+'handoff0_dissect_results.json'
CA=300; R0,R1=120,300; LI=0
