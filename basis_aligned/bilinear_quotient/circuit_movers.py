...
#
# circuit_movers: CHARACTERIZE THE CONTEXT-MATCHING MOVERS {5.5, 10.8} (S1521-23:
# shared by induction and content-copying, ~30% of the damage). Arms: each mover
# alone, the pair, scored on copy + induction + capitalized classes and global.
# Registered predictions:
#   pred_a the pair's copy-class rise >= 1.5x the max single (complementary, not
#          redundant backups).
#   pred_b the pair hits contextual classes >= 5x harder than the capitalized
#          token class (they are context machinery, not output machinery).
#   pred_c pair global rise <= .01.
...
OUT = PT + 'circuit_movers_results.json'
NR = 960
...
def main():
    ...
    ENS = {'m55': [(5, 5)], 'm108': [(10, 8)], 'pair': [(5, 5), (10, 8)]}
    ...
    pa = rises['pair']['copy'] >= 1.5 * max(rises['m55']['copy'],
                                            rises['m108']['copy'])
    ctx = max(rises['pair']['copy'], rises['pair']['induction'])
    pb = ctx >= 5 * max(rises['pair']['cap'], 1e-6)
    pc = rises['pair']['global'] <= 0.01
    out = {'res': res, 'pred_a_complementary_15': bool(pa),
           'pred_b_context_5x_cap': bool(pb), 'pred_c_global_le_01': bool(pc),
