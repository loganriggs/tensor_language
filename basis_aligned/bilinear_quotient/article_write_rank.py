"""ARTICLE WRITE RANK -- the article/newline decisions are diffuse over
mlp0's 4608 UNITS (610/611), but units are a privileged basis. Is
mlp0's article WRITE low-RANK in its 1152-dim output space -- i.e.
localizable in DIRECTIONS even though not in units? RSPD showed attn0's
write is rank-16 despite being unit-diffuse (589); this asks the same
of mlp0's article contribution.
...
OUT = PT + 'article_write_rank_results.json'
NFRESH = 64
...
def main():
    ...
    p0 = abs(rank0 - 0.015) < 0.01 and abs(full) < 1e-3
    print(f'baseline {base:.4f} | rank-0 (mean-fill) {rank0:+.4f} '
          f'(608 +0.015) | full-rank {full:+.5f}: '
