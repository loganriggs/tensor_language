"""[DRAW-ROBUSTNESS re-certification, §1013] Re-run the fresh held-out whole-model understanding benchmark on a DIFFERENT fineweb draw (offset past the first 400 rows) to certify the headline "how much of the whole model do we understand" number is DRAW-STABLE, not first-draw optimism. Same protocol as the fresh version. REGISTERED: the token+topic+prev understanding_frac and genuine-vs-shuffled are within ~+-0.05 of the original fresh run (draw-stable). [orig §900-fresh] CAPSTONE: HOW MUCH OF THE WHOLE MODEL DO WE UNDERSTAND? (user's core question). Tables/subspaces BUILT on the first 70% of rows, CE EVALUATED on the held-out 30% — certifies the 0.81 is not same-data optimism. Replace EVERY component's
...
OUT = PT + 'whole_model_understanding_recert_results.json'
NEVAL = 200; SEQ = 256; CONTENT_L = 15; K = 12; RTOK = 64; RPOS = 32; OFFSET = 400
NLAYER = 18
...
@torch.no_grad()
def main():
    t0 = time.time(); cl.use_state(PT + 'census_state_diverse.pt'); rows = cl.fineweb_rows(OFFSET + NEVAL)[OFFSET:]
    ...
    print(f"\nWHOLE MODEL (all 36 components replaced): full CE {ce_full:.3f}, all-mean {ce_mean:.3f}", flush=True)
    print(f"token+topic+prev understanding {out['levels']['token+topic+prev']['understanding_frac']} (genuine vs shuffled {g_frac:.3f})", flush=True)
    print(f"(a) named variables reconstruct most of the model: {out['pred_a_named_vars_reconstruct_most']}", flush=True)
