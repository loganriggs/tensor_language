...
| PATH-SET2-001 | Regional UK/US output contrast backward through MLP17 to earlier attention9 and MLP16 | Exact residual propagation followed by MLP17, attention-head, and QK source cross terms | Head9.8's three late-touching QK1 carry blocks replay with `.1119` selected-row and `.1385` fresh-row error; recursive removal is material (`.528–.598` of full-head effect). Its 18-term downstream response is led by direct propagated attention9 (`.5422`) and attention17 (`.2495`), while MLP17 is anti-aligned (`−.1378`). The complete pre-RMS numerator predicts the logit effect at cosine `.9990`; the earlier MLP16×MLP17 fold alone points oppositely | Preserve the causal QK1 group but reject the MLP16×MLP17 suffix as its explanation. On the next folding hour, split the direct propagated term and induced attention17 response by head/QK overlap; keep QK2/value branches distinct |
