...
| PATH-SET2-001 | Selected outputs backward through MLP17 to head17.2 and its retained three-contraction producer | Both QK factors, current/inherited values, shared raw-state corners, MLP17 mixed terms | Exact MLP16 × head17.2 selected-reader fold replays at `2.49e-14`, but is only `1.1715%` of full reader RMS and fails its fixed 2% live-term gate; both orderings are present | Do not promote this pair causally. At a later `WEIGHT_FOLDING` hour, census the other exact source pairs or let circuit evidence select a different endpoint/term |
