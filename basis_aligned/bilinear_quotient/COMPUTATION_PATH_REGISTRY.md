...
|---|---|---|---|---|
| PATH-SUBJECT-001 | MLP8 input/context forward to L11H3 subject-number write and its downstream readers | Native-axis coordinate multiplied by the head-local response to an MLP6/7 number-source switch | Circuit record `grammatical_subject_number.v33`: exact `[1,z,s,zs]` reaches `.3769` cross-construction relative L2. A two-vector donor-free MLP6/7 prototype predicts $s$ at `.2880` error but the composed coefficient degrades to `.5159`, missing all frozen program gates | Preserve the transferable response scalar, close the two-vector complete program, and test a vector-valued recipient-side predictive state. Do not grow lexical/background prototype tables; require fresh causal substitution after discovery |
