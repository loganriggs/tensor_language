...
|---|---|---|---|---|
| PATH-SUBJECT-001 | MLP8 input/context forward to L11H3 subject-number write and its downstream readers | Native-axis coordinate and upstream causal-response or predictive-state coordinates | Circuit record `grammatical_subject_number.v31`: four-scalar coefficient law and native weight axis held causally. Native head coordinates fail at `.5876` relative L2; adding the E/A/U/W MLP8-input secant interaction improves `.6170→.5531`, below the fixed `.10` gain and `.40` error gates; quadratic context is `.5499` | Test an independently defined upstream causal-response or predictive-state coordinate. Do not widen the base-head singular or E/A/U/W secant families, fit behavioral outcomes, or use quantization |
| PATH-SET2-001 | Regional UK/US output contrast backward through MLP17 to earlier attention9 and MLP16 | Exact residual propagation followed by MLP17, attention-head, and QK source cross terms | Task-matched `ep` is `.6184` of cue-change norm. Attention9 × MLP16 leads the 34-source fold (`.26094`), and head9.8 supplies `.9776` of its aligned change. QK1 is carry8×carry8 dominated (`.8588`; top-three replay error `.1964`). QK2 is mixed among carry, MLP8, and attention8 terms (top-three error `.3353`); attention8-containing aggregates are `.2048/.3642` in QK1/QK2. The broader upstream path remains distributed | On the next `WEIGHT_FOLDING` hour, split carry8 into embedding plus layer0–7 propagated writes inside the leading QK1 carry×carry term. Keep QK2's attention8 key interaction and the established head8.2 value route distinct |
