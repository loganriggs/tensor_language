...
|---|---|---|---|---|
| PATH-SUBJECT-001 | MLP8 input/context forward to L11H3 subject-number write and its downstream readers | Native-axis coordinate multiplied by the head-local response to an MLP6/7 number-source switch | Circuit record `grammatical_subject_number.v32`: the native coordinate alone has `.6093` cross-construction relative L2; response-only and additive forms miss at `.4449/.4515`, but `[1,z,s,zs]` reaches `.3769`, improving by `.2324` and passing the frozen gates | Freeze the selected interaction and predict $s$ without the opposite-number donor from recipient-side upstream state; then test fresh-authority causal substitution before claiming a native generator |
