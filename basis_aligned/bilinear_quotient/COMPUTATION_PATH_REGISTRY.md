...
| PATH-SET2-001 | Regional UK/US output contrast backward through MLP17 to late residual sources | MLP17 source self/cross terms: earlier residual `e`, scaled MLP16 `p`, other attention17 `o`, and head17.2 `a` | Task-matched exact screen: `ep .6184` is top, `ee .3978`, `ea .2655`, `pa .1272`, `pp .06872`, `eo .06026`; head17.2-containing aggregate `.3153`. All four families retain `ep >=.531`. Generic grammatical endpoints had a different ranking, proving state specificity | Fold `e` backward into propagated embedding/attention/MLP sources and rank their exact cross terms with `p`; later test the leading source term selectively. Preserve `ea` as a second branch |
