...
| PATH-SET2-001 | Regional UK/US output contrast backward through MLP17 to earlier attention9 and MLP16 | Exact residual propagation followed by MLP17, attention-head, and QK source cross terms | Task-matched `ep` is `.6184`; attention9 leads its 34-source fold (`.2609`), head9.8 supplies `.9776` aligned change, and QK1 carry×carry is `.8588`. Its 289 embedding/layer0–7 pairs close exactly but top ten leave `.6674` error. Cross-source mass is `.8735`; nine of the top ten involve attention5 with MLP5/6/7 or interactions within those MLPs. QK2 remains a separate mixed attention8/MLP8 branch | Test the frozen late group `{attention5, MLP5, MLP6, MLP7}` versus its remainder as four ordered QK1 blocks, then carry any selected block to fresh selective routing edits while keeping the head8.2 value route distinct |
