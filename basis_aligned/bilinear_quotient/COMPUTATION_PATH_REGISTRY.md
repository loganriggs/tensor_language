...
| PATH-SET2-001 | Regional UK/US output contrast backward through MLP17 to earlier attention9 and MLP16 | Exact residual propagation followed by MLP17, attention-head, and QK source cross terms | Task-matched `ep` is `.6184`; attention9 leads its 34-source fold (`.2609`), head9.8 supplies `.9776` aligned change, and QK1 carry×carry is `.8588`. Grouping its 289 layer0–7 interactions around `{attention5, MLP5, MLP6, MLP7}` gives late×late `.4548`, the two cross-boundary terms jointly `.4409`, and remainder×remainder `.1119`; the three late-touching blocks replay with `.1119` error | Carry the frozen three-block subtotal to a fresh selective routing edit with matched QK2, value-route, and remainder×remainder controls; keep the head8.2 value route distinct |
