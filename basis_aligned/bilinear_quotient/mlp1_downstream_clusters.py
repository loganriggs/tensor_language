# mlp1_downstream_clusters: THE S1442 DOWNSTREAM-CLUSTER WIN REPEATED ONE LAYER UP
...
#   pred_a downstream-defined beats activation-defined at BOTH K=16 and K=64 (the
#          S1442 result repeats at layer 1).
#   pred_b K=256 downstream >= .60 fid_opt.
#   pred_c K=2 downstream stays below the constant floor (fid < .05) — two clusters
...
OUT = PT + 'mlp1_downstream_clusters_results.json'
NFIT = 960; NEV = 960
K = 64
H = m.transformer.h
...
def main():
    ...
    maps = [at1.c_q.weight, at1.c_k.weight, at1.c_v.weight,
            H[2].mlp.Left.weight, H[2].mlp.Right.weight]
    ...
    pb = fids['down256'] >= 0.60
    pc = fids['down2'] < 0.05
    out = {'ce': res, 'fid_opt': fids, 'mbits': {k: round(v, 3) for k, v in MBITS.items()},
           'frozen_anchor': {'ce_mean': sw['ce_mean'], 'ce_opt': sw['ce_opt']},
           'pred_a_down_beats_act': bool(pa), 'pred_b_down256_60': bool(pb),
           'pred_c_K2_below_floor': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
