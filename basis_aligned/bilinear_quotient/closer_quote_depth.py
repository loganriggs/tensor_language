# closer_quote_depth: THE DEPTH RUNG THE TAXONOMY DEMANDS (§1357). Quote-parity state is
# long-scope; §1350's rule (criterion scope predicts annotator depth) says its annotator
# extends into the early-mid band. Parity gate widened: a02 (anchor) / a05 / a08, each
# + 13.8, scored at quote-close targets (bracket family retained as diagnostic).
#
# Registered predictions:
#   pred_a THE TAXONOMY BET (4th circuit): a05-parity kit >= 0.60 quote recovery.
#   pred_b MID-CONCENTRATED: L3-5 adds >= 0.08 over the a02 kit at quote targets.
#   pred_c SELECTIVITY: every arm's elsewhere within 0.05 of route.
...
OUT = PT + 'closer_quote_depth_results.json'
NMEAN = 24; NR = 1920
...
def main():
    ...
    res = {}
    for arm, gk, band in (('full', None, None), ('ymean', None, None),
                          ('route', None, None),
                          ('kit02', 'quote', (0, 1, 2)),
                          ('kit05', 'quote', (0, 1, 2, 3, 4, 5)),
                          ('kit08', 'quote', tuple(range(9)))):
    ...
    re_ = rec['route']['else']
    pa = rec['kit05']['quote'] >= 0.60
    pb = rec['kit05']['quote'] - rec['kit02']['quote'] >= 0.08
    pc = all(abs(rec[a]['else'] - re_) <= 0.05 for a in ('kit02', 'kit05', 'kit08'))
    out = {'n_bracket': int(TB.sum()), 'n_quote': int(TQ.sum()), 'n_rows': NR,
           'ce': res, 'recovery': rec,
           'pred_a_taxonomy_bet': bool(pa), 'pred_b_mid_concentrated': bool(pb),
           'pred_c_selective': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
    json.dump(out, open(OUT, 'w'), indent=1)
    print(f"\nquote rec: kit02 {rec['kit02']['quote']} kit05 {rec['kit05']['quote']} "
          f"kit08 {rec['kit08']['quote']} route {rec['route']['quote']}")
    print(f"pred_a taxonomy {pa} | pred_b mid {pb} | pred_c selective {pc}")
