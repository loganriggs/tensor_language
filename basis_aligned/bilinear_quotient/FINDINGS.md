...
1N. **"GO HARDER" — the middle is STRUCTURED bilinear, not "irreducible"; but the content is genuinely high-rank
   ...
   **REFINEMENT (§1049-1051): the deep-middle is ONE load-bearing high-rank content flow, not ten independent maps.**
   ...
   though per-variance uniformity is falsified+confounded. §1059-1060: content-subspace ACTIVATION PATCHING causally transports topic (inject one text's content coords into another's run -> output moves to source; ~16-64 top dirs carry it, 62x/22x a random subspace). §1061: bilin18 & swiglu18 encode the SAME content info (CCA 0.95-0.97, 58/64 dirs >0.7). §1062: content is FUNCTIONALLY INTERCHANGEABLE -- swiglu18 content mapped via linear W drives bilin18 at 96% of within-model. §1063: cross-WIDTH universality (bilin12 D=768 vs D=1152: CCA 0.96-0.97). §1064: content is genuine SEMANTIC topic, NOT surface register (surface features explain ~2%). §1065: content is MULTI-SCALE (doc-topic-coherent above chance, but 83% within-doc = running local context). §1066: cross-ARCHITECTURE causal transfer (bilin12->bilin18 mapped content = 93% of within-model). §1067/§1068: content preferentially supports CONTENT/TOPICAL-word prediction (clean K=16: ablation hurts rare targets 7.6x vs frequent, above 6.0x difficulty baseline; §1067's 'broad' claim was a K=256 destructive-regime artifact, corrected). §1070/§1071: per-module understanding does NOT compose (whole-model greedy substitution 12%, compounding; content passthrough triples to 0.39 but scaffold still compounds -> valid measure is per-module-in-isolation). §1072/§1073/§1074: content ORIGIN -- not a linear raw-word bag (R2~0), but progressively/linearly built from the PROCESSED stream (0.38@L1->0.88@L7); the SEED is created by early ATTENTION gathering (attn steps +0.64 content-R2, MLP steps -0.10 = write orthogonal grammar). §1075 (DUP of §985-987/1k, corrected): value-residual & x0 re-injection ablations reproduce 1k (+3.4/+2.3 nats); NEW = per-layer localization (x0 at L0-1, value-residual at L2-4 content-onset) + freq split. Framing corrected per §987: BOTH are content-heavy, x0 only RELATIVELY more grammar-weighted (rare/freq 2.69 vs 1.56) -- a relative tilt, NOT a clean dissociation. §1076: block-0 value is EXACTLY static per-token content (static-override cost 0.0) -- value-residual broadcasts each word's static c_v value; content = pooled bag of c_v VALUES not raw embeddings. Division of labor: attention GATHERS content, front MLP writes grammar, deep-middle MLPs MULTIPLY content, readout READS; value-residual feeds content, x0 feeds grammar. **NET whole-model status: grammar (L0-1), routing attention
