"""Phase B: layer 0, at the same depth as layers 1's treatment (§13-§19).
...
OUT = ('/workspace/tensor_language/basis_aligned/bilinear_quotient/'
       'bilin18_layer0_battery_results.json')
...
@torch.no_grad()
def tracked0(idx):
    """components of layer-0's MLP input: emb and attn0, exact."""
    B, T = idx.shape
    x0 = F.rms_norm(m.transformer.wte(idx), (D,))
    blk = m.transformer.h[0]
    E = (blk.lambdas[0] + blk.lambdas[1]) * x0
    x = E
    ...
    A0 = a.c_proj(ctx.reshape(B, T, -1))
    x = x + A0
    r = x.norm(dim=-1, keepdim=True) / D ** 0.5
    xhat = F.rms_norm(x, (D,))
    Wp = a.c_proj.weight.detach()
    A0h = torch.einsum('bqhd,ehd->bqeh', ctx, Wp.view(D, NH, HD).to(ctx.dtype)) / \
        r[..., None]
    return ((E / r).reshape(-1, D).float(), (A0 / r).reshape(-1, D).float(),
...
def main():
    ...
    for i in range(0, 96, 6):
        E_, A_, X_, Ah_ = tracked0(FW[i:i + 6, :513].to(DEV))
    ...
    for r_ in range(3):
        ...
        var_c = float(c.var())
        rows = []
        for a_, Ta in (('emb', E), ('attn0', A0)):
            for b_, Tb in (('emb', E), ('attn0', A0)):
                if a_ > b_:
                    continue
