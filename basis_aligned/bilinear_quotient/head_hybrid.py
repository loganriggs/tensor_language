"""HEAD-LEVEL HYBRID -- cash the motif census into the benchmark. The
middle-attention dictionary rungs (a2-a9) are the assembly's most
expensive band (~0.9 nats in-context). Alternative rung at FINER grain:
leave attention 2-9 REAL but swap each named prev/self motif head for
its literal one-hot pattern + fitted gain (282: individually <=+0.01
each), fit under the substituted front. Induction, first, and diffuse
heads stay real (honest partial coverage); the component census then
reads "26 full components + N motif heads".
REGISTERED PREDICTIONS:
  (a) hybrid total <= +2.55 on window C (v9-best reads +2.63 there;
      dropping the dict band must not give it all back);
  (b) the joint motif-head marginal (hybrid vs same config with all
      heads real) <= +0.35;
  (c) swapped-head count reported per layer/motif."""
...
OUT=PT+'head_hybrid_results.json'
...
def main():
    ...
    def evalH(active, motif):
        ...
        return float(torch.cat(ces).mean())
    base=evalH([],False)
    ref=evalH(hyb,False)-base          # band real, no motif swaps
    hy=evalH(hyb,True)-base            # band real + motif heads swapped
    full=evalH(best,False)-base        # v9-best (all-dict band)
    print(f'band-real {ref:+.4f} | hybrid {hy:+.4f} | v9-best(all-dict) '
          f'{full:+.4f}',flush=True)
    pa=hy<=2.55; pb=(hy-ref)<=0.35
    ...
    print(f"(a) hybrid <= +2.55: {'HELD' if pa else 'FAILED'}")
    print(f"(b) motif joint marginal <= +0.35: {'HELD' if pb else 'FAILED'}")
