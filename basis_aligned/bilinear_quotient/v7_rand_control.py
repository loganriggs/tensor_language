"""v7 SELECTION NULL IN ASSEMBLY -- ledger 21 showed the CP norm-product
ranking adds nothing SOLO (random-k matches top-k). Does it matter in the
assembly, where middle costs are 3-10x larger? Re-run v7 (k=2304 middle,
same front and tail) with the kept units chosen at RANDOM (3 seeds) vs
the ranked choice (+1.678, section 264).
REGISTERED PREDICTIONS (decision registered before running):
  (a) if the ranking is real, every random seed is worse by >= +0.15;
      if redundancy is the whole story, all seeds land within +-0.05 of
      +1.678. We PREDICT the redundancy branch (ledger 21).
  (b) seed spread (max-min) <= 0.10 (selection noise is small either way)."""
...
OUT=PT+'v7_rand_control_results.json'
...
def main():
    ...
    clsA=classify(CA,CB).to(DEV); flatA=clsA.reshape(-1)
    def mkcp(li,keep):
    ...
    # tail fit under TOP-ranked CP middle (fixed across arms so only the
    # middle selection varies at eval; noted in write-up)
    for li in MIDL:
        mlp=m.transformer.h[li].mlp
        imp=(mlp.Down.weight.detach().float().norm(dim=0)
             *mlp.Left.weight.detach().float().norm(dim=1)
             *mlp.Right.weight.detach().float().norm(dim=1))
        S[f'c{li}']=mkcp(li,imp.argsort(descending=True)[:2304])
        order.append(f'c{li}')
    ...
    base=evalC([])
    top=evalC(order)-base
    print(f'top-2304   {top:+.4f}',flush=True)
    g=torch.Generator(device=DEV).manual_seed(0)
    rands=[]
    for s in range(3):
        for li in MIDL:
            H=m.transformer.h[li].mlp.Left.weight.shape[0]
            S[f'c{li}']=mkcp(li,torch.randperm(H,device=DEV,
                                               generator=g)[:2304])
        r=evalC(order)-base
        rands.append(r)
        print(f'rand seed{s} {r:+.4f}',flush=True)
    spread=max(rands)-min(rands)
    close=all(abs(r-top)<=0.05 for r in rands)
    worse=all(r>=top+0.15 for r in rands)
    pa='redundancy' if close else ('ranking' if worse else 'MIXED')
    pb=spread<=0.10
    out={'base':round(base,4),'top':round(top,4),
         'rand':[round(r,4) for r in rands],'branch':pa,
         'pred_a_registered':'redundancy','pred_b':bool(pb)}
    print(f"\n(a) branch = {pa} (registered prediction: redundancy)")
    print(f"(b) seed spread {spread:.3f} <= 0.10: "
