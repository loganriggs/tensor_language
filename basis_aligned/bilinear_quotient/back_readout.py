...
OUT = PT + 'back_readout_results.json'
...
def main():
    ...
    for L in [16, 17]:
        ...
        for k in top:
            ...
            wlog = lm @ Dw[:, k]                             # (V,) logit push per token (sign depends on hid sign; use mean hid sign)
            sgn = float(np.sign(a.mean() + 1e-9))
            wlog = wlog * sgn
            wtop = torch.topk(wlog, NTOK).indices.cpu().numpy()
            units.append({'unit': int(k), 'read_class': CLASSES[rc], 'read_sel': round(rsel, 2),
                          'writes_tokens': [repr(d(int(t))) for t in wtop]})
        out['layers'][f'mlp{L}'] = units
        print(f"=== mlp{L} readout units (read class -> write tokens) ===", flush=True)
        for u in units[:10]:
            print(f"  u{u['unit']}: read {u['read_class']}({u['read_sel']}) -> writes {u['writes_tokens'][:7]}", flush=True)
