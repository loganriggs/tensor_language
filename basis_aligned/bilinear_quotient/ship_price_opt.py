# ship_price_opt: THE PRICE-OPTIMIZED SHIP (Pareto point per the spec). Config:
# attention MIXED tiers — rank-16 QK at layers where r16 fid >= .85 ({0,1,4,7,9},
# S1498) and rank-32 elsewhere (roster heads exact as always); MLP planks at the
# economic knee K=2048 EXCEPT saturated 16/17 at K=1024. mlp0/1/2 planks + loaded
# correction + mlp3 projection unchanged. Total description ~40% cheaper than v4.
...
#   pred_a price-optimized ship <= 4.15 (within .31 of the 171-Mbit v4).
#   pred_b description total <= 2.6 Gbit (computed in-script) vs v4's ~3.4.
#   pred_c fresh-row band <= .08.
...
def main():
    ...
    for LT in LAYERS:
        ...
        if LT in PLAIN:
            ...
            Wh, Whi = WHITEN[LT]
        rr_ = 16 if LT in R16_LAYERS else 32
        TWALL[LT] = {'q': trunc_perhead(at.c_q.weight, rr_, Wh, Whi),
                     'k': trunc_perhead(at.c_k.weight, rr_, Wh, Whi),
                     'q2': trunc_perhead(at.c_q2.weight, rr_, Wh, Whi),
                     'k2': trunc_perhead(at.c_k2.weight, rr_, Wh, Whi)}
    ...
    for L in range(4, 18):
        ...
        KL = 1024 if L in K1024_LAYERS else KU
        topu = score.argsort(descending=True)[:KL]
    ...
    price = 0.0
    for L in range(18):
        rr_ = 16 if L in R16_LAYERS else 32
        price += 4 * 9 * rr_ * (128 + D) * 16 / 1e9
    for L in range(4, 18):
        KL = 1024 if L in K1024_LAYERS else 2048
        price += 3 * KL * D * 16 / 1e9
    price += (2000 * D + 50257 * 64 + 64 * D) * 2 * 16 / 1e9   # two tier tables
    price += (D * D * 3 + 2 * D * D) * 16 / 1e9                 # ridges approx
    pa = res['ship36'] <= 4.15
    pb = price <= 2.6
    pc = abs(res['ship36_fresh2000'] - res['ship36']) <= 0.08
    out = {'ce': res, 'delta36': round(d36, 4),
           ...
           'price_gbit': round(price, 3),
           'pred_a_le_415': bool(pa), 'pred_b_price_le_26': bool(pb),
           'pred_c_fresh_le_08': bool(pc), 'runtime_s': round(time.time() - t0, 1)}
