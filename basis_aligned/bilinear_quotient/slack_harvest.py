"""SLACK HARVEST -- the census improves the REAL model (user
directive's boldest cash-in): 324 found members whose mode-score sign
predicts they IMPROVE when their circuit's components are ablated
(circuit-level slack). Policy: on fresh rows, at positions where any of
the top-10 modes scores below its fit-window 8th percentile (strongly
negative -- oracle-labeled, stated), MEAN-ABLATE that mode's top-2
loading components at those positions only. Everything else runs the
REAL model. Measure total CE change vs the untouched model.
REGISTERED PREDICTIONS (fresh rows):
  (a) net Delta-CE < 0 -- the policy IMPROVES the real model;
  (b) the same ablations at random positions (matched fraction per
      mode) give Delta-CE >= 0;
  (c) per-mode contributions + touched fraction reported."""
...
OUT=PT+'slack_harvest_results.json'
...
def main():
    ...
    for k in range(10):
        ...
        topc=[P36[i] for i in L.abs().argsort(descending=True)[:2]
              .tolist()]
        maskk=(scF[:,k]<qlo[k]).view(120,256)
        modes.append((k,topc,maskk))
        print(f'mode {k}: comps {topc} neg-frac '
