"""Deciding control for section 140: if easy+/hard- is the generic flattening
...
OUT=('/workspace/tensor_language/basis_aligned/bilinear_quotient/'
     'bilin18_generic_damage_results.json')
...
def main():
    ...
    def mlp_random_ce(li,k):
        ...
        h.remove()
        return torch.cat(ces)
    comps=[('L5rand96',5,mlp_random_ce(5,96)),
           ('L13rand96',13,mlp_random_ce(13,96)),
           ('L9rand256',9,mlp_random_ce(9,256))]
    out={}
    ...
    names=('L5rand96','L13rand96','L9rand256')
    pa=all(out[n]['easy_mean']>0 and out[n]['hard_mean']<0 for n in names)
    mags=[(abs(out[n]['hard_mean']),abs(out[n]['easy_mean'])) for n in names]
    tot={n: out[n]['easy_mean']-out[n]['hard_mean'] for n in names}
    pb=True
    out['pred_a']=bool(pa); out['pred_b']=bool(pb)
    print(f"\n(a) generic damages show easy+/hard-: {'HELD' if pa else 'FAILED'}")
