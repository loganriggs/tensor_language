"""STREAM FEATURES -- the decisive test after 363: half the census
is not token-definable, so either (i) membership is readable from
the model's own EARLY STREAM, or (ii) the leaves have no position-
set description at all. Add block-2 residual-stream predicates:
project each position's stream onto the top-16 PCA directions of
the stream over fit rows, threshold at median -> 16 predicates
(+32 shifted). Stream-legal, same stance as the 3.8x probe. Ladder
rerun until dry. Baseline band 55-58.
REGISTERED PREDICTIONS:
  (a) converged programmable total >=64 of 118 -- hypothesis (i);
      if FAILED, hypothesis (ii) strengthens and the census's
      unprogrammable half is recorded as activation-space-only;
  (b) shuffled-label null median <=0.6;
  (c) >=8 passes cite a stream_ predicate."""
...
OUT=PT+'stream_features_results.json'
CA=300; NB=78
MHL=list(range(2,10))
T=256
...
def main():
    ...
    # ---- block-2 stream predicates ----
    caps9=[]
    def sh9(mo,i_,o_): pass
    with torch.no_grad():
        for i9 in range(0,ntokr,4):
            bb=rows[i9:i9+4,:257].to(DEV)
            idx=bb[:,:-1].contiguous()
            x=F.rms_norm(m.transformer.wte(idx),(1152,)); x0=x; v1=None
            for lj,blk in enumerate(m.transformer.h):
                x,v1=blk(x,v1,x0)
                if lj==2: break
            caps9.append(x.float().reshape(-1,1152).cpu())
    XS=torch.cat(caps9)
    g9f=torch.Generator().manual_seed(3)
    sub=torch.randperm(len(XS),generator=g9f)[:20000]
    Xc=XS-XS.mean(0)
    _,_,Vh9=torch.linalg.svd(Xc[sub],full_matrices=False)
    SC9=Xc@Vh9[:16].T
    for k9 in range(16):
        LIB0[f'stream_d{k9}']=(SC9[:,k9]>=SC9[:,k9].median())
    print(f'stream predicates added ({len(LIB0)} base)',flush=True)
    LIB=dict(LIB0)
    ...
    nf=sum(1 for p in allp if 'stream_' in p)
    gainN=total-58
    pc=nf>=8
    out['pred_c']=bool(pc); out['n_stream_cited']=nf; out['gain_vs_58']=gainN
    print(f'stream-cited passes: {nf} | gain vs 58: {gainN:+d}')
    print(f"(a) converged >=64: {'HELD' if pa else 'FAILED'}")
    print(f"(b) null <=0.6: {'HELD' if pb else 'FAILED'}")
    print(f"(c) >=8 stream cites: {'HELD' if pc else 'FAILED'}")
