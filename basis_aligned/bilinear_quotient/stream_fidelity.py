"""STREAM FIDELITY -- 309's mechanism test. Hypothesis: fold tables are
CE-cheap but STREAM-unfaithful, and downstream rungs are priced by the
stream they read. Measure, for both matched-context arms (fold and
empirical twin), the MSE of the residual stream at layer 10's input
versus the real model's stream, on window C.
REGISTERED PREDICTIONS:
  (a) fold stream-MSE >= 1.2x empirical stream-MSE (CE and stream
      fidelity dissociate -- fold has LOWER CE, HIGHER stream error);
  (b) both MSEs reported per-layer (inputs of blocks 4, 7, 10, 14) --
      the divergence should grow with depth (conduction)."""
...
OUT=PT+'stream_fidelity_results.json'
...
def main():
    ...
    def collect(active):
        ...
        for li in PROBES:
            def mk3(li=li):
                def h(mo,args):
                    caps[li].append(args[0].detach().float()
                                    .reshape(-1,D))
                return h
            hs.append(m.transformer.h[li]
                      .register_forward_pre_hook(mk3()))
    ...
    for li in PROBES:
        ...
        mf=float(((foldS[li]-real[li])**2).mean(0).sum())/vr
        me=float(((empS[li]-real[li])**2).mean(0).sum())/vr
        out['per_layer'][li]={'fold':round(mf,4),'emp':round(me,4)}
        print(f'block {li:2d} input: fold rel-MSE {mf:.4f} | '
              f'empirical {me:.4f}',flush=True)
        if li==10: r10,e10=mf,me
    pa=r10>=1.2*e10
    grow=(out['per_layer'][14]['fold']-out['per_layer'][4]['fold'])>0
    out['pred_a']=bool(pa); out['divergence_grows']=bool(grow)
    print(f"(a) fold stream-MSE >= 1.2x empirical at block 10: "
          f"{'HELD' if pa else 'FAILED'}")
    print(f"(b) divergence grows with depth: {grow}")
