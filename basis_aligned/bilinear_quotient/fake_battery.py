"""FAKE BATTERY -- validate the certification pipeline on constructed
fakes (user direction; the scorecard-on-known-model rule at circuit
level). Candidates: 5 REAL positive-control leaves (redteam3 passers)
and 5 FAKES, each targeting one failure mode:
  F1 random-members (real components, random matched member set);
  F2 severity (global heavy components + most-fragile tokens);
  F3 frankenstein (half A + half B members, A components);
  F4 topic (4 contiguous documents, post-hoc components);
  F5 adversarial (members selected ON the joint-damage statistic of the
     strongest probes, fit half).
Battery per candidate, held-out rows only: SELECTIVITY (own-probe joint
ablation, members vs base-CE-matched controls) and the NEW dimension
this battery forces, SPECIFICITY (own-probe member effect / foreign-
probe member effect, foreign = next candidate's set).
REGISTERED PREDICTIONS:
  (a) >=4/5 real controls: selectivity >=2 AND specificity >=1.5;
  (b) F1 and F2 selectivity <=1.3 (matching + coherence reject them);
  (c) F3 selectivity below BOTH parents;
  (d) F5 in-sample selectivity >=2 (it cheats by construction) but
      held-out specificity <=1.2 (the new dimension deflates it);
  (e) F4 rejected by selectivity <=1.5 OR flagged by confound stats --
      if F4 passes everything, that is a discovered HOLE, reported as
      such and the battery gets a new leg, not the fake a pass."""
...
OUT=PT+'fake_battery_results.json'
CA=300; NB=78
MHL=list(range(2,10))
T=256
...
def main():
    ...
    for i,cd in enumerate(cands):
        ...
        print(f"{cd['name']:16s} sel {selv:5.2f} (in-sample "
              f"{selv_in:5.2f}) spec {spec:5.2f}",flush=True)
    ...
    hole=(not pe) and R['F4_topic']['specificity']>=1.5
    out={'results':results,'real_pass':realpass,
         'pred_a':bool(pa),'pred_b':bool(pb),'pred_c':bool(pc),
         'pred_d':bool(pdx),'pred_e':bool(pe),
         'F4_hole_discovered':bool(hole)}
    print(f"(a) reals pass ({realpass}/5): {'HELD' if pa else 'FAILED'}")
    print(f"(b) F1/F2 rejected: {'HELD' if pb else 'FAILED'}")
    print(f"(c) F3 below parents: {'HELD' if pc else 'FAILED'}")
    print(f"(d) F5 cheats in-sample, deflated by specificity: "
          f"{'HELD' if pdx else 'FAILED'}")
    print(f"(e) F4 rejected: {'HELD' if pe else 'FAILED'}"
          + ("  ** HOLE DISCOVERED **" if hole else ""))
