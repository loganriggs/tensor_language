"""MLP9 UNIT CLUSTER -- do nameable circuits exist in a MIDDLE layer,
or is clean surface-token circuitry a front-of-model phenomenon?

All unit-clustering so far has been layers 0-2 (579/581/587/596/600).
The depth map found the middle layers (L6-10) are the LEAST linearly
predictable and most representationally tangled. This runs the exact
same clustering on mlp9 (a middle layer) to test whether the method
still finds clean, reproducible, nameable structure there, or whether
clean surface-lexical circuits are specifically a front-of-model
thing. Extends the coverage map from importance-wise (600/601) to
layer-wise.

Note: a middle layer's input is the full residual stream (abstract
features), not the token embedding, so any clusters found may key on
ABSTRACT context rather than surface tokens -- the fine_class
(surface-token) naming may show LOW concentration even if real
structure exists. Low surface concentration with high stability would
itself be the finding: middle layers have reproducible structure that
is not surface-lexical.

REGISTERED PREDICTIONS:
  (0) SANITY: exact by construction (h @ Down.T IS the real output);
  (a) STABILITY: split-half reclustering ARI -- report against the
      front-of-model trend (mlp0 0.58, mlp1 0.326, mlp2 0.295).
      Prediction: mlp9 still clusters reproducibly (ARI > 0.1) --
      real structure exists mid-network -- but whether it is stronger
      or weaker than the front is the finding;
  (b) SURFACE CONCENTRATION: report whether any of the 3 largest
      clusters reaches >= 5/8 surface-token concentration. Prediction:
      LOWER than the front (mlp0/mlp1 hit 8/8) -- middle-layer
      clusters should key on abstract features, not surface tokens.
      Low concentration + high stability = reproducible-but-abstract;
  (c) READ THE CLUSTERS (no bar): are the top clusters nameable at
      all (a clean surface behaviour), semantically coherent but not
      surface-lexical, or noise? This characterizes what a middle
      layer's structure looks like;
...
OUT = PT + 'mlp9_unit_cluster_results.json'
NFRESH = 64
NSAMP = 4000
TOPK = 300
NCLUST = 20
