"""LADDER 16B -- 412: ladder16's k3 bars (0.98/0.976) are
UNSUPPORTED as content claims -- the shuffled-value null at k1
recovered ~100% of the k1 lift (for 16.8 it BEAT the true k1), so
at layer 16 generic positions the instrument may be measuring
write-magnitude structure, not code content. Decisive version:
shuffled arms at EVERY k (all relay levels use row-shuffled value
sources), so k3 vs k3shuf isolates content at full depth.
REGISTERED PREDICTIONS:
  (a) CONTENT fork: k3shuf <= k3 - 0.25 for both heads (content
      matters at depth); if FAILED, the ladder instrument has no
      discriminative power at layer-16 generic positions and the
      one-code-at-16 claim rests on the writer decomposition
      (411) alone -- recorded either way;
  (b) monotone: k1shuf <= k2shuf <= k3shuf;
  (c) report seen/fresh splits for shuffled arms."""
...
def main():
    ...
    ARMS=[f'k{k}' for k in range(KMAX+1)]+[f'k{k}shuf' for k in range(1,KMAX+1)]
    ...
    for i in range(0,NR,4):
        ...
        chains['k0']=codes0
        prev=resid; prevS=resid
        for k in range(1,KMAX+1):
            ck,resid=build(prev)
            chains[f'k{k}']=ck
            cs,residS=build(prevS,shuf=True)
            chains[f'k{k}shuf']=cs
            prev=resid; prevS=residS
        for li,hd in HEADS:
    ...
    pa=all(outj[k]['k3shuf']<=outj[k]['k3']-0.25 for k in outj)
    pb=all(outj[k]['k1shuf']<=outj[k]['k2shuf']+0.02
           and outj[k]['k2shuf']<=outj[k]['k3shuf']+0.02
           for k in outj)
    out={'heads':outj,'pred_a':bool(pa),'pred_b':bool(pb),
         'pred_c':True}
    for nm,v in (('a','k3shuf <= k3-0.25 (content matters)'),
                 ('b','shuffled arms monotone'),
                 ('c','splits reported')):
        print(f"({nm}) {v}: "
