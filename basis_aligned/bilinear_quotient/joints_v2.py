# joints_v2: CARRIER-DROPPED JOINT RE-CERTIFICATION (S1582: the is overlap is
# a split two-head channel {11.3, 7.8}; months' carrier is 9.7). Re-run the
# is and months joints with the overlap carriers REMOVED from the head
# ensemble (is: drop both 11.3 and 7.8 -> 3 heads; months: drop 9.7 -> 4
# heads), same mlp17 pos_r8 payload slices, NR=960.
# Registered predictions:
#   pred_a is (3 heads + slice) additivity >= .92.
#   pred_b months (4 heads + slice) additivity >= .95.
#   pred_c reduced head ensembles keep >= 60% of the full-ensemble class rise
#          (dropping carriers does not gut the attention side; is loses its 2
#          strongest members so the bar is moderate).
...
def main():
    ...
    for cname, L in SITES.items():
        DROP = {'is': {'11.3', '7.8'}, 'months': {'9.7'}}
        kept = [s for s in r2[cname]['W']['heads'] if s not in DROP[cname]]
        heads = [(int(s.split('.')[0]), int(s.split('.')[1])) for s in kept]
        mask_v = CLS[cname]
    ...
    FULL_RISE = {'is': 0.2209, 'months': 0.1493}   # S1580 full-ensemble refs
    def addfrac(cn):
        r = res[cn]
        return r['joint']['rise_class'] / max(
            r['heads_only']['rise_class'] + r['slice_only']['rise_class'], 1e-9)
    pa = addfrac('is') >= 0.92
    pb = addfrac('months') >= 0.95
    pc = all(res[cn]['heads_only']['rise_class'] >= 0.60 * FULL_RISE[cn]
             for cn in SITES)
    out = {'res': res,
           'additivity': {cn: round(addfrac(cn), 3) for cn in SITES},
           'pred_a_is_92': bool(pa), 'pred_b_months_95': bool(pb),
           'pred_c_heads_keep_60': bool(pc),
