...
| 11 | Token-only generator for all four heads: p × λ × v1(cue) with the native pattern retains 84% of the zeroed service; a constant pattern per (head, cue) from the other two constructions 88% pooled (accept 0.88 / bought 0.70 / chose 1.12) | edit | fresh (v131) | registered "pattern CV ≤ 0.35 in every (head, construction, cue)" false (max 0.48) — the constants still transfer at ≥ 0.70 in every held-out construction | 4/5 — the set closes to a two-entry lookup on the cue token with constant attention weights; the patterns vary more than on the person line but the lookup holds |
| U1 | MLP-8 unit 1512 (the both/neither line's neither-context detector) on the v119 rows: u_1512(final) = −22 on not-rows vs +5.5 on either-rows (sign agreement 98%), cue and final contrasts equal (ratio 1.0); cos(Down8[:,1512], r_16.8) = −0.01; zeroing 1512 at the final changes the pooled or − but margin by +0.2% (all positions +0.7%); 16 random units ≤ 0.01% | fold + edit | v119 rows (v265) | "separates either/not ≥ 0.90" held; "not < −30 < either" (−22), "fires at the final ≥ 1.5× the cue" (1.0), "edit beats random and removes > 0" (a growth) false | 1/4 — 1512 is a NEGATIVE-POLARITY detector: it fires on 'not' too, at a fifth of its 'neither' response, and is behaviourally inert for or / but. Its and / nor role (v262, v264b) is specific to the both/neither readout; its non-selectivity there (v264b) is the polarity signal leaking into other readers |
...
## Receipts
- v131: `.../correlative_either_not_dod_token_only_generator_v131_result.json`; code `ops/run_correlative_either_not_dod_token_only_generator_v131.py` (v113's body)
