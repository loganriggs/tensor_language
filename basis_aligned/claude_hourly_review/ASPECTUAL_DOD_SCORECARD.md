...
| Extracted | held at the head boundary with three scalar ports (row 22); **8.1's port is closed** — a token-only generator with two stored constants (row 25); 9.1/9.4's ports are contextual-bank reads of `last`+period+`the` (row 23) | fold the block-9 heads' bank read one step back (which block writes the bank state those heads read, and is that write token-only?) |
