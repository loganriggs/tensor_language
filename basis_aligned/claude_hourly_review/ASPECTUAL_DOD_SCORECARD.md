...
| Predicts OOD | held for the component's removal effect (sign, positivity, band) on new lexicon and new templates (rows 16, 21); **strong form (token IDs in) held for the 8.1 sub-component on new templates (row 30)**; not for the 9.1/9.4 relay (open ports) | a frozen numeric prediction of the whole-component effect on a new corpus |
