...
| Extracted | held at the head boundary (row 22); 8.1's port closed (row 25); 9.1/9.4 read a bank written by attention8 = head 8.1's token-only cue read (row 27) plus MLPs 6–8 (row 26) | edit: make head 8.1 a token-only generator at the bank positions too and measure what the block-9 heads retain; MLP6–8's share stays an open port |
