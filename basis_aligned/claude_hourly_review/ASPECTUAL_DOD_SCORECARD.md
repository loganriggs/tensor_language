...
| Extracted | held at the head boundary with three scalar ports (row 22). 8.1's port reduces to one native scalar (its attention pattern on the cue) times a token-only table (row 24); 9.1/9.4's ports are contextual-bank reads (row 23) | test whether 8.1's pattern scalar is a constant across rows (then 8.1 closes to a token-only generator); fold the block-9 bank one step back |
