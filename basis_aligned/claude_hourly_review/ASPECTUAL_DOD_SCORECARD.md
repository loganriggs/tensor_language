...
| Extracted | held at the head boundary (row 22); head 8.1 is a token-only generator at every position that matters (rows 25, 28); 9.1/9.4's remaining native inputs are the MLP6–8 bank writes (row 26) and their own attention patterns | fold MLP8's bilinear bank write into writer-pair terms (is it 8.1's write squared / crossed with the token?) |
