...
| Predicts OOD | held on authored panels (rows 16, 21, 32; strong form for 8.1, rows 30, 32); on natural corpus the `since` half transfers and the any-sense `by` half does not (row 33) | temporal-`by` natural rows (outcome-blind filter) and the 8.1 pattern-on-cue per natural row |
