...
| Predicts OOD | **held**: a frozen numeric prediction (0.54 ± 0.15) of the component's effect verified on a third lexicon and a new construction (row 32); strong form (token IDs in) held for the 8.1 sub-component (rows 30, 32); not for the 9.1/9.4 relay (open ports) | natural-corpus rows (FineWeb/Pile) with the same frozen numbers |
