...
print('mining datastore...', flush=True)
from collections import defaultdict
cnt3, cnt2 = defaultdict(lambda: defaultdict(int)), defaultdict(lambda: defaultdict(int))
CO = COOC.numpy()
for d in range(CO.shape[0]):
    row = CO[d]
    for p in range(2, len(row) - 1):
        cnt3[(row[p - 2], row[p - 1], row[p])][row[p + 1]] += 1
        cnt2[(row[p - 1], row[p])][row[p + 1]] += 1
...
for aname, (mod, X) in ARMS.items():
    ...
    print(f'{aname}: {nparam} params, val R2 {r2:.4f}, top-1% weight mass {top1:.3f}',
          flush=True)
    json.dump(out, open(f'{QK}/qk_explicit_memory.json', 'w'), indent=2)
