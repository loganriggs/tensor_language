"""SPIKE-SPARSITY TEST: are coordinates weighted indicators of <=k specific tokens (the mechanism
arc found token CLUSTERS like {the},{a/an}, not broad classes)? Concentration = fraction of
frequency-weighted |spectrum| mass in the top-8 tokens, computed for BOTH the archetype-144 and
PCA-576 bases; coordinates with concentration >=0.8 get 8-SPIKE codes (top-8 token values kept
exact, all else set to the weighted off-spike mean) and the archetype set is gated by substitution.
Built from: MECHANISM-LEDGER NAMING TEST: the same class battery + meaning gate as qk_coord_semantics_l0,
...
json.dump(res, open(f'{QK}/qk_coord_spike.json', 'w'), indent=2)
json.dump(results, open(f'{QK}/qk_coord_spike_table.json', 'w'), indent=2)
print("QK COORD COORD SPIKE DONE", flush=True)
