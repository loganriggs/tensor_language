...
ck = torch.load(f'{QK}/med_confound.pt', map_location=DEV)
...
def stamp(x):
    x = x.clone(); x[:, :, MPOS[0]:MPOS[1], MPOS[0]:MPOS[1]] = MVAL; return x
...
json.dump({**res, 'raw': scores}, open(f'{QK}/med_confound_detect.json', 'w'), indent=2)
print('MED CONFOUND DETECT DONE', flush=True)
