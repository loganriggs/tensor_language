"""EARLY-LAYER HIGH-RANK FEED-FORWARD TAIL characterization -- MLP layer 1 (the hub).

§71/§73: ~62% of bilin18's unfound feed-forward residual lives in MLP layer 1; it is
HIGH-RANK (80% needs ~28 SVD dirs/block, not 4) but BASIS-ALIGNED (SVD beats random
35-200x). OPEN QUESTION: is MLP1's high-rank tail NAMEABLE (many small individually
interpretable structured features) or IRREDUCIBLY DISTRIBUTED (structured superposition
no single direction captures)?

This script takes MLP layer 1's top-32 SVD OUTPUT directions (from the TRAIN gram,
same construction as qk_mlp_superposition.py) and for EACH direction d (ranks 0-31)
computes: (a) causal importance = mean-ablation delta cross-entropy at its top firing
positions, paired standard error; (b) class-level signature = class-summed delta-logit
per coarse token class + pushed/suppressed class and sign (§68 method); (c) trigger
signature = token-class distribution at top firing positions + trigger-purity; (d)
output entropy / top-token concentration. Then CLASSIFIES each direction and reports the
fraction of MLP1's causal effect the interpretable ones account for.

FORWARD + mean-ablation + MLP-SVD-dir construction copied VERBATIM from
qk_mlp_superposition.py (per-block MLP SVD from the TRAIN gram, per-direction project-out
mean-ablation). Class library (lex1/VOCAB_CLASS) + class-summed delta-logit copied
VERBATIM from qk_unsup_classpush.py. Paired standard errors as qk_unsup_verify.py.
...
K32 = 32        # top-32 SVD output directions
print(f"bilin18 NL={NL} NH={NH} HD={HD} D={D} V={V}; characterizing MLP L{LI} top-{K32} SVD dirs", flush=True)
...
NHELD = HELD.shape[0]
BATCH = 6
KCAUSAL = 200                                # top-K activation positions per direction (matches census)
...
# =====================================================================================
# MLP directions: recompute top-K32 SVD dirs for layer LI from the TRAIN gram (VERBATIM).
# =====================================================================================
...
DIRS = _evecs[:, -K32:].T.flip(0).contiguous()      # (K32, D) top-K32 descending
gram_eval = _evals.cpu().numpy()                    # ascending
del gram, _evecs
print("MLP directions ready.", flush=True)

# =====================================================================================
# Core forward (VERBATIM) with (a) collect, (b) project-out mean-ablation of a set of
# layer-LI directions.  proj=(Dirs (k,D), PM (T,k)) -> replace coeff by per-pos mean.
# =====================================================================================
@torch.no_grad()
def forward(idx, collect=False, proj=None):
    ...
    for li in range(NL):
        ...
        if collect and li == LI:
            pr = torch.einsum('btd,kd->btk', mo, DIRS)        # (B,T,K32)
            PROJ_SUM[0] += pr.sum(0)                          # (T,K32)
            out_proj[0] = pr.cpu().numpy()
        if proj is not None and li == LI:
            Dirs, PM = proj                                   # (k,D),(T,k)
...
for i in range(0, NHELD, BATCH):
    ...
    pj = np.abs(out_proj[0])                                  # (b,T,K32)
    for kk in range(K32): mlp_act[kk, i:i+b] = pj[:, :, kk]
PROJMEAN = PROJ_SUM[0] / NHELD                                # (T,K32)
del PROJ_SUM
...
# top-KCAUSAL activation-selected trigger mask per direction (VERBATIM census-style)
trig_mask = {}
for kk in range(K32):
...
# trigger class signature (current token class at top firing positions) -- numpy only
...
# =====================================================================================
# PASS B: for every direction -- mean-ablate, accumulate GLOBAL dCE + TRIGGER dCE
# (paired) + CLASS-SUMMED delta-logit + mean delta-logit vector (for token concentration).
# =====================================================================================
...
dlogit_sum = {k: torch.zeros(V, device=DEV) for k in range(K32)}   # base-ablated, over firing positions
...
# full-MLP-L1 mean-ablation (all 32 dirs together = near-full layer) reference
print("PASS B2: full-MLP-L1 (all top-32 dirs) mean-ablation reference ...", flush=True)
...
print(f"full-MLP-L1(top32) global dCE {full_mean:.4f} +- {full_se:.4f}", flush=True)
...
for kk in range(K32):
    ...
    # output token concentration / entropy of the mean delta-logit magnitude over vocab
    dmean = (dlogit_sum[kk] / max(1, nfire[kk]))
    dabs = dmean.abs().cpu().numpy()
    dsort = np.sort(dabs)[::-1]
    tot = float(dabs.sum()) + 1e-12
    top1_share = float(dsort[0]/tot)
    top10_share = float(dsort[:10].sum()/tot)
    out_ent = round(entropy_of(dabs + 1e-30), 3)   # nats; log(V)=10.82 uniform
    # top boosted / suppressed tokens (signed mean delta-logit; base-ablated => + boosts)
    dm = dmean.cpu().numpy()
    ...
    if not clear:
        bucket = 'null_subthreshold'
    elif concentration >= 0.45 and push_val >= 0:
        bucket = 'class_pusher'
    elif concentration >= 0.45 and push_val < 0:
        bucket = 'class_suppressor'
    elif top10_share >= 0.5:
        bucket = 'sharp_token'
    else:
        bucket = 'diffuse_distributed'
    records.append({
...
# bucket counts + fraction of MLP1 causal effect that is interpretable
...
summary = {
    'full_MLP_L1_top32_global_dCE': round(full_mean, 5), 'full_MLP_L1_top32_global_dCE_SE': round(full_se, 5),
    'sum_per_dir_global_dCE': round(sum_global_all, 5),
    'bucket_counts': dict(bucket_counts),
    'n_causally_clear': int(sum(1 for r in records if r['bucket'] != 'null_subthreshold')),
    'n_interpretable': int(sum(1 for r in records if r['bucket'] in INTERP)),
    'frac_interp_of_summed_global_dCE': round(sum_global_interp / sum_global_all, 4) if sum_global_all > 0 else None,
    'frac_interp_of_summed_trigger_dCE': round(sum_trig_interp / sum_trig_all, 4) if sum_trig_all > 0 else None,
    'frac_interp_global_dCE_over_full_layer': round(sum_global_interp / full_mean, 4) if full_mean > 0 else None,
    'sum_per_dir_global_over_full_layer': round(sum_global_all / full_mean, 4) if full_mean > 0 else None,
}

out = {
    'meta': {
        'model': 'bilin18', 'layer': LI, 'K_dirs': K32, 'held_slice': 'FW[448:600,:128]',
        ...
        'forward': 'VERBATIM qk_mlp_superposition.py (project-out mean-ablation) + class library VERBATIM qk_unsup_classpush.py',
        'classify_rule': 'clear = trigger_dCE_z>=3 & trigger_dCE>=0.02. class_pusher/suppressor: class_concentration>=0.45 '
                         '(single class holds >=45% of abs class-summed movement), sign of pushed class. sharp_token: '
                         'top10 token share>=0.5. diffuse_distributed: clear but neither. null_subthreshold: not clear. '
                         'INTERPRETABLE = {class_pusher, class_suppressor, sharp_token}.',
        'push_excluded_classes': sorted(PUSH_EXCLUDE),
    },
    'summary': summary,
    'records': records,
}
json.dump(out, open(f'{QK}/qk_mlp1_tail.json', 'w'), indent=2)

print("\n===== MLP L1 TOP-32 SVD DIRECTION CHARACTERIZATION =====", flush=True)
print(f"full-layer(top32) global dCE {full_mean:.4f}   sum-per-dir global dCE {sum_global_all:.4f}", flush=True)
print(f"bucket counts: {dict(bucket_counts)}", flush=True)
print(f"interpretable dirs: {summary['n_interpretable']}/{K32}; "
      f"they hold {summary['frac_interp_of_summed_global_dCE']} of summed-per-dir global dCE, "
      f"{summary['frac_interp_of_summed_trigger_dCE']} of summed trigger dCE", flush=True)
...
print("\nSaved qk_mlp1_tail.json", flush=True)
print("QK MLP1 TAIL DONE", flush=True)
