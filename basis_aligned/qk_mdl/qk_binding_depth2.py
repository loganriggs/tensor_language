"""TICK 241: at what depth is the compound key absorbed into the key position?
...
for (di, p, j, j2) in PAIRS[:48]:
    ...
    for L in LAYERS:
        ...
        agg[f'bindL{L}'].append((float(F.log_softmax(lg2[0, p].float(), 0)[tgt]) - lp_x) / den)
    lg2, _ = run_p(idx_x, patch=('resid', 13, 17, [j], cc))
    agg['bind_band_13_17'].append((float(F.log_softmax(lg2[0, p].float(), 0)[tgt]) - lp_x) / den)
    for L in LAYERS:
        lg2, _ = run_p(idx_x, patch=('resid', L, L, [p], cc))
        agg[f'targetL{L}'].append((float(F.log_softmax(lg2[0, p].float(), 0)[tgt]) - lp_x) / den)
out = {k: {'mean': round(float(np.mean(v)), 3), 'median': round(float(np.median(v)), 3),
           'n': len(v)} for k, v in agg.items() if v}
print(json.dumps(out, indent=1), flush=True)
json.dump(out, open(f'{QK}/qk_binding_depth2.json', 'w'), indent=2)
