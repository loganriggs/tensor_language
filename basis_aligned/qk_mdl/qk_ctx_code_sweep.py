...
ARMS = {
    'lin128': (nn.Linear(128, 576), C128),
    'lin512': (nn.Linear(512, 576), C512),
    'mixed_lin': (nn.Linear(140, 576), CMIX),
    'mixed_swiglu': (Skip(140, Bilin(140, 130, 576, gate='silu'), 576), CMIX),
}
out = {}
models = {}
for aname, (mod, X) in ARMS.items():
    ...
    print(f'{aname}: {nparam} params, val R2 {r2:.4f}, top-1% weight mass {top1:.3f}',
          flush=True)
    json.dump(out, open(f'{QK}/qk_ctx_code_sweep.json', 'w'), indent=2)
            ...
            if li == 1 and arm is not None and name is not None:
                ...
                if arm == '__static__':
                    ...
                    if arm == 'lin128':
                        ...
                        feat = torch.cat([(cm @ P64g) / STD64, ac, sc], 1)
