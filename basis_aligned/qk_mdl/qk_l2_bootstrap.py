...
# random counterparts (tick-258 null construction, same seed stream)
PA_R = {h: torch.linalg.qr(torch.randn(HD, 16))[0].to(DEV).contiguous() for h in range(NH)}
EPC96_R = torch.linalg.qr(torch.randn(D, 96))[0].to(DEV).contiguous()
EW_R = F.rms_norm(wte, (D,)) @ EPC96_R
from tier2_folding import scores_from_factors
TBL = {k: v.to(DEV) for k, v in torch.load(f'{QK}/qk_l2_tables.pt', map_location=DEV)['tables'].items()}
...
PB_R = {h: torch.linalg.qr(torch.randn(HD, 16))[0].to(DEV).contiguous() for h in range(NH)}
print('l1 bases ready', flush=True)


@torch.no_grad()
def codes_of(idx, y0, y1, EWx=None, PAx=None, PBx=None):
    EWx = EW if EWx is None else EWx
    PAx = PA if PAx is None else PAx
    PBx = PB if PBx is None else PBx
    B, T = idx.shape
    ce = EWx[idx.reshape(-1)]
    c0 = torch.cat([(y0[..., h, :].reshape(-1, HD) - mu0[h].float()) @ PAx[h]
                    for h in range(NH)], 1)
    c1 = torch.cat([(y1[..., h, :].reshape(-1, HD) - mu1[h].float()) @ PBx[h]
                    for h in range(NH)], 1)
...
AtA_R = torch.zeros(KDIM + 1, KDIM + 1, device=DEV, dtype=torch.float64)
AtY_R = torch.zeros(KDIM + 1, D, device=DEV, dtype=torch.float64)
ss_y = 0.0
n_fit = 0
with torch.no_grad():
    for i in range(0, 512, 4):
        ...
        Cr = codes_of(b[:, :-1], y0, y1, EW_R, PA_R, PB_R).double()
        Cr = torch.cat([Cr, torch.ones(Cr.shape[0], 1, device=DEV, dtype=torch.float64)], 1)
...
W_R = torch.linalg.solve(AtA_R + 10.0 * torch.eye(KDIM + 1, device=DEV, dtype=torch.float64), AtY_R)
WfR = W_R.float()
...
@torch.no_grad()
def forward_symbolic(idx, mode='sym'):
...
MODES = ['base', 'sym', 'rand', 'tab']
perdoc = {mo: [] for mo in MODES}
with torch.no_grad():
    for i in range(0, len(FINEWEB), 4):
        b = FINEWEB[i:i + 4].to(DEV)
        for mo in MODES:
            lg = forward_symbolic(b[:, :-1], 'sym' if mo == 'base' else mo).float() \
                if mo != 'base' else forward_symbolic(b[:, :-1], 'none').float()
            ce = F.cross_entropy(lg.reshape(-1, V), b[:, 1:].reshape(-1),
                                 reduction='none').view(b.shape[0], -1).mean(1)
            perdoc[mo].extend(ce.tolist())
import numpy as _np
PD = {mo: _np.array(v) for mo, v in perdoc.items()}
rng = _np.random.default_rng(0)
out = {}
for mo in ('sym', 'rand', 'tab'):
    d = PD[mo] - PD['base']
    bs = [float(d[rng.integers(0, len(d), len(d))].mean()) for _ in range(10000)]
    lo, hi = _np.percentile(bs, [2.5, 97.5])
    out[mo] = {'dce': round(float(d.mean()), 5), 'ci95': [round(float(lo), 5), round(float(hi), 5)]}
    print(f'{mo}: dCE {d.mean():+.5f}  95% CI [{lo:+.5f}, {hi:+.5f}]', flush=True)
for a_, b_ in (('tab', 'sym'), ('rand', 'sym')):
    d = PD[a_] - PD[b_]
    bs = [float(d[rng.integers(0, len(d), len(d))].mean()) for _ in range(10000)]
    lo, hi = _np.percentile(bs, [2.5, 97.5])
    out[f'{a_}_minus_{b_}'] = {'delta': round(float(d.mean()), 5),
                               'ci95': [round(float(lo), 5), round(float(hi), 5)]}
    print(f'{a_} - {b_}: {d.mean():+.5f}  95% CI [{lo:+.5f}, {hi:+.5f}]', flush=True)
json.dump(out, open(f'{QK}/qk_l2_bootstrap.json', 'w'), indent=2)
print('L2 BOOTSTRAP DONE', flush=True)
