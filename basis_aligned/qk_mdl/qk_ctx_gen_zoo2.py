...
ARMS = {
    ...
    'bilin': (Skip(64, Bilin(64, 150, 576), 576), C64),
    'swiglu': (Skip(64, Bilin(64, 150, 576, gate='silu'), 576), C64),
    'mlp': (Skip(64, nn.Sequential(nn.Linear(64, 180), nn.GELU(), nn.Linear(180, 576)), 576), C64),
    'hier': (Skip(64, Hier(), 576), C64),
}
out = {}
models = {}
for aname, (mod, X) in ARMS.items():
    ...
    print(f'{aname}: {nparam} params, val R2 {r2:.4f}, top-1% weight mass {top1:.3f}',
          flush=True)
    json.dump(out, open(f'{QK}/qk_ctx_gen_zoo2.json', 'w'), indent=2)
