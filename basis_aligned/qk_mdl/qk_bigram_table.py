"""TICK 215: BIGRAM CORRECTION TABLE for layer-1 head 1 — the residual program's
highest-resolution arm. Hypothesis (ticks 212-214): the missing generator signal is
high-resolution lexical identity of the immediately preceding token, concentrated in
l1-h1 (56% of remaining damage). Test: a correction TABLE keyed on the PREVIOUS token
id — shrunk mean of the stage-1 residual (adapter coords, h1's four maps only),
grouped by prev token, tau=8 toward zero — added on top of the stage-1 generator for
h1 channels only. If a bigram lookup lands near the h1-oracle repair (+0.0203 vs
stage-1 +0.0319), the missing computation is literally "prev-token-conditional key
shift", a nameable, MDL-priceable object. Arms: prev-token table; (prev+2back) sum of
two tables. Bits: V x 64 coords; priced at fp16 and count>=4-support variants.

Original tick-214 header:
"""
...
base = per_pos(None)
genl = per_pos('gen')
delta_s1 = float((genl - base).flatten().mean())
out = {'stage1_eval_dce': round(delta_s1, 5)}
print(f"stage1 eval dCE {delta_s1:+.5f}", flush=True)

# ---- stage-1 residual on capture, h1 channels (4 maps x 16) ----
with torch.no_grad():
    PRED1 = torch.cat([gen(CMIX[i:i + 65536]) for i in range(0, N, 65536)])
H1_IDX = []
for mi, (name, _) in enumerate(MAPS):
    b0 = mi * NH * R_AD + 1 * R_AD
    H1_IDX += list(range(b0, b0 + R_AD))
H1_IDX = torch.tensor(H1_IDX, device=DEV)
RT = (Y - PRED1)[:, H1_IDX]                                # (N, 64)

IDS2D = COOC[:N_CAP, :-1]
prev1 = torch.cat([IDS2D[:, :1], IDS2D[:, :-1]], 1).reshape(-1)[:N]
prev2 = torch.cat([IDS2D[:, :1].expand(-1, 2), IDS2D[:, :-2]], 1).reshape(-1)[:N]


def fit_table(keys, R):
    cntt = torch.bincount(keys[:ntr], minlength=V).float()
    sm = torch.zeros(V, R.shape[1])
    sm.index_add_(0, keys[:ntr], R[:ntr].cpu())
    tab = sm / cntt[:, None].clamp_min(1)
    tab = (cntt / (cntt + 8.0))[:, None] * tab
    return tab.to(DEV), cntt


TB1, cnt1 = fit_table(prev1, RT)
res_after1 = RT[ntr:] - TB1[prev1[ntr:].to(DEV)]
r2_1 = 1 - float((res_after1 ** 2).sum()) / float((RT[ntr:] ** 2).sum())
TB2, _ = fit_table(prev2, RT - TB1[prev1.to(DEV)].cpu().to(RT.device) if RT.is_cuda else RT - TB1[prev1.to(DEV)])
res_after2 = RT[ntr:] - TB1[prev1[ntr:].to(DEV)] - TB2[prev2[ntr:].to(DEV)]
r2_2 = 1 - float((res_after2 ** 2).sum()) / float((RT[ntr:] ** 2).sum())
out['bigram_val_r2'] = round(r2_1, 4)
out['bigram2_val_r2'] = round(r2_2, 4)
print(f"bigram table val R2 on h1 residual: prev1 {r2_1:.4f}; +prev2 {r2_2:.4f}", flush=True)
json.dump(out, open(f'{QK}/qk_bigram_table.json', 'w'), indent=2)

# ---- end-to-end eval with stage1 + bigram (h1 only) ----


@torch.no_grad()
def per_pos_bigram(two=False, batch=4):
    outs = []
    for i in range(0, N_EVAL, batch):
        b = SEQS[i:i + batch].to(DEV)
        idx = b[:, :-1]
        p1 = torch.cat([idx[:, :1], idx[:, :-1]], 1)
        p2 = torch.cat([idx[:, :1].expand(-1, 2), idx[:, :-2]], 1)
        dt = m.transformer.wte.weight.dtype
        x = m.transformer.wte(idx)
        x = F.rms_norm(x, (x.size(-1),))
        x0 = x
        ...
        for li, blk in enumerate(m.transformer.h):
            ...
            def factors(lin, name=None):
                ...
                if li == 1 and name is not None:
                    pred = gen(feats_of(mo_c[0], yh_c[0], xin1_c[0]))
                    mi = {'q1': 0, 'k1': 1, 'q2': 2, 'k2': 3}[name]
                    corr_bg = TB1[p1.reshape(-1)][:, mi * R_AD:(mi + 1) * R_AD]
                    if two:
                        corr_bg = corr_bg + TB2[p2.reshape(-1)][:, mi * R_AD:(mi + 1) * R_AD]
                    tab = TABLES[name][idx]
                    zc = tab.clone()
                    base_i = mi * NH * R_AD
                    for h in range(NH):
...
for two in (False, True):
    lm2 = per_pos_bigram(two=two)
    d = float((lm2 - base).flatten().mean())
    key = 'bigram2_eval_dce' if two else 'bigram_eval_dce'
    out[key] = round(d, 5)
    print(f"stage1 + bigram{'x2' if two else ''} (h1): eval dCE {d:+.5f} "
          f"(stage1 {delta_s1:+.5f}, h1-oracle +0.0203)", flush=True)
    json.dump(out, open(f'{QK}/qk_bigram_table.json', 'w'), indent=2)

# bits pricing
supp = int((cnt1 >= 4).sum())
out['bits'] = {'full_fp16_Mbit': round(V * 64 * 16 / 1e6, 1),
               'support_cnt4_fp16_Mbit': round(supp * 64 * 16 / 1e6, 1),
               'support_tokens': supp}
print('bits:', out['bits'], flush=True)
json.dump(out, open(f'{QK}/qk_bigram_table.json', 'w'), indent=2)
print('BIGRAM TABLE DONE', flush=True)
