"""MED PHASE 10: second-seed model for filter-reproducibility test."""
import sys, json
import numpy as np
import torch
...
for step in range(12000):
    ...
    if (step + 1) % 1500 == 0:
        ...
        print(f'step {step+1}: loss {loss.item():.3f} val {va:.4f} (best {best:.4f})',
              flush=True)
ckpt = torch.load(f'{QK}/med_seed1.pt', map_location=DEV)
...
json.dump(out, open(f'{QK}/med_seed1.json', 'w'), indent=2)
print('MED SEED1 DONE', flush=True)
