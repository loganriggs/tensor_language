"""TICK 235: SELF-DISTILLED MEMORY — mine the pair table from the MODEL, not a
corpus. Tick 234 estimated the MLP holds ~1.1M pairs (~70 Mbit explicit) and that the
barrier is identifying WHICH pairs; the model itself is the oracle. Keys: cooc-corpus
trigrams (clean arm) and, as a labeled DIAGNOSTIC-ONLY upper bound, trigrams from the
audit texts themselves (key-selection leakage, values still model-mined). Values: the
model's own confident next-token prediction given just the 3-token key (its memory,
read out; mining cost is offline like training — the table is priced at its bits).
Arms: named + self-mined (clean); named + self-mined (diagnostic). Anchors: named
+0.03087; corpus-datastore nulls.
...
print('self-distilling memory from the model...', flush=True)
CO = COOC.numpy()
keyset = set()
for d in range(CO.shape[0]):
    row = CO[d]
    for p in range(2, len(row) - 1):
        keyset.add((int(row[p - 2]), int(row[p - 1]), int(row[p])))
AUD = FINEWEB.numpy()
keyset_diag = set()
for d in range(AUD.shape[0]):
    row = AUD[d]
    for p in range(2, len(row) - 1):
        keyset_diag.add((int(row[p - 2]), int(row[p - 1]), int(row[p])))
all_keys = list(keyset | keyset_diag)
print(f'{len(keyset)} clean keys; {len(keyset_diag)} audit keys; '
      f'{len(all_keys)} union to query', flush=True)
DS3, DS3_DIAG = {}, {}
import math
with torch.no_grad():
    for i in range(0, len(all_keys), 2048):
        chunk = all_keys[i:i + 2048]
        kb = torch.tensor(chunk, device=DEV)
        logits = reference_forward(m, kb, 'bf16')[:, -1].float()
        pr = F.softmax(logits, 1)
        conf, tokp = pr.max(1)
        for j, k in enumerate(chunk):
            if float(conf[j]) > 0.35:
                ent = (int(tokp[j]), float(-math.log(max(1e-9, 1 - float(conf[j])))))
                if k in keyset:
                    DS3[k] = ent
                DS3_DIAG[k] = ent
        if i % (2048 * 80) == 0:
            print(f'  {i}/{len(all_keys)} queried; kept {len(DS3_DIAG)}', flush=True)
DS2 = {}
print(f'self-mined: clean {len(DS3)} keys, diagnostic {len(DS3_DIAG)}', flush=True)
ACTIVE = {'t': DS3}
def retrieve_rows(tokrows):
    ...
    for b in range(B):
        ...
        for p in range(T):
            ...
            hit = None
            if p >= 2:
                hit = ACTIVE['t'].get((int(row[p - 2]), int(row[p - 1]), int(row[p])))
                if hit is not None:
                    rn[gi] = 3
            if hit is not None:
                rt[gi] = hit[0]
...
for aname, (mod, X) in ARMS.items():
    ...
    print(f'{aname}: {nparam} params, val R2 {r2:.4f}, top-1% weight mass {top1:.3f}',
          flush=True)
    json.dump(out, open(f'{QK}/qk_selfmined.json', 'w'), indent=2)
...
for aname in ARMS:
    ...
    json.dump(out, open(f'{QK}/qk_selfmined.json', 'w'), indent=2)
# diagnostic arm: swap in the audit-keyed table (labeled leakage; upper bound only)
ACTIVE['t'] = DS3_DIAG
for aname in ARMS:
    ce = audit(aname)
    out[aname + '_DIAGNOSTIC_dce'] = round(ce - BASE, 5)
    print(f'{aname} DIAGNOSTIC (audit-keyed, upper bound): dCE {ce - BASE:+.5f}',
          flush=True)
    json.dump(out, open(f'{QK}/qk_selfmined.json', 'w'), indent=2)
print('SELF MINED DONE', flush=True)
