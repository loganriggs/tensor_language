"""COORDINATE SEMANTICS, layer 0, under the meaning gate. Each layer-0 PCA coordinate c of head h
...
BASIS = []   # per head: (D,64) ORTHONORMAL write basis (QR; fixes the broken-frame identity ref)
for hh in range(NH):
    ev, evec = torch.linalg.eigh(acc[hh])
    W_ = cw[:, hh*HD:(hh+1)*HD] @ evec[:, ev.argsort(descending=True)[:64]].float()
...
print(f"codable at R2>=0.8: {codable}/576 ({frac:.0%}); median spectrum R2 {np.median(r2s):.3f}", flush=True)
...
def forward_spec(idx, spectra):
    ...
    for li in range(NL):
        ...
        if li == 0 and spectra is not None:
            ...
            for hh in range(NH):
                sp = spectra[hh][idx.reshape(-1)].view(B, T2, 64)      # (B,T,64) per-key coord writes
...
json.dump(res, open(f'{QK}/qk_coord_semantics_l0.json', 'w'), indent=2)
json.dump(results, open(f'{QK}/qk_coord_semantics_l0_table.json', 'w'), indent=2)
print("QK COORD SEMANTICS L0 DONE", flush=True)
