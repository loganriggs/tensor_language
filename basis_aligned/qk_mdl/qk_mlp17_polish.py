...
WANT = 17
A512, U512 = torch.load(f'{QK}/qk_mlp17_program.pt', map_location=DEV)['table_prev_R512']
FLOOR1 = json.load(open(f'{QK}/qk_completeness_ledger.json'))['mlp_floor'][str(17)]
...
g = torch.nn.Parameter(torch.ones(512, device=DEV))
...
json.dump(res, open(f'{QK}/qk_mlp17_polish.json', 'w'), indent=2)
torch.save({'g': g.detach(), 'ab': ab.detach()}, f'{QK}/qk_mlp17_polish.pt')
print("QK MLP17 POLISH DONE", flush=True)
