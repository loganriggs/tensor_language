"""TICK 243: pipeline generality — the tick-239 battery on RANDOM strong-key
positions (any position where some context token in the last 16 carries >1 nat),
not worst-failure clusters. If the numbers match tick 239, the memory pipeline is
the model's general mechanism of context use, not a tail pathology."""
import json, sys
import numpy as np
import torch
...
json.dump(out, open(f'{QK}/qk_pipeline_generality.json', 'w'), indent=2)
