"""Stage-B v2: menu-static EXCEPT L5 heads H5,H7 stay LIVE (the two heads
...
STEPS = 4500
BATCH = 2
OUT = '/workspace/tensor_language/basis_aligned/qk_mdl/menu2_trained.json'
LIVE5 = (5, 7)
...
dT = audit() - base
res['ce_trained'] = dT
print(f'MENU2 (L5 H5,H7 LIVE) vq{K} CE-trained: dCE {dT:+.4f}', flush=True)
with open(OUT, 'w') as fh:
    json.dump(res, fh, indent=2)
torch.save({f'{L}_{h}_{br}': cbs[(L, h, br)].detach().cpu()
            for (L, h, br) in cbs}, '/workspace/tensor_language/basis_aligned/qk_mdl/menu2_cbs_trained.pt')
print('menu2 trained done', flush=True)
