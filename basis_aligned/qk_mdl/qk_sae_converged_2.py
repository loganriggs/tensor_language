"""DEFINITIVE follow-up to §78 -- SCRIPT 2: the CAUSAL test at CONVERGED fidelity.

Loads the converged TOP-K dictionary trained by qk_sae_converged.py and asks whether its
high-fidelity features are CAUSALLY load-bearing -- the decisive head-to-head with §74/§78,
where 0 of 32 single SVD directions and 0 of 32 §78 L1-SAE features cleared the causal bar
(z>=3 and trigger delta cross-entropy >= 0.02 nats).

For each top-32 feature (activation frequency x norm): MEAN-ABLATE its contribution to the
MLP1 output residual write -- subtract (f_i - per-position-held-mean) * decoder_direction_i --
and measure global + trigger-restricted delta cross-entropy on the held slice with PAIRED
standard errors. Then a CUMULATIVE sweep vs the full-MLP1 mean-ablation effect (5.57 nats in
§78). Positive control: remove SAE reconstruction vs remove SAE residual.

FORWARD + per-position mean-ablation + paired-SE convention COPIED VERBATIM from
qk_redteam_sae_hub_2.py / qk_mlp1_tail.py. Only the encode() is the TopK encode (matching
qk_sae_converged.py) instead of dense ReLU. Held-back FW[448:600,:128].
"""
...
# ---------------- load dictionary ----------------
Z = np.load(f'{QK}/qk_sae_converged.npz')
...
print(f"loaded converged dictionary NFEAT={NFEAT} k={K}; testing top-{len(TOP_FEATS)} features causally", flush=True)

# decoder directions mapped back to ORIGINAL mo space (VERBATIM)
DEC_ORIG = (W_dec.T / SCALE).contiguous()             # (NFEAT, D)

def encode(mo):
    """TopK encode matching qk_sae_converged.py (relu pre-activations, keep k largest)."""
    pre = F.relu(((mo - MU) * SCALE - b_dec) @ W_enc.T + b_enc)   # (B,T,NFEAT)
...
# =====================================================================================
# PASS A: per-position feature means (held) + tested-feature activations (trigger masks).
# =====================================================================================
...
# =====================================================================================
# PASS B: per single feature + cumulative top-N configs, against a single base pass.
# =====================================================================================
...
# =====================================================================================
# PASS B2: full-MLP1 mean-ablation reference + positive controls (remove residual / recon).
# =====================================================================================
    ...
    for li in range(NL):
        ...
        if li == LI:
            ...
            if recon_only:
                ...
                mo = xhat_n / SCALE + MU                       # keep reconstruction, drop residual
...
resid_sum = resid_sq = 0.0; resid_n = 0        # remove SAE residual (keep reconstruction)
for i in range(0, NHELD, BATCH):
...
summary = {
    ...
    'l1_sae_reference_single_clear': '0 of 32 (§78, held FVE 0.69)',
...
out = {
    'meta': {
        ...
        'ablation': ('per-feature mean-ablation: mo -= (f_i - per-position-held-mean_i) * decoder_dir_i, exact '
                     'analog of §74/§78 mean-ablation with converged TopK decoder direction + TopK coefficient'),
        'currency': 'mean-ablation delta cross-entropy per valid held position (nats), paired SE; attention intact',
        'causal_bar': f'trigger_dCE_z >= {CLEAR_Z} AND trigger_dCE >= {CLEAR_DCE} nats (§74 bar; SVD 0/32, §78 L1-SAE 0/32)',
        'forward': 'VERBATIM qk_redteam_sae_hub_2.py / qk_mlp1_tail.py',
    },
    'summary': summary,
    'single_feature_records_by_z': records_by_z,
    'cumulative_curve': cum_curve,
}
json.dump(out, open(f'{QK}/qk_sae_converged_causal.json', 'w'), indent=2)

print("\n===== CAUSAL HEAD-TO-HEAD at CONVERGED fidelity (delta cross-entropy, nats) =====", flush=True)
print(f"full-MLP1 mean-ablation {full_mean:.4f}", flush=True)
print(f"single features clearing §74 bar (z>=3 & trigger dCE>=0.02): {n_clear}/{len(tested)}  (SVD 0/32, §78 L1-SAE 0/32)", flush=True)
...
for c in cum_curve:
    print(f"  top-{c['n_features']:5d} feats: captured dCE {c['captured_global_dCE']:.4f} "
          f"({c['frac_of_full_MLP1']} of full)", flush=True)
print("\nSaved qk_sae_converged_causal.json", flush=True)
print("QK SAE CONVERGED (script 2: causal) DONE", flush=True)
