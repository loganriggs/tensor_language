"""MED PHASE 14c (texture confounder): plant confounders, train, verify reliance. Then (phase 14b) a
...
def stamp(x):
    x = x.clone()
    x[:, :, 0:7, 0:7] = (x[:, :, 0:7, 0:7] + _cb).clamp(0, 1)  # mean-preserving checkerboard in patch 0
    return x
...
json.dump(res, open(f'{QK}/med_confound2.json', 'w'), indent=2)
print('MED CONFOUND2 DONE', flush=True)
