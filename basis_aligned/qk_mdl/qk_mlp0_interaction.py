"""TN-native explicit-program substitution for MLP 0 (Logan: which methods rely on the tensor
...
ARMS = {}
resid = Y - TT[TOK]
for name, tgt, R in [('table_only', None, 0), ('table_R64', resid, 64), ('table_R256', resid, 256), ('R256_alone', Y - gmean, 256)]:
    if R == 0: ARMS[name] = (None, None); print(f"{name}: (no fit)", flush=True); continue
    A, U, fvu = fit_interaction(tgt, R)
    ...
    for i in range(0, len(FINEWEB), 4):
        ...
        for li in range(NL):
            ...
            if li == 0:
                ...
                if arm == 'R256_alone':
                    mo = (gmean + ((flat @ A.T)**2) @ U).view(B, T, D)
                else:
                    mo = TT[idx.reshape(-1)].view(B, T, D)
                    if A is not None: mo = mo + (((flat @ A.T)**2) @ U).view(B, T, D)
                x = x + mo.to(x.dtype)
            else:
                x = x + blk.mlp(hin)
...
json.dump(res, open(f'{QK}/qk_mlp0_interaction.json', 'w'), indent=2)
torch.save({k: v for k, v in ARMS.items() if v[0] is not None}, f'{QK}/qk_mlp0_interaction.pt')
print("QK MLP0 INTERACTION DONE", flush=True)
