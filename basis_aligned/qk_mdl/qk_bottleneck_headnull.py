...
def rand_bases():
    ...
    return Q
QB['rand'] = rand_bases()
def headspan_bases(seed):
    gg = torch.Generator(device=DEV).manual_seed(seed)
    Q = []
    for li in range(NL):
        cw = m.transformer.h[li].attn.c_proj.weight.detach().float(); cs = []
        for hh in range(NH):
            Rh, _ = torch.linalg.qr(torch.randn(HD, 64, generator=gg, device=DEV))
            cs.append(cw[:, hh*HD:(hh+1)*HD] @ Rh)
        Qx, _ = torch.linalg.qr(torch.cat(cs, 1)); Q.append(Qx)
    return Q
QB['headspan_s1'] = headspan_bases(11)
QB['headspan_s2'] = headspan_bases(12)
...
for PER in (64, 'headspan_s1', 'headspan_s2'):
    d = audit(PER, 17) - SUBBASE
    res[f'{PER}_L17'] = round(d, 5)
    print(f"bottleneck all-18: basis={PER}: dCE +{d:.5f}", flush=True)
json.dump(res, open(f'{QK}/qk_bottleneck_headnull.json', 'w'), indent=2)
print("QK BOTTLENECK HEADNULL DONE", flush=True)
