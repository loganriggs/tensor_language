...
# condition B: induction heads, NON-active first-copy trigger position
gB = torch.zeros(B0, T0, dtype=torch.bool, device=DEV); gB[:, TRIG_POS] = True; gB &= ~active

@torch.no_grad()
def reach_aim(aim_col, scale):
    x = F.rms_norm(m.transformer.wte(idx), (D,)); x0 = x; v1 = None
    cos, sin = rope_tables(T0, HD, DEV, x.dtype, 'bf16'); cosb, sinb = cos[None, :, None, :], sin[None, :, None, :]
    ...
    return {'P_tok@A1': round(float(pr.gather(-1, t1[:, None]).mean()), 4),
            'P_tok@A2': round(float(pr.gather(-1, t2[:, None]).mean()), 4)}

res = {'note': 'condition B (no natural match); does it copy the POINTED source?', 'sweep': {}}
for sc in SCALES:
    res['sweep'][f'scale={sc}'] = {'aim@col1': reach_aim(A1, sc), 'aim@col10': reach_aim(A2, sc)}
    print(f"scale {sc}:", res['sweep'][f'scale={sc}'], flush=True)
json.dump(res, open(f'{QK}/qk_nonactive_aimability.json', 'w'), indent=2)
print("QK NONACTIVE AIMABILITY DONE", flush=True)
