"""T8: does the composition arc GENERALIZE? Port the two load-bearing results to bilin12
...
g1=gate1(); print(f"GATE1 MLP-gauge (bilin12): rel-err {g1:.2e}", flush=True)
...
res={'model':'bilin12','base_CE':round(base,4),'gate1_mlp_gauge_relerr':g1,
     'pca_bottleneck':rep(ptok(QB['pca'])),'headspan_null':rep(ptok(QB['null']))}
print(f"bilin12 PCA/head bottleneck: dCE +{res['pca_bottleneck']['dCE']} (SE {res['pca_bottleneck']['SE']}) | null +{res['headspan_null']['dCE']}", flush=True)
json.dump(res,open(f'{QK}/qk_bilin12_composition.json','w'),indent=2)
