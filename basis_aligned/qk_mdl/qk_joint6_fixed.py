"""BUGFIX RERUN (red-team finding 1): the joint6 arm previously injected MLP deltas with UNIT
coefficients; correct coefficients are prod of downstream lambdas (coef of m_l in xb_5). This rerun
measures joint6 with CORRECT scaling. Original docstring: Extend the fully-analytic truncated chain through layers 3-5 at K=576 (64 dims/head, the width
    ...
    for i in range(0, len(FINEWEB), 4):
        ...
        for li in range(NL):
            ...
            if mode == 'joint6' and li == LMAX:
                ...
                # CORRECT coefficients: coef of m_l in xb_5 = prod_{i=l+1..5} lam_{i,0}
                cprod = []
                for l in range(LMAX):
                    c = 1.0
                    for i2 in range(l+1, LMAX+1): c *= lam[i2][0]
                    cprod.append(c)
                delta = sum(cprod[l]*(mh[l] - cache_m[l]) for l in range(LMAX)).view(B, T2, D)
...
json.dump(res, open(f'{QK}/qk_joint6_fixed.json', 'w'), indent=2)
print("QK JOINT6 FIXED DONE", flush=True)
