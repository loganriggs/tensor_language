"""§80 GENERALITY test -- SCRIPT 2: the CAUSAL test on swiglu18's LAYER-2 hub.

Loads the best-VAL TopK dictionary trained on swiglu18's layer-2 feed-forward output by
qk_sae_swiglu_hub.py and asks whether its features are CAUSALLY load-bearing on the canonical
held slice FW[448:600] -- the §80 head-to-head. For each top-32 feature: MEAN-ABLATE its
contribution to the layer-2 output residual write (mo -= (f_i - per-position-held-mean_i) *
decoder_direction_i) and measure global + trigger-restricted delta cross-entropy with PAIRED
standard errors. Cumulative sweep vs the full-layer-2 mean-ablation reference. Positive controls:
remove SAE reconstruction vs remove SAE residual. Merges into qk_sae_swiglu_hub.json under 'causal'.

CAUSAL harness / per-position mean-ablation / paired-SE convention / TopK encode() COPIED VERBATIM
from qk_sae_moredata_2.py (§80). ONLY changes: (i) swiglu18 not bilin18; (ii) LI=2; (iii) swiglu18
SOFTMAX forward (VERBATIM qk_content_gate_swiglu18.py); (iv) loaded npz = qk_sae_swiglu_hub.npz.
Held-back canonical FW[448:600,:128].
"""
...
m, cfg = load_elriggs('swiglu18')                 # CHANGE (i)
...
SEQL = 128
HELD = FINEWEB[448:600, :SEQL].to(DEV)     # SAME canonical held slice as §74-§80
NHELD = HELD.shape[0]
BATCH = 6
KCAUSAL = 200
...
# ---------------- load dictionary (swiglu18 layer-2 best-VAL SAE) ----------------
Z = np.load(f'{QK}/qk_sae_swiglu_hub.npz')       # CHANGE (iv)
...
print(f"loaded swiglu18 layer-2 dictionary NFEAT={NFEAT} k={K}; testing top-{len(TOP_FEATS)} features causally", flush=True)
...
# ---------------- swiglu18 SOFTMAX forward with layer-2 feature ablation (CHANGE iii) ----------------
@torch.no_grad()
...
# PASS B2: full-layer-2 ref + positive controls (VERBATIM §80, swiglu forward)
print("PASS B2: full-layer-2 mean-ablation reference + positive controls ...", flush=True)
@torch.no_grad()
...
print(f"full-layer-2 mean-ablation global dCE {full_mean:.4f} +- {full_se:.4f}", flush=True)
...
for n in CUM_NS:
    ...
    cum_curve.append({'n_features': n, 'captured_global_dCE': round(cm, 5), 'SE': round(cse, 5),
                      'frac_of_full_layer2': round(cm/full_mean, 4) if full_mean > 0 else None})

def feats_for_frac(frac):
    for c in cum_curve:
        if c['frac_of_full_layer2'] is not None and c['frac_of_full_layer2'] >= frac:
            return c['n_features']
    return None
...
causal = {
    'meta': {'held_slice': 'canonical FW[448:600,:128]', 'BATCH': BATCH, 'KCAUSAL': KCAUSAL,
             ...
             'forward': 'CAUSAL harness VERBATIM qk_sae_moredata_2.py (§80); swiglu forward VERBATIM qk_content_gate_swiglu18.py'},
    'summary': {
        'full_layer2_meanabl_global_dCE': round(full_mean, 5), 'full_layer2_meanabl_global_dCE_SE': round(full_se, 5),
        ...
        'all_dictionary_features_frac_of_full': round(max(c['frac_of_full_layer2'] for c in cum_curve), 4),
        'n_features_tested_individually': len(tested),
        'n_single_features_clear_74_bar': n_clear,
        'svd_reference_swiglu18': '0 of 32 (§77, layer-2)',
        'bilin18_sae_reference': '0 of 32 (§80, MLP1)',
...
outpath = f'{QK}/qk_sae_swiglu_hub.json'
try:
...
print("\n===== CAUSAL HEAD-TO-HEAD on swiglu18 layer-2 (delta cross-entropy, nats) =====", flush=True)
print(f"full-layer-2 mean-ablation {full_mean:.4f}", flush=True)
print(f"single features clearing §74 bar (z>=3 & trigger dCE>=0.02): {n_clear}/{len(tested)}  "
      f"(swiglu18 SVD 0/32 §77, bilin18 SAE 0/32 §80)", flush=True)
...
print(f"cumulative features for 50%/80% of full-layer-2 effect: {n50}/{n80}", flush=True)
print("\nMerged causal results into qk_sae_swiglu_hub.json", flush=True)
print("QK SAE SWIGLU HUB (script 2: causal) DONE", flush=True)
