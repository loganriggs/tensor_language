"""§34 CONTENT-NAMEABILITY PROTOCOL on swiglu18 (softmax + gated-bilinear MLP twin of bilin18).

VERBATIM COPY of qk_content_gate.py with ONLY these edits (see DIFF FROM qk_content_gate.py below):
  (a) model: load_elriggs('bilin18') -> load_elriggs('swiglu18');
  (b) attention convention: bilin18's TWO-BRANCH squared attention
        pat = ((qk/HD)*(q2k2/HD)).masked_fill(~mask, 0.0)                (NO softmax, unnormalized)
      replaced EVERYWHERE (lL_heads, blockL_input, forward_spec) by swiglu18's SOFTMAX convention,
      copied verbatim from qk_swiglu18_commandeer.py / qk_bilinsm12_composition.py:
        sc  = (einsum('bqhd,bkhd->bhqk', q, k)/(HD**0.5)).masked_fill(~mask, float('-inf'))
        pat = F.softmax(sc, -1)
      swiglu18 has no c_q2/c_k2, so the second branch is dropped.
  (c) value path: UNCHANGED. swiglu18 has attn.lamb on every layer (verified 0.50, 0.069, -0.004,
      0.015, 0.36, 0.45 at layers 0/1/6/11/16/17), identical v1-lerp v=(1-lamb)*v+lamb*v1 as bilin18.
  (d) feed-forward: UNCHANGED. The forward loops call blk.mlp(F.rms_norm(x,(D,))) as a black box, so
      whatever MLP swiglu18 carries (a gated Bilinear module here) is applied faithfully with no edit.
  (e) output file -> qk_content_gate_swiglu18_L{L}.json.

Everything else -- mean-residual table (shrink tau=8), PCA-64 per-head write basis, varimax, independent
grammar/orthography/frequency class library, frequency-weighted per-coordinate class R^2, exact/coded/
spike substitution gates, held slice FW[448:600] -- is byte-for-byte the qk_content_gate.py machinery.

USAGE: python qk_content_gate_swiglu18.py <L>    (L in 1..17)
"""
import json, sys
import numpy as np
import torch
...
# ---- layer-L head outputs (full forward captured at layer L); swiglu18 SOFTMAX attention ----
@torch.no_grad()
def lL_heads(idx):
    B, T = idx.shape
...
# ---- mean-residual table (blockL_input + shrink); swiglu18 SOFTMAX attention ----
@torch.no_grad()
...
def forward_spec(idx, spectra):
    ...
    swiglu18 SOFTMAX attention everywhere."""
    B, T2 = idx.shape
    x0 = F.rms_norm(m.transformer.wte(idx), (D,)); x = None; v1 = None
...
json.dump(res, open(f'{QK}/qk_content_gate_swiglu18_L{L}.json', 'w'), indent=2)
json.dump(results, open(f'{QK}/qk_content_gate_swiglu18_L{L}_table.json', 'w'), indent=2)
print(f"QK SWIGLU18 L{L} CONTENT GATE DONE", flush=True)
