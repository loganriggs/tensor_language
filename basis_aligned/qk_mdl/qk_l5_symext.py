"""TICK 257: LAYER-5 pattern with the 528-symbol dictionary (l5 fold).
...
def blocks01(idx):
    ...
    return yhs[0], yhs[1], yhs[2], blk5.lambdas[0] * x + blk5.lambdas[1] * x0
...
with torch.no_grad():
    for i in range(0, len(FINEWEB), 4):
        ...
        n += b[:, 1:].numel()
ce = tot / n
out = {'decoder_R2': round(R2, 4), 'l5_symext_dce': round(ce - BASE, 5),
       'gates': {'tables': 0.18690, 'zero': 2.30304}}
print(f'layer-5 symbol-generated pattern: dCE {ce - BASE:+.5f} '
      f'(tables +0.18690, zero +2.30304)', flush=True)
json.dump(out, open(f'{QK}/qk_l5_symext.json', 'w'), indent=2)
print('L5 SYMEXT DONE', flush=True)
