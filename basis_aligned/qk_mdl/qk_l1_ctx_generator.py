"""TICK 205: closing the loop — a LINEAR GENERATOR for the context adapter.

Tick 204 priced the interface (16 dims/channel = 78% of the static gap) but projected
the TRUE factors at runtime. Here the corrections are GENERATED from the block-0 MLP
output alone: c = P64 @ mlp_out (shared 64-dim PCA of the realized MLP output), then
per channel a ridge-fit linear map G (16 x 64) produces the adapter coordinates:
  factor = table[token] + U16 @ G @ P64 @ mlp_out(position).
Everything (P64, G, U16) fit on the 64 disjoint co-occurrence documents; audited on
the standard 307k set with the full manual forward. If this lands near tick 204's
rank-16 number (+0.0113), the two-layer circuit is closed in fully compact form:
archetype tables -> MLP (as a black-box generator of a 64-dim signal) -> linear
adapters -> layer-1 archetype tables.
"""
import json
import sys
import numpy as np
...
# ---- capture deviations AND mlp outputs; fit P64, U16, G ----
print('capturing deviations and mlp outputs...', flush=True)


@torch.no_grad()
def block1_input_and_mlp(idx):
    dt = m.transformer.wte.weight.dtype
    x = m.transformer.wte(idx)
    x = F.rms_norm(x, (x.size(-1),))
    x0 = x
...
with torch.no_grad():
    for i in range(0, N_CAP, 4):
        b = COOC[i:i + 4].to(DEV)
        idx = b[:, :-1]
        xin1, mo = block1_input_and_mlp(idx)
...
MO = torch.cat(MOS)
MO_MEAN = MO.mean(0)
Xm = (MO - MO_MEAN).to(DEV)
_, _, VhM = torch.linalg.svd(Xm, full_matrices=False)
P64 = VhM[:64].T.contiguous()                              # (D, 64)
C = Xm @ P64                                               # (N, 64) context codes
G64 = C.T @ C + 1e-3 * len(C) * torch.eye(64, device=DEV)
UBANK, GBANK = {}, {}
R_ADAPT = 16
for name, _ in MAPS:
    ...
    for h in range(NH):
        ...
        U = Vh[:R_ADAPT].T.contiguous()                    # (128, 16)
        Yt = Xc @ U                                        # (N, 16) target coords
        G = torch.linalg.solve(G64, C.T @ Yt)              # (64, 16)
        UBANK[(name, h)] = (U, mu.contiguous())
        GBANK[(name, h)] = G.contiguous()
        del X, Xc, Yt
    del Dv
    torch.cuda.empty_cache()
DEVS = None
del Xm, C
torch.cuda.empty_cache()
print('generator fitted (P64 + 36 x G[64x16])', flush=True)
        ...
        def factors(lin, name=None):
            ...
            if li == 1 and rank is not None and name is not None:
                ...
                if rank == 0:
                    ...
                    ctx = ((mo_cache[0].float() - MO_MEAN.to(DEV)) @ P64)   # (B,T,64)
                    zc = tab.clone()
                    for h in range(NH):
                        U, mu = UBANK[(name, h)]
                        G = GBANK[(name, h)]
...
for r in (0, 16):
    ce = audit(r)
    tag = 'static' if r == 0 else 'generated16'
    out[f'{tag}_dce'] = round(ce - base, 5)
    print(f'{tag}: dCE {ce - base:+.5f} (tick-204 true-projected rank-16 was +0.01132)',
          flush=True)
    json.dump(out, open(f'{QK}/qk_l1_ctx_generator.json', 'w'), indent=2)
print('L1 CTX GENERATOR DONE', flush=True)
