"""ECG SCALING TEST (capability-vs-interpretability confound): ~4x model on the SPECIFIC codes (not 5 superclasses),
...
D, NH, HD, NL, INNER = 192, 6, 32, 3, 384
NCLS = len(CODES)
    ...
    if (step+1) % 2000 == 0:
        ...
        if m > best:
            ...
            torch.save({'state': net.state_dict(), 'MU': MU, 'SD': SD, 'codes': CODES,
                       ...
                       f'{QK}/ecg_codes_big.pt')
        print(f'step {step+1}: loss {loss.item():.3f} val macro-AUC {m:.4f} (best {best:.4f})', flush=True)
ck = torch.load(f'{QK}/ecg_codes_big.pt', map_location=DEV); net.load_state_dict(ck['state'])
...
json.dump(res, open(f'{QK}/ecg_codes_big.json', 'w'), indent=2)
print('ECG CODES BIG DONE', flush=True)
