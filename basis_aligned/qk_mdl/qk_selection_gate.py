"""LAYER-L GATED SELECTION VOCABULARY (census-predicate simultaneous-substitution gate at layer L;
parametrized -- verbatim copy of qk_l3_selection_gate.py with LAYER=3 -> LAYER=int(sys.argv[1])).
...
on FW[448:600]. It uses the CENSUS PREDICATE LIBRARY (MATCH_same, MATCH_prev, KEY_punct/func/cap/
newline/digit/subword, PREV1/2, MATCH_prev2, FIRST + positional TEMPLATE + causal), not hand-coded
per-head archetypes. Tests whether any layer-L head is gated-nameable (predicate gain >= 0.05).

USAGE: python qk_selection_gate.py <L>    (L in 1..17)

DERIVATION: copied VERBATIM from qk_l3_selection_gate.py (its feats() predicate library, patterns(),
per-head normal equations, heldout_r2(), and forward_coded simultaneous-substitution gate are reused
unchanged). CHANGES, layer-index / output only:
  - LAYER = int(sys.argv[1]) (was LAYER = 3); the GATE's PROGSET is restricted to programmatic heads
    AT LAYER L only (predicate gain >= 0.05), so forward_coded substitutes coded patterns solely at
    layer L (isolating the layer, exactly as qk_l1_selection_gate.py restricts to li == 1);
  - cooc batches 4 for memory safety;
  - output file qk_selection_gate_L{L}.json (was qk_l3_selection_gate.json).
The heldout_r2 / gain / gate machinery, predicate definitions, and paired-SE computation are byte-for-
byte the L3 originals -- nothing is paraphrased.
"""
import json, sys
import numpy as np
import torch
...
json.dump({'layer': LAYER, 'census': census, 'programmatic': prog,
          ...
          open(f'{QK}/qk_selection_gate_L{LAYER}.json', 'w'), indent=2)
