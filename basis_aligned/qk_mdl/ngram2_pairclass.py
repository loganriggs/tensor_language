"""Pair-CLASS correction tables (TN-factored bigram, dense estimation):
table(t_prev, t_cur) = unigram(t_cur) + CORR(class(t_prev), class(t_cur)),
classes = kmeans-256 on normed embedding rows; 65k correction rows, ~50
samples each, 100% coverage. Tests whether the bigram signal survives
class-factoring (estimable + nameable atoms). Original: Bottom-up contextual tables: the earliest streams
...
OUT = f'{QK}/ngram2_pairclass.json'
...
# ---- class map: kmeans-256 on normed embedding ----
E_hat = F.rms_norm(m.transformer.wte.weight.detach().float(), (D,)).to(DEV)
...
for _ in range(10):
    ...
    Cn.index_add_(0, a_, E_hat); c2.index_add_(0, a_, torch.ones(V, device=DEV))
    nz = c2 > 0; C0[nz] = Cn[nz]/c2[nz][:,None]
CLS = a_.cpu()
print('class map built', flush=True)

# ---- pass 0: pick frequent bigrams ----
uc = torch.arange(65536)          # all (class_prev, class_cur) codes are valid
CLS_DEV = CLS.to(DEV)


def bigram_index(prev, cur):
    return (CLS_DEV[prev.long()] * 256 + CLS_DEV[cur.long()]).long()
...
for nm in NG_STREAMS:
    ...
    TAB_U[nm] = t
    # correction = pairclass mean MINUS the pairclass-weighted unigram mean
    pc_mean = acc_b[nm] / cnt_b.clamp_min(1)[:, None]
    TAB_B[nm] = pc_mean                                    # raw pairclass mean of z
del acc_u, acc_b
# build unigram-mean-by-pairclass to convert to corrections at lookup:
# corr(code) = pcmean(code) - E[unigram(t_cur) | code]; approximate the second
# term at lookup time by unigram(t_cur) itself (exact per-position correction):
# lookup = unigram(t_cur) + (pcmean(code) - pcmean_unigram(code)); we estimate
# pcmean_unigram by accumulating unigram rows the same way — done via a cheap pass:
pcm_u = {nm: torch.zeros(65536, D) for nm in NG_STREAMS}
pcm_c = torch.zeros(65536)
for i in range(0, len(TRAIN), 64):
    b = TRAIN[i:i + 64, :-1]
    code = (CLS[b[:, :-1].reshape(-1).long()] * 256 + CLS[b[:, 1:].reshape(-1).long()]).long()
    pcm_c.index_add_(0, code, torch.ones(len(code)))
    for nm in NG_STREAMS:
        pcm_u[nm].index_add_(0, code, TAB_U[nm][b[:, 1:].reshape(-1).long()])
CORR = {}
for nm in NG_STREAMS:
    CORR[nm] = TAB_B[nm] - pcm_u[nm] / pcm_c.clamp_min(1)[:, None]
    CORR[nm][pcm_c < 8] = 0.0          # shrink unreliable cells to zero correction
    TAB_B[nm] = None
print(f'pairclass corrections built (cells with cnt>=8: {(pcm_c>=8).float().mean():.2f})', flush=True)
    ...
    for i in range(0, len(AUDIT), 4):
        ...
        for li, blk in enumerate(m.transformer.h):
            ...
            if use_bigram and nm in NG_STREAMS:
                tabs[nm] = (TAB_U[nm][idx_cpu] + CORR[nm][bif.clamp_min(0)]).to(DEV, x.dtype)
            else:
...
for W in (1, 2):
    for ub in (False, True):
        name = f'W={W} {"pairclass-corr bottom4" if ub else "unigram (ref)"}'
        d = audit_ce(W, ub) - base
        res['arms'][name] = round(d, 4)
        print(f'{name}: dCE {d:+.4f}', flush=True)
        with open(OUT, 'w') as fh:
            json.dump(res, fh, indent=2)
torch.save({'corr': {nm: t.half() for nm, t in CORR.items()},
            'tab_u': {nm: t.half() for nm, t in TAB_U.items()}, 'cls': CLS},
           f'{QK}/ngram2_pairclass.pt')
print('ngram2 pairclass done', flush=True)
