...
for arm, layers in (('L2', {2}), ('L5', {5}), ('L9', {9}), ('L13', {13}), ('L16', {16}),
                    ('bottom L1-6', set(range(1, 7))), ('top L7-17', set(range(7, 18)))):
    MLP_LAYERS = layers
    globals()['MLP_LAYERS'] = layers
    d = audit_ce(AUDIT_EARLY, tabs, W=4, reads=('mlp',)) - base
    res[f'mlp-read W=4, {arm}'] = d
    print(f'mlp-read W=4, {arm}: dCE {d:+.4f}', flush=True)
    with open(OUT, 'w') as fh:
        json.dump(res, fh, indent=2)
print('d mlpread probe done', flush=True)
