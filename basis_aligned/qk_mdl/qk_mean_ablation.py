"""TICK 262 (reviewer-2 control): mean-ablation vs zero-ablation for layer loads.
...
out = {}
@torch.no_grad()
def audit(kill, batch=4):
    tot, n = 0.0, 0
    for i in range(0, len(SEQS_FULL), batch):
        b = SEQS_FULL[i:i + batch].to(DEV)
        lg = run_ab(b[:, :-1], kill).float()
        ce = F.cross_entropy(lg.reshape(-1, lg.shape[-1]), b[:, 1:].reshape(-1))
        tot += ce.item() * b[:, 1:].numel()
        n += b[:, 1:].numel()
    return tot / n

FINEWEB_ALL = torch.from_numpy(np.load('/workspace/tensor_language/data_fineweb_tokens.npy').astype(np.int64))
SEQS_FULL = FINEWEB_ALL
ZERO_REF = {1: 2.70, 2: 0.39042, 3: 0.16455, 4: 0.34771, 5: 2.30304}
for L in (2, 3, 4, 5):
    ce = audit(('mean', L))
    out[f'l{L}_mean_dce'] = round(ce - BASE, 5)
    print(f'layer {L} mean-ablated: dCE {ce - BASE:+.5f} (zero {ZERO_REF[L]:+.5f})', flush=True)
    json.dump(out, open(f'{QK}/qk_mean_ablation.json', 'w'), indent=2)
print('MEAN ABLATION DONE', flush=True)
