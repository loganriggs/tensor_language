"""§34 CONTENT-NAMEABILITY PROTOCOL, LAYER L (parametrized; verbatim copy of qk_l3_content_gate.py
with the hardcoded layer index 3 replaced by L=int(sys.argv[1]) EVERYWHERE it appears). Each layer-L
head-value coordinate c of head h has a mean-residual-derived vocabulary spectrum
S_h[t, c] = <basis_hc, c_projL_h vs_h(t)>, where vs_h(t) is the layer-L effective head value of token t
built from the MEAN-RESIDUAL TABLE (the per-token mean of the layer-L input residual, shrunk tau=8
toward the embedding prior; the residual is the block-L lambda-mixed input, estimated exactly as
qk_l4_port.py's block4_input generalization -- blockL_input runs blocks 0..L-1 with v1-mixing then
returns block-L's lambda-mixed input) mixed with the layer-0 value (v1 mixing). BASIS is the per-head
PCA-64 orthonormal write basis of the actual layer-L head outputs (varimax-sparsified). Naming
hypotheses, both from §34: (a) BROAD-CLASS code S_hat[t,c] = alpha*1[t in C] + beta with C an
INDEPENDENT hand library of grammar/orthography classes; (b) 8-SPIKE code (top-8 tokens exact, rest =
weighted off-spike mean) for concentrated coordinates. Frequency-weighted spectrum R^2 / concentration
rank codability. The MEANING GATE is substitution on LAYER L: run the model with layer-L attention
writes replaced by (a) the exact mean-residual spectra (identity reference) and (b) coded spectra; if
coded ~ exact the names are causally verified. SEs on the held-back slice FW[448:600].

USAGE: python qk_content_gate.py <L>    (L in 1..17)

DERIVATION: copied VERBATIM from qk_l3_content_gate.py with layer-index-only edits (3 -> L):
  - forward loop range(4)->range(L+1), captured/returned at li==L (was li==3); PCA-64 basis on
    layer-L outputs; write projection cw = h[L].attn.c_proj (was h[3]);
  - block3_input -> blockL_input: runs blocks range(L) (with v1-mixing) then returns block-L's
    lambda-mixed input, using qk_l4_port.py's validated block4_input structure (loop range(L),
    return h[L] mix);
  - value spectra: a3 -> aL = h[L].attn; vs = (1-aL.lamb)*c_vL(table) + aL.lamb*c_v0(embedding);
    a0 = h[0].attn UNCHANGED (v1 is always the layer-0 value);
  - the substitution branch fires at li == L (was li == 3);
  - PCA / mean-residual batches 4 for memory safety; NC = 64, denominators NH*NC = 576;
  - output file qk_content_gate_L{L}.json / _table.json (was qk_l3_content_gate*.json).
"""
import json, sys
import numpy as np
import torch
...
# ---- layer-L head outputs (full forward captured at layer L), verbatim forward loop ----
@torch.no_grad()
def lL_heads(idx):
    B, T = idx.shape
...
# per-head PCA-64 basis on layer-L head outputs (standard recipe)
acc = torch.zeros(NH, HD, HD, device=DEV, dtype=torch.float64)
for i in range(0, 64, 4):
    f = lL_heads(COOC[i:i+4].to(DEV)[:, :128]).double()
    acc += torch.einsum('nhd,nhe->hde', f, f)
cw = m.transformer.h[L].attn.c_proj.weight.detach().float()
BASIS = []   # per head: (D,NC) ORTHONORMAL layer-L write basis (QR of top-NC PCA dirs)
for hh in range(NH):
...
print(f"layer-{L} PCA-64 basis built", flush=True)


# ---- mean-residual table (blockL_input + shrink); blockL_input = qk_l4_port block4_input at L ----
@torch.no_grad()
def blockL_input(idx):
    ...
    blkL = m.transformer.h[L]
    return blkL.lambdas[0] * x + blkL.lambdas[1] * x0
...
with torch.no_grad():
    for i in range(0, N_EST, 4):
        b = COOC[i:i + 4].to(DEV)
        idx = b[:, :-1]
        x = blockL_input(idx).float().reshape(-1, D)
        ids = idx.reshape(-1)
...
aL = m.transformer.h[L].attn
with torch.no_grad():
    Vv0 = a0.c_v(F.rms_norm(wte, (D,))).view(V, NH, HD).float()
    xn = F.rms_norm(shrunk, (D,))
    v_new = aL.c_v(xn).view(V, NH, HD).float()
    vs = (1 - aL.lamb) * v_new + aL.lamb * Vv0.view_as(v_new)
del shrunk, xn, v_new
torch.cuda.empty_cache()

# layer-L head-value spectra from the mean-residual table: S[h][t,c] = <basis_hc, c_projL_h vs_h(t)>
S = []
for hh in range(NH):
...
del vs
torch.cuda.empty_cache()
print(f"layer-{L} spectra built (mean-residual table)", flush=True)
...
def varimax(Phi, w, iters=60):
    ...
    for _ in range(iters):
        L_ = X @ R
        u, sv, vt = torch.linalg.svd(X.T @ (L_**3 - L_ @ torch.diag((L_**2).mean(0))), full_matrices=False)
        R = u @ vt
    return R
...
def forward_spec(idx, spectra):
    """layer-L attention writes replaced by pattern-weighted SPECTRA (exact or coded); rest real."""
    B, T2 = idx.shape
    x0 = F.rms_norm(m.transformer.wte(idx), (D,)); x = None; v1 = None
    ...
    for li in range(NL):
        ...
        q, k, q2, k2 = qk(a.c_q), qk(a.c_k), qk(a.c_q2), qk(a.c_k2)
        pat = ((torch.einsum('bqhd,bkhd->bhqk', q, k)/HD)*(torch.einsum('bqhd,bkhd->bhqk', q2, k2)/HD)).masked_fill(~mask, 0.0)
        if li == L and spectra is not None:
...
print(f"SUMMARY L{L} CONTENT: class-nameable {codable}/{NH*NC} (median R2 {np.median(r2s):.3f}); "
      f"class gate +{res['gate_coded_minus_exact']['dCE']:.5f}; spike {spikey}/{NH*NC} gate +{res['spike']['gate_dCE']:.5f} nats/tok", flush=True)
json.dump(res, open(f'{QK}/qk_content_gate_L{L}.json', 'w'), indent=2)
json.dump(results, open(f'{QK}/qk_content_gate_L{L}_table.json', 'w'), indent=2)
print(f"QK L{L} CONTENT GATE DONE", flush=True)
