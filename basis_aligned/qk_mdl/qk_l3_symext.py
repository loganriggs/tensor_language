"""TICK 253: layer-3 pattern with the dictionary EXTENDED by 144 layer-2 activations (528 symbols).
...
def blocks01(idx):
    ...
    return yhs[0], yhs[1], yhs[2], blk3.lambdas[0] * x + blk3.lambdas[1] * x0
...
with torch.no_grad():
    for i in range(0, len(FINEWEB), 4):
        ...
        n += b[:, 1:].numel()
ce = tot / n
out = {'decoder_R2': round(R2, 4), 'l3_symext_dce': round(ce - BASE, 5),
       'gates': {'tables': 0.03902, 'zero': 0.16455}}
print(f'layer-3 symbol-generated pattern: dCE {ce - BASE:+.5f} '
      f'(tables +0.03902, zero +0.16455)', flush=True)
json.dump(out, open(f'{QK}/qk_l3_symext.json', 'w'), indent=2)
print('L3 SYMEXT DONE', flush=True)
