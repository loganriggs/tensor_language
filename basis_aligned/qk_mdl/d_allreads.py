"""Windowed-D extended to ALL reads: q,k (selection), v (content), and the
...
def estimate(train):
    ...
    for i in range(0, len(train), 8):
        ...
        for li, blk in enumerate(m.transformer.h):
            ...
            x_mlp = x
            if W is not None and 'mlp' in reads:
...
for reads, name in ((('v',), 'v-read only'),
                    (('qk', 'v'), 'qk+v reads'),
                    (('qk', 'v', 'mlp'), 'ALL reads (qk+v+mlp)')):
    for Wa in (4, 6):
        d = audit_ce(AUDIT_EARLY, tabs, W=Wa, reads=reads) - base
        res[f'{name}, W={Wa}'] = d
        print(f'{name}, W={Wa}: dCE {d:+.4f}', flush=True)
        with open(OUT, 'w') as fh:
            json.dump(res, fh, indent=2)
print('d allreads done', flush=True)
