...
WANT = 4
# NULL PROGRAM: random-A frozen, U fit (same recipe as randctl), then the IDENTICAL polish budget.
_r = torch.Generator(device='cuda').manual_seed(99)
A512 = torch.randn(256, 1152, generator=_r, device='cuda') * 0.02
U512 = None  # fit below after pairs are collected
...
for i in range(0, 400, 8):
    h, y, t, p = block1_pairs(COOC[i:i+8].to(DEV)[:, :128]); Hs.append(h); Ys.append(y); Ts.append(t); Ps.append(p)
Y = torch.cat(Ys); TOK = torch.cat(Ts); PRV = torch.cat(Ps); H = torch.cat(Hs)
...
_U = torch.nn.Parameter(torch.randn(256, D, device=DEV)*0.02)
_opt = torch.optim.Adam([_U], lr=3e-3)
for _s in range(2800):
    ii = torch.randint(0, H.shape[0], (8192,), device=DEV)
    _loss = (((H[ii] @ A512.T)**2) @ _U - RES0[ii]).pow(2).mean()
...
json.dump(res, open(f'{QK}/qk_mlp4_nullpolish.json', 'w'), indent=2)
torch.save({'g': g.detach(), 'ab': ab.detach()}, f'{QK}/qk_mlp4_nullpolish.pt')
print("QK MLP4 NULLPOLISH DONE", flush=True)
