...
@torch.no_grad()
def forward(tokens, kt=None, ks=None, k_self=None, k_pair=None):
    ...
    eh_s = class_table(EH, k_self) if k_self else None
    if ks is None:
        vt_c = None
    else:
    ...
    for li, blk in enumerate(m.transformer.h):
        ...
        if li == 0:
            ...
            if eh_s is not None:
                e_s = ((eh_s[tokens] * scale).float() / rms).to(x.dtype)
            else:
                e_s = e_n
            if k_pair is not None:
                vt_p = torch.empty_like(VT)
                for hh2 in range(NH):
                    vt_p[:, hh2] = class_table(VT[:, hh2].contiguous(), k_pair)
                v_p = vt_p[tokens].to(x.dtype)
                v_pm = (1 - a.lamb) * v_p + a.lamb * v_p
                att_p = a.c_proj(torch.einsum('bhqk,bkhd->bqhd', pat, v_pm
                                              ).reshape(B, T, -1))
                a_p = (att_p.float() / rms).to(x.dtype)
            else:
                a_p = a_n
            hidden = (L(e_s) * R(e_s)                       # self: classed if k_self
                      + L(e_c) * R(a_c) + L(a_c) * R(e_c)   # cross: classed if kt/ks
                      + L(a_p) * R(a_p))                    # pair: classed if k_pair
...
for kp in [64, 256, 1024]:
    d = ce(k_pair=kp) - CE0
    name = f'PAIR k={kp} (self+cross exact)'
    results['arms'][name] = d
    print(f'pair-block {name}: dCE {d:+.4f}', flush=True)
    with open(OUT.replace('cross_block', 'pair_block'), 'w') as fh:
        json.dump(results, fh, indent=2)
print('pair block done')
