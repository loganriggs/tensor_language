"""Stage-B KL variant: same as menu_trained.py but the loss is
KL(teacher || student) against the LIVE model's logits — pure imitation, no
pile adaptation. The dCE gap between this and the CE-trained run is the
adaptation share (cf. layer-0 precedent: ~-0.025).
Original docstring: Stage-B flagship: menu-static, jointly CE-trained. Layers {8,14,15,17}
...
STEPS = 4500
BATCH = 2
OUT = '/workspace/tensor_language/basis_aligned/qk_mdl/menu_kl.json'
...
torch.save({f'{L}_{h}_{br}': cbs[(L, h, br)].detach().cpu()
            for (L, h, br) in cbs}, '/workspace/tensor_language/basis_aligned/qk_mdl/menu_cbs_kl.pt')
print('menu kl done', flush=True)
