"""THIRD end-to-end circuit: PUNCTUATION / syntactic-boundary prediction (next token is punctuation).
...
for i in range(50257):
    ...
    core = s.replace('Ġ', '')
    if len(core) and all(ch in _PUNCT for ch in core):
        is_cont[i] = True
is_cont = is_cont.to(DEV)
print(f"punctuation-type tokens: {int(is_cont.sum())}", flush=True)
...
json.dump(res, open(f'{QK}/qk_punct_circuit.json', 'w'), indent=2)
print("QK PUNCT CIRCUIT DONE", flush=True)
