...
| joint codebook | DL ratio | ΔCE (L2-fit) | ΔCE (CE-trained) |
|---|---|---|---|
| all vq16 | 8.1e-4 (1240×) | +0.044 | **−0.019** |
| all vq64 | 2.0e-3 (500×) | +0.015 | **−0.032** |
| all vq256 | 6.1e-3 (165×) | +0.008 | **−0.039** |

**Every CE-trained codebook OUTPERFORMS the original model's layer-0 QK** on held-out
pile-10k — a 16-token-class layer-0 beats the full 884 MiB computation by 0.019 nats.
Caveat (flagged, then tested): train and eval are both pile-10k (disjoint chunks) while
the model was trained on other data, so part of the gain is domain adaptation; the
KL-distilled variant (teacher = original model, k=64) separates the two:
**+0.015 → −0.007 under pure imitation** — faithful compression alone reaches
parity-or-better at 500×; the remaining ~−0.025 of the CE-trained gain is adaptation.
...
## Cross-associations: does the QK partition need q/k roles? (negative)

Spec upgrade "codebook-2" would give each head separate query-role and key-role
partitions. Tested on bilin18 layer-0 (`../cross_assoc_real.json`): shared k=256
+0.0082 vs separate +0.0089; shared k=1024 +0.0019 vs separate k=1024 +0.0047 —
separate partitions are never better and pay double index bits. Per head, the
query-role and key-role class structures are congruent on this model; the shared
partition on [q̂|k̂] stays the program default.
