...
@torch.no_grad()
def reach_aim(aim_col, scale, gB):
    x = F.rms_norm(m.transformer.wte(idx), (D,)); x0 = x; v1 = None
    cos, sin = rope_tables(T0, HD, DEV, x.dtype, 'bf16'); cosb, sinb = cos[None, :, None, :], sin[None, :, None, :]
...
res = {'note': 'aimability generality: query positions x aim, with SE + no-edit baseline', 'sweep': {}}
for qp in QPOS_SWEEP:
    gB = torch.zeros(B0, T0, dtype=torch.bool, device=DEV); gB[:, qp] = True; gB &= ~active
    n = int(gB.sum())
    base = reach_aim(A1, 0, gB)  # no-edit baseline (scale 0)
    res['sweep'][f'qpos={qp}'] = {'n': n, 'no_edit_baseline': {'P_tok@A1': base['P_tok@A1'], 'P_tok@A2': base['P_tok@A2']},
        'aim@col1_s40': reach_aim(A1, 40, gB), 'aim@col10_s40': reach_aim(A2, 40, gB)}
    print(f"qpos={qp} n={n}:", res['sweep'][f'qpos={qp}'], flush=True)
json.dump(res, open(f'{QK}/qk_aim_generality.json', 'w'), indent=2)
print("QK AIM GENERALITY DONE", flush=True)
