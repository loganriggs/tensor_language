...
print(f"bilin12: NH{NH} HD{HD} D{D} NL{NL} V{V}", flush=True)
...
def run(EV, keep, MEAN, collect=False):
    ...
    for li in range(NL):
        ...
        pat = pat / pat.sum(-1, keepdim=True).clamp_min(1e-9)      # NORMALIZED single-branch squared
...
print(f"bilin12 base CE(128) {allce:.3f} | induction adv {adv_full:.3f}", {t: round(base_ce[t],2) for t in VOCAB_TASKS}, flush=True)
...
json.dump(res, open(f'{QK}/qk_atlas_bilin12.json', 'w'), indent=2)
print("\nQK ATLAS BILIN12 DONE", flush=True)
