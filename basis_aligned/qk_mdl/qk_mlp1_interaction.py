...
def fit_interaction(target, R, steps=2500):
...
for name, R in [('table_prev_R64', 64), ('table_prev_R256', 256)]:
    A, U, fvu = fit_interaction(R2, R); ARMS[name] = (A, U); print(f"{name}: FVU {fvu:.3f}", flush=True)

# MLP0 program + induction code for the joint test
m0d = torch.load(f'{QK}/qk_mlp0_interaction.pt', map_location=DEV)
A0, U0 = m0d['table_R256']
# rebuild MLP0 token table (same recipe as qk_mlp0_interaction)
H0, Y0, T0 = [], [], []
@torch.no_grad()
def block0_pairs(idx):
    ...
    pat = (s1*s2).masked_fill(~mask, 0.0); yh4 = torch.einsum('bhqk,bkhd->bqhd', pat, v)
    x = x + a.c_proj(yh4.reshape(B, T, -1)); hin = F.rms_norm(x, (D,))
    return blk.mlp(hin).reshape(-1, D), idx.reshape(-1)
for i in range(0, 400, 8):
    y, t = block0_pairs(COOC[i:i+8].to(DEV)[:, :128]); H0.append(y); T0.append(t)
TT0 = cond_table(torch.cat(T0), torch.cat(H0))
ftd = torch.load(f'{QK}/qk_induction_finetune.pt', map_location=DEV)
THETA = ftd['theta']; HEADS = ftd['heads']; SIDX = {tuple(lh): i for i, lh in enumerate(HEADS)}
...
# position templates for the substituted heads (recomputed on natural repeated eval)
P = 64; NSEQ = 48
...
def get_templates():
    ...
    for li in range(11):
        ...
        for h in range(NH):
            if (li, h) in SIDX: TEMPL[(li, h)] = pat[:, h].mean(0)
...
def forward(idx, arm=None, joint=False):
    ...
    cos, sin = rope_tables(T, HD, DEV, x.dtype, 'bf16'); cosb, sinb = cos[None, :, None, :], sin[None, :, None, :]
    mask = torch.tril(torch.ones(T, T, device=DEV, dtype=torch.bool))
    MM = match_matrix(idx) if joint else None
    prev = torch.roll(idx, 1, dims=1); prev[:, 0] = idx[:, 0]
    for li in range(NL):
        blk = m.transformer.h[li]; x = blk.lambdas[0]*x + blk.lambdas[1]*x0; a = blk.attn; hcur = F.rms_norm(x, (D,))
        def qk(lin): z = F.rms_norm(lin(hcur).view(B, T, NH, HD), (HD,)); return apply_rot(z, cosb, sinb)
        ...
        if li == 0 and joint:
            ...
            mo = (TT0[idx.reshape(-1)] + ((flat @ A0.T)**2) @ U0).view(B, T, D).to(x.dtype)
        elif li == 1 and (arm is not None or joint):
...
res = {'floor': FLOOR1}
for arm in ['table', 'table_prev', 'table_prev_R64', 'table_prev_R256']:
    d = audit(arm) - SUBBASE
...
# joint test at 128-token windows (position templates are 127x127); compare to same-length base
base128 = audit(Tcap=128)
dj = audit(joint=True, Tcap=128) - base128
ajn = adv_of(forward(EVN[:, :-1], joint=True), EVN); ajs = adv_of(forward(EVS[:, :-1], joint=True), EVS)
res['JOINT_mlp0prog_mlp1prog_inductioncode'] = {'dCE': round(dj, 5),
    'adv_nat_ret': round(ajn/adv_full_n, 3), 'adv_shuf_ret': round(ajs/adv_full_s, 3)}
print(f"JOINT (MLP0 prog + MLP1 prog + induction code): dCE +{dj:.5f} | induction nat {ajn/adv_full_n:.1%} shuf {ajs/adv_full_s:.1%}", flush=True)
json.dump(res, open(f'{QK}/qk_mlp1_interaction.json', 'w'), indent=2)
torch.save({k: v for k, v in ARMS.items() if v[0] is not None}, f'{QK}/qk_mlp1_interaction.pt')
print("QK MLP1 INTERACTION DONE", flush=True)
