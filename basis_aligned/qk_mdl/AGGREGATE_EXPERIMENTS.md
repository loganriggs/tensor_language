...
| 4 | planted-modular-content DGP | **DONE** (RESULTS_l0_mdl.md §38): pipeline NOT certified — D1 fails (66.7% vs 80% bar despite 100% noiseless ceiling), D3 fails decisively (4.1% codable on planted truth), overlap-variant recovery zero; registered rule fires: bilin18 content/codability verdicts downgraded to tool-limited. D4 supported (equal-ablation negative = what modular selection looks like). F0: modular truth need not be its own tensor's optimum. Review + fix round in progress. |
