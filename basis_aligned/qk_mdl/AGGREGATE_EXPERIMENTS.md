...
| 4 | planted-modular-content DGP | **DONE + reviewed + fix round** (§38-§38c): pipeline NOT certified — downgrade of bilin18 content/codability verdicts to tool-limited STANDS. Naming gate structurally broken (oracle truth certifies 6/24; per-head 16-dim bottleneck vs 24 units); recovery seed-stably ~45% vs analytic ceiling 100%; SGD's free embedding MORE recoverable AND more noise-robust than the planted orthonormal table; equal-ablation vs-PCA channel CAN detect planted selection (privilege >2SE all doses) so bilin18's negative there now carries evidence. F0: modular truth need not be its own tensor optimum. |
| 5 | archetype extraction into a standalone machine | **DONE** (§39): oracle-assembly gate at ±4e-6; semi arm reassembles at +0.017 nats (demonstrated); learned arm FAILS (+0.505, key spans lost — parts-list vs mechanism split); overlap machines FUNCTIONAL at +0.28-0.30 despite zero individuated recovery (spans suffice); shared directions are most of the machine but no consistent per-energy privilege (E2 3x bar failed). Review queued. |
