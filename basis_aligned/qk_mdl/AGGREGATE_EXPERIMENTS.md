...
| 4 | planted-modular-content DGP | **IN BUILD** — predictions D1–D4 registered (qk_dgp_modular_predictions.json); build agent writing qk_dgp_lang.py + qk_dgp_modular.py; card cleared for it (tiny-model queue paused per Logan 19:10). Logan amendment: freeze is phase 1 (measure current tools unchanged); phase 2 tool-improvement authorized, filtered by generalizability to realistic models, any improved tool re-run on bilin18 with its own planted control. |
