...
@torch.no_grad()
def forward(tokens, kt=None, ks=None, k_self=None):
...
for ksf in [256, 1024, 4096]:
    d = ce(k_self=ksf) - CE0
    name = f'SELF k={ksf} (cross+pair exact)'
    results['arms'][name] = d
    print(f'self-block {name}: dCE {d:+.4f}', flush=True)
    with open(OUT.replace('cross_block', 'self_block'), 'w') as fh:
        json.dump(results, fh, indent=2)
print('self block done')
