"""BLOCK-SPARSE PATTERN MDL: how block-sparse is the layer-0 selection
...
def audit_keep(keep_masks):
    ...
    for i in range(0, len(AUDIT), 4):
        ...
        for li, blk in enumerate(m.transformer.h):
            ...
            pat = (s1 * s2).masked_fill(~mask, 0.0)
            if li == 0:
                km = keep_masks[:, cls_pos][:, :, :, None].permute(0, 1, 3, 2) if False else None
                # gather per-(b,i,j) keep: keep_masks[h, cq(b,i), ck(b,j)]
...
for Bk in (32768, 8192, 2048, 512):
    keep = torch.zeros(NH, 256, 256, dtype=torch.bool, device=DEV)
    for hh in range(NH):
        flat_idx = energy[hh].view(-1).topk(Bk).indices
        keep[hh].view(-1)[flat_idx] = True
    frac = float(keep.float().mean())
    d = audit_keep(keep) - base
    res['arms'][f'top-{Bk} blocks/head ({frac:.3f} of pairs)'] = round(d, 4)
    print(f'top-{Bk} blocks/head ({frac:.1%} of class pairs): dCE {d:+.4f}', flush=True)
    with open(OUT, 'w') as fh:
        json.dump(res, fh, indent=2)
print('bs pattern done', flush=True)
