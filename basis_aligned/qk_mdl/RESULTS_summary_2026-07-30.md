...
- Whole model through per-layer PCA/head bottlenecks (residual truncated): bilin18 +0.047 (SE .001),
  99.56% of uniform-ceiling headroom; nulls (head-span random) 18-100x worse.
- Entire MLP stack replaced causally by the composed-fold analytic chain: +0.034 (SE .001); joint
  substitution ~free once correctly scaled (the "gap" was a lambda bug, retracted).
- Architecture-general: bilin18 +0.047 / bilin12 +0.116 / bilinsm12 +0.077, nulls 10-47x.
...
- Layer-0 content: NOT class-nameable (3/576 coords; median class-R2 0.02); complete description IS the
  exact weight-derived spectrum. Archetype coords no better (2/144); spike codes fail.
...
bilin18 is representationally exact, ~95-99% causally substitutable through compressed analytic
interfaces (architecture-general), functionally mapped into three families with a per-head selection
...
nameable/spectral boundary measured, not assumed. Every headline here has survived a dedicated
adversarial review (two full passes; one caught a real lambda-scaling bug and three framing
retractions). Open: content nameability is a spectrum bounded by function vs lexicality; scale
transfer HELD (Pythia).
