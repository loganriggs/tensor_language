"""TICK 232: SCALED explicit memory — datastore mined from ~30M streamed FineWeb
tokens (10x v1), ENTITY-FILTERED (keys kept only when the continuation is a subword
fragment or the current token starts a capitalized word — the mechanism's domain).
Causal discriminator: if the parametric-memory account is right, audit gain grows
with coverage and concentrates on entity positions; if gain stays flat at 10x data,
explicit n-gram memory is the wrong replacement form.

Original tick-231 header:
"""
"""TICK 231 (Logan go-ahead): EXPLICIT MEMORY — replace a slice of the parametric
entity memory with a mined datastore.

Datastore: suffix n-gram (n=3,2) -> most frequent continuation, mined from the 6000
DISJOINT co-occurrence documents (~3.1M tokens; small vs pretraining — coverage-
limited v1, noted). Features added to the named basis: the retrieved continuation
token's 96-dim embedding code + match length + log-count (99 dims; datastore bits
priced separately). Causal verification: full 307k audit (FineWeb, disjoint from the
datastore source) vs the named-basis row (+0.0309, 51%). Prediction under the
parametric-memory account: gains concentrated on entity-continuation positions,
limited by datastore coverage.
...
# ---- explicit datastore (n=3,2 suffix -> most frequent continuation) ----
print('mining datastore (streamed FineWeb, entity-filtered)...', flush=True)
from collections import defaultdict
from datasets import load_dataset
from transformers import AutoTokenizer as AT2
tok2 = AT2.from_pretrained('gpt2')
FRAG = torch.zeros(V, dtype=torch.bool)
CAPS = torch.zeros(V, dtype=torch.bool)
for t in range(V):
    s = tok2.decode([t])
    FRAG[t] = (not s.startswith(' ')) and len(s) > 0 and s[:1].isalpha()
    CAPS[t] = s.startswith(' ') and len(s) > 1 and s[1].isupper()
FRAGn = FRAG.numpy(); CAPSn = CAPS.numpy()
cnt3, cnt2 = defaultdict(lambda: defaultdict(int)), defaultdict(lambda: defaultdict(int))
ds = load_dataset('HuggingFaceFW/fineweb', name='sample-10BT', split='train', streaming=True)
tok_count, DOC_SKIP = 0, 1200   # skip region overlapping our audit/cooc extraction
it = iter(ds)
for _ in range(DOC_SKIP):
    next(it)
import numpy as np2
while tok_count < 30_000_000:
    try:
        doc = next(it)
    except StopIteration:
        break
    ids = tok2(doc['text'])['input_ids'][:4096]
    row = np2.array(ids)
    tok_count += len(row)
    keep = None
    for p in range(2, len(row) - 1):
        nxt = row[p + 1]
        if FRAGn[nxt] or CAPSn[row[p]] or FRAGn[row[p]]:
            cnt3[(row[p - 2], row[p - 1], row[p])][nxt] += 1
            cnt2[(row[p - 1], row[p])][nxt] += 1
    if tok_count % 3_000_000 < 4096:
        print(f'  {tok_count/1e6:.0f}M tokens, {len(cnt3)/1e6:.1f}M tri keys', flush=True)
DS3 = {k: max(v.items(), key=lambda x: x[1]) for k, v in cnt3.items() if sum(v.values()) >= 2}
DS2 = {k: max(v.items(), key=lambda x: x[1]) for k, v in cnt2.items() if sum(v.values()) >= 3}
del cnt3, cnt2
print(f'datastore: {len(DS3)} trigram keys, {len(DS2)} bigram keys', flush=True)


def retrieve_rows(tokrows):
    """(B, T) numpy -> (B*T,) retrieved tok id (or 0), n, logcount."""
    import numpy as np2
    B, T = tokrows.shape
    rt = np2.zeros((B * T,), dtype=np2.int64)
    rn = np2.zeros((B * T,), dtype=np2.float32)
    rc = np2.zeros((B * T,), dtype=np2.float32)
    for b in range(B):
        row = tokrows[b]
        for p in range(T):
            gi = b * T + p
            hit = None
            if p >= 2:
                hit = DS3.get((row[p - 2], row[p - 1], row[p]))
                if hit is not None:
                    rn[gi] = 3
            if hit is None and p >= 1:
                hit = DS2.get((row[p - 1], row[p]))
                if hit is not None:
                    rn[gi] = 2
            if hit is not None:
                rt[gi] = hit[0]
                rc[gi] = float(np2.log1p(hit[1]))
    return rt, rn, rc


print('retrieval for training positions...', flush=True)
rt, rn, rc = retrieve_rows(COOC[:N_CAP, :-1].numpy())
rt = torch.from_numpy(rt)[:N]
MEMF = torch.cat([EWcur[rt.to(DEV)] * (torch.from_numpy(rn)[:N, None].to(DEV) > 0),
                  torch.from_numpy(rn)[:N, None].to(DEV) / 3.0,
                  torch.from_numpy(rc)[:N, None].to(DEV) / 5.0], 1)
CMIX = torch.cat([CMIX, MEMF], 1)
hitrate = float((torch.from_numpy(rn)[:N] > 0).float().mean())
print(f'train hit rate {hitrate:.3f}; feature dim {CMIX.shape[1]}', flush=True)
...
ARMS = {
    'named_mem_swiglu': (Skip(338, Bilin(338, 140, 576, gate='silu'), 576), CMIX),
}
out = {}
models = {}
for aname, (mod, X) in ARMS.items():
    ...
    print(f'{aname}: {nparam} params, val R2 {r2:.4f}, top-1% weight mass {top1:.3f}',
          flush=True)
    json.dump(out, open(f'{QK}/qk_explicit_memory2.json', 'w'), indent=2)
        ...
        def factors(lin, name=None):
            ...
            if li == 1 and arm is not None and name is not None:
                ...
                if arm == '__static__':
                    ...
                    rt2, rn2, rc2 = retrieve_rows(idx.cpu().numpy())
                    rt2 = torch.from_numpy(rt2).to(DEV)
                    memf = torch.cat([EWcur[rt2] * (torch.from_numpy(rn2)[:, None].to(DEV) > 0),
                                      torch.from_numpy(rn2)[:, None].to(DEV) / 3.0,
                                      torch.from_numpy(rc2)[:, None].to(DEV) / 5.0], 1)
                    feat = torch.cat([ce, ac, memf], 1)
