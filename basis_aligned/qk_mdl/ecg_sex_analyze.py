...
ck = torch.load(f'{QK}/ecg_sex_model.pt', map_location=DEV)
...
yp = torch.from_numpy(df.sex.values[fold == 10].astype(np.float32)).to(DEV).bool()
...
for hp in recs:
    s = None
    for line in open(hp):
        if line.lower().startswith('# sex') or line.lower().startswith('#sex'):
            v = line.split(':', 1)[1].strip().lower()
            s = 1.0 if v.startswith('f') else 0.0
...
gvalid = yg >= 0
Xg, yg = Xg[gvalid], yg[gvalid].bool()
...
res = {'sex_auc_ptbxl': round(auc(sp, yp), 4), 'sex_auc_georgia': round(auc(sg, yg), 4),
...
with torch.no_grad():
    ...
    for L in range(NLEAD):
        ...
        lead_imp[LEADS[L]] = round(res['sex_auc_ptbxl'] - auc(s2, yp), 4)
...
ap = np.array([abs(auc(Fp[:, j], yp)-0.5) for j in range(INNER)])
ag = np.array([abs(auc(Fg[:, j], yg)-0.5) for j in range(INNER)])
res['feature_sexstrength_corr_ptbxl_vs_georgia'] = round(float(np.corrcoef(ap, ag)[0, 1]), 3)
...
torch.save(waves, f'{QK}/ecg_sex_waveforms.pt')
print(json.dumps(res, indent=1), flush=True)
json.dump(res, open(f'{QK}/ecg_sex_analyze.json', 'w'), indent=2)
print('ECG SEX ANALYZE DONE', flush=True)
