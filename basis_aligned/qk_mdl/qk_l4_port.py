"""TICK 254: layer-4 port (depth-decay point 5); derived from tick 251.
...
pushed through block 2's own q/k projections into factor tables; patch layer-4's
pattern with table scores; audit on the standard 307k set. Calibration: layer-4
...
def block4_input(idx):
    ...
    return blk4.lambdas[0] * x + blk4.lambdas[1] * x0
...
with torch.no_grad():
    for i in range(0, N_EST, 4):
        b = COOC[i:i + 4].to(DEV)
        idx = b[:, :-1]
        x = block4_input(idx).float().reshape(-1, D)
        ids = idx.reshape(-1)
...
out['l4_token_tables_dce'] = round(ce_tab - BASE, 5)
print(f'layer-4 token-table pattern: dCE {ce_tab - BASE:+.5f}', flush=True)
json.dump(out, open(f'{QK}/qk_l4_port.json', 'w'), indent=2)
ce_zero = audit(patch_zero_layer)
out['l4_zero_dce'] = round(ce_zero - BASE, 5)
print(f'layer-4 pattern zeroed: dCE {ce_zero - BASE:+.5f}', flush=True)
json.dump(out, open(f'{QK}/qk_l4_port.json', 'w'), indent=2)
for h in range(NH):
    ce_h = audit(patch_zero_head(h))
    out[f'l4_h{h}_zero_dce'] = round(ce_h - BASE, 5)
    print(f'layer-4 head {h} zeroed: dCE {ce_h - BASE:+.5f}', flush=True)
    json.dump(out, open(f'{QK}/qk_l4_port.json', 'w'), indent=2)
print('L4 PORT DONE', flush=True)
