"""TICK 258 (reviewer-2 control): RANDOM-PROJECTION NULL for the layer-2 symbol fold.
Same pipeline, but every basis (embedding 96, per-head archetype 16, per-head l1 PCA 16)
is replaced by a random orthonormal projection of matching dimension. If this matches
+0.0176, the fold result is a bandwidth claim, not a named-symbol claim.
...
for hh in range(NH):
    ...
    PA[hh] = torch.linalg.qr(torch.randn(HD, 16))[0].to(DEV).contiguous()
EPC96 = torch.linalg.qr(torch.randn(D, 96))[0].to(DEV).contiguous()
...
for h in range(NH):
    ...
    PB[h] = torch.linalg.qr(torch.randn(HD, 16))[0].to(DEV).contiguous()
...
with torch.no_grad():
    for i in range(0, len(FINEWEB), 4):
        ...
        n += b[:, 1:].numel()
ce = tot / n
out = {'decoder_R2': round(R2, 4), 'l2_randnull_dce': round(ce - BASE, 5),
       'gates': {'tables': 0.02777, 'zero': 0.39042}}
print(f'layer-2 RANDOM-BASIS pattern: dCE {ce - BASE:+.5f} '
      f'(tables +0.02777, zero +0.39042)', flush=True)
json.dump(out, open(f'{QK}/qk_l2_randnull.json', 'w'), indent=2)
print('L2 RANDNULL DONE', flush=True)
