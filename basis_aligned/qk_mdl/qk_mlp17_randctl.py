...
ARMS = {}
for name, R in [('trained_R256', 256), ('randomA_R256', -256)]:
    A, U, fvu = fit(abs(R), freeze_A=(R < 0)); ARMS[name] = (A, U); print(f"{name}: FVU {fvu:.3f}", flush=True)
torch.save({k: v for k, v in ARMS.items() if v[0] is not None}, f'{QK}/qk_mlp17_randctl.pt')
...
res = {'floor': FLOOR}
for arm in ['trained_R256', 'randomA_R256']:
    d = audit(arm) - SUBBASE
...
json.dump(res, open(f'{QK}/qk_mlp17_randctl.json', 'w'), indent=2)
print(f"QK MLP{WANT} RANDCTL DONE", flush=True)
