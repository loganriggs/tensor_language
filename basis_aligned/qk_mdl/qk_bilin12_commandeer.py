"""ARCHITECTURE-GENERALITY of the editing capstone: does copy-head COMMANDEERING replicate on bilin12
(a different attention family -- single-branch NORMALIZED squared attention, pat = sc^2 / sum)? Because
...
def reach(aim_col, HEADS):
    ...
    return {'P_tok@1': round(float(pr.gather(-1, t1[:, None]).mean()), 4),
            'P_tok@10': round(float(pr.gather(-1, t2[:, None]).mean()), 4)}

res = {'model': 'bilin12', 'n_gate': NG, 'sets': {}}
...
json.dump(res, open(f'{QK}/qk_bilin12_commandeer.json', 'w'), indent=2)
print("QK BILIN12 COMMANDEER DONE", flush=True)
