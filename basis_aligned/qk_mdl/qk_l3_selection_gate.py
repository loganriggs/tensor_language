"""LAYER-3 GATED SELECTION VOCABULARY (census-predicate simultaneous-substitution gate at layer 3).
...
(L3H8 = the model's strongest head, anti-self MATCH_same, gain ~0.31; L3H5 = MATCH_same). Tests whether the induction MATCH
predicate (meaning-verified held-out 98-111% at layer 1) survives the gate at layer 3, and whether
any OTHER layer-3 head is gated-nameable.
...
unchanged). CHANGES, all layer-index / input only:
  - LAYER = 3 constant added; the GATE's PROGSET is restricted to programmatic heads AT LAYER 3 only
    (predicate gain >= 0.05), so forward_coded substitutes coded patterns solely at layer 3 (isolating
  ...
  - summary + JSON report focus on layer 3 (full 162-head census still computed and saved);
  - output file qk_l3_selection_gate.json.
