...
# ---- register slowness/swap causal test (validates TM-3) ----
acc = torch.zeros(V, D)
cnt = torch.zeros(V)
outs, toks = [], []
with torch.no_grad():
    for i in range(0, len(EST), 4):
        ...
        o = forward(idx, target=16, grab=True)
...
O = torch.cat(outs)
dev = O - mean[torch.cat(toks)].to(DEV)
C = (dev.T @ dev) / len(dev)
evals, evecs = torch.linalg.eigh(C)
evecs = evecs.flip(1)
B64 = evecs[:, :64].T.contiguous()          # (64, D); dirs 0-3 = register dirs
print('PCs built', flush=True)


@torch.no_grad()
def forward_coef(idx, coef_fn):
    B, T = idx.shape
    x = m.transformer.wte(idx)
    x = F.rms_norm(x, (x.size(-1),))
    x0, v1 = x, None
    ...
    for li, blk in enumerate(m.transformer.h):
        ...
        if li == 16:
            mu = mean[idx.cpu()].to(DEV)
            dv = mlp_out - mu
            proj = torch.einsum('btd,kd->btk', dv, B64)     # (B,T,64)
            proj = coef_fn(proj)
            mlp_out = mu + torch.einsum('btk,kd->btd', proj, B64)
        x = x + mlp_out
    xf = F.rms_norm(x, (x.size(-1),))
    return 30 * torch.tanh(m.lm_head(xf) / 30)
...
def ce_coef(coef_fn):
    ...
    for i in range(0, len(AUDIT), 4):
        b = AUDIT[i:i + 4].to(DEV)
        logits = forward_coef(b[:, :-1], coef_fn).float()
        ce = F.cross_entropy(logits.reshape(-1, logits.shape[-1]), b[:, 1:].reshape(-1))
        tot += ce.item() * b[:, 1:].numel(); n += b[:, 1:].numel()
    return tot / n
...
arms = {
    'rank64 live (reference)': lambda p: p,
    'top-4 doc-mean (slowness)': lambda p: torch.cat(
        [p[..., :4].mean(1, keepdim=True).expand(-1, p.shape[1], -1), p[..., 4:]], -1),
    'top-4 swapped across docs': lambda p: torch.cat(
        [p.roll(1, dims=0)[..., :4], p[..., 4:]], -1),
    'top-4 zeroed': lambda p: torch.cat([p[..., :4] * 0, p[..., 4:]], -1),
}
for name, fn in arms.items():
    d = ce_coef(fn) - base
    res[name] = d
    print(f'{name}: dCE {d:+.4f}', flush=True)
    with open(f'{QK}/mlp16_register_swap.json', 'w') as fh:
        json.dump(res, fh, indent=2)
print('register swap done', flush=True)
