...
# =====================================================================================
# ATTACK 1 (continued) -- finer allocation search around the knobs that helped
# =====================================================================================
print("\n=== ATTACK 1b: finer allocation search ===", flush=True)
A1 = {'cells': {}}
PROF['150pct'] = kprof(1.50)
PROF['allterms'] = [len(ACTIVE[L]) for L in range(NL)]
KEPT['150pct'] = [RANKED[L][:PROF['150pct'][L]] for L in range(NL)]
KEPT['allterms'] = [list(ACTIVE[L]) for L in range(NL)]
need = sorted({(L, kk) for t in ['150pct', 'allterms'] for L in range(NL) for kk in KEPT[t][L]}
              - set(TVEC.keys()))
print(f"extra term output grams needed: {len(need)}", flush=True)
CH = [need[i:i+60] for i in range(0, len(need), 60)]
for chunk in CH:
    ...
    del stg; torch.cuda.empty_cache()
TNUM = {p: min(numrank(v), KMAX) for p, v in TEV.items()}
for t in ['150pct', 'allterms']: print(f"profile {t}: {PROF[t]} (sum {sum(PROF[t])})", flush=True)

def fitted(kept, **kw):
    R, b, al = fit_scalar(lambda c: alloc_uniform(kept, c, **kw), B16)
    Ri = int(round(R)); al = alloc_uniform(kept, Ri, **kw)
    while cost_of(al) > B16: Ri -= 1; al = alloc_uniform(kept, Ri, **kw)
    return Ri, al

# (i) finer output/input ratio sweep at the 125% profile
for rho in [0.75, 1.5]:
    Ri, al = fitted(KEPT['125pct'], rho=rho)
    A1['cells'][f'ratio_rho{rho}'] = cell(f'ratio_rho{rho}', KEPT['125pct'], al,
        f'uniform R={Ri}, output ceil({rho}*R)', per_term_inputs=False)
    json.dump(A1, open(OUT, 'w'), indent=1)

# (ii) richer term profiles at the better ratio (rho=1)
for ptag in ['150pct', 'allterms']:
    Ri, al = fitted(KEPT[ptag], rho=1.0)
    A1['cells'][f'profile_{ptag}_rho1'] = cell(f'profile_{ptag}_rho1', KEPT[ptag], al,
        f'profile {ptag} ({sum(PROF[ptag])} terms), rho=1, R={Ri}', per_term_inputs=False)
    json.dump(A1, open(OUT, 'w'), indent=1)

# (iii) best-of-knobs combinations: rho=1 with early boost
for mu in [2.0, 3.0, 4.0]:
    Ri, al = fitted(KEPT['125pct'], rho=1.0, mu_early=mu)
    A1['cells'][f'combo_rho1_mu{mu}'] = cell(f'combo_rho1_mu{mu}', KEPT['125pct'], al,
        f'rho=1 with layers 0-4 at rank {min(int(round(mu*Ri)),768)}, rest {Ri}',
        per_term_inputs=False)
    json.dump(A1, open(OUT, 'w'), indent=1)

# (iii-b) importance rule with UNCAPPED group bases (the 768-column cap BOUND in this cell)
print("recomputing full-width (1152-column) group bases for the uncapped importance cell ...", flush=True)
GVF = {}
for ch in [list(range(0, 6)), list(range(6, 12)), list(range(12, 18))]:
    stg = {'pairs': set(), 'Gt': {}, 'chunk': set(ch),
           'Gg': {(L, g): torch.zeros(D, D, device=DEV) for L in ch for g in range(NG)}}
    for i in range(0, STR, B0T): fwd_group_grams(TRAIN[i:i+B0T], stg)
    for (L, g), G in stg['Gg'].items():
        if float(G.abs().max()) < 1e-20: GVF[(L, g)] = None; continue
        w, vec = torch.linalg.eigh(G)
        GVF[(L, g)] = vec.flip(1).contiguous()
    del stg; torch.cuda.empty_cache()
gate = max(float((GVF[k][:, :50].abs() - GV[k][:, :50].abs()).abs().max())
           for k in GVF if GVF[k] is not None and GV[k] is not None)
print(f"full-width basis gate (|cols 0-49| vs cached): {gate:.2e}", flush=True)
GNUM_SAVE = dict(GNUM)
for k in GNUM: GNUM[k] = GNUM_UNCAP[k]
GV_SAVE = dict(GV)
for k in GVF: GV[k] = GVF[k]
c_, b_, al_ = fit_scalar(lambda c: alloc_importance(KEPT['125pct'], c, gamma=1.0, scope='global'), B16)
rs = [al_[L][kk][0] for L in range(NL) for kk in KEPT['125pct'][L]]
A1['cells']['importance_global_g1_UNCAPPED'] = cell(
    'importance_global_g1_UNCAPPED', KEPT['125pct'], al_,
    f'importance rule with group bases uncapped to the full numerical rank; rank range {min(rs)}-{max(rs)}')
A1['cells']['importance_global_g1_UNCAPPED']['rank_range'] = [int(min(rs)), int(max(rs))]
A1['cells']['importance_global_g1_UNCAPPED']['basis_gate'] = gate
for k in GNUM_SAVE: GNUM[k] = GNUM_SAVE[k]
for k in GV_SAVE: GV[k] = GV_SAVE[k]
del GVF; torch.cuda.empty_cache()
json.dump(A1, open(OUT, 'w'), indent=1)

# (iv) the single most favourable combination found, at the 4x budget too (sanity on budget scaling)
B4 = NL*1152*576*577//2
R4, b4, al4 = fit_scalar(lambda c: alloc_uniform(KEPT['125pct'], c, rho=1.0), B4)
R4i = int(round(R4)); al4 = alloc_uniform(KEPT['125pct'], R4i, rho=1.0)
while cost_of(al4) > B4: R4i -= 1; al4 = alloc_uniform(KEPT['125pct'], R4i, rho=1.0)
b = cost_of(al4)
spec = build_spec(KEPT['125pct'], al4, False, None, False)
ce = run(spec); del spec; torch.cuda.empty_cache()
mn, se = dstat(ce); dv, sv = pairdiff(ce, CE_STORE['s1_4x'])
A1['cells']['ratio_rho1_at_4x'] = {'note': f'rho=1 at the 4x budget, R={R4i}',
    'budget': int(b), 'dCE': round(mn, 4), 'SE': round(se, 5),
    'vs_rank_alloc_4x': [round(dv, 4), round(sv, 5)],
    'rank_alloc_4x': TC['scheme1']['4x']['dCE']}
print(f"[ratio_rho1_at_4x] dCE {mn:+.4f} +- {se:.5f} vs rank-alloc 4x {dv:+.4f} +- {sv:.5f}", flush=True)
A1['peak_gpu_MiB'] = round(torch.cuda.max_memory_allocated()/2**20, 1)
json.dump(A1, open(OUT, 'w'), indent=1)
print("QK REDTEAM TERMCOMPRESS PART 2 DONE", flush=True)
