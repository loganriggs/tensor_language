"""Data-scaling control for D-composed: 6x estimation tokens (3.15M).
If W=4 improves, tables were noise-limited; if not, window-boundary error
dominates. Original: Logan's method D composed, window form (guided by SI-1 + C-1): at EVERY
...
for W, name in ((4, 'W=4 window 6x data'), (3, 'W=3 window 6x data')):
    d = audit_ce(W=W) - base
    res['arms'][name] = d
    print(f'D-composed {name}: dCE {d:+.4f}', flush=True)
    with open(OUT, 'w') as fh:
        json.dump(res, fh, indent=2)
print('d composed 3 done', flush=True)
