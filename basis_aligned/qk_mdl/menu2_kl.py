"""Stage-B v2 KL control (pure imitation of live logits;
gap to the CE-trained menu2 = adaptation share). Stage-B v2: menu-static EXCEPT L5 heads H5,H7 stay LIVE (the two heads
that own ~91% of the model's contextual selection, per l5_probe/l5_pair; the
hot-swap audit showed live heads must be present DURING training, not spliced
in after). Everything else as menu_trained.py.
Original: Stage-B flagship: menu-static, jointly CE-trained. Layers {8,14,15,17}
...
STEPS = 4500
BATCH = 2
OUT = '/workspace/tensor_language/basis_aligned/qk_mdl/menu2_kl.json'
LIVE5 = (5, 7)
        ...
        if li in ZERO_L:
            ...
            if li == 0:
                ...
                if li == 5:
                    qn = lambda lin: apply_rot(F.rms_norm(lin(h).view(B, T, NH, HD), (HD,)), cos, sin)
                    q, k, q2, k2 = qn(a.c_q), qn(a.c_k), qn(a.c_q2), qn(a.c_k2)
                    ls1 = torch.einsum('bqhd,bkhd->bhqk', q, k) / HD
                    ls2 = torch.einsum('bqhd,bkhd->bhqk', q2, k2) / HD
                    keep = torch.tensor([hh in LIVE5 for hh in range(NH)],
                                        device=DEV)[None, :, None, None]
                    s1 = torch.where(keep, ls1, s1)
                    s2 = torch.where(keep, ls2, s2)
...
base = tot / n
d0 = audit() - base
print(f'baseline {base:.4f}; menu2 (H5,H7 live) vq{K} L2-fit dCE {d0:+.4f}', flush=True)
...
dT = audit() - base
res['ce_trained'] = dT
print(f'MENU2-KL (L5 H5,H7 LIVE) vq{K} KL-trained: dCE {dT:+.4f}', flush=True)
with open(OUT, 'w') as fh:
    json.dump(res, fh, indent=2)
torch.save({f'{L}_{h}_{br}': cbs[(L, h, br)].detach().cpu()
            for (L, h, br) in cbs}, '/workspace/tensor_language/basis_aligned/qk_mdl/menu2_cbs_kl.pt')
print('menu2 kl done', flush=True)
