"""§34 CONTENT-NAMEABILITY PROTOCOL, LAYER 2 (verbatim copy of qk_l1_content_gate.py,
layer index 1->2). Each layer-2 head-value coordinate c of head h has a mean-residual-derived
vocabulary spectrum S_h[t, c] = <basis_hc, c_proj2_h vs_h(t)>, where vs_h(t) is the layer-2
effective head value of token t built from the MEAN-RESIDUAL TABLE (the per-token mean of the
layer-2 input residual, shrunk tau=8 toward the embedding prior; the residual is the block-2
...
layer-2 head outputs (varimax-sparsified). Naming hypotheses, both from §34: (a) BROAD-CLASS code
...
substitution on LAYER 2: run the model with layer-2 attention writes replaced by (a) the exact
mean-residual spectra (identity reference) and (b) coded spectra; if coded ~ exact the names are
causally verified. Answers: does 'content is spectral, not class-nameable' stay the rule two layers
up? SEs on the held-back slice FW[448:600].

DERIVATION: copied VERBATIM from qk_l1_content_gate.py with these layer-index-only edits:
  - l1_heads -> l2_heads: forward loop range(2)->range(3), captured/returned at li==2 (was li==1);
    PCA-64 basis on layer-2 outputs; write projection cw = h[2].attn.c_proj (was h[1]);
  - block1_input -> block2_input: runs blocks 0 AND 1 (with v1-mixing) then returns block-2's
    lambda-mixed input, using qk_l4_port.py's validated block4_input structure (loop (0,1),
    return h[2] mix). block1_input's single-block form only worked because block-0 v-mixing is a
    no-op; deeper layers require the v1 term, so the multi-layer form is used;
  - value spectra: a1 -> a2 = h[2].attn; vs = (1-a2.lamb)*c_v2(table) + a2.lamb*c_v0(embedding);
    a0 = h[0].attn UNCHANGED (v1 is always the layer-0 value);
  - the substitution branch fires at li == 2 (was li == 1);
...
# ---- layer-2 head outputs (full forward captured at layer 2), verbatim forward loop ----
@torch.no_grad()
def l2_heads(idx):
    B, T = idx.shape
...
# per-head PCA-64 basis on layer-2 head outputs (standard recipe, two layers up)
acc = torch.zeros(NH, HD, HD, device=DEV, dtype=torch.float64)
for i in range(0, 64, 4):
    f = l2_heads(COOC[i:i+4].to(DEV)[:, :128]).double()
    acc += torch.einsum('nhd,nhe->hde', f, f)
cw = m.transformer.h[2].attn.c_proj.weight.detach().float()
BASIS = []   # per head: (D,NC) ORTHONORMAL layer-2 write basis (QR of top-NC PCA dirs)
for hh in range(NH):
...
print("layer-2 PCA-64 basis built", flush=True)


# ---- mean-residual table (block2_input + shrink); block2_input = qk_l4_port block4_input at L=2 ----
@torch.no_grad()
def block2_input(idx):
    ...
    return blk2.lambdas[0] * x + blk2.lambdas[1] * x0
...
with torch.no_grad():
    for i in range(0, N_EST, 4):
        b = COOC[i:i + 4].to(DEV)
        idx = b[:, :-1]
        x = block2_input(idx).float().reshape(-1, D)
        ids = idx.reshape(-1)
...
a2 = m.transformer.h[2].attn
with torch.no_grad():
    Vv0 = a0.c_v(F.rms_norm(wte, (D,))).view(V, NH, HD).float()
    xn = F.rms_norm(shrunk, (D,))
    v_new = a2.c_v(xn).view(V, NH, HD).float()
    vs = (1 - a2.lamb) * v_new + a2.lamb * Vv0.view_as(v_new)
del shrunk, xn, v_new
torch.cuda.empty_cache()

# layer-2 head-value spectra from the mean-residual table: S[h][t,c] = <basis_hc, c_proj2_h vs_h(t)>
S = []
for hh in range(NH):
...
del vs
torch.cuda.empty_cache()
print("layer-2 spectra built (mean-residual table)", flush=True)
...
def forward_spec(idx, spectra):
    """layer-2 attention writes replaced by pattern-weighted SPECTRA (exact or coded); rest real."""
    B, T2 = idx.shape
    x0 = F.rms_norm(m.transformer.wte(idx), (D,)); x = None; v1 = None
    ...
    for li in range(NL):
        ...
        q, k, q2, k2 = qk(a.c_q), qk(a.c_k), qk(a.c_q2), qk(a.c_k2)
        pat = ((torch.einsum('bqhd,bkhd->bhqk', q, k)/HD)*(torch.einsum('bqhd,bkhd->bhqk', q2, k2)/HD)).masked_fill(~mask, 0.0)
        if li == 2 and spectra is not None:
...
print(f"SUMMARY L2 CONTENT: class-nameable {codable}/{NH*NC} (median R2 {np.median(r2s):.3f}); "
      f"class gate +{res['gate_coded_minus_exact']['dCE']:.5f}; spike {spikey}/{NH*NC} gate +{res['spike']['gate_dCE']:.5f} nats/tok", flush=True)
json.dump(res, open(f'{QK}/qk_l2_content_gate.json', 'w'), indent=2)
json.dump(results, open(f'{QK}/qk_l2_content_gate_table.json', 'w'), indent=2)
print("QK L2 CONTENT GATE DONE", flush=True)
