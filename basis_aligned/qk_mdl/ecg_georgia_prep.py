"""ECG STAGE 2 prep: load the Georgia (US) cohort as an INDEPENDENT cross-country
...
feature found on German PTB-XL (Stage 1) generalizes to a US cohort.
"""
import os, sys, glob
import numpy as np
import wfdb
...
G = '/workspace/tensor_language/ecg_data/georgia'
...
print(f'{len(recs)} Georgia records', flush=True)
...
np.save(f'{OUT}/georgia_X.npy', X)
np.save(f'{OUT}/georgia_yCD.npy', yCD)
np.save(f'{OUT}/georgia_yNORM.npy', yNORM)
print(f'saved {len(X)} records | CD prevalence {yCD.mean():.3f} | NORM prevalence {yNORM.mean():.3f}', flush=True)
print('GEORGIA PREP DONE', flush=True)
