"""TICK 256: layer-5 port (depth-decay point 6); derived from tick 254.
...
pushed through block 2's own q/k projections into factor tables; patch layer-5's
pattern with table scores; audit on the standard 307k set. Calibration: layer-5
...
def block5_input(idx):
    ...
    blk5 = m.transformer.h[5]
    return blk5.lambdas[0] * x + blk5.lambdas[1] * x0
...
with torch.no_grad():
    for i in range(0, N_EST, 4):
        b = COOC[i:i + 4].to(DEV)
        idx = b[:, :-1]
        x = block5_input(idx).float().reshape(-1, D)
        ids = idx.reshape(-1)
...
out['l5_token_tables_dce'] = round(ce_tab - BASE, 5)
print(f'layer-5 token-table pattern: dCE {ce_tab - BASE:+.5f}', flush=True)
json.dump(out, open(f'{QK}/qk_l5_port.json', 'w'), indent=2)
ce_zero = audit(patch_zero_layer)
out['l5_zero_dce'] = round(ce_zero - BASE, 5)
print(f'layer-5 pattern zeroed: dCE {ce_zero - BASE:+.5f}', flush=True)
json.dump(out, open(f'{QK}/qk_l5_port.json', 'w'), indent=2)
for h in range(NH):
    ce_h = audit(patch_zero_head(h))
    out[f'l5_h{h}_zero_dce'] = round(ce_h - BASE, 5)
    print(f'layer-5 head {h} zeroed: dCE {ce_h - BASE:+.5f}', flush=True)
    json.dump(out, open(f'{QK}/qk_l5_port.json', 'w'), indent=2)
print('L5 PORT DONE', flush=True)
