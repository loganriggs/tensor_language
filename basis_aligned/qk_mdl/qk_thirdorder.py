"""TICK 226: THIRD-ORDER RUNG — add explicit low-rank trilinear terms over close
offset triples (a<b<c<=4, 10 triples, rank 4) jointly trained with the rank-8 pairwise
model. If val R2 clears ~0.35, interaction order is the right ladder; if the gain is
marginal, the remaining computation is higher-order still / entangled, and the ladder
report goes to Logan with the saturation curve.

Original:
"""
"""TICK 225: CONTROLLED pairwise scaling — 45 pairs (offsets<=8) at rank 8, 2x data
(512 docs, 262k samples), weight decay 1e-4, longer schedule. Isolates rank scaling
from the tick-224 confound (more pairs + more params + same data). If val R2 beats
0.245, second-order has headroom; if not, saturation is real at ~0.25.
...
TRIPLES = [(a, b, c) for a in range(5) for b in range(a + 1, 5) for c in range(b + 1, 5)]
...
class PairwiseModel(nn.Module):
    def __init__(self, npairs, rank=8, dout=576):
        ...
        nt = len(TRIPLES)
        self.TA = nn.Parameter(torch.randn(nt, 96, 4) * 0.05)
        self.TB = nn.Parameter(torch.randn(nt, 96, 4) * 0.05)
        self.TC = nn.Parameter(torch.randn(nt, 96, 4) * 0.05)
        self.TO = nn.Parameter(torch.randn(nt, 4, dout) * 0.02)

    def forward(self, ea_list, eb_list, et_list=None):
        ...
        if et_list is not None:
            for ti in range(len(TRIPLES)):
                fa = et_list[ti][0] @ self.TA[ti]
                fb = et_list[ti][1] @ self.TB[ti]
                fc = et_list[ti][2] @ self.TC[ti]
                outp = outp + (fa * fb * fc) @ self.TO[ti]
        return outp
...
opt = torch.optim.Adam(mod.parameters(), lr=2e-3, weight_decay=1e-4)
g = torch.Generator().manual_seed(0)
EWg = EW.to(DEV)
for step in range(9000):
    ...
    et = [(EWg[SH[a][bi.cpu()].to(DEV)], EWg[SH[b][bi.cpu()].to(DEV)],
           EWg[SH[c][bi.cpu()].to(DEV)]) for a, b, c in TRIPLES]
    loss = F.mse_loss(mod(ea, eb, et), Y[bi])
    opt.zero_grad()
    loss.backward()
...
with torch.no_grad():
    ...
    for i in range(0, len(vb), 8192):
        ...
        et = [(EWg[SH[a][sel.cpu()].to(DEV)], EWg[SH[b][sel.cpu()].to(DEV)],
               EWg[SH[c][sel.cpu()].to(DEV)]) for a, b, c in TRIPLES]
        preds.append(mod(ea, eb, et))
...
json.dump(out, open(f'{QK}/qk_thirdorder.json', 'w'), indent=2)
...
def audit_pairwise(batch=4):
    ...
    for i in range(0, len(FINEWEB), batch):
        ...
        et = [(EWg[sh[a]], EWg[sh[b2]], EWg[sh[c]]) for a, b2, c in TRIPLES]
        pred = mod(ea, eb, et)                                  # (B*T, 576)
        dt = m.transformer.wte.weight.dtype
        x = m.transformer.wte(idx)
        x = F.rms_norm(x, (x.size(-1),))
        x0 = x
