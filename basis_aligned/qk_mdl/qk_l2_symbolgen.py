"""TICK 250: symbol-generated layer-2 pattern — the compositional recursion test.
...
def blocks01(idx):
    ...
    blk2 = m.transformer.h[2]
    return yhs[0], yhs[1], blk2.lambdas[0] * x + blk2.lambdas[1] * x0
...
def codes_of(idx, y0, y1):
    ...
    c1 = torch.cat([(y1[..., h, :].reshape(-1, HD) - mu1[h].float()) @ PB[h]
                    for h in range(NH)], 1)
    return torch.cat([ce, c0, c1], 1)                      # (B*T, 384)
...
with torch.no_grad():
    for i in range(0, 512, 4):
        b = COOC[i:i + 4].to(DEV)
        y0, y1, x2 = blocks01(b[:, :-1])
        Cd = codes_of(b[:, :-1], y0, y1).double()
...
with torch.no_grad():
    for i in range(0, len(FINEWEB), 4):
        ...
        n += b[:, 1:].numel()
ce = tot / n
out = {'decoder_R2': round(R2, 4), 'l2_symbolgen_dce': round(ce - BASE, 5),
       'gates': {'tables': 0.02777, 'zero': 0.39042}}
print(f'layer-2 symbol-generated pattern: dCE {ce - BASE:+.5f} '
      f'(tables +0.02777, zero +0.39042)', flush=True)
json.dump(out, open(f'{QK}/qk_l2_symbolgen.json', 'w'), indent=2)
print('L2 SYMBOLGEN DONE', flush=True)
