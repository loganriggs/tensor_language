"""TICK 227 (metric-guided, free rung): PATTERN-AWARE LOSS for the explicit pairwise
object. Tick 212 proved per-map errors couple through the score product (single-map
oracle repairs backfire); yet all explicit objects so far trained with per-coordinate
MSE. Single change: add a product-space loss — for sampled in-window pairs (i, t=i-d),
penalize the error of the predicted pattern product s1*s2 (factors reconstructed from
predicted adapter coords vs true coords; pre-rotary surrogate; audit remains ground
truth). If cancellation structure is what the MSE fits miss, this buys fidelity at
ZERO added bits.

Original tick-225 header:
"""
...
# ---- factor-space reconstruction pieces for the pattern loss ----
BASE_CH, U_CH = {}, {}
for name, _ in MAPS:
    for h in range(NH):
        BASE_CH[(name, h)] = (TABLES[name][:, h] + MUB[(name, h)][None, :]).contiguous()
        U_CH[(name, h)] = UB[(name, h)]
CH_SLICE = {}
for mi, (name, _) in enumerate(MAPS):
    for h in range(NH):
        b0 = mi * NH * R_AD + h * R_AD
        CH_SLICE[(name, h)] = (b0, b0 + R_AD)


def factors_from_coords(tokids, coords, name, h):
    a, b = CH_SLICE[(name, h)]
    return BASE_CH[(name, h)][tokids] + coords[:, a:b] @ U_CH[(name, h)].T


def pattern_err(tok_i, tok_t, c_i_pred, c_t_pred, c_i_true, c_t_true):
    e = 0.0
    for h in range(NH):
        q1p = factors_from_coords(tok_i, c_i_pred, 'q1', h)
        k1p = factors_from_coords(tok_t, c_t_pred, 'k1', h)
        q2p = factors_from_coords(tok_i, c_i_pred, 'q2', h)
        k2p = factors_from_coords(tok_t, c_t_pred, 'k2', h)
        q1t = factors_from_coords(tok_i, c_i_true, 'q1', h)
        k1t = factors_from_coords(tok_t, c_t_true, 'k1', h)
        q2t = factors_from_coords(tok_i, c_i_true, 'q2', h)
        k2t = factors_from_coords(tok_t, c_t_true, 'k2', h)
        pp = (q1p * k1p).sum(1) * (q2p * k2p).sum(1) / (HD * HD)
        pt = (q1t * k1t).sum(1) * (q2t * k2t).sum(1) / (HD * HD)
        e = e + ((pp - pt) ** 2).mean()
    return e / NH


DOCPOS = torch.arange(N) % 511
...
for step in range(9000):
    ok = False
    while not ok:
        bi_cpu = torch.randint(0, ntr, (3072,), generator=g)
        ok = True
    d_off = torch.randint(1, 9, (3072,), generator=g)
    valid = DOCPOS[bi_cpu] >= d_off
    bi_cpu = torch.where(valid, bi_cpu, bi_cpu - bi_cpu % 511 + 16)
    ti_cpu = bi_cpu - d_off
    bi = bi_cpu.to(DEV)
    ti = ti_cpu.to(DEV)

    def predict(sel_cpu):
        ea = [EWg[SH[a][sel_cpu].to(DEV)] for a, b in PAIRS]
        eb = [EWg[SH[b][sel_cpu].to(DEV)] for a, b in PAIRS]
        return mod(ea, eb)

    ci = predict(bi_cpu)
    ct = predict(ti_cpu)
    loss_mse = 0.5 * (F.mse_loss(ci, Y[bi]) + F.mse_loss(ct, Y[ti]))
    tok_i = IDS_ALL[bi_cpu].to(DEV) if 'IDS_ALL' in dir() else SH[0][bi_cpu].to(DEV)
    tok_t = SH[0][ti_cpu].to(DEV)
    lp = pattern_err(SH[0][bi_cpu].to(DEV), tok_t, ci, ct, Y[bi], Y[ti])
    ema_p = 0.99 * ema_p + 0.01 * float(lp)
    lam_p = 0.0 if step < 1000 else 0.5
    loss = loss_mse + lam_p * lp / max(ema_p, 1e-12)
    opt.zero_grad()
    loss.backward()
...
json.dump(out, open(f'{QK}/qk_pairwise_patloss.json', 'w'), indent=2)
