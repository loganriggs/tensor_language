"""TICK 247: LAYER 2 OPENED (Logan 2026-07-26). Baseline port: how much of
layer-2's pattern function is token identity?
...
pushed through block 2's own q/k projections into factor tables; patch layer-2's
pattern with table scores; audit on the standard 307k set. Calibration: layer-2
...
out['l2_token_tables_dce'] = round(ce_tab - BASE, 5)
print(f'layer-2 token-table pattern: dCE {ce_tab - BASE:+.5f}', flush=True)
json.dump(out, open(f'{QK}/qk_l2_port.json', 'w'), indent=2)
ce_zero = audit(patch_zero_layer)
out['l2_zero_dce'] = round(ce_zero - BASE, 5)
print(f'layer-2 pattern zeroed: dCE {ce_zero - BASE:+.5f}', flush=True)
json.dump(out, open(f'{QK}/qk_l2_port.json', 'w'), indent=2)
for h in range(NH):
    ce_h = audit(patch_zero_head(h))
    out[f'l2_h{h}_zero_dce'] = round(ce_h - BASE, 5)
    print(f'layer-2 head {h} zeroed: dCE {ce_h - BASE:+.5f}', flush=True)
    json.dump(out, open(f'{QK}/qk_l2_port.json', 'w'), indent=2)
print('L2 PORT DONE', flush=True)
