"""MED PHASE 7 (generality): foldable bilinear ViT on BloodMNIST (8-class cells)."""
import sys, json
import numpy as np
import torch
...
for step in range(12000):
    ...
    if (step + 1) % 1500 == 0:
        ...
        print(f'step {step+1}: loss {loss.item():.3f} val {va:.4f} (best {best:.4f})',
              flush=True)
ckpt = torch.load(f'{QK}/med_blood.pt', map_location=DEV)
...
json.dump(out, open(f'{QK}/med_blood.json', 'w'), indent=2)
print('MED BLOOD TRAIN DONE', flush=True)
