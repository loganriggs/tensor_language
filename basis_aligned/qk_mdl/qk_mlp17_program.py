"""Layer-17 MLP explicit program (bottom-up ladder; same TN-native family that cracked MLP0/1):
...
ARMS = {}
for name, R in [('table_prev', 0), ('table_prev_R256', 256), ('table_prev_R512', 512)]:
    if R == 0: ARMS[name] = (None, None); continue
    A, U, fvu = fit(R); ARMS[name] = (A, U); print(f"{name}: FVU {fvu:.3f}", flush=True)
torch.save({k: v for k, v in ARMS.items() if v[0] is not None}, f'{QK}/qk_mlp17_program.pt')
...
res = {'floor': FLOOR}
for arm in ['table_prev', 'table_prev_R256', 'table_prev_R512']:
    d = audit(arm) - SUBBASE
...
json.dump(res, open(f'{QK}/qk_mlp17_program.json', 'w'), indent=2)
