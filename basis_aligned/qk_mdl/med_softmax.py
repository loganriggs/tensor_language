"""MED PHASE 11: standard softmax+GELU ViT control (isolates what breaks folding)."""
import sys, json
import numpy as np
import torch
...
class BilinAttn(nn.Module):
    ...
    def forward(self, x):
        B, T, _ = x.shape
        h = F.rms_norm(x, (D,))
        q = self.q(h).view(B, T, NH, HD)
        k = self.k(h).view(B, T, NH, HD)
        v = self.v(h).view(B, T, NH, HD)
        s = torch.einsum('bqhd,bkhd->bhqk', q, k) / (HD ** 0.5)
        pat = s.softmax(-1)                                   # STANDARD softmax attention
...
class BilinMLP(nn.Module):
    ...
    def forward(self, x):
        h = F.rms_norm(x, (D,))
        return x + self.Dn(F.gelu(self.L(h)))
...
for step in range(12000):
    ...
    if (step + 1) % 1500 == 0:
        ...
        print(f'step {step+1}: loss {loss.item():.3f} val {va:.4f} (best {best:.4f})',
              flush=True)
ckpt = torch.load(f'{QK}/med_softmax.pt', map_location=DEV)
...
json.dump(out, open(f'{QK}/med_softmax.json', 'w'), indent=2)
print('MED SOFTMAX DONE', flush=True)
