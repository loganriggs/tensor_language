...
FLOOR6 = None  # measured below (18-MLP joint floor)
LMAX = 17; PER = 64; K6 = 576
...
# measure the 18-MLP joint floor (mean-input at every MLP interface simultaneously)
@torch.no_grad()
def floor18():
    s_ = [torch.zeros(D, device=DEV, dtype=torch.float64) for _ in range(LMAX+1)]; nn = 0
    for i in range(0, 32, 8):
        ...
        for li in range(NL):
            ...
            hin = F.rms_norm(x, (D,))
            s_[li] += hin.reshape(-1, D).double().sum(0)
            x = x + blk.mlp(hin)
        nn += B*T
    MU = [F.rms_norm((si/nn).float(), (D,)) for si in s_]
    tot = 0.0; n = 0
    for i in range(0, len(FINEWEB), 4):
        ...
        for li in range(NL):
            ...
            q, k, q2, k2 = qk(a.c_q), qk(a.c_k), qk(a.c_q2), qk(a.c_k2)
            pat = ((torch.einsum('bqhd,bkhd->bhqk', q, k)/HD)*(torch.einsum('bqhd,bkhd->bhqk', q2, k2)/HD)).masked_fill(~mask, 0.0)
            x = x + a.c_proj(torch.einsum('bhqk,bkhd->bqhd', pat, v).reshape(B, T2, -1))
            x = x + blk.mlp(MU[li].expand(B, T2, D))
...
f18 = floor18() - SUBBASE
res = {'causal18_untrained_dCE': round(d_a, 5), 'floor18_dCE': round(f18, 5),
       'frac': round(1 - d_a/f18, 4)}
print(f"CAUSAL 18-MLP chain, untrained: +{d_a:.4f} vs 18-MLP floor +{f18:.4f} -> {1-d_a/f18:.1%}", flush=True)
json.dump(res, open(f'{QK}/qk_joint_causal18.json', 'w'), indent=2)
print("QK JOINT CAUSAL18 DONE", flush=True)
