"""Logan's method D composed, window form (guided by SI-1 + C-1): at EVERY
...
del acc
torch.save(TABLES, f'{QK}/stream_tables.pt')
print('creation-time stream tables built + saved', flush=True)
...
def vq_tables(k):
    ...
    for nm, t in TABLES.items():
        ...
        C = X[torch.randperm(len(X), generator=g)[:k].to(DEV)].clone()
        for _ in range(10):
...
FULL = dict(TABLES)
for W, name in ((4, 'W=4 window'), (5, 'W=5 window'), (6, 'W=6 window')):
    d = audit_ce(W=W) - base
    res['arms'][name] = d
...
for k in (1024, 256):
    vq = vq_tables(k)
    TABLES.clear(); TABLES.update(vq)
    for W in (3, 4):
        d = audit_ce(W=W) - base
        res['arms'][f'W={W} + vq{k} stream tables'] = d
        print(f'D-composed W={W} vq{k} tables: dCE {d:+.4f}', flush=True)
        with open(OUT, 'w') as fh:
            json.dump(res, fh, indent=2)
    TABLES.clear(); TABLES.update(FULL)
print('d composed 2 done', flush=True)
