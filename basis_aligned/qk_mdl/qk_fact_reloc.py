"""TICK 246: fact relocation — is the late fetch content- or position-addressed?
...
def run_p(idx, patch=None):
    ...
    for li, blk in enumerate(m.transformer.h):
        ...
        if patch and patch[0] == 'resid_add2' and li == patch[1]:
            x = x.clone()
            for pi_, pos_ in enumerate(patch[2]):
                x[:, pos_] = x[:, pos_] + patch[3][pi_].to(x.dtype)
...
for (di, p, j) in sel:
    ...
    # erased baseline
    lg_e, _ = run_p(idx_c, patch=('resid_add', L_ERASE, [j], -delta))
    lp_er = float(F.log_softmax(lg_e[0, p].float(), 0)[nxt[p]])
    lp_cl = float(lp_all_c[p])
    den = lp_cl - lp_er
    if den < 1.0:
        continue
    # relocation targets: adjacent, -4, distant (p-16), all >= 1 and != j, < p
    cands = {'reloc_j_minus_1': j - 1, 'reloc_j_minus_4': j - 4,
             'reloc_p_minus_16': p - 16}

    def rec2(patch2):
        lg2, _ = run_p(idx_c, patch=patch2)
        return (float(F.log_softmax(lg2[0, p].float(), 0)[nxt[p]]) - lp_er) / den

    for name, jp in cands.items():
        if jp < 1 or jp == j or jp >= p:
            continue
        # erase at j AND inject at jp in one pass: positions [j, jp] with vecs
        lg2, _ = run_p(idx_c, patch=('resid_add2', L_ERASE, [j, jp],
                                     torch.stack([-delta, delta])))
        agg.setdefault(name, []).append(
            (float(F.log_softmax(lg2[0, p].float(), 0)[nxt[p]]) - lp_er) / den)
    # inject-only at j-4 without erasing (duplicate fact): change vs clean
    jp = j - 4
    if jp >= 1 and jp != j:
        lg2, _ = run_p(idx_c, patch=('resid_add', L_ERASE, [jp], delta))
        agg.setdefault('duplicate_change', []).append(
            float(F.log_softmax(lg2[0, p].float(), 0)[nxt[p]]) - lp_cl)
out = {k: {'mean': round(float(np.mean(v)), 3), 'median': round(float(np.median(v)), 3),
           'n': len(v)} for k, v in agg.items() if v}
print(json.dumps(out, indent=1), flush=True)
json.dump(out, open(f'{QK}/qk_fact_reloc.json', 'w'), indent=2)
print('FACT RELOC DONE', flush=True)
