"""SQRD12 RULEBOOKS: is block-sparse same-kind selection model-general?
...
for Bk in (2048, 512):
    for name, layers in [('L3 only', [3]), ('L8 only', [8]), ('ALL layers', list(range(NL)))]:
        d = sweep(keep=kmask(Bk, layers)) - base
        res['arms'][f'{name} top-{Bk}'] = round(d, 4)
        print(f'{name} top-{Bk} ({Bk/65536:.1%}): dCE {d:+.4f}', flush=True)
        with open(OUT, 'w') as fh:
            json.dump(res, fh, indent=2)

# name the top blocks of one head for the rulebook flavor check
def exemplars(c, k=4):
...
tops = []
for hh in range(NH):
    v_, i_ = energy[5 if NL > 5 else 0, hh].view(-1).topk(3)
    for vv, ii in zip(v_.tolist(), i_.tolist()):
        cq, ck = ii // 256, ii % 256
        tops.append(f'L5H{hh}: {exemplars(cq)} -> {exemplars(ck)} ({vv:.1f})')
res['sample_blocks_L5'] = tops
with open(OUT, 'w') as fh:
    json.dump(res, fh, indent=2)
for t in tops[:8]:
    print(t, flush=True)
print('sqrd12 rulebook done', flush=True)
