...
for r in (256, 64, 16, 4, 0):
    proj = evecs[:, :r].T.float().contiguous() if r > 0 else torch.zeros(1, D, device=DEV)
    ce = audit(proj)
    out[f'rank{r}_dce'] = round(ce - BASE, 5)
    print(f'mlp1 rank {r}: dCE {ce - BASE:+.5f}', flush=True)
    json.dump(out, open(f'{QK}/qk_mlp1_rank.json', 'w'), indent=2)

# ---- Part B: strong-key selectivity under rank-64 truncation ----
...
with torch.no_grad():
    ...
    for di in docs_needed:
        ...
        lg_t = forward_proj(idx, proj64)[0].float()
        lp_full[di] = F.log_softmax(lg_f, -1)
        lp_tr[di] = F.log_softmax(lg_t, -1)
    for (di, p), kd in zip(POS[:96], KEY):
        tgt = int(SEQS[di, p + 1])
        d = float(lp_full[di][p, tgt] - lp_tr[di][p, tgt])
        (strong if kd > 1.0 else weak).append(d)
out['strongkey_rank64_drop'] = {'mean': round(float(np.mean(strong)), 3),
                                'median': round(float(np.median(strong)), 3), 'n': len(strong)}
if weak:
    out['weakkey_rank64_drop'] = {'mean': round(float(np.mean(weak)), 3),
                                  'median': round(float(np.median(weak)), 3), 'n': len(weak)}
print(json.dumps({k: out[k] for k in out if 'key' in k}, indent=1), flush=True)
json.dump(out, open(f'{QK}/qk_mlp1_rank.json', 'w'), indent=2)
print('MLP1 RANK DONE', flush=True)
