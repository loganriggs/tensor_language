...
## 6. Follow-up lanes (23 September)

AJ's branch-specific variant (exact replication + closed form) is in section 7 of the for_logan note above; the symmetric, deterministic DCT lane on bilin18 (five rungs: exact interaction forms, per-context rank and consistency, fixed head-form dictionary, exact mechanism budget, block-level location) is written up in `basis_aligned/polynomial_causal/explanations/for_logan/research_update_2026-09-23_symmetric_dct.md`, with plans in `plans/`, runners `scripts/run_symmetric_dct_v1–5.py` and `scripts/run_branch_dct_v1.py`, receipts in `results/`.
